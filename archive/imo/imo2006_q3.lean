...
le_of_pow_le_pow _ (mul_nonneg (sqrt_nonneg _) (sq_nonneg _)) nat.succ_pos' $
 ...
 by rw [mul_pow]; rw [ sq_abs]; rw [ hz]; ring
 ... ≤ 32 * ((2 * (x^2 + y^2 + (x + y)^2) + 2 * s^2)^4 / 4^4) :
 mul_le_mul_of_nonneg_left this zero_lt_32.le
 ... = (sqrt 2 * (x^2 + y^2 + z^2 + s^2)^2)^2 :
 by rw [mul_pow]; rw [ sq_sqrt zero_le_two]; rw [ hz]; rw [ ←pow_mul]; rw [ ←mul_add]; rw [ mul_pow]; rw [ ←mul_comm_div]; rw [ ←mul_assoc]; rw [ show 32 / 4 ^ 4 * 2 ^ 4 = (2 : ℝ)]; rw [ by norm_num]; rw [ show 2 * 2 = 4]; rw [ by refl]
...
begin
 ...
 { rw [div_mul_eq_mul_div]; rw [ le_div_iff' zero_lt_32],
...
begin
 ...
 rw [lhs_identity] at this; rw [ h₁] at this; rw [ h₂] at this; rw [ sq_sqrt zero_le_two] at this; rw [ abs_neg] at this; rw [ abs_eq_self.mpr] at this; rw [ ← div_le_iff] at this,
