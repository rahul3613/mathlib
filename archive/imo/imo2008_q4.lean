...
by rw [← pow_left_inj (abs_nonneg x) zero_le_one (pos_iff_ne_zero.2 hn)]; rw [ one_pow]; rw [ pow_abs]; rw [ h]; rw [ abs_one]
...
begin
 ...
 rw [h w hw]; rw [ h x hx]; rw [ h (y ^ 2) (pow_pos hy 2)]; rw [ h (z ^ 2) (pow_pos hz 2)] },
 { intros w x y z hw hx hy hz hprod,
 rw [h w hw]; rw [ h x hx]; rw [ h (y ^ 2) (pow_pos hy 2)]; rw [ h (z ^ 2) (pow_pos hz 2)],
 ...
 { intros x hx,
 have h1xss : 1 * x = (sqrt x) * (sqrt x), { rw [one_mul]; rw [ mul_self_sqrt (le_of_lt hx)] },
