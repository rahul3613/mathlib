...
begin
 ...
 use [i.succ, h_eq],
 intros y hy,
 rw [←fin.pred_inj]; rw [ fin.pred_succ],
 { apply h_uni,
...
begin
 ...
 all_goals
 { specialize h q,
 rw [ε] at h <|>; rw [ show q 0 = tt] at h <|>; rw [ from rfl] at h <|>; rw [ cond_tt] at h <|>
 rw [ε] at h; rw [ show q 0 = ff] at h; rw [ from rfl] at h; rw [ cond_ff] at h,
...
have module.rank ℝ (V n) = (2^n : ℕ),
 by { rw [rank_eq_card_basis (dual_bases_e_ε _).basis]; rw [ Q.card]; apply_instance },
...
begin
 ...
 convert hdW,
 rw [← (dual_bases_e_ε _).coe_basis]; rw [ cardinal.mk_image_eq (dual_bases_e_ε _).basis.injective]; rw [ cardinal.mk_fintype] },
...
begin
 ...
 s * |ε q y|
 = |ε q (s • y)| :
 by rw [map_smul]; rw [ smul_eq_mul]; rw [ abs_mul]; rw [ abs_of_nonneg (real.sqrt_nonneg _)]
 ...
 λ (i : Q (m + 1)), e i) i)| :
 by erw [(f $ m + 1).map_finsupp_total]; erw [ (ε q).map_finsupp_total]; erw [ finsupp.total_apply]
 ...
 by rw [finset.sum_const]; rw [ nsmul_eq_mul]
