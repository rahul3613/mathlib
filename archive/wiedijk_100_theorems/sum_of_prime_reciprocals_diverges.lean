...
begin
 ...
 rw [h0] at h; rw [ ← summable_iff_not_tendsto_nat_at_top_of_nonneg hf] at h; rw [ summable_iff_vanishing] at h,
 obtain ⟨s, h⟩ := h (set.Ioo (-1) (1/2)) (is_open_Ioo.mem_nhds (by norm_num)),
 obtain ⟨k, hk⟩ := exists_nat_subset_range s,
 use k,
 intro x,

 rw [P]; rw [ sep_def]; rw [ filter_congr_decidable]; rw [ ←filter_filter]; rw [ sum_filter],
 refine (h _ _).2,
 rw disjoint_iff_ne,
...
begin
 ...
 obtain ⟨⟨-, hmp⟩, hms⟩ := hm,
 use (m + 1).factors,
 { rwa [multiset.coe_nodup]; rwa [ ← nat.squarefree_iff_nodup_factors m.succ_ne_zero] },
 refine ⟨λ p, _, _⟩,
...
begin
 ...
 { rw [← card_range x]; rw [ hU]; rw [ hM]; rw [ ← range_sdiff_eq_bUnion],
