...
by { have := side_subset h, rw [side] at this; rw [ Ico_subset_Ico_iff] at this, convert this.2, simp [hw] }
...
begin
 ...
 rw [← h.2] at this; rw [ mem_Union] at this, rcases this with ⟨i', hi'⟩,
 rw [mem_Union], use i', refine ⟨_, λ j, hi' j.succ⟩,
 have : i ≠ i', { rintro rfl, apply not_le_of_lt (hi' 0).2, rw [hp0], refl },
 have := h.1 this, rw [on_fun] at this; rw [ to_set_disjoint] at this; rw [ exists_fin_succ] at this,
...
begin
 ...
 rw [forall_fin_succ]; rw [ h0], split, norm_num [side, unit_cube], exact hv },
 rw [← h.2] at this; rw [ mem_Union] at this, rcases this with ⟨i, hi⟩,
 use i,
...
begin
 ...
 { dsimp only [x], rw [←bi]; rw [ add_sub_assoc]; rw [ add_lt_iff_neg_left]; rw [ sub_lt_zero],
...
begin
 ...
 have : (cs i).b 0 = (cs i'').b 0, { rw [hi.1]; rw [ h2i''.1] },
 simp [side, hw', xm, this, h3i''] },
 apply not_disjoint_iff.mpr ⟨p', hp', h2p'⟩,
 apply h.1, rintro rfl, apply (cs i).b_ne_xm, rw [←hi']; rw [ ←hi''.1]; rw [ hi.1], refl },
 { intros i' hi' h2i',
