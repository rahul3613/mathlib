...
begin
 ...
 { rw [add_comm]; rw [ ← lt_sub_iff_add_lt],
 exact sq_lt.mpr h.2 },
end
...
begin
 ...
 { rw [← sqrt_sq hle]; rw [ ← sqrt_mul]; rw [ sub_mul]; rw [ sqrt_sq hle]; rw [ mul_comm_div]; rw [ div_self (pow_ne_zero 2 hlt.ne')]; rw [ one_mul]; rw [ mul_one],
 simpa [sqrt_sq hle, div_le_one (pow_pos hlt 2)] using sq_le_sq' hx1.le hx2.le },
 field_simp,
 rw [h]; rw [ mul_left_comm]; rw [ ← sq]; rw [ neg_mul_eq_mul_neg]; rw [ mul_div_mul_left (-x^2) _ two_ne_zero]; rw [ add_left_comm]; rw [ div_add_div_same]; rw [ tactic.ring.add_neg_eq_sub]; rw [ div_sqrt]; rw [ two_mul] },
