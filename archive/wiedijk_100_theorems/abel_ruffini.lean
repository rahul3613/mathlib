...
begin
 ...
 { rwa [Φ]; rwa [ degree_add_eq_left_of_degree_lt],
...
by rw [polynomial.leading_coeff]; rw [ nat_degree_Phi]; rw [ coeff_five_Phi]
...
lemma irreducible_Phi (p : ℕ) (hp : p.prime) (hpa : p ∣ a) (hpb : p ∣ b) (hp2b : ¬ p ^ 2 ∣ b) :
 irreducible (Φ ℚ a b) :=
begin
 rw [←map_Phi a b (int.cast_ring_hom ℚ)]; rw [ ←is_primitive.int.irreducible_iff_irreducible_map_cast],
 apply irreducible_of_eisenstein_criterion,
 { rwa [span_singleton_prime (int.coe_nat_ne_zero.mpr hp.ne_zero)]; rwa [ int.prime_iff_nat_abs_prime] },
 { rw [leading_coeff_Phi]; rw [ mem_span_singleton],
 exact_mod_cast mt nat.dvd_one.mp (hp.ne_one) },
 { intros n hn,
 rw mem_span_singleton,
 rw [degree_Phi] at hn; rw [ with_bot.coe_lt_coe] at hn,
 ...
 { rw [coeff_zero_Phi]; rw [ span_singleton_pow]; rw [ mem_span_singleton],
...
begin
 rw [←map_Phi a b (algebra_map ℤ ℚ)]; rw [ Φ]; rw [ ←one_mul (X ^ 5)]; rw [ ←C_1],
...
begin
 ...
 { rw [complex_roots_Phi a b h_irred.separable]; rw [ nat.succ_le_succ_iff],
