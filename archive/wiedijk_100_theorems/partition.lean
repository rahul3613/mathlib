...
begin
 ...
 { rw [mem_range]; rw [ nat.lt_succ_iff]; rw [ ← h],
...
begin
 ext ⟨x₁, x₂⟩,
 simp_rw [equiv.finset_congr_apply, mem_map, equiv.to_embedding, function.embedding.coe_fn_mk, ←equiv.eq_symm_apply],
...
begin
 ...
 -- use the definition.
 rw [cut]; rw [ range_one]; rw [ pi_const_singleton]; rw [ map_singleton]; rw [ function.embedding.coe_fn_mk]; rw [ filter_singleton]; rw [ if_pos]; rw [ singleton_inj],
 { ext, split_ifs; refl },
 rw sum_eq_zero_iff,
 intros x hx,
 apply dif_pos hx,
...
begin
 ext f,
 rw [mem_cut]; rw [ mem_bUnion]; rw [ sum_insert h],
 split,
 { rintro ⟨rfl, h₁⟩,
...
begin
 ...
 intros a s hi ih n,
 rw [cut_insert _ _ _ hi]; rw [ prod_insert hi]; rw [ coeff_mul]; rw [ sum_bUnion],
...
begin
 ...
 simp_rw [coeff_mul, coeff_X_pow, coeff_indicator, boole_mul, sum_ite, filter_filter, sum_const_zero, add_zero, sum_const, nsmul_eq_mul, mul_one, sub_eq_iff_eq_add, zero_add, filter_congr_decidable],
 symmetry,
 split_ifs,
 { suffices :
...
begin
 ...
 dsimp only at *,
 refine ⟨_, _, _⟩,
 { rw [←hp₂]; rw [ ←sum_multiset_count_of_subset p s (λ x hx, hp₄ _ (multiset.mem_to_finset.mp hx))] },
 { intros i hi,
 left,
 ...
 intro a, exact nat.lt_irrefl 0 (hs 0 (hp₁.2 0 a)) },
 { rwa [nat.nsmul_eq_mul] at h ; rwa [ nat.nsmul_eq_mul] at h ; rwa [ mul_left_inj' i.succ_ne_zero] at h } },
 ...
 rwa [← hw₂]; rwa [ nat.mul_div_cancel _ (hs i h)] },
 { exact hc _ h } },
 { intros i hi,
 rw mem_sum at hi,
...
begin
 rw [partial_odd_gf]; rw [ partial_distinct_gf],
 ...
 { rw [ring_hom.map_sub]; rw [ ring_hom.map_pow]; rw [ constant_coeff_one]; rw [ constant_coeff_X]; rw [ zero_pow (2 * m).succ_pos]; rw [ sub_zero],
 exact one_ne_zero },

 calc (∏ i in range (m + 1), (1 - X ^ (2 * i + 1))⁻¹) *
 ∏ i in range (m + 1), (1 - X ^ (m + 1 + i + 1))
 = π₁ * (1 - X ^ (2 * m + 1))⁻¹ * (π₀ * (1 - X ^ (m + 1 + m + 1))) :
 by rw [prod_range_succ _ m]; rw [ ←hπ₁]; rw [ prod_range_succ _ m]; rw [ ←hπ₀]
 ... = π₁ * (1 - X ^ (2 * m + 1))⁻¹ * (π₀ * ((1 + X ^ (m + 1)) * (1 - X ^ (m + 1)))) :
 by rw [←sq_sub_sq]; rw [ one_pow]; rw [ add_assoc _ m 1]; rw [ ←two_mul (m + 1)]; rw [ pow_mul']
 ...
 by rw [two_mul]; rw [ add_right_comm _ m 1]
 ... = (1 - X ^ (2 * m + 1)) * (1 - X ^ (2 * m + 1))⁻¹ * π₂ * (π₁ * (1 + X ^ (m + 1))) :
 by ring
 ... = π₂ * (π₁ * (1 + X ^ (m + 1))) : by rw [power_series.mul_inv_cancel _ h]; rw [ one_mul]
 ... = π₁ * π₂ * (1 + X ^ (m + 1)) : by ring
 ... = π₃ * (1 + X ^ (m + 1)) : by rw ih
