...
begin
 rw [pow_succ]; rw [ mul_eq_mul]; rw [ adj_matrix_mul_apply]; rw [ degree]; rw [ card_eq_sum_ones]; rw [ nat.cast_sum],
 apply sum_congr rfl,
 intros x hx,
 rw [adj_matrix_sq_of_ne _ hG]; rw [ nat.cast_one],
 rintro ⟨rfl⟩,
...
begin
 rw [← nat.cast_id (G.degree v)]; rw [ ← nat.cast_id (G.degree w)]; rw [ ← adj_matrix_pow_three_of_not_adj ℕ hG hvw]; rw [ ← adj_matrix_pow_three_of_not_adj ℕ hG (λ h, hvw (G.symm h))],
...
begin
 ...
 { rw [h]; rw [ sq]; rw [ mul_eq_mul]; rw [ adj_matrix_mul_self_apply_self]; rw [ hd], simp, },
 { rw [adj_matrix_sq_of_ne R hG h]; rw [ if_neg h], },
end
...
begin
 ...
 injection h', },
 apply hxy',
 rw [key ((mem_common_neighbors G).mpr ⟨hvx, G.symm hxw⟩)]; rw [ key ((mem_common_neighbors G).mpr ⟨hvy, G.symm hcontra⟩)],
end
...
begin
 ...
 { rw [adj_matrix_sq_of_regular hG hd]; rw [ mul_vec]; rw [ dot_product]; rw [ ← insert_erase (mem_univ v)],
 ...
 rw [finset.sum_const_nat]; rw [ card_erase_of_mem (mem_univ v)]; rw [ mul_one], { refl },
 intros x hx, simp [(ne_of_mem_erase hx).symm], },
 { rw [sq]; rw [ mul_eq_mul]; rw [ ← mul_vec_mul_vec],
...
by rw [adj_matrix_sq_mul_const_one_of_regular hd]; rw [ dmod]
...
begin
 ...
 rw [trace_adj_matrix]; rw [ zero_pow (fact.out p.prime).pos],
 ...
 rw [Vmod]; rw [ ← nat.cast_one]; rw [ zmod.nat_coe_zmod_eq_zero_iff_dvd]; rw [ nat.dvd_one]; rw [ nat.min_fac_eq_one_iff],
 linarith,
end
...
begin
 ...
 { exact zero_le _, },
 { have : n = 0,
 { rw [nat.succ_sub_succ_eq_sub] at h; rw [ tsub_zero] at h,
 linarith },
 subst n, } },
...
begin
 ...
 intro x,
 rw [mem_neighbor_finset]; rw [ finset.mem_erase],
...
begin
 ...
 use v,
 intros w hvw,
 rw [← mem_neighbor_finset]; rw [ neighbor_finset_eq_of_degree_eq_two hG hd v]; rw [ finset.mem_erase],
