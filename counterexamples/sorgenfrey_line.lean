...
begin
 ...
 simp_rw [@infi_comm _ ℝₗ (_ ≤ _), infi_subtype', ← Ici_inter_Iio, ← inf_principal, ← inf_infi, ← infi_inf, this, infi_subtype],
...
by rw [← map_to_real_nhds]; rw [ comap_map to_real.injective]

@[continuity] lemma continuous_to_real : continuous to_real :=
continuous_iff_continuous_at.2 $ λ x,
 by { rw [continuous_at]; rw [ tendsto]; rw [ map_to_real_nhds], exact inf_le_left }
...
begin
 ...
 rwa [← add_le_add_iff_right]; rwa [ hs _ H]; rwa [ add_le_add_iff_left] },
 { refine hy.antisymm _,
 rwa [← add_le_add_iff_left]; rwa [ hs _ H]; rwa [ add_le_add_iff_right] }
end
...
begin
 ...
 { rw [closure_Ioo h₀'.ne]; rw [ left_mem_Icc], exact h₀'.le },
