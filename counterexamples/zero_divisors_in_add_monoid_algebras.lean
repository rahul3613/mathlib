...
begin
 ...
 { rw [mul_sub]; rw [ add_monoid_algebra.single_mul_single]; rw [ add_monoid_algebra.single_mul_single]; rw [ sub_eq_zero]; rw [ add_zero]; rw [ ← succ_nsmul]; rw [ nat.sub_add_cancel (one_le_two.trans n2)]; rw [ na] },
end
...
begin
 ...
 false_implies_iff] },
 { rw [single_pow]; rw [ one_pow]; rw [ zero_smul]; rw [ single_eq_same] } },
 { apply_fun (λ x : add_monoid_algebra R A, x 0),
 ...
 { ext, rw [single_zero_one]; rw [ one_pow]; rw [ mul_one] },
 { rw [single_pow]; rw [ one_pow]; rw [ add_order_of_nsmul_eq_zero]; rw [ single_zero_one]; rw [ one_pow]; rw [ sub_self] },
