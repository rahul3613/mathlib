...
begin
 ...
 apply_fun f at this,
 rwa [f.additive _ _ (empty_disjoint _)] at this; rwa [ self_eq_add_left] at this,
end
...
begin
 ...
 { have : t \ (s₁ ∪ s₂) = (t \ (s₁ ∪ s₂)) \ s₁,
 by rw [diff_diff]; rw [ union_comm]; rw [ union_assoc]; rw [ union_self],
 rw this,
 exact h₁ _ (ht.mono (diff_subset _ _)) },
 { have : t \ (s₁ ∪ s₂) = (t \ (s₁ ∪ s₂)) \ s₂,
 by rw [diff_diff]; rw [ union_assoc]; rw [ union_self],
 rw this,
...
begin
 ...
 rw [this]; rw [ eq_add_parts]; rw [ discrete_part_apply]; rw [ hx]; rw [ ψ.empty]; rw [ zero_add]; rw [ U]; rw [ ψ.continuous_part.additive _ _ disjoint_sdiff_self_right]; rw [ ψ.continuous_part_apply_eq_zero_of_countable _ (countable_compl_spf Hcont x)]; rw [ add_zero],
end
