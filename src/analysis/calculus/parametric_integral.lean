...
begin
 ...
 intros x x_in,
 rw [set.mem_set_of_eq]; rw [ ← norm_smul_of_nonneg (nneg _)]; rw [ integral_smul]; rw [ integral_sub]; rw [ integral_sub]; rw [ ← continuous_linear_map.integral_apply hF'_int],
 exacts [hF_int' x x_in, hF_int, (hF_int' x x_in).sub hF_int,
 hF'_int.apply_continuous_linear_map _] },
 rw [has_fderiv_at_iff_tendsto]; rw [ tendsto_congr' this]; rw [ ← tendsto_zero_iff_norm_tendsto_zero]; rw [ ← show ∫ (a : α)]; rw [ ‖x₀ - x₀‖⁻¹ • (F x₀ a - F x₀ a - (F' a) (x₀ - x₀)) ∂μ = 0]; rw [ by simp],
 ...
 by { rw [norm_smul_of_nonneg]; rw [ norm_smul_of_nonneg] ; exact nneg _}
 ...
 rwa [has_fderiv_at_iff_tendsto] at ha ; rwa [ this] at ha },
end
