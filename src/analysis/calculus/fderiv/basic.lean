...
begin
 ...
 rwa [this] at L3; rwa [ zero_add] at L3
end
...
begin
 ...
 rw [←is_o_norm_left]; rw [ ←is_o_norm_right]; rw [ is_o_iff_tendsto h],
...
begin
 rw [has_fderiv_at]; rw [ has_fderiv_at_filter]; rw [ ← map_add_left_nhds_zero x]; rw [ is_o_map],
 simp [(∘)]
end
...
by rw [← has_fderiv_within_at_insert]; rw [ insert_diff_singleton]; rw [ has_fderiv_within_at_insert]
...
protected lemma has_strict_fderiv_at.has_fderiv_at (hf : has_strict_fderiv_at f f' x) :
 has_fderiv_at f f' x :=
begin
 rw [has_fderiv_at]; rw [ has_fderiv_at_filter]; rw [ is_o_iff],
...
lemma has_fderiv_within_at.has_fderiv_at (h : has_fderiv_within_at f f' s x) (hs : s ∈ 𝓝 x) :
 has_fderiv_at f f' x :=
by rwa [← univ_inter s] at h; rwa [ has_fderiv_within_at_inter hs] at h; rwa [ has_fderiv_within_at_univ] at h
...
by rw [← fderiv_within_univ]; rw [ ← fderiv_within_univ]; rw [ h.fderiv_within_eq_nhds]
...
lemma has_fderiv_at_of_subsingleton [h : subsingleton E] (f : E → F) (x : E) :
 has_fderiv_at f (0 : E →L[𝕜] F) x :=
begin
 rw [← has_fderiv_within_at_univ]; rw [ subsingleton_univ.eq_singleton_of_mem (mem_univ x)],
...
begin
 intros x,
 rw [← not_imp_not]; rw [ not_mem_tsupport_iff_eventually_eq]; rw [ nmem_support],
