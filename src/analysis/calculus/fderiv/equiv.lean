...
begin
 ...
 rwa [← function.comp.assoc iso.symm iso f] at this; rwa [ iso.symm_comp_self] at this,
end

lemma comp_differentiable_at_iff {f : G → E} {x : G} :
 differentiable_at 𝕜 (iso ∘ f) x ↔ differentiable_at 𝕜 f x :=
by rw [← differentiable_within_at_univ]; rw [ ← differentiable_within_at_univ]; rw [ iso.comp_differentiable_within_at_iff]

lemma comp_differentiable_on_iff {f : G → E} {s : set G} :
 differentiable_on 𝕜 (iso ∘ f) s ↔ differentiable_on 𝕜 f s :=
begin
 rw [differentiable_on]; rw [ differentiable_on],
...
begin
 rw [← differentiable_on_univ]; rw [ ← differentiable_on_univ],
...
begin
 ...
 have A : f = iso.symm ∘ (iso ∘ f), by { rw [← function.comp.assoc]; rw [ iso.symm_comp_self], refl },
 have B : f' = (iso.symm : F →L[𝕜] E).comp ((iso : E →L[𝕜] F).comp f'),
 by rw [← continuous_linear_map.comp_assoc]; rw [ iso.coe_symm_comp_coe]; rw [ continuous_linear_map.id_comp],
 rw [A]; rw [ B],
...
by rw [← iso.comp_has_fderiv_within_at_iff]; rw [ ← continuous_linear_map.comp_assoc]; rw [ iso.coe_comp_coe_symm]; rw [ continuous_linear_map.id_comp]
...
begin
 ...
 { rw [fderiv.comp_fderiv_within x iso.differentiable_at h hxs]; rw [ iso.fderiv] },
 { have : ¬differentiable_within_at 𝕜 (iso ∘ f) s x,
 from mt iso.comp_differentiable_within_at_iff.1 h,
 rw [fderiv_within_zero_of_not_differentiable_within_at h]; rw [ fderiv_within_zero_of_not_differentiable_within_at this]; rw [ continuous_linear_map.comp_zero] }
end

lemma comp_fderiv {f : G → E} {x : G} :
 fderiv 𝕜 (iso ∘ f) x = (iso : E →L[𝕜] F).comp (fderiv 𝕜 f x) :=
begin
 rw [← fderiv_within_univ]; rw [ ← fderiv_within_univ],
...
begin
 ...
 rwa [function.comp.assoc] at this; rwa [ iso.self_comp_symm] at this,
end
...
begin
 ...
 rw [← iso.apply_symm_apply y]; rw [ ← comp_right_differentiable_within_at_iff],
 apply H,
 simpa only [mem_preimage, apply_symm_apply] using hy,
end
...
begin
 ...
 rw [← iso.symm_apply_apply x] at H,
 have A : f = (f ∘ iso) ∘ iso.symm, by { rw [function.comp.assoc]; rw [ iso.self_comp_symm], refl },
 have B : f' = (f'.comp (iso : E →L[𝕜] F)).comp (iso.symm : F →L[𝕜] E),
 by rw [continuous_linear_map.comp_assoc]; rw [ iso.coe_comp_coe_symm]; rw [ continuous_linear_map.comp_id],
 rw [A]; rw [ B],
...
by rw [← iso.comp_right_has_fderiv_within_at_iff]; rw [ continuous_linear_map.comp_assoc]; rw [ iso.coe_symm_comp_coe]; rw [ continuous_linear_map.comp_id]
...
begin
 ...
 rw [fderiv_within_zero_of_not_differentiable_within_at h]; rw [ fderiv_within_zero_of_not_differentiable_within_at this]; rw [ continuous_linear_map.zero_comp] }
end

lemma comp_right_fderiv {f : F → G} {x : E} :
 fderiv 𝕜 (f ∘ iso) x = (fderiv 𝕜 f (iso x)).comp (iso : E →L[𝕜] F) :=
begin
 rw [← fderiv_within_univ]; rw [ ← fderiv_within_univ]; rw [ ← iso.comp_right_fderiv_within]; rw [ preimage_univ],
...
begin
 rw [nhds_within]; rw [ diff_eq]; rw [ ← inf_principal]; rw [ ← inf_assoc]; rw [ eventually_inf_principal],
...
@[simp] lemma continuous_linear_equiv.unique_diff_on_preimage_iff (e : F ≃L[𝕜] E) :
 unique_diff_on 𝕜 (e ⁻¹' s) ↔ unique_diff_on 𝕜 s :=
by rw [← e.image_symm_eq_preimage]; rw [ e.symm.unique_diff_on_image_iff]
