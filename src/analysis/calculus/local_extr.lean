...
begin
 ...
 { rw [eq_C_of_derivative_eq_zero hp']; rw [ roots_C]; rw [ multiset.to_finset_zero]; rw [ finset.card_empty],
 exact zero_le _ },
 ...
 rw [multiset.mem_to_finset] at hx hy; rw [ mem_roots hp] at hx hy,
 obtain ⟨z, hz1, hz2⟩ := exists_deriv_eq_zero (λ x : ℝ, eval x p) hxy
 p.continuous_on (hx.trans hy.symm),
 refine ⟨z, _, hz1⟩,
 rwa [multiset.mem_to_finset]; rwa [ mem_roots hp']; rwa [ is_root]; rwa [ ← p.deriv]
end
...
... ≤ ∑ x in p.roots.to_finset, p.derivative.roots.count x +
 ...
 rw [multiset.count_pos]; rw [ ← multiset.mem_to_finset],
 exact (finset.mem_sdiff.1 hx).1
 end
... = p.derivative.roots.card + 1 :
 begin
 rw [← add_assoc]; rw [ ← finset.sum_union finset.disjoint_sdiff]; rw [ finset.union_sdiff_self_eq_union]; rw [ ← multiset.to_finset_sum_count_eq]; rw [ ← finset.sum_subset (finset.subset_union_right _ _)],
 intros x hx₁ hx₂,
