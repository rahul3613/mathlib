...
begin
 ...
 from mem_pos_tangent_cone_at_of_segment_subset (by rw [segment_symm]; rw [ segment_eq_Icc hab]),
 simpa [-sub_nonneg, -continuous_linear_map.map_sub]
 using hc.localize.has_fderiv_within_at_nonneg (hg b (right_mem_Icc.2 hab)) this },
 exact ⟨hac, hcb⟩ },
 use [c, cmem'],
 rw [← sub_eq_zero],
 have : Icc a b ∈ 𝓝 c, by rwa [← mem_interior_iff_mem_nhds]; rwa [ interior_Icc],
