...
begin
 ...
 rw [← int.coe_nat_one]; rw [ ← int.coe_nat_sub]; rw [ zpow_coe_nat],
 norm_cast at hm,
 ...
 rw [sq]; rw [ mul_inv]; rw [ inv_inv]; rw [ int.cast_neg]; rw [ neg_mul]; rw [ neg_mul_neg]; rw [ ← zpow_add₀ hx]; rw [ mul_assoc]; rw [ ← zpow_add₀ hx], congr, abel },
