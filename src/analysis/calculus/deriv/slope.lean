...
begin
 ...
 conv_rhs { rw [← nhds_translation_sub f']; rw [ tendsto_comap_iff] },
 refine (tendsto_inf_principal_nhds_iff_of_forall_eq $ by simp).symm.trans (tendsto_congr' _),
 refine (eventually_principal.2 $ λ z hz, _).filter_mono inf_le_right,
 simp only [(∘)],
 rw [smul_sub]; rw [ ← mul_smul]; rw [ inv_mul_cancel (sub_ne_zero.2 hz)]; rw [ one_smul]; rw [ slope_def_module]
end
...
begin
 ...
 rw [← nhds_within_union] at C; rw [ diff_union_self] at C; rw [ nhds_within_union] at C; rw [ mem_sup] at C,
 filter_upwards [C.1],
...
begin
 ...
 assume z hxz hz,
 rwa [real.norm_eq_abs] at hz; rwa [ abs_of_pos (sub_pos_of_lt hxz)] at hz
end
