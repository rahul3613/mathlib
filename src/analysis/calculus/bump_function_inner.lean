...
(div_eq_one_iff_eq $ (pos_denom x).ne').2 $ by rw [zero_of_nonpos (sub_nonpos.2 h)]; rw [ add_zero]

lemma zero_of_nonpos (h : x ≤ 0) : smooth_transition x = 0 :=
by rw [smooth_transition]; rw [ zero_of_nonpos h]; rw [ zero_div]
...
begin
 ...
 { rw [smooth_transition.zero]; rw [ zero_of_nonpos hx.le] },
 { rw [smooth_transition.one]; rw [ one_of_one_le hx.le] }
end
