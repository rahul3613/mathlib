...
begin
 ...
 ... = M⁻¹ * δ n * ‖iterated_fderiv ℝ i (g n) x‖ :
 by { rw [norm_smul]; rw [ real.norm_of_nonneg], positivity }
 ... ≤ M⁻¹ * δ n * M :
...
begin
 rw [has_compact_support_def]; rw [ u_support]; rw [ closure_ball (0 : E) one_ne_zero],
...
begin
 ...
 by rw [smul_unit_ball Dpos.ne']; rw [ real.norm_of_nonneg Dpos.le],
...
begin
 rw [has_compact_support_def]; rw [ W_support E Dpos]; rw [ closure_ball (0 : E) Dpos.ne'],
 exact is_compact_closed_ball _ _,
end
variable {E}
