...
begin
 ...
 rw [← ennreal.of_real_div_of_pos (sub_pos.2 hxy)]; rw [ ennreal.to_real_of_real],
 ...
 rw [← ennreal.of_real_div_of_pos (sub_pos.2 hxy)]; rw [ ennreal.to_real_of_real]; rw [ ← neg_neg (y - x)]; rw [ div_neg]; rw [ neg_div']; rw [ neg_sub]; rw [ neg_sub],
 ...
 rw [has_deriv_at_iff_tendsto_slope]; rw [ slope_fun_def_field]; rw [ ← nhds_left'_sup_nhds_right']; rw [ tendsto_sup],
 exact ⟨L4, L1⟩
end
...
begin
 ...
 { rw [not_not] at h'x; rw [ hf.continuous_at_iff_left_lim_eq_right_lim] at h'x,
 apply le_antisymm _ (hf.le_right_lim (le_refl _)),
 rw ← h'x,
 exact hf.left_lim_le (le_refl _) },
 rw [has_deriv_at_iff_tendsto_slope] at hx; rw [ (nhds_left'_sup_nhds_right' x).symm] at hx; rw [ tendsto_sup] at hx; rw [ slope_fun_def_field] at hx; rw [ A] at hx,
 ...
 rw [has_deriv_at_iff_tendsto_slope]; rw [ slope_fun_def_field]; rw [ (nhds_left'_sup_nhds_right' x).symm]; rw [ tendsto_sup],
 exact ⟨L2, L1⟩
end
