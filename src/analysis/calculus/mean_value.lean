...
begin
 ...
 rwa [slope_def_field] at this ; rwa [ slope_def_field] at this ; rwa [ div_le_div_right (sub_pos.2 hz.1)] at this ; rwa [ hxB] at this ; rwa [ sub_le_sub_iff_right] at this }
end
...
begin
 ...
 { rwa [sub_self]; rwa [ mul_zero]; rwa [ add_zero] },
...
begin
 ...
 simp only [g, B], rw [sub_self]; rw [ norm_zero]; rw [ sub_self]; rw [ mul_zero]
end
...
begin
 ...
 { rw [← image_subset_iff]; rw [ ← segment_eq_image'],
 apply hs.segment_subset xs ys },
 have : f x = f (g 0), by { simp only [g], rw [zero_smul]; rw [ add_zero] },
 rw this,
 have : f y = f (g 1), by { simp only [g], rw [one_smul]; rw [ add_sub_cancel'_right] },
 rw this,
...
begin
 ...
 { rwa [hfgx] at this ; rwa [ sub_self] at this ; rwa [ eq_comm] at this ; rwa [ sub_eq_zero] at this },
 refine hs.is_const_of_fderiv_within_eq_zero (hf.sub hg) _ hx hy,
 intros z hz,
 rw [fderiv_within_sub (hs' _ hz) (hf _ hz) (hg _ hz)]; rw [ sub_eq_zero]; rw [ hf' _ hz],
end
...
begin
 assume x hx y hy hxy,
 cases eq_or_lt_of_le hxy with hxy' hxy', by rw [hxy']; rw [ sub_self]; rw [ sub_self]; rw [ mul_zero],
 have hxyD : Icc x y ⊆ D, from hD.ord_connected.out hx hy,
 have hxyD' : Ioo x y ⊆ interior D,
 from subset_sUnion_of_mem ⟨is_open_Ioo, subset.trans Ioo_subset_Icc_self hxyD⟩,
 obtain ⟨a, a_mem, ha⟩ : ∃ a ∈ Ioo x y, deriv f a = (f y - f x) / (y - x),
