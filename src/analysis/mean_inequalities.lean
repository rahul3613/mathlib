...
calc (∏ i in s, (z i) ^ (w i)) = ∏ i in s, x ^ w i :
 ...
 { rw [h₀]; rw [ rpow_zero]; rw [ rpow_zero] },
 { rw hx i hi h₀ }
 end
... = x :
 begin
...
calc ∑ i in s, w i * z i = ∑ i in s, w i * x :
 ...
 { rw [hwi]; rw [ zero_mul]; rw [ zero_mul] },
 { rw hx i hi hwi },
 end
... = x : by rw [←sum_mul]; rw [ hw']; rw [ one_mul]

theorem geom_mean_eq_arith_mean_weighted_of_constant (w z : ι → ℝ) (x : ℝ) (hw : ∀ i ∈ s, 0 ≤ w i)
 (hw' : ∑ i in s, w i = 1) (hz : ∀ i ∈ s, 0 ≤ z i) (hx : ∀ i ∈ s, w i ≠ 0 → z i = x) :
 (∏ i in s, (z i) ^ (w i)) = ∑ i in s, w i * z i :=
by rw [geom_mean_weighted_of_constant]; rw [ arith_mean_weighted_of_constant]; assumption
...
begin
 ...
 rw [←coe_to_nnreal h.left]; rw [ ←coe_to_nnreal h.right]; rw [ ←coe_mul]; rw [ coe_rpow_of_nonneg _ hpq.nonneg]; rw [ coe_rpow_of_nonneg _ hpq.symm.nonneg]; rw [ ennreal.of_real]; rw [ ennreal.of_real]; rw [ ←@coe_div (real.to_nnreal p) _ (by simp [hpq.pos]),
...
begin
 ...
 by rw [sum_add_distrib]; rw [ sum_div]; rw [ sum_div]
 ... ≤ 1 / real.to_nnreal p + 1 / real.to_nnreal q :
 by { refine add_le_add _ _,
 { rwa [div_le_iff hp_ne_zero]; rwa [ div_mul_cancel _ hp_ne_zero], },
 { rwa [div_le_iff hq_ne_zero]; rwa [ div_mul_cancel _ hq_ne_zero], }, }
...
begin
 ...
 rwa [div_le_iff] at this; rwa [ one_mul] at this,
 refine mul_ne_zero _ _,
 { rw [ne.def]; rw [ rpow_eq_zero_iff]; rw [ not_and_distrib], exact or.inl hF_zero, },
 { rw [ne.def]; rw [ rpow_eq_zero_iff]; rw [ not_and_distrib], exact or.inl hG_zero, }, },
...
begin
 ...
 { rw [hf.tsum_eq]; rw [ rpow_inv_rpow_self hpq.ne_zero] },
...
begin
 ...
 rw [div_eq_iff]; rw [ ← rpow_add hf]; rw [ hpq.inv_add_inv_conj]; rw [ rpow_one],
...
begin
 ...
 have hA : A = (∑' (i : ι), f i ^ p) ^ (1 / p) := by rw [hf.tsum_eq]; rw [ rpow_inv_rpow_self hp'],
 have hB : B = (∑' (i : ι), g i ^ p) ^ (1 / p) := by rw [hg.tsum_eq]; rw [ rpow_inv_rpow_self hp'],
