...
by rw [← inner_self]; rw [ real_inner_self_eq_norm_mul_norm]

instance : norm_one_class ℍ :=
⟨by rw [norm_eq_sqrt_real_inner]; rw [ inner_self]; rw [ norm_sq.map_one]; rw [ real.sqrt_one]⟩

@[simp, norm_cast] lemma norm_coe (a : ℝ) : ‖(a : ℍ)‖ = ‖a‖ :=
by rw [norm_eq_sqrt_real_inner]; rw [ inner_self]; rw [ norm_sq_coe]; rw [ real.sqrt_sq_eq_abs]; rw [ real.norm_eq_abs]
...
begin
 rw [norm_eq_sqrt_real_inner]; rw [ norm_eq_sqrt_real_inner]; rw [ inner_self]; rw [ norm_sq_def']; rw [ pi_Lp.inner_apply]; rw [ fin.sum_univ_four],
