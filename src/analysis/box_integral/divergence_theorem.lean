...
begin
 ...
 begin
 rw [← integral_sub (Hi _ Hu) (Hi _ Hl)]; rw [ ← box.volume_face_mul i]; rw [ mul_smul]; rw [ ← box.volume_apply]; rw [ ← box_additive_map.to_smul_apply]; rw [ ← integral_const]; rw [ ← box_additive_map.volume]; rw [ ← integral_sub (integrable_const _) ((Hi _ Hu).sub (Hi _ Hl))],
 ...
 ... = 2 * ε * c * ∏ j, (I.upper j - I.lower j) :
 begin
 rw [← measure.to_box_additive_apply]; rw [ box.volume_apply]; rw [ ← I.volume_face_mul i],
 ac_refl
 end
end
...
begin
 ...
 rw [box.coe_eq_pi]; rw [ real.volume_pi_Ioc_to_real (box.lower_le_upper _)],
 ...
 { rw [mul_right_comm (2 : ℝ)]; rw [ ← box.volume_apply],
