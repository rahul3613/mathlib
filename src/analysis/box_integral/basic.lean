...
lemma integral_sum_sub_partitions (f : ℝⁿ → E) (vol : ι →ᵇᵃ (E →L[ℝ] F))
 ...
 vol J (f $ (π₂.inf_prepartition π₁.to_prepartition).tag J)) :=
begin
 rw [← integral_sum_inf_partition f vol π₁ h₂]; rw [ ← integral_sum_inf_partition f vol π₂ h₁]; rw [ integral_sum]; rw [ integral_sum]; rw [ finset.sum_sub_distrib],
...
begin
 rw [integrable_iff_cauchy]; rw [ cauchy_map_iff']; rw [ (l.has_basis_to_filter_Union_top _).prod_self.tendsto_iff uniformity_basis_dist_le],
...
by { rw [integral]; rw [ dif_pos h], exact classical.some_spec h }
...
else by rw [integral]; rw [ integral]; rw [ dif_neg h]; rw [ dif_neg (mt integrable.of_neg h)]; rw [ neg_zero]
...
begin
 ...
 { rw [integral]; rw [ dif_neg hfi]; rw [ norm_zero],
...
begin
 rw [convergence_r] at hπ; rw [ dif_pos h₀] at hπ,
...
begin
 ...
 rw [prod_map_map_eq]; rw [ ← to_filter_inf_Union_eq]; rw [ ← prod_inf_prod]; rw [ prod_principal_principal],
...
{ to_fun := λ J, integral J l f vol,
 ...
 rw [(h.to_subbox (with_top.coe_le_coe.1 hJ)).sum_integral_congr hπ]; rw [ prepartition.top_boxes]; rw [ sum_singleton]
 end }
...
begin
 ...
 rw [← finset.sum_mul]; rw [ μ.to_box_additive.sum_partition_boxes le_top (h₁p.inf h₂p)],
 exact hε.le
end
...
begin
 ...
 rw [← g.sum_partition_boxes le_rfl hπp]; rw [ mem_closed_ball]; rw [ integral_sum]; rw [ ← sum_filter_add_sum_filter_not π.boxes (λ J, π.tag J ∈ s)]; rw [ ← sum_filter_add_sum_filter_not π.boxes (λ J, π.tag J ∈ s)]; rw [ ← add_halves ε],
 ...
 { rw [← mul_sum]; rw [ B.sum_partition_boxes le_rfl hπp]; rw [ mul_comm],
 exact hεI.le }
end
