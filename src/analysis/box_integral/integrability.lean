...
begin
 ...
 rw [dist_eq_norm]; rw [ sub_zero]; rw [ ← integral_sum_fiberwise (λ J, N (π.tag J))],
 ...
 rw [norm_smul]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg ennreal.to_real_nonneg],
 exact mul_le_mul_of_nonneg_left (hJ.2 ▸ nat.le_ceil _) ennreal.to_real_nonneg },
 refine (norm_sum_le_of_le _ this).trans _, clear this,
 rw [← sum_mul]; rw [ ← prepartition.measure_Union_to_real],
 ...
 rw [ennreal.coe_to_real]; rw [ ← nnreal.coe_nat_cast]; rw [ ← nnreal.coe_mul]; rw [ nnreal.coe_le_coe]; rw [ ← ennreal.coe_le_coe]; rw [ ennreal.coe_mul]; rw [ ennreal.coe_nat]; rw [ mul_comm],
...
begin
 ...
 { intros x m n hmn,
 rw [← dist_eq_norm]; rw [ ← dist_eq_norm]; rw [ dist_nndist]; rw [ dist_nndist]; rw [ nnreal.coe_le_coe]; rw [ ← ennreal.coe_le_coe]; rw [ ← edist_nndist]; rw [ ← edist_nndist],
 ...
 rw [← hπp.Union_eq]; rw [ π.to_prepartition.measure_Union_to_real]; rw [ sum_mul]; rw [ integral_sum],
 refine dist_sum_sum_le_of_le _ (λ J hJ, _), dsimp,
 rw [dist_eq_norm]; rw [ ← smul_sub]; rw [ norm_smul]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg ennreal.to_real_nonneg],
 ...
 rw [← π.to_prepartition.sum_fiberwise (λ J, Nx (π.tag J))]; rw [ ← π.to_prepartition.sum_fiberwise (λ J, Nx (π.tag J))],
 refine le_trans _ (nnreal.coe_lt_coe.2 hcε).le,
 ...
 simp [hNxn J hJ] },
 { refine sum_congr rfl (λ J hJ, _),
 rw [← simple_func.integral_eq_integral]; rw [ simple_func.box_integral_eq_integral _ _ _ _ hl]; rw [ hNxn J hJ],
 ...
 rw [← hπp.Union_eq]; rw [ prepartition.Union_def']; rw [ integral_finset_bUnion π.boxes (λ J hJ, J.measurable_set_coe) π.pairwise_disjoint hgi]; rw [ integral_finset_bUnion π.boxes (λ J hJ, J.measurable_set_coe) π.pairwise_disjoint (hfgi _)],
