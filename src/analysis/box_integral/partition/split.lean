...
begin
 ...
 rw [and_comm (y i ≤ x)]; rw [ pi.le_def]
end
...
begin
 rw [split_lower]; rw [ mk'_eq_bot]; rw [ exists_update_iff I.upper (λ j y, y ≤ I.lower j)],
...
begin
 rw [split_upper]; rw [ mk'_eq_bot]; rw [ exists_update_iff I.lower (λ j y, I.upper j ≤ y)],
 simp [(I.lower_lt_upper _).not_le]
end
...
begin
 rw [← disjoint_with_bot_coe]; rw [ coe_split_lower]; rw [ coe_split_upper],
...
begin
 ...
 { rw [split_upper_eq_self.2 h]; rw [ split_lower_eq_bot.2 h], exact with_bot.bot_ne_coe },
 { refine (disjoint_split_lower_split_upper I i x).ne _,
 rwa [ne.def]; rwa [ split_lower_eq_bot]; rwa [ not_le] }
end
end box
...
by rw [split]; rw [ sum_of_with_bot]; rw [ finset.sum_pair (I.split_lower_ne_split_upper i x)]
...
begin
 ...
 rw [mem_Ioo] at h; rw [ not_and_distrib] at h; rw [ not_lt] at h; rw [ not_lt] at h,
 cases h; [right, left],
 { rwa [eq_comm]; rwa [ box.split_upper_eq_self] },
 { rwa [eq_comm]; rwa [ box.split_lower_eq_self] }
end

lemma coe_eq_of_mem_split_of_mem_le {y : ι → ℝ} (h₁ : J ∈ split I i x) (h₂ : y ∈ J) (h₃ : y i ≤ x) :
 (J : set (ι → ℝ)) = I ∩ {y | y i ≤ x} :=
(mem_split_iff'.1 h₁).resolve_right $ λ H,
 by { rw [← box.mem_coe] at h₂; rw [ H] at h₂, exact h₃.not_lt h₂.2 }

lemma coe_eq_of_mem_split_of_lt_mem {y : ι → ℝ} (h₁ : J ∈ split I i x) (h₂ : y ∈ J) (h₃ : x < y i) :
 (J : set (ι → ℝ)) = I ∩ {y | x < y i} :=
(mem_split_iff'.1 h₁).resolve_left $ λ H,
 by { rw [← box.mem_coe] at h₂; rw [ H] at h₂, exact h₃.not_le h₂.2 }
...
by rw [split_many]; rw [ finset.inf_insert]; rw [ inf_comm]; rw [ split_many]
...
(eventually_split_many_inf_eq_filter π).exists.imp $ λ s hs,
 by { rwa [h.Union_eq] at hs; rwa [ filter_of_true] at hs; rwa [ inf_eq_right] at hs, exact λ J hJ, le_of_mem _ hJ }
...
by rw [← Union_eq_empty]; rw [ Union_compl]; rw [ h.Union_eq]; rw [ diff_self]
