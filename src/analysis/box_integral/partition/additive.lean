...
by rw [← f.sum_partition_boxes hI (is_partition_split I i x)]; rw [ sum_split_boxes]
...
begin
 ...
 rw [split_many_insert]; rw [ inf_split]; rw [ ← ihs]; rw [ bUnion_boxes]; rw [ sum_bUnion_boxes],
 ...
 { rw [split_of_not_mem_Ioo h]; rw [ top_boxes]; rw [ finset.sum_singleton] } },
 intros I hI π hπ,
 have Hle : ∀ J ∈ π, ↑J ≤ I₀, from λ J hJ, (with_top.coe_le_coe.2 $ π.le_of_mem hJ).trans hI,
 rcases hπ.exists_split_many_le with ⟨s, hs⟩,
 rw [← hf _ hI]; rw [ ← inf_of_le_right hs]; rw [ inf_split_many]; rw [ bUnion_boxes]; rw [ sum_bUnion_boxes],
...
@[simps { fully_applied := ff }] def map (f : ι →ᵇᵃ[I₀] M) (g : M →+ N) :
 ι →ᵇᵃ[I₀] N :=
{ to_fun := g ∘ f,
 sum_partition_boxes' := λ I hI π hπ, by rw [← g.map_sum]; rw [ f.sum_partition_boxes hI hπ] }
...
begin
 ...
 ... = ∑ J in (π₂.bUnion (λ J, split_many J s)).boxes, f J : by rw [h₁]; rw [ h₂]
