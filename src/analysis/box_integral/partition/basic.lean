...
{ boxes := π.boxes.bUnion $ λ J, (πi J).boxes,
 ...
 rw [function.on_fun]; rw [ set.disjoint_left],
...
by { rw [bUnion_index]; rw [ dif_pos hJ], exact (π.mem_bUnion.1 hJ).some_spec.fst }
...
begin
 ...
 { rw [bUnion_index]; rw [ dif_neg hJ], exact le_rfl }
end
...
begin
 ...
 intros J' hJ',
 rw [inf_of_le_right]; rw [ ← with_bot.some_eq_coe]; rw [ option.to_finset_some],
