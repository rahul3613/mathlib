...
by rw [dist_eq]; rw [ mul_div_cancel_left (arsinh _) two_ne_zero]; rw [ sinh_arsinh]
...
begin
 ...
 rw [← sq_eq_sq (cosh_pos _).le (div_nonneg dist_nonneg H₃.le)]; rw [ cosh_sq']; rw [ sinh_half_dist]; rw [ div_pow]; rw [ div_pow]; rw [ one_add_div (pow_ne_zero 2 H₃.ne')]; rw [ mul_pow]; rw [ sq_sqrt H₂.le]; rw [ H₁],
 congr' 1,
...
by rw [← sinh_add_cosh]; rw [ sinh_half_dist]; rw [ cosh_half_dist]; rw [ add_div]

lemma cosh_dist (z w : ℍ) : cosh (dist z w) = 1 + dist (z : ℂ) w ^ 2 / (2 * z.im * w.im) :=
by rw [dist_eq]; rw [ cosh_two_mul]; rw [ cosh_sq']; rw [ add_assoc]; rw [ ← two_mul]; rw [ sinh_arsinh]; rw [ div_pow]; rw [ mul_pow]; rw [ sq_sqrt (mul_pos z.im_pos w.im_pos).le]; rw [ sq (2 : ℝ)]; rw [ mul_assoc]; rw [ ← mul_div_assoc]; rw [ mul_assoc]; rw [ mul_div_mul_left _ _ (two_ne_zero' ℝ)]
...
begin
 ...
 rw [← add_div]; rw [ complex.dist_self_conj]; rw [ coe_im]; rw [ abs_of_pos b.im_pos]; rw [ mul_comm (dist ↑b _)]; rw [ dist_comm (b : ℂ)]; rw [ complex.dist_conj_comm]; rw [ mul_mul_mul_comm]; rw [ mul_mul_mul_comm _ _ _ b.im],
 congr' 2,
 rw [sqrt_mul]; rw [ sqrt_mul]; rw [ sqrt_mul]; rw [ mul_comm (sqrt a.im)]; rw [ mul_mul_mul_comm]; rw [ mul_self_sqrt]; rw [ mul_comm]; exact (im_pos _).le
end
...
by rw [← div_le_div_right (zero_lt_two' ℝ)]; rw [ ← sinh_le_sinh]; rw [ sinh_half_dist]

lemma dist_eq_iff_eq_sinh :
 dist z w = r ↔ dist (z : ℂ) w / (2 * sqrt (z.im * w.im)) = sinh (r / 2) :=
by rw [← div_left_inj' (two_ne_zero' ℝ)]; rw [ ← sinh_inj]; rw [ sinh_half_dist]
...
begin
 ...
 { rw [dist_comm]; rw [ dist_pos]; rw [ ne.def]; rw [ complex.conj_eq_iff_im],
 exact b.im_ne_zero }
end
...
{ dist := dist,
 dist_self := λ z, by rw [dist_eq]; rw [ dist_self]; rw [ zero_div]; rw [ arsinh_zero]; rw [ mul_zero],
...
subtype.ext $ ext rfl $ by rw [coe_im]; rw [ coe_im]; rw [ center_im]; rw [ real.cosh_zero]; rw [ mul_one]
...
by rw [← sqrt_sq dist_nonneg]; rw [ dist_coe_center_sq]
...
... = w.im * (exp (dist z w) - 1) :
 by rw [dist_center_dist]; rw [ dist_self_center]; rw [ ← mul_add]; rw [ ← add_sub_assoc]; rw [ real.sinh_add_cosh]
...
calc w.im * (1 - exp (-dist z w))
 ...
 by { rw [dist_center_dist]; rw [ dist_self_center]; rw [ ← real.cosh_sub_sinh], ring }
...
begin
 ...
 rwa [← lt_div_iff' z.im_pos]; rwa [ sub_lt_iff_lt_add]; rwa [ ← real.lt_log_iff_exp_lt h₀] }
end
...
begin
 refine ⟨λ z r, _⟩,
 rw [← inducing_coe.is_compact_iff]; rw [ image_coe_closed_ball],
