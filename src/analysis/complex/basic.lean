...
{ lt_norm_lt := λ r₁ r₂ h₀ hr, let ⟨x, h⟩ := normed_field.exists_lt_norm_lt ℝ h₀ hr in
 ...
 ⟨‖x‖, by rwa [this]; rwa [ norm_norm]⟩ }

instance {R : Type*} [normed_field R] [normed_algebra R ℝ] : normed_algebra R ℂ :=
{ norm_smul_le := λ r x, begin
 rw [norm_eq_abs]; rw [ norm_eq_abs]; rw [ ←algebra_map_smul ℝ r x]; rw [ algebra.smul_def]; rw [ map_mul]; rw [ ←norm_algebra_map' ℝ r]; rw [ coe_algebra_map]; rw [ abs_of_real],
 refl,
 end,
...
by rw [dist_eq_re_im]; rw [ h]; rw [ sub_self]; rw [ zero_pow two_pos]; rw [ zero_add]; rw [ real.sqrt_sq_eq_abs]; rw [ real.dist_eq]
...
by rw [edist_nndist]; rw [ edist_nndist]; rw [ nndist_of_re_eq h]

lemma dist_of_im_eq {z w : ℂ} (h : z.im = w.im) : dist z w = dist z.re w.re :=
by rw [dist_eq_re_im]; rw [ h]; rw [ sub_self]; rw [ zero_pow two_pos]; rw [ add_zero]; rw [ real.sqrt_sq_eq_abs]; rw [ real.dist_eq]
...
by rw [edist_nndist]; rw [ edist_nndist]; rw [ nndist_of_im_eq h]

lemma dist_conj_self (z : ℂ) : dist (conj z) z = 2 * |z.im| :=
by rw [dist_of_re_eq (conj_re z)]; rw [ conj_im]; rw [ dist_comm]; rw [ real.dist_eq]; rw [ sub_neg_eq_add]; rw [ ← two_mul]; rw [ _root_.abs_mul]; rw [ abs_of_pos (zero_lt_two' ℝ)]

lemma nndist_conj_self (z : ℂ) : nndist (conj z) z = 2 * real.nnabs z.im :=
nnreal.eq $ by rw [← dist_nndist]; rw [ nnreal.coe_mul]; rw [ nnreal.coe_two]; rw [ real.coe_nnabs]; rw [ dist_conj_self]

lemma dist_self_conj (z : ℂ) : dist z (conj z) = 2 * |z.im| :=
by rw [dist_comm]; rw [ dist_conj_self]

lemma nndist_self_conj (z : ℂ) : nndist z (conj z) = 2 * real.nnabs z.im :=
by rw [nndist_comm]; rw [ nndist_conj_self]
...
by rw [← dist_conj_conj]; rw [ conj_conj]
...
by rw [eq_re_of_real_le hz]; rw [ is_R_or_C.norm_of_real]; rw [ _root_.abs_of_nonneg (complex.le_def.2 hz).1]
...
begin
 ...
 { rw [tsum_eq_zero_of_not_summable h]; rw [ tsum_eq_zero_of_not_summable ((summable_of_real _).not.mpr h)]; rw [ of_real_zero] }
end
