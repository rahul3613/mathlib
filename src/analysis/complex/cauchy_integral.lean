...
begin
 ...
 simp_rw [← interval_integral.integral_smul, interval_integral.integral_symm w.im z.im, ← interval_integral.integral_neg, ← hF'],
 ...
 rw [← (volume_preserving_equiv_real_prod.symm _).integrable_on_comp_preimage (measurable_equiv.measurable_embedding _)] at Hi,
...
begin
 ...
 rwa [mem_closed_ball_iff_norm]; rwa [ hz]
 end
...
begin
 ...
 ((continuous_on_id.sub continuous_on_const).smul hc)
 (λ z hz, (differentiable_at_id.sub_const _).smul (hd z hz))
 ... = 0 : by rw [sub_self]; rw [ zero_smul]; rw [ smul_zero]
end
...
begin
 ...
 rw [← circle_integral.integral_sub_inv_of_mem_ball hw.1]; rw [ ← circle_integral.integral_smul_const]; rw [ ← sub_eq_zero]; rw [ ← circle_integral.integral_sub]; rw [ ← circle_integral.integral_congr hR.le hFeq]; rw [ HI],
...
begin
 ...
 intros z hz,
 rw [circle_integral_sub_inv_smul_of_differentiable_on_off_countable_aux hs hz hc hd]; rw [ inv_smul_smul₀],
 ...
 rw [← cardinal.le_aleph_0_iff_set_countable] at this; rw [ cardinal.mk_Ioo_real (hlu₀.1.trans hlu₀.2)] at this,
...
lemma circle_integral_sub_inv_smul_of_differentiable_on_off_countable
 ...
 (hc : continuous_on f (closed_ball c R)) (hd : ∀ x ∈ ball c R \ s, differentiable_at ℂ f x) :
 ∮ z in C(c, R), (z - w)⁻¹ • f z = (2 * π * I : ℂ) • f w :=
by { rw [← two_pi_I_inv_smul_circle_integral_sub_inv_smul_of_differentiable_on_off_countable hs hw hc hd]; rw [ smul_inv_smul₀], simp [real.pi_ne_zero, I_ne_zero] }
