...
begin
 ...
 rwa [of_real_mul_im]; rwa [ sub_im]; rwa [ mul_I_im]; rwa [ of_real_re]; rwa [ _root_.abs_mul]; rwa [ abs_of_pos hd₀]; rwa [ mul_le_mul_left hd₀] },
 ...
 { rw [mul_zero] at H; rw [ add_zero] at H,
 ...
 rw [frontier_re_prod_im] at hw; rw [ closure_Ioo (neg_lt_self hR₀).ne] at hw; rw [ frontier_Ioo hab] at hw; rw [ closure_Ioo hab.ne] at hw; rw [ frontier_Ioo (neg_lt_self hR₀)] at hw,
 by_cases him : w.im = a - b ∨ w.im = a + b,
 { rw [closure_re_prod_im] at hwc; rw [ closure_Ioo (neg_lt_self hR₀).ne] at hwc,
 rw [norm_smul]; rw [ ← one_mul C],
 ...
 { rw [closure_re_prod_im]; rw [ closure_Ioo hab.ne]; rw [ closure_Ioo (neg_lt_self hR₀).ne],
...
begin
 ...
 { intros z hz,
 rw [mem_re_prod_im]; rw [ exp_re]; rw [ exp_im]; rw [ mem_Ioi]; rw [ mem_Ioi],
 ...
 rw [← comap_comap]; rw [ comap_abs_at_top]; rw [ comap_sup]; rw [ inf_sup_right],
 ...
 rw [comp_app]; rw [ ← of_real_exp],
 exact hre _ (real.exp_pos _).le },
 { -- If `ζ.im = π / 2`, then `complex.exp ζ` is a purely imaginary number with positive `im`
 intros ζ hζ,
 rw [← re_add_im ζ]; rw [ hζ]; rw [ comp_app]; rw [ exp_add_mul_I]; rw [ ← of_real_cos]; rw [ ← of_real_sin]; rw [ real.cos_pi_div_two]; rw [ real.sin_pi_div_two]; rw [ of_real_zero]; rw [ of_real_one]; rw [ one_mul]; rw [ zero_add]; rw [ ← of_real_exp],
...
begin
 ...
 { rw [comp_app]; rw [ mul_assoc]; rw [ I_mul_I]; rw [ mul_neg_one]; rw [ ← of_real_neg],
...
begin
 ...
 { rw [comp_app]; rw [ ← of_real_neg],
 exact hre (-x) (neg_nonpos.2 hx) },
 { rw [comp_app]; rw [ ← neg_mul]; rw [ ← of_real_neg],
...
begin
 ...
 { refine ⟨0, le_rfl, λ y hy, _⟩, rw [h₀ y hy]; rw [ h₀ 0 le_rfl] },
 push_neg at h₀,
 rcases h₀ with ⟨x₀, hx₀, hne⟩,
 have hlt : ‖(0 : E)‖ < ‖f x₀‖, by rwa [norm_zero]; rwa [ norm_pos_iff],
 ...
 rw [real.cocompact_eq]; rw [ inf_sup_right]; rw [ (disjoint_at_bot_principal_Ici (0 : ℝ)).eq_bot]; rw [ bot_sup_eq],
 ...
 ... = |(z - x₀).re| : by rw [sub_re]; rw [ of_real_re]; rw [ _root_.abs_sub_comm]
...
begin
 ...
 { intro z, rw [norm_smul]; rw [ norm_eq_abs]; rw [ abs_exp]; rw [ of_real_mul_re] },
 ...
 { rw [hgn]; rw [ of_real_mul_re]; rw [ I_re]; rw [ mul_zero]; rw [ mul_zero]; rw [ real.exp_zero]; rw [ one_mul],
 exact him y }
end
...
begin
 ...
 exact hre n },
 { rw [hg]; rw [ of_real_mul_re]; rw [ I_re]; rw [ mul_zero]; rw [ real.exp_zero]; rw [ one_pow]; rw [ one_mul],
 exact hC y }
end
