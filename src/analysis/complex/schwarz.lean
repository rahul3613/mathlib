...
begin
 ...
 rw [dslope_of_ne _ hz']; rw [ slope_def_module]; rw [ norm_smul]; rw [ norm_inv]; rw [ mem_sphere_iff_norm.1 hz]; rw [ ← div_eq_inv_mul]; rw [ div_le_div_right hr₀]; rw [ ← dist_eq_norm],
 exact le_of_lt (h_maps (mem_ball.2 (by { rw mem_sphere.1 hz, exact hr.2 }))) },
 { rw [closure_ball c hr₀.ne']; rw [ mem_closed_ball],
 exact hr.1.le }
end
...
begin
 ...
 { rw [hc]; rw [ norm_zero], exact div_nonneg hR₂.le hR₁.le },
 ...
 begin
 rw [g.dslope_comp]; rw [ hgf]; rw [ is_R_or_C.norm_of_real]; rw [ abs_norm],
