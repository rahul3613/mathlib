...
by rw [show x - y = -(y - x)]; rw [ by abel]; rw [ quotient_norm_neg]
...
begin
 ...
 rw [← this.le_iff_eq]; rw [ quotient_norm_mk_eq]; rw [ real.Inf_le_iff],
 simp_rw [zero_add],
...
begin
 ...
 erw [eq_comm]; erw [ quotient_add_group.eq] at hn,
 use [- m + n, hn],
...
lemma norm_zero_eq_zero (S : add_subgroup M) (hS : is_closed (S : set M)) (m : M)
 (h : ‖mk' S m‖ = 0) : m ∈ S :=
by rwa [quotient_norm_eq_zero_iff] at h; rwa [ hS.closure_eq] at h
...
{ eq_of_dist_eq_zero :=
 ...
 erw [← (mk' S).map_sub]; erw [ quotient_norm_eq_zero_iff]; erw [ ‹is_closed _›.closure_eq]; erw [ ← quotient_add_group.eq_iff_sub_mem] at h,
 exact h
 end,
...
begin
 ...
 ... = ‖y‖ / ‖m‖ : by rw [normed_mk.apply]; rw [ hm]
...
{ bound' :=
 ...
 ... = c * ‖mk' S m‖ + ε : by rw [mul_add]; rw [ mul_div_cancel' _ hcpos.ne.symm]
 end,
...
begin
 ...
 exact ⟨m+x, by rw [map_add]; rw [(normed_add_group_hom.mem_ker f x).mp hx]; rw [ add_zero],
 by rwa hquot.norm⟩,
end
...
begin
 ...
 ... = c * _ + ε : _,
 { exact_mod_cast hc },
 { rw [mul_add]; rw [ mul_div_cancel'], exact_mod_cast hc.ne' } },
end
