...
by { rw [sub_eq_add_neg]; rw [ ← nonarch_add_group_seminorm_class.map_neg_eq_map' f y],
...
{ map_add_le_add := λ f x y, begin
 ...
 { intro a,
 rw [← nonarch_add_group_seminorm_class.map_zero f]; rw [ ← sub_self a],
...
⟨λ p q,
 { to_fun := λ x, p x + q x,
 map_one' := by rw [map_one_eq_zero p]; rw [ map_one_eq_zero q]; rw [ zero_add],
 mul_le' := λ _ _, (add_le_add (map_mul_le_add p _ _) $ map_mul_le_add q _ _).trans_eq $
 add_add_add_comm _ _ _ _,
 inv' := λ x, by rw [map_inv_eq_map p]; rw [ map_inv_eq_map q] }⟩
...
⟨λ p q,
 { to_fun := p ⊔ q,
 map_one' :=
 by rw [pi.sup_apply]; rw [ ←map_one_eq_zero p]; rw [ sup_eq_left]; rw [ map_one_eq_zero p]; rw [ map_one_eq_zero q],
 mul_le' := λ x y, sup_le
 ...
 inv' := λ x, by rw [pi.sup_apply]; rw [ pi.sup_apply]; rw [ map_inv_eq_map p]; rw [ map_inv_eq_map q] }⟩
...
{ to_fun := λ x, p (f x),
 map_one' := by rw [f.map_one]; rw [ map_one_eq_zero p],
 mul_le' := λ _ _, (congr_arg p $ f.map_mul _ _).trans_le $ map_mul_le_add p _ _,
 inv' := λ x, by rw [map_inv]; rw [ map_inv_eq_map p] }
...
⟨λ p q,
 ...
 (λ r hr, ⟨1, by rwa [div_one]; rwa [ map_one_eq_zero p]; rwa [ map_one_eq_zero q]; rwa [ add_zero]⟩),
...
{ inf := (⊓),
 ...
 by rw [div_self']; rw [ map_one_eq_zero q]; rw [ add_zero],
...
⟨λ p q,
 { to_fun := p ⊔ q,
 map_zero' := by rw [pi.sup_apply]; rw [ ←map_zero p]; rw [ sup_eq_left]; rw [ map_zero p]; rw [ map_zero q],
 ...
 neg' := λ x, by rw [pi.sup_apply]; rw [ pi.sup_apply]; rw [ map_neg_eq_map p]; rw [ map_neg_eq_map q] }⟩
...
⟨{ to_fun := λ x, if x = 0 then 0 else 1,
 ...
 by_cases hx : x = 0,
 { rw [if_pos hx]; rw [ hx]; rw [ zero_add], exact le_max_of_le_right (le_refl _) },
