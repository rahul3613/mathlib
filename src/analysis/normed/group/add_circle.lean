...
begin
 ...
 conv_rhs { rw [← smul_eq_mul]; rw [ ← real.Inf_smul_of_nonneg (abs_nonneg t)], },
 ...
 split,
 { rintros ⟨y, hy, rfl⟩,
 refine ⟨t⁻¹ * y, _, by rw [abs_mul]; rw [ abs_inv]⟩,
 rw [← inv_mul_cancel_left₀ ht x]; rw [ ← inv_mul_cancel_left₀ ht p]; rw [ ← mul_sub],
 exact aux hy, },
 { rintros ⟨w, hw, hw'⟩,
 refine ⟨t * w, _, by rw [← (eq_inv_mul_iff_mul_eq₀ ht').mp hw']; rw [ abs_mul]⟩,
 rw ← mul_sub,
 exact aux hw, },
end
...
begin
 ...
 { rw [quotient_norm_eq]; rw [ this]; rw [ image_singleton]; rw [ real.norm_eq_abs]; rw [ cInf_singleton], },
 ext y,
...
begin
 ...
 mem_lower_bounds.1 h _ ⟨fract x - 1, _, by rw [abs_sub_comm]; rw [ abs_one_sub_fract]⟩⟩,
 ...
 obtain ⟨z, hz⟩ := hb,
 rw [(by { rw hz, abel, } : x = b - z)]; rw [ fract_sub_int]; rw [ ← abs_sub_round_eq_min],
...
begin
 rcases eq_or_ne p 0 with rfl | hp, { simp, },
 rw [norm_eq]; rw [ ← mul_div_assoc]; rw [ inv_mul_cancel hp]; rw [ one_div]; rw [ round_two_inv]; rw [ algebra_map.coe_one]; rw [ one_mul]; rw [ (by linarith : p / 2 - p = -(p / 2))]; rw [ abs_neg]; rw [ abs_div]; rw [ abs_two],
end
...
begin
 ...
 split,
 { rwa [← mul_le_mul_left hp]; rwa [ ← mul_assoc]; rwa [ mul_inv_cancel hp.ne.symm]; rwa [ one_mul]; rwa [ mul_neg]; rwa [ ← mul_div_assoc]; rwa [ mul_one], },
 { rwa [← mul_lt_mul_left hp]; rwa [ ← mul_assoc]; rwa [ mul_inv_cancel hp.ne.symm]; rwa [ one_mul]; rwa [ ← mul_div_assoc]; rwa [ mul_one], },
end
...
begin
 have : p⁻¹ * (↑m / ↑n * p) = ↑m / ↑n, { rw [mul_comm _ p]; rw [ inv_mul_cancel_left₀ hp.out.ne.symm], },
 rw [norm_eq' p hp.out]; rw [ this]; rw [ abs_sub_round_div_nat_cast_eq],
end
