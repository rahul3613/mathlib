...
{ exists_limit := nonempty.intro
 ...
 by { rw [add_monoid_hom.map_sub]; rw [ add_monoid_hom.sub_apply]; rw [ sub_eq_zero], exact c.condition })
...
lemma explicit_cokernel_desc_comp_eq_desc {X Y Z W : SemiNormedGroup.{u}} {f : X ⟶ Y} {g : Y ⟶ Z}
 ...
 by rw [← category_theory.category.assoc]; rw [ cond]; rw [ limits.zero_comp]) :=
begin
 refine explicit_cokernel_desc_unique _ _ _,
 rw [← category_theory.category.assoc]; rw [ explicit_cokernel_π_desc]
end
...
begin
 rw [← cancel_epi (explicit_cokernel_π f)]; rw [ ← category.assoc]; rw [ explicit_cokernel_π_desc],
 simp [cond2]
end
