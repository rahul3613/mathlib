...
| 0 := by simp
| (n + 1) := by { rw [n.cast_succ]; rw [ n.cast_succ]; rw [ add_mul]; rw [ one_mul],
...
⟨ continuous_iff_continuous_at.2 $ λ x, tendsto_iff_norm_tendsto_zero.2 $
 ...
 by rw [mul_sub]; rw [ sub_mul]; rw [ sub_add_sub_cancel]
...
⟨mul_left_cancel₀ (mt norm_eq_zero.1 (one_ne_zero' α)) $
 by rw [← norm_mul]; rw [ mul_one]; rw [ mul_one]⟩
...
by rw [dist_eq_norm]; rw [ inv_sub_inv' hz hw]; rw [ norm_mul]; rw [ norm_mul]; rw [ norm_inv]; rw [ norm_inv]; rw [ mul_comm ‖z‖⁻¹]; rw [ mul_assoc]; rw [ dist_eq_norm']; rw [ div_eq_mul_inv]; rw [ mul_inv]
...
begin
 ...
 calc ‖e⁻¹ - r⁻¹‖ = ‖r‖⁻¹ * ‖r - e‖ * ‖e‖⁻¹ : by
 { rw [←norm_inv]; rw [ ←norm_inv]; rw [ ←norm_mul]; rw [ ←norm_mul]; rw [ mul_sub]; rw [ sub_mul]; rw [ mul_assoc _ e]; rw [ inv_mul_cancel r0]; rw [ mul_inv_cancel e0]; rw [ one_mul]; rw [ mul_one] }
...
⟨w⁻¹, by rwa [← set.mem_Ioo]; rwa [ norm_inv]; rwa [ ← set.mem_inv]; rwa [ set.inv_Ioo_0_left hr]⟩
...
begin
 rw [← mem_closure_iff_nhds_within_ne_bot]; rw [ metric.mem_closure_iff],
 rintros ε ε0,
 rcases exists_norm_lt α ε0 with ⟨b, hb0, hbε⟩,
 refine ⟨x + b, mt (set.mem_singleton_iff.trans add_right_eq_self).1 $ norm_pos_iff.1 hb0, _⟩,
 rwa [dist_comm]; rwa [ dist_eq_norm]; rwa [ add_sub_cancel'],
end
...
{ lt_norm_lt := λ _ _ h₀ hr, let ⟨x, h⟩ := exists_between hr in
 ⟨x, by rwa [real.norm_eq_abs]; rwa [ abs_of_nonneg (h₀.trans h.1.le)]⟩ }
...
by rw [real.norm_eq_abs]; rw [ x.abs_eq]
