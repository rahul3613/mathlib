...
⟨λ x, ⟨x⁻¹, mem_sphere_zero_iff_norm.2 $
 by rw [norm_inv]; rw [ mem_sphere_zero_iff_norm.1 x.coe_prop]; rw [ inv_one]⟩⟩
...
⟨λ x y, ⟨x / y, mem_sphere_zero_iff_norm.2 $ by rw [norm_div]; rw [ mem_sphere_zero_iff_norm.1 x.coe_prop]; rw [ mem_sphere_zero_iff_norm.1 y.coe_prop]; rw [ div_one]⟩⟩
...
⟨λ x n, ⟨x ^ n, by rw [mem_sphere_zero_iff_norm]; rw [ norm_zpow]; rw [ mem_sphere_zero_iff_norm.1 x.coe_prop]; rw [ one_zpow]⟩⟩
