...
begin
 ...
 rwa [real.norm_of_nonneg (zpow_nonneg (norm_nonneg _) _)]; rwa [ zpow_neg]; rwa [ ←div_eq_mul_inv]; rwa [ le_div_iff'],
...
begin
 ...
 rw [one_mul]; rw [ real.norm_of_nonneg (real.rpow_nonneg_of_nonneg (zero_le_one.trans hx) _)]; rw [ real.norm_of_nonneg (zpow_nonneg (zero_le_one.trans hx) _)]; rw [ ←real.rpow_int_cast]; rw [ int.cast_neg]; rw [ int.cast_coe_nat],
...
by rw [mul_comm (‖c‖)]; rw [ mul_assoc]; rw [ iterated_fderiv_const_smul_apply (f.smooth _)]; rw [ norm_smul]
...
begin
 ...
 rwa [← real.norm_eq_abs]; rwa [ ← norm_iterated_fderiv_eq_norm_iterated_deriv],
end
...
begin
 ...
 rwa [pow_zero] at this; rwa [ one_mul] at this,
end
...
mk_clm (λ f x, B (f x) (g x))
 ...
 rw [← mul_assoc (‖x‖^k)]; rw [ mul_comm (‖x‖^k)],
 simp_rw [mul_assoc (‖B‖)],
 refine mul_le_mul_of_nonneg_left _ (by positivity),
 rw [finset.mul_sum],
 ...
 refine finset.sum_le_sum (λ i hi, _),
 simp only [one_mul],
 rw [← mul_assoc]; rw [ mul_comm (‖x‖^k)]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc],
 refine mul_le_mul _ _ (by positivity) (by positivity),
 { norm_cast,
...
begin
 ...
 { rw [iterated_pderiv_zero]; rw [ iterated_pderiv_one],
 refl },
...
{ cont :=
 ...
 rw [finset.sup_singleton]; rw [ one_smul ]; rw [ seminorm.comp_apply]; rw [ coe_norm_seminorm]; rw [ schwartz_seminorm_family_apply_zero]; rw [ bounded_continuous_function.norm_le (map_nonneg _ _)],
 intros x,
