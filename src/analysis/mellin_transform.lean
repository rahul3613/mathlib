...
lemma cpow_mul_of_real_nonneg {x : ℝ} (hx : 0 ≤ x) (y : ℝ) (z : ℂ) :
 (x : ℂ) ^ (↑y * z) = (↑(x ^ y) : ℂ) ^ z :=
begin
 rw [cpow_mul]; rw [ of_real_cpow hx],
 { rw [←of_real_log hx]; rw [ ←of_real_mul]; rw [ of_real_im]; rw [ neg_lt_zero], exact real.pi_pos },
 { rw [←of_real_log hx]; rw [ ←of_real_mul]; rw [ of_real_im], exact real.pi_pos.le },
end
...
begin
 ...
 have h2 : (a : ℂ) ^ (s - 1) ≠ 0,
 { rw [ne.def]; rw [ cpow_eq_zero_iff]; rw [ not_and_distrib]; rw [ of_real_eq_zero], exact or.inl ha.ne' },
 simp_rw [mellin_convergent, ←this, integrable_on_congr_fun h1 measurable_set_Ioi, integrable_on, integrable_smul_iff h2],
end
...
begin
 ...
 rw [←complex.coe_smul (t ^ (a - 1))]; rw [ ←mul_smul]; rw [ ←cpow_mul_of_real_nonneg (le_of_lt ht)]; rw [ of_real_cpow (le_of_lt ht)]; rw [ ←cpow_add _ _ (of_real_ne_zero.mpr (ne_of_gt ht))]; rw [ of_real_sub]; rw [ of_real_one]; rw [ mul_sub]; rw [ mul_div_cancel' _ (of_real_ne_zero.mpr ha)]; rw [ mul_one]; rw [ add_comm]; rw [ ←add_sub_assoc]; rw [ sub_add_cancel],
end
...
begin
 ...
 simp_rw mellin,
 conv_rhs { rw [←integral_comp_rpow_Ioi _ ha]; rw [ ←integral_smul] },
 refine set_integral_congr measurable_set_Ioi (λ t ht, _),
 dsimp only,
 rw [←mul_smul]; rw [ ←mul_assoc]; rw [ inv_mul_cancel]; rw [ one_mul]; rw [ ←smul_assoc]; rw [ real_smul],
 show |a| ≠ 0, { contrapose! ha, exact abs_eq_zero.mp ha },
 rw [of_real_cpow (le_of_lt ht)]; rw [ ←cpow_mul_of_real_nonneg (le_of_lt ht)]; rw [ ←cpow_add _ _ (of_real_ne_zero.mpr $ ne_of_gt ht)]; rw [ of_real_sub]; rw [ of_real_one]; rw [ mul_sub]; rw [ mul_div_cancel' _ (of_real_ne_zero.mpr ha)]; rw [ add_comm]; rw [ ←add_sub_assoc]; rw [ mul_one]; rw [ sub_add_cancel]
end
...
begin
 ...
 { intros t ht,
 dsimp only,
 rw [of_real_mul]; rw [ mul_cpow_of_real_nonneg ha.le (le_of_lt ht)]; rw [ ←mul_smul]; rw [ (by ring : 1 - s = -(s - 1))]; rw [ cpow_neg]; rw [ inv_mul_cancel_left₀],
 rw [ne.def]; rw [ cpow_eq_zero_iff]; rw [ of_real_eq_zero]; rw [ not_and_distrib],
 exact or.inl ha.ne' },
 rw [set_integral_congr measurable_set_Ioi this]; rw [ integral_smul]; rw [ integral_comp_mul_left_Ioi _ _ ha]; rw [ mul_zero]; rw [ ←complex.coe_smul]; rw [ ←mul_smul]; rw [ sub_eq_add_neg]; rw [ cpow_add _ _ (of_real_ne_zero.mpr ha.ne')]; rw [ cpow_one]; rw [ abs_of_pos (inv_pos.mpr ha)]; rw [ of_real_inv]; rw [ mul_assoc]; rw [ mul_comm]; rw [ inv_mul_cancel_right₀ (of_real_ne_zero.mpr ha.ne')]
end
...
by simp_rw [←rpow_neg_one, mellin_comp_rpow _ _ (neg_ne_zero.mpr one_ne_zero), abs_neg, abs_one, inv_one, one_smul, of_real_neg, of_real_one, div_neg, div_one]
...
begin
 ...
 rw [integrable_on]; rw [ ←integrable_norm_iff this]; rw [ ←integrable_on],
...
begin
 ...
 rw [←integrable_on]; rw [ ←integrable_on_Ioc_iff_integrable_on_Ioo]; rw [ ←interval_integrable_iff_integrable_Ioc_of_le hε.le],
 ...
 { rw [dist_eq_norm]; rw [ sub_zero]; rw [ norm_of_nonneg (le_of_lt ht.1)],
 exact ht.2 },
...
begin
 ...
 rw [this]; rw [ integrable_on_union]; rw [ integrable_on_union],
...
begin
 rw [mellin_convergent]; rw [ mellin_convergent_iff_norm (subset_refl _) measurable_set_Ioi hfc.ae_strongly_measurable],
...
begin
 ...
 (eventually_nhds_within_iff.mpr $ eventually_of_forall (λ t ht, _)),
 { simp_rw [function.comp_app, ←one_div, log_div one_ne_zero (ne_of_gt ht), real.log_one, zero_sub, neg_neg] },
...
begin
 ...
 simp_rw [bound, F', norm_smul, norm_mul, complex.norm_eq_abs (log _), complex.abs_of_real, mul_assoc],
 ...
 rwa [sub_re] at hz' ; rwa [ sub_le_iff_le_add'] at hz' },
 ...
 rw [mem_ball_iff_norm'] at hz; rw [ complex.norm_eq_abs] at hz,
 have hz' := (re_le_abs _).trans hz.le,
 rwa [sub_re] at hz'; rwa [ sub_le_iff_le_add] at hz'; rwa [ ←sub_le_iff_le_add'] at hz', } },
...
begin
 ...
 simp_rw [has_mellin, mellin, mellin_convergent, ←indicator_smul, integrable_on, integrable_indicator_iff aux3, smul_eq_mul, integral_indicator aux3, mul_one, integrable_on, measure.restrict_restrict_of_subset Ioc_subset_Ioi_self],
 rw [←integrable_on]; rw [ ←interval_integrable_iff_integrable_Ioc_of_le zero_le_one],
 refine ⟨interval_integral.interval_integrable_cpow' aux1, _⟩,
 rw [←interval_integral.integral_of_le zero_le_one]; rw [ integral_cpow (or.inl aux1)]; rw [ sub_add_cancel]; rw [ of_real_zero]; rw [ of_real_one]; rw [ one_cpow]; rw [ zero_cpow aux2]; rw [ sub_zero]
end
...
begin
 ...
 simp_rw [has_mellin, ←mellin_convergent.cpow_smul, ←mellin_cpow_smul, ←indicator_smul, smul_eq_mul, mul_one] at this,
 exact this
end
