...
begin
 ...
 change 0 ≤ re ⟪_, _⟫,
 rw [zero_apply]; rw [ inner_zero_left]; rw [ zero_hom_class.map_zero]
end
...
begin
 ...
 rw [re_apply_inner_self]; rw [ add_apply]; rw [ inner_add_left]; rw [ map_add],
...
begin
 ...
 rw [re_apply_inner_self]; rw [ comp_apply]; rw [ ← adjoint_inner_right],
...
begin
 simp_rw [is_positive, forall_and_distrib, is_self_adjoint_iff_is_symmetric, linear_map.is_symmetric_iff_inner_map_self_real, conj_eq_iff_re],
 refl
end
