...
begin
 ...
 congr' 1,
 have h₁ : ‖-x - y‖ = ‖x + y‖,
 { rw [←neg_add']; rw [ norm_neg], },
 have h₂ : ‖-x + y‖ = ‖x - y‖,
 { rw [←neg_sub]; rw [ norm_neg]; rw [ sub_eq_neg_add], },
 have h₃ : ‖(I : 𝕜) • (-x) + y‖ = ‖(I : 𝕜) • x - y‖,
 { rw [←neg_sub]; rw [ norm_neg]; rw [ sub_eq_neg_add]; rw [ ←smul_neg], },
 have h₄ : ‖(I : 𝕜) • (-x) - y‖ = ‖(I : 𝕜) • x + y‖,
 { rw [smul_neg]; rw [ ←neg_add']; rw [ norm_neg] },
 rw [h₁]; rw [ h₂]; rw [ h₃]; rw [ h₄],
 ring,
end
...
begin
 ...
 rw [←this]; rw [ norm_sq_eq_def']; rw [ is_R_or_C.norm_of_nonneg (by norm_num : (0 : ℝ) ≤ 4)],
 norm_num },
 have h₂ : ‖x + x‖ = 2 * ‖x‖,
 { rw [←two_smul 𝕜]; rw [ norm_smul]; rw [ is_R_or_C.norm_two] },
...
begin
 simp only [inner_],
 have h4 : conj (4⁻¹ : 𝕜) = 4⁻¹,
 { rw [is_R_or_C.conj_inv]; rw [ ←of_real_one]; rw [ ←of_real_bit0]; rw [ ←of_real_bit0]; rw [ conj_of_real] },
 rw [map_mul]; rw [ h4],
 congr' 1,
 ...
 { rw [norm_smul]; rw [ norm_I_of_ne_zero hI]; rw [ one_mul] },
 { rw [smul_sub]; rw [ smul_smul]; rw [ I_mul_I_of_nonzero hI]; rw [ neg_one_smul]; rw [ ←neg_add']; rw [ add_comm]; rw [ norm_neg] } },
 have h₂ : ‖(I : 𝕜) • y + x‖ = ‖(I : 𝕜) • x - y‖,
 { transitivity ‖(I : 𝕜) • ((I : 𝕜) • y + x)‖,
 { rw [norm_smul]; rw [ norm_I_of_ne_zero hI]; rw [ one_mul] },
 { rw [smul_add]; rw [ smul_smul]; rw [ I_mul_I_of_nonzero hI]; rw [ neg_one_smul]; rw [ ←neg_add_eq_sub] }},
...
by { rw [add_left_aux1 ]; rw [ add_left_aux2], ring }
...
private lemma add_left_aux4' (y z : E) :
 (‖2 • z + y‖ * ‖2 • z + y‖ - ‖y - 2 • z‖ * ‖y - 2 • z‖) / 2 =
 (‖y + z‖ * ‖y + z‖) - (‖y - z‖ * ‖y - z‖) :=
by { rw [add_left_aux3 ]; rw [ add_left_aux4], ring }
...
begin
 ...
 congr,
 { rw [←add_sub_assoc]; rw [ add_left_aux2']; rw [ add_left_aux4'] },
 { rw [add_left_aux5]; rw [ add_left_aux6]; rw [ add_left_aux7]; rw [ add_left_aux8],
...
begin
 ...
 { rw [int.sign_eq_neg_one_of_neg hn]; rw [ inner_prop_neg_one ((n.nat_abs : 𝕜) • x)]; rw [ nat],
...
begin
 ...
 rw [←r.num_div_denom]; rw [ ←mul_right_inj' this]; rw [ ←nat r.denom _ y]; rw [ smul_smul]; rw [ rat.cast_div],
 simp only [map_nat_cast, rat.cast_coe_nat, map_int_cast, rat.cast_coe_int, map_div₀],
 rw [←mul_assoc]; rw [ mul_div_cancel' _ this]; rw [ int_prop _ x]; rw [ map_int_cast],
end
...
begin
 by_cases hI : (I : 𝕜) = 0,
 { rw [hI]; rw [ ←nat.cast_zero], exact nat_prop _ },
 intros x y,
 have hI' : (-I : 𝕜) * I = 1,
 { rw [←inv_I]; rw [ inv_mul_cancel hI], },
...
begin
 intros x y,
 rw [←re_add_im r]; rw [ add_smul]; rw [ add_left]; rw [ real_prop _ x]; rw [ ←smul_smul]; rw [ real_prop _ _ y]; rw [ I_prop]; rw [ map_add]; rw [ map_mul]; rw [ conj_of_real]; rw [ conj_of_real]; rw [ conj_I],
 ring,
end
