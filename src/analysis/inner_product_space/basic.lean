...
{ nonneg_re := λ x, by { rw [← inner_product_space.norm_sq_eq_inner], apply sq_nonneg },
 ...
 by rw [inner_product_space.norm_sq_eq_inner x]; rw [ hx]; rw [ map_zero],
 .. c }
...
by rw [← @of_real_inj 𝕜]; rw [ im_eq_conj_sub]; simp [inner_conj_symm]

lemma inner_add_left (x y z : F) : ⟪x + y, z⟫ = ⟪x, z⟫ + ⟪y, z⟫ :=
c.add_left _ _ _

lemma inner_add_right (x y z : F) : ⟪x, y + z⟫ = ⟪x, y⟫ + ⟪x, z⟫ :=
by rw [←inner_conj_symm]; rw [ inner_add_left]; rw [ ring_hom.map_add]; simp only [inner_conj_symm]
...
by rw [←inner_conj_symm]; rw [ conj_re]

lemma inner_im_symm (x y : F) : im ⟪x, y⟫ = -im ⟪y, x⟫ :=
by rw [←inner_conj_symm]; rw [ conj_im]

lemma inner_smul_left (x y : F) {r : 𝕜} : ⟪r • x, y⟫ = r† * ⟪x, y⟫ :=
c.smul_left _ _ _

lemma inner_smul_right (x y : F) {r : 𝕜} : ⟪x, r • y⟫ = r * ⟪x, y⟫ :=
by rw [←inner_conj_symm]; rw [ inner_smul_left]; simp only [conj_conj, inner_conj_symm, ring_hom.map_mul]

lemma inner_zero_left (x : F) : ⟪0, x⟫ = 0 :=
by rw [←zero_smul 𝕜 (0 : F)]; rw [ inner_smul_left]; simp only [zero_mul, ring_hom.map_zero]

lemma inner_zero_right (x : F) : ⟪x, 0⟫ = 0 :=
by rw [←inner_conj_symm]; rw [ inner_zero_left]; simp only [ring_hom.map_zero]
...
by rw [←inner_conj_symm]; rw [ norm_conj]

lemma inner_neg_left (x y : F) : ⟪-x, y⟫ = -⟪x, y⟫ :=
by { rw [← neg_one_smul 𝕜 x]; rw [ inner_smul_left], simp }

lemma inner_neg_right (x y : F) : ⟪x, -y⟫ = -⟪x, y⟫ :=
by rw [←inner_conj_symm]; rw [ inner_neg_left]; simp only [ring_hom.map_neg, inner_conj_symm]
...
by { rw [←inner_conj_symm]; rw [ mul_comm], exact re_eq_norm_of_mul_conj (inner y x), }
...
begin
 rw [← @of_real_inj 𝕜]; rw [ coe_norm_sq_eq_inner_self],
 ...
 rw [← mul_assoc]; rw [ mul_conj]; rw [ is_R_or_C.conj_mul]; rw [ norm_sq_eq_def']; rw [ mul_left_comm]; rw [ ← inner_conj_symm y]; rw [ mul_conj]; rw [ norm_sq_eq_def'],
 push_cast,
 ring
end
...
begin
 ...
 rw [← sub_nonneg]; rw [ ← mul_nonneg_iff_right_nonneg_of_pos hx']; rw [ ← norm_sq]; rw [ ← norm_sq]; rw [ norm_inner_symm y]; rw [ ← sq]; rw [ ← cauchy_schwarz_aux],
...
by rw [norm_eq_sqrt_inner]; rw [ ←sqrt_mul inner_self_nonneg (re ⟪x, x⟫)]; rw [ sqrt_mul_self inner_self_nonneg]
...
{ to_fun := λ x, sqrt (re ⟪x, x⟫),
 ...
 have h₄ : re ⟪y, x⟫ ≤ ‖x‖ * ‖y‖ := by rwa [←inner_conj_symm]; rwa [ conj_re],
...
{ norm_smul_le := assume r x,
 begin
 rw [norm_eq_sqrt_inner]; rw [ inner_smul_left]; rw [ inner_smul_right]; rw [ ←mul_assoc],
 rw [is_R_or_C.conj_mul]; rw [ of_real_mul_re]; rw [ sqrt_mul]; rw [ ← coe_norm_sq_eq_inner_self]; rw [ of_real_re],
...
by rw [← @of_real_inj 𝕜]; rw [ im_eq_conj_sub]; simp
...
by { rw [inner_smul_left]; rw [ conj_of_real]; rw [ algebra.smul_def], refl }

lemma inner_smul_right (x y : E) (r : 𝕜) : ⟪x, r • y⟫ = r * ⟪x, y⟫ :=
by rw [←inner_conj_symm]; rw [ inner_smul_left]; rw [ ring_hom.map_mul]; rw [ conj_conj]; rw [ inner_conj_symm]
lemma real_inner_smul_right (x y : F) (r : ℝ) : ⟪x, r • y⟫_ℝ = r * ⟪x, y⟫_ℝ :=
inner_smul_right _ _ _

lemma inner_smul_real_right (x y : E) (r : ℝ) : ⟪x, (r : 𝕜) • y⟫ = r • ⟪x, y⟫ :=
by { rw [inner_smul_right]; rw [ algebra.smul_def], refl }
...
by rw [← zero_smul 𝕜 (0:E)]; rw [ inner_smul_left]; rw [ ring_hom.map_zero]; rw [ zero_mul]
...
by rw [←inner_conj_symm]; rw [ inner_zero_left]; rw [ ring_hom.map_zero]
...
by rw [← inner_self_re_to_K]; rw [ ← norm_sq_eq_inner]; rw [ of_real_pow]
...
by rw [inner_self_eq_norm_sq_to_K]; rw [ sq_eq_zero_iff]; rw [ of_real_eq_zero]; rw [ norm_eq_zero]

lemma inner_self_ne_zero {x : E} : ⟪x, x⟫ ≠ 0 ↔ x ≠ 0 :=
inner_self_eq_zero.not

@[simp] lemma inner_self_nonpos {x : E} : re ⟪x, x⟫ ≤ 0 ↔ x = 0 :=
by rw [← norm_sq_eq_inner]; rw [ (sq_nonneg _).le_iff_eq]; rw [ sq_eq_zero_iff]; rw [ norm_eq_zero]
...
by rw [is_R_or_C.ext_iff]; exact ⟨by rw [conj_re], by rw [conj_im]; rw [ inner_self_im]; rw [ neg_zero]⟩
...
by rw [←sub_eq_zero]; rw [ ←@inner_self_eq_zero 𝕜]; rw [ inner_sub_right]; rw [ sub_eq_zero]; rw [ h (x - y)]

lemma ext_inner_right {x y : E} (h : ∀ v, ⟪x, v⟫ = ⟪y, v⟫) : x = y :=
by rw [←sub_eq_zero]; rw [ ←@inner_self_eq_zero 𝕜]; rw [ inner_sub_left]; rw [ sub_eq_zero]; rw [ h (x - y)]
...
calc ⟪x, y⟫_ℝ * ⟪x, y⟫_ℝ ≤ ‖⟪x, y⟫_ℝ‖ * ‖⟪y, x⟫_ℝ‖ :
 by { rw [real_inner_comm y]; rw [ ← norm_mul], exact le_abs_self _ }
...
begin
 ...
 { rw inner_smul_right },
 { intros j hj hji,
 rw [inner_smul_right]; rw [ ho i j hji.symm]; rw [ mul_zero] },
...
by rw [← inner_conj_symm]; rw [ hv.inner_right_finsupp]
...
by rw [@norm_eq_sqrt_inner 𝕜]; rw [ ←sqrt_mul inner_self_nonneg (re ⟪x, x⟫)]; rw [ sqrt_mul_self inner_self_nonneg]

lemma inner_self_eq_norm_sq (x : E) : re ⟪x, x⟫ = ‖x‖^2 :=
by rw [pow_two]; rw [ inner_self_eq_norm_mul_norm]
...
by rw [pow_two]; rw [ real_inner_self_eq_norm_mul_norm]

variables (𝕜)
/-- Expand the square -/
lemma norm_add_sq (x y : E) : ‖x + y‖^2 = ‖x‖^2 + 2 * (re ⟪x, y⟫) + ‖y‖^2 :=
begin
 repeat {rw [sq]; rw [ ←@inner_self_eq_norm_mul_norm 𝕜]},
...
by rw [sub_eq_add_neg]; rw [ @norm_add_sq 𝕜 _ _ _ _ x (-y)]; rw [ norm_neg]; rw [ inner_neg_right]; rw [ map_neg]; rw [ mul_neg]; rw [ sub_eq_add_neg]
...
begin
 rw [norm_eq_sqrt_inner x]; rw [ norm_eq_sqrt_inner y],
...
by { rw [@norm_add_mul_self 𝕜]; rw [ @norm_sub_mul_self 𝕜], ring }
...
calc dist ((R / ‖x‖) ^ 2 • x) ((R / ‖y‖) ^ 2 • y)
 ...
 by rw [dist_eq_norm]; rw [ sqrt_sq (norm_nonneg _)]
...
... = (R ^ 2 / (‖x‖ * ‖y‖)) * dist x y :
 by rw [sqrt_mul (sq_nonneg _)]; rw [ sqrt_sq (norm_nonneg _)]; rw [ sqrt_sq (div_nonneg (sq_nonneg _) (mul_nonneg (norm_nonneg _) (norm_nonneg _)))]; rw [ dist_eq_norm]
...
⟨λ ε hε, begin
 ...
 rw [sq]; rw [ eq_sub_iff_add_eq.2 (parallelogram_law_with_norm ℝ x y)]; rw [ ←sq (‖x - y‖)]; rw [ hx]; rw [ hy],
 norm_num,
...
begin
 ...
 rw [linear_map.sub_apply]; rw [ inner_sub_left]; rw [ sub_eq_zero],
end
...
f.isometry_of_inner $ λ x y, by rw [←v.total_repr x]; rw [ ←v.total_repr y]; rw [ finsupp.apply_total]; rw [ finsupp.apply_total]; rw [ hv.inner_finsupp_eq_sum_left]; rw [ hf.inner_finsupp_eq_sum_left]
...
f.isometry_of_inner $ λ x y, begin
 ...
 rw [←v.total_repr x]; rw [ ←v.total_repr y]; rw [ ←linear_equiv.coe_coe]; rw [ finsupp.apply_total]; rw [ finsupp.apply_total]; rw [ hv.inner_finsupp_eq_sum_left]; rw [ hf.inner_finsupp_eq_sum_left]
end
...
begin
 rw [@norm_add_mul_self ℝ]; rw [ add_right_cancel_iff]; rw [ add_right_eq_self]; rw [ mul_eq_zero],
 norm_num
end
...
by rw [←norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]; rw [ eq_comm]; rw [ sqrt_eq_iff_mul_self_eq (add_nonneg (mul_self_nonneg _) (mul_self_nonneg _)) (norm_nonneg _)]

/-- Pythagorean theorem, vector inner product form. -/
lemma norm_add_sq_eq_norm_sq_add_norm_sq_of_inner_eq_zero (x y : E) (h : ⟪x, y⟫ = 0) :
 ‖x + y‖ * ‖x + y‖ = ‖x‖ * ‖x‖ + ‖y‖ * ‖y‖ :=
begin
 rw [@norm_add_mul_self 𝕜]; rw [ add_right_cancel_iff]; rw [ add_right_eq_self]; rw [ mul_eq_zero],
 apply or.inr,
 simp only [h, zero_re'],
end
...
lemma norm_sub_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero (x y : F) :
 ‖x - y‖ * ‖x - y‖ = ‖x‖ * ‖x‖ + ‖y‖ * ‖y‖ ↔ ⟪x, y⟫_ℝ = 0 :=
begin
 rw [@norm_sub_mul_self ℝ]; rw [ add_right_cancel_iff]; rw [ sub_eq_add_neg]; rw [ add_right_eq_self]; rw [ neg_eq_zero]; rw [ mul_eq_zero],
 norm_num
end
...
lemma norm_sub_eq_sqrt_iff_real_inner_eq_zero {x y : F} :
 ‖x - y‖ = sqrt (‖x‖ * ‖x‖ + ‖y‖ * ‖y‖) ↔ ⟪x, y⟫_ℝ = 0 :=
by rw [←norm_sub_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero]; rw [ eq_comm]; rw [ sqrt_eq_iff_mul_self_eq (add_nonneg (mul_self_nonneg _) (mul_self_nonneg _)) (norm_nonneg _)]
...
by rw [real_inner_smul_left]; rw [ ←real_inner_self_eq_norm_mul_norm]

/-- The inner product of a vector with a multiple of itself. -/
lemma real_inner_smul_self_right (x : F) (r : ℝ) : ⟪x, r • x⟫_ℝ = r * (‖x‖ * ‖x‖) :=
by rw [inner_smul_right]; rw [ ←real_inner_self_eq_norm_mul_norm]
...
begin
 ...
 rw [← mul_assoc]; rw [ ← div_div]; rw [ mul_div_cancel _ hx']; rw [ ← div_div]; rw [ mul_comm]; rw [ mul_div_cancel _ hr']; rw [ div_self hx'],
end
...
begin
 ...
 rw [← sq_eq_sq (norm_nonneg _) (mul_nonneg (norm_nonneg _) (norm_nonneg _))] at h; rw [ mul_pow] at h; rw [ ← mul_right_inj' this] at h; rw [ eq_comm] at h; rw [ ← sub_eq_zero] at h; rw [ ← mul_sub] at h,
 simp only [@norm_sq_eq_inner 𝕜] at h,
 letI : inner_product_space.core 𝕜 E := inner_product_space.to_core,
 erw [← inner_product_space.core.cauchy_schwarz_aux]; erw [ inner_product_space.core.norm_sq_eq_zero]; erw [ sub_eq_zero] at h,
...
begin
 have h₀' := h₀,
 rw [← norm_ne_zero_iff] at h₀'; rw [ ne.def] at h₀'; rw [ ← @of_real_eq_zero 𝕜] at h₀',
 split; intro h,
 ...
 { conv_lhs { rw [← h]; rw [ inner_smul_right]; rw [ inner_self_eq_norm_sq_to_K] },
...
begin
 ...
 { simp },
 { rw [inner_eq_norm_mul_iff_div h₀]; rw [ div_eq_inv_mul]; rw [ mul_smul]; rw [ inv_smul_eq_iff₀],
 rwa [ne.def]; rwa [ of_real_eq_zero]; rwa [ norm_eq_zero] },
end
...
lemma real_inner_div_norm_mul_norm_eq_neg_one_iff (x y : F) :
 ⟪x, y⟫_ℝ / (‖x‖ * ‖y‖) = -1 ↔ (x ≠ 0 ∧ ∃ (r : ℝ), r < 0 ∧ y = r • x) :=
begin
 rw [← neg_eq_iff_eq_neg]; rw [ ← neg_div]; rw [ ← inner_neg_right]; rw [ ← norm_neg y]; rw [ real_inner_div_norm_mul_norm_eq_one_iff]; rw [ (@neg_surjective ℝ _).exists],
...
begin
 ...
 calc ‖x‖ * ‖x‖ = ‖(⟪x, x⟫ : 𝕜)‖ : by rw [← sq]; rw [ inner_self_eq_norm_sq_to_K]; rw [ norm_pow]; rw [ norm_of_real]; rw [ abs_norm]
...
{ inner := λ x y, conj x * y,
 ...
 by { unfold inner, rw [mul_comm]; rw [ mul_conj]; rw [ of_real_re]; rw [ norm_sq_eq_def'] },
...
begin
 ...
 { rw [← hV.norm_sq_diff_sum]; rw [ sq_lt_sq]; rw [ abs_of_nonneg (sqrt_nonneg _)]; rw [ abs_of_nonneg (norm_nonneg _)],
 exact H s₁ hs₁ s₂ hs₂ },
...
begin
 ...
 rw [← @linear_map.ker_eq_bot _ _ _ _ _ _ (direct_sum.add_comm_group (λ i, V i))]; rw [ submodule.eq_bot_iff],
 intros v hv,
...
by rw [← complex.inner]; rw [ f.inner_map_map]
...
begin
 ...
 rw [completion.has_inner]; rw [ uncurry_curry _],
