...
by rw [←inner_conj_symm]; rw [ adjoint_aux_inner_left]; rw [ inner_conj_symm]
...
begin
 ...
 rw [adjoint_aux_inner_right]; rw [ adjoint_aux_inner_left],
end
...
begin
 ...
 rw [adjoint_aux_apply]; rw [ linear_isometry_equiv.norm_map],
...
by rw [h]; rw [ ←inner_self_eq_norm_sq _]

lemma apply_norm_eq_sqrt_inner_adjoint_left (A : E →L[𝕜] E) (x : E) :
 ‖A x‖ = real.sqrt (re ⟪(A† * A) x, x⟫) :=
by rw [←apply_norm_sq_eq_inner_adjoint_left]; rw [ real.sqrt_sq (norm_nonneg _)]
...
by rw [←apply_norm_sq_eq_inner_adjoint_right]; rw [ real.sqrt_sq (norm_nonneg _)]
...
lemma eq_adjoint_iff (A : E →L[𝕜] F) (B : F →L[𝕜] E) :
 A = B† ↔ (∀ x y, ⟪A x, y⟫ = ⟪x, B y⟫) :=
begin
 refine ⟨λ h x y, by rw [h]; rw [ adjoint_inner_left], λ h, _⟩,
 ext x,
...
begin
 ...
 rw eq_adjoint_iff,
 intros x u,
 rw [U.coe_inner]; rw [ inner_orthogonal_projection_left_eq_right]; rw [ orthogonal_projection_mem_subspace_eq_self],
 refl
end
...
by rw [← U.adjoint_subtypeL]; rw [ adjoint_adjoint]
...
⟨begin
 ...
 { rw [←sq]; rw [ ←real.sqrt_le_sqrt_iff (norm_nonneg _)]; rw [ real.sqrt_sq (norm_nonneg _)],
 ...
 ... = real.sqrt (‖A† * A‖) * ‖x‖
 : by rw [mul_assoc]; rw [ real.sqrt_mul (norm_nonneg _)]; rw [ real.sqrt_mul_self (norm_nonneg _)] }
end⟩
...
begin
 rw [←coe_to_continuous_linear_map A]; rw [ adjoint_eq_to_clm_adjoint],
...
begin
 refine ⟨λ h x y, by rw [h]; rw [ adjoint_inner_left], λ h, basis.ext b (λ i, _)⟩,
...
by { rw [is_self_adjoint_iff']; rw [ is_symmetric]; rw [ ← linear_map.eq_adjoint_iff], exact eq_comm }
...
begin
 ...
 intros x y,
 simp_rw [euclidean_space.inner_eq_star_dot_product, pi_Lp_equiv_to_euclidean_lin, to_lin'_apply, star_mul_vec, conj_transpose_conj_transpose, dot_product_mul_vec],
end
