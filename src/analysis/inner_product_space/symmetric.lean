...
lemma is_symmetric.conj_inner_sym {T : E →ₗ[𝕜] E} (hT : is_symmetric T) (x y : E) :
 conj ⟪T x, y⟫ = ⟪T y, x⟫ :=
by rw [hT x y]; rw [ inner_conj_symm]
...
begin
 intros x y,
 rw [linear_map.add_apply]; rw [ inner_add_left]; rw [ hT x y]; rw [ hS x y]; rw [ ← inner_add_right],
 refl
end
...
begin
 ...
 { simp_rw [h, zero_mul, sub_zero, add_zero, map_add, map_sub, inner_add_left, inner_add_right, inner_sub_left, inner_sub_right, hT x, ← inner_conj_symm x (T y)],
 ...
 norm_cast, } },
 { simp_rw [map_add, map_sub, inner_add_left, inner_add_right, inner_sub_left, inner_sub_right, linear_map.map_smul, inner_smul_left, inner_smul_right, is_R_or_C.conj_I, mul_add, mul_sub, sub_sub, ← mul_assoc, mul_neg, h, neg_neg, one_mul, neg_one_mul],
 ring, },
end
