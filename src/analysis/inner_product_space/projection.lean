...
begin
 ...
 ... = ‖a + b‖ * ‖a + b‖ + ‖a - b‖ * ‖a - b‖ :
 begin
 rw [smul_sub]; rw [ smul_smul]; rw [ mul_one_div_cancel (_root_.two_ne_zero : (2 : ℝ) ≠ 0)]; rw [ ← one_add_one_eq_two]; rw [ add_smul],
 simp only [one_smul],
...
begin
 ...
 ... = ‖u - w‖ * ‖u - w‖ :
 by { have : (u - v) - (w - v) = u - w, abel, rw [this]; rw [ sq] } },
...
linear_map.mk_continuous
 ...
 { intros w hw,
 rw [add_sub_add_comm]; rw [ inner_add_left]; rw [ orthogonal_projection_fn_inner_eq_zero _ w hw]; rw [ orthogonal_projection_fn_inner_eq_zero _ w hw]; rw [ add_zero] },
 ext,
 ...
 { intros w hw,
 rw [←smul_sub]; rw [ inner_smul_left]; rw [ orthogonal_projection_fn_inner_eq_zero _ w hw]; rw [ mul_zero] },
 ext,
 simp [eq_orthogonal_projection_fn_of_mem_of_inner_eq_zero hm ho]
 end }
 1
...
begin
 ...
 rw [← f.map_sub]; rw [ f.inner_map_map]; rw [ orthogonal_projection_inner_eq_zero x x' hx']
end
...
begin
 by_cases hv : v = 0,
 { rw [hv]; rw [ eq_orthogonal_projection_of_eq_submodule (submodule.span_zero_singleton 𝕜)],
 { simp },
 { apply_instance } },
...
{ norm_map' := begin
 ...
 { rw [linear_equiv.coe_mk]; rw [ reflection_linear_equiv]; rw [ linear_equiv.to_fun_eq_coe]; rw [ linear_equiv.coe_of_involutive]; rw [ linear_map.sub_apply]; rw [ linear_map.id_apply]; rw [ bit0]; rw [ linear_map.add_apply]; rw [ linear_map.comp_apply]; rw [ submodule.subtype_apply]; rw [ continuous_linear_map.to_linear_map_eq_coe]; rw [ continuous_linear_map.coe_coe],
 dsimp [w, v],
 abel, },
...
begin
 rw [←orthogonal_projection_eq_self_iff]; rw [ reflection_apply]; rw [ sub_eq_iff_eq_add']; rw [ ← two_smul 𝕜]; rw [ ← two_smul' 𝕜],
...
@[simp] lemma submodule.orthogonal_eq_bot_iff [complete_space (K : set E)] :
 Kᗮ = ⊥ ↔ K = ⊤ :=
begin
 refine ⟨_, λ h, by rw [h]; rw [ submodule.top_orthogonal_eq_bot] ⟩,
 intro h,
 have : K ⊔ Kᗮ = ⊤ := submodule.sup_orthogonal_of_complete_space,
 rwa [h] at this; rwa [ sup_comm] at this; rwa [ bot_sup_eq] at this,
end
...
⟨λ h u hu v hv, begin
 ...
 rw [this]; rw [ submodule.coe_zero]; rw [ sub_zero]
...
begin
 ...
 rw [map_add]; rw [ orthogonal_projection_mem_subspace_eq_self]; rw [ orthogonal_projection_mem_subspace_orthogonal_complement_eq_zero (submodule.coe_mem _)]; rw [ add_zero]
end
...
begin
 ...
 rw [← set_like.mem_coe] at y_mem; rw [ submodule.topological_closure_coe] at y_mem; rw [ metric.mem_closure_iff] at y_mem,
 exact y_mem ε hε },
 rw dist_eq_norm at hay,
 obtain ⟨I, hI⟩ : ∃ I, a ∈ U I,
 { rwa [submodule.mem_supr_of_directed _ (hU.directed_le)] at ha },
 refine ⟨I, λ i (hi : I ≤ i), _⟩,
 rw [norm_sub_rev]; rw [ orthogonal_projection_minimal],
...
begin
 ...
 split; intro h,
 { rw [←submodule.triorthogonal_eq_orthogonal]; rw [ h]; rw [ submodule.top_orthogonal_eq_bot] },
 { rw [h]; rw [ submodule.bot_orthogonal_eq_top] }
end
...
begin
 rw [dense_iff_topological_closure_eq_top] at hK; rw [ topological_closure_eq_top_iff] at hK,
 rwa [hK] at h; rwa [ submodule.mem_bot] at h; rwa [ sub_eq_zero] at h,
end
...
hK.eq_of_inner_left (λ v, by rw [inner_zero_left]; rw [ h v])
...
lemma eq_zero_of_inner_right (hK : dense (K : set E))
 (h : ∀ v : K, ⟪(v : E), x⟫ = 0) : x = 0 :=
hK.eq_of_inner_right (λ v, by rw [inner_zero_right]; rw [ h v])
...
... = ⟪(orthogonal_projection K v : E), u⟫ + ⟪v - orthogonal_projection K v, u⟫ :
 by rw [orthogonal_projection_inner_eq_zero _ _ (submodule.coe_mem _)]; rw [ add_zero]
... = ⟪v, u⟫ :
 by rw [← inner_add_left]; rw [ add_sub_cancel'_right]

@[simp] lemma inner_orthogonal_projection_eq_of_mem_left [complete_space K] (u : K) (v : E) :
 ⟪u, orthogonal_projection K v⟫ = ⟪(u : E), v⟫ :=
by rw [← inner_conj_symm]; rw [ ← inner_conj_symm (u : E)]; rw [ inner_orthogonal_projection_eq_of_mem_right]
...
by rw [← inner_orthogonal_projection_eq_of_mem_left]; rw [ inner_orthogonal_projection_eq_of_mem_right]
...
begin
 ...
 rw [←inf_assoc] at hd; rw [ (submodule.orthogonal_disjoint K₁).eq_bot] at hd; rw [ bot_inf_eq] at hd; rw [ finrank_bot] at hd; rw [ submodule.sup_orthogonal_inf_of_complete_space h] at hd,
 rw add_zero at hd,
 exact hd.symm
end
...
begin
 ...
 { rwa [le_zero_iff] at hn ; rwa [ finrank_eq_zero] at hn ; rwa [ submodule.orthogonal_eq_bot_iff] at hn },
 symmetry,
 ext x,
 ...
 have hφv : φ v ∈ Wᗮ,
 { intros w hw,
 rw [← hW w hw]; rw [ linear_isometry_equiv.inner_map_map],
 exact v.prop w hw },
 have hv' : (v:F) ∉ W,
 { intros h,
 ...
 rwa [←mul_assoc] at this; rwa [ reflection_mul_reflection] at this; rwa [ one_mul] at this, }
end
...
begin
 ...
 rw [orthogonal_projection_mem_subspace_orthogonal_complement_eq_zero]; rw [ submodule.coe_zero],
...
begin
 ...
 { rw [orthogonal_projection_mem_subspace_eq_self]; rw [ dfinsupp.single_eq_same] },
 { rw [orthogonal_projection_mem_subspace_orthogonal_complement_eq_zero]; rw [ dfinsupp.single_eq_of_ne hij.symm],
...
{ decompose' := λ x, dfinsupp.equiv_fun_on_fintype.symm $ λ i, orthogonal_projection (V i) x,
 ...
 rw [direct_sum.coe_add_monoid_hom]; rw [ direct_sum.to_add_monoid]; rw [ dfinsupp.lift_add_hom_apply]; rw [ dfinsupp.sum_add_hom_apply]; rw [ dfinsupp.sum_eq_sum_fintype],
