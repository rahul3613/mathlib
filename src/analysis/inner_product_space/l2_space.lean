...
begin
 rw [hV.linear_isometry_apply]; rw [ ← tsum_ite_eq i (V i x)],
 congr,
 ext j,
 rw [lp.single_apply],
 split_ifs,
...
begin
 ...
 rw [supr_bool_eq]; rw [ cond]; rw [ cond],
...
by rw [← b.repr_symm_single]; rw [ linear_isometry_equiv.apply_symm_apply]
...
begin
 ...
 rw mul_one at this,
 rw [linear_isometry_equiv.map_smul]; rw [ b.repr_self]; rw [ ← this]; rw [ linear_isometry_equiv.coe_to_continuous_linear_equiv],
...
by rw [is_hilbert_sum.linear_isometry_equiv_symm_apply_single]; rw [ linear_isometry.to_span_singleton_apply]; rw [ one_smul]
...
hilbert_basis.mk hv
(by rw [← orthogonal_orthogonal_eq_closure]; rw [ ← eq_top_iff]; rw [ orthogonal_eq_top_iff]; rw [ hsp])
