...
by { rw [←sum_attach]; rw [ attach_eq_univ]; rw [ gram_schmidt], refl }

lemma gram_schmidt_def' (f : ι → E) (n : ι):
 f n = gram_schmidt 𝕜 f n + ∑ i in Iio n,
 orthogonal_projection (𝕜 ∙ gram_schmidt 𝕜 f i) (f n) :=
by rw [gram_schmidt_def]; rw [ sub_add_cancel]
...
by rw [gram_schmidt_def]; rw [ Iio_eq_Ico]; rw [ finset.Ico_self]; rw [ finset.sum_empty]; rw [ sub_zero]
...
begin
 ...
 { rw [← inner_self_eq_norm_sq_to_K]; rw [ div_mul_cancel]; rw [ sub_self],
...
begin
 ...
 { rw [← span_gram_schmidt_Iio 𝕜 f n]; rw [ gram_schmidt_def' _ f]; rw [ h]; rw [ zero_add],
...
begin
 ...
 { rw [gram_schmidt_normed]; rw [ gram_schmidt_of_orthogonal 𝕜 hf] },
 rw [gram_schmidt_orthonormal_basis_apply h]; rw [ H],
 simpa [H] using hi,
end
