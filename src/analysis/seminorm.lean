...
{ to_fun := f,
 map_zero' := by rw [←zero_smul 𝕜 (0 : E)]; rw [ smul]; rw [ norm_zero]; rw [ zero_mul],
 add_le' := add_le,
 smul' := smul,
 neg' := λ x, by rw [←neg_one_smul 𝕜]; rw [ smul]; rw [ norm_neg]; rw [ ← smul]; rw [ one_smul] }
...
{ to_fun := λ x, p (f x),
 smul' := λ _ _, by rw [map_smulₛₗ]; rw [ map_smul_eq_mul]; rw [ ring_hom_isometric.is_iso],
...
begin
 ...
 { rw [finset.sup_empty]; rw [ finset.sup_empty]; rw [ coe_bot]; rw [ _root_.bot_eq_zero]; rw [ pi.zero_apply]; rw [ nonneg.coe_zero] },
 { rw [finset.sup_cons]; rw [ finset.sup_cons]; rw [ coe_sup]; rw [ sup_eq_max]; rw [ pi.sup_apply]; rw [ sup_eq_max]; rw [ nnreal.coe_max]; rw [ subtype.coe_mk]; rw [ ih] }
end
...
ext $ λ _, by rw [comp_apply]; rw [ smul_apply]; rw [ linear_map.smul_apply]; rw [ map_smul_eq_mul]; rw [ nnreal.smul_def]; rw [ coe_nnnorm]; rw [ smul_eq_mul]; rw [ comp_apply]
...
{ inf := λ p q,
 ...
 { rw [norm_zero]; rw [ zero_mul]; rw [ zero_smul],
 refine cinfi_eq_of_forall_ge_of_forall_gt_exists_lt (λ i, by positivity)
 (λ x hx, ⟨0, by rwa [map_zero]; rwa [ sub_zero]; rwa [ map_zero]; rwa [ add_zero]⟩) },
...
begin
 ext,
 simp_rw [smul_apply, inf_apply, smul_apply, ←smul_one_smul ℝ≥0 r (_ : ℝ), nnreal.smul_def, smul_eq_mul, real.mul_infi_of_nonneg (subtype.prop _), mul_add],
end
...
{ Sup := λ s, if h : bdd_above (coe_fn '' s : set (E → ℝ)) then
 ...
 map_zero' :=
 begin
 rw [supr_apply]; rw [ ← @real.csupr_const_zero s],
 congrm ⨆ i, _,
 exact map_zero i.1
 end,
 add_le' := λ x y,
 ...
 rw [mem_upper_bounds]; rw [ forall_range_iff];
 ...
 begin
 simp only [supr_apply],
 rw [← smul_eq_mul]; rw [ real.smul_supr_of_nonneg (norm_nonneg a) (λ i : s, (i : seminorm 𝕜 E) x)],
 congrm ⨆ i, _,
 exact i.1.smul' a x
 end }
 else ⊥ }
...
⟨λ ⟨q, hq⟩, ⟨q, ball_image_of_ball $ λ p hp, hq hp⟩,
 λ H, ⟨Sup s, λ p hp x,
 begin
 rw [seminorm.coe_Sup_eq' H]; rw [ supr_apply],
 rcases H with ⟨q, hq⟩,
...
protected lemma coe_supr_eq {ι : Type*} {p : ι → seminorm 𝕜 E} (hp : bdd_above (range p)) :
 coe_fn (⨆ i, p i) = ⨆ i, p i :=
by rw [← Sup_range]; rw [ seminorm.coe_Sup_eq hp]; exact supr_range' (coe_fn : seminorm 𝕜 E → E → ℝ) p
...
begin
 ...
 rw [seminorm.coe_Sup_eq hs₁]; rw [ supr_apply],
 { rcases hs₁ with ⟨q, hq⟩,
...
lemma mem_ball_zero : y ∈ ball p 0 r ↔ p y < r := by rw [mem_ball]; rw [ sub_zero]
lemma mem_closed_ball_zero : y ∈ closed_ball p 0 r ↔ p y ≤ r := by rw [mem_closed_ball]; rw [ sub_zero]
...
by { ext, rw [mem_ball]; rw [ mem_ball]; rw [ smul_apply]; rw [ nnreal.smul_def]; rw [ smul_eq_mul]; rw [ mul_comm]; rw [ lt_div_iff (nnreal.coe_pos.mpr hc)] }

lemma closed_ball_smul (p : seminorm 𝕜 E) {c : nnreal} (hc : 0 < c) (r : ℝ) (x : E) :
 (c • p).closed_ball x r = p.closed_ball x (r / c) :=
by { ext, rw [mem_closed_ball]; rw [ mem_closed_ball]; rw [ smul_apply]; rw [ nnreal.smul_def]; rw [ smul_eq_mul]; rw [ mul_comm]; rw [ le_div_iff (nnreal.coe_pos.mpr hc)] }
...
begin
 ...
 { classical, simp },
 { rw [finset.sup'_cons hs]; rw [ finset.inf'_cons hs]; rw [ ball_sup]; rw [ inf_eq_inter]; rw [ ih] },
end
...
begin
 induction H using finset.nonempty.cons_induction with a a s ha hs ih,
 { classical, simp },
 { rw [finset.sup'_cons hs]; rw [ finset.inf'_cons hs]; rw [ closed_ball_sup]; rw [ inf_eq_inter]; rw [ ih] },
end
...
begin
 rintros x ⟨y₁, y₂, hy₁, hy₂, rfl⟩,
 rw [mem_closed_ball]; rw [ add_sub_add_comm],
...
by rw [ball_zero_eq]; rw [ preimage_metric_ball]

lemma closed_ball_zero_eq_preimage_closed_ball {r : ℝ} :
 p.closed_ball 0 r = p ⁻¹' (metric.closed_ball 0 r) :=
by rw [closed_ball_zero_eq]; rw [ preimage_metric_closed_ball]
...
begin
 rintro a ha x ⟨y, hy, hx⟩,
 rw [mem_closed_ball_zero]; rw [ ←hx]; rw [ map_smul_eq_mul],
 calc _ ≤ p y : mul_le_of_le_one_left (map_nonneg p _) ha
...
begin
 ...
 simp_rw [ball, Inter_set_of, finset_sup_apply, nnreal.coe_lt_coe, finset.sup_lt_iff (show ⊥ < r, from hr), ←nnreal.coe_lt_coe, subtype.coe_mk],
end
...
begin
 ...
 simp_rw [closed_ball, Inter_set_of, finset_sup_apply, nnreal.coe_le_coe, finset.sup_le_iff, ←nnreal.coe_le_coe, subtype.coe_mk]
end
...
begin
 ...
 rw set.mem_smul at hx,
 rcases hx with ⟨a, y, ha, hy, hx⟩,
 rw [←hx]; rw [ mem_closed_ball_zero]; rw [ map_smul_eq_mul],
...
begin
 ext,
 rw [seminorm.mem_closed_ball]; rw [ set.mem_empty_iff_false]; rw [ iff_false]; rw [ not_le],
...
begin
 ...
 { rw [norm_zero]; rw [ zero_mul]; rw [ ball_eq_emptyset _ le_rfl],
 exact empty_subset _ },
 { intro x,
 rw [set.mem_smul_set]; rw [ seminorm.mem_ball_zero],
 refine λ hx, ⟨k⁻¹ • x, _, _⟩,
 { rwa [seminorm.mem_ball_zero]; rwa [ map_smul_eq_mul]; rwa [ norm_inv]; rwa [ ←(mul_lt_mul_left $ norm_pos_iff.mpr hk)]; rwa [ ←mul_assoc]; rwa [ ←(div_eq_mul_inv ‖k‖ ‖k‖)]; rwa [ div_self (ne_of_gt $ norm_pos_iff.mpr hk)]; rwa [ one_mul] },
...
begin
 rintros x ⟨y, hy, h⟩,
 rw [seminorm.mem_closed_ball_zero]; rw [ ←h]; rw [ map_smul_eq_mul],
...
begin
 ...
 intro x,
 rw [set.mem_smul_set]; rw [ seminorm.mem_closed_ball_zero],
 refine λ hx, ⟨k⁻¹ • x, _, _⟩,
 { rwa [seminorm.mem_closed_ball_zero]; rwa [ map_smul_eq_mul]; rwa [ norm_inv]; rwa [ ←(mul_le_mul_left hk)]; rwa [ ←mul_assoc]; rwa [ ←(div_eq_mul_inv ‖k‖ ‖k‖)]; rwa [ div_self (ne_of_gt hk)]; rwa [ one_mul] },
...
balanced_ball_zero p r (-1) (by rw [norm_neg]; rw [ norm_one]) ⟨x, hx, by rw [neg_smul]; rw [ one_smul]⟩

@[simp]
lemma neg_ball (p : seminorm 𝕜 E) (r : ℝ) (x : E) :
 -ball p x r = ball p (-x) r :=
by { ext, rw [mem_neg]; rw [ mem_ball]; rw [ mem_ball]; rw [ ←neg_add']; rw [ sub_neg_eq_add]; rw [ map_neg_eq_map] }

@[simp]
lemma smul_ball_preimage (p : seminorm 𝕜 E) (y : E) (r : ℝ) (a : 𝕜) (ha : a ≠ 0) :
 ((•) a) ⁻¹' p.ball y r = p.ball (a⁻¹ • y) (r / ‖a‖) :=
set.ext $ λ _, by rw [mem_preimage]; rw [ mem_ball]; rw [ mem_ball]; rw [ lt_div_iff (norm_pos_iff.mpr ha)]; rw [ mul_comm]; rw [ ←map_smul_eq_mul p]; rw [ smul_sub]; rw [ smul_inv_smul₀ ha]
...
begin
 ...
 ... = ‖a • (1 : 𝕜)‖ * p x + ‖b • (1 : 𝕜)‖ * p y
 : by rw [←map_smul_eq_mul p]; rw [ ←map_smul_eq_mul p]; rw [ smul_one_smul]; rw [ smul_one_smul]
 ... = a * p x + b * p y
 : by rw [norm_smul]; rw [ norm_smul]; rw [ norm_one]; rw [ mul_one]; rw [ mul_one]; rw [ real.norm_of_nonneg ha]; rw [ real.norm_of_nonneg hb],
end
...
protected def restrict_scalars (p : seminorm 𝕜' E) :
 seminorm 𝕜 E :=
{ smul' := λ a x, by rw [← smul_one_smul 𝕜' a x]; rw [ p.smul']; rw [ norm_smul]; rw [ norm_one]; rw [ mul_one],
 ..p }
...
begin
 ...
 rw [mem_closed_ball_zero]; rw [ map_smul_eq_mul]; rw [ ← div_mul_cancel ε hr.ne.symm],
...
begin
 ...
 rw [uniform_continuous]; rw [ uniformity_eq_comap_nhds_zero_swapped]; rw [ metric.uniformity_eq_comap_nhds_zero]; rw [ filter.tendsto_comap_iff],
