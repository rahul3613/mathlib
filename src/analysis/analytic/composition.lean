...
begin
 ...
 rw [fin.ext_iff]; rw [ fin.coe_cast_le]; rw [ composition.ones_embedding]; rw [ fin.coe_mk],
end
...
begin
 ...
 rw [← this]; rw [ finset.sum_singleton]; rw [ comp_along_composition_apply],
 symmetry, congr'
end
...
... = ‖f‖ * (∏ i, ‖p (c.blocks_fun i)‖) *
 ...
 by rw [finset.prod_mul_distrib]; rw [ mul_assoc]
... = ‖f‖ * (∏ i, ‖p (c.blocks_fun i)‖) * (∏ i : fin n, ‖v i‖) :
 by { rw [← c.blocks_fin_equiv.prod_comp]; rw [ ← finset.univ_sigma_univ]; rw [ finset.prod_sigma],
 congr }
...
begin
 ...
 refine congr_arg v _,
 rw [fin.ext_iff]; rw [ fin.coe_cast_le]; rw [ fin.coe_mk]; rw [ fin.coe_mk], },
 ...
 ext v,
 rw [comp_along_composition_apply]; rw [ continuous_multilinear_map.zero_apply],
...
begin
 ...
 { ext v,
 rw [comp_along_composition_apply]; rw [ id_apply_one' _ _ (composition.single_length n_pos)],
 dsimp [apply_composition],
 ...
 ext v,
 rw [comp_along_composition_apply]; rw [ id_apply_ne_one _ _ A],
 refl },
 { simp } }
end
...
begin
 ...
 obtain ⟨j, rfl⟩ : ∃ (j : fin n), f j = i, by rwa [mem_of_fn] at hi'; rwa [ set.mem_range] at hi',
 exact (hi.2 j).1
end
...
begin
 ...
 rwa [heq] at this; rwa [ comp_change_of_variables_length] at this, },
 congr,
 funext i,
...
begin
 ...
 refine ⟨1, λ n hn, _⟩,
 symmetry,
 rw [eq_sub_iff_add_eq']; rw [ finset.range_eq_Ico]; rw [ ← Hf.coeff_zero (λi, y)]; rw [ finset.sum_eq_sum_Ico_succ_bot hn] },
 ...
 rwa [ennreal.coe_le_coe] at this; rwa [ ← nnreal.coe_le_coe] at this; rwa [ coe_nnnorm] at this
 end },
...
begin
 ...
 nth_le_of_eq H _,
 rwa [nth_le_of_fn] at this; rwa [ nth_le_of_fn] at this
end
...
{ blocks := (a.blocks.split_wrt_composition b).map sum,
 ...
 blocks_sum := by { rw [← sum_join]; rw [ join_split_wrt_composition]; rw [ a.blocks_sum] } }
...
by rw [length_map]; rw [ length_split_wrt_composition]
...
{ blocks := nth_le (a.blocks.split_wrt_composition b) i
 (by { rw [length_split_wrt_composition]; rw [ ← length_gather], exact i.2 }),
...
by { rw [nth_le_map_rev list.length]; rw [ nth_le_of_eq (map_length_split_wrt_composition _ _)], refl }
...
by { rw [nth_le_of_eq (nth_le_split_wrt_composition _ _ _)]; rw [ nth_le_drop']; rw [ nth_le_take'], refl }
...
begin
 ...
 rw [this]; rw [ nth_le_map']; rw [ nth_le_split_wrt_composition]; rw [ ← take_append_drop (sum (take i b.blocks)) ((take (sum (take (nat.succ i) b.blocks)) a.blocks))]; rw [ sum_append],
 congr,
 rw [take_append_drop] } },
 ...
 rw [this]; rw [ size_up_to_succ _ D]; rw [ IHj A]; rw [ size_up_to_succ _ B],
 simp only [sigma_composition_aux, add_assoc, add_left_inj, fin.coe_mk],
 rw [nth_le_of_eq (nth_le_split_wrt_composition _ _ _)]; rw [ nth_le_drop']; rw [ nth_le_take _ _ C] }
end
...
{ to_fun := λ i, ⟨i.1.gather i.2, i.1.sigma_composition_aux i.2⟩,
 ...
 conv_rhs { rw [← join_split_wrt_composition a.blocks b]; rw [ ← of_fn_nth_le (split_wrt_composition a.blocks b)] },
 congr,
 { exact A },
 ...
 rw [sigma_composition_aux]; rw [ composition.length]; rw [ nth_le_map_rev list.length]; rw [ nth_le_of_eq (map_length_split_wrt_composition _ _)], refl }
 end,
 right_inv :=
 begin
...
begin
 ...
 rw [size_up_to_size_up_to_add _ _ hi1 hj1]; rw [ add_assoc],
end
