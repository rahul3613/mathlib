...
begin
 ...
 ... ≤ C * (a * (‖y‖ / r')) ^ n : by rw [mul_pow]; rw [ mul_assoc]
end
...
continuous_multilinear_map.curry_fin_finset 𝕜 E F hs
 (by erw [finset.card_compl]; erw [ fintype.card_fin]; erw [ hs]; erw [ add_tsub_cancel_right]) (p $ k + l)
...
begin
 rw [← p.nnnorm_change_origin_series_term k l s hs]; rw [ ← fin.prod_const]; rw [ ← fin.prod_const],
...
begin
 ...
 { ext1 s, rw [tsub_add_cancel_of_le (card_finset_fin_le _)]; rw [ mul_assoc] },
...
begin
 ...
 rwa [lt_tsub_iff_right]; rwa [ add_comm] },
...
{ r_le := begin
 ...
 rw [p.change_origin_eval (hz.trans_le hf.r_le)]; rw [ add_assoc]; rw [ hf.sum],
...
begin
 rw [← map_add_left_nhds_zero]; rw [ eventually_map]; rw [ has_fpower_series_at_iff],
