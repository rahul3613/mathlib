...
begin
 ...
 rw [prod_mul_distrib]; rw [ prod_mul_distrib]; rw [ prod_pow_eq_pow_sum]; rw [ composition.sum_blocks_fun]; rw [ prod_const]; rw [ card_fin],
 ring,
end
...
... = ∑ j in Ico 2 (n+1), r ^ j * (∑ k in Ico 1 n, a ^ k * p k) ^ j :
begin
 rw [comp_partial_sum_source]; rw [ ← sum_sigma' (Ico 2 (n + 1)) (λ (k : ℕ), (fintype.pi_finset (λ (i : fin k), Ico 1 n) : finset (fin k → ℕ))) (λ n e, ∏ (j : fin n), r * (a ^ e j * p (e j)))],
...
begin
 congr' 1,
 apply sum_congr rfl (λ j hj, _),
 rw [right_inv_coeff _ _ _ (mem_Ico.1 hj).1]; rw [ norm_neg],
end
...
begin
 simp_rw [mul_assoc C, ← mul_sum, ← mul_assoc, mul_comm _ (‖↑i.symm‖), mul_assoc, ← mul_sum, ← mul_assoc, mul_comm _ C, mul_assoc, ← mul_sum],
 ring,
end
