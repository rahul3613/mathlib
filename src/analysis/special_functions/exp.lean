...
begin
 ...
 rw [← sub_le_iff_le_add']; rw [ ← norm_smul z (_ : ℂ)],
...
by rw [eq_comm]; rw [ sqrt_eq_iff_sq_eq]; rw [ sq]; rw [ ← exp_add]; rw [ add_halves]; exact (exp_pos _).le
...
begin
 ...
 ... = exp x / C : by rw [add_comm]; rw [ exp_add]; rw [ mul_div_mul_left _ _ (exp_pos _).ne']
end
...
(tendsto_inv_at_top_zero.comp (tendsto_exp_div_pow_at_top n)).congr $ λx,
 by rw [comp_app]; rw [ inv_eq_one_div]; rw [ div_div_eq_mul_div]; rw [ one_mul]; rw [ div_eq_mul_inv]; rw [ exp_neg]
...
by rw [← coe_comp_exp_order_iso]; rw [ range_comp]; rw [ exp_order_iso.range_eq]; rw [ image_univ]; rw [ subtype.range_coe]

@[simp] lemma map_exp_at_top : map exp at_top = at_top :=
by rw [← coe_comp_exp_order_iso]; rw [ ← filter.map_map]; rw [ order_iso.map_at_top]; rw [ map_coe_Ioi_at_top]

@[simp] lemma comap_exp_at_top : comap exp at_top = at_top :=
by rw [← map_exp_at_top]; rw [ comap_map exp_injective]; rw [ map_exp_at_top]

@[simp] lemma tendsto_exp_comp_at_top {f : α → ℝ} :
 tendsto (λ x, exp (f x)) l at_top ↔ tendsto f l at_top :=
by rw [← tendsto_comap_iff]; rw [ comap_exp_at_top]

lemma tendsto_comp_exp_at_top {f : ℝ → α} :
 tendsto (λ x, f (exp x)) at_top l ↔ tendsto f at_top l :=
by rw [← tendsto_map'_iff]; rw [ map_exp_at_top]

@[simp] lemma map_exp_at_bot : map exp at_bot = 𝓝[>] 0 :=
by rw [← coe_comp_exp_order_iso]; rw [ ← filter.map_map]; rw [ exp_order_iso.map_at_bot]; rw [ ← map_coe_Ioi_at_bot]

@[simp] lemma comap_exp_nhds_within_Ioi_zero : comap exp (𝓝[>] 0) = at_bot :=
by rw [← map_exp_at_bot]; rw [ comap_map exp_injective]

lemma tendsto_comp_exp_at_bot {f : ℝ → α} :
 tendsto (λ x, f (exp x)) at_bot l ↔ tendsto f (𝓝[>] 0) l :=
by rw [← map_exp_at_bot]; rw [ tendsto_map'_iff]
...
by rw [← tendsto_comap_iff]; rw [ comap_exp_nhds_zero]
...
by rw [← tendsto_comap_iff]; rw [ comap_exp_nhds_zero]; rw [ tendsto_comap_iff]
