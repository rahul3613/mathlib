...
... =o[l] re : is_o.add (is_o_const_left.2 $ or.inr $ hl.tendsto_abs_re) $ is_o_iff_nat_mul_le.2 $
 ...
 rwa [max_eq_right hle]; rwa [ real.norm_eq_abs]; rwa [ real.norm_eq_abs]; rwa [ abs_of_pos (real.log_pos H)]; rwa [ ← real.log_pow]; rwa [ real.log_le_iff_le_exp (pow_pos (one_pos.trans H) _)]; rwa [ abs_of_pos (one_pos.trans h₁)] }
 end
...
calc (λ z, z ^ a₁ * exp (b₁ * z)) =ᶠ[l] (λ z, z ^ a₂ * exp (b₁ * z) * z ^ (a₁ - a₂)) :
 ...
 by { simp only, rw [mul_right_comm]; rw [ ← cpow_add _ _ hz]; rw [ add_sub_cancel'_right] }
