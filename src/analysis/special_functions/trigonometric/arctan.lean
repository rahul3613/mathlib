...
by rw [← complex.of_real_ne_zero]; rw [ complex.of_real_tan]; rw [ complex.tan_ne_zero_iff]; norm_cast

lemma tan_eq_zero_iff {θ : ℝ} : tan θ = 0 ↔ ∃ k : ℤ, θ = k * π / 2 :=
by rw [← not_iff_not]; rw [ not_exists]; rw [ ← ne]; rw [ tan_ne_zero_iff]
...
begin
 ...
 have hr_le : r ≤ -1, by rwa [int.lt_iff_add_one_le] at h; rwa [ ← le_neg_iff_add_nonpos_right] at h,
 rw [← le_sub_iff_add_le]; rw [ mul_comm]; rw [ ← le_div_iff],
 { norm_num, rw [← int.cast_one]; rw [ ← int.cast_neg], norm_cast, exact hr_le, },
 { exact zero_lt_two, }, },
end
...
by rw [one_div]; rw [ ← inv_one_add_tan_sq (cos_arctan_pos x).ne']; rw [ tan_arctan]

lemma sin_arctan (x : ℝ) : sin (arctan x) = x / sqrt (1 + x ^ 2) :=
by rw [← tan_div_sqrt_one_add_tan_sq (cos_arctan_pos x)]; rw [ tan_arctan]

lemma cos_arctan (x : ℝ) : cos (arctan x) = 1 / sqrt (1 + x ^ 2) :=
by rw [one_div]; rw [ ← inv_sqrt_one_add_tan_sq (cos_arctan_pos x)]; rw [ tan_arctan]
...
begin
 rw [arctan_eq_arcsin]; rw [ div_pow]; rw [ sq_sqrt]; rw [ one_add_div]; rw [ div_div]; rw [ ← sqrt_mul]; rw [ mul_div_cancel']; rw [ sub_add_cancel]; rw [ sqrt_one]; rw [ div_one];
 nlinarith [h.1, h.2],
end
...
lemma arctan_eq_of_tan_eq {x y : ℝ} (h : tan x = y) (hx : x ∈ Ioo (-(π / 2)) (π / 2)) :
 arctan y = x :=
inj_on_tan (arctan_mem_Ioo _) hx (by rw [tan_arctan]; rw [ h])
...
begin
 ...
 congr' 1,
 rw [←sqrt_inv]; rw [ sq_sqrt]; rw [ ←one_div]; rw [ one_sub_div]; rw [ add_sub_cancel']; rw [ sqrt_div]; rw [ sqrt_sq h],
...
begin
 ...
 { rw [tan_pi_div_two_sub]; rw [ tan_arcsin]; rw [ inv_div] },
