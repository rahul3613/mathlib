...
by rw [← not_exists]; rw [ not_iff_not]; rw [ cos_eq_zero_iff]
...
begin
 ...
 rw mul_assoc at h,
 rw [tan]; rw [ div_eq_zero_iff]; rw [ ← mul_eq_zero]; rw [ ← zero_mul ((1/2):ℂ)]; rw [ mul_one_div]; rw [ cancel_factors.cancel_factors_eq_div h two_ne_zero]; rw [ mul_comm],
...
by rw [← not_exists]; rw [ not_iff_not]; rw [ tan_eq_zero_iff]
...
begin
 ...
 { rw [tan]; rw [ sin_add]; rw [ cos_add]; rw [ ← div_div_div_cancel_right (sin x * cos y + cos x * sin y) (mul_ne_zero (cos_ne_zero_iff.mpr h1) (cos_ne_zero_iff.mpr h2))]; rw [ add_div]; rw [ sub_div],
 ...
 rw [hx]; rw [ hy]; rw [ add_zero]; rw [ zero_div]; rw [ mul_div_assoc]; rw [ mul_div_assoc]; rw [ ← add_mul (2*(k:ℂ)+1) (2*l+1) (π/2)]; rw [ ← mul_div_assoc]; rw [ hxy] },
end
...
begin
 ...
 { rw [two_mul]; rw [ two_mul]; rw [ sq]; rw [ tan_add (or.inl ⟨h, h⟩)] },
...
by rw [tan_add h]; rw [ tan_mul_I]; rw [ mul_assoc]
...
begin
 ...
 { rwa [←div_eq_inv_mul]; rwa [ div_lt_one pi_div_two_pos] },
end
...
begin
 ...
 { rwa [←div_eq_inv_mul]; rwa [ div_le_one pi_div_two_pos] },
end
