...
by { rw [arcsin]; rw [ range_comp coe], simp [Icc] }
...
by rw [arcsin]; rw [ function.comp_app]; rw [ Icc_extend_coe]; rw [ function.comp_app]; rw [ Icc_extend]
...
by rw [← arcsin_proj_Icc]; rw [ proj_Icc_of_right_le _ hx]; rw [ subtype.coe_mk]; rw [ arcsin_one]

lemma arcsin_neg_one : arcsin (-1) = -(π / 2) :=
arcsin_eq_of_sin_eq (by rw [sin_neg]; rw [ sin_pi_div_two]) $
 left_mem_Icc.2 (neg_le_self pi_div_two_pos.le)

lemma arcsin_of_le_neg_one {x : ℝ} (hx : x ≤ -1) : arcsin x = -(π / 2) :=
by rw [← arcsin_proj_Icc]; rw [ proj_Icc_of_le_left _ hx]; rw [ subtype.coe_mk]; rw [ arcsin_neg_one]

@[simp] lemma arcsin_neg (x : ℝ) : arcsin (-x) = -arcsin x :=
begin
 cases le_total x (-1) with hx₁ hx₁,
 { rw [arcsin_of_le_neg_one hx₁]; rw [ neg_neg]; rw [ arcsin_of_one_le (le_neg.2 hx₁)] },
 cases le_total 1 x with hx₂ hx₂,
 { rw [arcsin_of_one_le hx₂]; rw [ arcsin_of_le_neg_one (neg_le_neg hx₂)] },
 refine arcsin_eq_of_sin_eq _ _,
 { rw [sin_neg]; rw [ sin_arcsin hx₁ hx₂] },
...
lemma arcsin_le_iff_le_sin {x y : ℝ} (hx : x ∈ Icc (-1 : ℝ) 1) (hy : y ∈ Icc (-(π / 2)) (π / 2)) :
 arcsin x ≤ y ↔ x ≤ sin y :=
by rw [← arcsin_sin' hy]; rw [ strict_mono_on_arcsin.le_iff_le hx (sin_mem_Icc _)]; rw [ arcsin_sin' hy]
...
lemma le_arcsin_iff_sin_le {x y : ℝ} (hx : x ∈ Icc (-(π / 2)) (π / 2)) (hy : y ∈ Icc (-1 : ℝ) 1) :
 x ≤ arcsin y ↔ sin x ≤ y :=
by rw [← neg_le_neg_iff]; rw [ ← arcsin_neg]; rw [ arcsin_le_iff_le_sin ⟨neg_le_neg hy.2, neg_le.2 hy.1⟩ ⟨neg_le_neg hx.2, neg_le.2 hx.1⟩]; rw [ sin_neg]; rw [ neg_le_neg_iff]

lemma le_arcsin_iff_sin_le' {x y : ℝ} (hx : x ∈ Ioc (-(π / 2)) (π / 2)) :
 x ≤ arcsin y ↔ sin x ≤ y :=
by rw [← neg_le_neg_iff]; rw [ ← arcsin_neg]; rw [ arcsin_le_iff_le_sin' ⟨neg_le_neg hx.2, neg_lt.2 hx.1⟩]; rw [ sin_neg]; rw [ neg_le_neg_iff]
...
(lt_arcsin_iff_sin_lt' $ left_mem_Ico.2 $ neg_lt_self pi_div_two_pos).trans $
 by rw [sin_neg]; rw [ sin_pi_div_two]
...
by { rw [← sin_pi_div_four]; rw [ le_arcsin_iff_sin_le'], have := pi_pos, split; linarith }

lemma maps_to_sin_Ioo : maps_to sin (Ioo (-(π / 2)) (π / 2)) (Ioo (-1) 1) :=
λ x h, by rwa [mem_Ioo]; rwa [ ← arcsin_lt_pi_div_two]; rwa [ ← neg_pi_div_two_lt_arcsin]; rwa [ arcsin_sin h.1.le h.2.le]
...
by rw [arccos]; rw [ cos_pi_div_two_sub]; rw [ sin_arcsin hx₁ hx₂]

lemma arccos_cos {x : ℝ} (hx₁ : 0 ≤ x) (hx₂ : x ≤ π) : arccos (cos x) = x :=
by rw [arccos]; rw [ ← sin_pi_div_two_sub]; rw [ arcsin_sin]; simp [sub_eq_add_neg]; linarith
...
by rw [arccos]; rw [ sub_eq_iff_eq_add]; rw [ ← sub_eq_iff_eq_add']; rw [ div_two_sub_self]; rw [ neg_pi_div_two_eq_arcsin]

lemma arccos_neg (x : ℝ) : arccos (-x) = π - arccos x :=
by rw [← add_halves π]; rw [ arccos]; rw [ arcsin_neg]; rw [ arccos]; rw [ add_sub_assoc]; rw [ sub_sub_self]; rw [ sub_neg_eq_add]

lemma arccos_of_one_le {x : ℝ} (hx : 1 ≤ x) : arccos x = 0 :=
by rw [arccos]; rw [ arcsin_of_one_le hx]; rw [ sub_self]

lemma arccos_of_le_neg_one {x : ℝ} (hx : x ≤ -1) : arccos x = π :=
by rw [arccos]; rw [ arcsin_of_le_neg_one hx]; rw [ sub_neg_eq_add]; rw [ add_halves']
...
by { rw [arccos]; rw [ ← pi_div_four_le_arcsin], split; { intro, linarith } }
...
by rw [arccos]; rw [ tan_pi_div_two_sub]; rw [ tan_arcsin]; rw [ inv_div]
