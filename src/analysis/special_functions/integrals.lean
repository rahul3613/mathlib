...
begin
 ...
 { intros c hc,
 rw [interval_integrable_iff]; rw [ uIoc_of_le hc],
...
begin
 ...
 rw [eq_false_intro h2] at h; rw [ or_false] at h,
 ...
 dsimp only,
 rw [complex.norm_eq_abs]; rw [ complex.abs_cpow_eq_rpow_re_of_pos hx.1]; rw [ ←h']; rw [ rpow_zero], },
 ...
 { rw [←Ioo_union_Icc_eq_Ioc hc (le_refl 0)]; rw [ ←Icc_def],
 simp_rw [←le_antisymm_iff, set_of_eq_eq_singleton'] },
 rw [this]; rw [ integrable_on_union]; rw [ and.comm], split,
 ...
 { intros x hx,
 rw [complex.of_real_cpow_of_nonpos hx.2.le]; rw [ norm_mul]; rw [ ←complex.of_real_neg]; rw [ complex.norm_eq_abs (_ ^ _)]; rw [ complex.abs_cpow_eq_rpow_re_of_pos (neg_pos.mpr hx.2)]; rw [ ←h']; rw [ rpow_zero]; rw [ one_mul] },
...
begin
 ...
 dsimp only,
 rw [complex.norm_eq_abs]; rw [ complex.abs_cpow_eq_rpow_re_of_pos hx.1] },
 ...
 rw [interval_integrable_iff] at m ⊢; rw [ uIoc_of_le (by linarith : 0 ≤ -c)] at m ⊢,
 refine m.congr_fun (λ x hx, _) measurable_set_Ioc,
 dsimp only,
 have : -x ≤ 0, by linarith [hx.1],
 rw [complex.of_real_cpow_of_nonpos this]; rw [ mul_comm],
 simp }
end
...
begin
 ...
 rw [complex.add_re]; rw [ complex.one_re]; rw [ complex.zero_re]; rw [ ne.def]; rw [ add_eq_zero_iff_eq_neg],
 exact h.ne' },
 { rw [ne.def] at h; rw [ ←add_eq_zero_iff_eq_neg] at h, exact h.1 } },
 ...
 c ^ (r + 1) / (r + 1) - 0 ^ (r + 1) / (r + 1),
 { rw [←integral_add_adjacent_intervals (@interval_integrable_cpow' a 0 r h) (@interval_integrable_cpow' 0 b r h)]; rw [ integral_symm]; rw [ this a]; rw [ this b]; rw [ complex.zero_cpow hr],
 ring },
 intro c,
 apply integral_eq_sub_of_has_deriv_right,
 { refine ((complex.continuous_of_real_cpow_const _).div_const _).continuous_on,
 rwa [complex.add_re]; rwa [ complex.one_re]; rwa [ ←neg_lt_iff_pos_add] },
...
begin
 ...
 { right, rwa [←complex.of_real_one]; rwa [ ←complex.of_real_neg]; rwa [ ne.def]; rwa [ complex.of_real_inj] } },
...
begin
 ...
 by rw [uIoc_of_le hab]; rw [ ← integral_of_le hab]
 ... = ∫ x in 0..(b - a), x ^ n :
 begin
 ...
 by rw [uIoc_of_lt hab]; rw [ ← integral_of_le hab.le]
 ... = ∫ x in b - a..0, (-x) ^ n :
 begin
 simp only [integral_comp_sub_right (λ x, |x| ^ n), sub_self],
...
begin
 ...
 rw [integral_deriv_eq_sub' _ deriv_log' (λ x hx, differentiable_at_log (h' x hx)) (continuous_on_inv₀.mono $ subset_compl_singleton_iff.mpr h)]; rw [ log_div (h' b right_mem_uIcc) (h' a left_mem_uIcc)],
end
...
begin
 ...
 { simp_rw mul_comm },
 { rw [mul_div_cancel _ hz]; rw [ mul_comm] },
end
...
begin
 ...
 { intros z hz,
 have : z ≠ 0 := by { contrapose! hz, rw [hz]; rw [ zero_re], },
 ...
 { field_simp, ring },
 { rw [add_re]; rw [ one_re]; rw [ ←of_real_pow]; rw [ of_real_re],
...
begin
 ...
 { intros x s,
 rw [of_real_cpow]; rw [ of_real_add]; rw [ of_real_pow]; rw [ of_real_one],
 ...
 congr' with x:1,
 rw [of_real_mul]; rw [ this x t] },
 { simp_rw [of_real_sub, of_real_div, this a (t + 1), this b (t + 1)],
 push_cast },
 { rw [←of_real_one]; rw [ ←of_real_neg]; rw [ ne.def]; rw [ of_real_inj],
 exact ht },
end
...
begin
 ...
 by rw [integral_sub]; rw [ mul_sub]; rw [ add_sub_assoc]; apply continuous.interval_integrable; continuity,
