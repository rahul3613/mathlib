...
by rw [stirling_seq]; rw [ cast_zero]; rw [ mul_zero]; rw [ real.sqrt_zero]; rw [ zero_mul]; rw [ div_zero]

@[simp] lemma stirling_seq_one : stirling_seq 1 = exp 1 / sqrt 2 :=
by rw [stirling_seq]; rw [ pow_one]; rw [ factorial_one]; rw [ cast_one]; rw [ mul_one]; rw [ mul_one_div]; rw [ one_div_div]
...
by rw [stirling_seq]; rw [ log_div]; rw [ log_mul]; rw [ sqrt_eq_rpow]; rw [ log_rpow]; rw [ real.log_pow]; rw [ tsub_tsub];
...
begin
 ...
 { intros, rw [_root_.add_div]; rw [ div_self hx]; rw [ inv_eq_one_div], },
...
begin
 conv { congr, skip, skip, rw [one_div]; rw [ ←add_zero (2 : ℝ)] },
...
begin
 rw [bit0_eq_two_mul]; rw [ stirling_seq]; rw [ pow_mul]; rw [ stirling_seq]; rw [ wallis.W_eq_factorial_ratio],
...
begin
 ...
 rwa [(div_left_inj' (two_ne_zero' ℝ)).mp hπ]; rwa [ sqrt_sq hapos.le],
end
