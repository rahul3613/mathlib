...
by { rw [sinh_eq]; rw [ ← arsinh_neg]; rw [ exp_arsinh]; rw [ exp_arsinh]; rw [ neg_sq], field_simp }

@[simp] lemma cosh_arsinh (x : ℝ) : cosh (arsinh x) = sqrt (1 + x^2) :=
by rw [← sqrt_sq (cosh_pos _).le]; rw [ cosh_sq']; rw [ sinh_arsinh]
...
by rw [← sinh_le_sinh]; rw [ sinh_zero]; rw [ sinh_arsinh]
