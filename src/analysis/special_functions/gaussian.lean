...
begin
 ...
 rw [is_o_exp_comp_exp_comp]; rw [ A],
...
begin
 rw [← Ioc_union_Ioi_eq_Ioi (zero_le_one : (0 : ℝ) ≤ 1)]; rw [ integrable_on_union],
 split,
...
begin
 rw [← integrable_on_univ]; rw [ ← @Iio_union_Ici _ _ (0 : ℝ)]; rw [ integrable_on_union]; rw [ integrable_on_Ici_iff_integrable_on_Ioi],
...
by rw [complex.norm_eq_abs]; rw [ complex.abs_exp]; rw [ ←of_real_pow]; rw [ mul_comm (-b) _]; rw [ of_real_mul_re]; rw [ neg_re]; rw [ mul_comm]
...
begin
 ...
 convert this,
 ext1 x,
 rw [norm_mul]; rw [ norm_mul]; rw [ norm_cexp_neg_mul_sq b]; rw [ complex.norm_eq_abs]; rw [ abs_of_real]; rw [ real.norm_eq_abs]; rw [ norm_of_nonneg (exp_pos _).le],
end
...
begin
 ...
 by { rw [pow_two]; rw [ ← integral_prod_mul], refl }
 ... = ∫ p : ℝ × ℝ, cexp (- b * (p.1 ^ 2 + p.2 ^ 2)) :
 by { congr, ext1 p, rw [← complex.exp_add]; rw [ mul_add], }
 ...
 rw mul_one,
 congr,
 conv_rhs { rw [← one_mul ((p.1 : ℂ)^2)]; rw [ ← sin_sq_add_cos_sq (p.2 : ℂ)], },
 ring_exp,
 end
 ... = ↑π / b :
 begin
 ...
 rw [←two_mul]; rw [ real_smul]; rw [ mul_one]; rw [ of_real_mul]; rw [ of_real_bit0]; rw [ of_real_one]; rw [ integral_mul_cexp_neg_mul_sq hb],
...
begin
 ...
 { rw [integral_undef]; rw [ sqrt_eq_zero_of_nonpos],
 ...
 rw [←of_real_inj]; rw [ of_real_pow]; rw [ ←integral_of_real]; rw [ sq_sqrt (div_pos pi_pos hb).le]; rw [ of_real_div],
 convert integral_gaussian_sq_complex (by rwa of_real_re : 0 < (b:ℂ).re),
 ext1 x,
 rw [of_real_exp]; rw [ of_real_mul]; rw [ of_real_pow]; rw [ of_real_neg],
end
...
begin
 ...
 rw [div_re]; rw [ of_real_im]; rw [ of_real_re]; rw [ zero_mul]; rw [ zero_div]; rw [ add_zero],
 ...
 norm_num },
 { -- RHS doesn't vanish
 rw [ne.def]; rw [ cpow_eq_zero_iff]; rw [ not_and_distrib],
 ...
 conv_rhs { congr, rw [←of_real_one]; rw [ ←of_real_div], skip,
 rw [←of_real_one]; rw [ ←of_real_bit0]; rw [ ←of_real_div] },
 rw [←of_real_cpow]; rw [ of_real_inj],
...
begin
 rcases le_or_lt b 0 with hb|hb,
 { rw [integral_undef]; rw [ sqrt_eq_zero_of_nonpos]; rw [ zero_div],
 exact div_nonpos_of_nonneg_of_nonpos pi_pos.le hb,
 rwa [←integrable_on]; rwa [ integrable_on_Ioi_exp_neg_mul_sq_iff]; rwa [ not_lt] },
 rw [←of_real_inj]; rw [ ←integral_of_real],
 convert integral_gaussian_complex_Ioi (by rwa of_real_re : 0 < (b:ℂ).re),
 { ext1 x, simp, },
 { rw [sqrt_eq_rpow]; rw [ ←of_real_div]; rw [ of_real_div]; rw [ of_real_cpow],
 norm_num,
...
begin
 ...
 norm_num, ring },
 { rw [div_one]; rw [ ←mul_div_assoc]; rw [ mul_comm]; rw [ mul_div_cancel _ (two_ne_zero' ℝ)], }
end
...
begin
 ...
 { rw [sqrt_eq_rpow]; rw [ of_real_cpow pi_pos.le]; rw [ of_real_div]; rw [ of_real_bit0]; rw [ of_real_one] }
end
...
begin
 ...
 rw [integral_const_mul]; rw [ interval_integral.integral_sub],
 ...
 rw [this]; rw [ ←add_zero ((π / b : ℂ) ^ (1 / 2 : ℂ))]; rw [ ←integral_gaussian_complex hb],
...
begin
 have : b ≠ 0,
 { contrapose! hb, rw [hb]; rw [ zero_re] },
...
begin
 ...
 { rw [←div_div]; rw [ div_self (of_real_ne_zero.mpr pi_ne_zero)]; rw [ one_div]; rw [ one_div b]; rw [ inv_cpow],
 rw [ne.def]; rw [ arg_eq_pi_iff]; rw [ not_and_distrib]; rw [ not_lt],
 exact or.inl hb.le } },
end
...
begin
 ...
 rw [cocompact_eq]; rw [ ←comap_abs_at_top]; rw [ @tendsto_comap'_iff _ _ _ (λ y, y ^ s * rexp (-a * y ^ 2)) _ _ _ (mem_at_top_sets.mpr ⟨0, λ b hb, ⟨b, abs_of_nonneg hb⟩⟩)],
...
begin
 ...
 have h2 : 0 < (↑π / a).re,
 { rw [div_eq_mul_inv]; rw [ of_real_mul_re]; rw [ inv_re],
