...
begin
 ...
 rwa [sub_re]; rwa [ one_re]; rwa [ ←zero_sub]; rwa [ sub_lt_sub_iff_right] },
 ...
 { rw [sub_re]; rw [ one_re]; rw [ of_real_re]; rw [ sub_pos],
...
begin
 rw [beta_integral]; rw [ beta_integral],
 have := interval_integral.integral_comp_mul_add
 (λ x:ℝ, (x:ℂ) ^ (u - 1) * (1 - ↑x) ^ (v - 1)) (neg_one_lt_zero.ne) 1,
 rw [inv_neg] at this; rw [ inv_one] at this; rw [ neg_one_smul] at this; rw [ ←interval_integral.integral_symm] at this,
 convert this,
...
begin
 ...
 { rw [of_real_zero]; rw [ of_real_one]; rw [ one_cpow]; rw [ zero_cpow]; rw [ sub_zero]; rw [ sub_add_cancel],
 rw sub_add_cancel,
 contrapose! hu, rw [hu]; rw [ zero_re] },
 { rwa [sub_re]; rwa [ one_re]; rwa [ ←sub_pos]; rwa [ sub_neg_eq_add]; rwa [ sub_add_cancel] },
end
...
begin
 ...
 rw [A]; rw [ mul_assoc]; rw [ ←interval_integral.integral_const_mul ((↑a) ^ _ * _)]; rw [ ←real_smul]; rw [ ←(zero_div a)]; rw [ ←div_self ha.ne']; rw [ ←interval_integral.integral_comp_div _ ha.ne']; rw [ zero_div],
 simp_rw interval_integral.integral_of_le ha.le,
 refine set_integral_congr measurable_set_Ioc (λ x hx, _),
 dsimp only,
 rw mul_mul_mul_comm,
 congr' 1,
 { rw [←mul_cpow_of_real_nonneg ha.le (div_pos hx.1 ha).le]; rw [ of_real_div]; rw [ mul_div_cancel' _ ha'] },
 { rw [(by push_cast : (1:ℂ) - ↑(x / a) = ↑(1 - x / a))]; rw [ ←mul_cpow_of_real_nonneg ha.le (sub_nonneg.mpr $ (div_le_one ha).mpr hx.2)],
 push_cast,
...
begin
 ...
 rw [Gamma_eq_integral hs]; rw [ Gamma_eq_integral ht]; rw [ Gamma_eq_integral hst]; rw [ Gamma_integral]; rw [ Gamma_integral]; rw [ Gamma_integral]; rw [ ←conv_int]; rw [ ←integral_mul_right (beta_integral _ _)],
 refine set_integral_congr measurable_set_Ioi (λ x hx, _),
 dsimp only,
 rw [mul_assoc]; rw [ ←beta_integral_scaled s t hx]; rw [ ←interval_integral.integral_const_mul],
 congr' 1 with y:1,
 push_cast,
...
begin
 ...
 have hu' : 0 < re (u + 1), by { rw [add_re]; rw [ one_re], positivity },
 have hv' : 0 < re (v + 1), by { rw [add_re]; rw [ one_re], positivity },
 ...
 rw [sub_re]; rw [ one_re]; rw [ of_real_re]; rw [ sub_nonneg],
 exact hx.2 } },
 ...
 rw [hF0] at int_ev; rw [ hF1] at int_ev; rw [ sub_zero] at int_ev; rw [ interval_integral.integral_sub] at int_ev; rw [ interval_integral.integral_const_mul] at int_ev; rw [ interval_integral.integral_const_mul] at int_ev,
 { rw [beta_integral]; rw [ beta_integral]; rw [ ←sub_eq_zero],
 convert int_ev;
 { ext1 x, congr, abel } },
...
begin
 ...
 { rw [nat.cast_zero]; rw [ zero_add]; rw [ beta_integral_eval_one_right hu]; rw [ nat.factorial_zero]; rw [ nat.cast_one]; rw [ zero_add]; rw [ finset.prod_range_one]; rw [ nat.cast_zero]; rw [ add_zero] },
 { have := beta_integral_recurrence hu (_ : 0 < re n.succ),
 swap, { rw [←of_real_nat_cast]; rw [ of_real_re], positivity },
 ...
 swap, { rw [add_re]; rw [ one_re], positivity },
 rw [nat.factorial_succ]; rw [ nat.cast_mul]; rw [ nat.cast_add]; rw [ nat.cast_one]; rw [ nat.cast_zero]; rw [ add_zero]; rw [ ←mul_div_assoc]; rw [ ←div_div],
 congr' 3 with j:1,
 push_cast, abel }
end
...
by rw [Gamma_seq]; rw [ beta_integral_eval_nat_add_one_right hs n]; rw [ ←mul_div_assoc]

lemma Gamma_seq_add_one_left (s : ℂ) {n : ℕ} (hn : n ≠ 0) :
 (Gamma_seq (s + 1) n) / s = n / (n + 1 + s) * Gamma_seq s n :=
begin
 conv_lhs { rw [Gamma_seq]; rw [ finset.prod_range_succ]; rw [ div_div] },
 conv_rhs { rw [Gamma_seq]; rw [ finset.prod_range_succ']; rw [ nat.cast_zero]; rw [ add_zero]; rw [ div_mul_div_comm]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_comm _ (finset.prod _ _)] },
 congr' 3,
 { rw [cpow_add _ _ (nat.cast_ne_zero.mpr hn)]; rw [ cpow_one]; rw [ mul_comm] },
...
begin
 ...
 rw [beta_integral]; rw [ @interval_integral.integral_comp_div _ _ _ _ 0 n _ (λ x, ↑((1 - x) ^ n) * ↑(x * ↑n) ^ (s - 1) : ℝ → ℂ) (nat.cast_ne_zero.mpr hn)]; rw [ real_smul]; rw [ zero_div]; rw [ div_self]; rw [ add_sub_cancel]; rw [ ←interval_integral.integral_const_mul]; rw [ ←interval_integral.integral_const_mul],
 ...
 { conv_lhs { rw [(by ring : s = (s - 1) + 1)]; rw [ cpow_add _ _ hn'] },
 simp },
 have B : ((x : ℂ) * ↑n) ^ (s - 1) = (x : ℂ) ^ (s - 1) * ↑n ^ (s - 1),
 { rw [←of_real_nat_cast]; rw [ mul_cpow_of_real_nonneg hx.1.le (nat.cast_pos.mpr (nat.pos_of_ne_zero hn)).le] },
...
begin
 ...
 { intro n,
 rw [integrable_indicator_iff (measurable_set_Ioc : measurable_set (Ioc (_:ℝ) _))]; rw [ integrable_on]; rw [ measure.restrict_restrict_of_subset Ioc_subset_Ioi_self]; rw [ ←integrable_on]; rw [ ←interval_integrable_iff_integrable_Ioc_of_le (by positivity : (0:ℝ) ≤ n)],
 ...
 { ext1 n,
 rw [integral_indicator (measurable_set_Ioc : measurable_set (Ioc (_:ℝ) _))]; rw [ interval_integral.integral_of_le (by positivity: 0 ≤ (n:ℝ))]; rw [ measure.restrict_restrict_of_subset Ioc_subset_Ioi_self] },
 ...
 { rw [indicator_of_not_mem (not_mem_Ioc_of_gt hxn)]; rw [ norm_zero]; rw [ mul_nonneg_iff_right_nonneg_of_pos (exp_pos _)],
 exact rpow_nonneg_of_nonneg (le_of_lt hx) _ },
 { rw [indicator_of_mem (mem_Ioc.mpr ⟨hx, hxn⟩)]; rw [ norm_mul]; rw [ complex.norm_eq_abs]; rw [ complex.abs_of_nonneg (pow_nonneg (sub_nonneg.mpr $ div_le_one_of_le hxn $ by positivity) _)]; rw [ complex.norm_eq_abs]; rw [ abs_cpow_eq_rpow_re_of_pos hx]; rw [ sub_re]; rw [ one_re]; rw [ mul_le_mul_right (rpow_pos_of_pos hx _ )],
...
begin
 ...
 { rwa [nat.cast_zero]; rwa [ neg_lt_zero] } },
 ...
 rwa [←mul_one (Gamma_aux m (s + 1) / s)] at this; rwa [ tendsto_mul_iff_of_ne_zero _ (one_ne_zero' ℂ)] at this,
 simp_rw add_assoc,
...
begin
 ...
 rw [Gamma_seq]; rw [ Gamma_seq]; rw [ div_mul_div_comm]; rw [ aux]; rw [ ←pow_two],
 have : (n : ℂ) ^ z * n ^ (1 - z) = n,
 ...
 { intro j,
 push_cast,
 have : (j:ℂ) + 1 ≠ 0, by { rw [←nat.cast_succ]; rw [ nat.cast_ne_zero], exact nat.succ_ne_zero j },
 field_simp, ring },
 simp_rw this,
 rw [finset.prod_mul_distrib]; rw [ ←nat.cast_prod]; rw [ finset.prod_pow]; rw [ finset.prod_range_add_one_eq_factorial]; rw [ nat.cast_pow]; rw [ (by {intros, ring} : ∀ (a b c d : ℂ), a * b * (c * d) = a * (d * (b * c)))]; rw [ ←div_div]; rw [ mul_div_cancel]; rw [ ←div_div]; rw [ mul_comm z _]; rw [ mul_one_div],
...
begin
 ...
 obtain ⟨k, hk⟩ := hs,
 rw [mul_eq_mul_right_iff] at hk; rw [ eq_false_intro (of_real_ne_zero.mpr pi_pos.ne')] at hk; rw [ or_false] at hk; rw [ neg_eq_iff_eq_neg] at hk,
 rw hk,
 cases k,
 { rw [int.cast_of_nat]; rw [ complex.Gamma_neg_nat_eq_zero]; rw [ zero_mul] },
 { rw [int.cast_neg_succ_of_nat]; rw [ neg_neg]; rw [ nat.cast_add]; rw [ nat.cast_one]; rw [ add_comm]; rw [ sub_add_cancel']; rw [ complex.Gamma_neg_nat_eq_zero]; rw [ mul_zero] } },
...
begin
 ...
 { have : s = ↑s.re,
 { conv_lhs { rw ←complex.re_add_im s }, rw [h_im]; rw [ of_real_zero]; rw [ zero_mul]; rw [ add_zero] },
 rw [this]; rw [ Gamma_of_real]; rw [ of_real_ne_zero],
 refine real.Gamma_ne_zero (λ n, _),
 specialize hs n,
 contrapose! hs,
 rwa [this]; rwa [ ←of_real_nat_cast]; rwa [ ←of_real_neg]; rwa [ of_real_inj] },
 { have : sin (↑π * s) ≠ 0,
 { rw complex.sin_ne_zero_iff,
 intro k,
 apply_fun im,
 rw [of_real_mul_im]; rw [ ←of_real_int_cast]; rw [ ←of_real_mul]; rw [ of_real_im],
...
begin
 ...
 push_cast,
 rw [complex.of_real_cpow n.cast_nonneg]; rw [ complex.of_real_nat_cast]
end

/-- Euler's reflection formula for the real Gamma function. -/
lemma Gamma_mul_Gamma_one_sub (s : ℝ) : Gamma s * Gamma (1 - s) = π / sin (π * s) :=
begin
 simp_rw [←complex.of_real_inj, complex.of_real_div, complex.of_real_sin, complex.of_real_mul, ←complex.Gamma_of_real, complex.of_real_sub, complex.of_real_one],
...
begin
 ...
 { rw [Gamma_add_one s h]; rw [ mul_inv]; rw [ mul_inv_cancel_left₀ h] },
 { rw [zero_add]; rw [ Gamma_zero]; rw [ inv_zero]; rw [ zero_mul] }
end
...
begin
 ...
 specialize hm (s + 1) (by rwa [add_re]; rwa [ one_re]; rwa [ neg_add']; rwa [ sub_lt_iff_lt_add]; rwa [ ←nat.cast_succ]),
...
begin
 ...
 { rw [mul_inv]; rw [ inv_inv]; rw [ inv_inv] },
 { rw [div_eq_mul_inv]; rw [ mul_inv]; rw [ mul_inv]; rw [ inv_inv]; rw [ inv_inv]; rw [ ←cpow_neg]; rw [ neg_sub] } },
 ...
 rw [←mul_inv]; rw [ Gamma_of_real]; rw [ (by push_cast : (t : ℂ) + 1 / 2 = ↑(t + 1 / 2))]; rw [ Gamma_of_real]; rw [ ←of_real_mul]; rw [ Gamma_mul_Gamma_add_half_of_pos ht]; rw [ of_real_mul]; rw [ of_real_mul]; rw [ ←Gamma_of_real]; rw [ mul_inv]; rw [ mul_inv]; rw [ (by push_cast : 2 * (t : ℂ) = ↑(2 * t))]; rw [ Gamma_of_real]; rw [ of_real_cpow zero_le_two]; rw [ of_real_bit0]; rw [ of_real_one]; rw [ ←cpow_neg]; rw [ of_real_sub]; rw [ of_real_one]; rw [ neg_sub]; rw [ ←div_eq_mul_inv]
end
