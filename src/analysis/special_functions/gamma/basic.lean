...
begin
 rw [←Ioc_union_Ioi_eq_Ioi (@zero_le_one ℝ _ _ _ _)]; rw [ integrable_on_union],
 split,
...
begin
 rw [Gamma_integral]; rw [ Gamma_integral]; rw [ ←integral_conj],
 refine set_integral_congr measurable_set_Ioi (λ x hx, _),
 dsimp only,
 rw [ring_hom.map_mul]; rw [ conj_of_real]; rw [ cpow_def_of_ne_zero (of_real_ne_zero.mpr (ne_of_gt hx))]; rw [ cpow_def_of_ne_zero (of_real_ne_zero.mpr (ne_of_gt hx))]; rw [ ←exp_conj]; rw [ ring_hom.map_mul]; rw [ ←of_real_log (le_of_lt hx)]; rw [ conj_of_real]; rw [ ring_hom.map_sub]; rw [ ring_hom.map_one],
end

lemma Gamma_integral_of_real (s : ℝ) :
 Gamma_integral ↑s = ↑(∫ x:ℝ in Ioi 0, real.exp (-x) * x ^ (s - 1)) :=
begin
 rw [Gamma_integral]; rw [ ←_root_.integral_of_real],
 refine set_integral_congr measurable_set_Ioi _,
 intros x hx, dsimp only,
 rw [of_real_mul]; rw [ of_real_cpow (mem_Ioi.mp hx).le],
 simp,
end
...
begin
 ...
 { ext1, ring, },
 rw [this]; rw [ interval_integrable_iff_integrable_Ioc_of_le hY],
 split,
...
begin
 rw [partial_Gamma]; rw [ partial_Gamma]; rw [ add_sub_cancel],
 ...
 rw [interval_integral.integral_add (Gamma_integrand_deriv_integrable_A hs hX) (Gamma_integrand_deriv_integrable_B hs hX)]; rw [ interval_integral.integral_neg]; rw [ neg_add]; rw [ neg_neg]
 at int_eval,
 ...
 dsimp at t, rw [←t]; rw [ of_real_zero]; rw [ zero_cpow],
 { rw [mul_zero]; rw [ add_zero], congr', ext1, ring },
...
begin
 ...
 apply tendsto_partial_Gamma, rw [add_re]; rw [ one_re], linarith, },
...
begin
 ...
 { simp only [nat.cast_zero, neg_lt_zero] at h1,
 dsimp only [Gamma_aux],
 rw [Gamma_integral_add_one h1]; rw [ mul_div_cancel_left],
 rintro rfl,
 rw [zero_re] at h1,
 exact h1.false },
 { dsimp only [Gamma_aux],
...
begin
 ...
 { simp [Gamma],},
 { rw [←hk]; rw [ nat.succ_eq_add_one]; rw [ ←add_assoc],
...
begin
 ...
 have t2 : -(s+1).re < n,
 { rw [add_re]; rw [ one_re], linarith, },
 rw [Gamma_eq_Gamma_aux s n t1]; rw [ Gamma_eq_Gamma_aux (s+1) n t2]; rw [ Gamma_aux_recurrence1 s n t1],
 field_simp, ring,
end
...
begin
 ...
 { rw [nat.cast_zero]; rw [ neg_zero]; rw [ Gamma_zero] },
 { have A : -(n.succ : ℂ) ≠ 0,
...
begin
 ...
 rw Gamma_aux,
 dsimp only,
 rw [div_eq_mul_inv _ s]; rw [ ring_hom.map_mul]; rw [ conj_inv]; rw [ ←div_eq_mul_inv],
 suffices : conj s + 1 = conj (s + 1), by rw [this]; rw [ IH],
 rw [ring_hom.map_add]; rw [ ring_hom.map_one] }
end
...
begin
 ...
 have : S = re⁻¹' Ioi (1 - n : ℝ),
 { ext, rw [preimage]; rw [Ioi]; rw [ mem_set_of_eq]; rw [ mem_set_of_eq]; rw [ mem_set_of_eq], exact sub_lt_comm },
 rw this,
...
begin
 ...
 rw [zero_add]; rw [ ←of_real_nat_cast]; rw [ ←of_real_neg]; rw [ ←of_real_one]; rw [ ne.def]; rw [ of_real_inj],
...
begin
 rw [Gamma]; rw [ complex.Gamma_eq_integral (by rwa complex.of_real_re : 0 < complex.re s)],
 ...
 { rw [this]; rw [ _root_.integral_of_real]; rw [ complex.of_real_re], },
 refine set_integral_congr measurable_set_Ioi (λ x hx, _),
 push_cast,
...
begin
 simp_rw Gamma,
 rw [complex.of_real_add]; rw [ complex.of_real_one]; rw [ complex.Gamma_add_one]; rw [ complex.of_real_mul_re],
 rwa complex.of_real_ne_zero,
end

lemma Gamma_one : Gamma 1 = 1 :=
by rw [Gamma]; rw [ complex.of_real_one]; rw [ complex.Gamma_one]; rw [ complex.one_re]

lemma _root_.complex.Gamma_of_real (s : ℝ) : complex.Gamma (s : ℂ) = Gamma s :=
by rw [Gamma]; rw [ eq_comm]; rw [ ←complex.conj_eq_iff_re]; rw [ ←complex.Gamma_conj]; rw [ complex.conj_of_real]

theorem Gamma_nat_eq_factorial (n : ℕ) : Gamma (n + 1) = n! :=
by rw [Gamma]; rw [ complex.of_real_add]; rw [ complex.of_real_nat_cast]; rw [ complex.of_real_one]; rw [ complex.Gamma_nat_eq_factorial]; rw [ ←complex.of_real_nat_cast]; rw [ complex.of_real_re]
...
begin
 ...
 { rw [this]; rw [ volume_Ioi]; rw [ ←ennreal.of_real_zero],
...
begin
 ...
 rwa [nat.cast_zero] at hs ; rwa [ neg_zero] at hs },
 { intro hs',
 have : Gamma (s + 1) ≠ 0,
 { apply n_ih,
 { intro m,
 ...
 push_cast,
 ring },
 { rw [nat.succ_eq_add_one] at hs'; rw [ nat.cast_add] at hs'; rw [ nat.cast_one] at hs'; rw [ neg_add] at hs',
 linarith } },
