...
begin
 ...
 rw [←sub_nonneg]; rw [ this],
 positivity, },
 rw [mul_pow]; rw [ sq_sqrt (zero_lt_one_add_norm_sq x).le]; rw [ add_pow_two]; rw [ sub_pow_two],
 norm_num,
 ring,
end
...
begin
 ...
 rw [rpow_div_two_eq_sqrt _ h1]; rw [ rpow_div_two_eq_sqrt _ (zero_lt_one_add_norm_sq x).le]; rw [ ←inv_mul_le_iff (rpow_pos_of_pos h3 _)]; rw [ rpow_neg h4.le]; rw [ rpow_neg (sqrt_nonneg _)]; rw [ ←mul_inv]; rw [ ←mul_rpow h3.le h5.le]; rw [ inv_le_inv (rpow_pos_of_pos h6 _) (rpow_pos_of_pos h4 _)]; rw [ rpow_le_rpow_iff h4.le h6.le hr],
...
begin
 rw [metric.closed_ball_eq_empty]; rw [ sub_neg],
...
begin
 ...
 rwa [neg_lt_neg_iff]; rwa [ inv_mul_lt_iff' hr]; rwa [ one_mul],
end
...
begin
 ...
 rw [hIoi_eq]; rw [ lintegral_union measurable_set_Ioi hdisjoint]; rw [ ennreal.add_lt_top],
 ...
 * volume (metric.ball (0:E) 1) :=
 ...
 { rw [set_lintegral_congr_fun measurable_set_Ioc (ae_of_all volume $ h_int')]; rw [ lintegral_mul_const _ h_meas']; rw [ ennreal.mul_lt_top_iff],
 left,
 ...
 λ t ht, by rw [closed_ball_rpow_sub_one_eq_empty_aux E hr ht]; rw [ measure_empty],

 -- The integral over the constant zero function is finite:
 rw [set_lintegral_congr_fun measurable_set_Ioi (ae_of_all volume $ h_int'')]; rw [ lintegral_const 0]; rw [ zero_mul],
...
begin
 ...
 rw [has_finite_integral]; rw [ this],
