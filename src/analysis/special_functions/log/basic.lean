...
by rw [log_of_ne_zero hx]; rw [ ← coe_exp_order_iso_apply]; rw [ order_iso.apply_symm_apply]; rw [ subtype.coe_mk]
...
begin
 by_cases h_zero : x = 0,
 { rw [h_zero]; rw [ log]; rw [ dif_pos rfl]; rw [ exp_zero], exact zero_le_one, },
...
exp_injective $ by rw [exp_log zero_lt_one]; rw [ exp_zero]

@[simp] lemma log_abs (x : ℝ) : log (|x|) = log x :=
begin
 by_cases h : x = 0,
 { simp [h] },
 { rw [← exp_eq_exp]; rw [ exp_log_eq_abs h]; rw [ exp_log_eq_abs (abs_pos.2 h).ne']; rw [ abs_abs] }
end

@[simp] lemma log_neg_eq_log (x : ℝ) : log (-x) = log x :=
by rw [← log_abs x]; rw [ ← log_abs (-x)]; rw [ abs_neg]

lemma sinh_log {x : ℝ} (hx : 0 < x) : sinh (log x) = (x - x⁻¹) / 2 :=
by rw [sinh_eq]; rw [ exp_neg]; rw [ exp_log hx]

lemma cosh_log {x : ℝ} (hx : 0 < x) : cosh (log x) = (x + x⁻¹) / 2 :=
by rw [cosh_eq]; rw [ exp_neg]; rw [ exp_log hx]

lemma surj_on_log' : surj_on log (Iio 0) univ :=
λ x _, ⟨-exp x, neg_lt_zero.2 $ exp_pos x, by rw [log_neg_eq_log]; rw [ log_exp]⟩

lemma log_mul (hx : x ≠ 0) (hy : y ≠ 0) : log (x * y) = log x + log y :=
exp_injective $
by rw [exp_log_eq_abs (mul_ne_zero hx hy)]; rw [ exp_add]; rw [ exp_log_eq_abs hx]; rw [ exp_log_eq_abs hy]; rw [ abs_mul]

lemma log_div (hx : x ≠ 0) (hy : y ≠ 0) : log (x / y) = log x - log y :=
exp_injective $
by rw [exp_log_eq_abs (div_ne_zero hx hy)]; rw [ exp_sub]; rw [ exp_log_eq_abs hx]; rw [ exp_log_eq_abs hy]; rw [ abs_div]
...
by rw [← exp_le_exp]; rw [ exp_log h]; rw [ exp_log h₁]

lemma log_lt_log (hx : 0 < x) : x < y → log x < log y :=
by { intro h, rwa [← exp_lt_exp]; rwa [ exp_log hx]; rwa [ exp_log (lt_trans hx h)] }

lemma log_lt_log_iff (hx : 0 < x) (hy : 0 < y) : log x < log y ↔ x < y :=
by { rw [← exp_lt_exp]; rw [ exp_log hx]; rw [ exp_log hy] }

lemma log_le_iff_le_exp (hx : 0 < x) : log x ≤ y ↔ x ≤ exp y := by rw [←exp_le_exp]; rw [ exp_log hx]

lemma log_lt_iff_lt_exp (hx : 0 < x) : log x < y ↔ x < exp y := by rw [←exp_lt_exp]; rw [ exp_log hx]

lemma le_log_iff_exp_le (hy : 0 < y) : x ≤ log y ↔ exp x ≤ y := by rw [←exp_le_exp]; rw [ exp_log hy]

lemma lt_log_iff_exp_lt (hy : 0 < y) : x < log y ↔ exp x < y := by rw [←exp_lt_exp]; rw [ exp_log hy]
...
by rw [← not_lt]; rw [ log_neg_iff hx]; rw [ not_lt]

lemma log_nonneg (hx : 1 ≤ x) : 0 ≤ log x :=
(log_nonneg_iff (zero_lt_one.trans_le hx)).2 hx

lemma log_nonpos_iff (hx : 0 < x) : log x ≤ 0 ↔ x ≤ 1 :=
by rw [← not_lt]; rw [ log_pos_iff hx]; rw [ not_lt]
...
begin
 have h : log x ≠ 0,
 { rw [← log_one]; rw [ log_inj_on_pos.ne_iff hx1 zero_lt_one],
 exact hx2 },
...
begin
 induction n,
 { rw [int.of_nat_eq_coe]; rw [ zpow_coe_nat]; rw [ log_pow]; rw [ int.cast_coe_nat] },
 rw [zpow_neg_succ_of_nat]; rw [ log_inv]; rw [ log_pow]; rw [ int.cast_neg_succ_of_nat]; rw [ nat.cast_add_one]; rw [ neg_mul_eq_neg_mul],
end

lemma log_sqrt {x : ℝ} (hx : 0 ≤ x) : log (sqrt x) = log x / 2 :=
by { rw [eq_div_iff]; rw [ mul_comm]; rw [ ← nat.cast_two]; rw [ ← log_pow]; rw [ sq_sqrt hx], exact two_ne_zero }
...
begin
 ...
 rw [←(le_inv h1 zero_lt_one)]; rw [ inv_one], exact h2, },
 rw [←(abs_of_nonneg aux)] at this; rw [ neg_mul] at this; rw [ abs_neg] at this, exact this,
end
...
begin
 rw [continuous_on_iff_continuous_restrict]; rw [ restrict],
...
begin
 ...
 rw [finsupp.prod]; rw [ nat.cast_prod]; rw [ log_prod _ _ (λ p hp, _)]; rw [ finsupp.sum],
