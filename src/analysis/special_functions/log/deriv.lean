...
if hx : x = 0 then
 by rw [deriv_zero_of_not_differentiable_at (mt differentiable_at_log_iff.1 (not_not.2 hx))]; rw [ hx]; rw [ inv_zero]
...
begin
 ...
 push_cast,
 ring_nf, },
 rw [← h_term_eq_goal]; rw [ (mul_right_injective₀ (two_ne_zero' ℕ)).has_sum_iff],
 { have h₁ := (has_sum_pow_div_log_of_abs_lt_1 (eq.trans_lt (abs_neg x) h)).mul_left (-1),
 convert h₁.add (has_sum_pow_div_log_of_abs_lt_1 h),
 ring_nf },
 { intros m hm,
 rw [range_two_mul] at hm; rw [ set.mem_set_of_eq] at hm; rw [ ← nat.even_add_one] at hm,
 dsimp [term],
