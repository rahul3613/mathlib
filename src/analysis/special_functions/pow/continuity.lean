...
begin
 ...
 from this.mono (λ x hx, by { dsimp only, rw [zero_cpow hx]; rw [ pi.zero_apply]} ),
...
begin
 by_cases ha : a = 0,
 { rw [ha]; rw [ continuous_at_congr (zero_cpow_eq_nhds h)], exact continuous_at_const, },
...
begin
 have : rpow a = λ x : ℝ, ((a : ℂ) ^ (x : ℂ)).re, by { ext1 x, rw [rpow_eq_pow]; rw [ rpow_def], },
 rw this,
...
begin
 ...
 { rw [nhds_within_singleton]; rw [ tendsto_pure]; rw [ pure_prod]; rw [ eventually_map],
...
begin
 ...
 rw [continuous_at]; rw [ zero_cpow hz₀]; rw [ tendsto_zero_iff_norm_tendsto_zero],
...
begin
 cases p with z w,
 rw [← not_lt_zero_iff] at h₁; rw [ lt_iff_le_and_ne] at h₁; rw [ not_and_distrib] at h₁; rw [ ne.def] at h₁; rw [ not_not] at h₁; rw [ not_le_zero_iff] at h₁,
...
begin
 ...
 rwa [neg_re]; rwa [ of_real_re]; rwa [ neg_pos] },
...
begin
 ...
 { ext p,
 rw [coe_rpow]; rw [ real.coe_to_nnreal _ (real.rpow_nonneg_of_nonneg p.1.2 _)],
 refl },
 rw this,
