...
begin
 ...
 rw [← rpow_mul (le_max_right b 0)]; rw [ (eq_div_iff (ne_of_gt hy)).mp rfl]; rw [ rpow_one] }),
end
...
begin
 ...
 rw [div_le_div_left (exp_pos _) (pow_pos hx₀ _) (rpow_pos_of_pos hx₀ _)]; rw [ ←rpow_nat_cast],
...
begin
 ...
 ... = c ^ r * |g x ^ r| : by rw [mul_rpow hc (abs_nonneg _)]; rw [ abs_rpow_of_nonneg hgx]
end
...
is_o.of_is_O_with $ λ c hc, ((h.forall_is_O_with (rpow_pos_of_pos hc r⁻¹)).rpow
 ...
 (by rw [←rpow_mul hc.le]; rw [ inv_mul_cancel hr.ne']; rw [ rpow_one])
...
((is_o_log_rpow_rpow_at_top r (neg_pos.2 hs)).comp_tendsto tendsto_inv_zero_at_top).congr'
 ...
 by rw [function.comp_app]; rw [ inv_rpow hx.out.le]; rw [ rpow_neg hx.out.le]; rw [ inv_inv])
...
(tendsto_log_div_rpow_nhds_zero $ neg_lt_zero.2 hr).congr' $
 eventually_mem_nhds_within.mono $ λ x hx, by rw [rpow_neg hx.out.le]; rw [ div_inv_eq_mul]
