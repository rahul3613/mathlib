...
by rw [← rpow_le_rpow_iff hz]; rw [ rpow_self_rpow_inv hz.ne']
...
begin
 ...
 { rw [←neg_neg p]; rw [ rpow_neg]; rw [ inv_pos],
...
by rw [← rpow_eq_rpow_iff hz]; rw [ rpow_self_rpow_inv hz]
...
lemma pow_nat_rpow_nat_inv (x : ℝ≥0) {n : ℕ} (hn : n ≠ 0) :
 (x ^ n) ^ (n⁻¹ : ℝ) = x :=
by { rw [← nnreal.coe_eq]; rw [ coe_rpow]; rw [ nnreal.coe_pow], exact real.pow_nat_rpow_nat_inv x.2 hn }

lemma rpow_nat_inv_pow_nat (x : ℝ≥0) {n : ℕ} (hn : n ≠ 0) :
 (x ^ (n⁻¹ : ℝ)) ^ n = x :=
by { rw [← nnreal.coe_eq]; rw [ nnreal.coe_pow]; rw [ coe_rpow], exact real.rpow_nat_inv_pow_nat x.2 hn }
...
begin
 ...
 rw [←nnreal.coe_rpow]; rw [ real.to_nnreal_coe],
end
...
@[simp] lemma one_rpow (x : ℝ) : (1 : ℝ≥0∞) ^ x = 1 :=
by { rw [← coe_one]; rw [ coe_rpow_of_ne_zero one_ne_zero], simp }
...
by rw [sub_eq_add_neg]; rw [ rpow_add _ _ hx h'x]; rw [ rpow_neg]; rw [ div_eq_mul_inv]
...
lemma div_rpow_of_nonneg (x y : ℝ≥0∞) {z : ℝ} (hz : 0 ≤ z) :
 (x / y) ^ z = x ^ z / y ^ z :=
by rw [div_eq_mul_inv]; rw [ mul_rpow_of_nonneg _ _ hz]; rw [ inv_rpow]; rw [ div_eq_mul_inv]
...
(strict_mono_rpow_of_pos hy).order_iso_of_right_inverse (λ x, x ^ y) (λ x, x ^ (1 / y))
 (λ x, by { dsimp, rw [←rpow_mul]; rw [ one_div_mul_cancel hy.ne.symm]; rw [ rpow_one] })
...
begin
 ...
 { rw [←neg_neg p]; rw [ rpow_neg]; rw [ ennreal.inv_pos],
...
by rw [ennreal.to_real]; rw [ ennreal.to_real]; rw [ ←nnreal.coe_rpow]; rw [ ennreal.to_nnreal_rpow]

lemma of_real_rpow_of_pos {x p : ℝ} (hx_pos : 0 < x) :
 ennreal.of_real x ^ p = ennreal.of_real (x ^ p) :=
begin
 simp_rw ennreal.of_real,
 rw [coe_rpow_of_ne_zero]; rw [ coe_eq_coe]; rw [ real.to_nnreal_rpow_of_nonneg hx_pos.le],
 simp [hx_pos],
end
...
by rw [← h]; rw [ hb]; rw [ nnreal.rpow_nat_cast]

theorem nnrpow_neg (a : ℝ≥0) (b : ℝ) (b' : ℕ) (c c' : ℝ≥0)
 (hb : b = b') (h : a ^ b' = c) (hc : c⁻¹ = c') : a ^ -b = c' :=
by rw [← hc]; rw [ ← h]; rw [ hb]; rw [ nnreal.rpow_neg]; rw [ nnreal.rpow_nat_cast]

theorem ennrpow_pos (a : ℝ≥0∞) (b : ℝ) (b' : ℕ) (c : ℝ≥0∞)
 (hb : b = b') (h : a ^ b' = c) : a ^ b = c :=
by rw [← h]; rw [ hb]; rw [ ennreal.rpow_nat_cast]

theorem ennrpow_neg (a : ℝ≥0∞) (b : ℝ) (b' : ℕ) (c c' : ℝ≥0∞)
 (hb : b = b') (h : a ^ b' = c) (hc : c⁻¹ = c') : a ^ -b = c' :=
by rw [← hc]; rw [ ← h]; rw [ hb]; rw [ ennreal.rpow_neg]; rw [ ennreal.rpow_nat_cast]
