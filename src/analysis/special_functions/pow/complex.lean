...
by rw [←zero_cpow_eq_iff]; rw [ eq_comm]

@[simp] lemma cpow_one (x : ℂ) : x ^ (1 : ℂ) = x :=
if hx : x = 0 then by simp [hx, cpow_def]
else by rw [cpow_def]; rw [ if_neg (one_ne_zero : (1 : ℂ) ≠ 0)]; rw [ if_neg hx]; rw [ mul_one]; rw [ exp_log hx]
...
by rw [sub_eq_add_neg]; rw [ cpow_add _ _ hx]; rw [ cpow_neg]; rw [ div_eq_mul_inv]
...
begin
 ...
 exact_mod_cast hn },
 rw [mul_comm]; rw [ ← of_real_nat_cast]; rw [ ← of_real_inv]; rw [ of_real_mul_im]; rw [ ← div_eq_inv_mul],
...
begin
 ...
 { rw [of_real_zero]; rw [ zero_mul]; rw [ zero_cpow hr]; rw [ zero_mul] },
 rcases eq_or_lt_of_le hb with rfl | hb',
 { rw [of_real_zero]; rw [ mul_zero]; rw [ zero_cpow hr]; rw [ mul_zero] },
 have ha'' : (a : ℂ) ≠ 0 := of_real_ne_zero.mpr ha'.ne',
 have hb'' : (b : ℂ) ≠ 0 := of_real_ne_zero.mpr hb'.ne',
 rw [cpow_def_of_ne_zero (mul_ne_zero ha'' hb'')]; rw [ log_of_real_mul ha' hb'']; rw [ of_real_log ha]; rw [ add_mul]; rw [ exp_add]; rw [ ←cpow_def_of_ne_zero ha'']; rw [ ←cpow_def_of_ne_zero hb'']
end

lemma inv_cpow_eq_ite (x : ℂ) (n : ℂ) :
 x⁻¹ ^ n = if x.arg = π then conj (x ^ conj n)⁻¹ else (x ^ n)⁻¹ :=
begin
 simp_rw [complex.cpow_def, log_inv_eq_ite, inv_eq_zero, map_eq_zero, ite_mul, neg_mul, is_R_or_C.conj_inv, apply_ite conj, apply_ite exp, apply_ite has_inv.inv, map_zero, map_one, exp_neg, inv_one, inv_zero, ←exp_conj, map_mul, conj_conj],
 split_ifs with hx hn ha ha; refl,
end

lemma inv_cpow (x : ℂ) (n : ℂ) (hx : x.arg ≠ π) : x⁻¹ ^ n = (x ^ n)⁻¹ :=
by rw [inv_cpow_eq_ite]; rw [ if_neg hx]
...
by rw [conj_cpow_eq_ite]; rw [ if_neg hx]

lemma cpow_conj (x : ℂ) (n : ℂ) (hx : x.arg ≠ π) : x ^ conj n = conj (conj x ^ n) :=
by rw [conj_cpow _ _ hx]; rw [ conj_conj]
...
by rw [← h]; rw [ hb]; rw [ complex.cpow_nat_cast]

theorem cpow_neg (a b : ℂ) (b' : ℕ) (c c' : ℂ)
 (hb : b = b') (h : a ^ b' = c) (hc : c⁻¹ = c') : a ^ -b = c' :=
by rw [← hc]; rw [ ← h]; rw [ hb]; rw [ complex.cpow_neg]; rw [ complex.cpow_nat_cast]
