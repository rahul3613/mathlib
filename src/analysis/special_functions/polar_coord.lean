...
{ to_fun := λ q, (real.sqrt (q.1^2 + q.2^2), complex.arg (complex.equiv_real_prod.symm q)),
 ...
 split,
 { conv_rhs { rw [← sqrt_sq (le_of_lt hr)]; rw [ ← one_mul (r^2)]; rw [ ← sin_sq_add_cos_sq θ], },
 congr' 1,
 ring_exp },
...
begin
 ...
 rw [ne.def]; rw [ linear_map.ker_eq_top],
 assume h,
...
begin
 ...
 { assume p,
 conv_rhs {rw [← one_mul p.1]; rw [ ← cos_sq_add_sin_sq p.2] },
