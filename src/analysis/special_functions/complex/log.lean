...
by rw [log]; rw [ exp_add_mul_I]; rw [ ← of_real_sin]; rw [ sin_arg]; rw [ ← of_real_cos]; rw [ cos_arg hx]; rw [ ← of_real_exp]; rw [ real.exp_log (abs.pos hx)]; rw [ mul_add]; rw [ of_real_div]; rw [ of_real_div]; rw [ mul_div_cancel' _ (of_real_ne_zero.2 $ abs.ne_zero hx)]; rw [ ← mul_assoc]; rw [ mul_div_cancel' _ (of_real_ne_zero.2 $ abs.ne_zero hx)]; rw [ re_add_im]
...
by rw [log]; rw [ abs_exp]; rw [ real.log_exp]; rw [ exp_eq_exp_re_mul_sin_add_cos]; rw [ ← of_real_exp]; rw [ arg_mul_cos_add_sin_mul_I (real.exp_pos _) ⟨hx₁, hx₂⟩]; rw [ re_add_im]

lemma exp_inj_of_neg_pi_lt_of_le_pi {x y : ℂ} (hx₁ : -π < x.im) (hx₂ : x.im ≤ π)
 (hy₁ : - π < y.im) (hy₂ : y.im ≤ π) (hxy : exp x = exp y) : x = y :=
by rw [← log_exp hx₁ hx₂]; rw [ ← log_exp hy₁ hy₂]; rw [ hxy]

lemma of_real_log {x : ℝ} (hx : 0 ≤ x) : (x.log : ℂ) = log x :=
complex.ext
 (by rw [log_re]; rw [ of_real_re]; rw [ abs_of_nonneg hx])
 (by rw [of_real_im]; rw [ log_im]; rw [ arg_of_real_of_nonneg hx])
...
by rw [mul_comm]; rw [ log_of_real_mul hr hx]; rw [ add_comm]
...
by rw [log_conj_eq_ite]; rw [ if_neg h]

lemma log_inv_eq_ite (x : ℂ) : log (x⁻¹) = if x.arg = π then -conj (log x) else -log x :=
begin
 by_cases hx : x = 0,
 { simp [hx] },
 rw [inv_def]; rw [ log_mul_of_real]; rw [ real.log_inv]; rw [ of_real_neg]; rw [ ←sub_eq_neg_add]; rw [ log_conj_eq_ite],
 { simp_rw [log, map_add, map_mul, conj_of_real, conj_I, norm_sq_eq_abs, real.log_pow, nat.cast_two, of_real_mul, of_real_bit0, of_real_one, neg_add, mul_neg, two_mul, neg_neg],
 split_ifs,
 { rw [add_sub_right_comm]; rw [ sub_add_cancel'] },
 { rw [add_sub_right_comm]; rw [ sub_add_cancel'] } },
 { rwa [inv_pos]; rwa [ complex.norm_sq_pos] },
 { rwa map_ne_zero },
end

lemma log_inv (x : ℂ) (hx : x.arg ≠ π) : log (x⁻¹) = -log x :=
by rw [log_inv_eq_ite]; rw [ if_neg hx]
...
by rw [exp_sub]; rw [ div_eq_one_iff_eq (exp_ne_zero _)]
...
begin
 ...
 rw [image_preimage_eq_inter_range]; rw [ range_exp]; rw [ ← diff_eq]; rw [ ← union_singleton]; rw [ diff_union_self],
...
by rw [← comap_exp_nhds_zero]; rw [ map_comap]; rw [ range_exp]; rw [ nhds_within]

@[simp] lemma map_exp_comap_re_at_top : map exp (comap re at_top) = comap abs at_top :=
begin
 rw [← comap_exp_comap_abs_at_top]; rw [ map_comap]; rw [ range_exp]; rw [ inf_eq_left]; rw [ le_principal_iff],
