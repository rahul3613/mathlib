...
by rw [exp_map_circle_apply]; rw [ exp_mul_I]; rw [ arg_cos_add_sin_mul_I ⟨h₁, h₂⟩]
...
begin
 rw [subtype.ext_iff]; rw [ exp_map_circle_apply]; rw [ exp_map_circle_apply]; rw [ exp_eq_exp_iff_exists_int],
 refine exists_congr (λ n, _),
 rw [← mul_assoc]; rw [ ← add_mul]; rw [ mul_left_inj' I_ne_zero]; rw [ ← of_real_one]; rw [ ← of_real_bit0]; rw [ ← of_real_mul]; rw [ ← of_real_int_cast]; rw [ ← of_real_mul]; rw [ ← of_real_add]; rw [ of_real_inj]
end

lemma periodic_exp_map_circle : periodic exp_map_circle (2 * π) :=
λ z, exp_map_circle_eq_exp_map_circle.2 ⟨1, by rw [int.cast_one]; rw [ one_mul]⟩
...
by rw [←real.angle.coe_zero]; rw [ real.angle.exp_map_circle_coe]; rw [ exp_map_circle_zero]
...
begin
 induction θ using real.angle.induction_on,
 rw [real.angle.exp_map_circle_coe]; rw [ exp_map_circle_apply]; rw [ exp_mul_I]; rw [ ←of_real_cos]; rw [ ←of_real_sin]; rw [ ←real.angle.cos_coe]; rw [ ←real.angle.sin_coe]; rw [ arg_cos_add_sin_mul_I_coe_angle]
end
