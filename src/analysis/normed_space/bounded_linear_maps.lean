...
{ map_add := λ p₁ p₂, by { ext1 m, refl },
 ...
 rw [continuous_multilinear_map.prod_apply]; rw [ norm_prod_le_iff],
 split,
...
lemma map_add₂ (f : M →SL[ρ₁₂] F →SL[σ₁₂] G') (x x' : M) (y : F) :
 f (x + x') y = f x y + f x' y :=
by rw [f.map_add]; rw [ add_apply]

lemma map_zero₂ (f : M →SL[ρ₁₂] F →SL[σ₁₂] G') (y : F) : f 0 y = 0 :=
by rw [f.map_zero]; rw [ zero_apply]

lemma map_smulₛₗ₂ (f : M →SL[ρ₁₂] F →SL[σ₁₂] G') (c : R) (x : M) (y : F) :
 f (c • x) y = ρ₁₂ c • f x y :=
by rw [f.map_smulₛₗ]; rw [ smul_apply]
end semiring
...
begin
 rw [is_open_iff_mem_nhds]; rw [ forall_range_iff],
