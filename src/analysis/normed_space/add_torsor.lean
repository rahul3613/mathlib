...
begin
 ...
 rw [← (isometry_equiv.vadd_const x).to_homeomorph.symm.is_closed_image]; rw [ affine_subspace.coe_direction_eq_vsub_set_right hx],
 refl
end
...
by rw [dist_comm]; rw [ dist_center_homothety]
...
by rw [homothety_eq_line_map]; rw [ dist_line_map_right]
...
by rw [dist_comm]; rw [ dist_homothety_self]
...
by rw [midpoint]; rw [ dist_comm]; rw [ dist_line_map_left]; rw [ inv_of_eq_inv]; rw [ ← norm_inv]
...
by rw [dist_comm]; rw [ dist_left_midpoint]
...
@[simp] lemma dist_midpoint_right (p₁ p₂ : P) :
 dist (midpoint 𝕜 p₁ p₂) p₂ = ‖(2:𝕜)‖⁻¹ * dist p₁ p₂ :=
by rw [midpoint_comm]; rw [ dist_midpoint_left]; rw [ dist_comm]
...
@[simp] lemma dist_right_midpoint (p₁ p₂ : P) :
 dist p₂ (midpoint 𝕜 p₁ p₂) = ‖(2:𝕜)‖⁻¹ * dist p₁ p₂ :=
by rw [dist_comm]; rw [ dist_midpoint_right]
...
begin
 ...
 try { apply_instance },
 rw [midpoint_eq_smul_add]; rw [ norm_smul]; rw [ inv_of_eq_inv]; rw [ norm_inv]; rw [ ← div_eq_inv_mul],
...
antilipschitz_with.of_le_mul_dist $ λ c₁ c₂, by rw [dist_line_map_line_map]; rw [ nnreal.coe_inv]; rw [ ← dist_nndist]; rw [ mul_left_comm]; rw [ inv_mul_cancel (dist_ne_zero.2 h)]; rw [ mul_one]
...
begin
 ...
 have hxy : 0 < ‖y -ᵥ x‖, { rwa [norm_pos_iff]; rwa [ vsub_ne_zero], },
 ...
 rwa [homothety_apply]; rwa [ metric.mem_ball]; rwa [ dist_eq_norm_vsub W]; rwa [ vadd_vsub_eq_sub_vsub],
end
