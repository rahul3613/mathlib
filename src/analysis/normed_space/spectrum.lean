...
lemma mem_resolvent_set_of_norm_lt_mul {a : A} {k : 𝕜} (h : ‖a‖ * ‖(1 : A)‖ < ‖k‖) :
 k ∈ ρ a :=
begin
 rw [resolvent_set]; rw [ set.mem_set_of_eq]; rw [ algebra.algebra_map_eq_smul_one],
 nontriviality A,
 have hk : k ≠ 0,
 ...
 rw [←inv_inv (‖(1 : A)‖)]; rw [ mul_inv_lt_iff (inv_pos.2 $ norm_pos_iff.2 (one_ne_zero : (1 : A) ≠ 0))]
 at h,
...
lemma mem_resolvent_set_of_norm_lt [norm_one_class A] {a : A} {k : 𝕜} (h : ‖a‖ < ‖k‖) :
 k ∈ ρ a :=
mem_resolvent_set_of_norm_lt_mul (by rwa [norm_one]; rwa [ mul_one])
...
begin
 ...
 erw [coe_pow]; erw [ ←rpow_nat_cast]; erw [ ←rpow_mul]; erw [ mul_one_div_cancel hn.ne']; erw [ rpow_one],
...
begin
 ...
 rw [is_O_with_iff] at hc; rw [ eventually_iff] at hc; rw [ metric.mem_nhds_iff] at hc,
 ...
 have lt_δ : ‖z⁻¹ • a‖ < δ,
 { rw [units.smul_def]; rw [ norm_smul]; rw [ units.coe_inv]; rw [ norm_inv],
 calc ‖(z : 𝕜)‖⁻¹ * ‖a‖ ≤ δ * (‖a‖ + 1)⁻¹ * ‖a‖
 ...
 rw [←inv_smul_smul z (resolvent a (z : 𝕜))]; rw [ units_smul_resolvent_self]; rw [ resolvent]; rw [ algebra.algebra_map_eq_smul_one]; rw [ one_smul]; rw [ units.smul_def]; rw [ norm_smul]; rw [ units.coe_inv]; rw [ norm_inv],
...
{ r_le :=
 ...
 rw [←norm_to_nnreal]; rw [ norm_mk_pi_field]; rw [ norm_to_nnreal],
 cases n,
 ...
 { rw [←coe_inv h] at hr; rw [ coe_lt_coe] at hr; rw [ nnreal.lt_inv_iff_mul_lt h] at hr,
 ...
 rwa [←coe_nnnorm]; rwa [ ←real.lt_to_nnreal_iff_coe_lt]; rwa [ real.to_nnreal_one]; rwa [ nnnorm_smul]; rwa [ ←nnreal.lt_inv_iff_mul_lt h] } },
...
begin
 ...
 { rwa [is_unit.smul_sub_iff_sub_inv_smul] at hu ; rwa [ inv_inv u] at hu },
 { rw [units.smul_def]; rw [ ←algebra.algebra_map_eq_smul_one]; rw [ ←mem_resolvent_set_iff],
 refine mem_resolvent_set_of_spectral_radius_lt _,
 rwa [units.coe_inv]; rwa [ nnnorm_inv]; rwa [ coe_inv (nnnorm_ne_zero_iff.mpr (units.coe_mk0 hz ▸ hz : (u : 𝕜) ≠ 0))]; rwa [ lt_inv_iff_lt_inv] } }
end
...
begin
 ...
 congr,
 ext n,
 rw [norm_to_nnreal]; rw [ ennreal.coe_rpow_def (‖a ^ n‖₊) (1 / n : ℝ)]; rw [ if_neg],
...
begin
 ...
 ext1,
 rw [←of_real_rpow_of_nonneg (norm_nonneg _) _]; rw [ ←coe_nnnorm]; rw [ coe_nnreal_eq],
...
begin
 ...
 by_contra h,
 have H₀ : resolvent_set ℂ a = set.univ, by rwa [spectrum] at h; rwa [ set.compl_empty_iff] at h,
...
by rwa [mem_iff] at h; rwa [ hA] at h; rwa [ not_not] at h; rwa [ sub_eq_zero] at h
...
begin
 ...
 { rw [algebra_map_exp_comm z]; rw [ ←exp_add_of_commute (algebra.commutes z (a - ↑ₐz)).symm]; rw [ sub_add_cancel] },
 ...
 rw [norm_smul]; rw [ mul_comm]; rw [ norm_inv]; rw [ is_R_or_C.norm_nat_cast]; rw [ ← div_eq_mul_inv],
 ...
 exact h₀.symm },
 rw [spectrum.mem_iff]; rw [ is_unit.sub_iff]; rw [ ←one_mul (↑ₐ(exp 𝕜 z))]; rw [ hexpmul]; rw [ ←_root_.sub_mul]; rw [ commute.is_unit_mul_iff (algebra.commutes (exp 𝕜 z) (exp 𝕜 (a - ↑ₐz) - 1)).symm]; rw [ sub_eq_iff_eq_add'.mpr h₃]; rw [ commute.is_unit_mul_iff (h₀ ▸ h₁ : (a - ↑ₐz) * b = b * (a - ↑ₐz))],
