...
lemma norm_zsmul (α) [normed_field α] [normed_space α β] (n : ℤ) (x : β) :
 ‖n • x‖ = ‖(n : α)‖ * ‖x‖ :=
by rw [← norm_smul]; rw [ ← int.smul_one_eq_coe]; rw [ smul_assoc]; rw [ one_smul]
...
lemma norm_smul_of_nonneg [normed_space ℝ β] {t : ℝ} (ht : 0 ≤ t) (x : β) :
 ‖t • x‖ = t * ‖x‖ := by rw [norm_smul]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg ht]
...
begin
 ...
 rw [mem_closed_ball] at hy; rw [ dist_eq_norm] at hy,
...
begin
 rw [frontier]; rw [ closure_ball x hr]; rw [ is_open_ball.interior_eq],
...
begin
 ...
 { rw [closed_ball_eq_empty.2 hr]; rw [ ball_eq_empty.2 hr.le]; rw [ interior_empty] },
...
by rw [frontier]; rw [ closure_closed_ball]; rw [ interior_closed_ball x hr]; rw [ closed_ball_diff_ball]

theorem interior_sphere [normed_space ℝ E] (x : E) {r : ℝ} (hr : r ≠ 0) :
 interior (sphere x r) = ∅ :=
by rw [←frontier_closed_ball x hr]; rw [ interior_frontier is_closed_ball]

theorem frontier_sphere [normed_space ℝ E] (x : E) {r : ℝ} (hr : r ≠ 0) :
 frontier (sphere x r) = sphere x r :=
by rw [is_closed_sphere.frontier_eq]; rw [ interior_sphere x hr]; rw [ diff_empty]
...
begin
 ...
 { rw [discrete_topology_iff_open_singleton_zero]; rw [ is_open_induced_iff],
 refine ⟨metric.ball 0 (‖e‖), metric.is_open_ball, _⟩,
 ext ⟨x, hx⟩,
 obtain ⟨k, rfl⟩ := add_subgroup.mem_zmultiples_iff.mp hx,
 rw [mem_preimage]; rw [ mem_ball_zero_iff]; rw [ add_subgroup.coe_mk]; rw [ mem_singleton_iff]; rw [ subtype.ext_iff]; rw [ add_subgroup.coe_mk]; rw [ add_subgroup.coe_zero]; rw [ norm_zsmul ℚ k e]; rw [ int.norm_cast_rat]; rw [ int.norm_eq_abs]; rw [ ← int.cast_abs]; rw [ mul_lt_iff_lt_one_left (norm_pos_iff.mpr he)]; rw [ ← @int.cast_one ℝ _]; rw [ int.cast_lt]; rw [ int.abs_lt_one_iff]; rw [ smul_eq_zero]; rw [ or_iff_left he], },
end
...
{ to_fun := λ x, ⟨(1 + ‖x‖^2).sqrt⁻¹ • x, begin
 ...
 rw [mem_ball_zero_iff]; rw [ norm_smul]; rw [ real.norm_eq_abs]; rw [ abs_inv]; rw [ ← div_eq_inv_mul]; rw [ div_lt_one (abs_pos.mpr $ real.sqrt_ne_zero'.mpr this)]; rw [ ← abs_norm x]; rw [ ← sq_lt_sq]; rw [ abs_norm]; rw [ real.sq_sqrt this.le],
 exact lt_one_add _,
 end⟩,
...
begin
 ...
 show ‖c ^ (-(n + 1)) • x‖ < ε,
 { rw [norm_smul]; rw [ zpow_neg]; rw [ norm_inv]; rw [ ← div_eq_inv_mul]; rw [ div_lt_iff cnpos]; rw [ mul_comm]; rw [ norm_zpow],
 exact (div_lt_iff εpos).1 (hn.2) },
 show ε / ‖c‖ ≤ ‖c ^ (-(n + 1)) • x‖,
 { rw [zpow_neg]; rw [ div_le_iff cpos]; rw [ norm_smul]; rw [ norm_inv]; rw [ norm_zpow]; rw [ zpow_add₀ (ne_of_gt cpos)]; rw [ zpow_one]; rw [ mul_inv_rev]; rw [ mul_comm]; rw [ ← mul_assoc]; rw [ ← mul_assoc]; rw [ mul_inv_cancel (ne_of_gt cpos)]; rw [ one_mul]; rw [ ← div_eq_inv_mul]; rw [ le_div_iff (zpow_pos_of_pos cpos _)]; rw [ mul_comm],
 exact (le_div_iff εpos).1 hn.1 },
 show ‖c ^ (-(n + 1))‖⁻¹ ≤ ε⁻¹ * ‖c‖ * ‖x‖,
 { rw [zpow_neg]; rw [ norm_inv]; rw [ inv_inv]; rw [ norm_zpow]; rw [ zpow_add₀ cpos.ne']; rw [ zpow_one]; rw [ mul_right_comm]; rw [ ← div_eq_inv_mul],
...
begin
 ...
 { rw [closed_ball_zero]; rw [ ball_zero]; rw [ interior_singleton] },
 { exact interior_closed_ball x hr }
end

theorem frontier_closed_ball' [normed_space ℝ E] [nontrivial E] (x : E) (r : ℝ) :
 frontier (closed_ball x r) = sphere x r :=
by rw [frontier]; rw [ closure_closed_ball]; rw [ interior_closed_ball' x r]; rw [ closed_ball_diff_ball]

@[simp] theorem interior_sphere' [normed_space ℝ E] [nontrivial E] (x : E) (r : ℝ) :
 interior (sphere x r) = ∅ :=
by rw [←frontier_closed_ball' x]; rw [ interior_frontier is_closed_ball]

@[simp] theorem frontier_sphere' [normed_space ℝ E] [nontrivial E] (x : E) (r : ℝ) :
 frontier (sphere x r) = sphere x r :=
by rw [is_closed_sphere.frontier_eq]; rw [ interior_sphere' x]; rw [ diff_empty]
...
by rw [norm_algebra_map]; rw [ norm_one]; rw [ mul_one]
...
begin
 ...
 rw [dist_eq_norm]; rw [ dist_eq_norm]; rw [ ← ring_hom.map_sub]; rw [ norm_algebra_map'],
end
...
instance normed_algebra_rat {𝕜} [normed_division_ring 𝕜] [char_zero 𝕜] [normed_algebra ℝ 𝕜] :
 normed_algebra ℚ 𝕜 :=
{ norm_smul_le := λ q x,
 by rw [←smul_one_smul ℝ q x]; rw [ rat.smul_one_eq_coe]; rw [ norm_smul]; rw [ rat.norm_cast_real], }
