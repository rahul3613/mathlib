...
begin
 ...
 rwa [mem_ball]; rwa [ dist_eq_norm]; rwa [ sub_zero] },
 ...
 ... ≤ δ + δ : begin
 apply add_le_add,
 { rw [← dist_eq_norm]; rw [ dist_comm], exact le_of_lt h₁ },
 { rw [← dist_eq_norm]; rw [ dist_comm], exact le_of_lt h₂ }
 end
 ... = 2 * δ : (two_mul _).symm,
 have J : ‖f (d⁻¹ • x) - y‖ ≤ 1/2 * ‖y‖ := calc
 ‖f (d⁻¹ • x) - y‖ = ‖d⁻¹ • f x - (d⁻¹ * d) • y‖ :
 by rwa [f.map_smul _]; rwa [ inv_mul_cancel]; rwa [ one_smul]
 ... = ‖d⁻¹ • (f x - d • y)‖ : by rw [mul_smul]; rw [ smul_sub]
 ... = ‖d‖⁻¹ * ‖f x - d • y‖ : by rw [norm_smul]; rw [ norm_inv]
 ... ≤ ‖d‖⁻¹ * (2 * δ) : begin
 ...
 ... = ‖y‖/2 : by { rw [inv_mul_cancel]; rw [ one_mul], simp [norm_eq_zero, hd] }
 ... = (1/2) * ‖y‖ : by ring,
 rw ← dist_eq_norm at J,
 have K : ‖d⁻¹ • x‖ ≤ (ε / 2)⁻¹ * ‖c‖ * 2 * ↑n * ‖y‖ := calc
 ‖d⁻¹ • x‖ = ‖d‖⁻¹ * ‖x₁ - x₂‖ : by rw [norm_smul]; rw [ norm_inv]
 ... ≤ ((ε / 2)⁻¹ * ‖c‖ * ‖y‖) * (n + n) : begin
...
begin
 ...
 ... = 2 * C * ‖y‖ : by rw [tsum_geometric_two]; rw [ mul_assoc]
 ...
 induction n with n IH,
 { simp [f.map_zero] },
 { rw [sum_range_succ]; rw [ f.map_add]; rw [ IH]; rw [ iterate_succ']; rw [ sub_add] } },
...
begin
 ...
 have : f (x + w) = z, by { rw [f.map_add]; rw [ wim]; rw [ fxy]; rw [ add_sub_cancel'_right] },
 rw ← this,
 ...
 rwa [mem_ball] at hz; rwa [ dist_eq_norm] at hz,
 end
...
continuous_linear_equiv.of_bijective (f.coprod G.subtypeL)
 ...
 { rw [hker]; rw [ submodule.ker_subtypeL]; rw [ submodule.prod_bot] },
...
begin
 rw [coprod_subtypeL_equiv_of_is_compl]; rw [ _root_.coe_coe]; rw [ continuous_linear_equiv.coe_of_bijective]; rw [ coe_coprod]; rw [ linear_map.coprod_map_prod]; rw [ submodule.map_bot]; rw [ sup_bot_eq]; rw [ submodule.map_top],
 refl
end
