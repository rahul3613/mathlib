...
set.subset_compl_comm.mp $ λ x hx, by simpa [sub_sub_self, units.coe_one_sub] using
 (units.one_sub (1 - x) (by rwa [metric.mem_ball] at hx)).is_unit; rwa [ dist_eq_norm] at hx)).is_unit; rwa [ norm_sub_rev] at hx)).is_unit
...
by rw [← inverse_unit (units.one_sub t h)]; rw [ units.coe_one_sub]
...
begin
 nontriviality R,
 rw [eventually_iff]; rw [ metric.mem_nhds_iff],
...
begin
 ...
 rw [geom_sum_mul_neg],
 simp },
 rw [← one_mul ↑(units.one_sub t ht)⁻¹]; rw [ h]; rw [ add_mul],
 congr,
 { rw [mul_assoc]; rw [ (units.one_sub t ht).mul_inv],
 simp },
 { simp only [units.coe_one_sub],
 rw [← add_mul]; rw [ geom_sum_mul_neg],
 simp }
end
...
begin
 ...
 rw [continuous_at]; rw [ tendsto_iff_norm_tendsto_zero]; rw [ inverse_unit],
...
begin
 ...
 intros x s,
 rw [mem_map]; rw [ mem_nhds_induced],
 rintros ⟨t, ht, hts⟩,
