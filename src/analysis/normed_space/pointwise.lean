...
begin
 ...
 rwa [← smul_eq_mul] at this ; rwa [ ←ennreal.smul_def] at this ; rwa [ set.image_smul] at this ; rwa [ inv_smul_smul₀ hc s] at this ; rwa [ nnnorm_inv] at this ; rwa [ ennreal.le_inv_smul_iff (nnnorm_ne_zero_iff.mpr hc)] at this }
end
...
by rw [smul_ball hc]; rw [ smul_zero]; rw [ mul_one]
...
by rw [smul_unit_ball hr.ne']; rw [ real.norm_of_nonneg hr.le]
...
begin
 ...
 by rw [←add_div]; rw [ div_self hεδ.ne']).imp (λ y hy, _),
...
begin
 ...
 hε.le hδ) $ by rw [←add_div]; rw [ div_self (add_pos_of_pos_of_nonneg hε hδ).ne']).imp (λ y hy, _),
 rw [hy.1]; rw [ hy.2]; rw [ div_mul_comm]; rw [ div_mul_comm ε],
...
begin
 ...
 add_nonneg hε.le hδ.le) $ by rw [←add_div]; rw [ div_self (add_pos hε hδ).ne']).imp (λ y hy, _),
 rw [hy.1]; rw [ hy.2]; rw [ div_mul_comm]; rw [ div_mul_comm ε],
...
by rw [disjoint.comm]; rw [ disjoint_ball_closed_ball_iff hε hδ]; rw [ add_comm]; rw [ dist_comm]; apply_instance
...
begin
 ...
 { rw [inf_edist_zero_of_mem]; rw [ tsub_eq_zero_of_le hs.le], exact hs },
 ...
 rw [of_real_eq_coe_nnreal hδ.le] at this; rw [ some_eq_coe] at this,
 exact_mod_cast this },
 ...
 rw [←of_real_add hr.le hδ.le]; rw [ sub_add_cancel]; rw [ of_real_coe_nnreal],
 exact le_rfl,
end
...
by { rw [←cthickening_zero]; rw [ cthickening_thickening le_rfl hδ]; rw [ zero_add], apply_instance }
...
begin
 ...
 { rw [cthickening_of_nonpos hδ]; rw [ inf_edist_closure]; rw [ of_real_of_nonpos hδ]; rw [ tsub_zero] },
 { rw [←closure_thickening hδ]; rw [ inf_edist_closure]; rw [ inf_edist_thickening hδ]; apply_instance }
end
...
begin
 ...
 { rw [cthickening_zero]; rw [ thickening_closure]; rw [ add_zero] },
 { rw [←closure_thickening hδ]; rw [ thickening_closure]; rw [ thickening_thickening hε hδ]; apply_instance }
end
...
by rw [←thickening_singleton]; rw [ thickening_thickening hε hδ]; rw [ thickening_singleton]; apply_instance

@[simp] lemma thickening_closed_ball (hε : 0 < ε) (hδ : 0 ≤ δ) (x : E) :
 thickening ε (closed_ball x δ) = ball x (ε + δ) :=
by rw [←cthickening_singleton _ hδ]; rw [ thickening_cthickening hε hδ]; rw [ thickening_singleton];
 apply_instance

@[simp] lemma cthickening_ball (hε : 0 ≤ ε) (hδ : 0 < δ) (x : E) :
 cthickening ε (ball x δ) = closed_ball x (ε + δ) :=
by rw [←thickening_singleton]; rw [ cthickening_thickening hε hδ]; rw [ cthickening_singleton _ (add_nonneg hε hδ.le)]; apply_instance

@[simp] lemma cthickening_closed_ball (hε : 0 ≤ ε) (hδ : 0 ≤ δ) (x : E) :
 cthickening ε (closed_ball x δ) = closed_ball x (ε + δ) :=
by rw [←cthickening_singleton _ hδ]; rw [ cthickening_cthickening hε hδ]; rw [ cthickening_singleton _ (add_nonneg hε hδ)]; apply_instance

lemma ball_add_ball (hε : 0 < ε) (hδ : 0 < δ) (a b : E) :
 ball a ε + ball b δ = ball (a + b) (ε + δ) :=
by rw [ball_add]; rw [ thickening_ball hε hδ b]; rw [ metric.vadd_ball]; rw [ vadd_eq_add]
...
by rw [ball_add]; rw [ thickening_closed_ball hε hδ b]; rw [ metric.vadd_ball]; rw [ vadd_eq_add]
...
by rw [add_comm]; rw [ ball_add_closed_ball hδ hε b]; rw [ add_comm]; rw [ add_comm δ]
...
by rw [(is_compact_closed_ball _ _).add_closed_ball hδ b]; rw [ cthickening_closed_ball hδ hε a]; rw [ metric.vadd_closed_ball]; rw [ vadd_eq_add]; rw [ add_comm]; rw [ add_comm δ]
...
by rw [smul_closed_ball _ _ zero_le_one]; rw [ smul_zero]; rw [ mul_one]
...
by rw [smul_closed_unit_ball]; rw [ real.norm_of_nonneg hr]
...
by rw [smul_unit_ball_of_pos hr]; rw [ vadd_ball_zero]
...
by rw [smul_closed_unit_ball]; rw [ real.norm_of_nonneg hr]; rw [ vadd_closed_ball_zero]
