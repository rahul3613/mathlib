...
begin
 ...
 { rw [← mul_assoc]; rw [ nnnorm_inv]; rw [ ennreal.coe_inv]; rw [ ennreal.mul_inv_cancel _ ennreal.coe_ne_top]; rw [ one_mul]; simp [hc] }
end

@[simp] lemma map_zero : e 0 = 0 :=
by { rw [← zero_smul 𝕜 (0:V)]; rw [ e.map_smul], norm_num }

@[simp] lemma eq_zero_iff {x : V} : e x = 0 ↔ x = 0 :=
⟨e.eq_zero' x, λ h, h.symm ▸ e.map_zero⟩

@[simp] lemma map_neg (x : V) : e (-x) = e x :=
calc e (-x) = ‖(-1 : 𝕜)‖₊ * e x : by rw [← map_smul]; rw [ neg_one_smul]
 ... = e x : by simp

lemma map_sub_rev (x y : V) : e (x - y) = e (y - x) :=
by rw [← neg_sub]; rw [ e.map_neg]
