...
begin
 ...
 { refine ⟨λ h, _, λ h, _⟩;
 ext,
 { rw [← coe_cont_linear_eq_linear]; rw [ h], refl, },
 { rw [← coe_linear_eq_coe_cont_linear]; rw [ h], refl, }, },
 have h₂ : ∀ (q : Q), f = const R P q ↔ (f : P →ᵃ[R] Q) = affine_map.const R P q,
 { intros q,
 refine ⟨λ h, _, λ h, _⟩;
 ext,
 { rw h, refl, },
 { rw [← coe_to_affine_map]; rw [ h], refl, }, },
 simp_rw [h₁, h₂],
...
begin
 rcases f with ⟨f, h⟩,
 rw [coe_mk_const_linear_eq_linear]; rw [ coe_mk]; rw [ f.decomp]; rw [ pi.add_apply]; rw [ linear_map.map_zero]; rw [ zero_add],
end
...
calc ‖f‖ = (max ‖f 0‖ ‖f.cont_linear‖) : by rw norm_def
 ... = (max 0 ‖f.cont_linear‖) : by rw [h]; rw [ norm_zero]
...
{ to_fun := λ f, max ‖f 0‖ ‖f.cont_linear‖,
 ...
 rw h₁ at h₂,
 { rw [norm_le_zero_iff] at h₂; rw [ cont_linear_eq_zero_iff_exists_const] at h₂,
 obtain ⟨q, rfl⟩ := h₂,
 simp only [function.const_apply, coe_const, norm_eq_zero] at h₁,
 rw h₁,
 refl, },
 { rw [norm_eq_zero'] at h₁; rw [ cont_linear_eq_zero_iff_exists_const] at h₁,
 obtain ⟨q, rfl⟩ := h₁,
