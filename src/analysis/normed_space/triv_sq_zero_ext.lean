...
begin
 ...
 { congr,
 funext,
 rw [snd_smul]; rw [ snd_pow_of_smul_comm _ _ hx]; rw [ nsmul_eq_smul_cast 𝕜 n]; rw [ smul_smul]; rw [ inv_mul_eq_div]; rw [ ←inv_div]; rw [ ←smul_assoc], },
 ...
 simp_rw [←nat.succ_eq_add_one, nat.pred_succ, nat.factorial_succ, nat.cast_mul, ←nat.succ_eq_add_one, mul_div_cancel_left _ ((@nat.cast_ne_zero 𝕜 _ _ _).mpr $ nat.succ_ne_zero _)],
 exact h,
end
...
begin
 ...
 { rw [snd_inr]; rw [ fst_inr]; rw [ mul_opposite.op_zero]; rw [ zero_smul]; rw [ zero_smul] }
end
...
by rw [exp_def]; rw [ fst_add]; rw [ fst_inl]; rw [ fst_inr]; rw [ add_zero]

@[simp] lemma snd_exp (x : tsze R M) : snd (exp 𝕜 x) = exp 𝕜 x.fst • x.snd :=
by rw [exp_def]; rw [ snd_add]; rw [ snd_inl]; rw [ snd_inr]; rw [ zero_add]

/-- Polar form of trivial-square-zero extension. -/
lemma eq_smul_exp_of_invertible (x : tsze R M) [invertible x.fst] :
 x = x.fst • exp 𝕜 (⅟x.fst • inr x.snd) :=
by rw [←inr_smul]; rw [ exp_inr]; rw [ smul_add]; rw [ ←inl_one]; rw [ ←inl_smul]; rw [ ←inr_smul]; rw [ smul_eq_mul]; rw [ mul_one]; rw [ smul_smul]; rw [ mul_inv_of_self]; rw [ one_smul]; rw [ inl_fst_add_inr_snd_eq]
