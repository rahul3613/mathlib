...
begin
 ...
 cases n,
 { rw [pow_zero]; rw [ nat.factorial_zero]; rw [ nat.cast_one]; rw [ inv_one]; rw [ one_smul]; rw [ pi.single_eq_same], },
 { rw [zero_pow (nat.succ_pos _)]; rw [ smul_zero]; rw [ pi.single_eq_of_ne (n.succ_ne_zero)], },
end
...
by rw [div_eq_mul_inv]; rw [ ←(nat.cast_commute n! (x ^ n)).inv_left₀.eq]; rw [ ←smul_eq_mul]; rw [ exp_series_apply_eq]; rw [ inv_nat_cast_smul_eq _ _ _ _]
...
lemma exp_add_of_commute_of_mem_ball [char_zero 𝕂]
 ...
 exp 𝕂 (x + y) = (exp 𝕂 x) * (exp 𝕂 y) :=
begin
 rw [exp_eq_tsum]; rw [ tsum_mul_tsum_eq_tsum_sum_antidiagonal_of_summable_norm (norm_exp_series_summable_of_mem_ball' x hx) (norm_exp_series_summable_of_mem_ball' y hy)],
 dsimp only,
 conv_lhs {congr, funext, rw [hxy.add_pow' _]; rw [ finset.smul_sum]},
...
begin
 ...
 rw [norm_mul]; rw [ norm_norm (exp_series 𝕂 𝔸 n)]; rw [ exp_series]; rw [ norm_smul]; rw [ norm_inv]; rw [ norm_pow]; rw [ nnreal.norm_eq]; rw [ norm_nat_cast]; rw [ mul_comm]; rw [ ←mul_assoc]; rw [ ←div_eq_mul_inv],
...
begin
 rw [continuous_iff_continuous_on_univ]; rw [ ← metric.eball_top_eq_univ (0 : 𝔸)]; rw [ ← exp_series_radius_eq_top 𝕂 𝔸],
...
lemma exp_mem_unitary_of_mem_skew_adjoint [star_ring 𝔸] [has_continuous_star 𝔸] {x : 𝔸}
 (h : x ∈ skew_adjoint 𝔸) :
 exp 𝕂 x ∈ unitary 𝔸 :=
by rw [unitary.mem_iff]; rw [ star_exp]; rw [ skew_adjoint.mem_iff.mp h]; rw [ ←exp_add_of_commute (commute.refl x).neg_left]; rw [ ←exp_add_of_commute (commute.refl x).neg_right]; rw [ add_left_neg]; rw [ add_right_neg]; rw [ exp_zero]; rw [ and_self]
...
begin
 ...
 { simp },
 rw [finset.noncomm_prod_insert_of_not_mem _ _ _ _ ha]; rw [ finset.sum_insert ha]; rw [ exp_add_of_commute]; rw [ ih (h.mono $ finset.subset_insert _ _)],
...
begin
 ...
 { rw [zero_smul]; rw [ pow_zero]; rw [ exp_zero], },
 { rw [succ_nsmul]; rw [ pow_succ]; rw [ exp_add_of_commute ((commute.refl x).smul_right n)]; rw [ ih] }
end
...
begin
 ...
 { rw [zpow_coe_nat]; rw [ coe_nat_zsmul]; rw [ exp_nsmul] },
 { rw [zpow_neg]; rw [ zpow_coe_nat]; rw [ neg_smul]; rw [ exp_neg]; rw [ coe_nat_zsmul]; rw [ exp_nsmul] },
end
...
begin
 rw [exp_sum_of_commute]; rw [ finset.noncomm_prod_eq_prod],
...
by rw [exp_series_apply_eq]; rw [ exp_series_apply_eq]; rw [ inv_nat_cast_smul_eq 𝕂 𝕂']
