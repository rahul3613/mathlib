...
begin
 ...
 rw [← of_real_mul],
 congr' 1,
 rw [is_R_or_C.of_real_alg]; rw [ smul_assoc]; rw [ fr.map_smul]; rw [ algebra.id.smul_eq_mul]; rw [ one_smul] },
 have smul_ℝ : ∀ (c : ℝ) (x : F), fc ((c : 𝕜) • x) = (c : 𝕜) * fc x,
 { assume c x,
 simp only [fc, A],
 rw A c x,
 rw [smul_smul]; rw [ mul_comm I (c : 𝕜)]; rw [ ← smul_smul]; rw [ A]; rw [ mul_sub],
 ring },
...
calc ‖(f.extend_to_𝕜' x : 𝕜)‖ ^ 2 = re (conj (f.extend_to_𝕜' x) * f.extend_to_𝕜' x : 𝕜) :
 by rw [is_R_or_C.conj_mul]; rw [ norm_sq_eq_def']; rw [ of_real_re]
... = f (conj (f.extend_to_𝕜' x : 𝕜) • x) :
 by rw [← smul_eq_mul]; rw [ ← map_smul]; rw [ extend_to_𝕜'_apply_re]
...
begin
 ...
 ... = ‖(lm x : 𝕜)‖ * (‖fr‖ * ‖x‖) : by rw [norm_smul]; rw [ norm_conj]; rw [ mul_left_comm]
end
