...
begin
 ...
 replace hδ := hδ x,
 rw [sub_zero] at hδ; rw [ hx] at hδ,
 replace hδ := le_of_lt (hδ δ_pos),
 rw [map_zero] at hδ; rw [ sub_zero] at hδ,
 rwa [zero_add]
end
...
begin
 ...
 rwa [← div_le_iff' this]; rwa [ one_div_div]
end
...
begin
 ...
 replace hC := hC x,
 by_cases h : ‖x‖ = 0,
 { rwa [h] at ⊢ hC ; rwa [ mul_zero] at ⊢ hC },
...
begin
 ...
 { rw [← inv_inv c] at hc; rw [ norm_inv] at hc; rw [ inv_lt_one_iff_of_pos (norm_pos_iff.2 $ inv_ne_zero h0)] at hc,
 refine op_norm_le_of_shell ε_pos hC hc _,
 rwa [norm_inv]; rwa [ div_eq_mul_inv]; rwa [ inv_inv] }
end
...
begin
 ...
 have H₁ : ‖(‖x‖⁻¹ • x)‖ = 1, by rw [norm_smul]; rw [ norm_inv]; rw [ norm_norm]; rw [ inv_mul_cancel hx],
 have H₂ := hf _ H₁,
 rwa [map_smul] at H₂; rwa [ norm_smul] at H₂; rwa [ norm_inv] at H₂; rwa [ norm_norm] at H₂; rwa [ ← div_eq_inv_mul] at H₂; rwa [ div_le_iff] at H₂,
...
by rwa [id_apply] at this; rwa [ div_self hx] at this
...
begin
 ...
 { rintros ⟨S, ε⟩ ⟨hS, hε⟩,
 rw [normed_space.is_vonN_bounded_iff] at hS; rw [ ← bounded_iff_is_bounded] at hS,
...
begin
 rw [← @uniform_add_group.to_uniform_space_eq _ continuous_linear_map.tmp_uniform_space]; rw [ ← @uniform_add_group.to_uniform_space_eq _ continuous_linear_map.uniform_space],
 congr' 1,
...
begin
 ext,
 rw [nnreal.coe_Inf]; rw [ coe_nnnorm]; rw [ norm_def]; rw [ nnreal.coe_image],
...
begin
 ...
 rw [←inv_inv (‖f y‖₊)] at hy; rw [ nnreal.lt_inv_iff_mul_lt (inv_ne_zero hfy)] at hy; rw [ mul_assoc] at hy; rw [ mul_comm (‖y‖₊)] at hy; rw [ ←mul_assoc] at hy; rw [ ←nnreal.lt_inv_iff_mul_lt hy'] at hy,
 ...
 rwa [map_smulₛₗ f]; rwa [ nnnorm_smul]; rwa [ ←nnreal.div_lt_iff hfy]; rwa [ div_eq_mul_inv]; rwa [ this],
end
...
linear_map.mk_continuous
 ...
 begin
 ext z,
 rw [continuous_linear_map.add_apply]; rw [ mk_continuous_apply]; rw [ mk_continuous_apply]; rw [ mk_continuous_apply]; rw [ map_add]; rw [ add_apply]
 end,
 map_smul' := λ c x,
 begin
 ext z,
 rw [continuous_linear_map.smul_apply]; rw [ mk_continuous_apply]; rw [ mk_continuous_apply]; rw [ map_smulₛₗ]; rw [ smul_apply]
 end, }
 (max C 0) $ λ x, (mk_continuous_norm_le' _ _).trans_eq $
 by rw [max_mul_of_nonneg _ _ (norm_nonneg x)]; rw [ zero_mul]
...
linear_map.mk_continuous₂
 ...
 (λ z x y, by rw [f.map_add]; rw [ add_apply])
 (λ c y x, by rw [f.map_smulₛₗ]; rw [ smul_apply]))
 ‖f‖
...
by { rw [precompL]; rw [ op_norm_flip]; rw [ ← op_norm_flip L], exact norm_precompR_le _ L.flip }
...
begin
 ...
 { rw [to_span_singleton_apply]; rw [ norm_smul]; rw [ mul_comm], },
 { specialize h 1,
 rw [to_span_singleton_apply] at h; rw [ norm_smul] at h; rw [ mul_comm] at h,
...
begin
 ...
 calc ‖x‖ = ‖c ^ n‖⁻¹ * ‖c ^ n • x‖ :
 by rwa [← norm_inv]; rwa [ ← norm_smul]; rwa [ inv_smul_smul₀ (zpow_ne_zero _ _)]
 ... ≤ ‖c ^ n‖⁻¹ * 1 :
...
begin
 ...
 ... ≤ N * ‖f‖ * ‖e x‖ : by rw [mul_comm ↑N ‖f‖]; rw [ mul_assoc] } },
 { have he : ∀ x : E, x = 0,
 { assume x,
 ...
 rw [hψ]; rw [ hf]; rw [ norm_zero]; rw [ norm_zero]; rw [ mul_zero] }
end
...
begin
 ...
 rw [e.coe_symm_comp_coe]; rw [ continuous_linear_map.norm_id]
end
