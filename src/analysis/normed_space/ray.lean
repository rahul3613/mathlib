...
begin
 rcases h.exists_eq_smul with ⟨u, a, b, ha, hb, -, rfl, rfl⟩,
 wlog hab : b ≤ a,
 { rw same_ray_comm at h, rw [norm_sub_rev]; rw [ abs_sub_comm],
 exact this u b a hb ha h (le_of_not_le hab), },
 rw ← sub_nonneg at hab,
 rw [← sub_smul]; rw [ norm_smul_of_nonneg hab]; rw [ norm_smul_of_nonneg ha]; rw [ norm_smul_of_nonneg hb]; rw [ ← sub_mul]; rw [ abs_of_nonneg (mul_nonneg hab (norm_nonneg _))]
end
...
by rw [inv_smul_eq_iff₀]; rw [ smul_comm]; rw [ eq_comm]; rw [ inv_smul_eq_iff₀]; rw [ same_ray_iff_norm_smul_eq];
