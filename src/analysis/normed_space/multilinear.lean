...
begin
 ...
 by { rw [← dist_eq_norm]; rw [ ← dist_eq_norm]; rw [ ← dist_eq_norm], exact dist_triangle _ _ _ }
 ... ≤ (C * ∑ i in s, ∏ j, if j = i then ‖m₁ i - m₂ i‖ else max ‖m₁ j‖ ‖m₂ j‖)
 + C * ∏ j, if j = i then ‖m₁ i - m₂ i‖ else max ‖m₁ j‖ ‖m₂ j‖ :
 add_le_add Hrec I
...
begin
 ...
 by { rw [sum_const]; rw [ card_univ]; rw [ nsmul_eq_mul], ring }
end
...
op_norm_le_bound _ zero_le_one $ λ m,
 by rw [fintype.prod_subsingleton _ i']; rw [ one_mul]; rw [ of_subsingleton_apply]
...
begin
 ...
 rwa [fintype.prod_subsingleton _ i'] at this; rwa [ of_subsingleton_apply] at this; rwa [ div_self hg] at this,
end
...
begin
 ...
 rw [fintype.prod_empty]; rw [ mul_one]; rw [ const_of_is_empty_apply], },
...
begin
 ...
 ... = ‖q.1 q.2 - q.1 p.2‖ + ‖q.1 p.2 - p.1 p.2‖ : by rw [dist_eq_norm]; rw [ dist_eq_norm]
...
begin
 ...
 { rw [ne.def]; rw [ list.map_eq_nil]; rw [ list.fin_range_eq_nil],
...
by ext; rw [mk_pi_field_apply]; rw [ smul_zero]; rw [ continuous_multilinear_map.zero_apply]

lemma mk_pi_field_eq_zero_iff (z : G) :
 continuous_multilinear_map.mk_pi_field 𝕜 ι z = 0 ↔ z = 0 :=
by rw [← mk_pi_field_zero]; rw [ mk_pi_field_eq_iff]
...
mk_continuous
 ...
 by { rw [max_mul_of_nonneg]; rw [ zero_mul], exact prod_nonneg (λ _ _, norm_nonneg _) }
...
... = (‖g‖ * ∏ i, ‖f i‖) * ∏ i, ‖m i‖ : by rw [prod_mul_distrib]; rw [ mul_assoc]
...
linear_isometry_equiv.of_bounds
 ...
 rw [sum.elim_comp_inl]; rw [ sum.elim_comp_inr] } }
