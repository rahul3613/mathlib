...
begin
 ...
 rw [←inv_inv (unitary.to_units u)] at hk; rw [ ←spectrum.map_inv] at hk; rw [ set.mem_inv] at hk,
...
begin
 ...
 refine funext (λ n, _),
 rw [function.comp_app]; rw [ ha.nnnorm_pow_two_pow]; rw [ ennreal.coe_pow]; rw [ ←rpow_nat_cast]; rw [ ←rpow_mul],
 simp,
end
...
begin
 ...
 { funext,
 rw [function.comp_apply]; rw [ ←rpow_nat_cast]; rw [ ←rpow_mul]; rw [ mul_comm]; rw [ rpow_mul]; rw [ rpow_nat_cast]; rw [ ←coe_pow]; rw [ sq]; rw [ ←nnnorm_star_mul_self]; rw [ commute.mul_pow (star_comm_self' a)]; rw [ star_pow], },
 ...
 rw [(is_self_adjoint.star_mul_self a).spectral_radius_eq_nnnorm]; rw [ sq]; rw [ nnnorm_star_mul_self]; rw [ coe_mul],
end
...
{ coe := λ φ, φ,
 ...
 rw [←heq]; rw [ is_R_or_C.star_def]; rw [ is_R_or_C.conj_of_real] }
 end }
