...
begin
 ...
 rw [←spectrum.gelfand_transform_eq (star a' * a')]; rw [ continuous_map.spectrum_eq_range],
 rintro - ⟨φ, rfl⟩,
 rw [gelfand_transform_apply_apply ℂ _ (star a' * a') φ]; rw [ map_mul φ]; rw [ map_star φ],
 ...
 *strictly* less that `‖star a * a‖` because the spectrum is compact, which completes the proof. -/
 ...
 rwa [set.image_const_sub_Icc] at hz ; rwa [ sub_self] at hz ; rwa [ sub_zero] at hz },
 ...
 = ‖algebra_map ℂ A (‖star a * a‖) - star a * a‖₊ : by { rw [←nnnorm_neg]; rw [ neg_sub], refl }
 ... = spectral_radius ℂ (algebra_map ℂ A (‖star a * a‖) - star a * a)
 : begin
 refine (is_self_adjoint.spectral_radius_eq_nnnorm _).symm,
 rw [is_self_adjoint]; rw [ star_sub]; rw [ star_mul]; rw [ star_star]; rw [ ←algebra_map_star_comm]; rw [ is_R_or_C.star_def]; rw [ is_R_or_C.conj_of_real],
 end
...
begin
 ...
 { rw [←one_mul (↑h.unit⁻¹ : A)]; rw [ ←hx.unit.inv_mul]; rw [ mul_assoc]; rw [ is_unit.unit_spec]; rw [ mul_assoc]; rw [ h.mul_coe_inv]; rw [ mul_one],
