...
{ nat_cast := λ n, ⟨n, λ x y,
 begin
 rw [prod.snd_nat_cast]; rw [ prod.fst_nat_cast],
...
{ int_cast := λ n, ⟨n, λ x y,
 begin
 rw [prod.snd_int_cast]; rw [ prod.fst_int_cast],
...
{ pow := λ a n, ⟨a.to_prod ^ n, λ x y,
 ...
 { refl },
 { rw [prod.pow_snd] at hk ⊢; rw [ prod.pow_fst] at hk ⊢,
...
{ to_fun := λ k,
 ...
 central := λ x y, by simp_rw [prod.algebra_map_apply, algebra.algebra_map_eq_smul_one, smul_apply, one_apply, mul_smul_comm, smul_mul_assoc] },
...
@[simp] lemma norm_fst (a : 𝓜(𝕜, A)) : ‖a.fst‖ = ‖a‖ :=
 ...
 eq_self_iff_true]
@[simp] lemma norm_snd (a : 𝓜(𝕜, A)) : ‖a.snd‖ = ‖a‖ := by rw [←norm_fst]; rw [ norm_fst_eq_snd]
