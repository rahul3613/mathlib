...
⟨begin
 ...
 ... ≤ ‖x⋆‖ * ‖x‖ : norm_mul_le _ _,
 have h₂ := calc
 ‖x⋆‖ * ‖x⋆‖ = ‖x * x⋆‖ : by rw [←norm_star_mul_self]; rw [ star_star]
 ... ≤ ‖x‖ * ‖x⋆‖ : norm_mul_le _ _,
...
by rw [norm_star_mul_self]; rw [ norm_star]
...
by { rw [←norm_eq_zero]; rw [ norm_star_mul_self], exact mul_self_eq_zero.trans norm_eq_zero }
...
{ norm_star_mul_self := λ x,
 ...
 { refine max_le _ _;
 rw [sq_le_sq]; rw [ abs_of_nonneg (norm_nonneg _)],
...
begin
 rw [←sq_eq_sq (norm_nonneg _) zero_le_one]; rw [ one_pow 2]; rw [ sq]; rw [ ←cstar_ring.norm_star_mul_self]; rw [ unitary.coe_star_mul_self]; rw [ cstar_ring.norm_one],
end
...
begin
 ...
 ... = ‖A‖ : by rw [norm_coe_unitary]; rw [ one_mul] },
 { calc _ = ‖(U : E)⋆ * U * A‖ : by rw [unitary.coe_star_mul_self U]; rw [ one_mul]
 ... ≤ ‖(U : E)⋆‖ * ‖(U : E) * A‖ : by { rw [mul_assoc], exact norm_mul_le _ _ }
 ... = ‖(U : E) * A‖ : by rw [norm_star]; rw [ norm_coe_unitary]; rw [ one_mul] },
end
...
begin
 ...
 { rw [pow_succ]; rw [ pow_mul']; rw [ sq],
 nth_rewrite 0 ←(self_adjoint.mem_iff.mp hx),
 rw [←star_pow]; rw [ cstar_ring.nnnorm_star_mul_self]; rw [ ←sq]; rw [ hk]; rw [ pow_mul'] },
end
