...
lemma eq_of_eq_on {f f' : α → E} {s : set α} (h : eq_on f f' s) :
 evariation_on f s = evariation_on f' s :=
eq_of_edist_zero_on (λ x xs, by rw [h xs]; rw [ edist_self])
...
begin
 rw [← ennreal.of_real_le_of_real_iff ennreal.to_real_nonneg]; rw [ ennreal.of_real_to_real h]; rw [ ← edist_dist],
...
begin
 ...
 { dsimp only [w], rw [if_neg (lt_irrefl N)]; rw [ if_pos rfl] },
 ...
 have : finset.Ico (N - 1) N = {N - 1}, by rw [← nat.Ico_succ_singleton]; rw [ A],
 ...
 have D : ¬(1 + i = N) := λ h, by
 { rw [←h] at hi; rw [ add_comm] at hi; rw [ nat.succ_le_iff] at hi, exact hi.left.ne rfl, },
 rw [if_neg A]; rw [ if_neg B]; rw [ if_neg C]; rw [ if_neg D],
 congr' 3;
 { rw [add_comm]; rw [ nat.sub_one], apply nat.pred_succ, } }
 end
 ... = ∑ i in finset.Ico 0 (N-1), edist (f (w (i + 1))) (f (w i)) +
...
begin
 ...
 by rw [finset.range_eq_Ico]; rw [ finset.sum_Ico_consecutive _ (zero_le _) hN.le]
...
begin
 ...
 rw [← evariation_on.union f A B]; rw [ ← inter_union_distrib_left]; rw [ Icc_union_Icc_eq_Icc hab hbc],
end
...
begin
 ...
 { rw [evariation_on.subsingleton]; rw [ evariation_on.subsingleton],
...
begin
 ...
 rw [if_pos le_rfl]; rw [ Icc_self]; rw [ evariation_on.subsingleton]; rw [ ennreal.zero_to_real],
...
by rw [eq_neg_swap]; rw [ neg_inj]; rw [ eq_of_le f s h]
...
begin
 ...
 rw [eq_of_le f s xy]; rw [ eq_of_le f s yz]; rw [ eq_of_le f s (xy.trans yz)]; rw [ ←ennreal.to_real_add (hf x y xs ys) (hf y z ys zs)]; rw [ evariation_on.Icc_add_Icc f xy yz ys], },
end
...
begin
 ...
 rw [eq_neg_swap]; rw [ h]; rw [ neg_zero] },
 { apply le_antisymm _ (zero_le _),
 rw [←ennreal.of_real_zero]; rw [ ←h]; rw [ eq_of_le f s h']; rw [ ennreal.of_real_to_real (hf a b ha hb)],
...
by rw [eq_of_le _ _ ab]; rw [ ennreal.to_real_eq_zero_iff]; rw [ or_iff_left (hf a b ha hb)]; rw [ evariation_on.eq_zero_iff]
...
by rw [eq_of_ge _ _ ba]; rw [ neg_eq_zero]; rw [ ennreal.to_real_eq_zero_iff]; rw [ or_iff_left (hf b a hb ha)]; rw [ evariation_on.eq_zero_iff]
...
begin
 ...
 ... = dist (f b) (f c) : by rw [dist_comm]; rw [ real.dist_eq]
 ... ≤ variation_on_from_to f s b c : by
 { rw [eq_of_le f s bc]; rw [ dist_edist],
 ...
 by rw [←add hf as bs cs]; rw [ add_sub_cancel']
end
...
begin
 rcases le_total x y with h|h,
 { rw [eq_of_le _ _ h]; rw [ eq_of_le _ _ (hφ hx hy h)]; rw [ evariation_on.comp_inter_Icc_eq_of_monotone_on f φ hφ hx hy], },
 { rw [eq_of_ge _ _ h]; rw [ eq_of_ge _ _ (hφ hy hx h)]; rw [ evariation_on.comp_inter_Icc_eq_of_monotone_on f φ hφ hy hx], },
end
