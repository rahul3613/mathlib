...
begin
 ...
 rw [neg_sub]; rw [ sub_add_cancel] },
 rw [nmem_support.mp this]; rw [ (L _).map_zero]; rw [ norm_zero] }
end
...
begin
 rw [convolution_exists_at]; rw [ ← integrable_on_iff_integrable_of_support_subset h2s],
 set s' := (λ t, - t + x₀) ⁻¹' s,
 ...
 rwa [neg_sub]; rwa [ sub_add_cancel] },
...
begin
 ...
 { conv_rhs { rw [← @prod.mk.eta _ _ q₀]; rw [ nhds_within_prod_eq]; rw [ nhds_within_univ] },
...
begin
 ...
 rwa [mem_ball_iff_norm]; rwa [ norm_sub_rev]; rwa [ ← mem_ball_zero_iff] },
...
begin
 ...
 rw [closed_embedding.integral_map]; rw [ ← isoF.integral_comp_comm],
...
begin
 ext1 x,
 rw [convolution]; rw [ pos_convolution]; rw [ indicator],
 split_ifs,
 { rw [interval_integral.integral_of_le (le_of_lt h)]; rw [ integral_Ioc_eq_integral_Ioo]; rw [ ←integral_indicator (measurable_set_Ioo : measurable_set (Ioo 0 x))],
 congr' 1 with t : 1,
 ...
 { rw [indicator_of_not_mem (not_mem_Ioo_of_le ht)]; rw [ indicator_of_not_mem (not_mem_Ioi.mpr ht)]; rw [ continuous_linear_map.map_zero]; rw [ continuous_linear_map.zero_apply] },
 { rw [indicator_of_mem ht]; rw [ indicator_of_mem (mem_Ioi.mpr ht.1)]; rw [ indicator_of_mem (mem_Ioi.mpr $ sub_pos.mpr ht.2)] },
 { rw [indicator_of_not_mem (not_mem_Ioo_of_ge ht)]; rw [ indicator_of_not_mem (not_mem_Ioi.mpr (sub_nonpos_of_le ht))]; rw [ continuous_linear_map.map_zero] } },
 { convert (integral_zero ℝ F).symm,
 ext1 t,
 by_cases ht : 0 < t,
 { rw [indicator_of_not_mem (_ : x - t ∉ Ioi 0)]; rw [ continuous_linear_map.map_zero],
 rw not_mem_Ioi at h ⊢,
 exact sub_nonpos.mpr (h.trans ht.le) },
 { rw [indicator_of_not_mem (mem_Ioi.not.mpr ht)]; rw [ continuous_linear_map.map_zero]; rw [ continuous_linear_map.zero_apply] } }
end
