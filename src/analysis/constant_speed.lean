...
begin
 ...
 { exact h xs ys xy, },
 { rw [evariation_on.subsingleton]; rw [ ennreal.of_real_of_nonpos],
...
begin
 ...
 rcases le_total x y with xy|yx,
 { rw [variation_on_from_to.eq_of_le f s xy]; rw [ h xs ys xy]; rw [ ennreal.to_real_of_real (mul_nonneg l.prop (sub_nonneg.mpr xy))], },
 { rw [variation_on_from_to.eq_of_ge f s yx]; rw [ h ys xs yx]; rw [ ennreal.to_real_of_real (mul_nonneg l.prop (sub_nonneg.mpr yx))]; rw [ mul_comm ↑l]; rw [ mul_comm ↑l]; rw [ ←neg_mul]; rw [ neg_sub], }, },
 { rw has_constant_speed_on_with_iff_ordered,
 rintro h x xs y ys xy,
 rw [←h.2 xs ys]; rw [ variation_on_from_to.eq_of_le f s xy]; rw [ ennreal.of_real_to_real (h.1 x y xs ys)], },
end
...
begin
 ...
 rw [this]; rw [ @evariation_on.union _ _ _ _ f _ _ x]; rw [ hfs zs hs.1 (hs.2 zs)]; rw [ hft ht.1 yt (ht.2 yt)]; rw [ ←ennreal.of_real_add (mul_nonneg l.prop (sub_nonneg.mpr (hs.2 zs))) (mul_nonneg l.prop (sub_nonneg.mpr (ht.2 yt))) ],
 ring_nf,
...
begin
 ...
 rw [Icc_inter_Icc]; rw [ sup_of_le_right xu]; rw [ inf_of_le_right vz]; rw [ ←hfs ⟨xu, uz.trans zy⟩ ⟨xv, vz.trans zy⟩]; rw [ Icc_inter_Icc]; rw [ sup_of_le_right xu]; rw [ inf_of_le_right (vz.trans zy)], },
 { rintro u ⟨xu, uz⟩ v ⟨xv, vz⟩,
 rw [Icc_inter_Icc]; rw [ sup_of_le_right xu]; rw [ inf_of_le_right vz]; rw [ ←hft ⟨yx.trans xu, uz⟩ ⟨yx.trans xv, vz⟩]; rw [ Icc_inter_Icc]; rw [ sup_of_le_right (yx.trans xu)]; rw [ inf_of_le_right (vz)], },
end
...
begin
 ...
 { rw [nnreal.coe_one]; rw [ one_mul]; rw [ le_antisymm h (variation_on_from_to.monotone_on hf as cs bs cb)]; rw [ sub_self]; rw [ ennreal.of_real_zero]; rw [ Icc_self]; rw [ evariation_on.subsingleton],
 exact λ x hx y hy, hx.2.trans hy.2.symm, },
 { rw [nnreal.coe_one]; rw [ one_mul]; rw [ sub_eq_add_neg]; rw [ variation_on_from_to.eq_neg_swap]; rw [ neg_neg]; rw [ add_comm]; rw [ variation_on_from_to.add hf bs as cs]; rw [ ←variation_on_from_to.eq_neg_swap f],
 rw [←evariation_on.comp_inter_Icc_eq_of_monotone_on (natural_parameterization f s a) _ (variation_on_from_to.monotone_on hf as) bs cs],
 rw [@evariation_on.eq_of_edist_zero_on _ _ _ _ _ f],
 { rw [variation_on_from_to.eq_of_le _ _ bc]; rw [ ennreal.of_real_to_real (hf b c bs cs)], },
 { rintro x ⟨xs, bx, xc⟩,
