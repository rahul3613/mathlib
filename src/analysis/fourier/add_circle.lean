...
begin
 rw [fourier_apply]; rw [ ←quotient_add_group.coe_zsmul]; rw [ to_circle]; rw [ function.periodic.lift_coe]; rw [ exp_map_circle_apply]; rw [ complex.of_real_mul]; rw [ complex.of_real_div]; rw [ complex.of_real_mul]; rw [ zsmul_eq_mul]; rw [ complex.of_real_mul]; rw [ complex.of_real_int_cast]; rw [ complex.of_real_bit0]; rw [ complex.of_real_one],
 congr' 1, ring,
end
...
by rw [←quotient_add_group.coe_zero]; rw [ fourier_coe_apply]; rw [ complex.of_real_zero]; rw [ mul_zero]; rw [ zero_div]; rw [ complex.exp_zero]

@[simp] lemma fourier_one {x : add_circle T} : fourier 1 x = to_circle x :=
by rw [fourier_apply]; rw [ one_zsmul]

@[simp] lemma fourier_neg {n : ℤ} {x : add_circle T} : fourier (-n) x = conj (fourier n x) :=
begin
 induction x using quotient_add_group.induction_on',
 simp_rw [fourier_apply, to_circle, ←quotient_add_group.coe_zsmul, function.periodic.lift_coe, ←coe_inv_circle_eq_conj, ←exp_map_circle_neg, neg_smul, mul_neg],
end
...
begin
 rw [fourier_apply]; rw [ zsmul_add]; rw [ ←quotient_add_group.coe_zsmul]; rw [ to_circle_add]; rw [ coe_mul_unit_sphere],
 have : (n : ℂ) ≠ 0 := by simpa using hn,
 have : ((@to_circle T ((n • (T / 2 / n)) : ℝ)) : ℂ) = -1,
 { rw [zsmul_eq_mul]; rw [ to_circle]; rw [ function.periodic.lift_coe]; rw [ exp_map_circle_apply],
...
begin
 ...
 rw [this]; rw [ integral_const]; rw [ measure_univ]; rw [ ennreal.one_to_real]; rw [ complex.real_smul]; rw [ complex.of_real_one]; rw [ mul_one] },
 have hij : -i + j ≠ 0,
 { rw add_comm,
...
begin
 ...
 { intro x, refl, },
 simp_rw this,
 rw [fourier_coeff]; rw [ add_circle.interval_integral_preimage T a]; rw [ volume_eq_smul_haar_add_circle]; rw [ integral_smul_measure]; rw [ ennreal.to_real_of_real hT.out.le]; rw [ ←smul_assoc]; rw [ smul_eq_mul]; rw [ one_div_mul_cancel hT.out.ne']; rw [ one_smul],
end
...
begin
 rw [fourier_coeff_on]; rw [ fourier_coeff_eq_interval_integral _ _ a],
 congr' 1,
 rw [add_sub]; rw [ add_sub_cancel'],
 ...
 rwa [add_sub]; rwa [ add_sub_cancel'],
end
...
begin
 rw [fourier_coeff_on_eq_integral]; rw [ fourier_coeff_eq_interval_integral]; rw [ add_sub_cancel' a T],
 congr' 1,
...
lemma fourier_coeff_lift_Ico_eq {a : ℝ} (f : ℝ → ℂ) (n : ℤ) :
 ...
 fourier_coeff_on (lt_add_of_pos_right a hT.out) f n :=
begin
 rw [fourier_coeff_on_eq_integral]; rw [ fourier_coeff_eq_interval_integral _ _ a]; rw [ add_sub_cancel' a T],
 congr' 1,
...
begin
 ...
 congr' 1,
 { conv_lhs {rw [mul_comm]; rw [ mul_div]; rw [ mul_one]},
