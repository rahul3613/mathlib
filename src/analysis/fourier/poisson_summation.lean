...
begin
 ...
 by simp_rw [fourier_coeff_eq_interval_integral _ m 0, div_one, one_smul, zero_add, comp_apply, coe_mk, periodic.lift_coe, zsmul_one, smul_eq_mul]
 ...
 congr' 1 with x : 1,
 rw [smul_eq_mul]; rw [ comp_apply]; rw [ coe_mk]; rw [ fourier_coe_apply],
 congr' 2,
 push_cast,
 ring
 end
...
begin
 ...
 { congr' 1 with n : 1,
 rw [←real.fourier_coeff_tsum_comp_add h_norm n]; rw [ fourier_eval_zero]; rw [ smul_eq_mul]; rw [ mul_one],
 refl },
end
...
begin
 ...
 { rw [←sub_lt_iff_lt_add]; rw [ zero_sub],
 refine lt_trans _ hx.2,
 rwa [neg_mul]; rwa [ neg_lt_neg_iff]; rwa [ two_mul]; rwa [ add_lt_iff_neg_left] } },
 ...
 rw max_lt_iff at hx',
 rw [norm_norm]; rw [ continuous_map.norm_le _ (mul_nonneg (mul_nonneg hc.le $ rpow_nonneg_of_nonneg one_half_pos.le _) (norm_nonneg _))],
 refine λ y, (hd y.1 (by linarith [hx.1, y.2.1])).trans _,
 have A : ∀ (x : ℝ), 0 ≤ |x| ^ (-b), from λ x, by positivity,
 rwa [mul_assoc]; rwa [ mul_le_mul_left hc]; rwa [ norm_of_nonneg (A _)]; rwa [ norm_of_nonneg (A _)],
...
begin
 ...
 rw [norm_norm]; rw [ function.comp_app]; rw [ norm_norm]; rw [ continuous_map.norm_le _ (norm_nonneg _)],
 rintro ⟨x, hx⟩,
 rw [continuous_map.restrict_apply_mk],
 refine (le_of_eq _).trans (continuous_map.norm_coe_le_norm _ ⟨-x, _⟩),
 { exact ⟨by linarith [hx.2], by linarith [hx.1]⟩ },
 { rw [continuous_map.restrict_apply_mk]; rw [ continuous_map.comp_apply]; rw [ continuous_map.coe_mk]; rw [ neg_neg] }
end
...
begin
 ...
 rw [closed_ball_eq_Icc] at hr; rw [ zero_add] at hr; rw [ zero_sub] at hr,
 ...
 simp_rw [continuous_map.restrict_apply, continuous_map.comp_apply, continuous_map.coe_add_right, subtype.coe_mk] },
