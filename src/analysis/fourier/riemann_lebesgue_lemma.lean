...
begin
 have hiw : ⟪i w, w⟫ = 1 / 2,
 { rw [inner_smul_left]; rw [ inner_self_eq_norm_sq_to_K]; rw [ is_R_or_C.coe_real_eq_id]; rw [ id.def]; rw [ is_R_or_C.conj_to_real]; rw [ ←div_div]; rw [ div_mul_cancel],
 rwa [ne.def]; rwa [ sq_eq_zero_iff]; rwa [ norm_eq_zero] },
 ...
 rw [this]; rw [ of_real_neg]; rw [ neg_mul]; rw [ exp_neg]; rw [ exp_pi_mul_I]; rw [ inv_neg]; rw [ inv_one]; rw [ mul_neg_one]; rw [ neg_neg]},
 rw [this]; rw [ integral_add_right_eq_self],
...
begin
 simp_rw [smul_sub],
 rw [integral_sub]; rw [ fourier_integral_half_period_translate hw]; rw [ sub_eq_add_neg]; rw [ neg_neg]; rw [ ←two_smul ℂ _]; rw [ ←@smul_assoc _ _ _ _ _ _ (is_scalar_tower.left ℂ)]; rw [ smul_eq_mul],
 norm_num,
...
begin
 ...
 { simp_rw [←comap_dist_left_at_top_eq_cocompact (0 : V), eventually_comap, eventually_at_top, dist_eq_norm', sub_zero],
 ...
 rw [ennreal.coe_add]; rw [ ennreal.coe_one]; rw [ ennreal.coe_to_nnreal hc.ne],
 exact le_self_add },
 ...
 rw [fourier_integral_eq_half_sub_half_period_translate hw_ne (hf1.integrable_of_has_compact_support hf2)]; rw [ norm_smul]; rw [ norm_eq_abs]; rw [ ←complex.of_real_one]; rw [ ←of_real_bit0]; rw [ ←of_real_div]; rw [ complex.abs_of_nonneg one_half_pos.le],
 ...
 rw [sub_add_cancel']; rw [ norm_neg]; rw [ hw'_nm]; rw [ ←div_div]; rw [ div_lt_iff (norm_pos_iff.mpr hw_ne)]; rw [ ←div_lt_iff' hδ1]; rw [ div_div],
 ...
 rw [(by refl : (↑B : ennreal).to_real = ↑B)]; rw [ two_mul],
...
begin
 ...
 simp_rw [←integral_sub ((fourier_integrand_integrable w).mp hfi) ((fourier_integrand_integrable w).mp (hg_cont.integrable_of_has_compact_support hg_supp)), ←smul_sub, ←pi.sub_apply],
...
begin
 ...
 { ext1 w,
 congr' 1 with v : 1,
 rw [←inner_conj_symm]; rw [ is_R_or_C.conj_to_real]; rw [ inner_product_space.to_dual_symm_apply]; rw [ real.fourier_char_apply], },
 rw this,
...
begin
 ...
 ext1 w,
 rw [function.comp_app]; rw [ integral_map_equiv],
 congr' 1 with v : 1,
 congr;
