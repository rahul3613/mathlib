...
⟨⟨0, 0, 0, ⟨0, le_rfl, le_rfl⟩, 0, 0, 0, 0,
 { ht₀ := by { rw [subtype.coe_mk]; rw [ Icc_self], exact mem_singleton _ },
 hR := by refl,
...
begin
 ...
 { rw [abs_of_nonpos (sub_nonpos.2 $ subtype.coe_le_coe.2 ht)]; rw [ neg_sub],
...
begin
 ...
 rw [range_to_continuous_map]; rw [ set_of_and],
...
{ to_fun := λ t, v.x₀ + ∫ τ : ℝ in v.t₀..t, f.v_comp τ,
 map_t₀' := by rw [integral_same]; rw [ add_zero],
 lipschitz' := lipschitz_with.of_dist_le_mul $ λ t₁ t₂,
 begin
 rw [dist_add_left]; rw [ dist_eq_norm]; rw [ integral_interval_sub_left (f.interval_integrable_v_comp _ _) (f.interval_integrable_v_comp _ _)],
...
begin
 ...
 simp_rw [mul_pow, div_eq_mul_inv, mul_assoc, measure_theory.integral_mul_left, measure_theory.integral_mul_right, integral_pow_abs_sub_uIoc, div_eq_mul_inv, pow_succ (v.L : ℝ), nat.factorial_succ, nat.cast_mul, nat.cast_succ, mul_inv, mul_assoc]
end
...
begin
 ...
 { rw [pow_zero]; rw [ nat.factorial_zero]; rw [ nat.cast_one]; rw [ div_one]; rw [ one_mul],
 exact dist_apply_le_dist f₁ f₂ t },
 { rw [iterate_succ_apply']; rw [ iterate_succ_apply'],
