...
begin
 ...
 rw [interior_closed_ball (0 : E) one_ne_zero]; rw [ closed_ball_diff_ball]; rw [ mem_sphere_zero_iff_norm]
 at hx hy,
 rcases h x y hx hy hne with ⟨a, b, hab, hlt⟩,
 use b,
 rwa [affine_map.line_map_apply_module]; rwa [ interior_closed_ball (0 : E) one_ne_zero]; rwa [ mem_ball_zero_iff]; rwa [ sub_eq_iff_eq_add.2 hab.symm]
end
...
begin
 ...
 { rw [closed_ball_zero] at hx hy; rw [ mem_singleton_iff] at hx hy,
...
begin
 ...
 (by rw [← add_div]; rw [ div_self hxy.ne']),
 rwa [mem_ball_zero_iff] at this; rwa [ div_eq_inv_mul] at this; rwa [ div_eq_inv_mul] at this; rwa [ mul_smul] at this; rwa [ mul_smul] at this; rwa [ smul_inv_smul₀ hx.ne'] at this; rwa [ smul_inv_smul₀ hy.ne'] at this; rwa [ ← smul_add] at this; rwa [ norm_smul] at this; rwa [ real.norm_of_nonneg (inv_pos.2 hxy).le] at this; rwa [ ← div_eq_inv_mul] at this; rwa [ div_lt_one hxy] at this
end
...
by rw [norm_smul]; rw [ real.norm_of_nonneg (one_div_nonneg.2 zero_le_two)]; rw [ ←inv_eq_one_div]; rw [ ←div_eq_inv_mul]; rw [ div_lt_iff (zero_lt_two' ℝ)]; rw [ mul_two]; rw [ ←not_same_ray_iff_of_norm_eq h]; rw [ not_same_ray_iff_norm_add_lt]; rw [ h]
...
begin
 ...
 { rw [← dist_add_dist_eq_iff]; rw [ dist_zero_left]; rw [ dist_vsub_cancel_right]; rw [ ← dist_eq_norm_vsub']; rw [ ← dist_eq_norm_vsub']; rw [ hxy]; rw [ hyz]; rw [ ← add_mul]; rw [ add_sub_cancel'_right]; rw [ one_mul] },
 ...
 rw [affine_map.line_map_apply]; rw [ ← H']; rw [ H]; rw [ vsub_vadd] },
end
...
begin
 ...
 { rwa [inv_of_eq_inv]; rwa [ ← div_eq_inv_mul] },
 { rwa [inv_of_eq_inv]; rwa [ ← one_div]; rwa [ sub_half]; rwa [ one_div]; rwa [ ← div_eq_inv_mul] }
end
...
{ norm_map := λ x, by simp [affine_map.of_map_midpoint, ←dist_eq_norm_vsub E, hi.dist_eq],
 ...
 { rw [hi.dist_eq]; rw [ hi.dist_eq]; rw [ dist_left_midpoint]; rw [ real.norm_of_nonneg zero_le_two]; rw [ div_eq_inv_mul] },
 { rw [hi.dist_eq]; rw [ hi.dist_eq]; rw [ dist_midpoint_right]; rw [ real.norm_of_nonneg zero_le_two]; rw [ div_eq_inv_mul] },
 end) hi.continuous }
