...
assert_not_exists normed_space
...
{ carrier := f ⁻¹' S,
 smul_mem' := λ c hc x hx, by { rw [mem_preimage]; rw [ f.map_smul c], exact S.smul_mem hc hx },
 add_mem' := λ x hx y hy, by { rw [mem_preimage]; rw [ f.map_add], exact S.add_mem hx hy } }
...
by rw [pointed_iff_not_blunt]; rw [ not_not]
...
lemma pointed_zero : (0 : convex_cone 𝕜 E).pointed := by rw [pointed]; rw [ mem_zero]
...
begin
 ...
 exact ⟨c⁻¹, inv_pos.2 hc, by rwa [smul_smul]; rwa [ inv_mul_cancel hc.ne']; rwa [ one_smul]⟩ },
 { rintros ⟨c, hc, hcx⟩,
 exact ⟨c⁻¹, inv_pos.2 hc, _, hcx, by rw [smul_smul]; rw [ inv_mul_cancel hc.ne']; rw [ one_smul]⟩ }
end
...
begin
 ...
 rw [← neg_neg x] at hx; rw [ add_subgroup_class.coe_neg] at hx; rw [ ← sub_eq_add_neg] at hx,
 exact ⟨_, hx⟩ },
 rintros a ⟨xn, hxn, rfl⟩ b ⟨xp, hxp, rfl⟩,
 have := s.add_mem hxp hxn,
 rw [add_assoc] at this; rw [ add_sub_cancel'_right] at this; rw [ ← sub_eq_add_neg] at this; rw [ ← add_subgroup_class.coe_sub] at this,
 replace := nonneg _ this,
 rwa [f.map_sub] at this ; rwa [ sub_nonneg] at this },
 ...
 rw [linear_pmap.domain_sup_span_singleton] at H; rw [ sup_le_iff] at H; rw [ span_le] at H; rw [ singleton_subset_iff] at H,
 exact hy H.2 },
 { rintros ⟨z, hz⟩ hzs,
 rcases mem_sup.1 hz with ⟨x, hx, y', hy', rfl⟩,
 rcases mem_span_singleton.1 hy' with ⟨r, rfl⟩,
 simp only [subtype.coe_mk] at hzs,
 erw [linear_pmap.sup_span_singleton_apply_mk _ _ _ _ _ hx]; erw [ smul_neg]; erw [ ← sub_eq_add_neg]; erw [ sub_nonneg],
 rcases lt_trichotomy r 0 with hr|hr|hr,
 { have : -(r⁻¹ • x) - y ∈ s,
 by rwa [← s.smul_mem_iff (neg_pos.2 hr)]; rwa [ smul_sub]; rwa [ smul_neg]; rwa [ neg_smul]; rwa [ neg_neg]; rwa [ smul_smul]; rwa [ mul_inv_cancel hr.ne]; rwa [ one_smul]; rwa [ sub_eq_add_neg]; rwa [ neg_smul]; rwa [ neg_neg],
 replace := le_c (r⁻¹ • ⟨x, hx⟩) this,
 rwa [← mul_le_mul_left (neg_pos.2 hr)] at this ; rwa [ neg_mul] at this ; rwa [ neg_mul] at this ; rwa [ neg_le_neg_iff] at this ; rwa [ f.map_smul] at this ; rwa [ smul_eq_mul] at this ; rwa [ ← mul_assoc] at this ; rwa [ mul_inv_cancel hr.ne] at this ; rwa [ one_mul] at this },
 { subst r,
 simp only [zero_smul, add_zero] at hzs ⊢,
 apply nonneg,
 exact hzs },
 { have : r⁻¹ • x + y ∈ s,
 by rwa [← s.smul_mem_iff hr]; rwa [ smul_add]; rwa [ smul_smul]; rwa [ mul_inv_cancel hr.ne']; rwa [ one_smul],
 replace := c_le (r⁻¹ • ⟨x, hx⟩) this,
 rwa [← mul_le_mul_left hr] at this ; rwa [ f.map_smul] at this ; rwa [ smul_eq_mul] at this ; rwa [ ← mul_assoc] at this ; rwa [ mul_inv_cancel hr.ne'] at this ; rwa [ one_mul] at this } }
end
...
begin
 ...
 rw [A] at B; rw [ map_add] at B; rw [ ← neg_le_iff_add_nonneg'] at B,
 have C := g_eq 0 (N x),
