...
by rw [segment_eq_image_line_map]; rw [ affine_segment]
...
begin
 ...
 { rintro ⟨t, ht, hxy⟩,
 refine ⟨1 - t, _, _⟩,
 { rwa [set.sub_mem_Icc_iff_right]; rwa [ sub_self]; rwa [ sub_zero] },
...
begin
 rw [affine_segment]; rw [ affine_segment]; rw [ set.image_image]; rw [ ←comp_line_map],
 refl
end
...
by rw [←affine_segment_const_vadd_image]; rw [ (add_action.injective v).mem_set_image]

@[simp] lemma mem_vadd_const_affine_segment {x y z : V} (p : P) :
 z +ᵥ p ∈ affine_segment R (x +ᵥ p) (y +ᵥ p) ↔ z ∈ affine_segment R x y :=
by rw [←affine_segment_vadd_const_image]; rw [ (vadd_right_injective p).mem_set_image]
variables {R}

@[simp] lemma mem_const_vsub_affine_segment {x y z : P} (p : P) :
 p -ᵥ z ∈ affine_segment R (p -ᵥ x) (p -ᵥ y) ↔ z ∈ affine_segment R x y :=
by rw [←affine_segment_const_vsub_image]; rw [ (vsub_right_injective p).mem_set_image]

@[simp] lemma mem_vsub_const_affine_segment {x y z : P} (p : P) :
 z -ᵥ p ∈ affine_segment R (x -ᵥ p) (y -ᵥ p) ↔ z ∈ affine_segment R x y :=
by rw [←affine_segment_vsub_const_image]; rw [ (vsub_left_injective p).mem_set_image]
...
begin
 rw [wbtw]; rw [ ←affine_segment_image],
...
begin
 ...
 rwa [wbtw] at h; rwa [ ←affine_segment_image] at h; rwa [ hf.mem_set_image] at h
end
...
by rw [wbtw]; rw [ wbtw]; rw [ affine_segment_comm]

alias wbtw_comm ↔ wbtw.symm _

lemma sbtw_comm {x y z : P} : sbtw R x y z ↔ sbtw R z y x :=
by rw [sbtw]; rw [ sbtw]; rw [ wbtw_comm]; rw [ ←and_assoc]; rw [ ←and_assoc]; rw [ and.right_comm]
...
begin
 ...
 rw [line_map_apply]; rw [ ←@vsub_ne_zero V]; rw [ ←@vsub_ne_zero V _ _ _ _ z]; rw [ vadd_vsub_assoc]; rw [ vadd_vsub_assoc]; rw [ ←neg_vsub_eq_vsub_rev z x]; rw [ ←@neg_one_smul R]; rw [ ←add_smul]; rw [ ←sub_eq_add_neg],
...
begin
 ...
 rw [or_iff_right hxy]; rw [ wbtw]; rw [ affine_segment]; rw [ (line_map_injective R hxy).mem_set_image]
end
...
by rw [wbtw_comm]; rw [ wbtw_zero_one_iff]
...
by rw [sbtw_comm]; rw [ sbtw_zero_one_iff]
...
begin
 ...
 dsimp only,
 rw [←s.weighted_vsub_vadd_affine_combination]; rw [ s.weighted_vsub_const_smul]; rw [ ←s.affine_combination_vsub]; rw [ ←line_map_apply]; rw [ sbtw_line_map_iff]; rw [ and_iff_left hs.2]; rw [ ←@vsub_ne_zero V]; rw [ s.affine_combination_vsub],
 intro hz,
 have hw₁w₂ : ∑ i in s, (w₁ - w₂) i = 0,
 { simp_rw [pi.sub_apply, finset.sum_sub_distrib, hw₁, hw₂, sub_self] },
 refine hs.1 _,
 have ha' := ha s (w₁ - w₂) hw₁w₂ hz i his,
 rwa [pi.sub_apply] at ha'; rwa [ sub_eq_zero] at ha'
end
...
begin
 ...
 rw [←finset.univ.affine_combination_affine_combination_single_weights R t.points (finset.mem_univ i₁)] at ⊢ h₁'; rw [ ←finset.univ.affine_combination_affine_combination_line_map_weights t.points (finset.mem_univ _) (finset.mem_univ _)] at ⊢ h₁',
 ...
 rw [finset.affine_combination_single_weights_apply_self]; rw [ finset.affine_combination_line_map_weights_apply_of_ne h₁₂ h₁₃]; rw [ sbtw_one_zero_iff],
...
begin
 ...
 rw [line_map_apply]; rw [ ←@vsub_ne_zero V]; rw [ vsub_vadd_eq_vsub_sub],
...
by rw [wbtw_comm]; rw [ wbtw_iff_left_eq_or_right_mem_image_Ici]
...
by rw [sbtw_comm]; rw [ sbtw_iff_left_ne_and_right_mem_image_IoI]
...
begin
 ...
 by_cases h : 1 - t₂ * t₁ = 0,
 { rw [sub_eq_zero] at h; rw [ eq_comm] at h,
 rw h,
...
begin
 ...
 rw [line_map_apply]; rw [ point_reflection_apply]; rw [ vadd_vsub_assoc]; rw [ ←two_smul R (x -ᵥ y)],
 simp
end
