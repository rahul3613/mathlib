...
begin
 ...
 rwa [←sub_smul]; rwa [ norm_smul_of_nonneg (sub_nonneg_of_le $ one_le_inv (hδ'.trans_le hδz) hz)]; rwa [ sub_mul]; rwa [ inv_mul_cancel (hδ'.trans_le hδz).ne']; rwa [ one_mul]; rwa [ sub_le_comm] },
 set x' := ‖x‖⁻¹ • x,
 set y' := ‖y‖⁻¹ • y,
 have hxy' : ε/3 ≤ ‖x' - y'‖ :=
...
begin
 ...
 try { apply_instance },
 have := h hx hy,
 simp_rw [←smul_add, ←smul_sub, norm_smul_of_nonneg (inv_nonneg.2 hr.le), ←div_eq_inv_mul, div_le_div_right hr, div_le_iff hr, sub_mul] at this,
 exact this hxy,
end
