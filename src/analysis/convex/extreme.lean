...
begin
 ...
 { rintro x₁ hx₁ x₂ hx₂ hi,
 refine (h (update x i x₁) _ (update x i x₂) _ _).imp (λ h₁, by rw [←h₁]; rw [ update_same])
 (λ h₂, by rw [←h₂]; rw [ update_same]),
 iterate 2
 { rintro j,
...
by rw [hA.mem_extreme_points_iff_convex_diff]; rw [ hA.convex_remove_iff_not_mem_convex_hull_remove]; rw [ mem_diff]
