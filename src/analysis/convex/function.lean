...
⟨hf.1, λ x hx y hy a b ha hb hab,
 ...
 ... = a • (f x + g x) + b • (f y + g y) : by rw [smul_add]; rw [ smul_add]; rw [ add_add_add_comm]⟩
...
⟨hf.1.translate_preimage_right _, λ x hx y hy a b ha hb hab,
 ...
 : by rw [smul_add]; rw [ smul_add]; rw [ add_add_add_comm]; rw [ convex.combo_self hab]
...
begin
 ...
 { rw [convex.combo_self hab]; rw [ convex.combo_self hab] },
...
⟨hs, λ _ _ _ _ _ _ _ _ _, by rw [f.map_add]; rw [ f.map_smul]; rw [ f.map_smul]⟩
...
begin
 ...
 wlog h : x < y,
 { rw [add_comm (a • x)]; rw [ add_comm (a • f x)], rw add_comm at hab,
...
⟨hf.1.linear_preimage _, λ x hx y hy a b ha hb hab,
 calc
 f (g (a • x + b • y)) = f (a • (g x) + b • (g y)) : by rw [g.map_add]; rw [ g.map_smul]; rw [ g.map_smul]
...
by rw [← neg_convex_on_iff]; rw [ neg_neg f]
...
by rw [← neg_strict_convex_on_iff]; rw [ neg_neg f]
...
⟨hf.1, λ x hx y hy a b ha hb hab,
 ...
 ... = a • (c • f x) + b • (c • f y)
 : by rw [smul_add]; rw [ smul_comm c]; rw [ smul_comm c]; apply_instance⟩
