...
begin
 ...
 by conv_rhs { rw [← insert_erase hi₀]; rw [ sum_insert (not_mem_erase i₀ t)]; rw [ hk]; rw [ zero_add] }
 ... = ∑ e in t, (f e - f i₀ / g i₀ * g e) : rfl
 ... = 1 : by rw [sum_sub_distrib]; rw [ fsum]; rw [ ← mul_sum]; rw [ gsum]; rw [ mul_zero]; rw [ sub_zero] },
 ...
 calc ∑ e in t.erase i₀, k e • e = ∑ e in t, k e • e : sum_erase _ (by rw [hk]; rw [ zero_smul])
...
begin
 rw [← finset.coe_nonempty]; rw [ ← @convex_hull_nonempty_iff 𝕜],
...
begin
 ...
 rw [← not_lt] at contra,
 apply contra,
 erw [card_erase_of_mem p.2]; erw [ hk],
 exact lt_add_one _,
end
...
begin
 ...
 { erw [finset.sum_attach]; erw [ finset.sum_filter_ne_zero]; erw [ hw₂] },
 { change ∑ (i : t') in t'.attach, (λ e, w e • e) ↑i = x,
 erw [finset.sum_attach]; erw [ finset.sum_filter_of_ne],
 { rw t.center_mass_eq_of_sum_1 id hw₂ at hw₃, exact hw₃ },
 { intros e he hwe contra, apply hwe, rw [contra]; rw [ zero_smul] } }
end
