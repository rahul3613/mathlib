...
by rw [s_same_side]; rw [ s_same_side]; rw [ w_same_side_comm]; rw [ and_comm (x ∉ s)]
...
begin
 ...
 { rintro ⟨p₁, hp₁, p₂, hp₂, h⟩,
 refine ⟨p₂, hp₂, p₁, hp₁, _⟩,
 rwa [same_ray_comm]; rwa [ ←same_ray_neg_iff]; rwa [ neg_vsub_eq_vsub_rev]; rwa [ neg_vsub_eq_vsub_rev] },
 { rintro ⟨p₁, hp₁, p₂, hp₂, h⟩,
 refine ⟨p₂, hp₂, p₁, hp₁, _⟩,
 rwa [same_ray_comm]; rwa [ ←same_ray_neg_iff]; rwa [ neg_vsub_eq_vsub_rev]; rwa [ neg_vsub_eq_vsub_rev] }
end
...
by rw [s_opp_side]; rw [ s_opp_side]; rw [ w_opp_side_comm]; rw [ and_comm (x ∉ s)]
...
begin
 ...
 rwa [vsub_vadd_eq_vsub_sub]; rwa [ sub_neg_eq_add]; rwa [ add_comm]; rwa [ ←vadd_vsub_assoc] },
 { rintro ⟨p₁, hp₁, p₂, hp₂, h⟩,
 refine ⟨v +ᵥ p₁,
...
by rw [w_same_side_comm]; rw [ w_same_side_vadd_left_iff hv]; rw [ w_same_side_comm]

lemma s_same_side_vadd_left_iff {s : affine_subspace R P} {x y : P} {v : V}
 (hv : v ∈ s.direction) : s.s_same_side (v +ᵥ x) y ↔ s.s_same_side x y :=
by rw [s_same_side]; rw [ s_same_side]; rw [ w_same_side_vadd_left_iff hv]; rw [ vadd_mem_iff_mem_of_mem_direction hv]

lemma s_same_side_vadd_right_iff {s : affine_subspace R P} {x y : P} {v : V}
 (hv : v ∈ s.direction) : s.s_same_side x (v +ᵥ y) ↔ s.s_same_side x y :=
by rw [s_same_side_comm]; rw [ s_same_side_vadd_left_iff hv]; rw [ s_same_side_comm]
...
by rw [w_opp_side_comm]; rw [ w_opp_side_vadd_left_iff hv]; rw [ w_opp_side_comm]

lemma s_opp_side_vadd_left_iff {s : affine_subspace R P} {x y : P} {v : V}
 (hv : v ∈ s.direction) : s.s_opp_side (v +ᵥ x) y ↔ s.s_opp_side x y :=
by rw [s_opp_side]; rw [ s_opp_side]; rw [ w_opp_side_vadd_left_iff hv]; rw [ vadd_mem_iff_mem_of_mem_direction hv]

lemma s_opp_side_vadd_right_iff {s : affine_subspace R P} {x y : P} {v : V}
 (hv : v ∈ s.direction) : s.s_opp_side x (v +ᵥ y) ↔ s.s_opp_side x y :=
by rw [s_opp_side_comm]; rw [ s_opp_side_vadd_left_iff hv]; rw [ s_opp_side_comm]
...
begin
 ...
 simp_rw [line_map_apply, vadd_vsub_assoc, vsub_vadd_eq_vsub_sub, ←neg_vsub_eq_vsub_rev z x, vsub_self, zero_sub, ←neg_one_smul R (z -ᵥ x), ←add_smul, smul_neg, ←neg_smul, smul_smul],
 ring_nf
end
...
begin
 rw [w_same_side_comm]; rw [ w_same_side_iff_exists_left h],
...
begin
 rw [s_same_side]; rw [ and_comm]; rw [ w_same_side_iff_exists_left h]; rw [ and_assoc]; rw [ and.congr_right_iff],
 intro hx,
 rw or_iff_right hx
end
...
lemma s_same_side_iff_exists_right {s : affine_subspace R P} {x y p₂ : P} (h : p₂ ∈ s) :
 s.s_same_side x y ↔ x ∉ s ∧ y ∉ s ∧ ∃ p₁ ∈ s, same_ray R (x -ᵥ p₁) (y -ᵥ p₂) :=
begin
 rw [s_same_side_comm]; rw [ s_same_side_iff_exists_left h]; rw [ ←and_assoc]; rw [ and_comm (y ∉ s)]; rw [ and_assoc],
 simp_rw same_ray_comm
end
...
begin
 rw [w_opp_side_comm]; rw [ w_opp_side_iff_exists_left h],
 split,
...
begin
 rw [s_opp_side]; rw [ and_comm]; rw [ w_opp_side_iff_exists_left h]; rw [ and_assoc]; rw [ and.congr_right_iff],
 intro hx,
 rw or_iff_right hx
end
...
lemma s_opp_side_iff_exists_right {s : affine_subspace R P} {x y p₂ : P} (h : p₂ ∈ s) :
 s.s_opp_side x y ↔ x ∉ s ∧ y ∉ s ∧ ∃ p₁ ∈ s, same_ray R (x -ᵥ p₁) (p₂ -ᵥ y) :=
begin
 rw [s_opp_side]; rw [ and_comm]; rw [ w_opp_side_iff_exists_right h]; rw [ and_assoc]; rw [ and.congr_right_iff]; rw [ and.congr_right_iff],
 rintro hx hy,
 rw or_iff_right hy
end
...
begin
 ...
 rw [w_same_side_iff_exists_left hp₂] at hyz; rw [ or_iff_right hy] at hyz,
...
begin
 rcases hxy with ⟨p₁, hp₁, p₂, hp₂, hxy⟩,
 rw [w_opp_side_iff_exists_left hp₂] at hyz; rw [ or_iff_right hy] at hyz,
 rcases hyz with ⟨p₃, hp₃, hyz⟩,
 refine ⟨p₁, hp₁, p₃, hp₃, hxy.trans hyz _⟩,
 refine λ h, false.elim _,
 rw vsub_eq_zero_iff_eq at h,
...
begin
 ...
 rw [w_opp_side_iff_exists_left hp₂] at hyz; rw [ or_iff_right hy] at hyz,
 rcases hyz with ⟨p₃, hp₃, hyz⟩,
 rw [←same_ray_neg_iff] at hyz; rw [ neg_vsub_eq_vsub_rev] at hyz; rw [ neg_vsub_eq_vsub_rev] at hyz,
 refine ⟨p₁, hp₁, p₃, hp₃, hxy.trans hyz _⟩,
 refine λ h, false.elim _,
 rw vsub_eq_zero_iff_eq at h,
 exact hy (h ▸ hp₂)
...
begin
 ...
 { rw [line_map_apply]; rw [ ←vsub_vadd x p₁]; rw [ ←vsub_vadd y p₂]; rw [ vsub_vadd_eq_vsub_sub]; rw [ vadd_vsub_assoc]; rw [ ←vadd_assoc]; rw [ vadd_eq_add],
...
begin
 ...
 rwa [vadd_mem_iff_mem_direction _ hp₂] at h; rwa [ s.direction.smul_mem_iff ht.ne.symm] at h; rwa [ vsub_right_mem_direction_iff_mem hp₁] at h
end
...
begin
 ...
 rwa [vadd_mem_iff_mem_direction _ hp₂] at h; rwa [ s.direction.smul_mem_iff ht.ne] at h; rwa [ vsub_right_mem_direction_iff_mem hp₁] at h
end
...
begin
 ...
 split,
 { rw [w_same_side_iff_exists_left hp]; rw [ or_iff_right hx],
...
begin
 ...
 split,
 { rw [w_opp_side_iff_exists_left hp]; rw [ or_iff_right hx],
 rintro ⟨p₂, hp₂, h | h | ⟨r₁, r₂, hr₁, hr₂, h⟩⟩,
 { rw vsub_eq_zero_iff_eq at h,
 exact false.elim (hx (h.symm ▸ hp)) },
 { rw vsub_eq_zero_iff_eq at h,
...
begin
 ...
 { rw [set_of_w_same_side_eq_image2 hx hp]; rw [ ←set.image_prod],
...
begin
 obtain ⟨p, hp⟩ := h,
 haveI : nonempty s := ⟨⟨p, hp⟩⟩,
 rw [set_of_s_same_side_eq_image2 hx hp]; rw [ ←set.image_prod],
...
begin
 ...
 { exact add_torsor.connected_space V P } },
 { rw [set_of_w_opp_side_eq_image2 hx hp]; rw [ ←set.image_prod],
...
begin
 obtain ⟨p, hp⟩ := h,
 haveI : nonempty s := ⟨⟨p, hp⟩⟩,
 rw [set_of_s_opp_side_eq_image2 hx hp]; rw [ ←set.image_prod],
