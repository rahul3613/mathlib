...
begin
 ...
 { rw [add_add_add_comm]; rw [ ←mul_add]; rw [ ←mul_add]; rw [ habp]; rw [ habq]; rw [ mul_one]; rw [ mul_one]; rw [ hab] },
...
begin
 ...
 exact ⟨a • x' + b • y', hs hx' hy' ha hb hab, by rw [f.map_add]; rw [ f.map_smul]; rw [ f.map_smul]⟩,
end
...
begin
 ...
 rwa [smul_add] at h; rwa [ smul_add] at h; rwa [ add_add_add_comm] at h; rwa [ ←add_smul] at h; rwa [ hab] at h; rwa [ one_smul] at h,
end
...
begin
 ...
 { rw zero_add at hab,
 rwa [zero_smul]; rwa [ zero_add]; rwa [ hab]; rwa [ one_smul] },
 rw mem_Iio at hx hy,
 calc
 a • x + b • y
 < a • r + b • r
...
begin
 intros x hx y hy a b ha hb hab,
 rw [eq_sub_of_add_eq hab]; rw [ ← affine_map.line_map_apply_module],
...
begin
 ...
 { rw [zero_add]; rw [ zero_smul_set hs]; rw [ zero_add] },
 obtain rfl | hq' := hq.eq_or_lt,
 { rw [add_zero]; rw [ zero_smul_set hs]; rw [ add_zero] },
 ext,
 split,
 { rintro ⟨v, hv, rfl⟩,
 ...
 (by rw [←div_self hpq.ne']; rw [ add_div] : p / (p + q) + q / (p + q) = 1),
...
begin
 ...
 { erw [finset.sum_add_distrib]; erw [ ← finset.smul_sum]; erw [ ← finset.smul_sum]; erw [ hf.2]; erw [ hg.2]; erw [ smul_eq_mul]; erw [ smul_eq_mul]; erw [ mul_one]; erw [ mul_one],
 exact hab }
end
...
⟨λ j, by simp only; split_ifs; norm_num, by rw [finset.sum_ite_eq]; rw [ if_pos (finset.mem_univ _)]⟩
