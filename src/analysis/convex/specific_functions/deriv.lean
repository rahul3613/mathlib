...
... = _ : by rw [finset.prod_mul_distrib]; rw [ finset.prod_ite]; rw [ finset.prod_const_one]; rw [ mul_one]; rw [ finset.prod_const]; rw [ neg_one_pow_eq_pow_mod_two]; rw [ nat.even_iff.1 h0]; rw [ pow_zero]; rw [ one_mul]
...
begin
 ...
 rw ← two_mul at ihn,
 rw [← two_mul]; rw [ nat.succ_eq_add_one]; rw [ mul_add]; rw [ mul_one]; rw [ bit0]; rw [ ← add_assoc]; rw [ finset.prod_range_succ]; rw [ finset.prod_range_succ]; rw [ mul_assoc],
...
begin
 ...
 { rw [sqrt_eq_zero_of_nonpos hx]; rw [ zero_pow zero_lt_three]; rw [ mul_zero]; rw [ div_zero],
 refine has_deriv_within_at.deriv_eq_zero _ (unique_diff_on_Iic 0 x hx),
