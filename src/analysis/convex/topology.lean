...
begin
 assume f hf,
 rw [metric.mem_closed_ball]; rw [ dist_pi_le_iff zero_le_one],
 intros x,
...
begin
 rw [segment_eq_image]; rw [ open_segment_eq_image]; rw [ ←closure_Ioo (zero_ne_one' 𝕜)],
...
begin
 ...
 rw [open_segment_eq_image_line_map]; rw [ ← inv_one]; rw [ ← inv_Ioi (zero_lt_one' ℝ)]; rw [ ← image_inv]; rw [ image_image]; rw [ homothety_eq_line_map],
