...
by rw [center_mass]; rw [ sum_singleton]; rw [ sum_singleton]; rw [ ← mul_smul]; rw [ inv_mul_cancel hw]; rw [ one_smul]
...
begin
 rw [s.center_mass_eq_of_sum_1 _ hws]; rw [ t.center_mass_eq_of_sum_1 _ hwt]; rw [ smul_sum]; rw [ smul_sum]; rw [ ← finset.sum_sum_elim]; rw [ finset.center_mass_eq_of_sum_1],
 { congr' with ⟨⟩; simp only [sum.elim_inl, sum.elim_inr, mul_smul] },
 { rw [sum_sum_elim]; rw [ ← mul_sum]; rw [ ← mul_sum]; rw [ hws]; rw [ hwt]; rw [ mul_one]; rw [ mul_one]; rw [ hab] }
end
...
begin
 rw [center_mass]; rw [ sum_subset ht h]; rw [ smul_sum]; rw [ center_mass]; rw [ smul_sum],
 apply sum_subset ht,
 assume i hit' hit,
...
begin
 rw [center_mass]; rw [ inv_smul_le_iff hw₁]; rw [ sum_smul],
...
begin
 ...
 { rwa [finsum] at h₁ ; rwa [ dif_pos hfin_w] at h₁ },
...
begin
 ...
 { rw [h_cases]; rw [ ←add_smul]; rw [ hab]; rw [ one_smul], exact hy },
...
begin
 rw [affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one _ w _ hw₂ (0 : E)]; rw [ finset.weighted_vsub_of_point_apply]; rw [ vadd_eq_add]; rw [ add_zero]; rw [ t.center_mass_eq_of_sum_1 _ hw₂],
...
begin
 ...
 have hW₁ : (s ∪ s').sum W = 1,
 { rw [sum_add_distrib]; rw [ ← sum_subset (subset_union_left s s')]; rw [ ← sum_subset (subset_union_right s s')]; rw [ sum_ite_of_true _ _ (λ i hi, hi)]; rw [ sum_ite_of_true _ _ (λ i hi, hi)]; rw [ ← mul_sum]; rw [ ← mul_sum]; rw [ hw₁]; rw [ hw₁']; rw [ ← add_mul]; rw [ hab]; rw [ mul_one];
 intros i hi hi';
 simp [hi'], },
 refine ⟨s ∪ s', W, _, hW₁, _⟩,
 { rintros i -,
 ...
 { simp_rw [affine_combination_eq_linear_combination (s ∪ s') v _ hW₁, affine_combination_eq_linear_combination s v w hw₁, affine_combination_eq_linear_combination s' v w' hw₁', add_smul, sum_add_distrib],
...
begin
 ...
 { rw [finset.sum_ite_eq]; rw [ if_pos hx] } },
...
by rw [finset.convex_hull_eq]; rw [ set.mem_set_of_eq]
...
begin
 ...
 { rw [coe_image]; rw [ set.image_subset_iff],
 exact hz },
...
begin
 ...
 ext,
 { rw [←hSp]; rw [ finset.center_mass_eq_of_sum_1 _ _ hw']; rw [ finset.center_mass_eq_of_sum_1 _ _ h_sum],
 simp_rw [prod.fst_sum, prod.smul_mk],
 rw finset.sum_product,
 congr,
 ext i,
 have : ∑ (j : κ) in b, (w i * v j) • S i = ∑ (j : κ) in b, v j • w i • S i,
 { congr, ext, rw [mul_smul]; rw [ smul_comm] },
 rw [this]; rw [ ←finset.sum_smul]; rw [ hv']; rw [ one_smul] },
 { rw [←hTp]; rw [ finset.center_mass_eq_of_sum_1 _ _ hv']; rw [ finset.center_mass_eq_of_sum_1 _ _ h_sum],
...
begin
 ...
 { rintros w ⟨hw₀, hw₁⟩,
 rw [pi_eq_sum_univ w]; rw [ ← finset.univ.center_mass_eq_of_sum_1 _ hw₁],
...
begin
 rw [← convex_hull_basis_eq_std_simplex]; rw [ ← linear_map.convex_hull_image]; rw [ ← set.range_comp]; rw [ (∘)],
 apply congr_arg,
