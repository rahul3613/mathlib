...
begin
 ...
 { rw [←add_assoc]; rw [ ←mul_add]; rw [ hab₁]; rw [ mul_one]; rw [ hab₂] },
...
begin
 ...
 { rw [finset.sum_filter_add_sum_filter_not]; rw [ hw₁] },
 { rw [finset.center_mass]; rw [ smul_inv_smul₀ hw.ne']; rw [ finset.sum_smul] }
end
...
by rw [←pair_eq_singleton]; rw [ ←convex_join_segments]; rw [ segment_same]; rw [ pair_eq_singleton]

lemma convex_join_singleton_segment (a b c : E) :
 convex_join 𝕜 {a} (segment 𝕜 b c) = convex_hull 𝕜 {a, b, c} :=
by rw [←segment_same 𝕜]; rw [ convex_join_segments]; rw [ insert_idem]
...
begin
 ...
 rw [convex_join_segments]; rw [ triv]; rw [ ←convex_join_segments],
...
begin
 rw [←convex_hull_convex_hull_union_left]; rw [ ←convex_hull_convex_hull_union_right],
