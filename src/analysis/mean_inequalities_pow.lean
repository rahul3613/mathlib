...
begin
 ...
 { rwa [← finset.sum_div] at this ; rwa [ ← finset.sum_div] at this ; rwa [ div_pow] at this ; rwa [ pow_succ' (s.card : ℝ)] at this ; rwa [ ← div_div] at this ; rwa [ div_le_iff hs0] at this ; rwa [ div_mul] at this ; rwa [ div_self hs0.ne'] at this ; rwa [ div_one] at this },
...
begin
 ...
 have h_add : a/(a+b) + b/(a+b) = 1, by rw [div_add_div_same]; rw [ div_self h_zero],
 ...
 rwa [div_eq_mul_inv] at h_mul; rwa [ div_eq_mul_inv] at h_mul; rwa [ mul_add] at h_mul; rwa [ mul_comm (a^p)] at h_mul; rwa [ mul_comm (b^p)] at h_mul; rwa [ ←mul_assoc] at h_mul; rwa [ ←mul_assoc] at h_mul; rwa [ mul_inv_cancel hab_0] at h_mul; rwa [ one_mul] at h_mul; rwa [ one_mul] at h_mul,
end
...
begin
 ...
 from λ a, by rw [←nnreal.rpow_mul]; rw [ div_eq_inv_mul]; rw [ ←mul_assoc]; rw [ _root_.mul_inv_cancel hp_pos.ne.symm]; rw [ one_mul],
...
begin
 ...
 rwa [ne.def]; rwa [ ←h_top_iff_rpow_top i hi], },
 ...
 rwa [←coe_eq_coe]; rwa [ ←h_sum_nnreal], },
end
...
begin
 have h_rpow : ∀ a : ℝ≥0∞, a^q = (a^p)^(q/p),
 from λ a, by rw [← ennreal.rpow_mul]; rw [ _root_.mul_div_cancel' _ hp_pos.ne'],
 have h_rpow_add_rpow_le_add : ((a^p)^(q/p) + (b^p)^(q/p)) ^ (1/(q/p)) ≤ a^p + b^p,
 { refine rpow_add_rpow_le_add (a^p) (b^p) _,
 rwa one_le_div hp_pos, },
