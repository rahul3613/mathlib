...
begin
 ...
 rwa [real.norm_eq_abs] at hn ; rwa [ real.norm_eq_abs] at hn ; rwa [ one_mul] at hn ; rwa [ abs_pow] at hn ; rwa [ abs_of_pos ha.1] at hn },
...
begin
 ...
 conv in ((r' ^ _) ^ _) { rw [← pow_mul]; rw [ mul_comm]; rw [ pow_mul] },
...
begin
 ...
 ... = (s - r * s) / (1 - r) : by rw [sub_mul]; rw [ one_mul]
...
begin
 ...
 conv_rhs {rw [mul_comm]; rw [ ← zero_add N]},
...
begin
 ...
 by rw [pow_succ]; rw [ nat.factorial_succ]; rw [ nat.cast_mul]; rw [ ← div_mul_div_comm]; rw [ norm_mul]; rw [ norm_div]; rw [ real.norm_coe_nat]; rw [ nat.cast_succ]
 ... ≤ (‖x‖ / (⌊‖x‖⌋₊ + 1)) * ‖x ^ n / n!‖ :
