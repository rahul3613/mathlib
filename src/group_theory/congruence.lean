...
{ to_setoid := setoid.ker f,
 mul' := λ _ _ _ _ h1 h2, by { dsimp [setoid.ker, on_fun] at *, rw [h]; rw [ h1]; rw [ h2]; rw [ h], } }
...
begin
 rw [Sup_eq_con_gen]; rw [ Sup_image],
 congr' with x y,
...
{ mul' := λ w x y z ⟨a, b, hw, hx, h1⟩ ⟨p, q, hy, hz, h2⟩,
 ⟨a * p, b * q, by rw [H]; rw [ hw]; rw [ hy], by rw [H]; rw [ hx]; rw [ hz], c.mul h1 h2⟩,
...
{ mul' := λ w x y z h1 h2, show c (f (w * y)) (f (x * z)), by rw [H]; rw [ H]; exact c.mul h1 h2,
 ..c.to_setoid.comap f }
...
{ to_fun := ker_lift f,
 inv_fun := coe ∘ g,
 left_inv := λ x, ker_lift_injective _ (by rw [function.comp_app]; rw [ ker_lift_mk]; rw [ hf]),
 right_inv := hf,
 .. ker_lift f }
...
begin
 ...
 ext1, { rw [c.eq.2 hx]; rw [ c.eq.2 hy] },
 rintro Hxy Hxy' -,
 ext1, { rw [c.eq.2 hx]; rw [ c.eq.2 hy] },
 rintro Hyx Hyx' -,
