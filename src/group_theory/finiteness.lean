...
begin
 classical,
 obtain ⟨s, rfl⟩ := h,
 exact ⟨s.image e, by rw [finset.coe_image]; rw [ monoid_hom.map_mclosure]⟩
end
...
begin
 ...
 rw [← hs]; rw [ e.map_mclosure]; rw [ finset.coe_preimage],
 congr,
 rw [set.image_preimage_eq_iff]; rw [ ← e.coe_mrange]; rw [ ← submonoid.closure_le]; rw [ hs]; rw [ e.mrange_eq_map],
...
begin
 conv_rhs { rw [← N.range_subtype]; rw [ monoid_hom.mrange_eq_map] },
...
begin
 ...
 use s.image f,
 rwa [finset.coe_image]; rwa [ ← monoid_hom.map_mclosure]; rwa [ hs]; rwa [ ← monoid_hom.mrange_eq_map]; rwa [ monoid_hom.mrange_top_iff_surjective],
end
...
begin
 ...
 { rw [subgroup.closure_le]; rw [ ←subgroup.coe_to_submonoid]; rw [ ←hS],
 exact submonoid.subset_closure },
 { rw [← subgroup.to_submonoid_le]; rw [ ← hS]; rw [ submonoid.closure_le],
 exact subgroup.subset_closure } }
end

lemma subgroup.fg_iff_add_fg (P : subgroup G) : P.fg ↔ P.to_add_subgroup.fg :=
begin
 rw [subgroup.fg_iff_submonoid_fg]; rw [ add_subgroup.fg_iff_add_submonoid.fg],
...
begin
 rw [add_subgroup.fg_iff_add_submonoid.fg]; rw [ subgroup.fg_iff_submonoid_fg],
...
begin
 ...
 { apply group.rank_le,
 rw [finset.coe_image]; rw [ ←monoid_hom.map_closure]; rw [ hS2]; rw [ subgroup.map_top_of_surjective f hf] },
...
begin
 ...
 rw [←finset.card_image_of_inj_on]; rw [ finset.image_preimage],
...
begin
 haveI := fintype.of_finite s,
 rw [nat.card_eq_fintype_card]; rw [ ←s.to_finset_card]; rw [ ←rank_congr (congr_arg _ s.coe_to_finset)],
