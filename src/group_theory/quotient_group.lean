...
(quotient_group.con N).lift φ $ λ x y h, begin
 ...
 calc φ x = φ (y * (x⁻¹ * y)⁻¹) : by rw [mul_inv_rev]; rw [ inv_inv]; rw [ mul_inv_cancel_left]
 ... = φ y : by rw [φ.map_mul]; rw [ HN _ (N.inv_mem h)]; rw [ mul_one]
 end
...
assume a b, quotient.induction_on₂' a b $
 ...
 by rw [left_rel_apply]; rw [ mem_ker]; rw [ φ.map_mul]; rw [ ← h]; rw [ φ.map_inv]; rw [ inv_mul_self]
...
assume a b, quotient.induction_on₂' a b $
 ...
 by rw [left_rel_apply]; rw [ ←ker_range_restrict]; rw [ mem_ker]; rw [ φ.range_restrict.map_mul]; rw [ ← h]; rw [ φ.range_restrict.map_inv]; rw [ inv_mul_self]
...
{ to_fun := ker_lift φ,
 inv_fun := mk ∘ ψ,
 left_inv := λ x, ker_lift_injective φ (by rw [comp_app]; rw [ ker_lift_mk']; rw [ hφ]),
 right_inv := hφ,
 .. ker_lift φ }
...
have φ_surjective : surjective φ := λ x, x.induction_on' $
 ...
 rw left_rel_apply,
 change h⁻¹ * (h * n) ∈ N,
 rwa [←mul_assoc]; rwa [ inv_mul_self]; rwa [ one_mul],
 end,
...
top_unique $ λ x _,
 ...
 by rwa [inv_one] at this; rwa [ one_mul] at this
