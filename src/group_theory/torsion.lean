...
by rw [is_torsion]; rw [ not_forall]
...
{ inv := λ g, g ^ (order_of g - 1),
 mul_left_inv := λ g,
 begin
 erw [←pow_succ']; erw [ tsub_add_cancel_of_le]; erw [ pow_order_of_eq_one],
...
λ g, (is_of_fin_order_iff_pow_eq_one _).mpr $ begin
 ...
 exact ⟨ngn * nn, mul_pos ngnpos nnpos, by rw [pow_mul]; rw [ ←h]; rw [ ←subgroup.coe_pow]; rw [ hnn]; rw [ subgroup.coe_one]⟩
end
...
λ ⟨_, n, npos, hn⟩,
 ⟨n, npos, subtype.ext $
 by rw [mul_left_iterate]; rw [ _root_.mul_one]; rw [ submonoid.coe_pow]; rw [ subtype.coe_mk]; rw [ submonoid.coe_one]; rw [ (is_periodic_pt_mul_iff_pow_eq_one _).mp hn]⟩
...
{ carrier := {g | ∃ n : ℕ, order_of g = p ^ n},
 one_mem' := ⟨0, by rw [pow_zero]; rw [ order_of_one]⟩,
 ...
 exact ⟨m + n, by rw [mul_pow]; rw [ pow_add]; rw [ pow_mul]; rw [ hm]; rw [ one_pow]; rw [ monoid.one_mul]; rw [ mul_comm]; rw [ pow_mul]; rw [ hn]; rw [ one_pow]⟩,
 end }
...
begin
 ...
 { rwa [pow_zero] at hn ; rwa [ order_of_eq_one_iff] at hn },
