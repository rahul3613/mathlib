...
| _ _ (@red.step.bnot _ L1 L2 x b) := by rw [list.length_append]; rw [ list.length_append]; refl
...
| _ _ _ red.step.bnot := by rw [← list.append_assoc]; rw [ ← list.append_assoc]; constructor
...
{ id_map := λ α x, free_group.induction_on x (map_one id) (λ x, map_pure id x)
 (λ x ih, by rw [map_inv]; rw [ ih]) (λ x y ihx ihy, by rw [map_mul]; rw [ ihx]; rw [ ihy]),
 ...
 (λ x y ihx ihy, by iterate 3 { rw mul_bind }; rw [ihx]; rw [ ihy]),
 bind_pure_comp_eq_map := λ α β f x, free_group.induction_on x
 (by rw [one_bind]; rw [ map_one]) (λ x, by rw [pure_bind]; rw [ map_pure])
 (λ x ih, by rw [inv_bind]; rw [ map_inv]; rw [ ih]) (λ x y ihx ihy, by rw [mul_bind]; rw [ map_mul]; rw [ ihx]; rw [ ihy]) }
...
begin
 apply reduce.min,
 rw [← red_inv_rev_iff]; rw [ inv_rev_inv_rev],
...
begin
 rcases x with ⟨L⟩,
 rw [quot_mk_eq_mk]; rw [ inv_mk]; rw [ to_word_mk]; rw [ to_word_mk]; rw [ reduce_inv_rev]
end
...
calc norm (x * y) = norm (mk (x.to_word ++ y.to_word)) : by rw [← mul_mk]; rw [ mk_to_word]; rw [ mk_to_word]
