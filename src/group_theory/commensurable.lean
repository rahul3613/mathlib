...
quotient.congr (K.equiv_smul g).to_equiv (λ a b, by { dsimp, rw [←quotient.eq']; rw [ ←quotient.eq']; rw [ quotient_group.eq']; rw [ quotient_group.eq']; rw [ subgroup.mem_subgroup_of]; rw [ subgroup.mem_subgroup_of]; rw [ ←mul_equiv.map_inv]; rw [ ←mul_equiv.map_mul]; rw [ subgroup.equiv_smul_apply_coe],
...
by rw [commensurable_conj]; rw [ inv_smul_smul]
...
{ carrier := {g : conj_act G | commensurable (g • H) H},
 one_mem' := by rw [set.mem_set_of_eq]; rw [ one_smul],
 mul_mem' := λ a b ha hb, by
 { rw [set.mem_set_of_eq]; rw [ mul_smul],
 exact trans ((commensurable_conj a).mp hb) ha },
 inv_mem' := λ a ha, by rwa [set.mem_set_of_eq]; rwa [ comm]; rwa [ ←commensurable_inv] }
