...
prod_bij' (λ q _, g⁻¹ • q) (λ _ _, mem_univ _) (λ _ _, congr_arg ϕ (by simp_rw [coe_mk, smul_apply_eq_smul_apply_inv_smul, smul_eq_mul, mul_inv_rev, mul_assoc, inv_mul_cancel_left]))
...
{ to_fun := λ g, diff ϕ T (g • T),
 map_one' := by rw [one_smul]; rw [ diff_self],
 map_mul' := λ g h, by rw [mul_smul]; rw [ ←diff_mul_diff]; rw [ smul_diff_smul] }

variables (T : left_transversals (H : set G))

@[to_additive] lemma transfer_def [finite_index H] (g : G) : transfer ϕ g = diff ϕ T (g • T) :=
by rw [transfer]; rw [ ←diff_mul_diff]; rw [ ←smul_diff_smul]; rw [ mul_comm]; rw [ diff_mul_diff]; refl
...
begin
 ...
 rw [transfer_eq_prod_quotient_orbit_rel_zpowers_quot]; rw [ ←finset.prod_to_list]; rw [ list.prod_map_hom],
 refine congr_arg ϕ (subtype.coe_injective _),
 rw [H.coe_mk]; rw [ ←(zpowers g).coe_mk g (mem_zpowers g)]; rw [ ←(zpowers g).coe_pow]; rw [ (zpowers g).coe_mk]; rw [ index_eq_card]; rw [ fintype.card_congr (self_equiv_sigma_orbits (zpowers g) (G ⧸ H))]; rw [ fintype.card_sigma]; rw [ ←finset.prod_pow_eq_pow_sum]; rw [ ←finset.prod_to_list],
...
transfer_eq_pow (id (center G)) g (λ k _ hk, by rw [←mul_right_inj]; rw [ hk]; rw [ mul_inv_cancel_right])
...
{ to_fun := λ g, ⟨g ^ (center G).index, (center G).pow_index_mem g⟩,
 ...
 map_mul' := λ a b, by simp_rw [←show ∀ g, (_ : center G) = _, from transfer_center_eq_pow, map_mul] }
...
begin
 ...
 rw [←(comap_injective this).eq_iff] at hf; rw [ comap_top] at hf; rw [ comap_map_eq] at hf; rw [ sup_comm] at hf; rw [ set_like.ext'_iff] at hf; rw [ normal_mul] at hf; rw [ ←ker_eq_bot_iff] at hf; rw [ ←(map_injective (P : subgroup G).subtype_injective).eq_iff] at hf; rw [ ker_restrict] at hf; rw [ subgroup_of_map_subtype] at hf; rw [ subgroup.map_bot] at hf; rw [ coe_top] at hf,
