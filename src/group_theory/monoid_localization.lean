...
begin
 ...
 by ac_refl
 ... = (t₂ * t₁ : M) * ((a.2 * c.2) * (b.1 * d.1)) : by { rw [ht₁]; rw [ ht₂], ac_refl } }
end
...
rec f (λ a c b d h, by rw [eq_rec_constant]; rw [ H h]) x
...
by { symmetry, rw [← mk_one]; rw [ mk_eq_mk_iff], exact one_rel a }
...
by rw [eq_comm]; rw [ mul_inv_left h]; rw [ mul_comm]; rw [ eq_comm]
...
by rw [mul_inv_right h]; rw [ mul_assoc]; rw [ mul_comm _ (f y₂)]; rw [ ←mul_assoc]; rw [ mul_inv_left h]; rw [ mul_comm x₂]; rw [ f.map_mul]; rw [ f.map_mul]
...
lemma inv_inj {f : M →* N} (hf : ∀ y : S, is_unit (f y)) {y z}
 ...
 f y = f z :=
by rw [←mul_one (f y)]; rw [ eq_comm]; rw [ ←mul_inv_left hf y (f z) 1]; rw [ h];
...
by rw [←one_mul ↑(_)⁻¹]; rw [ mul_inv_left]; rw [ ←H]
...
begin
 rw [f.to_map.map_mul] at h; rw [ f.to_map.map_mul] at h,
...
@[to_additive] lemma map_left_cancel {x y} {c : S} (h : f.to_map (x * c) = f.to_map (y * c)) :
 f.to_map x = f.to_map y :=
f.map_right_cancel $ by rw [mul_comm _ x]; rw [ mul_comm _ y]; rw [ h]
...
(mul_inv_left f.map_units _ _ _).2 $
 show _ = _ * (_ * _ * (_ * _)), by
 rw [←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_inv_right f.map_units]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_comm _ (f.to_map x₂)]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_inv_right f.map_units]; rw [ submonoid.coe_mul]; rw [ f.to_map.map_mul]; rw [ f.to_map.map_mul];
 ac_refl

@[to_additive] lemma mk'_one (x) : f.mk' x (1 : S) = f.to_map x :=
by rw [mk']; rw [ monoid_hom.map_one]; exact mul_one _
...
show _ * _ = _, by rw [←sec_spec]; rw [ mul_inv_left]; rw [ mul_comm]
...
show _ * _ * _ = _, by rw [mul_assoc]; rw [ mul_comm _ (f.to_map y)]; rw [ ←mul_assoc]; rw [ mul_inv_left]; rw [ mul_comm]

@[to_additive] lemma mk'_spec' (x) (y : S) :
 f.to_map y * f.mk' x y = f.to_map x :=
by rw [mul_comm]; rw [ mk'_spec]

@[to_additive] theorem eq_mk'_iff_mul_eq {x} {y : S} {z} :
 z = f.mk' x y ↔ z * f.to_map y = f.to_map x :=
⟨λ H, by rw [H, mk'_spec], λ H, by erw [mul_inv_right]; erw [ H]; refl⟩

@[to_additive] theorem mk'_eq_iff_eq_mul {x} {y : S} {z} :
 f.mk' x y = z ↔ f.to_map x = z * f.to_map y :=
by rw [eq_comm]; rw [ eq_mk'_iff_mul_eq]; rw [ eq_comm]

@[to_additive] lemma mk'_eq_iff_eq {x₁ x₂} {y₁ y₂ : S} :
 f.mk' x₁ y₁ = f.mk' x₂ y₂ ↔ f.to_map (y₂ * x₁) = f.to_map (y₁ * x₂) :=
⟨λ H, by rw [f.to_map.map_mul]; rw [ f.to_map.map_mul]; rw [ f.mk'_eq_iff_eq_mul.1 H]; rw [ ←mul_assoc]; rw [ mk'_spec']; rw [ mul_comm],
 λ H, by rw [mk'_eq_iff_eq_mul]; rw [ mk']; rw [ mul_assoc]; rw [ mul_comm _ (f.to_map y₁)]; rw [ ←mul_assoc]; rw [ ←f.to_map.map_mul]; rw [ mul_comm x₂]; rw [ ←H]; rw [ ←mul_comm x₁]; rw [ f.to_map.map_mul]; rw [ mul_inv_right f.map_units]⟩
...
by rw [f.eq]; rw [ localization.r_iff_exists]
...
@[simp, to_additive] lemma mk'_self' (y : S) :
 f.mk' (y : M) y = 1 :=
show _ * _ = _, by rw [mul_inv_left]; rw [ mul_one]
...
begin
 ...
 rw [←one_mul (g x)]; rw [ ←is_unit.lift_right_inv_mul (g.restrict S) hg c],
 show (_ * g c) * _ = _,
...
{ to_fun := λ z, g (f.sec z).1 * ↑(is_unit.lift_right (g.restrict S) hg (f.sec z).2)⁻¹,
 map_one' := by rw [mul_inv_left]; rw [ mul_one]; exact f.eq_of_eq hg
 (by rw [←sec_spec]; rw [ one_mul]),
 map_mul' := λ x y,
 begin
 rw [mul_inv_left hg]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_inv_right hg]; rw [ mul_comm _ (g (f.sec y).1)]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_inv_right hg],
 repeat { rw ←g.map_mul },
...
(mul_inv hg).2 $ f.eq_of_eq hg $ by
 rw [f.to_map.map_mul]; rw [ f.to_map.map_mul]; rw [ sec_spec']; rw [ mul_assoc]; rw [ f.mk'_spec]; rw [ mul_comm]
...
begin
 rw mul_comm,
 show _ * (_ * _) = _ ↔ _,
 rw [←mul_assoc]; rw [ mul_inv_left hg]; rw [ mul_comm],
end
...
show _ * _ * _ = _, by erw [mul_assoc]; erw [ is_unit.lift_right_inv_mul]; erw [ mul_one]
...
begin
 ...
 rw [mk']; rw [ ←mul_assoc]; rw [ mul_inv_right f.map_units]; rw [ ←f.to_map.map_mul]; rw [ ←f.to_map.map_mul],
 apply k.eq_of_eq f.map_units,
 rw [k.to_map.map_mul]; rw [ k.to_map.map_mul]; rw [ ←sec_spec]; rw [ mul_assoc]; rw [ lift_spec_mul],
...
begin
 ...
 erw [is_unit.mul_lift_right_inv (g.restrict S) hg]; erw [ mul_one] },
 { intros H v,
 obtain ⟨x, hx⟩ := H v,
 use f.mk' x.1 x.2,
...
by rw [mul_comm]; rw [ f.map_mul_right]
...
begin
 ...
 rw [←j.to_map.map_mul]; rw [ ←j.to_map.map_mul]; rw [ ←l.map_mul]; rw [ ←l.map_mul],
 exact k.comp_eq_of_eq hl j
 (by rw [k.to_map.map_mul]; rw [ k.to_map.map_mul]; rw [ sec_spec']; rw [ mul_assoc]; rw [ map_mul_right]) },
end
...
by rw [mul_mk'_eq_mk'_of_mul]; rw [ mk'_self]
...
by rw [add_mk'_eq_mk'_of_add]; rw [ mk'_self]
...
(λ v, let ⟨z, hz⟩ := k.to_equiv.surjective v in
 let ⟨x, hx⟩ := f.surj z in ⟨x, show v * k _ = k _, by rw [←hx]; rw [ k.map_mul]; rw [ ←hz]; refl⟩)
...
(f.to_map.comp k.to_monoid_hom).to_localization_map
 ...
 show z * f.to_map (k.to_equiv w) = f.to_map (k.to_equiv v), by erw [hv]; erw [ hw]; erw [ hx]; refl⟩)
 (λ x y, show f.to_map _ = f.to_map _ ↔ _, by erw f.eq_iff_exists;
 exact ⟨λ ⟨c, hc⟩, let ⟨d, hd⟩ := k.to_equiv.surjective c in
 ⟨⟨d, H' ▸ show k d ∈ S, from hd.symm ▸ c.2⟩, by erw [←hd]; erw [ ←k.map_mul]; erw [ ←k.map_mul] at hc;
 exact k.to_equiv.injective hc⟩, λ ⟨c, hc⟩, ⟨⟨k c, H ▸ set.mem_image_of_mem k c.2⟩,
 by erw ←k.map_mul; rw [hc]; rw [ k.map_mul]; refl⟩⟩)
...
{ to_fun := λ x, mk x 1,
 map_one' := mk_one,
 map_mul' := λ x y, by rw [mk_mul]; rw [ mul_one],
 map_units' := λ y, is_unit_iff_exists_inv.2 ⟨mk 1 y, by rw [mk_mul]; rw [ mul_one]; rw [ one_mul]; rw [ mk_self]⟩,
 surj' := λ z, induction_on z $ λ x, ⟨x,
 by rw [mk_mul]; rw [ mul_comm x.fst]; rw [ ← mk_mul]; rw [ mk_self]; rw [ one_mul]⟩,
 eq_iff_exists' := λ x y, mk_eq_mk_iff.trans $ r_iff_exists.trans $
 show (∃ (c : S), ↑c * (1 * x) = c * (1 * y)) ↔ _, by rw [one_mul]; rw [ one_mul],
...
begin
 rw [←mk_one_eq_monoid_of_mk]; rw [ ←mk_one_eq_monoid_of_mk]; rw [ show mk x y * mk y 1 = mk (x * y) (1 * y)]; rw [ by rw [mul_comm 1 y]; rw [ mk_mul],
 show mk x 1 = mk (x * 1) ((1 : S) * 1), by rw [mul_one]; rw [ mul_one]],
...
by rw [localization_map.sec_spec']; rw [ mul_zero]
...
{ map_zero' :=
 begin
 rw [monoid_hom.to_fun_eq_coe]; rw [ localization_map.lift_spec]; rw [ mul_zero]; rw [ ←map_zero g]; rw [ ←g.to_monoid_hom_coe],
...
⟨λ a b, localization.lift_on₂ a b (λ a₁ a₂ b₁ b₂, ↑b₂ * a₁ ≤ a₂ * b₁) $
 ...
 dsimp,
 rw [←mul_le_mul_iff_right]; rw [ mul_right_comm]; rw [ ←hf]; rw [ mul_right_comm]; rw [ mul_right_comm ↑a₂]; rw [ mul_le_mul_iff_right]; rw [ ←mul_le_mul_iff_left]; rw [ mul_left_comm]; rw [ he]; rw [ mul_left_comm]; rw [ mul_left_comm ↑b₂]; rw [ mul_le_mul_iff_left],
 end⟩
...
⟨λ a b, localization.lift_on₂ a b (λ a₁ a₂ b₁ b₂, ↑b₂ * a₁ < a₂ * b₁) $
 ...
 simp only [mul_right_inj] at he hf,
 dsimp,
 rw [←mul_lt_mul_iff_right]; rw [ mul_right_comm]; rw [ ←hf]; rw [ mul_right_comm]; rw [ mul_right_comm ↑a₂]; rw [ mul_lt_mul_iff_right]; rw [ ←mul_lt_mul_iff_left]; rw [ mul_left_comm]; rw [ he]; rw [ mul_left_comm]; rw [ mul_left_comm ↑b₂]; rw [ mul_lt_mul_iff_left],
 end⟩
...
{ le := (≤),
 ...
 rwa [mul_left_comm]; rwa [ mul_left_comm ↑b.2]; rwa [ mul_le_mul_iff_left],
 end,
