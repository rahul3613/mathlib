...
begin
 ...
 rwa [mul_assoc]; rwa [ mul_assoc]; rwa [ eq_inv_mul_iff_mul_eq]; rwa [ ←mul_assoc]; rwa [ ←mul_assoc]; rwa [ eq_mul_inv_iff_mul_eq],
end
...
begin
 ext a b,
 rw [rel_iff]; rw [ setoid.rel]; rw [ quotient_group.left_rel_apply],
 split,
 { rintros ⟨a, (rfl : a = 1), b, hb, rfl⟩,
 change a⁻¹ * (1 * a * b) ∈ H,
 rwa [one_mul]; rwa [ inv_mul_cancel_left] },
 { rintro (h : a⁻¹ * b ∈ H),
 exact ⟨1, rfl, a⁻¹ * b, h, by rw [one_mul]; rw [ mul_inv_cancel_left]⟩ },
end
...
begin
 ext a b,
 rw [rel_iff]; rw [ setoid.rel]; rw [ quotient_group.right_rel_apply],
 split,
 { rintros ⟨b, hb, a, (rfl : a = 1), rfl⟩,
 change b * a * 1 * a⁻¹ ∈ H,
 rwa [mul_one]; rwa [ mul_inv_cancel_right] },
 { rintro (h : b * a⁻¹ ∈ H),
 exact ⟨b * a⁻¹, h, 1, rfl, by rw [mul_one]; rw [ inv_mul_cancel_right]⟩ },
end
