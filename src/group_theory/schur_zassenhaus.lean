...
quotient (setoid.mk (λ α β, diff (monoid_hom.id H) α β = 1) ⟨λ α, diff_self (monoid_hom.id H) α,
 λ α β h, by rw [←diff_inv]; rw [ h]; rw [ inv_one], λ α β γ h h', by rw [←diff_mul_diff]; rw [ h]; rw [ h']; rw [ one_mul]⟩)
...
begin
 ...
 hH.mem_comm ((congr_arg (∈ H) (mul_inv_cancel_left _ _)).mpr (set_like.coe_mem _))⟩,
 map_one' := by rw [subtype.ext_iff]; rw [ coe_mk]; rw [ coe_one]; rw [ mul_one]; rw [ inv_mul_self],
 map_mul' := λ h₁ h₂, by rw [subtype.ext_iff]; rw [ coe_mk]; rw [ coe_mul]; rw [ coe_mul]; rw [ coe_mk]; rw [ coe_mk]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_inv_cancel_left] },
 ...
 simp_rw [monoid_hom.id_apply, monoid_hom.coe_mk, coe_mk, smul_apply_eq_smul_apply_inv_smul, smul_eq_mul_unop, mul_inv_rev, mul_assoc],
end

variables {H} [normal H]

instance : mul_action G H.quotient_diff :=
{ smul := λ g, quotient.map' (λ α, op g⁻¹ • α) (λ α β h, subtype.ext (by rwa [smul_diff_smul']; rwa [ coe_mk]; rwa [ coe_one]; rwa [ mul_eq_one_iff_eq_inv]; rwa [ mul_right_eq_self]; rwa [ ←coe_one]; rwa [ ←subtype.ext_iff])),
...
quotient.induction_on' α $ λ α hα, (pow_coprime hH).injective $
 ...
 by rw [←diff_inv]; rw [ smul_diff']; rw [ diff_self]; rw [ one_mul]; rw [ inv_pow]; rw [ inv_inv]
...
quotient.induction_on' α (quotient.induction_on' β (λ β α, exists_imp_exists (λ n, quotient.sound')
 ...
 (by rw [inv_pow]; rw [ ←pow_coprime_apply hH]; rw [ equiv.apply_symm_apply]; rw [ mul_inv_self])))⟩))
...
begin
 ...
 rwa [←key]; rwa [ comap_sup_eq]; rwa [ hH.symm.sup_eq_top]; rwa [ comap_top] },
 { rw ← comap_top (quotient_group.mk' K),
 intro hH',
 rw [comap_injective this hH'] at hH; rw [ is_complement'_top_right] at hH; rw [ map_eq_bot_iff] at hH; rw [ quotient_group.ker_mk] at hH,
...
begin
 ...
 conv at key { congr, skip, to_rhs, rw [←N.subtype_range]; rw [ N.subtype.range_eq_map] },
 have inj := map_injective N.subtype_injective,
 rwa [inj.eq_iff] at key; rwa [ inj.eq_iff] at key,
end
...
begin
 ...
 rw [←monoid_hom.range_top_iff_surjective]; rw [ subtype_range],
...
begin
 ...
 { rw [hN1] at hN; rw [ nat.coprime_zero_left] at hN; rw [ index_eq_one] at hN,
 rw hN,
 exact ⟨⊥, is_complement'_top_bot⟩ },
 by_cases hN2 : N.index = 0,
 { rw [hN2] at hN; rw [ nat.coprime_zero_right] at hN,
