...
(monoid_hom.cancel_right con.mk'_surjective).mp $ free_monoid.hom_eq $ λ ⟨i, x⟩,
 by rw [monoid_hom.comp_apply]; rw [ monoid_hom.comp_apply]; rw [ ←of_apply]; rw [ ←monoid_hom.comp_apply]; rw [ ←monoid_hom.comp_apply]; rw [ h]
...
{ to_fun := λ fi, con.lift _ (free_monoid.lift $ λ p : Σ i, M i, fi p.fst p.snd) $ con.con_gen_le
 ...
 rw [monoid_hom.comp_apply]; rw [ of_apply]; rw [ con.lift_mk']; rw [ free_monoid.lift_eval_of], },
...
@[simp] lemma lift_of {N} [monoid N] (fi : Π i, M i →* N) {i} (m : M i) :
 lift fi (of m) = fi i m :=
by conv_rhs { rw [←lift.symm_apply_apply fi]; rw [ lift_symm_apply]; rw [ monoid_hom.comp_apply] }
...
{ mul_left_inv := begin
 ...
 { rw [monoid_hom.map_one]; rw [ mul_opposite.unop_one]; rw [ one_mul], },
 { intros i m, change of m⁻¹ * of m = 1, rw [←of.map_mul]; rw [ mul_left_inv]; rw [ of.map_one], },
 { intros x y hx hy,
 rw [monoid_hom.map_mul]; rw [ mul_opposite.unop_mul]; rw [ mul_assoc]; rw [ ← mul_assoc _ x y]; rw [ hx]; rw [ one_mul]; rw [ hy], },
 end,
...
by { rw [rcons]; rw [ dif_neg], refl, exact h1 ⟨i, m⟩ (ls.mem_cons_self _) }

@[simp] lemma prod_rcons {i} (p : pair M i) :
 prod (rcons p) = of p.head * prod p.tail :=
if hm : p.head = 1 then by rw [rcons]; rw [ dif_pos hm]; rw [ hm]; rw [ monoid_hom.map_one]; rw [ one_mul]
else by rw [rcons]; rw [ dif_neg hm]; rw [ prod]; rw [ list.map_cons]; rw [ list.prod_cons]; rw [ prod]
...
by rw [cons_eq_rcons]; rw [ of_smul_def]; rw [ equiv_pair_eq_of_fst_idx_ne _]; simp only [mul_one]
...
begin
 apply m.induction_on,
 { intro, rw [one_smul]; rw [ one_mul] },
 { intros, rw [of_smul_def]; rw [ prod_rcons]; rw [ of.map_mul]; rw [ mul_assoc]; rw [ ←prod_rcons]; rw [ ←equiv_pair_symm]; rw [ equiv.symm_apply_apply] },
 { intros x y hx hy w, rw [mul_smul]; rw [ hx]; rw [ hy]; rw [ mul_assoc] },
end
...
{ to_fun := λ m, m • empty,
 inv_fun := λ w, prod w,
 left_inv := λ m, by dsimp only; rw [prod_smul]; rw [ prod_empty]; rw [ mul_one],
 right_inv := begin
 apply smul_induction,
 { dsimp only, rw [prod_empty]; rw [ one_smul], },
 { dsimp only, intros i m w ih, rw [prod_smul]; rw [ mul_smul]; rw [ ih], },
 end }
...
{ to_list := w.to_list,
 ...
 intros x hx y hy,
 rw [w_w₁.to_list_last'] at hx; rw [ option.mem_some_iff] at hx,
 rw [w_w₂.to_list_head'] at hy; rw [ option.mem_some_iff] at hy,
 subst hx, subst hy,
 exact w_hne, },
 end, }
...
begin
 ...
 simp only [H],
 rw [free_group.free_group_unit_equiv_int.cardinal_eq]; rw [ cardinal.mk_denumerable],
 apply le_of_lt,
 simp },
 ...
 { intros n hle hi,
 calc (a i ^ (n + 1)) • (Y i)ᶜ
 = (a i ^ n * a i) • (Y i)ᶜ : by rw [zpow_add]; rw [ zpow_one]
 ...
 { rw [zpow_neg]; rw [ zpow_one], exact hY i, },
 { intros n hle hi,
 calc (a i ^ (n - 1)) • (X i)ᶜ
 = (a i ^ n * (a i)⁻¹) • (X i)ᶜ : by rw [zpow_sub]; rw [ zpow_one]
