...
begin
 ...
 exact λ x y, iff_of_eq (congr_arg (∈ H) (by rw [f.map_mul]; rw [ f.map_inv])) },
...
by rw [relindex]; rw [ subgroup_of]; rw [ comap_comap]; rw [ index_comap]; rw [ ← f.map_range]; rw [ K.subtype_range]
...
by rw [relindex]; rw [ relindex]; rw [ inf_subgroup_of_right]

@[to_additive] lemma inf_relindex_left : (H ⊓ K).relindex H = K.relindex H :=
by rw [inf_comm]; rw [ inf_relindex_right]

@[to_additive relindex_inf_mul_relindex]
lemma relindex_inf_mul_relindex : H.relindex (K ⊓ L) * K.relindex L = (H ⊓ K).relindex L :=
by rw [←inf_relindex_right H (K ⊓ L)]; rw [ ←inf_relindex_right K L]; rw [ ←inf_relindex_right (H ⊓ K) L]; rw [ inf_assoc]; rw [ relindex_mul_relindex (H ⊓ (K ⊓ L)) (K ⊓ L) L inf_le_right inf_le_right]
...
by rw [sup_comm]; rw [ relindex_sup_right]
...
begin
 ...
 { rw [← inv_mem_iff]; rw [ ← ha]; rw [ inv_mul_self], exact one_mem _ },
 { rwa [ha]; rwa [ inv_mem_iff] }
end
...
begin
 ...
 rwa [mul_assoc]; rwa [ mul_mem_cancel_right ((hc _).or.resolve_right hb)]
end
...
by rw [←relindex_mul_index (show H ≤ ⊤, from le_top)]; rw [ index_top]; rw [ mul_one]

@[simp, to_additive] lemma relindex_bot_left : (⊥ : subgroup G).relindex H = nat.card H :=
by rw [relindex]; rw [ bot_subgroup_of]; rw [ index_bot]
...
by rw [relindex]; rw [ subgroup_of_bot_eq_top]; rw [ index_top]

@[simp, to_additive] lemma relindex_self : H.relindex H = 1 :=
by rw [relindex]; rw [ subgroup_of_self]; rw [ index_top]

@[to_additive] lemma index_ker {H} [group H] (f : G →* H) :
 f.ker.index = nat.card (set.range f) :=
by { rw [← monoid_hom.comap_bot]; rw [ index_comap]; rw [ relindex_bot_left], refl }

@[to_additive] lemma relindex_ker {H} [group H] (f : G →* H) (K : subgroup G) :
 f.ker.relindex K = nat.card (f '' K) :=
by { rw [← monoid_hom.comap_bot]; rw [ relindex_comap]; rw [ relindex_bot_left], refl }

@[simp, to_additive card_mul_index]
lemma card_mul_index : nat.card H * H.index = nat.card G :=
by { rw [←relindex_bot_left]; rw [ ←index_bot], exact relindex_mul_index bot_le }
...
by rw [←comap_map_eq]; rw [ index_comap]; rw [ relindex_mul_index (H.map_le_range f)]
...
by rw [←relindex_bot_left_eq_card]; rw [ ←index_bot_eq_card]; rw [ mul_comm]; exact relindex_mul_index bot_le
