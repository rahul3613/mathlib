...
⟨ { carrier := ⋃ (R : c), R,
 ...
 rwa [subtype.ext_iff] at hk ⊢ ; rwa [ coe_pow] at hk ⊢ },
...
begin
 rw [eq_comm]; rw [ set_like.ext_iff]; rw [ ←inv_mem_iff]; rw [ mem_normalizer_iff]; rw [ inv_inv],
...
by rw [P.sylow_mem_fixed_points_iff]; rw [ ←inf_eq_left]; rw [ hP.inf_normalizer_sylow]; rw [ inf_eq_left]
...
begin
 ...
 { rwa [le_centralizer_iff]; rwa [ zpowers_le] },
 have h2 : ↑(g • P) ≤ centralizer (zpowers x : set G),
 { rw [le_centralizer_iff]; rw [ zpowers_le],
 rintros - ⟨z, hz, rfl⟩,
 specialize hy z hz,
 rwa [←mul_assoc] at hy ; rwa [ ←eq_mul_inv_iff_mul_eq] at hy ; rwa [ mul_assoc] at hy ; rwa [ mul_assoc] at hy ; rwa [ mul_assoc] at hy ; rwa [ ←mul_assoc] at hy ; rwa [ eq_inv_mul_iff_mul_eq] at hy ; rwa [ ←mul_assoc] at hy ; rwa [ ←mul_assoc] at hy },
 obtain ⟨h, hh⟩ :=
...
begin
 ...
 rw [←zpowers_eq_bot] at hp; rw [ ←ne] at hp; rw [ ←bot_lt_iff_ne_bot] at hp; rw [ ←comap_lt_comap_of_surjective (quotient_group.mk'_surjective _)] at hp; rw [ monoid_hom.comap_bot] at hp; rw [ quotient_group.ker_mk] at hp,
...
begin
 ...
 rw [sylow.smul_def] at hn; rw [ ←mul_smul] at hn; rw [ ←mul_aut.conj_normal_coe] at hn; rw [ ←mul_aut.conj_normal.map_mul] at hn; rw [ sylow.ext_iff] at hn; rw [ sylow.pointwise_smul_def] at hn; rw [ pointwise_smul_def] at hn,
...
by rw [← sylow.normalizer_sup_eq_top (P.subtype hP)]; rw [ P.coe_subtype]; rw [ subgroup_of_map_subtype]; rw [ inf_of_le_left hP]
...
by rw [← fintype.card_prod]; rw [ fintype.card_congr (preimage_mk_equiv_subgroup_times_set _ _)]
...
⟨λ hx, have ha : ∀ {y : G ⧸ H}, y ∈ orbit H (x : G ⧸ H) → y = x,
 ...
 show _ ∈ H, by {rw [mul_inv_rev] at this; rw [ inv_inv] at this, convert this, rw inv_inv}
 )),
...
begin
 rw [card_eq_card_quotient_mul_card_subgroup H]; rw [ card_eq_card_quotient_mul_card_subgroup (H.subgroup_of (normalizer H))]; rw [ fintype.card_congr this]; rw [ hH]; rw [ pow_succ],
...
have hcard : card (G ⧸ H) = s * p :=
 ...
 (by rwa [← card_eq_card_quotient_mul_card_subgroup H]; rwa [ hH]; rwa [ hs]; rwa [ pow_succ']; rwa [ mul_assoc]; rwa [ mul_comm p]),
have hm : s * p % p =
...
nat.dvd_of_mod_eq_zero
 (by rwa [nat.mod_eq_zero_of_dvd (dvd_mul_left _ _)] at hm); rwa [ eq_comm] at hm)
...
begin
 ...
 rw [set.card_image_of_injective (subgroup.comap (mk' (H.subgroup_of H.normalizer)) (zpowers x) : set (H.normalizer)) subtype.val_injective]; rw [ pow_succ']; rw [ ← hH]; rw [ fintype.card_congr hequiv]; rw [ ← hx]; rw [ order_eq_card_zpowers]; rw [ ← fintype.card_prod],
...
begin
 ...
 rw [zpow_zero]; rw [ eq_comm]; rw [ quotient_group.eq_one_iff],
 simpa using hy
end⟩
...
| n m := λ hp hdvd H hH hnm,
 ...
 ⟨K', by rw [hK'.1]; rw [ tsub_add_cancel_of_le h0m.nat_succ_le], le_trans hK.2 hK'.2⟩)
...
begin
 ...
 rwa [h] at key; rwa [ card_bot] at key,
end
...
lemma normal_of_normalizer_normal {p : ℕ} [fact p.prime] [finite (sylow p G)]
 ...
 (↑P : subgroup G).normal :=
by rw [← normalizer_eq_top]; rw [ ← normalizer_sup_eq_top' P le_normalizer]; rw [ sup_idem]
...
begin
 ...
 simp_rw [←normalizer_eq_top, coe_subtype, ← subgroup_of_normalizer_eq le_normalizer, ← subgroup_of_normalizer_eq le_rfl, subgroup_of_self] at this,
 rw [←subtype_range (P : subgroup G).normalizer.normalizer]; rw [ monoid_hom.range_eq_map]; rw [ ←this rfl],
