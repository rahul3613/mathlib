...
begin
 ...
 rw [derived_series_succ]; rw [ ih],
 cases (commutator.normal G).eq_bot_or_eq_top with h h,
 { rw [h]; rw [ commutator_bot_left] },
 { rwa h },
end
...
begin
 ...
 induction n with n ih,
 { exact mem_top x },
 { rw [key]; rw [ (derived_series_normal _ _).mem_comm_iff]; rw [ inv_mul_cancel_left],
...
begin
 ...
 { rwa [←cardinal.lift_mk_le]; rwa [ cardinal.mk_fin]; rwa [ cardinal.lift_nat_cast]; rwa [ nat.cast_bit1]; rwa [ nat.cast_bit0]; rwa [ nat.cast_one]; rwa [ cardinal.lift_id] },
