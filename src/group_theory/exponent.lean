...
begin
 ...
 { refine ⟨1, by rw [h]; rw [ pow_zero]; rw [ order_of_one]⟩ },
 have he : 0 < exponent G := ne.bot_lt (λ ht,
 by {rw ht at h, apply h, rw [bot_eq_zero]; rw [ nat.factorization_zero]; rw [ finsupp.zero_apply] }),
 ...
 { rwa [hk] at hg ; rwa [ mul_comm] at hg ; rwa [ ht] at hg ; rwa [ pow_succ'] at hg ; rwa [ ←mul_assoc] at hg ; rwa [ nat.mul_div_cancel _ hp.pos] at hg ; rwa [ pow_mul] at hg },
 { rw [←nat.succ_eq_add_one]; rw [ ←ht]; rw [ ←pow_mul]; rw [ mul_comm]; rw [ ←hk],
...
begin
 ...
 rw [pow_eq_mod_order_of]; rw [ nat.mod_eq_zero_of_dvd]; rw [ pow_zero g],
...
by rwa [ne.def] at this; rwa [ not_iff_comm] at this; rwa [ iff.comm] at this
...
begin
 ...
 rw [hm]; rw [ pow_mul]; rw [ pow_order_of_eq_one]; rw [ one_pow]
end
...
begin
 ...
 { rw [he]; rw [ set.infinite.nat.Sup_eq_zero $ (exponent_eq_zero_iff_range_order_of_infinite h).1 he] },
 ...
 { rw [order_of_pow' t (pow_ne_zero k hp.ne_zero)]; rw [ nat.gcd_eq_right hpk] },
 ...
 simp [hp] },
 rw [(commute.all _ g).order_of_mul_eq_mul_order_of_of_coprime hcoprime]; rw [ hpk']; rw [ hg]; rw [ ha]; rw [ ←ht]; rw [ ←hk]; rw [ pow_add]; rw [ pow_add]; rw [ pow_one]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ nat.div_mul_cancel]; rw [ mul_assoc]; rw [ lt_mul_iff_one_lt_right $ h t]; rw [ ←pow_succ'],
...
begin
 rw [←finset.nonempty.cSup_eq_max']; rw [ finset.coe_image]; rw [ finset.coe_univ]; rw [ set.image_univ]; rw [ ← supr],
...
begin
 ...
 { rw [←monoid_hom.range_top_iff_surjective]; rw [ eq_top_iff]; rw [ ←hS2]; rw [ closure_le],
