...
⟨λ σ τ, σ = τ ∨ σ = swap i j * τ,
 λ σ, or.inl (refl σ),
 λ σ τ h, or.cases_on h (λ h, or.inl h.symm) (λ h, or.inr (by rw [h]; rw [ swap_mul_self_mul])),
 λ σ τ υ hστ hτυ, by cases hστ; cases hτυ; try {rw [hστ]; rw [ hτυ]; rw [ swap_mul_self_mul]}; simp [hστ, hτυ] ⟩
...
begin
 ...
 rw [monoid_hom.mem_range]; rw [ prod.exists],
 use [σ₁, σ₂],
 rw [perm.sum_congr_hom_apply],
 ext,
 cases x with a b,
 { rw [equiv.sum_congr_apply]; rw [ sum.map_inl]; rw [ perm_congr_apply]; rw [ equiv.symm_symm]; rw [ apply_of_injective_symm sum.inl_injective],
 erw subtype_perm_apply,
 rw [of_injective_apply]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk] },
 { rw [equiv.sum_congr_apply]; rw [ sum.map_inr]; rw [ perm_congr_apply]; rw [ equiv.symm_symm]; rw [ apply_of_injective_symm sum.inr_injective],
 erw subtype_perm_apply,
 rw [of_injective_apply]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk] }
end
...
begin
 ...
 λ n, by rw [hστ.commute.mul_pow]; rw [ disjoint.mul_eq_one_iff (hστ.pow_disjoint_pow n n)],
...
begin
 ...
 { intro h,
 rw [extend_domain_apply_subtype _ _ pb]; rw [ h]; rw [ apply_symm_apply]; rw [ subtype.coe_mk] } },
 { left,
...
then swap_factors_aux l f
 ...
 ⟨swap x (f x) :: m.1,
 by rw [list.prod_cons]; rw [ m.2.1]; rw [ ← mul_assoc]; rw [ mul_def (swap x (f x))]; rw [ swap_swap]; rw [ ← one_def]; rw [ one_mul],
...
is_conj_iff.2 (have h : ∀ {y z : α}, y ≠ z → w ≠ z →
 ...
 λ y z hyz hwz, by rw [mul_inv_rev]; rw [ swap_inv]; rw [ swap_inv]; rw [ mul_assoc (swap w y)]; rw [ mul_assoc (swap w y)]; rw [ ← mul_assoc _ (swap x z)]; rw [ swap_mul_swap_mul_swap hwx hwz]; rw [ ← mul_assoc]; rw [ swap_mul_swap_mul_swap hwz.symm hyz.symm],
 if hwz : w = z
 then have hwy : w ≠ y, by cc,
 ⟨swap w z * swap x y, by rw [swap_comm y z]; rw [ h hyz.symm hwy]⟩
...
then ⟨⟨f⁻¹ a₁, f⁻¹ a₂⟩, mem_fin_pairs_lt.2 hxa,
 ...
 rw [apply_inv_self]; rw [ apply_inv_self]; rw [ if_pos (mem_fin_pairs_lt.1 ha)] }⟩
else ⟨⟨f⁻¹ a₂, f⁻¹ a₁⟩, mem_fin_pairs_lt.2 $ (le_of_not_gt hxa).lt_of_ne $ λ h,
 by simpa [mem_fin_pairs_lt, (f⁻¹).injective h, lt_irrefl] using ha,
 by { dsimp [sign_bij_aux],
 rw [apply_inv_self]; rw [ apply_inv_self]; rw [ if_neg (mem_fin_pairs_lt.1 ha).le.not_lt] }⟩
...
prod_bij (λ a ha, sign_bij_aux f⁻¹ a)
 ...
 then by rw [sign_bij_aux]; rw [ dif_pos h]; rw [ if_neg h.not_le]; rw [ apply_inv_self]; rw [ apply_inv_self]; rw [ if_neg (mem_fin_pairs_lt.1 hab).not_le]
 else by rw [sign_bij_aux]; rw [ if_pos (le_of_not_gt h)]; rw [ dif_neg h]; rw [ apply_inv_self]; rw [ apply_inv_self]; rw [ if_pos (mem_fin_pairs_lt.1 hab).le])
 sign_bij_aux_inj
 sign_bij_aux_surj
...
begin
 ...
 rintros ⟨a, b⟩ hab,
 rw [sign_bij_aux]; rw [ mul_apply]; rw [ mul_apply],
 rw mem_fin_pairs_lt at hab,
 by_cases h : g b < g a,
 { rw dif_pos h,
 simp only [not_le_of_gt hab, mul_one, perm.inv_apply_self, if_false] },
 { rw [dif_neg h]; rw [ inv_apply_self]; rw [ inv_apply_self]; rw [ if_pos hab.le],
 ...
 refl },
 { rw [if_neg h₁]; rw [ if_pos (lt_of_not_ge h₁).le],
 refl } }
end
...
by { rw [← is_conj_iff_eq]; rw [ ← sign_aux_swap_zero_one h2n],
...
by rw [this]; rw [ one_def]; rw [ equiv.trans_refl]; rw [ equiv.symm_trans_self]; rw [ ← one_def]; rw [ sign_aux_one]; rw [ sign_aux2]
| (x::l) f e h := begin
 rw sign_aux2,
 by_cases hfx : x = f x,
 { rw if_pos hfx,
 ...
 rw [if_neg hfx]; rw [ ← sign_aux_eq_sign_aux2 _ _ e hy]; rw [ this]; rw [ sign_aux_mul]; rw [ sign_aux_swap hefx],
...
quotient.hrec_on s (λ l h, sign_aux2 l f)
 ...
 (λ h₁ h₂ _, by rw [← sign_aux_eq_sign_aux2 _ _ e (λ _ _, h₁ _)]; rw [ ← sign_aux_eq_sign_aux2 _ _ e (λ _ _, h₂ _)])))
...
begin
 ...
 split,
 { rw [← sign_aux_eq_sign_aux2 _ _ e (λ _ _, hs _)]; rw [ ← sign_aux_eq_sign_aux2 _ _ e (λ _ _, hs _)]; rw [ ← sign_aux_eq_sign_aux2 _ _ e (λ _ _, hs _)]; rw [ hfg]; rw [ sign_aux_mul] },
 { assume x y hxy,
 have hexy : e x ≠ e y, from mt e.injective.eq_iff.1 hxy,
 rw [← sign_aux_eq_sign_aux2 _ _ e (λ _ _, hs _)]; rw [ symm_trans_swap_trans]; rw [ sign_aux_swap hexy] }
end
...
by rw [monoid_hom.map_inv sign f]; rw [ int.units_inv_eq_self]
...
quotient.induction_on₂ t s
 ...
 by { rw [← sign_aux_eq_sign_aux2 _ _ n (λ _ _, h₁ _)]; rw [ ← sign_aux_eq_sign_aux2 _ _ (e.trans n) (λ _ _, h₂ _)],
...
by rw [← list.prod_replicate]; rw [ ← h₁]; rw [ list.prod_hom _ (@sign α _ _)]
...
λ a, (int.units_eq_one_or a).elim
 ...
 ⟨swap x y, by rw [sign_swap hxy]; rw [ h]⟩ )
...
have ∀ {f}, is_swap f → s f = -1 :=
 ...
 by { rw [← is_conj_iff_eq]; rw [ ← or.resolve_right (int.units_eq_one_or _) h]; rw [ hab'],
 ...
 have s l.prod = 1,
 by rw [← l.prod_hom s]; rw [ list.eq_replicate_length.2 this]; rw [ list.prod_replicate]; rw [ one_pow],
 by { rw [hl.1] at this; rw [ hg] at this,
...
by rw [← hl₁]; rw [ ← l.prod_hom s]; rw [ list.eq_replicate_length.2 hsl]; rw [ list.length_map]; rw [ list.prod_replicate]; rw [ sign_prod_list_swap hl₂]
...
have hl'₂ : (l.1.map of_subtype).prod = f,
 by rw [l.1.prod_hom of_subtype]; rw [ l.2.1]; rw [ of_subtype_subtype_perm _ h₂],
...
by rw [hg]; rw [ sign_symm_trans_trans]
...
begin
 ...
 rw [prod_eq]; rw [ prod_congr_right_apply] },
 clear mem_l,
...
begin
 ...
 rw [← prod_prod_extend_right σ hl mem_l]; rw [ sign.map_list_prod]; rw [ list.map_map]; rw [ ← l_to_finset]; rw [ list.prod_to_finset _ hl],
...
begin
 ...
 { rw [←this.1]; rw [ ←this.2]; rw [ ←sign_mul]; rw [ sum_congr_mul]; rw [ one_mul]; rw [ mul_one], },
 split,
 { apply σa.swap_induction_on _ (λ σa' a₁ a₂ ha ih, _),
 { simp },
 { rw [←one_mul (1 : perm β)]; rw [ ←sum_congr_mul]; rw [ sign_mul]; rw [ sign_mul]; rw [ ih]; rw [ sum_congr_swap_one]; rw [ sign_swap ha]; rw [ sign_swap (sum.inl_injective.ne_iff.mpr ha)], }, },
 { apply σb.swap_induction_on _ (λ σb' b₁ b₂ hb ih, _),
 { simp },
 { rw [←one_mul (1 : perm α)]; rw [ ←sum_congr_mul]; rw [ sign_mul]; rw [ sign_mul]; rw [ ih]; rw [ sum_congr_one_swap]; rw [ sign_swap hb]; rw [ sign_swap (sum.inr_injective.ne_iff.mpr hb)], }, }
end
