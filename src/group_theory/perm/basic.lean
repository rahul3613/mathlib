...
lemma zpow_apply_comm {α : Type*} (σ : perm α) (m n : ℤ) {x : α} :
 (σ ^ m) ((σ ^ n) x) = (σ ^ n) ((σ ^ m) x) :=
by rw [←equiv.perm.mul_apply]; rw [ ←equiv.perm.mul_apply]; rw [ zpow_mul_comm]
...
equiv.ext $ λ x, begin
 ...
 { rw [of_subtype]; rw [ monoid_hom.coe_mk]; rw [ equiv.perm.extend_domain_apply_not_subtype],
...
by rw [swap_mul_eq_mul_swap]; rw [ perm.inv_apply_self]; rw [ perm.inv_apply_self]

lemma swap_apply_apply (f : perm α) (x y : α) : swap (f x) (f y) = f * swap x y * f⁻¹ :=
by rw [mul_swap_eq_swap_mul]; rw [ mul_inv_cancel_right]
...
by rw [←mul_assoc]; rw [ swap_mul_self]; rw [ one_mul]
...
by rw [mul_assoc]; rw [ swap_mul_self]; rw [ mul_one]
...
⟨(assume h, have swap_id : swap i j = 1 := mul_right_cancel (trans h (one_mul σ).symm),
 by {rw [←swap_apply_right i j]; rw [ swap_id], refl}),
(assume h, by erw [h]; erw [ swap_self]; erw [ one_mul])⟩
...
⟨(assume h, have swap_id : swap i j = 1 := mul_left_cancel (trans h (one_mul σ).symm),
 by {rw [←swap_apply_right i j]; rw [ swap_id], refl}),
(assume h, by erw [h]; erw [ swap_self]; erw [ mul_one])⟩
