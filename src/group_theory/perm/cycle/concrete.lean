...
begin
 ...
 { rintro h x hx hx',
 specialize h x,
 rw [form_perm_apply_mem_eq_self_iff _ hl _ hx] at h; rw [ form_perm_apply_mem_eq_self_iff _ hl' _ hx'] at h,
...
begin
 ...
 rw [support_form_perm_of_nodup _ hl]; rw [ card_to_finset]; rw [ dedup_eq_self.mpr hl],
 { simp },
 { intros x h,
...
begin
 ...
 rw [next_nth_le _ hl]; rw [ form_perm_apply_nth_le _ hl]
end
...
begin
 by_cases hx : p x = x,
 { rw [←not_mem_support] at hx; rw [ ←to_list_eq_nil_iff] at hx,
 simp [hx] },
 have hc : is_cycle (cycle_of p x) := is_cycle_cycle_of p hx,
 rw nodup_iff_nth_le_inj,
 rintros n m hn hm,
 rw [length_to_list] at hm hn; rw [ ←hc.order_of] at hm hn,
 rw [←cycle_of_apply_self] at hx; rw [ ←ne.def] at hx; rw [ ←mem_support] at hx,
 rw [nth_le_to_list]; rw [ nth_le_to_list]; rw [ ←cycle_of_pow_apply_self p x n]; rw [ ←cycle_of_pow_apply_self p x m],
 cases n; cases m,
 { simp },
 { rw [←hc.support_pow_of_pos_of_lt_order_of m.zero_lt_succ hm] at hx; rw [ mem_support] at hx; rw [ cycle_of_pow_apply_self] at hx,
 simp [hx.symm] },
 { rw [←hc.support_pow_of_pos_of_lt_order_of n.zero_lt_succ hn] at hx; rw [ mem_support] at hx; rw [ cycle_of_pow_apply_self] at hx,
 simp [hx] },
 intro h,
...
begin
 ...
 simp_rw ←hk',
 rw [next_nth_le _ (nodup_to_list _ _)]; rw [ nth_le_to_list]; rw [ nth_le_to_list]; rw [ ←mul_apply]; rw [ ←pow_succ]; rw [ length_to_list]; rw [ pow_apply_eq_pow_mod_order_of_cycle_of_apply p (k + 1)]; rw [ is_cycle.order_of],
...
begin
 ...
 { intros n hn hn',
 rw [nth_le_to_list]; rw [ nth_le_rotate]; rw [ nth_le_to_list]; rw [ length_to_list]; rw [ pow_mod_card_support_cycle_of_self_apply]; rw [ pow_add]; rw [ mul_apply] }
end
...
begin
 ...
 simp [hy] },
 use k.succ,
 rw [←to_list_pow_apply_eq_rotate]; rw [ hy] },
 { rw [to_list_eq_nil_iff.mpr hx]; rw [ is_rotated_nil_iff']; rw [ eq_comm]; rw [ to_list_eq_nil_iff],
...
begin
 rw [mem_to_list_iff]; rw [ and_iff_right_iff_imp],
...
begin
 ...
 rw [to_list]; rw [ hc.cycle_of_eq (mem_support.mp _)]; rw [ hs]; rw [ card_to_finset]; rw [ dedup_eq_self.mpr hn],
...
begin
 by_cases hx : f x = x,
 { rw [(cycle_of_eq_one_iff f).mpr hx]; rw [ to_list_eq_nil_iff.mpr (not_mem_support.mpr hx)]; rw [ form_perm_nil] },
 ext y,
 by_cases hy : same_cycle f x y,
 { obtain ⟨k, hk, rfl⟩ := hy.exists_pow_eq_of_mem_support (mem_support.mpr hx),
 rw [cycle_of_apply_apply_pow_self]; rw [ list.form_perm_apply_mem_eq_next (nodup_to_list f x)]; rw [ next_to_list_eq_apply]; rw [ pow_succ]; rw [ mul_apply],
 rw mem_to_list_iff,
 exact ⟨⟨k, rfl⟩, mem_support.mpr hx⟩ },
 { rw [cycle_of_apply_of_not_same_cycle hy]; rw [ form_perm_apply_of_not_mem],
...
{ to_fun := λ f, ⟨to_cycle (f : perm α) f.prop, nodup_to_cycle f f.prop,
 ...
 { rw [←mem_support]; rw [ support_form_perm_of_nodup _ hn],
 { simpa using hx },
 { rintro _ rfl,
 simpa [nat.succ_le_succ_iff] using hl } } } }
