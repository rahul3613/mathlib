...
λ ⟨i, hi⟩, ⟨-i, by rw [zpow_neg]; rw [ ← hi]; rw [ inv_apply_self]⟩
...
λ ⟨i, hi⟩ ⟨j, hj⟩, ⟨j + i, by rw [zpow_add]; rw [ mul_apply]; rw [ hi]; rw [ hj]⟩
...
λ ⟨i, hi⟩, by rw [← hi]; rw [ ← mul_apply]; rw [ ← zpow_one_add]; rw [ add_comm]; rw [ zpow_add_one]; rw [ mul_apply]; rw [ (f ^ i).injective.eq_iff]
...
by rw [same_cycle_comm]; rw [ same_cycle_apply_left]; rw [ same_cycle_comm]

@[simp] lemma same_cycle_inv_apply_left : same_cycle f (f⁻¹ x) y ↔ same_cycle f x y :=
by rw [←same_cycle_apply_left]; rw [ apply_inv_self]

@[simp] lemma same_cycle_inv_apply_right : same_cycle f x (f⁻¹ y) ↔ same_cycle f x y :=
by rw [←same_cycle_apply_right]; rw [ apply_inv_self]
...
by rw [same_cycle_comm]; rw [ same_cycle_zpow_left]; rw [ same_cycle_comm]

@[simp] lemma same_cycle_pow_left {n : ℕ} : same_cycle f ((f ^ n) x) y ↔ same_cycle f x y :=
by rw [←zpow_coe_nat]; rw [ same_cycle_zpow_left]

@[simp] lemma same_cycle_pow_right {n : ℕ} : same_cycle f x ((f ^ n) y) ↔ same_cycle f x y :=
by rw [←zpow_coe_nat]; rw [ same_cycle_zpow_right]
...
exists_congr $ λ n, by rw [←extend_domain_zpow]; rw [ extend_domain_apply_image]; rw [ subtype.coe_inj]; rw [ f.injective.eq_iff]
...
begin
 ...
 rw [←zpow_coe_nat]; rw [ int.nat_abs_of_nonneg h₁]; rw [ ←zpow_eq_mod_order_of],
 refine ⟨_, rfl⟩,
 rw [←int.coe_nat_lt]; rw [ int.nat_abs_of_nonneg h₁],
...
begin
 ...
 rw [pow_order_of_eq_one]; rw [ pow_zero] },
...
⟨λ ⟨n, _, hn⟩, ⟨n, hn⟩, λ ⟨i, hi⟩, ⟨(i % order_of f).nat_abs, list.mem_range.2
 ...
 apply_instance }),
 by { rw [← zpow_coe_nat]; rw [ int.nat_abs_of_nonneg (int.mod_nonneg _ $ int.coe_nat_ne_zero_iff_pos.2 $ order_of_pos _)]; rw [ ← zpow_eq_mod_order_of]; rw [ hi],
 apply_instance }⟩⟩
...
let ⟨a, ha⟩ := hg.2 hx in
let ⟨b, hb⟩ := hg.2 hy in
⟨b - a, by rw [←ha]; rw [ ←mul_apply]; rw [ ←zpow_add]; rw [ sub_add_cancel]; rw [ hb]⟩
...
⟨λ hf y, ⟨λ ⟨i, hi⟩ hy, hx $
 by { rw [← zpow_apply_eq_self_of_apply_eq_self hy i] at hi; rw [ (f ^ i).injective.eq_iff] at hi,
 rw [hi]; rw [ hy] },
...
{ have := n.mod_nonneg (int.coe_nat_ne_zero.mpr (ne_of_gt (order_of_pos f))),
 rwa [← zpow_coe_nat]; rwa [ int.to_nat_of_nonneg this]; rwa [ ← zpow_eq_mod_order_of] }⟩
...
⟨y, by rwa swap_apply_right,
 ...
 else ⟨1, by { rw [zpow_one]; rw [ swap_apply_def], split_ifs at *; cc }⟩⟩
...
begin
 obtain ⟨τ, n, rfl⟩ := τ,
 rw [finset.mem_coe]; rw [ coe_fn_coe_base']; rw [ subtype.coe_mk]; rw [ zpow_apply_mem_support]; rw [ mem_support],
...
begin
 ...
 { rintros ⟨y, hy⟩,
 rw [finset.mem_coe] at hy; rw [ mem_support] at hy,
...
begin
 rw [order_eq_card_zpowers]; rw [ ←fintype.card_coe],
...
| (n+1 : ℕ) := λ b x f hb h,
 ...
 by { rw [mul_apply]; rw [ apply_inv_self]; rw [ swap_apply_of_ne_of_ne this.2 (ne.symm hfbx)]; rw [ ne.def]; rw [ ← f.injective.eq_iff]; rw [ apply_inv_self],
 exact this.1 },
 let ⟨i, hi⟩ := is_cycle_swap_mul_aux₁ n hb'
 (f.injective $ by { rw [apply_inv_self], rwa [pow_succ] at h ; rwa [ mul_apply] at h }) in
 ⟨i + 1, by rw [add_comm]; rw [ zpow_add]; rw [ mul_apply]; rw [ hi]; rw [ zpow_one]; rw [ mul_apply]; rw [ apply_inv_self]; rw [ swap_apply_of_ne_of_ne (ne_and_ne_of_swap_mul_apply_ne_self hb).2 (ne.symm hfbx)]⟩
...
| -[1+ n] := λ b x f hb h,
 ...
 have hb : (swap x (f⁻¹ x) * f⁻¹) (f⁻¹ b) ≠ f⁻¹ b,
 by { rw [mul_apply]; rw [ swap_apply_def],
 split_ifs;
 ...
 let ⟨i, hi⟩ := is_cycle_swap_mul_aux₁ n hb
 (show (f⁻¹ ^ n) (f⁻¹ x) = f⁻¹ b, by
 rw [← zpow_coe_nat]; rw [ ← h]; rw [ ← mul_apply]; rw [ ← mul_apply]; rw [ ← mul_apply]; rw [ zpow_neg_succ_of_nat]; rw [ ← inv_pow]; rw [ pow_succ']; rw [ mul_assoc]; rw [ mul_assoc]; rw [ inv_mul_self]; rw [ mul_one]; rw [ zpow_coe_nat]; rw [ ← pow_succ']; rw [ ← pow_succ]) in
 have h : (swap x (f⁻¹ x) * f⁻¹) (f x) = f⁻¹ x, by rw [mul_apply]; rw [ inv_apply_self]; rw [ swap_apply_left],
 ⟨-i, by rw [← add_sub_cancel i 1]; rw [ neg_sub]; rw [ sub_eq_add_neg]; rw [ zpow_add]; rw [ zpow_one]; rw [ zpow_neg]; rw [ ← inv_zpow]; rw [ mul_inv_rev]; rw [ swap_inv]; rw [ mul_swap_eq_swap_mul]; rw [ inv_apply_self]; rw [ swap_comm _ x]; rw [ zpow_add]; rw [ zpow_one]; rw [ mul_apply]; rw [ mul_apply (_ ^ i)]; rw [ h]; rw [ hi]; rw [ mul_apply]; rw [ apply_inv_self]; rw [ swap_apply_of_ne_of_ne this.2 (ne.symm hfbx')]⟩
...
if hyx : y = x then by simp [hyx]
 ...
 { rw [← hj] at hyx; rw [ hji] at hyx, cc },
 { rw [← hj] at hfyx; rw [ hji] at hfyx, cc }
end
...
⟨f x, by { simp [swap_apply_def, mul_apply, if_neg hffx, f.injective.eq_iff, if_neg hx, hx], },
 ...
 calc (f ^ (i - 1)) (f x) = (f ^ (i - 1) * f ^ (1 : ℤ)) x : by rw [zpow_one]; rw [ mul_apply]
 ... = y : by rwa [← zpow_add]; rwa [ sub_add_cancel],
...
calc sign f = sign (swap x (f x) * (swap x (f x) * f)) :
 by rw [← mul_assoc]; rw [ mul_def]; rw [ mul_def]; rw [ swap_swap]; rw [ trans_refl]
... = -(-1) ^ f.support.card :
 if h1 : f (f x) = x
 then
 ...
 by { rw [sign_mul]; rw [ sign_swap hx.1.symm]; rw [ h]; rw [ sign_one]; rw [ hf.eq_swap_of_apply_apply_eq_self hx.1 h1]; rw [ card_support_swap hx.1.symm], refl }
 else
 have h : card (support (swap x (f x) * f)) + 1 = card (support f),
 by rw [← insert_erase (mem_support.2 hx.1)]; rw [ support_swap_mul_eq _ _ h1]; rw [ card_insert_of_not_mem (not_mem_erase _ _)]; rw [ sdiff_singleton_eq_erase],
 have wf : card (support (swap x (f x) * f)) < card (support f),
 from card_support_swap_mul hx.1,
 by { rw [sign_mul]; rw [ sign_swap hx.1.symm]; rw [ (hf.swap_mul hx.1 h1).sign]; rw [ ← h],
...
begin
 ...
 rwa [←hm]; rwa [ ←pow_eq_on_of_mem_support h'' _ x (mem_inter_of_mem (mem_support.mpr hx) (mem_support.mpr hx'))]; rwa [ pow_apply_mem_support]; rwa [ mem_support] },
...
begin
 have hx'' : x ∈ g.support,
 { rwa [mem_support]; rwa [ ←hx]; rwa [ ←mem_support] },
 have : f.support ⊆ g.support,
 { intros y hy,
 obtain ⟨k, rfl⟩ := hf.exists_pow_eq (mem_support.mp hx') (mem_support.mp hy),
 rwa [pow_eq_on_of_mem_support h _ _ (mem_inter_of_mem hx' hx'')]; rwa [ pow_apply_mem_support] },
...
begin
 ...
 ext z,
 by_cases hz : f z = z,
 { rw [pow_apply_eq_self_of_apply_eq_self hz]; rw [ one_apply] },
...
begin
 ...
 { rw [h.order_of]; rw [ hr]; rw [ hf.order_of] },
 rw [order_of_pow] at this; rw [ nat.div_eq_self] at this,
 cases this,
 { exact absurd this (order_of_pos _).ne' },
 { rwa [nat.coprime_iff_gcd_eq_one]; rwa [ nat.gcd_comm] } },
 { intro h,
...
begin
 ...
 { rw [← mem_support] at hx; rw [ ← h] at hx; rw [ mem_support] at hx,
 contradiction },
 { rw [hf.support_pow_eq_iff] at h; rw [ not_not] at h,
 obtain ⟨k, rfl⟩ := h,
 rw [pow_mul]; rw [ pow_order_of_eq_one]; rw [ one_pow] } }
end
...
begin
 ...
 rw [not_mem_support] at hfa; rw [ h] at hfa; rw [ function.injective.eq_iff (f ^ a).injective] at hfa,
 contradiction }}
end
...
⟨bij_on_swap (by simp) (by simp), λ x hx y hy, begin
 ...
 { exact ⟨0, by rw [zpow_zero]; rw [ coe_one]; rw [ id.def]⟩ },
 { exact ⟨1, by rw [zpow_one]; rw [ swap_apply_left]⟩ },
 { exact ⟨1, by rw [zpow_one]; rw [ swap_apply_right]⟩ },
 { exact ⟨0, by rw [zpow_zero]; rw [ coe_one]; rw [ id.def]⟩ }
end⟩
...
begin
 ...
 { rw [coe_singleton] at hf; rw [ is_cycle_on_singleton] at hf,
 ...
 rw [←this]; rw [ order_of_dvd_iff_pow_eq_one]; rw [ (hf.is_cycle_subtype_perm hs).pow_eq_one_iff' (ne_of_apply_ne (coe : s → α) $ hf.apply_ne hs (⟨a, ha⟩ : s).prop)],
...
| (int.neg_succ_of_nat n) := by { rw [zpow_neg_succ_of_nat]; rw [ ←inv_pow],
...
begin
 ...
 rw [←zpow_coe_nat]; rw [ int.nat_mod]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ $ nat.cast_ne_zero.2 (nonempty.card_pos ⟨a, ha⟩).ne')]; rw [ sub_eq_iff_eq_add'.1 hk]; rw [ zpow_add]; rw [ zpow_mul],
...
equiv.ext $ λ y, begin
 rw [inv_eq_iff_eq]; rw [ cycle_of_apply]; rw [ cycle_of_apply],
...
| 0 := rfl
| (n+1) := by { rw [pow_succ]; rw [ mul_apply]; rw [ cycle_of_apply]; rw [ cycle_of_pow_apply_self]; rw [ if_pos]; rw [ pow_succ]; rw [ mul_apply],
 exact ⟨n, rfl⟩ }

@[simp] lemma cycle_of_zpow_apply_self (f : perm α) (x : α) :
 ∀ n : ℤ, (cycle_of f x ^ n) x = (f ^ n) x
| (n : ℕ) := cycle_of_pow_apply_self f x n
| -[1+ n] := by rw [zpow_neg_succ_of_nat]; rw [ ← inv_pow]; rw [ cycle_of_inv]; rw [ zpow_neg_succ_of_nat]; rw [ ← inv_pow]; rw [ cycle_of_pow_apply_self]
...
begin
 ...
 { rw [add_comm]; rw [ zpow_add]; rw [ zpow_one]; rw [ mul_apply] },
 { exact ⟨k, rfl⟩ }
end
...
equiv.ext $ λ y,
 ...
 by rw [cycle_of_apply_of_not_same_cycle h]; rw [ not_not.1 (mt ((is_cycle_iff_same_cycle hx).1 hf).2 h)]
...
begin
 by_cases hx : f x = x,
 { rwa [if_pos hx]; rwa [ cycle_of_eq_one_iff] },
 { rwa [if_neg hx]; rwa [ hf.cycle_of_eq] },
end
...
⟨λ h, mt h.apply_eq_self_iff.2 this,
 ...
 let ⟨i, hi⟩ := hxy in
 ⟨i, by rw [cycle_of_zpow_apply_self]; rw [ hi]⟩
...
begin
 rw [←two_le_card_support_cycle_of_iff]; rw [ ←nat.succ_le_iff],
...
by rw [←cycle_of_pow_apply_self f]; rw [ ←cycle_of_pow_apply_self f]; rw [ pow_eq_mod_order_of]
...
begin
 ...
 { rw [cycle_of_apply_of_not_same_cycle hxy]; rw [ cycle_of_apply_of_not_same_cycle],
 contrapose! hxy,
 obtain ⟨z, rfl⟩ := hxy,
 refine ⟨z, _⟩,
 simp [h.mul_zpow, zpow_apply_eq_self_of_apply_eq_self hx] }
...
begin
 intros y hy,
 rw [mem_support] at hy; rw [ cycle_of_apply] at hy,
 split_ifs at hy,
...
begin
 ...
 simp [hx] },
 { rw [mem_support]; rw [ cycle_of_apply],
 split_ifs with hy,
...
by rw [mem_support_cycle_of_iff]; rw [ and_iff_left (mem_support.2 hx)]
...
begin
 by_cases hx : f x = x,
 { rw [pow_apply_eq_self_of_apply_eq_self hx]; rw [ pow_apply_eq_self_of_apply_eq_self hx] },
 { rw [←cycle_of_pow_apply_self]; rw [ ←cycle_of_pow_apply_self f]; rw [ ←(is_cycle_cycle_of f hx).order_of]; rw [ ←pow_eq_mod_order_of] }
end
...
begin
 ...
 rw [ne.def]; rw [ ←cycle_of_eq_one_iff f],
 exact hx.ne_one,
end
...
⟨f.bij_on $ by simp [mem_support_cycle_of_iff], λ a ha b hb,
 by { rw [mem_coe] at ha hb; rw [ mem_support_cycle_of_iff] at ha hb, exact ha.1.symm.trans hb.1 }⟩
...
begin
 ...
 subst hk',
 rw [←(is_cycle_cycle_of _ $ mem_support.1 hx).order_of]; rw [ ←cycle_of_pow_apply_self]; rw [ pow_order_of_eq_one]; rw [ one_apply] } },
...
if hx : f x = x then
 ...
 (λ h : y = x,
 by { rw [h] at hy; rw [ mul_apply] at hy; rw [ ne.def] at hy; rw [ inv_eq_iff_eq] at hy; rw [ cycle_of_apply_self] at hy, exact hy rfl })
 (h (λ h : f y = y, by { rw [mul_apply] at hy; rw [ h] at hy; rw [ ne.def] at hy; rw [ inv_eq_iff_eq] at hy; rw [ cycle_of_apply] at hy,
 split_ifs at hy; cc }))) in
 ⟨(cycle_of f x) :: m, by { rw [list.prod_cons]; rw [ hm₁], simp },
 ...
 have hsc : same_cycle f⁻¹ x (f y), by rwa [same_cycle_inv]; rwa [ same_cycle_apply_right],
 by { rw [disjoint_prod_perm hm₃ hgm.symm] at hm₁; rw [ list.prod_cons] at hm₁; rw [ ← eq_inv_mul_iff_mul_eq] at hm₁,
 rwa [hm₁]; rwa [ mul_apply]; rwa [ mul_apply]; rwa [ cycle_of_inv]; rwa [ hsc.cycle_of_apply]; rwa [ inv_apply_self]; rwa [ inv_eq_iff_eq]; rwa [ eq_comm] }),
 hm₃⟩⟩
...
begin
 ...
 { rw [cycle_factors_finset]; rw [ ←hl]; rw [ trunc.lift_mk] },
 rw ht,
 split,
 { intro h,
...
begin
 ...
 rw ←hl',
 rw [eq_comm] at hl'; rw [ cycle_factors_finset_eq_list_to_finset hl] at hl',
...
begin
 ...
 { rintro ⟨hc, h⟩,
 contrapose! hc,
 rw [not_mem_support] at hc; rw [ ←cycle_of_eq_one_iff] at hc,
 simp [hc] },
 { intros hx,
...
begin
 ...
 intros x hx,
 rwa [mem_support]; rwa [ ←h.right x hx]; rwa [ ←mem_support]
end
...
begin
 ...
 { rw [coe_union]; rw [ set.pairwise_union_of_symmetric disjoint.symmetric],
 ...
 rw [cycle_factors_finset_noncomm_prod]; rw [ cycle_factors_finset_noncomm_prod] }
end
...
begin
 ...
 rwa [←support_inv]; rwa [ apply_mem_support]; rwa [ support_inv]; rwa [ mem_support] }
end
...
begin
 ...
 rintro (hf | hf),
 { rw [hd.commute.eq]; rw [ union_comm]; rw [ union_sdiff_distrib]; rw [ sdiff_singleton_eq_erase]; rw [ erase_eq_of_not_mem]; rw [ mul_assoc]; rw [ disjoint.cycle_factors_finset_mul_eq_union]; rw [ hσ hf],
 ...
 { rw [union_sdiff_distrib]; rw [ sdiff_singleton_eq_erase]; rw [ erase_eq_of_not_mem]; rw [ mul_assoc]; rw [ disjoint.cycle_factors_finset_mul_eq_union]; rw [ hτ hf],
 { rw mem_cycle_factors_finset_iff at hf,
 intro x,
...
begin
 ...
 exact swap_self x },
 { by_cases h5 : x = (σ ^ n) x,
 { rw [pow_succ]; rw [ mul_apply]; rw [ ←h5], exact h4 },
 by_cases h6 : x = (σ^(n+1)) x,
 { rw [←h6]; rw [ swap_self], exact H.one_mem },
...
begin
 rw [←finset.card_univ] at h0; rw [ ←h2] at h0; rw [ ←h1.order_of] at h0,
 ...
 rw [eq_top_iff]; rw [ ←closure_cycle_adjacent_swap h1' h2' x]; rw [ closure_le]; rw [ set.insert_subset],
...
begin
 ...
 cases h with m hm,
 rwa [hm] at hi; rwa [ pow_mul] at hi; rwa [ ←finset.card_univ] at hi; rwa [ ←h2] at hi; rwa [ ←h1.order_of] at hi; rwa [ pow_order_of_eq_one] at hi; rwa [ one_pow] at hi; rwa [ one_apply] at hi,
end
...
begin
 ...
 { rw [equiv.extend_subtype_apply_of_mem]; rw [ equiv.extend_subtype_apply_of_mem],
 { exact hf x (finset.mem_coe.2 hx) } },
 { rwa [not_not.1 ((not_congr mem_support).1 (equiv.extend_subtype_not_mem f _ _))]; rwa [ not_not.1 ((not_congr mem_support).mp hx)] }
end
...
begin
 ...
 (zpowers_equiv_zpowers $ by rw [hσ.order_of]; rw [ h]; rw [ hτ.order_of]).trans hτ.zpowers_equiv_support) _,
 intros x hx,
...
⟨begin
 ...
 contrapose! hb,
 rw [mem_support] at hb; rw [ not_not] at hb,
 rw [mem_support]; rw [ not_not]; rw [ perm.mul_apply]; rw [ perm.mul_apply]; rw [ hb]; rw [ perm.apply_inv_self] }
end, hσ.is_conj hτ⟩
...
begin
 ...
 { rw [mem_coe] at hxσ; rw [ mem_support] at hxσ,
 ...
 { rw [mul_apply] at h; rw [ mul_apply] at h,
 rw [h]; rw [ inv_apply_self]; rw [ (hd1 x).resolve_left hxσ] },
 { rwa [mul_apply]; rwa [ mul_apply]; rwa [ inv_apply_self]; rwa [ apply_eq_iff_eq] } },
 { rwa [subtype.coe_mk]; rwa [ subtype.coe_mk]; rwa [ mem_coe]; rwa [ mem_support] },
 { rwa [subtype.coe_mk]; rwa [ subtype.coe_mk]; rwa [ perm.mul_apply]; rwa [ (hd1 x).resolve_left hxσ]; rwa [ mem_coe]; rwa [ apply_mem_support]; rwa [ mem_support] } },
 { rw [mem_coe] at hxτ; rw [ ← apply_mem_support] at hxτ; rw [ mem_support] at hxτ,
 ...
 { rwa [mul_apply]; rwa [ mul_apply]; rwa [ inv_apply_self]; rwa [ apply_eq_iff_eq] } },
 { rwa [subtype.coe_mk]; rwa [ subtype.coe_mk]; rwa [ mem_coe]; rwa [ ← apply_mem_support]; rwa [ mem_support] },
 { rwa [subtype.coe_mk]; rwa [ subtype.coe_mk]; rwa [ perm.mul_apply]; rwa [ (hd1 (τ x)).resolve_right hxτ]; rwa [ mem_coe]; rwa [ mem_support] } } }
end
...
begin
 ...
 rw [set.mem_set_of] at ha hb; rw [ ←index_of_lt_length] at ha hb,
 rw [←index_of_nth_le ha]; rw [ ←index_of_nth_le hb],
...
begin
 ...
 rw [image_comp]; rw [ equiv.image_eq_preimage],
 ext,
 simp,
end
