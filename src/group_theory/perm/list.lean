...
begin
 ...
 { simpa using h },
 { by_cases hx : x ∈ z :: l,
 { rw [form_perm_cons_cons]; rw [ mul_apply]; rw [ swap_apply_def],
 split_ifs;
 simp [IH _ _ hx] },
...
begin
 ...
 { rw [list.form_perm_cons_cons] at h; rw [ mul_apply] at h; rw [ swap_apply_def] at h,
 split_ifs at h;
 simp [IH _ _ hx] },
...
by rw [nth_le_cons_length]; rw [ form_perm_apply_last]; refl
...
begin
 ...
 rw [IH]; rw [ swap_apply_of_ne_of_ne]; rw [ nth_le];
 { rintro rfl,
...
begin
 ...
 rw [set.mem_set_of_eq]; rw [ form_perm_apply_nth_le _ h],
 intro H,
...
begin
 ...
 rw [form_perm_apply_nth_le _ h']; rw [ nth_le_rotate l]; rw [ nth_le_rotate l]; rw [ form_perm_apply_nth_le _ h],
 simp },
 { rw [form_perm_apply_of_not_mem _ _ hx]; rw [ form_perm_apply_of_not_mem],
 simpa using hx }
end
...
begin
 ...
 { simp },
 { rw [nat.succ_eq_add_one]; rw [ ←rotate_rotate]; rw [ form_perm_rotate_one]; rw [ hn],
...
begin
 ...
 by_cases hx : x ∈ l,
 swap,
 { rw [form_perm_apply_of_not_mem x l.reverse]; rw [ form_perm_apply_of_not_mem _ _ hx],
 simpa using hx },
 { obtain ⟨k, hk, rfl⟩ := nth_le_of_mem (mem_reverse.mpr hx),
 rw [form_perm_apply_nth_le l.reverse (nodup_reverse.mpr h)]; rw [ nth_le_reverse']; rw [ form_perm_apply_nth_le _ h]; rw [ nth_le_reverse'],
 { congr,
 rw [length_reverse] at hk; rw [ ←nat.succ_le_iff] at hk; rw [ nat.succ_eq_add_one] at hk,
 cases hk.eq_or_lt with hk' hk',
 { simp [←hk'] },
 { rw [length_reverse]; rw [ nat.mod_eq_of_lt hk']; rw [ tsub_add_eq_add_tsub (nat.le_pred_of_lt hk')]; rw [ nat.mod_eq_of_lt],
 { simp },
 { rw tsub_add_cancel_of_le,
 refine tsub_lt_self _ (nat.zero_lt_succ _),
 all_goals { simpa using (nat.zero_le _).trans_lt hk' } } } },
 all_goals { rw [← tsub_add_eq_tsub_tsub]; rw [ ←length_reverse],
...
begin
 ...
 { rw [set.mem_set_of_eq]; rw [ h x']; rw [ form_perm_apply_head _ _ _ hd'],
 ...
 { rw [←dedup_eq_self.mpr hd]; rw [ ←dedup_eq_self.mpr hd']; rw [ ←card_to_finset]; rw [ ←card_to_finset],
 refine congr_arg finset.card _,
 rw [←finset.coe_inj]; rw [ ←support_form_perm_of_nodup' _ hd (by simp)]; rw [ ←support_form_perm_of_nodup' _ hd' (by simp)],
 simp only [h] },
 use n,
 apply list.ext_le,
 { rw [length_rotate]; rw [ hl] },
 { intros k hk hk',
 rw nth_le_rotate,
 ...
 simp_rw this,
 rw [←form_perm_apply_nth_le _ hd' k (k.lt_succ_self.trans hk')]; rw [ ←IH (k.lt_succ_self.trans hk)]; rw [ ←h]; rw [ form_perm_apply_nth_le _ hd],
 congr' 1,
...
begin
 obtain ⟨k, hk, rfl⟩ := nth_le_of_mem hx,
 rw [form_perm_apply_nth_le _ hl]; rw [ hl.nth_le_inj_iff],
 cases hn : l.length,
...
begin
 rw [ne.def]; rw [ form_perm_apply_mem_eq_self_iff _ hl x hx]; rw [ not_le],
