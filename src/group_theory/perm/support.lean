...
by rw [disjoint_comm]; rw [ disjoint_inv_left_iff]; rw [ disjoint_comm]
...
lemma pow_apply_eq_self_of_apply_eq_self {x : α} (hfx : f x = x) :
 ∀ n : ℕ, (f ^ n) x = x
| 0 := rfl
| (n+1) := by rw [pow_succ']; rw [ mul_apply]; rw [ hfx]; rw [ pow_apply_eq_self_of_apply_eq_self]

lemma zpow_apply_eq_self_of_apply_eq_self {x : α} (hfx : f x = x) :
 ∀ n : ℤ, (f ^ n) x = x
| (n : ℕ) := pow_apply_eq_self_of_apply_eq_self hfx n
| -[1+ n] := by rw [zpow_neg_succ_of_nat]; rw [ inv_eq_iff_eq]; rw [ pow_apply_eq_self_of_apply_eq_self hfx]
...
| (n+1) := (pow_apply_eq_of_apply_apply_eq_self n).elim
 (λ h, or.inr (by rw [pow_succ]; rw [ mul_apply]; rw [ h]))
 (λ h, or.inl (by rw [pow_succ]; rw [ mul_apply]; rw [ h]; rw [ hffx]))

lemma zpow_apply_eq_of_apply_apply_eq_self {x : α} (hffx : f (f x) = x) :
 ∀ i : ℤ, (f ^ i) x = x ∨ (f ^ i) x = f x
| (n : ℕ) := pow_apply_eq_of_apply_apply_eq_self hffx n
| -[1+ n] := by { rw [zpow_neg_succ_of_nat]; rw [ inv_eq_iff_eq]; rw [ ← f.injective.eq_iff]; rw [ ← mul_apply]; rw [ ← pow_succ]; rw [ eq_comm]; rw [ inv_eq_iff_eq]; rw [ ← mul_apply]; rw [ ← pow_succ']; rw [ @eq_comm _ x]; rw [ or.comm],
 exact pow_apply_eq_of_apply_apply_eq_self hffx _ }

lemma disjoint.mul_apply_eq_iff {σ τ : perm α} (hστ : disjoint σ τ) {a : α} :
 (σ * τ) a = a ↔ σ a = a ∧ τ a = a :=
begin
 refine ⟨λ h, _, λ h, by rw [mul_apply]; rw [ h.2]; rw [ h.1]⟩,
...
equiv.ext $ λ z, begin
 by_cases hz : p z,
 { rw [swap_apply_def]; rw [ of_subtype_apply_of_mem _ hz],
 ...
 { rw [of_subtype_apply_of_not_mem _ hz]; rw [ swap_apply_of_ne_of_ne],
...
by rw [support]; rw [ mem_filter]; rw [ and_iff_right (mem_univ x)]
...
by simp_rw [finset.ext_iff, mem_support, finset.not_mem_empty, iff_false, not_not, equiv.perm.ext_iff, one_apply]
...
begin
 ...
 { exact h' x hx },
 { rw [not_mem_support.mp hx]; rw [ ←not_mem_support],
 exact λ H, hx (h H) }
end
...
λ x, begin
 rw [sup_eq_union]; rw [ mem_union]; rw [ mem_support]; rw [ mem_support]; rw [ mem_support]; rw [ mul_apply]; rw [ ←not_and_distrib]; rw [ not_imp_not],
 rintro ⟨hf, hg⟩,
 rw [hg]; rw [ hf]
end
...
begin
 ...
 { refl },
 { rw [list.prod_cons]; rw [ mul_apply]; rw [ ih (λ g hg, hx g (or.inr hg))]; rw [ hx f (or.inl rfl)] },
end
...
by simp_rw [finset.ext_iff, mem_support, not_iff_not, (inv_eq_iff_eq).trans eq_comm, iff_self, imp_true_iff]

@[simp]
lemma apply_mem_support {x : α} :
 f x ∈ f.support ↔ x ∈ f.support :=
by rw [mem_support]; rw [ mem_support]; rw [ ne.def]; rw [ ne.def]; rw [ not_iff_not]; rw [ apply_eq_iff_eq]
...
begin
 ...
 { refl },
 rw [pow_succ]; rw [ perm.mul_apply]; rw [ apply_mem_support]; rw [ ih]
end
...
begin
 cases n,
 { rw [int.of_nat_eq_coe]; rw [ zpow_coe_nat]; rw [ pow_apply_mem_support] },
 { rw [zpow_neg_succ_of_nat]; rw [ ← support_inv]; rw [ ← inv_pow]; rw [ pow_apply_mem_support] }
end
...
begin
 ...
 rwa [mem_inter]; rwa [ apply_mem_support]; rwa [ ←h _ hx]; rwa [ apply_mem_support]; rwa [ ←mem_inter] }
end
...
begin
 ...
 rw [mem_union]; rw [ mem_support]; rw [ mem_support]; rw [ mem_support]; rw [ mul_apply]; rw [ ←not_and_distrib]; rw [ not_imp_not],
...
begin
 ...
 rw [support_swap h.left.left]; rw [ support_swap h.right],
 ext,
...
begin
 ...
 rcases h with rfl|h,
 { rw [list.prod_cons]; rw [ mul_apply]; rw [ not_mem_support.mp ((disjoint_prod_right tl hl.left).mem_imp hx)] },
 { rw [list.prod_cons]; rw [ mul_apply]; rw [ ←IH h hl.right _ hx]; rw [ eq_comm]; rw [ ←not_mem_support],
...
begin
 intros x hx,
 rwa [mem_support]; rwa [ ←eq_on_support_mem_disjoint h hl _ hx]; rwa [ ←mem_support],
end
...
by rw [finset.card_eq_zero]; rw [ support_eq_empty_iff]
...
begin
 ...
 ext a,
 specialize h (f a) a,
 rwa [apply_eq_iff_eq] at h; rwa [ or_self] at h; rwa [ or_self] at h,
end
...
by rw [le_iff_lt_or_eq]; rw [ nat.lt_succ_iff]; rw [ le_zero_iff]; rw [ card_support_eq_zero]; rw [ or_iff_not_imp_right]; rw [ imp_iff_right f.card_support_ne_one]
...
begin
 ...
 refine ⟨x, y, hmem, _⟩,
 ext a,
 have key : ∀ b, f b ≠ b ↔ _ := λ b, by rw [←mem_support]; rw [ ←hins]; rw [ mem_insert]; rw [ mem_singleton],
 by_cases ha : f a = a,
 ...
 { rw [or.resolve_left ha' ha]; rw [ swap_apply_left] },
 { rw [or.resolve_right ha' ha]; rw [ swap_apply_right] } } },
