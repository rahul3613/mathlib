...
by rw [← fin.succ_zero_eq_one]; rw [ equiv.perm.decompose_fin_symm_apply_succ e p 0]
...
begin
 ...
 { simp },
 rw [coe_fin_rotate]; rw [ decompose_fin_symm_apply_succ]; rw [ if_congr (i.succ_eq_last_succ) rfl rfl],
 split_ifs with h,
 { simp [h] },
 { rw [fin.coe_succ]; rw [ function.injective.map_swap fin.coe_injective]; rw [ fin.coe_succ]; rw [ coe_fin_rotate]; rw [ if_neg h]; rw [ fin.coe_zero]; rw [ fin.coe_one]; rw [ swap_apply_of_ne_of_ne (nat.succ_ne_zero _) (nat.succ_succ_ne_one _)] }
end
...
begin
 ...
 rw [add_comm]; rw [ support_fin_rotate],
end
...
begin
 ...
 clear hx',
 cases x with x hx,
 rw [coe_coe]; rw [ zpow_coe_nat]; rw [ fin.ext_iff]; rw [ fin.coe_mk],
 induction x with x ih, { refl },
 rw [pow_succ]; rw [ perm.mul_apply]; rw [ coe_fin_rotate_of_ne_last]; rw [ ih (lt_trans x.lt_succ_self hx)],
 rw [ne.def]; rw [ fin.ext_iff]; rw [ ih (lt_trans x.lt_succ_self hx)]; rw [ fin.coe_last],
...
begin
 rw [is_cycle_fin_rotate.cycle_type]; rw [ support_fin_rotate]; rw [ ← fintype.card]; rw [ fintype.card_fin],
 refl,
end
...
begin
 obtain ⟨m, rfl⟩ := exists_add_of_le h,
 rw [add_comm]; rw [ cycle_type_fin_rotate]
end
...
lemma cycle_range_of_gt {n : ℕ} {i j : fin n.succ} (h : i < j) :
 cycle_range i j = j :=
begin
 rw [cycle_range]; rw [ of_left_inverse'_eq_of_injective]; rw [ ←function.embedding.to_equiv_range_eq_of_injective]; rw [ ←via_fintype_embedding]; rw [ via_fintype_embedding_apply_not_mem_range],
 simpa
end
...
begin
 ...
 ext,
 rw [this]; rw [ cycle_range]; rw [ of_left_inverse'_eq_of_injective]; rw [ ←function.embedding.to_equiv_range_eq_of_injective]; rw [ ←via_fintype_embedding]; rw [ via_fintype_embedding_apply_image]; rw [ rel_embedding.coe_fn_to_embedding]; rw [ coe_cast_le]; rw [ coe_fin_rotate],
...
lemma cycle_range_of_lt {n : ℕ} {i j : fin n.succ} (h : j < i) :
 cycle_range i j = j + 1 :=
by rw [cycle_range_of_le h.le]; rw [ if_neg h.ne]

lemma coe_cycle_range_of_lt {n : ℕ} {i j : fin n.succ} (h : j < i) :
 (cycle_range i j : ℕ) = j + 1 :=
by rw [coe_cycle_range_of_le h.le]; rw [ if_neg h.ne]

lemma cycle_range_of_eq {n : ℕ} {i j : fin n.succ} (h : j = i) :
 cycle_range i j = 0 :=
by rw [cycle_range_of_le h.le]; rw [ if_pos h]
...
begin
 ...
 { simp },
 { rw [cycle_range_of_gt (fin.succ_pos j)]; rw [ one_apply] },
end

@[simp] lemma cycle_range_last (n : ℕ) : cycle_range (last n) = fin_rotate (n + 1) :=
by { ext i, rw [coe_cycle_range_of_le (le_last _)]; rw [ coe_fin_rotate] }
...
begin
 ...
 { ext, rw [coe_cast_succ]; rw [ coe_succ]; rw [ fin.coe_add_one_of_lt (lt_of_lt_of_le hlt i.le_last)] },
 rw [fin.cycle_range_of_lt hlt]; rw [ fin.succ_above_below]; rw [ this]; rw [ swap_apply_of_ne_of_ne],
 { apply fin.succ_ne_zero },
 { exact (fin.succ_injective _).ne hlt.ne },
 { rw fin.lt_iff_coe_lt_coe,
 simpa [this] using hlt } },
 { rw [heq]; rw [ fin.cycle_range_self]; rw [ fin.succ_above_below]; rw [ swap_apply_right]; rw [ fin.cast_succ_zero],
 { rw fin.cast_succ_zero, apply fin.succ_pos } },
 { rw [fin.cycle_range_of_gt hgt]; rw [ fin.succ_above_above]; rw [ swap_apply_of_ne_of_ne],
 { apply fin.succ_ne_zero },
...
begin
 ...
 { rw [fin.succ_above_below _ _ h]; rw [ fin.cycle_range_of_lt h]; rw [ fin.coe_succ_eq_succ] },
 { rw [fin.succ_above_above _ _ h]; rw [ fin.cycle_range_of_gt (fin.le_cast_succ_iff.mp h)] }
end
...
begin
 ...
 cases i,
 { exact (h0 rfl).elim },
 rw [cycle_range]; rw [ cycle_type_extend_domain],
...
begin
 rw [is_three_cycle]; rw [ cycle_type_cycle_range];
 dec_trivial
end
