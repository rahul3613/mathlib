...
le_antisymm
 ...
 (by conv_rhs { rw [← closure_eq H]; rw [ ← closure_eq K] }; apply closure_mul_le)
...
begin
 ...
 rw [pow_add]; rw [ smul_mul_assoc]; rw [ mul_smul]; rw [ mul_comm]; rw [ ← smul_mul_assoc]; rw [ mul_comm] }
end
...
begin
 apply le_antisymm,
 { rw [closure_le]; rw [ coe_inv]; rw [ ←set.inv_subset]; rw [ inv_inv],
 exact subset_closure },
 { rw [inv_le]; rw [ closure_le]; rw [ coe_inv]; rw [ ←set.inv_subset],
 exact subset_closure }
end
...
by rw [←closure_mul_closure]; rw [ closure_eq]; rw [ closure_eq]

@[simp] theorem mul_bot (S : add_submonoid R) : S * ⊥ = ⊥ :=
eq_bot_iff.2 $ mul_le.2 $ λ m hm n hn, by rw [add_submonoid.mem_bot] at hn ⊢; rw [hn]; rw [ mul_zero]

@[simp] theorem bot_mul (S : add_submonoid R) : ⊥ * S = ⊥ :=
eq_bot_iff.2 $ mul_le.2 $ λ m hm n hn, by rw [add_submonoid.mem_bot] at hm ⊢; rw [hm]; rw [ zero_mul]
...
{ one := 1,
 mul := (*),
 one_mul := λ M, by rw [one_eq_closure_one_set]; rw [ ←closure_eq M]; rw [ closure_mul_closure]; rw [ one_mul],
 mul_one := λ M, by rw [one_eq_closure_one_set]; rw [ ←closure_eq M]; rw [ closure_mul_closure]; rw [ mul_one] }
...
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ one_eq_closure_one_set]
| (n + 1) := by rw [pow_succ]; rw [ pow_succ]; rw [ closure_pow]; rw [ closure_mul_closure]

lemma pow_eq_closure_pow_set (s : add_submonoid R) (n : ℕ) : s ^ n = closure ((s : set R) ^ n) :=
by rw [←closure_pow]; rw [ closure_eq]
