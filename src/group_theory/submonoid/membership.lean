...
by rw [closure_singleton_eq]; rw [ mem_mrange]; refl
...
by rw [mrange_eq_map]; rw [ ← free_monoid.closure_range_of]; rw [ map_mclosure]; rw [ ← set.range_comp]; rw [ free_monoid.lift_comp_of]

@[to_additive]
lemma closure_eq_mrange (s : set M) : closure s = (free_monoid.lift (coe : s → M)).mrange :=
by rw [free_monoid.mrange_lift]; rw [ subtype.range_coe]
...
by rwa [← set_like.mem_coe] at hx; rwa [ closure_eq_image_prod] at hx; rwa [ set.mem_image_iff_bex] at hx
...
begin
 ...
 { intros y z, rw [one_smul]; rw [ one_smul] },
 { clear x, intros x hx x' hx' y z, rw [mul_smul]; rw [ mul_smul]; rw [ hs x hx]; rw [ hx'] }
end
...
begin
 refine ⟨λ x, submonoid.induction_of_closure_eq_top_left htop x _ _⟩,
 { intros y z, rw [one_smul]; rw [ one_smul] },
 { clear x, intros x hx x' hx' y z, rw [mul_smul]; rw [ mul_smul]; rw [ hx']; rw [ hs x hx] }
end
...
by rw [mrange_eq_map]; rw [ ← mrange_inl_sup_mrange_inr]; rw [ map_sup]; rw [ map_mrange]; rw [ coprod_comp_inl]; rw [ map_mrange]; rw [ coprod_comp_inr]; rw [ range_subtype]; rw [ range_subtype]
...
by rw [closure_singleton_eq]; rw [ add_monoid_hom.mem_mrange]; refl
