...
{ carrier := { x : M | ∃ y : S, x * y = 1 },
 ...
 ⟨b' * a', by rw [coe_mul]; rw [ ← mul_assoc]; rw [ mul_assoc a]; rw [ hb]; rw [ mul_one]; rw [ ha]⟩ }
...
by rw [mul_comm]; rw [ mul_from_left_inv]
...
by rw [← is_unit.mul_right_inj (left_inv_le_is_unit _ a.prop)]; rw [ S.mul_from_left_inv]; rw [ eq_comm]
...
{ to_fun := S.from_left_inv,
 ...
 by rw [from_left_inv_eq_iff]; rw [ mul_comm x]; rw [ submonoid.coe_mul]; rw [ submonoid.coe_mul]; rw [ mul_assoc]; rw [ ← mul_assoc (x : M)]; rw [ mul_from_left_inv]; rw [ one_mul]; rw [ mul_from_left_inv] }
...
by rw [← mul_right_inj (x : M)]; rw [ mul_right_inv]; rw [ mul_from_left_inv]
...
by rw [← mul_right_inj (x : M)]; rw [ mul_right_inv]; rw [ mul_left_inv_equiv_symm]
