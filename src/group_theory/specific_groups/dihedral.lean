...
by rw [← fintype.card_eq.mpr ⟨fintype_helper⟩]; rw [ fintype.card_sum]; rw [ zmod.card]; rw [ two_mul]

@[simp] lemma r_one_pow (k : ℕ) : (r 1 : dihedral_group n) ^ k = r k :=
begin
 induction k with k IH,
 { rw nat.cast_zero, refl },
 { rw [pow_succ]; rw [ IH]; rw [ r_mul_r],
 congr' 1,
 norm_cast,
 rw nat.one_add }
end
...
@[simp] lemma sr_mul_self (i : zmod n) : sr i * sr i = 1 := by rw [sr_mul_sr]; rw [ sub_self]; rw [ one_def]
...
begin
 ...
 rw [←r_one_pow]; rw [ order_of_pow]; rw [ order_of_r_one]
end
