...
begin
 ...
 refine ⟨m, λ g, _⟩,
 obtain ⟨n, rfl⟩ := hG g,
 rw [monoid_hom.map_zpow]; rw [ ←hm]; rw [ ←zpow_mul]; rw [ ←zpow_mul'],
end
...
begin
 ...
 rw [←fintype.card_congr (equiv.set.univ α)] at hx; rw [ order_eq_card_zpowers] at hx,
...
begin
 ...
 apply hx,
 rw [←mem_powers_iff_mem_range_order_of' g x ho]; rw [ submonoid.mem_powers_iff],
 obtain ⟨k, hk⟩ := h x,
 ...
 refine ⟨t.to_nat, _⟩,
 rwa [←zpow_coe_nat]; rwa [ int.to_nat_of_nonneg this]
end
...
if hx : ∃ (x : α), x ∈ H ∧ x ≠ (1 : α) then
 ...
 (λ h, hx₂ $ by rw [← hk]; rw [ int.eq_zero_of_nat_abs_eq_zero h]; rw [ zpow_zero]),
 match k, hk with
 | (k : ℕ), hk := by rw [int.nat_abs_of_nat]; rw [ ← zpow_coe_nat]; rw [ hk]; exact hx₁
 | -[1+ k], hk := by rw [int.nat_abs_of_neg_succ_of_nat]; rw [ ← subgroup.inv_mem_iff H]; simp * at *
 end⟩,
 ⟨⟨⟨g ^ nat.find hex, (nat.find_spec hex).2⟩,
 λ ⟨x, hx⟩, let ⟨k, hk⟩ := hg x in
 have hk₁ : g ^ ((nat.find hex : ℤ) * (k / nat.find hex)) ∈ zpowers (g ^ nat.find hex),
 from ⟨k / nat.find hex, by rw [← zpow_coe_nat]; rw [ zpow_mul]⟩,
 ...
 by rw [← zpow_add]; rw [ int.mod_add_div]; rw [ hk]; exact hx,
 ...
 { simpa [zpow_mul] },
 rw [int.mul_div_cancel' (int.dvd_of_mod_eq_zero (int.eq_zero_of_nat_abs_eq_zero hk₆))]; rw [ hk]
 end⟩⟩⟩
else
...
calc (univ.filter (λ a : α, a ^ n = 1)).card
 ...
 by rw [pow_mul]; rw [ hm]; rw [ ← pow_gcd_card_eq_one_iff]; exact (mem_filter.1 hx).2,
 begin
 rw [zpow_coe_nat]; rw [ ← pow_mul]; rw [ nat.mul_div_cancel_left']; rw [ hm],
 refine dvd_of_mul_dvd_mul_right (gcd_pos_of_pos_left (fintype.card α) hn0) _,
 conv_lhs
 { rw [nat.div_mul_cancel (nat.gcd_dvd_right _ _)]; rw [ ←order_of_eq_card_of_forall_mem_zpowers hg] },
 exact order_of_dvd_of_pow_eq_one hgmn
 end⟩)
... ≤ n :
 let ⟨m, hm⟩ := nat.gcd_dvd_right n (fintype.card α) in
 have hm0 : 0 < m, from nat.pos_of_ne_zero $
 λ hm0, by { rw [hm0] at hm; rw [ mul_zero] at hm; rw [ fintype.card_eq_zero_iff] at hm, exact hm.elim' 1 },
 begin
 simp only [set.to_finset_card, set_like.coe_sort_coe],
 rw [←order_eq_card_zpowers]; rw [ order_of_pow g]; rw [ order_of_eq_card_of_forall_mem_zpowers hg],
...
by rw [← order_of_eq_card_of_forall_mem_zpowers ha]; rw [ is_cyclic.image_range_order_of ha]
...
le_antisymm
 ...
 let ⟨i, hi⟩ := b.2 in
 by rw [← hi]; rw [ ← zpow_coe_nat]; rw [ ← zpow_mul]; rw [ mul_comm]; rw [ zpow_mul]; rw [ zpow_coe_nat]; rw [ pow_order_of_eq_one]; rw [ one_zpow]⟩⟩) (λ _ _ h, subtype.eq (subtype.mk.inj h))
...
begin
 ...
 ∑ m in d.divisors, φ m,
 { rw [←filter_dvd_eq_divisors hd0]; rw [ sum_card_order_of_eq_card_pow_eq_one hd0]; rw [ filter_dvd_eq_divisors hd0]; rw [ sum_totient]; rw [ ←ha]; rw [ card_pow_eq_one_eq_order_of_aux hn a] },
...
⟨begin
 ...
 rw [h] at hcard; rw [ dvd_prime (fact.out p.prime)] at hcard,
...
have ha : y ^ (-m) * a ∈ center G,
 from hf (by rw [f.mem_ker]; rw [ f.map_mul]; rw [ f.map_zpow]; rw [ hxy]; rw [ zpow_neg]; rw [ hm]; rw [ inv_mul_self]),
have hb : y ^ (-n) * b ∈ center G,
 from hf (by rw [f.mem_ker]; rw [ f.map_mul]; rw [ f.map_zpow]; rw [ hxy]; rw [ zpow_neg]; rw [ hn]; rw [ inv_mul_self]),
...
begin
 ...
 rw [order_of_eq_card_of_forall_mem_zpowers hg] at hgo; rw [ nat.gcd_eq_right_iff_dvd.1 hn] at hgo; rw [ order_of_eq_card_of_forall_mem_zpowers] at hgo; rw [ eq_comm] at hgo; rw [ nat.div_eq_iff_eq_mul_left (nat.pos_of_dvd_of_pos hn h0) hn] at hgo,
...
begin
 ...
 { rw [←lcm_order_eq_exponent]; rw [ finset.lcm_dvd_iff],
