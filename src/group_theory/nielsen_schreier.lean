...
{ quiver_generators := ⟨λ a b, { e : fgp.generators G // fgp.of e • a.back = b.back }⟩,
 ...
 ext,
 rw [monoid_hom.comp_apply]; rw [ hF'],
 refl },
...
by rw [tree_hom]; rw [ unique.default_eq]
...
begin
 rw [loop_of_hom]; rw [ ←category.assoc]; rw [ is_iso.comp_inv_eq]; rw [ category.id_comp],
 cases H,
 { rw [tree_hom_eq T (path.cons default ⟨sum.inl e, H⟩)]; rw [ hom_of_path], refl },
 { rw [tree_hom_eq T (path.cons default ⟨sum.inr e, H⟩)]; rw [ hom_of_path],
...
{ obj := λ _, (),
 ...
 map_id' := begin
 intro a,
 rw [loop_of_hom]; rw [ category.id_comp]; rw [ is_iso.hom_inv_id]; rw [ ←End.one_def]; rw [ f.map_one]; rw [ id_as_one],
 end,
 map_comp' := begin
 intros,
...
is_free_group.of_unique_lift
 ...
 { rintro ⟨⟨a, b, e⟩, h⟩,
 rw [functor.map_End_apply]; rw [ this]; rw [ hF'],
 exact dif_neg h },
 intros,
 ...
 { rw [hom_of_path]; rw [ F'.map_id]; rw [ id_as_one] },
 rw [hom_of_path]; rw [ F'.map_comp]; rw [ comp_as_mul]; rw [ ih]; rw [ mul_one],
 rcases e with ⟨e | e, eT⟩,
 { rw hF', exact dif_pos (or.inl eT) },
 { rw [F'.map_inv]; rw [ inv_as_inv]; rw [ inv_eq_one]; rw [ hF'], exact dif_pos (or.inr eT) } },
 { intros E hE,
 ext,
 ...
 split_ifs,
 { rw [loop_of_hom_eq_id T e h]; rw [ ←End.one_def]; rw [ E.map_one] },
 { exact hE ⟨⟨a, b, e⟩, h⟩ } }
 end
...
begin
 rintro ⟨p⟩,
 rw [←@weakly_connected_component.eq (generators G)]; rw [ eq_comm]; rw [ ←free_group.of_injective.eq_iff]; rw [ ←mul_inv_eq_one],
 ...
 congr, ext,
 rw [functor.const_obj_map]; rw [ id_as_one]; rw [ difference_functor_map]; rw [ mul_inv_eq_one],
