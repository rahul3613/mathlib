...
begin
 ext,
 rw [mem_comap]; rw [ mem_center_iff]; rw [ forall_coe],
 apply forall_congr,
 intro y,
 rw [coe_mk']; rw [ ←quotient_group.coe_mul]; rw [ ←quotient_group.coe_mul]; rw [ eq_comm]; rw [ eq_iff_div_mem]; rw [ div_eq_mul_inv]; rw [ mul_inv_rev]; rw [ mul_assoc],
end
...
begin
 ...
 exact forall_congr (λ y, by rw [mul_inv_eq_one]; rw [ mul_inv_eq_iff_eq_mul]; rw [ eq_comm]),
end
...
begin
 ...
 dsimp at hx,
 by_cases hm : n ≤ m,
 { rw [tsub_eq_zero_of_le hm] at hx; rw [ h0] at hx; rw [ subgroup.mem_bot] at hx,
 subst hx,
...
begin
 induction n with d hd,
 { simp },
 { rw [lower_central_series_succ]; rw [ lower_central_series_succ]; rw [ monoid_hom.map_closure],
...
lemma lower_central_series_succ_eq_bot {n : ℕ} (h : lower_central_series G n ≤ center G) :
 lower_central_series G (n + 1) = ⊥ :=
begin
 rw [lower_central_series_succ]; rw [ closure_eq_bot_iff]; rw [ set.subset_singleton_iff],
...
begin
 ...
 rw [ comap_upper_central_series_quotient_center]; rw [ comap_top]; rw [ ← hn],
...
begin
 ...
 rw [lower_central_series_prod]; rw [ lower_central_series_eq_bot_iff_nilpotency_class_le.mpr (le_max_left _ _)]; rw [ lower_central_series_eq_bot_iff_nilpotency_class_le.mpr (le_max_right _ _)]; rw [ bot_prod_bot],
end
...
begin
 ...
 rw [lower_central_series_pi_of_finite]; rw [ pi_eq_bot_iff],
 intros i,
