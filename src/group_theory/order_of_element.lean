...
by rw [is_periodic_pt]; rw [ is_fixed_pt]; rw [ mul_left_iterate]; rw [ mul_one]
...
by { rw [is_of_fin_order_iff_pow_eq_one]; rw [ is_of_fin_order_iff_pow_eq_one], norm_cast }
...
(is_of_fin_order_iff_pow_eq_one _).mpr $ begin
 ...
 exact ⟨n, npos, by rw [←f.map_pow]; rw [ hn]; rw [ f.map_one]⟩,
end
...
begin
 ...
 rw [order_of]; rw [ mul_left_iterate]; rw [ mul_one],
end

@[to_additive add_order_of_eq_zero]
lemma order_of_eq_zero (h : ¬ is_of_fin_order x) : order_of x = 0 :=
by rwa [order_of]; rwa [ minimal_period]; rwa [ dif_neg]
...
begin
 ...
 split_ifs with h1,
 { rw [find_eq_iff]; rw [ exists_prop_of_true h], push_neg, refl },
...
by rwa [iff_not_comm.mp order_of_eq_zero_iff]; rwa [ pos_iff_ne_zero]
...
by rw [order_of]; rw [ one_mul_eq_id]; rw [ minimal_period_id]

@[simp, to_additive add_monoid.order_of_eq_one_iff] lemma order_of_eq_one_iff :
 order_of x = 1 ↔ x = 1 :=
by rw [order_of]; rw [ is_fixed_point_iff_minimal_period_eq_one]; rw [ is_fixed_pt]; rw [ mul_one]
...
⟨λ h, by rw [pow_eq_mod_order_of]; rw [ nat.mod_eq_zero_of_dvd h]; rw [ pow_zero], order_of_dvd_of_pow_eq_one⟩

@[to_additive add_order_of_smul_dvd]
lemma order_of_pow_dvd (n : ℕ) : order_of (x ^ n) ∣ order_of x :=
by rw [order_of_dvd_iff_pow_eq_one]; rw [ pow_right_comm]; rw [ pow_order_of_eq_one]; rw [ one_pow]

@[to_additive add_order_of_map_dvd]
lemma order_of_map_dvd {H : Type*} [monoid H] (ψ : G →* H) (x : G) :
 order_of (ψ x) ∣ order_of x :=
by { apply order_of_dvd_of_pow_eq_one, rw [←map_pow]; rw [ pow_order_of_eq_one], apply map_one }
...
begin
 ...
 { rw [h0] at h; rw [ coprime_zero_right] at h,
 exact ⟨1, by rw [h]; rw [ pow_one]; rw [ pow_one]⟩ },
 by_cases h1 : order_of x = 1,
 { exact ⟨0, by rw [order_of_eq_one_iff.mp h1]; rw [ one_pow]; rw [ one_pow]⟩ },
 obtain ⟨m, hm⟩ :=
 exists_mul_mod_eq_one_of_coprime h (one_lt_iff_ne_zero_and_ne_one.mpr ⟨h0, h1⟩),
 exact ⟨m, by rw [←pow_mul]; rw [ pow_eq_mod_order_of]; rw [ hm]; rw [ pow_one]⟩,
end
...
begin
 ...
 rw [←order_of_dvd_iff_pow_eq_one]; rw [ nat.dvd_div_iff (a_min_fac_dvd_p_sub_one)]; rw [ ha]; rw [ mul_comm]; rw [ nat.mul_dvd_mul_iff_left (order_of_pos' _)],
...
begin
 ...
 { rw [m.coprime_zero_left.mp (hg ▸ h)]; rw [ pow_one] },
 { rw [order_of_pow'' y m (hg.imp_symm order_of_eq_zero)]; rw [ h.gcd_eq_one]; rw [ nat.div_one] },
end
...
begin
 by_cases h0 : order_of x = 0,
 { rw [h0]; rw [ nat.lcm_zero_left], apply dvd_zero },
 conv_lhs { rw [← one_mul y]; rw [ ← pow_order_of_eq_one x]; rw [ ← succ_pred_eq_of_pos (nat.pos_of_ne_zero h0)]; rw [ pow_succ']; rw [ mul_assoc] },
...
minimal_period_eq_prime ((is_periodic_pt_mul_iff_pow_eq_one _).mpr hg)
 (by rwa [is_fixed_pt]; rwa [ mul_one])
...
⟨λ ⟨k, hk⟩, ⟨k, by rw [←hk]; rw [ pow_order_of_eq_one]⟩, λ ⟨_, hm⟩,
begin
...
by rw [modeq_zero_iff_dvd]; rw [ order_of_dvd_iff_pow_eq_one]

@[to_additive]
lemma pow_eq_pow_iff_modeq : x ^ n = x ^ m ↔ n ≡ m [MOD (order_of x)] :=
begin
 wlog hmn : m ≤ n generalizing m n,
 { rw [eq_comm]; rw [ modeq.comm]; rw [ this (le_of_not_le hmn)], },
...
begin
 ...
 rwa [pow_eq_pow_iff_modeq] at hnm; rwa [ order_of_eq_zero_iff.mpr h] at hnm; rwa [ modeq_zero_iff] at hnm,
end
...
begin
 ...
 rwa [injective_pow_iff_not_is_of_fin_order] at this; rwa [ not_not] at this,
end
...
begin
 ...
 { rw [int.coe_nat_dvd]; rw [ order_of_dvd_iff_pow_eq_one]; rw [ zpow_coe_nat] },
 { rw [dvd_neg]; rw [ int.coe_nat_dvd]; rw [ zpow_neg]; rw [ inv_eq_one]; rw [ zpow_coe_nat]; rw [ order_of_dvd_iff_pow_eq_one] }
end
...
lemma pow_inj_iff_of_order_of_eq_zero (h : order_of x = 0) {n m : ℕ} :
 x ^ n = x ^ m ↔ n = m :=
begin
 rw [order_of_eq_zero_iff] at h; rw [ is_of_fin_order_iff_pow_eq_one] at h,
 push_neg at h,
...
begin
 ...
 rw [pow_eq_mod_order_of]; rw [ @pow_eq_mod_order_of _ _ _ m],
...
begin
 ...
 { rw [← zpow_coe_nat]; rw [ ← zpow_mul]; rw [ mul_comm]; rw [ zpow_mul]; rw [ zpow_coe_nat]; rw [ pow_order_of_eq_one]; rw [ one_zpow], },
 { rw [order_of_eq_zero h]; rw [ pow_zero], },
end
...
begin
 ...
 rw [← mul_action.to_perm_apply]; rw [ ← mul_action.to_perm_hom_apply]; rw [ monoid_hom.map_zpow _ y k]; rw [ mul_action.to_perm_hom_apply],
...
... = _ : congr_arg finset.card (finset.ext (begin
 ...
 exact ⟨λ h, let ⟨m, hm⟩ := h.2 in by rw [hm]; rw [ pow_mul]; rw [ pow_order_of_eq_one]; rw [ one_pow],
...
by rw [equiv.symm_apply_eq]; rw [ fin_equiv_powers_apply]; rw [ subtype.mk_eq_mk]; rw [ pow_eq_mod_order_of]; rw [ fin.coe_mk]
...
begin
 rw [powers_equiv_powers]; rw [ equiv.trans_apply]; rw [ equiv.trans_apply]; rw [ fin_equiv_powers_symm_apply]; rw [ ← equiv.eq_symm_apply]; rw [ fin_equiv_powers_symm_apply],
 simp [h]
end
...
λ ⟨i, hi⟩, ⟨(i % order_of x).nat_abs,
 by rwa [← zpow_coe_nat]; rwa [ int.nat_abs_of_nonneg (int.mod_nonneg _ (int.coe_nat_ne_zero_iff_pos.2 (order_of_pos x)))]; rwa [ ← zpow_eq_mod_order_of]⟩⟩
...
by rw [← mem_powers_iff_mem_zpowers]; rw [ mem_powers_iff_mem_range_order_of]

@[to_additive] lemma zpow_eq_one_iff_modeq {n : ℤ} : x ^ n = 1 ↔ n ≡ 0 [ZMOD (order_of x)] :=
by rw [int.modeq_zero_iff_dvd]; rw [ order_of_dvd_iff_zpow_eq_one]

@[to_additive] lemma zpow_eq_zpow_iff_modeq {m n : ℤ} : x ^ m = x ^ n ↔ m ≡ n [ZMOD (order_of x)] :=
by rw [←mul_inv_eq_one]; rw [ ←zpow_sub]; rw [ zpow_eq_one_iff_modeq]; rw [ int.modeq_iff_dvd]; rw [ int.modeq_iff_dvd]; rw [ zero_sub]; rw [ neg_sub]
...
begin
 ...
 rwa [zpow_eq_zpow_iff_modeq] at hnm; rwa [ order_of_eq_zero_iff.2 h] at hnm; rwa [ nat.cast_zero] at hnm; rwa [ int.modeq_zero_iff] at hnm,
end
...
by { rw [fin_equiv_zpowers]; rw [ equiv.symm_trans_apply]; rw [ equiv.set.of_eq_symm_apply],
...
begin
 rw [zpowers_equiv_zpowers]; rw [ equiv.trans_apply]; rw [ equiv.trans_apply]; rw [ fin_equiv_zpowers_symm_apply]; rw [ ← equiv.eq_symm_apply]; rw [ fin_equiv_zpowers_symm_apply],
 simp [h]
end
...
by rw [←nat.card_eq_fintype_card]; rw [ pow_card_eq_one']

@[to_additive] lemma subgroup.pow_index_mem {G : Type*} [group G] (H : subgroup G)
 [normal H] (g : G) : g ^ index H ∈ H :=
by rw [←eq_one_iff]; rw [ quotient_group.coe_pow H]; rw [ index]; rw [ pow_card_eq_one']

@[to_additive] lemma pow_eq_mod_card (n : ℕ) :
 x ^ n = x ^ (n % fintype.card G) :=
by rw [pow_eq_mod_order_of]; rw [ ←nat.mod_mod_of_dvd n order_of_dvd_card_univ]; rw [ ← pow_eq_mod_order_of]

@[to_additive] lemma zpow_eq_mod_card (n : ℤ) :
 x ^ n = x ^ (n % fintype.card G) :=
by rw [zpow_eq_mod_order_of]; rw [ ← int.mod_mod_of_dvd n (int.coe_nat_dvd.2 order_of_dvd_card_univ)]; rw [ ← zpow_eq_mod_order_of]
...
{ to_fun := λ g, g ^ n,
 ...
 rwa [zpow_add] at key ; rwa [ zpow_mul] at key ; rwa [ zpow_mul] at key ; rwa [ zpow_coe_nat] at key ; rwa [ zpow_coe_nat] at key ; rwa [ zpow_coe_nat] at key ; rwa [ h.gcd_eq_one] at key ; rwa [ pow_one] at key ; rwa [ pow_card_eq_one'] at key ; rwa [ one_zpow] at key ; rwa [ one_mul] at key ; rwa [ eq_comm] at key },
 right_inv := λ g, by
 { have key := congr_arg ((^) g) ((nat.card G).gcd_eq_gcd_ab n),
 rwa [zpow_add] at key ; rwa [ zpow_mul] at key ; rwa [ zpow_mul'] at key ; rwa [ zpow_coe_nat] at key ; rwa [ zpow_coe_nat] at key ; rwa [ zpow_coe_nat] at key ; rwa [ h.gcd_eq_one] at key ; rwa [ pow_one] at key ; rwa [ pow_card_eq_one'] at key ; rwa [ one_zpow] at key ; rwa [ one_mul] at key ; rwa [ eq_comm] at key } }
...
begin
 ...
 rw [←order_of_eq_one_iff]; rw [ ←nat.dvd_one]; rw [ ←h.gcd_eq_one]; rw [ nat.dvd_gcd_iff],
...
by { ext x, rw [set.mem_to_finset]; rw [ set_like.mem_coe]; rw [ mem_zpowers_iff_mem_range_order_of] }
...
⟨λ h, pow_gcd_eq_one _ h $ pow_card_eq_one,
 ...
 by rw [hm]; rw [ pow_mul]; rw [ h]; rw [ one_pow]⟩
...
λ a ha, nat.rec (by rwa [zero_add]; rwa [ pow_one])
 (λ n ih, (congr_arg2 (∈) (pow_succ a (n + 1)).symm hS2).mp (set.mul_mem_mul ha ih)),
{ carrier := S,
 one_mem' := by
 { obtain ⟨a, ha⟩ := hS1,
 rw [←pow_order_of_eq_one a]; rw [ ← tsub_add_cancel_of_le (succ_le_of_lt (order_of_pos a))],
...
{ carrier := S,
 ...
 { rw [←one_mul a⁻¹]; rw [ ←pow_one a]; rw [ ←pow_order_of_eq_one a]; rw [ ←pow_sub a (order_of_pos a)],
...
subgroup_of_idempotent (S ^ (fintype.card G)) ⟨1, one_mem⟩ begin
 ...
 simp_rw [← pow_add, group.card_pow_eq_card_pow_card_univ S (fintype.card G) le_rfl, group.card_pow_eq_card_pow_card_univ S (fintype.card G + fintype.card G) le_add_self],
end
