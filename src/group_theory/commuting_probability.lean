...
begin
 ...
 rw [comm_prob]; rw [ ←set.coe_set_of]; rw [ nat.card_eq_fintype_card]; rw [ nat.card_eq_fintype_card],
...
begin
 rw [comm_prob]; rw [ card_comm_eq_card_conj_classes_mul_card]; rw [ nat.cast_mul]; rw [ sq],
...
begin
 ...
 rw [comm_prob_def]; rw [ comm_prob_def]; rw [ div_le_iff]; rw [ mul_assoc]; rw [ ←mul_pow]; rw [ ←nat.cast_mul]; rw [ mul_comm H.index]; rw [ H.card_mul_index]; rw [ div_mul_cancel]; rw [ nat.cast_le],
...
begin
 ...
 rw [comm_prob_def']; rw [ comm_prob_def']; rw [ div_le_iff]; rw [ mul_assoc]; rw [ ←nat.cast_mul]; rw [ ←subgroup.index]; rw [ H.card_mul_index]; rw [ div_mul_cancel]; rw [ nat.cast_le],
