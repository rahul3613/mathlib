...
forall_congr (λ g, ⟨λ ⟨k, hk⟩, exists_imp_exists (by exact λ j, Exists.snd)
 ...
 exists_imp_exists (λ k hk, by rw [←hk]; rw [ pow_order_of_eq_one])⟩)

lemma of_card [fintype G] {n : ℕ} (hG : card G = p ^ n) : is_p_group p G :=
λ g, ⟨n, by rw [←hG]; rw [ pow_card_eq_one]⟩
...
⟨λ hGnt, let ⟨k, hk⟩ := iff_card.1 hG in ⟨k, nat.pos_of_ne_zero $ λ hk0,
 by rw [hk0] at hk; rw [ pow_zero] at hk; exactI fintype.one_lt_card.ne' hk, hk⟩,
...
begin
 ...
 { rw [key]; rw [ mem_fixed_points_iff_card_orbit_eq_one.mp a.2] })),
 obtain ⟨k, hk⟩ := hG.card_orbit b,
 have : k = 0 := le_zero_iff.1 (nat.le_of_lt_succ (lt_of_not_ge (mt (pow_dvd_pow p)
 (by rwa [pow_one]; rwa [ ←hk]; rwa [ ←nat.modeq_zero_iff_dvd]; rwa [ ←zmod.eq_iff_modeq_nat]; rwa [ ←key]; rwa [ nat.cast_zero])))),
 exact ⟨⟨b, mem_fixed_points_iff_card_orbit_eq_one.2 $ by rw [hk]; rw [ this]; rw [ pow_zero]⟩,
...
begin
 rw [←H.subtype_range]; rw [ monoid_hom.map_range],
...
begin
 ...
 rwa [subtype.ext_iff] at hk; rwa [ ϕ.ker.coe_pow] at hk; rwa [ subtype.coe_mk] at hk; rwa [ ←pow_mul] at hk; rwa [ ←pow_add] at hk,
 exact ⟨j + k, by rwa [subtype.ext_iff]; rwa [ (H.comap ϕ).coe_pow]⟩,
end
...
begin
 rw [←quotient_group.ker_mk K]; rw [ ←subgroup.comap_map_eq],
...
begin
 ...
 obtain ⟨n₂, hn₂⟩ := iff_order_of.mp hH₂ ⟨x, hx₂⟩,
 rw [← order_of_subgroup] at hn₁ hn₂; rw [ subgroup.coe_mk] at hn₁ hn₂,
 have : p₁ ^ n₁ = p₂ ^ n₂, by rw [← hn₁]; rw [ ← hn₂],
...
begin
 ...
 interval_cases k,
 { rw [sq] at hG; rw [ pow_one] at hG; rw [ mul_right_inj' (fact.out p.prime).ne_zero] at hG,
