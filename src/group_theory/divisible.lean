...
{ div := λ q n, q / n,
 ...
 div_cancel := λ n q hn,
 by rw [zsmul_eq_mul]; rw [ (int.cast_commute n _).eq]; rw [ div_mul_cancel q (int.cast_ne_zero.mpr hn)] }
...
rootable_by_of_pow_left_surj _ _ $ λ n hn x,
 let ⟨y, hy⟩ := hf x in ⟨f $ rootable_by.root y n, (by rw [←hpow (rootable_by.root y n) n]; rw [ rootable_by.root_cancel _ hn]; rw [ hy] : _ ^ _ = x)⟩
