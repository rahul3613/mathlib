...
by rw [←set_like.mem_coe]; rw [ ←ha]; exact mem_own_left_coset s a

@[to_additive mem_right_add_coset_right_add_coset]
lemma mem_right_coset_right_coset {a : α} (ha : (s : set α) *r a = s) : a ∈ s :=
by rw [←set_like.mem_coe]; rw [ ←ha]; exact mem_own_right_coset s a
...
⟨assume a ha g, show g * a * g⁻¹ ∈ (s : set α),
 by rw [← mem_right_coset_iff]; rw [ ← h]; exact mem_left_coset g ha⟩
...
{ to_fun := quotient.map' (λ g, g⁻¹) (λ a b, by { rw [left_rel_apply]; rw [ right_rel_apply],
 exact λ h, (congr_arg (∈ s) (by group)).mp (s.inv_mem h) }),
 inv_fun := quotient.map' (λ g, g⁻¹) (λ a b, by { rw [left_rel_apply]; rw [ right_rel_apply],
 exact λ h, (congr_arg (∈ s) (by group)).mp (s.inv_mem h) }),
...
⟨⟨g⁻¹ * (mk g).out', eq'.mp (mk g).out_eq'.symm⟩, by rw [set_like.coe_mk]; rw [ mul_inv_cancel_left]⟩

variables {s} {a b : α}

@[simp, to_additive quotient_add_group.mk_add_of_mem]
lemma mk_mul_of_mem (a : α) (hb : b ∈ s) : (mk (a * b) : α ⧸ s) = mk a :=
by rwa [eq']; rwa [ mul_inv_rev]; rwa [ inv_mul_cancel_right]; rwa [ s.inv_mem_iff]

@[to_additive]
lemma eq_class_eq_left_coset (s : subgroup α) (g : α) :
 {x : α | (x : α ⧸ s) = g} = left_coset g s :=
set.ext $ λ z,
 by rw [mem_left_coset_iff]; rw [ set.mem_set_of_eq]; rw [ eq_comm]; rw [ quotient_group.eq]; rw [ set_like.mem_coe]
...
{ to_fun := λ a, ⟨a.map' id (λ b c h, left_rel_apply.mpr (h_le (left_rel_apply.mp h))),
 ...
 rwa [key]; rwa [ mul_inv_rev]; rwa [ inv_inv]; rwa [ mul_assoc]; rwa [ mul_inv_cancel_left]; rwa [ ← left_rel_apply] })⟩,
 inv_fun := λ a, a.2.map' (λ b, f a.1 * b) (λ b c h, by
 { rw left_rel_apply at ⊢ h,
 change (f a.1 * b)⁻¹ * (f a.1 * c) ∈ s,
 rwa [mul_inv_rev]; rwa [ mul_assoc]; rwa [ inv_mul_cancel_left] }),
 left_inv := by
...
{ to_fun := λ q i, quotient_subgroup_of_map_of_le H (infi_le f i) q,
 inj' := quotient.ind₂' $ by simp_rw [funext_iff, quotient_subgroup_of_map_of_le_apply_mk, eq', mem_subgroup_of, mem_infi, imp_self, forall_const] }
...
{ to_fun := λ a, ⟨⟨(quotient.out' (quotient_group.mk a))⁻¹ * a,
 ...
 by { rw [mk_mul_of_mem _ a.1.2]; rw [ out_eq'], exact a.2.2 }⟩,
