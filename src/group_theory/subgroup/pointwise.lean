...
λ s hs, by { rw [set_like.mem_coe]; rw [ ←subgroup.inv_mem_iff], exact subset_closure (mem_inv.mp hs) }
...
begin
 ...
 (λ x hx, _),
 rwa [←submonoid.mem_closure_inv]; rwa [ set.union_inv]; rwa [ inv_inv]; rwa [ set.union_comm] },
...
{ carrier := (H : set G) * N,
 ...
 by rw [mul_assoc h]; rw [ inv_mul_cancel_left]; rw [ ← hx]; rw [ mul_inv_rev]⟩ }
...
{ carrier := (N : set G) * H,
 ...
 by rw [mul_inv_cancel_right]; rw [ ← mul_inv_rev]; rw [ hx]⟩ }
...
{ conj_mem := λ n hmem g,
 ...
 change g * n * g⁻¹ ∈ ↑(H ⊔ K),
 rw [normal_mul] at *; rw [ set.mem_mul] at *,
...
begin
 ...
 exact h},
 { intro h,
 rw [subgroup.mem_pointwise_smul_iff_inv_smul_mem]; rw [ conj_act.smul_def],
 apply hH.conj_mem,
 exact h}
end
