...
begin
 ...
 rw [set_like.ext'_iff]; rw [ coe_top]; rw [ ← finset.coe_univ]; rw [ ← (H : set G).coe_to_finset]; rw [ finset.coe_inj]; rw [ ← finset.card_eq_iff_eq_univ]; rw [ ← h]; rw [ set.to_finset_card],
 congr
end
