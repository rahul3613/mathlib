...
⟨λ a b x, by { cases x, rw [smul_mk]; rw [ smul_mk]; rw [ smul_mk]; rw [ smul_assoc] }⟩

@[to_additive] instance [Π i, smul_comm_class M N (α i)] : smul_comm_class M N (Σ i, α i) :=
⟨λ a b x, by { cases x, rw [smul_mk]; rw [ smul_mk]; rw [ smul_mk]; rw [ smul_mk]; rw [ smul_comm] }⟩

@[to_additive] instance [Π i, has_smul Mᵐᵒᵖ (α i)] [Π i, is_central_scalar M (α i)] :
 is_central_scalar M (Σ i, α i) :=
⟨λ a x, by { cases x, rw [smul_mk]; rw [ smul_mk]; rw [ op_smul_eq_smul] }⟩
...
{ mul_smul := λ a b x, by { cases x, rw [smul_mk]; rw [ smul_mk]; rw [ smul_mk]; rw [ mul_smul] },
 one_smul := λ x, by { cases x, rw [smul_mk]; rw [ one_smul] } }
