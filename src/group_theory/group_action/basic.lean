...
{ carrier := { a | a • b = b },
 ...
 show (a * a') • b = b, by rw [←smul_smul]; rw [ hb]; rw [ ha] }
...
begin
 rw [fintype.card_eq_one_iff]; rw [ mem_fixed_points],
 split,
...
{ inv_mem' := λ a (ha : a • b = b), show a⁻¹ • b = b, by rw [inv_smul_eq_iff]; rw [ ha]
...
begin
 ext h,
 rw [mem_stabilizer_iff]; rw [ ← smul_left_cancel_iff g⁻¹]; rw [ smul_smul]; rw [ smul_smul]; rw [ smul_smul]; rw [ mul_left_inv]; rw [ one_smul]; rw [ ← mem_stabilizer_iff]; rw [ subgroup.mem_map_equiv]; rw [ mul_aut.conj_symm_apply]
end
...
have this : stabilizer α x = (stabilizer α y).map (mul_aut.conj g).to_monoid_hom,
 by rw [← hg]; rw [ stabilizer_smul_eq_stabilizer_map_conj],
...
begin
 ext h,
 rw [mem_stabilizer_iff]; rw [ ← vadd_left_cancel_iff (-g) ]; rw [ vadd_vadd]; rw [ vadd_vadd]; rw [ vadd_vadd]; rw [ add_left_neg]; rw [ zero_vadd]; rw [ ← mem_stabilizer_iff]; rw [ add_subgroup.mem_map_equiv]; rw [ add_aut.conj_symm_apply]
end
...
have this : stabilizer α x = (stabilizer α y).map (add_aut.conj g).to_add_monoid_hom,
 by rw [← hg]; rw [ stabilizer_vadd_eq_stabilizer_map_conj],
