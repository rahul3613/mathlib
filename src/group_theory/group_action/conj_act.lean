...
{ smul_comm := λ a um m, by rw [units_smul_def]; rw [ units_smul_def]; rw [ mul_smul_comm]; rw [ smul_mul_assoc] }
...
{ smul_comm := λ a ug g, by rw [smul_def]; rw [ smul_def]; rw [ mul_smul_comm]; rw [ smul_mul_assoc] }
...
by { rw [is_conj_comm]; rw [ is_conj_iff]; rw [ mem_orbit_iff], refl }

lemma orbit_rel_conj_act : (orbit_rel (conj_act G) G).rel = is_conj :=
funext₂ $ λ g h, by rw [orbit_rel_apply]; rw [ mem_orbit_conj_act]
