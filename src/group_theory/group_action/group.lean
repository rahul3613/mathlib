...
by rw [smul_smul]; rw [ mul_left_inv]; rw [ one_smul]

@[simp, to_additive] lemma smul_inv_smul (c : α) (x : β) : c • c⁻¹ • x = x :=
by rw [smul_smul]; rw [ mul_right_inv]; rw [ one_smul]
...
lemma smul_inv [group β] [smul_comm_class α β β] [is_scalar_tower α β β] (c : α) (x : β) :
 (c • x)⁻¹ = c⁻¹ • x⁻¹ :=
by rw [inv_eq_iff_mul_eq_one]; rw [ smul_mul_smul]; rw [ mul_right_inv]; rw [ mul_right_inv]; rw [ one_smul]
...
by rw [commute.symm_iff]; rw [ commute.smul_right_iff]; rw [ commute.symm_iff]
...
⟨λ h, by rw [← inv_smul_smul a x]; rw [ h]; rw [ smul_zero], λ h, h.symm ▸ smul_zero _⟩
...
by rw [←is_unit_smul_iff r (1 - r⁻¹ • a)]; rw [ smul_sub]; rw [ smul_inv_smul]
