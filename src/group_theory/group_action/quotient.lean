...
⟨λ _ _ _ _, by rwa [smul_eq_mul]; rwa [ smul_eq_mul]; rwa [ mul_inv_rev]; rwa [ mul_assoc]; rwa [ inv_mul_cancel_left]⟩

@[to_additive] instance right_quotient_action : quotient_action H.normalizer.opposite H :=
⟨λ b c _ _, by rwa [smul_def]; rwa [ smul_def]; rwa [ smul_eq_mul_unop]; rwa [ smul_eq_mul_unop]; rwa [ mul_inv_rev]; rwa [ ←mul_assoc]; rwa [ mem_normalizer_iff'.mp b.prop]; rwa [ mul_assoc]; rwa [ mul_inv_cancel_left]⟩

@[to_additive] instance right_quotient_action' [hH : H.normal] : quotient_action αᵐᵒᵖ H :=
⟨λ _ _ _ _, by rwa [smul_eq_mul_unop]; rwa [ smul_eq_mul_unop]; rwa [ mul_inv_rev]; rwa [ mul_assoc]; rwa [ hH.mem_comm_iff]; rwa [ mul_assoc]; rwa [ mul_inv_cancel_right]⟩
...
by rw [←quotient.smul_mk]; rw [ quotient_group.out_eq']
...
by rw [mul_assoc]; rw [ ←quotient_group.eq']; rw [ quotient_group.out_eq']; rw [ ←smul_eq_mul]; rw [ quotient.mk_smul_out']; rw [ eq_comm]; rw [ pow_smul_eq_iff_minimal_period_dvd]
...
... = g2 • x : by rw [smul_smul]; rw [ mul_inv_cancel_left]
...
by { rw [left_rel_apply], show (g₁⁻¹ * g₂) • x = x, rw [mul_smul]; rw [ ← H]; rw [ inv_smul_smul] }
...
by rw [← fintype.card_prod]; rw [ fintype.card_congr (orbit_prod_stabilizer_equiv_group α b)]
...
begin
 ...
 { intro ω,
 rw [fintype.card_congr (@subgroup.group_equiv_quotient_times_subgroup α _ (stabilizer α $ φ ω))]; rw [ fintype.card_prod]; rw [ nat.mul_div_cancel],
...
by rw [← fintype.card_prod]; rw [ ← fintype.card_sigma]; rw [ fintype.card_congr (sigma_fixed_by_equiv_orbits_prod_group α β)]
...
begin
 ...
 { rw [←H.inv_mem_iff]; rw [ ←mul_one g⁻¹]; rw [ ←quotient_group.eq]; rw [ ←mul_one g],
