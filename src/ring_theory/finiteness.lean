...
begin
 ...
 { rw [← span_le]; rw [ hs], exact le_refl N } },
 ...
 rwa [mem_comap] at hrn ; rwa [ span_empty] at hrn ; rwa [ smul_bot] at hrn ; rwa [ mem_bot] at hrn },
 ...
 { specialize hrn hs.1, rw [mem_comap] at hrn; rw [ mem_sup] at hrn,
 ...
 { rw [sub_smul]; rw [ ← hyz]; rw [ add_sub_cancel'], exact hz } },
 ...
 { intros n hn, specialize hrn hn, rw [mem_comap] at hrn; rw [ mem_sup] at hrn,
...
⟨∅, by rw [finset.coe_empty]; rw [ span_empty]⟩
...
begin
 ...
 rw [← one_mul ↑I]; rw [ ← mul_one (span R ↑T)],
 conv_rhs { rw [← I.inv_mul]; rw [ ← mul_assoc] },
 refine mul_le_mul_left (le_trans _ $ mul_le_mul_right $ span_le.mpr hT'),
 rwa [one_le]; rwa [ span_mul_span],
end
...
fg_def.2 ⟨t₁ ∪ t₂, ht₁.1.union ht₂.1, by rw [span_union]; rw [ ht₁.2]; rw [ ht₂.2]⟩
...
let ⟨t, ht⟩ := fg_def.1 hs in fg_def.2 ⟨f '' t, ht.1.image _, by rw [span_image]; rw [ ht.2]⟩
...
submodule.map_injective_of_injective hf $ by { rw [f.map_span]; rw [ finset.coe_preimage]; rw [ set.image_preimage_eq_inter_range]; rw [ set.inter_eq_self_of_subset_left]; rw [ ht],
 rw [← linear_map.range_coe]; rw [ ← span_le]; rw [ ht]; rw [ ← map_top], exact map_mono le_top }⟩
...
λ h, fg_of_fg_map_injective N.subtype subtype.val_injective $ by rwa [map_top]; rwa [ range_subtype]⟩
...
fg_def.2 ⟨linear_map.inl R M P '' tb ∪ linear_map.inr R M P '' tc,
 ...
 by rw [linear_map.span_inl_union_inr]; rw [ htb.2]; rw [ htc.2]⟩
...
begin
 ...
 rw [← ht1] at this; rw [← set.image_id ↑t1] at this; rw [ finsupp.mem_span_image_iff_total] at this,
 ...
 { rw [← set.image_id (g '' ↑t1)]; rw [ finsupp.mem_span_image_iff_total], refine ⟨_, _, rfl⟩,
 haveI : inhabited P := ⟨0⟩,
 rw [← finsupp.lmap_domain_supported _ _ g]; rw [ mem_map],
 refine ⟨l, hl1, _⟩,
 refl, },
 rw [ht2]; rw [ mem_inf], split,
 { apply s.sub_mem hx,
 rw [finsupp.total_apply]; rw [ finsupp.lmap_domain_apply]; rw [ finsupp.sum_map_domain_index],
 refine s.sum_mem _,
 { intros y hy, exact s.smul_mem _ (hg y (hl1 hy)).1 },
 { exact zero_smul _ }, { exact λ _ _ _, add_smul _ _ _ } },
 { rw [linear_map.mem_ker]; rw [ f.map_sub]; rw [ ← hl2],
 rw [finsupp.total_apply]; rw [ finsupp.total_apply]; rw [ finsupp.lmap_domain_apply],
 rw [finsupp.sum_map_domain_index]; rw [ finsupp.sum]; rw [ finsupp.sum]; rw [ f.map_sum],
 rw sub_eq_zero,
...
begin
 ...
 { rw [finset.coe_empty]; rw [ submodule.span_empty]; rw [ ← submodule.span_zero_singleton], apply h₁ },
 { rw [finset.coe_insert]; rw [ submodule.span_insert], apply h₂; apply_assumption }
end
...
begin
 ...
 { rwa [submodule.map_comap_eq]; rwa [ linear_map.range_eq_top.2 hsur]; rwa [ top_inf_eq] },
...
begin
 ...
 (by { rw [H]; rw [ ← hS], exact submodule.subset_span s.2 }),
 choose f hf,
 use S.attach.sup f,
 apply le_antisymm,
 { conv_lhs { rw ← hS },
...
begin
 ...
 split,
 { rintro ⟨t, rfl⟩,
 rw [span_eq_supr_of_singleton_spans]; rw [ ←supr_rw]; rw [ ←(finset.sup_eq_supr t sp)],
 ...
 by rw [Sup_eq_supr]; rw [ supr_image]; rw [ ←span_eq_supr_of_singleton_spans]; rw [ eq_comm]; rw [ span_eq],
 -- by h, s is then below (and equal to) the sup of the spans of finitely many elements.
 obtain ⟨u, ⟨huspan, husup⟩⟩ := h (sp '' ↑s) (le_of_eq sSup),
 have ssup : s = u.sup id,
 { suffices : u.sup id ≤ s, from le_antisymm husup this,
 rw [sSup]; rw [ finset.sup_id_eq_Sup], exact Sup_le_Sup huspan, },
 obtain ⟨t, ⟨hts, rfl⟩⟩ := finset.subset_image_iff.mp huspan,
 rw [finset.sup_image] at ssup; rw [ function.comp.left_id] at ssup; rw [ finset.sup_eq_supr] at ssup; rw [ supr_rw] at ssup; rw [ ←span_eq_supr_of_singleton_spans] at ssup; rw [ eq_comm] at ssup,
 exact ⟨t, ssup⟩, },
end
...
begin
 ...
 exact ⟨n, by rwa [← ideal.span]; rwa [ span_singleton_pow]; rwa [ span_le]; rwa [ set.singleton_subset_iff]⟩ },
 ...
 rw [add_comm]; rw [ nat.add_sub_assoc h.le], apply nat.le_add_right } },
end
...
lemma of_surjective [hM : finite R M] (f : M →ₗ[R] N) (hf : surjective f) :
 finite R N :=
⟨begin
 rw [← linear_map.range_eq_top.2 hf]; rw [ ← submodule.map_top],
 exact hM.1.map f
end⟩
...
lemma of_restrict_scalars_finite (R A M : Type*) [comm_semiring R] [semiring A] [add_comm_monoid M]
 ...
 finite A M :=
begin
 rw [finite_def] at hM ⊢; rw [ fg_def] at hM ⊢,
...
| ⟨⟨s, hs⟩⟩ ⟨⟨t, ht⟩⟩ := ⟨submodule.fg_def.2
 ...
 by rw [set.image2_smul]; rw [ submodule.span_smul_of_span_eq_top hs (↑t : set M)]; rw [ ht]; rw [ submodule.restrict_scalars_top]⟩⟩
...
begin
 ...
 { exact zero_mem _ },
 { intros x y,
 rw [finset.coe_image]; rw [ ← submodule.span_span_of_tower R]; rw [ submodule.span_image]; rw [ hs]; rw [ submodule.map_top]; rw [ linear_map.range_coe],
 change _ ∈ submodule.span A (set.range $ tensor_product.mk R A M 1),
 rw [← mul_one x]; rw [ ← smul_eq_mul]; rw [ ← tensor_product.smul_tmul'],
