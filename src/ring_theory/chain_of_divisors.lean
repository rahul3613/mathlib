...
⟨λ hp, ⟨by simpa only [associates.is_unit_iff_eq_one] using hp.1,
 ...
 (λ hb, show b = ⊥, by rwa [associates.is_unit_iff_eq_one] at hb); rwa [ ← associates.bot_eq_one] at hb)
...
begin
 ...
 { rw [fin.coe_one']; rw [ nat.mod_eq_of_lt]; rw [ pow_one],
...
begin
 ...
 rw [associates.is_unit_iff_eq_one]; rw [ ← associates.le_one_iff]; rw [ hr],
...
begin
 ...
 { rw [fin.le_iff_coe_le_coe]; rw [ fin.coe_one]; rw [ nat.succ_le_iff]; rw [ ← fin.coe_zero (n.succ + 1)]; rw [ ← fin.lt_iff_coe_lt_coe]; rw [ fin.pos_iff_ne_zero],
 rintro rfl,
...
begin
 ...
 { rw associated_iff_eq at hu', rw [fin.coe_coe_of_lt (nat.lt_succ_of_le hu)]; rw [ hu'] },
...
begin
 ...
 { rw [c₂.def]; rw [ c₂.def]; rw [ subtype.coe_lt_coe]; rw [ d.lt_iff_lt]; rw [ subtype.mk_lt_mk]; rw [ (hc₁').lt_iff_lt],
 exact h },
 ...
 { rintros ⟨i, hr⟩,
 rw [hr]; rw [ c₂.def]; rw [ subtype.coe_le_coe]; rw [ d.le_iff_le],
...
begin
 ...
 { rw [ne.def]; rw [ ← associates.is_unit_iff_eq_bot]; rw [ associates.is_unit_iff_eq_one]; rw [ coe_factor_order_iso_map_eq_one_iff _ d],
 rintro rfl,
 ...
 rw [← subtype.coe_mk b _ ] at hb; rw [ subtype.coe_lt_coe] at hb; rw [ ← hx] at hb,
 letI : order_bot {l : associates M // l ≤ m} := subtype.order_bot bot_le,
 letI : order_bot {l : associates N // l ≤ n} := subtype.order_bot bot_le,
 suffices : x = ⊥,
 { rw [this] at hx; rw [ order_iso.map_bot d] at hx,
...
begin
 ...
 by_cases hm : m = 0,
 { simpa [hm] using hp, },
 rw [← part_enat.coe_get (finite_iff_dom.1 $ finite_prime_left (prime_of_normalized_factor p hp) hm)]; rw [ ← pow_dvd_iff_le_multiplicity],
...
begin
 ...
 { rw [d.symm_apply_apply ⟨p, dvd_of_mem_normalized_factors hp⟩] at this; rw [ subtype.coe_mk] at this,
 exact this },
...
{ to_fun := λ l, ⟨associates.mk (d ⟨associates_equiv_of_unique_units ↑l,
 by { obtain ⟨x, hx⟩ := l, rw [subtype.coe_mk]; rw [ associates_equiv_of_unique_units_apply]; rw [ out_dvd_iff], exact hx } ⟩),
