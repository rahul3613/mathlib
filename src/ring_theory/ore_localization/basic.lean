...
{ r := λ rs rs', ∃ (u : S) (v : R), rs'.1 * u = rs.1 * v
 ...
 { assoc_rw [h₁]; assoc_rw [ h₂]; assoc_rw [ hsu], symmetry, apply mul_assoc },
 rcases ore_left_cancel (v * r₃) (s₂ * s₃) s this with ⟨w, hw⟩,
 use s₂ * s₃ * w, use u * r₃ * w, split; simp only [submonoid.coe_mul],
 { assoc_rw [hru]; assoc_rw [ ←hw], simp [mul_assoc] },
 { assoc_rw [hsu]; assoc_rw [ ←hw], simp [mul_assoc] } },
 ...
 { assoc_rw [hur₂]; assoc_rw [ h]; assoc_rw [ hur₁]; assoc_rw [ mul_assoc] },
 { assoc_rw [hs₂u]; assoc_rw [ h]; assoc_rw [ hs₁u]; assoc_rw [ mul_assoc] } }
 end }
...
begin
 ...
 dsimp at *,
 have s₁vS : (s₁ : R) * v ∈ S,
 { rw [←hs₂]; rw [ ←S.coe_mul], exact set_like.coe_mem (s₂ * u) },
 replace hs₂ : s₂ * u = ⟨(s₁ : R) * v, s₁vS⟩, { ext, simp [hs₂] },
 rw [hP r₁ v s₁ s₁vS]; rw [ hP r₂ u s₂ (by { norm_cast, rw hs₂, assumption })]; rw [ hr₂],
 simpa [← hs₂]
end
...
lift_expand
 ...
 rw [lift_expand_of]; rw [ lift_expand_of]; rw [ hP r₁ t₁ s₁ ht₁ r₂ 1 s₂ (by simp)], simp,
 end
...
begin
 ...
 { rw [←mul_assoc]; rw [ ←h₂]; rw [ ←h₁]; rw [ mul_assoc]; rw [ h₃], simp only [mul_assoc] },
 rw mul'_char (r₂ * p) (r₁ * r) ⟨↑s₂ * p, hp⟩ ⟨↑s₁ * r, hr⟩ _ _ this, clear this,
 have hsssp : ↑s₁ * ↑s₂' * ↑s_star * p_flat ∈ S,
 { rw [mul_assoc]; rw [ mul_assoc]; rw [ ←mul_assoc ↑s₂']; rw [ ←h₃]; rw [ ←mul_assoc],
 ...
 rw [←mul_assoc]; rw [ mul_assoc r₂]; rw [ ore_localization.eq_of_num_factor_eq h₂''],
 norm_cast at ⊢ hsssp, rw [←ore_localization.expand _ _ _ hsssp]; rw [ ←mul_assoc],
...
by { rw [ore_localization.one_def]; rw [ ore_div_eq_iff], exact ⟨⟨r, hr⟩, 1, by simp, by simp⟩ }
...
begin
 ...
 (by { simp only [submonoid.coe_mul], assoc_rw [hb]; assoc_rw [ hc] }),
 rw [mul_assoc]; rw [ ←mul_assoc s₃],
 symmetry, apply ore_div_mul_char,
 assoc_rw [hc]; assoc_rw [ ←ha], apply mul_assoc
end
...
{ to_fun := λ x, x.lift_expand (λ r s, (f r) * ((fS s)⁻¹ : units T)) $ λ r t s ht,
 ...
 conv_rhs { rw [monoid_hom.map_mul]; rw [ ←mul_one (f r)]; rw [ ←units.coe_one]; rw [ ←mul_left_inv (fS s)],
 rw [units.coe_mul]; rw [ ←mul_assoc]; rw [ mul_assoc _ ↑(fS s)]; rw [ ←this]; rw [ mul_assoc] },
 simp only [mul_one, units.mul_inv]
 end,
 map_one' := by rw [ore_localization.one_def]; rw [ lift_expand_of]; simp,
 map_mul' := λ x y,
 begin
 induction x using ore_localization.ind with r₁ s₁,
 induction y using ore_localization.ind with r₂ s₂,
 rcases ore_div_mul_char' r₁ r₂ s₁ s₂ with ⟨ra, sa, ha, ha'⟩, rw ha', clear ha',
 rw [lift_expand_of]; rw [ lift_expand_of]; rw [ lift_expand_of],
 conv_rhs { congr, skip, congr,
 rw [←mul_one (f r₂)]; rw [ ←(fS sa).mul_inv]; rw [ ←mul_assoc]; rw [ ←hf]; rw [ ←f.map_mul]; rw [ ha]; rw [ f.map_mul] },
 rw [mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ ←mul_assoc _ (f s₁)]; rw [ hf s₁]; rw [ (fS s₁).inv_mul]; rw [ one_mul]; rw [ f.map_mul]; rw [ mul_assoc]; rw [ fS.map_mul]; rw [ ←units.coe_mul], refl
 end }
...
begin
 ...
 rw [universal_mul_hom_apply]; rw [ ←huniv r]; rw [ numerator_hom_apply]; rw [ ←mul_one (φ (r /ₒ s))]; rw [ ←units.coe_one]; rw [ ←mul_right_inv (fS s)]; rw [ units.coe_mul]; rw [ ←mul_assoc]; rw [ ←hf]; rw [ ←huniv]; rw [ ←φ.map_mul]; rw [ numerator_hom_apply]; rw [ ore_localization.mul_cancel],
end
...
{ mul_comm := λ x y,
 ...
 induction x using ore_localization.ind with r₁ s₁,
 induction y using ore_localization.ind with r₂ s₂,
 rw [ore_div_mul_ore_div_comm]; rw [ ore_div_mul_ore_div_comm]; rw [ mul_comm r₁]; rw [ mul_comm s₁],
 end,
...
{ to_fun := numerator_hom,
 ...
 induction z using ore_localization.ind with r s,
 use (r, s), dsimp,
 rw [numerator_hom_apply]; rw [ numerator_hom_apply], simp
 end,
 eq_iff_exists' := λ r₁ r₂,
 begin
 dsimp, split,
 { intro h,
 rw [numerator_hom_apply] at h; rw [ numerator_hom_apply] at h; rw [ ore_div_eq_iff] at h,
 ...
 { rintro ⟨s, h⟩,
 rw [numerator_hom_apply]; rw [ numerator_hom_apply]; rw [ ore_div_eq_iff],
 refine ⟨s, s, _, _⟩,
...
begin
 ...
 { noncomm_ring, assoc_rw [hd]; assoc_rw [ hc], noncomm_ring },
...
begin
 ...
 simp only [submonoid.coe_mul], assoc_rw [hb]; assoc_rw [ hd],
 rw [←mul_assoc]; rw [ ←add_mul]; rw [ ore_div_eq_iff],
 use 1, use rd, split,
 { simp },
 { simp only [mul_one, submonoid.coe_one, submonoid.coe_mul] at ⊢ this, assoc_rw [hc]; assoc_rw [ this] },
end
...
begin
 ...
 use sc * sd, use rc * sd,
 dsimp, split,
 { rw [add_mul, add_mul, add_comm], assoc_rw [←hd]; assoc_rw [ hc], noncomm_ring },
 { rw [mul_assoc]; rw [ ←mul_assoc ↑sa]; rw [ ←hd]; rw [ hb], noncomm_ring }
end
...
begin
 ...
 rw [←ore_localization.expand]; rw [ subtype.coe_eq_of_eq_mk hd]; rw [ ←mul_assoc]; rw [ ←ore_localization.expand]; rw [ subtype.coe_eq_of_eq_mk hb],
 apply ore_localization.expand } },
 congr' 1,
 { rw [←ore_localization.expand']; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ ←ore_localization.expand']; rw [ ←ore_localization.expand'] },
 { simp_rw [←submonoid.coe_mul] at ha hd,
 rw [subtype.coe_eq_of_eq_mk hd]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ ←ore_localization.expand]; rw [ ←ore_localization.expand']; rw [ subtype.coe_eq_of_eq_mk ha]; rw [ ←ore_localization.expand],
 apply ore_localization.expand' }
end
...
by { rw [ore_localization.zero_def]; rw [ ore_div_eq_iff], exact ⟨s, 1, by simp⟩ }

protected lemma zero_add (x : R[S⁻¹]) : 0 + x = x :=
begin
 induction x using ore_localization.ind,
 rw [←zero_div_eq_zero]; rw [ add_ore_div], simp
end
...
{ add_comm := ore_localization.add_comm,
 ...
 add_zero := λ x, by rw [ore_localization.add_comm]; rw [ ore_localization.zero_add],
 .. ore_localization.has_add }

protected lemma zero_mul (x : R[S⁻¹]) : 0 * x = 0 :=
begin
 induction x using ore_localization.ind with r s,
 rw [ore_localization.zero_def]; rw [ ore_div_mul_char 0 r 1 s r 1 (by simp)], simp
end

protected lemma mul_zero (x : R[S⁻¹]) : x * 0 = 0 :=
begin
 induction x using ore_localization.ind with r s,
 rw [ore_localization.zero_def]; rw [ ore_div_mul_char r 0 s 1 0 1 (by simp)], simp
end
...
begin
 ...
 have hs₃rasb : ↑s₃ * (ra * sb) ∈ S, { rw [←mul_assoc]; rw [ ←ha], norm_cast, apply set_like.coe_mem },
...
{ map_zero' :=
 begin
 rw [monoid_hom.to_fun_eq_coe]; rw [ ore_localization.zero_def]; rw [ universal_mul_hom_apply],
 simp
 end,
 map_add' := λ x y,
 begin
 ...
 congr, skip, rw [←hf]; rw [ ←mul_assoc (f s₂)]; rw [ ←f.map_mul],
...
lift_expand (λ (r : R) (s : S), (- r) /ₒ s) $
 λ r t s ht, by rw [neg_mul_eq_neg_mul]; rw [ ←ore_localization.expand]
...
begin
 ...
 rwa [←h₂] at h₁; rwa [ mul_cancel_right_mem_non_zero_divisor (hS (set_like.coe_mem u))] at h₁; rwa [ eq_comm] at h₁,
end

lemma nontrivial_of_non_zero_divisors [nontrivial R] (hS : S ≤ R⁰) : nontrivial R[S⁻¹] :=
⟨⟨0, 1, λ h,
 begin
 rw [ore_localization.one_def] at h; rw [ ore_localization.zero_def] at h,
...
begin
 induction x using ore_localization.ind with r s,
 rw [ore_localization.inv_def]; rw [ ore_localization.one_def],
 by_cases hr : r = 0,
...
by { rw [ore_localization.zero_def]; rw [ ore_localization.inv_def], simp }
