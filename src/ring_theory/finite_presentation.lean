...
begin
 ...
 rw [h]; rw [ h1] },
 rw [ring_hom.ker_eq_comap_bot]; rw [ hco]; rw [ ← ideal.comap_comap]; rw [ ← ring_hom.ker_eq_comap_bot]; rw [ ring_hom.ker_coe_equiv (alg_equiv.to_ring_equiv e)]; rw [ ring_hom.ker_eq_comap_bot] }
end
...
begin
 ...
 { rw [alg_hom.to_ring_hom_eq_coe]; rw [ mv_polynomial.map_alg_hom_coe_ring_hom]; rw [ mv_polynomial.ker_map],
...
begin
 ...
 { rw [← algebra.range_top_iff_surjective]; rw [ ← algebra.adjoin_range_eq_range_aeval]; rw [ set.range_comp]; rw [ _root_.eq_top_iff]; rw [ ← @adjoin_adjoin_of_tower R A B]; rw [ adjoin_image]; rw [ adjoin_range_X]; rw [ algebra.map_top]; rw [ (algebra.range_top_iff_surjective _).mpr hf],
 exact subset_adjoin },
 { obtain ⟨t, ht⟩ := hRA.out,
 have := λ i : t, hf (algebra_map A B i),
 choose t' ht',
 have ht'' : algebra.adjoin R ((algebra_map A AX) '' t ∪ set.range (X : _ → AX)) = ⊤,
 { rw [adjoin_union_eq_adjoin_adjoin]; rw [ ← subalgebra.restrict_scalars_top R],
 congr' 1,
 swap, { exact subalgebra.is_scalar_tower_mid _ },
 rw [adjoin_algebra_map]; rw [ ht],
 apply subalgebra.restrict_scalars_injective R,
 rw [← adjoin_restrict_scalars]; rw [ adjoin_range_X]; rw [ subalgebra.restrict_scalars_top]; rw [ subalgebra.restrict_scalars_top] },
 ...
 all_goals
 { dsimp [g], rw [ring_hom.mem_ker]; rw [ alg_hom.to_ring_hom_eq_coe]; rw [ alg_hom.coe_to_ring_hom] },
 { rw [mv_polynomial.aeval_map_algebra_map]; rw [ ← aeval_unique],
 have := ideal.subset_span hx,
 rwa hs at this },
 { rw [map_sub]; rw [ mv_polynomial.aeval_map_algebra_map]; rw [ ← aeval_unique]; rw [ aeval_C]; rw [ ht']; rw [ subtype.coe_mk]; rw [ sub_self] } },
 apply leI.antisymm,
 intros x hx,
 rw [ring_hom.mem_ker] at hx; rw [ alg_hom.to_ring_hom_eq_coe] at hx; rw [ alg_hom.coe_to_ring_hom] at hx,
 ...
 { rw [algebra_map_eq]; rw [ ← sub_add_cancel (C x) (map (algebra_map R A) (t' ⟨x, hx⟩))]; rw [ add_comm],
 ...
 rw [map_add] at hx; rw [ aeval_map_algebra_map] at hx; rw [ ← aeval_unique] at hx; rw [ (show aeval (f ∘ X) q = 0, from leI hq)] at hx; rw [ add_zero] at hx,
...
begin
 ...
 rw [alg_hom.comp_algebra_map]; rw [ ← aeval_eq_eval₂_hom]; rw [ ← aeval_unique] },
 let s' := set.range g' ∪ aeval_h '' s,
 have leI : ideal.span s' ≤ f.to_ring_hom.ker,
 { rw ideal.span_le,
 rintros _ (⟨i, rfl⟩|⟨x, hx, rfl⟩),
 { change f (g' i) = 0, rw [map_sub]; rw [ ← hg]; rw [ hh']; rw [ sub_self], },
...
begin
 ...
 rw [← ideal.comap_comap]; rw [ ideal.map_comap_of_surjective (g : mv_polynomial (fin n) R →+* A) hg],
end
