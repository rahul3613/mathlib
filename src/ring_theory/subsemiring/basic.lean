...
⟨λ hx, add_submonoid.closure_induction (mem_closure_iff.1 hx)
 ...
 by rw [list.map_singleton]; rw [ list.sum_singleton]; rw [ ht2]⟩,
 ...
 by rw [list.prod_append]; rw [ ht2]; rw [ hu2]⟩))
 ⟨[], list.forall_mem_nil _, rfl⟩
 (λ x y ⟨L, HL1, HL2⟩ ⟨M, HM1, HM2⟩, ⟨L ++ M, list.forall_mem_append.2 ⟨HL1, HM1⟩,
 by rw [list.map_append]; rw [ list.sum_append]; rw [ HL2]; rw [ HM2]⟩),
...
set_like.ext'_iff.trans $ iff.trans (by rw [coe_srange]; rw [ coe_top]) set.range_iff_surjective
...
{ mul_comm := λ x y,
 ...
 (λ x y z h₁ h₂, by simp only [add_mul, mul_add, h₁, h₂])
 (λ x y z h₁ h₂, by rw [mul_assoc]; rw [ h₂]; rw [ ←mul_assoc]; rw [ h₁]; rw [ mul_assoc])
 (λ x y z h₁ h₂, by rw [←mul_assoc]; rw [ h₁]; rw [ mul_assoc]; rw [ h₂]; rw [ ←mul_assoc])
 end,
