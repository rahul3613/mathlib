...
begin
 classical,
 rw [← basis.coe_reindex]; rw [ discr_def]; rw [ trace_matrix_reindex]; rw [ det_reindex_self]; rw [ ← discr_def]
end
...
by rw [discr_def]; rw [ trace_matrix_of_matrix_vec_mul]; rw [ det_mul]; rw [ det_mul]; rw [ det_transpose]; rw [ mul_comm]; rw [ ← mul_assoc]; rw [ discr_def]; rw [ pow_two]
...
by rw [discr_def]; rw [ trace_matrix_of_matrix_mul_vec]; rw [ det_mul]; rw [ det_mul]; rw [ det_transpose]; rw [ mul_comm]; rw [ ← mul_assoc]; rw [ discr_def]; rw [ pow_two]
...
begin
 ...
 classical,
 rw [discr_def]; rw [ trace_matrix],
 simp_rw [← basis.mk_apply b.linear_independent this.ge],
 rw [← trace_matrix]; rw [ trace_matrix_of_basis]; rw [ ← bilin_form.nondegenerate_iff_det_ne_zero],
...
by rw [discr_def]; rw [ ring_hom.map_det]; rw [ ring_hom.map_matrix_apply]; rw [ trace_matrix_eq_embeddings_matrix_reindex_mul_trans]; rw [ det_mul]; rw [ det_transpose]; rw [ pow_two]
...
begin
 rw [discr_eq_det_embeddings_matrix_reindex_pow_two K E pb.basis e]; rw [ embeddings_matrix_reindex_eq_vandermonde]; rw [ det_transpose]; rw [ det_vandermonde]; rw [ ← prod_pow],
 congr, ext i,
 rw [← prod_pow]
end
...
begin
 rw [discr_power_basis_eq_prod' _ _ _ e],
 simp_rw [λ i j, neg_eq_neg_one_mul ((e j pb.gen- (e i pb.gen)) * (e i pb.gen- (e j pb.gen))), prod_mul_distrib],
 congr,
 ...
 rw [← nat.cast_sum]; rw [ ← @finset.sum_range ℕ _ pb.dim (λ i, i)]; rw [ sum_range_id ],
 have hn : n = pb.dim,
 { rw [← alg_hom.card K L E]; rw [ ← fintype.card_fin pb.dim],
 ...
 have hle : 1 ≤ pb.dim,
 { rw [← hn]; rw [ nat.one_le_iff_ne_zero]; rw [ ← zero_lt_iff]; rw [ finite_dimensional.finrank_pos_iff],
 apply_instance },
...
begin
 ...
 rw [fintype.card_fin]; rw [ alg_hom.card],
 ...
 { intro σ,
 rw [mem_roots]; rw [ is_root.def]; rw [ eval_map]; rw [ ← aeval_def]; rw [ aeval_alg_hom_apply],
 repeat { simp [minpoly.ne_zero (is_separable.is_integral K pb.gen)] } },

 apply (algebra_map K E).injective,
 rw [ring_hom.map_mul]; rw [ ring_hom.map_pow]; rw [ ring_hom.map_neg]; rw [ ring_hom.map_one]; rw [ discr_power_basis_eq_prod'' _ _ _ e],
 congr,
 rw [norm_eq_prod_embeddings]; rw [ prod_prod_Ioi_mul_eq_prod_prod_off_diag],
 conv_rhs { congr, skip, funext,
 rw [← aeval_alg_hom_apply]; rw [ aeval_root_derivative_of_splits (minpoly.monic (is_separable.is_integral K pb.gen)) (is_alg_closed.splits_codomain _) (hroots σ)]; rw [ ← finset.prod_mk _ (hnodup.erase _)] },
 ...
 rw [← power_basis.lift_equiv_apply_coe] at h; rw [ ← power_basis.lift_equiv_apply_coe] at h,
 ...
 { rw [aeval_def]; rw [ eval₂_eq_eval_map]; rw [ ← is_root.def]; rw [ ← mem_roots],
...
begin
 ...
 simpa },
 classical,
 rw [← (b.reindex (b.index_equiv b')).to_matrix_map_vec_mul b']; rw [ discr_of_matrix_vec_mul]; rw [ ← one_mul (discr ℚ b)]; rw [ basis.coe_reindex]; rw [ discr_reindex],
 congr,
 ...
 rw [ring_hom.map_mul]; rw [ hr]; rw [ hr']; rw [ ← det_mul]; rw [ basis.to_matrix_mul_to_matrix_flip]; rw [ det_one] },
 rw [← ring_hom.map_one (algebra_map ℤ ℚ)]; rw [ ← hr],
...
begin
 ...
 rw [← smul_assoc]; rw [ ← hr]; rw [ algebra_map_smul],
 refine subalgebra.smul_mem _ _ _,
 rw [B.basis_eq_pow i],
 refine subalgebra.pow_mem _ (subset_adjoin (set.mem_singleton _)) _},
 intro i,
 rw [← H] at cramer; rw [ ← mul_vec_smul] at cramer,
 replace cramer := congr_arg (mul_vec (trace_matrix K B.basis)⁻¹) cramer,
 rw [mul_vec_mul_vec] at cramer; rw [ nonsing_inv_mul _ hinv] at cramer; rw [ mul_vec_mul_vec] at cramer; rw [ nonsing_inv_mul _ hinv] at cramer; rw [ one_mul_vec] at cramer; rw [ one_mul_vec] at cramer,
