...
begin
 ...
 { rw [prod_insert his] at hx; rw [ ← mul_assoc] at hx,
 ...
 from hpi.dvd_or_dvd ⟨a, by rw [← hbc]; rw [ mul_comm]⟩,
 { rw [mul_assoc] at hbc; rw [ mul_comm a] at hbc; rw [ mul_right_inj' hpi.ne_zero] at hbc,
 exact ⟨insert i t, u, d, c, by rw [insert_union]; rw [ htus],
 disjoint_insert_left.2 ⟨hiu, htu⟩,
 by simp [hbc, prod_insert hit, mul_assoc, mul_comm, mul_left_comm]⟩ },
 { rw [← mul_assoc] at hbc; rw [ mul_right_comm b] at hbc; rw [ mul_left_inj' hpi.ne_zero] at hbc,
 exact ⟨t, insert i u, b, d, by rw [union_insert]; rw [ htus],
...
begin
 ...
 exact ⟨t.card, u.card, b, c, by rw [← card_disjoint_union htu]; rw [ htus]; rw [ card_range], by simp⟩,
end
