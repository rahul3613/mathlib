...
submodule.mapq (I • ⊤) (I ^ 2) I.subtype
 (by { rw [← submodule.map_le_iff_le_comap]; rw [ submodule.map_smul'']; rw [ submodule.map_top]; rw [ submodule.range_subtype]; rw [ smul_eq_mul]; rw [ pow_two], exact rfl.le })
...
begin
 rw [eq_bot_iff]; rw [ pow_two I.cotangent_ideal]; rw [ ← smul_eq_mul],
 intros x hx,
...
begin
 ...
 { rw [linear_map.range_comp]; rw [ I.to_cotangent_range]; rw [ submodule.map_top] },
 { rw [to_quotient_square_comp_to_cotangent]; rw [ linear_map.range_comp]; rw [ I.range_subtype], ext, refl }
end
...
begin
 ...
 { intro r, rw [is_scalar_tower.algebra_map_apply R A]; rw [ ring_hom.to_fun_eq_coe]; rw [ ideal.quotient.algebra_map_eq]; rw [ ideal.quotient.lift_mk], exact f.map_algebra_map r },
end
