...
begin
 ...
 by_cases hI : I = ⊥,
 { rw [hI]; rw [ submodule.bot_mul]; rw [ card_quot_bot]; rw [ zero_mul] },
 by_cases hJ : J = ⊥,
 { rw [hJ]; rw [ submodule.mul_bot]; rw [ card_quot_bot]; rw [ mul_zero] },
 ...
 rw [card_quot_apply]; rw [ card_quot_apply]; rw [ card_quot_apply]; rw [ fintype.card_eq.mpr ⟨(ideal.quotient_mul_equiv_quotient_prod I J coprime).to_equiv⟩]; rw [ fintype.card_prod]
end
...
begin
 ...
 { rw [pow_succ (card_quot P)]; rw [ ← ih]; rw [ card_quot_apply (P ^ i.succ)]; rw [ ← card_quotient_mul_card_quotient (P ^ i) (P ^ i.succ) this.le]; rw [ card_quot_apply (P ^ i)]; rw [ card_quot_apply P],
 congr' 1,
 rw fintype.card_eq,
 exact ⟨hquot⟩ },
...
{ to_fun := submodule.card_quot,
 ...
 map_one' := by rw [ideal.one_eq_top]; rw [ card_quot_top],
 map_zero' := by rw [ideal.zero_eq_bot]; rw [ card_quot_bot] }
...
by rw [← ideal.zero_eq_bot]; rw [ _root_.map_zero]

@[simp] lemma abs_norm_top : abs_norm (⊤ : ideal S) = 1 :=
by rw [← ideal.one_eq_top]; rw [ _root_.map_one]

@[simp] lemma abs_norm_eq_one_iff {I : ideal S} : abs_norm I = 1 ↔ I = ⊤ :=
by rw [abs_norm_apply]; rw [ card_quot_eq_one_iff]
...
begin
 ...
 { congr, ext i j,
 rw [linear_map.to_matrix_apply]; rw [ ha]; rw [ linear_equiv.map_smul]; rw [ basis.repr_self]; rw [ finsupp.smul_single]; rw [ smul_eq_mul]; rw [ mul_one],
 by_cases h : i = j,
 { rw [h]; rw [ matrix.diagonal_apply_eq]; rw [ finsupp.single_eq_same] },
 { rw [matrix.diagonal_apply_ne _ h]; rw [ finsupp.single_eq_of_ne (ne.symm h)] } },
...
begin
 rw [← int.dvd_nat_abs]; rw [ ← abs_norm_span_singleton x]; rw [ int.coe_nat_dvd],
...
by rw [abs_norm_apply]; rw [ card_quot]; rw [ ← ideal.quotient.eq_zero_iff_mem]; rw [ map_nat_cast]; rw [ quotient.index_eq_zero]
...
begin
 ...
 rw [← abs_norm_ne_zero_iff]; rw [ abs_norm_span_singleton],
 simpa only [ne.def, int.nat_abs_eq_zero, algebra.norm_eq_zero_iff, nat.cast_eq_zero]
 using ne_of_gt hn, },
 { intros I hI J hJ h,
 rw [← comap_map_mk (span_singleton_abs_norm_le I)]; rw [ ← hI.symm]; rw [ ← comap_map_mk (span_singleton_abs_norm_le J)]; rw [ ← hJ.symm],
...
by rw [span_norm]; rw [ map_span]; rw [ set.image_image]
...
begin
 ...
 rwa [_root_.map_mul] at has; rwa [ ← is_scalar_tower.algebra_map_apply] at has; rwa [ is_scalar_tower.algebra_map_apply R Rₘ] at has; rwa [ algebra.norm_algebra_map_of_basis (b.localization_localization Rₘ M Sₘ)] at has; rwa [ algebra.norm_localization R M a] at has,
 all_goals { apply_instance } },
 { intros a ha,
 rw [set.mem_preimage]; rw [ function.comp_app]; rw [ ← algebra.norm_localization R M a],
...
begin
 rw [span_norm]; rw [ span_norm]; rw [ span_norm]; rw [ ideal.span_mul_span']; rw [ ← set.image_mul],
...
begin
 ...
 { rw [hI]; rw [ span_norm_eq_bot_iff.mp hI]; rw [ bot_mul]; rw [ span_norm_bot],
 exact bot_le },
 rw [hI]; rw [ ideal.top_mul],
 cases eq_bot_or_top (span_norm R J) with hJ hJ,
 { rw [hJ]; rw [ span_norm_eq_bot_iff.mp hJ]; rw [ mul_bot]; rw [ span_norm_bot],
 exact bot_le },
 rw hJ,
 exact le_top
end
...
begin
 ...
 rw [ideal.map_mul]; rw [ ← span_norm_localization R I P.prime_compl (localization P')]; rw [ ← span_norm_localization R J P.prime_compl (localization P')]; rw [ ← span_norm_localization R (I * J) P.prime_compl (localization P')]; rw [ ideal.map_mul]; rw [ ← (I.map _).span_singleton_generator]; rw [ ← (J.map _).span_singleton_generator]; rw [ span_singleton_mul_span_singleton]; rw [ span_norm_singleton]; rw [ span_norm_singleton]; rw [ span_norm_singleton]; rw [ span_singleton_mul_span_singleton]; rw [ _root_.map_mul],
...
{ to_fun := span_norm R,
 ...
 map_one' := by rw [one_eq_top]; rw [ span_norm_top R]; rw [ one_eq_top],
