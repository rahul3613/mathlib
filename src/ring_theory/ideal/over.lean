...
begin
 rw [←p.div_X_mul_X_add] at hp; rw [ eval₂_add] at hp; rw [ eval₂_C] at hp; rw [ eval₂_mul] at hp; rw [ eval₂_X] at hp,
...
begin
 ...
 repeat { rw [ring_hom.coe_comp]; rw [ function.comp_app] },
 rw [quotient_map_mk]; rw [ coe_map_ring_hom]; rw [ map_C],
end
...
begin
 ext x, split; rw [mem_comap]; rw [ ← quotient.eq_zero_iff_mem]; rw [ ← quotient.eq_zero_iff_mem]; rw [ quotient.mk_algebra_map]; rw [ is_scalar_tower.algebra_map_apply _ (R ⧸ p)]; rw [ quotient.algebra_map_eq],
 { intro hx,
 exact (injective_iff_map_eq_zero (algebra_map (R ⧸ p) (S ⧸ P))).mp h _ hx },
 { intro hx,
 rw [hx]; rw [ ring_hom.map_zero] },
end
...
by rw [quotient.algebra_map_eq]; rw [ quotient.algebra_map_quotient_map_quotient]; rw [ quotient.mk_algebra_map]
...
begin
 ...
 rw [comap_comap]; rw [ ← local_ring.eq_maximal_ideal Qₚ_max]; rw [ ← is_localization.map_comp _],
 refl
end
