...
show a - b ∈ ker f, by rw [mem_ker]; rw [ map_sub]; rw [ h]; rw [ sub_self]
...
begin
 rw [ring_hom.injective_iff_ker_eq_bot]; rw [ ring_hom.ker_eq_bot_iff_eq_zero],
 intros u hu,
...
by ext; rw [ring_hom.ker]; rw [ mem_comap]; rw [ submodule.mem_bot]; rw [ quotient.eq_zero_iff_mem]

lemma map_mk_eq_bot_of_le {I J : ideal R} (h : I ≤ J) : I.map (J^.quotient.mk) = ⊥ :=
by { rw [map_eq_bot_iff_le_ker]; rw [ mk_ker], exact h }
...
begin
 ...
 rw [ring_hom.mem_ker] at hx; rw [ ← hy] at hx; rw [ ideal.quotient.lift_mk] at hx; rw [ ← ring_hom.mem_ker] at hx,
 rw [← hy]; rw [ mem_map_iff_of_surjective I^.quotient.mk quotient.mk_surjective],
 exact ⟨y, hx, rfl⟩ },
 { intro hx,
 rw mem_map_iff_of_surjective I^.quotient.mk quotient.mk_surjective at hx,
 obtain ⟨y, hy⟩ := hx,
 rw [ring_hom.mem_ker]; rw [ ← hy.right]; rw [ ideal.quotient.lift_mk]; rw [ ← (ring_hom.mem_ker f)],
 exact hy.left },
end
...
by rw [← mem_comap]; rw [ comap_map_of_surjective (quotient.mk I) quotient.mk_surjective]; rw [ ← ring_hom.ker_eq_comap_bot]; rw [ mk_ker]
...
by rw [mem_quotient_iff_mem_sup]; rw [ sup_eq_left.mpr hIJ]
...
{ commutes' := λ r, begin
 ...
 rw [this]; rw [ ideal.quotient.algebra_map_eq]; rw [ ring_hom.to_fun_eq_coe]; rw [ ideal.quotient.lift_mk]; rw [ alg_hom.coe_to_ring_hom]; rw [ algebra.algebra_map_eq_smul_one]; rw [ algebra.algebra_map_eq_smul_one]; rw [ map_smul]; rw [ map_one],
 end
...
begin
 ...
 rw [← alg_hom.map_smul]; rw [ quotient.mkₐ_eq_mk]; rw [ ring_hom.ker_lift_mk],
...
begin
 ...
 rw [quotient_map_mk] at ha; rw [ quotient.eq_zero_iff_mem] at ha,
...
by rw [ring_hom.ker_eq_comap_bot]; rw [ quot_quot_mk]; rw [ ← comap_comap]; rw [ ← ring_hom.ker]; rw [ mk_ker]; rw [ comap_map_of_surjective (ideal.quotient.mk I) (quotient.mk_surjective)]; rw [ ← ring_hom.ker]; rw [ mk_ker]; rw [ sup_comm]
