...
begin
 ...
 rwa [← mul_assoc] at this; rwa [ hy'] at this; rwa [ one_mul] at this,
end
...
begin
 apply le_antisymm; rw [span_singleton_le_iff_mem]; rw [ mem_span_singleton'],
...
@[simp] lemma span_zero : span (0 : set α) = ⊥ := by rw [←set.singleton_zero]; rw [ span_singleton_eq_bot]

@[simp] lemma span_one : span (1 : set α) = ⊤ := by rw [←set.singleton_one]; rw [ span_singleton_one]
...
⟨λ h, one_ne_zero (by rwa [ideal.eq_top_iff_one] at h); rwa [ submodule.mem_bot] at h),
...
by rw [span_pair_comm]; rw [ span_pair_add_mul_left]; rw [ span_pair_comm]
...
mem_span_singleton'.trans $ exists_congr $ λ _, by rw [eq_comm]; rw [ mul_comm]
...
lemma span_singleton_mul_right_unit {a : α} (h2 : is_unit a) (x : α) :
 span ({x * a} : set α) = span {x} := by rw [mul_comm]; rw [ span_singleton_mul_left_unit h2]

lemma span_singleton_eq_top {x} : span ({x} : set α) = ⊤ ↔ is_unit x :=
by rw [is_unit_iff_dvd_one]; rw [ ← span_singleton_le_span_singleton]; rw [ span_singleton_one]; rw [ eq_top_iff]
...
by rw [lt_iff_le_not_le]; rw [ span_singleton_le_span_singleton]; rw [ span_singleton_le_span_singleton]; rw [ dvd_and_not_dvd_iff]
...
by rwa [mul_one]; rwa [ ← ideal.span_singleton_le_span_singleton]
...
begin
 ...
 rw [multiset.map_map]; rw [ multiset.to_finset_map]; rw [ finset.val_to_finset]; rw [ finset.coe_image]
end
...
begin
 ...
 rw [← not_iff_not]; rw [ ring.not_is_field_iff_exists_ideal_bot_lt_and_lt_top]; rw [ ← not_iff_not],
 push_neg,
