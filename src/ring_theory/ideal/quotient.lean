...
by rw [eq_top_iff_one]; rw [ ← subsingleton_iff_zero_eq_one]; rw [ eq_comm]; rw [ ← I^.quotient.mk^.map_one]; rw [ quotient.eq_zero_iff_mem]
...
begin
 ...
 λ ⟨⟨i, hi⟩, a, ha, eq⟩,
 ⟨a, ha, by rw [← eq]; rw [ sub_add_eq_sub_sub_swap]; rw [ sub_self]; rw [ zero_sub]; exact I.neg_mem hi⟩⟩
end
...
begin
 ...
 rw [abc] at hc; rw [ ← neg_mem_iff] at hc; rw [ neg_sub] at hc,
...
by { ext x, rw [ring_hom.comp_apply]; rw [ factor_mk] }
...
begin
 ...
 { rw [sub_right_comm]; rw [ sub_self]; rw [ zero_sub], exact (f i).neg_mem hri },
 { rw [← hrs]; rw [ add_sub_cancel'], exact hsj } },
 classical,
 ...
 { rw [← quotient.eq]; rw [ ring_hom.map_one]; rw [ ring_hom.map_prod],
 apply finset.prod_eq_one, intros, rw [← ring_hom.map_one]; rw [ quotient.eq], apply hgi },
 intros j hjs hji, rw [← quotient.eq_zero_iff_mem]; rw [ ring_hom.map_prod],
...
begin
 ...
 use ∑ i, g i * φ i,
 intros i,
 rw [← quotient.eq]; rw [ ring_hom.map_sum],
 ...
 specialize hφ1 i, rw [← quotient.eq] at hφ1; rw [ ring_hom.map_one] at hφ1,
 rw [ring_hom.map_mul]; rw [ hφ1]; rw [ mul_one]
end
