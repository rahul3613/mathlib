...
begin
 ...
 { exact smul_zero _ },
 { intros x y hx hy, rw [smul_add]; rw [ hx]; rw [ hy]; rw [ zero_add] },
 { intros a x hx, rw [smul_comm]; rw [ hx]; rw [ smul_zero] } }
end
...
⟨λ hx, smul_induction_on hx
 ...
 ⟨y1 + y2, I.add_mem hyi1 hyi2, by rw [add_smul]; rw [ hy1]; rw [ hy2]⟩),
...
by rw [mul_comm]; rw [ annihilator_mul]
...
begin
 ...
 conv_lhs { rw [← span_eq N]; rw [ span_smul_span] },
 simpa
end
...
by rw [← I.span_eq]; rw [ submodule.span_smul_span]; rw [ I.span_eq]; refl
...
by rw [← submodule.mem_ideal_smul_span_iff_exists_sum]; rw [ ← set.image_eq_range]
...
by erw [submodule.one_eq_range]; erw [ linear_map.range_id]
...
begin
 ...
 { intro _, rw [finset.prod_empty]; rw [ finset.prod_empty]; rw [ one_eq_top], exact submodule.mem_top },
 { intros a s ha IH h,
...
by { unfold span, rw [submodule.span_mul_span]; rw [ set.singleton_mul_singleton], }
...
begin
 rw [← finset.inf_univ_eq_infi]; rw [ finset_inf_span_singleton],
 rwa [finset.coe_univ]; rwa [ set.pairwise_univ]
end
...
begin
 ...
 rw [← hu.some_spec] at h1; rw [ ← hv.some_spec] at h1,
 exact ⟨_, _, h1⟩ },
 { exact λ ⟨u, v, h1⟩,
...
by { rw sup_comm at h, rw [sup_comm]; rw [ sup_mul_eq_of_coprime_left h]; rw [ sup_comm] }

lemma mul_sup_eq_of_coprime_right (h : K ⊔ J = ⊤) : (I * K) ⊔ J = I ⊔ J :=
by { rw sup_comm at h, rw [sup_comm]; rw [ sup_mul_eq_of_coprime_right h]; rw [ sup_comm] }
...
(by rw [one_eq_top]; rw [ sup_top_eq]) h
...
by { rw [← finset.card_range n]; rw [ ← finset.prod_const], exact sup_prod_eq_top (λ _ _, h) }

lemma pow_sup_eq_top {n : ℕ} (h : I ⊔ J = ⊤) : (I ^ n) ⊔ J = ⊤ :=
by { rw [← finset.card_range n]; rw [ ← finset.prod_const], exact prod_sup_eq_top (λ _ _, h) }
...
begin
 induction n,
 { rw [pow_zero]; rw [ pow_zero], exact rfl.le },
 { rw [pow_succ]; rw [ pow_succ], exact ideal.mul_mono e n_ih }
end
...
⟨λ hij, or_iff_not_imp_left.mpr (λ I_ne_bot, J.eq_bot_iff.mpr (λ j hj,
 ...
 λ h, by cases h; rw [← ideal.mul_bot]; rw [ h]; rw [ ideal.mul_comm]⟩
...
by rw [le_antisymm_iff]; rw [ and_iff_left le_radical]; rw [ is_radical]
...
have is_prime m, from ⟨by rintro rfl; rw radical_top at hrm; exact hrm trivial,
 ...
 hrm ⟨n + k, by rw [pow_add]; rw [ ← hpqrn]; rw [ ← hcxq]; rw [ ← hfgrk]; rw [ ← hdyg]; rw [ add_mul]; rw [ mul_add (c*x)]; rw [ mul_assoc c x (d*y)]; rw [ mul_left_comm x]; rw [ ← mul_assoc];
...
nat.rec_on n one_eq_top $ λ n ih, by rw [pow_succ]; rw [ ih]; rw [ top_mul]
variables {R}
...
begin
 ...
 rw [finset.coe_empty] at h; rw [ set.bUnion_empty] at h; rw [ set.union_empty] at h; rw [ subset_union] at h,
 ...
 { rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_assoc] at h; rw [ set.union_right_comm ↑(f a)] at h,
 ...
 { rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_assoc] at h; rw [ set.union_assoc ↑(f a)] at h,
 ...
 { rw [finset.inf_empty]; rw [ inf_top_eq]; rw [ hp.1.inf_le]; rw [ hp.1.inf_le]; rw [ not_or_distrib]; rw [ not_or_distrib],
 exact ⟨⟨Hi, Ha⟩, Hb⟩ },
 ...
 rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h,
...
begin
 ...
 rw [finset.coe_insert] at h; rw [ finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_assoc] at h; rw [ subset_union_prime' hp'] at h; rw [ bex_def] at h,
 rwa [finset.exists_mem_insert]; rwa [ finset.exists_mem_insert] },
 { have hp' : ∀ j ∈ t, is_prime (f j),
 { intros j hj, refine hp j (finset.mem_insert_of_mem hj) _ _;
 unfreezingI { rintro rfl }; solve_by_elim only [finset.mem_insert_of_mem, *], },
 rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_self (f a : set R)] at h; rw [ subset_union_prime' hp'] at h; rw [ ← or_assoc] at h; rw [ or_self] at h; rw [ bex_def] at h,
 ...
 { intros j hj, refine hp j (finset.mem_insert_of_mem hj) _ _;
 unfreezingI { rintro rfl }; solve_by_elim only [finset.mem_insert_of_mem, *], },
 rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_self (f b : set R)] at h; rw [ subset_union_prime' hp'] at h; rw [ ← or_assoc] at h; rw [ or_self] at h; rw [ bex_def] at h,
 rwa finset.exists_mem_insert },
 cases s.eq_empty_or_nonempty with hse hsne,
 { substI hse, rw [finset.coe_empty] at h; rw [ set.bUnion_empty] at h; rw [ set.subset_empty_iff] at h,
 ...
 { intros j hj, refine hp j (finset.mem_insert_of_mem hj) _ _;
 unfreezingI { rintro rfl }; solve_by_elim only [finset.mem_insert_of_mem, *], },
 rw [finset.coe_insert] at h; rw [ set.bUnion_insert] at h; rw [ ← set.union_self (f i : set R)] at h; rw [ subset_union_prime' hp'] at h; rw [ ← or_assoc] at h; rw [ or_self] at h; rw [ bex_def] at h,
 rwa finset.exists_mem_insert } }
end
...
is_unit_iff_dvd_one.trans ((@one_eq_top R _).symm ▸
 ⟨λ h, eq_top_iff.mpr (ideal.le_of_dvd h), λ h, ⟨⊤, by rw [mul_top]; rw [ h]⟩⟩)

instance unique_units : unique ((ideal R)ˣ) :=
{ default := 1,
 uniq := λ u, units.ext
 (show (u : ideal R) = 1, by rw [is_unit_iff.mp u.is_unit]; rw [ one_eq_top]) }
...
(ne_top_iff_one _).2 $ by rw [mem_comap]; rw [ map_one];
...
begin
 ...
 rintros x ⟨x, hx, rfl⟩,
 rw [set_like.mem_coe]; rw [ mem_comap]; rw [ hf hx],
 exact hx,
end
...
⟨comap_ne_top f H.ne_top,
 λ x y h, H.mem_or_mem $ by rwa [mem_comap] at h⟩; rwa [ map_mul] at h⟩
...
begin
 ...
 { rintros _ ⟨x, hx, rfl⟩,
 rw [← mul_one (algebra_map R S x)]; rw [ ← algebra.smul_def],
...
le_antisymm (assume r h, let ⟨s, hsi, hfsr⟩ := mem_image_of_mem_map_of_surjective f hf h in
 submodule.mem_sup.2 ⟨s, hsi, r - s, (submodule.mem_bot S).2 $ by rw [map_sub]; rw [ hfsr]; rw [ sub_self],
...
by { rw [←hK.radical]; rw [ comap_radical], apply radical_is_radical }
...
begin
 induction n,
 { rw [pow_zero]; rw [ pow_zero]; rw [ ideal.one_eq_top]; rw [ ideal.one_eq_top], exact rfl.le },
 { rw [pow_succ]; rw [ pow_succ], exact (ideal.mul_mono_right n_ih).trans (ideal.le_comap_mul f) }
end
...
begin
 dsimp [finsupp_total],
 rw [finsupp.total_apply]; rw [ finsupp.sum_map_range_index],
 exact λ _, zero_smul _ _
end

lemma finsupp_total_apply_eq_of_fintype [fintype ι] (f : ι →₀ I) :
 finsupp_total ι M I v f = ∑ i, (f i : R) • v i :=
by { rw [finsupp_total_apply]; rw [ finsupp.sum_fintype], exact λ _, zero_smul _ _ }
...
begin
 ...
 rw [finsupp_total_apply]; rw [ finsupp.sum_map_range_index],
...
b.ext (λ i, by erw [basis.constr_basis]; erw [ function.comp_app]; erw [ basis_span_singleton_apply]; erw [ linear_map.mul_apply'])
...
by rw [associates.mk_ne_zero]; rw [ ideal.zero_eq_bot]; rw [ ne.def]; rw [ ideal.span_singleton_eq_bot]
...
by rw [ker]; rw [ ideal.mem_comap]; rw [ submodule.mem_bot]
...
by rw [ring_hom.ker_eq_comap_bot]; rw [ ideal.comap_comap]; rw [ ring_hom.ker_eq_comap_bot]

include rcf
/-- If the target is not the zero ring, then one is not in the kernel.-/
lemma not_one_mem_ker [nontrivial S] (f : F) : (1:R) ∉ ker f :=
by { rw [mem_ker]; rw [ map_one], exact one_ne_zero }
...
by { rw [set_like.ext'_iff]; rw [ ker_eq]; rw [ set.ext_iff], exact injective_iff_map_eq_zero' f }

lemma ker_eq_bot_iff_eq_zero : ker f = ⊥ ↔ ∀ x, f x = 0 → x = 0 :=
by { rw [← injective_iff_map_eq_zero f]; rw [ injective_iff_ker_eq_bot] }
...
by rw [mem_ker]; rw [ map_sub]; rw [ sub_eq_zero]
...
lemma ker_is_prime {F : Type*} [ring R] [ring S] [is_domain S] [ring_hom_class F R S]
 (f : F) : (ker f).is_prime :=
⟨by { rw [ne.def]; rw [ ideal.eq_top_iff_one], exact not_one_mem_ker f },
...
by rw [ring_hom.ker]; rw [ eq_bot_iff]; rw [ map_le_iff_le_comap]
...
begin
 ...
 have : x - x' ∈ J,
 { apply h J hJ,
 rw [ring_hom.mem_ker]; rw [ map_sub]; rw [ hx]; rw [ sub_self] },
...
lemma map_eq_bot_iff_of_injective {I : ideal R} {f : F} (hf : function.injective f) :
 I.map f = ⊥ ↔ I = ⊥ :=
by rw [map_eq_bot_iff_le_ker]; rw [ (ring_hom.injective_iff_ker_eq_bot f).mp hf]; rw [ le_bot_iff]
...
by rw [← (comap_injective_of_surjective f hf).eq_iff]; rw [ comap_map_of_surjective f hf]; rw [ comap_map_of_surjective f hf]; rw [ ring_hom.ker_eq_comap_bot]
...
begin
 rw [radical_eq_Inf]; rw [ radical_eq_Inf],
...
{ to_fun := λ b, g (f_inv b),
 map_one' :=
 begin
 rw [← g.map_one]; rw [ ← sub_eq_zero]; rw [ ← g.map_sub]; rw [ ← g.mem_ker],
 apply hg,
 rw [f.mem_ker]; rw [ f.map_sub]; rw [ sub_eq_zero]; rw [ f.map_one],
 exact hf 1
 end,
 map_mul' :=
 begin
 intros x y,
 rw [← g.map_mul]; rw [ ← sub_eq_zero]; rw [ ← g.map_sub]; rw [ ← g.mem_ker],
 apply hg,
 rw [f.mem_ker]; rw [ f.map_sub]; rw [ sub_eq_zero]; rw [ f.map_mul],
 simp only [hf _],
 end,
