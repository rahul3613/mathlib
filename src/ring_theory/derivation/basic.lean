...
by rw [←mul_one r]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_one]; rw [ ←smul_def]; rw [ map_smul]; rw [ map_one_eq_zero]; rw [ smul_zero]

@[simp] lemma map_coe_nat (n : ℕ) : D (n : A) = 0 :=
by rw [← nsmul_one]; rw [ D.map_smul_of_tower n]; rw [ map_one_eq_zero]; rw [ smul_zero]

@[simp] lemma leibniz_pow (n : ℕ) : D (a ^ n) = n • a ^ (n - 1) • D a :=
begin
 induction n with n ihn,
 { rw [pow_zero]; rw [ map_one_eq_zero]; rw [ zero_smul] },
 { rcases (zero_le n).eq_or_lt with (rfl|hpos),
 { rw [pow_one]; rw [ one_smul]; rw [ pow_zero]; rw [ one_smul] },
 { have : a * a ^ (n - 1) = a ^ n, by rw [← pow_succ]; rw [ nat.sub_add_cancel hpos],
...
⟨λ r D,
 { to_linear_map := r • D,
 map_one_eq_zero' := by rw [linear_map.smul_apply]; rw [ coe_fn_coe]; rw [ D.map_one_eq_zero]; rw [ smul_zero],
...
by rw [← zsmul_one]; rw [ D.map_smul_of_tower n]; rw [ map_one_eq_zero]; rw [ smul_zero]
...
begin
 ...
 ... = a • D (a * b) : by rw [leibniz]; rw [ smul_add]; rw [ add_comm]
 ... = 0 : by rw [h]; rw [ map_one_eq_zero]; rw [ smul_zero]
end
