...
begin
 intro x,
 change f₁ x - f₂ x ∈ I,
 rw [← ideal.quotient.eq]; rw [ ← ideal.quotient.mkₐ_eq_mk R]; rw [ ← alg_hom.comp_apply]; rw [ e],
 refl,
end
...
begin
 ...
 { rw e, ext, refl },
 { ext, change f 1 - algebra_map A B 1 = 0, rw [map_one]; rw [ map_one]; rw [ sub_self] },
 { intros x y,
...
{ to_fun := λ x, f x + algebra_map A B x,
 map_one' := by rw [map_one]; rw [ f.map_one_eq_zero]; rw [ submodule.coe_zero]; rw [ zero_add],
 map_mul' := λ x y, begin
 have : (f x : B) * (f y) = 0,
 { rw [← ideal.mem_bot]; rw [ ← hI]; rw [ pow_two], convert (ideal.mul_mem_mul (f x).2 (f y).2) using 1 },
...
by { rw [lift_of_derivation_to_square_zero_apply]; rw [ map_add]; rw [ ideal.quotient.eq_zero_iff_mem.mpr (d x).prop]; rw [ zero_add], refl }
