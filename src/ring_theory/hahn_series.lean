...
by rw [support]; rw [ support_nonempty_iff]; rw [ ne.def]; rw [ coeff_fun_eq_zero_iff]
...
λ r s rs, by rw [← single_coeff_same a r]; rw [ ← single_coeff_same a s]; rw [ rs]
...
⟨begin
 ...
 rw [← single_coeff_same (arbitrary Γ) r]; rw [ con]; rw [ single_coeff_same],
end⟩
...
begin
 ...
 { simp },
 contrapose! hi,
 rw [←ne.def] at hi; rw [ ←mem_support] at hi,
 rw [order_of_ne hx],
...
λ x, { coeff := λ (b : Γ'),
 ...
 contrapose! hb,
 rw [function.mem_support]; rw [ dif_neg hb]; rw [ not_not],
 end) }
...
begin
 ...
 { rw [dif_neg]; rw [ not_not.1 (λ c, ha ((mem_support _ _).2 c))],
 contrapose! ha,
...
begin
 intros g hg,
 contrapose! hg,
 rw [mem_support]; rw [ emb_domain_notin_image_support hg]; rw [ not_not],
end
...
begin
 ...
 by_cases h : g' = f g,
 { simp [h] },
 rw [emb_domain_notin_image_support]; rw [ single_coeff_of_ne h],
 by_cases hr : r = 0,
 { simp [hr] },
 rwa [support_single_of_ne hr]; rwa [ set.image_singleton]; rwa [ set.mem_singleton_iff],
end
...
λ x y xy, begin
 ext g,
 rw [ext_iff] at xy; rw [ function.funext_iff] at xy,
 have xyg := xy (f g),
 rwa [emb_domain_coeff] at xyg; rwa [ emb_domain_coeff] at xyg,
end
...
begin
 ...
 { rw [subsingleton.elim (1 : hahn_series Γ R) 0]; rw [ order_zero] },
...
begin
 ...
 rw [sum_congr _ (λ _ _, rfl)]; rw [ sum_empty],
 ext ⟨a1, a2⟩,
...
begin
 by_cases hx : x = 0, { simp [hx], },
 by_cases hy : y = 0, { simp [hy], },
 rw [order_of_ne hx]; rw [ order_of_ne hy]; rw [ mul_coeff]; rw [ finset.add_antidiagonal_min_add_min]; rw [ finset.sum_singleton],
end
...
begin
 ext b,
 rw [mul_coeff_left' (x.is_pwo_support.add y.is_pwo_support) support_mul_subset_add_support]; rw [ mul_coeff_right' (y.is_pwo_support.add z.is_pwo_support) support_mul_subset_add_support],
...
{ eq_zero_or_eq_zero_of_mul_eq_zero := λ x y xy, begin
 ...
 right,
 contrapose! xy,
 rw [hahn_series.ext_iff]; rw [ function.funext_iff]; rw [ not_forall],
...
begin
 ...
 { rw [order_of_ne hx]; rw [ order_of_ne hy]; rw [ order_of_ne (mul_ne_zero hx hy)]; rw [ ← set.is_wf.min_add],
...
begin
 ext x,
 by_cases h : x = a + b,
 { rw [h]; rw [ mul_single_coeff_add],
 simp },
 { rw [single_coeff_of_ne h]; rw [ mul_coeff]; rw [ sum_eq_zero],
...
{ to_fun := single 0,
 ...
 map_mul' := λ x y, by rw [single_mul_single]; rw [ zero_add] }
...
begin
 intros r s rs,
 rw [ext_iff] at rs; rw [ function.funext_iff] at rs,
 have h := rs 0,
 rwa [C_apply] at h; rwa [ single_coeff_same] at h; rwa [ C_apply] at h; rwa [ single_coeff_same] at h,
end
...
begin
 ...
 exact ⟨i, j, h1, rfl⟩ } },
 { rw [emb_domain_notin_range hg]; rw [ eq_comm],
 contrapose! hg,
...
⟨⟨⊥, ⊤, begin
 ...
 intros x,
 rw [ext_iff]; rw [ function.funext_iff]; rw [ not_forall],
 refine ⟨a, _⟩,
 rw [single_coeff_same]; rw [ algebra_map_apply]; rw [ C_apply]; rw [ single_coeff_of_ne ha],
 exact zero_ne_one
end⟩⟩
...
begin
 ...
 { simp, refl },
 rw [pow_succ]; rw [ ih]; rw [ of_power_series_X]; rw [ mul_comm]; rw [ single_mul_single]; rw [ one_mul]; rw [ nat.cast_succ]
end
...
{ commutes' := λ r, begin
 ...
 rw [power_series.coeff_C]; rw [ if_neg n.succ_ne_zero] }
 end,
 .. to_power_series }
...
add_valuation.of (λ x, if x = (0 : hahn_series Γ R) then (⊤ : with_top Γ) else x.order)
 ...
 by_cases hxy : x + y = 0,
 { simp [hxy] },
 rw [if_neg hxy]; rw [ ← with_top.coe_min]; rw [ with_top.coe_le_coe],
 ...
 by_cases hy : y = 0,
 { simp [hy] },
 rw [if_neg hx]; rw [ if_neg hy]; rw [ if_neg (mul_ne_zero hx hy)]; rw [ ← with_top.coe_add]; rw [ with_top.coe_eq_coe]; rw [ order_mul hx hy],
 end)
...
lemma add_val_le_of_coeff_ne_zero {x : hahn_series Γ R} {g : Γ} (h : x.coeff g ≠ 0) :
 add_val Γ R x ≤ g :=
begin
 rw [add_val_apply_of_ne (ne_zero_of_coeff_ne_zero h)]; rw [ with_top.coe_le_coe],
...
{ coeff := λ g, ∑ᶠ i, (s i).coeff g,
 ...
 contrapose,
 rw [set.mem_Union]; rw [ not_exists]; rw [ function.mem_support]; rw [ not_not],
...
begin
 ...
 rw [not_not.1 (λ con, ha ⟨hU.1, con, hU.2.2⟩)]; rw [ mul_zero] },
 { rintro ⟨i, j⟩ hij,
 ...
 { rintro ⟨i, j⟩ hU ha,
 rw mem_add_antidiagonal at *,
 rw [← hsum_coeff]; rw [ not_not.1 (λ con, ha ⟨hU.1, con, hU.2.2⟩)]; rw [ mul_zero] } }
end
...
by rw [← lsum_apply]; rw [ linear_map.map_sub]; rw [ lsum_apply]; rw [ lsum_apply]
...
begin
 ...
 rw [add_monoid_hom.map_sum]; rw [ finsum_eq_sum_of_support_subset],
 intros x h,
...
begin
 rw [emb_domain_apply]; rw [ dif_pos (set.mem_range_self a)],
...
by rw [emb_domain_apply]; rw [ dif_neg h]
...
begin
 ...
 cases n,
 { rw [emb_domain_notin_range]; rw [ sub_apply]; rw [ coe_powers]; rw [ pow_zero]; rw [ coe_of_finsupp]; rw [ finsupp.single_eq_same]; rw [ sub_self],
...
begin
 rw [← hsum_smul]; rw [ sub_smul]; rw [ one_smul]; rw [ hsum_sub]; rw [ ← hsum_emb_domain (x • powers x hx) ⟨nat.succ, nat.succ_injective⟩]; rw [ emb_domain_succ_smul_powers],
 simp,
end
...
begin
 ...
 rw [add_val_apply_of_ne x0]; rw [ add_val_apply_of_ne (single_ne_zero h10)]; rw [ add_val_apply_of_ne _]; rw [ order_C]; rw [ order_single h10]; rw [ with_top.coe_zero]; rw [ zero_add]; rw [ ← with_top.coe_add]; rw [ neg_add_self]; rw [ with_top.coe_zero],
 { exact le_refl 0 },
 { exact C_ne_zero (left_ne_zero_of_mul_eq_one hr) } },
 { rw [add_val_apply]; rw [ ← with_top.coe_zero],
 split_ifs,
 { apply with_top.coe_ne_top },
 rw [ne.def]; rw [ with_top.coe_eq_coe],
 intro con,
