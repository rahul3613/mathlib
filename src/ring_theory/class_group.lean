...
by { erw [quotient_group.mk'_eq_mk']; erw [ canonical_equiv_self]; erw [ units.map_id]; erw [ set.exists_range_iff],
 refl }
...
begin
 ...
 split,
 { rintro ⟨x, rfl⟩,
 rw [units.coe_mul] at hJ; rw [ hI] at hJ; rw [ coe_to_principal_ideal] at hJ; rw [ mul_comm] at hJ; rw [ span_singleton_mul_coe_ideal_eq_coe_ideal] at hJ,
 exact ⟨_, _, sec_fst_ne_zero le_rfl x.ne_zero, sec_snd_ne_zero le_rfl ↑x, hJ⟩ },
 { rintro ⟨x, y, hx, hy, h⟩,
 split, rw [mul_comm]; rw [ ← units.eq_iff]; rw [ units.coe_mul]; rw [ coe_to_principal_ideal],
 ...
 rwa [ne]; rwa [ mk'_eq_zero_iff_eq_zero] }
end
...
begin
 rw [← map_one class_group.mk]; rw [ class_group.mk_eq_mk_of_coe_ideal hI (_ : _ = ↑⊤)],
 any_goals { refl },
 split,
 { rintro ⟨x, y, hx, hy, h⟩,
 ...
 { rintro ⟨x, hx, rfl⟩,
 exact ⟨1, x, one_ne_zero, hx, by rw [ideal.span_singleton_one]; rw [ ideal.top_mul]; rw [ ideal.mul_top]⟩ }
end
...
quotient_group.induction_on x (λ I, begin
 ...
 ext : 1,
 rw [units.coe_map]; rw [ units.coe_map_equiv],
...
begin
 ...
 rw [units.coe_map_equiv]; rw [ ← hx]; rw [ ring_equiv.coe_to_mul_equiv]; rw [ canonical_equiv_span_singleton],
 refl },
 { rintro ⟨x, hx⟩,
...
begin
 rw [class_group.equiv]; rw [ class_group.mk]; rw [ monoid_hom.comp_apply]; rw [ quotient_group.congr_mk'],
 congr,
 ext : 1,
 rw [units.coe_map_equiv]; rw [ units.coe_map_equiv]; rw [ units.coe_map],
...
@[simp] lemma class_group.mk_canonical_equiv (K' : Type*) [field K'] [algebra R K']
 ...
 class_group.mk I :=
by rw [class_group.mk]; rw [ monoid_hom.comp_apply]; rw [ ← monoid_hom.comp_apply (units.map _)]; rw [ ← units.map_comp]; rw [ ← ring_equiv.coe_monoid_hom_trans]; rw [ fractional_ideal.canonical_equiv_trans_canonical_equiv]; refl
...
by rw [class_group.mk0]; rw [ monoid_hom.comp_apply]; rw [ ← class_group.mk_canonical_equiv K (fraction_ring R)]; rw [ fractional_ideal.map_canonical_equiv_mk0]
...
begin
 rw [class_group.mk0]; rw [ monoid_hom.comp_apply]; rw [ class_group.equiv_mk],
 congr,
 ext,
 simp
end
...
begin
 ...
 { rintro hx, apply hz,
 rw [hx]; rw [ is_fraction_ring.mk'_eq_div]; rw [ _root_.map_zero]; rw [ zero_div] },
 ...
 { contrapose! hx,
 rwa [mk'_eq_iff_eq_mul]; rwa [ zero_mul]; rwa [ ← (algebra_map R (fraction_ring R)).map_zero]; rwa [ (is_fraction_ring.injective R (fraction_ring R)).eq_iff]
 at hx },
...
begin
 ...
 { rw [mem_non_zero_divisors_iff_ne_zero]; rw [ submodule.zero_eq_bot]; rw [ submodule.ne_bot_iff],
 ...
 rwa [ring_hom.map_mul]; rwa [ ← mul_assoc]; rwa [ inv_mul_cancel fa_ne_zero]; rwa [ one_mul] },
 { symmetry,
 ...
 rw [← algebra.smul_def]; rw [ mem_coe_ideal],
 refine ⟨y, _, hy⟩,
 show (algebra_map R _ a)⁻¹ * algebra_map R _ y ∈ (I : fractional_ideal R⁰ (fraction_ring R)),
 rwa [hy]; rwa [ algebra.smul_def]; rwa [ ← mul_assoc]; rwa [ inv_mul_cancel fa_ne_zero]; rwa [ one_mul] } }
end
...
begin
 ...
 refine ⟨λ ⟨x, hx⟩, ⟨⟨x, by rw [← hx]; rw [ coe_span_singleton]⟩⟩, _⟩,
 ...
 { apply subtype.coe_injective, rw [hx]; rw [ coe_span_singleton] },
