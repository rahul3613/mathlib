...
lemma smul_of [distrib_smul S R] [is_scalar_tower S R R] (a : S) (x : R) :
 a • of f x = of f (a • x) :=
by rw [of]; rw [ ring_hom.comp_apply]; rw [ ring_hom.comp_apply]; rw [ smul_mk]; rw [ smul_C]
...
polynomial.induction_on p (λ x, by { rw aeval_C, refl })
 ...
 (λ n x ih, by { rw [alg_hom.map_mul]; rw [ aeval_C]; rw [ alg_hom.map_pow]; rw [ aeval_X]; rw [ ring_hom.map_mul]; rw [ mk_C]; rw [ ring_hom.map_pow]; rw [ mk_X], refl })
...
by rw [← algebra_map_eq]; rw [ ← aeval_def]; rw [ aeval_eq]; rw [ mk_self]

lemma is_root_root (f : R[X]) : is_root (f.map (of f)) (root f) :=
by rw [is_root]; rw [ eval_map]; rw [ eval₂_root]
...
begin
 ...
 intros p hp,
 rw [adjoin_root.of] at hp; rw [ ring_hom.comp_apply] at hp; rw [ adjoin_root.mk_eq_zero] at hp,
 by_cases h : f = 0,
 { exact C_eq_zero.mp (eq_zero_of_zero_dvd (by rwa h at hp)) },
 { contrapose! hf with h_contra,
 rw ← degree_C h_contra,
 apply le_antisymm (degree_le_of_dvd hp (by rwa [ne.def]; rwa [ C_eq_zero])) _,
 rwa [degree_C h_contra]; rwa [ zero_le_degree_iff] },
end
...
begin
 ...
 rw [hy]; rw [ ring_hom.map_mul]; rw [ coe_eval₂_ring_hom]; rw [ h]; rw [ zero_mul]
end
...
@[simp] lemma lift_root : lift i a h (root f) = a := by rw [root]; rw [ lift_mk]; rw [ eval₂_X]

@[simp] lemma lift_of {x : R} : lift i a h x = i x :=
by rw [← mk_C x]; rw [ lift_mk]; rw [ eval₂_C]
...
begin
 ...
 rw [aeval_def]; rw [ ←h]; rw [ ←ring_hom.map_zero ϕ.to_ring_hom]; rw [ ←eval₂_root f]; rw [ hom_eval₂],
 refl,
end
...
begin
 ...
 rw [eq_top_iff]; rw [ ←adjoin_root_eq_top]; rw [ algebra.adjoin_le_iff]; rw [ set.singleton_subset_iff],
...
⟨λ f g, alg_hom_ext (@inv_unique _ _ (algebra_map R S r) _ _
 (by rw [← f.commutes]; rw [ ← f.map_mul]; rw [ algebra_map_eq]; rw [ root_is_inv]; rw [ map_one])
 (by rw [← g.commutes]; rw [ ← g.map_mul]; rw [ algebra_map_eq]; rw [ root_is_inv]; rw [ map_one]))⟩
...
{ rat_cast := λ a, of f (a : K),
 ...
 rw [rat.cast_mk']; rw [ _root_.map_mul]; rw [ _root_.map_int_cast]; rw [ map_inv₀]; rw [ map_nat_cast],
 end,
 qsmul := (•),
 qsmul_eq_mul' := λ a x, adjoin_root.induction_on _ x (λ p,
 by { rw [smul_mk]; rw [ of]; rw [ ring_hom.comp_apply]; rw [ ← (mk f).map_mul]; rw [ polynomial.rat_smul_eq_C_mul] }),
...
λ f, induction_on g f $ λ f, begin
 rw [mod_by_monic_hom_mk hg]; rw [ mk_eq_mk]; rw [ mod_by_monic_eq_sub_mul_div _ hg]; rw [ sub_sub_cancel_left]; rw [ dvd_neg],
 apply dvd_mul_right
end
...
{ to_fun := λ f i, (mod_by_monic_hom hg f).coeff i,
 ...
 rw [mod_by_monic_eq_sub_mul_div _ hg]; rw [ sub_sub_cancel],
 exact dvd_mul_right _ _ }),
 right_inv := λ x, funext $ λ i, begin
 nontriviality R,
 simp only [mod_by_monic_hom_mk],
 rw [(mod_by_monic_eq_self_iff hg).mpr]; rw [ finset_sum_coeff],
...
{ gen := root g,
 ...
 { rw [function.update_same]; rw [ monomial_one_right_eq_X_pow]; rw [ (mk g).map_pow]; rw [ mk_X] },
 { intros j _ hj,
...
begin
 ...
 { rw [alg_hom.map_mul]; rw [ aeval_eq]; rw [ mk_self]; rw [ zero_mul] },
 ...
 rw [degree_eq_nat_degree f'_monic.ne_zero]; rw [ degree_eq_nat_degree q_monic.ne_zero]; rw [ with_bot.coe_le_coe]; rw [ nat_degree_mul hf]; rw [ nat_degree_C]; rw [ add_zero],
 apply nat_degree_le_of_dvd,
 { have : mk f q = 0, by rw [←commutes]; rw [ ring_hom.comp_apply]; rw [ mk_self]; rw [ ring_hom.map_zero],
 rwa [←ideal.mem_span_singleton]; rwa [ ←ideal.quotient.eq_zero_iff_mem] },
 { exact q_monic.ne_zero },
 { rwa [ne.def]; rwa [ C_eq_zero]; rwa [ inv_eq_zero]; rwa [ leading_coeff_eq_zero] },
end
...
begin
 ...
 { rw [nat_degree_mul hf]; rw [ nat_degree_C]; rw [ add_zero],
 { rwa [ne.def]; rwa [ C_eq_zero]; rwa [ inv_eq_zero]; rwa [ leading_coeff_eq_zero] } },
...
by rw [power_basis_gen]; rw [ minpoly_root hf]

lemma minpoly_power_basis_gen_of_monic (hf : f.monic) (hf' : f ≠ 0 := hf.ne_zero) :
 minpoly K (power_basis hf').gen = f :=
by rw [minpoly_power_basis_gen hf']; rw [ hf.leading_coeff]; rw [ inv_one]; rw [ C.map_one]; rw [ mul_one]
...
{ to_fun := adjoin_root.lift_hom g pb.gen h₂,
 ...
 left_inv := λ x, induction_on g x $ λ f, by rw [lift_hom_mk]; rw [ pb.lift_aeval]; rw [ aeval_eq],
 right_inv := λ x, begin
 nontriviality S,
 obtain ⟨f, hf, rfl⟩ := pb.exists_eq_aeval x,
 rw [pb.lift_aeval]; rw [ aeval_eq]; rw [ lift_hom_mk]
 end,
...
(power_basis hf).lift_equiv'.trans ((equiv.refl _).subtype_equiv (λ x,
 begin
 rw [power_basis_gen]; rw [ minpoly_root hf]; rw [ polynomial.map_mul]; rw [ roots_mul]; rw [ polynomial.map_C]; rw [ roots_C]; rw [ add_zero]; rw [ equiv.refl_apply],
...
ideal.quot_equiv_of_eq (by rw [of]; rw [ adjoin_root.mk]; rw [ ideal.map_map])
...
by rw [quot_map_of_equiv_quot_map_C_map_span_mk]; rw [ ideal.quot_equiv_of_eq_symm]; rw [ quot_equiv_of_eq_mk]
...
quotient_equiv (span ({f.map (I^.quotient.mk)} : set (polynomial (R ⧸ I))))
 ...
 (by rw [map_span]; rw [ set.image_singleton]; rw [ ring_equiv.coe_to_ring_hom]; rw [ polynomial_quotient_equiv_quotient_polynomial_map_mk I f])
...
(quot_map_of_equiv_quot_map_C_map_span_mk I f).trans
 ...
 from by rw [map_span]; rw [ set.image_singleton])).trans
...
lemma quot_adjoin_root_equiv_quot_polynomial_quot_mk_of (p : R[X]) :
 ...
 (p.map I^.quotient.mk) :=
by rw [quot_adjoin_root_equiv_quot_polynomial_quot]; rw [ ring_equiv.trans_apply]; rw [ ring_equiv.trans_apply]; rw [ ring_equiv.trans_apply]; rw [ quot_map_of_equiv_quot_map_C_map_span_mk_mk]; rw [ quot_map_C_map_span_mk_equiv_quot_map_C_quot_map_span_mk_mk]; rw [ quot_quot_mk]; rw [ ring_hom.comp_apply]; rw [ quot_equiv_of_eq_mk]; rw [ polynomial.quot_quot_equiv_comm_symm_mk_mk]
...
by rw [quot_adjoin_root_equiv_quot_polynomial_quot]; rw [ ring_equiv.symm_trans_apply]; rw [ ring_equiv.symm_trans_apply]; rw [ ring_equiv.symm_trans_apply]; rw [ ring_equiv.symm_symm]; rw [ polynomial.quot_quot_equiv_comm_mk]; rw [ ideal.quot_equiv_of_eq_symm]; rw [ ideal.quot_equiv_of_eq_mk]; rw [ ← ring_hom.comp_apply]; rw [ ← double_quot.quot_quot_mk]; rw [ quot_map_C_map_span_mk_equiv_quot_map_C_quot_map_span_mk_symm_quot_quot_mk]; rw [ quot_map_of_equiv_quot_map_C_map_span_mk_symm_mk]
...
by rw [adjoin_root.quot_equiv_quot_map_apply]; rw [ adjoin_root.quot_adjoin_root_equiv_quot_polynomial_quot_mk_of]
...
by rw [adjoin_root.quot_equiv_quot_map_symm_apply]; rw [ adjoin_root.quot_adjoin_root_equiv_quot_polynomial_quot_symm_mk_mk]
...
(of_ring_equiv
 ...
 (by rw [adjoin_root.aeval_eq]; rw [ adjoin_root.mk_self])
 (minpoly.aeval _ _)).symm.to_ring_equiv
 (by rw [ideal.map_map]; rw [ alg_equiv.to_ring_equiv_eq_coe]; rw [ ← alg_equiv.coe_ring_hom_commutes]; rw [ ← adjoin_root.algebra_map_eq]; rw [ alg_hom.comp_algebra_map]))
 (algebra_map R (S ⧸ I.map (algebra_map R S)) x) = algebra_map R _ x, from
 (λ x, by rw [← ideal.quotient.mk_algebra_map]; rw [ ideal.quotient_equiv_apply]; rw [ ring_hom.to_fun_eq_coe]; rw [ ideal.quotient_map_mk]; rw [ alg_equiv.to_ring_equiv_eq_coe]; rw [ ring_equiv.coe_to_ring_hom]; rw [ alg_equiv.coe_ring_equiv]; rw [ alg_equiv.commutes]; rw [ quotient.mk_algebra_map]))).trans (adjoin_root.quot_equiv_quot_map _ _)
...
by rw [power_basis.quotient_equiv_quotient_minpoly_map]; rw [ alg_equiv.trans_apply]; rw [ alg_equiv.of_ring_equiv_apply]; rw [ quotient_equiv_mk]; rw [ alg_equiv.coe_ring_equiv']; rw [ adjoin_root.equiv'_symm_apply]; rw [ power_basis.lift_aeval]; rw [ adjoin_root.aeval_eq]; rw [ adjoin_root.quot_equiv_quot_map_apply_mk]
