...
nat.rec_on m rfl $ λ m ih, by erw [function.iterate_succ_apply']; erw [ coeff_frobenius]; erw [ ih]
...
by rw [ring_hom.comp_apply]; rw [ ring_hom.id_apply]; rw [ coeff_pth_root]; rw [ coeff_frobenius]

lemma frobenius_pth_root : (frobenius _ p).comp (pth_root R p) = ring_hom.id _ :=
ring_hom.ext $ λ x, ext $ λ n,
by rw [ring_hom.comp_apply]; rw [ ring_hom.id_apply]; rw [ ring_hom.map_frobenius]; rw [ coeff_pth_root]; rw [ ← ring_hom.map_frobenius]; rw [ coeff_frobenius]

lemma coeff_add_ne_zero {f : ring.perfection R p} {n : ℕ} (hfn : coeff R p n f ≠ 0) (k : ℕ) :
 coeff R p (n + k) f ≠ 0 :=
nat.rec_on k hfn $ λ k ih h, ih $ by erw [← coeff_pow_p]; erw [ ring_hom.map_pow]; erw [ h]; erw [ zero_pow hp.1.pos]
...
{ to_fun := λ f,
 ...
 λ n, by rw [← f.map_pow]; rw [ function.iterate_succ_apply']; rw [ pth_root_pow_p]⟩,
 ...
 by rw [← coeff_iterate_frobenius _ 0 n]; rw [ zero_add]; rw [ ← ring_hom.map_iterate_frobenius]; rw [ right_inverse_pth_root_frobenius.iterate] }
...
{ to_fun := λ f, ⟨λ n, φ (coeff R p n f), λ n, by rw [← φ.map_pow]; rw [ coeff_pow_p']⟩,
...
{ injective := λ x y hxy, g.injective $ (ring_hom.ext_iff.1 hfg x).symm.trans $
 ...
 by rw [hfg]; rw [ ← coe_fn_coe_base]; rw [ hx]⟩ }
...
{ injective := λ x y hxy, hxy 0,
 ...
 by rw [function.iterate_succ_apply']; rw [ pth_root_pow_p _]; rw [ ih]; rw [ hf]⟩ }
...
hom_ext _ (of p S) $ λ f, by rw [map_map]; rw [ perfection.coeff_map]
...
begin
 ...
 erw [← ring_hom.map_sub]; erw [ ← hr]; erw [ hv.le_iff_dvd],
...
begin
 ...
 rw [pre_val_mk hx0]; rw [ pre_val_mk hy0]; rw [ pre_val_mk hxy0]; rw [ ring_hom.map_mul]; rw [ v.map_mul]
end

lemma pre_val_add (x y : mod_p K v O hv p) :
 pre_val K v O hv p (x + y) ≤ max (pre_val K v O hv p x) (pre_val K v O hv p y) :=
begin
 by_cases hx0 : x = 0, { rw [hx0]; rw [ zero_add], exact le_max_right _ _ },
 by_cases hy0 : y = 0, { rw [hy0]; rw [ add_zero], exact le_max_left _ _ },
 by_cases hxy0 : x + y = 0, { rw [hxy0]; rw [ pre_val_zero], exact zero_le _ },
 obtain ⟨r, rfl⟩ := ideal.quotient.mk_surjective x,
 obtain ⟨s, rfl⟩ := ideal.quotient.mk_surjective y,
 rw ← ring_hom.map_add at hxy0 ⊢,
 rw [pre_val_mk hx0]; rw [ pre_val_mk hy0]; rw [ pre_val_mk hxy0]; rw [ ring_hom.map_add], exact v.map_add _ _
end

lemma v_p_lt_pre_val {x : mod_p K v O hv p} : v p < pre_val K v O hv p x ↔ x ≠ 0 :=
begin
 refine ⟨λ h hx, by { rw [hx] at h; rw [ pre_val_zero] at h, exact not_lt_zero' h },
 λ h, lt_of_not_le $ λ hp, h _⟩,
 obtain ⟨r, rfl⟩ := ideal.quotient.mk_surjective x,
 rw [pre_val_mk h] at hp; rw [ ← map_nat_cast (algebra_map O K) p] at hp; rw [ hv.le_iff_dvd] at hp,
...
⟨λ hvx, classical.by_contradiction $ λ hx0 : x ≠ 0,
 by { rw [← v_p_lt_pre_val] at hx0; rw [ hvx] at hx0, exact not_lt_zero' hx0 },
λ hx, hx.symm ▸ pre_val_zero⟩

variables (hv hvp)
lemma v_p_lt_val {x : O} :
 v p < v (algebra_map O K x) ↔ (ideal.quotient.mk _ x : mod_p K v O hv p) ≠ 0 :=
by rw [lt_iff_not_le]; rw [ not_iff_not]; rw [ ← map_nat_cast (algebra_map O K) p]; rw [ hv.le_iff_dvd]; rw [ ideal.quotient.eq_zero_iff_mem]; rw [ ideal.mem_span_singleton]
...
begin
 ...
 rw [ring_hom.map_pow] at hx hy; rw [ v.map_pow] at hx hy; rw [ ← rpow_lt_rpow_iff h1p] at hx hy; rw [ ← rpow_nat_cast] at hx hy; rw [ ← rpow_mul] at hx hy; rw [ mul_one_div_cancel (nat.cast_ne_zero.2 hp.1.ne_zero : (p : ℝ) ≠ 0)] at hx hy; rw [ rpow_one] at hx hy,
 rw [ring_hom.map_mul]; rw [ v.map_mul], refine lt_of_le_of_lt _ (mul_lt_mul₀ hx hy),
...
begin
 ...
 by { erw [ring_hom.map_pow]; erw [ hx]; erw [ ← ring_hom.map_pow]; erw [ coeff_pow_p],
 exact coeff_nat_find_add_ne_zero k },
 erw [ih (coeff_nat_find_add_ne_zero k)]; erw [ ← hx]; erw [ ← coeff_pow_p]; erw [ ring_hom.map_pow]; erw [ ← hx]; erw [ ← ring_hom.map_pow]; erw [ mod_p.pre_val_mk h1]; erw [ mod_p.pre_val_mk h2]; erw [ ring_hom.map_pow]; erw [ v.map_pow]; erw [ ← pow_mul]; erw [ pow_succ], refl
end
...
by { rw [pow_zero]; rw [ pow_one]; rw [ ring_hom.map_one]; rw [ ← (ideal.quotient.mk _).map_one]; rw [ mod_p.pre_val_mk]; rw [ ring_hom.map_one]; rw [ v.map_one],
...
begin
 by_cases hf : f = 0, { rw [hf]; rw [ zero_mul]; rw [ val_aux_zero]; rw [ zero_mul] },
 by_cases hg : g = 0, { rw [hg]; rw [ mul_zero]; rw [ val_aux_zero]; rw [ mul_zero] },
 ...
 { rw [← ring_hom.map_pow]; rw [ coeff_pow_p f], assumption },
 { rw [← ring_hom.map_pow]; rw [ coeff_pow_p g], assumption } },
 rw [val_aux_eq (coeff_add_ne_zero hm 1)]; rw [ val_aux_eq (coeff_add_ne_zero hn 1)]; rw [ val_aux_eq hfg],
 rw ring_hom.map_mul at hfg ⊢, rw [mod_p.pre_val_mul hfg]; rw [ mul_pow]
end

lemma val_aux_add (f g : pre_tilt K v O hv p) :
 val_aux K v O hv p (f + g) ≤ max (val_aux K v O hv p f) (val_aux K v O hv p g) :=
begin
 by_cases hf : f = 0, { rw [hf]; rw [ zero_add]; rw [ val_aux_zero]; rw [ max_eq_right], exact zero_le _ },
 by_cases hg : g = 0, { rw [hg]; rw [ add_zero]; rw [ val_aux_zero]; rw [ max_eq_left], exact zero_le _ },
 by_cases hfg : f + g = 0, { rw [hfg]; rw [ val_aux_zero], exact zero_le _ },
 ...
 rw [val_aux_eq hm]; rw [ val_aux_eq hn]; rw [ val_aux_eq hk]; rw [ ring_hom.map_add],
 cases le_max_iff.1
