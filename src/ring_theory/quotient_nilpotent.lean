...
begin
 ...
 by_cases hI' : I = ⊥,
 { subst hI', apply h₁, rw [← ideal.zero_eq_bot]; rw [ zero_pow], exact zero_lt_two },
 cases n,
 { rw [pow_zero] at hI; rw [ ideal.one_eq_top] at hI,
 ...
 { rw [← pow_mul]; rw [ eq_bot_iff]; rw [ ← hI]; rw [ nat.succ_eq_add_one]; rw [ nat.succ_eq_add_one],
 exact ideal.pow_le_pow (by linarith) },
 { exact le_refl n.succ.succ } },
 { apply h₁, rw [← ideal.map_pow]; rw [ ideal.map_quotient_self] },
end
...
begin
 ...
 { rw [map_one]; rw [ _root_.map_mul]; rw [ hy]; rw [ is_unit.mul_coe_inv] },
 rw ideal.quotient.eq at this,
 have : (x * y - 1) ^ 2 = 0,
 { rw [← ideal.mem_bot]; rw [ ← e], exact ideal.pow_mem_pow this _ },
 have : x * (y * (2 - x * y)) = 1,
 { rw [eq_comm]; rw [ ← sub_eq_zero]; rw [ ← this], ring },
