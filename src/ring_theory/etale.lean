...
begin
 rw [formally_unramified_iff]; rw [ formally_smooth_iff]; rw [ formally_etale_iff],
...
begin
 ...
 rwa [ideal.quotient.eq]; rwa [ ← map_sub]; rwa [ ideal.mem_quotient_iff_mem hIJ]; rwa [ ← ideal.quotient.eq] },
end
...
begin
 ...
 conv_rhs at e { rw [← alg_hom.comp_assoc]; rw [ alg_equiv.to_alg_hom_eq_coe]; rw [ alg_equiv.to_alg_hom_eq_coe]; rw [ alg_equiv.comp_symm]; rw [ alg_hom.id_comp] },
 exact ⟨g', e⟩ }
end
...
begin
 ...
 rw [alg_equiv.apply_symm_apply]; rw [ ideal.quotient_ker_alg_equiv_of_surjective]; rw [ ideal.quotient_ker_alg_equiv_of_right_inverse.apply],
...
begin
 ...
 rw [← alg_hom.comp_assoc]; rw [ formally_smooth.comp_lift]; rw [ alg_hom.comp_assoc]; rw [ alg_equiv.comp_symm]; rw [ alg_hom.comp_id],
end
...
begin
 ...
 rw [← f₁.comp_id]; rw [ ← f₂.comp_id]; rw [ ← e.comp_symm]; rw [ ← alg_hom.comp_assoc]; rw [ ← alg_hom.comp_assoc],
 congr' 1,
 refine formally_unramified.comp_injective I hI _,
 rw [← alg_hom.comp_assoc]; rw [ e']; rw [ alg_hom.comp_assoc],
end
...
begin
 ...
 ext s,
 rw [← hg]; rw [ alg_hom.comp_apply]; rw [ mv_polynomial.aeval_X],
 refl,
end
...
begin
 ...
 (f₂.comp $ is_scalar_tower.to_alg_hom R A B)
 (by rw [← alg_hom.comp_assoc]; rw [ e]; rw [ alg_hom.comp_assoc]),
...
begin
 ...
 rwa [hx] at this ; rwa [ map_zero] at this ; rwa [ ← ideal.quotient.mk_eq_mk] at this ; rwa [ submodule.quotient.mk_eq_zero] at this },
 intros x hx,
 ...
 exact ⟨l.comp g, by rw [← alg_hom.comp_assoc]; rw [ ← this]; rw [ alg_hom.comp_assoc]; rw [ hg]; rw [ alg_hom.comp_id]⟩
end
...
begin
 ...
 { rw [alg_hom.ker_ker_sqare_lift]; rw [ ideal.cotangent_ideal_square]; rw [ ideal.zero_eq_bot] },
 ...
 conv_rhs { rw [← this]; rw [ alg_hom.id_apply] },
...
begin
 ...
 rw [← alg_hom.to_ring_hom_eq_coe]; rw [ alg_hom.ker_ker_sqare_lift],
...
begin
 ...
 intros b a,
 have : b ⊗ₜ[R] a = b • (1 ⊗ₜ a), by { rw [tensor_product.smul_tmul']; rw [ smul_eq_mul]; rw [ mul_one] },
 rw [this]; rw [ alg_hom.restrict_scalars_apply]; rw [ alg_hom.restrict_scalars_apply]; rw [ map_smul]; rw [ map_smul],
 congr' 1,
...
begin
 ...
 rw [← algebra.smul_def]; rw [ ← map_smul]; rw [ tensor_product.smul_tmul']; rw [ smul_eq_mul]; rw [ mul_one] },
end
...
begin
 ...
 congr' 1,
 refine is_localization.ring_hom_ext M _,
 rw [ring_hom.comp_assoc]; rw [ ← is_scalar_tower.algebra_map_eq]; rw [ ← is_scalar_tower.algebra_map_eq]; rw [ alg_hom.comp_algebra_map] },
 use f,
 ext,
 simp,
end
