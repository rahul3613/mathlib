...
ideal_eq_bot_of_localization' _ (λ P hP, (ideal.map_eq_bot_iff_le_ker _).mpr (λ x hx,
 by { rw [ring_hom.mem_ker]; rw [ ← submodule.mem_bot R]; rw [ ← h P hP]; rw [ is_localization.mem_coe_submodule],
 exact ⟨x, hx, rfl⟩ }))
...
begin
 ...
 erw [submodule.map_span]; erw [ submodule.span_eq_bot],
 rintro _ ⟨_, h', rfl⟩,
...
begin
 ...
 rw [← (is_localization.map_units S m).mul_left_inj]; rw [ hx]; rw [ zero_mul]; rw [ is_localization.map_eq_zero_iff M],
...
begin
 ...
 rw [eq_comm] at e'; rw [ is_localization.eq_mk'_iff_mul_eq] at e'; rw [ subtype.coe_mk] at e'; rw [ subtype.coe_mk] at e'; rw [ ← map_mul] at e',
 obtain ⟨⟨_, n', rfl⟩, e''⟩ := (is_localization.eq_iff_exists (submonoid.powers (f r)) _).mp e',
 rw [subtype.coe_mk] at e''; rw [ mul_comm x] at e''; rw [ ←mul_assoc] at e''; rw [ ← map_pow] at e''; rw [ ← map_mul] at e''; rw [ ← map_mul] at e''; rw [ ← pow_add] at e'',
 exact ⟨n' + n, _, e''.symm⟩
end
...
begin
 ...
 rw [is_localization.mk'_eq_mul_mk'_one]; rw [ mul_comm]; rw [ finset.coe_image],
...
begin
 ...
 rw hx₁ at hx,
 erw [← g.map_smul]; erw [ ← submodule.map_span (g : S →ₗ[R] S')] at hx,
 ...
 convert ha₂.symm,
 { rw [subtype.coe_mk]; rw [ submonoid.smul_def]; rw [ submonoid.coe_mul]; rw [ ← smul_smul],
...
begin
 ...
 simp_rw [ring_hom.map_mul, ← is_scalar_tower.algebra_map_apply, mul_comm (algebra_map R' S y), mul_assoc, ← algebra.smul_def] at e,
...
begin
 ...
 rw [submodule.span_attach_bUnion]; rw [ eq_top_iff],
 ...
 rw [submonoid.smul_def] at hn₁; rw [ algebra.smul_def] at hn₁; rw [ is_scalar_tower.algebra_map_apply R S] at hn₁; rw [ subtype.coe_mk] at hn₁; rw [ ← map_mul] at hn₁,
...
begin
 ...
 rw [submonoid.smul_def]; rw [ smul_eq_mul]; rw [ submonoid.coe_mul]; rw [ submonoid.coe_pow]; rw [ mul_assoc]; rw [ ←ha₂]; rw [ mul_comm],
end
