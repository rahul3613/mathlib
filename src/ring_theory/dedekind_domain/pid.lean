...
begin
 ...
 { rwa [ideal.dvd_iff_le]; rwa [ ideal.span_singleton_le_iff_mem] },
...
begin
 ...
 { rw [subtype.ext_iff] at hinv; rw [ fractional_ideal.coe_mul] at hinv; rw [ fractional_ideal.coe_one] at hinv,
...
begin
 have hinv' := hinv,
 rw [subtype.ext_iff] at hinv; rw [ fractional_ideal.coe_mul] at hinv,
 let s := hf.to_finset,
...
begin
 ...
 (λ x, by erw [is_localization.map_eq]; erw [ is_scalar_tower.algebra_map_apply R S]),
 ...
 rw [← multiset.singleton_le]; rw [ ← normalize_eq P]; rw [ ← normalized_factors_irreducible (ideal.prime_of_is_prime hP0 hP).irreducible]; rw [ ← dvd_iff_normalized_factors_le_normalized_factors hP0]; rw [ dvd_iff_le]; rw [ is_scalar_tower.algebra_map_eq R (localization.at_prime p) Sₚ]; rw [ ← ideal.map_map]; rw [ localization.at_prime.map_eq_maximal_ideal]; rw [ ideal.map_le_iff_le_comap]; rw [ hpu (local_ring.maximal_ideal _) ⟨this, _⟩]; rw [ hpu (comap _ _) ⟨_, _⟩],
 { exact le_rfl },
 ...
 { rw [ne.def]; rw [ zero_eq_bot]; rw [ ideal.map_eq_bot_iff_of_injective],
 { assumption },
...
begin
 ...
 (λ P, _)),
 rw [finset.mem_filter]; rw [ finset.mem_union]; rw [ finset.mem_singleton]; rw [ set.mem_set_of]; rw [ multiset.mem_to_finset],
