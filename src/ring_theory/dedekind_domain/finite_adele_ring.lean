...
lemma add {x y : K_hat R K} (hx : x.is_finite_adele) (hy : y.is_finite_adele) :
 (x + y).is_finite_adele :=
begin
 rw [is_finite_adele] at hx hy ⊢; rw [ filter.eventually_cofinite] at hx hy ⊢,
 ...
 { intros v hv,
 rw [mem_union]; rw [ mem_set_of_eq]; rw [ mem_set_of_eq],
 rw mem_set_of_eq at hv,
 contrapose! hv,
 rw [mem_adic_completion_integers] at hv; rw [ mem_adic_completion_integers] at hv; rw [ ← max_le_iff] at hv,
 rw [mem_adic_completion_integers]; rw [ pi.add_apply],
...
begin
 rw [is_finite_adele]; rw [ filter.eventually_cofinite],
 have h_empty : {v : height_one_spectrum R |
 ¬ ((0 : v.adic_completion K) ∈ v.adic_completion_integers K)} = ∅,
 { ext v, rw [mem_empty_iff_false]; rw [ iff_false], intro hv,
...
begin
 ...
 { intro v,
 rw [mem_adic_completion_integers]; rw [ mem_adic_completion_integers]; rw [ valuation.map_neg], },
...
begin
 ...
 rw mem_set_of_eq at hv,
 contrapose! hv,
 rw [mem_adic_completion_integers] at hv; rw [ mem_adic_completion_integers] at hv,
 have h_mul : valued.v (x v * y v) = (valued.v (x v)) * (valued.v (y v)) :=
 (valued.v).map_mul' (x v) (y v),
 rw [mem_adic_completion_integers]; rw [ pi.mul_apply]; rw [ h_mul],
