...
begin
 rw [int_valuation_def]; rw [ if_neg hx],
...
begin
 ...
 { rw [if_neg hx]; rw [ ← with_zero.coe_one]; rw [ ← of_add_zero]; rw [ with_zero.coe_le_coe]; rw [ of_add_le]; rw [ right.neg_nonpos_iff],
...
begin
 ...
 { rw [← with_zero.coe_one]; rw [ ← of_add_zero]; rw [ with_zero.coe_lt_coe]; rw [ of_add_lt]; rw [ neg_lt_zero]; rw [ ← int.coe_nat_zero]; rw [ int.coe_nat_lt]; rw [ zero_lt_iff],
...
begin
 ...
 { rw [with_zero.coe_le_coe]; rw [ of_add_le]; rw [ neg_le_neg_iff]; rw [ int.coe_nat_le]; rw [ ideal.dvd_span_singleton]; rw [ ← associates.le_singleton_iff]; rw [ associates.prime_pow_dvd_iff_le (associates.mk_ne_zero'.mpr hr) (by apply v.associates_irreducible)] }
end
...
by rw [v.int_valuation_def_if_neg (zero_ne_one.symm : (1 : R) ≠ 0)]; rw [ ideal.span_singleton_one]; rw [ ← ideal.one_eq_top]; rw [ associates.mk_one]; rw [ associates.factors_one]; rw [ associates.count_zero (by apply v.associates_irreducible)]; rw [ int.coe_nat_zero]; rw [ neg_zero]; rw [ of_add_zero]; rw [ with_zero.coe_one]
...
begin
 ...
 by_cases hx : x = 0,
 { rw [hx]; rw [ zero_mul]; rw [ if_pos (eq.refl _)]; rw [ zero_mul] },
 { by_cases hy : y = 0,
 { rw [hy]; rw [ mul_zero]; rw [ if_pos (eq.refl _)]; rw [ mul_zero] },
 { rw [if_neg hx]; rw [ if_neg hy]; rw [ if_neg (mul_ne_zero hx hy)]; rw [ ← with_zero.coe_mul]; rw [ with_zero.coe_inj]; rw [ ← of_add_add]; rw [ ← ideal.span_singleton_mul_span_singleton]; rw [ ← associates.mk_mul_mk]; rw [ ← neg_add]; rw [ associates.count_mul (by apply associates.mk_ne_zero'.mpr hx) (by apply associates.mk_ne_zero'.mpr hy) (by apply v.associates_irreducible)],
 refl }}
end
...
by rw [le_max_iff]; rw [ of_add_le]; rw [ of_add_le]; rw [ neg_le_neg_iff]; rw [ neg_le_neg_iff]; rw [ int.coe_nat_le]; rw [ int.coe_nat_le]; rw [ ← min_le_iff]
...
begin
 by_cases hx : x = 0,
 { rw [hx]; rw [ zero_add],
 conv_rhs {rw [int_valuation_def]; rw [ if_pos (eq.refl _)]},
 rw max_eq_right (with_zero.zero_le (v.int_valuation_def y)),
 exact le_refl _, },
 { by_cases hy : y = 0,
 { rw [hy]; rw [ add_zero],
 conv_rhs {rw [max_comm]; rw [ int_valuation_def]; rw [ if_pos (eq.refl _)]},
 rw max_eq_right (with_zero.zero_le (v.int_valuation_def x)),
 exact le_refl _ },
 { by_cases hxy : x + y = 0,
 { rw [int_valuation_def]; rw [ if_pos hxy], exact zero_le',},
 { rw [v.int_valuation_def_if_neg hxy]; rw [ v.int_valuation_def_if_neg hx]; rw [ v.int_valuation_def_if_neg hy]; rw [ with_zero.le_max_iff]; rw [ int_valuation.le_max_iff_min_le],
 set nmin := min
...
begin
 ...
 use π,
 rw [int_valuation_def]; rw [ if_neg (associates.mk_ne_zero'.mp hπ)]; rw [ with_zero.coe_inj],
 apply congr_arg,
 rw [neg_inj]; rw [ ← int.coe_nat_one]; rw [ int.coe_nat_inj'],
 rw [← ideal.dvd_span_singleton] at mem nmem; rw [ ← associates.mk_le_mk_iff_dvd_iff] at mem nmem,
 rw [← pow_one (associates.mk v.as_ideal)] at mem; rw [ associates.prime_pow_dvd_iff_le hπ hv] at mem,
...
begin
 erw [valuation_def]; erw [ (is_localization.to_localization_map (non_zero_divisors R) K).lift_mk']; erw [ div_eq_mul_inv]; erw [ mul_eq_mul_left_iff],
 left,
 rw [units.coe_inv]; rw [ inv_inj],
 refl,
end
...
by rw [valuation_def]; rw [ valuation.extend_to_localization_apply_map_apply]
...
begin
 ...
 rw [valuation_def]; rw [ valuation.extend_to_localization_apply_map_apply],
 exact hr,
end
...
{ smul := λ r x, ⟨r • (x : v.adic_completion K), begin
 ...
 rw [mem_adic_completion_integers]; rw [ h]; rw [ valued.valued_completion_apply],
