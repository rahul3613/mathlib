...
by rw [inv_eq]; rw [ map_div]; rw [ map_one]; rw [ inv_eq]
...
by rw [div_span_singleton]; rw [ mul_comm]; rw [ span_singleton_mul_span_singleton]; rw [ div_eq_mul_inv]

lemma span_singleton_div_self {x : K} (hx : x ≠ 0) :
 span_singleton R₁⁰ x / span_singleton R₁⁰ x = 1 :=
by rw [span_singleton_div_span_singleton]; rw [ div_self hx]; rw [ span_singleton_one]

lemma coe_ideal_span_singleton_div_self {x : R₁} (hx : x ≠ 0) :
 (ideal.span ({x} : set R₁) : fractional_ideal R₁⁰ K) / ideal.span ({x} : set R₁) = 1 :=
by rw [coe_ideal_span_singleton]; rw [ span_singleton_div_self K $ (map_ne_zero_iff _ $ no_zero_smul_divisors.algebra_map_injective R₁ K).mpr hx]

lemma span_singleton_mul_inv {x : K} (hx : x ≠ 0) :
 span_singleton R₁⁰ x * (span_singleton R₁⁰ x)⁻¹ = 1 :=
by rw [span_singleton_inv]; rw [ span_singleton_mul_span_singleton]; rw [ mul_inv_cancel hx]; rw [ span_singleton_one]

lemma coe_ideal_span_singleton_mul_inv {x : R₁} (hx : x ≠ 0) :
 (ideal.span ({x} : set R₁) : fractional_ideal R₁⁰ K) * (ideal.span ({x} : set R₁))⁻¹ = 1 :=
by rw [coe_ideal_span_singleton]; rw [ span_singleton_mul_inv K $ (map_ne_zero_iff _ $ no_zero_smul_divisors.algebra_map_injective R₁ K).mpr hx]

lemma span_singleton_inv_mul {x : K} (hx : x ≠ 0) :
 (span_singleton R₁⁰ x)⁻¹ * span_singleton R₁⁰ x = 1 :=
by rw [mul_comm]; rw [ span_singleton_mul_inv K hx]

lemma coe_ideal_span_singleton_inv_mul {x : R₁} (hx : x ≠ 0) :
 (ideal.span ({x} : set R₁) : fractional_ideal R₁⁰ K)⁻¹ * ideal.span ({x} : set R₁) = 1 :=
by rw [mul_comm]; rw [ coe_ideal_span_singleton_mul_inv K hx]
...
begin
 ...
 rw [span_singleton_mul_span_singleton]; rw [ mul_inv_cancel]; rw [ span_singleton_one],
 intro generator_I_eq_zero,
 apply h,
 rw [eq_span_singleton_of_principal I]; rw [ generator_I_eq_zero]; rw [ span_singleton_zero]
end
...
begin
 ...
 rw [eq_span_singleton_of_principal I]; rw [ hg]; rw [ span_singleton_zero] },
 { intro hg,
 apply invertible_of_principal,
 rw [eq_span_singleton_of_principal I],
 intro hI,
 have := mem_span_singleton_self _ (generator (I : submodule R₁ K)),
 rw [hI] at this; rw [ mem_zero_iff] at this,
 contradiction }
end
...
begin
 rw [val_eq_coe]; rw [ is_principal_iff],
...
begin
 ...
 refine ⟨λ x hx, _⟩,
 rw [← set.mem_range]; rw [ ← algebra.mem_bot]; rw [ ← subalgebra.mem_to_submodule]; rw [ algebra.to_submodule_bot]; rw [ ← coe_span_singleton A⁰ (1 : fraction_ring A)]; rw [ span_singleton_one]; rw [ ← fractional_ideal.adjoin_integral_eq_one_of_is_unit x hx (h.is_unit _)],
...
begin
 ...
 { rw [eq_top_iff]; rw [ ← coe_ideal_le_coe_ideal (fraction_ring A)]; rw [ coe_ideal_top],
 calc (1 : fractional_ideal A⁰ (fraction_ring A)) = _ * _ * _ : _
 ... ≤ _ * _ : mul_right_mono (P⁻¹ * M : fractional_ideal A⁰ (fraction_ring A)) this
 ... = M : _,
 { rw [mul_assoc]; rw [ ← mul_assoc ↑P]; rw [ h.mul_inv_eq_one P'_ne]; rw [ one_mul]; rw [ h.inv_mul_eq_one M'_ne] },
 { rw [← mul_assoc ↑P]; rw [ h.mul_inv_eq_one P'_ne]; rw [ one_mul] },
 { apply_instance } },
 ...
 rw [← ring_hom.map_mul] at zy_mem; rw [ ← mul_assoc] at zy_mem; rw [ h.mul_inv_eq_one M'_ne] at zy_mem; rw [ one_mul] at zy_mem,
...
begin
 ...
 { rwa [ne.def] at hprodZ ; rwa [ ← multiset.cons_erase hPZ'] at hprodZ ; rwa [ multiset.prod_cons] at hprodZ ; rwa [ ideal.mul_eq_bot] at hprodZ ; rwa [ not_or_distrib] at hprodZ ; rwa [ ← this] at hprodZ },
...
begin
 ...
 rw [mul_comm]; rw [ ← mul_assoc]; rw [ ← ring_hom.map_mul],
 have h_yb : y * b ∈ J,
 { apply hle,
 ...
 rw [← hc]; rw [ ring_hom.map_mul]; rw [ mul_assoc]; rw [ mul_inv_cancel hnz_fa]; rw [ mul_one],
 apply coe_mem_one },
 { refine mt (mem_one_iff _).mp _,
 rintros ⟨x', h₂_abs⟩,
 rw [← div_eq_mul_inv] at h₂_abs; rw [ eq_div_iff_mul_eq hnz_fa] at h₂_abs; rw [ ← ring_hom.map_mul] at h₂_abs,
...
begin
 ...
 by_cases hI1 : I = ⊤,
 { rw [hI1]; rw [ coe_ideal_top]; rw [ one_mul]; rw [ inv_one] },
 ...
 { subst hJ0,
 refine absurd _ hI0,
 rw [eq_bot_iff]; rw [ ← coe_ideal_le_coe_ideal K]; rw [ hJ],
 exact coe_ideal_le_self_mul_inv K I,
 apply_instance },
 by_cases hJ1 : J = ⊤,
 { rw [← hJ]; rw [ hJ1]; rw [ coe_ideal_top] },
 obtain ⟨x, hx, hx1⟩ : ∃ (x : K),
...
begin
 ...
 { rw [hJ0]; rw [ inv_zero'], exact zero_le _ },
 intros x hx,
 -- In particular, we'll show all `x ∈ J⁻¹` are integral.
 suffices : x ∈ integral_closure A K,
 { rwa [is_integrally_closed.integral_closure_eq_bot] at this; rwa [ algebra.mem_bot] at this; rwa [ set.mem_range] at this; rwa [ ← mem_one_iff] at this;
 assumption },
...
begin
 ...
 subst hJ,
 rw [mul_assoc]; rw [ mul_left_comm (J : fractional_ideal A⁰ K)]; rw [ coe_ideal_mul_inv]; rw [ mul_one]; rw [ span_singleton_mul_span_singleton]; rw [ inv_mul_cancel]; rw [ span_singleton_one],
...
begin
 ...
 rw [mul_assoc]; rw [ fractional_ideal.mul_inv_cancel hJ]; rw [ mul_one] },
...
begin
 by_cases hJ : J = 0,
 { rw [hJ]; rw [ div_zero]; rw [ inv_zero']; rw [ mul_zero] },
 refine le_antisymm ((mul_right_le_iff hJ).mp _) ((le_div_iff_mul_le hJ).mpr _),
 { rw [mul_assoc]; rw [ mul_comm J⁻¹]; rw [ fractional_ideal.mul_inv_cancel hJ]; rw [ mul_one]; rw [ mul_le],
 intros x hx y hy,
 rw [mem_div_iff_of_nonzero hJ] at hx,
 exact hx y hy },
 rw [mul_assoc]; rw [ mul_comm J⁻¹]; rw [ fractional_ideal.mul_inv_cancel hJ]; rw [ mul_one],
 exact le_refl I
end
...
⟨ideal.le_of_dvd,
 λ h, begin
 by_cases hI : I = ⊥,
 { have hJ : J = ⊥, { rwa [hI] at h ; rwa [ ← eq_bot_iff] at h },
 rw [hI]; rw [ hJ] },
 have hI' : (I : fractional_ideal A⁰ (fraction_ring A)) ≠ 0 := coe_ideal_ne_zero.mpr hI,
 ...
 rw [coe_ideal_mul]; rw [ hH]; rw [ ← mul_assoc]; rw [ mul_inv_cancel hI']; rw [ one_mul]
end⟩
...
⟨λ ⟨hI, H, hunit, hmul⟩, lt_of_le_of_ne (ideal.dvd_iff_le.mp ⟨H, hmul⟩)
 (mt (λ h, have H = 1, from mul_left_cancel₀ hI (by rw [← hmul]; rw [ h]; rw [ mul_one]),
...
{ irreducible_iff_prime := λ P,
 ...
 rw [ideal.dvd_iff_le]; rw [ ideal.dvd_iff_le]; rw [ ideal.dvd_iff_le]; rw [ set_like.le_def]; rw [ set_like.le_def]; rw [ set_like.le_def],
 contrapose!,
...
begin
 ...
 rw [← ideal.dvd_not_unit_iff_lt]; rw [ dvd_not_unit_iff_normalized_factors_lt_normalized_factors]; rw [ normalized_factors_pow]; rw [ normalized_factors_irreducible P_prime'.irreducible]; rw [ multiset.nsmul_singleton]; rw [ multiset.lt_replicate_succ]
 at hlt,
 rw [← ideal.dvd_iff_le]; rw [ dvd_iff_normalized_factors_le_normalized_factors]; rw [ normalized_factors_pow]; rw [ normalized_factors_irreducible P_prime'.irreducible]; rw [ multiset.nsmul_singleton],
...
begin
 rw [← associates.dvd_eq_le]; rw [ ← associates.mk_pow]; rw [ associates.mk_dvd_mk]; rw [ ideal.dvd_span_singleton],
end
...
begin
 ...
 { rw [dvd_iff_le]; rw [ sup_le_iff]; rw [ ← dvd_iff_le]; rw [ ← dvd_iff_le],
 exact ⟨gcd_dvd_left _ _, gcd_dvd_right _ _⟩ },
 { rw [dvd_gcd_iff]; rw [ dvd_iff_le]; rw [ dvd_iff_le],
 simp } },
 have hlcm : lcm I J = I ⊓ J,
 { rw [lcm_eq_normalize _ _]; rw [ normalize_eq],
 { rw [lcm_dvd_iff]; rw [ dvd_iff_le]; rw [ dvd_iff_le],
 simp },
 { rw [dvd_iff_le]; rw [ le_inf_iff]; rw [ ← dvd_iff_le]; rw [ ← dvd_iff_le],
...
{ gcd := (⊔),
 ...
 gcd_mul_lcm := λ _ _, by rw [associated_iff_eq]; rw [ sup_mul_inf],
...
lemma inf_eq_mul_of_coprime {I J : ideal A} (coprime : I ⊔ J = ⊤) :
 I ⊓ J = I * J :=
by rw [← associated_iff_eq.mp (gcd_mul_lcm I J)]; rw [ lcm_eq_inf I J]; rw [ gcd_eq_sup]; rw [ coprime]; rw [ top_mul]
...
begin
 ...
 apply le_antisymm,
 { rw [sup_le_iff]; rw [ ← dvd_iff_le]; rw [ ← dvd_iff_le],
 split,
 { rw [dvd_iff_normalized_factors_le_normalized_factors this hI]; rw [ H],
 exact inf_le_left },
 { rw [dvd_iff_normalized_factors_le_normalized_factors this hJ]; rw [ H],
 exact inf_le_right } },
 { rw [← dvd_iff_le]; rw [ dvd_iff_normalized_factors_le_normalized_factors]; rw [ normalized_factors_prod_of_prime]; rw [ le_iff_count],
 { intro a,
...
by rw [sup_eq_prod_inf_factors (pow_ne_zero n hJ.ne_zero) hI]; rw [ min_comm]; rw [ normalized_factors_of_irreducible_pow hJ]; rw [ normalize_eq J]; rw [ replicate_inter]; rw [ prod_replicate]
...
begin
 ...
 rwa [multiplicity_eq_count_normalized_factors hJ hI] at hn; rwa [ part_enat.coe_le_coe] at hn; rwa [ normalize_eq J] at hn
end
...
begin
 ...
 congr,
 { rw [← part_enat.coe_inj]; rw [ part_enat.coe_get]; rw [ multiplicity_eq_count_normalized_factors hJ hI]; rw [ normalize_eq J] },
 { rwa [multiplicity_eq_count_normalized_factors hJ hI]; rwa [ part_enat.coe_le_coe]; rwa [ normalize_eq J]
 at hn }
end
...
begin
 ...
 exact hK },
 all_goals { rw [← maximal_spectrum.infi_localization_eq_bot]; rw [ algebra.mem_infi] },
...
{ to_fun := λ X, ⟨comap J^.quotient.mk (map f (map I^.quotient.mk X)),
 ...
 begin
 rintros ⟨X, hX⟩ ⟨Y, hY⟩ h,
 rw [← subtype.coe_le_coe] at h ⊢; rw [ subtype.coe_mk] at h ⊢; rw [ subtype.coe_mk] at h ⊢,
 rw [subtype.coe_mk]; rw [ comap_le_comap_iff_of_surjective J^.quotient.mk quotient.mk_surjective]; rw [ map_le_iff_le_comap]; rw [ subtype.coe_mk]; rw [ comap_map_of_surjective _ hf (map I^.quotient.mk Y)],
 suffices : map I^.quotient.mk X ≤ map I^.quotient.mk Y,
 { exact le_sup_of_le_left this },
 rwa [map_le_iff_le_comap]; rwa [ comap_map_of_surjective I^.quotient.mk quotient.mk_surjective]; rwa [ ← ring_hom.ker_eq_comap_bot]; rwa [ mk_ker]; rwa [ sup_eq_left.mpr $ le_of_dvd hY],
 end }
...
begin
 ...
 rw [ideal_factors_fun_of_quot_hom]; rw [ ideal_factors_fun_of_quot_hom]; rw [ order_hom.comp_coe]; rw [ order_hom.coe_fun_mk]; rw [ order_hom.coe_fun_mk]; rw [ function.comp_app]; rw [ ideal_factors_fun_of_quot_hom]; rw [ order_hom.coe_fun_mk]; rw [ subtype.mk_eq_mk]; rw [ subtype.coe_mk]; rw [ map_comap_of_surjective J^.quotient.mk quotient.mk_surjective]; rw [ map_map],
end
...
begin
 ...
 { rw [dvd_iff_le]; rw [ dvd_iff_le]; rw [ subtype.coe_le_coe]; rw [ this]; rw [ subtype.mk_le_mk] },
...
begin
 by_cases hI : I = ⊥,
 { exfalso,
 rw [hI] at hL; rw [ bot_eq_zero] at hL; rw [ normalized_factors_zero] at hL; rw [ ← multiset.empty_eq_zero] at hL,
 exact hL, },
...
begin
 rw [normalized_factors_equiv_of_quot_equiv]; rw [ equiv.coe_fn_mk]; rw [ subtype.coe_mk],
...
begin
 ...
 rw [← ideal.zero_eq_bot]; rw [ finset.prod_eq_zero_iff] },
...
begin
 ...
 rw [finset.inf_insert]; rw [ finset.prod_insert ha]; rw [ ih],
...
begin
 by_cases hb : b = 0,
 { rw [ideal.span_singleton_eq_bot.mpr hb]; rw [ bot_eq_zero]; rw [ normalized_factors_zero],
 rw [hb] at ha; rw [ normalized_factors_zero] at ha,
...
begin
 ...
 rw [ideal.span_singleton_pow]; rw [ span_singleton_dvd_span_singleton_iff_dvd],
 ...
 rw [h]; rw [ this] },
 refine not_finite_iff_forall.mpr (λ n, by {rw [ideal.span_singleton_pow]; rw [ span_singleton_dvd_span_singleton_iff_dvd], exact not_finite_iff_forall.mp h n }) }
end
...
begin
 split,
 { rintros ⟨a, ha⟩ ⟨b, hb⟩ h,
 rw [subtype.mk_eq_mk] at h; rw [ ideal.span_singleton_eq_span_singleton] at h; rw [ subtype.coe_mk] at h; rw [ subtype.coe_mk] at h,
...
begin
 ...
 obtain ⟨a, ha⟩ := x,
 rw [hx.symm]; rw [ equiv.symm_apply_apply]; rw [ subtype.coe_mk]; rw [ multiplicity_normalized_factors_equiv_span_normalized_factors_eq_multiplicity hr ha]; rw [ hx],
end
