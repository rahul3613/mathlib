...
begin
 ...
 { rw [zero_mul]; rw [ map_zero]; rw [ map_zero]; rw [ zero_smul]; rw [ smul_zero]; rw [ add_zero] },
 swap, { rintros, simp only [add_mul, map_add, add_smul, *, smul_add], rw add_add_add_comm },
 intros x₁ x₂,
 apply tensor_product.induction_on y,
 { rw [mul_zero]; rw [ map_zero]; rw [ map_zero]; rw [ zero_smul]; rw [ smul_zero]; rw [ add_zero] },
...
begin
 ...
 { rw [hx]; rw [ tensor_product.zero_tmul]; rw [ sub_zero] },
 rw ← this,
 clear this hx,
 apply tensor_product.induction_on x; clear x,
 { rw [map_zero]; rw [ tensor_product.zero_tmul]; rw [ sub_zero], exact zero_mem _ },
 { intros x y,
 convert_to x • (1 ⊗ₜ y - y ⊗ₜ 1) ∈ _,
 { rw [tensor_product.lmul'_apply_tmul]; rw [ smul_sub]; rw [ tensor_product.smul_tmul']; rw [ tensor_product.smul_tmul']; rw [ smul_eq_mul]; rw [ smul_eq_mul]; rw [ mul_one] },
 { refine submodule.smul_mem _ x _,
 apply submodule.subset_span,
 exact set.mem_range_self y } },
 { intros x y hx hy,
 rw [map_add]; rw [ tensor_product.add_tmul]; rw [ ← sub_add_sub_comm],
 exact add_mem hx hy } }
end
...
begin
 ...
 rw [← kaehler_differential.submodule_span_range_eq_ideal]; rw [ ideal.span],
...
{ map_one_eq_zero' := begin
 ...
 rw [ideal.to_cotangent_eq_zero]; rw [ subtype.coe_mk]; rw [ sub_self],
 exact zero_mem _
 end,
 leibniz' := λ a b, begin
 dsimp only [kaehler_differential.D_linear_map_apply],
 rw [← linear_map.map_smul_of_tower ((kaehler_differential.ideal R S).to_cotangent) a]; rw [ ← linear_map.map_smul_of_tower ((kaehler_differential.ideal R S).to_cotangent) b]; rw [ ← map_add]; rw [ ideal.to_cotangent_eq]; rw [ pow_two],
...
begin
 ...
 dsimp,
 rw [derivation.tensor_product_to_mul]; rw [ hx]; rw [ hy]; rw [ zero_smul]; rw [ zero_smul]; rw [ zero_add] },
 { intros x y ex ey, rw [map_add]; rw [ ex]; rw [ ey]; rw [ zero_add] } }
end
...
begin
 ...
 rw [subtype.coe_mk]; rw [ map_sub]; rw [ derivation.tensor_product_to_tmul]; rw [ derivation.tensor_product_to_tmul]; rw [ one_smul]; rw [ D.map_one_eq_zero]; rw [ smul_zero]; rw [ sub_zero],
end
...
begin
 ...
 { intros x y hx hy, rw [map_add]; rw [ map_add]; rw [ hx]; rw [ hy] },
 { intros a x e, rw [map_smul]; rw [ map_smul]; rw [ e] }
end
...
begin
 rw [← derivation.lift_kaehler_differential_apply]; rw [ derivation.lift_kaehler_differential_D],
 refl,
end
...
begin
 ...
 { intro x, rw [alg_hom.coe_to_ring_hom]; rw [ ← alg_hom.comp_apply]; rw [ tensor_product.lmul'_comp_include_left], refl },
...
begin
 rw [alg_hom.ext_iff]; rw [ alg_hom.ext_iff],
 apply forall_congr,
 intro x,
...
lemma kaehler_differential.ker_total_mkq_single_add (x y z) :
 (z 𝖣 (x + y)) = (z 𝖣 x) + (z 𝖣 y) :=
begin
 rw [← map_add]; rw [ eq_comm]; rw [ ← sub_eq_zero]; rw [ ← map_sub]; rw [ submodule.mkq_apply]; rw [ submodule.quotient.mk_eq_zero],
...
lemma kaehler_differential.ker_total_mkq_single_algebra_map_one (x) :
 (x 𝖣 1) = 0 :=
begin
 rw [← (algebra_map R S).map_one]; rw [ kaehler_differential.ker_total_mkq_single_algebra_map],
end

lemma kaehler_differential.ker_total_mkq_single_smul (r : R) (x y) :
 (y 𝖣 (r • x)) = r • (y 𝖣 x) :=
begin
 rw [algebra.smul_def]; rw [ kaehler_differential.ker_total_mkq_single_mul]; rw [ kaehler_differential.ker_total_mkq_single_algebra_map]; rw [ add_zero]; rw [ ← linear_map.map_smul_of_tower]; rw [ finsupp.smul_single]; rw [ mul_comm]; rw [ algebra.smul_def],
end
...
begin
 ...
 intros a b,
 conv_rhs { rw [← finsupp.smul_single_one a b]; rw [ linear_map.map_smul] },
...
begin
 ...
 { rw [kaehler_differential.ker_total]; rw [ submodule.span_le],
...
begin
 rw [← linear_map.range_eq_top]; rw [ finsupp.range_total]; rw [ kaehler_differential.span_range_derivation],
end
...
begin
 rw [kaehler_differential.ker_total]; rw [ submodule.map_span]; rw [ kaehler_differential.ker_total]; rw [ submodule.restrict_scalars_span _ _ h],
...
begin
 rw [← linear_map.range_eq_top]; rw [ _root_.eq_top_iff]; rw [ ← @submodule.restrict_scalars_top B A]; rw [ ← kaehler_differential.span_range_derivation]; rw [ submodule.restrict_scalars_span _ _ h]; rw [ submodule.span_le],
 rintros _ ⟨x, rfl⟩,
 obtain ⟨y, rfl⟩ := h x,
...
begin
 conv_lhs { rw [← mul_one x]; rw [ ← smul_eq_mul]; rw [ ← tensor_product.smul_tmul']; rw [ linear_map.map_smul] },
 congr' 1,
