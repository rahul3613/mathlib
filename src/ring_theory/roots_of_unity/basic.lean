...
begin
 ...
 rw [subgroup.coe_pow]; rw [ units.coe_pow],
end
...
{ change (σ (ξ : Rˣ)) ^ (n : ℕ) = 1,
 rw [←map_pow]; rw [ ←units.coe_pow]; rw [ show ((ξ : Rˣ) ^ (n : ℕ) = 1)]; rw [ from ξ.2]; rw [ units.coe_one]; rw [ map_one σ] } in
{ to_fun := λ ξ, ⟨@unit_of_invertible _ _ _ (invertible_of_pow_eq_one _ _ (h ξ) n.ne_zero),
 ...
 map_mul' := λ ξ₁ ξ₂, by { ext, rw [subgroup.coe_mul]; rw [ units.coe_mul], exact map_mul σ _ _ } }
...
begin
 ...
 rw [←restrict_roots_of_unity_coe_apply]; rw [ hm]; rw [ zpow_eq_mod_order_of]; rw [ ←int.to_nat_of_nonneg (m.mod_nonneg (int.coe_nat_ne_zero.mpr (pos_iff_ne_zero.mp (order_of_pos ζ))))]; rw [ zpow_coe_nat]; rw [ roots_of_unity.coe_pow],
...
begin
 rw [primitive_roots]; rw [ mem_filter]; rw [ multiset.mem_to_finset]; rw [ mem_nth_roots h0]; rw [ and_iff_right_iff_imp],
 exact is_primitive_root.pow_eq_one
end

@[simp] lemma primitive_roots_zero : primitive_roots 0 R = ∅ :=
by rw [primitive_roots]; rw [ nth_roots_zero]; rw [ multiset.to_finset_zero]; rw [ finset.filter_empty]
...
begin
 ...
 rw [← ((h.is_unit (lt_of_le_of_lt (nat.zero_le _) hi)).pow i).mul_left_inj]; rw [ ← pow_add]; rw [ tsub_add_cancel_of_le hij]; rw [ H]; rw [ one_mul]
end
...
begin
 ...
 refine
 { pow_eq_one := by rw [← pow_mul']; rw [ pow_mul]; rw [ h.pow_eq_one]; rw [ one_pow],
 dvd_of_pow_eq_one := _ },
 intros l hl,
 apply h.dvd_of_pow_eq_one,
 rw [← pow_one ζ]; rw [ ← zpow_coe_nat ζ]; rw [ ← hi.gcd_eq_one]; rw [ nat.gcd_eq_gcd_ab]; rw [ zpow_add]; rw [ mul_pow]; rw [ ← zpow_coe_nat]; rw [ ← zpow_mul]; rw [ mul_right_comm],
...
begin
 ...
 suffices : b = k,
 { rwa [this] at hb { occs := occurrences.pos [1] ; rwa [ ← one_mul k] at hb { occs := occurrences.pos [1] ; rwa [ mul_left_inj' h0.ne'] at hb { occs := occurrences.pos [1] ; rwa [ eq_comm] at hb { occs := occurrences.pos [1] } },
 rw [ha] at hi,
 rw [mul_comm] at hb,
...
begin
 ...
 rw [order_of_pow' _ hp]; rw [ ← eq_order_of h]; rw [ nat.gcd_eq_right hdiv]
end
...
{ pow_eq_one := by rw [←map_pow]; rw [ h.pow_eq_one]; rw [ _root_.map_one],
...
begin
 ...
 rw [← dvd_neg]; rw [ ← hl'],
 norm_cast,
 rw [← h.pow_eq_one_iff_dvd]; rw [ ← inv_inj]; rw [ ← zpow_neg]; rw [ ← hl']; rw [ zpow_coe_nat]; rw [ inv_one] }
end
...
begin
 ...
 rw [← inv_iff]; rw [ ← zpow_neg]; rw [ ← hi']; rw [ zpow_coe_nat],
 apply h.pow_of_coprime,
 rw [int.gcd] at hi; rw [ ← int.nat_abs_neg] at hi; rw [ ← hi'] at hi,
 exact hi
end
...
begin
 ...
 { intros x hx,
 rw [mem_primitive_roots zero_lt_one] at hx; rw [ is_primitive_root.one_right_iff] at hx,
 exact hx }
end
...
begin
 ...
 { rw [hm.1]; rw [ hk]; rw [ pow_succ]; rw [ mul_assoc]; rw [ mul_comm p],
...
by rw [eq_neg_iff_add_eq_zero]; rw [ add_comm]; rw [ ←sum_range_succ]; rw [ ←nat.succ_eq_add_one]; rw [ nat.succ_pred_eq_of_pos (pos_of_gt hk)]; rw [ hζ.geom_sum_eq_zero hk]
...
add_equiv.of_bijective
 ...
 rw [← (char_p.int_cast_eq_zero_iff (zmod k) k _).mpr this]; rw [ eq_comm],
...
by rw [← h.zmod_equiv_zpowers.symm_apply_apply i]; rw [ zmod_equiv_zpowers_apply_coe_int]
...
@[simp] lemma zmod_equiv_zpowers_symm_apply_pow (i : ℕ) :
 h.zmod_equiv_zpowers.symm (additive.of_mul (⟨ζ ^ i, i, rfl⟩ : subgroup.zpowers ζ)) = i :=
by rw [← h.zmod_equiv_zpowers.symm_apply_apply i]; rw [ zmod_equiv_zpowers_apply_coe_nat]
...
begin
 ...
 rw [← zpow_coe_nat]; rw [ hi₀]; rw [ ← int.mod_add_div n k]; rw [ zpow_add]; rw [ zpow_mul]; rw [ aux]; rw [ one_zpow]; rw [ mul_one] }
end
...
begin
 ...
 rw [mem_roots_of_unity]; rw [ units.ext_iff]; rw [ units.coe_pow]; rw [ hξ]; rw [ units.coe_one]
end
...
begin
 ...
 rw [← hζ] at h; rw [ is_primitive_root.coe_units_iff] at h,
...
begin
 ...
 set m := nat.to_pnat' n,
 rw [← fintype.card_congr (roots_of_unity_equiv_nth_roots R m)] at hcard; rw [ card_roots_of_unity h] at hcard,
 exact hcard }
end
...
begin
 ...
 set m := nat.to_pnat' n,
 rw [hrw] at ha; rw [ ← fintype.card_congr (roots_of_unity_equiv_nth_roots R m)] at ha; rw [ card_roots_of_unity h] at ha,
 exact nat.lt_asymm ha ha }
end

@[simp] lemma card_nth_roots_finset {ζ : R} {n : ℕ} (h : is_primitive_root ζ n) :
 (nth_roots_finset n R).card = n :=
by rw [nth_roots_finset]; rw [ ← multiset.to_finset_eq (nth_roots_nodup h)]; rw [ card_mk]; rw [ h.card_nth_roots]
...
begin
 ...
 intros ξ hξ,
 rw [mem_primitive_roots (nat.pos_of_ne_zero h0)] at hξ; rw [ h.is_primitive_root_iff (nat.pos_of_ne_zero h0)] at hξ,
...
begin
 ...
 { apply le_of_eq,
 rw [h.card_nth_roots_finset]; rw [ finset.card_bUnion],
...
have ho : order_of μ' = n :=
 by rw [hμ.eq_order_of]; rw [ ←hμ.coe_to_roots_of_unity_coe]; rw [ order_of_units]; rw [ order_of_subgroup],
...
{ to_fun := λ σ, (map_root_of_unity_eq_pow_self σ.to_alg_hom μ').some,
 ...
 rw [←@nat.cast_one $ zmod n]; rw [ zmod.nat_coe_eq_nat_coe_iff]; rw [ ←ho]; rw [ ←pow_eq_pow_iff_modeq μ']; rw [ h]
 end,
 map_mul' := begin
 ...
 rw [←nat.cast_mul]; rw [ zmod.nat_coe_eq_nat_coe_iff]; rw [ ←ho]; rw [ ←pow_eq_pow_iff_modeq μ']; rw [ hxy]
 end }
...
begin
 ...
 rw [←roots_of_unity.coe_pow]; rw [ ←roots_of_unity.coe_pow],
 congr' 1,
 rw [pow_eq_pow_iff_modeq]; rw [ ←order_of_subgroup]; rw [ ←order_of_units]; rw [ hμ.coe_to_roots_of_unity_coe]; rw [ ←hμ.eq_order_of]; rw [ zmod.val_nat_cast],
