...
begin
 ...
 rintro l k hk,
 have : n ∣ i * l,
 { rw [← int.coe_nat_dvd]; rw [ hk], apply dvd_mul_left },
...
begin
 rw [mem_roots_of_unity]; rw [ units.ext_iff]; rw [ units.coe_pow]; rw [ units.coe_one],
...
begin
 ...
 exact zero_le _ },
 rw [←int.nat_abs_neg]; rw [ neg_sub]; rw [ int.nat_abs_eq_iff],
 ...
 rw [←mul_rotate']; rw [ mul_div_assoc]; rw [ neg_lt]; rw [ ←mul_neg]; rw [ mul_lt_iff_lt_one_right real.pi_pos]; rw [ ←neg_div]; rw [ ←neg_mul]; rw [ neg_sub]; rw [ div_lt_iff]; rw [ one_mul]; rw [ sub_mul]; rw [ sub_lt_comm]; rw [ ←mul_sub_one],
 norm_num,
