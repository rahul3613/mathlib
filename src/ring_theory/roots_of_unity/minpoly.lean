...
begin
 ...
 { rw [aeval_def]; rw [ coe_expand]; rw [ ← comp]; rw [ eval₂_eq_eval_map]; rw [ map_comp]; rw [ polynomial.map_pow]; rw [ map_X]; rw [ eval_comp]; rw [ eval_pow]; rw [ eval_X]; rw [ ← eval₂_eq_eval_map]; rw [ ← aeval_def],
...
begin
 ...
 by rw [← zmod.expand_card]; rw [ map_expand],
 rw [hfrob],
...
begin
 ...
 have prod : P * Q ∣ X ^ n - 1,
 { rw [(is_primitive.int.dvd_iff_map_cast_dvd_map_cast (P * Q) (X ^ n - 1) PQprim (monic_X_pow_sub_C (1 : ℤ) (ne_of_gt hpos)).is_primitive)]; rw [ polynomial.map_mul],
 ...
 rw [coe_map_ring_hom] at prod; rw [ polynomial.map_mul] at prod; rw [ polynomial.map_sub] at prod; rw [ polynomial.map_one] at prod; rw [ polynomial.map_pow] at prod; rw [ map_X] at prod,
