...
{ carrier := { z : S | ∃ x : I × M, z * algebra_map R S x.2 = algebra_map R S x.1},
 ...
 rw [add_mul]; rw [ ← mul_assoc a]; rw [ ha]; rw [ mul_comm (algebra_map R S a'.2) (algebra_map R S b'.2)]; rw [ ← mul_assoc b]; rw [ hb],
 ring
 end,
 smul_mem' := begin
 rintros c x ⟨x', hx⟩,
...
begin
 ...
 have : (↑c * ↑s) * a ∈ I := by { rw [mul_assoc]; rw [ hc], exact I.mul_mem_left c b.2 },
...
begin
 ...
 rw [eq_top_iff]; rw [ ← (order_embedding M S).le_iff_le],
 exact le_of_eq hJ.symm },
 { intros x y hxy,
 rw [ideal.mem_comap] at hxy; rw [ ring_hom.map_mul] at hxy,
 exact h.mem_or_mem hxy } },
 { refine λ h, ⟨λ hJ, h.left.ne_top (eq_top_iff.2 _), _⟩,
 { rwa [eq_top_iff] at hJ ; rwa [ ← (order_embedding M S).le_iff_le] at hJ },
 { intros x y hxy,
 obtain ⟨a, s, ha⟩ := mk'_surjective M x,
 obtain ⟨b, t, hb⟩ := mk'_surjective M y,
 have : mk' S (a * b) (s * t) ∈ J := by rwa [mk'_mul]; rwa [ ha]; rwa [ hb],
 ...
 rwa [← ha]; rwa [ ← hb]; rwa [ mk'_mem_iff]; rwa [ mk'_mem_iff] } }
end
...
begin
 rw [is_prime_iff_is_prime_disjoint M S]; rw [ comap_map_of_is_prime_disjoint M S I hp hd],
 exact ⟨hp, hd⟩
end
...
begin
 ...
 rw [ideal.quotient_map_mk]; rw [ ← sub_eq_zero]; rw [ ← ring_hom.map_sub]; rw [ ideal.quotient.eq_zero_iff_mem]; rw [ ← ideal.quotient.eq_zero_iff_mem]; rw [ ring_hom.map_sub]; rw [ sub_eq_zero]; rw [ mk'_eq_mul_mk'_one],
 ...
 (by rw [← ring_hom.map_mul]; rw [ ← mk'_eq_mul_mk'_one]; rw [ mk'_self]; rw [ ring_hom.map_one]))) }
end
