...
by rw [coe_submodule]; rw [ submodule.map_bot]

@[simp] lemma coe_submodule_top : coe_submodule S (⊤ : ideal R) = 1 :=
by rw [coe_submodule]; rw [ submodule.map_top]; rw [ submodule.one_eq_range]
...
by { rw [is_localization.coe_submodule]; rw [ ideal.span]; rw [ submodule.map_span], refl }

@[simp]
lemma coe_submodule_span_singleton (x : R) :
 coe_submodule S (ideal.span {x}) = submodule.span R {(algebra_map R S) x} :=
by rw [coe_submodule_span]; rw [ set.image_singleton]
...
begin
 rw [is_noetherian_ring_iff] at h ⊢; rw [ is_noetherian_iff_well_founded] at h ⊢,
...
begin
 ...
 rw [ideal.submodule_span_eq]; rw [ hx]; rw [ coe_submodule_span_singleton] },
 { refine ⟨⟨algebra_map R S x, _⟩⟩,
 rw [hx]; rw [ ideal.submodule_span_eq]; rw [ coe_submodule_span_singleton] }
end
...
begin
 ...
 { rintros x hx,
 exact ⟨x, submodule.subset_span hx, 1, by rw [mk'_one]; rw [ _root_.map_one]; rw [ one_smul]⟩ },
 { exact ⟨0, submodule.zero_mem _, 1, by rw [mk'_one]; rw [ _root_.map_one]; rw [ one_smul]⟩ },
 { rintros _ _ ⟨y, hy, z, rfl⟩ ⟨y', hy', z', rfl⟩,
 refine ⟨(z' : R) • y + (z : R) • y',
 (submodule.add_mem _ (submodule.smul_mem _ _ hy) (submodule.smul_mem _ _ hy')), z * z', _⟩,
 rw [smul_add]; rw [ ← is_scalar_tower.algebra_map_smul S (z : R)]; rw [ ← is_scalar_tower.algebra_map_smul S (z' : R)]; rw [ smul_smul]; rw [ smul_smul],
 congr' 1,
 { rw [← mul_one (1 : R)]; rw [ mk'_mul]; rw [ mul_assoc]; rw [ mk'_spec]; rw [ _root_.map_one]; rw [ mul_one]; rw [ mul_one] },
 { rw [← mul_one (1 : R)]; rw [ mk'_mul]; rw [ mul_right_comm]; rw [ mk'_spec]; rw [ _root_.map_one]; rw [ mul_one]; rw [ one_mul] },
 all_goals { apply_instance } },
 { rintros a _ ⟨y, hy, z, rfl⟩,
 obtain ⟨y', z', rfl⟩ := mk'_surjective M a,
 refine ⟨y' • y, submodule.smul_mem _ _ hy, z' * z, _⟩,
 rw [← is_scalar_tower.algebra_map_smul S y']; rw [ smul_smul]; rw [ ← mk'_mul]; rw [ smul_smul]; rw [ mul_comm (mk' S _ _)]; rw [ mul_mk'_eq_mk'_of_mul],
 all_goals { apply_instance } } },
 { rintro ⟨y, hy, z, rfl⟩,
...
begin
 ...
 refine ⟨y, hy, z, _⟩,
 rw [hz]; rw [ algebra.linear_map_apply]; rw [ smul_eq_mul]; rw [ mul_comm]; rw [ mul_mk'_eq_mk'_of_mul]; rw [ mul_one] },
 { rintros ⟨y, hy, z, hz⟩,
