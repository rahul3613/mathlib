...
{ map_units :=
 ...
 rwa [mem_non_zero_divisors_iff_ne_zero]; rwa [ int.coe_nat_ne_zero_iff_pos]
 end,
 eq_iff_exists :=
 begin
 intros x y,
 rw [eq_int_cast]; rw [ eq_int_cast]; rw [ int.cast_inj],
...
begin
 rw [is_fraction_ring.inv]; rw [ dif_neg hx]; rw [ ←is_unit.mul_left_inj (map_units K ⟨(sec _ x).1, mem_non_zero_divisors_iff_ne_zero.2 $ λ h0, hx $ eq_zero_of_fst_eq_zero (sec_spec (non_zero_divisors A) x) h0⟩)]; rw [ one_mul]; rw [ mul_assoc],
...
begin
 refine ⟨λ hxy, _, λ h, by rw [h]; rw [ mk'_zero]⟩,
...
begin
 ...
 erw [h.to_equiv.image_eq_preimage]; erw [ set.preimage]; erw [ set.mem_set_of_eq]; erw [ mem_non_zero_divisors_iff_ne_zero]; erw [ mem_non_zero_divisors_iff_ne_zero],
...
by rw [localization.mk_eq_mk']; rw [ is_fraction_ring.mk'_eq_div]
