...
begin
 contrapose h,
 rw [ne.def]; rw [ not_not]; rw [ coeff_integer_normalization]; rw [ dif_neg h]
end
...
begin
 ...
 intro i,
 rw [integer_normalization_coeff]; rw [ coeff_integer_normalization],
 split_ifs with hi,
...
⟨b, polynomial.ext (λ i, by { rw [coeff_map]; rw [ coeff_smul], exact hb i })⟩
...
trans (eval₂_map (algebra_map R S) g x).symm
 (by rw [hb]; rw [ ← is_scalar_tower.algebra_map_smul S (b : R) p]; rw [ eval₂_smul]; rw [ hx]; rw [ mul_zero])
...
by rw [aeval_def]; rw [ is_scalar_tower.algebra_map_eq R S R']; rw [ integer_normalization_eval₂_eq_zero _ _ _ hx]
...
begin
 ...
 erw [eval₂_map]; erw [ is_localization.map_comp]; erw [ ← hom_eval₂ _ f (algebra_map S Sₘ) x],
...
begin
 ...
 rw [ring_hom.map_mul] at hv'; rw [ ring_hom.map_one] at hv'; rw [ ← ring_hom.comp_apply _ (algebra_map R Rₘ)] at hv',
...
begin
 ...
 { rwa [h₂]; rwa [ polynomial.coeff_nat_degree]; rwa [ tsub_self]; rwa [ pow_zero]; rwa [ _root_.mul_one] },
...
begin
 ...
 rw [is_scalar_tower.algebra_map_eq R Rₘ S]; rw [ ← polynomial.eval₂_map]; rw [ hp'₁]; rw [ submonoid.smul_def]; rw [ algebra.smul_def]; rw [ is_scalar_tower.algebra_map_apply R Rₘ S],
...
{ map_units := λ ⟨y, hy⟩,
 ...
 (by rw [is_scalar_tower.algebra_map_apply A C L]; rw [ h]; rw [ ring_hom.map_zero])))⟩,
 by rw [set_like.coe_mk]; rw [ algebra_map_mk']; rw [ ← is_scalar_tower.algebra_map_apply A C L]; rw [ hxy]⟩,
...
begin
 ...
 split,
 { intros h x,
 rw [is_fraction_ring.is_algebraic_iff R (fraction_ring R) K]; rw [ is_algebraic_iff_is_integral],
 ...
 split,
 { rwa [ne.def]; rwa [ polynomial.reverse_eq_zero]; rwa [ ← polynomial.degree_eq_bot]; rwa [ polynomial.degree_map_eq_of_injective (no_zero_smul_divisors.algebra_map_injective R (fraction_ring R))]; rwa [ polynomial.degree_eq_bot]},
 ...
 rw [polynomial.aeval_def]; rw [ ← inv_of_eq_inv]; rw [ polynomial.eval₂_reverse_eq_zero_iff]; rw [ polynomial.eval₂_map]; rw [ ← is_scalar_tower.algebra_map_eq]; rw [ ← polynomial.aeval_def]; rw [ polynomial.aeval_algebra_map_apply]; rw [ hf₂]; rw [ ring_hom.map_zero] } } },
 { intros h x,
...
begin
 ...
 rwa [← ring_hom.coe_comp]; rwa [ ← is_scalar_tower.algebra_map_eq] },
 ...
 { rw [algebra.smul_def]; rw [ mul_comm _ y]; rw [ mul_comm _ y']; rw [ ← set_like.coe_mk (algebra_map R S z) hz0']
 at yz_eq,
 exact is_localization.mk'_eq_of_eq (by rw [mul_comm _ y]; rw [ mul_comm _ y']; rw [ yz_eq]), },
 suffices hy : algebra_map S L (a * y) ∈ submodule.span K (⇑(algebra_map S L) '' b),
 { rw [mk_yz_eq]; rw [ is_fraction_ring.mk'_eq_div]; rw [ set_like.coe_mk]; rw [ ← is_scalar_tower.algebra_map_apply]; rw [ is_scalar_tower.algebra_map_apply R K L]; rw [ div_eq_mul_inv]; rw [ ← mul_assoc]; rw [ mul_comm]; rw [ ← map_inv₀]; rw [ ← algebra.smul_def]; rw [ ← _root_.map_mul],
