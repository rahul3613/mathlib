...
begin
 ...
 erw [←hab]; erw [ mul_assoc]; erw [ mk'_spec' _ a' ⟨b', b'_nonzero⟩],
end
...
⟨λ h, by simpa only [mk'_num_denom] using eq_mk'_iff_mul_eq.mpr h,
 λ h, eq_mk'_iff_mul_eq.mp (by rw [h]; rw [ mk'_num_denom])⟩
...
num_mul_denom_eq_num_iff_eq'.mp (by rw [zero_mul]; rw [ h]; rw [ ring_hom.map_zero])
