...
begin
 ...
 obtain ⟨u, hu⟩ := H c,
 rwa [← hu] at eq ; rwa [ units.mul_right_inj] at eq },
 { intros y,
 obtain ⟨⟨x, s⟩, eq⟩ := is_localization.surj M y,
 obtain ⟨u, hu⟩ := H s,
 use x * u.inv,
 dsimp only [algebra.of_id, ring_hom.to_fun_eq_coe, alg_hom.coe_mk],
 rw [ring_hom.map_mul]; rw [ ← eq]; rw [ ← hu]; rw [ mul_assoc]; rw [ ← ring_hom.map_mul],
 simp }
end
...
begin
 by_cases h : m ≤ n,
 { rw [is_localization.eq_mk'_iff_mul_eq]; rw [ submonoid.pow_apply]; rw [ subtype.coe_mk]; rw [ ← int.coe_nat_sub h]; rw [ self_zpow_coe_nat]; rw [ ← map_pow]; rw [ ← map_mul]; rw [ ← pow_add]; rw [ nat.sub_add_cancel h] },
 { rw [← neg_sub]; rw [ ← int.coe_nat_sub (le_of_not_le h)]; rw [ self_zpow_neg_coe_nat]; rw [ is_localization.mk'_eq_iff_eq],
...
begin
 ...
 { rw [self_zpow_of_nonneg _ _ hn]; rw [ self_zpow_of_nonneg _ _ hm]; rw [ self_zpow_of_nonneg _ _ (add_nonneg hn hm)]; rw [ int.nat_abs_add_nonneg hn hm]; rw [ pow_add] },
 { have : n + m = n.nat_abs - m.nat_abs,
 { rw [int.nat_abs_of_nonneg hn]; rw [ int.of_nat_nat_abs_of_nonpos hm.le]; rw [ sub_neg_eq_add] },
 rw [self_zpow_of_nonneg _ _ hn]; rw [ self_zpow_of_neg _ _ hm]; rw [ this]; rw [ self_zpow_sub_cast_nat]; rw [ is_localization.mk'_eq_mul_mk'_one]; rw [ map_pow] },
 { have : n + m = m.nat_abs - n.nat_abs,
 { rw [int.nat_abs_of_nonneg hm]; rw [ int.of_nat_nat_abs_of_nonpos hn.le]; rw [ sub_neg_eq_add]; rw [ add_comm] },
 rw [self_zpow_of_nonneg _ _ hm]; rw [ self_zpow_of_neg _ _ hn]; rw [ this]; rw [ self_zpow_sub_cast_nat]; rw [ is_localization.mk'_eq_mul_mk'_one]; rw [ map_pow]; rw [ mul_comm] },
 { rw [self_zpow_of_neg _ _ hn]; rw [ self_zpow_of_neg _ _ hm]; rw [ self_zpow_of_neg _ _ (add_neg hn hm)]; rw [ int.nat_abs_add_neg hn hm]; rw [ ← mk'_mul]; rw [ one_mul],
 congr,
 ext,
 simp [pow_add] },
end
...
begin
 by_cases hd : d ≤ 0,
 { erw [self_zpow_of_nonpos x B hd]; erw [ self_zpow_of_nonneg]; erw [ ← map_pow]; erw [ int.nat_abs_neg]; erw [ is_localization.mk'_spec]; erw [ map_one],
 apply nonneg_of_neg_nonpos,
 rwa [neg_neg]},
 { erw [self_zpow_of_nonneg x B (le_of_not_le hd)]; erw [ self_zpow_of_nonpos]; erw [ ← map_pow]; erw [ int.nat_abs_neg]; erw [ @is_localization.mk'_spec' R _ (submonoid.powers x) B _ _ _ 1 (submonoid.pow x d.nat_abs)]; erw [ map_one],
 refine nonpos_of_neg_nonneg (le_of_lt _),
 rwa [neg_neg]; rwa [ ← not_le] },
end

lemma self_zpow_neg_mul (d : ℤ) : self_zpow x B (-d) * self_zpow x B d = 1 :=
by rw [mul_comm]; rw [ self_zpow_mul_neg x B d]
...
begin
 rw [sub_eq_add_neg]; rw [ self_zpow_add]; rw [ mul_assoc]; rw [ mul_comm _ (mk' B a 1)]; rw [ ← mul_assoc],
 split,
 { intro h,
 have := congr_arg (λ s : B, s * self_zpow x B d) h,
 simp only at this,
 rwa [mul_assoc] at this; rwa [ mul_assoc] at this; rwa [ self_zpow_neg_mul] at this; rwa [ mul_one] at this; rwa [ mul_comm b _] at this},
 { intro h,
 have := congr_arg (λ s : B, s * self_zpow x B (-d)) h,
 simp only at this,
 rwa [mul_comm _ b] at this; rwa [ mul_assoc b _ _] at this; rwa [ self_zpow_mul_neg] at this; rwa [ mul_one] at this}
end
...
begin
 ...
 refine ⟨a, m-d, _⟩,
 rw [← mk'_one B]; rw [ self_zpow_pow_sub]; rw [ self_zpow_coe_nat]; rw [ self_zpow_coe_nat]; rw [ ← map_pow _ _ d]; rw [ mul_comm _ b]; rw [ H]; rw [ hyp2]; rw [ map_mul]; rw [ map_pow _ _ m],
