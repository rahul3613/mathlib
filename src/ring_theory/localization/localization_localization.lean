...
begin
 rw [localization_localization_submodule]; rw [ submonoid.mem_comap]; rw [ submonoid.mem_sup],
 split,
...
begin
 ...
 rw [is_scalar_tower.algebra_map_apply R S T]; rw [ eq]; rw [ ring_hom.map_mul]; rw [ is_unit.mul_iff],
...
begin
 ...
 refine ⟨s, t * t', _⟩,
 rw [ring_hom.map_mul]; rw [ ← eq₃]; rw [ mul_assoc]; rw [ ← ring_hom.map_mul]; rw [ mul_comm t]; rw [ submonoid.coe_mul] },
...
begin
 rw [is_scalar_tower.algebra_map_apply R S T]; rw [ is_scalar_tower.algebra_map_apply R S T]; rw [ is_localization.eq_iff_exists N T],
 split,
 { rintros ⟨z, eq₁⟩,
 ...
 swap,
 { rw [map_mul]; rw [ map_mul]; rw [ ←eq₂]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_comm _ ↑z]; rw [ eq₁]; rw [ mul_comm _ ↑z] },
 use c * z',
 { rw mem_localization_localization_submodule,
 refine ⟨z, c * s, _⟩,
 rw [map_mul]; rw [ ← eq₂]; rw [ submonoid.coe_mul]; rw [ map_mul]; rw [ mul_left_comm] },
 { rwa [mul_comm _ z'] at eq₃ ; rwa [ mul_comm _ z'] at eq₃ ; rwa [ ←mul_assoc] at eq₃ ; rwa [ ←mul_assoc] at eq₃ } },
 ...
 rw [←mul_assoc]; rw [ mul_comm _ ↑z₁]; rw [ ←eq₂]; rw [ ←map_mul]; rw [ eq₁]; rw [ map_mul]; rw [ eq₂]; rw [ ←mul_assoc]; rw [ mul_comm _ ↑z₁] }
end
...
begin
 ...
 intro hx',
 apply @zero_ne_one S,
 rw [← (algebra_map R S).map_one]; rw [ ← @mk'_one R _ M]; rw [ @comm _ eq]; rw [ mk'_eq_zero_iff],
