...
begin
 ...
 { rw [← @smul_zero _ M _ _ (a : R)]; rw [ ← hg]; rw [ finset.smul_sum],
 refine finset.sum_congr rfl (λ i hi, _),
 rw [← is_scalar_tower.algebra_map_smul Rₛ]; rw [ hg' i hi]; rw [ smul_assoc],
 apply_instance },
 refine ((is_localization.map_units Rₛ a).mul_right_eq_zero).mp _,
 rw [← algebra.smul_def]; rw [ ← map_zero (algebra_map R Rₛ)]; rw [ ← hli]; rw [ hg' i hi],
end
end add_comm_monoid
...
begin
 ...
 rw [map_sum]; rw [ ← hg],
 refine finset.sum_congr rfl (λ i hi, _),
 rw [← smul_assoc]; rw [ ← hg' i hi]; rw [ algebra.smul_def]; rw [ map_mul]; rw [ ← is_scalar_tower.algebra_map_apply]; rw [ ← algebra.smul_def]; rw [ algebra_map_smul] },
 ...
 rw [← (is_localization.map_units Rₛ a).mul_right_eq_zero]; rw [ ← algebra.smul_def]; rw [ ← hg' i hi],
...
begin
 ...
 rw [is_localization.mk'_eq_mul_mk'_one]; rw [ mul_comm]; rw [ ← map_one (algebra_map R A)],
 erw ← is_localization.algebra_map_mk' A Rₛ Aₛ (1 : R) ⟨s, hs⟩, -- `erw` needed to unify `⟨s, hs⟩`
 rw ← algebra.smul_def,
 refine smul_mem _ _ (span_subset_span R _ _ _),
 rw [← algebra.coe_linear_map]; rw [ ← linear_map.coe_restrict_scalars R]; rw [ ← linear_map.map_span],
...
basis.mk
 ...
 (by { rw [set.range_comp]; rw [ span_eq_top.localization_localization Rₛ S Aₛ b.span_eq],
 exact le_rfl })
...
calc (b.localization_localization Rₛ S Aₛ).repr (algebra_map A Aₛ x) i
 ...
 by simp_rw [is_scalar_tower.algebra_map_smul, algebra.smul_def, is_scalar_tower.algebra_map_apply R A Aₛ, ← _root_.map_mul, ← map_finsupp_sum, ← algebra.smul_def, ← finsupp.total_apply, basis.total_repr]
... = (b.repr x).sum (λ j c, algebra_map R Rₛ c • finsupp.single j 1 i) :
 by simp_rw [← b.localization_localization_apply Rₛ S Aₛ, map_finsupp_sum, linear_equiv.map_smul, basis.repr_self, finsupp.sum_apply, finsupp.smul_apply]
