...
by { rw [norm_apply]; rw [ linear_map.det], split_ifs with h, refl }
...
by { rwa [norm_apply]; rwa [ ← linear_map.det_to_matrix b]; rwa [ ← to_matrix_lmul_eq], refl }
...
begin
 ...
 rw [norm_apply]; rw [ ← det_to_matrix b]; rw [ lmul_algebra_map],
 convert @det_diagonal _ _ _ _ _ (λ (i : ι), x),
 { ext i j, rw [to_matrix_lsmul]; rw [ matrix.diagonal] },
 { rw [finset.prod_const]; rw [ finset.card_univ] }
end
...
begin
 ...
 { rw [norm_algebra_map_of_basis H.some_spec.some]; rw [ finrank_eq_card_basis H.some_spec.some] },
 { rw [norm_eq_one_of_not_exists_basis K H]; rw [ finrank_eq_zero_of_not_exists_basis]; rw [ pow_zero],
 rintros ⟨s, ⟨b⟩⟩,
 exact H ⟨s, ⟨b⟩⟩ },
end
...
by rw [norm_eq_matrix_det pb.basis]; rw [ det_eq_sign_charpoly_coeff]; rw [ charpoly_left_mul_matrix]; rw [ fintype.card_fin]
...
begin
 ...
 rw [power_basis.norm_gen_eq_coeff_zero_minpoly]; rw [ ← pb.nat_degree_minpoly]; rw [ ring_hom.map_mul]; rw [ ← coeff_map]; rw [ prod_roots_eq_coeff_zero_of_monic_of_split (this.map _) ((splits_id_iff_splits _).2 hf)]; rw [ this.nat_degree_map]; rw [ map_pow]; rw [ ← mul_assoc]; rw [ ← mul_pow],
...
begin
 nontriviality,
 rw [norm_apply]; rw [ coe_lmul_eq_mul]; rw [ map_zero]; rw [ linear_map.det_zero' (module.free.choose_basis R S)]
end
...
begin
 ...
 rw [norm_eq_matrix_det b]; rw [ ← matrix.exists_mul_vec_eq_zero_iff],
 rintros ⟨v, v_ne, hv⟩,
 rw [← b.equiv_fun.apply_symm_apply v] at hv; rw [ b.equiv_fun_symm_apply] at hv; rw [ b.equiv_fun_apply] at hv; rw [ left_mul_matrix_mul_vec_repr] at hv,
 ...
 rw [b.equiv_fun_symm_apply]; rw [ sum_eq]; rw [ linear_equiv.map_zero] } },
end
...
begin
 ...
 rw [← adjoin_simple.algebra_map_gen K x]; rw [ norm_eq_matrix_det (pbx.basis.smul pbL.basis) _]; rw [ smul_left_mul_matrix_algebra_map]; rw [ det_block_diagonal]; rw [ norm_eq_matrix_det pbx.basis],
 simp only [finset.card_fin, finset.prod_const],
 congr,
 rw [← power_basis.finrank]; rw [ adjoin_simple.algebra_map_gen K x]
end
...
begin
 ...
 apply_instance },
 rw [← adjoin.power_basis_gen hx]; rw [ power_basis.norm_gen_eq_prod_roots];
...
begin
 ...
 rw [pb.norm_gen_eq_prod_roots hE]; rw [ fintype.prod_equiv pb.lift_equiv']; rw [ finset.prod_mem_multiset]; rw [ finset.prod_eq_multiset_prod]; rw [ multiset.to_finset_val]; rw [ multiset.dedup_eq_self.mpr]; rw [ multiset.map_id],
 { exact nodup_roots hfx.map },
 { intro x, refl },
 { intro σ, rw [pb.lift_equiv'_apply_coe]; rw [ id.def] }
end
...
by rw [norm_eq_norm_adjoin K x]; rw [ map_pow]; rw [ intermediate_field.adjoin_simple.norm_gen_eq_prod_roots _ hF]
...
begin
 ...
 rw [norm_eq_norm_adjoin K x]; rw [ ring_hom.map_pow]; rw [ ← adjoin.power_basis_gen hx]; rw [ norm_eq_prod_embeddings_gen E (adjoin.power_basis hx) (is_alg_closed.splits_codomain _)],
...
begin
 ...
 rw [← is_integral_algebra_map_iff (algebra_map K (algebraic_closure L)).injective]; rw [ norm_eq_prod_roots],
...
begin
 ...
 λ _, infer_instance,
 rw [norm_eq_prod_embeddings K A (_ : F)]; rw [ fintype.prod_equiv alg_hom_equiv_sigma (λ σ : F →ₐ[K] A, σ x) (λ π : (Σ f : L →ₐ[K] A, _), (π.2 : F → A) x) (λ _, rfl)],
 suffices : ∀ σ : L →ₐ[K] A,
 ...
 rw [norm_eq_one_of_not_module_finite hLF]; rw [ _root_.map_one], },
