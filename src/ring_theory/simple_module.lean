...
begin
 ...
 rw [covby_iff_coatom_Iic hAB]; rw [ is_simple_module_iff_is_coatom]; rw [ ←order_iso.is_coatom_iff f]; rw [ hf],
...
{ is_maximal := (⋖),
 ...
 second_iso := λ A B h,
 ⟨by { rw [sup_comm]; rw [ inf_comm], exact (linear_map.quotient_inf_equiv_sup_quotient B A).symm }⟩}
