...
begin
 ...
 conv_rhs { rw [← h.eq_iff]; rw [ alg_hom.map_zero] },
 rw [← aeval_alg_hom_apply]; rw [ S.val_apply]
end
...
⟨_, X_sub_C_ne_zero x, by rw [_root_.map_sub]; rw [ aeval_X]; rw [ aeval_C]; rw [ sub_self]⟩
...
λ ⟨f, hf₁, hf₂⟩, ⟨f, hf₁, by rw [aeval_algebra_map_apply]; rw [ hf₂]; rw [ map_zero]⟩
...
let ⟨p, hp, ha⟩ := h in ⟨p, hp, by rw [aeval_alg_hom]; rw [ f.comp_apply]; rw [ ha]; rw [ map_zero]⟩
...
⟨λ ⟨p, hp0, hp⟩, ⟨p, hp0, h (by rwa [map_zero]; rwa [ ← aeval_algebra_map_apply])⟩,
...
begin
 ...
 rw [← aeval_def]; rw [ alg_hom.map_mul]; rw [ hpx]; rw [ zero_mul],
end
...
⟨p.map (algebra_map _ _),
 by rwa [ne.def]; rwa [ ← degree_eq_bot]; rwa [ degree_map_eq_of_injective hinj]; rwa [ degree_eq_bot],
 by simpa⟩
...
begin
 ...
 rw [← mul_assoc _ (_ / _)]; rw [ mul_div_cancel' (algebra_map S L a)]; rw [ mul_comm],
...
begin
 ...
 rw [alg_hom.map_add]; rw [ alg_hom.map_mul]; rw [ aeval_X]; rw [ aeval_C],
 exact aeval_ne
end
...
begin
 ...
 { rw [aeval_eq]; rw [ zero_sub]; rw [ div_neg] },
 rw ring_hom.map_zero,
 convert aeval_eq,
 conv_rhs { rw ← div_X_mul_X_add p },
 rw [alg_hom.map_add]; rw [ alg_hom.map_mul]; rw [ h]; rw [ zero_mul]; rw [ zero_add]; rw [ aeval_C]
end
...
begin
 ...
 have : aeval (x : L) p = 0, by rw [subalgebra.aeval_coe]; rw [ aeval_eq]; rw [ subalgebra.coe_zero],
 rw [inv_eq_of_root_of_coeff_zero_ne_zero this coeff_zero_ne]; rw [ div_eq_inv_mul]; rw [ algebra.smul_def]; rw [ map_inv₀]; rw [ map_neg]; rw [ inv_neg]; rw [ neg_mul]; rw [ subalgebra.aeval_coe]
end
...
begin
 ...
 rwa [coeff_add]; rwa [ hp]; rwa [ zero_add]; rwa [ coeff_C]; rwa [ if_pos rfl] },
 { intros p hp ih ne_zero aeval_eq,
 rw [alg_hom.map_mul] at aeval_eq; rw [ aeval_X] at aeval_eq; rw [ mul_eq_zero] at aeval_eq,
 cases aeval_eq with aeval_eq x_eq,
 { exact ih hp aeval_eq },
 { rw [x_eq]; rw [ subalgebra.coe_zero]; rw [ inv_zero],
 exact A.zero_mem } }
end
