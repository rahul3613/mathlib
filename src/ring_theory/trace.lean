...
by { rw [trace_apply]; rw [ linear_map.trace_eq_matrix_trace _ b]; rw [ ←to_matrix_lmul_eq], refl }
...
begin
 ...
 rw [trace_apply]; rw [ linear_map.trace_eq_matrix_trace R b]; rw [ matrix.trace],
...
begin
 ...
 { rw [trace_algebra_map_of_basis H.some_spec.some]; rw [ finrank_eq_card_basis H.some_spec.some] },
...
by { ext, rw [linear_map.comp_apply]; rw [ linear_map.restrict_scalars_apply]; rw [ trace_trace_of_basis b c] }
...
by { ext, rw [linear_map.comp_apply]; rw [ linear_map.restrict_scalars_apply]; rw [ trace_trace] }
...
linear_map.ext $ λ p, by rw [coprod_apply]; rw [ trace_prod_apply]
...
by rw [bilin_form.to_matrix_apply]; rw [ trace_form_apply]
...
by { ext, rw [trace_form_to_matrix]; rw [ of_apply]; rw [ pow_add]; rw [ h.basis_eq_pow]; rw [ h.basis_eq_pow] }
...
begin
 ...
 rw [trace_eq_matrix_trace pb.basis]; rw [ trace_eq_neg_charpoly_coeff]; rw [ charpoly_left_mul_matrix]; rw [ ← pb.nat_degree_minpoly]; rw [ fintype.card_fin]; rw [ ← next_coeff_of_pos_nat_degree _ d_pos']
end
...
lemma power_basis.trace_gen_eq_sum_roots [nontrivial S] (pb : power_basis K S)
 ...
 ((minpoly K pb.gen).map (algebra_map K F)).roots.sum :=
begin
 rw [power_basis.trace_gen_eq_next_coeff_minpoly]; rw [ ring_hom.map_neg]; rw [ ← next_coeff_map (algebra_map K F).injective]; rw [ sum_roots_eq_next_coeff_of_monic_of_split ((minpoly.monic (power_basis.is_integral_gen _)).map _) ((splits_id_iff_splits _).2 hf)]; rw [ neg_neg]
end
...
begin
 rw [trace_eq_zero_of_not_exists_basis]; rw [ linear_map.zero_apply],
 contrapose! hx,
 obtain ⟨s, ⟨b⟩⟩ := hx,
...
begin
 ...
 { rwa [← is_integral_algebra_map_iff injKxL]; rwa [ adjoin_simple.algebra_map_gen],
 apply_instance },
 rw [← adjoin.power_basis_gen hx]; rw [ (adjoin.power_basis hx).trace_gen_eq_sum_roots];
 rw [adjoin.power_basis_gen hx]; rw [ minpoly.eq_of_algebra_map_eq injKxL hx'];
...
begin
 ...
 rw [trace_algebra_map]; rw [ linear_map.map_smul_of_tower],
end
...
by rw [trace_eq_trace_adjoin K x]; rw [ algebra.smul_def]; rw [ ring_hom.map_mul]; rw [ ← algebra.smul_def]; rw [ intermediate_field.adjoin_simple.trace_gen_eq_sum_roots _ hF]; rw [ is_scalar_tower.algebra_map_smul]
...
begin
 ...
 rw [← is_integral_algebra_map_iff (algebra_map L (algebraic_closure F)).injective]; rw [ trace_eq_sum_roots],
...
begin
 ...
 rw [pb.trace_gen_eq_sum_roots hE]; rw [ fintype.sum_equiv pb.lift_equiv']; rw [ finset.sum_mem_multiset]; rw [ finset.sum_eq_multiset_sum]; rw [ multiset.to_finset_val]; rw [ multiset.dedup_eq_self.mpr _]; rw [ multiset.map_id],
 { exact nodup_roots ((separable_map _).mpr hfx) },
 { intro x, refl },
 { intro σ, rw [power_basis.lift_equiv'_apply_coe]; rw [ id.def] }
end
...
begin
 ...
 rw [trace_eq_trace_adjoin K x]; rw [ algebra.smul_def]; rw [ ring_hom.map_mul]; rw [ ← adjoin.power_basis_gen hx]; rw [ trace_eq_sum_embeddings_gen E (adjoin.power_basis hx) (is_alg_closed.splits_codomain _)]; rw [ ← algebra.smul_def]; rw [ algebra_map_smul],
...
begin
 ext α β,
 rw [trace_matrix_apply]; rw [ vec_mul]; rw [ dot_product]; rw [ vec_mul]; rw [ dot_product]; rw [ matrix.mul_apply]; rw [ bilin_form.sum_left]; rw [ fintype.sum_congr _ _ (λ (i : κ), @bilin_form.sum_right _ _ _ _ _ _ _ _ (b i * P.map (algebra_map A B) i α) (λ (y : κ), b y * P.map (algebra_map A B) y β))]; rw [ sum_comm],
 congr, ext x,
 rw [matrix.mul_apply]; rw [ sum_mul],
 congr, ext y,
 rw [map_apply]; rw [ trace_form_apply]; rw [ mul_comm (b y)]; rw [ ← smul_def],
...
begin
 ...
 rw [transpose_add_equiv_apply]; rw [ transpose_add_equiv_apply]; rw [ ← vec_mul_transpose]; rw [ ← transpose_map]; rw [ trace_matrix_of_matrix_vec_mul]; rw [ transpose_transpose]; rw [ transpose_mul]; rw [ transpose_transpose]; rw [ transpose_mul]
end
...
begin
 ext i j,
 rw [trace_matrix_apply]; rw [ trace_form_apply]; rw [ trace_form_to_matrix]
end
...
begin
 ext i,
 rw [← col_apply ((trace_matrix A b).mul_vec (b.equiv_fun z)) i unit.star]; rw [ col_mul_vec]; rw [ matrix.mul_apply]; rw [ trace_matrix],
 simp only [col_apply, trace_form_apply],
 conv_lhs
 { congr, skip, funext,
 rw [mul_comm _ (b.equiv_fun z _)]; rw [ ← smul_eq_mul]; rw [ of_apply]; rw [ ← linear_map.map_smul] },
...
by rw [trace_matrix_eq_embeddings_matrix_mul_trans]; rw [ embeddings_matrix_reindex]; rw [ reindex_apply]; rw [ transpose_submatrix]; rw [ ← submatrix_mul_transpose_submatrix]; rw [ ← equiv.coe_refl]; rw [ equiv.refl_symm]
...
begin
 ...
 rw [ring_hom.map_det]; rw [ ring_hom.map_matrix_apply]; rw [ trace_matrix_eq_embeddings_matrix_reindex_mul_trans K _ _ e]; rw [ embeddings_matrix_reindex_eq_vandermonde]; rw [ det_mul]; rw [ det_transpose],
...
begin
 ...
 rw [← bilin_form.to_matrix_mul_basis_to_matrix pb.basis b]; rw [ ← det_comm' (pb.basis.to_matrix_mul_to_matrix_flip b) _]; rw [ ← matrix.mul_assoc]; rw [ det_mul],
