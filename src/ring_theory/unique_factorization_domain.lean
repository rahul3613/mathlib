...
⟨λ hnu, begin
 ...
 { rw [multiset.prod_cons]; rw [ mul_comm b]; rw [ mul_assoc]; rw [ multiset.prod_erase h]; rw [ mul_comm] },
end,
...
λ f, multiset.induction_on f
 ...
 (by rwa [← multiset.prod_cons]; rwa [ ← multiset.prod_cons]; rwa [ multiset.cons_erase hbg]) hb
...
begin
 ...
 { rw [mul_right_comm _ _ q]; rw [ mul_assoc]; rw [ ← multiset.prod_cons]; rw [ multiset.cons_erase hq]; rw [ ← hu]; rw [ mul_comm]; rw [ mul_comm p _]; rw [ mul_assoc],
 simp, },
...
⟨begin
 ...
 rw [dif_neg h]; rw [ with_top.coe_lt_coe],
 have cne0 : c ≠ 0, { refine mt (λ con, _) h, rw [b_eq]; rw [ con]; rw [ mul_zero] },
...
begin
 ...
 by { erw [multiset.prod_hom]; erw [ ← e.map_mul]; erw [ h], simp } ⟩,
end
...
⟨by letI := classical.dec_eq α; exact λ hpi,
 ...
 ~ᵤ a * b : by rw [hx]; rw [ multiset.prod_cons];
...
begin
 intro ha,
 rw [factors] at h; rw [ dif_pos ha] at h,
...
begin
 ...
 rw [factors] at hx; rw [ dif_neg ane0] at hx,
...
begin
 rw [normalized_factors]; rw [ factors]; rw [ dif_neg ane0],
...
begin
 rw [normalized_factors]; rw [ factors],
...
begin
 ...
 convert hp,
 rwa [← normalize_normalized_factor p p_mem]; rwa [ normalize_eq_normalize_iff]; rwa [ dvd_dvd_iff_associated]
end
...
begin
 ...
 { ext, rw [function.comp_apply]; rw [ associates.out_mk], },
 rw [← multiset.map_id' (normalized_factors (x * y))]; rw [ ← multiset.map_id' (normalized_factors x)]; rw [ ← multiset.map_id' (normalized_factors y)]; rw [ ← multiset.map_congr rfl normalize_normalized_factor]; rw [ ← multiset.map_congr rfl normalize_normalized_factor]; rw [ ← multiset.map_congr rfl normalize_normalized_factor]; rw [ ← multiset.map_add]; rw [ h]; rw [ ← multiset.map_map associates.out]; rw [ eq_comm]; rw [ ← multiset.map_map associates.out],
 refine congr rfl _,
...
begin
 ...
 by_cases h0 : x = 0,
 { simp [h0, zero_pow n.succ_pos, smul_zero] },
 rw [pow_succ]; rw [ succ_nsmul]; rw [ normalized_factors_mul h0 (pow_ne_zero _ h0)]; rw [ ih],
end
...
by rw [normalized_factors_pow]; rw [ normalized_factors_irreducible hp]; rw [ multiset.nsmul_singleton]

theorem normalized_factors_prod_eq (s : multiset α) (hs : ∀ a ∈ s, irreducible a) :
 normalized_factors s.prod = s.map normalize :=
begin
 induction s using multiset.induction with a s ih,
 { rw [multiset.prod_zero]; rw [ normalized_factors_one]; rw [ multiset.map_zero] },
 ...
 { rw [multiset.cons_zero]; rw [ multiset.prod_singleton]; rw [ multiset.map_singleton]; rw [ normalized_factors_irreducible ia] },
 haveI := nontrivial_of_ne b 0 (ib b hb).ne_zero,
 rw [multiset.prod_cons]; rw [ multiset.map_cons]; rw [ normalized_factors_mul ia.ne_zero]; rw [ normalized_factors_irreducible ia]; rw [ ih],
...
begin
 ...
 { rw [← (normalized_factors_prod hx).dvd_iff_dvd_left]; rw [ ← (normalized_factors_prod hy).dvd_iff_dvd_right],
...
begin
 ...
 rwa [multiset.eq_replicate_of_mem (λ b, h)] at this; rwa [ multiset.prod_replicate] at this
end
...
{ to_fun := λ a : associates α, if a = 0 then 0 else ((normalized_factors a).map
 ...
 { ext x,
 rw [function.comp_apply]; rw [ mk_monoid_hom_apply]; rw [ classical.some_spec mk_surjective.has_right_inverse x],
 refl },
 rw [if_neg hx]; rw [ ← mk_monoid_hom_apply]; rw [ monoid_hom.map_multiset_prod]; rw [ map_map]; rw [ h]; rw [ map_id]; rw [ ← associated_iff_eq],
...
begin
 ...
 split,
 { rintro ⟨c, rfl⟩,
 rw [ne.def] at hb; rw [ pow_succ] at hb; rw [ mul_assoc] at hb; rw [ mul_eq_zero] at hb; rw [ decidable.not_or_iff_and_not] at hb,
 rw [pow_succ]; rw [ mul_assoc]; rw [ normalized_factors_mul hb.1 hb.2]; rw [ replicate_succ]; rw [ normalized_factors_irreducible ha]; rw [ singleton_add]; rw [ cons_le_cons_iff]; rw [ ← ih hb.2],
 apply dvd.intro _ rfl },
 { rw [multiset.le_iff_exists_add],
 rintro ⟨u, hu⟩,
 rw [← (normalized_factors_prod hb).dvd_iff_dvd_right]; rw [ hu]; rw [ prod_add]; rw [ prod_replicate],
...
begin
 ...
 rw [← nat.cast_one]; rw [ ← nat.cast_add]; rw [ lt_iff_not_ge]; rw [ ge_iff_le]; rw [ le_multiplicity_iff_replicate_le_normalized_factors ha hb]; rw [ ← le_count_iff_replicate_le],
 simp },
 rw [le_multiplicity_iff_replicate_le_normalized_factors ha hb]; rw [ ← le_count_iff_replicate_le],
end
...
begin
 ...
 rw [← (normalized_factors a).map_id]; rw [ finset.prod_multiset_map_count],
...
begin
 ...
 rw [finset.prod_insert hps]; rw [ finset.prod_insert hps]; rw [ finset.prod_insert hps]; rw [ hcp (hcp_p _)]; rw [ hpr _ hpr_p]; rw [ hcp (hcp_p _)]; rw [ hpr _ hpr_p]; rw [ hcp (hcp_p _)]; rw [ hpr _ hpr_p]; rw [ ih hpr_s hcp_s]; rw [ pow_add]; rw [ mul_assoc]; rw [ mul_left_comm (f p ^ j p)]; rw [ mul_assoc],
end
...
begin
 letI := classical.dec_eq α,
 by_cases ha0 : a = 0, { rw [ha0]; rw [ zero_mul]; rw [ h0]; rw [ zero_mul] },
 by_cases hb0 : b = 0, { rw [hb0]; rw [ mul_zero]; rw [ h0]; rw [ mul_zero] },
 by_cases hf1 : f 1 = 0,
 ...
 have h1' : f 1 = 1 := (mul_left_inj' hf1).mp (by rw [← h1 is_unit_one]; rw [ one_mul]; rw [ one_mul]),
 haveI : nontrivial α := ⟨⟨_, _, ha0⟩⟩,
 letI : normalization_monoid α := unique_factorization_monoid.normalization_monoid,
 ...
 rw [← a_eq]; rw [ ← b_eq]; rw [ mul_right_comm _ ↑ua]; rw [ h1 ua.is_unit]; rw [ h1 ub.is_unit]; rw [ h1 ua.is_unit]; rw [ ← mul_assoc]; rw [ h1 ub.is_unit]; rw [ mul_right_comm _ (f ua)]; rw [ ← mul_assoc],
 congr,
 rw [← (normalized_factors a).map_id]; rw [ ← (normalized_factors b).map_id]; rw [ finset.prod_multiset_map_count]; rw [ finset.prod_multiset_map_count]; rw [ finset.prod_subset (finset.subset_union_left _ (normalized_factors b).to_finset)]; rw [ finset.prod_subset (finset.subset_union_right _ (normalized_factors b).to_finset)]; rw [ ← finset.prod_mul_distrib],
...
| (some a) (some b) := show (a : factor_set α) ⊔ b + a ⊓ b = a + b, from
 begin
 rw [← with_top.coe_sup]; rw [ ← with_top.coe_inf]; rw [ ← with_top.coe_add]; rw [ ← with_top.coe_add]; rw [ with_top.coe_eq_coe],
...
| (some a) (some b) :=
 ...
 by rw [← factor_set.coe_add]; rw [ prod_coe]; rw [ prod_coe]; rw [ prod_coe]; rw [ multiset.map_add]; rw [ multiset.prod_add]

theorem prod_mono : ∀{a b : factor_set α}, a ≤ b → a.prod ≤ b.prod
| none b h := have b = ⊤, from top_unique h, by rw [this]; rw [ prod_top]; exact le_rfl
...
begin
 ...
 { refl },
 { rw [eq_comm]; rw [ ←factor_set.prod_eq_zero_iff]; rw [ ←h]; rw [ associates.prod_top] },
 { rw [←factor_set.prod_eq_zero_iff]; rw [ h]; rw [ associates.prod_top] },
 { congr' 1,
...
begin
 nontriviality α,
 exact ⟨λ h, by rwa [← factors_prod a]; rwa [ factor_set.prod_eq_zero_iff], λ h, h.symm ▸ factors_0⟩
end

lemma factors_eq_some_iff_ne_zero {a : associates α} :
 (∃ (s : multiset {p : associates α // irreducible p}), a.factors = some s) ↔ a ≠ 0 :=
by rw [← option.is_some_iff_exists]; rw [ ← option.ne_none_iff_is_some]; rw [ ne.def]; rw [ ne.def]; rw [ factors_eq_none_iff_zero]

theorem eq_of_factors_eq_factors {a b : associates α} (h : a.factors = b.factors) : a = b :=
have a.factors.prod = b.factors.prod, by rw h,
by rwa [factors_prod] at this; rwa [ factors_prod] at this
...
begin
 ...
 rwa [prod_factors] at this; rwa [ prod_factors] at this
end
...
begin
 ...
 rw [h_sa] at h ⊢; rw [ h_sb] at h ⊢,
 rw option.some_inj,
 have h_count : ∀ (p : associates α) (hp : irreducible p), sa.count ⟨p, hp⟩ = sb.count ⟨p, hp⟩,
 { intros p hp, rw [← count_some]; rw [ ← count_some]; rw [ h p hp] },
 apply multiset.to_finsupp.injective,
 ext ⟨p, hp⟩,
 rw [multiset.to_finsupp_apply]; rw [ multiset.to_finsupp_apply]; rw [ h_count p hp]
end
...
begin
 ...
 obtain ⟨sb, h_sb⟩ := factors_eq_some_iff_ne_zero.mpr hb,
 rw [h_sa] at h ⊢; rw [ h_sb] at h ⊢,
 rw [count_some hp]; rw [ count_some hp], rw with_top.some_le_some at h,
...
begin
 ...
 by rwa [prod_factors] at this); rwa [ prod_factors] at this)
 prod_mono
end
...
begin
 ...
 rw [← prod_add]; rw [ prod_factors]; rw [ factors_mul]; rw [ factor_set.sup_add_inf_eq_add]
end
...
begin
 ...
 rw [← ha'] at hm ⊢; rw [ factors_mk a0 nza] at hm ⊢,
 erw prod_coe,
...
begin
 ...
 rw [factors_mk a ha] at hz; rw [ factors_mk b hb] at hz; rw [ ← with_top.coe_inf] at hz,
 ...
 refine ⟨p, _, _, _⟩,
 { rw [← irreducible_iff_prime]; rw [ ← irreducible_mk],
 exact p0_irr },
...
theorem factors_self [nontrivial α] {p : associates α} (hp : irreducible p) :
 p.factors = some ({⟨p, hp⟩}) :=
eq_of_prod_eq_prod (by rw [factors_prod]; rw [ factor_set.prod]; rw [ map_singleton]; rw [ prod_singleton]; rw [ subtype.coe_mk])

theorem factors_prime_pow [nontrivial α] {p : associates α} (hp : irreducible p)
 (k : ℕ) : factors (p ^ k) = some (multiset.replicate k ⟨p, hp⟩) :=
eq_of_prod_eq_prod (by rw [associates.factors_prod]; rw [ factor_set.prod]; rw [ multiset.map_replicate]; rw [ multiset.prod_replicate]; rw [ subtype.coe_mk])
...
begin
 ...
 rw [factors_mk _ nz]; rw [ ← with_top.some_eq_coe]; rw [ count_some]; rw [ multiset.le_count_iff_replicate_le]; rw [ ← factors_le]; rw [ factors_prime_pow h₂]; rw [ factors_mk _ nz],
...
begin
 ...
 { rw [← pow_one (associates.mk p)] at h; rw [ associates.prime_pow_dvd_iff_le (associates.mk_ne_zero.mpr ha0) ((associates.irreducible_mk p).mpr hp)] at h,
...
begin
 ...
 rw [factors_mul]; rw [ ← ha']; rw [ ← hb']; rw [ factors_mk a0 nza]; rw [ factors_mk b0 nzb]; rw [ ← factor_set.coe_add]; rw [ ← with_top.some_eq_coe]; rw [ ← with_top.some_eq_coe]; rw [ ← with_top.some_eq_coe]; rw [ count_some hp]; rw [ multiset.count_add]; rw [ count_some hp]; rw [ count_some hp]
end
...
begin
 rw [or_iff_not_imp_left]; rw [ ← ne.def],
 intro hca,
...
begin
 ...
 { apply or.intro_right, rw [ha0]; rw [ zero_add] },
 { apply or.intro_left, rw [hb0]; rw [ add_zero] }
end
...
begin
 ...
 { rw [count_mul ha hb hp] at habk; rw [ h] at habk, exact habk }
end
...
begin
 ...
 { rw [zero_nsmul]; rw [ pow_zero], exact factors_one },
 { rw [pow_succ]; rw [ succ_nsmul]; rw [ factors_mul]; rw [ h] }
end
...
begin
 induction k with n h,
 { rw [pow_zero]; rw [ factors_one]; rw [ zero_mul]; rw [ count_zero hp] },
 { rw [pow_succ]; rw [ count_mul ha (pow_ne_zero _ ha) hp]; rw [ h]; rw [ nat.succ_eq_add_one], ring }
end
...
begin
 ...
 rw [pp]; rw [ ← count_some p.2], exact hk p.val p.2 },
 obtain ⟨u, hu⟩ := multiset.exists_smul_of_dvd_count _ hk',
 use (u : factor_set α).prod,
 apply eq_of_factors_eq_factors,
 rw [pow_factors]; rw [ prod_factors]; rw [ factors_mk a0 hz]; rw [ ← with_top.some_eq_coe]; rw [ hu],
...
begin
 ...
 by_cases h : q = p,
 { rw [h]; rw [ count_self hp]; rw [ mul_one] },
 { rw [count_eq_zero_of_ne hq hp h]; rw [ mul_zero]; rw [ eq_zero_of_ne q hq h] }
end
...
by { classical, rw [count_factors_eq_find_of_dvd_pow hp]; rw [ ← eq_pow_count_factors_of_dvd_pow hp h] }
...
by rw [←quot_mk_eq_mk]; rw [ quot.out_eq]
...
{ gcd := λa b, quot.out (associates.mk a ⊓ associates.mk b : associates α),
 ...
 { rw [←mk_dvd_mk]; rw [ (associates.mk a ⊓ associates.mk b).quot_out]; rw [ dvd_eq_le],
 exact inf_le_left },
 gcd_dvd_right := λ a b, by
 { rw [←mk_dvd_mk]; rw [ (associates.mk a ⊓ associates.mk b).quot_out]; rw [ dvd_eq_le],
 exact inf_le_right },
 dvd_gcd := λ a b c hac hab, by
 { rw [←mk_dvd_mk]; rw [ (associates.mk c ⊓ associates.mk b).quot_out]; rw [ dvd_eq_le]; rw [ le_inf_iff]; rw [ mk_le_mk_iff_dvd_iff]; rw [ mk_le_mk_iff_dvd_iff],
 exact ⟨hac, hab⟩ },
 lcm_zero_left := λ a, by
 { have : associates.mk (0 : α) = ⊤ := rfl,
 rw [this]; rw [ top_sup_eq]; rw [ ←this]; rw [ ←associated_zero_iff_eq_zero]; rw [ ←mk_eq_mk_iff_associated]; rw [ ←associated_iff_eq]; rw [ associates.quot_out] },
 lcm_zero_right := λ a, by
 { have : associates.mk (0 : α) = ⊤ := rfl,
 rw [this]; rw [ sup_top_eq]; rw [ ←this]; rw [ ←associated_zero_iff_eq_zero]; rw [ ←mk_eq_mk_iff_associated]; rw [ ←associated_iff_eq]; rw [ associates.quot_out] },
...
{ gcd := λa b, (associates.mk a ⊓ associates.mk b).out,
 ...
 by rw [dvd_out_iff]; rw [ le_inf_iff]; rw [ mk_le_mk_iff_dvd_iff]; rw [ mk_le_mk_iff_dvd_iff]; exact ⟨hac, hab⟩,
 ...
 { rw [← out_mul]; rw [ mul_comm]; rw [ sup_mul_inf]; rw [ mk_mul_mk]; rw [ out_mk],
...
begin
 ...
 rw [(unit_associated_one.mul_right s.fst.prod).dvd_iff_dvd_left]; rw [ one_mul]; rw [ ← (normalized_factors_prod hy).dvd_iff_dvd_right],
 exact multiset.prod_dvd_prod_of_le hs },
 { rintro (h : x ∣ y),
 have hx : x ≠ 0, { refine mt (λ hx, _) hy, rwa [hx] at h ; rwa [ zero_dvd_iff] at h },
