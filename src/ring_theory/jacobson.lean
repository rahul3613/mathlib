...
begin
 ...
 rw [← hI.radical]; rw [ radical_eq_Inf I]; rw [ mem_Inf],
 intros P hP,
 rw set.mem_set_of_eq at hP,
 erw mem_Inf at hx,
 erw [← h P hP.right]; erw [ mem_Inf],
...
(λ h, by rw [h]; rw [ jacobson_eq_top_iff])⟩
...
begin
 ...
 rw [eq_bot_iff]; rw [ ← jacobson_eq_iff_jacobson_quotient_eq_bot.1 ((is_jacobson_iff_prime_eq.1 hR) (comap (algebra_map R S) P) (comap_is_prime _ _))]; rw [ comap_jacobson],
...
begin
 ...
 erw [← H.out hJ.left.is_radical]; erw [ mem_Inf] at this,
 push_neg at this,
 rcases this with ⟨I, hI, hI'⟩,
 convert hI.right,
 by_cases hJ : J = map (algebra_map R S) I,
 { rw [hJ]; rw [ comap_map_of_is_prime_disjoint (powers y) S I (is_maximal.is_prime hI.right)],
 ...
 { rwa [eq_top_iff] at hJ ; rwa [ ← (is_localization.order_embedding (powers y) S).le_iff_le] at hJ },
 { have := congr_arg (map (algebra_map R S)) (h.1.1.2 _ ⟨comap_mono (le_of_lt hI), _⟩),
 rwa [map_comap (powers y) S I] at this; rwa [ map_top] at this,
...
begin
 rw [is_maximal_iff_is_maximal_disjoint S y]; rw [ comap_map_of_is_prime_disjoint (powers y) S I (is_maximal.is_prime hI) ((disjoint_powers_iff_not_mem y hI.is_prime.is_radical).2 hy)],
 exact ⟨hI, hy⟩
end
...
begin
 ...
 rw [ideal.jacobson]; rw [ comap_Inf']; rw [ Inf_eq_infi],
...
begin
 ...
 rw [← φ'.comp_apply]; rw [ is_localization.map_comp]; rw [ ring_hom.comp_apply]; rw [ subtype.coe_mk] },
 ...
 rwa [eval₂_map]; rwa [ hφ']; rwa [ ← hom_eval₂]; rwa [ quotient.eq_zero_iff_mem]; rwa [ eval₂_C_X] },
 { rw [set.mem_set_of_eq] at hy; rw [ degree_le_zero_iff] at hy,
 refine hy.symm ▸ ⟨X - C (algebra_map _ _ ((quotient.mk P') (p.coeff 0))), monic_X_sub_C _, _⟩,
 simp only [eval₂_sub, eval₂_C, eval₂_X],
 rw [sub_eq_zero]; rw [ ← φ'.comp_apply]; rw [ is_localization.map_comp],
 refl } },
 ...
 rw [ring_hom.map_add]; rw [ ring_hom.map_add] },
 { rw [pow_succ X n]; rw [ mul_comm X]; rw [ ← mul_assoc]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_mul],
...
begin
 ...
 { rw [← ring_hom.ker_eq_comap_bot]; rw [ ← ring_hom.injective_iff_ker_eq_bot],
 ...
 rw [← hSₘ.out is_radical_bot_of_no_zero_divisors]; rw [ comap_jacobson],
 ...
 { rwa [comap_comap] at this ; rwa [ hcomm] at this ; rwa [ ← comap_comap] at this },
 ...
 (by rwa [comap_comap]; rwa [ hcomm]; rwa [ ← bot_quotient_is_maximal_iff])))
...
begin
 ...
 have hx : x ≠ 0 := by rwa [ne.def]; rwa [ leading_coeff_eq_zero],
...
begin
 ...
 rwa [mem_comap]; rwa [ ← quotient.eq_zero_iff_mem]; rwa [ ← ring_hom.comp_apply], },
 ...
 rw [← map_jacobson_of_surjective _ hi'] at this; rw [ comap_map_of_surjective _ _] at this; rw [ comap_map_of_surjective _ _] at this,
...
begin
 ...
 have : (m : R[X]) ≠ 0, rwa [ne.def]; rwa [ submodule.coe_eq_zero],
 ...
 by rwa [quotient.eq_zero_iff_mem] at hx)); rwa [ (by rwa eq_bot_iff : (P.comap C : ideal R) = ⊥)] at hx))
 ...
 rwa [← quotient.maximal_ideal_iff_is_field_quotient]; rwa [ ← bot_quotient_is_maximal_iff],
end
...
begin
 ...
 rwa [quotient.eq_zero_iff_mem]; rwa [ mem_comap]; rwa [ hPJ]; rwa [ mem_comap]; rwa [ coe_map_ring_hom]; rwa [ map_C] }
end

lemma is_maximal_comap_C_of_is_jacobson :
 (P.comap (C : R →+* R[X])).is_maximal :=
begin
 rw [← @mk_ker _ _ P]; rw [ ring_hom.ker_eq_comap_bot]; rw [ comap_comap],
...
begin
 ...
 { rw [← fin_succ_equiv_comp_C_eq_C]; rw [ ← ring_hom.comp_assoc]; rw [ ← ring_hom.comp_assoc]; rw [ ← quotient_map_comp_mk le_rfl]; rw [ ring_hom.comp_assoc (polynomial.C)]; rw [ ← quotient_map_comp_mk le_rfl]; rw [ ring_hom.comp_assoc]; rw [ ring_hom.comp_assoc]; rw [ ← quotient_map_comp_mk le_rfl]; rw [ ← ring_hom.comp_assoc (quotient.mk _)],
