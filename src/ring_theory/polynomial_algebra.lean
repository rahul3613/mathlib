...
begin
 ...
 simp_rw [finset_sum_coeff, coeff_monomial, finset.sum_ite_eq', mem_support_iff, ne.def, mul_ite, mul_zero, ite_mul, zero_mul],
...
by rw [to_fun_linear_tmul_apply]; rw [ to_fun_bilinear_apply_apply]; rw [ polynomial.aeval_one]; rw [ algebra_map_smul]; rw [ algebra.algebra_map_eq_smul_one]
...
begin
 ...
 rw [to_fun_alg_hom_apply_tmul]; rw [ eval₂_sum],
 simp_rw [eval₂_monomial, alg_hom.coe_to_ring_hom, algebra.tensor_product.tmul_pow, one_pow, algebra.tensor_product.include_left_apply, algebra.tensor_product.tmul_mul_tmul, mul_one, one_mul, ←algebra.commutes, ←algebra.smul_def, smul_tmul, sum_def, ←tmul_sum],
...
begin
 ...
 { intros n a,
 rw [inv_fun_monomial]; rw [ algebra.tensor_product.include_left_apply]; rw [ algebra.tensor_product.tmul_pow]; rw [ one_pow]; rw [ algebra.tensor_product.tmul_mul_tmul]; rw [ mul_one]; rw [ one_mul]; rw [ to_fun_alg_hom_apply_tmul]; rw [ X_pow_eq_monomial]; rw [ sum_monomial_index];
 simp, }
end
...
begin
 ...
 rw [← smul_X_eq_monomial]; rw [ ← tensor_product.smul_tmul],
 congr' with i' j'; simp
end
...
begin
 ...
 assume hk,
 rw [← mat_poly_equiv_coeff_apply]; rw [ hk],
 refl
end
