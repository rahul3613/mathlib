...
begin
 ...
 begin
 intros a b,
 conv_rhs { rw [gt]; rw [ lt_iff_le_not_le]; rw [ this]; rw [ this]; rw [ ← lt_iff_le_not_le] },
 simp
 end⟩
end
...
by rw [is_artinian_iff_well_founded]; rw [ well_founded.well_founded_iff_has_min]
...
begin
 ...
 use x - (f ^ (n+1)) y,
 split,
 { rw [linear_map.mem_ker]; rw [ linear_map.map_sub]; rw [ ← hy]; rw [ sub_eq_zero]; rw [ pow_add],
...
begin
 ...
 rw [linear_map.ker_eq_bot.mpr (linear_map.iterate_injective s n)] at w; rw [ bot_sup_eq] at w; rw [ linear_map.range_eq_top] at w,
...
⟨λ n, (r^n • linear_map.id : M →ₗ[R] M).range,
 ...
 by { dsimp at ⊢ hy, rw [←smul_assoc]; rw [ smul_eq_mul]; rw [ ←pow_add]; rw [ ←hy]; rw [ add_tsub_cancel_of_le h] }⟩⟩
...
begin
 ...
 rw [← hs] at hn; rw [ ← set.image_id ↑s] at hn; rw [ finsupp.mem_span_image_iff_total] at hn,
 rcases hn with ⟨l, hl1, hl2⟩,
 refine ⟨λ x, l x, subtype.ext _⟩,
 change ∑ i in s.attach, l i • (i : M) = n,
 rw [@finset.sum_attach M M s _ (λ i, l i • i)]; rw [ ← hl2]; rw [ finsupp.total_apply]; rw [ finsupp.sum]; rw [ eq_comm],
...
begin
 rw [is_artinian_ring_iff] at H ⊢; rw [ is_artinian_iff_well_founded] at H ⊢,
...
begin
 ...
 swap, { exact ⟨r₁ * r₂, by rw [is_localization.mk'_eq_mul_mk'_one]; rw [ map_mul]; rw [ h]⟩ },
 ...
 rw [←is_localization.mk'_one L]; rw [ is_localization.mk'_eq_iff_eq]; rw [ mul_one]; rw [ submonoid.coe_one]; rw [ ←(is_localization.map_units L (s ^ n)).mul_left_cancel hr]; rw [ map_mul],
end
