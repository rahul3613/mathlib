...
by rw [finite_dimensional.finrank_eq_card_basis pb.basis]; rw [ fintype.card_fin]
...
begin
 ...
 rw [← polynomial.aeval_alg_hom_apply]; rw [ ← polynomial.aeval_alg_hom_apply]; rw [ h]
end
...
begin
 ...
 rw [finsupp.total_apply]; rw [ finsupp.sum_fintype];
...
lemma dim_le_degree_of_root (h : power_basis A S) {p : A[X]}
 ...
 ↑h.dim ≤ p.degree :=
by { rw [degree_eq_nat_degree ne_zero]; rw [ with_bot.coe_le_coe],
...
begin
 ...
 rw [algebra.left_mul_matrix_apply]; rw [ ← linear_equiv.eq_symm_apply]; rw [ linear_map.to_matrix_symm],
 ...
 rw [add_comm]; rw [ aeval_eq_sum_range]; rw [ finset.sum_range_succ]; rw [ ← leading_coeff]; rw [ pb.minpoly_gen_monic.leading_coeff]; rw [ one_smul]; rw [ nat_degree_minpoly_gen]; rw [ finset.sum_range] },
 { rw [fintype.sum_eq_single (⟨↑k + 1, lt_of_le_of_ne k.2 h⟩ : fin pb.dim)]; rw [ if_pos]; rw [ one_smul],
 { refl }, { refl }, intros x hx, rw [if_neg]; rw [ zero_smul], apply mt fin.ext hx },
end
...
begin
 ...
 { rw [(subsingleton.elim _ _ : f = 0)]; rw [ aeval_zero]; rw [ map_zero]; rw [ aeval_zero] },
 rw [← aeval_mod_by_monic_eq_self_of_root (minpoly.monic pb.is_integral_gen) (minpoly.aeval _ _)]; rw [ ← @aeval_mod_by_monic_eq_self_of_root _ _ _ _ _ f _ (minpoly.monic pb.is_integral_gen) y hy],
 ...
 rw [aeval_eq_sum_range' this]; rw [ aeval_eq_sum_range' this]; rw [ linear_map.map_sum],
 refine finset.sum_congr rfl (λ i (hi : i ∈ finset.range pb.dim), _),
 rw finset.mem_range at hi,
 rw linear_map.map_smul,
 congr,
 rw [← fin.coe_mk hi]; rw [ ← pb.basis_eq_pow ⟨i, hi⟩]; rw [ basis.constr_basis]
end
...
{ to_fun := λ f, ⟨f pb.gen, by rw [aeval_alg_hom_apply]; rw [ minpoly.aeval]; rw [ f.map_zero]⟩,
...
pb.lift_equiv.trans ((equiv.refl _).subtype_equiv (λ x,
 begin
 rw [mem_roots]; rw [ is_root.def]; rw [ equiv.refl_apply]; rw [ ← eval₂_eq_eval_map]; rw [ ← aeval_def],
...
begin
 ...
 { rw [minpoly.eq_zero h]; rw [ nat_degree_zero], exact linear_independent_empty_type },
...
{ dim := pb.dim,
 ...
 gen := e pb.gen,
 basis_eq_pow :=
 λ i, by rw [basis.map_apply]; rw [ pb.basis_eq_pow]; rw [ e.to_linear_equiv_apply]; rw [ e.map_pow] }
...
begin
 rw [← to_submodule_eq_top]; rw [ _root_.eq_top_iff]; rw [ ← B.basis.span_eq]; rw [ submodule.span_le],
 rintros x ⟨i, rfl⟩,
 rw [B.basis_eq_pow i],
