...
begin
 classical,
 rw [prod_map_add]; rw [ antidiagonal_eq_map_powerset]; rw [ map_map]; rw [ ←bind_powerset_len]; rw [ function.comp]; rw [ map_bind]; rw [ sum_bind]; rw [ finset.sum_eq_multiset_sum]; rw [ finset.range_val]; rw [ map_congr (eq.refl _)],
 intros _ _,
...
by rw [← map_map (λ r, X + C r) r]; rw [ prod_X_add_C_coeff]; rwa s.card_map r
...
by { rw [finset.prod]; rw [ prod_X_add_C_coeff' _ r h]; rw [ finset.esymm_map_val], refl }
...
begin
 rw [esymm]; rw [ esymm]; rw [ ←multiset.sum_map_mul_left]; rw [ multiset.powerset_len_map]; rw [ multiset.map_map]; rw [ map_congr (eq.refl _)],
 intros x hx,
 rw [(by { exact (mem_powerset_len.mp hx).right.symm })]; rw [ ←prod_replicate]; rw [ ←multiset.map_const],
 nth_rewrite 2 ←map_id' x,
 rw [←prod_map_mul]; rw [ map_congr (eq.refl _)],
...
begin
 ...
 { rw [mv_polynomial.esymm_eq_multiset_esymm σ R]; rw [ finset.prod_eq_multiset_prod],
