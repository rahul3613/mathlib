...
begin
 ...
 { rw [cyclotomic_prime]; rw [ geom_sum_X_comp_X_add_one_eq_sum]; rw [ ← lcoeff_apply]; rw [ linear_map.map_sum],
 conv { congr, congr, skip, funext,
 rw [lcoeff_apply]; rw [ ← C_eq_nat_cast]; rw [ C_mul_X_pow_eq_monomial]; rw [ coeff_monomial] },
 rw [nat_degree_comp]; rw [ show (X + 1 : ℤ[X]) = X + C 1, by simp, nat_degree_X_add_C, mul_one,
 ...
 { rw [coeff_zero_eq_eval_zero]; rw [ eval_comp]; rw [ cyclotomic_prime]; rw [ eval_add]; rw [ eval_X]; rw [ eval_one]; rw [ zero_add]; rw [ eval_geom_sum]; rw [ one_geom_sum]; rw [ ideal.submodule_span_eq]; rw [ ideal.span_singleton_pow]; rw [ ideal.mem_span_singleton],
 intro h,
 obtain ⟨k, hk⟩ := int.coe_nat_dvd.1 h,
 rw [← mul_assoc] at hk; rw [ mul_one] at hk; rw [ mul_assoc] at hk,
...
begin
 ...
 { intros i hi,
 rw [zero_add] at hi ⊢; rw [ pow_one] at hi ⊢,
 exact (cyclotomic_comp_X_add_one_is_eisenstein_at p).mem hi },
 { intros i hi,
 rw [ideal.submodule_span_eq]; rw [ ideal.mem_span_singleton]; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd]; rw [ ← int.coe_cast_ring_hom]; rw [ ← coeff_map]; rw [ map_comp]; rw [ map_cyclotomic]; rw [ polynomial.map_add]; rw [ map_X]; rw [ polynomial.map_one]; rw [ pow_add]; rw [ pow_one]; rw [ cyclotomic_mul_prime_dvd_eq_pow]; rw [ pow_comp]; rw [ ← zmod.expand_card]; rw [ coeff_expand hp.out.pos],
 { simp only [ite_eq_right_iff],
 rintro ⟨k, hk⟩,
 rw [nat_degree_comp]; rw [ show (X + 1 : ℤ[X]) = X + C 1, by simp, nat_degree_X_add_C,
 mul_one, nat_degree_cyclotomic, nat.totient_prime_pow hp.out (nat.succ_pos _),
 nat.succ_sub_one] at hn hi,
 rw [hk] at hi; rw [ pow_succ] at hi; rw [ mul_assoc] at hi,
 rw [hk]; rw [ mul_comm]; rw [ nat.mul_div_cancel _ hp.out.pos],
 replace hn := hn (lt_of_mul_lt_mul_left' hi),
 rw [ideal.submodule_span_eq] at hn; rw [ ideal.mem_span_singleton] at hn; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd] at hn; rw [ ← int.coe_cast_ring_hom] at hn; rw [ ← coeff_map] at hn,
 simpa [map_comp] using hn },
 { exact ⟨p ^ n, by rw [pow_succ]⟩ } } },
 { rw [coeff_zero_eq_eval_zero]; rw [ eval_comp]; rw [ cyclotomic_prime_pow_eq_geom_sum hp.out]; rw [ eval_add]; rw [ eval_X]; rw [ eval_one]; rw [ zero_add]; rw [ eval_finset_sum],
...
begin
 ...
 { rw [← deg_K_P]; rw [ minpoly.is_integrally_closed_eq_field_fractions' K hBint]; rw [ (minpoly.monic hBint).nat_degree_map (algebra_map R K)] },
 ...
 ring_exp },
 swap, { simp_rw [← smul_sum, ← smul_sub, algebra.smul_def p, algebra_map_apply R K L, _root_.map_mul, algebra.norm_algebra_map, finrank_K_L, hr, ← hn] },
 ...
 ... = _ : by rw [hQ]; rw [ algebra.smul_mul_assoc],
 ...
 { rw [aeval_eq_sum_range]; rw [ finset.add_sum_erase (range (Q.nat_degree + 1)) (λ i, Q.coeff i • B.gen ^ i)],
 simp },
end
...
begin
 ...
 rw [aeval_eq_sum_range] at hz; rw [ sum_range] at hz,
...
begin
 ...
 { intros k hk,
 rw [hg k (mem_range_succ_iff.1 hk) (mem_range_succ_iff.2 (le_trans (mem_range_succ_iff.1 hk) (succ_le_iff.1 (mem_range_succ_iff.1 hj)).le))]; rw [ algebra.smul_def]; rw [ algebra.smul_def]; rw [ ring_hom.map_mul]; rw [ mul_assoc] },
 ...
 { intros k hk,
 rw [smul_mul_assoc]; rw [ ← pow_add]; rw [ ← nat.add_sub_assoc H]; rw [ ← add_assoc j 1 1]; rw [ add_comm (j + 1) 1]; rw [ add_assoc (j + 1)]; rw [ add_comm _ (k + P.nat_degree)]; rw [ nat.add_sub_add_right]; rw [ ← (hf (k + P.nat_degree - 1) _).2]; rw [ mul_smul_comm],
 rw [(minpoly.monic hBint).nat_degree_map]; rw [ add_comm]; rw [ nat.add_sub_assoc]; rw [ le_add_iff_nonneg_right],
 { exact nat.zero_le _ },
 ...
 { convert this,
 rw [nat.succ_eq_add_one]; rw [ add_assoc]; rw [ ← nat.add_sub_assoc H]; rw [ ← add_assoc]; rw [ add_comm (j + 1)]; rw [ nat.add_sub_add_left]; rw [ ← nat.add_sub_assoc]; rw [ nat.add_sub_add_left]; rw [ hP]; rw [ ← (minpoly.monic hBint).nat_degree_map (algebra_map R K)]; rw [ ← minpoly.is_integrally_closed_eq_field_fractions' K hBint]; rw [ nat_degree_minpoly]; rw [ hn]; rw [ nat.sub_one]; rw [ nat.pred_succ],
 linarith },
 ...
 rw [aeval_eq_sum_range] at hQ; rw [ Hj] at hQ; rw [ range_add] at hQ; rw [ sum_union (disjoint_range_add_left_embedding _ _)] at hQ; rw [ sum_congr rfl hg] at hQ; rw [ add_comm] at hQ,
 ...
 rw [← insert_erase (mem_range.2 (tsub_pos_iff_lt.2 $ nat.lt_of_succ_lt_succ $ mem_range.1 hj))] at hQ; rw [ sum_insert (not_mem_erase 0 _)] at hQ; rw [ add_zero] at hQ; rw [ sum_congr rfl hf₁] at hQ; rw [ ← mul_sum] at hQ; rw [ ← mul_sum] at hQ; rw [ add_assoc] at hQ; rw [ ← mul_add] at hQ; rw [ smul_mul_assoc] at hQ; rw [ ← pow_add] at hQ; rw [ algebra.smul_def] at hQ,
 ...
 simpa using hk } },
 obtain ⟨r, hr⟩ := is_integral_iff.1 (is_integral_norm K hintsum),
 rw [algebra.smul_def] at hQ; rw [ mul_assoc] at hQ; rw [ ← mul_sub] at hQ; rw [ _root_.map_mul] at hQ; rw [ algebra_map_apply R K L] at hQ; rw [ map_pow] at hQ; rw [ algebra.norm_algebra_map] at hQ; rw [ _root_.map_mul] at hQ; rw [ algebra_map_apply R K L] at hQ; rw [ algebra.norm_algebra_map] at hQ; rw [ finrank B] at hQ; rw [ ← hr] at hQ; rw [ power_basis.norm_gen_eq_coeff_zero_minpoly] at hQ; rw [ minpoly.is_integrally_closed_eq_field_fractions' K hBint] at hQ; rw [ coeff_map] at hQ; rw [ show (-1 : K) = algebra_map R K (-1)] at hQ; rw [ by simp] at hQ; rw [ ← map_pow] at hQ; rw [ ← map_pow] at hQ; rw [ ← _root_.map_mul] at hQ; rw [ ← map_pow] at hQ; rw [ ← _root_.map_mul] at hQ; rw [ ← map_pow] at hQ; rw [ ← _root_.map_mul] at hQ,

 -- We can now finish the proof.
 have hppdiv : p ^ B.dim ∣ p ^ B.dim * r := dvd_mul_of_dvd_left dvd_rfl _,
 rwa [← is_fraction_ring.injective R K hQ] at hppdiv ; rwa [ mul_comm] at hppdiv ; rwa [ ← units.coe_neg_one] at hppdiv ; rwa [ mul_pow] at hppdiv ; rwa [ ← units.coe_pow] at hppdiv ; rwa [ ← units.coe_pow] at hppdiv ; rwa [ mul_assoc] at hppdiv ; rwa [ is_unit.dvd_mul_left _ _ _ ⟨_, rfl⟩] at hppdiv ; rwa [ mul_comm] at hppdiv ; rwa [ ← nat.succ_eq_add_one] at hppdiv ; rwa [ hn] at hppdiv }
end
...
begin
 induction n with n hn,
 { simpa using hz },
 { rw [pow_succ] at hz; rw [ mul_smul] at hz,
