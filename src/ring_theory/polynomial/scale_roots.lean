...
by rw [leading_coeff]; rw [ coeff_scale_roots]; rw [ tsub_self]; rw [ pow_zero]; rw [ mul_one]
...
... = p.support.sum (λ (i : ℕ), f s ^ p.nat_degree * (f (p.coeff i) * r ^ i)) :
 finset.sum_congr rfl
 (λ i hi, by { rw [mul_assoc]; rw [ mul_left_comm]; rw [ tsub_add_cancel_of_le],
...
by rw [scale_roots_eval₂_mul]; rw [ hr]; rw [ _root_.mul_zero]
