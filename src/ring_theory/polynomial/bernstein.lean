...
begin
 ...
 { rw [← tsub_add_eq_tsub_tsub]; rw [ ← mul_assoc]; rw [ ← mul_assoc], congr' 1,
...
begin
 ...
 rw [this]; rw [ pochhammer_eval_succ],
...
begin
 rw [bernstein_polynomial.iterate_derivative_at_1 _ _ _ h]; rw [ ne.def]; rw [ neg_one_pow_mul_eq_zero_iff]; rw [ ←nat.cast_succ]; rw [ ←pochhammer_eval_cast]; rw [ ←nat.cast_zero]; rw [ nat.cast_inj],
...
begin
 ...
 rw [add_pow]; rw [ (pderiv tt).map_sum]; rw [ (mv_polynomial.aeval e).map_sum]; rw [ finset.sum_mul],
 -- Step inside the sum:
 ...
 { rw [(pderiv tt).leibniz_pow]; rw [ (pderiv tt).map_add]; rw [ pderiv_tt_x]; rw [ pderiv_tt_y],
...
begin
 ...
 rw [add_pow]; rw [ (pderiv tt).map_sum]; rw [ (pderiv tt).map_sum]; rw [ (mv_polynomial.aeval e).map_sum]; rw [ finset.sum_mul],
 -- Step inside the sum:
 refine finset.sum_congr rfl (λ k hk, (w k).trans _),
 simp only [pderiv_tt_x, pderiv_tt_y, algebra.id.smul_eq_mul, nsmul_eq_mul,
