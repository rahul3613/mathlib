...
begin
 rw [←pochhammer_map (algebra_map ℕ S)]; rw [ eval_map]; rw [ ←eq_nat_cast (algebra_map ℕ S)]; rw [ eval₂_at_nat_cast]; rw [ nat.cast_id]; rw [ eq_nat_cast],
end
...
begin
 ...
 { simp, },
 { conv_lhs
 { rw [pochhammer_succ_left]; rw [ ih]; rw [ mul_comp]; rw [ ←mul_assoc]; rw [ ←pochhammer_succ_left]; rw [ add_comp]; rw [ X_comp]; rw [ nat_cast_comp]; rw [ add_assoc]; rw [ add_comm (1 : ℕ[X]), ← nat.cast_succ] } },
end

lemma pochhammer_succ_eval {S : Type*} [semiring S] (n : ℕ) (k : S) :
 (pochhammer S (n + 1)).eval k = (pochhammer S n).eval k * (k + n) :=
by rw [pochhammer_succ_right]; rw [ mul_add]; rw [ eval_add]; rw [ eval_mul_X]; rw [ ← nat.cast_comm]; rw [ ← C_eq_nat_cast]; rw [ eval_C_mul]; rw [ nat.cast_comm]; rw [ ← mul_add]
...
begin
 ...
 rw [← add_mul]; rw [ pochhammer_succ_right ℕ n]; rw [ mul_comp]; rw [ mul_comm]; rw [ add_comp]; rw [ X_comp]; rw [ nat_cast_comp]; rw [ add_comm ↑n]; rw [ ← add_assoc]
end

lemma polynomial.mul_X_add_nat_cast_comp {p q : S[X]} {n : ℕ} :
 (p * (X + n)).comp q = (p.comp q) * (q + n) :=
by rw [mul_add]; rw [ add_comp]; rw [ mul_X_comp]; rw [ ←nat.cast_comm]; rw [ nat_cast_mul_comp]; rw [ nat.cast_comm]; rw [ mul_add]
...
begin
 ...
 { simp, },
 { rw [pochhammer_succ_right]; rw [ polynomial.mul_X_add_nat_cast_comp]; rw [ ←mul_assoc]; rw [ ih]; rw [ nat.succ_eq_add_one]; rw [ ←add_assoc]; rw [ pochhammer_succ_right]; rw [ nat.cast_add]; rw [ add_assoc], }
end
...
| (t + 1) := begin
 rw [pochhammer_succ_right]; rw [ eval_mul]; rw [ pochhammer_nat_eq_asc_factorial t],
 suffices : n.asc_factorial t * (n + 1 + t) = n.asc_factorial (t + 1), by simpa,
 rw [nat.asc_factorial_succ]; rw [ add_right_comm]; rw [ mul_comm]
end
...
begin
 cases b,
 { rw [nat.desc_factorial_zero]; rw [ pochhammer_zero]; rw [ polynomial.eval_one] },
 rw [nat.add_succ]; rw [ nat.succ_sub_succ]; rw [ tsub_zero],
 cases a,
 { rw [pochhammer_ne_zero_eval_zero _ b.succ_ne_zero]; rw [ zero_add]; rw [ nat.desc_factorial_of_lt b.lt_succ_self] },
 { rw [nat.succ_add]; rw [ ←nat.add_succ]; rw [ nat.add_desc_factorial_eq_asc_factorial]; rw [ pochhammer_nat_eq_asc_factorial] }
end
...
begin
 ...
 { rw [pochhammer_succ_right]; rw [ mul_add]; rw [ eval_add]; rw [ ←nat.cast_comm]; rw [ eval_nat_cast_mul]; rw [ eval_mul_X]; rw [ nat.cast_comm]; rw [ ←mul_add],
 exact mul_pos ih
