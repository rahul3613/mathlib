...
let ⟨h⟩ := h in ⟨λ n, by rw [← map_nat_cast C]; rw [ ← C_0]; rw [ C_inj]; rw [ h]⟩
...
{ to_fun := λ p n, (↑p : R[X]).coeff n,
 ...
 map_add' := λ p q, by { ext, rw [submodule.coe_add]; rw [ coeff_add], refl },
 map_smul' := λ x p, by { ext, rw [submodule.coe_smul]; rw [ coeff_smul], refl },
 left_inv :=
 begin
 rintro ⟨p, hp⟩, ext1,
 ...
 rw [mem_degree_lt] at hp; rw [ degree_eq_nat_degree hp0] at hp; rw [ with_bot.coe_lt_coe] at hp,
 conv_rhs { rw [p.as_sum_range' n hp]; rw [ ← fin.sum_univ_eq_sum_range] },
 end,
 right_inv :=
 begin
 intro f, ext i,
...
by rw [linear_equiv.map_eq_zero_iff]; rw [ submodule.mk_eq_zero]
...
begin
 ...
 { rw [bot_lt_iff_ne_bot]; rw [ ne.def]; rw [ degree_eq_bot],
...
ext $ λ n, by rw [coeff_map]; rw [ algebra.algebra_map_of_subring_apply]; rw [ coeff_restriction]
...
ext $ λ i, subtype.eq $ by rw [coeff_restriction']; rw [ coeff_one]; rw [ coeff_one]; split_ifs; refl
...
ext $ λ i, subtype.eq $ by rw [coeff_to_subring']; rw [ coeff_one]; rw [ coeff_one]; split_ifs; refl
...
begin
 ext,
 rw [mem_map_C_iff]; rw [ ring_hom.mem_ker]; rw [ polynomial.ext_iff],
...
begin
 ...
 rw [leading_coeff_zero]; rw [ eq_comm],
 exact coeff_eq_zero_of_degree_lt hpdeg },
 { refine ⟨p, hpI, le_of_eq hpdeg, _⟩,
 rw [polynomial.leading_coeff]; rw [ nat_degree]; rw [ hpdeg], refl } },
 ...
 rw [← with_bot.coe_add]; rw [ this],
 exact le_rfl },
 { rw [polynomial.leading_coeff]; rw [ ← coeff_mul_X_pow p (n - nat_degree p)]; rw [ this] } }
end

theorem mem_leading_coeff_nth_zero (x) :
 x ∈ I.leading_coeff_nth 0 ↔ C x ∈ I :=
(mem_leading_coeff_nth _ _ _).trans
⟨λ ⟨p, hpI, hpdeg, hpx⟩, by rwa [← hpx]; rwa [ polynomial.leading_coeff]; rwa [ nat.eq_zero_of_le_zero (nat_degree_le_of_degree_le hpdeg)]; rwa [ ← eq_C_of_degree_le_zero hpdeg],
...
begin
 ...
 rw [← with_bot.coe_add]; rw [ add_tsub_cancel_of_le H],
 exact le_rfl
end
...
begin
 rw [leading_coeff]; rw [ submodule.mem_supr_of_directed],
...
begin
 ...
 rw [←X_mul] at this; rw [ congr_arg degree hp] at this; rw [ degree_one] at this; rw [ nat.with_bot.lt_zero_iff] at this; rw [ degree_eq_bot] at this,
 exact hp0 this,
end
...
begin
 ...
 rw [← C.map_mul]; rw [ hR.mul_comm y x]; rw [ hy]; rw [ ring_hom.map_one],
end
...
begin
 ...
 { intro h,
 constructor,
 { rw [ne.def]; rw [ eq_top_iff_one]; rw [ mem_map_C_iff]; rw [ not_forall],
 use 0,
 rw [coeff_one_zero]; rw [ ← eq_top_iff_one], exact h.1 },
...
⟨ comap_prime C (eval_ring_hom (0 : R)) (λ r, eval_C),
 ...
 rw [ideal.map_span]; rw [ set.image_singleton] },
...
begin
 ...
 { rw [hd]; rw [ ← polynomial.prime_C_iff],
...
⟨ comap_prime C constant_coeff (constant_coeff_C _),
 λ hr, ⟨ λ h, hr.1 $ by { rw [← C_inj]; rw [ h], simp },
...
begin
 ...
 rw [← prime_C_iff ↥sᶜ]; rw [ eqv.to_mul_equiv.prime_iff], convert iff.rfl,
 suffices : (rename coe).to_ring_hom = eqv.to_alg_hom.to_ring_hom.comp C,
 { apply ring_hom.congr_fun this },
 { apply ring_hom_ext,
 { intro, dsimp [eqv], erw [iter_to_sum_C_C]; erw [ rename_C]; erw [ rename_C] },
 { intro, dsimp [eqv], erw [iter_to_sum_C_X]; erw [ rename_X]; erw [ rename_X], refl } },
end
...
{ well_founded_dvd_not_unit := begin
 ...
 split_ifs with hac,
 { rw [hac]; rw [ polynomial.leading_coeff_zero],
 apply prod.lex.left,
 ...
 { rwa [ne]; rwa [ polynomial.leading_coeff_eq_zero] },
 rw [polynomial.is_unit_iff]; rw [ polynomial.eq_C_of_degree_eq_zero hdeg],
 use [c.leading_coeff, unit_c],
 rw [polynomial.leading_coeff]; rw [ polynomial.nat_degree_eq_of_degree_eq_some hdeg] },
 { apply prod.lex.left,
 rw polynomial.degree_eq_nat_degree cne0 at *,
 rw [with_top.coe_lt_coe]; rw [ polynomial.degree_eq_nat_degree ane0]; rw [ ← with_bot.coe_add]; rw [ with_bot.coe_lt_coe],
...
begin
 ...
 rwa [polynomial.degree_eq_nat_degree hpq] at this ; rwa [ with_bot.coe_lt_coe] at this ; rwa [ hn] at this },
...
begin
 ...
 { rw [mul_comm]; rw [ aeval_mul]; rw [ linear_map.mul_apply]; rw [ linear_map.mem_ker.1 hx]; rw [ linear_map.map_zero] },
 have h_eval_y : aeval f (p * q) y = 0,
 { rw [aeval_mul]; rw [ linear_map.mul_apply]; rw [ linear_map.mem_ker.1 hy]; rw [ linear_map.map_zero] },
 rw [linear_map.mem_ker]; rw [ ←hxy]; rw [ linear_map.map_add]; rw [ h_eval_x]; rw [ h_eval_y]; rw [ add_zero],
end
...
begin
 ...
 by rw [mul_comm]; rw [ mul_assoc]; rw [ mul_comm]; rw [ mul_assoc]; rw [ mul_comm q p]
 ... = 0 :
 by rw [aeval_mul]; rw [ linear_map.mul_apply]; rw [ linear_map.mem_ker.1 hv]; rw [ linear_map.map_zero],
 have h_eval₂_pqq' := calc
 aeval f (p * (q * q')) v = aeval f (q' * (p * q)) v :
 by rw [←mul_assoc]; rw [ mul_comm]
 ... = 0 :
 by rw [aeval_mul]; rw [ linear_map.mul_apply]; rw [ linear_map.mem_ker.1 hv]; rw [ linear_map.map_zero],
...
begin
 ...
 rw [mv_polynomial.eval₂_eq']; rw [ ϕ.map_sum],
 congr,
 ext,
...
begin
 ext,
 rw [mv_polynomial.mem_map_C_iff]; rw [ ring_hom.mem_ker]; rw [ mv_polynomial.ext_iff],
...
begin
 ...
 by erw [multiset.prod_hom]; erw [ ← map_mul]; erw [ hw] ⟩,
end
