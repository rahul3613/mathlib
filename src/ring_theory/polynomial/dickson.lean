...
begin
 ...
 rw [dickson_two_zero]; rw [ pow_add X (n + 1) 1]; rw [ mul_comm]; rw [ pow_one]
end
...
| 1 := by rw [dickson_one]; rw [ chebyshev.T_one]; rw [ X_comp]; rw [ ← mul_assoc]; rw [ ← C_1]; rw [ ← C_bit0]; rw [ ← C_mul]; rw [ mul_inv_of_self]; rw [ C_1]; rw [ one_mul]
| (n + 2) :=
begin
...
begin
 ...
 rw [inv_of_mul_self]; rw [ C_1]; rw [ one_mul]; rw [ one_mul]; rw [ comp_X]
end
...
begin
 ...
 rw [mul_comp]; rw [ C_comp]; rw [ X_comp]; rw [ ← mul_assoc]; rw [ ← C_1]; rw [ ← C_bit0]; rw [ ← C_mul]; rw [ inv_of_mul_self]; rw [ C_1]; rw [ one_mul]
end

lemma dickson_one_one_comp_comm (m n : ℕ) :
 (dickson 1 (1 : R) m).comp (dickson 1 1 n) = (dickson 1 1 n).comp (dickson 1 1 m) :=
by rw [← dickson_one_one_mul]; rw [ mul_comm]; rw [ dickson_one_one_mul]
...
begin
 ...
 have hφ : φ ≠ 0,
 { intro H,
 have : φ.eval 0 = 0, by rw [H]; rw [ eval_zero],
...
begin
 ...
 rw [h]; rw [ ← map_dickson (zmod.cast_hom (dvd_refl p) R)]; rw [ dickson_one_one_zmod_p]; rw [ polynomial.map_pow]; rw [ map_X]
end
