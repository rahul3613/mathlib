...
by rw [←mem_nth_roots h]; rw [ nth_roots]; rw [ mem_roots $ X_pow_sub_C_ne_zero h _]; rw [ C_1]; rw [ ←prod_cyclotomic_eq_X_pow_sub_one h]; rw [ is_root_prod]; apply_instance
...
begin
 rw [← mem_roots (cyclotomic_ne_zero n R)]; rw [ cyclotomic_eq_prod_X_sub_primitive_roots h]; rw [ roots_prod_X_sub_C]; rw [ ← finset.mem_def],
...
begin
 ...
 rw [←is_root_map_iff hf]; rw [ ←is_primitive_root.map_iff_of_injective hf]; rw [ map_cyclotomic]; rw [ ←is_root_cyclotomic_iff']
end
...
begin
 ...
 rw [mem_roots $ cyclotomic_ne_zero n R] at hζ; rw [ is_root_cyclotomic_iff] at hζ,
...
begin
 ...
 rw [nat_degree_one] at hnm; rw [ nat_degree_cyclotomic] at hnm,
 by_contra,
...
begin
 ...
 rwa [aeval_def]; rwa [ eval₂_eq_eval_map]; rwa [ map_cyclotomic]; rwa [ ←is_root.def]; rwa [ is_root_cyclotomic_iff]
end
