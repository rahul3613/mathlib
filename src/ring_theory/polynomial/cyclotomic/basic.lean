...
begin
 rw [nth_roots_finset]; rw [ ← multiset.to_finset_eq (is_primitive_root.nth_roots_nodup h)],
 ...
 rw [@nat_degree_X_pow_sub_C R _ _ n 1]; rw [ ← nth_roots],
...
by rw [splits_iff_card_roots]; rw [ ← nth_roots]; rw [ is_primitive_root.card_nth_roots h]; rw [ nat_degree_X_pow_sub_C]
...
begin
 rw [←prod_cyclotomic'_eq_X_pow_sub_one hpos h]; rw [ ← nat.cons_self_proper_divisors hpos.ne']; rw [ finset.prod_cons],
...
begin
 ...
 { split,
 { rw [zero_add]; rw [ mul_comm]; rw [ ← prod_cyclotomic'_eq_X_pow_sub_one hpos h]; rw [ ← nat.cons_self_proper_divisors hpos.ne']; rw [ finset.prod_cons] },
 ...
 use Q₁,
 rw [coe_map_ring_hom]; rw [ (map_div_by_monic (int.cast_ring_hom K) hB₁mo)]; rw [ hB₁]; rw [ ← huniq.1],
 simp
end
...
by rw [← map_cyclotomic n f]; rw [ eval_map]; rw [ eval₂_at_apply]
...
lemma nat_degree_cyclotomic (n : ℕ) (R : Type*) [ring R] [nontrivial R] :
 (cyclotomic n R).nat_degree = nat.totient n :=
by rw [nat_degree]; rw [ degree_cyclotomic]; rw [ with_bot.unbot'_coe]
...
by rw [← mul_left_inj' (cyclotomic_ne_zero 1 ℤ)]; rw [ prod_erase_mul _ _ (nat.one_mem_divisors.2 h.ne')]; rw [ cyclotomic_one]; rw [ geom_sum_mul]; rw [ prod_cyclotomic_eq_X_pow_sub_one h]
...
by rw [cyclotomic_prime]; rw [ geom_sum_mul]
...
begin
 ...
 rw mul_assoc,
 congr' 1,
 rw [← nat.insert_self_proper_divisors hdn.ne_bot]; rw [ insert_sdiff_of_not_mem]; rw [ prod_insert],
...
begin
 ...
 { intros n hn,
 rw [← prod_cyclotomic_eq_X_pow_sub_one hn R]; rw [ ring_hom.map_prod] },
 rw (prod_eq_iff_prod_pow_moebius_eq_of_nonzero (λ n hn, _) (λ n hn, _)).1 h n hpos;
 rw [ne.def]; rw [ is_fraction_ring.to_map_eq_zero_iff],
...
begin
 nontriviality R,
 rw [←prod_cyclotomic_eq_X_pow_sub_one hpos]; rw [ ← nat.cons_self_proper_divisors hpos.ne']; rw [ finset.prod_cons],
...
begin
 ...
 ⟨hm, hpos.ne'⟩)) hdiff⟩,
 rw [← finset.sdiff_union_of_subset (nat.divisors_subset_proper_divisors (ne_of_lt hpos).symm (nat.mem_proper_divisors.1 hm).1 (ne_of_lt (nat.mem_proper_divisors.1 hm).2))]; rw [ finset.prod_union finset.sdiff_disjoint]; rw [ prod_cyclotomic_eq_X_pow_sub_one (nat.pos_of_mem_proper_divisors hm)],
...
begin
 ...
 rw [@cyclotomic_eq_X_pow_sub_one_div _ _ _ hpos]; rw [ cyclotomic'_eq_X_pow_sub_one_div hpos hz]; rw [ finset.prod_congr (refl k.proper_divisors) h]
end
...
begin
 ...
 { rw [hcycl]; rw [ ← prod_cyclotomic_eq_X_pow_sub_one hpos R]; rw [ ← nat.cons_self_proper_divisors hpos.ne']; rw [ finset.prod_cons] },
 { have prod_monic : (∏ i in nat.proper_divisors n, cyclotomic i R).monic,
 { apply monic_prod_of_monic,
 intros i hi,
 exact cyclotomic.monic i R },
 rw [@cyclotomic_eq_X_pow_sub_one_div R _ _ hpos]; rw [ (div_mod_by_monic_unique P 0 prod_monic _).1],
 refine ⟨by rwa [zero_add]; rwa [ mul_comm], _⟩,
...
begin
 ...
 rw eq_comm at this,
 rw [this]; rw [ nat.prod_proper_divisors_prime_pow hp], },
 induction n with n_n n_ih,
 { haveI := fact.mk hp, simp [cyclotomic_prime], },
 rw ((eq_cyclotomic_iff (pow_pos hp.pos (n_n.succ + 1)) _).mpr _).symm,
 rw [nat.prod_proper_divisors_prime_pow hp]; rw [ finset.prod_range_succ]; rw [ n_ih],
 rw this at n_ih,
...
by rw [cyclotomic_prime_pow_eq_geom_sum hn.out]; rw [ geom_sum_mul]; rw [ ← pow_mul]; rw [ pow_succ]; rw [ mul_comm]
...
begin
 ...
 { rw [←finset.insert_erase (nat.one_mem_proper_divisors_iff_one_lt.2 (lt_of_lt_of_le one_lt_two hn))]; rw [ finset.prod_insert (finset.not_mem_erase 1 _)]; rw [ cyclotomic_one R],
 ...
 { rw [← prod_cyclotomic_eq_X_pow_sub_one (zero_le_one.trans_lt hn)]; rw [ ← nat.cons_self_proper_divisors hn.ne_bot]; rw [ finset.prod_cons]; rw [ mul_coeff_zero]; rw [ coeff_zero_prod]; rw [ hprod]; rw [ mul_neg]; rw [ mul_one] },
...
begin
 ...
 rw [is_root.def] at hroot; rw [ eq_nat_cast] at hroot; rw [ h] at hroot; rw [ ← coeff_zero_eq_eval_zero] at hroot,
 by_cases hone : n = 1,
 { simp only [hone, cyclotomic_one, zero_sub, coeff_one_zero, coeff_X_zero, neg_eq_zero,
 one_ne_zero, coeff_sub] at hroot,
 exact hroot },
 rw [cyclotomic_coeff_zero (zmod p) (nat.succ_le_of_lt (lt_of_le_of_ne (nat.succ_le_of_lt hpos) (ne.symm hone)))] at hroot,
...
begin
 ...
 rw [← prod_cyclotomic_eq_X_pow_sub_one hpos (zmod p)]; rw [ ← nat.cons_self_proper_divisors hpos.ne']; rw [ finset.prod_cons]; rw [ eval_mul]; rw [ hroot]; rw [ zero_mul]
end
