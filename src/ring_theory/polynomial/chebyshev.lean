...
by rw [U_eq_X_mul_U_add_T]; rw [ add_comm (X * U R n)]; rw [ add_sub_cancel]
...
... = X * (2 * X * T R (n + 2) - T R (n + 1)) - (1 - X ^ 2) * (2 * X * U R (n + 1) - U R n) :
 by ring
... = X * T R (n + 2 + 1) - (1 - X ^ 2) * U R (n + 2) : by rw [T_add_two _ (n + 1)]; rw [ U_add_two]

lemma one_sub_X_sq_mul_U_eq_pol_in_T (n : ℕ) :
 (1 - X ^ 2) * U R n = X * T R (n + 1) - T R (n + 2) :=
by rw [T_eq_X_mul_T_sub_pol_U]; rw [ ←sub_add]; rw [ sub_self]; rw [ zero_add]
...
lemma one_sub_X_sq_mul_derivative_T_eq_poly_in_T (n : ℕ) :
 ...
 by rw [one_sub_X_sq_mul_U_eq_pol_in_T]; rw [ T_add_two]
...
begin
 ...
 rw [T_eq_U_sub_X_mul_U]; rw [ C_eq_nat_cast]; rw [ nat.cast_bit0]; rw [ nat.cast_one],
 ring },
 ...
 by rw [←U_eq_X_mul_U_add_T]; rw [ ←T_derivative_eq_U]; rw [ ←nat.cast_one]; rw [ ←nat.cast_add]; rw [ nat.cast_one]; rw [ ←T_derivative_eq_U (n + 1)]
