...
begin
 ...
 { exact z_ne_zero (by rwa [key] at h1) ; rwa [ self_eq_add_left] at h1) },
 { exact one_ne_zero (by rwa [key] at h1) ; rwa [ self_eq_add_right] at h1) },
 { exact z_ne_zero (pow_eq_zero (by rwa [key] at h2)) ; rwa [ add_self_eq_zero] at h2)) },
end

lemma X_pow_sub_X_sub_one_irreducible (hn1 : n ≠ 1) : irreducible (X ^ n - X - 1 : ℤ[X]) :=
begin
 by_cases hn0 : n = 0,
 { rw [hn0]; rw [ pow_zero]; rw [ sub_sub]; rw [ add_comm]; rw [ ←sub_sub]; rw [ sub_self]; rw [ zero_sub],
...
begin
 ...
 { rwa [polynomial.map_sub] at h ; rwa [ polynomial.map_sub] at h ; rwa [ polynomial.map_pow] at h ; rwa [ polynomial.map_one] at h ; rwa [ polynomial.map_X] at h },
