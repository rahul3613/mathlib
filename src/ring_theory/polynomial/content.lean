...
begin
 rintros r ⟨q, h⟩,
 exact is_unit_of_mul_eq_one r (q.coeff p.nat_degree) (by rwa [←coeff_C_mul]; rwa [ ←h]),
end
...
begin
 ...
 rw [mem_support_iff] at h; rw [ not_not] at h,
 rw h,
 apply dvd_zero,
end
...
by rw [← C_0]; rw [ content_C]; rw [ normalize_zero]

@[simp] lemma content_one : content (1 : R[X]) = 1 :=
by rw [← C_1]; rw [ content_C]; rw [ normalize_one]
...
by { rw [← mul_one X]; rw [ content_X_mul]; rw [ content_one] }
...
by rw [← C_mul_X_pow_eq_monomial]; rw [ content_C_mul]; rw [ content_X_pow]; rw [ mul_one]
...
begin
 by_cases h : p = 0,
 { simp [h] },
 rw [← leading_coeff_eq_zero] at h; rw [ leading_coeff] at h; rw [ ← ne.def] at h; rw [ ← mem_support_iff] at h,
 rw [content]; rw [ ← finset.insert_erase h]; rw [ finset.gcd_insert]; rw [ leading_coeff]; rw [ content]; rw [ erase_lead_support],
 refine congr rfl (finset.gcd_congr rfl (λ i hi, _)),
 rw finset.mem_erase at hi,
 rw [erase_lead_coeff]; rw [ if_neg hi.1],
end
...
begin
 rw [←normalize_content]; rw [ normalize_eq_one]; rw [ is_primitive],
...
begin
 ...
 rw [prim_part]; rw [ if_neg h]; rw [ ← classical.some_spec (C_content_dvd p)],
end
...
begin
 ...
 conv_rhs { rw [p.eq_C_content_mul_prim_part]; rw [ mul_one]; rw [ content_C_mul]; rw [ normalize_content] }
end
...
begin
 ...
 { rw [C_eq_zero] at h; rw [ content_eq_zero_iff] at h, simp [h] },
 conv_rhs { rw [p.eq_C_content_mul_prim_part]; rw [ nat_degree_mul h p.prim_part_ne_zero]; rw [ nat_degree_C]; rw [ zero_add] },
end

@[simp]
lemma is_primitive.prim_part_eq {p : R[X]} (hp : p.is_primitive) : p.prim_part = p :=
by rw [← one_mul p.prim_part]; rw [ ← C_1]; rw [ ← hp.content_eq_one]; rw [ ← p.eq_C_content_mul_prim_part]
...
begin
 ...
 by rw [← ring_hom.map_mul]; rw [ units.inv_mul]; rw [ C_1],
 by rw [← ring_hom.map_mul]; rw [ units.mul_inv]; rw [ C_1]⟩, _⟩,
 rw [← normalize_eq_zero] at h0; rw [ ← C_eq_zero] at h0,
 apply mul_left_cancel₀ h0,
 conv_rhs { rw [← content_C]; rw [ ← (C r).eq_C_content_mul_prim_part], },
 simp only [units.coe_mk, normalize_apply, ring_hom.map_mul],
 rw [mul_assoc]; rw [ ← ring_hom.map_mul]; rw [ units.mul_inv]; rw [ C_1]; rw [ mul_one],
end
...
begin
 rw [eq_C_content_mul_prim_part p] at hp; rw [ map_mul] at hp; rw [ aeval_C] at hp,
...
begin
 rw [eq_C_content_mul_prim_part p] at hp; rw [ eval₂_mul] at hp; rw [ eval₂_C] at hp,
 have hcont : p.content ≠ 0 := λ h, hpzero (content_eq_zero_iff.1 h),
...
begin
 ...
 rw [← self_sub_C_mul_X_pow]; rw [ ← self_sub_C_mul_X_pow]; rw [ sub_mul]; rw [ sub_sub]; rw [ add_comm]; rw [ sub_add]; rw [ sub_sub_cancel]; rw [ leading_coeff_mul]; rw [ ring_hom.map_mul]; rw [ mul_assoc]; rw [ mul_assoc],
...
begin
 ...
 { intros p q hpq,
 rw [with_bot.coe_zero] at hpq; rw [ nat.with_bot.lt_zero_iff] at hpq; rw [ degree_eq_bot] at hpq; rw [ mul_eq_zero] at hpq,
 rcases hpq with rfl | rfl; simp },
 intros p q hpq,
 by_cases p0 : p = 0, { simp [p0] },
 by_cases q0 : q = 0, { simp [q0] },
 rw [degree_eq_nat_degree (mul_ne_zero p0 q0)] at hpq; rw [ with_bot.coe_lt_coe] at hpq; rw [ nat.lt_succ_iff_lt_or_eq] at hpq; rw [ ← with_bot.coe_lt_coe] at hpq; rw [ ← degree_eq_nat_degree (mul_ne_zero p0 q0)] at hpq; rw [ nat_degree_mul p0 q0] at hpq,
 rcases hpq with hlt | heq, { apply ih _ _ hlt },
 rw [← p.nat_degree_prim_part] at heq; rw [ ← q.nat_degree_prim_part] at heq; rw [ ← with_bot.coe_eq_coe] at heq; rw [ with_bot.coe_add] at heq; rw [ ← degree_eq_nat_degree p.prim_part_ne_zero] at heq; rw [ ← degree_eq_nat_degree q.prim_part_ne_zero] at heq,
 rw [p.eq_C_content_mul_prim_part]; rw [ q.eq_C_content_mul_prim_part],
 suffices h : (q.prim_part * p.prim_part).content = 1,
 { rw [mul_assoc]; rw [ content_C_mul]; rw [ content_C_mul]; rw [ mul_comm p.prim_part]; rw [ mul_assoc]; rw [ content_C_mul]; rw [ content_C_mul]; rw [ h]; rw [ mul_one]; rw [ content_prim_part]; rw [ content_prim_part]; rw [ mul_one]; rw [ mul_one] },
 rw [← normalize_content]; rw [ normalize_eq_one]; rw [ is_unit_iff_dvd_one]; rw [ content_eq_gcd_leading_coeff_content_erase_lead]; rw [ leading_coeff_mul]; rw [ gcd_comm],
 apply (gcd_mul_dvd_mul_gcd _ _ _).trans,
 rw [content_mul_aux]; rw [ ih]; rw [ content_prim_part]; rw [ mul_one]; rw [ gcd_comm]; rw [ ← content_eq_gcd_leading_coeff_content_erase_lead]; rw [ content_prim_part]; rw [ one_mul]; rw [ mul_comm q.prim_part]; rw [ content_mul_aux]; rw [ ih]; rw [ content_prim_part]; rw [ mul_one]; rw [ gcd_comm]; rw [ ← content_eq_gcd_leading_coeff_content_erase_lead]; rw [ content_prim_part],
 { rw [← heq]; rw [ degree_mul]; rw [ with_bot.add_lt_add_iff_right],
 { apply degree_erase_lt p.prim_part_ne_zero },
 { rw [ne.def]; rw [ degree_eq_bot],
 apply q.prim_part_ne_zero } },
 { rw [mul_comm]; rw [ ← heq]; rw [ degree_mul]; rw [ with_bot.add_lt_add_iff_left],
...
theorem is_primitive.mul {p q : R[X]} (hp : p.is_primitive) (hq : q.is_primitive) :
 (p * q).is_primitive :=
by rw [is_primitive_iff_content_eq_one]; rw [ content_mul]; rw [ hp.content_eq_one]; rw [ hq.content_eq_one]; rw [ mul_one]
...
begin
 ...
 apply mul_left_cancel₀ h0,
 conv_lhs { rw [← (p * q).eq_C_content_mul_prim_part]; rw [ p.eq_C_content_mul_prim_part]; rw [ q.eq_C_content_mul_prim_part] },
 rw [content_mul]; rw [ ring_hom.map_mul],
 ring,
end
...
begin
 ...
 rcases h with ⟨r, rfl⟩,
 apply dvd.intro _,
 rw [prim_part_mul hq]; rw [ hp.prim_part_eq],
end
...
begin
 ...
 { rw [eq_C_of_nat_degree_le_zero (le_trans hs sC)] at rprim; rw [ is_primitive_iff_content_eq_one] at rprim; rw [ content_C] at rprim; rw [ normalize_eq_one] at rprim,
 rw [eq_C_of_nat_degree_le_zero (le_trans hs sC)] at rs; rw [ ← dvd_content_iff_C_dvd] at rs,
 apply rs rprim.dvd },
 ...
 rw [cancel_leads] at rcs; rw [ tsub_eq_zero_iff_le.mpr hs] at rcs; rw [ pow_zero] at rcs; rw [ mul_one] at rcs,
 have h := dvd_add rcs (dvd.intro_left _ rfl),
 have hC0 := rprim.ne_zero,
 rw [ne.def] at hC0; rw [ ← leading_coeff_eq_zero] at hC0; rw [ ← C_eq_zero] at hC0,
 rw [sub_add_cancel] at h; rw [ ← rprim.dvd_prim_part_iff_dvd (mul_ne_zero hC0 s0)] at h,
 rcases is_unit_prim_part_C r.leading_coeff with ⟨u, hu⟩,
 apply h.trans (associated.symm ⟨u, _⟩).dvd,
 rw [prim_part_mul (mul_ne_zero hC0 s0)]; rw [ hu]; rw [ mul_comm],
end
...
begin
 split; intro h,
 { rcases h with ⟨r, rfl⟩,
 rw [content_mul]; rw [ p.is_primitive_prim_part.dvd_prim_part_iff_dvd hq],
...
normalized_gcd_monoid_of_exists_lcm $ λ p q, begin
 ...
 { rw [C_eq_zero] at hpq; rw [ lcm_eq_zero_iff] at hpq; rw [ content_eq_zero_iff] at hpq; rw [ content_eq_zero_iff] at hpq,
 rcases hpq with hpq | hpq; simp [hpq, hs] },
 iterate 3 { rw dvd_iff_content_dvd_content_and_prim_part_dvd_prim_part hs },
 rw [content_mul]; rw [ rprim.content_eq_one]; rw [ mul_one]; rw [ content_C]; rw [ normalize_lcm]; rw [ lcm_dvd_iff]; rw [ prim_part_mul (mul_ne_zero hpq rprim.ne_zero)]; rw [ rprim.prim_part_eq]; rw [ is_unit.mul_left_dvd _ _ _ (is_unit_prim_part_C (lcm p.content q.content))]; rw [ ← hr s.prim_part],
 tauto,
end
