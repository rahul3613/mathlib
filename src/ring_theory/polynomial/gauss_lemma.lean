...
begin
 ...
 { rw [lifts_iff_coeff_lifts] at this; rw [ ←ring_hom.coe_range] at this; rw [ subalgebra.range_algebra_map] at this,
 refine (lifts_iff_coeff_lifts _).2 (λ n, _),
 rw [← ring_hom.coe_range]; rw [ subalgebra.range_algebra_map],
 obtain ⟨p, hp, he⟩ := (set_like.mem_coe.mp (this n)), use [p, hp],
 rw [is_scalar_tower.algebra_map_eq R K] at he; rw [ coeff_map] at he; rw [ ← eval₂_map] at he; rw [ eval₂_at_apply] at he,
 rw eval₂_eq_eval_map, apply (injective_iff_map_eq_zero _).1 _ _ he,
 { apply ring_hom.injective } },
 rw [is_scalar_tower.algebra_map_eq R K _]; rw [ ← map_map],
...
begin
 ...
 { obtain ⟨g', hg'⟩ := lem,
 use g',
 rw [hg']; rw [ mul_assoc]; rw [ ← C_mul]; rw [ inv_mul_cancel (leading_coeff_ne_zero.mpr g_ne_0)]; rw [ C_1]; rw [ mul_one] },
...
begin
 ...
 λ a b h, (h_irr.is_unit_or_is_unit $ by rw [h]; rw [ polynomial.map_mul]).imp _ _⟩,
...
begin
 ...
 { rw [← leading_coeff_mul]; rw [ ← H]; rw [ monic.leading_coeff (h.map $ algebra_map R K)] },

 rw [← ha] at H; rw [ ← hb] at H; rw [ mul_comm _ (C b.leading_coeff)] at H; rw [ mul_assoc] at H; rw [ ← mul_assoc (C a.leading_coeff)] at H; rw [ ← C_mul] at H; rw [ this] at H; rw [ C_1] at H; rw [ one_mul] at H; rw [ ← polynomial.map_mul] at H,
 rw [← hb]; rw [ ← polynomial.coe_map_ring_hom],
 refine is_unit.mul
 (is_unit.map _ (or.resolve_left (hp.is_unit_or_is_unit _) (show ¬ is_unit a', from _)))
 (is_unit_iff_exists_inv'.mpr (exists.intro (C a.leading_coeff) $ by rwa [← C_mul]; rwa [ this]; rwa [ C_1])),
 { exact polynomial.map_injective _ (is_fraction_ring.injective R K) H },

 { by_contra h_contra,
 refine hₐ _,
 rw [← ha]; rw [ ← polynomial.coe_map_ring_hom],
 exact is_unit.mul (is_unit.map _ h_contra) (is_unit_iff_exists_inv.mpr
 (exists.intro (C b.leading_coeff) $ by rwa [← C_mul]; rwa [ this]; rwa [ C_1])) },
end
...
begin
 ...
 rw [is_root]; rw [ eval_map]; rw [ ← aeval_def]; rw [ minpoly.aeval R x] },
end
...
begin
 ...
 rw [monic.leading_coeff] at hr'; rw [ C_1] at hr'; rw [ mul_one] at hr',
...
begin
 ...
 rw [subtype.coe_mk] at hc; rw [ algebra.smul_def] at hc; rw [ algebra_map_apply] at hc,
 apply is_unit_of_mul_is_unit_right,
 rw [← hc]; rw [ (integer_normalization R⁰ p).eq_C_content_mul_prim_part]; rw [ ← hu]; rw [ ← ring_hom.map_mul]; rw [ is_unit_iff],
 ...
 rw [mul_eq_zero] at con; rw [ content_eq_zero_iff] at con; rw [ is_fraction_ring.integer_normalization_eq_zero_iff] at con,
...
begin
 ...
 rw [algebra.smul_def] at hc hd; rw [ algebra_map_apply] at hc hd; rw [ subtype.coe_mk] at hc hd,
 ...
 { rw [← dvd_dvd_iff_associated]; rw [ ← normalize_eq_normalize_iff]; rw [ normalize.map_mul]; rw [ normalize.map_mul]; rw [ normalize_content]; rw [ normalize_content]; rw [ ← mul_one (normalize c * normalize d)]; rw [ ← hp.content_eq_one]; rw [ ← content_C]; rw [ ← content_C]; rw [ ← content_mul]; rw [ ← content_mul]; rw [ ← content_mul]; rw [ h1] },
 rw [← ring_hom.map_mul] at h1; rw [ eq_comm] at h1; rw [ (integer_normalization R⁰ a).eq_C_content_mul_prim_part] at h1; rw [ (integer_normalization R⁰ b).eq_C_content_mul_prim_part] at h1; rw [ mul_assoc] at h1; rw [ mul_comm _ (C _ * _)] at h1; rw [ ← mul_assoc] at h1; rw [ ← mul_assoc] at h1; rw [ ← ring_hom.map_mul] at h1; rw [ ← hu] at h1; rw [ ring_hom.map_mul] at h1; rw [ mul_assoc] at h1; rw [ mul_assoc] at h1; rw [ ← mul_assoc (C ↑u)] at h1,
 have h0 : (a ≠ 0) ∧ (b ≠ 0),
 { classical,
 rw [ne.def]; rw [ ne.def]; rw [ ← decidable.not_or_iff_and_not]; rw [ ← mul_eq_zero]; rw [ ← hab],
 intro con,
...
begin
 ...
 rw [subtype.coe_mk] at hs; rw [ algebra.smul_def] at hs; rw [ algebra_map_apply] at hs,
 have h : p ∣ q * C s,
 ...
 simp },
 rw [← hp.dvd_prim_part_iff_dvd] at h; rw [ prim_part_mul] at h; rw [ hq.prim_part_eq] at h; rw [ associated.dvd_iff_dvd_right] at h,
 { exact h },
 { symmetry,
