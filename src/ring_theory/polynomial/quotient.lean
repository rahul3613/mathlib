...
(ideal.quotient_equiv_alg_of_eq R $ by rw [ideal.span_insert]; rw [ sup_comm]).trans $
...
begin
 intros a ha,
 rw [ring_hom.comp_apply]; rw [ quotient.eq_zero_iff_mem],
...
by rw [polynomial_quotient_equiv_quotient_polynomial]; rw [ ring_equiv.symm_mk]; rw [ ring_equiv.coe_mk]; rw [ ideal.quotient.lift_mk]; rw [ coe_eval₂_ring_hom]; rw [ eval₂_eq_eval_map]; rw [ ←polynomial.map_map]; rw [ ←eval₂_eq_eval_map]; rw [ polynomial.eval₂_C_X]
...
begin
 ...
 rw [ring_equiv.symm_apply_apply]; rw [ polynomial_quotient_equiv_quotient_polynomial_symm_mk],
end
...
begin
 ...
 rw [mem_comap]; rw [ ← quotient.eq_zero_iff_mem]; rw [ ← ring_hom.comp_apply],
 rw [ring_hom.mem_ker] at hf; rw [ coe_map_ring_hom] at hf,
 replace hf := congr_arg (λ (f : polynomial _), f.coeff n) hf,
 simp only [coeff_map, coeff_zero] at hf,
 rwa [subtype.ext_iff] at hf ; rwa [ ring_hom.coe_range_restrict] at hf },
 obtain ⟨x, hx'⟩ := x,
 ...
 rw [ring_hom.mem_ker]; rw [ ring_hom.map_sub]; rw [ hf.2]; rw [ sub_eq_zero]; rw [ coe_map_ring_hom]; rw [ map_C] },
 exact hx,
end
...
begin
 rw as_sum a,
 rw [coe_eval₂_hom]; rw [ eval₂_sum],
 refine finset.sum_eq_zero (λ n hn, _),
 simp only [eval₂_monomial, function.comp_app, ring_hom.coe_comp],
 refine mul_eq_zero_of_left _ _,
 suffices : coeff n a ∈ I,
 { rw [← @ideal.mk_ker R _ I] at this; rw [ ring_hom.mem_ker] at this,
 simp only [this, C_0] },
