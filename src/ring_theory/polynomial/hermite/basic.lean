...
begin
 ...
 { refl },
 { rw [function.iterate_succ_apply']; rw [ ← ih]; rw [ hermite_succ] }
end
...
begin
 rw [← coeff_nat_degree]; rw [ nat_degree_hermite]; rw [ coeff_hermite_self],
end
...
begin
 ...
 { rw (by rw [nat.succ_add]; rw [ nat.add_succ] : n.succ + k.succ = n + k + 2) at hnk,
...
| (n + 1) 0 := begin
 ...
 rw [coeff_hermite_explicit n 1]; rw [ (by ring_nf : 2 * (n + 1) - 1 = 2 * n + 1)]; rw [ nat.double_factorial_add_one]; rw [ nat.choose_zero_right]; rw [ nat.choose_one_right]; rw [ pow_succ],
 push_cast,
 ring,
end
| (n + 1) (k + 1) := begin
 ...
 -- Factor out (-1)'s.
 rw [mul_comm (↑k + _)]; rw [ sub_eq_add_neg],
 nth_rewrite 2 neg_eq_neg_one_mul,
 simp only [mul_assoc, ← mul_add, pow_succ],
 congr' 2,
 -- Factor out double factorials.
 norm_cast,
 rw [(by ring_nf : 2 * (n + 1) - 1 = 2 * n + 1)]; rw [ nat.double_factorial_add_one]; rw [ mul_comm (2 * n + 1)],
 ...
 { rw [(by ring : 2 * (n + 1) + k = 2 * n + (k + 2))]; rw [ coeff_hermite_explicit n (k + 2)] },
end
...
begin
 ...
 { rw [nat.even_add] at hnk; rw [ ← (nat.even_sub h_le)] at hnk,
 obtain ⟨m, hm⟩ := hnk,
 rw [(by linarith : n = 2 * m + k)]; rw [ nat.add_sub_cancel]; rw [ nat.mul_div_cancel_left _ (nat.succ_pos 1)]; rw [ coeff_hermite_explicit] },
