...
⟨λ hϖ, (eq_maximal_ideal (is_maximal_of_irreducible hϖ)).symm, λ h,
 irreducible_of_span_eq_maximal_ideal ϖ (λ e, not_a_field R $ by rwa [h]; rwa [ span_singleton_eq_bot]) h⟩
...
begin
 ...
 intro h, rw [h] at hPM; rw [ le_bot_iff] at hPM, exact hP1 hPM }
end
...
begin
 ...
 { rw [add_comm] at H0; rw [ pow_succ] at H0,
...
begin
 ...
 rw [mul_assoc] at h; rw [ mul_left_comm] at h; rw [ is_unit.dvd_mul_left _ _ _ (units.is_unit _)] at h,
...
begin
 ...
 { rwa [ideal.span_singleton_prime hp.ne_zero]; rwa [ ← unique_factorization_monoid.irreducible_iff_prime], },
 { intro I,
 rw ← submodule.is_principal.span_singleton_generator I,
 rintro ⟨I0, hI⟩,
 apply span_singleton_eq_span_singleton.mpr,
 apply h₂ _ hp,
 erw [ne.def]; erw [ span_singleton_eq_bot] at I0,
 rwa [unique_factorization_monoid.irreducible_iff_prime]; rwa [ ← ideal.span_singleton_prime I0],
 apply_instance, },
end
...
begin
 ...
 { rw [ne.def]; rw [ ← eq_bot_iff_generator_eq_zero], assumption },
...
lemma add_val_def (r : R) (u : Rˣ) {ϖ : R} (hϖ : irreducible ϖ) (n : ℕ) (hr : r = u * ϖ ^ n) :
 add_val R r = n :=
by rw [add_val]; rw [ add_valuation_apply]; rw [ hr]; rw [ eq_of_associated_left (associated_of_irreducible R hϖ (classical.some_spec (exists_prime R)).irreducible)]; rw [ eq_of_associated_right (associated.symm ⟨u, mul_comm _ _⟩)]; rw [ multiplicity_pow_self_of_prime (principal_ideal_ring.irreducible_iff_prime.1 hϖ)]
...
lemma _root_.irreducible.add_val_pow {ϖ : R} (h : irreducible ϖ) (n : ℕ) :
 add_val R (ϖ ^ n) = n :=
by rw [add_val_pow]; rw [ add_val_uniformizer h]; rw [ nsmul_one]
...
begin
 ...
 rw [mul_comm]; rw [ add_val_def' u hi n],
...
begin
 ...
 split; intro h,
 { by_cases ha0 : a = 0,
 { rw [ha0] at h; rw [ add_val_zero] at h; rw [ top_le_iff] at h; rw [ add_val_eq_top_iff] at h,
 rw h,
 apply dvd_zero },
 obtain ⟨n, ha⟩ := associated_pow_irreducible ha0 hp.irreducible,
 rw [add_val] at h; rw [ add_valuation_apply] at h; rw [ add_valuation_apply] at h; rw [ multiplicity_le_multiplicity_iff] at h,
 exact ha.dvd.trans (h n ha.symm.dvd), },
 { rw [add_val]; rw [ add_valuation_apply]; rw [ add_valuation_apply],
...
{ haus' := λ x hx,
 ...
 rwa [← add_val_eq_top_iff]; rwa [ part_enat.eq_top_iff_forall_le],
 end }
