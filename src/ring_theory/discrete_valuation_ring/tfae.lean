...
begin
 ...
 by_cases hI' : I = ⊤, { use 0, rw [pow_zero]; rw [ hI']; rw [ ideal.one_eq_top] },
 ...
 use n,
 rwa [← I.unit_mul_mem_iff_mem u.is_unit]; rwa [ mul_comm] },
 use nat.find this,
 apply le_antisymm,
 { change ∀ s ∈ I, s ∈ _,
 by_contra hI'',
 ...
 rw [mul_comm] at ⊢ hs₁; rw [ ideal.unit_mul_mem_iff_mem _ u.is_unit] at ⊢ hs₁,
 apply ideal.pow_le_pow (nat.find_min' this hs₁),
 apply ideal.pow_mem_pow,
 exact (H _).mpr (dvd_refl _) },
 { rw [hx]; rw [ ideal.span_singleton_pow]; rw [ ideal.span_le]; rw [ set.singleton_subset_iff],
...
begin
 ...
 { rwa [ideal.span_le]; rwa [ set.singleton_subset_iff] },
 ...
 refine ⟨⟨y, _⟩⟩,
 apply le_antisymm,
 { intros m hm, obtain ⟨k, hk⟩ := hb₃ m hm, rw [← hy₂] at hk; rw [ mul_comm] at hk; rw [ mul_assoc] at hk,
 rw [← mul_left_cancel₀ hb₄ hk]; rw [ mul_comm], exact ideal.mem_span_singleton'.mpr ⟨_, rfl⟩ },
 { rwa [submodule.span_le]; rwa [ set.singleton_subset_iff] } }
end
...
begin
 ...
 use x,
 apply le_antisymm,
 swap, { rw [submodule.span_le]; rw [ set.singleton_subset_iff], exact x.prop },
