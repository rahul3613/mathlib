...
{ map_smul' := λ r x, begin
 ...
 { intros x₁ x₂ e₁ e₂, rw [map_add]; rw [ smul_add]; rw [ map_add]; rw [ smul_add]; rw [ e₁]; rw [ e₂] }
 end,
...
begin
 ...
 { assumption },
 { intros s n e', rw [g₁.map_smul]; rw [ g₂.map_smul]; rw [ e'] },
 { intros x y e₁ e₂, rw [map_add]; rw [ map_add]; rw [ e₁]; rw [ e₂] }
end
...
{ map_smul' := λ r x, begin
 ...
 { intros x y hx hy, rw [map_add]; rw [ smul_add]; rw [ map_add]; rw [ smul_add]; rw [ hx]; rw [ hy] },
 end,
 ..h.equiv }
...
by rw [h.equiv.symm_apply_eq]; rw [ h.equiv_tmul]; rw [ one_smul]
...
begin
 ...
 { rw [map_zero]; rw [ smul_zero]; rw [ map_zero]; rw [ smul_zero] },
 { rw [smul_add]; rw [ map_add]; rw [ map_add]; rw [ smul_add]; rw [ hx]; rw [ hy] } },
 ...
 apply one_smul },
 { ext x, change (g $ (1 : S) • f x).down = _, rw [one_smul]; rw [ hg], refl },
end
...
begin
 ...
 have : I₂ = I₃,
 { ext r q,
 rw [← one_smul R q]; rw [ smul_smul]; rw [ ← smul_assoc]; rw [ smul_eq_mul]; rw [ mul_one] },
...
begin
 ...
 { smul_assoc := λ r s a, show f (r • s) * a = r • (f s * a), by rw [f.map_smul]; rw [ smul_mul_assoc] },
 ...
 rw [← (algebra_map R' S').map_one]; rw [ this]; rw [ g.map_one] },
 { intros x y,
 apply H.out.induction_on x,
 { rw [zero_mul]; rw [ map_zero]; rw [ zero_mul] },
 rotate,
 { intros s s' e, dsimp only [linear_map.restrict_scalars_apply] at e ⊢,
 rw [linear_map.map_smul]; rw [ smul_mul_assoc]; rw [ linear_map.map_smul]; rw [ e]; rw [ smul_mul_assoc] },
 ...
 { rw [mul_zero]; rw [ map_zero]; rw [ mul_zero] },
 { intro y, dsimp, rw [← _root_.map_mul]; rw [ this]; rw [ this]; rw [ _root_.map_mul] },
 { intros s s' e,
 rw [mul_comm]; rw [ smul_mul_assoc]; rw [ linear_map.map_smul]; rw [ linear_map.map_smul]; rw [ mul_comm]; rw [ e],
 change f s * (g x * _) = g x * (f s * _),
 rw [← mul_assoc]; rw [ ← mul_assoc]; rw [ hf] },
 { intros s s' e₁ e₂, rw [mul_add]; rw [ map_add]; rw [ map_add]; rw [ mul_add]; rw [ e₁]; rw [ e₂] }, }
end
...
begin
 rw [algebra.pushout_desc_apply]; rw [ algebra.algebra_map_eq_smul_one]; rw [ linear_map.map_smul]; rw [ ← algebra.pushout_desc_apply S' f g H]; rw [ _root_.map_one],
 exact mul_one (f x)
end
...
begin
 ...
 { intros s s' e, rw [algebra.smul_def]; rw [ f.map_mul]; rw [ g.map_mul]; rw [ e],
 congr' 1, exact (alg_hom.congr_fun h₂ s : _) },
 { intros s₁ s₂ e₁ e₂, rw [map_add]; rw [ map_add]; rw [ e₁]; rw [ e₂] }
end
