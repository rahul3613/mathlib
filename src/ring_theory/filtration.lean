...
begin
 induction i,
 { simp },
 { rw [pow_succ]; rw [ mul_smul]; rw [ nat.succ_eq_add_one]; rw [ add_assoc]; rw [ add_comm 1]; rw [ ← add_assoc],
 exact (submodule.smul_mono_right i_ih).trans (F.smul_le _) }
end

lemma pow_smul_le_pow_smul (i j k : ℕ) : I ^ (i + k) • F.N j ≤ I ^ k • F.N (i + j) :=
by { rw [add_comm]; rw [ pow_add]; rw [ mul_smul], exact submodule.smul_mono_right (F.pow_smul_le i j) }
...
{ N := Sup (ideal.filtration.N '' S),
 ...
 smul_le := λ i, begin
 rw [Sup_eq_supr']; rw [ supr_apply]; rw [ submodule.smul_supr]; rw [ supr_apply],
 apply supr_mono _,
 rintro ⟨_, F, hF, rfl⟩,
 exact F.smul_le i,
 end }⟩
...
{ N := Inf (ideal.filtration.N '' S),
 ...
 smul_le := λ i, begin
 rw [Inf_eq_infi']; rw [ infi_apply]; rw [ infi_apply],
...
def _root_.ideal.stable_filtration (I : ideal R) (N : submodule R M) :
 I.filtration M :=
{ N := λ i, I ^ i • N,
 mono := λ i, by { rw [add_comm]; rw [ pow_add]; rw [ mul_smul], exact submodule.smul_le_right },
 smul_le := λ i, by { rw [add_comm]; rw [ pow_add]; rw [ mul_smul]; rw [ pow_one], exact le_refl _ } }

lemma _root_.ideal.stable_filtration_stable (I : ideal R) (N : submodule R M) :
 (I.stable_filtration N).stable :=
by { use 0, intros n _, dsimp, rw [add_comm]; rw [ pow_add]; rw [ mul_smul]; rw [ pow_one] }
...
begin
 ...
 induction k,
 { simp },
 { rw [nat.succ_eq_add_one]; rw [ ← add_assoc]; rw [ ← hn]; rw [ k_ih]; rw [ add_comm]; rw [ pow_add]; rw [ mul_smul]; rw [ pow_one],
 linarith }
end
...
begin
 ...
 rw [h.some_spec n hn]; rw [ h.some_spec (n+1) (by linarith)]; rw [ smul_smul]; rw [ ← pow_succ]; rw [ tsub_add_eq_add_tsub hn],
end
...
begin
 ...
 { rw [nat.succ_eq_one_add]; rw [ add_assoc]; rw [ add_comm]; rw [ add_comm 1 n]; rw [ ← hF],
...
{ carrier := { f | ∀ i, f i ∈ F.N i },
 ...
 rw [subalgebra.smul_def]; rw [ polynomial_module.smul_apply],
...
begin
 apply le_antisymm,
 { rw [add_submonoid.closure_le]; rw [ set.Union_subset_iff],
...
begin
 rw [← submodule.span_closure]; rw [ submodule_closure_single],
 simp,
end
...
lemma submodule_eq_span_le_iff_stable_ge (n₀ : ℕ) :
 ...
 ∀ n ≥ n₀, I • F.N n = F.N (n + 1) :=
begin
 rw [← submodule_span_single]; rw [ ← has_le.le.le_iff_eq]; rw [ submodule.span_le]; rw [ set.Union_subset_iff],
 ...
 { intros r hr m' hm',
 rw [add_comm]; rw [ ← monomial_smul_single],
...
begin
 ...
 { dsimp,
 rw [← submodule.span_Union]; rw [ ← submodule_span_single],
 congr' 1,
 ext,
 ...
 { intros n m e,
 rw [submodule.span_le]; rw [ set.Union₂_subset_iff],
 intros i hi,
 ...
 { rw [finset.coe_image]; rw [ ← submodule.span_span_of_tower R]; rw [ ← submodule.map_span]; rw [ hs], refl },
...
begin
 ...
 { simp },
 { rw [nat.succ_eq_one_add]; rw [ pow_add]; rw [ ← smul_smul]; rw [ pow_one]; rw [ ← eq],
