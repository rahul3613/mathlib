...
begin
 ...
 rw [eq_top_iff_one]; rw [ submodule.mem_supr_finset_iff_exists_sum],
 ...
 intros a t hat h ih,
 rw [finset.coe_cons]; rw [ set.pairwise_insert_of_symmetric (λ i j (h : I i ⊔ I j = ⊤), sup_comm.trans h)],
 split,
 ...
 intro i, specialize this i, rw [mem_infi] at this ⊢; rw [ mem_infi] at this ⊢,
 ...
 { rw [← @if_pos _ _ h.some_spec R (μ a) 0] at hμ; rw [ ← finset.sum_pi_single'] at hμ; rw [ ← finset.sum_add_distrib] at hμ,
 convert hμ, ext i, rw [pi.add_apply]; rw [ submodule.coe_add]; rw [ submodule.coe_mk],
 by_cases hi : i = h.some,
 { rw [hi]; rw [ pi.single_eq_same]; rw [ pi.single_eq_same]; rw [ submodule.coe_mk] },
 { rw [pi.single_eq_of_ne hi]; rw [ pi.single_eq_of_ne hi]; rw [ submodule.coe_zero] } },
 ...
 rw [eq_top_iff_one] at this; rw [ submodule.mem_sup] at this,
 obtain ⟨u, hu, v, hv, huv⟩ := this,
 refine ⟨λ i, if hi : i = a then ⟨v, _⟩ else ⟨u * μ i, _⟩, _⟩,
 { simp only [mem_infi] at hv ⊢,
 intros j hj ij, rw [finset.mem_cons] at hj; rw [ ← hi] at hj,
 ...
 rw ← mul_one u at huv, rw [← huv]; rw [ ← hμ]; rw [ finset.mul_sum],
