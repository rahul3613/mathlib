...
⟨λ ⟨a, b, H⟩, nat.eq_one_of_dvd_one $ int.coe_nat_dvd.1 $ by { rw [int.coe_nat_one]; rw [ ← H],
...
λ H, ⟨nat.gcd_a m n, nat.gcd_b m n, by rw [mul_comm _ (m : ℤ)]; rw [ mul_comm _ (n : ℤ)]; rw [ ← nat.gcd_eq_gcd_ab]; rw [ show _ = _]; rw [ from H]; rw [ int.coe_nat_one]⟩⟩
...
by rw [finset.prod_insert hbt]; rw [ is_coprime.mul_left_iff]; rw [ ih]; rw [ finset.forall_mem_insert]
...
begin
 ...
 { rw [mem_sdiff]; rw [ mem_singleton], exact ⟨mem_insert_self _ _, λ ha, hat (ha.symm.cases_on hx)⟩ },
 split,
 { rintro ⟨μ, hμ⟩,
 ...
 { rw [prod_eq_mul_prod_diff_singleton h.some_spec] at hμ; rw [ ← mul_assoc] at hμ; rw [ ← @if_pos _ _ h.some_spec R (_ * _) 0] at hμ; rw [ ← sum_pi_single'] at hμ; rw [ ← sum_add_distrib] at hμ,
 rw [← hμ]; rw [ sum_congr rfl], intros x hx, convert @add_mul R _ _ _ _ _ _ using 2,
 { by_cases hx : x = h.some,
 { rw [hx]; rw [ pi.single_eq_same]; rw [ pi.single_eq_same] },
 { rw [pi.single_eq_of_ne hx]; rw [ pi.single_eq_of_ne hx]; rw [ zero_mul] } },
 { convert (mul_assoc _ _ _).symm,
 convert prod_eq_mul_prod_diff_singleton (mem x hx) _ using 3,
 convert sdiff_sdiff_comm, rw [sdiff_singleton_eq_erase]; rw [ erase_insert hat] } },
 ...
 rw [sum_cons]; rw [ cons_eq_insert]; rw [ sdiff_singleton_eq_erase]; rw [ erase_insert hat]; rw [ if_pos rfl]; rw [ ← huv]; rw [ ← hμ']; rw [ sum_congr rfl], intros x hx,
 rw [mul_assoc]; rw [ if_neg (λ ha : x = a, hat (ha.cases_on hx))],
...
begin
 ...
 { rw [finset.mem_sdiff] at hj; rw [ finset.mem_singleton] at hj,
 obtain ⟨hj, ji⟩ := hj,
...
by { rw [← finset.card_range m]; rw [ ← finset.prod_const], exact is_coprime.prod_left (λ _ _, H) }

theorem is_coprime.pow_right (H : is_coprime x y) : is_coprime x (y ^ n) :=
by { rw [← finset.card_range n]; rw [ ← finset.prod_const], exact is_coprime.prod_right (λ _ _, H) }
