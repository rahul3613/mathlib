...
begin
 ...
 rw bUnion_subset,
 intros x hx,
 rw [rename_monomial]; rw [ vars_monomial]; rw [ finsupp.map_domain_single],
...
begin
 ...
 rw [witt_poly_prod]; rw [ witt_polynomial]; rw [ alg_hom.map_sum]; rw [ alg_hom.map_sum]; rw [ sum_range_succ],
...
begin
 rw [← add_sub_assoc]; rw [ eq_sub_iff_add_eq]; rw [ mul_poly_of_interest_aux2],
...
begin
 ...
 { simp only [int.cast_coe_nat, eq_int_cast, C_pow, eq_self_iff_true] },
 rw [poly_of_interest]; rw [ this]; rw [ vars_C_mul],
 apply pow_ne_zero,
 exact_mod_cast hp.out.ne_zero
end
...
begin
 ...
 { rw [eq_int_cast]; rw [ int.cast_coe_nat] },
 have : ∀ (f : ℤ →+* k) (g : ℕ → k), eval₂ f g p = f p,
 { intros, rw [mvpz]; rw [ mv_polynomial.eval₂_C] },
