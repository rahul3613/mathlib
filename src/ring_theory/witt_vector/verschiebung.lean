...
by rw [verschiebung_fun]; rw [ coeff_mk]

lemma verschiebung_fun_coeff_zero (x : 𝕎 R) :
 (verschiebung_fun x).coeff 0 = 0 :=
by rw [verschiebung_fun_coeff]; rw [ if_pos rfl]
...
by rw [ghost_component_apply]; rw [ aeval_witt_polynomial]; rw [ finset.range_one]; rw [ finset.sum_singleton]; rw [ verschiebung_fun_coeff_zero]; rw [ pow_zero]; rw [ pow_zero]; rw [ pow_one]; rw [ one_mul]
...
begin
 ...
 rw [finset.sum_range_succ']; rw [ verschiebung_fun_coeff]; rw [ if_pos rfl]; rw [ zero_pow (pow_pos hp.1.pos _)]; rw [ mul_zero]; rw [ add_zero]; rw [ finset.mul_sum]; rw [ finset.sum_congr rfl],
 rintro i -,
...
begin
 ...
 { rw [verschiebung_poly]; rw [ verschiebung_fun_coeff_succ]; rw [ if_neg (n.succ_ne_zero)]; rw [ aeval_X]; rw [ nat.succ_eq_add_one]; rw [ add_tsub_cancel_right] }
end
...
begin
 ...
 rw [nat.succ_eq_add_one]; rw [ add_tsub_cancel_right]; rw [ ring_hom.map_mul]; rw [ map_nat_cast]; rw [ hom_bind₁],
 calc _
