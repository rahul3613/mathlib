...
lemma verschiebung_zmod (x : 𝕎 (zmod p)) :
 verschiebung x = x * p :=
by rw [← frobenius_verschiebung]; rw [ frobenius_zmodp]
...
begin
 ...
 { rw [pow_succ']; rw [ ← frobenius_verschiebung]; rw [ coeff_frobenius_char_p]; rw [ verschiebung_coeff_succ]; rw [ h]; rw [ one_pow], }
end
...
begin
 ...
 { rw [pow_succ']; rw [ ← frobenius_verschiebung]; rw [ coeff_frobenius_char_p],
 cases j,
 { rw [verschiebung_coeff_zero]; rw [ zero_pow],
 exact nat.prime.pos hp.out },
 { rw [verschiebung_coeff_succ]; rw [ hi]; rw [ zero_pow],
 { exact nat.prime.pos hp.out },
...
by { rw [coeff_p]; rw [ if_neg], exact zero_ne_one }

@[simp] lemma coeff_p_one [char_p R p] : (p : 𝕎 R).coeff 1 = 1 :=
by rw [coeff_p]; rw [ if_pos rfl]
...
begin
 rw [← frobenius_verschiebung]; rw [ coeff_frobenius_char_p]; rw [ verschiebung_coeff_zero]; rw [ zero_pow],
 exact nat.prime.pos hp.out
end

lemma mul_char_p_coeff_succ [char_p R p] (x : 𝕎 R) (i : ℕ) :
 (x * p).coeff (i + 1) = (x.coeff i)^p :=
by rw [← frobenius_verschiebung]; rw [ coeff_frobenius_char_p]; rw [ verschiebung_coeff_succ]

lemma verschiebung_frobenius [char_p R p] (x : 𝕎 R) :
 verschiebung (frobenius x) = x * p :=
begin
 ext ⟨i⟩,
 { rw [mul_char_p_coeff_zero]; rw [ verschiebung_coeff_zero], },
 { rw [mul_char_p_coeff_succ]; rw [ verschiebung_coeff_succ]; rw [ coeff_frobenius_char_p], }
end

lemma verschiebung_frobenius_comm [char_p R p] :
 function.commute (verschiebung : 𝕎 R → 𝕎 R) frobenius :=
λ x, by rw [verschiebung_frobenius]; rw [ frobenius_verschiebung]
...
begin
 ...
 { simp },
 { rw [iterate_succ_apply']; rw [ nat.add_succ]; rw [ verschiebung_coeff_succ],
 exact ih }
end
...
begin
 ...
 { simp },
 { rw [iterate_succ_apply']; rw [ ← verschiebung_mul_frobenius]; rw [ ih]; rw [ iterate_succ_apply'], refl }
end
...
begin
 ...
 { simp },
 { rw [iterate_succ_apply']; rw [ coeff_frobenius_char_p]; rw [ ih],
 ring_exp }
end
