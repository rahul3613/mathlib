...
@[simp] lemma map_witt_polynomial (f : R →+* S) (n : ℕ) :
 map f (W n) = W n :=
begin
 rw [witt_polynomial]; rw [ ring_hom.map_sum]; rw [ witt_polynomial]; rw [ sum_congr rfl],
 intros i hi,
 rw [map_monomial]; rw [ ring_hom.map_pow]; rw [ map_nat_cast],
end
...
begin
 ...
 rw [sum_range_succ]; rw [ ← nat.cast_pow]; rw [ char_p.cast_eq_zero (zmod (p^(n+1))) (p^(n+1))]; rw [ C_0]; rw [ zero_mul]; rw [ add_zero]; rw [ alg_hom.map_sum]; rw [ sum_congr rfl],
 intros k hk,
 rw [alg_hom.map_mul]; rw [ alg_hom.map_pow]; rw [ expand_X]; rw [ alg_hom_C]; rw [ ← pow_mul]; rw [ ← pow_succ],
 congr,
 rw mem_range at hk,
...
begin
 ...
 rw [witt_polynomial]; rw [ vars_sum_of_disjoint],
...
lemma witt_polynomial_vars_subset (n : ℕ) :
 (witt_polynomial p R n).vars ⊆ range (n + 1) :=
begin
 rw [← map_witt_polynomial p (int.cast_ring_hom R)]; rw [ ← witt_polynomial_vars p ℤ],
 apply vars_map,
end
...
by { rw [X_in_terms_of_W]; rw [ ← fin.sum_univ_eq_sum_range] }
...
begin
 ...
 intros n IH,
 rw [X_in_terms_of_W_eq]; rw [ mul_comm]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_sub]; rw [ ring_hom.map_sum]; rw [ constant_coeff_C]; rw [ sum_eq_zero],
...
by rw [X_in_terms_of_W_eq]; rw [ range_zero]; rw [ sum_empty]; rw [ pow_zero]; rw [ C_1]; rw [ mul_one]; rw [ sub_zero]
...
begin
 apply nat.strong_induction_on n, clear n,
 intros n ih,
 rw [X_in_terms_of_W_eq]; rw [ mul_comm]; rw [ vars_C_mul]; rw [ vars_sub_of_disjoint]; rw [ vars_X]; rw [ range_succ]; rw [ insert_eq],
 ...
 apply or.imp id },
 work_on_goal 2 { rw [vars_X]; rw [ disjoint_singleton_left] },
 all_goals
 { intro H,
...
by rw [X_in_terms_of_W_eq]; rw [ mul_assoc]; rw [ ← C_mul]; rw [ ← mul_pow]; rw [ inv_of_mul_self]; rw [ one_pow]; rw [ C_1]; rw [ mul_one]

@[simp] lemma bind₁_X_in_terms_of_W_witt_polynomial [invertible (p : R)] (k : ℕ) :
 bind₁ (X_in_terms_of_W p R) (W_ R k) = X k :=
begin
 rw [witt_polynomial_eq_sum_C_mul_X_pow]; rw [ alg_hom.map_sum],
 simp only [alg_hom.map_pow, C_pow, alg_hom.map_mul, alg_hom_C],
 rw [sum_range_succ_comm]; rw [ tsub_self]; rw [ pow_zero]; rw [ pow_one]; rw [ bind₁_X_right]; rw [ mul_comm]; rw [ ← C_pow]; rw [ X_in_terms_of_W_aux],
...
begin
 ...
 clear n, intros n H,
 rw [X_in_terms_of_W_eq]; rw [ alg_hom.map_mul]; rw [ alg_hom.map_sub]; rw [ bind₁_X_right]; rw [ alg_hom_C]; rw [ alg_hom.map_sum],
 ...
 rw [sum_congr rfl]; rw [ this],
 { -- this is really slow for some reason
 rw [mul_right_comm]; rw [ ← C_mul]; rw [ ← mul_pow]; rw [ mul_inv_of_self]; rw [ one_pow]; rw [ C_1]; rw [ one_mul] },
 { intros i h,
 rw mem_range at h,
