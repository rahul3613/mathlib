...
{ to_fun := λ f,
 ...
 begin
 rw [←multiset.map_map]; rw [ ←add_monoid_hom.map_multiset_sum],
...
ring_hom.ext $ λ x, free_comm_ring.induction_on x
 (by rw [ring_hom.map_neg]; rw [ ring_hom.map_one]; rw [ f.map_neg]; rw [ f.map_one])
 (lift_of _)
 (λ x y ihx ihy, by rw [ring_hom.map_add]; rw [ f.map_add]; rw [ ihx]; rw [ ihy])
 (λ x y ihx ihy, by rw [ring_hom.map_mul]; rw [ f.map_mul]; rw [ ihx]; rw [ ihy])
...
int.induction_on i is_supported_zero
 (λ i hi, by rw [int.cast_add]; rw [ int.cast_one]; exact is_supported_add hi is_supported_one)
 (λ i hi, by rw [int.cast_sub]; rw [ int.cast_one]; exact is_supported_sub hi is_supported_one)
...
assume hps : is_supported (of p) s, begin
 ...
 { use -1, rw [ring_hom.map_neg]; rw [ ring_hom.map_one]; rw [ int.cast_neg]; rw [ int.cast_one] },
 { rintros _ ⟨z, hzs, rfl⟩ _ _, use 0, rw [ring_hom.map_mul]; rw [ lift_of]; rw [ if_pos hzs]; rw [ zero_mul],
 norm_cast },
 { rintros x y ⟨q, hq⟩ ⟨r, hr⟩, refine ⟨q+r, _⟩, rw [ring_hom.map_add]; rw [ hq]; rw [ hr], norm_cast } },
 ...
 rwa [polynomial.coeff_C] at this; rwa [ if_neg (one_ne_zero : 1 ≠ 0)] at this; rwa [ polynomial.coeff_X] at this; rwa [ if_pos rfl] at this
end
...
begin
 ...
 { rw [ring_hom.map_neg]; rw [ ring_hom.map_neg]; rw [ ring_hom.map_one], refl },
 { rintros _ ⟨p, hps, rfl⟩ n ih,
 rw [ring_hom.map_mul]; rw [ restriction_of]; rw [ dif_pos hps]; rw [ ring_hom.map_mul]; rw [ map_of]; rw [ ih] },
 { intros x y ihx ihy, rw [ring_hom.map_add]; rw [ ring_hom.map_add]; rw [ ihx]; rw [ ihy] }
end
...
(λ hd tl ih, by { rw [(free_monoid.lift _).map_mul]; rw [ free_monoid.lift_eval_of]; rw [ ih], refl }) }
...
{ mul_comm := λ x y,
 by { rw [← (subsingleton_equiv_free_comm_ring α).symm_apply_apply (y * x)]; rw [ ((subsingleton_equiv_free_comm_ring α)).map_mul]; rw [ mul_comm]; rw [ ← ((subsingleton_equiv_free_comm_ring α)).map_mul]; rw [ (subsingleton_equiv_free_comm_ring α).symm_apply_apply], },
 .. free_ring.ring α }
