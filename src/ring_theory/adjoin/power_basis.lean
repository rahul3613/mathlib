...
begin
 ...
 { rw [← @aeval_X_pow R _ _ _ _ B.gen]; rw [ ← mod_by_monic_add_div (X ^ n) (minpoly.monic hB)],
 simp },
 by_cases hQ : Q = 0,
 { simp [this, hQ, is_integral_zero] },
 have hlt : Q.nat_degree < B.dim,
 { rw [← B.nat_degree_minpoly]; rw [ hmin]; rw [ (minpoly.monic hB).nat_degree_map]; rw [ nat_degree_lt_nat_degree_iff hQ],
 ...
 rw [← fin.coe_mk hj]; rw [ ← B.basis_eq_pow]; rw [ algebra.smul_def]; rw [ is_scalar_tower.algebra_map_apply R S A]; rw [ ← algebra.smul_def]; rw [ linear_equiv.map_smul],
...
begin
 intro i,
 rw [← B.basis.sum_repr x]; rw [ ← B.basis.sum_repr y]; rw [ finset.sum_mul_sum]; rw [ linear_equiv.map_sum]; rw [ finset.sum_apply'],
...
begin
 ...
 { intros hx i,
 rw [pow_zero]; rw [ ← pow_zero B.gen]; rw [ ← fin.coe_mk B.dim_pos]; rw [ ← B.basis_eq_pow]; rw [ B.basis.repr_self_apply],
 split_ifs,
...
begin
 intros i j,
 rw [B.basis.to_matrix_apply]; rw [ B'.coe_basis],
 refine repr_pow_is_integral hB (λ i, _) hmin _ _,
 rw [← h]; rw [ aeval_eq_sum_range]; rw [ linear_equiv.map_sum]; rw [ finset.sum_apply'],
 refine is_integral.sum _ (λ n hn, _),
 rw [algebra.smul_def]; rw [ is_scalar_tower.algebra_map_apply R K S]; rw [ ← algebra.smul_def]; rw [ linear_equiv.map_smul]; rw [ algebra_map_smul],
