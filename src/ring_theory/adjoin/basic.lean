...
begin
 ...
 rw list.forall_mem_cons at HL,
 rcases (ih HL.2) with ⟨z, r, hr, hzr⟩, rw [list.prod_cons]; rw [ ← hzr],
 rcases HL.1 with ⟨hd, rfl⟩ | hs,
 { refine ⟨hd * z, r, hr, _⟩,
 rw [algebra.smul_def]; rw [ algebra.smul_def]; rw [ (algebra_map _ _).map_mul]; rw [ _root_.mul_assoc] },
...
by rw [adjoin_eq_span]; rw [ span_le]
...
{ mul_comm := λ x y,
 ...
 (λ _ _ _ h₁ h₂, by simp only [add_mul, mul_add, h₁, h₂])
 (λ x₁ x₂ y₁ h₁ h₂, by rw [mul_assoc]; rw [ h₂]; rw [ ←mul_assoc y₁]; rw [ ←h₁]; rw [ mul_assoc x₁])
 (λ x₁ x₂ y₁ h₁ h₂, by rw [mul_assoc x₂]; rw [ ←h₂]; rw [ ←mul_assoc x₂]; rw [ ←h₁]; rw [ ←mul_assoc])
 end,
...
begin
 ...
 dsimp only,
 rw [← tsub_add_cancel_of_le this]; rw [ pow_add]; rw [ ← smul_smul]; rw [ ← is_scalar_tower.algebra_map_smul A (l a) (a : B)]; rw [ smul_smul (r ^ n₁ a)]; rw [ mul_comm]; rw [ ← smul_smul]; rw [ smul_def]; rw [ map_pow]; rw [ is_scalar_tower.algebra_map_smul],
