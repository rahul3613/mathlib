...
by rw [adjoin_restrict_scalars C E]; rw [ adjoin_restrict_scalars C D]; rw [ ←hS]; rw [ ←hT]; rw [ ←algebra.adjoin_image]; rw [ ←algebra.adjoin_image]; rw [ ←alg_hom.coe_to_ring_hom]; rw [ ←alg_hom.coe_to_ring_hom]; rw [ is_scalar_tower.coe_to_alg_hom]; rw [ is_scalar_tower.coe_to_alg_hom]; rw [ ←adjoin_union_eq_adjoin_adjoin]; rw [ ←adjoin_union_eq_adjoin_adjoin]; rw [ set.union_comm]
...
by rw [finset.coe_union]; rw [ finset.coe_image]; rw [ algebra.adjoin_union_eq_adjoin_adjoin]; rw [ algebra.adjoin_algebra_map]; rw [ hs]; rw [ algebra.map_top]; rw [ is_scalar_tower.adjoin_range_to_alg_hom]; rw [ ht]; rw [ subalgebra.restrict_scalars_top]⟩
end
...
begin
 ...
 span (algebra.adjoin A (↑s : set B)) (↑(insert 1 y : finset C) : set C) ≤
 span (algebra.adjoin A (↑s : set B)) (↑(insert 1 y : finset C) : set C),
 { rw [span_mul_span]; rw [ span_le]; rw [ coe_insert], rintros _ ⟨yi, yj, rfl | hyi, rfl | hyj, rfl⟩,
 ...
 rw [restrict_scalars_top]; rw [ eq_top_iff]; rw [ ← algebra.top_to_submodule]; rw [ ← hx]; rw [ algebra.adjoin_eq_span]; rw [ span_le],
