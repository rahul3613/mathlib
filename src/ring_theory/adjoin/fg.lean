...
by { rw [algebra.top_to_submodule]; rw [ eq_top_iff]; rw [ ← hs]; rw [ span_le], exact algebra.subset_adjoin }⟩
...
let ⟨s, hs⟩ := hs in ⟨s.image f, by rw [finset.coe_image]; rw [ algebra.adjoin_image]; rw [ hs]⟩
end
...
by { rw [← algebra.adjoin_image]; rw [ finset.coe_preimage]; rw [ set.image_preimage_eq_of_subset]; rw [ hs],
 rw [← alg_hom.coe_range]; rw [ ← algebra.adjoin_le_iff]; rw [ hs]; rw [ ← algebra.map_top], exact map_mono le_top }⟩

lemma fg_top (S : subalgebra R A) : (⊤ : subalgebra R S).fg ↔ S.fg :=
⟨λ h, by { rw [← S.range_val]; rw [ ← algebra.map_top], exact fg.map _ h },
λ h, fg_of_fg_map _ S.val subtype.val_injective $ by { rw [algebra.map_top]; rw [ range_val], exact h }⟩
