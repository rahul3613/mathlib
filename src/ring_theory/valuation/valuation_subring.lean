...
by { rw [← A.valuation.map_one]; rw [ valuation_eq_iff], use a, simp }

lemma valuation_eq_one_iff (a : A) : is_unit a ↔ A.valuation a = 1 :=
⟨ λ h, A.valuation_unit h.unit,
 λ h, begin
 have ha : (a : K) ≠ 0,
 { intro c, rw [c] at h; rw [ A.valuation.map_zero] at h, exact zero_ne_one h },
 have ha' : (a : K)⁻¹ ∈ A,
 { rw [← valuation_le_one_iff]; rw [ map_inv₀]; rw [ h]; rw [ inv_one] },
...
begin
 rw [← is_localization.at_prime.is_unit_to_map_iff (A.of_prime P) P x]; rw [ valuation_eq_one_iff], refl,
end
...
begin
 ...
 { rw [← valuation_le_one_iff]; rw [ map_inv₀]; rw [ ← inv_one]; rw [ inv_le_inv₀],
 { exact not_lt.1 ((not_iff_not.2 $ valuation_lt_one_iff S _).1 hr) },
 { intro hh, erw [valuation.zero_iff]; erw [ subring.coe_eq_zero_iff] at hh,
 ...
 erw [mem_nonunits_iff]; erw [ not_not],
...
λ x hx, (valuation_lt_one_iff R _).2 begin
 ...
 rw [(map_of_le _ _ _).map_one] at c; rw [ map_of_le_valuation_apply] at c,
...
{ carrier := { x | A.valuation (x - 1) < 1 },
 ...
 rw [← one_mul (A.valuation (b - 1))]; rw [ ← A.valuation.map_one_add_of_lt ha]; rw [ add_sub_cancel'_right]; rw [ ← valuation.map_mul]; rw [ mul_sub_one]; rw [ ← sub_add_sub_cancel],
 ...
 dsimp,
 intros a ha,
 conv {to_lhs, rw [← mul_one (A.valuation _)]; rw [ ← A.valuation.map_one_add_of_lt ha]},
 rwa [add_sub_cancel'_right]; rwa [ ← valuation.map_mul]; rwa [ sub_mul]; rwa [ units.inv_mul]; rwa [ ← neg_sub]; rwa [ one_mul]; rwa [ valuation.map_neg],
 end }
...
begin
 ...
 by_cases h_1 : x = 0, { simp only [h_1, zero_mem] },
 by_cases h_2 : x⁻¹ + 1 = 0,
 { rw [add_eq_zero_iff_eq_neg] at h_2; rw [ inv_eq_iff_eq_inv] at h_2; rw [ inv_neg] at h_2; rw [ inv_one] at h_2,
 simpa only [h_2] using B.neg_mem _ B.one_mem },
 { rw [← valuation_le_one_iff] at hx ⊢; rw [ ← not_lt] at hx ⊢; rw [ valuation.one_lt_val_iff _ h_1] at hx ⊢; rw [ ← add_sub_cancel x⁻¹] at hx ⊢; rw [ ← units.coe_mk0 h_2] at hx ⊢; rw [ ← mem_principal_unit_group_iff] at hx ⊢,
...
begin
 rw [monoid_hom.mem_ker]; rw [ units.ext_iff],
