...
by { rw [← v.map_one]; rw [ ← (algebra_map O R).map_one]; rw [ ← u.mul_inv]; rw [ ← mul_one (v (algebra_map O R x))]; rw [ hu]; rw [ (algebra_map O R).map_mul]; rw [ v.map_mul], exact mul_le_mul_left' (hv.2 (u⁻¹ : units O)) _ }
...
have h2 : v (u⁻¹ : Rˣ) ≤ 1,
 by rw [← one_mul (v _)]; rw [ ← hvx]; rw [ ← v.map_mul]; rw [ ← hu]; rw [ u.mul_inv]; rw [ hu]; rw [ hvx]; rw [ v.map_one],
let ⟨r1, hr1⟩ := hv.3 h1, ⟨r2, hr2⟩ := hv.3 h2 in
⟨⟨r1, r2, hv.1 $ by rw [ring_hom.map_mul]; rw [ ring_hom.map_one]; rw [ hr1]; rw [ hr2]; rw [ units.mul_inv],
 hv.1 $ by rw [ring_hom.map_mul]; rw [ ring_hom.map_one]; rw [ hr1]; rw [ hr2]; rw [ units.inv_mul]⟩,
hv.1 $ hr1.trans hu⟩

lemma le_of_dvd {x y : O} (h : x ∣ y) : v (algebra_map O R y) ≤ v (algebra_map O R x) :=
let ⟨z, hz⟩ := h in by { rw [← mul_one (v (algebra_map O R x))]; rw [ hz]; rw [ ring_hom.map_mul]; rw [ v.map_mul],
...
have v ((algebra_map O F y)⁻¹ * algebra_map O F x) ≤ 1,
 by { rw [← v.map_one]; rw [ ← inv_mul_cancel hy]; rw [ v.map_mul]; rw [ v.map_mul], exact mul_le_mul_left' h _ },
