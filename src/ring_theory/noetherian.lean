...
by rw [is_noetherian_iff_well_founded]; rw [ well_founded.well_founded_iff_has_min]

/-- A module is Noetherian iff every increasing chain of submodules stabilizes. -/
theorem monotone_stabilizes_iff_noetherian :
 (∀ (f : ℕ →o submodule R M), ∃ n, ∀ m, n ≤ m → f n = f m) ↔ is_noetherian R M :=
by rw [is_noetherian_iff_well_founded]; rw [ well_founded.monotone_chain_condition]
...
begin
 ...
 rw eq_bot_iff,
 rintros - ⟨h, ⟨y, rfl⟩⟩,
 rw [mem_bot]; rw [ ←linear_map.mem_ker]; rw [ w],
...
begin
 ...
 rw [linear_map.range_eq_top.mpr (linear_map.iterate_surjective s n)] at w; rw [ inf_top_eq] at w; rw [ linear_map.ker_eq_bot] at w,
...
instance is_noetherian_of_finite (R M) [finite M] [semiring R] [add_comm_monoid M] [module R M] :
 is_noetherian R M :=
⟨λ s, ⟨(s : set M).to_finite.to_finset, by rw [set.finite.coe_to_finset]; rw [ submodule.span_eq]⟩⟩
