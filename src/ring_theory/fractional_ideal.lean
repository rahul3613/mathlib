...
by rw [coe_one_eq_coe_submodule_top]; rw [ coe_submodule_top]
...
begin
 ...
 { intros a b ha hb,
 rw [mem_set_of_eq]; rw [ ring_hom.map_add],
 exact J.val.add_mem ha hb },
 { rw [mem_set_of_eq]; rw [ ring_hom.map_zero],
 exact J.val.zero_mem },
 { intros c x hx,
 rw [smul_eq_mul]; rw [ mem_set_of_eq]; rw [ ring_hom.map_mul]; rw [ ← algebra.smul_def],
...
@[simp] lemma one_le {I : fractional_ideal S P} :
 1 ≤ I ↔ (1 : P) ∈ I :=
by rw [← coe_le_coe]; rw [ coe_one]; rw [ submodule.one_le]; rw [ mem_coe]
...
{ to_fun := coe,
 ...
 map_one' := by rw [ideal.one_eq_top]; rw [ coe_ideal_top],
...
| ⟨a, a_nonzero, hI⟩ := ⟨a, a_nonzero, λ b hb, begin
 ...
 use x,
 erw [←g.commutes]; erw [ hx]; erw [ g.map_smul]; erw [ hb']
end⟩
...
{ to_fun := map g,
 ...
 left_inv := λ I, by { rw [←map_comp]; rw [ alg_equiv.symm_comp]; rw [ map_id] },
 right_inv := λ I, by { rw [←map_comp]; rw [ alg_equiv.comp_symm]; rw [ map_id] } }
...
map_equiv
 ...
 (show S.map _ = S, by rw [ring_equiv.to_monoid_hom_refl]; rw [ submonoid.map_id]) }
...
begin
 rw [canonical_equiv]; rw [ map_equiv_apply]; rw [ mem_map],
...
by { rw [mem_canonical_equiv_apply]; rw [ canonical_equiv]; rw [ map_equiv_symm]; rw [ map_equiv]; rw [ ring_equiv.coe_mk]; rw [ mem_map],
 exact ⟨λ ⟨y, mem, eq⟩, ⟨y, mem, eq⟩, λ ⟨y, mem, eq⟩, ⟨y, mem, eq⟩⟩ }

lemma canonical_equiv_flip (I) :
 canonical_equiv S P P' (canonical_equiv S P' P I) = I :=
by rw [←canonical_equiv_symm]; rw [ ring_equiv.symm_apply_apply]
...
begin
 ...
 refine ⟨x, _, _⟩,
 { rw [ne.def]; rw [ ← @is_fraction_ring.to_map_eq_zero_iff R _ K]; rw [ hx]; rw [ algebra.smul_def],
...
| ⟨aI, haI, hI⟩ ⟨aJ, haJ, hJ⟩ h := begin
 ...
 { rw [← algebra.smul_def]; rw [ ←hy']; rw [ y'_eq_zero]; rw [ ring_hom.map_zero] },
...
begin
 by_cases hI : I = 0,
 { rw [hI]; rw [ div_zero]; rw [ mul_zero],
 exact zero_le 1 },
 { rw [← coe_le_coe]; rw [ coe_mul]; rw [ coe_div hI]; rw [ coe_one],
...
begin
 by_cases hI_nz : I = 0,
 { rw [hI_nz]; rw [ div_zero]; rw [ mul_zero], exact zero_le 0 },
 { rw [← coe_le_coe]; rw [ coe_mul]; rw [ coe_div hI_nz]; rw [ coe_one],
...
begin
 by_cases H : J = 0,
 { rw [H]; rw [ div_zero]; rw [ map_zero]; rw [ div_zero] },
...
begin
 ...
 refine ⟨n / d, _⟩,
 rw [map_div₀]; rw [ is_fraction_ring.mk'_eq_div] },
 { rintro ⟨x, rfl⟩,
 obtain ⟨y, y_ne, y_mem⟩ := exists_ne_zero_mem_is_integer hI,
 rw [← div_mul_cancel x y_ne]; rw [ ring_hom.map_mul]; rw [ ← algebra.smul_def],
...
by rw [← coe_le_coe]; rw [ coe_span_singleton]; rw [ submodule.span_singleton_le_iff_mem x ↑I]; rw [ mem_coe]

lemma span_singleton_eq_span_singleton [no_zero_smul_divisors R P] {x y : P} :
 span_singleton S x = span_singleton S y ↔ ∃ z : Rˣ, z • x = y :=
by { rw [← submodule.span_singleton_eq_span_singleton]; rw [ span_singleton]; rw [ span_singleton],
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero]; rw [ span_singleton_one] },
 { rw [pow_succ]; rw [ hn]; rw [ span_singleton_mul_span_singleton]; rw [ pow_succ] }
end
...
begin
 ...
 use x',
 rw [smul_eq_mul]; rw [ ring_hom.map_mul]; rw [ algebra.smul_def] },
 { rintros ⟨y', rfl⟩,
 refine ⟨y' * x, submodule.mem_span_singleton.mpr ⟨y', rfl⟩, _⟩,
 rw [ring_hom.map_mul]; rw [ algebra.smul_def] }
end
...
begin
 ...
 { rw [span_singleton_mul_span_singleton]; rw [ mul_comm]; rw [ ← is_localization.mk'_eq_mul_mk'_one]; rw [ is_localization.mk'_self]; rw [ span_singleton_one] },
 ...
 rw [coe_y']; rw [ coe_ideal_mul]; rw [ coe_ideal_span_singleton]; rw [ coe_ideal_mul]; rw [ coe_ideal_span_singleton]; rw [ ←mul_assoc]; rw [ span_singleton_mul_span_singleton]; rw [ ←mul_assoc]; rw [ span_singleton_mul_span_singleton]; rw [ mul_comm (mk' _ _ _)]; rw [ ← is_localization.mk'_eq_mul_mk'_one]; rw [ mul_comm (mk' _ _ _)]; rw [ ← is_localization.mk'_eq_mul_mk'_one]; rw [ is_localization.mk'_self]; rw [ span_singleton_one]; rw [ one_mul],
end
...
by erw [← mk'_mul_coe_ideal_eq_coe_ideal K (is_localization.sec R₁⁰ z).2.prop]; erw [ is_localization.mk'_sec K z]
...
begin
 ...
 apply le_antisymm,
 { intros x hx,
 rw [← mem_coe] at hx; rw [ coe_div h_spand] at hx; rw [ submodule.mem_div_iff_forall_mul_mem] at hx,
 specialize hx d (mem_span_singleton_self R₁⁰ d),
 have h_xd : x = d⁻¹ * (x * d), { field_simp },
 rw [← mem_coe]; rw [ coe_mul]; rw [ one_div_span_singleton]; rw [ h_xd],
 exact submodule.mul_mem_mul (mem_span_singleton_self R₁⁰ _) hx },
 { rw [le_div_iff_mul_le h_spand]; rw [ mul_assoc]; rw [ mul_left_comm]; rw [ one_div_span_singleton]; rw [ span_singleton_mul_span_singleton]; rw [ inv_mul_cancel hd]; rw [ span_singleton_one]; rw [ mul_one],
 exact le_refl J },
end
...
begin
 ...
 { exact ⟨x, hx, hx'⟩ },
 { rw [hx']; rw [ ← mul_assoc]; rw [ inv_mul_cancel map_a_nonzero]; rw [ one_mul] } },
 { rintros ⟨y, hy, rfl⟩,
 obtain ⟨x', hx', rfl⟩ := (mem_coe_ideal _).mp hy,
 obtain ⟨y', hy', hx'⟩ := mem_singleton_mul.mp hx',
 rw algebra.linear_map_apply at hx',
 rwa [hx']; rwa [ ←mul_assoc]; rwa [ inv_mul_cancel map_a_nonzero]; rwa [ one_mul] }
end
...
begin
 ...
 conv_lhs { rw [ha]; rw [ ←span_singleton_generator aI] },
 rw [ideal.submodule_span_eq]; rw [ coe_ideal_span_singleton (generator aI)]; rw [ span_singleton_mul_span_singleton]
end
...
begin
 by_cases hx : x = 0,
 { rw [hx]; rw [ ring_hom.map_zero]; rw [ _root_.inv_zero]; rw [ span_singleton_zero]; rw [ zero_mul],
 ...
 intros J hJ,
 rw [← div_span_singleton] at hJ; rw [ le_div_iff_mul_le h_spanx] at hJ,
 obtain ⟨s, hs⟩ := hI _ hJ,
 use s * {(algebra_map R₁ K x)⁻¹},
 rw [finset.coe_mul]; rw [ finset.coe_singleton]; rw [ ← span_mul_span]; rw [ hs]; rw [ ← coe_span_singleton R₁⁰]; rw [ ← coe_mul]; rw [ mul_assoc]; rw [ span_singleton_mul_span_singleton]; rw [ mul_inv_cancel h_gx]; rw [ span_singleton_one]; rw [ mul_one],
end
