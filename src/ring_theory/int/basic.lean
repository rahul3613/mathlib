...
show z * ↑(ite _ _ _) = z, by rw [if_pos h]; rw [ units.coe_one]; rw [ mul_one]
...
begin
 ...
 rw [if_neg (not_le_of_gt h)]; rw [ units.coe_neg]; rw [ units.coe_one]; rw [ mul_neg_one] }
end
...
abs_eq_self.1 $ by rw [abs_eq_normalize]; rw [ hz]
...
{ gcd := λa b, int.gcd a b,
 ...
 { rw [← int.coe_nat_mul]; rw [ gcd_mul_lcm]; rw [ coe_nat_abs]; rw [ abs_eq_normalize],
...
begin
 ...
 { use [1, is_unit_one], rw [← h]; rw [ one_mul], },
...
begin
 ...
 rw [mul_assoc]; rw [ ← ha]; rw [ mul_assoc]; rw [ ← hb]; rw [ mul_comm]; rw [ mul_comm _ (int.nat_abs b : ℤ)]; rw [ ← nat.gcd_eq_gcd_ab]; rw [ ←gcd_eq_nat_abs]; rw [ hg]; rw [ int.coe_nat_one] },
 { rintro ⟨r, s, h⟩,
 by_contradiction hg,
 obtain ⟨p, ⟨hp, ha, hb⟩⟩ := nat.prime.not_coprime_iff_dvd.mp hg,
 apply nat.prime.not_dvd_one hp,
 rw [←coe_nat_dvd]; rw [ int.coe_nat_one]; rw [ ← h],
...
by rw [←gcd_eq_one_iff_coprime]; rw [ nat.coprime_iff_gcd_eq_one]; rw [ gcd_eq_nat_abs]
...
lemma sq_of_gcd_eq_one {a b c : ℤ} (h : int.gcd a b = 1) (heq : a * b = c ^ 2) :
 ∃ (a0 : ℤ), a = a0 ^ 2 ∨ a = - (a0 ^ 2) :=
begin
 have h' : is_unit (gcd_monoid.gcd a b), { rw [← coe_gcd]; rw [ h]; rw [ int.coe_nat_one], exact is_unit_one },
...
begin
 ...
 rw [int.coe_nat_abs]; rw [ int.abs_eq_normalize]; rw [ normalize_idem] },
 { intro n,
 dsimp,
 rw [←normalize_apply]; rw [ ←int.abs_eq_normalize]; rw [ int.nat_abs_abs]; rw [ int.nat_abs_of_nat] }
end
...
begin
 ...
 rw [sq] at hpp; rw [ int.nat_abs_mul] at hpp,
...
begin
 ...
 { rw [multiset.coe_prod]; rw [ nat.prod_factors n.succ_ne_zero],
...
by rw [finite_int_iff_nat_abs_finite]; rw [ finite_nat_iff]; rw [ pos_iff_ne_zero]; rw [ int.nat_abs_ne_zero]
...
begin
 rw [←dvd_dvd_iff_associated]; rw [ ←int.nat_abs_dvd_iff_dvd]; rw [ ←int.nat_abs_dvd_iff_dvd]; rw [ dvd_dvd_iff_associated],
...
begin
 ...
 replace hd := hd.symm,
 rw [associated_iff_nat_abs] at hd; rw [ nat_abs_eq_nat_abs_iff] at hd; rw [ ←neg_pow_bit1] at hd,
