...
begin
 rw [ne.def] at hp; rw [ ← weighted_total_degree'_eq_bot_iff w p] at hp; rw [ ← ne.def] at hp; rw [ with_bot.ne_bot_iff_exists] at hp,
 obtain ⟨m, hm⟩ := hp,
 apply le_antisymm,
...
begin
 ...
 rw [eq_bot_iff]; rw [ ← with_bot.coe_le_coe]; rw [ ← h],
...
begin
 ...
 intros d hd,
 contrapose! hd,
 rw [coeff_weighted_homogeneous_component]; rw [ if_neg hd]
end
...
begin
 classical,
 rw [weighted_homogeneous_component_apply]; rw [ sum_eq_zero],
...
begin
 ...
 { rw [coeff_weighted_homogeneous_component]; rw [ if_neg]; rw [ coeff_C]; rw [ if_neg (ne.symm hd)],
