...
lemma is_homogeneous_of_total_degree_zero {p : mv_polynomial σ R} (hp : p.total_degree = 0) :
 is_homogeneous p 0 :=
begin
 erw [total_degree]; erw [ finset.sup_eq_bot_iff] at hp,
...
begin
 intros d hd,
 contrapose! hd,
 rw [coeff_homogeneous_component]; rw [ if_neg hd]
end
...
begin
 ...
 { rw [coeff_homogeneous_component]; rw [ if_neg]; rw [ coeff_C]; rw [ if_neg (ne.symm hd)],
...
begin
 rw [homogeneous_component_apply]; rw [ sum_eq_zero],
