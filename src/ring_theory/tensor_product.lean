...
{ to_fun := f.ltensor A,
 ...
 by { rw [← comp_apply]; rw [ ← comp_apply],
...
{ smul_assoc := λ r x y, begin
 ...
 { intros a b a' b',
 dsimp,
 rw [tensor_product.smul_tmul']; rw [ tensor_product.smul_tmul']; rw [ tmul_mul_tmul]; rw [ smul_mul_assoc], },
...
{ smul_comm := λ r x y, begin
 ...
 { intros a b a' b',
 dsimp,
 rw [tensor_product.smul_tmul']; rw [ tensor_product.smul_tmul']; rw [ tmul_mul_tmul]; rw [ mul_smul_comm], },
 { intros, simp [smul_add, add_mul, *], } },
 { intros, simp [smul_add, mul_add, *], },
 end }
...
{ commutes' := λ r x,
 ...
 rw [algebra_map_eq_smul_one]; rw [ ←smul_tmul']; rw [ ←one_def]; rw [ mul_smul_comm]; rw [ smul_mul_assoc]; rw [ mul_one]; rw [ one_mul],
 end,
 smul_def' := λ r x,
 begin
 dsimp only [ring_hom.to_fun_eq_coe, ring_hom.comp_apply, include_left_ring_hom_apply],
 rw [algebra_map_eq_smul_one]; rw [ ←smul_tmul']; rw [ smul_mul_assoc]; rw [ ←one_def]; rw [ one_mul],
 end,
...
{ to_fun := λ b, 1 ⊗ₜ b,
 ...
 { rw [←tmul_smul]; rw [ algebra.smul_def], simp, },
...
def alg_hom_of_linear_map_tensor_product
 ...
 A ⊗[R] B →ₐ[R] C :=
{ map_one' := by rw [←(algebra_map R C).map_one]; rw [ ←w₂]; rw [ (algebra_map R A).map_one]; refl,
 map_zero' := by rw [linear_map.to_fun_eq_coe]; rw [ map_zero],
 map_mul' := λ x y, by
...
begin
 apply le_antisymm,
 { rw [←map_top]; rw [ ←adjoin_tmul_eq_top]; rw [ ←adjoin_image]; rw [ adjoin_le_iff],
 ...
 { rw [←map_comp_include_left f g]; rw [ ←map_comp_include_right f g],
...
by rw [product_map]; rw [ alg_hom.range_comp]; rw [ map_range]; rw [ map_sup]; rw [ ←alg_hom.range_comp]; rw [ ←alg_hom.range_comp]; rw [ ←alg_hom.comp_assoc]; rw [ ←alg_hom.comp_assoc]; rw [ lmul'_comp_include_left]; rw [ lmul'_comp_include_right]; rw [ alg_hom.id_comp]; rw [ alg_hom.id_comp]
...
begin
 rw [←E1.range_val]; rw [ ←E2.range_val]; rw [ ←algebra.tensor_product.product_map_range],
