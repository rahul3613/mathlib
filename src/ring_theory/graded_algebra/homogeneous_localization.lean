...
@[simp] lemma sub_val (y1 y2 : homogeneous_localization 𝒜 x) :
 (y1 - y2).val = y1.val - y2.val :=
by rw [show y1 - y2 = y1 + (-y2)]; rw [ from rfl]; rw [ add_val]; rw [ neg_val]; refl
...
⟨λ h1, begin
 ...
 rw [f.eq_num_div_denom] at eq1; rw [ localization.mk_mul] at eq1; rw [ show (⟨b, hb⟩ : 𝔭.prime_compl) * ⟨f.denom, _⟩ = ⟨b * f.denom, _⟩] at eq1; rw [ from rfl] at eq1; rw [ show (1 : localization.at_prime 𝔭) = localization.mk 1 1] at eq1; rw [ by erw localization.mk_self 1] at eq1; rw [ localization.mk_eq_mk'] at eq1; rw [ is_localization.eq] at eq1,
