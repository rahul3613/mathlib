...
begin
 classical,
 rw [←direct_sum.sum_support_decompose 𝒜 r]; rw [ finset.sum_mul]; rw [ map_sum],
 apply ideal.sum_mem,
 intros k hk,
 obtain ⟨i, hi⟩ := hx₁,
 have mem₁ : (direct_sum.decompose 𝒜 r k : A) * x ∈ 𝒜 (k + i) := graded_monoid.mul_mem
 (set_like.coe_mem _) hi,
 erw [graded_ring.proj_apply]; erw [ direct_sum.decompose_of_mem 𝒜 mem₁]; erw [ coe_of_apply]; erw [ set_like.coe_mk],
 split_ifs,
...
begin
 ...
 obtain ⟨s, rfl⟩ := hr,
 rw [finsupp.total_apply]; rw [ finsupp.sum]; rw [ decompose_sum]; rw [ dfinsupp.finset_sum_apply]; rw [ add_submonoid_class.coe_finset_sum],
...
begin
 rw [ideal.is_homogeneous.iff_eq]; rw [ eq_comm],
...
by rw [supr]; rw [ to_ideal_Sup]; rw [ supr_range]

@[simp] lemma to_ideal_infi {κ : Sort*} (s : κ → homogeneous_ideal 𝒜) :
 (⨅ i, s i).to_ideal = ⨅ i, (s i).to_ideal :=
by rw [infi]; rw [ to_ideal_Inf]; rw [ infi_range]
...
by { ext1, rw [ideal.to_ideal_homogeneous_hull_eq_supr]; rw [ to_ideal_supr], refl }
...
⟨(graded_ring.proj_zero_ring_hom 𝒜).ker, λ i r (hr : (decompose 𝒜 r 0 : A) = 0), begin
 ...
 by_cases h : i = 0,
 { rw [h]; rw [ hr]; rw [ decompose_zero]; rw [ zero_apply]; rw [ zero_mem_class.coe_zero] },
