...
by rw [graded_ring.proj_apply]; rw [ decompose_symm_of]; rw [ equiv.apply_symm_apply]
...
by { lift a to 𝒜 i using a_mem, rw [decompose_mul]; rw [ decompose_coe]; rw [ coe_of_mul_apply_add] }
...
by { lift b to 𝒜 j using b_mem, rw [decompose_mul]; rw [ decompose_coe]; rw [ coe_mul_of_apply_add] }
...
by rw [graded_algebra.proj_apply]; rw [ decompose_symm_of]; rw [ equiv.apply_symm_apply]
...
{ to_fun := λ a, decompose 𝒜 a 0,
 ...
 by_cases h : i + j = 0,
 { rw [decompose_of_mem_same 𝒜 (show c * c' ∈ 𝒜 0, from h ▸ mul_mem hc hc')]; rw [ decompose_of_mem_same 𝒜 (show c ∈ 𝒜 0, from (add_eq_zero_iff.mp h).1 ▸ hc)]; rw [ decompose_of_mem_same 𝒜 (show c' ∈ 𝒜 0, from (add_eq_zero_iff.mp h).2 ▸ hc')] },
 { rw [decompose_of_mem_ne 𝒜 (mul_mem hc hc') h],
 cases (show i ≠ 0 ∨ j ≠ 0, by rwa [add_eq_zero_iff] at h) with h' h'; rwa [ not_and_distrib] at h) with h' h',
...
by { lift a to 𝒜 i using a_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_of_mul_apply_of_not_le] }

lemma coe_decompose_mul_of_right_mem_of_not_le
 (b_mem : b ∈ 𝒜 i) (h : ¬ i ≤ n) : (decompose 𝒜 (a * b) n : A) = 0 :=
by { lift b to 𝒜 i using b_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_mul_of_apply_of_not_le] }
...
by { lift a to 𝒜 i using a_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_of_mul_apply_of_le] }

lemma coe_decompose_mul_of_right_mem_of_le
 (b_mem : b ∈ 𝒜 i) (h : i ≤ n) : (decompose 𝒜 (a * b) n : A) = decompose 𝒜 a (n - i) * b :=
by { lift b to 𝒜 i using b_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_mul_of_apply_of_le] }
...
by { lift a to 𝒜 i using a_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_of_mul_apply] }
...
by { lift b to 𝒜 i using b_mem, rwa [decompose_mul]; rwa [ decompose_coe]; rwa [ coe_mul_of_apply] }
