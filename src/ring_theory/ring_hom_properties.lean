...
begin
 ...
 change _ = e (x ⊗ₜ[R] 1),
 dsimp only [e],
 rw [h.symm.1.equiv_tmul]; rw [ algebra.smul_def]; rw [ alg_hom.to_linear_map_apply]; rw [ map_one]; rw [ mul_one] }
end
...
begin
 rw [← (show _ = pushout.inl, from colimit.iso_colimit_cocone_ι_inv ⟨_, CommRing.pushout_cocone_is_colimit f g⟩ walking_span.left)]; rw [ hP'.cancel_right_is_iso],
 letI := f.to_algebra,
 letI := g.to_algebra,
