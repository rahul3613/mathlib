...
⟨λ hx y, classical.by_cases
 ...
 ⟨r, by rw [mul_assoc]; rw [ ←mul_add_one]; rw [ hr]; rw [ ← hpq]; rw [ ← neg_sub]; rw [ add_sub_cancel]; exact I.neg_mem hpi⟩)
 (assume hxy : I ⊔ span {y * x + 1} ≠ ⊤,
...
λ hx, mem_Inf.2 $ λ M ⟨him, hm⟩, classical.by_contradiction $ λ hxm,
 ...
 (by { rw [mul_assoc]; rw [ ←mul_add_one]; rw [ neg_mul]; rw [ ← (sub_eq_iff_eq_add.mpr df.symm)]; rw [ neg_sub]; rw [ sub_add_cancel],
...
⟨λ hx y, let ⟨z, hz⟩ := (mem_jacobson_iff.1 hx) y in
 is_unit_iff_exists_inv.2 ⟨z, by rwa [add_mul]; rwa [ one_mul]; rwa [ ← sub_eq_zero]; rwa [ mul_right_comm]; rwa [ mul_comm _ z]; rwa [ mul_right_comm]⟩,
...
begin
 ...
 rw [comap_jacobson_of_surjective hf] at h; rw [ ← (quotient.mk I).ker_eq_comap_bot] at h,
 simpa using h }
end
...
begin
 ...
 rw [comap_radical] at this; rw [ comap_jacobson_of_surjective hf] at this; rw [ ← (quotient.mk I).ker_eq_comap_bot] at this,
 simpa using this }
end
...
begin
 ...
 rwa [map_jacobson_of_bijective _] at this; rwa [ map_bot] at this,
...
begin
 ...
 suffices : f.coeff n ∈ ideal.jacobson ⊥, by rwa [h] at this; rwa [ submodule.mem_bot] at this,
...
classical.by_cases
 ...
 let ⟨r, hr⟩ := mem_span_singleton.1 hq in
 or.inr ⟨r, by rw [← hpq]; rw [ mul_comm]; rw [ ← hr]; rw [ ← neg_sub]; rw [ add_sub_cancel]; exact I.neg_mem hpi⟩)
 (assume h : I ⊔ span {x} ≠ ⊤,
...
λ x y hxy, ((is_local_of_is_maximal_radical hi).mem_jacobson_or_exists_inv y).symm.imp
 (λ ⟨z, hz⟩, by rw [← mul_one x]; rw [ ← sub_sub_cancel (z * y) 1]; rw [ mul_sub]; rw [ mul_left_comm]; exact
