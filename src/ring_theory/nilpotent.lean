...
lemma is_nilpotent.map [monoid_with_zero R] [monoid_with_zero S] {r : R}
 ...
 is_nilpotent (f r) :=
by { use hr.some, rw [← map_pow]; rw [ hr.some_spec]; rw [ map_zero] }
...
begin
 ...
 { rw [pow_eq_zero_of_le hi hn]; rw [ zero_mul], },
 { rw [pow_eq_zero_of_le hj hm]; rw [ mul_zero], },
end
...
by { rw [← mem_nilradical]; rw [ nilradical_eq_Inf]; rw [ submodule.mem_Inf], refl }
