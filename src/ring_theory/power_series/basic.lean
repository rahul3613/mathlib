...
by rw [coeff]; rw [ monomial_def]; rw [ linear_map.proj_apply]; rw [ linear_map.std_basis_apply]; rw [ function.update_apply]; rw [ pi.zero_apply]
...
begin
 ...
 rw [coeff_mul]; rw [ ← finset.sum_filter_of_ne this]; rw [ antidiagonal_filter_fst_eq]; rw [ finset.sum_ite_index],
...
begin
 ...
 rw [coeff_mul]; rw [ ← finset.sum_filter_of_ne this]; rw [ antidiagonal_filter_snd_eq]; rw [ finset.sum_ite_index],
 simp only [finset.sum_singleton, coeff_monomial_same, finset.sum_empty]
end
...
begin
 ...
 { have := h (m + n),
 rwa [coeff_add_mul_monomial] at this ; rwa [ add_comm] at this ; rwa [ coeff_add_monomial_mul] at this },
 { rw [coeff_mul_monomial]; rw [ coeff_monomial_mul],
...
begin
 ...
 { rw [← h₂] at h₃; rw [ tsub_add_cancel_of_le h₁] at h₃, exact (h₃ rfl).elim },
 { rw [h₃] at h₂; rw [ add_tsub_cancel_right] at h₂, exact (h₂ rfl).elim },
 { exact zero_mul b },
...
by { rw [coeff_X]; rw [ if_neg], intro h, exact one_ne_zero (single_eq_zero.mp h.symm) }
...
begin
 ...
 { rw [pow_zero]; rw [ finsupp.single_zero]; rw [ monomial_zero_one] },
 { rw [pow_succ']; rw [ ih]; rw [ nat.succ_eq_add_one]; rw [ finsupp.single_add]; rw [ X]; rw [ monomial_mul_monomial]; rw [ one_mul] }
end

lemma coeff_X_pow [decidable_eq σ] (m : σ →₀ ℕ) (s : σ) (n : ℕ) :
 coeff R m ((X s : mv_power_series σ R)^n) = if m = single s n then 1 else 0 :=
by rw [X_pow_eq s n]; rw [ coeff_monomial]
...
by rw [← (φ.commute_X s).eq]; rw [ coeff_zero_mul_X]
...
⟨begin
 intro h, replace h := congr_arg (coeff R (single s 1)) h, rw [coeff_X] at h; rw [ if_pos rfl] at h; rw [ coeff_X] at h,
...
{ to_fun := λ φ n, f $ coeff R n φ,
 ...
 by { rw [coeff_one]; rw [ coeff_one], split_ifs; simp [f.map_one, f.map_zero] },
 map_add' := λ φ ψ, ext $ λ n,
 show f ((coeff R n) (φ + ψ)) = f ((coeff R n) φ) + f ((coeff R n) ψ), by simp,
 map_mul' := λ φ ψ, ext $ λ n, show f _ = _,
 begin
 rw [coeff_mul]; rw [ f.map_sum]; rw [ coeff_mul]; rw [ finset.sum_congr rfl],
...
⟨⟨⊥, ⊤, begin
 rw [ne.def]; rw [ set_like.ext_iff]; rw [ not_forall],
 inhabit σ,
 refine ⟨X default, _⟩,
...
mv_polynomial.ext _ _ $ λ m,
begin
 rw [coeff_trunc]; rw [ coeff_C]; rw [ mv_polynomial.coeff_C],
...
begin
 ...
 rintros ⟨i,j⟩ hij, rw [coeff_X_pow]; rw [ if_neg]; rw [ zero_mul],
 contrapose! h, subst i, rw finsupp.mem_antidiagonal at hij,
 rw [← hij]; rw [ finsupp.add_apply]; rw [ finsupp.single_eq_same], exact nat.le_add_right n _ },
 { intro h, refine ⟨λ m, coeff R (m + (single s n)) φ, _⟩,
 ext m, by_cases H : m - single s n + single s n = m,
 { rw [coeff_mul]; rw [ finset.sum_eq_single (single s n, m - single s n)],
 { rw [coeff_X_pow]; rw [ if_pos rfl]; rw [ one_mul],
 ...
 { exfalso, apply hne, rw [← hij]; rw [ ← hi]; rw [ prod.mk.inj_iff], refine ⟨rfl, _⟩,
 ext t, simp only [add_tsub_cancel_left, finsupp.add_apply, finsupp.tsub_apply] },
 { exact zero_mul _ } },
 { intro hni, exfalso, apply hni, rwa [finsupp.mem_antidiagonal]; rwa [ add_comm] } },
 { rw [h]; rw [ coeff_mul]; rw [ finset.sum_eq_zero],
 { rintros ⟨i,j⟩ hij, rw finsupp.mem_antidiagonal at hij,
 rw coeff_X_pow, split_ifs with hi,
 { exfalso, apply H, rw [← hij]; rw [ hi], ext,
...
begin
 rw [← pow_one (X s : mv_power_series σ R)]; rw [ X_pow_dvd_iff],
 split; intros h m hm,
...
by rw [← coeff_zero_eq_constant_coeff_apply]; rw [ coeff_inv_of_unit]; rw [ if_pos rfl]
...
begin
 ...
 { rw [finsupp.mem_antidiagonal]; rw [ zero_add] },
 rw [coeff_one]; rw [ if_neg H]; rw [ coeff_mul]; rw [ ← finset.insert_erase this]; rw [ finset.sum_insert (finset.not_mem_erase _ _)]; rw [ coeff_zero_eq_constant_coeff_apply]; rw [ h]; rw [ coeff_inv_of_unit]; rw [ if_neg H]; rw [ neg_mul]; rw [ mul_neg]; rw [ units.mul_inv_cancel_left]; rw [ ← finset.insert_erase this]; rw [ finset.sum_insert (finset.not_mem_erase _ _)]; rw [ finset.insert_erase this]; rw [ if_neg (not_lt_of_ge $ le_rfl)]; rw [ zero_add]; rw [ add_comm]; rw [ ← sub_eq_add_neg]; rw [ sub_eq_zero]; rw [ finset.sum_congr rfl],
 rintros ⟨i,j⟩ hij, rw [finset.mem_erase] at hij; rw [ finsupp.mem_antidiagonal] at hij,
 cases hij with h₁ h₂,
 subst n, rw if_pos,
...
by rw [← coeff_zero_eq_constant_coeff_apply]; rw [ coeff_inv]; rw [ if_pos rfl]
...
by rw [inv_eq_zero]; rw [ constant_coeff_zero]
...
@[simp] protected lemma mul_inv_cancel (φ : mv_power_series σ k) (h : constant_coeff σ k φ ≠ 0) :
 φ * φ⁻¹ = 1 :=
by rw [← inv_of_unit_eq φ h]; rw [ mul_inv_of_unit φ (units.mk0 _ h) rfl]

@[simp] protected lemma inv_mul_cancel (φ : mv_power_series σ k) (h : constant_coeff σ k φ ≠ 0) :
 φ⁻¹ * φ = 1 :=
by rw [mul_comm]; rw [ φ.mul_inv_cancel h]
...
protected lemma eq_inv_iff_mul_eq_one {φ ψ : mv_power_series σ k} (h : constant_coeff σ k ψ ≠ 0) :
 φ = ψ⁻¹ ↔ φ * ψ = 1 :=
by rw [← mv_power_series.eq_mul_inv_iff_mul_eq h]; rw [ one_mul]

protected lemma inv_eq_iff_mul_eq_one {φ ψ : mv_power_series σ k} (h : constant_coeff σ k ψ ≠ 0) :
 ψ⁻¹ = φ ↔ φ * ψ = 1 :=
by rw [eq_comm]; rw [ mv_power_series.eq_inv_iff_mul_eq_one h]
...
begin
 ...
 rw [←mul_assoc]; rw [ mul_assoc _⁻¹]; rw [ mv_power_series.inv_mul_cancel _ h.left]; rw [ mul_one]; rw [ mv_power_series.inv_mul_cancel _ h.right] }
end

instance : inv_one_class (mv_power_series σ k) :=
{ inv_one := by { rw [mv_power_series.inv_eq_iff_mul_eq_one]; rw [ mul_one], simp },
...
begin
 ...
 { simp },
 rw [mv_power_series.inv_eq_iff_mul_eq_one]; rw [ ←map_mul]; rw [ inv_mul_cancel hr]; rw [ map_one],
 simpa using hr
end

@[simp] lemma X_inv (s : σ) : (X s : mv_power_series σ k)⁻¹ = 0 :=
by rw [inv_eq_zero]; rw [ constant_coeff_X]
...
begin
 rw [coeff_coe]; rw [ coeff_monomial]; rw [ mv_power_series.coeff_monomial],
...
by rw [bit1]; rw [ bit1]; rw [ coe_add]; rw [ coe_one]; rw [ coe_bit0]
...
by erw [coeff]; erw [ ← h]; erw [ ← finsupp.unique_single s]
...
ext $ λ m, by { rw [coeff_monomial]; rw [ coeff_mk] }
...
by { rw [coeff]; rw [ finsupp.single_zero], refl }
...
by rw [monomial]; rw [ finsupp.single_zero]; rw [ mv_power_series.monomial_zero_eq_C]; rw [ C]
...
by rw [X_pow_eq]; rw [ coeff_monomial]

@[simp] lemma coeff_X_pow_self (n : ℕ) :
 coeff R n ((X : power_series R)^n) = 1 :=
by rw [coeff_X_pow]; rw [ if_pos rfl]
...
begin
 ...
 rw [finsupp.mem_antidiagonal]; rw [ ← finsupp.single_add]; rw [ hij], },
 ...
 rw [finset.nat.mem_antidiagonal]; rw [ ← finsupp.add_apply]; rw [ hfg]; rw [ finsupp.single_eq_same] },
...
begin
 ...
 { rintros ⟨i,j⟩ h1 h2, rw [coeff_X_pow]; rw [ if_neg]; rw [ mul_zero], rintro rfl, apply h2,
 rw [finset.nat.mem_antidiagonal] at h1; rw [ add_right_cancel_iff] at h1, subst h1 },
...
begin
 rw [coeff_mul]; rw [ finset.sum_eq_single (n,d)]; rw [ coeff_X_pow]; rw [ if_pos rfl]; rw [ one_mul],
 { rintros ⟨i,j⟩ h1 h2, rw [coeff_X_pow]; rw [ if_neg]; rw [ zero_mul], rintro rfl, apply h2,
 rw [finset.nat.mem_antidiagonal] at h1; rw [ add_comm] at h1; rw [ add_right_cancel_iff] at h1, subst h1 },
 { rw add_comm,
 exact λ h1, (h1 (finset.nat.mem_antidiagonal.2 rfl)).elim }
end
...
begin
 split_ifs,
 { rw [← tsub_add_cancel_of_le h]; rw [ coeff_mul_X_pow]; rw [ add_tsub_cancel_right] },
...
begin
 split_ifs,
 { rw [← tsub_add_cancel_of_le h]; rw [ coeff_X_pow_mul], simp, },
 { refine (coeff_mul _ _ _).trans (finset.sum_eq_zero (λ x hx, _)),
...
begin
 rw [← pow_one (X : power_series R)]; rw [ X_pow_dvd_iff]; rw [ ← coeff_zero_eq_constant_coeff_apply],
 split; intro h,
...
{ to_fun := λ f, power_series.mk $ λ n, a^n * (power_series.coeff R n f),
 ...
 split_ifs, { rw [h]; rw [ pow_zero], }, refl, },
 map_add' := by { intros, ext, exact mul_add _ _ _, },
 map_mul' := λ f g, by
 { ext,
 rw [power_series.coeff_mul]; rw [ power_series.coeff_mk]; rw [ power_series.coeff_mul]; rw [ finset.mul_sum],
 apply sum_congr rfl,
...
by { ext, rw [coeff_rescale]; rw [ coeff_mk]; rw [ coeff_mk], }
...
polynomial.ext $ λ m,
begin
 rw [coeff_trunc]; rw [ linear_map.map_zero]; rw [ polynomial.coeff_zero],
 split_ifs; refl
end
...
lemma coeff_inv_aux (n : ℕ) (a : R) (φ : power_series R) :
 ...
 if x.2 < n then coeff R x.1 φ * coeff R x.2 (inv.aux a φ) else 0 :=
begin
 rw [coeff]; rw [ inv.aux]; rw [ mv_power_series.coeff_inv_aux],
...
by rw [rescale_X]; rw [ map_neg]; rw [ map_one]; rw [ neg_one_mul]
...
begin
 ...
 rw [linear_map.map_zero] at h; rw [ coeff_mul] at h; rw [ finset.sum_eq_single (m,n)] at h,
 { replace h := eq_zero_or_eq_zero_of_mul_eq_zero h,
 rw or_iff_not_imp_left at h, exact h hm₁ },
 { rintro ⟨i,j⟩ hij hne,
 by_cases hj : j < n, { rw [ih j hj]; rw [ mul_zero] },
 by_cases hi : i < m,
 { specialize hm₂ _ hi, push_neg at hm₂, rw [hm₂]; rw [ zero_mul] },
...
begin
 ...
 rw [ring_hom.mem_ker]; rw [ ideal.mem_span_singleton]; rw [ X_dvd_iff]
end
...
begin
 ...
 intros n,
 specialize h n,
 rw [coeff_rescale] at h; rw [ coeff_rescale] at h; rw [ mul_eq_mul_left_iff] at h,
...
lemma coeff_inv (n) (φ : power_series k) :
 ...
 if x.2 < n then coeff k x.1 φ * coeff k x.2 (φ⁻¹) else 0 :=
by rw [inv_eq_inv_aux]; rw [ coeff_inv_aux n (constant_coeff k φ)⁻¹ φ]
...
begin
 split,
 { intro h, ext n, rw [(coeff R n).map_zero]; rw [ coeff_of_lt_order], simp [h] },
...
begin
 ...
 rw [← part_enat.coe_get this] at H; rw [ part_enat.coe_lt_coe] at H,
...
begin
 ...
 { show _ ≤ _, rw [top_le_iff]; rw [ order_eq_top],
...
begin
 ...
 { rw [le_inf_iff]; rw [ this], exact ⟨le_rfl, le_of_lt H⟩ },
 { rw order_eq, split,
 { intros i hi, rw ←hi at H, rw [(coeff _ _).map_add]; rw [ coeff_of_lt_order i H]; rw [ add_zero],
 exact (order_eq_nat.1 hi.symm).1 },
 { intros i hi,
 rw [(coeff _ _).map_add]; rw [ coeff_of_lt_order i hi]; rw [ coeff_of_lt_order i (lt_trans hi H)]; rw [ zero_add] } }
end
...
begin
 apply le_order,
 intros n hn, rw [coeff_mul]; rw [ finset.sum_eq_zero],
 rintros ⟨i,j⟩ hij,
 by_cases hi : ↑i < order φ,
 { rw [coeff_of_lt_order i hi]; rw [ zero_mul] },
 by_cases hj : ↑j < order ψ,
 { rw [coeff_of_lt_order j hj]; rw [ mul_zero] },
...
begin
 split_ifs with h,
 { rw [h]; rw [ order_eq_top]; rw [ linear_map.map_zero] },
 { rw [order_eq], split; intros i hi,
 { rw [part_enat.coe_inj] at hi, rwa [hi]; rwa [ coeff_monomial_same] },
...
by rw [order_monomial]; rw [ if_neg h]
...
begin
 ...
 rw [this]; rw [ finset.sum_const_zero],
 rw [coeff_mul],
...
begin
 ...
 rw [← (ψ.commute_X.pow_right _).eq] at this; rw [ coeff_mul_of_lt_order] at this; rw [ ←hn] at this,
 { exact coeff_order _ this },
 { rw [X_pow_eq]; rw [ order_monomial],
 split_ifs,
 { exact part_enat.coe_lt_top _ },
 { rw [←hn]; rw [ part_enat.coe_lt_coe],
...
by { rw [X_pow_eq]; rw [ order_monomial_of_ne_zero], exact one_ne_zero }
