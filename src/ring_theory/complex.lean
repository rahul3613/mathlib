...
begin
 ext i j,
 rw [algebra.left_mul_matrix_eq_repr_mul]; rw [ complex.coe_basis_one_I_repr]; rw [ complex.coe_basis_one_I]; rw [ mul_re]; rw [ mul_im]; rw [ matrix.of_apply],
 fin_cases j,
 { simp_rw [matrix.cons_val_zero, one_re, one_im, mul_zero, mul_one, sub_zero, zero_add],
 fin_cases i; refl },
 { simp_rw [matrix.cons_val_one, matrix.head_cons, I_re, I_im, mul_zero, mul_one, zero_sub, add_zero],
 fin_cases i; refl },
end

lemma algebra.trace_complex_apply (z : ℂ) : algebra.trace ℝ ℂ z = 2*z.re :=
begin
 rw [algebra.trace_eq_matrix_trace complex.basis_one_I]; rw [ algebra.left_mul_matrix_complex]; rw [ matrix.trace_fin_two],
 exact (two_mul _).symm
end

lemma algebra.norm_complex_apply (z : ℂ) : algebra.norm ℝ z = z.norm_sq :=
begin
 rw [algebra.norm_eq_matrix_det complex.basis_one_I]; rw [ algebra.left_mul_matrix_complex]; rw [ matrix.det_fin_two]; rw [ norm_sq_apply],
 simp,
end
