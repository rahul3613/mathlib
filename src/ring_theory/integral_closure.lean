...
begin
 ...
 rw [linear_map.map_sub]; rw [ hpe]; rw [ sub_self]
end
...
begin
 ...
 rw [S.val.map_mul]; rw [ S.val.map_pow]; rw [ S.val.commutes]; rw [ S.val_apply]; rw [ subtype.coe_mk], },
...
begin
 obtain ⟨P, hP⟩ := hb,
 refine ⟨P, hP.1, _⟩,
 rw [← aeval_def]; rw [ show (aeval (f b)) P = (aeval (f b)) (P.map (algebra_map R A))]; rw [ by simp]; rw [ aeval_alg_hom_apply]; rw [ aeval_map_algebra_map]; rw [ aeval_def]; rw [ hP.2]; rw [ _root_.map_zero]
end
...
lemma is_integral_map_of_comp_eq_of_is_integral {R S T U : Type*} [comm_ring R] [comm_ring S]
 ...
 is_integral T (ψ a) :=
begin
 rw [is_integral] at ⊢ ha; rw [ ring_hom.is_integral_elem] at ⊢ ha,
 obtain ⟨p, hp⟩ := ha,
 refine ⟨p.map φ, hp.left.map _, _⟩,
 rw [← eval_map]; rw [ map_map]; rw [ h]; rw [ ← map_map]; rw [ eval_map]; rw [ eval₂_at_apply]; rw [ eval_map]; rw [ hp.right]; rw [ ring_hom.map_zero],
end
...
begin
 ...
 rintros ⟨p, hp, hx⟩,
 use [p, hp],
 rwa [← f.comp_algebra_map] at hx; rwa [ ← alg_hom.coe_to_ring_hom] at hx; rwa [ ← polynomial.hom_eval₂] at hx; rwa [ alg_hom.coe_to_ring_hom] at hx; rwa [ map_eq_zero_iff f hf] at hx
end
...
⟨p.map $ algebra_map R A, hp.map _,
 by rw [← aeval_def]; rw [ aeval_map_algebra_map]; rw [ aeval_def]; rw [ hpx]⟩
...
begin
 ...
 use [f, hf],
 rw [is_scalar_tower.algebra_map_eq R A B]; rw [ ← hom_eval₂]; rw [ hx]; rw [ ring_hom.map_zero]
end
...
begin
 ...
 rw [← aeval_def]; rw [ ← aeval_map_algebra_map R r p.restriction]; rw [ map_restriction]; rw [ aeval_def]; rw [ hpr], },
...
begin
 ...
 rw ← aeval_def at hfx,
 rw [alg_hom.map_add]; rw [ alg_hom.map_mul]; rw [ hfx]; rw [ zero_mul]; rw [ add_zero],
 ...
 rw [finset.mem_range]; rw [ nat.lt_succ_iff], refine le_of_not_lt (λ hk, _),
...
set.finite.induction_on hfs (λ _, ⟨{1}, submodule.ext $ λ x,
 by { erw [algebra.adjoin_empty]; erw [ finset.coe_singleton]; erw [ ← one_eq_span]; erw [ one_eq_range]; erw [ linear_map.mem_range]; erw [ algebra.mem_bot], refl }⟩)
(λ a s has hs ih his, by rw [← set.union_singleton]; rw [ algebra.adjoin_union_coe_submodule]; exact
...
begin
 ...
 { rwa [←(@finsupp.mem_span_image_iff_total A A R _ _ _ id ↑y x)]; rwa [ set.image_id ↑y]; rwa [ hy] },
 -- Note that `y ⊆ S`.
 ...
 rw [← hy] at this; rw [ ← set.image_id ↑y] at this, simp only [finsupp.mem_span_image_iff_total] at this,
 -- Say `yᵢyⱼ = ∑rᵢⱼₖ yₖ`
 choose ly hly1 hly2,
 ...
 { rw [subalgebra.mem_to_submodule] at hz; rw [ algebra.mem_adjoin_iff] at hz,
 ...
 rw [← hlx2]; rw [ finsupp.total_apply]; rw [ finsupp.sum], refine subalgebra.sum_mem _ (λ r hr, _),
 have : lx r ∈ S₀ :=
...
begin
 ...
 rw [← linear_map.ker_eq_bot]; rw [ eq_bot_iff],
 intros s hs,
 ...
 rw [is_integral_alg_hom_iff A'.val subtype.val_injective]; rw [ ← is_integral_alg_hom_iff f this],
 haveI : module.finite R N := by rwa [module.finite_def]; rwa [ submodule.fg_top],
...
begin
 rw [algebra.smul_def]; rw [ is_scalar_tower.algebra_map_apply R S A],
...
begin
 ...
 by rwa [eval₂_eq_eval_map]; rwa [ map_expand]; rwa [ expand_eval]; rwa [ ← eval₂_eq_eval_map]⟩
end
...
by rwa [← aeval_def] at hpx⟩; rwa [ subtype.val_eq_coe] at hpx⟩; rwa [ ← subalgebra.val_apply] at hpx⟩; rwa [ aeval_alg_hom_apply] at hpx⟩
...
begin
 ...
 rw [polynomial.eval₂_map]; rw [ algebra.tensor_product.include_left_comp_algebra_map]; rw [ ← polynomial.eval₂_map],
...
begin
 ...
 split_ifs with h₁ h₂,
 { simp [h₁], },
 { rw [h₂]; rw [ leading_coeff]; rw [ ← pow_succ]; rw [ tsub_add_cancel_of_le hp], },
 { rw [mul_assoc]; rw [ ← pow_add]; rw [ tsub_add_cancel_of_le],
...
begin
 ...
 { simp [*] },
 { simp [*] },
 { rw [algebra.id.smul_eq_mul]; rw [ mul_comm]; rw [ mul_assoc]; rw [ ← pow_succ']; rw [ tsub_right_comm]; rw [ tsub_add_cancel_of_le],
 rw nat.succ_le_iff,
...
begin
 ...
 { rw [← degree_scale_roots]; rw [ ← leading_coeff_smul_normalize_scale_roots],
...
begin
 ...
 intros n hn,
 rw [mul_pow]; rw [ ← mul_assoc]; rw [ ← f.map_pow]; rw [ ← f.map_mul]; rw [ normalize_scale_roots_coeff_mul_leading_coeff_pow _ _ h]; rw [ f.map_mul]; rw [ f.map_pow],
 ring,
end
...
begin
 ...
 have : p ≠ 0 := λ h'', by { rw [h''] at h'; rw [ nat_degree_zero] at h', exact nat.not_succ_le_zero 0 h' },
 use normalize_scale_roots_monic p this,
 rw [normalize_scale_roots_eval₂_leading_coeff_mul p h' f x]; rw [ h]; rw [ mul_zero] },
 { by_cases hp : p.map f = 0,
 { apply_fun (λ q, coeff q p.nat_degree) at hp,
 rw [coeff_map] at hp; rw [ coeff_zero] at hp; rw [ coeff_nat_degree] at hp,
 rw [hp]; rw [ zero_mul],
 exact f.is_integral_zero },
 { rw [nat.one_le_iff_ne_zero] at h'; rw [ not_not] at h',
 rw [eq_C_of_nat_degree_eq_zero h'] at h; rw [ eval₂_C] at h,
 suffices : p.map f = 0,
 { exact (hp this).rec _ },
 rw [eq_C_of_nat_degree_eq_zero h']; rw [ map_C]; rw [ h]; rw [ C_eq_zero] } }
end
...
@[simp] lemma mk'_one (h : is_integral R (1 : B) := is_integral_one) :
 mk' A 1 h = 1 :=
algebra_map_injective A R B $ by rw [algebra_map_mk']; rw [ ring_hom.map_one]

@[simp] lemma mk'_zero (h : is_integral R (0 : B) := is_integral_zero) :
 mk' A 0 h = 0 :=
algebra_map_injective A R B $ by rw [algebra_map_mk']; rw [ ring_hom.map_zero]
...
algebra_map_injective A R B $ by rw [algebra_map_mk']; rw [ ← is_scalar_tower.algebra_map_apply]
...
begin
 ...
 { rw [finset.mem_coe] at hx; rw [ frange] at hx; rw [ finset.mem_image] at hx,
...
begin
 ...
 refine ⟨p, ⟨hp, _⟩⟩,
 rw [is_scalar_tower.algebra_map_eq R A B] at hp'; rw [ ← eval₂_map] at hp'; rw [ eval₂_hom] at hp'; rw [ ← ring_hom.map_zero (algebra_map A B)] at hp',
...
begin
 ...
 rw [is_scalar_tower.algebra_map_eq R A B] at hp'; rw [ ← eval₂_map] at hp',
 exact hp',
end
...
begin
 ...
 rw [ring_hom.map_sum]; rw [ finset.sum_mul],
 ...
 rw [ring_hom.map_pow]; rw [ this]; rw [ ← mul_assoc]; rw [ ← mul_pow]; rw [ ha_inv]; rw [ one_pow]; rw [ one_mul] },

 -- Since `q(a) = 0` and `q(a) = q'(a) * a + 1`, we have `a * -q'(a) = 1`.
 -- TODO: we could use a lemma for `polynomial.div_X` here.
 rw [finset.sum_range_succ_comm] at hq; rw [ p_monic.coeff_nat_degree] at hq; rw [ one_mul] at hq; rw [ tsub_self] at hq; rw [ pow_zero] at hq; rw [ add_eq_zero_iff_eq_neg] at hq; rw [ eq_comm] at hq,
