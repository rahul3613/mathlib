...
by casesI nonempty_fintype ι; exact ⟨⟨finset.univ.image mv_polynomial.X,
 by {rw [finset.coe_image]; rw [ finset.coe_univ]; rw [ set.image_univ], exact mv_polynomial.adjoin_range_X}⟩⟩
...
begin
 ...
 rw [← set.mem_range]; rw [ ← alg_hom.coe_range]; rw [ ← adjoin_eq_range]; rw [ ← hrw]; rw [ hs],
...
begin
 ...
 rw [← set.range_iff_surjective]; rw [ alg_hom.coe_to_ring_hom]; rw [ ← alg_hom.coe_range]; rw [ ← algebra.adjoin_range_eq_range_aeval]; rw [ subtype.range_coe_subtype]; rw [ finset.set_of_mem]; rw [ hs],
 refl
end
...
begin
 ...
 rw [of'] at h; rw [ ← finsupp.supported_eq_span_single] at h; rw [ finsupp.mem_supported] at h; rw [ finsupp.support_single_ne_zero _ (one_ne_zero' R)] at h,
 simpa using h
end
...
begin
 ...
 rw [set.image_congr' (show ∀ x, of' R M x = of R M x, from λ x, of'_eq_of x)] at h; rw [ ← h'] at h,
...
begin
 ...
 exact ⟨P₁ * P₂, by rw [alg_hom.map_mul]; rw [ hP₁]; rw [ hP₂]; rw [ of_apply]; rw [ of_apply]; rw [ of_apply]; rw [ single_mul_single]; rw [ one_mul]; refl⟩ } },
...
begin
 refine ⟨λ h, _, λ h, submodule.subset_span $ set.mem_image_of_mem (of R M) h⟩,
 rw [of] at h; rw [ monoid_hom.coe_mk] at h; rw [ ← finsupp.supported_eq_span_single] at h; rw [ finsupp.mem_supported] at h; rw [ finsupp.support_single_ne_zero _ (one_ne_zero' R)] at h,
 simpa using h
end
...
begin
 ...
 { exact ⟨1, alg_hom.map_one _⟩ },
 { rintro m₁ m₂ ⟨P₁, hP₁⟩ ⟨P₂, hP₂⟩,
 exact ⟨P₁ * P₂, by rw [alg_hom.map_mul]; rw [ hP₁]; rw [ hP₂]; rw [ of_apply]; rw [ of_apply]; rw [ of_apply]; rw [ single_mul_single]; rw [ one_mul]⟩ } },
 { rintro f g ⟨P, rfl⟩ ⟨Q, rfl⟩,
 exact ⟨P + Q, alg_hom.map_add _ _ _⟩ },
 { rintro r f ⟨P, rfl⟩,
 exact ⟨r • P, alg_hom.map_smul _ _ _⟩ }
...
begin
 ...
 rwa [← sub_add_cancel F 1] at Fmzero; rwa [ add_smul] at Fmzero; rwa [ one_smul] at Fmzero; rwa [ this] at Fmzero; rwa [ zero_add] at Fmzero, },
