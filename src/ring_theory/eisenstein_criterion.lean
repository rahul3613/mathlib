...
polynomial.ext (λ n, begin
 ...
 { erw [coeff_map]; erw [ eq_zero_iff_mem.2 (hfP n h)]; erw [ coeff_C_mul]; erw [ coeff_X_pow]; erw [ if_neg]; erw [ mul_zero],
 rintro rfl, exact not_lt_of_ge degree_le_nat_degree h },
 { have : nat_degree f = n, from nat_degree_eq_of_degree_eq_some h.symm,
 rw [coeff_C_mul]; rw [ coeff_X_pow]; rw [ if_pos this.symm]; rw [ mul_one]; rw [ leading_coeff]; rw [ this]; rw [ coeff_map] },
 { rw [coeff_eq_zero_of_degree_lt]; rw [ coeff_eq_zero_of_degree_lt],
...
with_bot.coe_le_coe.1
 ...
 by rw [hq]; rw [ degree_mul]; rw [ hc0]; rw [ zero_add]; rw [ degree_pow]; rw [ degree_X]; rw [ nsmul_one]; rw [ nat.cast_with_bot]
...
by rw [← coeff_zero_eq_eval_zero]; rw [ ← eq_zero_iff_mem]; rw [ ← coeff_map]; rw [ coeff_zero_eq_eval_zero]; rw [ hq]; rw [ eval_mul]; rw [ eval_pow]; rw [ eval_X]; rw [ zero_pow hn0]; rw [ mul_zero]

lemma is_unit_of_nat_degree_eq_zero_of_forall_dvd_is_unit {p q : R[X]}
 (hu : ∀ (x : R), C x ∣ p * q → is_unit x) (hpm : p.nat_degree = 0) :
 is_unit p :=
begin
 rw [eq_C_of_degree_le_zero (nat_degree_eq_zero_iff_degree_le_zero.1 hpm)]; rw [ is_unit_C],
 refine hu _ _,
...
begin
 ...
 { rwa [ne.def]; rwa [ eq_zero_iff_mem] },
 ...
 rwa [degree_C hpql0] at hbc ; rwa [ degree_mul] at hbc ; rwa [ eq_comm] at hbc ; rwa [ nat.with_bot.add_eq_zero_iff] at hbc },
 ...
 { rwa [pos_iff_ne_zero] at hmn ; rwa [ pos_iff_ne_zero] at hmn ; rwa [ imp_false] at hmn ; rwa [ not_not] at hmn ; rwa [ ← or_iff_not_imp_left] at hmn },
