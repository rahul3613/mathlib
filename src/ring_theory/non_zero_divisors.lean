...
lemma mul_left_mem_non_zero_divisors_eq_zero_iff {r x : M₁} (hr : r ∈ M₁⁰) :
 r * x = 0 ↔ x = 0 :=
by rw [mul_comm]; rw [ mul_right_mem_non_zero_divisors_eq_zero_iff hr]
...
begin
 ...
 rw [←sub_eq_zero]; rw [ ←mul_right_mem_non_zero_divisors_eq_zero_iff hr]; rw [ sub_mul]; rw [ h]; rw [ sub_self]
end
...
begin
 ...
 { rw [←mul_assoc]; rw [ h']; rw [ zero_mul] },
 { rw [mul_comm a b]; rw [ ←mul_assoc]; rw [ h']; rw [ zero_mul] } },
 { rintros ⟨ha, hb⟩ x hx,
 apply ha,
 apply hb,
 rw [mul_assoc]; rw [ hx] },
