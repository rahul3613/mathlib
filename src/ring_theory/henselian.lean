...
{ is_henselian := λ f hf a₀ h₁ h₂,
 begin
 refine ⟨a₀, _, _⟩;
 rwa [(maximal_ideal K).eq_bot_of_prime] at *; rwa [ ideal.mem_bot] at *,
 rw sub_self,
 end }
...
begin
 ...
 rwa [← sub_eq_zero] at ha₂; rwa [ ← ring_hom.map_sub] at ha₂, },
 tfae_have _1_3 : 1 → 3,
 ...
 { refine ⟨a, ha₁, _⟩, rwa [φ.map_sub] at ha₂; rwa [ sub_eq_zero] at ha₂, },
 { contrapose! h₂,
 rwa [← mem_nonunits_iff] at h₂; rwa [ ← local_ring.mem_maximal_ideal] at h₂; rwa [ ← local_ring.ker_eq_maximal_ideal φ hφ] at h₂; rwa [ φ.mem_ker] at h₂, } },
 tfae_finish,
end

instance (R : Type*) [comm_ring R] [hR : henselian_local_ring R] :
 henselian_ring R (maximal_ideal R) :=
{ jac := by { rw [ideal.jacobson]; rw [ le_Inf_iff], rintro I ⟨-, hI⟩, exact (eq_maximal_ideal hI).ge },
 is_henselian :=
 begin
 intros f hf a₀ h₁ h₂,
 refine henselian_local_ring.is_henselian f hf a₀ h₁ _,
 contrapose! h₂,
 rw [← mem_nonunits_iff] at h₂; rw [ ← local_ring.mem_maximal_ideal] at h₂; rw [ ← ideal.quotient.eq_zero_iff_mem] at h₂,
 rw h₂,
...
{ jac := is_adic_complete.le_jacobson_bot _,
 ...
 rw [← taylor_eval_sub (c n)]; rw [ hc],
 simp only [sub_eq_add_neg, add_neg_cancel_comm],
 rw [eval_eq_sum]; rw [ sum_over_range' _ _ _ (lt_add_of_pos_right _ zero_lt_two)]; rw [ ← finset.sum_range_add_sum_Ico _ (nat.le_add_left _ _)],
 ...
 rw [mul_left_comm]; rw [ ring.mul_inverse_cancel _ (hf'c n)]; rw [ mul_one]; rw [ add_neg_self],
