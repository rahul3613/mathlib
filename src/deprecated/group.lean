...
theorem is_mul_hom.to_is_monoid_hom [mul_one_class α] [group β] {f : α → β} (hf : is_mul_hom f) :
 is_monoid_hom f :=
{ map_one := mul_right_eq_self.1 $ by rw [← hf.map_mul]; rw [ one_mul],
 map_mul := hf.map_mul }
...
lemma comp (hf : is_monoid_hom f) {γ} [mul_one_class γ] {g : β → γ} (hg : is_monoid_hom g) :
 is_monoid_hom (g ∘ f) :=
{ map_one := show g _ = 1, by rw [hf.map_one]; rw [ hg.map_one],
...
eq_inv_of_mul_eq_one_left $ by rw [← hf.map_mul]; rw [ inv_mul_self]; rw [ hf.map_one]
...
⟨λ h _, by rw ← hf.map_one; exact @h _ _,
 λ h x y hxy, eq_of_div_eq_one $ h _ $ by rwa [hf.map_div]; rwa [ div_eq_one]⟩
