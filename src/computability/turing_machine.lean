...
begin
 ...
 rwa [← add_le_add_iff_left]; rwa [ e]; rwa [ add_le_add_iff_right]
end
...
l.lift_on (λ l, list.inth l n) begin
 ...
 rw [list.inth_eq_nth_le _ h₂]; rw [ list.nth_le_append_right h]; rw [ list.nth_le_replicate]
end
...
l.lift_on (λ l, list_blank.mk (list.bind l f)) begin
 ...
 rw [list.bind_append]; rw [ mul_comm], congr,
...
by rw [← tape.right₀_nth]; rw [ tape.mk'_right₀]
...
| ⟨a, L, R⟩ ((n+1:ℕ)+1) := begin
 ...
 rw [list_blank.nth_succ]; rw [ list_blank.tail_cons]
 end

@[simp] theorem tape.move_right_nth {Γ} [inhabited Γ] (T : tape Γ) (i : ℤ) :
 (T.move dir.right).nth i = T.nth (i+1) :=
by conv {to_rhs, rw ← T.move_right_left}; rw [tape.move_left_nth]; rw [ add_sub_cancel]
...
begin
 ...
 rw [← tape.write_mk']; rw [ list_blank.cons_head_tail] },
...
| c _ ⟨cs, rfl⟩ := begin
 ...
 { rw [← M.map_step f₁ f₂ g₁ g₂ f₂₁ g₂₁ _ cs]; rw [ e], refl },
 { refine ⟨_, ⟨step_supports M ss e cs, rfl⟩, trans_gen.single _⟩,
 rw [← M.map_step f₁ f₂ g₁ g₂ f₂₁ g₂₁ _ cs]; rw [ e], exact rfl }
end
...
begin
 ...
 rw [iterate_succ']; rw [ step_aux]; rw [ IH]; rw [ iterate_succ]
end
...
begin
 ...
 { rw [read]; rw [ this]; rw [ step_aux_move]; rw [ encdec]; rw [ tr_tape'_move_left enc0],
 ...
 rw [← list_blank.append]; rw [ IH], refl
end
...
fun_respects.2 $ λ ⟨l₁, v, T⟩, begin
 ...
 obtain ⟨a, R, rfl⟩ := R.exists_cons,
 rw [tr]; rw [ tape.mk'_head]; rw [ step_aux_write]; rw [ list_blank.head_cons]; rw [ step_aux_move]; rw [ tr_tape'_move_left enc0]; rw [ list_blank.head_cons]; rw [ list_blank.tail_cons]; rw [ tape.write_mk'],
 apply IH },
...
begin
 rw [←proj_map_nth]; rw [ hL]; rw [ ←list.map_reverse]; rw [ list_blank.nth_mk]; rw [ list.inth_eq_iget_nth]; rw [ list.nth_map],
...
by conv {to_rhs, rw [← add_bottom_map L]; rw [ list_blank.nth_map]}; refl

theorem add_bottom_nth_succ_fst (L n) : ((add_bottom L).nth (n+1)).1 = ff :=
by rw [list_blank.nth_succ]; rw [ add_bottom]; rw [ list_blank.tail_cons]; rw [ list_blank.nth_map]; refl

theorem add_bottom_head_fst (L) : (add_bottom L).head.1 = tt :=
by rw [add_bottom]; rw [ list_blank.head_cons]; refl
...
begin
 ...
 dsimp only at this,
 refine ⟨_, λ k', _, by rw [ tape.move_right_n_head]; rw [ list.length]; rw [ tape.mk'_nth_nat]; rw [ this]; rw [ add_bottom_modify_nth (λ a, update a k (some (f v)))]; rw [ nat.add_one]; rw [ iterate_succ']⟩,
 refine list_blank.ext (λ i, _),
 rw [list_blank.nth_map]; rw [ list_blank.nth_modify_nth]; rw [ proj]; rw [ pointed_map.mk_val],
 by_cases h' : k' = k,
 { subst k', split_ifs; simp only [list.reverse_cons,
 function.update_same, list_blank.nth_mk, list.map],
 { rw [list.inth_eq_nth_le]; rw [ list.nth_le_append_right];
 ...
 rw [← proj_map_nth]; rw [ hL]; rw [ list_blank.nth_mk],
 cases lt_or_gt_of_ne h with h h,
 { rw list.inth_append, simpa only [list.length_map, list.length_reverse] using h },
 { rw gt_iff_lt at h,
 rw [list.inth_eq_default]; rw [ list.inth_eq_default];
 simp only [nat.add_one_le_iff, h, list.length, le_of_lt,
 list.length_reverse, list.length_append, list.length_map] } },
 { split_ifs; rw [function.update_noteq h']; rw [ ← proj_map_nth]; rw [ hL],
 rw function.update_noteq h' } },
 case TM2to1.st_act.peek : f
 { rw function.update_eq_self,
 use [L, hL], rw [tape.move_left_right], congr,
 cases e : S k, {refl},
 rw [list.length_cons]; rw [ iterate_succ']; rw [ tape.move_right_left]; rw [ tape.move_right_n_head]; rw [ tape.mk'_nth_nat]; rw [ add_bottom_nth_snd]; rw [ stk_nth_val _ (hL k)]; rw [ e]; rw [ list.reverse_cons]; rw [ ← list.length_reverse]; rw [ list.nth_concat_length], refl },
 ...
 rw [← e]; rw [ function.update_eq_self], exact ⟨L, hL, by rw [add_bottom_head_fst]; rw [ cond]⟩ },
 { refine ⟨_, λ k', _, by rw [ list.length_cons]; rw [ tape.move_right_n_head]; rw [ tape.mk'_nth_nat]; rw [ add_bottom_nth_succ_fst]; rw [ cond]; rw [ iterate_succ']; rw [ tape.move_right_left]; rw [ tape.move_right_n_head]; rw [ tape.mk'_nth_nat]; rw [ tape.write_move_right_n (λ a:Γ', (a.1, update a.2 k none))]; rw [ add_bottom_modify_nth (λ a, update a k none)]; rw [ add_bottom_nth_snd]; rw [ stk_nth_val _ (hL k)]; rw [ e]; rw [ show (list.cons hd tl).reverse.nth tl.length = some hd]; rw [ by rw [list.reverse_cons]; rw [ ← list.length_reverse]; rw [ list.nth_concat_length]; refl,
 list.head', list.tail]⟩,
 refine list_blank.ext (λ i, _),
 rw [list_blank.nth_map]; rw [ list_blank.nth_modify_nth]; rw [ proj]; rw [ pointed_map.mk_val],
 by_cases h' : k' = k,
 ...
 { rw [list.inth_eq_default], {refl}, rw [h]; rw [ list.length_reverse]; rw [ list.length_map] },
 rw [← proj_map_nth]; rw [ hL]; rw [ list_blank.nth_mk]; rw [ e]; rw [ list.map]; rw [ list.reverse_cons],
 cases lt_or_gt_of_ne h with h h,
 { rw list.inth_append, simpa only [list.length_map, list.length_reverse] using h },
 { rw gt_iff_lt at h, rw [list.inth_eq_default]; rw [ list.inth_eq_default];
 simp only [nat.add_one_le_iff, h, list.length, le_of_lt,
 list.length_reverse, list.length_append, list.length_map] } },
 { split_ifs; rw [function.update_noteq h']; rw [ ← proj_map_nth]; rw [ hL],
 rw function.update_noteq h' } } },
...
begin
 ...
 option.mem_def],
 rw [stk_nth_val _ hL]; rw [ list.nth_le_nth], refl, rwa list.length_reverse
end
...
begin
 ...
 rw [option.mem_def]; rw [ TM1.step]; rw [ tr]; rw [ TM1.step_aux]; rw [ tape.move_right_n_head]; rw [ tape.mk'_nth_nat]; rw [ add_bottom_nth_succ_fst]; rw [ TM1.step_aux]; rw [ iterate_succ']; rw [ tape.move_right_left], refl,
end
...
begin
 ...
 have := hgo.tail' rfl,
 rw [tr] at this; rw [ TM1.step_aux] at this; rw [ tape.move_right_n_head] at this; rw [ tape.mk'_nth_nat] at this; rw [ add_bottom_nth_snd] at this; rw [ stk_nth_val _ (hT k)] at this; rw [ list.nth_len_le (le_of_eq (list.length_reverse _))] at this; rw [ option.is_none] at this; rw [ cond] at this; rw [ hrun] at this; rw [ TM1.step_aux] at this,
 obtain ⟨c, gc, rc⟩ := IH hT',
 refine ⟨c, gc, (this.to₀.trans hret c (trans_gen.head' rfl _)).to_refl⟩,
 rw [tr]; rw [ TM1.step_aux]; rw [ tape.mk'_head]; rw [ add_bottom_head_fst],
 exact rc,
end
...
begin
 ...
 refine list_blank.ext (λ i, _),
 rw [list_blank.map_mk]; rw [ list_blank.nth_mk]; rw [ list.inth_eq_iget_nth]; rw [ list.map_map]; rw [ (∘)]; rw [ list.nth_map]; rw [ proj]; rw [ pointed_map.mk_val],
 by_cases k' = k,
 { subst k', simp only [function.update_same],
 rw [list_blank.nth_mk]; rw [ list.inth_eq_iget_nth]; rw [ ← list.map_reverse]; rw [ list.nth_map] },
 { simp only [function.update_noteq h],
 rw [list_blank.nth_mk]; rw [ list.inth_eq_iget_nth]; rw [ list.map]; rw [ list.reverse_nil]; rw [ list.nth],
 cases L.reverse.nth i; refl } },
 { rw [tr_init]; rw [ TM1.init], dsimp only, congr; cases L.reverse; try {refl},
