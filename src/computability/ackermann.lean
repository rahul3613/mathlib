...
begin
 cases n,
 { simp },
 { rw [ack_succ_succ]; rw [ succ_eq_add_one],
...
begin
 ...
 { cases k,
 { norm_num },
 { rw [succ_eq_add_one]; rw [ add_sq]; rw [ pow_succ 2]; rw [ two_mul (2 ^ _)]; rw [ add_tsub_assoc_of_le]; rw [ add_comm (2 ^ _)]; rw [ add_assoc],
 { apply add_le_add hk,
 norm_num,
 apply succ_le_of_lt,
 rw [pow_succ]; rw [ mul_lt_mul_left (zero_lt_two' ℕ)],
 apply lt_two_pow },
...
| (m + 1) 0 := by { rw [ack_succ_zero]; rw [ ack_succ_zero], apply ack_add_one_sq_lt_ack_add_three }
| (m + 1) (n + 1) := begin
 rw [ack_succ_succ]; rw [ ack_succ_succ],
...
begin
 ...
 { rw [max_eq_left h₂.le]; rw [ add_assoc],
 ...
 rw [add_succ m]; rw [ add_succ _ 8]; rw [ ack_succ_succ (_ + 8)]; rw [ add_assoc],
