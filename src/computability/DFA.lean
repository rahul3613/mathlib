...
begin
 ...
 { rw [list.take_append_drop]; rw [ list.take_append_drop] },
 ...
 { rw [list.take_take]; rw [ min_eq_left hle]; rw [ ←eval_from_of_append]; rw [ heq]; rw [ ←min_eq_left hle]; rw [ ←list.take_take]; rw [ min_eq_left hle]; rw [ list.take_append_drop] },

 use hq,
 rwa [←hq]; rwa [ ←eval_from_of_append]; rwa [ ←eval_from_of_append]; rwa [ ←list.append_assoc]; rwa [ list.take_append_drop]; rwa [ list.take_append_drop]
end
...
begin
 ...
 rw [list.join]; rw [ eval_from_of_append]; rw [ ha]; rw [ hx],
 apply ih,
 intros z hz,
...
begin
 ...
 rwa [mem_accepts]; rwa [ eval_from_of_append]; rwa [ eval_from_of_append]; rwa [ h]; rwa [ hc]
end
