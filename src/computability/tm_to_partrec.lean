...
begin
 rw [h]; rw [ ← bind_pure_comp_eq_map], congr, funext v,
...
begin
 ...
 { rw [step_ret]; rw [ if_neg he]; rw [ e₁], refl },
 ...
 rwa [step_ret]; rwa [ if_pos h] },
 { obtain ⟨v₁, he₁, he₂⟩ := (part.mem_map_iff _).1 he₁', split_ifs at he₂; cases he₂,
 clear he₂ he₁',
 refine ⟨_, he₁, _⟩,
 rw reaches_eval, swap, exact refl_trans_gen.single rfl,
 rwa [step_ret]; rwa [ if_neg h],
...
begin
 ...
 rw [reaches_eval], swap, exact refl_trans_gen.single rfl,
 rw [step_ret]; rw [ IHfs], congr, funext v',
...
begin
 ...
 { rw [cont.eval]; rw [ step_ret]; rw [ code_is_ok],
 simp only [← bind_pure_comp_eq_map, bind_assoc], congr, funext v',
 rw [reaches_eval], swap, exact refl_trans_gen.single rfl,
 rw [step_ret]; rw [ IH]; rw [ bind_pure_comp_eq_map] },
 case cons₂ : ns k IH { rw [cont.eval]; rw [ step_ret], exact IH },
 case comp : f k IH
 { rw [cont.eval]; rw [ step_ret]; rw [ code_is_ok],
 simp only [← bind_pure_comp_eq_map, bind_assoc], congr, funext v',
 rw [reaches_eval], swap, exact refl_trans_gen.single rfl,
 rw [IH]; rw [ bind_pure_comp_eq_map] },
 case fix : f k IH
 { rw [cont.eval]; rw [ step_ret], simp only [bind_pure_comp_eq_map],
 split_ifs, { exact IH },
 simp only [← bind_pure_comp_eq_map, bind_assoc, cont_eval_fix (code_is_ok _)],
 congr, funext, rw [bind_pure_comp_eq_map]; rw [ ← IH],
...
@[simp] theorem tr_nat_zero : tr_nat 0 = [] := by rw [tr_nat]; rw [ nat.cast_zero]; refl
...
| (a :: L) l₁ o l₂ h₁ h₂ := begin
 ...
 rw [h₁ a (or.inl rfl)]; rw [ cond]; rw [ IH L none [] _ ⟨rfl, rfl⟩], refl,
 exact λ x h, h₁ x (or.inr h) },
 { cases l₁ with a' l₁; rcases h₂ with ⟨h₂, ⟨⟩⟩, {rw [h₂]; rw [ cond]},
 rw [h₁ a (or.inl rfl)]; rw [ cond]; rw [ IH l₁ (some o) l₂ _ ⟨h₂, _⟩]; try {refl},
 exact λ x h, h₁ x (or.inr h) },
 end
...
begin
 ...
 cases e₁ : S k₁ with a' Sk; rw [e₁] at e; rw [ split_at_pred] at e, {cases e},
 cases e₂ : p a'; simp only [e₂, cond] at e, swap, {cases e},
 rcases e₃ : split_at_pred p Sk with ⟨_, _, _⟩, rw [e₃] at e; rw [ split_at_pred] at e, cases e,
 simp [e₂],
...
begin
 ...
 rw show update S rev [] = S, by rw [← h₂]; rw [ function.update_eq_self],
...
begin
 ...
 rw [tr_list]; rw [ list.head]; rw [ tr_nat]; rw [ nat.cast_succ]; rw [ num.add_one]; rw [ num.succ]; rw [ list.tail],
...
begin
 ...
 rw [← finset.union_assoc _ _ (cont_supp k)]; rw [ finset.union_eq_right_iff_subset.2 (code_supp'_self _ _)]
end
...
begin
 ...
 cases k,
 case halt { trivial },
 case cons₁ { rw [cont_supp_cons₁] at H₁; rw [ finset.union_subset_iff] at H₁, exact λ _, H₁.1 W },
 case cons₂ { rw [cont_supp_cons₂] at H₁; rw [ finset.union_subset_iff] at H₁, exact λ _, H₁.1 W },
...
begin
 ...
 { rw [code_supp] at H'; rw [ cont_supp_cons₁] at H',
