...
begin
 ...
 { rw [eval_from_nil]; rw [ ε_closure_empty] },
 { rw [eval_from_append_singleton]; rw [ ih]; rw [ step_set_empty] }
end
...
begin
 ext x,
 rw [accepts]; rw [ NFA.accepts]; rw [ eval]; rw [ NFA.eval]; rw [ ←to_NFA_eval_from_match],
 refl
end
...
begin
 rw [eval_from]; rw [ ε_NFA.eval_from]; rw [ to_ε_NFA_ε_closure],
 congr,
 ext S s,
...
begin
 rw [accepts]; rw [ ε_NFA.accepts]; rw [ eval]; rw [ ε_NFA.eval]; rw [ to_ε_NFA_eval_from_match],
 refl
end
