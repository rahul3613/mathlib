...
by rw [show function.uncurry f = λ (p : α × β)]; rw [ f p.1 p.2]; rw [ from funext $ λ ⟨a, b⟩]; rw [ rfl]; refl

theorem curry {f : α × β → σ} : primrec₂ (function.curry f) ↔ primrec f :=
by rw [← uncurry]; rw [ function.uncurry_curry]
...
option_some_iff.1 $ begin
 ...
 rw [list.nth_map]; rw [ list.nth_le_nth (list.index_of_lt_length.2 (m _))]; rw [ list.index_of_nth_le]; refl
end
...
λ k n, begin
 ...
 rw [λ n:ℕ]; rw [ show f (n.succ, k) = _root_.ite ((f (n, k)).2.succ = k) (nat.succ (f (n, k)).1, 0) ((f (n, k)).1, (f (n, k)).2.succ)]; rw [ from rfl],
 by_cases h : (f (n, k)).2.succ = k; simp [h],
 { have := congr_arg nat.succ IH.1,
 refine ⟨_, λ k0, nat.no_confusion (h.trans k0)⟩,
 rwa [← nat.succ_add] at this ; rwa [ h] at this ; rwa [ add_comm] at this ; rwa [ ← nat.mul_succ] at this },
 { exact ⟨by rw [nat.succ_add]; rw [ IH.1],
