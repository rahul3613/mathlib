...
| (n+4) := let m := n.div2.div2 in
 ...
 begin
 transitivity, swap,
 rw [← nat.bit_decomp n]; rw [ ← nat.bit_decomp n.div2],
...
begin
 ...
 rw [one_mul] at this; rw [ mul_assoc] at this; rw [ ← bit0_eq_two_mul] at this; rw [ ← bit0_eq_two_mul] at this,
...
begin
 ...
 simp [G], rw [list.length_map]; rw [ list.length_range],
 let m := n.div2.div2,
...
by rw [eval]; rw [ nat.unpaired]; rw [ nat.unpair_mkpair]; rw [ nat.elim_zero]
...
begin
 rw [eval]; rw [ nat.unpaired]; rw [ part.bind_eq_bind]; rw [ nat.unpair_mkpair]; rw [ nat.elim_succ],
 simp,
end
...
primrec.nat_strong_rec _ (hG.comp snd).to₂ $
 ...
 rw [hg (nat.mkpair_lt_mkpair_right _ lf)]; rw [ hg (nat.mkpair_lt_mkpair_right _ lg)],
