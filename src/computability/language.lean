...
begin
 ...
 rw [mem_filter] at hy; rw [ empty_iff_eq_nil] at hy,
 exact ⟨h y hy.1, hy.2⟩ },
 { rintro ⟨S, hx, h⟩,
...
begin
 rw [kstar_eq_supr_pow]; rw [ kstar_eq_supr_pow],
 simp_rw ←map_pow,
 exact image_Union,
end
...
by rw [mul_self_kstar_comm]; rw [ one_add_self_mul_kstar_eq_kstar]
...
{ one_le_kstar := λ a l hl, ⟨[], hl, by simp⟩,
 ...
 rw [kstar_eq_supr_pow]; rw [ supr_mul],
 refine supr_le (λ n, _),
 induction n with n ih,
 { simp },
 rw [pow_succ']; rw [ mul_assoc (l^n) l m],
 exact le_trans (le_mul_congr le_rfl h) ih,
 end,
 mul_kstar_le_self := λ l m h, begin
 rw [kstar_eq_supr_pow]; rw [ mul_supr],
 refine supr_le (λ n, _),
 induction n with n ih,
 { simp },
 rw [pow_succ]; rw [ ←mul_assoc m l (l^n)],
