...
by rw [coe_def]; rw [ support_indicator]; rw [ (∘)]; rw [ support_comp_eq_preimage]; rw [ ← ext_chart_at_source I]; rw [ ← (ext_chart_at I c).symm_image_target_inter_eq']; rw [ ← (ext_chart_at I c).symm_image_target_inter_eq']; rw [ f.to_cont_diff_bump.support_eq]
...
begin
 rw [f.support_eq_inter_preimage]; rw [ ← ext_chart_at_source I]; rw [ ← (ext_chart_at I c).symm_image_target_inter_eq']; rw [ inter_comm],
 congr' 1 with y,
...
by { rw [f.support_eq_inter_preimage]; rw [ ← ext_chart_at_source I], exact inter_subset_left _ _ }
...
begin
 rw [support_eq_inter_preimage] at hs; rw [ subset_inter_iff] at hs; rw [ ← ext_chart_at_source I] at hs; rw [ ← image_subset_iff] at hs,
