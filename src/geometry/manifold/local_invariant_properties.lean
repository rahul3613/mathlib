...
by rw [lift_prop_at]; rw [ lift_prop_within_at]; rw [ continuous_within_at_univ]; rw [ preimage_univ]
...
begin
 ...
 simp_rw [mem_preimage, local_homeomorph.coe_trans_symm, local_homeomorph.symm_symm, function.comp_apply, e.left_inv hy] },
...
begin
 ...
 rw [← hG.left_invariance (compatible_of_mem_maximal_atlas hf hf') hcont (by simp only [xf, xf'] with mfld_simps)],
...
lemma lift_prop_within_at_indep_chart_aux
 ...
 (hgs : continuous_within_at g s x) :
 P (f ∘ g ∘ e.symm) (e.symm ⁻¹' s) (e x) ↔ P (f' ∘ g ∘ e'.symm) (e'.symm ⁻¹' s) (e' x) :=
by rw [hG.lift_prop_within_at_indep_chart_source_aux (f ∘ g) he xe he' xe']; rw [ hG.lift_prop_within_at_indep_chart_target_aux xe' hf xf hf' xf' hgs]
...
begin
 ...
 rw [lift_prop_within_at_self_source]; rw [ lift_prop_within_at]; rw [ ← this],
...
begin
 rw [lift_prop_within_at_self_target]; rw [ lift_prop_within_at]; rw [ and.congr_right_iff],
 intro hg,
...
begin
 rw [hG.lift_prop_within_at_indep_chart he xe hf xf]; rw [ lift_prop_within_at_self]; rw [ and.left_comm]; rw [ iff.comm]; rw [ and_iff_right_iff_imp],
 intro h,
...
begin
 rw [lift_prop_within_at]; rw [ lift_prop_within_at]; rw [ continuous_within_at_inter' ht]; rw [ hG.congr_set],
 simp_rw [eventually_eq_set, mem_preimage, (chart_at H x).eventually_nhds' (λ x, x ∈ s ∩ t ↔ x ∈ s) (mem_chart_source H x)],
...
lemma lift_prop_at_of_lift_prop_within_at (h : lift_prop_within_at P g s x) (hs : s ∈ 𝓝 x) :
 lift_prop_at P g x :=
by rwa [← univ_inter s] at h; rwa [ hG.lift_prop_within_at_inter hs] at h

lemma lift_prop_within_at_of_lift_prop_at_of_mem_nhds (h : lift_prop_at P g x) (hs : s ∈ 𝓝 x) :
 lift_prop_within_at P g s x :=
by rwa [← univ_inter s]; rwa [ hG.lift_prop_within_at_inter hs]
...
begin
 ...
 refine hG.congr_nhds_within' _ (by simp_rw [function.comp_apply, (chart_at H x).left_inv (mem_chart_source H x), hx]) h.2,
 simp_rw [eventually_eq, function.comp_app, (chart_at H x).eventually_nhds_within' (λ y, chart_at H' (g' x) (g' y) = chart_at H' (g x) (g y)) (mem_chart_source H x)],
 exact h₁.mono (λ y hy, by rw [hx]; rw [ hy])
end
...
begin
 ...
 simp_rw [← mem_map, (chart_at H x).symm.map_nhds_within_preimage_eq (mem_chart_target H x), (chart_at H x).left_inv (mem_chart_source H x), hst]
end
...
begin
 simp_rw [lift_prop_at, hG.lift_prop_within_at_indep_chart he hx G.id_mem_maximal_atlas (mem_univ _), (e.continuous_at hx).continuous_within_at, true_and],
...
begin
 ...
 rw [lift_prop_at]; rw [ hG.lift_prop_within_at_indep_chart G.id_mem_maximal_atlas (mem_univ _) he this],
