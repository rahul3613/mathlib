...
by { rw [← image_univ]; rw [ ← I.source_eq], exact (I.to_local_equiv.image_source_eq_target).symm }
...
begin
 ...
 { rw [inter_comm]; rw [ I.target_eq]; rw [ I.to_local_equiv_coe_symm] }
end
...
by rw [← I.map_nhds_within_eq]; rw [ map_map]; rw [ I.symm_comp_self]; rw [ map_id]

lemma symm_map_nhds_within_range (x : H) :
 map I.symm (𝓝[range I] (I x)) = 𝓝 x :=
by rw [← I.map_nhds_eq]; rw [ map_map]; rw [ I.symm_comp_self]; rw [ map_id]
...
begin
 ...
 rwa [function.comp.assoc] at this ; rwa [ I.symm_comp_self] at this },
...
begin
 ...
 rw [← prod_range_range_eq]; rw [ model_with_corners.boundaryless.range_eq_univ]; rw [ model_with_corners.boundaryless.range_eq_univ]; rw [ univ_prod_univ]
end
...
begin
 rw [cont_diff_groupoid]; rw [ cont_diff_groupoid],
...
begin
 ...
 rw [cont_diff_groupoid]; rw [ mem_groupoid_of_pregroupoid],
...
{ compatible :=
 ...
 rw [local_homeomorph.prod_symm]; rw [ local_homeomorph.prod_trans],
...
by rw [extend]; rw [ local_equiv.trans_source]; rw [ I.source_eq]; rw [ preimage_univ]; rw [ inter_univ]
...
begin
 rw [maps_to']; rw [ extend_coe]; rw [ extend_coe_symm]; rw [ preimage_comp]; rw [ ← I.image_eq]; rw [ image_comp]; rw [ f.image_eq_target_inter_inv_preimage hs],
...
by rwa [extend_coe]; rwa [ (∘)]; rwa [ ← I.map_nhds_eq]; rwa [ ← f.map_nhds_eq]; rwa [ map_map]

lemma extend_target_mem_nhds_within {y : M} (hy : y ∈ f.source) :
 (f.extend I).target ∈ 𝓝[range I] (f.extend I y) :=
begin
 rw [← local_equiv.image_source_eq_target]; rw [ ← map_extend_nhds f I hy],
...
by rw [map_extend_nhds_within_eq_image f I hy]; rw [ nhds_within_inter]; rw [ ← nhds_within_extend_target_eq _ _ hy]; rw [ ← nhds_within_inter]; rw [ (f.extend I).image_source_inter_eq']; rw [ inter_comm]

lemma map_extend_symm_nhds_within {y : M} (hy : y ∈ f.source) :
 map (f.extend I).symm
 (𝓝[(f.extend I).symm ⁻¹' s ∩ range I] (f.extend I y)) = 𝓝[s] y :=
begin
 rw [← map_extend_nhds_within f I hy]; rw [ map_map]; rw [ map_congr]; rw [ map_id],
...
by rw [← nhds_within_univ]; rw [ ← map_extend_symm_nhds_within f I hy]; rw [ preimage_univ]; rw [ univ_inter]
...
by rwa [← map_extend_symm_nhds_within f I h] at ht; rwa [ mem_map] at ht
...
lemma extend_coord_change_source :
 ...
 I '' (f.symm ≫ₕ f').source :=
by { simp_rw [local_equiv.trans_source, I.image_eq, extend_source, local_equiv.symm_source, extend_target, inter_right_comm _ (range I)], refl }
...
lemma ext_chart_at_preimage_mem_nhds_within' {x' : M} (h : x' ∈ (ext_chart_at I x).source)
 ...
 𝓝[(ext_chart_at I x).symm ⁻¹' s ∩ range I] ((ext_chart_at I x) x') :=
by rwa [← map_ext_chart_at_symm_nhds_within' I x h] at ht; rwa [ mem_map] at ht
