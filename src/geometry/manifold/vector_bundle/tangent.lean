...
{ base_set := λ i, i.1.source,
 ...
 rw [filter.eventually_eq.fderiv_within_eq]; rw [ fderiv_within_id']; rw [ continuous_linear_map.id_apply],
 ...
 rw [fderiv_within_fderiv_within]; rw [ filter.eventually_eq.fderiv_within_eq],
 ...
 { exact I.unique_diff_at_image },
 { rw [function.comp_apply]; rw [ i.1.extend_left_inv I hxi] }
 end }
...
begin
 rw [tangent_bundle.trivialization_at_symmL]; rw [ coord_change_model_space],
 apply mem_univ
end
...
lemma continuous_linear_map_at_model_space (b b' : F) :
 ...
 (1 : F →L[𝕜] F) :=
begin
 rw [tangent_bundle.trivialization_at_continuous_linear_map_at]; rw [ coord_change_model_space],
 apply mem_univ
end
...
begin
 refine ⟨λ i j, _⟩,
 rw [smooth_on]; rw [ cont_mdiff_on_iff_source_of_mem_maximal_atlas (subset_maximal_atlas I i.2)]; rw [ cont_mdiff_on_iff_cont_diff_on],
...
begin
 ...
 exact (tangent_bundle_core I H).coord_change_self (achart _ x.1) x.1
 (mem_achart_source H x.1) x.2 },
 simp_rw [tangent_bundle.chart_at, fiber_bundle_core.local_triv, fiber_bundle_core.local_triv_as_local_equiv, vector_bundle_core.to_fiber_bundle_core_base_set, tangent_bundle_core_base_set],
...
begin
 ...
 { exact mem_univ x },
 { exact mem_univ y },
 simp_rw [tangent_bundle_core_index_at, tangent_bundle_core_coord_change_model_space, continuous_linear_map.id_comp, continuous_linear_map.comp_id]
end
