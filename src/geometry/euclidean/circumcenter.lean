...
begin
 rw [←mul_self_inj_of_nonneg dist_nonneg dist_nonneg]; rw [ ←mul_self_inj_of_nonneg dist_nonneg dist_nonneg]; rw [ dist_sq_eq_dist_orthogonal_projection_sq_add_dist_orthogonal_projection_sq p3 hp1]; rw [ dist_sq_eq_dist_orthogonal_projection_sq_add_dist_orthogonal_projection_sq p3 hp2],
 simp
end
...
begin
 ...
 cases hp1,
 { rw hp1,
 rw [hpo]; rw [ dist_sq_smul_orthogonal_vadd_smul_orthogonal_vadd (orthogonal_projection_mem p) hcc _ _ (vsub_orthogonal_projection_mem_direction_orthogonal s p)]; rw [ ←dist_eq_norm_vsub V p]; rw [ dist_comm _ cc],
 field_simp [hy0],
 ring },
 { rw [dist_sq_eq_dist_orthogonal_projection_sq_add_dist_orthogonal_projection_sq _ (hps hp1)]; rw [ orthogonal_projection_vadd_smul_vsub_orthogonal_projection _ _ hcc]; rw [ subtype.coe_mk]; rw [ dist_of_mem_subset_mk_sphere hp1 hcr]; rw [ dist_eq_norm_vsub V cc₂ cc]; rw [ vadd_vsub]; rw [ norm_smul]; rw [ ←dist_eq_norm_vsub V]; rw [ real.norm_eq_abs]; rw [ abs_div]; rw [ abs_of_nonneg dist_nonneg]; rw [ div_mul_cancel _ hy0]; rw [ abs_mul_abs_self] } } },
 ...
 rw [exists_dist_eq_iff_exists_dist_orthogonal_projection_eq hps cc₃] at hcr₃'; rw [ hcc₃''] at hcr₃'; rw [ orthogonal_projection_vadd_smul_vsub_orthogonal_projection _ _ hcc₃'] at hcr₃',
 ...
 rw [←dist_of_mem_subset_mk_sphere (set.mem_insert_of_mem _ hp0) hcr₃]; rw [ hcc₃'']; rw [ ←mul_self_inj_of_nonneg dist_nonneg (real.sqrt_nonneg _)]; rw [ real.mul_self_sqrt (add_nonneg (mul_self_nonneg _) (mul_self_nonneg _))]; rw [ dist_sq_eq_dist_orthogonal_projection_sq_add_dist_orthogonal_projection_sq _ (hps hp0)]; rw [ orthogonal_projection_vadd_smul_vsub_orthogonal_projection _ _ hcc₃']; rw [ h']; rw [ dist_of_mem_subset_mk_sphere hp0 hcr]; rw [ dist_eq_norm_vsub V _ cc₃']; rw [ vadd_vsub]; rw [ norm_smul]; rw [ ←dist_eq_norm_vsub V p]; rw [ real.norm_eq_abs]; rw [ ←mul_assoc]; rw [ mul_comm _ (|t₃|)]; rw [ ←mul_assoc]; rw [ abs_mul_abs_self],
 ring },
 replace hcr₃ := dist_of_mem_subset_mk_sphere (set.mem_insert _ _) hcr₃,
 rw [hpo] at hcr₃; rw [ hcc₃''] at hcr₃; rw [ hcr₃val] at hcr₃; rw [ ←mul_self_inj_of_nonneg dist_nonneg (real.sqrt_nonneg _)] at hcr₃; rw [ dist_sq_smul_orthogonal_vadd_smul_orthogonal_vadd (orthogonal_projection_mem p) hcc₃' _ _ (vsub_orthogonal_projection_mem_direction_orthogonal s p)] at hcr₃; rw [ dist_comm] at hcr₃; rw [ ←dist_eq_norm_vsub V p] at hcr₃; rw [ real.mul_self_sqrt (add_nonneg (mul_self_nonneg _) (mul_self_nonneg _))] at hcr₃,
 change x * x + _ * (y * y) = _ at hcr₃,
 rw [(show x * x + (1 - t₃) * (1 - t₃) * (y * y) = x * x + y * y - 2 * y * (t₃ * y) + t₃ * y * (t₃ * y), by ring)] at hcr₃; rw [ add_left_inj] at hcr₃,
 have ht₃ : t₃ = ycc₂ / y,
 { field_simp [←hcr₃, hy0],
 ring },
 subst ht₃,
...
begin
 ...
 simp_rw eq_comm,
 rw [finset.filter_eq]; rw [ if_pos (finset.mem_univ _)]; rw [ finset.card_sdiff (finset.subset_univ _)]; rw [ finset.card_singleton]; rw [ finset.card_univ]; rw [ hn],
 simp },
 { simp } },
 ...
 congr' with j,
 simp [classical.em] },
 rw [hr]; rw [ ←affine_span_insert_affine_span],
...
begin
 ...
 rw [set.range_unique] at h; rw [ mem_affine_span_singleton] at h,
 rw h,
 congr
end
...
begin
 ...
 rw [finset.centroid_pair_fin]; rw [ dist_eq_norm_vsub V (s.points i)]; rw [ dist_eq_norm_vsub V (s.points j)]; rw [ vsub_vadd_eq_vsub_sub]; rw [ vsub_vadd_eq_vsub_sub]; rw [ ←one_smul ℝ (s.points i -ᵥ s.points 0)]; rw [ ←one_smul ℝ (s.points j -ᵥ s.points 0)],
...
begin
 rw [pseudo_metric_space.dist_comm p2 _]; rw [ dist_eq_norm_vsub V p1 _]; rw [ dist_eq_norm_vsub V p1 _]; rw [ dist_eq_norm_vsub V _ p2]; rw [ ← vsub_add_vsub_cancel p1 (s.orthogonal_projection_span p2) p2]; rw [ norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero],
...
begin
 rw [dist_comm]; rw [ ←h₁ 0]; rw [ s.dist_sq_eq_dist_orthogonal_projection_sq_add_dist_orthogonal_projection_sq p₁ h],
...
begin
 ...
 rw [←h] at hi; rw [ set.mem_range] at hi,
 rcases hi with ⟨j, hj⟩,
...
begin
 simp_rw [sum_points_with_circumcenter, reflection_circumcenter_weights_with_circumcenter, sum_ite, sum_const, filter_or, filter_eq'],
 rw card_union_eq,
 { simp },
...
begin
 ...
 simp },
 rw [euclidean_geometry.reflection_apply]; rw [ h_faces]; rw [ s.orthogonal_projection_circumcenter hc]; rw [ circumcenter_eq_centroid]; rw [ s.face_centroid_eq_centroid hc]; rw [ centroid_eq_affine_combination_of_points_with_circumcenter]; rw [ circumcenter_eq_affine_combination_of_points_with_circumcenter]; rw [ ←@vsub_eq_zero_iff_eq V]; rw [ affine_combination_vsub]; rw [ weighted_vsub_vadd_affine_combination]; rw [ affine_combination_vsub]; rw [ weighted_vsub_apply]; rw [ sum_points_with_circumcenter],
 simp_rw [pi.sub_apply, pi.add_apply, pi.sub_apply, sub_smul, add_smul, sub_smul, centroid_weights_with_circumcenter, circumcenter_weights_with_circumcenter, reflection_circumcenter_weights_with_circumcenter, ite_smul, zero_smul, sub_zero, apply_ite2 (+), add_zero, ←add_smul, hc, zero_sub, neg_smul, sub_self, add_zero],
...
begin
 ...
 rw [←finrank_top] at hd; rw [ ←direction_top ℝ V P] at hd,
...
begin
 ...
 have h₂' := s.orthogonal_projection_eq_circumcenter_of_dist_eq h₂,
 rw [←affine_span_insert_affine_span] at hp₁ hp₂; rw [ mem_affine_span_insert_iff (orthogonal_projection_mem p)] at hp₁ hp₂,
 ...
 r * r - s.circumradius * s.circumradius :=
 s.dist_circumcenter_sq_eq_sq_sub_circumradius h₂ h₂' h,
 rw [←hd₂] at hd₁; rw [ hp₁] at hd₁; rw [ hp₂] at hd₁; rw [ dist_eq_norm_vsub V _ s.circumcenter] at hd₁; rw [ dist_eq_norm_vsub V _ s.circumcenter] at hd₁; rw [ vadd_vsub] at hd₁; rw [ vadd_vsub] at hd₁; rw [ ←real_inner_self_eq_norm_mul_norm] at hd₁; rw [ ←real_inner_self_eq_norm_mul_norm] at hd₁; rw [ real_inner_smul_left] at hd₁; rw [ real_inner_smul_left] at hd₁; rw [ real_inner_smul_right] at hd₁; rw [ real_inner_smul_right] at hd₁; rw [ ←mul_assoc] at hd₁; rw [ ←mul_assoc] at hd₁,
 ...
 rw [mul_left_inj' hz] at hd₁; rw [ mul_self_eq_mul_self_iff] at hd₁,
 rw [hp₁]; rw [ hp₂],
 cases hd₁,
 { left,
 rw hd₁ },
 { right,
 rw [hd₁]; rw [ reflection_vadd_smul_vsub_orthogonal_projection p r₂ s.circumcenter_mem_affine_span]; rw [ neg_smul] } }
end
