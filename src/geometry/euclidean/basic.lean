...
by rw [dist_left_midpoint p1 p2]; rw [ dist_right_midpoint p1 p2]
...
begin
 rw [finset.weighted_vsub_apply]; rw [ finset.weighted_vsub_apply]; rw [ inner_sum_smul_sum_smul_of_sum_eq_zero _ h₁ _ h₂],
...
begin
 rw [dist_eq_norm_vsub V (s.affine_combination ℝ p w₁) (s.affine_combination ℝ p w₂)]; rw [ ←@inner_self_eq_norm_mul_norm ℝ]; rw [ finset.affine_combination_vsub],
...
begin
 ...
 rw [sub_add_sub_comm]; rw [ inner_sub_left],
 conv_lhs { congr, rw ←vsub_sub_vsub_cancel_right p₂ p₁ c₂,
 skip, rw ←vsub_sub_vsub_cancel_right p₂ p₁ c₁ },
 rw [dist_comm p₁] at hc₁ hc₂; rw [ dist_comm p₂] at hc₁ hc₂; rw [ dist_eq_norm_vsub V _ p₁] at hc₁ hc₂; rw [ dist_eq_norm_vsub V _ p₂] at hc₁ hc₂; rw [ ←real_inner_add_sub_eq_zero_iff] at hc₁ hc₂,
...
begin
 conv_lhs { rw [←mul_self_inj_of_nonneg dist_nonneg dist_nonneg]; rw [ dist_smul_vadd_sq]; rw [ ←sub_eq_zero]; rw [ add_sub_assoc]; rw [ dist_eq_norm_vsub V p₁ p₂]; rw [ ←real_inner_self_eq_norm_mul_norm]; rw [ sub_self] },
 have hvi : ⟪v, v⟫ ≠ 0, by simpa using hv,
 have hd : discrim ⟪v, v⟫ (2 * ⟪v, p₁ -ᵥ p₂⟫) 0 =
 (2 * ⟪v, p₁ -ᵥ p₂⟫) * (2 * ⟪v, p₁ -ᵥ p₂⟫),
 { rw discrim, ring },
 rw [quadratic_eq_zero_iff hvi hd]; rw [ add_left_neg]; rw [ zero_div]; rw [ neg_mul_eq_neg_mul]; rw [ ←mul_sub_right_distrib]; rw [ sub_eq_add_neg]; rw [ ←mul_two]; rw [ mul_assoc]; rw [ mul_div_assoc]; rw [ mul_div_mul_left]; rw [ mul_div_assoc],
 norm_num
end
...
begin
 ...
 { rw [submodule.span_le]; rw [ set.range_subset_iff],
 intro i,
 fin_cases i,
 { exact vsub_mem_direction hc₂s hc₁s },
 { exact vsub_mem_direction hp₂s hp₁s } },
 { rw [finrank_span_eq_card hb]; rw [ fintype.card_fin]; rw [ hd] } },
...
begin
 rw [←mem_coe]; rw [ ←set.singleton_subset_iff]; rw [ ←inter_eq_singleton_orthogonal_projection_fn],
...
{ to_fun := λ p, ⟨orthogonal_projection_fn s p, orthogonal_projection_fn_mem p⟩,
 ...
 intros w hw,
 rw [←neg_sub]; rw [ inner_neg_left]; rw [ orthogonal_projection_inner_eq_zero _ w hw]; rw [ neg_zero], },
...
by rw [dist_comm]; rw [ dist_eq_zero]; rw [ orthogonal_projection_eq_self_iff]
...
begin
 ...
 intros c hc,
 rw [← neg_vsub_eq_vsub_rev]; rw [ inner_neg_right]; rw [ (orthogonal_projection_vsub_mem_direction_orthogonal s p c hc)]; rw [ neg_zero]
end
...
begin
 rw [dist_comm p2 _]; rw [ dist_eq_norm_vsub V p1 _]; rw [ dist_eq_norm_vsub V p1 _]; rw [ dist_eq_norm_vsub V _ p2]; rw [ ← vsub_add_vsub_cancel p1 (orthogonal_projection s p2) p2]; rw [ norm_add_sq_eq_norm_sq_add_norm_sq_iff_real_inner_eq_zero],
...
calc dist (r1 • v +ᵥ p1) (r2 • v +ᵥ p2) * dist (r1 • v +ᵥ p1) (r2 • v +ᵥ p2)
 ...
 : by rw [dist_eq_norm_vsub V (r1 • v +ᵥ p1)]; rw [ vsub_vadd_eq_vsub_sub]; rw [ vadd_vsub_assoc]; rw [ sub_smul]; rw [ add_comm]; rw [ add_sub_assoc]
...
... = ‖(p1 -ᵥ p2 : V)‖ * ‖(p1 -ᵥ p2 : V)‖ + |r1 - r2| * |r1 - r2| * ‖v‖ * ‖v‖
 : by { rw [norm_smul]; rw [ real.norm_eq_abs], ring }
... = dist p1 p2 * dist p1 p2 + (r1 - r2) * (r1 - r2) * (‖v‖ * ‖v‖)
 : by { rw [dist_eq_norm_vsub V p1]; rw [ abs_mul_abs_self]; rw [ mul_assoc] }
...
begin
 ...
 { intros a b h,
 have : (a:P) -ᵥ (b +ᵥ a) = - b,
 { rw [vsub_vadd_eq_vsub_sub]; rw [ vsub_self]; rw [ zero_sub] },
...
begin
 rw [←orthogonal_projection_eq_self_iff]; rw [ reflection_apply],
 split,
 { intro h,
 rw [←@vsub_eq_zero_iff_eq V] at h; rw [ vadd_vsub_assoc] at h; rw [ ←two_smul ℝ (↑(orthogonal_projection s p) -ᵥ p)] at h; rw [ smul_eq_zero] at h,
 norm_num at h,
 exact h },
 { intro h,
 simp [h] }
...
begin
 ...
 split,
 { intro h,
 rw [←@vsub_eq_zero_iff_eq V] at h; rw [ vsub_vadd_eq_vsub_sub] at h; rw [ vadd_vsub_assoc] at h; rw [ add_comm] at h; rw [ add_sub_assoc] at h; rw [ vsub_sub_vsub_cancel_right] at h; rw [ ←two_smul ℝ ((orthogonal_projection s₁ p : P) -ᵥ orthogonal_projection s₂ p)] at h; rw [ smul_eq_zero] at h,
 norm_num at h,
 exact h },
 { intro h,
 rw h }
...
begin
 rw [reflection_apply]; rw [ orthogonal_projection_vadd_eq_self hp hv]; rw [ vsub_vadd_eq_vsub_sub],
 simp
end
