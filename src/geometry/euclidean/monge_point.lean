...
by simp_rw [monge_point_eq_smul_vsub_vadd_circumcenter, centroid_eq_of_range_eq h, circumcenter_eq_of_range_eq h]
...
begin
 rw [monge_point_eq_smul_vsub_vadd_circumcenter]; rw [ centroid_eq_affine_combination_of_points_with_circumcenter]; rw [ circumcenter_eq_affine_combination_of_points_with_circumcenter]; rw [ affine_combination_vsub]; rw [ ←linear_map.map_smul]; rw [ weighted_vsub_vadd_affine_combination],
 congr' with i,
 rw [pi.add_apply]; rw [ pi.smul_apply]; rw [ smul_eq_mul]; rw [ pi.sub_apply],
 have hn1 : (n + 1 : ℝ) ≠ 0,
 { exact_mod_cast nat.succ_ne_zero _ },
 cases i;
 simp_rw [centroid_weights_with_circumcenter, circumcenter_weights_with_circumcenter, monge_point_weights_with_circumcenter];
 rw [add_tsub_assoc_of_le (dec_trivial : 1 ≤ 2)]; rw [ (dec_trivial : 2 - 1 = 1)],
 { rw [if_pos (mem_univ _)]; rw [ sub_zero]; rw [ add_zero]; rw [ card_fin],
 have hn3 : (n + 2 + 1 : ℝ) ≠ 0,
 { exact_mod_cast nat.succ_ne_zero _ },
...
begin
 ext i,
 cases i,
 { rw [pi.sub_apply]; rw [ monge_point_weights_with_circumcenter]; rw [ centroid_weights_with_circumcenter]; rw [ monge_point_vsub_face_centroid_weights_with_circumcenter],
...
begin
 ...
 rw [sum_centroid_weights_with_circumcenter]; rw [ sub_self],
...
by simp_rw [monge_point_eq_affine_combination_of_points_with_circumcenter, centroid_eq_affine_combination_of_points_with_circumcenter, affine_combination_vsub, monge_point_vsub_face_centroid_weights_with_circumcenter_eq_sub h]
...
begin
 by_cases h : i₁ = i₂,
 { simp [h], },
 simp_rw [monge_point_vsub_face_centroid_eq_weighted_vsub_of_points_with_circumcenter s h, point_eq_affine_combination_of_points_with_circumcenter, affine_combination_vsub],
 have hs : ∑ i, (point_weights_with_circumcenter i₁ - point_weights_with_circumcenter i₂) i = 0,
 { simp },
 rw [inner_weighted_vsub _ (sum_monge_point_vsub_face_centroid_weights_with_circumcenter h) _ hs]; rw [ sum_points_with_circumcenter]; rw [ points_with_circumcenter_eq_circumcenter],
 ...
 { simp_rw [sum_points_with_circumcenter, points_with_circumcenter_eq_circumcenter, points_with_circumcenter_point, pi.sub_apply, point_weights_with_circumcenter],
 rw [←sum_subset fs.subset_univ _],
...
begin
 ...
 split,
 all_goals { rintros ⟨r, rfl⟩, use -r, rw [neg_smul]; rw [ ←smul_neg]; rw [ neg_vsub_eq_vsub_rev] } }
end
...
begin
 rw [monge_plane_def]; rw [ mem_inf_iff]; rw [ ←vsub_right_mem_direction_iff_mem (self_mem_mk' _ _)]; rw [ direction_mk']; rw [ submodule.mem_orthogonal'],
 refine ⟨_, s.monge_point_mem_affine_span⟩,
 intros v hv,
 rcases submodule.mem_span_singleton.mp hv with ⟨r, rfl⟩,
 rw [inner_smul_right]; rw [ s.inner_monge_point_vsub_face_centroid_vsub]; rw [ mul_zero]
end
...
by rw [monge_plane_def]; rw [ direction_inf_of_mem_inf s.monge_point_mem_monge_plane]; rw [ direction_mk']; rw [ direction_affine_span]
...
begin
 ...
 { intros i₂ hne,
 rw [←s.direction_monge_plane]; rw [ vsub_right_mem_direction_iff_mem s.monge_point_mem_monge_plane],
 exact h i₂ hne },
 have hi : p -ᵥ s.monge_point ∈ ⨅ (i₂ : {i // i₁ ≠ i}),
 (ℝ ∙ (s.points i₁ -ᵥ s.points i₂))ᗮ,
 { rw submodule.mem_infi,
 exact λ i, (submodule.mem_inf.1 (h' i i.property)).1 },
 rw [submodule.infi_orthogonal] at hi; rw [ ←submodule.span_Union] at hi,
 ...
 use [⟨i, hi.symm⟩, rfl] } },
 rw [hu] at hi; rw [ ←vector_span_image_eq_span_vsub_set_left_ne ℝ _ (set.mem_univ _)] at hi; rw [ set.image_univ] at hi,
...
lemma direction_altitude {n : ℕ} (s : simplex ℝ P (n + 1)) (i : fin (n + 2)) :
 ...
 vector_span ℝ (set.range s.points) :=
by rw [altitude_def]; rw [ direction_inf_of_mem (self_mem_mk' (s.points i) _) (mem_affine_span ℝ (set.mem_range_self _))]; rw [ direction_mk']; rw [ direction_affine_span]; rw [ direction_affine_span]
...
begin
 ...
 rw [s.independent.finrank_vector_span (fintype.card_fin _)]; rw [ ← finset.coe_image]; rw [ s.independent.finrank_vector_span_image_finset hc]
end
...
begin
 rw [eq_iff_direction_eq_of_mem (mem_affine_span ℝ (set.mem_insert_of_mem _ (set.mem_singleton _))) (s.mem_altitude _)]; rw [ ←vsub_right_mem_direction_iff_mem (mem_affine_span ℝ (set.mem_range_self i)) p]; rw [ direction_affine_span]; rw [ direction_affine_span]; rw [ direction_affine_span],
 split,
 { intro h,
 split,
 { intro heq,
 rw [heq] at h; rw [ set.pair_eq_singleton] at h; rw [ vector_span_singleton] at h,
 have hd : finrank ℝ (s.altitude i).direction = 0,
 { rw [←h]; rw [ finrank_bot] },
 simpa using hd },
 { rw [←submodule.mem_inf]; rw [ _root_.inf_comm]; rw [ ←direction_altitude]; rw [ ←h],
 ...
 rw [vector_span_eq_span_vsub_set_left_ne ℝ (set.mem_insert _ _)]; rw [ set.insert_diff_of_mem _ (set.mem_singleton _)]; rw [ set.diff_singleton_eq_self (λ h, hne (set.mem_singleton_iff.1 h))]; rw [ set.image_singleton],
 refine eq_of_le_of_finrank_eq _ _,
 { rw submodule.span_le,
 simpa using h },
 { rw [finrank_direction_altitude]; rw [ finrank_span_set_eq_card],
 { simp },
...
begin
 rw [orthocenter_eq_monge_point]; rw [ monge_point_eq_smul_vsub_vadd_circumcenter],
 norm_num
end
...
begin
 ...
 rw [monge_plane_def]; rw [ altitude_def]; rw [ direction_affine_span]; rw [ hs]; rw [ he]; rw [ centroid_singleton]; rw [ coe_insert]; rw [ coe_singleton]; rw [ vector_span_image_eq_span_vsub_set_left_ne ℝ _ (set.mem_insert i₂ _)],
...
begin
 ...
 rw [orthocenter_eq_monge_point]; rw [ t.altitude_eq_monge_plane h₁₂ h₁₃ h₂₃],
...
lemma dist_orthocenter_reflection_circumcenter (t : triangle ℝ P) {i₁ i₂ : fin 3} (h : i₁ ≠ i₂) :
 ...
 t.circumradius :=
begin
 rw [←mul_self_inj_of_nonneg dist_nonneg t.circumradius_nonneg]; rw [ t.reflection_circumcenter_eq_affine_combination_of_points_with_circumcenter h]; rw [ t.orthocenter_eq_monge_point]; rw [ monge_point_eq_affine_combination_of_points_with_circumcenter]; rw [ dist_affine_combination t.points_with_circumcenter (sum_monge_point_weights_with_circumcenter _) (sum_reflection_circumcenter_weights_with_circumcenter h)],
...
begin
 ...
 rw [set.insert_subset]; rw [ set.singleton_subset_iff],
...
begin
 symmetry,
 rw [←h₂]; rw [ t₂.affine_span_pair_eq_altitude_iff],
 rw [h₂],
 ...
 rw [←set.image_univ]; rw [ ←finset.coe_univ]; rw [ hu]; rw [ finset.coe_insert]; rw [ finset.coe_insert]; rw [ finset.coe_singleton]; rw [ set.image_insert_eq]; rw [ set.image_insert_eq]; rw [ set.image_singleton]; rw [ h₁]; rw [ h₂]; rw [ h₃]; rw [ set.insert_subset]; rw [ set.insert_subset]; rw [ set.singleton_subset_iff],
 exact ⟨t₁.orthocenter_mem_affine_span,
 mem_affine_span ℝ (set.mem_range_self _),
 mem_affine_span ℝ (set.mem_range_self _)⟩ },
 { rw [direction_affine_span]; rw [ direction_affine_span]; rw [ t₁.independent.finrank_vector_span (fintype.card_fin _)]; rw [ t₂.independent.finrank_vector_span (fintype.card_fin _)] } },
 rw he,
...
begin
 ...
 { rw [←h₂]; rw [ dist_reflection_eq_of_mem _ (mem_affine_span ℝ (set.mem_image_of_mem _ (set.mem_insert _ _)))],
 exact t.dist_circumcenter_eq_circumradius _ },
 { rw [←h₃]; rw [ dist_reflection_eq_of_mem _ (mem_affine_span ℝ (set.mem_image_of_mem _ (set.mem_insert_of_mem _ (set.mem_singleton _))))],
 exact t.dist_circumcenter_eq_circumradius _ } },
...
begin
 ...
 { rw [hts]; rw [ affine_span_insert_eq_affine_span ℝ t.orthocenter_mem_affine_span] },
 refine ext_of_direction_eq _
 ...
 rw [hs]; rw [ direction_affine_span]; rw [ direction_affine_span]; rw [ ha.finrank_vector_span (fintype.card_fin _)]; rw [ t.independent.finrank_vector_span (fintype.card_fin _)]
end
...
begin
 ...
 rw [affine_span_of_orthocentric_system ⟨t, hto, hts⟩ hs t.independent.injective] at hc; rw [ ←affine_span_of_orthocentric_system ⟨t, hto, hts⟩ ht₂s t₂.independent.injective] at hc,
