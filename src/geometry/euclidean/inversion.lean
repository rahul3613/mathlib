...
begin
 ...
 { rw [inversion]; rw [ div_self]; rw [ one_pow]; rw [ one_smul]; rw [ vsub_vadd],
 rwa [dist_ne_zero] }
end
...
by rw [dist_comm c]; rw [ dist_comm c]; rw [ dist_inversion_center]

@[simp] lemma inversion_inversion (c : P) {R : ℝ} (hR : R ≠ 0) (x : P) :
 inversion c R (inversion c R x) = x :=
begin
 rcases eq_or_ne x c with rfl|hne,
 { rw [inversion_self]; rw [ inversion_self] },
 { rw [inversion]; rw [ dist_inversion_center]; rw [ inversion_vsub_center]; rw [ smul_smul]; rw [ ← mul_pow]; rw [ div_mul_div_comm]; rw [ div_mul_cancel _ (dist_ne_zero.2 hne)]; rw [ ← sq]; rw [ div_self]; rw [ one_pow]; rw [ one_smul]; rw [ vsub_vadd],
...
begin
 ...
 { rw [dist_self]; rw [ zero_mul]; rw [ zero_add] },
 rcases eq_or_ne c a with rfl|hc,
 { rw [dist_self]; rw [ zero_mul],
 apply_rules [add_nonneg, mul_nonneg, dist_nonneg] },
 rcases eq_or_ne d a with rfl|hd,
 { rw [dist_self]; rw [ mul_zero]; rw [ add_zero]; rw [ dist_comm d]; rw [ dist_comm d]; rw [ mul_comm] },
