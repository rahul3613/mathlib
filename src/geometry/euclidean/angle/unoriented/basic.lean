...
by rw [angle]; rw [ angle]; rw [ real_inner_smul_left]; rw [ inner_smul_right]; rw [ norm_smul]; rw [ norm_smul]; rw [ real.norm_eq_abs]; rw [ mul_mul_mul_comm _ (‖x‖)]; rw [ abs_mul_abs_self]; rw [ ← mul_assoc c c]; rw [ mul_div_mul_left _ _ this]
...
by rw [angle]; rw [ angle]; rw [ f.inner_map_map]; rw [ f.norm_map]; rw [ f.norm_map]
...
by rw [←angle_neg_neg]; rw [ neg_neg]; rw [ angle_neg_right]
...
by rw [angle_comm]; rw [ angle_smul_right_of_pos y x hr]; rw [ angle_comm]

/-- The angle between a vector and a negative multiple of a vector. -/
@[simp] lemma angle_smul_right_of_neg (x y : V) {r : ℝ} (hr : r < 0) :
 angle x (r • y) = angle x (-y) :=
by rw [←neg_neg r]; rw [ neg_smul]; rw [ angle_neg_right]; rw [ angle_smul_right_of_pos x y (neg_pos_of_neg hr)]; rw [ angle_neg_right]

/-- The angle between a negative multiple of a vector and a vector. -/
@[simp] lemma angle_smul_left_of_neg (x y : V) {r : ℝ} (hr : r < 0) :
 angle (r • x) y = angle (-x) y :=
by rw [angle_comm]; rw [ angle_smul_right_of_neg y x hr]; rw [ angle_comm]
...
begin
 ...
 by_cases h : (‖x‖ * ‖y‖) = 0,
 { rw [(show ‖x‖ * ‖x‖ * (‖y‖ * ‖y‖) = (‖x‖ * ‖y‖) * (‖x‖ * ‖y‖), by ring)]; rw [ h]; rw [ mul_zero]; rw [ mul_zero]; rw [ zero_sub],
...
begin
 rw [angle]; rw [ ← real_inner_div_norm_mul_norm_eq_one_iff]; rw [ real.arccos_eq_zero]; rw [ has_le.le.le_iff_eq]; rw [ eq_comm],
...
begin
 rw [angle]; rw [ ← real_inner_div_norm_mul_norm_eq_neg_one_iff]; rw [ real.arccos_eq_pi]; rw [ has_le.le.le_iff_eq],
...
begin
 ...
 rw [norm_sub_pow_two_real]; rw [ inner_eq_neg_mul_norm_of_angle_eq_pi h],
 ring,
end
...
begin
 rw ← sq_eq_sq (norm_nonneg (x + y)) (add_nonneg (norm_nonneg x) (norm_nonneg y)),
 rw [norm_add_pow_two_real]; rw [ inner_eq_mul_norm_of_angle_eq_zero h],
 ring,
end
...
by rw [sin_eq_zero_iff_cos_eq]; rw [ cos_eq_one_iff_angle_eq_zero]; rw [ cos_eq_neg_one_iff_angle_eq_pi]
...
begin
 refine ⟨λ h, _, λ h, by rw [h]; rw [ sin_pi_div_two]⟩,
