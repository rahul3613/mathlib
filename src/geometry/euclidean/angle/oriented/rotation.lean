...
linear_isometry_equiv.ext $ λ _, by rw [←rotation_rotation]; rw [ linear_isometry_equiv.trans_apply]
...
by rw [←o.rotation_pi_apply]; rw [ rotation_rotation]
...
by rw [neg_rotation]; rw [ ←real.angle.coe_add]; rw [ neg_div]; rw [ ←sub_eq_add_neg]; rw [ sub_half]
...
begin
 ...
 rw [complex.arg_mul_coe_angle]; rw [ real.angle.arg_exp_map_circle],
 { abel },
...
by rw [←o.rotation_eq_self_iff_angle_eq_zero hx]; rw [ eq_comm]
...
by rw [←rotation_eq_self_iff]; rw [ eq_comm]
...
begin
 split,
 { intro h,
 rw [←h]; rw [ linear_isometry_equiv.norm_map] },
 { intro h,
...
begin
 ...
 split,
 { rintro rfl,
 rw [←linear_isometry_equiv.map_smul]; rw [ ←o.oangle_smul_left_of_pos x y hp]; rw [ eq_comm]; rw [ rotation_oangle_eq_iff_norm_eq]; rw [ norm_smul]; rw [ real.norm_of_nonneg hp.le]; rw [ div_mul_cancel _ (norm_ne_zero_iff.2 hx)] },
 { intro hye,
...
begin
 rw [← complex.rotation]; rw [ ← hf]; rw [ o.rotation_map],
 simp,
end
...
by rw [rotation_pi_div_two]; rw [ inner_right_angle_rotation_self]
...
by rw [real_inner_comm]; rw [ inner_rotation_pi_div_two_left]
...
by rw [inner_smul_left]; rw [ inner_rotation_pi_div_two_left]; rw [ mul_zero]
...
by rw [real_inner_comm]; rw [ inner_smul_rotation_pi_div_two_left]
...
by rw [inner_smul_right]; rw [ inner_rotation_pi_div_two_left]; rw [ mul_zero]
...
by rw [real_inner_comm]; rw [ inner_rotation_pi_div_two_left_smul]
...
by rw [inner_smul_right]; rw [ inner_smul_rotation_pi_div_two_left]; rw [ mul_zero]
...
by rw [real_inner_comm]; rw [ inner_smul_rotation_pi_div_two_smul_left]
