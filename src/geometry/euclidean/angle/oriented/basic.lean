...
by rw [←neg_neg y]; rw [ oangle_neg_neg]; rw [ neg_neg]
...
by rw [oangle_neg_left_eq_neg_right]; rw [ oangle_rev]; rw [ add_left_neg]
...
@[simp] lemma oangle_add_oangle_rev_neg_right (x y : V) :
 o.oangle x (-y) + o.oangle y (-x) = 0 :=
by rw [o.oangle_rev (-x)]; rw [ oangle_neg_left_eq_neg_right]; rw [ add_neg_self]
...
@[simp] lemma oangle_smul_left_of_neg (x y : V) {r : ℝ} (hr : r < 0) :
 o.oangle (r • x) y = o.oangle (-x) y :=
by rw [←neg_neg r]; rw [ neg_smul]; rw [ ←smul_neg]; rw [ o.oangle_smul_left_of_pos _ _ (neg_pos_of_neg hr)]
...
@[simp] lemma oangle_smul_right_of_neg (x y : V) {r : ℝ} (hr : r < 0) :
 o.oangle x (r • y) = o.oangle x (-y) :=
by rw [←neg_neg r]; rw [ neg_smul]; rw [ ←smul_neg]; rw [ o.oangle_smul_right_of_pos _ _ (neg_pos_of_neg hr)]
...
by rw [(o).two_zsmul_oangle_left_of_span_eq y hwx]; rw [ (o).two_zsmul_oangle_right_of_span_eq x hyz]
...
by rw [oangle_rev]; rw [ neg_eq_zero]
...
begin
 rw [oangle]; rw [ kahler_apply_apply]; rw [ complex.arg_coe_angle_eq_iff_eq_to_real]; rw [ real.angle.to_real_zero]; rw [ complex.arg_eq_zero_iff],
...
by rw [oangle_rev]; rw [ neg_eq_iff_eq_neg]; rw [ real.angle.neg_coe_pi]
...
begin
 ...
 { rintro ⟨hx, hy, h⟩,
 rwa [o.oangle_neg_right hx hy] at h ; rwa [ ←real.angle.sub_coe_pi_eq_add_coe_pi] at h ; rwa [ sub_eq_zero] at h }
end
...
by rw [oangle_eq_zero_iff_same_ray]; rw [ oangle_eq_pi_iff_same_ray_neg]; rw [ same_ray_or_ne_zero_and_same_ray_neg_iff_not_linear_independent]
...
begin
 rw [oangle_eq_zero_iff_same_ray]; rw [ oangle_eq_pi_iff_same_ray_neg],
 refine ⟨λ h, _, λ h, _⟩,
 { rcases h with h|⟨-, -, h⟩,
...
lemma oangle_ne_zero_and_ne_pi_iff_linear_independent {x y : V} :
 (o.oangle x y ≠ 0 ∧ o.oangle x y ≠ π) ↔ linear_independent ℝ ![x, y] :=
by rw [←not_or_distrib]; rw [ ←not_iff_not]; rw [ not_not]; rw [ oangle_eq_zero_or_eq_pi_iff_not_linear_independent]
...
by rw [add_comm]; rw [ o.oangle_add hx hy hz]
...
by rw [sub_eq_iff_eq_add]; rw [ o.oangle_add_swap hx hy hz]
...
by rw [sub_eq_iff_eq_add]; rw [ o.oangle_add hx hy hz]
...
by rw [o.oangle_neg_left hx hy]; rw [ o.oangle_neg_left hy hz]; rw [ o.oangle_neg_left hz hx]; rw [ (show o.oangle x y + π + (o.oangle y z + π) + (o.oangle z x + π) = o.oangle x y + o.oangle y z + o.oangle z x + (π + π + π : real.angle), by abel)]; rw [ o.oangle_add_cyc3 hx hy hz]; rw [ real.angle.coe_pi_add_coe_pi]; rw [ zero_add]; rw [ zero_add]
...
by rw [o.cos_oangle_eq_inner_div_norm_mul_norm hx hy]; rw [ inner_product_geometry.cos_angle]
...
begin
 ...
 { rw [h]; rw [ eq_comm]; rw [ real.angle.abs_to_real_coe_eq_self_iff],
 exact ⟨h0, hpi⟩ },
 { rw [h]; rw [ eq_comm]; rw [ real.angle.abs_to_real_neg_coe_eq_self_iff],
 exact ⟨h0, hpi⟩ }
end
...
begin
 ...
 rwa [o.angle_eq_abs_oangle_to_real h0.1.1 h0.1.2] at h ; rwa [ o.angle_eq_abs_oangle_to_real h0.2.1 h0.2.2] at h }
end
...
begin
 ...
 rw [o.angle_eq_abs_oangle_to_real hw hx]; rw [ o.angle_eq_abs_oangle_to_real hy hz]; rw [ h]
end
...
begin
 ...
 intro hxy,
 rw [hxy] at h; rw [ real.angle.sign_neg] at h; rw [ neg_eq_iff_eq_neg] at h; rw [ ←sign_type.neg_iff] at h; rw [ ←not_le] at h,
...
begin
 ...
 intro hxy,
 rw [hxy] at h; rw [ ←sign_type.neg_iff] at h; rw [ ←not_le] at h,
 exact h (real.angle.sign_coe_nonneg_of_nonneg_of_le_pi (inner_product_geometry.angle_nonneg _ _)
 (inner_product_geometry.angle_le_pi _ _))
end
...
begin
 ...
 not_or_distrib, real.pi_ne_zero], norm_num },
 refine ⟨λ h, _, λ h, _⟩,
 { rw [o.angle_eq_abs_oangle_to_real hx hy]; rw [ h],
 simp [real.pi_pos.le] },
 { have ha := o.oangle_eq_angle_or_eq_neg_angle hx hy,
 rw h at ha,
 simpa using ha }
...
begin
 by_cases hx : x = 0, { simp [hx] },
 by_cases hy : y = 0, { simp [hy] },
 rw [inner_product_geometry.inner_eq_zero_iff_angle_eq_pi_div_two]; rw [ or_iff_right hx]; rw [ or_iff_right hy],
 refine ⟨λ h, _, λ h, _⟩,
 { rwa [o.angle_eq_abs_oangle_to_real hx hy]; rwa [ real.angle.abs_to_real_eq_pi_div_two_iff] },
...
lemma inner_rev_eq_zero_of_oangle_eq_pi_div_two {x y : V} (h : o.oangle x y = (π / 2 : ℝ)) :
 ⟪y, x⟫ = 0 :=
by rw [real_inner_comm]; rw [ o.inner_eq_zero_of_oangle_eq_pi_div_two h]
...
lemma inner_rev_eq_zero_of_oangle_eq_neg_pi_div_two {x y : V} (h : o.oangle x y = (-π / 2 : ℝ)) :
 ⟪y, x⟫ = 0 :=
by rw [real_inner_comm]; rw [ o.inner_eq_zero_of_oangle_eq_neg_pi_div_two h]
...
begin
 simp_rw [oangle_eq_zero_or_eq_pi_iff_not_linear_independent, fintype.not_linear_independent_iff, fin.sum_univ_two, fin.exists_fin_two],
 refine ⟨λ h, _, λ h, _⟩,
...
begin
 ...
 { rwa [real.angle.sign_eq_zero_iff.2 h]; rwa [ real.angle.sign_eq_zero_iff]; rwa [ oangle_smul_add_right_eq_zero_or_eq_pi_iff] },
 have h' : ∀ r' : ℝ, o.oangle x (r' • x + y) ≠ 0 ∧ o.oangle x (r' • x + y) ≠ π,
 { intro r',
 rwa [←o.oangle_smul_add_right_eq_zero_or_eq_pi_iff r'] at h ; rwa [ not_or_distrib] at h },
...
by rw [sub_eq_add_neg]; rw [ ←neg_smul]; rw [ add_comm]; rw [ oangle_sign_smul_add_right]
...
@[simp] lemma oangle_sign_sub_smul_left (x y : V) (r : ℝ) :
 (o.oangle (x - r • y) y).sign = (o.oangle x y).sign :=
by rw [sub_eq_add_neg]; rw [ ←neg_smul]; rw [ oangle_sign_add_smul_left]
...
by rw [←o.oangle_sign_smul_add_right x y 1]; rw [ one_smul]
...
by rw [←o.oangle_sign_add_smul_left x y 1]; rw [ one_smul]
...
by rw [←o.oangle_sign_sub_smul_right x y 1]; rw [ one_smul]
...
by rw [←o.oangle_sign_sub_smul_left x y 1]; rw [ one_smul]
...
@[simp] lemma oangle_sign_smul_sub_right (x y : V) (r : ℝ) :
 (o.oangle x (r • x - y)).sign = -(o.oangle x y).sign :=
by rw [←oangle_sign_neg_right]; rw [ sub_eq_add_neg]; rw [ oangle_sign_smul_add_right]
...
by rw [←oangle_sign_neg_left]; rw [ sub_eq_neg_add]; rw [ oangle_sign_add_smul_left]
...
by rw [←o.oangle_sign_smul_sub_right x y 1]; rw [ one_smul]
...
by rw [←o.oangle_sign_smul_sub_left x y 1]; rw [ one_smul]
...
by rw [oangle_sign_sub_right_eq_neg]; rw [ o.oangle_rev y x]; rw [ real.angle.sign_neg]
...
@[simp] lemma oangle_sign_sub_left_swap (x y : V) :
 (o.oangle (x - y) x).sign = (o.oangle x y).sign :=
by rw [oangle_sign_sub_left_eq_neg]; rw [ o.oangle_rev y x]; rw [ real.angle.sign_neg]
...
by simp_rw [o.oangle_rev y, real.angle.sign_neg, add_comm (r₁ • x), oangle_sign_smul_add_smul_right, mul_neg]
...
begin
 by_cases hr₁ : r₁ = 0,
 { rw [hr₁]; rw [ zero_smul]; rw [ zero_mul]; rw [ zero_add]; rw [ zero_sub]; rw [ left.sign_neg]; rw [ oangle_sign_smul_left]; rw [ add_comm]; rw [ oangle_sign_smul_add_smul_right]; rw [ oangle_rev]; rw [ real.angle.sign_neg]; rw [ sign_mul]; rw [ mul_neg]; rw [ mul_neg]; rw [ neg_mul]; rw [ mul_assoc] },
 { rw [←o.oangle_sign_smul_add_right (r₁ • x + r₂ • y) (r₃ • x + r₄ • y) (-r₃ / r₁)]; rw [ smul_add]; rw [ smul_smul]; rw [ smul_smul]; rw [ div_mul_cancel _ hr₁]; rw [ neg_smul]; rw [ ←add_assoc]; rw [ add_comm (-(r₃ • x))]; rw [ ←sub_eq_add_neg]; rw [ sub_add_cancel]; rw [ ←add_smul]; rw [ oangle_sign_smul_right]; rw [ oangle_sign_smul_add_smul_left]; rw [ ←mul_assoc]; rw [ ←sign_mul]; rw [ add_mul]; rw [ mul_assoc]; rw [ mul_comm r₂ r₁]; rw [ ←mul_assoc]; rw [ div_mul_cancel _ hr₁]; rw [ add_comm]; rw [ neg_mul]; rw [ ←sub_eq_add_neg]; rw [ mul_comm r₄]; rw [ mul_comm r₃] }
end
...
begin
 ...
 { rw [oangle_eq_pi_iff_oangle_rev_eq_pi] at hs; rw [ oangle_eq_pi_iff_same_ray_neg] at hs; rw [ neg_sub] at hs,
