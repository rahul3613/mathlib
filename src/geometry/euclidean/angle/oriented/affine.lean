...
begin
 rw [oangle]; rw [ (o).oangle_ne_zero_and_ne_pi_iff_linear_independent]; rw [ affine_independent_iff_linear_independent_vsub ℝ _ (1 : fin 3)]; rw [ ←linear_independent_equiv (fin_succ_above_equiv (1 : fin 3)).to_equiv],
 convert iff.rfl,
 ext i,
 fin_cases i;
 refl
...
by rw [←not_iff_not]; rw [ not_or_distrib]; rw [ oangle_ne_zero_and_ne_pi_iff_affine_independent]; rw [ affine_independent_iff_not_collinear_set]
...
begin
 ...
 { rw [←neg_vsub_eq_vsub_rev p₁ p₃]; rw [ ←neg_vsub_eq_vsub_rev p₁ p₂]; rw [ (o).oangle_neg_neg] },
...
lemma angle_eq_pi_div_two_of_oangle_eq_pi_div_two {p₁ p₂ p₃ : P} (h : ∡ p₁ p₂ p₃ = ↑(π / 2)) :
 ∠ p₁ p₂ p₃ = π / 2 :=
begin
 rw [angle]; rw [ ←inner_product_geometry.inner_eq_zero_iff_angle_eq_pi_div_two],
...
begin
 rw [eq_comm]; rw [ oangle]; rw [ oangle]; rw [ ←(o).oangle_neg_neg]; rw [ neg_vsub_eq_vsub_rev]; rw [ neg_vsub_eq_vsub_rev]; rw [ ←vsub_sub_vsub_cancel_left p₁ p₃ p₂]; rw [ ←neg_vsub_eq_vsub_rev p₃ p₂]; rw [ sub_eq_add_neg]; rw [ neg_vsub_eq_vsub_rev p₂ p₁]; rw [ add_comm]; rw [ ←@neg_one_smul ℝ],
...
lemma oangle_swap₁₃_sign (p₁ p₂ p₃ : P) : -(∡ p₁ p₂ p₃).sign = (∡ p₃ p₂ p₁).sign :=
by rw [oangle_rev]; rw [ real.angle.sign_neg]; rw [ neg_neg]

/-- Swapping the second and third points in an oriented angle negates the sign of that angle. -/
lemma oangle_swap₂₃_sign (p₁ p₂ p₃ : P) : -(∡ p₁ p₂ p₃).sign = (∡ p₁ p₃ p₂).sign :=
by rw [oangle_swap₁₃_sign]; rw [ ←oangle_swap₁₂_sign]; rw [ oangle_swap₁₃_sign]
...
by rw [←oangle_swap₁₂_sign]; rw [ oangle_swap₁₃_sign]
...
by rw [oangle_eq_pi_iff_angle_eq_pi]; rw [ angle_eq_pi_iff_sbtw]
...
point (reversed) is π. -/
lemma _root_.sbtw.oangle₃₂₁_eq_pi {p₁ p₂ p₃ : P} (h : sbtw ℝ p₁ p₂ p₃) : ∡ p₃ p₂ p₁ = π :=
by rw [oangle_eq_pi_iff_oangle_rev_eq_pi]; rw [ ←h.oangle₁₂₃_eq_pi]
...
lemma _root_.wbtw.oangle₃₁₂_eq_zero {p₁ p₂ p₃ : P} (h : wbtw ℝ p₁ p₂ p₃) : ∡ p₃ p₁ p₂ = 0 :=
by rw [oangle_eq_zero_iff_oangle_rev_eq_zero]; rw [ h.oangle₂₁₃_eq_zero]
...
begin
 by_cases hp₃p₂ : p₃ = p₂, { simp [hp₃p₂] },
 by_cases hp₁'p₂ : p₁' = p₂, { rw [hp₁'p₂] at h; rw [ wbtw_self_iff] at h, exact false.elim (hp₁p₂ h) },
...
lemma _root_.wbtw.oangle_eq_right {p₁ p₂ p₃ p₃' : P} (h : wbtw ℝ p₂ p₃ p₃') (hp₃p₂ : p₃ ≠ p₂) :
 ∡ p₁ p₂ p₃ = ∡ p₁ p₂ p₃' :=
by rw [oangle_rev]; rw [ h.oangle_eq_left hp₃p₂]; rw [ ←oangle_rev]
...
by rw [midpoint_comm]; rw [ oangle_midpoint_left]
...
by rw [midpoint_comm]; rw [ oangle_midpoint_right]
...
by rw [←h.oangle₁₂₃_eq_pi]; rw [ oangle_add_swap h.left_ne h.right_ne hp₃p₂]
...
lemma _root_.sbtw.oangle_eq_add_pi_right {p₁ p₂ p₃ p₃' : P} (h : sbtw ℝ p₃ p₂ p₃')
 (hp₁p₂ : p₁ ≠ p₂) : ∡ p₁ p₂ p₃ = ∡ p₁ p₂ p₃' + π :=
by rw [←h.oangle₃₂₁_eq_pi]; rw [ oangle_add hp₁p₂ h.right_ne h.left_ne]
...
by rw [h₁.oangle_eq_add_pi_left h₃.left_ne]; rw [ h₃.oangle_eq_add_pi_right h₁.right_ne]; rw [ add_assoc]; rw [ real.angle.coe_pi_add_coe_pi]; rw [ add_zero]
...
lemma _root_.collinear.two_zsmul_oangle_eq_right {p₁ p₂ p₃ p₃' : P}
 (h : collinear ℝ ({p₃, p₂, p₃'} : set P)) (hp₃p₂ : p₃ ≠ p₂) (hp₃'p₂ : p₃' ≠ p₂) :
 (2 : ℤ) • ∡ p₁ p₂ p₃ = (2 : ℤ) • ∡ p₁ p₂ p₃' :=
by rw [oangle_rev]; rw [ smul_neg]; rw [ h.two_zsmul_oangle_eq_left hp₃p₂ hp₃'p₂]; rw [ ←smul_neg]; rw [ ←oangle_rev]
...
begin
 ...
 { rw [@dist_eq_norm_vsub' V] at hd; rw [ @dist_eq_norm_vsub' V] at hd; rw [ ←mul_self_inj (norm_nonneg _) (norm_nonneg _)] at hd; rw [ ←real_inner_self_eq_norm_mul_norm] at hd; rw [ ←real_inner_self_eq_norm_mul_norm] at hd,
 simp_rw [vsub_midpoint, ←vsub_sub_vsub_cancel_left p₂ p₁ p, inner_sub_left, inner_add_right, inner_smul_right, hd, real_inner_comm (p -ᵥ p₁)],
 abel },
 ...
 simp_rw [@dist_eq_norm_vsub V, vsub_vadd_eq_vsub_sub, left_vsub_midpoint, right_vsub_midpoint, inv_of_eq_inv, ←neg_vsub_eq_vsub_rev p₂ p₁, ←mul_self_inj (norm_nonneg _) (norm_nonneg _), ←real_inner_self_eq_norm_mul_norm, inner_sub_sub_self],
...
begin
 ...
 all_goals { simp_rw [s, set.mem_image, set.mem_prod, set.mem_univ, true_and, prod.ext_iff] at hp,
 obtain ⟨q₁, q₅, q₂⟩ := p,
 dsimp only at ⊢ hp,
 ...
 { intros p hp,
 simp_rw [s, set.mem_image, set.mem_prod, set.mem_set_of, set.mem_univ, true_and, prod.ext_iff] at hp,
 obtain ⟨q₁, q₅, q₂⟩ := p,
 dsimp only at ⊢ hp,
 obtain ⟨⟨⟨q, hq⟩, v⟩, hv, rfl, rfl, rfl⟩ := hp,
 dsimp only [subtype.coe_mk, set.mem_set_of] at ⊢ hv,
 ...
 { rwa [←@vsub_ne_zero V]; rwa [ vsub_vadd_eq_vsub_sub]; rwa [ vsub_self]; rwa [ zero_sub]; rwa [ neg_ne_zero] },
 { refine vadd_mem_of_mem_direction _ hq,
 rw direction_affine_span,
 exact smul_vsub_rev_mem_vector_span_pair _ _ _ } },
 have hp₁p₂s : (p₁, p₅, p₂) ∈ s,
 { simp_rw [s, set.mem_image, set.mem_prod, set.mem_set_of, set.mem_univ, true_and, prod.ext_iff],
...
begin
 ...
 rw [←(hp₃p₄.symm.trans (s_opp_side_point_reflection hp₁ hp₃p₄.left_not_mem)).oangle_sign_eq hp₁ hp₂]; rw [ ←oangle_rotate_sign p₁]; rw [ ←oangle_rotate_sign p₁]; rw [ oangle_swap₁₃_sign]; rw [ (sbtw_point_reflection_of_ne ℝ hp₁p₃).symm.oangle_sign_eq _],
end
