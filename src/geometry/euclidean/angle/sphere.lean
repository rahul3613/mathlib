...
begin
 ...
 by rw [o.oangle_eq_pi_sub_two_zsmul_oangle_sub_of_norm_eq hxzne.symm hxz.symm]; rw [ o.oangle_eq_pi_sub_two_zsmul_oangle_sub_of_norm_eq hxyne.symm hxy.symm]
 ...
 by rw [←oangle_neg_neg]; rw [ neg_sub]; rw [ neg_sub]
end
...
begin
 ...
 rw [oangle]; rw [ oangle]; rw [ ←vsub_sub_vsub_cancel_right p₁ p₂ s.center]; rw [ ←vsub_sub_vsub_cancel_right p₄ p₂ s.center]; rw [ (o).two_zsmul_oangle_sub_eq_two_zsmul_oangle_sub_of_norm_eq _ _ _ _ hp₂ hp₃ hp₁ hp₄];
...
by rw [oangle_eq_pi_sub_two_zsmul_oangle_of_dist_eq h.symm (dist_center_eq_dist_center_of_mem_sphere' hp₂ hp₁)]

/-- The angle at the apex of an isosceles triangle is `π` minus twice a base angle, oriented
angle-at-point form where the apex is given as the center of a circle. -/
lemma oangle_eq_pi_sub_two_zsmul_oangle_center_right {s : sphere P} {p₁ p₂ : P} (hp₁ : p₁ ∈ s)
 (hp₂ : p₂ ∈ s) (h : p₁ ≠ p₂) : ∡ p₁ s.center p₂ = π - (2 : ℤ) • ∡ p₂ p₁ s.center :=
by rw [oangle_eq_pi_sub_two_zsmul_oangle_center_left hp₁ hp₂ h]; rw [ oangle_eq_oangle_of_dist_eq (dist_center_eq_dist_center_of_mem_sphere' hp₂ hp₁)]
...
by rw [←oangle_center_eq_two_zsmul_oangle hp₁ hp₂ hp₃ hp₂p₁ hp₂p₃]; rw [ oangle_eq_pi_sub_two_zsmul_oangle_center_right hp₁ hp₃ hp₁p₃]; rw [ add_sub_cancel'_right]
...
begin
 rw [div_right_comm]; rw [ div_eq_mul_inv _ (2 : ℝ)]; rw [ mul_comm]; rw [ (show (2 : ℝ)⁻¹ * dist p₁ p₂ = dist p₁ (midpoint ℝ p₁ p₂), by simp)]; rw [ ←mem_sphere.1 hp₁]; rw [ ←tan_div_two_smul_rotation_pi_div_two_vadd_midpoint_eq_center hp₁ hp₂ h]; rw [ ←oangle_midpoint_rev_left]; rw [ oangle]; rw [ vadd_vsub_assoc]; rw [ (show p₂ -ᵥ p₁ = (2 : ℝ) • (midpoint ℝ p₁ p₂ -ᵥ p₁), by simp)]; rw [ map_smul]; rw [ smul_smul]; rw [ div_mul_cancel _ (two_ne_zero' ℝ)]; rw [ @dist_eq_norm_vsub' V]; rw [ @dist_eq_norm_vsub' V]; rw [ vadd_vsub_assoc]; rw [ add_comm]; rw [ (o).oangle_add_right_smul_rotation_pi_div_two]; rw [ real.angle.cos_coe]; rw [ real.cos_arctan]; rw [ one_div]; rw [ div_inv_eq_mul]; rw [ ←mul_self_inj (mul_nonneg (norm_nonneg _) (real.sqrt_nonneg _)) (norm_nonneg _)]; rw [ norm_add_sq_eq_norm_sq_add_norm_sq_real ((o).inner_smul_rotation_pi_div_two_right _ _)]; rw [ ←mul_assoc]; rw [ mul_comm]; rw [ mul_comm _ (real.sqrt _)]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ real.mul_self_sqrt (add_nonneg zero_le_one (sq_nonneg _))]; rw [ norm_smul]; rw [ linear_isometry_equiv.norm_map],
 swap, { simpa using h.symm },
 conv_rhs { rw [←mul_assoc]; rw [ mul_comm _ ‖real.angle.tan _‖]; rw [ ←mul_assoc]; rw [ real.norm_eq_abs]; rw [ abs_mul_abs_self] },
 ring
end
...
lemma dist_div_cos_oangle_center_eq_two_mul_radius {s : sphere P} {p₁ p₂ : P} (hp₁ : p₁ ∈ s)
 (hp₂ : p₂ ∈ s) (h : p₁ ≠ p₂) : dist p₁ p₂ / real.angle.cos (∡ p₂ p₁ s.center) = 2 * s.radius :=
by rw [←dist_div_cos_oangle_center_div_two_eq_radius hp₁ hp₂ h]; rw [ mul_div_cancel' _ (two_ne_zero' ℝ)]
...
begin
 ...
 rw [←real.angle.abs_cos_eq_abs_sin_of_two_zsmul_add_two_zsmul_eq_pi (two_zsmul_oangle_center_add_two_zsmul_oangle_eq_pi hp₁ hp₂ hp₃ hp₁p₂.symm hp₂p₃ hp₁p₃)]; rw [ _root_.abs_of_nonneg (real.angle.cos_nonneg_iff_abs_to_real_le_pi_div_two.2 _)],
...
lemma dist_div_sin_oangle_eq_two_mul_radius {s : sphere P} {p₁ p₂ p₃ : P} (hp₁ : p₁ ∈ s)
 (hp₂ : p₂ ∈ s) (hp₃ : p₃ ∈ s) (hp₁p₂ : p₁ ≠ p₂) (hp₁p₃ : p₁ ≠ p₃) (hp₂p₃ : p₂ ≠ p₃) :
 dist p₁ p₃ / |real.angle.sin (∡ p₁ p₂ p₃)| = 2 * s.radius :=
by rw [←dist_div_sin_oangle_div_two_eq_radius hp₁ hp₂ hp₃ hp₁p₂ hp₁p₃ hp₂p₃]; rw [ mul_div_cancel' _ (two_ne_zero' ℝ)]
...
begin
 rw [t₁.circumsphere_eq_of_dist_of_oangle h₁₂ h₁₃ h₂₃]; rw [ t₂.circumsphere_eq_of_dist_of_oangle h₁₂ h₁₃ h₂₃],
...
begin
 ...
 { rw [affine_independent_iff_not_collinear_of_ne h₁₂ h₁₃ h₂₃]; rw [ h₁]; rw [ h₂]; rw [ h₃]; rw [ collinear_iff_of_two_zsmul_oangle_eq h]; rw [ ←affine_independent_iff_not_collinear_of_ne h₁₂ h₁₃ h₂₃],
 exact t.independent },
 ...
 (2 : ℤ) • ∡ (t.points i₁) (t.points i₂) (t.points i₃), { rwa [h₁']; rwa [ h₂']; rwa [ h₃'] },
 rw [←circumsphere_eq_circumsphere_of_eq_of_eq_of_two_zsmul_oangle_eq h₁₂ h₁₃ h₂₃ h₁' h₃' h']; rw [ ←h₂'],
...
begin
 ...
 { by_cases he : p₁ = p₄,
 { rw [he]; rw [ set.insert_eq_self.2 (set.mem_insert_of_mem _ (set.mem_insert_of_mem _ (set.mem_singleton _)))],
 ...
 rwa [set.insert_comm p₁ p₂]; rwa [ hc'.collinear_insert_iff_of_ne (set.mem_insert _ _) (set.mem_insert_of_mem _ (set.mem_insert_of_mem _ (set.mem_singleton _))) he] },
