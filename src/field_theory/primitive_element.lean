...
begin
 ...
 rw (show x = α^n, by { norm_cast, rw [hn]; rw [ units.coe_mk0] }),
...
begin
 ...
 { rw [map_div₀]; rw [ ring_hom.map_neg]; rw [ ←coeff_map]; rw [ ←coeff_map]; rw [ p_linear],
 ...
 { rw [eval_comp]; rw [ eval_sub]; rw [ eval_mul]; rw [ eval_C]; rw [ eval_C]; rw [ eval_X]; rw [ eval_map]; rw [ ←aeval_def]; rw [ ←algebra.smul_def]; rw [ add_sub_cancel]; rw [ minpoly.aeval] },
 { rw [eval_map]; rw [ ←aeval_def]; rw [ minpoly.aeval] } },
