...
begin
 ...
 rw [algebra.top_to_submodule]; rw [ eq_top_iff]; rw [ ← s.span_eq]; rw [ submodule.span_le]; rw [ set.range_subset_iff],
 ...
 rw [is_root.def]; rw [ eval_map]; rw [ ← aeval_def]; rw [ alg_hom.map_prod],
...
{ letI : algebra E K := ϕ.to_ring_hom.to_algebra,
 ...
 { rw [ring_hom.map_zero]; rw [ aeval_map_algebra_map]; rw [ ← aeval_algebra_map_apply],
...
normal_iff.2 $ λ x, begin
 ...
 rw [alg_equiv.to_alg_hom_eq_coe] at H; rw [ alg_equiv.coe_alg_hom] at H; rw [ alg_equiv.apply_symm_apply] at H,
 use H,
...
begin
 ...
 { rw [power_basis.finrank pbED]; rw [ adjoin_root.power_basis_dim] },
 haveI : finite_dimensional F D := finite_dimensional.trans F E D,
 rsuffices ⟨ϕ⟩ : nonempty (D →ₐ[F] E),
 { rw [←with_bot.coe_one]; rw [ degree_eq_iff_nat_degree_eq q_irred.ne_zero]; rw [ ←finrankED],
 ...
 (algebra_map F D) (adjoin_root.root q) (by rw [algebra_map_eq F E D]; rw [ ←eval₂_map]; rw [ hr]; rw [ adjoin_root.algebra_map_eq]; rw [ eval₂_mul]; rw [ adjoin_root.eval₂_root]; rw [ zero_mul])),
 ...
 { rw [splits_map_iff]; rw [ ←algebra_map_eq F C E],
 ...
 { apply minpoly.dvd,
 rw [←hz2]; rw [ aeval_def]; rw [ eval₂_map]; rw [ ←algebra_map_eq F C D]; rw [ algebra_map_eq F E D]; rw [ ←hom_eval₂]; rw [ ←aeval_def]; rw [ minpoly.aeval F z]; rw [ ring_hom.map_zero] } },
 rw [←intermediate_field.to_subalgebra_le_to_subalgebra]; rw [ intermediate_field.top_to_subalgebra],
 ...
 { rw [adjoin_root.adjoin_root_eq_top] at this; rw [ subalgebra.restrict_scalars_top] at this; rw [ ←@subalgebra.restrict_scalars_top F C] at this,
 exact top_le_iff.mpr (subalgebra.restrict_scalars_injective F this) },
 dsimp only [S],
 rw [←finset.image_to_finset]; rw [ finset.coe_image],
 apply eq.trans (algebra.adjoin_res_eq_adjoin_res F E C D
 hFEp.adjoin_root_set adjoin_root.adjoin_root_eq_top),
 rw [set.image_singleton]; rw [ ring_hom.algebra_map_to_algebra]; rw [ adjoin_root.lift_root]
end
...
begin
 ...
 rw [adjoin_le_iff]; rw [ ←image_root_set ((h i.1).splits i.2) (t i.1).val],
...
begin
 ...
 rw [←show E.val.comp ↑g = f]; rw [ from fun_like.ext_iff.mpr (f.restrict_normal_commutes E)]; rw [ ←alg_hom.map_field_range]; rw [ g.field_range_eq_top]; rw [ ←E.val.field_range_eq_map]; rw [ E.field_range_val],
end
...
@alg_hom.restrict_scalars F K₁ E E _ _ _ _ _ _
 ...
 (by { rw [splits_map_iff]; rw [ ←is_scalar_tower.algebra_map_eq], exact (h.out x).2 })
 (minpoly.dvd_map_of_is_scalar_tower F K₁ x)⟩)
...
begin
 ...
 rw [mem_root_set_of_ne]; rw [ alg_hom.to_ring_hom_eq_coe]; rw [ alg_hom.coe_to_ring_hom]; rw [ polynomial.aeval_alg_hom_apply]; rw [ minpoly.aeval]; rw [ map_zero],
 exact minpoly.ne_zero ((is_integral_algebra_map_iff (algebra_map K L).injective).mp
 (h.is_integral (algebra_map K L x))) },
 { rw [polynomial.root_set] at hy; rw [ finset.mem_coe] at hy; rw [ multiset.mem_to_finset] at hy,
 ...
 rw [algebra.id.map_eq_id]; rw [ ring_hom.id_apply],
 apply power_basis.lift_gen },
end

instance normal [h : normal F L] : normal F (normal_closure F K L) :=
let ϕ := algebra_map K L in begin
 rw [←intermediate_field.restrict_scalars_normal]; rw [ restrict_scalars_eq_supr_adjoin],
