...
begin
 ...
 rw [← (module.rank F K).lift_id]; rw [ ← b.mk_eq_rank]; rw [ ← (module.rank K A).lift_id]; rw [ ← c.mk_eq_rank]; rw [ ← lift_umax.{w v}]; rw [ ← (b.smul c).mk_eq_rank]; rw [ mk_prod]; rw [ lift_mul]; rw [ lift_lift]; rw [ lift_lift]; rw [ lift_lift]; rw [ lift_lift]; rw [ lift_umax]
end
...
by { rw [submodule.restrict_scalars_top]; rw [ eq_top_iff]; rw [ ← hb]; rw [ submodule.span_le],
...
begin
 ...
 { rw [finrank_of_infinite_dimensional hA]; rw [ mul_zero]; rw [ finrank_of_infinite_dimensional],
