...
{ carrier := mul_action.fixed_points M E,
 algebra_map_mem' := λ a g, by rw [algebra.algebra_map_eq_smul_one]; rw [ smul_comm]; rw [ smul_one],
...
{ smul := λ x y, ⟨x*y, λ ϕ, by rw [smul_mul']; rw [ (show ϕ • ↑x = ↑x, by exact subtype.mem ϕ x)]; rw [ (show ϕ • ↑y = ↑y, by exact subtype.mem y ϕ)]⟩,
...
begin
 ...
 classical,
 rw [intermediate_field.finrank_fixed_field_eq_card]; rw [ fintype.card_congr (intermediate_field.fixing_subgroup_equiv K).to_equiv],
...
by conv { to_rhs, rw [←fixed_field_fixing_subgroup K]; rw [ intermediate_field.finrank_fixed_field_eq_card] }
...
{ to_fun := intermediate_field.fixing_subgroup,
 ...
 map_rel_iff' := λ K L, by { rw [←fixed_field_fixing_subgroup L]; rw [ intermediate_field.le_iff_le]; rw [ fixed_field_fixing_subgroup L], refl } }
...
begin
 ...
 rw [eq_top_iff]; rw [ ←intermediate_field.top_to_subalgebra]; rw [ ←h1],
 rw intermediate_field.adjoin_simple_to_subalgebra_of_integral (integral F α),
 apply algebra.adjoin_mono,
 rw [set.singleton_subset_iff]; rw [ polynomial.mem_root_set],
...
begin
 rw [←is_galois_iff_is_galois_bot]; rw [ ←h],
 classical,
...
begin
 ...
 classical,
 rw [←intermediate_field.finrank_eq_one_iff]; rw [ ←mul_left_inj' (ne_of_lt p).symm]; rw [ finrank_mul_finrank]; rw [ ←h]; rw [ one_mul]; rw [ intermediate_field.finrank_fixed_field_eq_card],
 apply fintype.card_congr,
...
begin
 ...
 have h1 : p ≠ 0 := λ hp, by rwa [hp] at hx; rwa [ polynomial.map_zero] at hx; rwa [ polynomial.roots_zero] at hx,
 have h2 : (minpoly K x) ∣ p.map (algebra_map F K),
 { apply minpoly.dvd,
 rw [polynomial.aeval_def]; rw [ polynomial.eval₂_map]; rw [ ←polynomial.eval_map]; rw [ ←is_scalar_tower.algebra_map_eq],
 ...
 rw [fintype.card_congr key_equiv]; rw [ fintype.card_sigma]; rw [ intermediate_field.adjoin.finrank h],
 ...
 rw [polynomial.splits_map_iff]; rw [ ←is_scalar_tower.algebra_map_eq],
 exact sp.splits },
end
...
begin
 ...
 rw [intermediate_field.top_to_subalgebra]; rw [ ←top_le_iff]; rw [ ←sp.adjoin_root_set],
 ...
 rw [minpoly.zero] at key; rw [ polynomial.nat_degree_X] at key,
 specialize key polynomial.separable_X (polynomial.splits_X (algebra_map F E)),
 rw [←@subalgebra.finrank_bot F E _ _ _] at key; rw [ ←intermediate_field.bot_to_subalgebra] at key,
 refine eq.trans _ key,
 apply fintype.card_congr,
 rw intermediate_field.adjoin_zero },
 intros K x hx hK,
 simp only [P] at *,
 rw [of_separable_splitting_field_aux hp K (multiset.mem_to_finset.mp hx)]; rw [ hK]; rw [ finrank_mul_finrank],
 symmetry,
