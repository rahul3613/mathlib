...
begin
 ...
 exact ⟨a, b, by rw [ha]; rw [ ← hb]; rw [ eval_neg]; rw [ neg_add_self]⟩
end,
...
... < nat_degree f * (univ.image (λ x : R, eval x f)).card +
 ...
 (card_image_polynomial_eval (by rw [degree_neg]; rw [ hg2]; exact dec_trivial))
...
calc a ^ (fintype.card K - 1) = (units.mk0 a ha ^ (fintype.card K - 1) : Kˣ) :
 by rw [units.coe_pow]; rw [ units.coe_mk0]
 ... = 1 : by { classical, rw [← fintype.card_units]; rw [ pow_card_eq_one], refl }
...
begin
 ...
 rw [←fintype.card_units]; rw [ ←order_of_eq_card_of_forall_mem_zpowers hx]; rw [ order_of_dvd_iff_pow_eq_one],
 split,
 { intro h, apply h },
 { intros h y,
...
begin
 let φ : Kˣ →* K :=
 { to_fun := λ x, x ^ i,
 map_one' := by rw [units.coe_one]; rw [ one_pow],
 map_mul' := by { intros, rw [units.coe_mul]; rw [ mul_pow] } },
 ...
 rw [fintype.card_units]; rw [ nat.cast_sub]; rw [ cast_card_eq_zero]; rw [ nat.cast_one]; rw [ zero_sub],
 show 1 ≤ q, from fintype.card_pos_iff.mpr ⟨0⟩ },
 rw [← forall_pow_eq_one_iff]; rw [ monoid_hom.ext_iff],
 apply forall_congr, intro x,
 rw [units.ext_iff]; rw [ units.coe_pow]; rw [ units.coe_one]; rw [ monoid_hom.one_apply],
 refl,
end
...
begin
 ...
 by rw [← sum_sdiff ({0} : finset K).subset_univ]; rw [ sum_singleton]; rw [ zero_pow (nat.pos_of_ne_zero hi)]; rw [ add_zero]
 ... = ∑ x : Kˣ, x ^ i : by { rw [← this]; rw [ univ.sum_map φ], refl }
 ... = 0 : by { rw [sum_pow_units K i]; rw [ if_neg], exact hiq, }
end
...
begin
 ...
 exact_mod_cast hp },
 rw [nat_degree_eq_of_degree_eq (degree_sub_eq_left_of_degree_lt h1)]; rw [ nat_degree_X_pow],
end
...
begin
 ...
 intro x,
 rw [multiset.mem_to_finset]; rw [ mem_roots aux]; rw [ is_root.def]; rw [ eval_sub]; rw [ eval_pow]; rw [ eval_X]; rw [ sub_eq_zero]; rw [ pow_card] },
 rw [←this]; rw [ multiset.to_finset_val]; rw [ eq_comm]; rw [ multiset.dedup_eq_self],
 apply nodup_roots,
 rw separable_def,
 convert is_coprime_one_right.neg_right using 1,
 { rw [derivative_sub]; rw [ derivative_X]; rw [ derivative_X_pow]; rw [ char_p.cast_card_eq_zero K]; rw [ C_0]; rw [ zero_mul]; rw [ zero_sub] },
 end
...
theorem frobenius_pow {p : ℕ} [fact p.prime] [char_p K p] {n : ℕ} (hcard : q = p^n) :
 (frobenius K p) ^ n = 1 :=
begin
 ext, conv_rhs { rw [ring_hom.one_def]; rw [ ring_hom.id_apply]; rw [ ← pow_card x]; rw [ hcard], }, clear hcard,
 induction n, {simp},
 rw [pow_succ]; rw [ pow_succ']; rw [ pow_mul]; rw [ ring_hom.mul_def]; rw [ ring_hom.comp_apply]; rw [ frobenius_def]; rw [ n_ih]
end
...
begin
 ...
 dsimp at hn,
 rw [hn]; rw [ ← map_expand_pow_char]; rw [ frobenius_pow hn]; rw [ ring_hom.one_def]; rw [ map_id]
end
...
begin
 ...
 (degree_X_pow 2) (degree_X_pow_sub_C dec_trivial _) (by rw [zmod.card]; rw [ hp_odd]),
 refine ⟨a, b, _⟩,
 rw ← sub_eq_zero,
...
begin
 cases n,
 { rw [nat.totient_zero]; rw [ pow_zero] },
 { rw [← card_units_eq_totient]; rw [ pow_card_eq_one] }
end
...
begin
 ...
 rw [module.card_fintype b]; rw [ ← finite_dimensional.finrank_eq_card_basis b],
end
...
begin
 haveI : fact p.prime := ⟨hp⟩,
 have : ¬ (n : zmod p) = 0,
 { rw [char_p.int_cast_eq_zero_iff _ p]; rw [ ← (nat.prime_iff_prime_int.mp hp).coprime_iff_not_dvd],
 { exact hpn.symm },
 exact zmod.char_p p },
...
begin
 ...
 { rw [← nat.even_iff]; rw [ nat.even_pow],
 rintros ⟨hev, hnz⟩,
...
begin
 ...
 rw [← nat.two_mul_odd_div_two (finite_field.odd_card_of_char_ne_two hF)] at h₁; rw [ mul_comm] at h₁; rw [ pow_mul] at h₁; rw [ pow_two] at h₁,
...
begin
 ...
 { rw [← pow_mul] at h,
 rw [hodd]; rw [ ← fintype.card_units]; rw [ ← order_of_eq_card_of_forall_mem_zpowers hg],
