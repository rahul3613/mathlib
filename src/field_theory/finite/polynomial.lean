...
begin
 ...
 { simp only [alg_hom.map_add, ring_hom.map_add], intros _ _ hf hg, rw [hf]; rw [ hg] },
 { simp only [expand_X, ring_hom.map_mul, alg_hom.map_mul],
 intros _ _ hf, rw [hf]; rw [ frobenius_def], },
end
...
begin
 rw [mem_restrict_degree_iff_sup]; rw [ indicator],
 assume n,
 refine le_trans (multiset.count_le_of_le _ $ degrees_indicator _) (le_of_eq _),
 simp_rw [ ← multiset.coe_count_add_monoid_hom, (multiset.count_add_monoid_hom n).map_sum, add_monoid_hom.map_nsmul, multiset.coe_count_add_monoid_hom, nsmul_eq_mul, nat.cast_id],
 transitivity,
 refine finset.sum_eq_single n _ _,
 { assume b hb ne, rw [multiset.count_singleton]; rw [ if_neg ne.symm]; rw [ mul_zero] },
 { assume h, exact (h $ finset.mem_univ _).elim },
 { rw [multiset.count_singleton_self]; rw [ mul_one] }
end
...
begin
 obtain ⟨i, hi⟩ : ∃ i, a i ≠ b i := by rwa [(≠)] at h; rwa [ function.funext_iff] at h; rwa [ not_forall] at h,
 simp only [indicator, map_prod, map_sub, map_one, map_pow, eval_X, eval_C,
 sub_self, finset.prod_eq_zero_iff],
 refine ⟨i, finset.mem_univ _, _⟩,
 rw [finite_field.pow_card_sub_one_eq_one]; rw [ sub_self],
 rwa [(≠)]; rwa [ sub_eq_zero],
end
...
{ to_fun := λ p e, eval e p,
 ...
 map_smul' := λ a p, by { ext e, rw [smul_eq_C_mul]; rw [ ring_hom.map_mul]; rw [ eval_C], refl } }
end
...
begin
 ...
 { rw [eval_indicator_apply_eq_zero _ _ h.symm]; rw [ smul_zero] },
 { exact λ h, (h $ finset.mem_univ n).elim },
 { rw [eval_indicator_apply_eq_one]; rw [ smul_eq_mul]; rw [ mul_one] } }
end
...
begin
 rw [evalᵢ]; rw [ linear_map.range_comp]; rw [ range_subtype],
...
begin
 ...
 rw [finite_dimensional.finrank_fintype_fun_eq_card]; rw [ finrank_R]
end
...
show p'.1 = (0 : R σ K).1, from congr_arg _ $ by rwa [ker_evalₗ] at this; rwa [ mem_bot] at this
