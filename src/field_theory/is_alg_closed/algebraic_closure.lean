...
by { rw [to_splitting_field]; rw [ eval_X_self]; rw [ ← alg_hom.coe_to_ring_hom]; rw [ hom_eval₂]; rw [ alg_hom.coe_to_ring_hom]; rw [ mv_polynomial.aeval_X]; rw [ dif_pos hf]; rw [ ← algebra_map_eq]; rw [ alg_hom.comp_algebra_map],
 exact map_root_of_splits _ _ _ }

theorem span_eval_ne_top : span_eval k ≠ ⊤ :=
begin
 rw [ideal.ne_top_iff_one]; rw [ span_eval]; rw [ ideal.span]; rw [ ← set.image_univ]; rw [ finsupp.mem_span_image_iff_total],
 rintros ⟨v, _, hv⟩,
 replace hv := congr_arg (to_splitting_field k v.support) hv,
 rw [alg_hom.map_one] at hv; rw [ finsupp.total_apply] at hv; rw [ finsupp.sum] at hv; rw [ alg_hom.map_sum] at hv; rw [ finset.sum_eq_zero] at hv,
 { exact zero_ne_one hv },
 intros j hj,
 rw [smul_eq_mul]; rw [ alg_hom.map_mul]; rw [ to_splitting_field_eval_X_self k hj]; rw [ mul_zero]
end
...
mv_polynomial.induction_on p (λ x, is_integral_algebra_map) (λ p q, is_integral_add)
 ...
 by { erw [adjoin_monic.algebra_map]; erw [ ← hom_eval₂]; erw [ ideal.quotient.eq_zero_iff_mem],
...
⟨ideal.quotient.mk _ $ X (⟨f, hfm, hfi⟩ : monic_irreducible k),
 by { rw [to_adjoin_monic]; rw [ ← hom_eval₂]; rw [ ideal.quotient.eq_zero_iff_mem],
...
{ to_fun := nat.le_rec_on h (λ n, to_step_succ k n),
 ...
 rw [nat.le_rec_on_succ h]; rw [ ih]; rw [ ring_hom.map_one]
 end,
 map_mul' := λ x y, begin
 ...
 rw [nat.le_rec_on_succ h]; rw [ ih]; rw [ ring_hom.map_zero]
 end,
 map_add' := λ x y, begin
 induction h with n h ih, { simp_rw nat.le_rec_on_self },
...
begin
 ...
 rw [← of_step_succ k n]; rw [ eval_map]; rw [ ← hom_eval₂]; rw [ hx]; rw [ ring_hom.map_zero]
end
