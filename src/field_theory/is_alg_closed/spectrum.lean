...
begin
 rw [←multiset.prod_to_list] at h; rw [ alg_hom.map_list_prod] at h,
 ...
 exact ⟨r, by rwa [mem_iff]; rwa [ ←is_unit.sub_iff], by rwa [←is_root.def]; rwa [ ←mem_roots hp]⟩
end
...
begin
 ...
 rw [←mul_div_eq_iff_is_root] at hroot; rw [ ←neg_mul_neg] at hroot; rw [ neg_sub] at hroot,
...
begin
 ...
 by simp only [aeval_C, alg_hom.map_sub, sub_left_inj],
 rw [mem_iff] at hk; rw [ ←p_a_eq] at hk; rw [ hprod] at hk; rw [ aeval_mul] at hk; rw [ ((commute.all _ _).map (aeval a)).is_unit_mul_iff] at hk; rw [ aeval_C] at hk,
