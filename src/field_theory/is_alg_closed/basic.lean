...
begin
 refine ⟨λ h, _, λ hp, by rw [hp]; rw [ roots_C]⟩,
...
⟨x, by rwa [eval₂_eq_eval_map]; rwa [ ← is_root]⟩
...
{ le := (≤),
 ...
 λ _, by erw [← inclusion_inclusion h₁₂.fst h₂₃.fst]; erw [ compat]; erw [ compat]⟩ }
...
{ carrier := ⨆ i : c, (i : subfield_with_hom K L M hL).carrier,
 ...
 { simp [← hij.snd x] },
 { erw [alg_hom.comp_apply]; erw [ ← hji.snd (inclusion h x)]; erw [ inclusion_inclusion]; erw [ inclusion_self]; erw [ alg_hom.id_apply x] }
 end _ rfl } in
...
begin
 ...
 { erw [card_root_set_eq_nat_degree hfsep (is_alg_closed.splits_domain _)]; erw [ nat_degree_X_pow_sub_C] },
 rw hroot,
...
begin
 ...
 refine ⟨_, λ ha, _⟩,
 { rintro ⟨ψ, rfl⟩, rw [aeval_alg_hom_apply ψ x]; rw [ minpoly.aeval]; rw [ map_zero] },
