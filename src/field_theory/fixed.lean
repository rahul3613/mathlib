...
{ smul_mem := λ g x hx g', by rw [hx]; rw [ hx] }

instance : smul_comm_class M (fixed_points.subfield M F) F :=
{ smul_comm := λ m f f', show m • (↑f * f') = f * (m • f'), by rw [smul_mul']; rw [ f.prop m] }
...
polynomial.induction_on p
 (λ x, by rw [polynomial.smul_C]; rw [ smul])
 (λ p q ihp ihq, by rw [smul_add]; rw [ ihp]; rw [ ihq])
 (λ n x ih, by rw [smul_mul']; rw [ polynomial.smul_C]; rw [ smul]; rw [ smul_pow']; rw [ polynomial.smul_X])
...
begin
 ...
 ext g', dsimp only,
 conv_lhs { rw sum_apply, congr, skip, funext, rw [pi.smul_apply]; rw [ sub_smul]; rw [ smul_eq_mul] },
 rw [sum_sub_distrib]; rw [ pi.zero_apply]; rw [ sub_eq_zero],
 conv_lhs { congr, skip, funext,
 rw [to_fun_apply]; rw [ ← mul_inv_cancel_left g g']; rw [ mul_smul]; rw [ ← smul_mul']; rw [ ← to_fun_apply _ x] },
 show ∑ x in s, g • (λ y, l y • mul_action.to_fun G F y) x (g⁻¹ * g') =
 ∑ x in s, (λ y, l y • mul_action.to_fun G F y) x g',
 rw [← smul_sum]; rw [ ← sum_apply _ _ (λ y, l y • to_fun G F y)]; rw [ ← sum_apply _ _ (λ y, l y • to_fun G F y)], dsimp only,
 rw [hla]; rw [ to_fun_apply]; rw [ to_fun_apply]; rw [ smul_smul]; rw [ mul_inv_cancel_left]
end
...
theorem eval₂ : polynomial.eval₂ (subring.subtype $ (fixed_points.subfield G F).to_subring) x
 (minpoly G F x) = 0 :=
begin
 rw [← prod_X_sub_smul.eval G F x]; rw [ polynomial.eval₂_eq_eval_map],
...
(one_ne_zero : (1 : F) ≠ 0) $ by rwa [H] at this; rwa [ polynomial.eval₂_one] at this
...
theorem of_eval₂ (f : polynomial (fixed_points.subfield G F))
 ...
 minpoly G F x ∣ f :=
begin
 erw [← polynomial.map_dvd_map' (subfield.subtype $ fixed_points.subfield G F)]; erw [ minpoly]; erw [ polynomial.map_to_subring _ (subfield G F).to_subring]; erw [ prod_X_sub_smul],
 ...
 rw [polynomial.dvd_iff_is_root]; rw [ polynomial.is_root.def]; rw [ mul_action.of_quotient_stabilizer_mk]; rw [ polynomial.eval_smul']; rw [ ← subfield.to_subring.subtype_eq_subtype]; rw [ ← is_invariant_subring.coe_subtype_hom' G (fixed_points.subfield G F).to_subring]; rw [ ← mul_semiring_action_hom.coe_polynomial]; rw [ ← mul_semiring_action_hom.map_smul]; rw [ smul_polynomial]; rw [ mul_semiring_action_hom.coe_polynomial]; rw [ is_invariant_subring.coe_subtype_hom']; rw [ polynomial.eval_map]; rw [ subfield.to_subring.subtype_eq_subtype]; rw [ hf]; rw [ smul_zero]
end
...
begin
 ...
 rwa [← mul_one (minpoly G F x)] at hfg ; rwa [ hf3] at hfg ; rwa [ mul_right_inj' (monic G F x).ne_zero] at hfg },
 { left,
 ...
 rwa [← one_mul (minpoly G F x)] at hfg ; rwa [ hg3] at hfg ; rwa [ mul_left_inj' (monic G F x).ne_zero] at hfg }
end
...
begin
 ...
 rw [←minpoly_eq_minpoly]; rw [ minpoly]; rw [ coe_algebra_map]; rw [ ←subfield.to_subring.subtype_eq_subtype]; rw [ polynomial.map_to_subring _ (subfield G F).to_subring]; rw [ prod_X_sub_smul],
...
{ casesI nonempty_fintype G,
 ...
 erw [← minpoly_eq_minpoly]; erw [ ← polynomial.separable_map (fixed_points.subfield G F).subtype]; erw [ minpoly]; erw [ polynomial.map_to_subring _ ((subfield G F).to_subring) ],
...
begin
 rw [← cardinal.nat_cast_le]; rw [ finrank_eq_rank],
 apply rank_le_card,
end
