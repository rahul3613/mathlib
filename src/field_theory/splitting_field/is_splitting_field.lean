...
⟨by { rw [splits_map_iff]; rw [ ← is_scalar_tower.algebra_map_eq], exact splits L f },
 subalgebra.restrict_scalars_injective F $
 by { rw [root_set]; rw [ map_map]; rw [ ← is_scalar_tower.algebra_map_eq]; rw [ subalgebra.restrict_scalars_top]; rw [ eq_top_iff]; rw [ ← adjoin_root_set L f]; rw [ algebra.adjoin_le_iff],
...
⟨λ h, eq_bot_iff.2 $ adjoin_root_set L f ▸
 ...
 (by rwa [root_set] at hy) in; rwa [ roots_map _ h] at hy) in; rwa [ multiset.to_finset_map] at hy) in
...
⟨(is_scalar_tower.algebra_map_eq F K L).symm ▸ splits_mul _
 ...
 by rw [root_set]; rw [ polynomial.map_mul]; rw [ roots_mul (mul_ne_zero (map_ne_zero hf : f.map (algebra_map F L) ≠ 0) (map_ne_zero hg))]; rw [ multiset.to_finset_add]; rw [ finset.coe_union]; rw [ algebra.adjoin_union_eq_adjoin_adjoin]; rw [ is_scalar_tower.algebra_map_eq F K L]; rw [ ← map_map]; rw [ roots_map (algebra_map K L) ((splits_id_iff_splits $ algebra_map F K).2 $ splits K f)]; rw [ multiset.to_finset_map]; rw [ finset.coe_image]; rw [ algebra.adjoin_algebra_map]; rw [ ←root_set]; rw [ adjoin_root_set]; rw [ algebra.map_top]; rw [ is_scalar_tower.adjoin_range_to_alg_hom]; rw [ ← map_map]; rw [ ←root_set]; rw [ adjoin_root_set]; rw [ subalgebra.restrict_scalars_top]⟩
...
⟨@algebra.top_to_submodule K L _ _ _ ▸ adjoin_root_set L f ▸
 ...
 if hf : f = 0
 then by { rw [hf] at hy; rw [ root_set_zero] at hy, cases hy }
...
begin
 ...
 { rw [←(algebra.range_top_iff_surjective f.to_alg_hom).mpr f.surjective]; rw [ adjoin_root_set_eq_range (splits F p)]; rw [ adjoin_root_set F p] },
end
...
begin
 ...
 conv_lhs { rw [polynomial.map_map]; rw [ ←is_scalar_tower.algebra_map_eq]; rw [ eq_prod_roots_of_splits h]; rw [ ←multiset.pmap_eq_map _ _ _ hF] },
