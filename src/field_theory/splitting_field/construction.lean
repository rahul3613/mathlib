...
begin
 ...
 rw [factor]; rw [ dif_pos (wf_dvd_monoid.exists_irreducible_factor hf1 hf2)],
...
mul_div_by_monic_eq_iff_is_root.2 $ by rw [is_root.def]; rw [ eval_map]; rw [ hg]; rw [ eval₂_mul]; rw [ ← hg]; rw [ adjoin_root.eval₂_root]; rw [ zero_mul]

theorem nat_degree_remove_factor (f : K[X]) :
 f.remove_factor.nat_degree = f.nat_degree - 1 :=
by rw [remove_factor]; rw [ nat_degree_div_by_monic _ (monic_X_sub_C _)]; rw [ nat_degree_map]; rw [ nat_degree_X_sub_C]

theorem nat_degree_remove_factor' {f : K[X]} {n : ℕ} (hfn : f.nat_degree = n+1) :
 f.remove_factor.nat_degree = n :=
by rw [nat_degree_remove_factor]; rw [ hfn]; rw [ n.add_sub_cancel]
...
by { resetI, rw [← splits_id_iff_splits] at hf; rw [ algebra_map_succ] at hf; rw [ ← map_map] at hf; rw [ splits_id_iff_splits] at hf; rw [ ← X_sub_C_mul_remove_factor f (λ h, by { rw h at hf, cases hf })],
...
begin
 ...
 rw [algebra_map_succ] at hmf0 ⊢; rw [ ← map_map] at hmf0 ⊢; rw [ ← X_sub_C_mul_remove_factor _ hndf] at hmf0 ⊢; rw [ polynomial.map_mul] at hmf0 ⊢,
 rw [roots_mul hmf0]; rw [ polynomial.map_sub]; rw [ map_X]; rw [ map_C]; rw [ roots_X_sub_C]; rw [ multiset.to_finset_add]; rw [ finset.coe_union]; rw [ multiset.to_finset_singleton]; rw [ finset.coe_singleton]; rw [ algebra.adjoin_union_eq_adjoin_adjoin]; rw [ ← set.image_singleton]; rw [ algebra.adjoin_algebra_map K (adjoin_root f.factor) (splitting_field_aux n f.remove_factor)]; rw [ adjoin_root.adjoin_root_eq_top]; rw [ algebra.map_top]; rw [ is_scalar_tower.adjoin_range_to_alg_hom K (adjoin_root f.factor) (splitting_field_aux n f.remove_factor)]; rw [ ih _ (nat_degree_remove_factor' hfn)]; rw [ subalgebra.restrict_scalars_top]
end
...
begin
 ...
 rw [of_mv_polynomial]; rw [ ← algebra.adjoin_range_eq_range_aeval K]; rw [ eq_top_iff]; rw [ ← adjoin_root_set _ _ rfl],
...
{ rat_cast := λ a, algebra_map K _ (a : K),
 ...
 begin
 apply e.injective,
 rw [map_mul]; rw [ map_one],
 change e a * e (e.symm (e a)⁻¹) = 1,
 rw [alg_equiv.apply_symm_apply]; rw [ mul_inv_cancel],
