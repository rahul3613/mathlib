...
eq_pth_root_iff.2 $ by rw [← f.map_frobenius]; rw [ frobenius_pth_root]
...
match x1, x2, H with
| _, _, r.intro n x := quot.sound $ by rw [← iterate_succ_apply]; rw [ iterate_succ']; rw [ iterate_succ']; rw [ ← frobenius_mul]; rw [ nat.succ_add]; apply r.intro
end
...
match y1, y2, H with
| _, _, r.intro n y := quot.sound $ by rw [← iterate_succ_apply]; rw [ iterate_succ']; rw [ iterate_succ']; rw [ ← frobenius_mul]; apply r.intro
end
...
private lemma add_aux_left (x1 x2 y : ℕ × K) (H : r K p x1 x2) :
 ...
 mk K p (x2.1 + y.1, ((frobenius K p)^[y.1] x2.2) + ((frobenius K p)^[x2.1] y.2)) :=
match x1, x2, H with
| _, _, r.intro n x := quot.sound $ by rw [← iterate_succ_apply]; rw [ iterate_succ']; rw [ iterate_succ']; rw [ ← frobenius_add]; rw [ nat.succ_add]; apply r.intro
end
...
private lemma add_aux_right (x y1 y2 : ℕ × K) (H : r K p y1 y2) :
 ...
 mk K p (x.1 + y2.1, ((frobenius K p)^[y2.1] x.2) + ((frobenius K p)^[x.1] y2.2)) :=
match y1, y2, H with
| _, _, r.intro n y := quot.sound $ by rw [← iterate_succ_apply]; rw [ iterate_succ']; rw [ iterate_succ']; rw [ ← frobenius_add]; apply r.intro
end
...
by subst H; induction m with m ih; [simp only [zero_add, iterate_zero_apply],
 rw [ih]; rw [ nat.succ_add]; rw [ iterate_succ']]; apply quot.sound; apply r.intro
...
begin
 ...
 rw [nat.cast_succ]; rw [ nat.cast_succ]; rw [ ih], refl },
...
begin
 split; intro H,
 { rw [nat_cast K p 0] at H; rw [ nat_cast K p 0] at H; rw [ eq_iff'] at H,
 cases H with z H,
...
begin
 ...
 case nat.zero { apply r.sound, rw [(frobenius _ _).iterate_map_one]; rw [ pow_zero] },
 case nat.succ
 { rw [pow_succ]; rw [ ih],
 symmetry,
 apply r.sound,
...
begin
 ...
 rw [iterate_succ_apply']; rw [ ← ih]; refl
end
...
begin
 ...
 rw [iterate_add_apply]; rw [ this _ _]; rw [ add_comm x.1]; rw [ iterate_add_apply]; rw [ this _ _] } },
 ...
 rw [eq_pth_root]; rw [ ring_hom.map_iterate_pth_root] }
end
