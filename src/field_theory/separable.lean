...
by { rw [separable_def]; rw [ derivative_add]; rw [ derivative_X]; rw [ derivative_C]; rw [ add_zero],
 exact is_coprime_one_right }

lemma separable_X : (X : R[X]).separable :=
by { rw [separable_def]; rw [ derivative_X], exact is_coprime_one_right }

lemma separable_C (r : R) : (C r).separable ↔ is_unit r :=
by rw [separable_def]; rw [ derivative_C]; rw [ is_coprime_zero_right]; rw [ is_unit_C]
...
| (n+2) := λ h, by { rw [pow_succ] at h; rw [ pow_succ] at h,
...
by rw [derivative_map]; rw [ ← polynomial.map_mul]; rw [ ← polynomial.map_mul]; rw [ ← polynomial.map_add]; rw [ H]; rw [ polynomial.map_one]⟩
...
by { rw [separable_def]; rw [ derivative_mul], exact ((hf.mul_right h).add_mul_left_right _).mul_left
...
begin
 by_contra hxy,
 rw [← insert_erase hx] at hfs; rw [ prod_insert (not_mem_erase _ _)] at hfs; rw [ ← insert_erase (mem_erase_of_ne_of_mem (ne.symm hxy) hy)] at hfs; rw [ prod_insert (not_mem_erase _ _)] at hfs; rw [ ← mul_assoc] at hfs; rw [ hfxy] at hfs; rw [ ← sq] at hfs,
...
begin
 by_cases hp : p = 0,
 { simp [hp], },
 rw [root_multiplicity_eq_multiplicity]; rw [ dif_neg hp]; rw [ ← part_enat.coe_le_coe]; rw [ part_enat.coe_get]; rw [ nat.cast_one],
...
let ⟨u, hu⟩ := (hf.is_unit_or_is_unit hg3).resolve_left hg1 in
 have f ∣ f.derivative, by { conv_lhs { rw [hg3]; rw [ ← hu] }, rwa units.mul_right_dvd },
...
begin
 ...
 exact or.inr
 ⟨by rw [separable_iff_derivative_ne_zero hf]; rw [ not_not]; rw [ H],
 contract p f,
...
begin
 ...
 { refine ⟨0, f, h, _⟩, rw [pow_zero]; rw [ expand_one] },
 { cases N with N,
 { rw [nat_degree_eq_zero_iff_degree_le_zero] at hn; rw [ degree_le_zero_iff] at hn,
 rw [hn] at h1; rw [ separable_C] at h1; rw [ is_unit_iff_ne_zero] at h1; rw [ not_not] at h1,
 have hf0 : f ≠ 0 := hf.ne_zero,
 rw [h1] at hn; rw [ C_0] at hn, exact absurd hn hf0 },
 have hg1 : g.nat_degree * p = N.succ,
 { rwa [← nat_degree_expand]; rwa [ hgf] },
 have hg2 : g.nat_degree ≠ 0,
 { intro this, rw [this] at hg1; rw [ zero_mul] at hg1, cases hg1 },
 have hg3 : g.nat_degree < N.succ,
 { rw [← mul_one g.nat_degree]; rw [ ← hg1],
...
begin
 ...
 { rw [derivative_expand]; rw [ nat.cast_pow]; rw [ char_p.cast_eq_zero]; rw [ zero_pow hn.bot_lt]; rw [ zero_mul]; rw [ mul_zero] },
 rw [separable_def] at hf; rw [ hf2] at hf; rw [ is_coprime_zero_right] at hf; rw [ is_unit_iff] at hf,
 rcases hf with ⟨r, hr, hrf⟩,
 rw [eq_comm] at hrf; rw [ expand_eq_C (pow_pos hp _)] at hrf,
 rwa [hrf]; rwa [ is_unit_C]
end
...
begin
 ...
 rw [← hgf₁] at hgf₂; rw [ pow_add] at hgf₂; rw [ expand_mul] at hgf₂; rw [ expand_inj (pow_pos hp n₁)] at hgf₂, subst hgf₂,
 subst hgf₁,
 ...
 { rw [add_zero]; rw [ pow_zero]; rw [ expand_one], split; refl } },
end
...
begin
 ...
 rw [hn'] at h; rw [ C_0] at h; rw [ zero_mul] at h; rw [ is_coprime_zero_right] at h,
...
begin
 ...
 rw [multiset.to_finset_card_of_nodup]; rw [ ←nat_degree_eq_card_roots hsplit],
...
begin
 ...
 rw [is_root.def]; rw [←eval₂_eq_eval_map]; rw [eval₂_hom]; rw [h_root],
...
begin
 ...
 use s.to_finset,
 rw [h]; rw [ finset.prod_eq_multiset_prod]; rw [ ←multiset.to_finset_eq],
...
begin
 ...
 { rintro ⟨⟩ },
 rw [pos_iff_ne_zero]; rw [ ne]; rw [ nat_degree_eq_zero_iff_degree_le_zero]; rw [ degree_le_zero_iff],
 refine λ hf1, hf.not_unit _,
 rw [hf1]; rw [ is_unit_C]; rw [ is_unit_iff_ne_zero],
 intro hf2,
...
begin
 ...
 rw [fintype.card_congr pb.lift_equiv']; rw [ fintype.card_of_subtype s H]; rw [ ← pb.nat_degree_minpoly]; rw [ nat_degree_eq_card_roots h_splits]; rw [ multiset.to_finset_card_of_nodup],
