...
begin
 ...
 ... = 0 : by rw [sum_pow_lt_card_sub_one _ hi]; rw [ mul_zero],
 intros a,
 ...
 by { rw [← e'.prod_comp]; rw [ fintype.prod_sum_type]; rw [ univ_unique]; rw [ prod_singleton], refl }
...
begin
 have hq : 0 < q - 1, { rw [← fintype.card_units]; rw [ fintype.card_pos_iff], exact ⟨1⟩ },
 ...
 rw [fintype.card_of_subtype S hS]; rw [ card_eq_sum_ones]; rw [ nat.cast_sum]; rw [ nat.cast_one]; rw [ ← fintype.sum_extend_by_zero S]; rw [ sum_congr rfl (λ x hx, hF x)],
 -- With these preparations under our belt, we will approach the main goal.
 show p ∣ fintype.card {x // ∀ (i : ι), i ∈ s → eval x (f i) = 0},
 rw [← char_p.cast_eq_zero_iff K]; rw [ ← key],
 show ∑ x, eval x F = 0,
