...
begin
 ...
 rwa [eq_top_iff]; rwa [ ←splitting_field.adjoin_root_set]; rwa [ algebra.adjoin_le_iff],
end
...
{ default := 1,
 ...
 rw [alg_equiv.commutes]; rw [ alg_equiv.commutes] }) }
...
begin
 ...
 rw [map_map] at key; rw [ alg_hom.comp_algebra_map] at key,
...
begin
 ...
 ext x hx,
 rw [root_set_def] at hx; rw [ polynomial.map_mul] at hx; rw [ polynomial.roots_mul] at hx,
 ...
 rw [key]; rw [ ←alg_equiv.restrict_normal_commutes]; rw [ ←alg_equiv.restrict_normal_commutes],
 ...
 { rwa [ne.def]; rwa [ mul_eq_zero]; rwa [ map_eq_zero]; rwa [ map_eq_zero]; rwa [ ←mul_eq_zero] }
end
...
begin
 ...
 { rw [h]; rw [ zero_mul],
 exact splits_zero _ },
 { exact false.rec _ (hq (by rw [h.2]; rw [ nat_degree_C])) } },
 by_cases h₂ : p₂.comp q = 0,
 { cases comp_eq_zero_iff.mp h₂ with h h,
 { rw [h]; rw [ mul_zero],
 exact splits_zero _ },
...
begin
 ...
 { rw [←finite_dimensional.finrank_mul_finrank F F⟮α⟯ p.splitting_field]; rw [ intermediate_field.adjoin.finrank hα]; rw [ this] },
 ...
 rw [aeval_def]; rw [ map_root_of_splits _ (splitting_field.splits p) hp],
end
...
begin
 by_cases hp : p = 0,
 { haveI : is_empty (p.root_set ℂ) := by { rw [hp]; rw [ root_set_zero], apply_instance },
 simp_rw [(gal_action_hom p ℂ _).support.eq_empty_of_is_empty, hp, root_set_zero, set.to_finset_empty, finset.card_empty] },
 have inj : function.injective (is_scalar_tower.to_alg_hom ℚ ℝ ℂ) := (algebra_map ℝ ℂ).injective,
 rw [←finset.card_image_of_injective _ subtype.coe_injective]; rw [ ←finset.card_image_of_injective _ inj],
 let a : finset ℂ := _,
 let b : finset ℂ := _,
 let c : finset ℂ := _,
 change a.card = b.card + c.card,
 have ha : ∀ z : ℂ, z ∈ a ↔ aeval z p = 0,
 { intro z, rw [set.mem_to_finset]; rw [ mem_root_set_of_ne hp], apply_instance },
 have hb : ∀ z : ℂ, z ∈ b ↔ aeval z p = 0 ∧ z.im = 0,
 { intro z,
 simp_rw [finset.mem_image, exists_prop, set.mem_to_finset, mem_root_set_of_ne hp],
 split,
 { rintros ⟨w, hw, rfl⟩,
 exact ⟨by rw [aeval_alg_hom_apply]; rw [ hw]; rw [ alg_hom.map_zero], rfl⟩ },
 { rintros ⟨hz1, hz2⟩,
 have key : is_scalar_tower.to_alg_hom ℚ ℝ ℂ z.re = z := by { ext, refl, rw hz2, refl },
 exact ⟨z.re, inj (by rwa [←aeval_alg_hom_apply]; rwa [ key]; rwa [ alg_hom.map_zero]), key⟩ } },
 have hc0 : ∀ w : p.root_set ℂ, gal_action_hom p ℂ
 (restrict p ℂ (complex.conj_ae.restrict_scalars ℚ)) w = w ↔ w.val.im = 0,
 { intro w,
 rw [subtype.ext_iff]; rw [ gal_action_hom_restrict],
...
begin
 ...
 rw [multiset.to_finset_card_of_nodup]; rw [ ←nat_degree_eq_card_roots],
 ...
 rw [←p_roots]; rw [ ←set.to_finset_card (root_set p ℝ)]; rw [ ←set.to_finset_card (root_set p ℂ)],
...
begin
 ...
 have hn : 2 ∣ n :=
 equiv.perm.two_dvd_card_support (by rw [←monoid_hom.map_pow]; rw [ ←monoid_hom.map_pow]; rw [ show alg_equiv.restrict_scalars ℚ complex.conj_ae ^ 2 = 1]; rw [ from alg_equiv.ext complex.conj_conj]; rw [ monoid_hom.map_one]; rw [ monoid_hom.map_one]),
