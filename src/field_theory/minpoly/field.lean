...
begin
 ...
 { rw [(monic hx).leading_coeff]; rw [ pmonic.leading_coeff] },
...
by { refine minpoly.dvd K x _, rw [aeval_map_algebra_map]; rw [ minpoly.aeval] }
...
begin
 ...
 rw [← is_scalar_tower.algebra_map_eq]; rw [ ← is_scalar_tower.algebra_map_eq]
end
...
begin
 ...
 by rwa [←C_mul]; rwa [ inv_mul_cancel]; rwa [ C_1], by rwa [←C_mul]; rwa [ mul_inv_cancel]; rwa [ C_1]⟩, rfl⟩ hp1 },
 { rw [aeval_mul]; rw [ hp2]; rw [ zero_mul] },
 { rwa [polynomial.monic]; rwa [ leading_coeff_mul]; rwa [ leading_coeff_C]; rwa [ mul_inv_cancel] },
end
...
minpoly.unique _ _ (minpoly.monic hx)
 (by rw [h]; rw [ aeval_algebra_map_apply]; rw [ minpoly.aeval]; rw [ ring_hom.map_zero])
...
begin
 ...
 rw [degree_eq_nat_degree qmo.ne_zero]; rw [ degree_eq_nat_degree ((minpoly.monic hx).comp_X_sub_C _).ne_zero]; rw [ with_bot.coe_le_coe]; rw [ nat_degree_comp]; rw [ nat_degree_X_sub_C]; rw [ mul_one],
 rwa [degree_eq_nat_degree (minpoly.ne_zero hx)] at H ; rwa [ degree_eq_nat_degree (qmo.comp_X_add_C _).ne_zero] at H ; rwa [ with_bot.coe_le_coe] at H ; rwa [ nat_degree_comp] at H ; rwa [ nat_degree_X_add_C] at H ; rwa [ mul_one] at H }
end
...
⟨φ x, by rw [mem_roots_map (minpoly.ne_zero_of_finite_field_extension F x.val)]; rw [ subtype.val_eq_coe]; rw [ ←aeval_def]; rw [ aeval_alg_hom_apply]; rw [ minpoly.aeval]; rw [ map_zero]⟩
...
eq_of_monic_of_associated (monic hx) (monic_X_sub_C y) (associated_of_dvd_dvd
 ...
 (dvd_iff_is_root.2 h)),
by { have := aeval A x, rwa [key] at this ; rwa [ alg_hom.map_sub] at this ; rwa [ aeval_X] at this ; rwa [ aeval_C] at this ; rwa [ sub_eq_zero] at this ; rwa [ eq_comm] at this }
