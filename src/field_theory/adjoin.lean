...
by { ext, rw [mem_to_subalgebra]; rw [ algebra.mem_bot]; rw [ mem_bot] }
...
begin
 ...
 rw [coe_algebra_map_over_bot]; rw [ (bot_equiv F E).apply_symm_apply]; rw [ bot_equiv_symm]; rw [ is_scalar_tower.algebra_map_apply F (⊥ : intermediate_field F E) E]
end
...
by { ext, rw [mem_restrict_scalars]; rw [ mem_bot], exact set.ext_iff.mp subtype.range_coe x }
...
by rw [adjoin_adjoin_left]; rw [ adjoin_adjoin_left]; rw [ set.union_comm]
...
begin
 ...
 rw [ring_hom.map_field_closure]; rw [ set.image_union]; rw [ ← set.range_comp]; rw [ ← ring_hom.coe_comp]; rw [ f.comp_algebra_map],
 refl,
end
...
begin
 ...
 by rwa [←le_integral_closure_iff_is_integral]; rwa [ algebra.adjoin_le_iff],
...
begin
 ...
 rw [←K.range_val]; rw [ adjoin_algebraic_to_subalgebra (λ x, is_algebraic_of_mem_root_set)],
...
begin
 ...
 rw [intermediate_field.minpoly_eq]; rw [ subtype.coe_mk]; rw [ mem_root_set_of_ne]; rw [ minpoly.aeval],
...
by { rw [eq_bot_iff]; rw [ adjoin_le_iff], refl, }
...
by rw [← to_subalgebra_eq_iff]; rw [ ← rank_eq_rank_subalgebra]; rw [ subalgebra.rank_eq_one_iff]; rw [ bot_to_subalgebra]

@[simp] lemma finrank_eq_one_iff : finrank F K = 1 ↔ K = ⊥ :=
by rw [← to_subalgebra_eq_iff]; rw [ ← finrank_eq_finrank_subalgebra]; rw [ subalgebra.finrank_eq_one_iff]; rw [ bot_to_subalgebra]
...
alg_equiv.of_bijective
 ...
 ⟨y, by { rw [ring_hom.comp_apply]; rw [ adjoin_root.lift_of], exact hy }⟩))
 (set.singleton_subset_iff.mpr ⟨adjoin_root.root (minpoly F α),
 by { rw [ring_hom.comp_apply]; rw [ adjoin_root.lift_root], refl }⟩)) } end)
...
{ gen := adjoin_simple.gen K x,
 ...
 basis_eq_pow := λ i,
 by rw [power_basis_aux]; rw [ basis.map_apply]; rw [ power_basis.basis_eq_pow]; rw [ alg_equiv.to_linear_equiv_apply]; rw [ alg_equiv.map_pow]; rw [ adjoin_root.power_basis_gen]; rw [ adjoin_root_equiv_adjoin_apply_root] }
...
(adjoin.power_basis h).lift_equiv'.trans ((equiv.refl _).subtype_equiv (λ x,
 by rw [adjoin.power_basis_gen]; rw [ minpoly_gen h]; rw [ equiv.refl_apply]))
...
begin
 ...
 rw [finset.coe_insert]; rw [ set.insert_eq]; rw [ set.union_comm]; rw [ ←adjoin_adjoin_left],
...
{ bot := ⟨⊥, (algebra.of_id F K).comp (bot_equiv F E).to_alg_hom⟩,
 ...
 rw [show s = (algebra_map F _) u]; rw [ from subtype.ext hu.symm]; rw [ alg_hom.commutes],
 rw [show t = (algebra_map F _) u]; rw [ from subtype.ext (eq.trans hu hst).symm]; rw [ alg_hom.commutes],
 end⟩ }
...
{ to_fun := λ s, (classical.some s.mem).2 ⟨s, (classical.some_spec s.mem).2⟩,
 ...
 rw [lifts.eq_of_le hxw]; rw [ lifts.eq_of_le hyw]; rw [ lifts.eq_of_le hzw]; rw [ ←w.2.map_add],
 refl,
 end,
 map_mul' := λ s t, begin
 obtain ⟨w, hw, hxw, hyw, hzw⟩ := lifts.exists_max_three hc
 (classical.some_spec s.mem).1 (classical.some_spec t.mem).1
 (classical.some_spec (s * t).mem).1,
 rw [lifts.eq_of_le hxw]; rw [ lifts.eq_of_le hyw]; rw [ lifts.eq_of_le hzw]; rw [ ←w.2.map_mul],
 refl,
 end,
...
begin
 ...
 { rw [←subtype.coe_mk x hx]; rw [ hx']; rw [ subalgebra.coe_zero]; rw [ inv_zero],
...
begin
 ...
 rwa this at h },
 rw [algebra.tensor_product.product_map_range]; rw [ E1.range_val]; rw [ E2.range_val]; rw [ sup_to_subalgebra],
end
...
begin
 ...
 rw [is_algebraic_iff]; rw [ subtype.coe_mk]; rw [ ←subtype.coe_mk x hx]; rw [ ←is_algebraic_iff],
...
(adjoin.power_basis pb.is_integral_gen).equiv_of_minpoly pb
 ...
 (by rw [adjoin.power_basis_gen]; rw [ adjoin_simple.algebra_map_gen]))
...
by rw [equiv_adjoin_simple]; rw [ equiv_of_minpoly_symm]; rw [ equiv_of_minpoly_aeval]; rw [ adjoin.power_basis_gen]

@[simp]
lemma equiv_adjoin_simple_symm_gen (pb : power_basis K L) :
 pb.equiv_adjoin_simple.symm pb.gen = (adjoin_simple.gen K pb.gen) :=
by rw [equiv_adjoin_simple]; rw [ equiv_of_minpoly_symm]; rw [ equiv_of_minpoly_gen]; rw [ adjoin.power_basis_gen]
