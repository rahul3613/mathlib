...
begin
 ext x,
 rw [subring.coe_Inf]; rw [ set.mem_Inter]; rw [ set.mem_Inter],
...
{ carrier := { (x / y) | (x ∈ subring.closure s) (y ∈ subring.closure s) },
 ...
 by_cases hx0 : dx = 0, { rwa [hx0]; rwa [ div_zero]; rwa [ zero_add] },
 by_cases hy0 : dy = 0, { rwa [hy0]; rwa [ div_zero]; rwa [ add_zero] },
 exact ⟨nx * dy + dx * ny,
...
{ inv_mem' := λ x (hx : f x = g x), show f x⁻¹ = g x⁻¹, by rw [map_inv₀ f]; rw [ map_inv₀ g]; rw [ hx],
