...
begin
 ...
 have : x = taylor (r - r) x,
 { simp },
 rwa [this] at hx; rwa [ sub_eq_add_neg] at hx; rwa [ ←taylor_taylor] at hx; rwa [ ←taylor_mul] at hx; rwa [ linear_map.map_eq_zero_iff _ (taylor_injective _)] at hx; rwa [ mul_right_mem_non_zero_divisors_eq_zero_iff hp] at hx; rwa [ linear_map.map_eq_zero_iff _ (taylor_injective _)] at hx,
end
...
by rw [←mk_one]; rw [ ←mk_one]; rw [ mk_eq_div]; rw [ laurent_aux_div]; rw [ mk_eq_div]; rw [ taylor_one]; rw [ _root_.map_one]
...
by rw [←algebra_map_X]; rw [ laurent_algebra_map]; rw [ taylor_X]; rw [ _root_.map_add]; rw [ algebra_map_C]

@[simp] lemma laurent_C (x : R) : laurent r (C x) = C x :=
by rw [←algebra_map_C]; rw [ laurent_algebra_map]; rw [ taylor_C]
