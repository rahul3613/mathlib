...
begin
 ...
 { simp },
 { rw [finset.sum_insert hi]; rw [ add_mem_class.coe_add]; rw [ H]; rw [ finset.sum_insert hi] }
end
...
begin
 ...
 induction finset.univ using finset.induction_on with i s hi H,
 { simp },
 { rw [finset.prod_insert hi]; rw [ mul_mem_class.coe_mul]; rw [ H]; rw [ finset.prod_insert hi] }
end
...
begin
 ...
 { rw [aeval_add]; rw [ aeval_add]; rw [ add_mem_class.coe_add]; rw [ hf]; rw [ hg] },
...
begin
 ...
 rwa [eval₂_eq_eval_map]; rwa [ ← eval₂_at_apply]; rwa [ eval₂_eq_eval_map]; rwa [ polynomial.map_map]; rwa [ ← is_scalar_tower.algebra_map_eq]; rwa [ ← eval₂_eq_eval_map] },
 { obtain ⟨P, hPmo, hProot⟩ := h,
 refine ⟨P, hPmo, _⟩,
 rw [← aeval_def]; rw [ aeval_coe]; rw [ aeval_def]; rw [ hProot]; rw [ zero_mem_class.coe_zero] },
end
...
by { ext, rw [← mem_to_subalgebra]; rw [ ← mem_to_subalgebra]; rw [ h] }
...
λ x hx, S.to_subalgebra.range_subset (by rwa [set.mem_range]; rwa [ ← ring_hom.mem_field_range])
...
λ U V H, ext $ λ x, by rw [← mem_restrict_scalars K]; rw [ H]; rw [ mem_restrict_scalars]
...
by { rw [set_like.ext_iff]; rw [ set_like.ext'_iff]; rw [ set.ext_iff], refl }
...
by rw [←is_algebraic_iff_is_integral]; rw [ is_algebraic_iff]; rw [ is_algebraic_iff_is_integral]
