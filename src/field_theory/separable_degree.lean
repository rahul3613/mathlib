...
let ⟨a, ha⟩ := hf.dvd_degree' in by rw [←ha]; rw [ one_pow a]; rw [ mul_one]
...
begin
 casesI hF,
 { exact ⟨f, irred.separable, ⟨0, by rw [pow_zero]; rw [ expand_one]⟩⟩ },
...
begin
 ...
 rw [pow_add] at h_expand; rw [ expand_mul] at h_expand; rw [ expand_inj (pow_pos (ne_zero.pos q) m)] at h_expand,
 subst h_expand,
 rcases is_unit_or_eq_zero_of_separable_expand q s (ne_zero.pos q) hg with h | rfl,
 { rw [nat_degree_expand]; rw [ nat_degree_eq_zero_of_is_unit h]; rw [ zero_mul] },
 { rw [nat_degree_expand]; rw [ pow_zero]; rw [ mul_one] },
end
