...
calc f p q = f (q' * p) (q' * q) : (H hq hq').symm
 ... = f (q * p') (q * q') : by rw [h]; rw [ mul_comm q']
 ... = f p' q' : H hq' hq
...
by rw [mk_eq_div']; rw [ ring_hom.map_zero]; rw [ div_zero]
...
lemma mk_eq_localization_mk (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
 ...
 ⟨q, mem_non_zero_divisors_iff_ne_zero.mpr hq⟩) :=
by rw [mk_def_of_ne]; rw [ localization.mk_eq_mk']
...
by rw [mk_def_of_ne _ hq]; rw [ mk_def_of_ne _ hq']; rw [ of_fraction_ring_injective.eq_iff]; rw [ is_localization.mk'_eq_iff_eq']; rw [ set_like.coe_mk]; rw [ set_like.coe_mk]; rw [ (is_fraction_ring.injective K[X] (fraction_ring K[X])).eq_iff]
...
lemma lift_on'_mk {P : Sort v} (p q : K[X])
 ...
 (ratfunc.mk p q).lift_on' f @H = f p q :=
begin
 rw [ratfunc.lift_on']; rw [ ratfunc.lift_on_mk _ _ _ f0],
...
| ⟨p⟩ h := have p ≠ 0 := λ hp, h $ by rw [hp]; rw [ of_fraction_ring_zero],
...
begin
 cases x,
 induction x,
 { rw [←of_fraction_ring_smul]; rw [ ←of_fraction_ring_smul]; rw [ localization.smul_mk]; rw [ localization.smul_mk]; rw [ smul_eq_mul]; rw [ polynomial.smul_eq_C_mul] },
 { simp only }
end
...
begin
 by_cases hq : q = 0,
 { rw [hq]; rw [ mk_zero]; rw [ mk_zero]; rw [ ←of_fraction_ring_smul]; rw [ smul_zero] },
 { rw [mk_eq_localization_mk _ hq]; rw [ mk_eq_localization_mk _ hq]; rw [ ←localization.smul_mk]; rw [ ←of_fraction_ring_smul] }
end

instance : is_scalar_tower R K[X] (ratfunc K) :=
⟨λ c p q, q.induction_on' (λ q r _, by rw [← mk_smul]; rw [ smul_assoc]; rw [ mk_smul]; rw [ mk_smul])⟩
...
{ to_fun := λ f, ratfunc.lift_on f (λ n d, if h : φ d ∈ S[X]⁰
 ...
 begin
 rw [dif_pos]; rw [ dif_pos]; rw [ of_fraction_ring.inj_eq]; rw [ localization.mk_eq_mk_iff],
 rotate,
 { exact hφ hq' },
 { exact hφ hq },
 ...
 map_one' := begin
 rw [←of_fraction_ring_one]; rw [ ←localization.mk_one]; rw [ lift_on_of_fraction_ring_mk]; rw [ dif_pos],
 ...
 simp_rw [←of_fraction_ring_mul, localization.mk_mul, lift_on_of_fraction_ring_mk, dif_pos hq, dif_pos hq', dif_pos hqq', ←of_fraction_ring_mul, submonoid.coe_mul, map_mul, localization.mk_mul, submonoid.mk_mul_mk] },
 { refl },
 { refl }
 end }
...
{ map_zero' := begin
 simp_rw [monoid_hom.to_fun_eq_coe, ←of_fraction_ring_zero, ←localization.mk_zero (1 : R[X]⁰),
...
begin
 ...
 rwa [←map_mul] at this; rwa [ ←map_mul] at this; rwa [ hφ.eq_iff] at this; rwa [ mul_comm] at this; rwa [ mul_comm y_a] at this,
...
{ map_add' := λ x y, by { simp only [monoid_with_zero_hom.to_fun_eq_coe],
 casesI subsingleton_or_nontrivial R,
 { rw [subsingleton.elim (x + y) y]; rw [ subsingleton.elim x 0]; rw [ map_zero]; rw [ zero_add] },
 cases x, cases y, induction x with p q, induction y with p' q',
 { rw [←of_fraction_ring_add]; rw [ localization.add_mk],
 ...
 { rw [mul_comm _ p]; rw [ mul_comm _ p']; rw [ mul_comm _ (φ p')]; rw [ add_comm],
...
{ to_fun := λ x, ratfunc.mk (algebra_map _ _ x) 1,
 ...
 by simp_rw [mk_one', ← mk_smul, mk_def_of_ne (c • p) hq, mk_def_of_ne p hq, ← of_fraction_ring_mul, is_localization.mul_mk'_eq_mk'_of_mul, algebra.smul_def],
...
by rw [←mk_eq_div]; rw [ mk_smul]; rw [ mk_eq_div]
...
⟨(injective_iff_map_eq_zero _).mp (algebra_map_injective K) _, λ hx, by rw [hx]; rw [ ring_hom.map_zero]⟩
...
{ commutes' := λ r, by simp_rw [ring_hom.to_fun_eq_coe, coe_map_ring_hom_eq_coe_map, algebra_map_apply r, map_apply_div, map_one, alg_hom.commutes],
 ..map_ring_hom φ hφ }
...
{ commutes' := λ r, by simp_rw [ring_hom.to_fun_eq_coe, alg_hom.to_ring_hom_eq_coe, algebra_map_apply r, lift_ring_hom_apply_div, alg_hom.coe_to_ring_hom, map_one, div_one, alg_hom.commutes],
...
{ map_units := λ y, by rw ← of_fraction_ring_algebra_map;
 ...
 eq_iff_exists := λ x y, by rw [← of_fraction_ring_algebra_map]; rw [ ← of_fraction_ring_algebra_map];
...
by rw [← mk_eq_div]; rw [ lift_on_mk _ _ f f0 @H']

@[simp] lemma lift_on'_div {P : Sort v} (p q : K[X])
 (f : ∀ (p q : K[X]), P) (f0 : ∀ p, f p 0 = f 0 1) (H) :
 (algebra_map _ (ratfunc K) p / algebra_map _ _ q).lift_on' f @H = f p q :=
begin
 rw [ratfunc.lift_on']; rw [ lift_on_div _ _ _ f0],
 exact lift_on_condition_of_lift_on'_condition @H
end
...
by rw [is_fraction_ring.mk'_eq_div]; rw [ is_fraction_ring.mk'_eq_div]; rw [ ← mk_eq_div']; rw [ ← mk_eq_div]
...
x.lift_on' (λ p q, if q = 0 then ⟨0, 1⟩ else let r := gcd p q in
 ...
 rw [euclidean_domain.mul_div_mul_cancel ha hdivp]; rw [ euclidean_domain.mul_div_mul_cancel ha hdivq]; rw [ leading_coeff_div hdeg]; rw [ leading_coeff_div hdeg']; rw [ polynomial.leading_coeff_mul]; rw [ polynomial.leading_coeff_C]; rw [ div_C_mul]; rw [ div_C_mul]; rw [ ← mul_assoc]; rw [ ← polynomial.C_mul]; rw [ ← mul_assoc]; rw [ ← polynomial.C_mul],
 split; congr; rw [inv_div]; rw [ mul_comm]; rw [ mul_div_assoc]; rw [ ← mul_assoc]; rw [ inv_inv]; rw [ _root_.mul_inv_cancel ha']; rw [ one_mul]; rw [ inv_div],
 end
...
begin
 rw [num_denom]; rw [ lift_on'_div]; rw [ if_neg hq],
 intros p,
...
private lemma num_div' (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
 ...
 polynomial.C ((q / gcd p q).leading_coeff⁻¹) * (p / gcd p q) :=
by rw [num]; rw [ num_denom_div _ hq]
...
@[simp] lemma denom_div (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
 ...
 polynomial.C ((q / gcd p q).leading_coeff⁻¹) * (q / gcd p q) :=
by rw [denom]; rw [ num_denom_div _ hq]
...
x.induction_on (λ p q hq, begin
 ...
 rw [num_div p q]; rw [ denom_div p hq]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ mul_div_mul_left]; rw [ div_eq_div_iff]; rw [ ← ring_hom.map_mul]; rw [ ← ring_hom.map_mul]; rw [ mul_comm _ q]; rw [ ← euclidean_domain.mul_div_assoc]; rw [ ← euclidean_domain.mul_div_assoc]; rw [ mul_comm],
 { apply gcd_dvd_right },
 { apply gcd_dvd_left },
...
⟨λ h, by rw [← num_div_denom x]; rw [ h]; rw [ ring_hom.map_zero]; rw [ zero_div],
 λ h, h.symm ▸ num_zero⟩
...
begin
 rw [← (algebra_map_injective K).eq_iff]; rw [ eq_div_iff (algebra_map_ne_zero hq)],
 conv_rhs { rw ← num_div_denom x },
 rw [ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ div_eq_mul_inv]; rw [ mul_assoc]; rw [ mul_comm (has_inv.inv _)]; rw [ ← mul_assoc]; rw [ ← div_eq_mul_inv]; rw [ div_eq_iff],
...
begin
 conv_lhs { rw [← num_div_denom x]; rw [ ← num_div_denom y] },
 rw [div_add_div]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_add]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_mul],
 { exact algebra_map_ne_zero (denom_ne_zero x) },
...
by rw [num_mul_eq_mul_denom_iff (denom_ne_zero x)]; rw [ _root_.map_neg]; rw [ neg_div]; rw [ num_div_denom]

lemma num_denom_mul (x y : ratfunc K) :
 (x * y).num * (x.denom * y.denom) = x.num * y.num * (x * y).denom :=
(num_mul_eq_mul_denom_iff (mul_ne_zero (denom_ne_zero x) (denom_ne_zero y))).mpr $
 by conv_lhs { rw [← num_div_denom x]; rw [ ← num_div_denom y]; rw [ div_mul_div_comm]; rw [ ← ring_hom.map_mul]; rw [ ← ring_hom.map_mul] }
...
begin
 ...
 use denom x * q,
 rw [ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ ← div_mul_div_comm]; rw [ div_self]; rw [ mul_one]; rw [ num_div_denom],
...
begin
 ...
 rw [ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ ← div_mul_div_comm]; rw [ num_div_denom]; rw [ num_div_denom]
end
...
begin
 ...
 rw [ring_hom.map_mul]; rw [ ring_hom.map_add]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ ← div_add_div]; rw [ num_div_denom]; rw [ num_div_denom],
 { exact algebra_map_ne_zero (denom_ne_zero x) },
 { exact algebra_map_ne_zero (denom_ne_zero y) },
end
...
by rw [←num_div_denom f]; rw [ lift_monoid_with_zero_hom_apply_div]; rw [ num_div_denom]
...
begin
 ...
 rw [h_zero] at h; rw [ zero_mul] at h,
...
lemma smul_eq_C_mul (r : K) (x : ratfunc K) :
 r • x = C r * x :=
by rw [algebra.smul_def]; rw [ algebra_map_eq_C]
...
begin
 ...
 rw [div_add_div _ _ hx hy]; rw [ eq_div_iff (mul_ne_zero hx hy)]; rw [ div_eq_mul_inv]; rw [ mul_right_comm]; rw [ ← div_eq_mul_inv]; rw [ div_eq_iff hxy],
...
begin
 ...
 rw polynomial.eval₂_mul at this,
 cases mul_eq_zero.mp this; contradiction },
 rw [div_mul_div_comm]; rw [ eq_div_iff (mul_ne_zero hx hy)]; rw [ div_eq_mul_inv]; rw [ mul_right_comm]; rw [ ← div_eq_mul_inv]; rw [ div_eq_iff hxy],
...
by rw [int_degree]; rw [ num_zero]; rw [ nat_degree_zero]; rw [ denom_zero]; rw [ nat_degree_one]; rw [ sub_self]

@[simp] lemma int_degree_one : int_degree (1 : ratfunc K) = 0 :=
by rw [int_degree]; rw [ num_one]; rw [ denom_one]; rw [ sub_self]

@[simp] lemma int_degree_C (k : K): int_degree (ratfunc.C k) = 0 :=
by rw [int_degree]; rw [ num_C]; rw [ nat_degree_C]; rw [ denom_C]; rw [ nat_degree_one]; rw [ sub_self]

@[simp] lemma int_degree_X : int_degree (X : ratfunc K) = 1 :=
by rw [int_degree]; rw [ ratfunc.num_X]; rw [ polynomial.nat_degree_X]; rw [ ratfunc.denom_X]; rw [ polynomial.nat_degree_one]; rw [ int.coe_nat_one]; rw [ int.coe_nat_zero]; rw [ sub_zero]

@[simp] lemma int_degree_polynomial {p : K[X]} :
 int_degree (algebra_map K[X] (ratfunc K) p) = nat_degree p :=
by rw [int_degree]; rw [ ratfunc.num_algebra_map]; rw [ ratfunc.denom_algebra_map]; rw [ polynomial.nat_degree_one]; rw [ int.coe_nat_zero]; rw [ sub_zero]
...
begin
 ...
 norm_cast,
 rw [← polynomial.nat_degree_mul x.denom_ne_zero y.denom_ne_zero]; rw [ ← polynomial.nat_degree_mul (ratfunc.num_ne_zero (mul_ne_zero hx hy)) (mul_ne_zero x.denom_ne_zero y.denom_ne_zero)]; rw [ ← polynomial.nat_degree_mul (ratfunc.num_ne_zero hx) (ratfunc.num_ne_zero hy)]; rw [ ← polynomial.nat_degree_mul (mul_ne_zero (ratfunc.num_ne_zero hx) (ratfunc.num_ne_zero hy)) (x * y).denom_ne_zero]; rw [ ratfunc.num_denom_mul]
end
...
begin
 by_cases hx : x = 0,
 { rw [hx]; rw [ neg_zero] },
 { rw [int_degree]; rw [ int_degree]; rw [ ← nat_degree_neg x.num],
...
begin
 by_cases hx : x = 0,
 { simp [hx] at *, },
 rw [int_degree_add hxy]; rw [ ← nat_degree_num_mul_right_sub_nat_degree_denom_mul_left_eq_int_degree hx y.denom_ne_zero]; rw [ mul_comm y.denom]; rw [ ← nat_degree_num_mul_right_sub_nat_degree_denom_mul_left_eq_int_degree hy x.denom_ne_zero]; rw [ le_max_iff]; rw [sub_le_sub_iff_right]; rw [ int.coe_nat_le]; rw [ sub_le_sub_iff_right]; rw [ int.coe_nat_le]; rw [ ← le_max_iff]; rw [ mul_comm y.num],
...
by rw [coe_num_denom]; rw [ num_C]; rw [ denom_C]; rw [ coe_coe]; rw [ polynomial.coe_C]; rw [ coe_C]; rw [ coe_coe]; rw [ polynomial.coe_one]; rw [ power_series.coe_one]; rw [ div_one]

-- TODO: generalize over other modules
@[simp, norm_cast] lemma coe_smul (r : F) : ((r • f : ratfunc F) : laurent_series F) = r • f :=
by rw [smul_eq_C_mul]; rw [ ←C_mul_eq_smul]; rw [ coe_mul]; rw [ coe_C]

@[simp, norm_cast] lemma coe_X : ((X : ratfunc F) : laurent_series F) = single 1 1 :=
by rw [coe_num_denom]; rw [ num_X]; rw [ denom_X]; rw [ coe_coe]; rw [ polynomial.coe_X]; rw [ coe_X]; rw [ coe_coe]; rw [ polynomial.coe_one]; rw [ power_series.coe_one]; rw [ div_one]
...
begin
 convert coe_div _ _;
 rw [←mk_one]; rw [ coe_def]; rw [ coe_alg_hom]; rw [ mk_eq_div]; rw [ lift_alg_hom_apply_div]; rw [ map_one]; rw [ div_one]; rw [ algebra.of_id_apply]
end
