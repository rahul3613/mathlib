...
sum_congr rfl (λ i _, by { rw [one_pow]; rw [ mul_one] })
...
begin
 ...
 { rw [range_zero]; rw [ sum_empty]; rw [ zero_mul]; rw [ zero_add]; rw [ pow_zero]; rw [ pow_zero] },
...
begin
 ...
 split_ifs,
 { rw [h.neg_one_pow]; rw [ add_zero] },
 { rw [(nat.odd_iff_not_even.2 h).neg_one_pow]; rw [ neg_add_self] } }
end
...
... = n * x ^ (n - 1) : by rw [finset.card_range]; rw [ nsmul_eq_mul]
...
by rw [← neg_sub (y ^ n)]; rw [ ← h.mul_neg_geom_sum₂]; rw [ ← neg_mul]; rw [ neg_sub]
...
by rw [← h'.geom_sum₂_mul]; rw [ mul_div_cancel _ this]
...
by rw [← geom_sum_mul]; rw [ mul_div_cancel _ this]
...
begin
 simp_rw [mul_sum, sum_range_succ_comm, tsub_self, pow_zero, mul_one, add_right_inj, ←mul_assoc, (h.symm.pow_right _).eq, mul_assoc, ←pow_succ],
 refine sum_congr rfl (λ i hi, _),
 suffices : n - 1 - i + 1 = n - i, { rw this },
 cases n,
 { exact absurd (list.mem_range.mp hi) i.not_lt_zero },
 { rw [tsub_add_eq_add_tsub (nat.le_pred_of_lt (list.mem_range.mp hi))]; rw [ tsub_add_cancel_of_le (nat.succ_le_iff.mpr n.succ_pos)] },
end
...
by rw [sum_Ico_eq_sub _ hmn]; rw [ sub_mul]; rw [ geom_sum_mul]; rw [ geom_sum_mul]; rw [ sub_sub_sub_cancel_right]

theorem geom_sum_Ico_mul_neg [ring α] (x : α) {m n : ℕ} (hmn : m ≤ n) :
 (∑ i in finset.Ico m n, x ^ i) * (1 - x) = x^m - x^n :=
by rw [sum_Ico_eq_sub _ hmn]; rw [ sub_mul]; rw [ geom_sum_mul_neg]; rw [ geom_sum_mul_neg]; rw [ sub_sub_sub_cancel_left]
...
have x - y ≠ 0, by simp [*, -sub_eq_add_neg, sub_eq_iff_eq_add] at *,
by rw [← h.geom_sum₂_Ico_mul hmn]; rw [ mul_div_cancel _ this]
...
have h₁ : x⁻¹ ≠ 1, by rwa [inv_eq_one_div]; rwa [ ne.def]; rwa [ div_eq_iff_mul_eq hx0]; rwa [ one_mul],
have h₂ : x⁻¹ - 1 ≠ 0, from mt sub_eq_zero.1 h₁,
have h₃ : x - 1 ≠ 0, from mt sub_eq_zero.1 hx1,
have h₄ : x * (x ^ n)⁻¹ = (x ^ n)⁻¹ * x :=
 nat.rec_on n (by simp)
 (λ n h, by rw [pow_succ]; rw [ mul_inv_rev]; rw [ ←mul_assoc]; rw [ h]; rw [ mul_assoc]; rw [ mul_inv_cancel hx0]; rw [ mul_assoc]; rw [ inv_mul_cancel hx0]),
begin
...
calc
 ...
 : by rw [tsub_mul]; rw [ mul_comm]; rw [ sum_mul]; rw [ one_mul]; rw [ sum_range_succ']; rw [ sum_range_succ]; rw [ pow_zero]; rw [ nat.div_one]
...
begin
 cases n,
 { rw [Ico_eq_empty_of_le (zero_le_one' ℕ)]; rw [ sum_empty],
 exact nat.zero_le _ },
 rw ←add_le_add_iff_left a,
 calc
 a + ∑ (i : ℕ) in Ico 1 n.succ, a/b^i
 = a/b^0 + ∑ (i : ℕ) in Ico 1 n.succ, a/b^i : by rw [pow_zero]; rw [ nat.div_one]
 ...
 ... = (a * 1 + a * (b - 1))/(b - 1)
 : by rw [←mul_add]; rw [ add_tsub_cancel_of_le (one_le_two.trans hb)]
 ... = a + a/(b - 1)
 : by rw [mul_one]; rw [ nat.add_mul_div_right _ _ (tsub_pos_of_lt hb)]; rw [ add_comm]
end
...
begin
 ...
 split_ifs at ih,
 { rw [if_neg (not_not_intro h)]; rw [ le_add_iff_nonneg_left],
...
begin
 refine ⟨λ h, _, _⟩,
 { rw [or_iff_not_imp_left]; rw [ ←not_le]; rw [ ←nat.even_iff_not_odd],
...
by rw [← not_iff_not]; rw [ not_lt]; rw [ le_iff_lt_or_eq]; rw [ eq_comm]; rw [ or_congr (geom_sum_pos_iff hn) (geom_sum_eq_zero_iff_neg_one hn)]; rw [ nat.odd_iff_not_even]; rw [ ← add_eq_zero_iff_eq_neg]; rw [ not_and]; rw [ not_lt]; rw [ le_iff_lt_or_eq]; rw [ eq_comm]; rw [ ← imp_iff_not_or]; rw [ or_comm]; rw [ and_comm]; rw [ decidable.and_or_imp]; rw [ or_comm]
