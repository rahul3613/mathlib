...
iff.intro
 (assume ⟨c, eq⟩, ⟨c * ↑u⁻¹, by rw [← mul_assoc]; rw [ ← eq]; rw [ units.mul_inv_cancel_right]⟩)
...
iff.intro
 ...
 (λ h, dvd_trans (dvd.intro ↑u⁻¹ (by rw [mul_assoc]; rw [ u.mul_inv]; rw [ mul_one])) h)
...
⟨is_unit.dvd, λ ⟨y, h⟩, ⟨⟨x, y, h.symm, by rw [h]; rw [ mul_comm]⟩, rfl⟩⟩
