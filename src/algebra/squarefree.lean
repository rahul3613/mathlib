...
begin
 ...
 { rw [multiplicity.finite_iff_dom]; rw [ multiplicity.is_unit_right ha.not_unit hu],
...
begin
 ...
 rwa [← one_mul (z * w)] at h; rwa [ mul_assoc] at h; rwa [ mul_dvd_mul_iff_right] at h; rwa [ ← is_unit_iff_dvd_one] at h,
...
begin
 ...
 obtain ⟨w, hw⟩ := hy,
 exact (hx z ⟨w, by rwa [mul_right_comm]; rwa [ ←hw]⟩).mul_right_dvd.2 dvd_rfl,
end
...
by rw [←squarefree_nat_abs]; rw [ nat_abs_of_nat]
