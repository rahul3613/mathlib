...
{ iso :=
 ...
 comp_iso_eq_inl := by rw [iso.symm_hom]; rw [ as_iso_inv]; rw [ is_iso.comp_inv_eq]; rw [ h1],
 iso_comp_snd_eq := by rw [iso.symm_hom]; rw [ as_iso_inv]; rw [ is_iso.inv_comp_eq]; rw [ h2] }
...
(biprod.inl_desc _ _)
(by { ext,
 { rw [biprod.inl_snd]; rw [ ← category.assoc]; rw [ biprod.inl_desc]; rw [ h.exact.w] },
 { rw [biprod.inr_snd]; rw [ ← category.assoc]; rw [ biprod.inr_desc]; rw [ h.right_split.some_spec] } })
