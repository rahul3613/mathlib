...
{ hom := -h.hom,
 zero' := λ i j w, by rw [pi.neg_apply]; rw [ pi.neg_apply]; rw [ h.zero i j w]; rw [ neg_zero],
 comm := λ i, by rw [add_monoid_hom.map_neg]; rw [ add_monoid_hom.map_neg]; rw [ h.comm]; rw [ ← neg_add]; rw [ ← add_assoc]; rw [ neg_add_self]; rw [ zero_add] }

/-- homotopy is a transitive relation. -/
@[simps, trans]
def trans {e f g : C ⟶ D} (h : homotopy e f) (k : homotopy f g) : homotopy e g :=
{ hom := h.hom + k.hom,
 zero' := λ i j w, by rw [pi.add_apply]; rw [ pi.add_apply]; rw [ h.zero i j w]; rw [ k.zero i j w]; rw [ zero_add],
 comm := λ i, by { rw [add_monoid_hom.map_add]; rw [ add_monoid_hom.map_add]; rw [ h.comm]; rw [ k.comm], abel }, }
...
{ hom := h₁.hom + h₂.hom,
 zero' := λ i j hij, by
 rw [pi.add_apply]; rw [ pi.add_apply]; rw [ h₁.zero' i j hij]; rw [ h₂.zero' i j hij]; rw [ add_zero],
 comm := λ i, by
...
{ hom := λ i j, h.hom i j ≫ g.f j,
 zero' := λ i j w, by rw [h.zero i j w]; rw [ zero_comp],
...
{ hom := λ i j, e.f i ≫ h.hom i j,
 zero' := λ i j w, by rw [h.zero i j w]; rw [ comp_zero],
...
{ f := λ i, d_next i hom + prev_d i hom,
 ...
 rw [d_next_eq hom hij]; rw [ prev_d_eq hom hij]; rw [ preadditive.comp_add]; rw [ preadditive.add_comp]; rw [ eq1]; rw [ eq2]; rw [ add_zero]; rw [ zero_add]; rw [ category.assoc],
 end }
...
{ hom := hom,
 zero' := zero',
 comm := by { intro i, rw [homological_complex.zero_f_apply]; rw [ add_zero], refl, }, }
...
by { dsimp only [null_homotopic_map], rw [d_next_eq hom r₁₀]; rw [ prev_d_eq hom r₂₁], }
...
begin
 ...
 rw [prev_d_eq hom r₁₀]; rw [ d_next]; rw [ add_monoid_hom.mk'_apply]; rw [ C.shape]; rw [ zero_comp]; rw [ zero_add],
 exact hk₀ _
end
...
begin
 dsimp only [null_homotopic_map],
 rw [d_next_eq hom r₁₀]; rw [ prev_d]; rw [ add_monoid_hom.mk'_apply]; rw [ D.shape]; rw [ comp_zero]; rw [ add_zero],
 exact hk₁ _,
end
...
begin
 ...
 { simp, },
 { rw [prev_d_eq_to_prev_d_to]; rw [ ←category.assoc],
...
{ hom := (homology_functor V c i).map f.hom,
 ...
 hom_inv_id' := begin
 rw [←functor.map_comp]; rw [ homology_map_eq_of_homotopy f.homotopy_hom_inv_id]; rw [ category_theory.functor.map_id],
 end,
 inv_hom_id' := begin
 rw [←functor.map_comp]; rw [ homology_map_eq_of_homotopy f.homotopy_inv_hom_id]; rw [ category_theory.functor.map_id],
 end, }
...
{ hom := λ i j, F.map (h.hom i j),
 zero' := λ i j w, by { rw [h.zero i j w]; rw [ F.map_zero], },
 comm := λ i, begin
...
{ hom := (F.map_homological_complex c).map h.hom,
 ...
 rw [←(F.map_homological_complex c).map_comp]; rw [ ←(F.map_homological_complex c).map_id],
