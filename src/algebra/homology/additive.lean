...
{ obj := λ C,
 ...
 shape' := λ i j w, by rw [C.shape _ _ w]; rw [ F.map_zero],
 d_comp_d' := λ i j k _ _, by rw [←F.map_comp]; rw [ C.d_comp_d]; rw [ F.map_zero], },
 map := λ C D f,
 { f := λ i, F.map (f.f i),
 comm' := λ i j h, by { dsimp, rw [←F.map_comp]; rw [ ←F.map_comp]; rw [ f.comm], }, }, }.
...
lemma map_chain_complex_of (F : V ⥤ W) [F.additive] (X : α → V) (d : Π n, X (n+1) ⟶ X n)
 ...
 (λ n, F.map (d n)) (λ n, by rw [ ← F.map_comp]; rw [ sq n]; rw [ functor.map_zero]) :=
begin
