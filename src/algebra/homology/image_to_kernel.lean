...
{ hom := homology.map w₁ w₂ α.hom β.hom p,
 ...
 (by { rw [← cancel_mono (α.hom.right)]; rw [ ← comma.comp_right]; rw [ α.inv_hom_id]; rw [ comma.id_right]; rw [ p]; rw [ ← comma.comp_left]; rw [ β.inv_hom_id]; rw [ comma.id_left], refl }),
...
`[ dsimp only [auto_param_eq], erw [category.id_comp]; erw [ category.comp_id], cases pf, cases pg, refl ]
...
{ hom := homology.map w w' ⟨𝟙 _, 𝟙 _, by aux_tac⟩ ⟨𝟙 _, 𝟙 _, by aux_tac⟩ rfl,
 ...
 hom_inv_id' := begin
 cases pf, cases pg, rw [homology.map_comp]; rw [ ← homology.map_id],
