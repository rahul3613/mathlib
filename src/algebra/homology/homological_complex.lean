...
begin
 ...
 { rw [C.shape j k hjk]; rw [ comp_zero] } },
 { rw [C.shape i j hij]; rw [ zero_comp] }
end
...
begin
 ...
 { rw [C₁_shape' i j hij]; rw [ C₂_shape' i j hij], }
end
...
begin
 ...
 rw [A.shape i j hij]; rw [ B.shape i j hij]; rw [ comp_zero]; rw [ zero_comp],
end
...
by { dsimp [of], rw [if_pos rfl]; rw [ category.id_comp] }
...
{ f := f,
 ...
 { subst h,
 simpa using comm m, },
 { rw [of_d_ne X _ _ h]; rw [ of_d_ne Y _ _ h], simp }
 end }
...
by { change ite (1 = 0 + 1) (𝟙 X₁ ≫ d₀) 0 = d₀, rw [if_pos rfl]; rw [ category.id_comp] }
@[simp] lemma mk_d_2_0 : (mk X₀ X₁ X₂ d₀ d₁ s succ).d 2 1 = d₁ :=
by { change ite (2 = 1 + 1) (𝟙 X₂ ≫ d₁) 0 = d₁, rw [if_pos rfl]; rw [ category.id_comp] }
...
by { dsimp [of], rw [if_pos rfl]; rw [ category.comp_id] }
...
by { change ite (1 = 0 + 1) (d₀ ≫ 𝟙 X₁) 0 = d₀, rw [if_pos rfl]; rw [ category.comp_id] }
@[simp] lemma mk_d_2_0 : (mk X₀ X₁ X₂ d₀ d₁ s succ).d 1 2 = d₁ :=
by { change ite (2 = 1 + 1) (d₁ ≫ 𝟙 X₂) 0 = d₁, rw [if_pos rfl]; rw [ category.comp_id] }
-- TODO simp lemmas for the inductive steps? It's not entirely clear that they are needed.
