...
begin
 rw [←image_subobject_arrow_comp f]; rw [ category.assoc],
 convert comp_zero,
 rw p,
 simp,
end
...
begin
 refine ⟨λ hfg, ⟨zero_of_comp_mono h (by rw [category.assoc]; rw [ hfg.1]), _⟩, λ h, exact_comp_mono h⟩,
...
by { rw [←kernel_subobject_arrow']; rw [ exact_iso_comp], exact exact_kernel_subobject_arrow }
...
begin
 rw [←cancel_epi (image_to_kernel (0 : A ⟶ B) g h.w)]; rw [ ←cancel_epi (factor_thru_image_subobject (0 : A ⟶ B))],
 simp
end
...
begin
 rw [←kernel_subobject_arrow']; rw [ category.assoc],
 convert comp_zero,
 apply zero_of_epi_comp (image_to_kernel f g h.w) _,
 rw [image_to_kernel_arrow_assoc]; rw [ ←image_subobject_arrow]; rw [ category.assoc]; rw [ ←iso.eq_inv_comp],
 ext,
 simp,
end
...
lemma comp_eq_zero_of_exact (h : exact f g) {X Y : V} {ι : X ⟶ B} (hι : ι ≫ g = 0) {π : B ⟶ Y}
 (hπ : f ≫ π = 0) : ι ≫ π = 0 :=
by rw [←kernel.lift_ι _ _ hι]; rw [ ←cokernel.π_desc _ _ hπ]; rw [ category.assoc]; rw [ kernel_comp_cokernel_assoc _ _ h]; rw [ zero_comp]; rw [ comp_zero]
...
⟨λ h, by exactI exact_epi_zero _,
 ...
 rw [category.assoc] at e₂; rw [ is_iso.inv_hom_id] at e₂; rw [ category.comp_id] at e₂,
