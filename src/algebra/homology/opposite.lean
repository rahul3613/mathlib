...
lemma image_to_kernel_op {X Y Z : V} (f : X ⟶ Y) (g : Y ⟶ Z) (w : f ≫ g = 0) :
 image_to_kernel g.op f.op (by rw [←op_comp]; rw [ w]; rw [ op_zero]) = ((image_subobject_iso _)
 ≪≫ (image_op_op _).symm).hom ≫ (cokernel.desc f (factor_thru_image g)
 (by rw [←cancel_mono (image.ι g)]; rw [ category.assoc]; rw [ image.fac]; rw [ w]; rw [ zero_comp])).op
...
lemma image_to_kernel_unop {X Y Z : Vᵒᵖ} (f : X ⟶ Y) (g : Y ⟶ Z) (w : f ≫ g = 0) :
 image_to_kernel g.unop f.unop (by rw [←unop_comp]; rw [ w]; rw [ unop_zero]) = ((image_subobject_iso _)
 ≪≫ (image_unop_unop _).symm).hom ≫ (cokernel.desc f (factor_thru_image g)
 (by rw [←cancel_mono (image.ι g)]; rw [ category.assoc]; rw [ image.fac]; rw [ w]; rw [ zero_comp])).unop
...
def homology_op {X Y Z : V} (f : X ⟶ Y) (g : Y ⟶ Z) (w : f ≫ g = 0) :
 homology g.op f.op (by rw [←op_comp]; rw [ w]; rw [ op_zero]) ≅ opposite.op (homology f g w) :=
...
def homology_unop {X Y Z : Vᵒᵖ} (f : X ⟶ Y) (g : Y ⟶ Z) (w : f ≫ g = 0) :
 homology g.unop f.unop (by rw [←unop_comp]; rw [ w]; rw [ unop_zero]) ≅ opposite.unop (homology f g w) :=
...
{ X := λ i, op (X.X i),
 d := λ i j, (X.d j i).op,
 shape' := λ i j hij, by { rw [X.shape j i hij]; rw [ op_zero], },
 d_comp_d' := by { intros, rw [← op_comp]; rw [ X.d_comp_d]; rw [ op_zero], } }
...
{ X := λ i, unop (X.X i),
 ...
 shape' := λ i j hij, by { rw [X.shape j i hij]; rw [ unop_zero], },
 d_comp_d' := by { intros, rw [← unop_comp]; rw [ X.d_comp_d]; rw [ unop_zero], } }
...
nat_iso.of_components (λ X, (homological_complex.hom.iso_of_components (λ i, iso.refl _)
 ...
 erw [category.id_comp]; erw [ category.comp_id (f.unop.f x)],
 end
...
def homology_op_def :
 ...
 (by rw [←op_comp]; rw [ C.d_to_comp_d_from i]; rw [ op_zero]) := iso.refl _
...
def homology_unop_def (C : homological_complex Vᵒᵖ c) :
 ...
 (by rw [←unop_comp]; rw [ C.d_to_comp_d_from i]; rw [ unop_zero]) := iso.refl _
