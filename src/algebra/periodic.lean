...
λ x, by rw [← add_assoc]; rw [ h]; rw [ h.periodic.nat_mul]
...
lemma antiperiodic.int_odd_mul_antiperiodic [ring α] [has_involutive_neg β]
 (h : antiperiodic f c) (n : ℤ) :
 antiperiodic f (n * (2 * c) + c) :=
λ x, by rw [← add_assoc]; rw [ h]; rw [ h.periodic.int_mul]

lemma antiperiodic.sub_eq [add_group α] [has_involutive_neg β]
 (h : antiperiodic f c) (x : α) :
 f (x - c) = -f x :=
by rw [← neg_eq_iff_eq_neg]; rw [ ← h (x - c)]; rw [ sub_add_cancel]
...
| 0 := by rwa [nat.cast_zero]; rwa [ zero_mul]
...
| (n : ℕ) := by rwa [int.cast_coe_nat]; rwa [ h.nat_mul_eq_of_eq_zero]
| -[1+n] := by rw [int.cast_neg_succ_of_nat]; rw [ neg_mul]; rw [ ← mul_neg]; rw [ h.neg.nat_mul_eq_of_eq_zero hi]
