...
quotient.induction_on' z $ λ z, direct_sum.induction_on z
 ...
 ⟨k, f i k hik x + f j k hjk y, by rw [linear_map.map_add]; rw [ of_f]; rw [ of_f]; rw [ ihx]; rw [ ihy]; refl⟩)
...
liftq _ (direct_sum.to_module R ι P g)
 (span_le.2 $ λ a ⟨i, j, hij, x, hx⟩, by rw [← hx]; rw [ set_like.mem_coe]; rw [ linear_map.sub_mem_ker_iff]; rw [ direct_sum.to_module_lof]; rw [ direct_sum.to_module_lof]; rw [ Hg])
variables {R ι G f}
...
theorem lift_unique [nonempty ι] [is_directed ι (≤)] (F : direct_limit G f →ₗ[R] P) (x) :
 ...
 (λ i j hij x, by rw [linear_map.comp_apply]; rw [ of_f]; refl) x :=
...
begin
 ...
 rw [direct_sum.single_eq_lof R k (x k)]; rw [ direct_sum.to_module_lof]; rw [ direct_sum.to_module_lof]; rw [ totalize_of_le (hx k hk)]; rw [ totalize_of_le (le_trans (hx k hk) hij)]; rw [ directed_system.map_map],
end
...
span_induction ((quotient.mk_eq_zero _).1 H)
 ...
 split,
 { intros i0 hi0,
 rw [dfinsupp.mem_support_iff] at hi0; rw [ direct_sum.sub_apply] at hi0; rw [ ← direct_sum.single_eq_lof] at hi0; rw [ ← direct_sum.single_eq_lof] at hi0; rw [ dfinsupp.single_apply] at hi0; rw [ dfinsupp.single_apply] at hi0,
...
quotient.induction_on' z $ λ x, free_abelian_group.induction_on x
 ...
 ⟨k, f i k hik x * f j k hjk y, by rw [(of _ _ _).map_mul]; rw [ of_f]; rw [ of_f]; rw [ hs]; refl⟩))
 (λ s ⟨i, x, ih⟩, ⟨i, -x, by rw [(of _ _ _).map_neg]; rw [ ih]; refl⟩)
 (λ p q ⟨i, x, ihx⟩ ⟨j, y, ihy⟩, let ⟨k, hik, hjk⟩ := exists_ge_ge i j in
 ⟨k, f i k hik x + f j k hjk y, by rw [(of _ _ _).map_add]; rw [ of_f]; rw [ of_f]; rw [ ihx]; rw [ ihy]; refl⟩)
...
polynomial.induction_on q
 (λ z, let ⟨i, x, h⟩ := exists_of z in ⟨i, C x, by rw [map_C]; rw [ h]⟩)
 (λ q₁ q₂ ⟨i₁, p₁, ih₁⟩ ⟨i₂, p₂, ih₂⟩, let ⟨i, h1, h2⟩ := exists_ge_ge i₁ i₂ in
 ⟨i, p₁.map (f' i₁ i h1) + p₂.map (f' i₂ i h2),
 by { rw [polynomial.map_add]; rw [ map_map]; rw [ map_map]; rw [ ← ih₁]; rw [ ← ih₂],
 congr' 2; ext x; simp_rw [ring_hom.comp_apply, of_f] }⟩)
 (λ n z ih, let ⟨i, x, h⟩ := exists_of z in ⟨i, C x * X ^ (n + 1),
 by rw [polynomial.map_mul]; rw [ map_C]; rw [ h]; rw [ polynomial.map_pow]; rw [ map_X]⟩)
...
begin
 ...
 { rw [(restriction _).map_one]; rw [ (free_comm_ring.lift _).map_one]; rw [ (f' j k hjk).map_one]; rw [ (restriction _).map_one]; rw [ (free_comm_ring.lift _).map_one] },
 { rw [(restriction _).map_neg]; rw [ (restriction _).map_one]; rw [ (free_comm_ring.lift _).map_neg]; rw [ (free_comm_ring.lift _).map_one]; rw [ (f' j k hjk).map_neg]; rw [ (f' j k hjk).map_one]; rw [ (restriction _).map_neg]; rw [ (restriction _).map_one]; rw [ (free_comm_ring.lift _).map_neg]; rw [ (free_comm_ring.lift _).map_one] },
 { rintros _ ⟨p, hps, rfl⟩ n ih,
 rw [(restriction _).map_mul]; rw [ (free_comm_ring.lift _).map_mul]; rw [ (f' j k hjk).map_mul]; rw [ ih]; rw [ (restriction _).map_mul]; rw [ (free_comm_ring.lift _).map_mul]; rw [ restriction_of]; rw [ dif_pos hps]; rw [ lift_of]; rw [ restriction_of]; rw [ dif_pos (hst hps)]; rw [ lift_of],
 dsimp only,
 have := directed_system.map_map (λ i j h, f' i j h),
 dsimp only at this,
 rw this, refl },
 { rintros x y ihx ihy,
 rw [(restriction _).map_add]; rw [ (free_comm_ring.lift _).map_add]; rw [ (f' j k hjk).map_add]; rw [ ihx]; rw [ ihy]; rw [ (restriction _).map_add]; rw [ (free_comm_ring.lift _).map_add] }
end
variables {G f f'}
...
begin
 ...
 { rw [(restriction _).map_sub]; rw [ (free_comm_ring.lift _).map_sub]; rw [ restriction_of]; rw [ dif_pos]; rw [ restriction_of]; rw [ dif_pos]; rw [ lift_of]; rw [ lift_of],
 dsimp only,
 have := directed_system.map_map (λ i j h, f' i j h),
 dsimp only at this,
 ...
 { rw [(restriction _).map_sub]; rw [ (free_comm_ring.lift _).map_sub]; rw [ restriction_of]; rw [ dif_pos]; rw [ (restriction _).map_one]; rw [ lift_of]; rw [ (free_comm_ring.lift _).map_one],
 dsimp only, rw [(f' i i _).map_one]; rw [ sub_self],
 ...
 { rw [(restriction _).map_sub]; rw [ (restriction _).map_add]; rw [ restriction_of]; rw [ restriction_of]; rw [ restriction_of]; rw [ dif_pos]; rw [ dif_pos]; rw [ dif_pos]; rw [ (free_comm_ring.lift _).map_sub]; rw [ (free_comm_ring.lift _).map_add]; rw [ lift_of]; rw [ lift_of]; rw [ lift_of],
 ...
 (is_supported_of.2 $ or.inr $ or.inr rfl)), _⟩,
 { rintros k (rfl | ⟨rfl | ⟨rfl | hk⟩⟩); refl },
 { rw [(restriction _).map_sub]; rw [ (restriction _).map_mul]; rw [ restriction_of]; rw [ restriction_of]; rw [ restriction_of]; rw [ dif_pos]; rw [ dif_pos]; rw [ dif_pos]; rw [ (free_comm_ring.lift _).map_sub]; rw [ (free_comm_ring.lift _).map_mul]; rw [ lift_of]; rw [ lift_of]; rw [ lift_of],
 ...
 rw [(restriction _).map_zero]; rw [ (free_comm_ring.lift _).map_zero] },
 ...
 { rw [(restriction _).map_add]; rw [ (free_comm_ring.lift _).map_add]; rw [ ← of.zero_exact_aux2 G f' hxs hi this hik (set.subset_union_left s t)]; rw [ ← of.zero_exact_aux2 G f' hyt hj this hjk (set.subset_union_right s t)]; rw [ ihs]; rw [ (f' i k hik).map_zero]; rw [ iht]; rw [ (f' j k hjk).map_zero]; rw [ zero_add] } },
 ...
 (is_supported_upwards hyt $ set.subset_union_right ↑s t), _⟩,
 rw [(restriction _).map_mul]; rw [ (free_comm_ring.lift _).map_mul]; rw [ ← of.zero_exact_aux2 G f' hyt hj this hjk (set.subset_union_right ↑s t)]; rw [ iht]; rw [ (f' j k hjk).map_zero]; rw [ mul_zero] }
end
...
⟨j, H ⟨i, x⟩ hixs, by rw [restriction_of] at hx; rw [ dif_pos hixs] at hx; rw [ lift_of] at hx; exact hx⟩
end of_zero_exact
...
begin
 ...
 apply hf i j hij, rw [hfx]; rw [ (f' i j hij).map_zero]
end
...
ideal.quotient.lift _ (free_comm_ring.lift $ λ (x : Σ i, G i), g x.1 x.2) begin
 ...
 rw [set_like.mem_coe]; rw [ ideal.mem_comap]; rw [ mem_bot],
...
⟨ring.direct_limit.of G f i (x⁻¹), by erw [← (ring.direct_limit.of _ _ _).map_mul]; erw [ mul_inv_cancel (assume h : x = 0, H $ by rw [h, (ring.direct_limit.of _ _ _).map_zero]),
...
by rw [inv]; rw [ dif_neg hp]; rw [ classical.some_spec (direct_limit.exists_inv G f hp)]

protected theorem inv_mul_cancel {p : ring.direct_limit G f} (hp : p ≠ 0) : inv G f p * p = 1 :=
by rw [_root_.mul_comm]; rw [ direct_limit.mul_inv_cancel G f hp]
