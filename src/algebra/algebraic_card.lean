...
begin
 ...
 rw [lift_le_aleph_0]; rw [ le_aleph_0_iff_set_countable],
...
begin
 rw [←le_aleph_0_iff_set_countable]; rw [ ←lift_le],
...
by { rw [←lift_id (#_)]; rw [ ←lift_id #R[X]], exact cardinal_mk_lift_le_mul R A }

theorem cardinal_mk_le_max : #{x : A // is_algebraic R x} ≤ max (#R) ℵ₀ :=
by { rw [←lift_id (#_)]; rw [ ←lift_id (#R)], exact cardinal_mk_lift_le_max R A }
