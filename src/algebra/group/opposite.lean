...
{ nat_cast_zero := show op ((0 : ℕ) : α) = 0, by rw [nat.cast_zero]; rw [ op_zero],
...
{ int_cast := λ n, op n,
 ...
 by erw [unop_op]; erw [ int.cast_neg_succ_of_nat]; refl,
...
{ zpow := λ n x, op $ x.unop ^ n,
 ...
 by rw [unop_op]; rw [ zpow_of_nat]; rw [ zpow_of_nat]; rw [ pow_succ']; rw [ unop_mul]; rw [ unop_op],
...
by conv_rhs { rw [← op_unop a]; rw [ ← op_unop x]; rw [ ← op_unop y]; rw [ semiconj_by_op] }
