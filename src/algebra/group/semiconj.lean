...
by unfold semiconj_by; assoc_rw [h.eq]; assoc_rw [ h'.eq]
...
by unfold semiconj_by; assoc_rw [hb.eq]; assoc_rw [ ha.eq]; assoc_rw [ mul_assoc]
...
lemma one_right (a : M) : semiconj_by a 1 1 := by rw [semiconj_by]; rw [ mul_one]; rw [ one_mul]
...
calc a * ↑x⁻¹ = ↑y⁻¹ * (y * a) * ↑x⁻¹ : by rw [units.inv_mul_cancel_left]
 ... = ↑y⁻¹ * a : by rw [← h.eq]; rw [ mul_assoc]; rw [ units.mul_inv_cancel_right]
...
calc ↑a⁻¹ * y = ↑a⁻¹ * (y * a * ↑a⁻¹) : by rw [units.mul_inv_cancel_right]
 ... = x * ↑a⁻¹ : by rw [← h.eq]; rw [ ← mul_assoc]; rw [ units.inv_mul_cancel_left]
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero], exact semiconj_by.one_right _ },
...
by unfold semiconj_by; rw [mul_assoc]; rw [ inv_mul_self]; rw [ mul_one]
...
⟨λ h, mul_left_cancel (h.trans (mul_comm _ _)), λ h, by rw [h]; rw [ semiconj_by]; rw [ mul_comm] ⟩
