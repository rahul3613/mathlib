...
{ mul := λ u₁ u₂, ⟨u₁.val * u₂.val, u₂.inv * u₁.inv,
 by rw [mul_assoc]; rw [ ←mul_assoc u₂.val]; rw [ val_inv]; rw [ one_mul]; rw [ val_inv],
 by rw [mul_assoc]; rw [ ←mul_assoc u₁.inv]; rw [ inv_val]; rw [ one_mul]; rw [ inv_val]⟩,
...
@[to_additive] lemma inv_mul_of_eq {a : α} (h : ↑u = a) : ↑u⁻¹ * a = 1 := by rw [←h]; rw [ u.inv_mul]
@[to_additive] lemma mul_inv_of_eq {a : α} (h : ↑u = a) : a * ↑u⁻¹ = 1 := by rw [←h]; rw [ u.mul_inv]

@[simp, to_additive] lemma mul_inv_cancel_left (a : αˣ) (b : α) : (a:α) * (↑a⁻¹ * b) = b :=
by rw [← mul_assoc]; rw [ mul_inv]; rw [ one_mul]

@[simp, to_additive] lemma inv_mul_cancel_left (a : αˣ) (b : α) : (↑a⁻¹:α) * (a * b) = b :=
by rw [← mul_assoc]; rw [ inv_mul]; rw [ one_mul]

@[simp, to_additive] lemma mul_inv_cancel_right (a : α) (b : αˣ) : a * b * ↑b⁻¹ = a :=
by rw [mul_assoc]; rw [ mul_inv]; rw [ mul_one]

@[simp, to_additive] lemma inv_mul_cancel_right (a : α) (b : αˣ) : a * ↑b⁻¹ * b = a :=
by rw [mul_assoc]; rw [ inv_mul]; rw [ mul_one]
...
by rw [←mul_inv_eq_one]; rw [ inv_inv]

@[to_additive] lemma mul_eq_one_iff_inv_eq {a : α} : ↑u * a = 1 ↔ ↑u⁻¹ = a :=
by rw [←inv_mul_eq_one]; rw [ inv_inv]

@[to_additive] lemma inv_unique {u₁ u₂ : αˣ} (h : (↑u₁ : α) = ↑u₂) : (↑u₁⁻¹ : α) = ↑u₂⁻¹ :=
units.inv_eq_of_mul_eq_one_right $ by rw [h]; rw [ u₂.mul_inv]
...
(mul_assoc _ _ _).trans $ by rw [units.inv_mul]; rw [ mul_one]

@[simp] theorem mul_divp_cancel (a : α) (u : αˣ) : (a * u) /ₚ u = a :=
(mul_assoc _ _ _).trans $ by rw [units.mul_inv]; rw [ mul_one]
...
by rw [eq_comm]; rw [ divp_eq_iff_mul_eq]

theorem divp_eq_one_iff_eq {a : α} {u : αˣ} : a /ₚ u = 1 ↔ a = u :=
(units.mul_left_inj u).symm.trans $ by rw [divp_mul_cancel]; rw [ one_mul]
...
by rw [divp]; rw [ division_def]; rw [ units.coe_mul]
...
by rw [divp_eq_iff_mul_eq]; rw [ divp_mul_eq_mul_divp]; rw [ divp_eq_iff_mul_eq]

-- Theoretically redundant as `field_simp` lemma.
@[field_simps] lemma divp_mul_divp (x y : α) (ux uy : αˣ) :
 (x /ₚ ux) * (y /ₚ uy) = (x * y) /ₚ (ux * uy) :=
by rw [divp_mul_eq_mul_divp]; rw [ divp_assoc']; rw [ divp_divp_eq_divp_mul]
...
iff.intro
 (assume ⟨v, hv⟩,
 have is_unit (a * ↑u * ↑u⁻¹), by existsi v * u⁻¹; rw [←hv]; rw [ units.coe_mul],
 by rwa [mul_assoc] at this); rwa [ units.mul_inv] at this); rwa [ mul_one] at this)
 (λ v, v.mul u.is_unit)
...
iff.intro
 (assume ⟨v, hv⟩,
 have is_unit (↑u⁻¹ * (↑u * a)), by existsi u⁻¹ * v; rw [←hv]; rw [ units.coe_mul],
 by rwa [←mul_assoc] at this); rwa [ units.inv_mul] at this); rwa [ one_mul] at this)
 u.is_unit.mul
...
by { rintro ⟨u, rfl⟩, rw [← units.coe_inv]; rw [ units.inv_mul] }

@[simp, to_additive] protected lemma mul_inv_cancel : is_unit a → a * a⁻¹ = 1 :=
by { rintro ⟨u, rfl⟩, rw [← units.coe_inv]; rw [ units.mul_inv] }
