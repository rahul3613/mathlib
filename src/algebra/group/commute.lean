...
{ val := a,
 inv := b * ↑u⁻¹,
 val_inv := by rw [← mul_assoc]; rw [ hu]; rw [ u.mul_inv],
 inv_val := have commute a u, from hu ▸ (commute.refl _).mul_right hc,
 by rw [← this.units_inv_right.right_comm]; rw [ ← hc.eq]; rw [ hu]; rw [ u.mul_inv] }
...
by simp_rw [div_eq_mul_inv, mul_inv_rev, inv_inv, hbd.symm.eq, hcd.symm.eq, hbc.inv_inv.mul_mul_mul_comm]
