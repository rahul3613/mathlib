...
protected def semigroup [semigroup M₂] (f : M₁ → M₂) (hf : injective f)
 ...
 semigroup M₁ :=
{ mul_assoc := λ x y z, hf $ by erw [mul]; erw [ mul]; erw [ mul]; erw [ mul]; erw [ mul_assoc],
 ..‹has_mul M₁› }
...
protected def comm_semigroup [comm_semigroup M₂] (f : M₁ → M₂) (hf : injective f)
 (mul : ∀ x y, f (x * y) = f x * f y) :
 comm_semigroup M₁ :=
{ mul_comm := λ x y, hf $ by erw [mul]; erw [ mul]; erw [ mul_comm],
 .. hf.semigroup f mul }
...
{ mul := (*),
 mul_left_cancel := λ x y z H, hf $ (mul_right_inj (f x)).1 $ by erw [← mul]; erw [ ← mul]; erw [ H]; refl,
 .. hf.semigroup f mul }
...
{ mul := (*),
 mul_right_cancel := λ x y z H, hf $ (mul_left_inj (f y)).1 $ by erw [← mul]; erw [ ← mul]; erw [ H]; refl,
 .. hf.semigroup f mul }
...
protected def mul_one_class [mul_one_class M₂] (f : M₁ → M₂) (hf : injective f)
 ...
 mul_one_class M₁ :=
{ one_mul := λ x, hf $ by erw [mul]; erw [ one]; erw [ one_mul],
 mul_one := λ x, hf $ by erw [mul]; erw [ one]; erw [ mul_one],
 ..‹has_one M₁›, ..‹has_mul M₁› }
...
protected def monoid [monoid M₂] (f : M₁ → M₂) (hf : injective f)
 ...
 monoid M₁ :=
{ npow := λ n x, x ^ n,
 npow_zero' := λ x, hf $ by erw [npow]; erw [ one]; erw [ pow_zero],
 npow_succ' := λ n x, hf $ by erw [npow]; erw [ pow_succ]; erw [ mul]; erw [ npow],
...
{ nat_cast := coe,
 nat_cast_zero := hf (by erw [nat_cast]; erw [ nat.cast_zero]; erw [ zero]),
 nat_cast_succ := λ n, hf (by erw [nat_cast]; erw [ nat.cast_succ]; erw [ add]; erw [ one]; erw [ nat_cast]),
...
{ inv := has_inv.inv,
 inv_inv := λ x, hf $ by rw [inv]; rw [ inv]; rw [ inv_inv] }
...
protected def div_inv_monoid [div_inv_monoid M₂]
 ...
 div_inv_monoid M₁ :=
{ zpow := λ n x, x ^ n,
 zpow_zero' := λ x, hf $ by erw [zpow]; erw [ zpow_zero]; erw [ one],
 zpow_succ' := λ n x, hf $ by erw [zpow]; erw [ mul]; erw [ zpow_of_nat]; erw [ pow_succ]; erw [ zpow]; erw [ zpow_of_nat],
 zpow_neg' := λ n x, hf $ by erw [zpow]; erw [ zpow_neg_succ_of_nat]; erw [ inv]; erw [ zpow]; erw [ zpow_coe_nat],
 div_eq_mul_inv := λ x y, hf $ by erw [div]; erw [ mul]; erw [ inv]; erw [ div_eq_mul_inv],
...
protected def division_monoid [division_monoid M₂] (f : M₁ → M₂) (hf : injective f)
 ...
 (zpow : ∀ x (n : ℤ), f (x ^ n) = f x ^ n) :
 division_monoid M₁ :=
{ mul_inv_rev := λ x y, hf $ by erw [inv]; erw [ mul]; erw [ mul_inv_rev]; erw [ mul]; erw [ inv]; erw [ inv],
 inv_eq_of_mul := λ x y h, hf $ by erw [inv]; erw [ inv_eq_of_mul_eq_one_right (by erw [←mul, h, one])],
...
protected def group [group M₂] (f : M₁ → M₂) (hf : injective f)
 ...
 (zpow : ∀ x (n : ℤ), f (x ^ n) = f x ^ n) :
 group M₁ :=
{ mul_left_inv := λ x, hf $ by erw [mul]; erw [ inv]; erw [ mul_left_inv]; erw [ one],
...
{ int_cast := coe,
 ...
 λ n, hf (by erw [int_cast]; erw [ neg]; erw [ nat_cast]; erw [ int.cast_neg]; erw [ int.cast_coe_nat]),
...
protected def comm_semigroup [comm_semigroup M₁] (f : M₁ → M₂) (hf : surjective f)
 (mul : ∀ x y, f (x * y) = f x * f y) :
 comm_semigroup M₂ :=
{ mul_comm := hf.forall₂.2 $ λ x y, by erw [← mul]; erw [ ← mul]; erw [ mul_comm],
 .. hf.semigroup f mul }
...
protected def mul_one_class [mul_one_class M₁] (f : M₁ → M₂) (hf : surjective f)
 (one : f 1 = 1) (mul : ∀ x y, f (x * y) = f x * f y) :
 mul_one_class M₂ :=
{ one_mul := hf.forall.2 $ λ x, by erw [← one]; erw [ ← mul]; erw [ one_mul],
 mul_one := hf.forall.2 $ λ x, by erw [← one]; erw [ ← mul]; erw [ mul_one],
 ..‹has_one M₂›, ..‹has_mul M₂› }
...
protected def monoid [monoid M₁] (f : M₁ → M₂) (hf : surjective f)
 ...
 monoid M₂ :=
{ npow := λ n x, x ^ n,
 npow_zero' := hf.forall.2 $ λ x, by erw [←npow]; erw [ pow_zero]; erw [ ←one],
 npow_succ' := λ n, hf.forall.2 $ λ x, by erw [←npow]; erw [ pow_succ]; erw [ ←npow]; erw [ ←mul],
 .. hf.semigroup f mul, .. hf.mul_one_class f one mul }
...
protected def add_monoid_with_one
 ...
 add_monoid_with_one M₂ :=
{ nat_cast := coe,
 nat_cast_zero := by { rw [← nat_cast]; rw [ nat.cast_zero]; rw [ zero], refl },
 nat_cast_succ := λ n, by { rw [← nat_cast]; rw [ nat.cast_succ]; rw [ add]; rw [ one]; rw [ nat_cast], refl },
 one := 1, .. hf.add_monoid f zero add nsmul }
...
protected def has_involutive_inv {M₂ : Type*} [has_inv M₂] [has_involutive_inv M₁]
 ...
 has_involutive_inv M₂ :=
{ inv := has_inv.inv,
 inv_inv := hf.forall.2 $ λ x, by erw [←inv]; erw [ ←inv]; erw [ inv_inv] }
...
protected def div_inv_monoid [div_inv_monoid M₁]
 ...
 div_inv_monoid M₂ :=
{ zpow := λ n x, x ^ n,
 zpow_zero' := hf.forall.2 $ λ x, by erw [←zpow]; erw [ zpow_zero]; erw [ ←one],
 zpow_succ' := λ n, hf.forall.2 $ λ x, by
 erw [←zpow]; erw [ ←zpow]; erw [ zpow_of_nat]; erw [ zpow_of_nat]; erw [ pow_succ]; erw [ ←mul],
 zpow_neg' := λ n, hf.forall.2 $ λ x, by
 erw [←zpow]; erw [ ←zpow]; erw [ zpow_neg_succ_of_nat]; erw [ zpow_coe_nat]; erw [ inv],
 div_eq_mul_inv := hf.forall₂.2 $ λ x y, by erw [← inv]; erw [ ← mul]; erw [ ← div]; erw [ div_eq_mul_inv],
...
protected def group [group M₁] (f : M₁ → M₂) (hf : surjective f)
 ...
 (zpow : ∀ x (n : ℤ), f (x ^ n) = f x ^ n) :
 group M₂ :=
{ mul_left_inv := hf.forall.2 $ λ x, by erw [← inv]; erw [ ← mul]; erw [ mul_left_inv]; erw [ one]; refl,
 .. hf.div_inv_monoid f one mul inv div npow zpow }
...
protected def add_group_with_one
 ...
 add_group_with_one M₂ :=
{ int_cast := coe,
 int_cast_of_nat := λ n, by rw [← int_cast]; rw [ int.cast_coe_nat]; rw [ nat_cast],
 int_cast_neg_succ_of_nat := λ n,
 by { rw [← int_cast]; rw [ int.cast_neg]; rw [ int.cast_coe_nat]; rw [ neg]; rw [ nat_cast], refl },
 .. hf.add_monoid_with_one f zero one add nsmul nat_cast,
 .. hf.add_group f zero add neg sub nsmul zsmul }
