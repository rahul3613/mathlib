...
@[to_additive map_nsmul.aux, simp] theorem map_pow [monoid G] [monoid H] [monoid_hom_class F G H]
 (f : F) (a : G) :
 ∀ (n : ℕ), f (a ^ n) = (f a) ^ n
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ map_one]
| (n+1) := by rw [pow_succ]; rw [ pow_succ]; rw [ map_mul]; rw [ map_pow]
...
theorem map_zpow' [div_inv_monoid G] [div_inv_monoid H] [monoid_hom_class F G H]
 ...
 ∀ n : ℤ, f (a ^ n) = (f a) ^ n
| (n : ℕ) := by rw [zpow_coe_nat]; rw [ map_pow]; rw [ zpow_coe_nat]
| -[1+n] := by rw [zpow_neg_succ_of_nat]; rw [ hf]; rw [ map_pow]; rw [ ← zpow_neg_succ_of_nat]
...
⟨λ h, one_hom.ext $ λ x, hg $ by rw [← one_hom.comp_apply]; rw [ h]; rw [ one_hom.comp_apply],
 λ h, h ▸ rfl⟩
@[to_additive]
lemma mul_hom.cancel_left [has_mul M] [has_mul N] [has_mul P]
 {g : N →ₙ* P} {f₁ f₂ : M →ₙ* N} (hg : function.injective g) :
 g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
⟨λ h, mul_hom.ext $ λ x, hg $ by rw [← mul_hom.comp_apply]; rw [ h]; rw [ mul_hom.comp_apply],
 λ h, h ▸ rfl⟩
@[to_additive]
lemma monoid_hom.cancel_left [mul_one_class M] [mul_one_class N] [mul_one_class P]
 {g : N →* P} {f₁ f₂ : M →* N} (hg : function.injective g) :
 g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
⟨λ h, monoid_hom.ext $ λ x, hg $ by rw [← monoid_hom.comp_apply]; rw [ h]; rw [ monoid_hom.comp_apply],
 λ h, h ▸ rfl⟩
lemma monoid_with_zero_hom.cancel_left [mul_zero_one_class M] [mul_zero_one_class N]
 ...
 g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
⟨λ h, monoid_with_zero_hom.ext $ λ x, hg $ by rw [ ← monoid_with_zero_hom.comp_apply]; rw [ h]; rw [ monoid_with_zero_hom.comp_apply],
 λ h, h ▸ rfl⟩
...
⟨λ f g,
 ...
 rw [f.map_mul]; rw [ g.map_mul]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_right_comm (f x)], end }⟩
...
⟨λ h x, (map_eq_one_iff f h).mp,
 λ h x y hxy, mul_inv_eq_one.1 $ h _ $ by rw [map_mul]; rw [ hxy]; rw [ ← map_mul]; rw [ mul_inv_self]; rw [ map_one]⟩
...
{ to_fun := f,
 map_mul' := map_mul,
 map_one' := mul_left_eq_self.1 $ by rw [←map_mul]; rw [ mul_one] }
...
⟨λ f, mk' (λ g, (f g)⁻¹) $ λ a b, by rw [←mul_inv]; rw [ f.map_mul]⟩
...
⟨λ f g,
 { to_fun := λ a, f a * g a,
 map_zero' := by rw [map_zero]; rw [ zero_mul],
 ..(f * g : M →* N) }⟩
