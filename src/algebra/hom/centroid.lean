...
⟨λ n f,
 { map_mul_left' := λ a b,
 by { change n • f (a * b) = a * n • f b, rw [map_mul_left f]; rw [ ←mul_smul_comm] },
 map_mul_right' := λ a b,
 by { change n • f (a * b) = n • f a * b, rw [map_mul_right f]; rw [ ←smul_mul_assoc] },
 .. (n • f : α →+ α) }⟩
...
by rw [comp_app]; rw [ map_mul_right]; rw [ map_mul_left]; rw [ ←map_mul_right]; rw [ ←map_mul_left]
