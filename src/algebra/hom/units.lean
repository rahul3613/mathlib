...
monoid_hom.mk'
 (λ u, ⟨f u.val, f u.inv,
 by rw [← f.map_mul]; rw [ u.val_inv]; rw [ f.map_one],
 by rw [← f.map_mul]; rw [ u.inv_val]; rw [ f.map_one]⟩)
...
by rw [div_eq_mul_inv]; rw [ divp]; rw [ u.coe_inv]
...
by rw [units.mul_inv_eq_iff_eq_mul]; rw [ one_mul]; rw [ coe_lift_right]

@[simp, to_additive] lemma lift_right_inv_mul {f : M →* N} {g : M → Nˣ}
 (h : ∀ x, ↑(g x) = f x) (x) : ↑(lift_right f g h x)⁻¹ * f x = 1 :=
by rw [units.inv_mul_eq_iff_eq_mul]; rw [ mul_one]; rw [ coe_lift_right]
...
by rw [div_eq_mul_inv]; rw [ h.mul_inv_cancel]
...
by rw [div_eq_mul_inv]; rw [ h.inv_mul_cancel_right]

@[simp, to_additive] protected lemma mul_div_cancel (h : is_unit b) (a : α) : a * b / b = a :=
by rw [div_eq_mul_inv]; rw [ h.mul_inv_cancel_right]
...
by rw [div_eq_mul_inv]; rw [ h.mul_inv_eq_iff_eq_mul]

@[to_additive] protected lemma eq_div_iff (h : is_unit c) : a = b / c ↔ a * c = b :=
by rw [div_eq_mul_inv]; rw [ h.eq_mul_inv_iff_mul_eq]
...
by rw [div_eq_mul_inv]; rw [ mul_inv_rev]; rw [ h.mul_inv_cancel_left]; rw [ one_div]
...
by rw [mul_comm]; rw [ h.div_mul_left]

@[to_additive] protected lemma mul_div_cancel_left (h : is_unit a) (b : α) : a * b / a = b :=
by rw [mul_comm]; rw [ h.mul_div_cancel]

@[to_additive] protected lemma mul_div_cancel' (h : is_unit a) (b : α) : a * (b / a) = b :=
by rw [mul_comm]; rw [ h.div_mul_cancel]

@[to_additive] protected lemma mul_div_mul_left (h : is_unit c) (a b : α) :
 (c * a) / (c * b) = a / b :=
by rw [mul_comm c]; rw [ mul_comm c]; rw [ h.mul_div_mul_right]

@[to_additive] protected lemma mul_eq_mul_of_div_eq_div (hb : is_unit b) (hd : is_unit d) (a c : α)
 (h : a / b = c / d) : a * d = c * b :=
by rw [←mul_one a]; rw [ ←hb.div_self]; rw [ ←mul_comm_div]; rw [ h]; rw [ div_mul_eq_mul_div]; rw [ hd.div_mul_cancel]

@[to_additive] protected lemma div_eq_div_iff (hb : is_unit b) (hd : is_unit d) :
 a / b = c / d ↔ a * d = c * b :=
by rw [←(hb.mul hd).mul_left_inj]; rw [ ←mul_assoc]; rw [ hb.div_mul_cancel]; rw [ ←mul_assoc]; rw [ mul_right_comm]; rw [ hd.div_mul_cancel]

@[to_additive] protected lemma div_div_cancel (h : is_unit a) : a / (a / b) = b :=
by rw [div_div_eq_mul_div]; rw [ h.mul_div_cancel_left]

@[to_additive] protected lemma div_div_cancel_left (h : is_unit a) :
 a / b / a = b⁻¹ :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ mul_right_comm]; rw [ h.mul_inv_cancel]; rw [ one_mul]
