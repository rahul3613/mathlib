...
{ to_fun := λ x, x, map_prod_eq_map_prod' := λ s t _ _ _ _ h, by rw [map_id']; rw [ map_id']; rw [ h] }
...
{ to_fun := f ∘ g,
 ...
 rw [←map_map]; rw [ map_prod_eq_map_prod f _ _ ((s.card_map _).trans hs) ((t.card_map _).trans ht) (map_prod_eq_map_prod g hsA htA hs ht h)]; rw [ map_map],
...
{ to_fun := λ _, b,
 ...
 by rw [multiset.map_const]; rw [ multiset.map_const]; rw [ prod_replicate]; rw [ prod_replicate]; rw [ hs]; rw [ ht] }
...
⟨λ f g, { to_fun := λ x, f x * g x,
 ...
 by rw [prod_map_mul]; rw [ prod_map_mul]; rw [ map_prod_eq_map_prod f hsA htA hs ht h]; rw [ map_prod_eq_map_prod g hsA htA hs ht h] }⟩
...
⟨λ f, { to_fun := λ x, (f x)⁻¹,
 map_prod_eq_map_prod' := λ s t hsA htA hs ht h,
 by rw [prod_map_inv]; rw [ prod_map_inv]; rw [ map_prod_eq_map_prod f hsA htA hs ht h] }⟩
...
⟨λ f g, { to_fun := λ x, f x / g x,
 map_prod_eq_map_prod' := λ s t hsA htA hs ht h,
 by rw [prod_map_div]; rw [ prod_map_div]; rw [ map_prod_eq_map_prod f hsA htA hs ht h]; rw [ map_prod_eq_map_prod g hsA htA hs ht h] }⟩
...
{ mul := (*),
 ...
 { to_fun := λ x, f x ^ m,
 map_prod_eq_map_prod' := λ s t hsA htA hs ht h,
 by rw [prod_map_pow]; rw [ prod_map_pow]; rw [ map_prod_eq_map_prod f hsA htA hs ht h] },
...
{ inv := has_inv.inv,
 ...
 map_prod_eq_map_prod' := λ s t hsA htA hs ht h,
 by rw [prod_map_zpow]; rw [ prod_map_zpow]; rw [ map_prod_eq_map_prod f hsA htA hs ht h] },
...
{ map_prod_eq_map_prod' := λ f s t _ _ _ _ h, by rw [←f.map_multiset_prod]; rw [ h]; rw [ f.map_multiset_prod] }
...
begin
 ...
 rw [hs]; rw [ ht] },
 rw [←hs] at hm; rw [ card_pos_iff_exists_mem] at hm,
 obtain ⟨a, ha⟩ := hm,
 ...
 rwa eq_of_mem_replicate h },
 { rw [card_add]; rw [ hs]; rw [ card_replicate]; rw [ add_tsub_cancel_of_le h] },
 { rw [card_add]; rw [ ht]; rw [ card_replicate]; rw [ add_tsub_cancel_of_le h] },
 { rw [prod_add]; rw [ prod_add]; rw [ hst] }
end
