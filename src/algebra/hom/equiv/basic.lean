...
{ to_fun := g,
 map_mul' := λ x y,
 calc g (x * y) = g (f (g x) * f (g y)) : by rw [h₂ x]; rw [ h₂ y]
...
{ coe := (coe : F → M → N),
 ...
 ... = e (inv e (1 : N)) : by rw [← map_mul]; rw [ one_mul]
 ... = 1 : right_inv e 1,
...
{ map_zero := λ e, calc e 0 = e 0 * e (equiv_like.inv e 0) : by rw [←map_mul]; rw [ zero_mul]
...
{ map_mul' := λ x y, show h2 (h1 (x * y)) = h2 (h1 x) * h2 (h1 y),
 by rw [h1.map_mul]; rw [ h2.map_mul],
