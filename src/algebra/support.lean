...
by rw [disjoint.comm]; rw [ mul_support_disjoint_iff]
...
by rw [nonempty_iff_ne_empty]; rw [ ne.def]; rw [ mul_support_eq_empty_iff]
...
λ x hbf hf, hbf $ by rw [pi.smul_apply]; rw [ hf]; rw [ smul_zero]

lemma support_smul_subset_left [has_zero M] [has_zero β] [smul_with_zero M β]
 (f : α → M) (g : α → β) :
 support (f • g) ⊆ support f :=
λ x hfg hf, hfg $ by rw [pi.smul_apply']; rw [ hf]; rw [ zero_smul]
...
lemma mul_support_one_sub' [has_one R] [add_group R] (f : α → R) :
 mul_support (1 - f) = support f :=
by rw [sub_eq_add_neg]; rw [ mul_support_one_add']; rw [ support_neg']
...
by rw [mul_support_comp_eq_preimage f g]; rw [ image_inter_preimage]
...
mul_support_mul_single_subset.antisymm $
 λ x (hx : x = a), by rwa [mem_mul_support]; rwa [ hx]; rwa [ mul_single_eq_same]
...
by rw [mul_support_mul_single_of_ne hb]; rw [ mul_support_mul_single_of_ne hb']; rw [ disjoint_singleton]
