...
{ le_total := λ a b, le_total (untrop a) (untrop b),
 ...
 max_def := begin
 ext x y,
 rw [max_default]; rw [ max_def]; rw [ apply_ite trop]; rw [ trop_untrop]; rw [ trop_untrop]; rw [ if_congr untrop_le_iff rfl rfl],
 end,
 min := (+),
 min_def := begin
 ext x y,
 rw [trop_add_def]; rw [ min_default]; rw [ min_def]; rw [ apply_ite trop]; rw [ trop_untrop]; rw [ trop_untrop]; rw [ if_congr untrop_le_iff rfl rfl],
 end,
...
by rw [trop_add_def]; rw [ trop_eq_iff_eq_untrop]; rw [ ←untrop_le_iff]; rw [ min_eq_left_iff]

lemma add_eq_right_iff {x y : tropical R} : x + y = y ↔ y ≤ x :=
by rw [trop_add_def]; rw [ trop_eq_iff_eq_untrop]; rw [ ←untrop_le_iff]; rw [ min_eq_right_iff]
...
by { rw [trop_add_def]; rw [ trop_eq_iff_eq_untrop], simp [min_eq_iff] }
...
⟨λ x y z h, begin
 ...
 { rw [add_eq_left hx]; rw [ add_eq_left (hx.trans h)] },
 { rw [add_eq_right hy],
...
begin
 ...
 { rw [add_eq_left h]; rw [ add_eq_left (pow_le_pow_of_le_left' h _)] },
 { rw [add_eq_right h]; rw [ add_eq_right (pow_le_pow_of_le_left' h _)] }
end
...
begin
 ...
 { simp },
 { rw [add_nsmul]; rw [ IH]; rw [ one_nsmul]; rw [ add_self] }
end
