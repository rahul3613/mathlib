...
begin
 ...
 rw [←inf'_eq_cInf_image _ h]; rw [ inf'_eq_inf]; rw [ s.trop_inf],
end

lemma trop_infi [conditionally_complete_linear_order R] [fintype S] (f : S → with_top R) :
 trop (⨅ (i : S), f i) = ∑ (i : S), trop (f i) :=
by rw [infi]; rw [ ←set.image_univ]; rw [ ←coe_univ]; rw [ trop_Inf_image]
...
begin
 ...
 rw [←inf'_eq_cInf_image _ h]; rw [ inf'_eq_inf]; rw [ finset.untrop_sum'],
end

lemma untrop_sum [conditionally_complete_linear_order R] [fintype S]
 (f : S → tropical (with_top R)) :
 untrop (∑ i : S, f i) = ⨅ i : S, untrop (f i) :=
by rw [infi]; rw [ ←set.image_univ]; rw [ ←coe_univ]; rw [ untrop_sum_eq_Inf_image]
