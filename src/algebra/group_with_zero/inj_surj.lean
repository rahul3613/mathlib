...
{ eq_zero_or_eq_zero_of_mul_eq_zero := λ x y H,
 have f x * f y = 0, by rw [← mul]; rw [ H]; rw [ zero],
...
protected def function.injective.group_with_zero [has_zero G₀'] [has_mul G₀'] [has_one G₀']
 ...
 group_with_zero G₀' :=
{ inv_zero := hf $ by erw [inv]; erw [ zero]; erw [ inv_zero],
 mul_inv_cancel := λ x hx, hf $ by erw [one]; erw [ mul]; erw [ inv]; erw [ mul_inv_cancel ((hf.ne_iff' zero).2 hx)],
...
protected def function.surjective.group_with_zero [has_zero G₀'] [has_mul G₀'] [has_one G₀']
 ...
 group_with_zero G₀' :=
{ inv_zero := by erw [← zero]; erw [ ← inv]; erw [ inv_zero],
 mul_inv_cancel := hf.forall.2 $ λ x hx,
 by erw [← inv]; erw [ ← mul]; erw [ mul_inv_cancel (mt (congr_arg f) $ trans_rel_left ne hx zero.symm)];
 exact one,
