...
have h2 : a ^ (m - n) * a ^ n = a ^ m, by rw [←pow_add]; rw [ h1],
...
begin
 ...
 { rw [zero_pow (tsub_pos_of_lt h)]; rw [ zero_pow (n.zero_le.trans_lt h)]; rw [ zero_mul] },
...
by rw [pow_sub₀ _ (inv_ne_zero ha) h]; rw [ inv_pow]; rw [ inv_pow]; rw [ inv_inv]

lemma inv_pow_sub_of_lt (a : G₀) (h : n < m) : a⁻¹ ^ (m - n) = (a ^ m)⁻¹ * a ^ n :=
by rw [pow_sub_of_lt a⁻¹ h]; rw [ inv_pow]; rw [ inv_pow]; rw [ inv_inv]
...
| (n : ℕ) h := by { rw [zpow_coe_nat]; rw [ zero_pow'], simpa using h }
| -[1+n] h := by simp
...
| -[1+0] := by erw [zpow_zero]; erw [ zpow_neg_succ_of_nat]; erw [ pow_one]; erw [ inv_mul_cancel ha]
| -[1+(n+1)] := by rw [int.neg_succ_of_nat_eq]; rw [ zpow_neg]; rw [ neg_add]; rw [ neg_add_cancel_right]; rw [ zpow_neg]; rw [ ← int.coe_nat_succ]; rw [ zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ pow_succ _ (n + 1)]; rw [ mul_inv_rev]; rw [ mul_assoc]; rw [ inv_mul_cancel ha]; rw [ mul_one]

lemma zpow_sub_one₀ {a : G₀} (ha : a ≠ 0) (n : ℤ) : a ^ (n - 1) = a ^ n * a⁻¹ :=
calc a ^ (n - 1) = a ^ (n - 1) * a * a⁻¹ : by rw [mul_assoc]; rw [ mul_inv_cancel ha]; rw [ mul_one]
 ... = a^n * a⁻¹ : by rw [← zpow_add_one₀ ha]; rw [ sub_add_cancel]
...
by rw [sub_eq_add_neg]; rw [ zpow_add₀ ha]; rw [ zpow_neg]; rw [ div_eq_mul_inv]

theorem zpow_bit1' (a : G₀) (n : ℤ) : a ^ bit1 n = (a * a) ^ n * a :=
by rw [zpow_bit1₀]; rw [ (commute.refl a).mul_zpow]
