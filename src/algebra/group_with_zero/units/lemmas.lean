...
by rw [div_div_eq_mul_div]; rw [ div_mul_cancel _ hc]

lemma div_mul_div_cancel (a : G₀) (hc : c ≠ 0) : (a / c) * (c / b) = a / b :=
by rw [← mul_div_assoc]; rw [ div_mul_cancel _ hc]
...
by rw [mul_comm]; rw [ mul_div_cancel_of_imp h]
...
by rw [mul_comm]; rw [ div_mul_cancel_of_imp h]
...
lemma mul_eq_mul_of_div_eq_div (a : G₀) {b : G₀} (c : G₀) {d : G₀} (hb : b ≠ 0) (hd : d ≠ 0)
 (h : a / b = c / d) : a * d = c * b :=
by rw [←mul_one a]; rw [ ←div_self hb]; rw [ ←mul_comm_div]; rw [ h]; rw [ div_mul_eq_mul_div]; rw [ div_mul_cancel _ hd]
...
by rw [div_mul_eq_mul_div]; rw [ one_mul]; rw [ div_mul_right _ h]
...
begin
 ...
 { rw [inv_zero]; rw [ map_zero]; rw [ map_zero] },
