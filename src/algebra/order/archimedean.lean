...
⟨λ x y hy, let ⟨n, hn⟩ := archimedean.arch (-x : α) (neg_pos.2 hy) in
 ⟨n, by rwa [neg_nsmul] at hn⟩⟩; rwa [ neg_le_neg_iff] at hn⟩⟩
...
by classical; exact let n := nat.find h in
 ...
 by rw [hn0] at hn; rw [ pow_zero] at hn; exact (not_le_of_gt hn hx)),
...
let ⟨N, hN⟩ := pow_unbounded_of_one_lt x⁻¹ hy in
 ...
 ⟨-N, le_of_lt (by { rw [zpow_neg y (↑N)]; rw [ zpow_coe_nat],
...
⟨-(m+1),
by rwa [zpow_neg]; rwa [ inv_lt (zpow_pos_of_pos hyp _) hx],
by rwa [neg_add]; rwa [ neg_add_cancel_right]; rwa [ zpow_neg]; rwa [ le_inv hx (zpow_pos_of_pos hyp _)]⟩
...
begin
 ...
 exact ⟨q, by rwa [inv_pow] at hq⟩; rwa [ inv_lt_inv hx (pow_pos y_pos _)] at hq⟩
end
...
begin
 ...
 with ⟨n, hn, h'n⟩,
 refine ⟨n, _, _⟩,
 { rwa [inv_pow] at h'n ; rwa [ inv_lt_inv xpos (pow_pos ypos _)] at h'n },
 { rwa [inv_pow] at hn ; rwa [ inv_le_inv (pow_pos ypos _) xpos] at hn }
end
...
begin
 ...
 rwa [← lt_sub_iff_add_lt']; rwa [ ← sub_mul]; rwa [ ← div_lt_iff' (sub_pos.2 h)]; rwa [ one_div],
 { rw [rat.coe_int_denom]; rw [ nat.cast_one], exact one_ne_zero },
 { intro H, rw [rat.coe_nat_num] at H; rw [ int.cast_coe_nat] at H; rw [ nat.cast_eq_zero] at H, subst H, cases n0 },
 { rw [rat.coe_nat_denom]; rw [ nat.cast_one], exact one_ne_zero }
end
...
⟨@exists_nat_gt α _, λ H, ⟨λ x y y0,
 ...
 by rwa [div_lt_iff y0] at h⟩⟩; rwa [ ← nsmul_eq_mul] at h⟩⟩
