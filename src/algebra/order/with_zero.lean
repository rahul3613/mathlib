...
if ha : a = 0 then by { rw [ha]; rw [ zero_mul]; rw [ zero_lt_iff], exact mul_ne_zero hb hd } else
if hc : c = 0 then by { rw [hc]; rw [ mul_zero]; rw [ zero_lt_iff], exact mul_ne_zero hb hd } else
...
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ mul_le_mul_right₀ (inv_ne_zero hc)]
...
by rw [div_eq_mul_inv]; rw [ le_mul_inv_iff₀ hc]

lemma div_le_iff₀ (hc : c ≠ 0) : a / c ≤ b ↔ a ≤ b*c :=
by rw [div_eq_mul_inv]; rw [ mul_inv_le_iff₀ hc]
