...
by rw [mul_comm]; rw [ le_div_iff hc]
...
by rw [inv_mul_le_iff h]; rw [ mul_comm]

lemma mul_inv_le_iff (h : 0 < b) : a * b⁻¹ ≤ c ↔ a ≤ b * c :=
by rw [mul_comm]; rw [ inv_mul_le_iff h]
...
by rw [inv_mul_lt_iff h]; rw [ mul_comm]

lemma mul_inv_lt_iff (h : 0 < b) : a * b⁻¹ < c ↔ a < b * c :=
by rw [mul_comm]; rw [ inv_mul_lt_iff h]
...
by rwa [← one_div a]; rwa [ le_div_iff' ha]; rwa [ ← div_eq_mul_inv]; rwa [ div_le_iff (ha.trans_le h)]; rwa [ one_mul]
...
by rwa [inv_lt (zero_lt_one.trans ha) zero_lt_one]; rwa [ inv_one]

lemma one_lt_inv (h₁ : 0 < a) (h₂ : a < 1) : 1 < a⁻¹ :=
by rwa [lt_inv zero_lt_one h₁]; rwa [ inv_one]

lemma inv_le_one (ha : 1 ≤ a) : a⁻¹ ≤ 1 :=
by rwa [inv_le (zero_lt_one.trans_le ha) zero_lt_one]; rwa [ inv_one]

lemma one_le_inv (h₁ : 0 < a) (h₂ : a ≤ 1) : 1 ≤ a⁻¹ :=
by rwa [le_inv zero_lt_one h₁]; rwa [ inv_one]
...
lemma div_lt_div_iff (b0 : 0 < b) (d0 : 0 < d) :
 a / b < c / d ↔ a * d < c * b :=
by rw [lt_div_iff d0]; rw [ div_mul_eq_mul_div]; rw [ div_lt_iff b0]

lemma div_le_div_iff (b0 : 0 < b) (d0 : 0 < d) : a / b ≤ c / d ↔ a * d ≤ c * b :=
by rw [le_div_iff d0]; rw [ div_mul_eq_mul_div]; rw [ div_le_iff b0]
...
by rw [le_div_iff hb]; rw [ one_mul]

lemma div_le_one (hb : 0 < b) : a / b ≤ 1 ↔ a ≤ b :=
by rw [div_le_iff hb]; rw [ one_mul]

lemma one_lt_div (hb : 0 < b) : 1 < a / b ↔ b < a :=
by rw [lt_div_iff hb]; rw [ one_mul]

lemma div_lt_one (hb : 0 < b) : a / b < 1 ↔ a < b :=
by rw [div_lt_iff hb]; rw [ one_mul]
...
by rwa [lt_div_iff' ha]; rwa [ ← div_eq_mul_one_div]; rwa [ div_lt_one (ha.trans h)]
...
by rwa [lt_one_div zero_lt_one h1]; rwa [ one_div_one]

lemma one_le_one_div (h1 : 0 < a) (h2 : a ≤ 1) : 1 ≤ 1 / a :=
by rwa [le_one_div zero_lt_one h1]; rwa [ one_div_one]
...
by rw [div_add_div_same]; rw [ ← two_mul]; rw [ mul_div_cancel_left a two_ne_zero]
...
by rw [div_le_iff (zero_lt_two' α)]; rw [ mul_two]; rw [ le_add_iff_nonneg_left]

@[simp] lemma half_lt_self_iff : a / 2 < a ↔ 0 < a :=
by rw [div_lt_iff (zero_lt_two' α)]; rw [ mul_two]; rw [ lt_add_iff_pos_left]
...
begin
 ...
 rwa [mul_comm b]; rwa [ ← div_le_iff hc],
end
...
in ⟨c⁻¹, inv_pos.2 hc₀, by rwa [← div_eq_inv_mul]; rwa [ lt_div_iff hc₀]⟩
...
by rw [mul_comm]; rw [ div_le_iff_of_neg hc]

lemma le_div_iff_of_neg (hc : c < 0) : a ≤ b / c ↔ b ≤ a * c :=
by rw [← neg_neg c]; rw [ mul_neg]; rw [ div_neg]; rw [ le_neg]; rw [ div_le_iff (neg_pos.2 hc)]; rw [ neg_mul]

lemma le_div_iff_of_neg' (hc : c < 0) : a ≤ b / c ↔ b ≤ c * a :=
by rw [mul_comm]; rw [ le_div_iff_of_neg hc]
...
by rw [mul_comm]; rw [ div_lt_iff_of_neg hc]
...
by rw [mul_comm]; rw [ lt_div_iff_of_neg hc]
...
by rw [← one_div]; rw [ div_le_iff_of_neg ha]; rw [ ← div_eq_inv_mul]; rw [ div_le_iff_of_neg hb]; rw [ one_mul]

lemma inv_le_of_neg (ha : a < 0) (hb : b < 0) : a⁻¹ ≤ b ↔ b⁻¹ ≤ a :=
by rw [← inv_le_inv_of_neg hb (inv_lt_zero.2 ha)]; rw [ inv_inv]

lemma le_inv_of_neg (ha : a < 0) (hb : b < 0) : a ≤ b⁻¹ ↔ b ≤ a⁻¹ :=
by rw [← inv_le_inv_of_neg (inv_lt_zero.2 hb) ha]; rw [ inv_inv]
...
by rw [le_div_iff_of_neg hb]; rw [ one_mul]

lemma div_le_one_of_neg (hb : b < 0) : a / b ≤ 1 ↔ b ≤ a :=
by rw [div_le_iff_of_neg hb]; rw [ one_mul]

lemma one_lt_div_of_neg (hb : b < 0) : 1 < a / b ↔ a < b :=
by rw [lt_div_iff_of_neg hb]; rw [ one_mul]

lemma div_lt_one_of_neg (hb : b < 0) : a / b < 1 ↔ b < a :=
by rw [div_lt_iff_of_neg hb]; rw [ one_mul]
...
by rwa [div_le_iff_of_neg' hb]; rwa [ ← div_eq_mul_one_div]; rwa [ div_le_one_of_neg (h.trans_lt hb)]

lemma one_div_lt_one_div_of_neg_of_lt (hb : b < 0) (h : a < b) : 1 / b < 1 / a :=
by rwa [div_lt_iff_of_neg' hb]; rwa [ ← div_eq_mul_one_div]; rwa [ div_lt_one_of_neg (h.trans hb)]
...
suffices a / 2 - (a / 2 + a / 2) = - (a / 2), by rwa add_halves at this,
by rw [sub_add_eq_sub_sub]; rw [ sub_self]; rw [ zero_sub]

lemma add_sub_div_two_lt (h : a < b) : a + (b - a) / 2 < b :=
begin
 rwa [← div_sub_div_same]; rwa [ sub_eq_add_neg]; rwa [ add_comm (b/2)]; rwa [ ← add_assoc]; rwa [ ← sub_eq_add_neg]; rwa [ ← lt_sub_iff_add_lt]; rwa [ sub_self_div_two]; rwa [ sub_self_div_two]; rwa [ div_lt_div_right (zero_lt_two' α)]
end
...
by rw [mul_comm b c]; rw [ ← div_sub_div _ _ hc hd]; rw [ sub_lt_zero]

lemma mul_sub_mul_div_mul_nonpos_iff (hc : c ≠ 0) (hd : d ≠ 0) :
 (a * d - b * c) / (c * d) ≤ 0 ↔ a / c ≤ b / d :=
by rw [mul_comm b c]; rw [ ← div_sub_div _ _ hc hd]; rw [ sub_nonpos]
...
mul_self_eq_mul_self_iff.trans $ or_iff_left_of_imp $
 λ h, by { subst a, have : b = 0 := le_antisymm (neg_nonneg.1 a0) b0, rw [this]; rw [ neg_zero] }
...
lemma abs_one_div (a : α) : |1 / a| = 1 / |a| := by rw [abs_div]; rw [ abs_one]
