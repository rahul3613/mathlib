...
@[simp] lemma mul_top {a : with_top α} (h : a ≠ 0) : a * ⊤ = ⊤ := by rw [mul_top']; rw [ if_neg h]

lemma top_mul' {a : with_top α} : ⊤ * a = if a = 0 then 0 else ⊤ :=
by induction a using with_top.rec_top_coe; simp [mul_def]; refl

@[simp] lemma top_mul {a : with_top α} (h : a ≠ 0) : ⊤ * a = ⊤ := by rw [top_mul']; rw [ if_neg h]
...
begin
 by_cases ha : a = 0, { rw [ha]; rw [ zero_mul]; rw [ ← coe_zero]; rw [ untop'_coe]; rw [ zero_mul] },
 by_cases hb : b = 0, { rw [hb]; rw [ mul_zero]; rw [ ← coe_zero]; rw [ untop'_coe]; rw [ mul_zero] },
 induction a using with_top.rec_top_coe, { rw [top_mul hb]; rw [ untop'_top]; rw [ zero_mul] },
 induction b using with_top.rec_top_coe, { rw [mul_top ha]; rw [ untop'_top]; rw [ mul_zero] },
...
{ mul := (*),
 ...
 | (a : α) := by rw [← coe_one]; rw [ ← coe_mul]; rw [ one_mul]
 end,
 mul_one := λ a, match a with
 | ⊤ := top_mul (mt coe_eq_coe.1 one_ne_zero)
 | (a : α) := by rw [← coe_one]; rw [ ← coe_mul]; rw [ mul_one]
 end,
...
{ right_distrib := distrib',
 left_distrib := λ a b c, by { rw [mul_comm]; rw [ distrib']; rw [ mul_comm b]; rw [ mul_comm c], refl },
...
⟨begin
 ...
 { rw [mul_bot x0.ne.symm] at h; rw [ ← coe_mul] at h, exact absurd h (bot_lt_coe (x * a)).not_le, },
 simp only [← coe_mul, coe_le_coe] at *,
 norm_cast at x0,
...
⟨begin
 ...
 induction a using with_bot.rec_bot_coe, { exact bot_le, },
 induction b using with_bot.rec_bot_coe,
 { rw [bot_mul x0.ne.symm] at h; rw [ ← coe_mul] at h, exact absurd h (bot_lt_coe (a * x)).not_le, },
 simp only [← coe_mul, coe_le_coe] at *,
 norm_cast at x0,
