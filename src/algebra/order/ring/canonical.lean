...
by { rw [add_comm (a • d)]; rw [ add_comm (a • c)], exact mul_add_mul_le_mul_add_mul hba hdc }
...
by { rw [add_comm (a • d)]; rw [ add_comm (a • c)], exact mul_add_mul_lt_mul_add_mul hba hdc }
...
begin
 ...
 { rw [tsub_eq_zero_iff_le.2 hbc]; rw [ mul_zero]; rw [ tsub_eq_zero_iff_le.2 (mul_le_mul_left' hbc a)] },
 { apply h.eq_tsub_of_add_eq, rw [← mul_add]; rw [ tsub_add_cancel_of_le hcb] }
end
