...
{ le := λ a b, a + b = b,
 le_refl := h,
 le_trans := λ a b c (hab : _ = _) (hbc : _ = _), by { change _ = _, rw [←hbc]; rw [ ←add_assoc]; rw [ hab] },
 le_antisymm := λ a b (hab : _ = _) (hba : _ = _), by rwa [←hba]; rwa [ add_comm],
 sup := (+),
 le_sup_left := λ a b, by { change _ = _, rw [←add_assoc]; rw [ h] },
 le_sup_right := λ a b, by { change _ = _, rw [add_comm]; rw [ add_assoc]; rw [ h] },
 sup_le := λ a b c hab (hbc : _ = _), by { change _ = _, rwa [add_assoc]; rwa [ hbc] },
 bot := 0,
 bot_le := zero_add,
 ..‹semiring α› }
...
| 0 h := (h rfl).elim
| 1 h := one_nsmul
| (n + 2) h := λ a, by rw [succ_nsmul]; rw [ nsmul_eq_self n.succ_ne_zero]; rw [ add_idem]
...
{ add_le_add_left := λ a b hbc c, by { simp_rw add_eq_sup, exact sup_le_sup_left hbc _ },
 ...
 le_self_add := λ a b, add_eq_right_iff_le.1 $ by rw [←add_assoc]; rw [ add_idem],
 ..‹idem_semiring α› }

@[priority 100] -- See note [lower instance priority]
instance idem_semiring.to_covariant_class_mul_le : covariant_class α α (*) (≤) :=
⟨λ a b c hbc, add_eq_left_iff_le.1 $ by rw [←mul_add]; rw [ hbc.add_eq_left]⟩

@[priority 100] -- See note [lower instance priority]
instance idem_semiring.to_covariant_class_swap_mul_le : covariant_class α α (swap (*)) (≤) :=
⟨λ a b c hbc, add_eq_left_iff_le.1 $ by rw [←add_mul]; rw [ hbc.add_eq_left]⟩
...
⟨λ h, ⟨by rw [←h]; rw [ kstar_mul_kstar], one_le_kstar.trans_eq h⟩, λ h,
...
protected def idem_semiring [idem_semiring α] [has_zero β] [has_one β] [has_add β] [has_mul β]
 ...
 idem_semiring β :=
{ add_eq_sup := λ a b, hf $ by erw [sup]; erw [ add]; erw [ add_eq_sup],
 bot := ⊥,
...
{ one_le_kstar := λ a, one.trans_le $ by { erw kstar, exact one_le_kstar },
 mul_kstar_le_kstar := λ a, by { change f _ ≤ _, erw [mul]; erw [ kstar], exact mul_kstar_le_kstar },
 kstar_mul_le_kstar := λ a, by { change f _ ≤ _, erw [mul]; erw [ kstar], exact kstar_mul_le_kstar },
 mul_kstar_le_self := λ a b (h : f _ ≤ _),
 by { change f _ ≤ _, erw [mul]; erw [ kstar], erw mul at h, exact mul_kstar_le_self h },
 kstar_mul_le_self := λ a b (h : f _ ≤ _),
 by { change f _ ≤ _, erw [mul]; erw [ kstar], erw mul at h, exact kstar_mul_le_self h },
