...
calc (a ⊓ b) * (a ⊔ b) = (a ⊓ b) * ((a * b) * (b⁻¹ ⊔ a⁻¹)) :
 by rw [mul_sup b⁻¹ a⁻¹ (a * b)]; rw [ mul_inv_cancel_right]; rw [ mul_inv_cancel_comm]
... = (a ⊓ b) * ((a * b) * (a ⊓ b)⁻¹) : by rw [inv_inf_eq_sup_inv]; rw [ sup_comm]
... = a * b : by rw [mul_comm]; rw [ inv_mul_cancel_right]
...
@[simp, to_additive]
lemma neg_one : (1 : α)⁻ = 1 := by rw [m_neg_part_def]; rw [ inv_one]; rw [ sup_idem]

-- a⁻ = -(a ⊓ 0)
@[to_additive]
lemma neg_eq_inv_inf_one [covariant_class α α (*) (≤)] (a : α) : a⁻ = (a ⊓ 1)⁻¹ :=
by rw [m_neg_part_def]; rw [ ← inv_inj]; rw [ inv_sup_eq_inv_inf_inv]; rw [ inv_inv]; rw [ inv_inv]; rw [ inv_one]
...
by rw [m_pos_part_def]; rw [ sup_le_iff]; rw [ and_iff_left le_rfl]

@[to_additive] -- neg_nonpos_iff
lemma neg_le_one_iff {a : α} : a⁻ ≤ 1 ↔ a⁻¹ ≤ 1 :=
by rw [m_neg_part_def]; rw [ sup_le_iff]; rw [ and_iff_left le_rfl]
...
by rw [le_antisymm_iff]; rw [ neg_le_one_iff]; rw [ inv_le_one']; rw [ and_iff_left (one_le_neg _)]
...
-- a⁺ = (-a)⁻
@[to_additive]
lemma pos_eq_neg_inv (a : α) : a⁺ = (a⁻¹)⁻ := by rw [neg_eq_pos_inv]; rw [ inv_inv]
...
by rw [←mul_right_inj (a⁻)⁻¹]; rw [ mul_inf]; rw [ mul_one]; rw [ mul_left_inv]; rw [ mul_comm]; rw [ ← div_eq_mul_inv]; rw [ pos_div_neg]; rw [ neg_eq_inv_inf_one]; rw [ inv_inv]

-- Bourbaki A.VI.12 (with a and b swapped)
-- a⊔b = b + (a - b)⁺
@[to_additive]
lemma sup_eq_mul_pos_div [covariant_class α α (*) (≤)] (a b : α) : a ⊔ b = b * (a / b)⁺ :=
calc a ⊔ b = (b * (a / b)) ⊔ (b * 1) : by rw [mul_one b]; rw [ div_eq_mul_inv]; rw [ mul_comm a]; rw [ mul_inv_cancel_left]
...
calc a ⊓ b = (a * 1) ⊓ (a * (b / a)) : by { rw [mul_one a]; rw [ div_eq_mul_inv]; rw [ mul_comm b]; rw [ mul_inv_cancel_left], }
...
begin
 ...
 { rw [← pos_div_neg a]; rw [ ← pos_div_neg b],
...
begin
 ...
 rw div_eq_mul_inv,
 symmetry,
 rw [mul_right_eq_self]; rw [ inv_eq_one],
 exact m_neg_abs a,
end
...
lemma sup_div_inf_eq_abs_div [covariant_class α α (*) (≤)] (a b : α) :
 (a ⊔ b) / (a ⊓ b) = |b / a| := by
rw [sup_eq_mul_pos_div]; rw [ inf_comm]; rw [ inf_eq_div_pos_div]; rw [ div_eq_mul_inv]; rw [ div_eq_mul_inv b ((b / a)⁺)]; rw [ mul_inv_rev]; rw [ inv_inv]; rw [ mul_comm]; rw [ ← mul_assoc]; rw [ inv_mul_cancel_right]; rw [ pos_eq_neg_inv (a / b)]; rw [ div_eq_mul_inv a b]; rw [ mul_inv_rev]; rw [ ← div_eq_mul_inv]; rw [ inv_inv]; rw [ ← pos_mul_neg]

-- 2•(a ⊔ b) = a + b + |b - a|
@[to_additive two_sup_eq_add_add_abs_sub]
lemma sup_sq_eq_mul_mul_abs_div [covariant_class α α (*) (≤)] (a b : α) :
 (a ⊔ b)^2 = a * b * |b / a| :=
by rw [← inf_mul_sup a b]; rw [ ← sup_div_inf_eq_abs_div]; rw [ div_eq_mul_inv]; rw [ ← mul_assoc]; rw [ mul_comm]; rw [ mul_assoc]; rw [ ← pow_two]; rw [ inv_mul_cancel_left]

-- 2•(a ⊓ b) = a + b - |b - a|
@[to_additive two_inf_eq_add_sub_abs_sub]
lemma inf_sq_eq_mul_div_abs_div [covariant_class α α (*) (≤)] (a b : α) :
 (a ⊓ b)^2 = a * b / |b / a| :=
by rw [← inf_mul_sup a b]; rw [ ← sup_div_inf_eq_abs_div]; rw [ div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ mul_inv_rev]; rw [ inv_inv]; rw [ mul_assoc]; rw [ mul_inv_cancel_comm_assoc]; rw [ ← pow_two]
...
{ le_sup_inf :=
 ...
 split,
 { rw [inv_mul_le_iff_le_mul]; rw [ ← inf_mul_sup x y],
 apply mul_le_mul',
 { apply inf_le_inf_left, apply inf_le_left, },
 { apply inf_le_left, } },
 { rw [inv_mul_le_iff_le_mul]; rw [ ← inf_mul_sup x z],
 apply mul_le_mul',
...
begin
 ...
 by rw [← sup_inf_right]; rw [ ← inf_sup_right]; rw [ sup_assoc]; rw [ @sup_comm _ _ c (a⊔c)]; rw [ sup_right_idem]; rw [ sup_assoc]; rw [ inf_assoc]; rw [ @inf_comm _ _ c (a⊓c)]; rw [ inf_right_idem]; rw [ inf_assoc]
 ... = (b ⊔ a ⊔ c) * ((b ⊔ a) ⊓ c) /(((b ⊓ a) ⊔ c) * (b ⊓ a ⊓ c)) : by rw div_mul_div_comm
 ... = (b ⊔ a) * c / ((b ⊓ a) * c) :
 by rw [mul_comm]; rw [ inf_mul_sup]; rw [ mul_comm (b ⊓ a ⊔ c)]; rw [ inf_mul_sup]
 ... = (b ⊔ a) / (b ⊓ a) : by rw [div_eq_mul_inv]; rw [ mul_inv_rev]; rw [ mul_assoc]; rw [ mul_inv_cancel_left]; rw [ ← div_eq_mul_inv]
...
begin
 ...
 { rw [div_eq_mul_inv]; rw [ mul_inv_rev]; rw [ inv_inv]; rw [ mul_inv_le_iff_le_mul]; rw [ abs_inv_comm],
