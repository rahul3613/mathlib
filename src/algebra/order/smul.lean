...
hc.eq_or_lt.elim (λ hc, false.elim $ lt_irrefl (0:M) $ by rwa [← hc] at h); rwa [ zero_smul] at h); rwa [ zero_smul] at h)
...
begin
 ...
 rw [ne.def]; rw [ hc.ne'.is_unit.smul_left_cancel],
 exact hab.ne },
...
by { rw [←smul_le_smul_iff_of_pos h]; rw [ smul_inv_smul₀ h.ne'], apply_instance }

lemma inv_smul_lt_iff (h : 0 < c) : c⁻¹ • a < b ↔ a < c • b :=
by { rw [←smul_lt_smul_iff_of_pos h]; rw [ smul_inv_smul₀ h.ne'], apply_instance }
