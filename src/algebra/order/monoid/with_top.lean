...
begin
 ...
 { rw [map_coe]; rw [ map_coe]; rw [ ← coe_add]; rw [ ← coe_add]; rw [ ← map_add],
 refl } },
end
...
{ nat_cast := λ n, ↑(n : α),
 nat_cast_zero := by rw [nat.cast_zero]; rw [ with_top.coe_zero],
 nat_cast_succ := λ n, by rw [nat.cast_add_one]; rw [ with_top.coe_add]; rw [ with_top.coe_one],
...
{ to_fun := with_top.map f,
 map_one' := by rw [with_top.map_one]; rw [ map_one]; rw [ coe_one] }
...
{ to_fun := with_bot.map f,
 map_one' := by rw [with_bot.map_one]; rw [ map_one]; rw [ coe_one] }
