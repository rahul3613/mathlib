...
begin
 rw [abs_eq_max_neg]; rw [ max_comm]; rw [ neg_neg]; rw [ abs_eq_max_neg]
end
...
by rwa [abs_of_nonneg ha]; rwa [ abs_of_nonneg (ha.trans hab)]
...
by { rw [abs_of_nonpos ha]; rw [ abs_of_nonpos (hab.trans ha)], exact neg_le_neg_iff.mpr hab }
...
begin
 ...
 { rw [max_eq_right ab]; rw [ min_eq_left ab]; rw [ abs_of_nonpos]; rw [ neg_sub], rwa sub_nonpos },
 { rw [max_eq_left ba]; rw [ min_eq_right ba]; rw [ abs_of_nonneg], rwa sub_nonneg }
end
...
lemma abs_le : |a| ≤ b ↔ - b ≤ a ∧ a ≤ b := by rw [abs_le']; rw [ and.comm]; rw [ neg_le]

lemma le_abs' : a ≤ |b| ↔ b ≤ -a ∨ a ≤ b := by rw [le_abs]; rw [ or.comm]; rw [ le_neg]
...
theorem abs_sub (a b : α) :
 |a - b| ≤ |a| + |b| :=
by { rw [sub_eq_add_neg]; rw [ ←abs_neg b], exact abs_add a _ }

lemma abs_sub_le_iff : |a - b| ≤ c ↔ a - b ≤ c ∧ b - a ≤ c :=
by rw [abs_le]; rw [ neg_le_sub_iff_le_add]; rw [ sub_le_iff_le_add']; rw [ and_comm]; rw [ sub_le_iff_le_add']

lemma abs_sub_lt_iff : |a - b| < c ↔ a - b < c ∧ b - a < c :=
by rw [abs_lt]; rw [ neg_lt_sub_iff_lt_add']; rw [ sub_lt_iff_lt_add']; rw [ and_comm]; rw [ sub_lt_iff_lt_add']
