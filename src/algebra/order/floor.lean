...
eq_of_forall_le_iff $ λ a, by { rw [le_floor_iff]; rw [ nat.cast_le], exact n.cast_nonneg }

@[simp] lemma floor_zero : ⌊(0 : α)⌋₊ = 0 := by rw [← nat.cast_zero]; rw [ floor_coe]

@[simp] lemma floor_one : ⌊(1 : α)⌋₊ = 1 := by rw [←nat.cast_one]; rw [ floor_coe]
...
by { rw [←lt_one_iff]; rw [ ←@cast_one α], exact floor_lt' nat.one_ne_zero }

lemma floor_eq_iff (ha : 0 ≤ a) : ⌊a⌋₊ = n ↔ ↑n ≤ a ∧ a < ↑n + 1 :=
by rw [←le_floor_iff ha]; rw [ ←nat.cast_one]; rw [ ←nat.cast_add]; rw [ ←floor_lt ha]; rw [ nat.lt_add_one_iff]; rw [ le_antisymm_iff]; rw [ and.comm]

lemma floor_eq_iff' (hn : n ≠ 0) : ⌊a⌋₊ = n ↔ ↑n ≤ a ∧ a < ↑n + 1 :=
by rw [← le_floor_iff' hn]; rw [ ← nat.cast_one]; rw [ ← nat.cast_add]; rw [ ← floor_lt' (nat.add_one_ne_zero n)]; rw [ nat.lt_add_one_iff]; rw [ le_antisymm_iff]; rw [ and.comm]
...
by rw [← nat.lt_ceil]; rw [ nat.add_one_le_iff]

@[simp] lemma one_le_ceil_iff : 1 ≤ ⌈a⌉₊ ↔ 0 < a :=
by rw [← zero_add 1]; rw [ nat.add_one_le_ceil_iff]; rw [ nat.cast_zero]

lemma ceil_le_floor_add_one (a : α) : ⌈a⌉₊ ≤ ⌊a⌋₊ + 1 :=
by { rw [ceil_le]; rw [ nat.cast_add]; rw [ nat.cast_one], exact (lt_floor_add_one a).le }
...
eq_of_forall_ge_iff $ λ a, by rw [ceil_le]; rw [ cast_le]

lemma ceil_mono : monotone (ceil : α → ℕ) := gc_ceil_coe.monotone_l

@[simp] lemma ceil_zero : ⌈(0 : α)⌉₊ = 0 := by rw [← nat.cast_zero]; rw [ ceil_nat_cast]

@[simp] lemma ceil_one : ⌈(1 : α)⌉₊ = 1 := by rw [←nat.cast_one]; rw [ ceil_nat_cast]

@[simp] lemma ceil_eq_zero : ⌈a⌉₊ = 0 ↔ a ≤ 0 := by rw [← le_zero_iff]; rw [ ceil_le]; rw [ nat.cast_zero]

@[simp] lemma ceil_pos : 0 < ⌈a⌉₊ ↔ 0 < a := by rw [lt_ceil]; rw [ cast_zero]
...
begin
 ...
 { rwa [floor_of_nonpos ha.le]; rwa [ lt_ceil]; rwa [ nat.cast_zero] }
end

lemma ceil_eq_iff (hn : n ≠ 0) : ⌈a⌉₊ = n ↔ ↑(n - 1) < a ∧ a ≤ n :=
by rw [← ceil_le]; rw [ ← not_le]; rw [ ← ceil_le]; rw [ not_le]; rw [ tsub_lt_iff_right (nat.add_one_le_iff.2 (pos_iff_ne_zero.2 hn))]; rw [ nat.lt_add_one_iff]; rw [ le_antisymm_iff]; rw [ and.comm]
...
begin
 obtain ha | ha := le_total a 0,
 { rw [floor_of_nonpos ha]; rw [ floor_of_nonpos (tsub_nonpos_of_le (ha.trans n.cast_nonneg))]; rw [ zero_tsub] },
 cases le_total a n,
 { rw [floor_of_nonpos (tsub_nonpos_of_le h)]; rw [ eq_comm]; rw [ tsub_eq_zero_iff_le],
 exact nat.cast_le.1 ((nat.floor_le ha).trans h) },
 { rw [eq_tsub_iff_add_eq_of_le (le_floor h)]; rw [ ←floor_add_nat _]; rw [ tsub_add_cancel_of_le h],
 exact le_tsub_of_add_le_left ((add_zero _).trans_le h), }
end

lemma ceil_add_nat (ha : 0 ≤ a) (n : ℕ) : ⌈a + n⌉₊ = ⌈a⌉₊ + n :=
eq_of_forall_ge_iff $ λ b, begin
 rw [←not_lt]; rw [ ←not_lt]; rw [ not_iff_not],
 rw [lt_ceil],
...
begin
 ...
 { rw [floor_of_nonpos]; rw [ floor_of_nonpos ha],
 { simp },
 apply div_nonpos_of_nonpos_of_nonneg ha n.cast_nonneg },
 obtain rfl | hn := n.eq_zero_or_pos,
 { rw [cast_zero]; rw [ div_zero]; rw [ nat.div_zero]; rw [ floor_zero] },
...
begin
 ...
 { ext a,
 cases lt_or_le a 0,
 { rw [H₁.floor_of_neg]; rw [ H₂.floor_of_neg]; exact h },
...
{ floor := floor,
 ...
 gc_ceil_coe := λ a z, by rw [neg_le]; rw [ ←gc_coe_floor]; rw [ int.cast_neg]; rw [ neg_le_neg_iff] }
...
{ floor := λ a, -ceil (-a),
 ceil := ceil,
 gc_coe_floor := λ a z, by rw [le_neg]; rw [ gc_ceil_coe]; rw [ int.cast_neg]; rw [ neg_le_neg_iff],
...
lemma floor_nonneg : 0 ≤ ⌊a⌋ ↔ 0 ≤ a := by rw [le_floor]; rw [ int.cast_zero]

@[simp] lemma floor_le_sub_one_iff : ⌊a⌋ ≤ z - 1 ↔ a < z := by rw [← floor_lt]; rw [ le_sub_one_iff]

@[simp] lemma floor_le_neg_one_iff : ⌊a⌋ ≤ -1 ↔ a < 0 :=
by rw [← zero_sub (1 : ℤ)]; rw [ floor_le_sub_one_iff]; rw [ cast_zero]
...
eq_of_forall_le_iff $ λ a, by rw [le_floor]; rw [ int.cast_le]

@[simp] lemma floor_nat_cast (n : ℕ) : ⌊(n : α)⌋ = n :=
eq_of_forall_le_iff $ λ a, by rw [le_floor]; rw [ ← cast_coe_nat]; rw [ cast_le]

@[simp] lemma floor_zero : ⌊(0 : α)⌋ = 0 := by rw [← cast_zero]; rw [ floor_int_cast]

@[simp] lemma floor_one : ⌊(1 : α)⌋ = 1 := by rw [← cast_one]; rw [ floor_int_cast]
...
eq_of_forall_le_iff $ λ a, by rw [le_floor]; rw [ ← sub_le_iff_le_add]; rw [ ← sub_le_iff_le_add]; rw [ le_floor]; rw [ int.cast_sub]
...
by rw [← int.cast_coe_nat]; rw [ floor_add_int]

@[simp] lemma floor_nat_add (n : ℕ) (a : α) : ⌊↑n + a⌋ = n + ⌊a⌋ :=
by rw [← int.cast_coe_nat]; rw [ floor_int_add]

@[simp] lemma floor_sub_int (a : α) (z : ℤ) : ⌊a - z⌋ = ⌊a⌋ - z :=
eq.trans (by rw [int.cast_neg]; rw [ sub_eq_add_neg]) (floor_add_int _ _)

@[simp] lemma floor_sub_nat (a : α) (n : ℕ) : ⌊a - n⌋ = ⌊a⌋ - n :=
by rw [← int.cast_coe_nat]; rw [ floor_sub_int]
...
by rw [le_antisymm_iff]; rw [ le_floor]; rw [ ←int.lt_add_one_iff]; rw [ floor_lt]; rw [ int.cast_add]; rw [ int.cast_one]; rw [ and.comm]
...
@[simp] lemma fract_zero : fract (0 : α) = 0 := by rw [fract]; rw [ floor_zero]; rw [ cast_zero]; rw [ sub_self]
...
by rw [floor_eq_iff]; rw [ int.cast_zero]; rw [ zero_add]; exact ⟨fract_nonneg _, fract_lt_one _⟩
...
⟨λ h, by { rw ←h, exact ⟨fract_nonneg _, fract_lt_one _, ⟨⌊a⌋, sub_sub_cancel _ _⟩⟩},
 ...
 rw [eq_sub_iff_add_eq]; rw [ add_comm]; rw [ ←eq_sub_iff_add_eq],
 rw [hz]; rw [ int.cast_inj]; rw [ floor_eq_iff]; rw [ ←hz],
 clear hz, split; simpa [sub_eq_add_neg, add_assoc]
 end⟩

lemma fract_eq_fract {a b : α} : fract a = fract b ↔ ∃ z : ℤ, a - b = z :=
⟨λ h, ⟨⌊a⌋ - ⌊b⌋, begin
 unfold fract at h, rw [int.cast_sub]; rw [ sub_eq_sub_iff_sub_eq_sub.1 h],
 end⟩, begin
 rintro ⟨z, hz⟩,
...
begin
 rw fract_eq_iff,
 split,
 { rw [le_sub_iff_add_le]; rw [ zero_add],
...
lemma fract_div_mul_self_add_zsmul_eq (a b : k) (ha : a ≠ 0) :
 fract (b/a) * a + ⌊b/a⌋ • a = b :=
by rw [zsmul_eq_mul]; rw [ ← add_mul]; rw [ fract_add_floor]; rw [ div_mul_cancel b ha]
...
sub_lt_iff_lt_add.2 $ by { rw [←one_add_mul]; rw [ ←div_lt_iff hb]; rw [ add_comm], exact lt_floor_add_one _ }
...
begin
 ...
 { rw [sub_eq_iff_eq_add']; rw [ ← mul_right_inj' hn'.ne.symm]; rw [ mul_div_cancel' _ hn'.ne.symm]; rw [ mul_add]; rw [ mul_div_cancel' _ hn'.ne.symm],
 norm_cast,
...
begin
 ...
 { rw [cast_coe_nat]; rw [ ← coe_nat_mod]; rw [ cast_coe_nat]; rw [ fract_div_nat_cast_eq_div_nat_cast_mod], },
 { rw [right.nonneg_neg_iff] at hl; rw [ coe_nat_nonpos_iff] at hl, simp [hl, zero_mod], }, },
 ...
 ... = ↑(m₁ % (n : ℤ)) / ↑n : this hm₁
 ... = ↑(-(↑m₀ : ℤ) % ↑n) / ↑n : _,
 { rw [← fract_int_add q]; rw [ ← mul_div_cancel (q : k) (ne_of_gt hn)]; rw [ ← add_div]; rw [ ← sub_eq_add_neg],
 push_cast, },
 { congr' 2,
 change ((q * ↑n) -(↑m₀ : ℤ)) % ↑n = _,
 rw [sub_eq_add_neg]; rw [ add_comm (q * ↑n)]; rw [ add_mul_mod_self], },
end
...
lemma floor_neg : ⌊-a⌋ = -⌈a⌉ :=
eq_of_forall_le_iff (λ z, by rw [le_neg]; rw [ ceil_le]; rw [ le_floor]; rw [ int.cast_neg]; rw [ le_neg])

lemma ceil_neg : ⌈-a⌉ = -⌊a⌋ :=
eq_of_forall_ge_iff (λ z, by rw [neg_le]; rw [ ceil_le]; rw [ le_floor]; rw [ int.cast_neg]; rw [ neg_le])

lemma lt_ceil : z < ⌈a⌉ ↔ (z : α) < a := lt_iff_lt_of_le_iff_le ceil_le

@[simp] lemma add_one_le_ceil_iff : z + 1 ≤ ⌈a⌉ ↔ (z : α) < a := by rw [← lt_ceil]; rw [ add_one_le_iff]

@[simp] lemma one_le_ceil_iff : 1 ≤ ⌈a⌉ ↔ 0 < a :=
by rw [← zero_add (1 : ℤ)]; rw [ add_one_le_ceil_iff]; rw [ cast_zero]

lemma ceil_le_floor_add_one (a : α) : ⌈a⌉ ≤ ⌊a⌋ + 1 :=
by { rw [ceil_le]; rw [ int.cast_add]; rw [ int.cast_one], exact (lt_floor_add_one a).le }

lemma le_ceil (a : α) : a ≤ ⌈a⌉ := gc_ceil_coe.le_u_l a

@[simp] lemma ceil_int_cast (z : ℤ) : ⌈(z : α)⌉ = z :=
eq_of_forall_ge_iff $ λ a, by rw [ceil_le]; rw [ int.cast_le]

@[simp] lemma ceil_nat_cast (n : ℕ) : ⌈(n : α)⌉ = n :=
eq_of_forall_ge_iff $ λ a, by rw [ceil_le]; rw [ ← cast_coe_nat]; rw [ cast_le]

lemma ceil_mono : monotone (ceil : α → ℤ) := gc_ceil_coe.monotone_l

@[simp] lemma ceil_add_int (a : α) (z : ℤ) : ⌈a + z⌉ = ⌈a⌉ + z :=
by rw [←neg_inj]; rw [ neg_add']; rw [ ←floor_neg]; rw [ ←floor_neg]; rw [ neg_add']; rw [ floor_sub_int]

@[simp] lemma ceil_add_nat (a : α) (n : ℕ) : ⌈a + n⌉ = ⌈a⌉ + n :=
by rw [← int.cast_coe_nat]; rw [ ceil_add_int]
...
eq.trans (by rw [int.cast_neg]; rw [ sub_eq_add_neg]) (ceil_add_int _ _)

@[simp] lemma ceil_sub_nat (a : α) (n : ℕ) : ⌈a - n⌉ = ⌈a⌉ - n :=
by convert ceil_sub_int a n using 1; simp

@[simp] lemma ceil_sub_one (a : α) : ⌈a - 1⌉ = ⌈a⌉ - 1 :=
by rw [eq_sub_iff_add_eq]; rw [ ← ceil_add_one]; rw [ sub_add_cancel]

lemma ceil_lt_add_one (a : α) : (⌈a⌉ : α) < a + 1 :=
by { rw [← lt_ceil]; rw [ ← int.cast_one]; rw [ ceil_add_int], apply lt_add_one }
...
@[simp] lemma ceil_zero : ⌈(0 : α)⌉ = 0 := by rw [← cast_zero]; rw [ ceil_int_cast]

@[simp] lemma ceil_one : ⌈(1 : α)⌉ = 1 := by rw [← cast_one]; rw [ ceil_int_cast]

lemma ceil_nonneg (ha : 0 ≤ a) : 0 ≤ ⌈a⌉ :=
by exact_mod_cast ha.trans (le_ceil a)

lemma ceil_eq_iff : ⌈a⌉ = z ↔ ↑z - 1 < a ∧ a ≤ z :=
by rw [←ceil_le]; rw [ ←int.cast_one]; rw [ ←int.cast_sub]; rw [ ←lt_ceil]; rw [ int.sub_one_lt_iff]; rw [ le_antisymm_iff]; rw [ and.comm]
...
begin
 ...
 suffices : (⌈a⌉ : α) = ⌊a⌋ + 1, { rw [this]; rw [ ← self_sub_fract], abel, },
 norm_cast,
 rw ceil_eq_iff,
...
by rw [round]; rw [ round]; rw [ int.fract_add_int]; rw [ int.floor_add_int]; rw [ int.ceil_add_int]; rw [ ← apply_ite2]; rw [ if_t_t]
...
by { rw [sub_eq_add_neg], norm_cast, rw [round_add_int]; rw [ sub_eq_add_neg] }
...
by rw [round]; rw [ round]; rw [ fract_add_nat]; rw [ int.floor_add_nat]; rw [ int.ceil_add_nat]; rw [ ← apply_ite2]; rw [ if_t_t]

@[simp]
lemma round_sub_nat (x : α) (y : ℕ) : round (x - y) = round x - y :=
by { rw [sub_eq_add_neg]; rw [ ← int.cast_coe_nat], norm_cast, rw [round_add_int]; rw [ sub_eq_add_neg] }

@[simp]
lemma round_int_add (x : α) (y : ℤ) : round ((y : α) + x) = y + round x :=
by { rw [add_comm]; rw [ round_add_int]; rw [ add_comm] }

@[simp]
lemma round_nat_add (x : α) (y : ℕ) : round ((y : α) + x) = y + round x :=
by { rw [add_comm]; rw [ round_add_nat]; rw [ add_comm] }
...
begin
 ...
 { rw [if_pos hx]; rw [ if_pos hx]; rw [ self_sub_floor]; rw [ abs_fract], },
 { have : 0 < fract x,
...
begin
 ...
 { conv_rhs { rw [abs_eq_self.mpr (sub_nonneg.mpr hx)]; rw [ ← fract_add_floor x]; rw [ add_sub_assoc], },
...
begin
 ...
 { conv_rhs { rw [← fract_add_floor x]; rw [ add_assoc]; rw [ add_left_comm]; rw [ floor_int_add], },
...
begin
 ...
 rw [abs_sub_round_eq_min]; rw [ nat.cast_min]; rw [ ← min_div_div_right hn'.le]; rw [ fract_div_nat_cast_eq_div_nat_cast_mod]; rw [ nat.cast_sub (m.mod_lt hn).le]; rw [ sub_div]; rw [ div_self hn'.ne.symm],
end
...
begin
 ...
 { rw [floor_of_nonpos ha.le]; rw [ floor_of_nonpos (le_of_not_le $ h₀.not.mp ha.not_le)] },
...
floor_congr $ λ n, by rw [←map_nat_cast f]; rw [ hf.le_iff_le]

lemma map_ceil (f : F) (hf : strict_mono f) (a : α) : ⌈f a⌉₊ = ⌈a⌉₊ :=
ceil_congr $ λ n, by rw [←map_nat_cast f]; rw [ hf.le_iff_le]
...
lemma map_floor (f : F) (hf : strict_mono f) (a : α) : ⌊f a⌋ = ⌊a⌋ :=
floor_congr $ λ n, by rw [←map_int_cast f]; rw [ hf.le_iff_le]

lemma map_ceil (f : F) (hf : strict_mono f) (a : α) : ⌈f a⌉ = ⌈a⌉ :=
ceil_congr $ λ n, by rw [←map_int_cast f]; rw [ hf.le_iff_le]
...
{ floor := λ a, ⌊a⌋.to_nat,
 ...
 gc_floor := λ a n ha,
 by rw [int.le_to_nat_iff (int.floor_nonneg.2 ha)]; rw [ int.le_floor]; rw [ int.cast_coe_nat],
 gc_ceil := λ a n, by rw [int.to_nat_le]; rw [ int.ceil_le]; rw [ int.cast_coe_nat] }
...
by rw [←int.floor_to_nat]; rw [ int.to_nat_of_nonneg (int.floor_nonneg.2 ha)]

lemma nat.cast_floor_eq_cast_int_floor (ha : 0 ≤ a) : (⌊a⌋₊ : α) = ⌊a⌋ :=
by rw [←nat.cast_floor_eq_int_floor ha]; rw [ int.cast_coe_nat]

lemma nat.cast_ceil_eq_int_ceil (ha : 0 ≤ a) : (⌈a⌉₊ : ℤ) = ⌈a⌉ :=
by { rw [←int.ceil_to_nat]; rw [ int.to_nat_of_nonneg (int.ceil_nonneg ha)] }

lemma nat.cast_ceil_eq_cast_int_ceil (ha : 0 ≤ a) : (⌈a⌉₊ : α) = ⌈a⌉ :=
by rw [←nat.cast_ceil_eq_int_ceil ha]; rw [ int.cast_coe_nat]
