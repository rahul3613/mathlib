...
begin
 ...
 { rw [mem_set_of_eq] at hq; rw [ ←sub_lt_iff_lt_add] at hq,
...
eq_of_forall_rat_lt_iff_lt $ λ q,
 by rw [coe_lt_induced_map_iff]; rw [ coe_lt_induced_map_iff]; rw [ iff.comm]; rw [ coe_lt_induced_map_iff]

@[simp] lemma induced_map_inv_self (b : β) : induced_map γ β (induced_map β γ b) = b :=
by rw [induced_map_induced_map]; rw [ induced_map_self]

lemma induced_map_add (x y : α) : induced_map α β (x + y) = induced_map α β x + induced_map α β y :=
begin
 rw [induced_map]; rw [ cut_map_add],
...
begin
 ...
 { refine ⟨0, _, hb⟩,
 rw [←rat.cast_zero]; rw [ coe_mem_cut_map_iff]; rw [ rat.cast_zero],
...
{ monotone' := induced_map_mono _ _,
 ...
 { rw [neg_mul]; rw [ mul_neg]; rw [ neg_neg] },
