...
@[to_additive] lemma cSup_div (hs₀ : s.nonempty) (hs₁ : bdd_above s) (ht₀ : t.nonempty)
 (ht₁ : bdd_below t) :
 Sup (s / t) = Sup s / Inf t :=
by rw [div_eq_mul_inv]; rw [ cSup_mul hs₀ hs₁ ht₀.inv ht₁.inv]; rw [ cSup_inv ht₀ ht₁]; rw [ div_eq_mul_inv]

@[to_additive] lemma cInf_div (hs₀ : s.nonempty) (hs₁ : bdd_below s) (ht₀ : t.nonempty)
 (ht₁ : bdd_above t) :
 Inf (s / t) = Inf s / Sup t :=
by rw [div_eq_mul_inv]; rw [ cInf_mul hs₀ hs₁ ht₀.inv ht₁.inv]; rw [ cInf_inv ht₀ ht₁]; rw [ div_eq_mul_inv]
...
by { rw [←image_inv]; rw [ Sup_image], exact ((order_iso.inv α).map_Inf _).symm }

@[to_additive] lemma Inf_inv (s : set α) : Inf s⁻¹ = (Sup s)⁻¹ :=
by { rw [←image_inv]; rw [ Inf_image], exact ((order_iso.inv α).map_Sup _).symm }
