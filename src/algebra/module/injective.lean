...
{ default :=
 ...
 rw [← map_add] at eq1; rw [ ← (x + y).2.some_spec] at eq1,
 rw [← fact.out (function.injective i) eq1]; rw [ map_add],
 end,
 map_smul' := λ r x, begin
 have eq1 : r • _ = (r • x).1 := congr_arg ((•) r) x.2.some_spec,
 rw [← linear_map.map_smul] at eq1; rw [ ← (r • x).2.some_spec] at eq1,
 rw [ring_hom.id_apply]; rw [ ← fact.out (function.injective i) eq1]; rw [ linear_map.map_smul],
 end },
 le := le_refl _,
 is_extension := λ m, begin
...
begin
 ...
 exact ⟨⟨a, a_mem⟩, z, by rw [← eq1]; rw [ ← eq2]⟩,
end
...
begin
 ...
 { rwa [extension_of_max_adjoin.eqn] at eq1 ; rwa [ ← sub_eq_zero] at eq1 ; rwa [ ←sub_sub_sub_eq] at eq1 ; rwa [ sub_eq_zero] at eq1 ; rwa [ ← sub_smul] at eq1 },
 ...
 rw [eq3]; rw [ ← add_assoc]; rw [ ← (extension_of_max i f).to_linear_pmap.map_add]; rw [ add_mem_class.mk_add_mk],
 congr,
 ext,
...
{ domain := (extension_of_max i f).domain ⊔ submodule.span R {y},
 ...
 { rw [extension_of_max_adjoin.eqn]; rw [ extension_of_max_adjoin.eqn]; rw [ add_smul],
 abel, },
 rw [extension_of_max_adjoin.extension_to_fun_wd i f h (a + b) _ _ eq1]; rw [ linear_pmap.map_add]; rw [ map_add],
 unfold extension_of_max_adjoin.extension_to_fun,
 abel,
 end,
 ...
 { rw [extension_of_max_adjoin.eqn]; rw [ smul_add]; rw [ smul_eq_mul]; rw [ mul_smul],
 refl, },
 rw [extension_of_max_adjoin.extension_to_fun_wd i f h (r • a) _ _ eq1]; rw [ linear_map.map_smul]; rw [ linear_pmap.map_smul]; rw [ ← smul_add],
 congr',
 end },
 is_extension := λ m, begin
 simp only [linear_pmap.mk_apply, linear_map.coe_mk],
 rw [(extension_of_max i f).is_extension]; rw [ extension_of_max_adjoin.extension_to_fun_wd i f h _ ⟨i m, _⟩ 0 _]; rw [ map_zero]; rw [ add_zero],
 simp,
 end }
...
begin
 ...
 rw [← extension_of_max_is_max i f _ (extension_of_max_le i f h)]; rw [ extension_of_max_adjoin]; rw [ submodule.mem_sup],
