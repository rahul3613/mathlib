...
by rw [← map₂_span_span]; rw [ span_eq]; rw [ span_eq]

lemma map₂_flip (f : M →ₗ[R] N →ₗ[R] P) (p : submodule R M) (q : submodule R N) :
 map₂ f.flip q p = map₂ f p q :=
by { rw [map₂_eq_span_image2]; rw [ map₂_eq_span_image2]; rw [ set.image2_swap], refl }
...
begin
 ...
 { rw [span_le]; rw [ set.image2_subset_iff],
 intros x hx y hy,
...
by rw [← map₂_span_singleton_eq_map]; rw [ map₂_flip]
