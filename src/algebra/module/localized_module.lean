...
mk_eq.mpr ⟨1, by rw [one_smul]; rw [ smul_zero]; rw [ smul_zero]; rw [ one_smul]⟩
...
begin
 ...
 congr' 1,
 { rw [mul_assoc] },
 { rw [eq_comm]; rw [ mul_comm]; rw [ add_assoc]; rw [ mul_smul]; rw [ mul_smul]; rw [ ←mul_smul sx sz]; rw [ mul_comm]; rw [ mul_smul], },
end

private lemma add_comm' (x y : localized_module S M) :
 x + y = y + x :=
localized_module.induction_on₂ (λ m m' s s', by rw [mk_add_mk]; rw [ mk_add_mk]; rw [ add_comm]; rw [ mul_comm]) x y

private lemma zero_add' (x : localized_module S M) : 0 + x = x :=
induction_on (λ m s, by rw [← zero_mk s]; rw [ mk_add_mk]; rw [ smul_zero]; rw [ zero_add]; rw [ mk_eq];
 exact ⟨1, by rw [one_smul]; rw [ mul_smul]; rw [ one_smul]⟩) x

private lemma add_zero' (x : localized_module S M) : x + 0 = x :=
induction_on (λ m s, by rw [← zero_mk s]; rw [ mk_add_mk]; rw [ smul_zero]; rw [ add_zero]; rw [ mk_eq];
 exact ⟨1, by rw [one_smul]; rw [ mul_smul]; rw [ one_smul]⟩) x
...
{ mul := λ m₁ m₂, lift_on₂ m₁ m₂ (λ x₁ x₂, localized_module.mk (x₁.1 * x₂.1) (x₁.2 * x₂.2))
 ...
 all_goals { rw [smul_smul]; rw [ mul_mul_mul_comm]; rw [ ← smul_eq_mul]; rw [ ← smul_eq_mul A]; rw [ smul_smul_smul_comm]; rw [ mul_smul]; rw [ mul_smul] }
 end),
 left_distrib := begin
 intros x₁ x₂ x₃,
...
begin
 ...
 rw [localization.lift_on_mk]; rw [ lift_on_mk],
end
...
by erw [mk_smul_mk r m 1 s]; erw [ one_mul]
...
begin
 ...
 change _ • mk _ _ = _,
 rw [mk_smul_mk]; rw [ algebra.algebra_map_eq_smul_one]; rw [ mul_one]
end
...
{ commutes' := begin
 ...
 obtain ⟨⟨a, s⟩, rfl : mk a s = x⟩ := quotient.exists_rep x,
 dsimp,
 rw [← localization.mk_one_eq_algebra_map]; rw [ algebra_map_mk]; rw [ mk_mul_mk]; rw [ mk_mul_mk]; rw [ mul_comm]; rw [ algebra.commutes],
 end,
 smul_def' := begin
 intros r x,
 obtain ⟨⟨a, s⟩, rfl : mk a s = x⟩ := quotient.exists_rep x,
 dsimp,
 rw [← localization.mk_one_eq_algebra_map]; rw [ algebra_map_mk]; rw [ mk_mul_mk]; rw [ smul'_mk]; rw [ algebra.smul_def]; rw [ one_mul],
 end,
...
{ to_fun := λ p, p.lift_on (λ p, mk p.1 (s * p.2)) $ λ ⟨a, b⟩ ⟨a', b'⟩ ⟨c, eq1⟩, mk_eq.mpr ⟨c,
 begin
 rw [mul_smul]; rw [ mul_smul]; rw [ smul_comm c]; rw [ eq1]; rw [ smul_comm s];
 apply_instance,
 end⟩,
...
begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←h2.unit⁻¹.1.map_smul], symmetry,
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff], dsimp,
 have : c • s • g m' = c • s' • g m,
 { erw [←g.map_smul]; erw [ ←g.map_smul]; erw [ ←g.map_smul]; erw [ ←g.map_smul]; erw [ eq1], refl, },
 ...
 erw [units.inv_eq_coe_inv]; erw [ module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←(h c).unit⁻¹.1.map_smul], symmetry,
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←g.map_smul]; erw [ ←g.map_smul]; erw [ ←g.map_smul]; erw [ ←g.map_smul]; erw [ eq1], refl,
end
...
localized_module.induction_on₂ begin
 intros a a' b b',
 erw [localized_module.lift'_mk]; erw [ localized_module.lift'_mk]; erw [ localized_module.lift'_mk],
 dsimp, generalize_proofs h1 h2 h3,
 erw [map_add]; erw [ module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ smul_add]; erw [ ←h2.unit⁻¹.1.map_smul]; erw [ ←h3.unit⁻¹.1.map_smul],
 congr' 1; symmetry,
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ mul_smul]; erw [ ←map_smul], refl,
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ mul_comm]; erw [ mul_smul]; erw [ ←map_smul], refl,
end x y
...
m.induction_on begin
 intros a b,
 rw [localized_module.lift'_mk]; rw [ localized_module.smul'_mk]; rw [ localized_module.lift'_mk],
 generalize_proofs h1 h2,
 erw [←h1.unit⁻¹.1.map_smul]; erw [ ←g.map_smul],
end
...
{ to_fun := localized_module.lift' S g h,
 ...
 map_smul' := λ r x, by rw [localized_module.lift'_smul]; rw [ ring_hom.id_apply] }
...
begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ one_smul],
end
...
begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←hl]; erw [ linear_map.coe_comp]; erw [ function.comp_app]; erw [ localized_module.mk_linear_map_apply]; erw [ ←l.map_smul]; erw [ localized_module.smul'_mk],
...
{ map_units := λ s, ⟨⟨algebra_map R (module.End R (localized_module S M)) s,
 ...
 intros m t,
 refine ⟨⟨m, t⟩, _⟩,
 erw [localized_module.smul'_mk]; erw [ localized_module.mk_linear_map_apply]; erw [ submonoid.coe_subtype]; erw [ localized_module.mk_cancel t ],
 end,
...
begin
 ...
 dsimp,
 generalize_proofs h1 h2,
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←h2.unit⁻¹.1.map_smul]; erw [ module.End_algebra_map_is_unit_inv_apply_eq_iff']; erw [ ←linear_map.map_smul]; erw [ ←linear_map.map_smul],
...
localized_module.induction_on₂ begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ smul_add]; erw [ ←h2.unit⁻¹.1.map_smul]; erw [ ←h3.unit⁻¹.1.map_smul]; erw [ map_add],
 congr' 1,
 all_goals { erw [module.End_algebra_map_is_unit_inv_apply_eq_iff'] },
 { dsimp, erw [mul_smul]; erw [ f.map_smul], refl, },
 { dsimp, erw [mul_comm]; erw [ f.map_smul]; erw [ mul_smul], refl, },
end x y
...
localized_module.induction_on begin
 intros a b,
 rw [from_localized_module'_mk]; rw [ localized_module.smul'_mk]; rw [ from_localized_module'_mk],
 generalize_proofs h1, erw [f.map_smul]; erw [ h1.unit⁻¹.1.map_smul], refl,
end x
...
{ to_fun := from_localized_module' S f,
 ...
 map_smul' := λ r x, by rw [from_localized_module'_smul]; rw [ ring_hom.id_apply] }
...
begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ ←linear_map.map_smul]; erw [ module.End_algebra_map_is_unit_inv_apply_eq_iff'] at eq1,
 erw [localized_module.mk_eq]; erw [ ←is_localized_module.eq_iff_exists S f]; erw [ f.map_smul]; erw [ f.map_smul]; erw [ eq1],
 refl,
end
...
by { rw [from_localized_module_mk]; rw [ module.End_algebra_map_is_unit_inv_apply_eq_iff]; rw [ ←eq1], refl }⟩
...
begin
 ...
 erw [module.End_algebra_map_is_unit_inv_apply_eq_iff']; erw [ h2.some_spec],
end
...
begin
 ...
 erw [←is_localized_module.eq_iff_exists S f]; erw [ f.map_smul]; erw [ f.map_smul]; erw [ ←h1.some_spec]; erw [ ←mul_smul]; erw [ mul_comm]; erw [ mul_smul]; erw [ eq1],
end

lemma iso_symm_comp : (iso S f).symm.to_linear_map.comp f = localized_module.mk_linear_map S M :=
begin
 ext m, rw [linear_map.comp_apply]; rw [ localized_module.mk_linear_map_apply],
...
begin
 dunfold is_localized_module.lift,
 rw [localized_module.lift_unique S g h (l.comp (iso S f).to_linear_map)]; rw [ linear_map.comp_assoc]; rw [ show (iso S f).to_linear_map.comp (iso S f).symm.to_linear_map = linear_map.id]; rw [ from _]; rw [ linear_map.comp_id],
 { rw [linear_equiv.comp_to_linear_map_symm_eq]; rw [ linear_map.id_comp], },
 { rw [linear_map.comp_assoc]; rw [ ←hl], congr' 1, ext x,
 erw [from_localized_module_mk]; erw [ module.End_algebra_map_is_unit_inv_apply_eq_iff]; erw [ one_smul], },
end
...
by { rw [←lift_unique S f (k.comp f) map_unit j h]; rw [ lift_unique], refl }
...
by { delta mk', rw [← localized_module.smul'_mk]; rw [ linear_map.map_smul] }

lemma mk'_add_mk' (m₁ m₂ : M) (s₁ s₂ : S) :
 mk' f m₁ s₁ + mk' f m₂ s₂ = mk' f (s₂ • m₁ + s₁ • m₂) (s₁ * s₂) :=
by { delta mk', rw [← map_add]; rw [ localized_module.mk_add_mk] }

@[simp] lemma mk'_zero (s : S) :
 mk' f 0 s = 0 :=
by rw [← zero_smul R (0 : M)]; rw [ mk'_smul]; rw [ zero_smul]

variable (S)

@[simp] lemma mk'_one (m : M) :
 mk' f m (1 : S) = f m :=
by { delta mk', rw [from_localized_module_mk]; rw [ module.End_algebra_map_is_unit_inv_apply_eq_iff]; rw [ submonoid.coe_one]; rw [ one_smul] }

variable {S}

@[simp] lemma mk'_cancel (m : M) (s : S) :
 mk' f (s • m) s = f m :=
by { delta mk', rw [localized_module.mk_cancel]; rw [ ← mk'_one S f], refl }

@[simp] lemma mk'_cancel' (m : M) (s : S) :
 s • mk' f m s = f m :=
by rw [submonoid.smul_def]; rw [ ← mk'_smul]; rw [ ← submonoid.smul_def]; rw [ mk'_cancel]
...
by { rw [mk'_add_mk']; rw [ ← smul_add]; rw [ mk'_cancel_left] }

lemma mk'_eq_mk'_iff (m₁ m₂ : M) (s₁ s₂ : S) :
 mk' f m₁ s₁ = mk' f m₂ s₂ ↔ ∃ s : S, s • s₁ • m₂ = s • s₂ • m₁ :=
begin
 delta mk',
 rw [(from_localized_module.inj S f).eq_iff]; rw [ localized_module.mk_eq],
 simp_rw eq_comm
end
...
lemma mk'_neg {M M' : Type*} [add_comm_group M] [add_comm_group M'] [module R M]
 ...
 mk' f (-m) s = - mk' f m s :=
by { delta mk', rw [localized_module.mk_neg]; rw [ map_neg] }
...
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ mk'_add]; rw [ mk'_neg]
...
lemma mk'_sub_mk' {M M' : Type*} [add_comm_group M] [add_comm_group M'] [module R M]
 ...
 mk' f m₁ s₁ - mk' f m₂ s₂ = mk' f (s₂ • m₁ - s₁ • m₂) (s₁ * s₂) :=
by rw [sub_eq_add_neg]; rw [ ← mk'_neg]; rw [ mk'_add_mk']; rw [ smul_neg]; rw [ ← sub_eq_add_neg]
...
@[simp] lemma mk'_eq_iff {m : M} {s : S} {m' : M'} :
 mk' f m s = m' ↔ f m = s • m' :=
by rw [← smul_inj f s]; rw [ submonoid.smul_def]; rw [ ← mk'_smul]; rw [ ← submonoid.smul_def]; rw [ mk'_cancel]

@[simp] lemma mk'_eq_zero {m : M} (s : S) :
 mk' f m s = 0 ↔ f m = 0 :=
by rw [mk'_eq_iff]; rw [ smul_zero]
...
by rw [eq_comm]; rw [ mk'_eq_iff]; rw [ submonoid.smul_def]; rw [ localized_module.smul'_mk]; rw [ ← submonoid.smul_def]; rw [ localized_module.mk_cancel]; rw [ localized_module.mk_linear_map_apply]
...
begin
 ...
 rw [algebra.smul_def]; rw [ ← mul_assoc]; rw [ is_unit.mul_coe_inv]; rw [ one_mul] } },
 { exact h₂ },
