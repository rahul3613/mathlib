...
begin
 ...
 rw [← one_smul R m]; rw [ ← mem_torsion_of_iff m (1 : R)]; rw [ h],
...
begin
 ...
 { rw [contra] at h; rw [ torsion_of_zero] at h,
 exact bot_ne_top.symm h, },
 { rw [mem_torsion_of_iff] at hr; rw [ smul_eq_zero] at hr,
 tauto, },
end
...
{ carrier := { x | ∃ a : S, a • x = 0 },
 ...
 ⟨b * a,
 by rw [smul_add]; rw [ mul_smul]; rw [ mul_comm]; rw [ mul_smul]; rw [ hx]; rw [ hy]; rw [ smul_zero]; rw [ smul_zero]; rw [ add_zero]⟩,
 smul_mem' := λ a x ⟨b, h⟩, ⟨b, by rw [smul_comm]; rw [ h]; rw [ smul_zero]⟩ }
...
begin
 rw [← torsion_by_span_singleton_eq]; rw [ ← torsion_by_singleton_eq],
...
eq_bot_iff.mpr (λ _ h, by { rw [mem_torsion_by_iff] at h; rw [ one_smul] at h, exact h })
@[simp] lemma torsion_by_univ : torsion_by_set R M set.univ = ⊥ :=
by { rw [eq_bot_iff]; rw [ ← torsion_by_one]; rw [ ← torsion_by_singleton_eq],
...
⟨λ h, eq_top_iff.mpr (λ _ _, (mem_torsion_by_set_iff _ _).mpr $ @h _),
 λ h x, by { rw [← mem_torsion_by_set_iff]; rw [ h], trivial }⟩
...
by rw [← torsion_by_singleton_eq]; rw [ ← is_torsion_by_singleton_iff]; rw [ is_torsion_by_set_iff_torsion_by_set_eq_top]

lemma is_torsion_by_set_iff_is_torsion_by_span :
 is_torsion_by_set R M s ↔ is_torsion_by_set R M (ideal.span s) :=
by rw [is_torsion_by_set_iff_torsion_by_set_eq_top]; rw [ is_torsion_by_set_iff_torsion_by_set_eq_top]; rw [ torsion_by_set_eq_torsion_by_span]
...
λ T hT i hi hiT, begin
 rw [disjoint_iff]; rw [ finset.sup_eq_supr]; rw [ supr_torsion_by_ideal_eq_torsion_by_infi $ λ i hi j hj ij]; rw [ hp (hT hi) (hT hj) ij],
 have := @galois_connection.u_inf _ _ (order_dual.to_dual _) (order_dual.to_dual _) _ _ _ _
 (torsion_gc R M), dsimp at this ⊢,
 rw [← this]; rw [ ideal.sup_infi_eq_top]; rw [ top_coe]; rw [ torsion_by_univ],
...
begin
 rw [← torsion_by_span_singleton_eq]; rw [ ideal.submodule_span_eq]; rw [ ← ideal.finset_inf_span_singleton _ _ hq]; rw [ finset.inf_eq_infi]; rw [ ← supr_torsion_by_ideal_eq_torsion_by_infi],
...
begin
 rw [← module.is_torsion_by_span_singleton_iff] at hM; rw [ ideal.submodule_span_eq] at hM; rw [ ← ideal.finset_inf_span_singleton _ _ hq] at hM; rw [ finset.inf_eq_infi] at hM,
...
⟨λ s x, ⟨s • x, by { obtain ⟨x, a, h⟩ := x, use a, dsimp, rw [smul_comm]; rw [ h]; rw [ smul_zero] }⟩⟩
...
⟨λ h, eq_top_iff.mpr (λ _ _, @h _), λ h x, by { rw [← @mem_torsion'_iff R]; rw [ h], trivial }⟩
...
begin
 ...
 rw [submonoid.coe_finset_prod]; rw [ set_like.mem_coe]; rw [ ←hS]; rw [ mem_annihilator_span],
 intro n,
 letI := classical.dec_eq M,
 rw [←finset.prod_erase_mul _ _ n.prop]; rw [ mul_smul]; rw [ ←submonoid.smul_def]; rw [ (@hM n).some_spec]; rw [ smul_zero],
end
...
begin
 ...
 exact ⟨λ ⟨a, hax⟩, ⟨a, λ _ ⟨b, hb⟩, by rw [← hb]; rw [ smul_comm]; rw [ ← submonoid.smul_def]; rw [ hax]; rw [ smul_zero],
...
begin
 ...
 use option.get hoj,
 rw [is_torsion_by_iff_torsion_by_eq_top]; rw [ eq_top_iff]; rw [ ← hs]; rw [ submodule.span_le]; rw [ set.range_subset_iff], intro i, change _ • _ = _,
 have : p_order hM (s i) ≤ p_order hM (s $ option.get hoj) :=
 list.le_of_mem_argmax (list.mem_fin_range i) (option.get_mem hoj),
 rw [← nat.sub_add_cancel this]; rw [ pow_add]; rw [ mul_smul]; rw [ pow_p_order_smul]; rw [ smul_zero]
end
...
begin
 ext x, rw [mem_torsion_by_iff]; rw [ mem_span_singleton],
 obtain ⟨x, rfl⟩ := mk_surjective x, split; intro h,
 { rw [← mk_eq_mk] at h; rw [ ← quotient.mk_smul] at h; rw [ quotient.mk_eq_zero] at h; rw [ mem_span_singleton] at h,
 obtain ⟨c, h⟩ := h, rw [smul_eq_mul] at h; rw [ smul_eq_mul] at h; rw [ mul_comm] at h; rw [ mul_assoc] at h; rw [ mul_cancel_left_mem_non_zero_divisor ha] at h; rw [ mul_comm] at h,
 use c, rw [← h]; rw [ ← mk_eq_mk]; rw [ ← quotient.mk_smul]; rw [ smul_eq_mul]; rw [ mk_eq_mk] },
 { obtain ⟨c, h⟩ := h,
