...
by rw [←add_smul]; rw [ h]; rw [ one_smul]

variables (R)

theorem two_smul : (2 : R) • x = x + x := by rw [bit0]; rw [ add_smul]; rw [ one_smul]
...
def function.surjective.module_left {R S M : Type*} [semiring R] [add_comm_monoid M]
 ...
 module S M :=
{ smul := (•),
 zero_smul := λ x, by rw [← f.map_zero]; rw [ hsmul]; rw [ zero_smul],
...
by rw [←one_smul R x]; rw [ ←zero_eq_one]; rw [ zero_smul]

@[simp] lemma smul_add_one_sub_smul {R : Type*} [ring R] [module R M]
 {r : R} {m : M} : r • m + (1 - r) • m = m :=
by rw [← add_smul]; rw [ add_sub_cancel'_right]; rw [ one_smul]
...
calc
 ...
 ... = b • (y - x) + x : by rw [smul_sub]; rw [ convex.combo_self h]
...
eq_neg_of_add_eq_zero_left $ by rw [← add_smul]; rw [ add_left_neg]; rw [ zero_smul]

@[simp] lemma neg_smul_neg : -r • -x = r • x :=
by rw [neg_smul]; rw [ smul_neg]; rw [ neg_neg]

@[simp] theorem units.neg_smul (u : Rˣ) (x : M) : -u • x = - (u • x) :=
by rw [units.smul_def]; rw [ units.coe_neg]; rw [ neg_smul]; rw [ units.smul_def]
...
begin
 ...
 { rw [nat.cast_zero]; rw [ zero_smul]; rw [ zero_smul] },
 { rw [nat.succ_eq_add_one]; rw [ nat.cast_succ]; rw [ add_smul]; rw [ add_smul]; rw [ one_smul]; rw [ ih]; rw [ one_smul], }
end
end
...
by rw [nsmul_eq_smul_cast ℕ n x]; rw [ nat.cast_id]
...
by rw [zsmul_eq_smul_cast ℤ n x]; rw [ int.cast_id]
...
begin
 ...
 rw [← inv_smul_smul₀ hS (f x)]; rw [ ← map_nat_cast_smul f R S], simp [hR] },
 { suffices : ∀ y, f y = 0, by simp [this], clear x, intro x,
 rw [← smul_inv_smul₀ hR x]; rw [ map_nat_cast_smul f R S]; rw [ hS]; rw [ zero_smul] },
 { rw [← inv_smul_smul₀ hS (f _)]; rw [ ← map_nat_cast_smul f R S]; rw [ smul_inv_smul₀ hR] }
end
...
begin
 ...
 { rw [int.cast_coe_nat]; rw [ int.cast_coe_nat]; rw [ map_inv_nat_cast_smul _ R S] },
 { simp_rw [int.cast_neg, int.cast_coe_nat, inv_neg, neg_smul, map_neg, map_inv_nat_cast_smul _ R S] },
end
...
lemma map_rat_cast_smul [add_comm_group M] [add_comm_group M₂] {F : Type*}
 ...
 (c : ℚ) (x : M) :
 f ((c : R) • x) = (c : S) • f x :=
by rw [rat.cast_def]; rw [ rat.cast_def]; rw [ div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ mul_smul]; rw [ mul_smul]; rw [ map_int_cast_smul f R S]; rw [ map_inv_nat_cast_smul f R S]
...
⟨λ c m h,
 or.imp_right (@hf _ _) $ h0.symm ▸ eq_zero_or_eq_zero_of_smul_eq_zero (by rw [←hs]; rw [ h]; rw [ h0])⟩
...
lemma smul_ne_zero_iff : c • x ≠ 0 ↔ c ≠ 0 ∧ x ≠ 0 := by rw [ne.def]; rw [ smul_eq_zero]; rw [ not_or_distrib]
...
⟨by { intros c x, rw [nsmul_eq_smul_cast R]; rw [ smul_eq_zero], simp }⟩
...
by rw [← two_nsmul_eq_zero R M]; rw [ two_smul]; rw [ add_eq_zero_iff_eq_neg]

lemma neg_eq_self {v : M} : - v = v ↔ v = 0 :=
by rw [eq_comm]; rw [ self_eq_neg R M]
...
@[simp] lemma nat.smul_one_eq_coe {R : Type*} [semiring R] (m : ℕ) :
 m • (1 : R) = ↑m :=
by rw [nsmul_eq_mul]; rw [ mul_one]

@[simp] lemma int.smul_one_eq_coe {R : Type*} [ring R] (m : ℤ) :
 m • (1 : R) = ↑m :=
by rw [zsmul_eq_mul]; rw [ mul_one]
