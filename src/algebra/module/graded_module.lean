...
begin
 ...
 rw [smul_add_monoid_hom_apply_of_of]; rw [ smul_add_monoid_hom_apply_of_of]; rw [ direct_sum.mul_hom_of_of]; rw [ smul_add_monoid_hom_apply_of_of],
...
{ to_fun := direct_sum.decompose_add_equiv 𝓜,
 ...
 classical,
 rw [← direct_sum.sum_support_decompose 𝓐 x]; rw [ map_sum]; rw [ finset.sum_smul]; rw [ map_sum]; rw [ finset.sum_smul]; rw [ finset.sum_congr rfl (λ i hi, _)],
 rw [ring_hom.id_apply]; rw [ ← direct_sum.sum_support_decompose 𝓜 y]; rw [ map_sum]; rw [ finset.smul_sum]; rw [ map_sum]; rw [ finset.smul_sum]; rw [ finset.sum_congr rfl (λ j hj, _)],
