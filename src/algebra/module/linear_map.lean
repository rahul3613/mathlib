...
{ coe := λ f, (f : M → M₃),
 map_zero := λ f, show f 0 = 0, by { rw [← zero_smul R (0 : M)]; rw [ map_smulₛₗ], simp },
...
{ coe := λ f, (f : M → M₂),
 map_smul := λ f c x, by rw [map_smulₛₗ]; rw [ ring_hom.id_apply],
...
⟨λ fₗ c x, by rw [← smul_one_smul S c x]; rw [ ← smul_one_smul S c (fₗ x)]; rw [ map_smul]⟩
...
ext $ λ x, by rw [← mul_one x]; rw [ ← smul_eq_mul]; rw [ f.map_smulₛₗ]; rw [ g.map_smulₛₗ]; rw [ h]

theorem ext_ring_iff {σ : R →+* R} {f g : R →ₛₗ[σ] M} : f = g ↔ f 1 = g 1 :=
⟨λ h, h ▸ rfl, ext_ring⟩

@[ext] theorem ext_ring_op {σ : Rᵐᵒᵖ →+* S} {f g : R →ₛₗ[σ] M₃} (h : f 1 = g 1) : f = g :=
ext $ λ x, by rw [← one_mul x]; rw [ ← op_smul_eq_mul]; rw [ f.map_smulₛₗ]; rw [ g.map_smulₛₗ]; rw [ h]
...
{ to_fun := f ∘ g,
 ...
 map_smul' := λ r x, by rw [comp_app]; rw [ map_smulₛₗ]; rw [ map_smulₛₗ]; rw [ ring_hom_comp_triple.comp_apply] }
...
⟨λ h, ext $ λ x, hf $ by rw [← comp_apply]; rw [ h]; rw [ comp_apply], λ h, h ▸ rfl⟩
omit σ₁₃
...
by dsimp [left_inverse, function.right_inverse] at h₁ h₂; exact
 { to_fun := g,
 map_add' := λ x y, by { rw [← h₁ (g (x + y))]; rw [ ← h₁ (g x + g y)]; simp [h₂] },
 map_smul' := λ a b, by { rw [← h₁ (g (a • b))]; rw [ ← h₁ ((σ' a) • g b)], simp [h₂] } }
...
ext $ assume c, by rw [comp_apply]; rw [ zero_apply]; rw [ zero_apply]; rw [ g.map_zero]
