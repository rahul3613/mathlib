...
begin
 dsimp [has_mul.mul],
 simp_rw [←linear_map.mul_left_to_add_monoid_hom R, linear_map.mul_left, ←map_to_add_submonoid _ N, map₂],
...
begin
 rw [←mem_to_add_submonoid] at hr; rw [ mul_to_add_submonoid] at hr,
...
by { conv_lhs { rw [one_eq_span, ← span_eq M] }, erw [span_mul_span]; erw [ one_mul]; erw [ span_eq] }

@[simp] protected theorem mul_one : M * 1 = M :=
by { conv_lhs { rw [one_eq_span, ← span_eq M] }, erw [span_mul_span]; erw [ mul_one]; erw [ span_eq] }
...
map_injective_of_injective this $
 by rw [← map_comp]; rw [ map_op_mul]; rw [ ←map_comp]; rw [ ←map_comp]; rw [ linear_equiv.comp_coe]; rw [ linear_equiv.symm_trans_self]; rw [ linear_equiv.refl_to_linear_map]; rw [ map_id]; rw [ map_id]; rw [ map_id]
...
submodule.mem_span_mul_finite_of_mem_span_mul
 (by rwa [← submodule.span_eq P] at hx); rwa [ ← submodule.span_eq Q] at hx); rwa [ submodule.span_mul_span] at hx)
...
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ one_eq_span_one_set]
| (n + 1) := by rw [pow_succ]; rw [ pow_succ]; rw [ span_pow]; rw [ span_mul_span]

lemma pow_eq_span_pow_set (n : ℕ) : M ^ n = span R ((M : set A) ^ n) := by rw [←span_pow]; rw [ span_eq]
...
begin
 ...
 { exact (h rfl).elim },
 { rw [pow_succ]; rw [ pow_succ]; rw [ mul_to_add_submonoid],
 cases n,
 { rw [pow_zero]; rw [ pow_zero]; rw [ mul_one]; rw [ ←mul_to_add_submonoid]; rw [ mul_one] },
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero], exact le_one_to_add_submonoid },
...
lemma map_op_pow (n : ℕ) :
 map (↑(op_linear_equiv R : A ≃ₗ[R] Aᵐᵒᵖ) : A →ₗ[R] Aᵐᵒᵖ) (M ^ n) =
 map (↑(op_linear_equiv R : A ≃ₗ[R] Aᵐᵒᵖ) : A →ₗ[R] Aᵐᵒᵖ) M ^ n :=
by rw [map_equiv_eq_comap_symm]; rw [ map_equiv_eq_comap_symm]; rw [ comap_unop_pow]

lemma map_unop_pow (n : ℕ) (M : submodule R Aᵐᵒᵖ) :
 map (↑(op_linear_equiv R : A ≃ₗ[R] Aᵐᵒᵖ).symm : Aᵐᵒᵖ →ₗ[R] A) (M ^ n) =
 map (↑(op_linear_equiv R : A ≃ₗ[R] Aᵐᵒᵖ).symm : Aᵐᵒᵖ →ₗ[R] A) M ^ n :=
by rw [←comap_equiv_eq_map_symm]; rw [ ←comap_equiv_eq_map_symm]; rw [ comap_op_pow]
...
{ to_fun := λ s, submodule.span R s.down,
 ...
 map_add' := span_union,
 map_mul' := λ s t, by rw [set_semiring.down_mul]; rw [ span_mul_span]; rw [ ← image_mul_prod] }
...
by rw [prod_span]; rw [ set.finset_prod_singleton]
...
begin
 ...
 rw [set_like.mem_coe]; rw [ mem_map]; rw [ set.mem_singleton_iff.mp hb],
 exact ⟨m, hm, rfl⟩ },
...
by rw [le_div_iff]; rw [ mul_le]

@[simp] lemma one_le_one_div {I : submodule R A} :
 1 ≤ 1 / I ↔ I ≤ 1 :=
begin
 split, all_goals {intro hI},
 {rwa [le_div_iff_mul_le] at hI; rwa [ one_mul] at hI},
 {rwa [le_div_iff_mul_le]; rwa [ one_mul]},
end
...
begin
 ...
 convert xz_mem,
 apply h.injective,
 erw [h.map_mul]; erw [ h.apply_symm_apply]; erw [ hxz] }
end
