...
λ S T h, ext $ λ x, by rw [← mem_to_subsemiring]; rw [ ← mem_to_subsemiring]; rw [ h]
...
λ S T h, ext $ λ x, by rw [← mem_to_subring]; rw [ ← mem_to_subring]; rw [ h]
...
{ commutes' := λ c x, subtype.eq $ algebra.commutes _ _,
 ...
 rw [algebra.algebra_map_eq_smul_one]; rw [ ←smul_one_smul R x (1 : A)]; rw [ ←algebra.algebra_map_eq_smul_one],
...
by { ext, rw [set_like.mem_coe]; rw [ mem_range], refl }
...
{ carrier := {a | ϕ a = ψ a},
 ...
 by rw [set.mem_set_of_eq]; rw [ ϕ.map_add]; rw [ ψ.map_add]; rw [ hx]; rw [ hy],
 mul_mem' := λ x y (hx : ϕ x = ψ x) (hy : ϕ y = ψ y),
 by rw [set.mem_set_of_eq]; rw [ ϕ.map_mul]; rw [ ψ.map_mul]; rw [ hx]; rw [ hy],
 algebra_map_mem' := λ x,
 by rw [set.mem_set_of_eq]; rw [ alg_hom.commutes]; rw [ alg_hom.commutes] }
...
by { rw [← this]; rw [ ←set_like.mem_coe]; rw [ alg_hom.coe_range], refl },
...
instance _root_.alg_equiv.subsingleton_right [subsingleton (subalgebra R B)] :
 subsingleton (A ≃ₐ[R] B) :=
⟨λ f g, by rw [← f.symm_symm]; rw [ subsingleton.elim f.symm g.symm]; rw [ g.symm_symm]⟩
...
by rw [algebra_map_eq]; rw [ algebra.id.map_eq_id]; rw [ ring_hom.id_comp]; rw [ ← to_subsemiring_subtype]; rw [ subsemiring.srange_subtype]

@[simp] lemma range_algebra_map {R A : Type*} [comm_ring R] [comm_ring A]
 [algebra R A] (S : subalgebra R A) :
 (algebra_map S A).range = S.to_subring :=
by rw [algebra_map_eq]; rw [ algebra.id.map_eq_id]; rw [ ring_hom.id_comp]; rw [ ← to_subring_subtype]; rw [ subring.range_subtype]
...
begin
 ...
 have : ideal.span (s' '' ι')= ⊤,
 { rw [ideal.eq_top_iff_one]; rw [ ideal.span]; rw [ finsupp.mem_span_iff_total],
 ...
 rw [finsupp.total_map_domain]; rw [ finsupp.total_apply]; rw [ finsupp.sum_fintype]; rw [ map_sum]; rw [ submodule.subtype_apply]; rw [ subalgebra.coe_one],
...
{ algebra_map_mem' := λ i, int.induction_on i (by simpa using S.zero_mem)
 ...
 (λ i ih, show ((-i - 1 : ℤ) : R) ∈ S, by { rw [int.cast_sub]; rw [ int.cast_one],
