...
by rw [algebra.algebra_map_eq_smul_one]; rw [ smul_assoc]; rw [ one_smul]
...
by rw [algebra_map_eq R S A]; rw [ ring_hom.comp_apply]
...
by rw [algebra_map_apply R S A r]; rw [ f.commutes]; rw [ ← algebra_map_apply R S B]
...
{ commutes' := λ r, by { rw [algebra_map_apply R S A]; rw [ algebra_map_apply R S B],
...
span_induction hx (λ x hx, let ⟨p, q, hp, hq, hpq⟩ := set.mem_smul.1 hx in
 by { rw [← hpq]; rw [ smul_smul], exact smul_mem_span_smul_of_mem (hs.symm ▸ mem_top) hq })
 (by { rw smul_zero, exact zero_mem _ })
 (λ x y ihx ihy, by { rw smul_add, exact add_mem ihx ihy })
 (λ c x hx, smul_comm c k x ▸ smul_mem _ _ hx)
...
by { rw [span_algebra_map_image_of_tower]; rw [ mem_map], exact ⟨x, hx, rfl⟩ }
