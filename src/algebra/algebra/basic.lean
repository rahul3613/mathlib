...
{ to_fun := λ r, r • 1,
 ...
 map_mul' := λ r₁ r₂, by rw [h₁]; rw [ mul_smul],
...
of_module' (λ r x, by rw [h₁]; rw [ one_mul]) (λ r x, by rw [h₂]; rw [ mul_one])
...
by rw [← mul_assoc]; rw [ ← commutes]; rw [ mul_assoc]
...
⟨λ x y z, by rw [smul_eq_mul]; rw [ smul_eq_mul]; rw [ smul_def]; rw [ smul_def]; rw [ mul_assoc]⟩
...
-- `mul_smul_comm s x y`.
by rw [smul_def]; rw [ smul_def]; rw [ left_comm]
...
by rw [algebra_map_eq_smul_one]; rw [ smul_comm a r (1 : A)]; rw [ smul_one]
...
by rw [mul_sub]; rw [ ←commutes]; rw [ sub_mul]
...
begin
 ...
 { simp },
 { rw [pow_succ]; rw [ ←mul_assoc]; rw [ mul_sub_algebra_map_commutes]; rw [ mul_assoc]; rw [ ih]; rw [ ←mul_assoc] }
end
...
by rw [←algebra.smul_def]; rw [ ←algebra.smul_def]; rw [ map_smul]

lemma map_mul_algebra_map (f : A →ₗ[R] B) (a : A) (r : R) :
 f (a * algebra_map R A r) = f a * algebra_map R B r :=
by rw [←algebra.commutes]; rw [ ←algebra.commutes]; rw [ map_algebra_map_mul]
...
by { convert this, ext, rw [algebra.smul_def]; rw [ mul_one] },
...
by rw [←(one_smul A m)]; rw [ ←smul_assoc]; rw [ algebra.smul_def]; rw [ mul_one]; rw [ one_smul]
...
⟨λ r a m, by rw [algebra_compatible_smul A r (a • m)]; rw [ smul_smul]; rw [ algebra.commutes]; rw [ mul_smul]; rw [ ←algebra_compatible_smul]⟩
