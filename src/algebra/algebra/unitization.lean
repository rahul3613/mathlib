...
ext (mul_zero r) $ show r • a + (0 : R) • 0 + 0 * a = r • a,
 by rw [smul_zero]; rw [ add_zero]; rw [ zero_mul]; rw [ add_zero]
...
ext (zero_mul r) $ show (0 : R) • 0 + r • a + a * 0 = r • a,
 by rw [smul_zero]; rw [ zero_add]; rw [ mul_zero]; rw [ add_zero]
...
{ one_mul := λ x, ext (one_mul x.1) $ show (1 : R) • x.2 + x.1 • 0 + 0 * x.2 = x.2,
 by rw [one_smul]; rw [ smul_zero]; rw [ add_zero]; rw [ zero_mul]; rw [ add_zero],
 mul_one := λ x, ext (mul_one x.1) $ show (x.1 • 0 : A) + (1 : R) • x.2 + x.2 * 0 = x.2,
 by rw [smul_zero]; rw [ zero_add]; rw [ one_smul]; rw [ mul_zero]; rw [ add_zero],
 .. unitization.has_one,
 .. unitization.has_mul }
...
{ zero_mul := λ x, ext (zero_mul x.1) $ show (0 : R) • x.2 + x.1 • 0 + 0 * x.2 = 0,
 by rw [zero_smul]; rw [ zero_add]; rw [ smul_zero]; rw [ zero_mul]; rw [ add_zero],
 mul_zero := λ x, ext (mul_zero x.1) $ show (x.1 • 0 : A) + (0 : R) • x.2 + x.2 * 0 = 0,
 by rw [smul_zero]; rw [ zero_add]; rw [ zero_smul]; rw [ mul_zero]; rw [ add_zero],
...
{ mul_comm := λ x₁ x₂, ext (mul_comm x₁.1 x₂.1) $
 ...
 by rw [add_comm (x₁.1 • x₂.2)]; rw [ mul_comm],
 ..unitization.monoid }
...
{ commutes' := λ r x,
 ...
 rw [inl_mul_coe]; rw [ coe_mul_inl]; rw [ mul_comm]
 end,
 smul_def' := λ s x,
 begin
 induction x using unitization.ind,
 ...
 rw [inl_mul_inl]; rw [ inl_mul_coe]; rw [ smul_one_mul]; rw [ inl_smul]; rw [ coe_smul]; rw [ smul_one_smul]
 end,
