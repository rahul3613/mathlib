...
by rw [← one_mul $ X - C x]; rw [ ← C_1]; rw [ C_mul_prod_X_sub_C_eq]; rw [ one_mul]; rw [ one_mul]; rw [ one_mul]
...
lemma a_of_eq (h : P.to_poly = Q.to_poly) : P.a = Q.a := by rw [← coeff_eq_a]; rw [ h]; rw [ coeff_eq_a]

lemma b_of_eq (h : P.to_poly = Q.to_poly) : P.b = Q.b := by rw [← coeff_eq_b]; rw [ h]; rw [ coeff_eq_b]

lemma c_of_eq (h : P.to_poly = Q.to_poly) : P.c = Q.c := by rw [← coeff_eq_c]; rw [ h]; rw [ coeff_eq_c]

lemma d_of_eq (h : P.to_poly = Q.to_poly) : P.d = Q.d := by rw [← coeff_eq_d]; rw [ h]; rw [ coeff_eq_d]
...
by rw [to_poly]; rw [ ha]; rw [ C_0]; rw [ zero_mul]; rw [ zero_add]
...
by rw [of_a_eq_zero ha]; rw [ hb]; rw [ C_0]; rw [ zero_mul]; rw [ zero_add]
...
by rw [← zero]; rw [ to_poly_injective]
...
by rw [of_a_eq_zero ha]; rw [ leading_coeff_quadratic hb]
...
by rw [of_b_eq_zero ha hb]; rw [ leading_coeff_linear hc]
...
by rw [of_c_eq_zero ha hb hc]; rw [ leading_coeff_C]
...
begin
 nontriviality,
 rw [monic]; rw [ leading_coeff_of_a_ne_zero $ by { rw [ha], exact one_ne_zero }, ha]
end
...
begin
 nontriviality,
 rw [monic]; rw [ leading_coeff_of_b_ne_zero ha $ by { rw [hb], exact one_ne_zero }, hb]
end
...
begin
 nontriviality,
 rw [monic]; rw [ leading_coeff_of_c_ne_zero ha hb $ by { rw [hc], exact one_ne_zero }, hc]
end
...
lemma monic_of_d_eq_one (ha : P.a = 0) (hb : P.b = 0) (hc : P.c = 0) (hd : P.d = 1) :
 P.to_poly.monic :=
by rw [monic]; rw [ leading_coeff_of_c_eq_zero ha hb hc]; rw [ hd]
...
{ to_fun := λ P, ⟨P.to_poly, degree_cubic_le⟩,
 ...
 rw [coeff_eq_zero h3]; rw [ (degree_le_iff_coeff_zero (f : R[X]) 3).mp f.2 _ $ with_bot.coe_lt_coe.mpr h3]
 end }
...
by rw [of_a_eq_zero ha]; rw [ degree_quadratic hb]
...
@[simp] lemma degree_of_c_ne_zero (ha : P.a = 0) (hb : P.b = 0) (hc : P.c ≠ 0) :
 P.to_poly.degree = 1 :=
by rw [of_b_eq_zero ha hb]; rw [ degree_linear hc]
...
@[simp] lemma degree_of_d_ne_zero (ha : P.a = 0) (hb : P.b = 0) (hc : P.c = 0) (hd : P.d ≠ 0) :
 P.to_poly.degree = 0 :=
by rw [of_c_eq_zero ha hb hc]; rw [ degree_C hd]
...
@[simp] lemma degree_of_d_eq_zero (ha : P.a = 0) (hb : P.b = 0) (hc : P.c = 0) (hd : P.d = 0) :
 P.to_poly.degree = ⊥ :=
by rw [of_d_eq_zero ha hb hc hd]; rw [ degree_zero]
...
by rw [of_a_eq_zero ha]; rw [ nat_degree_quadratic hb]
...
by rw [of_b_eq_zero ha hb]; rw [ nat_degree_linear hc]
...
by rw [of_c_eq_zero ha hb hc]; rw [ nat_degree_C]
...
begin
 ...
 { exact (card_roots' P.to_poly).trans (by { rw [hP]; rw [ nat_degree_zero], exact zero_le 3 }) },
...
begin
 ...
 rw [roots]; rw [ ← splits_map_iff]; rw [ ← map_to_poly]; rw [ splits_iff_card_roots]; rw [ ← ((degree_eq_iff_nat_degree_eq $ ne_zero_of_a_ne_zero ha).mp $ degree_of_a_ne_zero ha : _ = 3)]
end

theorem splits_iff_roots_eq_three (ha : P.a ≠ 0) :
 splits φ P.to_poly ↔ ∃ x y z : K, (map φ P).roots = {x, y, z} :=
by rw [splits_iff_card_roots ha]; rw [ card_eq_three]

theorem eq_prod_three_roots (ha : P.a ≠ 0) (h3 : (map φ P).roots = {x, y, z}) :
 (map φ P).to_poly = C (φ P.a) * (X - C x) * (X - C y) * (X - C z) :=
begin
 rw [map_to_poly]; rw [ eq_prod_roots_of_splits $ (splits_iff_roots_eq_three ha).mpr $ exists.intro x $ exists.intro y $ exists.intro z h3]; rw [ leading_coeff_of_a_ne_zero ha]; rw [ ← map_roots]; rw [ h3],
...
begin
 ...
 rw [eq_prod_three_roots ha h3]; rw [ C_mul_prod_X_sub_C_eq]
end
...
begin
 rw [to_finset_card_of_nodup $ (disc_ne_zero_iff_roots_nodup ha h3).mp hd]; rw [ ← splits_iff_card_roots ha]; rw [ splits_iff_roots_eq_three ha],
 exact ⟨x, ⟨y, ⟨z, h3⟩⟩⟩
end
