...
{ neg_zero := by rw [←zero_mul (0 : α)]; rw [ ←neg_mul]; rw [ mul_zero]; rw [ mul_zero],
...
{ neg := has_neg.neg,
 neg_neg := neg_neg,
 neg_mul := λ a b, eq_neg_of_add_eq_zero_left $ by rw [←right_distrib]; rw [ add_left_neg]; rw [ zero_mul],
 mul_neg := λ a b, eq_neg_of_add_eq_zero_left $ by rw [←left_distrib]; rw [ add_left_neg]; rw [ mul_zero] }
...
{ zero_mul := λ a, add_left_cancel $ show 0 * a + 0 * a = 0 * a + 0,
 by rw [← add_mul]; rw [ zero_add]; rw [ add_zero],
 mul_zero := λ a, add_left_cancel $ show a * 0 + a * 0 = a * 0 + 0,
 by rw [← mul_add]; rw [ add_zero]; rw [ add_zero],
 ..‹ring α› }
