...
protected def function.injective.has_distrib_neg [has_neg β] [has_mul β] (f : β → α)
 ...
 has_distrib_neg β :=
{ neg_mul := λ x y, hf $ by erw [neg]; erw [ mul]; erw [ neg]; erw [ neg_mul]; erw [ mul],
 mul_neg := λ x y, hf $ by erw [neg]; erw [ mul]; erw [ neg]; erw [ mul_neg]; erw [ mul],
...
protected def function.surjective.has_distrib_neg [has_neg β] [has_mul β] (f : α → β)
 ...
 has_distrib_neg β :=
{ neg_mul := hf.forall₂.2 $ λ x y, by { erw [←neg]; erw [ ← mul]; erw [ neg_mul]; erw [ neg]; erw [ mul], refl },
 mul_neg := hf.forall₂.2 $ λ x y, by { erw [←neg]; erw [ ← mul]; erw [ mul_neg]; erw [ neg]; erw [ mul], refl },
 ..hf.has_involutive_neg _ neg, ..‹has_mul β› }
