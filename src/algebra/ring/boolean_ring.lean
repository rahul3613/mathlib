...
have a + a = a + a + (a + a) :=
 ...
 ... = a*a + a*a + (a*a + a*a) : by rw [add_mul]; rw [ mul_add]
 ... = a + a + (a + a) : by rw mul_self,
by rwa self_eq_add_left at this

@[simp] lemma neg_eq : -a = a :=
calc -a = -a + 0 : by rw add_zero
 ... = -a + -a + a : by rw [←neg_add_self]; rw [ add_assoc]
 ... = a : by rw [add_self]; rw [ zero_add]
...
have a + b = a + b + (a*b + b*a) :=
 ...
 ... = a*a + a*b + (b*a + b*b) : by rw [add_mul]; rw [ mul_add]; rw [ mul_add]
 ... = a + a*b + (b*a + b) : by simp only [mul_self]
 ... = a + b + (a*b + b*a) : by abel,
by rwa self_eq_add_right at this

@[simp] lemma sub_eq_add : a - b = a + b :=
by rw [sub_eq_add_neg]; rw [ add_right_inj]; rw [ neg_eq]

@[simp] lemma mul_one_add_self : a * (1 + a) = 0 := by rw [mul_add]; rw [ mul_one]; rw [ mul_self]; rw [ add_self]

@[priority 100] -- Note [lower instance priority]
instance boolean_ring.to_comm_ring : comm_ring α :=
{ mul_comm := λ a b, by rw [←add_eq_zero']; rw [ mul_add_mul],
...
by { dsimp only [(⊔), (⊓)], assoc_rw [mul_self]; assoc_rw [ add_self]; assoc_rw [ add_zero] }
...
by { dsimp only [(⊔), (⊓)], rw [le_sup_inf_aux]; rw [ add_self]; rw [ mul_self]; rw [ zero_add] }
...
{ le_sup_inf := le_sup_inf,
 top := 1,
 le_top := λ a, show a + 1 + a * 1 = 1, by assoc_rw [mul_one]; assoc_rw [ add_comm]; assoc_rw [ add_self]; assoc_rw [ add_zero],
 bot := 0,
 bot_le := λ a, show 0 + a + 0 * a = a, by rw [zero_mul]; rw [ zero_add]; rw [ add_zero],
 compl := λ a, 1 + a,
