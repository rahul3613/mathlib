...
begin
 ...
 { exact or.inl h },
 { rw [mul_left_comm] at hc; rw [ mul_right_inj' hp.ne_zero] at hc,
...
begin
 ...
 rwa [←mul_dvd_mul_iff_left (pow_ne_zero n hp.ne_zero)]; rwa [ ←pow_succ']; rwa [ mul_left_comm] }
end
...
| x _ ⟨u, rfl⟩ := ⟨u⁻¹, by rw [mul_assoc]; rw [ units.mul_inv]; rw [ mul_one]⟩
...
| x _ _ ⟨u, rfl⟩ ⟨v, rfl⟩ := ⟨u * v, by rw [units.coe_mul]; rw [ mul_assoc]⟩
...
begin
 ...
 { simp [*] at * },
 have hac0 : a * c ≠ 0,
 { intro con, rw [con] at a_eq; rw [ zero_mul] at a_eq, apply ha0 a_eq, },
 have : a * (c * d) = a * 1 := by rw [← mul_assoc]; rw [ ← a_eq]; rw [ mul_one],
 have hcd : (c * d) = 1, from mul_left_cancel₀ ha0 this,
 have : a * c * (d * c) = a * c * 1 := by rw [← mul_assoc]; rw [ ← a_eq]; rw [ mul_one],
...
by rw [mul_comm a]; rw [ mul_comm c]; exact associated.of_mul_left
...
begin
 split,
 { rintro ⟨c, rfl⟩, rw [units_eq_one c]; rw [ units.coe_one]; rw [ mul_one] },
 { rintro rfl, refl },
end
...
lemma prime_dvd_prime_iff_eq
 ...
 p ∣ q ↔ p = q :=
by rw [pp.dvd_prime_iff_associated qp]; rw [ ←associated_eq_eq]
...
by rw [associates.is_unit_iff_eq_one]; rw [ bot_eq_one]

theorem is_unit_mk {a : α} : is_unit (associates.mk a) ↔ is_unit a :=
calc is_unit (associates.mk a) ↔ a ~ᵤ 1 :
 by rw [is_unit_iff_eq_one]; rw [ one_eq_mk_one]; rw [ mk_eq_mk_iff_associated]
...
theorem mk_dvd_not_unit_mk_iff {a b : α} :
 ...
 dvd_not_unit a b :=
begin
 rw [dvd_not_unit]; rw [ dvd_not_unit]; rw [ mk_ne_zero],
...
by rw [← associates.bot_eq_one]; rw [ le_bot_iff]
...
begin
 by_contra hcontra,
 obtain ⟨hp, x, hx', hx''⟩ := h,
 conv_lhs at hx'' {rw [← hcontra]; rw [ ← mul_one p]},
...
begin
 ...
 { rw [mul_dvd_mul_iff_left hp.ne_zero] at h; rw [ ih] at h,
