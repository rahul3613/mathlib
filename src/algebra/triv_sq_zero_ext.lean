...
@[simp] lemma inl_mul [monoid R] [add_monoid M]
 ...
 (inl (r₁ * r₂) : tsze R M) = inl r₁ * inl r₂ :=
ext rfl $ show (0 : M) = r₁ • 0 + op r₂ • 0, by rw [smul_zero]; rw [ zero_add]; rw [ smul_zero]
...
{ one_mul := λ x, ext (one_mul x.1) $ show (1 : R) • x.2 + op x.1 • 0 = x.2,
 by rw [one_smul]; rw [ smul_zero]; rw [ add_zero],
 mul_one := λ x, ext (mul_one x.1) $ show (x.1 • 0 : M) + (1 : Rᵐᵒᵖ) • x.2 = x.2,
 by rw [smul_zero]; rw [ zero_add]; rw [ one_smul],
...
{ zero_mul := λ x, ext (zero_mul x.1) $ show (0 : R) • x.2 + op x.1 • 0 = 0,
 by rw [zero_smul]; rw [ zero_add]; rw [ smul_zero],
 mul_zero := λ x, ext (mul_zero x.1) $ show (x.1 • 0 : M) + (0 : Rᵐᵒᵖ) • x.2 = 0,
 by rw [smul_zero]; rw [ zero_add]; rw [ zero_smul],
...
begin
 ...
 { simp },
 { rw [pow_succ']; rw [ mul_opposite.op_mul]; rw [ mul_smul]; rw [ mul_smul]; rw [ ←h]; rw [ smul_comm (_ : R) (op x.fst) x.snd]; rw [ ih] } },
 simp_rw [snd_pow_eq_sum, this, smul_smul, ←pow_add],
 cases n,
 { rw [nat.pred_zero]; rw [ pow_zero]; rw [ list.range_zero]; rw [ zero_smul]; rw [ list.map_nil]; rw [ list.sum_nil] },
...
{ mul_assoc := λ x y z, ext (mul_assoc x.1 y.1 z.1) $
 ...
 simp_rw [nat.pred_succ],
 rw [list.range_succ]; rw [ list.map_append]; rw [ list.sum_append]; rw [ list.map_singleton]; rw [ list.sum_singleton]; rw [ nat.sub_self]; rw [ pow_zero]; rw [ one_smul]; rw [ list.smul_sum]; rw [ list.map_map]; rw [ function.comp]; rw [ fst_pow],
...
begin
 ...
 { simp },
 { rw [list.enum_cons]; rw [ ←list.map_fst_add_enum_eq_enum_from],
 simp_rw [list.map_cons, list.map_map, function.comp, prod.map_snd, prod.map_fst, id, list.take_zero, list.take_cons, list.prod_nil, list.prod_cons, snd_mul, one_smul, list.drop, mul_smul, list.sum_cons, fst_list_prod, ih, list.smul_sum, list.map_map],
 exact add_comm _ _, }
end
...
{ mul_comm := λ x₁ x₂, ext (mul_comm x₁.1 x₂.1) $
 ...
 by rw [op_smul_eq_smul]; rw [ op_smul_eq_smul]; rw [ add_comm]
 .. triv_sq_zero_ext.monoid }
...
{ smul := (•),
 ...
 rw [algebra.algebra_map_eq_smul_one]; rw [ mul_opposite.op_smul]; rw [ mul_opposite.op_one]; rw [ smul_assoc]; rw [ one_smul]; rw [ smul_assoc]; rw [ one_smul],
 end,
 smul_def' := λ r x, ext (algebra.smul_def _ _) $
 show r • x.2 = algebra_map S R r • x.2 + op x.1 • 0,
 by rw [smul_zero]; rw [ add_zero]; rw [ algebra_map_smul],
...
alg_hom.of_linear_map
 ...
 (show algebra_map R' _ 1 + f (0 : M) = 1, by rw [map_zero]; rw [ map_one]; rw [ add_zero])
 ...
 op_smul_eq_smul],
 rw [←ring_hom.map_mul]; rw [ linear_map.map_add]; rw [ ←algebra.commutes _ (f _)]; rw [ ←algebra.smul_def]; rw [ ←algebra.smul_def]; rw [ add_right_comm]; rw [ add_assoc]; rw [ linear_map.map_smul]; rw [ linear_map.map_smul],
 end)

@[simp] lemma lift_aux_apply_inr (f : M →ₗ[R'] A) (hf : ∀ x y, f x * f y = 0) (m : M) :
 lift_aux f hf (inr m) = f m :=
show algebra_map R' A 0 + f m = f m, by rw [ring_hom.map_zero]; rw [ zero_add]
