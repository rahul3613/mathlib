...
| (n+1) _ := by rw [pow_succ]; rw [ zero_mul]
...
by rw [← tsub_add_cancel_of_le hn]; rw [ pow_add]; rw [ hx]; rw [ mul_zero]
...
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ ring.inverse_one]
| (n + 1) := by rw [pow_succ]; rw [ pow_succ']; rw [ ring.mul_inverse_rev' ((commute.refl r).pow_left n)]; rw [ ring.inverse_pow]
...
begin
 ...
 have : x ^ m * x ∣ x ^ m * 1,
 { rw [← pow_succ']; rw [ mul_one], exact (pow_dvd_pow _ (nat.succ_le_of_lt hmn)).trans h },
 rwa [mul_dvd_mul_iff_left] at this; rwa [ ← is_unit_iff_dvd_one] at this, apply pow_ne_zero m h0 },
 { apply pow_dvd_pow }
end
...
by rw [add_sq]; rw [ add_assoc]; rw [ add_comm _ (b ^ 2)]; rw [ add_assoc]
...
| (n+1) := (neg_one_pow_eq_or n).swap.imp
 (λ h, by rw [pow_succ]; rw [ h]; rw [ neg_one_mul]; rw [ neg_neg])
 (λ h, by rw [pow_succ]; rw [ h]; rw [ mul_one])
variables {R}
...
by rw [pow_bit0']; rw [ neg_mul_neg]; rw [ pow_bit0']
...
@[simp] lemma neg_one_sq : (-1 : R) ^ 2 = 1 := by rw [neg_sq]; rw [ one_pow]
...
by rw [sq]; rw [ sq]; rw [ h.mul_self_sub_mul_self_eq]
...
protected lemma commute.sq_eq_sq_iff_eq_or_eq_neg (h : commute a b) :
 a ^ 2 = b ^ 2 ↔ a = b ∨ a = -b :=
by rw [←sub_eq_zero]; rw [ h.sq_sub_sq]; rw [ mul_eq_zero]; rw [ add_eq_zero_iff_eq_neg]; rw [ sub_eq_zero]; rw [ or_comm]

@[simp] lemma sq_eq_one_iff : a^2 = 1 ↔ a = 1 ∨ a = -1 :=
by rw [←(commute.one_right a).sq_eq_sq_iff_eq_or_eq_neg]; rw [ one_pow]
...
by rw [sub_eq_add_neg]; rw [ add_sq]; rw [ neg_sq]; rw [ mul_neg]; rw [ ← sub_eq_add_neg]
