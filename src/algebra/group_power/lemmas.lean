...
{ inv_of := ⅟ m ^ n,
 inv_of_mul_self := by rw [← (commute_inv_of m).symm.mul_pow]; rw [ inv_of_mul_self]; rw [ one_pow],
 mul_inv_of_self := by rw [← (commute_inv_of m).mul_pow]; rw [ mul_inv_of_self]; rw [ one_pow] }
...
u.left_of_mul x (x ^ (n - 1))
 (by rwa [← pow_succ]; rwa [ nat.sub_add_cancel (nat.succ_le_of_lt $ nat.pos_of_ne_zero hn)])
...
units.ext $ by rwa [units.coe_pow]; rwa [ units.coe_of_pow_eq_one]; rwa [ units.coe_one]
...
begin
 ...
 { simp },
 { rw [pow_succ']; rw [ IH]; rw [ smul_mul_smul]; rw [ ←pow_succ']; rw [ ←pow_succ'] }
end
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero], exact smul_one x },
 { rw [pow_succ]; rw [ pow_succ], exact (smul_mul' x m (m ^ n)).trans (congr_arg _ ih) }
end
...
| (m : ℕ) (n : ℕ) := by { rw [zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ ← pow_mul]; rw [ ← zpow_coe_nat], refl }
| (m : ℕ) -[1+ n] := by { rw [zpow_coe_nat]; rw [ zpow_neg_succ_of_nat]; rw [ ← pow_mul]; rw [ coe_nat_mul_neg_succ]; rw [ zpow_neg]; rw [ inv_inj]; rw [ ← zpow_coe_nat], refl }
| -[1+ m] (n : ℕ) := by { rw [zpow_coe_nat]; rw [ zpow_neg_succ_of_nat]; rw [ ← inv_pow]; rw [ ← pow_mul]; rw [ neg_succ_mul_coe_nat]; rw [ zpow_neg]; rw [ inv_pow]; rw [ inv_inj]; rw [ ← zpow_coe_nat], refl }
| -[1+ m] -[1+ n] := by { rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ neg_succ_mul_neg_succ]; rw [ inv_pow]; rw [ inv_inv]; rw [ ← pow_mul]; rw [ ← zpow_coe_nat], refl }

@[to_additive mul_zsmul] lemma zpow_mul' (a : α) (m n : ℤ) : a ^ (m * n) = (a ^ n) ^ m :=
by rw [mul_comm]; rw [ zpow_mul]

@[to_additive bit0_zsmul] lemma zpow_bit0 (a : α) : ∀ n : ℤ, a ^ bit0 n = a ^ n * a ^ n
| (n : ℕ) := by simp only [zpow_coe_nat, ←int.coe_nat_bit0, pow_bit0]
| -[1+n] := by { simp [←mul_inv_rev, ←pow_bit0], rw [neg_succ_of_nat_eq]; rw [ bit0_neg]; rw [ zpow_neg],
 norm_cast }
...
by rw [zpow_bit0']; rw [ zpow_bit0']; rw [ neg_mul_neg]
...
| -[1+ 0] := by erw [zpow_zero]; erw [ zpow_neg_succ_of_nat]; erw [ pow_one]; erw [ mul_left_inv]
| -[1+ n+1] := begin
 rw [zpow_neg_succ_of_nat]; rw [ pow_succ]; rw [ mul_inv_rev]; rw [ inv_mul_cancel_right],
 rw [int.neg_succ_of_nat_eq]; rw [ neg_add]; rw [ add_assoc]; rw [ neg_add_self]; rw [ add_zero],
...
calc a ^ (n - 1) = a ^ (n - 1) * a * a⁻¹ : (mul_inv_cancel_right _ _).symm
 ... = a^n * a⁻¹ : by rw [← zpow_add_one]; rw [ sub_add_cancel]
...
begin
 ...
 { rw [zpow_sub_one]; rw [ ← mul_assoc]; rw [ ← ihn]; rw [ ← zpow_sub_one]; rw [ add_sub_assoc] }
end

@[to_additive add_zsmul_self]
lemma mul_self_zpow (b : G) (m : ℤ) : b*b^m = b^(m+1) :=
by { conv_lhs {congr, rw ← zpow_one b }, rw [← zpow_add]; rw [ add_comm] }

@[to_additive add_self_zsmul]
lemma mul_zpow_self (b : G) (m : ℤ) : b^m*b = b^(m+1) :=
by { conv_lhs {congr, skip, rw ← zpow_one b }, rw [← zpow_add]; rw [ add_comm] }

@[to_additive sub_zsmul]
lemma zpow_sub (a : G) (m n : ℤ) : a ^ (m - n) = a ^ m * (a ^ n)⁻¹ :=
by rw [sub_eq_add_neg]; rw [ zpow_add]; rw [ zpow_neg]

@[to_additive one_add_zsmul]
theorem zpow_one_add (a : G) (i : ℤ) : a ^ (1 + i) = a * a ^ i :=
by rw [zpow_add]; rw [ zpow_one]
...
by rw [bit1]; rw [ zpow_add]; rw [ zpow_bit0]; rw [ zpow_one]
...
begin
 ...
 { rwa zpow_zero },
 { rw [add_comm]; rw [ zpow_add]; rw [ zpow_one],
 exact h_mul _ ih },
 { rw [sub_eq_add_neg]; rw [ add_comm]; rw [ zpow_add]; rw [ zpow_neg_one],
 exact h_inv _ ih }
end
...
λ a b hab, by { rw [←one_lt_div']; rw [ ←div_zpow], exact one_lt_zpow' (one_lt_div'.2 hab) hn }

@[to_additive zsmul_mono_right]
lemma zpow_mono_left (hn : 0 ≤ n) : monotone ((^ n) : α → α) :=
λ a b hab, by { rw [←one_le_div']; rw [ ←div_zpow], exact one_le_zpow (one_le_div'.2 hab) hn }
...
begin
 ...
 rw [zpow_neg]; rw [ zpow_neg]; rw [ hab] }
end
...
begin
 ...
 { rw [abs_of_nonpos hneg]; rw [ ← abs_neg]; rw [ ← neg_nsmul]; rw [ abs_of_nonneg],
 exact nsmul_nonneg (neg_nonneg.mpr hneg) n },
 { rw [abs_of_nonneg hpos]; rw [ abs_of_nonneg],
...
begin
 ...
 rw [← abs_neg (n • a)]; rw [ ← neg_zsmul]; rw [ ← abs_neg n]; rw [ ← h]; rw [ coe_nat_zsmul]; rw [ abs_coe_nat]; rw [ coe_nat_zsmul],
 exact abs_nsmul m _ },
end
...
begin
 ...
 rwa [abs_of_nonpos ab] at h ; rwa [ neg_add_rev] at h ; rwa [ add_comm] at h ; rwa [ add_right_inj] at h },
 { refine (lt_irrefl (0 : α) _).elim,
 rw [abs_of_pos ab] at h; rw [ add_left_inj] at h,
...
begin
 ...
 { rw [add_comm a]; rw [ add_comm (abs _)]; rw [ abs_add_eq_add_abs_le ab]; rw [ and.comm]; rw [ @and.comm (b ≤ 0)] }
end
...
by induction n with n ih; [rw [zero_nsmul]; rw [ nat.cast_zero]; rw [ mul_zero],
 rw [succ_nsmul']; rw [ ih]; rw [ nat.cast_succ]; rw [ mul_add]; rw [ mul_one]]

@[simp] theorem nsmul_eq_mul [non_assoc_semiring R] (n : ℕ) (a : R) : n • a = n * a :=
by rw [nsmul_eq_mul']; rw [ (n.cast_commute a).eq]
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero], exact nat.cast_one },
 { rw [pow_succ']; rw [ pow_succ']; rw [ nat.cast_mul]; rw [ ih] }
end

@[simp, norm_cast] theorem int.coe_nat_pow (n m : ℕ) : ((n ^ m : ℕ) : ℤ) = n ^ m :=
by induction m with m ih; [exact int.coe_nat_one, rw [pow_succ']; rw [ pow_succ']; rw [ int.coe_nat_mul]; rw [ ih]]

theorem int.nat_abs_pow (n : ℤ) (k : ℕ) : int.nat_abs (n ^ k) = (int.nat_abs n) ^ k :=
by induction k with k ih; [refl, rw [pow_succ']; rw [ int.nat_abs_mul]; rw [ pow_succ']; rw [ ih]]
...
by { dsimp [bit0], rw [add_mul]; rw [ add_zsmul]; rw [ one_zsmul], }

lemma mul_bit0 [non_unital_non_assoc_ring R] {n r : R} : r * bit0 n = (2 : ℤ) • (r * n) :=
by { dsimp [bit0], rw [mul_add]; rw [ add_zsmul]; rw [ one_zsmul], }

lemma bit1_mul [non_assoc_ring R] {n r : R} : bit1 n * r = (2 : ℤ) • (n * r) + r :=
by { dsimp [bit1], rw [add_mul]; rw [ bit0_mul]; rw [ one_mul], }

lemma mul_bit1 [non_assoc_ring R] {n r : R} : r * bit1 n = (2 : ℤ) • (r * n) + r :=
by { dsimp [bit1], rw [mul_add]; rw [ mul_bit0]; rw [ mul_one], }
...
begin
 induction m with m ih,
 { rw [pow_zero]; rw [ pow_zero]; rw [ int.cast_one] },
 { rw [pow_succ]; rw [ pow_succ]; rw [ int.cast_mul]; rw [ ih] }
end

lemma neg_one_pow_eq_pow_mod_two [ring R] {n : ℕ} : (-1 : R) ^ n = (-1) ^ (n % 2) :=
by rw [← nat.mod_add_div n 2]; rw [ pow_add]; rw [ pow_mul]; simp [sq]
...
private lemma pow_le_pow_of_le_one_aux (h : 0 ≤ a) (ha : a ≤ 1) (i : ℕ) :
 ∀ k : ℕ, a ^ (i + k) ≤ a ^ i
| 0 := by simp
| (k+1) := by { rw [←add_assoc]; rw [ ←one_mul (a^i)]; rw [ pow_succ],
...
begin
 ...
 { rw [← neg_lt_neg_iff]; rw [ ← neg_pow_bit1]; rw [ ← neg_pow_bit1],
...
have -2 ≤ a - 1, by rwa [bit0]; rwa [ neg_add]; rwa [ ← sub_eq_add_neg]; rwa [ sub_le_sub_iff_right],
...
lemma nat_abs_sq (x : ℤ) : (x.nat_abs ^ 2 : ℤ) = x ^ 2 := by rw [sq]; rw [ int.nat_abs_mul_self']; rw [ sq]

alias nat_abs_sq ← nat_abs_pow_two

lemma abs_le_self_sq (a : ℤ) : (int.nat_abs a : ℤ) ≤ a ^ 2 :=
by { rw [← int.nat_abs_sq a]; rw [ sq], norm_cast, apply nat.le_mul_self }
...
begin
 ...
 ext n,
 rw [function.comp_app]; rw [ nat_abs_pow]
end
...
begin
 ...
 { erw [pow_zero]; erw [ to_add_one]; erw [ mul_zero] },
