...
lemma pow_le_pow_of_le_left' [covariant_class M M (swap (*)) (≤)] {a b : M} (hab : a ≤ b) :
 ∀ i : ℕ, a ^ i ≤ b ^ i
| 0 := by simp
| (k+1) := by { rw [pow_succ]; rw [ pow_succ],
...
begin
 ...
 by { rw [pow_succ _ n]; rw [ pow_succ _ n], exact le_add_of_nonneg_right h1 }
 ... = (x+y) * (x^n + y^n) :
 by rw [add_mul]; rw [ mul_add]; rw [ mul_add]; rw [ add_comm (y*x^n)]; rw [ ← add_assoc]; rw [ ← add_assoc]; rw [ add_assoc (x*x^n) (x*y^n)]; rw [ add_comm (x*y^n) (y*y^n)]; rw [ ← add_assoc]
 ... ≤ (x+y)^n.succ :
...
by rw [←pow_abs]; rw [ abs_neg]; rw [ abs_one]; rw [ one_pow]
...
by rwa [← abs_of_nonneg hy]; rwa [ ← sq_lt_sq]
...
by rwa [← abs_of_nonneg hy]; rwa [ ← sq_le_sq]
...
(pow_eq_one_iff (nat.succ_ne_zero 1)).1 $ by rw [←map_pow]; rw [ neg_one_sq]; rw [ map_one]

@[simp] lemma map_neg (x : R) : f (-x) = f x :=
by rw [←neg_one_mul]; rw [ map_mul]; rw [ map_neg_one]; rw [ one_mul]

lemma map_sub_swap (x y : R) : f (x - y) = f (y - x) := by rw [←map_neg]; rw [ neg_sub]
