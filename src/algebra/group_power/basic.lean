...
by rw [pow_succ]; rw [ pow_zero]; rw [ mul_one]

/-- Note that most of the lemmas about powers of two refer to it as `sq`. -/
@[to_additive two_nsmul, nolint to_additive_doc]
theorem pow_two (a : M) : a^2 = a * a :=
by rw [pow_succ]; rw [ pow_one]

alias pow_two ← sq

theorem pow_three' (a : M) : a^3 = a * a * a := by rw [pow_succ']; rw [ pow_two]
...
by induction n with n ih; [rw [nat.add_zero]; rw [ pow_zero]; rw [ mul_one],
 rw [pow_succ']; rw [ ← mul_assoc]; rw [ ← ih]; rw [ ← pow_succ']; rw [ nat.add_assoc]]
...
by induction n with n ih; [exact pow_zero _, rw [pow_succ]; rw [ ih]; rw [ one_mul]]

@[to_additive mul_nsmul']
theorem pow_mul (a : M) (m n : ℕ) : a^(m * n) = (a^m)^n :=
begin
 induction n with n ih,
 { rw [nat.mul_zero]; rw [ pow_zero]; rw [ pow_zero] },
 { rw [nat.mul_succ]; rw [ pow_add]; rw [ pow_succ']; rw [ ih] }
end

@[to_additive nsmul_left_comm]
lemma pow_right_comm (a : M) (m n : ℕ) : (a^m)^n = (a^n)^m :=
by rw [←pow_mul]; rw [ nat.mul_comm]; rw [ pow_mul]

@[to_additive mul_nsmul]
theorem pow_mul' (a : M) (m n : ℕ) : a^(m * n) = (a^n)^m :=
by rw [nat.mul_comm]; rw [ pow_mul]

@[to_additive nsmul_add_sub_nsmul]
theorem pow_mul_pow_sub (a : M) {m n : ℕ} (h : m ≤ n) : a ^ m * a ^ (n - m) = a ^ n :=
by rw [←pow_add]; rw [ nat.add_comm]; rw [ nat.sub_add_cancel h]

@[to_additive sub_nsmul_nsmul_add]
theorem pow_sub_mul_pow (a : M) {m n : ℕ} (h : m ≤ n) : a ^ (n - m) * a ^ m = a ^ n :=
by rw [←pow_add]; rw [ nat.sub_add_cancel h]
...
by rw [bit1]; rw [ pow_succ']; rw [ pow_bit0]
...
by rw [pow_bit0]; rw [ (commute.refl a).mul_pow]
...
begin
 ...
 { simp },
 { calc a ^ n.succ * b ^ n.succ = a ^ n * a * (b * b ^ n) : by rw [pow_succ']; rw [ pow_succ]
...
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ inv_one]
| (n + 1) := by rw [pow_succ']; rw [ pow_succ]; rw [ inv_pow]; rw [ mul_inv_rev]
...
| -[1+ n] := by { rw [zpow_neg_succ_of_nat]; rw [ inv_inv]; rw [ ← zpow_coe_nat], refl }
...
lemma inv_zpow' (a : α) (n : ℤ) : a⁻¹ ^ n = a ^ (-n) := by rw [inv_zpow]; rw [ zpow_neg]
...
eq_mul_inv_of_mul_eq $ by rw [←pow_add]; rw [ nat.sub_add_cancel h]
...
by rw [pow_sub a⁻¹ h]; rw [ inv_pow]; rw [ inv_pow]; rw [ inv_inv]

end group

lemma pow_dvd_pow [monoid R] (a : R) {m n : ℕ} (h : m ≤ n) :
 a ^ m ∣ a ^ n := ⟨a ^ (n - m), by rw [← pow_add]; rw [ nat.add_comm]; rw [ nat.sub_add_cancel h]⟩
