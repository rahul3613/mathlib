...
begin
 ...
 simp_rw [←zsmul_eq_mul, smul_add, ←mul_smul_comm, zsmul_eq_mul (z : R)⁻¹, mul_inv_cancel hz', mul_one, ←coe_nat_zsmul, smul_smul, ←add_smul],
 split,
 { rintro ⟨k, h⟩,
 simp_rw ← h,
 refine ⟨⟨(k % z).to_nat, _⟩, k / z, _⟩,
 { rw [←int.coe_nat_lt]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ hz)],
 exact (int.mod_lt _ hz).trans_eq (int.abs_eq_nat_abs _) },
 rw [fin.coe_mk]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ hz)]; rw [ int.div_add_mod] },
 { rintro ⟨k, n, h⟩,
 exact ⟨_, h⟩, },
end
...
begin
 ...
 simp only [this],
 simp_rw [←coe_zsmul, ←coe_nsmul, ←coe_add, quotient_add_group.eq_iff_sub_mem, ←smul_sub, ←sub_sub, add_subgroup.zsmul_mem_zmultiples_iff_exists_sub_div hz],
end

lemma zmultiples_nsmul_eq_nsmul_iff {ψ θ : R ⧸ add_subgroup.zmultiples p} {n : ℕ} (hz : n ≠ 0) :
 n • ψ = n • θ ↔ (∃ k : fin n, ψ = θ + (k : ℕ) • (p / n : R)) :=
begin
 simp_rw [←coe_nat_zsmul ψ, ←coe_nat_zsmul θ, zmultiples_zsmul_eq_zsmul_iff (int.coe_nat_ne_zero.mpr hz), int.cast_coe_nat],
 refl,
end
