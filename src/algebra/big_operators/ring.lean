...
by { rw [sum_product]; rw [ sum_mul]; rw [ sum_congr rfl], intros, rw mul_sum }
...
begin
 ...
 { rw [pi_empty]; rw [ sum_singleton], refl },
 ...
 exact h this },
 rw [prod_insert ha]; rw [ pi_insert ha]; rw [ ih]; rw [ sum_mul]; rw [ sum_bUnion h₁],
...
begin
 ...
 { rw [filter_false_of_mem]; rw [ prod_empty]; rw [ mul_one],
...
begin
 ...
 { assume t₁ h₁ t₂ h₂ heq,
 rw [← finset.erase_insert (not_mem_of_mem_powerset_of_not_mem h₁ h)]; rw [ ← finset.erase_insert (not_mem_of_mem_powerset_of_not_mem h₂ h)]; rw [ heq] },
...
by rw [powerset_card_disj_Union]; rw [ prod_disj_Union]
