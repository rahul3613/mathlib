...
by rw [← s.coe_to_list]; rw [ coe_erase]; rw [ coe_prod]; rw [ coe_prod]; rw [ list.prod_erase (mem_to_list.2 h)]

@[simp, to_additive]
lemma prod_map_erase [decidable_eq ι] {a : ι} (h : a ∈ m) :
 f a * ((m.erase a).map f).prod = (m.map f).prod :=
by rw [← m.coe_to_list]; rw [ coe_erase]; rw [ coe_map]; rw [ coe_map]; rw [ coe_prod]; rw [ coe_prod]; rw [ list.prod_map_erase f (mem_to_list.2 h)]
...
by rw [insert_eq_cons]; rw [ prod_cons]; rw [ prod_singleton]
...
| 0 := by { rw [zero_nsmul]; rw [ pow_zero], refl }
| (n + 1) :=
 by rw [add_nsmul]; rw [ one_nsmul]; rw [ pow_add]; rw [ pow_one]; rw [ prod_add]; rw [ prod_nsmul n]
...
by rw [filter_eq]; rw [ prod_replicate]
...
@[to_additive]
lemma prod_map_one : prod (m.map (λ i, (1 : α))) = 1 := by rw [map_const]; rw [ prod_replicate]; rw [ one_pow]
...
quotient.induction_on s $ λ l, by { rw [quot_mk_to_coe]; rw [ coe_prod], exact list.prod_eq_zero_iff }
...
by { rw [←multiset.prod_replicate]; rw [ ←multiset.map_const], exact prod_map_le_prod _ h }
