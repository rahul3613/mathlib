...
by rw [finset.prod]; rw [ multiset.map_id]
...
by rw [eq_empty_of_is_empty s]; rw [ prod_empty]
...
begin
 ...
 { rw [prod_insert hm]; rw [ h hm]; rw [ one_mul] },
end
...
by rw [prod_insert (not_mem_singleton.2 h)]; rw [ prod_singleton]
...
by rw [finset.prod]; rw [ finset.map_val]; rw [ multiset.map_map]; refl
...
by rw [←prod_union_inter]; rw [ (disjoint_iff_inter_eq_empty.mp h)]; exact (mul_one _).symm
...
begin
 ...
 rw [← prod_union (disjoint_filter_filter_neg _ _ p)]; rw [ filter_union_filter_neg_eq]
end
...
by rw [finset.prod]; rw [ ← multiset.coe_prod]; rw [ ← multiset.coe_map]; rw [ finset.coe_to_list]
...
(finset.prod_disj_union h.disjoint).symm.trans $
 by { classical, rw [finset.disj_union_eq_union]; rw [ ← finset.sup_eq_union]; rw [ h.sup_eq_top]; refl }
...
lemma prod_sdiff [decidable_eq α] (h : s₁ ⊆ s₂) :
 (∏ x in (s₂ \ s₁), f x) * (∏ x in s₁, f x) = (∏ x in s₂, f x) :=
by rw [←prod_union sdiff_disjoint]; rw [ sdiff_union_of_subset h]

@[simp, to_additive]
lemma prod_disj_sum (s : finset α) (t : finset γ) (f : α ⊕ γ → β) :
 ∏ x in s.disj_sum t, f x = (∏ x in s, f (sum.inl x)) * (∏ x in t, f (sum.inr x)) :=
begin
 rw [←map_inl_disj_union_map_inr]; rw [ prod_disj_union]; rw [ prod_map]; rw [ prod_map],
 refl,
end
...
by rw [←disj_Union_eq_bUnion _ _ hs]; rw [ prod_disj_Union]
...
begin
 ...
 {intros a1 a2 h1 h2 eq, rw [←left_inv a1 h1]; rw [ ←left_inv a2 h2], cc,},
...
prod_subset (filter_subset _ _) $ λ x,
 by { classical, rw [not_imp_comm]; rw [ mem_filter], exact λ h₁ h₂, ⟨h₁, hp _ h₁ h₂⟩ }
...
by rw [← finset.prod_filter]; rw [ finset.filter_mem_eq_inter]
...
begin
 split_ifs with h,
 { rw [finset.prod_eq_single a]; rw [ dif_pos rfl],
...
by rw [range_succ]; rw [ prod_insert not_mem_range_self]
...
| (n + 1) := by rw [prod_range_succ _ n]; rw [ mul_right_comm]; rw [ ← prod_range_succ']; rw [ prod_range_succ]
...
begin
 induction m with m hm,
 { simp },
 { rw [nat.add_succ]; rw [ prod_range_succ]; rw [ hm]; rw [ prod_range_succ]; rw [ mul_assoc], },
end
...
lemma prod_range_zero (f : ℕ → β) :
 ∏ k in range 0, f k = 1 :=
by rw [range_zero]; rw [ prod_empty]
...
begin
 ...
 { rw [insert_eq_of_mem has]; rw [ ← insert_erase has]; rw [ prod_insert (not_mem_erase _ _)]; rw [ prod_insert (not_mem_erase _ _)]; rw [ ← mul_assoc]; rw [ count_cons_self]; rw [ pow_succ],
 congr' 1,
 refine prod_congr rfl (λ x hx, _),
 rw [count_cons_of_ne (ne_of_mem_erase hx)] },
 rw [prod_insert has]; rw [ count_cons_self]; rw [ count_eq_zero_of_not_mem (mt mem_to_finset.2 has)]; rw [ pow_one],
 congr' 1,
 refine prod_congr rfl (λ x hx, _),
 rw count_cons_of_ne,
 rintro rfl,
...
by rw [prod_range_div]; rw [ mul_div_cancel'_right]
...
begin
 ...
 { rw [prod_range_one]; rw [ prod_range_one] },
...
finset.strong_induction_on s
 ...
 from λ x hx y hy h, by rw [← g_inv x hx]; rw [ ← g_inv y hy]; simp [h],
 ...
 else by rw [← insert_erase hx]; rw [ prod_insert (not_mem_erase _ _)]; rw [ ← insert_erase (mem_erase.2 ⟨g_ne x hx hx1, g_mem x hx⟩)]; rw [ prod_insert (not_mem_erase _ _)]; rw [ ih']; rw [ mul_one]; rw [ h x hx]))
...
by { rw [piecewise]; rw [ prod_ite]; rw [ filter_mem_eq_inter]; rw [ ← sdiff_eq_filter], }
...
by { rw [prod_eq_mul_prod_diff_singleton h]; rw [ mul_comm] }
...
begin
 ...
 rw [←xbar_eq_x]; rw [ filter_congr (λ y _, @quotient.eq _ R y x)],
 apply h x x_in_s,
end
...
by { rw [update_eq_piecewise]; rw [ prod_piecewise], simp [h] }
...
by rw [← prod_insert (not_mem_erase a s)]; rw [ insert_erase h]
...
by rw [mul_comm]; rw [ mul_prod_erase s f h]
...
begin
 ...
 { rw h,
 rw h at hx,
 rw [←prod_subset (singleton_subset_iff.2 hx) (λ t ht ha, h1 t ht (not_mem_singleton.1 ha))] at hp; rw [ prod_singleton] at hp,
 exact hp },
 { exact h1 x hx h }
end
...
@[simp, to_additive] lemma prod_sdiff_eq_div (h : s₁ ⊆ s₂) :
 (∏ x in (s₂ \ s₁), f x) = (∏ x in s₂, f x) / (∏ x in s₁, f x) :=
by rw [eq_div_iff_mul_eq']; rw [ prod_sdiff h]
...
by rw [eq_div_iff_mul_eq']; rw [ prod_erase_mul _ _ h]
...
by { haveI := classical.dec_eq α, rw [←prod_erase_mul _ _ ha]; rw [ h]; rw [ mul_zero] }
...
begin
 ...
 assume a s ha ih,
 rw [prod_insert ha]; rw [ mul_eq_zero]; rw [ bex_def]; rw [ exists_mem_insert]; rw [ ih]; rw [ ← bex_def]
end

theorem prod_ne_zero_iff : (∏ x in s, f x) ≠ 0 ↔ (∀ a ∈ s, f a ≠ 0) :=
by { rw [ne]; rw [ prod_eq_zero_iff], push_neg }
...
by rw [h.eq_singleton_default]; rw [ finset.prod_singleton]

lemma sum_nat_mod (s : finset α) (n : ℕ) (f : α → ℕ) :
 (∑ i in s, f i) % n = (∑ i in s, f i % n) % n :=
(multiset.sum_nat_mod _ _).trans $ by rw [finset.sum]; rw [ multiset.map_map]

lemma prod_nat_mod (s : finset α) (n : ℕ) (f : α → ℕ) :
 (∏ i in s, f i) % n = (∏ i in s, f i % n) % n :=
(multiset.prod_nat_mod _ _).trans $ by rw [finset.prod]; rw [ multiset.map_map]

lemma sum_int_mod (s : finset α) (n : ℤ) (f : α → ℤ) :
 (∑ i in s, f i) % n = (∑ i in s, f i % n) % n :=
(multiset.sum_int_mod _ _).trans $ by rw [finset.sum]; rw [ multiset.map_map]

lemma prod_int_mod (s : finset α) (n : ℤ) (f : α → ℤ) :
 (∏ i in s, f i) % n = (∏ i in s, f i % n) % n :=
(multiset.prod_int_mod _ _).trans $ by rw [finset.prod]; rw [ multiset.map_map]
...
lemma prod_unique {α β : Type*} [comm_monoid β] [unique α] [fintype α] (f : α → β) :
 (∏ x : α, f x) = f default :=
by rw [univ_unique]; rw [ prod_singleton]
...
begin
 ...
 { simp_rw [finset.sum_cons hz, finset.sup_cons, finset.mem_cons, multiset.sup_eq_union, forall_eq_or_imp, ne.def, eq_self_iff_true, not_true, is_empty.forall_iff, true_and, imp_and_distrib, forall_and_distrib, ←hr, @eq_comm _ z],
 have := λ x ∈ i, ne_of_mem_of_not_mem H hz,
 simp only [this, not_false_iff, true_implies_iff] {contextual := tt},
 simp_rw [←disjoint_finset_sum_left, ←disjoint_finset_sum_right, disjoint_comm, ←and_assoc, and_self],
...
by rw [← finset.sum_multiset_map_count]; rw [ multiset.sum_map_singleton]
...
begin
 ...
 intros x hx,
 rw [← mul_nsmul]; rw [ nat.mul_div_cancel' (h x (mem_to_finset.mp hx))] },
 rw [← finset.sum_nsmul]; rw [ h₂]; rw [ to_finset_sum_count_nsmul_eq]
end
...
begin
 ...
 { rw [finset.sum_empty]; rw [ finset.prod_empty]; rw [ multiset.prod_zero] },
 { rw [finset.sum_insert hat]; rw [ finset.prod_insert hat]; rw [ multiset.prod_add]; rw [ ih] }
end
