...
by rw [finset.prod_eq_multiset_prod]; rw [ ← multiset.map_map]; rw [ prod_mk]; rw [ ← finset.prod_eq_multiset_prod]

theorem rel_associated_iff_map_eq_map {p q : multiset α} :
 multiset.rel associated p q ↔ p.map associates.mk = q.map associates.mk :=
by { rw [← multiset.rel_eq]; rw [ multiset.rel_map], simp only [mk_eq_mk_iff_associated] }
