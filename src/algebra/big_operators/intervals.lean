...
by { rw [← map_add_right_Ico]; rw [ prod_map], refl }
...
by rw [nat.Ico_succ_right_eq_insert_Ico hab]; rw [ sum_insert right_not_mem_Ico]; rw [ add_comm]
...
by rw [← sum_insert ha]; rw [ nat.Ico_insert_succ_left hab]
...
by rw [← prod_Ioc_consecutive _ hab (nat.le_succ b)]; rw [ nat.Ioc_succ_singleton]; rw [ prod_singleton]
...
begin
 by_cases h : m ≤ n,
 { rw [←nat.Ico_zero_eq_range]; rw [ prod_Ico_add]; rw [ zero_add]; rw [ tsub_add_cancel_of_le h] },
...
| 0 := rfl
| (n+1) := by rw [prod_Ico_succ_top $ nat.succ_le_succ $ zero_le n]; rw [ nat.factorial_succ]; rw [ prod_Ico_id_eq_factorial n]; rw [ nat.succ_eq_add_one]; rw [ mul_comm]
...
calc (∑ i in range n, i) * 2 = (∑ i in range n, i) + (∑ i in range n, (n - 1 - i)) :
 by rw [sum_range_reflect (λ i, i) n]; rw [ mul_two]
...
... = n * (n - 1) : by rw [sum_const]; rw [ card_range]; rw [ nat.nsmul_eq_mul]

/-- Gauss' summation formula -/
lemma sum_range_id (n : ℕ) : (∑ i in range n, i) = (n * (n - 1)) / 2 :=
by rw [← sum_range_id_mul_two n]; rw [ nat.mul_div_cancel]; exact dec_trivial
...
begin
 ...
 by rw [←sum_Ico_sub_bot _ hmn]; rw [ ←sum_Ico_succ_sub_top _ (nat.le_pred_of_lt hmn)]; rw [ nat.sub_add_cancel (pos_of_gt hmn)]; rw [ sub_add_cancel],
 rw sum_eq_sum_Ico_succ_bot hmn,
 conv { for (f _ • g _) [2] { rw ← sum_range_succ_sub_sum g } },
 simp_rw [smul_sub, sum_sub_distrib, h₂, h₁],
 conv_lhs { congr, skip, rw [←add_sub]; rw [ add_comm]; rw [ ←add_sub]; rw [ ←sum_sub_distrib] },
...
begin
 by_cases hn : n = 0,
 { simp [hn], },
 { rw [range_eq_Ico]; rw [ sum_Ico_by_parts f g (nat.pos_of_ne_zero hn)]; rw [ sum_range_zero]; rw [ smul_zero]; rw [ sub_zero]; rw [ range_eq_Ico] },
end
