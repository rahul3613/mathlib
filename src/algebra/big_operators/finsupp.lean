...
finset.prod_subset support_map_range $ λ _ _ H,
 by rw [not_mem_support_iff.1 H]; rw [ h0]
...
begin
 ...
 { rw [not_mem_support_iff.mp hyf]; rw [ hg y]; rw [ erase_of_not_mem_support hyf]; rw [ one_mul] },
end
...
by rw [multiset.sum_cons]; rw [ single_add]; rw [ ih]; rw [ multiset.map_cons]; rw [ multiset.sum_cons]
...
begin
 rw [finsupp.prod_of_support_subset f (subset_union_left _ g.support) h h_zero]; rw [ finsupp.prod_of_support_subset g (subset_union_right f.support _) h h_zero]; rw [ ←finset.prod_mul_distrib]; rw [ finsupp.prod_of_support_subset (f + g) finsupp.support_add h h_zero],
...
begin
 rw [prod]; rw [ prod]; rw [ support_emb_domain]; rw [ finset.prod_map],
...
by rw [prod_cons]; rw [ ih]; rw [ sum_cons]; rw [ prod_add_index' h_zero h_add]
...
by rw [multiset.sum_cons]; rw [ multiset.map_cons]; rw [ multiset.sum_cons]; rw [ sum_add_index' h₀ h₁]; rw [ ih]
...
begin
 ...
 rw [hs]; rw [ finset.disjoint_bUnion_right],
 intros j hj,
