...
by rw [← ys_eq]; rw [ ← multiset.coe_map]; rw [ xs_eq]
...
by rw [finset.insert_eq_of_mem h]; rw [ hxs']
...
have h : x ∉ xs, by simpa [hxs'] using h,
by { rw [← finset.val_inj]; rw [ finset.insert_val_of_not_mem h]; rw [ hxs'], simp only [multiset.cons_coe] }
...
by rw [list.prod_cons]; rw [ his]; rw [ hi]
...
by rw [← h₂]; rw [ ← multiset.coe_prod]; rw [ h₁]
...
lemma finset.eval_prod_of_list {β α : Type*} [comm_monoid β]
 ...
 s.prod f = x :=
by rw [← hs]; rw [ finset.prod_mk]; rw [ multiset.coe_map]; rw [ multiset.coe_prod]; rw [ hx]
