...
begin
 ...
 rwa [hf.mem_to_finset] at hxf; rwa [ nmem_mul_support] at hxf
end
...
begin
 ...
 from λ x h, h rfl,
 rw [finprod_eq_prod_plift_of_mul_support_subset this]; rw [ finset.prod_empty]
end
...
begin
 ...
 rw [finprod_eq_prod_plift_of_mul_support_subset this]; rw [ finset.prod_singleton],
end
...
@[to_additive] lemma monoid_hom.map_finprod_plift (f : M →* N) (g : α → M)
 ...
 f (∏ᶠ x, g x) = ∏ᶠ x, f (g x) :=
begin
 rw [finprod_eq_prod_plift_of_mul_support_subset h.coe_to_finset.ge]; rw [ finprod_eq_prod_plift_of_mul_support_subset]; rw [ f.map_prod],
 rw [h.coe_to_finset],
...
@[simp, to_additive] lemma finprod_mem_mul_support (f : α → M) (a : α) :
 ∏ᶠ (h : f a ≠ 1), f a = f a :=
by rw [← mem_mul_support]; rw [ finprod_eq_mul_indicator_apply]; rw [ mul_indicator_mul_support]
...
begin
 ...
 { rw [A]; rw [ finset.coe_map], exact image_subset _ h },
...
begin
 ...
 erw [finprod_mem_def]; erw [ finprod_eq_prod_of_mul_support_subset _ this],
...
begin
 ...
 intros x hx,
 rw [finset.mem_erase]; rw [ finite.mem_to_finset]; rw [ mem_mul_support],
...
by rw [finprod_mem_def]; rw [ finprod_mem_def]; rw [ mul_indicator_inter_mul_support]

@[to_additive] lemma finprod_mem_inter_mul_support_eq (f : α → M) (s t : set α)
 (h : s ∩ mul_support f = t ∩ mul_support f) :
 ∏ᶠ i ∈ s, f i = ∏ᶠ i ∈ t, f i :=
by rw [← finprod_mem_inter_mul_support]; rw [ h]; rw [ finprod_mem_inter_mul_support]
...
begin
 classical,
 rw [finprod_eq_prod_of_mul_support_to_finset_subset _ hf (finset.subset_union_left _ _)]; rw [ finprod_eq_prod_of_mul_support_to_finset_subset _ hg (finset.subset_union_right _ _)]; rw [ ← finset.prod_mul_distrib],
...
`t ∩ support f` rather than `s` and `t` to be finite."] lemma finprod_mem_union_inter'
 ...
 (∏ᶠ i ∈ s ∪ t, f i) * ∏ᶠ i ∈ s ∩ t, f i = (∏ᶠ i ∈ s, f i) * ∏ᶠ i ∈ t, f i :=
begin
 rw [← finprod_mem_inter_mul_support f s]; rw [ ← finprod_mem_inter_mul_support f t]; rw [ ← finprod_mem_union_inter hs ht]; rw [ ← union_inter_distrib_right]; rw [ finprod_mem_inter_mul_support]; rw [ ← finprod_mem_inter_mul_support f (s ∩ t)],
 congr' 2,
...
by rw [← finprod_mem_union_inter' hs ht]; rw [ disjoint_iff_inter_eq_empty.1 hst]; rw [ finprod_mem_empty]; rw [ mul_one]
...
lemma finprod_mem_union'' (hst : disjoint (s ∩ mul_support f) (t ∩ mul_support f))
 (hs : (s ∩ mul_support f).finite) (ht : (t ∩ mul_support f).finite) :
 ∏ᶠ i ∈ s ∪ t, f i = (∏ᶠ i ∈ s, f i) * ∏ᶠ i ∈ t, f i :=
by rw [← finprod_mem_inter_mul_support f s]; rw [ ← finprod_mem_inter_mul_support f t]; rw [ ← finprod_mem_union hst hs ht]; rw [ ← union_inter_distrib_right]; rw [ finprod_mem_inter_mul_support]
...
by rw [← finset.coe_singleton]; rw [ finprod_mem_coe_finset]; rw [ finset.prod_singleton]
...
by { rw [finprod_mem_insert]; rw [ finprod_mem_singleton], exacts [h, finite_singleton b] }
...
begin
 ...
 rw [finprod_mem_eq_prod _ hs]; rw [ ← finset.prod_image hg],
 refine finprod_mem_eq_prod_of_inter_mul_support_eq f _,
 rw [finset.coe_image]; rw [ hs.coe_to_finset]; rw [ ← image_inter_mul_support_eq]; rw [ inter_assoc]; rw [ inter_self] },
 { rw [finprod_mem_eq_one_of_infinite hs]; rw [ finprod_mem_eq_one_of_infinite],
 rwa [image_inter_mul_support_eq]; rwa [ infinite_image_iff hg] }
end
...
lemma finprod_mem_eq_of_bij_on {s : set α} {t : set β} {f : α → M} {g : β → M} (e : α → β)
 ...
 ∏ᶠ i ∈ s, f i = ∏ᶠ j ∈ t, g j :=
begin
 rw [← set.bij_on.image_eq he₀]; rw [ finprod_mem_image he₀.2.1],
...
by rw [← finprod_mem_inter_mul_diff' _ ht]; rw [ inter_eq_self_of_subset_right hst]
...
begin
 ...
 classical,
 rw [← bUnion_univ]; rw [ ← finset.coe_univ]; rw [ ← finset.coe_bUnion]; rw [ finprod_mem_coe_finset]; rw [ finset.prod_bUnion],
...
begin
 haveI := hI.fintype,
 rw [bUnion_eq_Union]; rw [ finprod_mem_Union]; rw [ ← finprod_set_coe_eq_finprod_mem],
...
begin
 ...
 { intros x hx,
 rw [finset.mem_sdiff]; rw [ finset.mem_singleton]; rw [ finite.mem_to_finset]; rw [ mem_mul_support],
 exact ⟨λ h, and.intro hx h, λ h, h.2⟩,},
 rw [finprod_cond_eq_prod_of_cond_iff f h]; rw [ finset.sdiff_singleton_eq_erase],
 by_cases ha : a ∈ mul_support f,
 { apply finset.mul_prod_erase _ _ ((finite.mem_to_finset _ ).mpr ha), },
 { rw [mem_mul_support] at ha; rw [ not_not] at ha,
 rw [ha]; rw [ one_mul],
...
begin
 ...
 contrapose! hx,
 rw [mem_mul_support]; rw [ not_not]; rw [ finset.prod_congr rfl hx]; rw [ finset.prod_const_one] },
 rw [finprod_eq_prod_of_mul_support_subset _ hU]; rw [ finset.prod_comm],
...
begin
 ...
 rw [finprod_eq_prod_of_mul_support_subset _ (s.mul_support_of_fiberwise_prod_subset_image f prod.fst)]; rw [ ← finset.prod_fiberwise_of_maps_to _ f], -- `finish` could close the goal here
...
begin
 ...
 rw [finprod_dmem]; rw [ finprod_mem_range hf]; rw [ finprod_congr (λ a, _)],
 rw [dif_pos (set.mem_range_self a)]; rw [ hf (classical.some_spec (set.mem_range_self a))]
end
