...
begin
 unfold mul_hom,
 rw [to_add_monoid_of]; rw [ flip_apply]; rw [ to_add_monoid_of]; rw [ flip_apply]; rw [ coe_comp]; rw [ function.comp_app]; rw [ comp_hom_apply_apply]; rw [ coe_comp]; rw [ function.comp_app]; rw [ gmul_hom_apply_apply],
end
...
begin
 apply add_hom_ext, intros i xi,
 unfold has_one.one,
 rw [flip_apply]; rw [ mul_hom_of_of],
...
begin
 ...
 rw [mul_hom_of_of]; rw [ mul_hom_of_of]; rw [ mul_hom_of_of]; rw [ mul_hom_of_of],
...
{ one := 1,
 ...
 nat_cast_zero := by rw [gsemiring.nat_cast_zero]; rw [ map_zero],
 nat_cast_succ := λ n, by { rw [gsemiring.nat_cast_succ]; rw [ map_add], refl },
...
by rw [list.of_fn_eq_map]; rw [ of_list_dprod]
...
begin
 ...
 rw [add_monoid_hom.flip_apply]; rw [ mul_hom_of_of]; rw [ mul_hom_of_of],
...
| 0 := by rw [pow_zero]; rw [ pow_zero]; rw [ direct_sum.of_zero_one]
| (n + 1) := by rw [pow_succ]; rw [ pow_succ]; rw [ of_zero_mul]; rw [ of_zero_pow]
...
{ to_fun := to_add_monoid f,
 ...
 rw [of_mul_of]; rw [ to_add_monoid_of]; rw [ to_add_monoid_of]; rw [ to_add_monoid_of],
 exact hmul _ _,
 end,
 .. to_add_monoid f}
...
by rw [direct_sum.of_mul_of]; rw [ has_mul.ghas_mul_mul]
