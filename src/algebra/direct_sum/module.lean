...
begin
 rw [submodule.supr_eq_range_dfinsupp_lsum]; rw [ linear_map.range_eq_top],
...
⟨hi.submodule_independent.pairwise_disjoint hij,
 ...
 by rw [←Sup_pair]; rw [ supr]; rw [ ←set.image_univ]; rw [ h]; rw [ set.image_insert_eq]; rw [ set.image_singleton]⟩
...
begin
 ...
 rw [is_internal_submodule_iff_independent_and_supr_eq_top]; rw [ supr]; rw [ ←set.image_univ]; rw [ h]; rw [ set.image_insert_eq]; rw [ set.image_singleton]; rw [ Sup_pair]; rw [ complete_lattice.independent_pair hij this],
