...
by { rw [from_add_monoid]; rw [ to_add_monoid_of], refl }

lemma from_add_monoid_of_apply (i : ι) (f : γ →+ β i) (x : γ) :
 from_add_monoid (of _ i f) x = of _ i (f x) :=
by rw [from_add_monoid_of]; rw [ add_monoid_hom.coe_comp]
...
{ to_fun := direct_sum.to_add_monoid (λ _, add_monoid_hom.id M),
 ...
 (by rw [add_monoid_hom.map_zero]; rw [ add_monoid_hom.map_zero])
 (λ p x, by rw [unique.default_eq p]; rw [ to_add_monoid_of]; refl)
 (λ x y ihx ihy, by rw [add_monoid_hom.map_add]; rw [ add_monoid_hom.map_add]; rw [ ihx]; rw [ ihy]),
...
begin
 ...
 { rw [direct_sum.of_eq_same]; rw [ if_pos rfl], },
 { rw [direct_sum.of_eq_of_ne _ _ _ _ h]; rw [ if_neg h]; rw [ zero_mem_class.coe_zero], },
end
...
begin
 rw [add_submonoid.supr_eq_mrange_dfinsupp_sum_add_hom]; rw [ add_monoid_hom.mrange_top_iff_surjective],
