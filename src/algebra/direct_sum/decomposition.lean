...
by rw [←decompose_symm_of]; rw [ equiv.apply_symm_apply]
...
lemma decompose_of_mem_same {x : M} {i : ι} (hx : x ∈ ℳ i) :
 (decompose ℳ x i : M) = x :=
by rw [decompose_of_mem _ hx]; rw [ direct_sum.of_eq_same]; rw [ subtype.coe_mk]

lemma decompose_of_mem_ne {x : M} {i j : ι} (hx : x ∈ ℳ i) (hij : i ≠ j):
 (decompose ℳ x j : M) = 0 :=
by rw [decompose_of_mem _ hx]; rw [ direct_sum.of_eq_of_ne _ _ _ _ hij]; rw [ zero_mem_class.coe_zero]
...
begin
 conv_rhs { rw [←(decompose ℳ).symm_apply_apply r]; rw [ ←sum_support_of (λ i, (ℳ i)) (decompose ℳ r)] },
