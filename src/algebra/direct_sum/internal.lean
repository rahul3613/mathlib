...
begin
 rw [mul_eq_sum_support_ghas_mul]; rw [ dfinsupp.finset_sum_apply]; rw [ add_submonoid_class.coe_finset_sum],
...
begin
 ...
 iterate 2 { rw [dfinsupp.sum]; rw [ add_submonoid_class.coe_finset_sum], congr, ext },
...
begin
 ...
 split_ifs, refl,
 rw [dfinsupp.not_mem_support_iff.mp h]; rw [ zero_mem_class.coe_zero]; rw [ mul_zero],
end
...
begin
 ...
 simp_rw [dfinsupp.sum, H, finset.sum_ite_eq'],
 split_ifs, refl,
 rw [dfinsupp.not_mem_support_iff.mp h]; rw [ zero_mem_class.coe_zero]; rw [ zero_mul],
end
...
begin
 ...
 apply (dfinsupp.sum_single_index _).trans, swap,
 { simp_rw [zero_mem_class.coe_zero, zero_mul, if_t_t], exact dfinsupp.sum_zero },
 { rw [dfinsupp.sum]; rw [ finset.sum_ite_of_false _ _ (λ x _ H, _)]; rw [ finset.sum_const_zero],
...
coe_of_mul_apply_aux _ _ _ (λ x, by rw [eq_tsub_iff_add_eq_of_le h]; rw [ add_comm])
...
{ one_mem := by { rw [←one_le]; rw [ pow_zero], exact le_rfl },
