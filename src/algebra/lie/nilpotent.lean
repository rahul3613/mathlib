...
begin
 ...
 { rw N.range_incl,
 apply lcs_le_self, },
 rw [ih]; rw [ lie_submodule.comap_bracket_eq _ _ N.incl N.ker_incl this], },
end

lemma lower_central_series_map_eq_lcs :
 (lower_central_series R L N k).map N.incl = N.lcs k :=
begin
 rw [lower_central_series_eq_lcs_comap]; rw [ lie_submodule.map_comap_incl]; rw [ inf_eq_right],
 apply lcs_le_self,
end
...
begin
 split; intros h,
 { erw [eq_bot_iff]; erw [ lie_submodule.lie_span_le], rintros m ⟨x, n, hn⟩, rw [← hn, h.trivial], simp,},
...
begin
 ...
 { rw [derived_series_def]; rw [ derived_series_of_ideal_zero]; rw [ lower_central_series_zero],
 exact le_rfl, },
 { have h' : derived_series R L k ≤ ⊤, { simp only [le_top], },
 rw [derived_series_def]; rw [ derived_series_of_ideal_succ]; rw [ lower_central_series_succ],
...
begin
 ...
 rw [N.lower_central_series_eq_lcs_comap k]; rw [ lie_submodule.comap_incl_eq_bot]; rw [ inf_eq_right.mpr (N.lcs_le_self k)],
end
...
begin
 ...
 use k,
 intros x, ext m,
 rw [linear_map.pow_apply]; rw [ linear_map.zero_apply]; rw [ ← @lie_submodule.mem_bot R L M]; rw [ ← hM],
...
begin
 ...
 rwa [ideal_oper_max_triv_submodule_eq_bot] at this; rwa [ le_bot_iff] at this, },
 rw [← lie_submodule.quotient.map_mk'_eq_bot_le]; rw [ ← le_bot_iff]; rw [ ← hk],
...
begin
 ...
 exact ⟨k, hk⟩, },
 change Inf s = 0 ↔ _,
 rw [← lie_submodule.subsingleton_iff R L M]; rw [ ← subsingleton_iff_bot_eq_top]; rw [ ← lower_central_series_zero]; rw [ @eq_comm (lie_submodule R L M)],
...
begin
 ...
 rw [nilpotency_length_eq_succ_iff] at h; rw [ lower_central_series_succ] at h,
 exact h.1, },
end
...
begin
 rw [lie_submodule.nontrivial_iff_ne_bot]; rw [ lower_central_series_last],
 cases h : nilpotency_length R L M,
 { rw [nilpotency_length_eq_zero_iff] at h; rw [ ← not_nontrivial_iff_subsingleton] at h,
 contradiction, },
...
lemma ucs_eq_self_of_normalizer_eq_self (h : N₁.normalizer = N₁) (k : ℕ) :
 N₁.ucs k = N₁ :=
by { induction k with k ih, { simp, }, { rwa [ucs_succ]; rwa [ ih], }, }
...
by { rw [eq_top_iff]; rw [ ← lcs_le_iff], refl, }
...
begin
 ...
 rw [linear_equiv.coe_coe]; rw [ lie_equiv.coe_to_lie_hom]; rw [ ← g.symm_apply_apply ⁅f.symm x]; rw [ g.symm m⁆]; rw [ ← hfg]; rw [ f.apply_symm_apply]; rw [ g.apply_symm_apply], },
end
...
begin
 ...
 erw [← lie_submodule.mem_coe_submodule]; erw [ ih]; erw [ lie_submodule.mem_coe_submodule] at hz,
...
{ nilpotent :=
 ...
 obtain ⟨k, hk⟩ := id h₁,
 use k,
 apply lie_ideal.bot_of_map_eq_bot h₂, rw [eq_bot_iff]; rw [ ← hk],
...
{ nilpotent :=
 ...
 obtain ⟨k, hk⟩ := id h₁,
 use k,
 rw [← lie_ideal.lower_central_series_map_eq k h₂]; rw [ hk],
...
begin
 induction k with k ih,
 { simp, },
 { simp_rw [lower_central_series_succ, lcs_succ, lie_submodule.lie_ideal_oper_eq_linear_span', ← (I.lcs M k).mem_coe_submodule, ih, lie_submodule.mem_coe_submodule, lie_submodule.mem_top, exists_true_left, (I : lie_subalgebra R L).coe_bracket_of_module],
 congr,
 ext m,
 split,
