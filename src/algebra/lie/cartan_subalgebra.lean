...
by rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ coe_normalizer_eq_normalizer]; rw [ is_cartan_subalgebra.self_normalizing]; rw [ coe_to_lie_submodule]
...
begin
 ...
 refine ⟨k, λ l hl, _⟩,
 rw [← nat.sub_add_cancel hl]; rw [ lie_submodule.ucs_add]; rw [ ← hk]; rw [ lie_subalgebra.ucs_eq_self_of_is_cartan_subalgebra], },
 { rintros ⟨k, hk⟩,
 exact
 { nilpotent :=
 begin
 ...
 use k,
 rw [_root_.eq_bot_iff]; rw [ lie_submodule.lcs_le_iff]; rw [ hk k (le_refl k)],
 exact le_refl _,
 end,
 self_normalizing :=
 begin
 have hk' := hk (k + 1) k.le_succ,
 rw [lie_submodule.ucs_succ] at hk'; rw [ hk k (le_refl k)] at hk',
 rw [← lie_subalgebra.coe_to_submodule_eq_iff]; rw [ ← lie_subalgebra.coe_normalizer_eq_normalizer]; rw [ hk']; rw [ lie_subalgebra.coe_to_lie_submodule],
 end } },
end
...
{ nilpotent := infer_instance,
 self_normalizing :=
 by { rw [← top_coe_lie_subalgebra]; rw [ normalizer_eq_top]; rw [ top_coe_lie_subalgebra], }, }
