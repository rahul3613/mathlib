...
begin
 ...
 intros c,
 have c' : A.val ⬝ B.val = B.val ⬝ A.val, by { rw [← sub_eq_zero]; rw [ ← sl_bracket]; rw [ c.trivial], refl },
...
by erw [lie_equiv.trans_apply]; erw [ lie_equiv.of_eq_apply]; erw [ skew_adjoint_matrices_lie_subalgebra_equiv_apply]
...
begin
 rw [← matrix.diagonal_one]; rw [ matrix.diagonal_neg]; rw [ matrix.from_blocks_diagonal],
 refl,
end
...
begin
 ...
 erw [h]; erw [ S_as_blocks]; erw [ matrix.from_blocks_multiply]; erw [ matrix.from_blocks_smul],
 congr; simp [two_smul],
end
...
begin
 ...
 rw [← smul_add]; rw [ ← (two_smul R _)]; rw [ smul_smul]; rw [ inv_of_mul_self]; rw [ one_smul],
 erw [matrix.from_blocks_transpose]; erw [ matrix.from_blocks_smul]; erw [ matrix.mul_eq_mul]; erw [ matrix.from_blocks_multiply],
 simp [h],
end
...
begin
 ...
 apply lie_equiv.of_eq,
 ext A,
 rw [JD_transform]; rw [ ← coe_unit_of_invertible (2 : R)]; rw [ ←units.smul_def]; rw [ lie_subalgebra.mem_coe]; rw [ mem_skew_adjoint_matrices_lie_subalgebra_unit_smul],
 refl,
end
...
begin
 rw [PB]; rw [ matrix.mul_eq_mul]; rw [ matrix.from_blocks_multiply]; rw [ matrix.mul_inv_of_self],
...
begin
 ...
 apply lie_equiv.of_eq,
 ext A,
 rw [JB_transform]; rw [ ← coe_unit_of_invertible (2 : R)]; rw [ ←units.smul_def]; rw [ lie_subalgebra.mem_coe]; rw [ lie_subalgebra.mem_coe]; rw [ mem_skew_adjoint_matrices_lie_subalgebra_unit_smul],
