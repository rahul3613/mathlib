...
{ bracket := λ x, has_bracket_aux x,
 ...
 { simp only [← linear_map.add_apply], rw [← linear_map.comp_apply]; rw [ this], refl },
 ext m n,
...
begin
 ...
 { rw [← this]; rw [ lie_module_hom.coe_to_linear_map], },
 ext m n,
...
begin
 rw [← coe_to_submodule_eq_iff]; rw [ lie_ideal_oper_eq_linear_span]; rw [ lie_module_hom.coe_submodule_range]; rw [ lie_module_hom.coe_linear_map_comp]; rw [ linear_map.range_comp]; rw [ map_incl_def]; rw [ coe_linear_map_map]; rw [ tensor_product.map_range_eq_span_tmul]; rw [ submodule.map_span],
 congr, ext m, split,
