...
begin
 ...
 rw [this]; rw [ ← lie_algebra_of_apply R ι L i ⁅x]; rw [ hij.rec_on y⁆]; rw [ lie_hom.map_lie]; rw [ lie_algebra_of_apply]; rw [ lie_algebra_of_apply],
end
...
{ to_fun := to_module R ι L' (λ i, (f i : L i →ₗ[R] L')),
 ...
 rw [← linear_map.comp_apply]; rw [ ← linear_map.comp_apply],
 ...
 { intros i y,
 rw [← lie_skew x]; rw [ ← lie_skew (to_module R ι L' f' x)],
