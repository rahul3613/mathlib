...
begin
 ...
 { rw [submodule.map_sup]; rw [ ← submodule.map_comp]; rw [ ← linear_map.mul_eq_comp]; rw [ ← pow_succ]; rw [ ← I.lcs_succ],
...
begin
 ...
 have hI₂ : (R ∙ (⟨x, hxK'⟩ : K')) ⊔ I = ⊤,
 { rw [← lie_ideal.coe_to_lie_subalgebra_to_submodule R K' I]; rw [ hI₁],
...
begin
 ...
 rw [ne.def]; rw [ eq_comm]; rw [ K.normalizer_eq_self_iff]; rw [ ← ne.def]; rw [ ← lie_submodule.nontrivial_iff_ne_bot R K],
 haveI : nontrivial (L' ⧸ K.to_lie_submodule),
 { replace hK₂ : K.to_lie_submodule ≠ ⊤ :=
 by rwa [ne.def]; rwa [ ← lie_submodule.coe_to_submodule_eq_iff]; rwa [ K.coe_to_lie_submodule]; rwa [ lie_submodule.top_coe_submodule]; rwa [ ← lie_subalgebra.top_coe_submodule]; rwa [ K.coe_to_submodule_eq_iff],
