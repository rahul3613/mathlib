...
{ trivial := λ x y,
 ...
 obtain ⟨u, rfl⟩ := h₁ x,
 obtain ⟨v, rfl⟩ := h₁ y,
 rw [← lie_hom.map_lie]; rw [ trivial_lie_zero]; rw [ lie_hom.map_zero],
 end }
...
{ carrier := { m | ∀ (x : L), ⁅x, m⁆ = 0 },
 ...
 add_mem' := λ x y hx hy z, by rw [lie_add]; rw [ hx]; rw [ hy]; rw [ add_zero],
 smul_mem' := λ c x hx y, by rw [lie_smul]; rw [ hx]; rw [ smul_zero],
 lie_mem := λ x m hm y, by rw [hm]; rw [ lie_zero], }
...
begin
 rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ lie_submodule.lie_ideal_oper_eq_linear_span]; rw [ lie_submodule.bot_coe_submodule]; rw [ submodule.span_eq_bot],
...
begin
 ...
 { rw [← le_bot_iff]; rw [ ← ideal_oper_max_triv_submodule_eq_bot R L M ⊤],
...
begin
 ...
 rw [← mem_max_triv_submodule R L M]; rw [ h], exact lie_submodule.mem_top m, },
end
...
{ to_fun := λ f,
 { to_linear_map := f.val,
 map_lie' := λ x m, by
 { have hf : ⁅x, f.val⁆ m = 0, { rw [f.property x]; rw [ linear_map.zero_apply], },
 rw [lie_hom.lie_apply] at hf; rw [ sub_eq_zero] at hf; rw [ ← linear_map.to_fun_eq_coe] at hf, exact hf.symm, }, },
