...
begin
 ...
 { rintros m'' ⟨x, n, hm''⟩, rw [← hm'']; rw [ leibniz_lie],
 ...
 rw [coe_submodule_le_coe_submodule]; rw [ lie_ideal_oper_eq_span]; rw [ lie_span_le],
...
begin
 rw [lie_ideal_oper_eq_span]; rw [ lie_submodule.lie_span_le],
...
begin
 ...
 rw [lie_ideal_oper_eq_span]; rw [ lie_span_le], rintros x ⟨y, z, h⟩, rw ← h,
 rw [← lie_skew]; rw [ ← lie_neg]; rw [ ← lie_submodule.coe_neg],
 apply lie_coe_mem_lie,
end

lemma lie_le_right : ⁅I, N⁆ ≤ N :=
begin
 rw [lie_ideal_oper_eq_span]; rw [ lie_span_le], rintros m ⟨x, n, hn⟩, rw ← hn,
...
begin
 ...
 rw [lie_ideal_oper_eq_span]; rw [ lie_span_le], rintros m ⟨⟨x, hx⟩, n, hn⟩, rw ← hn,
 change x ∈ (⊥ : lie_ideal R L) at hx, rw mem_bot at hx, simp [hx],
end

lemma lie_eq_bot_iff : ⁅I, N⁆ = ⊥ ↔ ∀ (x ∈ I) (m ∈ N), ⁅(x : L), m⁆ = 0 :=
begin
 rw [lie_ideal_oper_eq_span]; rw [ lie_submodule.lie_span_eq_bot_iff],
...
begin
 intros m h,
 rw [lie_ideal_oper_eq_span] at h; rw [ mem_lie_span] at h, rw [lie_ideal_oper_eq_span, mem_lie_span],
...
begin
 ...
 rw [lie_ideal_oper_eq_span]; rw [ lie_span_le], rintros m ⟨x, ⟨n, hn⟩, h⟩, erw lie_submodule.mem_sup,
...
begin
 ...
 rw [lie_ideal_oper_eq_span]; rw [ lie_span_le], rintros m ⟨⟨x, hx⟩, n, h⟩, erw lie_submodule.mem_sup,
...
begin
 rw [← coe_to_submodule_eq_iff]; rw [ coe_submodule_map]; rw [ lie_ideal_oper_eq_linear_span]; rw [ lie_ideal_oper_eq_linear_span]; rw [ submodule.map_span],
 congr,
 ext m,
 split,
...
begin
 ...
 rw [← lie_submodule.coe_submodule_le_coe_submodule]; rw [ coe_map_of_surjective h]; rw [ lie_submodule.lie_ideal_oper_eq_linear_span]; rw [ lie_submodule.lie_ideal_oper_eq_linear_span]; rw [ linear_map.map_span],
...
begin
 rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ comap_coe_submodule]; rw [ lie_submodule.sup_coe_to_submodule]; rw [ f.ker_coe_submodule]; rw [ ← submodule.comap_map_eq]; rw [ lie_submodule.lie_ideal_oper_eq_linear_span]; rw [ lie_submodule.lie_ideal_oper_eq_linear_span]; rw [ linear_map.map_span],
 congr, simp only [lie_hom.coe_to_linear_map, set.mem_set_of_eq], ext y,
 split,
 { rintros ⟨⟨x₁, hx₁⟩, ⟨x₂, hx₂⟩, hy⟩, rw ← hy,
 erw [lie_submodule.mem_inf]; erw [ f.mem_ideal_range_iff h] at hx₁ hx₂,
 obtain ⟨⟨z₁, hz₁⟩, hz₁'⟩ := hx₁, rw ← hz₁ at hz₁',
 obtain ⟨⟨z₂, hz₂⟩, hz₂'⟩ := hx₂, rw ← hz₂ at hz₂',
 use [⁅z₁, z₂⁆, ⟨z₁, hz₁'⟩, ⟨z₂, hz₂'⟩, rfl],
 simp only [hz₁, hz₂, submodule.coe_mk, lie_hom.map_lie], },
 { rintros ⟨x, ⟨⟨z₁, hz₁⟩, ⟨z₂, hz₂⟩, hx⟩, hy⟩, rw [← hy]; rw [ ← hx],
...
by { rw [← map_sup_ker_eq_map]; rw [ ← comap_bracket_eq h]; rw [ map_comap_eq h]; rw [ inf_eq_right],
...
by { rw comap_bracket_incl, rw ← inf_eq_right at h₁ h₂, rw [h₁]; rw [ h₂], }
