...
begin
 ...
 { rw [zero_add]; rw [ derived_series_of_ideal_zero], },
 { rw [nat.succ_add k l]; rw [ derived_series_of_ideal_succ]; rw [ derived_series_of_ideal_succ]; rw [ ih], },
end
...
begin
 ...
 { rw le_zero_iff at h₂, rw [h₂]; rw [ derived_series_of_ideal_zero], exact h₁, },
 { have h : l = k.succ ∨ l ≤ k, by rwa [le_iff_eq_or_lt] at h₂; rwa [ nat.lt_succ_iff] at h₂,
 cases h,
 { rw [h]; rw [ derived_series_of_ideal_succ]; rw [ derived_series_of_ideal_succ],
...
by rw [derived_series_of_ideal_succ]; rw [ derived_series_of_ideal_zero]; rw [ lie_submodule.lie_abelian_iff_lie_self_eq_bot]
...
by rw [add_comm]; rw [ derived_series_of_ideal_add I 1 k]; rw [ abelian_iff_derived_one_eq_bot]
...
by { rw [derived_series_eq_derived_series_of_ideal_comap]; rw [ map_comap_incl]; rw [ inf_eq_right],
...
by rw [← derived_series_eq_derived_series_of_ideal_map]; rw [ map_eq_bot_iff]; rw [ ker_incl]; rw [ eq_bot_iff]
...
begin
 obtain ⟨k, hk⟩ := id h₁,
 use k,
 rw [← lie_ideal.derived_series_map_eq k h₂]; rw [ hk],
...
begin
 use 1,
 rw [← abelian_iff_derived_one_eq_bot]; rw [ lie_abelian_iff_equiv_lie_abelian lie_ideal.top_equiv],
 apply_instance,
end
...
begin
 ...
 rw [derived_series_def] at hk; rw [ lie_ideal.derived_series_eq_bot_iff] at hk, exact hk, },
 simp [hne],
end
