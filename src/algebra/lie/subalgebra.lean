...
⟨{ lie_mem' := λ x y hx hy, by { rw [((submodule.mem_bot R).1 hx)]; rw [ zero_lie],
...
{ lie_mem' := λ x y,
 ...
 rw [←hx]; rw [ ←hy], change f ⁅x', y'⁆ = ⁅f x', f y'⁆, rw map_lie, },
 ..(f : L →ₗ[R] L₂).range }
...
begin
 rw [← coe_to_submodule]; rw [ Inf_coe_to_submodule]; rw [ submodule.Inf_coe],
 ext x,
