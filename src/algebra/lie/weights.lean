...
begin
 ...
 { use k,
 rw [← linear_map.comp_apply]; rw [ linear_map.commute_pow_left_of_commute h_comm_square]; rw [ linear_map.comp_apply]; rw [ hk]; rw [ linear_map.map_zero], },
 ...
 { rw [(hf_comm.pow_pow i j).eq]; rw [ linear_map.mul_apply]; rw [ linear_map.pow_map_zero_of_le hi hf₁]; rw [ linear_map.map_zero], },
 { rw [linear_map.mul_apply]; rw [ linear_map.pow_map_zero_of_le hj hf₂]; rw [ linear_map.map_zero], },
end
...
begin
 rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ lie_submodule.top_coe_submodule],
...
begin
 ...
 rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ lie_submodule.top_coe_submodule]; rw [ ← h₀]; rw [ coe_weight_space_of_top]; rw [ ← infi_max_gen_zero_eigenspace_eq_top_of_nilpotent R L M],
 refl,
end
...
by { rw [is_weight]; rw [ lie_hom.coe_zero]; rw [ zero_weight_space_eq_top_of_nilpotent], exact top_ne_bot, }
...
begin
 ...
 use k,
 ext ⟨m, hm⟩,
 rw [linear_map.zero_apply]; rw [ lie_submodule.coe_zero]; rw [ submodule.coe_eq_zero]; rw [ ← lie_submodule.to_endomorphism_restrict_eq_to_endomorphism]; rw [ linear_map.pow_restrict]; rw [ ← set_like.coe_eq_coe]; rw [ linear_map.restrict_apply]; rw [ submodule.coe_mk]; rw [ submodule.coe_zero],
 exact hk hm,
end
...
{ to_fun := λ x,
 ...
 map_smul' := λ t m, by { conv_lhs { congr, rw [lie_submodule.coe_smul]; rw [ lie_smul], }, refl, }, },
 map_add' := λ x y, by ext m; rw [linear_map.add_apply]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ subtype.coe_mk]; rw [ lie_submodule.coe_add]; rw [ lie_submodule.coe_add]; rw [ add_lie]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk],
 map_smul' := λ t x,
 begin
 simp only [ring_hom.id_apply],
 ext m,
 rw [linear_map.smul_apply]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ subtype.coe_mk]; rw [ lie_submodule.coe_smul]; rw [ smul_lie]; rw [ lie_submodule.coe_smul]; rw [ subtype.coe_mk],
 end, }
...
{ to_linear_map := root_space_weight_space_product_aux R L H M hχ,
 map_lie' := λ x y, by ext m; rw [root_space_weight_space_product_aux]; rw [ lie_hom.lie_apply]; rw [ lie_submodule.coe_sub]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk]; rw [ lie_submodule.coe_bracket]; rw [ lie_submodule.coe_bracket]; rw [ subtype.coe_mk]; rw [ lie_subalgebra.coe_bracket_of_module]; rw [ lie_subalgebra.coe_bracket_of_module]; rw [ lie_submodule.coe_bracket]; rw [ lie_subalgebra.coe_bracket_of_module]; rw [ lie_lie], }
...
{ lie_mem' := λ x y hx hy, by
 ...
 { rwa [root_space_product_tmul] at this; rwa [ subtype.coe_mk] at this; rwa [ subtype.coe_mk] at this, },
...
begin
 ...
 rw [hk] at h; rw [ lie_submodule.mem_bot] at h,
...
begin
 rw [← lie_subalgebra.coe_submodule_le_coe_submodule]; rw [ ← H.coe_to_lie_submodule]; rw [ coe_zero_root_subalgebra]; rw [ lie_submodule.coe_submodule_le_coe_submodule],
...
begin
 ...
 obtain ⟨k, hk⟩ := hx ⟨y, hy⟩,
 rw [← lie_skew] at hk; rw [ linear_map.map_neg] at hk; rw [ neg_eq_zero] at hk,
 use k + 1,
 rw [linear_map.iterate_succ]; rw [ linear_map.coe_comp]; rw [ function.comp_app]; rw [ to_endomorphism_apply_apply]; rw [ lie_subalgebra.coe_bracket_of_module]; rw [ submodule.coe_mk]; rw [ hk],
end
...
{ lie_mem := λ x m hm, by
 ...
 { rw [← lie_submodule.mem_coe_submodule]; rw [ ← coe_zero_root_subalgebra], exact x.property, },
 rw ← zero_add χ,
