...
lemma lie_mem_left (I : lie_ideal R L) (x y : L) (h : x ∈ I) : ⁅x, y⁆ ∈ I :=
 by { rw [←lie_skew]; rw [ ←neg_lie], apply lie_mem_right, assumption, }
...
begin
 rw [← lie_submodule.coe_to_submodule]; rw [ Inf_coe_to_submodule]; rw [ submodule.Inf_coe],
 ext m,
...
by rw [← mem_coe_submodule]; rw [ ← mem_coe_submodule]; rw [ ← mem_coe_submodule]; rw [ inf_coe_to_submodule]; rw [ submodule.mem_inf]

lemma mem_sup (x : M) : x ∈ N ⊔ N' ↔ ∃ (y ∈ N) (z ∈ N'), y + z = x :=
by { rw [← mem_coe_submodule]; rw [ sup_coe_to_submodule]; rw [ submodule.mem_sup], exact iff.rfl, }
...
{ rw [← subsingleton_iff_bot_eq_top]; rw [ ← subsingleton_iff_bot_eq_top]; rw [ ← coe_to_submodule_eq_iff]; rw [ top_coe_submodule]; rw [ bot_coe_submodule], },
...
begin
 ...
 { rw [not_nontrivial_iff_subsingleton]; rw [ lie_submodule.eq_bot_iff],
 rintros ⟨h⟩ m hm,
...
begin
 ...
 { intros x m hm, rw [← h]; rw [ mem_coe_submodule], exact lie_mem _ (subset_lie_span hm), },
 { rw [← coe_to_submodule_mk p h]; rw [ coe_to_submodule]; rw [ coe_to_submodule_eq_iff]; rw [ lie_span_eq], },
end
...
by rw [_root_.eq_bot_iff]; rw [ lie_span_le]; rw [ bot_coe]; rw [ subset_singleton_iff]
...
by { rw [set_like.ext'_iff]; rw [ lie_submodule.coe_to_submodule]; rw [ h]; rw [ submodule.map_coe], refl, }
...
begin
 apply le_antisymm,
 { erw [lie_submodule.lie_span_le]; erw [ submodule.map_coe]; erw [ h], },
 { rw [← set_like.coe_subset_coe]; rw [ ← h], exact lie_submodule.subset_lie_span, },
end
...
begin
 ...
 { intros h x y z hz, obtain ⟨w, hw⟩ := h x z, use w, rw [← hw]; rw [ hz], },
end
...
begin
 ...
 rw [← lie_submodule.mem_coe]; rw [ ← lie_ideal.coe_to_subalgebra]; rw [ h]; rw [ f.range_coe]; rw [ set.mem_range],
end
...
by rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ ker_coe_submodule]; rw [ lie_submodule.bot_coe_submodule]; rw [ linear_map.ker_eq_bot]; rw [ coe_to_linear_map]

@[simp] lemma range_coe_submodule : (f.range : submodule R L') = (f : L →ₗ[R] L').range := rfl

lemma range_eq_top : f.range = ⊤ ↔ function.surjective f :=
begin
 rw [← lie_subalgebra.coe_to_submodule_eq_iff]; rw [ range_coe_submodule]; rw [ lie_subalgebra.top_coe_submodule],
...
begin
 ...
 rw [ideal_range_eq_lie_span_range]; rw [ h]; rw [ ← lie_subalgebra.coe_to_submodule]; rw [ ← lie_submodule.coe_to_submodule_eq_iff]; rw [ lie_submodule.top_coe_submodule]; rw [ lie_subalgebra.top_coe_submodule]; rw [ lie_submodule.coe_lie_span_submodule_eq_iff],
 use ⊤,
 exact lie_submodule.top_coe_submodule,
end

lemma is_ideal_morphism_of_surjective (h : function.surjective f) : f.is_ideal_morphism :=
by rw [is_ideal_morphism_def]; rw [ f.ideal_range_eq_top_of_surjective h]; rw [ f.range_eq_top.mpr h]; rw [ lie_ideal.top_coe_lie_subalgebra]
...
lemma mem_map_of_surjective {y : L'} (h₁ : function.surjective f) (h₂ : y ∈ I.map f) :
 ∃ (x : I), f x = y :=
begin
 rw [← lie_submodule.mem_coe_submodule] at h₂; rw [ coe_map_of_surjective h₁] at h₂; rw [ submodule.mem_map] at h₂,
 obtain ⟨x, hx, rfl⟩ := h₂,
 use ⟨x, hx⟩,
 refl,
end
...
begin
 ...
 rw [f.coe_to_linear_map]; rw [ f.map_add]; rw [ f.mem_ker.mp hz₂]; rw [ add_zero], exact ⟨z₁, hz₁, rfl⟩,
end
...
begin
 ...
 { rw f.is_ideal_morphism_def at h,
 rw [← set_like.coe_subset_coe]; rw [ lie_submodule.inf_coe]; rw [ ← coe_to_subalgebra]; rw [ h],
 rintros y ⟨⟨x, h₁⟩, h₂⟩, rw ← h₁ at h₂ ⊢, exact mem_map h₂, },
end

@[simp] lemma comap_map_eq (h : ↑(map f I) = f '' I) : comap f (map f I) = I ⊔ f.ker :=
by rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ comap_coe_submodule]; rw [ I.map_coe_submodule f h]; rw [ lie_submodule.sup_coe_to_submodule]; rw [ f.ker_coe_submodule]; rw [ submodule.comap_map_eq]
...
by rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ lie_submodule.top_coe_submodule]; rw [ lie_ideal.comap_coe_submodule]; rw [ lie_ideal.incl_coe]; rw [ submodule.comap_subtype_self]

@[simp] lemma ker_incl : I.incl.ker = ⊥ :=
by rw [← lie_submodule.coe_to_submodule_eq_iff]; rw [ I.incl.ker_coe_submodule]; rw [ lie_submodule.bot_coe_submodule]; rw [ incl_coe]; rw [ submodule.ker_subtype]

@[simp] lemma incl_ideal_range : I.incl.ideal_range = I :=
begin
 rw [lie_hom.ideal_range_eq_lie_span_range]; rw [ ← lie_subalgebra.coe_to_submodule]; rw [ ← lie_submodule.coe_to_submodule_eq_iff]; rw [ incl_range]; rw [ coe_to_lie_subalgebra_to_submodule]; rw [ lie_submodule.coe_lie_span_submodule_eq_iff],
 use I,
end

lemma incl_is_ideal_morphism : I.incl.is_ideal_morphism :=
begin
 rw [I.incl.is_ideal_morphism_def]; rw [ incl_ideal_range],
...
by rw [ker]; rw [ eq_bot_iff]; rw [ lie_submodule.map_le_iff_le_comap]
