...
begin
 ...
 use [x', y', hu],
 rw [mul_comm]; rw [ is_localization.mul_mk'_eq_mk'_of_mul],
...
⟨λ X ⟨p, hp₁, hp₂⟩, begin
 ...
 (is_fraction_ring.injective R $ fraction_ring R) hp₁ y x _ hp₂ (by rw [mul_comm]; rw [ he]),
 have : is_unit y,
 { rw [is_unit_iff_dvd_one]; rw [ ← one_pow],
 exact (dvd_gcd this $ dvd_refl y).trans (gcd_pow_left_dvd_pow_gcd.trans $
 pow_dvd_pow_of_dvd (is_unit_iff_dvd_one.1 hg) _) },
 use x * (this.unit⁻¹ : _),
 erw [map_mul]; erw [ ← units.coe_map_inv]; erw [ eq_comm]; erw [ units.eq_mul_inv_iff_mul_eq],
 exact he,
end⟩
