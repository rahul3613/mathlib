...
by { rw [← lcm_dedup]; rw [ dedup_ext.2]; rw [ lcm_dedup]; rw [ lcm_add], simp }
...
by { rw [← lcm_dedup]; rw [ dedup_ext.2]; rw [ lcm_dedup]; rw [ lcm_cons], simp }
...
by { rw [← gcd_dedup]; rw [ dedup_ext.2]; rw [ gcd_dedup]; rw [ gcd_add], simp }
...
by { rw [← gcd_dedup]; rw [ dedup_ext.2]; rw [ gcd_dedup]; rw [ gcd_cons], simp }

end

lemma extract_gcd' (s t : multiset α) (hs : ∃ x, x ∈ s ∧ x ≠ (0 : α))
 (ht : s = t.map ((*) s.gcd)) : t.gcd = 1 :=
((@mul_right_eq_self₀ _ _ s.gcd _).1 $ by conv_lhs { rw [← normalize_gcd]; rw [ ← gcd_map_mul]; rw [ ← ht] })
...
begin
 ...
 rw [map_replicate]; rw [ eq_replicate]; rw [ mul_one]; rw [ s.gcd_eq_zero_iff.2 h]; rw [ ←nsmul_singleton]; rw [ ←gcd_dedup],
 rw [dedup_nsmul (card_pos.2 hs).ne']; rw [ dedup_singleton]; rw [ gcd_singleton],
 ...
 rw map_pmap, conv_lhs { rw [← s.map_id]; rw [ ← s.pmap_eq_map _ _ (λ _, id)] },
 congr' with x hx, rw [id]; rw [ ← hf hx] },
end
