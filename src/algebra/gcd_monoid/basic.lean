...
{ to_fun := λ x, x * norm_unit x,
 map_zero' := by simp,
 map_one' := by rw [norm_unit_one]; rw [ units.coe_one]; rw [ mul_one],
 map_mul' := λ x y,
 classical.by_cases (λ hx : x = 0, by rw [hx]; rw [ zero_mul]; rw [ zero_mul]; rw [ zero_mul]) $ λ hx,
 classical.by_cases (λ hy : y = 0, by rw [hy]; rw [ mul_zero]; rw [ zero_mul]; rw [ mul_zero]) $ λ hy,
...
begin
 ...
 { rw [norm_unit_zero]; rw [ zero_mul]; rw [ norm_unit_zero] },
 { rw [norm_unit_mul h (units.ne_zero _)]; rw [ norm_unit_coe_units]; rw [ mul_inv_eq_one] }
end
...
iff.intro
 ...
 by rw [h] at ha hb; rw [ zero_mul] at ha hb; exact ⟨ha, hb⟩)
 (assume ⟨ha, hb⟩, by
...
begin
 ...
 have h2 : d₁ ^ k ∣ a * b, { use d₂ ^ k, rw [h]; rw [ hc], exact mul_pow d₁ d₂ k },
 rw mul_comm at h2,
 have h3 : d₁ ^ k ∣ a,
...
begin
 ...
 { exfalso, revert h, rw [ha]; rw [ zero_mul]; rw [ pow_zero], apply zero_ne_one },
 { rw zero_pow hk } },
 by_cases hb : b = 0,
 { use 1, rw [one_pow],
 ...
 rw [units.coe_mk_of_mul_eq_one]; rw [ one_mul] },
 ...
 rw [units.coe_mk_of_mul_eq_one]; rw [ ha']
end
...
begin
 ...
 exact ⟨1, 1, by rw [h]; rw [ zero_mul], by rw [h]; rw [ zero_mul], gcd_one_left' 1⟩ },
...
begin
 ...
 rw [← mul_dvd_mul_iff_left h]; rw [ (gcd_mul_lcm a b).dvd_iff_dvd_left]; rw [ ←(gcd_mul_right' c a b).dvd_iff_dvd_right]; rw [ dvd_gcd_iff]; rw [ mul_comm b c]; rw [ mul_dvd_mul_iff_left h1]; rw [ mul_dvd_mul_iff_right h2]; rw [ and_comm] }
end
...
iff.intro
 ...
 by rw [lcm_units_coe_left]; rw [ normalize_coe_units])
...
by rw [gcd_comm _ a]; rw [ gcd_comm _ a]; rw [ gcd_eq_of_dvd_sub_right h]
...
{ norm_unit := λ a, if a = 0 then 1 else
 ...
 { rw [if_neg (mul_ne_zero ha hb)]; rw [ if_neg ha]; rw [ if_neg hb]; rw [ units.ext_iff]; rw [ units.coe_mul],
 ...
 rw [map_mk_unit_aux hinv a]; rw [ map_mk_unit_aux hinv (a * b)]; rw [ map_mk_unit_aux hinv b]; rw [ ← monoid_hom.map_mul]; rw [ associates.mk_mul_mk] },
 norm_unit_coe_units := λ u, by
 { nontriviality α,
 rw [if_neg (units.ne_zero u)]; rw [ units.ext_iff],
 apply mul_left_cancel₀ (units.ne_zero u),
 rw [units.mul_inv]; rw [ map_mk_unit_aux hinv u]; rw [ associates.mk_eq_mk_iff_associated.2 (associated_one_iff_is_unit.2 ⟨u, rfl⟩)]; rw [ associates.mk_one]; rw [ monoid_hom.map_one] } }
...
{ gcd := gcd,
 ...
 { rw [this]; rw [ normalize_idem] },
 rw ←normalize_gcd at this,
 rwa [normalize.map_mul] at h2 ; rwa [ normalize_gcd] at h2 ; rwa [ mul_right_inj' h1] at h2 } },
 gcd_mul_lcm := λ a b, by
 { split_ifs with a0,
 { rw [mul_zero]; rw [ a0]; rw [ zero_mul] },
...
{ lcm := lcm,
 ...
 gcd_mul_lcm := λ a b, by
 { split_ifs,
 { rw [h]; rw [ eq_zero_of_zero_dvd (dvd_lcm_left _ _)]; rw [ mul_zero]; rw [ zero_mul] },
 { rw [h_1]; rw [ eq_zero_of_zero_dvd (dvd_lcm_right _ _)]; rw [ mul_zero] },
 rw [mul_comm]; rw [ ←classical.some_spec (exists_gcd a b)] },
 ...
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (exists_gcd a b)]; rw [ mul_comm]; rw [ mul_dvd_mul_iff_right h],
 apply dvd_lcm_right },
 ...
 rw [con] at h; rw [ zero_dvd_iff] at h; rw [ mul_eq_zero] at h,
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (exists_gcd a b)]; rw [ mul_dvd_mul_iff_right h_1],
 apply dvd_lcm_left },
 ...
 rw [con] at h; rw [ zero_dvd_iff] at h; rw [ mul_eq_zero] at h,
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (exists_gcd c b)],
...
{ lcm := lcm,
 ...
 { split_ifs with h h_1,
 { rw [h]; rw [ eq_zero_of_zero_dvd (dvd_lcm_left _ _)]; rw [ mul_zero]; rw [ zero_mul] },
 { rw [h_1]; rw [ eq_zero_of_zero_dvd (dvd_lcm_right _ _)]; rw [ mul_zero]; rw [ mul_zero] },
 rw [mul_comm]; rw [ ←classical.some_spec (exists_gcd a b)],
 exact normalize_associated (a * b) },
 ...
 erw [← normalize.map_mul]; erw [ ← classical.some_spec (exists_gcd a b)]; erw [ normalize_idem] },
 lcm_zero_left := λ a, eq_zero_of_zero_dvd (dvd_lcm_left _ _),
 lcm_zero_right := λ a, eq_zero_of_zero_dvd (dvd_lcm_right _ _),
 gcd_dvd_left := λ a b, by
 { split_ifs,
 ...
 rw [con] at h; rw [ zero_dvd_iff] at h; rw [ mul_eq_zero] at h,
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (exists_gcd a b)]; rw [ normalize_dvd_iff]; rw [ mul_comm]; rw [ mul_dvd_mul_iff_right h],
 apply dvd_lcm_right },
 gcd_dvd_right := λ a b, by
 { split_ifs,
 { exact (normalize_associated _).dvd },
 ...
 rw [con] at h; rw [ zero_dvd_iff] at h; rw [ mul_eq_zero] at h,
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (exists_gcd a b)]; rw [ normalize_dvd_iff]; rw [ mul_dvd_mul_iff_right h_1],
 apply dvd_lcm_left },
 dvd_gcd := λ a b c ac ab, by
 { split_ifs,
 ...
 rw [con] at h; rw [ zero_dvd_iff] at h; rw [ mul_eq_zero] at h,
 cases h; tauto },
 rw [← mul_dvd_mul_iff_left h0]; rw [ ← classical.some_spec (dvd_normalize_iff.2 (lcm_dvd (dvd.intro b rfl) (dvd.intro_left c rfl)))]; rw [ dvd_normalize_iff],
 rcases ab with ⟨d, rfl⟩,
 rw mul_eq_zero at h_1,
 push_neg at h_1,
 rw [mul_comm a]; rw [ ← mul_assoc]; rw [ mul_dvd_mul_iff_right h_1.1],
...
{ norm_unit := λ x, if h : x = 0 then 1 else (units.mk0 x h)⁻¹,
 ...
 norm_unit_coe_units := λ u, by { rw [dif_neg (units.ne_zero _)]; rw [ units.mk0_coe], apply_instance },
 ...
 rw [if_neg (not_and_of_not_left _ ha)]; rw [ one_mul]; rw [ if_neg (not_or ha hb)],
