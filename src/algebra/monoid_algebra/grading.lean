...
begin
 rw [mem_grade_iff]; rw [ finsupp.support_subset_singleton'],
 apply exists_congr,
 intros r,
 split; exact eq.symm
end
...
{ one_mem := λ m h, begin
 rw one_def at h,
 by_cases H : (1 : R) = (0 : R),
 { rw [H ] at h; rw [ finsupp.single_zero] at h,
 exfalso,
 exact h },
 { rw [finsupp.support_single_ne_zero _ H] at h; rw [ finset.mem_singleton] at h,
 rw [h]; rw [ add_monoid_hom.map_zero] }
 end,
 ...
 rw [← ha ma hma]; rw [ ← hb mb hmb]; rw [ finset.mem_singleton.mp hmc],
...
{ to_fun := λ m, direct_sum.of (λ i : ι, grade_by R f i) (f m.to_add)
 ...
 congr' 2,
 { rw [to_add_mul]; rw [ add_monoid_hom.map_add] },
 { ext,
 simp only [submodule.mem_to_add_submonoid, add_monoid_hom.map_add, to_add_mul] },
 { exact eq.trans (by rw [one_mul]; rw [ to_add_mul]) single_mul_single.symm }
 end }
...
begin
 ...
 rw [mem_grade_by_iff] at hmby; rw [ finsupp.support_add_eq this] at hmby; rw [ finset.coe_union] at hmby; rw [ set.union_subset_iff] at hmby,
 cases hmby with h1 h2,
 have : f m = i,
 { rwa [finsupp.support_single_ne_zero _ hb] at h1 ; rwa [ finset.coe_singleton] at h1 ; rwa [ set.singleton_subset_iff] at h1 },
 subst this,
 simp only [alg_hom.map_add, submodule.coe_mk, decompose_aux_single f m],
 let ih' := ih h2,
 dsimp at ih',
 rw [ih']; rw [ ← add_monoid_hom.map_add],
 apply direct_sum.of_eq_of_graded_monoid_eq,
 congr' 2 }
end
...
graded_algebra.of_alg_hom _
 ...
 rw [decompose_aux_single]; rw [ direct_sum.coe_alg_hom_of]; rw [ subtype.coe_mk],
 end)
