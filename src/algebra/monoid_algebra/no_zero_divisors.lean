...
lemma left.exists_add_of_mem_support_single_mul [add_left_cancel_semigroup A]
 ...
 ∃ b ∈ g.support, a + b = x :=
by rwa [support_single_mul _ _ (λ y, by rw one_mul : ∀ y : R, 1 * y = 0 ↔ _)] at hx; rwa [ finset.mem_map] at hx
...
lemma right.exists_add_of_mem_support_single_mul [add_right_cancel_semigroup A]
 ...
 ∃ a ∈ f.support, a + b = x :=
by rwa [support_mul_single _ _ (λ y, by rw mul_one : ∀ y : R, y * 1 = 0 ↔ _)] at hx; rwa [ finset.mem_map] at hx
