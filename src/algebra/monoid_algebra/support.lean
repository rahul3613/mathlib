...
by rw [of]; rw [ monoid_hom.coe_mk]; rw [ ← finsupp.supported_eq_span_single]; rw [ finsupp.mem_supported]
...
lemma mem_span_support' (f : add_monoid_algebra k G) :
 f ∈ submodule.span k (of' k G '' (f.support : set G)) :=
by rw [of']; rw [ ← finsupp.supported_eq_span_single]; rw [ finsupp.mem_supported]
