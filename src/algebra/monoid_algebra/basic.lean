...
begin
 conv_rhs { rw [← sum_single a]; rw [ ← sum_single b] },
...
(sum_single_index (by simp only [zero_mul, single_zero, sum_zero])).trans
 (sum_single_index (by rw [mul_zero]; rw [ single_zero]))
...
{ to_fun := λ f,
 ...
 simp_rw [finsupp.mul_sum, finsupp.sum_mul, smul_mul_smul, ← f.map_mul, mul_def, sum_comm a₂ a₁, sum_sum_index h₁ h₂, sum_single_index (h₁ _)],
 end,
...
by { ext, rw [single_one_mul_apply]; rw [ mul_single_one_apply]; rw [ mul_comm] }

/-- `finsupp.single 1` as a `ring_hom` -/
@[simps] def single_one_ring_hom [semiring k] [mul_one_class G] : k →+* monoid_algebra k G :=
{ map_one' := rfl,
 map_mul' := λ x y, by rw [single_add_hom]; rw [ single_mul_single]; rw [ one_mul],
...
ring_hom.coe_add_monoid_hom_injective $ add_hom_ext $ λ a b,
 by rw [← one_mul a]; rw [ ← mul_one b]; rw [ ← single_mul_single]; rw [ f.coe_add_monoid_hom]; rw [ g.coe_add_monoid_hom]; rw [ f.map_mul]; rw [ g.map_mul]; rw [ h₁]; rw [ h_of]
...
by rw [of_apply]; rw [ ← lift_symm_apply]; rw [ equiv.symm_apply_apply]

@[simp] lemma lift_single (F : G →* A) (a b) :
 lift k G A F (single a b) = b • F a :=
by rw [lift_def]; rw [ lift_nc_single]; rw [ algebra.smul_def]; rw [ ring_hom.coe_add_monoid_hom]
...
{ to_fun := f,
 ...
 erw [algebra_map_smul (monoid_algebra k G) r]; erw [ algebra_map_smul (monoid_algebra k G) r]; erw [ f.map_smul]; erw [ h g v]; erw [ of_apply],
...
finset.cons_induction_on s rfl $ λ a s has ih, by rw [prod_cons has]; rw [ ih]; rw [ single_mul_single]; rw [ prod_cons has]; rw [ prod_cons has]
...
calc (f * g) x = sum f (λ a b, (single a b * g) x) :
 by rw [← finsupp.sum_apply]; rw [ ← finsupp.sum_mul]; rw [ f.sum_single]
...
calc (f * g) x = sum g (λ a b, (f * single a b) x) :
 by rw [← finsupp.sum_apply]; rw [ ← finsupp.mul_sum]; rw [ g.sum_single]
...
| (n+1) :=
by rw [pow_succ]; rw [ pow_succ]; rw [ single_pow n]; rw [ single_mul_single]; rw [ add_comm]; rw [ add_nsmul]; rw [ one_nsmul]
...
{ map_one' := rfl,
 map_mul' := λ x y, by rw [single_add_hom]; rw [ single_mul_single]; rw [ zero_add],
...
lemma prod_single [comm_semiring k] [add_comm_monoid G]
 {s : finset ι} {a : ι → G} {b : ι → k} :
 (∏ i in s, single (a i) (b i)) = single (∑ i in s, a i) (∏ i in s, b i) :=
finset.cons_induction_on s rfl $ λ a s has ih, by rw [prod_cons has]; rw [ ih]; rw [ single_mul_single]; rw [ sum_cons has]; rw [ prod_cons has]
