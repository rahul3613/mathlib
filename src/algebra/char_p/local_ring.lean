...
begin
 ...
 begin
 rw [nat.cast_pow]; rw [ ←@mul_one R _ (r ^ n)]; rw [ mul_comm]; rw [ ←(classical.some_spec a_unit.exists_left_inv)]; rw [ mul_assoc]; rw [ ←nat.cast_pow]; rw [ ←nat.cast_mul]; rw [ ←q_eq_a_mul_rn]; rw [ char_p.cast_eq_zero R q],
 simp,
 end,
