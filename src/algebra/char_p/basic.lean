...
begin
 ...
 { simp_rw [h.add_pow, ←nat.Ico_zero_eq_range, nat.Ico_succ_right, Icc_eq_cons_Ico (zero_le _), finset.sum_cons, Ico_eq_cons_Ioo (pow_pos hp.pos _), finset.sum_cons, tsub_self, tsub_zero, pow_zero, nat.choose_zero_right, nat.choose_self, nat.cast_one, mul_one, one_mul, ←add_assoc] },
 { congr' 1,
...
@[simp] lemma char_p.cast_card_eq_zero [add_group_with_one R] [fintype R] :
 (fintype.card R : R) = 0 :=
by rw [← nsmul_one]; rw [ card_nsmul_eq_zero]

lemma char_p.add_order_of_one (R) [semiring R] : char_p R (add_order_of (1 : R)) :=
⟨λ n, by rw [← nat.smul_one_eq_coe]; rw [ add_order_of_dvd_iff_nsmul_eq_zero]⟩
...
begin
 ...
 { rw [← neg_eq_zero]; rw [ ← int.cast_neg]; rw [ ← dvd_neg],
 lift -a to ℕ using neg_nonneg.mpr (le_of_lt h) with b,
 rw [int.cast_coe_nat]; rw [ char_p.cast_eq_zero_iff R p]; rw [ int.coe_nat_dvd] },
...
lemma char_p.int_cast_eq_int_cast [add_group_with_one R] (p : ℕ) [char_p R p] {a b : ℤ} :
 (a : R) = b ↔ a ≡ b [ZMOD p] :=
by rw [eq_comm]; rw [ ←sub_eq_zero]; rw [ ←int.cast_sub]; rw [ char_p.int_cast_eq_zero_iff R p]; rw [ int.modeq_iff_dvd]

lemma char_p.nat_cast_eq_nat_cast [add_group_with_one R] (p : ℕ) [char_p R p] {a b : ℕ} :
 (a : R) = b ↔ a ≡ b [MOD p] :=
begin
 rw [←int.cast_coe_nat]; rw [ ←int.cast_coe_nat b],
...
⟨λ x, by rw [zero_dvd_iff]; rw [ ← nat.cast_zero]; rw [ nat.cast_inj]⟩
...
classical.by_cases
 ...
 (not_imp_of_and_not ⟨by rwa [← nat.mod_add_div x (nat.find (not_forall.1 H))] at H1; rwa [ nat.cast_add] at H1; rwa [ nat.cast_mul] at H1; rwa [ of_not_not (not_not_of_not_imp $ nat.find_spec (not_forall.1 H))] at H1; rwa [ zero_mul] at H1; rwa [ add_zero] at H1, H2⟩)),
 λ H1, by rw [← nat.mul_div_cancel' H1]; rw [ nat.cast_mul]; rw [ of_not_not (not_not_of_not_imp $ nat.find_spec (not_forall.1 H))]; rw [ zero_mul]⟩⟩⟩)
...
begin
 suffices : (2 : R) ≠ 0,
 { symmetry, rw [ne.def]; rw [ ← sub_eq_zero]; rw [ sub_neg_eq_add], exact this },
 assume h,
 rw [show (2 : R) = (2 : ℕ)] at h; rw [ by norm_cast] at h,
...
begin
 ...
 rw [← add_pow_char]; rw [ add_left_neg]; rw [ zero_pow (fact.out (nat.prime p)).pos],
end
...
begin
 ...
 rw [← add_pow_char_pow]; rw [ add_left_neg]; rw [ zero_pow (pow_pos (fact.out (nat.prime p)).pos _)],
end
...
begin
 ...
 { refine ⟨λ h, _, λ h, _⟩,
 { rw [pow_succ] at h; rw [ mul_assoc] at h; rw [ pow_mul'] at h; rw [ ← frobenius_def] at h; rw [ ← frobenius_one p] at h,
 exact hk.1 (frobenius_inj R p h) },
 { rw [pow_mul']; rw [ h]; rw [ one_pow] } }
end
...
subsingleton.intro $
suffices ∀ (r : R), r = 0,
 from assume a b, show a = b, by rw [this a]; rw [ this b],
assume r,
...
by { intros h, apply zero_ne_one' R, symmetry, rw [←nat.cast_one]; rw [ ring_char.spec]; rw [ h], }

lemma nontrivial_of_char_ne_one {v : ℕ} (hv : v ≠ 1) [hr : char_p R v] :
 nontrivial R :=
⟨⟨(1 : ℕ), 0, λ h, hv $ by rwa [char_p.cast_eq_zero_iff _ v] at h; rwa [ nat.dvd_one] at h; assumption ⟩⟩
...
lemma ring.two_ne_zero {R : Type*} [non_assoc_semiring R] [nontrivial R] (hR : ring_char R ≠ 2) :
 (2 : R) ≠ 0 :=
begin
 rw [ne.def]; rw [ (by norm_cast : (2 : R) = (2 : ℕ))]; rw [ ring_char.spec]; rw [ nat.dvd_prime nat.prime_two],
...
{ cast_eq_zero_iff :=
 begin
 have H : (n : R) = 0, by { rw [← hn]; rw [ char_p.cast_card_eq_zero] },
 intro k,
 split,
 { intro h,
 ...
 exact ⟨0⟩, },
 { rintro ⟨k, rfl⟩, rw [nat.cast_mul]; rw [ H]; rw [ zero_mul] }
 end }
...
begin
 ...
 have hcpn : c ∣ p ^ n,
 { rw [← char_p.cast_eq_zero_iff R c]; rw [ ← hn]; rw [ char_p.cast_card_eq_zero], },
 obtain ⟨i, hi, hc⟩ : ∃ i ≤ n, c = p ^ i, by rwa nat.dvd_prime_pow hp.1 at hcpn,
 obtain rfl : i = n,
 { apply hR i hi, rw [← nat.cast_pow]; rw [ ← hc]; rw [ char_p.cast_eq_zero] },
 rwa ← hc
end
...
begin
 ...
 iterate 3 { rw [ha] at hf; rw [ hb] at hf; rw [ sub_self] at hf, tauto, }, -- 6 goals remain
 all_goals { rw [ha]; rw [ hb], norm_num, }, },
 ...
 { rw [hh] at h'; rw [ (by norm_cast : ((1 : ℤ) : R) = 1)] at h', exact one_ne_zero h', },
 { rw [hh] at h''; rw [ (by norm_cast : ((1 : ℤ) : R) = 1)] at h'', exact one_ne_zero h'', },
 { rw [hh] at h'; rw [ (by norm_cast : ((2 : ℤ) : R) = 2)] at h', exact ring.two_ne_zero hR h', },
 { rw [hh] at h''; rw [ (by norm_cast : ((2 : ℤ) : R) = 2)] at h'', exact ring.two_ne_zero hR h'', },
end
...
by rwa [←char_p.cast_eq_zero_iff R p k]; rwa [ ←ne.def]; rwa [ ←ne_zero_iff]
