...
{ nat_cast := λ n, ((n : R) : ℍ[R, c₁, c₂]),
 ...
 show ↑↑_ = -↑↑_, by rw [int.cast_neg]; rw [ int.cast_coe_nat]; rw [ coe_neg],
 one := 1,
...
{ smul := (•),
 ...
 map_mul' := λ x y, by rw [map_mul]; rw [ coe_mul],
 map_add' := λ x y, by rw [map_add]; rw [ coe_add],
...
by { rw [rank_eq_card_basis (basis_one_i_j_k c₁ c₂)]; rw [ fintype.card_fin], norm_num }

lemma finrank_eq_four [strong_rank_condition R] : finite_dimensional.finrank R ℍ[R, c₁, c₂] = 4 :=
have cardinal.to_nat 4 = 4,
 by rw [←cardinal.to_nat_cast 4]; rw [ nat.cast_bit0]; rw [ nat.cast_bit0]; rw [ nat.cast_one],
by rw [finite_dimensional.finrank]; rw [ rank_eq_four]; rw [ this]
...
by rw [← coe_commutes]; rw [ coe_mul_eq_smul]

@[norm_cast, simp] lemma coe_algebra_map : ⇑(algebra_map R ℍ[R, c₁, c₂]) = coe := rfl

lemma smul_coe : x • (y : ℍ[R, c₁, c₂]) = ↑(x * y) := by rw [coe_mul]; rw [ coe_mul_eq_smul]
...
lemma star_add_self' : star a + a = ↑(2 * a.re) := by rw [add_comm]; rw [ self_add_star']
...
{ to_fun := λ a, (a * star a).re,
 map_zero' := by rw [star_zero]; rw [ zero_mul]; rw [ zero_re],
 map_one' := by rw [star_one]; rw [ one_mul]; rw [ one_re],
 map_mul' := λ x y, coe_injective $ by conv_lhs { rw [← mul_star_eq_coe]; rw [ star_mul]; rw [ mul_assoc]; rw [ ← mul_assoc y]; rw [ y.mul_star_eq_coe]; rw [ coe_commutes]; rw [ ← mul_assoc]; rw [ x.mul_star_eq_coe]; rw [ ← coe_mul] } }
...
by rw [norm_sq_def]; rw [ star_coe]; rw [ ← coe_mul]; rw [ coe_re]; rw [ sq]
...
by rw [←coe_nat_cast]; rw [ norm_sq_coe]

@[norm_cast] lemma norm_sq_int_cast (z : ℤ) : norm_sq (z : ℍ[R]) = z^2 :=
by rw [←coe_int_cast]; rw [ norm_sq_coe]

@[simp] lemma norm_sq_neg : norm_sq (-a) = norm_sq a :=
by simp only [norm_sq_def, star_neg, neg_mul_neg]

lemma self_mul_star : a * star a = norm_sq a := by rw [mul_star_eq_coe]; rw [ norm_sq_def]

lemma star_mul_self : star a * a = norm_sq a := by rw [star_comm_self']; rw [ self_mul_star]
...
calc norm_sq (a + b) = (norm_sq a + (a * star b).re) + ((b * star a).re + norm_sq b)
 ...
 : by rw [←add_re]; rw [ ←star_mul_star a b]; rw [ self_add_star']; rw [ coe_re]
...
begin
 ...
 rw [norm_sq_def'] at h; rw [ add_eq_zero_iff'] at h; rw [ add_eq_zero_iff'] at h; rw [ add_eq_zero_iff'] at h,
...
{ eq_zero_or_eq_zero_of_mul_eq_zero := λ a b hab,
 have norm_sq a * norm_sq b = 0, by rwa [← norm_sq.map_mul]; rwa [ norm_sq_eq_zero],
...
begin
 ...
 { simp },
 { rw [←star_mul_self]; rw [ sq]; rw [ mul_left_inj' hq0]; rw [ eq_comm] }
end
...
begin
 ...
 obtain rfl | hq0 := eq_or_ne a 0,
 { simp },
 rw [←star_mul_self]; rw [ ←mul_neg]; rw [ ←neg_sq]; rw [ sq]; rw [ mul_left_inj' (neg_ne_zero.mpr hq0)]; rw [ eq_comm],
end
...
{ inv := has_inv.inv,
 inv_zero := by rw [has_inv_inv]; rw [ star_zero]; rw [ smul_zero],
 mul_inv_cancel := λ a ha, by rw [has_inv_inv]; rw [ algebra.mul_smul_comm]; rw [ self_mul_star]; rw [ smul_coe]; rw [ inv_mul_cancel (norm_sq_ne_zero.2 ha)]; rw [ coe_one],
 .. quaternion.nontrivial,
...
{ rat_cast := λ q, ↑(q : R),
 rat_cast_mk := λ n d hd h, by rw [rat.cast_mk']; rw [ coe_mul]; rw [ coe_int_cast]; rw [ coe_inv]; rw [ coe_nat_cast],
 qsmul := (•),
...
by rw [←coe_rat_cast]; rw [ norm_sq_coe]
...
by rw [mk_quaternion]; rw [ pow_four]
...
by rw [mk_univ_quaternion]; rw [ pow_four]
