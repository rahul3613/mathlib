...
{ pure_bind := λ _ _ _ _, rfl,
 ...
 (λ x y ih1 ih2, by rw [mul_bind]; rw [ mul_bind]; rw [ mul_bind]; rw [ ih1]; rw [ ih2]),
 id_map := λ α x, free_magma.rec_on_pure x (λ _, rfl)
 (λ x y ih1 ih2, by rw [map_mul']; rw [ ih1]; rw [ ih2]) }
...
{ id_traverse := λ α x, free_magma.rec_on_pure x (λ x, rfl)
 (λ x y ih1 ih2, by rw [traverse_mul]; rw [ ih1]; rw [ ih2]; rw [ mul_map_seq]),
 ...
 (λ x y ih1 ih2, by resetI; rw [traverse_mul]; rw [ ih1]; rw [ ih2]; rw [ traverse_mul];
 ...
 (λ x y ih1 ih2, by simp only [traverse_mul] with functor_norm; rw [ih1]; rw [ ih2]),
 traverse_eq_map_id := λ α β f x, free_magma.rec_on_pure x (λ _, rfl)
 (λ x y ih1 ih2, by rw [traverse_mul]; rw [ ih1]; rw [ ih2]; rw [ map_mul']; rw [ mul_map_seq]; refl),
...
{ mul := λ x y,
 ...
 { rw [← quot_mk_assoc]; rw [ quot_mk_assoc_left]; rw [ quot_mk_assoc] } },
 { rintro a₁ a₂ b (⟨c, d, e⟩ | ⟨c, d, e, f⟩); simp only,
 { simp only [quot_mk_assoc, quot_mk_assoc_left] },
 { rw [quot_mk_assoc]; rw [ quot_mk_assoc]; rw [ quot_mk_assoc_left]; rw [ quot_mk_assoc_left]; rw [ quot_mk_assoc_left]; rw [ ← quot_mk_assoc c d]; rw [ ← quot_mk_assoc c d]; rw [ quot_mk_assoc_left] } }
 end,
...
{ pure_bind := λ _ _ _ _, rfl,
 ...
 (λ x y ih1 ih2, by rw [mul_bind]; rw [ mul_bind]; rw [ mul_bind]; rw [ ih1]; rw [ ih2]),
 id_map := λ α x, rec_on_pure x (λ _, rfl) (λ x y ih1 ih2, by rw [map_mul']; rw [ ih1]; rw [ ih2]) }
...
{ id_traverse := λ α x, free_semigroup.rec_on_mul x (λ x, rfl)
 ...
 (λ x, by simp only [traverse_pure] with functor_norm)
 (λ x y ih1 ih2, by resetI; simp only [traverse_mul] with functor_norm; rw [ih1]; rw [ ih2]),
...
free_magma.rec_on_mul x (λ x, rfl) $ λ x y hx hy,
 by rw [map_mul]; rw [ free_semigroup.length_mul]; rw [ length]; rw [ hx]; rw [ hy]
