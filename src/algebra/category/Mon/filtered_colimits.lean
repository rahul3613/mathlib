...
{ one_mul := λ x, begin
 ...
 rw [colimit_one_eq F j]; rw [ colimit_mul_mk_eq F ⟨j, 1⟩ ⟨j, x⟩ j (𝟙 j) (𝟙 j)]; rw [ monoid_hom.map_one]; rw [ one_mul]; rw [ F.map_id]; rw [ id_apply],
 end,
 mul_one := λ x, begin
 apply quot.induction_on x, clear x, intro x, cases x with j x,
 rw [colimit_one_eq F j]; rw [ colimit_mul_mk_eq F ⟨j, x⟩ ⟨j, 1⟩ j (𝟙 j) (𝟙 j)]; rw [ monoid_hom.map_one]; rw [ mul_one]; rw [ F.map_id]; rw [ id_apply],
 end,
 ...
 rw [colimit_mul_mk_eq F ⟨j₁, x⟩ ⟨j₂, y⟩ _ (first_to_max₃ j₁ j₂ j₃) (second_to_max₃ j₁ j₂ j₃)]; rw [ colimit_mul_mk_eq F ⟨max₃ j₁ j₂ j₃, _⟩ ⟨j₃, z⟩ _ (𝟙 _) (third_to_max₃ j₁ j₂ j₃)]; rw [ colimit_mul_mk_eq F ⟨j₂, y⟩ ⟨j₃, z⟩ _ (second_to_max₃ j₁ j₂ j₃) (third_to_max₃ j₁ j₂ j₃)]; rw [ colimit_mul_mk_eq F ⟨j₁, x⟩ ⟨max₃ j₁ j₂ j₃, _⟩ _ (first_to_max₃ j₁ j₂ j₃) (𝟙 _)],
...
{ to_fun := (types.colimit_cocone (F ⋙ forget Mon)).ι.app j,
 ...
 rw [F.map_id]; rw [ id_apply]; rw [ id_apply], refl,
 end }
...
{ to_fun := (types.colimit_cocone_is_colimit (F ⋙ forget Mon)).desc ((forget Mon).map_cocone t),
 ...
 rw [monoid_hom.map_mul]; rw [ t.w_apply]; rw [ t.w_apply],
 end }
...
{ mul_comm := λ x y, begin
 ...
 rw [colimit_mul_mk_eq _ x y k f g]; rw [ colimit_mul_mk_eq _ y x k g f],
 dsimp,
 rw mul_comm,
 end
 ..M.monoid }
