...
begin
 ...
 { simp only [map_zero], },
 { rw [linear_map.base_change_tmul]; rw [ Module.id_apply], },
...
{ smul := λ s g,
 ...
 map_smul' := λ r (t : S), by rw [ring_hom.id_apply]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ ←linear_map.map_smul]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ smul_eq_mul]; rw [ smul_eq_mul]; rw [ mul_assoc] } }
...
{ to_fun := λ (y : Y),
 ...
 map_smul' := λ r (s : S), by rw [ring_hom.id_apply]; rw [ ←g.map_smul]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ smul_eq_mul]; rw [ mul_smul]; rw [ @restrict_scalars.smul_def _ _ _ _ f Y] },
 map_add' := λ (y1 y2 : Y), linear_map.ext $ λ (s : S),
 by rw [linear_map.add_apply]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ smul_add]; rw [ map_add],
...
{ to_fun := λ (y : Y), (g y).to_fun (1 : S),
 ...
 map_smul' := λ r (y : Y), by rw [linear_map.to_fun_eq_coe]; rw [ linear_map.to_fun_eq_coe]; rw [ ring_hom.id_apply]; rw [ ←linear_map.map_smul]; rw [ restrict_scalars.smul_def f r y]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ smul_eq_mul]; rw [ mul_one]; rw [ linear_map.map_smul]; rw [ coextend_scalars.smul_apply]; rw [ one_mul], }
...
{ app := λ Y,
 ...
 map_smul' := λ r (s : S), by rw [ring_hom.id_apply]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ smul_eq_mul]; rw [ mul_smul]; rw [ restrict_scalars.smul_def f] },
 map_add' := λ y1 y2, linear_map.ext $ λ (s : S), by rw [linear_map.add_apply]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk]; rw [ smul_add],
...
{ app := λ X,
 ...
 rw [restrict_scalars.smul_def f]; rw [ coextend_scalars.smul_apply]; rw [ one_mul]; rw [ ←linear_map.map_smul]; rw [ @restrict_scalars.smul_def _ _ _ _ f ⟨S⟩]; rw [ smul_eq_mul]; rw [ mul_one],
 end },
...
{ to_fun := λ x, g $ (1 : S) ⊗ₜ[R, f] x,
 map_add' := λ _ _, by rw [tmul_add]; rw [ map_add],
 map_smul' := λ r x,
 begin
 letI : module R S := module.comp_hom S f,
 letI : module R Y := module.comp_hom Y f,
 rw [ring_hom.id_apply]; rw [ restrict_scalars.smul_def]; rw [ ←linear_map.map_smul]; rw [ tmul_smul],
 congr,
 end }
...
begin
 ...
 { exact λ x, s • g x },
 { intros, rw [map_add]; rw [ smul_add], },
 { intros, rw [ring_hom.id_apply]; rw [ smul_comm]; rw [ ←linear_map.map_smul], },
 ...
 { rw [smul_add]; rw [ map_add]; rw [ ih1]; rw [ ih2]; rw [ map_add]; rw [ smul_add], }, },
end
...
{ to_fun := hom_equiv.to_restrict_scalars f,
 ...
 change S at x,
 erw [←linear_map.map_smul]; erw [ extend_scalars.smul_tmul]; erw [ mul_one x], },
 { rw [map_add]; rw [ map_add]; rw [ ih1]; rw [ ih2], }
 end,
 right_inv := λ g,
 begin
 ext,
 rw [hom_equiv.to_restrict_scalars_apply]; rw [ hom_equiv.from_extend_scalars_apply]; rw [ lift.tmul]; rw [ linear_map.coe_mk]; rw [ linear_map.coe_mk],
...
begin
 ...
 { intros, rw [ring_hom.id_apply]; rw [ restrict_scalars.smul_def]; rw [ ←mul_smul]; rw [ mul_comm]; rw [ mul_smul]; rw [ restrict_scalars.smul_def], },
 ...
 { rw [smul_add]; rw [ map_add]; rw [ map_add]; rw [ ih1]; rw [ ih2]; rw [ smul_add], } },
end
