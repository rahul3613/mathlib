...
begin
 ...
 rw [linear_map.map_smul]; rw [ linear_map.map_smul]; rw [ hfg],
end
...
{ one_smul := λ x, begin
 ...
 erw [colimit_smul_mk_eq F 1 ⟨j, x⟩]; erw [ one_smul],
 refl,
 end,
 mul_smul := λ r s x, begin
 apply quot.induction_on x, clear x, intro x, cases x with j x,
 erw [colimit_smul_mk_eq F (r * s) ⟨j, x⟩]; erw [ colimit_smul_mk_eq F s ⟨j, x⟩]; erw [ colimit_smul_mk_eq F r ⟨j, _⟩]; erw [ mul_smul],
 end,
 smul_add := λ r x y, begin
 apply quot.induction_on₂ x y, clear x y, intros x y, cases x with i x, cases y with j y,
 erw [colimit_add_mk_eq _ ⟨i, x⟩ ⟨j, y⟩ (max' i j) (left_to_max i j) (right_to_max i j)]; erw [ colimit_smul_mk_eq]; erw [ smul_add]; erw [ colimit_smul_mk_eq]; erw [ colimit_smul_mk_eq]; erw [ colimit_add_mk_eq _ ⟨i, _⟩ ⟨j, _⟩ (max' i j) (left_to_max i j) (right_to_max i j)]; erw [ linear_map.map_smul]; erw [ linear_map.map_smul],
 refl,
 end,
 smul_zero := λ r, begin
 erw [colimit_zero_eq _ (is_filtered.nonempty.some : J)]; erw [ colimit_smul_mk_eq]; erw [ smul_zero],
 refl,
 end,
 zero_smul := λ x, begin
 apply quot.induction_on x, clear x, intro x, cases x with j x,
 erw [colimit_smul_mk_eq]; erw [ zero_smul]; erw [ colimit_zero_eq _ j],
 refl,
 end,
 add_smul := λ r s x, begin
 apply quot.induction_on x, clear x, intro x, cases x with j x,
 erw [colimit_smul_mk_eq]; erw [ add_smul]; erw [ colimit_smul_mk_eq]; erw [ colimit_smul_mk_eq]; erw [ colimit_add_mk_eq _ ⟨j, _⟩ ⟨j, _⟩ j (𝟙 j) (𝟙 j)]; erw [ category_theory.functor.map_id]; erw [ id_apply]; erw [ id_apply],
 refl,
 end }
