...
{ mul_zero := λ x, begin
 ...
 cases x with j x,
 erw [colimit_zero_eq _ j]; erw [ colimit_mul_mk_eq _ ⟨j, _⟩ ⟨j, _⟩ j (𝟙 j) (𝟙 j)],
 rw [category_theory.functor.map_id]; rw [ id_apply]; rw [ id_apply]; rw [ mul_zero x],
 refl,
 end,
 zero_mul := λ x, begin
 apply quot.induction_on x, clear x, intro x,
 cases x with j x,
 erw [colimit_zero_eq _ j]; erw [ colimit_mul_mk_eq _ ⟨j, _⟩ ⟨j, _⟩ j (𝟙 j) (𝟙 j)],
 rw [category_theory.functor.map_id]; rw [ id_apply]; rw [ id_apply]; rw [ zero_mul x],
 refl,
 end,
 ...
 erw [colimit_add_mk_eq _ ⟨j₂, _⟩ ⟨j₃, _⟩ k g h]; erw [ colimit_mul_mk_eq _ ⟨j₁, _⟩ ⟨k, _⟩ k f (𝟙 k)]; erw [ colimit_mul_mk_eq _ ⟨j₁, _⟩ ⟨j₂, _⟩ k f g]; erw [ colimit_mul_mk_eq _ ⟨j₁, _⟩ ⟨j₃, _⟩ k f h]; erw [ colimit_add_mk_eq _ ⟨k, _⟩ ⟨k, _⟩ k (𝟙 k) (𝟙 k)],
 ...
 let g := second_to_max₃ j₁ j₂ j₃,
 let h := third_to_max₃ j₁ j₂ j₃,
 erw [colimit_add_mk_eq _ ⟨j₁, _⟩ ⟨j₂, _⟩ k f g]; erw [ colimit_mul_mk_eq _ ⟨k, _⟩ ⟨j₃, _⟩ k (𝟙 k) h]; erw [ colimit_mul_mk_eq _ ⟨j₁, _⟩ ⟨j₃, _⟩ k f h]; erw [ colimit_mul_mk_eq _ ⟨j₂, _⟩ ⟨j₃, _⟩ k g h]; erw [ colimit_add_mk_eq _ ⟨k, _⟩ ⟨k, _⟩ k (𝟙 k) (𝟙 k)],
 simp only [category_theory.functor.map_id, id_apply],
