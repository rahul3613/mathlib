...
{ factors := λ X Y g f m,
 ...
 resetI,
 let G : (⟨X⟩ : Module ℤ) ⟶ ⟨A⟩ :=
 { map_smul' := by { intros, rw [ring_hom.id_apply]; rw [ g.to_fun_eq_coe]; rw [ map_zsmul], }, ..g },
 let F : (⟨X⟩ : Module ℤ) ⟶ ⟨Y⟩ :=
 { map_smul' := by { intros, rw [ring_hom.id_apply]; rw [ f.to_fun_eq_coe]; rw [ map_zsmul], }, ..f },
 haveI : mono F,
...
{ factors := λ X Y g f m,
 ...
 { intros m x, rw [add_monoid_hom.to_fun_eq_coe]; rw [ ring_hom.id_apply],
...
λ I g, begin
 ...
 { rw [submodule.span_singleton_eq_bot.mpr rfl] at hn; rw [ submodule.mem_bot] at hn,
 ...
 { intros n1 n2,
 rw [ring_hom.id_apply]; rw [ smul_eq_mul]; rw [ mul_smul], },
 { rw submodule.mem_span_singleton at hn,
 rcases hn with ⟨n, rfl⟩,
 simp only [gm_eq, algebra.id.smul_eq_mul, linear_map.coe_mk],
 rw [mul_smul]; rw [ divisible_by.div_cancel (g ⟨m, _⟩) m_eq_zero]; rw [ ←linear_map.map_smul],
 congr, }, },
end
