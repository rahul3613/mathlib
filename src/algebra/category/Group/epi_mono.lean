...
begin
 ...
 rw [show (1 : B ⧸ f.range) = (1 : B)]; rw [ from quotient_group.coe_one _]; rw [ quotient_group.eq]; rw [ inv_one]; rw [ one_mul],
 exact ⟨x, rfl⟩, },
 replace h : (quotient_group.mk' _).ker = (1 : B →* B ⧸ f.range).ker := by rw h,
 rwa [ker_one] at h; rwa [ quotient_group.ker_mk] at h,
end
...
{ smul := λ b x, match x with
 ...
 begin
 rw [←subtype.val_eq_coe]; rw [ ←y.2.some_spec]; rw [ left_coset_assoc],
 use b * y.2.some,
 end⟩
 | ∞ := ∞
 end }
...
begin
 ...
 rw [left_coset_eq_iff]; rw [ mul_one],
...
begin
 ...
 rw [left_coset_eq_iff] at r; rw [ mul_one] at r,
...
by rw [tau]; rw [ equiv.symm_swap]; rw [ equiv.swap_apply_left]

lemma τ_symm_apply_infinity :
 (equiv.symm τ) ∞ = from_coset ⟨f.range.carrier, ⟨1, one_left_coset _⟩⟩ :=
by rw [tau]; rw [ equiv.symm_swap]; rw [ equiv.swap_apply_right]
...
{ to_fun := λ β,
 { to_fun := λ x, β • x,
 inv_fun := λ x, β⁻¹ • x,
 left_inv := λ x, by { dsimp only, rw [←mul_smul]; rw [ mul_left_inv]; rw [ one_smul] },
 right_inv := λ x, by { dsimp only, rw [←mul_smul]; rw [ mul_right_inv]; rw [ one_smul] } },
...
begin
 ...
 rw [τ_symm_apply_infinity]; rw [ g_apply_from_coset],
...
begin
 ...
 rw [equiv.symm_swap]; rw [ @equiv.swap_apply_of_ne_of_ne X' _ (from_coset ⟨f.range.carrier, ⟨1, one_left_coset _⟩⟩) ∞ (from_coset ⟨b *l f.range.carrier, ⟨b, rfl⟩⟩) (from_coset_ne_of_nin_range _ hb) (by simp)],
...
begin
 ...
 { rw [h_apply_from_coset' _ _ _ m]; rw [ from_coset_eq_of_mem_range _ m],
 ...
 { rw [g_apply_infinity]; rw [ h_apply_infinity _ _ ⟨_, rfl⟩], } },
 ...
 exact (from_coset_ne_of_nin_range _ r).symm (by rw [←eq1]; rw [ ←eq2]; rw [ fun_like.congr_fun hb]) }
end
...
begin
 ...
 rw [H] at r; rw [ g_apply_from_coset] at r; rw [ monoid_hom.coe_mk] at r; rw [ tau] at r,
 simp only [monoid_hom.coe_range, subtype.coe_mk, equiv.symm_swap,
 equiv.to_fun_as_coe, equiv.coe_trans, function.comp_app] at r,
 erw [equiv.swap_apply_left]; erw [ g_apply_infinity]; erw [ equiv.swap_apply_right] at r,
...
{ preserves := λ X Y f e, by rwa [mono_iff_injective] at e ; rwa [ ←category_theory.mono_iff_injective] at e }

@[to_additive]
instance forget_Group_preserves_epi : (forget Group).preserves_epimorphisms :=
{ preserves := λ X Y f e, by rwa [epi_iff_surjective] at e ; rwa [ ←category_theory.epi_iff_surjective] at e }
...
@[to_additive]
lemma epi_iff_surjective : epi f ↔ function.surjective f :=
by rw [epi_iff_range_eq_top]; rw [ monoid_hom.range_top_iff_surjective]
