...
lemma same_add_div (h : b ≠ 0) : (b + a) / b = 1 + a / b := by rw [←div_self h]; rw [ add_div]
lemma div_add_same (h : b ≠ 0) : (a + b) / b = a / b + 1 := by rw [←div_self h]; rw [ add_div]
...
lemma one_div_mul_add_mul_one_div_eq_one_div_add_one_div (ha : a ≠ 0) (hb : b ≠ 0) :
 (1 / a) * (a + b) * (1 / b) = 1 / a + 1 / b :=
by rw [mul_add]; rw [ one_div_mul_cancel ha]; rw [ add_mul]; rw [ one_mul]; rw [ mul_assoc]; rw [ mul_one_div_cancel hb]; rw [ mul_one]; rw [ add_comm]

lemma add_div_eq_mul_add_div (a b : α) (hc : c ≠ 0) : a + b / c = (a * c + b) / c :=
(eq_div_iff_mul_eq hc).2 $ by rw [right_distrib]; rw [ (div_mul_cancel _ hc)]

@[field_simps] lemma add_div' (a b c : α) (hc : c ≠ 0) : b + a / c = (b * c + a) / c :=
by rw [add_div]; rw [ mul_div_cancel _ hc]

@[field_simps] lemma div_add' (a b c : α) (hc : c ≠ 0) : a / c + b = (a + b * c) / c :=
by rwa [add_comm]; rwa [ add_div']; rwa [ add_comm]

protected lemma commute.div_add_div (hbc : commute b c) (hbd : commute b d) (hb : b ≠ 0)
 (hd : d ≠ 0) : a / b + c / d = (a * d + b * c) / (b * d) :=
by rw [add_div]; rw [ mul_div_mul_right _ b hd]; rw [ hbc.eq]; rw [ hbd.eq]; rw [ mul_div_mul_right c d hb]

protected lemma commute.one_div_add_one_div (hab : commute a b) (ha : a ≠ 0) (hb : b ≠ 0) :
 1 / a + 1 / b = (a + b) / (a * b) :=
by rw [(commute.one_right a).div_add_div hab ha hb]; rw [ one_mul]; rw [ mul_one]; rw [ add_comm]

protected lemma commute.inv_add_inv (hab : commute a b) (ha : a ≠ 0) (hb : b ≠ 0) :
 a⁻¹ + b⁻¹ = (a + b) / (a * b) :=
by rw [inv_eq_one_div]; rw [ inv_eq_one_div]; rw [ hab.one_div_add_one_div ha hb]
...
have (-1) * (-1) = (1:K), by rw [neg_mul_neg]; rw [ one_mul],
...
calc
 ...
 ... = - (1 / a) : by rw [mul_neg]; rw [ mul_one]

lemma div_neg_eq_neg_div (a b : K) : b / (- a) = - (b / a) :=
calc
 b / (- a) = b * (1 / (- a)) : by rw [← inv_eq_one_div]; rw [ division_def]
 ... = b * -(1 / a) : by rw one_div_neg_eq_neg_one_div
 ... = -(b * (1 / a)) : by rw neg_mul_eq_mul_neg
 ... = - (b / a) : by rw mul_one_div

lemma neg_div (a b : K) : (-b) / a = - (b / a) :=
by rw [neg_eq_neg_one_mul]; rw [ mul_div_assoc]; rw [ ← neg_eq_neg_one_mul]

@[field_simps] lemma neg_div' (a b : K) : - (b / a) = (-b) / a :=
by simp [neg_div]

lemma neg_div_neg_eq (a b : K) : (-a) / (-b) = a / b :=
by rw [div_neg_eq_neg_div]; rw [ neg_div]; rw [ neg_neg]

lemma neg_inv : - a⁻¹ = (- a)⁻¹ :=
by rw [inv_eq_one_div]; rw [ inv_eq_one_div]; rw [ div_neg_eq_neg_div]
...
by rw [div_neg_eq_neg_div]; rw [ div_self h]

@[simp] lemma neg_div_self {a : K} (h : a ≠ 0) : (-a) / a = -1 :=
by rw [neg_div]; rw [ div_self h]

lemma div_sub_div_same (a b c : K) : (a / c) - (b / c) = (a - b) / c :=
by rw [sub_eq_add_neg]; rw [ ← neg_div]; rw [ div_add_div_same]; rw [ sub_eq_add_neg]
...
by rw [mul_sub]; rw [ sub_mul]; rw [ mul_inv_cancel_right₀ hb]; rw [ inv_mul_cancel ha]; rw [ one_mul]

lemma one_div_mul_sub_mul_one_div_eq_one_div_add_one_div (ha : a ≠ 0) (hb : b ≠ 0) :
 (1 / a) * (b - a) * (1 / b) = 1 / a - 1 / b :=
by rw [(mul_sub_left_distrib (1 / a))]; rw [ (one_div_mul_cancel ha)]; rw [ mul_sub_right_distrib]; rw [ one_mul]; rw [ mul_assoc]; rw [ (mul_one_div_cancel hb)]; rw [ mul_one]
...
protected def function.injective.division_ring [division_ring K] {K'}
 ...
 division_ring K' :=
{ rat_cast := coe,
 rat_cast_mk := λ a b h1 h2, hf (by erw [rat_cast]; erw [ mul]; erw [ inv]; erw [ int_cast]; erw [ nat_cast];
 exact division_ring.rat_cast_mk a b h1 h2),
 qsmul := (•),
 qsmul_eq_mul' := λ a x, hf (by erw [qsmul]; erw [ mul]; erw [ rat.smul_def]; erw [ rat_cast]),
 .. hf.group_with_zero f zero one mul inv div npow zpow,
