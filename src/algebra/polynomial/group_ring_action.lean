...
polynomial.induction_on f
 (λ r, by rw [smul_C]; rw [ eval_C]; rw [ eval_C])
 (λ f g ihf ihg, by rw [smul_add]; rw [ eval_add]; rw [ ihf]; rw [ ihg]; rw [ eval_add]; rw [ smul_add])
 (λ n r ih, by rw [smul_mul']; rw [ smul_pow']; rw [ smul_C]; rw [ smul_X]; rw [ eval_mul]; rw [ eval_C]; rw [ eval_pow]; rw [ eval_X]; rw [ eval_mul]; rw [ eval_C]; rw [ eval_pow]; rw [ eval_X]; rw [ smul_mul']; rw [ smul_pow'])

variables (G : Type*) [group G]

theorem eval_smul' [mul_semiring_action G S] (g : G) (f : S[X]) (x : S) :
 f.eval (g • x) = g • (g⁻¹ • f).eval x :=
by rw [← smul_eval_smul]; rw [ smul_inv_smul]
...
finset.smul_prod.trans $ fintype.prod_bijective _ (mul_action.bijective g) _ _
 (λ g', by rw [of_quotient_stabilizer_smul]; rw [ smul_sub]; rw [ polynomial.smul_X]; rw [ polynomial.smul_C])

theorem prod_X_sub_smul.coeff (x : R) (g : G) (n : ℕ) :
 g • (prod_X_sub_smul G R x).coeff n =
 (prod_X_sub_smul G R x).coeff n :=
by rw [← polynomial.coeff_smul]; rw [ prod_X_sub_smul.smul]
...
{ to_fun := map g,
 ...
 (λ b, by rw [smul_C]; rw [ map_C]; rw [ coe_fn_coe]; rw [ g.map_smul]; rw [ map_C]; rw [ coe_fn_coe]; rw [ smul_C])
 (λ p q ihp ihq, by rw [smul_add]; rw [ polynomial.map_add]; rw [ ihp]; rw [ ihq]; rw [ polynomial.map_add]; rw [ smul_add])
 (λ n b ih, by rw [smul_mul']; rw [ smul_C]; rw [ smul_pow']; rw [ smul_X]; rw [ polynomial.map_mul]; rw [ map_C]; rw [ polynomial.map_pow]; rw [ map_X]; rw [ coe_fn_coe]; rw [ g.map_smul]; rw [ polynomial.map_mul]; rw [ map_C]; rw [ polynomial.map_pow]; rw [ map_X]; rw [ smul_mul']; rw [ smul_C]; rw [ smul_pow']; rw [ smul_X]; rw [ coe_fn_coe]),
