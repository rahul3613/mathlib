...
by rw [← mul_indicator_mul_indicator]; rw [ mul_indicator_mul_support]
...
by rw [← mul_indicator_comp_right]; rw [ preimage_image_eq _ hg]
...
begin
 classical,
 rw [mul_indicator_one']; rw [ preimage_one],
 split_ifs; simp
end
...
begin
 rw [mul_indicator_preimage]; rw [ preimage_one]; rw [ preimage_const],
...
by rw [← mul_indicator_union_mul_inter_apply f s t]; rw [ mul_indicator_of_not_mem h]; rw [ mul_one]
...
begin
 ext x,
 by_cases h : x ∈ s,
 { rw [piecewise_eq_of_mem _ _ _ h]; rw [ pi.mul_apply]; rw [ set.mul_indicator_of_mem h]; rw [ set.mul_indicator_of_not_mem (set.not_mem_compl_iff.2 h)]; rw [ mul_one] },
 { rw [piecewise_eq_of_not_mem _ _ _ h]; rw [ pi.mul_apply]; rw [ set.mul_indicator_of_not_mem h]; rw [ set.mul_indicator_of_mem (set.mem_compl h)]; rw [ one_mul] },
end
...
by rw [sub_eq_add_neg]; rw [ indicator_compl']

@[to_additive indicator_diff'] lemma mul_indicator_diff (h : s ⊆ t) (f : α → G) :
 mul_indicator (t \ s) f = mul_indicator t f * (mul_indicator s f)⁻¹ :=
eq_mul_inv_of_mul_eq $ by { rw [pi.mul_def]; rw [ ←mul_indicator_union_of_disjoint]; rw [ diff_union_self]; rw [ union_eq_self_of_subset_right h], exact disjoint_sdiff_self_left }

lemma indicator_diff {G : Type*} [add_group G] {s t : set α} (h : s ⊆ t) (f : α → G) :
 indicator (t \ s) f = indicator t f - indicator s f :=
by rw [indicator_diff' h]; rw [ sub_eq_add_neg]
...
begin
 ...
 funext,
 rw [finset.prod_insert haI]; rw [ finset.set_bUnion_insert]; rw [ mul_indicator_union_of_not_mem_inter]; rw [ ih _],
