...
{ carrier := {x | star x = -x},
 ...
 show star (x + y) = -(x + y), by rw [star_add x y]; rw [ hx]; rw [ hy]; rw [ neg_add],
...
by rw [mem_iff]; rw [ star_bit0]; rw [ mem_iff.mp hx]; rw [ bit0]; rw [ bit0]; rw [ neg_add]
...
by rw [mem_iff]; rw [ star_smul]; rw [ star_trivial]; rw [ mem_iff.mp h]; rw [ smul_neg r]
...
⟨show star (star x) * (star x) = (star x) * star (star x), by rw [star_star]; rw [ star_comm_self']⟩
