...
λ S T h, ext $ λ x, by rw [← mem_to_subalgebra]; rw [ ← mem_to_subalgebra]; rw [ h]
...
{ star_mem' := λ a ha,
 ...
 rw [←mem_star_iff _ a]; rw [ star_adjoin_comm],
 convert ha,
 simp [set.union_comm],
 end,
 .. S ⊔ star S }
...
{ star_mem' := λ x hx, by rwa [subalgebra.mem_carrier]; rwa [ ←subalgebra.mem_star_iff]; rwa [ subalgebra.star_adjoin_comm]; rwa [ set.union_star]; rwa [ star_star]; rwa [ set.union_comm],
...
begin
 intros s S,
 rw [←to_subalgebra_le_iff]; rw [ adjoin_to_subalgebra]; rw [ algebra.adjoin_le_iff]; rw [ coe_to_subalgebra],
...
{ mul_comm :=
 ...
 rw [←mem_to_subalgebra] at hx hy; rw [ adjoin_to_subalgebra] at hx hy,
...
by rw [←mem_to_subalgebra]; rw [ bot_to_subalgebra]; rw [ algebra.mem_bot]
...
{ carrier := {a | f a = g a},
 ...
 by rw [set.mem_set_of_eq]; rw [ map_mul f]; rw [ map_mul g]; rw [ ha]; rw [ hb],
 add_mem' := λ a b (ha : f a = g a) (hb : f b = g b),
 by rw [set.mem_set_of_eq]; rw [ map_add f]; rw [ map_add g]; rw [ ha]; rw [ hb],
 algebra_map_mem' := λ r, by simp only [set.mem_set_of_eq, alg_hom_class.commutes],
 star_mem' := λ a (ha : f a = g a), by rw [set.mem_set_of_eq]; rw [ map_star f]; rw [ map_star g]; rw [ ha] }
...
begin
 ...
 { rw [map_add]; rw [ map_add]; rw [ hx]; rw [ hy] },
 { rw [map_mul]; rw [ map_mul]; rw [ hx]; rw [ hy] },
 { rw [map_star]; rw [ map_star]; rw [ hx] },
end
