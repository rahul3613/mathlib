...
by { ext, rw [mem_ne_locus]; rw [ mem_support_iff]; rw [ coe_zero]; rw [ pi.zero_apply] }
...
lemma ne_locus_neg : ne_locus (-f) g = f.ne_locus (-g) := by rw [←ne_locus_neg_neg]; rw [ neg_neg]

lemma ne_locus_eq_support_sub : f.ne_locus g = (f - g).support :=
by rw [←@ne_locus_add_right α N _ _ _ _ _ (-g)]; rw [ add_right_neg]; rw [ ne_locus_zero_right]; rw [ sub_eq_add_neg]
...
by rw [←ne_locus_zero_left]; rw [ ←@ne_locus_add_left α N _ _ _ f 0 g]; rw [ add_zero]

@[simp] lemma ne_locus_self_add_left : ne_locus (f + g) f = g.support :=
by rw [ne_locus_comm]; rw [ ne_locus_self_add_right]

@[simp] lemma ne_locus_self_sub_right : ne_locus f (f - g) = g.support :=
by rw [sub_eq_add_neg]; rw [ ne_locus_self_add_right]; rw [ support_neg]

@[simp] lemma ne_locus_self_sub_left : ne_locus (f - g) f = g.support :=
by rw [ne_locus_comm]; rw [ ne_locus_self_sub_right]
