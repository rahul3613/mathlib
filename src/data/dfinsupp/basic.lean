...
⟨λ i, if p i then x i else 0, x.support'.map
 (λ xs, ⟨xs, λ i, (xs.prop i).imp_right $ λ H : x i = 0, by rw [H]; rw [ if_t_t]⟩)⟩
...
begin
 ...
 { rw eq_of_heq hxi, },
 { rw [hi]; rw [ hj]; rw [ dfinsupp.single_zero]; rw [ dfinsupp.single_zero], }, },
end
...
by rw [update_eq_erase_add_single f i b]; rw [ erase_eq_sub_single f i]
...
begin
 ...
 { rw [h]; rw [ single_zero]; rw [ zero_add], exact H2 },
...
begin
 ...
 rw [finset.mem_filter]; rw [ multiset.mem_to_finset]; rw [ coe_mk'],
...
assume f g, decidable_of_iff (f.support = g.support ∧ (∀i∈f.support, f i = g i))
 ...
 have hf : f i = 0, by rwa [mem_support_iff] at h; rwa [ not_not] at h,
 have hg : g i = 0, by rwa [h₁] at h; rwa [ mem_support_iff] at h; rwa [ not_not] at h,
 by rw [hf]; rw [ hg],
 by { rintro rfl, simp }⟩
...
by { ext, rw [zero_apply]; rw [ comap_domain_apply]; rw [ zero_apply] }
...
by { ext, rw [add_apply]; rw [ comap_domain_apply]; rw [ comap_domain_apply]; rw [ comap_domain_apply]; rw [ add_apply] }
...
by { ext, rw [smul_apply]; rw [ comap_domain_apply]; rw [ smul_apply]; rw [ comap_domain_apply] }
...
begin
 ...
 { rw [single_eq_same]; rw [ single_eq_same] },
 { rw [single_eq_of_ne hik.symm]; rw [ single_eq_of_ne (hh.ne hik.symm)] },
end
...
@[simp] lemma comap_domain'_zero [Π i, has_zero (β i)] (h : κ → ι) {h' : ι → κ}
 ...
 comap_domain' h hh' (0 : Π₀ i, β i) = 0 :=
by { ext, rw [zero_apply]; rw [ comap_domain'_apply]; rw [ zero_apply] }

@[simp] lemma comap_domain'_add [Π i, add_zero_class (β i)] (h : κ → ι) {h' : ι → κ}
 (hh' : function.left_inverse h' h) (f g : Π₀ i, β i) :
 comap_domain' h hh' (f + g) = comap_domain' h hh' f + comap_domain' h hh' g :=
by { ext, rw [add_apply]; rw [ comap_domain'_apply]; rw [ comap_domain'_apply]; rw [ comap_domain'_apply]; rw [ add_apply] }
...
@[simp] lemma comap_domain'_smul [monoid γ] [Π i, add_monoid (β i)]
 ...
 comap_domain' h hh' (r • f) = r • comap_domain' h hh' f :=
by { ext, rw [smul_apply]; rw [ comap_domain'_apply]; rw [ smul_apply]; rw [ comap_domain'_apply] }
...
begin
 ...
 obtain rfl | hik := decidable.eq_or_ne i k,
 { rw [single_eq_same]; rw [ single_eq_same] },
 { rw [single_eq_of_ne hik.symm]; rw [ single_eq_of_ne (hh'.injective.ne hik.symm)] },
end
...
{ to_fun := comap_domain' h.symm h.right_inv,
 ...
 left_inv := λ f, by { ext i, rw [map_range_apply]; rw [ comap_domain'_apply]; rw [ comap_domain'_apply]; rw [ equiv.cast_eq_iff_heq]; rw [ h.symm_apply_apply] },
 right_inv := λ f, by { ext k, rw [comap_domain'_apply]; rw [ map_range_apply]; rw [ comap_domain'_apply]; rw [ equiv.cast_eq_iff_heq]; rw [ h.apply_symm_apply] } }
...
begin
 ...
 { rw [h]; rw [ mk_apply], split_ifs, { rw mk_apply, split_ifs, { exact h }, { refl } }, { refl } },
 { rw [mk_of_mem]; rw [ mk_of_mem], { refl },
 { rw [mem_preimage]; rw [ mem_support_to_fun], exact h },
...
begin
 ext i j,
 rw [@add_apply _ (λ i, Π₀ j, δ i j) _ (sigma_curry _)]; rw [ add_apply]; rw [ sigma_curry_apply]; rw [ sigma_curry_apply]; rw [ sigma_curry_apply]; rw [ add_apply]
end
...
begin
 ext i j,
 rw [@smul_apply _ _ (λ i, Π₀ j, δ i j) _ _ _ _ (sigma_curry _)]; rw [ smul_apply]; rw [ sigma_curry_apply]; rw [ sigma_curry_apply]; rw [ smul_apply]
end
...
begin
 ...
 { rw [single_eq_same]; rw [ single_eq_same] },
 { rw [single_eq_of_ne]; rw [ single_eq_of_ne hj],
 simpa using hj }, },
 { rw [single_eq_of_ne]; rw [ single_eq_of_ne hi]; rw [ zero_apply],
 simpa using hi },
end
...
{ to_fun := λ i, f i.1 i.2,
 ...
 simp_rw [multiset.mem_bind, map_val, multiset.mem_map, function.embedding.coe_fn_mk, ←finset.mem_def, mem_support_to_fun],
...
begin
 ...
 obtain rfl | hj := eq_or_ne j j',
 { rw [single_eq_same]; rw [ single_eq_same] },
 { rw [single_eq_of_ne hj]; rw [ single_eq_of_ne],
 simpa using hj }, },
 { rw [single_eq_of_ne hi]; rw [ single_eq_of_ne]; rw [ zero_apply],
 simpa using hi },
end
...
{ to_fun := sigma_curry,
 ...
 left_inv := λ f, by { ext ⟨i, j⟩, rw [sigma_uncurry_apply]; rw [ sigma_curry_apply] },
 right_inv := λ f, by { ext i j, rw [sigma_curry_apply]; rw [ sigma_uncurry_apply] } }
...
begin
 ext (_ | j),
 { rw [extend_with_none]; rw [ single_eq_of_ne (option.some_ne_none _)] },
 { rw extend_with_some,
 obtain rfl | hij := decidable.eq_or_ne i j,
 { rw [single_eq_same]; rw [ single_eq_same] },
 { rw [single_eq_of_ne hij]; rw [ single_eq_of_ne ((option.some_injective _).ne hij)] }, },
end
...
begin
 ext (_ | j),
 { rw [extend_with_none]; rw [ single_eq_same] },
 { rw [extend_with_some]; rw [ single_eq_of_ne (option.some_ne_none _).symm]; rw [ zero_apply] },
end
...
{ to_fun := λ f, (f none, comap_domain some (option.some_injective _) f),
 ...
 { rw extend_with_none },
 { rw [extend_with_some]; rw [ comap_domain_apply] }
 end,
 right_inv := λ x, begin
 dsimp only,
 ext,
 { exact extend_with_none x.snd _ },
 { rw [comap_domain_apply]; rw [ extend_with_some] }
 end }
...
{ to_fun := (λ f,
 ...
 rw [(hy i).resolve_left (mt (and.intro H1) H2)]; rw [ add_monoid_hom.map_zero] },
 { intros i H1, refl },
 { intros i H1 H2, rw finset.mem_inter at H2,
 simp only [multiset.mem_to_finset] at H1 H2,
 rw [(hx i).resolve_left (mt (λ H3, and.intro H3 H1) H2)]; rw [ add_monoid_hom.map_zero] }
 end),
 map_add' := begin
 ...
 rw [(hf i).resolve_left H2]; rw [ add_monoid_hom.map_zero] } },
 { refine (finset.sum_subset _ _).symm,
 { intro i, simp only [multiset.mem_to_finset, multiset.mem_add], exact or.inr },
 { intros i H1 H2, simp only [multiset.mem_to_finset, multiset.mem_add] at H2,
 rw [(hg i).resolve_left H2]; rw [ add_monoid_hom.map_zero] } }
 end,
 map_zero' := rfl }
...
begin
 ...
 rw [multiset.to_finset_singleton]; rw [ finset.sum_singleton]; rw [ pi.single_eq_same],
end
...
begin
 ...
 split_ifs,
 refl,
 rw [(not_not.mp h)]; rw [ add_monoid_hom.map_zero],
end
...
begin
 ...
 rw [add_monoid_hom.comp_apply]; rw [ filter_add_monoid_hom_apply]; rw [ filter_single_pos _ _ hi],
...
lift_add_hom.symm_apply_eq.1 $ funext $ λ a,
 by rw [lift_add_hom_symm_apply]; rw [ add_monoid_hom.comp_assoc]; rw [ lift_add_hom_comp_single]
...
begin
 ...
 rw [lift_add_hom_apply] at this; rw [ sum_add_hom_apply] at this; rw [ sum_add_hom_apply] at this; rw [ sum_add_hom_apply] at this,
 exact this,
end
...
begin
 ...
 rw [lift_add_hom_apply] at this; rw [ sum_add_hom_apply] at this,
 exact this,
end
