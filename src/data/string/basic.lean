...
⟨λ h, by rw [←list.to_list_inv_as_string l]; rw [ ←list.to_list_inv_as_string l']; rw [ to_list_inj]; rw [ h],
 λ h, h ▸ rfl⟩

@[simp] lemma string.length_to_list (s : string) : s.to_list.length = s.length :=
by rw [←string.as_string_inv_to_list s]; rw [ list.to_list_inv_as_string]; rw [ list.length_as_string]

lemma list.as_string_eq {l : list char} {s : string} :
 l.as_string = s ↔ l = s.to_list :=
by rw [←as_string_inv_to_list s]; rw [ list.as_string_inj]; rw [ as_string_inv_to_list s]
