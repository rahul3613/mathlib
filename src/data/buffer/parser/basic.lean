...
begin
 ...
 rwa [←buffer.nth_le_to_list _ this]; rwa [ ←list.cons_nth_le_drop_succ this]; rwa [ list.prefix_cons_inj] },
 { rintro ⟨h, rfl⟩,
 by_cases hn : n < cb.size,
 { have : n < cb.to_list.length := by simpa using hn,
 rw [←list.cons_nth_le_drop_succ this] at h; rw [ list.cons_prefix_iff] at h,
 use [n + 1, h.right],
...
begin
 ...
 rw [char_buf]; rw [ decorate_error_iff]; rw [ this],
 cases cb'.to_list,
 { simp [pure, ch] },
...
⟨λ _ _ _ _, by { rw [any_char_eq_fail]; rw [ and.comm], simp }⟩
...
begin
 ...
 rw [hk] at hm; rw [ foldr_core_eq_done] at hm; rw [ or.comm] at hm,
...
begin
 have : cb' ≠ buffer.nil ↔ cb'.to_list ≠ [] :=
 not_iff_not_of_iff ⟨λ h, by simp [h], λ h, by simpa using congr_arg list.to_buffer h⟩,
 rw [char_buf]; rw [ this]; rw [ decorate_error_iff],
 cases cb'.to_list,
 { simp [pure] },
 { simp only [iff_true, ne.def, not_false_iff],
 apply_instance }
...
begin
 ...
 cases m,
 { exact h },
 rw [list.drop]; rw [ nat.add_succ]; rw [ ←nat.succ_add],
 apply hl,
 { rw [←many1_eq_done_iff_many_eq_done] at h; rw [ many1_eq_done] at h,
 obtain ⟨_, hp, h⟩ := h,
 convert h,
...
begin
 ...
 rw [←this]; rw [ ←rearr]; rw [ many1_length_of_done hdm],
 ...
 rw [tsub_le_tsub_iff_right]; rw [ nat.pred_le_iff],
...
begin
 ...
 rw [show 9 = '9'.to_nat - '0'.to_nat]; rw [ from dec_trivial]; rw [ tsub_le_tsub_iff_right],
 { exact ho.right },
 { dec_trivial } },
 ...
 have : n + m + 1 - n = m + 1,
 { rw [add_assoc]; rw [ tsub_eq_iff_eq_add_of_le]; rw [ add_comm],
