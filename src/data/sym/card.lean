...
{ to_fun := λ s, map (fin.pred_above 0) s.1,
 ...
 nth_rewrite_rhs 0 ←(map_id' s),
 refine sym.map_congr (λ v hv, _),
 rw [←fin.zero_succ_above v]; rw [ ←@fin.cast_succ_zero n.succ]; rw [ fin.pred_above_succ_above 0 v] } }
...
begin
 ...
 rw [multichoose_zero_succ]; rw [ card_eq_zero_iff],
 apply_instance },
 { intros x y h1 h2,
 rw [multichoose_succ_succ]; rw [ ←h1]; rw [ ←h2]; rw [ add_comm],
 cases x,
...
by rw [card_sym_eq_multichoose]; rw [ nat.multichoose_eq]
...
begin
 rw [card_image_of_inj_on]; rw [ diag_card],
 rintro ⟨x₀, x₁⟩ hx _ _ h,
...
begin
 rw [card_eq_sum_card_fiberwise (λ x, mem_image_of_mem _ : ∀ x ∈ s.off_diag, quotient.mk x ∈ s.off_diag.image quotient.mk)]; rw [ sum_const_nat (quotient.ind _)]; rw [ mul_comm],
 rintro ⟨x, y⟩ hxy,
...
by rw [nat.choose_two_right]; rw [ mul_tsub]; rw [ mul_one]; rw [ ←off_diag_card]; rw [ nat.div_eq_of_eq_mul_right zero_lt_two (two_mul_card_image_off_diag s).symm]

lemma card_subtype_diag [fintype α] :
 card {a : sym2 α // a.is_diag} = card α :=
begin
 convert card_image_diag (univ : finset α),
 rw [fintype.card_of_subtype]; rw [ ←filter_image_quotient_mk_is_diag],
 rintro x,
...
begin
 ...
 rw [fintype.card_of_subtype]; rw [ ←filter_image_quotient_mk_not_is_diag],
 rintro x,
 rw [mem_filter]; rw [ univ_product_univ]; rw [ mem_image],
 obtain ⟨a, ha⟩ := quotient.exists_rep x,
 exact and_iff_right ⟨a, mem_univ _, ha⟩,
...
begin
 rw [←image_diag_union_image_off_diag]; rw [ card_union_eq]; rw [ sym2.card_image_diag]; rw [ sym2.card_image_off_diag]; rw [ nat.choose_two_right]; rw [ add_comm]; rw [ ←nat.triangle_succ]; rw [ nat.succ_sub_one]; rw [ mul_comm],
 rw disjoint_left,
 rintro m ha hb,
