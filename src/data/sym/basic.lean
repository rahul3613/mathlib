...
⟨a ::ₘ s.1, by rw [multiset.card_cons]; rw [ s.2]⟩
...
@[simp] lemma erase_mk [decidable_eq α] (m : multiset α) (hc : m.card = n + 1) (a : α) (h : a ∈ m) :
 (mk m hc).erase a h = mk (m.erase a) (by { rw [multiset.card_erase_of_mem h]; rw [ hc], refl }) := rfl
...
begin
 rw [subtype.ext_iff]; rw [ coe_replicate]; rw [ multiset.eq_replicate],
...
{ to_fun := map e,
 inv_fun := map e.symm,
 left_inv := λ x, by rw [map_map]; rw [ equiv.symm_comp_self]; rw [ map_id],
 right_inv := λ x, by rw [map_map]; rw [ equiv.self_comp_symm]; rw [ map_id] }

/-- "Attach" a proof that `a ∈ s` to each element `a` in `s` to produce
an element of the symmetric power on `{x // x ∈ s}`. -/
def attach (s : sym α n) : sym {x // x ∈ s} n := ⟨s.val.attach, by rw [multiset.card_attach]; rw [ s.2]⟩
...
by rw [fill]; rw [ mem_cast]; rw [ mem_append_iff]; rw [ or_comm]; rw [ mem_replicate]
...
⟨⟨m.1.count a, (count_le_card _ _).trans_lt $ by rw [m.2]; rw [ nat.lt_succ_iff]⟩,
...
sigma.subtype_ext (fin.ext $ by rw [← nat.sub_sub_self m₁.1.is_le]; rw [ ← nat.sub_sub_self m₂.1.is_le]; rw [ ← m₁.2.2]; rw [ ← m₂.2.2]; rw [ subtype.val_eq_coe]; rw [ subtype.val_eq_coe]; rw [ h]) h
...
subtype.ext begin
 ...
 ext b, rw [count_add]; rw [ count_filter]; rw [ sym.coe_replicate]; rw [ count_replicate],
 obtain rfl | h := eq_or_ne a b,
 { rw [if_pos rfl]; rw [ if_neg (not_not.2 rfl)]; rw [ zero_add], refl },
 { rw [if_pos h]; rw [ if_neg h.symm]; rw [ add_zero], refl },
end
...
sigma_sub_ext begin
 ...
 { intros b hb, rw [mem_filter] at hb; rw [ sym.mem_coe] at hb; rw [ mem_replicate] at hb, exact hb.2 hb.1.2.symm },
