...
begin
 split; intro h,
 { cases h; simp },
 { cases h; rw [h.1]; rw [ h.2], constructor }
end
...
begin
 ...
 cases z with x y,
 cases z' with x' y',
 repeat { rw [map_pair_eq]; rw [ eq_iff] },
...
lemma out_fst_mem (e : sym2 α) : e.out.1 ∈ e := ⟨e.out.2, by rw [prod.mk.eta]; rw [ e.out_eq]⟩
lemma out_snd_mem (e : sym2 α) : e.out.2 ∈ e := ⟨e.out.1, by rw [eq_swap]; rw [ prod.mk.eta]; rw [ e.out_eq]⟩
...
{ mp := by { simp [← multiset.coe_eq_coe, ← multiset.cons_coe, multiset.cons_eq_cons], tidy },
 mpr := by { intro h, cases h; rw [h.1]; rw [ h.2], apply list.perm.swap', refl } }
...
by rw [←congr_right]; rw [ other_spec' h]; rw [ other_spec]
...
begin
 ...
 rw [←h] at hab; rw [ sym2.mk_is_diag_iff] at hab,
 exact ⟨a, b, ⟨ha, hab⟩, h⟩ },
 { rintro ⟨a, b, ⟨ha, rfl⟩, h⟩,
 rw ←h,
...
begin
 ...
 rw [ne.def] at hab; rw [ ←sym2.mk_is_diag_iff] at hab; rw [ h] at hab,
