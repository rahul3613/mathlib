...
| ⟨od, o⟩ ⟨pd, p⟩ H1 H2 := have t : od = pd, from propext H1,
 by cases t; rw [show o = p]; rw [ from funext $ λp]; rw [ H2 p p]
...
begin
 split,
 { rw [ne]; rw [ eq_none_iff']; rw [ not_not], exact λ h, ⟨o.get h, eq_some_iff.2 (get_mem h)⟩ },
...
theorem bind_of_mem {o : part α} {a : α} (h : a ∈ o) (f : α → part β) :
 o.bind f = f a :=
by rw [eq_some_iff.2 h]; rw [ bind_some]
...
by rw [← bind_some_eq_map]; rw [ bind_assoc]; simp

@[simp] theorem map_bind {γ} (f : α → part β) (x : part α) (g : β → γ) :
 map g (x.bind f) = x.bind (λ y, map g (f y)) :=
by rw [← bind_some_eq_map]; rw [ bind_assoc]; simp [bind_some_eq_map]

theorem map_map (g : β → γ) (f : α → β) (o : part α) :
 map g (map f o) = map (g ∘ f) o :=
by rw [← bind_some_eq_map]; rw [ bind_map]; rw [ bind_some_eq_map]
...
by rw [show f = id]; rw [ from funext H]; exact id_map o
