...
begin
 ...
 rw [←lift_rel_o.swap]; rw [ computation.lift_rel.swap],
...
by rwa [lift_rel.swap, show swap R = R, from funext $ λ a, funext $ λ b, propext $ by constructor; apply H] at h
...
begin
 simp [tail], rw [← bind_pure_comp_eq_map]; rw [ is_lawful_monad.bind_assoc],
...
| (n+1) := show computation.bind (return none) (drop.aux n) = return none,
 by rw [ret_bind]; rw [ drop.aux_none]

theorem destruct_dropn :
 ∀ (s : wseq α) n, destruct (drop s n) = destruct s >>= drop.aux n
| s 0 := (bind_ret' _).symm
| s (n+1) := by rw [← dropn_tail]; rw [ destruct_dropn _ n]; rw [ destruct_tail]; rw [ is_lawful_monad.bind_assoc]; refl
...
by rw [lift_rel_destruct_iff]; rw [ lift_rel_destruct_iff]; simp
...
begin
 apply flatten_congr,
 unfold functor.map, rw [←bind_ret]; rw [ ←bind_ret],
...
⟨λ s t, ∀ n, nth s n ~ nth t n, h, λ s t h, begin
 ...
 { rw [←head_terminates_iff]; rw [ ←head_terminates_iff],
 ...
 rw [nth_tail]; rw [ nth_tail], apply h } }
end⟩
...
begin
 ...
 _ ⟨[], s, rfl, rfl⟩,
 intros s1 s2 h, rcases h with ⟨l, s, h⟩, rw [h.left]; rw [ h.right],
...
destruct_eq_ret $ begin
 ...
 rw [show seq.nth (some <$> s) 0 = some <$> seq.nth s 0]; rw [ by apply seq.map_nth],
...
begin
 ...
 rw [seq.head_cons]; rw [ seq.tail_cons], refl
end

@[simp] theorem dropn_of_seq (s : seq α) : ∀ n, drop (of_seq s) n = of_seq (s.drop n)
| 0 := rfl
| (n+1) := by dsimp [drop]; rw [dropn_of_seq]; rw [ tail_of_seq]

theorem nth_of_seq (s : seq α) (n) : nth (of_seq s) n = return (seq.nth s n) :=
by dsimp [nth]; rw [dropn_of_seq]; rw [ head_of_seq]; rw [ seq.head_dropn]
...
begin
 ...
 { intros c1 c2 h, cases h with s h, rw [h.left]; rw [ h.right],
...
begin
 ...
 intros c1 c2 h, rcases h with ⟨s, t, h⟩, rw [h.left]; rw [ h.right],
 apply s.rec_on _ (λ a s, _) (λ s, _); simp,
...
λ s1 s2 ⟨s, t, S, T, h1, h2, st, ST⟩, begin
 ...
 { rw [←lift_rel.swap R]; rw [ ←lift_rel.swap], apply ST },
 { rw [←lift_rel.swap R]; rw [ ←lift_rel.swap (lift_rel R)],
...
begin
 simp [bind], rw [← map_comp f (map g)]; rw [ map_comp (map g ∘ f) join],
 apply join_join
end
