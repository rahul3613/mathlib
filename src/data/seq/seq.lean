...
λ x y s t h, ⟨by rw [←option.some_inj]; rw [ ←nth_cons_zero]; rw [ h]; rw [ nth_cons_zero],
...
by rw [head_eq_destruct]; rw [ destruct_cons]; refl
...
begin
 ...
 { trivial },
 { intros x s,
 rw [cons_append]; rw [ destruct_cons]; rw [ destruct_cons], dsimp,
 exact ⟨rfl, s, rfl, rfl⟩ }
end
...
| ⟨s, al⟩ := begin
 ...
 rw [option.map_id]; rw [ stream.map_id]; refl
end
...
begin
 ...
 rw [nat.succ_eq_add_one]; rw [ ←nth_tail]; rw [ ←dropn_tail], apply IH
end
