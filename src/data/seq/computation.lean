...
section bisim
 ...
 { rw [destruct_ret] at this; rw [ destruct_think] at this,
...
by haveI := h1.terminates; haveI := h2.terminates; rw [←h1.length]; rw [ h2.length]
...
by haveI := h.terminates; exact ⟨think_mem h.mem, by rw [length_think]; rw [ h.length]⟩
...
by rw [lift_rel.swap]; rw [ lift_rel_return_left]
...
by rw [←lift_rel.swap R]; rw [ ←lift_rel.swap R]; apply lift_rel_think_left
...
by rw [←bind_ret]; rw [ ←bind_ret]; apply lift_rel_bind _ _ h1; simp; exact @h2
...
by rw [←lift_rel_aux.swap]; rw [ lift_rel_aux.ret_left]
