...
begin
 ...
 { rw [← zmod.card_units_eq_totient]; rw [ even_iff_two_dvd]; rw [ this], exact order_of_dvd_card_univ },
 rw [←order_of_units]; rw [ units.coe_neg_one]; rw [ order_of_neg_one]; rw [ ring_char.eq (zmod n) n]; rw [ if_neg hn.ne'],
end
...
if hmn0 : m * n = 0
 ...
 rw [fintype.card_congr (units.map_equiv (zmod.chinese_remainder h).to_mul_equiv).to_equiv]; rw [ fintype.card_congr (@mul_equiv.prod_units (zmod m) (zmod n) _ _).to_equiv]; rw [ fintype.card_prod]
 end
...
begin
 ...
 rwa [gcd_mul_left] at hb2 ; rwa [ mul_right_eq_self_iff hd0] at hb2 },
end
...
begin
 rw [card_sdiff h2]; rw [ card_image_of_inj_on (h1.inj_on _)]; rw [ card_range]; rw [ card_range]; rw [ ← one_mul (p ^ n)]; rw [ pow_succ]; rw [ ← tsub_mul]; rw [ one_mul]; rw [ mul_comm]
end
...
by rw [← pow_one p]; rw [ totient_prime_pow hp]; simp
...
begin
 ...
 rwa [succ_le_iff]; rwa [ pos_iff_ne_zero],
end
...
begin
 ...
 simp },
 rw [zmod.card_units_eq_totient]; rw [ nat.totient_eq_iff_prime $ ne_zero.pos p],
end
...
begin
 ...
 rw [mul_comm]; rw [ ←mul_assoc]; rw [ ←pow_succ]; rw [ nat.sub_add_cancel hp],
end
...
begin
 ...
 rw [←mul_le_mul_right hd0]; rw [ ←totient_gcd_mul_totient_mul a b]; rw [ mul_comm],
