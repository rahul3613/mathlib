...
⟨λ ⟨m, hm⟩, by norm_num [hm, add_mod],
 λ h, ⟨n / 2, (mod_add_div n 2).symm.trans (by rw [h]; rw [ add_comm])⟩⟩

lemma not_even_iff : ¬ even n ↔ n % 2 = 1 :=
by rw [even_iff]; rw [ mod_two_ne_zero]

lemma not_odd_iff : ¬ odd n ↔ n % 2 = 0 :=
by rw [odd_iff]; rw [ mod_two_ne_one]

lemma even_iff_not_odd : even n ↔ ¬ odd n :=
by rw [not_odd_iff]; rw [ even_iff]

@[simp] lemma odd_iff_not_even : odd n ↔ ¬ even n :=
by rw [not_even_iff]; rw [ odd_iff]
...
(even_or_odd m).elim (λ hm, by rw [even_iff.1 hm]; rw [ odd_iff.1 (hm.add_odd hn)]) $
 λ hm, by rw [odd_iff.1 hm]; rw [ even_iff.1 (hm.add_odd hn)]
...
by rw [add_comm]; rw [ mod_two_add_succ_mod_two]
...
by rw [even_add]; rw [ even_iff_not_odd]; rw [ even_iff_not_odd]; rw [ not_iff_not]
...
begin
 conv { to_rhs, rw [←tsub_add_cancel_of_le h]; rw [ even_add] },
 by_cases h : even n; simp [h]
end

theorem even_sub' (h : n ≤ m) : even (m - n) ↔ (odd m ↔ odd n) :=
by rw [even_sub h]; rw [ even_iff_not_odd]; rw [ even_iff_not_odd]; rw [ not_iff_not]
...
by rw [even_iff_two_dvd]; rw [ dvd_iff_mod_eq_zero]; rw [ nat.div_mod_eq_mod_mul_div]; rw [ mul_comm]

@[parity_simps] theorem odd_add : odd (m + n) ↔ (odd m ↔ even n) :=
by rw [odd_iff_not_even]; rw [ even_add]; rw [ not_iff]; rw [ odd_iff_not_even]

theorem odd_add' : odd (m + n) ↔ (odd n ↔ even m) :=
by rw [add_comm]; rw [ odd_add]
...
by rw [odd_iff_not_even]; rw [ even_sub h]; rw [ not_iff]; rw [ odd_iff_not_even]
...
by rw [odd_iff_not_even]; rw [ even_sub h]; rw [ not_iff]; rw [ not_iff_comm]; rw [ odd_iff_not_even]
...
by rw [←nat.bit0_eq_bit0]; rw [ bit0_eq_two_mul]; rw [ two_mul_div_two_of_even (even_bit0 n)]

lemma bit1_div_two : bit1 n / 2 = n :=
by rw [←nat.bit1_eq_bit1]; rw [ bit1]; rw [ bit0_eq_two_mul]; rw [ nat.two_mul_div_two_add_one_of_odd (odd_bit1 n)]

@[simp] lemma bit0_div_bit0 : bit0 n / bit0 m = n / m :=
by rw [bit0_eq_two_mul m]; rw [ ←nat.div_div_eq_div_mul]; rw [ bit0_div_two]

@[simp] lemma bit1_div_bit0 : bit1 n / bit0 m = n / m :=
by rw [bit0_eq_two_mul]; rw [ ←nat.div_div_eq_div_mul]; rw [ bit1_div_two]

@[simp] lemma bit0_mod_bit0 : bit0 n % bit0 m = bit0 (n % m) :=
by rw [bit0_eq_two_mul n]; rw [ bit0_eq_two_mul m]; rw [ bit0_eq_two_mul (n % m)]; rw [ nat.mul_mod_mul_left]
...
by rw [bit0]; rw [ ← two_mul]; rw [ iterate_mul]; rw [ involutive_iff_iter_2_eq_id.1 hf]; rw [ iterate_id]

theorem iterate_bit1 (hf : involutive f) (n : ℕ) : f^[bit1 n] = f :=
by rw [bit1]; rw [ iterate_succ]; rw [ hf.iterate_bit0]; rw [ comp.left_id]
...
⟨λ H, odd_iff_not_even.2 $ λ hn, hne $ by rwa [hf.iterate_even hn] at H; rwa [ eq_comm] at H, hf.iterate_odd⟩
