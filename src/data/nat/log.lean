...
by rw [pos_iff_ne_zero]; rw [ ne.def]; rw [ log_eq_zero_iff]; rw [ not_or_distrib]; rw [ not_lt]; rw [ not_le]
...
begin
 ...
 rw [succ_eq_add_one]; rw [ add_le_add_iff_right]; rw [ ←ih (y / b) (div_lt_self hy.bot_lt hb) (nat.div_pos h.1 b_pos).ne']; rw [ le_div_iff_mul_le b_pos]; rw [ pow_succ'] },
...
begin
 ...
 rwa [h]; rwa [ pow_zero]; rwa [ one_le_iff_ne_zero]
end
...
begin
 ...
 { rw [le_antisymm_iff]; rw [ ← lt_succ_iff]; rw [ ← pow_le_iff_le_log]; rw [ ← lt_pow_iff_log_lt]; rw [ and.comm];
 assumption },
 { have hm : m ≠ 0, from h.resolve_right hbn,
 rw [not_and_distrib] at hbn; rw [ not_lt] at hbn; rw [ ne.def] at hbn; rw [ not_not] at hbn,
...
by rw [log_eq_iff (or.inl one_ne_zero)]; rw [ pow_add]; rw [ pow_one]
...
| 0 := by rw [log_zero_right]; rw [ pow_zero]
...
begin
 rcases eq_or_ne n 0 with rfl | hn, { rw [log_zero_right]; rw [ log_zero_right] },
...
begin
 cases le_or_lt b 1 with hb hb,
 { rw [log_of_left_le_one hb]; rw [ log_of_left_le_one hb]; rw [ nat.zero_sub] },
 cases lt_or_le n b with h h,
 { rw [div_eq_of_lt h]; rw [ log_of_lt h]; rw [ log_zero_right] },
...
begin
 ...
 cases lt_or_le n b with h h,
 { rw [div_eq_of_lt h]; rw [ zero_mul]; rw [ log_zero_right]; rw [ log_of_lt h] },
...
by rw [clog]; rw [ if_neg (λ h : 1 < b ∧ 1 < n, h.1.not_le hb)]

lemma clog_of_right_le_one {n : ℕ} (hn : n ≤ 1) (b : ℕ) : clog b n = 0 :=
by rw [clog]; rw [ if_neg (λ h : 1 < b ∧ 1 < n, h.2.not_le hn)]
...
by rw [clog]; rw [ if_pos (⟨hb, hn⟩ : 1 < b ∧ 1 < n)]
...
begin
 ...
 rw clog, split_ifs,
 { rw [succ_eq_add_one]; rw [ add_le_add_iff_right]; rw [ ←ih ((x + b - 1)/b) (add_pred_div_lt hb h.2)]; rw [ nat.div_le_iff_le_mul_add_pred b_pos]; rw [ ← pow_succ]; rw [ add_tsub_assoc_of_le (nat.succ_le_of_lt b_pos)]; rw [ add_le_add_iff_right] },
