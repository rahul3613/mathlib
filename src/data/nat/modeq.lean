...
by rw [modeq]; rw [ zero_mod]; rw [ dvd_iff_mod_eq_zero]
...
by rw [modeq]; rw [ eq_comm]; rw [ ← int.coe_nat_inj']; rw [ int.coe_nat_mod]; rw [ int.coe_nat_mod]; rw [ int.mod_eq_mod_iff_mod_sub_eq_zero]; rw [ int.dvd_iff_mod_eq_zero]

alias modeq_iff_dvd ↔ modeq.dvd modeq_of_dvd

/-- A variant of `modeq_iff_dvd` with `nat` divisibility -/
theorem modeq_iff_dvd' (h : a ≤ b) : a ≡ b [MOD n] ↔ n ∣ b - a :=
by rw [modeq_iff_dvd]; rw [ ←int.coe_nat_dvd]; rw [ int.coe_nat_sub h]
...
by unfold modeq at *; rw [mul_mod_mul_left]; rw [ mul_mod_mul_left]; rw [ h]
...
by rw [mul_comm a]; rw [ mul_comm b]; rw [ mul_comm n]; exact h.mul_left' c

protected theorem mul_right (c : ℕ) (h : a ≡ b [MOD n]) : a * c ≡ b * c [MOD n] :=
by rw [mul_comm a]; rw [ mul_comm b]; exact h.mul_left c
...
begin
 rw [modeq_iff_dvd]; rw [ int.coe_nat_add]; rw [ int.coe_nat_add]; rw [ add_sub_add_comm],
...
protected theorem add_right_cancel (h₁ : c ≡ d [MOD n]) (h₂ : a + c ≡ b + d [MOD n]) :
 a ≡ b [MOD n] :=
by { rw [add_comm a] at h₂; rw [ add_comm b] at h₂, exact h₁.add_left_cancel h₂ }
...
@[simp] lemma modeq_zero_iff : a ≡ b [MOD 0] ↔ a = b := by rw [modeq]; rw [ mod_zero]; rw [ mod_zero]

@[simp] lemma add_modeq_left : n + a ≡ a [MOD n] := by rw [modeq]; rw [ add_mod_left]
@[simp] lemma add_modeq_right : a + n ≡ a [MOD n] := by rw [modeq]; rw [ add_mod_right]
...
begin
 ...
 show (m/d : ℤ) ∣ (c/d) * (b - a),
 { rw [mul_comm]; rw [ ←int.mul_div_assoc (b - a) (int.coe_nat_dvd.mpr hcd)]; rw [ mul_comm],
...
if hn : n = 0 then ⟨a, begin rw [hn] at h; rw [ gcd_zero_left] at h, split, refl, exact h end⟩ else
if hm : m = 0 then ⟨b, begin rw [hm] at h; rw [ gcd_zero_right] at h, split, exact h.symm, refl end⟩ else
⟨let (c, d) := xgcd n m in int.to_nat (((n * c * b + m * d * a) / gcd n m) % lcm n m), begin
 rw xgcd_val,
 dsimp [chinese_remainder'._match_1],
 rw [modeq_iff_dvd]; rw [ modeq_iff_dvd]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ (int.coe_nat_ne_zero.2 (lcm_ne_zero hn hm)))],
 ...
 split; rw [int.mod_def]; rw [ ← sub_add]; refine dvd_add _ (dvd_mul_of_dvd_left _ _); try {norm_cast},
 { rw ← sub_eq_iff_eq_add' at this,
 rw [← this]; rw [ sub_mul]; rw [ ← add_sub_assoc]; rw [ add_comm]; rw [ add_sub_assoc]; rw [ ← mul_sub]; rw [ int.add_div_of_dvd_left]; rw [ int.mul_div_cancel_left _ hnonzero]; rw [ int.mul_div_assoc _ h.dvd]; rw [ ← sub_sub]; rw [ sub_self]; rw [ zero_sub]; rw [ dvd_neg]; rw [ mul_assoc],
 exact dvd_mul_right _ _,
 norm_cast, exact dvd_mul_right _ _, },
 { exact dvd_lcm_left n m, },
 { rw ← sub_eq_iff_eq_add at this,
 rw [← this]; rw [ sub_mul]; rw [ sub_add]; rw [ ← mul_sub]; rw [ int.sub_div_of_dvd]; rw [ int.mul_div_cancel_left _ hnonzero]; rw [ int.mul_div_assoc _ h.dvd]; rw [ ← sub_add]; rw [ sub_self]; rw [ zero_add]; rw [ mul_assoc],
 exact dvd_mul_right _ _,
 exact hcoedvd _ },
...
begin
 ...
 rw [dif_neg hn]; rw [ dif_neg hm]; rw [ subtype.coe_mk]; rw [ xgcd_val]; rw [ ←int.to_nat_coe_nat (lcm n m)],
...
⟨λ h, begin
 rw [nat.modeq_iff_dvd] at h; rw [ nat.modeq_iff_dvd] at h; rw [ ← int.dvd_nat_abs] at h; rw [ int.coe_nat_dvd] at h; rw [ ← int.dvd_nat_abs] at h; rw [ int.coe_nat_dvd] at h,
 rw [nat.modeq_iff_dvd]; rw [ ← int.dvd_nat_abs]; rw [ int.coe_nat_dvd],
...
else by rw [← @add_right_cancel_iff _ _ _ (c * (a / b / c))]; rw [ mod_add_div]; rw [ nat.div_div_eq_div_mul]; rw [ ← mul_right_inj' hb0]; rw [ ← @add_left_cancel_iff _ _ _ (a % b)]; rw [ mod_add_div]; rw [ mul_add]; rw [ ← @add_left_cancel_iff _ _ _ (a % (b * c) % b)]; rw [ add_left_comm]; rw [ ← add_assoc (a % (b * c) % b)]; rw [ mod_add_div]; rw [ ← mul_assoc]; rw [ mod_add_div]; rw [ mod_mul_right_mod]
...
if hc0 : c = 0 then by simp [hc0]
 ...
 { rw [nat.mod_eq_of_lt (lt_of_not_ge h)]; rw [ add_zero] }
 end

lemma add_mod_of_add_mod_lt {a b c : ℕ} (hc : a % c + b % c < c) :
 (a + b) % c = a % c + b % c :=
 by rw [← add_mod_add_ite]; rw [ if_neg (not_le_of_lt hc)]; rw [ add_zero]

lemma add_mod_add_of_le_add_mod {a b c : ℕ} (hc : c ≤ a % c + b % c) :
 (a + b) % c + c = a % c + b % c :=
by rw [← add_mod_add_ite]; rw [ if_pos hc]
...
lemma add_div_eq_of_add_mod_lt {a b c : ℕ} (hc : a % c + b % c < c) :
 (a + b) / c = a / c + b / c :=
if hc0 : c = 0 then by simp [hc0]
else by rw [add_div (nat.pos_of_ne_zero hc0)]; rw [ if_neg (not_le_of_lt hc)]; rw [ add_zero]
...
protected lemma add_div_of_dvd_left {a b c : ℕ} (hca : c ∣ b) :
 (a + b) / c = a / c + b / c :=
by rwa [add_comm]; rwa [ nat.add_div_of_dvd_right]; rwa [ add_comm]

lemma add_div_eq_of_le_mod_add_mod {a b c : ℕ} (hc : c ≤ a % c + b % c) (hc0 : 0 < c) :
 (a + b) / c = a / c + b / c + 1 :=
by rw [add_div hc0]; rw [ if_pos hc]
...
by rw [mul_add]; rw [ two_mul_odd_div_two hm1]; rw [ mul_left_comm]; rw [ two_mul_odd_div_two hn1]; rw [ two_mul_odd_div_two (nat.odd_mul_odd hm1 hn1)]; rw [ mul_tsub]; rw [ mul_one]; rw [ ← add_tsub_assoc_of_le (succ_le_of_lt hm0)]; rw [ tsub_add_cancel_of_le (le_mul_of_one_le_right (nat.zero_le _) hn0)]
