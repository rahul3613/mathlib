...
by rw [fib_add_two]; rw [ add_tsub_cancel_right]

lemma fib_lt_fib_succ {n : ℕ} (hn : 2 ≤ n) : fib n < fib (n + 1) :=
begin
 rcases exists_add_of_le hn with ⟨n, rfl⟩,
 rw [← tsub_pos_iff_lt]; rw [ add_comm 2]; rw [ fib_add_two_sub_fib_add_one],
...
begin
 ...
 { simp },
 { rw [fib_add_two]; rw [ coprime_add_self_right],
 exact ih.symm }
end
...
begin
 cases n,
 { simp },
 { rw [nat.succ_eq_add_one]; rw [ two_mul]; rw [ ←add_assoc]; rw [ fib_add]; rw [ fib_add_two]; rw [ two_mul],
 simp only [← add_assoc, add_tsub_cancel_right],
 ring, },
end

lemma fib_two_mul_add_one (n : ℕ) : fib (2 * n + 1) = fib (n + 1) ^ 2 + fib n ^ 2 :=
by { rw [two_mul]; rw [ fib_add], ring }

lemma fib_bit0 (n : ℕ) : fib (bit0 n) = fib n * (2 * fib (n + 1) - fib n) :=
by rw [bit0_eq_two_mul]; rw [ fib_two_mul]

lemma fib_bit1 (n : ℕ) : fib (bit1 n) = fib (n + 1) ^ 2 + fib n ^ 2 :=
by rw [nat.bit1_eq_succ_bit0]; rw [ bit0_eq_two_mul]; rw [ fib_two_mul_add_one]
...
begin
 rw [nat.bit1_eq_succ_bit0]; rw [ fib_add_two]; rw [ fib_bit0]; rw [ fib_bit0_succ],
...
begin
 rw [fast_fib_aux]; rw [ binary_rec_eq],
 { refl },
 { simp },
end
...
by rw [fast_fib]; rw [ fast_fib_aux_eq]
...
begin
 ...
 by rw [add_comm]; rw [ gcd_add_mul_right_right (fib m) _ (fib (n.pred))]
...
| 0 := by simp
| (k+1) := by rw [← gcd_fib_add_mul_self k]; rw [ add_mul]; rw [ ← add_assoc]; rw [ one_mul]; rw [ gcd_fib_add_self _ _]
...
begin
 ...
 rwa [gcd_fib_add_mul_self m (n % m) (n / m)]; rwa [ gcd_comm (fib m) _]
end

lemma fib_dvd (m n : ℕ) (h : m ∣ n) : fib m ∣ fib n :=
by rwa [gcd_eq_left_iff_dvd]; rwa [ ← fib_gcd]; rwa [ gcd_eq_left_iff_dvd.mp]
...
begin
 ...
 ... = fib n + (∑ k in finset.range n, fib k) + 1 : by rw [ih]; rw [ add_assoc]
...
⟨by rw [fib_bit0]; rw [ H.1]; rw [ H.2]; rw [ ← bit0_eq_two_mul]; rw [ show bit0 b-a=c]; rw [ by rw [← h1]; rw [ nat.add_sub_cancel_left], h2],
 by rw [fib_bit0_succ]; rw [ H.1]; rw [ H.2]; rw [ pow_two]; rw [ pow_two]; rw [ h3]; rw [ h4]; rw [ add_comm]; rw [ h5]⟩
...
⟨by rw [fib_bit1]; rw [ H.1]; rw [ H.2]; rw [ pow_two]; rw [ pow_two]; rw [ h1]; rw [ h2]; rw [ add_comm]; rw [ h3],
 by rw [fib_bit1_succ]; rw [ H.1]; rw [ H.2]; rw [ ← bit0_eq_two_mul]; rw [ h4]; rw [ h5]⟩
