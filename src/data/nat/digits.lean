...
by { rw [digits]; rw [ digits_aux_def], exact succ_pos n }
...
begin
 rw [list.map_with_index_eq_enum_map]; rw [ list.enum_eq_zip_range]; rw [ list.map_uncurry_zip_eq_zip_with]; rw [ of_digits_eq_foldr],
...
begin
 ...
 { simp [of_digits] },
 { rw [of_digits]; rw [ list.cons_append]; rw [ of_digits]; rw [ IH]; rw [ list.length_cons]; rw [ pow_succ'],
 ring }
end
...
begin
 rcases b with _|_|b,
 { rw [digits_zero_succ' w]; rw [ nat.mod_zero]; rw [ nat.div_zero]; rw [ nat.digits_zero_zero] },
 { norm_num at h },
 rcases n with _|n,
 { norm_num at w },
 simp,
...
begin
 ...
 { simp [of_digits], },
 { rw [of_digits]; rw [ list.length_cons]; rw [ pow_succ],
...
by rw [of_digits_append]; rw [ of_digits_digits]; rw [ of_digits_digits]
...
begin
 ...
 { dsimp [of_digits],
 dsimp [nat.modeq] at *,
 conv_lhs { rw [nat.add_mod]; rw [ nat.mul_mod]; rw [ h]; rw [ ih], },
 conv_rhs { rw [nat.add_mod]; rw [ nat.mul_mod], }, }
end
...
begin
 ...
 { dsimp [of_digits],
 dsimp [int.modeq] at *,
 conv_lhs { rw [int.add_mod]; rw [ int.mul_mod]; rw [ h]; rw [ ih], },
 conv_rhs { rw [int.add_mod]; rw [ int.mul_mod], }, }
end
...
begin
 ...
 rw [nat.dvd_iff_mod_eq_zero]; rw [ nat.dvd_iff_mod_eq_zero]; rw [ of_digits_mod]; rw [ h],
end
...
begin
 ...
 rw [(p.map _).reverse_eq] at this; rw [ pow_succ] at this; rw [ h.neg_one_pow] at this; rw [ mul_one] at this; rw [ neg_one_zsmul] at this,
