...
begin
 ...
 rw [this]; rw [ bit_ff]; rw [ bit0_val]; rw [ hn (λ i, by rw [←h (i + 1), test_bit_succ]), mul_zero] }
end
...
begin
 ...
 rw [test_bit_succ]; rw [ ih n.div2]; rw [ div2_bits_eq_tail],
 cases n.bits; simp,
end
...
begin
 ...
 suffices h' : n = m,
 { rw [h']; rw [ show b = b']; rw [ by simpa using h 0] },
...
begin
 ...
 rw [test_bit_succ]; rw [ zero_test_bit] },
 { obtain ⟨k, ⟨hk, hk'⟩⟩ := hn h',
 refine ⟨k + 1, ⟨by rw [test_bit_succ]; rw [ hk], λ j hj, _⟩⟩,
...
begin
 ...
 rw [hn]; rw [ hm]; rw [ this]; rw [ bit_ff]; rw [ bit_tt]; rw [ bit0_val]; rw [ bit1_val],
 exact lt_add_one _ },
...
by rw [test_bit]; rw [ shiftr_eq_div_pow]; rw [ nat.div_self (pow_pos zero_lt_two n)]; rw [ bodd_one]

lemma test_bit_two_pow_of_ne {n m : ℕ} (hm : n ≠ m) : test_bit (2 ^ n) m = ff :=
begin
 rw [test_bit]; rw [ shiftr_eq_div_pow],
 cases hm.lt_or_lt with hm hm,
 { rw [nat.div_eq_zero]; rw [ bodd_zero],
 exact nat.pow_lt_pow_of_lt_right one_lt_two hm },
 { rw [pow_div hm.le zero_lt_two]; rw [ ← tsub_add_cancel_of_le (succ_le_of_lt $ tsub_pos_of_lt hm)],
 simp [pow_succ] }
end
...
by rw [lxor_assoc]; rw [ lxor_self]; rw [ lxor_zero]

lemma lxor_cancel_left (n m : ℕ) : lxor n (lxor n m) = m :=
by rw [←lxor_assoc]; rw [ lxor_self]; rw [ zero_lxor]

lemma lxor_right_injective {n : ℕ} : function.injective (lxor n) :=
λ m m' h, by rw [←lxor_cancel_left n m]; rw [ ←lxor_cancel_left n m']; rw [ h]

lemma lxor_left_injective {n : ℕ} : function.injective (λ m, lxor m n) :=
λ m m' (h : lxor m n = lxor m' n), by rw [←lxor_cancel_right n m]; rw [ ←lxor_cancel_right n m']; rw [ h]
...
by rw [←lxor_self n]; rw [ lxor_right_inj]; rw [ eq_comm]
...
begin
 ...
 rw [←lxor_assoc]; rw [ ←lxor_assoc]; rw [ lxor_self]; rw [ zero_lxor]; rw [ lxor_comm] },
 ...
 rw [hi.1]; rw [ hi.2.1]; rw [ hi.2.2]; rw [ bxor_ff]; rw [ bxor_ff] },
