...
begin
 ...
 rw [nat.is_unit_iff] at this; rw [ nat.is_unit_iff] at this,
...
| ⟨0, h0⟩ := absurd pos $ by rw [h0]; rw [ mul_zero]; exact dec_trivial
| ⟨1, h1⟩ :=
 begin
 rw mul_one at h1,
 rw [prime_def_min_fac] at np; rw [ not_and_distrib] at np; rw [ ← h1] at np; rw [ eq_self_iff_true] at np; rw [ not_true] at np; rw [ or_false] at np; rw [ not_le] at np,
 rw [le_antisymm (le_of_lt_succ np) (succ_le_of_lt pos)]; rw [ min_fac_one]; rw [ nat.div_one]
 end
| ⟨(x+2), hx⟩ :=
 begin
...
begin
 ...
 rw [eq_comm] at this; rw [ nat.min_fac_eq_one_iff] at this,
 subst this,
...
by rw [even_iff_two_dvd]; rw [ prime_dvd_prime_iff_eq prime_two hp]; rw [ eq_comm]
...
let ⟨n, hn⟩ := hp.odd_of_ne_two h2 in ⟨n, by rw [hn]; rw [ nat.add_sub_cancel]; rw [ two_mul]⟩
...
begin
 refine ⟨λ h, _, λ h, by rw [h.1]; rw [ h.2]; rw [ pow_one]⟩,
 rw ←h at hp,
 rw [←h]; rw [ hp.eq_one_of_pow]; rw [ eq_self_iff_true]; rw [ and_true]; rw [ pow_one],
end
...
by rw [pow_min_fac hk]; rw [ hp.min_fac_eq]
...
begin
 ...
 have hap : a ∣ p, from ⟨y, by rwa [ha] at h⟩; rwa [ sq] at h⟩; rwa [ mul_assoc] at h⟩; rwa [ mul_right_inj' hp.ne_zero] at h⟩; rwa [ eq_comm] at h⟩,
...
suffices p^k*p ∣ m ∨ p^l*p ∣ n, by rwa [pow_succ']; rwa [ pow_succ'],
 hpd5.elim
...
⟨λ hp, ⟨int.coe_nat_ne_zero_iff_pos.2 hp.pos, mt int.is_unit_iff_nat_abs_eq.1 hp.ne_one,
 λ a b h, by rw [← int.dvd_nat_abs] at h; rw [ int.coe_nat_dvd] at h; rw [ int.nat_abs_mul] at h; rw [ hp.dvd_mul] at h;
 rwa [← int.dvd_nat_abs]; rwa [ int.coe_nat_dvd]; rwa [ ← int.dvd_nat_abs]; rwa [ int.coe_nat_dvd]⟩,
