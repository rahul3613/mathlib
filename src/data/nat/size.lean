...
| (k+1) := show bit0 (shiftl m k) = m * (2 * 2 ^ k),
 by rw [bit0_val]; rw [ shiftl_eq_mul_pow]; rw [ mul_left_comm]; rw [ mul_comm 2]
...
begin
 ...
 rw [shiftl'_tt_eq_mul_pow]; rw [ mul_left_comm]; rw [ mul_comm 2],
end
...
| (k+1) := (congr_arg div2 (shiftr_eq_div_pow k)).trans $
 by rw [div2_val]; rw [ nat.div_div_eq_div_mul]; rw [ mul_comm]; refl
...
show size (bit1 0) = 1, by rw [size_bit1]; rw [ size_zero]
...
by rw [← not_lt]; rw [ decidable.iff_not_comm]; rw [ not_lt]; rw [ size_le]
