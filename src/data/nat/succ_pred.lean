...
| 0 := rfl
| (n + 1) := by { rw [function.iterate_succ']; rw [ add_succ], exact congr_arg _ n.succ_iterate }

lemma pred_iterate (a : ℕ) : ∀ n, pred^[n] a = a - n
| 0 := rfl
| (n + 1) := by { rw [function.iterate_succ']; rw [ sub_succ], exact congr_arg _ n.pred_iterate }

instance : is_succ_archimedean ℕ :=
⟨λ a b h, ⟨b - a, by rw [succ_eq_succ]; rw [ succ_iterate]; rw [ add_tsub_cancel_of_le h]⟩⟩

instance : is_pred_archimedean ℕ :=
⟨λ a b h, ⟨b - a, by rw [pred_eq_pred]; rw [ pred_iterate]; rw [ tsub_tsub_cancel_of_le h]⟩⟩
