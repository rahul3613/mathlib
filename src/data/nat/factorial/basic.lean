...
| m 0 := by simp
| m (n+1) :=
by rw [← add_assoc]; rw [ nat.factorial_succ]; rw [ mul_comm (nat.succ _)]; rw [ pow_succ']; rw [ ← mul_assoc];
...
begin
 ...
 { rw [← factorial_lt $ pos_of_gt $ one_lt_factorial.mp hn] at hnm; rw [ h] at hnm,
...
begin
 rw [← succ_pred_eq_of_pos ((zero_lt_two.trans (lt.base 2)).trans_le hi)]; rw [ factorial_succ],
...
lemma add_factorial_succ_lt_factorial_add_succ {i : ℕ} (n : ℕ) (hi : 2 ≤ i) :
 i + (n + 1)! < (i + n + 1)! :=
begin
 rw [factorial_succ (i + _)]; rw [ add_mul]; rw [ one_mul],
...
begin
 ...
 { rw [←add_assoc]; rw [ factorial_succ (1 + n)]; rw [ add_mul]; rw [ one_mul]; rw [ add_comm 1 n],
 exact (add_le_add_iff_right _).mpr (one_le_mul (nat.le_add_left 1 n) (n + 1).factorial_pos) },
 rw [le_zero_iff.mp (nat.succ_le_succ_iff.mp i0)]; rw [ zero_add]; rw [ zero_add]
end
...
begin
 ...
 { refl },
 rw [asc_factorial]; rw [ ht]; rw [ zero_add]; rw [ nat.factorial_succ],
end
...
| 0 := by rw [add_zero]; rw [ asc_factorial_zero]; rw [ asc_factorial_zero]
| (k + 1) := by rw [asc_factorial]; rw [ mul_left_comm]; rw [ succ_asc_factorial]; rw [ asc_factorial]; rw [ succ_add]; rw [ ←add_assoc]
...
| 0 := by rw [asc_factorial]; rw [ add_zero]; rw [ mul_one]
| (k + 1) := by rw [asc_factorial_succ]; rw [ mul_left_comm]; rw [ factorial_mul_asc_factorial]; rw [ ← add_assoc]; rw [ factorial]
...
begin
 ...
 suffices h' : n - k = t.succ, by rw [←ht]; rw [ h']; rw [ succ_asc_factorial]; rw [ asc_factorial_succ],
...
| 0 := by rw [asc_factorial_zero]; rw [ pow_zero]
| (k + 1) := begin
 rw pow_succ,
...
| 0 := by rw [asc_factorial_zero]; rw [ pow_zero]
| (k + 1) := begin
 rw [asc_factorial_succ]; rw [ pow_succ],
...
by rw [desc_factorial_succ]; rw [ zero_tsub]; rw [ zero_mul]

@[simp] lemma desc_factorial_one (n : ℕ) :
 n.desc_factorial 1 = n :=
by rw [desc_factorial_succ]; rw [ desc_factorial_zero]; rw [ mul_one]; rw [ tsub_zero]

@[simp] lemma succ_desc_factorial_succ (n : ℕ) :
 ∀ k : ℕ, (n + 1).desc_factorial (k + 1) = (n + 1) * n.desc_factorial k
| 0 := by rw [desc_factorial_zero]; rw [ desc_factorial_one]; rw [ mul_one]
| (succ k) := by rw [desc_factorial_succ]; rw [ succ_desc_factorial_succ]; rw [ desc_factorial_succ]; rw [ succ_sub_succ]; rw [ mul_left_comm]

lemma succ_desc_factorial (n : ℕ) :
 ∀ k, (n + 1 - k) * (n + 1).desc_factorial k = (n + 1) * n.desc_factorial k
| 0 := by rw [tsub_zero]; rw [ desc_factorial_zero]; rw [ desc_factorial_zero]
| (k + 1) := by rw [desc_factorial]; rw [ succ_desc_factorial]; rw [ desc_factorial_succ]; rw [ succ_sub_succ]; rw [ mul_left_comm]

lemma desc_factorial_self : ∀ n : ℕ, n.desc_factorial n = n!
| 0 := by rw [desc_factorial_zero]; rw [ factorial_zero]
| (succ n) := by rw [succ_desc_factorial_succ]; rw [ desc_factorial_self]; rw [ factorial_succ]
...
| (succ k) := begin
 rw [desc_factorial_succ]; rw [ mul_eq_zero]; rw [ desc_factorial_eq_zero_iff_lt]; rw [ lt_succ_iff]; rw [ tsub_eq_zero_iff_le]; rw [ lt_iff_le_and_ne]; rw [ or_iff_left_iff_imp]; rw [ and_imp],
 exact λ h _, h,
end
...
| 0 := by rw [asc_factorial_zero]; rw [ desc_factorial_zero]
| (succ k) := by rw [nat.add_succ]; rw [ succ_desc_factorial_succ]; rw [ asc_factorial_succ]; rw [ add_desc_factorial_eq_asc_factorial]
...
| n 0 := λ _, by rw [desc_factorial_zero]; rw [ mul_one]; rw [ tsub_zero]
| 0 (succ k) := λ h, by { exfalso, exact not_succ_le_zero k h }
| (succ n) (succ k) := λ h, by rw [succ_desc_factorial_succ]; rw [ succ_sub_succ]; rw [ ←mul_assoc]; rw [ mul_comm (n - k)!]; rw [ mul_assoc]; rw [ factorial_mul_desc_factorial (nat.succ_le_succ_iff.1 h)]; rw [ factorial_succ]
...
| 0 := by rw [desc_factorial_zero]; rw [ pow_zero]
| (k + 1) := begin
 rw [desc_factorial_succ]; rw [ pow_succ]; rw [ succ_sub_succ],
...
| 0 := λ h, begin
 rw [desc_factorial_succ]; rw [ pow_succ]; rw [ pow_one]; rw [ desc_factorial_one],
 exact nat.mul_lt_mul_of_pos_left (tsub_lt_self (lt_of_lt_of_le zero_lt_two h) zero_lt_one)
 (tsub_pos_of_lt h),
end
| (k + 1) := λ h, begin
 rw [desc_factorial_succ]; rw [ pow_succ],
...
| 1 := by rintro (_ | ⟨⟨⟩⟩)
| (k + 2) := λ _, begin
 rw [desc_factorial_succ]; rw [ pow_succ']; rw [ mul_comm],
