...
| 0 := rfl
| (k + 1) := begin
 rw [double_factorial_add_two]; rw [ factorial]; rw [ factorial_eq_mul_double_factorial]; rw [ mul_comm _ (k‼)]; rw [ mul_assoc]
end
...
| 0 := rfl
| (n + 1) := begin
 rw [mul_add]; rw [ mul_one]; rw [ double_factorial_add_two]; rw [ factorial]; rw [ pow_succ]; rw [ double_factorial_two_mul]; rw [ succ_eq_add_one],
 ring,
end
...
| 0 := rfl
| (n + 1) := begin
 rw [finset.prod_range_succ]; rw [ ← double_factorial_eq_prod_even]; rw [ mul_comm (2 * n)‼]; rw [ (by ring : 2 * (n + 1) = 2 * n + 2)],
 refl,
end
...
| 0 := rfl
| (n + 1) := begin
 rw [finset.prod_range_succ]; rw [ ← double_factorial_eq_prod_odd]; rw [ mul_comm (2 * n + 1)‼]; rw [ (by ring : 2 * (n + 1) + 1 = (2 * n + 1) + 2)],
 refl,
end
