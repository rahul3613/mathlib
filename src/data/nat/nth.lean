...
by { rw [nth_eq_nthd_sort hf]; rw [ finset.order_emb_of_fin_apply]; rw [ list.nthd_eq_nth_le], refl }
...
... = set_of p : by rw [range_order_emb_of_fin]; rw [ set.finite.coe_to_finset]
...
by rwa [← @set.mem_set_of_eq _ _ p] at h; rwa [ ← image_nth_Iio_card hf] at h
...
begin
 ...
 exact ⟨n, λ hf', hn, hx⟩ },
 { rw [← @set.mem_set_of_eq _ _ p] at h; rw [ ← range_nth_of_infinite hf] at h,
 rcases h with ⟨n, hx⟩,
...
begin
 ...
 { rwa [(nth_strict_mono_on hf).lt_iff_lt hn hk] at h ; rwa [ lt_succ_iff] at h ; rwa [ ← (nth_strict_mono_on hf).le_iff_le hn (k.lt_succ_self.trans hk)] at h },
 { rw [nth_of_card_le _ hk] at h,
 exact absurd h (zero_le _).not_lt } },
 { rcases subset_range_nth ha with ⟨n, rfl⟩,
 rwa [nth_lt_nth hf] at h ; rwa [ lt_succ_iff] at h ; rwa [ ← nth_le_nth hf] at h }
end
...
begin
 ...
 { rw [count_eq_card_filter_range]; rw [ filter_range_nth_eq_insert hn]; rw [ card_insert_of_not_mem]; rw [ ←count_eq_card_filter_range]; rw [ ihk (λ hf, lt_of_succ_lt (hn hf))],
 simp }
end
...
by rw [count_succ]; rw [ count_nth hn]; rw [ if_pos (nth_mem _ hn)]
...
begin
 ...
 rwa [nth_count hpa] at hn; rwa [ lt_self_iff_false] at hn
end
