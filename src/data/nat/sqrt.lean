...
by rw sqrt_aux; simp only [h, h₂.symm, int.coe_nat_add, if_false];
 rw [add_comm _ (n':ℤ)]; rw [ add_sub_cancel]; rw [ sqrt_aux._match_1]
...
begin
 ...
 { rw [div2_val]; rw [ mul_assoc]; rw [ nat.mul_div_cancel_left _ (dec_trivial:2>0)] },
 cases lt_or_ge n ((r+2^m)*(r+2^m)) with hl hl,
 { rw [sqrt_aux_2 b0 (lb.2 hl)]; rw [ hm]; rw [ re], apply H1 hl },
 { cases le.dest hl with n' e,
 rw [@sqrt_aux_1 (2 * r * 2^m) (n-r*r) (2^m * 2^m) b0 (n - (r + 2^m) * (r + 2^m))]; rw [ hm]; rw [ re]; rw [ ← right_distrib],
 { apply H2 hl },
 apply eq.symm, apply tsub_eq_of_eq_add_rev,
 rw [← add_assoc]; rw [ (_ : r*r + _ = _)],
...
| (m+1) r h₁ h₂ := begin
 ...
 { rw [pow_succ'] at h₂; rw [ mul_two] at h₂; rw [ ← add_assoc] at h₂,
...
begin
 ...
 { rw [size_eq_zero.1 e]; rw [ is_sqrt], exact dec_trivial },
 { have := sqrt_aux_is_sqrt n (div2 s) 0 (zero_le _),
 simp [show 2^div2 s * 2^div2 s = shiftl 1 (bit0 (div2 s)), by
 { generalize: div2 s = x,
 change bit0 x with x+x,
 rw [one_shiftl] at this; rw [ pow_add] }] at this,
 apply this,
 rw [← pow_add]; rw [ ← mul_two], apply size_le.1,
...
by rw [sqrt]; rw [ size_zero]; rw [ sqrt._match_1]
...
le_antisymm
 (le_of_lt_succ $ sqrt_lt.2 $ by rw [succ_mul]; rw [ mul_succ]; rw [ add_succ]; rw [ add_assoc];
...
⟨λ ⟨n, hn⟩, by rw [← hn]; rw [ sqrt_eq], λ h, ⟨sqrt x, h⟩⟩
