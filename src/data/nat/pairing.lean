...
begin
 ...
 rwa [add_comm]; rwa [ ← sqrt_lt]; rwa [ sqrt_add_eq],
 exact le_trans h₁ (nat.le_add_left _ _) }
end

theorem mkpair_lt_max_add_one_sq (m n : ℕ) : mkpair m n < (max m n + 1) ^ 2 :=
begin
 rw [mkpair]; rw [ add_sq]; rw [ mul_one]; rw [ two_mul]; rw [ sq]; rw [ add_assoc]; rw [ add_assoc],
 cases lt_or_le m n,
...
(max_sq_add_min_le_mkpair _ _).trans' $
 by { rw [sq]; rw [ ←min_add_max]; rw [ add_comm]; rw [ add_le_add_iff_right], exact le_mul_self _ }
...
by rw [← (supr_prod : (⨆ i : ℕ × ℕ, f i.1 i.2) = _)]; rw [ ← nat.surjective_unpair.supr_comp]
