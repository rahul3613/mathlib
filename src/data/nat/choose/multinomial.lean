...
begin
 ...
 function.comp_app],
 rw [div_mul_div_comm ((f a).factorial_mul_factorial_dvd_factorial_add (s.sum f)) (prod_factorial_dvd_factorial_sum _ _)]; rw [ mul_comm (f a)! (s.sum f)!]; rw [ mul_assoc]; rw [ mul_comm _ (s.sum f)!]; rw [ nat.mul_div_mul _ _ (factorial_pos _)],
end
...
begin
 ...
 { rw [← finset.insert_erase h]; rw [ nat.multinomial_insert _ f (finset.not_mem_erase a _)]; rw [ finset.add_sum_erase _ f h]; rw [ support_update_zero], congr' 1,
...
begin
 ...
 { rw [← finsupp.card_to_multiset]; rw [ m.to_finsupp_to_multiset] },
 { ext1 a', rw [to_finsupp_apply]; rw [ count_filter]; rw [ finsupp.coe_update],
 split_ifs,
 { rw [function.update_noteq h.symm]; rw [ to_finsupp_apply] },
 { rw [not_ne_iff.1 h]; rw [ function.update_same] } },
end
...
begin
 ...
 conv in (m.1.1.map _) { rw [← m.1.1.filter_add_not ((=) a)]; rw [ multiset.map_add] },
