...
begin
 ...
 rw [factorization_def _ hp]; rw [ @padic_val_nat_def _ ⟨hp⟩ _ (choose_pos hkn)],
...
begin
 ...
 { rw [pow_one]; rw [ ←add_lt_add_iff_left (2 * p)]; rw [ ←succ_mul]; rw [ two_mul]; rw [ add_add_add_comm],
 ...
 (by rwa [div_add_mod]; rwa [ div_add_mod]; rwa [ add_tsub_cancel_of_le hkn]) },
 { replace hn : n < p ^ i,
 { calc n < 3 * p : hn
 ... ≤ p * p : mul_le_mul_right' (lt_of_le_of_ne hp.two_le hp'.symm) p
 ... = p ^ 2 : (sq p).symm
 ... ≤ p ^ i : pow_le_pow hp.one_lt.le hi },
 rwa [mod_eq_of_lt (lt_of_le_of_lt hkn hn)]; rwa [ mod_eq_of_lt (lt_of_le_of_lt tsub_le_self hn)]; rwa [ add_tsub_cancel_of_le hkn] },
end
...
begin
 ...
 { rintro rfl, linarith },
 { rw [two_mul]; rw [ add_tsub_cancel_left] },
end
...
begin
 ...
 rw finset.mem_range,
 contrapose! hp,
 rw [finsupp.mem_support_iff]; rw [ not_not]; rw [ factorization_choose_eq_zero_of_lt hp] },
