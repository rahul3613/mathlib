...
⟨λ h, by rw [← mod_add_div a b]; rw [ h]; rw [ mul_zero]; rw [ add_zero]; exact mod_lt _ hb,
 λ h, by rw [← mul_right_inj' hb.ne']; rw [ ← @add_left_cancel_iff _ _ _ (a % b)]; rw [ mod_add_div]; rw [ mod_eq_of_lt h]; rw [ mul_zero]; rw [ add_zero]⟩
...
by { rw [bit1]; rw [ nat.dvd_add_right two_dvd_bit0]; rw [ nat.dvd_one], cc }
...
| (a+1) (b+1) := begin
 ...
 rw [if_pos h₁]; rw [ if_pos h₂]; rw [ add_tsub_add_eq_tsub_right]; rw [ ← tsub_add_eq_add_tsub hb_le_a]; rw [ by exact have _ := wf]; rw [ succ_div (a - b)]; rw [ add_tsub_add_eq_tsub_right],
...
lemma succ_div_of_dvd {a b : ℕ} (hba : b ∣ a + 1) :
 (a + 1) / b = a / b + 1 :=
by rw [succ_div]; rw [ if_pos hba]

lemma succ_div_of_not_dvd {a b : ℕ} (hba : ¬ b ∣ a + 1) :
 (a + 1) / b = a / b :=
by rw [succ_div]; rw [ if_neg hba]; rw [ add_zero]
...
begin
 ...
 rw [nat.mul_div_cancel_left k (nat.pos_of_ne_zero a_split)]; rw [ pr]; rw [ nat.mul_div_cancel_left k2 (nat.pos_of_ne_zero c_split)]; rw [ nat.mul_comm ((c + 1) * k2) k]; rw [ ←nat.mul_assoc k (c + 1) k2]; rw [ nat.mul_div_cancel _ (nat.pos_of_ne_zero k2_nonzero)]; rw [ nat.mul_div_cancel _ (nat.pos_of_ne_zero c_split)],
end
