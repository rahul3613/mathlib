...
by rw [eq_comm]; rw [ nat.mul_eq_zero]
...
begin
 rw [le_add_one_iff]; rw [ and_or_distrib_left]; rw [ ←le_antisymm_iff]; rw [ eq_comm]; rw [ and_iff_right_of_imp],
 rintro rfl,
 exact n.le_succ
end
...
begin
 ...
 rw [m.add_sub_assoc hn] at h'; rw [ add_lt_add_iff_left] at h',
...
begin
 cases m,
 { cases hm rfl },
 { rw [succ_add]; rw [ succ_sub_one]; rw [ succ_mul],
...
begin
 ...
 { have : a + 1 = b ∨ a + 1 < b,
 { rwa [← le_iff_eq_or_lt]; rwa [ ← nat.lt_succ_iff] },
...
(nat.eq_zero_or_pos k).elim
 (λ k0, by rw [k0]; rw [ nat.div_zero]; apply zero_le)
...
(nat.eq_zero_or_pos n).elim
 (λ n0, by rw [n0]; rw [ nat.div_zero]; apply zero_le)
...
if hm0 : m = 0 then by simp [hm0]
 ...
 ... = n / k : by rw [nat.div_div_eq_div_mul]; rw [ mul_comm n]; rw [ mul_comm k]; rw [ nat.mul_div_mul _ _ (nat.pos_of_ne_zero hm0)]
...
by conv {to_rhs, rw [← nat.mod_add_div n 2]; rw [ hn]; rw [ add_tsub_cancel_left]}
...
by rw [mul_comm k]; rw [ mod_mul_right_div_self]
...
by rw [←nat.mod_add_div m k]; rw [ ←nat.mod_add_div n k]; rw [ ←h]; rw [ tsub_add_eq_tsub_tsub]; rw [ add_tsub_cancel_left]; rw [ ←mul_tsub]; rw [ nat.mul_mod_right]
...
begin
 ...
 rw nat.add_mod,
 split_ifs with h,
 { rw [nat.mod_eq_sub_mod h]; rw [ nat.mod_eq_of_lt],
...
begin
 by_cases n0 : n = 0,
 { rw [n0]; rw [ nat.div_zero]; rw [ nat.div_zero] },
 { rw [← mod_add_div m n] { occs := occurrences.pos [2] },
 rw [add_tsub_cancel_left]; rw [ mul_div_right _ (nat.pos_of_ne_zero n0)] }
end
...
by rw [pos_iff_ne_zero]; rw [ ne]; rw [ nat.find_eq_zero]
...
begin
 ...
 { rw [find_greatest_of_not hk]; rw [ ihk],
 split,
 { rintros ⟨hle, hP, hm⟩,
...
begin
 ...
 { refl },
 by_cases P (n + 1),
 { rw [find_greatest_eq h]; rw [ find_greatest_eq (hPQ _ h)] },
...
⟨λ h, by rwa [← nat.lt_succ_iff] at h; rwa [ n.bit1_eq_succ_bit0] at h; rwa [ ← n.bit0_succ_eq] at h; rwa [ bit0_lt_bit0] at h; rwa [ nat.lt_succ_iff] at h, λ h, le_of_lt (nat.bit0_lt_bit1 h)⟩
...
⟨λ h, by rwa [m.bit1_eq_succ_bit0] at h; rwa [ succ_le_iff] at h; rwa [ bit0_lt_bit0] at h,
