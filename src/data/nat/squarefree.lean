...
begin
 rw [unique_factorization_monoid.squarefree_iff_nodup_normalized_factors h0]; rw [ nat.factors_eq],
 simp,
end
...
begin
 ...
 cases h₂,
 { rwa [h₂]; rwa [ eq_comm]; rwa [ ←h₁] },
 { rw [h₂]; rw [ h₃.resolve_left],
 rw [←h₁]; rw [ h₂],
...
begin
 ...
 cases o with d,
 { rw [min_sq_fac_prop] at H ⊢; rw [ squarefree_iff_prime_squarefree] at H ⊢,
...
| n k := λ n0 i e ih, begin
 ...
 rw [← me] at d; rw [ e] at d, change 2 * (i + 2) ∣ n' at d,
...
begin
 ...
 rw [multiset.to_finset_subset]; rw [ multiset.to_finset_val]; rw [ hsq.dedup]; rw [ ← associated_iff_eq]; rw [ normalized_factors_mul h0.1 h0.2],
 exact ⟨multiset.subset_of_le (multiset.le_add_right _ _), normalized_factors_prod h0.1⟩ },
 { rintro ⟨s, hs, rfl⟩,
 rw [finset.mem_powerset] at hs; rw [ ← finset.val_le_iff] at hs; rw [ multiset.to_finset_val] at hs,
 ...
 rw [unique_factorization_monoid.squarefree_iff_nodup_normalized_factors hs0]; rw [ h],
 apply s.nodup } },
 { intros x hx y hy h,
...
by rw [finset.sum_eq_multiset_sum]; rw [ divisors_filter_squarefree h0]; rw [ multiset.map_map]; rw [ finset.sum_eq_multiset_sum]
...
begin
 ...
 { rw [bit0_eq_two_mul (bit1 n)]; rw [ mul_dvd_mul_iff_left (two_ne_zero' ℕ)],
 exact nat.not_two_dvd_bit1 _ },
 { rw [bit0_eq_two_mul]; rw [ nat.mul_div_right _ (dec_trivial:0<2)],
...
begin
 ...
 { rw [bit1] at hp; rw [ bit0_eq_two_mul] at hp, change 2*(_+1) ≤ _ at hp,
...
begin
 ...
 rwa [← hc]; rwa [ ← nat.dvd_iff_mod_eq_zero],
end
...
λ k0 ih, begin
 ...
 { rw [← hn']; rw [ nat.mul_div_cancel _ k0'] },
 ...
 have dkk' : ¬ bit1 k ∣ bit1 n', {rw [nat.dvd_iff_mod_eq_zero]; rw [ hc], exact ne_of_gt c0},
 have dkk : ¬ bit1 k * bit1 k ∣ bit1 n, {rwa [← nat.dvd_div_iff dk]; rwa [ this]},
...
lemma not_squarefree_mul (a aa b n : ℕ) (ha : a * a = aa) (hb : aa * b = n)
 (h₁ : 1 < a) : ¬ squarefree n :=
by { rw [← hb]; rw [ ← ha], exact λ H, ne_of_gt h₁ (nat.is_unit_iff.1 $ H _ ⟨_, rfl⟩) }
