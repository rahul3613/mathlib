...
{ add := nat.add,
 ...
 nsmul_succ' := λ n x,
 by rw [nat.succ_eq_add_one]; rw [ nat.add_comm]; rw [ nat.right_distrib]; rw [ nat.one_mul] }
...
by rw [nat.succ_eq_add_one]; rw [ nat.add_comm]
...
⟨n - (m + 1), by rw [add_right_comm]; rw [ nat.add_sub_of_le h]⟩

/-! ### `pred` -/

@[simp]
lemma add_succ_sub_one (n m : ℕ) : (n + succ m) - 1 = n + m :=
by rw [add_succ]; rw [ succ_sub_one]

@[simp]
lemma succ_add_sub_one (n m : ℕ) : (succ n + m) - 1 = n + m :=
by rw [succ_add]; rw [ succ_sub_one]
...
by rw [← nat.sub_one]; rw [ nat.sub_sub]; rw [ one_add]; rw [ sub_succ]
...
by rw [add_comm]; rw [ add_one]; rw [ pred_succ]

/-! ### `mul` -/


theorem two_mul_ne_two_mul_add_one {n m} : 2 * n ≠ 2 * m + 1 :=
mt (congr_arg (%2)) (by { rw [add_comm]; rw [ add_mul_mod_self_left]; rw [ mul_mod_right]; rw [ mod_eq_of_lt]; simp })
...
by rw [mul_comm]; rw [ nat.mul_right_eq_self_iff hb]
...
by conv { to_lhs, rw [le_rec_on]; rw [ or.by_cases]; rw [ dif_pos h1] }

theorem le_rec_on_succ' {C : ℕ → Sort u} {n} {h : n ≤ n+1} {next} (x : C n) :
 (le_rec_on h next x : C (n+1)) = next x :=
by rw [le_rec_on_succ (le_refl n)]; rw [ le_rec_on_self]
...
begin
 ...
 rw [le_rec_on_succ (le_trans hnm hmk)]; rw [ ih]; rw [ le_rec_on_succ]
end
...
begin
 rw [subsingleton.elim h1 (le_trans (le_succ n) h2)]; rw [ le_rec_on_trans (le_succ n) h2]; rw [ le_rec_on_succ']
end
...
begin
 induction hnm with m hnm ih, { intros x y H, rwa [le_rec_on_self] at H ; rwa [ le_rec_on_self] at H },
 intros x y H, rw [le_rec_on_succ hnm] at H; rw [ le_rec_on_succ hnm] at H, exact ih (Hnext _ H)
end
...
by { induction nk with k nk ih, rw [decreasing_induction_self],
 rw [decreasing_induction_succ h (le_trans mn nk)]; rw [ ih]; rw [ decreasing_induction_succ] }
...
by { rw [subsingleton.elim mn (le_trans (le_succ m) smn)]; rw [ decreasing_induction_trans]; rw [ decreasing_induction_succ'] }
...
by rw [le_div_iff_mul_le hb]; rw [ one_mul]
...
protected theorem eq_mul_of_div_eq_right {a b c : ℕ} (H1 : b ∣ a) (H2 : a / b = c) :
 a = b * c :=
by rw [← H2]; rw [ nat.mul_div_cancel' H1]
...
protected theorem eq_mul_of_div_eq_left {a b c : ℕ} (H1 : b ∣ a) (H2 : a / b = c) :
 a = c * b :=
by rw [mul_comm]; rw [ nat.eq_mul_of_div_eq_right H1 H2]


protected theorem mul_div_cancel_left' {a b : ℕ} (Hd : a ∣ b) : a * (b / a) = b :=
by rw [mul_comm]; rw [nat.div_mul_cancel Hd]
...
by rw [mul_comm]; rw [ mul_comm b]; rw [ a.mul_div_mul_left b hc]
...
⟨λ ⟨e₁, e₂⟩, e₁ ▸ e₂ ▸ ⟨mod_add_div _ _, mod_lt _ h⟩,
 λ ⟨h₁, h₂⟩, h₁ ▸ by rw [add_mul_div_left _ _ h]; rw [ add_mul_mod_self_left];
...
exists_congr $ λ d, by rw [mul_assoc]; rw [ mul_right_inj' ha.ne']

protected theorem mul_dvd_mul_iff_right {a b c : ℕ} (hc : 0 < c) : a * c ∣ b * c ↔ a ∣ b :=
exists_congr $ λ d, by rw [mul_right_comm]; rw [ mul_left_inj' hc.ne']
...
begin
 conv_lhs
 { rw [←mod_add_div a n]; rw [ ←mod_add_div' b n]; rw [ right_distrib]; rw [ left_distrib]; rw [ left_distrib]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ ←left_distrib n _ _]; rw [ add_mul_mod_self_left]; rw [ ← mul_assoc]; rw [ add_mul_mod_self_right] }
end
...
by rw [←nat.div_mul_cancel w]; rw [ h]; rw [ one_mul]

lemma eq_zero_of_dvd_of_div_eq_zero {a b : ℕ} (w : a ∣ b) (h : b / a = 0) : b = 0 :=
by rw [←nat.div_mul_cancel w]; rw [ h]; rw [ zero_mul]
...
begin
 ...
 { rw [n0]; rw [ zero_mul], exact m.zero_le },
 { rw [mul_comm]; rw [ ← nat.le_div_iff_mul_le' h] },
end
...
by rw [nat.mul_add_mod]; rw [ nat.mod_eq_of_lt h]
