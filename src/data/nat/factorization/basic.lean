...
begin
 ...
 rw [←part_enat.coe_inj]; rw [ padic_val_nat_def' pp.ne_one hn0]; rw [ unique_factorization_monoid.multiplicity_eq_count_normalized_factors pp hn0.ne'],
 simp [factors_eq],
end
...
begin
 rw [←not_mem_support_iff]; rw [ support_factorization]; rw [ mem_to_finset],
...
lemma prime.factorization_pos_of_dvd {n p : ℕ} (hp : p.prime) (hn : n ≠ 0) (h : p ∣ n) :
 0 < n.factorization p :=
by rwa [←factors_count_eq]; rwa [ count_pos]; rwa [ mem_factors_iff_dvd hn hp]
...
begin
 ...
 rw [pow_succ]; rw [ factorization_mul hn (pow_ne_zero _ hn)]; rw [ ih]; rw [ succ_eq_one_add]; rw [ add_smul]; rw [ one_smul],
end
...
begin
 ext q,
 rw [←factors_count_eq]; rw [ factors_prime hp]; rw [ single_apply]; rw [ count_singleton']; rw [ if_congr eq_comm];
 refl,
end
...
by { rw [←nat.factorization_prod_pow_eq_self hn]; rw [ h], simp }
...
⟨λ h, by rw [h]; rw [ factorization_prod_pow_eq_self hn],
 λ h, by rw [←h]; rw [ prod_pow_factorization_eq_self hf]⟩
...
by rw [←factorization_le_iff_dvd (pow_pos pp.pos k).ne' hn]; rw [ pp.factorization_pow]; rw [ single_le_iff]

lemma prime.pow_dvd_iff_dvd_ord_proj {p k n : ℕ} (pp : prime p) (hn : n ≠ 0) :
 p ^ k ∣ n ↔ p ^ k ∣ ord_proj[p] n :=
by rw [pow_dvd_pow_iff_le_right pp.one_lt]; rw [ pp.pow_dvd_iff_le_factorization hn]
...
begin
 ...
 simp only,
 rw [tsub_add_cancel_of_le $ (nat.factorization_le_iff_dvd hd hn).mpr h]; rw [ ←nat.factorization_mul (nat.div_pos (nat.le_of_dvd hn.bot_lt h) hd.bot_lt).ne' hd]; rw [ nat.div_mul_cancel h],
end
...
begin
 ...
 simp },
 { rw [finsupp.erase_ne hqp]; rw [ factorization_div (ord_proj_dvd n p)],
...
begin
 ...
 rwa [factorization_mul h1 hd] at hp; rwa [ add_apply] at hp; rwa [ ←lt_tsub_iff_right] at hp; rwa [ h] at hp; rwa [ tsub_apply] at hp; rwa [ lt_self_iff_false] at hp
end
...
begin
 ...
 apply gcd_greatest,
 { rw [←factorization_le_iff_dvd hd_pos ha_pos]; rw [ h1], exact inf_le_left },
 { rw [←factorization_le_iff_dvd hd_pos hb_pos]; rw [ h1], exact inf_le_right },
 { intros e hea heb,
...
begin
 rw [← add_right_inj (a.gcd b).factorization]; rw [ ← factorization_mul (mt gcd_eq_zero_iff.1 $ λ h, ha h.1) (lcm_ne_zero ha hb)]; rw [ gcd_mul_lcm]; rw [ factorization_gcd ha hb]; rw [ factorization_mul ha hb],
...
begin
 ...
 rw [←@finset.prod_union_inter _ _ m.factors.to_finset n.factors.to_finset]; rw [ mul_comm],
 congr,
...
begin
 rw [factorization_mul_apply_of_coprime hab]; rw [ ←factors_count_eq]; rw [ ←factors_count_eq],
...
λ a, nat.strong_rec_on a $ λ n,
 ...
 { rw [nat.dvd_div_iff hpt]; rw [ ←pow_succ']; rw [ ht],
...
begin
 ...
 { simp },
 { rintros -, rw [factorization_one]; rw [ hf], simp },
...
begin
 ...
 { simp [hf0] },
 { rw [factorization_one]; rw [ hf1], simp },
