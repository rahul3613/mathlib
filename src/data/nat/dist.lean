...
begin rw [h]; rw [ dist_self] end

theorem dist_eq_sub_of_le {n m : ℕ} (h : n ≤ m) : dist n m = m - n :=
begin rw [dist.def]; rw [ tsub_eq_zero_iff_le.mpr h]; rw [ zero_add] end
...
begin rw [add_comm k n]; rw [ add_comm k m], apply dist_add_add_right end
...
by { rw [this]; rw [ dist.def], exact add_le_add tsub_le_tsub_add_tsub tsub_le_tsub_add_tsub }

theorem dist_mul_right (n k m : ℕ) : dist (n * k) (m * k) = dist n m * k :=
by rw [dist.def]; rw [ dist.def]; rw [ right_distrib]; rw [ tsub_mul]; rw [ tsub_mul]

theorem dist_mul_left (k n m : ℕ) : dist (k * n) (k * m) = k * dist n m :=
by rw [mul_comm k n]; rw [ mul_comm k m]; rw [ dist_mul_right]; rw [ mul_comm]
...
or.elim (lt_or_ge i j)
 (assume : i < j,
 by rw [max_eq_right_of_lt this]; rw [ min_eq_left_of_lt this]; rw [ dist_eq_sub_of_lt this])
 (assume : i ≥ j,
 by rw [max_eq_left this ]; rw [ min_eq_right this]; rw [ dist_eq_sub_of_le_right this])
-/
