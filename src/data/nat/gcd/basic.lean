...
⟨λ h, by rw [gcd_rec]; rw [ mod_eq_zero_of_dvd h]; rw [ gcd_zero_left],
...
gcd.induction n k
 ...
 (λk n H IH, by rwa [←mul_mod_mul_left] at IH); rwa [ ←gcd_rec] at IH); rwa [ ←gcd_rec] at IH)

theorem gcd_mul_right (m n k : ℕ) : gcd (m * n) (k * n) = gcd m k * n :=
by rw [mul_comm m n]; rw [ mul_comm k n]; rw [ mul_comm (gcd m k) n]; rw [ gcd_mul_left]
...
(decidable.eq_or_ne k 0).elim
 (λk0, by rw [k0]; rw [ nat.div_zero]; rw [ nat.div_zero]; rw [ nat.div_zero]; rw [ gcd_zero_right])
 (λH3, mul_right_cancel₀ H3 $ by rw [ nat.div_mul_cancel (dvd_gcd H1 H2)]; rw [ ←gcd_mul_right]; rw [ nat.div_mul_cancel H1]; rw [ nat.div_mul_cancel H2])
...
by rw [gcd_comm]; rw [ gcd_eq_left H]
...
by rw [gcd_comm]; rw [ gcd_mul_left_left]

@[simp] lemma gcd_mul_right_left (m n : ℕ) : gcd (n * m) n = n :=
by rw [mul_comm]; rw [ gcd_mul_left_left]

@[simp] lemma gcd_mul_right_right (m n : ℕ) : gcd n (n * m) = n :=
by rw [gcd_comm]; rw [ gcd_mul_right_left]
...
by rw [gcd_comm n m]; rw [ gcd_gcd_self_right_left]

@[simp] lemma gcd_gcd_self_left_right (m n : ℕ) : gcd (gcd n m) m = gcd n m :=
by rw [gcd_comm]; rw [ gcd_gcd_self_right_right]

@[simp] lemma gcd_gcd_self_left_left (m n : ℕ) : gcd (gcd m n) m = gcd m n :=
by rw [gcd_comm m n]; rw [ gcd_gcd_self_left_right]
...
by rw [gcd_comm]; rw [ gcd_add_mul_right_right]; rw [ gcd_comm]

@[simp] lemma gcd_add_mul_left_left (m n k : ℕ) : gcd (m + n * k) n = gcd m n :=
by rw [gcd_comm]; rw [ gcd_add_mul_left_right]; rw [ gcd_comm]

@[simp] lemma gcd_mul_right_add_left (m n k : ℕ) : gcd (k * n + m) n = gcd m n :=
by rw [gcd_comm]; rw [ gcd_mul_right_add_right]; rw [ gcd_comm]

@[simp] lemma gcd_mul_left_add_left (m n k : ℕ) : gcd (n * k + m) n = gcd m n :=
by rw [gcd_comm]; rw [ gcd_mul_left_add_right]; rw [ gcd_comm]
...
by rw [gcd_comm]; rw [ gcd_add_self_right]; rw [ gcd_comm]

@[simp] lemma gcd_self_add_left (m n : ℕ) : gcd (m + n) m = gcd n m :=
by rw [add_comm]; rw [ gcd_add_self_left]

@[simp] lemma gcd_self_add_right (m n : ℕ) : gcd m (m + n) = gcd m n :=
by rw [add_comm]; rw [ gcd_add_self_right]

/-! ### `lcm` -/

theorem lcm_comm (m n : ℕ) : lcm m n = lcm n m :=
by delta lcm; rw [mul_comm]; rw [ gcd_comm]

@[simp]
theorem lcm_zero_left (m : ℕ) : lcm 0 m = 0 :=
by delta lcm; rw [zero_mul]; rw [ nat.zero_div]
...
by delta lcm; rw [one_mul]; rw [ gcd_one_left]; rw [ nat.div_one]
...
or.elim (nat.eq_zero_or_pos m)
 (λh, by rw [h]; rw [ lcm_zero_left])
 (λh, by delta lcm; rw [gcd_self]; rw [ nat.mul_div_cancel _ h])
...
or.elim (nat.eq_zero_or_pos k)
 ...
 by rw [gcd_mul_lcm]; rw [ ←gcd_mul_right]; rw [ mul_comm n k];
...
by rw [←one_mul (lcm m n)]; rw [ ←h.gcd_eq_one]; rw [ gcd_mul_lcm]
...
by rwa [gcd_mul_left] at t; rwa [ H1.gcd_eq_one] at t; rwa [ mul_one] at t
...
by rw [coprime]; rw [ gcd_assoc]; rw [ H.symm.gcd_eq_one]; rw [ gcd_one_right],
dvd_antisymm
...
theorem coprime.gcd_mul_right_cancel (m : ℕ) {k n : ℕ} (H : coprime k n) :
 gcd (m * k) n = gcd m n :=
by rw [mul_comm m k]; rw [ H.gcd_mul_left_cancel m]

theorem coprime.gcd_mul_left_cancel_right {k m : ℕ} (n : ℕ) (H : coprime k m) :
 gcd m (k * n) = gcd m n :=
by rw [gcd_comm m n]; rw [ gcd_comm m (k * n)]; rw [ H.gcd_mul_left_cancel n]

theorem coprime.gcd_mul_right_cancel_right {k m : ℕ} (n : ℕ) (H : coprime k m) :
 gcd m (n * k) = gcd m n :=
by rw [mul_comm n k]; rw [ H.gcd_mul_left_cancel_right n]

theorem coprime_div_gcd_div_gcd {m n : ℕ} (H : 0 < gcd m n) :
 coprime (m / gcd m n) (n / gcd m n) :=
by rw [coprime_iff_gcd_eq_one]; rw [ gcd_div (gcd_dvd_left m n) (gcd_dvd_right m n)]; rw [ nat.div_self H]
...
⟨λ h, ⟨coprime.coprime_mul_right h, coprime.coprime_mul_left h⟩,
 λ ⟨h, _⟩, by rwa [coprime_iff_gcd_eq_one]; rwa [ coprime.gcd_mul_left_cancel n h]⟩
...
by rw [nat.coprime_comm]; rw [ coprime_pow_left_iff hn]; rw [ nat.coprime_comm]

theorem coprime.eq_one_of_dvd {k m : ℕ} (H : coprime k m) (d : k ∣ m) : k = 1 :=
by rw [← H.gcd_eq_one]; rw [ gcd_eq_left d]
...
begin
 ...
 { rw [gcd_comm a _]; rw [ gcd_comm b _],
 transitivity c.gcd (a * b),
 rw [h]; rw [ gcd_mul_right_right d c],
