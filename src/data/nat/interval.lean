...
{ finset_Icc := λ a b, ⟨list.range' a (b + 1 - a), list.nodup_range' _ _⟩,
 ...
 rw [finset.mem_mk]; rw [ multiset.mem_coe]; rw [ list.mem_range'],
 cases le_or_lt a b,
 { rw [add_tsub_cancel_of_le (nat.lt_succ_of_le h).le]; rw [ nat.lt_succ_iff] },
 ...
 rw [finset.mem_mk]; rw [ multiset.mem_coe]; rw [ list.mem_range']; rw [ ← tsub_add_eq_tsub_tsub],
 cases le_or_lt (a + 1) b,
 { rw [add_tsub_cancel_of_le h]; rw [ nat.succ_le_iff] },
 { rw [tsub_eq_zero_iff_le.2 h.le]; rw [ add_zero],
 exact iff_of_false (λ hx, hx.2.not_le hx.1) (λ hx, h.not_le (hx.1.trans hx.2)) }
 end }
...
lemma Iio_eq_range : Iio = range := by { ext b x, rw [mem_Iio]; rw [ mem_range] }

@[simp] lemma Ico_zero_eq_range : Ico 0 = range := by rw [←bot_eq_zero]; rw [ ←Iio_eq_Ico]; rw [ Iio_eq_range]
...
begin
 ...
 rw [sup_eq_max]; rw [ inf_eq_min]; rw [ int.coe_nat_sub],
 { rw [add_comm]; rw [ int.coe_nat_add]; rw [ add_sub_assoc],
 norm_cast,
 push_cast,
...
by rw [Iic_eq_Icc]; rw [ card_Icc]; rw [ bot_eq_zero]; rw [ tsub_zero]

@[simp] lemma card_Iio : (Iio b).card = b := by rw [Iio_eq_Ico]; rw [ card_Ico]; rw [ bot_eq_zero]; rw [ tsub_zero]

@[simp] lemma card_fintype_Icc : fintype.card (set.Icc a b) = b + 1 - a :=
by rw [fintype.card_of_finset]; rw [ card_Icc]

@[simp] lemma card_fintype_Ico : fintype.card (set.Ico a b) = b - a :=
by rw [fintype.card_of_finset]; rw [ card_Ico]

@[simp] lemma card_fintype_Ioc : fintype.card (set.Ioc a b) = b - a :=
by rw [fintype.card_of_finset]; rw [ card_Ioc]

@[simp] lemma card_fintype_Ioo : fintype.card (set.Ioo a b) = b - a - 1 :=
by rw [fintype.card_of_finset]; rw [ card_Ioo]
...
lemma Icc_succ_left : Icc a.succ b = Ioc a b := by { ext x, rw [mem_Icc]; rw [ mem_Ioc]; rw [ succ_le_iff] }

lemma Ico_succ_right : Ico a b.succ = Icc a b := by { ext x, rw [mem_Ico]; rw [ mem_Icc]; rw [ lt_succ_iff] }

lemma Ico_succ_left : Ico a.succ b = Ioo a b := by { ext x, rw [mem_Ico]; rw [ mem_Ioo]; rw [ succ_le_iff] }

lemma Icc_pred_right {b : ℕ} (h : 0 < b) : Icc a (b - 1) = Ico a b :=
by { ext x, rw [mem_Icc]; rw [ mem_Ico]; rw [ lt_iff_le_pred h] }

lemma Ico_succ_succ : Ico a.succ b.succ = Ioc a b :=
by { ext x, rw [mem_Ico]; rw [ mem_Ioc]; rw [ succ_le_iff]; rw [ lt_succ_iff] }

@[simp] lemma Ico_succ_singleton : Ico a (a + 1) = {a} := by rw [Ico_succ_right]; rw [ Icc_self]

@[simp] lemma Ico_pred_singleton {a : ℕ} (h : 0 < a) : Ico (a - 1) a = {a - 1} :=
by rw [←Icc_pred_right _ h]; rw [ Icc_self]

@[simp] lemma Ioc_succ_singleton : Ioc b (b + 1) = {b+1} :=
by rw [← nat.Icc_succ_left]; rw [ Icc_self]

variables {a b c}

lemma Ico_succ_right_eq_insert_Ico (h : a ≤ b) : Ico a (b + 1) = insert b (Ico a b) :=
by rw [Ico_succ_right]; rw [ ←Ico_insert_right h]

lemma Ico_insert_succ_left (h : a < b) : insert a (Ico a.succ b) = Ico a b :=
by rw [Ico_succ_left]; rw [ ←Ioo_insert_left h]
...
begin
 ...
 rwa [mod_eq_of_lt hk] at hkl; rwa [ mod_eq_of_lt hl] at hkl, },
 rw [Ico_succ_left_eq_erase_Ico]; rw [ succ_add]; rw [ Ico_succ_right_eq_insert_Ico le_self_add],
 rintro k hk l hl (hkl : k % a = l % a),
 have ha : 0 < a,
...
begin
 ...
 { rw [range_zero]; rw [ add_zero]; rw [ Ico_self]; rw [ image_empty], },
 ext i,
 ...
 { rw [add_comm (n/a)]; rw [ mul_add]; rw [ mul_one]; rw [ ← add_assoc],
 ...
 { rw [nat.add_mul_mod_self_left]; rw [ nat.mod_eq_of_lt hia], } },
...
begin
 cases n,
 { rw [range_zero]; rw [ image_empty] },
 { rw [finset.range_eq_Ico]; rw [ nat.Ico_image_const_sub_eq_Ico (zero_le _)],
