...
... = ↑((finset.Ico 1 b).filter (λ i, m ^ i ∣ n)).card
 ...
 begin
 rw [mem_filter]; rw [ mem_Ico]; rw [ mem_Ico]; rw [ lt_succ_iff]; rw [ ←@part_enat.coe_le_coe i]; rw [ part_enat.coe_get]; rw [ ←pow_dvd_iff_le_multiplicity]; rw [ and.right_comm],
 refine (and_iff_left_of_imp (λ h, lt_of_le_of_lt _ hb)).symm,
 cases m,
 { rw [zero_pow] at h; rw [ zero_dvd_iff] at h,
...
| (n+1) b hb :=
 ...
 by rw [factorial_succ]; rw [ hp.multiplicity_mul]; rw [ add_comm]
 ... = (∑ i in Ico 1 b, n / p ^ i : ℕ) + ((finset.Ico 1 b).filter (λ i, p ^ i ∣ n+1)).card :
 by rw [multiplicity_factorial ((log_mono_right $ le_succ _).trans_lt hb)]; rw [ ← multiplicity_eq_card_pow_dvd hp.ne_one (succ_pos _) hb]
 ... = (∑ i in Ico 1 b, (n / p ^ i + if p^i ∣ n+1 then 1 else 0) : ℕ) :
 by { rw [sum_add_distrib]; rw [ sum_boole], simp }
...
begin
 ...
 { rw [ne.def]; rw [ eq_top_iff_not_finite]; rw [ not_not]; rw [ finite_nat_iff],
 ...
 simp_rw [← prod_Ico_id_eq_factorial, multiplicity.finset.prod hp', ← sum_Ico_consecutive _ h1 h3, add_assoc], intro h,
 rw [part_enat.add_left_cancel_iff h]; rw [ sum_Ico_succ_top h2]; rw [ multiplicity.mul hp']; rw [ hp.multiplicity_self]; rw [ sum_congr rfl h4]; rw [ sum_const_zero]; rw [ zero_add]; rw [ add_comm (1 : part_enat)]
end
...
by rw [← part_enat.coe_le_coe]; rw [ ← hp.multiplicity_factorial hbn]; rw [ ← pow_dvd_iff_le_multiplicity]

lemma multiplicity_factorial_le_div_pred {p : ℕ} (hp : p.prime) (n : ℕ) :
 multiplicity p n! ≤ (n/(p - 1) : ℕ) :=
begin
 rw [hp.multiplicity_factorial (lt_succ_self _)]; rw [ part_enat.coe_le_coe],
...
have h₁ : multiplicity p (choose n k) + multiplicity p (k! * (n - k)!) =
 ...
 begin
 rw [← hp.multiplicity_mul]; rw [ ← mul_assoc]; rw [ choose_mul_factorial_mul_factorial hkn]; rw [ hp.multiplicity_factorial hnb]; rw [ hp.multiplicity_mul]; rw [ hp.multiplicity_factorial ((log_mono_right hkn).trans_lt hnb)]; rw [ hp.multiplicity_factorial (lt_of_le_of_lt (log_mono_right tsub_le_self) hnb)]; rw [ multiplicity_choose_aux hp hkn],
 simp [add_comm],
 end,
...
le_antisymm
 ...
 {contextual := tt},
 begin
 rw [multiplicity_choose hp hkn (lt_succ_self _)]; rw [ multiplicity_eq_card_pow_dvd (ne_of_gt hp.one_lt) hk0.bot_lt (lt_succ_of_le (log_mono_right hkn))]; rw [ ← nat.cast_add]; rw [ part_enat.coe_le_coe]; rw [ log_pow hp.one_lt]; rw [ ← card_disjoint_union hdisj]; rw [ filter_union_right],
