...
by rw [count]; rw [ list.range_zero]; rw [ list.countp]
...
by { rw [count]; rw [ list.countp_eq_length_filter], refl, }

/-- `count p n` can be expressed as the cardinality of `{k // k < n ∧ p k}`. -/
lemma count_eq_card_fintype (n : ℕ) : count p n = fintype.card {k : ℕ // k < n ∧ p k} :=
by { rw [count_eq_card_filter_range]; rw [ ←fintype.card_of_finset]; rw [ ←count_set.fintype], refl, }
...
begin
 ...
 simp_rw [count_eq_card_filter_range, range_add, filter_union, card_disjoint_union this, filter_map, add_left_embedding, card_map], refl,
end
