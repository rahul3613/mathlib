...
| n@(k+2) := λ h,
 ...
 by rw [factors]; rw [ list.prod_cons]; rw [ prod_factors h₁]; rw [ nat.mul_div_cancel' (min_fac_dvd _)]
...
begin
 ...
 rw [← prod_factors hpos]; rw [ ← prod_replicate k p]; rw [ eq_replicate_of_mem (λ d hd, h (prime_of_mem_factors hd) (dvd_of_mem_factors hd))],
end
...
begin
 ...
 { rw [list.prod_append]; rw [ prod_factors ha]; rw [ prod_factors hb] },
 { intros p hp,
...
begin
 rw [mem_factors (mul_ne_zero ha hb)]; rw [ mem_factors ha]; rw [ mem_factors hb]; rw [ ←and_or_distrib_left],
...
begin
 ...
 rcases b.eq_zero_or_pos with rfl | hb,
 { simp [(coprime_zero_right _).mp hab] },
 rw [mem_factors_mul ha.ne' hb.ne']; rw [ list.mem_union]
end
