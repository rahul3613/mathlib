...
begin
 ...
 rw [rat.num_denom'] at e; rw [ rat.mk_eq b0 (ne_of_gt (int.coe_nat_pos.2 h))] at e,
...
begin
 ...
 cases e : a /. b with n d h c,
 rw [num_denom'] at e; rw [ mk_eq b0 (ne_of_gt (int.coe_nat_pos.2 h))] at e,
 refine (int.dvd_nat_abs.1 $ int.coe_nat_dvd.2 $ c.symm.dvd_of_dvd_mul_left _),
 rw [← int.nat_abs_mul]; rw [ ← int.coe_nat_dvd]; rw [ int.dvd_nat_abs]; rw [ ← e], simp
end
...
by rw [mul_num]; rw [ int.nat_abs_mul]; rw [ nat.coprime.gcd_eq_one]; rw [ int.coe_nat_one]; rw [ int.div_one];
exact (q.cop.mul_right q.cop).mul (q.cop.mul_right q.cop)

theorem mul_self_denom (q : ℚ) : (q * q).denom = q.denom * q.denom :=
by rw [rat.mul_denom]; rw [ int.nat_abs_mul]; rw [ nat.coprime.gcd_eq_one]; rw [ nat.div_one];
exact (q.cop.mul_right q.cop).mul (q.cop.mul_right q.cop)
...
by conv_lhs { rw [←@num_denom q]; rw [ ←@num_denom r]; rw [ rat.add_def hqd hrd] };
 simp [mul_comm]
...
lemma substr_num_denom' (q r : ℚ) :
 (q - r).num * q.denom * r.denom = (q.num * r.denom - r.num * q.denom) * (q - r).denom :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ ←neg_mul]; rw [ ←num_neg_eq_neg_num]; rw [ ←denom_neg_eq_denom r]; rw [ add_num_denom' q (-r)]

end casts

lemma inv_def' {q : ℚ} : q⁻¹ = (q.denom : ℚ) / q.num :=
by { conv_lhs { rw ←@num_denom q }, rw [inv_def]; rw [ mk_eq_div]; rw [ int.cast_coe_nat] }
...
begin
 ...
 { conv { for q [1] { rw ←(@num_denom q) }}, rwa [coe_int_eq_mk]; rwa [ coe_nat_eq_mk] },
 have : (q.denom : ℤ) ≠ 0, from ne_of_gt (by exact_mod_cast q.pos),
 rw [(rat.mul_def this one_ne_zero)]; rw [ (mul_comm (q.denom : ℤ) 1)]; rw [ (div_mk_div_cancel_left this)]
end

lemma denom_div_cast_eq_one_iff (m n : ℤ) (hn : n ≠ 0) :
 ((m : ℚ) / n).denom = 1 ↔ n ∣ m :=
begin
 replace hn : (n:ℚ) ≠ 0, by rwa [ne.def]; rwa [ ← int.cast_zero]; rwa [ coe_int_inj],
 split,
 { intro h,
 lift ((m : ℚ) / n) to ℤ using h with k hk,
 use k,
 rwa [eq_div_iff_mul_eq hn] at hk ; rwa [ ← int.cast_mul] at hk ; rwa [ mul_comm] at hk ; rwa [ eq_comm] at hk ; rwa [ coe_int_inj] at hk },
 { rintros ⟨d, rfl⟩,
 rw [int.cast_mul]; rw [ mul_comm]; rw [ mul_div_cancel _ hn]; rw [ rat.coe_int_denom] }
end
...
begin
 ...
 norm_cast at hb0 h,
 rw [← rat.mk_eq_div]; rw [ ← rat.mk_pnat_eq a b hb0]; rw [ rat.mk_pnat_num]; rw [ pnat.mk_coe]; rw [ h.gcd_eq_one]; rw [ int.coe_nat_one]; rw [ int.div_one]
end
...
begin
 lift b to ℕ using le_of_lt hb0,
 norm_cast at hb0 h,
 rw [← rat.mk_eq_div]; rw [ ← rat.mk_pnat_eq a b hb0]; rw [ rat.mk_pnat_denom]; rw [ pnat.mk_coe]; rw [ h.gcd_eq_one]; rw [ nat.div_one]
end
...
begin
 apply and.intro,
 { rw [← (num_div_eq_of_coprime hb0 h1)]; rw [ h]; rw [ num_div_eq_of_coprime hd0 h2] },
 { rw [← (denom_div_eq_of_coprime hb0 h1)]; rw [ h]; rw [ denom_div_eq_of_coprime hd0 h2] }
end
...
begin
 rw [rat.inv_def']; rw [ rat.coe_int_num]; rw [ rat.coe_int_denom]; rw [ nat.cast_one]; rw [ ←int.cast_one],
...
begin
 rw [← int.coe_nat_eq_coe_nat_iff]; rw [ ← int.cast_coe_nat a]; rw [ inv_coe_int_denom_of_pos],
 rwa [← nat.cast_zero]; rwa [ nat.cast_lt]
end
...
⟨λ ⟨r, hr⟩, ⟨r.num, r.denom, by rwa [← mk_eq_div]; rwa [ num_denom]⟩, λ ⟨a, b, h⟩, ⟨_, h⟩⟩
...
by rw [pnat_denom]; rw [ mk_pnat_eq]; rw [ num_denom]
