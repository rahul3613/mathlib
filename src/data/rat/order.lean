...
by { have := eq_neg_of_add_eq_zero_left (rat.nonneg_antisymm hba $
 by rwa [← sub_eq_add_neg]; rwa [ neg_sub]), rwa neg_neg at this }
...
begin
 ...
 { rw [abs_of_nonpos hq],
 rw [←(@num_denom q)] at hq; rw [ ← mk_zero_one] at hq; rw [ rat.le_def (int.coe_nat_pos.2 q.pos) zero_lt_one] at hq; rw [ mul_one] at hq; rw [ zero_mul] at hq,
 rw [int.of_nat_nat_abs_of_nonpos hq]; rw [ ← neg_def]; rw [ num_denom] },
 { rw [abs_of_nonneg hq],
 rw [←(@num_denom q)] at hq; rw [ ← mk_zero_one] at hq; rw [ rat.le_def zero_lt_one (int.coe_nat_pos.2 q.pos)] at hq; rw [ mul_one] at hq; rw [ zero_mul] at hq,
 rw [int.nat_abs_of_nonneg hq]; rw [ num_denom] }
end
