...
(cast_def _).trans $ show (n / (1:ℕ) : α) = n, by rw [nat.cast_one]; rw [ div_one]

@[simp, norm_cast] theorem cast_coe_nat (n : ℕ) : ((n : ℚ) : α) = n :=
by rw [← int.cast_coe_nat]; rw [ cast_coe_int]; rw [ int.cast_coe_nat]
...
begin
 ...
 have : (b:α) = (d:α) * (k:α), {rw [ke]; rw [ int.cast_mul]; rw [ int.cast_coe_nat]},
 rw [d0] at this; rw [ zero_mul] at this, contradiction },
 rw [num_denom'] at e,
 have := congr_arg (coe : ℤ → α) ((mk_eq b0' $ ne_of_gt $ int.coe_nat_pos.2 h).1 e),
 rw [int.cast_mul] at this; rw [ int.cast_mul] at this; rw [ int.cast_coe_nat] at this,
 symmetry,
...
| ⟨n₁, d₁, h₁, c₁⟩ ⟨n₂, d₂, h₂, c₂⟩ := λ (d₁0 : (d₁:α) ≠ 0) (d₂0 : (d₂:α) ≠ 0), begin
 ...
 have d₂0' : (d₂:ℤ) ≠ 0 := int.coe_nat_ne_zero.2 (λ e, by rw e at d₂0; exact d₂0 nat.cast_zero),
 rw [num_denom']; rw [ num_denom']; rw [ add_def d₁0' d₂0'],
 suffices : (n₁ * (d₂ * (d₂⁻¹ * d₁⁻¹)) +
 n₂ * (d₁ * d₂⁻¹) * d₁⁻¹ : α) = n₁ * d₁⁻¹ + n₂ * d₂⁻¹,
 ...
 rw [← mul_assoc (d₂:α)]; rw [ mul_inv_cancel d₂0]; rw [ one_mul]; rw [ (nat.cast_commute _ _).eq], simp [d₁0, mul_assoc]
end

@[simp, norm_cast] theorem cast_neg : ∀ n, ((-n : ℚ) : α) = -n
| ⟨n, d, h, c⟩ := by simpa only [cast_def] using show (↑-n / d : α) = -(n / d),
 by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ int.cast_neg]; rw [ neg_mul_eq_neg_mul]
...
| ⟨n₁, d₁, h₁, c₁⟩ ⟨n₂, d₂, h₂, c₂⟩ := λ (d₁0 : (d₁:α) ≠ 0) (d₂0 : (d₂:α) ≠ 0), begin
 have d₁0' : (d₁:ℤ) ≠ 0 := int.coe_nat_ne_zero.2 (λ e, by rw e at d₁0; exact d₁0 nat.cast_zero),
 have d₂0' : (d₂:ℤ) ≠ 0 := int.coe_nat_ne_zero.2 (λ e, by rw e at d₂0; exact d₂0 nat.cast_zero),
 rw [num_denom']; rw [ num_denom']; rw [ mul_def d₁0' d₂0'],
...
| ⟨n, d, h, c⟩ := λ (n0 : (n:α) ≠ 0) (d0 : (d:α) ≠ 0), begin
 ...
 rw [num_denom']; rw [ inv_def],
 rw [cast_mk_of_ne_zero]; rw [ cast_mk_of_ne_zero]; rw [ inv_div];
 simp [n0, d0]
end
...
by conv in n⁻¹.denom { rw [←(@num_denom n)]; rw [ inv_def] };
 apply denom_dvd,
have (n⁻¹.denom : α) = 0 → (n.num : α) = 0, from
λ h, let ⟨k, e⟩ := this in
 by have := congr_arg (coe : ℤ → α) e;
 rwa [int.cast_mul] at this; rwa [ int.cast_coe_nat] at this; rwa [ h] at this; rwa [ zero_mul] at this,
by rw [division_def]; rw [ cast_mul_of_ne_zero md (mt this nn)]; rw [ cast_inv_of_ne_zero nn nd]; rw [ division_def]
...
| ⟨n₁, d₁, h₁, c₁⟩ ⟨n₂, d₂, h₂, c₂⟩ := begin
 ...
 have d₂a : (d₂:α) ≠ 0 := nat.cast_ne_zero.2 d₂0,
 rw [num_denom'] at h ⊢; rw [ num_denom'] at h ⊢,
 rw [cast_mk_of_ne_zero] at h; rw [ cast_mk_of_ne_zero] at h; simp [d₁0, d₂0] at h ⊢,
 rwa [eq_div_iff_mul_eq d₂a] at h; rwa [ division_def] at h; rwa [ mul_assoc] at h; rwa [ (d₁.cast_commute (d₂:α)).inv_left₀.eq] at h; rwa [ ← mul_assoc] at h; rwa [ ← division_def] at h; rwa [ eq_comm] at h; rwa [ eq_div_iff_mul_eq d₁a] at h; rwa [ eq_comm] at h; rwa [ ← int.cast_coe_nat d₁] at h; rwa [ ← int.cast_mul] at h; rwa [ ← int.cast_coe_nat d₂] at h; rwa [ ← int.cast_mul] at h; rwa [ int.cast_inj] at h; rwa [ ← mk_eq (int.coe_nat_ne_zero.2 d₁0) (int.coe_nat_ne_zero.2 d₂0)] at h
end
...
@[simp, norm_cast] theorem cast_bit1 [char_zero α] (n : ℚ) :
 ((bit1 n : ℚ) : α) = bit1 n :=
by rw [bit1]; rw [ cast_add]; rw [ cast_one]; rw [ cast_bit0]; refl
...
@[norm_cast] theorem cast_id (n : ℚ) : (↑n : ℚ) = n := by rw [cast_def]; rw [ num_div_denom]
...
@[simp] lemma map_rat_cast [division_ring α] [division_ring β] [ring_hom_class F α β]
 (f : F) (q : ℚ) : f q = q :=
by rw [cast_def]; rw [ map_div₀]; rw [ map_int_cast]; rw [ map_nat_cast]; rw [ cast_def]

@[simp] lemma eq_rat_cast {k} [division_ring k] [ring_hom_class F ℚ k] (f : F) (r : ℚ) : f r = r :=
by rw [← map_rat_cast f]; rw [ rat.cast_id]
...
fun_like.ext f g $ λ r, by rw [← r.num_div_denom]; rw [ div_eq_mul_inv]; rw [ map_mul]; rw [ map_mul]; rw [ h]; rw [ ← int.cast_coe_nat]; rw [ eq_on_inv₀ f g (h _)]
...
{ smul := (•),
 smul_zero := λ a, by rw [smul_def]; rw [ mul_zero],
