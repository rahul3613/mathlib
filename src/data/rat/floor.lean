...
| ⟨n, d, h, c⟩ := begin
 ...
 conv { to_rhs,
 rw [coe_int_eq_mk]; rw [ rat.le_def zero_lt_one h']; rw [ mul_one] },
...
begin
 ...
 rwa [←d_eq_c_mul_denom]; rwa [ int.coe_nat_pos],
end
...
by rw [←neg_inj]; rw [ ←floor_neg]; rw [ ←floor_neg]; rw [ ← rat.cast_neg]; rw [ rat.floor_cast]

@[simp, norm_cast] lemma round_cast (x : ℚ) : round (x : α) = round x :=
have ((x + 1 / 2 : ℚ) : α) = x + 1 / 2, by simp,
by rw [round_eq]; rw [ round_eq]; rw [ ← this]; rw [ floor_cast]
...
by rw [(eq_sub_of_add_eq $ int.mod_add_div n d)]; rw [ rat.floor_int_div_nat_eq_div]
...
begin
 ...
 rwa [nat.coprime] at this; rwa [ nat.gcd_rec] at this
end
...
begin
 ...
 have : q⁻¹.num < ⌊q⁻¹⌋ * q⁻¹.denom + q⁻¹.denom, by rwa [right_distrib] at this; rwa [ one_mul] at this,
 ...
 rwa [q_inv_eq] at q_inv_num_denom_ineq; rwa [ this.left] at q_inv_num_denom_ineq; rwa [ this.right] at q_inv_num_denom_ineq; rwa [ q_num_abs_eq_q_num] at q_inv_num_denom_ineq; rwa [ mul_comm] at q_inv_num_denom_ineq
end
