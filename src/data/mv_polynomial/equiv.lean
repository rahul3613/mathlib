...
{ to_fun := eval₂ polynomial.C (λu:punit, polynomial.X),
 ...
 show ∀ p, f.comp g p = p,
 apply is_id,
 { ext a, dsimp, rw [eval₂_C]; rw [ polynomial.eval₂_C] },
 { rintros ⟨⟩, dsimp, rw [eval₂_X]; rw [ polynomial.eval₂_X] }
 end,
 right_inv := assume p, polynomial.induction_on p
 (assume a, by rw [polynomial.eval₂_C]; rw [ mv_polynomial.eval₂_C])
 (assume p q hp hq, by rw [polynomial.eval₂_add]; rw [ mv_polynomial.eval₂_add]; rw [ hp]; rw [ hq])
 (assume p n hp,
 by rw [polynomial.eval₂_mul]; rw [ polynomial.eval₂_pow]; rw [ polynomial.eval₂_X]; rw [ polynomial.eval₂_C]; rw [ eval₂_mul]; rw [ eval₂_C]; rw [ eval₂_pow]; rw [ eval₂_X]),
...
begin
 ...
 { ext1 a, dsimp, rw [iter_to_sum_C_C R S₁ S₂]; rw [ sum_to_iter_C R S₁ S₂] },
 { assume c, dsimp, rw [iter_to_sum_C_X R S₁ S₂]; rw [ sum_to_iter_Xr R S₁ S₂] } },
 { assume b, rw [iter_to_sum_X R S₁ S₂]; rw [ sum_to_iter_Xl R S₁ S₂] },
 { ext1 a, rw [ring_hom.comp_apply]; rw [ ring_hom.comp_apply]; rw [ sum_to_iter_C R S₁ S₂]; rw [ iter_to_sum_C_C R S₁ S₂] },
 { assume n, cases n with b c,
 { rw [sum_to_iter_Xl]; rw [ iter_to_sum_X] },
 { rw [sum_to_iter_Xr]; rw [ iter_to_sum_C_X] } },
end
...
begin
 ...
 { rintro ⟨m, h, rfl⟩,
 refine ⟨tail m, _⟩,
 rwa [← coeff]; rwa [ ← mem_support_iff]; rwa [ support_coeff_fin_succ_equiv]; rwa [ cons_tail] },
end
...
begin
 ...
 { congr,
 funext,
 rw [mem_support_iff]; rw [ fin_succ_equiv_coeff_coeff]; rw [ ne.def] },
 split,
 { rintros ⟨m',⟨h, hm'⟩⟩,
 simp only [←hm'],
...
begin
 ...
 { rw [degree_of_eq_sup]; rw [ fin_succ_equiv_support f]; rw [ finset.sup_image] },
 rw [polynomial.degree]; rw [ ← h']; rw [ finset.coe_sup_of_nonempty (support_fin_succ_equiv_nonempty h)]; rw [ finset.max_eq_sup_coe],
end
...
begin
 by_cases c : f = 0,
 { rw [c]; rw [ (fin_succ_equiv R n).map_zero]; rw [ polynomial.nat_degree_zero]; rw [ degree_of_zero] },
 { rw [polynomial.nat_degree]; rw [ degree_fin_succ_equiv (by simpa only [ne.def]) ],
 simp },
end
...
begin
 rw [degree_of_eq_sup]; rw [ degree_of_eq_sup]; rw [ finset.sup_le_iff],
 intros m hm,
