...
by rw [← monomial_sum_one]; rw [ C_mul']; rw [ ← (monomial _).map_smul]; rw [ smul_eq_mul]; rw [ mul_one]
...
by classical; rw [X]; rw [ support_monomial]; rw [ if_neg]; exact one_ne_zero

lemma support_X_pow [nontrivial R] (s : σ) (n : ℕ) :
 (X s ^ n : mv_polynomial σ R).support = {finsupp.single s n} :=
by classical; rw [X_pow_eq_monomial]; rw [ support_monomial]; rw [ if_neg (one_ne_zero' R)]
...
begin
 ...
 rwa [@monomial_eq _ _ (1:R) (finsupp.single i k) _] at this; rwa [ C_1] at this; rwa [ one_mul] at this; rwa [ finsupp.prod_single_index] at this,
 exact pow_zero _
end

lemma coeff_X' [decidable_eq σ] (i : σ) (m) :
 coeff m (X i : mv_polynomial σ R) = if single i 1 = m then 1 else 0 :=
by rw [← coeff_X_pow]; rw [ pow_one]

@[simp] lemma coeff_X (i : σ) :
 coeff (single i 1) (X i : mv_polynomial σ R) = 1 :=
by classical; rw [coeff_X']; rw [ if_pos rfl]
...
lemma support_symm_diff_support_subset_support_add [decidable_eq σ] (p q : mv_polynomial σ R) :
 p.support ∆ q.support ⊆ (p + q).support :=
begin
 rw [symm_diff_def]; rw [ finset.sup_eq_union],
...
begin
 ...
 rw [support_monomial] at hi'; rw [ if_neg hr] at hi'; rw [ finset.mem_singleton] at hi', subst i',
...
lemma ne_zero_iff {p : mv_polynomial σ R} :
 p ≠ 0 ↔ ∃ d, coeff d p ≠ 0 :=
by { rw [ne.def]; rw [ eq_zero_iff], push_neg, }
...
by rw [constant_coeff_eq]; rw [ coeff_monomial]
...
by rw [C_apply]; rw [ eval₂_monomial]; rw [ prod_zero_index]; rw [ mul_one]
...
begin
 ...
 by rw [monomial_single_add]; rw [ pow_one]; rw [ mul_assoc]
...
| 0 := by { rw [pow_zero]; rw [ pow_zero], exact eval₂_one _ _ }
| (n + 1) := by rw [pow_add]; rw [ pow_one]; rw [ pow_add]; rw [ pow_one]; rw [ eval₂_mul]; rw [ eval₂_pow]
...
begin
 ...
 { intro r, rw [ring_hom.comp_apply]; rw [ eval₂_hom_C]; rw [ eval₂_hom_C]; rw [ ring_hom.comp_apply] },
 { intro i, rw [ring_hom.comp_apply]; rw [ eval₂_hom_X']; rw [ eval₂_hom_X'] }
end
...
by rw [smul_eq_C_mul]; rw [ (eval x).map_mul]; rw [ eval_C]
...
begin
 apply mv_polynomial.induction_on p,
 { intro r, rw [eval₂_C]; rw [ map_C]; rw [ eval₂_C] },
 { intros p q hp hq, rw [eval₂_add]; rw [ k.map_add]; rw [ (map f).map_add]; rw [ eval₂_add]; rw [ hp]; rw [ hq] },
 { intros p s hp,
...
begin
 apply mv_polynomial.induction_on p,
 { intro r, rw [eval₂_C]; rw [ map_C]; rw [ map_C]; rw [ eval₂_C] },
 { intros p q hp hq, rw [eval₂_add]; rw [ (map f).map_add]; rw [ hp]; rw [ hq]; rw [ (map f).map_add]; rw [ eval₂_add] },
 { intros p s hp,
...
λ x, by rw [map_map]; rw [ (ring_hom.ext hf : f.comp g = ring_hom.id _)]; rw [ map_id]
...
by { rw [← eval_map]; rw [ ← eval_map]; rw [ map_map], }
...
lemma C_dvd_iff_map_hom_eq_zero
 ...
 C r ∣ φ ↔ map q φ = 0 :=
begin
 rw [C_dvd_iff_dvd_coeff]; rw [ mv_polynomial.ext_iff],
...
{ to_fun := map ↑f,
 ...
 have h₂ : algebra_map R (mv_polynomial σ S₂) r = C (algebra_map R S₂ r) := rfl,
 rw [h₁]; rw [ h₂]; rw [ map]; rw [ eval₂_hom_C]; rw [ ring_hom.comp_apply]; rw [ alg_hom.coe_to_ring_hom]; rw [ alg_hom.commutes],
 end,
 ..map ↑f }
...
lemma eval₂_hom_eq_zero (f : R →+* S₂) (g : σ → S₂) (φ : mv_polynomial σ R)
 ...
 eval₂_hom f g φ = 0 :=
begin
 rw [φ.as_sum]; rw [ ring_hom.map_sum]; rw [ finset.sum_eq_zero],
 intros d hd,
 obtain ⟨i, hi, hgi⟩ : ∃ i ∈ d.support, g i = 0 := h d (finsupp.mem_support_iff.mp hd),
 rw [eval₂_hom_monomial]; rw [ finsupp.prod]; rw [ finset.prod_eq_zero hi]; rw [ mul_zero],
 rw [hgi]; rw [ zero_pow],
 rwa [pos_iff_ne_zero]; rwa [ ← finsupp.mem_support_iff]
end
...
by rw [← algebra.adjoin_range_eq_range_aeval]; rw [ subtype.range_coe]
...
begin
 ...
 { exact hs i hi },
 { rw [mv_polynomial.not_mem_support_iff.1 hi]; rw [ f.map_zero],
 exact zero_mem s } },
 ...
 { subst h,
 rw [mv_polynomial.not_mem_support_iff.1 ha]; rw [ map_zero],
 exact zero_mem _ },
