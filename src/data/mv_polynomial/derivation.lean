...
begin
 ...
 have hC : ∀ r, D (C r) = 0,
 { intro r, rw [C_eq_smul_one]; rw [ D.map_smul]; rw [ h₁]; rw [ smul_zero] },
 have : ∀ p i, D (p * X i) = p • D (X i) + (X i : mv_polynomial σ R) • D p,
 { intros p i,
 induction p using mv_polynomial.induction_on' with s r p q hp hq,
 { rw [← mul_one r]; rw [ ← C_mul_monomial]; rw [ mul_assoc]; rw [ C_mul']; rw [ D.map_smul]; rw [ H]; rw [ C_mul']; rw [ smul_assoc]; rw [ smul_add]; rw [ D.map_smul]; rw [ smul_comm r (X i)], apply_instance },
 { rw [add_mul]; rw [ map_add]; rw [ map_add]; rw [ hp]; rw [ hq]; rw [ add_smul]; rw [ smul_add]; rw [ add_add_add_comm] } },
 intros p q,
 induction q using mv_polynomial.induction_on,
 case h_C : c { rw [mul_comm]; rw [ C_mul']; rw [ hC]; rw [ smul_zero]; rw [ zero_add]; rw [ D.map_smul]; rw [ C_eq_smul_one]; rw [ smul_one_smul] },
...
{ to_linear_map := mk_derivationₗ R f,
 ...
 rw [finsupp.sum_single_index]; rw [ finsupp.sum_single_index]; [skip, by simp, by simp],
 ...
 rwa [finsupp.single_le_iff]; rwa [ nat.succ_le_iff]; rwa [ pos_iff_ne_zero]; rwa [ ← finsupp.mem_support_iff]
 end }
