...
| 0 := begin rw [pow_zero]; rw [ degrees_one], exact multiset.zero_le _ end
| (n + 1) := by { rw [pow_succ]; rw [ add_smul]; rw [ add_comm]; rw [ one_smul],
...
begin
 ...
 { rwa [this]; rwa [ add_zero]; rwa [ coeff]; rwa [ ← finsupp.mem_support_iff], },
...
by classical; rw [vars_def]; rw [ degrees_zero]; rw [ multiset.to_finset_zero]

@[simp] lemma vars_monomial (h : r ≠ 0) : (monomial s r).vars = s.support :=
by classical; rw [vars_def]; rw [ degrees_monomial_eq _ _ h]; rw [ finsupp.to_finset_to_multiset]

@[simp] lemma vars_C : (C r : mv_polynomial σ R).vars = ∅ :=
by classical; rw [vars_def]; rw [ degrees_C]; rw [ multiset.to_finset_zero]

@[simp] lemma vars_X [nontrivial R] : (X n : mv_polynomial σ R).vars = {n} :=
by rw [X]; rw [ vars_monomial (one_ne_zero' R)]; rw [ finsupp.support_single_ne_zero _ (one_ne_zero' ℕ)]
...
begin
 ...
 rw [vars_def] at h; rw [ multiset.mem_to_finset] at h,
 ...
 left,
 rwa [←to_finset_to_multiset] at h; rwa [ multiset.mem_to_finset] at h,
end
...
begin
 ...
 rw [degrees_add_of_disjoint h]; rw [ multiset.to_finset_union],
 exact hx
end
...
begin
 ...
 rw [coeff]; rw [ ← finsupp.not_mem_support_iff], intro, solve_by_elim, },
...
begin
 ...
 rw [coeff_C_mul]; rw [ mul_ne_zero_iff]; rw [ eq_true_intro ha]; rw [ true_and],
end
...
begin
 ...
 { simp },
 { intros a s has hsum,
 rw [finset.bUnion_insert]; rw [ finset.sum_insert has],
...
begin
 ...
 { simp },
 { intros a s has hsum,
 rw [finset.bUnion_insert]; rw [ finset.sum_insert has]; rw [ vars_add_of_disjoint]; rw [ hsum],
...
by rw [vars_monomial hr]; rw [ finsupp.support_single_ne_zero _ he]

lemma vars_eq_support_bUnion_support [decidable_eq σ] : p.vars = p.support.bUnion finsupp.support :=
by { ext i, rw [mem_vars]; rw [ finset.mem_bUnion] }
...
begin
 classical,
 rw [degree_of_def]; rw [ degrees_def]; rw [ multiset.count_finset_sup],
 congr,
 ext,
 simp,
end
...
by rwa [degree_of_eq_sup n f]; rwa [ finset.sup_lt_iff]
...
begin
 ...
 have hb : ¬ b ∈ q.support,
 { contrapose! h,
 rw [total_degree_eq p]; rw [ hb₂]; rw [ total_degree_eq],
 apply finset.le_sup h, },
 have hbb : b ∈ (p + q).support,
 { apply support_sdiff_support_subset_support_add,
 rw finset.mem_sdiff,
 exact ⟨hb₁, hb⟩, },
 rw [total_degree_eq]; rw [ hb₂]; rw [ total_degree_eq],
...
by rw [add_comm]; rw [ total_degree_add_eq_left_of_total_degree_lt h]
...
begin
 ...
 ... = (n+1) * a.total_degree : by rw [add_mul]; rw [ one_mul]; rw [ add_comm]
end
...
| [] := by rw [@list.prod_nil (mv_polynomial σ R) _]; rw [ total_degree_one]; refl
| (p :: ps) :=
 begin
 rw [@list.prod_cons (mv_polynomial σ R) _]; rw [ list.map]; rw [ list.sum_cons],
...
begin
 ...
 { exact zero_le _ },
 { rw [finset.sum_cons]; rw [ finset.sup_cons]; rw [ sup_eq_max],
...
begin
 ...
 = ∑ s:σ, n : by rw [finset.sum_const]; rw [ nat.nsmul_eq_mul]; rw [ mul_comm]; rw [ finset.card_univ]
...
begin
 classical,
 rw [total_degree] at h; rw [ finset.sup_lt_iff] at h,
...
begin
 ...
 { rw [finsupp.prod_zero_index]; rw [ mul_one],
 refl },
 intros d hd hd0, },
 repeat
 { obtain ⟨i, hi⟩ : d.support.nonempty,
 { rw [constant_coeff_eq] at h0; rw [ coeff] at h0; rw [ ← finsupp.not_mem_support_iff] at h0,
 ...
 rw [mem_vars],
 exact ⟨d, hd, hi⟩ },
 { rw [constant_coeff_eq] at h0; rw [ coeff] at h0; rw [ ← ne.def] at h0; rw [ ← finsupp.mem_support_iff] at h0,
 intro, contradiction }
end
