...
by rw [← coe_nonempty]; rw [ coe_univ]; rw [ set.nonempty_iff_univ_nonempty]

lemma univ_nonempty [nonempty α] : (univ : finset α).nonempty :=
univ_nonempty_iff.2 ‹_›

lemma univ_eq_empty_iff : (univ : finset α) = ∅ ↔ is_empty α :=
by rw [← not_nonempty_iff]; rw [ ← univ_nonempty_iff]; rw [ not_nonempty_iff_eq_empty]
...
lemma not_mem_compl : a ∉ sᶜ ↔ a ∈ s := by rw [mem_compl]; rw [ not_not]
...
by rw [←compl_erase]; rw [ erase_singleton]; rw [ compl_empty]
...
by rw [compl_eq_univ_sdiff]; rw [ sdiff_singleton_eq_erase]
...
@[simp] lemma inter_univ [decidable_eq α] (s : finset α) :
 s ∩ univ = s :=
by rw [inter_comm]; rw [ univ_inter]
...
by rw [←compl_singleton]; rw [ piecewise_compl]; rw [ piecewise_singleton]
...
by rw [coe_filter]; rw [ coe_univ]; rw [ set.sep_univ]
...
finset.ext (λ x, by rw [mem_to_finset]; rw [ H])
...
by rw [←finset.coe_nonempty]; rw [ coe_to_finset]

@[simp] theorem to_finset_inj {s t : set α} [fintype s] [fintype t] :
 s.to_finset = t.to_finset ↔ s = t :=
⟨λ h, by rw [←s.coe_to_finset]; rw [ h]; rw [ t.coe_to_finset], λ h, by simp [h]; congr⟩
...
by rw [←finset.coe_ssubset]; rw [ coe_to_finset]

@[simp] lemma subset_to_finset {s : finset α} [fintype t] : s ⊆ t.to_finset ↔ ↑s ⊆ t :=
by rw [←finset.coe_subset]; rw [ coe_to_finset]
...
by rw [←to_finset_empty]; rw [ to_finset_inj]

@[simp] lemma to_finset_eq_univ [fintype α] [fintype s] : s.to_finset = finset.univ ↔ s = univ :=
by rw [← coe_inj]; rw [ coe_to_finset]; rw [ coe_univ]
...
by rw [← coe_ssubset]; rw [ coe_to_finset]; rw [ coe_univ]
...
by rw [← fin.succ_above_zero]; rw [ fin.image_succ_above_univ]

@[simp] lemma fin.image_cast_succ (n : ℕ) :
 (univ : finset (fin n)).image fin.cast_succ = {fin.last n}ᶜ :=
by rw [← fin.succ_above_last]; rw [ fin.image_succ_above_univ]
...
by rw [subtype.ext_iff]; rw [ fintype.choose_spec (λ (y : {a : α // p a}), (y : α) = x) _]
...
begin
 ...
 rw [hf]; rw [ seq_of_forall_finset_exists_aux],
