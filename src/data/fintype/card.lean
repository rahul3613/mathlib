...
lemma finset.eq_univ_of_card [fintype α] (s : finset α) (hs : s.card = fintype.card α) :
 s = univ :=
eq_of_subset_of_card_le (subset_univ _) $ by rw [hs]; rw [ finset.card_univ]
...
begin
 classical,
 rw [← set.to_finset_card]; rw [ ← set.to_finset_card]; rw [ ← finset.card_compl]; rw [ set.to_finset_compl]
end
...
by rw [finset.card_univ]; rw [ fintype.card_fin]
...
... < card β : finset.card_lt_univ_of_not_mem $
 by rwa [← mem_coe]; rwa [ coe_map]; rwa [ coe_univ]; rwa [ set.image_univ]
...
by rw [←card_unit]; rw [ card_eq]; exact
...
by rw [card]; rw [ finset.card_eq_zero]; rw [ univ_eq_empty_iff]
...
begin
 ...
 rw ←not_iff_not,
 push_neg,
 rw [not_nontrivial_iff_subsingleton]; rw [ card_le_one_iff_subsingleton]
end
...
by rw [←set.to_finset_card]; rw [ finset.card_eq_iff_eq_univ]; rw [ ←set.to_finset_univ]; rw [ set.to_finset_inj]
...
lemma finset.univ_map_embedding {α : Type*} [fintype α] (e : α ↪ α) :
 univ.map e = univ :=
by rw [←e.equiv_of_fintype_self_embedding_to_embedding]; rw [ univ_map_equiv_to_embedding]
...
begin
 classical,
 rw [fintype.card_of_subtype (set.to_finset pᶜ)]; rw [ set.to_finset_compl p]; rw [ finset.card_compl]; rw [ fintype.card_of_subtype (set.to_finset p)];
...
of_not_fintype $ λ h, begin
 ...
 by rwa [set.to_finset_ssubset_univ]; rwa [ set.ssubset_univ_iff]
end
...
lemma exists_subset_card_eq (α : Type*) [infinite α] (n : ℕ) :
 ∃ s : finset α, s.card = n :=
⟨(range n).map (nat_embedding α), by rw [card_map]; rw [ card_range]⟩
