...
by rw [← prod_filter]; rw [ filter_mem_eq_inter]; rw [ univ_inter]
...
by rw [fintype.card_pi]; rw [ finset.prod_const]; refl
...
... = ∏ i in range n, f i : by rw [← attach_eq_univ]; rw [ prod_attach]
...
begin
 rw [← (equiv.sigma_fiber_equiv f).prod_comp]; rw [ ← univ_sigma_univ]; rw [ prod_sigma],
 refl
end
