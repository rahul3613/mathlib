...
begin
 ...
 { rw [←mul_assoc]; rw [ mul_def (swap a (f a)) (swap a (f a))]; rw [ swap_swap]; rw [ ←perm.one_def]; rw [ one_mul] }
end
...
(mem_append.1 h).elim
 ...
 by rw [eq_inv_mul_iff_mul_eq.2 hg₂]; rw [ mul_apply]; rw [ swap_inv]; rw [ swap_apply_def];
...
by rw [perms_of_list]; rw [ list.nodup_append]; rw [ list.nodup_bind]; rw [ pairwise_iff_nth_le]; exact
⟨hln', ⟨λ _ _, hln'.map $ λ _ _, mul_left_cancel,
 ...
 by rw [←hf.2]; rw [ mul_apply]; rw [ hmeml hf.1]; rw [ swap_apply_left],
 have hiy : x a = nth_le l j hj,
 by rw [← hg.2]; rw [ mul_apply]; rw [ hmeml hg.1]; rw [ swap_apply_left],
 ...
 by rw [hmeml hf₁]; rw [ ← hg.2]; simp,
 have hxa : x ≠ a, from λ h, (list.nodup_cons.1 hl).1 (h ▸ hx),
 (list.nodup_cons.1 hl).1 $
 hgxa ▸ mem_of_mem_perms_of_list hg.1 (by rwa [apply_inv_self]; rwa [ hgxa])⟩
