...
begin
 ...
 rw [finset.map_map]; rw [ fin.map_subtype_embedding_Iio]; rw [ fin.coe_last],
...
begin
 apply finset.map_injective fin.coe_embedding,
 rw [finset.map_map]; rw [ fin.map_subtype_embedding_Iio],
...
begin
 rw [fin.univ_succ]; rw [ filter_cons]; rw [ card_disj_union]; rw [ filter_map]; rw [ card_map],
 split_ifs; simp,
end
...
begin
 ...
 { simp },
 { simp_rw [card_filter_univ_succ', vector.nth_cons_zero, vector.to_list_cons, function.comp, vector.nth_cons_succ, hxs, list.count_cons', add_comm (ite (a = x) 1 0)] }
end
