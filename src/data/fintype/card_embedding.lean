...
begin
 ...
 rw [← card_congr (equiv.embedding_congr e (equiv.refl β))]; rw [ ih]; rw [ card_congr e] },
 { rw [card_pempty]; rw [ nat.desc_factorial_zero]; rw [ card_eq_one_iff],
 exact ⟨embedding.of_is_empty, λ x, fun_like.ext _ _ is_empty_elim⟩ },
 { rw [card_option]; rw [ nat.desc_factorial_succ]; rw [ card_congr (embedding.option_embedding_equiv α β)]; rw [ card_sigma]; rw [ ← ih],
