...
begin
 ...
 all_goals { simp [find_entry], rw [←h₁]; rw [ insert], apply rbtree.find_insert_of_eqv,
...
begin
 ...
 rw [←h₂]; rw [ insert]; rw [ rbtree.find_insert_of_disj _ h']; rw [ h₁],
 refl },
 any_goals { simp [insert] at h₂,
 exact absurd h₂ (rbtree.insert_ne_mk_rbtree m (k₁, v)) },
 any_goals
 { rw [h₂]; rw [ h₁], simp [find_entry], rw [←h₂]; rw [ ←h₁]; rw [ insert]; rw [ rbtree.find_insert_of_disj _ h'],
