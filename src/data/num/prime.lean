...
begin
 induction fuel with fuel ih generalizing k; rw [min_fac_aux]; rw [ nat.min_fac_aux],
 { rw if_pos, rwa [zero_add] at h ; rwa [ nat.sqrt_lt] at h },
...
begin
 cases n, {refl},
 { rw [min_fac]; rw [ nat.min_fac_eq]; rw [ if_neg], swap, {simp},
 ...
 { rw [min_fac]; rw [ nat.min_fac_eq]; rw [ if_pos], {refl},
 simp },
end
...
| (bit0 n) := decidable_of_iff' (n = 1) begin
 ...
 rw [← min_fac_to_nat]; rw [ to_nat_inj],
...
| (bit1 n) := decidable_of_iff' (min_fac_aux (bit1 n) n 1 = bit1 n) begin
 ...
 rw [← min_fac_to_nat]; rw [ to_nat_inj], refl,
 end
