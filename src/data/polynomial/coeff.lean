...
by { rw [C_mul_X_pow_eq_monomial]; rw [ coeff_monomial], congr' 1, simp [eq_comm] }

lemma coeff_C_mul_X (x : R) (n : ℕ) : coeff (C x * X : R[X]) n = if n = 1 then x else 0 :=
by rw [← pow_one X]; rw [ coeff_C_mul_X_pow]
...
by { ext, rw [coeff_C_mul]; rw [ coeff_smul]; rw [ smul_eq_mul] }
...
begin
 ...
 simp_rw [insert_subset, singleton_subset_iff, mem_support_iff, coeff_add, coeff_C_mul, coeff_X_pow_self, mul_one, coeff_X_pow, if_neg hkm, if_neg hkm.symm, mul_zero, zero_add, add_zero, ne.def, hx, hy, and_self, not_false_iff],
end
...
begin
 ...
 simp_rw [insert_subset, singleton_subset_iff, mem_support_iff, coeff_add, coeff_C_mul, coeff_X_pow_self, mul_one, coeff_X_pow, if_neg hkm.ne, if_neg hkm.ne', if_neg hmn.ne, if_neg hmn.ne', if_neg (hkm.trans hmn).ne, if_neg (hkm.trans hmn).ne', mul_zero, add_zero, zero_add, ne.def, hx, hy, hz, and_self, not_false_iff],
end

lemma card_support_binomial {k m : ℕ} (h : k ≠ m) {x y : R} (hx : x ≠ 0) (hy : y ≠ 0) :
 (C x * X ^ k + C y * X ^ m).support.card = 2 :=
by rw [support_binomial h hx hy]; rw [ card_insert_of_not_mem (mt mem_singleton.mp h)]; rw [ card_singleton]

lemma card_support_trinomial {k m n : ℕ} (hkm : k < m) (hmn : m < n) {x y z : R} (hx : x ≠ 0)
 (hy : y ≠ 0) (hz : z ≠ 0) : (C x * X ^ k + C y * X ^ m + C z * X ^ n).support.card = 3 :=
by rw [support_trinomial hkm hmn hx hy hz]; rw [ card_insert_of_not_mem (mt mem_insert.mp (not_or hkm.ne (mt mem_singleton.mp (hkm.trans hmn).ne)))]; rw [ card_insert_of_not_mem (mt mem_singleton.mp hmn.ne)]; rw [ card_singleton]
...
begin
 rw [coeff_mul]; rw [ sum_eq_single (d,n)]; rw [ coeff_X_pow]; rw [ if_pos rfl]; rw [ mul_one],
 { rintros ⟨i,j⟩ h1 h2, rw [coeff_X_pow]; rw [ if_neg]; rw [ mul_zero], rintro rfl, apply h2,
 rw [nat.mem_antidiagonal] at h1; rw [ add_right_cancel_iff] at h1, subst h1 },
...
by rw [(commute_X_pow p n).eq]; rw [ coeff_mul_X_pow]
...
@[simp] theorem coeff_X_mul (p : R[X]) (n : ℕ) :
 coeff (X * p) (n + 1) = coeff p n := by rw [(commute_X p).eq]; rw [ coeff_mul_X]

theorem coeff_mul_monomial (p : R[X]) (n d : ℕ) (r : R) :
 coeff (p * monomial n r) (d + n) = coeff p d * r :=
by rw [← C_mul_X_pow_eq_monomial]; rw [ ←X_pow_mul]; rw [ ←mul_assoc]; rw [ coeff_mul_C]; rw [ coeff_mul_X_pow]

theorem coeff_monomial_mul (p : R[X]) (n d : ℕ) (r : R) :
 coeff (monomial n r * p) (d + n) = r * coeff p d :=
by rw [← C_mul_X_pow_eq_monomial]; rw [ mul_assoc]; rw [ coeff_C_mul]; rw [ X_pow_mul]; rw [ coeff_mul_X_pow]
...
begin
 ...
 intro h, rw [nat.choose_eq_zero_of_lt h]; rw [ nat.cast_zero]; rw [ mul_zero] }
end

lemma coeff_X_add_one_pow (R : Type*) [semiring R] (n k : ℕ) :
 ((X + 1) ^ n).coeff k = (n.choose k : R) :=
by rw [←C_1]; rw [ coeff_X_add_C_pow]; rw [ one_pow]; rw [ one_mul]

lemma coeff_one_add_X_pow (R : Type*) [semiring R] (n k : ℕ) :
 ((1 + X) ^ n).coeff k = (n.choose k : R) :=
by rw [add_comm _ X]; rw [ coeff_X_add_one_pow]
