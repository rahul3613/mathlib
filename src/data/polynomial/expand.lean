...
polynomial.induction_on f (λ r, by simp_rw expand_C)
 ...
 (λ n r ih, by simp_rw [alg_hom.map_mul, expand_C, alg_hom.map_pow, expand_X, alg_hom.map_pow, expand_X, pow_mul])
...
polynomial.induction_on f
 (λ r, by rw expand_C)
 (λ f g ihf ihg, by rw [alg_hom.map_add]; rw [ ihf]; rw [ ihg])
 (λ n r ih, by rw [alg_hom.map_mul]; rw [ expand_C]; rw [ alg_hom.map_pow]; rw [ expand_X]; rw [ pow_one])

theorem expand_pow (f : R[X]) : expand R (p ^ q) f = (expand R p ^[q] f) :=
nat.rec_on q (by rw [pow_zero]; rw [ expand_one]; rw [ function.iterate_zero]; rw [ id]) $ λ n ih,
by rw [function.iterate_succ_apply']; rw [ pow_succ]; rw [ expand_mul]; rw [ ih]

theorem derivative_expand (f : R[X]) :
 (expand R p f).derivative = expand R p f.derivative * (p * X ^ (p - 1)) :=
by rw [coe_expand]; rw [ derivative_eval₂_C]; rw [ derivative_pow]; rw [ C_eq_nat_cast]; rw [ derivative_X]; rw [ mul_one]
...
begin
 ...
 split_ifs with h,
 { rw [finset.sum_eq_single (n/p)]; rw [ nat.mul_div_cancel' h]; rw [ if_pos rfl],
 { intros b hb1 hb2, rw if_neg, intro hb3, apply hb2, rw [← hb3]; rw [ nat.mul_div_cancel_left b hp] },
...
by rw [coeff_expand hp]; rw [ if_pos (dvd_mul_left _ _)]; rw [ nat.mul_div_cancel _ hp]

@[simp] theorem coeff_expand_mul' {p : ℕ} (hp : 0 < p) (f : R[X]) (n : ℕ) :
 (expand R p f).coeff (p * n) = f.coeff n :=
by rw [mul_comm]; rw [ coeff_expand_mul hp]

/-- Expansion is injective. -/
lemma expand_injective {n : ℕ} (hn : 0 < n) : function.injective (expand R n) :=
λ g g' H, ext $ λ k, by rw [← coeff_expand_mul hn]; rw [ H]; rw [ coeff_expand_mul hn]
...
by rw [← expand_C]; rw [ expand_inj hp]; rw [ expand_C]

theorem nat_degree_expand (p : ℕ) (f : R[X]) :
 (expand R p f).nat_degree = f.nat_degree * p :=
begin
 cases p.eq_zero_or_pos with hp hp,
 { rw [hp]; rw [ coe_expand]; rw [ pow_zero]; rw [ mul_zero]; rw [ ← C_1]; rw [ eval₂_hom]; rw [ nat_degree_C] },
 by_cases hf : f = 0,
 { rw [hf]; rw [ alg_hom.map_zero]; rw [ nat_degree_zero]; rw [ zero_mul] },
 have hf1 : expand R p f ≠ 0 := mt (expand_eq_zero hp).1 hf,
 rw [← with_bot.coe_eq_coe]; rw [ ← degree_eq_nat_degree hf1],
 ...
 rw [with_bot.coe_le_coe]; rw [ ← nat.div_mul_cancel hpn], exact nat.mul_le_mul_right p hn },
 { refl } },
 { refine le_degree_of_ne_zero _,
 rw [coeff_expand_mul hp]; rw [ ← leading_coeff], exact mt leading_coeff_eq_zero.1 hf }
end

lemma monic.expand {p : ℕ} {f : R[X]} (hp : 0 < p) (h : f.monic) : (expand R p f).monic :=
begin
 rw [monic.def]; rw [ leading_coeff]; rw [ nat_degree_expand]; rw [ coeff_expand hp],
 simp [hp, h],
end
...
begin
 ...
 rw [alg_hom.map_add]; rw [ eval_add]; rw [ eval_add]; rw [ hf]; rw [ hg]
end
...
begin
 refine polynomial.induction_on P (λ a, by simp) (λ f g hf hg, _) (λ n a h, by simp),
 rw [alg_hom.map_add]; rw [ aeval_add]; rw [ aeval_add]; rw [ hf]; rw [ hg]
end
...
begin
 ...
 cases this,
 { rw this },
 rw [← nat.cast_succ] at this; rw [ char_p.cast_eq_zero_iff R p] at this,
 exact absurd this h }
end
...
begin
 ...
 { rw [alg_hom.map_add]; rw [ polynomial.map_add]; rw [ ha]; rw [ hb]; rw [ add_pow_char], },
 { rw [expand_monomial]; rw [ map_monomial]; rw [ ← C_mul_X_pow_eq_monomial]; rw [ ← C_mul_X_pow_eq_monomial]; rw [ mul_pow]; rw [ ← C.map_pow]; rw [ frobenius_def],
 ring_exp }
end
...
begin
 ...
 symmetry,
 rw [pow_succ']; rw [ pow_mul]; rw [ ← n_ih]; rw [ ← expand_char]; rw [ pow_succ]; rw [ ring_hom.mul_def]; rw [ ← map_map]; rw [ mul_comm]; rw [ expand_mul]; rw [ ← map_expand]
end
...
begin
 ...
 rw [hf2, is_unit_C] at hf1, rw expand_eq_C hp at hf2, rwa [hf2]; rwa [ is_unit_C]
end
...
nat.rec_on n (λ hf, by rwa [pow_zero] at hf) $ λ n ih hf; rwa [ expand_one] at hf) $ λ n ih hf,
