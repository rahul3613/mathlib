...
begin
 ...
 rw [h] at this; rw [ degree_C ha] at this; rw [ degree_mul] at this; rw [ @eq_comm (with_bot ℕ) 0] at this; rw [ nat.with_bot.add_eq_zero_iff] at this,
 exact this.1,
end
...
if hif : degree (f.map i) ≤ 0 then splits_of_map_eq_C i (degree_le_zero_iff.mp hif)
 ...
 push_neg at hif,
 rw [← order.succ_le_iff] at hif; rw [ ← with_bot.coe_zero] at hif; rw [ with_bot.succ_coe] at hif; rw [ nat.succ_eq_succ] at hif,
...
by rw [splits_map_iff]; rw [ ring_hom.id_comp]
...
if hf0' : f.map i = 0 then by simp [eval₂_eq_eval_map, hf0']
 ...
 let ⟨i, hi⟩ := hg.2 in
 ⟨x, by rw [← eval_map]; rw [ hi]; rw [ eval_mul]; rw [ show _ = _]; rw [ from hx]; rw [ zero_mul]⟩
...
by rw [degree_eq_nat_degree p_ne_zero]; rw [ nat_degree_eq_card_roots' hsplit]
...
begin
 classical,
 rw [root_set]; rw [ ←finset.coe_image]; rw [ ←multiset.to_finset_map]; rw [ ←f.coe_to_ring_hom]; rw [ ←roots_map ↑f ((splits_id_iff_splits (algebra_map F K)).mpr h)]; rw [ map_map]; rw [ f.comp_algebra_map]; rw [ ←root_set],
end
...
begin
 rw [←image_root_set h f]; rw [ algebra.adjoin_image]; rw [ ←algebra.map_top],
...
begin
 rw [eq_prod_roots_of_monic_of_splits_id hm hs]; rw [ lifts_iff_lifts_ring],
...
if hf0 : f = 0 then hf0.symm ▸ splits_zero i
 ...
 rw [hs] at hdp; rw [ ← multiset.prod_to_list] at hdp,
...
begin
 split,
 { intro H, rw [nat_degree_eq_card_roots H]; rw [ map_id] },
 { intro hroots,
...
begin
 ...
 rw [multiset.prod_map_mul]; rw [ multiset.map_const]; rw [ multiset.prod_replicate]; rw [ multiset.map_id']; rw [ splits_iff_card_roots.1 hP]
end
