...
begin
 rw [hasse_deriv_apply]; rw [ coeff_sum]; rw [ sum_def]; rw [ finset.sum_eq_single (n + k)]; rw [ coeff_monomial],
...
begin
 rw [hasse_deriv_apply]; rw [ sum_def],
...
begin
 ...
 by_cases hnik : n = i + k,
 { rw [if_pos hnik]; rw [ if_pos]; rw [ ← hnik], apply tsub_eq_of_eq_add_rev, rwa add_comm },
 { rw [if_neg hnik]; rw [ mul_zero],
 by_cases hkn : k ≤ n,
 { rw [← tsub_eq_iff_eq_add_of_le hkn] at hnik, rw [if_neg hnik] },
 { push_neg at hkn, rw [nat.choose_eq_zero_of_lt hkn]; rw [ nat.cast_zero]; rw [ zero_mul]; rw [ if_t_t] } }
end

lemma hasse_deriv_C (r : R) (hk : 0 < k) : hasse_deriv k (C r) = 0 :=
by rw [← monomial_zero_left]; rw [ hasse_deriv_monomial]; rw [ nat.choose_eq_zero_of_lt hk]; rw [ nat.cast_zero]; rw [ zero_mul]; rw [ monomial_zero_right]

lemma hasse_deriv_apply_one (hk : 0 < k) : hasse_deriv k (1 : R[X]) = 0 :=
by rw [← C_1]; rw [ hasse_deriv_C k _ hk]

lemma hasse_deriv_X (hk : 1 < k) : hasse_deriv k (X : R[X]) = 0 :=
by rw [← monomial_one_one_eq_X]; rw [ hasse_deriv_monomial]; rw [ nat.choose_eq_zero_of_lt hk]; rw [ nat.cast_zero]; rw [ zero_mul]; rw [ monomial_zero_right]

lemma factorial_smul_hasse_deriv :
 ⇑(k! • @hasse_deriv R _ k) = ((@derivative R _)^[k]) :=
begin
 induction k with k ih,
 { rw [hasse_deriv_zero]; rw [ factorial_zero]; rw [ iterate_zero]; rw [ one_smul]; rw [ linear_map.id_coe], },
 ext f n : 2,
 ...
 rw [eq_div_iff_mul_eq (mul_ne_zero (H _) (H _))]; rw [ eq_comm]; rw [ div_mul_eq_mul_div]; rw [ eq_div_iff_mul_eq (mul_ne_zero (H _) (H _))],
 norm_cast,
...
begin
 ...
 congr' 2,
 by_cases hikl : i < k + l,
 { rw [choose_eq_zero_of_lt hikl]; rw [ mul_zero],
 by_cases hil : i < l,
 { rw [choose_eq_zero_of_lt hil]; rw [ mul_zero] },
 ...
 succ_sub_succ_eq_sub, add_tsub_cancel_right, add_tsub_cancel_left] with field_simps,
 rw [eq_div_iff_mul_eq]; rw [ eq_comm]; rw [ div_mul_eq_mul_div]; rw [ eq_div_iff_mul_eq]; rw [ ← tsub_add_eq_tsub_tsub]; rw [ add_comm l k],
 { ring, },
