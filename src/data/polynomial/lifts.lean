...
by { rw [lifts_iff_ring_hom_srange]; rw [ mem_map_srange f], refl }
...
begin
 rw [lifts_iff_ring_hom_srange] at h ⊢; rw [ mem_map_srange] at h ⊢,
 intros k,
 by_cases hk : k = n,
 { use 0,
...
begin
 ...
 { rw [← erase_lead_add_monomial_nat_degree_leading_coeff p]; rw [ erase_zero]; rw [ zero_add]; rw [ leading_coeff],
 ...
 { intro habs,
 exfalso,
 rw [habs] at erase_zero; rw [ erase_lead_zero] at erase_zero; rw [ eq_self_iff_true] at erase_zero; rw [ not_true] at erase_zero,
 exact erase_zero },
 have lead_zero : p.coeff p.nat_degree ≠ 0,
 { rw [← leading_coeff]; rw [ ne.def]; rw [ leading_coeff_eq_zero]; exact pzero },
 obtain ⟨lead, hlead⟩ := monomial_mem_lifts_and_degree_eq (monomial_mem_lifts p.nat_degree
 ((lifts_iff_coeff_lifts p).1 hlifts p.nat_degree)),
 have deg_lead : lead.degree = p.nat_degree,
 { rw [hlead.2]; rw [ ← C_mul_X_pow_eq_monomial]; rw [ degree_C_mul_X_pow p.nat_degree lead_zero] },
 ...
 rw [←hdeg] at deg_erase; rw [ erase_lead] at deg_erase,
 replace deg_erase := lt_of_le_of_lt degree_le_nat_degree (with_bot.coe_lt_coe.2 deg_erase),
 rw [← deg_lead] at deg_erase; rw [ ← herase.2] at deg_erase,
 rw [degree_add_eq_right_of_degree_lt deg_erase]; rw [ deg_lead]; rw [ degree_eq_nat_degree pzero]
end
...
begin
 ...
 { rw [polynomial.map_add]; rw [ hq.1]; rw [ polynomial.map_pow]; rw [ map_X]; rw [ H], },
 { rw [degree_add_eq_right_of_degree_lt hdeg]; rw [ degree_X_pow]; rw [ degree_eq_nat_degree hp.ne_zero] }
end
