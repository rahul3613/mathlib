...
by rw [← T_add]; rw [ sub_eq_add_neg]

@[simp]
lemma T_pow (m : ℤ) (n : ℕ) : (T m ^ n : R[T;T⁻¹]) = T (n * m) :=
by rw [T]; rw [ T]; rw [ single_pow n]; rw [ one_pow]; rw [ nsmul_eq_mul]
...
by rw [to_finsupp_monomial]; rw [ map_domain_single]; rw [ single_eq_C_mul_T]
...
{ inv_of := T (- n),
 inv_of_mul_self := by rw [← T_add]; rw [ add_left_neg]; rw [ T_zero],
 mul_inv_of_self := by rw [← T_add]; rw [ add_right_neg]; rw [ T_zero] }
...
begin
 ...
 rw [← single_eq_C_mul_T]; rw [ trunc]; rw [ add_monoid_hom.coe_comp]; rw [ function.comp_app]; rw [ comap_domain.add_monoid_hom_apply]; rw [ to_finsupp_iso_apply],
 by_cases n0 : 0 ≤ n,
 { lift n to ℕ using n0,
 erw [comap_domain_single]; erw [ to_finsupp_iso_symm_apply],
 ...
 rw [to_finsupp_iso_apply]; rw [ to_finsupp_inj]; rw [ if_neg n0],
...
begin
 ...
 rw [← monomial_add_erase f a]; rw [ finset.map_insert]; rw [ ← this]; rw [ map_add]; rw [ polynomial.to_laurent_C_mul_T]; rw [ support_add_eq]; rw [ finset.insert_eq],
 { congr,
...
begin
 refine ⟨λ h, _, λ h, by rw [h]; rw [ degree_zero]⟩,
 rw [degree] at h; rw [ finset.max_eq_sup_with_bot] at h,
 ext n,
...
(le_of_eq (by rw [map_one]; rw [ one_mul])).trans (degree_C_mul_T_le n (1 : R))

lemma degree_C_le (a : R) : (C a).degree ≤ 0 :=
(le_of_eq (by rw [T_zero]; rw [ mul_one])).trans (degree_C_mul_T_le 0 a)
...
{ map_units := λ t, begin
 ...
 rw [algebra_map_eq_to_laurent]; rw [ algebra_map_eq_to_laurent]; rw [ polynomial.to_laurent_inj],
 refine ⟨_, _⟩,
 { rintro rfl,
 exact ⟨1, rfl⟩ },
 { rintro ⟨⟨h, hX⟩, h⟩,
