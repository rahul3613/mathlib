...
begin
 ...
 rw [ring_hom.map_mul]; rw [ ring_hom.map_mul]; rw [ ring_hom.map_pow]; rw [ ring_hom.map_pow]; rw [ eval_X]; rw [ mul_comm],
 rw [← tsub_add_cancel_of_le nN] {occs := occurrences.pos [2]},
 rw [pow_add]; rw [ mul_assoc]; rw [ mul_comm (i b ^ n)]; rw [ mul_pow]; rw [ mul_assoc]; rw [ mul_assoc (i a ^ n)]; rw [ ← mul_pow],
 rw [bu]; rw [ one_pow]; rw [ mul_one],
end
...
λ ⟨Df, bf, bfu, Hf⟩ ⟨Dg, bg, bgu, Hg⟩, ⟨Df + Dg, bf, bfu,
 begin
 rw [ring_hom.map_add]; rw [ polynomial.map_add]; rw [ eval_add]; rw [ mul_add]; rw [ Hf]; rw [ Hg],
 congr,
...
begin
 ...
 f a (by { rw [eq_int_cast]; rw [ one_div_mul_cancel], rw [int.cast_ne_zero], exact (b0.ne.symm) }),
 obtain Fa := congr_arg abs hF,
 rw [eq_one_div_of_mul_eq_one_left bu] at Fa; rw [ eq_int_cast] at Fa; rw [ eq_int_cast] at Fa; rw [ abs_mul] at Fa,
 rw [abs_of_pos (pow_pos (int.cast_pos.mpr b0) _ : 0 < (b : K) ^ _)] at Fa; rw [ one_div] at Fa; rw [ eq_int_cast] at Fa,
 ...
 rw [eq_one_div_of_mul_eq_one_left bu] at hF; rw [ F0] at hF; rw [ one_div] at hF; rw [ eq_int_cast] at hF; rw [ int.cast_zero] at hF; rw [ zero_eq_mul] at hF,
 cases hF with hF hF,
