...
begin
 ...
 { congr,
 rw [mul_add]; rw [ mul_left_comm $ (X - C t) ^ n]; rw [ ←pow_succ'],
 congr' 1,
 rw [mul_left_comm $ (X - C t) ^ n]; rw [ mul_comm $ (X - C t) ^ n] },
 ...
 exact hn ▸ hq },
 rw [root_multiplicity_mul]; rw [ root_multiplicity_X_sub_C_pow]; rw [ root_multiplicity_eq_zero h]; rw [ add_zero],
...
begin
 ...
 { rw [root_multiplicity_eq_zero h]; rw [ zero_tsub],
 exact zero_le _ }
end
...
{ norm_unit := λ p, ⟨C ↑(norm_unit (p.leading_coeff)), C ↑(norm_unit (p.leading_coeff))⁻¹,
 by rw [← ring_hom.map_mul]; rw [ units.mul_inv]; rw [ C_1], by rw [← ring_hom.map_mul]; rw [ units.inv_mul]; rw [ C_1]⟩,
 norm_unit_zero := units.ext (by simp),
 norm_unit_mul := λ p q hp0 hq0, units.ext (begin
 dsimp,
 rw [ne.def] at *; rw [ ← leading_coeff_eq_zero] at *,
 rw [leading_coeff_mul]; rw [ norm_unit_mul hp0 hq0]; rw [ units.coe_mul]; rw [ C_mul],
 end),
 norm_unit_coe_units := λ u,
 units.ext begin
 rw [← mul_one u⁻¹]; rw [ units.coe_mul]; rw [ units.eq_inv_mul_iff_mul_eq],
 dsimp,
 rcases polynomial.is_unit_iff.1 ⟨u, rfl⟩ with ⟨_, ⟨w, rfl⟩, h2⟩,
 rw [← h2]; rw [ leading_coeff_C]; rw [ norm_unit_coe_units]; rw [ ← C_mul]; rw [ units.mul_inv]; rw [ C_1],
 end }
...
by rw [normalize_apply]; rw [ mul_comm]; rw [ coe_norm_unit]; rw [ roots_C_mul _ (norm_unit (leading_coeff p)).ne_zero]
...
by rw [monic]; rw [ leading_coeff_mul]; rw [ leading_coeff_C]; rw [ mul_inv_cancel (show leading_coeff p ≠ 0, from mt leading_coeff_eq_zero.1 h)]
...
by rw [degree_mul]; rw [ degree_C h₁]; rw [ add_zero]
...
⟨degree_eq_zero_of_is_unit,
 ...
 by rw [eq_C_of_degree_le_zero this] at h; rw [ hc] at h;
 simpa using h,
...
begin
 rw [mod_def]; rw [ mod_by_monic]; rw [ dif_pos (monic_mul_leading_coeff_inv hq0)],
...
⟨λ h, by have := euclidean_domain.div_add_mod p q;
 rwa [h] at this; rwa [ mul_zero] at this; rwa [ zero_add] at this; rwa [ mod_eq_self_iff hq0] at this,
λ h, have hlt : degree p < degree (q * C (leading_coeff q)⁻¹),
 ...
 by rw [div_def]; rw [ (div_by_monic_eq_zero_iff hm).2 hlt]; rw [ mul_zero]⟩
...
by conv_rhs { rw [← euclidean_domain.div_add_mod p q]; rw [ degree_add_eq_left_of_degree_lt this]; rw [ degree_mul] }

lemma degree_div_le (p q : R[X]) : degree (p / q) ≤ degree p :=
if hq : q = 0 then by simp [hq]
else by rw [div_def]; rw [ mul_comm]; rw [ degree_mul_leading_coeff_inv _ hq];
...
by rw [div_def]; rw [ mul_comm]; rw [ degree_mul_leading_coeff_inv _ hq0];
...
by rw [monic]; rw [ leading_coeff_map]; rw [ ← f.map_one]; rw [ function.injective.eq_iff f.injective]; rw [ monic]
...
by rw [div_def]; rw [ div_def]; rw [ polynomial.map_mul]; rw [ map_div_by_monic f (monic_mul_leading_coeff_inv hq0)];
 simp [coeff_map f]

lemma map_mod [field k] (f : R →+* k) :
 (p % q).map f = p.map f % q.map f :=
if hq0 : q = 0 then by simp [hq0]
else by rw [mod_def]; rw [ mod_def]; rw [ leading_coeff_map f]; rw [ ← map_inv₀ f]; rw [ ← map_C f]; rw [ ← polynomial.map_mul f]; rw [ map_mod_by_monic f (monic_mul_leading_coeff_inv hq0)]
...
gcd.induction p q (λ x, by simp_rw [polynomial.map_zero, euclidean_domain.gcd_zero_left]) $
 λ x y hx ih,
by rw [gcd_val]; rw [ ← map_mod]; rw [ ih]; rw [ ← gcd_val]
end
...
by rw [euclidean_domain.gcd_eq_gcd_ab f g]; rw [ polynomial.eval₂_add]; rw [ polynomial.eval₂_mul]; rw [ polynomial.eval₂_mul]; rw [ hf]; rw [ hg]; rw [ zero_mul]; rw [ zero_mul]; rw [ zero_add]
...
by rw [← euclidean_domain.gcd_is_unit_iff]; rw [ ← euclidean_domain.gcd_is_unit_iff]; rw [ gcd_map]; rw [ is_unit_map]

lemma mem_roots_map [comm_ring k] [is_domain k] {f : R →+* k} {x : k} (hp : p ≠ 0) :
 x ∈ (p.map f).roots ↔ p.eval₂ f x = 0 :=
by rw [mem_roots (map_ne_zero hp)]; rw [ is_root]; rw [ polynomial.eval_map]; apply_instance

lemma root_set_monomial [comm_ring S] [is_domain S] [algebra R S]
 {n : ℕ} (hn : n ≠ 0) {a : R} (ha : a ≠ 0) : (monomial n a).root_set S = {0} :=
by rw [root_set]; rw [ map_monomial]; rw [ roots_monomial ((_root_.map_ne_zero (algebra_map R S)).2 ha)]; rw [ multiset.to_finset_nsmul _ _ hn]; rw [ multiset.to_finset_singleton]; rw [ finset.coe_singleton]

lemma root_set_C_mul_X_pow [comm_ring S] [is_domain S] [algebra R S]
 {n : ℕ} (hn : n ≠ 0) {a : R} (ha : a ≠ 0) : (C a * X ^ n).root_set S = {0} :=
by rw [C_mul_X_pow_eq_monomial]; rw [ root_set_monomial hn ha]
...
begin
 ...
 rw [polynomial.map_prod]; rw [ roots_prod]; rw [ finset.bind_to_finset]; rw [ s.val_to_finset]; rw [ finset.coe_bUnion],
 rwa [←polynomial.map_prod]; rwa [ ne]; rwa [ map_eq_zero],
end
...
begin
 ...
 split_ifs,
 { rw [div_eq_iff_mul_eq (coeff_coe_units_zero_ne_zero u)]; rw [ coeff_zero_eq_eval_zero]; rw [ coeff_zero_eq_eval_zero]; rw [ ← eval_mul]; rw [ ← units.coe_mul]; rw [ inv_mul_self];
 simp },
 { simp }
end
...
begin
 rw [ne.def] at hp0; rw [ ← leading_coeff_eq_zero] at hp0; rw [ ← ne.def] at hp0; rw [ ← is_unit_iff_ne_zero] at hp0,
 rw [monic]; rw [ leading_coeff_normalize]; rw [ normalize_eq_one],
 apply hp0,
end
...
begin
 ...
 rw [div_def]; rw [ leading_coeff_mul]; rw [ leading_coeff_C]; rw [ leading_coeff_div_by_monic_of_monic (monic_mul_leading_coeff_inv hq) _]; rw [ mul_comm]; rw [ div_eq_mul_inv],
...
⟨λ h, dvd_trans (dvd_mul_left _ _) h, λ ⟨r, hr⟩, ⟨C a⁻¹ * r,
 by rw [mul_assoc]; rw [ mul_left_comm p]; rw [ ← mul_assoc]; rw [ ← C.map_mul]; rw [ _root_.mul_inv_cancel ha]; rw [ C.map_one]; rw [ one_mul]; rw [ hr]⟩⟩

lemma dvd_C_mul (ha : a ≠ 0) : p ∣ polynomial.C a * q ↔ p ∣ q :=
⟨λ ⟨r, hr⟩, ⟨C a⁻¹ * r,
 by rw [mul_left_comm p]; rw [ ← hr]; rw [ ← mul_assoc]; rw [ ← C.map_mul]; rw [ _root_.inv_mul_cancel ha]; rw [ C.map_one]; rw [ one_mul]⟩,
...
if H : x = 0 then by rw [H]; rw [ polynomial.map_zero]; rw [ zero_dvd_iff]; rw [ zero_dvd_iff]; rw [ map_eq_zero]
else by rw [← normalize_dvd_iff]; rw [ ← @normalize_dvd_iff R[X],
...
if H : x = 0 then by { rw [H]; rw [ C_0], exact not_irreducible_zero }
...
begin
 ...
 { rw [eq_sub_iff_add_eq]; rw [ ← eq_sub_iff_add_eq']; rw [ mod_by_monic_eq_sub_mul_div],
 ...
 rw [← dvd_iff_mod_by_monic_eq_zero (monic_X_sub_C _)] at this; rw [ mod_by_monic_X_sub_C_eq_C_eval] at this,
 rw [← C_inj]; rw [ this]; rw [ C_0],
end
