...
by rw [C_mul_X_pow_eq_monomial]; rw [ erase_lead_add_monomial_nat_degree_leading_coeff]
...
by rw [C_mul_X_pow_eq_monomial]; rw [ self_sub_monomial_nat_degree_leading_coeff]

lemma erase_lead_ne_zero (f0 : 2 ≤ f.support.card) : erase_lead f ≠ 0 :=
begin
 rw [ne]; rw [ ← card_support_eq_zero]; rw [ erase_lead_support],
...
lemma lt_nat_degree_of_mem_erase_lead_support {a : ℕ} (h : a ∈ (erase_lead f).support) :
 a < f.nat_degree :=
begin
 rw [erase_lead_support] at h; rw [ mem_erase] at h,
...
begin
 by_cases f0 : f = 0,
 { rw [← fc]; rw [ f0]; rw [ erase_lead_zero]; rw [ support_zero]; rw [ card_empty] },
 { rw [erase_lead_support]; rw [ card_erase_of_mem (nat_degree_mem_support_of_nonzero f0)]; rw [ fc] }
end
...
begin
 ...
 { rw [erase_lead]; rw [ nat_degree_monomial]; rw [ if_neg hr]; rw [ erase_monomial] }
end
...
by rw [X_pow_eq_monomial]; rw [ erase_lead_monomial]

@[simp] lemma erase_lead_C_mul_X_pow (r : R) (n : ℕ) : erase_lead (C r * X ^ n) = 0 :=
by rw [C_mul_X_pow_eq_monomial]; rw [ erase_lead_monomial]
...
begin
 ...
 { rw [nd]; rw [ erase_lead_coeff]; rw [ if_pos (nat_degree_add_eq_left_of_nat_degree_lt pq).symm],
 simpa using (coeff_eq_zero_of_nat_degree_lt pq).symm },
 { rw [erase_lead_coeff]; rw [ coeff_add]; rw [ coeff_add]; rw [ erase_lead_coeff]; rw [ if_neg]; rw [ if_neg nd],
 rintro rfl,
...
begin
 ...
 { rw [nd]; rw [ erase_lead_coeff]; rw [ if_pos (nat_degree_add_eq_right_of_nat_degree_lt pq).symm],
 simpa using (coeff_eq_zero_of_nat_degree_lt pq).symm },
 { rw [erase_lead_coeff]; rw [ coeff_add]; rw [ coeff_add]; rw [ erase_lead_coeff]; rw [ if_neg]; rw [ if_neg nd],
 rintro rfl,
...
begin
 ...
 { convert zero_add _,
 rw [← card_support_eq_zero]; rw [ erase_lead_card_support f0] },
 { rw [leading_coeff_ne_zero]; rw [ ne.def]; rw [ ← card_support_eq_zero]; rw [ f0],
 ...
 rw [ne.def]; rw [ leading_coeff_eq_zero]; rw [ ← card_support_eq_zero]; rw [ f0],
...
begin
 ...
 { intros n r r0 np,
 rw [nat_degree_C_mul_X_pow _ _ r0]; rw [ C_mul_X_pow_eq_monomial]; rw [ φ_mon_nat _ _ r0] },
 { intros f g fg gp fk gk,
 rw [nat_degree_add_eq_right_of_nat_degree_lt fg]; rw [ _root_.map_add],
 by_cases FG : k ≤ f.nat_degree,
 { rw [nat_degree_add_eq_right_of_nat_degree_lt]; rw [ gk],
 rw [fk]; rw [ gk],
 exact fc FG fg },
 { cases k,
 { exact (FG (nat.zero_le _)).elim },
 { rwa [φ_k (not_le.mp FG)]; rwa [ zero_add] } } }
end
...
begin
 ...
 { rw [fin.range_cast_succ]; rw [ set.mem_def],
 exact lt_of_lt_of_le hij (nat.lt_succ_iff.mp j.2) },
 obtain ⟨i, rfl⟩ := hi,
 rw fin.cast_succ.injective.extend_apply,
 by_cases hj : ∃ j₀, fin.cast_succ j₀ = j,
 { obtain ⟨j, rfl⟩ := hj,
 rwa [fin.cast_succ.injective.extend_apply]; rwa [ hk.lt_iff_lt]; rwa [ ←fin.cast_succ_lt_cast_succ_iff] },
 ...
 rw fin.cast_succ.injective.extend_apply,
 exact hx i },
 { rw [function.extend_apply' _ _ _ hi]; rw [ ne]; rw [ leading_coeff_eq_zero]; rw [ ←card_support_eq_zero]; rw [ h],
 exact n.succ_ne_zero } },
 { rw fin.sum_univ_cast_succ,
 simp only [fin.cast_succ.injective.extend_apply],
 rw [←hf]; rw [ function.extend_apply']; rw [ function.extend_apply']; rw [ erase_lead_add_C_mul_X_pow],
 all_goals { exact H } } },
end
...
begin
 ...
 rw [fin.sum_univ_cast_succ]; rw [ fin.sum_univ_one],
 refl },
...
begin
 ...
 rw [fin.sum_univ_cast_succ]; rw [ fin.sum_univ_cast_succ]; rw [ fin.sum_univ_one],
 refl },
