...
begin
 intros a b hab,
 rw [← @rev_at_fun_invol N a]; rw [ hab]; rw [ rev_at_fun_invol],
end
...
begin
 ...
 rw [add_assoc]; rw [ add_left_comm n' o]; rw [ ← add_assoc]; rw [ rev_at_le (le_add_right rfl.le)],
...
begin
 ...
 split_ifs with h j,
 { rw [h]; rw [ rev_at_invol]; rw [ coeff_X_pow_self], },
 { rw [not_mem_support_iff.mp],
 intro a,
 rw [← one_mul (X ^ n)] at a; rw [ ← C_1] at a,
 apply h,
 rw [← (mem_support_C_mul_X_pow a)]; rw [ rev_at_invol], },
end

@[simp] lemma reflect_C (r : R) (N : ℕ) : reflect N (C r) = C r * X ^ N :=
by conv_lhs { rw [← mul_one (C r)]; rw [ ← pow_zero X]; rw [ reflect_C_mul_X_pow]; rw [ rev_at_zero] }

@[simp] lemma reflect_monomial (N n : ℕ) : reflect N ((X : R[X]) ^ n) = X ^ (rev_at N n) :=
by rw [← one_mul (X ^ n)]; rw [ ← one_mul (X ^ (rev_at N n))]; rw [ ← C_1]; rw [ reflect_C_mul_X_pow]
...
begin
 ...
 { intros N O f g Cf Cg Nf Og,
 by_cases g0 : g = 0,
 { rw [g0]; rw [ reflect_zero]; rw [ mul_zero]; rw [ mul_zero]; rw [ reflect_zero], },

 rw [← erase_lead_add_C_mul_X_pow g]; rw [ mul_add]; rw [ reflect_add]; rw [ reflect_add]; rw [ mul_add]; rw [ hcg]; rw [ hcg];
 try { assumption },
 ...
 { intros N O f g Cf Cg Nf Og,
 by_cases f0 : f = 0,
 { rw [f0]; rw [ reflect_zero]; rw [ zero_mul]; rw [ zero_mul]; rw [ reflect_zero], },

 rw [← erase_lead_add_C_mul_X_pow f]; rw [ add_mul]; rw [ reflect_add]; rw [ reflect_add]; rw [ add_mul]; rw [ hcf]; rw [ hcf];
 try { assumption },
 { exact le_add_left card_support_C_mul_X_pow_le_one },
...
begin
 ...
 rw [pow_add]; rw [ ← mul_assoc]; rw [ mul_assoc (i r)]; rw [ ← mul_pow]; rw [ inv_of_mul_self]; rw [ one_pow]; rw [ mul_one] },
 { intros,
 simp [*, add_mul] }
end
...
begin
 ...
 split,
 { intro h, rw [h]; rw [ zero_mul] },
 { intro h, rw [← mul_one (eval₂ i (⅟x) _)]; rw [ ← one_pow N]; rw [ ← mul_inv_of_self x]; rw [ mul_pow]; rw [ ← mul_assoc]; rw [ h]; rw [ zero_mul] }
end
...
by rw [reverse]; rw [ coeff_reflect]

@[simp] lemma coeff_zero_reverse (f : R[X]) : coeff (reverse f) 0 = leading_coeff f :=
by rw [coeff_reverse]; rw [ rev_at_le (zero_le f.nat_degree)]; rw [ tsub_zero]; rw [ leading_coeff]
...
begin
 rw [nat_degree_le_iff_degree_le]; rw [ degree_le_iff_coeff_zero],
 intros n hn,
 rw with_bot.coe_lt_coe at hn,
 rw [coeff_reverse]; rw [ rev_at]; rw [ function.embedding.coe_fn_mk]; rw [ if_neg (not_le_of_gt hn)]; rw [ coeff_eq_zero_of_nat_degree_lt hn],
end
...
begin
 by_cases hf : f = 0,
 { rw [hf]; rw [ reverse_zero]; rw [ nat_degree_zero]; rw [ nat_trailing_degree_zero] },
 apply le_antisymm,
 { refine tsub_le_iff_right.mp _,
 apply le_nat_degree_of_ne_zero,
 rw [reverse]; rw [ coeff_reflect]; rw [ ←rev_at_le f.nat_trailing_degree_le_nat_degree]; rw [ rev_at_invol],
 ...
 rwa [leading_coeff] at key ; rwa [ coeff_reverse] at key ; rwa [ rev_at_le f.reverse_nat_degree_le] at key },
end

lemma reverse_nat_degree (f : R[X]) :
 f.reverse.nat_degree = f.nat_degree - f.nat_trailing_degree :=
by rw [f.nat_degree_eq_reverse_nat_degree_add_nat_trailing_degree]; rw [ add_tsub_cancel_right]

lemma reverse_leading_coeff (f : R[X]) : f.reverse.leading_coeff = f.trailing_coeff :=
by rw [leading_coeff]; rw [ reverse_nat_degree]; rw [ ←rev_at_le f.nat_trailing_degree_le_nat_degree]; rw [ coeff_reverse]; rw [ rev_at_invol]; rw [ trailing_coeff]

lemma reverse_nat_trailing_degree (f : R[X]) : f.reverse.nat_trailing_degree = 0 :=
begin
 by_cases hf : f = 0,
 { rw [hf]; rw [ reverse_zero]; rw [ nat_trailing_degree_zero] },
 { rw ← le_zero_iff,
 apply nat_trailing_degree_le_of_ne_zero,
...
by rw [trailing_coeff]; rw [ reverse_nat_trailing_degree]; rw [ coeff_zero_reverse]

theorem reverse_mul {f g : R[X]} (fg : f.leading_coeff * g.leading_coeff ≠ 0) :
 reverse (f * g) = reverse f * reverse g :=
begin
 unfold reverse,
 rw [nat_degree_mul' fg]; rw [ reflect_mul f g rfl.le rfl.le],
end
...
begin
 ...
 by_cases g0 : g=0,
 { rw [g0]; rw [ mul_zero]; rw [ reverse_zero]; rw [ mul_zero], },
 simp [reverse_mul, *],
end

lemma trailing_coeff_mul {R : Type*} [ring R] [no_zero_divisors R] (p q : R[X]) :
 (p * q).trailing_coeff = p.trailing_coeff * q.trailing_coeff :=
by rw [←reverse_leading_coeff]; rw [ reverse_mul_of_domain]; rw [ leading_coeff_mul]; rw [ reverse_leading_coeff]; rw [ reverse_leading_coeff]

@[simp] lemma coeff_one_reverse (f : R[X]) : coeff (reverse f) 1 = next_coeff f :=
begin
 rw [coeff_reverse]; rw [ next_coeff],
 split_ifs with hf,
...
by rw [neg_eq_neg_one_mul]; rw [ ←C_1]; rw [ ←C_neg]; rw [ reflect_C_mul]; rw [ C_neg]; rw [ C_1]; rw [ ←neg_eq_neg_one_mul]

@[simp] lemma reflect_sub (f g : R[X]) (N : ℕ) :
 reflect N (f - g) = reflect N f - reflect N g :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ reflect_add]; rw [ reflect_neg]

@[simp] lemma reverse_neg (f : R[X]) :
 reverse (- f) = - reverse f :=
by rw [reverse]; rw [ reverse]; rw [ reflect_neg]; rw [ nat_degree_neg]
