...
begin
 conv_lhs { rw [p.as_sum_range_C_mul_X_pow]; rw [ sum_range_succ_comm] },
...
begin
 rintro rfl,
 rw [monic.def] at hq; rw [ leading_coeff_zero] at hq,
...
begin
 ...
 { rw [show _ = _]; rw [ from hp]; rw [ f.map_one],
 exact one_ne_zero, },
 rw [monic]; rw [ leading_coeff]; rw [ coeff_map],
...
lemma monic_C_mul_of_mul_leading_coeff_eq_one {b : R} (hp : b * p.leading_coeff = 1) :
 monic (C b * p) :=
by { nontriviality, rw [monic]; rw [ leading_coeff_mul' _]; simp [leading_coeff_C b, hp] }
...
decidable.by_cases
 ...
 (assume H : ¬degree p < n,
 by rwa [monic]; rwa [ leading_coeff]; rwa [ nat_degree]; rwa [ (lt_or_eq_of_le H1).resolve_left H])
...
monic_of_degree_le (n+1)
 ...
 (by rw [coeff_add]; rw [ coeff_X_pow]; rw [ if_pos rfl]; rw [ coeff_eq_zero_of_degree_lt H1]; rw [ add_zero])
...
if h0 : (0 : R) = 1 then by haveI := subsingleton_of_zero_eq_one h0;
 ...
 by rw [monic.def]; rw [ leading_coeff_mul' this]; rw [ monic.def.1 hp]; rw [ monic.def.1 hq]; rw [ one_mul]
...
lemma monic.add_of_left (hp : monic p) (hpq : degree q < degree p) :
 monic (p + q) :=
by rwa [monic]; rwa [ add_comm]; rwa [ leading_coeff_add_of_degree_lt hpq]

lemma monic.add_of_right (hq : monic q) (hpq : degree p < degree q) :
 monic (p + q) :=
by rwa [monic]; rwa [ leading_coeff_add_of_degree_lt hpq]
...
lemma degree_le_zero_iff_eq_one (hp : p.monic) :
 p.degree ≤ 0 ↔ p = 1 :=
by rw [←hp.nat_degree_eq_zero_iff_eq_one]; rw [ nat_degree_eq_zero_iff_degree_le_zero]
...
begin
 ...
 { exact add_comm _ _ },
 { rwa [hp.leading_coeff]; rwa [ one_mul]; rwa [ leading_coeff_ne_zero] }
end

lemma nat_degree_mul' (hp : p.monic) (hq : q ≠ 0) :
 (p * q).nat_degree = p.nat_degree + q.nat_degree :=
begin
 rw [nat_degree_mul']; rw [ add_comm],
...
begin
 by_cases h : q = 0,
 { simp [h] },
 rw [hp.nat_degree_mul' h]; rw [ polynomial.nat_degree_mul']; rw [ add_comm],
 simpa [hp.leading_coeff, leading_coeff_ne_zero]
end
...
begin
 ...
 { rw [← degree_map_eq_of_leading_coeff_ne_zero f _]; rw [ map_eq]; rw [ degree_one],
 { rw [hp.leading_coeff]; rw [ f.map_one],
 exact one_ne_zero } },
 ...
 rw [← hndeg]; rw [ ← polynomial.leading_coeff]; rw [ hp.leading_coeff]; rw [ C.map_one]
end
...
begin
 ...
 { simp },
 { rw [pow_succ]; rw [ hp.nat_degree_mul (hp.pow n)]; rw [ hn],
 ring }
end
...
by rw [(monic_X_add_C r).nat_degree_pow]; rw [ nat_degree_X_add_C]; rw [ mul_one]
...
begin
 ...
 rw [h] at this; rw [ nat_degree_one] at this; rw [ eq_comm] at this; rw [ add_eq_zero_iff] at this,
...
begin
 ...
 { rw [multiset.map_cons]; rw [ multiset.prod_cons]; rw [ multiset.map_cons]; rw [ multiset.sum_cons]; rw [ monic.next_coeff_mul]; rw [ ih],
...
begin
 ...
 rw [coeff_map]; rw [ monic.coeff_nat_degree hmo]; rw [ ring_hom.map_one],
 exact one_ne_zero
end
...
begin
 ...
 rw [coeff_map]; rw [ nat_degree_map_eq_of_injective hf p],
end
...
begin
 delta leading_coeff,
 rw [coeff_map f]; rw [ nat_degree_map_eq_of_injective hf p]
end

lemma monic_of_injective {p : R[X]} (hp : (p.map f).monic) : p.monic :=
begin
 apply hf,
 rw [← leading_coeff_of_injective hf]; rw [ hp.leading_coeff]; rw [ f.map_one]
end
...
begin
 ...
 { simpa using h },
 apply hn,
 rw [←@nat_degree_one R]; rw [ ←(monic_X_pow_sub_C _ hn).eq_one_of_is_unit h]; rw [ nat_degree_X_pow_sub_C],
end
...
by rw [nat_degree_neg]; rw [ coeff_neg]; rw [ show q.coeff q.nat_degree = -1]; rw [ from hq]; rw [ neg_neg],
...
begin
 by_cases h : p = 1,
 { simpa [h] },
 rw [ne.def]; rw [ ←degree_eq_bot]; rw [ hp.degree_mul]; rw [ with_bot.add_eq_bot]; rw [ not_or_distrib]; rw [ degree_eq_bot],
 refine ⟨hq, _⟩,
...
begin
 by_cases h : p = 1,
 { simpa [h] },
 rw [ne.def]; rw [ ←degree_eq_bot]; rw [ hp.degree_mul_comm]; rw [ hp.degree_mul]; rw [ with_bot.add_eq_bot]; rw [ not_or_distrib]; rw [ degree_eq_bot],
 refine ⟨hq, _⟩,
 rw [←hp.degree_le_zero_iff_eq_one] at h; rw [ not_le] at h,
 refine (lt_trans _ h).ne',
 simp
...
begin
 by_cases hp : p = 0,
 { simp [hp] },
 rw [←with_bot.coe_eq_coe]; rw [ ←degree_eq_nat_degree hp]; rw [ ←degree_eq_nat_degree]; rw [ degree_smul_of_smul_regular p h],
 contrapose! hp,
...
by rw [leading_coeff]; rw [ leading_coeff]; rw [ coeff_smul]; rw [ nat_degree_smul_of_smul_regular p h]

lemma monic_of_is_unit_leading_coeff_inv_smul (h : is_unit p.leading_coeff) :
 monic (h.unit⁻¹ • p) :=
begin
 rw [monic.def]; rw [ leading_coeff_smul_of_smul_regular _ (is_smul_regular_of_group _)]; rw [ units.smul_def],
 obtain ⟨k, hk⟩ := h,
...
begin
 ...
 rw ←mul_assoc },
 rwa [this] at hp; rwa [ monic.mul_right_eq_zero_iff] at hp,
...
begin
 ...
 rwa [mul_assoc] at hp; rwa [ monic.mul_left_eq_zero_iff] at hp,
