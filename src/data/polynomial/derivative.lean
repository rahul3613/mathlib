...
begin
 ...
 { assume b, cases b,
 { intros, rw [nat.cast_zero]; rw [ mul_zero]; rw [ zero_mul], },
 { intros _ H, rw [nat.succ_sub_one b]; rw [ if_neg (mt (congr_arg nat.succ) H.symm)]; rw [ mul_zero] } },
 { rw [if_pos (add_tsub_cancel_right n 1).symm]; rw [ mul_one]; rw [ nat.cast_add]; rw [ nat.cast_one]; rw [ mem_support_iff],
...
by { rw [derivative_apply]; rw [ sum_monomial_index]; rw [ C_mul_X_pow_eq_monomial], simp }
...
by rw [C_mul_X_pow_eq_monomial]; rw [ C_mul_X_pow_eq_monomial]; rw [ derivative_monomial]

lemma derivative_C_mul_X_sq (a : R) : derivative (C a * X ^ 2) = C (a * 2) * X :=
by rw [derivative_C_mul_X_pow]; rw [ nat.cast_two]; rw [ pow_one]
...
by rw [derivative_X_pow]; rw [ nat.cast_two]; rw [ pow_one]
...
by rw [eq_C_of_nat_degree_eq_zero hp]; rw [ derivative_C]
...
by rw [derivative_add]; rw [ derivative_X]; rw [ derivative_C]; rw [ add_zero]
...
show p.derivative.coeff n = 0, by rw [coeff_derivative]; rw [ h1]; rw [ zero_mul]
...
if H : p = 0 then le_of_eq $ by rw [H]; rw [ derivative_zero] else (degree_derivative_lt H).le
...
begin
 ...
 { rw [hp']; rw [ polynomial.nat_degree_zero],
 exact hp.bot_lt },
...
begin
 ...
 { rw [derivative_of_nat_degree_zero hp]; rw [ iterate_derivative_zero] },
...
begin
 ...
 rw [hm] at h2; rw [ ←leading_coeff] at h2; rw [ leading_coeff_eq_zero] at h2,
 exact hf h2
end
...
calc derivative (f * g) = f.sum (λ n a, g.sum (λ m b, (n + m) • (C (a * b) * X ^ ((n + m) - 1)))) :
 ...
 by cases n; cases m; simp_rw [add_smul, mul_smul_comm, smul_mul_assoc, X_pow_mul_assoc, ← mul_assoc, ← C_mul, mul_assoc, ← pow_add];
...
by { rw [← nsmul_eq_mul']; rw [ smul_eq_zero], simp only [nat.succ_ne_zero, false_or] }
...
begin
 ...
 assume h, rw [h] at hp; rw [ nat_degree_zero] at hp,
...
begin
 ...
 { intro m, rw [function.iterate_succ_apply']; rw [ coeff_derivative]; rw [ ih (m+1)]; rw [ ← nat.cast_add_one]; rw [ ← nsmul_eq_mul']; rw [ smul_smul]; rw [ mul_comm],
 apply congr_arg2,
 ...
 rw [set_eq]; rw [ prod_union]; rw [ prod_singleton],
 { rw [disjoint_singleton_right]; rw [ mem_Ico],
...
begin
 ...
 by rw [function.iterate_succ_apply']; rw [ coeff_derivative]; rw [ ih m.succ]; rw [ nat.succ_add]; rw [ nat.add_succ]
 ... = ((∏ i in range k, (m + k.succ - i)) * (m + 1)) • p.coeff (m + k.succ) :
 by rw [← nat.cast_add_one]; rw [ ← nsmul_eq_mul']; rw [ smul_smul]; rw [ mul_comm]
 ... = (∏ i in range k.succ, (m + k.succ - i)) • p.coeff (m + k.succ) :
 by rw [prod_range_succ]; rw [ add_tsub_assoc_of_le k.le_succ]; rw [ nat.succ_sub le_rfl]; rw [ tsub_self]
end
...
begin
 ...
 by rw [function.iterate_succ_apply']; rw [ IH]
 ... = ∑ (k : ℕ) in range n.succ,
 (n.choose k) • ((derivative^[n - k + 1] p) * (derivative^[k] q)) +
 ∑ (k : ℕ) in range n.succ,
 (n.choose k) • ((derivative^[n - k] p) * (derivative^[k + 1] q)) :
 by simp_rw [derivative_sum, derivative_smul, derivative_mul, function.iterate_succ_apply', smul_add, sum_add_distrib]
 ... = (∑ (k : ℕ) in range n.succ,
 ...
 by rw [sum_range_succ' _ n.succ]; rw [ nat.choose_zero_right]; rw [ tsub_zero],

 congr,
 refine (sum_range_succ' _ _).trans (congr_arg2 (+) _ _),
 { rw [sum_range_succ]; rw [ nat.choose_succ_self]; rw [ zero_smul]; rw [ add_zero],
 refine sum_congr rfl (λ k hk, _),
 rw mem_range at hk,
 congr,
 rw [tsub_add_eq_add_tsub (nat.succ_le_of_lt hk)]; rw [ nat.succ_sub_succ] },
 { rw [nat.choose_zero_right]; rw [ tsub_zero] },
end
...
nat.rec_on n (by rw [pow_one]; rw [ nat.cast_one]; rw [ C_1]; rw [ one_mul]; rw [ pow_zero]; rw [ one_mul]) $ λ n ih,
by rw [pow_succ']; rw [ derivative_mul]; rw [ ih]; rw [ nat.add_one]; rw [ mul_right_comm]; rw [ nat.cast_add n.succ]; rw [ C_add]; rw [ add_mul]; rw [ add_mul]; rw [ pow_succ']; rw [ ← mul_assoc]; rw [ nat.cast_one]; rw [ C_1]; rw [ one_mul]

theorem derivative_pow (p : R[X]) (n : ℕ) :
 (p ^ n).derivative = C ↑n * (p ^ (n - 1)) * p.derivative :=
nat.cases_on n (by rw [pow_zero]; rw [ derivative_one]; rw [ nat.cast_zero]; rw [ C_0]; rw [ zero_mul]; rw [ zero_mul]) $ λ n,
by rw [p.derivative_pow_succ n]; rw [ n.succ_sub_one]; rw [ n.cast_succ]

theorem derivative_sq (p : R[X]) : (p ^ 2).derivative = C 2 * p * p.derivative :=
by rw [derivative_pow_succ]; rw [ nat.cast_two]; rw [ pow_one]
...
begin
 induction k with k ih,
 { rw [function.iterate_zero_apply]; rw [ tsub_zero]; rw [ nat.desc_factorial_zero]; rw [ nat.cast_one]; rw [ one_mul] },
 { rw [function.iterate_succ_apply']; rw [ ih]; rw [ derivative_nat_cast_mul]; rw [ derivative_X_pow]; rw [ C_eq_nat_cast]; rw [ nat.succ_eq_add_one]; rw [ nat.desc_factorial_succ]; rw [ nat.sub_sub]; rw [ nat.cast_mul]; rw [ ←mul_assoc]; rw [ mul_comm ↑(nat.desc_factorial _ _)] },
end

lemma iterate_derivative_X_pow_eq_C_mul (n k : ℕ) :
 (derivative^[k] (X ^ n : R[X])) = C ↑(nat.desc_factorial n k) * X ^ (n - k) :=
by rw [iterate_derivative_X_pow_eq_nat_cast_mul n k]; rw [ C_eq_nat_cast]

lemma iterate_derivative_X_pow_eq_smul (n : ℕ) (k : ℕ) :
 (derivative^[k] (X ^ n : R[X])) = (nat.desc_factorial n k : R) • X ^ (n - k) :=
by rw [iterate_derivative_X_pow_eq_C_mul n k]; rw [ smul_eq_C_mul]

lemma derivative_X_add_C_pow (c : R) (m : ℕ) :
 ((X + C c) ^ m).derivative = C ↑m * (X + C c) ^ (m - 1) :=
by rw [derivative_pow]; rw [ derivative_X_add_C]; rw [ mul_one]

lemma derivative_X_add_C_sq (c : R) : ((X + C c) ^ 2).derivative = C 2 * (X + C c) :=
by rw [derivative_sq]; rw [ derivative_X_add_C]; rw [ mul_one]

lemma iterate_derivative_X_add_pow (n k : ℕ) (c : R) : derivative^[k] ((X + C c) ^ n) =
 ↑(∏ i in finset.range k, (n - i)) * (X + C c) ^ (n - k) :=
begin
 induction k with k IH,
 { rw [function.iterate_zero_apply]; rw [ finset.range_zero]; rw [ finset.prod_empty]; rw [ nat.cast_one]; rw [ one_mul]; rw [ tsub_zero] },
...
polynomial.induction_on p
 (λ r, by rw [eval₂_C]; rw [ derivative_C]; rw [ eval₂_zero]; rw [ zero_mul])
 (λ p₁ p₂ ih₁ ih₂, by rw [eval₂_add]; rw [ derivative_add]; rw [ ih₁]; rw [ ih₂]; rw [ derivative_add]; rw [ eval₂_add]; rw [ add_mul])
 (λ n r ih, by rw [pow_succ']; rw [ ← mul_assoc]; rw [ eval₂_mul]; rw [ eval₂_X]; rw [ derivative_mul]; rw [ ih]; rw [ @derivative_mul _ _ _ X]; rw [ derivative_X]; rw [ mul_one]; rw [ eval₂_add]; rw [ @eval₂_mul _ _ _ _ X]; rw [ eval₂_X]; rw [ add_mul]; rw [ mul_right_comm])
...
begin
 ...
 rw [multiset.map_cons]; rw [ multiset.prod_cons]; rw [ derivative_mul]; rw [ multiset.map_cons _ i s]; rw [ multiset.sum_cons]; rw [ multiset.erase_cons_head]; rw [ mul_comm (f i).derivative],
 congr,
 rw [h]; rw [ ← add_monoid_hom.coe_mul_left]; rw [ (add_monoid_hom.mul_left (f i)).map_multiset_sum _]; rw [ add_monoid_hom.coe_mul_left],
...
by rw [derivative_sub]; rw [ derivative_X]; rw [ derivative_C]; rw [ sub_zero]
...
lemma derivative_X_sub_C_pow (c : R) (m : ℕ) :
 ((X - C c) ^ m).derivative = C ↑m * (X - C c) ^ (m - 1) :=
by rw [derivative_pow]; rw [ derivative_X_sub_C]; rw [ mul_one]

lemma derivative_X_sub_C_sq (c : R) : ((X - C c) ^ 2).derivative = C 2 * (X - C c) :=
by rw [derivative_sq]; rw [ derivative_X_sub_C]; rw [ mul_one]
