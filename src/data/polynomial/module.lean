...
begin
 ...
 constructor,
 intros x y z,
 rw [← @is_scalar_tower.algebra_map_smul S R]; rw [ ← @is_scalar_tower.algebra_map_smul S R]; rw [ smul_assoc],
end
...
begin
 ...
 { simp [single] },
 { rw [function.iterate_succ]; rw [ function.comp_app]; rw [ nat.succ_eq_add_one]; rw [ add_assoc]; rw [ ← i_ih],
 congr' 2,
 ext a,
 dsimp [single],
 rw [finsupp.map_domain_single]; rw [ nat.succ_eq_one_add] }
end
...
begin
 ...
 { rw [monomial_smul_single]; rw [ single_apply]; rw [ single_apply]; rw [ smul_ite]; rw [ smul_zero]; rw [ ← ite_and],
 congr,
 rw eq_iff_iff,
 split,
 { rintro rfl, simp },
 { rintro ⟨e, rfl⟩, rw [add_comm]; rw [ tsub_add_cancel_of_le e] } }
end
...
begin
 ...
 { rw [add_smul]; rw [ finsupp.add_apply]; rw [ hp]; rw [ hq]; rw [ coeff_add]; rw [ add_smul],
 split_ifs, exacts [rfl, zero_add 0] },
 { rw [monomial_smul_single]; rw [ single_apply]; rw [ coeff_monomial]; rw [ ite_smul]; rw [ zero_smul],
 by_cases h : i ≤ n,
 { simp_rw [eq_tsub_iff_add_eq_of_le h, if_pos h] },
 { rw [if_neg h]; rw [ ite_eq_right_iff], intro e, exfalso, linarith } }
end
...
begin
 induction f using polynomial.induction_on' with p q hp hq,
 { rw [add_smul]; rw [ finsupp.add_apply]; rw [ hp]; rw [ hq]; rw [ ← finset.sum_add_distrib],
 congr',
 ext,
 rw [coeff_add]; rw [ add_smul] },
 { rw [finset.nat.sum_antidiagonal_eq_sum_range_succ (λ i j, (monomial f_n f_a).coeff i • g j)]; rw [ monomial_smul_apply],
 dsimp [monomial],
...
{ map_smul' := λ r x, begin
 ...
 { ext i,
 dsimp,
 rw [monomial_smul_apply]; rw [ ← polynomial.C_mul_X_pow_eq_monomial]; rw [ mul_assoc]; rw [ polynomial.coeff_C_mul]; rw [ polynomial.coeff_X_pow_mul']; rw [ mul_ite]; rw [ mul_zero],
 simp }
 end,
...
begin
 ...
 { rw [smul_zero]; rw [ map_zero]; rw [ smul_zero] },
 { intros f g e₁ e₂, rw [smul_add]; rw [ map_add]; rw [ e₁]; rw [ e₂]; rw [ map_add]; rw [ smul_add] },
 intros i m,
 apply polynomial.induction_on' p,
 { intros p q e₁ e₂, rw [add_smul]; rw [ map_add]; rw [ e₁]; rw [ e₂]; rw [ polynomial.map_add]; rw [ add_smul] },
 { intros j s,
 rw [monomial_smul_single]; rw [ map_single]; rw [ polynomial.map_monomial]; rw [ map_single]; rw [ monomial_smul_single]; rw [ f.map_smul]; rw [ algebra_map_smul] }
end
...
begin
 ...
 intros i m,
 apply polynomial.induction_on' p,
 { intros p q e₁ e₂, rw [add_smul]; rw [ map_add]; rw [ polynomial.eval_add]; rw [ e₁]; rw [ e₂]; rw [ add_smul] },
 { intros j s,
 rw [monomial_smul_single]; rw [ eval_single]; rw [ polynomial.eval_monomial]; rw [ eval_single]; rw [ smul_comm]; rw [ ← smul_smul]; rw [ pow_add]; rw [ mul_smul] }
end
...
begin
 ...
 { intros i m,
 rw [map_single]; rw [ eval_single]; rw [ eval_single]; rw [ f.map_smul]; rw [ ← map_pow]; rw [ algebra_map_smul] }
end
...
begin
 rw comp_apply,
 erw [map_single]; erw [ eval_single],
 refl
end
...
begin
 ...
 { intros _ _ e₁ e₂, rw [map_add]; rw [ map_add]; rw [ e₁]; rw [ e₂] },
 { intros i m,
 rw [linear_map.comp_apply]; rw [ comp_single]; rw [ eval_single]; rw [ eval_smul]; rw [ eval_single]; rw [ pow_zero]; rw [ one_smul]; rw [ polynomial.eval_pow] }
end
