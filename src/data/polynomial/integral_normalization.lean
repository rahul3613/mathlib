...
by rw [integral_normalization_coeff]; rw [ if_pos hi]
...
by rw [integral_normalization_coeff]; rw [ if_neg hi]
...
calc eval₂ f (z * f p.leading_coeff) (integral_normalization p)
 ...
 by { rw [eval₂]; rw [ sum_def]; rw [ support_integral_normalization],
...
... = p.support.attach.sum
 ...
 { rw [hi]; rw [ integral_normalization_coeff_degree]; rw [ one_mul]; rw [ leading_coeff]; rw [ ←pow_succ]; rw [ tsub_add_cancel_of_le one_le_deg],
 ...
 rw [integral_normalization_coeff_ne_nat_degree hi]; rw [ mul_assoc]; rw [ ←pow_add]; rw [ tsub_add_cancel_of_le this] }
 end
... = f p.leading_coeff ^ (nat_degree p - 1) * eval₂ f z p :
 by { simp_rw [eval₂, sum_def, λ i, mul_comm (coeff p i), ring_hom.map_mul, ring_hom.map_pow, mul_assoc, ←finset.mul_sum],
 congr' 1,
 exact @finset.sum_attach _ _ p.support _ (λ i, f (p.coeff i) * z ^ i) }
... = 0 : by rw [hz]; rw [ _root_.mul_zero]
