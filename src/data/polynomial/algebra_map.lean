...
{ smul_def' := λ r p, to_finsupp_injective $ begin
 ...
 rw [to_finsupp_smul]; rw [ to_finsupp_mul]; rw [ to_finsupp_C],
...
by rw [← aeval_def]; rw [ aeval_alg_hom]; rw [ aeval_X_left]; rw [ alg_hom.comp_id]
...
begin
 conv_rhs {rw [aeval_def]; rw [ ← eval_map]},
...
by rw [← algebra.map_top]; rw [ ← adjoin_X]; rw [ alg_hom.map_adjoin]; rw [ set.image_singleton]; rw [ aeval_X]
...
begin
 ...
 { rw [eval] at dvd_eval; rw [ eval₂] at dvd_eval; rw [ sum] at dvd_eval,
...
begin
 ...
 rw [coeff_mul_X_sub_C]; rw [ sub_mul]; rw [ mul_assoc]; rw [ ←pow_succ], },
 simp [sum_range_sub', coeff_monomial],
end

theorem not_is_unit_X_sub_C [nontrivial R] (r : R) : ¬ is_unit (X - C r) :=
λ ⟨⟨_, g, hfg, hgf⟩, rfl⟩, zero_ne_one' R $ by erw [← eval_mul_X_sub_C]; erw [ hgf]; erw [ eval_one]
