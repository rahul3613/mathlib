...
begin
 classical,
 by_cases ha : a = 0,
 { rw [ha]; rw [ monomial_zero_right]; rw [ mirror_zero] },
 { rw [mirror]; rw [ reverse]; rw [ nat_degree_monomial n a]; rw [ if_neg ha]; rw [ nat_trailing_degree_monomial ha]; rw [ ← C_mul_X_pow_eq_monomial]; rw [ reflect_C_mul_X_pow]; rw [ rev_at_le (le_refl n)]; rw [ tsub_self]; rw [ pow_zero]; rw [ mul_one] },
end
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ mirror_zero] },
 nontriviality R,
 rw [mirror]; rw [ nat_degree_mul']; rw [ reverse_nat_degree]; rw [ nat_degree_X_pow]; rw [ tsub_add_cancel_of_le p.nat_trailing_degree_le_nat_degree],
 rwa [leading_coeff_X_pow]; rwa [ mul_one]; rwa [ reverse_leading_coeff]; rwa [ ne]; rwa [ trailing_coeff_eq_zero]
end
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ mirror_zero] },
 { rw [mirror]; rw [ nat_trailing_degree_mul_X_pow ((mt reverse_eq_zero.mp) hp)]; rw [ reverse_nat_trailing_degree]; rw [ zero_add] },
end
...
begin
 ...
 { rw [rev_at_le h1]; rw [ coeff_eq_zero_of_lt_nat_trailing_degree],
 exact (tsub_lt_iff_left h1).mpr (nat.add_lt_add_right h2 _) },
 { rw [←rev_at_fun_eq]; rw [ rev_at_fun]; rw [ if_neg h1]; rw [ coeff_eq_zero_of_nat_degree_lt h2] } },
 rw not_lt at h2,
 rw [rev_at_le (h2.trans (nat.le_add_right _ _))],
 by_cases h3 : p.nat_trailing_degree ≤ n,
 { rw [←tsub_add_eq_add_tsub h2]; rw [ ←tsub_tsub_assoc h2 h3]; rw [ mirror]; rw [ coeff_mul_X_pow']; rw [ if_pos h3]; rw [ coeff_reverse]; rw [ rev_at_le (tsub_le_self.trans h2)] },
 rw not_le at h3,
...
begin
 ...
 rw [tsub_le_iff_tsub_le]; rw [ add_comm]; rw [ add_tsub_cancel_right]; rw [ ←mirror_nat_trailing_degree],
 exact nat_trailing_degree_le_of_ne_zero hp },
 { exact λ n₁ n₂ hn₁ hp₁ hn₂ hp₂ h, by rw [←@rev_at_invol _ n₁]; rw [ h]; rw [ rev_at_invol] },
 { intros n hn hp,
 ...
 rwa [coeff_mirror]; rwa [ rev_at_invol] } },
 { exact λ n hn hp, p.coeff_mirror n },
end

lemma mirror_mirror : p.mirror.mirror = p :=
polynomial.ext (λ n, by rw [coeff_mirror]; rw [ coeff_mirror]; rw [ mirror_nat_degree]; rw [ mirror_nat_trailing_degree]; rw [ rev_at_invol])
...
⟨λ h, by rw [←p.mirror_mirror]; rw [ h]; rw [ mirror_zero], λ h, by rw [h]; rw [ mirror_zero]⟩

variables (p q)

@[simp] lemma mirror_trailing_coeff : p.mirror.trailing_coeff = p.leading_coeff :=
by rw [leading_coeff]; rw [ trailing_coeff]; rw [ mirror_nat_trailing_degree]; rw [ coeff_mirror]; rw [ rev_at_le (nat.le_add_left _ _)]; rw [ add_tsub_cancel_right]

@[simp] lemma mirror_leading_coeff : p.mirror.leading_coeff = p.trailing_coeff :=
by rw [←p.mirror_mirror]; rw [ mirror_trailing_coeff]; rw [ p.mirror_mirror]
...
begin
 ...
 rw [coeff_mirror]; rw [ ←rev_at_le (finset.mem_range_succ_iff.mp hn)]; rw [ rev_at_invol]; rw [ ←sq],
end
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ zero_mul]; rw [ nat_degree_zero]; rw [ mul_zero] },
 rw [nat_degree_mul hp (mt mirror_eq_zero.mp hp)]; rw [ mirror_nat_degree]; rw [ two_mul],
end
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ zero_mul]; rw [ nat_trailing_degree_zero]; rw [ mul_zero] },
 rw [nat_trailing_degree_mul hp (mt mirror_eq_zero.mp hp)]; rw [ mirror_nat_trailing_degree]; rw [ two_mul],
end
...
by rw [mirror]; rw [ mirror]; rw [ reverse_neg]; rw [ nat_trailing_degree_neg]; rw [ neg_mul_eq_neg_mul]
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ zero_mul]; rw [ mirror_zero]; rw [ zero_mul] },
 by_cases hq : q = 0,
 { rw [hq]; rw [ mul_zero]; rw [ mirror_zero]; rw [ mul_zero] },
 rw [mirror]; rw [ mirror]; rw [ mirror]; rw [ reverse_mul_of_domain]; rw [ nat_trailing_degree_mul hp hq]; rw [ pow_add],
 rw [mul_assoc]; rw [ ←mul_assoc q.reverse],
 conv_lhs { congr, skip, congr, rw [←X_pow_mul] },
 repeat { rw [mul_assoc], },
end

lemma mirror_smul (a : R) : (a • p).mirror = a • p.mirror :=
by rw [←C_mul']; rw [ ←C_mul']; rw [ mirror_mul_of_domain]; rw [ mirror_C]
...
begin
 ...
 { exact or.inr (h3 h h_dvd_f (by rwa [← neg_eq_iff_eq_neg.mpr hk]; rwa [ mirror_neg]; rwa [ dvd_neg])) },
 { exact or.inl (h3 g g_dvd_f (by rwa ← hk)) },
 { exact or.inl (h3 g g_dvd_f (by rwa [← neg_eq_iff_eq_neg.mpr hk]; rwa [ dvd_neg])) } },
end
