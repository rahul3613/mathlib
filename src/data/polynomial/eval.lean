...
by rw [← C_1]; rw [ eval₂_C]; rw [ f.map_one]

@[simp] lemma eval₂_bit0 : (bit0 p).eval₂ f x = bit0 (p.eval₂ f x) :=
by rw [bit0]; rw [ eval₂_add]; rw [ bit0]

@[simp] lemma eval₂_bit1 : (bit1 p).eval₂ f x = bit1 (p.eval₂ f x) :=
by rw [bit1]; rw [ eval₂_add]; rw [ eval₂_bit0]; rw [ eval₂_one]; rw [ bit1]
...
polynomial.induction_on' p (λ p q hp hq, by simp [hp, hq])
 (λ n x, by rw [eval₂_monomial]; rw [ ← smul_X_eq_monomial]; rw [ C_mul'])
...
begin
 ...
 { rw [n.cast_succ]; rw [ eval₂_add]; rw [ ih]; rw [ eval₂_one]; rw [ n.cast_succ] }
end
...
by rw [← smul_one_smul R s p]; rw [ eval]; rw [ eval₂_smul]; rw [ ring_hom.id_apply]; rw [ smul_one_mul]
...
by rw [←C_eq_nat_cast]; rw [ eval_C_mul]
...
by rwa [is_root]; rwa [ eval]; rwa [ eval₂_eq_zero_of_dvd_of_eval₂_eq_zero _ _ hpq]
...
@[simp] lemma nat_cast_comp {n : ℕ} : (n : R[X]).comp p = n := by rw [←C_eq_nat_cast]; rw [ C_comp]

@[simp] lemma comp_zero : p.comp (0 : R[X]) = C (p.eval 0) := by rw [← C_0]; rw [ comp_C]

@[simp] lemma zero_comp : comp (0 : R[X]) p = 0 := by rw [← C_0]; rw [ C_comp]

@[simp] lemma comp_one : p.comp 1 = C (p.eval 1) := by rw [← C_1]; rw [ comp_C]

@[simp] lemma one_comp : comp (1 : R[X]) p = 1 := by rw [← C_1]; rw [ C_comp]
...
by rw [←C_eq_nat_cast]; rw [ C_mul_comp]; rw [ C_eq_nat_cast]
...
by rw [← smul_one_smul R s p]; rw [ comp]; rw [ comp]; rw [ eval₂_smul]; rw [ ← smul_eq_C_mul]; rw [ smul_assoc]; rw [ one_smul]
...
begin
 dsimp only [map],
 rw [eval₂_monomial]; rw [ ← C_mul_X_pow_eq_monomial], refl,
end
...
by { rw [map]; rw [ eval₂_mul_noncomm], exact λ k, (commute_X _).symm }

@[simp] protected lemma map_smul (r : R) : (r • p).map f = f r • p.map f :=
by rw [map]; rw [ eval₂_smul]; rw [ ring_hom.comp_apply]; rw [ C_mul']
...
begin
 rw [map]; rw [ eval₂]; rw [ coeff_sum]; rw [ sum],
 conv_rhs { rw [← sum_C_mul_X_pow_eq p]; rw [ coeff_sum]; rw [ sum]; rw [ ring_hom.map_sum], },
...
λ p q h, ext $ λ m, hf $ by rw [← coeff_map f]; rw [ ← coeff_map f]; rw [ h]
...
λ p, polynomial.induction_on' p
 ...
 in ⟨p' + q', by rw [polynomial.map_add f]; rw [ hp']; rw [ hq']⟩)
 (λ n s, let ⟨r, hr⟩ := hf s in ⟨monomial n r, by rw [map_monomial f]; rw [ hr]⟩)
...
begin
 ...
 rw [coeff_map]; rw [ nat_degree_map_of_leading_coeff_ne_zero f hf],
end
...
begin
 split,
 { rintro ⟨p, rfl⟩ n, rw [coe_map_ring_hom]; rw [ coeff_map], exact set.mem_range_self _ },
 { intro h, rw p.as_sum_range_C_mul_X_pow,
 refine (map_ring_hom f).srange.sum_mem _,
 intros i hi,
 rcases h i with ⟨c, hc⟩,
 use [C c * X^i],
 rw [coe_map_ring_hom]; rw [ polynomial.map_mul]; rw [ map_C]; rw [ hc]; rw [ polynomial.map_pow]; rw [ map_X] }
end
...
by rw [eval₂_eq_eval_map]; rw [ eval₂_eq_eval_map]; rw [ map_map]
...
by rw [←eval₂_map]; rw [ eval₂_at_apply]; rw [ eval_map]
...
by rw [comp]; rw [ p.as_sum_range]; simp [eval₂_finset_sum, eval₂_pow]
...
begin
 induction k with k IH,
 { simp },
 { rw [function.iterate_succ_apply']; rw [ function.iterate_succ_apply']; rw [ eval₂_comp]; rw [ IH] }
end
...
λ H, by rw [is_root]; rw [ eval_mul]; rw [ is_root.def.1 H]; rw [ mul_zero]

lemma root_mul_right_of_is_root {p : R[X]} (q : R[X]) :
 is_root p a → is_root (p * q) a :=
λ H, by rw [is_root]; rw [ eval_mul]; rw [ is_root.def.1 H]; rw [ zero_mul]
...
lemma support_map_of_injective [semiring R] [semiring S]
 ...
 (map f p).support = p.support :=
by simp_rw [finset.ext_iff, mem_support_iff, coeff_map, ←map_zero f, hf.ne_iff, iff_self, forall_const]
...
lemma is_root.map {f : R →+* S} {x : R} {p : R[X]} (h : is_root p x) :
 is_root (p.map f) (f x) :=
by rw [is_root]; rw [ eval_map]; rw [ eval₂_hom]; rw [ h.eq_zero]; rw [ f.map_zero]

lemma is_root.of_map {R} [comm_ring R] {f : R →+* S} {x : R} {p : R[X]}
 (h : is_root (p.map f) (f x)) (hf : function.injective f) : is_root p x :=
by rwa [is_root]; rwa [ ←(injective_iff_map_eq_zero' f).mp hf]; rwa [ ←eval₂_hom]; rwa [ ←eval_map]
...
by rw [eq_neg_iff_add_eq_zero]; rw [ ←eval₂_add]; rw [ add_left_neg]; rw [ eval₂_zero]

@[simp] lemma eval₂_sub {S} [ring S] (f : R →+* S) {x : S} :
 (p - q).eval₂ f x = p.eval₂ f x - q.eval₂ f x :=
by rw [sub_eq_add_neg]; rw [ eval₂_add]; rw [ eval₂_neg]; rw [ sub_eq_add_neg]
...
by rw [is_root.def]; rw [ eval_sub]; rw [ eval_X]; rw [ eval_C]; rw [ sub_eq_zero]; rw [ eq_comm]
