...
by rw [taylor_zero']; rw [ linear_map.id_apply]

@[simp] lemma taylor_one : taylor r (1 : R[X]) = C 1 :=
by rw [← C_1]; rw [ taylor_C]
...
by rw [taylor_coeff]; rw [ hasse_deriv_zero]; rw [ linear_map.id_apply]

@[simp] lemma taylor_coeff_one : (taylor r f).coeff 1 = f.derivative.eval r :=
by rw [taylor_coeff]; rw [ hasse_deriv_one]
...
by rw [taylor_eval]; rw [ sub_add_cancel]
...
by rw [←comp_eq_sum_left]; rw [ sub_eq_add_neg]; rw [ ←C_neg]; rw [ ←taylor_apply]; rw [ taylor_taylor]; rw [ neg_add_self]; rw [ taylor_zero]
