...
begin
 ...
 rw [sub_eq_iff_eq_add.mp sub_eq]; rw [ mul_add]; rw [ ← add_assoc]; rw [ mod_by_monic_add_div _ hq]; rw [ add_comm]
end
...
begin
 ...
 ⟨by rw [mul_add]; rw [ add_left_comm]; rw [ add_assoc]; rw [ mod_by_monic_add_div _ hq]; rw [ ← add_assoc]; rw [ add_comm (q * _)]; rw [ mod_by_monic_add_div _ hq],
...
begin
 ...
 ⟨by rw [mul_smul_comm]; rw [ ← smul_add]; rw [ mod_by_monic_add_div p hq],
...
by rw [mod_by_monic_eq_sub_mul_div p hq]; rw [ _root_.map_sub]; rw [ _root_.map_mul]; rw [ hx]; rw [ zero_mul]; rw [ sub_zero]
...
{ eq_zero_or_eq_zero_of_mul_eq_zero := λ a b h, begin
 rw [← leading_coeff_eq_zero]; rw [ ← leading_coeff_eq_zero],
 refine eq_zero_or_eq_zero_of_mul_eq_zero _,
 rw [← leading_coeff_zero]; rw [ ← leading_coeff_mul]; rw [ h],
 end }

lemma nat_degree_mul (hp : p ≠ 0) (hq : q ≠ 0) : nat_degree (p * q) =
 nat_degree p + nat_degree q :=
by rw [← with_bot.coe_eq_coe]; rw [ ← degree_eq_nat_degree (mul_ne_zero hp hq)]; rw [ with_bot.coe_add]; rw [ ← degree_eq_nat_degree hp]; rw [ ← degree_eq_nat_degree hq]; rw [ degree_mul]

lemma trailing_degree_mul : (p * q).trailing_degree = p.trailing_degree + q.trailing_degree :=
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ zero_mul]; rw [ trailing_degree_zero]; rw [ top_add] },
 by_cases hq : q = 0,
 { rw [hq]; rw [ mul_zero]; rw [ trailing_degree_zero]; rw [ add_top] },
 rw [trailing_degree_eq_nat_trailing_degree hp]; rw [ trailing_degree_eq_nat_trailing_degree hq]; rw [ trailing_degree_eq_nat_trailing_degree (mul_ne_zero hp hq)]; rw [ nat_trailing_degree_mul hp hq]; rw [ with_top.coe_add],
end
...
then if hn0 : n = 0 then by simp [hp0, hn0]
 else by rw [hp0]; rw [ zero_pow (nat.pos_of_ne_zero hn0)]; simp
else nat_degree_pow'
 (by rw [← leading_coeff_pow]; rw [ ne.def]; rw [ leading_coeff_eq_zero]; exact pow_ne_zero _ hp0)

lemma degree_le_mul_left (p : R[X]) (hq : q ≠ 0) : degree p ≤ degree (p * q) :=
by classical; exact
if hp : p = 0 then by simp only [hp, zero_mul, le_refl]
else by rw [degree_mul]; rw [ degree_eq_nat_degree hp]; rw [ degree_eq_nat_degree hq];
...
begin
 ...
 norm_cast,
 rw [← nat_degree_mul hp₁ hq₂]; rw [ ← nat_degree_mul hp₂ hq₁]; rw [ h_eq]
end
...
begin
 ...
 rw [hq] at this; rw [ nat_degree_one] at this; rw [ eq_comm] at this; rw [ add_eq_zero_iff] at this,
 exact this.1,
end
...
by rw [bit0_eq_two_mul]; rw [ degree_mul]; rw [ (by simp : (2 : R[X]) = C 2),
...
begin
 rw [bit1]; rw [ degree_add_eq_left_of_degree_lt]; rw [ degree_bit0_eq],
 rwa [degree_one]; rwa [ degree_bit0_eq]
end
...
begin
 ...
 { rwa [monic]; rwa [ leading_coeff_mul]; rwa [ leading_coeff_C]; rwa [ ←leading_coeff_mul]; rwa [ mul_comm]; rwa [ ←hfg]; rwa [ ←monic] },
 { rwa [monic]; rwa [ leading_coeff_mul]; rwa [ leading_coeff_C]; rwa [ ←leading_coeff_mul]; rwa [ ←hfg]; rwa [ ←monic] },
 { rw [mul_mul_mul_comm]; rw [ ←C_mul]; rw [ ←leading_coeff_mul]; rw [ ←hfg]; rw [ hp.leading_coeff]; rw [ C_1]; rw [ mul_one]; rw [ mul_comm]; rw [ ←hfg] },
end
...
begin
 ...
 rw [ha.nat_degree_eq_zero_iff_eq_one]; rw [ hb.nat_degree_eq_zero_iff_eq_one],
end
...
begin
 ...
 { rw [hf.nat_degree_mul hg]; rw [ add_le_add_iff_right],
...
begin
 ...
 rw [hm.irreducible_iff_nat_degree']; rw [ and_iff_right]; rw [ hnd],
 push_neg, split,
 { rintros ⟨a, b, ha, hb, rfl, hdb|⟨⟨⟩⟩⟩,
 have hda := hnd, rw [ha.nat_degree_mul hb] at hda; rw [ hdb] at hda,
 ...
 rw [p.as_sum_range_C_mul_X_pow]; rw [ hnd]; rw [ finset.sum_range_succ]; rw [ finset.sum_range_succ]; rw [ finset.sum_range_one]; rw [ ← hnd]; rw [ hm.coeff_nat_degree]; rw [ hnd]; rw [ hmul]; rw [ hadd]; rw [ C_mul]; rw [ C_add]; rw [ C_1],
 ring },
...
by rw [← (le_root_multiplicity_iff p0).not]; rw [ not_le]; rw [ nat.lt_add_one_iff]
...
begin
 ...
 rw [← hu] at hq; rw [ leading_coeff_mul] at hq; rw [ hp] at hq; rw [ one_mul] at hq; rw [ leading_coeff_C] at hq,
 rwa [hq] at hu; rwa [ C_1] at hu; rwa [ mul_one] at hu,
...
begin
 ...
 rw [root_multiplicity_eq_multiplicity (p * q)]; rw [ dif_neg hpq]; rw [ root_multiplicity_eq_multiplicity p]; rw [ dif_neg hp]; rw [ root_multiplicity_eq_multiplicity q]; rw [ dif_neg hq]; rw [ multiplicity.mul' (prime_X_sub_C x)],
end

lemma root_multiplicity_X_sub_C_self {x : R} :
 root_multiplicity x (X - C x) = 1 :=
by classical; rw [root_multiplicity_eq_multiplicity]; rw [ dif_neg (X_sub_C_ne_zero x)]; rw [ multiplicity.get_multiplicity_self]
...
then
 ...
 λ h, by rw [← mul_div_by_monic_eq_iff_is_root.2 hx] at hp; rw [ h] at hp; rw [ mul_zero] at hp; exact hp rfl,
 ...
 rw [degree_X_sub_C]; rw [ degree_eq_nat_degree hp],
 ...
 ... ≤ degree p :
 by rw [← degree_add_div_by_monic (monic_X_sub_C x) hdeg]; rw [ degree_X_sub_C]; rw [ add_comm];
 exact add_le_add (le_refl (1 : with_bot ℕ)) htd,
 begin
 assume a,
 conv_rhs { rw ← mul_div_by_monic_eq_iff_is_root.mpr hx },
 rw [root_multiplicity_mul (mul_ne_zero (X_sub_C_ne_zero x) hdiv0)]; rw [ root_multiplicity_X_sub_C]; rw [ ← htr a],
 split_ifs with ha,
 { rw [ha]; rw [ count_cons_self]; rw [ nat.succ_eq_add_one]; rw [ add_comm] },
 { rw [count_cons_of_ne ha]; rw [ zero_add] },
 end⟩
else
 ⟨0, (degree_eq_nat_degree hp).symm ▸ with_bot.coe_le_coe.2 (nat.zero_le _),
 by { intro a, rw [count_zero]; rw [ root_multiplicity_eq_zero (not_exists.mp h a)] }⟩
...
... = degree p : by rw [sub_eq_add_neg]; rw [ ← C_neg]; exact degree_add_C hp0
...
by classical; rw [← count_pos]; rw [ count_roots p]; rw [ root_multiplicity_pos']
...
by classical; exact (multiset.ext.mpr $ λ r,
 by rw [count_add]; rw [ count_roots]; rw [ count_roots]; rw [ count_roots]; rw [ root_multiplicity_mul hpq])
...
by rw [mem_roots']; rw [ is_root.def]; rw [ sub_ne_zero]; rw [ eval_sub]; rw [ sub_eq_zero]; rw [ eval_C]
...
begin
 classical,
 ext s,
 rw [count_roots]; rw [ root_multiplicity_X_sub_C]; rw [ count_singleton],
end

@[simp] lemma roots_X : roots (X : R[X]) = {0} := by rw [← roots_X_sub_C]; rw [ C_0]; rw [ sub_zero]

@[simp] lemma roots_C (x : R) : (C x).roots = 0 :=
by classical; exact if H : x = 0 then by rw [H]; rw [ C_0]; rw [ roots_zero] else multiset.ext.mpr $ λ r,
by rw [count_roots]; rw [ count_zero]; rw [ root_multiplicity_eq_zero (not_is_root_C _ _ H)]
...
by rw [smul_eq_C_mul]; rw [ roots_C_mul _ ha]

lemma roots_list_prod (L : list R[X]) :
 ((0 : R[X]) ∉ L) → L.prod.roots = (L : multiset R[X]).bind roots :=
list.rec_on L (λ _, roots_one) $ λ hd tl ih H,
begin
 rw [list.mem_cons_iff] at H; rw [ not_or_distrib] at H,
 rw [list.prod_cons]; rw [ roots_mul (mul_ne_zero (ne.symm H.1) $ list.prod_ne_zero H.2)]; rw [ ← multiset.cons_coe]; rw [ multiset.cons_bind]; rw [ ih H.2]
end
...
begin
 ...
 { rw [pow_zero]; rw [ roots_one]; rw [ zero_smul]; rw [ empty_eq_zero] },
 { rcases eq_or_ne p 0 with rfl | hp,
 { rw [zero_pow n.succ_pos]; rw [ roots_zero]; rw [ smul_zero] },
 { rw [pow_succ']; rw [ roots_mul (mul_ne_zero (pow_ne_zero _ hp) hp)]; rw [ ihn]; rw [ nat.succ_eq_add_one]; rw [ add_smul]; rw [ one_smul] } }
end

lemma roots_X_pow (n : ℕ) : (X ^ n : R[X]).roots = n • {0} := by rw [roots_pow]; rw [ roots_X]

lemma roots_C_mul_X_pow (ha : a ≠ 0) (n : ℕ) : (C a * X ^ n).roots = n • {0} :=
by rw [roots_C_mul _ ha]; rw [ roots_X_pow]

@[simp] lemma roots_monomial (ha : a ≠ 0) (n : ℕ) : (monomial n a).roots = n • {0} :=
by rw [← C_mul_X_pow_eq_monomial]; rw [ roots_C_mul_X_pow ha]
...
begin
 rw [nat_degree_multiset_prod_of_monic]; rw [ multiset.map_map],
...
by rw [nth_roots]; rw [ mem_roots (X_pow_sub_C_ne_zero hn a)]; rw [ is_root.def]; rw [ eval_sub]; rw [ eval_C]; rw [ eval_pow]; rw [ eval_X]; rw [ sub_eq_zero]
...
if hn : n = 0 then
 ...
 (by { rw [hn]; rw [ pow_zero]; rw [ ← C_1]; rw [ ← ring_hom.map_sub ],
 exact degree_C_le }))
else by rw [← with_bot.coe_le_coe]; rw [ ← degree_X_pow_sub_C (nat.pos_of_ne_zero hn) a];
 exact card_roots (X_pow_sub_C_ne_zero (nat.pos_of_ne_zero hn) a)

@[simp]
lemma nth_roots_two_eq_zero_iff {r : R} : nth_roots 2 r = 0 ↔ ¬ is_square r :=
by simp_rw [is_square_iff_exists_sq, eq_zero_iff_forall_not_mem, mem_nth_roots (by norm_num : 0 < 2), ← not_exists, eq_comm]
...
by rw [nth_roots_finset]; rw [ mem_to_finset]; rw [ mem_nth_roots h]
...
by rw [monic.def]; rw [ leading_coeff_comp h]; rw [ monic.def.1 hp]; rw [ monic.def.1 hq]; rw [ one_pow]; rw [ one_mul]
...
lemma units_coeff_zero_smul (c : R[X]ˣ) (p : R[X]) :
 (c : R[X]).coeff 0 • p = c * p :=
by rw [←polynomial.C_mul']; rw [ ←polynomial.eq_C_of_degree_eq_zero (degree_coe_units c)]
...
begin
 ...
 { rw [←mul_eq_zero]; rw [ ←nat_degree_comp]; rw [ h]; rw [ nat_degree_zero] },
 replace key := or.imp eq_C_of_nat_degree_eq_zero eq_C_of_nat_degree_eq_zero key,
 cases key,
 { rw [key] at h; rw [ C_comp] at h,
 exact or.inl (key.trans h) },
 { rw [key] at h; rw [ comp_C] at h; rw [ C_eq_zero] at h,
 exact or.inr ⟨h, key⟩ }, },
 { exact λ h, or.rec (λ h, by rw [h]; rw [ zero_comp]) (λ h, by rw [h.2]; rw [ comp_C]; rw [ h.1]; rw [ C_0]) h },
end
...
@[simp] lemma root_set_C [comm_ring S] [is_domain S] [algebra T S] (a : T) :
 (C a).root_set S = ∅ :=
by classical; rw [root_set_def]; rw [ map_C]; rw [ roots_C]; rw [ multiset.to_finset_zero]; rw [ finset.coe_empty]

@[simp] lemma root_set_zero (S) [comm_ring S] [is_domain S] [algebra T S] :
 (0 : T[X]).root_set S = ∅ :=
by rw [← C_0]; rw [ root_set_C]
...
by rw [root_set]; rw [ finset.mem_coe]; rw [ mem_to_finset]; rw [ mem_roots']; rw [ is_root.def]; rw [ ← eval₂_eq_eval_map]; rw [ aeval_def]
...
by rw [mem_root_set']; rw [ (map_injective _ (no_zero_smul_divisors.algebra_map_injective T S)).ne_iff' (polynomial.map_zero _)]
...
begin
 ...
 rw [← leading_coeff]; rw [ ne.def]; rw [ leading_coeff_eq_zero],
...
this.elim
 ...
 (λ hgu, by rw [hg]; rw [ degree_mul]; rw [ degree_X_sub_C]; rw [ degree_eq_zero_of_is_unit hgu]; rw [ add_zero])
...
begin
 ...
 rw [leading_coeff_add_of_degree_lt]; rw [ leading_coeff_monic_mul hmonic],
 rw [degree_mul' h]; rw [ degree_add_div_by_monic hmonic hdegree],
...
begin
 ...
 { rw [degree_eq_bot.mp $ (nat.with_bot.lt_zero_iff _).mp hd]; rw [ zero_div_by_monic] },
 refine leading_coeff_div_by_monic_of_monic (monic_X_sub_C a) _,
 rwa [degree_X_sub_C]; rwa [ nat.with_bot.one_le_iff_zero_lt]
end
...
begin
 ...
 rw [hr] at hdeg; rw [ nat_degree_mul'] at hdeg, swap,
 { rw [hp.leading_coeff]; rw [ one_mul]; rw [ leading_coeff_ne_zero], exact rzero },
 rw [mul_comm] at hr; rw [ @eq_C_of_nat_degree_eq_zero _ _ r] at hr,
 { convert hr, convert leading_coeff_C _ using 1, rw [hr]; rw [ leading_coeff_mul_monic hp] },
...
begin
 ...
 rw [hq.leading_coeff]; rw [ C_1]; rw [ one_mul],
end

lemma is_coprime_X_sub_C_of_is_unit_sub {R} [comm_ring R] {a b : R}
 (h : is_unit (a - b)) : is_coprime (X - C a) (X - C b) :=
⟨-C h.unit⁻¹.val, C h.unit⁻¹.val, by { rw [neg_mul_comm]; rw [ ← left_distrib]; rw [ neg_add_eq_sub]; rw [ sub_sub_sub_cancel_left]; rw [ ← C_sub]; rw [ ← C_mul], convert C_1, exact h.coe_inv_mul }⟩
...
⟨λ h, multiset.le_iff_count.2 $ λ r, begin
 rw [count_roots]; rw [ le_root_multiplicity_iff hp]; rw [ ← multiset.prod_replicate]; rw [ ← multiset.map_replicate (λ a, X - C a)]; rw [ ← multiset.filter_eq],
...
begin
 ...
 split,
 { conv_rhs { rw he },
 rw [monic_prod_multiset_X_sub_C.nat_degree_mul' hq]; rw [ nat_degree_multiset_prod_X_sub_C_eq_card] },
...
by { convert C_leading_coeff_mul_prod_multiset_X_sub_C hroots, rw [hp.leading_coeff]; rw [ C_1]; rw [ one_mul] }
...
begin
 ...
 rw [map_pow]; rw [ map_sub]; rw [ coe_map_ring_hom]; rw [ map_X]; rw [ map_C],
end
...
begin
 ...
 rw [le_root_multiplicity_iff hp0]; rw [ ← map_dvd_map f hf ((monic_X_sub_C a).pow _)]; rw [ polynomial.map_pow]; rw [ polynomial.map_sub]; rw [ map_X]; rw [ map_C],
...
begin
 rw [le_root_multiplicity_iff hmap]; rw [ ← multiset.prod_replicate]; rw [ ← multiset.map_replicate (λ a, X - C a)],
