...
by { rw [sub_eq_add_neg]; rw [ of_finsupp_add]; rw [ of_finsupp_neg], refl }
...
by { rw [sub_eq_add_neg]; rw [ ←to_finsupp_neg]; rw [ ←to_finsupp_add], refl }
...
by rw [←to_finsupp_zero]; rw [ to_finsupp_inj]

@[simp] lemma to_finsupp_eq_one {a : R[X]} : a.to_finsupp = 1 ↔ a = 1 :=
by rw [←to_finsupp_one]; rw [ to_finsupp_inj]
...
by rw [←of_finsupp_zero]; rw [ of_finsupp_inj]

@[simp] lemma of_finsupp_eq_one {a} : (⟨a⟩ : R[X]) = 1 ↔ a = 1 :=
by rw [←of_finsupp_one]; rw [ of_finsupp_inj]
...
begin
 ...
 { rw [pow_succ]; rw [ ←ih]; rw [ ←monomial_one_one_eq_X]; rw [ monomial_mul_monomial]; rw [ add_comm]; rw [ one_mul], }
end
...
by rw [mul_assoc]; rw [ X_pow_mul]; rw [ ←mul_assoc]
...
by erw [monomial_mul_monomial]; erw [ mul_one]
...
by rw [X_mul]; rw [ monomial_mul_X]

@[simp]
lemma X_pow_mul_monomial (k n : ℕ) (r : R) : X^k * monomial n r = monomial (n+k) r :=
by rw [X_pow_mul]; rw [ monomial_mul_X_pow]
...
by { rw [← monomial_zero_one]; rw [ coeff_monomial], simp }
...
by rw [coeff_X]; rw [ if_neg hn.symm]
...
by rw [coeff_C]; rw [ if_neg h]

lemma C_mul_X_pow_eq_monomial : ∀ {n : ℕ}, C a * X ^ n = monomial n a
| 0 := mul_one _
| (n+1) := by rw [pow_succ']; rw [ ←mul_assoc]; rw [ C_mul_X_pow_eq_monomial]; rw [ X]; rw [ monomial_mul_monomial]; rw [ mul_one]

@[simp] lemma to_finsupp_C_mul_X_pow (a : R) (n : ℕ) :
 (C a * X ^ n).to_finsupp = finsupp.single n a :=
by rw [C_mul_X_pow_eq_monomial]; rw [ to_finsupp_monomial]

lemma C_mul_X_eq_monomial : C a * X = monomial 1 a := by rw [← C_mul_X_pow_eq_monomial]; rw [ pow_one]

@[simp] lemma to_finsupp_C_mul_X (a : R) : (C a * X).to_finsupp = finsupp.single 1 a :=
by rw [C_mul_X_eq_monomial]; rw [ to_finsupp_monomial]
...
begin
 apply top_unique,
 rw [← add_submonoid.map_equiv_top (to_finsupp_iso R).symm.to_add_equiv]; rw [ ← finsupp.add_closure_set_of_eq_single]; rw [ add_monoid_hom.map_mclosure],
...
by rw [←one_smul R p]; rw [ ←h]; rw [ zero_smul]

section fewnomials

lemma support_monomial (n) {a : R} (H : a ≠ 0) : (monomial n a).support = singleton n :=
by rw [←of_finsupp_single]; rw [ support]; rw [ finsupp.support_single_ne_zero _ H]

lemma support_monomial' (n) (a : R) : (monomial n a).support ⊆ singleton n :=
by { rw [←of_finsupp_single]; rw [ support], exact finsupp.support_single_subset }

lemma support_C_mul_X {c : R} (h : c ≠ 0) : (C c * X).support = singleton 1 :=
by rw [C_mul_X_eq_monomial]; rw [ support_monomial 1 h]
...
by rw [C_mul_X_pow_eq_monomial]; rw [ support_monomial n h]
...
begin
 ...
 { rw [pow_zero]; rw [ monomial_zero_one] },
 { rw [pow_succ']; rw [ hn]; rw [ X]; rw [ monomial_mul_monomial]; rw [ one_mul] },
end

@[simp] lemma to_finsupp_X_pow (n : ℕ) : (X ^ n).to_finsupp = finsupp.single n (1 : R) :=
by rw [X_pow_eq_monomial]; rw [ to_finsupp_monomial]

lemma smul_X_eq_monomial {n} : a • X ^ n = monomial n (a : R) :=
by rw [X_pow_eq_monomial]; rw [ smul_monomial]; rw [ smul_eq_mul]; rw [ mul_one]
...
by rw [X]; rw [ H]; rw [ monomial_zero_right]; rw [ support_zero]

lemma support_X (H : ¬(1 : R) = 0) : (X : R[X]).support = singleton 1 :=
by rw [← pow_one X]; rw [ support_X_pow H 1]
...
to_finsupp_injective $ begin
 rcases p,
 rw [to_finsupp_add]; rw [ to_finsupp_monomial]; rw [ to_finsupp_erase]; rw [ coeff],
...
by { rcases p, rw [←of_finsupp_neg]; rw [ coeff]; rw [ coeff]; rw [ finsupp.neg_apply] }

@[simp]
lemma coeff_sub (p q : R[X]) (n : ℕ) : coeff (p - q) n = coeff p n - coeff q n :=
by { rcases p, rcases q, rw [←of_finsupp_sub]; rw [ coeff]; rw [ coeff]; rw [ coeff]; rw [ finsupp.sub_apply] }

@[simp] lemma monomial_neg (n : ℕ) (a : R) : monomial n (-a) = -(monomial n a) :=
by rw [eq_neg_iff_add_eq_zero]; rw [ ←monomial_add]; rw [ neg_add_self]; rw [ monomial_zero_right]

@[simp] lemma support_neg {p : R[X]} : (-p).support = p.support :=
by { rcases p, rw [←of_finsupp_neg]; rw [ support]; rw [ support]; rw [ finsupp.support_neg] }
...
by rw [←rat.smul_one_eq_coe]; rw [ ←polynomial.smul_C]; rw [ C_1]; rw [ smul_one_mul]
