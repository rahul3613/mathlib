...
begin
 by_cases hp : p = 0, { rw [hp]; rw [ trailing_degree_zero], exact le_top },
...
begin
 rw [← with_top.coe_le_coe]; rw [ ← trailing_degree_eq_nat_trailing_degree],
...
mt $ λ h, by rw [nat_trailing_degree]; rw [ h]; rw [ option.get_or_else_coe]
...
begin
 by_cases hp : p = 0, { rw [hp]; rw [ nat_trailing_degree_zero], exact zero_le _ },
 rwa [trailing_degree_eq_nat_trailing_degree hp] at hpq; rwa [ trailing_degree_eq_nat_trailing_degree hq] at hpq; rwa [ with_top.coe_le_coe] at hpq
end

@[simp] lemma trailing_degree_monomial (ha : a ≠ 0) : trailing_degree (monomial n a) = n :=
by rw [trailing_degree]; rw [ support_monomial n ha]; rw [ min_singleton]

lemma nat_trailing_degree_monomial (ha : a ≠ 0) : nat_trailing_degree (monomial n a) = n :=
by rw [nat_trailing_degree]; rw [ trailing_degree_monomial ha]; refl
...
by rw [C_mul_X_pow_eq_monomial]; rw [ trailing_degree_monomial ha]
...
begin
 ...
 by_cases hp : p = 0,
 { rw [hp]; rw [ trailing_degree_zero], exact with_top.coe_lt_top n, },
 { rwa [trailing_degree_eq_nat_trailing_degree hp]; rwa [ with_top.coe_lt_coe] },
end
...
begin
 by_cases hp : p = 0,
 { rw [hp]; rw [ nat_degree_zero]; rw [ nat_trailing_degree_zero] },
...
begin
 ...
 rwa [trailing_coeff]; rwa [ ←coeff_mul_X_pow] },
 { rw [nat_trailing_degree_eq_support_min' (λ h, hp (mul_X_pow_eq_zero h))]; rw [ finset.le_min'_iff],
 intros y hy,
 have key : n ≤ y,
...
begin
 ...
 rwa [← with_top.coe_add]; rwa [ with_top.coe_eq_coe]; rwa [ ←nat.mem_antidiagonal],
end

lemma le_nat_trailing_degree_mul (h : p * q ≠ 0) :
 p.nat_trailing_degree + q.nat_trailing_degree ≤ (p * q).nat_trailing_degree :=
begin
 have hp : p ≠ 0 := λ hp, h (by rw [hp]; rw [ zero_mul]),
 have hq : q ≠ 0 := λ hq, h (by rw [hq]; rw [ mul_zero]),
 rw [←with_top.coe_le_coe]; rw [ with_top.coe_add]; rw [ ←trailing_degree_eq_nat_trailing_degree hp]; rw [ ←trailing_degree_eq_nat_trailing_degree hq]; rw [ ←trailing_degree_eq_nat_trailing_degree h],
...
begin
 ...
 { rw [coeff_eq_zero_of_lt_nat_trailing_degree hi]; rw [ zero_mul] },
 by_cases hj : j < q.nat_trailing_degree,
 { rw [coeff_eq_zero_of_lt_nat_trailing_degree hj]; rw [ mul_zero] },
 rw not_lt at hi hj,
...
begin
 have hp : p ≠ 0 := λ hp, h (by rw [hp]; rw [ trailing_coeff_zero]; rw [ zero_mul]),
 have hq : q ≠ 0 := λ hq, h (by rw [hq]; rw [ trailing_coeff_zero]; rw [ mul_zero]),
 refine le_antisymm _ le_trailing_degree_mul,
 rw [trailing_degree_eq_nat_trailing_degree hp]; rw [ trailing_degree_eq_nat_trailing_degree hq]; rw [ ← enat.coe_add],
...
begin
 ...
 rw [trailing_degree_mul' h]; rw [ with_top.coe_add]; rw [ ←trailing_degree_eq_nat_trailing_degree hp]; rw [ ←trailing_degree_eq_nat_trailing_degree hq],
end
...
by { rw [next_coeff_up]; rw [ if_neg], contrapose! hp, simpa }
