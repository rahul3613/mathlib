...
if h0 : p.comp q = 0 then by rw [h0]; rw [ nat_degree_zero]; exact nat.zero_le _
 ...
 ... = (n * nat_degree q : ℕ) :
 by rw [nat_degree_C]; rw [ with_bot.coe_zero]; rw [ zero_add]; rw [ ← with_bot.coe_nsmul]; rw [ nsmul_eq_mul]; simp
...
lt_of_not_ge $ λ hlt, begin
 ...
 rw [is_root] at h; rw [ this] at h; rw [ eval_C] at h,
...
begin
 ...
 rw [coeff_add]; rw [ nat_degree_le_iff_coeff_eq_zero.mp qn _ hm]; rw [ add_zero],
end
...
le_antisymm (nat_degree_C_mul_le a p) (calc
 ...
 ... = (C ai * (C a * p)).nat_degree : by rw [← C_1]; rw [ ← au]; rw [ ring_hom.map_mul]; rw [ ← mul_assoc]
...
le_antisymm (nat_degree_mul_C_le p a) (calc
 ...
 ... = ((p * C a) * C ai).nat_degree : by rw [← C_1]; rw [ ← au]; rw [ ring_hom.map_mul]; rw [ ← mul_assoc]
...
begin
 ...
 { rw [coeff_eq_zero_of_nat_degree_lt hn]; rw [ mul_zero],
 exact nat_degree_lt_coeff_mul (add_lt_add_left hn _) },
 { rw [coeff_eq_zero_of_nat_degree_lt hm]; rw [ zero_mul],
...
begin
 ...
 { simp },
 { rw [pow_succ']; rw [ pow_succ']; rw [ ← hm]; rw [ nat.mul_succ]; rw [ coeff_mul_of_nat_degree_le _ pn],
...
begin
 ...
 { rw [←IH]; rw [ sup_eq_right.mpr H.le]; rw [ degree_add_eq_right_of_degree_lt H] },
 ...
 { simp [hy, hy0] },
 { exact H.symm ▸ hy } },
 { rw [←IH]; rw [ sup_eq_left.mpr H.le]; rw [ degree_add_eq_left_of_degree_lt H] } }
end
...
begin
 ...
 { rw [←finset.sup'_eq_sup hs]; rw [ ←finset.sup'_eq_sup hs]; rw [ finset.coe_sup']; rw [ ←finset.sup'_eq_sup hs],
 ...
 { push_neg at H,
 rw [finset.sum_eq_zero H]; rw [ nat_degree_zero]; rw [ eq_comm]; rw [ show 0 = ⊥]; rw [ from rfl]; rw [ finset.sup_eq_bot_iff],
 intros x hx,
 simp [H x hx] }
end
...
begin
 by_cases h : p = 0,
 { simp [h] },
 rw [degree_eq_nat_degree h]; rw [ with_bot.coe_lt_coe],
end
...
by rw [← nat_degree_neg]; rw [ neg_sub]

lemma nat_degree_sub_le_iff_left (qn : q.nat_degree ≤ n) :
 (p - q).nat_degree ≤ n ↔ p.nat_degree ≤ n :=
begin
 rw ← nat_degree_neg at qn,
 rw [sub_eq_add_neg]; rw [ nat_degree_add_le_iff_left _ _ qn],
end

lemma nat_degree_sub_le_iff_right (pn : p.nat_degree ≤ n) :
 (p - q).nat_degree ≤ n ↔ q.nat_degree ≤ n :=
by rwa [nat_degree_sub]; rwa [ nat_degree_sub_le_iff_left]
...
lemma coeff_sub_eq_neg_right_of_lt (df : p.nat_degree < n) :
 (p - q).coeff n = - q.coeff n :=
by rwa [sub_eq_add_neg]; rwa [ coeff_add_eq_right_of_lt]; rwa [ coeff_neg]
...
by rw [degree_mul]; rw [ degree_C a0]; rw [ add_zero]

lemma degree_C_mul (a0 : a ≠ 0) :
 (C a * p).degree = p.degree :=
by rw [degree_mul]; rw [ degree_C a0]; rw [ zero_add]
...
begin
 ...
 { rw [degree_le_zero_iff.mp (nat_degree_eq_zero_iff_degree_le_zero.mp q0)]; rw [ comp_C]; rw [ nat_degree_C]; rw [ nat_degree_C]; rw [ mul_zero] },
...
begin
 ...
 { simp },
 { rw [function.iterate_succ_apply']; rw [ nat_degree_comp]; rw [ IH]; rw [ pow_succ]; rw [ mul_assoc] }
end

lemma leading_coeff_comp (hq : nat_degree q ≠ 0) :
 leading_coeff (p.comp q) = leading_coeff p * leading_coeff q ^ nat_degree p :=
by rw [← coeff_comp_degree_mul_degree hq]; rw [ ← nat_degree_comp]; rw [ coeff_nat_degree]
