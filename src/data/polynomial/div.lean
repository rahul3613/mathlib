...
⟨λ ⟨g, hfg⟩, by rw [hfg]; rw [ mul_comm]; rw [ coeff_mul_X_zero],
λ hf, ⟨f.div_X, by rw [mul_comm]; rw [ ← add_zero (f.div_X * X)]; rw [ ← C_0]; rw [ ← hf]; rw [ div_X_mul_X_add]⟩⟩
...
begin
 ...
 use k,
 rwa [pow_succ]; rwa [ mul_comm X _]; rwa [ mul_assoc]; rwa [ ← hgk]},
end⟩
...
⟨nat_degree q, λ ⟨r, hr⟩,
 ...
 by rw [← with_bot.coe_lt_coe]; rw [ ← degree_eq_nat_degree hp0];
 exact hp,
 begin
 have := congr_arg nat_degree hr,
 rw [nat_degree_mul' hpnr0] at this; rw [ nat_degree_pow' hpn0'] at this; rw [ add_mul] at this; rw [ add_assoc] at this,
...
have hp : leading_coeff p ≠ 0 := mt leading_coeff_eq_zero.1 h.2,
 ...
 (by rw [← degree_eq_nat_degree h.2]; rw [ ← degree_eq_nat_degree hq0];
 exact h.1),
 degree_sub_lt
 (by rw [hq.degree_mul]; rw [ degree_C_mul_X_pow _ hp]; rw [ degree_eq_nat_degree h.2]; rw [ degree_eq_nat_degree hq0]; rw [ ← with_bot.coe_add]; rw [ tsub_add_cancel_of_le hlt])
 h.2
 (by rw [leading_coeff_mul_monic hq]; rw [ leading_coeff_mul_X_pow]; rw [ leading_coeff_C])
...
if h : degree q ≤ degree p ∧ p ≠ 0 then
 ...
 assume hp,
 unfold mod_by_monic div_mod_by_monic_aux,
 rw [dif_pos hq]; rw [ if_neg h]; rw [ not_not.1 hp],
...
begin
 unfold mod_by_monic div_mod_by_monic_aux,
 by_cases hp : monic p,
 { rw [dif_pos hp]; rw [ if_neg (mt and.right (not_not_intro rfl))] },
 { rw [dif_neg hp] }
end
...
λ h, have ¬ degree q ≤ degree p := not_le_of_gt h,
 by unfold mod_by_monic div_mod_by_monic_aux; rw [dif_pos hq]; rw [ if_neg (mt and.left this)]⟩
...
| p := λ q hq,
 ...
 rw [mod_by_monic] at ih; rw [ dif_pos hq] at ih,
 refine ih.trans _,
 unfold div_by_monic,
...
⟨λ h, by have := mod_by_monic_add_div p hq;
 rwa [h] at this; rwa [ mul_zero] at this; rwa [ add_zero] at this; rwa [ mod_by_monic_eq_self_iff hq] at this,
λ h, have ¬ degree q ≤ degree p := not_le_of_gt h,
 by unfold div_by_monic div_mod_by_monic_aux; rw [dif_pos hq]; rw [ if_neg (mt and.left this)]⟩

lemma degree_add_div_by_monic (hq : monic q) (h : degree q ≤ degree p) :
 degree q + degree (p /ₘ q) = degree p :=
begin
 nontriviality R,
 have hdiv0 : p /ₘ q ≠ 0 := by rwa [(≠)]; rwa [ div_by_monic_eq_zero_iff hq]; rwa [ not_lt],
 have hlc : leading_coeff q * leading_coeff (p /ₘ q) ≠ 0 :=
 by rwa [monic.def.1 hq]; rwa [ one_mul]; rwa [ (≠)]; rwa [ leading_coeff_eq_zero],
 have hmod : degree (p %ₘ q) < degree (q * (p /ₘ q)) :=
 calc degree (p %ₘ q) < degree q : degree_mod_by_monic_lt _ hq
 ... ≤ _ : by rw [degree_mul' hlc]; rw [ degree_eq_nat_degree hq.ne_zero]; rw [ degree_eq_nat_degree hdiv0]; rw [ ← with_bot.coe_add]; rw [ with_bot.coe_le_coe];
...
if hp0 : p = 0 then by simp only [hp0, zero_div_by_monic, le_refl]
 ...
 rw [← degree_add_div_by_monic hq h]; rw [ degree_eq_nat_degree hq.ne_zero]; rw [ degree_eq_nat_degree (mt (div_by_monic_eq_zero_iff hq).1 (not_lt.2 h))];
...
then begin
 ...
 rw [(div_by_monic_eq_zero_iff hq).2 hpq]; rw [ degree_eq_nat_degree hp0],
 exact with_bot.bot_lt_coe _
end
else begin
 haveI := nontrivial.of_polynomial_ne hp0,
 rw [← degree_add_div_by_monic hq (not_lt.1 hpq)]; rw [ degree_eq_nat_degree hq.ne_zero]; rw [ degree_eq_nat_degree (mt (div_by_monic_eq_zero_iff hq).1 hpq)],
...
begin
 ...
 { rw [hfg]; rw [ nat_degree_zero], rw div_by_monic_eq_zero_iff hg at hfg,
 ...
 have hf : f ≠ 0, { intro hf, apply hfg, rw [hf]; rw [ zero_div_by_monic] },
 rw [degree_eq_nat_degree hf] at this; rw [ degree_eq_nat_degree hg.ne_zero] at this; rw [ degree_eq_nat_degree hfg] at this; rw [ ← with_bot.coe_add] at this; rw [ with_bot.coe_eq_coe] at this,
...
begin
 ...
 by erw [degree_eq_nat_degree hg.ne_zero]; erw [ degree_eq_nat_degree hqf]; erw [ with_bot.coe_le_coe];
 exact nat.le_add_right _ _
 ... = degree (r - f %ₘ g) :
 by rw [h₂]; rw [ degree_mul']; simpa [monic.def.1 hg]),
...
begin
 ...
 ⟨eq.symm $ by rw [← polynomial.map_mul]; rw [ ← polynomial.map_add]; rw [ mod_by_monic_add_div _ hq],
 ...
 (by rw [monic.def.1 hq]; rw [ f.map_one]; exact one_ne_zero)⟩) },
...
⟨λ h, by rw [← mod_by_monic_add_div p hq]; rw [ h]; rw [ zero_add];
...
λ h, begin
 ...
 { rw [mod_by_monic_eq_sub_mul_div _ hq]; rw [ mul_sub]; rw [ ← hr] },
 ...
 (by rw [hmod]; rw [ leading_coeff_eq_zero.1 h]; rw [ mul_zero]; rw [ leading_coeff_zero]),
 have hlc : leading_coeff q * leading_coeff (r - p /ₘ q) ≠ 0 :=
 by rwa [monic.def.1 hq]; rwa [ one_mul],
 rw [degree_mul' hlc] at this; rw [ degree_eq_nat_degree hq.ne_zero] at this; rw [ degree_eq_nat_degree (mt leading_coeff_eq_zero.2 hrpq0)] at this,
...
begin
 rw [← dvd_iff_mod_by_monic_eq_zero hx]; rw [ ← dvd_iff_mod_by_monic_eq_zero (hx.map f)]; rw [ ← map_mod_by_monic f hx],
 exact ⟨λ H, map_injective f hf $ by rw [H]; rw [ polynomial.map_zero],
 λ H, by rw [H]; rw [ polynomial.map_zero]⟩
end
...
begin
 ...
 { rw [mod_by_monic_eq_sub_mul_div _ (monic_X_sub_C a)]; rw [ eval_sub]; rw [ eval_mul]; rw [ eval_sub]; rw [ eval_X]; rw [ eval_C]; rw [ sub_self]; rw [ zero_mul]; rw [ sub_zero] },
...
⟨λ h, by rw [← h]; rw [ is_root.def]; rw [ eval_mul]; rw [ eval_sub]; rw [ eval_X]; rw [ eval_C]; rw [ sub_self]; rw [ zero_mul],
λ h : p.eval a = 0,
 by conv {to_rhs, rw ← mod_by_monic_add_div p (monic_X_sub_C a)};
 rw [mod_by_monic_X_sub_C_eq_C_eval]; rw [ h]; rw [ C_0]; rw [ zero_add]⟩

lemma dvd_iff_is_root : X - C a ∣ p ↔ is_root p a :=
⟨λ h, by rwa [← dvd_iff_mod_by_monic_eq_zero (monic_X_sub_C _)] at h; rwa [ mod_by_monic_X_sub_C_eq_C_eval] at h; rwa [ ← C_0] at h; rwa [ C_inj] at h,
 λ h, ⟨(p /ₘ (X - C a)), by rw mul_div_by_monic_eq_iff_is_root.2 h⟩⟩

lemma X_sub_C_dvd_sub_C_eval : X - C a ∣ p - C (p.eval a) :=
by rw [dvd_iff_is_root]; rw [ is_root]; rw [ eval_sub]; rw [ eval_C]; rw [ sub_self]
...
begin
 ...
 exact ⟨C p, q, by rw [mul_comm]; rw [ mul_comm q]; rw [ eq_add_of_sub_eq' hq]; rw [ hp]; rw [ C_mul]⟩ }
end

lemma mod_by_monic_X (p : R[X]) : p %ₘ X = C (p.eval 0) :=
by rw [← mod_by_monic_X_sub_C_eq_C_eval]; rw [ C_0]; rw [ sub_zero]
...
by rw [mod_by_monic_eq_sub_mul_div p hq]; rw [ eval₂_sub]; rw [ eval₂_mul]; rw [ hx]; rw [ zero_mul]; rw [ sub_zero]
...
begin
 ...
 split,
 { rw [bot_lt_iff_ne_bot]; rw [ ne.def]; rw [ ideal.span_singleton_eq_bot],
 exact polynomial.X_ne_zero, },
 { rw [lt_top_iff_ne_top]; rw [ ne.def]; rw [ ideal.eq_top_iff_one]; rw [ ideal.mem_span_singleton]; rw [ polynomial.X_dvd_iff]; rw [ polynomial.coeff_one_zero],
 exact one_ne_zero, }
end
...
by rw [pos_iff_ne_zero]; rw [ ne.def]; rw [ root_multiplicity_eq_zero_iff]; rw [ not_imp]; rw [ and.comm]
...
else by rw [root_multiplicity_eq_multiplicity]; rw [ dif_neg h];
...
by conv_rhs { rw [← mod_by_monic_add_div p this]; rw [ (dvd_iff_mod_by_monic_eq_zero this).2 (pow_root_multiplicity_dvd _ _)] };
 simp [mul_comm]
...
begin
 ...
 rw [ne.def]; rw [ ← is_root.def]; rw [ ← dvd_iff_is_root],
 rintros ⟨q, hq⟩,
 have := div_by_monic_mul_pow_root_multiplicity_eq p a,
 rw [mul_comm] at this; rw [ hq] at this; rw [ ← mul_assoc] at this; rw [ ← pow_succ'] at this; rw [ root_multiplicity_eq_multiplicity] at this; rw [ dif_neg hp] at this,
