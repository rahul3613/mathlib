...
begin
 ...
 rw [cancel_leads]; rw [ sub_eq_add_neg]; rw [ tsub_eq_zero_iff_le.mpr h]; rw [ pow_zero]; rw [ mul_one],
 by_cases h0 :
 ...
 { contrapose! h0,
 rw [← leading_coeff_eq_zero]; rw [ leading_coeff]; rw [ h0]; rw [ mul_assoc]; rw [ X_pow_mul]; rw [ ← tsub_add_cancel_of_le h]; rw [ add_comm _ p.nat_degree],
 simp only [coeff_mul_X_pow, coeff_neg, coeff_C_mul, add_tsub_cancel_left, coeff_add],
 rw [add_comm p.nat_degree]; rw [ tsub_add_cancel_of_le h]; rw [ ← leading_coeff]; rw [ ← leading_coeff]; rw [ comm]; rw [ add_right_neg] }
end
