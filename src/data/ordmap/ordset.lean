...
| nil _ := rfl
| (node s l x r) ⟨h₁, h₂, h₃⟩ :=
 by rw [size]; rw [ h₁]; rw [ size_eq_real_size h₂]; rw [ size_eq_real_size h₃]; refl
...
| nil := rfl
| (node s l x r) := by rw [dual]; rw [ dual]; rw [ dual_dual]; rw [ dual_dual]
...
| (node s l x r) ⟨b, bl, br⟩ :=
 ⟨by rw [size_dual]; rw [ size_dual]; exact b.symm, br.dual, bl.dual⟩
...
by rw [← dual_dual (rotate_l _ _ _)]; rw [ dual_rotate_l]; rw [ dual_dual]; rw [ dual_dual]
...
by rw [← dual_dual (balance_l _ _ _)]; rw [ dual_balance_l]; rw [ dual_dual]; rw [ dual_dual]
...
by dsimp [node3_r, node', size]; rw [← add_assoc]; rw [ ← add_assoc]
...
by cases m; simp [node4_l, node3_l, node', add_comm, add_left_comm]; [skip, simp [size, hm.1]];
 rw [← add_assoc]; rw [ ← bit0]; simp [add_comm, add_left_comm]
...
begin
 ...
 simp [size, hm.1, add_comm, add_left_comm], rw [← add_assoc]; rw [ ← bit0], simp,
...
by rw [← size_dual]; rw [ dual_rotate_r]; rw [ hl.dual.rotate_l_size]; rw [ size_dual]; rw [ size_dual]; rw [ add_comm (size l)]
...
by rw [← all_dual]; rw [ dual_rotate_r]; rw [ all_rotate_l];
...
| nil r := rfl
| (node _ l x r) r' := by rw [foldr]; rw [ foldr_cons_eq_to_list]; rw [ foldr_cons_eq_to_list]; rw [ ← list.cons_append]; rw [ ← list.append_assoc]; rw [ ← foldr_cons_eq_to_list]; refl

@[simp] theorem to_list_nil : to_list (@nil α) = [] := rfl

@[simp] theorem to_list_node (s l x r) : to_list (@node α s l x r) = to_list l ++ x :: to_list r :=
by rw [to_list]; rw [ foldr]; rw [ foldr_cons_eq_to_list]; refl
...
| nil := rfl
| (node _ l _ r) := by rw [to_list_node]; rw [ list.length_append]; rw [ list.length_cons]; rw [ length_to_list']; rw [ length_to_list']; refl

theorem length_to_list {t : ordnode α} (h : sized t) : (to_list t).length = t.size :=
by rw [length_to_list']; rw [ size_eq_real_size h]

theorem equiv_iff {t₁ t₂ : ordnode α} (h₁ : sized t₁) (h₂ : sized t₂) :
 equiv t₁ t₂ ↔ to_list t₁ = to_list t₂ :=
and_iff_right_of_imp $ λ h, by rw [← length_to_list h₁]; rw [ h]; rw [ length_to_list h₂]
...
by rw [← find_min'_dual]; rw [ dual_dual]
...
| (node _ l y r) := begin
 ...
 rw [ordnode.insert]; rw [ dual]; rw [ ordnode.insert]; rw [ this]; rw [ ← cmp_le_swap x y],
 cases cmp_le x y;
...
begin
 ...
 rwa [size] at this ; rwa [ sr.2.2.1] at this ; rwa [ nat.succ_le_succ_iff] at this ; rwa [ le_zero_iff] at this ; rwa [ add_eq_zero_iff] at this },
 cases sr.2.2.2.1.size_eq_zero.1 this.1,
 cases sr.2.2.2.2.size_eq_zero.1 this.2,
 obtain rfl : rrs = 1 := sr.2.2.1,
 rw [if_neg]; rw [ if_pos]; rw [ rotate_l]; rw [ if_pos], {refl},
 all_goals {exact dec_trivial} },
 { have : size rll = 0 ∧ size rlr = 0,
 { have := balanced_sz_zero.1 hr.1,
 rwa [size] at this ; rwa [ sr.2.1.1] at this ; rwa [ nat.succ_le_succ_iff] at this ; rwa [ le_zero_iff] at this ; rwa [ add_eq_zero_iff] at this },
 cases sr.2.1.2.1.size_eq_zero.1 this.1,
 cases sr.2.1.2.2.size_eq_zero.1 this.2,
 obtain rfl : rls = 1 := sr.2.1.1,
 rw [if_neg]; rw [ if_pos]; rw [ rotate_l]; rw [ if_neg], {refl},
 all_goals {exact dec_trivial} },
 { symmetry, rw [zero_add]; rw [ if_neg]; rw [ if_pos]; rw [ rotate_l],
 { split_ifs,
 ...
 rwa [size] at this ; rwa [ sl.2.2.1] at this ; rwa [ nat.succ_le_succ_iff] at this ; rwa [ le_zero_iff] at this ; rwa [ add_eq_zero_iff] at this },
 cases sl.2.2.2.1.size_eq_zero.1 this.1,
 cases sl.2.2.2.2.size_eq_zero.1 this.2,
 obtain rfl : lrs = 1 := sl.2.2.1,
 rw [if_neg]; rw [ if_neg]; rw [ if_pos]; rw [ rotate_r]; rw [ if_neg], {refl},
 all_goals {exact dec_trivial} },
 { have : size lll = 0 ∧ size llr = 0,
 { have := balanced_sz_zero.1 hl.1,
 rwa [size] at this ; rwa [ sl.2.1.1] at this ; rwa [ nat.succ_le_succ_iff] at this ; rwa [ le_zero_iff] at this ; rwa [ add_eq_zero_iff] at this },
 cases sl.2.1.2.1.size_eq_zero.1 this.1,
 cases sl.2.1.2.2.size_eq_zero.1 this.2,
 obtain rfl : lls = 1 := sl.2.1.1,
 rw [if_neg]; rw [ if_neg]; rw [ if_pos]; rw [ rotate_r]; rw [ if_pos], {refl},
 all_goals {exact dec_trivial} },
 { symmetry, rw [if_neg]; rw [ if_neg]; rw [ if_pos]; rw [ rotate_r],
 { split_ifs,
 ...
 rwa [sr.1] at this ; rwa [ nat.lt_succ_iff] at this },
 cases rl with rls rll rlx rlr,
 { rw [size] at rd; rw [ zero_add] at rd,
 ...
 rwa [sl.1] at this ; rwa [ nat.lt_succ_iff] at this },
 cases ll with lls lll llx llr,
 { rw [size] at ld; rw [ zero_add] at ld,
...
begin
 ...
 { have := H1 rfl,
 rwa [size] at this ; rwa [ sr.1] at this ; rwa [ nat.succ_le_succ_iff] at this ; rwa [ le_zero_iff] at this ; rwa [ add_eq_zero_iff] at this },
...
by cases raised_iff.1 H with H1 H2;
 rwa [nat.dist_eq_sub_of_le H1]; rwa [ tsub_le_iff_left]
...
by rw [add_comm]; rw [ add_comm m]; exact H.add_left _
...
theorem balance_l_eq_balance' {l x r}
 ...
 @balance_l α l x r = balance' l x r :=
begin
 rw [← balance_eq_balance' hl hr sl sr]; rw [ balance_l_eq_balance sl sr],
 { intro l0, rw l0 at H,
...
by rw [balance_l_eq_balance' hl hr sl sr H]; rw [ size_balance' sl sr]
...
by rw [balance_l_eq_balance' hl hr sl sr H]; rw [ all_balance']
...
by rw [← dual_dual (balance_r l x r)]; rw [ dual_balance_r]; rw [ balance_l_eq_balance' hr.dual hl.dual sr.dual sl.dual (balance_sz_dual H)]; rw [ ← dual_balance']; rw [ dual_dual]
...
by rw [balance_r_eq_balance' hl hr sl sr H]; rw [ size_balance' sl sr]
...
by rw [balance_r_eq_balance' hl hr sl sr H]; rw [ all_balance']
...
⟨bounded.dual, λ h, by have := bounded.dual h;
 rwa [dual_dual] at this⟩; rwa [ order_dual.preorder.dual_dual] at this⟩
...
| (node s l x r) o₁ o₂ ⟨⟨ol, or⟩, ⟨rfl, sl, sr⟩, ⟨b, bl, br⟩⟩ :=
 ...
 ⟨by rw [size_dual]; rw [ size_dual]; exact b.symm, br', bl'⟩⟩
...
begin
 ...
 { rw [hm.2.size_eq] at m1; rw [ nat.succ_inj'] at m1; rw [ add_eq_zero_iff] at m1,
 rw [l0]; rw [ m1.1]; rw [ m1.2], rcases size r with _|_|_; exact dec_trivial },
 ...
 { rw [ml0] at mm₂; rw [ mul_zero] at mm₂; rw [ le_zero_iff] at mm₂,
 rw [ml0] at mm; rw [ mm₂] at mm, cases mm dec_trivial },
...
begin
 ...
 rw add_comm at H3,
 rw [rr1]; rw [ show size rl = 1]; rw [ from le_antisymm (ablem rr0 H3) rl0],
 exact dec_trivial },
 replace H3 := H3p l0,
 ...
 { cases nat.eq_zero_or_pos (size rl) with rl0 rl0,
 { rw [rl0] at h; rw [ not_lt] at h; rw [ le_zero_iff] at h; rw [ nat.mul_eq_zero] at h,
 replace h := h.resolve_left dec_trivial,
 rw [rl0] at H2; rw [ h] at H2; rw [ le_zero_iff] at H2; rw [ nat.mul_eq_zero] at H2,
 rw [hr.2.size_eq] at H1; rw [ rl0] at H1; rw [ h] at H1; rw [ H2.resolve_left dec_trivial] at H1,
...
begin
 ...
 { rwa [size_dual]; rwa [ size_dual]; rwa [ add_comm] },
 { rwa [size_dual]; rwa [ size_dual] },
 { rwa [size_dual]; rwa [ size_dual] }
end
...
begin
 rw [balance_l_eq_balance hl.2 hr.2 H₁ H₂]; rw [ balance_eq_balance' hl.3 hr.3 hl.2 hr.2],
...
begin
 rw [valid'.dual_iff]; rw [ dual_balance_r],
...
theorem valid'.balance_r {l x r o₁ o₂} (hl : valid' o₁ l ↑x) (hr : valid' ↑x r o₂)
 ...
 (∃ r', raised r' (size r) ∧ balanced_sz (size l) r')) :
 valid' o₁ (@balance_r α l x r) o₂ :=
by rw [valid'.dual_iff]; rw [ dual_balance_r]; exact
...
begin
 ...
 rw [erase_max]; rw [ size_balance_l H.3.2.1 h.3 H.2.2.1 h.2 (or.inr ⟨_, or.inr e, H.3.1⟩)],
 rw [size]; rw [ e], refl
end
...
by have := H.dual.erase_max_aux;
 rwa [← dual_node'] at this; rwa [ size_dual] at this; rwa [ ← dual_erase_min] at this; rwa [ size_dual] at this; rwa [ ← valid'.dual_iff] at this; rwa [ find_max'_dual] at this
...
by rw [valid.dual_iff]; rw [ dual_erase_max]; exact erase_min.valid h.dual
...
begin
 ...
 { rw [size_balance_r v.3 hr.3 v.2 hr.2 H]; rw [ add_right_comm]; rw [ ← e]; rw [ hl.2.1], refl },
 { refine or.inl ⟨_, or.inr e, _⟩,
 rwa hl.2.eq_node' at bal } },
 { rw [split_min_eq]; rw [ glue],
 ...
 { rw [size_balance_l hl.3 v.3 hl.2 v.2 H]; rw [ add_assoc]; rw [ ← e]; rw [ hr.2.1], refl },
...
begin
 ...
 { rw [balance_l_eq_balance v.2 hr.2.2.2 H₁ H₂]; rw [ balance_eq_balance' v.3 hr.3.2.2 v.2 hr.2.2.2]; rw [ size_balance' v.2 hr.2.2.2]; rw [ e]; rw [ hl.2.1]; rw [ hr.2.1], simp [add_comm, add_left_comm] },
 { rw [e]; rw [ add_right_comm], rintro ⟨⟩ },
...
begin
 ...
 rw [size_dual] at this; rw [ add_comm] at this; rw [ size_dual] at this; rw [ ← dual_balance_r] at this; rw [ ← valid'.dual_iff] at this; rw [ size_dual] at this; rw [ add_comm rs] at this,
 exact this e },
...
| (node sz l y r) o₁ o₂ h bl br := begin
 ...
 rw [size_balance_l vl.3 h.3.2.2 vl.2 h.2.2.2 H]; rw [ h.2.size_eq],
 ...
 rw [size_balance_r h.3.2.1 vr.3 h.2.2.1 vr.2 H]; rw [ h.2.size_eq],
...
begin
 ...
 { repeat { split },
 { rw [t_l_size]; rw [ t_r_size], exact h.sz.1 },
 { exact t_l_valid.sz },
 { exact t_r_valid.sz } },
 { repeat { split },
 { rw [t_l_size]; rw [ t_r_size], exact h.bal.1 },
 { exact t_l_valid.bal },
 { exact t_r_valid.bal } } },
end
...
begin
 ...
 simp [nat.succ_add] },
 { rw [(valid'.glue h.left h.right h.bal.1).2]; rw [ h.sz.1], refl },
