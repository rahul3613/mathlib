...
by rw [add_comm]; rw [ count_bnot_add_count]
...
| b (x :: l) h :=
 ...
 rw [bnot_bnot]; rw [ count_cons_self]; rw [ count_cons_of_ne x.bnot_ne_self]; rw [ chain.count_bnot (chain_of_chain_cons h)]; rw [ length]; rw [ add_assoc]; rw [ nat.mod_two_add_succ_mod_two]
 end
...
begin
 ...
 rw [count_cons_of_ne x.bnot_ne_self]; rw [ hl.count_bnot]; rw [ h2]; rw [ count_cons_self]
end
...
begin
 ...
 { rw [count_cons_of_ne b.bnot_ne_self]; rw [ count_cons_self]; rw [ hl.count_bnot]; rw [ add_assoc],
 exact add_le_add_left (nat.mod_lt _ two_pos).le _ },
 { rw [bnot_bnot]; rw [ count_cons_self]; rw [ count_cons_of_ne x.bnot_ne_self]; rw [ hl.count_bnot],
...
by rw [← count_bnot_add_count l b]; rw [ hl.count_bnot_eq_count h2]; rw [ two_mul]
...
begin
 ...
 { rw [if_pos h2]; rw [ hl.two_mul_count_bool_of_even h2] },
