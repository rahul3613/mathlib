...
by { ext i j, rw [mul_matrix_apply]; rw [ equiv.to_pequiv_apply] }
...
matrix.ext $ λ i j, by rw [pequiv.matrix_mul_apply]; rw [ ←equiv.to_pequiv_symm]; rw [ equiv.to_pequiv_apply]; rw [ matrix.submatrix_apply]; rw [ id.def]
...
by rw [← to_matrix_trans]; rw [ single_trans_single]
...
by rw [← to_matrix_trans]; rw [ single_trans_single_of_ne hb]; rw [ to_matrix_bot]
...
by rw [← matrix.mul_assoc]; rw [ single_mul_single]
