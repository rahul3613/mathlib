...
by simp_rw [matrix.trace, matrix.diag, kronecker_map_bilinear_apply_apply, linear_map.map_sum₂, map_sum, ←finset.univ_product_univ, finset.sum_product, kronecker_map_apply]
...
calc det (kronecker_map_bilinear f A B)
 ...
 : by rw [←kronecker_map_bilinear_mul_mul f h_comm]; rw [ matrix.mul_one]; rw [ matrix.one_mul]
... = det (block_diagonal (λ _, A.map (λ a, f a 1)))
 * det (block_diagonal (λ _, B.map (λ b, f 1 b)))
 : begin
 rw [det_mul]; rw [ ←diagonal_one]; rw [ ←diagonal_one]; rw [ kronecker_map_bilinear_apply_apply]; rw [ kronecker_map_diagonal_right _ (λ _, _)]; rw [ kronecker_map_bilinear_apply_apply]; rw [ kronecker_map_diagonal_left _ (λ _, _)]; rw [ det_reindex_self],
...
begin
 ...
 rw [nonsing_inv_apply_not_is_unit _ hA]; rw [ zero_kronecker]; rw [ nonsing_inv_apply_not_is_unit _ hAB] },
 ...
 rw [nonsing_inv_apply_not_is_unit _ hB]; rw [ kronecker_zero]; rw [ nonsing_inv_apply_not_is_unit _ hAB] },
