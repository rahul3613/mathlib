...
by { ext i j, rw [vec_mul_vec_apply]; rw [ pi.smul_apply]; rw [ smul_eq_mul] }
...
lemma vec2_add [has_add α] (a₀ a₁ b₀ b₁ : α) :
 ![a₀, a₁] + ![b₀, b₁] = ![a₀ + b₀, a₁ + b₁] :=
by rw [cons_add_cons]; rw [ cons_add_cons]; rw [ empty_add_empty]

lemma vec3_add [has_add α] (a₀ a₁ a₂ b₀ b₁ b₂ : α) :
 ![a₀, a₁, a₂] + ![b₀, b₁, b₂] = ![a₀ + b₀, a₁ + b₁, a₂ + b₂] :=
by rw [cons_add_cons]; rw [ cons_add_cons]; rw [ cons_add_cons]; rw [ empty_add_empty]

lemma smul_vec2 {R : Type*} [has_smul R α] (x : R) (a₀ a₁ : α) :
 x • ![a₀, a₁] = ![x • a₀, x • a₁] :=
by rw [smul_cons]; rw [ smul_cons]; rw [ smul_empty]

lemma smul_vec3 {R : Type*} [has_smul R α] (x : R) (a₀ a₁ a₂ : α) :
 x • ![a₀, a₁, a₂] = ![x • a₀, x • a₁, x • a₂] :=
by rw [smul_cons]; rw [ smul_cons]; rw [ smul_cons]; rw [ smul_empty]

variables [add_comm_monoid α] [has_mul α]

lemma vec2_dot_product' {a₀ a₁ b₀ b₁ : α} :
 ![a₀, a₁] ⬝ᵥ ![b₀, b₁] = a₀ * b₀ + a₁ * b₁ :=
by rw [cons_dot_product_cons]; rw [ cons_dot_product_cons]; rw [ dot_product_empty]; rw [ add_zero]
...
by rw [cons_dot_product_cons]; rw [ cons_dot_product_cons]; rw [ cons_dot_product_cons]; rw [ dot_product_empty]; rw [ add_zero]; rw [ add_assoc]
