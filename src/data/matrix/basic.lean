...
begin
 rw [conj_transpose]; rw [ diagonal_transpose]; rw [ diagonal_map (star_zero _)],
 refl,
end
...
begin
 ...
 rw [matrix.mul_apply]; rw [ finset.sum_fin_eq_sum_range]; rw [ finset.sum_range_succ]; rw [ finset.sum_range_succ],
 simp},
end
...
@[simp] protected theorem one_mul [fintype m] [decidable_eq m] (M : matrix m n α) :
 (1 : matrix m m α) ⬝ M = M :=
by ext i j; rw [← diagonal_one]; rw [ diagonal_mul]; rw [ one_mul]

@[simp] protected theorem mul_one [fintype n] [decidable_eq n] (M : matrix m n α) :
 M ⬝ (1 : matrix n n α) = M :=
by ext i j; rw [← diagonal_one]; rw [ mul_diagonal]; rw [ mul_one]
...
protected theorem sub_mul (M M' : matrix m n α) (N : matrix n o α) :
 (M - M') ⬝ N = M ⬝ N - M' ⬝ N :=
by rw [sub_eq_add_neg]; rw [ matrix.add_mul]; rw [ matrix.neg_mul]; rw [ sub_eq_add_neg]

protected theorem mul_sub (M : matrix m n α) (N N' : matrix n o α) :
 M ⬝ (N - N') = M ⬝ N - M ⬝ N' :=
by rw [sub_eq_add_neg]; rw [ matrix.mul_add]; rw [ matrix.mul_neg]; rw [ sub_eq_add_neg]
...
begin
 ...
 { intro h,
 inhabit n,
 rw [← scalar_apply_eq r (arbitrary n)]; rw [ ← scalar_apply_eq s (arbitrary n)]; rw [ h] },
 { rintro rfl, refl }
end
...
begin
 rw [algebra_map_eq_diagonal]; rw [ algebra_map_eq_diagonal]; rw [ diagonal_map hf],
 congr' 1 with x,
...
by { ext, rw [←diagonal_one]; rw [ mul_vec_diagonal]; rw [ one_mul] }

@[simp] lemma vec_mul_one (v : m → α) : vec_mul v 1 = v :=
by { ext, rw [←diagonal_one]; rw [ vec_mul_diagonal]; rw [ mul_one] }
...
ext $ λ i j, begin
 rw submatrix_apply,
 by_cases h : i = j,
 { rw [h]; rw [ diagonal_apply_eq]; rw [ diagonal_apply_eq], },
 { rw [diagonal_apply_ne _ h]; rw [ diagonal_apply_ne _ (he.ne h)], },
end
...
by rw [submatrix_mul_equiv]; rw [ submatrix_id_id]
...
begin
 by_cases i' = i,
 { rw [h]; rw [ update_row_self]; rw [ if_pos rfl] },
 { rwa [update_row_ne h]; rwa [ if_neg h] }
end
...
begin
 by_cases j' = j,
 { rw [h]; rw [ update_column_self]; rw [ if_pos rfl] },
 { rwa [update_column_ne h]; rwa [ if_neg h] }
end
...
begin
 ext i' j',
 rw [update_row_apply]; rw [ map_apply]; rw [ map_apply]; rw [ update_row_apply],
...
begin
 ext i' j',
 rw [update_column_apply]; rw [ map_apply]; rw [ map_apply]; rw [ update_column_apply],
 exact apply_ite f _ _ _,
end
...
begin
 ext i' j,
 rw [transpose_apply]; rw [ update_row_apply]; rw [ update_column_apply],
 refl
end
...
begin
 rw [conj_transpose]; rw [ conj_transpose]; rw [ transpose_map]; rw [ transpose_map]; rw [ update_row_transpose]; rw [ map_update_column],
 refl,
end
...
begin
 rw [conj_transpose]; rw [ conj_transpose]; rw [ transpose_map]; rw [ transpose_map]; rw [ update_column_transpose]; rw [ map_update_row],
 refl,
end
...
begin
 ...
 { rw [update_column_self]; rw [ pi.single_eq_same]; rw [ function.update_same], },
 { rw [update_column_ne hji]; rw [ diagonal_apply_eq]; rw [ function.update_noteq hji], } },
 { rw [diagonal_apply_ne _ hjk],
 obtain rfl | hki := eq_or_ne k i,
 { rw [update_column_self]; rw [ pi.single_eq_of_ne hjk] },
 { rw [update_column_ne hki]; rw [ diagonal_apply_ne _ hjk] } }
end
...
by rw [←diagonal_transpose]; rw [ update_row_transpose]; rw [ diagonal_update_column_single]; rw [ diagonal_transpose]
