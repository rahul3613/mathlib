...
by rw [rank]; rw [ mul_vec_lin_one]; rw [ linear_map.range_id]; rw [ finrank_top]; rw [ finrank_pi]

@[simp] lemma rank_zero [nontrivial R] : rank (0 : matrix m n R) = 0 :=
by rw [rank]; rw [ mul_vec_lin_zero]; rw [ linear_map.range_zero]; rw [ finrank_bot]
...
lemma rank_mul_le_left [strong_rank_condition R] (A : matrix m n R) (B : matrix n o R) :
 (A ⬝ B).rank ≤ A.rank :=
begin
 rw [rank]; rw [ rank]; rw [ mul_vec_lin_mul],
...
begin
 ...
 rwa [← mul_eq_mul] at this; rwa [ ← units.coe_mul] at this; rwa [ mul_inv_self] at this; rwa [ units.coe_one] at this; rwa [ rank_one] at this,
end
...
begin
 rw [rank]; rw [ rank]; rw [ mul_vec_lin_submatrix]; rw [ linear_map.range_comp]; rw [ linear_map.range_comp]; rw [ (show linear_map.fun_left R R e.symm = linear_equiv.fun_congr_left R R e.symm, from rfl)]; rw [ linear_equiv.range]; rw [ submodule.map_top],
 exact submodule.finrank_map_le _ _,
end

lemma rank_reindex [fintype m] (e₁ e₂ : m ≃ n) (A : matrix m m R) :
 rank (reindex e₁ e₂ A) = rank A :=
by rw [rank]; rw [ rank]; rw [ mul_vec_lin_reindex]; rw [ linear_map.range_comp]; rw [ linear_map.range_comp]; rw [ linear_equiv.range]; rw [ submodule.map_top]; rw [ linear_equiv.finrank_map_eq]
...
begin
 ...
 rw [← linear_map.range_comp]; rw [ ← linear_map.range_comp_of_range_eq_top (to_lin v₂ v₁ A) range_e₂],
 congr' 1,
 ...
 rw [to_lin_eq_to_lin'] at aux₁; rw [ to_lin'_apply'] at aux₁,
 rw [pi.basis_fun_apply] at aux₁ aux₂; rw [ linear_map.coe_std_basis] at aux₁ aux₂,
...
by rw [rank]; rw [ matrix.range_mul_vec_lin]
...
begin
 ...
 rwa [dot_product_mul_vec] at h ; rwa [ dot_product_zero] at h ; rwa [ vec_mul_conj_transpose] at h ; rwa [ star_star] at h ; rwa [ dot_product_star_self_eq_zero] at h },
 { intro h, rw [h]; rw [ mul_vec_zero] },
end
...
begin
 ...
 dsimp only,
 rw [linear_map.finrank_range_add_finrank_ker]; rw [ ←((Aᴴ ⬝ A).mul_vec_lin).finrank_range_add_finrank_ker],
 congr' 1,
...
begin
 ...
 rwa [dot_product_mul_vec] at h ; rwa [ dot_product_zero] at h ; rwa [ vec_mul_transpose] at h ; rwa [ dot_product_self_eq_zero] at h },
 { intro h, rw [h]; rw [ mul_vec_zero] },
end
...
begin
 ...
 refine add_left_injective (finrank R (A.mul_vec_lin).ker) _,
 dsimp only,
 rw [linear_map.finrank_range_add_finrank_ker]; rw [ ←((Aᵀ ⬝ A).mul_vec_lin).finrank_range_add_finrank_ker],
 congr' 1,
