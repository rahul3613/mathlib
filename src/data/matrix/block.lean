...
⟨λ h, h ▸ ⟨rfl, rfl, rfl, rfl⟩, λ ⟨h₁₁, h₁₂, h₂₁, h₂₂⟩,
 by rw [←from_blocks_to_blocks A]; rw [ ←from_blocks_to_blocks B]; rw [ h₁₁]; rw [ h₁₂]; rw [ h₂₁]; rw [ h₂₂]⟩
...
ext $ λ i j, begin
 ...
 { rw [block_diag_apply]; rw [ diagonal_apply_eq]; rw [ diagonal_apply_eq] },
 { rw [block_diag_apply]; rw [ diagonal_apply_ne _ hij]; rw [ diagonal_apply_ne _ (mt _ hij)],
...
begin
 ...
 { split_ifs; simp },
 { intros j' hj', exact finset.sum_eq_zero (λ _ _, by rw [dif_neg hj'.symm]; rw [ zero_mul]) },
end
...
ext $ λ i j, begin
 obtain rfl | hij := decidable.eq_or_ne i j,
 { rw [block_diag'_apply]; rw [ diagonal_apply_eq]; rw [ diagonal_apply_eq] },
 { rw [block_diag'_apply]; rw [ diagonal_apply_ne _ hij]; rw [ diagonal_apply_ne _ (mt (λ h, _) hij)],
 cases h, refl },
end
