...
by rw [foldl]; rw [ as_list]; rw [ list.foldl_join]; rw [ ← array.to_list_foldl]
...
begin
 ...
 rintros l ⟨i, rfl⟩,
 apply v.nodup },
 rw [← list.enum_map_snd bkts.to_list]; rw [ list.pairwise_map]; rw [ list.pairwise_map],
 ...
 apply ij, rwa [← v.idx_enum_1 _ me₁ ml₁]; rwa [ ← v.idx_enum_1 _ me₂ ml₂]
end
...
section
 ...
 { conv { to_lhs,
 rw [← list.take_append_drop bidx bkts.to_list]; rw [ list.drop_eq_nth_le_cons h],
 simp [hl] }, simp },
 { conv { to_lhs,
 rw [bkts']; rw [ array.write_to_list]; rw [ list.update_nth_eq_take_cons_drop _ h],
 simp [hfl] }, simp }
 end
...
begin
 ...
 rw [mk_as_list] at p; rw [ list.append_nil] at p; rw [ zero_add] at p; rw [ v.len] at p,
...
| ⟨hash_fn, size, n, bkts, v⟩ a b a' b' := begin
 ...
 rw [show bkts'' = bkts'.as_list.foldl _ _]; rw [ from bkts'.foldl_eq _ _]; rw [ ← list.foldr_reverse],
