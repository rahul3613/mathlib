...
| ⟨v, hv⟩ ⟨w, hw⟩ h := subtype.eq (list.ext_le (by rw [hv]; rw [ hw])
 (λ m hm hn, h ⟨m, hv ▸ hm⟩))
...
⟨λ h, h.symm ▸ ⟨head_cons a v', tail_cons a v'⟩,
 λ h, trans (cons_head_tail v).symm (by rw [h.1]; rw [ h.2])⟩

lemma ne_cons_iff (a : α) (v : vector α n.succ) (v' : vector α n) :
 v ≠ a ::ᵥ v' ↔ v.head ≠ a ∨ v.tail ≠ v' :=
by rw [ne.def]; rw [ eq_cons_iff a v v']; rw [ not_and_distrib]
...
| 0 f := rfl
| (n+1) f := by rw [of_fn]; rw [ list.of_fn_succ]; rw [ to_list_cons]; rw [ to_list_of_fn]
...
by rw [nth_eq_nth_le]; rw [ ← list.nth_le_of_fn f];
...
by rw [← nth_tail_succ]; rw [ tail_cons]
...
begin
 ...
 list.length_reverse] },
 rw [←nth_zero]; rw [ last_def]; rw [ nth_eq_nth_le]; rw [ nth_eq_nth_le],
...
⟨list.scanl f b v.to_list, by rw [list.length_scanl]; rw [ to_list_length]⟩
...
begin
 ...
 { rw [←cons_head_tail v]; rw [ scanl_cons]; rw [ nth_cons_succ],
...
⟨v.1.insert_nth i a,
 begin
 rw [list.length_insert_nth]; rw [ v.2],
...
⟨v.1.update_nth i.1 a, by rw [list.update_nth_length]; rw [ v.2]⟩
