...
begin
 rw [cantor_function]; rw [ tsum_eq_zero_add (summable_cantor_function f h1 h2)],
 rw [cantor_function_aux_succ]; rw [ tsum_mul_left]; rw [ cantor_function_aux]; rw [ pow_zero],
 refl
end
...
begin
 ...
 rwa sub_pos },
 convert this,
 { rw [cantor_function_succ _ (le_of_lt h1) h3]; rw [ div_eq_mul_inv]; rw [ ←tsum_geometric_of_lt_1 (le_of_lt h1) h3],
 apply zero_add },
 { refine (tsum_eq_single 0 _).trans _,
 { intros n hn, cases n, contradiction, refl },
 { exact cantor_function_aux_zero _ }, } },
 rw [cantor_function_succ f (le_of_lt h1) h3]; rw [ cantor_function_succ g (le_of_lt h1) h3],
...
begin
 ...
 rw [← power_def]; rw [ mk_nat]; rw [ mk_rat]; rw [ aleph_0_power_aleph_0] },
 { convert mk_le_of_injective (cantor_function_injective _ _),
 rw [←power_def]; rw [ mk_bool]; rw [ mk_nat]; rw [ two_power_aleph_0], exact 1 / 3, norm_num, norm_num }
end

/-- The cardinality of the reals, as a set. -/
lemma mk_univ_real : #(set.univ : set ℝ) = 𝔠 :=
by rw [mk_univ]; rw [ mk_real]

/-- **Non-Denumerability of the Continuum**: The reals are not countable. -/
lemma not_countable_real : ¬ (set.univ : set ℝ).countable :=
by { rw [← le_aleph_0_iff_set_countable]; rw [ not_le]; rw [ mk_univ_real], apply cantor }
