...
λ r hr, by rw [←neg_neg r]; rw [ coe_neg];
...
by rw [is_st_real_iff_eq]; rw [ is_st_real_iff_eq]; rw [ eq_comm]

lemma is_st_trans_real {r s t : ℝ} : is_st r s → is_st s t → is_st r t :=
by rw [is_st_real_iff_eq]; rw [ is_st_real_iff_eq]; rw [ is_st_real_iff_eq]; exact eq.trans
...
λ hxr hys, by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; exact is_st_add hxr (is_st_neg hys)
...
lemma is_st_le_of_le {x y : ℝ*} {r s : ℝ} (hrx : is_st x r) (hsy : is_st y s) :
 x ≤ y → r ≤ s := by rw [←not_lt]; rw [ ←not_lt]; rw [ not_imp_not]; exact lt_of_is_st_lt hsy hrx
...
⟨ λ hi d, or.cases_on hi
 ...
 { exact false.elim (ne_zero_of_infinite (or.inl (by rw [h]; rwa [h] at hipa)) h) ; rwa [ abs_zero] at hipa)) h) },
 { exact or.inl (by rwa abs_of_pos h at hipa) } } ⟩

lemma infinite_iff_infinite_abs {x : ℝ*} : infinite x ↔ infinite (|x|) :=
by rw [←infinite_pos_iff_infinite_of_nonneg (abs_nonneg _)]; rw [ infinite_iff_infinite_pos_abs]
...
by rw [@infinite_neg_iff_infinite_pos_neg x]; rw [ @infinite_pos_iff_infinite_neg_neg y]; rw [ @infinite_neg_iff_infinite_pos_neg (x + y)]; rw [ neg_add];
...
Exists.cases_on (hf' (r + 1)) $ λ i hi,
 ...
 λ a, by rw [←not_le]; rw [ ←not_le]; exact not_imp_not.mpr (hi a),
...
is_st_iff_abs_sub_lt_delta.mpr $ λ d hd,
 calc |x * y - r * s|
 = |x * (y - s) + (x - r) * s| :
 by rw [mul_sub]; rw [ sub_mul]; rw [ add_sub]; rw [ sub_add_cancel]
 ...
 ... = (d : ℝ*) : by rw [mul_one]; rw [ add_halves]
...
begin
 ...
 rw [hs] at hys'; rw [ coe_zero] at hys'; rw [ sub_zero] at hys',
 rw [hs]; rw [ mul_zero]; rw [ coe_zero]; rw [ sub_zero]; rw [ abs_mul]; rw [ mul_comm]; rw [ ←div_mul_cancel (d : ℝ*) (ne_of_gt (lt_of_le_of_lt (abs_nonneg x) ht))]; rw [ ←coe_div],
...
if h : infinite x
then by rw [st_infinite h]; rw [ st_infinite (infinite_iff_infinite_neg.mp h)]; rw [ neg_zero]
...
λ hf d hd, by rw [sub_eq_add_neg]; rw [ ←coe_neg]; rw [ ←coe_add]; rw [ ←coe_add]; rw [ zero_add]; rw [ zero_add];
...
show is_st (x - r) 0,
by { rw [sub_eq_add_neg]; rw [ ← add_neg_self r], exact is_st_add hxr (is_st_refl_real (-r)) }
...
⟨ λ hin, have hin' : _ := infinite_pos_iff_infinitesimal_inv_pos.mp
 ...
 by rwa [infinitesimal_neg_iff]; rwa [ ←neg_pos]; rwa [ neg_inv],
 λ hin, by rwa [←neg_pos] at hin ⟩; rwa [ infinitesimal_neg_iff] at hin ⟩; rwa [ neg_inv] at hin ⟩; rwa [ ←infinite_pos_iff_infinitesimal_inv_pos] at hin ⟩; rwa [ ←infinite_neg_iff_infinite_pos_neg] at hin ⟩
...
by rw [←(zero_add r)]; rw [ ←(sub_add_cancel f (λ n, r))];
...
begin
 ...
 rw [st_infinite ((infinitesimal_iff_infinite_inv h0).mp h1)]; rw [ st_of_is_st h1]; rw [ inv_zero],
 by_cases h2 : infinite x,
 rw [st_of_is_st (infinitesimal_inv_of_infinite h2)]; rw [ st_infinite h2]; rw [ inv_zero],
...
have hyr : _ := by rw [not_imp] at hy₁''; rw [ ←abs_lt] at hy₁''; rw [ not_lt] at hy₁''; rw [ abs_of_pos hy₂] at hy₁''; exact hy₁'',
by rw [←div_mul_cancel r (ne_of_gt hyr.1)]; rw [ coe_mul];
...
by rw [infinite_neg_iff_infinite_pos_neg]; rw [ ←neg_pos]; rw [ ←neg_mul_neg]; rw [ infinitesimal_neg_iff];
...
by rw [infinite_neg_iff_infinite_pos_neg]; rw [ ←neg_pos]; rw [ neg_mul_eq_mul_neg]; rw [ infinitesimal_neg_iff];
exact infinite_pos_mul_of_infinite_pos_not_infinitesimal_pos
...
by rw [infinite_neg_iff_infinite_pos_neg]; rw [ infinite_neg_iff_infinite_pos_neg]; rw [ neg_mul_eq_neg_mul];
exact infinite_pos_mul_of_infinite_pos_not_infinitesimal_pos
