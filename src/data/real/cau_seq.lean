...
⟨g, λ ε, by rw [show g = f]; rw [ from (funext e).symm]; exact f.cauchy⟩
...
begin
 ...
 rw [add_sub] at this; rw [ add_comm] at this, simpa }
end
...
| ε ε0 := let ⟨F, F0, hF⟩ := f.bounded' 0 in
 ...
 rwa [mul_comm F] at this; rwa [ div_mul_cancel _ (ne_of_gt F0)] at this; rwa [ ← abv_mul abv] at this
...
| ε ε0 := let ⟨G, G0, hG⟩ := g.bounded' 0 in
 ...
 rwa [div_mul_cancel _ (ne_of_gt G0)] at this; rwa [ ← abv_mul abv] at this
...
by have := lt_of_le_of_lt (abv_add abv (f j - g j) _) (add_lt_add h₁ (h₂ _ jk));
 rwa [sub_add_sub_cancel'] at this; rwa [ add_halves] at this
...
begin
 ...
 rwa [sub_add_cancel] at this; rwa [ add_halves] at this
end
...
| ε ε0 :=
 ...
 rwa [add_halves] at this; rwa [ add_halves] at this; rwa [ add_right_comm] at this; rwa [ sub_add_sub_cancel] at this; rwa [ sub_add_sub_cancel] at this
 end⟩
...
show lim_zero _ ↔ _, by rw [← const_sub]; rw [ const_lim_zero]; rw [ sub_eq_zero]
...
| ⟨F, F0, hF⟩ H :=
 ...
 rwa [← sub_eq_add_neg] at this; rwa [ sub_self_div_two] at this
 end⟩
...
by have := pos_add_lim_zero gh (neg_lim_zero fg);
 rwa [← sub_eq_add_neg] at this; rwa [ sub_sub_sub_cancel_right] at this
...
show pos _ ↔ _, by rw [← const_sub]; rw [ const_pos]; rw [ sub_pos]
...
let ⟨a, h⟩ := (-f).exists_gt in ⟨-a, show pos _,
 by rwa [const_neg]; rwa [ sub_neg_eq_add]; rwa [ add_comm]; rwa [ ← sub_neg_eq_add]⟩
...
begin
 ...
 rwa [sub_self]; rwa [ max_eq_right]; rwa [ abs_zero],
...
begin
 ...
 rwa [sub_self]; rwa [ min_eq_right]; rwa [ abs_zero],
 exact ε0.le.trans (h _ hj) },
