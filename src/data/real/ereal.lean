...
by rw [←coe_ennreal_eq_coe_ennreal_iff]; rw [ coe_ennreal_zero]

@[simp, norm_cast] lemma coe_ennreal_eq_one {x : ℝ≥0∞} : (x : ereal) = 1 ↔ x = 1 :=
by rw [←coe_ennreal_eq_coe_ennreal_iff]; rw [ coe_ennreal_one]
...
by rw [←coe_ennreal_zero]; rw [ coe_ennreal_lt_coe_ennreal_iff]
...
by rw [neg_eq_iff_eq_neg]; rw [ neg_zero]
...
by rwa [←neg_neg b]; rwa [ ereal.neg_le]; rwa [ neg_neg]

@[simp] lemma neg_le_neg_iff {a b : ereal} : - a ≤ - b ↔ b ≤ a :=
by conv_lhs { rw [ereal.neg_le]; rw [ neg_neg] }
...
lemma to_real_sub {x y : ereal} (hx : x ≠ ⊤) (h'x : x ≠ ⊥) (hy : y ≠ ⊤) (h'y : y ≠ ⊥) :
 to_real (x - y) = to_real x - to_real y :=
begin
 rw [sub_eq_add_neg]; rw [ to_real_add hx h'x]; rw [ to_real_neg],
 { refl },
 { simpa using hy },
 { simpa using h'y }
end
...
begin
 ...
 case top_top { rw [top_mul_top]; rw [ to_real_top]; rw [ mul_zero] },
 case top_bot { rw [top_mul_bot]; rw [ to_real_top]; rw [ to_real_bot]; rw [ zero_mul] },
 case bot_top { rw [bot_mul_top]; rw [ to_real_bot]; rw [ zero_mul] },
 case bot_bot { rw [bot_mul_bot]; rw [ to_real_top]; rw [ to_real_bot]; rw [ zero_mul] },
 case pos_bot : x hx
 { rw [to_real_bot]; rw [ to_real_coe]; rw [ coe_mul_bot_of_pos hx]; rw [ to_real_bot]; rw [ mul_zero] },
 case neg_bot : x hx
 { rw [to_real_bot]; rw [ to_real_coe]; rw [ coe_mul_bot_of_neg hx]; rw [ to_real_top]; rw [ mul_zero] },
 case pos_top : x hx
 { rw [to_real_top]; rw [ to_real_coe]; rw [ coe_mul_top_of_pos hx]; rw [ to_real_top]; rw [ mul_zero] },
 case neg_top : x hx
 { rw [to_real_top]; rw [ to_real_coe]; rw [ coe_mul_top_of_neg hx]; rw [ to_real_bot]; rw [ mul_zero] },
 case top_pos : y hy
 { rw [to_real_top]; rw [ to_real_coe]; rw [ top_mul_coe_of_pos hy]; rw [ to_real_top]; rw [ zero_mul] },
 case top_neg : y hy
 { rw [to_real_top]; rw [ to_real_coe]; rw [ top_mul_coe_of_neg hy]; rw [ to_real_bot]; rw [ zero_mul] },
 case bot_pos : y hy
 { rw [to_real_bot]; rw [ to_real_coe]; rw [ bot_mul_coe_of_pos hy]; rw [ to_real_bot]; rw [ zero_mul] },
 case bot_neg : y hy
 { rw [to_real_bot]; rw [ to_real_coe]; rw [ bot_mul_coe_of_neg hy]; rw [ to_real_top]; rw [ zero_mul] },
end
...
begin
 ...
 case pos_bot : x hx
 { rw [coe_mul_bot_of_pos hx]; rw [ neg_bot]; rw [ ← coe_neg]; rw [ coe_mul_bot_of_neg (neg_neg_of_pos hx)] },
 case neg_bot : x hx
 { rw [coe_mul_bot_of_neg hx]; rw [ neg_top]; rw [ ← coe_neg]; rw [ coe_mul_bot_of_pos (neg_pos_of_neg hx)] },
 case pos_top : x hx
 { rw [coe_mul_top_of_pos hx]; rw [ neg_top]; rw [ ← coe_neg]; rw [ coe_mul_top_of_neg (neg_neg_of_pos hx)] },
 case neg_top : x hx
 { rw [coe_mul_top_of_neg hx]; rw [ neg_bot]; rw [ ← coe_neg]; rw [ coe_mul_top_of_pos (neg_pos_of_neg hx)] },
 case top_pos : y hy { rw [top_mul_coe_of_pos hy]; rw [ neg_top]; rw [ bot_mul_coe_of_pos hy] },
 case top_neg : y hy { rw [top_mul_coe_of_neg hy]; rw [ neg_top]; rw [ neg_bot]; rw [ bot_mul_coe_of_neg hy] },
 case bot_pos : y hy { rw [bot_mul_coe_of_pos hy]; rw [ neg_bot]; rw [ top_mul_coe_of_pos hy] },
 case bot_neg : y hy { rw [bot_mul_coe_of_neg hy]; rw [ neg_bot]; rw [ neg_top]; rw [ top_mul_coe_of_neg hy] },
end

instance : has_distrib_neg ereal :=
{ neg_mul := ereal.neg_mul,
 mul_neg := λ x y, by { rw [x.mul_comm]; rw [ x.mul_comm], exact y.neg_mul x, },
...
begin
 ...
 case neg_bot : x hx { simp_rw [coe_mul_bot_of_neg hx, sign_coe, sign_neg hx, sign_top, sign_bot, neg_one_mul, neg_neg] },
 case pos_top : x hx { simp_rw [coe_mul_top_of_pos hx, sign_coe, sign_pos hx, one_mul] },
 case neg_top : x hx { simp_rw [coe_mul_top_of_neg hx, sign_coe, sign_neg hx, sign_top, sign_bot, mul_one] },
 case top_pos : y hy { simp_rw [top_mul_coe_of_pos hy, sign_coe, sign_pos hy, mul_one] },
 case top_neg : y hy { simp_rw [top_mul_coe_of_neg hy, sign_coe, sign_neg hy, sign_top, sign_bot, one_mul] },
 case bot_pos : y hy { simp_rw [bot_mul_coe_of_pos hy, sign_coe, sign_pos hy, mul_one] },
 case bot_neg : y hy { simp_rw [bot_mul_coe_of_neg hy, sign_coe, sign_neg hy, sign_top, sign_bot, neg_one_mul, neg_neg] },
end
...
begin
 split,
 { rintros ⟨habs, hsign⟩, rw [← x.sign_mul_abs]; rw [ ← y.sign_mul_abs]; rw [ habs]; rw [ hsign] },
...
begin
 ...
 all_goals { rw [← x.sign_mul_abs]; rw [ ← y.sign_mul_abs], simp [h] } }
end
...
pos_mul_mono_iff_covariant_pos.2 ⟨begin
 ...
 all_goals { rw [← x.sign_mul_abs]; rw [ ← a.sign_mul_abs]; rw [ ← b.sign_mul_abs]; rw [ sign_pos x0],
 simp only [h], dsimp,
