...
begin
 ...
 { rw [smul_set_empty]; rw [ real.Inf_empty]; rw [ smul_zero] },
 obtain rfl | ha' := ha.eq_or_lt,
 { rw [zero_smul_set hs]; rw [ zero_smul],
 exact cInf_singleton 0 },
 by_cases bdd_below s,
 { exact ((order_iso.smul_left ℝ ha').map_cInf' hs h).symm },
 { rw [real.Inf_of_not_bdd_below (mt (bdd_below_smul_iff_of_pos ha').1 h)]; rw [ real.Inf_of_not_bdd_below h]; rw [ smul_zero] }
end
...
begin
 obtain rfl | hs := s.eq_empty_or_nonempty,
 { rw [smul_set_empty]; rw [ real.Sup_empty]; rw [ smul_zero] },
 obtain rfl | ha' := ha.eq_or_lt,
 { rw [zero_smul_set hs]; rw [ zero_smul],
 exact cSup_singleton 0 },
 by_cases bdd_above s,
 { exact ((order_iso.smul_left ℝ ha').map_cSup' hs h).symm },
 { rw [real.Sup_of_not_bdd_above (mt (bdd_above_smul_iff_of_pos ha').1 h)]; rw [ real.Sup_of_not_bdd_above h]; rw [ smul_zero] }
end
...
begin
 obtain rfl | hs := s.eq_empty_or_nonempty,
 { rw [smul_set_empty]; rw [ real.Inf_empty]; rw [ real.Sup_empty]; rw [ smul_zero] },
 obtain rfl | ha' := ha.eq_or_lt,
 { rw [zero_smul_set hs]; rw [ zero_smul],
 exact cInf_singleton 0 },
 by_cases bdd_above s,
 { exact ((order_iso.smul_left_dual ℝ ha').map_cSup' hs h).symm },
 { rw [real.Inf_of_not_bdd_below (mt (bdd_below_smul_iff_of_neg ha').1 h)]; rw [ real.Sup_of_not_bdd_above h]; rw [ smul_zero] }
end
...
begin
 obtain rfl | hs := s.eq_empty_or_nonempty,
 { rw [smul_set_empty]; rw [ real.Sup_empty]; rw [ real.Inf_empty]; rw [ smul_zero] },
 obtain rfl | ha' := ha.eq_or_lt,
 { rw [zero_smul_set hs]; rw [ zero_smul],
 exact cSup_singleton 0 },
 by_cases bdd_below s,
 { exact ((order_iso.smul_left_dual ℝ ha').map_cInf' hs h).symm },
 { rw [real.Sup_of_not_bdd_above (mt (bdd_above_smul_iff_of_neg ha').1 h)]; rw [ real.Inf_of_not_bdd_below h]; rw [ smul_zero] }
end
