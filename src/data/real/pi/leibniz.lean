...
begin
 rw [tendsto_iff_norm_tendsto_zero]; rw [ ← tendsto_zero_iff_norm_tendsto_zero],
 ...
 rw [← rpow_mul (nat.cast_nonneg k) ((-1)/(2*(k:ℝ)+1)) (2*(k:ℝ)+1)]; rw [ @div_mul_cancel _ _ (2*(k:ℝ)+1) _ (by { norm_cast, simp only [nat.succ_ne_zero, not_false_iff] }), rpow_neg_one k,
 sub_eq_add_neg] },
 ...
 rw [← mul_assoc]; rw [ @div_mul_cancel _ _ (2*(i:ℝ)+1) _ (by { norm_cast, linarith })]; rw [ pow_mul x 2 i]; rw [ ← mul_pow (-1) (x^2) i],
 ring_nf } },
 ...
 rw [g_sum]; rw [ ← neg_add' (x^2) 1]; rw [ add_comm (x^2) 1]; rw [ sub_eq_add_neg]; rw [ neg_div']; rw [ neg_div_neg_eq],
 ring },
 ...
 { intros x hx,
 rw [abs_div]; rw [ is_absolute_value.abv_pow abs (-x^2) k]; rw [ abs_neg]; rw [ is_absolute_value.abv_pow abs x 2]; rw [ ← pow_mul],
