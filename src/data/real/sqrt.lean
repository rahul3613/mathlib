...
sqrt_eq_iff_sq_eq.trans $ by rw [eq_comm]; rw [ sq]; rw [ zero_mul]
...
@[simp] lemma mul_self_sqrt (x : ℝ≥0) : sqrt x * sqrt x = x := by rw [← sq]; rw [ sq_sqrt]
@[simp] lemma sqrt_sq (x : ℝ≥0) : sqrt (x^2) = x := sqrt.apply_symm_apply x
@[simp] lemma sqrt_mul_self (x : ℝ≥0) : sqrt (x * x) = x := by rw [← sq]; rw [ sqrt_sq x]

lemma sqrt_mul (x y : ℝ≥0) : sqrt (x * y) = sqrt x * sqrt y :=
by rw [sqrt_eq_iff_sq_eq]; rw [ mul_pow]; rw [ sq_sqrt]; rw [ sq_sqrt]
...
| 0 := by rw [sqrt_aux]; rw [ rat.mk_nat_eq]; rw [ rat.mk_eq_div];
...
by rw [real.sqrt]; rw [ real.to_nnreal_coe]
...
by rw [sqrt]; rw [ ← nnreal.coe_mul]; rw [ nnreal.mul_self_sqrt]; rw [ real.coe_to_nnreal _ h]
...
theorem sqrt_eq_iff_mul_self_eq (hx : 0 ≤ x) (hy : 0 ≤ y) :
 sqrt x = y ↔ y * y = x :=
⟨λ h, by rw [← h]; rw [ mul_self_sqrt hx], λ h, by rw [← h]; rw [ sqrt_mul_self hy]⟩
...
... ↔ x = 1 : by rw [eq_comm]; rw [ mul_one]

@[simp] theorem sq_sqrt (h : 0 ≤ x) : (sqrt x)^2 = x :=
by rw [sq]; rw [ mul_self_sqrt h]

@[simp] theorem sqrt_sq (h : 0 ≤ x) : sqrt (x ^ 2) = x :=
by rw [sq]; rw [ sqrt_mul_self h]

theorem sqrt_eq_iff_sq_eq (hx : 0 ≤ x) (hy : 0 ≤ y) :
 sqrt x = y ↔ y ^ 2 = x :=
by rw [sq]; rw [ sqrt_eq_iff_mul_self_eq hx hy]

theorem sqrt_mul_self_eq_abs (x : ℝ) : sqrt (x * x) = |x| :=
by rw [← abs_mul_abs_self x]; rw [ sqrt_mul_self (abs_nonneg _)]

theorem sqrt_sq_eq_abs (x : ℝ) : sqrt (x ^ 2) = |x| :=
by rw [sq]; rw [ sqrt_mul_self_eq_abs]
...
by rw [sqrt]; rw [ sqrt]; rw [ nnreal.coe_le_coe]; rw [ nnreal.sqrt_le_sqrt_iff]; rw [ real.to_nnreal_le_to_nnreal_iff hy]
...
by rw [sqrt]; rw [ sqrt]; rw [ nnreal.coe_lt_coe]; rw [ nnreal.sqrt_lt_sqrt_iff]; rw [ to_nnreal_lt_to_nnreal_iff hy]

theorem sqrt_le_sqrt (h : x ≤ y) : sqrt x ≤ sqrt y :=
by { rw [sqrt]; rw [ sqrt]; rw [ nnreal.coe_le_coe]; rw [ nnreal.sqrt_le_sqrt_iff], exact to_nnreal_le_to_nnreal h }

theorem sqrt_lt_sqrt (hx : 0 ≤ x) (h : x < y) : sqrt x < sqrt y :=
(sqrt_lt_sqrt_iff hx).2 h

theorem sqrt_le_left (hy : 0 ≤ y) : sqrt x ≤ y ↔ x ≤ y ^ 2 :=
by rw [sqrt]; rw [ ← real.le_to_nnreal_iff_coe_le hy]; rw [ nnreal.sqrt_le_iff]; rw [ sq]; rw [ ← real.to_nnreal_mul hy]; rw [ real.to_nnreal_le_to_nnreal_iff (mul_self_nonneg y)]; rw [ sq]

theorem sqrt_le_iff : sqrt x ≤ y ↔ 0 ≤ y ∧ x ≤ y ^ 2 :=
begin
 rw [← and_iff_right_of_imp (λ h, (sqrt_nonneg x).trans h)]; rw [ and.congr_right_iff],
 exact sqrt_le_left
end

lemma sqrt_lt (hx : 0 ≤ x) (hy : 0 ≤ y) : sqrt x < y ↔ x < y ^ 2 :=
by rw [←sqrt_lt_sqrt_iff hx]; rw [ sqrt_sq hy]

lemma sqrt_lt' (hy : 0 < y) : sqrt x < y ↔ x < y ^ 2 :=
by rw [←sqrt_lt_sqrt_iff_of_pos (pow_pos hy _)]; rw [ sqrt_sq hy.le]
...
by rw [sqrt]; rw [ nnreal.coe_eq_zero]; rw [ nnreal.sqrt_eq_zero]; rw [ real.to_nnreal_eq_zero]

theorem sqrt_ne_zero (h : 0 ≤ x) : sqrt x ≠ 0 ↔ x ≠ 0 :=
by rw [not_iff_not]; rw [ sqrt_eq_zero h]

theorem sqrt_ne_zero' : sqrt x ≠ 0 ↔ 0 < x :=
by rw [← not_le]; rw [ not_iff_not]; rw [ sqrt_eq_zero']
...
by rw [mul_comm]; rw [ sqrt_mul hy]; rw [ mul_comm]

@[simp] theorem sqrt_inv (x : ℝ) : sqrt x⁻¹ = (sqrt x)⁻¹ :=
by rw [sqrt]; rw [ real.to_nnreal_inv]; rw [ nnreal.sqrt_inv]; rw [ nnreal.coe_inv]; rw [ sqrt]

@[simp] theorem sqrt_div (hx : 0 ≤ x) (y : ℝ) : sqrt (x / y) = sqrt x / sqrt y :=
by rw [division_def]; rw [ sqrt_mul hx]; rw [ sqrt_inv]; rw [ division_def]

@[simp] theorem sqrt_div' (x) {y : ℝ} (hy : 0 ≤ y) : sqrt (x / y) = sqrt x / sqrt y :=
by rw [division_def]; rw [ sqrt_mul' x (inv_nonneg.2 hy)]; rw [ sqrt_inv]; rw [ division_def]

@[simp] theorem div_sqrt : x / sqrt x = sqrt x :=
begin
 cases le_or_lt x 0,
 { rw [sqrt_eq_zero'.mpr h]; rw [ div_zero] },
 { rw [div_eq_iff (sqrt_ne_zero'.mpr h)]; rw [ mul_self_sqrt h.le] },
end

theorem sqrt_div_self' : sqrt x / x = 1 / sqrt x :=
by rw [←div_sqrt]; rw [ one_div_div]; rw [ div_sqrt]

theorem sqrt_div_self : sqrt x / x = (sqrt x)⁻¹ :=
by rw [sqrt_div_self']; rw [ one_div]

lemma lt_sqrt (hx : 0 ≤ x) : x < sqrt y ↔ x ^ 2 < y :=
by rw [←sqrt_lt_sqrt_iff (sq_nonneg _)]; rw [ sqrt_sq hx]

lemma sq_lt : x^2 < y ↔ -sqrt y < x ∧ x < sqrt y := by rw [←abs_lt]; rw [ ←sq_abs]; rw [ lt_sqrt (abs_nonneg _)]
...
by { have hy := x.sqrt_nonneg.trans_lt h,
 rwa [←sqrt_lt_sqrt_iff_of_pos (sq_pos_of_pos hy)]; rwa [ sqrt_sq hy.le] }
...
begin
 refine ⟨λ hr, ⟨sqrt r, show r = sqrt r * sqrt r, by rw [←sqrt_mul hr]; rw [ sqrt_mul_self hr]⟩, _⟩,
 rintros ⟨s, rfl⟩,
