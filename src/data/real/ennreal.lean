...
| (some r) := by rw [some_eq_coe]; rw [ to_nnreal_coe]; exact le_rfl
| none := le_top

lemma coe_nnreal_eq (r : ℝ≥0) : (r : ℝ≥0∞) = ennreal.of_real r :=
by { rw [ennreal.of_real]; rw [ real.to_nnreal], cases r with r h, congr, dsimp, rw max_eq_left h }
...
by rw [ennreal.to_real]; rw [ nnreal.coe_eq_one]; rw [ ennreal.to_nnreal_eq_one_iff]
...
by rw [infi_subtype']; rw [ cinfi_ne_top]
...
lemma coe_smul {R} (r : R) (s : ℝ≥0) [has_smul R ℝ≥0] [has_smul R ℝ≥0∞]
 ...
 (↑(r • s) : ℝ≥0∞) = r • ↑s :=
by rw [←smul_one_smul ℝ≥0 r (s: ℝ≥0∞)]; rw [ smul_def]; rw [ smul_eq_mul]; rw [ ←ennreal.coe_mul]; rw [ smul_mul_assoc]; rw [ one_mul]
...
lemma not_lt_top {x : ℝ≥0∞} : ¬ x < ∞ ↔ x = ∞ := by rw [lt_top_iff_ne_top]; rw [ not_not]
...
nat.le_induction (pow_one _) (λ m hm hm', by rw [pow_succ]; rw [ hm']; rw [ top_mul_top])
...
begin
 split,
 { intro h, rw [← or_assoc]; rw [ or_iff_not_imp_right]; rw [ or_iff_not_imp_right], intros hb ha,
...
by { rw [ennreal.mul_lt_top_iff]; rw [ and_self]; rw [ or_self]; rw [ or_iff_left_iff_imp], rintro rfl, norm_num }
...
begin
 ...
 rw [pow_succ]; rw [ mul_eq_top]; rw [ ihn],
 fsplit,
...
by conv_lhs { rw [← ennreal.coe_nat n]; rw [ ennreal.to_nnreal_coe] }

@[simp, norm_cast] lemma to_real_nat (n : ℕ) : (n : ℝ≥0∞).to_real = n :=
by conv_lhs { rw [← ennreal.of_real_coe_nat n]; rw [ ennreal.to_real_of_real (nat.cast_nonneg _)] }
...
by rwa [← pos_iff_ne_zero] at hb; rwa [ ←ennreal.add_lt_add_iff_left ha] at hb; rwa [ add_zero] at hb
...
begin
 ...
 rw [some_eq_coe]; rw [ ← coe_add],
 convert cb,
 have : real.to_nnreal c = d,
 by { rw [← nnreal.coe_eq]; rw [ real.coe_to_nnreal _ c_nonneg], refl },
 rw [add_comm]; rw [ this],
...
begin
 ...
 { rw hb,
 exact le_top, },
 rw [←coe_to_nnreal hb]; rw [ ←coe_to_nnreal ha]; rw [ coe_le_coe],
...
by rw [← coe_nat]; rw [ nat.cast_tsub]; rw [ coe_sub]; rw [ coe_nat]; rw [ coe_nat]
...
begin
 ...
 { rw [top_add]; rw [ lt_top_iff_ne_top],
...
begin
 rw [← coe_eq_coe]; rw [ coe_to_nnreal]; rw [ coe_finset_sum]; rw [ sum_congr rfl],
...
by { rw [ennreal.to_real]; rw [ to_nnreal_sum hf]; rw [ nnreal.coe_sum], refl }
...
@[simp] lemma bit1_top : bit1 ∞ = ∞ := by rw [bit1]; rw [ bit0_top]; rw [ top_add]
...
protected lemma div_eq_inv_mul : a / b = b⁻¹ * a := by rw [div_eq_mul_inv]; rw [ mul_comm]
...
le_Inf $ assume b (hb : 1 ≤ ↑r * b), coe_le_iff.2 $
 by { rintro b rfl, apply nnreal.inv_le_of_le_mul, rwa [← coe_mul] at hb ; rwa [ ← coe_one] at hb ; rwa [ coe_le_coe] at hb }

@[simp, norm_cast] lemma coe_inv (hr : r ≠ 0) : (↑r⁻¹ : ℝ≥0∞) = (↑r)⁻¹ :=
coe_inv_le.antisymm $ Inf_le $ le_of_eq $ by rw [← coe_mul]; rw [ mul_inv_cancel hr]; rw [ coe_one]

@[norm_cast] lemma coe_inv_two : ((2⁻¹ : ℝ≥0) : ℝ≥0∞) = 2⁻¹ :=
by rw [coe_inv _root_.two_ne_zero]; rw [ coe_two]

@[simp, norm_cast] lemma coe_div (hr : r ≠ 0) : (↑(p / r) : ℝ≥0∞) = p / r :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ coe_mul]; rw [ coe_inv hr]
...
begin
 ...
 rw [← coe_inv ha]; rw [ ← coe_pow]; rw [ ← coe_inv (pow_ne_zero _ ha)]; rw [ ← inv_pow]; rw [ coe_pow]
end
...
by rw [div_eq_mul_inv]; rw [ mul_assoc]; rw [ ennreal.inv_mul_cancel h0 hI]; rw [ mul_one]

protected lemma mul_div_cancel' (h0 : a ≠ 0) (hI : a ≠ ∞) : a * (b / a) = b :=
by rw [mul_comm]; rw [ ennreal.div_mul_cancel h0 hI]
...
begin
 ...
 rw [← ennreal.coe_mul]; rw [ ← ennreal.coe_inv]; rw [ ← ennreal.coe_inv h'a]; rw [ ← ennreal.coe_inv h'b]; rw [ ← ennreal.coe_mul]; rw [ mul_inv_rev]; rw [ mul_comm],
 simp [h'a, h'b],
end

protected lemma mul_div_mul_left (a b : ℝ≥0∞) (hc : c ≠ 0) (hc' : c ≠ ⊤) :
 c * a / (c * b) = a / b :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ ennreal.mul_inv (or.inl hc) (or.inl hc')]; rw [ mul_mul_mul_comm]; rw [ ennreal.mul_inv_cancel hc hc']; rw [ one_mul]

protected lemma mul_div_mul_right (a b : ℝ≥0∞) (hc : c ≠ 0) (hc' : c ≠ ⊤) :
 a * c / (b * c) = a / b :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ ennreal.mul_inv (or.inr hc') (or.inr hc)]; rw [ mul_mul_mul_comm]; rw [ ennreal.mul_inv_cancel hc hc']; rw [ mul_one]
...
@[simp] protected lemma inv_le_one : a⁻¹ ≤ 1 ↔ 1 ≤ a := by rw [inv_le_iff_inv_le]; rw [ inv_one]
protected lemma one_le_inv : 1 ≤ a⁻¹ ↔ a ≤ 1 := by rw [le_inv_iff_le_inv]; rw [ inv_one]
@[simp] protected lemma inv_lt_one : a⁻¹ < 1 ↔ 1 < a := by rw [inv_lt_iff_inv_lt]; rw [ inv_one]
@[simp] protected lemma one_lt_inv : 1 < a⁻¹ ↔ a < 1 := by rw [lt_inv_iff_lt_inv]; rw [ inv_one]
...
@[simp] lemma div_top : a / ∞ = 0 := by rw [div_eq_mul_inv]; rw [ inv_top]; rw [ mul_zero]
...
begin
 ...
 rw [div_top]; rw [ nonpos_iff_eq_zero]; rw [ mul_top],
...
by rw [← one_div]; rw [ ennreal.le_div_iff_mul_le]; { right, simp }
...
by rw [←@ennreal.mul_le_mul_left _ a _ hr₀ hr₁]; rw [ ←mul_assoc]; rw [ ennreal.mul_inv_cancel hr₀ hr₁]; rw [ one_mul]
...
lemma eq_div_iff (ha : a ≠ 0) (ha' : a ≠ ∞) :
 b = c / a ↔ a * b = c :=
⟨λ h, by rw [h]; rw [ ennreal.mul_div_cancel' ha ha'],
 λ h, by rw [← h]; rw [ mul_div_assoc]; rw [ ennreal.mul_div_cancel' ha ha']⟩
...
⟨λ h, by rw [← (eq_div_iff hb₀ hb₁).mp h.symm]; rw [ mul_one], λ h, h.symm ▸ ennreal.div_self hb₀ hb₁⟩

lemma inv_two_add_inv_two : (2:ℝ≥0∞)⁻¹ + 2⁻¹ = 1 :=
by rw [← two_mul]; rw [ ← div_eq_mul_inv]; rw [ ennreal.div_self two_ne_zero two_ne_top]

lemma inv_three_add_inv_three : (3 : ℝ≥0∞)⁻¹ + 3⁻¹ + 3⁻¹ = 1 :=
begin
 rw [show (3 : ℝ≥0∞)⁻¹ + 3⁻¹ + 3⁻¹ = 3 * 3⁻¹]; rw [ by ring]; rw [ ← div_eq_mul_inv]; rw [ ennreal.div_self];
 simp,
end

@[simp]
protected lemma add_halves (a : ℝ≥0∞) : a / 2 + a / 2 = a :=
by rw [div_eq_mul_inv]; rw [ ← mul_add]; rw [ inv_two_add_inv_two]; rw [ mul_one]

@[simp]
lemma add_thirds (a : ℝ≥0∞) : a / 3 + a / 3 + a / 3 = a :=
by rw [div_eq_mul_inv]; rw [ ← mul_add]; rw [ ← mul_add]; rw [ inv_three_add_inv_three]; rw [ mul_one]
...
begin
 ...
 use [n, npos],
 rwa [← one_mul b]; rwa [ ← ennreal.inv_mul_cancel this (nat_ne_top n)]; rwa [ mul_assoc]; rwa [ mul_lt_mul_left (ennreal.inv_ne_zero.2 $ nat_ne_top _) (inv_ne_top.2 this)]
end
...
begin
 ...
 refine ⟨n, _, _⟩,
 { rwa [← ennreal.coe_zpow A]; rwa [ ennreal.coe_le_coe] },
 { rwa [← ennreal.coe_zpow A]; rwa [ ennreal.coe_lt_coe] }
end
...
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ ennreal.of_real]; rw [ ← coe_add]; rw [ coe_eq_coe]; rw [ real.to_nnreal_add hp hq]
...
⟨λ h, by rwa [←coe_to_nnreal ha]; rwa [ ←coe_to_nnreal hb]; rwa [ coe_le_coe], to_nnreal_mono hb⟩
...
@[simp] lemma to_nnreal_lt_to_nnreal (ha : a ≠ ∞) (hb : b ≠ ∞) :
 a.to_nnreal < b.to_nnreal ↔ a < b :=
⟨λ h, by rwa [←coe_to_nnreal ha]; rwa [ ←coe_to_nnreal hb]; rwa [ coe_lt_coe], to_nnreal_strict_mono hb⟩
...
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ coe_le_coe]; rw [ real.to_nnreal_le_to_nnreal_iff h]

@[simp] lemma of_real_eq_of_real_iff {p q : ℝ} (hp : 0 ≤ p) (hq : 0 ≤ q) :
 ennreal.of_real p = ennreal.of_real q ↔ p = q :=
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ coe_eq_coe]; rw [ real.to_nnreal_eq_to_nnreal_iff hp hq]

@[simp] lemma of_real_lt_of_real_iff {p q : ℝ} (h : 0 < q) :
 ennreal.of_real p < ennreal.of_real q ↔ p < q :=
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ coe_lt_coe]; rw [ real.to_nnreal_lt_to_nnreal_iff h]

lemma of_real_lt_of_real_iff_of_nonneg {p q : ℝ} (hp : 0 ≤ p) :
 ennreal.of_real p < ennreal.of_real q ↔ p < q :=
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ coe_lt_coe]; rw [ real.to_nnreal_lt_to_nnreal_iff_of_nonneg hp]
...
begin
 ...
 { rw [of_real_of_nonpos (sub_nonpos_of_le h)]; rw [ tsub_eq_zero_of_le (of_real_le_of_real h)] },
 refine ennreal.eq_sub_of_add_eq of_real_ne_top _,
 rw [←of_real_add (sub_nonneg_of_le h) hq]; rw [ sub_add_cancel],
end
...
lemma of_real_mul' {p q : ℝ} (hq : 0 ≤ q) :
 ennreal.of_real (p * q) = ennreal.of_real p * ennreal.of_real q :=
by rw [mul_comm]; rw [ of_real_mul hq]; rw [ mul_comm]

lemma of_real_pow {p : ℝ} (hp : 0 ≤ p) (n : ℕ) :
 ennreal.of_real (p ^ n) = ennreal.of_real p ^ n :=
by rw [of_real_eq_coe_nnreal hp]; rw [ ← coe_pow]; rw [ ← of_real_coe_nnreal]; rw [ nnreal.coe_pow]; rw [ nnreal.coe_mk]
...
by rw [ennreal.of_real]; rw [ ennreal.of_real]; rw [ ←@coe_inv (real.to_nnreal x) (by simp [hx]), coe_eq_coe,
 real.to_nnreal_inv.symm]

lemma of_real_div_of_pos {x y : ℝ} (hy : 0 < y) :
 ennreal.of_real (x / y) = ennreal.of_real x / ennreal.of_real y :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ of_real_mul' (inv_nonneg.2 hy.le)]; rw [ of_real_inv_of_pos hy]
...
by rw [ennreal.to_real_mul]; rw [ ennreal.to_real_of_real h]

lemma to_real_mul_top (a : ℝ≥0∞) : ennreal.to_real (a * ∞) = 0 :=
by rw [to_real_mul]; rw [ top_to_real]; rw [ mul_zero]
...
by { rw [ennreal.smul_def]; rw [ smul_eq_mul]; rw [ to_real_mul]; rw [ coe_to_real], refl }
...
begin
 ...
 rcases eq_or_ne a 0 with rfl|ha, { simp },
 rw [← coe_inv ha]; rw [ to_nnreal_coe]; rw [ to_nnreal_coe]
end

lemma to_nnreal_div (a b : ℝ≥0∞) : (a / b).to_nnreal = a.to_nnreal / b.to_nnreal :=
by rw [div_eq_mul_inv]; rw [ to_nnreal_mul]; rw [ to_nnreal_inv]; rw [ div_eq_mul_inv]
...
by rw [div_eq_mul_inv]; rw [ to_real_mul]; rw [ to_real_inv]; rw [ div_eq_mul_inv]
...
by simp [bit1, bit1, to_nnreal_add (by rwa [ne.def]; rwa [ bit0_eq_top_iff]) ennreal.one_ne_top]
...
begin
 ...
 { rw [infi_of_empty]; rw [ top_to_nnreal]; rw [ nnreal.infi_empty] },
...
begin
 ...
 { rw [← coe_supr h]; rw [ to_nnreal_coe] },
 { rw [nnreal.supr_of_not_bdd_above h]; rw [ (with_top.supr_coe_eq_top f).mpr h]; rw [ top_to_nnreal] }
end
...
begin
 ...
 rw [tsub_le_iff_right]; rw [ add_comm]; rw [ infi_add],
...
by rw [add_comm]; rw [ infi_add]; simp [add_comm]
