...
by { rw [sub_eq_add_neg]; rw [ of_cauchy_add]; rw [ of_cauchy_neg], refl }
...
| ⟨a⟩ ⟨b⟩ := by { rw [sub_eq_add_neg]; rw [ ←cauchy_neg]; rw [ ←cauchy_add], refl }
...
by rw [← mk_zero]; rw [ mk_lt]; exact iff_of_eq (congr_arg pos (sub_zero f))
...
{ inv := has_inv.inv,
 ...
 by rw [←of_cauchy_rat_cast]; rw [ rat.cast_mk']; rw [ of_cauchy_mul]; rw [ of_cauchy_inv]; rw [ of_cauchy_nat_cast]; rw [ of_cauchy_int_cast],
...
begin
 ...
 rwa [← sub_eq_add_neg] at this; rwa [ sub_self_div_two] at this; rwa [ sub_apply] at this; rwa [ sub_add_sub_cancel] at this
end
...
begin
 ...
 simp [-sub_eq_add_neg],
 rwa [lt_div_iff ((nat.cast_pos.2 n0):((_:ℝ) < _))]; rwa [ sub_mul]; rwa [ _root_.inv_mul_cancel],
 exact ne_of_gt (nat.cast_pos.2 n0) },
 have hg : is_cau_seq abs (λ n, f n / n : ℕ → ℚ),
 { intros ε ε0,
 suffices : ∀ j k ≥ ⌈ε⁻¹⌉₊, (f j / j - f k / k : ℚ) < ε,
 { refine ⟨_, λ j ij, abs_lt.2 ⟨_, this _ ij _ le_rfl⟩⟩,
 rw [neg_lt]; rw [ neg_sub], exact this _ le_rfl _ ij },
 intros j ij k ik,
 ...
 rwa [le_sub_comm]; rwa [ inv_le ((nat.cast_pos.2 n0):((_:ℝ) < _)) xz] },
...
protected theorem is_glb_Inf (S : set ℝ) (h₁ : S.nonempty) (h₂ : bdd_below S) :
 is_glb S (Inf S) :=
begin
 rw [Inf_def]; rw [ ← is_lub_neg']; rw [ neg_neg],
