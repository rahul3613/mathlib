...
by rw [← nnreal.coe_zero]; rw [ nnreal.coe_eq]

@[simp, norm_cast] protected lemma coe_eq_one (r : ℝ≥0) : ↑r = (1 : ℝ) ↔ r = 1 :=
by rw [← nnreal.coe_one]; rw [ nnreal.coe_eq]
...
begin
 rw [←nnreal.coe_eq]; rw [ nnreal.coe_sum]; rw [ real.coe_to_nnreal _ (finset.sum_nonneg hf)],
...
lemma _root_.real.to_nnreal_prod_of_nonneg {α} {s : finset α} {f : α → ℝ}
 ...
 real.to_nnreal (∏ a in s, f a) = ∏ a in s, real.to_nnreal (f a) :=
begin
 rw [←nnreal.coe_eq]; rw [ nnreal.coe_prod]; rw [ real.coe_to_nnreal _ (finset.prod_nonneg hf)],
...
by rw [supr]; rw [ supr]; rw [ coe_Sup]; rw [ set.range_comp]
...
by rw [← nnreal.coe_eq_zero]; rw [ coe_Inf]; rw [ set.image_empty]; rw [ real.Inf_empty]

@[norm_cast] lemma coe_infi {ι : Sort*} (s : ι → ℝ≥0) : (↑(⨅ i, s i) : ℝ) = ⨅ i, (s i) :=
by rw [infi]; rw [ infi]; rw [ coe_Inf]; rw [ set.range_comp]

lemma le_infi_add_infi {ι ι' : Sort*} [nonempty ι] [nonempty ι'] {f : ι → ℝ≥0} {g : ι' → ℝ≥0}
 {a : ℝ≥0} (h : ∀ i j, a ≤ f i + g j) : a ≤ (⨅ i, f i) + ⨅ j, g j :=
begin
 rw [← nnreal.coe_le_coe]; rw [ nnreal.coe_add]; rw [ coe_infi]; rw [ coe_infi],
...
by rw [← nnreal.coe_le_coe]; rw [ real.coe_to_nnreal p hp]
...
by rw [← nnreal.coe_lt_coe]; rw [ real.coe_to_nnreal r ha]

lemma lt_to_nnreal_iff_coe_lt {r : ℝ≥0} {p : ℝ} : r < real.to_nnreal p ↔ ↑r < p :=
begin
 cases le_total 0 p,
 { rw [← nnreal.coe_lt_coe]; rw [ real.coe_to_nnreal p h] },
...
begin
 ...
 { rw [to_nnreal_of_nonpos hr]; rw [ to_nnreal_of_nonpos]; rw [ bit0_zero],
...
by rw [← nnreal.coe_eq]; rw [ nnreal.coe_pow]; rw [ real.coe_to_nnreal _ (pow_nonneg hx _)]; rw [ real.coe_to_nnreal x hx]
...
by rw [mul_eq_mul_left_iff]; rw [ or_iff_left h]
...
begin
 ...
 rw [to_nnreal_eq_zero.2 hq]; rw [ to_nnreal_eq_zero.2 hpq]; rw [ mul_zero] }
end
...
by rw [← mul_le_mul_left (pos_iff_ne_zero.2 h)]; rw [ mul_inv_cancel h]
...
by rw [← mul_le_mul_left (pos_iff_ne_zero.2 h)]; rw [ mul_inv_cancel h]; rw [ mul_comm]

@[simp] lemma lt_inv_iff_mul_lt {r p : ℝ≥0} (h : p ≠ 0) : (r < p⁻¹ ↔ r * p < 1) :=
by rw [← mul_lt_mul_left (pos_iff_ne_zero.2 h)]; rw [ mul_inv_cancel h]; rw [ mul_comm]

lemma mul_le_iff_le_inv {a b r : ℝ≥0} (hr : r ≠ 0) : r * a ≤ b ↔ a ≤ r⁻¹ * b :=
have 0 < r, from lt_of_le_of_ne (zero_le r) hr.symm,
by rw [← mul_le_mul_left (inv_pos.mpr this)]; rw [ ← mul_assoc]; rw [ inv_mul_cancel hr]; rw [ one_mul]
...
le_of_forall_ge_of_dense $ assume a ha,
 ...
 have hx' : x⁻¹ ≠ 0, by rwa [(≠)]; rwa [ inv_eq_zero],
 have a * x⁻¹ < 1, by rwa [← lt_inv_iff_mul_lt hx']; rwa [ inv_inv],
 have (a * x⁻¹) * x ≤ y, from h _ this,
 by rwa [mul_assoc] at this; rwa [ inv_mul_cancel hx] at this; rwa [ mul_one] at this
...
begin
 ...
 rw [←nnreal.coe_inv]; rw [ real.to_nnreal_coe], },
 { have hx' := le_of_not_ge hx,
 rw [to_nnreal_eq_zero.mpr hx']; rw [ inv_zero]; rw [ to_nnreal_eq_zero.mpr (inv_nonpos.mpr hx')], },
end

lemma _root_.real.to_nnreal_div {x y : ℝ} (hx : 0 ≤ x) :
 real.to_nnreal (x / y) = real.to_nnreal x / real.to_nnreal y :=
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ ← real.to_nnreal_inv]; rw [ ← real.to_nnreal_mul hx]

lemma _root_.real.to_nnreal_div' {x y : ℝ} (hy : 0 ≤ y) :
 real.to_nnreal (x / y) = real.to_nnreal x / real.to_nnreal y :=
by rw [div_eq_inv_mul]; rw [ div_eq_inv_mul]; rw [ real.to_nnreal_mul (inv_nonneg.2 hy)]; rw [ real.to_nnreal_inv]

lemma inv_lt_one_iff {x : ℝ≥0} (hx : x ≠ 0) : x⁻¹ < 1 ↔ 1 < x :=
by rwa [← one_div]; rwa [ div_lt_iff hx]; rwa [ one_mul]
...
by { rw [← nnreal.coe_eq]; rw [ coe_infi], exact real.cinfi_empty _, }

@[simp] lemma infi_const_zero {α : Sort*} : (⨅ i : α, (0 : ℝ≥0)) = 0 :=
by { rw [← nnreal.coe_eq]; rw [ coe_infi], exact real.cinfi_const_zero, }

lemma infi_mul (f : ι → ℝ≥0) (a : ℝ≥0) : infi f * a = ⨅ i, f i * a :=
begin
 rw [← nnreal.coe_eq]; rw [ nnreal.coe_mul]; rw [ coe_infi]; rw [ coe_infi],
...
begin
 rw [← nnreal.coe_eq]; rw [ nnreal.coe_mul]; rw [ nnreal.coe_supr]; rw [ nnreal.coe_supr],
...
begin
 ...
 { rw [mem_Icc] at hz; rw [ real.to_nnreal_of_nonpos hy₀] at hz; rw [ nonpos_iff_eq_zero] at hz,
...
by { ext, rw [nnreal.coe_nat_cast]; rw [ int.cast_nat_abs]; rw [ real.coe_nnabs] }
