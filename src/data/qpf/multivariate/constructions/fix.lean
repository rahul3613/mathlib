...
by rw [recF]; rw [ mvpfunctor.W_rec_eq]; refl
...
begin
 ...
 intros a f' f,
 rw [recF_eq]; rw [ q.P.W_dest'_W_mk]; rw [ mvpfunctor.map_eq]; rw [ append_fun_comp_split_fun]; rw [ typevec.id_comp]
end
...
by rw [Wrepr]; rw [ recF_eq']; rw [ q.P.W_dest'_W_mk]; refl
...
begin
 ...
 { apply Wequiv.abs',
 rw [Wrepr_W_mk]; rw [ q.P.W_dest'_W_mk']; rw [ q.P.W_dest'_W_mk']; rw [ abs_repr] },
 rw [q.P.map_eq]; rw [ mvpfunctor.W_mk']; rw [ append_fun_comp_split_fun]; rw [ id_comp],
...
begin
 intro h, induction h,
 case mvqpf.Wequiv.ind : a f' f₀ f₁ h ih
 { rw [q.P.W_map_W_mk]; rw [ q.P.W_map_W_mk], apply Wequiv.ind, apply ih },
 case mvqpf.Wequiv.abs : a₀ f'₀ f₀ a₁ f'₁ f₁ h
 { rw [q.P.W_map_W_mk]; rw [ q.P.W_map_W_mk], apply Wequiv.abs,
 show abs (q.P.obj_append1 a₀ (g ⊚ f'₀) (λ x, q.P.W_map g (f₀ x))) =
 abs (q.P.obj_append1 a₁ (g ⊚ f'₁) (λ x, q.P.W_map g (f₁ x))),
 rw [←q.P.map_obj_append1]; rw [ ←q.P.map_obj_append1]; rw [ abs_map]; rw [ abs_map]; rw [ h] },
 case mvqpf.Wequiv.trans : x y z e₁ e₂ ih₁ ih₂
...
begin
 ...
 rw [mvpfunctor.map_eq]; rw [ recF_eq']; rw [ ←mvpfunctor.map_eq]; rw [ mvpfunctor.W_dest'_W_mk'],
 rw [←mvpfunctor.comp_map]; rw [ abs_map]; rw [ ←h]; rw [ abs_repr]; rw [ ←append_fun_comp]; rw [ id_comp]; rw [ this]
end
...
have fix.mk (abs ⟨a, q.P.append_contents f' (λ x, ⟦f x⟧)⟩) = ⟦Wrepr (q.P.W_mk a f' f)⟧,
 ...
 rw [mvpfunctor.W_dest'_W_mk']; rw [ abs_map]; rw [ abs_repr]; rw [ ←abs_map]; rw [ mvpfunctor.map_eq],
 conv { to_rhs, rw [Wrepr_W_mk]; rw [ q.P.W_dest'_W_mk']; rw [ abs_repr]; rw [ mvpfunctor.map_eq] },
 congr' 2, rw [mvpfunctor.append_contents]; rw [ mvpfunctor.append_contents],
 rw [append_fun]; rw [ append_fun]; rw [ ←split_fun_comp]; rw [ ←split_fun_comp],
 reflexivity
 end,
...
begin
 ...
 congr' 2,
 rw [mvpfunctor.append_contents]; rw [ append_fun]; rw [ append_fun]; rw [ ←split_fun_comp]; rw [ ←split_fun_comp],
...
begin
 ...
 apply fix.ind_rec,
 intro x, dsimp,
 rw [fix.dest]; rw [ fix.rec_eq]; rw [ ←comp_map]; rw [ ←append_fun_comp]; rw [ id_comp],
 intro h, rw h,
 show fix.mk (append_fun id id <$$> x) = fix.mk x,
 rw [append_fun_id_id]; rw [ mvfunctor.id_map]
end
...
begin
 ...
 rw [←append_fun_comp]; rw [ id_comp],
 have : fix.mk ∘ fix.dest = id, {ext x, apply fix.mk_dest },
 rw [this]; rw [ append_fun_id_id]
end
...
have x = y.1,
 ...
 congr, conv { to_rhs, rw [← ih] }, rw [mvfunctor.map_map]; rw [← append_fun_comp]; rw [id_comp], },
cast (by rw this) y.2
