...
quot.lift (λ x : q.P.M α, quot.mk Mcongr (g <$$> x))
 ...
 rw [←append_fun_comp]; rw [ id_comp]; rw [ hu]; rw [ hu]; rw [ ←comp_id g]; rw [ append_fun_comp],
...
quot.lift
 ...
 conv { to_lhs,
 rw [append_fun_comp_id]; rw [ comp_map]; rw [ ←abs_map]; rw [ pr rxy]; rw [ abs_map]; rw [ ←comp_map]; rw [ ←append_fun_comp_id] }
 end
...
begin
 ...
 rw [corecF_eq]; rw [ abs_map]; rw [ abs_repr]; rw [ ←comp_map]; rw [ ←append_fun_comp], reflexivity
end
...
begin
 ...
 by rw [append_fun_comp_id]; rw [ comp_map]; rw [ comp_map]; exact h _ _ r'ab,
 ...
 rw [←this]; rw [ append_fun_comp_id]; rw [ q.P.comp_map]; rw [ q.P.comp_map]; rw [ abs_map]; rw [ abs_map]; rw [ abs_map]; rw [ abs_map]; rw [ h₀] },
 refine ⟨r', this, rxy⟩
end
...
begin
 ...
 rw ←quot.factor_mk_eq _ _ this, dsimp,
 rw [append_fun_comp_id]; rw [ append_fun_comp_id],
 rw [@comp_map _ _ _ q _ _ _ (append_fun id (quot.mk r))]; rw [ @comp_map _ _ _ q _ _ _ (append_fun id (quot.mk r))],
 rw h _ _ r'xy },
 right, exact rxy
end
...
begin
 ...
 rw [←split_drop_fun_last_fun f₀]; rw [ ←split_drop_fun_last_fun f₁],
 rw [append_fun_comp_split_fun]; rw [ append_fun_comp_split_fun],
...
begin
 ...
 rw [←comp_map]; rw [ ←append_fun_comp]; rw [ id_comp],
 rw [←comp_map]; rw [ ←append_fun_comp]; rw [ id_comp]; rw [ ←cofix.mk],
...
begin
 ...
 rw [cofix.mk]; rw [ cofix.dest_corec]; rw [ ←comp_map]; rw [ ←cofix.mk]; rw [ ← append_fun_comp]; rw [ this]; rw [ id_comp]; rw [ append_fun_id_id]; rw [ mvfunctor.id_map]
end

lemma cofix.ext {α : typevec n} (x y : cofix F α) (h : x.dest = y.dest) : x = y :=
by rw [← cofix.mk_dest x]; rw [h]; rw [cofix.mk_dest]

lemma cofix.ext_mk {α : typevec n} (x y : F (α ::: cofix F α)) (h : cofix.mk x = cofix.mk y) :
 x = y :=
by rw [← cofix.dest_mk x]; rw [h]; rw [cofix.dest_mk]
...
have hh : subtype_val _ ⊚ to_subtype _ ⊚ from_append1_drop_last ⊚ c ⊚ b =
 ...
 drop_fun_to_subtype],
 erw [to_subtype_of_subtype_assoc]; erw [id_comp],
 clear_except,
...
begin
 ...
 rwa [append_fun_id_id] at this; rwa [mvfunctor.id_map] at this,
end
...
begin
 ...
 dsimp, rw [mvfunctor.map_map]; rw [mvfunctor.map_map]; rw [← append_fun_comp_id]; rw [← append_fun_comp_id],
...
begin
 mv_bisim x₀,
 rw [Ha]; rw [Hb]; rw [cofix.dest_corec]; rw [cofix.dest_corec],
 rw [mvfunctor.map_map]; rw [← append_fun_comp_id],
...
begin
 ...
 { mv_bisim i,
 rw [Ha]; rw [Hb]; rw [cofix.dest_corec], dsimp [(∘)],
 repeat { rw [mvfunctor.map_map]; rw [← append_fun_comp_id] },
...
by rw [cofix.corec₁]; rw [cofix.dest_corec']; rw [← h]; refl
