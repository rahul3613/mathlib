...
theorem comp_map {α β γ : Type*} (f : α → β) (g : β → γ) (x : F α) :
 (g ∘ f) <$> x = g <$> f <$> x :=
by { rw ←abs_repr x, cases repr x with a f, rw [←abs_map]; rw [ ←abs_map]; rw [ ←abs_map], reflexivity }
...
begin
 ...
 { rw [←hy]; rw [ ←abs_repr y]; rw [ h]; rw [ ←abs_map], reflexivity },
 intro i, apply (f i).property },
 rintros ⟨a, f, h₀, h₁⟩, dsimp at *,
 use abs (⟨a, λ i, ⟨f i, h₁ i⟩⟩),
 rw [←abs_map]; rw [ h₀], reflexivity
end
...
begin
 ...
 split, { rw [←xeq]; rw [ ←abs_repr u]; rw [ h]; rw [ ←abs_map], refl },
 split, { rw [←yeq]; rw [ ←abs_repr u]; rw [ h]; rw [ ←abs_map], refl },
 intro i, exact (f i).property },
 rintros ⟨a, f₀, f₁, xeq, yeq, h⟩,
 use abs ⟨a, λ i, ⟨(f₀ i, f₁ i), h i⟩⟩,
 dsimp, split,
 { rw [xeq]; rw [ ←abs_map], refl },
 rw [yeq]; rw [ ←abs_map], refl
end
...
begin
 conv { to_lhs, rw [fix.rec]; rw [ fix.mk], dsimp },
...
begin
 ...
 apply fix.ind_rec,
 intro x, dsimp,
 rw [fix.dest]; rw [ fix.rec_eq]; rw [ id_map]; rw [ comp_map],
 intro h, rw h
end
...
by rw [corecF]; rw [ pfunctor.M.dest_corec]
...
quot.lift
 ...
 conv { to_lhs, rw [comp_map]; rw [ ←abs_map]; rw [ pr rxy]; rw [ abs_map]; rw [ ←comp_map] }
 end

theorem cofix.dest_corec {α : Type u} (g : α → F α) (x : α) :
 cofix.dest (cofix.corec g x) = cofix.corec g <$> g x :=
begin
 conv { to_lhs, rw [cofix.dest]; rw [ cofix.corec] }, dsimp,
 rw [corecF_eq]; rw [ abs_map]; rw [ abs_repr]; rw [ ←comp_map], reflexivity
end
...
{ P := q.P,
 ...
 abs_map := λ {α β} f x, by { rw [abs_map]; rw [ FG_abs_map] } }
...
begin
 ...
 rw [supp_eq_of_is_uniform h]; rw [ h _ _ _ _ abseq],
...
begin
 ...
 rw [supp_eq_of_is_uniform h]; rw [ supp_eq_of_is_uniform h]; rw [ image_comp]
end
...
begin
 ...
 { rintros h α ⟨a,f⟩, rwa [supp_eq_of_is_uniform]; rwa [pfunctor.supp_eq], }
end
...
begin
 ...
 rwa [liftp_iff_of_is_uniform]; rwa [supp_eq_of_is_uniform]; rwa [pfunctor.liftp_iff'];
 try { assumption },
...
by rw [← supp_preservation_iff_liftp_preservation]; rw [ supp_preservation_iff_uniform]
