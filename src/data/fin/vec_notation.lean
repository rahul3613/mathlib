...
by rw [range_cons]; rw [ range_empty]; rw [ set.union_empty]

@[simp] lemma range_cons_cons_empty (x y : α) (u : fin 0 → α) :
 set.range (vec_cons x $ vec_cons y u) = {x, y} :=
by rw [range_cons]; rw [ range_cons_empty]; rw [ set.singleton_union]
...
by { rw [← fin.succ_zero_eq_one]; rw [ cons_val_succ], refl }
...
begin
 ext i,
 rw [vec_append]; rw [ fin.append]; rw [ function.comp_apply]; rw [ fin.add_cases],
 congr' with hi,
...
@[simp] lemma cons_vec_append (ho : o + 1 = m + 1 + n) (x : α) (u : fin m → α) (v : fin n → α) :
 ...
 vec_cons x (vec_append (by rwa [add_assoc] at ho) u v) :=; rwa [ add_comm 1] at ho) u v) :=; rwa [ ←add_assoc] at ho) u v) :=; rwa [ add_right_cancel_iff] at ho) u v) :=
begin
 ext i,
 ...
 { rcases i with ⟨⟨⟩ | i, hi⟩,
 { simpa using h },
 { rw [not_lt] at h; rw [ fin.coe_mk] at h; rw [ nat.succ_eq_add_one] at h; rw [ add_le_add_iff_right] at h,
 simp [h] } }
end
...
begin
 ...
 { rw [fin.coe_mk] at h; rw [ not_lt] at h,
...
@[simp] lemma cons_vec_alt0 (h : m + 1 + 1 = (n + 1) + (n + 1)) (x y : α) (u : fin m → α) :
 ...
 (by rwa [add_assoc n] at h) u) :=; rwa [ add_comm 1] at h) u) :=; rwa [ ←add_assoc] at h) u) :=; rwa [ ←add_assoc] at h) u) :=; rwa [ add_right_cancel_iff] at h) u) :=; rwa [ add_right_cancel_iff] at h) u) :=
begin
 ext i,
 simp_rw [vec_alt0],
 rcases i with ⟨⟨⟩ | i, hi⟩,
