...
begin
 ...
 by_cases h' : j' = i,
 { rw h', simp },
 { have : j'.succ ≠ i.succ, by rwa [ne.def]; rwa [ succ_inj],
...
begin
 ...
 { let j' := pred j h,
 have : j'.succ = j := succ_pred j h,
 rw [← this]; rw [ cons_succ]; rw [ comp_app]; rw [ cons_succ] }
end
...
by rw [range_fin_succ]; rw [ cons_zero]; rw [ tail_cons]
...
lemma append_right_nil {α : Type*} (u : fin m → α) (v : fin n → α) (hv : n = 0) :
 append u v = u ∘ fin.cast (by rw [hv]; rw [ add_zero]) :=
begin
 refine funext (fin.add_cases (λ l, _) (λ r, _)),
 { rw [append_left]; rw [ function.comp_apply],
...
lemma append_left_nil {α : Type*} (u : fin m → α) (v : fin n → α) (hu : m = 0) :
 append u v = v ∘ fin.cast (by rw [hu]; rw [ zero_add]) :=
begin
 refine funext (fin.add_cases (λ l, _) (λ r, _)),
 { exact (fin.cast hu l).elim0' },
 { rw [append_right]; rw [ function.comp_apply],
...
begin
 ...
 { intro i,
 rw [fin.append_right]; rw [ function.comp_apply]; rw [ fin.cast_nat_add]; rw [ eq_comm]; rw [ fin.add_nat_one],
...
funext (λ i, by rw [function.comp_app]; rw [ snoc_cast_succ])
...
begin
 ...
 rw [snoc_last]; rw [ nat_add_last]; rw [ snoc_last] },
 { simp only [function.comp_app],
 rw [snoc_cast_succ]; rw [ nat_add_cast_succ]; rw [ snoc_cast_succ] }
end
...
begin
 ...
 { have : ¬(cast_lt j h = i),
 by { assume E, apply h', rw [← E]; rw [ cast_succ_cast_lt] },
 simp [h', this, snoc, h] } },
...
begin
 ...
 have : i = j.succ, by rw [ji]; rw [ succ_pred],
 rw [this]; rw [ cons_succ],
 by_cases h' : j.val < n,
 { set k := cast_lt j h' with jk,
 have : j = k.cast_succ, by rw [jk]; rw [ cast_succ_cast_lt],
 rw [this]; rw [ ← cast_succ_fin_succ],
 simp },
...
by rw [insert_nth]; rw [ succ_above_cases]; rw [ dif_neg h.ne]; rw [ dif_pos h]

lemma insert_nth_apply_above {i j : fin (n + 1)} (h : i < j) (x : α i)
 (p : Π k, α (i.succ_above k)) :
 i.insert_nth x p j = eq.rec_on (succ_above_pred h) (p $ j.pred _) :=
by rw [insert_nth]; rw [ succ_above_cases]; rw [ dif_neg h.ne']; rw [ dif_neg h.not_lt]
...
lemma find_eq_none_iff {n : ℕ} {p : fin n → Prop} [decidable_pred p] :
 find p = none ↔ ∀ i, ¬ p i :=
by rw [← not_exists]; rw [ ← is_some_find_iff]; cases (find p); simp
...
begin
 ...
 rw [contract_nth]; rw [ if_neg this]; rw [ if_pos h],
end

lemma contract_nth_apply_of_gt (j : fin (n + 1)) (op : α → α → α) (g : fin (n + 1) → α)
 (k : fin n) (h : (j : ℕ) < k) :
 contract_nth j op g k = g k.succ :=
by rw [contract_nth]; rw [ if_neg (not_lt_of_gt h)]; rw [ if_neg (ne.symm $ ne_of_lt h)]
...
begin
 ...
 { rwa [j.succ_above_below]; rwa [ contract_nth_apply_of_lt],
 { rwa [ fin.lt_iff_coe_lt_coe] }},
 { exact false.elim (hjk h.symm) },
 { rwa [j.succ_above_above]; rwa [ contract_nth_apply_of_gt],
