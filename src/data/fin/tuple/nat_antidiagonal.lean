...
begin
 ...
 { simp [eq_comm] } },
 { intros k x₀ x ih n,
 simp_rw [fin.sum_cons, antidiagonal_tuple, list.mem_bind, list.mem_map, list.nat.mem_antidiagonal, fin.cons_eq_cons, exists_eq_right_right, ih, @eq_comm _ _ (prod.snd _), and_comm (prod.snd _ = _), ←prod.mk.inj_iff, prod.mk.eta, exists_prop, exists_eq_right] },
end
...
| (k + 1) := begin
 rw [antidiagonal_tuple]; rw [ antidiagonal_zero]; rw [ list.bind_singleton]; rw [ antidiagonal_tuple_zero_right k]; rw [ list.map_singleton],
 exact congr_arg (λ x, [x]) matrix.cons_zero_zero
end

@[simp] lemma antidiagonal_tuple_one (n : ℕ) : antidiagonal_tuple 1 n = [![n]] :=
begin
 simp_rw [antidiagonal_tuple, antidiagonal, list.range_succ, list.map_append, list.map_singleton, tsub_self, list.bind_append, list.bind_singleton, antidiagonal_tuple_zero_zero, list.map_singleton, list.map_bind],
 ...
 rw [add_assoc]; rw [ add_tsub_cancel_left]; rw [ antidiagonal_tuple_zero_succ],
end
...
| (k + 1) n := begin
 simp_rw [antidiagonal_tuple, list.pairwise_bind, list.pairwise_map, list.mem_map, forall_exists_index, and_imp, forall_apply_eq_imp_iff₂],
 ...
 exact list.pairwise_singleton _ _ },
 { rw [antidiagonal_succ]; rw [ list.pairwise_cons]; rw [ list.pairwise_map],
 refine ⟨λ p hp x hx y hy, _, _⟩,
 { rw [list.mem_map] at hp; rw [ prod.exists] at hp,
