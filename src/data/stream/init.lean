...
λ x y s t h, ⟨by rw [←nth_zero_cons x s]; rw [ h]; rw [ nth_zero_cons],
 stream.ext (λ n, by rw [←nth_succ_cons n _ x]; rw [ h]; rw [ nth_succ_cons])⟩
...
assume ⟨n, h⟩,
exists.intro (succ n) (by rw [nth_succ]; rw [ tail_cons]; rw [ h])
...
begin
 cases n with n',
 { left, exact h },
 { right, rw [nth_succ] at h; rw [ tail_cons] at h, exact ⟨n', h⟩ }
end
...
by rw [← stream.eta (map f s)]; rw [ tail_map]; rw [ head_map]

theorem map_cons (a : α) (s : stream α) : map f (a :: s) = f a :: map f s :=
begin rw [← stream.eta (map f (a :: s))]; rw [ map_eq], refl end
...
assume ⟨n, h⟩,
exists.intro n (by rw [nth_map]; rw [ h])
...
by rw [nth_succ]; rw [ tail_iterate]
...
assume hh ht₁ ht₂, eq_of_bisim
 ...
 (λ s₁ s₂ ⟨h₁, h₂, h₃⟩,
 begin
 constructor, exact h₁, rw [← h₂]; rw [ ← h₃], repeat { constructor }; assumption
 end)
...
coinduction
 rfl
 (λ β fr ch, begin rw [tail_iterate]; rw [ tail_const], exact ch end)
...
begin rw [corec_def]; rw [ map_eq]; rw [ head_iterate]; rw [ tail_iterate], refl end

theorem corec_id_id_eq_const (a : α) : corec id id a = const a :=
by rw [corec_def]; rw [ map_id]; rw [ iterate_id]
...
begin
 ...
 { intro s, refl },
 { intro s, rw [nth_succ]; rw [ nth_succ]; rw [ unfolds_eq]; rw [ tail_cons]; rw [ ih] }
end
...
| (succ n) s₁ s₂ :=
 ...
 rw [nth_succ]; rw [ nth_succ]; rw [ interleave_eq]; rw [ tail_cons]; rw [ tail_cons]; rw [ nth_interleave_left],
 refl
 end
...
| (succ n) s₁ s₂ :=
 ...
 rw [nth_succ]; rw [ nth_succ]; rw [ interleave_eq]; rw [ tail_cons]; rw [ tail_cons]; rw [ nth_interleave_right],
 refl
 end

theorem mem_interleave_left {a : α} {s₁ : stream α} (s₂ : stream α) : a ∈ s₁ → a ∈ s₁ ⋈ s₂ :=
assume ⟨n, h⟩,
exists.intro (2*n) (by rw [h]; rw [ nth_interleave_left])

theorem mem_interleave_right {a : α} {s₁ : stream α} (s₂ : stream α) : a ∈ s₂ → a ∈ s₁ ⋈ s₂ :=
assume ⟨n, h⟩,
exists.intro (2*n+1) (by rw [h]; rw [ nth_interleave_right])
...
eq_of_bisim
 ...
 constructor,
 {refl},
 {exact ⟨tail s₂, by rw [interleave_eq]; rw [ even_cons_cons]; rw [ tail_cons]⟩}
 end)
 (exists.intro s₂ rfl)
...
| (succ n) s :=
 ...
 rw [nth_succ]; rw [ nth_succ]; rw [ tail_even]; rw [ nth_even], refl
 end

theorem nth_odd : ∀ (n : nat) (s : stream α), nth (odd s) n = nth s (2 * n + 1) :=
λ n s, begin rw [odd_eq]; rw [ nth_even], refl end

theorem mem_of_mem_even (a : α) (s : stream α) : a ∈ even s → a ∈ s :=
assume ⟨n, h⟩,
exists.intro (2*n) (by rw [h]; rw [ nth_even])

theorem mem_of_mem_odd (a : α) (s : stream α) : a ∈ odd s → a ∈ s :=
assume ⟨n, h⟩,
exists.intro (2*n+1) (by rw [h]; rw [ nth_odd])
...
| [] l₂ s := rfl
| (list.cons a l₁) l₂ s := by rw [list.cons_append]; rw [ cons_append_stream]; rw [ cons_append_stream]; rw [ append_append_stream]

theorem map_append_stream (f : α → β) :
 ∀ (l : list α) (s : stream α), map f (l ++ₛ s) = list.map f l ++ₛ map f s
| [] s := rfl
| (list.cons a l) s := by rw [cons_append_stream]; rw [ list.map_cons]; rw [ map_cons]; rw [ cons_append_stream]; rw [ map_append_stream]

theorem drop_append_stream : ∀ (l : list α) (s : stream α), drop l.length (l ++ₛ s) = s
| [] s := by refl
| (list.cons a l) s := by rw [list.length_cons]; rw [ add_one]; rw [ drop_succ]; rw [ cons_append_stream]; rw [ tail_cons]; rw [ drop_append_stream]

theorem append_stream_head_tail (s : stream α) : [head s] ++ₛ tail s = s :=
by rw [cons_append_stream]; rw [ nil_append_stream]; rw [ stream.eta]
...
| 0 s := rfl
| (n+1) s := begin rw [take_succ]; rw [ add_one]; rw [ list.nth]; rw [ nth_take_succ], refl end
...
begin
 ...
 induction n with n' ih,
 { intro s, refl },
 { intro s, rw [take_succ]; rw [ drop_succ]; rw [ cons_append_stream]; rw [ ih (tail s)]; rw [ stream.eta] }
end
...
begin
 ...
 { rw [← nth_take_succ]; rw [ ← nth_take_succ]; rw [ h (succ (succ n))] },
 injection h₁ }
end
...
| (list.cons a l) h :=
 ...
 {intros, rw [corec_eq]; rw [ stream.cycle_g_cons]; rw [ ih a₁], refl}
 end,
 gen l a
...
coinduction
 rfl
 (λ β fr ch, by rwa [cycle_eq]; rwa [ const_eq])
...
begin
 intro n, induction n with n' ih,
 { intros, refl },
 { intro s, rw [nth_succ]; rw [ drop_succ]; rw [ tails_eq]; rw [ tail_cons]; rw [ ih] }
end
...
begin
 ...
 induction n with n' ih,
 { intros, refl },
 { intros l s, rw [nth_succ]; rw [ inits_core_eq]; rw [ tail_cons]; rw [ ih]; rw [ inits_core_eq (a::l) s], refl }
end
...
begin
 intro n, induction n with n' ih,
 { intros, refl },
 { intros, rw [nth_succ]; rw [ take_succ]; rw [ ← ih]; rw [ tail_inits]; rw [ inits_tail]; rw [ cons_nth_inits_core] }
end
...
begin
 ...
 cases n,
 { refl },
 { rw [nth_inits]; rw [ nth_succ]; rw [ tail_cons]; rw [ nth_map]; rw [ nth_inits], refl }
end
