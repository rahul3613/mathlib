...
by simp only [singleton]; erw [mem_cons_eq]; erw [ mem_nil_iff]; erw [ or_false]
...
by rw [list.to_finmap]; rw [ lookup_to_finmap]; rw [ lookup_to_alist]
...
by rw [singleton]; rw [ lookup_to_finmap]; rw [ alist.singleton]; rw [ alist.lookup]; rw [ lookup_cons_eq]
...
by rw [mem_erase]; rw [ not_and_distrib]; rw [ not_not]; left; refl
...
begin
 ...
 { rw [lookup_union_right h']; rw [ lookup_eq_none.mpr h]; rw [ lookup_eq_none.mpr h'] }
end
...
(λ x, by { by_cases h' : x = a,
 { subst a, rw [lookup_union_right not_mem_erase_self]; rw [ lookup_singleton_eq]; rw [ h], },
...
by rw [disjoint.symm_iff]; rw [ disjoint_union_left]; rw [ disjoint.symm_iff _ x]; rw [ disjoint.symm_iff _ x]
...
⟨λ h'', begin
 ...
 by_cases hs₁ : x ∈ s₁,
 { rwa [lookup_union_left hs₁] at this; rwa [ lookup_union_left_of_not_in (h _ hs₁)] at this, },
 { by_cases hs₂ : x ∈ s₂,
 { rwa [lookup_union_left_of_not_in (h' _ hs₂)] at this; rwa [ lookup_union_left hs₂] at this, },
 { rw [lookup_eq_none.mpr hs₁]; rw [ lookup_eq_none.mpr hs₂] } }
 end,
 λ h, h ▸ rfl⟩
