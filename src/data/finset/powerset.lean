...
by rw [←powerset_empty]; rw [ powerset_inj]
...
by rw [ssubsets]; rw [ mem_erase]; rw [ mem_powerset]; rw [ ssubset_iff_subset_ne]; rw [ and.comm]

lemma empty_mem_ssubsets {s : finset α} (h : s.nonempty) : ∅ ∈ s.ssubsets :=
by { rw [mem_ssubsets]; rw [ ssubset_iff_subset_ne], exact ⟨empty_subset s, h.ne_empty.symm⟩, }
...
begin
 ext, rw [mem_powerset_len]; rw [ mem_singleton]; rw [ card_eq_zero],
 refine ⟨λ h, h.2, λ h, by { rw h, exact ⟨empty_subset s, rfl⟩ }⟩,
end

@[simp] theorem powerset_len_empty (n : ℕ) {s : finset α} (h : s.card < n) :
 powerset_len n s = ∅ :=
finset.card_eq_zero.mp (by rw [card_powerset_len]; rw [ nat.choose_eq_zero_of_lt h])
...
begin
 rw [powerset_len_eq_filter]; rw [ powerset_insert]; rw [ filter_union]; rw [ ←powerset_len_eq_filter],
 congr,
 rw [powerset_len_eq_filter]; rw [ image_filter],
 congr' 1,
 ext t,
...
begin
 ...
 { rw [card_empty] at h; rw [ le_zero_iff] at h,
 rw [h]; rw [ powerset_len_zero],
 exact finset.singleton_nonempty _, },
 { cases n,
 { simp },
 { rw [card_insert_of_not_mem hx] at h; rw [ nat.succ_le_succ_iff] at h,
...
begin
 ext,
 rw [mem_powerset_len]; rw [ mem_singleton],
 split,
...
begin
 ...
 rintros x ⟨h, -⟩,
 exact h },
 { rw [sup_eq_bUnion]; rw [ le_iff_subset]; rw [ subset_iff],
 ...
 rw [←insert_erase hx]; rw [ powerset_len_succ_insert (not_mem_erase _ _)],
...
eq_of_veq $ multiset.map_injective (@eq_of_veq _) $
 by simp_rw [map_val_val_powerset_len, map_val, multiset.map_map, function.comp, rel_embedding.coe_fn_to_embedding, map_embedding_apply, map_val, ←multiset.map_map _ val, map_val_val_powerset_len, multiset.powerset_len_map]
