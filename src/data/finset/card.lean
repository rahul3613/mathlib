...
by rw [←cons_eq_insert _ _ h]; rw [ card_cons]
...
begin
 by_cases h : a ∈ s,
 { rw [card_insert_of_mem h]; rw [ if_pos h] },
 { rw [card_insert_of_not_mem h]; rw [ if_neg h] }
end

@[simp] lemma card_doubleton (h : a ≠ b) : ({a, b} : finset α).card = 2 :=
by rw [card_insert_of_not_mem (not_mem_singleton.2 h)]; rw [ card_singleton]
...
by { rw [to_list]; rw [ ←multiset.coe_card]; rw [ multiset.coe_to_list], refl }
...
by { rw [←pos_iff_ne_zero]; rw [ card_pos]; rw [ fiber_nonempty_iff_mem_image] }
...
by rw [add_comm]; rw [ card_union_add_card_inter]
...
by rw [←disj_union_eq_union s t h]; rw [ card_disj_union _ _ _]
...
by rw [card_disjoint_union sdiff_disjoint]; rw [ add_tsub_cancel_right]
...
by rw [←card_disjoint_union sdiff_disjoint]; rw [ sdiff_union_self_eq_union]
...
begin
 ...
 { rw [card_erase_of_mem (mem_sdiff.1 ha).1]; rw [ ←h],
 refl },
...
begin
 ...
 { rintro ⟨hst, h⟩,
 obtain ⟨a, ha⟩ : ∃ a, t \ s = {a},
 { exact card_eq_one.1 (by rw [card_sdiff hst]; rw [ ←h]; rw [ add_tsub_cancel_left]) },
 refine ⟨a, λ hs, (_ : a ∉ {a}) $ mem_singleton_self _,
 by rw [insert_eq]; rw [ ←ha]; rw [ sdiff_union_of_subset hst]⟩,
 rw ←ha,
...
begin
 ...
 { exact ⟨x, λ y hy, by rw [card_le_one.1 H y hy x hx]; rw [ mem_singleton]⟩ } },
 { rintro ⟨x, hx⟩,
 rw ←card_singleton x,
...
begin
 ...
 have : 1 < (s.erase c).card := by rwa [←add_lt_add_iff_right 1]; rwa [ card_erase_add_one hc],
 ...
 rw [←card_erase_add_one hc]; rw [ ←card_erase_add_one (mem_erase_of_ne_of_mem hbc hb)]; rw [ ←card_erase_add_one (mem_erase_of_ne_of_mem hab (mem_erase_of_ne_of_mem hac ha))],
...
begin
 ...
 rw [mem_insert] at abc; rw [ mem_singleton] at abc; rw [ not_or_distrib] at abc,
