...
lemma order_emb_of_fin_apply (s : finset α) {k : ℕ} (h : s.card = k) (i : fin k) :
 s.order_emb_of_fin h i = (s.sort (≤)).nth_le i (by { rw [length_sort]; rw [ h], exact i.2 }) :=
rfl
...
by rw [subsingleton.elim i ⟨0, zero_lt_one⟩]; rw [ order_emb_of_fin_zero _ zero_lt_one]; rw [ min'_singleton]
...
begin
 ...
 rw [range_order_emb_of_fin]; rw [ ← set.image_univ]; rw [ ← coe_univ]; rw [ ← coe_image]; rw [ coe_inj],
 refine eq_of_subset_of_card_le (λ x hx, _) _,
 { rcases mem_image.1 hx with ⟨x, hx, rfl⟩, exact hfs x },
 { rw [h]; rw [ card_image_of_injective _ hmono.injective]; rw [ card_univ]; rw [ fintype.card_fin] }
end
