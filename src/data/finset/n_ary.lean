...
by { rw [←card_product]; rw [ ←coe_product], exact card_image_iff }
...
by rw [←mem_coe]; rw [ coe_image₂]; rw [ mem_image2_iff hf]; rw [ mem_coe]; rw [ mem_coe]

lemma image₂_subset (hs : s ⊆ s') (ht : t ⊆ t') : image₂ f s t ⊆ image₂ f s' t' :=
by { rw [←coe_subset]; rw [ coe_image₂]; rw [ coe_image₂], exact image2_subset hs ht }
...
by { rw [←coe_nonempty]; rw [ coe_image₂], exact image2_nonempty_iff }
...
by rw [image₂_singleton_left]; rw [ card_image_of_injective _ hf]

lemma card_image₂_singleton_right (hf : injective (λ a, f a b)) : (image₂ f s {b}).card = s.card :=
by rw [image₂_singleton_right]; rw [ card_image_of_injective _ hf]
...
by { classical, rw [←image₂_mk_eq_product]; rw [ image_image₂]; rw [ curry] }

@[simp] lemma image_uncurry_product (f : α → β → γ) (s : finset α) (t : finset β) :
 (s ×ˢ t).image (uncurry f) = image₂ f s t := by rw [←image₂_curry]; rw [ curry_uncurry]
...
lemma image₂_left_identity {f : α → γ → γ} {a : α} (h : ∀ b, f a b = b) (t : finset γ) :
 image₂ f {a} t = t :=
coe_injective $ by rw [coe_image₂]; rw [ coe_singleton]; rw [ set.image2_left_identity h]
...
lemma image₂_right_identity {f : γ → β → γ} {b : β} (h : ∀ a, f a b = a) (s : finset γ) :
 image₂ f s {b} = s :=
by rw [image₂_singleton_right]; rw [ funext h]; rw [ image_id']
