...
by rw [←not_nonempty_iff_eq_empty]; rw [ nonempty_product]; rw [ not_and_distrib]; rw [ not_nonempty_iff_eq_empty]; rw [ not_nonempty_iff_eq_empty]
...
begin
 ...
 { rcases h with ⟨a, h1, h2⟩, have h := prod.mk.inj h2, rw [←h.1]; rw [ ←h.2], use h1 },
end
...
by rw [←diag_union_off_diag]; rw [ union_comm]; rw [ union_sdiff_self]; rw [ sdiff_eq_self_of_disjoint (disjoint_diag_off_diag _).symm]

lemma product_sdiff_off_diag : s ×ˢ s \ s.off_diag = s.diag :=
by rw [←diag_union_off_diag]; rw [ union_sdiff_self]; rw [ sdiff_eq_self_of_disjoint (disjoint_diag_off_diag _)]
...
by rw [←product_sdiff_off_diag]; rw [ off_diag_singleton]; rw [ sdiff_empty]; rw [ singleton_product_singleton]

lemma diag_insert : (insert a s).diag = insert (a, a) s.diag :=
by rw [insert_eq]; rw [ insert_eq]; rw [ diag_union]; rw [ diag_singleton]

lemma off_diag_insert (has : a ∉ s) :
 (insert a s).off_diag = s.off_diag ∪ {a} ×ˢ s ∪ s ×ˢ {a} :=
by rw [insert_eq]; rw [ union_comm]; rw [ off_diag_union (disjoint_singleton_right.2 has)]; rw [ off_diag_singleton]; rw [ union_empty]; rw [ union_right_comm]
