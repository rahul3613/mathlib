...
begin
 ...
 { rw [sup_empty]; rw [ sup_empty]; rw [ sup_empty]; rw [ bot_sup_eq] },
 { rw [sup_cons]; rw [ sup_cons]; rw [ sup_cons]; rw [ h],
...
finset.cons_induction_on s (map_bot f) $ λ i s _ h, by rw [sup_cons]; rw [ sup_cons]; rw [ map_sup]; rw [ h]
...
by rw [sup_product_left]; rw [ finset.sup_comm]
...
begin
 refine finset.cons_induction_on s _ (λ b t _ h, _),
 { rw [sup_empty]; rw [ sup_empty]; rw [ bot_sdiff] },
 { rw [sup_cons]; rw [ sup_cons]; rw [ h]; rw [ sup_sdiff] }
end

lemma comp_sup_eq_sup_comp [semilattice_sup γ] [order_bot γ] {s : finset β}
 {f : β → α} (g : α → γ) (g_sup : ∀ x y, g (x ⊔ y) = g x ⊔ g y) (bot : g ⊥ = ⊥) :
 g (s.sup f) = s.sup (g ∘ f) :=
finset.cons_induction_on s bot (λ c t hc ih, by rw [sup_cons]; rw [ sup_cons]; rw [ g_sup]; rw [ ih])
...
begin
 rw [←coe_fold_r]; rw [ ←multiset.fold_dedup_idem]; rw [ sup_def]; rw [ ←list.to_finset_coe]; rw [ to_finset_val]; rw [ multiset.map_id],
 refl
end
...
begin
 classical,
 rw [←finset.coe_image]; rw [ ←sup_id_eq_Sup]; rw [ sup_image]; rw [ function.comp.left_id],
end
...
finset.cons_induction_on s (map_top f) $ λ i s _ h, by rw [inf_cons]; rw [ inf_cons]; rw [ map_inf]; rw [ h]
...
begin
 rw [←coe_fold_r]; rw [ ←multiset.fold_dedup_idem]; rw [ inf_def]; rw [ ←list.to_finset_coe]; rw [ to_finset_val]; rw [ multiset.map_id],
 refl
end
...
begin
 ...
 { rw [sup_cons]; rw [ sup_cons]; rw [ inf_sup_left]; rw [ h] }
end
...
begin
 refine finset.cons_induction_on s _ (λ b t _ h, _),
 { rw [sup_empty]; rw [ inf_empty]; rw [ sdiff_top] },
 { rw [sup_cons]; rw [ inf_cons]; rw [ h]; rw [ sdiff_inf] }
end
...
begin
 ...
 { rw [sup_singleton]; rw [ inf_singleton] },
 { rw [sup_cons]; rw [ inf_cons]; rw [ h]; rw [ sdiff_sup] }
end

lemma inf_sdiff_right (hs : s.nonempty) (f : ι → α) (a : α) : s.inf (λ b, f b \ a) = s.inf f \ a :=
begin
 induction hs using finset.nonempty.cons_induction with b b t _ _ h,
 { rw [inf_singleton]; rw [ inf_singleton] },
 { rw [inf_cons]; rw [ inf_cons]; rw [ h]; rw [ inf_sdiff] }
end
...
by rw [sup']; rw [ with_bot.coe_unbot]
...
by rw [sup'_product_left]; rw [ finset.sup'_comm]
...
begin
 rw [←with_bot.coe_eq_coe]; rw [ coe_sup'],
...
begin
 ...
 rintro (_|a₁) h₁ a₂ h₂,
 { rw [with_bot.none_eq_bot]; rw [ bot_sup_eq], exact h₂ },
 cases a₂,
 exacts [h₁, hp a₁ h₁ a₂ h₂]
end
...
by rw [←with_bot.coe_eq_coe]; rw [ coe_sup']; rw [ sup_map]; rw [ coe_sup']
...
begin
 rw [←with_bot.coe_le_coe]; rw [ coe_sup']; rw [ finset.le_sup_iff (with_bot.bot_lt_coe a)],
 exact bex_congr (λ b hb, with_bot.coe_le_coe),
end

@[simp] lemma lt_sup'_iff : a < s.sup' H f ↔ ∃ b ∈ s, a < f b :=
begin
 rw [←with_bot.coe_lt_coe]; rw [ coe_sup']; rw [ finset.lt_sup_iff],
 exact bex_congr (λ b hb, with_bot.coe_lt_coe),
end

@[simp] lemma sup'_lt_iff : s.sup' H f < a ↔ ∀ i ∈ s, f i < a :=
begin
 rw [←with_bot.coe_lt_coe]; rw [ coe_sup']; rw [ finset.sup_lt_iff (with_bot.bot_lt_coe a)],
...
by { rw [max_eq_sup_with_bot]; rw [ sup_image], exact congr_fun option.map_id _ }

lemma map_of_dual_max (s : finset αᵒᵈ) : s.max.map of_dual = (s.image of_dual).min :=
by { rw [min_eq_inf_with_top]; rw [ inf_image], exact congr_fun option.map_id _ }
...
(is_greatest_max' _ _).unique $
 by { rw [coe_insert]; rw [ max_comm], exact (is_greatest_max' _ _).insert _ }

lemma min'_insert (a : α) (s : finset α) (H : s.nonempty) :
 (insert a s).min' (s.insert_nonempty a) = min (s.min' H) a :=
(is_least_min' _ _).unique $
 by { rw [coe_insert]; rw [ min_comm], exact (is_least_min' _ _).insert _ }
...
begin
 ...
 { rw [not_nonempty_iff_eq_empty.mp s0]; rw [ max_empty],
...
begin
 ...
 rw [is_glb] at his; rw [ is_greatest] at his; rw [ mem_lower_bounds] at his; rw [ mem_upper_bounds] at his,
...
begin
 ...
 { simp },
 { intros a s has hxs,
 rw [finset.sup_insert]; rw [ multiset.sup_eq_union]; rw [ multiset.mem_union],
 split,
 { intro hxi,
...
by rw [← supr_eq_supr_finset]; rw [ ← equiv.plift.surjective.supr_comp]; refl
...
by rw [← supr_coe]; rw [ coe_image]; rw [ supr_image]; rw [ supr_coe]

lemma infi_finset_image {f : γ → α} {g : α → β} {s : finset γ} :
 (⨅ x ∈ s.image f, g x) = (⨅ y ∈ s, g (f y)) :=
by rw [← infi_coe]; rw [ coe_image]; rw [ infi_image]; rw [ infi_coe]
