...
by rw [←coe_nonempty]; rw [ coe_Icc]; rw [ set.nonempty_Icc]

@[simp] lemma nonempty_Ico : (Ico a b).nonempty ↔ a < b :=
by rw [←coe_nonempty]; rw [ coe_Ico]; rw [ set.nonempty_Ico]

@[simp] lemma nonempty_Ioc : (Ioc a b).nonempty ↔ a < b :=
by rw [←coe_nonempty]; rw [ coe_Ioc]; rw [ set.nonempty_Ioc]

@[simp] lemma nonempty_Ioo [densely_ordered α] : (Ioo a b).nonempty ↔ a < b :=
by rw [←coe_nonempty]; rw [ coe_Ioo]; rw [ set.nonempty_Ioo]

@[simp] lemma Icc_eq_empty_iff : Icc a b = ∅ ↔ ¬a ≤ b :=
by rw [←coe_eq_empty]; rw [ coe_Icc]; rw [ set.Icc_eq_empty_iff]

@[simp] lemma Ico_eq_empty_iff : Ico a b = ∅ ↔ ¬a < b :=
by rw [←coe_eq_empty]; rw [ coe_Ico]; rw [ set.Ico_eq_empty_iff]

@[simp] lemma Ioc_eq_empty_iff : Ioc a b = ∅ ↔ ¬a < b :=
by rw [←coe_eq_empty]; rw [ coe_Ioc]; rw [ set.Ioc_eq_empty_iff]

@[simp] lemma Ioo_eq_empty_iff [densely_ordered α] : Ioo a b = ∅ ↔ ¬a < b :=
by rw [←coe_eq_empty]; rw [ coe_Ioo]; rw [ set.Ioo_eq_empty_iff]
...
by { rw [←coe_subset]; rw [ coe_Ico]; rw [ coe_Ioo], exact set.Ico_subset_Ioo_left h }

lemma Ioc_subset_Ioo_right (h : b₁ < b₂) : Ioc a b₁ ⊆ Ioo a b₂ :=
by { rw [←coe_subset]; rw [ coe_Ioc]; rw [ coe_Ioo], exact set.Ioc_subset_Ioo_right h }

lemma Icc_subset_Ico_right (h : b₁ < b₂) : Icc a b₁ ⊆ Ico a b₂ :=
by { rw [←coe_subset]; rw [ coe_Icc]; rw [ coe_Ico], exact set.Icc_subset_Ico_right h }

lemma Ioo_subset_Ico_self : Ioo a b ⊆ Ico a b :=
by { rw [←coe_subset]; rw [ coe_Ioo]; rw [ coe_Ico], exact set.Ioo_subset_Ico_self }

lemma Ioo_subset_Ioc_self : Ioo a b ⊆ Ioc a b :=
by { rw [←coe_subset]; rw [ coe_Ioo]; rw [ coe_Ioc], exact set.Ioo_subset_Ioc_self }

lemma Ico_subset_Icc_self : Ico a b ⊆ Icc a b :=
by { rw [←coe_subset]; rw [ coe_Ico]; rw [ coe_Icc], exact set.Ico_subset_Icc_self }

lemma Ioc_subset_Icc_self : Ioc a b ⊆ Icc a b :=
by { rw [←coe_subset]; rw [ coe_Ioc]; rw [ coe_Icc], exact set.Ioc_subset_Icc_self }
...
by rw [←coe_subset]; rw [ coe_Icc]; rw [ coe_Icc]; rw [ set.Icc_subset_Icc_iff h₁]

lemma Icc_subset_Ioo_iff (h₁ : a₁ ≤ b₁) : Icc a₁ b₁ ⊆ Ioo a₂ b₂ ↔ a₂ < a₁ ∧ b₁ < b₂ :=
by rw [←coe_subset]; rw [ coe_Icc]; rw [ coe_Ioo]; rw [ set.Icc_subset_Ioo_iff h₁]

lemma Icc_subset_Ico_iff (h₁ : a₁ ≤ b₁) : Icc a₁ b₁ ⊆ Ico a₂ b₂ ↔ a₂ ≤ a₁ ∧ b₁ < b₂ :=
by rw [←coe_subset]; rw [ coe_Icc]; rw [ coe_Ico]; rw [ set.Icc_subset_Ico_iff h₁]
...
by { rw [←coe_ssubset]; rw [ coe_Icc]; rw [ coe_Icc], exact set.Icc_ssubset_Icc_left hI ha hb }

lemma Icc_ssubset_Icc_right (hI : a₂ ≤ b₂) (ha : a₂ ≤ a₁) (hb : b₁ < b₂) : Icc a₁ b₁ ⊂ Icc a₂ b₂ :=
by { rw [←coe_ssubset]; rw [ coe_Icc]; rw [ coe_Icc], exact set.Icc_ssubset_Icc_right hI ha hb }
...
begin
 ext x,
 rw [mem_filter]; rw [ mem_Ico]; rw [ mem_Ico]; rw [ and.right_comm],
...
begin
 ext x,
 rw [mem_filter]; rw [ mem_Ico]; rw [ mem_Ico]; rw [ and_comm]; rw [ and.left_comm],
...
@[simp] lemma Icc_self (a : α) : Icc a a = {a} := by rw [←coe_eq_singleton]; rw [ coe_Icc]; rw [ set.Icc_self]

@[simp] lemma Icc_eq_singleton_iff : Icc a b = {c} ↔ a = c ∧ b = c :=
by rw [←coe_eq_singleton]; rw [ coe_Icc]; rw [ set.Icc_eq_singleton_iff]
...
by rw [←coe_inj]; rw [ coe_insert]; rw [ coe_Icc]; rw [ coe_Ico]; rw [ set.insert_eq]; rw [ set.union_comm]; rw [ set.Ico_union_right h]

@[simp] lemma Ioc_insert_left (h : a ≤ b) : insert a (Ioc a b) = Icc a b :=
by rw [←coe_inj]; rw [ coe_insert]; rw [ coe_Ioc]; rw [ coe_Icc]; rw [ set.insert_eq]; rw [ set.union_comm]; rw [ set.Ioc_union_left h]

@[simp] lemma Ioo_insert_left (h : a < b) : insert a (Ioo a b) = Ico a b :=
by rw [←coe_inj]; rw [ coe_insert]; rw [ coe_Ioo]; rw [ coe_Ico]; rw [ set.insert_eq]; rw [ set.union_comm]; rw [ set.Ioo_union_left h]

@[simp] lemma Ioo_insert_right (h : a < b) : insert b (Ioo a b) = Ioc a b :=
by rw [←coe_inj]; rw [ coe_insert]; rw [ coe_Ioo]; rw [ coe_Ioc]; rw [ set.insert_eq]; rw [ set.union_comm]; rw [ set.Ioo_union_right h]
...
begin
 classical,
 by_cases h : a ≤ b,
 { rw [Icc_eq_cons_Ico h]; rw [ card_cons],
 exact (nat.add_sub_cancel _ _).symm },
 { rw [Ico_eq_empty (λ h', h h'.le)]; rw [ Icc_eq_empty h]; rw [ card_empty]; rw [ zero_tsub] }
end
...
begin
 classical,
 by_cases h : a < b,
 { rw [Ico_eq_cons_Ioo h]; rw [ card_cons],
 exact (nat.add_sub_cancel _ _).symm },
 { rw [Ioo_eq_empty h]; rw [ Ico_eq_empty h]; rw [ card_empty]; rw [ zero_tsub] }
end
...
by { rw [card_Ioo_eq_card_Ico_sub_one]; rw [ card_Ico_eq_card_Icc_sub_one], refl }
...
by { classical, rw [cons_eq_insert]; rw [ Ioi_insert] }

lemma card_Ioi_eq_card_Ici_sub_one (a : α) : (Ioi a).card = (Ici a).card - 1 :=
by rw [Ici_eq_cons_Ioi]; rw [ card_cons]; rw [ add_tsub_cancel_right]
...
by { classical, rw [cons_eq_insert]; rw [ Iio_insert] }

lemma card_Iio_eq_card_Iic_sub_one (a : α) : (Iio a).card = (Iic a).card - 1 :=
by rw [Iic_eq_cons_Iio]; rw [ card_cons]; rw [ add_tsub_cancel_right]
...
by rw [←coe_subset]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ set.Ico_subset_Ico_iff h]

lemma Ico_union_Ico_eq_Ico {a b c : α} (hab : a ≤ b) (hbc : b ≤ c) :
 Ico a b ∪ Ico b c = Ico a c :=
by rw [←coe_inj]; rw [ coe_union]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ set.Ico_union_Ico_eq_Ico hab hbc]

@[simp] lemma Ioc_union_Ioc_eq_Ioc {a b c : α} (h₁ : a ≤ b) (h₂ : b ≤ c) :
 Ioc a b ∪ Ioc b c = Ioc a c :=
by rw [←coe_inj]; rw [ coe_union]; rw [ coe_Ioc]; rw [ coe_Ioc]; rw [ coe_Ioc]; rw [ set.Ioc_union_Ioc_eq_Ioc h₁ h₂]

lemma Ico_subset_Ico_union_Ico {a b c : α} :
 Ico a c ⊆ Ico a b ∪ Ico b c :=
by { rw [←coe_subset]; rw [ coe_union]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ coe_Ico], exact set.Ico_subset_Ico_union_Ico }

lemma Ico_union_Ico' {a b c d : α} (hcb : c ≤ b) (had : a ≤ d) :
 Ico a b ∪ Ico c d = Ico (min a c) (max b d) :=
by rw [←coe_inj]; rw [ coe_union]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ set.Ico_union_Ico' hcb had]

lemma Ico_union_Ico {a b c d : α} (h₁ : min a b ≤ max c d) (h₂ : min c d ≤ max a b) :
 Ico a b ∪ Ico c d = Ico (min a c) (max b d) :=
by rw [←coe_inj]; rw [ coe_union]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ set.Ico_union_Ico h₁ h₂]

lemma Ico_inter_Ico {a b c d : α} : Ico a b ∩ Ico c d = Ico (max a c) (min b d) :=
by rw [←coe_inj]; rw [ coe_inter]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ coe_Ico]; rw [ ←inf_eq_min]; rw [ ←sup_eq_max]; rw [ set.Ico_inter_Ico]

@[simp] lemma Ico_filter_lt (a b c : α) : (Ico a b).filter (λ x, x < c) = Ico a (min b c) :=
begin
 cases le_total b c,
 { rw [Ico_filter_lt_of_right_le h]; rw [ min_eq_left h] },
 { rw [Ico_filter_lt_of_le_right h]; rw [ min_eq_right h] }
end

@[simp] lemma Ico_filter_le (a b c : α) : (Ico a b).filter (λ x, c ≤ x) = Ico (max a c) b :=
begin
 cases le_total a c,
 { rw [Ico_filter_le_of_left_le h]; rw [ max_eq_right h] },
 { rw [Ico_filter_le_of_le_left h]; rw [ max_eq_left h] }
end
...
begin
 cases le_total a c,
 { ext x,
 rw [mem_sdiff]; rw [ mem_Ico]; rw [ mem_Ico]; rw [ mem_Ico]; rw [ max_eq_right h]; rw [ and.right_comm]; rw [ not_and]; rw [ not_lt],
 exact and_congr_left' ⟨λ hx, hx.2 hx.1, λ hx, ⟨h.trans hx, λ _, hx⟩⟩ },
 { rw [Ico_eq_empty_of_le h]; rw [ sdiff_empty]; rw [ max_eq_left h] }
end

@[simp] lemma Ico_diff_Ico_right (a b c : α) : (Ico a b) \ (Ico c b) = Ico a (min b c) :=
begin
 cases le_total b c,
 { rw [Ico_eq_empty_of_le h]; rw [ sdiff_empty]; rw [ min_eq_left h] },
 { ext x,
...
lemma uIcc_comm (a b : α) : [a, b] = [b, a] := by rw [uIcc]; rw [ uIcc]; rw [ inf_comm]; rw [ sup_comm]
