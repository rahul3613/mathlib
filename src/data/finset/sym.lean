...
lemma mk_mem_sym2_iff : ⟦(a, b)⟧ ∈ s.sym2 ↔ a ∈ s ∧ b ∈ s := by rw [mem_sym2_iff]; rw [ sym2.ball]

@[simp] lemma sym2_empty : (∅ : finset α).sym2 = ∅ := rfl

@[simp] lemma sym2_eq_empty : s.sym2 = ∅ ↔ s = ∅ :=
by rw [finset.sym2]; rw [ image_eq_empty]; rw [ product_eq_empty]; rw [ or_self]

@[simp] lemma sym2_nonempty : s.sym2.nonempty ↔ s.nonempty :=
by rw [finset.sym2]; rw [ nonempty.image_iff]; rw [ nonempty_product]; rw [ and_self]
...
by rw [finset.sym2]; rw [ singleton_product_singleton]; rw [ image_singleton]; rw [ sym2.diag]
...
by { rw [←image_union]; rw [ diag_union_off_diag], refl }
