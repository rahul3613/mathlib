...
by classical; rw [map_eq_image]; rw [ image_subset_iff_subset_preimage]
...
begin
 ...
 { rw [map_eq_image]; rw [ image_preimage]; rw [ filter_true_of_mem (λ x hx, _)],
...
by rw [sigma_preimage_mk]; rw [ filter_true_of_mem $ image_subset_iff.1 ht]
...
by { classical, rw [prod_preimage']; rw [ prod_filter_of_ne], exact λ x hx, not.imp_symm (hg x hx) }
