...
by rw [nonempty_iff_ne_empty]; rw [ nonempty_iff_ne_empty]; rw [ ne.def]; rw [ map_eq_empty]

alias map_nonempty ↔ _ nonempty.map

lemma attach_map_val {s : finset α} : s.attach.map (embedding.subtype _) = s :=
eq_of_veq $ by rw [map_val]; rw [ attach_val]; exact attach_map_val _
...
by { ext, rw [mem_filter]; rw [ mem_image],
...
iff.intro
 ...
 ⟨int.to_nat (i % n),
 by rw [←int.coe_nat_lt]; rw [ int.to_nat_of_nonneg this]; exact ⟨int.mod_lt_of_pos i hn', hi⟩⟩)
 (assume ⟨i, hi, ha⟩,
 ⟨i, by rw [int.mod_eq_of_lt (int.coe_zero_le _) (int.coe_nat_lt_coe_nat_of_lt hi)]; rw [ ha]⟩)

lemma range_add (a b : ℕ) : range (a + b) = range a ∪ (range b).map (add_left_embedding a) :=
by { rw [←val_inj]; rw [ union_val], exact multiset.range_add_eq_union a b }

@[simp] lemma attach_image_val [decidable_eq α] {s : finset α} : s.attach.image subtype.val = s :=
eq_of_veq $ by rw [image_val]; rw [ attach_val]; rw [ multiset.attach_map_val]; rw [ dedup_eq_self]
...
by rw [subtype_map]; rw [ filter_true_of_mem h]
