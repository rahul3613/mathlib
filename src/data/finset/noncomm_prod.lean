...
noncomm_foldr op s (λ x hx y hy h b, by rw [←assoc.assoc]; rw [ comm hx hy h]; rw [ assoc.assoc])
...
lemma noncomm_prod_congr {s₁ s₂ : finset α} {f g : α → β} (h₁ : s₁ = s₂)
 ...
 (λ x hx y hy h, by { rw [←h₂ _ hx]; rw [ ←h₂ _ hy], subst h₁, exact comm hx hy h }) :=
...
begin
 rw [noncomm_prod]; rw [ multiset.noncomm_prod_eq_pow_card _ _ m],
...
begin
 ...
 rw [ ← insert_erase (mem_univ i)]; rw [ noncomm_prod_insert_of_not_mem' _ _ _ _ (not_mem_erase _ _)]; rw [ noncomm_prod_eq_pow_card]; rw [ one_pow],
 { simp, },
...
begin
 ...
 ext x,
 rw [← noncomm_prod_mul_single x]; rw [ univ.noncomm_prod_map]; rw [ univ.noncomm_prod_map],
