...
by rw [← drop_fun_split_fun f g]; rw [ H]; rw [ ← last_fun_split_fun f g]; rw [ H]; simp
...
by induction i; [refl, erw [typevec.const]; erw [@i_ih (drop α) x]]
...
by induction i; [refl, erw [repeat_eq]; erw [@i_ih (drop α) x y]]
...
by rw [← comp_assoc]; rw [to_subtype_of_subtype]; simp
