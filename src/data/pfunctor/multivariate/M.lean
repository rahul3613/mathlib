...
| ._ b h ._ (M.path.root x a f h' i c) :=
 have a = g₀ b,
 by { rw [h] at h'; rw [ M.corec_shape] at h'; rw [ pfunctor.M.dest_corec] at h', cases h', refl },
...
begin
 ...
 conv { to_rhs, rw [←split_drop_fun_last_fun f]; rw [ append_fun_comp_split_fun] },
 refl
end
...
begin
 ...
 intro h, rw [map_eq] at h; rw [map_eq] at h, injection h with h₀ h₁, subst ay,
 simp at h₁, clear h,
 have Hdrop : drop_fun fx = drop_fun fy,
 { replace h₁ := congr_arg drop_fun h₁,
 simpa using h₁, },
 existsi [ax,drop_fun fx,last_fun fx,last_fun fy],
 rw [split_drop_fun_last_fun]; rw [Hdrop]; rw [split_drop_fun_last_fun],
 simp, intro i,
...
begin
 ...
 rwa [append_fun_comp_id]; rwa [← mvfunctor.map_map]; rwa [← mvfunctor.map_map]; rwa [h] },
 all_goals { cc } }
end
...
begin
 cases x with a f,
 rw map_eq,
 conv { to_rhs, rw [M.dest]; rw [ M.dest']; rw [ map_eq]; rw [ append_fun_comp_split_fun] },
 reflexivity
end
