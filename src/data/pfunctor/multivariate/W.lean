...
begin
 rw [W_mk]; rw [ W_rec], dsimp, rw [Wp_rec_eq],
...
begin
 ...
 { ext1, cases (f x), refl },
 rw this, dsimp,
 rw [W_mk_eq]; rw [ W_mk_eq],
 have h := mvpfunctor.map_eq P.Wp g,
 rw [h]; rw [ comp_W_path_cases_on]
end
...
by rw [obj_append1]; rw [ obj_append1]; rw [ map_eq]; rw [ append_fun]; rw [ ← split_fun_comp]; refl
...
by rw [W_dest']; rw [ W_rec_eq]

theorem W_dest'_W_mk' {α : typevec n} (x : P.obj (α.append1 (P.W α))) :
 P.W_dest' (P.W_mk' x) = x :=
by cases x with a f; rw [W_mk']; rw [ W_dest'_W_mk]; rw [ split_drop_fun_last_fun]
