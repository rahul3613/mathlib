...
begin
 ...
 { rw [← head_succ' n]; rw [h]; rw [head'], apply x.consistent },
...
lemma mk_inj {x y : F.obj $ M F}
 (h : M.mk x = M.mk y) : x = y :=
by rw [← dest_mk x]; rw [h]; rw [dest_mk]
...
begin
 ...
 { rw [← agree_iff_agree']; rw [i_ih], apply y.consistent },
 introv H',
...
begin
 ...
 obtain rfl : a₀ = a₁ := bisim.head h₁,
 apply (ps_ih _ _ _ h₁),
 rw [← h]; rw [← h'], apply or_of_or_of_imp_of_imp hh is_path_cons' is_path_cons'
end
...
begin
 ...
 { rw [hyp]; rw [ gxeq]; rw [ pfunctor.map_eq] },
 have h₁ : M.dest (M.corec g x) = ⟨a, M.corec g ∘ f'⟩,
 { rw [dest_corec]; rw [ gxeq]; rw [ pfunctor.map_eq], },
 refine ⟨_, _, _, h₀, h₁, _⟩,
 intro i,
