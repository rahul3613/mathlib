...
begin
 rw [Sup_eq_eqv_gen]; rw [ Sup_image],
 congr' with x y,
...
begin
 ext ⟨x⟩,
 erw [quotient.lift_mk f H]; erw [ Hg],
 refl
end
...
{ to_fun := λ s, map_of_surjective s.1 quotient.mk ((ker_mk_eq r).symm ▸ s.2) exists_rep,
 inv_fun := λ s, ⟨comap quotient.mk' s, λ x y h, by rw [comap_rel]; rw [ eq_rel.2 h]⟩,
 ...
 λ x y h, by rw [comap_rel]; rw [ (@eq_rel _ r x y).2 ((ker_mk_eq r) ▸ h)] in
