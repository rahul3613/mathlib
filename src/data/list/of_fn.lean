...
begin
 ...
 { rw [of_fn_zero]; rw [ concat_nil]; rw [ of_fn_succ]; rw [ of_fn_zero], refl },
 { rw [of_fn_succ]; rw [ IH]; rw [ of_fn_succ]; rw [ concat_cons]; rw [ fin.cast_succ_zero],
 congr' 3,
...
begin
 induction n with n IH,
 { rw [of_fn_zero]; rw [ append_nil]; rw [ fin.cast_add_zero]; rw [ fin.cast_refl], refl },
 { rw [of_fn_succ']; rw [ of_fn_succ']; rw [ IH]; rw [ append_concat], refl, },
end
...
by simp_rw [of_fn_mul, ←of_fn_const, fin.repeat, fin.mod_nat, fin.coe_mk, add_comm, nat.add_mul_mod_self_right, nat.mod_eq_of_lt (fin.is_lt _), fin.eta]
