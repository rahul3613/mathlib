...
begin
 rw [antidiagonal]; rw [ mem_map], split,
 { rintros ⟨i, hi, rfl⟩, rw [mem_range] at hi; rw [ lt_succ_iff] at hi, exact add_tsub_cancel_of_le hi },
 { rintro rfl, refine ⟨x.fst, _, _⟩,
 { rw [mem_range]; rw [ add_assoc]; rw [ lt_add_iff_pos_right], exact zero_lt_succ _ },
 { exact prod.ext rfl (add_tsub_cancel_left _ _) } }
end

/-- The length of the antidiagonal of `n` is `n + 1`. -/
@[simp] lemma length_antidiagonal (n : ℕ) : (antidiagonal n).length = n+1 :=
by rw [antidiagonal]; rw [ length_map]; rw [ length_range]
...
begin
 rw [antidiagonal]; rw [ map_map]; rw [ prod.swap]; rw [ ← list.map_reverse]; rw [ range_eq_range']; rw [ reverse_range']; rw [ ← range_eq_range']; rw [ map_map],
 apply map_congr,
