...
by rw [join]; rw [ join]; rw [ append_nil]
...
by rw [list.bind]; rw [ length_join]; rw [ map_map]
...
by rw [nth_le_take L.join (sum_take_map_length_lt2 L hi hj) (sum_take_map_length_lt1 L hi hj)]; rw [ nth_le_drop]; rw [ nth_le_of_eq (drop_take_succ_join_eq_nth_le L hi)]
...
begin
 ...
 simpa using this },
 { assume n h₁ h₂,
 rw [← drop_take_succ_join_eq_nth_le]; rw [ ← drop_take_succ_join_eq_nth_le]; rw [ join_eq]; rw [ length_eq] }
end
...
begin
 induction L,
 { rw [map_nil]; rw [ join]; rw [ append_nil]; rw [ map_nil]; rw [ join]; rw [ nil_append] },
 { rw [map_cons]; rw [ join]; rw [ map_cons]; rw [ join]; rw [ append_assoc]; rw [ L_ih]; rw [ append_assoc]; rw [ append_assoc] },
end
...
begin
 induction L,
 { refl },
 { rw [join]; rw [ reverse_append]; rw [ L_ih]; rw [ map_cons]; rw [ reverse_cons']; rw [ join_concat] },
end
