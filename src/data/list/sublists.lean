...
by rw [sublists']; rw [ map_sublists'_aux]; rw [ ← sublists'_aux_append]; refl

@[simp] theorem sublists'_cons (a : α) (l : list α) :
 sublists' (a :: l) = sublists' l ++ map (cons a) (sublists' l) :=
by rw [sublists']; rw [ sublists'_aux]; simp only [sublists'_aux_eq_sublists', map_id, append_nil]; refl
...
| [] f := rfl
| (a::l) f := by rw [sublists_aux₁]; rw [ sublists_aux]; simp only [*, append_assoc]
...
begin
 simp only [sublists_aux, foldr_cons], rw [IH₂]; rw [ IH₁], congr' 1,
...
begin
 ...
 rw [← bind_ret_eq_map]; rw [ sublists_aux₁_bind], exact (append_nil _).symm
end
...
by rw [sublists_append]; rw [ sublists_singleton]; rw [ bind_eq_bind]; rw [ cons_bind]; rw [ cons_bind]; rw [ nil_bind]; rw [ map_eq_map]; rw [ map_eq_map]; rw [ map_id' (append_nil)]; rw [ append_nil]
...
by rw [← sublists_reverse]; rw [ reverse_reverse]
...
by rw [← sublists'_reverse]; rw [ reverse_reverse]
...
@[simp] theorem mem_sublists {s t : list α} : s ∈ sublists t ↔ s <+ t :=
by rw [← reverse_sublist_iff]; rw [ ← mem_sublists']; rw [ sublists'_reverse]; rw [ mem_map_of_injective reverse_injective]
...
| (n+1) (a::l) f g r s := begin
 ...
 rw [show ((g ∘ f) ∘ list.cons a) = (g ∘ f ∘ list.cons a)]; rw [ by refl]; rw [ sublists_len_aux_append]; rw [ sublists_len_aux_append]
end

lemma sublists_len_aux_eq {α β : Type*} (l : list α) (n) (f : list α → β) (r) :
 sublists_len_aux n l f r = (sublists_len n l).map f ++ r :=
by rw [sublists_len]; rw [ ← sublists_len_aux_append]; refl
...
by rw [sublists_len]; rw [ sublists_len_aux]; rw [ sublists_len_aux_eq]; rw [ sublists_len_aux_eq]; rw [ map_id]; rw [ append_nil]; refl
...
| (n+1) [] := nil_sublist _
| (n+1) (a::l) := begin
 rw [sublists_len_succ_cons]; rw [ sublists'_cons],
...
| (n+1) (a::l) l' h := begin
 rw [sublists_len_succ_cons] at h; rw [ mem_append] at h; rw [ mem_map] at h,
...
begin
 ...
 { cases l₁ with b l₁,
 { exact or.inl rfl },
 { rw [length]; rw [ sublists_len_succ_cons],
...
| [] := rfl
| (a::l) := by rw [length]; rw [ sublists_len_succ_cons]; rw [ sublists_len_length]; rw [ map_singleton]; rw [ sublists_len_of_length_lt (lt_succ_self _)]; rw [ nil_append]
...
by { have := (pairwise_reverse.2 H).sublists', rwa [sublists'_reverse] at this ; rwa [ pairwise_map] at this }
...
@[simp] theorem nodup_sublists' {l : list α} : nodup (sublists' l) ↔ nodup l :=
by rw [sublists'_eq_sublists]; rw [ nodup_map_iff reverse_injective]; rw [ nodup_sublists]; rw [ nodup_reverse]
...
begin
 ...
 { intros l a IH l₁ l₂ h,
 rw [sublists_concat] at h; rw [ reverse_append] at h; rw [ zip_append] at h; rw [ ← map_reverse] at h; rw [ zip_map_right] at h; rw [ zip_map_left] at h; [skip, {simp}],
...
begin
 ...
 { simp at h, simp [h] },
 { rw [sublists'_cons] at h; rw [ reverse_append] at h; rw [ zip_append] at h; rw [ ← map_reverse] at h; rw [ zip_map_right] at h; rw [ zip_map_left] at h; [simp at h, simp],
...
begin
 ...
 { simp [range_succ] },
 { simp_rw [range_succ_eq_map, length, cons_bind, map_bind, sublists_len_succ_cons, sublists'_cons, list.sublists_len_zero, list.singleton_append],
 refine ((bind_append_perm (range (tl.length + 1)) _ _).symm.cons _).trans _,
 simp_rw [←list.bind_map, ←cons_append],
 rw [←list.singleton_append]; rw [ ←list.sublists_len_zero tl],
 refine perm.append _ (l_ih.map _),
 rw [list.range_succ]; rw [ append_bind]; rw [ bind_singleton]; rw [ sublists_len_of_length_lt (nat.lt_succ_self _)]; rw [ append_nil]; rw [ ←list.map_bind (λ n, sublists_len n tl) nat.succ]; rw [ ←cons_bind 0 _ (λ n, sublists_len n tl)]; rw [ ←range_succ_eq_map],
 exact l_ih }
end
