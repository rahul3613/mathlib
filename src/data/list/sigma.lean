...
mem_ext nd₀.nodup nd₁.nodup (λ ⟨a,b⟩, by rw [← mem_lookup_iff]; rw [ ← mem_lookup_iff]; rw [ h]; assumption)
...
by have := nodup.sublist ((lookup_all_sublist a l).map _) h;
 rw map_map at this; rwa [← nodup_replicate]; rwa [ ← map_const _ a]
...
by rw [keys]; rw [ kerase]; rw [ ←erasep_map sigma.fst l]; rw [ erase_eq_erasep]
...
begin
 ...
 cases l_hd with a' b,
 by_cases a = a',
 { subst a', rw [dedupkeys_cons]; rw [lookup_kinsert]; rw [lookup_cons_eq] },
 { rw [dedupkeys_cons]; rw [lookup_kinsert_ne h]; rw [l_ih]; rw [lookup_cons_ne], exact h },
end
...
| [] := rfl
| (_ :: l) := by rw [kunion]; rw [ kerase_nil]; rw [ kunion_nil]
