...
theorem mem_map_of_involutive {f : α → α} (hf : involutive f) {a : α} {l : list α} :
 a ∈ map f l ↔ f a ∈ l :=
by rw [mem_map]; rw [ hf.exists_mem_and_apply_eq_iff]
...
by { rw [insert_neg, singleton_eq], rwa [singleton_eq]; rwa [ mem_singleton] }
...
by rw [eq_comm]; rw [ append_eq_nil]
...
by rw [eq_comm]; rw [ append_eq_cons_iff]
...
begin
 ...
 { rintro (⟨a', rfl, rfl⟩ | ⟨a', H, rfl⟩), {refl}, {rw [← append_assoc]; rw [ ← H], refl} } },
 case cons : a as ih
 { cases c,
...
| (a::s₁) (b::s₂) t₁ t₂ h hl := list.no_confusion h $ λab hap,
 ...
 by rw [ab]; rw [ e1]; rw [ e2]; exact ⟨rfl, rfl⟩
...
begin
 ...
 rw [length_map]; rw [ length_take]; rw [ min_eq_left],
 rw [← length_map f l]; rw [ h]; rw [ length_append],
...
by { transitivity, rw [← bind_singleton' l]; rw [ bind_map], refl }
...
by induction s; [rw [nil_append]; rw [ reverse_nil]; rw [ append_nil],
simp only [*, cons_append, reverse_cons, append_assoc]]

theorem reverse_concat (l : list α) (a : α) : reverse (concat l a) = a :: reverse l :=
by rw [concat_eq_append]; rw [ reverse_append]; rw [ reverse_singleton]; rw [ singleton_append]
...
eq_replicate.2 ⟨by rw [length_reverse]; rw [ length_replicate],
...
| (a::b::l) h :=
begin
 rw [init]; rw [ cons_append]; rw [ last_cons (cons_ne_nil _ _)],
 congr,
...
| [] a l₂ := rfl
| [b] a l₂ := rfl
| (b::c::l₁) a l₂ := by rw [cons_append]; rw [ cons_append]; rw [ last']; rw [ ← cons_append]; rw [ last'_append_cons]
...
theorem tail_append_singleton_of_ne_nil {a : α} {l : list α} (h : l ≠ nil) :
 tail (l ++ [a]) = tail l ++ [a] :=
by { induction l, contradiction, rw [tail]; rw [cons_append]; rw [tail], }
...
begin
 ...
 { simp [nth_le, h] },
 cases l,
 { rw [length_singleton] at hl; rw [ lt_succ_iff] at hl; rw [ nonpos_iff_eq_zero] at hl, contradiction },
 cases n,
 { contradiction },
 refl
end
...
begin
 ...
 { rw [reverse_cons]; rw [ reverse_cons], exact ih.append_right [a] }
end
...
| ._ ._ (sublist.cons2 l₁ l₂ a s) h :=
 by rw [length] at h; rw [ length] at h; injection h with h; rw s.eq_of_length h
...
begin
 ...
 rw [index_of_cons_ne _ hh]; rw [ index_of_cons_ne _ hh]; rw [ ih (mem_of_ne_of_mem hh h)],
end
...
begin
 induction l₁ with d₁ t₁ ih,
 { rw [list.nil_append]; rw [ list.length]; rw [ zero_add] },
 rw [list.cons_append]; rw [ index_of_cons_ne _ (ne_of_not_mem_cons h)]; rw [ list.length]; rw [ ih (not_mem_of_not_mem_cons h)]; rw [ nat.succ_add],
end
...
begin
 ...
 rw [← nth_eq_some] at h₂; rw [ h] at h₂, cases h₂ },
 { solve_by_elim [nth_len_le] },
end

theorem nth_of_mem {a} {l : list α} (h : a ∈ l) : ∃ n, nth l n = some a :=
let ⟨n, h, e⟩ := nth_le_of_mem h in ⟨n, by rw [nth_le_nth]; rw [ e]⟩
...
option.some.inj $ by rw [← nth_le_nth]; rw [ nth_map]; rw [ nth_le_nth]; refl
...
begin
 ...
 rwa [nat.sub_add_cancel h₁]; rwa [ nat.add_comm],
end
...
by rw [nth_le_nth hn]; rw [ nth_le_nth hn']; rw [ nth_le_append]

lemma nth_append_right {l₁ l₂ : list α} {n : ℕ} (hn : l₁.length ≤ n) :
 (l₁ ++ l₂).nth n = l₂.nth (n - l₁.length) :=
begin
 by_cases hl : n < (l₁ ++ l₂).length,
 { rw [nth_le_nth hl]; rw [ nth_le_nth]; rw [ nth_le_append_right hn] },
 { rw [nth_len_le (le_of_not_lt hl)]; rw [ nth_len_le],
 rw [not_lt] at hl; rw [ length_append] at hl,
...
| [] h := rfl
| [a] h := by rw [last_singleton]; rw [ nth_le_singleton]
| (a :: b :: l) h := by { rw [last_cons]; rw [ last_eq_nth_le (b :: l)],
...
| [] a := rfl
| (b::l) a := by rw [cons_append]; rw [ length_cons]; rw [ nth]; rw [ nth_concat_length]
...
begin
 ...
 have h₂ := h, rw [length_cons] at h₂; rw [ nat.lt_succ_iff] at h₂; rw [ le_iff_eq_or_lt] at h₂,
 cases n, { simp }, rw [drop]; rw [ nth_le], apply ih },
end
...
by rw [nth_le_nth]; rw [ index_of_nth_le (index_of_lt_length.2 h)]
...
begin
 ...
 rw [add_tsub_cancel_left]; rw [ add_comm]; rw [ modify_nth_tail_modify_nth_tail]
end
...
by rw [modify_nth_tail_modify_nth_tail_le n n l (le_refl n)]; rw [ tsub_self]; refl
...
by rw [nth_update_nth_eq]; rw [ nth_le_nth h]; refl
...
by rw [← option.some_inj]; rw [ ← nth_le_nth]; rw [ nth_update_nth_eq]; rw [ nth_le_nth]; simp * at *
...
by rw [← option.some_inj]; rw [ ← list.nth_le_nth]; rw [ list.nth_update_nth_ne _ _ h]; rw [ list.nth_le_nth]
...
by rw [remove_nth_eq_nth_tail]; rw [ insert_nth]; rw [ modify_nth_tail_modify_nth_tail_same];
...
| (n+1) (a'::as) h := begin
 ...
 erw [mem_insert_nth (nat.le_of_succ_le_succ h)]; erw [ ← or.assoc]; erw [ or_comm (a = a')]; erw [ or.assoc]
end
...
@[simp] lemma nth_le_insert_nth_self (l : list α) (x : α) (n : ℕ)
 ...
 by rwa [length_insert_nth _ _ hn]; rwa [ nat.lt_succ_iff]) :
...
lemma nth_le_insert_nth_add_succ (l : list α) (x : α) (n k : ℕ)
 ...
 by rwa [length_insert_nth _ _ (le_self_add.trans hk'.le)]; rwa [ nat.succ_lt_succ_iff]) :
...
| (a::l) h := let ⟨h₁, h₂⟩ := forall_mem_cons.1 h in
 by rw [map]; rw [ map]; rw [ h₁]; rw [ map_congr h₂]
...
begin
 ...
 rw [nth_le_map_rev f]; rw [ nth_le_map_rev g], congr, exact h
end
...
eq_nil_of_length_eq_zero $ by rw [← length_map f l]; rw [ h]; refl
...
| n 0 l := by rw [min_zero]; rw [ take_zero]; rw [ take_nil]
| 0 m l := by rw [zero_min]; rw [ take_zero]; rw [ take_zero]
...
begin
 ...
 { rw take_append_drop },
 rw [take_append_eq_append_take]; rw [ take_all_of_le]; rw [ append_right_inj], swap,
 { transitivity m,
...
begin
 ...
 rw [nth_le_of_eq (take_append_drop i L).symm h]; rw [ nth_le_append_right];
 simp [A]
end
...
by rw [modify_nth_eq_take_drop]; rw [ drop_eq_nth_le_cons h]; refl

theorem update_nth_eq_take_cons_drop (a : α) {n l} (h : n < length l) :
 update_nth l n a = take n l ++ a :: drop (n+1) l :=
by rw [update_nth_eq_modify_nth]; rw [ modify_nth_eq_take_cons_drop _ h]
...
begin
 ...
 rwa [take_append_of_le_length]; rwa [ xs_ih _ h'],
 rw [show xs_tl.length + 1 - n = succ (xs_tl.length - n)]; rw [ from _]; rw [ drop],
 { rwa [succ_eq_add_one]; rwa [ ← tsub_add_eq_add_tsub] },
 { rwa length_reverse } },
 { subst h', rw [length]; rw [ tsub_self]; rw [ drop],
 suffices : xs_tl.length + 1 = (xs_tl.reverse ++ [xs_hd]).length,
 by rw [this]; rw [ take_length]; rw [ reverse_cons],
 rw [length_append]; rw [ length_reverse], refl }
end
...
begin
 ...
 unfold foldl,
 rw [ih (λ a b bin, H a b $ mem_cons_of_mem _ bin)]; rw [ H a hd (mem_cons_self _ _)]
end
...
| [] := rfl
| (b::l) := by rw [foldl_cons]; rw [ hf b]; rw [ foldl_fixed' l]

theorem foldr_fixed' {f : α → β → β} {b : β} (hf : ∀ a, f a b = b) :
 Π l : list α, foldr f b l = b
| [] := rfl
| (a::l) := by rw [foldr_cons]; rw [ foldr_fixed' l]; rw [ hf a]
...
| a [] := rfl
| a (x :: l) := by erw [length_cons]; erw [ length_cons]; erw [ length_scanl]
...
| a b (c::l) := by simp only [foldl_cons];
 rw [← foldl_eq_of_comm_of_assoc]; rw [ right_comm _ hcomm hassoc]; refl
...
| a b [] := rfl
| a b (c :: l) := by rw [foldl]; rw [foldl]; rw [foldl]; rw [← foldl_eq_of_comm']; rw [foldl]; rw [hf]

theorem foldl_eq_foldr' : ∀ a l, foldl f a l = foldr (flip f) a l
| a [] := rfl
| a (b :: l) := by rw [foldl_eq_of_comm' hf]; rw [foldr]; rw [foldl_eq_foldr']; refl
...
| a b [] := rfl
| a b (c :: l) := by rw [foldr]; rw [foldr]; rw [foldr]; rw [hf]; rw [← foldr_eq_of_comm']; refl
...
| (a :: l) a₁ a₂ :=
 ...
 ... = a₁ * (a::l <*> a₂) : by rw [foldl_assoc]; rw [ foldl_cons]
...
by rw [foldl_cons]; rw [ hc.comm]; rw [ foldl_assoc]
...
begin
 rw [split_on_p]; rw [ split_on_p_aux_nil],
 suffices : ∀ xs,
...
begin
 ...
 rw [ih (λ l, f (hd :: l))]; rw [ ih (λ l, id (hd :: l))],
 simp,
end
...
begin
 induction l with _ _ ih,
 { refl, },
 { rw [pmap]; rw [ pmap]; rw [ h _ (mem_cons_self _ _)]; rw [ ih (λ a ha, h a (mem_cons_of_mem _ ha))], },
end
...
by rw [attach]; rw [ map_pmap]; exact pmap_congr l (λ _ _ _ _, rfl)

@[simp] lemma attach_map_coe' (l : list α) (f : α → β) : l.attach.map (λ i, f i) = l.map f :=
by rw [attach]; rw [ map_pmap]; exact (pmap_eq_map _ _ _ _)
...
@[simp] lemma pmap_eq_nil {p : α → Prop} {f : Π a, p a → β}
 {l H} : pmap f l H = [] ↔ l = [] :=
by rw [← length_eq_zero]; rw [ length_pmap]; rw [ length_eq_zero]
...
begin
 ...
 { rwa [find_cons_of_neg _ h]; rwa [ iff_true_intro h]; rwa [ true_and] }
end
...
by rw [← map_id (l.lookmap f)]; rw [ lookmap_map_eq]; rw [ map_id]; exact h

theorem length_lookmap (l : list α) : length (l.lookmap f) = length l :=
by rw [← length_map]; rw [ lookmap_map_eq _ (λ _, ())]; rw [ length_map]; simp
...
begin
 ...
 { simp },
 { rw [cons_append]; rw [ filter_map]; rw [ filter_map],
 cases f hd;
...
begin
 induction l with a l IH, {refl},
 cases h : f a with b,
 { rw [filter_map_cons_none _ _ h]; rw [ filter_map_cons_none]; rw [ IH],
 simp only [h, option.none_bind'] },
 rw filter_map_cons_some _ _ _ h,
 cases h' : g b with c;
 [ rw [filter_map_cons_none _ _ h']; rw [ filter_map_cons_none]; rw [ IH],
 rw [filter_map_cons_some _ _ _ h']; rw [ filter_map_cons_some]; rw [ IH] ];
...
by rw [← filter_map_eq_map]; rw [ filter_map_filter_map]; refl
...
by rw [← filter_map_eq_filter]; rw [ filter_map_filter_map]; refl
...
begin
 rw [← filter_map_eq_filter]; rw [ filter_map_filter_map], congr,
 funext x,
...
begin
 ...
 { simp },
 { cases h : f x; rw [list.filter_map_cons]; rw [ h]; simp [h, ih] },
end
...
begin
 induction L with hd tl ih,
 { refl },
 { rw [map]; rw [ join]; rw [ join]; rw [ filter_map_append]; rw [ ih] },
end
...
begin
 rw [← list.length_map some]; rw [ list.map_filter_map_some_eq_filter_map_is_some]; rw [ ← list.length_map f],
...
begin
 rw [(reduce_option_length_le l).lt_iff_ne]; rw [ ne]; rw [ reduce_option_length_eq_iff],
 induction l; simp *,
 rw [eq_comm]; rw [ ← option.not_is_some_iff_eq_none]; rw [ decidable.imp_iff_not_or]
end
...
by rw [←mem_iff_nth]; rw [ ←mem_iff_nth]; rw [ reduce_option_mem_iff]
...
begin
 ...
 { rw [filter_cons_of_pos _ h]; rw [ cons_inj]; rw [ ih]; rw [ and_iff_right h] },
...
begin
 ...
 { refl },
 { by_cases hp : p hd,
 { rw [filter_cons_of_pos _ hp]; rw [ filter_cons_of_pos _ (h _ hp)],
...
by rw [← filter_map_eq_map]; rw [ filter_filter_map]; rw [ filter_map_filter]; refl
...
| [] := rfl
| (a::l) := if pa : p a then by rw [take_while]; rw [ drop_while]; rw [ if_pos pa]; rw [ if_pos pa]; rw [ cons_append]; rw [ take_while_append_drop l]
 else by rw [take_while]; rw [ drop_while]; rw [ if_neg pa]; rw [ if_neg pa]; rw [ nil_append]
...
by { rw [h₂]; rw [ h₁]; rw [ length_append]; rw [ length_append], refl }
...
by rw [erase_eq_erasep]; rw [ erasep_of_forall_not]; rintro b h' rfl; exact h h'
...
by rw [erase_eq_erasep]; rw [ length_erasep_add_one h rfl]
...
by rw [erase_eq_erasep]; rw [ erase_eq_erasep]; rw [ erasep_append_right];
...
| ._, ._, ⟨l₁, l₂, ha', rfl, rfl⟩, hb :=
 if h₁ : b ∈ l₁ then
 by rw [erase_append_left _ h₁]; rw [ erase_append_left _ h₁]; rw [ erase_append_right _ (mt mem_of_mem_erase ha')]; rw [ erase_cons_head]
 else
 by rw [erase_append_right _ h₁]; rw [ erase_append_right _ h₁]; rw [ erase_append_right _ ha']; rw [ erase_cons_tail _ ab]; rw [ erase_cons_head]
end
...
begin
 ...
 { rw [diff_cons]; rw [ diff_cons]; rw [ erase_comm]; rw [ ← diff_cons]; rw [ ih]; rw [ ← diff_cons] }
end

lemma diff_erase (l₁ l₂ : list α) (a : α) : (l₁.diff l₂).erase a = (l₁.erase a).diff l₂ :=
by rw [← diff_cons_right]; rw [ diff_cons]
...
begin
 ...
 { simp },
 { rw [cons_append]; rw [ enum_from_cons]; rw [ IH]; rw [ ←cons_append]; rw [ ←enum_from_cons]; rw [ length]; rw [ add_right_comm]; rw [ add_assoc] }
end
...
by rw [enum_from_cons]; rw [ add_comm]; rw [ ←map_fst_add_enum_from_eq_enum_from]
...
begin
 induction l with hd tl IH,
 { refl },
 { rw [map_cons]; rw [ enum_from_cons']; rw [ enum_from_cons']; rw [ map_cons]; rw [ map_map]; rw [ IH]; rw [ map_map],
 refl, },
end
...
| [] i := by cases i; refl
| (x::xs) 0 := by rw [to_chunks_aux]; rw [ drop]; rw [ to_chunks]; cases to_chunks_aux n xs n; refl
| (x::xs) (i+1) := by rw [to_chunks_aux]; rw [ to_chunks_aux_eq]; refl

theorem to_chunks_eq_cons' (n) : ∀ {xs : list α} (h : xs ≠ []),
 xs.to_chunks (n+1) = xs.take (n+1) :: (xs.drop (n+1)).to_chunks (n+1)
| [] e := (e rfl).elim
| (x::xs) _ := by rw [to_chunks]; rw [ to_chunks_aux_eq]; refl
...
| (x::xs) i l L e := begin
 ...
 { rw [to_chunks_aux] at e; rw [ e'] at e; rw [ to_chunks_aux] at e, cases e,
...
| (x :: l) := by rw [ball_cons]; rw [ all₂_cons]; rw [ all₂_iff_forall]
...
begin
 rw [last_eq_nth_le]; rw [ nth_le_reverse'],
 { simp, },
 { simpa using hl' }
end
...
by rw [nthd_eq_nth_le _ _ h']; rw [ nth_le_append h' h]; rw [ nthd_eq_nth_le]

lemma nthd_append_right (l l' : list α) (d : α) (n : ℕ) (h : l.length ≤ n) :
 (l ++ l').nthd n d = l'.nthd (n - l.length) d :=
begin
 cases lt_or_le _ _ with h' h',
 { rw [nthd_eq_nth_le _ _ h']; rw [ nth_le_append_right h h']; rw [ nthd_eq_nth_le] },
 { rw [nthd_eq_default _ _ h']; rw [ nthd_eq_default],
 rwa [le_tsub_iff_left h]; rwa [ ←length_append] }
end
...
begin
 cases lt_or_le _ _ with h h,
 { rw [nthd_eq_nth_le _ _ h]; rw [ nth_le_nth h]; rw [ option.get_or_else_some] },
 { rw [nthd_eq_default _ _ h]; rw [ nth_eq_none_iff.mpr h]; rw [ option.get_or_else_none] }
end
...
by rw [←nthd_default_eq_inth]; rw [ nthd_eq_get_or_else_nth]; rw [ option.get_or_else_default_eq_iget]
