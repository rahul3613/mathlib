...
begin
 refine ⟨λ h, _, λ h, _⟩,
 { refine ⟨mem_dedup.1 (h.symm ▸ mem_cons_self _ _), λ ha, _, by rw [h]; rw [ tail_cons]⟩,
 ...
 rw [← this] at hal; rw [ mem_cons_iff] at hal; rw [ or_iff_not_imp_right] at hal,
...
begin
 induction l₁ with a l₁ IH, {refl}, rw [cons_union]; rw [ ← IH],
 show dedup (a :: (l₁ ++ l₂)) = insert a (dedup (l₁ ++ l₂)),
 by_cases a ∈ dedup (l₁ ++ l₂);
 [ rw [dedup_cons_of_mem' h]; rw [ insert_of_mem h],
 rw [dedup_cons_of_not_mem' h]; rw [ insert_of_not_mem h]]
end

lemma replicate_dedup {x : α} : ∀ {k}, k ≠ 0 → (replicate k x).dedup = [x]
| 0 h := (h rfl).elim
| 1 _ := rfl
| (n+2) _ := by rw [replicate_succ]; rw [ dedup_cons_of_mem (mem_replicate.2 ⟨n.succ_ne_zero, rfl⟩)]; rw [ replicate_dedup n.succ_ne_zero]
