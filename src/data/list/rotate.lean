...
| [] n := rfl
| (a::l) 0 := rfl
| (a::l) (n+1) := by rw [list.rotate']; rw [ length_rotate' (l ++ [a]) n]; simp
...
have hnl' : n ≤ (l ++ [a]).length,
 by rw [length_append]; rw [ length_cons]; rw [ list.length]; rw [ zero_add];
 exact (le_of_succ_le h),
by rw [rotate'_cons_succ]; rw [ rotate'_eq_drop_append_take hnl']; rw [ drop]; rw [ take]; rw [ drop_append_of_le_length hnl]; rw [ take_append_of_le_length hnl];
 simp

lemma rotate'_rotate' : ∀ (l : list α) (n m : ℕ), (l.rotate' n).rotate' m = l.rotate' (n + m)
| (a::l) 0 m := by simp
| [] n m := by simp
| (a::l) (n+1) m := by rw [rotate'_cons_succ]; rw [ rotate'_rotate']; rw [ add_right_comm]; rw [ rotate'_cons_succ]
...
... = l : by rw [rotate'_length]; rw [ rotate'_length_mul]
...
... = l.rotate' n : by rw [rotate'_rotate']; rw [ length_rotate']; rw [ nat.mod_add_div]
...
by rw [rotate_eq_rotate']; rw [ rotate_eq_rotate']; rw [ rotate'_cons_succ]
...
@[simp] lemma length_rotate (l : list α) (n : ℕ) : (l.rotate n).length = l.length :=
by rw [rotate_eq_rotate']; rw [ length_rotate']

lemma rotate_replicate (a : α) (n : ℕ) (k : ℕ) : (replicate n a).rotate k = replicate n a :=
eq_replicate.2 ⟨by rw [length_rotate]; rw [ length_replicate],
...
by rw [rotate_eq_rotate']; rw [ rotate_eq_rotate']; rw [ rotate_eq_rotate']; rw [ rotate'_rotate']

@[simp] lemma rotate_length (l : list α) : rotate l l.length = l :=
by rw [rotate_eq_rotate']; rw [ rotate'_length]

@[simp] lemma rotate_length_mul (l : list α) (n : ℕ) : l.rotate (l.length * n) = l :=
by rw [rotate_eq_rotate']; rw [ rotate'_length_mul]
...
by rw ← list.take_append_drop (n % list.length (a :: l)) (a :: l) at hl;
 rw [← rotate_mod]; rw [ rotate_eq_drop_append_take this]; rw [ list.prod_append]; rw [ mul_eq_one_iff_inv_eq]; rw [ ← one_mul (list.prod _)⁻¹]; rw [ ← hl]; rw [ list.prod_append]; rw [ mul_assoc]; rw [ mul_inv_self]; rw [ mul_one]
...
by rw [eq_comm]; rw [ rotate_eq_nil_iff]; rw [ eq_comm]
...
begin
 rw [rotate_eq_drop_append_take_mod]; rw [ rotate_eq_drop_append_take_mod]; rw [ rotate_eq_drop_append_take_mod]; rw [ h]; rw [ zip_with_append]; rw [ ←zip_with_distrib_drop]; rw [ ←zip_with_distrib_take]; rw [ list.length_zip_with]; rw [ h]; rw [ min_self],
...
begin
 ...
 congr,
 set m := l.length,
 rw [mod_add_mod]; rw [ add_assoc]; rw [ add_left_comm]; rw [ add_comm]; rw [ add_mod]; rw [ add_mod _ n],
...
begin
 rw [nth_le_nth]; rw [ nth_le_nth (nat.mod_lt _ _)]; rw [ nth_le_rotate],
 rwa [length_rotate]
end

lemma head'_rotate {l : list α} {n : ℕ} (h : n < l.length) :
 head' (l.rotate n) = l.nth n :=
by rw [← nth_zero]; rw [ nth_rotate (n.zero_le.trans_lt h)]; rw [ zero_add]; rw [ nat.mod_eq_of_lt h]
...
| (a :: l) := ⟨λ h, ⟨a, ext_le (length_replicate _ _).symm $ λ n h₁ h₂,
 begin
 inhabit α,
 rw [nth_le_replicate]; rw [ ← option.some_inj]; rw [ ← nth_le_nth]; rw [ ← head'_rotate h₁]; rw [ h]; rw [ head']
 end⟩, λ ⟨b, hb⟩ n, by rw [hb]; rw [ rotate_replicate]⟩
...
⟨λ h, rotate_eq_self_iff_eq_replicate.mp (λ n, nat.rec l.rotate_zero
 (λ n hn, by rwa [nat.succ_eq_add_one]; rwa [ ←l.rotate_rotate]; rwa [ hn]) n),
...
begin
 ...
 rw [← nth_le_rotate' l₁ n]; rw [ ← nth_le_rotate' l₂ n],
...
begin
 ...
 cases l'.length.zero_le.eq_or_lt with hl hl,
 { rw [eq_nil_of_length_eq_zero hl.symm]; rw [ rotate_nil]; rw [ rotate_eq_nil_iff] },
 { cases (nat.zero_le (n % l'.length)).eq_or_lt with hn hn,
 { simp [←hn] },
 { rw [mod_eq_of_lt (tsub_lt_self hl hn)]; rw [ tsub_add_cancel_of_le]; rw [ mod_self]; rw [ rotate_zero],
 exact (nat.mod_lt _ hl).le } }
end

@[simp] lemma rotate_eq_singleton_iff {l : list α} {n : ℕ} {x : α} : l.rotate n = [x] ↔ l = [x] :=
by rw [rotate_eq_iff]; rw [ rotate_singleton]

@[simp] lemma singleton_eq_rotate_iff {l : list α} {n : ℕ} {x : α} : [x] = l.rotate n ↔ [x] = l :=
by rw [eq_comm]; rw [ rotate_eq_singleton_iff]; rw [ eq_comm]
...
begin
 ...
 { cases l with hd tl,
 { simp },
 { rw [rotate_cons_succ]; rw [ nat.succ_eq_add_one]; rw [ ←rotate_rotate]; rw [ hn],
 simp } }
end
...
begin
 ...
 rw [nat.mod_eq_of_lt]; rw [ tsub_add_cancel_of_le]; rw [ rotate_length],
 { exact tsub_le_self },
...
begin
 ...
 rw [←nth_le_rotate' l i]; rw [ ←nth_le_rotate' l j],
...
by rw [is_rotated_comm]; rw [ is_rotated_nil_iff]; rw [ eq_comm]

@[simp] lemma is_rotated_singleton_iff {x : α} : l ~r [x] ↔ l = [x] :=
⟨λ ⟨n, hn⟩, by simpa using hn, λ h, h ▸ by refl⟩

@[simp] lemma is_rotated_singleton_iff' {x : α} : [x] ~r l ↔ [x] = l :=
by rw [is_rotated_comm]; rw [ is_rotated_singleton_iff]; rw [ eq_comm]
...
begin
 ...
 rw [eq_comm]; rw [ ←is_rotated_nil_iff']; rw [ ←mem_cyclic_permutations_iff]; rw [ h]; rw [ mem_singleton]
end
...
begin
 ...
 rw [eq_comm]; rw [ ←is_rotated_singleton_iff']; rw [ ←mem_cyclic_permutations_iff]; rw [ h]; rw [ mem_singleton]
end
...
begin
 ...
 rw [←mod_eq_of_lt hi]; rw [ ←mod_eq_of_lt hj]; rw [ ←length_cons x l],
...
begin
 ...
 rw [nth_le_cyclic_permutations]; rw [ nth_le_rotate]; rw [ nth_le_cyclic_permutations]; rw [ rotate_rotate]; rw [ ←rotate_mod]; rw [ add_comm],
 cases l;
 simp
end
...
begin
 ...
 rw [←nth_le_cyclic_permutations _ _ (hk'.trans_le hl'.ge)]; rw [ ←nth_le_rotate' _ k],
