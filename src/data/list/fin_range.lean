...
begin
 ...
 rw [map_cons]; rw [ map_coe_fin_range]; rw [ range_succ_eq_map]; rw [ fin.coe_zero]; rw [ ←map_coe_fin_range]; rw [ map_map]; rw [ map_map]; rw [ function.comp]; rw [ function.comp],
 congr' 2 with x,
...
ext_le (by rw [length_map]; rw [ length_fin_range]) $ λ n _ h,
...
by rw [← of_fn_id]; rw [ map_of_fn]; rw [ function.right_id]
...
lemma equiv.perm.of_fn_comp_perm {n : ℕ} {α : Type u} (σ : equiv.perm (fin n)) (f : fin n → α) :
 of_fn (f ∘ σ) ~ of_fn f :=
begin
 rw [of_fn_eq_map]; rw [ of_fn_eq_map]; rw [ ←map_map],
