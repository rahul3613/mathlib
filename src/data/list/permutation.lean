...
begin
 rw [map_permutations_aux2' id]; rw [ map_id]; rw [ map_id], refl,
 simp
end
...
by rw [← permutations_aux2_append]; rw [ map_permutations_aux2]; rw [ permutations_aux2_comp_append]
...
begin
 ...
 { simp {contextual := tt} },
 rw [permutations_aux2_snd_cons]; rw [ show (λ (x : list α), l ++ y :: x) = append (l ++ [y]),
...
begin
 rw [length_foldr_permutations_aux2]; rw [ (_ : sum (map length L) = n * length L)],
...
by rw [permutations_aux]; rw [ permutations_aux.rec]
...
by rw [permutations_aux]; rw [ permutations_aux.rec]; refl

@[simp] lemma permutations_nil : permutations ([] : list α) = [[]] :=
by rw [permutations]; rw [ permutations_aux_nil]
...
by rw [permutations]; rw [ permutations]; rw [ map]; rw [ map_permutations_aux]; rw [ map]
