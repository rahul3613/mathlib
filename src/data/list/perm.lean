...
have ((flip (forall₂ r) ∘r forall₂ r) ∘r perm) b d,
 by rwa [← forall₂_comp_perm_eq_perm_comp_forall₂] at this; rwa [ ← relation.comp_assoc] at this,
...
by rw [countp_eq_length_filter]; rw [ countp_eq_length_filter];
...
perm_induction_on p
 ...
 (λ x y t₁ t₂ p r b, by simp; rw [lcomm]; rw [ r b])
...
by rw [erase_of_not_mem h₁]; rw [ erase_of_not_mem h₂]; exact p
...
begin
 ...
 simp_rw [ne.def, ← and_imp, ← not_or_distrib, decidable.not_imp_not, subset_def, mem_cons_iff, not_mem_nil, or_false, or_comm],
end
...
⟨λ ⟨s, hla, h⟩, by rwa [perm_singleton.mp hla] at h; rwa [ singleton_sublist] at h,
...
theorem perm.bind_left (l : list α) {f g : α → list β} (h : ∀ a ∈ l, f a ~ g a) :
 l.bind f ~ l.bind g :=
perm.join_congr $
 by rwa [list.forall₂_map_right_iff]; rwa [list.forall₂_map_left_iff]; rwa [ list.forall₂_same]
...
begin
 ...
 cases h₃ with _ _ h₃ h₄,
 rwa [@filter_congr' _ _ (∈ take n_n h_l_tl)]; rwa [ n_ih],
...
begin
 ...
 introv IH1 IH2 m,
 rw [permutations_aux_cons] at m; rw [ permutations] at m; rw [ mem_foldr_permutations_aux2] at m,
...
begin
 ...
 rw [permutations_aux_cons]; rw [ length_foldr_permutations_aux2' _ _ _ _ _ (λ l m, (perm_of_mem_permutations m).length_eq)]; rw [ permutations]; rw [ length]; rw [ length]; rw [ IH2]; rw [ nat.succ_add]; rw [ nat.factorial_succ]; rw [ mul_comm (nat.succ _)]; rw [ ← IH1]; rw [ add_comm (_*_)]; rw [ add_assoc]; rw [ nat.mul_succ]; rw [ mul_comm]
end
...
begin
 ...
 rw [permutations_aux_cons]; rw [ mem_foldr_permutations_aux2],
...
begin
 ...
 rw [permutations'_aux_eq_permutations_aux2]; rw [ permutations_aux2_append]
end
...
begin
 ...
 { simp [take_while], },
 { rw [permutations'_aux]; rw [ count_cons_self],
 by_cases hx : x = y,
 { subst hx,
 ...
 { simp },
 { rw [count_eq_zero_of_not_mem]; rw [ length]; rw [ zero_add],
 simp [hx, ne.symm hx] } } }
end
...
begin
 ...
 rw [←nth_le_permutations'_aux s x s.length (by simp)]; rw [ ←nth_le_permutations'_aux t x s.length (by simp [hl])],
 simp [h, hl]
end
...
begin
 ...
 { simpa using hk },
 rw [nth_le_permutations'_aux]; rw [ nth_le_permutations'_aux],
 have hl : length (insert_nth k x s) = length (insert_nth (k + 1) x s),
 { rw [length_insert_nth _ _ hk.le]; rw [ length_insert_nth _ _ (nat.succ_le_of_lt hk)] },
 refine ext_le hl (λ n hn hn', _),
 rcases lt_trichotomy n k with H|rfl|H,
 { rw [nth_le_insert_nth_of_lt _ _ _ _ H (H.trans hk)]; rw [ nth_le_insert_nth_of_lt _ _ _ _ (H.trans (nat.lt_succ_self _))] },
 { rw [nth_le_insert_nth_self _ _ _ hk.le]; rw [ nth_le_insert_nth_of_lt _ _ _ _ (nat.lt_succ_self _) hk]; rw [ hk'] },
 ...
 rw [length_insert_nth _ _ hk.le] at hn; rw [ nat.succ_lt_succ_iff] at hn; rw [ nat.succ_add] at hn,
...
begin
 ...
 rw [nodup_permutations'_aux_iff]; rw [ hy.mem_iff],
 exact λ H, h x H rfl },
 ...
 (by rwa [length_insert_nth _ _ hn]; rwa [ nat.lt_succ_iff]; rwa [ hl]) = x,
 { simp [hn', ←hm', hm] },
 have hx' : nth_le (insert_nth m x bs) n
 (by rwa [length_insert_nth _ _ hm]; rwa [ nat.lt_succ_iff]; rwa [ ←hl]) = x,
 { simp [hm', ←hn', hn] },
 rcases lt_trichotomy n m with ht|ht|ht,
 { suffices : x ∈ bs,
 { exact h x (hb.subset this) rfl },
 rw [←hx']; rw [ nth_le_insert_nth_of_lt _ _ _ _ ht (ht.trans_le hm)],
 ...
 rw [←hx]; rw [ nth_le_insert_nth_of_lt _ _ _ _ ht (ht.trans_le hn)],
 exact nth_le_mem _ _ _ } } }
end
