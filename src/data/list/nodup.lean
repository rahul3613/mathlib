...
begin
 ...
 by_cases b = a,
 { subst h, rw [erase_cons_head]; rw [ filter_cons_of_neg],
 symmetry, rw filter_eq_self, simpa only [ne.def, eq_comm] using m, exact not_not_intro rfl },
 { rw [erase_cons_tail _ h]; rw [ filter_cons_of_pos]; rw [ IH], exact h }
end
...
by rw [d.erase_eq_filter]; rw [ mem_filter]; rw [ and_comm]
...
by simp only [list.bind, nodup_join, pairwise_map, and_comm, and.left_comm, mem_map,
 ...
 rw [show (∀ (l : list β) (x : α), f x = l → x ∈ l₁ → nodup l) ↔ (∀ (x : α), x ∈ l₁ → nodup (f x))]; rw [ from forall_swap.trans $ forall_congr $ λ_]; rw [ forall_eq']
...
else by rw [insert_of_not_mem h']; rw [ nodup_cons]; split; assumption
...
by rw [hl₁.diff_eq_filter]; rw [ mem_filter]
