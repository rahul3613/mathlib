...
by rw [Ico]; rw [ tsub_zero]; rw [ range_eq_range']
...
begin
 ...
 { rw [tsub_eq_zero_iff_le.mpr hmn]; rw [ add_zero],
...
by rw [Ico]; rw [ Ico]; rw [ map_add_range']; rw [ add_tsub_add_eq_tsub_right]; rw [ add_comm n k]

theorem map_sub (n m k : ℕ) (h₁ : k ≤ n) : (Ico n m).map (λ x, x - k) = Ico (n - k) (m - k) :=
by rw [Ico]; rw [ Ico]; rw [ tsub_tsub_tsub_cancel_right h₁]; rw [ map_sub_range' _ _ _ h₁]

@[simp] theorem self_empty {n : ℕ} : Ico n n = [] :=
eq_nil_of_le (le_refl n)

@[simp] theorem eq_empty_iff {n m : ℕ} : Ico n m = [] ↔ m ≤ n :=
iff.intro (assume h, tsub_eq_zero_iff_le.mp $ by rw [← length]; rw [ h]; rw [ list.length]) eq_nil_of_le
...
begin
 ...
 { rwa [← add_tsub_assoc_of_le hnm]; rwa [ tsub_add_cancel_of_le] }
end
...
by { rwa [← succ_singleton]; rwa [ append_consecutive], exact nat.le_succ _ }

theorem eq_cons {n m : ℕ} (h : n < m) : Ico n m = n :: Ico (n + 1) m :=
by { rw [← append_consecutive (nat.le_succ n) h]; rw [ succ_singleton], refl }
...
begin
 ...
 { rw [← append_consecutive hnl hlm]; rw [ filter_append]; rw [ filter_lt_of_top_le (le_refl l)]; rw [ filter_lt_of_le_bot (le_refl l)]; rw [ append_nil] },
 { rw [eq_nil_of_le hln]; rw [ filter_lt_of_le_bot hln] }
end

@[simp] lemma filter_lt (n m l : ℕ) : (Ico n m).filter (λ x, x < l) = Ico n (min m l) :=
begin
 cases le_total m l with hml hlm,
 { rw [min_eq_left hml]; rw [ filter_lt_of_top_le hml] },
 { rw [min_eq_right hlm]; rw [ filter_lt_of_ge hlm] }
end
...
begin
 ...
 { rw [← append_consecutive hnl hlm]; rw [ filter_append]; rw [ filter_le_of_top_le (le_refl l)]; rw [ filter_le_of_le_bot (le_refl l)]; rw [ nil_append] },
 { rw [eq_nil_of_le hml]; rw [ filter_le_of_top_le hml] }
end

@[simp] lemma filter_le (n m l : ℕ) : (Ico n m).filter (λ x, l ≤ x) = Ico (max n l) m :=
begin
 cases le_total n l with hnl hln,
 { rw [max_eq_right hnl]; rw [ filter_le_of_le hnl] },
 { rw [max_eq_left hln]; rw [ filter_le_of_le_bot hln] }
end
