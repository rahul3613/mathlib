...
begin
 ...
 rw [finsupp.single_apply_eq_zero.mpr]; rw [ zero_add]; rw [ hi]; rw [ finsupp.emb_domain_apply],
 simp }
end
...
begin
 ...
 { rw [to_finsupp_apply_le _ _ hi.le]; rw [ to_finsupp_apply_le]; rw [ if_neg hi.ne]; rw [ add_zero],
 simpa using nat.succ_le_of_lt hi },
 { rw [to_finsupp_apply_lt]; rw [ to_finsupp_apply_le _ _ le_rfl]; rw [ if_pos rfl]; rw [ zero_add]; rw [ nth_le_append_right le_rfl],
 { simp },
 { simp } },
 { rw [to_finsupp_apply_lt _ _ hi]; rw [ to_finsupp_apply_lt]; rw [ if_neg hi.ne']; rw [ add_zero]; rw [ nth_le_append],
