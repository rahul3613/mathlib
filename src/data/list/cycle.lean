...
begin
 ...
 by_cases h : x = y,
 { rw [h]; rw [ next_or_self_cons_cons]; rw [ next_or_self_cons_cons] },
 { rw [next_or]; rw [ next_or]; rw [ IH];
 simpa [h] using x_mem }
end
...
begin
 ...
 rw [cons_append]; rw [ next_or_cons_of_ne _ _ _ _ hz]; rw [ IH hzs] }
end
...
lemma next_cons_cons_eq' (y z : α) (h : x ∈ (y :: z :: l)) (hx : x = y) :
 next (y :: z :: l) x h = z :=
by rw [next]; rw [ next_or]; rw [ if_pos hx]
...
begin
 rw [next]; rw [ next]; rw [ next_or_cons_of_ne _ _ _ _ hy]; rw [ next_or_eq_next_or_of_mem_of_ne],
 { rwa last_cons at hx },
 { simpa [hy] using h }
end
...
lemma next_last_cons (y : α) (h : x ∈ (y :: l)) (hy : x ≠ y)
 ...
 next (y :: l) x h = y :=
begin
 rw [next]; rw [ nth_le]; rw [ ←init_append_last (cons_ne_nil y l)]; rw [ hx]; rw [ next_or_concat],
 subst hx,
 intro H,
...
begin
 cases l,
 { simp [prev, hy, hz] },
 { rw [prev]; rw [ dif_neg hy]; rw [ if_pos hz] }
end
...
begin
 cases l,
 { simpa [hy, hz] using h },
 { rw [prev]; rw [ dif_neg hy]; rw [ if_neg hz] }
end
...
begin
 ...
 { simp },
 { intros,
 rw [nth_le_pmap]; rw [ nth_le_rotate]; rw [ next_nth_le _ h] }
end
...
begin
 ...
 { simp },
 { intros n hn hn',
 rw [nth_le_rotate]; rw [ nth_le_pmap]; rw [ prev_nth_le _ h] }
end
...
begin
 ...
 { simp_rw [pmap_prev_eq_rotate_length_sub_one _ (nodup_reverse.mpr h), rotate_reverse, length_reverse, nat.mod_eq_of_lt (tsub_lt_self lpos nat.succ_pos'), tsub_tsub_cancel_of_le (nat.succ_le_of_lt lpos)],
 rw ←nth_le_reverse,
...
begin
 ...
 rw [next_nth_le _ (h.nodup_iff.mp hn)]; rw [ ←nth_le_rotate' _ n],
 simp [add_assoc]
end
...
quot.sound ⟨1, by rw [rotate_cons_succ]; rw [ rotate_zero]⟩
...
quotient.induction_on' s $ λ l, by { rw [lists]; rw [ quotient.lift_on'_mk'], simp }

@[simp] lemma lists_nil : lists (@nil α) = [([] : list α)] :=
by rw [nil]; rw [ lists_coe]; rw [ cyclic_permutations_nil]
...
by { rw [←next_reverse_eq_prev]; rw [ ←mem_reverse_iff], apply next_mem }
...
λ a b hab, propext $ begin
 ...
 rw [hn.1]; rw [ hn.2] },
 { rw [nat.succ_eq_one_add] at hn; rw [ ←rotate_rotate] at hn; rw [ rotate_cons_succ] at hn; rw [ rotate_zero] at hn; rw [ cons_append] at hn,
 rw [←hd c _ _ _ hn],
 simp [and.comm] } } }
end
...
by rw [chain_coe_cons]; rw [ nil_append]; rw [ chain_singleton]
...
begin
 ...
 intros m a H _,
 rw [←coe_cons_eq_coe_append]; rw [ chain_coe_cons]; rw [ last_append_singleton]
end
...
by rw [range_succ]; rw [ ←coe_cons_eq_coe_append]; rw [ chain_coe_cons]; rw [ ←range_succ]; rw [ chain_range_succ]
...
⟨begin
 ...
 intros hs b hb c hc,
 rw [cycle.chain_coe_cons] at hs; rw [ chain_iff_pairwise] at hs,
