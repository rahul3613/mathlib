...
by rw [rdrop_while_concat]; rw [ if_pos h]
@[simp] lemma rdrop_while_concat_neg (x : α) (h : ¬ p x) :
 rdrop_while p (l ++ [x]) = l ++ [x] :=
by rw [rdrop_while_concat]; rw [ if_neg h]
...
begin
 rw [←reverse_suffix]; rw [ rdrop_while]; rw [ reverse_reverse],
...
begin
 ...
 rw [nth_le_reverse'],
 { simp },
 { rw [←ne.def] at h; rw [ ←pos_iff_ne_zero] at h,
...
by rw [rtake_while_concat]; rw [ if_pos h]
@[simp] lemma rtake_while_concat_neg (x : α) (h : ¬ p x) :
 rtake_while p (l ++ [x]) = [] :=
by rw [rtake_while_concat]; rw [ if_neg h]

lemma rtake_while_suffix : l.rtake_while p <:+ l :=
begin
 rw [←reverse_prefix]; rw [ rtake_while]; rw [ reverse_reverse],
...
begin
 ...
 rwa [←mem_reverse]; rwa [ ←rtake_while]
end
