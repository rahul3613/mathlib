...
| [] := rfl
| (a :: l) := by rw [join]; rw [ countp_append]; rw [ map_cons]; rw [ sum_cons]; rw [ countp_join]
...
by { rw [← not_iff_not]; rw [ ← ne.def]; rw [ ← pos_iff_ne_zero]; rw [ countp_pos], simp }

@[simp] lemma countp_eq_length {l} : countp p l = l.length ↔ ∀ a ∈ l, p a :=
by rw [countp_eq_length_filter]; rw [ filter_length_eq_length]

lemma length_filter_lt_length_iff_exists (l) : length (filter p l) < length l ↔ ∃ x ∈ l, ¬p x :=
by rw [length_eq_countp_add_countp p l]; rw [ ← countp_pos]; rw [ countp_eq_length_filter]; rw [ lt_add_iff_pos_right]
...
| [] := rfl
| (a::l) := by rw [map_cons]; rw [ countp_cons]; rw [ countp_cons]; rw [ countp_map]

@[simp] lemma countp_attach (l : list α) : l.attach.countp (λ a, p ↑a) = l.countp p :=
by rw [←countp_map]; rw [ attach_map_coe]
...
by rw [count]; rw [ countp_eq_length_filter]; rw [ filter_eq_self.2]; rw [ length_replicate];
...
by rw [list.bind]; rw [ count_join]; rw [ map_map]
...
begin
 rw [count]; rw [ count]; rw [ countp_map],
...
begin
 rw [erase_cons],
 by_cases hc : c = b,
 { rw [if_pos hc]; rw [ hc]; rw [ count_cons']; rw [ nat.add_sub_cancel] },
 { rw [if_neg hc]; rw [ count_cons']; rw [ count_cons']; rw [ count_erase],
 by_cases ha : a = b,
 { rw [← ha] at hc; rw [ eq_comm] at hc,
 rw [if_pos ha]; rw [ if_neg hc]; rw [ add_zero]; rw [ add_zero] },
 { rw [if_neg ha]; rw [ tsub_zero]; rw [ tsub_zero] } }
end

@[simp] lemma count_erase_self (a : α) (l : list α) : count a (list.erase l a) = count a l - 1 :=
by rw [count_erase]; rw [ if_pos rfl]

@[simp] lemma count_erase_of_ne {a b : α} (ab : a ≠ b) (l : list α) :
 count a (l.erase b) = count a l :=
by rw [count_erase]; rw [ if_neg ab]; rw [ tsub_zero]
