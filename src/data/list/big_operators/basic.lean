...
by rw [concat_eq_append]; rw [ prod_append]; rw [ prod_singleton]
...
list.rec_on l rfl $ λ a l ihl, by rw [prod_cons]; rw [ foldr_cons]; rw [ ihl]
...
begin
 ...
 { rw pow_zero, refl },
 { rw [list.replicate_succ]; rw [ list.prod_cons]; rw [ ih]; rw [ pow_succ] }
end

@[to_additive sum_eq_card_nsmul]
lemma prod_eq_pow_card (l : list M) (m : M) (h : ∀ (x ∈ l), x = m) :
 l.prod = m ^ l.length :=
by rw [← prod_replicate]; rw [ ← eq_replicate_length.2 h]
...
begin
 ...
 rw [prod_cons] at h; rw [ is_unit.mul_iff] at h,
...
| L 0 := by simp
| (h :: t) (n+1) := by { dsimp, rw [prod_cons]; rw [ prod_cons]; rw [ mul_assoc]; rw [ prod_take_mul_prod_drop] }
...
| [] i p := by cases p
| (h :: t) 0 _ := by simp
| (h :: t) (n+1) _ := by { dsimp, rw [prod_cons]; rw [ prod_cons]; rw [ prod_take_succ]; rw [ mul_assoc] }
...
begin
 induction L with a L ihL,
 { simp },
 { rw [prod_cons]; rw [ mul_eq_zero]; rw [ ihL]; rw [ mem_cons_iff]; rw [ eq_comm] }
end
...
begin
 ...
 { rw [mul_comm _ a]; rw [ mul_assoc a]; rw [ prod_drop_succ L n hn]; rw [ mul_comm _ (drop n L).prod]; rw [ ← mul_assoc (take n L).prod]; rw [ prod_take_mul_prod_drop]; rw [ mul_comm a]; rw [ mul_assoc] },
...
begin
 ...
 { refl },
 rw [list.prod_cons]; rw [ hil (λ x hx, hl _ (mem_cons_of_mem i hx))]; rw [ hl _ (mem_cons_self i l)]; rw [ one_mul]
end
...
by rw [sum_replicate]; rw [ smul_eq_mul]
...
| (x :: xs) := by simp_rw [prod_cons, mem_cons_iff, forall_eq_or_imp, canonically_ordered_comm_semiring.mul_pos, _root_.canonically_ordered_comm_semiring.list_prod_pos]
...
by rw [← head_add_tail_sum L]; rw [ add_comm]; rw [ add_tsub_cancel_right]
...
by rw [div_eq_mul_inv]; rw [ alternating_prod_cons_cons']

variables [comm_group α]

@[to_additive] lemma alternating_prod_cons' :
 ∀ (a : α) (l : list α), alternating_prod (a :: l) = a * (alternating_prod l)⁻¹
| a [] := by rw [alternating_prod_nil]; rw [ inv_one]; rw [ mul_one]; rw [ alternating_prod_singleton]
| a (b :: l) :=
by rw [alternating_prod_cons_cons']; rw [ alternating_prod_cons' b l]; rw [ mul_inv]; rw [ inv_inv]; rw [ mul_assoc]

@[simp, to_additive] lemma alternating_prod_cons (a : α) (l : list α) :
 alternating_prod (a :: l) = a / alternating_prod l :=
by rw [div_eq_mul_inv]; rw [ alternating_prod_cons']
