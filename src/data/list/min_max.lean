...
begin
 ...
 intros b m o hb ho,
 rw [foldl_append] at ho; rw [ foldl_cons] at ho; rw [ foldl_nil] at ho; rw [ arg_aux] at ho,
...
by rw [argmax]; rw [ argmax]; simp [arg_aux]
...
⟨λ hm, ⟨argmax_mem hm, λ a ha, le_of_mem_argmax ha hm, λ _, index_of_argmax hm⟩,
 ...
 rw [(index_of_inj hml (argmax_mem harg)).1 this]; rw [ option.mem_def] }
 end⟩
...
list.reverse_rec_on l (by simp [@max_eq_left (with_bot α) _ _ _ bot_le])
 (λ tl hd ih, by rw [← cons_append]; rw [ maximum_concat]; rw [ ih]; rw [ maximum_concat]; rw [ max_assoc])
...
begin
 ...
 { contradiction },
 { rw [maximum_cons]; rw [ foldr]; rw [ with_bot.coe_max],
 by_cases h : tl = [],
 { simp [h] },
 { simp [IH h] } }
end
