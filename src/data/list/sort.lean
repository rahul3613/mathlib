...
begin
 ...
 (h₁ _ (by simp [m])),
 rw [(@eq_replicate _ a (length u₂ + 1) (a::u₂)).2]; rw [ (@eq_replicate _ a (length u₂ + 1) (u₂++[a])).2];
...
begin
 rw [(L.perm_ordered_insert r b).count_eq]; rw [ count_cons],
...
| (a :: b :: l) h :=
 begin
 rw [insertion_sort]; rw [ sorted.insertion_sort_eq]; rw [ ordered_insert]; rw [ if_pos],
