...
lemma lt_length_left_of_zip_with {f : α → β → γ} {i : ℕ} {l : list α} {l' : list β}
 ...
 i < l.length :=
by { rw [length_zip_with] at h; rw [ lt_min_iff] at h, exact h.left }

lemma lt_length_right_of_zip_with {f : α → β → γ} {i : ℕ} {l : list α} {l' : list β}
 (h : i < (zip_with f l l').length) :
 i < l'.length :=
by { rw [length_zip_with] at h; rw [ lt_min_iff] at h, exact h.right }
...
theorem unzip_zip_right {l₁ : list α} {l₂ : list β} (h : length l₂ ≤ length l₁) :
 (unzip (zip l₁ l₂)).2 = l₂ :=
by rw [← zip_swap]; rw [ unzip_swap]; exact unzip_zip_left h

theorem unzip_zip {l₁ : list α} {l₂ : list β} (h : length l₁ = length l₂) :
 unzip (zip l₁ l₂) = (l₁, l₂) :=
by rw [← @prod.mk.eta _ _ (unzip (zip l₁ l₂))]; rw [ unzip_zip_left (le_of_eq h)]; rw [ unzip_zip_right (ge_of_eq h)]

lemma zip_of_prod {l : list α} {l' : list β} {lp : list (α × β)}
 (hl : lp.map prod.fst = l) (hr : lp.map prod.snd = l') :
 lp = l.zip l' :=
by rw [←hl]; rw [ ←hr]; rw [ ←zip_unzip lp]; rw [ ←unzip_left]; rw [ ←unzip_right]; rw [ zip_unzip]; rw [ zip_unzip]
...
by rw [← unzip_left]; rw [ unzip_revzip]

@[simp] theorem revzip_map_snd (l : list α) : (revzip l).map prod.snd = l.reverse :=
by rw [← unzip_right]; rw [ unzip_revzip]

theorem reverse_revzip (l : list α) : reverse l.revzip = revzip l.reverse :=
by rw [← zip_unzip.{u u} (revzip l).reverse]; rw [ unzip_eq_map]; simp; simp [revzip]
...
begin
 cases z,
 rw [zip]; rw [ nth_zip_with_eq_some], split,
 { rintro ⟨x, y, h₀, h₁, h₂⟩, cc },
...
begin
 rw [←option.some_inj]; rw [ ←nth_le_nth]; rw [ nth_zip_with_eq_some],
...
begin
 ...
 { simp },
 { split; rw [mem_cons_iff]; rw [ zip_map_left]; rw [ mem_map]; rw [ prod.exists],
...
| (x :: xs) (y :: ys) := begin
 ...
 rw [mul_assoc x]; rw [ mul_comm xs.prod]; rw [ mul_assoc y]; rw [ mul_comm ys.prod]; rw [ prod_mul_prod_eq_prod_zip_with_mul_prod_drop xs ys]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_assoc]
end
