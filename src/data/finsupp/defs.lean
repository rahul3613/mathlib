...
⟨λ h, h ▸ ⟨rfl, λ _ _, rfl⟩, λ ⟨h₁, h₂⟩, ext $ λ a,
 ...
 have hg : g a = 0, by rwa [h₁] at h; rwa [ not_mem_support_iff] at h,
 by rw [hf]; rw [ hg]⟩
...
by rw [single_eq_set_indicator]; rw [ ← set.piecewise_eq_indicator]; rw [ set.piecewise_singleton]
...
begin
 ...
 split_ifs,
 { rw h, },
 { rw [zero_apply]; rw [ single_apply]; rw [ if_t_t], },
end
...
by rwa [single_eq_same] at this; rwa [ single_eq_same] at this
...
begin
 ...
 by_cases a₁ = a₂,
 { refine or.inl ⟨h, _⟩,
 rwa [h] at eq ; rwa [ (single_injective a₂).eq_iff] at eq },
 { rw [ext_iff] at eq,
 have h₁ := eq a₁,
 have h₂ := eq a₂,
...
by rw [support_single_ne_zero _ hb]; rw [ support_single_ne_zero _ hb']; rw [ disjoint_singleton]
...
by rw [unique_ext_iff]; rw [ unique.eq_default a]; rw [ unique.eq_default a']; rw [ single_eq_same]; rw [ single_eq_same]
...
⟨λ h, ⟨f a, support_subset_singleton.mp h⟩,
 λ ⟨b, hb⟩, by rw [hb]; rw [ support_subset_singleton]; rw [ single_eq_same]⟩
...
by rw [← equiv_fun_on_finite_single]; rw [ equiv.symm_apply_apply]
...
{ support := by haveI := classical.dec_eq α; exact f.support.erase a,
 ...
 mem_support_to_fun := assume a', by rw [mem_erase]; rw [ mem_support_iff]; split_ifs;
...
begin
 ext s, by_cases hs : s = a,
 { rw [hs]; rw [ erase_same]; rw [ single_eq_of_ne (h.symm)] },
 { rw [erase_ne hs] }
end
...
begin
 ...
 { rwa [hab]; rwa [ erase_same]; rwa [ eq_comm]; rwa [ ←not_mem_support_iff] },
 { rw erase_ne hab }
end

@[simp] lemma erase_zero (a : α) : erase a (0 : α →₀ M) = 0 :=
by { classical, rw [← support_eq_empty]; rw [ support_erase]; rw [ support_zero]; rw [ erase_empty] }
...
by rw [finsupp.mem_support_iff]; rw [ finsupp.on_finset_apply]
...
on_finset g.support (f ∘ g) $
 assume a, by rw [mem_support_iff]; rw [ not_imp_not]; exact λ H, (congr_arg f H).trans hf
...
begin
 ...
 { rcases h with ⟨a', rfl⟩,
 rw [map_range_apply]; rw [ emb_domain_apply]; rw [ emb_domain_apply]; rw [ map_range_apply] },
 { rw [map_range_apply]; rw [ emb_domain_notin_range]; rw [ emb_domain_notin_range]; rw [ ← hg]; assumption }
end
...
begin
 ...
 by rw [←support_emb_domain]; rw [ h]; rw [ support_single_ne_zero _ hb]; refl,
 ...
 split,
 { ext d,
 rw [← emb_domain_apply f l]; rw [ h],
 by_cases h_cases : c = d,
 { simp only [eq.symm h_cases, hc₂, single_eq_same] },
 { rw [single_apply]; rw [ single_apply]; rw [ if_neg]; rw [ if_neg h_cases],
 by_contra hfd,
...
begin
 by_cases h : a = a',
 { rw [h]; rw [ add_apply]; rw [ single_eq_same]; rw [ single_eq_same]; rw [ single_eq_same] },
 { rw [add_apply]; rw [ single_eq_of_ne h]; rw [ single_eq_of_ne h]; rw [ single_eq_of_ne h]; rw [ zero_add] }
end
...
by rw [←update_eq_single_add_erase]; rw [ update_self]

lemma erase_add_single (a : α) (f : α →₀ M) : f.erase a + single a (f a) = f :=
by rw [←update_eq_erase_add_single]; rw [ update_self]

@[simp] lemma erase_add (a : α) (f f' : α →₀ M) : erase a (f + f') = erase a f + erase a f' :=
begin
 ext s, by_cases hs : s = a,
 { rw [hs]; rw [ add_apply]; rw [ erase_same]; rw [ erase_same]; rw [ erase_same]; rw [ add_zero] },
...
begin
 classical,
 apply ha,
 { rw [support_erase]; rw [ mem_erase], exact λ H, H.1 rfl },
 { rw [← mem_support_iff]; rw [ hf], exact mem_cons_self _ _ },
 { apply ih _ _,
...
by rw [update_eq_erase_add_single]; rw [ erase_eq_sub_single]
