...
⟨f.graph.to_list.map prod.to_sigma, begin
 rw [list.nodupkeys]; rw [ list.keys]; rw [ list.map_map]; rw [ prod.fst_comp_to_sigma]; rw [ list.nodup_map_iff_inj_on],
 { rintros ⟨b, m⟩ hb ⟨c, n⟩ hc (rfl : b = c),
 rw [finset.mem_to_list] at hb hc; rw [ finsupp.mem_graph_iff] at hb hc,
 dsimp at hb hc,
 rw [←hc.1]; rw [ hb.1] },
...
begin
 classical,
 rw [alist.mem_keys]; rw [ ←list.mem_to_finset]; rw [ to_alist_keys_to_finset]; rw [ mem_support_iff]
end
...
by { rw [lookup_finsupp_apply]; rw [ ←lookup_eq_none], cases lookup a l with m; simp }
