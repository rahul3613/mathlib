...
begin
 intros f g h,
 classical,
 have hsup : f.support = g.support, by rw [← image_fst_graph]; rw [ h]; rw [ image_fst_graph],
...
lemma map_range.equiv_trans
 ...
 (map_range.equiv (f.trans f₂) (by rw [equiv.trans_apply]; rw [ hf]; rw [ hf₂])
 (by rw [equiv.symm_trans_apply]; rw [ hf₂']; rw [ hf']) : (α →₀ _) ≃ _) =
...
begin
 rw [map_domain]; rw [ sum_apply]; rw [ sum]; rw [ finset.sum_eq_single a]; rw [ single_eq_same],
 { assume b _ hba, exact single_eq_of_ne (hf.ne hba) },
 { assume h, rw [not_mem_support_iff.1 h]; rw [ single_zero]; rw [ zero_apply] }
end

lemma map_domain_notin_range {f : α → β} (x : α →₀ M) (a : β) (h : a ∉ set.range f) :
 map_domain f x a = 0 :=
begin
 rw [map_domain]; rw [ sum_apply]; rw [ sum],
...
begin
 ...
 { rw [← finset.add_sum_erase _ _ hax]; rw [ if_pos rfl],
 convert add_zero _,
...
begin
 ...
 { rcases h with ⟨a, rfl⟩,
 rw [map_domain_apply f.injective]; rw [ emb_domain_apply] },
 { rw [map_domain_notin_range]; rw [ emb_domain_notin_range]; assumption }
end
...
by rw [←function.embedding.coe_fn_mk f hf]; rw [ ←emb_domain_eq_map_domain]; rw [ prod_emb_domain]
...
begin
 ...
 rwa [map_domain_apply hf] at this; rwa [ map_domain_apply hf] at this,
end
...
begin
 ...
 { rw [← map_domain_apply' S _ hv₁ hf _]; rw [ ← map_domain_apply' S _ hv₂ hf _]; rw [ eq];
...
begin
 rw [← support_eq_empty]; rw [ ← support_eq_empty]; rw [ comap_domain],
...
begin
 ...
 { rw [eq_single_iff]; rw [ comap_domain_apply]; rw [ comap_domain_support]; rw [ ← finset.coe_subset]; rw [ coe_preimage]; rw [ support_single_ne_zero _ hm]; rw [ coe_singleton]; rw [ coe_singleton]; rw [ single_eq_same],
...
begin
 ...
 rw [hb.symm]; rw [ map_domain_apply hf]; rw [ comap_domain_apply] },
...
by { classical, rw [← support_eq_empty]; rw [ support_filter]; rw [ support_zero]; rw [ finset.filter_empty] }
...
begin
 ...
 rw [finsupp.curry]; rw [ sum_apply]; rw [ sum_apply]; rw [ finsupp.sum]; rw [ finset.sum_eq_single]; rw [ this]; rw [ if_pos rfl],
 { intros b hb b_ne, rw [this b]; rw [ if_neg b_ne] },
 { intros hxy, rw [this (x, y)]; rw [ if_pos rfl]; rw [ not_mem_support_iff.mp hxy] }
end
...
begin
 ...
 dsimp only,
 split_ifs,
 { rw [filter_apply_pos]; rw [ filter_single_of_pos]; exact h },
...
{ one_smul := λ f, by rw [comap_smul_def]; rw [ one_smul_eq_id]; rw [ map_domain_id],
 mul_smul := λ g g' f, by rw [comap_smul_def]; rw [ comap_smul_def]; rw [ comap_smul_def]; rw [ ←comp_smul_left]; rw [ map_domain_comp], }
...
begin
 ...
 rw [function.comp_apply]; rw [ smul_zero]; rw [ hf],
end

lemma smul_single_one [semiring R] (a : α) (b : R) : b • single a 1 = single a b :=
by rw [smul_single]; rw [ smul_eq_mul]; rw [ mul_one]
...
{ to_fun := λ f, subtype_domain (λ x, x ∈ s) f.1,
 ...
 rw [finset.coe_image]; rw [ set.image_subset_iff],
 ...
 { rcases h with ⟨x, rfl⟩,
 rw [map_domain_apply subtype.val_injective]; rw [ subtype_domain_apply] },
 ...
 ext ⟨a, ha⟩,
 dsimp only,
 rw [subtype_domain_apply]; rw [ map_domain_apply subtype.val_injective]
 end }
...
begin
 rw [split_support]; rw [ mem_image]; rw [ ne.def]; rw [ ← support_eq_empty]; rw [ ← ne.def]; rw [ ← finset.nonempty_iff_ne_empty]; rw [ split]; rw [ comap_domain]; rw [ finset.nonempty],
...
{ support := split_support l,
 ...
 begin
 intros i,
 rw [mem_split_support_iff_nonzero]; rw [ not_iff_not]; rw [ hg],
 end }
