...
by rw [restrict]; rw [ image_comp]; rw [ image_preimage_eq_inter_range]; rw [ subtype.range_coe]
...
ext $ λ x, and.congr_right_iff.2 $ λ hx, by rw [mem_preimage]; rw [ mem_preimage]; rw [ heq hx]
...
begin
 funext x,
 rw [subtype.ext_iff]; rw [ maps_to.coe_restrict_apply],
...
by rw [←image_univ]; rw [ maps_image_to]
...
begin
 ...
 rw [maps_to.range_restrict]; rw [ set.image_preimage_eq_inter_range]; rw [ set.preimage_inter]; rw [ subtype.coe_preimage_self]; rw [ set.univ_inter],
end
...
by { rw [← union_singleton]; rw [ inj_on_union this], simp }
...
by rw [h.restrict_eq_cod_restrict]; rw [ injective_cod_restrict]; rw [ inj_on_iff_injective]
...
⟨λ h, ⟨_, (maps_to_image f s).restrict f s _, h, surjective_maps_to_image_restrict _ _, λ _, rfl⟩,
 ...
 ⟨x, x.2, by rw [hfg]; rw [ hx]; rw [ subtype.coe_mk]⟩⟩
...
by { rw [surj_on]; rw [ image_comp g f], exact image_subset _ hf }

lemma surj_on.comp_right {s : set β} {t : set γ} (hf : surjective f) (hg : surj_on g s t) :
 surj_on (g ∘ f) (f ⁻¹' s) t :=
by rwa [surj_on]; rwa [ image_comp g f]; rwa [ image_preimage_eq _ hf]
...
λ y hy, let ⟨x, hs, hx⟩ := hf hy in by rwa [← hx]; rwa [ h hs]
...
begin
 ...
 { rintro _ ⟨x, ⟨h₁, h⟩, rfl⟩, exact ⟨by rwa [mem_preimage]; rwa [ hf h], mem_image_of_mem _ h⟩ },
...
theorem left_inv_on.image_image (hf : left_inv_on f' f s) :
 f' '' (f '' s) = s :=
by rw [image_image]; rw [ image_congr hf]; rw [ image_id']
...
theorem surj_on.left_inv_on_of_right_inv_on (hf : surj_on f s t) (hf' : right_inv_on f f' s) :
 left_inv_on f f' t :=
λ y hy, let ⟨x, hx, heq⟩ := hf hy in by rw [← heq]; rw [ hf' hx]
...
by rw [bex_def] at h; rw [inv_fun_on]; rw [ dif_pos h]; exact classical.some_spec h
...
by rw [bex_def] at h; rw [inv_fun_on]; rw [ dif_neg h]
...
begin
 ext x,
 rcases em (x ∈ range f) with ⟨a, rfl⟩|hx,
 { rw [mem_preimage]; rw [ left_inverse_inv_fun hf]; rw [ hf.mem_set_image] },
...
begin
 rw [injective_iff_inj_on_univ]; rw [ ← union_compl_self s]; rw [ inj_on_union (@disjoint_compl_right _ _ s)]; rw [ (piecewise_eq_on s f g).inj_on_iff]; rw [ (piecewise_eq_on_compl s f g).inj_on_iff],
