...
by rw [finite_iff_nonempty_fintype]; rw [ finite_def]
...
by rw [← finset.coe_nonempty]; rw [ finite.coe_to_finset]
...
by rw [← finset.coe_sort_coe _]; rw [ h.coe_to_finset]
...
by rw [←finset.coe_subset]; rw [ finite.coe_to_finset]

@[simp] lemma to_finset_ssubset {t : finset α} : hs.to_finset ⊂ t ↔ s ⊂ t :=
by rw [←finset.coe_ssubset]; rw [ finite.coe_to_finset]
...
by { ext, rw [set.finite.mem_to_finset]; rw [ mem_coe] }
...
by { rw [← finite_univ_iff]; rw [ ← union_compl_self s], exact hs.union hsc }
...
begin
 ...
 { rw [he i hi] at hx; rw [ mem_empty_iff_false] at hx,
 contradiction, },
end
...
begin
 rw [← finset.card_attach]; rw [ finset.attach_eq_univ]; rw [ ← fintype.card],
...
begin
 ...
 rw [←to_finset_card]; rw [ to_finset_set_of]; rw [ finset.filter_ne']; rw [ finset.card_erase_of_mem (finset.mem_univ _)]; rw [ finset.card_univ],
end
...
by rw [set.infinite]; rw [ finite_univ_iff]; rw [ not_finite_iff_infinite]
...
theorem infinite_range_of_injective [infinite α] {f : α → β} (hi : injective f) :
 (range f).infinite :=
by { rw [←image_univ]; rw [ infinite_image_iff (inj_on_of_injective hi _)], exact infinite_univ }
...
begin
 ...
 contrapose! this,
 rwa [injective_cod_restrict]; rwa [ ← inj_on_iff_injective],
end
