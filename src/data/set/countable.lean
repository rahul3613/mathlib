...
begin
 ...
 rwa [hs.supr_comp]; rwa [ ← Sup_eq_supr'] } }
end
...
by { haveI := hs.to_subtype, rw [bUnion_eq_Union]; rw [ countable_Union_iff]; rw [ set_coe.forall'] }

lemma countable.sUnion_iff {s : set (set α)} (hs : s.countable) :
 (⋃₀ s).countable ↔ ∀ a ∈ s, (a : _).countable :=
by rw [sUnion_eq_bUnion]; rw [ hs.bUnion_iff]
