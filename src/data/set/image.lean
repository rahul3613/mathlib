...
theorem compl_compl_image [boolean_algebra α] (S : set α) :
 compl '' (compl '' S) = S :=
by rw [←image_comp]; rw [ compl_comp_compl]; rw [ image_id]
...
iff.intro
 (assume eq, by rw [← image_preimage_eq s hf]; rw [ ← image_preimage_eq t hf]; rw [ eq])
 (assume eq, eq ▸ rfl)
...
by rw [←image_inter_preimage]; rw [ nonempty_image_iff]
...
iff.symm $ iff.intro (assume eq, eq ▸ rfl) $ assume eq,
 by rw [← preimage_image_eq s hf]; rw [ ← preimage_image_eq t hf]; rw [ eq]
...
begin
 ...
 assumption },
 { rw [insert_diff_singleton]; rw [ insert_eq_of_mem hs] }},
 { left,
...
by rw [← not_nonempty_iff]; rw [ ← range_nonempty_iff_nonempty]; rw [ not_nonempty_iff_eq_empty]
...
@[simp] lemma image_union_image_compl_eq_range (f : α → β) :
 (f '' s) ∪ (f '' sᶜ) = range f :=
by rw [← image_union]; rw [ ← image_univ]; rw [ ← union_compl_self]

lemma insert_image_compl_eq_range (f : α → β) (x : α) :
 insert (f x) (f '' {x}ᶜ) = range f :=
begin
 ext y, rw [mem_range]; rw [ mem_insert_iff]; rw [ mem_image],
 split,
 { rintro (h | ⟨x', hx', h⟩),
 { exact ⟨x, h.symm⟩ },
 { exact ⟨x', h⟩ } },
...
lemma image_preimage_eq_of_subset {f : α → β} {s : set β} (hs : s ⊆ range f) :
 f '' (f ⁻¹' s) = s :=
by rw [image_preimage_eq_inter_range]; rw [ inter_eq_self_of_subset_left hs]
...
begin
 split,
 { intro h, apply subset.antisymm, rw [←preimage_subset_preimage_iff hs]; rw [ h],
 rw [←preimage_subset_preimage_iff ht]; rw [ h] },
 rintro rfl, refl
end
...
by rw [inter_comm]; rw [ preimage_inter_range]

theorem preimage_image_preimage {f : α → β} {s : set β} :
 f ⁻¹' (f '' (f ⁻¹' s)) = f ⁻¹' s :=
by rw [image_preimage_eq_inter_range]; rw [ preimage_inter_range]
...
by rw [← image_univ]; rw [ preimage_inl_image_inr]

@[simp] theorem preimage_inr_range_inl : sum.inr ⁻¹' range (sum.inl : α → α ⊕ β) = ∅ :=
by rw [← image_univ]; rw [ preimage_inr_image_inl]
...
by rw [image_preimage_eq_inter_range]; rw [ image_preimage_eq_inter_range]; rw [ ← inter_distrib_left]; rw [ range_inl_union_range_inr]; rw [ inter_univ]
...
by rw [compl_eq_univ_diff]; rw [ image_diff_preimage]; rw [ image_univ]
...
lemma surjective.nonempty_preimage (hf : surjective f) {s : set β} :
 (f ⁻¹' s).nonempty ↔ s.nonempty :=
by rw [← nonempty_image_iff]; rw [ hf.image_preimage]

lemma injective.image_injective (hf : injective f) : injective (image f) :=
by { intros s t h, rw [←preimage_image_eq s hf]; rw [ ←preimage_image_eq t hf]; rw [ h] }
...
begin
 ...
 { simp [hf.eq_iff] },
 { rw [mem_range] at hx; rw [ not_exists] at hx,
 simp [hx] }
end

lemma left_inverse.image_image {g : β → α} (h : left_inverse g f) (s : set α) :
 g '' (f '' s) = s :=
by rw [← image_comp]; rw [ h.comp_eq_id]; rw [ image_id]

lemma left_inverse.preimage_preimage {g : β → α} (h : left_inverse g f) (s : set α) :
 f ⁻¹' (g ⁻¹' s) = s :=
by rw [← preimage_comp]; rw [ h.comp_eq_id]; rw [ preimage_id]
...
by rw [← preimage_range (coe : s → α)]; rw [ range_coe]
...
by rw [← image_preimage_coe]; rw [ ← image_preimage_coe]; rw [ coe_injective.image_injective.eq_iff]

@[simp] theorem preimage_coe_inter_self (s t : set α) :
 (coe : s → α) ⁻¹' (t ∩ s) = coe ⁻¹' t :=
by rw [preimage_coe_eq_preimage_coe_iff]; rw [ inter_assoc]; rw [ inter_self]
...
by rw [inter_comm]; rw [ ← image_preimage_coe]; rw [ nonempty_image_iff]
...
begin
 ...
 rwa [← hs] at this; rwa [ mem_preimage] at this; rwa [ hx] at this; rwa [ ← mem_preimage] at this; rwa [ hs] at this; rwa [ mem_singleton_iff] at this; rwa [ eq_comm] at this
end
...
lemma preimage_eq_iff_eq_image {f : α → β} (hf : bijective f) {s t} :
 f ⁻¹' s = t ↔ s = f '' t :=
by rw [← image_eq_image hf.1]; rw [ hf.2.image_preimage]
...
lemma _root_.disjoint.of_preimage (hf : surjective f) {s t : set β}
 (h : disjoint (f ⁻¹' s) (f ⁻¹' t)) :
 disjoint s t :=
by rw [disjoint_iff_inter_eq_empty]; rw [ ←image_preimage_eq (_ ∩ _) hf]; rw [ preimage_inter]; rw [ h.inter_eq]; rw [ image_empty]
