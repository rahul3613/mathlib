...
theorem diff_Union [nonempty ι] (s : set β) (t : ι → set β) :
 s \ (⋃ i, t i) = ⋂ i, s \ t i :=
by rw [diff_eq]; rw [ compl_Union]; rw [ inter_Inter]; refl

theorem diff_Inter (s : set β) (t : ι → set β) :
 s \ (⋂ i, t i) = ⋃ i, s \ t i :=
by rw [diff_eq]; rw [ compl_Inter]; rw [ inter_Union]; refl
...
by rw [Union_nonempty_index]; rw [ bUnion_self]
...
by rw [bInter_insert]; rw [ bInter_singleton]
...
lemma inter_bInter {ι α : Type*} {s : set ι} (hs : s.nonempty) (f : ι → set α) (t : set α) :
 (⋂ i ∈ s, t ∩ f i) = t ∩ ⋂ i ∈ s, f i :=
begin
 rw [inter_comm]; rw [ ← bInter_inter hs],
 simp [inter_comm]
end
...
theorem sUnion_eq_compl_sInter_compl (S : set (set α)) :
 ⋃₀ S = (⋂₀ (compl '' S))ᶜ :=
by rw [←compl_compl (⋃₀ S)]; rw [ compl_sUnion]

-- classical
theorem compl_sInter (S : set (set α)) :
 (⋂₀ S)ᶜ = ⋃₀ (compl '' S) :=
by rw [sUnion_eq_compl_sInter_compl]; rw [ compl_compl_image]

-- classical
theorem sInter_eq_compl_sUnion_compl (S : set (set α)) :
 ⋂₀ S = (⋃₀ (compl '' S))ᶜ :=
by rw [←compl_compl (⋂₀ S)]; rw [ compl_sInter]
...
by rw [← sUnion_image]; rw [ image_id']

lemma sInter_eq_bInter {s : set (set α)} : (⋂₀ s) = (⋂ (i : set α) (h : i ∈ s), i) :=
by rw [← sInter_image]; rw [ image_id']
...
begin
 ext x, rw [mem_Union]; rw [ mem_Union], split,
...
by simp_rw [bijective, forall_and_distrib, injective_iff_injective_of_Union_eq_univ hU, surjective_iff_surjective_of_Union_eq_univ hU]
end
...
begin
 intros y hy,
 rw [Hinj.image_Inter_eq]; rw [ mem_Inter],
 exact λ i, H i hy
end
...
@[simp] theorem preimage_sUnion {f : α → β} {s : set (set β)} :
 f ⁻¹' (⋃₀ s) = (⋃ t ∈ s, f ⁻¹' t) :=
by rw [sUnion_eq_bUnion]; rw [ preimage_Union₂]
...
by rw [sInter_eq_bInter]; rw [ preimage_Inter₂]

@[simp] lemma bUnion_preimage_singleton (f : α → β) (s : set β) : (⋃ y ∈ s, f ⁻¹' {y}) = f ⁻¹' s :=
by rw [← preimage_Union₂]; rw [ bUnion_of_singleton]

lemma bUnion_range_preimage_singleton (f : α → β) : (⋃ y ∈ range f, f ⁻¹' {y}) = univ :=
by rw [bUnion_preimage_singleton]; rw [ preimage_range]
...
by rw [Union_image_left]; rw [ image2_mk_eq_prod]

lemma prod_eq_bUnion_right : s ×ˢ t = ⋃ b ∈ t, (λ a, (a, b)) '' s :=
by rw [Union_image_right]; rw [ image2_mk_eq_prod]
...
lemma image_seq {f : β → γ} {s : set (α → β)} {t : set α} :
 f '' seq s t = seq ((∘) f '' s) t :=
by rw [← singleton_seq]; rw [ ← singleton_seq]; rw [ seq_seq]; rw [ image_singleton]
...
by rw [← prod_eq_seq]; rw [ ← image_swap_prod]; rw [ prod_eq_seq]; rw [ image_seq]; rw [ ← image_comp]; rw [ prod.swap]
...
lemma Ici_Sup (s : set α) : Ici (Sup s) = ⋂ a ∈ s, Ici a := by rw [Sup_eq_supr]; rw [ Ici_supr₂]
lemma Iic_Inf (s : set α) : Iic (Inf s) = ⋂ a ∈ s, Iic a := by rw [Inf_eq_infi]; rw [ Iic_infi₂]
