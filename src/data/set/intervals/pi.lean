...
begin
 ...
 by rw [← Ioi_inter_Iic]; rw [ ← Ioi_inter_Iic]; rw [ ← inter_assoc]; rw [ inter_eq_self_of_subset_left (Ioi_subset_Ioi hm)],
...
begin
 ...
 by rw [← Ioi_inter_Iic]; rw [ ← Ioi_inter_Iic]; rw [ inter_left_comm]; rw [ inter_eq_self_of_subset_left (Iic_subset_Iic.2 hm)],
...
by rw [←Icc_diff_right]; rw [ ←Icc_diff_right]; rw [ image_diff (f.update_injective _)]; rw [ image_singleton]; rw [ image_update_Icc]

lemma image_update_Ioc (f : Π i, α i) (i : ι) (a b : α i) :
 f.update i '' Ioc a b = Ioc (f.update i a) (f.update i b) :=
by rw [←Icc_diff_left]; rw [ ←Icc_diff_left]; rw [ image_diff (f.update_injective _)]; rw [ image_singleton]; rw [ image_update_Icc]

lemma image_update_Ioo (f : Π i, α i) (i : ι) (a b : α i) :
 f.update i '' Ioo a b = Ioo (f.update i a) (f.update i b) :=
by rw [←Ico_diff_left]; rw [ ←Ico_diff_left]; rw [ image_diff (f.update_injective _)]; rw [ image_singleton]; rw [ image_update_Ico]
...
by simp_rw [pi_univ_Ioc_update_left hm.1, pi_univ_Ioc_update_right hm.2, ← union_inter_distrib_right, ← set_of_or, le_or_lt, set_of_true, univ_inter]
