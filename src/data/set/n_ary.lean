...
by rw [←image2_swap]; rw [ image2_union_left]; rw [ image2_swap f]; rw [ image2_swap f]
...
by rw [insert_eq]; rw [ image2_union_left]; rw [ image2_singleton_left]

@[simp] lemma image2_insert_right : image2 f s (insert b t) = (λ a, f a b) '' s ∪ image2 f s t :=
by rw [insert_eq]; rw [ image2_union_right]; rw [ image2_singleton_right]
...
by { rw [image2_swap f']; rw [ image2_swap f], exact image2_assoc (λ _ _ _, h_left_comm _ _ _) }

lemma image2_right_comm {f : δ → γ → ε} {g : α → β → δ} {f' : α → γ → δ'} {g' : δ' → β → ε}
 (h_right_comm : ∀ a b c, f (g a b) c = g' (f' a c) b) :
 image2 f (image2 g s t) u = image2 g' (image2 f' s u) t :=
by { rw [image2_swap g]; rw [ image2_swap g'], exact image2_assoc (λ _ _ _, h_right_comm _ _ _) }
...
lemma image2_left_identity {f : α → β → β} {a : α} (h : ∀ b, f a b = b) (t : set β) :
 image2 f {a} t = t :=
by rw [image2_singleton_left]; rw [ show f a = id]; rw [ from funext h]; rw [ image_id]
...
lemma image2_right_identity {f : α → β → α} {b : β} (h : ∀ a, f a b = a) (s : set α) :
 image2 f s {b} = s :=
by rw [image2_singleton_right]; rw [ funext h]; rw [ image_id']
