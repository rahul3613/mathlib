...
by rw [ncard_def]; rw [ @nat.card_eq_fintype_card _ hs.fintype]; rw [ @finite.card_to_finset _ _ hs.fintype hs]
...
by rw [ncard_eq_to_finset_card]; rw [ finset.finite_to_set_to_finset]
...
begin
 ...
 rw [ncard_eq_to_finset_card]; rw [ finite.to_finset_univ]; rw [ finset.card_univ]; rw [ nat.card_eq_fintype_card]},
 rw [(@infinite_univ _ h).ncard]; rw [ @nat.card_eq_zero_of_infinite _ h],
end
...
by rw [pos_iff_ne_zero]; rw [ ne.def]; rw [ ncard_eq_zero hs]; rw [ nonempty_iff_ne_empty]
...
begin
 classical,
 rw [←inter_self {a}]; rw [ inter_assoc]; rw [ ncard_eq_to_finset_card]; rw [ finite.to_finset_inter]; rw [ finite.to_finset_singleton],
...
begin
 classical,
 haveI := hs.fintype,
 rw [ncard_eq_to_finset_card]; rw [ ncard_eq_to_finset_card]; rw [ finite.to_finset_insert]; rw [ finset.card_insert_of_not_mem],
...
begin
 by_cases h : a ∈ s,
 { rw [ncard_insert_of_mem h]; rw [ if_pos h] },
 { rw [ncard_insert_of_not_mem h hs]; rw [ if_neg h] }
end
...
by {rw [ncard_insert_of_not_mem]; rw [ ncard_singleton], simpa}
...
begin
 ...
 { haveI := h.to_subtype,
 rw [ncard_insert_of_not_mem]; rw [ ncard_diff_singleton_add_one hb],
 simpa only [mem_diff, not_and] using ha},
 rw [((h.diff (set.to_finite {b})).mono (subset_insert _ _)).ncard]; rw [ h.ncard],
end

lemma ncard_exchange' (ha : a ∉ s) (hb : b ∈ s) : ((insert a s) \ {b}).ncard = s.ncard :=
by rw [←ncard_exchange ha hb]; rw [ ←singleton_union]; rw [ ←singleton_union]; rw [ union_diff_distrib]; rw [ @diff_singleton_eq_self _ b {a} (λ h, ha (by rwa ← mem_singleton_iff.mp h) )]
...
begin
 ...
 rw [ncard_eq_to_finset_card]; rw [ finite.to_finset_image _ hs],
 { congr', rw [←finset.coe_inj]; rw [ finite.coe_to_finset]; rw [ coe_to_finset]},
 { apply_instance},
 rw [←finset.coe_inj]; rw [ finite.coe_to_finset]; rw [ coe_to_finset],
end
...
begin
 ...
 rw [h.ncard]; rw [ ((infinite_image_iff H).mpr h).ncard],
end
...
by rw [←ncard_image_of_injective _ H]; rw [ image_preimage_eq_iff.mpr hs]
...
begin
 ...
 { simpa, },
 rw [ncard_eq_to_finset_card _ ht] at h'; rw [ ncard_eq_to_finset_card _ (ht.subset h)] at h',
 convert h',
end
...
begin
 rw [ncard_eq_to_finset_card _ (ht.subset h.subset)]; rw [ ncard_eq_to_finset_card t ht],
...
begin
 ...
 rw [ncard_def]; rw [ nat.card_eq_fintype_card],
 rw [ncard_def]; rw [ nat.card_eq_fintype_card],
end
...
begin
 cases s.finite_or_infinite,
 { haveI := h.to_subtype,
 rw [ncard_eq_to_finset_card _ ht]; rw [ ncard_eq_to_finset_card _ (to_finite s)],
...
begin
 ...
 rw [ncard_eq_to_finset_card _ hs]; rw [ ncard_eq_to_finset_card _ ht]; rw [ ncard_eq_to_finset_card _ hu]; rw [ ncard_eq_to_finset_card _ hi]; rw [ finite.to_finset_union]; rw [ finite.to_finset_inter],
...
lemma ncard_inter_add_ncard_union (s t : set α) (hs : s.finite . to_finite_tac)
 (ht : t.finite . to_finite_tac) :
 (s ∩ t).ncard + (s ∪ t).ncard = s.ncard + t.ncard :=
by rw [add_comm]; rw [ ncard_union_add_ncard_inter _ _ hs ht]
...
begin
 ...
 rw [ncard_eq_to_finset_card _ hs]; rw [ ncard_eq_to_finset_card _ ht]; rw [ ncard_eq_to_finset_card _ h]; rw [ finite.to_finset_union],
...
begin
 classical,
 rw [ncard_eq_to_finset_card _ hs]; rw [ ncard_eq_to_finset_card _ ht]; rw [ ncard_eq_to_finset_card _ (hs.union ht)]; rw [finite.to_finset_union],
...
begin
 classical,
 rw [ncard_eq_to_finset_card _ ht]; rw [ ncard_eq_to_finset_card _ (ht.subset h)]; rw [ ncard_eq_to_finset_card _ (ht.diff s)]; rw [ finite.to_finset_diff],
...
by rw [←ncard_diff_add_ncard_eq_ncard h ht]; rw [ add_tsub_cancel_right]

lemma ncard_le_ncard_diff_add_ncard (s t : set α) (ht : t.finite . to_finite_tac) :
 s.ncard ≤ (s \ t).ncard + t.ncard :=
begin
 cases s.finite_or_infinite,
 { rw [←diff_inter_self_eq_diff]; rw [ ←ncard_diff_add_ncard_eq_ncard (inter_subset_right t s) h]; rw [ add_le_add_iff_left],
...
by rw [←union_diff_right]; rw [ncard_diff_add_ncard_eq_ncard (subset_union_right s t) (hs.union ht)]
...
by rw [←ncard_diff_add_ncard_eq_ncard (diff_subset s t) hs]; rw [ sdiff_sdiff_right_self]; rw [ inf_eq_inter]

lemma ncard_eq_ncard_iff_ncard_diff_eq_ncard_diff (hs : s.finite . to_finite_tac)
 (ht : t.finite . to_finite_tac) :
 s.ncard = t.ncard ↔ (s \ t).ncard = (t \ s).ncard :=
by rw [←ncard_inter_add_ncard_diff_eq_ncard s t hs]; rw [ ←ncard_inter_add_ncard_diff_eq_ncard t s ht]; rw [ inter_comm]; rw [ add_right_inj]

lemma ncard_le_ncard_iff_ncard_diff_le_ncard_diff (hs : s.finite . to_finite_tac)
 (ht : t.finite . to_finite_tac) :
 s.ncard ≤ t.ncard ↔ (s \ t).ncard ≤ (t \ s).ncard :=
by rw [←ncard_inter_add_ncard_diff_eq_ncard s t hs]; rw [ ←ncard_inter_add_ncard_diff_eq_ncard t s ht]; rw [ inter_comm]; rw [ add_le_add_iff_left]

lemma ncard_lt_ncard_iff_ncard_diff_lt_ncard_diff (hs : s.finite . to_finite_tac)
 (ht : t.finite . to_finite_tac) :
 s.ncard < t.ncard ↔ (s \ t).ncard < (t \ s).ncard :=
by rw [←ncard_inter_add_ncard_diff_eq_ncard s t hs]; rw [ ←ncard_inter_add_ncard_diff_eq_ncard t s ht]; rw [ inter_comm]; rw [ add_lt_add_iff_left]

lemma ncard_add_ncard_compl (s : set α) (hs : s.finite . to_finite_tac)
 (hsc : sᶜ.finite . to_finite_tac) :
 s.ncard + sᶜ.ncard = nat.card α :=
by rw [←ncard_univ]; rw [ ←ncard_union_eq (@disjoint_compl_right _ _ s) hs hsc]; rw [ union_compl_self]
...
begin
 ...
 exact ⟨r', by simpa using hsr', by simpa using hr't, by rw [←hr']; rw [ ncard_coe_finset]⟩},
 rw [ht.ncard] at h₁,
 have h₁' := nat.eq_zero_of_le_zero h₁,
 rw [add_eq_zero_iff] at h₁',
 exact ⟨t, h₂, rfl.subset, by rw [ht.ncard]; rw [ h₁'.1]; rw [ h₁'.2]⟩
end
...
begin
 ...
 rwa [ncard_union_eq (disjoint_of_subset_right hs₁ disjoint_sdiff_right) hs hs₁fin]; rwa [ hs₁card]; rwa [ add_tsub_cancel_of_le],
end
...
begin
 ...
 rw [ncard_eq_to_finset_card _ hu] at hst; rw [ finite.to_finset_union (hu.subset (subset_union_left _ _)) (hu.subset (subset_union_right _ _))] at hst,
...
begin
 ...
 rw [ncard_eq_to_finset_card] at h; rw [ finset.card_eq_one] at h,
 exact h.imp (λ a ha, by rwa [←finite.to_finset_singleton] at ha); rwa [ finite.to_finset_inj] at ha),
end
...
begin
 ...
 split,
 { rintro ⟨a, ha, rfl⟩,
 rw [ncard_eq_to_finset_card _ hs]; rw [ ncard_eq_to_finset_card _ (hs.insert a)]; rw [ finite.to_finset_insert]; rw [ ←@finite.to_finset_subset_to_finset _ _ _ hs (hs.insert a)]; rw [ finite.to_finset_insert],
 ...
 rw [ncard_eq_to_finset_card _ hs] at h; rw [ ncard_eq_to_finset_card _ ht] at h,
...
lemma ncard_le_one_iff_subset_singleton [nonempty α] (hs : s.finite . to_finite_tac) :
 s.ncard ≤ 1 ↔ ∃ (x : α), s ⊆ {x} :=
by simp_rw [ncard_eq_to_finset_card _ hs, finset.card_le_one_iff_subset_singleton, finite.to_finset_subset, finset.coe_singleton]
...
begin
 ...
 rw [ncard_eq_to_finset_card] at h; rw [ finset.card_eq_succ] at h,
...
begin
 ...
 exact ⟨_,_,hxt,rfl⟩},
 rintro ⟨x,y,hxy,rfl⟩,
 rw [ncard_eq_to_finset_card]; rw [ finset.card_eq_two],
...
begin
 ...
 rw [mem_insert_iff] at hxt; rw [ mem_singleton_iff] at hxt; rw [ not_or_distrib] at hxt,
 exact ⟨x,y,z,hxt.1,hxt.2,hyz,rfl⟩},
 rintro ⟨x, y, z, xy, xz, yz, rfl⟩,
 rw [ncard_insert_of_not_mem]; rw [ ncard_insert_of_not_mem]; rw [ ncard_singleton],
