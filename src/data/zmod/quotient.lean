...
(of_bijective (map _ (stabilizer (zmultiples a) b)
 ...
 { rw [zmultiples_le]; rw [ mem_comap]; rw [ mem_stabilizer_iff]; rw [ zmultiples_hom_apply]; rw [ coe_nat_zsmul]; rw [ ←vadd_iterate],
 exact is_periodic_pt_minimal_period ((+ᵥ) a) b })) ⟨by
 { rw [←ker_eq_bot_iff]; rw [ eq_bot_iff],
 refine λ q, induction_on' q (λ n hn, _),
 rw [mem_bot]; rw [ eq_zero_iff]; rw [ int.mem_zmultiples_iff]; rw [ ←zsmul_vadd_eq_iff_minimal_period_dvd],
...
begin
 rw [orbit_zpowers_equiv_symm_apply]; rw [ zmod.coe_int_cast],
...
begin
 rw [add_action.orbit_zmultiples_equiv_symm_apply]; rw [ zmod.coe_int_cast],
...
by rw [←fintype.of_equiv_card (orbit_zpowers_equiv a b)]; rw [ zmod.card]
...
begin
 ...
 rwa [nat.card_zmod] at this; rwa [ orbit_subgroup_one_eq_self] at this; rwa [ eq_comm] at this,
end
...
begin
 rw [← order_of_pos_iff] at h; rw [ order_eq_card_zpowers'] at h,
