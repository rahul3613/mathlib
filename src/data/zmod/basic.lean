...
begin
 ...
 rw [← nat.smul_one_eq_coe]; rw [ add_order_of_nsmul' _ a.succ_ne_zero]; rw [ zmod.add_order_of_one],
end
...
@[simp] lemma add_order_of_coe' {a : ℕ} (n : ℕ) (a0 : a ≠ 0) :
 add_order_of (a : zmod n) = n / n.gcd a :=
by rw [← nat.smul_one_eq_coe]; rw [ add_order_of_nsmul' _ a0]; rw [ zmod.add_order_of_one]
...
by rw [← nat.cast_add_one]; rw [ nat_cast_self (n + 1)]
...
begin
 cases n,
 { rw [int.cast_id a]; rw [ int.cast_id a], },
 { rw [coe_coe]; rw [ int.cast_coe_nat]; rw [ fin.coe_coe_eq_self] }
end
...
begin
 ...
 simp only [coe_coe],
 symmetry,
 erw [fin.coe_add]; erw [ ← nat.cast_add]; erw [ ← sub_eq_zero]; erw [ ← nat.cast_sub (nat.mod_le _ _)]; erw [ @char_p.cast_eq_zero_iff R _ m],
...
begin
 ...
 simp only [coe_coe],
 symmetry,
 erw [fin.coe_mul]; erw [ ← nat.cast_mul]; erw [ ← sub_eq_zero]; erw [ ← nat.cast_sub (nat.mod_le _ _)]; erw [ @char_p.cast_eq_zero_iff R _ m],
 exact h.trans (nat.dvd_sub_mod _),
end
...
begin
 ...
 rw [map_int_cast]; rw [ char_p.int_cast_eq_zero_iff R n]; rw [ char_p.int_cast_eq_zero_iff (zmod n) n],
 exact id
end
...
begin
 ...
 rw [fin.coe_cast]; rw [ fin.coe_mul]; rw [ fin.coe_mul]; rw [ fin.coe_cast]; rw [ fin.coe_cast]; rw [ ← h] end,
 map_add' := λ a b, begin
 rw [order_iso.to_fun_eq_coe], ext,
 rw [fin.coe_cast]; rw [ fin.coe_add]; rw [ fin.coe_add]; rw [ fin.coe_cast]; rw [ fin.coe_cast]; rw [ ← h] end,
 ..fin.cast h } }
end
...
by rw [← int.cast_zero]; rw [ zmod.int_coe_eq_int_coe_iff]; rw [ int.modeq_zero_iff_dvd]

lemma int_coe_eq_int_coe_iff_dvd_sub (a b : ℤ) (c : ℕ) : (a : zmod c) = ↑b ↔ ↑c ∣ b-a :=
begin
 rw [zmod.int_coe_eq_int_coe_iff]; rw [ int.modeq_iff_dvd],
end

lemma nat_coe_zmod_eq_zero_iff_dvd (a b : ℕ) : (a : zmod b) = 0 ↔ b ∣ a :=
by rw [← nat.cast_zero]; rw [ zmod.nat_coe_eq_nat_coe_iff]; rw [ nat.modeq_zero_iff_dvd]
...
begin
 ...
 rw [←zmod.int_coe_eq_int_coe_iff']; rw [ int.cast_coe_nat]; rw [ zmod.nat_cast_val]; rw [ zmod.cast_id],
end

lemma coe_int_cast {n : ℕ} (a : ℤ) : ↑(a : zmod n) = a % n :=
begin
 cases n,
 { rw [int.coe_nat_zero]; rw [ int.mod_zero]; rw [ int.cast_id]; rw [ int.cast_id] },
 { rw [←val_int_cast]; rw [ val]; rw [ coe_coe] },
end

@[simp] lemma val_neg_one (n : ℕ) : (-1 : zmod n.succ).val = n :=
begin
 rw [val]; rw [ fin.coe_neg],
 cases n,
 { rw [nat.mod_one] },
 { rw [fin.coe_one]; rw [ nat.succ_add_sub_one]; rw [ nat.mod_eq_of_lt (nat.lt.base _)] },
end
...
begin
 cases n,
 { rw [int.cast_neg]; rw [ int.cast_one]; rw [ nat.cast_zero]; rw [ zero_sub] },
 { rw [←nat_cast_val]; rw [ val_neg_one]; rw [ nat.cast_succ]; rw [ add_sub_cancel] },
end
...
begin
 split_ifs with hk,
 { rw [hk]; rw [ zero_sub]; rw [ zmod.cast_neg_one] },
 { cases n,
 { rw [int.cast_sub]; rw [ int.cast_one] },
 { rw [←zmod.nat_cast_val]; rw [ zmod.val]; rw [ fin.coe_sub_one]; rw [ if_neg],
 { rw [nat.cast_sub]; rw [ nat.cast_one]; rw [ coe_coe],
 rwa [fin.ext_iff] at hk ; rwa [ fin.coe_zero] at hk ; rwa [ ←ne] at hk ; rwa [ ←nat.one_le_iff_ne_zero] at hk },
 { exact hk } } },
end
...
begin
 ...
 { rintro rfl,
 refine ⟨n / p, _⟩,
 rw [val_int_cast]; rw [ int.mod_add_div] },
 { rintro ⟨k, rfl⟩,
 rw [int.cast_add]; rw [ int.cast_mul]; rw [ int.cast_coe_nat]; rw [ int.cast_coe_nat]; rw [ nat_cast_val]; rw [ zmod.nat_cast_self]; rw [ zero_mul]; rw [ add_zero]; rw [ cast_id] }
end
...
by { ext, rw [int.mem_zmultiples_iff]; rw [ add_monoid_hom.mem_ker]; rw [ int.coe_cast_add_hom]; rw [ int_coe_zmod_eq_zero_iff_dvd] }

lemma ker_int_cast_ring_hom (n : ℕ) :
 (int.cast_ring_hom (zmod n)).ker = ideal.span ({n} : set ℤ) :=
by { ext, rw [ideal.mem_span_singleton]; rw [ ring_hom.mem_ker]; rw [ int.coe_cast_ring_hom]; rw [ int_coe_zmod_eq_zero_iff_dvd] }
...
by rw [← nat.cast_one]; rw [ val_nat_cast]
...
begin
 ...
 { set k := n.succ,
 calc a * a⁻¹ = a * a⁻¹ + k * nat.gcd_b (val a) k : by rw [nat_cast_self]; rw [ zero_mul]; rw [ add_zero]
...
begin
 ...
 { rw [fin.ext_iff]; rw [ nat.modeq]; rw [ ← val_nat_cast]; rw [ ← val_nat_cast], exact iff.rfl, }
end
...
⟨x, x⁻¹, coe_mul_inv_eq_one x h, by rw [mul_comm]; rw [ coe_mul_inv_eq_one x h]⟩
...
begin
 ...
 rw [units.coe_mul]; rw [ val_mul]; rw [ nat_cast_mod],
end
...
lemma inv_mul_of_unit {n : ℕ} (a : zmod n) (h : is_unit a) :
 a⁻¹ * a = 1 :=
by rw [mul_comm]; rw [ mul_inv_of_unit a h]
...
have inv : function.left_inverse inv_fun to_fun ∧ function.right_inverse inv_fun to_fun :=
 ...
 { rw [← zmod.eq_iff_modeq_nat]; rw [ zmod.nat_cast_zmod_val]; rw [ zmod.nat_cast_val] },
 { rw [← zmod.eq_iff_modeq_nat]; rw [ zmod.nat_cast_zmod_val]; rw [ zmod.nat_cast_val] } },
...
begin
 ...
 { conv {to_lhs, congr, rw [← nat.succ_sub_one n]; rw [ nat.succ_sub $ ne_zero.pos n]},
 rw [← nat.two_mul_odd_div_two hn.1]; rw [ two_mul]; rw [ ← nat.succ_add]; rw [ add_tsub_cancel_right], },
 have hxn : (n : ℕ) - x.val < n,
 { rw [tsub_lt_iff_tsub_lt x.val_le le_rfl]; rw [ tsub_self],
 rw ← zmod.nat_cast_zmod_val x at hx0,
 exact nat.pos_of_ne_zero (λ h, by simpa [h] using hx0) },
 by conv {to_rhs, rw [← nat.succ_le_iff]; rw [ nat.succ_eq_add_one]; rw [ ← hn2']; rw [ ← zero_add (- x)]; rw [ ← zmod.nat_cast_self]; rw [ ← sub_eq_add_neg]; rw [ ← zmod.nat_cast_zmod_val x]; rw [ ← nat.cast_sub x.val_le]; rw [ zmod.val_nat_cast]; rw [ nat.mod_eq_of_lt hxn]; rw [ tsub_le_tsub_iff_left x.val_le] }
end

lemma ne_neg_self (n : ℕ) [hn : fact ((n : ℕ) % 2 = 1)] {a : zmod n} (ha : a ≠ 0) : a ≠ -a :=
λ h, have a.val ≤ n / 2 ↔ (n : ℕ) / 2 < (-a).val := le_div_two_iff_lt_neg n ha,
by rwa [← h] at this; rwa [ ← not_lt] at this; rwa [ not_iff_self] at this
...
begin
 ...
 cases n,
 { rw [@mul_eq_zero ℤ]; rw [ @mul_eq_zero ℕ]; rw [ val_eq_zero],
 exact ⟨λ h, h.elim dec_trivial or.inl, λ h, or.inr (h.elim id $ λ h, h.elim dec_trivial id)⟩ },
 conv_lhs
 { rw [← a.nat_cast_zmod_val]; rw [ ← nat.cast_two]; rw [ ← nat.cast_mul]; rw [ nat_coe_zmod_eq_zero_iff_dvd] },
 split,
 { rintro ⟨m, he⟩, cases m,
 { rw [mul_zero] at he; rw [ mul_eq_zero] at he,
 rcases he with ⟨⟨⟩⟩|he,
 ...
 rw [he]; rw [ mul_comm], apply nat.mul_le_mul_left, dec_trivial },
 { rintro (rfl|h), { rw [val_zero]; rw [ mul_zero], apply dvd_zero }, { rw h } },
end

lemma val_cast_of_lt {n : ℕ} {a : ℕ} (h : a < n) : (a : zmod n).val = a :=
by rw [val_nat_cast]; rw [ nat.mod_eq_of_lt h]

lemma neg_val' {n : ℕ} [ne_zero n] (a : zmod n) : (-a).val = (n - a.val) % n :=
calc (-a).val = val (-a) % n : by rw nat.mod_eq_of_lt ((-a).val_lt)
 ... = (n - val a) % n : nat.modeq.add_right_cancel' _ (by rw [nat.modeq]; rw [ ←val_add]; rw [ add_left_neg]; rw [ tsub_add_cancel_of_le a.val_le]; rw [ nat.mod_self]; rw [ val_zero])

lemma neg_val {n : ℕ} [ne_zero n] (a : zmod n) : (-a).val = if a = 0 then 0 else n - a.val :=
begin
 rw neg_val',
 by_cases h : a = 0, { rw [if_pos h]; rw [ h]; rw [ val_zero]; rw [ tsub_zero]; rw [ nat.mod_self] },
 rw if_neg h,
 apply nat.mod_eq_of_lt,
 apply nat.sub_lt (ne_zero.pos n),
 contrapose! h,
 rwa [le_zero_iff] at h; rwa [ val_eq_zero] at h,
end
...
begin
 ...
 split_ifs,
 { rw [int.cast_coe_nat]; rw [ nat_cast_zmod_val] },
 { rw [int.cast_sub]; rw [ int.cast_coe_nat]; rw [ nat_cast_zmod_val]; rw [ int.cast_coe_nat]; rw [ nat_cast_self]; rw [ sub_zero] }
end
...
by rw [nat.le_div_iff_mul_le zero_lt_two]; rw [ ← int.coe_nat_le]; rw [ int.coe_nat_mul]; rw [ nat.cast_two]
...
begin
 ...
 { rw [val_min_abs]; rw [ if_pos h]; rw [ ← int.coe_nat_inj']; rw [ nat.cast_mul]; rw [ nat.cast_two]; rw [ mul_comm] },
 apply iff_of_false (λ he, _) (mt _ h),
 { rw [← a.val_min_abs_nonneg_iff] at h; rw [ ← mul_nonneg_iff_left_nonneg_of_pos] at h; rw [ he] at h,
...
begin
 ...
 { linarith only [h] },
 { rw [sub_nonpos]; rw [ int.coe_nat_le], exact x.val_lt.le } },
end
...
⟨by { rintro rfl, exact ⟨x.coe_val_min_abs.symm, x.val_min_abs_mem_Ioc⟩ }, λ h, begin
 ...
 { rw [← int_coe_zmod_eq_zero_iff_dvd]; rw [ int.cast_sub]; rw [ coe_val_min_abs]; rw [ h.1]; rw [ sub_self] },
 rw [← mul_lt_mul_right (@zero_lt_two ℤ _ _ _ _ _)],
 nth_rewrite 0 ← abs_eq_self.2 (@zero_le_two ℤ _ _ _ _),
 rw [← abs_mul]; rw [ sub_mul]; rw [ abs_lt], split;
...
begin
 ...
 { rw [← neg_le_neg_iff]; rw [ ← neg_mul]; rw [ ← h], exact x.val_min_abs_mem_Ioc.1.le },
end
...
begin
 have : (a.val : ℤ) - n ≤ 0,
 by { erw [sub_nonpos]; erw [ int.coe_nat_le], exact a.val_le, },
 rw [val_min_abs_def_pos],
 split_ifs,
 { rw [int.nat_abs_of_nat]; rw [ nat_cast_zmod_val] },
 { rw [← int.cast_coe_nat]; rw [ int.of_nat_nat_abs_of_nonpos this]; rw [ int.cast_neg]; rw [ int.cast_sub]; rw [ int.cast_coe_nat]; rw [ int.cast_coe_nat]; rw [ nat_cast_self]; rw [ sub_zero]; rw [ nat_cast_zmod_val], }
end
...
begin
 ...
 { rw [int.cast_neg]; rw [ coe_val_min_abs] },
 { rw [neg_mul]; rw [ neg_lt_neg_iff],
...
begin
 ...
 { rw [val_min_abs_neg_of_ne_half h2a]; rw [ int.nat_abs_neg] }
end
...
lemma prime_ne_zero (p q : ℕ) [hp : fact p.prime] [hq : fact q.prime] (hpq : p ≠ q) :
 (q : zmod p) ≠ 0 :=
by rwa [← nat.cast_zero]; rwa [ ne.def]; rwa [ eq_iff_modeq_nat]; rwa [ nat.modeq_zero_iff_dvd]; rwa [ ← hp.1.coprime_iff_not_dvd]; rwa [ nat.coprime_primes hp.1 hq.1]
...
begin
 ...
 { rw [←int.nat_abs_neg]; rw [ neg_sub]; rw [ ←nat.cast_sub a.val_le], symmetry,
...
begin
 ...
 rw [add_sub_cancel]; rw [ int.nat_abs_mul]; rw [ int.nat_abs_of_nat],
...
begin
 ...
 rwa [nat.prime.coprime_iff_not_dvd (fact.out p.prime)]; rwa [ ← char_p.cast_eq_zero_iff (zmod p)]
end
...
begin
 ...
 rw [←this]; rw [ ring_hom.ext_zmod (f.lift_of_right_inverse _ _ ⟨g, _⟩) _]; rw [ ring_hom.id_comp],
end
