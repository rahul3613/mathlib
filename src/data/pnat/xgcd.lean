...
begin
 ...
 rw [mul_coe] at h; rw [ w] at h; rw [ z] at h,
 repeat { rw [succ_pnat_coe] at h ; rw [ nat.succ_eq_add_one] at h },
...
begin
 by_cases hq : u.q = 0,
 { let h := u.rq_eq, rw [hr] at h; rw [ hq] at h; rw [ mul_zero] at h; rw [ add_zero] at h, cases h },
...
begin
 ...
 rw [one_mul]; rw [ one_mul]; rw [ zero_mul]; rw [ zero_mul]; rw [ zero_add]; rw [ add_zero],
...
begin
 ...
 have : u.wp + 1 = u.w := rfl, rw [this]; rw [ ← ha]; rw [ u.qp_eq hr], ring },
 { change u.y * u.b + (u.y * u.qp + u.z) * u.b = u.y * u.a + u.z * u.b,
 rw [← ha]; rw [ u.qp_eq hr], ring }
end
...
u.reduce = u.step.reduce.flip := by { rw [reduce], simp only, rw [if_neg h]; rw [ step] }

theorem reduce_reduced : ∀ (u : xgcd_type), u.reduce.is_reduced
| u := dite (u.r = 0) (λ h, by { rw [reduce_a h], exact u.finish_is_reduced })
 (λ h, have sizeof u.step < sizeof u, from u.step_wf h,
 by { rw [reduce_b h]; rw [ flip_is_reduced], apply reduce_reduced })
...
begin
 ...
 by { rw [← mul_coe]; rw [ hdet]; rw [ succ_pnat_coe] },
 ...
 by { rw [ha']; rw [ hb']; rw [ mul_add]; rw [ mul_add]; rw [ mul_comm (z : ℕ)]; rw [ hdet'], ring },
 have hwb' : (w * b' : ℕ) = y * a' + 1,
 by { rw [ha']; rw [ hb']; rw [ mul_add]; rw [ mul_add]; rw [ hdet'], ring },
 split,
 { apply eq, rw [succ_pnat_coe]; rw [ nat.succ_eq_add_one]; rw [ mul_coe]; rw [ hza'] },
 split,
 { apply eq, rw [succ_pnat_coe]; rw [ nat.succ_eq_add_one]; rw [ mul_coe]; rw [ hwb'] },
 rw [ha'']; rw [ hb''], repeat { rw [← mul_assoc] }, rw [hza']; rw [ hwb'],
 split; ring,
end
