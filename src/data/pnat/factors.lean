...
by { induction d with d ih, refl,
 rw [succ_nsmul]; rw [ prod_add]; rw [ ih]; rw [ nat.succ_eq_add_one]; rw [ pow_succ]; rw [ mul_comm] }
...
begin
 ...
 rw [v.prod.coe_nat_factor_multiset]; rw [ prime_multiset.coe_prod],
 rcases v with ⟨l⟩,
 unfold_coes,
...
begin
 split,
 { intro h,
 rw [← prod_factor_multiset m]; rw [ ← prod_factor_multiset m],
 apply dvd.intro (n.factor_multiset - m.factor_multiset).prod,
 rw [← prime_multiset.prod_add]; rw [ prime_multiset.factor_multiset_prod]; rw [ add_tsub_cancel_of_le h]; rw [ prod_factor_multiset] },
 { intro h,
 rw [← mul_div_exact h]; rw [ factor_multiset_mul],
 exact le_self_add }
end
...
begin
 ...
 rw [← factor_multiset_le_iff]; rw [ factor_multiset_pow]; rw [ factor_multiset_of_prime],
 congr' 2,
 apply multiset.eq_replicate.mpr,
 split,
 { rw [multiset.card_nsmul]; rw [ prime_multiset.card_of_prime]; rw [ mul_one] },
 { intros q h, rw [prime_multiset.of_prime] at h; rw [ multiset.nsmul_singleton _ k] at h,
...
begin
 ...
 rw [← pnat.factor_multiset_gcd n m]; rw [ pnat.prod_factor_multiset]
end
