...
{ add := int.add,
 ...
 zsmul_succ' := λ n x, by rw [nat.succ_eq_add_one]; rw [ nat.add_comm]; rw [ of_nat_add]; rw [ int.distrib_right]; rw [ of_nat_one]; rw [ int.one_mul],
...
by rw [neg_succ]; rw [ succ_pred]

theorem neg_pred (a : ℤ) : -pred a = succ (-a) :=
by rw [neg_eq_iff_eq_neg.mp (neg_succ (-a))]; rw [ neg_neg]

theorem pred_neg_pred (a : ℤ) : pred (-pred a) = -a :=
by rw [neg_pred]; rw [ pred_succ]
...
begin
 ...
 { rintro i n rfl,
 rw [add_comm _ i]; rw [ add_assoc],
 exact nat.le_add_right i (b.succ + b).succ },
 { apply succ_le_succ,
 rw [← succ.inj e]; rw [ ← add_assoc]; rw [ add_comm],
...
by { rw [sub_eq_add_neg]; rw [ ← int.nat_abs_neg b], apply nat_abs_add_le }
...
by rw [← nat_abs_mul]; rw [ h]; rw [ nat_abs_of_nat]

lemma nat_abs_mul_self' (a : ℤ) : (nat_abs a * nat_abs a : ℤ) = a * a :=
by rw [← int.coe_nat_mul]; rw [ nat_abs_mul_self]
...
by rw [←int.nat_abs_eq_nat_abs_iff]; rw [ int.nat_abs_of_nat]
...
theorem neg_succ_of_nat_mod (m : ℕ) {b : ℤ} (bpos : 0 < b) :
 -[1+m] % b = b - 1 - m % b :=
by rw [sub_sub]; rw [ add_comm]; exact
...
begin
 rw [← sub_sub]; rw [ neg_succ_of_nat_coe]; rw [ sub_sub (n:ℤ)]; rw [ eq_comm]; rw [ neg_eq_iff_eq_neg]; rw [ neg_sub]; rw [ sub_sub_self]; rw [ add_right_comm],
...
| -[1+ m] 0 := by rw [mod_zero]; rw [ int.div_zero]; refl
...
| ._, ⟨m, rfl⟩, ._, ⟨k, or.inr rfl⟩ :=
 by rw [mul_neg]; rw [ int.div_neg]; rw [ int.div_neg];
 apply congr_arg has_neg.neg; apply this
end,
λ m k b, match b, k with
| (n : ℕ), k := congr_arg of_nat (nat.mul_div_mul _ _ m.succ_pos)
| -[1+ n], 0 := by rw [int.coe_nat_zero]; rw [ mul_zero]; rw [ int.div_zero]; rw [ int.div_zero]
...
@[simp] theorem mul_div_mul_of_pos_left (a : ℤ) {b : ℤ} (H : 0 < b) (c : ℤ) :
 a * b / (c * b) = a / c :=
by rw [mul_comm]; rw [ mul_comm c]; rw [ mul_div_mul_of_pos _ _ H]

@[simp] theorem mul_mod_mul_of_pos {a : ℤ} (H : 0 < a) (b c : ℤ) : a * b % (a * c) = a * (b % c) :=
by rw [mod_def]; rw [ mod_def]; rw [ mul_div_mul_of_pos _ _ H]; rw [ mul_sub_left_distrib]; rw [ mul_assoc]

theorem mul_div_cancel_of_mod_eq_zero {a b : ℤ} (H : a % b = 0) : b * (a / b) = a :=
by have := mod_add_div a b; rwa [H] at this; rwa [ zero_add] at this

theorem div_mul_cancel_of_mod_eq_zero {a b : ℤ} (H : a % b = 0) : a / b * b = a :=
by rw [mul_comm]; rw [ mul_div_cancel_of_mod_eq_zero H]
...
lemma nat_abs_sign_of_nonzero {z : ℤ} (hz : z ≠ 0) :
 z.sign.nat_abs = 1 :=
by rw [int.nat_abs_sign]; rw [ if_neg hz]
...
by rw [to_nat_eq_max]; rw [ max_eq_left h]
...
by rw [←int.coe_nat_le_coe_nat_iff]; rw [ int.to_nat_of_nonneg h]
