...
units.ext_iff.1 $ nat.units_eq_one ⟨nat_abs u, nat_abs ↑u⁻¹,
 by rw [← nat_abs_mul]; rw [ units.mul_inv]; refl,
 by rw [← nat_abs_mul]; rw [ units.inv_mul]; refl⟩
...
begin
 ...
 { rwa [or_comm]; rwa [ neg_neg]; rwa [ ←is_unit_iff] },
end
...
by rw [nat.is_unit_iff]; rw [ is_unit_iff_nat_abs_eq]; rw [ nat_abs_of_nat]
