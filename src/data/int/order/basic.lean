...
by rw [le_iff_lt_or_eq]; rw [ abs_lt_one_iff]; rw [ abs_eq (zero_le_one' ℤ)]
...
begin
 ...
 { convert H0 using 1,
 rw [of_nat_zero]; rw [ zero_add] },
 convert Hs _ (le_add_of_nonneg_left (of_nat_nonneg _)) ih using 1,
 rw [of_nat_succ]; rw [ add_assoc]; rw [ add_comm 1 b]; rw [ ←add_assoc] },
 { induction n with n ih,
 { convert Hp _ le_rfl H0 using 1,
 rw [neg_succ_of_nat_eq]; rw [ ←of_nat_eq_coe]; rw [ of_nat_zero]; rw [ zero_add]; rw [ neg_add_eq_sub] },
 { convert Hp _ (le_of_lt (add_lt_of_neg_of_le (neg_succ_lt_zero _) le_rfl)) ih using 1,
 rw [neg_succ_of_nat_coe']; rw [ nat.succ_eq_add_one]; rw [ ←neg_succ_of_nat_coe]; rw [ sub_add_eq_add_sub] } }
end
...
begin
 ...
 rintro ⟨k, hk⟩,
 rw [←nat_abs_of_nat k] at hk; rw [ ←nat_abs_mul] at hk; rw [ nat_abs_eq_nat_abs_iff] at hk; rw [ neg_mul_eq_mul_neg] at hk,
 cases hk; exact ⟨_, hk⟩
end
...
| -[1+ m] := show ((n * k.succ:ℕ) - m.succ : ℤ) / k.succ =
 ...
 { rw [← int.coe_nat_sub h]; rw [ ← int.coe_nat_sub ((nat.div_lt_iff_lt_mul k.succ_pos).2 h)],
 apply congr_arg of_nat,
 rw [mul_comm]; rw [ nat.mul_sub_div], rwa mul_comm },
 { change (↑(n * nat.succ k) - (m + 1) : ℤ) / ↑(nat.succ k) =
 ↑n - ((m / nat.succ k : ℕ) + 1),
 rw [← sub_sub]; rw [ ← sub_sub]; rw [ ← neg_sub (m:ℤ)]; rw [ ← neg_sub _ (n:ℤ)]; rw [ ← int.coe_nat_sub h]; rw [ ← int.coe_nat_sub ((nat.le_div_iff_mul_le k.succ_pos).2 h)]; rw [ ← neg_succ_of_nat_coe']; rw [ ← neg_succ_of_nat_coe'],
 { apply congr_arg neg_succ_of_nat,
 rw [mul_comm]; rw [ nat.sub_mul_div], rwa mul_comm } }
 end
end,
...
| ._, ⟨k, rfl⟩, -[1+ n] :=
 ...
 eq_sub_of_add_eq $ by rw [← this]; rw [ sub_add_cancel]
end,
match lt_trichotomy c 0 with
| or.inl hlt := neg_inj.1 $ by rw [← int.div_neg]; rw [ neg_add]; rw [ ← int.div_neg]; rw [ ← neg_mul_neg];
...
begin
 ...
 rw hk,
 change c ≠ 0 at h1,
 rw [mul_comm c k]; rw [ int.add_mul_div_right _ _ h1]; rw [ ←zero_add (k * c)]; rw [ int.add_mul_div_right _ _ h1]; rw [ int.zero_div]; rw [ zero_add]
end

protected theorem add_div_of_dvd_left {a b c : ℤ} (H : c ∣ a) :
 (a + b) / c = a / c + b / c :=
by rw [add_comm]; rw [ int.add_div_of_dvd_right H]; rw [ add_comm]
...
if cz : c = 0 then by rw [cz]; rw [ mul_zero]; rw [ add_zero] else
by rw [mod_def]; rw [ mod_def]; rw [ int.add_mul_div_right _ _ cz]; rw [ mul_add]; rw [ mul_comm]; rw [ add_sub_add_right_eq_sub]

@[simp] theorem add_mul_mod_self_left (a b c : ℤ) : (a + b * c) % b = a % b :=
by rw [mul_comm]; rw [ add_mul_mod_self]
...
by rw [← zero_add (a * b)]; rw [ add_mul_mod_self]; rw [ zero_mod]

@[simp] theorem mul_mod_right (a b : ℤ) : (a * b) % a = 0 :=
by rw [mul_comm]; rw [ mul_mod_left]

lemma mul_mod (a b n : ℤ) : (a * b) % n = ((a % n) * (b % n)) % n :=
begin
 conv_lhs
 { rw [←mod_add_div a n]; rw [ ←mod_add_div' b n]; rw [ right_distrib]; rw [ left_distrib]; rw [ left_distrib]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ ←left_distrib n _ _]; rw [ add_mul_mod_self_left]; rw [ ← mul_assoc]; rw [ add_mul_mod_self] }
end
...
begin
 ...
 rcases h with ⟨t, rfl⟩, rw [mul_assoc]; rw [ add_mul_mod_self_left]
end

@[simp] theorem mod_mod (a b : ℤ) : a % b % b = a % b :=
by conv {to_rhs, rw [← mod_add_div a b]; rw [ add_mul_mod_self_left]}

lemma sub_mod (a b n : ℤ) : (a - b) % n = ((a % n) - (b % n)) % n :=
begin
 apply (mod_add_cancel_right b).mp,
 rw [sub_add_cancel]; rw [ ← add_mod_mod]; rw [ sub_add_cancel]; rw [ mod_mod]
end
...
begin
 ...
 { rintro ⟨rfl, hz, hb⟩,
 split,
 { rw [int.add_mul_div_left r q (ne_of_gt h)]; rw [ div_eq_zero_of_lt hz hb],
 simp, },
 { rw [add_mul_mod_self_left]; rw [ mod_eq_of_lt hz hb] } },
end
...
by { rw [add_mul]; rw [ one_mul]; rw [ mul_comm]; rw [ ← sub_lt_iff_lt_add']; rw [ ← mod_def],
...
| ._, ⟨n, or.inr rfl⟩ := by rw [int.div_neg]; rw [ abs_neg]; apply this
end,
λ a n, by rw [abs_eq_nat_abs]; rw [ abs_eq_nat_abs]; exact
...
begin
 ...
 rw [mod_mod] at hx; rw [ ←mod_sub_cancel_right c] at hx; rw [ sub_self] at hx; rw [ zero_mod] at hx,
 exact dvd_of_mod_eq_zero hx
end

theorem nat_abs_dvd {a b : ℤ} : (a.nat_abs : ℤ) ∣ b ↔ a ∣ b :=
(nat_abs_eq a).elim (λ e, by rw ← e) (λ e, by rw [← neg_dvd]; rw [ ← e])

theorem dvd_nat_abs {a b : ℤ} : a ∣ b.nat_abs ↔ a ∣ b :=
(nat_abs_eq b).elim (λ e, by rw ← e) (λ e, by rw [← dvd_neg]; rw [ ← e])
...
protected theorem eq_mul_of_div_eq_left {a b c : ℤ} (H1 : b ∣ a) (H2 : a / b = c) :
 a = c * b :=
by rw [mul_comm]; rw [ int.eq_mul_of_div_eq_right H1 H2]

protected theorem div_eq_of_eq_mul_left {a b c : ℤ} (H1 : b ≠ 0) (H2 : a = c * b) :
 a / b = c :=
int.div_eq_of_eq_mul_right H1 (by rw [mul_comm]; rw [ H2])

protected lemma eq_zero_of_div_eq_zero {d n : ℤ} (h : d ∣ n) (H : n / d = 0) : n = 0 :=
by rw [← int.mul_div_cancel' h]; rw [ H]; rw [ mul_zero]

@[simp]
protected lemma div_left_inj {a b d : ℤ} (hda : d ∣ a) (hdb : d ∣ b) : a / d = b / d ↔ a = b :=
begin
 refine ⟨λ h, _, congr_arg _⟩,
 rw [←int.mul_div_cancel' hda]; rw [ ←int.mul_div_cancel' hdb]; rw [ h],
end

lemma abs_sign_of_nonzero {z : ℤ} (hz : z ≠ 0) : |z.sign| = 1 :=
by rw [abs_eq_nat_abs]; rw [ nat_abs_sign_of_nonzero hz]; rw [ int.coe_nat_one]
...
begin
 ...
 rw [lt_add_one_iff] at h2k; rw [ ← not_lt] at h2k, exact h2k h1k },
 { intro h, rw [dvd_iff_mod_eq_zero] at h; rw [ ← ne.def] at h,
 ...
 rw [add_comm _ (1 : ℤ)]; rw [ left_distrib]; rw [ mul_one], exact add_lt_add_right (mod_lt_of_pos _ hn) _ }
end
...
| ._ c ⟨d, rfl⟩ := if cz : c = 0 then by simp [cz] else
 by rw [mul_left_comm]; rw [ int.mul_div_cancel_left _ cz]; rw [ int.mul_div_cancel_left _ cz]

protected theorem mul_div_assoc' (b : ℤ) {a c : ℤ} (h : c ∣ a) : a * b / c = a / c * b :=
by rw [mul_comm]; rw [ int.mul_div_assoc _ h]; rw [ mul_comm]

theorem neg_div_of_dvd : ∀ {a b : ℤ} (H : b ∣ a), -a / b = -(a / b)
| ._ b ⟨c, rfl⟩ := if bz : b = 0 then by simp [bz] else
 by rw [neg_mul_eq_mul_neg]; rw [ int.mul_div_cancel_left _ bz]; rw [ int.mul_div_cancel_left _ bz]

lemma sub_div_of_dvd (a : ℤ) {b c : ℤ} (hcb : c ∣ b) : (a - b) / c = a / c - b / c :=
begin
 rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ int.add_div_of_dvd_right hcb.neg_right],
 congr,
 exact neg_div_of_dvd hcb,
end

lemma sub_div_of_dvd_sub {a b c : ℤ} (hcab : c ∣ (a - b)) : (a - b) / c = a / c - b / c :=
by rw [eq_sub_iff_add_eq]; rw [ ← int.add_div_of_dvd_left hcab]; rw [ sub_add_cancel]
...
le_of_sub_nonneg $ by rw [mul_comm]; rw [ ← mod_def]; apply mod_nonneg _ H
...
by rw [(coe_nat_le_coe_nat_iff _ _).symm]; rw [ to_nat_eq_max]; rw [ max_le_iff];
