...
by rw [is_unit_iff_nat_abs_eq]; rw [ abs_eq_nat_abs]; rw [ ←int.coe_nat_one]; rw [ coe_nat_inj']

lemma is_unit_sq {a : ℤ} (ha : is_unit a) : a ^ 2 = 1 :=
by rw [sq]; rw [ is_unit_mul_self ha]

@[simp] lemma units_sq (u : ℤˣ) : u ^ 2 = 1 :=
by rw [units.ext_iff]; rw [ units.coe_pow]; rw [ units.coe_one]; rw [ is_unit_sq u.is_unit]

alias units_sq ← units_pow_two

@[simp] lemma units_mul_self (u : ℤˣ) : u * u = 1 :=
by rw [←sq]; rw [ units_sq]

@[simp] lemma units_inv_eq_self (u : ℤˣ) : u⁻¹ = u :=
by rw [inv_eq_iff_mul_eq_one]; rw [ units_mul_self]
...
by rw [←units.coe_mul]; rw [ units_mul_self]; rw [ units.coe_one]
...
by conv {to_lhs, rw ← nat.mod_add_div n 2}; rw [pow_add]; rw [ pow_mul]; rw [ units_sq]; rw [ one_pow]; rw [ mul_one]
