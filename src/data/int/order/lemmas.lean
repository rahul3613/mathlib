...
begin
 rw [← abs_eq_iff_mul_self_eq]; rw [ abs_eq_nat_abs]; rw [ abs_eq_nat_abs],
 exact int.coe_nat_inj'.symm
end

lemma eq_nat_abs_iff_mul_eq_zero : a.nat_abs = n ↔ (a - n) * (a + n) = 0 :=
by rw [nat_abs_eq_iff]; rw [ mul_eq_zero]; rw [ sub_eq_zero]; rw [ add_eq_zero_iff_eq_neg]

lemma nat_abs_lt_iff_mul_self_lt {a b : ℤ} : a.nat_abs < b.nat_abs ↔ a * a < b * b :=
begin
 rw [← abs_lt_iff_mul_self_lt]; rw [ abs_eq_nat_abs]; rw [ abs_eq_nat_abs],
 exact int.coe_nat_lt.symm
end

lemma nat_abs_le_iff_mul_self_le {a b : ℤ} : a.nat_abs ≤ b.nat_abs ↔ a * a ≤ b * b :=
begin
 rw [← abs_le_iff_mul_self_le]; rw [ abs_eq_nat_abs]; rw [ abs_eq_nat_abs],
...
begin
 ...
 rcases h with ⟨d, rfl⟩,
 refine ⟨d, _⟩,
 rw [mul_assoc]; rw [ int.mul_div_cancel_left _ ha],
end
