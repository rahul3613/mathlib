...
eq_sub_of_add_eq $ by rw [← cast_add]; rw [ nat.sub_add_cancel h]

@[simp, norm_cast] theorem cast_pred : ∀ {n}, 0 < n → ((n - 1 : ℕ) : R) = n - 1
| 0 h := by cases h
| (n+1) h := by rw [cast_succ]; rw [ add_sub_cancel]; refl
...
| (0 : ℕ) := by erw [cast_zero]; erw [ neg_zero]
| (n + 1 : ℕ) := by erw [cast_of_nat]; erw [ cast_neg_succ_of_nat]; refl
| -[1+ n] := by erw [cast_of_nat]; erw [ cast_neg_succ_of_nat]; erw [ neg_neg]
...
begin
 ...
 { rw [sub_nat_nat]; rw [ cast_neg_succ_of_nat]; rw [ nat.add_one]; rw [ ← e]; rw [ nat.cast_sub $ _root_.le_of_lt $ nat.lt_of_sub_eq_succ e]; rw [ neg_sub] },
end
...
| (m : ℕ) -[1+ n] := by erw [cast_sub_nat_nat]; erw [ cast_coe_nat]; erw [ cast_neg_succ_of_nat]; erw [ sub_eq_add_neg]
| -[1+ m] (n : ℕ) := by erw [cast_sub_nat_nat]; erw [ cast_coe_nat]; erw [ cast_neg_succ_of_nat]; erw [ sub_eq_iff_eq_add]; erw [ add_assoc]; erw [ eq_neg_add_iff_add_eq]; erw [ ← nat.cast_add]; erw [ ← nat.cast_add]; erw [ nat.add_comm]
| -[1+ m] -[1+ n] := show (-[1+ m + n + 1] : R) = _,
 by rw [cast_neg_succ_of_nat]; rw [ cast_neg_succ_of_nat]; rw [ cast_neg_succ_of_nat]; rw [ ← neg_add_rev]; rw [ ← nat.cast_add]; rw [ nat.add_right_comm m n 1]; rw [ nat.add_assoc]; rw [ nat.add_comm]
