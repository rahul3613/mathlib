...
⟨λ h, begin cases n,
 ...
 { rw [cast_neg_succ_of_nat] at h; rw [ neg_eq_zero] at h; rw [ nat.cast_eq_zero] at h,
 contradiction }
end, λ h, by rw [h]; rw [ cast_zero]⟩

@[simp, norm_cast] theorem cast_inj [add_group_with_one α] [char_zero α] {m n : ℤ} :
 (m : α) = n ↔ m = n :=
by rw [← sub_eq_zero]; rw [ ← cast_sub]; rw [ cast_eq_zero]; rw [ sub_eq_zero]
