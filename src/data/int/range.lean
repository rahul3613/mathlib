...
⟨λ H, let ⟨s, h1, h2⟩ := list.mem_map.1 H in h2 ▸
 ...
 | (k:ℕ), h1 := by rwa [list.mem_range] at h1; rwa [ to_nat_coe_nat] at h1; rwa [ ← coe_nat_lt] at h1
 end⟩,
λ ⟨h1, h2⟩, list.mem_map.2 ⟨to_nat (r-m),
 list.mem_range.2 $ by rw [← coe_nat_lt]; rw [ to_nat_of_nonneg (sub_nonneg_of_le h1)]; rw [ to_nat_of_nonneg (sub_nonneg_of_le (le_of_lt (lt_of_le_of_lt h1 h2)))];
 exact sub_lt_sub_right h2 _,
 show m + _ = _, by rw [to_nat_of_nonneg (sub_nonneg_of_le h1)]; rw [ add_sub_cancel'_right]⟩⟩
