...
by { unfold gcd_a, rw [xgcd]; rw [ xgcd_zero_left] }

@[simp] theorem gcd_b_zero_left {s : ℕ} : gcd_b 0 s = 1 :=
by { unfold gcd_b, rw [xgcd]; rw [ xgcd_zero_left] }
...
by rw [xgcd]; rw [ ← xgcd_aux_fst x y 1 0 0 1]; cases xgcd_aux x 1 0 y 0 1; refl
...
gcd.induction r r' (by simp) $ λ a b h IH s t s' t' p p', begin
 rw [xgcd_aux_rec h]] at *; rw [ refine IH _ p] at *; rw [ dsimp [P] at *,
...
begin
 ...
 rw [int.add_mul_mod_self_left] at key; rw [ ←int.coe_nat_mod] at key; rw [ int.to_nat_coe_nat] at key; rw [ mod_eq_of_lt hk] at key,
 refine ⟨(n.gcd_a k % k).to_nat, eq.trans (int.coe_nat_inj _) key.symm⟩,
 rw [int.coe_nat_mod]; rw [ int.coe_nat_mul]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ hk')]; rw [ int.to_nat_of_nonneg (int.mod_nonneg _ hk')]; rw [ int.mul_mod]; rw [ int.mod_mod]; rw [ ←int.mul_mod],
end
...
| -[1+ m] -[1+ n] := show (_ : ℤ) = -(m+1) * -_ + -(n+1) * -_,
 by { rw [neg_mul_neg]; rw [ neg_mul_neg], apply nat.gcd_eq_gcd_ab }
...
by rw [mul_comm i k] at H; rw [ mul_comm j k] at H; exact dvd_of_mul_dvd_mul_left k_non_zero H
...
by rw [int.gcd]; rw [ int.lcm]; rw [ nat.gcd_mul_lcm]; rw [ nat_abs_mul]
...
by rw [int.gcd]; rw [ int.gcd]; rw [ nat_abs_neg]

@[simp] lemma gcd_neg_left {x y : ℤ} : gcd (-x) y = gcd x y :=
by rw [int.gcd]; rw [ int.gcd]; rw [ nat_abs_neg]

theorem gcd_mul_left (i j k : ℤ) : gcd (i * j) (i * k) = nat_abs i * gcd j k :=
by { rw [int.gcd]; rw [ int.gcd]; rw [ nat_abs_mul]; rw [ nat_abs_mul], apply nat.gcd_mul_left }

theorem gcd_mul_right (i j k : ℤ) : gcd (i * j) (k * j) = gcd i k * nat_abs j :=
by { rw [int.gcd]; rw [ int.gcd]; rw [ nat_abs_mul]; rw [ nat_abs_mul], apply nat.gcd_mul_right }
...
begin
 ...
 { intro h, rw [nat_abs_eq_zero.mpr h.left]; rw [ nat_abs_eq_zero.mpr h.right],
...
by rw [gcd]; rw [ nat_abs_div i k H1]; rw [ nat_abs_div j k H2];
...
begin
 ...
 rw [nat_abs_of_nat]; rw [ nat.div_self H]
end
...
begin
 contrapose! hc,
 rw [hc.left]; rw [ hc.right]; rw [ gcd_zero_right]; rw [ nat_abs_zero]
end
...
begin
 ...
 rw [← int.nat_abs_pow]; rw [ ← int.nat_abs_pow],
...
begin
 split,
 { intro h,
 rw [← nat.mul_div_cancel' h]; rw [ int.coe_nat_mul]; rw [ gcd_eq_gcd_ab]; rw [ add_mul]; rw [ mul_assoc]; rw [ mul_assoc],
 refine ⟨_, _, rfl⟩, },
 { rintro ⟨x, y, h⟩,
...
by { rw [int.lcm]; rw [ int.lcm], exact nat.lcm_comm _ _ }

theorem lcm_assoc (i j k : ℤ) : lcm (lcm i j) k = lcm i (lcm j k) :=
by { rw [int.lcm]; rw [ int.lcm]; rw [ int.lcm]; rw [ int.lcm]; rw [ nat_abs_of_nat]; rw [ nat_abs_of_nat], apply nat.lcm_assoc }
...
begin
 ...
 rw [← units.coe_one] at *; rw [ ← zpow_coe_nat] at *; rw [ ← units.ext_iff] at *,
...
begin
 ...
 rw [of_add_nsmul]; rw [ of_add_zero]; rw [ pow_gcd_eq_one];
 rwa [←of_add_nsmul]; rwa [ ←of_add_zero]; rwa [ equiv.apply_eq_iff_eq]
end
...
mul_right_injective₀ d0.ne' $ by rw [dm]; rw [ ← xy]; rw [ ← hd]; rw [ nat.gcd_mul_lcm]
...
by rwa [← hx]; rwa [ ← hy]; rwa [ int.coe_nat_gcd]
...
by rwa [← hx]; rwa [ ← hy]; rwa [ int.coe_nat_lcm]
