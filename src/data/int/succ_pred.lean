...
| 0 := (add_zero a).symm
| (n + 1) := by { rw [function.iterate_succ']; rw [ int.coe_nat_succ]; rw [ ←add_assoc],
 exact congr_arg _ (succ_iterate n) }

lemma pred_iterate (a : ℤ) : ∀ n, pred^[n] a = a - n
| 0 := (sub_zero a).symm
| (n + 1) := by { rw [function.iterate_succ']; rw [ int.coe_nat_succ]; rw [ ←sub_sub],
 exact congr_arg _ (pred_iterate n) }

instance : is_succ_archimedean ℤ :=
⟨λ a b h, ⟨(b - a).to_nat,
 by rw [succ_eq_succ]; rw [ succ_iterate]; rw [ to_nat_sub_of_le h]; rw [ ←add_sub_assoc]; rw [ add_sub_cancel']⟩⟩

instance : is_pred_archimedean ℤ :=
⟨λ a b h, ⟨(b - a).to_nat, by rw [pred_eq_pred]; rw [ pred_iterate]; rw [ to_nat_sub_of_le h]; rw [ sub_sub_cancel]⟩⟩
...
by rw [int.covby_iff_succ_eq]; rw [ sub_add_cancel]
...
by { rw [nat.covby_iff_succ_eq]; rw [ int.covby_iff_succ_eq], exact int.coe_nat_inj' }
