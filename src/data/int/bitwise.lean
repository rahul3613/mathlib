...
| (n : ℕ) :=
 by rw [show (cond (bodd n) 1 0 : ℤ) = (cond (bodd n) 1 0 : ℕ), by cases bodd n; refl]; exact congr_arg of_nat n.bodd_add_div2
| -[1+ n] := begin
...
by rw [bit_val]; rw [ nat.bit_val]; cases b; refl
...
private meta def bitwise_tac : tactic unit := `[
 ...
 try {rw [ show nat.bitwise (λ a b, a && bnot b) n m = nat.bitwise (λ a b, b && bnot a) m n]; rw [ from congr_fun (congr_fun (@nat.bitwise_swap (λ a b, b && bnot a) rfl) n) m]},
...
@[simp] lemma shiftr_neg (m n : ℤ) : shiftr m (-n) = shiftl m n := by rw [← shiftl_neg]; rw [ neg_neg]
...
| (m : ℕ) n k := by rw [shiftr_coe_nat]; rw [ shiftr_coe_nat]; rw [ ← int.coe_nat_add]; rw [ shiftr_coe_nat]; rw [ nat.shiftr_add]
| -[1+ m] n k := by rw [shiftr_neg_succ]; rw [ shiftr_neg_succ]; rw [ ← int.coe_nat_add]; rw [ shiftr_neg_succ]; rw [ nat.shiftr_add]
...
| (m : ℕ) n -[1+k] := sub_nat_nat_elim n k.succ
 ...
 (λ i n, congr_arg coe $
 by rw [← nat.shiftl_sub]; rw [ add_tsub_cancel_left]; apply nat.le_add_right)
 (λ i n, congr_arg coe $
 by rw [add_assoc]; rw [ nat.shiftr_add]; rw [ ← nat.shiftl_sub]; rw [ tsub_self]; refl)
| -[1+ m] n -[1+k] := sub_nat_nat_elim n k.succ
 (λ n k i, shiftl -[1+ m] i = -[1+ nat.shiftr (nat.shiftl' tt m n) k])
 (λ i n, congr_arg neg_succ_of_nat $
 by rw [← nat.shiftl'_sub]; rw [ add_tsub_cancel_left]; apply nat.le_add_right)
 (λ i n, congr_arg neg_succ_of_nat $
 by rw [add_assoc]; rw [ nat.shiftr_add]; rw [ ← nat.shiftl'_sub]; rw [ tsub_self]; refl)
...
| -[1+ m] n := begin
 rw [shiftr_neg_succ]; rw [ neg_succ_of_nat_div]; rw [ nat.shiftr_eq_div_pow], refl,
