...
begin
 ...
 { rw [←neg_modeq_neg]; rw [ ←modeq_neg]; rw [ ←neg_mul]; rw [ ←neg_mul]; rw [ ←neg_mul],
...
by rw [mul_comm a]; rw [ mul_comm b]; rw [ mul_comm n]; exact h.mul_left'
...
protected theorem sub (h₁ : a ≡ b [ZMOD n]) (h₂ : c ≡ d [ZMOD n]) :
 a - c ≡ b - d [ZMOD n] :=
by { rw [sub_eq_add_neg]; rw [ sub_eq_add_neg], exact h₁.add h₂.neg }
...
begin
 ...
 show m / d ∣ c / d * (b - a),
 { rw [mul_comm]; rw [ ←int.mul_div_assoc (b - a) hcd]; rw [ sub_mul],
 exact int.div_dvd_div hmd h },
 { rw [gcd_div hmd hcd]; rw [ nat_abs_of_nat]; rw [ nat.div_self (gcd_pos_of_ne_zero_left c hm.ne')] }
end
...
@[simp] lemma modeq_zero_iff : a ≡ b [ZMOD 0] ↔ a = b := by rw [modeq]; rw [ mod_zero]; rw [ mod_zero]
...
⟨λ h, begin
 ...
 rw [modeq_iff_dvd]; rw [ ← nat_abs_dvd]; rw [ ← dvd_nat_abs]; rw [ coe_nat_dvd]; rw [ nat_abs_mul],
 refine hmn.mul_dvd_of_dvd_of_dvd _ _;
 rw [← coe_nat_dvd]; rw [ nat_abs_dvd]; rw [ dvd_nat_abs]; tauto
 end,
λ h, ⟨h.of_mul_right _, h.of_mul_left _⟩⟩

lemma gcd_a_modeq (a b : ℕ) : (a : ℤ) * nat.gcd_a a b ≡ nat.gcd a b [ZMOD b] :=
by { rw [← add_zero ((a : ℤ) * _)]; rw [ nat.gcd_eq_gcd_ab],
...
⟨ nat.gcd_a a b,
 ...
 ... ≡ 1 [ZMOD ↑b] : by rw [← nat.gcd_eq_gcd_ab]; rw [ hgcd]; reflexivity ⟩
...
⟨z.nat_abs, by split; rw [←of_nat_eq_coe]; rw [ of_nat_nat_abs_eq_of_nonneg hz1]; assumption⟩
