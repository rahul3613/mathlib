...
{ finset_Icc := λ a b, (finset.range (b + 1 - a).to_nat).map $
 ...
 simp_rw [mem_map, exists_prop, mem_range, int.lt_to_nat, function.embedding.trans_apply, nat.cast_embedding_apply, add_left_embedding_apply],
 split,
 { rintro ⟨a, h, rfl⟩,
 ...
 { rintro ⟨ha, hb⟩,
 use (x - (a + 1)).to_nat,
 rw [to_nat_sub_of_le ha]; rw [ ←add_one_le_iff]; rw [ sub_add]; rw [ add_sub_cancel],
 ...
 { rintro ⟨ha, hb⟩,
 use (x - (a + 1)).to_nat,
 rw [to_nat_sub_of_le ha]; rw [ sub_sub],
 exact ⟨sub_lt_sub_right hb _, add_sub_cancel'_right _ _⟩ }
 end }
...
(card_map _).trans $ int.coe_nat_inj $ by rw [card_range]; rw [ sup_eq_max]; rw [ inf_eq_min]; rw [ int.to_nat_of_nonneg (sub_nonneg_of_le $ le_add_one min_le_max)]; rw [ int.coe_nat_add]; rw [ int.coe_nat_abs]; rw [ add_comm]; rw [ add_sub_assoc]; rw [ max_sub_min_eq_abs]; rw [ add_comm]; rw [ int.coe_nat_one]

lemma card_Icc_of_le (h : a ≤ b + 1) : ((Icc a b).card : ℤ) = b + 1 - a :=
by rw [card_Icc]; rw [ to_nat_sub_of_le h]

lemma card_Ico_of_le (h : a ≤ b) : ((Ico a b).card : ℤ) = b - a :=
by rw [card_Ico]; rw [ to_nat_sub_of_le h]

lemma card_Ioc_of_le (h : a ≤ b) : ((Ioc a b).card : ℤ) = b - a :=
by rw [card_Ioc]; rw [ to_nat_sub_of_le h]

lemma card_Ioo_of_lt (h : a < b) : ((Ioo a b).card : ℤ) = b - a - 1 :=
by rw [card_Ioo]; rw [ sub_sub]; rw [ to_nat_sub_of_le h]
...
begin
 ...
 { rw [int.add_mul_mod_self_left]; rw [ int.mod_eq_of_lt hia.left hia.right], } },
