...
by rw [finrank_eq_card_basis basis_one_I]; rw [ fintype.card_fin]
...
cardinal.lift_inj.1 $
 by { rw [← lift_rank_mul_lift_rank ℝ ℂ E]; rw [ complex.rank_real_complex], simp [bit0] }
...
by rw [← finite_dimensional.finrank_mul_finrank ℝ ℂ E]; rw [ complex.finrank_real_complex]

@[priority 900]
instance star_module.complex_to_real {E : Type*} [add_comm_group E] [has_star E] [module ℂ E]
 [star_module ℂ E] : star_module ℝ E :=
⟨λ r a, by rw [←smul_one_smul ℂ r a]; rw [ star_smul]; rw [ star_smul]; rw [ star_one]; rw [ smul_one_smul]⟩
...
begin
 refine (eq_or_eq_neg_of_sq_eq_sq (f I) I $ by rw [← map_pow]; rw [ I_sq]; rw [ map_neg]; rw [ map_one]).imp _ _;
...
alg_hom.of_linear_map
 ...
 by rw [ring_hom.map_one]; rw [ zero_smul]; rw [ add_zero])
 ...
 { rw [smul_mul_smul]; rw [ hf]; rw [ smul_neg]; rw [ ←algebra.algebra_map_eq_smul_one]; rw [ ←sub_eq_add_neg]; rw [ ←ring_hom.map_mul]; rw [ ←ring_hom.map_sub], },
 { rw [algebra.smul_def]; rw [ algebra.smul_def]; rw [ algebra.smul_def]; rw [ ←algebra.right_comm _ x₂]; rw [ ←mul_assoc]; rw [ ←add_mul]; rw [ ←ring_hom.map_mul]; rw [ ←ring_hom.map_mul]; rw [ ←ring_hom.map_add] }
 end)
...
{ to_fun := λ I', lift_aux I' I'.prop,
 inv_fun := λ F, ⟨F I, by rw [←F.map_mul]; rw [ I_mul_I]; rw [ alg_hom.map_neg]; rw [ alg_hom.map_one]⟩,
