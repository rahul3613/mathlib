...
by rw [pow_bit0']; rw [ I_mul_I]

@[simp] lemma I_pow_bit1 (n : ℕ) : I ^ (bit1 n) = (-1) ^ n * I :=
by rw [pow_bit1']; rw [ I_mul_I]
...
by rw [← mk_eq_add_mul_I]; rw [ norm_sq_mk]; rw [ sq]; rw [ sq]
...
subset.antisymm (range_subset_iff.2 norm_sq_nonneg) $ λ x hx,
 ⟨real.sqrt x, by rw [norm_sq_of_real]; rw [ real.mul_self_sqrt hx]⟩
...
@[simp] lemma I_sq : I ^ 2 = -1 := by rw [sq]; rw [ I_mul_I]
...
lemma norm_sq_sub (z w : ℂ) : norm_sq (z - w) =
 norm_sq z + norm_sq w - 2 * (z * conj w).re :=
by { rw [sub_eq_add_neg]; rw [ norm_sq_add],
...
by rw [← of_real_zero]; rw [ ← of_real_inv]; rw [ inv_zero]

protected theorem mul_inv_cancel {z : ℂ} (h : z ≠ 0) : z * z⁻¹ = 1 :=
by rw [inv_def]; rw [ ← mul_assoc]; rw [ mul_conj]; rw [ ← of_real_mul]; rw [ mul_inv_cancel (mt norm_sq_eq_zero.1 h)]; rw [ of_real_one]
...
by rw [zpow_bit0']; rw [ I_mul_I]

@[simp] lemma I_zpow_bit1 (n : ℤ) : I ^ (bit1 n) = (-1) ^ n * I :=
by rw [zpow_bit1']; rw [ I_mul_I]
...
by rwa [← of_real_nat_cast] at h; rwa [ of_real_eq_zero] at h; rwa [ nat.cast_eq_zero] at h
...
by rw [norm_sq_mul]; rw [ real.sqrt_mul (norm_sq_nonneg _)]
...
begin
 rw [mul_self_abs]; rw [ add_mul_self_eq]; rw [ mul_self_abs]; rw [ mul_self_abs]; rw [ add_right_comm]; rw [ norm_sq_add]; rw [ add_le_add_iff_left]; rw [ mul_assoc]; rw [ mul_le_mul_left (zero_lt_two' ℝ)]; rw [ ←real.sqrt_mul $ norm_sq_nonneg z]; rw [ ←norm_sq_conj w]; rw [ ←map_mul],
...
by rw [sq_abs]; rw [ norm_sq_apply]; rw [ ← sq]; rw [ ← sq]; rw [ add_sub_cancel']

@[simp] lemma sq_abs_sub_sq_im (z : ℂ) : abs z ^ 2 - z.im ^ 2 = z.re ^ 2 :=
by rw [← sq_abs_sub_sq_re]; rw [ sub_sub_cancel]

@[simp] lemma abs_I : abs I = 1 := by simp [abs]

@[simp] lemma abs_two : abs 2 = 2 :=
calc abs 2 = abs (2 : ℝ) : by rw [of_real_bit0]; rw [ of_real_one]
...
real.abs_le_sqrt $ by { rw [norm_sq_apply]; rw [ ← sq], exact le_add_of_nonneg_right (mul_self_nonneg _) }

lemma abs_im_le_abs (z : ℂ) : |z.im| ≤ abs z :=
real.abs_le_sqrt $ by { rw [norm_sq_apply]; rw [ ← sq]; rw [ ← sq], exact le_add_of_nonneg_left (sq_nonneg _) }
...
by rw [abs]; rw [ absolute_value.coe_mk]; rw [ mul_hom.coe_mk]; rw [ real.lt_sqrt (abs_nonneg _)]; rw [ norm_sq_apply]; rw [ _root_.sq_abs]; rw [ ← sq]; rw [ lt_add_iff_pos_right]; rw [ mul_self_pos]
...
begin
 ...
 wlog hle : |x| ≤ |y|,
 { rw [add_comm]; rw [ max_comm], exact this _ _ (le_of_not_le hle), },
 calc real.sqrt (x ^ 2 + y ^ 2) ≤ real.sqrt (y ^ 2 + y ^ 2) :
 real.sqrt_le_sqrt (add_le_add_right (sq_le_sq.2 hle) _)
 ... = real.sqrt 2 * max (|x|) (|y|) :
 by rw [max_eq_right hle]; rw [ ← two_mul]; rw [ real.sqrt_mul two_pos.le]; rw [ real.sqrt_sq_eq_abs],
end
...
by rw [← of_real_nat_cast]; rw [ abs_of_nonneg (nat.cast_nonneg n)]

@[simp, norm_cast] lemma int_cast_abs (n : ℤ) : ↑|n| = abs n :=
by rw [← of_real_int_cast]; rw [ abs_of_real]; rw [ int.cast_abs]
...
by rw [le_def]; rw [ not_and_distrib]; rw [ not_le]

lemma not_lt_iff {z w : ℂ} : ¬(z < w) ↔ w.re ≤ z.re ∨ z.im ≠ w.im :=
by rw [lt_def]; rw [ not_and_distrib]; rw [ not_lt]
...
⟨i, λ j hj, by rw [← ring_hom.map_sub]; rw [ abs_conj]; exact hi j hj⟩
