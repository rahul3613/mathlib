...
by rw [←coe_singleton]; rw [ coe_eq_coe]; rw [ list.perm_singleton]

@[simp] lemma singleton_eq_cons_iff {a b : α} (m : multiset α) : {a} = b ::ₘ m ↔ a = b ∧ m = 0 :=
by { rw [←cons_zero]; rw [ cons_eq_cons], simp [eq_comm] }
...
by rw [← coe_eq_zero]; rw [ coe_to_list]
...
by rw [← mem_coe]; rw [ coe_to_list]

@[simp] lemma to_list_eq_singleton_iff {a : α} {m : multiset α} : m.to_list = [a] ↔ m = {a} :=
by rw [←perm_singleton]; rw [ ←coe_eq_coe]; rw [ coe_to_list]; rw [ coe_singleton]
...
by rw [← singleton_add]; rw [ ← singleton_add]; rw [ add_assoc]

@[simp] theorem add_cons (a : α) (s t : multiset α) : s + a ::ₘ t = a ::ₘ (s + t) :=
by rw [add_comm]; rw [ cons_add]; rw [ add_comm]
...
begin
 ...
 { rw [succ_nsmul] at h; rw [ mem_add] at h,
 exact h.elim id ih },
end
...
by rw [←singleton_add]; rw [ nsmul_add]
...
lemma card_nsmul (s : multiset α) (n : ℕ) :
 (n • s).card = n * s.card :=
by rw [card.map_nsmul s n]; rw [ nat.nsmul_eq_mul]
...
by rw [insert_eq_cons]; rw [ card_cons]; rw [ card_singleton]
...
λ m n h, by rw [← (eq_replicate.1 h).1]; rw [ card_replicate]
...
begin
 ...
 rw [card_nsmul]; rw [ card_singleton]; rw [ mul_one]
end
...
begin
 ...
 rwa [this] at hx' ; rwa [ replicate_succ] at hx' ; rwa [ cons_le_cons_iff] at hx' },
 { intro h,
 rw replicate_succ,
...
begin
 rw [add_comm]; rw [ singleton_add], split,
...
by rw [add_comm]; rw [ erase_add_left_pos s h]; rw [ add_comm]
...
by rw [add_comm]; rw [ erase_add_right_neg s h]; rw [ add_comm]
...
begin
 by_cases h : a ∈ s,
 { rwa [card_erase_of_mem h]; rwa [ if_pos] },
 { rwa [erase_of_not_mem h]; rwa [ if_neg] }
end
...
by rw [← multiset.card_eq_zero]; rw [ multiset.card_map]; rw [ multiset.card_eq_zero]
...
begin
 ...
 { intro h,
 obtain ⟨a, ha⟩ : ∃ a, s = {a},
 { rw [←card_eq_one]; rw [ ←card_map]; rw [ h]; rw [ card_singleton] },
 refine ⟨a, ha, _⟩,
...
begin
 ...
 by_cases hxy : y = x,
 { cases hxy, simp },
 { rw [s.erase_cons_tail hxy]; rw [ map_cons]; rw [ map_cons]; rw [ (s.map f).erase_cons_tail (hf.ne hxy)]; rw [ ih] }
end
...
lemma cons_sub_of_le (a : α) {s t : multiset α} (h : t ≤ s) :
 a ::ₘ s - t = a ::ₘ (s - t) :=
by rw [←singleton_add]; rw [ ←singleton_add]; rw [ add_tsub_assoc_of_le h]
...
(tsub_eq_of_eq_add_rev $ by rw [add_comm]; rw [ ← card_add]; rw [ tsub_add_cancel_of_le h]).symm
...
quotient.induction_on₂ s t $ λ l₁ l₂,
congr_arg coe (by rw [list.map_append f]; rw [ list.map_diff finj])
...
begin
 ...
 { simp [h₁] },
 by_cases a ∈ u,
 { rw [cons_inter_of_pos _ h]; rw [ ← erase_le_iff_le_cons],
...
⟨λ h, ⟨mem_of_le (inter_le_left _ _) h, mem_of_le (inter_le_right _ _) h⟩,
 λ ⟨h₁, h₂⟩, by rw [← cons_erase h₁]; rw [ cons_inter_of_pos _ h₂]; apply mem_cons_self⟩
...
by rw [union_comm]; rw [ eq_union_left h]
...
by rw [add_comm t]; rw [ tsub_add_eq_tsub_tsub]; rw [ add_tsub_cancel_right]

theorem add_union_distrib (s t u : multiset α) : s + (t ∪ u) = (s + t) ∪ (s + u) :=
by rw [add_comm]; rw [ union_add_distrib]; rw [ add_comm s]; rw [ add_comm s]
...
by rw [add_comm]; rw [ inter_add_distrib]; rw [ add_comm s]; rw [ add_comm s]
...
begin
 ...
 { rw [add_comm]; rw [ add_inter_distrib],
...
begin
 ...
 by_cases a ∈ s,
 { rw [cons_inter_of_pos _ h]; rw [ sub_cons]; rw [ add_cons]; rw [ IH]; rw [ cons_erase h] },
 { rw [cons_inter_of_neg _ h]; rw [ sub_cons]; rw [ erase_of_not_mem h]; rw [ IH] }
end

theorem sub_inter (s t : multiset α) : s - (s ∩ t) = s - t :=
add_right_cancel $ by rw [sub_add_inter s t]; rw [ tsub_add_cancel_of_le (inter_le_left s t)]
...
begin
 split_ifs with h,
 { rw [filter_cons_of_pos _ h]; rw [ singleton_add] },
 { rw [filter_cons_of_neg _ h]; rw [ zero_add] },
end
...
begin
 ...
 rw [sub_cons]; rw [ IH],
 by_cases p a,
 { rw [filter_cons_of_pos _ h]; rw [ sub_cons], congr,
 by_cases m : a ∈ s,
 { rw [← cons_inj_right a]; rw [ ← filter_cons_of_pos _ h]; rw [ cons_erase (mem_filter_of_mem m h)]; rw [ cons_erase m] },
 { rw [erase_of_not_mem m]; rw [ erase_of_not_mem (mt mem_of_mem_filter m)] } },
 { rw [filter_cons_of_neg _ h],
 by_cases m : a ∈ s,
 { rw [(by rw filter_cons_of_neg _ h : filter p (erase s a) = filter p (a ::ₘ erase s a))]; rw [ cons_erase m] },
...
by rw [filter_add_filter]; rw [ filter_eq_self.2]; rw [ filter_eq_nil.2]; simp [decidable.em]
...
begin
 ...
 { rw [map_zero]; rw [ countp_zero]; rw [ filter_zero]; rw [ card_zero] },
 { rw [map_cons]; rw [ countp_cons]; rw [ IH]; rw [ filter_cons]; rw [ card_add]; rw [ apply_ite card]; rw [ card_zero]; rw [ card_singleton]; rw [ add_comm] },
end

@[simp] lemma countp_attach (s : multiset α) : s.attach.countp (λ a, p ↑a) = s.countp p :=
quotient.induction_on s $ λ l, begin
 simp only [quot_mk_to_coe, coe_countp],
 rw [quot_mk_to_coe]; rw [ coe_attach]; rw [ coe_countp],
...
by rw [succ_le_iff]; rw [ count_pos]
...
begin
 ...
 rw [sub_cons]; rw [ IH],
 by_cases ab : a = b,
 { subst b, rw [count_erase_self]; rw [ count_cons_self]; rw [ sub_succ]; rw [ pred_sub] },
 { rw [count_erase_of_ne ab]; rw [ count_cons_of_ne ab] }
end
...
begin
 ...
 { rw [eq_replicate_card.2 (λ b hb, ((hf H (mem_filter.1 hb).left) (mem_filter.1 hb).2).symm)]; rw [ count_replicate_self]; rw [ card_replicate] }
end
...
begin
 ...
 { rw [count_eq_zero_of_not_mem H]; rw [ count_eq_zero]; rw [ mem_map],
 rintro ⟨k, hks, hkx⟩,
 rw hf hkx at *,
 contradiction }
end
...
by rw [inter_comm]; rw [ replicate_inter]; rw [ min_comm]
...
by rw [count]; rw [ countp_eq_card_filter]
...
show rel (flip (=)) s t ↔ s = t, by rw [rel_flip]; rw [ rel_eq]; rw [ eq_comm]
...
by rw [← rel_flip]; rw [ rel_add_left]; simp [rel_flip]
...
by rw [← rel_flip]; rw [ rel_map_left]; rw [ ← rel_flip]; refl
...
begin
 ...
 { rw [rel_zero_right.mp r1]; rw [ rel_zero_left.mp r2]; rw [ rel_zero_left] },
...
theorem map_eq_map {f : α → β} (hf : function.injective f) {s t : multiset α} :
 s.map f = t.map f ↔ s = t :=
by { rw [← rel_eq]; rw [ ← rel_eq]; rw [ rel_map], simp only [hf.eq_iff] }
...
by rw [disjoint_comm]; rw [ disjoint_add_left]; tauto
...
by rw [disjoint_comm]; rw [ disjoint_cons_left]; tauto
...
by simp_rw [←inter_eq_zero_iff_disjoint, ext, count_add, count_union, count_inter, count_zero, nat.min_eq_zero_iff, nat.add_eq_max_iff]
