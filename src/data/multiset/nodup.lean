...
nodup_iff_le.trans
 ...
 h a t (by rwa [cons_add] at s_eq )⟩; rwa [ cons_add] at s_eq )⟩; rwa [ zero_add] at s_eq )⟩
...
lemma nodup.mem_erase_iff [decidable_eq α] {a b : α} {l} (d : nodup l) :
 a ∈ l.erase b ↔ a ≠ b ∧ a ∈ l :=
by rw [d.erase_eq_filter b]; rw [ mem_filter]; rw [ and_comm]
...
⟨λ h, ⟨mem_of_le tsub_le_self h, λ h',
 by refine count_eq_zero.1 _ h; rw [count_sub a s t]; rw [ tsub_eq_zero_iff_le];
...
... = t.map g : by rw [this]; rw [ multiset.map_map]; exact map_congr rfl (λ x _, h _ _)
