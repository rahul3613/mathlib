...
by rw [list.bind]; rw [ ←coe_join]; rw [ list.map_map]; refl
...
multiset.induction_on s (by rw [zero_bind]; rw [ map_zero]) (by simp [singleton_add])
...
by { rw [product]; rw [ list.product]; rw [ ←coe_bind], simp }
...
multiset.induction_on s (λ t u, rfl) $ λ a s IH t u,
 by rw [cons_product]; rw [ IH]; simp; cc
...
by rw [multiset.sigma]; rw [ list.sigma]; rw [ ←coe_bind]; simp
...
multiset.induction_on s (λ t u, rfl) $ λ a s IH t u,
 by rw [cons_sigma]; rw [ IH]; simp; cc
