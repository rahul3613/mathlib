...
begin
 rw [revzip] at h; rw [ powerset_aux_eq_map_coe] at h; rw [ ← map_reverse] at h; rw [ zip_map] at h; rw [ ← revzip] at h,
...
theorem revzip_powerset_aux' {l : list α} ⦃x⦄
 (h : x ∈ revzip (powerset_aux' l)) : x.1 + x.2 = ↑l :=
begin
 rw [revzip] at h; rw [ powerset_aux'] at h; rw [ ← map_reverse] at h; rw [ zip_map] at h; rw [ ← revzip] at h,
 simp at h, rcases h with ⟨l₁, l₂, h, rfl, rfl⟩,
 exact quot.sound (revzip_sublists' _ _ _ h)
end
...
begin
 ...
 rintro ⟨s, t⟩ h,
 dsimp, rw [← H h]; rw [ add_tsub_cancel_left] },
 rw [← forall₂_eq_eq_eq]; rw [ forall₂_map_right_iff], simpa
end
...
begin
 ...
 rw [revzip_powerset_aux_lemma l revzip_powerset_aux]; rw [ revzip_powerset_aux_lemma l revzip_powerset_aux'],
...
by rw [powerset_len_aux]; rw [ sublists_len_aux_eq]; rw [ append_nil]
