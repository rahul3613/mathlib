...
by rw [fold_eq_foldl]; rw [ foldl_cons]; rw [ ← fold_eq_foldl]

theorem fold_cons'_left (b a : α) (s : multiset α) : (a ::ₘ s).fold op b = s.fold op (a * b) :=
by rw [fold_cons'_right]; rw [ hc.comm]

theorem fold_add (b₁ b₂ : α) (s₁ s₂ : multiset α) :
 (s₁ + s₂).fold op (b₁ * b₂) = s₁.fold op b₁ * s₂.fold op b₂ :=
multiset.induction_on s₂
 (by rw [add_zero]; rw [ fold_zero]; rw [ ← fold_cons'_right]; rw [ ← fold_cons_right op])
...
begin
 ...
 { rw [zero_bind]; rw [ map_zero]; rw [ map_zero]; rw [ fold_zero] },
 { rw [cons_bind]; rw [ map_cons]; rw [ map_cons]; rw [ fold_cons_left]; rw [ fold_cons_left]; rw [ fold_add]; rw [ ih] },
end
...
by rw [← fold_add op]; rw [ union_add_inter]; rw [ fold_add op]
