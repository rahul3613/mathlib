...
lemma real_smul_eq_coe_smul [add_comm_group E] [module K E] [module ℝ E] [is_scalar_tower ℝ K E]
 (r : ℝ) (x : E) : r • x = (r : K) • x :=
by rw [is_R_or_C.of_real_alg]; rw [ smul_one_smul]
...
@[simp, is_R_or_C_simps] lemma one_re : re (1 : K) = 1 := by rw [← of_real_one]; rw [ of_real_re]
@[simp, is_R_or_C_simps] lemma one_im : im (1 : K) = 0 := by rw [← of_real_one]; rw [ of_real_im]
...
by rw [real_smul_eq_coe_mul]; rw [ of_real_mul_re]

@[is_R_or_C_simps] lemma smul_im (r : ℝ) (z : K) : im (r • z) = r * (im z) :=
by rw [real_smul_eq_coe_mul]; rw [ of_real_mul_im]
...
by rw [map_neg]; rw [ conj_I]; rw [ neg_neg]

lemma conj_eq_re_sub_im (z : K) : conj z = re z - im z * I :=
(congr_arg conj (re_add_im z).symm).trans $ by rw [map_add]; rw [ map_mul]; rw [ conj_I]; rw [ conj_of_real]; rw [ conj_of_real]; rw [ mul_neg]; rw [ sub_eq_add_neg]

theorem sub_conj (z : K) : z - conj z = 2 * im z * I :=
begin
 nth_rewrite 0 [← re_add_im z],
 rw [conj_eq_re_sub_im]; rw [ add_sub_sub_cancel]; rw [ ← two_mul]; rw [ mul_assoc]
end

@[is_R_or_C_simps] lemma conj_smul (r : ℝ) (z : K) : conj (r • z) = r • conj z :=
by rw [conj_eq_re_sub_im]; rw [ conj_eq_re_sub_im]; rw [ smul_re]; rw [ smul_im]; rw [ of_real_mul]; rw [ of_real_mul]; rw [ real_smul_eq_coe_mul]; rw [ mul_sub]; rw [ mul_assoc]

theorem add_conj (z : K) : z + conj z = 2 * re z :=
calc z + conj z = re z + im z * I + (re z - im z * I) : by rw [re_add_im]; rw [ conj_eq_re_sub_im]
... = 2 * re z : by rw [add_add_sub_cancel]; rw [ two_mul]

theorem re_eq_add_conj (z : K) : ↑(re z) = (z + conj z) / 2 :=
by rw [add_conj]; rw [ mul_div_cancel_left ((re z):K) two_ne_zero]

theorem im_eq_conj_sub (z : K) : ↑(im z) = I * (conj z - z) / 2 :=
by rw [← neg_inj]; rw [ ← of_real_neg]; rw [ ← I_mul_re]; rw [ re_eq_add_conj]; rw [ map_mul]; rw [ conj_I]; rw [ ← neg_div]; rw [ ← mul_neg]; rw [ neg_sub]; rw [ mul_sub]; rw [ neg_mul]; rw [ sub_eq_add_neg]
...
begin
 tfae_have : 1 → 4,
 { intro h,
 rw [← @of_real_inj K]; rw [ im_eq_conj_sub]; rw [ h]; rw [ sub_self]; rw [ mul_zero]; rw [ zero_div]; rw [ of_real_zero] },
 tfae_have : 4 → 3,
 { intro h,
 conv_rhs { rw [← re_add_im z]; rw [ h]; rw [ of_real_zero]; rw [ zero_mul]; rw [ add_zero] } },
...
by rw [lt_iff_le_and_ne]; rw [ ne]; rw [ eq_comm]; simp [norm_sq_nonneg]
...
lemma conj_mul (x : K) : conj x * x = ((norm_sq x) : K) := by rw [mul_comm]; rw [ mul_conj]
...
by rw [norm_sq_eq_def']; rw [ real.sqrt_sq (norm_nonneg _)]
...
begin
 ...
 rw [← mul_assoc]; rw [ mul_conj]; rw [ of_real_inv]; rw [ ← norm_sq_eq_def']; rw [ mul_inv_cancel],
 rwa [of_real_ne_zero]; rwa [ ne.def]; rwa [ norm_sq_eq_zero] }
end

@[simp, is_R_or_C_simps] lemma inv_re (z : K) : re (z⁻¹) = re z / norm_sq z :=
by rw [inv_def]; rw [ norm_sq_eq_def']; rw [ mul_comm]; rw [ of_real_mul_re]; rw [ conj_re]; rw [ div_eq_inv_mul]

@[simp, is_R_or_C_simps] lemma inv_im (z : K) : im (z⁻¹) = -im z / norm_sq z :=
by rw [inv_def]; rw [ norm_sq_eq_def']; rw [ mul_comm]; rw [ of_real_mul_im]; rw [ conj_im]; rw [ div_eq_inv_mul]
...
by rw [div_eq_inv_mul]; rw [ div_eq_inv_mul]; rw [ ← of_real_inv]; rw [ of_real_mul_re]
...
by rw [div_eq_mul_inv]; rw [ inv_I]; rw [ mul_neg]
...
by rw [← of_real_nat_cast]; rw [ of_real_re]

@[simp, is_R_or_C_simps, norm_cast] lemma nat_cast_im (n : ℕ) : im (n : K) = 0 :=
by rw [← of_real_nat_cast]; rw [ of_real_im]
...
by rw [← of_real_int_cast]; rw [ of_real_re]

@[simp, is_R_or_C_simps, norm_cast] lemma int_cast_im (n : ℤ) : im (n : K) = 0 :=
by rw [← of_real_int_cast]; rw [ of_real_im]
...
by rw [← of_real_rat_cast]; rw [ of_real_re]

@[simp, is_R_or_C_simps, norm_cast] lemma rat_cast_im (q : ℚ) : im (q : K) = 0 :=
by rw [← of_real_rat_cast]; rw [ of_real_im]
...
by rw [← nat.cast_two]; rw [ norm_nat_cast]; rw [ nat.cast_two]

lemma abs_re_le_norm (z : K) : |re z| ≤ ‖z‖ :=
by rw [mul_self_le_mul_self_iff (_root_.abs_nonneg (re z)) (norm_nonneg _)]; rw [ abs_mul_abs_self]; rw [ mul_self_norm];
 apply re_sq_le_norm_sq

lemma abs_im_le_norm (z : K) : |im z| ≤ ‖z‖ :=
by rw [mul_self_le_mul_self_iff (_root_.abs_nonneg (im z)) (norm_nonneg _)]; rw [ abs_mul_abs_self]; rw [ mul_self_norm];
...
by rw [(is_real_tfae a).out 2 3]; rw [ im_eq_zero_of_le h]
...
by rw [mul_conj]; rw [ of_real_re]; rw [ norm_of_real]; rw [ abs_of_nonneg (norm_sq_nonneg _)]

lemma norm_sq_re_add_conj (x : K) : (‖x + conj x‖)^2 = (re (x + conj x))^2 :=
by rw [add_conj]; rw [ norm_mul]; rw [ norm_two]; rw [ norm_of_real]; rw [ two_mul (re x : K)]; rw [ map_add]; rw [ of_real_re]; rw [ ← two_mul]; rw [ mul_pow]; rw [ mul_pow]; rw [ sq_abs]

lemma norm_sq_re_conj_add (x : K) : (‖conj x + x‖)^2 = (re (conj x + x))^2 :=
by rw [add_comm]; rw [ norm_sq_re_add_conj]
