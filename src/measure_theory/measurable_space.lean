...
begin
 ...
 rintros (-|-),
 { convert h.compl,
 rw [← preimage_compl]; rw [ bool.compl_singleton]; rw [ bool.bnot_tt] },
 exact h,
end
...
| ⟨u, (hu : measurable_set u), (eq : coe ⁻¹' u = t)⟩ :=
 begin
 rw [← eq]; rw [ subtype.image_preimage_coe],
 exact hu.inter hs
 end
...
measurable.of_le_map $ sup_le
 (by { rw [measurable_space.comap_le_iff_le_map]; rw [ measurable_space.map_comp], exact hf₁ })
 (by { rw [measurable_space.comap_le_iff_le_map]; rw [ measurable_space.map_comp], exact hf₂ })
...
by simp_rw [measurable_iff_comap_le, measurable_space.pi, measurable_space.comap_supr, measurable_space.comap_comp, function.comp, supr_le_iff]
...
⟨hg.injective.comp hf.injective, hg.measurable.comp hf.measurable,
 λ s hs, by rwa [← image_image]; rwa [ hg.measurable_set_image]; rwa [ hf.measurable_set_image]⟩
...
by rw [← image_preimage_eq_inter_range]; rw [ hf.measurable_set_image]

lemma measurable_range_splitting (hf : measurable_embedding f) :
 measurable (range_splitting f) :=
λ s hs, by rwa [preimage_range_splitting hf.injective]; rwa [ ← (subtype_coe hf.measurable_set_range).measurable_set_image]; rwa [ ← image_comp]; rwa [ coe_comp_range_factorization]; rwa [ hf.measurable_set_image]
...
@[simp] theorem measurable_set_image (e : α ≃ᵐ β) {s : set α} :
 measurable_set (e '' s) ↔ measurable_set s :=
by rw [image_eq_preimage]; rw [ measurable_set_preimage]
...
iff.intro
 ...
 by rwa [coe_to_equiv] at this); rwa [ symm_trans_self] at this)
...
begin
 ...
 rw [← generate_from_insert_empty] at hb; rw [ he] at hb,
