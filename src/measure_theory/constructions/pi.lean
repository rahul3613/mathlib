...
by rw [← funext hC]; rw [ generate_from_pi_eq h2C]
...
begin
 rw [outer_measure.pi]; rw [ le_bounded_by'], split,
...
by rw [pi']; rw [ ← measurable_equiv.pi_measurable_equiv_tprod_symm_apply]; rw [ measurable_equiv.map_apply]; rw [ measurable_equiv.pi_measurable_equiv_tprod_symm_apply]; rw [ elim_preimage_pi]; rw [ tprod_tprod _ μ]; rw [ ← list.prod_to_finset]; rw [ sorted_univ_to_finset]; exact sorted_univ_nodup ι
...
begin
 ...
 { rw [measure.pi]; rw [ to_measure_apply _ _ (measurable_set.pi countable_univ (λ i _, hs i))],
 apply outer_measure.pi_pi_le },
 { haveI : encodable ι := fintype.to_encodable ι,
 rw [← pi'_pi μ s],
 simp_rw [← pi'_pi μ s, measure.pi, to_measure_apply _ _ (measurable_set.pi countable_univ (λ i _, hs i)), ← to_outer_measure_apply],
...
begin
 ...
 { simp_rw [(surjective_decode_iget (ι → ℕ)).Union_comp (λ x, pi univ (λ i, (hμ i).set (x i))), Union_univ_pi (λ i, (hμ i).set), (hμ _).spanning, set.pi_univ] }
end
...
begin
 ...
 rw [map_apply (measurable_const_mul _) (measurable_set.univ_pi hs)]; rw [ (show (*) v ⁻¹' univ.pi s = univ.pi (λ i, (*) (v i) ⁻¹' s i), by refl)]; rw [ pi_pi],
...
begin
 refine ⟨λ v, (pi_eq $ λ s hs, _).symm⟩,
 rw [map_apply (measurable_mul_const _) (measurable_set.univ_pi hs)]; rw [ (show (* v) ⁻¹' univ.pi s = univ.pi (λ i, (* v i) ⁻¹' s i), by refl)]; rw [ pi_pi],
...
begin
 ...
 { ext, simp },
 simp_rw [measure.inv, measure.map_apply measurable_inv (measurable_set.univ_pi hs), A, pi_pi, measure_preimage_inv]
end
...
begin
 ...
 refine measure_preserving.symm e _,
 refine ⟨e.measurable, (pi_eq $ λ s hs, _).symm⟩,
 rw [e.map_apply]; rw [ i.prod_univ_succ_above _]; rw [ ← pi_pi]; rw [ ← prod_prod],
 congr' 1 with ⟨x, f⟩,
...
begin
 ...
 { ext1 s,
 rw [pi_premeasure]; rw [ fintype.prod_unique]; rw [ to_outer_measure_apply]; rw [ e.symm.map_apply],
...
begin
 ...
 rw [measurable_equiv.map_apply]; rw [ measurable_equiv.pi_fin_two_apply]; rw [ fin.preimage_apply_01_prod]; rw [ measure.pi_pi]; rw [ fin.prod_univ_two],
 refl
end
...
begin
 ...
 rw [measure.pi_of_empty]; rw [ measure.map_dirac e.measurable], refl
end
