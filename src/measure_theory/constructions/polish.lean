...
begin
 ...
 resetI,
 have : f '' s = range (f ∘ g), by rw [range_comp]; rw [ gs],
 rw this,
...
begin
 ...
 contrapose!,
 assume H,
 rw [← Union_cylinder_update x n]; rw [ ← Union_cylinder_update y n]; rw [ image_Union]; rw [ image_Union],
 ...
 { rw [pn_fst]; rw [ ← mem_cylinder_iff_eq]; rw [ mem_cylinder_iff],
 assume i hi,
 rw hx,
...
begin
 rw [hf.measurable_set_preimage_iff_preimage_coe]; rw [ ← (measurable_embedding.subtype_coe hr).measurable_set_image]; rw [ subtype.image_preimage_coe]
end
...
begin
 ...
 simp_rw [and_iff_right (hl.map _), filter.has_basis.le_basis_iff (this _).to_has_basis metric.uniformity_basis_dist_inv_nat_succ, set.set_of_forall],
...
begin
 ...
 (by rwa [← countable_coe_iff]; rwa [ (equiv.set.univ _).countable_iff]),
 obtain ⟨g, gmeas, ginj⟩ :=
