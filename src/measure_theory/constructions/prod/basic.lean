...
begin
 ...
 rw [Union_unpair_prod]; rw [ h2s]; rw [ h2t]; rw [ univ_prod_univ]
end
...
by rw [← hC]; rw [ ← hD]; rw [ generate_from_prod_eq h2C h2D]
...
begin
 ...
 by simp_rw [prod_apply hSTm, mk_preimage_prod_right_eq_if, measure_if, lintegral_indicator _ (measurable_set_to_measurable _ _), lintegral_const, restrict_apply_univ, mul_comm]
 ... = μ s * ν t : by rw [measure_to_measurable]; rw [ measure_to_measurable] },
 ...
 ... = ∫⁻ x in s', ν t ∂μ : by rw [set_lintegral_const]; rw [ mul_comm]
...
⟨by rw [← univ_prod_univ]; rw [ prod_prod]; rw [ measure_univ]; rw [ measure_univ]; rw [ mul_one]⟩
...
by { rw [← prod_swap]; rw [ map_apply measurable_swap hs],
...
begin
 ...
 simp_rw [map_apply (measurable_equiv.measurable _) (hs.prod (ht.prod hu)), measurable_equiv.prod_assoc, measurable_equiv.coe_mk, equiv.prod_assoc_preimage, prod_prod, mul_assoc]
end
...
begin
 ...
 rwa [this]; rwa [ measure.prod_restrict]; rwa [ ← this],
end

lemma prod_dirac (y : β) : μ.prod (dirac y) = map (λ x, (x, y)) μ :=
begin
 refine prod_eq (λ s t hs ht, _),
 simp_rw [map_apply measurable_prod_mk_right (hs.prod ht), mk_preimage_prod_left_eq_if, measure_if, dirac_apply' _ ht, ← indicator_mul_right _ (λ x, μ s), pi.one_apply, mul_one]
end
...
by rw [prod_dirac]; rw [ map_dirac measurable_prod_mk_right]
...
begin
 ...
 { rw [← hf.map_eq]; rw [ zero_prod]; rw [ measure.map_zero]; rw [ zero_prod],
...
begin
 refine ⟨hf, _⟩,
 refine absolutely_continuous.mk (λ s hs h2s, _),
 simp_rw [map_apply hf hs, prod_apply (hf hs), preimage_preimage, lintegral_congr_ae (h2f.mono (λ x hx, hx.preimage_null h2s)), lintegral_zero],
end
...
by { rw ← prod_swap at hf, rw [← lintegral_map' hf measurable_swap.ae_measurable]; rw [ prod_swap] }
...
by rw [fst]; rw [ measure.map_apply measurable_fst hs]

lemma fst_univ : ρ.fst univ = ρ univ :=
by rw [fst_apply measurable_set.univ]; rw [ preimage_univ]
...
begin
 ext1 s hs,
 rw [measure.fst_apply hs]; rw [ measure.map_apply_of_ae_measurable (hX.prod_mk hY) (measurable_fst hs)]; rw [ measure.map_apply_of_ae_measurable hX hs]; rw [ ← prod_univ]; rw [ mk_preimage_prod]; rw [ preimage_univ]; rw [ inter_univ],
end
...
by rw [snd]; rw [ measure.map_apply measurable_snd hs]

lemma snd_univ : ρ.snd univ = ρ univ :=
by rw [snd_apply measurable_set.univ]; rw [ preimage_univ]
...
begin
 ext1 s hs,
 rw [measure.snd_apply hs]; rw [ measure.map_apply_of_ae_measurable (hX.prod_mk hY) (measurable_snd hs)]; rw [ measure.map_apply_of_ae_measurable hY hs]; rw [ ← univ_prod]; rw [ mk_preimage_prod]; rw [ preimage_univ]; rw [ univ_inter],
end
