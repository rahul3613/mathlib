...
lemma has_finite_integral_prod_iff' ⦃f : α × β → E⦄ (h1f : ae_strongly_measurable f (μ.prod ν)) :
 ...
 has_finite_integral (λ x, ∫ y, ‖f (x, y)‖ ∂ν) μ :=
begin
 rw [has_finite_integral_congr h1f.ae_eq_mk]; rw [ has_finite_integral_prod_iff h1f.strongly_measurable_mk],
 apply and_congr,
...
begin
 rw ← prod_swap at hf,
 rw [← integral_map measurable_swap.ae_measurable hf]; rw [ prod_swap]
end
...
begin
 ...
 simp_rw [← lintegral_fn_integral_sub (λ x, (‖x‖₊ : ℝ≥0∞)) (L1.integrable_coe_fn _) (L1.integrable_coe_fn g)],
...
begin
 ...
 { intros c s hs h2s,
 simp_rw [integral_indicator hs, ← indicator_comp_right, function.comp, integral_indicator (measurable_prod_mk_left hs), set_integral_const, integral_smul_const, integral_to_real (measurable_measure_prod_mk_left hs).ae_measurable (ae_measure_lt_top hs h2s.ne), prod_apply hs] },
