...
begin
 ...
 intros x hx,
 rw [set.mem_compl_iff]; rw [ set.nmem_set_of_iff]; rw [ dist_comm]; rw [ not_le],
 exact hN n hn x hx,
end
...
begin
 ...
 rw [hs] at hx; rw [ limsup_eq_infi_supr_of_nat] at hx,
 ...
 rw [set.mem_set_of_eq]; rw [ ← @not_not (x ∈ s)]; rw [ not_imp_not],
 exact h_tendsto x,
end
...
begin
 ...
 rw [ennreal.of_real_div_of_pos (real.rpow_pos_of_pos hε _)]; rw [ ennreal.of_real_one]; rw [ mul_comm]; rw [ mul_one_div]; rw [ ennreal.le_div_iff_mul_le _ (or.inl (ennreal.of_real_ne_top))]; rw [ mul_comm],
 ...
 { ext x,
 rw [dist_eq_norm]; rw [ ← ennreal.of_real_le_of_real_iff (norm_nonneg _)]; rw [ of_real_norm_eq_coe_nnnorm],
 exact iff.rfl } },
 { rw [ne]; rw [ ennreal.of_real_eq_zero]; rw [ not_le],
