...
by rwa [← f.mk_coe_fn]; rwa [ ← g.mk_coe_fn]; rwa [ mk_eq_mk]
...
by rw [← comp_mk g hg f f.ae_strongly_measurable]; rw [ mk_coe_fn]
...
by rw [← comp_measurable_mk g hg f f.ae_strongly_measurable]; rw [ mk_coe_fn]
...
by rw [comp₂_eq_pair]; rw [ pair_eq_mk]; rw [ comp_mk]; refl
...
by rw [comp₂_measurable_eq_pair]; rw [ pair_eq_mk]; rw [ comp_measurable_mk]; refl
...
by rw [← mk_to_germ]; rw [ mk_coe_fn]

lemma to_germ_injective : injective (to_germ : (α →ₘ[μ] β) → germ μ.ae β) :=
λ f g H, ext $ germ.coe_eq.1 $ by rwa [← to_germ_eq]; rwa [ ← to_germ_eq]
...
by rw [← lift_rel_mk_mk]; rw [ mk_coe_fn]; rw [ mk_coe_fn]
...
by rw [← lintegral_mk]; rw [ mk_coe_fn]
