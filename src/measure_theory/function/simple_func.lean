...
begin
 ...
 rwa [set.mem_preimage] at this; rwa [ set.mem_singleton_iff] at this, }, },
end
...
begin
 ...
 refine ⟨c, _⟩,
 ext1 x,
 rw [h_eq x]; rw [ simple_func.coe_const],
end
...
by rw [f.sum_measure_preimage_singleton]; rw [ coe_range]; rw [ preimage_range]
...
begin
 ...
 by_contra,
 obtain ⟨y, hy_mem⟩ := h,
 rw [simple_func.mem_range] at hy_mem; rw [ set.mem_range] at hy_mem,
 obtain ⟨x, hxy⟩ := hy_mem,
...
by { rw [restrict]; rw [ dif_pos hs], refl }
...
by rw [← finset.mem_coe]; rw [ coe_range]; rw [ coe_restrict _ hs]; rw [ mem_range_indicator]
...
by rw [approx_apply _ hf]; rw [ approx_apply _ (hf.comp hg)]
...
begin
 ...
 { rw [approx_apply a hf]; rw [ h_zero],
...
by rw [ennreal_rat_embed]; rw [ encodable.encodek]; refl
...
begin
 rw [eapprox]; rw [ supr_approx_apply ennreal_rat_embed f a hf rfl],
...
begin
 ...
 { rw [finset.sum_range_succ]; rw [ nat.succ_eq_add_one]; rw [ IH]; rw [ eapprox_diff]; rw [ coe_map]; rw [ function.comp_app]; rw [ coe_sub]; rw [ pi.sub_apply]; rw [ ennreal.coe_to_nnreal]; rw [ add_tsub_cancel_of_le (monotone_eapprox f (nat.le_succ _) _)],
...
by simp_rw [ennreal.tsum_eq_supr_nat' (tendsto_add_at_top_nat 1), sum_eapprox_diff, supr_eapprox_apply f hf a]
...
calc (f + g).lintegral μ =
 ...
 by rw [add_eq_map₂]; rw [ map_lintegral]; exact finset.sum_congr rfl (assume a ha, add_mul _ _ _)
 ...
 by rw [map_lintegral]; rw [ map_lintegral]
...
... = ∑ r in f.range, r * μ (f ⁻¹' {r} ∩ s) :
 ...
 else by rw [restrict_preimage_singleton _ hs hb]; rw [ inter_comm]
...
by rw [f.restrict_lintegral hs]; rw [ lintegral_restrict]
...
by rw [const_lintegral]; rw [ measure.restrict_apply measurable_set.univ]; rw [ univ_inter]

lemma restrict_const_lintegral (c : ℝ≥0∞) {s : set α} (hs : measurable_set s) :
 ((const α c).restrict s).lintegral μ = c * μ s :=
by rw [restrict_lintegral_eq_lintegral_restrict _ hs]; rw [ const_lintegral_restrict]
...
calc f.lintegral μ ⊔ g.lintegral μ =
 ...
 ... = (f ⊔ g).lintegral μ : by rw [sup_eq_map₂]; rw [ map_lintegral]
...
begin
 ...
 { intro H,
 rw [fin_meas_supp_iff_support]; rw [ support_eq],
...
begin
 ...
 rw [← finset.coe_inj] at h; rw [ finset.coe_sdiff] at h; rw [ finset.coe_singleton] at h; rw [ simple_func.coe_range] at h,
 revert s f h, refine finset.induction _ _,
 { intros f hf, rw [finset.coe_empty] at hf; rw [ diff_eq_empty] at hf; rw [ range_subset_singleton] at hf,
 ...
 rw [image_compl_preimage]; rw [ union_diff_distrib]; rw [ diff_diff_comm]; rw [ hf]; rw [ finset.coe_insert]; rw [ insert_diff_self_of_not_mem]; rw [ diff_eq_empty.mpr]; rw [ set.empty_union],
