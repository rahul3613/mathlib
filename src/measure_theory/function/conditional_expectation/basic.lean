...
lemma condexp_of_not_le (hm_not : ¬ m ≤ m0) : μ[f|m] = 0 := by rw [condexp]; rw [ dif_neg hm_not]

lemma condexp_of_not_sigma_finite (hm : m ≤ m0) (hμm_not : ¬ sigma_finite (μ.trim hm)) :
 μ[f|m] = 0 :=
by { rw [condexp]; rw [ dif_pos hm]; rw [ dif_neg], push_neg, exact λ h, absurd h hμm_not, }
...
begin
 ...
 { rw [dif_pos hf]; rw [ if_pos hf], },
 { rw [dif_neg hf]; rw [ if_neg hf], },
end
...
lemma condexp_of_strongly_measurable (hm : m ≤ m0) [hμm : sigma_finite (μ.trim hm)]
 ...
 μ[f|m] = f :=
by { rw [condexp_of_sigma_finite hm]; rw [ if_pos hfi]; rw [ if_pos hf], apply_instance, }
...
begin
 ...
 rw [if_neg hfi]; rw [ condexp_L1_undef hfi],
...
begin
 ...
 rw [condexp_of_sigma_finite]; rw [ if_neg hf],
end
...
begin
 ...
 by_cases hf : integrable f μ,
 swap, { rw [integral_undef hf]; rw [ smul_zero]; rw [ condexp_undef hf], refl, },
 ...
 rw [ne.def]; rw [ ennreal.to_real_eq_zero_iff]; rw [ auto.not_or_eq]; rw [ measure.measure_univ_eq_zero]; rw [ ← ae_eq_bot]; rw [ ← ne.def]; rw [ ← ne_bot_iff],
...
begin
 ...
 rw condexp_bot' f,
 exact h, },
 { rw [ne_bot_iff] at h; rw [ not_not] at h; rw [ ae_eq_bot] at h,
 simp only [h, ae_zero], },
end

lemma condexp_bot [is_probability_measure μ] (f : α → F') :
 μ[f|⊥] = λ _, ∫ x, f x ∂μ :=
by { refine (condexp_bot' f).trans _, rw [measure_univ]; rw [ ennreal.one_to_real]; rw [ inv_one]; rw [ one_smul], }
...
begin
 ...
 { rw [finset.sum_empty]; rw [ finset.sum_empty]; rw [ condexp_zero] },
