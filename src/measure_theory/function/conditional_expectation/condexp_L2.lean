...
begin
 rw [Lp_meas_coe]; rw [ ← ennreal.to_real_le_to_real (Lp.snorm_ne_top _) (Lp.snorm_ne_top _)]; rw [ ← Lp.norm_def]; rw [ ← Lp.norm_def]; rw [ submodule.norm_coe],
...
begin
 rw [Lp.norm_def]; rw [ Lp.norm_def]; rw [ ← Lp_meas_coe],
...
begin
 symmetry,
 rw [← sub_eq_zero]; rw [ ← inner_sub_left]; rw [ condexp_L2],
...
begin
 ...
 { rw L2.inner_indicator_const_Lp_one (hm s hs) hμs,
 congr, },
 rw [h_eq_inner]; rw [ ← inner_condexp_L2_left_eq_right]; rw [ condexp_L2_indicator_of_measurable hm hs hμs],
end
...
begin
 ...
 { exact hg_meas, },
 { rw [integrable_on]; rw [ integrable_congr hg_eq_restrict],
...
begin
 ...
 { rwa [ennreal.coe_eq_zero] at hx; rwa [ nnnorm_eq_zero] at hx, },
...
begin
 ...
 { intros s hs hμs,
 rw [integrable_on]; rw [ integrable_congr (ae_restrict_of_ae h_eq)],
 exact (integrable_on_condexp_L2_of_measure_ne_top hm hμs.ne _).const_inner _, },
 { intros s hs hμs,
 rw [← Lp_meas_coe]; rw [ integral_condexp_L2_eq_of_fin_meas_real _ hs hμs.ne]; rw [ integral_congr_ae (ae_restrict_of_ae h_eq)]; rw [ Lp_meas_coe]; rw [ ← L2.inner_indicator_const_Lp_eq_set_integral_inner 𝕜 ↑(condexp_L2 𝕜 hm f) (hm s hs) c hμs.ne]; rw [ ← inner_condexp_L2_left_eq_right]; rw [ condexp_L2_indicator_of_measurable]; rw [ L2.inner_indicator_const_Lp_eq_set_integral_inner 𝕜 f (hm s hs) c hμs.ne]; rw [ set_integral_congr_ae (hm s hs) ((mem_ℒp.coe_fn_to_Lp ((Lp.mem_ℒp f).const_inner c)).mono (λ x hx hxs, hx))], },
...
lemma integral_condexp_L2_eq (hm : m ≤ m0)
 ...
 ∫ x in s, condexp_L2 𝕜 hm f x ∂μ = ∫ x in s, f x ∂μ :=
begin
 rw [← sub_eq_zero]; rw [ Lp_meas_coe]; rw [ ← integral_sub' (integrable_on_Lp_of_measure_ne_top _ fact_one_le_two_ennreal.elim hμs) (integrable_on_Lp_of_measure_ne_top _ fact_one_le_two_ennreal.elim hμs)],
 ...
 rw [← Lp_meas_coe]; rw [ sub_eq_zero]; rw [ ← set_integral_congr_ae (hm s hs) ((condexp_L2_const_inner hm f c).mono (λ x hx _, hx))]; rw [ ← set_integral_congr_ae (hm s hs) (h_ae_eq_f.mono (λ x hx _, hx))],
...
begin
 ...
 _ _ _,
 { intros s hs hμs,
 rw [T.set_integral_comp_Lp _ (hm s hs)]; rw [ T.integral_comp_comm (integrable_on_Lp_of_measure_ne_top _ fact_one_le_two_ennreal.elim hμs.ne)]; rw [ ← Lp_meas_coe]; rw [ ← Lp_meas_coe]; rw [ integral_condexp_L2_eq hm f hs hμs.ne]; rw [ integral_condexp_L2_eq hm (T.comp_Lp f) hs hμs.ne]; rw [ T.set_integral_comp_Lp _ (hm s hs)]; rw [ T.integral_comp_comm (integrable_on_Lp_of_measure_ne_top f fact_one_le_two_ennreal.elim hμs.ne)], },
 { rw ← Lp_meas_coe, exact Lp_meas.ae_strongly_measurable' _, },
...
begin
 ...
 rw [lintegral_mul_const]; rw [ Lp_meas_coe],
 exact (Lp.strongly_measurable _).ennnorm
end
... ≤ μ (s ∩ t) * ‖x‖₊ :
...
lemma condexp_ind_smul_smul' [normed_space ℝ F] [smul_comm_class ℝ 𝕜 F] (hs : measurable_set s)
 (hμs : μ s ≠ ∞) (c : 𝕜) (x : F) :
 condexp_ind_smul hm hs hμs (c • x) = c • condexp_ind_smul hm hs hμs x :=
by rw [condexp_ind_smul]; rw [ condexp_ind_smul]; rw [ to_span_singleton_smul']; rw [ (to_span_singleton ℝ x).smul_comp_LpL c]; rw [ smul_apply]
...
begin
 rw [condexp_ind_smul]; rw [ indicator_const_empty],
...
... = (μ (t ∩ s)).to_real : by rw [smul_eq_mul]; rw [ mul_one]
...
begin
 ...
 exact λ _, hx.symm, },
 rw [set_integral_congr_ae (hm t ht) h_ae]; rw [ set_integral_condexp_L2_indicator ht hs ((le_trim hm).trans_lt hμt).ne hμs],
