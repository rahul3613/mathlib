...
begin
 ...
 rw [L1.norm_eq_integral_norm]; rw [ ← ennreal.to_real_of_real (norm_nonneg x)]; rw [ ← ennreal.to_real_mul]; rw [ ← ennreal.to_real_of_real this]; rw [ ennreal.to_real_le_to_real ennreal.of_real_ne_top (ennreal.mul_ne_top hμs ennreal.of_real_ne_top)]; rw [ of_real_integral_norm_eq_lintegral_nnnorm],
 ...
 dsimp only,
 rw hz, },
 rw [h_eq]; rw [ of_real_norm_eq_coe_nnnorm],
...
begin
 ...
 by_cases hμs : μ s = ∞,
 { rw [condexp_ind_L1_of_measure_eq_top hμs x]; rw [ Lp.norm_zero],
 exact mul_nonneg ennreal.to_real_nonneg (norm_nonneg _), },
...
begin
 ...
 (lt_top_iff_ne_top.mpr (ennreal.add_ne_top.mpr ⟨hμs, hμt⟩))).ne,
 rw [condexp_ind_L1_of_measurable_set_of_measure_ne_top hs hμs x]; rw [ condexp_ind_L1_of_measurable_set_of_measure_ne_top ht hμt x]; rw [ condexp_ind_L1_of_measurable_set_of_measure_ne_top (hs.union ht) hμst x],
...
begin
 ...
 rw [Lp_meas_coe]; rw [ condexp_L2_indicator_of_measurable hm hs hμs (1 : ℝ)],
...
begin
 ...
 rw [Lp.simple_func.coe_indicator_const]; rw [ set_integral_indicator_const_Lp (hm _ hs)],
 ...
 rw [integral_add (L1.integrable_coe_fn _).integrable_on (L1.integrable_coe_fn _).integrable_on]; rw [ integral_add (L1.integrable_coe_fn _).integrable_on (L1.integrable_coe_fn _).integrable_on]; rw [ hf]; rw [ hg], },
...
begin
 ...
 rw [← set.inter_Union]; rw [ (Union_spanning_sets (μ.trim hm))]; rw [ set.inter_univ], },
...
begin
 ...
 { intros c s hs hμs,
 rw [Lp.simple_func.coe_indicator_const]; rw [ Lp_meas_to_Lp_trim_lie_symm_indicator hs hμs.ne c]; rw [ condexp_L1_clm_indicator_const_Lp],
