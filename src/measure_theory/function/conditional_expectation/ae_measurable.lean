...
⟨λ x, g (hf.mk _ x),
 ...
 hf.ae_eq_mk.mono (λ x hx, by rw [function.comp_apply]; rw [ hx])⟩
...
by rw [← add_subgroup.mem_carrier]; rw [ Lp_meas_subgroup]; rw [ set.mem_set_of_eq]
...
by rw [← set_like.mem_coe]; rw [ ← submodule.mem_carrier]; rw [ Lp_meas]; rw [ set.mem_set_of_eq]
...
lemma Lp_meas_subgroup_to_Lp_trim_sub (hm : m ≤ m0) (f g : Lp_meas_subgroup F m p μ) :
 Lp_meas_subgroup_to_Lp_trim F p μ hm (f - g)
 = Lp_meas_subgroup_to_Lp_trim F p μ hm f - Lp_meas_subgroup_to_Lp_trim F p μ hm g :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ Lp_meas_subgroup_to_Lp_trim_add]; rw [ Lp_meas_subgroup_to_Lp_trim_neg]
...
begin
 rw [Lp.norm_def]; rw [ snorm_trim hm (Lp.strongly_measurable _)]; rw [ snorm_congr_ae (Lp_meas_subgroup_to_Lp_trim_ae_eq hm _)]; rw [ Lp_meas_subgroup_coe]; rw [ ← Lp.norm_def],
 congr,
end

lemma isometry_Lp_meas_subgroup_to_Lp_trim [hp : fact (1 ≤ p)] (hm : m ≤ m0) :
 isometry (Lp_meas_subgroup_to_Lp_trim F p μ hm) :=
isometry.of_dist_eq $ λ f g, by rw [dist_eq_norm]; rw [ ← Lp_meas_subgroup_to_Lp_trim_sub]; rw [ Lp_meas_subgroup_to_Lp_trim_norm_map]; rw [ dist_eq_norm]
...
begin
 ...
 { intros b t ht hμt,
 rw [Lp.simple_func.coe_indicator_const]; rw [ Lp_meas_to_Lp_trim_lie_symm_indicator ht hμt.ne b],
