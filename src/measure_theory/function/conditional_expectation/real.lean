...
begin
 ...
 { intros s hs hlt,
 conv_rhs { rw [← hf.with_densityᵥ_trim_eq_integral hm hs]; rw [ ← signed_measure.with_densityᵥ_rn_deriv_eq ((μ.with_densityᵥ f).trim hm) (μ.trim hm) (hf.with_densityᵥ_trim_absolutely_continuous hm)], },
 rw [with_densityᵥ_apply (signed_measure.integrable_rn_deriv ((μ.with_densityᵥ f).trim hm) (μ.trim hm)) hs]; rw [ ← set_integral_trim hm _ hs],
...
begin
 ...
 swap, { rw [condexp_undef hf]; rw [ snorm_zero], exact zero_le _ },
 by_cases hm : m ≤ m0,
 swap, { rw [condexp_of_not_le hm]; rw [ snorm_zero], exact zero_le _ },
 by_cases hsig : sigma_finite (μ.trim hm),
 swap, { rw [condexp_of_not_sigma_finite hm hsig]; rw [ snorm_zero], exact zero_le _ },
 ...
 ... = snorm f 1 μ :
 begin
 rw [snorm_one_eq_lintegral_nnnorm]; rw [ snorm_one_eq_lintegral_nnnorm]; rw [ ← ennreal.to_real_eq_to_real (ne_of_lt integrable_condexp.2) (ne_of_lt hf.2)]; rw [ ← integral_norm_eq_lintegral_nnnorm (strongly_measurable_condexp.mono hm).ae_strongly_measurable]; rw [ ← integral_norm_eq_lintegral_nnnorm hf.1],
...
begin
 ...
 { rw [← snorm_one_eq_lintegral_nnnorm]; rw [ ← snorm_one_eq_lintegral_nnnorm],
...
begin
 ...
 { rw [ennreal.coe_le_coe]; rw [ real.nnnorm_of_nonneg R.coe_nonneg],
...
begin
 ...
 rw [ennreal.one_to_real] at this; rw [ ennreal.rpow_one] at this; rw [ ennreal.rpow_one] at this; rw [ mul_comm] at this; rw [ ← ennreal.le_div_iff_mul_le (or.inl (ennreal.coe_ne_zero.2 hCpos.ne.symm)) (or.inl ennreal.coe_lt_top.ne)] at this,
 simp_rw [ennreal.coe_le_coe] at this,
 refine this.trans _,
 rw [ennreal.div_le_iff_le_mul (or.inl (ennreal.coe_ne_zero.2 hCpos.ne.symm)) (or.inl ennreal.coe_lt_top.ne)]; rw [ hC]; rw [ nonneg.inv_mk]; rw [ ennreal.coe_mul]; rw [ ennreal.coe_to_nnreal hg.snorm_lt_top.ne]; rw [ ← mul_assoc]; rw [ ← ennreal.of_real_eq_coe_nnreal]; rw [ ← ennreal.of_real_mul hδ.le]; rw [ mul_inv_cancel hδ.ne.symm]; rw [ ennreal.of_real_one]; rw [ one_mul],
 exact snorm_one_condexp_le_snorm _ },
...
begin
 ...
 ... =ᵐ[μ] ⇑(g₁ + g₂) * μ[g|m] : by rw [h_add]; rw [ add_mul], },
end
