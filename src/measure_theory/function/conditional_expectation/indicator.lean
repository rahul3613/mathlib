...
begin
 ...
 { intros t ht hμt,
 rw [measure.restrict_restrict (hm _ ht)]; rw [ set_integral_condexp hm hf_int (ht.inter hs)]; rw [ ← measure.restrict_restrict (hm _ ht)],
...
begin
 ...
 classical,
 rw [set.indicator_apply]; rw [ set.indicator_apply]; rw [ hx],
 end
 ...
 { rw [set.indicator_indicator]; rw [ set.inter_self], }, },
 filter_upwards [this] with x hx,
 by_cases hxs : x ∈ s,
 ...
 by rw [set.indicator_indicator]; rw [ set.inter_compl_self]; rw [ set.indicator_empty']; rw [ add_zero]
...
begin
 ...
 { intros t ht hμt,
 rw [← integrable_indicator_iff (hm _ ht)]; rw [ set.indicator_indicator]; rw [ set.inter_comm]; rw [ ← set.indicator_indicator],
 suffices h_int_restrict : integrable (t.indicator ((μ.restrict s)[f|m])) (μ.restrict s),
 { rw [integrable_indicator_iff (hm _ hs_m)]; rw [ integrable_on],
 rw [integrable_indicator_iff (hm _ ht)] at h_int_restrict ⊢; rw [ integrable_on] at h_int_restrict ⊢,
 ...
 by rw [integral_indicator (hm _ hs_m)]; rw [ measure.restrict_restrict (hm _ hs_m)]; rw [ measure.restrict_restrict (hm _ ht)]; rw [ set.inter_comm]
...
begin
 ...
 { rwa [← set.inter_univ s]; rwa [ ← hs set.univ]; rwa [ set.inter_univ], },
 by_cases hf_int : integrable f μ,
 ...
 by rw [this]; rw [ add_zero]; rw [ measure.restrict_restrict (hm _ hs_m)],
 ...
 simp_rw [this, set_integral_condexp hm₂ (hf_int.indicator (hm _ hs_m)) ht, set_integral_condexp hm (hf_int.indicator (hm _ hs_m)) hst_m, integral_indicator (hm _ hs_m), measure.restrict_restrict (hm _ hs_m), ← set.inter_assoc, set.inter_self],
end
