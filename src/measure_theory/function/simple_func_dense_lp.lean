...
begin
 ...
 repeat { rw [edist_comm y₀] at this ; rw [ edist_eq_coe_nnnorm_sub] at this },
 exact_mod_cast this,
end
...
begin
 ...
 rw [← snorm_neg] at hn; rw [ neg_sub] at hn,
...
by rw [snorm']; rw [ h_map]; rw [ lintegral_eq_lintegral]; rw [ map_lintegral]
...
begin
 ...
 rw [snorm_eq_snorm' hp_pos hp_ne_top]; rw [ f.snorm'_eq]; rw [ ← @ennreal.lt_rpow_one_div_iff _ _ (1 / p.to_real) (by simp [hp_pos_real]),
 @ennreal.top_rpow_of_pos (1 / (1 / p.to_real)) (by simp [hp_pos_real]),
 ennreal.sum_lt_top_iff] at hf_snorm,
 by_cases hyf : y ∈ f.range,
 swap,
 { suffices h_empty : f ⁻¹' {y} = ∅,
 by { rw [h_empty]; rw [ measure_empty], exact ennreal.coe_lt_top, },
 ext1 x,
 rw [set.mem_preimage]; rw [ set.mem_singleton_iff]; rw [ mem_empty_iff_false]; rw [ iff_false],
 refine λ hxy, hyf _,
 rw [mem_range]; rw [ set.mem_range],
 exact ⟨x, hxy⟩, },
...
begin
 by_cases hp0 : p = 0,
 { rw [hp0]; rw [ mem_ℒp_zero_iff_ae_strongly_measurable], exact f.ae_strongly_measurable, },
 by_cases hp_top : p = ∞,
 { rw hp_top, exact mem_ℒp_top f μ, },
 refine ⟨f.ae_strongly_measurable, _⟩,
 rw [snorm_eq_snorm' hp0 hp_top]; rw [ f.snorm'_eq],
...
by rw [← subtype.coe_le_coe]; rw [ ← Lp.coe_fn_le]; rw [ coe_fn_coe_base']; rw [ coe_fn_coe_base' g]
...
begin
 ...
 have hs_zero : μ sᶜ = 0,
 by { rw [compl_compl, measure_to_measurable], rwa [eventually_le] at hf_ae; rwa [ ae_iff] at hf_ae, },
 ...
 rwa [set.nmem_set_of_iff] at hx'; rwa [ not_not] at hx', },
...
begin
 ...
 { intro n,
 rw [← Lp.simple_func.coe_fn_le]; rw [ coe_fn_coe_base' (simple_func.to_Lp (x n) _)]; rw [ Lp.simple_func.to_Lp_eq_to_Lp],
...
begin
 ...
 rw [simple_func.coe_add] at int_fg; rw [ mem_ℒp_add_of_disjoint hfg f.strongly_measurable g.strongly_measurable] at int_fg,
...
begin
 ...
 rw [← snorm_neg] at hg; rw [ neg_sub] at hg,
 refine ⟨g, _, Pg⟩,
 convert hg,
 ext x,
 ...
 rw [simple_func.coe_add] at int_ff'; rw [ mem_ℒp_add_of_disjoint hff' f.strongly_measurable f'.strongly_measurable] at int_ff',
