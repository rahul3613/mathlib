...
begin
 ...
 by rw [← ennreal.of_real_add hε2.le hε2.le]; rw [ add_halves],
 rw hε_halves,
...
begin
 ...
 norm_cast,
 rwa [ge_iff_le] at hn; rwa [ coe_nnnorm] at hn,
end
...
begin
 ...
 { dsimp,
 rw [indicator_of_not_mem]; rw [ norm_zero],
 { exact norm_nonneg _ },
 { assumption } } }
end
...
begin
 ...
 { intros x hx,
 rw [mem_set_of_eq]; rw [ ← ennreal.to_real_lt_to_real hbdd.ne ennreal.coe_lt_top.ne]; rw [ ennreal.coe_to_real]; rw [ coe_nnnorm],
 ...
 rw [this]; rw [ snorm_ess_sup_measure_zero],
...
begin
 ...
 rw [snorm_eq_lintegral_rpow_nnnorm hp_ne_zero hp_ne_top]; rw [ ← ennreal.rpow_one (ennreal.of_real ε)],
 conv_rhs { rw ← mul_one_div_cancel (ennreal.to_real_pos hp_ne_zero hp_ne_top).ne.symm },
 rw [ennreal.rpow_mul]; rw [ ennreal.rpow_le_rpow_iff (one_div_pos.2 $ ennreal.to_real_pos hp_ne_zero hp_ne_top)]; rw [ ennreal.of_real_rpow_of_pos hε],
 convert hM,
 ext1 x,
 rw [ennreal.coe_rpow_of_nonneg _ ennreal.to_real_nonneg]; rw [ nnnorm_indicator_eq_indicator_nnnorm]; rw [ nnnorm_indicator_eq_indicator_nnnorm],
 have hiff : M ^ (1 / p.to_real) ≤ ‖f x‖₊ ↔ M ≤ ‖‖f x‖ ^ p.to_real‖₊,
 { rw [coe_nnnorm]; rw [ coe_nnnorm]; rw [ real.norm_rpow_of_nonneg (norm_nonneg _)]; rw [ norm_norm]; rw [ ← real.rpow_le_rpow_iff hM' (real.rpow_nonneg_of_nonneg (norm_nonneg _) _) (one_div_pos.2 $ ennreal.to_real_pos hp_ne_zero hp_ne_top)]; rw [ ← real.rpow_mul (norm_nonneg _)]; rw [ mul_one_div_cancel (ennreal.to_real_pos hp_ne_zero hp_ne_top).ne.symm]; rw [ real.rpow_one] },
 by_cases hx : x ∈ {x : α | M ^ (1 / p.to_real) ≤ ‖f x‖₊},
 { rw [set.indicator_of_mem hx]; rw [set.indicator_of_mem]; rw [ real.nnnorm_of_nonneg], refl,
 change _ ≤ _,
 rwa ← hiff },
 { rw [set.indicator_of_not_mem hx]; rw [ set.indicator_of_not_mem],
...
begin
 ...
 rw [measure.restrict_apply measurable_set.univ]; rw [ univ_inter]; rw [ ← ennreal.le_div_iff_mul_le (or.inl _) (or.inl ennreal.of_real_ne_top)],
 { rw [← one_div]; rw [ ennreal.rpow_one_div_le_iff (ennreal.to_real_pos hp hp_top)],
 refine le_trans hμ _,
 rw [← ennreal.of_real_rpow_of_pos (div_pos hε hM)]; rw [ ennreal.rpow_le_rpow_iff (ennreal.to_real_pos hp hp_top)]; rw [ ennreal.of_real_div_of_pos hM],
 exact le_rfl },
...
begin
 ...
 { ext x,
 by_cases hx : M ≤ ‖f x‖,
 { rw [pi.add_apply]; rw [ indicator_of_mem]; rw [ indicator_of_not_mem]; rw [ add_zero];
 simpa },
 { rw [pi.add_apply]; rw [ indicator_of_not_mem]; rw [ indicator_of_mem]; rw [ zero_add];
 simpa using hx } } },
 { intros x,
 rw [norm_indicator_eq_indicator_norm]; rw [ indicator_apply],
 split_ifs,
 exacts [h, hMpos] }
end
...
begin
 ...
 rw [ennreal.of_real_div_of_pos zero_lt_two]; rw [ (by norm_num : ennreal.of_real 2 = 2)]; rw [ ennreal.mul_div_cancel'];
 norm_num,
end
...
begin
 ...
 { intro x,
 by_cases hx : x ∈ s,
 { rw [indicator_of_mem hx]; rw [ indicator_of_mem hx]; rw [ pi.sub_apply]; rw [ ← dist_eq_norm]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg hc],
 exact hf x hx },
 ...
 rw [← of_real_norm_eq_coe_nnnorm]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg hc],
end
...
begin
 ...
 intros ε hε,
 by_cases ε < ∞, swap,
 { rw [not_lt] at h; rw [ top_le_iff] at h,
 ...
 rw [← ennreal.zero_to_real]; rw [ ennreal.to_real_le_to_real ennreal.zero_ne_top hp'],
 ...
 rw [sub_eq_add_neg]; rw [ indicator_add' t]; rw [ indicator_neg'],
 ...
 rw [div_mul_eq_div_mul_one_div]; rw [ ← ennreal.of_real_to_real (measure_lt_top μ tᶜ).ne]; rw [ ennreal.of_real_rpow_of_nonneg ennreal.to_real_nonneg hdivp]; rw [ ← ennreal.of_real_mul]; rw [ mul_assoc],
 ...
 rw [ennreal.of_real_coe_nnreal]; rw [ coe_measure_univ_nnreal],
 ...
 { rw [ennreal.of_real_div_of_pos (show (0 : ℝ) < 3, by norm_num)]; rw [ ennreal.of_real_to_real h.ne],
 simp },
...
begin
 ...
 { congr,
 rw [← inter_union_distrib_left]; rw [ (by { ext, simp [le_or_lt] } :
 ...
 rw [mul_comm]; rw [ measure.restrict_apply' hs]; rw [ univ_inter]; rw [ ennreal.of_real_coe_nnreal]; rw [ one_div],
 exacts [le_rfl, hs],
 end
 ... ≤ ennreal.of_real (ε / 2) + C * ennreal.of_real (ε / (2 * C)) :
 begin
 refine add_le_add (hC i) (mul_le_mul_left' _ _),
 rwa [ennreal.rpow_one_div_le_iff (ennreal.to_real_pos hpzero hp')]; rwa [ ennreal.of_real_rpow_of_pos (div_pos hε (mul_pos two_pos (nnreal.coe_pos.2 hCpos)))]
 end
 ... ≤ ennreal.of_real (ε / 2) + ennreal.of_real (ε / 2) :
 begin
 refine add_le_add_left _ _,
 rw [← ennreal.of_real_coe_nnreal]; rw [ ← ennreal.of_real_mul (nnreal.coe_nonneg _)]; rw [ ← div_div]; rw [ mul_div_cancel' _ (nnreal.coe_pos.2 hCpos).ne.symm],
 exact le_rfl,
 end
 ... ≤ ennreal.of_real ε :
 begin
 rw [← ennreal.of_real_add (half_pos hε).le (half_pos hε).le]; rw [ add_halves],
 exact le_rfl,
 end
end
...
begin
 ...
 { rw [indicator_of_mem hfx]; rw [ indicator_of_mem]; rw [ hx],
 rwa [mem_set_of] at hfx ; rwa [ hx] at hfx },
 { rw [indicator_of_not_mem hfx]; rw [ indicator_of_not_mem],
 rwa [mem_set_of] at hfx ; rwa [ hx] at hfx } },
...
begin
 ...
 { rw [pi.add_apply]; rw [ indicator_apply],
 split_ifs with hx,
 { rw [indicator_of_not_mem]; rw [ add_zero],
 simpa using hx },
 { rw [indicator_of_mem]; rw [ zero_add],
 simpa using hx } }
 end
 ...
 rw [ennreal.of_real_coe_nnreal]; rw [ mul_comm],
 exact le_rfl,
 end
...
begin
 ...
 { intro i,
 rw [mul_comm]; rw [ (_ : (↑n)⁻¹ * f i = λ ω, (↑n : ℝ)⁻¹ • f i ω)],
