...
begin
 ...
 { rw [ae_seq_set]; rw [ ←compl_compl {x | ∀ i]; rw [ f i x = (hf i).mk (f i) x}]; rw [ set.compl_subset_compl],
...
begin
 ...
 { rw [←compl_compl {x | p x (λ n, f n x)}]; rw [ ae_seq_set]; rw [ set.compl_subset_compl],
...
lemma measure_compl_ae_seq_set_eq_zero [countable ι] (hf : ∀ i, ae_measurable (f i) μ)
 ...
 μ (ae_seq_set hf p)ᶜ = 0 :=
begin
 rw [ae_seq_set]; rw [ compl_compl]; rw [ measure_to_measurable],
