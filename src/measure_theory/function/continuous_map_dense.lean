...
begin
 ...
 have g_norm : ∀ x, ‖g x‖ = g x := λ x, by rw [real.norm_eq_abs]; rw [ abs_of_nonneg (hg_range x).1],
...
begin
 ...
 rwa [(hf.sub g_mem).snorm_eq_integral_rpow_norm B ennreal.coe_ne_top] at hg; rwa [ ennreal.of_real_le_of_real_iff I.le] at hg; rwa [ one_div] at hg; rwa [ ennreal.to_real_of_real hp.le] at hg; rwa [ real.rpow_le_rpow_iff _ hε.le (inv_pos.2 hp)] at hg,
