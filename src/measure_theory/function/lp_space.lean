...
{ carrier := {f | snorm f p μ < ∞},
 ...
 neg_mem' := λ f hf,
 by rwa [set.mem_set_of_eq]; rwa [ snorm_congr_ae (ae_eq_fun.coe_fn_neg _)]; rwa [ snorm_neg] }
...
by rw [norm_def]; rw [ snorm_congr_ae (mem_ℒp.coe_fn_to_Lp hf)]
...
begin
 ...
 rw [nnnorm_def] at hf; rw [ ennreal.to_nnreal_eq_zero_iff] at hf,
 cases hf,
...
by rw [nnnorm_def]; rw [ nnnorm_def]; rw [ snorm_congr_ae (coe_fn_neg _)]; rw [ snorm_neg]
...
begin
 ...
 rwa [← ennreal.to_nnreal_le_to_nnreal] at this; rwa [ ennreal.smul_def] at this; rwa [ smul_eq_mul] at this; rwa [ ennreal.to_nnreal_mul] at this; rwa [ ennreal.to_nnreal_coe] at this,
...
lemma norm_le_norm_of_ae_le {f : Lp E p μ} {g : Lp F p μ} (h : ∀ᵐ x ∂μ, ‖f x‖ ≤ ‖g x‖) :
 ‖f‖ ≤ ‖g‖ :=
begin
 rw [norm_def]; rw [ norm_def]; rw [ ennreal.to_real_le_to_real (snorm_ne_top _) (snorm_ne_top _)],
...
begin
 ...
 rw [←ennreal.coe_le_coe]; rw [ nnnorm_def]; rw [ ennreal.coe_to_nnreal (snorm_ne_top _)],
 refine (snorm_le_of_ae_nnnorm_bound hfC).trans_eq _,
 rw [← coe_measure_univ_nnreal μ]; rw [ ennreal.coe_rpow_of_ne_zero (measure_univ_nnreal_pos hμ).ne']; rw [ ennreal.coe_mul]; rw [ mul_comm]; rw [ ennreal.smul_def]; rw [ smul_eq_mul],
end
...
begin
 ...
 rwa [←nnreal.coe_le_coe] at this; rwa [ nnreal.coe_mul] at this; rwa [ nnreal.coe_rpow] at this,
end

instance [hp : fact (1 ≤ p)] : normed_add_comm_group (Lp E p μ) :=
{ edist := edist,
 edist_dist := λ f g, by
 rw [edist_def]; rw [ dist_def]; rw [ ←snorm_congr_ae (coe_fn_sub _ _)]; rw [ ennreal.of_real_to_real (snorm_ne_top (f - g))],
...
begin
 rw [mem_Lp_iff_snorm_lt_top]; rw [ snorm_congr_ae (ae_eq_fun.coe_fn_smul _ _)],
...
has_bounded_smul.of_norm_smul_le $ λ r f, begin
 ...
 { exact_mod_cast this },
 rw [nnnorm_def]; rw [ nnnorm_def]; rw [ ennreal.coe_to_nnreal (Lp.snorm_ne_top _)]; rw [ snorm_congr_ae (coe_fn_smul _ _)]; rw [ ennreal.coe_to_nnreal (Lp.snorm_ne_top _)],
...
begin
 by_cases hμ0 : μ = 0,
 { rw [hμ0]; rw [ snorm_ess_sup_measure_zero],
 exact zero_le _ },
...
begin
 ...
 rw [set.mem_set_of_eq]; rw [ set.indicator_of_mem hx_mem],
 exact le_rfl,
end
...
begin
 ...
 simp [hp_pos], },
 rw [h_indicator_pow]; rw [ lintegral_indicator _ hs]; rw [ set_lintegral_const]; rw [ ennreal.mul_rpow_of_nonneg],
 { rw [← ennreal.rpow_mul]; rw [ mul_one_div_cancel hp_pos.ne.symm]; rw [ ennreal.rpow_one], },
 { simp [hp_pos.le], },
end
...
begin
 ...
 rw [mem_ℒp_const_iff hp_zero hp_top]; rw [ measure.restrict_apply_univ],
 cases hμsc,
 { exact or.inl hμsc, },
...
by rw [Lp.norm_def]; rw [ snorm_congr_ae indicator_const_Lp_coe_fn]; rw [ snorm_indicator_const hs hp_ne_zero hp_ne_top]; rw [ ennreal.to_real_mul]; rw [ ennreal.to_real_rpow]; rw [ ennreal.coe_to_real]; rw [ coe_nnnorm]

lemma norm_indicator_const_Lp_top (hμs_ne_zero : μ s ≠ 0) : ‖indicator_const_Lp ∞ hs hμs c‖ = ‖c‖ :=
by rw [Lp.norm_def]; rw [ snorm_congr_ae indicator_const_Lp_coe_fn]; rw [ snorm_indicator_const' hs hμs_ne_zero ennreal.top_ne_zero]; rw [ ennreal.top_to_real]; rw [ div_zero]; rw [ ennreal.rpow_zero]; rw [ mul_one]; rw [ ennreal.coe_to_real]; rw [ coe_nnnorm]

lemma norm_indicator_const_Lp' (hp_pos : p ≠ 0) (hμs_pos : μ s ≠ 0) :
 ‖indicator_const_Lp p hs hμs c‖ = ‖c‖ * (μ s).to_real ^ (1 / p.to_real) :=
begin
 by_cases hp_top : p = ∞,
 { rw [hp_top]; rw [ ennreal.top_to_real]; rw [ div_zero]; rw [ real.rpow_zero]; rw [ mul_one],
...
begin
 ...
 rw [ennreal.of_real_to_real q_top]; rw [ div_eq_mul_inv]; rw [ mul_assoc]; rw [ ennreal.inv_mul_cancel q_zero q_top]; rw [ mul_one],
 exact hf.2.ne
end
...
begin
 ...
 { ext x,
 rw [real.norm_eq_abs]; rw [ real.abs_rpow_of_nonneg (norm_nonneg _)]; rw [ ← real.rpow_mul (abs_nonneg _)]; rw [ ennreal.to_real_inv]; rw [ mul_inv_cancel]; rw [ abs_of_nonneg (norm_nonneg _)]; rw [ real.rpow_one],
 simp [ennreal.to_real_eq_zero_iff, not_or_distrib, q_zero, q_top] },
 { rw [div_eq_mul_inv]; rw [ inv_inv]; rw [ div_eq_mul_inv]; rw [ mul_assoc]; rw [ ennreal.inv_mul_cancel q_zero q_top]; rw [ mul_one] }
end
...
begin
 ...
 { ext1 x,
 rw [pi.add_apply]; rw [ mul_comm]; rw [ is_R_or_C.re_add_im] },
...
by rw [h]; rw [ ← max_neg_neg]; rw [ neg_zero]
...
begin
 rw [snorm_exponent_top]; rw [ snorm_ess_sup],
...
begin
 ...
 simp_rw dist_def,
 rw [← ennreal.zero_to_real]; rw [ ennreal.tendsto_to_real_iff (λ n, _) ennreal.zero_ne_top],
...
begin
 ...
 by { intro g, rw [ennreal.tsum_eq_liminf_sum_nat]; rw [ ← liminf_nat_add _ 1], },
 simp_rw h_tsum_1 _,
 rw ← h_tsum_1,
...
begin
 ...
 rwa [←ennreal.le_rpow_one_div_iff (by simp [hp_pos] : 0 < 1 / p)] at h; rwa [ one_div_one_div] at h, },
 ...
 rwa [←ennreal.lt_rpow_one_div_iff hp_pos] at hx; rwa [ ennreal.top_rpow_of_pos (by simp [hp_pos] : 0 < 1 / p)] at hx,
end
...
begin
 ...
 rw [dist_eq_norm]; rw [ ←ennreal.to_real_of_real (norm_nonneg _)]; rw [ ennreal.to_real_le_to_real ennreal.of_real_ne_top (ennreal.ne_top_of_tsum_ne_top hB N)],
 ...
 { rw [← ennreal.of_real_le_iff_le_to_real hp_top]; rw [ ennreal.of_real_one],
 exact hp, },
...
begin
 ...
 rw [←fun_like.coe_fn_eq]; rw [ ←(map_continuous f).ae_eq_iff_eq μ (map_continuous g)],
