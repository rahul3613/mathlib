...
begin
 ...
 ... = ‖hf.approx n x‖ : by rw [norm_one]; rw [ one_mul], },
 rw ← one_smul ℝ (f x),
...
begin
 ...
 { rw min_eq_right _,
 { rw [norm_div]; rw [ norm_norm]; rw [ mul_comm]; rw [ mul_div]; rw [ div_eq_mul_inv]; rw [ mul_comm]; rw [ ← mul_assoc]; rw [ inv_mul_cancel h0]; rw [ one_mul]; rw [ real.norm_of_nonneg hc], },
...
begin
 ...
 { by_cases hxt : x ∈ t,
 swap, { rw [funext (λ n, h_fs_t_compl n x hxt)]; rw [ hft_zero x hxt], exact tendsto_const_nhds, },
 ...
 rw [← set.mem_Union]; rw [ Union_spanning_sets (μ.restrict t)],
 trivial, },
 refine ⟨n, λ m hnm, _⟩,
...
begin
 ...
 { intros x hx n,
 rw [simple_func.coe_restrict _ hs]; rw [ set.indicator_of_mem hx], },
 have hg_zero : ∀ x ∉ s, ∀ n, g_seq_s n x = 0,
 { intros x hx n,
 rw [simple_func.coe_restrict _ hs]; rw [ set.indicator_of_not_mem hx], },
...
begin
 ...
 exact_mod_cast hij, },
 rw [this]; rw [ norm_sets_spanning]; rw [ Union_spanning_sets (μ.trim hm)]; rw [ set.inter_univ], },
end
...
begin
 ...
 { intros n x hxt,
 rw [set.mem_compl_iff] at hxt; rw [ set.mem_Union] at hxt; rw [ not_exists] at hxt,
 simpa using (hxt n), },
 refine λ x hxt, tendsto_nhds_unique (h_approx x) _,
 rw funext (λ n, h_fs_zero n x hxt),
 exact tendsto_const_nhds, },
 { refine ⟨⟨⟨λ n, tᶜ ∪ T n, λ n, trivial, λ n, _, _⟩⟩⟩,
 { rw [measure.restrict_apply' (measurable_set.Union hT_meas)]; rw [ set.union_inter_distrib_right]; rw [ set.compl_inter_self t]; rw [ set.empty_union],
...
by rw [← hf.map_eq]; rw [ h₂.ae_strongly_measurable_map_iff]
...
by { rw [← sum_cond]; rw [ ae_strongly_measurable_sum_measure_iff]; rw [ bool.forall_bool]; rw [ and.comm], refl }
...
by rw [uIoc_eq_union]; rw [ ae_strongly_measurable_union_iff]
...
begin
 ...
 rw [eventually_eq]; rw [ ae_restrict_iff' ht.compl],
