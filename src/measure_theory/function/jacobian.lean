...
begin
 ...
 rw [←HC.add_closed_ball_zero (le_of_lt hr)]; rw [ add_comm] },
 ...
 { rw [mem_closed_ball_iff_norm]; rw [ add_sub_cancel],
 calc ‖f z - f x - A (z - x)‖
 ≤ δ * ‖z - x‖ : hf _ zs _ xs
 ... ≤ ε * r :
 ...
 by rw [smul_add]; rw [ ← add_assoc]; rw [ add_comm ({f x})]; rw [ add_assoc]; rw [ smul_closed_ball _ _ εpos.le]; rw [ smul_zero]; rw [ singleton_add_closed_ball_zero]; rw [ ← image_smul_set ℝ E E A]; rw [ smul_closed_ball _ _ zero_le_one]; rw [ smul_zero]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg r0]; rw [ mul_one]; rw [ mul_comm],
 rw this at K,
...
begin
 ...
 rwa [← F.symm_image_target_eq_source]; rwa [ mul_comm]; rwa [ ← ennreal.le_div_iff_mul_le]; rwa [ div_eq_mul_inv]; rwa [ mul_comm]; rwa [ ← ennreal.coe_inv (mpos.ne')],
 { apply or.inl,
...
begin
 ...
 have A : s = ⋃ n, s ∩ u n,
 by rw [← inter_Union]; rw [ Union_disjointed]; rw [ Union_spanning_sets]; rw [ inter_univ],
 calc μ (f '' s) ≤ ∑' n, μ (f '' (s ∩ u n)) :
 begin
 conv_lhs { rw [A]; rw [ image_Union] },
...
begin
 ...
 rwa [with_top.coe_sub] at this; rwa [ ennreal.sub_mul] at this; rwa [ tsub_le_iff_right] at this,
 ...
 begin
 conv_rhs { rw s_eq },
 rw [image_Union]; rw [ measure_Union], rotate,
 { assume i j hij,
...
begin
 ...
 rw [map_apply h'f ht]; rw [ with_density_apply _ (h'f ht)]; rw [ measure.restrict_apply ht]; rw [ restrict_restrict (h'f ht)]; rw [ lintegral_abs_det_fderiv_eq_add_haar_image μ ((h'f ht).inter hs) (λ x hx, (hf' x hx.2).mono (inter_subset_right _ _)) (hf.mono (inter_subset_right _ _))]; rw [ image_preimage_inter]
end
...
begin
 ...
 { rw [hF]; rw [ ← measure.map_map u_meas measurable_subtype_coe]; rw [ map_comap_subtype_coe hs]; rw [ restrict_with_density hs],
...
theorem lintegral_image_eq_lintegral_abs_det_fderiv_mul (hs : measurable_set s)
 ...
 ∫⁻ x in f '' s, g x ∂μ = ∫⁻ x in s, ennreal.of_real (|(f' x).det|) * g (f x) ∂μ :=
begin
 rw [← restrict_map_with_density_abs_det_fderiv_eq_add_haar μ hs hf' hf]; rw [ (measurable_embedding_of_fderiv_within hs hf' hf).lintegral_map],
 have : ∀ (x : s), g (s.restrict f x) = (g ∘ f) x := λ x, rfl,
 simp only [this],
 rw [← (measurable_embedding.subtype_coe hs).lintegral_map]; rw [ map_comap_subtype_coe hs]; rw [ set_lintegral_with_density_eq_set_lintegral_mul_non_measurable₀ _ _ _ hs],
 { refl },
...
begin
 rw [integrable_on]; rw [ ← restrict_map_with_density_abs_det_fderiv_eq_add_haar μ hs hf' hf]; rw [ (measurable_embedding_of_fderiv_within hs hf' hf).integrable_map_iff],
 change (integrable ((g ∘ f) ∘ (coe : s → E)) _) ↔ _,
 rw [← (measurable_embedding.subtype_coe hs).integrable_map_iff]; rw [ map_comap_subtype_coe hs],
 simp only [ennreal.of_real],
 rw [restrict_with_density hs]; rw [ integrable_with_density_iff_integrable_coe_smul₀]; rw [ integrable_on],
 { congr' 2 with x,
...
theorem integral_image_eq_integral_abs_det_fderiv_smul [complete_space F] (hs : measurable_set s)
 ...
 ∫ x in f '' s, g x ∂μ = ∫ x in s, |(f' x).det| • g (f x) ∂μ :=
begin
 rw [← restrict_map_with_density_abs_det_fderiv_eq_add_haar μ hs hf' hf]; rw [ (measurable_embedding_of_fderiv_within hs hf' hf).integral_map],
 have : ∀ (x : s), g (s.restrict f x) = (g ∘ f) x := λ x, rfl,
 simp only [this, ennreal.of_real],
 rw [← (measurable_embedding.subtype_coe hs).integral_map]; rw [ map_comap_subtype_coe hs]; rw [ set_integral_with_density_eq_set_integral_smul₀ (ae_measurable_to_nnreal_abs_det_fderiv_within μ hs hf') _ hs],
 congr' with x,
...
begin
 ...
 rw [this]; rw [ continuous_linear_map.det]; rw [ continuous_linear_map.coe_smul],
 change ((1 : 𝕜 →L[𝕜] 𝕜) : 𝕜 →ₗ[𝕜] 𝕜) with linear_map.id,
 rw [linear_map.det_smul]; rw [ finite_dimensional.finrank_self]; rw [ linear_map.det_id]; rw [ pow_one]; rw [ mul_one],
end
