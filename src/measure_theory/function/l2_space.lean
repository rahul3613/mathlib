...
begin
 ...
 { simp },
 { rw [div_eq_mul_inv]; rw [ ennreal.mul_inv_cancel two_ne_zero ennreal.two_ne_top] }
end
...
lemma _root_.integral_eq_zero_of_forall_integral_inner_eq_zero (f : α → E) (hf : integrable f μ)
 ...
 ∫ x, f x ∂μ = 0 :=
by { specialize hf_int (∫ x, f x ∂μ), rwa [integral_inner hf] at hf_int ; rwa [ inner_self_eq_zero] at hf_int }
...
begin
 ...
 rw [← real.rpow_nat_cast _ 2]; rw [ ← h_two]; rw [ ← ennreal.of_real_rpow_of_nonneg (norm_nonneg _) zero_le_two]; rw [ of_real_norm_eq_coe_nnnorm],
 norm_cast,
end
...
begin
 ...
 rw [inner_def]; rw [ integral_inner_eq_sq_snorm]; rw [ norm_def]; rw [ ← ennreal.to_real_pow]; rw [ is_R_or_C.of_real_re]; rw [ ennreal.to_real_eq_to_real (ennreal.pow_ne_top (Lp.snorm_ne_top f)) _],
 { rw [←ennreal.rpow_nat_cast]; rw [ snorm_eq_snorm' two_ne_zero ennreal.two_ne_top]; rw [ snorm']; rw [ ← ennreal.rpow_mul]; rw [ one_div]; rw [ h_two],
 simp, },
...
begin
 ...
 rw [smul_eq_mul]; rw [ ← inner_smul_left],
 congr,
...
by rw [← integral_inner (integrable_on_Lp_of_measure_ne_top f fact_one_le_two_ennreal.elim hμs)]; rw [ L2.inner_indicator_const_Lp_eq_set_integral_inner]
