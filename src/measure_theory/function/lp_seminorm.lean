...
by rw [snorm_eq_snorm' hp_ne_zero hp_ne_top]; rw [ snorm']

lemma snorm_one_eq_lintegral_nnnorm {f : α → F} : snorm f 1 μ = ∫⁻ x, ‖f x‖₊ ∂μ :=
by simp_rw [snorm_eq_lintegral_rpow_nnnorm one_ne_zero ennreal.coe_ne_top, ennreal.one_to_real, one_div_one, ennreal.rpow_one]
...
by rw [snorm']; rw [ div_zero]; rw [ ennreal.rpow_zero]
...
begin
 ...
 congr,
 rw ←ennreal.rpow_mul,
 suffices hq_cancel : q * (1/q) = 1, by rw [hq_cancel]; rw [ ennreal.rpow_one],
...
begin
 ...
 { congr,
 rw ←ennreal.rpow_mul,
 suffices hp_cancel : q * (1/q) = 1, by rw [hp_cancel]; rw [ ennreal.rpow_one],
 rw [one_div]; rw [ mul_inv_cancel hq_ne_zero], },
 { rw [ne.def]; rw [ ennreal.rpow_eq_top_iff]; rw [ not_or_distrib]; rw [ not_and_distrib]; rw [ not_and_distrib],
 split,
 { left,
 rwa [ennreal.coe_eq_zero]; rwa [ nnnorm_eq_zero], },
 { exact or.inl ennreal.coe_ne_top, }, },
end

lemma snorm_ess_sup_const (c : F) (hμ : μ ≠ 0) :
 snorm_ess_sup (λ x : α, c) μ = (‖c‖₊ : ℝ≥0∞) :=
by rw [snorm_ess_sup]; rw [ ess_sup_const _ hμ]
...
begin
 ...
 rw [snorm_const _ hp hμ]; rw [ C.nnnorm_eq]; rw [ one_div]; rw [ ennreal.smul_def]; rw [ smul_eq_mul],
end
...
begin
 ...
 rw [real.norm_eq_abs]; rw [ abs_eq_self.mpr (real.rpow_nonneg_of_nonneg (norm_nonneg _) _)]; rw [ mul_comm]; rw [ ← ennreal.of_real_rpow_of_nonneg (norm_nonneg _) hq_pos.le]; rw [ ennreal.rpow_mul],
end
...
begin
 ...
 swap, { refine mul_ne_zero h0 _, rwa [ne.def]; rwa [ ennreal.of_real_eq_zero]; rwa [ not_le], },
 swap, { exact ennreal.mul_ne_top hp_top ennreal.of_real_ne_top, },
 rw [ennreal.to_real_mul]; rw [ ennreal.to_real_of_real hq_pos.le],
...
by { rw [snorm']; rw [ lintegral_smul_measure]; rw [ ennreal.mul_rpow_of_nonneg]; rw [ snorm'], simp [hp], }
...
lemma snorm'_eq_zero_of_ae_zero {f : α → F} (hq0_lt : 0 < q) (hf_zero : f =ᵐ[μ] 0) :
 snorm' f q μ = 0 :=
by rw [snorm'_congr_ae hf_zero]; rw [ snorm'_zero hq0_lt]

lemma snorm'_eq_zero_of_ae_zero' (hq0_ne : q ≠ 0) (hμ : μ ≠ 0) {f : α → F} (hf_zero : f =ᵐ[μ] 0) :
 snorm' f q μ = 0 :=
by rw [snorm'_congr_ae hf_zero]; rw [ snorm'_zero' hq0_ne hμ]
...
begin
 ...
 cases hx,
 { cases hx with hx _,
 rwa [←ennreal.coe_zero] at hx; rwa [ ennreal.coe_eq_coe] at hx; rwa [ nnnorm_eq_zero] at hx, },
...
begin
 by_cases h_top : p = ∞,
 { rw [h_top]; rw [ snorm_exponent_top]; rw [ snorm_ess_sup_eq_zero_iff], },
...
begin
 ...
 by rwa [← ennreal.one_to_real]; rwa [ ennreal.to_real_le_to_real ennreal.one_ne_top hp_top],
...
... = Lp_add_const p * (snorm f p μ + snorm g p μ) : by rw [← snorm_norm f]; rw [ ← snorm_norm g]
...
begin
 ...
 by_cases hpq_eq : p = q,
 { rw [hpq_eq]; rw [ sub_self]; rw [ ennreal.rpow_zero]; rw [ mul_one],
 exact le_rfl, },
...
begin
 ...
 rw [snorm']; rw [ ←ennreal.rpow_one (snorm_ess_sup f μ)],
...
begin
 ...
 rwa [measure_univ] at h_le_μ; rwa [ ennreal.one_rpow] at h_le_μ; rwa [ mul_one] at h_le_μ,
end
...
begin
 ...
 rwa [le_sub_comm]; rwa [ sub_zero]; rwa [ one_div]; rwa [ one_div]; rwa [ inv_le_inv hq_pos hp_pos],
 end
end
...
begin
 ...
 rw [ne]; rw [ ennreal.to_real_eq_zero_iff],
 exact not_or hp_ne_zero hp_ne_top },
 rw [← ennreal.rpow_one (ε * μ {x | ε ≤ ‖f x‖₊ ^ p.to_real})]; rw [ ← this]; rw [ ennreal.rpow_mul],
...
begin
 ...
 by_cases hp0 : p = 0,
 { rwa [hp0]; rwa [ mem_ℒp_zero_iff_ae_strongly_measurable], },
 rw ←ne.def at hp0,
 refine ⟨hfq_m, _⟩,
 by_cases hp_top : p = ∞,
 { have hq_top : q = ∞,
 by rwa [hp_top] at hpq; rwa [ top_le_iff] at hpq,
 rw [hp_top],
 ...
 rw [hp_eq_zero] at hp_pos; rw [ ennreal.zero_to_real] at hp_pos,
...
begin
 ...
 simp_rw [←ennreal.rpow_mul, one_div, inv_mul_cancel hp.ne.symm, ennreal.rpow_one, ennreal.coe_rpow_of_nonneg _ hp.le, ←lintegral_const_mul' _ _ ennreal.coe_ne_top, ←ennreal.coe_mul],
...
begin
 simp_rw [le_mul_iff_eq_zero_of_nonneg_of_neg_of_nonneg (norm_nonneg _) hc (norm_nonneg _), norm_eq_zero, eventually_and] at h,
...
begin
 ...
 rw [ne.def]; rw [ ennreal.to_real_eq_zero_iff]; rw [ auto.not_or_eq],
...
begin
 ...
 have hpq : p < q,
 { suffices : 1 / q < 1 / p,
 { rwa [one_div] at this; rwa [ one_div] at this; rwa [ ennreal.inv_lt_inv] at this, },
 rw hpqr,
 ...
 rw [← ennreal.one_to_real]; rw [ ← ennreal.to_real_div]; rw [ ← ennreal.to_real_div]; rw [ ← ennreal.to_real_div]; rw [ hpqr]; rw [ ennreal.to_real_add],
 { simp only [hq_ne_zero, one_div, ne.def, ennreal.inv_eq_top, not_false_iff], },
...
begin
 ...
 rwa [inv_smul_smul₀ hc] at this; rwa [ nnnorm_inv] at this; rwa [ ennreal.le_inv_smul_iff (nnnorm_ne_zero_iff.mpr hc)] at this,
end
...
begin
 rw [ennreal.smul_def]; rw [ smul_eq_mul]; rw [ snorm_eq_lintegral_rpow_nnnorm hp hp']; rw [ ennreal.le_rpow_one_div_iff (ennreal.to_real_pos hp hp')]; rw [ ennreal.mul_rpow_of_nonneg _ _ ennreal.to_real_nonneg]; rw [ ← ennreal.rpow_mul]; rw [ one_div_mul_cancel (ennreal.to_real_pos hp hp').ne.symm]; rw [ ennreal.rpow_one]; rw [ ← set_lintegral_const]; rw [ ← lintegral_indicator _ hs],
 refine lintegral_mono_ae _,
...
begin
 ...
 is_bounded_default },
 rw this at hx,
 rw [← ennreal.rpow_one (liminf (λ n, ‖f n x‖₊) at_top)]; rw [ ← mul_inv_cancel hppos.ne.symm]; rw [ ennreal.rpow_mul],
