...
begin
 ...
 ... < ‖(x : E)‖ : I
 ... = ‖s x x‖ : by rw [(hs x).2]; rw [ is_R_or_C.norm_coe_norm] },
...
begin
 ...
 rw [h_Union]; rw [ measure_Union_null_iff],
 assume n,
 exact hc _ (u_lt n),
end
...
begin
 ...
 rw [eventually_le]; rw [ ae_restrict_iff hs],
 exact eventually_of_forall (λ x hxs, hxs), },
 rwa [set_integral_const] at h_const_le; rwa [ smul_eq_mul] at h_const_le; rwa [ mul_comm] at h_const_le, },
 by_contra,
...
begin
 ...
 rw [integral_sub' hg.integrable_on hf.integrable_on]; rw [ sub_nonneg],
 exact hf_le s hs
end
...
begin
 ...
 (λ s hs hμts, _),
 { rw [integrable_on]; rw [ measure.restrict_restrict hs],
...
begin
 ...
 { assume s hs hμs,
 rw [continuous_linear_map.integral_comp_comm c (hf_int_finite s hs hμs)]; rw [ hf_zero s hs hμs],
...
begin
 ...
 rw [← @measure.restrict_univ _ _ μ]; rw [ ← Union_spanning_sets μ]; rw [ eventually_eq]; rw [ ae_iff]; rw [ measure.restrict_apply' (measurable_set.Union (measurable_spanning_sets μ))],
 rw [set.inter_Union]; rw [ measure_Union_null_iff],
 intro n,
...
begin
 ...
 { rw [eventually_eq]; rw [ ae_restrict_iff' (measurable_set.compl (hm _ ht_meas))],
 ...
 refine ae_eq_zero_of_forall_set_integral_eq_of_sigma_finite _ _,
 { intros s hs hμs,
 rw [integrable_on]; rw [ restrict_trim hm (μ.restrict t) hs]; rw [ measure.restrict_restrict (hm s hs)],
 rw [← restrict_trim hm μ ht_meas] at hμs; rw [ measure.restrict_apply hs] at hμs; rw [ trim_measurable_set_eq hm (hs.inter ht_meas)] at hμs,
...
begin
 ...
 { congr' 1,
 rw [lintegral_congr_ae (of_real_to_real_ae_eq _)]; rw [ lintegral_congr_ae (of_real_to_real_ae_eq _)],
 { exact hfg hs hs' },
