...
begin
 ...
 refine ⟨T, hT_count, _⟩,
 rw [←hT_un] at this; rw [ bUnion_eq_Union] at this,
 rw [←Union_inter]; rw [ eq_comm]; rw [ inter_eq_right_iff_subset],
 exact this },
 haveI : countable T, from countable_coe_iff.mpr T_count,
 rw [hT]; rw [ ae_strongly_measurable_Union_iff],
...
begin
 ...
 rw [integrable_on]; rw [ restrict_restrict hu_o.measurable_set],
 exact ht_int.mono_set hu_sub },
 { rw ←is_open_compl_iff at hs,
 refine ⟨sᶜ, hs.mem_nhds h, _⟩,
 rw [integrable_on]; rw [ restrict_restrict]; rw [ inter_comm]; rw [ inter_compl_self]; rw [ ←integrable_on],
...
begin
 ...
 rw [integrable_on] at hg ⊢; rw [ ← mem_ℒp_one_iff_integrable] at hg ⊢,
...
begin
 rw [integrable_on]; rw [ ←integrable_norm_iff],
 { simp_rw norm_smul,
