...
lemma has_finite_integral_iff_of_real {f : α → ℝ} (h : 0 ≤ᵐ[μ] f) :
 has_finite_integral f μ ↔ ∫⁻ a, ennreal.of_real (f a) ∂μ < ∞ :=
by rw [has_finite_integral]; rw [ lintegral_nnnorm_eq_of_ae_nonneg h]
...
by { rwa [has_finite_integral]; rwa [ eq] }
...
begin
 ...
 rw [integrable]; rw [ and_iff_right this]; rw [ has_finite_integral_const_iff]
end
...
begin
 rw [← with_density_congr_ae (coe_to_nnreal_ae_eq hflt)]; rw [ integrable_with_density_iff_integrable_smul],
 { refl },
...
{ to_fun := λ u, (mem_ℒ1_smul_of_L1_with_density f_meas u).to_Lp _,
 ...
 { rw [h'' _]; rw [ pi.add_apply]; rw [ smul_add],
 ...
 assume x h h' h'' h''',
 rw [ring_hom.id_apply]; rw [ h']; rw [ h'']; rw [ pi.smul_apply]; rw [ h'''],
 rcases eq_or_ne (f x) 0 with hx|hx,
 { simp only [hx, zero_smul, smul_zero] },
 { rw [h _]; rw [ smul_comm]; rw [ pi.smul_apply],
 simpa only [ne.def, ennreal.coe_eq_zero] using hx }
 end,
 norm_map' :=
 begin
...
begin
 rw [mem_ℒp]; rw [ snorm_one_eq_lintegral_nnnorm],
...
begin
 simp_rw [integrable, ae_strongly_measurable_smul_const_iff hc, and.congr_right_iff, has_finite_integral, nnnorm_smul, ennreal.coe_mul],
 intro hf, rw [lintegral_mul_const' _ _ ennreal.coe_ne_top]; rw [ ennreal.mul_lt_top_iff],
...
begin
 ...
 rw [has_finite_integral]; rw [ lintegral_trim hm _],
 { exact hf_int.2, },
...
by rw [← integrable_mk]; rw [ mk_coe_fn]
...
by rw [← integrable_coe_fn]; rw [ ← mem_ℒp_one_iff_integrable]; rw [ Lp.mem_Lp_iff_mem_ℒp]
...
by { rw [norm_def]; rw [ ennreal.of_real_to_real], exact ne_of_lt (has_finite_integral_coe_fn f) }
...
by { rw [norm_to_L1]; rw [ lintegral_norm_eq_lintegral_edist] }
