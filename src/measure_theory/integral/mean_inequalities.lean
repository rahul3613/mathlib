...
begin
 ...
 { rw [lintegral_mul_const'' _ (hf.pow_const p)]; rw [ lintegral_mul_const']; rw [ hf_norm]; rw [ hg_norm]; rw [ ← div_eq_mul_inv]; rw [ ← div_eq_mul_inv]; rw [ hpq.inv_add_inv_conj_ennreal],
 simp [hpq.symm.pos], },
...
begin
 rw [fun_mul_inv_snorm]; rw [ mul_rpow_of_nonneg _ _ (le_of_lt hp0)],
 ...
 * (npf * nqg) ∂μ :
 ...
 rw [pi.mul_apply]; rw [ fun_eq_fun_mul_inv_snorm_mul_snorm f hf_nonzero hf_nontop]; rw [ fun_eq_fun_mul_inv_snorm_mul_snorm g hg_nonzero hg_nontop]; rw [ pi.mul_apply],
 ring,
 end
 ... ≤ npf * nqg :
 begin
...
begin
 ...
 { rw [mul_comm]; rw [ mul_comm ((∫⁻ (a : α), (f a) ^ p ∂μ) ^ (1 / p))],
...
begin
 ...
 { rw [sub_eq_add_neg]; rw [ ennreal.rpow_add _ _ two_ne_zero ennreal.coe_ne_top]; rw [ ←mul_assoc]; rw [ ←ennreal.mul_rpow_of_nonneg _ _ hp0]; rw [ one_div]; rw [ ennreal.inv_mul_cancel two_ne_zero ennreal.coe_ne_top]; rw [ ennreal.one_rpow]; rw [ one_mul]; rw [ ennreal.rpow_neg_one], },
 rw ←ennreal.mul_le_mul_left (ne_of_lt h_zero_lt_half_rpow).symm _,
 { rw [mul_add]; rw [ ← mul_assoc]; rw [ ← mul_assoc]; rw [ h_rw]; rw [ ←ennreal.mul_rpow_of_nonneg _ _ hp0]; rw [ mul_add],
...
begin
 ...
 by rwa [h_one_div_r]; rwa [ sub_pos]; rwa [ one_div_lt_one_div hq0_lt hp0_lt],
 ...
 ←ennreal.rpow_mul],
 have hpp2 : p * p2 = q,
 { symmetry, rw [mul_comm]; rw [ ←div_eq_iff hp0_ne], },
 have hpq2 : p * q2 = r,
...
begin
 ...
 { rw [h_zero]; rw [ ennreal.zero_rpow_of_pos hpq.pos],
 exact zero_le _, },
 by_cases h_top : (f + g) a = ⊤,
 { rw [h_top]; rw [ ennreal.top_rpow_of_pos hpq.sub_one_pos]; rw [ ennreal.top_mul_top],
 exact le_top, },
 refine le_of_eq _,
 ...
 * ((∫⁻ (a : α), (f a)^p ∂μ) ^ (1/p) + (∫⁻ (a : α), (g a)^p ∂μ) ^ (1/p)),
 by rwa [←mul_le_mul_left h0_rpow htop_rpow] at h; rwa [ ←mul_assoc] at h; rwa [ ←rpow_add _ _ h_add_zero h_add_top] at h; rwa [ ←sub_eq_add_neg] at h; rwa [ _root_.sub_self] at h; rwa [ rpow_zero] at h; rwa [ one_mul] at h; rwa [ mul_one] at h,
 ...
 * (∫⁻ (a : α), ((f+g) a)^p ∂μ) ^ (1/q),
 ...
 rwa [←mul_assoc] at h; rwa [ ennreal.mul_le_mul_right h_add_zero h_add_top] at h; rwa [ mul_comm] at h,
end
...
begin
 ...
 { rw [h0]; rw [ @ennreal.zero_rpow_of_pos (1/p) (by simp [lt_of_lt_of_le zero_lt_one hp1])],
 exact zero_le _, },
