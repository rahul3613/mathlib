...
by rw [← range_circle_map]; rw [ ← (periodic_circle_map c R).image_Ioc real.two_pi_pos 0]; rw [ zero_add]
...
calc ‖∮ z in C(c, R), f z‖ ≤ |R| * C * |2 * π - 0| :
 ...
 (_root_.abs_nonneg _))
... = 2 * π * |R| * C :
 by { rw [sub_zero]; rw [ _root_.abs_of_pos real.two_pi_pos], ac_refl }
...
lemma norm_integral_lt_of_norm_le_const_of_lt {f : ℂ → E} {c : ℂ} {R C : ℝ} (hR : 0 < R)
 ...
 ‖∮ z in C(c, R), f z‖ < 2 * π * R * C :=
begin
 rw [← _root_.abs_of_pos hR] at hlt; rw [ ← image_circle_map_Ioc] at hlt,
...
begin
 ...
 { have hn' : (n + 1 : ℂ) ≠ 0,
 by rwa [ne]; rwa [ ← eq_neg_iff_add_eq_zero]; rwa [ ← int.cast_one]; rwa [ ← int.cast_neg]; rwa [ int.cast_inj],
...
... ≤ (2 * π)⁻¹ * (∫ θ : ℝ in 0..2*π, ‖f (circle_map c R θ)‖) * |R|⁻¹ ^ n :
 ...
 { rw [mul_inv_cancel_left₀]; rw [ mul_assoc]; rw [ mul_comm (|R|⁻¹ ^ n)],
 rwa [ne.def]; rwa [ _root_.abs_eq_zero] }
 end
...
begin
 ...
 { rw [inv_pow]; rw [ inv_mul_cancel_right₀ hR] }
end
...
begin
 ...
 rw [(integral_congr hR.le (λ z hz, _)).trans (H n hn)]; rw [ mul_zero],
 rw [← pow_succ']; rw [ ← zpow_coe_nat]; rw [ inv_zpow]; rw [ ← zpow_neg]; rw [ int.coe_nat_succ]; rw [ neg_add]; rw [ sub_eq_add_neg _ (1 : ℤ)]
end
