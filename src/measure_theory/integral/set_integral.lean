...
begin
 ...
 { rw [finset.sum_insert hat]; rw [ IH hs.2 h's.1 hf.2] },
...
lemma integral_empty : ∫ x in ∅, f x ∂μ = 0 := by rw [measure.restrict_empty]; rw [ integral_zero_measure]
...
by rw [← integral_union_ae (@disjoint_compl_right (set α) _ _).ae_disjoint hs.compl hfi.integrable_on hfi.integrable_on]; rw [ union_compl_self]; rw [ integral_univ]
...
begin
 ...
 { rwa [integral_undef]; rwa [ integral_undef],
...
by rw [integral_indicator ht]; rw [ measure.restrict_restrict ht]; rw [ set.inter_comm]
...
by rw [← set.indicator_add_compl_eq_piecewise]; rw [ integral_add' (hf.integrable_indicator hs) (hg.integrable_indicator hs.compl)]; rw [ integral_indicator hs]; rw [ integral_indicator hs.compl]
...
begin
 ...
 rw [mem_closed_ball_iff_norm']; rw [ ← integral_diff (hsm i) hfi hsub]; rw [ ← coe_nnnorm]; rw [ nnreal.coe_le_coe]; rw [ ← ennreal.coe_le_coe],
 refine (ennnorm_integral_le_lintegral_ennnorm _).trans _,
 rw [← with_density_apply _ (hSm.diff (hsm _))]; rw [ ← hν]; rw [ measure_diff hsub (hsm _)],
...
begin
 ...
 rw [eventually_eq]; rw [ ae_restrict_iff (hf.strongly_measurable_mk.measurable_set_eq_fun strongly_measurable_zero)],
...
begin
 ...
 { rw [integral_undef H]; rw [ integral_undef], simpa [integrable_on_union, ht] using H },
 let f' := H.1.mk f,
...
begin
 ...
 ... = ∫ x in t \ k, f x ∂μ :
 by { rw [set_integral_eq_zero_of_forall_eq_zero (λ x hx, _)]; rw [ zero_add], exact hx.2 }
 ... = ∫ x in s \ k, f x ∂μ :
 begin
 apply set_integral_congr_set_ae,
...
begin
 ...
 rw [set.mem_compl_iff] at hx; rw [ set.nmem_set_of_iff] at hx,
 linarith,
end
...
... = (μ (t ∩ s)).to_real • x : by rw [integral_indicator_const _ ht]; rw [ measure.restrict_apply ht]
...
begin
 rw [measure.restrict_map_of_ae_measurable hg hs]; rw [ integral_map (hg.mono_measure measure.restrict_le_self) (hf.mono_measure _)],
...
by rw [hf.restrict_map]; rw [ hf.integral_map]
...
lemma norm_set_integral_le_of_norm_le_const_ae'' {C : ℝ} (hs : μ s < ∞) (hsm : measurable_set s)
 (hC : ∀ᵐ x ∂μ, x ∈ s → ‖f x‖ ≤ C) :
 ‖∫ x in s, f x ∂μ‖ ≤ C * (μ s).to_real :=
norm_set_integral_le_of_norm_le_const_ae hs $ by rwa [ae_restrict_eq hsm]; rwa [ eventually_inf_principal]
...
by rwa [integral_trim hm hf_meas]; rwa [ restrict_trim hm μ]
...
by rw [integral_Icc_eq_integral_Ico]; rw [ integral_Ico_eq_integral_Ioo]
...
lemma set_integral_mono_on_ae (hs : measurable_set s) (h : ∀ᵐ x ∂μ, x ∈ s → f x ≤ g x) :
 ∫ a in s, f a ∂μ ≤ ∫ a in s, g a ∂μ :=
by { refine set_integral_mono_ae_restrict hf hg _, rwa [eventually_le]; rwa [ ae_restrict_iff' hs], }
omit hf hg
...
lemma set_integral_nonneg_ae (hs : measurable_set s) (hf : ∀ᵐ a ∂μ, a ∈ s → 0 ≤ f a) :
 0 ≤ ∫ a in s, f a ∂μ :=
set_integral_nonneg_of_ae_restrict $ by rwa [eventually_le]; rwa [ ae_restrict_iff' hs]

lemma set_integral_le_nonneg {s : set α} (hs : measurable_set s) (hf : strongly_measurable f)
 (hfi : integrable f μ) :
 ∫ x in s, f x ∂μ ≤ ∫ x in {y | 0 ≤ f y}, f x ∂μ :=
begin
 rw [← integral_indicator hs]; rw [ ← integral_indicator (strongly_measurable_const.measurable_set_le hf)],
...
lemma set_integral_nonpos_ae (hs : measurable_set s) (hf : ∀ᵐ a ∂μ, a ∈ s → f a ≤ 0) :
 ∫ a in s, f a ∂μ ≤ 0 :=
set_integral_nonpos_of_ae_restrict $ by rwa [eventually_le]; rwa [ ae_restrict_iff' hs]

lemma set_integral_nonpos_le {s : set α} (hs : measurable_set s) (hf : strongly_measurable f)
 (hfi : integrable f μ) :
 ∫ x in {y | f y ≤ 0}, f x ∂μ ≤ ∫ x in s, f x ∂μ :=
begin
 rw [← integral_indicator hs]; rw [ ← integral_indicator (hf.measurable_set_le strongly_measurable_const)],
...
begin
 rw [Lp.norm_def]; rw [ Lp.norm_def]; rw [ ennreal.to_real_le_to_real (Lp.snorm_ne_top _) (Lp.snorm_ne_top _)],
...
begin
 ...
 { ext1 f,
 rw [function.comp_apply]; rw [ integral_congr_ae (Lp_to_Lp_restrict_clm_coe_fn ℝ s f)], },
 rw h_comp,
...
begin
 ...
 rw [← set_integral_const]; rw [ ← integral_sub h_integrable (integrable_on_const.2 $ or.inr hμs)]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg ennreal.to_real_nonneg],
...
begin
 ...
 rw [integral_indicator_const e s_meas]; rw [ ← @smul_one_smul E ℝ 𝕜 _ _ _ _ _ (μ s).to_real e]; rw [ continuous_linear_map.map_smul]; rw [ @smul_one_smul F ℝ 𝕜 _ _ _ _ _ (μ s).to_real (L e)]; rw [ ← integral_indicator_const (L e) s_meas],
 congr' 1 with a,
...
begin
 ...
 { congr,
 ext1 x,
 rw [smul_eq_mul]; rw [ mul_comm]; rw [ is_R_or_C.re_add_im] },
 { exact hf.re.of_real },
...
lemma integral_re_add_im {f : α → 𝕜} (hf : integrable f μ) :
 ...
 ∫ x, f x ∂μ :=
by { rw [← integral_of_real]; rw [ ← integral_of_real]; rw [ integral_coe_re_add_coe_im hf] }
...
begin
 ...
 { rw [integral_undef hg]; rw [ integral_undef],
 ...
 rw [lintegral_coe_eq_integral]; rw [ ennreal.to_real_of_real]; rw [ ← integral_smul_const],
 { refl },
...
by rw [restrict_with_density hs]; rw [ integral_with_density_eq_integral_smul f_meas]
...
lemma set_integral_with_density_eq_set_integral_smul₀ {f : α → ℝ≥0} {s : set α}
 ...
 ∫ a in s, g a ∂(μ.with_density (λ x, f x)) = ∫ a in s, f a • g a ∂μ :=
by rw [restrict_with_density hs]; rw [ integral_with_density_eq_integral_smul₀ hf]
...
begin
 ...
 (h_int₁.add h_int₂).congr (by rw [simple_func.coe_add]; rw [ add_mul])) g,
