...
by rw [uIoc_eq_union]; rw [ integrable_on_union]; rw [ interval_integrable]
...
by rw [interval_integrable_iff]; rw [ uIoc_of_le hab]

lemma interval_integrable_iff' [has_no_atoms μ] :
 interval_integrable f μ a b ↔ integrable_on f (uIcc a b) μ :=
by rw [interval_integrable_iff]; rw [ ←Icc_min_max]; rw [ uIoc]; rw [ integrable_on_Icc_iff_integrable_on_Ioc]
...
by rw [interval_integrable_iff_integrable_Ioc_of_le hab]; rw [ integrable_on_Icc_iff_integrable_on_Ioc]
...
begin
 ...
 rw [←real.smul_map_volume_mul_right (inv_ne_zero hc)]; rw [ integrable_on]; rw [ measure.restrict_smul]; rw [ integrable_smul_measure (by simpa : ennreal.of_real (|c⁻¹|) ≠ 0) ennreal.of_real_ne_top]; rw [ ←integrable_on]; rw [ measurable_embedding.integrable_on_map_iff A],
 convert hf using 1,
...
lemma integral_non_ae_strongly_measurable
 (hf : ¬ ae_strongly_measurable f (μ.restrict (Ι a b))) :
 ∫ x in a..b, f x ∂μ = 0 :=
by rw [interval_integral_eq_integral_uIoc]; rw [ integral_non_ae_strongly_measurable hf]; rw [ smul_zero]
...
lemma norm_integral_eq_norm_integral_Ioc (f : ℝ → E) :
 ‖∫ x in a..b, f x ∂μ‖ = ‖∫ x in Ι a b, f x ∂μ‖ :=
by rw [← norm_integral_min_max]; rw [ integral_of_le min_le_max]; rw [ uIoc]
...
norm_integral_le_integral_norm_Ioc.trans_eq $ by rw [uIoc_of_le h]; rw [ integral_of_le h]
...
by simp_rw [norm_interval_integral_eq, abs_interval_integral_eq, abs_eq_self.mpr (integral_nonneg_of_ae $ h.mono $ λ t ht, (norm_nonneg _).trans ht), norm_integral_le_of_norm_le hbound.def h]
...
begin
 ...
 { rw [real.volume_Ioc]; rw [ max_sub_min_eq_abs]; rw [ ennreal.to_real_of_real (abs_nonneg _)] },
...
by simp_rw [interval_integral_eq_integral_uIoc, ← integral_apply hφ.def v, coe_smul', pi.smul_apply]
...
begin
 ...
 simp_rw [integral_smul_measure, interval_integral, A.set_integral_map, ennreal.to_real_of_real (abs_nonneg c)],
 cases hc.lt_or_lt,
...
by rw [← integral_comp_add_right]; rw [ ← integral_comp_mul_left _ hc]
...
by rw [← integral_comp_add_left]; rw [ ← integral_comp_mul_left _ hc]
...
by rw [← add_neg_eq_zero]; rw [ ← integral_symm]; rw [ integral_add_adjacent_intervals_cancel hab hbc]
...
by rw [← integral_add_adjacent_intervals hac hcd]; rw [ add_assoc]; rw [ add_left_comm]; rw [ integral_add_adjacent_intervals hac (hac.symm.trans hab)]; rw [ add_comm]
...
by { rw [integral_interval_sub_interval_comm hab hcd hac]; rw [ integral_symm b d]; rw [ integral_symm a c]; rw [ sub_neg_eq_add]; rw [ sub_eq_neg_add], }
...
begin
 ...
 { rw [integral_symm]; rw [ ← this hb ha (le_of_not_le hab)]; rw [ neg_sub] },
...
begin
 ...
 { rw [integral_of_le hab]; rw [ ← integral_indicator measurable_set_Ioc]; rw [ indicator_eq_self.2 h];
 apply_instance },
 { rw [Ioc_eq_empty hab.not_lt] at h; rw [ subset_empty_iff] at h; rw [ support_eq_empty_iff] at h,
 simp [h] }
end
...
begin
 ...
 intros x hx,
 erw [ae_strongly_measurable_indicator_iff]; erw [ measure.restrict_restrict]; erw [ Iic_inter_Ioc_of_le],
 { rw min₁₂,
...
by rw [integral_of_le hab]; rw [ integral_eq_zero_iff_of_nonneg_ae hf hfi.1]
...
begin
 ...
 { rw [integral_symm]; rw [ neg_eq_zero]; rw [ integral_eq_zero_iff_of_le_of_nonneg_ae hab hf hfi.symm] }
end
...
begin
 ...
 rw [uIoc_swap] at hf; rw [ uIoc_of_le hba] at hf,
...
begin
 ...
 { rw [eventually_le]; rw [ uIoc_of_le hab.le],
...
lemma integral_lt_integral_of_ae_le_of_measure_set_of_lt_ne_zero (hab : a ≤ b)
 ...
 ∫ x in a..b, f x ∂μ < ∫ x in a..b, g x ∂μ :=
begin
 rw [← sub_pos]; rw [ ← integral_sub hgi hfi]; rw [ integral_of_le hab]; rw [ measure_theory.integral_pos_iff_support_of_nonneg_ae],
...
begin
 ...
 rw [←integral_univ]; rw [ ←Union_Ioc_add_int_cast y],
...
begin
 ...
 ext1 n,
 rw [integral_comp_add_right]; rw [ zero_add]; rw [ add_comm],
end
