...
by rw [← simple_func.const_lintegral]; rw [ ← simple_func.lintegral_eq_lintegral]; rw [ simple_func.coe_const]
...
by rw [lintegral_const]; rw [ one_mul]

lemma set_lintegral_const (s : set α) (c : ℝ≥0∞) : ∫⁻ a in s, c ∂μ = c * μ s :=
by rw [lintegral_const]; rw [ measure.restrict_apply_univ]

lemma set_lintegral_one (s) : ∫⁻ a in s, 1 ∂μ = μ s :=
by rw [set_lintegral_const]; rw [ one_mul]
...
begin
 ...
 rw [← ennreal.add_lt_add_iff_left this]; rw [ ← add_lintegral]; rw [ ← map_add @ennreal.coe_add],
...
begin
 ...
 rw [real.nnnorm_of_nonneg hx]; rw [ ennreal.of_real_eq_coe_nnreal hx],
end
...
begin
 ...
 { assume p,
 rw [← inter_Union]; rw [ ← inter_univ ((map c rs) ⁻¹' {p})] {occs := occurrences.pos [1]},
 refine set.ext (assume x, and_congr_right $ assume hx, (true_iff _).2 _),
 by_cases p_eq : p = 0, { simp [p_eq] },
 simp at hx, subst hx,
 have : r * s x ≠ 0, { rwa [(≠)]; rwa [ ← ennreal.coe_eq_zero] },
 have : s x ≠ 0, { refine mt _ this, assume h, rw [h]; rw [ mul_zero] },
 ...
 by rw [← const_mul_lintegral]; rw [ eq_rs]; rw [ simple_func.lintegral]
 ...
 begin
 rw [measure_Union_eq_supr (directed_of_sup $ mono x)]; rw [ ennreal.mul_supr],
 end
...
begin
 ...
 begin
 rw [← simple_func.add_lintegral]; rw [ ← simple_func.map_add @ennreal.coe_add],
...
calc (∫⁻ a, f a + g a ∂μ) =
 ...
 rw [lintegral_supr],
 { congr,
 funext n, rw [← simple_func.add_lintegral]; rw [ ← simple_func.lintegral_eq_lintegral],
 refl },
 { measurability, },
 ...
 ... = (∫⁻ a, f a ∂μ) + (∫⁻ a, g a ∂μ) :
 by rw [lintegral_eq_supr_eapprox_lintegral hf]; rw [ lintegral_eq_supr_eapprox_lintegral hg]
...
lemma lintegral_add_left' {f : α → ℝ≥0∞} (hf : ae_measurable f μ) (g : α → ℝ≥0∞) :
 ∫⁻ a, f a + g a ∂μ = ∫⁻ a, f a ∂μ + ∫⁻ a, g a ∂μ :=
by rw [lintegral_congr_ae hf.ae_eq_mk]; rw [ ← lintegral_add_left hf.measurable_mk]; rw [ lintegral_congr_ae (hf.ae_eq_mk.add (ae_eq_refl g))]
...
by { rw [← measure.sum_coe_finset]; rw [ lintegral_sum_measure]; rw [ ← finset.tsum_subtype'], refl }
...
by rw [measure.restrict_empty]; rw [ lintegral_zero_measure]
...
calc (∫⁻ a, r * f a ∂μ) = (∫⁻ a, (⨆n, (const α r * eapprox f n) a) ∂μ) :
 by { congr, funext a, rw [← supr_eapprox_apply f hf]; rw [ ennreal.mul_supr], refl }
 ... = (⨆n, r * (eapprox f n).lintegral μ) :
 begin
 rw [lintegral_supr],
 { congr, funext n,
 rw [← simple_func.const_mul_lintegral]; rw [ ← simple_func.lintegral_eq_lintegral] },
 { assume n, exact simple_func.measurable _ },
 { assume i j h a, exact mul_le_mul_left' (monotone_eapprox _ h _) _ }
 end
 ... = r * (∫⁻ a, f a ∂μ) : by rw [← ennreal.mul_supr]; rw [ lintegral_eq_supr_eapprox_lintegral hf]
...
lemma lintegral_const_mul_le (r : ℝ≥0∞) (f : α → ℝ≥0∞) :
 r * (∫⁻ a, f a ∂μ) ≤ (∫⁻ a, r * f a ∂μ) :=
begin
 rw [lintegral]; rw [ ennreal.mul_supr],
 refine supr_le (λs, _),
 rw [ennreal.mul_supr],
 simp only [supr_le_iff],
 assume hs,
 rw [← simple_func.const_mul_lintegral]; rw [ lintegral],
...
lintegral_congr_ae $ h₁.mp $ h₂.mono $ λ _ h₂ h₁, by rw [h₁]; rw [ h₂]
...
lemma lintegral_indicator₀ (f : α → ℝ≥0∞) {s : set α} (hs : null_measurable_set s μ) :
 ∫⁻ a, s.indicator f a ∂μ = ∫⁻ a in s, f a ∂μ :=
by rw [← lintegral_congr_ae (indicator_ae_eq_of_ae_eq_set hs.to_measurable_ae_eq)]; rw [ lintegral_indicator _ (measurable_set_to_measurable _ _)]; rw [ measure.restrict_congr_set hs.to_measurable_ae_eq]

lemma lintegral_indicator_const {s : set α} (hs : measurable_set s) (c : ℝ≥0∞) :
 ∫⁻ a, s.indicator (λ _, c) a ∂μ = c * μ s :=
by rw [lintegral_indicator _ hs]; rw [ set_lintegral_const]
...
begin
 ...
 dsimp,
 rw [lintegral_const]; rw [ measure.restrict_apply measurable_set.univ]; rw [ set.univ_inter],
...
begin
 ...
 rw [bUnion_eq_Union]; rw [ lintegral_Union₀ (set_coe.forall'.1 hm) (hd.subtype _ _)]
end
...
lemma lintegral_map_le {mβ : measurable_space β} (f : β → ℝ≥0∞) {g : α → β} (hg : measurable g) :
 ∫⁻ a, f a ∂(measure.map g μ) ≤ ∫⁻ a, f (g a) ∂μ :=
begin
 rw [← supr_lintegral_measurable_le_eq_lintegral]; rw [ ← supr_lintegral_measurable_le_eq_lintegral],
...
by rw [restrict_map hg hs]; rw [ lintegral_map hf hg]
...
by rw [lintegral_comp (measurable_const.indicator hs) hf]; rw [ lintegral_indicator_const hs]; rw [ measure.map_apply hf hs]
...
begin
 ...
 { rw [← f₀.extend_comp_eq hg (const _ 0)]; rw [ ← simple_func.lintegral_map]; rw [ ← simple_func.lintegral_eq_lintegral]; rw [ ← lintegral],
...
lemma measure_preserving.lintegral_comp {mb : measurable_space β} {ν : measure β} {g : α → β}
 ...
 ∫⁻ a, f (g a) ∂μ = ∫⁻ b, f b ∂ν :=
by rw [← hg.map_eq]; rw [ lintegral_map hf hg.measurable]
...
lemma measure_preserving.lintegral_comp_emb {mb : measurable_space β} {ν : measure β} {g : α → β}
 ...
 ∫⁻ a, f (g a) ∂μ = ∫⁻ b, f b ∂ν :=
by rw [← hg.map_eq]; rw [ hge.lintegral_map]
...
by rw [← hg.map_eq]; rw [ set_lintegral_map hs hf hg.measurable]
...
lemma measure_preserving.set_lintegral_comp_preimage_emb {mb : measurable_space β} {ν : measure β}
 ...
 (s : set β) :
 ∫⁻ a in g ⁻¹' s, f (g a) ∂μ = ∫⁻ b in s, f b ∂ν :=
by rw [← hg.map_eq]; rw [ hge.restrict_map]; rw [ hge.lintegral_map]

lemma measure_preserving.set_lintegral_comp_emb {mb : measurable_space β} {ν : measure β}
 {g : α → β} (hg : measure_preserving g μ ν) (hge : measurable_embedding g) (f : β → ℝ≥0∞)
 (s : set α) :
 ∫⁻ a in s, f (g a) ∂μ = ∫⁻ b in g '' s, f b ∂ν :=
by rw [← hg.set_lintegral_comp_preimage_emb hge]; rw [ preimage_image_eq _ hge.injective]
...
by rw [← lintegral_count]; rw [ lintegral_const]
...
lemma _root_.nnreal.count_const_le_le_of_tsum_le [measurable_singleton_class α]
 ...
 measure.count {i : α | ε ≤ a i} ≤ c / ε :=
begin
 rw [show (λ i, ε ≤ a i) = (λ i, (ε : ℝ≥0∞) ≤ (coe ∘ a) i)]; rw [ by { funext i]; rw [ simp only [ennreal.coe_le_coe], }],
...
lemma lintegral_countable' [countable α] [measurable_singleton_class α] (f : α → ℝ≥0∞) :
 ∫⁻ a, f a ∂μ = ∑' a, f a * μ {a} :=
begin
 conv_lhs { rw [← sum_smul_dirac μ]; rw [ lintegral_sum_measure] },
 congr' 1 with a : 1,
...
begin
 rw [← union_singleton]; rw [ lintegral_union (measurable_set_singleton a)]; rw [ lintegral_singleton]; rw [ add_comm],
...
lemma lintegral_fintype [measurable_singleton_class α] [fintype α] (f : α → ℝ≥0∞) :
 ∫⁻ x, f x ∂μ = ∑ x, f x * μ {x} :=
by rw [← lintegral_finset]; rw [ finset.coe_univ]; rw [ measure.restrict_univ]
...
begin
 ...
 rw [with_density_apply _ hs]; rw [ with_density_apply _ hs],
...
begin
 ...
 rw [with_density_apply _ hs]; rw [ measure.add_apply]; rw [ with_density_apply _ hs]; rw [ with_density_apply _ hs]; rw [ ← lintegral_add_left hf],
 refl,
end
...
begin
 refine measure.ext (λ s hs, _),
 rw [with_density_apply _ hs]; rw [ measure.coe_smul]; rw [ pi.smul_apply]; rw [ with_density_apply _ hs]; rw [ smul_eq_mul]; rw [ ← lintegral_const_mul r hf],
 refl,
end
...
begin
 refine measure.ext (λ s hs, _),
 rw [with_density_apply _ hs]; rw [ measure.coe_smul]; rw [ pi.smul_apply]; rw [ with_density_apply _ hs]; rw [ smul_eq_mul]; rw [ ← lintegral_const_mul' r f hr],
 refl,
end
...
{ measure_univ_lt_top :=
 by rwa [with_density_apply _ measurable_set.univ]; rwa [ measure.restrict_univ]; rwa [ lt_top_iff_ne_top] }
...
begin
 ext1 t ht,
 rw [with_density_apply _ ht]; rw [ lintegral_indicator _ hs]; rw [ restrict_comm hs]; rw [ ← with_density_apply _ ht]
end

lemma with_density_indicator_one {s : set α} (hs : measurable_set s) :
 μ.with_density (s.indicator 1) = μ.restrict s :=
by rw [with_density_indicator hs]; rw [ with_density_one]
...
begin
 ...
 refine ⟨S, hS, _, _⟩,
 { rw [with_density_apply _ hS]; rw [ lintegral_eq_zero_iff hf.ennreal_of_real]; rw [ eventually_eq],
 exact (ae_restrict_mem hS).mono (λ x hx, ennreal.of_real_eq_zero.2 (le_of_lt hx)) },
 { rw [with_density_apply _ hS.compl]; rw [ lintegral_eq_zero_iff hf.neg.ennreal_of_real]; rw [ eventually_eq],
...
begin
 ext1 t ht,
 rw [restrict_apply ht]; rw [ with_density_apply _ ht]; rw [ with_density_apply _ (ht.inter hs)]; rw [ restrict_restrict ht],
end

lemma with_density_eq_zero {f : α → ℝ≥0∞}
 (hf : ae_measurable f μ) (h : μ.with_density f = 0) :
 f =ᵐ[μ] 0 :=
by rw [← lintegral_eq_zero_iff' hf]; rw [ ← set_lintegral_univ]; rw [ ← with_density_apply _ measurable_set.univ]; rw [ h]; rw [ measure.coe_zero]; rw [ pi.zero_apply]
...
begin
 ...
 have A : μ.with_density f t = 0, by rw [measure_to_measurable]; rw [ hs],
 rw [with_density_apply f (measurable_set_to_measurable _ s)] at A; rw [ lintegral_eq_zero_iff hf] at A; rw [ eventually_eq] at A; rw [ ae_restrict_iff] at A; rw [ ae_iff] at A,
 ...
 rw [with_density_apply _ M]; rw [ (lintegral_eq_zero_iff hf)],
...
begin
 rw [ae_iff]; rw [ ae_iff]; rw [ with_density_apply_eq_zero hf],
 congr',
 ext x,
...
begin
 rw [ae_with_density_iff hf]; rw [ ae_restrict_iff'],
 { refl },
...
begin
 ...
 { rw [eventually_eq] at hg'; rw [ ae_with_density_iff hf.coe_nnreal_ennreal] at hg',
 ...
 rw [eventually_eq]; rw [ ae_with_density_iff hf.coe_nnreal_ennreal],
 filter_upwards [hg'],
 assume x hx h'x,
...
by rw [restrict_with_density hs]; rw [ lintegral_with_density_eq_lintegral_mul _ hf hg]
...
begin
 ...
 apply ae_of_ae_restrict_of_ae_restrict_compl {x | f' x ≠ 0},
 { have Z := hg.ae_eq_mk,
 rw [eventually_eq] at Z; rw [ ae_with_density_iff_ae_restrict hf.measurable_mk] at Z,
 filter_upwards [Z],
 assume x hx,
...
by rw [restrict_with_density hs]; rw [ lintegral_with_density_eq_lintegral_mul_non_measurable _ f_meas hf]
...
lemma set_lintegral_with_density_eq_set_lintegral_mul_non_measurable₀ (μ : measure α)
 ...
 ∫⁻ a in s, g a ∂(μ.with_density f) = ∫⁻ a in s, (f * g) a ∂μ :=
by rw [restrict_with_density hs]; rw [ lintegral_with_density_eq_lintegral_mul_non_measurable₀ _ hf h'f]
...
begin
 ...
 rwa [hf_prop] at h_m; rwa [ hg_prop] at h_m; rwa [ ← h_m0] at h_m, },
...
lemma lintegral_trim_ae {μ : measure α} (hm : m ≤ m0)
 ...
 ∫⁻ a, f a ∂(μ.trim hm) = ∫⁻ a, f a ∂μ :=
by rw [lintegral_congr_ae (ae_eq_of_ae_eq_trim hf.ae_eq_mk)]; rw [ lintegral_congr_ae hf.ae_eq_mk]; rw [ lintegral_trim hm hf.measurable_mk]
...
begin
 ...
 rw [h_integral_indicator]; rw [ ← lintegral_supr],
...
begin
 ...
 have : L / c < μ s,
 { rwa [ennreal.div_lt_iff]; rwa [ mul_comm],
 ...
 rwa [mul_comm]; rwa [ ← ennreal.div_lt_iff],
 { simp only [c_ne_zero, ne.def, coe_eq_zero, not_false_iff, true_or] },
 { simp only [ne.def, coe_ne_top, not_false_iff, true_or] } } },
...
begin
 ...
 rw [← with_density_mul _ gmeas.coe_nnreal_ennreal gmeas.coe_nnreal_ennreal.inv]; rw [ A]; rw [ with_density_one] },
 conv_lhs { rw this },
