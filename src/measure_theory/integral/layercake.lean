...
begin
 ...
 simp_rw [integrand_eq, ← lintegral_indicator (λ t, ennreal.of_real (g t)) measurable_set_Ioc, ← lintegral_indicator _ measurable_set_Ioi],
 ...
 * (Ici s).indicator (λ (t : ℝ), (1 : ℝ≥0∞)) (f x)),
 ...
 simp_rw [(show (λ a, (Ici s).indicator (λ (t : ℝ), (1 : ℝ≥0∞)) (f a)) = (λ a, {a : α | s ≤ f a}.indicator (λ _, 1) a), by { funext a, by_cases s ≤ f a; simp [h], })],
 rw lintegral_indicator,
 swap, { exact f_mble measurable_set_Ici, },
 rw [lintegral_one, measure.restrict_apply measurable_set.univ, univ_inter, indicator_mul_left, mul_assoc, (show (Ioi 0).indicator (λ (_x : ℝ), (1 : ℝ≥0∞)) s * μ {a : α | s ≤ f a} = (Ioi 0).indicator (λ (_x : ℝ), 1 * μ {a : α | s ≤ f a}) s, by { by_cases 0 < s; simp [h], })],
...
begin
 ...
 rw [uIoc_of_le fω_nn]; rw [ ← ae_restrict_iff' (measurable_set_Ioc : measurable_set (Ioc (0 : ℝ) (f ω)))],
 exact g_eq_G_on (f ω), },
 simp_rw [eq₁, eq₂],
...
begin
 ...
 { congr' with ω,
 rw [← ennreal.of_real_mul p_pos.le]; rw [ mul_div_cancel' ((f ω)^p) p_pos.ne.symm], },
...
begin
 ...
 by rw [uni]; rw [ measure_union (disjoint_iff_inter_eq_empty.mpr disj) (g_mble (finite.measurable_set (finite_singleton t)))],
 by_contra con,
 rw [not_lt] at con; rw [ nonpos_iff_eq_zero] at con,
 rw [con] at μ_add; rw [ add_zero] at μ_add,
 exact ht μ_add,
end
