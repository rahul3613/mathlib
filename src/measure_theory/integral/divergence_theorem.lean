...
begin
 ...
 rw [mem_closed_ball_zero_iff]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg dist_nonneg]; rw [ dist_eq_norm]; rw [ ← integral_sub (Hid.mono_set Hsub) ((Hic _).mono_set Hsub)],
 ...
 rw [fin.dist_insert_nth_insert_nth]; rw [ dist_self]; rw [ dist_comm],
 exact max_le hk.le δpos.lt.le
 end
 ... ≤ ε :
 begin
 rw [box.Icc_def]; rw [ real.volume_Icc_pi_to_real ((J k).face i).lower_le_upper]; rw [ ← le_div_iff (hvol_pos _)],
...
begin
 ...
 rw [this]; rw [ integral_empty]; rw [ integral_empty]; rw [ sub_self] } },
...
have hIcc' : Icc (eL a) (eL b) = eL.symm ⁻¹' (Icc a b),
 by rw [← hIcc]; rw [ eL.symm_preimage_preimage],
...
... = ∑ i : fin (n + 1), ((∫ x in Icc (eL a ∘ i.succ_above) (eL b ∘ i.succ_above),
 ...
 { rw [← he_vol.integrable_on_comp_preimage he_emb]; rw [ hIcc],
 simp [← hDF, (∘), Hi] }
 end
...
begin
 ...
 rw [interval_integral.integral_symm]; rw [ neg_eq_iff_eq_neg]; rw [ neg_sub],
...
calc ∫ x in Icc a b, f' x (1, 0) + g' x (0, 1)
 ...
 { rw [Icc_prod_eq] at hx; rw [ interior_prod_eq] at hx; rw [ interior_Icc] at hx; rw [ interior_Icc] at hx,
