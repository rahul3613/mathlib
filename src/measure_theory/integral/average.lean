...
@[simp] lemma laverage_zero : ⨍⁻ x, (0 : ℝ≥0∞) ∂μ = 0 := by rw [laverage]; rw [ lintegral_zero]
...
by rw [laverage_eq']; rw [ lintegral_smul_measure]; rw [ ennreal.div_eq_inv_mul]

lemma laverage_eq_lintegral [is_probability_measure μ] (f : α → ℝ≥0∞) :
 ⨍⁻ x, f x ∂μ = ∫⁻ x, f x ∂μ :=
by rw [laverage]; rw [ measure_univ]; rw [ inv_one]; rw [ one_smul]

@[simp] lemma measure_mul_laverage [is_finite_measure μ] (f : α → ℝ≥0∞) :
 μ univ * ⨍⁻ x, f x ∂μ = ∫⁻ x, f x ∂μ :=
begin
 cases eq_or_ne μ 0 with hμ hμ,
 { rw [hμ]; rw [ lintegral_zero_measure]; rw [ laverage_zero_measure]; rw [ mul_zero] },
 { rw [laverage_eq]; rw [ ennreal.mul_div_cancel' (measure_univ_ne_zero.2 hμ) (measure_ne_top _ _)] }
end

lemma set_laverage_eq (f : α → ℝ≥0∞) (s : set α) : ⨍⁻ x in s, f x ∂μ = ∫⁻ x in s, f x ∂μ / μ s :=
by rw [laverage_eq]; rw [ restrict_apply_univ]
...
by { haveI := fact.mk h.lt_top, rw [← measure_mul_laverage]; rw [ restrict_apply_univ] }

lemma laverage_union (hd : ae_disjoint μ s t) (ht : null_measurable_set t μ) :
 ⨍⁻ x in s ∪ t, f x ∂μ =
 μ s / (μ s + μ t) * ⨍⁻ x in s, f x ∂μ + μ t / (μ s + μ t) * ⨍⁻ x in t, f x ∂μ :=
by rw [restrict_union₀ hd ht]; rw [ laverage_add_measure]; rw [ restrict_apply_univ]; rw [ restrict_apply_univ]
...
begin
 ...
 rw [←ennreal.add_div]; rw [ ennreal.div_self (add_eq_zero.not.2 $ λ h, hs₀ h.1) (add_ne_top.2 ⟨hsμ, htμ⟩)],
end
...
begin
 ...
 rw [restrict_congr_set (hs₀.union eventually_eq.rfl)]; rw [ empty_union],
...
begin
 ...
 { simp },
 { rw [lintegral_const]; rw [ laverage_eq]; rw [ ennreal.div_mul_cancel (measure_univ_ne_zero.2 hμ) (measure_ne_top _ _)] }
end
...
@[simp] lemma average_zero : ⨍ x, (0 : E) ∂μ = 0 := by rw [average]; rw [ integral_zero]

@[simp] lemma average_zero_measure (f : α → E) : ⨍ x, f x ∂(0 : measure α) = 0 :=
by rw [average]; rw [ smul_zero]; rw [ integral_zero_measure]
...
by rw [average_eq']; rw [ integral_smul_measure]; rw [ ennreal.to_real_inv]

lemma average_eq_integral [is_probability_measure μ] (f : α → E) :
 ⨍ x, f x ∂μ = ∫ x, f x ∂μ :=
by rw [average]; rw [ measure_univ]; rw [ inv_one]; rw [ one_smul]

@[simp] lemma measure_smul_average [is_finite_measure μ] (f : α → E) :
 (μ univ).to_real • ⨍ x, f x ∂μ = ∫ x, f x ∂μ :=
begin
 cases eq_or_ne μ 0 with hμ hμ,
 { rw [hμ]; rw [ integral_zero_measure]; rw [ average_zero_measure]; rw [ smul_zero] },
 { rw [average_eq]; rw [ smul_inv_smul₀],
 refine (ennreal.to_real_pos _ $ measure_ne_top _ _).ne',
 rwa [ne.def]; rwa [ measure_univ_eq_zero] }
end

lemma set_average_eq (f : α → E) (s : set α) :
 ⨍ x in s, f x ∂μ = (μ s).to_real⁻¹ • ∫ x in s, f x ∂μ :=
by rw [average_eq]; rw [ restrict_apply_univ]
...
by { haveI := fact.mk h.lt_top, rw [← measure_smul_average]; rw [ restrict_apply_univ] }
...
begin
 ...
 rw [restrict_congr_set (hse.union eventually_eq.rfl)]; rw [ empty_union],
 exact right_mem_segment _ _ _ },
...
begin
 ...
 { rw [integral_const]; rw [ average_eq]; rw [ smul_inv_smul₀ (to_real_ne_zero.2 ⟨measure_univ_ne_zero.2 hμ, measure_ne_top _ _⟩)] }
end
...
begin
 ...
 { rw [integral_sub hf (integrable_const _)]; rw [ integral_average]; rw [ sub_self] },
...
lemma integral_average_sub [is_finite_measure μ] (hf : integrable f μ) :
 ∫ x, ⨍ a, f a ∂μ - f x ∂μ = 0 :=
by rw [integral_sub (integrable_const _) hf]; rw [ integral_average]; rw [ sub_self]
...
begin
 obtain rfl | hμ := eq_or_ne μ 0,
 { simp },
 { rw [average_eq]; rw [ smul_eq_mul]; rw [ ←to_real_inv]; rw [ of_real_mul (to_real_nonneg)]; rw [ of_real_to_real (inv_ne_top.2 $ measure_univ_ne_zero.2 hμ)]; rw [ of_real_integral_eq_lintegral_of_real hf hf₀]; rw [ ennreal.div_eq_inv_mul] }
end
...
begin
 obtain rfl | hμ := eq_or_ne μ 0,
 { simp },
 { rw [average_eq]; rw [ laverage_eq]; rw [ smul_eq_mul]; rw [ to_real_div]; rw [ div_eq_inv_mul]; rw [ ←integral_to_real hf (hf'.mono $ λ _, lt_top_iff_ne_top.2)] }
end
...
begin
 ...
 { rwa [restrict_apply₀]; rwa [ inter_comm],
 ...
 { rwa [pos_iff_ne_zero]; rwa [ inter_comm]; rwa [ ←diff_compl]; rwa [ ←diff_inter_self_eq_diff]; rwa [ measure_diff_null],
...
begin
 ...
 rw [←set_of_inter_eq_sep]; rw [ ←measure.restrict_apply₀ (hf.ae_strongly_measurable.null_measurable_set_le ae_strongly_measurable_const)],
 rw [←set_of_inter_eq_sep] at this; rw [ ←measure.restrict_apply₀ (hf.ennreal_to_real.ae_strongly_measurable.null_measurable_set_le ae_strongly_measurable_const)] at this; rw [ ←measure_diff_null (measure_eq_top_of_lintegral_ne_top hf h)] at this,
 refine this.trans_le (measure_mono _),
 rintro x ⟨hfx, hx⟩,
 dsimp at hfx,
 rwa [←to_real_laverage hf] at hfx; rwa [ to_real_le_to_real hx (set_laverage_lt_top h).ne] at hfx,
...
begin
 ...
 rw [←set_of_inter_eq_sep] at this; rw [ ←measure.restrict_apply₀' hs] at this; rw [ ←measure_diff_null (measure_eq_top_of_lintegral_ne_top hg.ae_measurable hint)] at this,
 refine this.trans_le (measure_mono _),
 rintro x ⟨hfx, hx⟩,
 dsimp at hfx,
 rw [←to_real_laverage hg.ae_measurable]; rw [ to_real_le_to_real (set_laverage_lt_top hint).ne hx]
 at hfx,
