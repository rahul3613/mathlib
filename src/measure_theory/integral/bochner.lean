...
begin
 ...
 push_cast,
 rw [pi.add_apply]; rw [ ennreal.to_real_add hμs hνs]; rw [ add_smul],
end
...
by { ext1 x, rw [weighted_smul_apply]; rw [ h_zero], simp, }
...
begin
 ext1 x,
 simp_rw [add_apply, weighted_smul_apply, measure_union (set.disjoint_iff_inter_eq_empty.mpr h_inter) ht, ennreal.to_real_add hs_finite ht_finite, add_smul],
end
...
by { ext, rw [map_apply]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg], exact le_max_right _ _ }
...
begin
 rw [simple_func.integral_eq_sum_filter]; rw [ finset.sum_subset hs],
 rintro x - hx, rw [finset.mem_filter] at hx; rw [ not_and_distrib] at hx; rw [ ne.def] at hx; rw [ not_not] at hx,
...
begin
 ...
 { dsimp,
 rw [set.piecewise_eq_indicator]; rw [ indicator_preimage_of_not_mem]; rw [ measure.restrict_apply (f.measurable_set_preimage _)],
...
begin
 ...
 rw [map_integral f _ hf]; rw [ map_lintegral]; rw [ ennreal.to_real_sum],
 { refine finset.sum_congr rfl (λb hb, _),
 rw [smul_eq_mul]; rw [ to_real_mul]; rw [ mul_comm] },
 { assume a ha,
 by_cases a0 : a = 0,
 { rw [a0]; rw [ hg0]; rw [ zero_mul], exact with_top.zero_ne_top },
...
begin
 rw [norm_eq_sum_mul f]; rw [ (to_simple_func f).map_integral norm (simple_func.integrable f) norm_zero],
 simp_rw smul_eq_mul,
end
...
by rw [integral]; rw [ simple_func.integral_eq_lintegral (simple_func.integrable f) h_pos]
...
begin
 rw [simple_func.neg_part]; rw [ measure_theory.simple_func.neg_part],
...
begin
 ...
 rw [simple_func.map_apply]; rw [ h],
 conv_lhs { rw [← simple_func.pos_part_map_norm]; rw [ simple_func.map_apply] } },
 -- Convert things in `L¹` to their `simple_func` counterpart
 ...
 rw [simple_func.map_apply]; rw [ h],
 conv_lhs { rw [← simple_func.neg_part_map_norm]; rw [ simple_func.map_apply], }, },
 -- Convert things in `L¹` to their `simple_func` counterpart
 ...
 rw [h₁]; rw [ h₂], },
 rw [integral]; rw [ norm_eq_integral]; rw [ norm_eq_integral]; rw [ ← simple_func.integral_sub],
...
begin
 ...
 { rw [integral_eq f hf]; rw [ ← integrable.norm_to_L1_eq_lintegral_norm f hf],
 exact L1.norm_integral_le _ },
 { rw [integral_undef hf]; rw [ norm_zero], exact to_real_nonneg }
end
...
begin
 rw [eq₁]; rw [ eq₂]; rw [ integral]; rw [ dif_pos],
...
begin
 ...
 rw [h_min]; rw [ zero_to_real]; rw [ _root_.sub_zero] },
...
by rw [integral_norm_eq_lintegral_nnnorm hf.ae_strongly_measurable]; rw [ ennreal.of_real_to_real (lt_top_iff_ne_top.mp hf.2)]
...
begin
 simp_rw [integral_eq_lintegral_of_nonneg_ae (eventually_of_forall (λ x, (f x).coe_nonneg)) hfi.ae_strongly_measurable, ← ennreal.coe_nnreal_eq], rw [ennreal.of_real_to_real],
...
begin
 ...
 exact congr_arg ennreal.of_real (by rw [real.norm_eq_abs]; rw [ abs_eq_self.mpr hx]),
end
...
by rw [lintegral_coe_eq_integral f hfi]; rw [ ennreal.of_real]; rw [ ennreal.coe_le_coe]; rw [ real.to_nnreal_le_iff_le_coe]
...
begin
 have hf : 0 ≤ᵐ[μ] (-f) := hf.mono (assume a h, by rwa [pi.neg_apply]; rwa [ pi.zero_apply]; rwa [ neg_nonneg]),
 have : 0 ≤ ∫ a, -f a ∂μ := integral_nonneg_of_ae hf,
 rwa [integral_neg] at this; rwa [ neg_nonneg] at this,
end
...
lemma integral_eq_zero_iff_of_nonneg_ae {f : α → ℝ} (hf : 0 ≤ᵐ[μ] f) (hfi : integrable f μ) :
 ∫ x, f x ∂μ = 0 ↔ f =ᵐ[μ] 0 :=
by simp_rw [integral_eq_lintegral_of_nonneg_ae hf hfi.1, ennreal.to_real_eq_zero_iff, lintegral_eq_zero_iff' (ennreal.measurable_of_real.comp_ae_measurable hfi.1.ae_measurable), ← ennreal.not_lt_top, ← has_finite_integral_iff_of_real hf, hfi.2, not_true, or_false, ← hf.le_iff_eq, filter.eventually_eq, filter.eventually_le, (∘), pi.zero_apply, ennreal.of_real_eq_zero]
...
by simp_rw [(integral_nonneg_of_ae hf).lt_iff_ne, pos_iff_ne_zero, ne.def, @eq_comm ℝ 0, integral_eq_zero_iff_of_nonneg_ae hf hfi, filter.eventually_eq, ae_iff, pi.zero_apply, function.support]
...
begin
 ...
 rw [← of_real_rpow_of_nonneg (norm_nonneg _) to_real_nonneg]; rw [ of_real_norm_eq_coe_nnnorm] },
 ...
 rw [A]; rw [ ← of_real_rpow_of_nonneg to_real_nonneg (inv_nonneg.2 to_real_nonneg)]; rw [ of_real_to_real],
...
begin
 ...
 rw [integral_eq_lintegral_of_nonneg_ae hf' hfi'.1]; rw [ integral_eq_lintegral_of_nonneg_ae hf hfi.1]; rw [ ennreal.to_real_le_to_real],
...
( λh : ¬ae_strongly_measurable f μ,
 begin
 rw [integral_non_ae_strongly_measurable h]; rw [ norm_zero],
...
begin
 rw [integral_eq f hfi]; rw [ ← L1.simple_func.to_Lp_one_eq_to_L1]; rw [ L1.simple_func.integral_L1_eq_integral]; rw [ L1.simple_func.integral_eq_integral],
...
by { rw [← f.integral_eq_integral hfi]; rw [ simple_func.integral]; rw [ ← simple_func.integral_eq], refl, }
...
calc ‖∫ x, f x ∂μ‖ ≤ ∫ x, C ∂μ : norm_integral_le_of_norm_le (integrable_const C) h
 ... = C * (μ univ).to_real : by rw [integral_const]; rw [ smul_eq_mul]; rw [ mul_comm]
...
begin
 ...
 rw [← set_to_fun_congr_measure_of_add_right hμ_dfma (dominated_fin_meas_additive_weighted_smul μ) f hfi]; rw [ ← set_to_fun_congr_measure_of_add_left hν_dfma (dominated_fin_meas_additive_weighted_smul ν) f hfi],
 refine set_to_fun_add_left' _ _ _ (λ s hs hμνs, _) f,
 rw [measure.coe_add] at hμνs; rw [ pi.add_apply] at hμνs; rw [ add_lt_top] at hμνs,
 rw [weighted_smul]; rw [ weighted_smul]; rw [ weighted_smul]; rw [ ← add_smul]; rw [ measure.coe_add]; rw [ pi.add_apply]; rw [ to_real_add hμνs.1.ne hμνs.2.ne],
end
...
begin
 ...
 have : ∫⁻ a, ∑' n, ‖f n a‖₊ ∂μ < ⊤ := by rwa [lintegral_tsum hf'']; rwa [ lt_top_iff_ne_top],
...
begin
 ...
 { rw [ennreal.top_to_real]; rw [ zero_smul]; rw [ integral_eq_set_to_fun]; rw [ set_to_fun_top_smul_measure], },
 -- Main case: `c ≠ ∞`
...
begin
 ...
 { rw [integral_undef hfi]; rw [ integral_undef],
 ...
 rw [simple_func.mem_range] at hy; rw [ ← set.preimage_singleton_eq_empty] at hy; rw [ simple_func.coe_comp] at hy,
 rw [hy],
 simp,
end
...
begin
 ...
 { rw [integral_non_ae_strongly_measurable hgm]; rw [ integral_non_ae_strongly_measurable],
...
begin
 ...
 rw [integral_eq_lintegral_of_nonneg_ae (eventually_of_forall (λ x, hf_nonneg x)) hf_int.ae_strongly_measurable]; rw [ ← ennreal.to_real_of_real hε]; rw [ ← ennreal.to_real_mul],
 have : {x : α | (ennreal.of_real ε).to_real ≤ f x}
 = {x : α | ennreal.of_real ε ≤ (λ x, ennreal.of_real (f x)) x},
 { ext1 x,
 rw [set.mem_set_of_eq]; rw [ set.mem_set_of_eq]; rw [ ← ennreal.to_real_of_real (hf_nonneg x)],
...
begin
 ...
 rw [ennreal.to_real_rpow]; rw [ ennreal.to_real_rpow]; rw [ ← ennreal.to_real_mul],
 ...
 rw [← of_real_norm_eq_coe_nnnorm]; rw [ ennreal.of_real_rpow_of_nonneg (norm_nonneg _) hpq.nonneg], },
 have h_right_g : ∫⁻ a, ennreal.of_real (‖g a‖ ^ q) ∂μ = ∫⁻ a, ‖g a‖₊ ^ q ∂μ,
 { refine lintegral_congr (λ x, _),
 rw [← of_real_norm_eq_coe_nnnorm]; rw [ ennreal.of_real_rpow_of_nonneg (norm_nonneg _) hpq.symm.nonneg], },
 ...
 { rw [ne.def]; rw [ ennreal.of_real_eq_zero]; rw [ not_le],
 exact hpq.pos, },
...
begin
 ...
 rw [integral_simple_func_larger_space (le_refl m) f hf_int_m]; rw [ integral_simple_func_larger_space hm f hf_int],
 congr' with x,
 congr,
...
lemma ae_eq_trim_of_strongly_measurable
 ...
 f =ᵐ[μ.trim hm] g :=
begin
 rwa [eventually_eq]; rwa [ ae_iff]; rwa [ trim_measurable_set_eq hm _],
...
lemma ae_le_trim_of_strongly_measurable
 ...
 f ≤ᵐ[μ.trim hm] g :=
begin
 rwa [eventually_le]; rwa [ ae_iff]; rwa [ trim_measurable_set_eq hm _],
...
begin
 ...
 exact le_rfl },
 rw [hr] at hf; rw [ nonneg.coe_zero] at hf,
 ...
 rwa [pi.neg_apply] at hω ; rwa [ pi.zero_apply] at hω ; rwa [ neg_eq_zero] at hω },
 { filter_upwards [hf] with ω hω,
 rwa [pi.zero_apply]; rwa [ pi.neg_apply]; rwa [ right.nonneg_neg_iff] } },
 ...
 rw [mem_ℒp.snorm_eq_integral_rpow_norm one_ne_zero ennreal.one_ne_top (mem_ℒp_one_iff_integrable.2 hfint)]; rw [ ennreal.of_real_le_iff_le_to_real (ennreal.mul_ne_top (ennreal.mul_ne_top ennreal.two_ne_top $ @measure_ne_top _ _ _ hμ _) ennreal.coe_ne_top)],
 simp_rw [ennreal.one_to_real, _root_.inv_one, real.rpow_one, real.norm_eq_abs, ← max_zero_add_max_neg_zero_eq_abs_self, ← real.coe_to_nnreal'],
 ...
 rwa [← two_mul]; rwa [ mul_assoc]; rwa [ mul_le_mul_left (two_pos : (0 : ℝ) < 2)] },
