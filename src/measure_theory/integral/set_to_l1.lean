...
begin
 ...
 rw [measure.smul_apply] at hμs; rw [ smul_eq_mul] at hμs; rw [ with_top.mul_eq_top_iff] at hμs,
...
begin
 refine of_eq_top_imp_eq_top (λ s hs hμs, _) hT,
 rw [measure.smul_apply]; rw [ smul_eq_mul]; rw [ with_top.mul_eq_top_iff],
...
begin
 ...
 rw [hs_zero]; rw [ ennreal.zero_to_real]; rw [ mul_zero],
end
...
begin
 ...
 { intros s hs hμs, rw [eq_top_iff]; rw [ ← hμs], exact h s hs, },
...
begin
 ...
 by_cases hx0 : x = 0,
 { simp [hx0], },
 rw [h_zero (f ⁻¹' ({x} : set E)) (measurable_set_fiber _ _) (measure_preimage_lt_top_of_integrable f hf hx0)]; rw [ continuous_linear_map.zero_apply],
end
...
begin
 ...
 by_cases h0 : g (f a) = 0,
 { simp_rw h0,
 rw [continuous_linear_map.map_zero]; rw [ finset.sum_eq_zero (λ x hx, _)],
 rw mem_filter at hx,
 rw [hx.2]; rw [ continuous_linear_map.map_zero], },
...
begin
 ...
 rw [eventually_eq] at h; rw [ ae_iff] at h,
...
calc set_to_simple_func T (f + g) = ∑ x in (pair f g).range,
 ...
 by { rw [add_eq_map₂]; rw [ map_set_to_simple_func T h_add hp_pair], simp, }
...
... = ((pair f g).map prod.fst).set_to_simple_func T
 ...
 by rw [map_set_to_simple_func T h_add hp_pair prod.snd_zero]; rw [ map_set_to_simple_func T h_add hp_pair prod.fst_zero]
...
calc set_to_simple_func T (-f) = set_to_simple_func T (f.map (has_neg.neg)) : rfl
 ...
 begin
 rw [map_set_to_simple_func T h_add hf neg_zero]; rw [ set_to_simple_func]; rw [ ← sum_neg_distrib],
...
lemma set_to_simple_func_sub (T : set α → E →L[ℝ] F) (h_add : fin_meas_additive μ T)
 ...
 set_to_simple_func T (f - g) = set_to_simple_func T f - set_to_simple_func T g :=
begin
 rw [sub_eq_add_neg]; rw [ set_to_simple_func_add T h_add hf]; rw [ set_to_simple_func_neg T h_add hg]; rw [ sub_eq_add_neg],
...
calc set_to_simple_func T (c • f) = ∑ x in f.range, T (f ⁻¹' {x}) (c • x) :
 by { rw [smul_eq_map c f]; rw [ map_set_to_simple_func T h_add hf], rw smul_zero, }
...
lemma set_to_simple_func_smul {E} [normed_add_comm_group E] [normed_field 𝕜]
 ...
 set_to_simple_func T (c • f) = c • set_to_simple_func T f :=
calc set_to_simple_func T (c • f) = ∑ x in f.range, T (f ⁻¹' {x}) (c • x) :
 by { rw [smul_eq_map c f]; rw [ map_set_to_simple_func T h_add hf],
 rw smul_zero, }
...
begin
 rw [← sub_nonneg]; rw [ ← set_to_simple_func_sub T h_add hgi hfi],
...
begin
 ...
 rw [indicator_preimage]; rw [ preimage_const_of_mem],
 swap, { exact set.mem_singleton x, },
 rw [← pi.const_zero]; rw [ preimage_const_of_not_mem],
...
begin
 rw [norm_to_simple_func]; rw [ snorm_one_eq_lintegral_nnnorm],
 have h_eq := simple_func.map_apply (λ x, (‖x‖₊ : ℝ≥0∞)) (to_simple_func f),
 dsimp only at h_eq,
 simp_rw ← h_eq,
 rw [simple_func.lintegral_eq_lintegral]; rw [ simple_func.map_lintegral]; rw [ ennreal.to_real_sum],
 { congr,
 ext1 x,
 rw [ennreal.to_real_mul]; rw [ mul_comm]; rw [ ← of_real_norm_eq_coe_nnnorm]; rw [ ennreal.to_real_of_real (norm_nonneg _)], },
 { intros x hx,
 by_cases hx0 : x = 0,
 { rw hx0, simp, },
...
lemma set_to_L1s_sub {T : set α → E →L[ℝ] F}
 ...
 (f g : α →₁ₛ[μ] E) :
 set_to_L1s T (f - g) = set_to_L1s T f - set_to_L1s T g :=
by rw [sub_eq_add_neg]; rw [ set_to_L1s_add T h_zero h_add]; rw [ set_to_L1s_neg h_zero h_add]; rw [ sub_eq_add_neg]
...
lemma norm_set_to_L1s_le (T : set α → E →L[ℝ] F) {C : ℝ}
 ...
 ‖set_to_L1s T f‖ ≤ C * ‖f‖ :=
begin
 rw [set_to_L1s]; rw [ norm_eq_sum_mul f],
...
begin
 ...
 ext1 f,
 rw [set_to_L1s_clm_zero_left hT f]; rw [ continuous_linear_map.zero_comp]; rw [ continuous_linear_map.zero_apply],
end
...
begin
 ...
 refine continuous_linear_map.extend_unique (set_to_L1s_clm α E μ hT) _ _ _ _ _,
 ext1 f,
 rw [set_to_L1s_clm_zero_left' hT h_zero f]; rw [ continuous_linear_map.zero_comp]; rw [ continuous_linear_map.zero_apply],
end
...
begin
 ...
 by { rw [this]; rw [ continuous_linear_map.add_apply], },
 ...
 by { rw ← this, congr, },
 rw [set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1s_clm_add_left hT hT'],
end
...
begin
 ...
 by { rw ← this, congr, },
 rw [set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1s_clm_add_left' hT hT' hT'' h_add],
end
...
begin
 ...
 by { rw [this]; rw [ continuous_linear_map.smul_apply], },
 ...
 rw [set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1s_clm_smul_left c hT],
end
...
begin
 ...
 rw [set_to_L1_eq_set_to_L1s_clm]; rw [ set_to_L1s_clm_smul_left' c hT hT' h_smul],
end
...
begin
 rw [set_to_L1_eq_set_to_L1' hT h_smul]; rw [ set_to_L1_eq_set_to_L1' hT h_smul],
...
begin
 ...
 { intros c s hs hμs,
 rw [set_to_L1_simple_func_indicator_const hT hs hμs]; rw [ set_to_L1_simple_func_indicator_const hT' hs hμs],
 exact hTT' s hs hμs c, },
 { intros f g hf hg hfg_disj hf_le hg_le,
 rw [(set_to_L1 hT).map_add]; rw [ (set_to_L1 hT').map_add],
...
begin
 ...
 rw [this]; rw [ set_to_L1_eq_set_to_L1s_clm],
...
... = ‖set_to_L1s_clm α E μ hT‖ : by rw [nnreal.coe_one]; rw [ one_mul]
...
by rw [set_to_fun_eq hT (L1.integrable_coe_fn f)]; rw [ integrable.to_L1_coe_fn]
...
by rw [set_to_fun_eq hT (hf.add hg)]; rw [ set_to_fun_eq hT hf]; rw [ set_to_fun_eq hT hg]; rw [ integrable.to_L1_add]; rw [ (L1.set_to_L1 hT).map_add]
...
begin
 by_cases hf : integrable f μ,
 { rw [set_to_fun_eq hT hf]; rw [ set_to_fun_eq hT hf.neg]; rw [ integrable.to_L1_neg]; rw [ (L1.set_to_L1 hT).map_neg], },
 { rw [set_to_fun_undef hT hf]; rw [ set_to_fun_undef hT]; rw [ neg_zero],
...
lemma set_to_fun_sub (hT : dominated_fin_meas_additive μ T C)
 (hf : integrable f μ) (hg : integrable g μ) :
 set_to_fun μ T hT (f - g) = set_to_fun μ T hT f - set_to_fun μ T hT g :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ set_to_fun_add hT hf hg.neg]; rw [ set_to_fun_neg hT g]
...
begin
 by_cases hf : integrable f μ,
 { rw [set_to_fun_eq hT hf]; rw [ set_to_fun_eq hT]; rw [ integrable.to_L1_smul']; rw [ L1.set_to_L1_smul hT h_smul c _], },
 { by_cases hr : c = 0,
 { rw hr, simp, },
 { have hf' : ¬ integrable (c • f) μ, by rwa [integrable_smul_iff hr f],
 rw [set_to_fun_undef hT hf]; rw [ set_to_fun_undef hT hf']; rw [ smul_zero], }, },
end
...
begin
 ...
 rw [set_to_fun_eq hT hfi]; rw [ set_to_fun_eq hT hgi]; rw [ (integrable.to_L1_eq_to_L1_iff f g hfi hgi).2 h] },
 { have hgi : ¬ integrable g μ, { rw integrable_congr h at hfi, exact hfi },
 rw [set_to_fun_undef hT hfi]; rw [ set_to_fun_undef hT hgi] },
end

lemma set_to_fun_measure_zero (hT : dominated_fin_meas_additive μ T C) (h : μ = 0) :
 set_to_fun μ T hT f = 0 :=
by { have : f =ᵐ[μ] 0, by simp [h], rw [set_to_fun_congr_ae hT this]; rw [ set_to_fun_zero], }
...
lemma set_to_fun_mono {T : set α → G' →L[ℝ] G''} {C : ℝ} (hT : dominated_fin_meas_additive μ T C)
 ...
 set_to_fun μ T hT f ≤ set_to_fun μ T hT g :=
begin
 rw [← sub_nonneg]; rw [ ← set_to_fun_sub hT hg hf],
...
begin
 ...
 ... = ε / 2 :
 by { refine mul_div_cancel' (ε / 2) _, rw [ne.def]; rw [ to_real_eq_zero_iff], simp [hc', hc'0], }
...
begin
 ...
 rw [set_to_fun_indicator_const hT hs hμs.ne]; rw [ set_to_fun_indicator_const hT' hs hμ's], },
 { intros f₂ g₂ h_dish hf₂ hg₂ h_eq_f h_eq_g,
 rw [set_to_fun_add hT hf₂ hg₂]; rw [ set_to_fun_add hT' (h_int f₂ hf₂) (h_int g₂ hg₂)]; rw [ h_eq_f]; rw [ h_eq_g], },
 ...
 rw [← set_to_fun_congr_ae hT hfg]; rw [ hf_eq]; rw [ set_to_fun_congr_ae hT' hfg'], },
end
...
begin
 ...
 rw [set_to_fun_zero_left' _ h]; rw [ set_to_fun_measure_zero],
 simp [hc0], },
 refine set_to_fun_congr_measure c⁻¹ c _ hc_ne_top (le_of_eq _) le_rfl hT hT_smul f,
 { simp [hc0], },
 { rw [smul_smul]; rw [ ennreal.inv_mul_cancel hc0 hc_ne_top]; rw [ one_smul], },
end
...
begin
 ...
 dsimp only,
 rw [hx]; rw [ of_real_norm_eq_coe_nnnorm]; rw [ pi.sub_apply],
end
