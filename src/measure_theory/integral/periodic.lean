...
begin
 ...
 { intros hε,
 rw [inter_eq_left_iff_subset]; rw [ real.closed_ball_eq_Icc]; rw [ zero_sub]; rw [ zero_add],
 ...
 { conv_rhs { rw [← if_ctx_congr (iff.rfl : ε < T/2 ↔ ε < T/2) h₁ (λ _, rfl)]; rw [ ← hT'], },
...
begin
 ...
 { ext1 x, refl, },
 simp_rw [this, ←map_map add_circle.measurable_mk' measurable_subtype_coe, ←map_comap_subtype_coe m],
 refl, }
end
...
begin
 ...
 rw [←(add_circle.measure_preserving_mk T t).map_eq]; rw [ set_integral_eq_subtype m]; rw [ ←this],
 have : (coe : Ioc t (t + T) → add_circle T) = (coe : ℝ → add_circle T) ∘ (coe : _ → ℝ),
 { ext1 x, refl, },
 simp_rw [this, ←map_map add_circle.measurable_mk' measurable_subtype_coe, ←map_comap_subtype_coe m],
 refl,
...
begin
 rw [integral_of_le]; rw [ add_circle.integral_preimage T t f],
 linarith [hT.out],
end
...
begin
 ...
 { simp },
 { rw [← neg_inj]; rw [ ← integral_symm]; rw [ ← integral_symm],
...
by rw [hf.interval_integral_add_eq t s]; rw [ integral_add_adjacent_intervals (h_int t s) (h_int s _)]
...
begin
 ...
 simp_rw [integral_comp_add_right, h₀, zero_add, this (n+1), add_comm T, hf.interval_integral_add_eq ((n+1) • T) 0, zero_add], },
end
...
begin
 ...
 conv_rhs { rw [← int.fract_div_mul_self_add_zsmul_eq T t (by linarith)]; rw [ ← integral_add_adjacent_intervals (h_int 0 ε) (h_int _ _)] },
 rw [hg.interval_integral_add_zsmul_eq ⌊t/T⌋ ε h_int]; rw [ hg.interval_integral_add_eq ε 0]; rw [ zero_add]; rw [ add_le_add_iff_right],
...
begin
 let ε := int.fract (t/T) * T,
 conv_lhs { rw [← int.fract_div_mul_self_add_zsmul_eq T t (by linarith)]; rw [ ← integral_add_adjacent_intervals (h_int 0 ε) (h_int _ _)] },
 rw [hg.interval_integral_add_zsmul_eq ⌊t/T⌋ ε h_int]; rw [ hg.interval_integral_add_eq ε 0]; rw [ zero_add]; rw [ add_le_add_iff_right],
