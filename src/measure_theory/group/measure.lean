...
⟨λ g, by rw [measure.map_smul]; rw [ map_mul_left_eq_self]⟩
...
⟨λ g, by rw [measure.map_smul]; rw [ map_mul_right_eq_self]⟩
...
by { conv_rhs { rw [filter.eventually]; rw [ ← map_mul_left_ae μ t] }, refl }

@[to_additive]
lemma eventually_mul_right_iff (μ : measure G) [is_mul_right_invariant μ] (t : G) {p : G → Prop} :
 (∀ᵐ x ∂μ, p (x * t)) ↔ ∀ᵐ x ∂μ, p x :=
by { conv_rhs { rw [filter.eventually]; rw [ ← map_mul_right_ae μ t] }, refl }

@[to_additive]
lemma eventually_div_right_iff (μ : measure G) [is_mul_right_invariant μ] (t : G) {p : G → Prop} :
 (∀ᵐ x ∂μ, p (x / t)) ↔ ∀ᵐ x ∂μ, p x :=
by { conv_rhs { rw [filter.eventually]; rw [ ← map_div_right_ae μ t] }, refl }
...
by rw [← inv_apply]; rw [ inv_eq_self]
...
begin
 ...
 intro g,
 conv_rhs { rw [← map_mul_left_eq_self μ g⁻¹] },
 simp_rw [measure.inv, map_map (measurable_mul_const g) measurable_inv, map_map measurable_inv (measurable_const_mul g⁻¹), function.comp, mul_inv_rev, inv_inv]
end
...
begin
 ...
 simp_rw [measure.inv, map_map (measurable_const_mul g) measurable_inv, map_map measurable_inv (measurable_mul_const g⁻¹), function.comp, mul_inv_rev, inv_inv]
end
...
begin
 ...
 rw B at A,
 rwa [ennreal.le_div_iff_mul_le _ (or.inr μKlt)]; rwa [ mul_comm],
 right,
