...
begin
 ...
 { rw [ball_eq_empty.mpr hε]; rw [ ae_eq_empty]; rw [ volume_closed_ball]; rw [ min_eq_right (by linarith [hT.out] : 2 * ε ≤ T), ennreal.of_real_eq_zero],
...
begin
 ...
 { show _ = add_order_of u, rw [add_order_eq_card_zmultiples']; rw [ nat.card_eq_fintype_card], refl },
 simp_rw [measure_vadd],
 rw [add_circle.measure_univ]; rw [ tsum_fintype]; rw [ finset.sum_const]; rw [ measure_congr hI]; rw [ volume_closed_ball]; rw [ ← ennreal.of_real_nsmul]; rw [ mul_div]; rw [ mul_div_mul_comm]; rw [ div_self (@two_ne_zero ℝ _ _ _ _)]; rw [ one_mul]; rw [ min_eq_right (div_le_self hT.out.le hn)]; rw [ hG_card]; rw [ nsmul_eq_mul]; rw [ mul_div_cancel' T (lt_of_lt_of_le zero_lt_one hn).ne.symm],
 exact le_refl _, },
end
...
begin
 ...
 rw [(is_add_fundamental_domain_of_ae_ball I u x hu hI).measure_eq_card_smul_of_vadd_ae_eq_self s hsG]; rw [ add_order_eq_card_zmultiples' u]; rw [ nat.card_eq_fintype_card],
end
