...
begin
 ...
 simp_rw [map_apply hf hsm', prod_apply_symm (hf hsm'), preimage_preimage, mk_preimage_prod, inv_preimage, inv_inv, measure_mono_null (inter_subset_right _ _) hμs, lintegral_zero]
end
...
begin
 rw [← set_lintegral_one]; rw [ ← lintegral_indicator _ sm]; rw [ ← lintegral_lintegral_mul (measurable_const.indicator sm).ae_measurable hf.ae_measurable]; rw [ ← lintegral_lintegral_mul_inv μ ν],
...
begin
 ...
 simp_rw [pi.one_apply, lintegral_one, mul_one, (measure_mul_right_null ν _).mpr hνs, lintegral_zero, mul_eq_zero, measure_univ_eq_zero.not.mpr hν, or_false] at h1,
 exact h1
end
...
begin
 ...
 simp_rw [pi.one_apply, set_lintegral_one, ← image_inv, indicator_image inv_injective, image_inv, ← indicator_mul_right _ (λ x, ν ((λ y, y * x) ⁻¹' s)), function.comp, pi.one_apply, mul_one] at h1,
