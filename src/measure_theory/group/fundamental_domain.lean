...
{ null_measurable_set := h_meas,
 ...
 λ g, by { rw [← inv_inv g]; rw [ ← preimage_smul], exact h_meas.preimage (h_qmp g⁻¹), },
 have h_meas' : null_measurable_set {a | ∃ (g : G), g • a ∈ s} μ,
 { rw ← Union_smul_eq_set_of_exists, exact null_measurable_set.Union h_meas, },
 rw [ae_iff_measure_eq h_meas']; rw [ ← Union_smul_eq_set_of_exists],
...
{ null_measurable_set := h.null_measurable_set.preimage hf,
 ae_covers := (hf.ae h.ae_covers).mono $ λ x ⟨g, hg⟩, ⟨e g, by rwa [mem_preimage]; rwa [ hef g x]⟩,
...
by rw [← restrict_Union_ae (h.ae_disjoint.mono $ λ i j h, hν h) (λ g, (h.null_measurable_set_smul g).mono_ac hν)]; rw [ restrict_congr_set (hν h.Union_smul_ae_eq)]; rw [ restrict_univ]

@[to_additive] lemma lintegral_eq_tsum_of_ac (h : is_fundamental_domain G s μ) (hν : ν ≪ μ)
 (f : α → ℝ≥0∞) : ∫⁻ x, f x ∂ν = ∑' g : G, ∫⁻ x in g • s, f x ∂ν :=
by rw [← lintegral_sum_measure]; rw [ h.sum_restrict_of_ac hν]
...
... ↔ ∀ g : G, ae_strongly_measurable f (μ.restrict (g • s ∩ t)) :
 ...
 rw [← image_smul]; rw [ ← ((measure_preserving_smul g⁻¹ μ).restrict_image_emb he _).ae_strongly_measurable_comp_iff he],
 simp only [(∘), hf]
 end
...
begin
 rw [← measure_theory.integral_sum_measure]; rw [ h.sum_restrict_of_ac hν],
...
begin
 ...
 { rw [integral_undef hfs]; rw [ integral_undef],
...
begin
 ...
 rw [ess_sup_eq_Inf (μ.restrict s) f]; rw [ ess_sup_eq_Inf μ f],
 refine Inf_le_Inf _,
...
begin
 ...
 refine hx.2 (a⁻¹ * b) _ _,
 rwa [ne.def]; rwa [ inv_mul_eq_iff_eq_mul]; rwa [ mul_one]; rwa [ eq_comm],
...
{ null_measurable_set := hs.null_measurable_set.fundamental_interior _ _,
 ae_covers := begin
 simp_rw [ae_iff, not_exists, ←mem_inv_smul_set_iff, set_of_forall, ←compl_set_of, set_of_mem_eq, ←compl_Union],
 have : (⋃ g : G, g⁻¹ • s) \ (⋃ g : G, g⁻¹ • fundamental_frontier G s) ⊆
 ⋃ g : G, g⁻¹ • fundamental_interior G s,
 { simp_rw [diff_subset_iff, ←Union_union_distrib, ←smul_set_union, fundamental_frontier_union_fundamental_interior] },
