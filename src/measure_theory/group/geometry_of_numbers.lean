...
begin
 ...
 { rwa [add_haar_smul_of_nonneg μ (by norm_num : 0 ≤ (2 : ℝ)⁻¹) s]; rwa [ ←mul_lt_mul_right (pow_ne_zero (finrank ℝ E) (two_ne_zero' _)) (pow_ne_top two_ne_top)]; rwa [ mul_right_comm]; rwa [ of_real_pow (by norm_num : 0 ≤ (2 : ℝ)⁻¹)]; rwa [ ←of_real_inv_of_pos zero_lt_two]; rwa [ of_real_bit0]; rwa [ of_real_one]; rwa [ ←mul_pow]; rwa [ ennreal.inv_mul_cancel two_ne_zero two_ne_top]; rwa [ one_pow]; rwa [ one_mul] },
 ...
 simp_rw [add_subgroup.vadd_def, vadd_eq_add, add_comm _ w, ←sub_eq_sub_iff_add_eq_add, ←add_subgroup.coe_sub] at hvw,
