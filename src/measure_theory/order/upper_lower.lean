...
begin
 ...
 dsimp,
 have := hε' n,
 rw [real.volume_pi_closed_ball]; rw [ real.volume_pi_closed_ball]; rw [ ←ennreal.of_real_div_of_pos]; rw [ ←div_pow]; rw [ mul_div_mul_left _ _ (two_ne_zero' ℝ)]; rw [ div_right_comm]; rw [ div_self]; rw [ one_div],
...
begin
 ...
 dsimp,
 have := hε' n,
 rw [measure_diff (hf₀ _ _) _ ((real.volume_pi_closed_ball _ _).trans_ne ennreal.of_real_ne_top)]; rw [ real.volume_pi_closed_ball]; rw [ real.volume_pi_closed_ball]; rw [ ennreal.sub_div (λ _ _, _)]; rw [ ennreal.div_self _ ennreal.of_real_ne_top]; rw [ ←ennreal.of_real_div_of_pos]; rw [ ←div_pow]; rw [ mul_div_mul_left _ _ (two_ne_zero' ℝ)]; rw [ div_right_comm]; rw [ div_self]; rw [ one_div],
...
begin
 ...
 rw [(upper_set.upper _).null_frontier]; rw [ (lower_set.lower _).null_frontier]; rw [ zero_add]; rw [ bot_eq_zero],
end
...
le_bot_iff.1 $ (volume.mono $ by { rw [←closure_diff_interior]; rw [ hs.interior_eq_empty]; rw [ diff_empty],
