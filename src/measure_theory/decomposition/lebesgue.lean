...
begin
 rw [singular_part]; rw [ rn_deriv]; rw [ dif_pos h]; rw [ dif_pos h],
...
{ lebesgue_decomposition :=
 ...
 rw [with_density_smul _ hmeas]; rw [ ← smul_add]; rw [ ← hadd],
 refl }
 end }
...
begin
 ...
 { rw [singular_part]; rw [ dif_neg hl],
...
begin
 ...
 { rw [rn_deriv]; rw [ dif_neg hl]; rw [ with_density_zero],
 exact measure.zero_le μ }
end
...
begin
 ...
 rw [← measure.add_apply]; rw [ ← hadd]; rw [ measure_to_measurable],
 exact hs.lt_top },
 { erw [measure.rn_deriv]; erw [ dif_neg hl]; erw [ lintegral_zero],
...
begin
 ...
 rw ← nonpos_iff_eq_zero,
 exact hνinter ▸ measure_mono (inter_subset_right _ _) },
 rw [restrict_apply hA]; rw [ restrict_apply hA]; rw [ ← add_zero (s (A ∩ (S ∩ T)ᶜ))]; rw [ ← hf]; rw [ ← add_apply]; rw [ ← hadd]; rw [ add_apply]; rw [ hrn]; rw [ add_zero] },
 ...
 rw [restrict_apply hA]; rw [ ← diff_eq]; rw [ ae_disjoint.measure_diff_left hsinter] },
 ext1 A hA,
 have hμinter : μ.singular_part ν (A ∩ (S ∩ T)) = 0,
 { rw ← nonpos_iff_eq_zero,
 exact hT₂ ▸ measure_mono ((inter_subset_right _ _).trans (inter_subset_right _ _)) },
 rw [heq' A hA]; rw [ heq]; rw [ restrict_apply hA]; rw [ ← diff_eq]; rw [ ae_disjoint.measure_diff_left hμinter]
end
...
begin
 ...
 rw [zero_add]; rw [ with_density_zero],
end
...
begin
 by_cases hr : r = 0,
 { rw [hr]; rw [ zero_smul]; rw [ zero_smul]; rw [ singular_part_zero] },
 by_cases hl : have_lebesgue_decomposition μ ν,
 { haveI := hl,
 ...
 rw [with_density_smul _ (measurable_rn_deriv _ _)]; rw [ ← smul_add]; rw [ ← have_lebesgue_decomposition_add μ ν]; rw [ ennreal.smul_def] },
 { rw [singular_part]; rw [ singular_part]; rw [ dif_neg hl]; rw [ dif_neg]; rw [ smul_zero],
 refine λ hl', hl _,
...
begin
 ...
 conv_rhs { rw [add_assoc]; rw [ add_comm (μ₂.singular_part ν)]; rw [ ← add_assoc]; rw [ ← add_assoc] },
 rw [← have_lebesgue_decomposition_add μ₁ ν]; rw [ add_assoc]; rw [ add_comm (ν.with_density (μ₂.rn_deriv ν))]; rw [ ← have_lebesgue_decomposition_add μ₂ ν]
end
...
begin
 ...
 rw [restrict_apply hA]; rw [ restrict_apply hA]; rw [ ← add_zero (ν.with_density f (A ∩ (S ∩ T)))]; rw [ ← hs]; rw [ ← add_apply]; rw [ add_comm]; rw [ ← hadd]; rw [ add_apply]; rw [ hsing]; rw [ zero_add] },
 ...
 rw [restrict_apply hA]; rw [ ← add_zero (ν.with_density f (A ∩ (S ∩ T)))]; rw [ ← hνfinter]; rw [ ← diff_eq]; rw [ measure_inter_add_diff _ (hS₁.inter hT₁)] },
 ext1 A hA,
 have hνrn : ν.with_density (μ.rn_deriv ν) (A ∩ (S ∩ T)ᶜ) = 0,
 { rw ← nonpos_iff_eq_zero,
 exact with_density_absolutely_continuous ν (μ.rn_deriv ν) hνinter ▸
 measure_mono (inter_subset_right _ _) },
 rw [heq' A hA]; rw [ heq]; rw [ ← add_zero ((ν.with_density (μ.rn_deriv ν)).restrict (S ∩ T) A)]; rw [ ← hνrn]; rw [ restrict_apply hA]; rw [ ← diff_eq]; rw [ measure_inter_add_diff _ (hS₁.inter hT₁)]
end
...
begin
 ...
 rwa [to_signed_measure_sub_apply hAmeas] at this; rwa [ sub_nonpos] at this; rwa [ ennreal.to_real_le_to_real] at this,
 ...
 have h' : 1 / (↑n + 1) * νA < c,
 { rw [← nnreal.coe_lt_coe]; rw [ ← mul_lt_mul_right hb₁]; rw [ nnreal.coe_mul]; rw [ mul_assoc]; rw [ ← nnreal.coe_inv]; rw [ ← nnreal.coe_mul]; rw [ _root_.mul_inv_cancel]; rw [ ← nnreal.coe_mul]; rw [ mul_one]; rw [ nnreal.coe_inv],
 { exact hn },
 { exact ne.symm (ne_of_lt hb) } },
 refine le_trans _ (le_of_lt h'),
 rw [← ennreal.coe_le_coe]; rw [ ennreal.coe_mul],
 exact hA₃ n },
 { rw [not_lt] at hb; rw [ le_zero_iff] at hb,
 specialize hA₃ 0,
...
begin
 ...
 rw [@le_antisymm_iff ℝ≥0∞]; rw [ hc]; rw [ hd], -- Specifying the type is weirdly necessary
 refine ⟨_, _⟩,
...
⟨begin
 ...
 { intros B hB,
 rw [hξ]; rw [ with_density_apply _ hB],
 simp_rw [supr_apply],
 ...
 rw [with_density_apply _ measurable_set.univ] at hle'; rw [ measure.restrict_univ] at hle',
 ...
 rwa [zero_apply] at this; rwa [ to_signed_measure_sub_apply (hA.inter hE₁)] at this; rwa [ measure.sub_apply (hA.inter hE₁) hle] at this; rwa [ ennreal.to_real_sub_of_le _ (ne_of_lt (measure_lt_top _ _))] at this; rwa [ sub_nonneg] at this; rwa [ le_sub_iff_add_le] at this; rwa [ ← ennreal.to_real_add] at this; rwa [ ennreal.to_real_le_to_real] at this; rwa [ measure.coe_smul] at this; rwa [ pi.smul_apply] at this; rwa [ with_density_apply _ (hA.inter hE₁)] at this; rwa [ show ε • ν (A ∩ E) = (ε : ℝ≥0∞) * ν (A ∩ E)] at this; rwa [ by refl] at this; rwa [ ← set_lintegral_const] at this; rwa [ ← lintegral_add_left measurable_const] at this,
 { rw [ne.def]; rw [ ennreal.add_eq_top]; rw [ not_or_distrib],
 ...
 rwa [with_density_apply _ measurable_set.univ] at this ; rwa [ measure.restrict_univ] at this },
...
⟨by { rw [restrict_apply measurable_set.univ]; rw [ univ_inter], exact S.finite _ }⟩
...
⟨begin
 ...
 have hμ : μ = sum μn,
 { rw [hμn]; rw [ ← restrict_Union h₂ S.set_mem]; rw [ S.spanning]; rw [ restrict_univ] },
 set νn : ℕ → measure α := λ n, ν.restrict (T.set n) with hνn,
 have hν : ν = sum νn,
 { rw [hνn]; rw [ ← restrict_Union h₃ T.set_mem]; rw [ T.spanning]; rw [ restrict_univ] },
 ...
 rw [top_eq_univ] at hx; rw [ ← S.spanning] at hx; rw [ mem_Union] at hx,
 obtain ⟨i, hi⟩ := hx,
 exact ⟨i, hi, h i hi⟩ } },
 rw [hcompl.compl_eq]; rw [ measure_Union]; rw [ tsum_eq_zero_iff ennreal.summable],
 ...
 { simp only [hξ, hf, hμ],
 rw [with_density_tsum _]; rw [ sum_add_sum],
 ...
 { rw heq },
 rw [hν]; rw [ with_density_indicator (S.set_mem n)]; rw [ restrict_sum _ (S.set_mem n)],
...
instance have_lebesgue_decomposition_neg
 ...
 (-s).have_lebesgue_decomposition μ :=
{ pos_part :=
 by { rw [to_jordan_decomposition_neg]; rw [ jordan_decomposition.neg_pos_part],
 apply_instance },
 neg_part :=
 by { rw [to_jordan_decomposition_neg]; rw [ jordan_decomposition.neg_neg_part],
 apply_instance } }
...
{ pos_part :=
 by { rw [to_jordan_decomposition_smul]; rw [ jordan_decomposition.smul_pos_part],
 apply_instance },
 neg_part :=
 by { rw [to_jordan_decomposition_smul]; rw [ jordan_decomposition.smul_neg_part],
 apply_instance } }
...
begin
 ...
 refine
 { pos_part :=
 by { rw [to_jordan_decomposition_smul_real]; rw [ jordan_decomposition.real_smul_pos_part_neg _ _ hr],
 apply_instance },
 neg_part :=
 by { rw [to_jordan_decomposition_smul_real]; rw [ jordan_decomposition.real_smul_neg_part_neg _ _ hr],
 apply_instance } } }
end
...
begin
 ...
 cases hl with hp hn,
 { rw [measure.singular_part]; rw [ dif_neg hp],
 exact mutually_singular.zero_left },
 { rw [measure.singular_part]; rw [ measure.singular_part]; rw [ dif_neg hn],
...
begin
 conv_rhs { rw [← to_signed_measure_to_jordan_decomposition s]; rw [ jordan_decomposition.to_signed_measure] },
 rw [singular_part]; rw [ rn_deriv]; rw [ with_densityᵥ_sub' (integrable_to_real_of_lintegral_ne_top _ _) (integrable_to_real_of_lintegral_ne_top _ _)]; rw [ with_densityᵥ_to_real]; rw [ with_densityᵥ_to_real]; rw [ sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ add_comm (s.to_jordan_decomposition.pos_part.singular_part μ).to_signed_measure]; rw [ ← add_assoc]; rw [ add_assoc (-(s.to_jordan_decomposition.neg_part.singular_part μ).to_signed_measure)]; rw [ ← to_signed_measure_add]; rw [ add_comm]; rw [ ← add_assoc]; rw [ ← neg_add]; rw [ ← to_signed_measure_add]; rw [ add_comm]; rw [ ← sub_eq_add_neg],
...
begin
 rw [mutually_singular_ennreal_iff] at htμ; rw [ total_variation_mutually_singular_iff] at htμ,
 change _ ⟂ₘ vector_measure.equiv_measure.to_fun (vector_measure.equiv_measure.inv_fun μ) ∧
...
begin
 ...
 ext i hi,
 rw [vector_measure.sub_apply]; rw [ to_signed_measure_apply_measurable hi]; rw [ to_signed_measure_apply_measurable hi]; rw [ add_apply]; rw [ add_apply]; rw [ ennreal.to_real_add]; rw [ ennreal.to_real_add]; rw [ add_sub_add_comm]; rw [ ← to_signed_measure_apply_measurable hi]; rw [ ← to_signed_measure_apply_measurable hi]; rw [ ← vector_measure.sub_apply]; rw [ ← jordan_decomposition.to_signed_measure]; rw [ to_signed_measure_to_jordan_decomposition]; rw [ vector_measure.add_apply]; rw [ ← to_signed_measure_apply_measurable hi]; rw [ ← to_signed_measure_apply_measurable hi]; rw [ with_densityᵥ_eq_with_density_pos_part_sub_with_density_neg_part hfi]; rw [ vector_measure.sub_apply];
...
begin
 ...
 rw [vector_measure.equiv_measure.right_inv] at htμ; rw [ total_variation_mutually_singular_iff] at htμ,
 refine
 { pos_part :=
...
begin
 ...
 { rw [with_densityᵥ] at hadd; rw [ dif_neg hfi] at hadd; rw [ add_zero] at hadd,
 refine have_lebesgue_decomposition_mk' μ measurable_zero (integrable_zero _ _ μ) htμ _,
 rwa [with_densityᵥ_zero]; rwa [ add_zero] }
end
...
begin
 ...
 _ ⟂ₘ vector_measure.equiv_measure.to_fun (vector_measure.equiv_measure.inv_fun μ) at htμ,
 rw [vector_measure.equiv_measure.right_inv] at htμ,
 { rw [singular_part]; rw [ ← t.to_signed_measure_to_jordan_decomposition]; rw [ jordan_decomposition.to_signed_measure],
 congr,
...
begin
 ...
 rw [zero_add]; rw [ with_densityᵥ_zero],
end
...
begin
 ...
 rw [to_jordan_decomposition_neg]; rw [ jordan_decomposition.neg_pos_part] },
 have h₂ : ((-s).to_jordan_decomposition.neg_part.singular_part μ).to_signed_measure =
 (s.to_jordan_decomposition.pos_part.singular_part μ).to_signed_measure,
 { refine to_signed_measure_congr _,
 rw [to_jordan_decomposition_neg]; rw [ jordan_decomposition.neg_neg_part] },
 rw [singular_part]; rw [ singular_part]; rw [ neg_sub]; rw [ h₁]; rw [ h₂],
end

lemma singular_part_smul_nnreal (s : signed_measure α) (μ : measure α) (r : ℝ≥0) :
 (r • s).singular_part μ = r • s.singular_part μ :=
begin
 rw [singular_part]; rw [ singular_part]; rw [ smul_sub]; rw [ ← to_signed_measure_smul]; rw [ ← to_signed_measure_smul],
 conv_lhs { congr, congr,
 rw [to_jordan_decomposition_smul]; rw [ jordan_decomposition.smul_pos_part]; rw [ singular_part_smul], skip, congr,
 rw [to_jordan_decomposition_smul]; rw [ jordan_decomposition.smul_neg_part]; rw [ singular_part_smul] }
end
...
begin
 ...
 { rw [singular_part]; rw [ singular_part],
 conv_lhs { congr, congr,
 rw [to_jordan_decomposition_smul_real]; rw [ jordan_decomposition.real_smul_pos_part_neg _ _ (not_le.1 hr)]; rw [ singular_part_smul],
 skip, congr,
 rw [to_jordan_decomposition_smul_real]; rw [ jordan_decomposition.real_smul_neg_part_neg _ _ (not_le.1 hr)]; rw [ singular_part_smul] },
 rw [to_signed_measure_smul]; rw [ to_signed_measure_smul]; rw [ ← neg_sub]; rw [ ← smul_sub],
 change -(((-r).to_nnreal : ℝ) • _) = _,
 rw [← neg_smul]; rw [ real.coe_to_nnreal _ (le_of_lt (neg_pos.mpr (not_le.1 hr)))]; rw [ neg_neg] }
end
...
begin
 ...
 rw [add_assoc]; rw [ add_comm (t.singular_part μ)]; rw [ add_assoc]; rw [ add_comm _ (t.singular_part μ)]; rw [ singular_part_add_with_density_rn_deriv_eq]; rw [ ← add_assoc]; rw [ singular_part_add_with_density_rn_deriv_eq],
end

lemma singular_part_sub (s t : signed_measure α) (μ : measure α)
 [s.have_lebesgue_decomposition μ] [t.have_lebesgue_decomposition μ] :
 (s - t).singular_part μ = s.singular_part μ - t.singular_part μ :=
by { rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ singular_part_add]; rw [ singular_part_neg] }
...
begin
 ...
 rw [← add_right_inj t]; rw [ ← hadd]; rw [ eq_singular_part _ f htμ hadd]; rw [ singular_part_add_with_density_rn_deriv_eq],
end
...
begin
 ...
 rw [with_densityᵥ_neg]; rw [ ← add_right_inj ((-s).singular_part μ)]; rw [ singular_part_add_with_density_rn_deriv_eq]; rw [ singular_part_neg]; rw [ ← neg_add]; rw [ singular_part_add_with_density_rn_deriv_eq]
end
...
begin
 ...
 rw [with_densityᵥ_smul (rn_deriv s μ) (r : ℝ)]; rw [ ← add_right_inj ((r • s).singular_part μ)]; rw [ singular_part_add_with_density_rn_deriv_eq]; rw [ singular_part_smul],
 change _ = _ + r • _,
 rw [← smul_add]; rw [ singular_part_add_with_density_rn_deriv_eq],
end
...
begin
 ...
 rw [← add_right_inj ((s + t).singular_part μ)]; rw [ singular_part_add_with_density_rn_deriv_eq]; rw [ with_densityᵥ_add (integrable_rn_deriv _ _) (integrable_rn_deriv _ _)]; rw [ singular_part_add]; rw [ add_assoc]; rw [ add_comm (t.singular_part μ)]; rw [ add_assoc]; rw [ add_comm _ (t.singular_part μ)]; rw [ singular_part_add_with_density_rn_deriv_eq]; rw [ ← add_assoc]; rw [ singular_part_add_with_density_rn_deriv_eq],
end
...
begin
 rw [← mem_ℒp_one_iff_integrable]; rw [ ← mem_ℒp_re_im_iff],
...
begin
 ...
 ext i hi : 1,
 rw [vector_measure.add_apply]; rw [ signed_measure.to_complex_measure_apply],
 ext,
 { rw [complex.add_re]; rw [ with_densityᵥ_apply (c.integrable_rn_deriv μ) hi]; rw [ ←is_R_or_C.re_eq_complex_re]; rw [ ←integral_re (c.integrable_rn_deriv μ).integrable_on]; rw [ is_R_or_C.re_eq_complex_re]; rw [ ← with_densityᵥ_apply _ hi],
 ...
 { rw [complex.add_im]; rw [ with_densityᵥ_apply (c.integrable_rn_deriv μ) hi]; rw [ ←is_R_or_C.im_eq_complex_im]; rw [ ←integral_im (c.integrable_rn_deriv μ).integrable_on]; rw [ is_R_or_C.im_eq_complex_im]; rw [ ← with_densityᵥ_apply _ hi],
