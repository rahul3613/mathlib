...
begin
 ...
 have d_empty : d ∅ = 0,
 { change _ - _ = _, rw [measure_empty]; rw [ measure_empty]; rw [ sub_self] },

 have d_split : ∀s t, measurable_set s → measurable_set t →
 d s = d (s \ t) + d (s ∩ t),
 { assume s t hs ht,
 simp only [d],
 rw [← measure_inter_add_diff s ht]; rw [ ← measure_inter_add_diff s ht]; rw [ ennreal.to_nnreal_add (hμ _) (hμ _)]; rw [ ennreal.to_nnreal_add (hν _) (hν _)]; rw [ nnreal.coe_add]; rw [ nnreal.coe_add],
 ...
 dsimp only [f],
 rw [finset.inf_eq_infi]; rw [ finset.inf_eq_infi],
 ...
 simp only [f],
 rw [nat.Ico_succ_right_eq_insert_Ico this]; rw [ finset.inf_insert]; rw [ set.inter_comm],
 refl },
 ...
 { have := he₂ m,
 simp only [f],
 rw [nat.Ico_succ_singleton]; rw [ finset.inf_singleton],
 linarith },
 { assume n (hmn : m ≤ n) ih,
 ...
 by rw [f_succ _ _ hmn]; rw [ d_split (f m n) (e (n + 1)) (hf _ _) (he₁ _)]; rw [ add_assoc]
 ...
 ... = d (s \ t) + d t : by rw [d_split _ _ hs ht]; rw [ inter_eq_self_of_subset_right hts]
 ...
 ... = d (s ∪ t) :
 by rw [d_split _ _ (hs.union ht) ht]; rw [ union_diff_right]; rw [ union_inter_cancel_right]; rw [ (subset_compl_iff_disjoint_left.1 hts).sdiff_eq_left]
