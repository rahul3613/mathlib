...
begin
 rw [integral_to_real]; rw [ ← with_density_apply _ hi]; rw [ with_density_rn_deriv_eq μ ν h],
 { measurability },
 { refine ae_lt_top (μ.measurable_rn_deriv ν)
 (lt_of_le_of_lt (lintegral_mono_set i.subset_univ) _).ne,
 rw [← with_density_apply _ measurable_set.univ]; rw [ with_density_rn_deriv_eq μ ν h],
...
begin
 rw [absolutely_continuous_ennreal_iff] at h; rw [ (_ : μ.to_ennreal_vector_measure.ennreal_to_measure = μ)] at h; rw [ total_variation_absolutely_continuous_iff] at h,
 { ext1 i hi,
 rw [with_densityᵥ_apply (integrable_rn_deriv _ _) hi]; rw [ rn_deriv]; rw [ integral_sub]; rw [ with_density_rn_deriv_to_real_eq h.1 hi]; rw [ with_density_rn_deriv_to_real_eq h.2 hi],
 { conv_rhs { rw ← s.to_signed_measure_to_jordan_decomposition },
 erw vector_measure.sub_apply,
 rw [to_signed_measure_apply_measurable hi]; rw [ to_signed_measure_apply_measurable hi] },
