...
begin
 rw [find_exists_one_div_lt]; rw [ dif_pos hi],
...
begin
 rw [find_exists_one_div_lt] at hm; rw [ dif_pos hi] at hm,
...
begin
 rw [some_exists_one_div_lt]; rw [ dif_pos hi],
...
begin
 ...
 { rw [some_exists_one_div_lt]; rw [ dif_neg hi],
...
begin
 intros n m h,
 rw [function.on_fun]; rw [ set.disjoint_iff_inter_eq_empty],
 rcases lt_or_gt_of_ne h with (h | h),
 { rw [restrict_nonpos_seq_disjoint' h] },
 { rw [set.inter_comm]; rw [ restrict_nonpos_seq_disjoint' h] }
end
...
begin
 ...
 rw [of_diff hmeas hi₁]; rw [ s.of_disjoint_Union_nat],
 ...
 { convert h₁ _ h,
 ext x,
 rw [set.mem_Union]; rw [ exists_prop]; rw [ and_iff_right_iff_imp],
 exact λ _, h },
...
begin
 ...
 { rw [hA]; rw [ ← s.of_disjoint_Union_nat]; rw [ add_comm]; rw [ of_add_of_diff],
...
begin
 ...
 rw [hA]; rw [ ← set.diff_union_of_subset (set.subset_Union _ n)]; rw [ of_union set.disjoint_sdiff_left _ (hmeas n)],
...
begin
 ...
 rw [← (hB n).2]; rw [ hA]; rw [ ← set.diff_union_of_subset (set.subset_Union _ n)]; rw [ of_union set.disjoint_sdiff_left _ (hB₁ n)],
 { refine add_le_of_nonpos_left _,
 have : s ≤[A] 0 :=
 ...
 { rw [← hA₃]; rw [ of_union (set.disjoint_of_subset_right (set.subset.trans hD hC₁) disjoint_compl_right) hA₁ hD₁],
...
begin
 ...
 split,
 { rw [symm_diff_def]; rw [ set.diff_eq_compl_inter]; rw [ set.diff_eq_compl_inter]; rw [ set.sup_eq_union]; rw [ of_union]; rw [ le_antisymm (hi'.2 (hi.compl.inter hj) (set.inter_subset_left _ _)) (hj'.1 (hi.compl.inter hj) (set.inter_subset_right _ _))]; rw [ le_antisymm (hj'.2 (hj.compl.inter hi) (set.inter_subset_left _ _)) (hi'.1 (hj.compl.inter hi) (set.inter_subset_right _ _))]; rw [ zero_apply]; rw [ zero_apply]; rw [ zero_add],
 ...
 { rw [symm_diff_def]; rw [ set.diff_eq_compl_inter]; rw [ set.diff_eq_compl_inter]; rw [ compl_compl]; rw [ compl_compl]; rw [ set.sup_eq_union]; rw [ of_union]; rw [ le_antisymm (hi'.2 (hj.inter hi.compl) (set.inter_subset_right _ _)) (hj'.1 (hj.inter hi.compl) (set.inter_subset_left _ _))]; rw [ le_antisymm (hj'.2 (hi.inter hj.compl) (set.inter_subset_right _ _)) (hi'.1 (hi.inter hj.compl) (set.inter_subset_left _ _))]; rw [ zero_apply]; rw [ zero_apply]; rw [ zero_add],
 { exact set.disjoint_of_subset_left (set.inter_subset_left _ _)
 (set.disjoint_of_subset_right (set.inter_subset_right _ _)
