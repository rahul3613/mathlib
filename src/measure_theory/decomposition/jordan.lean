...
show dite _ _ _ = _, by rw [dif_pos (nnreal.coe_nonneg r)]; rw [ real.to_nnreal_coe]
...
by { rw [real_smul_def]; rw [ ← smul_pos_part]; rw [ dif_pos hr] }

lemma real_smul_neg_part_nonneg (r : ℝ) (hr : 0 ≤ r) :
 (r • j).neg_part = r.to_nnreal • j.neg_part :=
by { rw [real_smul_def]; rw [ ← smul_neg_part]; rw [ dif_pos hr] }

lemma real_smul_pos_part_neg (r : ℝ) (hr : r < 0) :
 (r • j).pos_part = (-r).to_nnreal • j.neg_part :=
by { rw [real_smul_def]; rw [ ← smul_neg_part]; rw [ dif_neg (not_le.2 hr)]; rw [ neg_pos_part] }

lemma real_smul_neg_part_neg (r : ℝ) (hr : r < 0) :
 (r • j).neg_part = (-r).to_nnreal • j.pos_part :=
by { rw [real_smul_def]; rw [ ← smul_pos_part]; rw [ dif_neg (not_le.2 hr)]; rw [ neg_neg_part] }
...
begin
 ext1 i hi,
 erw [to_signed_measure]; erw [ to_signed_measure_sub_apply hi]; erw [ sub_self]; erw [ zero_apply],
end

lemma to_signed_measure_neg : (-j).to_signed_measure = -j.to_signed_measure :=
begin
 ext1 i hi,
 rw [neg_apply]; rw [ to_signed_measure]; rw [ to_signed_measure]; rw [ to_signed_measure_sub_apply hi]; rw [ to_signed_measure_sub_apply hi]; rw [ neg_sub],
 refl,
end
...
begin
 ext1 i hi,
 rw [vector_measure.smul_apply]; rw [ to_signed_measure]; rw [ to_signed_measure]; rw [ to_signed_measure_sub_apply hi]; rw [ to_signed_measure_sub_apply hi]; rw [ smul_sub]; rw [ smul_pos_part]; rw [ smul_neg_part]; rw [ ← ennreal.to_real_smul]; rw [ ← ennreal.to_real_smul],
 refl
end
...
begin
 ...
 rw [to_signed_measure]; rw [ to_signed_measure_sub_apply hA]; rw [ show j.pos_part A = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hS₂ ▸ measure_mono hA₁)]; rw [ ennreal.zero_to_real]; rw [ zero_sub]; rw [ neg_le]; rw [ zero_apply]; rw [ neg_zero],
 exact ennreal.to_real_nonneg },
 { refine restrict_le_restrict_of_subset_le _ _ (λ A hA hA₁, _),
 rw [to_signed_measure]; rw [ to_signed_measure_sub_apply hA]; rw [ show j.neg_part A = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hS₃ ▸ measure_mono hA₁)]; rw [ ennreal.zero_to_real]; rw [ sub_zero],
 exact ennreal.to_real_nonneg },
end
...
begin
 ...
 ext k hk,
 rw [to_signed_measure_sub_apply hk]; rw [ to_measure_of_zero_le_apply _ hi₂ hi₁ hk]; rw [ to_measure_of_le_zero_apply _ hi₃ hi₁.compl hk],
 simp only [ennreal.coe_to_real, subtype.coe_mk, ennreal.some_eq_coe, sub_neg_eq_add],
 rw [← of_union _ (measurable_set.inter hi₁ hk) (measurable_set.inter hi₁.compl hk)]; rw [ set.inter_comm i]; rw [ set.inter_comm iᶜ]; rw [ set.inter_union_compl _ _],
 { apply_instance },
...
begin
 ...
 { rw [to_signed_measure]; rw [ to_signed_measure_sub_apply (hi.inter hS₁.compl)]; rw [ show j₁.neg_part (i ∩ Sᶜ) = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hS₅ ▸ measure_mono (set.inter_subset_right _ _))]; rw [ ennreal.zero_to_real]; rw [ sub_zero],
 conv_lhs { rw ← set.inter_union_compl i S },
 rw [measure_union]; rw [ show j₁.pos_part (i ∩ S) = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hS₄ ▸ measure_mono (set.inter_subset_right _ _))]; rw [ zero_add],
 ...
 { rw [to_signed_measure]; rw [ to_signed_measure_sub_apply (hi.inter hT₁.compl)]; rw [ show j₂.neg_part (i ∩ Tᶜ) = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hT₅ ▸ measure_mono (set.inter_subset_right _ _))]; rw [ ennreal.zero_to_real]; rw [ sub_zero],
 conv_lhs { rw ← set.inter_union_compl i T },
 rw [measure_union]; rw [ show j₂.pos_part (i ∩ T) = 0]; rw [ by exact nonpos_iff_eq_zero.1 (hT₄ ▸ measure_mono (set.inter_subset_right _ _))]; rw [ zero_add],
 ...
 rw [← ennreal.to_real_eq_to_real (measure_ne_top _ _) (measure_ne_top _ _)]; rw [ hμ₁]; rw [ hμ₂]; rw [ ← hj],
...
begin
 ...
 rw [jordan_decomposition.coe_smul]; rw [ ← to_jordan_decomposition_smul],
 refl
end
...
begin
 ...
 { ext1,
 { rw [real_smul_pos_part_neg _ _ (not_le.1 hr)]; rw [ show r • s = -(-r • s)]; rw [ by rw [neg_smul]; rw [ neg_neg], to_jordan_decomposition_neg,
 ...
 { rw [real_smul_neg_part_neg _ _ (not_le.1 hr)]; rw [ show r • s = -(-r • s)]; rw [ by rw [neg_smul]; rw [ neg_neg], to_jordan_decomposition_neg,
...
by rw [h]; rw [ to_jordan_decomposition_to_signed_measure]
...
begin
 rw [total_variation] at hs; rw [ measure.coe_add] at hs; rw [ pi.add_apply] at hs; rw [ add_eq_zero_iff] at hs,
 rw [← to_signed_measure_to_jordan_decomposition s]; rw [ to_signed_measure]; rw [ vector_measure.coe_sub]; rw [ pi.sub_apply]; rw [ measure.to_signed_measure_apply]; rw [ measure.to_signed_measure_apply],
 by_cases hi : measurable_set i,
 { rw [if_pos hi]; rw [ if_pos hi], simp [hs.1, hs.2] },
 { simp [if_neg hi] }
end
...
begin
 ...
 rw [total_variation]; rw [ measure.add_apply]; rw [ hpos]; rw [ hneg]; rw [ to_measure_of_zero_le_apply _ _ _ hS₁]; rw [ to_measure_of_le_zero_apply _ _ _ hS₁],
...
begin
 ...
 { refine measure.absolutely_continuous.mk (λ S hS₁ hS₂, _),
 have := h hS₂,
 rw [total_variation] at this ; rw [ measure.add_apply] at this ; rw [ add_eq_zero_iff] at this },
 exacts [this.1, this.2] },
 { refine measure.absolutely_continuous.mk (λ S hS₁ hS₂, _),
 rw [total_variation]; rw [ measure.add_apply]; rw [ h.1 hS₂]; rw [ h.2 hS₂]; rw [ add_zero] }
end
...
begin
 ...
 refine ⟨u, hmeas, _, _⟩,
 { rw [total_variation]; rw [ measure.add_apply]; rw [ hipos]; rw [ hineg]; rw [ to_measure_of_zero_le_apply _ _ _ hmeas]; rw [ to_measure_of_le_zero_apply _ _ _ hmeas],
 simp [hu₁ _ (set.inter_subset_right _ _)] },
 { rw [total_variation]; rw [ measure.add_apply]; rw [ hjpos]; rw [ hjneg]; rw [ to_measure_of_zero_le_apply _ _ _ hmeas.compl]; rw [ to_measure_of_le_zero_apply _ _ _ hmeas.compl],
 simp [hu₂ _ (set.inter_subset_right _ _)] } },
 { rintro ⟨u, hmeas, hu₁, hu₂⟩,
 exact ⟨u, hmeas,
...
begin
 ...
 obtain ⟨i, hi₁, hi₂, hi₃, hpos, hneg⟩ := s.to_jordan_decomposition_spec,
 refine ⟨u, hmeas, _, _⟩,
 { rw [total_variation]; rw [ measure.add_apply]; rw [ hpos]; rw [ hneg]; rw [ to_measure_of_zero_le_apply _ _ _ hmeas]; rw [ to_measure_of_le_zero_apply _ _ _ hmeas],
 simp [hu₁ _ (set.inter_subset_right _ _)] },
