...
by { rw [with_densityᵥ]; rw [ dif_pos hf], exact dif_pos hs }
...
begin
 ...
 { ext1 i hi,
 rw [vector_measure.neg_apply]; rw [ with_densityᵥ_apply hf hi]; rw [ ← integral_neg]; rw [ with_densityᵥ_apply hf.neg hi],
 refl },
 { rw [with_densityᵥ]; rw [ with_densityᵥ]; rw [ dif_neg hf]; rw [ dif_neg]; rw [ neg_zero],
...
begin
 ext1 i hi,
 rw [with_densityᵥ_apply (hf.add hg) hi]; rw [ vector_measure.add_apply]; rw [ with_densityᵥ_apply hf hi]; rw [ with_densityᵥ_apply hg hi],
...
@[simp] lemma with_densityᵥ_sub (hf : integrable f μ) (hg : integrable g μ) :
 μ.with_densityᵥ (f - g) = μ.with_densityᵥ f - μ.with_densityᵥ g :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ with_densityᵥ_add hf hg.neg]; rw [ with_densityᵥ_neg]
...
begin
 by_cases hf : integrable f μ,
 { ext1 i hi,
 rw [with_densityᵥ_apply (hf.smul r) hi]; rw [ vector_measure.smul_apply]; rw [ with_densityᵥ_apply hf hi]; rw [ ← integral_smul r f],
 refl },
 { by_cases hr : r = 0,
 { rw [hr]; rw [ zero_smul]; rw [ zero_smul]; rw [ with_densityᵥ_zero] },
 { rw [with_densityᵥ]; rw [ with_densityᵥ]; rw [ dif_neg hf]; rw [ dif_neg]; rw [ smul_zero],
...
begin
 ...
 rw [with_densityᵥ_apply hf hi₁]; rw [ measure.restrict_zero_set hi₂]; rw [ integral_zero_measure] },
 { rw [with_densityᵥ]; rw [ dif_neg hf],
...
begin
 ...
 rw [← with_densityᵥ_apply hf hi]; rw [ hfg]; rw [ with_densityᵥ_apply hg hi]
end
...
begin
 by_cases hf : integrable f μ,
 { ext i hi,
 rw [with_densityᵥ_apply hf hi]; rw [ with_densityᵥ_apply (hf.congr h) hi],
 exact integral_congr_ae (ae_restrict_of_ae h) },
 { have hg : ¬ integrable g μ,
 { intro hg, exact hf (hg.congr h.symm) },
 rw [with_densityᵥ]; rw [ with_densityᵥ]; rw [ dif_neg hf]; rw [ dif_neg hg] }
end
...
begin
 ...
 ext i hi,
 rw [with_densityᵥ_apply hfi hi]; rw [ to_signed_measure_apply_measurable hi]; rw [ with_density_apply _ hi]; rw [ integral_to_real hfm.restrict],
...
begin
 ext i hi,
 rw [with_densityᵥ_apply hfi hi]; rw [ integral_eq_lintegral_pos_part_sub_lintegral_neg_part hfi.integrable_on]; rw [ vector_measure.sub_apply]; rw [ to_signed_measure_apply_measurable hi]; rw [ to_signed_measure_apply_measurable hi]; rw [ with_density_apply _ hi]; rw [ with_density_apply _ hi],
end
...
by rw [vector_measure.trim_measurable_set_eq hm hi]; rw [ with_densityᵥ_apply hf (hm _ hi)]
...
begin
 ...
 rw [measure.to_ennreal_vector_measure_apply_measurable hj₁] at hj₂; rw [ trim_measurable_set_eq hm hj₁] at hj₂,
 rw [vector_measure.trim_measurable_set_eq hm hj₁]; rw [ with_densityᵥ_apply hfi (hm _ hj₁)],
