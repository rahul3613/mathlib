...
begin
 ...
 rw [finset.set_bUnion_insert]; rw [ hm]; rw [ ihI]; rw [ finset.sum_insert hiI],
...
begin
 ...
 rw [← Union_S]; rw [ ennreal.tsum_eq_supr_nat]; rw [ supr_le_iff],
 intro n,
 ...
 by { rw [ennreal.inv_lt_inv]; rw [ nat.cast_lt], linarith },
...
begin
 rw [← map_coe_Ioi_at_bot]; rw [ tendsto_map'_iff],
...
begin
 ...
 rw [←smul_eq_mul]; rw [ ← smul_apply]; rw [ smul_bounded_by hc],
...
lemma mk_metric_nnreal_smul (m : ℝ≥0∞ → ℝ≥0∞) {c : ℝ≥0} (hc : c ≠ 0) :
 (mk_metric (c • m) : outer_measure X) = c • mk_metric m :=
by rw [ennreal.smul_def]; rw [ ennreal.smul_def]; rw [ mk_metric_smul m (ennreal.coe_ne_top) (ennreal.coe_ne_zero.mpr hc)]

lemma isometry_map_mk_metric (m : ℝ≥0∞ → ℝ≥0∞) {f : X → Y} (hf : isometry f)
 (H : monotone m ∨ surjective f) :
 map f (mk_metric m) = restrict (range f) (mk_metric m) :=
by rw [← isometry_comap_mk_metric _ hf H]; rw [ map_comap]
...
by rw [← isometry_equiv_comap_mk_metric _ f]; rw [ map_comap_of_surjective f.surjective]
...
by rw [← measure.mk_metric_to_outer_measure]; rw [ coe_to_outer_measure]
...
begin
 intros s hs,
 rw [← outer_measure.coe_mk_metric]; rw [ coe_smul]; rw [ ← outer_measure.coe_mk_metric],
...
begin
 ...
 rw [mk_metric_to_outer_measure]; rw [ outer_measure.mk_metric_top]; rw [ to_outer_measure_top],
end
...
lemma le_mk_metric (m : ℝ≥0∞ → ℝ≥0∞) (μ : measure X) (ε : ℝ≥0∞) (h₀ : 0 < ε)
 ...
 μ ≤ mk_metric m :=
begin
 rw [← to_outer_measure_le]; rw [ mk_metric_to_outer_measure],
...
begin
 refine ⟨λ x, _⟩,
 rw [← nonpos_iff_eq_zero]; rw [ hausdorff_measure_apply],
...
begin
 ...
 rw [←ennreal.le_inv_smul_iff]; rw [ ←nnreal.inv_rpow]; rw [ ←nnnorm_inv],
...
by rw [← hf.hausdorff_measure_image hd]; rw [ image_preimage_eq_inter_range]

lemma map_hausdorff_measure (hf : isometry f) (hd : 0 ≤ d ∨ surjective f) :
 measure.map f μH[d] = (μH[d]).restrict (range f) :=
begin
 ext1 s hs,
 rw [map_apply hf.continuous.measurable hs]; rw [ restrict_apply hs]; rw [ hf.hausdorff_measure_preimage hd]
end
...
@[simp] lemma hausdorff_measure_preimage (e : X ≃ᵢ Y) (d : ℝ) (s : set Y) :
 μH[d] (e ⁻¹' s) = μH[d] s :=
by rw [← e.image_symm]; rw [ e.symm.hausdorff_measure_image]

@[simp] lemma map_hausdorff_measure (e : X ≃ᵢ Y) (d : ℝ) : measure.map e μH[d] = μH[d] :=
by rw [e.isometry.map_hausdorff_measure (or.inr e.surjective)]; rw [ e.surjective.range_eq]; rw [ restrict_univ]
...
by rw [←(volume_preserving_fun_unique unit ℝ).map_eq]; rw [ ←(hausdorff_measure_measure_preserving_fun_unique unit ℝ 1).map_eq]; rw [ ←hausdorff_measure_pi_real]; rw [ fintype.card_unit]; rw [ nat.cast_one]
...
by rw [←(volume_preserving_pi_fin_two (λ i, ℝ)).map_eq]; rw [ ←(hausdorff_measure_measure_preserving_pi_fin_two (λ i, ℝ) _).map_eq]; rw [ ←hausdorff_measure_pi_real]; rw [ fintype.card_fin]; rw [ nat.cast_two]
...
begin
 ...
 rw [norm_smul]; rw [ norm_inv]; rw [ norm_norm]; rw [ inv_mul_cancel hn]; rw [ mul_one]; rw [ linear_map.id_apply] },
 rw [set.image_smul]; rw [ measure.hausdorff_measure_smul₀ zero_le_one hn]; rw [ nnnorm_norm]; rw [ nnreal.rpow_one]; rw [ iso_smul.hausdorff_measure_image (or.inl $ zero_le_one' ℝ)],
end
...
begin
 ...
 borelize E,
 rw [isometry_equiv.hausdorff_measure_image]; rw [ set.image_smul]; rw [ measure.hausdorff_measure_smul₀ hd hc]; rw [ isometry_equiv.hausdorff_measure_image],
end
...
begin
 ...
 rw [←affine_equiv.image_symm]; rw [ affine_equiv.coe_homothety_units_mul_hom_apply_symm]; rw [ hausdorff_measure_homothety_image hd x (_ : 𝕜ˣ).is_unit.ne_zero]; rw [ units.coe_inv]; rw [ units.coe_mk0]; rw [ nnnorm_inv],
end
...
begin
 ...
 { simpa only [set.image_image] },
 borelize E,
 rw [isometry_equiv.hausdorff_measure_image]; rw [ hausdorff_measure_smul_right_image]; rw [ nndist_eq_nnnorm_vsub' E],
end
...
begin
 rw [affine_segment]; rw [ hausdorff_measure_line_map_image]; rw [ hausdorff_measure_real]; rw [ real.volume_Icc]; rw [ sub_zero]; rw [ ennreal.of_real_one]; rw [ ← algebra.algebra_map_eq_smul_one],
...
by rw [←affine_segment_eq_segment]; rw [ hausdorff_measure_affine_segment]
