...
lemma ext_iff' (v w : vector_measure α M) :
 v = w ↔ ∀ i : set α, v i = w i :=
by rw [← coe_injective.eq_iff]; rw [ function.funext_iff]
...
begin
 ...
 rw [hg] at this; rw [ encodable.Union_decode₂] at this,
 ...
 rw [summable.has_sum_iff]; rw [ this]; rw [ ← tsum_Union_decode₂],
 { exact v.empty },
...
lemma of_union {A B : set α}
 ...
 v (A ∪ B) = v A + v B :=
begin
 rw [union_eq_Union]; rw [ of_disjoint_Union]; rw [ tsum_fintype]; rw [ fintype.sum_bool]; rw [ cond]; rw [ cond],
...
lemma of_add_of_diff {A B : set α} (hA : measurable_set A) (hB : measurable_set B)
 (h : A ⊆ B) : v A + v (B \ A) = v B :=
begin
 rw [← of_union disjoint_sdiff_right hA (hB.diff hA)]; rw [ union_diff_cancel h],
 apply_instance,
end
...
begin
 ...
 { exact hA.inter hB } }
 ... = v (A \ B) + v (A ∩ B ∪ B \ A) :
 by { rw [of_union]; rw [ h']; rw [ add_zero],
 { exact set.disjoint_of_subset_left (A.inter_subset_left B) disjoint_sdiff_self_right },
 { exact hA.inter hB },
 { exact hB.diff hA } }
 ... = v (A \ B) + v B :
 by { rw [set.union_comm]; rw [ set.inter_comm]; rw [ set.diff_union_inter] }
end
...
{ measure_of' := r • v,
 empty' := by rw [pi.smul_apply]; rw [ empty]; rw [ smul_zero],
 not_measurable' := λ _ hi, by rw [pi.smul_apply]; rw [ v.not_measurable hi]; rw [ smul_zero],
...
{ measure_of' := λ i : set α, if measurable_set i then (μ.measure_of i).to_real else 0,
 ...
 begin
 intros _ hf₁ hf₂,
 rw [μ.m_Union hf₁ hf₂]; rw [ ennreal.tsum_to_real_eq]; rw [ if_pos (measurable_set.Union hf₁)]; rw [ summable.has_sum_iff],
 ...
 rw [eq_top_iff]; rw [ ← ha]; rw [ outer_measure.measure_of_eq_coe]; rw [ coe_to_outer_measure],
...
begin
 ...
 { rw h },
 rwa [to_signed_measure_apply_measurable hi] at this; rwa [ to_signed_measure_apply_measurable hi] at this; rwa [ ennreal.to_real_eq_to_real] at this;
...
begin
 ext i hi,
 rw [to_signed_measure_apply_measurable hi]; rw [ add_apply]; rw [ ennreal.to_real_add (ne_of_lt (measure_lt_top _ _ )) (ne_of_lt (measure_lt_top _ _))]; rw [ vector_measure.add_apply]; rw [ to_signed_measure_apply_measurable hi]; rw [ to_signed_measure_apply_measurable hi],
...
begin
 ext i hi,
 rw [to_signed_measure_apply_measurable hi]; rw [ vector_measure.smul_apply]; rw [ to_signed_measure_apply_measurable hi]; rw [ coe_smul]; rw [ pi.smul_apply]; rw [ ennreal.to_real_smul],
end
...
{ measure_of' := λ i : set α, if measurable_set i then μ i else 0,
 ...
 { rw [if_pos (measurable_set.Union hf₁)]; rw [ measure_theory.measure_Union hf₂ hf₁],
...
begin
 ...
 rw [to_ennreal_vector_measure_apply_measurable hi]; rw [ add_apply]; rw [ vector_measure.add_apply]; rw [ to_ennreal_vector_measure_apply_measurable hi]; rw [ to_ennreal_vector_measure_apply_measurable hi]
end
...
begin
 rw [vector_measure.sub_apply]; rw [ to_signed_measure_apply_measurable hi]; rw [ measure.to_signed_measure_apply_measurable hi]; rw [ sub_eq_add_neg]
end
...
by rw [ennreal_to_measure]; rw [ of_measurable_apply _ hs]
...
{ to_fun := ennreal_to_measure,
 ...
 by rw [to_ennreal_vector_measure_apply_measurable hs]; rw [ ennreal_to_measure_apply hs]),
 right_inv := λ _, measure.ext (λ s hs,
 by rw [ennreal_to_measure_apply hs]; rw [ to_ennreal_vector_measure_apply_measurable hs]) }
...
{ measure_of' := λ s, if measurable_set s then v (f ⁻¹' s) else 0,
 ...
 { rw [preimage_Union]; rw [ if_pos (measurable_set.Union hg₁)] },
 end } else 0
...
lemma map_apply {f : α → β} (hf : measurable f) {s : set β} (hs : measurable_set s) :
 v.map f s = v (f ⁻¹' s) :=
by { rw [map]; rw [ dif_pos hf], exact if_pos hs }

@[simp] lemma map_id : v.map id = v :=
ext (λ i hi, by rw [map_apply v measurable_id hi]; rw [ preimage_id])
...
{ measure_of' := λ s, f (v s),
 empty' := by rw [empty]; rw [ add_monoid_hom.map_zero],
 not_measurable' := λ i hi, by rw [not_measurable v hi]; rw [ add_monoid_hom.map_zero],
...
{ measure_of' := λ s, if measurable_set s then v (s ∩ i) else 0,
 ...
 { rw [Union_inter]; rw [ if_pos (measurable_set.Union hf₁)] }
 end } else 0
...
by { rw [restrict]; rw [ dif_pos hi], exact if_pos hj }

lemma restrict_eq_self {i : set α} (hi : measurable_set i)
 {j : set α} (hj : measurable_set j) (hij : j ⊆ i) : v.restrict i j = v j :=
by rw [restrict_apply v hi hj]; rw [ inter_eq_left_iff_subset.2 hij]

@[simp] lemma restrict_empty : v.restrict ∅ = 0 :=
ext (λ i hi, by rw [restrict_apply v measurable_set.empty hi]; rw [ inter_empty]; rw [ v.empty]; rw [ zero_apply])

@[simp] lemma restrict_univ : v.restrict univ = v :=
ext (λ i hi, by rw [restrict_apply v measurable_set.univ hi]; rw [ inter_univ])
...
begin
 ...
 by_cases hi : measurable_set i,
 { exact h i hi },
 { rw [v.not_measurable hi]; rw [ w.not_measurable hi] }
end
...
begin
 ...
 { rw [v.not_measurable hj₁]; rw [ w.not_measurable hj₁] },
end
...
begin
 ...
 { rw [restrict_not_measurable v hi]; rw [ restrict_not_measurable w hi],
 exact le_rfl },
end
...
begin
 ...
 { intros h s hs,
 have := h s hs,
 rwa [restrict_apply _ measurable_set.univ hs] at this ; rwa [ inter_univ] at this ; rwa [ restrict_apply _ measurable_set.univ hs] at this ; rwa [ inter_univ] at this },
 { intros h s hs,
 rw [restrict_apply _ measurable_set.univ hs]; rw [ inter_univ]; rw [ restrict_apply _ measurable_set.univ hs]; rw [ inter_univ],
 exact h s hs }
end
...
begin
 ...
 { rwa [← inter_Union]; rwa [ Union_disjointed]; rwa [ inter_eq_left_iff_subset] },
 ...
 rw [← ha₃]; rw [ v.of_disjoint_Union_nat _ ha₄]; rw [ w.of_disjoint_Union_nat _ ha₄],
...
λ s hs, by { rw [neg_apply]; rw [ h hs]; rw [ neg_zero] }
...
λ s hs, by { rw [add_apply]; rw [ hv₁ hs]; rw [ hv₂ hs]; rw [ zero_add] }
...
lemma sub {M : Type*} [add_comm_group M] [topological_space M] [topological_add_group M]
 ...
 v₁ - v₂ ≪ᵥ w :=
λ s hs, by { rw [sub_apply]; rw [ hv₁ hs]; rw [ hv₂ hs]; rw [ zero_sub]; rw [ neg_zero] }
...
lemma smul {R : Type*} [semiring R] [distrib_mul_action R M] [has_continuous_const_smul R M]
 ...
 r • v ≪ᵥ w :=
λ s hs, by { rw [smul_apply]; rw [ h hs]; rw [ smul_zero] }
...
begin
 ...
 rw [← hs]; rw [ ennreal_to_measure_apply hmeas] },
 { intros s hs,
 by_cases hmeas : measurable_set s,
...
begin
 ...
 { rw [add_apply]; rw [ hu₁ _ (subset_inter_iff.1 ht).1]; rw [ hv₁ _ (subset_inter_iff.1 ht).2]; rw [ zero_add] },
...
{ measure_of' := λ i, if measurable_set[m] i then v i else 0,
 empty' := by rw [if_pos measurable_set.empty]; rw [ v.empty],
...
begin
 ext j hj,
 rw [restrict_apply]; rw [ trim_measurable_set_eq hle hj]; rw [ restrict_apply]; rw [ trim_measurable_set_eq],
...
measure.of_measurable (s.to_measure_of_zero_le' i hi₂)
 (by { simp_rw [to_measure_of_zero_le', s.restrict_apply hi₁ measurable_set.empty, set.empty_inter i, s.empty], refl })
 begin
 intros f hf₁ hf₂,
...
by simp_rw [to_measure_of_zero_le, measure.of_measurable_apply _ hj₁, to_measure_of_zero_le', s.restrict_apply hi₁ hj₁, set.inter_comm]
