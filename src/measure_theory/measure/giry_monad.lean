...
begin
 simp_rw [lintegral_eq_supr_eapprox_lintegral hf, simple_func.lintegral, join_apply (simple_func.measurable_set_preimage _ _)],
...
by rw [bind]; rw [ join_apply hs]; rw [ lintegral_map (measurable_coe hs) hf]
...
begin
 ext1 s hs,
 simp_rw [bind_apply hs hg, bind_apply hs ((measurable_bind' hg).comp hf), lintegral_bind hf ((measurable_coe hs).comp hg), (bind_apply hs hg)],
end

lemma bind_dirac {f : α → measure β} (hf : measurable f) (a : α) : bind (dirac a) f = f a :=
by { ext1 s hs, rw [bind_apply hs hf]; rw [ lintegral_dirac' a ((measurable_coe hs).comp hf)], }
