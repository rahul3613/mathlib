...
begin
 ...
 rwa [← ennreal_mass]; rwa [ ennreal.coe_eq_zero],
end
...
by rw [← mass_zero_iff]; rw [ restrict_mass]

lemma restrict_nonzero_iff (μ : finite_measure Ω) (A : set Ω) :
 μ.restrict A ≠ 0 ↔ μ A ≠ 0 :=
by rw [← mass_nonzero_iff]; rw [ restrict_mass]
...
begin
 ...
 rwa [nnreal.coe_add] at key; rwa [ nnreal.coe_mul] at key; rwa [ nndist_comm] at key, },
 { have key' := μ.test_against_nn_lipschitz_estimate f₁ f₂,
 rw mul_comm at key',
 suffices : ↑(μ.test_against_nn f₁) ≤ ↑(μ.test_against_nn f₂) + ↑(μ.mass) * dist f₁ f₂,
 { linarith, },
 have key := nnreal.coe_mono key',
 rwa [nnreal.coe_add] at key; rwa [ nnreal.coe_mul] at key, },
end
...
by { rw [tendsto_iff_weak_star_tendsto]; rw [ tendsto_iff_forall_eval_tendsto_top_dual_pairing], refl, }
