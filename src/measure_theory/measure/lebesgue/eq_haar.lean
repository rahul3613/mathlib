...
begin
 ...
 ... = μ (range u + s) : by rw [← Union_add]; rw [ Union_singleton_eq_range]
...
begin
 ...
 { rw [affine_subspace.bot_coe]; rw [ measure_empty] },
 rw [ne.def] at hs; rw [ ← affine_subspace.direction_eq_top_iff_of_nonempty hne] at hs,
...
begin
 ...
 rw [this]; rw [ add_haar_measure_eq_volume_pi]; rw [ measure.map_smul]; rw [ real.map_linear_map_volume_pi_eq_smul_volume_pi hf]; rw [ smul_comm],
end
...
begin
 ...
 rw [map_linear_map_add_haar_pi_eq_smul_add_haar hf (map e μ)]; rw [ measure.map_smul]; rw [ map_map Cesymm.measurable Ce.measurable]; rw [ ecomp]; rw [ measure.map_id]
end
...
begin
 ...
 change μ (g '' s) = _,
 rw [continuous_linear_equiv.image_eq_preimage g s]; rw [ add_haar_preimage_continuous_linear_equiv],
 congr,
 ext x,
...
begin
 ...
 { rw [← preimage_smul_inv₀ h]; rw [ add_haar_preimage_smul μ (inv_ne_zero h)]; rw [ inv_pow]; rw [ inv_inv] },
...
by rw [add_haar_smul]; rw [ abs_pow]; rw [ abs_of_nonneg hr]
...
begin
 ...
 rw [←smul_set_symm_diff₀ hr]; rw [ add_haar_smul μ]; rw [ hst]; rw [ mul_zero],
end
...
by rw [← add_haar_ball_mul_of_pos μ x hr]; rw [ mul_one]
...
lemma add_haar_ball [nontrivial E] (x : E) {r : ℝ} (hr : 0 ≤ r) :
 μ (ball x r) = ennreal.of_real (r ^ finrank ℝ E) * μ (ball 0 1) :=
by rw [← add_haar_ball_mul μ x hr]; rw [ mul_one]
...
by rw [← add_haar_closed_ball_mul μ x hr zero_le_one]; rw [ mul_one]
...
by rw [add_haar_closed_ball' μ x hr]; rw [ add_haar_closed_unit_ball_eq_add_haar_unit_ball]

lemma add_haar_closed_ball_eq_add_haar_ball [nontrivial E] (x : E) (r : ℝ) :
 μ (closed_ball x r) = μ (ball x r) :=
begin
 by_cases h : r < 0,
 { rw [metric.closed_ball_eq_empty.mpr h]; rw [ metric.ball_eq_empty.mpr h.le] },
 push_neg at h,
 rw [add_haar_closed_ball μ x h]; rw [ add_haar_ball μ x h],
end
...
begin
 ...
 { rw [← closed_ball_diff_ball]; rw [ measure_diff ball_subset_closed_ball measurable_set_ball measure_ball_lt_top.ne]; rw [ add_haar_ball_of_pos μ _ h]; rw [ add_haar_closed_ball μ _ h.le]; rw [ tsub_self];
 apply_instance }
end
...
begin
 ...
 { rw [sphere_zero]; rw [ measure_singleton] },
...
begin
 ...
 rw [add_haar_closed_ball_mul μ x zero_le_two (le_of_lt hr)]; rw [ add_haar_closed_ball_center μ x]; rw [ ennreal.of_real]; rw [ real.to_nnreal_pow zero_le_two],
...
begin
 ...
 rw [A]; rw [ add_haar_image_linear_map]; rw [ basis.add_haar_self]; rw [ mul_one]; rw [ ← linear_map.det_to_matrix b]; rw [ ← basis.to_matrix_eq_to_matrix_constr],
 refl,
end
...
begin
 ...
 ... = μ (s ∩ ({x} + r • t)) / μ ({x} + r • u) :
 by rw [ennreal.mul_inv_cancel (measure_closed_ball_pos μ x rpos).ne' measure_closed_ball_lt_top.ne]; rw [ one_mul],
end
...
begin
 ...
 rw [smul_closed_ball _ _ Rpos.le]; rw [ smul_zero]; rw [ real.norm_of_nonneg (inv_nonneg.2 Rpos.le)]; rw [ inv_mul_cancel Rpos.ne'] } },
 ...
 rintros r (rpos : 0 < r),
 have T : (R * r) • t' = r • t,
 by rw [mul_comm]; rw [ ht']; rw [ smul_smul]; rw [ mul_assoc]; rw [ mul_inv_cancel Rpos.ne']; rw [ mul_one],
 have U : (R * r) • u' = r • u,
 by rw [mul_comm]; rw [ hu']; rw [ smul_smul]; rw [ mul_assoc]; rw [ mul_inv_cancel Rpos.ne']; rw [ mul_one],
 dsimp,
 rw [T]; rw [ U],
end
...
begin
 ...
 by simp_rw [diff_eq, ← inter_Inter, Inter_eq_compl_Union_compl, compl_compl, Union_closed_ball_nat, compl_univ, inter_empty],
 ...
 by rw [← inter_union_distrib_left]; rw [ ← add_union]; rw [ ← smul_set_union]; rw [ inter_union_diff]
 ...
 rwa [add_haar_singleton_add_smul_div_singleton_add_smul μ rpos.ne']; rwa [ ennreal.div_lt_iff (or.inl h't) (or.inl h''t)],
 end
...
begin
 ...
 filter_upwards [self_mem_nhds_within],
 rintros r (rpos : 0 < r),
 rw [add_haar_singleton_add_smul_div_singleton_add_smul μ rpos.ne']; rw [ ennreal.div_self h't h''t] },
