...
begin
 ...
 conv_rhs { rw [add_haar_measure_unique stieltjes_function.id.measure (std_orthonormal_basis ℝ ℝ).to_basis.parallelepiped]; rw [ A] },
...
by rw [ball_eq_Ioo]; rw [ volume_Ioo]; rw [ ← sub_add]; rw [ add_sub_cancel']; rw [ two_mul]

@[simp] lemma volume_closed_ball (a r : ℝ) :
 volume (metric.closed_ball a r) = of_real (2 * r) :=
by rw [closed_ball_eq_Icc]; rw [ volume_Icc]; rw [ ← sub_add]; rw [ add_sub_cancel']; rw [ two_mul]

@[simp] lemma volume_emetric_ball (a : ℝ) (r : ℝ≥0∞) :
 volume (emetric.ball a r) = 2 * r :=
begin
 rcases eq_or_ne r ∞ with rfl|hr,
 { rw [metric.emetric_ball_top]; rw [ volume_univ]; rw [ two_mul]; rw [ _root_.top_add] },
 { lift r to ℝ≥0 using hr,
 rw [metric.emetric_ball_nnreal]; rw [ volume_ball]; rw [ two_mul]; rw [ ← nnreal.coe_add]; rw [ ennreal.of_real_coe_nnreal]; rw [ ennreal.coe_add]; rw [ two_mul] }
end
...
begin
 rcases eq_or_ne r ∞ with rfl|hr,
 { rw [emetric.closed_ball_top]; rw [ volume_univ]; rw [ two_mul]; rw [ _root_.top_add] },
 { lift r to ℝ≥0 using hr,
 rw [metric.emetric_closed_ball_nnreal]; rw [ volume_closed_ball]; rw [ two_mul]; rw [ ← nnreal.coe_add]; rw [ ennreal.of_real_coe_nnreal]; rw [ ennreal.coe_add]; rw [ two_mul] }
end
...
by rw [←Icc_min_max]; rw [ volume_Icc]; rw [ max_sub_min_eq_abs]
...
begin
 ...
 { rw [real.ediam_eq hs]; rw [ ← volume_Icc],
...
by conv_rhs { rw [← real.smul_map_volume_mul_left h]; rw [ smul_smul]; rw [ ← ennreal.of_real_mul (abs_nonneg _)]; rw [ ← abs_mul]; rw [ inv_mul_cancel h]; rw [ abs_one]; rw [ ennreal.of_real_one]; rw [ one_smul] }
...
begin
 ...
 rw [volume_preimage_mul_left A]; rw [ ← mul_assoc]; rw [ ← ennreal.of_real_mul (abs_nonneg _)]; rw [ ← abs_mul]; rw [ mul_inv_cancel A]; rw [ abs_one]; rw [ ennreal.of_real_one]; rw [ one_mul] },
 rw [this]; rw [ volume_pi_pi]; rw [ finset.abs_prod]; rw [ ennreal.of_real_prod_of_nonneg (λ i hi, abs_nonneg (D i))]; rw [ ← finset.prod_mul_distrib],
 simp only [B]
end
...
begin
 ...
 rw [smul_smul]; rw [ ← ennreal.of_real_mul (abs_nonneg _)]; rw [ ← abs_mul]; rw [ inv_mul_cancel hD]; rw [ abs_one]; rw [ ennreal.of_real_one]; rw [ one_smul] },
 ...
 { rw [to_lin'_mul]; rw [ det_mul]; rw [ linear_map.coe_comp]; rw [ ← measure.map_map]; rw [ IHB]; rw [ measure.map_smul]; rw [ IHA]; rw [ smul_smul]; rw [ ← ennreal.of_real_mul (abs_nonneg _)]; rw [ ← abs_mul]; rw [ mul_comm]; rw [ mul_inv],
...
begin
 ...
 rw [lintegral_congr_ae h₁]; rw [ ← volume_region_between_eq_lintegral' hf.measurable_mk hg.measurable_mk hs],
 convert h₂ using 1,
...
begin
 ...
 rw [ae_restrict_union_iff]; rw [ ae_restrict_bUnion_iff _ A_count],
 split,
