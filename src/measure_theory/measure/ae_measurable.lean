...
begin
 ...
 { rw [restrict_piecewise_compl]; rw [ compl_Inter],
 intros t ht,
...
by { rw [← sum_cond]; rw [ ae_measurable_sum_measure_iff]; rw [ bool.forall_bool]; rw [ and.comm], refl }
...
begin
 ...
 { rw [measure_to_measurable]; rw [ ← compl_mem_ae_iff]; rw [ compl_compl],
...
by rw [uIoc_eq_union]; rw [ ae_measurable_union_iff]
...
by rw [← map_comap_subtype_coe hs]; rw [ (measurable_embedding.subtype_coe hs).ae_measurable_map_iff]
...
begin
 ...
 rw [Ioi_eq_Union]; rw [ ae_measurable_Union_iff],
 intros n,
 cases lt_or_le x (u n),
 { exact g_meas (u n) h, },
 { rw [Ioc_eq_empty (not_lt.mpr h)]; rw [ measure.restrict_empty],
 exact ae_measurable_zero_measure, },
end
