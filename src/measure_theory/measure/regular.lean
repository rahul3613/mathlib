...
begin
 ...
 rwa [measure_empty]; rwa [ h₀]; rwa [ zero_add]; rwa [ pos_iff_ne_zero] },
...
begin
 ...
 rwa [map_apply_of_ae_measurable hf (hB₁ _ $ hAB' _ hKc)]; rwa [ f.preimage_image]
end

lemma smul (H : inner_regular μ p q) (c : ℝ≥0∞) : inner_regular (c • μ) p q :=
begin
 intros U hU r hr,
 rw [smul_apply] at hr; rw [ H.measure_eq_supr hU] at hr; rw [ smul_eq_mul] at hr,
...
begin
 ...
 rwa [map_apply f.measurable this.measurable_set]; rwa [ f.preimage_symm]; rwa [ f.preimage_image],
end
...
begin
 ...
 { refine ⟨λ A hA r hr, _⟩,
 rw [smul_apply] at hr; rw [ A.measure_eq_infi_is_open] at hr; rw [ smul_eq_mul] at hr,
...
begin
 ...
 rw [← inter_Union]; rw [ Union_disjointed]; rw [ s.spanning]; rw [ inter_univ] },
...
begin
 ...
 ... = μ (⋃ k ∈ t, F k) + ε : _,
 rw [measure_bUnion_finset]; rw [ add_assoc]; rw [ ennreal.add_halves],
 exacts [λ k _ n _ hkn, (hsd hkn).mono (hFs k) (hFs n), λ k hk, (hFc k).measurable_set] },
 { calc μ (⋃ n, U n) ≤ ∑' n, μ (U n) : measure_Union_le _
 ... ≤ ∑' n, (μ (s n) + δ n) : ennreal.tsum_le_tsum hU
 ... = μ (⋃ n, s n) + ∑' n, δ n : by rw [measure_Union hsd hsm]; rw [ ennreal.tsum_add]
...
begin
 ...
 have hBU : (⋃ n, F ∩ B n) = F, by rw [← inter_Union]; rw [ Union_compact_covering]; rw [ set.inter_univ],
 have : μ F = ⨆ n, μ (F ∩ B n),
 { rw [← measure_Union_eq_supr]; rw [ hBU],
...
by simp_rw [ne.def, ← measure_univ_eq_zero, is_open_univ.measure_eq_supr_is_compact, ennreal.supr_eq_zero, not_forall, exists_prop, subset_univ, true_and]
