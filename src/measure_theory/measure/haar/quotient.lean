...
{ measure_preimage_smul :=
 ...
 rw [measure.map_apply meas_π hA]; rw [ measure.map_apply meas_π (measurable_set_preimage (measurable_const_smul g) hA)]; rw [ measure.restrict_apply₀' 𝓕meas]; rw [ measure.restrict_apply₀' 𝓕meas],
 set π_preA := π ⁻¹' A,
 ...
 { rw preimage_inter,
 congr,
 rw [← preimage_comp]; rw [ comp_mul_left]; rw [ mul_left_inv],
 ext,
 simp, },
 rw measure_preimage_mul, },
 rw this,
 have h𝓕_translate_fundom : is_fundamental_domain Γ.opposite (g • 𝓕) μ := h𝓕.smul_of_comm g,
 rw [h𝓕.measure_set_eq h𝓕_translate_fundom meas_πA]; rw [ ← preimage_smul_inv], refl,
 rintros ⟨γ, γ_in_Γ⟩,
 ext,
...
begin
 ...
 ⟨by { rw [measure.restrict_apply₀' 𝓕meas]; rw [ univ_inter], exact h𝓕_finite }⟩,
 ...
 rw [measure.haar_measure_unique (measure.map (quotient_group.mk' Γ) (μ.restrict 𝓕)) K]; rw [ measure.map_apply meas_π]; rw [ measure.restrict_apply₀' 𝓕meas]; rw [ inter_comm],
...
{ measurable := continuous_quotient_mk.measurable,
 map_eq := by rw [h𝓕.map_restrict_quotient K h𝓕_finite]; rw [ h]; refl }
