...
begin
 ext,
 simp_rw [set.uIcc, mem_parallelepiped_iff, set.mem_Icc, pi.le_def, ←forall_and_distrib, pi.inf_apply, pi.sup_apply, ←pi.single_smul', pi.one_apply, pi.zero_apply, ←pi.smul_apply', finset.univ_sum_single (_ : ι → ℝ)],
 split,
 { rintros ⟨t, ht, rfl⟩ i,
 specialize ht i,
 simp_rw [smul_eq_mul, pi.mul_apply],
 cases le_total (a i) 0 with hai hai,
 { rw [sup_eq_left.mpr hai]; rw [ inf_eq_right.mpr hai],
 exact ⟨le_mul_of_le_one_left hai ht.2, mul_nonpos_of_nonneg_of_nonpos ht.1 hai⟩ },
 { rw [sup_eq_right.mpr hai]; rw [ inf_eq_left.mpr hai],
 exact ⟨mul_nonneg ht.1 hai, mul_le_of_le_one_left hai ht.2⟩ } },
 { intro h,
 refine ⟨λ i, x i / a i, λ i, _, funext $ λ i, _⟩,
 { specialize h i,
 cases le_total (a i) 0 with hai hai,
 { rw [sup_eq_left.mpr hai] at h; rw [ inf_eq_right.mpr hai] at h,
 exact ⟨div_nonneg_of_nonpos h.2 hai, div_le_one_of_ge h.1 hai⟩ },
 { rw [sup_eq_right.mpr hai] at h; rw [ inf_eq_left.mpr hai] at h,
 exact ⟨div_nonneg h.1 hai, div_le_one_of_le h.2 hai⟩ } },
 { specialize h i,
 simp only [smul_eq_mul, pi.mul_apply],
 cases eq_or_ne (a i) 0 with hai hai,
 { rw [hai] at h; rw [ inf_idem] at h; rw [ sup_idem] at h; rw [ ←le_antisymm_iff] at h,
...
{ carrier := parallelepiped b,
 ...
 rwa [← homeomorph.image_interior]; rwa [ nonempty_image_iff],
 end }
