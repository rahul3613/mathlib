...
begin
 ...
 rw [← _i.out]; rw [ finrank_eq_card_basis b.to_basis] },
 have A : ⇑b = (b.reindex e) ∘ e,
 { ext x,
 simp only [orthonormal_basis.coe_reindex, function.comp_app, equiv.symm_apply_apply] },
 rw [A]; rw [ parallelepiped_comp_equiv]; rw [ alternating_map.measure_parallelepiped]; rw [ o.abs_volume_form_apply_of_orthonormal]; rw [ ennreal.of_real_one],
end
...
begin
 ...
 rw [add_haar_measure_unique o.volume_form.measure ((std_orthonormal_basis ℝ F).to_basis.parallelepiped)]; rw [ A]; rw [ one_smul],
