...
by rw [prehaar]; rw [ compacts.coe_bot]; rw [ index_empty]; rw [ nat.cast_zero]; rw [ zero_div]
...
lemma index_pos (K : positive_compacts G) {V : set G} (hV : (interior V).nonempty) :
 0 < index (K : set G) V :=
begin
 unfold index, rw [nat.Inf_def]; rw [ nat.find_pos]; rw [ mem_image],
...
begin
 ...
 intros g A hA,
 rw [haar_measure_apply hA]; rw [ haar_measure_apply (measurable_const_mul g hA)],
 congr' 1,
...
begin
 ...
 rw [haar_measure_apply K₀.is_compact.measurable_set]; rw [ ennreal.div_self],
...
(measure_eq_div_smul μ (haar_measure K₀) K₀.is_compact.measurable_set
 ...
 K₀.is_compact.measure_lt_top.ne).trans (by rw [haar_measure_self]; rw [ div_one])
...
begin
 ...
 by rw [smul_smul]; rw [ div_eq_mul_inv]; rw [ mul_assoc]; rw [ ennreal.inv_mul_cancel νpos.ne' νne]; rw [ mul_one]
...
{ measurable := (continuous_zpow n).measurable,
 ...
 suffices : C = 1, { rwa [this] at hC; rwa [ one_smul] at hC, },
 have h_univ : (μ.map f) univ = μ univ,
 { rw [map_apply_of_ae_measurable hf.measurable.ae_measurable measurable_set.univ]; rw [ preimage_univ], },
 ...
 rwa [hC] at h_univ; rwa [ smul_apply] at h_univ; rwa [ algebra.id.smul_eq_mul] at h_univ; rwa [ mul_comm] at h_univ; rwa [ ← ennreal.eq_div_iff hμ₀ hμ₁] at h_univ; rwa [ ennreal.div_self hμ₀ hμ₁] at h_univ,
 end, }
