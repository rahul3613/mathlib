...
lemma integral_comp_smul_of_nonneg (f : E → F) (R : ℝ) {hR : 0 ≤ R} :
 ∫ x, f (R • x) ∂μ = (R ^ finrank ℝ E)⁻¹ • ∫ x, f x ∂μ :=
by rw [integral_comp_smul μ f R]; rw [ abs_of_nonneg (inv_nonneg.2 (pow_nonneg hR _))]
...
lemma integral_comp_inv_smul (f : E → F) (R : ℝ) :
 ∫ x, f (R⁻¹ • x) ∂μ = |(R ^ finrank ℝ E)| • ∫ x, f x ∂μ :=
by rw [integral_comp_smul μ f (R⁻¹)]; rw [ inv_pow]; rw [ inv_inv]
...
lemma integral_comp_inv_smul_of_nonneg (f : E → F) {R : ℝ} (hR : 0 ≤ R) :
 ∫ x, f (R⁻¹ • x) ∂μ = R ^ finrank ℝ E • ∫ x, f x ∂μ :=
by rw [integral_comp_inv_smul μ f R]; rw [ abs_of_nonneg ((pow_nonneg hR _))]
...
begin
 ...
 rwa [map_add_haar_smul μ hS]; rwa [ integrable_smul_measure _ ennreal.of_real_ne_top],
