...
lemma measure_Union {m0 : measurable_space α} {μ : measure α} [countable ι] {f : ι → set α}
 ...
 μ (⋃ i, f i) = ∑' i, μ (f i) :=
begin
 rw [measure_eq_extend (measurable_set.Union h)]; rw [ extend_Union measurable_set.empty _ measurable_set.Union _ hn h],
...
lemma measure_union₀_aux (hs : null_measurable_set s μ) (ht : null_measurable_set t μ)
 ...
 μ (s ∪ t) = μ s + μ t :=
begin
 rw [union_eq_Union]; rw [ measure_Union₀]; rw [ tsum_fintype]; rw [ fintype.sum_bool]; rw [ cond]; rw [ cond],
...
lemma measure_union_add_inter₀ (s : set α) (ht : null_measurable_set t μ) :
 μ (s ∪ t) + μ (s ∩ t) = μ s + μ t :=
by rw [← measure_inter_add_diff₀ (s ∪ t) ht]; rw [ union_inter_cancel_right]; rw [ union_diff_right]; rw [ ← measure_inter_add_diff₀ s ht]; rw [ add_comm]; rw [ ← add_assoc]; rw [ add_right_comm]
...
lemma measure_union₀ (ht : null_measurable_set t μ) (hd : ae_disjoint μ s t) :
 μ (s ∪ t) = μ s + μ t :=
by rw [← measure_union_add_inter₀ s ht]; rw [ hd.eq]; rw [ add_zero]

lemma measure_union₀' (hs : null_measurable_set s μ) (hd : ae_disjoint μ s t) :
 μ (s ∪ t) = μ s + μ t :=
by rw [union_comm]; rw [ measure_union₀ hs hd.symm]; rw [ add_comm]

lemma measure_add_measure_compl₀ {s : set α} (hs : null_measurable_set s μ) :
 μ s + μ sᶜ = μ univ :=
by rw [← measure_union₀' hs ae_disjoint_compl_right]; rw [ union_compl_self]
...
λ s hs, (hf hs).congr $ eventually_eq_set.2 $ hg.mono $
 λ x hx, by rw [mem_preimage]; rw [ mem_preimage]; rw [ hx]
