...
lemma measure_union_add_inter (s : set α) (ht : measurable_set t) :
 μ (s ∪ t) + μ (s ∩ t) = μ s + μ t :=
by { rw [← measure_inter_add_diff (s ∪ t) ht]; rw [ set.union_inter_cancel_right]; rw [ union_diff_right]; rw [ ← measure_inter_add_diff s ht], ac_refl }

lemma measure_union_add_inter' (hs : measurable_set s) (t : set α) :
 μ (s ∪ t) + μ (s ∩ t) = μ s + μ t :=
by rw [union_comm]; rw [ inter_comm]; rw [ measure_union_add_inter t hs]; rw [ add_comm]
...
lemma measure_sUnion₀ {S : set (set α)} (hs : S.countable)
 ...
 μ (⋃₀ S) = ∑' s : S, μ s :=
by rw [sUnion_eq_bUnion]; rw [ measure_bUnion₀ hs hd h]
...
lemma tsum_measure_preimage_singleton {s : set β} (hs : s.countable) {f : α → β}
 ...
 ∑' b : s, μ (f ⁻¹' {↑b}) = μ (f ⁻¹' s) :=
by rw [← set.bUnion_preimage_singleton]; rw [ measure_bUnion hs (pairwise_disjoint_fiber _ _) hf]
...
by rw [← measure_union' disjoint_sdiff_right hs]; rw [ union_diff_self]

lemma measure_diff' (s : set α) (hm : measurable_set t) (h_fin : μ t ≠ ∞) :
 μ (s \ t) = μ (s ∪ t) - μ t :=
eq.symm $ ennreal.sub_eq_of_add_eq h_fin $ by rw [add_comm]; rw [ measure_add_diff hm]; rw [ union_comm]

lemma measure_diff (h : s₂ ⊆ s₁) (h₂ : measurable_set s₂) (h_fin : μ s₂ ≠ ∞) :
 μ (s₁ \ s₂) = μ s₁ - μ s₂ :=
by rw [measure_diff' _ h₂ h_fin]; rw [ union_eq_self_of_subset_right h]
...
by rwa [measure_diff hst hs hs']; rwa [ tsub_le_iff_left]
...
begin
 ...
 ⟨by rwa [ae_le_set]; rwa [ union_diff_left], has_subset.subset.eventually_le $ subset_union_left s t⟩⟩,
end

@[simp] lemma union_ae_eq_right_iff_ae_subset : (s ∪ t : set α) =ᵐ[μ] t ↔ s ≤ᵐ[μ] t :=
by rw [union_comm]; rw [ union_ae_eq_left_iff_ae_subset]
...
lemma measure_union_congr_of_subset {t₁ t₂ : set α} (hs : s₁ ⊆ s₂) (hsμ : μ s₂ ≤ μ s₁)
 ...
 μ (s₁ ∪ t₁) = μ (s₂ ∪ t₂) :=
begin
 rw [union_eq_Union]; rw [ union_eq_Union],
...
begin
 ...
 intros i j hij,
 rw [function.on_fun]; rw [ disjoint_iff_inf_le],
...
begin
 ...
 rw [bUnion_eq_Union]; rw [ measure_Union_eq_supr hd.directed_coe]; rw [ ← supr_subtype'']
end
...
begin
 ...
 rw [← ennreal.sub_sub_cancel (by exact hk) (infi_le _ k)]; rw [ ennreal.sub_infi]; rw [ ← ennreal.sub_sub_cancel (by exact hk) (measure_mono (Inter_subset _ k))]; rw [ ← measure_diff (Inter_subset _ k) (measurable_set.Inter h) (this _ (Inter_subset _ k))]; rw [ diff_Inter]; rw [ measure_Union_eq_supr],
 { congr' 1,
 ...
 { rw [tsub_le_iff_right]; rw [ ← measure_union disjoint_sdiff_left (h i)]; rw [ set.union_comm],
...
{ to_outer_measure := μ₁.to_outer_measure + μ₂.to_outer_measure,
 ...
 by rw [ennreal.tsum_add]; rw [ measure_Union hd hs]; rw [ measure_Union hd hs],
 trimmed := by rw [outer_measure.trim_add]; rw [ μ₁.trimmed]; rw [ μ₂.trimmed] }⟩
...
⟨λ c μ,
 ...
 trimmed := by rw [outer_measure.trim_smul]; rw [ μ.trimmed] }⟩
...
by rw [← μ₂.trimmed]; rw [ outer_measure.le_trim_iff]; refl
...
λ s hs, by rw [Inf_apply hs]; rw [ ← to_outer_measure_apply]; exact this s
...
by rw [←outer_measure.to_measure_top]; rw [ to_measure_to_outer_measure]; rw [ outer_measure.trim_top]
...
begin
 rw [← trimmed]; rw [ outer_measure.trim_eq_trim_iff],
 intros s hs,
 rw [coe_to_outer_measure]; rw [ map_apply_of_ae_measurable hf hs]; rw [ outer_measure.map_apply]; rw [ coe_to_outer_measure]
end
...
lemma comapₗ_apply {β} [measurable_space α] {mβ : measurable_space β}
 ...
 comapₗ f μ s = μ (f '' s) :=
begin
 rw [comapₗ]; rw [ dif_pos]; rw [ lift_linear_apply _ hs]; rw [ outer_measure.comap_apply]; rw [ coe_to_outer_measure],
 exact ⟨hfi, hf⟩
end
...
begin
 ...
 rw [to_measure_apply₀ _ _ hs]; rw [ outer_measure.comap_apply]; rw [ coe_to_outer_measure]
end
...
lemma le_comap_apply {β} [measurable_space α] {mβ : measurable_space β} (f : α → β) (μ : measure β)
 ...
 μ (f '' s) ≤ comap f μ s :=
by { rw [comap]; rw [ dif_pos (and.intro hfi hf)], exact le_to_measure_apply _ _ _, }
...
lemma ae_eq_image_of_ae_eq_comap {β} [measurable_space α] {mβ : measurable_space β}
 ...
 f '' s =ᵐ[μ] f '' t :=
begin
 rw [eventually_eq] at hst ⊢; rw [ ae_iff] at hst ⊢,
...
by rw [comap_apply₀ _ _ hf h (hf' hs).null_measurable_set]; rw [ image_preimage_eq_inter_range]
...
begin
 rw [subtype.measurable_space] at ht; rw [ comap_eq_generate_from] at ht,
...
begin
 rw [subtype.volume_def]; rw [ comap_apply₀ _ _ _ _ measurable_set.univ.null_measurable_set],
...
by simp_rw [restrict, restrictₗ, lift_linear, linear_map.coe_mk, to_measure_to_outer_measure, outer_measure.restrict_trim h, μ.trimmed]
...
by rw [← coe_to_outer_measure]; rw [ measure.restrict_to_outer_measure_eq_to_outer_measure_restrict hs]; rw [ outer_measure.restrict_apply s t _]; rw [ coe_to_outer_measure]

lemma restrict_apply₀' (hs : null_measurable_set s μ) : μ.restrict s t = μ (t ∩ s) :=
by rw [← restrict_congr_set hs.to_measurable_ae_eq]; rw [ restrict_apply' (measurable_set_to_measurable _ _)]; rw [ measure_congr ((ae_eq_refl t).inter hs.to_measurable_ae_eq)]
...
... = μ.restrict t s :
 by rw [← restrict_apply (measurable_set_to_measurable _ _)]; rw [ measure_to_measurable]
...
by rw [restrict_apply measurable_set.univ]; rw [ set.univ_inter]
...
by rw [restrict_restrict hs]; rw [ restrict_restrict' hs]; rw [ inter_comm]
...
by rw [← measure_univ_eq_zero]; rw [ restrict_apply_univ]
...
by rw [← restrict_inter_add_diff₀ (s ∪ t) ht]; rw [ union_inter_cancel_right]; rw [ union_diff_right]; rw [ ← restrict_inter_add_diff₀ s ht]; rw [ add_comm]; rw [ ← add_assoc]; rw [ add_right_comm]
...
lemma restrict_union' (h : disjoint s t) (hs : measurable_set s) :
 μ.restrict (s ∪ t) = μ.restrict s + μ.restrict t :=
by rw [union_comm]; rw [ restrict_union h.symm hs]; rw [ add_comm]

@[simp] lemma restrict_add_restrict_compl (hs : measurable_set s) :
 μ.restrict s + μ.restrict sᶜ = μ :=
by rw [← restrict_union (@disjoint_compl_right (set α) _ _) hs.compl]; rw [ union_compl_self]; rw [ restrict_univ]

@[simp] lemma restrict_compl_add_restrict (hs : measurable_set s) :
 μ.restrict sᶜ + μ.restrict s = μ :=
by rw [add_comm]; rw [ restrict_add_restrict_compl hs]
...
ext $ λ t ht, by rw [restrict_apply ht]; rw [ restrict_apply ht]; rw [ inter_comm]; rw [ measure_to_measurable_inter ht h]; rw [ inter_comm]
...
⟨λ H t hts ht,
 by rw [← inter_eq_self_of_subset_left hts]; rw [ ← restrict_apply ht]; rw [ H]; rw [ restrict_apply ht],
 λ H, ext $ λ t ht,
 by rw [restrict_apply ht]; rw [ restrict_apply ht]; rw [ H _ (inter_subset_right _ _) (ht.inter hs)]⟩

lemma restrict_congr_mono (hs : s ⊆ t) (h : μ.restrict t = ν.restrict t) :
 μ.restrict s = ν.restrict s :=
by rw [← restrict_restrict_of_subset hs]; rw [ h]; rw [ restrict_restrict_of_subset hs]
...
begin
 ...
 ... = restrict ν s u + restrict ν t (u \ US) : by rw [hs]; rw [ ht]
 ... = ν US + ν (u ∩ t \ US) :
...
by rw [sUnion_eq_bUnion]; rw [ restrict_bUnion_congr hc]
...
by rw [← restrict_Union_congr]; rw [ hs]; rw [ restrict_univ]; rw [ restrict_univ]
...
by rw [← restrict_bUnion_congr hc]; rw [ hs]; rw [ restrict_univ]; rw [ restrict_univ]
...
begin
 ...
 rwa [← measure_inter_add_diff t hv] at this; rwa [ ← measure_inter_add_diff t hv] at this; rwa [ ← hvt] at this; rwa [ ennreal.add_right_inj] at this,
...
@[simp] lemma dirac_apply [measurable_singleton_class α] (a : α) (s : set α) :
 dirac a s = s.indicator 1 a :=
begin
 by_cases h : a ∈ s, by rw [dirac_apply_of_mem h]; rw [ indicator_of_mem h]; rw [ pi.one_apply],
...
by rw [sum_fintype]; rw [ finset.sum_coe_sort s μ]
...
by rw [sum_fintype]; rw [ fintype.sum_bool]
...
by rw [← measure_univ_eq_zero]; rw [ sum_apply _ measurable_set.univ]; rw [ tsum_empty]
...
by rw [count_apply measurable_set.empty]; rw [ tsum_empty]
...
by rw [← count_apply_finset]; rw [ finite.coe_to_finset]
...
@[simp] lemma count_singleton' {a : α} (ha : measurable_set ({a} : set α)) :
 count ({a} : set α) = 1 :=
begin
 rw [count_apply_finite' (set.finite_singleton a) ha]; rw [ set.finite.to_finset],
 simp,
end
...
⟨λ h s, by { rw [measure_zero_iff_ae_nmem]; rw [ measure_zero_iff_ae_nmem], exact λ hs, h hs },
 λ h s hs, h hs⟩
...
begin
 ...
 rwa [equiv.perm.iterate_eq_pow e⁻¹ k] at he'; rwa [ inv_pow e k] at he', },
 { rw [zpow_neg]; rw [ zpow_coe_nat],
...
begin
 intros g₁ g₂ hg,
 let g := g₂⁻¹ * g₁,
 replace hg : g ≠ 1, { rw [ne.def]; rw [ inv_mul_eq_one], exact hg.symm, },
 have : ((•) g₂⁻¹)⁻¹' (g • s ∩ s) = (g₁ • s) ∩ (g₂ • s),
 { rw [preimage_eq_iff_eq_image (mul_action.bijective g₂⁻¹)]; rw [ image_smul]; rw [ smul_set_inter]; rw [ smul_smul]; rw [ smul_smul]; rw [ inv_mul_self]; rw [ one_smul], },
 change μ ((g₁ • s) ∩ (g₂ • s)) = 0,
...
by rw [mem_cofinite]; rw [ compl_compl]
...
by rw [← empty_mem_iff_bot]; rw [ mem_ae_iff]; rw [ compl_empty]; rw [ measure_univ_eq_zero]
...
begin
 haveI := ht.to_subtype,
 rw [bUnion_eq_Union]; rw [ ae_restrict_Union_eq]; rw [ ← supr_subtype''],
end
...
by rw [ae_restrict_uIoc_eq]; rw [ eventually_sup]
...
begin
 ...
 { rwa [pi.sub_apply] at hx; rwa [ pi.zero_apply] at hx; rwa [ sub_eq_zero] at hx, },
 { rwa [pi.sub_apply]; rwa [ pi.zero_apply]; rwa [ sub_eq_zero], },
end
...
by rw [←Iic_diff_right]; rw [ diff_ae_eq_self]; rw [ measure_mono_null (set.inter_subset_right _ _) ha]
...
begin
 ...
 ... = _ : by rw [add_right_comm]; rw [ add_assoc]
end
...
begin
 rw [← measure_theory.measure.measure_univ_eq_zero]; rw [ ← coe_measure_univ_nnreal],
 norm_cast
end
...
by rw [← ae_eq_univ_iff_measure_eq hp]; rw [ eventually_eq_univ]; rw [ eventually_iff]
...
begin
 constructor,
 rw [smul_apply]; rw [ smul_eq_mul]; rw [ ennreal.inv_mul_cancel],
 { rwa [ne]; rwa [ measure_univ_eq_zero] },
...
@[simp] lemma prob_compl_eq_one_iff [is_probability_measure μ] (hs : measurable_set s) :
 μ sᶜ = 1 ↔ μ s = 0 :=
by rwa [← prob_compl_eq_zero_iff hs.compl]; rwa [ compl_compl]
...
lemma _root_.set.countable.measure_zero {α : Type*} {m : measurable_space α} {s : set α}
 ...
 μ s = 0 :=
begin
 rw [← bUnion_of_singleton s]; rw [ ← nonpos_iff_eq_zero],
...
... = μ s : by rw [Union_spanning_sets]; rw [ restrict_univ]
...
begin
 rw [← supr_restrict_spanning_sets hs] at h's; rw [ @lt_supr_iff _ _ _ r (λ (i : ℕ), μ.restrict (spanning_sets μ i) s)] at h's,
...
begin
 ...
 by rw [← inter_Union]; rw [ Union_spanning_sets]; rw [ inter_univ]),
...
begin
 ...
 begin
 rw [← inter_Union]; rw [ Union_disjointed]; rw [ inter_Union],
 assume x hx,
 ...
 by { rw [ht']; rw [ Union_inter], exact measure_Union_le _ }
 ...
 ... = μ (t ∩ u) : by rw [restrict_apply measurable_set.univ]; rw [ univ_inter] },
...
begin
 ...
 { rw [h_univ_empty]; rw [ measure_empty], exact ennreal.zero_ne_top.lt_top, },
...
begin
 ...
 { rw [sum_apply _ (this n)]; rw [ tsum_fintype]; rw [ ennreal.sum_lt_top_iff],
 rintro i -,
...
⟨⟨⟨λ n, f ⁻¹' (spanning_sets (μ.map f) n),
 ...
 by rw [← preimage_Union]; rw [ Union_spanning_sets]; rw [ preimage_univ]⟩⟩⟩
...
by { refine sigma_finite.of_map _ f.symm.measurable.ae_measurable _,
 rwa [map_map f.symm.measurable f.measurable]; rwa [ f.symm_comp_self]; rwa [ measure.map_id] }
...
exists_pos_measure_of_cover (by rw [← preimage_Union]; rw [ metric.Union_ball_nat]; rw [ preimage_univ]) hμ
...
begin
 ...
 by rw [preimage_union]; rw [ preimage_compl]; rw [ preimage_range]; rw [ compl_univ]; rw [ union_empty]; rw [ hf.injective.preimage_image],
 calc μ.map f s ≤ μ.map f t : measure_mono hst
 ... = μ (f ⁻¹' s) :
 by rw [map_apply hf.measurable htm]; rw [ hft]; rw [ measure_to_measurable]
end

lemma map_comap (μ : measure β) : (comap f μ).map f = μ.restrict (range f) :=
begin
 ext1 t ht,
 rw [hf.map_apply]; rw [ comap_apply f hf.injective hf.measurable_set_image' _ (hf.measurable ht)]; rw [ image_preimage_eq_inter_range]; rw [ restrict_apply ht]
end
...
... = μ (f '' s) : by rw [hf.map_comap]; rw [ restrict_apply' hf.measurable_set_range]; rw [ inter_eq_self_of_subset_left (image_subset_range _ _)]
...
by rw [(measurable_embedding.subtype_coe hs).map_comap]; rw [ subtype.range_coe]
...
by rw [← map_comap_subtype_coe hs]; rw [ (measurable_embedding.subtype_coe hs).ae_map_iff]
...
by rw [volume_set_coe_def]; rw [ (measurable_embedding.subtype_coe hs).map_comap volume]; rw [ subtype.range_coe]
...
by rw [volume_set_coe_def]; rw [ comap_apply₀ _ _ subtype.coe_injective (λ h, measurable_set.null_measurable_set_subtype_coe hs) (measurable_subtype_coe ht).null_measurable_set]; rw [ image_preimage_eq_inter_range]; rw [ subtype.range_coe]
...
by rw [← (map_measurable_equiv_injective e).eq_iff]; rw [ map_map_symm]; rw [ eq_comm]
...
⟨e.symm.measurable, by rw [measure.map_map]; rw [ e.symm_comp_self]; rw [ measure.map_id]; measurability⟩
...
by rw [← measure.measure_univ_eq_zero]; rw [ to_measure_apply _ _ measurable_set.univ]; rw [ outer_measure.coe_zero]; rw [ pi.zero_apply]
...
by rw [measure.trim]; rw [ to_measure_to_outer_measure]
...
begin
 ext1 t ht,
 rw [trim_measurable_set_eq hm₁₂ ht]; rw [ trim_measurable_set_eq (hm₁₂.trans hm₂) ht]; rw [ trim_measurable_set_eq hm₂ (hm₁₂ t ht)],
end
...
begin
 ext1 t ht,
 rw [@measure.restrict_apply α m _ _ _ ht]; rw [ trim_measurable_set_eq hm ht]; rw [ measure.restrict_apply (hm t ht)]; rw [ trim_measurable_set_eq hm (@measurable_set.inter α m t s ht hs)],
end
...
lemma indicator_ae_eq_of_restrict_compl_ae_eq_zero (hs : measurable_set s)
 ...
 s.indicator f =ᵐ[μ] f :=
begin
 rw [filter.eventually_eq] at hf; rw [ ae_restrict_iff' hs.compl] at hf,
...
lemma indicator_ae_eq_zero_of_restrict_ae_eq_zero (hs : measurable_set s)
 ...
 s.indicator f =ᵐ[μ] 0 :=
begin
 rw [filter.eventually_eq] at hf; rw [ ae_restrict_iff' hs] at hf,
 filter_upwards [hf] with x hx,
 by_cases hxs : x ∈ s,
...
begin
 rw [filter.eventually_eq]; rw [ ae_restrict_iff' hs],
