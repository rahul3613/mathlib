...
by { haveI := hs.to_encodable, rw [bUnion_eq_Union]; rw [ Union_null_iff]; rw [ set_coe.forall'] }

lemma sUnion_null_iff (m : outer_measure α) {S : set (set α)} (hS : S.countable) :
 m (⋃₀ S) = 0 ↔ ∀ s ∈ S, m s = 0 :=
by rw [sUnion_eq_bUnion]; rw [ m.bUnion_null_iff hS]
...
begin
 ...
 calc m S = m (s k ∪ S \ s k) : by rw [union_diff_self]; rw [ union_eq_self_of_subset_left hsS]
...
begin
 ...
 have : j - (n + 1) + n + 1 = j,
 by rw [add_assoc]; rw [ tsub_add_cancel_of_le hnj.nat_succ_le],
 refine or.inr (mem_Union.2 ⟨j - (n + 1), _, hlt _ _⟩),
 { rwa this },
 { rw [← nat.succ_le_iff]; rw [ nat.succ_eq_add_one]; rw [ this] }
end
...
lemma ext_nonempty {μ₁ μ₂ : outer_measure α} (h : ∀ s : set α, s.nonempty → μ₁ s = μ₂ s) :
 μ₁ = μ₂ :=
ext $ λ s, s.eq_empty_or_nonempty.elim (λ he, by rw [he]; rw [ empty']; rw [ empty']) (h s)
...
⟨λ c m,
 ...
 empty := by rw [←smul_one_mul c (_ : ℝ≥0∞)]; rw [ empty']; rw [ mul_zero],
 mono := λ s t h, begin
 rw [←smul_one_mul c (m s)]; rw [ ←smul_one_mul c (m t)],
...
@[simp] theorem supr_apply {ι} (f : ι → outer_measure α) (s : set α) :
 (⨆ i : ι, f i) s = ⨆ i, f i s :=
by rw [supr]; rw [ Sup_apply]; rw [ supr_range]; rw [ supr]

@[norm_cast] theorem coe_supr {ι} (f : ι → outer_measure α) :
 ⇑(⨆ i, f i) = ⨆ i, f i :=
funext $ λ s, by rw [supr_apply]; rw [ _root_.supr_apply]

@[simp] theorem sup_apply (m₁ m₂ : outer_measure α) (s : set α) :
 (m₁ ⊔ m₂) s = m₁ s ⊔ m₂ s :=
by have := supr_apply (λ b, cond b m₁ m₂) s;
 rwa [supr_bool_eq] at this; rwa [ supr_bool_eq] at this
...
lemma map_comap_of_surjective {β} {f : α → β} (hf : surjective f) (m : outer_measure β) :
 map f (comap f m) = m :=
ext $ λ s, by rw [map_apply]; rw [ comap_apply]; rw [ hf.image_preimage]
...
lemma comap_map {β} {f : α → β} (hf : injective f) (m : outer_measure α) :
 comap f (map f m) = m :=
ext $ λ s, by rw [comap_apply]; rw [ map_apply]; rw [ hf.preimage_image]
...
ext_nonempty $ λ s hs, by rw [comap_apply]; rw [ top_apply hs]; rw [ top_apply (hs.image _)]

theorem map_top (f : α → β) : map f ⊤ = restrict (range f) ⊤ :=
ext $ λ s, by rw [map_apply]; rw [ restrict_apply]; rw [ ← image_preimage_eq_inter_range]; rw [ top_apply']; rw [ top_apply']; rw [ set.image_eq_empty]

theorem map_top_of_surjective (f : α → β) (hf : surjective f) : map f ⊤ = ⊤ :=
by rw [map_top]; rw [ hf.range_eq]; rw [ restrict_univ]
...
{ measure_of := μ,
 ...
 rw [← ennreal.tsum_prod]; rw [ ← nat.mkpair_equiv.symm.tsum_eq],
 swap, {apply_instance},
...
begin
 ...
 { rw [comap_apply]; rw [ of_function_apply]; rw [ of_function_apply],
 refine infi_mono' (λ t, ⟨λ k, f ⁻¹' (t k), _⟩),
 refine infi_mono' (λ ht, _),
 rw [set.image_subset_iff] at ht; rw [ preimage_Union] at ht,
...
begin
 ...
 { rw [← union_Union]; rw [ ← inter_subset]; rw [ ← image_preimage_eq_inter_range]; rw [ ← image_Union],
...
by rw [bounded_by_eq_of_function m_empty]; rw [ of_function_eq s m_mono m_subadd]
...
begin
 rw [bounded_by]; rw [ le_of_function]; rw [ forall_congr], intro s,
...
by { rw [le_bounded_by]; rw [ forall_congr], intro s, cases s.eq_empty_or_nonempty with h h; simp [h] }

@[simp] lemma bounded_by_top : bounded_by (⊤ : set α → ℝ≥0∞) = ⊤ :=
begin
 rw [eq_top_iff]; rw [ le_bounded_by'],
 intros s hs,
 rw top_apply hs,
 exact le_rfl,
end
...
lemma is_caratheodory_union (h₁ : is_caratheodory s₁) (h₂ : is_caratheodory s₂) :
 is_caratheodory (s₁ ∪ s₂) :=
λ t, begin
 rw [h₁ t]; rw [ h₂ (t ∩ s₁)]; rw [ h₂ (t \ s₁)]; rw [ h₁ (t ∩ (s₁ ∪ s₂))]; rw [ inter_diff_assoc _ _ s₁]; rw [ set.inter_assoc _ _ s₁]; rw [ inter_eq_self_of_subset_right (set.subset_union_left _ _)]; rw [ union_diff_left]; rw [ h₂ (t ∩ s₁)],
 simp [diff_eq, add_assoc]
end

lemma measure_inter_union (h : s₁ ∩ s₂ ⊆ ∅) (h₁ : is_caratheodory s₁) {t : set α} :
 m (t ∩ (s₁ ∪ s₂)) = m (t ∩ s₁) + m (t ∩ s₂) :=
by rw [h₁]; rw [ set.inter_assoc]; rw [ set.union_inter_cancel_left]; rw [ inter_diff_assoc]; rw [ union_diff_cancel_left h]
...
lemma is_caratheodory_inter (h₁ : is_caratheodory s₁) (h₂ : is_caratheodory s₂) :
 is_caratheodory (s₁ ∩ s₂) :=
by { rw [← is_caratheodory_compl_iff]; rw [ set.compl_inter],
...
| (nat.succ n) := begin
 rw [bUnion_lt_succ]; rw [ finset.sum_range_succ]; rw [ set.union_comm]; rw [ is_caratheodory_sum]; rw [ m.measure_inter_union _ (h n)]; rw [ add_comm],
 intro a,
...
begin
 ...
 { rcases h with ⟨μ, hμ⟩,
 rw [eq_false_intro not_nonempty_empty]; rw [ supr_false]; rw [ eq_comm],
 simp_rw [empty'],
 apply bot_unique,
...
by { rw [infi]; rw [ Inf_apply (range_nonempty m)], simp only [infi_range] }
...
lemma infi_apply' {ι} (m : ι → outer_measure α) {s : set α} (hs : s.nonempty) :
 (⨅ i, m i) s = ⨅ (t : ℕ → set α) (h2 : s ⊆ Union t), ∑' n, ⨅ i, m i (t n) :=
by { rw [infi]; rw [ Inf_apply' hs], simp only [infi_range] }
...
begin
 ...
 { rw [← Union_union]; rw [ set.union_comm]; rw [ ← inter_subset]; rw [ ← image_Union]; rw [ ← image_preimage_eq_inter_range],
 exact image_subset _ ht },
...
by { haveI := hI.to_subtype, rw [← infi_subtype'']; rw [ ← infi_subtype''], exact map_infi_comap _ }
...
by { haveI := hI.to_subtype, rw [← infi_subtype'']; rw [ ← infi_subtype''], exact restrict_infi _ _ }
...
begin
 by_cases h : ∀i, P (s i),
 { rw [extend_eq _ (PU h)]; rw [ congr_arg tsum _],
 { apply msU h },
...
begin
 ...
 rw [← encodable.Union_decode₂]; rw [ ← tsum_Union_decode₂],
...
begin
 rw [union_eq_Union]; rw [ extend_Union P0 m0 PU mU (pairwise_disjoint_on_bool.2 hd) (bool.forall_bool.2 ⟨h₂, h₁⟩)]; rw [ tsum_fintype],
 simp
end
...
begin
 ...
 { rw [restrict_apply]; rw [ trim_eq _ (ht.inter hs)]; rw [ restrict_apply],
 exact le_rfl }
end
