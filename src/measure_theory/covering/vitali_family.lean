...
calc ρ s ≤ ρ ((s \ ⋃ (p ∈ h.index), h.covering p) ∪ (⋃ (p ∈ h.index), h.covering p)) :
 ...
 ... = ∑' (p : h.index), ρ (h.covering p) : by rw [hρ h.measure_diff_bUnion]; rw [ measure_bUnion h.index_countable h.covering_disjoint (λ x hx, h.measurable_set_u hx)]; rw [ zero_add]
