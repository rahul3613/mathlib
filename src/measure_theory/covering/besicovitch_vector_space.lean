...
{ c := λ i, (a.r (last N))⁻¹ • (a.c i - a.c (last N)),
 ...
 { left,
 split,
 { rw [dist_eq_norm]; rw [ ← smul_sub]; rw [ norm_smul]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg (inv_nonneg.2 ((a.rpos _)).le)],
 refine mul_le_mul_of_nonneg_left _ (inv_nonneg.2 ((a.rpos _)).le),
 rw [dist_eq_norm] at H,
 convert H.1 using 2,
 abel },
 { rw [← mul_assoc]; rw [ mul_comm τ]; rw [ mul_assoc],
 refine mul_le_mul_of_nonneg_left _ (inv_nonneg.2 ((a.rpos _)).le),
 exact H.2 } },
 { right,
 split,
...
begin
 ...
 begin
 rw [hA]; rw [ measure_bUnion_finset D (λ c hc, measurable_set_ball)],
...
begin
 ...
 have K : 1 - δ / 4 ≤ τ⁻¹, by { rw [inv_eq_one_div]; rw [ le_div_iff τpos], exact I },
...
begin
 ...
 rwa [sub_nonneg]; rwa [ div_le_iff (zero_lt_two.trans hj)]; rwa [ one_mul],
 end,
 ...
 conv_lhs { rw [norm_sub_rev]; rw [ ← abs_of_nonneg invs_nonneg] },
