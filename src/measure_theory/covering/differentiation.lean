...
begin
 ...
 ... = μ (⋃ m n, (to_measurable (ρ + μ) (u m)) ∩ (to_measurable (ρ + μ) (w n))) :
 by { have : μ sᶜ = 0 := v.ae_tendsto_div hρ, rw [measure_to_measurable]; rw [ this]; rw [ zero_add] }
 ...
 rw [inter_comm]; rw [ measure_to_measurable_add_inter_right (measurable_set_to_measurable _ _) J]
 end
...
begin
 ...
 ... ≤ ρ s :
 by { rw [A]; rw [ mul_zero]; rw [ add_zero], exact measure_mono (inter_subset_left _ _) },
 refine v.measure_le_of_frequently_le _ (absolutely_continuous.rfl.smul _) _ _,
 assume x hx,
...
begin
 ...
 ... = ε * μ a : by rw [← add_mul]; rw [ ennreal.add_halves]
end
...
begin
 ...
 rw [lintegral_coe_eq_integral _ A]; rw [ ennreal.to_real_of_real],
 { simp_rw [coe_nnnorm],
 refl },
