...
begin
 ...
 rw [← image_univ]; rw [ eq_empty_of_is_empty (univ : set β)],
 simp },
...
begin
 ...
 rw [this]; rw [ measure_bUnion (u_count i)],
 { refl },
 ...
 rw [← diff_inter_self_eq_diff]; rw [ measure_diff_le_iff_le_add _ (inter_subset_right _ _) ((measure_lt_top μ _).ne)], swap,
 ...
 by { rw [μo]; rw [ ← add_mul]; rw [ ennreal.div_add_div_same]; rw [ add_comm]; rw [ ennreal.div_self]; rw [ one_mul]; simp }
 ...
 rw [← finset.set_bUnion_coe]; rw [ inter_comm _ o]; rw [ inter_Union₂]; rw [ finset.set_bUnion_coe]; rw [ measure_bUnion_finset],
...
begin
 ...
 rw [hB]; rw [ ← finset.set_bUnion_coe],
 ...
 { convert hμv using 2,
 rw [finset.set_bUnion_union]; rw [ ← diff_diff]; rw [ finset.set_bUnion_finset_image] } },
 ...
 ... ≤ (N/(N+1))^n.succ * μ s :
 by { rw [pow_succ]; rw [ mul_assoc], exact mul_le_mul_left' IH _ } },
...
begin
 ...
 rwa [← im_t] at v_disj ; rwa [ A.pairwise_disjoint_image] at v_disj }
end
...
begin
 ...
 ... ≤ (ε / 2) / N : by { have : μ s' = 0 := μt0, rwa [this] at μv ; rwa [ zero_add] at μv },
 ...
 ... = μ s + ε : by rw [add_assoc]; rw [ ennreal.add_halves] }
end
