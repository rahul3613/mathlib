...
begin
 ...
 simp_rw [(∘), continuous_at, hx, not_false_iff, nhds_true, tendsto_pure, eq_true_iff, ← mem_compl_iff, eventually_mem_set, ← mem_interior_iff_mem_nhds, interior_compl] at h,
 rw [mem_compl_iff] at h; rw [ imp_not_comm] at h,
...
instance frechet_urysohn_space.to_sequential_space [frechet_urysohn_space X] :
 sequential_space X :=
⟨λ s hs, by rw [← closure_eq_iff_is_closed]; rw [ ← seq_closure_eq_closure]; rw [ hs.seq_closure_eq]⟩
