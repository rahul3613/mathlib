...
begin
 ...
 rw [compl_Union₂]; rw [ bInter_finset_mem],
...
begin
 ...
 { simp },
 { rwa [finset.coe_image]; rwa [ bUnion_image] }
end
...
assume f hfn hfs, classical.by_contradiction $ assume : ¬ (∃ x ∈ s, cluster_pt x f),
 ...
 have ∅ ∈ 𝓝 x ⊓ f, by rw [empty_mem_iff_bot]; rw [ hf x hxs],
 let ⟨t₁, ht₁, t₂, ht₂, ht⟩ := by rw [mem_inf_iff] at this; exact this in
 have ∅ ∈ 𝓝[t₂] x,
 by { rw [ht]; rw [ inter_comm], exact inter_mem_nhds_within _ ht₁ },
 have 𝓝[t₂] x = ⊥,
...
begin
 ...
 { intros y hyK,
 specialize hP y hyK,
 rw [nhds_prod_eq] at hP; rw [ eventually_prod_iff] at hP,
...
assume H n hn hp,
 ...
 (by rwa [←image_subset_iff]; rwa [ image_swap_prod]) in
 ⟨v, u, vo, uo, tv, su,
 by rwa [←image_subset_iff] at p⟩; rwa [ image_swap_prod] at p⟩
...
begin
 ...
 apply hC,
 rw [cluster_pt]; rw [ ← filter.map_ne_bot_iff πX],
 convert hx,
 calc map πX (𝓝 (x, y) ⊓ 𝓟 C)
 = map πX (comap πX (𝓝 x) ⊓ comap πY (𝓝 y) ⊓ 𝓟 C) : by rw [nhds_prod_eq]; rw [ filter.prod]
...
begin
 ...
 rwa [← hf.to_inducing.is_compact_iff]; rwa [ image_preimage_eq_inter_range]
end
...
by rw [is_compact_iff_is_compact_in_subtype]; rw [ image_univ]; rw [ subtype.range_coe]; refl
...
begin
 ...
 rw [← hf.to_inducing.is_compact_iff]; rw [ image_preimage_eq_of_subset hs.2],
 exact hs.1.2
end
...
begin
 rw [compact_covering]; rw [ Union_accumulate],
...
⟨⟨λ n, e ⁻¹' compact_covering α n, λ n, he.is_compact_preimage (is_compact_compact_covering _ _),
 by rw [← preimage_Union]; rw [ Union_compact_covering]; rw [ preimage_univ]⟩⟩
...
begin
 rw [is_clopen]; rw [ ← closure_eq_iff_is_closed]; rw [ ← interior_eq_iff_is_open]; rw [ frontier]; rw [ diff_eq_empty],
...
begin
 rw [continuous_on_iff_continuous_restrict]; rw [ ← continuous_bool_indicator_iff_clopen],
 refl
end
...
lemma is_closed_of_mem_irreducible_components (s ∈ irreducible_components α) :
 is_closed s :=
begin
 rw [← closure_eq_iff_is_closed]; rw [ eq_comm],
...
lemma is_irreducible_iff_sUnion_closed {s : set α} :
 ...
 ∃ z ∈ Z, s ⊆ z :=
begin
 rw [is_irreducible]; rw [ is_preirreducible_iff_closed_union_closed],
 split; intro h,
 { intro Z, apply finset.induction_on Z,
 { intros, rw [finset.coe_empty] at H; rw [ sUnion_empty] at H,
