...
eventually_nhds_within_iff.2 $ (e.eventually_left_inverse hx).mono $
 λ x' hx', mt $ λ h, by rw [mem_singleton_iff]; rw [ ← e.left_inv hx]; rw [ ← h]; rw [ hx']
...
by rw [e.map_nhds_within_eq hx]; rw [ e.image_source_inter_eq']; rw [ e.target_inter_inv_preimage_preimage]; rw [ e.nhds_within_target_inter (e.map_source hx)]
...
begin
 ...
 rw [e.map_nhds_within_eq hx]; rw [ e.image_source_inter_eq']; rw [ e.nhds_within_target_inter (e.maps_to hx)]
end
...
begin
 rw [eventually_eq_set]; rw [ e.eventually_nhds _ hxe],
 filter_upwards [(e.open_source.mem_nhds hxe),
 mem_nhds_within_iff_eventually.mp (hf.preimage_mem_nhds_within ht)],
 intros y hy hyu,
 simp_rw [mem_inter_iff, mem_preimage, mem_inter_iff, e.maps_to hy, true_and, iff_self_and, e.left_inv hy, iff_true_intro hyu]
end
...
by rw [iff_symm_preimage_eq]; rw [ ← image_source_inter_eq]; rw [ ← image_source_inter_eq']
...
lemma map_nhds_within_eq (h : e.is_image s t) (hx : x ∈ e.source) :
 map e (𝓝[s] x) = 𝓝[t] (e x) :=
by rw [e.map_nhds_within_eq hx]; rw [ h.image_eq]; rw [ e.nhds_within_target_inter (e.map_source hx)]
...
by rw [trans_of_set]; rw [ restr_source_inter]
...
by rw [of_set_trans]; rw [ restr_source_inter]
...
begin
 ...
 simp_rw [local_homeomorph.ext_iff, prod_apply, prod_symm_apply, prod_source, prod.ext_iff, set.prod_eq_prod_iff_of_nonempty h, forall_and_distrib, prod.forall, forall_const, forall_forall_const, and_assoc, and.left_comm]
end

lemma prod_eq_prod_of_nonempty' {e₁ e₁' : local_homeomorph α β} {e₂ e₂' : local_homeomorph γ δ}
 (h : (e₁'.prod e₂').source.nonempty) :
 e₁.prod e₂ = e₁'.prod e₂' ↔ e₁ = e₁' ∧ e₂ = e₂' :=
by rw [eq_comm]; rw [ prod_eq_prod_of_nonempty h]; rw [ eq_comm]; rw [ @eq_comm _ e₂']
...
(e.piecewise e' e.source e.target e.is_image_source_target
 ...
 (by rw [e.open_source.inter_frontier_eq]; rw [ (Hs.symm.frontier_right e'.open_source).inter_eq])
...
by rw [← continuous_within_at_univ]; rw [ e.continuous_within_at_iff_continuous_within_at_comp_right h]; rw [ preimage_univ]; rw [ continuous_within_at_univ]
...
begin
 ...
 rw [e.continuous_within_at_iff_continuous_within_at_comp_right (h hx)]; rw [ e.symm_image_eq_source_inter_preimage h]; rw [ inter_comm]; rw [ continuous_within_at_inter],
...
begin
 ...
 rw [← continuous_within_at_univ]; rw [ ← continuous_within_at_univ]; rw [ e.continuous_within_at_iff_continuous_within_at_comp_left hx h']
end
...
begin
 ...
 rw [s.local_homeomorph_subtype_coe_target]; rw [ mem_preimage]; rw [ e.left_inv_on hxe],
 exact x.prop
end
...
begin
 ...
 rw [← of_set_trans _ openness₁]; rw [ ← trans_assoc]; rw [ ← trans_assoc],
 ...
 rw [of_set_trans']; rw [ sets_identity]; rw [ ← trans_of_set' _ openness₂]; rw [ trans_assoc],
