...
lemma to_pou_fun_zero_of_zero {i : ι} {x : X} (h : f i x = 0) :
 f.to_pou_fun i x = 0 :=
by rw [to_pou_fun]; rw [ h]; rw [ zero_mul]
...
begin
 ...
 { rw [hs]; rw [ mul_support_one_sub], exact λ i, id },
 letI : linear_order ι := linear_order_of_STO well_ordering_rel,
 rw [finsum_eq_sum_of_support_subset _ A]; rw [ finprod_eq_prod_of_mul_support_subset _ B]; rw [ finset.prod_one_sub_ordered]; rw [ sub_sub_cancel],
