...
by rw [← coe_inj]; rw [ opens.coe_bot]; rw [ ← set.not_nonempty_iff_eq_empty]

lemma ne_bot_iff_nonempty (U : opens α) : U ≠ ⊥ ↔ set.nonempty (U : set α) :=
by rw [ne.def]; rw [ ← opens.not_nonempty_iff_eq_bot]; rw [ not_not]
...
begin
 ...
 apply ext,
 rw [coe_Sup]; rw [ hB.open_eq_sUnion' U.is_open],
...
begin
 rw [is_compact_iff_finite_subcover]; rw [ complete_lattice.is_compact_element_iff],
 refine ⟨_, λ H ι U hU, _⟩,
 { introv H hU hU',
 obtain ⟨t, ht⟩ := H ι (λ i, ⟨U i, hU i⟩) (by simpa),
 refine ⟨t, set.subset.trans ht _⟩,
 rw [coe_finset_sup]; rw [ finset.sup_eq_supr],
 refl },
