...
begin
 ...
 rw [coequalizer_is_open_iff]; rw [ colimit_is_open_iff.{u}],
 split,
...
begin
 ...
 erw [sigma_iso_sigma_hom_ι_apply]; erw [ sigma_iso_sigma_hom_ι_apply],
...
by rw [← set.preimage_image_eq (set.range (D.f j i)) (D.ι_injective j)]; rw [ ← set.image_univ]; rw [ ← set.image_univ]; rw [ ←set.image_comp]; rw [ ←coe_comp]; rw [ set.image_univ]; rw [set.image_univ]; rw [ ← image_inter]; rw [ set.preimage_range_inter]
...
begin
 ...
 rw [coe_comp]; rw [ coe_comp]; rw [ ← set.image_image],
 congr' 1,
...
{ J := h.J,
 ...
 begin
 delta mk_core.t',
 rw [category.assoc]; rw [ category.assoc]; rw [ pullback_iso_prod_subtype_inv_snd]; rw [ ← iso.eq_inv_comp]; rw [ pullback_iso_prod_subtype_inv_fst_assoc],
 ...
 rw [← subtype.coe_injective.eq_iff]; rw [ subtype.val_eq_coe]; rw [ subtype.coe_mk]; rw [ and_self],
...
begin
 ...
 rw [ι_from_open_subsets_glue_apply] at e; rw [ ι_from_open_subsets_glue_apply] at e,
 change x = y at e,
 subst e,
...
begin
 ...
 rw [← ι_from_open_subsets_glue]; rw [ coe_comp]; rw [ set.preimage_comp],
 congr' 1,
