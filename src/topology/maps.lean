...
protected lemma inducing.comp {g : β → γ} {f : α → β} (hg : inducing g) (hf : inducing f) :
 inducing (g ∘ f) :=
⟨by rw [hf.induced]; rw [ hg.induced]; rw [ induced_compose]⟩
...
⟨le_antisymm
 ...
 (by { rw [hgf.induced]; rw [ ← continuous_iff_le_induced], apply hg.comp continuous_induced_dom })⟩
...
by rw [hg.nhds_eq_comap]; rw [ tendsto_comap_iff]
...
by { ext x, rw [set.mem_preimage]; rw [ ← closure_induced]; rw [ hf.induced] }

lemma inducing.is_closed_iff {f : α → β} (hf : inducing f) {s : set α} :
 is_closed s ↔ ∃ t, is_closed t ∧ f ⁻¹' t = s :=
by rw [hf.induced]; rw [ is_closed_induced_iff]
...
by rw [hf.induced]; rw [ is_open_induced_iff]
...
discrete_topology_iff_nhds.2 $ λ x, by rw [hf.nhds_eq_comap]; rw [ nhds_discrete]; rw [ comap_pure]; rw [ ← image_singleton]; rw [ hf.inj.preimage_image]; rw [ principal_singleton]
...
protected lemma comp (hg : quotient_map g) (hf : quotient_map f) :
 quotient_map (g ∘ f) :=
⟨hg.left.comp hf.left, by rw [hg.right]; rw [ hf.right]; rw [ coinduced_compose]⟩

protected lemma of_quotient_map_compose (hf : continuous f) (hg : continuous g)
 (hgf : quotient_map (g ∘ f)) : quotient_map g :=
⟨hgf.1.of_comp,
 le_antisymm
 (by { rw [hgf.right]; rw [ ← continuous_iff_coinduced_le], apply continuous_coinduced_rng.comp hf })
...
by rw [continuous_iff_coinduced_le]; rw [ continuous_iff_coinduced_le]; rw [ hf.right]; rw [ coinduced_compose]
...
calc 𝓝 (f x) = map f (map g (𝓝 (f x))) : by rw [map_map]; rw [ hgf.comp_eq_id]; rw [ map_id]
...
begin
 ...
 rwa [image_preimage_eq_inter_range]; rwa [ inter_eq_self_of_subset_left]
end
...
begin
 ...
 rw [←is_closed_compl_iff]; rw [ ←is_closed_compl_iff],
 generalize : s'ᶜ = s,
...
⟨hg.to_embedding.comp hf.to_embedding, show is_closed (range (g ∘ f)),
 by rw [range_comp]; rw [ ←hg.closed_iff_image_closed]; exact hf.closed_range⟩
