...
by rw [h.mem_open_set_iff hU]; rw [ ← not_disjoint_iff_nonempty_inter]; rw [ not_not]

lemma mem_closed_set_iff (h : is_generic_point x S) (hZ : is_closed Z) :
 x ∈ Z ↔ S ⊆ Z :=
by rw [← h.def]; rw [ hZ.closure_subset_iff]; rw [ singleton_subset_iff]

protected lemma image (h : is_generic_point x S) {f : α → β} (hf : continuous f) :
 is_generic_point (f x) (closure (f '' S)) :=
begin
 rw [is_generic_point_def]; rw [ ← h.def]; rw [ ← image_singleton],
...
by simp_rw [is_generic_point, subset_antisymm_iff, this, true_and, closure, subset_sInter_iff, mem_set_of_eq, and_imp, singleton_subset_iff]
...
begin
 ...
 rw [← hx]; rw [ ← hf.closure_image_eq]; rw [ set.image_singleton]
end
...
begin
 ...
 use y,
 change _ = _,
 rw [hf.to_embedding.closure_eq_preimage_closure_image]; rw [ set.image_singleton]; rw [ (show _ = _, from hx)],
 apply set.image_injective.mpr hf.inj,
 ext z,
...
begin
 ...
 rw [← set.image_singleton]; rw [ ← closure_closure],
