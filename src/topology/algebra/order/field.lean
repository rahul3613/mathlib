...
begin
 ...
 rw [filter.mem_map] at this ⊢; rw [ mem_prod_iff] at this ⊢,
...
begin
 ...
 ... = i : by rw [← mul_div_assoc']; rw [ div_self (ne_of_lt $ abs_pos.2 hx₀).symm]; rw [ mul_one],
 specialize hit (x / x₀) this,
 rwa [mul_div_assoc'] at hit ; rwa [ mul_div_cancel_left x hx₀] at hit }
end
...
⟨begin
 ...
 rintro ⟨x₀, y₀⟩,
 by_cases hx₀ : x₀ = 0,
 { rw [hx₀]; rw [ continuous_at]; rw [ zero_mul]; rw [ nhds_prod_eq],
 exact mul_tendsto_nhds_zero_right y₀ },
 by_cases hy₀ : y₀ = 0,
 { rw [hy₀]; rw [ continuous_at]; rw [ mul_zero]; rw [ nhds_prod_eq],
 ...
 : by rw [uncurry]; rw [ nhds_eq_map_mul_left_nhds_one hx₀]; rw [ nhds_eq_map_mul_right_nhds_one hy₀]; rw [ prod_map_map_eq]; rw [ filter.map_map]
 ... = map ((λ x, x₀ * x) ∘ λ x, x * y₀) (map (uncurry (*)) (𝓝 1 ×ᶠ 𝓝 1))
 : by rw [key]; rw [ ← filter.map_map]
 ... ≤ map ((λ (x : α), x₀ * x) ∘ λ x, x * y₀) (𝓝 1) : map_mono (mul_tendsto_nhds_one_nhds_one)
 ... = 𝓝 (x₀*y₀) : by rw [← filter.map_map]; rw [ ← nhds_eq_map_mul_right_nhds_one hy₀]; rw [ nhds_eq_map_mul_left_nhds_one hy₀]; rw [ filter.map_map]; rw [ key₂]; rw [ ← nhds_eq_map_mul_left_nhds_one hxy],
end⟩
...
begin
 ...
 rw [tendsto_const_mul_pow_nhds_iff hc] at h; rw [ ← int.coe_nat_eq_zero] at h,
 exact or.inl h },
 { rw not_le at hn,
...
{ continuous_at_inv₀ :=
 ...
 simp [neg_inv] },
 intros t ht,
 rw [continuous_at]; rw [ (nhds_basis_Ioo_pos t).tendsto_iff $ nhds_basis_Ioo_pos_of_pos $ inv_pos.2 ht],
 rintros ε ⟨hε : ε > 0, hεt : ε ≤ t⁻¹⟩,
 refine ⟨min (t ^ 2 * ε / 2) (t / 2), by positivity, λ x h, _⟩,
 have hx : t / 2 < x,
 { rw [set.mem_Ioo] at h; rw [ sub_lt_comm] at h; rw [ lt_min_iff] at h,
 nlinarith },
 have hx' : 0 < x := (half_pos ht).trans hx,
 have aux : 0 < 2 / t ^ 2 := by positivity,
 rw [set.mem_Ioo] at h ⊢; rw [ ←sub_lt_iff_lt_add'] at h ⊢; rw [ sub_lt_comm] at h ⊢; rw [ ←abs_sub_lt_iff] at h ⊢,
 rw [inv_sub_inv ht.ne' hx'.ne']; rw [ abs_div]; rw [ div_eq_mul_inv],
 suffices : |t * x|⁻¹ < 2 / t ^ 2,
 { rw [←abs_neg]; rw [ neg_sub],
 refine (mul_lt_mul'' h this (by positivity) (by positivity)).trans_le _,
 rw [mul_comm]; rw [ mul_min_of_nonneg _ _ aux.le],
 apply min_le_of_left_le,
 rw [←mul_div]; rw [ ←mul_assoc]; rw [ div_mul_cancel _ (sq_pos_of_pos ht).ne']; rw [ mul_div_cancel' ε two_ne_zero] },
 refine inv_lt_of_inv_lt aux _,
 rw [inv_div]; rw [ abs_of_pos $ mul_pos ht hx']; rw [ sq]; rw [ ←mul_div_assoc'],
