...
begin
 ...
 { rw [partial_inv_left he]; rw [ option.cases_on'],
 exact h _ },
 { have : partial_inv e c = none := dif_neg h,
 rw [this]; rw [ option.cases_on'],
 exact hs _ h }
end
...
by rw [←has_sum_zero_iff]; rw [ hf.has_sum_iff]

lemma tsum_ne_zero_iff (hf : summable f) : ∑' i, f i ≠ 0 ↔ ∃ x, f x ≠ 0 :=
by rw [ne.def]; rw [ tsum_eq_zero_iff hf]; rw [ not_forall]
...
begin
 ...
 rw [set.mem_Ioi]; rw [ inv_eq_one_div]; rw [ one_div]; rw [ pi.inv_apply]; rw [ _root_.inv_pos],
 exact hf' _,
end
