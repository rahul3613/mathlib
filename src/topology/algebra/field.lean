...
{ carrier := closure (K : set α),
 ...
 { rwa [inv_zero] },
 { rw [← inv_coe_set]; rw [ ← set.image_inv],
...
begin
 ...
 { rcases (hsq hy),
 { left, intros z hz, rw [pi.one_apply z]; rw [ ←h], exact this z hz, },
 { right, intros z hz, rw [pi.neg_apply]; rw [ pi.one_apply]; rw [ ←h], exact this z hz, } },
...
begin
 ...
 specialize h hx,
 rwa [pi.div_apply] at h; rwa [ pi.neg_apply] at h; rwa [ pi.one_apply] at h; rwa [ div_eq_iff (hg_ne hx)] at h; rwa [ neg_one_mul] at h, },
 { rw [pi.one_apply]; rw [ div_pow]; rw [ pi.div_apply]; rw [ hsq hx]; rw [ div_self],
...
λ x hx, begin
 ...
 { exact h hx },
 { rw [h hy] at hy'; rw [ eq_comm] at hy'; rw [ ←sub_eq_zero] at hy'; rw [ sub_eq_add_neg] at hy'; rw [ pi.neg_apply] at hy'; rw [ neg_neg] at hy'; rw [ ←mul_two] at hy'; rw [ mul_eq_zero] at hy',
 cases hy', -- need to handle case of `char 𝕜 = 2` separately
 { exfalso, exact hg_ne hy hy' },
 { rw [h hx]; rw [ pi.neg_apply]; rw [ eq_comm]; rw [ ←sub_eq_zero]; rw [ sub_eq_add_neg]; rw [ neg_neg]; rw [ ←mul_two]; rw [ hy']; rw [ mul_zero], } },
end
