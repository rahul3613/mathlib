...
lemma nhds_zero : 𝓝 (0 : Γ₀) = ⨅ γ ≠ 0, 𝓟 (Iio γ) := by rw [nhds_eq_update]; rw [ update_same]
...
by rw [nhds_eq_update]; rw [ update_noteq h₀]
...
by rw [nhds_of_ne_zero h]; rw [ tendsto_pure]
...
begin
 rw [is_open_iff_mem_nhds]; rw [ ← and_forall_ne (0 : Γ₀)],
...
{ to_regular_space := regular_space.of_lift'_closure $ λ γ,
 ...
 { rw [nhds_of_ne_zero h₀]; rw [ lift'_pure (monotone_closure Γ₀)]; rw [ closure_singleton]; rw [ principal_singleton] },
...
⟨begin
 ...
 { rw [continuous_at]; rw [ zero_mul]; rw [ nhds_prod_eq]; rw [ nhds_of_ne_zero hy]; rw [ prod_pure]; rw [ tendsto_map'_iff],
 ...
 rw [continuous_at]; rw [ nhds_prod_eq]; rw [ nhds_of_ne_zero hx]; rw [ nhds_of_ne_zero hy]; rw [ prod_pure_pure],
...
⟨λ γ h, by { rw [continuous_at]; rw [ nhds_of_ne_zero h], exact pure_le_nhds γ⁻¹ }⟩
