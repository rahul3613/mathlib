...
{ smul := (•),
 smul_zero := λ r, by rw [← coe_zero]; rw [ ← coe_smul]; rw [ mul_action_with_zero.smul_zero r],
 zero_smul := ext' (continuous_const_smul _) continuous_const $ λ a,
 by rw [← coe_smul]; rw [ zero_smul]; rw [ coe_zero],
...
{ zero_add := assume a, completion.induction_on a
 ...
 (λ a, by rw [←coe_smul]; rw [ ←coe_zero]; rw [ zero_smul]),
...
{ sub_eq_add_neg := λ a b, completion.induction_on₂ a b
 ...
 (λ a, by rw [←coe_smul]; rw [ ←coe_smul]; rw [ ←coe_neg]; rw [ show -[1+ n] • a = -((n.succ : ℤ) • a),
...
{ smul := (•),
 ...
 smul_zero := λ r, by rw [← coe_zero]; rw [ ← coe_smul]; rw [ smul_zero r],
 .. completion.mul_action M α }
...
{ add_comm := assume a b, completion.induction_on₂ a b
 ...
 (assume x y, by { change ↑x + ↑y = ↑y + ↑x, rw [← coe_add]; rw [ ← coe_add]; rw [ add_comm]}),
