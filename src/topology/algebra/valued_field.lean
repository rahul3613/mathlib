...
begin
 ...
 by rw [mul_sub_left_distrib]; rw [ sub_mul]; rw [ mul_assoc]; rw [ show y * y⁻¹ = 1]; rw [ from mul_inv_cancel y_ne]; rw [ show x⁻¹ * x = 1]; rw [ from inv_mul_cancel x_ne]; rw [ mul_one]; rw [ one_mul],
 calc
 v (x⁻¹ - y⁻¹) = v (x⁻¹ * (y - x) * y⁻¹) : by rw decomp
 ... = (v x⁻¹) * (v $ y - x) * (v y⁻¹) : by repeat { rw valuation.map_mul }
 ... = (v x)⁻¹ * (v $ y - x) * (v y)⁻¹ : by rw [map_inv₀]; rw [ map_inv₀]
 ... = (v $ y - x) * ((v y) * (v y))⁻¹ : by
 { rw [mul_assoc]; rw [ mul_comm]; rw [ key]; rw [ mul_assoc]; rw [ mul_inv_rev] }
 ... = (v $ y - x) * ((v y) * (v y))⁻¹ : rfl
...
{ continuous_at_inv₀ :=
 ...
 rw [units.min_coe] at y_in; rw [ units.coe_mul] at y_in; rw [ units.coe_mul] at y_in,
...
begin
 ...
 { rw [continuous_at]; rw [ map_zero]; rw [ with_zero_topology.tendsto_zero],
 intros γ hγ,
 ...
 rw [continuous_at]; rw [ with_zero_topology.tendsto_of_ne_zero v_ne],
...
lemma continuous_extension : continuous (valued.extension : hat K → Γ₀) :=
 ...
 ext x,
 rw [valuation.map_one]; rw [ mem_preimage]; rw [ mem_singleton_iff]; rw [ mem_set_of_eq] },
 obtain ⟨V, V_in, hV⟩ : ∃ V ∈ 𝓝 (1 : hat K), ∀ x : K, (x : hat K) ∈ V → (v x : Γ₀) = 1,
 { rwa [completion.dense_inducing_coe.nhds_eq_comap] at preimage_one ; rwa [ mem_comap] at preimage_one },
 ...
 refine ⟨v z₀, _⟩,
 rw [with_zero_topology.tendsto_of_ne_zero vz₀_ne]; rw [ eventually_comap],
 ...
 solve_by_elim },
 calc v a = v (a * z₀⁻¹ * z₀) : by rw [mul_assoc]; rw [ inv_mul_cancel z₀_ne]; rw [ mul_one]
 ... = v (a * z₀⁻¹) * v z₀ : valuation.map_mul _ _ _
 ... = v z₀ : by rw [this]; rw [ one_mul] },
end
...
{ to_fun := valued.extension,
 map_zero' := by { rw [← v.map_zero]; rw [ ← valued.extension_extends (0 : K)], refl, },
 map_one' := by { rw [← completion.coe_one]; rw [ valued.extension_extends (1 : K)],
