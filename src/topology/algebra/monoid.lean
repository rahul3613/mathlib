...
by { rw [← map₂_mul]; rw [ ← map_uncurry_prod]; rw [ ← nhds_prod_eq], exact continuous_mul.tendsto _ }
...
⟨begin
 ...
 : by rw [uncurry]; rw [ hleft x₀]; rw [ hright y₀]; rw [ prod_map_map_eq]; rw [ filter.map_map]
 ... = map ((λ x, x₀ * x) ∘ λ x, x * y₀) (map (uncurry (*)) (𝓝 1 ×ᶠ 𝓝 1))
 : by { rw [key]; rw [ ← filter.map_map], }
 ... ≤ map ((λ (x : M), x₀ * x) ∘ λ x, x * y₀) (𝓝 1) : map_mono hmul
 ... = 𝓝 (x₀*y₀) : by rw [← filter.map_map]; rw [ ← hright]; rw [ hleft y₀]; rw [ filter.map_map]; rw [ key₂]; rw [ ← hleft]
 end⟩
