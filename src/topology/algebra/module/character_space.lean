...
begin
 have h₁ : (φ 1) * (1 - φ 1) = 0 := by rw [mul_sub]; rw [ sub_eq_zero]; rw [ mul_one]; rw [ ←map_mul φ]; rw [ one_mul],
...
{ map_one := map_one',
 commutes := λ φ r,
 begin
 { rw [algebra.algebra_map_eq_smul_one]; rw [ algebra.id.map_eq_id]; rw [ ring_hom.id_apply],
 change ((φ : weak_dual 𝕜 A) : A →L[𝕜] 𝕜) (r • 1) = r,
 rw [map_smul]; rw [ algebra.id.smul_eq_mul]; rw [ character_space.coe_coe]; rw [ map_one' φ]; rw [ mul_one] },
 end,
...
begin
 rw [eq_set_map_one_map_mul]; rw [ set.set_of_and],
...
begin
 ...
 { rwa [ring_hom.mem_ker] at this; rwa [ map_sub] at this; rwa [ alg_hom_class.commutes] at this; rwa [ sub_eq_zero] at this, }
end
