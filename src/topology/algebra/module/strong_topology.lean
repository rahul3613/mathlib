...
@uniform_space.replace_topology _ (strong_topology σ F 𝔖)
 ...
 (by rw [strong_topology]; rw [ uniform_add_group.to_uniform_space_eq]; refl)
...
begin
 letI : uniform_space (E →SL[σ] F) := strong_uniformity σ F 𝔖,
 rw [strong_uniformity]; rw [ uniform_space.replace_topology_eq],
...
{ -- given explicitly to help `simps`
 ...
 map_add' := λ f g, by rw [add_comp]; rw [ comp_add],
 map_smul' := λ t f, by rw [smul_comp]; rw [ comp_smulₛₗ],
 continuous_to_fun :=
