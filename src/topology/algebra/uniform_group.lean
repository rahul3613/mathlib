...
{ comap_uniformity := begin
 rw [← uniformity_translate_mul a]; rw [ comap_map] {occs := occurrences.pos [1]},
 rintros ⟨p₁, p₂⟩ ⟨q₁, q₂⟩,
...
by { rw [← comap_swap_uniformity]; rw [ uniformity_eq_comap_nhds_one]; rw [ comap_comap]; rw [ (∘)], refl }
...
@[to_additive] lemma uniform_group.ext {G : Type*} [group G] {u v : uniform_space G}
 ...
 u = v :=
uniform_space_eq $
 by rw [@uniformity_eq_comap_nhds_one _ u _ hu]; rw [ @uniformity_eq_comap_nhds_one _ v _ hv]; rw [ h]
...
begin
 rw [← comap_uniformity_mul_opposite]; rw [ uniformity_eq_comap_nhds_one]; rw [ ← op_one]; rw [ ← comap_unop_nhds]; rw [ comap_comap]; rw [ comap_comap],
 simp [(∘)]
end

@[to_additive] lemma uniformity_eq_comap_inv_mul_nhds_one_swapped :
 𝓤 α = comap (λx:α×α, x.2⁻¹ * x.1) (𝓝 (1:α)) :=
by { rw [← comap_swap_uniformity]; rw [ uniformity_eq_comap_inv_mul_nhds_one]; rw [ comap_comap]; rw [ (∘)], refl }
...
begin
 ...
 { simp only [map_div] },
 rw [uniform_continuous]; rw [ uniformity_eq_comap_nhds_one α]; rw [ uniformity_eq_comap_nhds_one β]; rw [ tendsto_comap_iff]; rw [ this],
...
begin
 ...
 rw [continuous_at]; rw [ nhds_discrete β]; rw [ map_one]; rw [ tendsto_pure],
...
{ uniformity := comap (λp:G×G, p.2 / p.1) (𝓝 1),
 ...
 rw [← nhds_translation_div]; rw [ mem_comap]; rw [ mem_comap],
...
begin
 constructor,
 rw [uniform_continuous]; rw [ uniformity_prod_eq_prod]; rw [ tendsto_map'_iff]; rw [ uniformity_eq_comap_nhds_one' G]; rw [ tendsto_comap_iff]; rw [ prod_comap_comap_eq],
...
begin
 ...
 rw [← separated_iff_t2]; rw [ separated_space_iff]; rw [ ← closure_eq_iff_is_closed],
 split; intro h,
 { apply subset.antisymm,
 { intros x x_in,
 have := group_separation_rel x 1,
 rw div_one at this,
 rw [← this] at x_in; rw [ h] at x_in,
 change x = 1 at x_in,
 simp [x_in] },
 { exact subset_closure } },
 { ext p,
 cases p with x y,
 rw [group_separation_rel x]; rw [ h]; rw [ mem_singleton_iff]; rw [ div_eq_one],
 refl }
end
...
begin
 rw [topological_group.t2_space_iff_one_closed]; rw [ ← is_open_compl_iff]; rw [ is_open_iff_mem_nhds],
 intros x x_not,
...
begin
 ...
 repeat { rw [nhds_prod_eq] at this ; rw [ ←prod_comap_comap_eq] at this },
...
begin
 ...
 (𝓝 (x₀, y₀) ×ᶠ 𝓝 (x₀, y₀))) ≤ 𝓝 0,
 by rwa [uniformity_eq_comap_nhds_zero G]; rwa [ prod_map_map_eq]; rwa [ ←map_le_iff_le_comap]; rwa [ filter.map_map]; rwa [ prod_comap_comap_eq],
...
begin
 ...
 rw [←quotient_group.nhds_eq N 1] at hv; rw [ quotient_group.coe_one] at hv,
 ...
 rw [quotient_group.coe_mul]; rw [ quotient_group.coe_inv]; rw [ hy]; rw [ hg]; rw [ inv_div]; rw [ div_mul_cancel'], },
