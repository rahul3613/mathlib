...
{ one_mul := assume a, completion.induction_on a
 ...
 (assume a b c, by rw [← coe_mul]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ mul_assoc]),
 ...
 (assume a b c, by rw [← coe_add]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ ←coe_add]; rw [ mul_add]),
 ...
 (continuous.mul continuous_fst (continuous_snd.comp continuous_snd))
 (continuous.mul (continuous_fst.comp continuous_snd) (continuous_snd.comp continuous_snd))))
 (assume a b c, by rw [← coe_add]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ ← coe_mul]; rw [ ←coe_add]; rw [ add_mul]),
...
{ to_fun := completion.extension f,
 map_zero' := by rw [← coe_zero]; rw [ extension_coe hf]; rw [ f.map_zero],
 ...
 (assume a b,
 by rw [← coe_add]; rw [ extension_coe hf]; rw [ extension_coe hf]; rw [ extension_coe hf]; rw [ f.map_add]),
 map_one' := by rw [← coe_one]; rw [ extension_coe hf]; rw [ f.map_one],
 ...
 (assume a b,
 by rw [← coe_mul]; rw [ extension_coe hf]; rw [ extension_coe hf]; rw [ extension_coe hf]; rw [ f.map_mul]) }
...
begin
 ...
 { rw [map_coe (uniform_continuous_const_smul r) a]; rw [ algebra.smul_def]; rw [ coe_mul] },
end
...
{ mul_comm := assume a b, completion.induction_on₂ a b
 ...
 (assume a b, by rw [← coe_mul]; rw [ ← coe_mul]; rw [ mul_comm]),
 ..completion.ring }
...
noncomputable def dense_inducing.extend_ring_hom {i : α →+* β} {f : α →+* γ}
 ...
 { simp_rw [← i.map_add, dense_inducing.extend_eq (ue.dense_inducing dr) hf.continuous _, ← f.map_add], },
 end,
 map_mul' :=
 begin
 have h := (uniform_continuous_uniformly_extend ue dr hf).continuous,
 ...
 { simp_rw [← i.map_mul, dense_inducing.extend_eq (ue.dense_inducing dr) hf.continuous _, ← f.map_mul], },
 end, }
