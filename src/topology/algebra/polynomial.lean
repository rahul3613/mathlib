...
begin
 ...
 { rintros c - hc,
 rw [leading_coeff_mul_X] at hc; rw [ leading_coeff_C] at hc,
 ...
 rw [add_comm] at hf; rw [ leading_coeff_add_of_degree_lt (degree_C_le.trans_lt hd)] at hf,
...
h1.nat_degree_eq_zero_iff_eq_one.mp begin
 contrapose !hB,
 rw [← h1.nat_degree_map f] at hB; rw [ nat_degree_eq_card_roots' h2] at hB,
...
begin
 ...
 { rw [eq_one_of_roots_le hB h1 h2 h3]; rw [ polynomial.map_one]; rw [ nat_degree_one]; rw [ zero_tsub]; rw [ pow_zero]; rw [ one_mul]; rw [ coeff_one],
 split_ifs; norm_num [h] },
 rw ← h1.nat_degree_map f,
 obtain hi | hi := lt_or_le (map f p).nat_degree i,
 { rw [coeff_eq_zero_of_nat_degree_lt hi]; rw [ norm_zero], positivity },
 rw [coeff_eq_esymm_roots_of_splits ((splits_id_iff_splits f).2 h2) hi]; rw [ (h1.map _).leading_coeff]; rw [ one_mul]; rw [ norm_mul]; rw [ norm_pow]; rw [ norm_neg]; rw [ norm_one]; rw [ one_pow]; rw [ one_mul],
 apply ((norm_multiset_sum_le _).trans $ sum_le_card_nsmul _ _ $ λ r hr, _).trans,
 { rw [multiset.map_map]; rw [ card_map]; rw [ card_powerset_len]; rw [ ←nat_degree_eq_card_roots' h2]; rw [ nat.choose_symm hi]; rw [ mul_comm]; rw [ nsmul_eq_mul] },
 simp_rw multiset.mem_map at hr,
 obtain ⟨_, ⟨s, hs, rfl⟩, rfl⟩ := hr,
 rw mem_powerset_len at hs,
 lift B to ℝ≥0 using hB,
 rw [←coe_nnnorm]; rw [ ←nnreal.coe_pow]; rw [ nnreal.coe_le_coe]; rw [ ←nnnorm_hom_apply]; rw [ ←monoid_hom.coe_coe]; rw [ monoid_hom.map_multiset_prod],
 refine (prod_le_pow_card _ B $ λ x hx, _).trans_eq (by rw [card_map]; rw [ hs.2]),
...
begin
 ...
 { rw [eq_one_of_roots_le hB h1 h2 h4]; rw [ polynomial.map_one]; rw [ coeff_one],
