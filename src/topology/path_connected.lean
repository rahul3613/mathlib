...
begin
 ...
 rw γ₁.extend_one,
 rwa [← γ₂.extend_extends] at hxt ; rwa [ h] at hxt ; rwa [ γ₂.extend_zero] at hxt },
...
by { ext t, rw [trans_apply]; rw [ map_coe]; rw [ comp_app]; rw [ trans_apply], split_ifs; refl }
...
@[simp] lemma truncate_zero_zero {X : Type*} [topological_space X] {a b : X} (γ : path a b) :
 γ.truncate 0 0 = (path.refl a).cast (by rw [min_self]; rw [ γ.extend_zero]) γ.extend_zero :=
...
@[simp] lemma truncate_one_one {X : Type*} [topological_space X] {a b : X} (γ : path a b) :
 γ.truncate 1 1 = (path.refl b).cast (by rw [min_self]; rw [ γ.extend_one]) γ.extend_one :=
...
begin
 ...
 rw [truncate]; rw [ coe_mk]; rw [ max_eq_left this.1]; rw [ min_eq_left this.2]; rw [ extend_extends']
end
...
begin
 ...
 { rw [Icc_extend_left] at this; rw [ Icc_extend_right] at this,
 change Icc (f 0) (f 1) ⊆ _ at this,
 rw [hf₀] at this; rw [ hf₁] at this,
...
begin
 ...
 left,
 exact hγ₀.1 i hi' },
 { rw [not_le] at hi'; rw [ ← nat.succ_le_iff] at hi',
...
begin
 rw [is_connected_iff_connected_space]; rw [ is_path_connected_iff_path_connected_space],
