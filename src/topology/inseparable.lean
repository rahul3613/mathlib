...
begin
 ...
 tfae_have : 5 ↔ 7, by rw [mem_closure_iff_cluster_pt]; rw [ principal_singleton],
 tfae_have : 5 → 1,
...
by rw [← comap_mk_nhds_mk]; rw [ map_comap_of_surjective surjective_mk]

lemma map_mk_nhds_set : map mk (𝓝ˢ s) = 𝓝ˢ (mk '' s) :=
by rw [← comap_mk_nhds_set_image]; rw [ map_comap_of_surjective surjective_mk]

lemma comap_mk_nhds_set : comap mk (𝓝ˢ t) = 𝓝ˢ (mk ⁻¹' t) :=
by conv_lhs { rw [← image_preimage_eq t surjective_mk]; rw [ comap_mk_nhds_set_image] }
...
by rw [nhds_prod_eq]; rw [ ← prod_map_map_eq']; rw [ map_mk_nhds]; rw [ map_mk_nhds]; rw [ nhds_prod_eq]

lemma map_mk_nhds_within_preimage (s : set (separation_quotient X)) (x : X) :
 map mk (𝓝[mk ⁻¹' s] x) = 𝓝[s] (mk x) :=
by rw [nhds_within]; rw [ ← comap_principal]; rw [ filter.push_pull]; rw [ nhds_within]; rw [ map_mk_nhds]
...
by { rw [← map_prod_map_mk_nhds]; rw [ tendsto_map'_iff], refl }
...
by { rw [nhds_within]; rw [ ← map_prod_map_mk_nhds]; rw [ ← filter.push_pull]; rw [ comap_principal], refl }
