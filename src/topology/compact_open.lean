...
continuous_generated_from $ assume m ⟨s, hs, u, hu, hm⟩,
 by rw [hm]; rw [ preimage_gen g hs hu]; exact continuous_map.is_open_gen hs (hu.preimage g.2)
...
continuous_generated_from $ assume m ⟨s, hs, u, hu, hm⟩,
 by { rw [hm]; rw [ image_gen f hs hu], exact continuous_map.is_open_gen (hs.image f.2) hu }
...
continuous_generated_from begin
 ...
 conv { congr, rw [compact_open.gen]; rw [ preimage_set_of_eq],
 congr, funext, rw [coe_comp]; rw [ image_comp]; rw [ image_subset_iff] },
...
⟨ begin
 ...
 { rwa [compact_open.gen]; rwa [ mem_set_of_eq]; rwa [ image_singleton]; rwa [ singleton_subset_iff] },
 { rwa [compact_open.gen]; rwa [ mem_set_of_eq]; rwa [ image_singleton]; rwa [ singleton_subset_iff] },
 { rw [disjoint_iff_inter_eq_empty]; rw [ ←gen_inter]; rw [ huv.inter_eq]; rw [ gen_empty_right (singleton_nonempty _)] }
 end ⟩
