...
{ obj := λ U, if p₀ ∈ unop U then A else terminal C,
 ...
 then eq_to_hom $ by erw [if_pos h]; erw [ if_pos (le_of_hom i.unop h)]
...
{ X := A,
 ...
 { erw [category.comp_id]; erw [ eq_to_hom_trans], refl },
...
{ desc := λ c, eq_to_hom (if_pos trivial).symm ≫ c.ι.app (op ⊤),
 ...
 uniq' := λ c f h, by rw [← h]; rw [ skyscraper_presheaf_cocone_of_specializes_ι_app]; rw [ eq_to_hom_trans_assoc]; rw [ eq_to_hom_refl]; rw [ category.id_comp] }
...
{ app := λ U, if h : p₀ ∈ U.unop
 ...
 erw [←category.assoc]; erw [ 𝓕.germ_res inc.unop]; erw [ category.assoc]; erw [ category.assoc]; erw [ eq_to_hom_trans],
 refl, },
...
{ app := λ U, f.app (op U.unop.1) ≫ eq_to_hom (if_pos U.unop.2),
 naturality' := λ U V inc,
 begin
 dsimp, erw [category.comp_id]; erw [ ←category.assoc]; erw [ comp_eq_to_hom_iff]; erw [ category.assoc]; erw [ eq_to_hom_trans]; erw [ f.naturality]; erw [ skyscraper_presheaf_map],
...
(λ h, by { dsimp, split_ifs, erw [←category.assoc]; erw [ colimit.ι_desc]; erw [ category.assoc]; erw [ eq_to_hom_trans]; erw [ eq_to_hom_refl]; erw [ category.comp_id], refl }) $
...
colimit.hom_ext $ λ U, by { erw [colimit.ι_desc], dsimp, rw dif_pos U.unop.2, rw [category.assoc, category.assoc, eq_to_hom_trans, eq_to_hom_refl, category.comp_id, presheaf.germ], congr' 3,
...
{ app := λ c, (skyscraper_presheaf_stalk_of_specializes p₀ c specializes_rfl).hom,
 ...
 begin
 erw [←category.assoc]; erw [ colimit.ι_map]; erw [ colimit.iso_colimit_cocone_ι_hom_assoc]; erw [ skyscraper_presheaf_cocone_of_specializes_ι_app]; erw [ category.assoc]; erw [ colimit.ι_desc]; erw [ whiskering_left_obj_map]; erw [ whisker_left_app]; erw [ skyscraper_presheaf_functor.map'_app]; erw [ dif_pos U.unop.2]; erw [ skyscraper_presheaf_cocone_of_specializes_ι_app]; erw [ comp_eq_to_hom_iff]; erw [ category.assoc]; erw [ eq_to_hom_comp_iff]; erw [ ←category.assoc]; erw [ eq_to_hom_trans]; erw [ eq_to_hom_refl]; erw [ category.id_comp]; erw [ comp_eq_to_hom_iff]; erw [ category.assoc]; erw [ eq_to_hom_trans]; erw [ eq_to_hom_refl]; erw [ category.comp_id]; erw [ category_theory.functor.id_map],
 end }
...
{ hom_equiv := λ c 𝓕,
 ...
 { erw [category.id_comp]; erw [ ←category.assoc]; erw [ comp_eq_to_hom_iff]; erw [ category.assoc]; erw [ category.assoc]; erw [ category.assoc]; erw [ category.assoc]; erw [ eq_to_hom_trans]; erw [ eq_to_hom_refl]; erw [ category.comp_id]; erw [ ←category.assoc _ _ α]; erw [ eq_to_hom_trans]; erw [ eq_to_hom_refl]; erw [ category.id_comp], },
 { apply ((if_neg h).symm.rec terminal_is_terminal).hom_ext }
 end,
 hom_equiv_counit' := λ 𝓕 c α,
 begin
 ext U, simp only [equiv.coe_fn_symm_mk, counit_app],
 erw [colimit.ι_desc]; erw [ ←category.assoc]; erw [ colimit.ι_map]; erw [ whisker_left_app]; erw [ category.assoc]; erw [ colimit.ι_desc], refl,
 end }
