...
begin
 rw [stalk_pushforward]; rw [ germ]; rw [ colimit.ι_map_assoc]; rw [ colimit.ι_pre]; rw [ whisker_right_app],
 erw [category_theory.functor.map_id]; erw [ category.id_comp],
 refl,
end
...
begin
 ...
 tactic.op_induction',
 rcases j with ⟨⟨_, _⟩, _⟩,
 rw [colimit.ι_map_assoc]; rw [ colimit.ι_map]; rw [ colimit.ι_pre]; rw [ whisker_left_app]; rw [ whisker_right_app]; rw [ pushforward.id_hom_app]; rw [ eq_to_hom_map]; rw [ eq_to_hom_refl],
 dsimp,
...
begin
 ...
 erw [category_theory.functor.map_id]; erw [ category.id_comp]; erw [ category.id_comp]; erw [ category.id_comp]; erw [ colimit.ι_pre]; erw [ colimit.ι_pre],
 refl,
end
...
lemma stalk_pushforward_iso_of_open_embedding {f : X ⟶ Y} (hf : open_embedding f)
 ...
 { intros U V i, erw [← F.map_comp]; erw [ ← F.map_comp], congr } },
 { ext U,
 rw ← iso.comp_inv_eq,
 erw colimit.ι_map_assoc,
 rw [colimit.ι_pre]; rw [ category.assoc],
 erw [colimit.ι_map_assoc]; erw [ colimit.ι_pre]; erw [ ← F.map_comp_assoc],
...
{ X := F.stalk (f x),
 ...
 naturality' := λ _ _ _, by { erw [colimit.pre_desc]; erw [ category.comp_id], congr } } }
...
{ hom := stalk_pullback_hom _ _ _ _,
 ...
 colimit.ι_pre_assoc],
 erw [colimit.ι_desc]; erw [ colimit.pre_desc]; erw [ colimit.ι_desc]; erw [ category.comp_id],
 simpa
 end,
 inv_hom_id' :=
 begin
 ...
 erw [colimit.map_desc]; erw [ colimit.map_desc]; erw [ colimit.ι_desc_assoc]; erw [ colimit.ι_desc_assoc]; erw [ colimit.ι_desc]; erw [ category.comp_id],
...
by erw [← F.germ_res iWU ⟨x, hxW⟩]; erw [ ← F.germ_res iWV ⟨x, hxW⟩]; erw [ comp_apply]; erw [ comp_apply]; erw [ ih]
...
λ s t hst, section_ext F _ _ _ $ λ x, h x $ by
 rw [stalk_functor_map_germ_apply]; rw [ stalk_functor_map_germ_apply]; rw [ hst]
...
begin
 ...
 dsimp only,
 erw [stalk_functor_map_germ_apply]; erw [ stalk_functor_map_germ_apply],
