...
by rw [is_sheaf_iff_is_sheaf_opens_le_cover]; rw [ is_sheaf_opens_le_cover_iff_is_sheaf_pairwise_intersections]
...
pullback_cone.mk (F.1.map (hom_of_le le_sup_left).op) (F.1.map (hom_of_le le_sup_right).op)
 (by { rw [← F.1.map_comp]; rw [ ← F.1.map_comp], congr })
...
begin
 ...
 have hι : U ⊔ V = supr ι,
 { ext,
 rw [opens.coe_supr]; rw [ set.mem_Union],
 split,
 { rintros (h|h),
 ...
 { rw [← cancel_mono (F.1.map (eq_to_hom $ inf_comm : U ⊓ V ⟶ _).op)]; rw [ category.assoc]; rw [ category.assoc],
 erw [← F.1.map_comp]; erw [ ← F.1.map_comp],
...
begin
 erw [category.assoc]; erw [ ←F.1.map_comp],
...
begin
 ...
 apply opposite.rec,
 rintro ((_|_)|(_|_)); rw [category.assoc]; rw [ category.assoc],
 { erw ← F.1.map_comp,
 convert h₁,
 apply inter_union_pullback_cone_lift_left },
 { erw ← F.1.map_comp,
