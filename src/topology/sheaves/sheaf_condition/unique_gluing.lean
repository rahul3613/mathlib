...
begin
 split ; intros h,
 { ext ⟨i, j⟩,
 rw [left_res]; rw [ types.limit.lift_π_apply']; rw [ fan.mk_π_app]; rw [ right_res]; rw [ types.limit.lift_π_apply']; rw [ fan.mk_π_app],
 exact h i j, },
 { intros i j,
 convert congr_arg (limits.pi.π (λ p : ι × ι, F.obj (op (U p.1 ⊓ U p.2))) (i,j)) h,
 { rw [left_res]; rw [ types.pi_lift_π_apply], refl },
 { rw [right_res]; rw [ types.pi_lift_π_apply], refl } }
end
...
begin
 split ; intros h,
 { ext ⟨i⟩,
 rw [res]; rw [ types.limit.lift_π_apply']; rw [ fan.mk_π_app],
 exact h i, },
 { intro i,
 convert congr_arg (limits.pi.π (λ i : ι, F.obj (op (U i))) i) h,
 rw [res]; rw [ types.pi_lift_π_apply],
 refl },
end
...
begin
 ...
 { rwa [← compatible_iff_left_res_eq_right_res F U sf']; rwa [ inv_hom_id_apply] },
...
begin
 ...
 { intro i,
 rw [← comp_apply]; rw [ ← F.1.map_comp],
 exact gl_spec i },
 { intros gl' gl'_spec,
 convert congr_arg _ (gl_uniq (F.1.map (eq_to_hom V_eq_supr_U.symm).op gl') (λ i,_)) ;
 rw [← comp_apply]; rw [ ← F.1.map_comp],
 { rw [eq_to_hom_op]; rw [ eq_to_hom_op]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]; rw [ F.1.map_id]; rw [ id_apply] },
 { convert gl'_spec i } }
end
...
begin
 ...
 rw [← comp_apply]; rw [ ← F.1.map_comp]; rw [ eq_to_hom_op]; rw [ eq_to_hom_op]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]; rw [ F.1.map_id]; rw [ id_apply] },
 apply eq_of_locally_eq,
 intro i,
 rw [← comp_apply]; rw [ ← comp_apply]; rw [ ← F.1.map_comp],
 convert h i,
end
