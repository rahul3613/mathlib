...
lemma restrict_restrict {X : Top} {C : Type*} [category C] [concrete_category C]
 ...
 x |_ V |_ U = x |_ U :=
by { delta restrict_open restrict, rw [← comp_apply]; rw [ ← functor.map_comp], refl }
...
by { delta restrict_open restrict, rw [← comp_apply]; rw [ nat_trans.naturality]; rw [ comp_apply] }
...
begin
 rw [← category.id_comp ((id ℱ).inv.app (op U))]; rw [ ← nat_iso.app_inv]; rw [ iso.comp_inv_eq],
 dsimp [id],
 rw colimit.ι_desc_assoc,
 dsimp,
 rw [← ℱ.map_comp]; rw [ ← ℱ.map_id], refl,
end
