...
begin
 obtain H | ⟨j, H⟩ := H,
 { use i, rwa [fun_split_at_symm_apply]; rwa [ dif_pos rfl] },
 { use j, rwa [fun_split_at_symm_apply]; rwa [ dif_neg j.prop]; rwa [ subtype.coe_eta] },
end
...
begin
 simp_rw [to_loop, from_loop, continuous_map.comp_assoc, to_continuous_map_as_coe, to_continuous_map_comp_symm, continuous_map.comp_id], ext, refl,
end
...
begin
 ...
 all_goals { use i, rw [fun_split_at_symm_apply]; rw [ dif_pos rfl], exact yH } },
 all_goals { intro, ext, erw [homotopy_to_apply]; erw [ to_loop_apply] },
...
begin
 ...
 { rw [p.2 _ ⟨j, jH⟩]; rw [ q.2 _ ⟨j, jH⟩], split; { apply boundary, exact ⟨⟨j, h⟩, jH⟩ } } },
 all_goals { intro,
...
by { rw [trans_at_indep _ q]; rw [ ← from_loop_trans_to_loop], apply quotient.sound, refl }

/-- Characterization of multiplicative inverse -/
lemma inv_spec [nonempty N] {i} {p : Ω^N X x} : (⟦p⟧⁻¹ : homotopy_group N X x) = ⟦symm_at i p⟧ :=
by { rw [symm_at_indep _ p]; rw [ ← from_loop_symm_to_loop], apply quotient.sound, refl }
