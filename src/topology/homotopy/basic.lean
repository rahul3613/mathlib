...
show ite _ _ _ = _,
by split_ifs; { rw [extend]; rw [ continuous_map.coe_Icc_extend]; rw [ set.Icc_extend_of_mem], refl }
...
begin
 ...
 ext,
 rw [F.to_homotopy.extend_apply_of_mem_I ⟨ht₀, ht₁⟩]; rw [ F.to_homotopy.curry_apply] },
 { convert F.prop 1,
 ext,
 rw [F.to_homotopy.extend_apply_of_one_le (le_of_not_le ht₁)]; rw [ F.to_homotopy.curry_apply]; rw [ F.to_homotopy.apply_one] } },
 { convert F.prop 0,
 ext,
 rw [F.to_homotopy.extend_apply_of_le_zero (le_of_not_le ht₀)]; rw [ F.to_homotopy.curry_apply]; rw [ F.to_homotopy.apply_zero] }
end
