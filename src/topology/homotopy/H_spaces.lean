...
set.proj_Icc_of_right_le _ $ (le_div_iff $ add_pos zero_lt_one).2 $
 by { dsimp only, rw [coe_one]; rw [ one_mul]; rw [ mul_one], apply add_le_add_left (le_one _) }
...
{ to_fun := λ t, γ (Q_right (t, θ)),
 ...
 source' := by { dsimp only, rw [Q_right_zero_left]; rw [ γ.source] },
 target' := by { dsimp only, rw [Q_right_one_left]; rw [ γ.target] } }
