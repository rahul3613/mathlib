...
begin
 ...
 replace huv : s ⊆ u' ∪ v',
 { rw [image_subset_iff] at huv; rw [ preimage_union] at huv,
...
begin
 ...
 rwa [set.image_preimage_eq_of_subset hsf] at this ; rwa [ image_union] at this },
...
begin
 ...
 rwa [subtype.image_preimage_coe]; rwa [ inter_eq_left_iff_subset.mpr hsF] },
 ...
 rw [subtype.image_preimage_coe]; rw [ inter_eq_left_iff_subset.mpr hsF]
end
...
begin
 by_cases hx : x ∈ F,
 { rw [connected_component_in_eq_image hx, connected_component_in_eq_image (h hx), ← show (coe : G → α) ∘ inclusion h = coe, by ext ; refl, image_comp],
...
⟨λ hs, classical.by_contradiction $ λ h,
 ...
 mt (λ h2, or.inr $ (by rw [← compl_compl s]; rw [ h2]; rw [ compl_empty] : s = univ)) h⟩,
...
{ is_preconnected_univ := by rwa [← embedding_subtype_coe.to_inducing.is_preconnected_image]; rwa [ image_univ]; rwa [ subtype.range_coe] }
...
lemma is_connected_iff_sUnion_disjoint_open {s : set α} :
 ...
 ∃ u ∈ U, s ⊆ u :=
begin
 rw [is_connected]; rw [ is_preconnected_iff_subset_of_disjoint],
 split; intro h,
 ...
 rw [finset.coe_insert] at H; rw [ sUnion_insert] at H,
 ...
 { rw [finset.mem_insert] at ht; rw [ finset.mem_singleton] at ht,
 rcases ht with rfl|rfl; tauto },
 { intros t₁ t₂ ht₁ ht₂ hst,
 rw nonempty_iff_ne_empty at hst,
 rw [finset.mem_insert] at ht₁ ht₂; rw [ finset.mem_singleton] at ht₁ ht₂,
...
begin
 ...
 { rwa [disjoint_iff_inter_eq_empty]; rwa [ ←inter_inter_distrib_right]; rwa [ inter_comm] }
end
...
by rw [← hf.preimage_connected_component h_fibers]; rw [ image_preimage_eq _ hf.surjective]
...
begin
 ...
 rw [← connected_components.quotient_map_coe.image_connected_component]; rw [ ← connected_components_preimage_singleton]; rw [ image_preimage_eq _ connected_components.surjective_coe],
