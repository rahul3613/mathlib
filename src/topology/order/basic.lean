...
mem_nhds_within.2 ⟨Iio c, is_open_Iio, H.2,
 by rw [inter_comm]; rw [ Ioi_inter_Iio]; exact Ioo_subset_Ioo_left H.1⟩
...
by rw [t.topology_eq_generate_intervals]; rw [ nhds_generate_from];
from le_antisymm
 (le_inf
...
begin
 constructor,
 conv in ((𝓝 f).small_sets) { rw [nhds_pi]; rw [ filter.pi] },
...
begin
 ...
 rw [nhds_induced]; rw [ nhds_generate_from]; rw [ nhds_eq_order (f a)],
 apply le_antisymm,
...
begin
 ...
 refine ⟨eq_of_nhds_eq_nhds (λ a, _)⟩,
 rw [nhds_induced]; rw [ nhds_generate_from]; rw [ nhds_eq_order (a : α)],
 apply le_antisymm,
 { refine le_infi (λ s, le_infi $ λ hs, le_principal_iff.2 _),
 rcases hs with ⟨ab, b, rfl|rfl⟩,
...
begin
 rw [nhds_within]; rw [ nhds_eq_order],
...
begin
 ...
 { rw [← Icc_union_Ioo_eq_Ico le_rfl hab]; rw [ H]; rw [ union_empty] },
...
⟨begin
 ...
 rintros x ⟨hx, hx'⟩,
 apply h',
 rw [sub_lt_comm] at hx; rw [ lt_min_iff] at hx; rw [ sub_lt_sub_iff_left] at hx,
 rw [← sub_lt_iff_lt_add'] at hx'; rw [ lt_min_iff] at hx'; rw [ sub_lt_sub_iff_right] at hx',
 exact ⟨hx.1, hx'.2⟩ },
 { rintros ⟨ε, ε_pos, h⟩,
...
lemma is_lub_of_mem_closure {s : set α} {a : α} (hsa : a ∈ upper_bounds s) (hsf : a ∈ closure s) :
 is_lub s a :=
begin
 rw [mem_closure_iff_cluster_pt] at hsf; rw [ cluster_pt] at hsf; rw [ inf_comm] at hsf,
...
begin
 ...
 { rw [← diff_subset_closure_iff]; rw [ Ici_diff_Ioi_same]; rw [ singleton_subset_iff],
...
begin
 ...
 { rw [← diff_subset_closure_iff]; rw [ Icc_diff_Ioo_same hab.le],
...
by rw [← compl_Iio]; rw [ interior_compl]; rw [ closure_Iio' ha]; rw [ compl_Iic]
...
by rw [← Ici_inter_Iic]; rw [ interior_inter]; rw [ interior_Ici]; rw [ interior_Iic]; rw [ Ioi_inter_Iio]

@[simp] lemma interior_Ico [no_min_order α] {a b : α} : interior (Ico a b) = Ioo a b :=
by rw [← Ici_inter_Iio]; rw [ interior_inter]; rw [ interior_Ici]; rw [ interior_Iio]; rw [ Ioi_inter_Iio]

@[simp] lemma interior_Ioc [no_max_order α] {a b : α} : interior (Ioc a b) = Ioo a b :=
by rw [← Ioi_inter_Iic]; rw [ interior_inter]; rw [ interior_Ioi]; rw [ interior_Iic]; rw [ Ioi_inter_Iio]
...
by rw [frontier]; rw [ closure_Ioo h.ne]; rw [ interior_Ioo]; rw [ Icc_diff_Ioo_same h.le]

@[simp] lemma frontier_Ico [no_min_order α] {a b : α} (h : a < b) : frontier (Ico a b) = {a, b} :=
by rw [frontier]; rw [ closure_Ico h.ne]; rw [ interior_Ico]; rw [ Icc_diff_Ioo_same h.le]

@[simp] lemma frontier_Ioc [no_max_order α] {a b : α} (h : a < b) : frontier (Ioc a b) = {a, b} :=
by rw [frontier]; rw [ closure_Ioc h.ne]; rw [ interior_Ioc]; rw [ Icc_diff_Ioo_same h.le]
...
begin
 ...
 { rw [filter_eq_bot_of_is_empty at_top]; rw [ filter.map_bot]; rw [ hb']; rw [ nhds_within_empty],
 exact ⟨λ x, hb'.subset (hb x.2)⟩ },
 { rw [← comap_coe_nhds_within_Iio_of_Ioo_subset hb (λ _, hs a ha)]; rw [ map_comap_of_mem],
...
lemma monotone.map_supr_of_continuous_at' {ι : Sort*} [nonempty ι] {f : α → β} {g : ι → α}
 ...
 f (⨆ i, g i) = ⨆ i, f (g i) :=
by rw [supr]; rw [ Mf.map_Sup_of_continuous_at' Cf (range_nonempty g)]; rw [ ← range_comp]; rw [ supr]
...
lemma monotone.map_supr_of_continuous_at {ι : Sort*} {f : α → β} {g : ι → α}
 ...
 f (⨆ i, g i) = ⨆ i, f (g i) :=
by rw [supr]; rw [ Mf.map_Sup_of_continuous_at Cf fbot]; rw [ ← range_comp]; rw [ supr]
...
lemma monotone.map_csupr_of_continuous_at {f : α → β} {g : γ → α}
 ...
 f (⨆ i, g i) = ⨆ i, f (g i) :=
by rw [supr]; rw [ Mf.map_cSup_of_continuous_at Cf (range_nonempty _) H]; rw [ ← range_comp]; rw [ supr]
