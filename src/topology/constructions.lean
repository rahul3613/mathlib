...
by rw [inf_principal_eq_bot_iff_comap]; rw [ nhds_within]; rw [ nhds_within]; rw [ comap_inf]; rw [ comap_principal]; rw [ nhds_induced]

lemma nhds_ne_subtype_eq_bot_iff {S : set α} {x : S} : 𝓝[{x}ᶜ] x = ⊥ ↔ 𝓝[{x}ᶜ] (x : α) ⊓ 𝓟 S = ⊥ :=
by rw [← nhds_within_subtype_eq_bot_iff]; rw [ preimage_compl]; rw [ ← image_singleton]; rw [ subtype.coe_injective.preimage_image ]

lemma nhds_ne_subtype_ne_bot_iff {S : set α} {x : S} :
 (𝓝[{x}ᶜ] x).ne_bot ↔ (𝓝[{x}ᶜ] (x : α) ⊓ 𝓟 S).ne_bot :=
by rw [ne_bot_iff]; rw [ ne_bot_iff]; rw [ not_iff_not]; rw [ nhds_ne_subtype_eq_bot_iff]
...
by rw [filter.prod]; rw [ prod.topological_space]; rw [ nhds_inf]; rw [ nhds_induced]; rw [ nhds_induced]
...
by rw [nhds_prod_eq]; rw [ mem_prod_iff]
...
by { cases x, rw [nhds_prod_eq]; rw [ filter.tendsto_prod_iff'], }
...
discrete_topology_iff_nhds.2 $ λ ⟨a, b⟩,
 by rw [nhds_prod_eq]; rw [ nhds_discrete α]; rw [ nhds_discrete β]; rw [ filter.prod_pure_pure]

lemma prod_mem_nhds_iff {s : set α} {t : set β} {a : α} {b : β} :
 s ×ˢ t ∈ 𝓝 (a, b) ↔ s ∈ 𝓝 a ∧ t ∈ 𝓝 b :=
by rw [nhds_prod_eq]; rw [ prod_mem_prod_iff]
...
by rw [nhds_prod_eq]; rw [ filter.prod_comm]; rw [ nhds_prod_eq]; refl
...
begin
 ...
 rcases x with ⟨x, y⟩,
 rw [mem_map] at hs; rw [ nhds_within] at hs; rw [ mem_inf_principal] at hs; rw [ mem_nhds_prod_iff] at hs,
...
have (𝓝 a ×ᶠ 𝓝 b) ⊓ 𝓟 (s ×ˢ t) = (𝓝 a ⊓ 𝓟 s) ×ᶠ (𝓝 b ⊓ 𝓟 t),
 by rw [←prod_inf_prod]; rw [ prod_principal_principal],
...
⟨by rw [prod.topological_space]; rw [ prod.topological_space]; rw [ hf.induced]; rw [ hg.induced]; rw [ induced_compose]; rw [ induced_compose]; rw [ induced_inf]; rw [ induced_compose]; rw [ induced_compose]⟩
...
by { rw [← is_open_compl_iff]; rw [ compl_range_inl], exact is_open_range_inr }

lemma is_closed_range_inr : is_closed (range (inr : β → α ⊕ β)) :=
by { rw [← is_open_compl_iff]; rw [ compl_range_inr], exact is_open_range_inl }
...
by { rw [inducing_coe.dense_iff]; rw [ set_coe.forall], refl }
...
| ⟨a, ha⟩ := by rw [nhds_subtype_eq_comap]; rw [ tendsto_comap_iff]; rw [ subtype.coe_mk]
...
by rw [nhds_pi]; rw [ filter.tendsto_pi]
...
lemma set_pi_mem_nhds {i : set ι} {s : Π a, set (π a)}
 ...
 pi i s ∈ 𝓝 x :=
by { rw [pi_def]; rw [ bInter_mem hi], exact λ a ha, (continuous_apply a).continuous_at (hs a ha) }

lemma set_pi_mem_nhds_iff {I : set ι} (hI : I.finite) {s : Π i, set (π i)} (a : Π i, π i) :
 I.pi s ∈ 𝓝 a ↔ ∀ (i : ι), i ∈ I → s i ∈ 𝓝 (a i) :=
by { rw [nhds_pi]; rw [ pi_mem_pi_iff hI], apply_instance }
...
begin
 rw [← bUnion_of_singleton s]; rw [ preimage_Union₂],
