...
by rw [nhds_list]; rw [ list.traverse_nil _]; apply_instance

lemma nhds_cons (a : α) (l : list α) :
 𝓝 (a :: l) = list.cons <$> 𝓝 a <*> 𝓝 l :=
by rw [nhds_list]; rw [ list.traverse_cons _]; rw [ ← nhds_list]; apply_instance

lemma list.tendsto_cons {a : α} {l : list α} :
 tendsto (λp:α×list α, list.cons p.1 p.2) (𝓝 a ×ᶠ 𝓝 l) (𝓝 (a :: l)) :=
by rw [nhds_cons]; rw [ tendsto]; rw [ filter.map_prod]; exact le_rfl
...
by rw [this]; rw [ filter.tendsto_map'_iff]
...
continuous_iff_continuous_at.mpr $
 assume ⟨a, l⟩, by rw [continuous_at]; rw [ nhds_prod_eq]; exact tendsto_insert_nth'
...
begin
 ...
 rw [continuous_at] at this; rw [ nhds_prod_eq] at this,
...
| ⟨l, hl⟩ :=
begin
 rw [insert_nth]; rw [ tendsto_subtype_rng],
 simp [insert_nth_val],
...
continuous_iff_continuous_at.mpr $ assume ⟨a, l⟩,
 by rw [continuous_at]; rw [ nhds_prod_eq]; exact tendsto_insert_nth
...
--| ⟨l, hl⟩ :=
begin
 rw [continuous_at]; rw [ remove_nth]; rw [ tendsto_subtype_rng],
