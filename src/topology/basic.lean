...
by { rw [← is_open_compl_iff]; rw [ compl_empty], exact is_open_univ }

@[simp] lemma is_closed_univ : is_closed (univ : set α) :=
by { rw [← is_open_compl_iff]; rw [ compl_univ], exact is_open_empty }
...
by rw [←is_open_compl_iff]; rw [ compl_compl]
...
by rw [diff_subset_iff]; rw [ union_eq_self_of_subset_left subset_closure]
...
begin
 rw [interior]; rw [ closure]; rw [ compl_sUnion]; rw [ compl_image_set_of],
...
⟨λ h, bot_unique $ h ▸ l.le_lift'_closure,
 λ h, h.symm ▸ by rw [lift'_bot (monotone_closure _)]; rw [ closure_empty]; rw [ principal_empty]⟩
...
by rw [dense_iff_closure_eq]; rw [ closure_compl]; rw [ compl_univ_iff]
...
by rw [dense]; rw [ dense]; rw [ closure_closure]
...
by rw [frontier]; rw [ diff_diff_right_self]; rw [ inter_eq_self_of_subset_right interior_subset_closure]

@[simp] lemma self_diff_frontier (s : set α) : s \ frontier s = interior s :=
by rw [frontier]; rw [ diff_diff_right]; rw [ diff_eq_empty.2 subset_closure]; rw [ inter_eq_self_of_subset_right interior_subset]; rw [ empty_union]

lemma frontier_eq_closure_inter_closure {s : set α} :
 frontier s = closure s ∩ closure sᶜ :=
by rw [closure_compl]; rw [ frontier]; rw [ diff_eq]
...
begin
 ...
 rwa [inter_diff_self] at this; rwa [ subset_empty_iff] at this,
end
...
by { rw [←frontier_compl]; rw [ ←closure_compl], refl }
...
by rwa [cluster_pt]; rwa [ inf_eq_right.mpr H]
...
by rw [acc_pt]; rw [ nhds_within]; rw [ cluster_pt]; rw [ inf_assoc]

/-- `x` is an accumulation point of a set `C` iff it is a cluster point of `C ∖ {x}`.-/
lemma acc_principal_iff_cluster (x : α) (C : set α) :
 acc_pt x (𝓟 C) ↔ cluster_pt x (𝓟(C \ {x})) :=
by rw [acc_iff_cluster]; rw [ inf_principal]; rw [ inter_comm]; refl
...
lemma mem_interior_iff_mem_nhds {s : set α} {a : α} :
 a ∈ interior s ↔ s ∈ 𝓝 a :=
by rw [interior_eq_nhds']; rw [ mem_set_of_eq]
...
lemma is_open_singleton_iff_punctured_nhds {α : Type*} [topological_space α] (a : α) :
 is_open ({a} : set α) ↔ (𝓝[≠] a) = ⊥ :=
by rw [is_open_singleton_iff_nhds_eq_pure]; rw [ nhds_within]; rw [ ← mem_iff_inf_principal_compl]; rw [ ← le_pure_iff]; rw [ nhds_ne_bot.le_pure_iff]

lemma mem_closure_iff_frequently {s : set α} {a : α} : a ∈ closure s ↔ ∃ᶠ x in 𝓝 a, x ∈ s :=
by rw [filter.frequently]; rw [ filter.eventually]; rw [ ← mem_interior_iff_mem_nhds]; rw [ closure_eq_compl_interior_compl]; refl
...
lemma dense.open_subset_closure_inter {s t : set α} (hs : dense s) (ht : is_open t) :
 t ⊆ closure (t ∩ s) :=
calc t = t ∩ closure s : by rw [hs.closure_eq]; rw [ inter_univ]
...
begin
 ...
 have := sup_le h' h,
 rw [sup_inf_right] at this; rw [ sup_principal] at this; rw [ union_compl_self] at this; rw [ principal_univ] at this; rw [ inf_top_eq] at this; rw [ sup_le_iff] at this,
 exact this.1
end
...
by rw [← mem_compl_iff]; rw [ ← interior_compl]; rw [ mem_interior_iff_mem_nhds]; rw [ function.compl_support]; refl
...
⟨assume hf s hs, by simpa using (continuous_def.1 hf sᶜ hs.is_open_compl).is_closed_compl,
 ...
 by rw [←is_closed_compl_iff]; rw [ ←is_closed_compl_iff]; exact hf _⟩
...
lemma continuous.range_subset_closure_image_dense {f : α → β} (hf : continuous f)
 {s : set α} (hs : dense s) :
 range f ⊆ closure (f '' s) :=
by { rw [← image_univ]; rw [ ← hs.closure_eq], exact image_closure_subset_closure_image hf }
...
lemma dense_range.comp {g : β → γ} {f : κ → β} (hg : dense_range g) (hf : dense_range f)
 (cg : continuous g) :
 dense_range (g ∘ f) :=
by { rw [dense_range]; rw [ range_comp], exact hg.dense_image cg hf }
