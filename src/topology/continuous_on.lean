...
by rw [nhds_within]; rw [ principal_univ]; rw [ inf_top_eq]
...
lemma diff_mem_nhds_within_diff {x : α} {s t : set α} (hs : s ∈ 𝓝[t] x) (t' : set α) :
 s \ t' ∈ 𝓝[t \ t'] x :=
begin
 rw [nhds_within]; rw [ diff_eq]; rw [ diff_eq]; rw [ ← inf_principal]; rw [ ← inf_assoc],
...
begin
 ...
 rw [set.inter_comm]; rw [ ←(subtype.preimage_coe_eq_preimage_coe_iff.mp hu2)],
 exact hu1.inter ht,
end
...
by rw [nhds_within_restrict' t hs]; rw [ nhds_within_restrict' u hs]; rw [ h₂]

theorem nhds_within_eq_nhds_within {a : α} {s t u : set α}
 (h₀ : a ∈ s) (h₁ : is_open s) (h₂ : t ∩ s = u ∩ s) :
 𝓝[t] a = 𝓝[u] a :=
by rw [nhds_within_restrict t h₀ h₁]; rw [ nhds_within_restrict u h₀ h₁]; rw [ h₂]

@[simp] theorem nhds_within_eq_nhds {a : α} {s : set α} : 𝓝[s] a = 𝓝 a ↔ s ∈ 𝓝 a :=
by rw [nhds_within]; rw [ inf_eq_left]; rw [ le_principal_iff]
...
by rw [nhds_within]; rw [ principal_empty]; rw [ inf_bot_eq]

theorem nhds_within_union (a : α) (s t : set α) :
 𝓝[s ∪ t] a = 𝓝[s] a ⊔ 𝓝[t] a :=
by { delta nhds_within, rw [←inf_sup_left]; rw [ sup_principal] }
...
theorem nhds_within_sUnion {S : set (set α)} (hS : S.finite) (a : α) :
 𝓝[⋃₀ S] a = ⨆ s ∈ S, 𝓝[s] a :=
by rw [sUnion_eq_bUnion]; rw [ nhds_within_bUnion hS]

theorem nhds_within_Union {ι} [finite ι] (s : ι → set α) (a : α) : 𝓝[⋃ i, s i] a = ⨆ i, 𝓝[s i] a :=
by rw [← sUnion_range]; rw [ nhds_within_sUnion (finite_range s)]; rw [ supr_range]

theorem nhds_within_inter (a : α) (s t : set α) :
 𝓝[s ∩ t] a = 𝓝[s] a ⊓ 𝓝[t] a :=
by { delta nhds_within, rw [inf_left_comm]; rw [ inf_assoc]; rw [ inf_principal]; rw [ ←inf_assoc]; rw [ inf_idem] }

theorem nhds_within_inter' (a : α) (s t : set α) :
 𝓝[s ∩ t] a = (𝓝[s] a) ⊓ 𝓟 t :=
by { delta nhds_within, rw [←inf_principal]; rw [ inf_assoc] }

theorem nhds_within_inter_of_mem {a : α} {s t : set α} (h : s ∈ 𝓝[t] a) :
 𝓝[s ∩ t] a = 𝓝[t] a :=
by { rw [nhds_within_inter]; rw [ inf_eq_right], exact nhds_within_le_of_mem h }

theorem nhds_within_inter_of_mem' {a : α} {s t : set α} (h : s ∈ 𝓝[t] a) :
 𝓝[t ∩ s] a = 𝓝[t] a :=
by rw [inter_comm]; rw [ nhds_within_inter_of_mem h]

@[simp] theorem nhds_within_singleton (a : α) : 𝓝[{a}] a = pure a :=
by rw [nhds_within]; rw [ principal_singleton]; rw [ inf_eq_right.2 (pure_le_nhds a)]

@[simp] theorem nhds_within_insert (a : α) (s : set α) :
 𝓝[insert a s] a = pure a ⊔ 𝓝[s] a :=
by rw [← singleton_union]; rw [ nhds_within_union]; rw [ nhds_within_singleton]
...
by rw [← nhds_within_singleton]; rw [ ← nhds_within_union]; rw [ compl_union_self]; rw [ nhds_within_univ]
...
by { delta nhds_within, rw [nhds_prod_eq]; rw [ ←filter.prod_inf_prod]; rw [ filter.prod_principal_principal] }
...
begin
 ...
 rw [infi_split _ (λ i, i ∈ I)]; rw [ inf_right_comm],
...
by rw [comap_principal]; rw [ set.preimage_image_eq _ subtype.coe_injective]
...
by rw [nhds_within]; rw [ nhds_subtype]; rw [ principal_subtype]; rw [ ←comap_inf]; rw [ ←nhds_within]
...
theorem mem_nhds_subtype_iff_nhds_within {s : set α} {a : s} {t : set s} :
 t ∈ 𝓝 a ↔ coe '' t ∈ 𝓝[s] (a : α) :=
by rw [nhds_within_eq_map_subtype_coe a.coe_prop]; rw [ mem_map]; rw [ preimage_image_eq _ subtype.coe_injective]; rw [ subtype.coe_eta]
...
by rw [continuous_at]; rw [ continuous_within_at]; rw [ nhds_within_univ]
...
begin
 ...
 rw [nhds_within_prod_eq]; rw [ prod.map]; rw [ nhds_prod_eq],
...
begin
 rw [continuous_on]; rw [ continuous_iff_continuous_at], split,
 { rintros h ⟨x, xs⟩,
...
have ∀ t, is_open (s.restrict f ⁻¹' t) ↔ ∃ (u : set α), is_open u ∧ f ⁻¹' t ∩ s = u ∩ s,
 begin
 intro t,
 rw [is_open_induced_iff]; rw [ set.restrict_eq]; rw [ set.preimage_comp],
 simp only [subtype.preimage_coe_eq_preimage_coe_iff],
 split; { rintros ⟨u, ou, useq⟩, exact ⟨u, ou, useq.symm⟩ }
 end,
by rw [continuous_on_iff_continuous_restrict]; rw [ continuous_def]; simp only [this]
...
have ∀ t, is_closed (s.restrict f ⁻¹' t) ↔ ∃ (u : set α), is_closed u ∧ f ⁻¹' t ∩ s = u ∩ s,
 begin
 intro t,
 rw [is_closed_induced_iff]; rw [ set.restrict_eq]; rw [ set.preimage_comp],
 simp only [subtype.preimage_coe_eq_preimage_coe_iff, eq_comm]
 end,
by rw [continuous_on_iff_continuous_restrict]; rw [ continuous_iff_is_closed]; simp only [this]
...
continuous_iff_continuous_at.mpr $ λ x, let ⟨i, hi⟩ := hs x in
 by { rw [continuous_at]; rw [ ← nhds_within_eq_nhds.2 hi], exact hf _ _ (mem_of_mem_nhds hi) }
...
by rw [compl_eq_univ_diff]; rw [ continuous_within_at_diff_self]; rw [ continuous_within_at_univ]
...
calc continuous_within_at (update f x y) s x ↔ tendsto (update f x y) (𝓝[s \ {x}] x) (𝓝 y) :
 by rw [← continuous_within_at_diff_self]; rw [ continuous_within_at]; rw [ function.update_same]
...
by rw [← continuous_within_at_univ]; rw [ continuous_within_at_update_same]; rw [ compl_eq_univ_diff]
...
begin
 ...
 { rw [inter_eq_self_of_subset_left (image_subset f (inter_subset_right t s))]; rw [ hleft.image_inter'] },
end
...
by rw [← univ_inter s]; rw [ continuous_within_at_inter h]; rw [ continuous_within_at_univ]
...
by rw [continuous_within_at]; rw [ hx]; rw [ tendsto_congr' h]; rw [ continuous_within_at]
...
... = s ∩ interior (f ⁻¹' t) : by rw [interior_inter]; rw [ hs.interior_eq]
...
begin
 ...
 have := ct x ⟨xs, xt⟩,
 rwa [continuous_within_at] at this; rwa [ ← nhds_within_restrict _ xt open_t] at this
end
...
begin
 ...
 by rw [preimage_inter]; rw [ inter_assoc]; rw [ inter_left_comm _ s]; rw [ ← inter_assoc s s]; rw [ inter_self],
 rw this,
 exact hu.inter hv },
 { rw [preimage_sUnion]; rw [ inter_Union₂],
...
by rw [nhds_within]; rw [ map_inf hf.inj]; rw [ hf.map_nhds_eq]; rw [ map_principal]; rw [ ← nhds_within_inter']; rw [ inter_eq_self_of_subset_right (image_subset_range _ _)]

lemma open_embedding.map_nhds_within_preimage_eq {f : α → β} (hf : open_embedding f)
 (s : set β) (x : α) :
 map f (𝓝[f ⁻¹' s] x) = 𝓝[s] (f x) :=
begin
 rw [hf.to_embedding.map_nhds_within_eq]; rw [ image_preimage_eq_inter_range],
...
begin
 intros hx,
 rw [mem_closure_iff_nhds_within_ne_bot] at hx; rw [ ne_bot_iff] at hx; rw [ not_not] at hx,
 rw [continuous_within_at]; rw [ hx],
 exact tendsto_bot,
end
...
begin
 ...
 { rw [← inter_univ s] at hx ⊢; rw [ ← union_compl_self t] at hx ⊢; rw [ inter_union_distrib_left] at hx ⊢,
 cases hx,
...
by rw [closure_eq_self_union_frontier]; rw [ inter_union_distrib_left]; rw [ inter_union_distrib_left]; rw [ ite_inter_self]; rw [ ite_inter_of_inter_eq _ ht]

lemma ite_inter_closure_compl_eq_of_inter_frontier_eq {s s' t : set α}
 (ht : s ∩ frontier t = s' ∩ frontier t) :
 t.ite s s' ∩ closure tᶜ = s' ∩ closure tᶜ :=
by { rw [← ite_compl, ite_inter_closure_eq_of_inter_frontier_eq], rwa [frontier_compl]; rwa [ eq_comm] }
