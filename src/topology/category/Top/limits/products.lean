...
by { rw [← sigma_iso_sigma_hom_ι_apply]; rw [ ← comp_app], simp, }
...
begin
 ...
 split,
 { rintro ⟨h⟩,
 rw [← show _ = c.inl]; rw [ from h.comp_cocone_point_unique_up_to_iso_inv (binary_cofan_is_colimit X Y) ⟨walking_pair.left⟩]; rw [ ← show _ = c.inr]; rw [ from h.comp_cocone_point_unique_up_to_iso_inv (binary_cofan_is_colimit X Y) ⟨walking_pair.right⟩],
 dsimp,
 refine
 ...
 erw [set.range_comp]; erw [ ← eq_compl_iff_is_compl]; erw [ set.range_comp _ sum.inr]; erw [ ← set.image_compl_eq (homeo_of_iso $ h.cocone_point_unique_up_to_iso (binary_cofan_is_colimit X Y)) .symm.bijective],
 congr' 1,
