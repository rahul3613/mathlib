...
⟨λ h, begin
 ...
 { rw [sUnion_eq_empty]; rw [ or_iff_not_imp_left],
...
lemma mem_nhds_discrete [topological_space α] [discrete_topology α] {x : α} {s : set α} :
 s ∈ 𝓝 x ↔ x ∈ s :=
by rw [nhds_discrete]; rw [ mem_pure]

lemma le_of_nhds_le_nhds {t₁ t₂ : topological_space α} (h : ∀x, @nhds α t₁ x ≤ @nhds α t₂ x) :
 t₁ ≤ t₂ :=
begin
 intro s,
 rw [@is_open_iff_mem_nhds _ t₁]; rw [ @is_open_iff_mem_nhds α t₂],
...
begin
 ...
 split,
 { rintros ⟨V, hV, rfl⟩,
 rwa [is_open_coinduced]; rwa [ e.preimage_symm_preimage] },
...
by rw [← e.symm.induced_symm]; rw [ e.symm_symm]
...
begin
 ...
 rw [le_nhds_adjoint_iff']; rw [ and.congr_right_iff],
...
by { ext s, rw [mem_nhds_induced]; rw [ mem_comap] }

lemma induced_iff_nhds_eq [tα : topological_space α] [tβ : topological_space β] (f : β → α) :
tβ = tα.induced f ↔ ∀ b, 𝓝 b = comap f (𝓝 $ f b) :=
⟨λ h a, h.symm ▸ nhds_induced f a, λ h, eq_of_nhds_eq_nhds $ λ x, by rw [h]; rw [ nhds_induced]⟩
...
by rw [nhds_induced]; rw [ map_comap_of_surjective hf]
...
by rw [nhds_induced]; rw [ filter.map_comap]; rw [ nhds_within]

lemma map_nhds_induced_of_mem {a : α} (h : range f ∈ 𝓝 (f a)) :
 map f (@nhds α (induced f t) a) = 𝓝 (f a) :=
by rw [nhds_induced]; rw [ filter.map_comap_of_mem h]
