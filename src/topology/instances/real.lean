...
⟨_, hε (show abs _ < _,
 by rwa [abs_of_nonneg (le_of_lt $ sub_pos.2 h₁)]; rwa [ sub_lt_iff_lt_add']),
...
begin
 ...
 intros K hK,
 rw [filter.mem_map]; rw [ mem_cofinite],
 apply set.to_finite },
 intros K hK,
 have H := int.tendsto_zmultiples_hom_cofinite ha hK,
 simp only [filter.mem_map, mem_cofinite, ← preimage_compl] at ⊢ H,
 rw [← (zmultiples_hom ℝ a).range_restrict_surjective.image_preimage ((zmultiples a).subtype ⁻¹' Kᶜ)]; rw [ ← preimage_comp]; rw [ ← add_monoid_hom.coe_comp_range_restrict],
...
begin
 ...
 { right,
 use 0,
 rw [H]; rw [ add_subgroup.closure_singleton_zero] },
 { let G_pos := {g : ℝ | g ∈ G ∧ 0 < g},
