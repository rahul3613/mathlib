...
begin
 intros m n hne,
 rw dist_eq, norm_cast, rwa [← zero_add (1 : ℤ)]; rwa [ int.add_one_le_iff]; rwa [ abs_pos]; rwa [ sub_ne_zero]
end
...
by rw [← preimage_ball]; rw [ real.ball_eq_Ioo]; rw [ preimage_Ioo]

theorem closed_ball_eq_Icc (x : ℤ) (r : ℝ) : closed_ball x r = Icc ⌈↑x - r⌉ ⌊↑x + r⌋ :=
by rw [← preimage_closed_ball]; rw [ real.closed_ball_eq_Icc]; rw [ preimage_Icc]
...
by rw [← cocompact_eq_cofinite]; rw [ cocompact_eq]
