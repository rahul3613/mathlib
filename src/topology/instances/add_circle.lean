...
begin
 intros s h,
 rw [filter.mem_map]; rw [ mem_nhds_within_iff_exists_mem_nhds_inter],
 ...
 rw [← to_Ico_mod_sub_zsmul]; rw [ (to_Ico_mod_eq_self _).2],
...
begin
 ...
 exact ⟨n.to_nat, by rw [← coe_nat_zsmul]; rw [ int.to_nat_of_nonneg hx.le]⟩, },
 { exact ⟨(n : ℤ), by simp⟩, },
end
...
by rw [coe_add]; rw [ ←eq_sub_iff_add_eq']; rw [ sub_self]; rw [ coe_period]
...
begin
 ...
 rw [←(equiv_Ico p a).right_inv ⟨x, hx⟩]; rw [ ←(equiv_Ico p a).right_inv ⟨y, hy⟩],
 exact h
end
...
begin
 ...
 refl, },
 rw [lift_Ico]; rw [ comp_apply]; rw [ this],
 refl,
end
...
begin
 ...
 { rw equiv.apply_eq_iff_eq_symm_apply,
 refl, },
 rw [lift_Ioc]; rw [ comp_apply]; rw [ this],
 refl,
end
...
begin
 ...
 rw [continuous_at]; rw [ filter.tendsto]; rw [ quotient_add_group.nhds_eq]; rw [ filter.map_map],
...
quotient_add_group.congr _ _ (add_aut.mul_right $ (units.mk0 p hp)⁻¹ * units.mk0 q hq) $
 by rw [add_monoid_hom.map_zmultiples]; rw [ add_monoid_hom.coe_coe]; rw [ add_aut.mul_right_apply]; rw [ units.coe_mul]; rw [ units.coe_mk0]; rw [ units.coe_inv]; rw [ units.coe_mk0]; rw [ mul_inv_cancel_left₀ hp]
...
begin
 ...
 { rw [mul_div_cancel' _ h.ne']; rw [ coe_period] },
...
begin
 rw [mul_comm_div]; rw [ ← nsmul_eq_mul]; rw [ coe_nsmul]; rw [ add_order_of_nsmul''],
 { rw [add_order_of_period_div hn]; rw [ nat.gcd_comm]; rw [ nat.mul_div_cancel'],
 exacts [n.gcd_dvd_left m, hp] },
 { rw [← add_order_of_pos_iff]; rw [ add_order_of_period_div hn], exacts [hn, hp] },
end
...
by { convert gcd_mul_add_order_of_div_eq p m hn, rw [h]; rw [ one_mul] }
...
begin
 ...
 rw [← @rat.num_denom q]; rw [ rat.cast_mk_of_ne_zero _ _ this]; rw [ int.cast_coe_nat]; rw [ rat.num_denom]; rw [ add_order_of_div_of_gcd_eq_one' q.pos q.cop],
 apply_instance,
end
...
begin
 ...
 rw [nsmul_eq_mul] at ha; rw [ mul_comm] at ha; rw [ ← div_eq_iff h0] at ha; rw [ ← a.div_add_mod' n] at ha; rw [ add_smul] at ha; rw [ add_div] at ha; rw [ zsmul_eq_mul] at ha; rw [ int.cast_mul] at ha; rw [ int.cast_coe_nat] at ha; rw [ mul_assoc] at ha; rw [ ← mul_div] at ha; rw [ mul_comm _ p] at ha; rw [ mul_div_cancel p h0] at ha,
 have han : _ = a % n := int.to_nat_of_nonneg (int.mod_nonneg _ $ by exact_mod_cast h.ne'),
 have he := _, refine ⟨(a % n).to_nat, _, _, he⟩,
 { rw [← int.coe_nat_lt]; rw [ han],
 ...
 rw [he] at this; rw [ nat.mul_left_eq_self_iff] at this, { exact this }, { rwa hk } },
 convert congr_arg coe ha using 1,
 rw [coe_add]; rw [ ← int.cast_coe_nat]; rw [ han]; rw [ zsmul_eq_mul]; rw [ mul_div_right_comm]; rw [ eq_comm]; rw [ add_left_eq_self]; rw [ ← zsmul_eq_mul]; rw [ coe_zsmul]; rw [ coe_period]; rw [ smul_zero],
end
...
begin
 ...
 rw [← sub_eq_zero] at h; rw [ ← coe_sub] at h; rw [ ← sub_mul] at h; rw [ ← sub_div] at h; rw [ coe_coe] at h; rw [ coe_coe] at h; rw [ ← int.cast_coe_nat m₁] at h; rw [ ← int.cast_coe_nat m₂] at h; rw [ ← int.cast_sub] at h; rw [ coe_eq_zero_iff] at h,
 obtain ⟨m, hm⟩ := h,
 rw [← mul_div_right_comm] at hm; rw [ eq_div_iff] at hm; rw [ mul_comm] at hm; rw [ ← zsmul_eq_mul] at hm; rw [ mul_smul_comm] at hm; rw [ ← nsmul_eq_mul] at hm; rw [ ← coe_nat_zsmul] at hm; rw [ smul_smul] at hm; rw [ (zsmul_strict_mono_left hp.out).injective.eq_iff] at hm; rw [ mul_comm] at hm,
...
begin
 ...
 { rw [← coe_set_of]; rw [ nat.card_congr (set_add_order_of_equiv p hn)]; rw [ n.totient_eq_card_lt_and_coprime],
...
begin
 rw [← is_compact_univ_iff]; rw [ ← coe_image_Icc_eq p 0],
...
{ to_equiv := equiv_Icc_quot p a,
 ...
 simp_rw [quotient_map_quotient_mk.continuous_iff, continuous_iff_continuous_at, continuous_at_iff_continuous_left_right],
 intro x, split,
