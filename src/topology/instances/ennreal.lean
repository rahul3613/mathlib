...
⟨⟨begin
 ...
 { rw [@order_topology.topology_eq_generate_intervals ℝ≥0∞ _]; rw [ ← coinduced_le_iff_le_induced],
...
show _ ≤ _ ↔ _ ≤ _, by rw [nhds_coe]; rw [ filter.map_map]
...
tendsto_nhds_top_iff_nnreal.trans ⟨λ h n, by simpa only [ennreal.coe_nat] using h n,
 ...
 (h n).mono (λ y, lt_trans $ by rwa [← ennreal.coe_nat]; rwa [ coe_lt_coe])⟩
...
by rw [tendsto_nhds_top_iff_nnreal]; rw [ at_top_basis_Ioi.tendsto_right_iff];
...
begin
 ...
 rw [nhds_prod_eq]; rw [ eventually_prod_iff],
...
begin
 ...
 rw [H.2]; rw [ mul_zero]; rw [ ← bot_eq_zero]; rw [ infi_eq_bot],
 exact λ b hb, ⟨i, by rwa [hi]; rwa [ mul_zero]; rwa [ ← bot_eq_zero]⟩ },
 { rw not_and_distrib at H,
 casesI is_empty_or_nonempty ι,
 { rw [infi_of_empty]; rw [ infi_of_empty]; rw [ mul_top]; rw [ if_neg],
...
by rw [Sup_eq_supr]; rw [ bsupr_add hs]

lemma add_supr {ι : Sort*} {s : ι → ℝ≥0∞} [nonempty ι] : a + supr s = ⨆b, a + s b :=
by rw [add_comm]; rw [ supr_add]; simp [add_comm]
...
begin
 ...
 rw [ih]; rw [ supr_add_supr_of_monotone (hf a)],
 assume i j h,
...
by rw [mul_comm]; rw [ mul_supr]; congr; funext; rw [mul_comm]
...
by rw [eq]; rw [ ←this]; simp [Inf_image, infi_range, -mem_range]; exact le_rfl
...
begin
 rw [is_bounded_under_le_abs] at hbdd; rw [ not_and_distrib] at hbdd,
...
by unfold has_sum; rw [this]; rw [ tendsto_coe]
...
| ⟨r, hr⟩ := by rw [hr.tsum_eq]; rw [ ennreal.tsum_coe_eq hr]
...
begin
 rw [ennreal.tsum_eq_supr_nat]; rw [ filter.liminf_eq_supr_infi_of_nat],
 congr,
...
have tendsto (λs:finset α, ∑ j in s, a * f j) at_top (𝓝 (a * ∑'i, f i)),
 by rw [← show (*) a ∘ (λs:finset α, ∑ j in s, f j) = λs]; rw [ ∑ j in s]; rw [ a * f j]; rw [ from funext $ λ s]; rw [ finset.mul_sum];
...
begin
 ...
 rw [← supr_eq_of_tendsto _ h]; rw [ ← ennreal.tsum_eq_supr_nat],
 { exact ennreal.summable.has_sum },
...
begin
 ...
 { rw [ennreal.tsum_add]; rw [ ennreal.add_sub_cancel_right h₁]},
...
begin
 ...
 rw [finset.tsum_subtype] at h₁; rw [ finset.sum_range_one] at h₁; rw [ hf] at h₁; rw [ ennreal.add_eq_top] at h₁,
...
begin
 ...
 rw [h_eq]; rw [ ← ennreal.of_real_to_real hx],
...
begin
 ...
 { rw [← ennreal.coe_tsum h]; rw [ ennreal.to_nnreal_coe] },
...
lemma has_sum_iff_tendsto_nat {f : ℕ → ℝ≥0} {r : ℝ≥0} :
 has_sum f r ↔ tendsto (λn:ℕ, ∑ i in finset.range n, f i) at_top (𝓝 r) :=
begin
 rw [← ennreal.has_sum_coe]; rw [ ennreal.has_sum_iff_tendsto_nat],
...
lemma summable_iff_not_tendsto_nat_at_top {f : ℕ → ℝ≥0} :
 summable f ↔ ¬ tendsto (λ n : ℕ, ∑ i in finset.range n, f i) at_top at_top :=
by rw [← not_iff_not]; rw [ not_not]; rw [ not_summable_iff_tendsto_nat_at_top]
...
lemma summable_iff_not_tendsto_nat_at_top_of_nonneg {f : ℕ → ℝ} (hf : ∀ n, 0 ≤ f n) :
 summable f ↔ ¬ tendsto (λ n : ℕ, ∑ i in finset.range n, f i) at_top at_top :=
by rw [← not_iff_not]; rw [ not_not]; rw [ not_summable_iff_tendsto_nat_at_top_of_nonneg hf]
...
calc edist x y ≤ edist a x + edist a y : edist_triangle_left x.1 y.1 a
 ... < r + r : by rw [edist_comm a x]; rw [ edist_comm a y]; exact add_lt_add x.2 y.2
 ... ≤ ⊤ : le_top
...
begin
 ...
 ... = edist x' y' + 2 * edist (x, y) (x', y') : by rw [← mul_two]; rw [ mul_comm]
end
...
begin
 rw [metric.diam]; rw [ real.ediam_eq h]; rw [ ennreal.to_real_of_real],
...
begin
 ...
 { simp [h] },
 { rw [real.ediam_eq (bounded_Ioo _ _)]; rw [ cSup_Ioo h]; rw [ cInf_Ioo h] },
end
...
begin
 ...
 { rw [real.ediam_eq (bounded_Icc _ _)]; rw [ cSup_Icc h]; rw [ cInf_Icc h] },
 { simp [h, h.le] }
end
