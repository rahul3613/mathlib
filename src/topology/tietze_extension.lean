...
begin
 ...
 rwa [Iic_disjoint_Ici]; rwa [ not_le] },
 ...
 by rw [hg₁ (mem_image_of_mem _ hle₁)]; rw [ abs_of_nonneg (sub_nonneg.2 hle₁)]
 ... ≤ (2 / 3) * ‖f‖ : by linarith },
 { cases le_total (f x) (‖f‖ / 3) with hle₂ hle₂,
 ...
 by rw [hg₂ (mem_image_of_mem _ hle₂)]; rw [ abs_sub_comm]; rw [ abs_of_nonneg (sub_nonneg.2 hle₂)]
 ... ≤ (2 / 3) * ‖f‖ : by linarith } } }
end
...
begin
 ...
 { simp [g0] },
 { rw [g_succ n]; rw [ add_comp_continuous]; rw [ ← dist_sub_right]; rw [ add_sub_cancel']; rw [ pow_succ]; rw [ mul_assoc],
 refine (hF_dist _).trans (mul_le_mul_of_nonneg_left _ (by norm_num1)),
 rwa ← dist_eq_norm' } },
 have hg_dist : ∀ n, dist (g n) (g (n + 1)) ≤ 1 / 3 * ‖f‖ * (2 / 3) ^ n,
 { intro n,
 calc dist (g n) (g (n + 1)) = ‖F (f - (g n).comp_continuous e)‖ :
 by rw [g_succ]; rw [ dist_eq_norm']; rw [ add_sub_cancel']
 ... ≤ ‖f - (g n).comp_continuous e‖ / 3 : hF_norm _
 ... = (1 / 3) * dist ((g n).comp_continuous e) f :
 by rw [dist_eq_norm']; rw [ one_div]; rw [ div_eq_inv_mul]
 ... ≤ (1 / 3) * ((2 / 3) ^ n * ‖f‖) :
...
begin
 ...
 ... = b : by rw [hsub]; rw [ add_sub_cancel'_right] } } },
 ...
 calc f x = c - (b - c) : by rw [← hsub]; rw [ sub_sub_cancel]
 ... ≤ g y - dg y : sub_le_sub hc.le (dgmem _).2 },
 { have hay : a < (g - dg) y,
 { calc a = c - (b - c) : by rw [← hsub]; rw [ sub_sub_cancel]
