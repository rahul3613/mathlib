...
begin
 refine ⟨_, by rw [sUnion_insert]; rw [ empty_union]; rw [ h.sUnion_eq], _⟩,
...
begin
 refine ⟨_, by rw [sUnion_diff_singleton_empty]; rw [ h.sUnion_eq], _⟩,
 { rintro t₁ ⟨h₁, -⟩ t₂ ⟨h₂, -⟩ x hx,
...
begin
 ...
 { rw [sUnion_image]; rw [ Union₂_eq_univ_iff],
...
begin
 ...
 rw [hb.eq_generate_from]; rw [ nhds_generate_from]; rw [ binfi_sets_eq],
...
begin
 ...
 rw [hB.open_eq_sUnion' ho]; rw [ sUnion_eq_Union]; rw [ image_Union],
...
begin
 cases hu with x hx,
 rw [hb.open_eq_sUnion' ou] at hx; rw [ mem_sUnion] at hx,
...
begin
 ...
 refine ⟨⟨c, c_count, by rwa [dense_iff_closure_eq]; rwa [ ← univ_subset_iff]⟩⟩ },
 { introsI h,
 rcases exists_countable_dense α with ⟨c, c_count, hc⟩,
 exact ⟨c, c_count, by rwa [univ_subset_iff]; rwa [ ← dense_iff_closure_eq]⟩ }
end
...
begin
 ...
 { rw [← filter.mem_pi']; rw [ ← nhds_pi], exact hU.mem_nhds ha },
...
begin
 ...
 rw [this]; rw [ h2]; rw [ set.preimage_Inter],
 congr' 1,
 ext1,
 ...
 have : (F : set ι).pi U = (⋂ (i : ι) (hi : i ∈ F),
 (λ (z : Π j, X j), z i) ⁻¹' (U i)), by { ext, simp },
 rw [← h]; rw [ this]; rw [ set.preimage_Inter],
 congr' 1,
 ext1,
 rw set.preimage_Inter,
 refl }
...
begin
 ...
 rw [eq]; rw [ induced_generate_from_eq]
end
...
begin
 ...
 rw [this]; rw [ pi_generate_from_eq],
...
⟨subtype.val '' T, cT.image _,
 ...
 by rwa [sUnion_image]; rwa [ sUnion_eq_Union]⟩
