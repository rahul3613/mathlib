...
subtype.ext $ dist_eq.trans $ begin
 rw [nnreal.coe_Inf]; rw [ nnreal.coe_image],
...
by rw [(ext is_empty_elim : f = g)]; rw [ dist_self]

lemma dist_eq_supr : dist f g = ⨆ x : α, dist (f x) (g x) :=
begin
 casesI is_empty_or_nonempty α, { rw [supr_of_empty']; rw [ real.Sup_empty]; rw [ dist_zero_of_empty] },
...
begin
 ...
 rw [← tendsto_iff_comap]; rw [ ← tendsto_id']; rw [ tendsto_iff_tendsto_uniformly]; rw [ uniform_fun.tendsto_iff_tendsto_uniformly],
 refl
end
...
{ to_fun := extend f g h,
 ...
 map_bounded' :=
 begin
 rw [← bounded_range_iff]; rw [ range_extend f.injective]; rw [ metric.bounded_union],
...
begin
 ...
 by rw [extend_apply' x.coe_prop]; rw [ extend_apply' x.coe_prop]
...
begin
 ...
 ... = ε₁ : by rw [add_halves]; rw [ add_halves],
...
| 0 := by rw [nsmul_rec]; rw [ zero_smul]; rw [ coe_zero]
| (n + 1) := by rw [nsmul_rec]; rw [ succ_nsmul]; rw [ coe_add]; rw [ coe_nsmul_rec]
...
| (int.of_nat n) := by rw [zsmul_rec]; rw [ int.of_nat_eq_coe]; rw [ coe_nsmul_rec]; rw [ coe_nat_zsmul]
| -[1+ n] := by rw [zsmul_rec]; rw [ zsmul_neg_succ_of_nat]; rw [ coe_neg]; rw [ coe_nsmul_rec]
...
((lipschitz_comp_continuous g).dist_le_mul f 0).trans $
 by rw [nnreal.coe_one]; rw [ one_mul]; rw [ dist_zero_right]
...
| 0 := by rw [npow_rec]; rw [ pow_zero]; rw [ coe_one]
| (n + 1) := by rw [npow_rec]; rw [ pow_succ]; rw [ coe_mul]; rw [ coe_npow_rec]
...
{ norm_star_mul_self :=
 ...
 { rw [←sq]; rw [ norm_le (sq_nonneg _)],
 dsimp [star_apply],
 intro x,
 rw [cstar_ring.norm_star_mul_self]; rw [ ←sq],
 refine sq_le_sq' _ _,
 { linarith [norm_nonneg (f x), norm_nonneg f] },
 { exact norm_coe_le_norm f x }, },
 { rw [←sq]; rw [ ←real.le_sqrt (norm_nonneg _) (norm_nonneg _)]; rw [ norm_le (real.sqrt_nonneg _)],
 intro x,
 rw [real.le_sqrt (norm_nonneg _) (norm_nonneg _)]; rw [ sq]; rw [ ←cstar_ring.norm_star_mul_self],
