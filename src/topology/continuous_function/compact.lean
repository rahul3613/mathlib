...
{ dist_eq := λ x y, by
 rw [← norm_mk_of_compact]; rw [ ← dist_mk_of_compact]; rw [ dist_eq_norm]; rw [ mk_of_compact_sub],
...
{ norm_star := λ f, by rw [←bounded_continuous_function.norm_mk_of_compact]; rw [ bounded_continuous_function.mk_of_compact_star]; rw [ norm_star]; rw [ bounded_continuous_function.norm_mk_of_compact] }
...
{ norm_star_mul_self :=
 ...
 { rw [←sq]; rw [ continuous_map.norm_le _ (sq_nonneg _)],
 intro x,
 ...
 { rw [←sq]; rw [ ←real.le_sqrt (norm_nonneg _) (norm_nonneg _)]; rw [ continuous_map.norm_le _ (real.sqrt_nonneg _)],
 intro x,
