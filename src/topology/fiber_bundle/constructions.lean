...
{ to_fun := λ x, (x.proj, x.snd),
 ...
 continuous_to_fun := by { rw [←continuous_iff_continuous_on_univ]; rw [ continuous_iff_le_induced],
 simp only [prod.topological_space, induced_inf, induced_compose], exact le_rfl, },
 continuous_inv_fun := by { rw [←continuous_iff_continuous_on_univ]; rw [ continuous_iff_le_induced],
...
begin
 rw [continuous_iff_le_induced]; rw [ pullback.total_space.topological_space]; rw [ pullback_topology],
 exact inf_le_left,
end
...
begin
 constructor,
 simp_rw [prod.topological_space, induced_inf, induced_compose, pullback.total_space.topological_space, pullback_topology],
 refl
end
...
{ to_fun := λ z, (z.proj, (e (pullback.lift f z)).2),
 ...
 map_target' := λ y h, by { rw [mem_prod] at h; rw [ mem_preimage] at h,
 ...
 dsimp only,
 simp_rw [(inducing_pullback_total_space_embedding F E f).continuous_on_iff, function.comp, pullback_total_space_embedding],
