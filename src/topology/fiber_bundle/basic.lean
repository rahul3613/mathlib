...
begin
 ...
 { rwa [source_eq]; rwa [ preimage_preimage] }
end
...
{ source := (Z.base_set i ∩ Z.base_set j) ×ˢ univ,
 ...
 rw [Z.coord_change_comp]; rw [ Z.coord_change_self],
 { exact hx.1 },
 { simp [hx] }
 end,
 right_inv' := begin
...
by { rw [local_triv_at]; rw [ local_triv_apply]; rw [ coord_change_self], exact Z.mem_base_set_at p.1 }
...
by { rw [local_triv_at]; rw [ ←base_set_at], exact Z.mem_base_set_at b, }
...
begin
 rw [continuous_iff_le_induced]; rw [ fiber_bundle_core.to_topological_space],
 ...
 rw [←((Z.local_triv i).source_inter_preimage_target_inter t)]; rw [ preimage_inter]; rw [ ←preimage_comp]; rw [ trivialization.source_eq],
 apply is_open.inter,
 ...
 rw [preimage_inter]; rw [ preimage_comp],
 ...
 { rw [(Z.local_triv i).target_eq]; rw [ ←base_set_at]; rw [ mk_preimage_prod_right_eq_empty h]; rw [ preimage_empty]; rw [ empty_inter],
 exact is_open_empty, }}
end
...
{ total_space_mk_inducing := λ b, ⟨ begin refine le_antisymm _ (λ s h, _),
 ...
 rw [preimage_inter]; rw [ ←preimage_comp]; rw [ function.comp],
...
begin
 ...
 rw [pretrivialization.set_symm]; rw [ restrict]; rw [ e.target_eq]; rw [ e.source_eq]; rw [ preimage_comp]; rw [ subtype.preimage_coe_eq_preimage_coe_iff]; rw [ e'.target_eq]; rw [ prod_inter_prod]; rw [ inter_univ]; rw [ pretrivialization.preimage_symm_proj_inter],
end
...
{ open_source := a.is_open_source e,
 ...
 by { rw [inter_assoc]; rw [ inter_self], refl }⟩),
 refine (is_open_supr_iff.mpr (λ he', _)),
 rw [is_open_coinduced]; rw [ is_open_induced_iff],
...
begin
 ...
 rw [(a.pretrivialization_at b).source_eq]; rw [ ←preimage_comp]; rw [ function.comp],
