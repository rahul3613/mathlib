...
lemma mem_source : x ∈ e.source ↔ proj x ∈ e.base_set := by rw [e.source_eq]; rw [ mem_preimage]
...
by rw [e.target_eq]; rw [ prod_univ]; rw [ mem_preimage]
...
begin
 ...
 rwa [← e.coe_coe] at this; rwa [ e.to_local_equiv.right_inv hx] at this
end
...
by rw [← e.coe_fst ex]; rw [ prod.mk.eta]; rw [ ← e.coe_coe]; rw [ e.to_local_equiv.left_inv ex]
...
by rw [inter_comm]; rw [ f.target_eq]; rw [ e.source_eq]; rw [ f.preimage_symm_proj_inter]
...
by rw [local_equiv.trans_source]; rw [ local_equiv.symm_source]; rw [ e.target_inter_preimage_symm_source_eq]
...
by rw [local_equiv.trans_symm_eq_symm_trans_symm]; rw [ local_equiv.symm_symm]
...
by rw [local_equiv.trans_source]; rw [ e'.source_eq]; rw [ local_equiv.symm_source]; rw [ e.target_eq]; rw [ inter_comm]; rw [ e.preimage_symm_proj_inter]; rw [ inter_comm]
...
by rw [← local_equiv.symm_source]; rw [ symm_trans_symm]; rw [ symm_trans_source_eq]; rw [ inter_comm]
...
by rw [e.symm_apply hb]; rw [ total_space.mk_cast]; rw [ total_space.eta]

lemma symm_proj_apply (e : pretrivialization F (π F E)) (z : total_space F E)
 (hz : z.proj ∈ e.base_set) : e.symm z.proj (e z).2 = z.2 :=
by rw [e.symm_apply hz]; rw [ cast_eq_iff_heq]; rw [ e.mk_proj_snd' hz]; rw [ e.symm_apply_apply (e.mem_source.mpr hz)]
...
lemma apply_mk_symm (e : pretrivialization F (π F E)) {b : B} (hb : b ∈ e.base_set) (y : F) :
 e ⟨b, e.symm b y⟩ = (b, y) :=
by rw [e.mk_symm hb]; rw [ e.apply_symm_apply (e.mk_mem_target.mpr hb)]
...
by { intros e e', rw [pretrivialization.ext_iff]; rw [ trivialization.ext_iff]; rw [ ← local_homeomorph.to_local_equiv_injective.eq_iff], exact id }
...
by rw [← e.coe_fst ex]; rw [ ← map_congr (e.coe_fst_eventually_eq_proj ex)]; rw [ ← map_map]; rw [ ← e.coe_coe]; rw [ e.to_local_homeomorph.map_nhds_eq ex]; rw [ map_fst_nhds]
...
@[simp] lemma preimage_singleton_homeomorph_symm_apply {b : B} (hb : b ∈ e.base_set) (p : F) :
 ...
 ⟨e.symm (b, p), by rw [mem_preimage]; rw [ e.proj_symm_apply' hb]; rw [ mem_singleton_iff]⟩ :=
rfl
...
begin
 ...
 { rw [local_equiv.symm_target]; rw [ e.mem_source],
 exact he },
 rwa [e.to_local_homeomorph.symm.continuous_at_iff_continuous_at_comp_right hez]; rwa [ local_homeomorph.symm_symm]
end
...
by rw [coord_change]; rw [ e₁.symm_apply_mk_proj (e₁.mem_source.2 h)]

lemma coord_change_same_apply
 (e : trivialization F proj) {b : B} (h : b ∈ e.base_set) (x : F) :
 e.coord_change e b x = x :=
by rw [coord_change]; rw [ e.apply_symm_apply' h]
...
begin
 rw [coord_change]; rw [ e₁.mk_coord_change _ h₁ h₂]; rw [ ← e₂.coe_coe]; rw [ e₂.to_local_homeomorph.left_inv]; rw [ coord_change],
 rwa [e₂.mem_source]; rwa [ e₁.proj_symm_apply' h₁]
end
...
by rw [← (e.is_image_preimage_prod s).frontier.preimage_eq]; rw [ frontier_prod_univ_eq]; rw [ (e.is_image_preimage_prod _).preimage_eq]; rw [ e.source_eq]; rw [ preimage_inter]
...
{ to_local_homeomorph := e.to_local_homeomorph.piecewise e'.to_local_homeomorph
 ...
 (by rw [e.frontier_preimage]; rw [ e'.frontier_preimage]; rw [ Hs])
...
{ to_local_homeomorph := e.to_local_homeomorph.disjoint_union e'.to_local_homeomorph
 (by { rw [e.source_eq]; rw [ e'.source_eq], exact H.preimage _, })
 (by { rw [e.target_eq]; rw [ e'.target_eq]; rw [ disjoint_iff_inf_le],
