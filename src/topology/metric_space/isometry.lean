...
by rw [metric.diam]; rw [ metric.diam]; rw [ hf.ediam_image]
...
complete_space_of_is_complete_univ $ is_complete_of_complete_image e.isometry.uniform_inducing $
 by rwa [set.image_univ]; rwa [ isometry_equiv.range_eq_univ]; rwa [ ← complete_space_iff_is_complete_univ]
...
by rw [← image_symm]; rw [ diam_image]
