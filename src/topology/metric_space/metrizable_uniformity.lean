...
{ dist := λ x y, ↑(⨅ l : list X, ((x :: l).zip_with d (l ++ [y])).sum : ℝ≥0),
 ...
 rw [← sum_reverse]; rw [ zip_with_distrib_reverse]; rw [ reverse_append]; rw [ reverse_reverse]; rw [ reverse_singleton]; rw [ singleton_append]; rw [ reverse_cons]; rw [ reverse_reverse]; rw [ zip_with_comm_of_comm _ dist_comm],
 ...
 rw [← sum_append]; rw [ ← zip_with_append]; rw [ cons_append]; rw [ ← @singleton_append _ y]; rw [ append_assoc]; rw [ append_assoc]; rw [ append_assoc],
...
begin
 ...
 rw [dist_of_prenndist]; rw [ ← nnreal.coe_two]; rw [ ← nnreal.coe_mul]; rw [ nnreal.mul_infi]; rw [ nnreal.coe_le_coe],
 ...
 intro hLm,
 rw [mem_set_of_eq]; rw [ take_all_of_le hLm]; rw [ two_mul]; rw [ add_le_iff_nonpos_left]; rw [ nonpos_iff_eq_zero]; rw [ sum_eq_zero_iff]; rw [ ← all₂_iff_forall]; rw [ all₂_zip_with]; rw [ ← chain_append_singleton_iff_forall₂]
 at hm; [skip, by simp],
 ...
 have hM_lt : M < length L, by rwa [hL_len]; rwa [ nat.lt_succ_iff],
 ...
 convert hMs.1.out,
 rw [zip_with_distrib_take]; rw [ take]; rw [ take_succ]; rw [ nth_append hMl]; rw [ nth_le_nth hMl]; rw [ ← option.coe_def]; rw [ option.to_list_some]; rw [ take_append_of_le_length hMl.le],
 refl },
 ...
 rw [← sum_take_add_sum_drop L (M + 1)] at hMs'; rw [ two_mul] at hMs'; rw [ add_le_add_iff_left] at hMs'; rw [ ← add_le_add_iff_right] at hMs'; rw [ sum_take_add_sum_drop] at hMs'; rw [ ← two_mul] at hMs',
 convert hMs',
 rwa [zip_with_distrib_drop]; rwa [ drop]; rwa [ drop_append_of_le_length] }
end
...
begin
 ...
 { rw [(strict_anti_pow hr.1 hr.2).le_iff_le]; rw [ nat.find_le_iff],
 ...
 rw [← nnreal.div_le_iff' two_ne_zero]; rw [ ← mul_one_div (_ ^ _)]; rw [ ← pow_succ'],
 ...
 rwa [← nnreal.coe_pow]; rwa [ nnreal.coe_lt_coe]; rwa [ ← not_le]; rwa [ hle_d]; rwa [ not_not]; rwa [ prod.mk.eta] },
 { refine λ n hn, ⟨n + 1, trivial, λ x hx, _⟩,
 rw [mem_set_of_eq] at hx,
 contrapose! hx,
 refine le_trans _ ((div_le_iff' (zero_lt_two' ℝ)).2 (hd_le x.1 x.2)),
 rwa [← nnreal.coe_two]; rwa [ ← nnreal.coe_div]; rwa [ ← nnreal.coe_pow]; rwa [ nnreal.coe_le_coe]; rwa [ pow_succ']; rwa [ mul_one_div]; rwa [ nnreal.div_le_iff two_ne_zero]; rwa [ div_mul_cancel _ (two_ne_zero' ℝ≥0)]; rwa [ hle_d]; rwa [ prod.mk.eta] }
end
