...
by rw [GH_dist]; rw [ p.to_GH_space_rep]; rw [ q.to_GH_space_rep]
...
begin
 ...
 diam (univ : set Y) :
 by { rw [DΦ]; rw [ DΨ], apply add_le_add (add_le_add le_rfl (le_of_lt dy)) le_rfl }
...
begin
 ...
 { rw [← image_univ]; rw [ ← image_univ]; rw [ image_comp F]; rw [ image_univ]; rw [ image_comp F (optimal_GH_injr X Y)]; rw [ image_univ]; rw [ ← Hausdorff_dist_optimal],
...
{ dist := dist,
 ...
 rw [← dist_GH_dist] at DΦΨ; rw [ hxy] at DΦΨ,
 ...
 rw [← x.to_GH_space_rep]; rw [ ← y.to_GH_space_rep]; rw [ to_GH_space_eq_to_GH_space_iff_isometry_equiv],
 exact ⟨e⟩
 end,
 ...
 by rw [x.to_GH_space_rep]; rw [ z.to_GH_space_rep]
 ...
 by rw [Hausdorff_dist_image (to_glue_l_isometry hΦ hΨ)]; rw [ Hausdorff_dist_image (to_glue_r_isometry hΦ hΨ)]
 ... = dist (to_GH_space X) (to_GH_space Y) + dist (to_GH_space Y) (to_GH_space Z) :
 by rw [Hausdorff_dist_optimal]; rw [ Hausdorff_dist_optimal]; rw [ GH_dist]; rw [ GH_dist]
 ... = dist x y + dist y z:
 by rw [x.to_GH_space_rep]; rw [ y.to_GH_space_rep]; rw [ z.to_GH_space_rep]
 end }
...
begin
 ...
 { rw [← image_univ]; rw [ Hausdorff_dist_image Il],
 ...
 { rw [← @image_univ _ _ Fr]; rw [ Hausdorff_dist_image Ir],
...
begin
 ...
 have ihi_eq : (⟨i, hi⟩ : fin (N q)) = (E q) ⟨y, ys⟩, by rw [fin.ext_iff]; rw [ fin.coe_mk],
 have hiq : i < N q := hi,
 ...
 have : Φ z = y,
 { simp only [Φ, Ψ], rw [C1]; rw [ C2]; rw [ C3], refl },
 rw this,
 exact le_of_lt hy },
 ...
 by rw [mul_inv_cancel (ne_of_gt εpos)]; rw [ one_mul]
 ... = ε * (|ε⁻¹| * |dist x y - dist (Ψ x) (Ψ y)|) :
 by rw [abs_of_nonneg (le_of_lt (inv_pos.2 εpos))]; rw [ mul_assoc]
...
begin
 ...
 rw [hN] at scard; rw [ cardinal.nat_cast_le] at scard,
 have : cardinal.mk s = cardinal.mk (fin N), by rw [hN]; rw [ cardinal.mk_fin],
 ...
 have : Φ z = y :=
 by { simp only [Φ, Ψ], rw [C1]; rw [ C2]; rw [ C3], refl },
 rw this,
 ...
 floor_nonneg.2 (mul_nonneg (le_of_lt (inv_pos.2 εpos)) dist_nonneg),
 rw [← int.to_nat_of_nonneg D]; rw [ ← int.to_nat_of_nonneg D']; rw [ int.floor_to_nat]; rw [int.floor_to_nat]; rw [ this] },
 -- deduce that the distances coincide up to `ε`, by a straightforward computation
 -- that should be automated
 have I := calc
 |ε⁻¹| * |dist x y - dist (Ψ x) (Ψ y)| =
...
begin
 ...
 rw [X2n]; rw [ X2nsucc]; rw [ Hausdorff_dist_image]; rw [ Hausdorff_dist_optimal]; rw [ ← dist_GH_dist],
 ...
 { assume n,
 rw [nonempty_compacts.to_GH_space]; rw [ ← (u n).to_GH_space_rep]; rw [ to_GH_space_eq_to_GH_space_iff_isometry_equiv],
 constructor,
