...
by rw [← edist_mul_left a]; rw [ ← edist_mul_right _ _ b]; rw [ mul_right_inv]; rw [ one_mul]; rw [ inv_mul_cancel_right]; rw [ edist_comm]
...
by rw [← edist_inv_inv]; rw [ inv_inv]
...
by rw [div_eq_mul_inv]; rw [ div_eq_mul_inv]; rw [ edist_mul_left]; rw [ edist_inv_inv]
...
@[simp, to_additive] lemma preimage_smul_ball (c : G) (x : X) (r : ℝ≥0∞) :
 ((•) c) ⁻¹' ball x r = ball (c⁻¹ • x) r :=
by rw [preimage_smul]; rw [ smul_ball]
...
@[simp, to_additive] lemma preimage_smul_closed_ball (c : G) (x : X) (r : ℝ≥0∞) :
 ((•) c) ⁻¹' closed_ball x r = closed_ball (c⁻¹ • x) r :=
by rw [preimage_smul]; rw [ smul_closed_ball]
...
@[simp, to_additive] lemma preimage_smul_sphere (c : G) (x : X) (r : ℝ) :
 ((•) c) ⁻¹' sphere x r = sphere (c⁻¹ • x) r :=
by rw [preimage_smul]; rw [ smul_sphere]
