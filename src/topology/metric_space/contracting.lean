...
suffices edist x y ≤ edist x (f x) + edist y (f y) + K * edist x y,
 by rwa [ennreal.le_div_iff_mul_le (or.inl hf.one_sub_K_ne_zero) (or.inl one_sub_K_ne_top)]; rwa [ mul_comm]; rwa [ ennreal.sub_mul (λ _ _, h)]; rwa [ one_mul]; rwa [ tsub_le_iff_right],
calc edist x y ≤ edist x (f x) + edist (f x) (f y) + edist (f y) y : edist_triangle4 _ _ _ _
 ... = edist x (f x) + edist y (f y) + edist (f x) (f y) : by rw [edist_comm y]; rw [ add_right_comm]
...
begin
 ...
 { convert hle n,
 rw [maps_to.iterate_restrict]; rw [ eq_comm]; rw [ maps_to.coe_restrict_apply]; rw [ subtype.coe_mk] }
end
...
suffices dist x y ≤ dist x (f x) + dist y (f y) + K * dist x y,
 by rwa [le_div_iff hf.one_sub_K_pos]; rwa [ mul_comm]; rwa [ sub_mul]; rwa [ one_mul]; rwa [ sub_le_iff_le_add],
...
calc dist x y = dist y x : dist_comm x y
 ...
 ... = (dist (f y) (g y)) / (1 - K) : by rw [hy.eq]; rw [ dist_comm]
