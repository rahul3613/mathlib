...
by { rw [pos_iff_ne_zero]; rw [ ne.def]; rw [ einfsep_zero], simp only [not_forall, not_exists, not_lt] }
...
begin
 ...
 simp_rw [le_einfsep_iff, le_infi_iff, imp_forall_iff, set_coe.forall, subtype.coe_mk, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp]
end
...
begin
 refine eq_of_forall_le_iff (λ _, _),
 simp_rw [le_einfsep_iff, imp_forall_iff, finset.le_inf_iff, mem_to_finset, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp]
end
...
begin
 refine eq_of_forall_le_iff (λ _, _),
 simp_rw [le_einfsep_iff, imp_forall_iff, finset.le_inf_iff, finite.mem_to_finset, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp]
end
...
by simp_rw [einfsep_insert, infi_insert, infi_singleton, einfsep_singleton, inf_top_eq, cinfi_pos hxy, cinfi_pos hyz, cinfi_pos hxz]
...
by rw [infsep]; rw [ ennreal.to_real_eq_zero_iff]
...
by { rw [infsep_zero]; rw [ hs.einfsep], right, refl }
...
by simp_rw [infsep, ← ennreal.of_real_le_iff_le_to_real (hs.einfsep_ne_top), le_einfsep_iff, edist_dist, ennreal.of_real_le_of_real_iff (dist_nonneg)]
...
by { rw [infsep_pair_eq_to_real]; rw [ edist_dist], exact ennreal.to_real_of_real (dist_nonneg) }
...
begin
 ...
 exact dist_nonneg },
 refine eq_of_forall_le_iff (λ _, _),
 simp_rw [hs.le_infsep_iff, le_cinfi_set_iff (off_diag_nonempty.mpr hs) hb, imp_forall_iff, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp] },
...
by { classical, rw [infsep_eq_infi]; rw [ if_pos hs] }
...
begin
 split_ifs with hs,
 { refine eq_of_forall_le_iff (λ _, _),
 simp_rw [hs.le_infsep_iff, imp_forall_iff, finset.le_inf'_iff, mem_to_finset, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp] },
...
by { classical, rw [infsep_of_fintype]; rw [ dif_pos hs] }
...
begin
 split_ifs with hs,
 { refine eq_of_forall_le_iff (λ _, _),
 simp_rw [hs.le_infsep_iff, imp_forall_iff, finset.le_inf'_iff, finite.mem_to_finset, mem_off_diag, prod.forall, uncurry_apply_pair, and_imp] },
 { rw not_nontrivial_iff at hs, exact hs.infsep_zero }
end
...
begin
 ...
 by rwa [← set.off_diag_nonempty]; rwa [ ← finset.coe_off_diag]; rwa [ finset.coe_nonempty],
 split_ifs with hs,
...
by rw [finset.coe_infsep]; rw [ dif_pos hs]

lemma _root_.finset.coe_infsep_of_off_diag_empty [decidable_eq α] {s : finset α}
 (hs : s.off_diag = ∅) : (s : set α).infsep = 0 :=
by { rw ← finset.not_nonempty_iff_eq_empty at hs, rw [finset.coe_infsep]; rw [ dif_neg hs] }
