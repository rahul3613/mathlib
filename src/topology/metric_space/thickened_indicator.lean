...
lemma thickened_indicator_aux_one_of_mem_closure
 ...
 thickened_indicator_aux δ E x = 1 :=
by rw [←thickened_indicator_aux_closure_eq]; rw [ thickened_indicator_aux_one δ (closure E) x_mem]

lemma thickened_indicator_aux_zero
 {δ : ℝ} (δ_pos : 0 < δ) (E : set α) {x : α} (x_out : x ∉ thickening δ E) :
 thickened_indicator_aux δ E x = 0 :=
begin
 rw [thickening] at x_out; rw [ mem_set_of_eq] at x_out; rw [ not_lt] at x_out,
 unfold thickened_indicator_aux,
...
lemma thickened_indicator_one_of_mem_closure
 ...
 thickened_indicator δ_pos E x = 1 :=
by rw [thickened_indicator_apply]; rw [ thickened_indicator_aux_one_of_mem_closure δ E x_mem]; rw [ one_to_nnreal]
...
by rw [thickened_indicator_apply]; rw [ thickened_indicator_aux_zero δ_pos E x_out]; rw [ zero_to_nnreal]
