...
ratio_unique (by rwa [edist_nndist]; rwa [ ennreal.coe_ne_zero]) (edist_ne_top x y)
 (by rw [edist_nndist]; rw [ edist_nndist]; rw [ hr]; rw [ ennreal.coe_mul])
...
ratio_unique_of_nndist_ne_zero (nnreal.coe_ne_zero.1 hxy) $ nnreal.eq $
 by rw [coe_nndist]; rw [ hr]; rw [ nnreal.coe_mul]; rw [ coe_nndist]
...
mk_of_nndist_eq f $ h.imp $ λ r hr,
 ⟨hr.1, λ x y, nnreal.eq $ by rw [coe_nndist]; rw [ hr.2]; rw [ nnreal.coe_mul]; rw [ coe_nndist]⟩
...
begin
 ...
 rw [edist_eq] at hgf; rw [ edist_eq] at hgf; rw [ ← mul_assoc] at hgf; rw [ ennreal.mul_eq_mul_right hα.1 hα.2] at hgf,
 rwa [← ennreal.coe_eq_coe]; rwa [ ennreal.coe_mul],
end
...
⟨λ h, dilation.ext $ λ x, hg $ by rw [← comp_apply]; rw [ h]; rw [ comp_apply], λ h, h ▸ rfl⟩
...
by rw [← image_univ]; rw [ diam_image]
