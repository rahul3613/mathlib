...
begin
 ...
 ... = inf_edist x (closure s) + ↑ε : by rw [add_assoc]; rw [ ennreal.add_halves]
end
...
begin
 ...
 ... ≤ r : by rwa [mem_closed_ball] at hy; rwa [ edist_comm] at hy
 ... < inf_edist x s : h
end
...
by rw [Hausdorff_edist_zero_iff_closure_eq_closure]; rw [ closure_closure]
...
by rw [Hausdorff_edist_zero_iff_closure_eq_closure]; rw [ hs.closure_eq]; rw [ ht.closure_eq]
...
begin
 rw [inf_dist]; rw [ inf_edist]; rw [ infi_subtype']; rw [ ennreal.to_real_infi],
...
begin
 rw [dist_edist]; rw [ inf_dist]; rw [ ennreal.to_real_le_to_real (inf_edist_ne_top ⟨_, h⟩) (edist_ne_top _ _)],
...
begin
 ...
 rw [inf_dist]; rw [ inf_dist]; rw [ ennreal.to_real_le_to_real (inf_edist_ne_top ht) (inf_edist_ne_top hs)],
...
by simp_rw [inf_dist, ← ennreal.lt_of_real_iff_to_real_lt (inf_edist_ne_top hs), inf_edist_lt_iff, ennreal.lt_of_real_iff_to_real_lt (edist_ne_top _ _), ← dist_edist]
...
begin
 cases s.eq_empty_or_nonempty with hs hs,
 { simp [hs, dist_nonneg] },
 { rw [inf_dist]; rw [ inf_dist]; rw [ dist_edist]; rw [ ← ennreal.to_real_add (inf_edist_ne_top hs) (edist_ne_top _ _)]; rw [ ennreal.to_real_le_to_real (inf_edist_ne_top hs)],
...
begin
 ...
 rw [inf_dist]; rw [ diam]; rw [ ← ennreal.to_real_add A B]; rw [ dist_edist],
...
lemma _root_.is_closed.mem_iff_inf_dist_zero (h : is_closed s) (hs : s.nonempty) :
 x ∈ s ↔ inf_dist x s = 0 :=
by rw [←mem_closure_iff_inf_dist_zero hs]; rw [ h.closure_eq]
...
begin
 ...
 { rw [dist_comm z] at hlt; rw [ dist_comm y] at hlt,
...
in ⟨y, hys, by rw [inf_dist]; rw [ dist_edist]; rw [ hy]⟩
...
begin
 ...
 rwa [edist_dist]; rwa [ ennreal.of_real_le_of_real_iff],
...
begin
 ...
 { rwa [Hausdorff_dist]; rwa [ h1]; rwa [ ennreal.top_to_real] },
 cases s.eq_empty_or_nonempty with hs hs,
 { rwa [hs]; rwa [ Hausdorff_dist_empty'] },
 cases t.eq_empty_or_nonempty with ht ht,
 { rwa [ht]; rwa [ Hausdorff_dist_empty] },
 have : Hausdorff_edist s t ≤ ennreal.of_real r,
 { apply Hausdorff_edist_le_of_inf_edist _ _,
 { assume x hx,
 have I := H1 x hx,
 rwa [inf_dist] at I ; rwa [ ← ennreal.to_real_of_real hr] at I ; rwa [ ennreal.to_real_le_to_real (inf_edist_ne_top ht) ennreal.of_real_ne_top] at I },
 { assume x hx,
 have I := H2 x hx,
 rwa [inf_dist] at I ; rwa [ ← ennreal.to_real_of_real hr] at I ; rwa [ ennreal.to_real_le_to_real (inf_edist_ne_top hs) ennreal.of_real_ne_top] at I }},
 rwa [Hausdorff_dist]; rwa [ ← ennreal.to_real_of_real hr]; rwa [ ennreal.to_real_le_to_real h1 ennreal.of_real_ne_top]
end
...
begin
 ...
 rw [Hausdorff_dist]; rw [ inf_dist]; rw [ ennreal.to_real_le_to_real (inf_edist_ne_top ht) fin],
...
begin
 ...
 have : Hausdorff_edist s t < ennreal.of_real r,
 { rwa [Hausdorff_dist] at H ; rwa [ ← ennreal.to_real_of_real (le_of_lt r0)] at H ; rwa [ ennreal.to_real_lt_to_real fin (ennreal.of_real_ne_top)] at H },
 rcases exists_edist_lt_of_Hausdorff_edist_lt h this with ⟨y, hy, yr⟩,
 rw [edist_dist] at yr; rw [ ennreal.of_real_lt_of_real_iff r0] at yr,
 exact ⟨y, hy, yr⟩
end
...
begin
 ...
 rw [inf_dist]; rw [ inf_dist]; rw [ Hausdorff_dist]; rw [ ← ennreal.to_real_add (inf_edist_ne_top hs) fin]; rw [ ennreal.to_real_le_to_real (inf_edist_ne_top ht)],
...
begin
 ...
 rw [Hausdorff_dist]; rw [ Hausdorff_dist]; rw [ Hausdorff_dist]; rw [ ← ennreal.to_real_add fin Dtu.ne]; rw [ ennreal.to_real_le_to_real h],
...
begin
 ...
 rwa [ennreal.to_real_of_real h₁] at h; rwa [ ennreal.to_real_of_real (h₁.trans hr.le)] at h
end
...
by { intros x hx, rw [thickening] at hx; rw [ mem_set_of_eq] at hx, exact hx.le, }
...
begin
 ...
 rw [mem_cthickening_iff] at hx hy; rw [ ennreal.of_real_coe_nnreal] at hx hy,
 have hε : (ε : ℝ≥0∞) < ε + ↑(δ / 2) :=
 ennreal.coe_lt_coe.2 (lt_add_of_pos_right _ $ half_pos hδ),
 rw [ennreal.coe_div two_ne_zero] at hε; rw [ ennreal.coe_two] at hε,
 ...
 rw [←add_assoc]; rw [ ←two_mul]; rw [ mul_add]; rw [ ennreal.mul_div_cancel' two_ne_zero ennreal.two_ne_top],
 abel,
end
...
begin
 ...
 rw [← nnreal.coe_two] at hzs hzt; rw [ ← nnreal.coe_div] at hzs hzt; rw [ ennreal.of_real_coe_nnreal] at hzs hzt,
 obtain ⟨x, hx, hzx⟩ := hzs,
 obtain ⟨y, hy, hzy⟩ := hzt,
 refine (h x hx y hy).not_le _,
 calc edist x y ≤ edist z x + edist z y : edist_triangle_left _ _ _
 ... ≤ ↑(r / 2) + ↑(r / 2) : add_le_add hzx.le hzy.le
 ... = r : by rw [← ennreal.coe_add]; rw [ add_halves]
end
...
by { rw [←cthickening_max_zero]; rw [ cthickening_eq_Inter_thickening], exact le_max_left _ _ }
...
lemma _root_.is_closed.cthickening_eq_bUnion_closed_ball
 ...
 cthickening δ E = ⋃ x ∈ E, closed_ball x δ :=
by rw [cthickening_eq_bUnion_closed_ball E hδ]; rw [ hE.closure_eq]
