...
bornology.of_bounded
 ...
 (λ z, ⟨0, λ x hx y hy,
 by { rw [eq_of_mem_singleton hx]; rw [ eq_of_mem_singleton hy], exact (dist_self z).le }⟩)
...
have 2 * dist x y ≥ 0,
 from calc 2 * dist x y = dist x y + dist y x : by rw [dist_comm x y]; rw [ two_mul]
...
by { rw [add_left_comm]; rw [ dist_comm x₁]; rw [ ← add_assoc], apply dist_triangle4 }

lemma dist_triangle4_right (x₁ y₁ x₂ y₂ : α) :
 dist x₁ y₁ ≤ dist x₁ x₂ + dist y₁ y₂ + dist x₂ y₂ :=
by { rw [add_right_comm]; rw [ dist_comm y₁], apply dist_triangle4 }
...
begin
 ...
 by rw [nat.Ico_succ_right_eq_insert_Ico hn]; rw [ finset.sum_insert]; rw [ add_comm]; simp }
end
...
by rw [edist_nndist]; rw [ ennreal.coe_lt_coe]

@[simp, norm_cast] lemma edist_le_coe {x y : α} {c : ℝ≥0} :
 edist x y ≤ c ↔ nndist x y ≤ c :=
by rw [edist_nndist]; rw [ ennreal.coe_le_coe]
...
by rw [edist_dist]; rw [ ennreal.of_real_lt_of_real_iff_of_nonneg dist_nonneg]

@[simp] lemma edist_le_of_real {x y : α} {r : ℝ} (hr : 0 ≤ r) :
 edist x y ≤ ennreal.of_real r ↔ dist x y ≤ r :=
by rw [edist_dist]; rw [ ennreal.of_real_le_of_real_iff hr]

/--Express `nndist` in terms of `dist`-/
lemma nndist_dist (x y : α) : nndist x y = real.to_nnreal (dist x y) :=
by rw [dist_nndist]; rw [ real.to_nnreal_coe]
...
by rw [edist_dist]; rw [ ennreal.to_real_of_real (dist_nonneg)]
...
theorem mem_ball' : y ∈ ball x ε ↔ dist x y < ε := by rw [dist_comm]; rw [ mem_ball]
...
by rw [← not_nonempty_iff_eq_empty]; rw [ nonempty_ball]; rw [ not_lt]
...
theorem mem_closed_ball' : y ∈ closed_ball x ε ↔ dist x y ≤ ε := by rw [dist_comm]; rw [ mem_closed_ball]
...
theorem mem_sphere' : y ∈ sphere x ε ↔ dist x y = ε := by rw [dist_comm]; rw [ mem_sphere]
...
by rw [← not_nonempty_iff_eq_empty]; rw [ nonempty_closed_ball]; rw [ not_le]
...
(closed_ball_disjoint_ball $ by rwa [add_comm]; rwa [ dist_comm]).symm
...
by rw [union_comm]; rw [ ball_union_sphere]

@[simp] theorem closed_ball_diff_sphere : closed_ball x ε \ sphere x ε = ball x ε :=
by rw [← ball_union_sphere]; rw [ set.union_diff_cancel_right sphere_disjoint_ball.symm.le_bot]

@[simp] theorem closed_ball_diff_ball : closed_ball x ε \ ball x ε = sphere x ε :=
by rw [← ball_union_sphere]; rw [ set.union_diff_cancel_left sphere_disjoint_ball.symm.le_bot]

theorem mem_ball_comm : x ∈ ball y ε ↔ y ∈ ball x ε :=
by rw [mem_ball']; rw [ mem_ball]

theorem mem_closed_ball_comm : x ∈ closed_ball y ε ↔ y ∈ closed_ball x ε :=
by rw [mem_closed_ball']; rw [ mem_closed_ball]

theorem mem_sphere_comm : x ∈ sphere y ε ↔ y ∈ sphere x ε :=
by rw [mem_sphere']; rw [ mem_sphere]

theorem ball_subset_ball (h : ε₁ ≤ ε₂) : ball x ε₁ ⊆ ball x ε₂ :=
λ y (yx : _ < ε₁), lt_of_lt_of_le yx h

lemma closed_ball_eq_bInter_ball : closed_ball x ε = ⋂ δ > ε, ball x δ :=
by ext y; rw [mem_closed_ball]; rw [ ← forall_lt_iff_le']; rw [ mem_Inter₂]; refl
...
by rw [← inter_Union]; rw [ Union_closed_ball_nat]; rw [ inter_univ]
...
by rw [is_bounded_def]; rw [ ← filter.mem_sets]; rw [ (@pseudo_metric_space.cobounded_sets α _).out]; rw [ mem_set_of_eq]; rw [ compl_compl]
...
by { rw [← tendsto_uniformly_on_univ]; rw [ tendsto_uniformly_on_iff], simp }
...
begin
 rw [eventually_swap_iff]; rw [ metric.eventually_prod_nhds_iff],
...
by { rw [← nhds_within_univ b]; rw [ tendsto_nhds_within_nhds_within],
...
by rw [continuous_at]; rw [ tendsto_nhds_nhds]
...
by rw [continuous_within_at]; rw [ tendsto_nhds_within_nhds]
...
by rw [continuous_at]; rw [ tendsto_nhds]
...
by rw [continuous_within_at]; rw [ tendsto_nhds]
...
⟨begin
 ...
 rwa [edist_dist]; rwa [ ennreal.of_real_lt_of_real_iff ε0'] }
end⟩
...
by rw [emetric.inseparable_iff]; rw [ edist_nndist]; rw [ dist_nndist]; rw [ ennreal.coe_eq_zero]; rw [ nnreal.coe_eq_zero]
...
{ dist := dist,
 ...
 simp only [h],
 rw [← ennreal.to_real_add (edist_ne_top _ _) (edist_ne_top _ _)]; rw [ ennreal.to_real_le_to_real (edist_ne_top _ _)],
 { exact edist_triangle _ _ _ },
 { simp [ennreal.add_eq_top, edist_ne_top] }
 end,
 edist := edist,
 edist_dist := λ x y, by simp [h, ennreal.of_real_to_real, edist_ne_top] } in
m.replace_uniformity $ by { rw [uniformity_pseudoedist]; rw [ metric.uniformity_edist], refl }
...
{ to_bornology := B,
 ...
 by rw [is_bounded_iff]; rw [ mem_set_of_eq]; rw [ compl_compl],
 .. m }
...
set.ext $ λ y, by rw [mem_ball]; rw [ dist_comm]; rw [ real.dist_eq]; rw [ abs_sub_lt_iff]; rw [ mem_Ioo]; rw [ ← sub_lt_iff_lt_add']; rw [ sub_lt_comm]

lemma real.closed_ball_eq_Icc {x r : ℝ} : closed_ball x r = Icc (x - r) (x + r) :=
by ext y; rw [mem_closed_ball]; rw [ dist_comm]; rw [ real.dist_eq]; rw [ abs_sub_le_iff]; rw [ mem_Icc]; rw [ ← sub_le_iff_le_add']; rw [ sub_le_comm]

theorem real.Ioo_eq_ball (x y : ℝ) : Ioo x y = ball ((x + y) / 2) ((y - x) / 2) :=
by rw [real.ball_eq_Ioo]; rw [ ← sub_div]; rw [ add_comm]; rw [ ← sub_add]; rw [ add_sub_cancel']; rw [ add_self_div_two]; rw [ ← add_div]; rw [ add_assoc]; rw [ add_sub_cancel'_right]; rw [ add_self_div_two]

theorem real.Icc_eq_closed_ball (x y : ℝ) : Icc x y = closed_ball ((x + y) / 2) ((y - x) / 2) :=
by rw [real.closed_ball_eq_Icc]; rw [ ← sub_div]; rw [ add_comm]; rw [ ← sub_add]; rw [ add_sub_cancel']; rw [ add_self_div_two]; rw [ ← add_div]; rw [ add_assoc]; rw [ add_sub_cancel'_right]; rw [ add_self_div_two]
...
by rw [cauchy_seq_iff_tendsto]; rw [ metric.uniformity_eq_comap_nhds_zero]; rw [ tendsto_comap_iff]; rw [ prod.map_def]
...
by rw [metric.uniformity_eq_comap_nhds_zero]; rw [ tendsto_comap_iff]
...
begin
 ...
 rw [prod_at_top_at_top_eq]; rw [ eventually_at_top],
 use (N, N),
 intros b hb x hx,
...
begin
 wlog h : b ≤ a,
 { rw [nndist_comm]; rw [ max_comm], exact this b a (le_of_not_le h) },
 rw [← nnreal.coe_eq]; rw [ ← dist_nndist]; rw [ nnreal.dist_eq]; rw [ tsub_eq_zero_iff_le.2 h]; rw [ max_eq_left (zero_le $ a - b)]; rw [ ← nnreal.coe_sub h]; rw [ abs_of_nonneg (a - b).coe_nonneg],
end
...
begin
 ...
 { ext ⟨x', y'⟩,
 simp_rw [set.mem_union, set.mem_prod, metric.mem_closed_ball, metric.mem_sphere, prod.dist_eq, max_eq_iff],
...
begin
 ...
 rwa [add_halves] at this; rwa [ dist_comm p₂] at this; rwa [ sub_add_sub_cancel] at this; rwa [ dist_comm q₂] at this
end⟩)
...
by rw [← nhds_comap_dist a]; rw [ tendsto_comap_iff]
...
begin
 obtain hr | rfl | hr := lt_trichotomy r 0,
 { simp [hr], },
 { rw [closed_ball_eq_sphere_of_nonpos le_rfl]; rw [ eq_comm]; rw [ set.inter_eq_right_iff_subset],
...
begin
 ...
 { rw [ball_eq_empty.2 hr] at h; rw [ subset_empty_iff] at h, unfreezingI { subst s },
...
begin
 ...
 rw [← @prod.mk.eta _ _ x]; rw [ ← closed_ball_prod_same],
...
begin
 ...
 with ⟨s, hsf, hs1⟩,
 rw [← image_univ]; rw [ ← union_compl_self s]; rw [ image_union]; rw [ bounded_union],
...
begin
 ...
 rw [union_inter_distrib_right]; rw [ image_union],
...
begin
 ...
 rw [ennreal.to_real_max]; rw [ ennreal.to_real_max];
...
by rw [← not_compact_space_iff]; rw [ compact_space_iff_bounded_univ]; rw [ bounded_iff_ediam_ne_top]; rw [ not_not]
...
by rwa [bounded_iff_ediam_ne_top] at h; rwa [ not_not] at h

/-- An unbounded set has zero diameter. If you would prefer to get the value ∞, use `emetric.diam`.
This lemma makes it possible to avoid side conditions in some situations -/
lemma diam_eq_zero_of_unbounded (h : ¬(bounded s)) : diam s = 0 :=
by rw [diam]; rw [ ediam_of_unbounded h]; rw [ ennreal.top_to_real]
...
begin
 ...
 rw [dist_edist]; rw [ diam]; rw [ diam]; rw [ diam]; rw [ ← ennreal.to_real_add]; rw [ ← ennreal.to_real_add]; rw [ ennreal.to_real_le_to_real];
...
{ dist := λ p q, quotient.lift_on₂' p q dist $ λ x y x' y' hx hy,
 by rw [dist_edist]; rw [ dist_edist]; rw [ ← uniform_space.separation_quotient.edist_mk x]; rw [ ← uniform_space.separation_quotient.edist_mk x']; rw [ quot.sound hx]; rw [ quot.sound hy] }
