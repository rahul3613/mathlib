...
{ dist_self := λ x, by simp [dist_complete_copy_eq],
 ...
 begin
 rw [← real.dist_eq]; rw [ ← real.dist_eq]; rw [ ← real.dist_eq],
 exact add_le_add (dist_triangle _ _ _) (dist_triangle _ _ _)
 end
 ... = dist x y + dist y z : by { rw [dist_complete_copy_eq]; rw [ dist_complete_copy_eq], abel },
 eq_of_dist_eq_zero :=
 begin
 assume x y hxy,
...
{ to_fun := id,
 ...
 { rw [continuous_at]; rw [ tendsto_iff_dist_tendsto_zero],
