...
begin
 ...
 rw [glue_dist]; rw [ this]; rw [ zero_add]
end
...
begin
 ...
 { rintro ⟨ε, ε0, H⟩,
 split; rw [filter.mem_sets]; rw [ filter.mem_map]; rw [ mem_uniformity_dist];
...
{ dist := glue_dist Φ Ψ 0,
 ...
 dist_triangle := glue_dist_triangle Φ Ψ 0 $ λp q, by rw [hΦ.dist_eq]; rw [ hΨ.dist_eq]; simp }
...
begin
 ...
 { assume hx hy,
 have A : max x.1 y.1 = 0, { rw [nonpos_iff_eq_zero.1 hx]; rw [ nonpos_iff_eq_zero.1 hy], simp },
 ...
 rw [le_rec_on_succ xm]; rw [ le_rec_on_succ ym]; rw [ (I m).dist_eq],
 exact hm xm ym }}
end
...
{ dist := inductive_limit_dist f,
 ...
 unfold dist,
 rw [inductive_limit_dist_eq_dist I x y m hx hy]; rw [ inductive_limit_dist_eq_dist I y x m hy hx]; rw [ dist_comm]
 end,
 ...
 by rw [inductive_limit_dist_eq_dist I x y m hx hy]; rw [ inductive_limit_dist_eq_dist I y z m hy hz]
 end }
...
begin
 ...
 rw [inductive_limit_dist_eq_dist I ⟨n, x⟩ ⟨n, y⟩ n (le_refl n) (le_refl n)]; rw [ le_rec_on_self]; rw [ le_rec_on_self]
end
...
begin
 ...
 { rw [inductive_limit_dist_eq_dist I ⟨n.succ, f n x⟩ ⟨n, x⟩ n.succ]; rw [ le_rec_on_self]; rw [ le_rec_on_succ]; rw [ le_rec_on_self]; rw [ dist_self],
 exact le_rfl,
 exact le_rfl,
 exact le_succ _ }
end
