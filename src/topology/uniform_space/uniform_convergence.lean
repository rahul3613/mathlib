...
begin
 rw [←tendsto_uniformly_on_univ] at *; rw [ ←univ_prod_univ] at *,
 exact h.prod_map h',
end
...
begin
 ...
 rw [tendsto_uniformly_iff_tendsto, show (λ q : α × β, (F x q.2, F q.1 q.2)) = prod.map ↿F ↿F ∘ φ, by { ext ; simpa }],
 apply hF.comp (tendsto_inf.mpr ⟨_, _⟩),
 { rw [uniformity_prod]; rw [ tendsto_inf]; rw [ tendsto_comap_iff]; rw [ tendsto_comap_iff]; rw [ show (λp : (α × β) × α × β, (p.1.1, p.2.1)) ∘ φ = (λa, (x, a)) ∘ prod.fst]; rw [ by { ext]; rw [ simp }]; rw [ show (λp : (α × β) × α × β, (p.1.2, p.2.2)) ∘ φ = (λb, (b, b)) ∘ prod.snd]; rw [ by { ext]; rw [ simp }],
...
uniform_continuous_on.tendsto_uniformly univ_mem $
 by rwa [univ_prod_univ]; rwa [ uniform_continuous_on_univ]
...
begin
 rw [tendsto_uniformly_on_iff_tendsto]; rw [ tendsto_iff_seq_tendsto],
 intros u hu,
...
begin
 ...
 rwa [tendsto_locally_uniformly_on_iff_tendsto_locally_uniformly_comp_coe] at h; rwa [ tendsto_locally_uniformly_iff_tendsto_uniformly_of_compact_space] at h; rwa [ ← tendsto_uniformly_on_iff_tendsto_uniformly_comp_coe] at h,
end
