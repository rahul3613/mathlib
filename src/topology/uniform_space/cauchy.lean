...
by rw [cauchy]; rw [ map_ne_bot_iff]; rw [ prod_map_map_eq]; rw [ tendsto]
...
begin
 rw [cauchy_seq_iff_tendsto]; rw [ ← prod_at_top_at_top_eq],
...
{ complete := λ f hf,
 ...
 ⟨(x1, x2), by rw [nhds_prod_eq]; rw [ filter.prod_def];
...
begin
 ...
 rw [range_subset_iff]; rw [ bUnion_image],
 intro m,
 rw [mem_Union₂],
