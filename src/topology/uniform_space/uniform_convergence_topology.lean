...
begin
 ...
 by rw [uniform_fun.filter]; rw [ ← filter_basis.generate]; rw [ sets_iff_generate]
...
begin
 change _ ≤ _,
 rw [map_le_iff_le_comap]; rw [ (uniform_fun.has_basis_uniformity α β).le_basis_iff ((𝓤 _).basis_sets.comap _)],
...
begin
 ...
 rw [infi_uniformity]; rw [ infi_uniformity],
...
begin
 -- This follows directly from the fact that the upper adjoint in a Galois connection maps
 -- infimas to infimas.
 rw [inf_eq_infi]; rw [ inf_eq_infi]; rw [ uniform_fun.infi_eq],
...
begin
 ...
 congr,
 rw [← uniform_space_eq hf]; rw [ uniform_fun.comap_eq],
 refl
end
...
begin
 rw [(uniform_fun.has_basis_nhds α β f).tendsto_right_iff]; rw [ tendsto_uniformly],
 exact iff.rfl,
end
...
begin
 ...
 rw ← uniformity_comap,
 congr,
 rw [prod.uniform_space]; rw [ prod.uniform_space]; rw [ uniform_space.comap_inf]; rw [ uniform_fun.inf_eq],
 congr;
 rw [← uniform_space.comap_comap]; rw [ uniform_fun.comap_eq];
...
begin
 ...
 rw ← uniformity_comap,
 congr,
 rw [Pi.uniform_space]; rw [ uniform_space.of_core_eq_to_core]; rw [ Pi.uniform_space]; rw [ uniform_space.of_core_eq_to_core]; rw [ uniform_space.comap_infi]; rw [ uniform_fun.infi_eq],
 refine infi_congr (λ i, _),
 rw [← uniform_space.comap_comap]; rw [ uniform_fun.comap_eq]
...
protected lemma inf_eq {u₁ u₂ : uniform_space γ} :
 ...
 𝒱(α, γ, 𝔖, u₂) :=
begin
 rw [inf_eq_infi]; rw [ inf_eq_infi]; rw [ uniform_on_fun.infi_eq],
 refine infi_congr (λ i, _),
 cases i; refl
end
...
begin
 ...
 rw [← uniformity_comap] at ⊢ hf,
 congr,
 rw [← uniform_space_eq hf]; rw [ uniform_on_fun.comap_eq],
 refl
end
...
begin
 ...
 rw [this]; rw [ @uniform_space.comap_comap (α →ᵤ[𝔖] β) ((f '' t) →ᵤ β)],
...
begin
 rw [uniform_on_fun.topological_space_eq]; rw [ nhds_infi]; rw [ tendsto_infi],
 refine forall_congr (λ s, _),
 rw [nhds_infi]; rw [ tendsto_infi],
 refine forall_congr (λ hs, _),
 rw [nhds_induced]; rw [ tendsto_comap_iff]; rw [ tendsto_uniformly_on_iff_tendsto_uniformly_comp_coe]; rw [ uniform_fun.tendsto_iff_tendsto_uniformly],
 refl
end
...
begin
 split,
 rw [uniformity_prod]; rw [ comap_inf]; rw [ comap_comap]; rw [ comap_comap]; rw [ uniform_on_fun.inf_eq]; rw [ inf_uniformity]; rw [ uniform_on_fun.comap_eq]; rw [ uniform_on_fun.comap_eq]; rw [ uniformity_comap]; rw [ uniformity_comap],
 refl -- the relevant diagram commutes by definition
end
...
begin
 ...
 rw ← uniformity_comap,
 congr,
 rw [Pi.uniform_space]; rw [ uniform_space.of_core_eq_to_core]; rw [ Pi.uniform_space]; rw [ uniform_space.of_core_eq_to_core]; rw [ uniform_space.comap_infi]; rw [ uniform_on_fun.infi_eq],
 refine infi_congr (λ i, _),
 rw [← uniform_space.comap_comap]; rw [ uniform_on_fun.comap_eq]
 -- Like in the previous lemma, the diagram actually commutes by definition
end
