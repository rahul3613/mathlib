...
begin
 ...
 simp_rw [uniformity_comap, (filter.comap_has_basis (prod.map f f) (𝓤 β)).sInter_sets, ← preimage_Inter, sInter_eq_bInter],
 refl,
end
...
begin
 classical,
 split ; introI h,
 { rw [t2_iff_is_closed_diagonal]; rw [ ← show 𝓢 α = diagonal α]; rw [ from h.1],
...
{ to_topological_space := u.to_topological_space.coinduced (λx, ⟦x⟧),
 ...
 simp at a_eq,
 simp at b_eq,
 have h : ⟦a₂⟧ = ⟦b₁⟧, { rw [a_eq.right]; rw [ b_eq.left] },
...
begin
 rw [uniform_continuous]; rw [ uniformity_prod_eq_prod]; rw [ uniformity_quotient]; rw [ uniformity_quotient]; rw [ filter.prod_map_map_eq]; rw [ filter.tendsto_map'_iff]; rw [ filter.tendsto_map'_iff],
 rwa [uniform_continuous] at hf; rwa [ uniformity_prod_eq_prod] at hf; rwa [ filter.tendsto_map'_iff] at hf
end
...
lemma lift_mk [separated_space β] {f : α → β} (h : uniform_continuous f) (a : α) :
 lift f ⟦a⟧ = f a :=
by rw [lift]; rw [ dif_pos h]; refl

lemma uniform_continuous_lift [separated_space β] (f : α → β) : uniform_continuous (lift f) :=
begin
 by_cases hf : uniform_continuous f,
 { rw [lift]; rw [ dif_pos hf], exact uniform_continuous_quotient_lift hf },
 { rw [lift]; rw [ dif_neg hf], exact uniform_continuous_of_const (assume a b, rfl) }
end
...
by rw [map]; rw [ lift_mk (uniform_continuous_quotient_mk.comp h)]
