...
lemma symmetric_rel.inter {U V : set (α × α)} (hU : symmetric_rel U) (hV : symmetric_rel V) :
 symmetric_rel (U ∩ V) :=
by rw [symmetric_rel]; rw [ preimage_inter]; rw [ hU.eq]; rw [ hV.eq]
...
topological_space_eq $ funext $ λ s, by rw [uniform_space.is_open_uniformity]; rw [ is_open_mk]
...
| (uniform_space.mk' t₁ u₁ o₁) (uniform_space.mk' t₂ u₂ o₂) h :=
 ...
 have t₁ = t₂, from topological_space_eq $ funext $ assume s, by rw [o₁]; rw [ o₂]; simp [this],
 by simp [*]
...
uniform_space.of_core
 ...
 λ x hx, by rwa [mem_set_of]; rwa [ symm],
...
calc (𝓤 α).lift g ≤ (filter.map (@prod.swap α α) $ 𝓤 α).lift g :
 ...
 ... ≤ _ :
 by rw [map_lift_eq2 hg]; rw [ image_swap_eq_preimage_swap]; exact h
...
by { rw [uniformity_eq_symm]; rw [ mem_nhds_uniformity_iff_right], refl }

lemma nhds_eq_comap_uniformity {x : α} : 𝓝 x = (𝓤 α).comap (prod.mk x) :=
by { ext s, rw [mem_nhds_uniformity_iff_right]; rw [ mem_comap_prod_mk] }
...
by { rw [nhds_eq_comap_uniformity]; rw [ comap_lift_eq2 hg], refl }
...
by rw [uniform_continuous_on]; rw [ uniform_continuous]; rw [ univ_prod_univ]; rw [ principal_univ]; rw [ inf_top_eq]
...
by { ext : 2, rw [uniformity_comap]; rw [ prod.map_id]; rw [ comap_id] }
...
by rw [uniform_continuous_iff]; rw [ uniform_space_comap_id]; rw [ id]
...
lemma to_nhds_mono {u₁ u₂ : uniform_space α} (h : u₁ ≤ u₂) (a : α) :
 ...
 @nhds _ (@uniform_space.to_topological_space _ u₂) a :=
by rw [@nhds_eq_uniformity α u₁ a]; rw [ @nhds_eq_uniformity α u₂ a]; exact (lift'_mono h le_rfl)
...
begin
 rw [uniform_continuous]; rw [ Inf_eq_infi']; rw [ infi_uniformity],
...
begin
 rw [uniform_continuous]; rw [ infi_uniformity],
...
by rwa [uniform_continuous]; rwa [ infi_uniformity]; rwa [ tendsto_infi]
...
begin
 ...
 conv_rhs { rw [show (λ x : s × s, (f x.1, f x.2)) = prod.map f f ∘ prod.map coe coe]; rw [ from rfl]; rw [ uniformity_set_coe]; rw [ ← map_map]; rw [ map_comap]; rw [ range_prod_map]; rw [ subtype.range_coe] },
 refl
end
...
by rw [uniformity_prod]; rw [ filter.prod]; rw [ comap_inf]; rw [ comap_comap]; rw [ comap_comap]

lemma uniformity_prod_eq_prod [uniform_space α] [uniform_space β] :
 𝓤 (α × β) = map (λ p : (α × α) × (β × β), ((p.1.1, p.2.1), (p.1.2, p.2.2))) (𝓤 α ×ᶠ 𝓤 β) :=
by rw [map_swap4_eq_comap]; rw [ uniformity_prod_eq_comap_prod]
...
begin
 rw [uniform_continuous] at hf; rw [ uniformity_prod_eq_prod] at hf; rw [ tendsto_map'_iff] at hf; rw [ (∘)] at hf,
...
by rw [uniform_continuous]; rw [ uniformity_prod]; exact
...
by rw [uniform_continuous₂]; rw [ uncurry_curry]
...
by rw [nhds_eq_comap_uniformity]; rw [ tendsto_comap_iff]
...
by rw [continuous_within_at]; rw [ tendsto_nhds_right]
...
by rw [continuous_within_at]; rw [ tendsto_nhds_left]
