...
by rw [uniform_inducing_iff]; rw [ uniform_continuous]; rw [ tendsto_iff_comap]; rw [ le_antisymm_iff]; rw [ and_comm]; refl
...
begin
 ...
 rw [← hgf.1]; rw [ ← prod.map_def]; rw [ ← prod.map_def]; rw [ ← prod.map_comp_map f f g g]; rw [ ← @comap_comap _ _ _ _ (prod.map f f)],
...
by { dsimp only [uniform_continuous, tendsto],
 rw [← hg.comap_uniformity]; rw [ ← map_le_iff_le_comap]; rw [ filter.map_map] }
...
by rw [uniform_embedding_iff]; rw [ and_comm]; rw [ uniform_inducing_iff']
...
theorem filter.has_basis.uniform_embedding_iff' {ι ι'} {p : ι → Prop} {p' : ι' → Prop} {s s'}
 ...
 (∀ i, p' i → ∃ j, p j ∧ ∀ x y, (x, y) ∈ s j → (f x, f y) ∈ s' i) ∧
 (∀ j, p j → ∃ i, p' i ∧ ∀ x y, (f x, f y) ∈ s' i → (x, y) ∈ s j) :=
by rw [uniform_embedding_iff]; rw [ and_comm]; rw [ h.uniform_inducing_iff h']
...
{ comap_uniformity :=
 by { erw [uniformity_subtype]; erw [ uniformity_subtype]; erw [ comap_comap], congr },
...
uniform_embedding_iff'.2 ⟨f.injective, h₁, by rwa [← equiv.prod_congr_apply]; rwa [ ← map_equiv_symm]⟩

theorem uniform_embedding_inl : uniform_embedding (sum.inl : α → α ⊕ β) :=
begin
 refine ⟨⟨_⟩, sum.inl_injective⟩,
 rw [sum.uniformity]; rw [ comap_sup]; rw [ comap_map]; rw [ comap_eq_bot_iff_compl_range.2 _]; rw [ sup_bot_eq],
...
begin
 ...
 rw [sum.uniformity]; rw [ comap_sup]; rw [ comap_eq_bot_iff_compl_range.2 _]; rw [ comap_map]; rw [ bot_sup_eq],
...
begin
 intros b' hb',
 rw [nhds_eq_uniformity]; rw [ lift'_inf_principal_eq]; rw [ lift'_ne_bot_iff],
...
begin
 ...
 set m' : s → β := m ∘ coe,
 suffices : is_complete (range m'), by rwa [range_comp] at this; rwa [ subtype.range_coe] at this,
...
by rw [complete_space_iff_is_complete_univ]; rw [ ← is_complete_image_iff hf]; rw [ image_univ]
...
by rw [complete_space_iff_is_complete_range he.to_uniform_inducing]; rw [ e.range_eq_univ]; rw [ complete_space_iff_is_complete_univ]
...
instance complete_space.sum [complete_space α] [complete_space β] :
 complete_space (α ⊕ β) :=
begin
 rw [complete_space_iff_is_complete_univ]; rw [ ← range_inl_union_range_inr],
...
begin
 ...
 rw [←comap_comap] at hc; rw [ tendsto_comap'_iff] at hc,
 exact ⟨c, hc⟩,
 exact ⟨_, hb, assume x,
 begin
 change e x ∈ (closure (e '' s)) → x ∈ range subtype.val,
 rw [← closure_induced]; rw [ mem_closure_iff_cluster_pt]; rw [ cluster_pt]; rw [ ne_bot_iff]; rw [ nhds_induced]; rw [ ← de.to_dense_inducing.nhds_eq_comap]; rw [ ← mem_closure_iff_nhds_ne_bot]; rw [ hs.closure_eq],
...
show preimage (λp:(α×α), (ψ p.1, ψ p.2)) d ∈ 𝓤 α,
 ...
 by rwa [nhds_prod_eq] at this; rwa [ le_principal_iff] at this,
