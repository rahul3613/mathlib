...
let ⟨t, ht, hts⟩ :=
 by rw [← (@uniform_embedding_pure_cauchy α _).comap_uniformity] at hs; rw [ filter.mem_comap] at hs;
 exact hs in
...
begin
 ...
 rw [comap_quotient_eq_uniformity]; rw [ uniform_embedding_pure_cauchy.comap_uniformity]
end
...
begin
 ...
 rw [extension_coe (separation_quotient.uniform_continuous_lift _)]; rw [ separation_quotient.lift_mk (uniform_continuous_coe α)]; rw [ completion.map_coe uniform_continuous_quotient_mk] ; apply_instance },
 { assume a,
 refine completion.induction_on a
 (is_closed_eq (continuous_extension.comp continuous_map) continuous_id) (λ a, _),
 rw [map_coe uniform_continuous_quotient_mk]; rw [ extension_coe (separation_quotient.uniform_continuous_lift _)]; rw [ separation_quotient.lift_mk (uniform_continuous_coe α) _] ; apply_instance }
end
