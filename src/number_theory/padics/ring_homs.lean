...
begin
 ...
 rw [← not_lt]; rw [ val_eq_coe]; rw [ coe_nat_cast],
 intro norm_denom_lt,
 ...
 rwa [← r.cop.gcd_eq_one]; rwa [ nat.dvd_gcd_iff]; rwa [ ← int.coe_nat_dvd_left]; rwa [ ← int.coe_nat_dvd],
end
...
begin
 let n := mod_part p r,
 rw [norm_lt_one_iff_dvd]; rw [ ← (is_unit_denom r h).dvd_mul_right],
...
begin
 ...
 rw [← sub_eq_zero]; rw [ ← int.cast_sub]; rw [ zmod.int_coe_zmod_eq_zero_iff_dvd]; rw [ int.coe_nat_pow],
 rw [← dvd_neg] at ha; rw [ neg_sub] at ha,
 have := dvd_add ha hb,
 rwa [sub_eq_add_neg] at this; rwa [ sub_eq_add_neg] at this; rwa [ add_assoc] at this; rwa [ neg_add_cancel_left] at this; rwa [ ← sub_eq_add_neg] at this; rwa [ ← int.cast_sub] at this; rwa [ pow_p_dvd_int_iff] at this,
end
...
begin
 ...
 apply le_pred_of_lt,
 apply zmod.val_lt },
 { rw [mul_tsub]; rw [ mul_one]; rw [ ← pow_succ'],
...
begin
 ...
 { rw hc },
 rw [valuation_p_pow_mul _ _ hc']; rw [ add_sub_cancel']; rw [ pow_succ']; rw [ ← mul_sub],
 apply mul_dvd_mul_left,
 obtain hc0 | hc0 := c.valuation.nat_abs.eq_zero_or_pos,
 { simp only [hc0, mul_one, pow_zero],
 rw [mul_comm] at hc; rw [ unit_coeff_spec h] at hc,
 suffices : c = unit_coeff h,
 { rw [← this]; rw [ ← ideal.mem_span_singleton]; rw [ ← maximal_ideal_eq_span_p],
 apply to_zmod_spec },
 obtain ⟨c, rfl⟩ : is_unit c, -- TODO: write a can_lift instance for units
 { rw int.nat_abs_eq_zero at hc0,
 rw [is_unit_iff]; rw [ norm_eq_pow_val hc']; rw [ hc0]; rw [ neg_zero]; rw [ zpow_zero], },
...
begin
 ...
 ext x,
 rw [ring_hom.mem_ker]; rw [ ring_hom.mem_ker],
...
begin
 ...
 rw [← int.coe_nat_pow]; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd]; rw [ int.cast_sub],
 dsimp [nth_hom],
 rw [← f_compat]; rw [ ring_hom.comp_apply],
...
begin
 ...
 dsimp [nth_hom_seq],
 apply lt_of_le_of_lt _ hn,
 rw [← int.cast_add]; rw [ ← int.cast_sub]; rw [ ← padic_norm.dvd_iff_norm_le]; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd],
 dsimp [nth_hom],
 simp only [zmod.nat_cast_val, ring_hom.map_add, int.cast_sub, zmod.int_cast_cast, int.cast_add],
 rw [zmod.cast_add (show p^n ∣ p^j, from pow_dvd_pow _ hj)]; rw [ sub_self],
 { apply_instance },
end
...
begin
 ...
 dsimp [nth_hom_seq],
 apply lt_of_le_of_lt _ hn,
 rw [← int.cast_mul]; rw [ ← int.cast_sub]; rw [ ← padic_norm.dvd_iff_norm_le]; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd],
 dsimp [nth_hom],
...
begin
 ext r,
 rw [ring_hom.comp_apply]; rw [ ← zmod.nat_cast_zmod_val (f n r)]; rw [ ← map_nat_cast $ to_zmod_pow n]; rw [ ← sub_eq_zero]; rw [ ← ring_hom.map_sub]; rw [ ← ring_hom.mem_ker]; rw [ ker_to_zmod_pow],
...
begin
 ...
 rw [dist_eq_norm]; rw [ norm_le_pow_iff_mem_span_pow]; rw [ ← ker_to_zmod_pow]; rw [ ring_hom.mem_ker]; rw [ ring_hom.map_sub]; rw [ ← ring_hom.comp_apply]; rw [ ← ring_hom.comp_apply]; rw [ lift_spec]; rw [ hg]; rw [ sub_self],
end
