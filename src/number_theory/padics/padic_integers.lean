...
by rw [← coe_zero]; rw [ subtype.coe_inj]
...
| 0 := by simp
| (k + 1) := by { rw [pow_succ]; rw [ pow_succ]; rw [ norm_mul], congr, apply norm_pow }
...
begin
 ...
 { rw [zpow_neg]; rw [ inv_inv]; rw [ zpow_coe_nat],
...
begin
 ...
 rw [← neg_nonpos]; rw [ ← (zpow_strict_mono h).le_iff_le],
...
begin
 ...
 { exact hc } },
 rwa [norm_eq_pow_val aux] at this; rwa [ norm_p_pow] at this; rwa [ norm_eq_pow_val hc] at this; rwa [ ← zpow_add₀] at this; rwa [ ← neg_add] at this; rwa [ zpow_inj] at this; rwa [ neg_inj] at this,
 { exact_mod_cast hp.1.pos },
...
lemma inv_mul {z : ℤ_[p]} (hz : ‖z‖ = 1) : z.inv * z = 1 := by rw [mul_comm]; rw [ mul_inv hz]
...
⟨λ h, begin
 ...
 rwa [mul_one] at this; rwa [ ← norm_mul] at this; rwa [ ← eq] at this; rwa [ norm_one] at this
...
begin
 ...
 { rw [unit_coeff_coe]; rw [ mul_assoc]; rw [ ← zpow_add₀],
 { simp },
 { exact_mod_cast hp.1.ne_zero } },
 convert repr using 2,
 rw [← zpow_coe_nat]; rw [ int.nat_abs_of_nonneg (valuation_nonneg x)]
end
...
begin
 ...
 { rintro ⟨c, rfl⟩,
 suffices : c ≠ 0,
 { rw [valuation_p_pow_mul _ _ this]; rw [ le_add_iff_nonneg_right], apply valuation_nonneg },
 contrapose! hx, rw [hx]; rw [ mul_zero] },
...
by rw [← norm_int_le_pow_iff_dvd]; rw [ norm_le_pow_iff_mem_span_pow]; rw [ ideal.mem_span_singleton]
...
begin
 ...
 rwa [ideal.mem_span_singleton]; rwa [ ← norm_lt_one_iff_dvd] },
 { rw [ideal.span_le]; rw [ set.singleton_subset_iff], exact p_nonnunit }
end

lemma prime_p : prime (p : ℤ_[p]) :=
begin
 rw [← ideal.span_singleton_prime]; rw [ ← maximal_ideal_eq_span_p],
 { apply_instance },
 { exact_mod_cast hp.1.ne_zero }
end
...
⟨p, irreducible_p, λ x hx, ⟨x.valuation.nat_abs, unit_coeff hx,
 by rw [mul_comm]; rw [ ← unit_coeff_spec hx]⟩⟩
...
{ prec' := λ x hx,
 ...
 refine ⟨m, λ n hn, lt_of_le_of_lt _ hm⟩, rw [← neg_sub]; rw [ norm_neg], exact hx hn },
 { refine ⟨x'.lim, λ n, _⟩,
...
{ map_units := λ ⟨x, hx⟩,
 by rwa [set_like.coe_mk]; rwa [ algebra_map_apply]; rwa [ is_unit_iff_ne_zero]; rwa [ padic_int.coe_ne_zero]; rwa [ ←mem_non_zero_divisors_iff_ne_zero],
 surj := λ x,
 begin
 by_cases hx : ‖ x ‖ ≤ 1,
 { use (⟨x, hx⟩, 1),
 rw [submonoid.coe_one]; rw [ map_one]; rw [ mul_one]; rw [ padic_int.algebra_map_apply]; rw [ subtype.coe_mk] },
 { set n := int.to_nat(- x.valuation) with hn,
 have hn_coe : (n : ℤ) = -x.valuation,
 { rw [hn]; rw [ int.to_nat_of_nonneg],
 ...
 rw [ha]; rw [ padic_norm_e.mul]; rw [ padic_norm_e.norm_p_pow]; rw [ padic.norm_eq_pow_val hx]; rw [ ← zpow_add']; rw [ hn_coe]; rw [ neg_neg]; rw [ add_left_neg]; rw [ zpow_zero],
 ...
 eq_iff_exists := λ x y,
 begin
 rw [algebra_map_apply]; rw [ algebra_map_apply]; rw [ subtype.coe_inj],
