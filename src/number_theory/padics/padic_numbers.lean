...
⟨ max N1 N2,
 ...
 rw [padic_norm.neg] at hnam; rw [ max_comm] at hnam,
 rw [← hnam] at this; rw [ sub_eq_add_neg] at this; rw [ add_comm] at this,
...
begin
 rw [norm]; rw [ valuation]; rw [ dif_neg hf]; rw [ dif_neg hf]; rw [ padic_norm]; rw [ if_neg],
 intro H,
...
begin
 rw [norm_eq_pow_val hf]; rw [ norm_eq_pow_val hg]; rw [ ← neg_inj]; rw [ zpow_inj],
...
begin
 ...
 rw [hpeq] at hN'; rw [ max_eq_left_of_lt hlt] at hN',
...
if hf : f ≈ 0 then
 ...
 by rw [h1]; rw [ h2]; rw max_eq_right (norm_nonneg _)
else if hg : g ≈ 0 then
 ...
 by rw [h1]; rw [ h2]; rw max_eq_left (norm_nonneg _)
else
begin
...
by rw [norm_zpow]; rw [ norm_p]; rw [ zpow_neg]; rw [ inv_zpow]

@[simp] lemma norm_p_pow (n : ℕ) : ‖(p ^ n : ℚ_[p])‖ = p ^ (-n : ℤ) :=
by rw [←norm_p_zpow]; rw [ zpow_coe_nat]
...
| ⟨n, d, hn, hd⟩ := λ hq : ¬ p ∣ d,
 ...
 rw [padic_norm_e.eq_padic_norm],
 norm_cast,
 rw [padic_norm.eq_zpow_of_nonzero hnz']; rw [ padic_val_rat]; rw [ neg_sub]; rw [ padic_val_nat.eq_zero_of_not_dvd hq],
 norm_cast,
 rw [zero_sub]; rw [ zpow_neg]; rw [ zpow_coe_nat],
 apply inv_le_one,
 { norm_cast,
 apply one_le_pow,
 exact hp.1.pos }
...
begin
 ...
 { norm_cast at H ⊢,
 convert zpow_zero _,
 rw [neg_eq_zero]; rw [ padic_val_rat.of_int],
 norm_cast,
 ...
 ... < 1 : _,
 { rw [mul_one]; rw [ padic_norm_e.norm_p],
 apply inv_lt_one,
 exact_mod_cast hp.1.one_lt } }
end
...
begin
 ...
 { rw [rat.cast_zpow]; rw [ rat.cast_coe_nat] },
...
begin
 ...
 have hp_one : (1 : ℝ) < p,
 { rw [← nat.cast_one]; rw [ nat.cast_lt],
 exact nat.prime.one_lt hp.elim },
 rwa [norm_eq_pow_val hx] at h_norm ; rwa [ norm_eq_pow_val hy] at h_norm ; rwa [ norm_eq_pow_val hxy] at h_norm ; rwa [ zpow_le_max_iff_min_le hp_one] at h_norm } }
end
...
begin
 ...
 { rw [← nat.cast_one]; rw [ ne.def]; rw [ nat.cast_inj],
 exact nat.prime.ne_one hp.elim },
 have hp_pos : (0 : ℝ) < p,
 { rw [← nat.cast_zero]; rw [ nat.cast_lt],
 exact nat.prime.pos hp.elim },
 rw [norm_eq_pow_val hx] at h_norm; rw [ norm_eq_pow_val hy] at h_norm; rw [ norm_eq_pow_val (mul_ne_zero hx hy)] at h_norm; rw [ ← zpow_add₀ (ne_of_gt hp_pos)] at h_norm; rw [ zpow_inj hp_pos hp_ne_one] at h_norm; rw [ ← neg_add] at h_norm; rw [ neg_inj] at h_norm,
 exact h_norm
end
...
begin
 ...
 by_cases hx : x = 0,
 { rw [hx]; rw [ if_pos (eq.refl _)]; rw [ zero_mul]; rw [ if_pos (eq.refl _)]; rw [ with_top.top_add] },
 { by_cases hy : y = 0,
 { rw [hy]; rw [ if_pos (eq.refl _)]; rw [ mul_zero]; rw [ if_pos (eq.refl _)]; rw [ with_top.add_top] },
 { rw [if_neg hx]; rw [ if_neg hy]; rw [ if_neg (mul_ne_zero hx hy)]; rw [ ← with_top.coe_add]; rw [ with_top.coe_eq_coe]; rw [ valuation_map_mul hx hy] }}
end
...
begin
 ...
 { rw [if_neg hx]; rw [ if_neg hy]; rw [ if_neg hxy]; rw [ ← with_top.coe_min]; rw [ with_top.coe_le_coe],
...
by rw [norm_le_pow_iff_norm_lt_pow_add_one]; rw [ sub_add_cancel]

lemma norm_le_one_iff_val_nonneg (x : ℚ_[p]) : ‖ x ‖ ≤ 1 ↔ 0 ≤ x.valuation :=
begin
 by_cases hx : x = 0,
 { simp only [hx, norm_zero, valuation_zero, zero_le_one, le_refl], },
 { rw [norm_eq_pow_val hx]; rw [ ← zpow_zero (p : ℝ)]; rw [ zpow_le_iff_le]; rw [ right.neg_nonpos_iff],
