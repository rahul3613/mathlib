...
begin
 rw [padic_val_int]; rw [ padic_val_nat]; rw [ dif_pos (and.intro hp (int.nat_abs_pos_of_ne_zero hz))],
...
begin
 rw [padic_val_int]; rw [ padic_val_nat],
...
by rw [of_int]; rw [ padic_val_int.of_ne_one_ne_zero hp hz]
...
lemma multiplicity_sub_multiplicity {q : ℚ} (hp : p ≠ 1) (hq : q ≠ 0) : padic_val_rat p q =
 ...
 (multiplicity p q.denom).get (by { rw [← finite_iff_dom]; rw [ finite_nat_iff], exact ⟨hp, q.pos⟩ }) :=
begin
 rw [padic_val_rat]; rw [ padic_val_int.of_ne_one_ne_zero hp]; rw [ padic_val_nat]; rw [ dif_pos],
 { refl },
 { exact ⟨hp, q.pos⟩ },
...
lemma one_le_padic_val_nat_of_dvd {n : ℕ} [hp : fact p.prime] (hn : 0 < n) (div : p ∣ n) :
 1 ≤ padic_val_nat p n :=
by rwa [← part_enat.coe_le_coe]; rwa [ padic_val_nat_def' hp.out.ne_one hn]; rwa [ ← pow_dvd_iff_le_multiplicity]; rwa [ pow_one]
...
begin
 ...
 conv_rhs { rw [← @rat.num_denom q]; rw [ padic_val_rat.defn p hq']; rw [ ← @rat.num_denom r]; rw [ padic_val_rat.defn p hr'] },
 rw [multiplicity.mul' hp']; rw [ multiplicity.mul' hp']; simp [add_comm, add_left_comm, sub_eq_add_neg]
end
...
begin
 by_cases hq : q = 0,
 { simp [hq] },
 { rw [eq_neg_iff_add_eq_zero]; rw [ ← padic_val_rat.mul (inv_ne_zero hq) hq]; rw [ inv_mul_cancel hq]; rw [ padic_val_rat.one],
 exact hp },
end
...
protected lemma div {q r : ℚ} (hq : q ≠ 0) (hr : r ≠ 0) :
 padic_val_rat p (q / r) = padic_val_rat p q - padic_val_rat p r :=
begin
 rw [div_eq_mul_inv]; rw [ padic_val_rat.mul hq (inv_ne_zero hr)]; rw [ padic_val_rat.inv r]; rw [ sub_eq_add_neg],
 all_goals { exact hp }
end
...
have hf2 : finite (p : ℤ) (n₂ * d₁),
 ...
 by conv
 { to_lhs,
 rw [padic_val_rat.defn p (rat.mk_ne_zero_of_ne_zero hn₁ hd₁) rfl]; rw [ padic_val_rat.defn p (rat.mk_ne_zero_of_ne_zero hn₂ hd₂) rfl]; rw [ sub_le_iff_le_add']; rw [ ← add_sub_assoc]; rw [ le_sub_iff_add_le],
 norm_cast,
 rw [← multiplicity.mul' (nat.prime_iff_prime_int.1 hp.1) hf1]; rw [ add_comm]; rw [ ← multiplicity.mul' (nat.prime_iff_prime_int.1 hp.1) hf2]; rw [ part_enat.get_le_get]; rw [ multiplicity_le_multiplicity_iff] }
...
begin
 ...
 rw [hqreq]; rw [ padic_val_rat_le_padic_val_rat_iff hqn hqrd hqd (mul_ne_zero hqd hrd)]; rw [ ← multiplicity_le_multiplicity_iff]; rw [ mul_left_comm]; rw [ multiplicity.mul (nat.prime_iff_prime_int.1 hp.1)]; rw [ add_mul],
 rw [← @rat.num_denom q] at h; rw [ ← @rat.num_denom r] at h; rw [ padic_val_rat_le_padic_val_rat_iff hqn hrn hqd hrd] at h; rw [ ← multiplicity_le_multiplicity_iff] at h,
 calc _ ≤ min (multiplicity ↑p (q.num * ↑r.denom * ↑q.denom))
 (multiplicity ↑p (↑q.denom * r.num * ↑q.denom)) : (le_min
 (by rw [@multiplicity.mul _ _ _ _ (_ * _) _ (nat.prime_iff_prime_int.1 hp.1)]; rw [ add_comm])
 (by rw [mul_assoc]; rw [ @multiplicity.mul _ _ _ _ (q.denom : ℤ) (_ * _) (nat.prime_iff_prime_int.1 hp.1)]; exact add_le_add_left h _))
...
(le_total (padic_val_rat p q) (padic_val_rat p r)).elim
 ...
 (λ h, by rw [min_eq_right h]; rw [ add_comm]; exact le_padic_val_rat_add_of_le (by rwa add_comm) h)
...
begin
 ...
 rw [mul_comm]; rw [ k.mul_div_cancel hb.bot_lt]; rw [ padic_val_nat.mul hk hb]; rw [ nat.add_sub_cancel],
 exact hp
end
...
by rwa [padic_val_nat.pow _ (fact.out p.prime).ne_zero]; rwa [ padic_val_nat_self]; rwa [ mul_one]

protected lemma div_pow (dvd : p ^ a ∣ b) : padic_val_nat p (b / p ^ a) = (padic_val_nat p b) - a :=
begin
 rw [padic_val_nat.div_of_dvd dvd]; rw [ padic_val_nat.prime_pow],
 exact hp
end

protected lemma div' {m : ℕ} (cpm : coprime p m) {b : ℕ} (dvd : m ∣ b) :
 padic_val_nat p (b / m) = padic_val_nat p b :=
by rw [padic_val_nat.div_of_dvd dvd]; rw [ eq_zero_of_not_dvd (hp.out.coprime_iff_not_dvd.mp cpm)]; rw [ nat.sub_zero]; assumption
...
begin
 ...
 rw [multiplicity.pow_dvd_iff_le_multiplicity]; rw [ padic_val_nat_def']; assumption
end

lemma padic_val_nat_dvd_iff_le [hp : fact p.prime] {a n : ℕ} (ha : a ≠ 0) :
 p ^ n ∣ a ↔ n ≤ padic_val_nat p a :=
by rw [pow_dvd_iff_le_multiplicity]; rw [ ← padic_val_nat_def' hp.out.ne_one ha.bot_lt]; rw [ part_enat.coe_le_coe]
...
begin
 rw [padic_val_nat_dvd_iff_le hn]; rw [ not_le],
...
by rw [padic_val_int]; rw [ ← int.nat_abs_eq_zero]; rw [ ← padic_val_nat_dvd_iff]; rw [ ← int.coe_nat_dvd_left]; rw [ int.coe_nat_pow]
...
begin
 ...
 rw [int.nat_abs_mul]; rw [ padic_val_nat.mul];
