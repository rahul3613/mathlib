...
begin
 ...
 dec_trivial,
 rw [← int.mod_mod_of_dvd n (by norm_num : (2 : ℤ) ∣ 4)]; rw [ ← zmod.int_cast_mod n 4],
...
by { rw [χ₄_nat_mod_four]; rw [ hn], refl }
...
by { rw [χ₄_int_mod_four]; rw [ hn], refl }
...
lemma _root_.neg_one_pow_div_two_of_one_mod_four {n : ℕ} (hn : n % 4 = 1) :
 (-1 : ℤ) ^ (n / 2) = 1 :=
by { rw [← χ₄_eq_neg_one_pow (nat.odd_of_mod_four_eq_one hn)]; rw [ ← nat_cast_mod]; rw [ hn], refl }

/-- If `n % 4 = 3`, then `(-1)^(n/2) = -1`. -/
lemma _root_.neg_one_pow_div_two_of_three_mod_four {n : ℕ} (hn : n % 4 = 3) :
 (-1 : ℤ) ^ (n / 2) = -1 :=
by { rw [← χ₄_eq_neg_one_pow (nat.odd_of_mod_four_eq_three hn)]; rw [ ← nat_cast_mod]; rw [ hn], refl }
...
begin
 ...
 dec_trivial,
 rw [← int.mod_mod_of_dvd n (by norm_num : (2 : ℤ) ∣ 8)]; rw [ ← zmod.int_cast_mod n 8],
