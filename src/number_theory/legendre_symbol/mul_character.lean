...
begin
 ...
 { exact h ha.unit, },
 { rw [map_nonunit χ ha]; rw [ map_nonunit χ' ha], },
end
...
{ to_fun := to_unit_hom,
 inv_fun := of_unit_hom,
 left_inv :=
 by { intro χ, ext x, rw [of_unit_hom_coe]; rw [ coe_to_unit_hom] },
 right_inv :=
 by { intro f, ext x, rw [coe_to_unit_hom]; rw [ of_unit_hom_coe], } }
...
lemma map_ring_char {R : Type u} [comm_ring R] [nontrivial R] (χ : mul_char R R') :
 χ (ring_char R) = 0 :=
by rw [ring_char.nat.cast_ring_char]; rw [ χ.map_zero]
...
begin
 ...
 rw [ring.mul_inverse_cancel _ h]; rw [ ← map_mul]; rw [ ring.mul_inverse_cancel _ ha]; rw [ mul_char.map_one], },
 { revert ha, nontriviality R, intro ha, -- `nontriviality R` by itself doesn't do it
 rw [map_nonunit _ ha]; rw [ ring.inverse_non_unit a ha]; rw [ mul_char.map_zero χ], },
end
...
begin
 ext x,
 rw [coe_to_fun_mul]; rw [ pi.mul_apply]; rw [ inv_apply_eq_inv]; rw [ ring.inverse_mul_cancel _ (is_unit.map _ x.is_unit)]; rw [ one_apply_coe],
end
...
begin
 ...
 { rw [pow_zero]; rw [ pow_zero]; rw [ one_apply_coe], },
 { rw [pow_succ]; rw [ pow_succ]; rw [ mul_apply]; rw [ ih], },
end
...
begin
 ...
 { rw [map_nonunit (χ ^ n) ha]; rw [ map_nonunit χ ha]; rw [ zero_pow hn], },
end
...
begin
 obtain ⟨a, ha⟩ := hχ,
 use a,
 rw [ring_hom_comp_apply]; rw [ ← ring_hom.map_one f],
 exact λ h, ha (hf h),
end
...
begin
 ...
 { rw [h₀]; rw [ ring.inverse_zero], },
 { rw [h₁]; rw [ ring.inverse_one], },
 { rw [h₂]; rw [ (by norm_cast : (-1 : R') = (-1 : R'ˣ))]; rw [ ring.inverse_unit (-1 : R'ˣ)],
 refl, },
end
