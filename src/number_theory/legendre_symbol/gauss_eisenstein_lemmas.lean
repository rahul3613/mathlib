...
begin
 ...
 split_ifs,
 { erw [mul_div_cancel' _ hap]; erw [ val_min_abs_def_pos]; erw [ val_cast_of_lt (hep hb)]; erw [ if_pos (le_of_lt_succ (mem_Ico.1 hb).2)]; erw [ int.nat_abs_of_nat], },
 { erw [mul_neg]; erw [ mul_div_cancel' _ hap]; erw [ nat_abs_val_min_abs_neg]; erw [ val_min_abs_def_pos]; erw [ val_cast_of_lt (hep hb)]; erw [ if_pos (le_of_lt_succ (mem_Ico.1 hb).2)]; erw [ int.nat_abs_of_nat] } } },
...
calc (a ^ (p / 2) * (p / 2)! : zmod p) =
 ...
 by rw [prod_mul_distrib]; rw [ ← prod_nat_cast]; rw [ prod_Ico_id_eq_factorial]; rw [ prod_const]; rw [ card_Ico]; rw [ succ_sub_one]; simp
...
... = (-1)^((Ico 1 (p / 2).succ).filter
 ...
 by rw [prod_mul_distrib]; rw [ this]; rw [ prod_const]
... = (-1)^((Ico 1 (p / 2).succ).filter
 (λ x : ℕ, ¬(a * x : zmod p).val ≤ p / 2)).card * (p / 2)! :
 by rw [← prod_nat_cast]; rw [ finset.prod_eq_multiset_prod]; rw [ Ico_map_val_min_abs_nat_abs_eq_Ico_map_id p a hap]; rw [ ← finset.prod_eq_multiset_prod]; rw [ prod_Ico_id_eq_factorial]
...
(mul_left_inj'
 (show ((p / 2)! : zmod p) ≠ 0,
 by rw [ne.def]; rw [ char_p.cast_eq_zero_iff (zmod p) p]; rw [ hp.1.dvd_factorial]; rw [ not_le];
...
begin
 ...
 by { rw [legendre_sym.eq_pow]; rw [ gauss_lemma_aux p ha0]; simp },
...
... = _ : congr_arg2 (+)
 ...
 ... = _ : by rw [finset.sum_eq_multiset_sum]; rw [ Ico_map_val_min_abs_nat_abs_eq_Ico_map_id p a hap]; rw [ ← finset.sum_eq_multiset_sum];
...
begin
 ...
 rw [neg_one_pow_eq_pow_mod_two]; rw [ gauss_lemma hp ha0']; rw [ neg_one_pow_eq_pow_mod_two]; rw [ (by norm_cast : ((a : ℤ) : zmod p) = (a : zmod p))]; rw [ show _ = _]; rw [ from eisenstein_lemma_aux p ha1 ha0]
end
