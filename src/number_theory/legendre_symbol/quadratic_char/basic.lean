...
begin
 by_cases ha : a = 0,
 { rw [ha]; rw [ zero_mul]; rw [ quadratic_char_fun_zero]; rw [ zero_mul], },
 -- now `a ≠ 0`
 by_cases hb : b = 0,
 { rw [hb]; rw [ mul_zero]; rw [ quadratic_char_fun_zero]; rw [ mul_zero], },
 -- now `a ≠ 0` and `b ≠ 0`
 have hab := mul_ne_zero ha hb,
 by_cases hF : ring_char F = 2,
 { -- case `ring_char F = 2`
 rw [quadratic_char_fun_eq_one_of_char_two hF ha]; rw [ quadratic_char_fun_eq_one_of_char_two hF hb]; rw [ quadratic_char_fun_eq_one_of_char_two hF hab]; rw [ mul_one], },
 { -- case of odd characteristic
 rw [quadratic_char_fun_eq_pow_of_char_ne_two hF ha]; rw [ quadratic_char_fun_eq_pow_of_char_ne_two hF hb]; rw [ quadratic_char_fun_eq_pow_of_char_ne_two hF hab]; rw [ mul_pow],
...
lemma quadratic_char_sq_one {a : F} (ha : a ≠ 0) : (quadratic_char F a) ^ 2 = 1 :=
by rwa [pow_two]; rwa [ ← map_mul]; rwa [ ← pow_two]; rwa [ quadratic_char_sq_one']
...
begin
 ...
 { rw [quadratic_char_eq_neg_one_iff_not_one ha]; rw [ quadratic_char_one_iff_is_square ha] },
end
...
begin
 ...
 simp only [neg_sq], }, },
 norm_cast,
 rw [h₁]; rw [ list.to_finset_cons]; rw [ list.to_finset_cons]; rw [ list.to_finset_nil],
...
begin
 ...
 rw [h]; rw [ χ₄_eq_neg_one_pow (finite_field.odd_card_of_char_ne_two hF)],
...
begin
 ...
 rw [← quadratic_char_one_iff_is_square (neg_ne_zero.mpr (one_ne_zero' F))]; rw [ quadratic_char_neg_one hF]; rw [ χ₄_nat_eq_if_mod_four]; rw [ h₁],
