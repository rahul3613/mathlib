...
begin
 ...
 rw [← quadratic_char_one_iff_is_square (ring.two_ne_zero hF)]; rw [ quadratic_char_two hF]; rw [ χ₈_nat_eq_if_mod_eight],
...
lemma quadratic_char_neg_two [decidable_eq F] (hF : ring_char F ≠ 2) :
 quadratic_char F (-2) = χ₈' (fintype.card F) :=
begin
 rw [(by norm_num : (-2 : F) = (-1) * 2)]; rw [ map_mul]; rw [ χ₈'_eq_χ₄_mul_χ₈]; rw [ quadratic_char_neg_one hF]; rw [ quadratic_char_two hF]; rw [ @cast_nat_cast _ (zmod 4) _ _ _ (by norm_num : 4 ∣ 8)],
end
...
begin
 ...
 rotate, focus
 { have h := finite_field.odd_card_of_char_ne_two hF,
 rw [← quadratic_char_one_iff_is_square (neg_ne_zero.mpr (ring.two_ne_zero hF))]; rw [ quadratic_char_neg_two hF]; rw [ χ₈'_nat_eq_if_mod_eight],
 simp only [h, nat.one_ne_zero, if_false, ite_eq_left_iff, ne.def, (dec_trivial : (-1 : ℤ) ≠ 1),
 imp_false, not_not], },
 all_goals
 { rw [← nat.mod_mod_of_dvd _ (by norm_num : 2 ∣ 8)] at h,
...
begin
 ...
 { rw [show (p : F) = 0]; rw [ by { rw ← hFp]; rw [ exact ring_char.nat.cast_ring_char }],
 ...
 conv {congr, to_lhs, congr, skip, rw [hc]; rw [ nat.cast_pow]; rw [ map_pow]; rw [ hchar]; rw [ map_ring_char], },
 simp only [zero_pow n.pos, mul_zero, zero_eq_neg, one_ne_zero, not_false_iff], },
 { rw [← iff.not_left (@quadratic_char_neg_one_iff_not_is_square F _ _ _ _)]; rw [ quadratic_char_odd_prime hF hp],
 exact hFp, },
end
