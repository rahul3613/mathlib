...
begin
 rw [finite_field.is_square_two_iff]; rw [ card p],
...
begin
 rw [finite_field.is_square_neg_two_iff]; rw [ card p],
 have h₁ := prime.mod_two_eq_one_iff_ne_two.mpr hp,
 rw [← mod_mod_of_dvd p (by norm_num : 2 ∣ 8)] at h₁,
 have h₂ := mod_lt p (by norm_num : 0 < 8),
 revert h₂ h₁,
...
begin
 ...
 rw [legendre_sym]; rw [ legendre_sym]; rw [ nc]; rw [ nc]; rw [ h]; rw [ map_mul]; rw [ mul_rotate']; rw [ mul_comm (p / 2)]; rw [ ← pow_two]; rw [ quadratic_char_sq_one (prime_ne_zero q p hpq.symm)]; rw [ mul_one]; rw [ pow_mul]; rw [ χ₄_eq_neg_one_pow hp₁]; rw [ nc']; rw [ map_pow]; rw [ quadratic_char_neg_one hq₂]; rw [ card q]; rw [ χ₄_eq_neg_one_pow hq₁],
end
...
by rw [quadratic_reciprocity' (prime.mod_two_eq_one_iff_ne_two.mp (odd_of_mod_four_eq_one hp)) hq]; rw [ pow_mul]; rw [ neg_one_pow_div_two_of_one_mod_four hp]; rw [ one_pow]; rw [ one_mul]
...
let nop := @neg_one_pow_div_two_of_three_mod_four in begin
 ...
 rwa [← prime.mod_two_eq_one_iff_ne_two]; rwa [ odd_of_mod_four_eq_three],
end
...
begin
 cases eq_or_ne p q with h h,
 { substI p },
 { rw [← eq_one_iff' p (prime_ne_zero p q h)]; rw [ ← eq_one_iff' q (prime_ne_zero q p h.symm)]; rw [ quadratic_reciprocity_one_mod_four hp1 hq1], }
end
...
lemma exists_sq_eq_prime_iff_of_mod_four_eq_three (hp3 : p % 4 = 3) (hq3 : q % 4 = 3)
 (hpq : p ≠ q) :
 is_square (q : zmod p) ↔ ¬ is_square (p : zmod q) :=
by rw [← eq_one_iff' p (prime_ne_zero p q hpq)]; rw [ ← eq_neg_one_iff' q]; rw [ quadratic_reciprocity_three_mod_four hp3 hq3]; rw [ neg_inj]
