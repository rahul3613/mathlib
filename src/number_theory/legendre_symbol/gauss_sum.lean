...
begin
 rw [pow_two]; rw [ ← gauss_sum_mul_gauss_sum_eq_card hχ₁ hψ]; rw [ hχ₂.inv]; rw [ mul_rotate'],
 congr,
 rw [mul_comm]; rw [ ← gauss_sum_mul_shift _ _ (-1 : Rˣ)]; rw [ inv_mul_shift],
 refl,
end
...
begin
 rw [← frobenius_def]; rw [ gauss_sum]; rw [ gauss_sum]; rw [ map_sum],
...
by rw [gauss_sum_frob]; rw [ pow_mul_shift]; rw [ hχ.pow_char p]; rw [ ← gauss_sum_mul_shift χ ψ hp.unit]; rw [ ← mul_assoc]; rw [ hp.unit_spec]; rw [ ← pow_two]; rw [ ← pow_apply' _ (by norm_num : 0 < 2)]; rw [ hχ.sq_eq_one]; rw [ ← hp.unit_spec]; rw [ one_apply_coe]; rw [ one_mul]
...
begin
 ...
 { rw [pow_zero]; rw [ pow_one]; rw [ pow_zero]; rw [ mul_char.map_one]; rw [ one_mul], },
 { rw [pow_succ]; rw [ mul_comm p]; rw [ pow_mul]; rw [ ih]; rw [ mul_pow]; rw [ hχ.gauss_sum_frob _ hp]; rw [ ← mul_assoc]; rw [ pow_succ]; rw [ mul_comm (p : R)]; rw [ map_mul]; rw [ ← pow_apply' χ fp.1.pos (p ^ n)]; rw [ hχ.pow_char p], },
end
...
begin
 ...
 rw [← hχ.gauss_sum_frob_iter p n hp ψ]; rw [ ← pow_mul]; rw [ mul_comm]; rw [ ← pow_succ]; rw [ nat.two_mul_div_two_add_one_of_odd ((fp.1.eq_two_or_odd').resolve_left hp').pow],
end
...
begin
 ...
 rw [map_pow]; rw [ map_mul]; rw [ map_nat_cast]; rw [ hc']; rw [ hchar]; rw [ nat.cast_pow],
 simp only [← mul_char.ring_hom_comp_apply],
 haveI := fact.mk hp',
 haveI := fact.mk (hchar.subst hp'),
 rw [ne] at hch₁; rw [ ← nat.prime_dvd_prime_iff_eq hp' hp] at hch₁; rw [ ← is_unit_iff_not_dvd_char] at hch₁; rw [ hchar] at hch₁,
...
begin
 ...
 { rw [is_unit_iff_not_dvd_char]; rw [ ring_char_zmod_n],
 ...
 rw [card] at h; rw [ ← hchar] at h; rw [ hχ] at h; rw [ one_mul] at h; rw [ ← hc] at h; rw [ ← nat.cast_pow (ring_char F)] at h; rw [ ← hc] at h,

 -- finally, we change `2` to `8` on the left hand side
 convert_to (8 : F) ^ (fintype.card F / 2) = _,
 { rw [(by norm_num : (8 : F) = 2 ^ 2 * 2)]; rw [ mul_pow]; rw [ (finite_field.is_square_iff hF $ hp2 2).mp ⟨2, pow_two 2⟩]; rw [ one_mul] },
 apply (algebra_map F FF).injective,
