...
begin
 ...
 { rw [is_root.def]; rw [ ← map_cyclotomic_int k (zmod p)]; rw [ eval_map]; rw [ coe_cast_ring_hom]; rw [ ← int.cast_coe_nat]; rw [ ← int.coe_cast_ring_hom]; rw [ eval₂_hom]; rw [ int.coe_cast_ring_hom]; rw [ zmod.int_coe_zmod_eq_zero_iff_dvd _ _],
