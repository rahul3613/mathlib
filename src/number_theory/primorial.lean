...
begin
 ...
 rw [range_succ]; rw [ filter_insert]; rw [ if_neg (λ h, odd_iff_not_even.mp hn _)],
...
begin
 ...
 ... = (m + 1)# * choose (2 * m + 1) m : by rw [choose_symm_add]; rw [ two_mul]; rw [ add_right_comm]
 ... ≤ 4 ^ (m + 1) * 4 ^ m :
 mul_le_mul' (ihn _ $ succ_lt_succ $ (lt_add_iff_pos_left _).2 hm) (choose_middle_le_pow _)
 ... ≤ 4 ^ (m + m + 1) : by rw [← pow_add]; rw [ add_right_comm] },
