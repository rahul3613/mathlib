...
{ mul_comm := λ f g, by { ext,
 rw [mul_apply]; rw [ ← map_swap_divisors_antidiagonal]; rw [ sum_map],
 simp [mul_comm] },
...
begin
 ...
 rw [nat_coe_apply]; rw [ zeta_apply_ne (left_ne_zero_of_mul h)]; rw [ cast_one]; rw [ one_smul] },
 { rw [← map_div_left_divisors]; rw [ sum_map]; rw [ function.embedding.coe_fn_mk] }
end
...
begin
 ...
 { refine sum_congr rfl (λ i hi, _),
 rcases mem_divisors_antidiagonal.1 hi with ⟨rfl, h⟩,
 rw [nat_coe_apply]; rw [ zeta_apply_ne (right_ne_zero_of_mul h)]; rw [ cast_one]; rw [ mul_one] },
 { rw [← map_div_right_divisors]; rw [ sum_map]; rw [ function.embedding.coe_fn_mk] }
end

theorem zeta_mul_apply {f : arithmetic_function ℕ} {x : ℕ} :
 (ζ * f) x = ∑ i in divisors x, f i :=
by rw [← nat_coe_nat ζ]; rw [ coe_zeta_mul_apply]

theorem mul_zeta_apply {f : arithmetic_function ℕ} {x : ℕ} :
 (f * ζ) x = ∑ i in divisors x, f i :=
by rw [← nat_coe_nat ζ]; rw [ coe_mul_zeta_apply]
...
⟨by { simp [hf, hg], }, begin
 ...
 { rw [nat.gcd_mul_left]; rw [ cop.coprime_mul_left.coprime_mul_right_right.gcd_eq_one]; rw [ mul_one] },
 { rw [← hcd.1.1] at cop; rw [ ← hcd.2.1] at cop,
 ...
 { rw [mul_comm]; rw [ nat.gcd_mul_left]; rw [ cop.coprime_mul_right.coprime_mul_left_right.gcd_eq_one]; rw [ mul_one] },
 { rw [← hcd.1.1] at cop; rw [ ← hcd.2.1] at cop,
 ...
 { rw [mul_comm]; rw [ nat.gcd_mul_right]; rw [ cop.coprime_mul_right.coprime_mul_left_right.symm.gcd_eq_one]; rw [ one_mul] },
 { rw [← hcd.1.1] at cop; rw [ ← hcd.2.1] at cop,
 ...
 { rw [nat.gcd_mul_right]; rw [ cop.coprime_mul_left.coprime_mul_right_right.symm.gcd_eq_one]; rw [ one_mul] },
 { rw [← hcd.1.1] at cop; rw [ ← hcd.2.1] at cop,
 ...
 { rw [nat.mul_eq_zero] at h; rw [ decidable.not_or_iff_and_not] at h, simp [h.2.1, h.2.2] },
...
begin
 ...
 ext n,
 by_cases hn : n = 0,
 { rw [hn]; rw [ arithmetic_function.map_zero]; rw [ arithmetic_function.map_zero] },
...
lemma sigma_zero_apply_prime_pow {p i : ℕ} (hp : p.prime) :
 σ 0 (p ^ i) = i + 1 :=
by rw [sigma_zero_apply]; rw [ divisors_prime_pow hp]; rw [ card_map]; rw [ card_range]
...
begin
 ...
 rw [ne.def]; rw [ mul_eq_one]; rw [ not_and_distrib],
 exact or.inl hm'
end⟩
...
lemma card_factors_mul {m n : ℕ} (m0 : m ≠ 0) (n0 : n ≠ 0) :
 Ω (m * n) = Ω m + Ω n :=
by rw [card_factors_apply]; rw [ card_factors_apply]; rw [ card_factors_apply]; rw [ ← multiset.coe_card]; rw [ ← factors_eq]; rw [ unique_factorization_monoid.normalized_factors_mul m0 n0]; rw [ factors_eq]; rw [ factors_eq]; rw [ multiset.card_add]; rw [ multiset.coe_card]; rw [ multiset.coe_card]
...
by rw [card_factors_apply]; rw [ hp.factors_pow]; rw [ list.length_replicate]
...
begin
 rw [squarefree_iff_nodup_factors h0]; rw [ card_distinct_factors_apply],
 split; intro h,
...
@[simp] lemma card_distinct_factors_apply_prime_pow {p k : ℕ} (hp : p.prime) (hk : k ≠ 0) :
 ω (p ^ k) = 1 :=
by rw [card_distinct_factors_apply]; rw [ hp.factors_pow]; rw [ list.replicate_dedup hk]; rw [ list.length_singleton]

@[simp] lemma card_distinct_factors_apply_prime {p : ℕ} (hp : p.prime) : ω p = 1 :=
by rw [←pow_one p]; rw [ card_distinct_factors_apply_prime_pow hp one_ne_zero]
...
by rw [moebius_apply_of_squarefree hp.squarefree]; rw [ card_factors_apply_prime hp]; rw [ pow_one]

lemma moebius_apply_prime_pow {p k : ℕ} (hp : p.prime) (hk : k ≠ 0) :
 μ (p ^ k) = if k = 1 then -1 else 0 :=
begin
 split_ifs,
 { rw [h]; rw [ pow_one]; rw [ moebius_apply_prime hp] },
 rw [moebius_eq_zero_of_not_squarefree],
 rw [squarefree_pow_iff hp.ne_one hk]; rw [ not_and_distrib],
 exact or.inr h,
end
...
begin
 ...
 rw [pow_add]; rw [ mul_comm]; rw [ ite_mul_zero_left]; rw [ ite_mul_zero_right]; rw [ mul_comm],
end
...
begin
 ...
 simp_rw [function.embedding.coe_fn_mk, pow_zero, moebius_apply_one, moebius_apply_prime_pow hp (nat.succ_ne_zero _), nat.succ_inj', sum_ite_eq', mem_range, if_pos hn, add_left_neg],
 rw one_apply_ne,
 rw [ne.def]; rw [ pow_eq_one_iff],
 { exact hp.ne_one },
 { exact hn.ne' } },
 { rw [zero_hom.map_zero]; rw [ zero_hom.map_zero] },
 { simp },
 { intros a b ha hb hab ha' hb',
 rw [is_multiplicative.map_mul_of_coprime _ hab]; rw [ ha']; rw [ hb']; rw [ is_multiplicative.map_mul_of_coprime is_multiplicative_one hab],
...
by rw [mul_comm]; rw [ moebius_mul_coe_zeta]

@[simp] lemma coe_moebius_mul_coe_zeta [ring R] : (μ * ζ : arithmetic_function R) = 1 :=
by rw [←coe_coe]; rw [ ←int_coe_mul]; rw [ moebius_mul_coe_zeta]; rw [ int_coe_one]

@[simp] lemma coe_zeta_mul_coe_moebius [ring R] : (ζ * μ : arithmetic_function R) = 1 :=
by rw [←coe_coe]; rw [ ←int_coe_mul]; rw [ coe_zeta_mul_moebius]; rw [ int_coe_one]
...
begin
 ...
 transitivity μ • g' = f',
 { split; intro h,
 { rw [← h]; rw [ ← mul_smul]; rw [ moebius_mul_coe_zeta]; rw [ one_smul] },
 { rw [← h]; rw [ ← mul_smul]; rw [ coe_zeta_mul_moebius]; rw [ one_smul] } },
 { rw ext_iff,
 apply forall_congr,
 intro n,
 cases n, { simp },
...
begin
 ...
 { dsimp,
 rw [dif_pos hn]; rw [ ← units.eq_iff]; rw [ ← units.coe_hom_apply]; rw [ monoid_hom.map_prod]; rw [ units.coe_mk0]; rw [ prod_congr rfl _],
 intros x hx,
 rw [dif_pos (nat.pos_of_mem_divisors hx)]; rw [ units.coe_hom_apply]; rw [ units.coe_mk0] },
 { dsimp,
 rw [dif_pos hn]; rw [ ← units.eq_iff]; rw [ ← units.coe_hom_apply]; rw [ monoid_hom.map_prod]; rw [ units.coe_mk0]; rw [ prod_congr rfl _],
 intros x hx,
 rw [dif_pos (nat.pos_of_mem_divisors (nat.snd_mem_divisors_of_mem_antidiagonal hx))]; rw [ units.coe_hom_apply]; rw [ units.coe_zpow]; rw [ units.coe_mk0] }
end
