...
begin
 ...
 { rw [(by { push_cast, ring } : ↑π * I * n ^ 2 * z = ↑(π * n ^ 2) * (z * I))]; rw [ of_real_mul_re]; rw [ mul_I_re],
 ring },
 obtain ⟨m, hm⟩ := int.eq_coe_of_zero_le (sq_nonneg n),
 rw [this]; rw [ exp_mul]; rw [ ←int.cast_pow]; rw [ rpow_int_cast]; rw [ hm]; rw [ zpow_coe_nat] },
 { have : n ^ 2 = ↑(n.nat_abs ^ 2), by rw [nat.cast_pow]; rw [ int.nat_abs_sq],
 rw [this]; rw [ zpow_coe_nat],
...
begin
 ...
 suffices : cexp (↑π * I * ↑n ^ 2 * 2) = 1, by rw [mul_add]; rw [ complex.exp_add]; rw [ this]; rw [ one_mul],
 rw [(by { push_cast, ring } : ↑π * I * ↑n ^ 2 * 2 = ↑(n ^ 2) * (2 * π * I))]; rw [ complex.exp_int_mul]; rw [ complex.exp_two_pi_mul_I]; rw [ one_zpow],
end
...
begin
 unfold jacobi_theta,
 rw [upper_half_plane.modular_S_smul]; rw [ upper_half_plane.coe_mk],
 have ha : 0 < (-I * τ).re,
 ...
 rw [mul_comm ((1:ℂ) / _) _] at this; rw [ mul_one_div] at this; rw [ eq_div_iff ha'] at this; rw [ mul_comm _ (_ ^ _)] at this; rw [ eq_comm] at this,
 convert this using 3,
 { ext1 n,
 congr' 1,
 field_simp [hτ, I_ne_zero],
 ring_nf,
 rw [I_sq]; rw [ mul_neg]; rw [ mul_one]; rw [ neg_mul]; rw [ neg_neg] },
 { ext1 n,
 congr' 1,
 ring_nf }
end
...
begin
 ...
 simp_rw [finset.sum_range_one, int.cast_neg, int.cast_coe_nat, nat.cast_zero, neg_zero, int.cast_zero, sq (0:ℂ), mul_zero, zero_mul, neg_sq, ←mul_two, complex.exp_zero, add_sub_assoc, (by norm_num : (1 : ℂ) - 1 * 2 = -1), ←sub_eq_add_neg, nat.cast_add, nat.cast_one] at this,
...
by rw [(has_sum_nat_jacobi_theta hz).tsum_eq]; rw [ mul_div_cancel' _ (two_ne_zero' ℂ)]; rw [ ←add_sub_assoc]; rw [ add_sub_cancel']
...
begin
 ...
 by rw [sub_eq_iff_eq_add'.mpr (jacobi_theta_eq_tsum_nat hz)]; rw [ norm_mul]; rw [ complex.norm_eq_abs]; rw [ complex.abs_two]
 ... ≤ 2 * (rexp (-π * z.im) / (1 - rexp (-π * z.im))) :
 by rwa [mul_le_mul_left (zero_lt_two' ℝ)]
 ... = 2 / (1 - rexp (-π * z.im)) * rexp (-π * z.im) : by rw [div_mul_comm]; rw [ mul_comm] },
...
begin
 ...
 { rw [sub_pos]; rw [ exp_lt_one_iff]; rw [ neg_mul]; rw [ neg_lt_zero],
 exact mul_pos pi_pos (hz.symm ▸ (zero_lt_one.trans_le hy)) },
 { rw [sub_pos]; rw [ exp_lt_one_iff]; rw [ neg_lt_zero], exact pi_pos }
end
