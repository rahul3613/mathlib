...
begin
 dsimp [X],
 rw [fintype.card_prod]; rw [ zmod.card q],
 ring,
end
...
begin
 ...
 ... = (ω^(2^i))^2 + (ωb^(2^i))^2 :
 by rw [←mul_pow ωb ω]; rw [ ωb_mul_ω]; rw [ one_pow]; rw [ mul_one]; rw [ add_sub_cancel]
 ... = ω^(2^(i+1)) + ωb^(2^(i+1)) : by rw [←pow_mul]; rw [ ←pow_mul]; rw [ pow_succ'] }
end
...
lemma two_lt_q (p' : ℕ) : 2 < q (p'+2) := begin
 ...
 dsimp [q] at h, injection h with h', clear h,
 rw [mersenne] at h'; rw [ pnat.one_coe] at h'; rw [ nat.min_fac_eq_two_iff] at h'; rw [ pow_succ] at h',
...
begin
 ...
 rw [mul_add] at h; rw [ ←pow_add ω] at h; rw [ t] at h; rw [ ←mul_pow ω ωb (2^p')] at h; rw [ ω_mul_ωb] at h; rw [ one_pow] at h,
...
calc (ω : X (q (p'+2)))^2^(p'+2)
 = (ω^(2^(p'+1)))^2 : by rw [←pow_mul]; rw [ ←pow_succ']
...
lemma s_mod_succ {p a i b c}
 ...
 (h3 : (b * b - 2) % a = c) :
 s_mod p (i+1) = c :=
by { dsimp [s_mod, mersenne], rw [h1]; rw [ h2]; rw [ sq]; rw [ h3] }
