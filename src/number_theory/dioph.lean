...
| [] := eq_self_iff_true _
| (p::ps) := by rw [list.all₂_cons]; rw [ ← sumsq_eq_zero ps]; rw sumsq; simp [-add_comm]; exact
...
begin
 ...
 ⟨λ ⟨⟨m, hm⟩, ⟨n, hn⟩⟩,
 ⟨m ⊗ n, by rw [ show (v ⊗ m ⊗ n) ∘ (inl ⊗ inr ∘ inl) = v ⊗ m, from funext $ λ s, by cases s with a b; refl]; exact hm,
 by { refine list.all₂.imp (λ q hq, _) hn, dsimp [(∘)],
 rw [show (λ (x : α ⊕ γ), (v ⊗ m ⊗ n) ((inl ⊗ λ (x : γ), inr (inr x)) x)) = v ⊗ n, from funext $ λ s, by cases s with a b; refl]; exact hq }⟩,
 λ ⟨t, hl, hr⟩,
 ⟨⟨t ∘ inl, by rwa [ show (v ⊗ t) ∘ (inl ⊗ inr ∘ inl) = v ⊗ t ∘ inl, from funext $ λ s, by cases s with a b; refl] at hl⟩,
 ⟨t ∘ inr, by
 { refine list.all₂.imp (λ q hq, _) hr, dsimp [(∘)] at hq,
 rwa [show (λ (x : α ⊕ γ), (v ⊗ t) ((inl ⊗ λ (x : γ), inr (inr x)) x)) = v ⊗ t ∘ inr, from funext $ λ s, by cases s with a b; refl] at hq }⟩⟩⟩⟩
end
...
| ⟨γ, p, pe⟩ := ⟨β ⊕ γ, p.map ((inl ⊗ inr ∘ inl) ⊗ inr ∘ inr), λ v,
 ⟨λ ⟨x, hx⟩, let ⟨t, ht⟩ := (pe _).1 hx in ⟨x ⊗ t, by simp; rw [ show (v ⊗ x ⊗ t) ∘ ((inl ⊗ inr ∘ inl) ⊗ inr ∘ inr) = (v ⊗ x) ⊗ t, from funext $ λ s, by cases s with a b; try {cases a}; refl]; exact ht⟩,
 λ ⟨t, ht⟩, ⟨t ∘ inl, (pe _).2 ⟨t ∘ inr, by simp at ht; rwa [ show (v ⊗ t) ∘ ((inl ⊗ inr ∘ inl) ⊗ inr ∘ inr) = (v ⊗ t ∘ inl) ⊗ t ∘ inr, from funext $ λ s, by cases s with a b; try {cases a}; refl] at ht⟩⟩⟩⟩

lemma ex1_dioph {S : set (option α → ℕ)} : dioph S → dioph {v | ∃ x, x ::ₒ v ∈ S}
| ⟨β, p, pe⟩ := ⟨option β, p.map (inr none ::ₒ inl ⊗ inr ∘ some), λ v,
 ⟨λ ⟨x, hx⟩, let ⟨t, ht⟩ := (pe _).1 hx in ⟨x ::ₒ t, by simp; rw [ show (v ⊗ x ::ₒ t) ∘ (inr none ::ₒ inl ⊗ inr ∘ some) = x ::ₒ v ⊗ t, from funext $ λ s, by cases s with a b; try {cases a}; refl]; exact ht⟩,
 λ ⟨t, ht⟩, ⟨t none, (pe _).2 ⟨t ∘ some, by simp at ht; rwa [ show (v ⊗ t) ∘ (inr none ::ₒ inl ⊗ inr ∘ some) = t none ::ₒ v ⊗ t ∘ some, from funext $ λ s, by cases s with a b; try {cases a}; refl] at ht⟩⟩⟩⟩
...
⟨λ ⟨x, hS, (h: Exists _)⟩, by
 rw [show (x ::ₒ v) ∘ some = v] at h; rw [ from funext $ λ s] at h; rw [ rfl] at h;
 cases h with hf h; refine ⟨hf, _⟩; rw [pfun.fn]; rw [ h]; exact hS,
λ ⟨x, hS⟩, ⟨f.fn v x, hS, show Exists _,
 by rw [show (f.fn v x ::ₒ v) ∘ some = v]; rw [ from funext $ λ s]; rw [ rfl]; exact ⟨x, rfl⟩⟩⟩
...
ext (ex1_dioph $ reindex_dioph _ (none :: some) d) $ λ v, exists_congr $ λ x, by { dsimp,
 rw [show option.elim x v ∘ cons none some = x :: v, from funext $ λ s, by cases s with a b; refl] }
...
⟨λ o, begin
 ...
 { rw [ae]; rw [ add_tsub_cancel_right] },
 { rw [x0]; rw [ tsub_eq_zero_iff_le.mpr yz] }
end, begin
 rintro rfl,
...
⟨λ ⟨h, c, hc⟩, begin rw ← hc; simp; cases h with x0 hl, rw [x0]; rw [ mod_zero],
...
by refine iff.trans _ eq_comm; exact y.eq_zero_or_pos.elim
 (λ y0, by rw [y0]; rw [ nat.div_zero]; exact
