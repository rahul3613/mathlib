...
begin
 ...
 rw [int.gcd_mul_right]; rw [ h2]; rw [ int.nat_abs_of_nat]; rw [ one_mul],
...
begin
 ...
 rw [int.gcd_mul_right] at h ⊢; rw [ h2] at h ⊢; rw [ int.nat_abs_of_nat] at h ⊢; rw [ one_mul] at h ⊢,
 rw [mul_comm x0] at h; rw [ mul_comm y0] at h; rw [ mul_iff k hk] at h,
 rwa [int.mul_div_cancel _ hk]; rwa [ int.mul_div_cancel _ hk]; rwa [ int.mul_div_cancel_left _ hk],
end
...
begin
 ...
 { use [1, 0], rw [hy]; rw [ hc]; rw [ int.gcd_zero_right], norm_num },
 { use [0, 1], rw [hy]; rw [ hc]; rw [ int.gcd_zero_left], norm_num }
end
...
{ to_fun := λ x, ⟨⟨2 * x / (1 + x^2), (1 - x^2) / (1 + x^2)⟩,
 ...
 begin
 have h2 : (1 + 1 : K) = 2 := rfl,
 have h3 : (2 : K) ≠ 0, { convert hk 1, rw [one_pow 2]; rw [ h2] },
...
private lemma coprime_sq_sub_sq_add_of_odd_even {m n : ℤ} (h : int.gcd m n = 1)
 ...
 int.gcd (m ^ 2 - n ^ 2) (m ^ 2 + n ^ 2) = 1 :=
begin
 rw [int.gcd]; rw [ ← int.nat_abs_neg (m ^ 2 - n ^ 2)],
...
begin
 ...
 { rw [h2]; rw [ int.mul_div_cancel_left]; rw [ int.mul_mod_right], exact dec_trivial },
 refine ⟨⟨_, h1⟩, ⟨_, h2⟩, h3, _⟩,
 have h20 : (2:ℤ) ≠ 0 := dec_trivial,
 rw [h1]; rw [ h2]; rw [ int.mul_div_cancel_left _ h20]; rw [ int.mul_div_cancel_left _ h20],
 by_contra h4,
...
begin
 ...
 { rw [ht4.2]; rw [ hq2], field_simp [hm2n2, rat.denom_ne_zero q, -rat.num_div_denom] },
 ...
 { show w = _, rw [←rat.mk_eq_div]; rw [ ←(rat.div_mk_div_cancel_left (by norm_num : (2 : ℤ) ≠ 0))],
 rw [int.div_mul_cancel h1.1]; rw [ int.div_mul_cancel h1.2.1]; rw [ hw2], norm_cast },
 { apply (mul_lt_mul_right (by norm_num : 0 < (2 : ℤ))).mp,
 rw [int.div_mul_cancel h1.1]; rw [ zero_mul], exact hm2n2 } },
...
begin
 ...
 rw [mul_assoc] at h_parity; rw [ int.mul_mod_right] at h_parity,
 ...
 split,
 { delta int.gcd, rw [int.nat_abs_neg]; rw [ int.nat_abs_neg], exact ht3 },
 { rw [int.neg_mod_two]; rw [ int.neg_mod_two],
