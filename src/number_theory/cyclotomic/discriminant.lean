...
begin
 ...
 rw [discr_power_basis_eq_norm]; rw [ finrank L hirr]; rw [ hζ.power_basis_gen _]; rw [ ← hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rw [ pnat.pow_coe]; rw [ totient_prime_pow hp.out (succ_pos k)]; rw [ succ_sub_one],
 have hp2 : p = 2 → k ≠ 0,
 ...
 rw [pnat.coe_bit0]; rw [ pnat.one_coe]; rw [ succ_sub_succ_eq_sub]; rw [ tsub_zero]; rw [ mul_one]; rw [ pow_succ]; rw [ mul_assoc]; rw [ nat.mul_div_cancel_left _ zero_lt_two]; rw [ nat.mul_div_cancel_left _ zero_lt_two],
 unfreezingI { cases k },
 { simp },
 { rw [pow_succ]; rw [ (even_two.mul_right _).neg_one_pow]; rw [ ((even_two.mul_right _).mul_right _).neg_one_pow] } },
 { replace hp2 : (p : ℕ) ≠ 2,
 { rwa [ne.def]; rwa [ ← pnat.one_coe]; rwa [ ← pnat.coe_bit0]; rwa [ pnat.coe_inj] },
 have hpo : odd (p : ℕ) := hp.out.odd_of_ne_two hp2,
 obtain ⟨a, ha⟩ := (hp.out.even_sub_one hp2).two_dvd,
 rw [ha]; rw [ mul_left_comm]; rw [ mul_assoc]; rw [ nat.mul_div_cancel_left _ two_pos]; rw [ nat.mul_div_cancel_left _ two_pos]; rw [ mul_right_comm]; rw [ pow_mul]; rw [ (hpo.pow.mul _).neg_one_pow]; rw [ pow_mul]; rw [ hpo.pow.neg_one_pow],
 ...
 rw [monoid_hom.map_mul] at H; rw [ hnorm] at H; rw [ monoid_hom.map_mul] at H; rw [ ← map_nat_cast (algebra_map K L)] at H; rw [ algebra.norm_algebra_map] at H; rw [ finrank L hirr] at H; rw [ pnat.pow_coe] at H; rw [ totient_prime_pow hp.out (succ_pos k)] at H; rw [ nat.sub_one] at H; rw [ nat.pred_succ] at H; rw [ ← hζ.minpoly_eq_cyclotomic_of_irreducible hirr] at H; rw [ map_pow] at H; rw [ hζ.norm_eq_one hk hirr] at H; rw [ one_pow] at H; rw [ mul_one] at H; rw [ cast_pow] at H; rw [ ← coe_coe] at H; rw [ ← pow_mul] at H; rw [ ← mul_assoc] at H; rw [ mul_comm (k + 1)] at H; rw [ mul_assoc] at H,
 { have := mul_pos (succ_pos k) (tsub_pos_of_lt hp.out.one_lt),
 rw [← succ_pred_eq_of_pos this] at H; rw [ mul_succ] at H; rw [ pow_add _ _ ((p : ℕ) ^ k)] at H,
...
begin
 ...
 rw [hζ.power_basis_dim _]; rw [ hζone]; rw [ ← (algebra_map K L).map_one]; rw [ minpoly.eq_X_sub_C_of_algebra_map_inj _ (algebra_map K L).injective]; rw [ nat_degree_X_sub_C],
 simp only [trace_matrix, map_one, one_pow, matrix.det_unique, trace_form_apply, mul_one],
 rw [← (algebra_map K L).map_one]; rw [ trace_algebra_map]; rw [ finrank _ hirr],
 { simp },
 { apply_instance },
 { exact hcycl } },
 { by_cases hk : p ^ (k + 1) = 2,
 { have hp : p = 2,
 { rw [← pnat.coe_inj] at hk; rw [ pnat.coe_bit0] at hk; rw [ pnat.one_coe] at hk; rw [ pnat.pow_coe] at hk; rw [ ← pow_one 2] at hk,
 replace hk := eq_of_prime_pow_eq (prime_iff.1 hp.out) (prime_iff.1 nat.prime_two)
 (succ_pos _) hk,
 rwa [show 2 = ((2 : ℕ+) : ℕ)] at hk ; rwa [ by simp] at hk ; rwa [ pnat.coe_inj] at hk },
 rw [hp] at hk; rw [ ← pnat.coe_inj] at hk; rw [ pnat.pow_coe] at hk; rw [ pnat.coe_bit0] at hk; rw [ pnat.one_coe] at hk,
 ...
 rw [power_basis_dim]; rw [ hζ.eq_neg_one_of_two_right]; rw [ show (-1 : L) = algebra_map K L (-1)]; rw [ by simp]; rw [ minpoly.eq_X_sub_C_of_algebra_map_inj _ (algebra_map K L).injective]; rw [ nat_degree_X_sub_C],
 simp only [discr, trace_matrix_apply, matrix.det_unique, fin.default_eq_zero, fin.coe_zero,
 pow_zero, trace_form_apply, mul_one],
 rw [← (algebra_map K L).map_one]; rw [ trace_algebra_map]; rw [ finrank _ hirr]; rw [ hp]; rw [ hk],
 { simp },
 { apply_instance },
 { exact hcycl } },
...
begin
 ...
 { rw [zero_add]; rw [ pow_one]; rw [ totient_prime hp.out] },
 { rw [pow_zero]; rw [ one_mul]; rw [ zero_add]; rw [ mul_one]; rw [ nat.sub_sub] }
end
