...
begin
 rw [aeval_def]; rw [ ← eval_map]; rw [ ← is_root.def]; rw [ map_cyclotomic]; rw [ is_root_cyclotomic_iff],
 exact zeta_spec n A B
end

lemma zeta_is_root [is_domain B] [ne_zero ((n : ℕ) : B)] :
 is_root (cyclotomic n B) (zeta n A B) :=
by { convert aeval_zeta n A B, rw [is_root.def]; rw [ aeval_def]; rw [ eval₂_eq_eval_map]; rw [ map_cyclotomic] }
...
begin
 rw [power_basis_gen]; rw [ adjoin_singleton_eq_range_aeval]; rw [ alg_hom.mem_range],
 exact ⟨X + 1, by simp⟩
end
...
{ to_fun := λ x,
 ...
 refine ⟨x.1, _⟩,
 cases x,
 rwa [mem_primitive_roots n.pos]; rwa [ ←is_root_cyclotomic_iff]; rwa [ is_root.def]; rwa [ ←map_cyclotomic _ (algebra_map K C)]; rwa [ hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rwa [ ←eval₂_eq_eval_map]; rwa [ ←aeval_def]
 end,
 inv_fun := λ x,
 begin
 haveI := is_cyclotomic_extension.ne_zero' n K L,
 ...
 refine ⟨x.1, _⟩,
 cases x,
 rwa [aeval_def]; rwa [ eval₂_eq_eval_map]; rwa [ hζ.power_basis_gen K]; rwa [ ←hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rwa [ map_cyclotomic]; rwa [ ←is_root.def]; rwa [ is_root_cyclotomic_iff]; rwa [ ← mem_primitive_roots n.pos]
 end,
...
begin
 haveI := is_cyclotomic_extension.ne_zero' n K L,
 rw [((zeta_spec n K L).power_basis K).finrank]; rw [ is_primitive_root.power_basis_dim]; rw [ ←(zeta_spec n K L).minpoly_eq_cyclotomic_of_irreducible hirr]; rw [ nat_degree_cyclotomic]
end
...
by rw [hζ.eq_neg_one_of_two_right]; rw [ show -1 = algebra_map K L (-1)]; rw [ by simp]; rw [ algebra.norm_algebra_map]
...
begin
 haveI := is_cyclotomic_extension.ne_zero' n K L,
 by_cases h1 : n = 1,
 { rw [h1] at hζ; rw [ one_coe] at hζ; rw [ one_right_iff] at hζ,
 rw [hζ]; rw [ show 1 = algebra_map K L 1]; rw [ by simp]; rw [ algebra.norm_algebra_map]; rw [ one_pow] },
 { replace h1 : 2 ≤ n,
 { by_contra' h,
 exact h1 (pnat.eq_one_of_lt_two h) },
 rw [← hζ.power_basis_gen K]; rw [ power_basis.norm_gen_eq_coeff_zero_minpoly]; rw [ hζ.power_basis_gen K]; rw [ ← hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rw [ cyclotomic_coeff_zero _ h1]; rw [ mul_one]; rw [ hζ.power_basis_dim K]; rw [ ← hζ.minpoly_eq_cyclotomic_of_irreducible hirr]; rw [ nat_degree_cyclotomic],
...
begin
 ...
 rw [←(algebra_map K L).map_one ] at hz; rw [ algebra.norm_algebra_map] at hz; rw [ one_pow] at hz; rw [ map_pow] at hz; rw [ ←one_pow ↑n] at hz,
...
begin
 ...
 erw [is_cyclotomic_extension.finrank _ hirr]; erw [ totient_two]; erw [ pow_one],
...
begin
 ...
 rw [← neg_sub]; rw [ alg_hom.map_neg]; rw [ alg_hom.map_sub]; rw [ alg_hom.map_one]; rw [ neg_eq_neg_one_mul] },
 rw [prod_mul_distrib]; rw [ prod_const]; rw [ card_univ]; rw [ alg_hom.card]; rw [ is_cyclotomic_extension.finrank L hirr]; rw [ (totient_even h).neg_one_pow]; rw [ one_mul],
 ...
 rw [this]; rw [ cyclotomic']; rw [ ← cyclotomic_eq_prod_X_sub_primitive_roots (is_root_cyclotomic_iff.1 hz)]; rw [ ← map_cyclotomic_int]; rw [ _root_.map_int_cast]; rw [ ←int.cast_one]; rw [ eval_int_cast_map]; rw [ eq_int_cast]; rw [ int.cast_id]
end
...
begin
 ...
 rw [show ζ - 1 = ζ + (algebra_map K A (-1))]; rw [ by simp [sub_eq_add_neg], minpoly.add_algebra_map
...
begin
 ...
 rw [H] at this,
 exact this },
 rw [intermediate_field.adjoin_simple_to_subalgebra_of_integral (is_cyclotomic_extension.integral {p ^ (k + 1)} K L _)],
 ...
 rw [H]; rw [ coe_coe],
 congr,
 { rw [pnat.pow_coe]; rw [ nat.pow_min_fac]; rw [ hpri.1.min_fac_eq], exact nat.succ_ne_zero _ },
 have := finite_dimensional.finrank_mul_finrank K K⟮η⟯ L,
 rw [is_cyclotomic_extension.finrank L hirr] at this; rw [ is_cyclotomic_extension.finrank K⟮η⟯ hirr₁] at this; rw [ pnat.pow_coe] at this; rw [ pnat.pow_coe] at this; rw [ nat.totient_prime_pow hpri.out (k - s).succ_pos] at this; rw [ nat.totient_prime_pow hpri.out k.succ_pos] at this; rw [ mul_comm _ (↑p - 1)] at this; rw [ mul_assoc] at this; rw [ mul_comm (↑p ^ (k.succ - 1))] at this,
...
begin
 ...
 rw [← pnat.coe_inj] at h; rw [ pnat.coe_bit0] at h; rw [ pnat.one_coe] at h; rw [ pnat.pow_coe] at h; rw [ ← pow_one 2] at h,
 replace h := eq_of_prime_pow_eq (prime_iff.1 hpri.out) (prime_iff.1 nat.prime_two)
 ((k - s).succ_pos) h,
 rw [← pnat.one_coe] at h; rw [ ← pnat.coe_bit0] at h; rw [ pnat.coe_inj] at h,
 exact hodd h
end
...
begin
 ...
 rw [this.eq_neg_one_of_two_right]; rw [ H]; rw [ algebra.norm_algebra_map]; rw [ is_cyclotomic_extension.finrank L hirr]; rw [ pow_coe]; rw [ pnat.coe_bit0]; rw [ one_coe]; rw [ totient_prime_pow nat.prime_two (zero_lt_succ k)]; rw [ succ_sub_succ_eq_sub]; rw [ tsub_zero]; rw [ mul_one]
end
...
begin
 ...
 { have hp : p = 2,
 { rw [← pnat.coe_inj] at htwo; rw [ pnat.coe_bit0] at htwo; rw [ pnat.one_coe] at htwo; rw [ pnat.pow_coe] at htwo; rw [ ← pow_one 2] at htwo,
 replace htwo := eq_of_prime_pow_eq (prime_iff.1 hpri.out) (prime_iff.1 nat.prime_two)
 (succ_pos _) htwo,
 rwa [show 2 = ((2 : ℕ+) : ℕ)] at htwo ; rwa [ by simp] at htwo ; rwa [ pnat.coe_inj] at htwo },
 replace hs : s = k,
 { rw [hp] at htwo; rw [ ← pnat.coe_inj] at htwo; rw [ pnat.pow_coe] at htwo; rw [ pnat.coe_bit0] at htwo; rw [ pnat.one_coe] at htwo,
 ...
 rw [hk₁]; rw [ pow_succ]; rw [ pow_mul]; rw [ neg_eq_neg_one_mul]; rw [ mul_pow]; rw [ neg_one_sq]; rw [ one_mul]; rw [ ← pow_mul]; rw [ ← pow_succ] },
