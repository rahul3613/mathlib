...
begin
 ...
 rw [is_scalar_tower.to_alg_hom_apply] at hb; rw [ ← adjoin_image] at hb,
 refine adjoin_mono (λ y hy, _) hb,
 obtain ⟨b₁, ⟨⟨n, hn⟩, h₁⟩⟩ := hy,
 exact ⟨n, ⟨mem_union_left T hn.1, by rw [← h₁]; rw [ ← alg_hom.map_pow]; rw [ hn.2]; rw [ alg_hom.map_one]⟩⟩ } }
end
...
begin
 ...
 rwa [this] at h; rwa [ adjoin_union_eq_adjoin_adjoin] at h; rwa [ subalgebra.mem_restrict_scalars] at h
end
...
begin
 ...
 rwa [← is_primitive_root.coe_submonoid_class_iff]; rwa [ subtype.coe_mk] },
 { convert mem_top,
 rw [← adjoin_adjoin_coe_preimage]; rw [ preimage_set_of_eq],
 norm_cast }
end
...
begin
 ...
 { rw [mem_union] at hs; rw [ mem_singleton_iff] at hs,
 obtain hs|rfl := hs,
...
begin
 ...
 { rw [_root_.eq_top_iff]; rw [ ← ((iff_adjoin_eq_top _ A B).1 H).2],
 refine adjoin_mono (λ x hx, _),
 simp only [union_singleton, mem_insert_iff, mem_set_of_eq] at ⊢ hx,
...
begin
 classical,
 rw [module.finite_def]; rw [ ← top_to_submodule]; rw [ ← ((iff_adjoin_eq_top _ _ _).1 h).2],
...
begin
 ...
 rw [is_root.def]; rw [ ←map_cyclotomic n (algebra_map A B)]; rw [ eval_map]; rw [ ←aeval_def],
 exact hx.2 },
 ...
 rw [mem_root_set']; rw [ map_cyclotomic]; rw [ aeval_def]; rw [ ←eval_map]; rw [ map_cyclotomic]; rw [ ←is_root],
...
begin
 ...
 rw is_root_of_unity_iff n.pos,
 refine ⟨n, nat.mem_divisors_self n n.ne_zero, _⟩,
 rw [mem_root_set'] at hx; rw [ aeval_def] at hx; rw [ ←eval_map] at hx; rw [ map_cyclotomic] at hx; rw [ ←is_root] at hx,
 exact hx.2 },
 { simp only [mem_singleton_iff, exists_eq_left, mem_set_of_eq] at hx,
...
{ exists_prim_root := λ i hi,
 ...
 rwa [← is_primitive_root.coe_submonoid_class_iff]; rwa [ subtype.coe_mk]; rwa [ hi],
 end,
 adjoin_roots := λ x,
 begin
 ...
 rw [← subalgebra.coe_eq_one]; rw [ subalgebra.coe_pow]; rw [ set_like.coe_mk],
...
begin
 ...
 rw [(eq_cyclotomic_iff n.pos _).1 rfl]; rw [ ring_hom.map_one],
end
...
begin
 ...
 rw [← eval_map] at hζ; rw [ ← is_root.def] at hζ; rw [ map_cyclotomic] at hζ; rw [ is_root_cyclotomic_iff] at hζ,
 refine ⟨forall_eq.2 ⟨ζ, hζ⟩, _⟩,
 rw [←algebra.eq_top_iff]; rw [ ←splitting_field.adjoin_root_set]; rw [ eq_comm],
...
{ exists_prim_root := λ a han,
 ...
 { refine subset_adjoin _,
 simp only [mem_singleton_iff, exists_eq_left, mem_set_of_eq],
 rwa [← subalgebra.coe_eq_one]; rwa [ subalgebra.coe_pow]; rwa [ subtype.coe_mk] },
...
{ map_units := λ ⟨x, hx⟩, begin
 ...
 rw [set_like.coe_mk]; rw [ ← is_scalar_tower.algebra_map_apply]; rw [ ← is_scalar_tower.algebra_map_apply]; rw [ @is_scalar_tower.algebra_map_apply A K _ _ _ _ _ (_root_.cyclotomic_field.algebra n K) _ _ w]; rw [ ← ring_hom.map_mul]; rw [ hw]; rw [ ← is_scalar_tower.algebra_map_apply] },
 { rintro y z ⟨a, ha⟩ ⟨b, hb⟩,
 refine ⟨⟨a.1 * b.2 + b.1 * a.2, a.2 * b.2, mul_mem_non_zero_divisors.2 ⟨a.2.2, b.2.2⟩⟩, _⟩,
 rw [set_like.coe_mk]; rw [ ring_hom.map_mul]; rw [ add_mul]; rw [ ← mul_assoc]; rw [ ha]; rw [ mul_comm ((algebra_map _ _) ↑a.2)]; rw [ ← mul_assoc]; rw [ hb],
 simp only [map_add, map_mul] },
 { rintro y z ⟨a, ha⟩ ⟨b, hb⟩,
 refine ⟨⟨a.1 * b.1, a.2 * b.2, mul_mem_non_zero_divisors.2 ⟨a.2.2, b.2.2⟩⟩, _⟩,
 rw [set_like.coe_mk]; rw [ ring_hom.map_mul]; rw [ mul_comm ((algebra_map _ _) ↑a.2)]; rw [ mul_assoc]; rw [ ← mul_assoc z]; rw [ hb]; rw [ ← mul_comm ((algebra_map _ _) ↑a.2)]; rw [ ← mul_assoc]; rw [ ha],
 simp only [map_mul] }
 end,
...
begin
 rw [←is_cyclotomic_extension.adjoin_roots_cyclotomic_eq_adjoin_root_cyclotomic h]; rw [ is_cyclotomic_extension.adjoin_roots_cyclotomic_eq_adjoin_nth_roots h],
...
begin
 ...
 refine ⟨r, _⟩,
 haveI := h a ha,
 rwa [coe_aeval_eq_eval] at hr; rwa [ ← is_root.def] at hr; rwa [ is_root_cyclotomic_iff] at hr,
end
