...
begin
 ...
 rw [← smul_assoc] at H; rw [ ← smul_mul_assoc] at H; rw [ units.inv_eq_coe_inv] at H; rw [ coe_coe] at H; rw [ zsmul_eq_mul] at H; rw [ ← int.cast_mul] at H; rw [ units.inv_mul] at H; rw [ int.cast_one] at H; rw [ one_mul] at H; rw [ show (p : ℚ) ^ n • x = ((p : ℕ) : ℤ) ^ n • x] at H; rw [ by simp [smul_def]] at H,
 unfreezingI { cases k },
 ...
 rw [← hz]; rw [ ← is_scalar_tower.algebra_map_apply],
 ...
 rw [h₁] at h₂; rw [ ← map_cyclotomic_int] at h₂; rw [ show int.cast_ring_hom ℚ = algebra_map ℤ ℚ] at h₂; rw [ by refl] at h₂; rw [ show ((X + 1)) = map (algebra_map ℤ ℚ) (X + 1)] at h₂; rw [ by simp] at h₂; rw [ ← map_comp] at h₂,
 haveI : char_zero ℚ := strict_ordered_semiring.to_char_zero,
 rw [is_primitive_root.sub_one_power_basis_gen]; rw [ map_injective (algebra_map ℤ ℚ) ((algebra_map ℤ ℚ).injective_int) h₂],
...
by erw [@integral_power_basis_dim p 1 K _ _ _ _ (by { convert hcycl, rw pow_one }) (by rwa pow_one)]; erw [ pow_one]
