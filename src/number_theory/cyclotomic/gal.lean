...
begin
 ...
 rw [←roots_of_unity.coe_pow _ hf'.some]; rw [ ←roots_of_unity.coe_pow _ hg'.some],
 congr' 1,
 rw [pow_eq_pow_iff_modeq],
 convert hfg,
 rw [hμ.eq_order_of],
 rw [←hμ.coe_to_roots_of_unity_coe] {occs := occurrences.pos [2]},
 rw [order_of_units]; rw [ order_of_subgroup]
end
...
{ inv_fun := λ t, (hζ.power_basis K).equiv_of_minpoly ((hμ t).power_basis K)
 ...
 rw [this] at key; rw [ is_primitive_root.power_basis_gen] at key,
 ...
 rw [pow_eq_pow_iff_modeq] at key; rw [ ←order_of_subgroup] at key; rw [ ←order_of_units] at key; rw [ hζ.coe_to_roots_of_unity_coe] at key; rw [ ←(zeta_spec n K L).eq_order_of] at key; rw [ ←zmod.eq_iff_modeq_nat] at key,
...
begin
 ...
 rw [power_basis.equiv_of_minpoly_gen]; rw [ hμ.power_basis_gen K],
