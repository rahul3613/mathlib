...
begin
 ...
 { rw [mem_set_of_eq]; rw [ order_of_pow' b hm.ne']; rw [ hb]; rw [ nat.gcd_mul_left_left]; rw [ n.mul_div_cancel hm], },
...
begin
 simp_rw [approx_order_of, thickening_eq_bUnion_ball, ← image_smul, image_Union₂, image_smul, smul_ball'', smul_eq_mul, mem_set_of_eq],
...
begin
 ...
 rw ← hb at han hn,
 rw sq at han,
 rwa [(commute.all a b).order_of_mul_eq_right_of_forall_prime_mul_dvd (order_of_pos_iff.mp hn) (λ p hp hp', dvd_trans (mul_dvd_mul_right hp' $ order_of a) han)], },
 let f : {b : A | order_of b = n} → {b : A | order_of b = n} := λ b, ⟨a * b, han b.property⟩,
 have hf : surjective f,
 { rintros ⟨b, hb⟩,
 refine ⟨⟨a⁻¹ * b, _⟩, _⟩,
 { rw [mem_set_of_eq]; rw [ ← order_of_inv]; rw [ mul_inv_rev]; rw [ inv_inv]; rw [ mul_comm],
 apply han,
 simpa, },
...
begin
 ...
 simp_rw [comp_app],
 rw [le_eq_subset]; rw [ Sup_hom.set_image_to_fun]; rw [ hf]; rw [ image_comp],
