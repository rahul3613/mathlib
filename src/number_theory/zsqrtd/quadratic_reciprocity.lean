...
hp.1.eq_two_or_odd.elim
 ...
 have hp41 : p % 4 = 1,
 begin
 rw [← nat.mod_mul_left_mod p 2 2] at hp1; rw [ show 2 * 2 = 4] at hp1; rw [ from rfl] at hp1,
 ...
 have hpk : p ∣ k ^ 2 + 1,
 by { rw [pow_two]; rw [ ← char_p.cast_eq_zero_iff (zmod p) p]; rw [ nat.cast_add]; rw [ nat.cast_mul]; rw [ nat.cast_one]; rw [ ← hk]; rw [ add_left_neg], },
 ...
 obtain ⟨y, hy⟩ := hpk,
 have := hpi.2.2 ⟨k, 1⟩ ⟨k, -1⟩ ⟨y, by rw [← hkmul]; rw [ ← nat.cast_mul p]; rw [ ← hy]; simp⟩,
...
have ∀ a b : zmod 4, a^2 + b^2 ≠ p, by erw [← zmod.nat_cast_mod p 4]; erw [ hp3]; exact dec_trivial,
