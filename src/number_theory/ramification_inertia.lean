...
begin
 ...
 rw [dvd_iff_normalized_factors_le_normalized_factors (pow_ne_zero _ hP0) hp0]; rw [ normalized_factors_pow]; rw [ normalized_factors_irreducible hPirr]; rw [ normalize_eq]; rw [ multiset.nsmul_singleton]; rw [ ← multiset.le_count_iff_replicate_le],
...
by rw [is_dedekind_domain.ramification_idx_eq_normalized_factors_count hp0 hP hP0]; rw [ factors_eq_normalized_factors]
...
begin
 ...
 rwa [multiset.count_ne_zero]; rwa [ associated_iff_eq.mp P'_eq],
end
...
begin
 ...
 rw [inertia_deg]; rw [ dif_pos this],
 congr,
 ...
 rw [ideal.quotient.lift_mk]; rw [ ← ideal.quotient.algebra_map_eq]; rw [ ← is_scalar_tower.algebra_map_eq]; rw [ ← ideal.quotient.algebra_map_eq]; rw [ ← is_scalar_tower.algebra_map_apply]
end
...
begin
 ...
 { rw [linear_map.map_sum]; rw [ ← smul_zero a]; rw [ ← eq]; rw [ finset.smul_sum]; rw [ finset.sum_congr rfl],
 intros i hi,
 rw [linear_map.map_smul]; rw [ ← is_scalar_tower.algebra_map_smul K]; rw [ hg' i hi]; rw [ ← smul_assoc]; rw [ smul_eq_mul],
 apply_instance },
...
begin
 ...
 by rw [submodule.map_smul'']; rw [ submodule.map_top]; rw [ M.range_mkq]
 ... = ⊤ : by rw [ideal.smul_top_eq_map]; rw [ (submodule.map_mkq_eq_top M _).mpr hb'] },
 ...
 rw [smul_eq_mul] at ⊢ hx; rw [ mul_comm] at ⊢ hx; rw [ ← algebra.smul_def] at ⊢ hx,
 ...
 { rw [eq_comm]; rw [ submodule.restrict_scalars_eq_top_iff]; rw [ ideal.span_singleton_eq_top],
 ...
 by rw [ring_hom.map_det]; rw [ ring_hom.map_matrix_apply]
 ...
 ... = (-1 : R ⧸ p) ^ n : by rw [matrix.det_neg]; rw [ fintype.card_fin]; rw [ matrix.det_one]; rw [ mul_one]
 ... ≠ 0 : is_unit.ne_zero (is_unit_one.neg.pow _),
 { refine congr_arg matrix.det (matrix.ext (λ i j, _)),
 rw [map_sub]; rw [ ring_hom.map_matrix_apply]; rw [ map_one],
 refl },
 { refine congr_arg matrix.det (matrix.ext (λ i j, _)),
 rw [ideal.quotient.eq_zero_iff_mem.mpr (hA'p i j)]; rw [ zero_sub],
 refl } },
 -- And we conclude `L = span L {det A} ≤ span K b`, so `span K b` spans everything.
 { intros x hx,
 rw [submodule.restrict_scalars_mem] at hx; rw [ is_scalar_tower.algebra_map_apply R S L] at hx,
...
begin
 ...
 rw [← @submodule.restrict_scalars_mem R]; rw [ submodule.restrict_scalars_span R (R ⧸ p) ideal.quotient.mk_surjective]; rw [ b_eq_b']; rw [ set.range_comp]; rw [ ← submodule.map_span]
 at mem_span_b,
 obtain ⟨y, y_mem, y_eq⟩ := submodule.mem_map.mp mem_span_b,
 suffices : y + -(y - x) ∈ _, { simpa },
 rw [linear_map.restrict_scalars_apply] at y_eq; rw [ submodule.mkq_apply] at y_eq; rw [ submodule.mkq_apply] at y_eq; rw [ submodule.quotient.eq] at y_eq,
 ...
 this,
 { rw [quotient.algebra_map_eq]; rw [ ideal.mk_ker],
 exact hp.ne_top },
...
begin
 rw [← linear_map.ker_eq_bot]; rw [ linear_map.ker_eq_bot'],
 rintro ⟨x, hx⟩ hx0,
...
quotient.map' (λ (x : S), ⟨_, ideal.mem_map_of_mem _ (ideal.mul_mem_left _ x a_mem)⟩)
 ...
 ext,
 rw [pow_quot_succ_inclusion_apply_coe]; rw [ subtype.coe_mk]; rw [ submodule.coe_sub]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk]; rw [ _root_.map_mul]; rw [ map_sub]; rw [ sub_mul]
 end)
...
begin
 ...
 rw [submodule.quotient.quot_mk_eq_mk] at hz; rw [ ideal.quotient.mk_eq_mk] at hz; rw [ ideal.mem_quotient_iff_mem_sup] at hz; rw [ sup_eq_left.mpr Pe_le_Pi1] at hz,
 rw [pow_quot_succ_inclusion_apply_coe] at h; rw [ subtype.coe_mk] at h; rw [ submodule.quotient.quot_mk_eq_mk] at h; rw [ ideal.quotient.mk_eq_mk] at h; rw [ ← map_sub] at h; rw [ ideal.quotient.eq] at h; rw [ ← sub_mul] at h,
...
begin
 ...
 have Pe_le_Pi1 : P^e ≤ P^(i + 1) := ideal.pow_le_pow hi,
 rw [submodule.quotient.quot_mk_eq_mk] at hx; rw [ ideal.quotient.mk_eq_mk] at hx; rw [ ideal.mem_quotient_iff_mem_sup] at hx; rw [ sup_eq_left.mpr Pe_le_Pi] at hx,
 ...
 rw [pow_quot_succ_inclusion_apply_coe]; rw [ subtype.coe_mk]; rw [ ideal.quotient.mk_eq_mk]; rw [ map_add]; rw [ mul_comm y a]; rw [ sub_add_cancel']; rw [ map_neg] },
 letI := classical.dec_eq (ideal S),
 rw [sup_eq_prod_inf_factors _ (pow_ne_zero _ hP0)]; rw [ normalized_factors_pow]; rw [ normalized_factors_irreducible ((ideal.prime_iff_is_prime hP0).mpr hP).irreducible]; rw [ normalize_eq]; rw [ multiset.nsmul_singleton]; rw [ multiset.inter_replicate]; rw [ multiset.prod_replicate],
 rw [← submodule.span_singleton_le_iff_mem] at a_mem a_not_mem; rw [ ideal.submodule_span_eq] at a_mem a_not_mem,
 rwa [ideal.count_normalized_factors_eq a_mem a_not_mem]; rwa [ min_eq_left i.le_succ],
 { intro ha,
...
begin
 letI : field (R ⧸ p) := ideal.quotient.field _,
 rw [rank_eq_of_injective _ (pow_quot_succ_inclusion_injective f p P i)]; rw [ (quotient_range_pow_quot_succ_inclusion_equiv f p P hP0 hi).symm.rank_eq],
...
begin
 ...
 { rw [rank_pow_quot_aux f p P _ lt_e]; rw [ ih]; rw [ ← succ_nsmul]; rw [ nat.sub_succ]; rw [ ← nat.succ_eq_add_one]; rw [ nat.succ_pred_eq_of_pos (nat.sub_pos_of_lt lt_e)],
 assumption },
...
begin
 ...
 rw [pow_zero] at this; rw [ nat.sub_zero] at this; rw [ ideal.one_eq_top] at this; rw [ ideal.map_top] at this,
...
begin
 rw [finrank_prime_pow_ramification_idx]; rw [ inertia_deg_algebra_map],
...
begin
 ...
 rw [pos_iff_ne_zero]; rw [ factors.finrank_pow_ramification_idx],
...
begin
 ...
 { rw [is_scalar_tower.algebra_map_eq R K L]; rw [ ring_hom.coe_comp],
 ...
 rw [← ring_hom.coe_comp]; rw [ ← is_scalar_tower.algebra_map_eq],
 exact inj_RL },
 ...
 rwa [ne.def]; rwa [ ideal.map_eq_bot_iff_le_ker]; rwa [ (ring_hom.injective_iff_ker_eq_bot _).mp inj_RS]; rwa [ le_bot_iff] },
