...
begin
 have : 0 < (↑t * I).im, by rwa [of_real_mul_im]; rwa [ I_im]; rwa [ mul_one],
 ...
 rw [I_sq]; rw [ mul_neg_one]; rw [ ←of_real_neg]; rw [ of_real_re]; rw [ neg_mul]; rw [ neg_mul],
end
...
begin
 rw [jacobi_theta_eq_tsum_nat ((mul_I_im t).symm ▸ ht : 0 < (↑t * I).im)]; rw [ add_comm]; rw [ add_sub_cancel]; rw [ mul_div_cancel_left _ (two_ne_zero' ℂ)]; rw [ zeta_kernel₁],
 congr' 1 with n : 1,
 push_cast,
 rw [(by ring : ↑π * I * (n + 1) ^ 2 * (t * I) = I ^ 2 * π * t * (n + 1) ^ 2)]; rw [ I_sq]; rw [ neg_one_mul],
end
...
begin
 ...
 rw [integrable_on]; rw [ measure.restrict_restrict]; rw [ ←integrable_on],
...
begin
 ...
 rw [zeta_kernel₁_eq_jacobi_theta (one_div_pos.mpr $ zero_lt_one.trans hu)]; rw [ zeta_kernel₁_eq_jacobi_theta (zero_lt_one.trans hu)]; rw [ ←add_div]; rw [ ←mul_div_assoc]; rw [ add_sub]; rw [ sub_add_cancel]; rw [ sqrt_div zero_le_one]; rw [ sqrt_one]; rw [ one_div (sqrt _)]; rw [ of_real_inv]; rw [ ←one_div]; rw [ one_div_one_div]; rw [ mul_sub]; rw [ mul_one],
 congr' 2,
 let τ : upper_half_plane := ⟨u * I, (mul_I_im u).symm ▸ (zero_lt_one.trans hu)⟩,
 convert jacobi_theta_S_smul τ using 2,
 { rw [upper_half_plane.modular_S_smul]; rw [ upper_half_plane.coe_mk]; rw [ subtype.coe_mk]; rw [ ←neg_inv]; rw [ mul_inv]; rw [ inv_I]; rw [ mul_neg]; rw [ neg_neg]; rw [ one_div]; rw [ of_real_inv], },
 { rw [subtype.coe_mk]; rw [ mul_comm]; rw [ mul_assoc]; rw [ mul_neg]; rw [ I_mul_I]; rw [ neg_neg]; rw [ mul_one]; rw [ sqrt_eq_rpow]; rw [ of_real_cpow (zero_lt_one.trans hu).le],
 push_cast } },
 rcases lt_trichotomy 1 t with h | rfl | h,
 { exact aux h },
 { simp only [div_self, ne.def, one_ne_zero, not_false_iff, sqrt_one, of_real_one, one_mul], },
 { have := aux (show 1 < 1 / t, by rwa [lt_one_div (zero_lt_one' ℝ) ht]; rwa [ div_one]),
 rw one_div_one_div at this,
 rw [this]; rw [ ←mul_assoc]; rw [ ←of_real_mul]; rw [ ←sqrt_mul ht.le]; rw [ mul_one_div_cancel ht.ne']; rw [ sqrt_one]; rw [ of_real_one]; rw [ one_mul] },
end
...
begin
 ...
 convert tendsto_id,
 ext1 t,
 rw [function.comp_app]; rw [ mul_I_im]; rw [ of_real_re]; rw [ id.def] },
 ...
 { rw [function.comp_app]; rw [ ←zeta_kernel₁_eq_jacobi_theta ht] },
 { rw [function.comp_app]; rw [ mul_I_im]; rw [ of_real_re] }
end
...
begin
 ...
 rw [zeta_kernel₂]; rw [ pi.add_apply]; rw [ indicator_of_not_mem (not_mem_Ioc_of_gt ht)]; rw [ add_zero],
end
...
begin
 ...
 { refine eventually_of_mem self_mem_nhds_within (λ x hx, _),
 dsimp only,
 rw [function.comp_app]; rw [ mul_one_div]; rw [ one_div (↑(sqrt _))]; rw [ ←of_real_inv]; rw [ is_R_or_C.norm_of_real]; rw [ abs_inv]; rw [ abs_of_nonneg (sqrt_nonneg _)]; rw [ ←div_eq_mul_inv] },
end
...
begin
 ...
 rw [pi.add_apply]; rw [ zeta_kernel₂]; rw [ pi.add_apply]; rw [ indicator_of_mem h],
 ring },
 ...
 rw [is_R_or_C.norm_of_real]; rw [ abs_of_nonneg (sqrt_nonneg _)],
...
begin
 ...
 rw [tendsto_nhds_within_iff]; rw [ (by simp : 𝓝 (0 : ℂ) = 𝓝 (0 / 2))],
 ...
 rw show 𝓝 (-1 : ℂ) = 𝓝 (0 - 1 + 0), by rw [zero_sub]; rw [ add_zero],
 ...
 { -- The hard case: `s = 0`.
 rw [riemann_zeta]; rw [ ←(lim_eq_iff ⟨-1 / 2, c2⟩).mpr c2],
 ...
 rw is_o_iff_tendsto',
 { exact tendsto.congr (λ x, by rw [←one_div]; rw [ one_div_one_div]) nhds_within_le_nhds },
...
begin
 ...
 rw [riemann_zeta]; rw [ function.update_noteq this]; rw [ (show (-2) * ((n : ℂ) + 1) / 2 = -↑(n + 1), by { push_cast, ring })]; rw [ complex.Gamma_neg_nat_eq_zero]; rw [ div_zero],
end
...
begin
 ...
 rwa [(by push_cast : (-1 / 2 : ℂ) = (-1 / 2 : ℝ))]; rwa [ of_real_re]; rwa [ neg_div]; rwa [ ←sub_eq_add_neg]; rwa [ sub_pos]
end
...
begin
 ...
 rwa [of_real_mul_re]; rwa [ ←div_eq_inv_mul]; rwa [ div_lt_div_right (zero_lt_two' ℝ)] },
 rw [riemann_completed_zeta]; rw [ riemann_completed_zeta₀]; rw [ mellin_zeta_kernel₂_eq_of_lt_re this]; rw [ sub_add]; rw [ sub_sub]; rw [ ←add_sub],
...
begin
 rw [complex.Gamma_eq_integral hs]; rw [ Gamma_integral_eq_mellin],
 conv_rhs { congr, rw [←smul_eq_mul]; rw [ ←mellin_comp_mul_left _ _ pi_pos] },
 have : (1 / ((n : ℂ) + 1) ^ (2 * s)) = ↑(((n : ℝ) + 1) ^ (2 : ℝ)) ^ (-s),
 { rw [(by push_cast: ((n : ℂ) + 1) = ↑( (n : ℝ) + 1))]; rw [ (by push_cast : (2 * s) = (↑(2 : ℝ) * s))]; rw [ cpow_mul_of_real_nonneg]; rw [ one_div]; rw [ cpow_neg],
 rw [←nat.cast_succ],
 exact nat.cast_nonneg _ },
 conv_rhs { rw [this]; rw [ mul_comm]; rw [ ←smul_eq_mul] },
 rw [← mellin_comp_mul_right _ _ (show 0 < ((n : ℝ) + 1) ^ (2 : ℝ), by positivity)],
 refine set_integral_congr measurable_set_Ioi (λ t ht, _),
 simp_rw smul_eq_mul,
 congr' 3,
 conv_rhs { rw [←nat.cast_two]; rw [ rpow_nat_cast] },
 ring
end
...
begin
 ...
 (λ t ht, _) hm,
 simp_rw [tsum_mul_left, norm_smul, complex.norm_eq_abs ((t : ℂ) ^ _), abs_cpow_eq_rpow_re_of_pos ht, sub_re, one_re],
 rw [zeta_kernel₁]; rw [ ←of_real_tsum]; rw [ complex.norm_eq_abs]; rw [ complex.abs_of_nonneg],
...
begin
 rw [completed_zeta_eq_mellin_of_one_lt_re hs]; rw [ mellin_zeta_kernel₁_eq_tsum]; rw [ neg_div]; rw [ mul_div_cancel' _ (two_ne_zero' ℂ)],
 rw (show s / 2 = ↑(2⁻¹ : ℝ) * s, by { push_cast, rw mul_comm, refl }),
 rwa [of_real_mul_re]; rwa [ ←div_eq_inv_mul]; rwa [ div_lt_div_right (zero_lt_two' ℝ)]
end
...
begin
 ...
 rw [riemann_zeta]; rw [ function.update_noteq this]; rw [ completed_zeta_eq_tsum_of_one_lt_re hs]; rw [ ←mul_assoc]; rw [ neg_div]; rw [ cpow_neg]; rw [ mul_inv_cancel_left₀]; rw [ mul_div_cancel_left],
 { apply Gamma_ne_zero_of_re_pos,
 rw [←of_real_one]; rw [ ←of_real_bit0]; rw [ div_eq_mul_inv]; rw [ ←of_real_inv]; rw [ mul_comm]; rw [ of_real_mul_re],
 exact mul_pos (inv_pos_of_pos two_pos) (zero_lt_one.trans hs), },
 { rw [ne.def]; rw [ cpow_eq_zero_iff]; rw [ not_and_distrib]; rw [ ←ne.def]; rw [ of_real_ne_zero],
...
begin
 ...
 rw [tsum_eq_zero_add],
 { simp_rw [nat.cast_zero, zero_cpow hs', div_zero, zero_add, zeta_eq_tsum_one_div_nat_add_one_cpow hs, nat.cast_add, nat.cast_one] },
 { rw ←summable_norm_iff,
 simp_rw [norm_div, norm_one, complex.norm_eq_abs, ←of_real_nat_cast, abs_cpow_eq_rpow_re_of_nonneg (nat.cast_nonneg _) (zero_lt_one.trans hs).ne', summable_one_div_nat_rpow],
 assumption }
end
...
by simp only [zeta_eq_tsum_one_div_nat_cpow (by rwa [←of_real_nat_cast]; rwa [ of_real_re]; rwa [ ←nat.cast_one]; rwa [ nat.cast_lt] : 1 < re k), cpow_nat_cast]
...
begin
 ...
 { rw [←nat.cast_two]; rw [ ←nat.cast_mul]; rw [ zeta_nat_eq_tsum_of_gt_one],
 { push_cast },
 { refine (one_lt_two).trans_le _,
 conv_lhs { rw ←mul_one 2 },
 rwa [mul_le_mul_left (zero_lt_two' ℕ)]; rwa [ nat.one_le_iff_ne_zero] } },
 { push_cast }
end
...
begin
 ...
 { rw [←nat.cast_two]; rw [ zeta_nat_eq_tsum_of_gt_one one_lt_two]; rw [ of_real_tsum],
 push_cast },
 { push_cast }
end
...
begin
 ...
 { rw [←nat.cast_one]; rw [ ←nat.cast_bit0]; rw [ ←nat.cast_bit0]; rw [ zeta_nat_eq_tsum_of_gt_one (by norm_num : 1 < 4)]; rw [ of_real_tsum],
 push_cast },
 { push_cast }
end
...
begin
 ...
 simp_rw [rpow_neg_one, ←one_div, abs_neg, abs_one, div_one, one_smul, of_real_neg, of_real_one, div_neg, div_one, neg_sub] at this,
 conv_lhs { rw [riemann_completed_zeta₀]; rw [ sub_div]; rw [ ←this] },
 refine set_integral_congr measurable_set_Ioi (λ t ht, _),
 simp_rw [zeta_kernel₂_one_div ht, smul_eq_mul, ←mul_assoc, sqrt_eq_rpow, of_real_cpow (le_of_lt ht), ←cpow_add _ _ (of_real_ne_zero.mpr $ ne_of_gt ht)],
 congr' 2,
 push_cast,
 ring,
end
...
lemma riemann_completed_zeta_one_sub (s : ℂ) :
 riemann_completed_zeta (1 - s) = riemann_completed_zeta s :=
by simp_rw [riemann_completed_zeta, riemann_completed_zeta₀_one_sub, sub_add, (by abel : 1 - s - 1 = -s), (by abel : 1 - s = -(s - 1)), div_neg, neg_sub_neg]
...
begin
 ...
 have hs_ne : s ≠ 0, by { contrapose! hs, rw hs, exact ⟨0, by rw [nat.cast_zero]; rw [ neg_zero]⟩ },
 have h_sqrt : (sqrt π : ℂ) ≠ 0, from of_real_ne_zero.mpr (sqrt_ne_zero'.mpr pi_pos),
 have h_pow : (2 : ℂ) ^ (s - 1) ≠ 0,
 { rw [ne.def]; rw [ cpow_eq_zero_iff]; rw [ not_and_distrib], exact or.inl two_ne_zero },
 ...
 rw [riemann_zeta, function.update_noteq (by rwa [sub_ne_zero]; rwa [ ne_comm] : 1 - s ≠ 0),
 ...
 rwa [(by ring : 1 - (1 - s) / 2 = s / 2 + 1 / 2)] at this ; rwa [ inv_div] at this ; rwa [ div_eq_iff (of_real_ne_zero.mpr pi_pos.ne')] at this ; rwa [ mul_comm _ ↑π] at this ; rwa [ mul_div_assoc'] at this },
 rw [(by rw ←neg_sub : (2 : ℂ) ^ (1 - s) = 2 ^ -(s - 1))]; rw [ cpow_neg]; rw [ h_Ga_eq],
 suffices : (π : ℂ) ^ ((1 - s) / 2) = π ^ -s * sqrt π * π ^ (s / 2),
 { rw this, field_simp, ring_nf, rw [←of_real_pow]; rw [ sq_sqrt pi_pos.le], ring },
...
begin
 ...
 rw [mul_zero]; rw [ nat.cast_zero]; rw [ pow_zero]; rw [ one_mul]; rw [ zero_add]; rw [ neg_zero]; rw [ zero_add]; rw [ div_one]; rw [ bernoulli_one]; rw [ riemann_zeta_zero]; rw [ rat.cast_div]; rw [ rat.cast_neg]; rw [ rat.cast_one]; rw [ rat.cast_bit0]; rw [ rat.cast_one] },
 { -- k = 2 * (m + 1) : both sides "trivially" zero
 rw [nat.cast_mul]; rw [ ←neg_mul]; rw [ nat.cast_two]; rw [ nat.cast_succ]; rw [ riemann_zeta_neg_two_mul_nat_add_one]; rw [ bernoulli_eq_bernoulli'_of_ne_one],
 ...
 rotate,
 { intro n,
 rw [(by norm_cast : (2 * (m : ℂ) + 2) = ↑(2 * m + 2))]; rw [ ←int.cast_neg_nat_cast]; rw [ ←int.cast_coe_nat]; rw [ ne.def]; rw [ int.cast_inj],
 apply ne_of_gt,
 refine lt_of_le_of_lt (by norm_num : (-n : ℤ) ≤ 0) (by positivity) },
 { rw [(by norm_cast : (2 * (m : ℂ) + 2) = ↑(2 * m + 2))]; rw [ ne.def]; rw [ nat.cast_eq_one], norm_num },
 ...
 { rw [pow_mul]; rw [ neg_one_sq]; rw [ one_pow],
 convert complex.cos_nat_mul_two_pi t using 2, push_cast, ring },
 { rw [pow_add]; rw [ pow_one]; rw [ pow_mul]; rw [ neg_one_sq]; rw [ one_pow]; rw [ one_mul],
 ...
 { rw [(by { push_cast, ring } : (2 * m + 2 : ℂ) = ↑(2 * m + 1) + 1)]; rw [ complex.Gamma_nat_eq_factorial]; rw [ (by ring : 2 * (m + 1) = (2 * m + 1) + 1)]; rw [ nat.factorial_succ]; rw [ nat.cast_mul]; rw [ mul_comm],
 push_cast },
 rw [←div_div]; rw [ neg_one_mul],
 congr' 1,
 rw [div_eq_iff (Gamma_ne_zero_of_re_pos _)],
 swap, { rw [(by push_cast : 2 * (m : ℂ) + 2 = ↑(2 * (m : ℝ) + 2))]; rw [ of_real_re], positivity },
 simp_rw [of_real_mul, ←mul_assoc, of_real_rat_cast, mul_add, nat.add_assoc, mul_one, one_add_one_eq_two, mul_neg, neg_mul, neg_inj],
 conv_rhs { rw mul_comm },
 congr' 1,
 rw [of_real_pow]; rw [ of_real_neg]; rw [ of_real_one]; rw [ pow_add]; rw [ neg_one_sq]; rw [ mul_one],
 ...
 { rw [of_real_pow]; rw [ ←cpow_nat_cast]; rw [ ←cpow_neg]; rw [ of_real_bit0]; rw [ of_real_one],
 congr' 1,
 ...
 { rw [of_real_pow]; rw [ ←cpow_nat_cast]; rw [ ←cpow_neg]; rw [ nat.cast_add]; rw [ nat.cast_mul]; rw [ nat.cast_two] },
 rw (by { intros, ring } : ∀ (a b c d e : ℂ), a * b * c * d * e = (a * d) * (b * e) * c),
 rw [inv_mul_cancel (of_real_ne_zero.mpr $ pow_ne_zero _ pi_pos.ne')]; rw [ inv_mul_cancel (of_real_ne_zero.mpr $ pow_ne_zero _ two_ne_zero)]; rw [ one_mul]; rw [ one_mul] }
end
