...
ring_equiv.of_bijective (ideal.quotient.lift (I.map (algebra_map R R<x>))
 ...
 rw [ring_hom.comp_apply]; rw [ ideal.quotient.eq_zero_iff_mem]; rw [ this]; rw [ ← ideal.map_map],
...
begin
 ...
 rwa [ring_hom.mem_ker] at hu ; rwa [ ring_hom.comp_apply] at hu ; rwa [ ideal.quotient.eq_zero_iff_mem] at hu ; rwa [ ← ideal.mem_comap] at hu ; rwa [ comap_map_eq_map_adjoin_of_coprime_conductor hx h_alg] at hu },
 ...
 rw [ideal.map_sup] at hx; rw [ ring_hom.map_one] at hx,
...
(normalized_factors_equiv_of_quot_equiv
 ...
 by rwa [ne.def]; rwa [ map_eq_bot_iff_of_injective (no_zero_smul_divisors.algebra_map_injective R S)]; rwa [ ← ne.def])
...
by rw [normalized_factors_map_equiv_normalized_factors_min_poly_mk]; rw [ equiv.coe_trans]; rw [ function.comp_app]; rw [ multiplicity_normalized_factors_equiv_span_normalized_factors_symm_eq_multiplicity]; rw [ normalized_factors_equiv_of_quot_equiv_multiplicity_eq_multiplicity]
...
begin
 ...
 { rw [multiset.count_eq_zero.mpr hJ]; rw [ eq_comm]; rw [ multiset.count_eq_zero]; rw [ multiset.mem_map],
 ...
 rw [multiplicity_eq_count_normalized_factors]; rw [ multiplicity_eq_count_normalized_factors]; rw [ unique_factorization_monoid.normalize_normalized_factor _ hJ]; rw [ unique_factorization_monoid.normalize_normalized_factor]; rw [ part_enat.coe_inj]
 at this,
 refine this.trans _,
 ...
 rw [multiset.count_map_eq_count' (λ f, ((normalized_factors_map_equiv_normalized_factors_min_poly_mk hI hI' hx hx').symm f : ideal S))]; rw [ multiset.count_attach],
 ...
 { rwa [← bot_eq_zero]; rwa [ ne.def]; rwa [ map_eq_bot_iff_of_injective (no_zero_smul_divisors.algebra_map_injective R S)] },
end
...
begin
 ...
 { obtain ⟨y, hy⟩ := this,
 have h := normalized_factors_prod (show I.map (algebra_map R S) ≠ 0, by rwa [← bot_eq_zero]; rwa [ ne.def]; rwa [ map_eq_bot_iff_of_injective (no_zero_smul_divisors.algebra_map_injective R S)]),
 ...
 rw [multiset.attach_map_coe]; rw [ multiset.map_singleton]; rw [ subtype.coe_mk],
