...
begin
 ...
 by rw [← finset.prod_Ico_id_eq_factorial]; rw [ prod_nat_cast]
 ... = (∏ x : (zmod p)ˣ, x) : _
 ... = -1 : by simp_rw [← units.coe_hom_apply, ← (units.coe_hom (zmod p)).map_prod, prod_univ_units_id_eq_neg_one, units.coe_hom_apply, units.coe_neg, units.coe_one],
...
begin
 conv in (Ico 1 p) { rw [← succ_sub_one p]; rw [ succ_sub (fact.out p.prime).pos] },
 rw [← prod_nat_cast]; rw [ finset.prod_Ico_id_eq_factorial]; rw [ wilsons_lemma]
end
...
begin
 ...
 rw [←zmod.nat_coe_zmod_eq_zero_iff_dvd]; rw [ cast_add]; rw [ cast_one]; rw [ h]; rw [ add_left_neg],
end
