...
by rw [divisors]; rw [ proper_divisors]; rw [ Ico_succ_right_eq_insert_Ico (one_le_iff_ne_zero.2 h)]; rw [ finset.filter_insert]; rw [ if_pos (dvd_refl n)]

lemma cons_self_proper_divisors (h : n ≠ 0) :
 cons n (proper_divisors n) proper_divisors.not_self_mem = divisors n :=
by rw [cons_eq_insert]; rw [ insert_self_proper_divisors h]
...
begin
 ...
 split; intro h,
 { contrapose! h, simp [h], },
 { rw [nat.lt_add_one_iff]; rw [ nat.lt_add_one_iff],
...
by rw [proper_divisors]; rw [ Ico_self]; rw [ filter_empty]

lemma pos_of_mem_divisors {m : ℕ} (h : m ∈ n.divisors) : 0 < m :=
begin
 cases m,
 { rw [mem_divisors] at h; rw [ zero_dvd_iff] at h, cases h.2 h.1 },
 apply nat.succ_pos,
end
...
by rw [mem_proper_divisors]; rw [ and_iff_right (one_dvd _)]
...
by rw [mem_divisors_antidiagonal]; rw [ mem_divisors_antidiagonal]; rw [ mul_comm]; rw [ prod.swap]
...
begin
 rw [← coe_inj]; rw [ coe_map]; rw [ equiv.coe_to_embedding]; rw [ equiv.coe_prod_comm]; rw [ set.image_swap_eq_preimage_swap],
 ext,
...
begin
 rw [←map_swap_divisors_antidiagonal]; rw [ map_eq_image]; rw [ image_image],
...
begin
 ...
 rw [map_swap_divisors_antidiagonal]; rw [ ←map_div_right_divisors]; rw [ finset.map_map],
 refl,
end
...
begin
 ...
 { simp },
 { rw [← cons_self_proper_divisors hn]; rw [ finset.sum_cons]; rw [ add_comm] }
end
...
begin
 rw [perfect_iff_sum_proper_divisors h]; rw [ sum_divisors_eq_sum_proper_divisors_add_self]; rw [ two_mul],
 split; intro h,
 { rw h },
...
by rw [mem_divisors]; rw [ nat.dvd_prime_pow pp]; rw [ and_iff_left (ne_of_gt (pow_pos pp.pos k))]
...
lemma prime.proper_divisors {p : ℕ} (pp : p.prime) :
 proper_divisors p = {1} :=
by rw [← erase_insert proper_divisors.not_self_mem]; rw [ insert_self_proper_divisors pp.ne_zero]; rw [ pp.divisors]; rw [ pair_comm]; rw [ erase_insert (λ con, pp.ne_one (mem_singleton.1 con))]
...
begin
 cases n,
 { rw [proper_divisors_zero] at hsub; rw [ subset_empty] at hsub,
 simp [hsub] },
 classical,
 rw [← sum_sdiff hsub],
 intros h,
...
begin
 ...
 symmetry,
 rw [← mem_singleton]; rw [ eq_proper_divisors_of_subset_of_sum_eq_sum (singleton_subset_iff.2 (mem_proper_divisors.2 ⟨h, hlt⟩)) sum_singleton]; rw [ mem_proper_divisors],
...
by rw [← cons_self_proper_divisors h.ne_zero]; rw [ prod_cons]; rw [ h.prod_proper_divisors]

lemma proper_divisors_eq_singleton_one_iff_prime :
 n.proper_divisors = {1} ↔ n.prime :=
⟨λ h, begin
 have h1 := mem_singleton.2 rfl,
 rw [← h] at h1; rw [ mem_proper_divisors] at h1,
 refine nat.prime_def_lt''.mpr ⟨h1.2, λ m hdvd, _⟩,
 rw [← mem_singleton]; rw [ ← h]; rw [ mem_proper_divisors],
...
begin
 rw [mem_proper_divisors]; rw [ nat.dvd_prime_pow pp]; rw [ ← exists_and_distrib_right],
...
begin
 rw [←map_swap_divisors_antidiagonal]; rw [ finset.prod_map],
