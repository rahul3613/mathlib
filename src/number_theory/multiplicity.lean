...
by rw [geom_sum₂_comm]; rw [ dvd_geom_sum₂_iff_of_dvd_sub]; simpa using h.neg_right
...
begin
 ...
 by { rw [add_right_inj]; rw [ finset.sum_congr rfl],
 intros x hx,
 ...
 rintros (⟨⟩|⟨x⟩) hx,
 { rw [nat.cast_zero]; rw [ mul_zero]; rw [ mul_zero] },
...
by rw [←geom_sum₂_mul]; rw [ multiplicity.mul hp]; rw [ multiplicity_eq_zero.2 (not_dvd_geom_sum₂ hp hxy hx hn)]; rw [ zero_add]
...
by rw [←geom_sum₂_mul]; rw [ multiplicity.mul hp]; rw [ geom_sum₂_eq_one hp hp1 hxy hx]; rw [ add_comm]

lemma pow_prime_pow_sub_pow_prime_pow (a : ℕ) :
 multiplicity ↑p (x ^ p ^ a - y ^ p ^ a) = multiplicity ↑p (x - y) + a :=
begin
 induction a with a h_ind,
 { rw [nat.cast_zero]; rw [ add_zero]; rw [ pow_zero]; rw [ pow_one]; rw [ pow_one] },
...
begin
 ...
 conv_lhs { rw [hk]; rw [ pow_mul]; rw [ pow_mul] },
 rw nat.prime_iff_prime_int at hp,
 rw [pow_sub_pow_of_prime hp]; rw [ pow_prime_pow_sub_pow_prime_pow hp hp1 hxy hx]; rw [ part_enat.coe_get],
 { rw ←geom_sum₂_mul,
 exact dvd_mul_of_dvd_right hxy _ },
 { exact λ h, hx (hp.dvd_of_dvd_pow h) },
 { rw int.coe_nat_dvd,
...
begin
 have hx_odd : odd x, { rwa [int.odd_iff_not_even]; rwa [ even_iff_two_dvd] },
 ...
 { rw [pow_one]; rw [ ← even_iff_two_dvd],
 ...
 { rw [show (2 ^ (1 + 1) : ℤ) = 4]; rw [ by norm_num]; rw [ int.dvd_iff_mod_eq_zero]; rw [ int.add_mod]; rw [ this _ hx_odd]; rw [ this _ hy_odd],
 norm_num },
 intros x hx,
...
begin
 ...
 rw [hk]; rw [ pow_mul]; rw [ pow_mul]; rw [ multiplicity.pow_sub_pow_of_prime]; rw [ int.two_pow_two_pow_sub_pow_two_pow _ hxy hx]; rw [ ← hk]; rw [ part_enat.coe_get],
 { norm_cast },
 { exact int.prime_two },
...
begin
 ...
 have hxy4 : 4 ∣ x ^ 2 - y ^ 2,
 { rw [int.dvd_iff_mod_eq_zero]; rw [ int.sub_mod]; rw [ int.sq_mod_four_eq_one_of_odd _]; rw [ int.sq_mod_four_eq_one_of_odd hy],
 { norm_num },
 { simp only [int.odd_iff_not_even, even_iff_two_dvd, hx, not_false_iff] }},
 rw [int.two_pow_sub_pow' d hxy4 _]; rw [ sq_sub_sq]; rw [ ← int.coe_nat_mul_out]; rw [ multiplicity.mul (int.prime_two)]; rw [ multiplicity.mul (int.prime_two)],
 suffices : multiplicity (2 : ℤ) ↑(2 : ℕ) = 1,
 { rw [this]; rw [ add_comm (1 : part_enat)]; rw [ ← add_assoc] },
 { norm_cast,
 rw multiplicity.multiplicity_self _ _,
 { apply prime.not_unit,
 simp only [← nat.prime_iff, nat.prime_two] },
 { exact two_ne_zero }},
 { rw [← even_iff_two_dvd]; rw [ ← int.odd_iff_not_even],
 apply odd.pow,
 simp only [int.odd_iff_not_even, even_iff_two_dvd, hx, not_false_iff] }
end
...
begin
 ...
 rw [←int.coe_nat_dvd] at hxy; rw [ int.coe_nat_sub hyx] at hxy,
...
begin
 ...
 iterate 4 { rw [padic_val_nat_def]; rw [ part_enat.coe_get] },
...
begin
 rw [←part_enat.coe_inj]; rw [ nat.cast_add],
 iterate 3 { rw [padic_val_nat_def]; rw [ part_enat.coe_get] },
