...
begin
 ...
 conv_rhs { rw [←nat.cast_one]; rw [ ←nat.cast_add]; rw [ ←C_eq_nat_cast]; rw [ C_mul_monomial]; rw [ mul_comm], },
 rw [mul_assoc]; rw [ mul_assoc]; rw [ ←nat.cast_mul]; rw [ ←nat.cast_mul],
 congr' 3,
 rw [(choose_mul_succ_eq k m).symm]; rw [ mul_comm],
end

lemma derivative_bernoulli (k : ℕ) : (bernoulli k).derivative = k * bernoulli (k - 1) :=
begin
 cases k,
 { rw [nat.cast_zero]; rw [ zero_mul]; rw [ bernoulli_zero]; rw [ derivative_one], },
...
begin
 ...
 { apply_congr, skip,
 rw [← nat.cast_mul]; rw [ choose_mul ((le_tsub_iff_left $ mem_range_le H).1 $ mem_range_le H_1) (le.intro rfl)]; rw [ nat.cast_mul]; rw [ add_comm x x_1]; rw [ add_tsub_cancel_right]; rw [ mul_assoc]; rw [ mul_comm]; rw [ ←smul_eq_mul]; rw [ ←smul_monomial] },
 rw [←sum_smul], },
...
by rw [nat.cast_succ]; rw [ ← sum_bernoulli n]; rw [ sum_range_succ]; rw [ add_sub_cancel']; rw [ choose_succ_self_right]; rw [ nat.cast_succ]
...
begin
 ...
 conv_lhs { congr, skip, apply_congr, skip, rw [eval_smul]; rw [ hd x_1 (mem_range.1 H)], },
...
begin
 ...
 rw [coeff_succ_X_mul]; rw [ coeff_rescale]; rw [ coeff_exp]; rw [ power_series.coeff_mul]; rw [ nat.sum_antidiagonal_eq_sum_range_succ_mk]; rw [ sum_range_succ],
 ...
 rw [mul_left_comm]; rw [ ←ring_hom.map_mul],
 change _ = t^n * algebra_map ℚ A (((n+1)*n! : ℕ)*(1/n!)),
 rw [cast_mul]; rw [ mul_assoc]; rw [ mul_one_div_cancel (show (n! : ℚ) ≠ 0, from cast_ne_zero.2 (factorial_ne_zero n))]; rw [ mul_one]; rw [ mul_comm (t^n)]; rw [ ← aeval_monomial]; rw [ cast_add]; rw [ cast_one],
 -- But this is the RHS of `sum_bernoulli_poly`
 rw [← sum_bernoulli]; rw [ finset.mul_sum]; rw [ alg_hom.map_sum],
 ...
 congr',
 apply congr_arg,
 rw [mul_assoc]; rw [ div_eq_mul_inv]; rw [ ← mul_inv],
end
