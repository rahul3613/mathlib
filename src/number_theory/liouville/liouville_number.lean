...
... ≤ 1 / m ^ (n! * n) :
 ...
 conv_rhs { rw [one_mul]; rw [ mul_add]; rw [ pow_add]; rw [ mul_one]; rw [ pow_mul]; rw [ mul_comm]; rw [ ← pow_mul] },
...
begin
 ...
 { exact ⟨1, by rw [partial_sum]; rw [ range_one]; rw [ sum_singleton]; rw [ nat.cast_one]⟩ },
 { rcases h with ⟨p_k, h_k⟩,
 use p_k * (m ^ ((k + 1)! - k!)) + 1,
 rw [partial_sum_succ]; rw [ h_k]; rw [ div_add_div]; rw [ div_eq_div_iff]; rw [ add_mul],
 { norm_cast,
...
begin
 ...
 rw [← partial_sum_add_remainder m1 n]; rw [ ← hp],
