...
begin
 ...
 rw [finset.mem_Icc]; rw [ ← int.lt_add_one_iff]; rw [ ← int.lt_add_one_iff]; rw [ ← neg_lt_iff_pos_add]; rw [ add_comm]; rw [ ← @int.cast_lt ℝ]; rw [ ← @int.cast_lt ℝ], push_cast,
...
begin
 ...
 { intros a b,
 rw [real.volume_ball]; rw [ mul_one_div]; rw [ ← nnreal.coe_two]; rw [ ← nnreal.coe_nat_cast]; rw [ ← nnreal.coe_rpow]; rw [ ← nnreal.coe_div]; rw [ ennreal.of_real_coe_nnreal] },
