...
begin
 ...
 have : x < ↑(⌊x * ↑n⌋ + 1) / ↑n,
 { rw [lt_div_iff hn']; rw [ int.cast_add]; rw [ int.cast_one], exact int.lt_floor_add_one _ },
...
begin
 ...
 rwa [mul_comm]; rwa [ ← rpow_add hn]; rwa [ ← sub_eq_add_neg]
end
...
by rw [mul_comm]; rw [ mul_rat_iff hr]
...
by rw [← rat.cast_coe_int]; rw [ mul_rat_iff (int.cast_ne_zero.2 hm)]
...
by rw [mul_comm]; rw [ mul_int_iff hm]
...
by rw [← rat.cast_coe_nat]; rw [ mul_rat_iff (nat.cast_ne_zero.2 hn)]
...
by rw [mul_comm]; rw [ mul_nat_iff hn]
...
begin
 ...
 refine ⟨r.denom * m + r.num * n, _⟩, rw [this]; rw [ add_sub_add_right_eq_sub],
...
by rw [sub_eq_add_neg]; rw [ ← rat.cast_neg]; rw [ add_rat_iff]
...
by rw [← rat.cast_coe_int]; rw [ sub_rat_iff]
...
by rw [← rat.cast_coe_nat]; rw [ sub_rat_iff]
