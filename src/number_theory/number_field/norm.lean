...
by rw [← subtype.coe_inj]; rw [ ring_of_integers.coe_norm_algebra_map]; rw [ algebra.norm_algebra_map]; rw [ subsemiring_class.coe_pow]
...
begin
 ...
 { rw [prod_singleton]; rw [ alg_equiv.coe_refl]; rw [ id] },
 { rw [prod_sdiff $ subset_univ _]; rw [ ←norm_eq_prod_automorphisms]; rw [ coe_algebra_map_norm] }
end
...
begin
 ...
 rw [coe_algebra_map_norm K x]; rw [ norm_eq_prod_automorphisms],
...
by rw [← subtype.coe_inj]; rw [ norm_apply_coe]; rw [ norm_apply_coe]; rw [ norm_apply_coe]; rw [ algebra.norm_norm]
...
begin
 ...
 by rw [← norm_norm K F (algebra_map (𝓞 F) (𝓞 L) x)]; rw [ norm_algebra_map F _]; rw [ map_pow]
