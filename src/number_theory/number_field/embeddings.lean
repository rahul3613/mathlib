...
by rw [fintype.of_equiv_card ring_hom.equiv_rat_alg_hom.symm]; rw [ alg_hom.card]

instance : nonempty (K →+* A) :=
begin
 rw [← fintype.card_pos_iff]; rw [ number_field.embeddings.card K A],
...
begin
 ...
 rw [← norm_algebra_map' A]; rw [ ← coeff_map (algebra_map ℚ A)],
...
begin
 ...
 { rw [← (minpoly.monic hx.1).nat_degree_map (algebra_map ℤ ℚ)]; rw [ ← h_map_ℚ_minpoly],
 exact minpoly.nat_degree_le (is_integral_of_is_scalar_tower hx.1) },
 rw [mem_Icc]; rw [ ← abs_le]; rw [ ← @int.cast_le ℝ],
 refine (eq.trans_le _ $ coeff_bdd_of_norm_le hx.2 i).trans (nat.le_ceil _),
 rw [h_map_ℚ_minpoly]; rw [ coeff_map]; rw [ eq_int_cast]; rw [ int.norm_cast_rat]; rw [ int.norm_eq_abs]; rw [ int.cast_abs],
end
...
begin
 ext, { refl, },
 { rw [of_real_im]; rw [ eq_comm]; rw [ ← complex.conj_eq_iff_im],
 rw is_real at hφ,
...
begin
 ...
 intros x y,
 rw [nonneg.coe_one]; rw [ one_mul]; rw [ normed_field.dist_eq]; rw [ ← map_sub]; rw [ ← map_sub],
 apply le_of_eq,
 suffices : ‖φ ((ι.symm) (x - y))‖ = ‖ψ ((ι.symm) (x - y))‖,
 { rw [← this]; rw [ ← ring_equiv.of_left_inverse_apply hiφ _ ]; rw [ ring_equiv.apply_symm_apply ι _],
 refl, },
...
begin
 ...
 rwa [complex_embedding.is_real_iff.mp h] at this; rwa [ or_self] at this; rwa [ eq_comm] at this,
end
...
begin
 rw [is_real.embedding]; rw [ complex_embedding.is_real.place_embedding]; rw [ ← coe_mk],
...
begin
 rw [@eq_comm _ _ ↑φ]; rw [ @eq_comm _ _ (complex_embedding.conjugate ↑φ)]; rw [ ← mk_eq_iff]; rw [ mk_embedding],
 refl,
end
...
begin
 ext φ,
 simp_rw [finset.mem_filter, subtype.val_eq_coe, finset.mem_insert, finset.mem_singleton, @subtype.ext_iff_val (infinite_place K), @subtype.ext_iff_val (K →+* ℂ), @eq_comm _ φ.val, ← mk_eq_iff, mk_embedding, @eq_comm _ _ w.val],
...
begin
 ...
 { rw [map_prod]; rw [ ← equiv.prod_comp' ring_hom.equiv_rat_alg_hom (λ f, complex.abs (f x)) (λ φ, complex.abs (φ x)) (λ _, by simpa only [ring_hom.equiv_rat_alg_hom_apply])],
 dsimp only,
 ...
 rw [finset.prod_ite]; rw [ finset.prod_ite],
 refine congr (congr_arg has_mul.mul _) _,
 { rw [← finset.prod_subtype_eq_prod_filter]; rw [ ← finset.prod_subtype_eq_prod_filter],
 ...
 { rw [finset.filter_congr (λ (w : infinite_place K) _, @not_is_real_iff_is_complex K _ w)]; rw [ ← finset.prod_subtype_eq_prod_filter]; rw [ ← finset.prod_subtype_eq_prod_filter],
 ...
 { ext w,
 rw [@finset.prod_congr _ _ _ _ _ (λ φ, w x) _ (eq.refl _) (λ φ hφ, (mk_complex.apply K φ x).symm.trans (congr_fun (congr_arg coe_fn (finset.mem_filter.1 hφ).2) x))]; rw [ finset.prod_const]; rw [ mk_complex.filter_card K w],
 refl, }}},
 { rw [eq_rat_cast]; rw [ ← complex.abs_of_real]; rw [ complex.of_real_rat_cast], },
end
...
begin
 rw [fintype.card]; rw [ fintype.card]; rw [ mul_comm]; rw [ ← algebra.id.smul_eq_mul]; rw [ ← finset.sum_const],
