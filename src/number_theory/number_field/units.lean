...
by simp_rw [(is_unit_map_iff (rat.ring_of_integers_equiv : 𝓞 ℚ →+* ℤ) x).symm, int.is_unit_iff, ring_equiv.coe_to_ring_hom, ring_equiv.map_eq_one_iff, ring_equiv.map_eq_neg_one_iff, ← subtype.coe_injective.eq_iff, add_subgroup_class.coe_neg, algebra_map.coe_one]
...
by { convert (ring_of_integers.is_unit_norm ℚ).symm,
 rw [← abs_one]; rw [ abs_eq_abs]; rw [ ← rat.ring_of_integers.is_unit_iff], }
