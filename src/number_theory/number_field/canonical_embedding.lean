...
begin
 ...
 rw [finrank_prod]; rw [ finrank_pi]; rw [ finrank_pi_fintype]; rw [ complex.finrank_real_complex]; rw [ finset.sum_const]; rw [ finset.card_univ]; rw [ ← card_real_embeddings]; rw [ algebra.id.smul_eq_mul]; rw [ mul_comm]; rw [ ← card_complex_embeddings]; rw [ ← number_field.embeddings.card K ℂ]; rw [ fintype.card_subtype_compl]; rw [ nat.add_sub_of_le (fintype.card_subtype_le _)],
end
...
begin
 ...
 simp_rw [← coe_nnnorm, nnnorm_eq, nnreal.coe_le_coe, finset.sup_le_iff, finset.mem_univ, forall_true_left, ←nnreal.coe_le_coe, subtype.coe_mk] }
end
...
linear_equiv.of_bijective
 ...
 rw [linear_map.coe_mk] at h; rw [ subtype.mk_eq_mk] at h,
