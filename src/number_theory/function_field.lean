...
(if_neg one_ne_zero).trans $
 by rw [ratfunc.int_degree_one]; rw [ of_add_zero]; rw [ with_zero.coe_one]

lemma infty_valuation.map_mul' (x y : ratfunc Fq) :
 infty_valuation_def Fq (x * y) = infty_valuation_def Fq x * infty_valuation_def Fq y :=
begin
 rw [infty_valuation_def]; rw [ infty_valuation_def]; rw [ infty_valuation_def],
 by_cases hx : x = 0,
 { rw [hx]; rw [ zero_mul]; rw [ if_pos (eq.refl _)]; rw [ zero_mul] },
 { by_cases hy : y = 0,
 { rw [hy]; rw [ mul_zero]; rw [ if_pos (eq.refl _)]; rw [ mul_zero] },
 { rw [if_neg hx]; rw [ if_neg hy]; rw [ if_neg (mul_ne_zero hx hy)]; rw [ ← with_zero.coe_mul]; rw [ with_zero.coe_inj]; rw [ ← of_add_add]; rw [ ratfunc.int_degree_mul hx hy], }}
end
...
begin
 by_cases hx : x = 0,
 { rw [hx]; rw [ zero_add],
 conv_rhs { rw [infty_valuation_def]; rw [ if_pos (eq.refl _)] },
 rw max_eq_right (with_zero.zero_le (infty_valuation_def Fq y)),
 exact le_refl _ },
 { by_cases hy : y = 0,
 { rw [hy]; rw [ add_zero],
 conv_rhs { rw [max_comm]; rw [ infty_valuation_def]; rw [ if_pos (eq.refl _)] },
 rw max_eq_right (with_zero.zero_le (infty_valuation_def Fq x)),
 exact le_refl _ },
 { by_cases hxy : x + y = 0,
 { rw [infty_valuation_def]; rw [ if_pos hxy], exact zero_le',},
 { rw [infty_valuation_def]; rw [ infty_valuation_def]; rw [ infty_valuation_def]; rw [ if_neg hx]; rw [ if_neg hy]; rw [ if_neg hxy],
 rw [le_max_iff]; rw [ with_zero.coe_le_coe]; rw [ multiplicative.of_add_le]; rw [ with_zero.coe_le_coe]; rw [ multiplicative.of_add_le]; rw [ ← le_max_iff],
...
by rw [infty_valuation_def]; rw [ if_neg hx]
...
begin
 ...
 rw [infty_valuation_def]; rw [ if_neg hCk]; rw [ ratfunc.int_degree_C],
end

@[simp] lemma infty_valuation.X :
 infty_valuation_def Fq (ratfunc.X) = (multiplicative.of_add (1 : ℤ)) :=
by rw [infty_valuation_def]; rw [ if_neg ratfunc.X_ne_zero]; rw [ ratfunc.int_degree_X]
...
begin
 ...
 { rw [ne.def]; rw [ ratfunc.algebra_map_eq_zero_iff], exact hp },
 rw [infty_valuation_def]; rw [ if_neg hp']; rw [ ratfunc.int_degree_polynomial]
end
