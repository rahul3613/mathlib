...
begin
 ...
 rw [← div_le_one (rpow_pos_of_pos four_pos x)]; rw [ ← div_div_eq_mul_div]; rw [ ← rpow_sub four_pos]; rw [ ← mul_div 2 x]; rw [ mul_div_left_comm]; rw [ ← mul_one_sub]; rw [ (by norm_num1 : (1 : ℝ) - 2 / 3 = 1 / 3)]; rw [ mul_one_div]; rw [ ← log_nonpos_iff (hf' x h5)]; rw [ ← hf x h5],
 ...
 rw [hf]; rw [ log_nonneg_iff (hf' 18 _)]; rw [ this]; norm_num1 },
 { have : sqrt (2 * 512) = 32,
 { exact (sqrt_eq_iff_mul_self_eq_of_pos (by norm_num1)).mpr (by norm_num1) },
 rw [hf]; rw [ log_nonpos_iff (hf' _ _)]; rw [ this]; rw [ div_le_one (rpow_pos_of_pos four_pos _)]; rw [ ← rpow_le_rpow_iff _ (rpow_pos_of_pos four_pos _).le three_pos]; rw [ ← rpow_mul]; norm_num1 },
end
...
begin
 ...
 rw [←and_assoc] at no_prime; rw [ not_and'] at no_prime; rw [ not_and_distrib] at no_prime; rw [ not_lt] at no_prime,
 cases no_prime hx with h h,
 { rw [factorization_eq_zero_of_non_prime n.central_binom h]; rw [ pow_zero] },
 { rw [factorization_central_binom_of_two_mul_self_lt_three_mul n_large h h2x]; rw [ pow_zero] },
end
...
begin
 ...
 rw [factorization_eq_zero_of_non_prime n.central_binom h]; rw [ pow_zero] },
 rw [central_binom_factorization_small n n_big no_prime]; rw [ ← this]; rw [ ← finset.prod_filter_mul_prod_filter_not S (≤ sqrt (2 * n))],
 apply mul_le_mul',
 ...
 { rw [card_Icc]; rw [ nat.add_sub_cancel] },
