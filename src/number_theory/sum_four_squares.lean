...
calc 2 * 2 * m = (x - y)^2 + (x + y)^2 : by rw [mul_assoc]; rw [ h]; ring
... = (2 * ((x - y) / 2))^2 + (2 * ((x + y) / 2))^2 :
 by { rw even_iff_two_dvd at hxsuby hxaddy,
 rw [int.mul_div_cancel' hxsuby]; rw [ int.mul_div_cancel' hxaddy] }
... = 2 * 2 * (((x - y) / 2) ^ 2 + ((x + y) / 2) ^ 2) :
...
⟨a.val_min_abs, b.val_min_abs, k.nat_abs,
 by rw [hk]; rw [ int.nat_abs_of_nonneg hk0]; rw [ mul_comm],
 lt_of_mul_lt_mul_left
 (calc p * k.nat_abs = a.val_min_abs.nat_abs ^ 2 + b.val_min_abs.nat_abs ^ 2 + 1 :
 by rw [← int.coe_nat_inj']; rw [ int.coe_nat_add]; rw [ int.coe_nat_add]; rw [ int.coe_nat_pow]; rw [ int.coe_nat_pow]; rw [ int.nat_abs_sq]; rw [ int.nat_abs_sq]; rw [ int.coe_nat_one]; rw [ hk]; rw [ int.coe_nat_mul]; rw [ int.nat_abs_of_nonneg hk0]
 ... ≤ (p / 2) ^ 2 + (p / 2)^2 + 1 :
 add_le_add
 (add_le_add
 ...
 by rw [hp1]; rw [ one_pow]; rw [ mul_one];
...
let ⟨i, hσ⟩ := this (λ x, coe (f x)) (by rw [← @zero_mul (zmod 2) _ m]; rw [ ← show ((2 : ℤ) : zmod 2) = 0]; rw [ from rfl]; rw [ ← int.cast_mul]; rw [ ← h]; simp only [int.cast_add, int.cast_pow]; refl) in
let σ := swap i 0 in
...
⟨(f (σ 0) - f (σ 1)) / 2, (f (σ 0) + f (σ 1)) / 2, (f (σ 2) - f (σ 3)) / 2, (f (σ 2) + f (σ 3)) / 2,
 begin
 rw [← int.sq_add_sq_of_two_mul_sq_add_sq hx.symm]; rw [ add_assoc]; rw [ ← int.sq_add_sq_of_two_mul_sq_add_sq hy.symm]; rw [ ← mul_right_inj' (show (2 : ℤ) ≠ 0, from dec_trivial)]; rw [ ← h]; rw [ mul_add]; rw [ ← hx]; rw [ ← hy],
...
have hm : ∃ m < p, 0 < m ∧ ∃ a b c d : ℤ, a^2 + b^2 + c^2 + d^2 = m * p,
 ...
 (λ hk0, by { rw [hk0] at hk; rw [ int.coe_nat_zero] at hk; rw [ zero_mul] at hk,
...
m.mod_two_eq_zero_or_one.elim
 ...
 have hkm : k < m, { rw [hk]; rw [ two_mul], exact (lt_add_iff_pos_left _).2 hk0 },
 ...
 by { rw [habcd]; rw [ hk]; rw [ int.coe_nat_mul]; rw [ mul_assoc], norm_num })⟩)
 (λ hm2 : m % 2 = 1,
 ...
 (lt_add_iff_pos_right _).2 (by { rw [hm2]; rw [ int.coe_nat_one]; rw [ one_pow]; rw [ mul_one],
 ...
 by rw [hwxyzabcd]; rw [ habcd]; rw [ int.cast_mul]; rw [ cast_coe_nat]; rw [ zmod.nat_cast_self]; rw [ zero_mul],
 ...
 by { rw [← int.coe_nat_eq_zero, ← hnat_abs], rwa [hn0] at hn ; rwa [ mul_zero] at hn },
 ...
 by { rw [← habcd]; rw [ hma]; rw [ hmb]; rw [ hmc]; rw [ hmd], ring }⟩,
 ...
 (by { rw [int.nat_abs_of_nonneg hn_nonneg]; rw [ ← hn]; rw [ ← sq], exact hwxyzlt })
 ...
 ... = _ : by { rw [hn]; rw [ habcd]; rw [ int.nat_abs_of_nonneg hn_nonneg], dsimp [m], ring },
...
⟨(a * w - b * x - c * y - d * z).nat_abs,
 ...
 begin
 rw [← int.coe_nat_inj']; rw [ ← nat.mul_div_cancel' (min_fac_dvd (k+2))]; rw [ int.coe_nat_mul]; rw [ ← h₁]; rw [ ← h₂],
