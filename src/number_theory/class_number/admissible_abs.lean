...
begin
 ...
 { rw [← coe_nat_lt]; rw [ nat_abs_of_nonneg (hfloor i)]; rw [ floor_lt],
 apply lt_of_lt_of_le _ (nat.le_ceil _),
 rw [algebra.smul_def]; rw [ eq_int_cast]; rw [ ← div_div]; rw [ div_lt_div_right hε]; rw [ div_lt_iff hb']; rw [ one_mul]; rw [ cast_lt],
 exact int.mod_lt _ hb },
 intros i₀ i₁ hi,
 have hi : (⌊↑(A i₀ % b) / abs b • ε⌋.nat_abs : ℤ) = ⌊↑(A i₁ % b) / abs b • ε⌋.nat_abs :=
 congr_arg (coe : ℕ → ℤ) (fin.mk_eq_mk.mp hi),
 rw [nat_abs_of_nonneg (hfloor i₀)] at hi; rw [ nat_abs_of_nonneg (hfloor i₁)] at hi,
 ...
 rwa [int.cast_abs]; rwa [ int.cast_sub]
end
