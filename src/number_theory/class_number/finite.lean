...
begin
 ...
 rw [algebra.norm_apply]; rw [ ← linear_map.det_to_matrix bS],
 ...
 { rw [finset.card_univ]; rw [ smul_mul_assoc]; rw [ mul_comm] },
 { intros i j k,
 apply finset.le_max',
 exact finset.mem_image.mpr ⟨⟨i, j, k⟩, finset.mem_univ _, rfl⟩ },
end
...
begin
 ...
 have : (y' : T) < y,
 { rw [y'_def]; rw [ ← finset.max'_image (show monotone (coe : ℤ → T), from λ x y h, int.cast_le.mpr h)],
...
begin
 ...
 have := abv.nonneg b,
 rw [ε_eq]; rw [ algebra.smul_def]; rw [ eq_int_cast]; rw [ ← rpow_nat_cast]; rw [ mul_rpow]; rw [ ← rpow_mul]; rw [ div_mul_cancel]; rw [ rpow_neg_one]; rw [ mul_left_comm]; rw [ mul_inv_cancel]; rw [ mul_one]; rw [ rpow_nat_cast];
 ...
 ring },
 rw [this]; rw [ algebra.norm_algebra_map_of_basis bS]; rw [ abv.map_pow],

 refine int.cast_lt.mp ((norm_lt abv bS _ (λ i, lt_of_le_of_lt _ (hjk' i))).trans_le _),
 { apply le_of_eq,
 congr,
 simp_rw [linear_equiv.map_sum, linear_equiv.map_sub, linear_equiv.map_smul, finset.sum_apply', finsupp.sub_apply, finsupp.smul_apply, finset.sum_sub_distrib, basis.repr_self_apply, smul_eq_mul, mul_boole, finset.sum_ite_eq', finset.mem_univ, if_true] },
 { exact_mod_cast ε_le },
end
...
begin
 ...
 rw [← abv.map_mul]; rw [ ← monoid_hom.map_mul]; rw [ ← abv.map_mul]; rw [ ← monoid_hom.map_mul]; rw [ ← algebra.smul_def]; rw [ smul_sub b']; rw [ sub_mul]; rw [ smul_comm]; rw [ h]; rw [ mul_comm b a']; rw [ algebra.smul_mul_assoc r a' b]; rw [ algebra.smul_mul_assoc b' q b]
end
...
begin
 ...
 rintro rfl,
 rw [ideal.zero_eq_bot] at hJ; rw [ ideal.mul_bot] at hJ,
 exact hM (ideal.span_singleton_eq_bot.mp (I.2 _ hJ)) },
 { rw class_group.mk0_eq_mk0_iff,
 exact ⟨algebra_map _ _ M, b, hM, b_ne_zero, hJ⟩ },
 rw [← set_like.mem_coe]; rw [ ← set.singleton_subset_iff]; rw [ ← ideal.span_le]; rw [ ← ideal.dvd_iff_le],
...
@fintype.of_surjective _ _ _
 ...
 (by { rw [ne.def]; rw [ ideal.zero_eq_bot]; rw [ ideal.span_singleton_eq_bot],
 exact prod_finset_approx_ne_zero bS adm }))
 ((equiv.refl _).subtype_equiv (λ I, ideal.dvd_iff_le.trans
 (by rw [equiv.refl_apply]; rw [ ideal.span_le]; rw [ set.singleton_subset_iff]))))
