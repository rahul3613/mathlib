...
begin
 ...
 { rw [coeff_eq_zero_of_degree_lt (lt_of_lt_of_le (hA _) hbj)]; rw [ coeff_eq_zero_of_degree_lt (lt_of_lt_of_le (hA _) hbj)] },
...
begin
 ...
 rw [not_le] at hbj; rw [ degree_eq_nat_degree hb] at hbj; rw [ with_bot.coe_lt_coe] at hbj,
...
begin
 ...
 { rw [algebra.smul_def]; rw [ eq_int_cast],
 ...
 refine ⟨i₀, i₁, i_ne, _⟩,
 simp only at mod_eq,
 rwa [mod_eq]; rwa [ sub_self]; rwa [ absolute_value.map_zero]; rwa [ int.cast_zero] },
 ...
 { rwa [h]; rwa [ sub_self]; rwa [ absolute_value.map_zero]; rwa [ int.cast_zero] },
 ...
 { rwa [← real.log_lt_log_iff (int.cast_pos.mpr (card_pow_degree.pos h')) hbε]; rwa [ card_pow_degree_nonzero _ h']; rwa [ card_pow_degree_nonzero _ hb]; rwa [ algebra.smul_def]; rwa [ eq_int_cast]; rwa [ int.cast_pow]; rwa [ int.cast_coe_nat]; rwa [ int.cast_pow]; rwa [ int.cast_coe_nat]; rwa [ log_mul (pow_ne_zero _ q_pos'.ne') hε.ne']; rwa [ ← rpow_nat_cast]; rwa [ ← rpow_nat_cast]; rwa [ log_rpow q_pos']; rwa [ log_rpow q_pos']; rwa [ ← lt_div_iff (log_pos one_lt_q')]; rwa [ add_div]; rwa [ mul_div_cancel _ (log_pos one_lt_q').ne'] },
...
begin
 ...
 { rwa ← hyz' },
 by_cases hxz' : x = z,
 { rwa [hxz']; rwa [ sub_self]; rwa [ absolute_value.map_zero] },
 rw [← ne.def] at hxy' hyz' hxz'; rw [ ← sub_ne_zero] at hxy' hyz' hxz',
 refine lt_of_le_of_lt _ (max_lt hxy hyz),
 rw [card_pow_degree_nonzero _ hxz']; rw [ card_pow_degree_nonzero _ hxy']; rw [ card_pow_degree_nonzero _ hyz'],
 ...
 rw [nat_degree_le_iff_degree_le]; rw [ nat_degree_le_iff_degree_le]; rw [ ← le_max_iff]; rw [ ← degree_eq_nat_degree hxy']; rw [ ← degree_eq_nat_degree hyz'],
...
begin
 ...
 { simpa using hbε },
 { rw [fin.cons_succ]; rw [ fin.cons_zero]; rw [ eq_comm]; rw [ absolute_value.map_sub],
 exact hj i₁ },
 { rw [fin.cons_succ]; rw [ fin.cons_zero],
 exact hj i₀ },
 ...
 { rw [fin.cons_succ] at approx; rw [ fin.cons_zero] at approx; rw [ ← not_le] at approx; rw [ absolute_value.map_sub] at approx,
 have := (classical.some_spec (this j₁)).2,
 contradiction },
 { rw [fin.cons_succ] at approx; rw [ fin.cons_zero] at approx; rw [ ← not_le] at approx,
 have := (classical.some_spec (this j₀)).2,
 contradiction },
 { rw [fin.cons_succ] at approx; rw [ fin.cons_succ] at approx,
