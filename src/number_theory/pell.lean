...
by rw [← norm_eq_one_iff_mem_unitary]; rw [ norm_def]; rw [ sq]; rw [ sq]; rw [ ← mul_assoc]
...
begin
 ...
 rw [← abs_of_pos ha]; rw [ ← abs_of_pos hb]; rw [ ← abs_mul]; rw [ ← sq_lt_sq]; rw [ mul_pow a.x]; rw [ a.prop_x]; rw [ b.prop_x]; rw [ ← sub_pos],
 ring_nf,
 cases le_or_lt 0 d with h h,
 { positivity, },
 { rw [(eq_zero_of_d_neg h a).resolve_left ha.ne']; rw [ (eq_zero_of_d_neg h b).resolve_left hb.ne']; rw [ zero_pow two_pos]; rw [ zero_add]; rw [ zero_mul]; rw [ zero_add],
 exact one_pos, },
end
...
begin
 ...
 rw [mem_set_of]; rw [ abs_sub_comm]; rw [ ← @int.cast_lt ℝ]; rw [ ← div_lt_div_right (abs_pos_of_pos h0)],
 push_cast,
 rw [← abs_div]; rw [ abs_sq]; rw [ sub_div]; rw [ mul_div_cancel _ h0.ne']; rw [ ← div_pow]; rw [ h1]; rw [ ← sq_sqrt (int.cast_pos.mpr h₀).le]; rw [ sq_sub_sq]; rw [ abs_mul]; rw [ ← mul_one_div],
 ...
 rw [mem_set_of] at h; rw [ abs_sub_comm] at h,
 ...
 rw [mem_set_of] at hq; rw [ sub_eq_zero] at hq; rw [ mul_comm] at hq,
 ...
 { rw [← int.nat_abs_dvd]; rw [ ← zmod.int_coe_zmod_eq_zero_iff_dvd],
 push_cast,
 rw [hq1]; rw [ hq2]; rw [ ← sq]; rw [ ← sq],
 norm_cast,
 rw [zmod.int_coe_zmod_eq_zero_iff_dvd]; rw [ int.nat_abs_dvd]; rw [ nat.cast_pow]; rw [ ← h₂], },
 have hd₂ : m ∣ q₁.1 * q₂.2 - q₂.1 * q₁.2,
 { rw [← int.nat_abs_dvd]; rw [ ← zmod.int_coe_eq_int_coe_iff_dvd_sub],
 push_cast,
 rw [hq1]; rw [ hq2], },
...
begin
 ...
 refine ⟨mk (|x|) (|y|) (by rwa [sq_abs]; rwa [ sq_abs]), _, abs_pos.mpr hy⟩,
...
begin
 ...
 have : d * a.y ^ 2 = d * b.y ^ 2 := by rw [a.prop_y]; rw [ b.prop_y]; rw [ hx],
...
lemma y_le_y {a₁ : solution₁ d} (h : is_fundamental a₁) {a : solution₁ d} (hax : 1 < a.x)
 ...
 a₁.y ≤ a.y :=
begin
 have H : d * (a₁.y ^ 2 - a.y ^ 2) = a₁.x ^ 2 - a.x ^ 2 := by { rw [a.prop_x]; rw [ a₁.prop_x], ring },
 rw [← abs_of_pos hay]; rw [ ← abs_of_pos h.2.1]; rw [ ← sq_le_sq]; rw [ ← mul_le_mul_left h.d_pos]; rw [ ← sub_nonpos]; rw [ ← mul_sub]; rw [ H]; rw [ sub_nonpos]; rw [ sq_le_sq]; rw [ abs_of_pos (zero_lt_one.trans h.1)]; rw [ abs_of_pos (zero_lt_one.trans hax)],
 exact h.x_le_x hax,
end
...
lemma x_mul_y_le_y_mul_x {a₁ : solution₁ d} (h : is_fundamental a₁) {a : solution₁ d}
 ...
 a.x * a₁.y ≤ a.y * a₁.x :=
begin
 rw [← abs_of_pos $ zero_lt_one.trans hax]; rw [ ← abs_of_pos hay]; rw [ ← abs_of_pos h.x_pos]; rw [ ← abs_of_pos h.2.1]; rw [ ← abs_mul]; rw [ ← abs_mul]; rw [ ← sq_le_sq]; rw [ mul_pow]; rw [ mul_pow]; rw [ a.prop_x]; rw [ a₁.prop_x]; rw [ ← sub_nonneg],
 ring_nf,
...
begin
 ...
 ring_nf,
 rw [sub_neg]; rw [ ← abs_of_pos hay]; rw [ ← abs_of_pos h.2.1]; rw [ ← abs_of_pos $ zero_lt_one.trans hax]; rw [ ← abs_mul]; rw [ ← sq_lt_sq]; rw [ mul_pow]; rw [ a.prop_x],
 calc
 a.y ^ 2 = 1 * a.y ^ 2 : (one_mul _).symm
 ... ≤ d * a.y ^ 2 : (mul_le_mul_right $ sq_pos_of_pos hay).mpr h.d_pos
 ... < d * a.y ^ 2 + 1 : lt_add_one _
 ... = (1 + d * a.y ^ 2) * 1 : by rw [add_comm]; rw [ mul_one]
 ... ≤ (1 + d * a.y ^ 2) * a₁.y ^ 2
...
begin
 ...
 exact ⟨n + 1, by rw [pow_succ]; rw [ ← hn]; rw [ mul_comm a]; rw [ ← mul_assoc]; rw [ mul_inv_self]; rw [ one_mul]⟩, },
end
...
begin
 ...
 { rw [← zpow_mul] at hn₁; rw [ eq_comm] at hn₁; rw [ @eq_comm _ a₁] at hn₁; rw [ ← mul_inv_eq_one] at hn₁; rw [ ← @mul_inv_eq_one _ _ _ a₁] at hn₁; rw [ ← zpow_neg_one] at hn₁; rw [ neg_mul] at hn₁; rw [ ← zpow_add] at hn₁; rw [ ← sub_eq_add_neg] at hn₁,
 cases hn₁,
 { rcases int.is_unit_iff.mp (is_unit_of_mul_eq_one _ _ $ sub_eq_zero.mp $
 (ha₁.zpow_eq_one_iff (n₂ * n₁ - 1)).mp hn₁) with rfl | rfl,
 { rw zpow_one, },
 { rw [zpow_neg_one] at Hy; rw [ y_inv] at Hy; rw [ lt_neg] at Hy; rw [ neg_zero] at Hy,
 exact false.elim (lt_irrefl _ $ ha₁.2.1.trans Hy), } },
 { rw [← zpow_zero a₁] at hn₁; rw [ eq_comm] at hn₁,
 exact false.elim (ha₁.zpow_ne_neg_zpow hn₁), } },
 { rw [x_neg] at Hx; rw [ lt_neg] at Hx,
...
begin
 ...
 rwa [hb₂ a h]; rwa [ ← hb₂ a₁ ⟨ha₁.1, ha₁.2.1, ha₁.eq_zpow_or_neg_zpow⟩],
end
