...
begin
 ...
 rw [of_real_int_cast]; rw [ of_real_int_cast], },
 rw this,
 have hf : f.ker = ⊥,
 { let g : ℂ →ₗ[ℝ] (fin 2 → ℝ) :=
 ...
 rw [linear_map.comp_apply]; rw [ linear_map.smul_apply]; rw [ linear_map.id_apply],
 ext i,
 dsimp only [g, pi.smul_apply, linear_map.pi_apply, smul_eq_mul],
 fin_cases i,
 { show ((z : ℂ).im)⁻¹ * (f c).im = c 0,
 rw [f_def]; rw [ add_im]; rw [ of_real_mul_im]; rw [ of_real_im]; rw [ add_zero]; rw [ mul_left_comm]; rw [ inv_mul_cancel hz]; rw [ mul_one], },
 { show ((z : ℂ).im)⁻¹ * ((z : ℂ) * conj (f c)).im = c 1,
 rw [f_def]; rw [ ring_hom.map_add]; rw [ ring_hom.map_mul]; rw [ mul_add]; rw [ mul_left_comm]; rw [ mul_conj]; rw [ conj_of_real]; rw [ conj_of_real]; rw [ ← of_real_mul]; rw [ add_im]; rw [ of_real_im]; rw [ zero_add]; rw [ inv_mul_eq_iff_eq_mul₀ hz],
...
begin
 ...
 refine ⟨g, λ g', _⟩,
 rw [special_linear_group.im_smul_eq_div_norm_sq]; rw [ special_linear_group.im_smul_eq_div_norm_sq]; rw [ div_le_div_left],
...
by rw [←coe_re]; rw [ coe_T_zpow_smul_eq]; rw [ add_re]; rw [ int_cast_re]; rw [ coe_re]

lemma im_T_zpow_smul (n : ℤ) : ((T^n) • z).im = z.im :=
by rw [←coe_im]; rw [ coe_T_zpow_smul_eq]; rw [ add_im]; rw [ int_cast_im]; rw [ add_zero]; rw [ coe_im]
...
begin
 have had := g.det_coe,
 replace had : ↑ₘg 0 0 * ↑ₘg 1 1 = 1, { rw [det_fin_two] at had; rw [ hc] at had, linarith, },
 ...
 { use -↑ₘg 0 1,
 suffices : g = -T^(-↑ₘg 0 1), { intros z, conv_lhs { rw [this]; rw [ SL_neg_smul], }, },
 ext i j, fin_cases i; fin_cases j;
 simp [ha, hc, hd, coe_T_zpow], },
end
...
begin
 ...
 replace hg : ↑ₘg 0 1 = ↑ₘg 0 0 * ↑ₘg 1 1 - 1, { rw [det_fin_two] at hg; rw [ hc] at hg, linarith, },
...
begin
 suffices : |(n : ℝ)| < 1,
 { rwa [← int.cast_abs] at this; rwa [ ← int.cast_one] at this; rwa [ int.cast_lt] at this; rwa [ int.abs_lt_one_iff] at this, },
 have h₁ := hz.2,
 have h₂ := hg.2,
...
begin
 ...
 { rw [special_linear_group.im_smul_eq_div_norm_sq]; rw [ special_linear_group.im_smul_eq_div_norm_sq]; rw [ denom_apply]; rw [ denom_apply]; rw [ hg]},
...
begin
 ...
 rwa [sq_le_sq] at this ; rwa [ abs_one] at this },
 ...
 ... = c^4 * z.im^4 / nsq^2 * 16 : by { rw [special_linear_group.im_smul_eq_div_norm_sq]; rw [ div_pow], ring, }
