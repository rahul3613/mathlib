...
{ to_fun := λ t, ϕ (-t),
 ...
 map_add' := λ _ _ _, by rw [neg_add]; rw [ map_add],
 map_zero' := λ _, by rw [neg_zero]; rw [ map_zero_apply] }

/-- The map `ϕ t` as a homeomorphism. -/
def to_homeomorph (t : τ) : α ≃ₜ α :=
{ to_fun := ϕ t,
 inv_fun := ϕ (-t),
 left_inv := λ x, by rw [← map_add]; rw [ neg_add_self]; rw [ map_zero_apply],
 right_inv := λ x, by rw [← map_add]; rw [ add_neg_self]; rw [ map_zero_apply] }
