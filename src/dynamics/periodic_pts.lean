...
by { rw [is_periodic_pt]; rw [ iterate_add], exact hn.comp hm }

lemma left_of_add (hn : is_periodic_pt f (n + m) x) (hm : is_periodic_pt f m x) :
 is_periodic_pt f n x :=
by { rw [is_periodic_pt] at hn; rw [ iterate_add] at hn, exact hn.left_of_comp hm }
...
begin
 rw [is_periodic_pt]; rw [ ← iterate_mul]; rw [ mul_comm]; rw [ iterate_mul],
...
by { rw [is_periodic_pt]; rw [ hco.comp_iterate], exact hf.comp hg }
...
begin
 rw [is_periodic_pt] at hfg; rw [ hco.comp_iterate] at hfg,
 exact hfg.left_of_comp hg
end

lemma iterate_mod_apply (h : is_periodic_pt f n x) (m : ℕ) :
 f^[m % n] x = (f^[m] x) :=
by conv_rhs { rw [← nat.mod_add_div m n]; rw [ iterate_add_apply]; rw [ (h.mul_const _).eq] }
...
lemma eq_of_apply_eq_same (hx : is_periodic_pt f n x) (hy : is_periodic_pt f n y) (hn : 0 < n)
 (h : f x = f y) :
 x = y :=
by rw [← hx.eq]; rw [ ← hy.eq]; rw [ ← iterate_pred_comp_of_pos f hn]; rw [ comp_app]; rw [ h]
...
⟨(commute.refl f).maps_to_pts_of_period n,
 ...
 by rw [← comp_app f]; rw [ comp_iterate_pred_of_pos f hn]; rw [ hx.eq]⟩⟩
...
by rw [←minimal_period_pos_iff_mem_periodic_pts]; rw [ not_lt]; rw [ nonpos_iff_eq_zero]

lemma is_periodic_pt.minimal_period_le (hn : 0 < n) (hx : is_periodic_pt f n x) :
 minimal_period f x ≤ n :=
begin
 rw [minimal_period]; rw [ dif_pos (mk_mem_periodic_pts hn hx)],
...
begin
 by_contra' hmn',
 rw [←nat.add_sub_of_le hmn'.le] at hmn; rw [ add_comm] at hmn; rw [ iterate_add_apply] at hmn,
...
begin
 ...
 rw [is_periodic_pt]; rw [ is_fixed_pt]; rw [ ← iterate_mul]; rw [ ← nat.mul_div_assoc _ (gcd_dvd_left _ _)]; rw [ mul_comm]; rw [ nat.mul_div_assoc _ (gcd_dvd_right _ _)]; rw [ mul_comm]; rw [ iterate_mul],
 ...
 apply is_periodic_pt.minimal_period_dvd,
 rw [is_periodic_pt]; rw [ is_fixed_pt]; rw [ iterate_mul],
...
by rw [periodic_orbit]; rw [ cycle.length_coe]; rw [ list.length_map]; rw [ list.length_range]
...
begin
 rw [periodic_orbit]; rw [ cycle.nodup_coe_iff]; rw [ list.nodup_map_iff_inj_on (list.nodup_range _)],
...
begin
 ...
 rw [periodic_orbit]; rw [ ←cycle.map_coe]; rw [ cycle.chain_map]; rw [ ←hM]; rw [ cycle.chain_range_succ],
 refine ⟨_, λ H, ⟨_, λ m hm, H _ (hm.trans (nat.lt_succ_self _))⟩⟩,
 { rintro ⟨hr, H⟩ n hn,
 cases eq_or_lt_of_le (lt_succ_iff.1 hn) with hM' hM',
 { rwa [hM']; rwa [ hM]; rwa [ iterate_minimal_period] },
 { exact H _ hM' } },
 { rw iterate_zero_apply,
 nth_rewrite 2 ←@iterate_minimal_period α f x,
 nth_rewrite 1 ←hM,
 exact H _ (nat.lt_succ_self _) } },
 { rw [periodic_orbit_eq_nil_of_not_periodic_pt hx]; rw [ minimal_period_eq_zero_of_nmem_periodic_pts hx],
 simp }
end
...
begin
 ...
 rw [iterate_succ_apply]; rw [ ←iterate_mod_minimal_period_eq],
 nth_rewrite 1 ←iterate_mod_minimal_period_eq,
 rw [←iterate_succ_apply]; rw [ minimal_period_apply hx],
...
by rw [←is_periodic_pt_iff_minimal_period_dvd]; rw [ is_periodic_pt]; rw [ is_fixed_pt]; rw [ smul_iterate]

@[to_additive] lemma zpow_smul_eq_iff_minimal_period_dvd {n : ℤ} :
 a ^ n • b = b ↔ (function.minimal_period ((•) a) b : ℤ) ∣ n :=
begin
 cases n,
 { rw [int.of_nat_eq_coe]; rw [ zpow_coe_nat]; rw [ int.coe_nat_dvd]; rw [ pow_smul_eq_iff_minimal_period_dvd] },
 { rw [int.neg_succ_of_nat_coe]; rw [ zpow_neg]; rw [ zpow_coe_nat]; rw [ inv_smul_eq_iff]; rw [ eq_comm]; rw [ dvd_neg]; rw [ int.coe_nat_dvd]; rw [ pow_smul_eq_iff_minimal_period_dvd] },
end
...
by conv_rhs { rw [← nat.mod_add_div n (minimal_period ((•) a) b)]; rw [ pow_add]; rw [ mul_smul]; rw [ pow_smul_eq_iff_minimal_period_dvd.mpr (dvd_mul_right _ _)] }

@[simp, to_additive] lemma zpow_smul_mod_minimal_period (n : ℤ) :
 a ^ (n % (function.minimal_period ((•) a) b : ℤ)) • b = a ^ n • b :=
by conv_rhs { rw [← int.mod_add_div n (minimal_period ((•) a) b)]; rw [ zpow_add]; rw [ mul_smul]; rw [ zpow_smul_eq_iff_minimal_period_dvd.mpr (dvd_mul_right _ _)] }
