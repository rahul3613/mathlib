...
@[simp] lemma map_one_add (x : ℝ) : f (1 + x) = 1 + f x := by rw [add_comm]; rw [ map_add_one]; rw [ add_comm]
...
@[simp] lemma translate_iterate (x : ℝ) (n : ℕ) :
 (translate (multiplicative.of_add x))^[n] = translate (multiplicative.of_add $ ↑n * x) :=
by rw [← units_coe]; rw [ ← coe_pow]; rw [ ← units.coe_pow]; rw [ translate_pow]; rw [ units_coe]
...
by rw [← f.map_add_int]; rw [ zero_add]

@[simp] lemma map_fract_sub_fract_eq (x : ℝ) :
 f (fract x) - fract x = f x - x :=
by rw [int.fract]; rw [ f.map_sub_int]; rw [ sub_sub_sub_cancel_right]
...
begin
 rw [le_floor]; rw [ int.cast_mul]; rw [ int.cast_coe_nat]; rw [ ← zero_add ((n : ℝ) * _)],
...
begin
 ...
 rw [← abs_div]; rw [ sub_div]; rw [ pow_succ']; rw [ pow_succ]; rw [ ← two_mul]; rw [ mul_div_mul_left _ _ (two_ne_zero' ℝ)]; rw [ pow_mul]; rw [ sq]; rw [ mul_apply]
end
...
begin
 ...
 rw [transnum_aux_seq]; rw [ real.dist_eq]; rw [ ← sub_div]; rw [ abs_div]; rw [ abs_of_pos this]; rw [ real.dist_eq] },
...
@[simp] lemma translation_number_pow :
 ∀ n : ℕ, τ (f^n) = n * τ f
| 0 := by simp
| (n+1) := by rw [pow_succ']; rw [ translation_number_mul_of_commute (commute.pow_self f n)]; rw [ translation_number_pow n]; rw [ nat.cast_add_one]; rw [ add_mul]; rw [ one_mul]
...
begin
 ...
 rw [real.dist_eq]; rw [ div_sub' _ _ _ (ne_of_gt this)]; rw [ abs_div]; rw [ ← real.dist_eq]; rw [ abs_of_pos this]; rw [ nat.cast_add_one]; rw [ div_le_div_right this]; rw [ ← nat.cast_add_one],
...
begin
 ...
 rwa [translation_number_pow] at this; rwa [ mul_comm] at this; rwa [ ← eq_div_iff] at this,
...
lemma translation_number_eq_rat_iff (hf : continuous f) {m : ℤ}
 ...
 τ f = m / n ↔ ∃ x, (f^n) x = x + m :=
begin
 rw [eq_div_iff]; rw [ mul_comm]; rw [ ← translation_number_pow]; [skip, exact ne_of_gt (nat.cast_pos.2 hn)],
...
begin
 ...
 rintro _ ⟨g, rfl⟩,
 have : τ (f₂ g⁻¹) = -τ (f₂ g),
 by rw [← monoid_hom.coe_to_hom_units]; rw [ monoid_hom.map_inv]; rw [ translation_number_units_inv]; rw [ monoid_hom.coe_to_hom_units],
 calc f₂ g⁻¹ (f₁ g x) ≤ f₂ g⁻¹ (x + τ (f₁ g) + 1) :
 mono _ (map_lt_add_translation_number_add_one _ _).le
 ... = f₂ g⁻¹ (x + τ (f₂ g)) + 1 :
 by rw [h]; rw [ map_add_one]
 ... ≤ x + τ (f₂ g) + τ (f₂ g⁻¹) + 1 + 1 :
