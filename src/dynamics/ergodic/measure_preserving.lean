...
⟨e.symm.measurable,
 by rw [← h.map_eq]; rw [ map_map e.symm.measurable e.measurable]; rw [ e.symm_comp_self]; rw [ map_id]⟩
...
⟨hf.measurable, by rw [← hf.map_eq]; rw [ restrict_map hf.measurable hs]⟩
...
⟨hf.measurable, by rw [← hf.map_eq]; rw [ h₂.restrict_map]⟩
...
by rw [← hf.map_eq]; rw [ h₂.ae_measurable_map_iff]
...
⟨hg.1.comp hf.1, by rw [← map_map hg.1 hf.1]; rw [ hf.2]; rw [ hg.2]⟩
...
begin
 ...
 rwa [← iterate_add_apply]; rwa [ tsub_add_cancel_of_le hlt.le]
end
