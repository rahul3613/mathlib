...
begin
 ...
 rw [ae_eq_empty]; rw [ ae_eq_univ_iff_measure_eq hs]; rw [ add_circle.measure_univ],
 ...
 rwa [tendsto_const_nhds_iff] at hd; rwa [ ennreal.div_eq_one_iff hT₁ ennreal.of_real_ne_top] at hd, },
 ...
 { rw [volume_closed_ball]; rw [ mul_div]; rw [ mul_div_mul_left T _ two_ne_zero]; rw [ min_eq_right (div_le_self hT₀.le huj')]; rw [ mul_comm]; rw [ ← nsmul_eq_mul]; rw [ ← ennreal.of_real_nsmul]; rw [ nsmul_eq_mul]; rw [ mul_div_cancel'],
 exact nat.cast_ne_zero.mpr hj.ne', },
 rw [ennreal.div_eq_div_iff hT₁ ennreal.of_real_ne_top hI₀ hI₁]; rw [ volume_of_add_preimage_eq s _ (u j) d huj (hu₁ j) closed_ball_ae_eq_ball]; rw [ nsmul_eq_mul]; rw [ ← mul_assoc]; rw [ hI₂],
end
...
{ ae_empty_or_univ := λ s hs hs',
 ...
 replace hn : 1 < n.nat_abs, { rwa [int.abs_eq_nat_abs] at hn; rwa [ nat.one_lt_cast] at hn, },
 have hu₀ : ∀ j, add_order_of (u j) = n.nat_abs^j,
 { exact λ j, add_order_of_div_of_gcd_eq_one (pow_pos (pos_of_gt hn) j) (gcd_one_left _), },
 have hnu : ∀ j, n^j • (u j) = 0 := λ j, by rw [← add_order_of_dvd_iff_zsmul_eq_zero]; rw [ hu₀]; rw [ int.coe_nat_pow]; rw [ int.coe_nat_abs]; rw [ ← abs_pow]; rw [ abs_dvd],
...
begin
 ...
 { conv_rhs { congr, rw ←divisible_by.div_cancel x h }, rw [sub_smul]; rw [ one_smul]; rw [ sub_add_cancel], },
 ext y,
