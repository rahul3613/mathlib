...
begin
 ...
 { rwa [set.mem_preimage] at hxm ; rwa [ ← iterate_add_apply] at hxm }
end
...
begin
 ...
 rw [← map_add_at_top_eq_nat k]; rw [ frequently_map],
 refine (hx hk).mono (λ n hn, _),
 rwa [add_comm]; rwa [ iterate_add_apply]
end
...
begin
 ...
 { rwa [← iterate_mul]; rwa [ this]; rwa [ ← iterate_add_apply]; rwa [ tsub_add_cancel_of_le],
 exact hkl.le }
end
