...
by rw [supr]; rw [ Sup_inf_eq]; rw [ supr_range]
...
distrib_lattice.of_inf_sup_le $ λ a b c,
 by rw [←Sup_pair]; rw [ ←Sup_pair]; rw [ inf_Sup_eq]; rw [ ←Sup_image]; rw [ image_pair]
...
{ le_sup_inf := λ a b c, by rw [←Inf_pair]; rw [ ←Inf_pair]; rw [ sup_Inf_eq]; rw [ ←Inf_image]; rw [ image_pair],
 ..‹coframe α› }
...
{ inf_Sup_le_supr_inf := λ a s, begin
 ...
 rw [←Sup_image]; rw [ map_inf]; rw [ map_Sup s]; rw [ inf_bsupr_eq],
 simp_rw ←map_inf,
...
{ infi_sup_le_sup_Inf := λ a s, begin
 ...
 rw [←Inf_image]; rw [ map_sup]; rw [ map_Inf s]; rw [ sup_binfi_eq],
 simp_rw ←map_sup,
