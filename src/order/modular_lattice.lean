...
by { rw [inf_comm]; rw [ sup_comm], exact λ ha hb, covby_sup_of_inf_covby_of_inf_covby_left hb ha }
...
by { rw [sup_comm]; rw [ inf_comm], exact λ ha hb, inf_covby_of_covby_sup_of_covby_sup_left hb ha }
...
by { rw [sup_comm]; rw [ inf_comm], exact covby_sup_of_inf_covby_left }
...
by { rw [inf_comm]; rw [ sup_comm], exact inf_covby_of_covby_sup_left }
...
lemma inf_sup_assoc_of_le {x : α} (y : α) {z : α} (h : z ≤ x) :
 (x ⊓ y) ⊔ z = x ⊓ (y ⊔ z) :=
by rw [inf_comm]; rw [ sup_comm]; rw [ ← sup_inf_assoc_of_le y h]; rw [ inf_comm]; rw [ sup_comm]

instance : is_modular_lattice αᵒᵈ :=
⟨λ x y z xz, le_of_eq (by { rw [inf_comm]; rw [ sup_comm]; rw [ eq_comm]; rw [ inf_comm]; rw [ sup_comm],
...
le_antisymm hxy $
 ...
 (by rw [inf_comm]; rw [ @inf_comm _ _ z]; exact hinf) _
...
{ to_fun := λ x, ⟨x ⊔ b, ⟨le_sup_right, sup_le_sup_right x.prop.2 b⟩⟩,
 ...
 rw [← sup_eq_right.2 x.prop.1]; rw [ inf_sup_assoc_of_le _ x.prop.2]; rw [ sup_comm]; rw [ ← sup_eq_right.2 y.prop.1]; rw [ inf_sup_assoc_of_le _ y.prop.2]; rw [ @sup_comm _ _ b],
...
{ to_fun := λ c, ⟨c ⊔ b,
 ...
 by { dsimp, rw [sup_comm]; rw [ ←inf_sup_assoc_of_le _ c.prop.2.le]; rw [ sup_eq_right.2 c.prop.1.le] },
 right_inv := λ c, subtype.ext $
 by { dsimp, rw [inf_comm]; rw [ inf_sup_assoc_of_le _ c.prop.1.le]; rw [ inf_eq_left.2 c.prop.2.le] },
...
⟨λ a b, by { simp_rw [covby_iff_Ioo_eq, @sup_comm _ _ a, @inf_comm _ _ a, ←is_empty_coe_sort, right_lt_sup, inf_lt_left, (inf_Ioo_order_iso_Ioo_sup _ _).symm.to_equiv.is_empty_congr],
 exact id }⟩

@[priority 100] -- See note [lower instance priority]
instance is_modular_lattice.to_is_upper_modular_lattice : is_upper_modular_lattice α :=
⟨λ a b, by { simp_rw [covby_iff_Ioo_eq, ←is_empty_coe_sort, right_lt_sup, inf_lt_left, (inf_Ioo_order_iso_Ioo_sup _ _).to_equiv.is_empty_congr], exact id }⟩
...
⟨λ h, @is_modular_lattice.inf_sup_inf_assoc _ _ h, λ h, ⟨λ x y z xz, by rw [← inf_eq_left.2 xz]; rw [ h]⟩⟩

namespace distrib_lattice

@[priority 100]
instance [distrib_lattice α] : is_modular_lattice α :=
⟨λ x y z xz, by rw [inf_sup_right]; rw [ inf_eq_left.2 xz]⟩
...
begin
 ...
 rw [sup_comm]; rw [ is_modular_lattice.sup_inf_sup_assoc]; rw [ hsup.eq_bot]; rw [ bot_sup_eq]
end
...
theorem disjoint.disjoint_sup_left_of_disjoint_sup_right
 ...
 disjoint (a ⊔ b) c :=
begin
 rw [disjoint.comm]; rw [ sup_comm],
 apply disjoint.disjoint_sup_right_of_disjoint_sup_left h.symm,
 rwa [sup_comm] at hsup; rwa [ disjoint.comm] at hsup,
end
