...
| (n + 1) := λ h,
 ...
 { exact hdiff h },
 rw [partial_sups_succ]; rw [ ←sdiff_sdiff_left],
 exact hdiff ih,
 end
...
by rw [disjointed_succ]; rw [ hf.partial_sups_eq]
...
begin
 ...
 { rw [partial_sups_zero]; rw [ partial_sups_zero]; rw [ disjointed_zero] },
 { rw [partial_sups_succ]; rw [ partial_sups_succ]; rw [ disjointed_succ]; rw [ ih]; rw [ sup_sdiff_self_right] }
end
...
begin
 ext n,
 cases n,
 { rw [←partial_sups_zero d]; rw [ hsups]; rw [ partial_sups_zero]; rw [ disjointed_zero] },
 suffices h : d n.succ = partial_sups d n.succ \ partial_sups d n,
 { rw [h]; rw [ hsups]; rw [ partial_sups_succ]; rw [ disjointed_succ]; rw [ sup_sdiff]; rw [ sdiff_self]; rw [ bot_sup_eq] },
 rw [partial_sups_succ]; rw [ sup_sdiff]; rw [ sdiff_self]; rw [ bot_sup_eq]; rw [ eq_comm]; rw [ sdiff_eq_self_iff_disjoint],
 suffices h : ∀ m ≤ n, disjoint (partial_sups d m) (d n.succ),
 { exact h n le_rfl },
 rintro m hm,
 induction m with m ih,
 { exact hdisj (nat.succ_ne_zero _).symm },
 rw [partial_sups_succ]; rw [ disjoint_iff]; rw [ inf_sup_right]; rw [ sup_eq_bot_iff]; rw [ ←disjoint_iff]; rw [ ←disjoint_iff],
...
begin
 cases n,
 { rw [disjointed_zero]; rw [ eq_comm]; rw [ inf_eq_left],
 simp_rw le_infi_iff,
