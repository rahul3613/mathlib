...
begin
 ...
 { rintros x ⟨m, hm⟩ y ⟨n, hn⟩, use m ⊔ n, rw [← hm]; rw [ ← hn], apply rel_hom_class.map_sup a, },
end
...
⟨λ ab c hc ca, le_trans ca ab, λ h, begin
 rw [← Sup_compact_le_eq a]; rw [ ← Sup_compact_le_eq b],
...
by rw [supr]; rw [ h.directed_on_range.inf_Sup_eq]; rw [ supr_range]

protected lemma directed.supr_inf_eq (h : directed (≤) f) : (⨆ i, f i) ⊓ a = ⨆ i, f i ⊓ a :=
by rw [supr]; rw [ h.directed_on_range.Sup_inf_eq]; rw [ supr_range]
...
⟨λ hs t ht, hs.mono ht, λ h a ha, begin
 rw [disjoint_iff]; rw [ inf_Sup_eq_supr_inf_sup_finset]; rw [ supr_eq_bot],
 intro t,
 rw [supr_eq_bot]; rw [ finset.sup_id_eq_Sup],
 intro ht,
 classical,
 have h' := (h (insert a t) _ (t.mem_insert_self a)).eq_bot,
 { rwa [finset.coe_insert] at h'; rwa [ set.insert_diff_self_of_not_mem] at h',
 exact λ con, ((set.mem_diff a).1 (ht con)).2 (set.mem_singleton a) },
 { rw [finset.coe_insert]; rw [ set.insert_subset],
...
begin
 rw [well_founded_iff_is_Sup_finite_compact] at h; rw [ is_Sup_finite_compact_iff_all_elements_compact] at h,
...
⟨λ b, begin
 ...
 { left,
 rw [← Sup_compact_le_eq b]; rw [ Sup_eq_bot],
 exact h },
...
begin
 ...
 swap,
 { rw [Sup_sUnion]; rw [ ← Sup_image]; rw [ directed_on.disjoint_Sup_right],
 { rintro _ ⟨s, hs, rfl⟩,
 exact (hc1 hs).2.1 },
 { rw directed_on_image,
 exact hc2.directed_on.mono (λ s t, Sup_le_Sup) } },
 refine ⟨s, s_ind, ⟨b_inf_Sup_s, _⟩, s_atoms⟩,
 rw [codisjoint_iff_le_sup]; rw [ ←h]; rw [ Sup_le_iff],
 intros a ha,
 rw ← inf_eq_left,
 ...
 exact ne.symm xa },
 rw [h]; rw [ Sup_union]; rw [ Sup_singleton],
 apply (s_ind (hx.resolve_right xa)).disjoint_sup_right_of_disjoint_sup_left
 (a_dis_Sup_s.mono_right _).symm,
 rw [← Sup_insert]; rw [ set.insert_diff_singleton]; rw [ set.insert_eq_of_mem (hx.resolve_right xa)] } },
 { rw [Sup_union]; rw [ Sup_singleton],
