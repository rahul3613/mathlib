...
by { rw [le_is_glb_iff (succ_fn_spec i)]; rw [ mem_lower_bounds], exact λ x hx, (le_of_lt hx), }
...
by { have h := succ_fn_spec i, rw [is_glb] at h; rw [ is_greatest] at h; rw [ mem_lower_bounds] at h, exact h.1 j hij, }
...
{ exists_succ_iterate_of_le := λ i j hij,
 ...
 rw [function.iterate_succ']; rw [ function.comp_app],
...
by { rw [to_Z_of_ge hi]; rw [ int.to_nat_coe_nat], exact nat.find_spec (exists_succ_iterate_of_le hi), }

lemma iterate_pred_to_Z (i : ι) (hi : i < i0) : pred^[(- to_Z i0 i).to_nat] i0 = i :=
begin
 rw [to_Z_of_lt hi]; rw [ neg_neg]; rw [ int.to_nat_coe_nat],
...
begin
 refine lt_of_le_of_ne _ _,
 { rw [to_Z_of_lt hi]; rw [ neg_nonpos], exact nat.cast_nonneg _, },
 { by_contra,
...
begin
 ...
 rw [h_eq]; rw [ to_Z_of_eq],
 simp only [right.neg_nonpos_iff, nat.cast_nonneg],},
 { rw [to_Z_of_lt h]; rw [ neg_le_neg_iff],
 norm_cast,
...
begin
 ...
 { nth_rewrite 1 ← hmn,
 simp_rw [m],
 rw [int.to_nat_eq_max]; rw [ to_Z_of_ge (le_succ_iterate _ _)]; rw [ max_eq_left],
...
begin
 ...
 { rwa [function.iterate_zero]; rwa [ id.def], },
 ...
 { nth_rewrite 1 ← hmn,
 simp_rw [m],
 rw [int.to_nat_eq_max]; rw [ to_Z_of_lt this]; rw [ max_eq_left]; rw [ neg_neg],
 rw neg_neg,
 exact nat.cast_nonneg _, },
...
begin
 ...
 { rw [← iterate_succ_to_Z i hi]; rw [ ← iterate_succ_to_Z j hj],
 ...
 { rw [← iterate_pred_to_Z i hi]; rw [ ← iterate_pred_to_Z j hj],
...
begin
 ...
 push_neg at h,
 by_cases hm0 : m = 0,
 { rw [hm0] at hm; rw [ function.iterate_zero] at hm; rw [ id.def] at hm,
 rw hm at h,
 exact lt_irrefl _ h, },
 ...
 rwa [function.iterate_succ'] at h_succ_le; rwa [ function.comp_app] at h_succ_le; rwa [ iterate_succ_to_Z i hi] at h_succ_le, },
 { exact h.le, }, },
 ...
 exact nat.one_le_iff_ne_zero.mpr hm0, },
 rwa [function.iterate_succ']; rwa [ function.comp_app]; rwa [ iterate_pred_to_Z j hj]
 at h_le_pred, }, },
end
...
{ to_fun := λ i, (to_Z ⊥ i).to_nat,
 ...
 rw [← @to_Z_le_iff ι _ _ _ _ ⊥]; rw [ int.to_nat_of_nonneg (to_Z_nonneg bot_le)],
 end, }
...
{ to_fun := λ i, ⟨(to_Z ⊥ i).to_nat,
 ...
 { rw [← is_top_iff_is_max] at hn_max; rw [ is_top_iff_eq_top] at hn_max,
 rw hn_max,
