...
by rw [le_iff_lt_or_eq]; rw [ h.lt_iff]
...
⟨λ ab c hc ca, le_trans ca ab, λ h, begin
 rw [← Sup_atoms_le_eq a]; rw [ ← Sup_atoms_le_eq b],
...
by rw [←gi.is_atom_iff hbot h_atom]; rw [ gi.l_u_eq]
...
by rw [is_simple_order_iff_is_atom_top]; rw [ is_simple_order_iff_is_atom_top]; rw [ ← f.is_atom_iff ⊤]; rw [ f.map_top]
...
⟨λ x, begin
 ...
 rw [← hb.is_atom_iff_is_coatom]; rw [ order_iso.is_atom_iff],
 apply ha.Iic }
end⟩
...
begin
 ...
 rw [is_atom_iff]; rw [ bot_eq_empty]; rw [ ←nonempty_iff_ne_empty],
 rintro ⟨⟨x, hx⟩, hs⟩,
...
{ eq_Sup_atoms := λ s, ⟨(λ x, {x}) '' s,
 by rw [Sup_eq_sUnion]; rw [ sUnion_image]; rw [ bUnion_of_singleton],
 by { rintro - ⟨x, hx, rfl⟩, exact is_atom_singleton x }⟩ }

instance : is_coatomistic (set α) :=
{ eq_Inf_coatoms := λ s, ⟨(λ x, {x}ᶜ) '' sᶜ,
 by rw [Inf_eq_sInter]; rw [ sInter_image]; rw [ ←compl_Union₂]; rw [ bUnion_of_singleton]; rw [ compl_compl],
