...
⟨λ c d, { fst := extent_closure r (c.snd ∩ d.snd),
 snd := c.snd ∩ d.snd,
 closure_fst := by rw [←c.closure_fst]; rw [ ←d.closure_fst]; rw [ ←intent_closure_union]; rw [ intent_closure_extent_closure_intent_closure],
 closure_snd := rfl }⟩
...
⟨λ c d, { fst := c.fst ∩ d.fst,
 ...
 closure_fst := rfl,
 closure_snd := by rw [←c.closure_snd]; rw [ ←d.closure_snd]; rw [ ←extent_closure_union]; rw [ extent_closure_intent_closure_extent_closure] }⟩
...
begin
 refine ⟨λ h, _, λ h, _⟩,
 { rw [←fst_subset_fst_iff]; rw [ ←c.closure_snd]; rw [ ←d.closure_snd],
 exact extent_closure_anti _ h },
 { rw [←c.closure_fst]; rw [ ←d.closure_fst],
 exact intent_closure_anti _ h }
end

@[simp] lemma snd_ssubset_snd_iff : c.snd ⊂ d.snd ↔ d < c :=
by rw [ssubset_iff_subset_not_subset]; rw [ lt_iff_le_not_le]; rw [ snd_subset_snd_iff]; rw [ snd_subset_snd_iff]
...
⟨λ S, { fst := ⋂ c ∈ S, (c : concept _ _ _).fst,
 ...
 closure_fst := rfl,
 closure_snd := by simp_rw [←closure_snd, ←extent_closure_Union₂, extent_closure_intent_closure_extent_closure] }⟩
