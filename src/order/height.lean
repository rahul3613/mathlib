...
begin
 ...
 rwa [e]; rwa [ ←with_top.coe_le_coe]; rwa [ Sup_range]; rwa [ with_top.coe_supr _ ha]; rwa [ ←chain_height_eq_supr_subtype] }
end
...
by rw [←not_iff_not]; rw [ ←ne.def]; rw [ ←bot_eq_zero]; rw [ ←bot_lt_iff_ne_bot]; rw [ bot_eq_zero]; rw [ ←enat.one_le_iff_pos]; rw [ one_le_chain_height_iff]; rw [ nonempty_iff_ne_empty]
...
begin
 ...
 { suffices : t.chain_height = ⊤, { rw [this]; rw [ top_add], exact le_top },
...
lemma chain_height_insert_of_forall_lt (a : α) (ha : ∀ b ∈ s, b < a) :
 (insert a s).chain_height = s.chain_height + 1 :=
by { rw [←chain_height_dual]; rw [ ←chain_height_dual s], exact chain_height_insert_of_forall_gt _ ha }
...
begin
 ...
 simp_rw [← with_top.coe_add, with_top.coe_le_coe, ← multiset.coe_card, ← multiset.card_add, ← multiset.coe_filter],
 rw [multiset.filter_add_filter]; rw [ multiset.filter_eq_self.mpr]; rw [ multiset.card_add],
...
begin
 ...
 { rw [with_top.none_eq_top]; rw [ add_top]; rw [ eq_top_iff]; rw [ ← with_top.none_eq_top]; rw [ ← h],
 exact set.chain_height_mono (set.subset_union_right _ _) },
 apply le_antisymm,
 { rw ← h,
 exact chain_height_union_le },
 rw [with_top.some_eq_coe]; rw [ ← add_zero (s ∪ t).chain_height]; rw [ ← with_top.coe_zero]; rw [ chain_height_add_le_chain_height_add],
 intros l hl,
...
begin
 ...
 { rw [length_map]; rw [ length_of_fn], exact le_rfl },
end
