...
begin
 rw [←to_dual_lt_to_dual_iff]; rw [ with_bot.lt_iff_exists_coe]; rw [ order_dual.exists],
...
lemma map_le_iff [preorder α] [preorder β] (f : α → β)
 ...
 a.map f ≤ b.map f ↔ a ≤ b :=
begin
 rw [←to_dual_le_to_dual_iff]; rw [ to_dual_map]; rw [ to_dual_map]; rw [ with_bot.map_le_iff]; rw [ to_dual_le_to_dual_iff],
 simp [mono_iff]
end
