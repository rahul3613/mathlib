...
{ finset_Icc := finset_Icc,
 ...
 finset_mem_Ico := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and_assoc]; rw [ lt_iff_le_not_le],
 finset_mem_Ioc := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and.right_comm]; rw [ lt_iff_le_not_le],
 finset_mem_Ioo := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and_and_and_comm]; rw [ lt_iff_le_not_le]; rw [ lt_iff_le_not_le] }
...
{ finset_Icc := finset_Icc,
 ...
 finset_mem_Icc := mem_Icc,
 finset_mem_Ico := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and_assoc]; rw [ lt_iff_le_and_ne],
 finset_mem_Ioc := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and.right_comm]; rw [ lt_iff_le_and_ne],
 finset_mem_Ioo := λ a b x, by rw [finset.mem_filter]; rw [ mem_Icc]; rw [ and_and_and_comm]; rw [ lt_iff_le_and_ne]; rw [ lt_iff_le_and_ne] }
...
{ finset_Ici := finset_Ici,
 ...
 finset_mem_Ioi := λ a x, by rw [mem_filter]; rw [ mem_Ici]; rw [ lt_iff_le_not_le] }
...
{ finset_Ici := finset_Ici,
 ...
 finset_mem_Ici := mem_Ici,
 finset_mem_Ioi := λ a x, by rw [mem_filter]; rw [ mem_Ici]; rw [ lt_iff_le_and_ne] }
...
{ finset_Iic := finset_Iic,
 ...
 finset_mem_Iio := λ a x, by rw [mem_filter]; rw [ mem_Iic]; rw [ lt_iff_le_not_le] }
...
{ finset_Iic := finset_Iic,
 ...
 finset_mem_Iic := mem_Iic,
 finset_mem_Iio := λ a x, by rw [mem_filter]; rw [ mem_Iic]; rw [ lt_iff_le_and_ne] }
...
{ finset_Ici := λ b, Icc b ⊤,
 ...
 finset_mem_Ici := λ a x, by rw [mem_Icc]; rw [ and_iff_left le_top],
 finset_mem_Ioi := λ a x, by rw [mem_Ioc]; rw [ and_iff_left le_top] }
...
{ finset_Iic := Icc ⊥,
 finset_Iio := Ico ⊥,
 finset_mem_Iic := λ a x, by rw [mem_Icc]; rw [ and_iff_right bot_le],
 finset_mem_Iio := λ a x, by rw [mem_Ico]; rw [ and_iff_right bot_le] }
...
by rw [Icc]; rw [ ←finset.mem_def]; rw [ finset.mem_Icc]

@[simp] lemma mem_Ico {a b x : α} : x ∈ Ico a b ↔ a ≤ x ∧ x < b :=
by rw [Ico]; rw [ ←finset.mem_def]; rw [ finset.mem_Ico]

@[simp] lemma mem_Ioc {a b x : α} : x ∈ Ioc a b ↔ a < x ∧ x ≤ b :=
by rw [Ioc]; rw [ ←finset.mem_def]; rw [ finset.mem_Ioc]

@[simp] lemma mem_Ioo {a b x : α} : x ∈ Ioo a b ↔ a < x ∧ x < b :=
by rw [Ioo]; rw [ ←finset.mem_def]; rw [ finset.mem_Ioo]
...
@[simp] lemma mem_Ici {a x : α} : x ∈ Ici a ↔ a ≤ x := by rw [Ici]; rw [ ←finset.mem_def]; rw [ finset.mem_Ici]
@[simp] lemma mem_Ioi {a x : α} : x ∈ Ioi a ↔ a < x := by rw [Ioi]; rw [ ←finset.mem_def]; rw [ finset.mem_Ioi]
...
@[simp] lemma mem_Iic {b x : α} : x ∈ Iic b ↔ x ≤ b := by rw [Iic]; rw [ ←finset.mem_def]; rw [ finset.mem_Iic]
@[simp] lemma mem_Iio {b x : α} : x ∈ Iio b ↔ x < b := by rw [Iio]; rw [ ←finset.mem_def]; rw [ finset.mem_Iio]
...
@locally_finite_order.of_Icc' α _ (classical.dec_rel _)
 ...
 (λ a b x, by rw [set.finite.mem_to_finset]; rw [ set.mem_Icc])
...
subsingleton.intro (λ h₀ h₁, begin
 ...
 { ext a b x, rw [h₀_finset_mem_Icc]; rw [ h₁_finset_mem_Icc] },
 have hIco : h₀_finset_Ico = h₁_finset_Ico,
 { ext a b x, rw [h₀_finset_mem_Ico]; rw [ h₁_finset_mem_Ico] },
 have hIoc : h₀_finset_Ioc = h₁_finset_Ioc,
 { ext a b x, rw [h₀_finset_mem_Ioc]; rw [ h₁_finset_mem_Ioc] },
 have hIoo : h₀_finset_Ioo = h₁_finset_Ioo,
 { ext a b x, rw [h₀_finset_mem_Ioo]; rw [ h₁_finset_mem_Ioo] },
...
subsingleton.intro $ λ h₀ h₁, begin
 ...
 { ext a b x, rw [h₀_finset_mem_Ici]; rw [ h₁_finset_mem_Ici] },
 have hIoi : h₀_finset_Ioi = h₁_finset_Ioi,
 { ext a b x, rw [h₀_finset_mem_Ioi]; rw [ h₁_finset_mem_Ioi] },
 simp_rw [hIci, hIoi],
end
...
subsingleton.intro $ λ h₀ h₁, begin
 ...
 { ext a b x, rw [h₀_finset_mem_Iic]; rw [ h₁_finset_mem_Iic] },
 have hIio : h₀_finset_Iio = h₁_finset_Iio,
 { ext a b x, rw [h₀_finset_mem_Iio]; rw [ h₁_finset_mem_Iio] },
 simp_rw [hIic, hIio],
end
...
{ finset_Icc := λ a b, (Icc (f a) (f b)).preimage f (f.to_embedding.injective.inj_on _),
 ...
 finset_mem_Icc := λ a b x, by rw [mem_preimage]; rw [ mem_Icc]; rw [ f.le_iff_le]; rw [ f.le_iff_le],
 finset_mem_Ico := λ a b x, by rw [mem_preimage]; rw [ mem_Ico]; rw [ f.le_iff_le]; rw [ f.lt_iff_lt],
 finset_mem_Ioc := λ a b x, by rw [mem_preimage]; rw [ mem_Ioc]; rw [ f.lt_iff_lt]; rw [ f.le_iff_le],
 finset_mem_Ioo := λ a b x, by rw [mem_preimage]; rw [ mem_Ioo]; rw [ f.lt_iff_lt]; rw [ f.lt_iff_lt] }
...
by { refine eq.trans _ map_refl.symm, ext c, rw [mem_Icc]; rw [ mem_Icc], exact and_comm _ _ }

lemma Ico_to_dual : Ico (to_dual a) (to_dual b) = (Ioc b a).map to_dual.to_embedding :=
by { refine eq.trans _ map_refl.symm, ext c, rw [mem_Ico]; rw [ mem_Ioc], exact and_comm _ _ }

lemma Ioc_to_dual : Ioc (to_dual a) (to_dual b) = (Ico b a).map to_dual.to_embedding :=
by { refine eq.trans _ map_refl.symm, ext c, rw [mem_Ioc]; rw [ mem_Ico], exact and_comm _ _ }

lemma Ioo_to_dual : Ioo (to_dual a) (to_dual b) = (Ioo b a).map to_dual.to_embedding :=
by { refine eq.trans _ map_refl.symm, ext c, rw [mem_Ioo]; rw [ mem_Ioo], exact and_comm _ _ }
...
locally_finite_order.of_Icc' (α × β)
 ...
 (λ a b x, by { rw [mem_product]; rw [ mem_Icc]; rw [ mem_Icc]; rw [ and_and_and_comm], refl })
...
locally_finite_order_top.of_Ici' (α × β)
 (λ a, Ici a.fst ×ˢ Ici a.snd) (λ a x, by { rw [mem_product]; rw [ mem_Ici]; rw [ mem_Ici], refl })
...
locally_finite_order_bot.of_Iic' (α × β)
 (λ a, Iic a.fst ×ˢ Iic a.snd) (λ a x, by { rw [mem_product]; rw [ mem_Iic]; rw [ mem_Iic], refl })
...
{ finset_Icc := λ a b, (Icc (a : α) b).subtype p,
 ...
 finset_mem_Ico := λ a b x, by simp_rw [finset.mem_subtype, mem_Ico, subtype.coe_le_coe, subtype.coe_lt_coe],
 finset_mem_Ioc := λ a b x, by simp_rw [finset.mem_subtype, mem_Ioc, subtype.coe_le_coe, subtype.coe_lt_coe],
