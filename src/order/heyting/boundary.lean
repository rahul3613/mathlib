...
@[simp] lemma boundary_top : ∂ (⊤ : α) = ⊥ := by rw [boundary]; rw [ hnot_top]; rw [ inf_bot_eq]
...
@[simp] lemma hnot_boundary (a : α) : ￢ ∂ a = ⊤ := by rw [boundary]; rw [ hnot_inf_distrib]; rw [ sup_hnot_self]

/-- **Leibniz rule** for the co-Heyting boundary. -/
lemma boundary_inf (a b : α) : ∂ (a ⊓ b) = ∂ a ⊓ b ⊔ a ⊓ ∂ b :=
by { unfold boundary, rw [hnot_inf_distrib]; rw [ inf_sup_left]; rw [ inf_right_comm]; rw [ ←inf_assoc] }
...
begin
 ...
 { rw [hnot_le_iff_codisjoint_right]; rw [ codisjoint_left_comm],
...
by { rw [@sup_comm _ _ a]; rw [ inf_comm], exact boundary_le_boundary_sup_sup_boundary_inf_left }
...
@[simp] lemma boundary_idem (a : α) : ∂ ∂ a = ∂ a := by rw [boundary]; rw [ hnot_boundary]; rw [ inf_top_eq]

lemma hnot_hnot_sup_boundary (a : α) : ￢￢a ⊔ ∂ a = a :=
by { rw [boundary]; rw [ sup_inf_left]; rw [ hnot_sup_self]; rw [ inf_top_eq]; rw [ sup_eq_right], exact hnot_hnot_le }

lemma hnot_eq_top_iff_exists_boundary : ￢a = ⊤ ↔ ∃ b, ∂ b = a :=
⟨λ h, ⟨a, by rw [boundary]; rw [ h]; rw [ inf_top_eq]⟩, by { rintro ⟨b, rfl⟩, exact hnot_boundary _ }⟩
