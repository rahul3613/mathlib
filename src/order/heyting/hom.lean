...
{ map_top := λ f, by rw [←@himp_self α _ ⊥]; rw [ ←himp_self]; rw [ map_himp],
...
{ map_bot := λ f, by rw [←@sdiff_self α _ ⊤]; rw [ ←sdiff_self]; rw [ map_sdiff],
...
instance biheyting_hom_class.to_heyting_hom_class [biheyting_algebra α] [biheyting_algebra β]
 ...
 heyting_hom_class F α β :=
{ map_bot := λ f, by rw [←@sdiff_self α _ ⊤]; rw [ ←sdiff_self]; rw [ biheyting_hom_class.map_sdiff],
 ..‹biheyting_hom_class F α β› }
...
def bounded_lattice_hom_class.to_biheyting_hom_class [boolean_algebra α] [boolean_algebra β]
 ...
 biheyting_hom_class F α β :=
{ map_himp := λ f a b, by rw [himp_eq]; rw [ himp_eq]; rw [ map_sup]; rw [ (is_compl_compl.map _).compl_eq],
 map_sdiff := λ f a b, by rw [sdiff_eq]; rw [ sdiff_eq]; rw [ map_inf]; rw [ (is_compl_compl.map _).compl_eq],
 ..‹bounded_lattice_hom_class F α β› }
...
@[simp] lemma map_compl (a : α) : f aᶜ = (f a)ᶜ := by rw [←himp_bot]; rw [ ←himp_bot]; rw [ map_himp]; rw [ map_bot]
...
by rw [←top_sdiff']; rw [ ←top_sdiff']; rw [ map_sdiff]; rw [ map_top]
...
⟨λ h, heyting_hom.ext $ λ a, hg $ by rw [←comp_apply]; rw [ h]; rw [ comp_apply], congr_arg _⟩
...
lemma cancel_left (hg : injective g) : g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
⟨λ h, coheyting_hom.ext $ λ a, hg $ by rw [←comp_apply]; rw [ h]; rw [ comp_apply], congr_arg _⟩
...
lemma cancel_left (hg : injective g) : g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
⟨λ h, biheyting_hom.ext $ λ a, hg $ by rw [←comp_apply]; rw [ h]; rw [ comp_apply], congr_arg _⟩
