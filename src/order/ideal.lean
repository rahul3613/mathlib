...
⟨λ S, { to_lower_set := ⨅ s ∈ S, to_lower_set s,
 nonempty' := ⟨⊥, begin
 rw [lower_set.carrier_eq_coe]; rw [ lower_set.coe_infi₂]; rw [ set.mem_Inter₂],
 exact λ s _, s.bot_mem,
 end⟩,
 directed' := λ a ha b hb, ⟨a ⊔ b, ⟨
 begin
 rw [lower_set.carrier_eq_coe] at ⊢ ha hb; rw [ lower_set.coe_infi₂] at ⊢ ha hb; rw [ set.mem_Inter₂] at ⊢ ha hb,
...
{ ..ideal.lattice,
 ...
 refine ⟨λ s hs, _, λ s hs, by rwa [←coe_subset_coe]; rwa [ coe_Inf]; rwa [ subset_Inter₂_iff]⟩,
 rw [←coe_subset_coe]; rw [ coe_Inf],
