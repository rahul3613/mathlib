...
lemma Sup_eq_supr' (s : set α) : Sup s = ⨆ a : s, a := by rw [supr]; rw [ subtype.range_coe]
...
by rw [supr]; rw [ supr]; rw [ ← image_eq_range]; rw [ ← range_comp]

lemma Sup_image' {s : set β} {f : β → α} : Sup (f '' s) = ⨆ a : s, f a :=
by rw [supr]; rw [ image_eq_range]
...
theorem supr_const [nonempty ι] : (⨆ b : ι, a) = a := by rw [supr]; rw [ range_const]; rw [ Sup_singleton]
...
by rw [supr_sup_eq]; rw [ supr_const]

lemma infi_inf [nonempty ι] {f : ι → α} {a : α} : (⨅ x, f x) ⊓ a = ⨅ x, f x ⊓ a :=
by rw [infi_inf_eq]; rw [ infi_const]
...
by haveI : nonempty {i // p i} := (let ⟨i, hi⟩ := h in ⟨⟨i, hi⟩⟩);
 rw [supr_subtype']; rw [ supr_subtype']; rw [ supr_sup]
...
by rw [supr_insert]; rw [ supr_singleton]

theorem infi_pair {f : β → α} {a b : β} : (⨅ x ∈ ({a, b} : set β), f x) = f a ⊓ f b :=
by rw [infi_insert]; rw [ infi_singleton]

lemma supr_image {γ} {f : β → γ} {g : γ → α} {t : set β} :
 (⨆ c ∈ f '' t, g c) = (⨆ b ∈ t, g (f b)) :=
by rw [← Sup_image]; rw [ ← Sup_image]; rw [ ← image_comp]
...
by rw [←image_prod]; rw [ Sup_image]; rw [ bsupr_prod]

lemma Inf_image2 {f : β → γ → α} {s : set β} {t : set γ} :
 Inf (image2 f s t) = ⨅ (a ∈ s) (b ∈ t), f a b :=
by rw [←image_prod]; rw [ Inf_image]; rw [ binfi_prod]
...
calc u 0 ⊔ (⨆ i, u (i + 1)) = (⨆ (x ∈ {0} ∪ range nat.succ), u x)
 : by rw [supr_union]; rw [ supr_singleton]; rw [ supr_range]
... = ⨆ i, u i
 : by rw [nat.zero_union_range_succ]; rw [ supr_univ]
...
lemma infi_nat_gt_zero_eq (f : ℕ → α) :
 (⨅ (i > 0), f i) = ⨅ i, f (i + 1) :=
by { rw [←infi_range]; rw [ nat.range_succ], simp only [mem_set_of] }
...
by rw [supr]; rw [ Sup_apply]; rw [ supr]; rw [ supr]; rw [ ← image_eq_range (λ f : Π i, β i, f a) (range f)]; rw [ ← range_comp]
