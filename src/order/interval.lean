...
by rw [←set_like.mem_coe]; rw [ coe_pure]; rw [ mem_singleton_iff]
...
{ inf := λ s t, match s, t with
 ...
 rw [dif_pos]; rw [ with_bot.coe_le_coe],
...
begin
 cases s,
 { rw [with_bot.none_eq_bot]; rw [ bot_inf_eq],
 exact (empty_inter _).symm },
 cases t,
 { rw [with_bot.none_eq_bot]; rw [ inf_bot_eq],
...
begin
 classical,
 rw [disjoint_iff_inf_le]; rw [ disjoint_iff_inf_le]; rw [ le_eq_subset]; rw [ ←coe_subset_coe]; rw [ coe_inf], refl
end
...
by rw [←interval.coe_inj]; rw [ coe_pure_interval]
...
by classical; exact { Sup := λ S, if h : S ⊆ {⊥} then ⊥ else some
 ...
 rw [not_and_distrib] at h; rw [ not_not] at h,
 cases h,
 { exact ha _ h },
