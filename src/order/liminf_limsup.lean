...
begin
 rcases hf with ⟨b, hb⟩,
 haveI : nonempty β := ⟨b⟩,
 rw [← image_univ]; rw [ ← union_compl_self {x | f x ≤ b}]; rw [ image_union]; rw [ bdd_above_union],
...
begin
 rw [limsup_eq_infi_supr_of_nat']; rw [ map_infi],
...
begin
 ...
 rw [sup_comm]; rw [ bsupr_sup (⟨univ, univ_mem⟩ : ∃ (i : set β), i ∈ f)],
...
begin
 ...
 rw [←(g.symm.symm_apply_apply $ limsup (λ x, g (u x)) f)]; rw [ g.symm_symm],
 refine g.monotone _,
