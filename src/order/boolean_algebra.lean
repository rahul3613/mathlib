...
by rw [sup_comm]; rw [ sup_inf_sdiff]
@[simp] theorem inf_sdiff_inf (x y : α) : (x \ y) ⊓ (x ⊓ y) = ⊥ :=
by rw [inf_comm]; rw [ inf_inf_sdiff]

@[priority 100] -- see Note [lower instance priority]
instance generalized_boolean_algebra.to_order_bot : order_bot α :=
{ bot_le := λ a, by { rw [←inf_inf_sdiff a a]; rw [ inf_assoc], exact inf_le_left },
...
begin
 conv_rhs at s { rw [←sup_inf_sdiff x y]; rw [ sup_comm] },
 rw sup_comm at s,
 conv_rhs at i { rw [←inf_inf_sdiff x y]; rw [ inf_comm] },
 rw inf_comm at i,
...
eq.symm $
 ...
 ... = (x ⊓ y ⊓ (x \ y)) ⊔ (x ⊓ (y \ x) ⊓ (x \ y)) : by rw [inf_sup_right]; rw [ @inf_comm _ _ x y]
 ... = x ⊓ (y \ x) ⊓ (x \ y) : by rw [inf_inf_sdiff]; rw [ bot_sup_eq]
...
calc x ⊓ (y \ x) = ((x ⊓ y) ⊔ (x \ y)) ⊓ (y \ x) : by rw sup_inf_sdiff
 ...
 ... = ⊥ : by rw [@inf_comm _ _ x y]; rw [ inf_inf_sdiff]; rw [ sdiff_inf_sdiff]; rw [ bot_sup_eq]
@[simp] theorem inf_sdiff_self_left : (y \ x) ⊓ x = ⊥ := by rw [inf_comm]; rw [ inf_sdiff_self_right]
...
{ sdiff := (\),
 ...
 ... = (x ⊓ (y \ x)) ⊔ (z ⊓ (y \ x))
 : by rw [inf_eq_right.2 h]; rw [ inf_sdiff_self_right]; rw [ bot_sup_eq]
 ... = (x ⊔ z) ⊓ (y \ x) : inf_sup_right.symm))
 (calc y ⊔ y \ x = y : sup_of_le_left sdiff_le'
 ... ≤ y ⊔ (x ⊔ z) : le_sup_left
 ... = ((y \ x) ⊔ x) ⊔ z : by rw [←sup_assoc]; rw [ ←@sdiff_sup_self' _ x y]
 ... = x ⊔ z ⊔ y \ x : by ac_refl),
 ...
 ... ≤ z ⊔ x : by rw [sup_assoc]; rw [ sup_comm]; rw [ sup_assoc]; rw [ sup_idem])⟩,
...
sdiff_unique (by rw [h]; rw [ hs]) (by rw [h]; rw [ hi.eq_bot])
...
sdiff_unique
 ...
 rwa [sup_inf_right]; rwa [ inf_sup_right]; rwa [ @sup_comm _ _ x]; rwa [ inf_sup_self]; rwa [ inf_comm]; rwa [ @sup_comm _ _ z]; rwa [ hs]; rwa [ sup_eq_left],
 end)
 (by rw [inf_assoc]; rw [ hd.eq_bot]; rw [ inf_bot_eq])
...
sdiff_unique
 ...
 ... = (y ⊓ z ⊔ y) ⊓ (y ⊓ x ⊔ y) : by rw [sup_inf_sdiff]; rw [ sup_inf_sdiff]
 ... = (y ⊔ y ⊓ z) ⊓ (y ⊔ y ⊓ x) : by ac_refl
 ... = y : by rw [sup_inf_self]; rw [ sup_inf_self]; rw [ inf_idem])
 (calc y ⊓ (x ⊔ z) ⊓ ((y \ x) ⊓ (y \ z)) =
 (y ⊓ x ⊔ y ⊓ z) ⊓ ((y \ x) ⊓ (y \ z)) : by rw inf_sup_left
 ... = ((y ⊓ x) ⊓ ((y \ x) ⊓ (y \ z))) ⊔ ((y ⊓ z) ⊓ ((y \ x) ⊓ (y \ z))) : by rw inf_sup_right
 ... = ((y ⊓ x) ⊓ (y \ x) ⊓ (y \ z)) ⊔ ((y \ x) ⊓ ((y \ z) ⊓ (y ⊓ z))) : by ac_refl
 ... = ⊥ : by rw [inf_inf_sdiff]; rw [ bot_inf_eq]; rw [ bot_sup_eq]; rw [ @inf_comm _ _ (y \ z)]; rw [ inf_inf_sdiff]; rw [ inf_bot_eq])

lemma sdiff_eq_sdiff_iff_inf_eq_inf : y \ x = y \ z ↔ y ⊓ x = y ⊓ z :=
⟨λ h, eq_of_inf_eq_sup_eq
 (by rw [inf_inf_sdiff]; rw [ h]; rw [ inf_inf_sdiff])
 (by rw [sup_inf_sdiff]; rw [ h]; rw [ sup_inf_sdiff]),
 λ h, by rw [←sdiff_inf_self_right]; rw [ ←sdiff_inf_self_right z y]; rw [ inf_comm]; rw [ h]; rw [ inf_comm]⟩

theorem sdiff_eq_self_iff_disjoint : x \ y = x ↔ disjoint y x :=
calc x \ y = x ↔ x \ y = x \ ⊥ : by rw sdiff_bot
 ... ↔ x ⊓ y = x ⊓ ⊥ : sdiff_eq_sdiff_iff_inf_eq_inf
 ... ↔ disjoint y x : by rw [inf_bot_eq]; rw [ inf_comm]; rw [ disjoint_iff]

theorem sdiff_eq_self_iff_disjoint' : x \ y = x ↔ disjoint x y :=
by rw [sdiff_eq_self_iff_disjoint]; rw [ disjoint.comm]
...
calc (x ⊓ y) ⊓ z ⊔ (y \ z) = x ⊓ (y ⊓ z) ⊔ (y \ z) : by rw inf_assoc
 ... = (x ⊔ (y \ z)) ⊓ y : by rw [sup_inf_right]; rw [ sup_inf_sdiff]
 ... = (x ⊓ y) ⊔ (y \ z) : by rw [inf_sup_right]; rw [ inf_sdiff_left]
...
begin
 ...
 ... = x ⊓ (y \ z ⊔ x ⊓ z ⊔ x \ y) : by rw [sup_inf_self]; rw [ sup_sdiff_left]; rw [ ←sup_assoc]
 ... = x ⊓ (y \ z ⊓ (z ⊔ y) ⊔ x ⊓ (z ⊔ y) ⊔ x \ y) :
 by rw [sup_inf_left]; rw [ sdiff_sup_self']; rw [ inf_sup_right]; rw [ @sup_comm _ _ y]
 ... = x ⊓ (y \ z ⊔ (x ⊓ z ⊔ x ⊓ y) ⊔ x \ y) :
 by rw [inf_sdiff_sup_right]; rw [ @inf_sup_left _ _ x z y]
 ... = x ⊓ (y \ z ⊔ (x ⊓ z ⊔ (x ⊓ y ⊔ x \ y))) : by ac_refl
 ... = x ⊓ (y \ z ⊔ (x ⊔ x ⊓ z)) : by rw [sup_inf_sdiff]; rw [ @sup_comm _ _ (x ⊓ z)]
 ... = x : by rw [sup_inf_self]; rw [ sup_comm]; rw [ inf_sup_self] },
 { calc x ⊓ (y \ z) ⊓ (z ⊓ x ⊔ x \ y) =
 x ⊓ (y \ z) ⊓ (z ⊓ x) ⊔ x ⊓ (y \ z) ⊓ (x \ y) : by rw inf_sup_left
 ... = x ⊓ (y \ z ⊓ z ⊓ x) ⊔ x ⊓ (y \ z) ⊓ (x \ y) : by ac_refl
 ... = x ⊓ (y \ z) ⊓ (x \ y) : by rw [inf_sdiff_self_left]; rw [ bot_inf_eq]; rw [ inf_bot_eq]; rw [ bot_sup_eq]
 ... = x ⊓ (y \ z ⊓ y) ⊓ (x \ y) : by conv_lhs { rw ←inf_sdiff_left }
 ... = x ⊓ (y \ z ⊓ (y ⊓ (x \ y))) : by ac_refl
 ... = ⊥ : by rw [inf_sdiff_self_right]; rw [ inf_bot_eq]; rw [ inf_bot_eq] }
end

lemma sdiff_sdiff_right' : x \ (y \ z) = (x \ y) ⊔ (x ⊓ z) :=
calc x \ (y \ z) = (x \ y) ⊔ (x ⊓ y ⊓ z) : sdiff_sdiff_right
 ... = z ⊓ x ⊓ y ⊔ (x \ y) : by ac_refl
 ... = (x \ y) ⊔ (x ⊓ z) : by rw [sup_inf_inf_sdiff]; rw [ sup_comm]; rw [ inf_comm]

lemma sdiff_sdiff_eq_sdiff_sup (h : z ≤ x) : x \ (y \ z) = x \ y ⊔ z :=
by rw [sdiff_sdiff_right']; rw [ inf_eq_right.2 h]

@[simp] lemma sdiff_sdiff_right_self : x \ (x \ y) = x ⊓ y :=
by rw [sdiff_sdiff_right]; rw [ inf_idem]; rw [ sdiff_self]; rw [ bot_sup_eq]

lemma sdiff_sdiff_eq_self (h : y ≤ x) : x \ (x \ y) = y :=
by rw [sdiff_sdiff_right_self]; rw [ inf_of_le_right h]

lemma sdiff_eq_symm (hy : y ≤ x) (h : x \ y = z) : x \ z = y := by rw [←h]; rw [ sdiff_sdiff_eq_self hy]
...
by rw [←sdiff_sdiff_eq_self hxz]; rw [ h]; rw [ sdiff_sdiff_eq_self hyz]

lemma sdiff_sdiff_left' : (x \ y) \ z = (x \ y) ⊓ (x \ z) :=
by rw [sdiff_sdiff_left]; rw [ sdiff_sup]

lemma sdiff_sdiff_sup_sdiff : z \ (x \ y ⊔ y \ x) = z ⊓ (z \ x ⊔ y) ⊓ (z \ y ⊔ x) :=
calc z \ (x \ y ⊔ y \ x) = (z \ x ⊔ z ⊓ x ⊓ y) ⊓ (z \ y ⊔ z ⊓ y ⊓ x) :
 by rw [sdiff_sup]; rw [ sdiff_sdiff_right]; rw [ sdiff_sdiff_right]
... = z ⊓ (z \ x ⊔ y) ⊓ (z \ y ⊔ z ⊓ y ⊓ x) : by rw [sup_inf_left]; rw [ sup_comm]; rw [ sup_inf_sdiff]
... = z ⊓ (z \ x ⊔ y) ⊓ (z ⊓ (z \ y ⊔ x)) :
 by rw [sup_inf_left]; rw [ @sup_comm _ _ (z \ y)]; rw [ sup_inf_sdiff]
...
... = (z \ x ⊔ z ⊓ x ⊓ y) ⊓ (z \ y ⊔ z ⊓ y ⊓ x) : by rw [sdiff_sdiff_right]; rw [ sdiff_sdiff_right]
...
sdiff_unique
 ...
 by rw [sup_inf_right]; rw [ sup_sdiff_self_right]; rw [ inf_sup_right]; rw [ inf_sdiff_sup_right]
 ... = (y ⊓ (x ⊓ (x ⊔ z)) ⊔ x \ z) ⊓ (x ⊓ y ⊓ z ⊔ y \ z) : by ac_refl
 ... = ((y ⊓ x) ⊔ (x \ z)) ⊓ ((x ⊓ y) ⊔ (y \ z)) : by rw [inf_sup_self]; rw [ sup_inf_inf_sdiff]
 ... = (x ⊓ y) ⊔ ((x \ z) ⊓ (y \ z)) : by rw [@inf_comm _ _ y]; rw [ sup_inf_left]
 ... = x ⊓ y : sup_eq_left.2 (inf_le_inf sdiff_le sdiff_le))
 (calc (x ⊓ y) ⊓ z ⊓ ((x \ z) ⊓ (y \ z)) =
 x ⊓ y ⊓ (z ⊓ (x \ z)) ⊓ (y \ z) : by ac_refl
 ... = ⊥ : by rw [inf_sdiff_self_right]; rw [ inf_bot_eq]; rw [ bot_inf_eq])
...
sdiff_unique
 ...
 ... = ⊥ : by rw [inf_inf_sdiff]; rw [ inf_bot_eq])

lemma inf_sdiff_right_comm : x \ z ⊓ y = (x ⊓ y) \ z :=
by rw [@inf_comm _ _ x]; rw [ inf_comm]; rw [ inf_sdiff_assoc]

lemma inf_sdiff_distrib_left (a b c : α) : a ⊓ b \ c = (a ⊓ b) \ (a ⊓ c) :=
by rw [sdiff_inf]; rw [ sdiff_eq_bot_iff.2 inf_le_left]; rw [ bot_sup_eq]; rw [ inf_sdiff_assoc]
...
eq.symm $
 ...
 ... = (x ⊔ (y \ x)) ⊓ ((x \ y) ⊔ y) : by rw [sup_sdiff_right]; rw [ sup_sdiff_right]
 ... = x ⊔ y : by rw [sup_sdiff_self_right]; rw [ sup_sdiff_self_left]; rw [ inf_idem]
...
{ compl := λ a, ⊤ \ a,
 ...
 sdiff_eq := λ _ _, by { rw [←inf_sdiff_assoc]; rw [ inf_top_eq], refl },
...
{ sup_inf_sdiff := λ a b, by rw [sdiff_eq]; rw [ ←inf_sup_left]; rw [ sup_compl_eq_top]; rw [ inf_top_eq],
 inf_inf_sdiff := λ a b, by { rw [sdiff_eq]; rw [ ←inf_inf_distrib_left]; rw [ inf_compl_eq_bot']; rw [ inf_bot_eq],
 congr },
 ..‹boolean_algebra α› }
...
{ hnot := compl,
 le_himp_iff := λ a b c, by rw [himp_eq]; rw [ is_compl_compl.le_sup_right_iff_inf_left_le],
 himp_bot := λ _, himp_eq.trans bot_sup_eq,
 top_sdiff := λ a, by rw [sdiff_eq]; rw [ top_inf_eq],
...
by rw [eq_comm]; rw [ compl_eq_iff_is_compl]; rw [ eq_compl_iff_is_compl]
...
@[simp] lemma sdiff_compl : x \ yᶜ = x ⊓ y := by rw [sdiff_eq]; rw [ compl_compl]
...
by rw [← sdiff_eq]; rw [ sup_inf_sdiff _ _]

@[simp] lemma compl_sdiff : (x \ y)ᶜ = x ⇨ y :=
by rw [sdiff_eq]; rw [ himp_eq]; rw [ compl_inf]; rw [ compl_compl]; rw [ sup_comm]

@[simp] lemma compl_himp : (x ⇨ y)ᶜ = x \ y := @compl_sdiff αᵒᵈ _ _ _

@[simp] lemma compl_sdiff_compl : xᶜ \ yᶜ = y \ x := by rw [sdiff_compl]; rw [ sdiff_eq]; rw [ inf_comm]
@[simp] lemma compl_himp_compl : xᶜ ⇨ yᶜ = y ⇨ x := @compl_sdiff_compl αᵒᵈ _ _ _

lemma disjoint_compl_left_iff : disjoint xᶜ y ↔ y ≤ x :=
by rw [←le_compl_iff_disjoint_left]; rw [ compl_compl]

lemma disjoint_compl_right_iff : disjoint x yᶜ ↔ x ≤ y :=
by rw [←le_compl_iff_disjoint_right]; rw [ compl_compl]
...
{ sup_inf_sdiff := λ a b, hf $ by erw [map_sup]; erw [ map_sdiff]; erw [ map_inf]; erw [ sup_inf_sdiff],
 inf_inf_sdiff := λ a b, hf $ by erw [map_inf]; erw [ map_sdiff]; erw [ map_inf]; erw [ inf_inf_sdiff]; erw [ map_bot],
...
{ compl := compl,
 ...
 inf_compl_le_bot := λ a, ((map_inf _ _).trans $ by rw [map_compl]; rw [ inf_compl_eq_bot]; rw [ map_bot]).le,
 top_le_sup_compl := λ a, ((map_sup _ _).trans $ by rw [map_compl]; rw [ sup_compl_eq_top]; rw [ map_top]).ge,
