...
begin
 ...
 rw [s.strict_mono.le_iff_le]; rw [ s.strict_mono.le_iff_le],
 exact le_total i j
end
...
by rw [to_list]; rw [ list.length_of_fn]

lemma to_list_ne_nil (s : composition_series X) : s.to_list ≠ [] :=
by rw [← list.length_pos_iff_ne_nil]; rw [ length_to_list]; exact nat.succ_pos _
...
have h₂ : ∀ i : fin s₁.length.succ, (s₁ i) = s₂ (fin.cast (congr_arg nat.succ h₁) i),
 begin
 assume i,
 rw [← list.nth_le_of_fn s₁ i]; rw [ ← list.nth_le_of_fn s₂],
 simp [h]
 end,
begin
 cases s₁, cases s₂,
...
list.pairwise_iff_nth_le.2 (λ i j hi hij,
 begin
 dsimp [to_list],
 rw [list.nth_le_of_fn']; rw [ list.nth_le_of_fn'],
...
by rw [to_list]; rw [ list.mem_of_fn]; rw [ mem_def]
...
begin
 refine ext_fun _ _,
 { rw [length_of_list]; rw [ length_to_list]; rw [ nat.succ_sub_one] },
 { rintros ⟨i, hi⟩,
...
begin
 ...
 { rw [length_to_list]; rw [ length_of_list]; rw [ tsub_add_cancel_of_le (nat.succ_le_of_lt $ list.length_pos_of_ne_nil hl)] },
 { assume i hi hi',
 dsimp [of_list, to_list],
 rw [list.nth_le_of_fn'],
 refl }
...
begin
 ...
 { conv_rhs { rw [← nat.succ_sub (length_pos_of_mem_ne ⟨i, rfl⟩ s.top_mem hx)]; rw [ nat.succ_sub_one] },
...
{ length := s₁.length + s₂.length,
 ...
 { intro i,
 rw [append_succ_cast_add_aux _ _ _ h]; rw [ append_cast_add_aux],
 exact s₁.step i },
 { intro i,
 rw [append_nat_add_aux]; rw [ append_succ_nat_add_aux],
 exact s₂.step i }
 end }
...
{ length := s.length + 1,
 ...
 { rwa [fin.snoc_cast_succ]; rwa [ fin.succ_last]; rwa [ fin.snoc_last]; rwa [ ← top] },
 { intro i,
 rw [fin.snoc_cast_succ]; rw [ ← fin.cast_succ_fin_succ]; rw [ fin.snoc_cast_succ],
 exact s.step _ }
 end }
...
by rw [bot]; rw [ bot]; rw [ ← snoc_cast_succ s _ _ 0]; rw [ fin.cast_succ_zero]
...
⟨e, begin
 ...
 dsimp only [e],
 refine fin.last_cases _ (λ i, _) i,
 { erw [equiv.swap_apply_left]; erw [ snoc_cast_succ]; erw [ snoc_last]; erw [ fin.succ_last]; erw [ snoc_last]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_last]; erw [ snoc_last],
 exact hr₂ },
 { refine fin.last_cases _ (λ i, _) i,
 { erw [equiv.swap_apply_right]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_last]; erw [ snoc_last]; erw [ snoc_last]; erw [ fin.succ_last]; erw [ snoc_last],
 exact hr₁ },
 { erw [equiv.swap_apply_of_ne_of_ne h2 h1]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_cast_succ]; erw [ snoc_cast_succ]; erw [ fin.succ_cast_succ]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ]; erw [ snoc_cast_succ],
