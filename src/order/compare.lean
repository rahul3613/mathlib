...
by rw [←cmp_swap x']; rw [ ←cmp_swap x]; rw [ swap_inj]

lemma lt_iff_lt_of_cmp_eq_cmp (h : cmp x y = cmp x' y') : x < y ↔ x' < y' :=
by rw [←cmp_eq_lt_iff]; rw [ ←cmp_eq_lt_iff]; rw [ h]

lemma le_iff_le_of_cmp_eq_cmp (h : cmp x y = cmp x' y') : x ≤ y ↔ x' ≤ y' :=
by { rw [←not_lt]; rw [ ←not_lt], apply not_congr,
 apply lt_iff_lt_of_cmp_eq_cmp, rwa cmp_eq_cmp_symm }

lemma eq_iff_eq_of_cmp_eq_cmp (h : cmp x y = cmp x' y') : x = y ↔ x' = y' :=
by rw [le_antisymm_iff]; rw [ le_antisymm_iff]; rw [ le_iff_le_of_cmp_eq_cmp h]; rw [ le_iff_le_of_cmp_eq_cmp (cmp_eq_cmp_symm.1 h)]
