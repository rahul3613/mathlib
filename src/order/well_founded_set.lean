...
begin
 ...
 { refl },
 rw [ih]; rw [ ((min_bad_seq_of_bad_seq r rk s (m + k).succ (fs (m + k)).1 (fs (m + k)).2.1).2.1 m (nat.lt_succ_iff.2 (nat.add_le_add_left k.zero_le m)))],
 refl },
 refine ⟨λ n, (fs n).1 n, ⟨(λ n, ((fs n).2).1.1 n), λ m n mn, _⟩, λ n g hg1 hg2, _⟩,
 { dsimp,
 rw [← subtype.val_eq_coe]; rw [ h m n (le_of_lt mn)],
...
begin
 rw [iff_forall_not_is_bad_seq]; rw [ ← not_exists]; rw [ not_congr],
 split,
...
begin
 ...
 { intros l hl,
 rw [finset.mem_coe]; rw [ finset.mem_singleton]; rw [ list.eq_nil_iff_forall_not_mem],
 exact hl, },
 apply_instance },
 ...
 by_cases hn : n < g 0,
 { apply hf1.2 m n mn,
 rwa [if_pos hn] at hmn ; rwa [ if_pos (mn.trans hn)] at hmn },
 ...
 rw [← list.cons_head_tail (hnil (g (m - g 0)))]; rw [ ← list.cons_head_tail (hnil (g n'))],
