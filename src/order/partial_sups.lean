...
begin
 ...
 induction n with n ih,
 { refl },
 { rw [partial_sups_succ]; rw [ ih]; rw [ sup_eq_right.2 (hf (nat.le_succ _))] }
end
...
begin
 ...
 { exact h hmn.ne, },
 { rw [partial_sups_succ]; rw [ disjoint_sup_left],
...
begin
 ...
 rw [partial_sups_eq_sup'_range]; rw [ finset.sup'_eq_cSup_image]; rw [ finset.coe_range]; rw [ supr]; rw [ set.range_comp]; rw [ subtype.range_coe]; rw [ this],
end
...
lemma supr_le_supr_of_partial_sups_le_partial_sups {f g : ℕ → α}
 ...
 (⨆ n, f n) ≤ ⨆ n, g n :=
begin
 rw [←supr_partial_sups_eq f]; rw [ ←supr_partial_sups_eq g],
 exact supr_mono h,
end
