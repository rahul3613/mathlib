...
λ ⟨a₁, b₁⟩ ⟨a₂, b₂⟩, by rw [prod.swap]; rw [ game_add_mk_iff]; rw [ game_add_mk_iff]; rw [ or_comm]
...
⟨λ a₁ b₁ a₂ b₂, prod.game_add rα rα (a₁, b₁) (a₂, b₂) ∨ prod.game_add rα rα (b₁, a₁) (a₂, b₂),
 λ a₁ b₁ a₂ b₂, begin
 rw [prod.game_add_swap_swap_mk _ _ b₁ b₂ a₁ a₂]; rw [ prod.game_add_swap_swap_mk _ _ a₁ b₂ b₁ a₂],
 simp [or_comm]
 end⟩
