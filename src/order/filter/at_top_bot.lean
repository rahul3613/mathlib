...
begin
 ...
 rw [at_top] at hs; rw [ infi_unique] at hs; rw [ unique.default_eq x] at hs; rw [ mem_principal] at hs,
...
by rw [← e.comap_at_top]; rw [ map_comap_of_surjective e.surjective]
...
by rw [← e.comap_at_top]; rw [ tendsto_comap_iff]
...
by rw [tendsto_iff_comap]; rw [ comap_principal]; rw [ le_principal_iff]; rw [ mem_at_top_sets]; refl
...
by rw [← comap_embedding_at_top hm hu]; rw [ tendsto_comap_iff]
...
by rw [prod_map_map_eq]; rw [ prod_at_top_at_top_eq]; rw [ prod.map_def]
...
begin
 ...
 rw [← map_coe_at_top_of_Ici_subset (Ici_subset_Ioi.2 hb)]; rw [ comap_map subtype.coe_injective]
end
...
by rw [← map_coe_Ici_at_top a]; rw [ comap_map subtype.coe_injective]
...
show tendsto (f ∘ (λn, n + k)) at_top l ↔ tendsto f at_top l,
 by rw [← tendsto_map'_iff]; rw [ map_add_at_top_eq_nat]
...
map_at_top_eq_of_gc (λb, b * k + (k - 1)) 1
 ...
 (assume a b _,
 calc a / k ≤ b ↔ a / k < b + 1 : by rw [← nat.succ_eq_add_one]; rw [ nat.lt_succ_iff]
 ... ↔ a < (b + 1) * k : nat.div_lt_iff_lt_mul hk
 ... ↔ _ :
 begin
 cases k,
 exact (lt_irrefl _ hk).elim,
 rw [add_mul]; rw [ one_mul]; rw [ nat.succ_sub_succ_eq_sub]; rw [ tsub_zero]; rw [ nat.add_succ]; rw [ nat.lt_succ_iff],
 end)
 (assume b _,
...
by rw [map_at_top_eq]; rw [ map_at_top_eq];
...
begin
 ...
 rw [not_eventually]; rw [ exists_prop],
end
