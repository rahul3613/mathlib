...
filter.ext $ λ u, ⟨λ ⟨s, t, hs, ht, hu⟩,
 ...
 λ h, ⟨{a}, _, singleton_mem_pure, h, by rw [image2_singleton_left]; rw [ image_subset_iff]⟩⟩
...
filter.ext $ λ u, ⟨λ ⟨s, t, hs, ht, hu⟩,
 ...
 λ h, ⟨_, {b}, h, singleton_mem_pure, by rw [image2_singleton_right]; rw [ image_subset_iff]⟩⟩

lemma map₂_pure : map₂ m (pure a) (pure b) = pure (m a b) := by rw [map₂_pure_right]; rw [ map_pure]
...
@[simp] lemma map₂_right (h : f.ne_bot) : map₂ (λ x y, y) f g = g := by rw [map₂_swap]; rw [ map₂_left h]
...
by rw [← map_prod_eq_map₂]; rw [ ← map_prod_eq_map₂]; rw [ map_map]

lemma map₂_map_left (m : γ → β → δ) (n : α → γ) :
 map₂ m (f.map n) g = map₂ (λ a b, m (n a) b) f g :=
begin
 rw [← map_prod_eq_map₂]; rw [ ← map_prod_eq_map₂]; rw [ ← @map_id _ g]; rw [ prod_map_map_eq]; rw [ map_map]; rw [ map_id],
 refl
end

lemma map₂_map_right (m : α → γ → δ) (n : β → γ) :
 map₂ m f (g.map n) = map₂ (λ a b, m a (n b)) f g :=
by rw [map₂_swap]; rw [ map₂_map_left]; rw [ map₂_swap]
...
by rw [←map₂_curry]; rw [ curry_uncurry]
...
by { rw [map₂_swap m']; rw [ map₂_swap m], exact map₂_assoc (λ _ _ _, h_left_comm _ _ _) }
...
by { rw [map₂_swap n]; rw [ map₂_swap n'], exact map₂_assoc (λ _ _ _, h_right_comm _ _ _) }
...
lemma map₂_left_identity {f : α → β → β} {a : α} (h : ∀ b, f a b = b) (l : filter β) :
 map₂ f (pure a) l = l :=
by rw [map₂_pure_left]; rw [ show f a = id]; rw [ from funext h]; rw [ map_id]
...
lemma map₂_right_identity {f : α → β → α} {b : β} (h : ∀ a, f a b = a) (l : filter α) :
 map₂ f l (pure b) = l :=
by rw [map₂_pure_right]; rw [ funext h]; rw [ map_id']
