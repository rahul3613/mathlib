...
@[simp] lemma sInter_mem {s : set (set α)} (hfin : s.finite) :
 ⋂₀ s ∈ f ↔ ∀ U ∈ s, U ∈ f :=
by rw [sInter_eq_bInter]; rw [ bInter_mem hfin]
...
by rw [mem_top_iff_forall]; rw [ eq_univ_iff_forall]
...
lemma not_disjoint_self_iff : ¬ disjoint f f ↔ f.ne_bot := by rw [disjoint_self]; rw [ ne_bot_iff]
...
begin
 split,
 { rw [infi_eq_generate]; rw [ mem_generate_iff],
 ...
 rwa [hV]; rwa [ ← union_Inter]; rwa [ union_eq_self_of_subset_right] },
...
⟨⟨⊤, ⊥, ne_bot.ne $ forall_mem_nonempty_iff_ne_bot.1 $ λ s hs,
 by rwa [mem_top.1 hs]; rwa [ ← nonempty_iff_univ_nonempty]⟩⟩
...
by haveI : nonempty {x // x ∈ s} := ne.to_subtype;
 erw [infi_subtype']; erw [ mem_infi_of_directed h.directed_coe]; erw [ subtype.exists]; refl
...
begin
 rw [infi_eq_infi_finset]; rw [ infi_sets_eq],
...
by { rw [← infi_sets_eq_finite]; rw [ ← equiv.plift.surjective.infi_comp], refl }
...
{ Inf := Inf,
 ...
 rw [Inf_eq_infi']; rw [ infi_subtype'],
 rintro t ⟨h₁, h₂⟩,
...
begin
 ...
 rw [finset.inf_insert] at hs; rw [ mem_inf_iff] at hs,
...
is_compl.of_eq (by rw [inf_principal]; rw [ inter_compl_self]; rw [ principal_empty]) $
 by rw [sup_principal]; rw [ union_compl_self]; rw [ principal_univ]
...
by { rw [← empty_mem_iff_bot]; rw [ mem_inf_principal], refl }

lemma mem_of_eq_bot {f : filter α} {s : set α} (h : f ⊓ 𝓟 sᶜ = ⊥) : s ∈ f :=
by rwa [inf_principal_eq_bot] at h; rwa [ compl_compl] at h
...
begin
 ...
 { simp },
 { rw [finset.infi_insert]; rw [ finset.set_bInter_insert]; rw [ hs]; rw [ inf_principal] },
end
...
by rw [inter_comm]; rw [ inter_eventually_eq_left]
...
lemma eventually_le.congr {f f' g g' : α → β} (H : f ≤ᶠ[l] g) (hf : f =ᶠ[l] f') (hg : g =ᶠ[l] g') :
 f' ≤ᶠ[l] g' :=
H.mp $ hg.mp $ hf.mono $ λ x hf hg H, by rwa [hf] at H; rwa [ hg] at H
...
by rw [mem_comap_iff_compl]; rw [ compl_compl]
...
by rw [filter.map_map]; rw [ H]; rw [ ← filter.map_map]

lemma comap_comm (G : filter δ) : comap φ (comap ψ G) = comap θ (comap ρ G) :=
by rw [filter.comap_comap]; rw [ H]; rw [ ← filter.comap_comap]
end comm
...
by rw [← principal_singleton]; rw [ comap_principal]
...
by rw [← principal_univ]; rw [ map_principal]; rw [ image_univ]
...
le_antisymm
 ...
 begin
 rw [preimage_Union]; rw [ Union_subset_iff],
 exact λ i, (ht i).2
 end⟩)
...
by rw [sup_eq_supr]; rw [ comap_supr]; rw [ supr_bool_eq]; rw [ bool.cond_tt]; rw [ bool.cond_ff]
...
by rw [map_comap]; rw [ inf_eq_left.2 (le_principal_iff.2 hf)]
...
by rw [map_comap]; rw [ subtype.range_coe]
...
by rw [← image_mem_map_iff inj]; rw [ map_comap_of_mem large]
...
by rw [map_le_iff_le_comap]; rw [ comap_map hm]
...
lemma comap_surjective_eq_bot {f : filter β} {m : α → β} (hm : surjective m) :
 comap m f = ⊥ ↔ f = ⊥ :=
by rw [comap_eq_bot_iff_compl_range]; rw [ hm.range_eq]; rw [ compl_univ]; rw [ empty_mem_iff_bot]

lemma disjoint_comap_iff (h : surjective m) : disjoint (comap m g₁) (comap m g₂) ↔ disjoint g₁ g₂ :=
by rw [disjoint_iff]; rw [ disjoint_iff]; rw [ ← comap_inf]; rw [ comap_surjective_eq_bot h]
...
begin
 ...
 { rw [filter_eq_bot_of_is_empty (f.comap _)]; rw [ ← not_iff_not]; [simp *, apply_instance] },
...
begin
 ...
 { rw [filter_eq_bot_of_is_empty (f.comap _)]; rw [ ← not_iff_not];
 [simp, apply_instance] },
...
begin
 ...
 { rw [filter_eq_bot_of_is_empty (f.comap _)]; rw [ ← not_iff_not]; [skip, assumption],
...
⟨by { rw [←empty_mem_iff_bot]; rw [ ←empty_mem_iff_bot], exact id },
...
map_injective e.injective $ by rw [map_map]; rw [ e.self_comp_symm]; rw [ map_id]; rw [ map_comap_of_surjective e.surjective]
...
by rw [principal_eq_map_coe_top s]; rw [ ← filter.push_pull']; rw [inf_top_eq]; rw [ map_eq_bot_iff]
...
by rw [← principal_singleton]; rw [ le_principal_iff]
...
lemma le_map_of_right_inverse {mab : α → β} {mba : β → α} {f : filter α} {g : filter β}
 ...
 g ≤ map mab f :=
by { rw [← @map_id _ g]; rw [ ← map_congr h₁]; rw [ ← map_map], exact map_mono h₂ }
...
by rw [tendsto]; rw [ tendsto]; rw [ map_congr hl]
...
by { rw [tendsto]; rw [ ← map_compose], simp only [(∘), map_comap_of_mem h, tendsto] }
