...
by { rw [eventually_iff]; rw [ eventually_iff]; rw [ mem_prod_principal], simp only [mem_set_of_eq], }

lemma comap_prod (f : α → β × γ) (b : filter β) (c : filter γ) :
 comap f (b ×ᶠ c) = (comap (prod.fst ∘ f) b) ⊓ (comap (prod.snd ∘ f) c) :=
by erw [comap_inf]; erw [ filter.comap_comap]; erw [ filter.comap_comap]

lemma prod_top {f : filter α} : f ×ᶠ (⊤ : filter β) = f.comap prod.fst :=
by rw [filter.prod]; rw [ comap_top]; rw [ inf_top_eq]

lemma sup_prod (f₁ f₂ : filter α) (g : filter β) : (f₁ ⊔ f₂) ×ᶠ g = (f₁ ×ᶠ g) ⊔ (f₂ ×ᶠ g) :=
by rw [filter.prod]; rw [ comap_sup]; rw [ inf_sup_right]; rw [ ← filter.prod]; rw [ ← filter.prod]

lemma prod_sup (f : filter α) (g₁ g₂ : filter β) : f ×ᶠ (g₁ ⊔ g₂) = (f ×ᶠ g₁) ⊔ (f ×ᶠ g₂) :=
by rw [filter.prod]; rw [ comap_sup]; rw [ inf_sup_left]; rw [ ← filter.prod]; rw [ ← filter.prod]
...
lemma prod_infi_left [nonempty ι] {f : ι → filter α} {g : filter β}:
 (⨅ i, f i) ×ᶠ g = (⨅ i, (f i) ×ᶠ g) :=
by { rw [filter.prod]; rw [ comap_infi]; rw [ infi_inf], simp only [filter.prod, eq_self_iff_true] }

lemma prod_infi_right [nonempty ι] {f : filter α} {g : ι → filter β} :
 f ×ᶠ (⨅ i, g i) = (⨅ i, f ×ᶠ (g i)) :=
by { rw [filter.prod]; rw [ comap_infi]; rw [ inf_infi], simp only [filter.prod, eq_self_iff_true] }
...
by { rw [prod_comm]; rw [ eventually_map], simpa, }
...
begin
 erw [tendsto]; erw [ ← prod_map_map_eq],
...
by rw [prod_eq]; rw [ map_pure]; rw [ pure_seq_eq_map]
...
by rw [prod_eq]; rw [ seq_pure]; rw [ map_map]
...
begin
 ...
 rw [subset_empty_iff] at hst; rw [ set.prod_eq_empty_iff] at hst,
...
by rw [filter.coprod]; rw [ comap_principal]; rw [ comap_principal]; rw [ sup_principal]; rw [ set.prod_eq]; rw [ compl_inter]; rw [ preimage_compl]; rw [ preimage_compl]; rw [ compl_compl]; rw [ compl_compl]
