...
by rw [← principal_singleton]; rw [ lift'_principal hh]

lemma lift'_bot (hh : monotone h) : (⊥ : filter α).lift' h = 𝓟 (h ∅) :=
by rw [← principal_empty]; rw [ lift'_principal hh]
...
lemma lift'_infi [nonempty ι] {f : ι → filter α} {g : set α → set β}
 (hg : ∀ s t, g (s ∩ t) = g s ∩ g t) : (infi f).lift' g = (⨅ i, (f i).lift' g) :=
lift_infi $ λ s t, by rw [inf_principal]; rw [ (∘)]; rw [ ← hg]

lemma lift'_infi_of_map_univ {f : ι → filter α} {g : set α → set β}
 (hg : ∀{s t}, g (s ∩ t) = g s ∩ g t) (hg' : g univ = univ) :
 (infi f).lift' g = (⨅ i, (f i).lift' g) :=
lift_infi_of_map_univ (λ s t, by rw [inf_principal]; rw [ (∘)]; rw [ ← hg])
 (by rw [function.comp_app]; rw [ hg']; rw [ principal_univ])
...
begin
 ...
 rw [← comap_lift_eq]; rw [ ← comap_lift_eq],
...
lemma mem_prod_same_iff {s : set (α×α)} :
 s ∈ f ×ᶠ f ↔ (∃t∈f, t ×ˢ t ⊆ s) :=
by { rw [prod_same_eq]; rw [ mem_lift'_sets], exact monotone_id.set_prod monotone_id }
