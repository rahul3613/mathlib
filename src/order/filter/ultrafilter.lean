...
by rw [← inf_ne_bot_iff]; rw [ ne_bot_iff]; rw [ ne.def]; rw [ not_not]; rw [ disjoint_iff]
...
lemma compl_mem_iff_not_mem : sᶜ ∈ f ↔ s ∉ f := by rw [← compl_not_mem_iff]; rw [ compl_compl]
...
coe_injective $ comap_pure.trans $
 by rw [coe_pure]; rw [ ←principal_singleton]; rw [ ←image_singleton]; rw [ preimage_image_eq _ inj]
