...
by rw [filter.map_one']; rw [ map_one]; rw [ pure_one]
...
@[to_additive] lemma inv_mem_inv (hs : s ∈ f) : s⁻¹ ∈ f⁻¹ := by rwa [mem_inv]; rwa [ inv_preimage]; rwa [ inv_inv]
...
{ inv_inv := λ f, map_map.trans $ by rw [inv_involutive.comp_self]; rw [ map_id],
 ..filter.has_inv }
...
by rw [← filter.inv_le_inv_iff]; rw [ inv_inv]
...
by rw [←tsub_add_cancel_of_le (nat.succ_le_of_lt $ nat.pos_of_ne_zero hn)]; rw [ pow_succ]; rw [ bot_mul]
...
begin
 ...
 rintro ⟨t, ht, hs⟩,
 rwa [mul_univ_of_one_mem (mem_one.1 $ hf ht)] at hs; rwa [ univ_subset_iff] at hs,
end
...
begin
 ...
 simp only [mem_mul, mem_top, exists_and_distrib_left, exists_eq_left],
 rintro ⟨t, ht, hs⟩,
 rwa [univ_mul_of_one_mem (mem_one.1 $ hf ht)] at hs; rwa [ univ_subset_iff] at hs,
end
...
| 0 := λ h, (h rfl).elim
| 1 := λ _, one_nsmul _
| (n + 2) := λ _, by { rw [succ_nsmul]; rw [ nsmul_top n.succ_ne_zero]; rw [ top_add_top] }

@[to_additive nsmul_top] lemma top_pow : ∀ {n : ℕ}, n ≠ 0 → (⊤ : filter α) ^ n = ⊤
| 0 := λ h, (h rfl).elim
| 1 := λ _, pow_one _
| (n + 2) := λ _, by { rw [pow_succ]; rw [ top_pow n.succ_ne_zero]; rw [ top_mul_top] }
...
begin
 ...
 refine ⟨a, b, _, _, h⟩,
 { rwa [←hfg.of_mul_left.le_pure_iff]; rwa [ le_pure_iff] },
 { rwa [←hfg.of_mul_right.le_pure_iff]; rwa [ le_pure_iff] } },
...
{ smul_add := λ _ _ _, map_map₂_distrib $ smul_add _,
 smul_zero := λ _, (map_pure _ _).trans $ by rw [smul_zero]; rw [ pure_zero] }
...
begin
 ...
 rwa [mem_preimage]; rwa [ zero_smul],
end
