...
by rw [← h.is_basis.filter_eq_generate]; rw [ h.filter_eq]

lemma generate_eq_generate_inter (s : set (set α)) :
 generate s = generate (sInter '' { t | set.finite t ∧ t ⊆ s}) :=
by erw [(filter_basis.of_sets s).generate]; erw [ ← (has_basis_generate s).filter_eq] ; refl

lemma of_sets_filter_eq_generate (s : set (set α)) : (filter_basis.of_sets s).filter = generate s :=
by rw [← (filter_basis.of_sets s).generate]; rw [ generate_eq_generate_inter s] ; refl
...
begin
 refine ⟨λ t, _⟩,
 rw [mem_infi_of_directed h]; rw [ sigma.exists],
...
begin
 refine ⟨λ t, _⟩,
 rw [mem_infi_of_directed h]; rw [ prod.exists],
 exact exists_congr (λ i, (hl i).mem_iff)
end
...
begin
 refine ⟨λ t, _⟩,
 rw [mem_binfi_of_directed h hdom]; rw [ sigma.exists],
...
begin
 refine ⟨λ t, _⟩,
 rw [mem_binfi_of_directed h hdom]; rw [ prod.exists],
 refine exists_congr (λ i, ⟨_, _⟩),
 { rintros ⟨hi, hti⟩,
 rcases (hl i hi).mem_iff.mp hti with ⟨b, hb, hbt⟩,
 exact ⟨b, ⟨hi, hb⟩, hbt⟩ },
...
@[simp] lemma disjoint_principal_right {f : filter α} {s : set α} :
 disjoint f (𝓟 s) ↔ sᶜ ∈ f :=
by rw [mem_iff_inf_principal_compl]; rw [ compl_compl]; rw [ disjoint_iff]

@[simp] lemma disjoint_principal_left {f : filter α} {s : set α} :
 disjoint (𝓟 s) f ↔ sᶜ ∈ f :=
by rw [disjoint.comm]; rw [ disjoint_principal_right]
...
⟨begin
 ...
 rw [directed_on_iff_directed] at h ⊢; rw [ ← directed_comp] at h ⊢; rw [ (∘)] at h ⊢,
...
begin
 ...
 rw [generate_eq_binfi]; rw [ infi_range]
end
