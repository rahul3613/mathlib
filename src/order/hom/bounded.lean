...
by rw [←map_top f]; rw [ (equiv_like.injective f).eq_iff]

@[simp] lemma map_eq_bot_iff [has_le α] [order_bot α] [partial_order β] [order_bot β]
 [order_iso_class F α β] (f : F) {a : α} : f a = ⊥ ↔ a = ⊥ :=
by rw [←map_bot f]; rw [ (equiv_like.injective f).eq_iff]
...
{ to_fun := f ∘ g,
 map_top' := by rw [comp_apply]; rw [ map_top]; rw [ map_top] }
...
⟨λ h, top_hom.ext $ λ a, hg $
 by rw [←top_hom.comp_apply]; rw [ h]; rw [ top_hom.comp_apply], congr_arg _⟩
...
⟨λ f g, ⟨f ⊓ g, by rw [pi.inf_apply]; rw [ map_top]; rw [ map_top]; rw [ inf_top_eq]⟩⟩
...
⟨λ f g, ⟨f ⊔ g, by rw [pi.sup_apply]; rw [ map_top]; rw [ map_top]; rw [ sup_top_eq]⟩⟩
...
{ to_fun := f ∘ g,
 map_bot' := by rw [comp_apply]; rw [ map_bot]; rw [ map_bot] }
...
⟨λ h, bot_hom.ext $ λ a, hg $
 by rw [←bot_hom.comp_apply]; rw [ h]; rw [ bot_hom.comp_apply], congr_arg _⟩
...
⟨λ f g, ⟨f ⊓ g, by rw [pi.inf_apply]; rw [ map_bot]; rw [ map_bot]; rw [ inf_bot_eq]⟩⟩
...
⟨λ f g, ⟨f ⊔ g, by rw [pi.sup_apply]; rw [ map_bot]; rw [ map_bot]; rw [ sup_bot_eq]⟩⟩
...
⟨λ h, bounded_order_hom.ext $ λ a, hg $
 by rw [←bounded_order_hom.comp_apply]; rw [ h]; rw [ bounded_order_hom.comp_apply], congr_arg _⟩
