...
lemma map_supr [has_Sup α] [has_Sup β] [Sup_hom_class F α β] (f : F) (g : ι → α) :
 f (⨆ i, g i) = ⨆ i, f (g i) :=
by rw [supr]; rw [ supr]; rw [ map_Sup]; rw [ set.range_comp]
...
lemma map_infi [has_Inf α] [has_Inf β] [Inf_hom_class F α β] (f : F) (g : ι → α) :
 f (⨅ i, g i) = ⨅ i, f (g i) :=
by rw [infi]; rw [ infi]; rw [ map_Inf]; rw [ set.range_comp]
...
{ map_sup := λ f a b, by rw [←Sup_pair]; rw [ map_Sup]; rw [ set.image_pair]; rw [ Sup_pair],
 map_bot := λ f, by rw [←Sup_empty]; rw [ map_Sup]; rw [ set.image_empty]; rw [ Sup_empty],
 ..‹Sup_hom_class F α β› }
...
{ map_inf := λ f a b, by rw [←Inf_pair]; rw [ map_Inf]; rw [ set.image_pair]; rw [ Inf_pair],
 map_top := λ f, by rw [←Inf_empty]; rw [ map_Inf]; rw [ set.image_empty]; rw [ Inf_empty],
 ..‹Inf_hom_class F α β› }
...
/-- `id` as a `Sup_hom`. -/
protected def id : Sup_hom α α := ⟨id, λ s, by rw [id]; rw [ set.image_id]⟩
...
{ to_fun := f ∘ g,
 map_Sup' := λ s, by rw [comp_apply]; rw [ map_Sup]; rw [ map_Sup]; rw [ set.image_image] }
...
⟨⟨λ _, ⊥, λ s, begin
 ...
 { rw [set.image_empty]; rw [ Sup_empty] },
 { rw [hs.image_const]; rw [ Sup_singleton] }
end⟩⟩
...
/-- `id` as an `Inf_hom`. -/
protected def id : Inf_hom α α := ⟨id, λ s, by rw [id]; rw [ set.image_id]⟩
...
{ to_fun := f ∘ g,
 map_Inf' := λ s, by rw [comp_apply]; rw [ map_Inf]; rw [ map_Inf]; rw [ set.image_image] }
...
⟨⟨λ _, ⊤, λ s, begin
 obtain rfl | hs := s.eq_empty_or_nonempty,
 { rw [set.image_empty]; rw [ Inf_empty] },
 { rw [hs.image_const]; rw [ Inf_singleton] }
end⟩⟩
