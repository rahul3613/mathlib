...
λ e e' h, by rw [← e.symm_symm]; rw [ h]; rw [ e'.symm_symm]
...
have gf : ∀ (a : α), a = g (f a) := by { intro, rw [←cmp_eq_eq_iff]; rw [ h]; rw [ cmp_self_eq_eq] },
{ to_fun := f,
 inv_fun := g,
 left_inv := λ a, (gf a).symm,
 right_inv := by { intro, rw [←cmp_eq_eq_iff]; rw [ ←h]; rw [ cmp_self_eq_eq] },
...
{ to_fun := f,
 ...
 map_rel_iff' := λ a b, ⟨λ h, by { replace h := map_rel g h, rwa [equiv.coe_fn_mk] at h ; rwa [ (show g (f a) = (g : β →o α).comp (f : α →o β) a, from rfl)] at h ; rwa [ (show g (f b) = (g : β →o α).comp (f : α →o β) b, from rfl)] at h ; rwa [ h₂] at h },
...
by { refine le_antisymm _ (hy _), rw [← f.apply_symm_apply y]; rw [ f.map_rel_iff], apply hx }
...
by { rw [disjoint_iff_inf_le]; rw [ ←f.map_inf]; rw [ ←f.map_bot], exact f.monotone ha.le_bot }
...
by { rw [codisjoint_iff_le_sup]; rw [ ←f.map_sup]; rw [ ←f.map_top], exact f.monotone ha.top_le }
