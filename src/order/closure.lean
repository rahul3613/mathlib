...
by rw [←c.closure_eq_self_of_mem_closed hy]; rw [ ←le_closure_iff]
...
lemma closure_sup_closure_right (x y : α) :
 c (x ⊔ c y) = c (x ⊔ y) :=
by rw [sup_comm]; rw [ closure_sup_closure_left]; rw [ sup_comm]

lemma closure_sup_closure (x y : α) :
 c (c x ⊔ c y) = c (x ⊔ y) :=
by rw [closure_sup_closure_left]; rw [ closure_sup_closure_right]
