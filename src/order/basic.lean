...
by rw [lt_iff_le_and_ne]; rw [ not_and]; rw [ not_not]; rw [ eq_comm]
...
begin
 ...
 { rw [if_pos lt.le]; rw [ if_neg (not_le.mpr lt)], },
 { rw [if_pos eq.le]; rw [ if_pos eq.ge]; rw [ eq], },
 { rw [if_neg (not_le.mpr gt)]; rw [ if_pos gt.le], }
end
-- Variant of `min_def` with the branches reversed.
...
lemma associative_of_commutative_of_le {f : α → α → α} (comm : commutative f)
 ...
 associative f :=
λ a b c, le_antisymm (assoc _ _ _) $ by { rw [comm]; rw [ comm b]; rw [ comm _ c]; rw [ comm a], exact assoc _ _ _ }
...
{ le_total := λ x y, le_total (f x) (f y),
 ...
 min := (⊓),
 max := (⊔),
 min_def := by { ext x y, apply inj, rw [hinf]; rw [ min_def]; rw [ min_default]; rw [ apply_ite f], refl },
 max_def := by { ext x y, apply inj, rw [hsup]; rw [ max_def]; rw [ max_default]; rw [ apply_ite f], refl },
