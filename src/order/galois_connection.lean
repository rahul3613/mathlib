...
eq.symm $ is_lub.supr_eq $ show is_lub (range (l ∘ f)) (l (supr f)),
 by rw [range_comp]; rw [ ← Sup_range]; exact gc.is_lub_l_image (is_lub_Sup _)
...
by rw [← e.bdd_above_image]; rw [ e.image_preimage]

@[simp] lemma bdd_below_preimage (e : α ≃o β) {s : set β} : bdd_below (e ⁻¹' s) ↔ bdd_below s :=
by rw [← e.bdd_below_image]; rw [ e.image_preimage]
...
by rw [Sup_image]; rw [ gi.l_bsupr_u]; rw [ Sup_eq_supr]
...
by rw [Inf_image]; rw [ gi.l_binfi_u]; rw [ Inf_eq_infi]
...
by { rw [infi_subtype']; rw [ infi_subtype'], exact gi.l_infi_of_ul_eq_self _ (λ _, hf _ _) }
