...
theorem with_top.cinfi_empty {α : Type*} [is_empty ι] [has_Inf α] (f : ι → with_top α) :
 (⨅ i, f i) = ⊤ :=
by rw [infi]; rw [ range_eq_empty]; rw [ with_top.cInf_empty]
...
by rw [infi]; rw [ infi]; rw [ with_top.coe_Inf' (range_nonempty f)]; rw [ range_comp]
...
@[norm_cast] lemma with_top.coe_supr [preorder α] [has_Sup α] (f : ι → α)
 ...
 ↑(⨆ i, f i) = (⨆ i, f i : with_top α) :=
by rw [supr]; rw [ supr]; rw [ with_top.coe_Sup' h]; rw [ range_comp]
...
begin
 ...
 { rw [supr_of_empty']; rw [ supr_of_empty'] },
...
by rw [supr]; rw [ range_const]; rw [ cSup_singleton]
...
by rw [supr_of_empty']; rw [ cSup_empty]
...
begin
 ...
 split_ifs,
 { cases h },
 { rw [preimage_empty]; rw [ cSup_empty], exact is_lub_empty },
...
lemma l_cSup' (gc : galois_connection l u) {s : set α} (hne : s.nonempty) (hbdd : bdd_above s) :
 l (Sup s) = Sup (l '' s) :=
by rw [gc.l_cSup hne hbdd]; rw [ Sup_image']

lemma l_csupr (gc : galois_connection l u) {f : ι → α}
 (hf : bdd_above (range f)) :
 l (⨆ i, f i) = ⨆ i, l (f i) :=
by rw [supr]; rw [ gc.l_cSup (range_nonempty _) hf]; rw [ supr_range']
...
begin
 ...
 rw [cSup_le_iff (hs₁.image2 (λ _, (h₁ _).monotone_l) (λ _, (h₂ _).monotone_l) ht₁) (hs₀.image2 ht₀)]; rw [ forall_image2_iff]; rw [ forall₂_swap]; rw [ (h₂ _).le_iff_le]; rw [ cSup_le_iff ht₁ ht₀],
...
begin
 rw [supr_eq_top]; rw [ not_bdd_above_iff],
