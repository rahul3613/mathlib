...
begin
 ...
 rw [this]; rw [ ←Iic_pred_of_not_is_min hi_min],
...
begin
 ...
 rw [h_lt_eq_preimage]; rw [ h_Ioi_eq_Union],
...
begin
 ...
 rw [unique.eq_default s]; rw [ unique.eq_default ∅], },
...
protected lemma measurable_set_eq' [topological_space ι] [order_topology ι]
 ...
 measurable_set[hτ.measurable_space] {ω | τ ω = i} :=
begin
 rw [← set.univ_inter {ω | τ ω = i}]; rw [ measurable_set_inter_eq_iff]; rw [ set.univ_inter],
...
by rw [hτ.measurable_space_min (is_stopping_time_const _ i)]; rw [ measurable_space_const]
...
by rw [measurable_space_min_const]; rw [ measurable_space.measurable_set_inf]
...
begin
 ...
 by rw [set.inter_assoc]; rw [ set.inter_self],
 rw this,
...
begin
 rw [is_stopping_time.measurable_set_min_iff hτ (is_stopping_time_const _ i)]; rw [ is_stopping_time.measurable_space_const]; rw [ is_stopping_time.measurable_set],
 refine ⟨λ h, ⟨h, _⟩, λ h j, h.1 j⟩,
 specialize h i,
 rwa [set.inter_assoc] at h; rwa [ set.inter_self] at h,
end
...
begin
 ...
 rw [← set.univ_inter {ω : Ω | τ ω ≤ π ω}]; rw [ ← hτ.measurable_set_inter_le_iff hπ]; rw [ set.univ_inter],
...
begin
 ...
 rwa [min_eq_left hτ_le] at h_eq; rwa [ min_eq_left hσ_le] at h_eq, }, },
 rw this,
...
begin
 ...
 cases h' with h_eq hτ_le,
 rwa [min_eq_left hτ_le] at h_eq; rwa [ min_eq_left hπ_le] at h_eq, }, },
 rw this,
 refine measurable_set.inter (measurable_set.inter _ (hτ.measurable_set_le j))
 (hπ.measurable_set_le j),
...
begin
 ...
 { rw [set.restrict]; rw [ h_min_eq_left],
 exact h_meas_fst _, },
 ext1 ω,
 rw min_eq_left,
...
begin
 ext y,
 rw [stopped_value]; rw [ finset.sum_apply]; rw [ finset.sum_indicator_eq_sum_filter],
 suffices : finset.filter (λ i, y ∈ {ω : Ω | τ ω = i}) s = ({τ y} : finset ι),
 by rw [this]; rw [ finset.sum_singleton],
 ext1 ω,
...
begin
 ...
 { rw [stopped_process_eq_of_le h]; rw [ set.indicator_of_mem]; rw [ finset.sum_eq_zero]; rw [ add_zero],
 { intros m hm,
 refine set.indicator_of_not_mem _ _,
 rw [finset.mem_filter] at hm,
 exact (hm.2.trans_le h).ne', },
 { exact h, } },
 { rw [stopped_process_eq_of_ge (le_of_lt h)]; rw [ finset.sum_eq_single_of_mem (τ ω)],
 { rw [set.indicator_of_not_mem]; rw [ zero_add]; rw [ set.indicator_of_mem],
...
begin
 ext ω,
 rw [finset.sum_Ico_eq_sub _ (hle ω)]; rw [ finset.sum_range_sub]; rw [ finset.sum_range_sub],
 simp [stopped_value],
end
...
begin
 ...
 { ext x,
 rw [add_comm]; rw [ pi.add_apply]; rw [ ← set.indicator_union_of_not_mem_inter],
 { simp_rw [@eq_comm _ _ n, @le_iff_eq_or_lt _ _ n, nat.succ_le_iff],
 refl },
 { rintro ⟨h₁, h₂⟩,
 exact (nat.succ_le_iff.1 h₂).ne h₁.symm } },
 rw [stopped_process_eq]; rw [ this]; rw [ finset.sum_range_succ_comm]; rw [ ← add_assoc],
end
...
begin
 ...
 rw [set.inter_comm _ t]; rw [ is_stopping_time.measurable_set_inter_eq_iff],
end
...
begin
 ...
 rw [set.inter_comm _ t]; rw [ hτ.measurable_set_inter_le_const_iff],
end
...
begin
 ...
 rw [set.inter_comm _ t]; rw [ is_stopping_time.measurable_set_inter_le_iff],
end
