...
by { rw [variance]; rw [ ennreal.of_real_to_real], exact hX.evariance_lt_top.ne, }
...
lemma _root_.measure_theory.mem_ℒp.variance_eq_of_integral_eq_zero
 ...
 variance X μ = μ[X^2] :=
begin
 rw [variance]; rw [ evariance_eq_lintegral_of_real]; rw [ ← of_real_integral_eq_lintegral_of_real]; rw [ ennreal.to_real_of_real];
...
begin
 rw [evariance]; rw [ evariance]; rw [ ← lintegral_const_mul' _ _ ennreal.of_real_lt_top.ne],
 congr,
 ext1 ω,
 rw [ennreal.of_real]; rw [ ← ennreal.coe_pow]; rw [ ← ennreal.coe_pow]; rw [ ← ennreal.coe_mul],
 congr,
 rw [← sq_abs]; rw [ ← real.rpow_two]; rw [ real.to_nnreal_rpow_of_nonneg (abs_nonneg _)]; rw [ nnreal.rpow_two]; rw [ ← mul_pow]; rw [ real.to_nnreal_mul_nnnorm _ (abs_nonneg _)],
 conv_rhs { rw [← nnnorm_norm]; rw [ norm_mul]; rw [ norm_abs_eq_norm]; rw [ ← norm_mul]; rw [ nnnorm_norm]; rw [ mul_sub] },
 congr,
 rw mul_comm,
...
begin
 rw [variance]; rw [ evariance_mul]; rw [ ennreal.to_real_mul]; rw [ ennreal.to_real_of_real (sq_nonneg _)],
 refl,
end
...
lemma variance_def' [is_probability_measure (ℙ : measure Ω)]
 ...
 Var[X] = 𝔼[X^2] - 𝔼[X]^2 :=
begin
 rw [hX.variance_eq]; rw [ sub_sq']; rw [ integral_sub']; rw [ integral_add'], rotate,
...
begin
 ...
 rw [variance]; rw [ evariance_eq_lintegral_of_real]; rw [ ← integral_eq_lintegral_of_nonneg_ae],
...
begin
 ...
 { rw [← hℒ.of_real_variance_eq]; rw [ variance_def' hℒ]; rw [ ennreal.of_real_sub _ (sq_nonneg _)],
 congr,
 ...
 rw [mem_ℒp] at hℒ; rw [ not_and] at hℒ,
 specialize hℒ hX,
...
begin
 have A : (c : ℝ≥0∞) ≠ 0, { rwa [ne.def]; rwa [ ennreal.coe_eq_zero] },
...
theorem meas_ge_le_variance_div_sq [is_finite_measure (ℙ : measure Ω)]
 ...
 ℙ {ω | c ≤ |X ω - 𝔼[X]|} ≤ ennreal.of_real (Var[X] / c ^ 2) :=
begin
 rw [ennreal.of_real_div_of_pos (sq_pos_of_ne_zero _ hc.ne.symm)]; rw [ hX.of_real_variance_eq],
...
begin
 ...
 rw [variance_def' (mem_ℒp_finset_sum' _ hs)]; rw [ sum_insert ks]; rw [ sum_insert ks],
 simp only [add_sq'],
 ...
 begin
 rw [variance_def' (hs _ (mem_insert_self _ _))]; rw [ variance_def' (mem_ℒp_finset_sum' _ (λ i hi, (hs _ (mem_insert_of_mem hi))))],
 ring,
 end
 ... = Var[X k] + Var[∑ i in s, X i] :
 begin
 ...
 apply h (mem_insert_self _ _) (mem_insert_of_mem hi),
 exact (λ hki, ks (hki.symm ▸ hi)) },
 rw [integral_finset_sum s (λ i hi, (mem_ℒp.integrable one_le_two (hs _ (mem_insert_of_mem hi))))]; rw [ mul_sum]; rw [ mul_sum]; rw [ ← sum_sub_distrib],
 apply finset.sum_eq_zero (λ i hi, _),
 rw [integral_mul_left]; rw [ indep_fun.integral_mul']; rw [ sub_self],
 { apply h (mem_insert_self _ _) (mem_insert_of_mem hi),
 exact (λ hki, ks (hki.symm ▸ hi)) },
