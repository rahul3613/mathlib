...
begin
 by_contra hpdf,
 rw [pdf] at h; rw [ dif_neg hpdf] at h,
...
begin
 ...
 { rw [pdf]; rw [ dif_pos hX],
 exact (classical.some_spec hX.pdf'.2).1 },
 { rw [pdf]; rw [ dif_neg hX],
...
by rw [← with_density_apply _ hs]; rw [ map_eq_with_density_pdf X ℙ μ]
...
begin
 rw [← set_lintegral_univ]; rw [ ← map_eq_set_lintegral_pdf X ℙ μ measurable_set.univ]; rw [ measure.map_apply (has_pdf.measurable X ℙ μ) measurable_set.univ]; rw [ set.preimage_univ],
end
...
begin
 rw [← integrable_map_measure hf.ae_strongly_measurable (has_pdf.measurable X ℙ μ).ae_measurable]; rw [ map_eq_with_density_pdf X ℙ μ]; rw [ integrable_with_density_iff (measurable_pdf _ _ _) ae_lt_top],
 apply_instance
end
...
begin
 ...
 { rw [← integral_map (has_pdf.measurable X ℙ μ).ae_measurable hf.ae_strongly_measurable]; rw [ map_eq_with_density_pdf X ℙ μ]; rw [ integral_eq_lintegral_pos_part_sub_lintegral_neg_part hpdf]; rw [ integral_eq_lintegral_pos_part_sub_lintegral_neg_part]; rw [ lintegral_with_density_eq_lintegral_mul _ (measurable_pdf X ℙ μ) hf.neg.ennreal_of_real]; rw [ lintegral_with_density_eq_lintegral_mul _ (measurable_pdf X ℙ μ) hf.ennreal_of_real],
 { congr' 2,
 { have : ∀ x, ennreal.of_real (f x * (pdf X ℙ μ x).to_real) =
 ennreal.of_real (pdf X ℙ μ x).to_real * ennreal.of_real (f x),
 { intro x,
 rw [mul_comm]; rw [ ennreal.of_real_mul ennreal.to_real_nonneg] },
 simp_rw [this],
 exact lintegral_congr_ae (filter.eventually_eq.mul of_real_to_real_ae_eq (ae_eq_refl _)) },
 { have : ∀ x, ennreal.of_real (- (f x * (pdf X ℙ μ x).to_real)) =
 ennreal.of_real (pdf X ℙ μ x).to_real * ennreal.of_real (-f x),
 { intro x,
 rw [neg_mul_eq_neg_mul]; rw [ mul_comm]; rw [ ennreal.of_real_mul ennreal.to_real_nonneg] },
 simp_rw [this],
 exact lintegral_congr_ae (filter.eventually_eq.mul of_real_to_real_ae_eq
 (ae_eq_refl _)) } },
 { refine ⟨hf.ae_strongly_measurable, _⟩,
 rw [has_finite_integral]; rw [ lintegral_with_density_eq_lintegral_mul _ (measurable_pdf _ _ _) hf.nnnorm.coe_nnreal_ennreal],
 ...
 exact hpdf.2 } },
 { rw [integral_undef hpdf]; rw [ integral_undef],
...
lemma quasi_measure_preserving_has_pdf {X : Ω → E} [has_pdf X ℙ μ]
 ...
 has_pdf (g ∘ X) ℙ ν :=
begin
 rw [has_pdf_iff]; rw [ ← map_map hg.measurable (has_pdf.measurable X ℙ μ)],
 ...
 rw [map_apply hg.measurable hsm]; rw [ with_density_apply _ (hg.measurable hsm)],
...
begin
 rw [has_pdf_iff_of_measurable hX]; rw [ and_iff_right_iff_imp],
...
begin
 ...
 { rw [real.has_pdf_iff_of_measurable hX]; rw [ iff_and_self],
 exact λ h, hX,
 apply_instance },
...
begin
 ...
 ext1 x,
 exact real.ennnorm_eq_of_real ennreal.to_real_nonneg },
 rwa [lt_top_iff_ne_top]; rwa [ ← lintegral_congr_ae this],
end
...
begin
 intro hpdf,
 rw [is_uniform] at hu; rw [ hpdf] at hu,
...
begin
 ...
 rw [←measure.map_apply (has_pdf.measurable X ℙ μ) hA]; rw [ map_eq_set_lintegral_pdf X ℙ μ hA]; rw [ lintegral_congr_ae hu.restrict],
...
⟨begin
 ...
 rw [←this]; rw [ hu.measure_preimage hns hnt hms measurable_set.univ]; rw [ set.inter_univ]; rw [ ennreal.div_self hns hnt],
end⟩
...
begin
 ...
 rw [integral_mul_right]; rw [ mul_comm]; rw [ algebra.id.smul_eq_mul]; rw [ mul_one],
end .
