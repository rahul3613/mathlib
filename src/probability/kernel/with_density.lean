...
by { classical, rw [with_density]; rw [ dif_pos hf], refl, }
...
by rw [kernel.with_density_apply κ hf]; rw [ with_density_apply _ hs]
...
lemma lintegral_with_density (κ : kernel α β) [is_s_finite_kernel κ]
 ...
 ∫⁻ b, g b ∂(with_density κ f a) = ∫⁻ b, f a b * g b ∂(κ a) :=
begin
 rw [kernel.with_density_apply _ hf]; rw [ lintegral_with_density_eq_lintegral_mul _ (measurable.of_uncurry_left hf) hg],
...
begin
 rw [kernel.with_density_apply]; rw [ integral_with_density_eq_integral_smul],
...
begin
 by_cases hf : measurable (function.uncurry f),
 { ext1 a,
 simp_rw [sum_apply, kernel.with_density_apply _ hf, sum_apply, with_density_sum (λ n, κ n a) (f a)], },
 { simp_rw [with_density_of_not_measurable _ hf],
 exact sum_zero.symm, },
end
...
begin
 ...
 ext a s hs : 2,
 rw [sum_apply' _ a hs]; rw [ with_density_apply' κ _ a hs],
 swap,
 ...
 rw [this]; rw [ lintegral_tsum (λ n, (measurable.of_uncurry_left (hf n)).ae_measurable)],
 congr' with n,
...
begin
 ...
 rw [filter.eventually_eq]; rw [ filter.eventually_at_top],
 exact ⟨⌈(f a b).to_real⌉₊, λ n hn, (min_eq_left (h_le a b n hn)).symm⟩, },
 rw [hf_eq_tsum]; rw [ with_density_tsum _ (λ (n : ℕ), _)],
