...
begin
 ...
 rw [this]; rw [ lintegral_tsum (λ n : ℕ, _)],
...
begin
 ...
 rw [hp_eq_mk]; rw [ function.uncurry_apply_pair], },
 rw this,
...
begin
 rw [kernel.comp_prod_apply _ _ _ hs]; rw [ lintegral_eq_zero_iff],
 { refl, },
...
begin
 ext a u hu : 2,
 rw [comp_prod_apply _ _ _ hu]; rw [ restrict_apply' _ _ _ hu]; rw [ comp_prod_apply _ _ _ (hu.inter (hs.prod ht))],
...
begin
 ...
 rw [comp_prod_apply κ η _ hs]; rw [ ← lintegral_const_mul c _],
 ...
 rw [lintegral_add_left (simple_func.measurable _)]; rw [ hf_eq]; rw [ hf'_eq]; rw [ ← lintegral_add_left],
...
begin
 ...
 rw [A]; rw [ B]; rw [ lintegral_comp_prod],
...
instance is_markov_kernel.map (κ : kernel α β) [is_markov_kernel κ] (hf : measurable f) :
 ...
 ⟨λ a, ⟨by rw [map_apply' κ hf a measurable_set.univ]; rw [ set.preimage_univ]; rw [ measure_univ]⟩⟩
...
⟨λ a, ⟨by rw [comap_apply' κ hg a set.univ]; rw [ measure_univ]⟩⟩
...
by { rw [swap_left]; rw [ lintegral_comap _ measurable_swap a], }
...
by { rw [swap_right_apply]; rw [ measure.map_apply measurable_swap hs], refl, }

lemma lintegral_swap_right (κ : kernel α (β × γ)) (a : α) {g : γ × β → ℝ≥0∞} (hg : measurable g) :
 ∫⁻ c, g c ∂(swap_right κ a) = ∫⁻ (bc : β × γ), g bc.swap ∂(κ a) :=
by rw [swap_right]; rw [ lintegral_map _ measurable_swap a hg]
...
by { rw [fst_apply]; rw [ measure.map_apply measurable_fst hs], refl, }

lemma lintegral_fst (κ : kernel α (β × γ)) (a : α) {g : β → ℝ≥0∞} (hg : measurable g) :
 ∫⁻ c, g c ∂(fst κ a) = ∫⁻ (bc : β × γ), g bc.fst ∂(κ a) :=
by rw [fst]; rw [ lintegral_map _ measurable_fst a hg]
...
by { rw [snd_apply]; rw [ measure.map_apply measurable_snd hs], refl, }

lemma lintegral_snd (κ : kernel α (β × γ)) (a : α) {g : γ → ℝ≥0∞} (hg : measurable g) :
 ∫⁻ c, g c ∂(snd κ a) = ∫⁻ (bc : β × γ), g bc.snd ∂(κ a) :=
by rw [snd]; rw [ lintegral_map _ measurable_snd a hg]
...
by rw [comp_apply]; rw [ measure.bind_apply hs (kernel.measurable _)]
...
begin
 ext a s hs : 2,
 rw [comp_apply' _ _ _ hs]; rw [ snd_apply' _ _ hs]; rw [ comp_prod_apply],
...
lemma lintegral_comp (η : kernel β γ) (κ : kernel α β)
 ...
 ∫⁻ c, g c ∂((η ∘ₖ κ) a) = ∫⁻ b, ∫⁻ c, g c ∂(η b) ∂(κ a) :=
by rw [comp_apply]; rw [ measure.lintegral_bind (kernel.measurable _) hg]
...
begin
 ext a s hs : 2,
 simp_rw [map_apply' _ _ _ hs, comp_apply' _ _ _ hs, deterministic_apply' hf _ hs, lintegral_indicator_const_comp hf hs, one_mul],
end
...
begin
 ext a s hs : 2,
 simp_rw [comap_apply' _ _ _ s, comp_apply' _ _ _ hs, deterministic_apply hg a, lintegral_dirac' _ (kernel.measurable_coe κ hs)],
end
