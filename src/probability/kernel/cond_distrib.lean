...
by rw [cond_distrib]; rw [ ← hf.ae_integrable_cond_kernel_iff]; rw [ measure.fst_map_prod_mk₀ hX hY]
...
by { rw [cond_distrib]; rw [ ← measure.fst_map_prod_mk₀ hX hY], exact hf_int.cond_kernel_ae, }
...
by { rw [cond_distrib]; rw [ ← measure.fst_map_prod_mk₀ hX hY], exact hf_int.integral_norm_cond_kernel, }
...
lemma _root_.measure_theory.integrable.norm_integral_cond_distrib_map
 ...
 (hf_int : integrable f (μ.map (λ a, (X a, Y a)))) :
 integrable (λ x, ‖∫ y, f (x, y) ∂(cond_distrib Y X μ x)‖) (μ.map X) :=
by { rw [cond_distrib]; rw [ ← measure.fst_map_prod_mk₀ hX hY], exact hf_int.norm_integral_cond_kernel, }
...
by rw [lintegral_comp (kernel.measurable_coe _ hs) hX]; rw [ cond_distrib]; rw [ ← measure.restrict_map hX ht]; rw [ ← measure.fst_map_prod_mk₀ hX.ae_measurable hY]; rw [ set_lintegral_cond_kernel_eq_measure_prod _ ht hs]; rw [ measure.map_apply_of_ae_measurable (hX.ae_measurable.prod_mk hY) (ht.prod hs)]; rw [ mk_preimage_prod]
...
begin
 ...
 { intros t ht _,
 rw [integral_to_real ((measurable_cond_distrib hs).mono hX.comap_le le_rfl).ae_measurable (eventually_of_forall (λ ω, measure_lt_top (cond_distrib Y X μ (X ω)) _))]; rw [ integral_indicator_const _ (hY hs)]; rw [ measure.restrict_apply (hY hs)]; rw [ smul_eq_mul]; rw [ mul_one]; rw [ inter_comm]; rw [ set_lintegral_cond_distrib_of_measurable_set hX hY.ae_measurable hs ht], },
...
begin
 ...
 rw [← measure.restrict_map hX ht]; rw [ ← measure.fst_map_prod_mk₀ hX.ae_measurable hY]; rw [ cond_distrib]; rw [ set_integral_cond_kernel_univ_right ht hf_int.integrable_on]; rw [ set_integral_map (ht.prod measurable_set.univ) hf_int.1 (hX.ae_measurable.prod_mk hY)]; rw [ mk_preimage_prod]; rw [ preimage_univ]; rw [ inter_univ], },
...
begin
 ...
 { rw [← univ_prod]; rw [ mk_preimage_prod]; rw [ preimage_univ]; rw [ univ_inter]; rw [ preimage_id'],
 exact hμs, },
...
begin
 ...
 refine ⟨hf, _⟩,
 rw [has_finite_integral]; rw [ lintegral_map' hf.ennnorm (hX.prod_mk measurable_id).ae_measurable],
 exact hf_int.2,
end
