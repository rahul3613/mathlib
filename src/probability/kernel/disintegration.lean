...
by rw [← set_lintegral_univ]; rw [ set_lintegral_cond_kernel_real_univ ρ measurable_set.univ]; rw [ univ_prod_univ]
...
begin
 ...
 ... = ρ (s ×ˢ univ) - ρ (s ×ˢ t) :
 by rw [set_lintegral_cond_kernel_real_univ ρ hs]; rw [ ht_lintegral]
 ... = ρ (s ×ˢ tᶜ) :
 begin
 rw ← measure_diff _ (hs.prod ht) (measure_ne_top ρ _),
 { rw [prod_diff_prod]; rw [ compl_eq_univ_diff],
 ...
 rw [lintegral_tsum (λ i, (kernel.measurable_coe _ (hf_meas i)).ae_measurable.restrict)]; rw [ prod_Union]; rw [ measure_Union],
 { simp_rw hf_eq, },
 { intros i j hij,
 rw [function.on_fun]; rw [ disjoint_prod],
...
begin
 ...
 ... = ρ univ - ρ t : by rw [ht_eq]; rw [ lintegral_cond_kernel_real_univ]
...
begin
 ext a s hs : 2,
 rw [kernel.comp_prod_apply _ _ _ hs]; rw [ kernel.const_apply]; rw [ kernel.const_apply],
...
by rw [← kernel.const_eq_comp_prod_real unit ρ]; rw [ kernel.const_apply]
...
begin
 ...
 rw [kernel.lintegral_comp_prod _ _ _ hf]; rw [ kernel.const_apply],
 simp_rw kernel.prod_mk_left_apply,
end
...
begin
 ...
 rw [hρ] at h; rw [ kernel.comp_prod_apply] at h,
...
begin
 ...
 { rw [kernel.deterministic_apply' _ _ hf.measurable_set_range]; rw [ set.indicator_apply]; rw [ if_pos hx₀], }, },
 have : kernel.const γ ρ = kernel.comap_right (kernel.const γ ρ') h_prod_embed,
 { ext c t ht : 2,
 rw [kernel.const_apply]; rw [ kernel.comap_right_apply' _ _ _ ht]; rw [ kernel.const_apply]; rw [ measure.map_apply h_prod_embed.measurable (h_prod_embed.measurable_set_image.mpr ht)],
 congr' with x : 1,
 ...
 rw [this]; rw [ kernel.const_eq_comp_prod_real _ ρ'],
 ext c t ht : 2,
 rw [kernel.comap_right_apply' _ _ _ ht]; rw [ kernel.comp_prod_apply _ _ _ (h_prod_embed.measurable_set_image.mpr ht)]; rw [ kernel.const_apply]; rw [ h_fst]; rw [ kernel.comp_prod_apply _ _ _ ht]; rw [ kernel.const_apply],
 refine lintegral_congr_ae _,
 filter_upwards [h_ae] with a ha,
 rw [kernel.prod_mk_left_apply']; rw [ kernel.prod_mk_left_apply']; rw [ kernel.comap_right_apply'],
...
theorem measure_eq_comp_prod :
 ρ = ((kernel.const unit ρ.fst) ⊗ₖ (kernel.prod_mk_left unit ρ.cond_kernel)) () :=
by rw [← kernel.const_unit_eq_comp_prod]; rw [ kernel.const_apply]
...
begin
 ...
 rw [this]; rw [ kernel.comp_prod_apply _ _ _ (hs.prod ht)],
...
begin
 conv_rhs { rw measure_eq_comp_prod ρ, },
 simp_rw [← kernel.restrict_apply _ (hs.prod ht), ← kernel.comp_prod_restrict, kernel.lintegral_comp_prod _ _ _ hf, kernel.restrict_apply, kernel.const_apply, kernel.prod_mk_left_apply],
end
...
begin
 ...
 rw [integral_comp_prod hf']; rw [ kernel.const_apply],
 simp_rw kernel.prod_mk_left_apply,
end
