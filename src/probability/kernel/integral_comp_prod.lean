...
begin
 ...
 simp_rw [integral_eq_lintegral_of_nonneg_ae (this _) (h1f.norm.comp_measurable measurable_prod_mk_left).ae_strongly_measurable, ennnorm_eq_of_real to_real_nonneg, of_real_norm_eq_coe_nnnorm],
 have : ∀ {p q r : Prop} (h1 : r → p), (r ↔ p ∧ q) ↔ (p → (r ↔ q)) :=
 λ p q r h1, by rw [← and.congr_right_iff]; rw [ and_iff_right_of_imp h1],
 rw [this],
...
lemma has_finite_integral_comp_prod_iff' ⦃f : β × γ → E⦄
 ...
 has_finite_integral (λ x, ∫ y, ‖f (x, y)‖ ∂(η (a, x))) (κ a) :=
begin
 rw [has_finite_integral_congr h1f.ae_eq_mk]; rw [ has_finite_integral_comp_prod_iff h1f.strongly_measurable_mk],
 apply and_congr,
...
begin
 ...
 simp_rw [← kernel.lintegral_fn_integral_sub (λ x, (‖x‖₊ : ℝ≥0∞)) (L1.integrable_coe_fn _) (L1.integrable_coe_fn g)],
 ...
 simp_rw [← kernel.lintegral_comp_prod _ _ _ (this _), ← L1.of_real_norm_sub_eq_lintegral, ← of_real_zero],
...
begin
 ...
 { intros c s hs h2s,
 simp_rw [integral_indicator hs, ← indicator_comp_right, function.comp, integral_indicator (measurable_prod_mk_left hs), measure_theory.set_integral_const, integral_smul_const],
 congr' 1,
 rw integral_to_real,
 rotate,
...
begin
 rw [← kernel.restrict_apply (κ ⊗ₖ η) (hs.prod ht)]; rw [ ← comp_prod_restrict]; rw [ integral_comp_prod],
 { simp_rw kernel.restrict_apply, },
 { rw [comp_prod_restrict]; rw [ kernel.restrict_apply], exact hf, },
end
