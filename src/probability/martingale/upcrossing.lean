...
begin
 rw [upcrossing_strat]; rw [ ← set.indicator_finset_bUnion_apply],
 { exact set.indicator_le_self' (λ _ _, zero_le_one) _ },
 { intros i hi j hj hij,
 rw set.Ico_disjoint_Ico,
 obtain (hij' | hij') := lt_or_gt_of_ne hij,
 { rw [min_eq_left ((upper_crossing_time_mono (nat.succ_le_succ hij'.le)) : upper_crossing_time a b f N _ ω ≤ upper_crossing_time a b f N _ ω)]; rw [ max_eq_right (lower_crossing_time_mono hij'.le : lower_crossing_time a b f N _ _ ≤ lower_crossing_time _ _ _ _ _ _)],
 refine le_trans upper_crossing_time_le_lower_crossing_time (lower_crossing_time_mono
 (nat.succ_le_of_lt hij')) },
 { rw gt_iff_lt at hij',
 rw [min_eq_right ((upper_crossing_time_mono (nat.succ_le_succ hij'.le)) : upper_crossing_time a b f N _ ω ≤ upper_crossing_time a b f N _ ω)]; rw [ max_eq_left (lower_crossing_time_mono hij'.le : lower_crossing_time a b f N _ _ ≤ lower_crossing_time _ _ _ _ _ _)],
...
begin
 ...
 refine hitting_eq_hitting_of_exists hNM _,
 rw [lower_crossing_time] at h; rw [ hitting_lt_iff _ le_rfl] at h,
 swap, { apply_instance },
 obtain ⟨j, hj₁, hj₂⟩ := h,
 exact ⟨j, ⟨hj₁.1, hj₁.2.le⟩, hj₂⟩ }
end
...
begin
 ...
 refine ⟨_, this⟩,
 rw [upper_crossing_time_succ_eq]; rw [ upper_crossing_time_succ_eq]; rw [ eq_comm]; rw [ this],
 refine hitting_eq_hitting_of_exists hNM _,
 simp only [upper_crossing_time_succ_eq, hitting_lt_iff] at h,
 obtain ⟨j, hj₁, hj₂⟩ := h,
 exact ⟨j, ⟨hj₁.1, hj₁.2.le⟩, hj₂⟩
...
begin
 ...
 rw [set.mem_set_of_eq]; rw [ upper_crossing_time_eq_upper_crossing_time_of_lt hNM hn],
...
begin
 ...
 rw [set.mem_set_of_eq]; rw [ upper_crossing_time_succ_eq]; rw [ hitting_lt_iff _ le_rfl],
 swap,
 { apply_instance },
 { refine ⟨N₂, ⟨_, nat.lt_succ_self _⟩, hN₂'.le⟩,
 rw [lower_crossing_time]; rw [ hitting_le_iff_of_lt _ (nat.lt_succ_self _)],
 ...
 exact this.le },
 { rw [not_lt] at hN; rw [ le_zero_iff] at hN,
 rw [hN]; rw [ upcrossings_before_zero]; rw [ upper_crossing_time_zero],
 refl } },
end
...
begin
 ...
 { intro k,
 rw [finset.sum_indicator_eq_sum_filter]; rw [ (_ : (finset.filter (λ i, i ∈ set.Ico (lower_crossing_time a b f N k ω) (upper_crossing_time a b f N (k + 1) ω)) (finset.range N)) = finset.Ico (lower_crossing_time a b f N k ω) (upper_crossing_time a b f N (k + 1) ω))]; rw [ finset.sum_Ico_eq_add_neg _ lower_crossing_time_le_upper_crossing_time_succ]; rw [ finset.sum_range_sub (λ n, f n ω)]; rw [ finset.sum_range_sub (λ n, f n ω)]; rw [ neg_sub]; rw [ sub_add_sub_cancel],
 { refl },
 { ext i,
...
begin
 ...
 { intros i ω,
 refine ⟨λ h, _, λ h, _⟩,
 { rwa [← sub_le_sub_iff_right a]; rwa [ ← lattice_ordered_comm_group.pos_eq_self_of_pos_pos (lt_of_lt_of_le hab' h)] },
 ...
 { intros ω i,
 rw [lattice_ordered_comm_group.pos_nonpos_iff]; rw [ sub_nonpos] },
 induction n with k ih,
 { refine ⟨rfl, _⟩,
...
begin
 ...
 { rw [not_lt] at hab; rw [ ← sub_nonpos] at hab,
...
begin
 ...
 rwa [← ennreal.coe_nat]; rwa [ ennreal.coe_le_coe] },
 { refine ⟨k, λ N, _⟩,
...
begin
 ...
 intro N,
 rw [(by simp : ∫⁻ ω, upcrossings_before a b f N ω ∂μ = ∫⁻ ω, ↑(upcrossings_before a b f N ω : ℝ≥0) ∂μ)]; rw [ lintegral_coe_eq_integral]; rw [ ← ennreal.of_real_mul (sub_pos.2 hab).le],
 ...
 { rw [not_lt] at hab; rw [ ← sub_nonpos] at hab,
 rw [ennreal.of_real_of_nonpos hab]; rw [ zero_mul],
 exact zero_le _ }
end
