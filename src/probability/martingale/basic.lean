...
⟨adapted_zero E ℱ, λ i j hij, by { rw [pi.zero_apply]; rw [ condexp_zero], simp, }⟩
variables {E}
...
begin
 ...
 rwa [← set_integral_trim _ (strongly_measurable_condexp.sub $ hadp i) hs]; rwa [ integral_sub' integrable_condexp.integrable_on (hint i).integrable_on]; rwa [ sub_nonneg]; rwa [ set_integral_condexp (ℱ.le i) (hint j) hs],
end
...
begin
 refine ⟨hadp, λ i j hij, _, hint⟩,
 rw [← condexp_of_strongly_measurable (ℱ.le _) (hadp _) (hint _)]; rw [ ← eventually_sub_nonneg],
...
begin
 ...
 rw [eventually_sub_nonneg]; rw [ condexp_of_strongly_measurable (ℱ.le _) (hf.adapted _) (hf.integrable _)],
...
begin
 ...
 rw [← condexp_of_strongly_measurable (𝒢.le _) (hadp _) (hint _)]; rw [ ← eventually_sub_nonneg],
