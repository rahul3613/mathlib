...
begin
 rw [martingale_part]; rw [ predictable_part],
 ext1 n,
...
begin
 ...
 rw [← finset.sum_range_add_sum_Ico _ hij]; rw [ ← add_zero (∑ i in finset.range i, (f (i + 1) - f i - μ[f (i + 1) - f i | ℱ i]))],
...
begin
 ...
 { rw [hhdef]; rw [ sub_eq_sub_iff_add_eq_add]; rw [ add_comm (predictable_part (f + g) ℱ μ)]; rw [ martingale_part_add_predictable_part] },
 ...
 rw [hhdef] at hω; rw [ pi.sub_apply] at hω,
 rw [hω]; rw [ pi.sub_apply]; rw [ martingale_part],
 simp [hg0],
end
...
begin
 ...
 conv_rhs { rw [← pi.add_apply]; rw [ ← pi.add_apply]; rw [ ← martingale_part_add_predictable_part ℱ μ (f + g)] },
