...
begin
 ...
 rw [ennreal.of_real_le_iff_le_to_real]; rw [ ennreal.coe_to_real]; rw [ coe_nnnorm],
 by_cases hnonneg : 0 ≤ f n ω - a,
 { rw [lattice_ordered_comm_group.pos_of_nonneg _ hnonneg]; rw [ real.norm_eq_abs]; rw [ abs_of_nonneg hnonneg] },
...
lemma submartingale.exists_ae_trim_tendsto_of_bdd [is_finite_measure μ]
 ...
 ∃ c, tendsto (λ n, f n ω) at_top (𝓝 c) :=
begin
 rw [ae_iff]; rw [ trim_measurable_set_eq],
...
begin
 ...
 { rw [limit_process]; rw [ dif_pos this],
...
lemma martingale.eq_condexp_of_tendsto_snorm {μ : measure Ω}
 ...
 f n =ᵐ[μ] μ[g | ℱ n] :=
begin
 rw [← sub_ae_eq_zero]; rw [ ← snorm_eq_zero_iff ((((hf.strongly_measurable n).mono (ℱ.le _)).sub (strongly_measurable_condexp.mono (ℱ.le _))).ae_strongly_measurable) one_ne_zero],
...
begin
 ...
 rw [set_integral_trim hle hgmeas htmeas.compl]; rw [ set_integral_trim hle strongly_measurable_limit_process htmeas.compl]; rw [ hgeq]; rw [ hheq]; rw [ ← set_integral_trim hle hgmeas htmeas]; rw [ ← set_integral_trim hle strongly_measurable_limit_process htmeas]; rw [ ← integral_trim hle hgmeas]; rw [ ← integral_trim hle strongly_measurable_limit_process]; rw [ ← integral_univ]; rw [ this 0 _ measurable_set.univ]; rw [ integral_univ]; rw [ ht (measure_lt_top _ _)] },
 { rintro f hf hfmeas heq -,
 rw [integral_Union (λ n, hle _ (hfmeas n)) hf hg.integrable_on]; rw [ integral_Union (λ n, hle _ (hfmeas n)) hf hlimint.integrable_on],
