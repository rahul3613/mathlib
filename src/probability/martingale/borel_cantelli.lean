...
begin
 ...
 { rw [min_eq_left hle]; rw [ least_ge],
 by_cases h : ∃ j ∈ set.Icc 0 (π ω), f j ω ∈ set.Ici r,
 { refine hle.trans (eq.le _),
 rw [least_ge]; rw [ ← hitting_eq_hitting_of_exists (hπn ω) h] },
 { simp only [hitting, if_neg h] } },
 { rw [min_eq_right (not_le.1 hle).le]; rw [ least_ge]; rw [ least_ge]; rw [ ← hitting_eq_hitting_of_exists (hπn ω) _],
 rw [not_le] at hle; rw [ least_ge] at hle; rw [ hitting_lt_iff _ (hπn ω)] at hle,
...
begin
 ...
 { rw [heq]; rw [ hf0]; rw [ pi.zero_apply],
...
begin
 ...
 { intro n,
 rw [least_ge]; rw [ hitting]; rw [ stopped_value],
 simp only,
 rw if_neg,
...
begin
 ...
 erw [← abs_neg]; erw [ neg_sub]; erw [ sub_neg_eq_add]; erw [ neg_add_eq_sub],
 exact hω i },
...
by rw [process]; rw [ finset.range_zero]; rw [ finset.sum_empty]
...
begin
 rw [nonneg.coe_one]; rw [ process]; rw [ process]; rw [ finset.sum_apply]; rw [ finset.sum_apply]; rw [ finset.sum_range_succ_sub_sum]; rw [ ← real.norm_eq_abs]; rw [ norm_indicator_eq_indicator_norm],
...
begin
 ...
 swap,
 { rw [process]; rw [ process]; rw [ ← sub_nonneg]; rw [ finset.sum_apply]; rw [ finset.sum_apply]; rw [ finset.sum_range_succ_sub_sum],
