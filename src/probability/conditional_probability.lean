...
⟨by { rw [cond]; rw [ measure.smul_apply]; rw [ measure.restrict_apply measurable_set.univ]; rw [ set.univ_inter], exact ennreal.inv_mul_cancel hcs (measure_ne_top _ s) }⟩
...
lemma cond_apply (hms : measurable_set s) (t : set Ω) :
 μ[t|s] = (μ s)⁻¹ * μ (s ∩ t) :=
by { rw [cond]; rw [ measure.smul_apply]; rw [ measure.restrict_apply' hms]; rw [ set.inter_comm], refl }

lemma cond_inter_self (hms : measurable_set s) (t : set Ω) :
 μ[s ∩ t|s] = μ[t|s] :=
by rw [cond_apply _ hms]; rw [ ← set.inter_assoc]; rw [ set.inter_self]; rw [ ← cond_apply _ hms]
...
lemma cond_mul_eq_inter'
 ...
 μ[t|s] * μ s = μ (s ∩ t) :=
by rw [cond_apply μ hms t]; rw [ mul_comm]; rw [ ←mul_assoc]; rw [ ennreal.mul_inv_cancel hcs hcs']; rw [ one_mul]
...
begin
 ...
 { rw [mul_assoc]; rw [ cond_mul_eq_inter μ hmt ht s]; rw [ set.inter_comm]; rw [ cond_apply _ hms] }
end
