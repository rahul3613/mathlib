...
begin
 rw [to_measure_apply_eq_to_outer_measure_apply _ s hs]; rw [ to_measure_apply_eq_to_outer_measure_apply _ (f ⁻¹' s) (measurable_set_preimage hf hs)],
...
by rw [filter]; rw [ normalize_apply]

lemma filter_apply_eq_zero_of_not_mem {a : α} (ha : a ∉ s) : (p.filter s h) a = 0 :=
by rw [filter_apply]; rw [ set.indicator_apply_eq_zero.mpr (λ ha', absurd ha' ha)]; rw [ zero_mul]
...
by erw [apply_eq_zero_iff]; erw [ support_filter]; erw [ set.mem_inter_iff]; erw [ not_and_distrib]

lemma filter_apply_ne_zero_iff (a : α) : (p.filter s h) a ≠ 0 ↔ a ∈ s ∧ a ∈ p.support :=
by rw [ne.def]; rw [ filter_apply_eq_zero_iff]; rw [ not_or_distrib]; rw [ not_not]; rw [ not_not]
