...
by rw [mem_support_iff]; rw [ not_not]
...
begin
 ...
 ... = ite (a = a) (p a) 0 + ∑' b, ite (b = a) 0 (p b) : by rw [eq_self_iff_true]; rw [ if_true]
...
begin
 rw [to_outer_measure_apply]; rw [ ennreal.tsum_eq_zero],
...
by rw [to_measure_apply_eq_to_outer_measure_apply p s hs]; rw [ to_outer_measure_apply_eq_zero_iff]
...
pmf.ext (λ x, by rw [← p.to_measure_apply_singleton x (measurable_set_singleton x)]; rw [ p.to_measure.to_pmf_apply])

lemma to_measure_eq_iff_eq_to_pmf (μ : measure α) [is_probability_measure μ] :
 p.to_measure = μ ↔ p = μ.to_pmf :=
by rw [← to_measure_inj]; rw [ measure.to_pmf_to_measure]
