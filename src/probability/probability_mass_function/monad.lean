...
by rw [to_pmf_eq_iff_to_measure_eq]; rw [ to_measure_pure]
...
pmf.ext (λ x, (bind_apply _ _ _).trans (trans (tsum_eq_single x $
 (λ y hy, by rw [pure_apply_of_ne _ _ hy.symm]; rw [ mul_zero])) $ by rw [pure_apply_self]; rw [ mul_one]))

@[simp] lemma bind_const (p : pmf α) (q : pmf β) : p.bind (λ _, q) = q :=
pmf.ext (λ x, by rw [bind_apply]; rw [ ennreal.tsum_mul_right]; rw [ tsum_coe]; rw [ one_mul])
