...
of_finset (λ a, if a ∈ s then s.card⁻¹ else 0) s (Exists.rec_on hs (λ x hx,
 ...
 ... = (s.card : ℝ≥0∞) * (s.card : ℝ≥0∞)⁻¹ : by rw [finset.sum_const]; rw [ nsmul_eq_mul]
...
⟨λ a, s.count a / s.card, ennreal.summable.has_sum_iff.2
 ...
 by rwa [multiset.count_eq_zero]; rwa [ ← multiset.mem_to_finset]))
 ... = 1 : by rw [← nat.cast_sum]; rw [ multiset.to_finset_sum_count_eq s]; rw [ ennreal.inv_mul_cancel (nat.cast_ne_zero.2 (hs ∘ multiset.card_eq_zero.1)) (ennreal.nat_ne_top _)] ) ⟩
...
begin
 rw [div_eq_mul_inv]; rw [ ← ennreal.tsum_mul_right]; rw [ to_outer_measure_apply],
