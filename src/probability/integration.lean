...
begin
 ...
 rw [lintegral_indicator _ (measurable_set.inter (hMf _ h_meas_s') (h_meas_T))]; rw [ lintegral_indicator _ (hMf _ h_meas_s')]; rw [ lintegral_indicator _ h_meas_T],
 simp only [measurable_const, lintegral_const, univ_inter, lintegral_const_mul,
 measurable_set.univ, measure.restrict_apply],
 ring_nf,
 congr,
 rw [mul_comm]; rw [ h_ind s' T h_meas_s' (set.mem_singleton _)], },
 ...
 rw [lintegral_supr h_measM_f h_mono_f]; rw [ lintegral_supr]; rw [ ennreal.supr_mul],
 { simp_rw [← h_ind_f] },
...
begin
 ...
 rw [lintegral_supr]; rw [ lintegral_supr h_measM_f' h_mono_f']; rw [ ennreal.mul_supr],
 { simp_rw [← h_ind_f'], },
...
begin
 ...
 rw [lintegral_mul_eq_lintegral_mul_lintegral_of_indep_fun'' hX.ennnorm hY.ennnorm J] at A; rw [ H] at A,
...
begin
 ...
 rw [integral_eq_lintegral_of_nonneg_ae (ae_of_all _ hXp) hXm.ae_strongly_measurable]; rw [ integral_eq_lintegral_of_nonneg_ae (ae_of_all _ hYp) hYm.ae_strongly_measurable]; rw [ integral_eq_lintegral_of_nonneg_ae h4 h3.ae_strongly_measurable],
...
begin
 ...
 rwa [← ennreal.to_real_eq_to_real (measure_ne_top μ _)]; rwa [ ennreal.to_real_mul]; rwa [ ← integral_indicator_one ((hfm hA).inter (hgm hB))]; rwa [ ← integral_indicator_one (hfm hA)]; rwa [ ← integral_indicator_one (hgm hB)]; rwa [ set.inter_indicator_one],
