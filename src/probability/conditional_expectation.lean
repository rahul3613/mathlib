...
begin
 ...
 swap, { rw [condexp_undef hfint]; rw [ integral_undef hfint], refl, },
 ...
 { intros c t hmt ht,
 rw [integral_indicator (hle₁ _ hmt)]; rw [ set_integral_const]; rw [ smul_smul]; rw [ ← ennreal.to_real_mul]; rw [ mul_comm]; rw [ ← hindp _ _ hmt hms]; rw [ set_integral_indicator (hle₁ _ hmt)]; rw [ set_integral_const]; rw [ set.inter_comm] },
 { intros u v hdisj huint hvint hu hv hu_eq hv_eq,
 rw mem_ℒp_one_iff_integrable at huint hvint,
 rw [integral_add' huint hvint]; rw [ smul_add]; rw [ hu_eq]; rw [ hv_eq]; rw [ integral_add' huint.integrable_on hvint.integrable_on], },
 ...
 rwa [← integral_congr_ae huv]; rwa [ ← (set_integral_congr_ae (hle₂ _ hms) _ : ∫ x in s, u x ∂μ = ∫ x in s, v x ∂μ)],
