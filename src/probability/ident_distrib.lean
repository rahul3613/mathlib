...
{ ae_measurable_fst := hu.comp_ae_measurable h.ae_measurable_fst,
 ...
 map_eq :=
 begin
 rw [← ae_measurable.map_map_of_ae_measurable hu h.ae_measurable_fst]; rw [ ← ae_measurable.map_map_of_ae_measurable _ h.ae_measurable_snd]; rw [ h.map_eq],
 rwa ← h.map_eq,
 end }
...
lemma measure_mem_eq (h : ident_distrib f g μ ν) {s : set γ} (hs : measurable_set s) :
 μ (f ⁻¹' s) = ν (g ⁻¹' s) :=
by rw [← measure.map_apply_of_ae_measurable h.ae_measurable_fst hs]; rw [ ← measure.map_apply_of_ae_measurable h.ae_measurable_snd hs]; rw [ h.map_eq]
...
begin
 ...
 rw [← lintegral_map' ae_measurable_id h.ae_measurable_fst]; rw [ ← lintegral_map' ae_measurable_id h.ae_measurable_snd]; rw [ h.map_eq],
end
...
begin
 ...
 rw h.map_eq at A,
 rw [← integral_map h.ae_measurable_snd A]; rw [ h.map_eq] },
...
by { rw [variance]; rw [ h.evariance_eq], refl, }
...
begin
 ...
 rw [← snorm_map_measure _ (hf i).ae_measurable_fst]; rw [ (hf i).map_eq]; rw [ snorm_map_measure _ (hf j).ae_measurable_fst],
 { refl },
