...
begin
 ...
 { rw log_exp _, },
 { rw [ne.def]; rw [ ennreal.to_real_eq_zero_iff]; rw [ measure.measure_univ_eq_zero],
...
begin
 ...
 rw [sum_insert hi_notin_s]; rw [ indep_fun.mgf_add (h_indep.indep_fun_finset_sum_of_not_mem h_meas hi_notin_s).symm (h_int' i) (ae_strongly_measurable_exp_mul_sum (λ i hi, h_int' i))]; rw [ h_rec]; rw [ prod_insert hi_notin_s] }
end
...
begin
 ...
 rwa [mul_comm (exp (t * ε))⁻¹]; rwa [ ← div_eq_mul_inv]; rwa [ le_div_iff' (exp_pos _)],
 end
 ... = exp (- t * ε) * mgf X μ t : by { rw [neg_mul]; rw [ exp_neg], refl, },
end
