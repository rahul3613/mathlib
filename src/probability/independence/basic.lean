...
@[symm] lemma indep_sets.symm {s₁ s₂ : set (set Ω)} [measurable_space Ω] {μ : measure Ω}
 (h : indep_sets s₁ s₂ μ) :
 indep_sets s₂ s₁ μ :=
by { intros t1 t2 ht1 ht2, rw [set.inter_comm]; rw [ mul_comm], exact h t2 t1 ht2 ht1, }
...
begin
 intros s t hs ht,
 rw [set.mem_set_of_eq] at ht; rw [ measurable_space.measurable_set_bot_iff] at ht,
 cases ht,
 { rw [ht]; rw [ set.inter_empty]; rw [ measure_empty]; rw [ mul_zero], },
 { rw [ht]; rw [ set.inter_univ]; rw [ measure_univ]; rw [ mul_one], },
end
...
⟨λ h, h s t rfl rfl,
 λ h s1 t1 hs1 ht1, by rwa [set.mem_singleton_iff.mp hs1]; rwa [ set.mem_singleton_iff.mp ht1]⟩
...
begin
 ...
 nth_rewrite 1 h2,
 nth_rewrite 3 h2,
 rw [← h_inter]; rw [ ← h_prod]; rw [ h_indep {i]; rw [ j} hf_m],
end
...
begin
 ...
 by rw [measure.restrict_apply_univ]; rw [ measure.smul_apply]; rw [ smul_eq_mul]; rw [ measure_univ]; rw [ mul_one],
 haveI : is_finite_measure μ_inter := @restrict.is_finite_measure Ω _ t1 μ ⟨measure_lt_top μ t1⟩,
 rw [set.inter_comm]; rw [ ← measure.restrict_apply (h2 t2 ht2m)],
 ...
 rw [measure.restrict_apply ht2]; rw [ measure.smul_apply]; rw [ set.inter_comm],
...
begin
 ...
 rw [mul_comm]; rw [ ← measure.restrict_apply (h1 t1 ht1)],
 refine ext_on_measurable_space_of_generate_finite m p1 (λ t ht, _) h1 hpm1 hp1 h_univ ht1,
 have ht1 : measurable_set[m] t,
 { refine h1 _ _,
 rw hpm1,
 exact measurable_set_generate_from ht, },
 rw [measure.restrict_apply ht1]; rw [ measure.smul_apply]; rw [ smul_eq_mul]; rw [ mul_comm],
...
begin
 ...
 rw [ht1_eq]; rw [ ht2_eq]; rw [ h_p1_inter_p2]; rw [ ← h_indep _ hgm], },
 ...
 simp_rw [h_P_inter, h_μg, finset.prod_mul_distrib, finset.prod_ite_mem (p1 ∪ p2) p1 (λ x, μ (f1 x)), finset.union_inter_cancel_left, finset.prod_ite_mem (p1 ∪ p2) p2 (λ x, μ (f2 x)), finset.union_inter_cancel_right, ht1_eq, ← h_indep p1 ht1_m, ht2_eq, ← h_indep p2 ht2_m],
end
...
begin
 rw [← generate_from_pi_Union_Inter_singleton_left]; rw [ ← generate_from_pi_Union_Inter_singleton_left],
...
begin
 ...
 have h_μ_t1 : μ t1 = ∏ n in s, μ (f n), by rw [h_t1]; rw [ ← hp_ind s h_f_mem_pi],
 have h_t2 : t2 = f a, by { simp_rw [f], simp, },
 have h_μ_inter : μ (t1 ∩ t2) = ∏ n in insert a s, μ (f n),
 { have h_t1_inter_t2 : t1 ∩ t2 = ⋂ n ∈ insert a s, f n,
 by rw [h_t1]; rw [ h_t2]; rw [ finset.set_bInter_insert]; rw [ set.inter_comm],
 rw [h_t1_inter_t2]; rw [ ← hp_ind (insert a s) h_f_mem], },
 have has : a ∉ s := λ has_mem, haS (hs_mem has_mem),
 rw [h_μ_inter]; rw [ finset.prod_insert has]; rw [ h_t2]; rw [ mul_comm]; rw [ h_μ_t1],
end
...
begin
 ...
 rw [finset.set_bInter_insert]; rw [ finset.prod_insert ha_notin_S]; rw [ ← h_rec hf_m_S],
...
begin
 ...
 rw [← measure_congr h1]; rw [ ← measure_congr h2]; rw [ ← measure_congr (h1.inter h2)],
...
begin
 ...
 { rw [generate_from_pi.symm]; rw [ comap_generate_from],
 { congr' with s,
 ...
 rw [h_eq_inter_S]; rw [ h_eq_inter_T]; rw [ hf_Indep S h_meas_s']; rw [ hf_Indep T h_meas_t'],
 ...
 rw [h_Inter_inter]; rw [ hf_Indep (S ∪ T)],
 swap, { intros i hi_mem,
 rw finset.mem_union at hi_mem,
 cases hi_mem,
 { rw [h_sets_t'_univ hi_mem]; rw [ set.inter_univ], exact h_meas_s' i hi_mem, },
 { rw [h_sets_s'_univ hi_mem]; rw [ set.univ_inter], exact h_meas_t' i hi_mem, }, },
