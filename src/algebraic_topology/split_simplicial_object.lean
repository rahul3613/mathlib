...
by { erw [ι_summand_eq]; erw [ X.map_id]; erw [ comp_id], refl, }
...
begin
 dsimp [ι_summand],
 erw [colimit.ι_desc]; erw [ colimit.ι_desc]; erw [ cofan.mk_ι_app]; erw [ cofan.mk_ι_app],
...
by rw [S₁.s.ι_summand_eq]; rw [ S₂.s.ι_summand_eq]; rw [ assoc]; rw [ Φ.F.naturality]; rw [ ← Φ.comm_assoc]
