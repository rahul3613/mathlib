...
begin
 ...
 rw [show (λ i , (∑ j : fin (n+3), d_l j) ≫ d_r i) = (λ i, ∑ j : fin (n+3), (d_l j ≫ d_r i))]; rw [ by { ext i]; rw [ rw sum_comp]; rw [ }],
 ...
 erw [show ∑ (ij : P)]; erw [ term ij = (∑ ij in S, term ij) + (∑ ij in Sᶜ, term ij)]; erw [ by rw finset.sum_add_sum_compl],
 rw [← eq_neg_iff_add_eq_zero]; rw [ ← finset.sum_neg_distrib],
 ...
 rw [category_theory.simplicial_object.δ_comp_δ X ineq]; rw [ fin.cast_succ_cast_lt]; rw [ mul_comm] },
 { -- φ : S → Sᶜ is injective
...
chain_complex.of_hom _ _ _ _ _ _
 ...
 { intro j,
 rw [def_t]; rw [ comp_zsmul]; rw [ ← zsmul_zero ((-1 : ℤ)^(j.succ : ℕ))],
 apply congr_arg,
 ...
 let eq := def_t 0,
 rw [show (-1 : ℤ)^((0 : fin (n+2)) : ℕ) = 1] at eq; rw [ by ring] at eq,
 rw one_smul at eq,
 rw eq,
 cases n; dsimp; simp,
 end)
