...
begin
 ...
 { rw [fin.cast_succ_cast_lt]; rw [ ← fin.succ_lt_succ_iff]; rw [ fin.succ_pred],
 exact H, },
end
...
begin
 ...
 rw [NonemptyFinLinOrd.mono_iff_injective]; rw [ skeletal_functor.coe_map]; rw [ function.injective.of_comp_iff ulift.up_injective]; rw [ function.injective.of_comp_iff' _ ulift.down_bijective],
end
...
begin
 ...
 dsimp only [skeletal_equivalence, functor.as_equivalence_functor],
 rw [NonemptyFinLinOrd.epi_iff_surjective]; rw [ skeletal_functor.coe_map]; rw [ function.surjective.of_comp_iff' ulift.up_bijective]; rw [ function.surjective.of_comp_iff _ ulift.down_surjective],
end
...
begin
 ...
 { use b.succ,
 rw [fin.pred_above_above i b.succ _]; rw [ fin.pred_succ],
 rw not_le at h,
...
begin
 ...
 { rwa [eq]; rwa [ ← fin.le_cast_succ_iff], },
 rw eq, },
 ...
 rw [fin.pred_above_above i y.succ h']; rw [ fin.pred_succ],
 by_cases h'' : y = i,
 { rw h'',
 convert hi.symm,
...
begin
 ...
 erw [fin.succ_above_above i _]; erw [ fin.succ_pred],
...
begin
 ...
 dsimp,
 rw [fintype.bijective_iff_injective_and_card i.to_order_hom]; rw [ ← mono_iff_injective]; rw [ eq_self_iff_true]; rw [ and_true],
 apply_instance,
end
...
begin
 ...
 apply is_iso_of_bijective,
 dsimp,
 rw [fintype.bijective_iff_surjective_and_card i.to_order_hom]; rw [ ← epi_iff_surjective]; rw [ eq_self_iff_true]; rw [ and_true],
 apply_instance,
end
...
begin
 haveI := strong_epi_of_epi e,
 rw [← image.iso_strong_epi_mono_hom_comp_ι e i fac]; rw [ simplex_category.eq_id_of_is_iso (image.iso_strong_epi_mono e i fac).hom]; rw [ category.id_comp],
end

lemma factor_thru_image_eq {Δ Δ'' : simplex_category } {φ : Δ ⟶ Δ''}
 {e : Δ ⟶ image φ} [epi e] {i : image φ ⟶ Δ''} [mono i] (fac : e ≫ i = φ) :
 factor_thru_image φ = e :=
by rw [← cancel_mono i]; rw [ fac]; rw [ ← image_ι_eq fac]; rw [ image.fac]
