...
{ left :=
 ...
 (λ i, by { dsimp, erw [category.assoc]; erw [ arrow.w]; erw [ augmented.to_arrow_obj_hom]; erw [ nat_trans.naturality_assoc]; erw [ functor.const_obj_map]; erw [ category.id_comp] } ),
 naturality' := begin
 intros x y f,
 ext,
 { dsimp,
 simp only [wide_pullback.lift_π, category.assoc],
 rw [← category.assoc]; rw [ ← X.left.map_comp],
 refl },
 { dsimp,
...
{ app := λ n, wide_pushout.desc (F.left ≫ wide_pushout.head _)
 ...
 λ i, by { rw [← arrow.w_assoc F]; rw [ wide_pushout.arrow_ι (λ i, g.hom)] },
 naturality' := λ x y f, by { ext, { simp }, { simp } } }
...
(begin
 ...
 erw [wide_pullback.lift_π]; erw [ limit.cone_point_unique_up_to_iso_inv_comp (wide_cospan.limit_cone _ _).2]; erw [ (limit.is_limit _).cone_point_unique_up_to_iso_inv_comp (wide_cospan.limit_cone _ _).2]; erw [ limit.lift_π],
 refl,
end)
