...
begin
 ...
 by_cases h₃ : is_δ₀ i,
 { by_cases h₄ : is_δ₀ i',
 { rw [map_mono_δ₀' K i h₃]; rw [ map_mono_δ₀' K i' h₄]; rw [ homological_complex.d_comp_d], },
...
{ obj := λ Δ, obj.obj₂ K Δ,
 ...
 ext A,
 cases A,
 have fac : A.e ≫ 𝟙 A.1.unop = (𝟙 Δ).unop ≫ A.e := by rw [unop_id]; rw [ comp_id]; rw [ id_comp],
 erw [obj.map_on_summand₀ K A fac]; erw [ obj.termwise.map_mono_id]; erw [ id_comp]; erw [ comp_id],
 ...
 ext A,
 cases A,
 have fac : θ.unop ≫ θ'.unop ≫ A.e = (θ' ≫ θ).unop ≫ A.e := by rw [unop_comp]; rw [ assoc],
 rw [← image.fac (θ'.unop ≫ A.e)] at fac; rw [ ← assoc] at fac; rw [ ← image.fac (θ.unop ≫ factor_thru_image (θ'.unop ≫ A.e))] at fac; rw [ assoc] at fac,
...
begin
 ...
 rw [Γ₀.obj.map_on_summand₀ K (simplicial_object.splitting.index_set.id A.1) (show A.e ≫ 𝟙 _ = A.e.op.unop ≫ 𝟙 _, by refl)]; rw [ Γ₀.obj.termwise.map_mono_id]; rw [ A.ext'],
 apply id_comp,
end
...
begin
 ...
 rw [Γ₀.obj.termwise.map_mono_eq_zero K] at eq; rw [ zero_comp] at eq, rotate,
 { intro h,
