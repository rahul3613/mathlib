...
begin
 ...
 rw [← factor_thru_arrow _ _ (finset_inf_arrow_factors finset.univ _ j (by simp only [finset.mem_univ])), assoc, kernel_subobject_arrow_comp, comp_zero],
end
...
begin
 cases n,
 { apply top_factors, },
 { rw [P_infty_f]; rw [ normalized_Moore_complex.obj_X]; rw [ finset_inf_factors],
 intros i hi,
...
chain_complex.of_hom _ _ _ _ _ _
 ...
 (λ n, begin
 rw [← cancel_mono (normalized_Moore_complex.obj_X X n).arrow]; rw [ assoc]; rw [ assoc]; rw [ factor_thru_arrow]; rw [ ← inclusion_of_Moore_complex_map_f]; rw [ ← normalized_Moore_complex_obj_d]; rw [ ← (inclusion_of_Moore_complex_map X).comm' (n+1) n rfl]; rw [ inclusion_of_Moore_complex_map_f]; rw [ factor_thru_arrow_assoc]; rw [ ← alternating_face_map_complex_obj_d],
...
{ hom :=
 ...
 comm := by erw [comp_id]; erw [ P_infty_comp_P_infty_to_normalized_Moore_complex] },
 ...
 comm := by erw [inclusion_of_Moore_complex_map_comp_P_infty]; erw [ id_comp] },
