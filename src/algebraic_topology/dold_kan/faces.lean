...
begin
 ...
 subst hi,
 apply v i,
 rw [← @nat.add_le_add_iff_right 1]; rw [ add_assoc],
...
by rw [assoc]; rw [ v j hj]; rw [ comp_zero]
...
begin
 ...
 { intro d, rw [add_assoc]; rw [ add_comm d]; rw [ ← add_assoc]; rw [ hnaq], },
 rw [Hσ]; rw [ homotopy.null_homotopic_map'_f (c_mk (n+2) (n+1) rfl) (c_mk (n+1) n rfl)]; rw [ hσ'_eq hnaq (c_mk (n+1) n rfl)]; rw [ hσ'_eq (hnaq_shift 1) (c_mk (n+2) (n+1) rfl)],
 ...
 rw [← fin.sum_congr' _ (hnaq_shift 2).symm]; rw [ fin.sum_trunc], swap,
 { rintro ⟨k, hk⟩,
 ...
 rw [← fin.sum_congr' _ (hnaq_shift 3).symm]; rw [ @fin.sum_trunc _ _ (a+3)], swap,
 { rintros ⟨k, hk⟩,
 ...
 conv_lhs { congr, skip, rw [fin.sum_univ_cast_succ]; rw [ fin.sum_univ_cast_succ], },
 ...
 rw [add_assoc c d e]; rw [ h2]; rw [ add_zero]; rw [ add_comm a b]; rw [ add_assoc]; rw [ add_comm a c]; rw [ h3]; rw [ add_zero]; rw [ h1], },
 apply simplif,
 { /- b=f -/
 ...
 linarith, },
 { /- d+e = 0 -/
 rw [assoc]; rw [ assoc]; rw [ X.δ_comp_σ_self' (fin.cast_succ_mk _ _ _).symm]; rw [ X.δ_comp_σ_succ' (fin.succ_mk _ _ _).symm],
...
begin
 ...
 by_cases hqn' : n+1<q,
 { rw [hσ'_eq_zero hqn' (c_mk (n+2) (n+1) rfl)]; rw [ zero_comp]; rw [ comp_zero], },
 ...
 erw [δ_comp_σ_self]; erw [ δ_comp_σ_succ]; erw [ add_right_neg], },
 { intro j,
...
begin
 ...
 by_cases hqn : n<q,
 { rw [v.comp_Hσ_eq_zero hqn]; rw [ zero_comp]; rw [ add_zero]; rw [ v j (by linarith)], },
 ...
 linarith, },
 rw [X.δ_comp_σ_of_gt']; rw [ j.pred_succ], swap,
 { rw fin.lt_iff_coe_lt_coe,
 ...
 { ext,
 dsimp,
 have hq : q = 1 := by rw [← add_left_inj a]; rw [ ha]; rw [ ham'']; rw [ add_comm],
 linarith, },
 simp only [← assoc, v j (by linarith), zero_comp], },
end
