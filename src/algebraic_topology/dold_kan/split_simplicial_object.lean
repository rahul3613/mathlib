...
begin
 ...
 erw [colimit.ι_desc]; erw [ cofan.mk_ι_app],
 dsimp,
...
begin
 apply s.hom_ext',
 intro A,
 rw [comp_id]; rw [ comp_sum]; rw [ finset.sum_eq_single A]; rw [ ι_π_summand_eq_id_assoc],
 { intros B h₁ h₂,
 rw [s.ι_π_summand_eq_zero_assoc _ _ h₂]; rw [ zero_comp], },
...
begin
 ...
 rw [comp_zero]; rw [ s.ι_summand_epi_naturality_assoc A (simplex_category.σ i).op]; rw [ ι_π_summand_eq_zero],
 symmetry,
...
begin
 ...
 rw [simplicial_object.splitting.ι_summand_eq]; rw [ assoc]; rw [ degeneracy_comp_P_infty X n A.e hA]; rw [ comp_zero],
end
...
begin
 ...
 rw [← h']; rw [ assoc]; rw [ Q_infty_f]; rw [ decomposition_Q]; rw [ preadditive.sum_comp]; rw [ preadditive.comp_sum]; rw [ finset.sum_eq_zero],
 intros i hi,
...
begin
 ...
 symmetry,
 rw [← sub_eq_zero]; rw [ ← sub_comp]; rw [ ← comp_P_infty_eq_zero_iff]; rw [ sub_comp]; rw [ id_comp]; rw [ P_infty_f_idem]; rw [ sub_self],
end
...
begin
 ...
 erw [s.decomposition_id]; erw [ preadditive.sum_comp],
 rw [fintype.sum_eq_single (index_set.id (op [n])), assoc],
 rintros A (hA : ¬A.eq_id),
 rw [assoc]; rw [ s.ι_summand_comp_P_infty_eq_zero A hA]; rw [ comp_zero],
end
...
begin
 ...
 rw [← assoc]; rw [ ← s.comp_P_infty_eq_zero_iff]; rw [ assoc]; rw [ ← P_infty.comm j k]; rw [ s.ι_summand_eq]; rw [ assoc]; rw [ degeneracy_comp_P_infty_assoc X j A.e hA]; rw [ zero_comp]; rw [ comp_zero],
end
...
{ X := s.N,
 ...
 by erw [id_comp]; erw [ homological_complex.d_comp_d_assoc]; erw [ zero_comp],
...
{ hom :=
 ...
 dsimp,
 rw [assoc]; rw [ assoc]; rw [ assoc]; rw [ π_summand_comp_ι_summand_comp_P_infty_eq_P_infty]; rw [ homological_complex.hom.comm],
 end, },
 comm := by { ext n, dsimp, rw [id_comp]; rw [ assoc]; rw [ P_infty_f_idem], }, },
 inv :=
 { f :=
 ...
 rw [sum_comp]; rw [ sum_comp]; rw [ finset.sum_eq_single (index_set.id (op [i])), assoc, assoc],
 { intros A h hA,
...
{ obj := λ S, S.s.nondeg_complex,
 ...
 rw [S₁.s.ι_π_summand_eq_zero_assoc _ _ h']; rw [ S₂.s.ι_π_summand_eq_zero _ _ h']; rw [ zero_comp]; rw [ comp_zero], },
 end }, }
