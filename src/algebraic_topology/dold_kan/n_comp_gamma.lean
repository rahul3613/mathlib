...
begin
 ...
 rw [assoc]; rw [ ← simplex_category.δ_comp_δ'' (fin.zero_le _)],
 simp only [op_comp, X.map_comp, assoc, P_infty_f],
 erw [(higher_faces_vanish.of_P _ _).comp_δ_eq_zero_assoc _ j₂.succ_ne_zero]; erw [ zero_comp],
 rw fin.coe_succ,
 linarith, },
 { simp only [op_comp, X.map_comp, assoc, P_infty_f],
 erw [(higher_faces_vanish.of_P _ _).comp_δ_eq_zero_assoc _ hj₁]; erw [ zero_comp],
 by_contra,
...
begin
 ...
 dsimp,
 rw [← P_infty.comm' _ n rfl]; rw [ alternating_face_map_complex.obj_d_eq],
 simp only [eq_self_iff_true, id_comp, if_true, preadditive.comp_sum],
 rw finset.sum_eq_single (0 : fin (n+2)), rotate,
 { intros b hb hb',
 rw preadditive.comp_zsmul,
 erw [P_infty_comp_map_mono_eq_zero X (simplex_category.δ b) h (by { rw is_δ₀.iff, exact hb', })]; erw [ zsmul_zero], },
 { simp only [finset.mem_univ, not_true, is_empty.forall_iff], },
 { simpa only [hi.eq_δ₀, fin.coe_zero, pow_zero, one_zsmul], }, },
 /- The case `i ≠ δ 0` -/
 { rw [Γ₀.obj.termwise.map_mono_eq_zero _ i _ hi]; rw [ zero_comp], swap,
 { by_contradiction h',
...
begin
 rw [← cancel_epi (compatibility_Γ₂N₁_Γ₂N₂.app X).hom]; rw [ iso.hom_inv_id_assoc],
...
begin
 ...
 erw [splitting.ι_desc_assoc]; erw [ assoc]; erw [ assoc]; erw [ splitting.ι_desc_assoc],
...
by { ext P : 2, dsimp, rw [Γ₂.map_id]; rw [ N₂.map_id]; rw [ comp_id]; rw [ id_comp]; rw [ identity_N₂_objectwise P], }
