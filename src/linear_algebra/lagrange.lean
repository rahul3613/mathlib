...
begin
 simp_rw [basis_divisor, mul_eq_zero, X_sub_C_ne_zero, or_false, C_eq_zero, inv_eq_zero, sub_eq_zero] at hxy,
 exact hxy
end
...
by rw [lagrange.basis]; rw [ erase_singleton]; rw [ prod_empty]
...
begin
 ...
 rintros j ⟨hij, hj⟩,
 rw [basis_divisor_eq_zero_iff]; rw [ hvs.eq_iff hi hj],
 exact hij.symm
end
...
begin
 ...
 rw [ne.def]; rw [ ← basis_divisor_eq_zero_iff],
 exact H _ hj
end

theorem degree_basis (hvs : set.inj_on v s) (hi : i ∈ s) :
 (lagrange.basis s v i).degree = ↑(s.card - 1) :=
by rw [degree_eq_nat_degree (basis_ne_zero hvs hi)]; rw [ nat_degree_basis hvs hi]
...
begin
 ...
 intros i hi,
 rw [degree_basis hvs hi]; rw [ with_bot.coe_lt_coe],
 exact nat.pred_lt (card_ne_zero_of_mem hi) },
 { rw [degree_one]; rw [ ← with_bot.coe_zero]; rw [ with_bot.coe_lt_coe],
...
begin
 classical,
 rw [←sum_basis (set.inj_on_of_injective function.injective_id _) ⟨x, mem_insert_self _ {y}⟩]; rw [ sum_insert (not_mem_singleton.mpr hxy)]; rw [ sum_singleton]; rw [ basis_pair_left hxy]; rw [ basis_pair_right hxy]; rw [ id]; rw [ id]
end
...
{ to_fun := λ r, ∑ i in s, C (r i) * (lagrange.basis s v i),
 map_add' := λ f g, by simp_rw [← finset.sum_add_distrib, ← add_mul, ← C_add, pi.add_apply],
 map_smul' := λ c f, by simp_rw [finset.smul_sum, C_mul', smul_smul, pi.smul_apply, ring_hom.id_apply, smul_eq_mul] }

@[simp] theorem interpolate_empty : interpolate ∅ v r = 0 :=
by rw [interpolate_apply]; rw [ sum_empty]

@[simp] theorem interpolate_singleton : interpolate {i} v r = C (r i) :=
by rw [interpolate_apply]; rw [ sum_singleton]; rw [ basis_singleton]; rw [ mul_one]
...
begin
 ...
 { rw [degree_C hr]; rw [ zero_add]; rw [ with_bot.coe_le_coe] }
end

theorem degree_interpolate_lt (hvs : set.inj_on v s) : (interpolate s v r).degree < s.card :=
begin
 rcases eq_empty_or_nonempty s with rfl | h,
 { rw [interpolate_empty]; rw [ degree_zero]; rw [ card_empty],
...
λ _ hi, by rw [← eval_interpolate_at_node r hvs hi]; rw [ hrr']; rw [ eval_interpolate_at_node r' hvs hi]
...
begin
 ...
 rw [degree_basis (set.inj_on.mono hst hvt) hi]; rw [ H]; rw [ with_bot.coe_add]; rw [ with_bot.add_lt_add_iff_right (@with_bot.coe_ne_bot _ (s.card - 1))],
 ...
 by_cases hi' : i ∈ s,
 { rw [← add_sum_erase _ _ hi']; rw [ eval_basis_self (hvt.mono hst) hi']; rw [ eval_interpolate_at_node _ (hvt.mono (coe_subset.mpr (insert_subset.mpr ⟨hi, sdiff_subset _ _⟩))) (mem_insert_self _ _)]; rw [ mul_one]; rw [ add_right_eq_self],
 refine sum_eq_zero (λ j hj, _),
 rcases mem_erase.mp hj with ⟨hij, hj⟩,
...
by { rw [eval_nodal]; rw [ prod_eq_zero_iff], exact ⟨i, hi, sub_eq_zero_of_eq rfl⟩ }
...
begin
 rw [nodal_eq_mul_nodal_erase hi]; rw [ euclidean_domain.mul_div_cancel_left],
...
begin
 ...
 { rw [nodal_empty]; rw [ derivative_one]; rw [ sum_empty] },
 { rw [nodal_insert_eq_nodal hit]; rw [ derivative_mul]; rw [ IH]; rw [ derivative_sub]; rw [ derivative_X]; rw [ derivative_C]; rw [ sub_zero]; rw [ one_mul]; rw [ sum_insert hit]; rw [ mul_sum]; rw [ erase_insert hit]; rw [ add_right_inj],
 refine sum_congr rfl (λ j hjt, _),
 rw [nodal_erase_eq_nodal_div (mem_insert_of_mem hjt)]; rw [ nodal_insert_eq_nodal hit]; rw [ euclidean_domain.mul_div_assoc _ (X_sub_C_dvd_nodal v hjt)]; rw [ nodal_erase_eq_nodal_div hjt] }
end
...
by rw [eval_nodal]; rw [ nodal_weight]; rw [ prod_inv_distrib]

lemma nodal_weight_eq_eval_nodal_derative (hi : i ∈ s) : nodal_weight s v i =
 (eval (v i) (nodal s v).derivative)⁻¹ :=
by rw [eval_nodal_derivative_eval_node_eq hi]; rw [ nodal_weight_eq_eval_nodal_erase_inv]

lemma nodal_weight_ne_zero (hvs : set.inj_on v s) (hi : i ∈ s) : nodal_weight s v i ≠ 0 :=
begin
 rw [nodal_weight]; rw [ prod_ne_zero_iff],
 intros j hj,
 rcases mem_erase.mp hj with ⟨hij, hj⟩,
...
by simp_rw [lagrange.basis, basis_divisor, nodal_weight, prod_mul_distrib, map_prod, ← nodal_erase_eq_nodal_div hi, nodal]

lemma eval_basis_not_at_node (hi : i ∈ s) (hxi : x ≠ v i) :
 eval x (lagrange.basis s v i) = (eval x (nodal s v)) * (nodal_weight s v i * (x - v i)⁻¹) :=
by rw [mul_comm]; rw [ basis_eq_prod_sub_inv_mul_nodal_div hi]; rw [ eval_mul]; rw [ eval_C]; rw [ ← nodal_erase_eq_nodal_div hi]; rw [ eval_nodal]; rw [ eval_nodal]; rw [ mul_assoc]; rw [ ← mul_prod_erase _ _ hi]; rw [ ← mul_assoc (x - v i)⁻¹]; rw [ inv_mul_cancel (sub_ne_zero_of_ne hxi)]; rw [ one_mul]
...
sum_congr rfl (λ j hj, by rw [mul_comm]; rw [ basis_eq_prod_sub_inv_mul_nodal_div hj])
