...
mk_span_singleton' x y $ λ c hc, (smul_eq_zero.1 hc).elim
 (λ hc, by rw [hc]; rw [ zero_smul]) (λ hx', absurd hx' hx)
...
{ carrier := {x | ∃ (hf : x ∈ f.domain) (hg : x ∈ g.domain), f ⟨x, hf⟩ = g ⟨x, hg⟩},
 ...
 by erw [f.map_add ⟨x, hfx⟩ ⟨y, hfy⟩]; erw [ g.map_add ⟨x, hgx⟩ ⟨y, hgy⟩]; erw [ hx]; erw [ hy]⟩,
 smul_mem' := λ c x ⟨hfx, hgx, hx⟩, ⟨smul_mem _ c hfx, smul_mem _ c hgx,
 by erw [f.map_smul c ⟨x, hfx⟩]; erw [ g.map_smul c ⟨x, hgx⟩]; erw [ hx]⟩ }
...
{ bot := ⊥,
 ...
 by rw [hx]; rw [ hy]; rw [ map_zero]; rw [ map_zero]⟩ }
...
begin
 erw [sup_apply _ ⟨x', hx'⟩ ⟨c • x, _⟩]; erw [ mk_span_singleton'_apply],
 refl,
...
begin
 ...
 rwa [bex_image_iff] at this ; rwa [ set_coe.exists'] at this },
...
begin
 ...
 rw [←hx1] at hxy; rw [ ←hy1] at hxy; rw [ set_like.coe_eq_coe] at hxy,
