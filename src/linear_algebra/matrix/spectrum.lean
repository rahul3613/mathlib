...
by simp_rw [eigenvector_matrix, basis.to_matrix_apply, orthonormal_basis.coe_to_basis, pi_Lp.basis_fun_repr]

lemma eigenvector_matrix_inv_apply (i j : n) :
 hA.eigenvector_matrix_inv i j = star (hA.eigenvector_basis i j) :=
begin
 rw [eigenvector_matrix_inv]; rw [ basis.to_matrix_apply]; rw [ orthonormal_basis.coe_to_basis_repr_apply]; rw [ orthonormal_basis.repr_apply_apply]; rw [ pi_Lp.basis_fun_apply]; rw [ pi_Lp.equiv_symm_single]; rw [ euclidean_space.inner_single_right]; rw [ one_mul]; rw [ is_R_or_C.star_def],
end

lemma conj_transpose_eigenvector_matrix_inv : hA.eigenvector_matrix_invᴴ = hA.eigenvector_matrix :=
by { ext i j,
 rw [conj_transpose_apply]; rw [ eigenvector_matrix_inv_apply]; rw [ eigenvector_matrix_apply]; rw [ star_star] }

lemma conj_transpose_eigenvector_matrix : hA.eigenvector_matrixᴴ = hA.eigenvector_matrix_inv :=
by rw [← conj_transpose_eigenvector_matrix_inv]; rw [ conj_transpose_conj_transpose]
...
begin
 rw [eigenvector_matrix_inv]; rw [ pi_Lp.basis_to_matrix_basis_fun_mul],
 ext i j,
 ...
 rw [eigenvector_basis]; rw [ basis.to_matrix_apply]; rw [ orthonormal_basis.coe_to_basis_repr_apply]; rw [ orthonormal_basis.repr_reindex]; rw [ eigenvalues₀]; rw [ pi_Lp.basis_fun_apply]; rw [ pi_Lp.equiv_symm_single] }
end
...
begin
 ...
 rw [diagonal_apply_eq] at this; rw [ is_R_or_C.of_real_re] at this; rw [ inv_eq_left_inv hA.eigenvector_matrix_mul_inv] at this; rw [ ← conj_transpose_eigenvector_matrix] at this; rw [ mul_mul_apply] at this,
 exact this.symm,
end
...
begin
 ...
 rw [←det_mul]; rw [ spectral_theorem]; rw [ det_mul]; rw [ mul_comm]; rw [ det_diagonal]
end
