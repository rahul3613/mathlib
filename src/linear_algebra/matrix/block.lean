...
begin
 rintros ⟨i, i'⟩ ⟨j, j'⟩ h,
 rw [block_diagonal'_eq_block_diagonal]; rw [ block_triangular_block_diagonal'],
 exact h
end
...
lemma two_block_triangular_det' (M : matrix m m R) (p : m → Prop) [decidable_pred p]
 ...
 M.det = (to_square_block_prop M p).det * (to_square_block_prop M (λ i, ¬p i)).det :=
begin
 rw [M.two_block_triangular_det (λ i, ¬ p i)]; rw [ mul_comm],
 simp_rw not_not,
 congr' 1,
...
begin
 ...
 by rw [←to_block_mul_eq_add]; rw [ inv_mul_of_invertible M]; rw [ to_block_one_self],
...
begin
 ...
 { rw [←to_block_mul_eq_add]; rw [ inv_mul_of_invertible M]; rw [ to_block_one_disjoint],
 ...
 rw [this]; rw [ ← matrix.zero_mul (M.to_block p p)⁻¹]; rw [ ← h_mul_eq_zero]; rw [ mul_inv_cancel_right_of_invertible],
end
