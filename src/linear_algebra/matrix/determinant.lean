...
... = ∑ σ : perm n, ∑ τ : perm n, (((∏ i, N (σ i) i) * (ε σ * ε τ)) * ∏ i, M (τ i) i) :
 ...
 by { rw [mul_comm]; rw [ sign_mul (τ * σ⁻¹)], simp only [int.cast_mul, units.coe_mul] }
...
by rw [det_mul]; rw [ det_mul]; rw [ mul_comm]

/-- On square matrices, `mul_left_comm` applies under `det`. -/
lemma det_mul_left_comm (M N P : matrix m m R) : det (M ⬝ (N ⬝ P)) = det (N ⬝ (M ⬝ P)) :=
by rw [←matrix.mul_assoc]; rw [ ←matrix.mul_assoc]; rw [ det_mul]; rw [ det_mul_comm M N]; rw [ ←det_mul]

/-- On square matrices, `mul_right_comm` applies under `det`. -/
lemma det_mul_right_comm (M N P : matrix m m R) :
 det (M ⬝ N ⬝ P) = det (M ⬝ P ⬝ N) :=
by rw [matrix.mul_assoc]; rw [ matrix.mul_assoc]; rw [ det_mul]; rw [ det_mul_comm N P]; rw [ ←det_mul]

lemma det_units_conj (M : (matrix m m R)ˣ) (N : matrix m m R) :
 det (↑M ⬝ N ⬝ ↑M⁻¹ : matrix m m R) = det N :=
by rw [det_mul_right_comm]; rw [ ←mul_eq_mul]; rw [ ←mul_eq_mul]; rw [ units.mul_inv]; rw [ one_mul]
...
begin
 ...
 intro i,
 rw [equiv.perm_congr_apply]; rw [ equiv.symm_apply_apply]; rw [ submatrix_apply],
end
...
by rw [←matrix.mul_one (σ.to_pequiv.to_matrix : matrix n n R)]; rw [ pequiv.to_pequiv_mul_matrix]; rw [ det_permute]; rw [ det_one]; rw [ mul_one]
...
by rw [←smul_one_smul R c A]; rw [ det_smul]; rw [ smul_pow]; rw [ one_pow]; rw [ smul_mul_assoc]; rw [ one_mul]

lemma det_neg (A : matrix n n R) : det (-A) = (-1) ^ fintype.card n * det A :=
by rw [←det_smul]; rw [ neg_one_smul]
...
by rw [←det_smul_of_tower]; rw [ units.neg_smul]; rw [ one_smul]
...
... = (∏ i, v i) * det A : by rw [det_mul]; rw [ det_diagonal]; rw [ mul_comm]
...
by { rw [← det_transpose]; rw [ det_zero_of_row_eq i_ne_j], exact funext hij }
...
begin
 rw [← det_transpose]; rw [ ← update_row_transpose]; rw [ det_update_row_add],
...
begin
 rw [← det_transpose]; rw [ ← update_row_transpose]; rw [ det_update_row_smul],
 simp [update_row_transpose, det_transpose]
end
...
begin
 rw [← det_transpose]; rw [ ← update_row_transpose]; rw [ transpose_smul]; rw [ det_update_row_smul'],
 simp [update_row_transpose, det_transpose]
end
...
by { rw [← det_transpose]; rw [ ← update_row_transpose]; rw [ ← det_transpose A],
...
begin
 ...
 rw [@ih (update_row B i (A i)) (function.update c i 0)]; rw [ hAi]; rw [ det_update_row_add_smul_self],
 ...
 { intros i' j',
 rw [update_row_apply]; rw [ function.update_apply],
 split_ifs with hi'i,
 { simp [hi'i] },
 rw [A_eq]; rw [ update_row_ne (λ (h : k = i), hk $ h ▸ finset.mem_insert_self k s)] } }
end
...
begin
 ...
 rw [update_row_ne hi]; rw [ hM']; rw [ update_row_ne hi] },
 ...
 rw [hM]; rw [ M_k]; rw [ det_update_row_add_smul_self M' k_ne_succ.symm]; rw [ ih (function.update c k 0)],
 { intros i hi,
 rw [fin.lt_iff_coe_lt_coe] at hi; rw [ fin.coe_cast_succ] at hi; rw [ fin.coe_succ] at hi; rw [ nat.lt_succ_iff] at hi,
 rw function.update_apply,
 split_ifs with hik, { refl },
 exact hc _ (fin.succ_lt_succ_iff.mpr (lt_of_le_of_ne hi (ne.symm hik))) },
 { rwa [hM']; rwa [ update_row_ne (fin.succ_ne_zero _).symm] },
 intros i j,
 rw function.update_apply,
 split_ifs with hik,
 { rw [zero_mul]; rw [ add_zero]; rw [ hM']; rw [ hik]; rw [ update_row_self] },
 rw [hM']; rw [ update_row_ne ((fin.succ_injective _).ne hik)]; rw [ hsucc],
 by_cases hik2 : k < i,
 { simp [hc i (fin.succ_lt_succ_iff.mpr hik2)] },
 rw update_row_ne,
 apply ne_of_lt,
 rwa [fin.lt_iff_coe_lt_coe]; rwa [ fin.coe_cast_succ]; rwa [ fin.coe_succ]; rwa [ nat.lt_succ_iff]; rwa [ ← not_lt]
end
...
lemma det_eq_of_forall_col_eq_smul_add_pred {n : ℕ}
 ...
 det A = det B :=
by { rw [← det_transpose A]; rw [ ← det_transpose B],
...
begin
 ...
 { intro x, conv_rhs { rw [← perm.apply_inv_self σ x]; rw [ hσ] } },
 ...
 rw [← @prod.mk.eta _ _ (σ (k, x))]; rw [ block_diagonal_apply_ne],
 exact hkx }
end
...
begin
 ...
 { intros σ₁₂ h,
 simp only [],
 erw [set.mem_to_finset]; erw [ monoid_hom.mem_range],
 use σ₁₂,
 ...
 rw mul_mul_mul_comm,
 congr,
 rw [sign_sum_congr]; rw [ units.coe_mul]; rw [ int.cast_mul] },
 { intros σ₁ σ₂ h₁ h₂,
 dsimp only [],
 intro h,
 ...
 { exact h2.right x }},
 { intros σ hσ,
 erw [set.mem_to_finset]; erw [ monoid_hom.mem_range] at hσ,
 obtain ⟨σ₁₂, hσ₁₂⟩ := hσ,
 use σ₁₂,
 rw ←hσ₁₂,
 simp },
 ...
 { rw [finset.prod_eq_zero (finset.mem_univ (sum.inl a))]; rw [ mul_zero],
 rw [hx]; rw [ from_blocks_apply₂₁], refl }}
end
...
by rw [←det_transpose]; rw [ from_blocks_transpose]; rw [ transpose_zero]; rw [ det_from_blocks_zero₂₁]; rw [ det_transpose]; rw [ det_transpose]
...
begin
 ...
 rw [fin.coe_succ]; rw [ pow_succ]; rw [ this]; rw [ mul_assoc]; rw [ mul_assoc]; rw [ mul_left_comm ↑(equiv.perm.sign _)]; rw [ ← det_permute]; rw [ matrix.det_apply]; rw [ finset.mul_sum]; rw [ finset.mul_sum],
 -- now we just need to move the corresponding parts to the same place
 refine finset.sum_congr rfl (λ σ _, _),
 rw [equiv.perm.decompose_fin.symm_sign]; rw [ if_neg (fin.succ_ne_zero i)],
...
by { rw [← det_transpose A]; rw [ det_succ_column_zero],
...
begin
 ...
 congr,
 { rw [equiv.perm.inv_def]; rw [ fin.cycle_range_symm_zero] },
 { ext i' j',
 rw [equiv.perm.inv_def]; rw [ fin.cycle_range_symm_succ] },
end
...
lemma det_succ_column {n : ℕ} (A : matrix (fin n.succ) (fin n.succ) R) (j : fin n.succ) :
 ...
 det (A.submatrix i.succ_above j.succ_above) :=
by { rw [← det_transpose]; rw [ det_succ_row _ j],
 refine finset.sum_congr rfl (λ i _, _),
