...
invertible_of_left_inverse _ (from_blocks (⅟A) (-(⅟A⬝B⬝⅟D)) 0 (⅟D)) $
 by simp_rw [from_blocks_multiply, matrix.mul_zero, matrix.zero_mul, zero_add, add_zero, matrix.neg_mul, matrix.inv_of_mul_self, matrix.mul_inv_of_mul_self_cancel, add_right_neg, from_blocks_one]
...
invertible_of_left_inverse _ (from_blocks (⅟A) 0 (-(⅟D⬝C⬝⅟A)) (⅟D)) $
 ...
 by simp_rw [from_blocks_multiply, matrix.mul_zero, matrix.zero_mul, zero_add, add_zero, matrix.neg_mul, matrix.inv_of_mul_self, matrix.mul_inv_of_mul_self_cancel, add_left_neg, from_blocks_one]
...
{ fst := invertible_of_left_inverse _ (⅟(from_blocks A B 0 D)).to_blocks₁₁ $ begin
 ...
 rw [←from_blocks_to_blocks (⅟(from_blocks A B 0 D))] at this; rw [ from_blocks_multiply] at this,
...
{ fst := invertible_of_right_inverse _ (⅟(from_blocks A 0 C D)).to_blocks₁₁ $ begin
 ...
 rw [←from_blocks_to_blocks (⅟(from_blocks A 0 C D))] at this; rw [ from_blocks_multiply] at this,
 replace := congr_arg matrix.to_blocks₁₁ this,
...
begin
 ...
 simp_rw [nonsing_inv_eq_ring_inverse, ring.inverse_non_unit _ hA, ring.inverse_non_unit _ hD, ring.inverse_non_unit _ this, matrix.zero_mul, neg_zero, from_blocks_zero] }
end
...
by rw [from_blocks_eq_of_invertible₁₁]; rw [ det_mul]; rw [ det_mul]; rw [ det_from_blocks_zero₂₁]; rw [ det_from_blocks_zero₂₁]; rw [ det_from_blocks_zero₁₂]; rw [ det_one]; rw [ det_one]; rw [ one_mul]; rw [ one_mul]; rw [ mul_one]
...
begin
 ...
 rw [det_from_blocks₁₁]; rw [ inv_of_one]; rw [ matrix.mul_one]; rw [ det_one]; rw [ one_mul],
end
...
begin
 ...
 rw [this]; rw [ det_submatrix_equiv_self]; rw [ det_from_blocks₁₁],
end
...
lemma det_one_add_mul_comm (A : matrix m n α) (B : matrix n m α) :
 det (1 + A ⬝ B) = det (1 + B ⬝ A) :=
calc det (1 + A ⬝ B)
 = det (from_blocks 1 (-A) B 1) : by rw [det_from_blocks_one₂₂]; rw [ matrix.neg_mul]; rw [ sub_neg_eq_add]
... = det (1 + B ⬝ A) : by rw [det_from_blocks_one₁₁]; rw [ matrix.mul_neg]; rw [ sub_neg_eq_add]

/-- Alternate statement of the **Weinstein–Aronszajn identity** -/
lemma det_mul_add_one_comm (A : matrix m n α) (B : matrix n m α) :
 det (A ⬝ B + 1) = det (B ⬝ A + 1) :=
by rw [add_comm]; rw [ det_one_add_mul_comm]; rw [ add_comm]

lemma det_one_sub_mul_comm (A : matrix m n α) (B : matrix n m α) :
 det (1 - A ⬝ B) = det (1 - B ⬝ A) :=
by rw [sub_eq_add_neg]; rw [ ←matrix.neg_mul]; rw [ det_one_add_mul_comm]; rw [ matrix.mul_neg]; rw [ ←sub_eq_add_neg]

/-- A special case of the **Matrix determinant lemma** for when `A = I`.

TODO: show this more generally. -/
lemma det_one_add_col_mul_row (u v : m → α) : det (1 + col u ⬝ row v) = 1 + v ⬝ᵥ u :=
by rw [det_one_add_mul_comm]; rw [ det_unique]; rw [ pi.add_apply]; rw [ pi.add_apply]; rw [ matrix.one_apply_eq]; rw [ matrix.row_mul_col_apply]
...
begin
 rw [←is_hermitian_submatrix_equiv (equiv.sum_comm n m)]; rw [ equiv.sum_comm_apply]; rw [ from_blocks_submatrix_sum_swap_sum_swap],
...
begin
 rw [pos_semidef]; rw [ is_hermitian.from_blocks₁₁ _ _ hA.1],
 split,
 { refine λ h, ⟨h.1, λ x, _⟩,
 have := h.2 (- ((A⁻¹ ⬝ B).mul_vec x) ⊕ᵥ x),
 rw [dot_product_mul_vec] at this; rw [ schur_complement_eq₁₁ B D _ _ hA.1] at this; rw [ neg_add_self] at this; rw [ dot_product_zero] at this; rw [ zero_add] at this,
 rw [dot_product_mul_vec], exact this },
 { refine λ h, ⟨h.1, λ x, _⟩,
 rw [dot_product_mul_vec]; rw [ ← sum.elim_comp_inl_inr x]; rw [ schur_complement_eq₁₁ B D _ _ hA.1]; rw [ map_add],
...
begin
 rw [←pos_semidef_submatrix_equiv (equiv.sum_comm n m)]; rw [ equiv.sum_comm_apply]; rw [ from_blocks_submatrix_sum_swap_sum_swap],
