...
begin
 ...
 rw [mul_vec_transpose]; rw [ matrix.dot_product_mul_vec]; rw [ star_star]; rw [ dot_product_comm]
end
...
lemma pos_def_of_to_matrix'
 ...
 Q.pos_def :=
begin
 rw [←to_quadratic_form_associated ℝ Q]; rw [ ←bilin_form.to_matrix'.left_inv ((associated_hom _) Q)],
...
lemma pos_def_to_matrix' [decidable_eq n] {Q : quadratic_form ℝ (n → ℝ)} (hQ : Q.pos_def) :
 Q.to_matrix'.pos_def :=
begin
 rw [←to_quadratic_form_associated ℝ Q] at hQ; rw [ ←bilin_form.to_matrix'.left_inv ((associated_hom _) Q)] at hQ,
...
{ inner := λ x y, dot_product (star x) (M.mul_vec y),
 ...
 rw [star_dot_product]; rw [ star_ring_end_apply]; rw [ star_star]; rw [ star_mul_vec]; rw [ dot_product_mul_vec]; rw [ hM.is_hermitian.eq],
 nonneg_re := λ x,
 begin
 by_cases h : x = 0,
 { simp [h] },
 ...
 smul_left := λ x y r, by rw [← smul_eq_mul]; rw [ ←smul_dot_product]; rw [ star_ring_end_apply]; rw [ ← star_smul] }
