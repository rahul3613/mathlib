...
begin
 ...
 { rw [function.update_noteq (ne.symm h)]; rw [ pi.zero_apply] }
end
...
{ to_fun := M'.to_linear_map_right',
 ...
 left_inv := λ x, by
 rw [← matrix.to_linear_map_right'_mul_apply]; rw [ hM'M]; rw [ matrix.to_linear_map_right'_one]; rw [ id_apply],
 right_inv := λ x, by
 rw [← matrix.to_linear_map_right'_mul_apply]; rw [ hMM']; rw [ matrix.to_linear_map_right'_one]; rw [ id_apply],
...
begin
 ...
 simp_rw [range_eq_map, ←supr_range_std_basis, submodule.map_supr, range_eq_map, ←ideal.span_singleton_one, ideal.span, submodule.map_span, image_image, image_singleton, matrix.mul_vec_lin_apply, M.mul_vec_std_basis_apply, supr_span, range_eq_Union]
end
...
by { ext, rw [matrix.one_apply]; rw [ linear_map.to_matrix'_apply]; rw [ id_apply] }
...
by rw [matrix.to_lin'_mul]; rw [ linear_map.comp_apply]
...
suffices (f.comp g) = (f.to_matrix' ⬝ g.to_matrix').to_lin',
 by rw [this]; rw [ linear_map.to_matrix'_to_lin'],
by rw [matrix.to_lin'_mul]; rw [ matrix.to_lin'_to_matrix']; rw [ matrix.to_lin'_to_matrix']
...
{ to_fun := matrix.to_lin' M',
 inv_fun := M.to_lin',
 left_inv := λ x, by rw [← matrix.to_lin'_mul_apply]; rw [ hMM']; rw [ matrix.to_lin'_one]; rw [ id_apply],
 right_inv := λ x, by rw [← matrix.to_lin'_mul_apply]; rw [ hM'M]; rw [ matrix.to_lin'_one]; rw [ id_apply],
 .. matrix.to_lin' M' }
...
by rw [← matrix.to_lin_symm]; rw [ linear_equiv.apply_symm_apply]

@[simp] lemma linear_map.to_matrix_to_lin (M : matrix m n R) :
 linear_map.to_matrix v₁ v₂ (matrix.to_lin v₁ v₂ M) = M :=
by rw [← matrix.to_lin_symm]; rw [ linear_equiv.symm_apply_apply]

lemma linear_map.to_matrix_apply (f : M₁ →ₗ[R] M₂) (i : m) (j : n) :
 linear_map.to_matrix v₁ v₂ f i j = v₂.repr (f (v₁ j)) i :=
begin
 rw [linear_map.to_matrix]; rw [ linear_equiv.trans_apply]; rw [ linear_map.to_matrix'_apply]; rw [ linear_equiv.arrow_congr_apply]; rw [ basis.equiv_fun_symm_apply]; rw [ finset.sum_eq_single j]; rw [ if_pos rfl]; rw [ one_smul]; rw [ basis.equiv_fun_apply],
 { intros j' _ hj',
...
by rw [matrix.to_lin'_apply]; rw [ v₂.equiv_fun_symm_apply]

@[simp] lemma matrix.to_lin_self (M : matrix m n R) (i : n) :
 matrix.to_lin v₁ v₂ M (v₁ i) = ∑ j, M j i • v₂ j :=
begin
 rw [matrix.to_lin_apply]; rw [ finset.sum_congr rfl (λ j hj, _)],
 rw [basis.repr_self]; rw [ matrix.mul_vec]; rw [ dot_product]; rw [ finset.sum_eq_single i]; rw [ finsupp.single_eq_same]; rw [ mul_one],
 { intros i' _ i'_ne, rw [finsupp.single_eq_of_ne i'_ne.symm]; rw [ mul_zero] },
 { intros,
...
by rw [← linear_map.to_matrix_id v₁]; rw [ matrix.to_lin_to_matrix]
...
by simp_rw [linear_map.to_matrix, linear_equiv.trans_apply, linear_equiv.arrow_congr_comp _ v₂.equiv_fun, linear_map.to_matrix'_comp]
...
by { ext i,
 rw [← matrix.to_lin'_apply]; rw [ linear_map.to_matrix]; rw [ linear_equiv.trans_apply]; rw [ matrix.to_lin'_to_matrix']; rw [ linear_equiv.arrow_congr_apply]; rw [ v₂.equiv_fun_apply],
 congr,
...
by rw [matrix.to_lin_mul v₁ v₂]; rw [ linear_map.comp_apply]
...
{ to_fun := matrix.to_lin v₁ v₂ M,
 ...
 left_inv := λ x, by rw [← matrix.to_lin_mul_apply]; rw [ hM'M]; rw [ matrix.to_lin_one]; rw [ id_apply],
 right_inv := λ x, by rw [← matrix.to_lin_mul_apply]; rw [ hMM']; rw [ matrix.to_lin_one]; rw [ id_apply],
 .. matrix.to_lin v₁ v₂ M }
...
by rw [← matrix.to_lin_alg_equiv_symm]; rw [ alg_equiv.apply_symm_apply]
...
by rw [← matrix.to_lin_alg_equiv_symm]; rw [ alg_equiv.symm_apply_apply]
...
by rw [matrix.to_lin_alg_equiv'_apply]; rw [ v₁.equiv_fun_symm_apply]
...
by simp_rw [linear_map.to_matrix_alg_equiv, alg_equiv.of_linear_equiv_apply, linear_map.to_matrix_id]

@[simp]
lemma matrix.to_lin_alg_equiv_one : matrix.to_lin_alg_equiv v₁ 1 = id :=
by rw [← linear_map.to_matrix_alg_equiv_id v₁]; rw [ matrix.to_lin_alg_equiv_to_matrix_alg_equiv]
...
begin
 ext,
 rw [linear_map.to_matrix_apply]; rw [ distrib_mul_action.to_linear_map_apply]; rw [ linear_equiv.map_smul]; rw [ basis.repr_self]; rw [ finsupp.smul_single_one]; rw [ finsupp.single_eq_pi_single]; rw [ matrix.diagonal_apply]; rw [ pi.single_apply],
end
...
{ to_fun := λ x, linear_map.to_matrix b b (algebra.lmul R S x),
 map_zero' := by rw [alg_hom.map_zero]; rw [ linear_equiv.map_zero],
 map_one' := by rw [alg_hom.map_one]; rw [ linear_map.to_matrix_one],
 map_add' := λ x y, by rw [alg_hom.map_add]; rw [ linear_equiv.map_add],
 map_mul' := λ x y, by rw [alg_hom.map_mul]; rw [ linear_map.to_matrix_mul]; rw [ matrix.mul_eq_mul],
 commutes' := λ r, by { ext, rw [lmul_algebra_map]; rw [ to_matrix_lsmul]; rw [ algebra_map_eq_diagonal]; rw [ pi.algebra_map_def]; rw [ algebra.id.map_eq_self] } }
...
by rw [left_mul_matrix_apply]; rw [ to_matrix_lmul' b x i j]
...
begin
 ext ⟨i, k⟩ ⟨j, k'⟩,
 rw [smul_left_mul_matrix]; rw [ alg_hom.commutes]; rw [ block_diagonal_apply]; rw [ algebra_map_matrix_apply],
...
by rw [smul_left_mul_matrix_algebra_map]; rw [ block_diagonal_apply_eq]

lemma smul_left_mul_matrix_algebra_map_ne (x : S) (i j) {k k'}
 (h : k ≠ k') : left_mul_matrix (b.smul c) (algebra_map _ _ x) (i, k) (j, k') = 0 :=
by rw [smul_left_mul_matrix_algebra_map]; rw [ block_diagonal_apply_ne _ _ _ h]
...
{ map_mul' := λ f g, by apply e.arrow_congr_comp,
 ...
 rw [linear_equiv.map_smul]; rw [ linear_equiv.conj_id], },
 ..e.conj }
