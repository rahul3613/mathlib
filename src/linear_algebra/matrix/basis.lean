...
by { ext, rw [basis.to_matrix_apply]; rw [ linear_map.to_matrix_apply]; rw [ basis.constr_basis] }
...
begin
 ext i' k,
 rw [basis.to_matrix]; rw [ matrix.update_column_apply]; rw [ e.to_matrix_apply],
 split_ifs,
 { rw [h]; rw [ update_same j x v] },
 { rw update_noteq h },
end
...
v.ext (λ i, by rw [to_lin_self]; rw [ id_apply]; rw [ e.sum_to_matrix_smul_self])
...
{ to_fun := e.to_matrix,
 ...
 ext i j,
 change _ = _ + _,
 rw [e.to_matrix_apply]; rw [ pi.add_apply]; rw [ linear_equiv.map_add],
 refl
 end,
 map_smul' := begin
 intros c v,
 ext i j,
 rw [e.to_matrix_apply]; rw [ pi.smul_apply]; rw [ linear_equiv.map_smul],
 refl
 end,
...
(matrix.to_lin b' c).injective
 ...
 rw [to_lin_to_matrix]; rw [ to_lin_mul b' c' c]; rw [ to_lin_to_matrix]; rw [ c.to_lin_to_matrix]; rw [ id_comp])
...
(matrix.to_lin b c').injective
 (by rw [to_lin_to_matrix]; rw [ to_lin_mul b b' c']; rw [ to_lin_to_matrix]; rw [ b'.to_lin_to_matrix]; rw [ comp_id])
...
by rw [basis_to_matrix_mul_linear_map_to_matrix]; rw [ linear_map_to_matrix_mul_basis_to_matrix]
...
begin
 ...
 ext i j,
 rw [linear_map.to_matrix_apply]; rw [ matrix.to_lin'_apply]; rw [ pi.basis_fun_apply]; rw [ matrix.mul_vec_std_basis_apply]; rw [ matrix.of_apply]
end
...
by { haveI := classical.dec_eq ι',
 rw [←@basis_to_matrix_mul_linear_map_to_matrix _ _ ι]; rw [ to_matrix_id]; rw [ matrix.mul_one] }
...
by rw [basis.to_matrix_mul_to_matrix]; rw [ basis.to_matrix_self]
