...
begin
 rw [← comap_bot]; rw [ ← infi_ker_proj]; rw [ comap_infi],
...
begin
 ...
 rw [← submodule.map_top]; rw [ ← supr_range_std_basis]; rw [ submodule.map_supr],
 congr, funext i,
 rw [← linear_map.range_comp]; rw [ diagonal_comp_std_basis]; rw [ ← range_smul']
end
...
begin
 ...
 rw [rank]; rw [ range_diagonal]; rw [ B₁]; rw [ ←@rank_fun' K],
