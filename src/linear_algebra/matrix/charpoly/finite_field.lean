...
begin
 ...
 unfold charpoly, rw [alg_hom.map_det]; rw [ ← coe_det_monoid_hom]; rw [ ← (det_monoid_hom : matrix n n K[X] →* K[X]).map_pow],
 apply congr_arg det,
 refine mat_poly_equiv.injective _,
 rw [alg_equiv.map_pow]; rw [ mat_poly_equiv_charmatrix]; rw [ hk]; rw [ sub_pow_char_pow_of_commute]; rw [ ← C_pow],
...
begin
 ...
 rw [matrix.trace_eq_neg_charpoly_coeff]; rw [ matrix.trace_eq_neg_charpoly_coeff]; rw [ finite_field.matrix.charpoly_pow_card]; rw [ finite_field.pow_card]
end
