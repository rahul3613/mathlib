...
begin
 ...
 intros c hc, rw [← C_eq_int_cast]; rw [ C_mul'],
...
begin
 nontriviality,
 by_cases fintype.card n = 0, {rw [charpoly]; rw [ det_of_card_zero h], apply monic_one},
...
begin
 ...
 rw [fintype.card]; rw [ prod_X_sub_C_coeff_card_pred univ (λ i : n, M i i) fintype.card_pos]; rw [ neg_neg]; rw [ trace],
 refl
end
...
begin
 rw [polynomial.eval]; rw [ ← coe_eval₂_ring_hom]; rw [ ring_hom.map_det],
...
begin
 rw [coeff_zero_eq_eval_zero]; rw [ charpoly]; rw [ eval_det]; rw [ mat_poly_equiv_charmatrix]; rw [ ← det_smul],
 simp
end
...
begin
 ext m,
 rw [coeff_sub]; rw [ coeff_C]; rw [ mat_poly_equiv_coeff_apply]; rw [ ring_hom.map_matrix_apply]; rw [ matrix.map_apply]; rw [ alg_hom.coe_to_ring_hom]; rw [ dmatrix.sub_apply]; rw [ coeff_X_pow],
 by_cases hij : i = j,
 { rw [hij]; rw [ charmatrix_apply_eq]; rw [ alg_hom.map_sub]; rw [ expand_C]; rw [ expand_X]; rw [ coeff_sub]; rw [ coeff_X_pow]; rw [ coeff_C],
 split_ifs with mp m0;
 simp only [matrix.one_apply_eq, dmatrix.zero_apply] },
 { rw [charmatrix_apply_ne _ _ _ hij]; rw [ alg_hom.map_neg]; rw [ expand_C]; rw [ coeff_neg]; rw [ coeff_C],
...
begin
 ...
 have : ∑ (x : n), 1 = fintype.card n := by rw [finset.sum_const]; rw [ card_univ]; rw [ smul_eq_mul]; rw [ mul_one],
 rw ← this,
 apply coeff_prod_mem_ideal_pow_tsub,
 rintro i - (_|k),
 { rw [tsub_zero]; rw [ pow_one]; rw [ charmatrix_apply]; rw [ coeff_sub]; rw [ coeff_X_mul_zero]; rw [ coeff_C_zero]; rw [ zero_sub]; rw [ neg_mem_iff],
 exact h (c i) i },
 { rw [nat.succ_eq_one_add]; rw [ tsub_self_add]; rw [ pow_zero]; rw [ ideal.one_eq_top],
