...
begin
 rw [pi_to_module.from_matrix_apply]; rw [ fintype.total_apply]; rw [ matrix.mul_vec_single],
 simp_rw [mul_one]
end
...
begin
 ...
 rwa [pi_to_module.from_End_apply_single_one] at this ; rwa [ pi_to_module.from_matrix_apply_single_one] at this },
 { intros h,
 ext,
 simp_rw [linear_map.comp_apply, linear_map.coe_single, pi_to_module.from_End_apply_single_one, pi_to_module.from_matrix_apply_single_one],
 apply h }
end
...
begin
 ...
 rw [linear_map.comp_apply]; rw [ alg_equiv.to_linear_map_apply]; rw [ map_mul],
 ext,
...
begin
 ...
 rw [linear_map.comp_apply]; rw [ alg_equiv.to_linear_map_apply]; rw [ map_one],
 ext,
 refl
end
...
begin
 delta matrix.represents, rw [map_zero]; rw [ map_zero],
end
...
begin
 delta matrix.represents at ⊢ h, rw [map_smul]; rw [ map_smul]; rw [ h],
end
...
begin
 ...
 { rw [ideal.range_finsupp_total]; rw [ hb], exact λ x, hI (f.mem_range_self x) },
 choose bM' hbM',
...
begin
 ...
 (by rw [subtype.range_coe_subtype]; rw [ finset.set_of_mem]; rw [ hs]) f I hI,
 refine ⟨A.1.charpoly, A.1.charpoly_monic, _, _⟩,
 { rw A.1.charpoly_nat_degree_eq_dim,
 exact coeff_charpoly_mem_ideal_pow h },
 { rw [polynomial.aeval_alg_hom_apply]; rw [ ← map_zero (matrix.is_representation.to_End R coe _)],
 congr' 1,
 ext1,
 rw [polynomial.aeval_subalgebra_coe]; rw [ subtype.val_eq_coe]; rw [ matrix.aeval_self_charpoly]; rw [ subalgebra.coe_zero] },
 { apply_instance }
end
