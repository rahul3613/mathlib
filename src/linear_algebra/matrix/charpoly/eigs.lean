...
begin
 rw [det_eq_sign_charpoly_coeff]; rw [ ← (charpoly_nat_degree_eq_dim A)]; rw [ polynomial.prod_roots_eq_coeff_zero_of_monic_of_split A.charpoly_monic (hAps)]; rw [ ← mul_assoc]; rw [ ← pow_two]; rw [ pow_right_comm]; rw [ neg_one_sq]; rw [ one_pow]; rw [ one_mul],
end
...
begin
 ...
 { rw [matrix.trace]; rw [ fintype.sum_empty]; rw [ matrix.charpoly]; rw [ det_eq_one_of_card_eq_zero (fintype.card_eq_zero_iff.2 h)]; rw [ polynomial.roots_one]; rw [ multiset.empty_eq_zero]; rw [ multiset.sum_zero], },
 { rw [trace_eq_neg_charpoly_coeff]; rw [ neg_eq_iff_eq_neg]; rw [ ← polynomial.sum_roots_eq_next_coeff_of_monic_of_split A.charpoly_monic (hAps)]; rw [ next_coeff]; rw [ charpoly_nat_degree_eq_dim]; rw [ if_neg (fintype.card_ne_zero : fintype.card n ≠ 0)], },
end
variables (A)
