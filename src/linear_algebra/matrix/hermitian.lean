...
lemma is_hermitian.transpose {A : matrix n n α} (h : A.is_hermitian) :
 Aᵀ.is_hermitian :=
by { rw [is_hermitian]; rw [ conj_transpose]; rw [ transpose_map], congr, exact h }
...
begin
 have hCB : Cᴴ = B,
 { rw [← hBC]; rw [ conj_transpose_conj_transpose] },
...
lemma is_hermitian_mul_conj_transpose_self [fintype n] (A : matrix m n α) :
 (A ⬝ Aᴴ).is_hermitian :=
by rw [is_hermitian]; rw [ conj_transpose_mul]; rw [ conj_transpose_conj_transpose]
...
lemma is_hermitian.coe_re_apply_self {A : matrix n n α} (h : A.is_hermitian) (i : n) :
 (re (A i i) : α) = A i i :=
by rw [←conj_eq_iff_re]; rw [ ←star_def]; rw [ ←conj_transpose_apply]; rw [ h.eq]
...
begin
 rw [linear_map.is_symmetric]; rw [ (pi_Lp.equiv 2 (λ _ : n, α)).symm.surjective.forall₂],
