...
begin
 ...
 { simp },
 { rw [pow_succ A]; rw [ mul_eq_mul]; rw [ mul_inv_rev]; rw [ ← ih]; rw [ ← mul_eq_mul]; rw [ ← pow_succ'] }
end

theorem pow_sub' (A : M) {m n : ℕ} (ha : is_unit A.det) (h : n ≤ m) :
 A ^ (m - n) = A ^ m ⬝ (A ^ n)⁻¹ :=
begin
 rw [←tsub_add_cancel_of_le h]; rw [ pow_add]; rw [ mul_eq_mul]; rw [ matrix.mul_assoc]; rw [ mul_nonsing_inv]; rw [ tsub_add_cancel_of_le h]; rw [ matrix.mul_one],
...
| (n : ℕ) := by rw [zpow_coe_nat]; rw [ one_pow]
| -[1+ n] := by rw [zpow_neg_succ_of_nat]; rw [ one_pow]; rw [ inv_one]

lemma zero_zpow : ∀ z : ℤ, z ≠ 0 → (0 : M) ^ z = 0
| (n : ℕ) h := by { rw [zpow_coe_nat]; rw [ zero_pow], refine lt_of_le_of_ne n.zero_le (ne.symm _),
 simpa using h }
...
| (n : ℕ) := by rw [zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ inv_pow']
| -[1+ n] := by rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ inv_pow']
...
begin
 ...
 { simp },
 { rw [←int.coe_nat_succ]; rw [ zpow_coe_nat]; rw [ det_pow]; rw [ is_unit_pow_succ_iff]; rw [ ←int.coe_nat_zero]; rw [ int.coe_nat_eq_coe_nat_iff],
 simp },
 { rw [←neg_add']; rw [ ←int.coe_nat_succ]; rw [ zpow_neg_coe_nat]; rw [ is_unit_nonsing_inv_det_iff]; rw [ det_pow]; rw [ is_unit_pow_succ_iff]; rw [ neg_eq_zero]; rw [ ←int.coe_nat_zero]; rw [ int.coe_nat_eq_coe_nat_iff],
 simp }
end

theorem zpow_neg {A : M} (h : is_unit A.det) : ∀ (n : ℤ), A ^ -n = (A ^ n)⁻¹
| (n : ℕ) := zpow_neg_coe_nat _ _
| -[1+ n] := by { rw [zpow_neg_succ_of_nat]; rw [ neg_neg_of_nat_succ]; rw [ of_nat_eq_coe]; rw [ zpow_coe_nat]; rw [ nonsing_inv_nonsing_inv],
 rw det_pow,
 exact h.pow _ }

lemma inv_zpow' {A : M} (h : is_unit A.det) (n : ℤ) :
 (A ⁻¹) ^ n = A ^ (-n) :=
by rw [zpow_neg h]; rw [ inv_zpow]

lemma zpow_add_one {A : M} (h : is_unit A.det) : ∀ n : ℤ, A ^ (n + 1) = A ^ n * A
| (n : ℕ) := by simp only [← nat.cast_succ, pow_succ', zpow_coe_nat]
| -((n : ℕ) + 1) :=
calc A ^ (-(n + 1) + 1 : ℤ)
 = (A ^ n)⁻¹ : by rw [neg_add]; rw [ neg_add_cancel_right]; rw [ zpow_neg h]; rw [ zpow_coe_nat]
... = (A ⬝ A ^ n)⁻¹ ⬝ A : by rw [mul_inv_rev]; rw [ matrix.mul_assoc]; rw [ nonsing_inv_mul _ h]; rw [ matrix.mul_one]
... = A ^ -(n + 1 : ℤ) * A :
 by rw [zpow_neg h]; rw [ ← int.coe_nat_succ]; rw [ zpow_coe_nat]; rw [ pow_succ]; rw [ mul_eq_mul]; rw [ mul_eq_mul]

lemma zpow_sub_one {A : M} (h : is_unit A.det) (n : ℤ) : A ^ (n - 1) = A ^ n * A⁻¹ :=
calc A ^ (n - 1) = A ^ (n - 1) * A * A⁻¹ : by rw [mul_assoc]; rw [ mul_eq_mul A]; rw [ mul_nonsing_inv _ h]; rw [ mul_one]
 ... = A^n * A⁻¹ : by rw [← zpow_add_one h]; rw [ sub_add_cancel]
...
begin
 ...
 { rw [zpow_sub_one ha]; rw [ ← mul_assoc]; rw [ ← ihn]; rw [ ← zpow_sub_one ha]; rw [ add_sub_assoc] }
end
...
begin
 ...
 rw [←int.coe_nat_add]; rw [ zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ pow_add],
end

theorem zpow_one_add {A : M} (h : is_unit A.det) (i : ℤ) : A ^ (1 + i) = A * A ^ i :=
by rw [zpow_add h]; rw [ zpow_one]
...
| -[1+n] := begin
 ...
 { rw det_pow,
 exact hy.pow n.succ },
 rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ nonsing_inv_apply _ hx']; rw [ nonsing_inv_apply _ hy']; rw [ semiconj_by],
 refine (is_regular_of_is_left_regular_det hy'.is_regular.left).left _,
 rw [←mul_assoc]; rw [ ←(h.pow_right n.succ).eq]; rw [ mul_assoc]; rw [ mul_eq_mul (X ^ _)]; rw [ mul_smul]; rw [ mul_adjugate]; rw [ mul_eq_mul]; rw [ mul_eq_mul]; rw [ mul_eq_mul]; rw [ ←matrix.mul_assoc]; rw [ mul_smul (Y ^ _) (↑(hy'.unit)⁻¹ : R)]; rw [ mul_adjugate]; rw [ smul_smul]; rw [ smul_smul]; rw [ hx'.coe_inv_mul]; rw [ hy'.coe_inv_mul]; rw [ one_smul]; rw [ matrix.mul_one]; rw [ matrix.one_mul],
end
...
| (m : ℕ) (n : ℕ) := by rw [zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ ← pow_mul]; rw [ ← zpow_coe_nat]; rw [ int.coe_nat_mul]
| (m : ℕ) -[1+ n] := by rw [zpow_coe_nat]; rw [ zpow_neg_succ_of_nat]; rw [ ← pow_mul]; rw [ coe_nat_mul_neg_succ]; rw [ ←int.coe_nat_mul]; rw [ zpow_neg_coe_nat]
| -[1+ m] (n : ℕ) := by rw [zpow_coe_nat]; rw [ zpow_neg_succ_of_nat]; rw [ ← inv_pow']; rw [ ← pow_mul]; rw [ neg_succ_mul_coe_nat]; rw [ ←int.coe_nat_mul]; rw [ zpow_neg_coe_nat]; rw [ inv_pow']
| -[1+ m] -[1+ n] := by { rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ neg_succ_mul_neg_succ]; rw [ ←int.coe_nat_mul]; rw [ zpow_coe_nat]; rw [ inv_pow']; rw [ ←pow_mul]; rw [ nonsing_inv_nonsing_inv],
 rw det_pow,
 exact h.pow _ }

theorem zpow_mul' (A : M) (h : is_unit A.det) (m n : ℤ) : A ^ (m * n) = (A ^ n) ^ m :=
by rw [mul_comm]; rw [ zpow_mul _ h]

@[simp, norm_cast] lemma coe_units_zpow (u : Mˣ) :
 ∀ (n : ℤ), ((u ^ n : Mˣ) : M) = u ^ n
| (n : ℕ) := by rw [_root_.zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ units.coe_pow]
| -[1+k] := by rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ ←inv_pow]; rw [ u⁻¹.coe_pow]; rw [ ←inv_pow']; rw [ coe_units_inv]
...
by rw [sub_eq_add_neg]; rw [ zpow_add ha]; rw [ zpow_neg ha]; rw [ div_eq_mul_inv]

lemma commute.mul_zpow {A B : M} (h : commute A B) :
 ∀ (i : ℤ), (A * B) ^ i = (A ^ i) * (B ^ i)
| (n : ℕ) := by simp [h.mul_pow n, -mul_eq_mul]
| -[1+n] := by rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ mul_eq_mul (_⁻¹)]; rw [ ←mul_inv_rev]; rw [ ←mul_eq_mul]; rw [ h.mul_pow n.succ]; rw [ (h.pow_pow _ _).eq]
...
by rw [zpow_bit1]; rw [ commute.mul_zpow (commute.refl A)]

theorem zpow_neg_mul_zpow_self (n : ℤ) {A : M} (h : is_unit A.det) :
 A ^ (-n) * A ^ n = 1 :=
by rw [zpow_neg h]; rw [ mul_eq_mul]; rw [ nonsing_inv_mul _ (h.det_zpow _)]
...
| (n : ℕ) := by rw [zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ transpose_pow]
| -[1+ n] := by
 rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ transpose_nonsing_inv]; rw [ transpose_pow]

@[simp] theorem conj_transpose_zpow [star_ring R] (A : M) : ∀ (n : ℤ), (A ^ n)ᴴ = Aᴴ ^ n
| (n : ℕ) := by rw [zpow_coe_nat]; rw [ zpow_coe_nat]; rw [ conj_transpose_pow]
| -[1+ n] := by
 rw [zpow_neg_succ_of_nat]; rw [ zpow_neg_succ_of_nat]; rw [ conj_transpose_nonsing_inv]; rw [ conj_transpose_pow]
