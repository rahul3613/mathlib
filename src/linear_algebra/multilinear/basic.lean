...
begin
 ...
 rw [← update_eq_self i m]; rw [ h]; rw [ ← this]; rw [ f.map_smul]; rw [ zero_smul]
end
...
{ to_fun := λ v, f (λ j, if h : j ∈ s then v ((s.order_iso_of_fin hk).symm ⟨j, h⟩) else z),
 map_add' := λ _ v i x y,
 by { erw [dite_comp_equiv_update]; erw [ dite_comp_equiv_update]; erw [ dite_comp_equiv_update], simp },
 map_smul' := λ _ v i c x, by { erw [dite_comp_equiv_update]; erw [ dite_comp_equiv_update], simp } }
variable {R}
...
by rw [← update_cons_zero x m (x+y)]; rw [ f.map_add]; rw [ update_cons_zero]; rw [ update_cons_zero]
...
by rw [← update_cons_zero x m (c • x)]; rw [ f.map_smul]; rw [ update_cons_zero]
...
by rw [← update_snoc_last x m (x+y)]; rw [ f.map_add]; rw [ update_snoc_last]; rw [ update_snoc_last]
...
by rw [← update_snoc_last x m (c • x)]; rw [ f.map_smul]; rw [ update_snoc_last]
...
begin
 ...
 rw [←zero_comp_linear_map f']; rw [ comp_linear_map_inj f' (λ i, (f i).surjective)],
end
...
begin
 ...
 rw [A]; rw [ f.map_add]; rw [ B]; rw [ C]; rw [ finset.sum_powerset_insert hit]; rw [ Hrec]; rw [ Hrec]; rw [ add_comm],
 congr' 1,
...
begin
 ...
 have : ∑ j in A i, g i j = 0, by rw [hi]; rw [ finset.sum_empty],
...
begin
 ...
 conv_rhs { rw [this]; rw [ f.map_smul_univ] },
 refl
end
...
by ext; rw [mk_pi_ring_apply]; rw [ smul_zero]; rw [ multilinear_map.zero_apply]

lemma mk_pi_ring_eq_zero_iff [fintype ι] (z : M₂) :
 multilinear_map.mk_pi_ring R ι z = 0 ↔ z = 0 :=
by rw [← mk_pi_ring_zero]; rw [ mk_pi_ring_eq_iff]
...
eq_neg_of_add_eq_zero_left $ by rw [←multilinear_map.map_add]; rw [ add_left_neg]; rw [ f.map_coord_zero i (update_same i 0 m)]

@[simp] lemma map_sub [decidable_eq ι] (m : Πi, M₁ i) (i : ι) (x y : M₁ i) :
 f (update m i (x - y)) = f (update m i x) - f (update m i y) :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ multilinear_map.map_add]; rw [ map_neg]
...
{ to_fun := λm, f (m 0) (tail m),
 ...
 by_cases h : i = 0,
 { subst i,
 rw [update_same]; rw [ update_same]; rw [ update_same]; rw [ f.map_add]; rw [ add_apply]; rw [ tail_update_zero]; rw [ tail_update_zero]; rw [ tail_update_zero] },
 { rw [update_noteq (ne.symm h)]; rw [ update_noteq (ne.symm h)]; rw [ update_noteq (ne.symm h)],
 revert x y,
 rw ← succ_pred i h,
 assume x y,
 rw [tail_update_succ]; rw [ multilinear_map.map_add]; rw [ tail_update_succ]; rw [ tail_update_succ] }
 end,
 map_smul' := λ dec m i c x, begin
 rw subsingleton.elim dec (by apply_instance),
 by_cases h : i = 0,
 { subst i,
 rw [update_same]; rw [ update_same]; rw [ tail_update_zero]; rw [ tail_update_zero]; rw [ ← smul_apply]; rw [ f.map_smul] },
 { rw [update_noteq (ne.symm h)]; rw [ update_noteq (ne.symm h)],
 revert x,
 rw ← succ_pred i h,
 assume x,
 rw [tail_update_succ]; rw [ tail_update_succ]; rw [ multilinear_map.map_smul] }
 end }
...
{ to_fun := λm, f (init m) (m (last n)),
 ...
 rw [update_noteq this]; rw [ update_noteq this]; rw [ update_noteq this],
 revert x y,
 rw [(cast_succ_cast_lt i h).symm],
 assume x y,
 rw [init_update_cast_succ]; rw [ multilinear_map.map_add]; rw [ init_update_cast_succ]; rw [ init_update_cast_succ]; rw [ linear_map.add_apply] },
 { revert x y,
 rw eq_last_of_not_lt h,
 assume x y,
 rw [init_update_last]; rw [ init_update_last]; rw [ init_update_last]; rw [ update_same]; rw [ update_same]; rw [ update_same]; rw [ linear_map.map_add] }
 end,
 map_smul' := λ dec m i c x, begin
 rw subsingleton.elim dec (by apply_instance),
 by_cases h : i.val < n,
 { have : last n ≠ i := ne.symm (ne_of_lt h),
 rw [update_noteq this]; rw [ update_noteq this],
 revert x,
 rw [(cast_succ_cast_lt i h).symm],
 assume x,
 rw [init_update_cast_succ]; rw [ init_update_cast_succ]; rw [ multilinear_map.map_smul]; rw [ linear_map.smul_apply] },
 { revert x,
 rw eq_last_of_not_lt h,
 assume x,
 rw [update_same]; rw [ update_same]; rw [ init_update_last]; rw [ init_update_last]; rw [ map_smul] }
 end }
...
begin
 ...
 { ext i, rw [fin_sum_equiv_of_finset_inl]; rw [ finset.piecewise_eq_of_mem],
 apply finset.order_emb_of_fin_mem },
 { ext i, rw [fin_sum_equiv_of_finset_inr]; rw [ finset.piecewise_eq_of_not_mem],
...
{ carrier := f '' { v | ∀ i, v i ∈ p i},
 ...
 { rw [hij]; rw [ update_same], exact (p i).smul_mem _ (hx i) },
 { rw [update_noteq hij], exact hx j },
 { rw [f.map_smul]; rw [ update_eq_self] } } }
