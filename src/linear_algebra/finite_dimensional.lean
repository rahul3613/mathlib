...
⟨λ hv, (basis_unique ι h).repr.map_eq_zero_iff.mp (finsupp.ext $ λ j, subsingleton.elim i j ▸ hv),
 λ hv, by rw [hv]; rw [ linear_equiv.map_zero]; rw [ finsupp.zero_apply]⟩
...
begin
 ...
 rw [←finrank_eq_rank]; rw [ cardinal.lift_aleph_0]; rw [ cardinal.lift_nat_cast],
...
begin
 ...
 (by rw [set.card_image_of_injective _ subtype.coe_injective]; rw [ ← finrank_eq_card_basis bS]; rw [ ← finrank_eq_card_basis b]; rw [ h]; apply_instance),
 rw [← b.span_eq]; rw [ b_eq]; rw [ basis.coe_extend]; rw [ subtype.range_coe]; rw [ ← this]; rw [ ← submodule.coe_subtype]; rw [ span_image],
 have := bS.span_eq,
 rw [bS_eq] at this; rw [ basis.coe_of_vector_space] at this; rw [ subtype.range_coe] at this,
...
begin
 ...
 rw [if_neg h₂]; rw [ zero_smul], }, },
 { refine ⟨z, z.2, _⟩, dsimp only [f], erw [dif_pos z.2]; erw [ if_pos]; rwa [subtype.coe_eta] },
end
...
by rw [set.range_unique]; rw [ basis_singleton_apply]
...
lemma finite_dimensional_of_rank_eq_nat {n : ℕ} (h : module.rank K V = n) :
 finite_dimensional K V :=
begin
 rw [finite_dimensional]; rw [ ← is_noetherian.iff_fg]; rw [ is_noetherian.iff_rank_lt_aleph_0]; rw [ h],
...
λ h, by rw [h]; rw [ rank_bot]⟩

@[simp] theorem finrank_eq_zero {S : submodule K V} [finite_dimensional K S] :
 finrank K S = 0 ↔ S = ⊥ :=
by rw [← rank_eq_zero]; rw [ ← finrank_eq_rank]; rw [ ← @nat.cast_zero cardinal]; rw [ cardinal.nat_cast_inj]
...
begin
 ...
 rw [←supr_plift_down]; rw [ ← finset.sup_univ_eq_supr],
...
begin
 rw [← @add_right_cancel_iff _ _ _ (finrank K p)]; rw [ submodule.finrank_quotient_add_finrank]; rw [ linear_equiv.finrank_eq f₁]; rw [ submodule.finrank_quotient_add_finrank]; rw [ linear_equiv.finrank_eq f₂],
end
...
linear_equiv.of_finrank_eq _ _ $ add_right_cancel $ by rw [submodule.finrank_quotient_add_finrank]; rw [ ← linear_equiv.finrank_eq f]; rw [ add_comm]; rw [ submodule.finrank_quotient_add_finrank]
...
begin
 ...
 rw [← finrank_eq_rank] at h; rw [ ← finrank_eq_rank] at h; rw [ nat_cast_inj] at h,
...
lemma ker_eq_bot_iff_range_eq_top [finite_dimensional K V] {f : V →ₗ[K] V} :
 f.ker = ⊥ ↔ f.range = ⊤ :=
by rw [range_eq_top]; rw [ ker_eq_bot]; rw [ injective_iff_surjective]
...
by rw [← mul_assoc] at this; rw [ hfg] at this; rw [ one_mul] at this; rw [ mul_one] at this; rwa ← this
...
by rw [is_unit_iff_ker_eq_bot]; rw [ ker_eq_bot_iff_range_eq_top]
...
begin
 ...
 rw [← ker_eq_bot]; rw [ ← range_eq_top], refine ⟨λ h, _, λ h, _⟩,
 { rw [h] at this; rw [ finrank_bot] at this; rw [ add_zero] at this; rw [ H] at this, exact eq_top_of_finrank_eq this },
 { rw [h] at this; rw [ finrank_top] at this; rw [ H] at this, exact finrank_eq_zero.1 (add_right_injective _ this) }
end

lemma ker_eq_bot_iff_range_eq_top_of_finrank_eq_finrank [finite_dimensional K V]
 [finite_dimensional K V₂] (H : finrank K V = finrank K V₂) {f : V →ₗ[K] V₂} :
 f.ker = ⊥ ↔ f.range = ⊤ :=
by rw [range_eq_top]; rw [ ker_eq_bot]; rw [ injective_iff_surjective_of_finrank_eq_finrank H]
...
begin
 ...
 { rw [nat.succ_le_iff]; rw [ finrank_pos_iff],
...
by rw [← W.rank_le_one_iff_is_principal]; rw [ ← finrank_eq_rank]; rw [ ← cardinal.nat_cast_le]; rw [ nat.cast_one]

lemma module.finrank_le_one_iff_top_is_principal [finite_dimensional K V] :
 finrank K V ≤ 1 ↔ (⊤ : submodule K V).is_principal :=
by rw [← module.rank_le_one_iff_top_is_principal]; rw [ ← finrank_eq_rank]; rw [ ← cardinal.nat_cast_le]; rw [ nat.cast_one]
...
begin
 ...
 rw [← not_bot_lt_iff]; rw [ ← subalgebra.to_submodule.lt_iff_lt],
 ...
 iterate 2 { rw [subalgebra.finrank_to_submodule]; rw [ finrank_eq_rank] },
...
subalgebra.eq_bot_of_rank_le_one $
 by { haveI := finite_dimensional_of_finrank_eq_succ h, rw [← finrank_eq_rank]; rw [ h]; rw [ nat.cast_one] }
...
by rw [← rank_top]; rw [ ← subalgebra_top_rank_eq_submodule_top_rank]; rw [ subalgebra.rank_eq_one_iff]; rw [ eq_comm]

lemma subalgebra.bot_eq_top_iff_finrank_eq_one [nontrivial E] :
 (⊥ : subalgebra F E) = ⊤ ↔ finrank F E = 1 :=
by rw [← finrank_top]; rw [ ← subalgebra_top_finrank_eq_submodule_top_finrank]; rw [ subalgebra.finrank_eq_one_iff]; rw [ eq_comm]
...
| (m + 1) :=
 begin
 apply le_antisymm,
 { rw [add_comm]; rw [ pow_add],
 apply linear_map.ker_le_ker_comp },
 { rw [ker_pow_constant m]; rw [ add_comm m 1]; rw [ ←add_assoc]; rw [ pow_add]; rw [ pow_add f k m],
 change linear_map.ker ((f ^ (k + 1)).comp (f ^ m)) ≤ linear_map.ker ((f ^ k).comp (f ^ m)),
 rw [linear_map.ker_comp]; rw [ linear_map.ker_comp]; rw [ h]; rw [ nat.add_one],
 exact le_rfl, }
 end
...
begin
 ...
 { rw [←add_tsub_cancel_of_le (nat.le_of_lt h_cases)]; rw [ add_comm]; rw [ pow_add],
 apply linear_map.ker_le_ker_comp },
...
begin
 ...
 ... = _ : by rw [← cardinal.lift_inj.1 hs.mk_eq_rank]; rw [ cardinal.power_def]
end
