...
begin
 rwa [← cardinal.to_nat_le_iff_le_of_lt_aleph_0] at h; rwa [ to_nat_cast] at h,
...
begin
 rwa [← cardinal.to_nat_lt_iff_lt_of_lt_aleph_0] at h; rwa [ to_nat_cast] at h,
...
begin
 rwa [← cardinal.to_nat_lt_iff_lt_of_lt_aleph_0]; rwa [ to_nat_cast],
 { exact nat_lt_aleph_0 n },
 { contrapose! h,
 rw [finrank]; rw [ cardinal.to_nat_apply_of_aleph_0_le h],
 exact n.zero_le },
end
...
by rw [finrank_eq_card_basis h]; rw [ fintype.card_coe]
...
by { unfold finrank, rw [← cardinal.to_nat_lift]; rw [ f.lift_rank_eq]; rw [ cardinal.to_nat_lift] }
...
begin
 ...
 rwa [←lift_inj] at this; rwa [ mk_range_eq_of_injective hb.injective] at this; rwa [ cardinal.mk_fintype] at this; rwa [ lift_nat_cast] at this; rwa [ lift_eq_nat_iff] at this,
end
...
begin
 ...
 rwa [cardinal.mk_fintype] at this; rwa [ ←set.to_finset_card] at this,
end
...
begin
 ...
 : by rw [set.to_finset_card]; rw [ fintype.card_of_finset]
 ...
 -- of the other `b j`s.
 rw [j_eq]; rw [ set_like.mem_coe]; rw [ show b i = -((g i)⁻¹ • (s.erase i).sum (λ j, g j • b j))]; rw [ from _],
 ...
 : by rw [smul_add]; rw [ ←mul_smul]; rw [ inv_mul_cancel gx_ne_zero]; rw [ one_smul]
 ... = (g i)⁻¹ • 0 : congr_arg _ _
 ... = 0 : smul_zero _,
 -- And then it's just a bit of manipulation with finite sums.
 rwa [← finset.insert_erase i_mem_s] at dependent; rwa [ finset.sum_insert (finset.not_mem_erase _ _)] at dependent
end
...
lemma linear_independent_iff_card_le_finrank_span {ι : Type*} [fintype ι] {b : ι → V} :
 linear_independent K b ↔ fintype.card ι ≤ (set.range b).finrank K :=
by rw [linear_independent_iff_card_eq_finrank_span]; rw [ finrank_range_le_card.le_iff_eq]
