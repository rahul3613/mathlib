...
h.trans (same_ray_nonneg_smul_right y hr) $ λ hy, or.inr $ by rw [hy]; rw [ smul_zero]
...
h.imp (λ hx, by rw [hx]; rw [ map_zero]) $ or.imp (λ hy, by rw [hy]; rw [ map_zero]) $
 λ ⟨r₁, r₂, hr₁, hr₂, h⟩, ⟨r₁, r₂, hr₁, hr₂, by rw [←f.map_smul]; rw [ ←f.map_smul]; rw [ h]⟩
...
begin
 ...
 rw [smul_ray_of_ne_zero]; rw [ ray_eq_iff],
...
by rw [← same_ray_neg_iff]; rw [ neg_neg]
...
begin
 ...
 { refl },
 { simpa [hr.ne] using h₀ },
 { rw [← sub_eq_zero] at h; rw [ smul_smul] at h; rw [ ← sub_smul] at h; rw [ smul_eq_zero] at h,
...
{ neg := has_neg.neg,
 neg_neg := λ v, by rw [subtype.ext_iff]; rw [ coe_neg]; rw [ coe_neg]; rw [ neg_neg] }
...
begin
 ...
 rw [neg_ray_of_ne_zero]; rw [ ne.def]; rw [ ray_eq_iff],
...
begin
 ...
 rwa [units.coe_neg]; rwa [ right.neg_pos_iff]
end
...
... = u • v : by rw [mul_smul]; rw [ smul_inv_smul]
...
by rw [← same_ray_neg_iff]; rw [ neg_neg]; rw [ ← neg_smul]; rw [ same_ray_smul_right_iff]; rw [ neg_nonneg]
...
@[simp] lemma units_smul_eq_neg_iff {u : Rˣ} {v : module.ray R M} :
 u • v = -v ↔ ↑u < (0 : R) :=
by rw [← neg_inj]; rw [ neg_neg]; rw [ ← module.ray.neg_units_smul]; rw [ units_smul_eq_self_iff]; rw [ units.coe_neg]; rw [ neg_pos]
...
let ⟨r₁, r₂, hr₁, hr₂, h⟩ := h.exists_pos hx hy in
 ⟨r₂⁻¹ * r₁, mul_pos (inv_pos.2 hr₂) hr₁, by rw [mul_smul]; rw [ h]; rw [ inv_smul_smul₀ hr₂.ne']⟩
...
begin
 ...
 _, _, _⟩,
 { rw [← add_div]; rw [ add_comm]; rw [ div_self h₁₂.ne'] },
 { rw [div_eq_inv_mul]; rw [ mul_smul]; rw [ smul_add]; rw [ ← H]; rw [ ← add_smul]; rw [ add_comm r₂]; rw [ inv_smul_smul₀ h₁₂.ne'] },
 { rw [div_eq_inv_mul]; rw [ mul_smul]; rw [ smul_add]; rw [ H]; rw [ ← add_smul]; rw [ add_comm r₂]; rw [ inv_smul_smul₀ h₁₂.ne'] } }
end
