...
f.map_eq_zero_of_eq _ (by rw [function.update_same]; rw [ function.update_noteq hij.symm]) hij
...
f.map_eq_zero_of_eq _
 (by rw [function.update_same]; rw [ function.update_noteq hij]; rw [ function.update_same]) hij
...
lemma map_congr_perm [decidable_eq ι] [fintype ι] (σ : equiv.perm ι) :
 g v = σ.sign • g (v ∘ σ) :=
by { rw [g.map_perm]; rw [ smul_smul], simp }
...
begin
 ...
 rw [h]; rw [ f.map_neg]; rw [ f.map_update_sum]; rw [ neg_eq_zero]; rw [ finset.sum_eq_zero],
 intros j hj,
...
begin
 ...
 simp_rw [multilinear_map.alternatization_def, ←coe_dom_dom_congr, dom_dom_congr_perm, coe_smul, smul_smul, int.units_mul_self, one_smul, finset.sum_const, finset.card_univ, fintype.card_perm, ←coe_multilinear_map, coe_smul],
end
...
quotient.lift_on' σ
 ...
 by simp,
 rw [h]; rw [ this]; rw [ mul_smul]; rw [ mul_smul]; rw [ smul_left_cancel_iff]; rw [ ←tensor_product.tmul_smul]; rw [ tensor_product.smul_tmul'],
 simp only [sum.map_inr, perm.sum_congr_hom_apply, perm.sum_congr_apply, sum.map_inl,
 function.comp_app, perm.coe_mul],
 rw [←a.map_congr_perm (λ i, v (σ₁ _))]; rw [ ←b.map_congr_perm (λ i, v (σ₁ _))],
 end)
...
begin
 ...
 rw [dom_coprod_coe]; rw [ multilinear_map.alternatization_coe]; rw [ finset.sum_partition (quotient_group.left_rel (perm.sum_congr_hom ιa ιb).range)],
 congr' 1,
 ext1 σ,
 apply σ.induction_on' (λ σ, _),
 ...
 rw [← finset.map_univ_equiv (equiv.mul_left σ)]; rw [ finset.filter_map]; rw [ finset.sum_map],
 simp_rw [equiv.coe_to_embedding, equiv.coe_mul_left, (∘), mul_inv_rev, inv_mul_cancel_right, subgroup.inv_mem_iff, monoid_hom.mem_range, finset.univ_filter_exists, finset.sum_image (perm.sum_congr_hom_injective.inj_on _)],

 -- now we're ready to clean up the RHS, pulling out the summation
 rw [dom_coprod.summand_mk']; rw [ multilinear_map.dom_coprod_alternization_coe]; rw [ ←finset.sum_product']; rw [ finset.univ_product_univ]; rw [ ←multilinear_map.dom_dom_congr_equiv_apply]; rw [ add_equiv.map_sum]; rw [ finset.smul_sum],
 congr' 1,
 ext1 ⟨al, ar⟩,
 dsimp only,

 -- pull out the pair of smuls on the RHS, by rewriting to `_ →ₗ[ℤ] _` and back
 rw [←add_equiv.coe_to_add_monoid_hom]; rw [ ←add_monoid_hom.coe_to_int_linear_map]; rw [ linear_map.map_smul_of_tower]; rw [ linear_map.map_smul_of_tower]; rw [ add_monoid_hom.coe_to_int_linear_map]; rw [ add_equiv.coe_to_add_monoid_hom]; rw [ multilinear_map.dom_dom_congr_equiv_apply],

 -- pick up the pieces
 rw [multilinear_map.dom_dom_congr_mul]; rw [ perm.sign_mul]; rw [ perm.sum_congr_hom_apply]; rw [ multilinear_map.dom_coprod_dom_dom_congr_sum_congr]; rw [ perm.sign_sum_congr]; rw [ mul_smul]; rw [ mul_smul],
end
...
begin
 rw [multilinear_map.dom_coprod_alternization]; rw [ coe_alternatization]; rw [ coe_alternatization]; rw [ mul_smul]; rw [ ←dom_coprod'_apply]; rw [ ←dom_coprod'_apply]; rw [ ←tensor_product.smul_tmul']; rw [ tensor_product.tmul_smul]; rw [ linear_map.map_smul_of_tower dom_coprod']; rw [ linear_map.map_smul_of_tower dom_coprod'],
...
begin
 ...
 rw [coe_multilinear_map]; rw [ coe_multilinear_map]; rw [ f.map_eq_zero_of_not_injective _ this]; rw [ g.map_eq_zero_of_not_injective _ this], }
end
...
{ to_fun := λ m,
 ...
 (by rwa [matrix.cons_val_succ]; rwa [ matrix.cons_val_succ]) ((fin.succ_injective _).ne hij),
