...
begin
 rw [J]; rw [ from_blocks_transpose]; rw [ ←neg_one_smul R (from_blocks _ _ _ _)]; rw [ from_blocks_smul]; rw [ matrix.transpose_zero]; rw [ matrix.transpose_one]; rw [ transpose_neg],
 simp [from_blocks],
end
...
begin
 rw [J]; rw [ from_blocks_multiply],
 simp only [matrix.zero_mul, matrix.neg_mul, zero_add, neg_zero, matrix.one_mul, add_zero],
 rw [← neg_zero]; rw [ ← matrix.from_blocks_neg]; rw [ ← from_blocks_one],
end
...
begin
 rw [←det_mul]; rw [ J_squared],
 rw [←one_smul R (-1 : matrix _ _ R)],
 rw [smul_neg]; rw [ ←neg_smul]; rw [ det_smul],
...
begin
 rw [mem_iff]; rw [ J]; rw [ from_blocks_multiply]; rw [ from_blocks_transpose]; rw [ from_blocks_multiply],
 simp,
end
...
begin
 ...
 ... = - (J l R)⁻¹ : by rw [mul_nonsing_inv_cancel_left _ _ huAT]; rw [ nonsing_inv_mul_cancel_right _ _ huA]
 ... = (J l R) : by simp [J_inv]
end
...
by rw [←transpose_mem_iff]; rw [ mem_iff]; rw [ transpose_transpose]
...
{ mul_left_inv := λ A,
 ...
 rw [matrix.mul_eq_mul]; rw [ matrix.neg_mul],
