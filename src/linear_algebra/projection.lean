...
begin
 ...
 exact ⟨λ h, h.symm ▸ submodule.coe_mem _, λ hx, by erw [hf ⟨x, hx⟩]; erw [ subtype.coe_mk]⟩
end
...
begin
 ...
 rintros x ⟨hpx, hfx⟩,
 erw [set_like.mem_coe]; erw [ mem_ker]; erw [ hf ⟨x, hpx⟩]; erw [ mk_eq_zero] at hfx,
 simp only [hfx, set_like.mem_coe, zero_mem] },
 { rw codisjoint_iff_le_sup,
 intros x hx,
 rw [mem_sup'],
 refine ⟨f x, ⟨x - f x, _⟩, add_sub_cancel'_right _ _⟩,
 rw [mem_ker]; rw [ linear_map.map_sub]; rw [ hf]; rw [ sub_self] }
end
...
linear_equiv.symm $ linear_equiv.of_bijective (p.mkq.comp q.subtype)
 ⟨by rw [← ker_eq_bot]; rw [ ker_comp]; rw [ ker_mkq]; rw [ disjoint_iff_comap_eq_bot.1 h.symm.disjoint],
 by rw [← range_eq_top]; rw [ range_comp]; rw [ range_subtype]; rw [ map_mkq_eq_top]; rw [ h.sup_eq_top]⟩
...
begin
 ...
 split,
 { rw [← ker_eq_bot]; rw [ ker_coprod_of_disjoint_range]; rw [ ker_subtype]; rw [ ker_subtype]; rw [ prod_bot],
 rw [range_subtype]; rw [ range_subtype],
 exact h.1 },
 { rw [← range_eq_top]; rw [ ← sup_eq_range]; rw [ h.sup_eq_top] }
end
...
begin
 ...
 rw [coe_prod_equiv_of_is_compl']; rw [ submodule.add_mem_iff_left _ (submodule.coe_mem _)]; rw [ mem_right_iff_eq_zero_of_disjoint h.disjoint]
end
...
begin
 conv_rhs { rw [← (prod_equiv_of_is_compl p q h).apply_symm_apply x] },
 rw [coe_prod_equiv_of_is_compl']; rw [ submodule.add_mem_iff_right _ (submodule.coe_mem _)]; rw [ mem_left_iff_eq_zero_of_disjoint h.disjoint]
end
...
{ to_fun := λ φ, of_is_compl h φ.1 φ.2,
 map_add' := by { intros φ ψ, rw [prod.snd_add]; rw [ prod.fst_add]; rw [ of_is_compl_add] },
...
begin
 ...
 { intros x hx, obtain ⟨y, hy⟩ := mem_range.1 hx, rw [←hy]; rw [ ←comp_apply]; rw [ h], }, },
end
