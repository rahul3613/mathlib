...
calc matrix.trace (linear_map.to_matrix b b f)
 ...
 by rw [linear_map.id_comp]; rw [ linear_map.comp_id]
... = matrix.trace (linear_map.to_matrix c b linear_map.id ⬝
 linear_map.to_matrix c c f ⬝
 linear_map.to_matrix b c linear_map.id) :
 by rw [linear_map.to_matrix_comp _ c]; rw [ linear_map.to_matrix_comp _ c]
... = matrix.trace (linear_map.to_matrix c c f ⬝
 linear_map.to_matrix b c linear_map.id ⬝
 linear_map.to_matrix c b linear_map.id) :
 by rw [matrix.mul_assoc]; rw [ matrix.trace_mul_comm]
... = matrix.trace (linear_map.to_matrix c c ((f.comp linear_map.id).comp linear_map.id)) :
 by rw [linear_map.to_matrix_comp _ b]; rw [ linear_map.to_matrix_comp _ c]
... = matrix.trace (linear_map.to_matrix c c f) :
 by rw [linear_map.comp_id]; rw [ linear_map.comp_id]
...
from ⟨s, ⟨b⟩⟩,
by { rw [trace]; rw [ dif_pos this]; rw [ ← trace_aux_def], congr' 1, apply trace_aux_eq }

theorem trace_eq_matrix_trace (f : M →ₗ[R] M) :
 trace R M f = matrix.trace (linear_map.to_matrix b b f) :=
by rw [trace_eq_matrix_trace_of_finset R b.reindex_finset_range]; rw [ ← trace_aux_def]; rw [ ← trace_aux_def]; rw [ trace_aux_eq R b]
...
else by rw [trace]; rw [ dif_neg H]; rw [ linear_map.zero_apply]; rw [ linear_map.zero_apply]
...
begin
 ...
 rw [trace_eq_matrix_trace R b]; rw [ to_matrix_dual_tensor_hom],
 by_cases hij : i = j,
 { rw [hij], simp },
...
by rw [←comp_apply]; rw [ trace_eq_contract]
...
begin
 ...
 rw [trace_eq_matrix_trace R b]; rw [ to_matrix_one]; rw [ finrank_eq_card_choose_basis_index],
 simp,
end

/-- The trace of the identity endomorphism is the dimension of the free module -/
@[simp] theorem trace_id : trace R M id = (finrank R M : R) :=
by rw [←one_eq_id]; rw [ trace_one]
...
by { rw [←comp_apply]; rw [ trace_transpose] }
...
by rw [e.conj_apply]; rw [ trace_comp_comm']; rw [ ←comp_assoc]; rw [ linear_equiv.comp_coe]; rw [ linear_equiv.self_trans_symm]; rw [ linear_equiv.refl_to_linear_map]; rw [ id_comp]
...
by rw [h.eq_conj_prod_map]; rw [ trace_conj']; rw [ trace_prod_map']; rw [ trace_id]; rw [ map_zero]; rw [ add_zero]
