...
begin
 rw [ghas_mul_def]; rw [ ghas_one_def],
 induction a using pi_tensor_product.induction_on with r a x y hx hy,
 { dsimp only at a,
 rw [tensor_product.tmul_smul]; rw [ linear_equiv.map_smul]; rw [ linear_equiv.map_smul]; rw [ ←ghas_mul_def]; rw [ tprod_mul_tprod]; rw [ cast_tprod],
 congr' 2 with i,
 rw fin.elim0'_append,
 refine congr_arg a (fin.ext _),
 simp },
 { rw [tensor_product.tmul_add]; rw [ map_add]; rw [ map_add]; rw [ hx]; rw [ hy], },
end
...
begin
 ...
 induction a using pi_tensor_product.induction_on with r a x y hx hy,
 { dsimp only at a,
 rw [←tensor_product.smul_tmul']; rw [ linear_equiv.map_smul]; rw [ linear_equiv.map_smul]; rw [ ←ghas_mul_def]; rw [ tprod_mul_tprod R a _]; rw [ cast_tprod],
 congr' 2 with i,
 rw fin.append_elim0',
 refine congr_arg a (fin.ext _),
 simp },
 { rw [tensor_product.add_tmul]; rw [ map_add]; rw [ map_add]; rw [ hx]; rw [ hy], },
end
...
by rw [ghas_mul_eq_coe_linear_map]; rw [ algebra_map₀_eq_smul_one]; rw [ linear_map.map_smul₂]; rw [ linear_equiv.map_smul]; rw [ ←ghas_mul_eq_coe_linear_map]; rw [ one_mul]

lemma mul_algebra_map₀ {n} (r : R) (a : ⨂[R]^n M) :
 cast R M (add_zero _) (a ₜ* algebra_map₀ r) = r • a :=
by rw [ghas_mul_eq_coe_linear_map]; rw [ algebra_map₀_eq_smul_one]; rw [ linear_map.map_smul]; rw [ linear_equiv.map_smul]; rw [ ←ghas_mul_eq_coe_linear_map]; rw [ mul_one]

lemma algebra_map₀_mul_algebra_map₀ (r s : R) :
 cast R M (add_zero _) (algebra_map₀ r ₜ* algebra_map₀ s) = algebra_map₀ (r * s) :=
begin
 rw [←smul_eq_mul]; rw [ linear_equiv.map_smul],
...
{ mul_zero := λ i j a, linear_map.map_zero _,
 ...
 nat_cast_zero := by rw [nat.cast_zero]; rw [ map_zero],
 nat_cast_succ := λ n, by rw [nat.cast_succ]; rw [ map_add]; rw [ algebra_map₀_one],
...
{ to_fun := (algebra_map₀ : R ≃ₗ[R] ⨂[R]^0 M).to_linear_map.to_add_monoid_hom,
 ...
 rw [←linear_equiv.eq_symm_apply]; rw [ cast_symm],
 rw [←linear_equiv.eq_symm_apply] at this; rw [ cast_symm] at this; rw [ cast_cast] at this,
 exact this,
 end,
