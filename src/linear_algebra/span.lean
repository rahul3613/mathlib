...
begin
 rw [f.map_span]; rw [ span_le]; rw [ set.image_subset_iff],
 exact iff.rfl
end
...
begin
 ...
 rw [span_le]; rw [ set.insert_subset],
...
by { rw [span_le]; rw [ comap_coe], exact preimage_mono (subset_span), }
...
by rw [span_nat_eq_add_submonoid_closure]; rw [ s.closure_eq]
...
by rw [span_int_eq_add_subgroup_closure]; rw [ s.closure_eq]
...
by rw [submodule.span_union]; rw [ p.span_eq]
...
by rw [span_eq_supr_of_singleton_spans]; rw [ supr_range]
...
@[simp] theorem mem_supr_of_directed {ι} [nonempty ι]
 ...
 x ∈ supr S ↔ ∃ i, x ∈ S i :=
by { rw [← set_like.mem_coe]; rw [ coe_supr_of_directed S H]; rw [ mem_Union], refl }
...
by { ext, rw [set_like.mem_coe]; rw [ mem_sup]; rw [ set.mem_add], simp, }

lemma sup_to_add_submonoid :
 (p ⊔ p').to_add_submonoid = p.to_add_submonoid ⊔ p'.to_add_submonoid :=
begin
 ext x,
 rw [mem_to_add_submonoid]; rw [ mem_sup]; rw [ add_submonoid.mem_sup],
 refl,
end
...
begin
 ext x,
 rw [mem_to_add_subgroup]; rw [ mem_sup]; rw [ add_subgroup.mem_sup],
 refl,
end
...
by { rw [eq_top_iff]; rw [ le_span_singleton_iff], tauto }
...
begin
 rw [span_le]; rw [ set.singleton_subset_iff]; rw [ set_like.mem_coe],
...
lemma mem_span_singleton_trans {x y z : M} (hxy : x ∈ R ∙ y) (hyz : y ∈ R ∙ z) :
 x ∈ R ∙ z :=
begin
 rw [← set_like.mem_coe] at *; rw [ ← singleton_subset_iff] at *,
...
@[simp] lemma span_zero : span R (0 : set M) = ⊥ := by rw [←singleton_zero]; rw [ span_singleton_eq_bot]
...
begin
 by_cases hx : x = 0,
 { rw [hx]; rw [ span_zero_singleton]; rw [ eq_comm]; rw [ span_singleton_eq_bot],
 exact ⟨λ hy, ⟨1, by rw [hy]; rw [ smul_zero]⟩, λ ⟨_, hz⟩, by rw [← hz]; rw [ smul_zero]⟩ },
 by_cases hy : y = 0,
 { rw [hy]; rw [ span_zero_singleton]; rw [ span_singleton_eq_bot],
 exact ⟨λ hx, ⟨1, by rw [hx]; rw [ smul_zero]⟩, λ ⟨z, hz⟩, (smul_eq_zero_iff_eq z).mp hz⟩ },
 split,
 { intro hxy,
 ...
 have vi : v * i = 1 :=
 by { rw [← one_smul R y] at hv; rw [ ← hi] at hv; rw [ smul_smul] at hv, exact smul_left_injective R hy hv },
 have iv : i * v = 1 :=
 by { rw [← one_smul R x] at hi; rw [ ← hv] at hi; rw [ smul_smul] at hi, exact smul_left_injective R hx hi },
 exact ⟨⟨v, i, vi, iv⟩, hv⟩ },
 { rintro ⟨v, rfl⟩,
...
begin
 rw [←mem_to_add_submonoid] at hx; rw [ supr_to_add_submonoid] at hx,
...
by rw [span_le]; rw [ singleton_subset_iff]; rw [ set_like.mem_coe]
...
end, by rw [Sup_eq_supr]; rw [ supr_image]; rw [ ←span_eq_supr_of_singleton_spans]; rw [ span_eq]⟩⟩⟩
...
begin
 rw [← span_singleton_le_iff_mem]; rw [ le_supr_iff],
...
begin
 ...
 { refine ⟨{x}, _, _⟩,
 { rwa [finset.coe_singleton]; rwa [ set.singleton_subset_iff] },
...
lemma comap_map_eq_self {f : F} {p : submodule R M} (h : linear_map.ker f ≤ p) :
 comap f (map f p) = p :=
by rw [submodule.comap_map_eq]; rw [ sup_of_le_left h]
omit sc
...
by rw [map_le_iff_le_comap]; rw [ submodule.comap_map_eq]

theorem map_le_map_iff' {f : F} (hf : ker f = ⊥) {p p'} :
 map f p ≤ map f p' ↔ p ≤ p' :=
by rw [linear_map.map_le_map_iff]; rw [ hf]; rw [ sup_bot_eq]
...
eq_top_iff.2 (λ y hy, submodule.mem_sup.2 ⟨(f y * (f x)⁻¹) • x,
 ...
 ⟨y - (f y * (f x)⁻¹) • x,
 by rw [linear_map.mem_ker]; rw [ f.map_sub]; rw [ f.map_smul]; rw [ smul_eq_mul]; rw [ mul_assoc]; rw [ inv_mul_cancel hx]; rw [ mul_one]; rw [ sub_self],
...
begin
 ...
 rw [this]; rw [ to_span_singleton_one]; rw [ submodule.coe_mk],
end
...
by rw [← to_span_nonzero_singleton_one R M x h]; rw [ linear_equiv.symm_apply_apply]
