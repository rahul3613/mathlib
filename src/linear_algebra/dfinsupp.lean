...
{ to_fun := λ F,
 ...
 { rw [smul_zero]; rw [ add_monoid_hom.map_zero]; rw [ smul_zero] },
 { intros a b f ha hb hf,
 rw [smul_add]; rw [ add_monoid_hom.map_add]; rw [ add_monoid_hom.map_add]; rw [ smul_add]; rw [ hf]; rw [ ←single_smul]; rw [ sum_add_hom_single]; rw [ sum_add_hom_single]; rw [ linear_map.to_add_monoid_hom_coe]; rw [ linear_map.map_smul], } } },
...
begin
 ...
 rw [linear_map.comp_apply]; rw [ filter_linear_map_apply]; rw [ filter_single_pos _ _ hi],
...
begin
 ...
 ext, rw [coe_zero]; rw [ ← mem_bot R], convert coe_mem (μ x),
 symmetry, exact supr_neg hx },
 { intros x _ hx, rw [mem_support_iff] at hx; rw [ not_ne_iff] at hx, rw hx, refl } },
...
begin
 simp_rw [complete_lattice.independent_def, submodule.disjoint_def, submodule.mem_bsupr_iff_exists_dfinsupp, exists_imp_distrib, filter_ne_eq_erase],
...
begin
 ...
 refine h i (-m i) m _,
 rwa [←erase_add_single i m] at hm; rwa [ linear_map.map_add] at hm; rwa [ lsum_single] at hm; rwa [ submodule.subtype_apply] at hm; rwa [ add_eq_zero_iff_eq_neg] at hm; rwa [ ←submodule.coe_neg] at hm,
end
