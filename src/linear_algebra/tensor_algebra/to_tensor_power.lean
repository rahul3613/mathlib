...
begin
 ...
 rw [tensor_power.ghas_mul_eq_coe_linear_map]; rw [ ←linear_map.compr₂_apply]; rw [ ←@linear_map.mul_apply' R]; rw [ ←linear_map.compl₂_apply]; rw [ ←linear_map.comp_apply],
 ...
 congr',
 rw [←list.map_of_fn _ (tensor_algebra.ι R)]; rw [ ←list.map_of_fn _ (tensor_algebra.ι R)]; rw [ ←list.map_of_fn _ (tensor_algebra.ι R)]; rw [ ←list.map_append]; rw [ list.of_fn_fin_append],
end
...
lemma to_tensor_algebra_galgebra_to_fun (r : R) :
 ...
 = algebra_map _ _ r :=
by rw [tensor_power.galgebra_to_fun_def]; rw [ tensor_power.algebra_map₀_eq_smul_one]; rw [ linear_map.map_smul]; rw [ tensor_power.to_tensor_algebra_ghas_one]; rw [ algebra.algebra_map_eq_smul_one]
...
@[simp] lemma mk_reindex_fin_cast {n m : ℕ} (h : n = m) (x : ⨂[R]^n M) :
 ...
 graded_monoid.mk n x :=
by rw [fin.cast_to_equiv]; rw [ mk_reindex_cast h]
...
begin
 ...
 { rw [list.fin_range_zero]; rw [ list.map_nil]; rw [ list.prod_nil],
 refl, },
 { intros n x₀ x ih,
 rw [list.fin_range_succ_eq_map]; rw [ list.map_cons]; rw [ list.prod_cons]; rw [ list.map_map]; rw [ function.comp],
 simp_rw [fin.cons_zero, fin.cons_succ],
 rw [ih]; rw [ graded_monoid.mk_mul_mk]; rw [ tensor_power.tprod_mul_tprod],
 ...
 congr' 1 with i,
 congr' 1,
 rw [fin.cast_trans]; rw [ fin.cast_refl]; rw [ order_iso.refl_apply] },
end
...
begin
 rw [tprod_apply]; rw [ alg_hom.map_list_prod]; rw [ list.map_of_fn]; rw [ function.comp],
