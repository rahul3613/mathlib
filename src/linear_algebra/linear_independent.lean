...
⟨λ hf s g hg i his, have h : _ := hf (∑ i in s, finsupp.single i (g i)) $
 ...
 by rw [finsupp.lapply_apply]; rw [ finsupp.single_eq_same]
 ... = ∑ j in s, (finsupp.lapply i : (ι →₀ R) →ₗ[R] R) (finsupp.single j (g j)) :
 eq.symm $ finset.sum_eq_single i
 (λ j hjs hji, by rw [finsupp.lapply_apply]; rw [ finsupp.single_eq_of_ne hji])
...
begin
 rw [linear_independent_iff]; rw [ finsupp.total_comp],
 intros l hl,
...
begin
 rw [disjoint_iff_inf_le] at hf_inj; rw [ ← set.image_univ] at hf_inj; rw [ finsupp.span_image_eq_map_total] at hf_inj; rw [ map_inf_eq_map_inf_comap] at hf_inj; rw [ map_le_iff_le_comap] at hf_inj; rw [ comap_bot] at hf_inj; rw [ finsupp.supported_univ] at hf_inj; rw [ top_inf_eq] at hf_inj,
 unfold linear_independent at hv ⊢,
 rw [hv] at hf_inj; rw [ le_bot_iff] at hf_inj,
 haveI : inhabited M := ⟨0⟩,
 rw [finsupp.total_comp]; rw [ @finsupp.lmap_domain_total _ _ R _ _ _ _ _ _ _ _ _ _ f]; rw [ linear_map.ker_comp]; rw [ hf_inj],
 exact λ _, rfl,
end
...
begin
 ...
 rw [this] at total_eq; rw [ zero_smul] at total_eq; rw [ zero_add] at total_eq,
...
begin
 ...
 intros, assumption },
 { rwa [finsupp.emb_domain_eq_map_domain]; rwa [ finsupp.sum_map_domain_index],
...
by rw [linear_independent_comp_subtype]; rw [ linear_map.disjoint_ker]
...
by rw [finsupp.total_on]; rw [ linear_map.ker]; rw [ linear_map.comap_cod_restrict]; rw [ submodule.map_bot]; rw [ comap_bot]; rw [ linear_map.ker_comp]; rw [ linear_independent_subtype_disjoint]; rw [ disjoint_iff_inf_le]; rw [ ← map_comap_subtype]; rw [ map_le_iff_le_comap]; rw [ comap_bot]; rw [ ker_subtype]; rw [ le_bot_iff]
...
begin
 ...
 { rw [← hsum]; rw [ finset.sum_congr rfl _],
 intros, erw [pi.smul_apply]; erw [ smul_assoc]; erw [ smul_comm] },
end
...
begin
 ...
 { rw [← hsum]; rw [ finset.sum_congr rfl _],
 intros,
 erw [pi.smul_apply]; erw [ smul_assoc],
 refl }
end
...
begin
 ...
 { rw [finset.sum_preimage']; rw [ finset.sum_preimage']; rw [ ← finset.sum_union]; rw [ ← finset.filter_or],
 ...
 { rw [this] at A; rw [ neg_eq_zero] at A,
...
begin
 ...
 split,
 { rw [← linear_map.ker_eq_bot]; rw [ linear_map.ker_cod_restrict],
 apply hv },
 { rw [← linear_map.range_eq_top]; rw [ linear_map.range_eq_map]; rw [ linear_map.map_cod_restrict]; rw [ ← linear_map.range_le_iff_comap]; rw [ range_subtype]; rw [ submodule.map_top],
...
by rw [linear_independent.repr]; rw [ linear_equiv.ker]

lemma linear_independent.repr_range : hv.repr.range = ⊤ :=
by rw [linear_independent.repr]; rw [ linear_equiv.range]
...
⟨ λ hv i a ha, begin
 rw [finsupp.span_image_eq_map_total] at ha; rw [ mem_map] at ha,
...
begin
 ...
 rw [← finset.insert_erase hfi'] at sum_f; rw [ finset.sum_insert (finset.not_mem_erase _ _)] at sum_f; rw [ add_eq_zero_iff_eq_neg] at sum_f,
 rw sum_f,
...
begin
 ...
 by rw [h_total_l]; rw [ finsupp.total_single]; rw [ one_smul],
...
have h1 : ∀ i ∈ s, (g i • i : G → L) = g i • a, from λ i his, funext $ λ x : G,
 ...
 by rw [finset.sum_insert has]; rw [ finset.sum_insert has]
 ... = ∑ i in insert a s, g i * i (x * y) - ∑ i in insert a s, a x * (g i * i y) :
 congr (congr_arg has_sub.sub (finset.sum_congr rfl $ λ i _, by rw [i.map_mul]; rw [ mul_assoc]))
 (finset.sum_congr rfl $ λ _ _, by rw [mul_assoc]; rw [ mul_left_comm])
 ... = (∑ i in insert a s, (g i • i : G → L)) (x * y)
 - a x * (∑ i in insert a s, (g i • i : G → L)) y :
 by rw [finset.sum_apply]; rw [ finset.sum_apply]; rw [ finset.mul_sum]; refl
 ... = 0 - a x * 0 : by rw hg; refl
 ... = 0 : by rw [mul_zero]; rw [ sub_zero])
 i
 his,
...
have h3 : ∀ i ∈ s, g i = 0, from λ i his, let ⟨y, hy⟩ := h2 i his in
 ...
 or.resolve_right (mul_eq_zero.1 $ by rw [mul_sub]; rw [ sub_eq_zero]; exact h) (sub_ne_zero_of_ne hy),
...
have h4 : g a = 0, from calc
 ...
 rw [h3 i (his.resolve_left hia)]; rw [ zero_smul] },
...
begin
 rw [← linear_independent_equiv (equiv.option_equiv_sum_punit ι).symm]; rw [ linear_independent_sum]; rw [ @range_unique _ punit]; rw [ @linear_independent_unique_iff punit]; rw [ disjoint_span_singleton],
 dsimp [(∘)],
...
by { rw [← linear_independent_equiv ((equiv.option_equiv_sum_punit _).trans (equiv.set.insert has).symm)]; rw [ linear_independent_option], simp [(∘), range_comp f] }
...
lemma linear_independent_fin_cons {n} {v : fin n → V} :
 ...
 linear_independent K v ∧ x ∉ submodule.span K (range v) :=
begin
 rw [← linear_independent_equiv (fin_succ_equiv n).symm]; rw [ linear_independent_option],
 convert iff.rfl,
 { ext,
 -- TODO: why doesn't simp use `fin_succ_equiv_symm_coe` here?
 rw [comp_app]; rw [ comp_app]; rw [ fin_succ_equiv_symm_coe]; rw [ fin.cons_succ] },
 { ext,
 rw [comp_app]; rw [ comp_app]; rw [ fin_succ_equiv_symm_coe]; rw [ fin.cons_succ] }
end
...
lemma linear_independent_fin_snoc {n} {v : fin n → V} :
 ...
 linear_independent K v ∧ x ∉ submodule.span K (range v) :=
by rw [fin.snoc_eq_cons_rotate]; rw [ linear_independent_equiv]; rw [ linear_independent_fin_cons]
...
by rw [← linear_independent_fin_cons]; rw [ fin.cons_self_tail]

lemma linear_independent_fin_succ' {n} {v : fin (n + 1) → V} :
 linear_independent K v ↔
 linear_independent K (fin.init v) ∧ v (fin.last _) ∉ submodule.span K (range $ fin.init v) :=
by rw [← linear_independent_fin_snoc]; rw [ fin.snoc_init_self]

lemma linear_independent_fin2 {f : fin 2 → V} :
 linear_independent K f ↔ f 1 ≠ 0 ∧ ∀ a : K, a • f 1 ≠ f 0 :=
by rw [linear_independent_fin_succ, linear_independent_unique_iff, range_unique, mem_span_singleton, not_exists, show fin.tail f default = f 1, by rw ← fin.succ_zero_eq_one; refl]
