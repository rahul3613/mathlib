...
begin
 conv_lhs { rw [← fin.cons_self_tail v]; rw [ vandermonde_cons] },
 simp only [fin.tail]
end
...
begin
 ...
 (fin.succ_above 0 i)) :
 by simp_rw [det_succ_column_zero, fin.sum_univ_succ, of_apply, matrix.cons_val_zero, submatrix, of_apply, matrix.cons_val_succ, fin.coe_zero, pow_zero, one_mul, sub_self, mul_zero, zero_mul, finset.sum_const_zero, add_zero]
 ... = det (of $ λ (i j : fin n), (v (fin.succ i) - v 0) *
 (∑ k in finset.range (j + 1 : ℕ), v i.succ ^ k * v 0 ^ (j - k : ℕ)) :
 matrix _ _ R) :
 by { congr, ext i j, rw [fin.succ_above_zero]; rw [ matrix.cons_val_succ]; rw [ fin.coe_succ]; rw [ mul_comm],
