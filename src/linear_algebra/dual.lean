...
by { erw [constr_basis b]; erw [ constr_basis b], ac_refl }

@[simp] lemma to_dual_total_left (f : ι →₀ R) (i : ι) :
 b.to_dual (finsupp.total ι M R b f) (b i) = f i :=
begin
 rw [finsupp.total_apply]; rw [ finsupp.sum]; rw [ linear_map.map_sum]; rw [ linear_map.sum_apply],
...
begin
 rw [finsupp.total_apply]; rw [ finsupp.sum]; rw [ linear_map.map_sum],
...
by rw [← b.to_dual_total_left]; rw [ b.total_repr]

lemma to_dual_apply_right (i : ι) (m : M) : b.to_dual (b i) m = b.repr m i :=
by rw [← b.to_dual_total_right]; rw [ b.total_repr]
...
by rw [b.equiv_fun_apply]; rw [ to_dual_eq_repr]

lemma to_dual_inj (m : M) (a : b.to_dual m = 0) : m = 0 :=
begin
 rw [← mem_bot R]; rw [ ← b.repr.ker]; rw [ mem_ker]; rw [ linear_equiv.coe_coe],
 apply finsupp.ext,
 intro b,
 rw [← to_dual_eq_repr]; rw [ a],
 refl
end
...
begin
 ...
 rw [b.to_dual_eq_repr _ i]; rw [ repr_total b],
 refl,
end
...
begin
 ext m,
 simp_rw [linear_map.sum_apply, linear_map.smul_apply, smul_eq_mul, mul_comm (f _), ←smul_eq_mul, ←f.map_smul, ←f.map_sum, basis.coord_apply, basis.sum_repr],
end
...
begin
 casesI nonempty_fintype ι,
 rw [finsupp.total_apply]; rw [ finsupp.sum_fintype]; rw [ linear_map.sum_apply],
...
by rw [← total_dual_basis b]; rw [ basis.total_repr b.dual_basis l]
...
begin
 ...
 rw [linear_map.comp_apply]; rw [ to_dual_apply_left]; rw [ coe_to_dual_self]; rw [ ← coe_dual_basis]; rw [ dual.eval_apply]; rw [ basis.repr_self]; rw [ finsupp.single_apply]; rw [ dual_basis_apply_self]
end
...
by rw [basis.equiv_fun_apply]; rw [ dual_basis_repr]
...
begin
 classical,
 casesI nonempty_fintype ι,
 rw [← b.to_dual_to_dual]; rw [ range_comp]; rw [ b.to_dual_range]; rw [ submodule.map_top]; rw [ to_dual_range _],
 apply_instance
end
...
by { haveI := classical.dec_eq ι, rw [← coe_dual_basis]; rw [ total_dual_basis] }
...
by { rw [← eval_apply_eq_zero_iff K v]; rw [ linear_map.ext_iff], refl }
...
{ to_fun := λ i, ε i m,
 ...
 mem_support_to_fun := by { intro i, rw [set.finite.mem_to_finset]; rw [ set.mem_set_of_eq] } }
...
by { ext i, rw [h.coeffs_apply]; rw [ h.dual_lc] }
...
begin
 ...
 rwa [← lc_def] at hi; rwa [ h.dual_lc] at hi
end

lemma coe_dual_basis [fintype ι] : ⇑h.basis.dual_basis = ε :=
funext (λ i, h.basis.ext (λ j, by rw [h.basis.dual_basis_apply_self]; rw [ h.coe_basis]; rw [ h.eval]; rw [ if_congr eq_comm rfl rfl]))
...
begin
 rw [le_dual_annihilator_iff_le_dual_coannihilator]; rw [ dual_coannihilator_sup_eq],
...
lemma supr_dual_annihilator_le_infi {ι : Type*} (U : ι → submodule R M) :
 (⨆ (i : ι), (U i).dual_annihilator) ≤ (⨅ (i : ι), U i).dual_annihilator :=
begin
 rw [le_dual_annihilator_iff_le_dual_coannihilator]; rw [ dual_coannihilator_supr_eq],
 apply' infi_mono,
...
by rw [dual_coannihilator]; rw [ dual_annihilator_top]; rw [ comap_bot]; rw [ module.eval_ker]
...
by rw [← set_like.ext_iff.mp dual_annihilator_dual_coannihilator_eq v]; rw [ mem_dual_coannihilator]
...
begin
 ...
 rw [dual_coannihilator] at this; rw [ ← module.map_eval_equiv_symm_apply] at this,
...
begin
 rw [submodule.dual_coannihilator]; rw [ ← module.eval_equiv_to_linear_map],
...
begin
 rw [finrank_dual_coannihilator_eq]; rw [ W.quot_equiv_annihilator.finrank_eq.symm]; rw [ add_comm]; rw [ submodule.finrank_quotient_add_finrank]; rw [ subspace.dual_finrank_eq],
end
...
begin
 ...
 rintro y ⟨x, rfl⟩,
 rw [← dual_map_apply]; rw [ hφ]; rw [ zero_apply] },
 { ext x,
 rw dual_map_apply,
...
begin
 ...
 rintro x hx,
 rw [dual_map_apply]; rw [ hx]; rw [ map_zero]
end
...
begin
 ...
 { rw [← linear_map.range_eq_top]; rw [ linear_map.range_range_restrict] },
 ...
 rw [← linear_map.dual_map_comp_dual_map]; rw [ linear_map.range_comp_of_range_eq_top],
...
begin
 ...
 ext x,
 rw [linear_map.dual_map_apply]; rw [ subspace.dual_lift_of_mem (mem_range_self f x)]; rw [ linear_equiv.dual_map_apply],
 congr' 1,
...
by rw [← linear_map.range_eq_top]; rw [ range_dual_map_eq_dual_annihilator_ker]; rw [ ← submodule.dual_annihilator_bot]; rw [ subspace.dual_annihilator_inj]; rw [ linear_map.ker_eq_bot]
...
begin
 split,
 { rw [linear_map.separating_left_iff_ker_eq_bot]; rw [ dual_pairing_eq],
...
begin
 split,
 { rw [linear_map.separating_left_iff_ker_eq_bot]; rw [ dual_copairing_eq],
 apply linear_equiv.ker, },
 { rintro ⟨x⟩,
 simp only [quotient.quot_mk_eq_mk, dual_copairing_apply, quotient.mk_eq_zero],
 rw [← forall_mem_dual_annihilator_apply_eq_zero_iff]; rw [ set_like.forall],
 exact id, }
end
...
begin
 ...
 rw [← this]; rw [ ← linear_map.range_dual_map_eq_dual_annihilator_ker],
 intro φ,
 rw [linear_map.mem_range],
 rintro ⟨x, rfl⟩,
...
begin
 ...
 { intros α β h hyp W,
 rw [← h.infi_comp]; rw [ hyp (W ∘ h)]; rw [ ← h.supr_comp], },
 { intro W,
 rw [supr_of_empty']; rw [ infi_of_empty']; rw [ Inf_empty]; rw [ Sup_empty]; rw [ dual_annihilator_top], },
 { introsI α _ h W,
 rw [infi_option]; rw [ supr_option]; rw [ dual_annihilator_inf_eq]; rw [ h], }
end
...
begin
 rw [is_compl_iff] at h ⊢; rw [ disjoint_iff] at h ⊢; rw [ codisjoint_iff] at h ⊢,
 rw [← dual_annihilator_inf_eq]; rw [ ← dual_annihilator_sup_eq]; rw [ h.1]; rw [ h.2]; rw [ dual_annihilator_top]; rw [ dual_annihilator_bot],
 exact ⟨rfl, rfl⟩
end
...
begin
 ...
 rw [(subspace.quot_equiv_annihilator f.range).finrank_eq] at this; rw [ ← ker_dual_map_eq_dual_annihilator_range] at this,
 conv_rhs at this { rw ← subspace.dual_finrank_eq },
 refine add_left_injective (finrank K f.dual_map.ker) _,
 change _ + _ = _ + _,
 rw [finrank_range_add_finrank_ker f.dual_map]; rw [ add_comm]; rw [ this],
end
...
begin
 ...
 rw [← add_zero (finite_dimensional.finrank K f.range)]; rw [ ← h]; rw [ ← linear_map.finrank_range_dual_map_eq_finrank_range]; rw [ linear_map.finrank_range_add_finrank_ker]; rw [ subspace.dual_finrank_eq],
end
