...
lemma smul_tprod_coeff [decidable_eq ι] (z : R) (f : Π i, s i) (i : ι) (r : R₁)
 ...
 tprod_coeff R z (update f i (r • f i)) = tprod_coeff R (r • z) f :=
begin
 have h₁ : r • z = (r • (1 : R)) * z := by rw [smul_mul_assoc]; rw [ one_mul],
...
{ smul := (•),
 ...
 (λ x y ihx ihy, by rw [pi_tensor_product.smul_add]; rw [ ihx]; rw [ ihy]; rw [ add_zero]),
...
begin
 ...
 refine λ z,
 (pi_tensor_product.induction_on' z _ (λ x y hx hy, by rw [φ₁.map_add]; rw [ φ₂.map_add]; rw [ hx]; rw [ hy])),
 { intros r f,
 rw [tprod_coeff_eq_smul_tprod]; rw [ φ₁.map_smul]; rw [ φ₂.map_smul],
...
def lift_aux (φ : multilinear_map R s E) : (⨂[R] i, s i) →+ E :=
 ...
 (λ z f i hf, by rw [map_coord_zero φ i hf]; rw [ smul_zero])
 (λ f, by rw [zero_smul])
 (λ _ z f i m₁ m₂, by { resetI, rw [←smul_add]; rw [ φ.map_add] })
...
begin
 ...
 { intros z f,
 rw [smul_tprod_coeff' r z f]; rw [ lift_aux_tprod_coeff]; rw [ lift_aux_tprod_coeff]; rw [ smul_assoc] },
 { intros z y ihz ihy,
 rw [smul_add]; rw [ (lift_aux φ).map_add]; rw [ ihz]; rw [ ihy]; rw [ (lift_aux φ).map_add]; rw [ smul_add] }
end
...
begin
 apply linear_equiv.to_linear_map_injective,
 ext1,
 rw [reindex_comp_tprod]; rw [ linear_equiv.refl_to_linear_map]; rw [ equiv.refl_symm],
 refl,
end
...
begin
 erw [tensor_product.lift.tmul]; erw [ pi_tensor_product.lift.tprod]; erw [ pi_tensor_product.lift.tprod],
 refl
end
