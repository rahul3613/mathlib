...
{ bilin := linear_map.mk₂ R (λ m₁ m₂, ⟨ι Q m₁ * ι Q m₂, ι_mul_ι_mem_even_odd_zero _ _ _⟩)
 ...
 ... = Q m₂ • (ι Q m₁ * ι Q m₃) : by rw [algebra.smul_def]; rw [ ι_sq_scalar]; rw [ algebra.left_comm] }
...
linear_map.mk₂ R (λ m acc,
 ...
 by rw [map_add]; rw [ add_mul]))
 (λ c m a, prod.ext
 (linear_map.map_smul _ c m)
 (subtype.ext $ linear_map.ext $ λ m₃,
 show f.bilin m₃ (c • m) * a.1 = c • (f.bilin m₃ m * a.1),
 by rw [linear_map.map_smul]; rw [ smul_mul_assoc]))
...
begin
 ...
 rw [linear_map.add_apply]; rw [ linear_map.add_apply]; rw [ mul_add]; rw [ smul_add]; rw [ ihx]; rw [ ihy] },
 { rintros x hx c ihx,
 rw [linear_map.smul_apply]; rw [ linear_map.smul_apply]; rw [ mul_smul_comm]; rw [ ihx]; rw [ smul_comm] } },
end
...
begin
 ...
 { intros r,
 rw [foldr_algebra_map]; rw [ aux_algebra_map],
 exact (algebra.smul_def r _), },
 { intros x y hx hy ihx ihy,
 rw [linear_map.map_add]; rw [ prod.fst_add]; rw [ ihx]; rw [ ihy]; rw [ ←add_mul]; rw [ ←linear_map.map_add],
 refl, },
 { rintros m₁ m₂ x (hx : x ∈ even Q) ih,
 rw [aux_apply]; rw [ foldr_mul]; rw [ foldr_mul]; rw [ foldr_ι]; rw [ foldr_ι]; rw [ fst_f_fold_f_fold]; rw [ ih]; rw [ ←mul_assoc]; rw [ subtype.coe_mk]; rw [ foldr_mul]; rw [ foldr_mul]; rw [ foldr_ι]; rw [ foldr_ι]; rw [ fst_f_fold_f_fold],
 refl }
end
