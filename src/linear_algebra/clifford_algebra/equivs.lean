...
{ mul_comm := λ x y, begin
 ...
 case h_add : x₁ x₂ hx₁ hx₂ { rw [mul_add]; rw [ add_mul]; rw [ hx₁]; rw [ hx₂], },
 case h_mul : x₁ x₂ hx₁ hx₂ { rw [mul_assoc]; rw [ hx₂]; rw [ ←mul_assoc]; rw [ hx₁]; rw [ ←mul_assoc], },
 end,
...
begin
 ...
 case h_grade1 : x { rw [ι_eq_zero]; rw [ linear_map.zero_apply]; rw [ reverse.map_zero] },
 case h_mul : x₁ x₂ hx₁ hx₂ { rw [reverse.map_mul]; rw [ mul_comm]; rw [ hx₁]; rw [ hx₂] },
 case h_add : x₁ x₂ hx₁ hx₂ { rw [reverse.map_add]; rw [ hx₁]; rw [ hx₂] },
end
...
alg_equiv.of_alg_hom
 ...
 (by { ext : 1, rw [ι_eq_zero]; rw [ linear_map.comp_zero]; rw [ linear_map.comp_zero], })
...
complex.lift ⟨
 ...
 by rw [clifford_algebra.ι_sq_scalar]; rw [ Q_apply]; rw [ one_mul]; rw [ ring_hom.map_neg]; rw [ ring_hom.map_one]⟩
...
begin
 ...
 rw [of_complex_I]; rw [ to_complex_ι]; rw [ one_smul],
end
...
begin
 ...
 rw [to_complex_ι]; rw [ one_smul]; rw [ of_complex_I],
end
...
{ mul_comm := λ x y, clifford_algebra_complex.equiv.injective $
 by rw [alg_equiv.map_mul]; rw [ mul_comm]; rw [ alg_equiv.map_mul],
 .. clifford_algebra.ring _ }
...
clifford_algebra_complex.equiv.injective $
 by rw [equiv_apply]; rw [ equiv_apply]; rw [ to_complex_involute]; rw [ to_complex_of_complex]; rw [ to_complex_of_complex]
...
{ i := ι (Q c₁ c₂) (1, 0),
 ...
 i_mul_i := begin
 rw [ι_sq_scalar]; rw [ Q_apply]; rw [ ←algebra.algebra_map_eq_smul_one],
 simp,
 end,
 j_mul_j := begin
 rw [ι_sq_scalar]; rw [ Q_apply]; rw [ ←algebra.algebra_map_eq_smul_one],
 ...
 i_mul_j := rfl,
 j_mul_i := begin
 rw [eq_neg_iff_add_eq_zero]; rw [ ι_mul_ι_add_swap]; rw [ quadratic_form.polar],
 simp,
 end }
...
begin
 ...
 case h_grade1 : x
 { rw [reverse_ι]; rw [ involute_ι]; rw [ to_quaternion_ι]; rw [ alg_hom.map_neg]; rw [ to_quaternion_ι]; rw [ quaternion_algebra.neg_mk]; rw [ star_mk]; rw [ neg_zero], },
...
clifford_algebra_quaternion.equiv.injective $
 by rw [equiv_apply]; rw [ equiv_apply]; rw [ to_quaternion_star]; rw [ to_quaternion_of_quaternion]; rw [ to_quaternion_of_quaternion]
...
by rw [←mul_one r₁]; rw [ ←mul_one r₂]; rw [ ←smul_eq_mul R]; rw [ ←smul_eq_mul R]; rw [ linear_map.map_smul]; rw [ linear_map.map_smul]; rw [ smul_mul_smul]; rw [ ι_sq_scalar]; rw [ quadratic_form.zero_apply]; rw [ ring_hom.map_zero]; rw [ smul_zero]
...
alg_equiv.of_alg_hom
 ...
 (by { ext x : 1, dsimp, rw [lift_apply_eps]; rw [ subtype.coe_mk]; rw [ lift_ι_apply]; rw [ inr_hom_apply]; rw [ eps] })
 (by { ext : 2, dsimp, rw [lift_ι_apply]; rw [ inr_hom_apply]; rw [ ←eps]; rw [ lift_apply_eps]; rw [ subtype.coe_mk] })
