...
begin
 rw [graded_algebra.ι_apply Q]; rw [ direct_sum.of_mul_of]; rw [ direct_sum.algebra_map_apply],
...
begin
 ...
 { rw [alg_hom.commutes]; rw [ direct_sum.algebra_map_apply], refl },
 { rw [alg_hom.map_add]; rw [ ihx]; rw [ ihy]; rw [ ←map_add], refl },
 { obtain ⟨_, rfl⟩ := hm,
 rw [alg_hom.map_mul]; rw [ ih]; rw [ lift_ι_apply]; rw [ graded_algebra.ι_apply Q]; rw [ direct_sum.of_mul_of],
 refine direct_sum.of_eq_of_graded_monoid_eq (sigma.subtype_ext _ _);
 dsimp only [graded_monoid.mk, subtype.coe_mk],
 { rw [nat.succ_eq_add_one]; rw [ add_comm]; rw [ nat.cast_add]; rw [ nat.cast_one] },
 refl } },
 { rw alg_hom.map_zero,
 apply eq.symm,
...
graded_algebra.of_alg_hom (even_odd Q)
 ...
 alg_hom.id_apply],
 rw [lift_ι_apply]; rw [ graded_algebra.ι_apply Q]; rw [ direct_sum.coe_alg_hom_of]; rw [ subtype.coe_mk],
 end)
 (by apply graded_algebra.lift_ι_eq Q)

lemma supr_ι_range_eq_top : (⨆ i : ℕ, (ι Q).range ^ i) = ⊤ :=
begin
 rw [← (direct_sum.decomposition.is_internal (even_odd Q)).submodule_supr_eq_top]; rw [ eq_comm],
