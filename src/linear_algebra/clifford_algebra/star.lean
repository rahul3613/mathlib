...
by rw [star_def]; rw [ involute_ι]; rw [ map_neg]; rw [ reverse_ι]
...
@[simp] lemma star_smul (r : R) (x : clifford_algebra Q) :
 star (r • x) = r • star x :=
by rw [star_def]; rw [ star_def]; rw [ map_smul]; rw [ map_smul]

@[simp] lemma star_algebra_map (r : R) :
 star (algebra_map R (clifford_algebra Q) r) = algebra_map R (clifford_algebra Q) r :=
by rw [star_def]; rw [ involute.commutes]; rw [ reverse.commutes]
