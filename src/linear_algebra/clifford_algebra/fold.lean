...
begin
 ...
 { rw [list.map_nil]; rw [ list.prod_nil]; rw [ list.foldr_nil]; rw [ foldr_one] },
 { rw [list.map_cons]; rw [ list.prod_cons]; rw [ list.foldr_cons]; rw [ foldr_mul]; rw [ foldr_ι]; rw [ ih] },
end
...
by rw [←foldr_reverse]; rw [ reverse_prod_map_ι]; rw [ ←list.map_reverse]; rw [ foldr_prod_map_ι]; rw [ list.foldr_reverse]
...
begin
 dsimp [foldr'],
 rw [foldr_mul]; rw [ foldr_ι]; rw [ foldr'_aux_apply_apply],
 ...
 { rw [map_add]; rw [ prod.fst_add]; rw [ hx]; rw [ hy] },
 { rw [foldr_mul]; rw [ foldr_ι]; rw [ foldr'_aux_apply_apply]; rw [ hx], },
end
