...
begin
 ...
 case h_grade0 : { rw [reverse.commutes]; rw [ reverse.commutes] },
 case h_grade1 : { rw [reverse_ι]; rw [ reverse_ι] },
 case h_mul : a b ha hb { rw [reverse.map_mul]; rw [ reverse.map_mul]; rw [ ha]; rw [ hb], },
 case h_add : a b ha hb { rw [reverse.map_add]; rw [ reverse.map_add]; rw [ ha]; rw [ hb], },
end
...
@[simp] lemma ι_range_comap_involute :
 ...
 = (ι Q).range :=
by rw [←submodule_map_involute_eq_comap]; rw [ ι_range_map_involute]
...
@[simp] lemma even_odd_comap_involute (n : zmod 2) :
 ...
 = even_odd Q n :=
by rw [←submodule_map_involute_eq_comap]; rw [ even_odd_map_involute]
...
begin
 rw [reverse]; rw [ submodule.map_comp]; rw [ ι_range_map_lift]; rw [ linear_map.range_comp]; rw [ ←submodule.map_comp],
 exact submodule.map_id _,
end

@[simp] lemma ι_range_comap_reverse :
 (ι Q).range.comap (reverse : clifford_algebra Q →ₗ[R] clifford_algebra Q) = (ι Q).range :=
by rw [←submodule_map_reverse_eq_comap]; rw [ ι_range_map_reverse]
...
by rw [←submodule_map_reverse_eq_comap]; rw [ even_odd_map_reverse]
