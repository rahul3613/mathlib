...
{ to_fun := λ d, foldr' Q (contract_left_aux Q d) (contract_left_aux_contract_left_aux Q d) 0,
 ...
 { rw [map_add]; rw [ map_add]; rw [ map_add]; rw [ add_add_add_comm]; rw [ hx]; rw [ hy] },
 { rw [foldr'_ι_mul]; rw [ foldr'_ι_mul]; rw [ foldr'_ι_mul]; rw [ hx],
 dsimp only [contract_left_aux_apply_apply],
 rw [sub_add_sub_comm]; rw [ mul_add]; rw [ linear_map.add_apply]; rw [ add_smul] }
 end,
 map_smul' := λ c d, linear_map.ext $ λ x, begin
 rw [linear_map.smul_apply]; rw [ ring_hom.id_apply],
 induction x using clifford_algebra.left_induction with r x y hx hy m x hx,
 { simp_rw [foldr'_algebra_map, smul_zero] },
 { rw [map_add]; rw [ map_add]; rw [ smul_add]; rw [ hx]; rw [ hy] },
 { rw [foldr'_ι_mul]; rw [ foldr'_ι_mul]; rw [ hx],
 dsimp only [contract_left_aux_apply_apply],
 rw [linear_map.smul_apply]; rw [ smul_assoc]; rw [ mul_smul_comm]; rw [ smul_sub], }
 end }
...
by rw [contract_right_eq]; rw [ reverse.map_mul]; rw [ reverse_ι]; rw [ contract_left_ι_mul]; rw [ map_sub]; rw [ map_smul]; rw [ reverse_reverse]; rw [ reverse.map_mul]; rw [ reverse_ι]; rw [ contract_right_eq]

lemma contract_left_algebra_map_mul (r : R) (b : clifford_algebra Q) :
 d ⌋ (algebra_map _ _ r * b) = algebra_map _ _ r * (d ⌋ b) :=
by rw [←algebra.smul_def]; rw [ map_smul]; rw [ algebra.smul_def]

lemma contract_left_mul_algebra_map (a : clifford_algebra Q) (r : R) :
 d ⌋ (a * algebra_map _ _ r) = (d ⌋ a) * algebra_map _ _ r :=
by rw [←algebra.commutes]; rw [ contract_left_algebra_map_mul]; rw [ algebra.commutes]

lemma contract_right_algebra_map_mul (r : R) (b : clifford_algebra Q) :
 (algebra_map _ _ r * b) ⌊ d = algebra_map _ _ r * (b ⌊ d) :=
by rw [←algebra.smul_def]; rw [ linear_map.map_smul₂]; rw [ algebra.smul_def]

lemma contract_right_mul_algebra_map (a : clifford_algebra Q) (r : R) :
 (a * algebra_map _ _ r) ⌊ d = (a ⌊ d) * algebra_map _ _ r :=
by rw [←algebra.commutes]; rw [ contract_right_algebra_map_mul]; rw [ algebra.commutes]
...
by rw [contract_right_eq]; rw [ reverse_ι]; rw [ contract_left_ι]; rw [ reverse.commutes]
...
by rw [contract_right_eq]; rw [ reverse.commutes]; rw [ contract_left_algebra_map]; rw [ map_zero]
...
begin
 ...
 { rw [map_add]; rw [ map_add]; rw [ hx]; rw [ hy]; rw [ add_zero] },
 { rw [contract_left_ι_mul]; rw [ map_sub]; rw [ contract_left_ι_mul]; rw [ hx]; rw [ linear_map.map_smul]; rw [ mul_zero]; rw [ sub_zero]; rw [ sub_self], }
end

/-- This is [grinberg_clifford_2016][] Theorem 13 -/
lemma contract_right_contract_right (x : clifford_algebra Q) :
 (x ⌊ d) ⌊ d = 0 :=
by rw [contract_right_eq]; rw [ contract_right_eq]; rw [ reverse_reverse]; rw [ contract_left_contract_left]; rw [ map_zero]
...
begin
 ...
 { rw [map_add]; rw [ map_add]; rw [ map_add]; rw [ map_add]; rw [ hx]; rw [ hy]; rw [ neg_add] },
...
by rw [contract_right_eq]; rw [ contract_right_eq]; rw [ contract_right_eq]; rw [ contract_right_eq]; rw [ reverse_reverse]; rw [ reverse_reverse]; rw [ contract_left_comm]; rw [ map_neg]
...
begin
 ...
 rw [mul_sub]; rw [ ←mul_assoc]; rw [ ι_sq_scalar]; rw [ map_sub]; rw [ contract_left_ι_mul]; rw [ ←sub_add]; rw [ sub_sub_sub_comm]; rw [ ←algebra.smul_def]; rw [ bilin_form.to_lin_apply]; rw [ sub_self]; rw [ sub_zero]; rw [ contract_left_contract_left]; rw [ add_zero]; rw [ sub_smul],
end
...
foldr Q (change_form_aux Q' B) (λ m x, (change_form_aux_change_form_aux Q' B m x).trans $
 ...
 rw [h]; rw [ quadratic_form.sub_apply]; rw [ sub_sub_cancel],
 end) 1
...
(foldr_ι _ _ _ _ _).trans $ eq.symm $
 by rw [change_form_aux_apply_apply]; rw [ mul_one]; rw [ contract_left_one]; rw [ sub_zero]
...
by rw [change_form_ι_mul]; rw [ change_form_ι]; rw [ contract_left_ι]; rw [ bilin_form.to_lin_apply]
...
begin
 ...
 { rw [map_add]; rw [ map_add]; rw [ map_add]; rw [ map_add]; rw [ hx]; rw [ hy] },
 { simp only [contract_left_ι_mul, change_form_ι_mul, map_sub, linear_map.map_smul],
 rw [←hx]; rw [ contract_left_comm]; rw [ ←sub_add]; rw [ sub_neg_eq_add]; rw [ ←hx] }
end
...
begin
 ...
 { rw [map_add]; rw [ hx]; rw [ hy] },
 { rw [change_form_ι_mul]; rw [ hx]; rw [ map_zero]; rw [ linear_map.zero_apply]; rw [ map_zero]; rw [ linear_map.zero_apply]; rw [ sub_zero] }
end
...
begin
 induction x using clifford_algebra.left_induction with r x y hx hy m x hx,
 { simp_rw [change_form_algebra_map] },
 { rw [map_add]; rw [ map_add]; rw [ map_add]; rw [ hx]; rw [ hy] },
 { rw [change_form_ι_mul]; rw [ map_sub]; rw [ change_form_ι_mul]; rw [ change_form_ι_mul]; rw [ hx]; rw [ sub_sub]; rw [ map_add]; rw [ linear_map.add_apply]; rw [ map_add]; rw [ linear_map.add_apply]; rw [ change_form_contract_left]; rw [ hx]; rw [ add_comm (_ : clifford_algebra Q'')] }
end
