...
begin
 ...
 ⨆ i, supported M R (s i),
 { rwa [linear_map.range_comp] at this ; rwa [ range_restrict_dom] at this ; rwa [ submodule.map_top] at this ; rwa [ range_subtype] at this },
...
by erw [set.union_eq_Union]; erw [ supported_Union]; erw [ supr_bool_eq]; refl
...
theorem supported_inter (s t : set α) :
 supported M R (s ∩ t) = supported M R s ⊓ supported M R t :=
by rw [set.inter_eq_Inter]; rw [ supported_Inter]; rw [ infi_bool_eq]; refl

theorem disjoint_supported_supported {s t : set α} (h : disjoint s t) :
 disjoint (supported M R s) (supported M R t) :=
disjoint_iff.2 $ by rw [← supported_inter]; rw [ disjoint_iff_inter_eq_empty.1 h]; rw [ supported_empty]
...
begin
 ...
 { rw [← linear_map.comp_apply]; rw [ ← lmap_domain_comp],
...
begin
 rw disjoint_iff_inf_le,
 rintro l ⟨h₁, h₂⟩,
 rw [set_like.mem_coe] at h₂; rw [ mem_ker] at h₂; rw [ lmap_domain_apply] at h₂; rw [ map_domain] at h₂,
 simp, ext x,
 haveI := classical.dec_pred (λ x, x ∈ s),
 by_cases xs : x ∈ s,
 { have : finsupp.sum l (λ a, finsupp.single (f a)) (f x) = 0, {rw h₂, refl},
 rw [finsupp.sum_apply] at this; rw [ finsupp.sum] at this; rw [ finset.sum_eq_single x] at this,
...
finset.sum_subset hs $ λ x _ hxg, show l x • v x = 0, by rw [not_mem_support_iff.1 hxg]; rw [ zero_smul]
...
by rw [← total_single]; rw [ ← unique_single l]
...
begin
 ...
 intros x hx,
 rcases hx with ⟨i, hi⟩,
 rw [set_like.mem_coe]; rw [ linear_map.mem_range],
...
by rw [equiv_map_domain_eq_map_domain]; rw [ total_map_domain]
...
by rw [range_total]; rw [ subtype.range_coe_subtype]; rw [ set.set_of_mem_eq]
...
by rw [total_apply]; rw [ sum_option_index_smul]; rw [ total_apply]
...
begin
 rw [finsupp.total_on]; rw [ linear_map.range_eq_map]; rw [ linear_map.map_cod_restrict]; rw [ ← linear_map.range_le_iff_comap]; rw [ range_subtype]; rw [ submodule.map_top]; rw [ linear_map.range_comp]; rw [ range_subtype],
...
by rw [finsupp_prod_lequiv]; rw [ linear_equiv.coe_mk]; rw [ finsupp_prod_equiv]; rw [ finsupp.curry_apply]
...
by conv_rhs
 { rw [← (finsupp_prod_lequiv R).apply_symm_apply f]; rw [ finsupp_prod_lequiv_apply]; rw [ prod.mk.eta] }
...
by rw [← finsupp.total_eq_fintype_total]; rw [ linear_map.range_comp]; rw [ linear_equiv.to_linear_map_eq_coe]; rw [ linear_equiv.range]; rw [ submodule.map_top]; rw [ finsupp.range_total]
...
begin
 ...
 rw [linear_equiv_fun_on_finite_symm_single]; rw [ finsupp.sum_single_index]; rw [ one_smul]; rw [ (s (finsupp.single x 1)).some_spec]; rw [ finsupp.single_eq_pi_single],
 rw [zero_smul],
end
