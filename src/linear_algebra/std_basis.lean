...
@[simp] lemma std_basis_apply' (i i' : ι) : (std_basis R (λ (_x : ι), R) i) 1 i' =
 ite (i = i') 1 0 :=
begin
 rw [linear_map.std_basis_apply]; rw [ function.update_apply]; rw [ pi.zero_apply],
...
by rw [std_basis_eq_pi_diag]; rw [ proj_pi]
...
begin
 ...
 rintro b - j hj,
 rw [proj_std_basis_ne R φ j i]; rw [ zero_apply],
 rintro rfl,
...
begin
 ...
 { ext i,
 rw [finset.sum_apply]; rw [ ← std_basis_same R φ i (b i)],
...
begin
 ...
 { rw [finset.coe_univ]; rw [ set.union_empty] }
end
...
begin
 ...
 { intro j,
 rw [span_le]; rw [ linear_map.range_coe],
...
begin
 ...
 dsimp,
 rw [std_basis_ne _ _ _ _ (ne.symm hj)]; rw [ linear_equiv.map_zero]; rw [ finsupp.zero_apply]; rw [ finsupp.single_eq_of_ne],
 rintros ⟨⟩,
 contradiction
end
