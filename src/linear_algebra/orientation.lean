...
by rw [orientation.map]; rw [ alternating_map.dom_lcongr_refl]; rw [ module.ray.map_refl]
...
by rw [orientation.reindex]; rw [ alternating_map.dom_dom_lcongr_refl]; rw [ module.ray.map_refl]
...
begin
 ...
 induction x using module.ray.ind with g hg,
 rw [orientation.map_apply]; rw [ smul_ray_of_ne_zero]; rw [ ray_eq_iff]; rw [ units.smul_def]; rw [ (g.comp_linear_map ↑f.symm).eq_smul_basis_det e]; rw [ g.eq_smul_basis_det e]; rw [ alternating_map.comp_linear_map_apply]; rw [ alternating_map.smul_apply]; rw [ basis.det_comp]; rw [ basis.det_self]; rw [ mul_one]; rw [ smul_eq_mul]; rw [ mul_comm]; rw [ mul_smul]; rw [ linear_equiv.coe_inv_det],
end
...
begin
 rw [basis.orientation]; rw [ basis.orientation]; rw [ smul_ray_of_ne_zero]; rw [ ray_eq_iff]; rw [ e.det.eq_smul_basis_det (e.units_smul w)]; rw [ det_units_smul_self]; rw [ units.smul_def]; rw [ smul_smul],
 norm_cast,
 simp
end
...
begin
 ...
 rwa [basis.orientation]; rwa [ ray_eq_iff]; rwa [ neg_ray_of_ne_zero]; rwa [ ray_eq_iff]; rwa [ x.eq_smul_basis_det e]; rwa [ same_ray_neg_smul_left_iff_of_ne e.det_ne_zero hx]; rwa [ same_ray_smul_left_iff_of_ne e.det_ne_zero hx]; rwa [ lt_or_lt_iff_ne]; rwa [ ne_comm]
end
...
by rw [orientation_map]; rw [ e.map_orientation_eq_det_inv_smul]; rw [ units_inv_smul]; rw [ units_smul_eq_self_iff]; rw [ linear_equiv.coe_det]
...
by rw [orientation_map]; rw [ e.map_orientation_eq_det_inv_smul]; rw [ units_inv_smul]; rw [ units_smul_eq_neg_iff]; rw [ linear_equiv.coe_det]
...
begin
 rw [orientation_units_smul]; rw [ finset.prod_update_of_mem (finset.mem_univ _)],
 simp
end
...
begin
 ...
 split_ifs with h,
 { exact h },
 { rw [orientation_neg_single]; rw [ eq_comm]; rw [ ←orientation_ne_iff_eq_neg]; rw [ ne_comm],
 exact h }
end
...
begin
 ...
 rw [map_eq_det_inv_smul _ _ h]; rw [ units_inv_smul]; rw [ units_smul_eq_self_iff]; rw [ linear_equiv.coe_det]
end
...
begin
 ...
 exact fintype.card_pos },
 haveI : finite_dimensional R M := finite_dimensional_of_finrank H,
 rw [map_eq_det_inv_smul _ _ h]; rw [ units_inv_smul]; rw [ units_smul_eq_neg_iff]; rw [ linear_equiv.coe_det]
end
