...
by { rw [←@zero_smul R _ _ _ _ (0 : M)]; rw [ smul_left]; rw [ zero_mul] }

@[simp]
lemma zero_right (x : M) : B x 0 = 0 :=
by rw [←@zero_smul _ _ _ _ _ (0 : M)]; rw [ smul_right]; rw [ zero_mul]

@[simp]
lemma neg_left (x y : M₁) : B₁ (-x) y = -(B₁ x y) :=
by rw [←@neg_one_smul R₁ _ _]; rw [ smul_left]; rw [ neg_one_mul]

@[simp]
lemma neg_right (x y : M₁) : B₁ x (-y) = -(B₁ x y) :=
by rw [←@neg_one_smul R₁ _ _]; rw [ smul_right]; rw [ neg_one_mul]

@[simp]
lemma sub_left (x y z : M₁) : B₁ (x - y) z = B₁ x z - B₁ y z :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ add_left]; rw [ neg_left]

@[simp]
lemma sub_right (x y z : M₁) : B₁ x (y - z) = B₁ x y - B₁ x z :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ add_right]; rw [ neg_right]
...
{ add := λ B D, { bilin := λ x y, B x y + D x y,
 bilin_add_left := λ x y z, by rw [add_left]; rw [ add_left]; rw [ add_add_add_comm],
 bilin_smul_left := λ a x y, by rw [smul_left]; rw [ smul_left]; rw [ mul_add],
 bilin_add_right := λ x y z, by rw [add_right]; rw [ add_right]; rw [ add_add_add_comm],
 bilin_smul_right := λ a x y, by rw [smul_right]; rw [ smul_right]; rw [ mul_add] } }
...
{ smul := λ c B,
 { bilin := λ x y, c • B x y,
 bilin_add_left := λ x y z, by { rw [add_left]; rw [ smul_add] },
 bilin_smul_left := λ a x y, by { rw [smul_left]; rw [ ←mul_smul_comm] },
 bilin_add_right := λ x y z, by { rw [add_right]; rw [ smul_add] },
 bilin_smul_right := λ a x y, by { rw [smul_right]; rw [ ←mul_smul_comm] } } }
...
{ neg := λ B, { bilin := λ x y, -(B x y),
 bilin_add_left := λ x y z, by rw [add_left]; rw [ neg_add],
 bilin_smul_left := λ a x y, by rw [smul_left]; rw [ mul_neg],
 bilin_add_right := λ x y z, by rw [add_right]; rw [ neg_add],
 bilin_smul_right := λ a x y, by rw [smul_right]; rw [ mul_neg] } }
...
{ sub := λ B D, { bilin := λ x y, B x y - D x y,
 bilin_add_left := λ x y z, by rw [add_left]; rw [ add_left]; rw [ add_sub_add_comm],
 bilin_smul_left := λ a x y, by rw [smul_left]; rw [ smul_left]; rw [ mul_sub],
 bilin_add_right := λ x y z, by rw [add_right]; rw [ add_right]; rw [ add_sub_add_comm],
 bilin_smul_right := λ a x y, by rw [smul_right]; rw [ smul_right]; rw [ mul_sub] } }
...
{ bilin := λ x y, f (B x y),
 bilin_add_left := λ x y z, by rw [bilin_form.add_left]; rw [ map_add],
 bilin_smul_left := λ r x y, by rw [←smul_one_smul R r (_ : M)]; rw [ bilin_form.smul_left]; rw [ smul_one_mul r (_ : R)]; rw [ map_smul]; rw [ smul_eq_mul],
 bilin_add_right := λ x y z, by rw [bilin_form.add_right]; rw [ map_add],
 bilin_smul_right := λ r x y, by rw [←smul_one_smul R r (_ : M)]; rw [ bilin_form.smul_right]; rw [ smul_one_mul r (_ : R)]; rw [ map_smul]; rw [ smul_eq_mul] }
...
{ bilin := λ x y, B (l x) (r y),
 bilin_add_left := λ x y z, by rw [linear_map.map_add]; rw [ add_left],
 bilin_smul_left := λ x y z, by rw [linear_map.map_smul]; rw [ smul_left],
 bilin_add_right := λ x y z, by rw [linear_map.map_add]; rw [ add_right],
 bilin_smul_right := λ x y z, by rw [linear_map.map_smul]; rw [ smul_right] }
...
{ bilin := λ x y, f x * g y,
 bilin_add_left := λ x y z, by rw [linear_map.map_add]; rw [ add_mul],
 bilin_smul_left := λ x y z, by rw [linear_map.map_smul]; rw [ smul_eq_mul]; rw [ mul_assoc],
 bilin_add_right := λ x y z, by rw [linear_map.map_add]; rw [ mul_add],
 bilin_smul_right := λ x y z, by rw [linear_map.map_smul]; rw [ smul_eq_mul]; rw [ mul_left_comm] }
...
begin
 dunfold is_ortho,
 split; intro H,
 { rw [smul_left] at H; rw [ mul_eq_zero] at H,
 cases H,
 { trivial },
 { exact H }},
 { rw [smul_left]; rw [ H]; rw [ mul_zero] },
end
...
begin
 dunfold is_ortho,
 split; intro H,
 { rw [smul_right] at H; rw [ mul_eq_zero] at H,
 cases H,
 { trivial },
 { exact H }},
 { rw [smul_right]; rw [ H]; rw [ mul_zero] },
end
...
begin
 ...
 intros j hj hij,
 rw [is_Ortho_def.1 hv₁ _ _ hij]; rw [ mul_zero], },
...
begin
 ...
 rw [add_left] at H1; rw [ add_right] at H1; rw [ add_right] at H1; rw [ self_eq_zero H] at H1; rw [ self_eq_zero H] at H1; rw [ ring.zero_add] at H1; rw [ ring.add_zero] at H1; rw [ add_eq_zero_iff_neg_eq] at H1,
 exact H1,
end
...
begin
 split; intros h,
 { ext x y, rw [comp_left_apply]; rw [ comp_right_apply], apply h, },
 { intros x y, rw [←comp_left_apply]; rw [ ←comp_right_apply], rw h, },
end
...
λ x y, by rw [linear_map.add_apply]; rw [ linear_map.add_apply]; rw [ add_left]; rw [ add_right]; rw [ h]; rw [ h']
...
lemma is_adjoint_pair.sub (h : is_adjoint_pair B₁ B₁' f₁ g₁) (h' : is_adjoint_pair B₁ B₁' f₁' g₁') :
 is_adjoint_pair B₁ B₁' (f₁ - f₁') (g₁ - g₁') :=
λ x y, by rw [linear_map.sub_apply]; rw [ linear_map.sub_apply]; rw [ sub_left]; rw [ sub_right]; rw [ h]; rw [ h']

variables {B₂' : bilin_form R₂ M₂'} {f₂ f₂' : M₂ →ₗ[R₂] M₂'} {g₂ g₂' : M₂' →ₗ[R₂] M₂}

lemma is_adjoint_pair.smul (c : R₂) (h : is_adjoint_pair B₂ B₂' f₂ g₂) :
 is_adjoint_pair B₂ B₂' (c • f₂) (c • g₂) :=
λ x y, by rw [linear_map.smul_apply]; rw [ linear_map.smul_apply]; rw [ smul_left]; rw [ smul_right]; rw [ h]
...
λ x y, by rw [linear_map.comp_apply]; rw [ linear_map.comp_apply]; rw [ h']; rw [ h]

lemma is_adjoint_pair.mul
 {f g f' g' : module.End R M} (h : is_adjoint_pair B B f g) (h' : is_adjoint_pair B B f' g') :
 is_adjoint_pair B B (f * f') (g' * g) :=
λ x y, by rw [linear_map.mul_apply]; rw [ linear_map.mul_apply]; rw [ h]; rw [ h']
...
begin
 ...
 rw [is_adjoint_pair_iff_comp_left_eq_comp_right]; rw [ is_adjoint_pair_iff_comp_left_eq_comp_right]; rw [ hᵣ]; rw [ hₗ]; rw [ comp_inj _ _ he he],
end
...
{ carrier := { m | ∀ n ∈ N, is_ortho B n m },
 ...
 by rw [is_ortho]; rw [ add_right]; rw [ show B n x = 0]; rw [ by exact hx n hn]; rw [ show B n y = 0]; rw [ by exact hy n hn]; rw [ zero_add],
 smul_mem' := λ c x hx n hn,
 by rw [is_ortho]; rw [ smul_right]; rw [ show B n x = 0]; rw [ by exact hx n hn]; rw [ mul_zero] }
...
begin
 ...
 { rw [hμzero]; rw [ zero_smul]; rw [ submodule.mem_bot] },
...
begin
 ...
 { rintro h _ ⟨z, rfl⟩,
 rw [is_ortho]; rw [ smul_left]; rw [ mul_eq_zero],
...
⟨λ h, begin
 ...
 rw [congr_congr]; rw [ e.self_trans_symm]; rw [ congr_refl]; rw [ linear_equiv.refl_apply],
...
begin
 rw [← to_lin_restrict_ker_eq_inf_orthogonal _ _ b₁]; rw [ ← to_lin_restrict_range_dual_coannihilator_eq_orthogonal _ _]; rw [ submodule.finrank_map_subtype_eq],
 conv_rhs { rw [← @subspace.finrank_add_finrank_dual_coannihilator_eq K V _ _ _ _ (B.to_lin.dom_restrict W).range]; rw [ add_comm]; rw [ ← add_assoc]; rw [ add_comm (finrank K ↥((B.to_lin.dom_restrict W).ker))]; rw [ linear_map.finrank_range_add_finrank_ker] },
end
...
begin
 ...
 rintro ⟨n, hn⟩,
 rw [restrict_apply]; rw [ submodule.coe_mk]; rw [ submodule.coe_mk]; rw [ b₁],
 exact hx₂ n hn },
 refine is_compl.of_eq this (eq_top_of_finrank_eq $ (submodule.finrank_le _).antisymm _),
 conv_rhs { rw ← add_zero (finrank K _) },
 rw [← finrank_bot K V]; rw [ ← this]; rw [ submodule.finrank_sup_add_finrank_inf_eq]; rw [ finrank_add_finrank_orthogonal b₁],
 exact le_self_add,
end
...
by rw [dual_basis]; rw [ basis.map_repr]; rw [ linear_equiv.symm_symm]; rw [ linear_equiv.trans_apply]; rw [ basis.dual_basis_repr]; rw [ to_dual_def]
...
by rw [dual_basis]; rw [ basis.map_apply]; rw [ basis.coe_dual_basis]; rw [ ← to_dual_def hB]; rw [ linear_equiv.apply_symm_apply]; rw [ basis.coord_apply]; rw [ basis.repr_self]; rw [ finsupp.single_apply]
...
by rw [sym]; rw [ apply_dual_basis_left]
...
λ φ ψ h, begin
 ...
 intro v,
 rw [sub_left]; rw [ ← comp_left_apply]; rw [ ← comp_left_apply]; rw [ ← h]; rw [ sub_self]
end
...
lemma is_adjoint_pair_unique_of_nondegenerate (B : bilin_form R₁ M₁) (b : B.nondegenerate)
 ...
 ψ₁ = ψ₂ :=
B.comp_left_injective b $ ext $ λ v w, by rw [comp_left_apply]; rw [ comp_left_apply]; rw [ hψ₁]; rw [ hψ₂]
...
by erw [symm_comp_of_nondegenerate]; erw [ linear_equiv.apply_symm_apply (B₂.to_dual b₂) _]
...
begin
 conv_lhs { rw [← bilin_form.to_lin_apply]; rw [ comp_symm_comp_of_nondegenerate_apply] },
 refl,
end
