...
begin
 ...
 classical,
 rw [← matrix.det_diagonal]; rw [ ← linear_map.det_to_lin b'],
 ...
 refine b'.ext (λ i, _),
 simp_rw [linear_map.comp_apply, linear_equiv.coe_to_linear_map, matrix.to_lin_apply, basis.repr_self, finsupp.single_eq_pi_single, matrix.diagonal_mul_vec_single, pi.single_apply, ite_smul, zero_smul, finset.sum_ite_eq', mul_one, if_pos (finset.mem_univ _), b'.equiv_apply],
 change _ = f * _,
 rw [mul_comm]; rw [ ← smul_eq_mul]; rw [ linear_equiv.restrict_scalars_apply]; rw [ linear_equiv.coord_apply_smul]; rw [ ideal.self_basis_def],
 refl
end
...
begin
 ...
 rw [nat_degree_eq_of_degree_eq (degree_eq_degree_of_associated $ associated_norm_prod_smith b hf)]; rw [ nat_degree_prod _ _ (λ i _, smith_coeffs_ne_zero b _ h i)]; rw [ finrank_quotient_eq_sum F h b],
