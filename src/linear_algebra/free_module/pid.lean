...
begin
 ...
 rw [linear_equiv.map_zero]; rw [ finsupp.zero_apply],
...
begin
 ...
 erw [ideal.span_singleton_eq_span_singleton]; erw [ ← dvd_dvd_iff_associated]; erw [ ← mem_iff_generator_dvd],
...
begin
 ...
 rw [dvd_generator_iff]; rw [ ideal.span]; rw [ ← span_singleton_generator (submodule.span R {a, ψ ⟨y, hNO yN⟩})],
 ...
 { rw [span_le]; rw [ singleton_subset_iff]; rw [ set_like.mem_coe]; rw [ linear_map.mem_submodule_image_of_le hNO],
 refine ⟨y, yN, _⟩,
...
begin
 ...
 rw [linear_map.map_sub]; rw [ linear_map.map_smul]; rw [ hb]; rw [ ϕy_eq]; rw [ smul_eq_mul]; rw [ mul_comm]; rw [ sub_self] },
 ...
 { rw [linear_map.map_sub]; rw [ linear_map.map_smul]; rw [ ϕy'_eq]; rw [ smul_eq_mul]; rw [ mul_one]; rw [ sub_self] },
 { rw [linear_map.map_sub]; rw [ linear_map.map_smul]; rw [ sub_eq_add_neg]; rw [ neg_smul], refl } },

 -- It remains to show the extended bases are compatible with each other.
 intros as h,
 refine ⟨fin.cons a as, _⟩,
 intro i,
 rw [basis.coe_mk_fin_cons_of_le]; rw [ basis.coe_mk_fin_cons_of_le],
...
begin
 ...
 { suffices : ∀ i, φ (s i) ∈ N,
 { rw [linear_map.range_eq_map]; rw [ ← hs]; rw [ φ.map_span_le],
 rintros _ ⟨i, rfl⟩, apply this },
 intro i,
 calc (∏ j, a j) • s i = (∏ j in {i}ᶜ, a j) • a i • s i :
 by rw [fintype.prod_eq_prod_compl_mul i]; rw [ mul_smul]
 ... ∈ N : N.smul_mem _ (ha' i) },
...
let e : fin n ≃ fin (fintype.card ι) := fintype.equiv_of_card_eq (by rw [eq]; rw [ fintype.card_fin]) in
