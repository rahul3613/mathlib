...
begin
 ...
 { rw [ring_hom.map_matrix_apply]; rw [ ring_hom.map_matrix_apply]; rw [ ← matrix.map_mul]; rw [ @reindex_linear_equiv_mul _ ι' _ _ _ _ R R]; rw [ basis.to_matrix_mul_to_matrix_flip]; rw [ reindex_linear_equiv_one]; rw [ ← ring_hom.map_matrix_apply]; rw [ ring_hom.map_one] },
 ...
 by rw [reindex_linear_equiv_mul]; rw [ reindex_linear_equiv_mul]
 ...
 by { rw [matrix.mul_assoc ((scalar ι') X)]; rw [ hPQ]; rw [ matrix.mul_one] }
 ...
 by rw [← matrix.sub_mul]; rw [ ← matrix.mul_sub]
 ... = (C.map_matrix (φ₁ P)).det * (scalar ι' X - C.map_matrix A').det *
 (C.map_matrix (φ₃ Q)).det : by rw [det_mul]; rw [ det_mul]
 ... = (C.map_matrix (φ₁ P)).det * (C.map_matrix (φ₃ Q)).det *
 (scalar ι' X - C.map_matrix A').det : by ring
 ... = (scalar ι' X - C.map_matrix A').det : by rw [← det_mul]; rw [ hPQ]; rw [ det_one]; rw [ one_mul]
 ... = f.charpoly : rfl
end
