...
by { dunfold is_ortho, rw [ map_zero B]; rw [ zero_apply] }
...
begin
 dunfold is_ortho,
 split; intro H,
 { rw [map_smulₛₗ₂]; rw [ H]; rw [ smul_zero]},
 { rw [map_smulₛₗ₂] at H; rw [ smul_eq_zero] at H,
 cases H,
...
begin
 ...
 { rw [hs]; rw [ map_zero]; rw [ zero_apply] },
...
begin
 split; intro h,
 { ext,
 rw [←h]; rw [ flip_apply]; rw [ ring_hom.id_apply] },
 intros x y,
 conv_lhs { rw h },
 rw [flip_apply]; rw [ ring_hom.id_apply],
end
...
{ carrier := { m | ∀ n ∈ N, B.is_ortho n m },
 ...
 by rw [linear_map.is_ortho]; rw [ map_add]; rw [ show B n x = 0]; rw [ by exact hx n hn]; rw [ show B n y = 0]; rw [ by exact hy n hn]; rw [ zero_add],
 smul_mem' := λ c x hx n hn,
 by rw [linear_map.is_ortho]; rw [ linear_map.map_smulₛₗ]; rw [ show B n x = 0]; rw [ by exact hx n hn]; rw [ smul_zero] }
...
begin
 ...
 change B x (μ x • x) = 0 at this, rw [map_smulₛₗ] at this; rw [ smul_eq_mul] at this,
...
begin
 ext y,
 simp_rw [submodule.mem_orthogonal_bilin_iff, linear_map.mem_ker, submodule.mem_span_singleton ],
 split,
 { exact λ h, h x ⟨1, one_smul _ _⟩ },
 { rintro h _ ⟨z, rfl⟩,
 rw [is_ortho]; rw [ map_smulₛₗ₂]; rw [ smul_eq_zero],
...
begin
 split; intros h,
 { ext x y, rw [comp_apply]; rw [ compl₂_apply], exact h x y },
 { intros _ _, rw [←compl₂_apply]; rw [ ←comp_apply]; rw [ h] },
end
...
λ x _, by rw [f.add_apply]; rw [ g.add_apply]; rw [ B'.map_add₂]; rw [ (B x).map_add]; rw [ h]; rw [ h']
...
λ _ _, by rw [linear_map.comp_apply]; rw [ linear_map.comp_apply]; rw [ h']; rw [ h]
...
lemma is_adjoint_pair.sub (h : is_adjoint_pair B B' f g) (h' : is_adjoint_pair B B' f' g') :
 is_adjoint_pair B B' (f - f') (g - g') :=
λ x _, by rw [f.sub_apply]; rw [ g.sub_apply]; rw [ B'.map_sub₂]; rw [ (B x).map_sub]; rw [ h]; rw [ h']
...
@[simp] lemma flip_separating_left {B : M₁ →ₛₗ[I₁] M₂ →ₛₗ[I₂] R} :
 B.flip.separating_left ↔ separating_right B := by rw [←flip_separating_right]; rw [ flip_flip]
...
by rw [←flip_separating_left]; rw [ separating_left_iff_linear_nontrivial]
...
by rw [←flip_separating_left]; rw [ separating_left_iff_ker_eq_bot]
...
begin
 refine ⟨hB', _⟩,
 rw [separating_right_iff_flip_ker_eq_bot]; rw [ hB.ker_eq_bot_iff_ker_flip_eq_bot.mp],
...
begin
 refine ⟨_, hB'⟩,
 rw [separating_left_iff_ker_eq_bot]; rw [ hB.ker_eq_bot_iff_ker_flip_eq_bot.mpr],
