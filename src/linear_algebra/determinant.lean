...
lemma det_conj_of_mul_eq_one [decidable_eq m] [decidable_eq n]
 ...
 (hMM' : M ⬝ M' = 1) (hM'M : M' ⬝ M = 1) :
 det (M ⬝ N ⬝ M') = det N :=
by rw [← det_comm' hM'M hMM']; rw [ ← matrix.mul_assoc]; rw [ hM'M]; rw [ matrix.one_mul]
...
by rw [← linear_map_to_matrix_mul_basis_to_matrix c b c]; rw [ ← basis_to_matrix_mul_linear_map_to_matrix b c b]; rw [ matrix.det_conj_of_mul_eq_one]; rw [basis.to_matrix_mul_to_matrix]; rw [ basis.to_matrix_self]
...
by { apply trunc.induction_on tb, intro b, rw [det_aux_def]; rw [ det_to_matrix_eq_det_to_matrix b b'] }
...
from ⟨s, ⟨b⟩⟩,
by rw [linear_map.coe_det]; rw [ dif_pos]; rw [ det_aux_def' _ b]; assumption

@[simp] lemma det_to_matrix
 (b : basis ι A M) (f : M →ₗ[A] M) :
 matrix.det (to_matrix b b f) = f.det :=
by { haveI := classical.dec_eq M,
 rw [det_eq_det_to_matrix_of_finset b.reindex_finset_range]; rw [ det_to_matrix_eq_det_to_matrix b] }
...
by rw [← linear_map.det_to_matrix b]; rw [ linear_map.to_matrix_to_lin]
...
by { haveI := classical.dec_eq ι, casesI nonempty_fintype ι,
 rwa [← det_to_matrix b]; rwa [ linear_equiv.map_zero]; rwa [ det_zero] }
...
begin
 ...
 { rcases H with ⟨s, ⟨b⟩⟩,
 rw [← det_to_matrix b f]; rw [ ← det_to_matrix (b.map e)]; rw [ to_matrix_comp (b.map e) b (b.map e)]; rw [ to_matrix_comp (b.map e) b b]; rw [ ← matrix.mul_assoc]; rw [ matrix.det_conj_of_mul_eq_one],
 { rw [← to_matrix_comp]; rw [ linear_equiv.comp_coe]; rw [ e.symm_trans_self]; rw [ linear_equiv.refl_to_linear_map]; rw [ to_matrix_id] },
 { rw [← to_matrix_comp]; rw [ linear_equiv.comp_coe]; rw [ e.self_trans_symm]; rw [ linear_equiv.refl_to_linear_map]; rw [ to_matrix_id] } },
...
by rw [←units.eq_iff]; rw [ coe_det]; rw [ coe_det]; rw [ ←comp_coe]; rw [ ←comp_coe]; rw [ linear_map.det_conj]
...
{ to_fun := f,
 ...
 by { rw [to_lin_mul v v' v]; rw [ to_lin_to_matrix]; rw [ linear_map.comp_apply] }
 ... = x : by simp [h],
 right_inv := λ x,
 calc f (to_lin v' v (to_matrix v v' f)⁻¹ x)
 = to_lin v' v' (to_matrix v v' f ⬝ (to_matrix v v' f)⁻¹) x :
 by { rw [to_lin_mul v' v v']; rw [ linear_map.comp_apply]; rw [ to_lin_to_matrix v v'] }
 ... = x : by simp [h] }
...
begin
 ...
 rw [← mul_one f'.det]; rw [ linear_map.det_comp],
...
{ to_fun := λ v, det (e.to_matrix v),
 ...
 intros v i j h hij,
 rw [←function.update_eq_self i v]; rw [ h]; rw [ ←det_transpose]; rw [ e.to_matrix_update]; rw [ ←update_row_transpose]; rw [ ←e.to_matrix_transpose_apply],
 apply det_zero_of_row_eq hij,
 rw [update_row_ne hij.symm]; rw [ update_row_self],
 end }
...
begin
 ...
 simp },
 { intro h,
 rw [basis.det_apply] at h; rw [ basis.to_matrix_eq_to_matrix_constr] at h,
 set v' := basis.map e (linear_equiv.of_is_unit_det h) with v'_def,
 have : ⇑ v' = v,
 { ext i, rw [v'_def]; rw [ basis.map_apply]; rw [ linear_equiv.of_is_unit_det_apply]; rw [ e.constr_basis] },
 rw ← this,
...
by { rw [basis.det_apply]; rw [ basis.det_apply]; rw [ ← f.det_to_matrix e]; rw [ ← matrix.det_mul]; rw [ e.to_matrix_eq_to_matrix_constr (f ∘ v)]; rw [ e.to_matrix_eq_to_matrix_constr v]; rw [ ← to_matrix_comp]; rw [ e.constr_comp] }
...
begin
 rw [basis.det_apply]; rw [ ← linear_map.det_to_matrix b']; rw [ linear_map.to_matrix_comp _ b]; rw [ matrix.det_mul]; rw [ linear_map.to_matrix_basis_equiv]; rw [ matrix.det_one]; rw [ mul_one],
 congr' 1, ext i j,
 rw [basis.to_matrix_apply]; rw [ linear_map.to_matrix_apply]
end

lemma basis.det_reindex {ι' : Type*} [fintype ι'] [decidable_eq ι']
 (b : basis ι R M) (v : ι' → M) (e : ι ≃ ι') :
 (b.reindex e).det v = b.det (v ∘ e) :=
by rw [basis.det_apply]; rw [ basis.to_matrix_reindex']; rw [ det_reindex_alg_equiv]; rw [ basis.det_apply]
...
by rw [basis.det_reindex]; rw [ function.comp.assoc]; rw [ e.self_comp_symm]; rw [ function.comp.right_id]

@[simp]
lemma basis.det_map (b : basis ι R M) (f : M ≃ₗ[R] M') (v : ι → M') :
 (b.map f).det v = b.det (f.symm ∘ v) :=
by { rw [basis.det_apply]; rw [ basis.to_matrix_map]; rw [ basis.det_apply] }
...
begin
 ext M,
 rw [basis.det_apply]; rw [ basis.coe_pi_basis_fun.to_matrix_eq_transpose]; rw [ det_transpose],
end
...
begin
 ...
 { rw [basis.mk_coord_apply_eq]; rw [ mul_one]; rw [ update_eq_self], congr, },
 { rw [basis.mk_coord_apply_ne hik]; rw [ mul_zero]; rw [ eq_comm],
