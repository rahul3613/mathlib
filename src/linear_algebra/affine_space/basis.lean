...
begin
 ...
 { rw [this]; rw [ ← direction_affine_span]; rw [ b.tot]; rw [ affine_subspace.direction_top], exact le_rfl },
...
{ to_fun := λ q, 1 - (b.basis_of i).sum_coords (q -ᵥ b i),
 ...
 map_vadd' := λ q v, by rw [vadd_vsub_assoc]; rw [ linear_map.map_add]; rw [ vadd_eq_add]; rw [ linear_map.neg_apply]; rw [ sub_add_eq_sub_sub_swap]; rw [ add_comm]; rw [ sub_eq_add_neg], }
...
by rw [coord]; rw [ affine_map.coe_mk]; rw [ ← subtype.coe_mk j h.symm]; rw [ ← b.basis_of_apply]; rw [ basis.sum_coords_self_apply]; rw [ sub_self]
...
begin
 ...
 rw [← b.affine_combination_coord_eq_self q₁]; rw [ ← b.affine_combination_coord_eq_self q₂],
 simp only [h],
end
...
begin
 ...
 rw [pi.one_apply]; rw [ hq]; rw [ b.coord_apply_combination_of_mem hi hw],
end
...
by rw [finset.centroid]; rw [ b.coord_apply_combination_of_mem hi (s.sum_centroid_weights_eq_one_of_nonempty _ ⟨i, hi⟩)]; rw [ finset.centroid_weights]
