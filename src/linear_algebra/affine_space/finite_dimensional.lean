...
begin
 ...
 rw [affine_independent_set_iff_linear_independent_vsub k hp₁'] at hi'; rw [ ← finset.coe_singleton] at hi'; rw [ ← finset.coe_image] at hi'; rw [ ← finset.coe_sdiff] at hi'; rw [ finset.sdiff_singleton_eq_erase] at hi'; rw [ ← finset.coe_image] at hi',
 ...
 rwa [vector_span_eq_span_vsub_finset_right_ne k hp₁]; rwa [ finrank_span_finset_eq_card]; rwa [ hc]
end
...
begin
 have hn : (s.image p).nonempty,
 { rw [finset.nonempty.image_iff]; rw [ ← finset.card_pos]; rw [ hc], apply nat.succ_pos },
 rcases hn with ⟨p₁, hp₁⟩,
...
begin
 ...
 cases hn with i₁,
 rw [affine_independent_iff_linear_independent_vsub _ _ i₁]; rw [ linear_independent_iff_card_eq_finrank_span]; rw [ eq_comm]; rw [ vector_span_range_eq_span_range_vsub_right_ne k p i₁],
 congr',
 rw ←finset.card_univ at hc,
...
lemma affine_independent_iff_not_finrank_vector_span_le [fintype ι] (p : ι → P) {n : ℕ}
 ...
 affine_independent k p ↔ ¬ finrank k (vector_span k (set.range p)) ≤ n :=
by rw [affine_independent_iff_le_finrank_vector_span k p hc]; rw [ ←nat.lt_iff_add_one_le]; rw [ lt_iff_not_ge]
...
begin
 have hn : s.nonempty,
 { rw [←finset.card_pos]; rw [ hc], apply nat.succ_pos },
...
begin
 rw ←finset.card_univ at hc,
 rw [←set.image_univ] at ⊢ hle; rw [ ←finset.coe_univ] at ⊢ hle; rw [ ← finset.coe_image] at ⊢ hle,
...
begin
 ...
 rw [hn]; rw [ ← finrank_top]; rw [ ← (vector_span_eq_top_of_affine_span_eq_top k V P) h_tot]; rw [ ← hi.finrank_vector_span hn], },
 { intros hc,
 rw [← finrank_top] at hc; rw [ ← direction_top k V P] at hc,
...
by rw [affine_independent.affine_span_eq_top_iff_card_eq_finrank_add_one T.independent]; rw [ fintype.card_fin]; rw [ hrank]
...
begin
 rw [←direction_affine_span]; rw [ ←affine_span_insert_affine_span],
 rcases (s : set P).eq_empty_or_nonempty with hs | ⟨p₀, hp₀⟩,
 { rw coe_eq_bot_iff at hs,
 rw [hs]; rw [ bot_coe]; rw [ span_empty]; rw [ bot_coe]; rw [ direction_affine_span],
 convert finite_dimensional_bot _ _;
 simp },
 { rw [affine_span_coe]; rw [ direction_affine_span_insert hp₀],
 apply_instance }
end
...
begin
 ...
 rw [←direction_affine_span]; rw [ ←affine_span_insert_affine_span]; rw [ direction_affine_span],
...
begin
 rw [collinear_iff_rank_le_one]; rw [ vector_span_empty],
 simp
end
...
begin
 rw [collinear_iff_rank_le_one]; rw [ vector_span_singleton],
 simp
end
...
begin
 ...
 { rw [vector_span_eq_span_vsub_set_right k h]; rw [ submodule.span_le]; rw [ set.subset_def],
 intros x hx,
 rw [set_like.mem_coe]; rw [ submodule.mem_span_singleton],
...
begin
 ...
 use [p₁, p₂ -ᵥ p₁],
 intros p hp,
 rw [set.mem_insert_iff] at hp; rw [ set.mem_singleton_iff] at hp,
 cases hp,
 { use 0,
 simp [hp] },
 { use 1,
...
by rw [collinear_iff_finrank_le_one]; rw [ affine_independent_iff_not_finrank_vector_span_le k p (fintype.card_fin 3)]
...
by rw [collinear_iff_finrank_le_one]; rw [ finrank_vector_span_le_iff_not_affine_independent k p (fintype.card_fin 3)]
...
begin
 ...
 rw [affine_independent_iff_not_collinear]; rw [ ←set.image_univ]; rw [ ←finset.coe_univ]; rw [ hu]; rw [ finset.coe_insert]; rw [ finset.coe_insert]; rw [ finset.coe_singleton]; rw [ set.image_insert_eq]; rw [ set.image_pair]
end
...
begin
 ...
 { conv_lhs { rw [←direction_affine_span]; rw [ ←affine_span_insert_affine_span] },
 conv_rhs { rw [←direction_affine_span]; rw [ ←affine_span_insert_affine_span] },
...
by rw [collinear]; rw [ collinear]; rw [ vector_span_insert_eq_vector_span h]
...
begin
 rw [collinear_insert_iff_of_mem_affine_span ((affine_subspace.le_def' _ _).1 (affine_span_mono k (set.subset_insert _ _)) _ h₁)]; rw [ collinear_insert_iff_of_mem_affine_span h₂],
 exact collinear_pair _ _ _
end
...
begin
 rw [collinear_insert_iff_of_mem_affine_span ((affine_subspace.le_def' _ _).1 (affine_span_mono k ((set.subset_insert _ _).trans (set.subset_insert _ _))) _ h₁)]; rw [ collinear_insert_iff_of_mem_affine_span ((affine_subspace.le_def' _ _).1 (affine_span_mono k (set.subset_insert _ _)) _ h₂)]; rw [ collinear_insert_iff_of_mem_affine_span h₃],
 exact collinear_pair _ _ _
end
...
by rw [coplanar]; rw [ coplanar]; rw [ vector_span_insert_eq_vector_span h]
...
begin
 ...
 { conv_lhs { rw [←affine_span_coe s]; rw [ direction_affine_span] },
 ...
 rcases (s : set P).eq_empty_or_nonempty with hs | ⟨p₀, hp₀⟩,
 { rw coe_eq_bot_iff at hs,
 rw [hs]; rw [ bot_coe]; rw [ span_empty]; rw [ bot_coe]; rw [ direction_affine_span]; rw [ direction_bot]; rw [ finrank_bot]; rw [ zero_add],
 convert zero_le_one' ℕ,
 rw ←finrank_bot k V,
 convert rfl;
 simp },
 { rw [affine_span_coe]; rw [ direction_affine_span_insert hp₀]; rw [ add_comm],
