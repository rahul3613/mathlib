...
begin
 ext i j,
 rw [to_matrix_apply]; rw [ coord_apply]; rw [ matrix.one_eq_pi_single]; rw [ pi.single_apply],
end
...
begin
 ...
 rw [← finset.univ.affine_combination_eq_linear_combination _ _ hw₁]; rw [ ← finset.univ.affine_combination_eq_linear_combination _ _ hw₂]; rw [ ← finset.univ.map_affine_combination p w₁ hw₁]; rw [ ← finset.univ.map_affine_combination p w₂ hw₂]; rw [ hweq], },
...
begin
 ...
 { rw [eq_top_iff]; rw [ ← b.tot]; rw [ affine_span_le],
 rintros q ⟨i, rfl⟩,
 exact this i, },
 intros i,
 have hAi : ∑ j, A i j = 1,
 ...
 { apply b.ext_elem,
 intros j,
 rw [b.coord_apply]; rw [ finset.univ.map_affine_combination _ _ hAi]; rw [ finset.univ.affine_combination_eq_linear_combination _ _ hAi],
...
begin
 ...
 rw [to_matrix_vec_mul_coords]; rw [ coords_apply]; rw [ ← to_matrix_apply]; rw [ to_matrix_self],
end
...
begin
 ...
 rw [← b.to_matrix_vec_mul_coords b₂]; rw [ matrix.vec_mul_vec_mul]; rw [ matrix.mul_nonsing_inv _ hu]; rw [ matrix.vec_mul_one],
end
...
begin
 have hu := b.is_unit_to_matrix b₂,
 rw matrix.is_unit_iff_is_unit_det at hu,
 rw [← b.to_matrix_inv_vec_mul_to_matrix]; rw [ matrix.det_smul_inv_vec_mul_eq_cramer_transpose _ _ hu],
end
end comm_ring
