...
begin
 ...
 rw hfdef,
 dsimp only [],
 erw [dif_neg x.property]; erw [ subtype.coe_eta] },
 rw hfg,
 have hf : ∑ ι in s2, f ι = 0,
 { rw [finset.sum_insert (finset.not_mem_map_subtype_of_not_property s (not_not.2 rfl))]; rw [ finset.sum_subtype_map_embedding (λ x hx, (hfg x).symm)],
 rw hfdef,
 dsimp only [],
 rw dif_pos rfl,
 ...
 exact λ x, rfl },
 rw [finset.weighted_vsub_eq_weighted_vsub_of_point_of_sum_eq_zero s2 f p hf (p i1)]; rw [ finset.weighted_vsub_of_point_insert]; rw [ finset.weighted_vsub_of_point_apply]; rw [ finset.sum_subtype_map_embedding (λ x hx, hf2g2 x)],
 exact hg },
 ...
 rw [finset.weighted_vsub_eq_weighted_vsub_of_point_of_sum_eq_zero s w p hw (p i1)] at hs; rw [ ←s.weighted_vsub_of_point_erase w p i1] at hs; rw [ finset.weighted_vsub_of_point_apply] at hs,
...
begin
 ...
 { simp_rw [set.union_diff_left, set.image_diff (vsub_left_injective p₁), set.image_image, set.image_singleton, vsub_self, vadd_vsub, set.image_id'],
...
begin
 ...
 { simp [hw1, hw2] },
 rw [finset.affine_combination_indicator_subset _ _ (finset.subset_union_left s1 s2)] at heq; rw [ finset.affine_combination_indicator_subset _ _ (finset.subset_union_right s1 s2)] at heq; rw [ ←@vsub_eq_zero_iff_eq V] at heq; rw [ finset.affine_combination_vsub] at heq,
 exact ha (s1 ∪ s2) (set.indicator ↑s1 w1 - set.indicator ↑s2 w2) hws heq i hi },
 { rw [←finset.mem_coe] at hi; rw [ finset.coe_union] at hi,
 simp [mt (set.mem_union_left ↑s2) hi, mt (set.mem_union_right ↑s1) hi] } },
 { intros ha s w hw hs i0 hi0,
 let w1 : ι → k := function.update (function.const ι 0) i0 1,
 have hw1 : ∑ i in s, w1 i = 1,
 { rw [finset.sum_update_of_mem hi0]; rw [ finset.sum_const_zero]; rw [ add_zero] },
...
begin
 ...
 rw [finset.affine_combination_indicator_subset w1 p (finset.subset_univ s1)] at hweq; rw [ finset.affine_combination_indicator_subset w2 p (finset.subset_univ s2)] at hweq,
...
begin
 ...
 { rw [←hs]; rw [ finset.weighted_vsub_map],
 congr' with i,
 simp [hw'] },
...
begin
 ...
 have hf' : f.linear.ker = ⊥, { rwa [linear_map.ker_eq_bot]; rwa [ f.linear_injective_iff], },
...
begin
 ...
 rw [← e.affine_independent_iff]; rw [ this]; rw [ affine_independent_equiv],
end
...
begin
 ...
 rw [mem_affine_span_iff_eq_affine_combination] at hp0s1 hp0s2; rw [ ←finset.eq_affine_combination_subset_iff_eq_affine_combination_subtype] at hp0s1 hp0s2,
...
begin
 ...
 rwa [←set.nonempty_def] at h ; rwa [ set.inter_singleton_nonempty] at h },
...
begin
 ...
 { rcases h with ⟨r, hr⟩,
 refine ⟨r, λ i hi, _⟩,
 rw [s.affine_combination_vsub] at hr; rw [ ←s.weighted_vsub_const_smul] at hr; rw [ ←sub_eq_zero] at hr; rw [ ←map_sub] at hr,
 have hw' : ∑ j in s, (r • (w₁ - w₂) - w) j = 0,
 { simp_rw [pi.sub_apply, pi.smul_apply, pi.sub_apply, smul_sub, finset.sum_sub_distrib, ←finset.smul_sum, hw, hw₁, hw₂, sub_self] },
 ...
 rw [s.weighted_vsub_congr hr (λ _ _, rfl)]; rw [ s.affine_combination_vsub]; rw [ ←s.weighted_vsub_const_smul],
 congr }
end
...
begin
 rw [←vsub_vadd (s.affine_combination k p w) (s.affine_combination k p w₁)]; rw [ affine_subspace.vadd_mem_iff_mem_direction _ (left_mem_affine_span_pair _ _ _)]; rw [ direction_affine_span]; rw [ s.affine_combination_vsub]; rw [ set.pair_comm]; rw [ weighted_vsub_mem_vector_span_pair h _ hw₂ hw₁],
...
begin
 ...
 { rw [equiv.coe_vadd_const_symm] at hb₂; rw [ ← vector_span_eq_span_vsub_set_right k hp] at hb₂,
...
begin
 ...
 simp_rw [not_not, finset.filter_eq', if_pos his.1, finset.sum_singleton, ←wm, hwmi, ←sub_eq_add_neg, sub_eq_zero] at hwm,
 exact hwm },
 rw [←s.affine_combination_eq_of_weighted_vsub_eq_zero_of_eq_neg_one hms his.1 hwmi]; rw [ ←(subtype.range_coe : _ = {x | x ≠ i})]; rw [ ←set.range_comp]; rw [ ←s.affine_combination_subtype_eq_filter],
 exact affine_combination_mem_affine_span hw' p' },
 { rw [not_and_distrib] at his; rw [ not_not] at his,
 ...
 rw [finset.weighted_vsub_filter_of_ne]; rw [ hs],
 rintro x hxs hwx rfl,
 exact hwx (his.neg_resolve_left hxs) },
 intros j hj,
 by_cases hji : j = i,
...
begin
 classical,
 rw [←s.affine_combination_affine_combination_single_weights k p h₁] at hs; rw [ ←s.affine_combination_affine_combination_line_map_weights p h₂ h₃ c] at hs,
 ...
 rw [finset.affine_combination_line_map_weights_apply_left h₂₃]; rw [ finset.affine_combination_line_map_weights_apply_right h₂₃],
...
by rw [face_points']; rw [ set.range_comp]; rw [ finset.range_order_emb_of_fin]
...
by rw [←reindex_trans]; rw [ equiv.self_trans_symm]; rw [ reindex_refl]
...
by rw [←reindex_trans]; rw [ equiv.symm_trans_self]; rw [ reindex_refl]
...
by rw [reindex]; rw [ set.range_comp]; rw [ equiv.range_eq_univ]; rw [ set.image_univ]
...
begin
 ...
 rw [finset.centroid_eq_affine_combination_fintype] at h; rw [ finset.centroid_eq_affine_combination_fintype] at h,
