...
by rw [midpoint]; rw [ point_reflection_apply]; rw [ line_map_apply]; rw [ vadd_vsub]; rw [ vadd_vadd]; rw [ ← add_smul]; rw [ ← two_mul]; rw [ mul_inv_of_self]; rw [ one_smul]; rw [ vsub_vadd]

lemma midpoint_comm (x y : P) : midpoint R x y = midpoint R y x :=
by rw [midpoint]; rw [ ← line_map_apply_one_sub]; rw [ one_sub_inv_of_two]; rw [ midpoint]

@[simp] lemma affine_equiv.point_reflection_midpoint_right (x y : P) :
 point_reflection R (midpoint R x y) y = x :=
by rw [midpoint_comm]; rw [ affine_equiv.point_reflection_midpoint_left]
...
by rw [midpoint_comm]; rw [ midpoint_vsub_left]
...
by rw [midpoint_comm]; rw [ left_vsub_midpoint]

lemma midpoint_vsub (p₁ p₂ p : P) :
 midpoint R p₁ p₂ -ᵥ p = (⅟2:R) • (p₁ -ᵥ p) + (⅟2:R) • (p₂ -ᵥ p) :=
by rw [←vsub_sub_vsub_cancel_right p₁ p p₂]; rw [ smul_sub]; rw [ sub_eq_add_neg]; rw [ ←smul_neg]; rw [ neg_vsub_eq_vsub_rev]; rw [ add_assoc]; rw [ inv_of_two_smul_add_inv_of_two_smul]; rw [ ←vadd_vsub_assoc]; rw [ midpoint_comm]; rw [ midpoint]; rw [ line_map_apply]

lemma vsub_midpoint (p₁ p₂ p : P) :
 p -ᵥ midpoint R p₁ p₂ = (⅟2:R) • (p -ᵥ p₁) + (⅟2:R) • (p -ᵥ p₂) :=
by rw [←neg_vsub_eq_vsub_rev]; rw [ midpoint_vsub]; rw [ neg_add]; rw [ ←smul_neg]; rw [ ←smul_neg]; rw [ neg_vsub_eq_vsub_rev]; rw [ neg_vsub_eq_vsub_rev]
...
by rw [midpoint_eq_iff]; rw [ point_reflection_self]

@[simp] lemma left_eq_midpoint_iff {x y : P} : x = midpoint R x y ↔ x = y :=
by rw [eq_comm]; rw [ midpoint_eq_left_iff]

@[simp] lemma midpoint_eq_right_iff {x y : P} : midpoint R x y = y ↔ x = y :=
by rw [midpoint_comm]; rw [ midpoint_eq_left_iff]; rw [ eq_comm]

@[simp] lemma right_eq_midpoint_iff {x y : P} : y = midpoint R x y ↔ x = y :=
by rw [eq_comm]; rw [ midpoint_eq_right_iff]

lemma midpoint_eq_midpoint_iff_vsub_eq_vsub {x x' y y' : P} :
 midpoint R x y = midpoint R x' y' ↔ x -ᵥ x' = y' -ᵥ y :=
by rw [← @vsub_eq_zero_iff_eq V]; rw [ midpoint_vsub_midpoint]; rw [ midpoint_eq_iff]; rw [ point_reflection_apply]; rw [ vsub_eq_sub]; rw [ zero_sub]; rw [ vadd_eq_add]; rw [ add_zero]; rw [ neg_eq_iff_eq_neg]; rw [ neg_vsub_eq_vsub_rev]
...
... = x + y : by rw [midpoint_vadd_midpoint]; rw [ vadd_eq_add]; rw [ vadd_eq_add]; rw [ add_comm]; rw [ midpoint_self]
...
by rw [midpoint_eq_iff]; rw [ point_reflection_apply]; rw [ vsub_eq_sub]; rw [ vadd_eq_add]; rw [ sub_add_eq_add_sub]; rw [ ← two_smul R]; rw [ smul_smul]; rw [ mul_inv_of_self]; rw [ one_smul]; rw [ add_sub_cancel']

@[simp] lemma midpoint_self_neg (x : V) :
 midpoint R x (-x) = 0 :=
by rw [midpoint_eq_smul_add]; rw [ add_neg_self]; rw [ smul_zero]
...
by rw [sub_eq_add_neg]; rw [ ← vadd_eq_add]; rw [ ← vadd_eq_add]; rw [ ← midpoint_vadd_midpoint]; simp
...
{ to_fun := f,
 map_zero' := h0,
 map_add' := λ x y,
 calc f (x + y) = f 0 + f (x + y) : by rw [h0]; rw [ zero_add]
 ... = midpoint R' (f 0) (f (x + y)) + midpoint R' (f 0) (f (x + y)) :
 (midpoint_add_self _ _ _).symm
 ... = f (midpoint R x y) + f (midpoint R x y) : by rw [← hm]; rw [ midpoint_zero_add]
 ... = f x + f y : by rw [hm]; rw [ midpoint_add_self] }
