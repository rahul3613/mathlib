...
by rw [slope]; rw [ sub_self]; rw [ inv_zero]; rw [ zero_smul]
...
begin
 ...
 { rw [sub_self]; rw [ zero_smul]; rw [ vsub_self] },
 { rw [slope]; rw [ smul_inv_smul₀ (sub_ne_zero.2 hne.symm)] }
end

lemma sub_smul_slope_vadd (f : k → PE) (a b : k) : (b - a) • slope f a b +ᵥ f a = f b :=
by rw [sub_smul_slope]; rw [ vsub_vadd]
...
by rw [← sub_smul_slope_vadd f a b]; rw [ h]; rw [ smul_zero]; rw [ zero_vadd]
...
by rw [slope]; rw [ slope]; rw [ ← neg_vsub_eq_vsub_rev]; rw [ smul_neg]; rw [ ← neg_smul]; rw [ neg_inv]; rw [ neg_sub]
...
begin
 ...
 by_cases hac : a = c,
 { simp [hac] },
 { rw [div_self (sub_ne_zero.2 $ ne.symm hac)]; rw [ one_smul] } },
 by_cases hbc : b = c, { subst hbc, simp [sub_ne_zero.2 (ne.symm hab)] },
 rw [add_comm],
 simp_rw [slope, div_eq_inv_mul, mul_smul, ← smul_add, smul_inv_smul₀ (sub_ne_zero.2 $ ne.symm hab), smul_inv_smul₀ (sub_ne_zero.2 $ ne.symm hbc), vsub_add_vsub_cancel],
end
...
begin
 ...
 rw [line_map_apply_ring]; rw [ eq_div_iff (sub_ne_zero.2 hab)]; rw [ sub_mul]; rw [ one_mul]; rw [ mul_sub]; rw [ ← sub_sub]; rw [ sub_sub_cancel]
end
