...
by conv_rhs { rw [←vsub_vadd p1 p2]; rw [ map_vadd]; rw [ vadd_vsub] }
...
begin
 ...
 ext,
 rw [coe_const]; rw [ function.const_apply]; rw [ ← @vsub_eq_zero_iff_eq V2]; rw [ ← f.linear_map_vsub]; rw [ h]; rw [ linear_map.zero_apply], },
 { rcases h with ⟨q, rfl⟩,
...
{ to_fun := f,
 linear := f',
 map_vadd' := λ p' v, by rw [h]; rw [ h p']; rw [ vadd_vsub_assoc]; rw [ f'.map_add]; rw [ vadd_vadd] }
...
{ to_fun := f ∘ g,
 ...
 map_vadd' := begin
 intros p v,
 rw [function.comp_app]; rw [ g.map_vadd]; rw [ f.map_vadd],
 refl
 end }
...
by rw [line_map_apply]; rw [ vadd_vsub]
...
by rw [line_map_apply]; rw [ line_map_apply]; rw [ ←@vsub_eq_zero_iff_eq V1]; rw [ vadd_vsub_vadd_cancel_right]; rw [ ←sub_smul]; rw [ smul_eq_zero]; rw [ sub_eq_zero]; rw [ vsub_eq_zero_iff_eq]; rw [ or_comm]; rw [ eq_comm]

@[simp] lemma line_map_eq_left_iff [no_zero_smul_divisors k V1] {p₀ p₁ : P1} {c : k} :
 line_map p₀ p₁ c = p₀ ↔ p₀ = p₁ ∨ c = 0 :=
by rw [←@line_map_eq_line_map_iff k V1]; rw [ line_map_apply_zero]

@[simp] lemma line_map_eq_right_iff [no_zero_smul_divisors k V1] {p₀ p₁ : P1} {c : k} :
 line_map p₀ p₁ c = p₁ ↔ p₀ = p₁ ∨ c = 1 :=
by rw [←@line_map_eq_line_map_iff k V1]; rw [ line_map_apply_one]
...
by { rw [line_map_symm p₀]; rw [ comp_apply], congr, simp [line_map_apply] }
...
by rw [← neg_vsub_eq_vsub_rev]; rw [ line_map_vsub_left]; rw [ ← smul_neg]; rw [ neg_vsub_eq_vsub_rev]

@[simp] lemma line_map_vsub_right (p₀ p₁ : P1) (c : k) :
 line_map p₀ p₁ c -ᵥ p₁ = (1 - c) • (p₀ -ᵥ p₁) :=
by rw [← line_map_apply_one_sub]; rw [ line_map_vsub_left]

@[simp] lemma right_vsub_line_map (p₀ p₁ : P1) (c : k) :
 p₁ -ᵥ line_map p₀ p₁ c = (1 - c) • (p₁ -ᵥ p₀) :=
by rw [← line_map_apply_one_sub]; rw [ left_vsub_line_map]
...
begin
 ...
 rw [← f.linear_map_vsub]; rw [ ← f.linear.map_smul]; rw [ ← f.map_vadd],
