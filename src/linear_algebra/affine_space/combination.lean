...
by rw [weighted_vsub_of_point_apply]; rw [ sum_smul]
...
begin
 ...
 rw [weighted_vsub_of_point_apply]; rw [ weighted_vsub_of_point_apply]; rw [ ←sum_sub_distrib],
 conv_lhs
 { congr,
 skip,
 funext,
...
begin
 erw [weighted_vsub_of_point_apply]; erw [ weighted_vsub_of_point_apply]; erw [ ←@vsub_eq_zero_iff_eq V]; erw [ vadd_vsub_assoc]; erw [ vsub_vadd_eq_vsub_sub]; erw [ ←add_sub_assoc]; erw [ add_comm]; erw [ add_sub_assoc]; erw [ ←sum_sub_distrib],
 conv_lhs
 { congr,
 skip,
 congr,
...
begin
 rw [weighted_vsub_of_point_apply]; rw [ weighted_vsub_of_point_apply],
 apply sum_erase,
...
by rw [sum_smul_vsub_eq_weighted_vsub_of_point_sub]; rw [ weighted_vsub_of_point_apply_const]
...
lemma weighted_vsub_of_point_sdiff_sub [decidable_eq ι] {s₂ : finset ι} (h : s₂ ⊆ s) (w : ι → k)
 (p : ι → P) (b : P) : (s \ s₂).weighted_vsub_of_point p b w - s₂.weighted_vsub_of_point p b (-w) =
 s.weighted_vsub_of_point p b w :=
by rw [map_neg]; rw [ sub_neg_eq_add]; rw [ s.weighted_vsub_of_point_sdiff h]
...
by rw [weighted_vsub_of_point_apply]; rw [ weighted_vsub_of_point_apply]; rw [ ←sum_subtype_eq_sum_filter]
...
begin
 rw [weighted_vsub_of_point_apply]; rw [ weighted_vsub_of_point_apply]; rw [ sum_filter_of_ne],
 intros i hi hne,
 refine h i hi _,
 intro hw,
 simpa [hw] using hne,
...
by rw [weighted_vsub]; rw [ weighted_vsub_of_point_apply_const]; rw [ h]; rw [ zero_smul]
...
by rw [sum_smul_vsub_eq_weighted_vsub_sub]; rw [ s.weighted_vsub_apply_const _ _ h]; rw [ sub_zero]
...
by rw [sum_smul_vsub_eq_weighted_vsub_sub]; rw [ s.weighted_vsub_apply_const _ _ h]; rw [ zero_sub]
...
by rw [affine_combination_apply]; rw [ s.weighted_vsub_of_point_apply_const]; rw [ h]; rw [ one_smul]; rw [ vsub_vadd]
...
by rw [←vadd_eq_add]; rw [ affine_map.map_vadd]; rw [ affine_combination_linear]
...
by rw [←affine_map.linear_map_vsub]; rw [ affine_combination_linear]; rw [ vsub_eq_sub]
...
by classical; rw [attach_affine_combination_of_injective s w (coe : s → P) subtype.coe_injective]; rw [ univ_eq_attach]; rw [ attach_image_coe]
...
begin
 ...
 rw [s.affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one w p h1 (p i)]; rw [ weighted_vsub_of_point_apply],
...
by rw [affine_combination_apply]; rw [ affine_combination_apply]; rw [ weighted_vsub_of_point_indicator_subset _ _ _ h]
...
by rw [sum_smul_vsub_eq_affine_combination_vsub]; rw [ affine_combination_apply_const _ _ _ h]
...
begin
 classical,
 rw [←@vsub_eq_zero_iff_eq V]; rw [ ←hw]; rw [ ←s.affine_combination_sdiff_sub (singleton_subset_iff.2 his)]; rw [ sdiff_singleton_eq_erase]; rw [ ←filter_ne'],
 congr,
...
by rw [affine_combination_apply]; rw [ affine_combination_apply]; rw [ weighted_vsub_of_point_subtype_eq_filter]
...
by rw [affine_combination_apply]; rw [ affine_combination_apply]; rw [ s.weighted_vsub_of_point_filter_of_ne _ _ _ h]
...
begin
 ...
 have b₂ := classical.choice (infer_instance : affine_space V₂ P₂).nonempty,
 rw [s.affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one w p hw b]; rw [ s.affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one w (f ∘ p) hw b₂]; rw [ ← s.weighted_vsub_of_point_vadd_eq_of_sum_eq_one w (f ∘ p) hw (f b) b₂],
...
begin
 rw [weighted_vsub_vsub_weights]; rw [ ←affine_combination_vsub]; rw [ s.affine_combination_affine_combination_single_weights k p hi]; rw [ s.affine_combination_affine_combination_single_weights k p hj]
end
...
by rw [affine_combination_line_map_weights]; rw [ ←weighted_vsub_vadd_affine_combination]; rw [ weighted_vsub_const_smul]; rw [ s.affine_combination_affine_combination_single_weights k p hi]; rw [ s.weighted_vsub_weighted_vsub_vsub_weights k p hj hi]; rw [ affine_map.line_map_apply]
...
by { rw [centroid]; rw [ centroid]; rw [ ← s.attach_affine_combination_coe], congr, ext, simp, }
...
begin
 ...
 rw [centroid_def]; rw [ affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one _ _ _ (sum_centroid_weights_eq_one_of_cast_card_ne_zero _ hc) (p i₁)],
 simp [h] }
end
...
by classical; rw [s.centroid_eq_centroid_image_of_inj_on k hi rfl]; rw [ s₂.centroid_eq_centroid_image_of_inj_on k hi₂ he]
...
begin
 ...
 { rw [vector_span_range_eq_span_range_vsub_right k p i0]; rw [ ←set.image_univ]; rw [ finsupp.mem_span_image_iff_total]; rw [ finset.weighted_vsub_eq_weighted_vsub_of_point_of_sum_eq_zero s w p h (p i0)]; rw [ finset.weighted_vsub_of_point_apply],
 let w' := set.indicator ↑s w,
 have hwx : ∀ i, w' i ≠ 0 → i ∈ s := λ i, set.mem_of_indicator_ne_zero,
 use [finsupp.on_finset s w' hwx, set.subset_univ _],
 rw [finsupp.total_apply]; rw [ finsupp.on_finset_sum hwx],
...
begin
 ...
 { rw [finset.sum_update_of_mem hi1]; rw [ finset.sum_const_zero]; rw [ add_zero] },
 ...
 { rw [direction_affine_span]; rw [ ←hw1s]; rw [ finset.affine_combination_vsub],
...
begin
 ...
 { rw [vector_span_range_eq_span_range_vsub_right k p i0]; rw [ ←set.image_univ]; rw [ finsupp.mem_span_image_iff_total],
 rintros ⟨l, hl, hv⟩,
 ...
 rw [finset.weighted_vsub_eq_weighted_vsub_of_point_of_sum_eq_zero _ w p hw (p i0)]; rw [ finset.weighted_vsub_of_point_apply]; rw [ ←hv]; rw [ finsupp.total_apply]; rw [ finset.sum_insert_zero hz],
 ...
 { simp [hwdef, h] } },
 { resetI,
 rw [set.range_eq_empty]; rw [ vector_span_empty]; rw [ submodule.mem_bot],
 rintro rfl,
 use [∅],
 simp } },
 { rintros ⟨s, w, hw, rfl⟩,
...
begin
 ...
 rw [affine_span_nonempty] at hn; rw [ set.range_nonempty_iff_nonempty] at hn,
 cases hn with i0,
 ...
 rw [direction_affine_span] at hd; rw [ mem_vector_span_iff_eq_weighted_vsub] at hd,
 ...
 { rw [finset.sum_update_of_mem (finset.mem_insert_self _ _)]; rw [ finset.sum_const_zero]; rw [ add_zero] },
 ...
 { rw [add_comm]; rw [ ←finset.weighted_vsub_vadd_affine_combination]; rw [ hw0s]; rw [ hs']; rw [ vsub_vadd] }
end
...
begin
 ...
 rw [s.weighted_vsub_of_point_eq_of_weights_eq p j w w' hww]; rw [ ← s.weighted_vsub_of_point_insert w' p j]; rw [ ← (insert j s).affine_combination_eq_weighted_vsub_of_point_vadd_of_sum_eq_one w' p h₁ (p j)],
