...
iff.trans (by { rw [left_rel_apply]; rw [ sub_eq_add_neg]; rw [ neg_add]; rw [ neg_neg], refl }) neg_mem_iff
...
by { rw [mul_comm]; rw [ ← fintype.card_prod],
...
(R ∙ x).liftq f $ by rw [span_singleton_le_iff_mem]; rw [ linear_map.mem_ker]; rw [ h]
...
by rw [eq_bot_iff]; rw [ map_le_iff_le_comap]; rw [ comap_bot]; rw [ ker_mkq]; exact le_rfl
...
le_antisymm
 ...
 (by rw [map_le_iff_le_comap]; rw [ ← comap_comp]; rw [ liftq_mkq]; exact le_rfl)
...
by rw [ker_liftq]; rw [ le_antisymm h h']; rw [ mkq_map_self]
...
begin
 ...
 rw [hk]; rw [ ←linear_map.map_le_map_iff]; rw [ map_span]; rw [ map_comap_eq]; rw [ set.image_preimage_eq_of_subset h₁],
 exact inf_le_right,
end
...
begin
 ...
 rw [mapq_comp]; rw [ linear_map.comp_apply]
end
...
by rw [←range_le_ker_iff]; rw [ submodule.ker_mkq]; rw [ submodule.range_subtype]
...
begin
 ...
 rw [←submodule.ker_mkq f.range]; rw [ ←h 0 f.range.mkq (eq.trans h₁ (range_mkq_comp _).symm)],
 exact ker_zero
end
