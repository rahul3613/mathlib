...
begin
 ...
 rw [←linear_equiv.coe_to_linear_map]; rw [ ←linear_map.coe_comp],
 ...
 dsimp [h_d, module.dual.eval, linear_equiv.prod_comm],
 simp } },
 rw [this]; rw [ linear_map.coe_prod_map],
...
{ to_fun := λ p, p.1 p.2,
 to_fun_smul := λ a p, by rw [prod.smul_fst]; rw [ prod.smul_snd]; rw [ linear_map.smul_apply]; rw [ linear_map.map_smul]; rw [ smul_eq_mul]; rw [ smul_eq_mul]; rw [ mul_assoc],
 exists_companion' := ⟨bilin_form.dual_prod R M, λ p q, begin
 rw [bilin_form.dual_prod_apply]; rw [ prod.fst_add]; rw [ prod.snd_add]; rw [ linear_map.add_apply]; rw [ map_add]; rw [ map_add]; rw [ add_right_comm _ (q.1 q.2)]; rw [ add_comm (q.1 p.2) (p.1 q.2)]; rw [ ←add_assoc]; rw [ ←add_assoc],
 end⟩ }
