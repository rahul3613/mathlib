...
by rw [polar]; rw [ polar]; rw [ add_comm]; rw [ sub_sub]; rw [ sub_sub]; rw [ add_comm (f x) (f y)]
...
by { rw [←one_smul R x]; rw [ ←add_smul]; rw [ map_smul], norm_num }

@[simp] lemma map_zero : Q 0 = 0 :=
by rw [←@zero_smul R _ _ _ _ (0 : M)]; rw [ map_smul]; rw [ zero_mul]; rw [ zero_mul]
...
lemma map_smul_of_tower [comm_semiring S] [algebra S R] [module S M] [is_scalar_tower S R M]
 (a : S) (x : M) :
 Q (a • x) = (a * a) • Q x :=
by rw [←is_scalar_tower.algebra_map_smul R a x]; rw [ map_smul]; rw [ ←ring_hom.map_mul]; rw [ algebra.smul_def]
...
by rw [←@neg_one_smul R _ _ _ _ x]; rw [ map_smul]; rw [ neg_one_mul]; rw [ neg_neg]; rw [ one_mul]

lemma map_sub (x y : M) : Q (x - y) = Q (y - x) :=
by rw [←neg_sub]; rw [ map_neg]
...
by rw [←neg_one_smul R x]; rw [ polar_smul_left]; rw [ neg_one_mul]

@[simp]
lemma polar_sub_left (x x' y : M) :
 polar Q (x - x') y = polar Q x y - polar Q x' y :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ polar_add_left]; rw [ polar_neg_left]
...
by rw [polar_comm Q x]; rw [ polar_comm Q x]; rw [ polar_comm Q x]; rw [ polar_add_left]

@[simp]
lemma polar_smul_right (a : R) (x y : M) :
 polar Q x (a • y) = a * polar Q x y :=
by rw [polar_comm Q x]; rw [ polar_comm Q x]; rw [ polar_smul_left]

@[simp]
lemma polar_neg_right (x y : M) :
 polar Q x (-y) = -polar Q x y :=
by rw [←neg_one_smul R y]; rw [ polar_smul_right]; rw [ neg_one_mul]

@[simp]
lemma polar_sub_right (x y y' : M) :
 polar Q x (y - y') = polar Q x y - polar Q x y' :=
by rw [sub_eq_add_neg]; rw [ sub_eq_add_neg]; rw [ polar_add_right]; rw [ polar_neg_right]
...
lemma polar_smul_left_of_tower (a : S) (x y : M) :
 polar Q (a • x) y = a • polar Q x y :=
by rw [←is_scalar_tower.algebra_map_smul R a x]; rw [ polar_smul_left]; rw [ algebra.smul_def]

@[simp]
lemma polar_smul_right_of_tower (a : S) (x y : M) :
 polar Q x (a • y) = a • polar Q x y :=
by rw [←is_scalar_tower.algebra_map_smul R a y]; rw [ polar_smul_right]; rw [ algebra.smul_def]
...
{ to_fun := to_fun,
 ...
 λ x y, by rw [bilin_form.coe_fn_mk]; rw [ polar]; rw [ sub_sub]; rw [ add_sub_cancel'_right]⟩ }
...
bilin_form.ext $ λ x y,
 by rw [polar_bilin_apply]; rw [ polar]; rw [ Q.exists_companion.some_spec]; rw [ sub_sub]; rw [ add_sub_cancel']
...
⟨ λ a Q,
 { to_fun := a • Q,
 to_fun_smul := λ b x, by rw [pi.smul_apply]; rw [ map_smul]; rw [ pi.smul_apply]; rw [ mul_smul_comm],
...
def _root_.linear_map.comp_quadratic_form {S : Type*}
 ...
 quadratic_form S M :=
{ to_fun := λ x, f (Q x),
 to_fun_smul := λ b x, by rw [Q.map_smul_of_tower b x]; rw [ f.map_smul]; rw [ smul_eq_mul],
 exists_companion' :=
 let ⟨B, h⟩ := Q.exists_companion in
...
bilin_form.ext $ λ x y,
by rw [associated_to_quadratic_form]; rw [ is_symm.eq h x y]; rw [ ←two_mul]; rw [ ←mul_assoc]; rw [ inv_of_mul_self]; rw [ one_mul]
...
quadratic_form.ext $ λ x,
 ...
 ... = Q x : by rw [← two_mul (Q x)]; rw [ ←mul_assoc]; rw [ inv_of_mul_self]; rw [ one_mul]
...
begin
 rw [associated_apply]; rw [ map_add_self],
...
begin
 ...
 intro h,
 apply hB₁,
 rw [(quadratic_form.ext h : Q = 0)]; rw [ linear_map.map_zero],
end
...
begin
 ext i j,
 rw [to_matrix']; rw [ bilin_form.to_matrix'_apply]; rw [ bilin_form.to_matrix'_apply]; rw [ associated_is_symm]
end
...
begin
 ...
 rw [← submodule.finrank_add_eq_of_is_compl (is_compl_span_singleton_orthogonal hx).symm] at hd; rw [ finrank_span_singleton (ne_zero_of_not_is_ortho_self x hx)] at hd,
 ...
 rw [is_ortho]; rw [ smul_left]; rw [ add_right]; rw [ smul_right]; rw [ div_mul_cancel _ hx]; rw [ add_neg_self]; rw [ mul_zero],
 end),
 refine ⟨b, _⟩,
...
begin
 ext w,
 rw [basis_repr_apply]; rw [ ←@associated_eq_self_apply R₁]; rw [ sum_left]; rw [ weighted_sum_squares_apply],
 refine sum_congr rfl (λ j hj, _),
 rw [←@associated_eq_self_apply R₁]; rw [ sum_right]; rw [ sum_eq_single_of_mem j hj],
 { rw [smul_left]; rw [ smul_right]; rw [ smul_eq_mul], ring },
 { intros i _ hij,
 rw [smul_left]; rw [ smul_right]; rw [ show associated_hom R₁ Q (v j) (v i) = 0]; rw [ from hv₂ hij.symm]; rw [ mul_zero]; rw [ mul_zero] },
end
