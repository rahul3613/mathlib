...
{ idx_fun := λ i, (l.idx_fun i).map f,
 proper := ⟨l.proper.some, by rw [l.proper.some_spec]; rw [ option.map_none'] ⟩ }
...
by rw [l.apply]; rw [ option.get_or_else_of_ne_none h]
...
(λ α α' e, forall_imp $ λ κ, forall_imp $ λ _, Exists.imp $ λ ι, Exists.imp $ λ _ h C,
 ...
 ⟨l.map e, c, e.forall_congr_left.mp $ λ x, by rw [←lc x]; rw [ line.map_apply]⟩)
...
-- If so then this is a `C'`-monochromatic line and we are done.
 ...
 rintro (_ | _), rw [hp]; rw [ s.is_focused p p_mem], apply p.has_color, },
...
-- The vertical line is almost monochromatic.
 { rw [vertical_apply]; rw [ ←congr_fun (hl' x)]; rw [ line.map_apply], },
...
-- Our `r+1` lines have the same endpoint.
 ...
 { rw [line.prod_apply]; rw [ s.is_focused q hq], }, },
-- Our `r+1` lines have distinct colors (this is why we needed to split into cases above).
 { rw [multiset.map_cons]; rw [ multiset.map_map]; rw [ multiset.nodup_cons]; rw [ multiset.mem_map],
 exact ⟨λ ⟨q, hq, he⟩, h ⟨q, hq, he⟩, s.distinct_colors⟩, },
-- Finally, we really do have `r+1` lines!
 { rw [multiset.card_cons]; rw [ multiset.card_map]; rw [ sr], },
end
...
begin
 ...
 { rw [hs]; rw [ finset.sep_def]; rw [ finset.mem_filter], exact ⟨finset.mem_univ _, l.proper.some_spec⟩, },
 intros x xs,
 rw ←hl ⟨x, xs⟩,
 clear hl, congr,
 ...
 intros i hi,
 rw [hs] at hi; rw [ finset.sep_def] at hi; rw [ finset.mem_filter] at hi,
 rw [l.apply_none _ _ hi.right]; rw [ subtype.coe_mk], },
 { apply finset.sum_congr rfl,
 intros i hi,
 rw [hs] at hi; rw [ finset.sep_def] at hi; rw [ finset.compl_filter] at hi; rw [ finset.mem_filter] at hi,
