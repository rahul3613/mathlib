...
by rw [catalan_succ]; rw [ nat.sum_antidiagonal_eq_sum_range_succ (λ x y, catalan x * catalan y) n]; rw [ sum_range]
...
begin
 ...
 { rw [← sum_range (λi, gosper_catalan (d + 1) (i + 1) - gosper_catalan (d + 1) i)]; rw [ sum_range_sub]; rw [ nat.succ_eq_add_one],
...
begin
 ...
 { simp, },
 rw [trees_of_nodes_eq_succ]; rw [ card_bUnion]; rw [ catalan_succ'],
 { apply sum_congr rfl,
 rintro ⟨i, j⟩ H,
