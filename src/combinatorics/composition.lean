...
by conv_rhs { rw [← c.blocks_sum]; rw [ ← of_fn_blocks_fun]; rw [ sum_of_fn] }
...
begin
 conv_rhs { rw [← c.blocks_sum]; rw [ ← sum_take_add_sum_drop _ i] },
...
begin
 rw [tsub_lt_iff_right]; rw [ add_comm]; rw [ ← size_up_to_succ'],
...
begin
 ...
 { rw [tsub_lt_iff_left]; rw [ ← size_up_to_succ'],
 { exact h.2 },
 { exact h.1 } },
 { rw fin.ext_iff,
...
begin
 ...
 rcases c with ⟨n, c⟩,
 rcases c' with ⟨n', c'⟩,
 have : n = n', by { rw [← c.blocks_sum]; rw [ ← c'.blocks_sum]; rw [ H] },
 induction this,
...
begin
 ...
 have : c.blocks.length = n, by { conv_rhs { rw [← c.blocks_sum]; rw [ A] }, simp },
...
begin
 ...
 rw [← of_fn_blocks_fun] at hi; rw [ mem_of_fn c.blocks_fun] at hi; rw [ set.mem_range] at hi,
...
begin
 ...
 rw IH, {simp [this]},
 rwa [length_drop]; rwa [ le_tsub_iff_left this]
end
...
begin
 ...
 cases i; simp [IH],
 rw [add_comm n]; rw [ drop_add]; rw [ drop_take],
end
...
begin
 ...
 rw IH, {simp},
 rwa [length_drop]; rwa [ ← h]; rwa [ add_tsub_cancel_left]
end
...
begin
 rw [boundary]; rw [ finset.order_emb_of_fin_zero rfl c.card_boundaries_pos],
...
begin
 ...
 have : c.boundary ⟨i, hi⟩ = j, by rwa [fin.ext_iff]; rwa [ ← c.blocks_partial_sum hi],
 exact this.symm }
end
...
begin
 ...
 rw [d.blocks_partial_sum i_lt]; rw [ composition_as_set.boundary]; rw [ ← composition.size_up_to]; rw [ B]; rw [ A]; rw [ c.order_emb_of_fin_boundaries]
end
