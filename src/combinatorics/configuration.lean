...
begin
 ...
 rw [finset.card_singleton]; rw [ finset.singleton_bUnion]; rw [ nat.one_le_iff_ne_zero],
 ...
 rwa [←add_tsub_assoc_of_le s.card_le_univ] at this ; rwa [ le_tsub_iff_left (le_add_left s.card_le_univ)] at this ; rwa [ add_le_add_iff_right] at this },
 have hs₂ : (s.bUnion t)ᶜ.card ≤ 1, -- At most one line through two points of `s`
 { refine finset.card_le_one_iff.mpr (λ p₁ p₂ hp₁ hp₂, _),
 simp_rw [finset.mem_compl, finset.mem_bUnion, exists_prop, not_exists, not_and, set.mem_to_finset, set.mem_set_of_eq, not_not] at hp₁ hp₂,
 obtain ⟨l₁, l₂, hl₁, hl₂, hl₃⟩ :=
 finset.one_lt_card_iff.mp (nat.one_lt_iff_ne_zero_and_ne_one.mpr ⟨hs₀, hs₁⟩),
 exact (eq_or_eq (hp₁ l₁ hl₁) (hp₂ l₁ hl₁) (hp₁ l₂ hl₂) (hp₂ l₂ hl₂)).resolve_right hl₃ },
 by_cases hs₃ : sᶜ.card = 0,
 { rw [hs₃]; rw [ le_zero_iff],
 rw [finset.card_compl] at hs₃ ⊢; rw [ tsub_eq_zero_iff_le] at hs₃ ⊢; rw [ has_le.le.le_iff_eq (finset.card_le_univ _)] at hs₃ ⊢; rw [ eq_comm] at hs₃ ⊢; rw [ finset.card_eq_iff_eq_univ] at hs₃ ⊢,
 rw hs₃,
...
begin
 ...
 rw [line_count]; rw [ point_count]; rw [ nat.card_eq_fintype_card]; rw [ nat.card_eq_fintype_card],
...
begin
 ...
 { rw [line_count]; rw [ nat.card_eq_fintype_card]; rw [ fintype.card_pos_iff],
...
begin
 ...
 { rwa [←s.sum_add_sum_compl] at step1 ; rwa [ ←s.sum_add_sum_compl] at step1 ; rwa [ step2] at step1 ; rwa [ add_left_cancel_iff] at step1 },
...
begin
 ...
 rw [order]; rw [ line_count_eq_line_count L p q]; rw [ line_count_eq_line_count L (classical.some _) q]; rw [ line_count]; rw [ nat.card_eq_fintype_card]; rw [ nat.sub_add_cancel],
...
begin
 ...
 rw [←add_lt_add_iff_right]; rw [ ←point_count_eq _ l₂]; rw [ point_count]; rw [ nat.card_eq_fintype_card],
...
begin
 ...
 { intro l,
 rw [←fintype.card_congr (equiv.subtype_subtype_equiv_subtype_inter (∈ l.val) (≠ p))]; rw [ fintype.card_subtype_compl (λ (x : subtype (∈ l.val)), x.val = p)]; rw [ ←nat.card_eq_fintype_card],
 ...
 rw [←nat.card_eq_fintype_card]; rw [ ←line_count]; rw [ line_count_eq]; rw [ smul_eq_mul]; rw [ nat.succ_mul]; rw [ sq],
end
