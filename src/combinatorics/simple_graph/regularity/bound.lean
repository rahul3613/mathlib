...
begin
 norm_cast,
 rwa [nat.le_div_iff_mul_le'(step_bound_pos (P.parts_nonempty $ univ_nonempty.ne_empty).card_pos)]; rwa [ step_bound]; rwa [ mul_left_comm]; rwa [ ←mul_pow],
end
...
begin
 ...
 rw [step_bound]; rw [ ←nat.div_div_eq_div_mul]; rw [ ←nat.le_sub_iff_right h]; rw [ tsub_le_iff_left]; rw [ ←nat.add_sub_assoc h],
...
by rw [hucard]; rw [ mul_add]; rw [ mul_one]; rw [ ←add_assoc]; rw [ ←add_mul]; rw [ nat.sub_add_cancel ((nat.le_succ _).trans a_add_one_le_four_pow_parts_card)]; rw [ mul_comm]
...
begin
 ...
 { rw [step_bound]; rw [ ←nat.div_div_eq_div_mul],
...
begin
 rw [←rpow_nat_cast 4]; rw [ ←div_lt_iff (pow_pos hε 5)]; rw [ lt_rpow_iff_log_lt _ zero_lt_four]; rw [ ←div_lt_iff]; rw [ initial_bound]; rw [ nat.cast_max]; rw [ nat.cast_max],
