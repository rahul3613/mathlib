...
begin
 ...
 { intros x hx,
 rw [←bUnion_filter_atomise hX (G.nonuniform_witness_subset h₂)]; rw [ star]; rw [ mem_sdiff]; rw [ mem_bUnion] at
 hx,
...
begin
 ...
 = (2 * 2)^P.parts.card * m/U.card : by rw [mul_pow]; rw [ ←mul_div_assoc]; rw [ mul_assoc]
 ...
 ... = 2^P.parts.card * (ε * (ε / 10)) : by rw [mul_div_assoc]; rw [ sq]; rw [ mul_div_assoc] },
 calc
 ...
 * (G.nonuniform_witness ε U V).card
 ...
 norm_cast,
 rw [sub_le_comm]; rw [ ←cast_sub (card_le_of_subset $ bUnion_star_subset_nonuniform_witness hP G ε hU V)]; rw [ ←card_sdiff (bUnion_star_subset_nonuniform_witness hP G ε hU V)]; rw [ cast_le],
...
begin
 unfold chunk,
 split_ifs,
 { rw [card_parts_equitabilise _ _ hm]; rw [ tsub_add_cancel_of_le],
 exact le_of_lt a_add_one_le_four_pow_parts_card },
 { rw [card_parts_equitabilise _ _ hm]; rw [ tsub_add_cancel_of_le a_add_one_le_four_pow_parts_card] }
end
...
begin
 ...
 { rw [←(of_subset _ h𝒜 rfl).sum_card_parts]; rw [ of_subset_parts]; rw [ ←cast_mul]; rw [ cast_le],
...
begin
 rw [sup_eq_bUnion]; rw [ mul_div_assoc']; rw [ le_div_iff m_pos]; rw [ mul_right_comm],
...
begin
 ...
 { positivity },
 rw [add_sq]; rw [ one_pow]; rw [ add_assoc]; rw [ add_le_add_iff_left]; rw [ mul_one]; rw [ ←le_sub_iff_add_le']; rw [ div_eq_mul_one_div _ (49:ℝ)]; rw [ mul_div_left_comm (2:ℝ)]; rw [ ←mul_sub_left_distrib]; rw [ div_pow]; rw [ div_le_iff (show (0:ℝ) < 100 ^ 2, by norm_num)]; rw [ mul_assoc]; rw [ sq],
 refine mul_le_mul_of_nonneg_left _ (by positivity),
...
begin
 ...
 positivity },
 refine mul_pos (mul_pos _ _) (mul_pos _ _); rw [cast_pos]; rw [ finset.card_pos],
...
begin
 ...
 { rw [sq]; rw [ mul_mul_mul_comm]; rw [ mul_comm (_/(m:ℝ))]; rw [ mul_comm (_/(m:ℝ))],
 ...
 apply mul_pos; rw [finset.card_pos]; rw [ sup_eq_bUnion]; rw [ bUnion_nonempty],
...
begin
 ...
 { rw [bUnion_parts]; rw [ bUnion_parts] },
 { rw [card_chunk (m_pos hPα).ne']; rw [ card_chunk (m_pos hPα).ne']; rw [ ←cast_mul]; rw [ ←mul_pow]; rw [ cast_pow],
 norm_cast } },
 ...
 rw [mul_right_comm]; rw [ mul_div_left_comm]; rw [ div_eq_mul_inv (ε ^ 5)]; rw [ show (2:ℝ)/50 = 25⁻¹]; rw [ by norm_num],
...
begin
 ...
 : by rw [mul_assoc]; rw [ mul_assoc]; rw [ mul_div_left_comm]
 ...
 begin
 rw [mul_assoc]; rw [ mul_comm ((4:ℝ)^P.parts.card)]; rw [ ←div_div]; rw [ ←mul_div_assoc]; rw [ ←mul_comm_div],
 refine mul_le_of_le_one_right (by positivity) _,
...
private lemma edge_density_star_not_uniform [nonempty α]
 ...
 G.edge_density ab.1 ab.2)/16 ^ P.parts.card| :=
begin
 rw [(show (16:ℝ) = 4 ^ 2, by norm_num)]; rw [ pow_right_comm]; rw [ sq ((4:ℝ)^_)],
...
calc
 ...
 rw [(show (16 : ℝ) = 4 ^ 2, by norm_num)]; rw [ pow_right_comm]; rw [ sq ((4:ℝ)^_)]; rw [ ←_root_.div_mul_div_comm]; rw [ mul_assoc],
 ...
 { simp_rw [card_product, card_chunk (m_pos hPα).ne', ←mul_pow, cast_pow, mul_pow, div_pow, ←mul_assoc] at this,
 norm_num at this,
 exact this },
...
begin
 ...
 rw [card_product]; rw [ cast_mul]; rw [ card_chunk (m_pos hPα).ne']; rw [ card_chunk (m_pos hPα).ne']; rw [ ←cast_mul]; rw [ ←mul_pow];
 norm_cast,
end
