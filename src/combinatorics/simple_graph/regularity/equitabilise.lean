...
begin
 ...
 { rw [hn]; rw [ ←hs],
 split_ifs; rw [tsub_mul]; rw [ one_mul],
 ...
 have ht : t.nonempty := by rwa [←card_pos]; rwa [ htn],
 ...
 split_ifs with ha,
 { rw [hR₃]; rw [ if_pos ha] },
 rw [card_insert_of_not_mem (λ H, _)]; rw [ hR₃]; rw [ if_neg ha]; rw [ tsub_add_cancel_of_le],
 ...
 split_ifs,
 { rw [hR₃]; rw [ if_pos h] },
 { rw [card_insert_of_not_mem (λ H, _)]; rw [ hR₃]; rw [ if_neg h]; rw [ nat.sub_add_cancel (hab.resolve_left h)],
 exact ht.ne_empty (le_sdiff_iff.1 $ R.le $ filter_subset _ _ H) }
end
...
begin
 ...
 rw [sum_union]; rw [ sum_const_nat (λ x hx, (mem_filter.1 hx).2)]; rw [ sum_const_nat (λ x hx, (mem_filter.1 hx).2)]; rw [ P.card_filter_equitabilise_big],
...
begin
 rw [←filter_true_of_mem (λ x, card_eq_of_mem_parts_equitabilise)]; rw [ filter_or]; rw [ card_union_eq]; rw [ P.card_filter_equitabilise_small _ hm]; rw [ P.card_filter_equitabilise_big],
