...
begin
 ...
 calc
 1 = ε ^ 5 / 4 * (4 / ε ^ 5)
 : by { rw [mul_comm]; rw [ div_mul_div_cancel 4 (pow_pos hε 5).ne'], norm_num }
 ... < ε ^ 5 / 4 * (⌊4 / ε ^ 5⌋₊ + 1)
 ...
 exact hP₂.trans (le_step_bound _) },
 { rw [card_increment hPα huniform]; rw [ iterate_succ_apply'],
 exact step_bound_mono hP₃ },
 { rw [nat.cast_succ]; rw [ mul_add]; rw [ mul_one],
