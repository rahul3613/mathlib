...
lemma apply_ne_zero_iff [mul_zero_one_class α] [nontrivial α] (h : is_adj_matrix A) (i j : V) :
 ¬ A i j = 0 ↔ A i j = 1 :=
by rw [←apply_ne_one_iff h]; rw [ not_not]
...
by rw [mul_vec]; rw [ adj_matrix_dot_product]
...
begin
 rw [← dot_product_adj_matrix]; rw [ vec_mul],
 refine congr rfl _, ext,
 rw [← transpose_apply (adj_matrix α G) x v]; rw [ transpose_adj_matrix],
end
