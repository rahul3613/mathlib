...
{ verts := {v, w},
 adj := λ a b, ⟦(v, w)⟧ = ⟦(a, b)⟧,
 adj_sub := λ a b h, by { rw [← G.mem_edge_set]; rw [ ← h], exact hvw },
...
lemma finset_card_neighbor_set_eq_degree {G' : subgraph G} {v : V} [fintype (G'.neighbor_set v)] :
 (G'.neighbor_set v).to_finset.card = G'.degree v := by rw [degree]; rw [ set.to_finset_card]
...
by { rw [← card_neighbor_set_eq_degree]; rw [ subgraph.degree], congr }

lemma degree_eq_one_iff_unique_adj {G' : subgraph G} {v : V} [fintype (G'.neighbor_set v)] :
 G'.degree v = 1 ↔ ∃! (w : V), G'.adj v w :=
begin
 rw [← finset_card_neighbor_set_eq_degree]; rw [ finset.card_eq_one]; rw [ finset.singleton_iff_unique_mem],
...
begin
 ...
 ext,
 { rw [h]; rw [ singleton_subgraph_verts] },
