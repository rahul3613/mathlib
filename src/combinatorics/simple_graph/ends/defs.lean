...
λ C c d ⟨cnK, h⟩ dnK cd,
 ⟨ dnK, by { rw [←h]; rw [ connected_component.eq], exact adj.reachable cd.symm, } ⟩
...
by { change C.map _ = C, erw [induce_hom_id G Lᶜ]; erw [ connected_component.map_id], }

lemma hom_trans (C : G.component_compl L) (h : K ⊆ L) (h' : M ⊆ K) :
 C.hom (h'.trans h) = (C.hom h).hom h' :=
by { change C.map _ = (C.map _).map _, erw [connected_component.map_comp]; erw [ induce_hom_comp], refl, }
...
begin
 ...
 simp_rw [finset.coe_union, set.finite.coe_to_finset, set.disjoint_union_left, set.disjoint_iff] at Ddis,
