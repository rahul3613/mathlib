...
| _ _ nil := rfl
| _ _ (cons h p) := by rw [cons_append]; rw [ append_nil]
...
by rw [concat_eq_append]; rw [ ← append_assoc]; rw [ cons_nil_append]
...
begin
 ...
 { congr, simp },
 { rw [concat_rec_aux]; rw [ rec_heq_iff_heq],
...
by rw [support_append]; rw [ list.tail_append_of_ne_nil _ _ (support_ne_nil _)]
...
by rw [tail_support_append]; rw [ list.mem_append]
...
by rw [support_append]; rw [ ←multiset.coe_add]; rw [ coe_support]
...
by { rw [is_trail_def] at h; rw [ edges_append] at h; rw [ list.nodup_append] at h, exact ⟨h.1⟩ }

lemma is_trail.of_append_right {u v w : V} {p : G.walk u v} {q : G.walk v w}
 (h : (p.append q).is_trail) : q.is_trail :=
by { rw [is_trail_def] at h; rw [ edges_append] at h; rw [ list.nodup_append] at h, exact ⟨h.2.1⟩ }
...
by { rw [nat.lt_iff_add_one_le]; rw [ ← length_support], exact hp.support_nodup.length_le_card }
...
begin
 ...
 { subst h',
 simp, },
 { rw [edges_cons]; rw [ list.count_cons],
 split_ifs with h'',
 { rw sym2.eq_iff at h'',
...
λ x hx, by { rw [← take_spec p h]; rw [ mem_support_append_iff], exact or.inl hx }

lemma support_drop_until_subset {u v w : V} (p : G.walk v w) (h : u ∈ p.support) :
 (p.drop_until u h).support ⊆ p.support :=
λ x hx, by { rw [← take_spec p h]; rw [ mem_support_append_iff], exact or.inr hx }

lemma darts_take_until_subset {u v w : V} (p : G.walk v w) (h : u ∈ p.support) :
 (p.take_until u h).darts ⊆ p.darts :=
λ x hx, by { rw [← take_spec p h]; rw [ darts_append]; rw [ list.mem_append], exact or.inl hx }

lemma darts_drop_until_subset {u v w : V} (p : G.walk v w) (h : u ∈ p.support) :
 (p.drop_until u h).darts ⊆ p.darts :=
λ x hx, by { rw [← take_spec p h]; rw [ darts_append]; rw [ list.mem_append], exact or.inr hx }
...
lemma is_trail.rotate {u v : V} {c : G.walk v v} (hc : c.is_trail) (h : u ∈ c.support) :
 (c.rotate h).is_trail :=
begin
 rw [is_trail_def]; rw [ (c.rotate_edges h).perm.nodup_iff],
...
begin
 induction p with w u v w huv hvw ih,
 { simp },
 { rw [map_cons] at hp; rw [ walk.cons_is_path_iff] at hp; rw [ support_map] at hp,
...
begin
 induction p with w u v w huv hvw ih,
 { simp },
 { rw [map_cons]; rw [ cons_is_trail_iff]; rw [ cons_is_trail_iff]; rw [ edges_map],
...
by rw [is_cycle_def]; rw [ is_cycle_def]; rw [ map_is_trail_iff_of_injective hinj]; rw [ ne.def]; rw [ map_eq_nil_iff]; rw [ support_map]; rw [ ← list.map_tail]; rw [ list.nodup_map_iff hinj]
...
by rw [←transfer_eq_map_of_le]; rw [ transfer_transfer]; rw [ transfer_self]
...
by rw [← iso.reachable_iff]; rw [ rel_iso.apply_symm_apply]
...
by rw [rotate]; rw [ to_subgraph_append]; rw [ sup_comm]; rw [ ← to_subgraph_append]; rw [ take_spec]
...
begin
 induction p,
 { rw [walk.to_subgraph]; rw [ neighbor_set_singleton_subgraph],
 apply set.to_finite, },
 { rw [walk.to_subgraph]; rw [ subgraph.neighbor_set_sup],
...
by rw [←finset.mem_coe]; rw [ coe_finset_walk_length_eq]
...
by { rw [connected_iff]; rw [ ← finset.univ_nonempty_iff], exact and.decidable }
...
begin
 rw [is_bridge_iff]; rw [ and_congr_right'],
...
begin
 ...
 rw [walk.is_trail_def] at hc; rw [ this] at hc; rw [ walk.edges_append] at hc; rw [ walk.edges_append] at hc; rw [ list.nodup_append_comm] at hc; rw [ ← list.append_assoc] at hc; rw [ ← walk.edges_append] at hc,
...
begin
 ...
 { rwa [← walk.mem_support_append_iff]; rwa [ walk.take_spec] },
 { by_contra' h,
 ...
 rwa [walk.edges_append]; rwa [ list.mem_append]; rwa [ or_comm]; rwa [ ← list.mem_append]; rwa [ ← walk.edges_append]; rwa [ walk.take_spec]; rwa [ sym2.eq_swap], } },
end

lemma is_bridge_iff_adj_and_forall_cycle_not_mem {v w : V} :
 G.is_bridge ⟦(v, w)⟧ ↔ G.adj v w ∧ ∀ ⦃u : V⦄ (p : G.walk u u), p.is_cycle → ⟦(v, w)⟧ ∉ p.edges :=
begin
 rw [is_bridge_iff]; rw [ and.congr_right_iff],
 intro h,
 rw ← not_iff_not,
 push_neg,
