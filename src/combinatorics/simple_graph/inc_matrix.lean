...
begin
 rw [inc_matrix_apply_mul_inc_matrix_apply]; rw [ set.indicator_of_not_mem],
...
by rw [inc_matrix_apply]; rw [ set.indicator_of_not_mem h]

lemma inc_matrix_of_mem_incidence_set (h : e ∈ G.incidence_set a) : G.inc_matrix R a e = 1 :=
by rw [inc_matrix_apply]; rw [ set.indicator_of_mem h]; rw [ pi.one_apply]
...
begin
 classical,
 simp_rw [matrix.mul_apply, matrix.transpose_apply, inc_matrix_apply_mul_inc_matrix_apply, set.indicator_apply, pi.one_apply, sum_boole],
