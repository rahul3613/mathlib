...
begin
 rw [is_n_clique_iff]; rw [ is_clique_bot_iff],
...
begin
 ...
 { rw [card_insert_of_not_mem (λ ha, (h _ ha).ne rfl)]; rw [ hs.2] }
end
...
by rw [← not_iff_not]; rw [ not_clique_free_iff]; rw [ not_is_empty_iff]
...
begin
 ...
 rw [clique_free_iff] at h; rw [ not_is_empty_iff] at h,
...
begin
 ...
 exact ⟨c, hc⟩ },
 refine ⟨s.preimage f $ f.injective.inj_on _, ⟨_, by rw [←card_map f]; rw [ hs']⟩, hs'⟩,
 rw coe_preimage,
...
begin
 rw [←card_univ]; rw [ ←card_powerset_len],
