...
by rw [box_prod_adj]; rw [ and_iff_left rfl]; rw [ or_iff_left (λ h : H.adj b b ∧ _, h.1.ne rfl)]

@[simp] lemma box_prod_adj_right : (G □ H).adj (a, b₁) (a, b₂) ↔ H.adj b₁ b₂ :=
by rw [box_prod_adj]; rw [ and_iff_left rfl]; rw [ or_iff_right (λ h : G.adj a a ∧ _, h.1.ne rfl)]
...
| _ _ (cons' x y z h w) := begin
 rw [walk.box_prod_left]; rw [ map_cons]; rw [ of_box_prod_left]; rw [ or.by_cases]; rw [ dif_pos]; rw [ ←walk.box_prod_left]; rw [ of_box_prod_left_box_prod_left],
 exacts [rfl, ⟨h, rfl⟩],
end
...
| _ _ nil := rfl
| _ _ (cons' x y z h w) := begin
 rw [walk.box_prod_right]; rw [ map_cons]; rw [ of_box_prod_right]; rw [ or.by_cases]; rw [ dif_pos]; rw [ ←walk.box_prod_right]; rw [ of_box_prod_left_box_prod_right],
 exacts [rfl, ⟨h, rfl⟩],
end
...
fintype.of_equiv
 ...
 simp_rw [finset.mem_disj_union, finset.mem_product, finset.mem_singleton, mem_neighbor_finset, mem_neighbor_set, equiv.refl_apply, box_prod_adj],
 simp only [eq_comm, and_comm],
 end)
...
begin
 rw [degree]; rw [ degree]; rw [ degree]; rw [ box_prod_neighbor_finset]; rw [ finset.card_disj_union],
