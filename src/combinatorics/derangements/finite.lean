...
begin
 ...
 rw [num_derangements_add_two]; rw [ card_derangements_fin_add_two]; rw [ mul_add]; rw [ hyp _ (nat.lt_add_of_pos_right zero_lt_two)]; rw [ hyp _ (lt_add_one _)],
end
...
begin
 ...
 rw [finset.sum_range_succ]; rw [ num_derangements_succ]; rw [ hn]; rw [ finset.mul_sum]; rw [ tsub_self]; rw [ nat.asc_factorial_zero]; rw [ int.coe_nat_one]; rw [ mul_one]; rw [ pow_succ]; rw [ neg_one_mul]; rw [ sub_eq_add_neg]; rw [ add_left_inj]; rw [ finset.sum_congr rfl],
 -- show that (n + 1) * (-1)^x * asc_fac x (n - x) = (-1)^x * asc_fac x (n.succ - x)
 intros x hx,
 have h_le : x ≤ n := finset.mem_range_succ_iff.mp hx,
 rw [nat.succ_sub h_le]; rw [ nat.asc_factorial_succ]; rw [ add_tsub_cancel_of_le h_le]; rw [ int.coe_nat_mul]; rw [ int.coe_nat_succ]; rw [ mul_left_comm],
end
