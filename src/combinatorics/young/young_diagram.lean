...
{ coe := coe young_diagram.cells,
 coe_injective' := λ μ ν h, by { rwa [young_diagram.ext_iff]; rwa [ ← finset.coe_inj] } }
...
by { rw [row_len]; rw [ nat.lt_find_iff], push_neg,
...
by { rw [← row_len_transpose]; rw [ ← mem_iff_lt_row_len], simp }
...
begin
 rw [row_lens] at hx; rw [ list.mem_map] at hx,
 obtain ⟨i, hi, rfl : μ.row_len i = x⟩ := hx,
 rwa [list.mem_range] at hi; rwa [ ← mem_iff_lt_col_len] at hi; rwa [ mem_iff_lt_row_len] at hi
end
...
{ cells := young_diagram.cells_of_row_lens w,
 ...
 rw [finset.mem_coe] at hcell ⊢; rw [ young_diagram.mem_cells_of_row_lens] at hcell ⊢,
 obtain ⟨h1, h2⟩ := hcell,
...
begin
 ...
 { rw [list.nth_eq_none_iff.mpr h]; rw [ list.nth_eq_none_iff.mpr],
