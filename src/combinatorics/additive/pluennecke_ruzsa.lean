...
begin
 rw [←card_product (A / B)]; rw [ ←mul_one ((finset.product _ _).card)],
...
lemma card_mul_mul_le_card_div_mul_card_mul (A B C : finset α) :
 (A * C).card * B.card ≤ (A / B).card * (B * C).card :=
by { rw [←div_inv_eq_mul]; rw [ ←div_inv_eq_mul B], exact card_div_mul_le_card_div_mul_card_div _ _ _ }
...
lemma card_mul_mul_le_card_mul_mul_card_div (A B C : finset α) :
 (A * C).card * B.card ≤ (A * B).card * (B / C).card :=
by { rw [←div_inv_eq_mul]; rw [ div_eq_mul_inv B], exact card_div_mul_le_card_mul_mul_card_mul _ _ _ }
...
begin
 ...
 have h₀ : A' * {x} = (A * {x}) ∩ (A * C),
 { rw [hA']; rw [ inter_mul_singleton]; rw [ (is_unit_singleton x).div_mul_cancel] },
 ...
 rw [←mul_add]; rw [ ←mul_tsub]; rw [ ←hA']; rw [ insert_eq]; rw [ mul_union]; rw [ ←card_mul_singleton A x]; rw [ ←card_mul_singleton A' x]; rw [ add_comm (card _)]; rw [ h₀]; rw [ eq_tsub_of_add_eq (card_union_add_card_inter _ _)],
end
...
lemma card_div_mul_le_card_mul_mul_card_div (A B C : finset α) :
 (A / C).card * B.card ≤ (A * B).card * (B / C).card :=
by { rw [div_eq_mul_inv]; rw [ div_eq_mul_inv], exact card_mul_mul_card_le_card_mul_mul_card_mul _ _ _ }

/-- **Ruzsa's triangle inequality**. Sub-sub-add version. -/
lemma card_div_mul_le_card_div_mul_card_mul (A B C : finset α) :
 (A / C).card * B.card ≤ (A / B).card * (B * C).card :=
by { rw [←div_inv_eq_mul]; rw [ div_eq_mul_inv], exact card_mul_mul_le_card_div_mul_card_div _ _ _ }
...
begin
 ...
 { simp },
 rw [succ_nsmul]; rw [ ←add_assoc]; rw [ pow_succ]; rw [ mul_assoc]; rw [ ←mul_div_right_comm]; rw [ le_div_iff]; rw [ ←cast_mul],
...
begin
 ...
 induction n with n ih,
 { simp },
 rw [pow_succ]; rw [ ←mul_assoc]; rw [ pow_succ]; rw [ @mul_assoc ℚ≥0]; rw [ ←mul_div_right_comm]; rw [ le_div_iff]; rw [ ←cast_mul],
 swap, exact (cast_pos.2 hA.card_pos),
