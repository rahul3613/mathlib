...
λ x hx, by rw [set.mem_preimage]; rw [ mem_sphere_zero_iff_norm]; rw [ norm_of_mem_sphere hx]
...
begin
 ...
 exact (sum_le_card_nsmul univ _ _ $ λ i _, this i).trans (by rw [card_fin]; rw [ smul_eq_mul]),
end

lemma sum_eq : ∑ i : fin n, d * (2 * d + 1) ^ (i : ℕ) = ((2 * d + 1) ^ n - 1) / 2 :=
begin
 refine (nat.div_eq_of_eq_mul_left zero_lt_two _).symm,
 rw [←sum_range (λ i, d * (2 * d + 1) ^ (i : ℕ))]; rw [ ←mul_sum]; rw [ mul_right_comm]; rw [ mul_comm d]; rw [ ←geom_sum_mul_add]; rw [ add_tsub_cancel_right]; rw [ mul_comm],
end
...
begin
 ...
 { rw [mem_range]; rw [ lt_succ_iff],
 exact sum_sq_le_of_mem_box hx },
 { rw [card_range]; rw [ _root_.nsmul_eq_mul]; rw [ mul_div_assoc']; rw [ cast_add_one]; rw [ mul_div_cancel_left]; rw [ card_box],
...
begin
 ...
 rw [cast_mul]; rw [ cast_pow]; rw [ mul_comm]; rw [ ←div_div]; rw [ pow_sub₀ _ _ hn]; rw [ ←div_eq_mul_inv],
 rwa cast_ne_zero,
end
...
begin
 rw [show (8 : ℝ) = 2 ^ ((3 : ℕ) : ℝ)]; rw [ by norm_num1]; rw [ log_rpow zero_lt_two (3:ℕ)],
...
begin
 ...
 norm_num },
 rw [l8]; rw [ cast_bit1]; rw [ cast_one],
...
begin
 ...
 rwa [le_sub_comm]; rwa [ div_eq_mul_one_div x]; rwa [ div_eq_mul_one_div x]; rwa [ ←mul_sub]; rwa [ div_sub']; rwa [ ←div_eq_mul_one_div]; rwa [ mul_div_assoc']; rwa [ one_le_div]; rwa [ ←div_le_iff this],
 { exact zero_lt_two },
 { exact two_ne_zero }
end
...
begin
 ...
 rwa [←le_sub_iff_add_le']; rwa [ ←sub_one_mul]; rwa [ show (69/50 - 1 : ℝ) = (50/19)⁻¹]; rwa [ by norm_num1]; rwa [ ←div_eq_inv_mul]; rwa [ one_le_div],
 norm_num1,
end
...
begin
 rw [n_value]; rw [ ←lt_iff_add_one_le]; rw [ lt_ceil]; rw [ cast_two],
 apply lt_sqrt_of_sq_lt,
 have : (2 : ℝ)^((6 : ℕ) : ℝ) ≤ N,
 { rw rpow_nat_cast,
 exact (cast_le.2 hN).trans' (by norm_num1) },
 apply lt_of_lt_of_le _ ((log_le_log (rpow_pos_of_pos zero_lt_two _) _).2 this),
 rw [log_rpow zero_lt_two]; rw [ cast_bit0]; rw [ cast_bit1]; rw [ cast_one]; rw [ ←div_lt_iff'],
...
begin
 ...
 rw [d_value]; rw [ floor_pos]; rw [ ←log_le_log zero_lt_one]; rw [ log_one]; rw [ log_div _ two_ne_zero]; rw [ log_rpow hN₀]; rw [ div_mul_eq_mul_div]; rw [ one_mul]; rw [ sub_nonneg]; rw [ le_div_iff],
...
begin
 ...
 { rw [cast_pos]; rw [ lt_ceil]; rw [ cast_zero]; rw [ real.sqrt_pos],
 refine log_pos _,
 rw one_lt_cast,
 exact hN.trans_lt' (by norm_num1) },
 apply le_sqrt_of_sq_le,
...
begin
 ...
 { rw [←le_log_iff_exp_le (rpow_pos_of_pos hN₀ _)]; rw [ log_rpow hN₀]; rw [ ←le_div_iff]; rw [ mul_div_assoc]; rw [ div_sqrt]; rw [ neg_mul]; rw [ neg_le_neg_iff]; rw [ div_mul_eq_mul_div]; rw [ div_le_iff hn],
...
begin
 rw [show (64 : ℝ) = 2 ^ ((6 : ℕ) : ℝ)]; rw [ by norm_num1]; rw [ ←lt_log_iff_exp_lt (rpow_pos_of_pos zero_lt_two _)]; rw [ log_rpow zero_lt_two]; rw [ ←div_lt_iff'],
 exact log_two_gt_d9.trans_le' (by norm_num1),
 norm_num
end
...
begin
 rw [←log_lt_iff_lt_exp (show (0 : ℝ) < 4096, by norm_num)]; rw [ show (4096 : ℝ) = 2 ^ 12]; rw [ by norm_num]; rw [ ←rpow_nat_cast]; rw [ log_rpow zero_lt_two]; rw [ cast_bit0]; rw [ cast_bit0]; rw [ cast_bit1]; rw [ cast_one],
 linarith [log_two_lt_d9],
end

lemma lower_bound_le_one' (hN : 2 ≤ N) (hN' : N ≤ 4096) : (N : ℝ) * exp (-4 * sqrt (log N)) ≤ 1 :=
begin
 rw [←log_le_log (mul_pos (cast_pos.2 (zero_lt_two.trans_le hN)) (exp_pos _)) zero_lt_one]; rw [ log_one]; rw [ log_mul (cast_pos.2 (zero_lt_two.trans_le hN)).ne' (exp_pos _).ne']; rw [ log_exp]; rw [ neg_mul]; rw [ ←sub_eq_add_neg]; rw [ sub_nonpos]; rw [ ←div_le_iff (real.sqrt_pos.2 $ log_pos $ one_lt_cast.2 $ one_lt_two.trans_le hN)]; rw [ div_sqrt]; rw [ sqrt_le_left (zero_le_bit0.2 zero_le_two)]; rw [ log_le_iff_le_exp (cast_pos.2 (zero_lt_two.trans_le hN))],
 norm_num1,
