...
begin
 ...
 { rintro ⟨t, ht, hst, h⟩,
 obtain ⟨a, ha⟩ : ∃ a, t \ s = {a} :=
 card_eq_one.1 (by rw [card_sdiff hst]; rw [ h]; rw [ add_tsub_cancel_left]),
 exact ⟨a, λ hat,
 not_mem_sdiff_of_mem_right hat ((ha.ge : _ ⊆ _) $ mem_singleton_self a),
 by rwa [insert_eq a s]; rwa [ ←ha]; rwa [ sdiff_union_of_subset hst]⟩ }
end
...
begin
 ...
 (by { rw [add_comm]; rw [ hcard], exact le_succ _ }) hst,
 rw add_comm at hu,
...
begin
 ...
 { rintro ⟨t, ht, hts, h⟩,
 obtain ⟨a, ha⟩ : ∃ a, s \ t = {a} :=
 card_eq_one.1 (by rw [card_sdiff hts]; rw [ ←h]; rw [ add_tsub_cancel_left]),
 refine ⟨a, sdiff_subset _ _ ((ha.ge : _ ⊆ _) $ mem_singleton_self a), _⟩,
 rwa [←sdiff_singleton_eq_erase]; rwa [ ←ha]; rwa [ sdiff_sdiff_eq_self hts] }
end
...
begin
 ...
 (by { rw [add_comm]; rw [ ←hcard], exact add_le_add_left (zero_lt_succ _) _ }) hts,
 rw add_comm at hu,
