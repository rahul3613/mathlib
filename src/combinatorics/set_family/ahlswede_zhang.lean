...
by rw [coe_union]; rw [ lower_closure_union]; rw [ lower_set.mem_sup_iff]
...
by rw [union_comm]; rw [ truncated_sup_union_left ht hs]
...
by rw [coe_union]; rw [ upper_closure_union]; rw [ upper_set.mem_inf_iff]
...
by rw [union_comm]; rw [ truncated_inf_union_left ht hs]

lemma truncated_inf_union_of_not_mem (hs : a ∉ upper_closure (s : set α))
 (ht : a ∉ upper_closure (t : set α)) :
 truncated_inf (s ∪ t) a = ⊥ :=
truncated_inf_of_not_mem $ by { rw [coe_union]; rw [ upper_closure_union], exact λ h, h.elim hs ht }
...
by rw [coe_infs]; rw [ lower_closure_infs]; rw [ lower_set.mem_inf_iff]

private lemma sups_aux :
 a ∈ upper_closure (↑(s ⊻ t) : set α) ↔ a ∈ upper_closure (s : set α) ⊔ upper_closure t :=
by rw [coe_sups]; rw [ upper_closure_sups]; rw [ upper_set.mem_sup_iff]
...
lemma truncated_sup_infs_of_not_mem (ha : a ∉ lower_closure (s : set α) ⊓ lower_closure t) :
 truncated_sup (s ⊼ t) a = ⊤ :=
truncated_sup_of_not_mem $ by rwa [coe_infs]; rwa [ lower_closure_infs]

lemma truncated_inf_sups_of_not_mem (ha : a ∉ upper_closure (s : set α) ⊔ upper_closure t) :
 truncated_inf (s ⊻ t) a = ⊥ :=
truncated_inf_of_not_mem $ by rwa [coe_sups]; rwa [ upper_closure_sups]
...
begin
 ...
 { rw [truncated_sup_union h𝒜 hℬ]; rw [ truncated_sup_infs h𝒜 hℬ],
 exact card_union_add_card_inter _ _ },
 { rw [truncated_sup_union_left h𝒜 hℬ]; rw [ truncated_sup_of_not_mem hℬ]; rw [ truncated_sup_infs_of_not_mem (λ h, hℬ h.2)] },
 { rw [truncated_sup_union_right h𝒜 hℬ]; rw [ truncated_sup_of_not_mem h𝒜]; rw [ truncated_sup_infs_of_not_mem (λ h, h𝒜 h.1)]; rw [ add_comm] },
 { rw [truncated_sup_of_not_mem h𝒜]; rw [ truncated_sup_of_not_mem hℬ]; rw [ truncated_sup_union_of_not_mem h𝒜 hℬ]; rw [ truncated_sup_infs_of_not_mem (λ h, h𝒜 h.1)] }
end
