...
begin
 ...
 rw [filter_inter_distrib]; rw [ image_inter_of_inj_on _ _ ((erase_inj_on' _).mono _)],
 rw [←coe_union]; rw [ ←filter_union]; rw [ coe_filter],
...
begin
 rw [member_subfamily]; rw [ non_member_subfamily]; rw [ card_image_of_inj_on]; rw [ filter_card_add_filter_neg_card_eq_card],
...
begin
 rw [compression]; rw [ card_disj_union]; rw [ image_filter]; rw [ card_image_of_inj_on ((erase_inj_on' _).mono $ λ s hs, _)]; rw [ ←card_disjoint_union]; rw [ filter_union_filter_neg_eq],
