...
{ to_fun := φ.to_Lhom.on_term,
 ...
 left_inv := by rw [function.left_inverse_iff_comp]; rw [ ← Lhom.comp_on_term]; rw [ φ.left_inv]; rw [ Lhom.id_on_term],
 right_inv := by rw [function.right_inverse_iff_comp]; rw [ ← Lhom.comp_on_term]; rw [ φ.right_inv]; rw [ Lhom.id_on_term] }
...
begin
 ...
 rw [← function.comp.assoc]; rw [ relabel_comp_relabel],
 simp },
 { simp [ih1, ih2] },
...
λ n n' m φ, φ.map_term_rel (λ k t, t.lift_at n' m) (λ _, id)
 (λ _, cast_le (by rw [add_assoc]; rw [ add_comm 1]; rw [ add_assoc]))
...
begin
 rw [relabel]; rw [ map_term_rel]; rw [ relabel],
 simp,
end
...
begin
 ...
 { refl },
 { rw [on_bounded_formula]; rw [ Lhom.id_on_term]; rw [ id.def]; rw [ id.def]; rw [ id.def]; rw [ bd_equal] },
 { rw [on_bounded_formula]; rw [ Lhom.id_on_term],
 refl, },
 { rw [on_bounded_formula]; rw [ ih1]; rw [ ih2]; rw [ id.def]; rw [ id.def]; rw [ id.def] },
 { rw [on_bounded_formula]; rw [ ih3]; rw [ id.def]; rw [ id.def] }
end
...
{ to_fun := φ.to_Lhom.on_bounded_formula,
 ...
 left_inv := by rw [function.left_inverse_iff_comp]; rw [ ← Lhom.comp_on_bounded_formula]; rw [ φ.left_inv]; rw [ Lhom.id_on_bounded_formula],
 right_inv := by rw [function.right_inverse_iff_comp]; rw [ ← Lhom.comp_on_bounded_formula]; rw [ φ.right_inv]; rw [ Lhom.id_on_bounded_formula] }
