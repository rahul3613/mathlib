...
begin
 ...
 { refl },
 { rw [cons_bind]; rw [ h t (l.bind list_encode)]; rw [ lih]; rw [ list.map] } },
 { intro t,
 induction t with a n f ts ih; intro l,
 { rw [list_encode]; rw [ singleton_append]; rw [ list_decode] },
 { rw [list_encode]; rw [ cons_append]; rw [ list_decode],
...
begin
 ...
 rw [mk_nat]; rw [ lift_aleph_0]; rw [ mul_eq_max_of_aleph_0_le_left le_rfl]; rw [ max_le_iff]; rw [ csupr_le_iff' (bdd_above_range _)],
 { refine ⟨le_max_left _ _, λ i, card_le.trans _⟩,
 refine max_le (le_max_left _ _) _,
 rw [← add_eq_max le_rfl]; rw [ mk_sum]; rw [ mk_sum]; rw [ mk_sum]; rw [ add_comm (cardinal.lift (#α))]; rw [ lift_add]; rw [ add_assoc]; rw [ lift_lift]; rw [ lift_lift]; rw [ mk_fin]; rw [ lift_nat_cast],
...
encodable.of_left_injection list_encode (λ l, (list_decode l).head'.join)
 (λ t, begin
 rw [← bind_singleton list_encode]; rw [ list_decode_encode_list],
...
| ((sum.inr (sum.inr 0)) :: l) :=
 ...
 rw [add_assoc]; rw [ add_comm]; rw [ nat.lt_succ_iff]; rw [ add_assoc],
 exact le_self_add,
 end,
...
begin
 ...
 { rw [list.nil_bind],
 simp [list_decode], },
 { rw [cons_bind]; rw [ (h φ _).1]; rw [ head_cons] } },
 { rintro ⟨n, φ⟩,
 induction φ with _ _ _ _ _ _ _ ts _ _ _ ih1 ih2 _ _ ih; intro l,
 { rw [list_encode]; rw [ singleton_append]; rw [ list_decode],
 simp only [eq_self_iff_true, heq_iff_eq, and_self], },
 { rw [list_encode]; rw [ cons_append]; rw [ cons_append]; rw [ list_decode]; rw [ dif_pos],
 { simp only [eq_mp_eq_cast, cast_eq, eq_self_iff_true, heq_iff_eq, and_self, nil_append], },
 { simp only [eq_self_iff_true, heq_iff_eq, and_self], } },
 { rw [list_encode]; rw [ cons_append]; rw [ cons_append]; rw [ singleton_append]; rw [ cons_append]; rw [ list_decode],
 ...
 rw [nth_le_append]; rw [ nth_le_map],
 ...
 rw [list.drop_append_eq_append_drop]; rw [ length_map]; rw [ length_fin_range]; rw [ nat.sub_self]; rw [ drop]; rw [ drop_eq_nil_of_le]; rw [ nil_append],
 rw [length_map]; rw [ length_fin_range], }, },
 { rw [list_encode]; rw [ list.append_assoc]; rw [ cons_append]; rw [ list_decode],
 simp only [subtype.val_eq_coe] at *,
 rw [(ih1 _).1]; rw [ (ih1 _).2]; rw [ (ih2 _).1]; rw [ (ih2 _).2]; rw [ sigma_imp]; rw [ dif_pos rfl],
 exact ⟨rfl, rfl⟩, },
 { rw [list_encode]; rw [ cons_append]; rw [ list_decode],
 simp only,
 simp only [subtype.val_eq_coe] at *,
 rw [(ih _).1]; rw [ (ih _).2]; rw [ sigma_all],
 exact ⟨rfl, rfl⟩ } }
end
...
begin
 ...
 rw [encoding_Γ]; rw [ mk_list_eq_max_mk_aleph_0]; rw [ lift_max]; rw [ lift_aleph_0]; rw [ lift_max]; rw [ lift_aleph_0]; rw [ max_le_iff],
 ...
 rw [← add_assoc]; rw [ add_comm]; rw [ ← add_assoc]; rw [ ← add_assoc]; rw [ aleph_0_add_aleph_0]; rw [ add_assoc]; rw [ add_eq_max le_rfl]; rw [ add_assoc]; rw [ card]; rw [ symbols]; rw [ mk_sum]; rw [ lift_add]; rw [ lift_lift]; rw [ lift_lift],
end
