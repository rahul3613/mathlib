...
begin
 induction t,
 { refl },
 { rw [term.realize]; rw [ term.realize]; rw [ g.map_fun],
 refine congr rfl _,
 ext x,
 simp [t_ih x], },
end
...
begin
 rw [relations.bounded_formula₁]; rw [ realize_rel]; rw [ iff_eq_eq],
...
begin
 induction l with φ l ih,
 { simp },
 { simp_rw [list.foldr_cons, realize_sup, ih, exists_prop, list.mem_cons_iff, or_and_distrib_right, exists_or_distrib, exists_eq_left] }
end
...
begin
 rw [bounded_formula.ex]; rw [ realize_not]; rw [ realize_all]; rw [ not_forall],
...
by rw [relabel]; rw [ realize_map_term_rel_add_cast_le]; intros; simp
...
begin
 rw [realize_lift_at_one (refl n)]; rw [ iff_eq_eq],
...
begin
 ...
 { rw [to_prenex_imp_right]; rw [ realize_ex],
...
begin
 ...
 { rw [to_prenex_imp]; rw [ realize_ex],
...
begin
 ...
 (λ _ f1 f2 h1 h2 _, _)
 (λ _ f h xs, _),
 { rw [to_prenex]; rw [ realize_to_prenex_imp f1.to_prenex_is_prenex f2.to_prenex_is_prenex]; rw [ realize_imp]; rw [ realize_imp]; rw [ h1]; rw [ h2],
 apply_instance },
 { rw [realize_all]; rw [ to_prenex]; rw [ realize_all],
...
begin
 rw [realize]; rw [ realize]; rw [ relabel]; rw [ bounded_formula.realize_relabel]; rw [ iff_eq_eq]; rw [ fin.cast_add_zero],
...
by rw [bounded_formula.realize_relabel]; rw [ formula.realize]; rw [ sum.elim_comp_inr]; rw [ fin.cast_add_zero]; rw [ cast_refl]; rw [ order_iso.coe_refl]; rw [ function.comp.right_id]; rw [ subsingleton.elim (x ∘ (nat_add n : fin 0 → fin n)) default]
...
by rw [← realize_equiv_sentence_symm_con M (equiv_sentence φ)]; rw [ _root_.equiv.symm_apply_apply]
...
begin
 ...
 { simp [bounded_formula.realize] },
 { rw [to_formula]; rw [ formula.realize]; rw [ realize_imp]; rw [ ← formula.realize]; rw [ ih1]; rw [ ← formula.realize]; rw [ ih2]; rw [ realize_imp], },
 { rw [to_formula]; rw [ formula.realize]; rw [ realize_all]; rw [ realize_all],
 ...
 rw [← h]; rw [ realize_relabel]; rw [ formula.realize],
 rcongr,
 { cases x,
 { simp },
 { refine fin.last_cases _ (λ i, _) x,
 { rw [sum.elim_inr]; rw [ snoc_last]; rw [ function.comp_app]; rw [ sum.elim_inr]; rw [ function.comp_app]; rw [ fin_sum_fin_equiv_symm_last]; rw [ sum.map_inr]; rw [ sum.elim_inr]; rw [ function.comp_app],
 ...
 rw [← cast_succ]; rw [ snoc_cast_succ] } } },
...
begin
 ...
 { rw [bounded_formula.realize]; rw [ ih1]; rw [ ih2]; rw [ bounded_formula.realize] },
 { rw [bounded_formula.realize]; rw [ bounded_formula.realize],
 split,
 { intros h a,
 have h' := h (g a),
...
by rw [formula.realize]; rw [ formula.realize]; rw [ ← g.realize_bounded_formula φ]; rw [ iff_eq_eq]; rw [ unique.eq_default (g ∘ default)]

lemma realize_sentence (g : M ≃[L] N) (φ : L.sentence) :
 M ⊨ φ ↔ N ⊨ φ :=
by rw [sentence.realize]; rw [ sentence.realize]; rw [ ← g.realize_formula]; rw [ unique.eq_default (g ∘ default)]
...
begin
 rw [← lift_mk_fin]; rw [ ← lift_le]; rw [ lift_lift]; rw [ lift_mk_le]; rw [ sentence.card_ge]; rw [ sentence.realize]; rw [ bounded_formula.realize_exs],
...
by rw [Theory.model_iff_subset_complete_theory]; rw [ Theory.model_iff_subset_complete_theory]; rw [ h.complete_theory_eq]
