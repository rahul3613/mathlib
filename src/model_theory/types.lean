...
begin
 rw [iff_not_comm]; rw [ ← not_nonempty_iff_eq_empty]; rw [ not_not]; rw [ set.nonempty],
...
begin
 rw [models_iff_not_satisfiable]; rw [ ← compl_empty_iff]; rw [ compl_set_of_mem]; rw [ ← set_of_subset_eq_empty_iff],
 simp,
end
...
begin
 ...
 rw [nonempty_iff_univ_nonempty]; rw [ nonempty_iff_ne_empty]; rw [ ne.def]; rw [ not_iff_comm]; rw [ ← union_empty ((L.Lhom_with_constants α).on_Theory T)]; rw [ ← set_of_subset_eq_empty_iff],
 simp,
end
...
begin
 simp_rw [subset_def, ← set_like.mem_coe, p.is_maximal.mem_iff_models, models_sentence_iff, sentence.realize, formula.realize, bounded_formula.realize_foldr_inf, finset.mem_to_list],
