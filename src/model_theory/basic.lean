...
begin
 ...
 rw [add_assoc]; rw [ ←add_assoc (cardinal.sum (λ i, (# (L'.functions i)).lift))]; rw [ add_comm (cardinal.sum (λ i, (# (L'.functions i)).lift))]; rw [ add_assoc]; rw [ add_assoc]
end
...
{ map_fun' := λ n f' x, begin
 ...
 rw [← function.comp.assoc]; rw [ equiv.to_fun_as_coe]; rw [ equiv.self_comp_symm]; rw [ function.comp.left_id]
 end,
