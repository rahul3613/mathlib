...
begin
 rw [definable]; rw [ equiv.exists_congr_left (Lequiv.add_empty_constants L (∅ : set M)).on_formula],
 simp,
end
...
begin
 ...
 refine ⟨φ ⊔ θ, _⟩,
 ext,
 rw [hφ]; rw [ hθ]; rw [ mem_set_of_eq]; rw [ formula.realize_sup]; rw [ mem_union]; rw [ mem_set_of_eq]; rw [ mem_set_of_eq],
end
...
begin
 ...
 refine ⟨y, ys, _⟩,
 ext a,
 rw [hx a]; rw [ ← function.comp_apply x]; rw [ ← hy],
 simp, },
 { rintro ⟨y, ys, rfl⟩,
 refine ⟨⟨y, ys, _⟩, λ a, _⟩,
 { ext,
 simp [set.apply_range_splitting f] },
 { rw [function.comp_apply]; rw [ function.comp_apply]; rw [ apply_range_splitting f]; rw [ range_factorization_coe], }}
end
