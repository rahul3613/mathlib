...
begin
 ...
 rw [← lift_le]; rw [ lift_lift]; rw [ lift_mk_le],
...
begin
 ...
 rw [lift_umax] at h; rw [ lift_id'] at h,
 rw [coe_sort_elementary_skolem₁_reduct]; rw [ ← h]; rw [ lift_inj],
 ...
 rw [max_le_iff]; rw [ aleph_0_le_lift]; rw [ ← aleph_0_le_lift]; rw [ h]; rw [ add_eq_max]; rw [ max_le_iff]; rw [ lift_le],
 refine ⟨h1, (mk_union_le _ _).trans _, (lift_le.2 card_functions_sum_skolem₁_le).trans _⟩,
 { rw [← lift_le]; rw [ lift_add]; rw [ h]; rw [ add_comm]; rw [ add_eq_max h1],
 exact max_le le_rfl h2 },
 { rw [lift_max]; rw [ lift_aleph_0]; rw [ max_le_iff]; rw [ aleph_0_le_lift]; rw [ and_comm]; rw [ ← lift_le.{_ w'}]; rw [ lift_lift]; rw [ lift_lift]; rw [ ← aleph_0_le_lift]; rw [ h],
 refine ⟨_, h1⟩,
 simp only [← lift_lift, lift_umax, lift_umax'],
 rw [lift_lift]; rw [ ← lift_lift.{w' w} L.card],
 refine trans ((lift_le.{_ w}).2 h3) _,
 rw [lift_lift]; rw [ ← lift_lift.{w (max u v)}]; rw [ ← hs']; rw [ ← h]; rw [ lift_lift]; rw [ lift_lift]; rw [ lift_lift] },
 { refine trans _ (lift_le.2 (mk_le_mk_of_subset (set.subset_union_right _ _))),
 rw [aleph_0_le_lift]; rw [ ← aleph_0_le_lift]; rw [ h],
 exact h1 }
end
