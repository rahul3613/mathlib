...
begin
 ...
 rw [← embedding.coe_to_hom]; rw [ finset.coe_image]; rw [ closure_image PM.to_hom]; rw [ hs]; rw [ ← hom.range_eq_map],
...
begin
 ...
 rw [embedding.coe_to_hom]; rw [ direct_limit.of_apply]; rw [ quotient.mk_eq_iff_out]; rw [ direct_limit.equiv_iff G f _ (hi (out x).1 (finset.mem_image_of_mem _ hx))]; rw [ directed_system.map_self],
 refl },
 { rintro ⟨i, Mfg, ⟨e⟩⟩,
