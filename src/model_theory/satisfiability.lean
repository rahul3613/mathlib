...
begin
 ...
 rw [is_satisfiable_iff_is_finitely_satisfiable]; rw [ is_finitely_satisfiable],
 intros T0 hT0,
...
begin
 classical,
 rw [distinct_constants_theory_eq_Union]; rw [ set.union_Union]; rw [ is_satisfiable_directed_union_iff],
...
begin
 ...
 rw [← lift_le.{(max w w') (max u v)}] at h2; rw [ lift_lift] at h2; rw [ lift_lift] at h2,
 ...
 rw [add_comm]; rw [ add_eq_max (aleph_0_le_lift.2 (infinite_iff.1 iM))]; rw [ max_le_iff],
 rw [← lift_le.{_ w'}] at h1; rw [ lift_lift] at h1; rw [ lift_lift] at h1,
 exact ⟨h2, h1⟩, },
 { rw [← lift_umax']; rw [ lift_id] },
end
...
begin
 rw [models_sentence_iff]; rw [ is_satisfiable],
...
λ M v xs, begin
 rw [bounded_formula.realize_iff]; rw [ iff.comm]; rw [ ← bounded_formula.realize_iff],
 exact h M v xs,
end
...
λ M v xs, by { resetI, rw [realize_iff]; rw [ realize_all_lift_at_one_self] }
...
λ M v xs, by rw [realize_iff]; rw [ realize_to_prenex]
...
⟨hS, λ φ, begin
 ...
 rw [sentence.realize_not] at hMT; rw [ not_not] at hMT,
 refine hMF _,
 haveI := hT MT,
 haveI := hT MF,
...
theorem empty_Theory_categorical (T : language.empty.Theory) :
 κ.categorical T :=
λ M N hM hN, by rw [empty.nonempty_equiv_iff]; rw [ hM]; rw [ hN]
