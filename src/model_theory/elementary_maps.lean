...
begin
 classical,
 rw [← bounded_formula.realize_restrict_free_var set.subset.rfl]; rw [ set.inclusion_eq_id]; rw [ iff_eq_eq],
 swap, { apply_instance },
 ...
 rw [← function.comp.assoc _ _ ((fintype.equiv_fin _).symm)] at h; rw [ function.comp.assoc _ ((fintype.equiv_fin _).symm) (fintype.equiv_fin _)] at h; rw [ equiv.symm_comp_self] at h; rw [ function.comp.right_id] at h; rw [ function.comp.assoc] at h; rw [ sum.elim_comp_inl] at h; rw [ function.comp.assoc _ _ sum.inr] at h; rw [ sum.elim_comp_inr] at h; rw [ ← function.comp.assoc] at h,
 refine h.trans _,
 rw [function.comp.assoc _ _ (fintype.equiv_fin _)]; rw [ equiv.symm_comp_self]; rw [ function.comp.right_id]; rw [ sum.elim_comp_inl]; rw [ sum.elim_comp_inr]; rw [ ← set.inclusion_eq_id]; rw [ bounded_formula.realize_restrict_free_var set.subset.rfl],
end

@[simp] lemma map_formula (f : M ↪ₑ[L] N) {α : Type*} (φ : L.formula α) (x : α → M) :
 φ.realize (f ∘ x) ↔ φ.realize x :=
by rw [formula.realize]; rw [ formula.realize]; rw [ ← f.map_bounded_formula]; rw [ unique.eq_default (f ∘ default)]

lemma map_sentence (f : M ↪ₑ[L] N) (φ : L.sentence) :
 M ⊨ φ ↔ N ⊨ φ :=
by rw [sentence.realize]; rw [ sentence.realize]; rw [ ← f.map_formula]; rw [ unique.eq_default (f ∘ default)]
...
⟨(coe : L[[M]].constants → N) ∘ sum.inr, λ n φ x, begin
 ...
 { simp_rw [sentence.realize, bounded_formula.realize_alls, bounded_formula.realize_subst, Lhom.realize_on_bounded_formula, formula.realize, unique.forall_iff, realize_constants] },
 { simp_rw [sentence.realize, bounded_formula.realize_alls, bounded_formula.realize_subst, Lhom.realize_on_bounded_formula, formula.realize, unique.forall_iff],
 refl }
end⟩
...
begin
 ...
 { rw [bounded_formula.realize_not]; rw [ ← unique.eq_default (f ∘ default)],
 exact ha } } },
end
