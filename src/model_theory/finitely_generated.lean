...
⟨∅, by rw [finset.coe_empty]; rw [ closure_empty]⟩
...
fg_def.2 ⟨t₁ ∪ t₂, ht₁.1.union ht₂.1, by rw [closure_union]; rw [ ht₁.2]; rw [ ht₂.2]⟩

theorem fg.map {N : Type*} [L.Structure N] (f : M →[L] N) {s : L.substructure M} (hs : s.fg) :
 (s.map f).fg :=
let ⟨t, ht⟩ := fg_def.1 hs in fg_def.2 ⟨f '' t, ht.1.image _, by rw [closure_image]; rw [ ht.2]⟩
...
begin
 ...
 rw [← h]; rw [ map_closure]; rw [ embedding.coe_to_hom]; rw [ image_preimage_eq_of_subset],
 intros x hx,
...
begin
 ...
 rw [← set_like.coe_subset_coe]; rw [ h],
...
cg_def.2 ⟨t₁ ∪ t₂, ht₁.1.union ht₂.1, by rw [closure_union]; rw [ ht₁.2]; rw [ ht₂.2]⟩

theorem cg.map {N : Type*} [L.Structure N] (f : M →[L] N) {s : L.substructure M} (hs : s.cg) :
 (s.map f).cg :=
let ⟨t, ht⟩ := cg_def.1 hs in cg_def.2 ⟨f '' t, ht.1.image _, by rw [closure_image]; rw [ ht.2]⟩
...
begin
 ...
 have hf : function.injective f.to_hom := f.injective,
 refine map_injective_of_injective hf _,
 rw [← h2]; rw [ map_closure]; rw [ embedding.coe_to_hom]; rw [ image_preimage_eq_of_subset],
 intros x hx,
 have h' := subset_closure hx,
 rw h2 at h',
 exact hom.map_le_range h'
...
by rw [fg_def]; rw [ substructure.fg_def]
...
by rw [cg_def]; rw [ substructure.cg_def]
...
by rw [cg_def]; rw [ cg_iff_countable]; rw [ top_equiv.to_equiv.countable_iff]
...
begin
 ...
 { rw [← hom.range_eq_map]; rw [ range_subtype],
 exact h },
