...
begin
 ...
 { rw [nat_le_rec]; rw [ nat.le_rec_on_self]; rw [ embedding.refl_apply]; rw [ nat.le_rec_on_self] },
 { rw [nat.le_rec_on_succ le_self_add]; rw [ nat_le_rec]; rw [ nat.le_rec_on_succ le_self_add]; rw [ ← nat_le_rec]; rw [ embedding.comp_apply]; rw [ ih] }
end
...
{ r := λ ⟨i, x⟩ ⟨j, y⟩, ∃ (k : ι) (ik : i ≤ k) (jk : j ≤ k), f i k ik x = f j k jk y,
 ...
 rw [← directed_system.map_map]; rw [ hij]; rw [ directed_system.map_map],
 symmetry,
 rw [← directed_system.map_map]; rw [ ← hjk]; rw [ directed_system.map_map],
 end ⟩ }
...
begin
 ...
 rw [directed_system.map_map] at *; rw [ directed_system.map_map] at *,
 exact h
end
...
begin
 ...
 rw [sum.elim_range] at hi; rw [ upper_bounds_union] at hi,
...
begin
 ...
 obtain ⟨k, ik, jk⟩ := directed_of (≤) i (classical.some (fintype.bdd_above_range
 (λ (a : fin n), i))),
 rw [rel_map_equiv_unify G f R (λ a, ⟨i, x a⟩) i]; rw [ unify_sigma_mk_self],
end
...
begin
 ...
 ext a,
 rw [quotient.eq_mk_iff_out]; rw [ function.comp_app]; rw [ unify]; rw [ equiv_iff G f _],
...
{ to_fun := quotient.lift (λ (x : Σ i, G i), (g x.1) x.2) (λ x y xy, begin
 ...
 rw [fun_map_quotient_mk_sigma_mk]; rw [ ← function.comp.assoc]; rw [ quotient.lift_comp_mk],
 simp only [quotient.lift_mk, embedding.map_fun],
 end,
 map_rel' := λ n R x, begin
 obtain ⟨i, y, rfl⟩ := exists_quotient_mk_sigma_mk_eq G f x,
 rw [rel_map_quotient_mk_sigma_mk G f]; rw [ ← (g i).map_rel R y]; rw [ ← function.comp.assoc]; rw [ quotient.lift_comp_mk],
 end }
...
theorem lift_unique (F : direct_limit G f ↪[L] P) (x) :
 ...
 (λ i j hij x, by rw [F.comp_apply]; rw [ F.comp_apply]; rw [ of_f]) x :=
...
begin
 ...
 { rw [eq_top_iff]; rw [ substructure.closure_Union],
