...
instance small_closure [small.{u} s] :
 small.{u} (closure L s) :=
begin
 rw [← set_like.coe_sort_coe]; rw [ substructure.coe_closure_eq_range_term_realize],
 exact small_range _,
end

lemma mem_closure_iff_exists_term {x : M} :
 x ∈ closure L s ↔ ∃ (t : L.term s), t.realize (coe : s → M) = x :=
by rw [← set_like.mem_coe]; rw [ coe_closure_eq_range_term_realize]; rw [ mem_range]

lemma lift_card_closure_le_card_term : cardinal.lift.{max u w} (# (closure L s)) ≤ # (L.term s) :=
begin
 rw [← set_like.coe_sort_coe]; rw [ coe_closure_eq_range_term_realize],
...
begin
 ...
 rw [← mk_le_aleph_0_iff]; rw [ ← lift_le_aleph_0],
...
by rw [set_like.ext'_iff]; rw [ range_coe]; rw [ coe_top]; rw [ set.range_iff_surjective]
...
{ to_fun := f.to_hom.cod_restrict p h,
 ...
 simp only,
 rw [← p.subtype.map_rel]; rw [ function.comp.assoc],
 change rel_map r ((hom.comp p.subtype.to_hom (f.to_hom.cod_restrict p h)) ∘ x) ↔ _,
 rw [hom.subtype_comp_cod_restrict]; rw [ ← f.map_rel],
 refl,
 end }
