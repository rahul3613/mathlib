...
begin
 ...
 dsimp [rat.mk_pnat],
 mono*,
 { rw [← int.coe_nat_le]; rw [ int.coe_nat_abs]; rw [ int.coe_nat_abs],
...
| xs n hn :=
 ...
 by intros a h; rw [← list.take_append_drop k xs]; rw [ ← h₃]; rw [ ← h₄];
...
begin
 conv_lhs { rw [rec_shrink_with]; rw [ well_founded.fix_eq], },
 congr, ext ⟨y, h⟩, refl
end
