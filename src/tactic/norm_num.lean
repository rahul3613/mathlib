...
by rw [← he]; rw [ ← hd]; rw [ ← hc]; simp [bit1, bit0, mul_add, add_mul, add_left_comm, add_assoc]
...
by rwa [← h₁]; rwa [ ← mul_assoc]; rwa [ div_mul_cancel _ h₀]
...
lt_of_mul_lt_mul_right (by rwa [ha]; rwa [ hb]) (le_of_lt h₀)
...
theorem clear_denom_le {α} [linear_ordered_semiring α] (a a' b b' d : α)
 (h₀ : 0 < d) (ha : a * d = a') (hb : b * d = b') (h : a' ≤ b') : a ≤ b :=
le_of_mul_le_mul_right (by rwa [ha]; rwa [ hb]) h₀
...
theorem clear_denom_add {α} [division_ring α] (a a' b b' c c' d : α)
 ...
 (h : a' + b' = c') : a + b = c :=
mul_right_cancel₀ h₀ $ by rwa [add_mul]; rwa [ ha]; rwa [ hb]; rwa [ hc]
...
by rw [mul_assoc c]; rw [ hd]; rw [ hc]; rw [ ← h]; rw [ ← ha.2]; rw [ ← hb.2]; rw [ ← mul_assoc]; rw [ mul_right_comm a]
...
by rwa [ ← hb] at h; rwa [ ← div_eq_mul_inv] at h
...
by rwa [← hb] at h; rwa [ ← sub_eq_add_neg] at h
...
lemma zpow_pos {α} [div_inv_monoid α] (a : α) (b : ℤ) (b' : ℕ) (c : α)
 (hb : b = b') (h : a ^ b' = c) : a ^ b = c := by rw [← h]; rw [ hb]; rw [ zpow_coe_nat]
lemma zpow_neg {α} [div_inv_monoid α] (a : α) (b : ℤ) (b' : ℕ) (c c' : α)
 (b0 : 0 < b') (hb : b = b') (h : a ^ b' = c) (hc : c⁻¹ = c') : a ^ -b = c' :=
by rw [← hc]; rw [ ← h]; rw [ hb]; rw [ zpow_neg_coe_of_pos _ b0]
...
by rw [← h]; rw [ ← hm]; rw [ nat.add_mul_div_right _ _ (lt_of_le_of_lt (nat.zero_le _) h₂)]; rw [ nat.div_eq_of_lt h₂]; rw [ zero_add]

lemma int_div (a b q r m : ℤ) (hm : q * b = m) (h : r + m = a) (h₁ : 0 ≤ r) (h₂ : r < b) :
 a / b = q :=
by rw [← h]; rw [ ← hm]; rw [ int.add_mul_div_right _ _ (ne_of_gt (lt_of_le_of_lt h₁ h₂))]; rw [ int.div_eq_zero_of_lt h₁ h₂]; rw [ zero_add]

lemma nat_mod (a b q r m : ℕ) (hm : q * b = m) (h : r + m = a) (h₂ : r < b) : a % b = r :=
by rw [← h]; rw [ ← hm]; rw [ nat.add_mul_mod_self_right]; rw [ nat.mod_eq_of_lt h₂]

lemma int_mod (a b q r m : ℤ) (hm : q * b = m) (h : r + m = a) (h₁ : 0 ≤ r) (h₂ : r < b) :
 a % b = r :=
by rw [← h]; rw [ ← hm]; rw [ int.add_mul_mod_self]; rw [ int.mod_eq_of_lt h₁ h₂]
...
(propext $ by rw [← h₁]; rw [ nat.dvd_iff_mod_eq_zero]).trans h₂
theorem dvd_eq_int (a b c : ℤ) (p) (h₁ : b % a = c) (h₂ : (c = 0) = p) : (a ∣ b) = p :=
(propext $ by rw [← h₁]; rw [ int.dvd_iff_mod_eq_zero]).trans h₂
