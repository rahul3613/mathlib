...
by rw [←h3]; rw [ mul_comm n1]; rw [ mul_assoc n2]; rw [ ←mul_assoc n1]; rw [ h1]; rw [ ←mul_assoc n2]; rw [ mul_comm n2]; rw [ mul_assoc]; rw [ h2]

lemma div_subst {α} [field α] {n1 n2 k e1 e2 t1 : α} (h1 : n1 * e1 = t1) (h2 : n2 / e2 = 1)
 (h3 : n1*n2 = k) : k * (e1 / e2) = t1 :=
by rw [←h3]; rw [ mul_assoc]; rw [ mul_div_left_comm]; rw [ h2]; rw [ ←mul_assoc]; rw [ h1]; rw [ mul_comm]; rw [ one_mul]
...
begin
 rw [mul_lt_mul_left]; rw [ ←ha]; rw [ ←hb]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_comm bd]; rw [ mul_lt_mul_left],
...
lemma cancel_factors_le {α} [linear_ordered_field α] {a b ad bd a' b' gcd : α} (ha : ad*a = a')
 ...
 a ≤ b = ((1/gcd)*(bd*a') ≤ (1/gcd)*(ad*b')) :=
begin
 rw [mul_le_mul_left]; rw [ ←ha]; rw [ ←hb]; rw [ ←mul_assoc]; rw [ ←mul_assoc]; rw [ mul_comm bd]; rw [ mul_le_mul_left],
 exact mul_pos had hbd,
 exact one_div_pos.2 hgcd
end
...
lemma cancel_factors_eq {α} [linear_ordered_field α] {a b ad bd a' b' gcd : α} (ha : ad*a = a')
 ...
 a = b = ((1/gcd)*(bd*a') = (1/gcd)*(ad*b')) :=
begin
 rw [←ha]; rw [ ←hb]; rw [ ←mul_assoc bd]; rw [ ←mul_assoc ad]; rw [ mul_comm bd],
 ext, split,
 { rintro rfl, refl },
 { intro h,
