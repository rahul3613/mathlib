...
begin
 intro h1,
 unfold symmod,
 rw [int.mod_eq_of_lt (le_of_lt h1) (lt_add_one _)]; rw [ if_neg],
...
begin
 ...
 by_cases h1 : (2 * (i % j)) < j,
 { repeat {rw if_pos h1},
 rw [int.mod_def]; rw [ sub_sub_cancel] },
 { repeat {rw if_neg h1},
 rw [int.mod_def]; rw [ sub_sub]; rw [ sub_sub_cancel]; rw [ mul_add]; rw [ mul_one] }
end

lemma symmod_eq {i j : int} :
 symmod i j = i - j * (symdiv i j) :=
by rw [mul_symdiv_eq]; rw [ sub_sub_cancel]
...
| (k+1) :=
 ...
 { rw [get_map hk]; rw [ symmod_eq]; rw [ sub_mul],
...
begin
 ...
 ring },
 cases h4 with c h4,
 rw [←dvd_add_right (dvd_mul_right m c)]; rw [ h4]; rw [ ← h1],
 apply dvd_zero },
 apply calc v n
 = -(m * sgm v b as n) + (symmod b m) +
 (coeffs.val_except n v (as.map (λ x, symmod x m))) :
 begin
 rw [h2]; rw [ ← coeffs.val_except_add_eq n],
 have hn : n < as.length,
 ...
 rw get_map hn,
 simp only [a_n, m],
 rw [add_comm]; rw [ symmod_add_one_self h0],
 ring
 end
 ... = term.val (v⟨n↦sgm v b as n⟩) (rhs n b as) :
 begin
 unfold rhs, unfold term.val,
 rw [← coeffs.val_except_add_eq n]; rw [ get_set]; rw [ update_eq],
...
begin
 ...
 begin
 unfold term.val,
 rw [← coeffs.val_except_add_eq n]; rw [ rhs_correct n h1 h2],
 ...
 rw [← coeffs.val_except_add_eq n]; rw [ get_set]; rw [ update_eq]; rw [ mul_add],
 apply fun_mono_2,
 ...
 { simp only [m],
 rw add_sub_cancel },
 rw [h5]; rw [ sub_mul]; rw [ one_mul]; rw [ add_sub]; rw [ add_comm]; rw [ add_sub_assoc]; rw [ ← mul_symdiv_eq],
 ...
 apply fun_mono_2,
 { rw [mul_comm _ m]; rw [ ← coeffs.val_between_map_mul]; rw [ list.map_map] },
 simp only [list.length_map, mul_comm _ m],
 rw [← coeffs.val_between_map_mul]; rw [ list.map_map]
 end
 ...
 rw [← coeffs.val_except_add_eq n]; rw [ coeffs.val_except_update_set]; rw [ get_set]; rw [ update_eq]
 end,
 rw [← int.mul_div_cancel (term.val _ _) h3]; rw [ ← h4]; rw [ int.zero_div]
end
...
begin
 ...
 coeffs.val_except n v as',
 { rw [← coeffs.val_except_add_eq n]; rw [ get_set]; rw [ zero_mul]; rw [ add_zero]; rw [ coeffs.val_except_update_set] },
 rw [h3]; rw [ ← coeffs.val_except_add_eq n], ring
end
...
| (ee.factor i::es) ((b,as)::eqs, les) h1 :=
 ...
 rw [term.val_div h2.left h2.right]; rw [ ← h5]; rw [ int.zero_div] },
 { rw if_neg h2, apply sat_empty }
 end
| (ee.reduce n::es) ((b,as)::eqs, les) h1 :=
 ...
 rw [← h7]; rw [ ← subst_correct h2 h4], apply h5 _ h6 } },
 { intros x h6, rw list.mem_map at h6,
 cases h6 with t h6, cases h6 with h6 h7,
 rw [← h7]; rw [ ← subst_correct h2 h4], apply h3 _ h6 }
 end
