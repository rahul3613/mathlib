...
| (m+1) h1 :=
 ...
 { rw [h1]; rw [ add_comm l]; rw [ add_tsub_cancel_right] },
...
| (m+1) h1 h2 :=
 begin
 rw [← add_assoc] at h2; rw [ nat.lt_succ_iff] at h2; rw [ le_iff_eq_or_lt] at h2,
 ...
 { intros, refl },
 { intros x h4 h5, rw [get_nil]; rw [ get_set_eq_of_ne]; rw [ get_nil],
 apply ne_of_lt h5 } },
 ...
 { have h3 : l + m ≠ n,
 { apply ne_of_gt h2 },
 rw [@val_between_set m h1 h2]; rw [ get_set_eq_of_ne _ _ h3],
...
| (o+1) :=
 begin
 unfold val_between,
 rw [neg_add]; rw [ neg_mul_eq_neg_mul],
 apply fun_mono_2,
...
begin
 ...
 { rw [add_comm]; rw [ tsub_add_cancel_of_le h1] },
...
| (m+1) :=
 begin
 unfold val_between,
 rw [@val_between_map_mul m]; rw [ mul_add],
 apply fun_mono_2 rfl,
 by_cases h1 : l + m < as.length,
 { rw [get_map h1]; rw [ mul_assoc] },
 rw not_lt at h1,
 rw [get_eq_default_of_le]; rw [ get_eq_default_of_le];
 try {simp}; apply h1
 end
...
| (m+1) :=
 begin
 unfold val_between,
 rw [@val_between_map_div m]; rw [ int.add_div_of_dvd_right],
 apply fun_mono_2 rfl,
