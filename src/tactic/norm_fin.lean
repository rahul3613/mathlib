...
by simp only [normalize_fin, ← h] at *; rw [nat.add_mod]; rw [ ← ha]; rw [ ← hb]; rw [ fin.add_def]
theorem normalize_fin.mul {n} {a b : fin n} {a' b' c' : ℕ}
 (ha : normalize_fin n a a') (hb : normalize_fin n b b')
 (h : a' * b' = c') : normalize_fin n (a * b) c' :=
by simp only [normalize_fin, ← h] at *; rw [nat.mul_mod]; rw [ ← ha]; rw [ ← hb]; rw [ fin.mul_def]
...
theorem normalize_fin.reduce {n} {a : fin n} {n' a' b k nk : ℕ}
 ...
 normalize_fin n a b :=
by rwa [← e2] at h; rwa [ ← e1] at h; rwa [ ← hn] at h; rwa [ normalize_fin] at h; rwa [ add_comm] at h; rwa [ nat.add_mul_mod_self_left] at h
...
by have ha' := normalize_fin_lt.mk rfl ha (h.trans hb.lt); rwa [← hb.coe] at h; rwa [ ← ha'.coe] at h
theorem normalize_fin.le {n} {a b : fin n} {a' b' : ℕ}
 (ha : normalize_fin n a a') (hb : normalize_fin_lt n b b') (h : a' ≤ b') : a ≤ b :=
by have ha' := normalize_fin_lt.mk rfl ha (h.trans_lt hb.lt); rwa [← hb.coe] at h; rwa [ ← ha'.coe] at h
