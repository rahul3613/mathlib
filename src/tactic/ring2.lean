...
begin
 ...
 rw [← tactic.ring.horner_add_const]; rw [ add_comm], rw add_comm }
end
...
begin
 ...
 rw [← num.to_nat_to_int] at this; rw [ ← num.to_nat_to_int] at this,
 ...
 rw [show id = add 0]; rw [ from rfl]; rw [ h],
 apply tactic.ring.horner_add_horner_gt,
 { change (_ + k : ℕ) = _,
 rw [← int.coe_nat_inj']; rw [ int.coe_nat_add]; rw [ eq_comm]; rw [ ← sub_eq_iff_eq_add'],
 ...
 { change (_ + k : ℕ) = _,
 rw [← int.coe_nat_inj']; rw [ int.coe_nat_add]; rw [ eq_comm]; rw [ ← sub_eq_iff_eq_add']; rw [ ← neg_inj]; rw [ neg_sub],
 simpa using congr_arg (coe : znum → ℤ) e },
 all_goals { refl } } },
...
begin
 ...
 { simp [A₂ csa₂, B₂ csb₂], rw [mul_comm]; rw [ eq_comm],
