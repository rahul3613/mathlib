...
λ pps_pf qqs_pf pq_pf pqs_pf, calc
 pps + qqs = (p + ps) + (q + qs) : by rw [pps_pf]; rw [ qqs_pf]
 ... = (p + q) + (ps + qs) : by cc
 ... = 0 + pqs : by rw [pq_pf]; rw [ pqs_pf]
 ... = pqs : zero_add _
...
λ ps_pf, by rw [ps_pf]; rw [ zero_mul]
...
λ qs_pf, by rw [qs_pf]; rw [ mul_zero]
...
λ ps_pf, by rw [ps_pf]; rw [ one_pow]
...
λ pps_pf pqs_pf psqs_pf, calc
 ...
 ... = pqs * psqs : by rw [pqs_pf]; rw [ psqs_pf]
...
λ ps_pf qs_pf, calc
 ps ^ qs = ps' ^ 1 : by rw [ps_pf]; rw [ qs_pf]
 ... = ps' : pow_one _

lemma pow_p_pf_zero {ps : α} {qs qs' : ℕ} : ps = 0 → qs = succ qs' → ps ^ qs = 0 :=
λ ps_pf qs_pf, calc
 ps ^ qs = 0 ^ (succ qs') : by rw [ps_pf]; rw [ qs_pf]
...
λ pps_pf pqs_pf, by rw [pps_pf]; rw [ add_zero]; rw [ pqs_pf]
