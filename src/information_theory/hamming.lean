...
by { rw [hamming_dist]; rw [ card_eq_zero]; rw [ filter_eq_empty_iff], exact λ _ _ H, H rfl }
...
by simp_rw [hamming_dist, card_eq_zero, filter_eq_empty_iff, not_not, funext_iff, mem_univ, forall_true_left, imp_self]
...
by rw [eq_comm]; rw [ hamming_dist_eq_zero]
...
by rw [←hamming_dist_ne_zero]; rw [ iff_not_comm]; rw [ not_lt]; rw [ le_zero_iff]

@[simp] lemma hamming_dist_lt_one {x y : Π i, β i} : hamming_dist x y < 1 ↔ x = y :=
by rw [nat.lt_one_iff]; rw [ hamming_dist_eq_zero]
...
funext $ λ x, by rw [hamming_dist_comm]; rw [ hamming_dist_zero_right]
...
{ dist_self := by { push_cast, exact_mod_cast hamming_dist_self },
 ...
 rw [of_hamming_inj] at hab; rw [ ← mem_id_rel] at hab,
 exact hs hab },
