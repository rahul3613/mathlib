...
| c := begin
 ...
 erw [hc]; erw [ chain.map_comp], refl
end
...
λ c, by { ext x y, dsimp [curry,ωSup], rw [map_comp]; rw [map_comp], refl }

lemma continuous_uncurry : continuous $ monotone_uncurry α β γ :=
λ c, by { ext x y, dsimp [uncurry,ωSup], rw [map_comp]; rw [map_comp], refl }
