...
lemma eq_symm_apply {x : α} {y : β} (hx : x ∈ e.source) (hy : y ∈ e.target) :
 x = e.symm y ↔ e x = y :=
⟨λ h, by rw [← e.right_inv hy]; rw [ h], λ h, by rw [← e.left_inv hx]; rw [ h]⟩
...
by rw [← image_source_eq_target]; rw [ ball_image_iff]

lemma exists_mem_target {p : β → Prop} : (∃ y ∈ e.target, p y) ↔ ∃ x ∈ e.source, p (e x) :=
by rw [← image_source_eq_target]; rw [ bex_image_iff]
...
e.forall_mem_target.mpr $ λ x hx, by rw [e.left_inv hx]; rw [ h hx]
...
begin
 ...
 { rw [piecewise_eq_of_mem _ _ _ hs]; rw [ piecewise_eq_of_mem _ _ _ ((h he).2 hs)]; rw [ e.left_inv he], },
 { rw [piecewise_eq_of_not_mem _ _ _ hs]; rw [ piecewise_eq_of_not_mem _ _ _ ((h'.compl he).2 hs)]; rw [ e'.left_inv he] }
end
...
by rw [← h.image_eq]; rw [ ← h'.image_eq]; rw [ ← hs]; rw [ Heq.image_eq]
...
is_image.of_image_eq $ by rw [hs.inter_eq]; rw [ ht.inter_eq]; rw [ image_empty]

lemma image_source_inter_eq' (s : set α) :
 e '' (e.source ∩ s) = e.target ∩ e.symm ⁻¹' s :=
by rw [inter_comm]; rw [ e.left_inv_on.image_inter']; rw [ image_source_eq_target]; rw [ inter_comm]
...
by rw [← e.image_source_inter_eq']; rw [ inter_eq_self_of_subset_right h]
...
by rw [e.trans_source']; rw [ e.symm_image_target_inter_eq]
...
local_equiv.ext (λx, rfl) (λx, rfl) $ by { simp [trans_source], rw [← inter_assoc]; rw [ inter_self] }
...
⟨e.maps_to he, by rwa [mem_preimage]; rwa [ local_equiv.symm_symm]; rwa [ e.left_inv he]⟩
...
{ r := eq_on_source,
 ...
 λe e' e'' h h', ⟨by rwa [← h'.1]; rwa [ ← h.1], λx hx, by { rw [← h'.2, h.2 hx], rwa ← h.1 }⟩⟩ }
...
begin
 split,
 { rw [trans_source'']; rw [ trans_source'']; rw [ ← he.target_eq]; rw [ ← hf.1],
...
by rw [he.eq_on.inter_preimage_eq]; rw [ he.source_eq]
