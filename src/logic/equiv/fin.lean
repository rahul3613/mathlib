...
by rw [← fin.succ_above_below _ _ h]; rw [ fin_succ_equiv'_succ_above]

lemma fin_succ_equiv'_above {n : ℕ} {i : fin (n + 1)} {m : fin n} (h : i ≤ m.cast_succ) :
 (fin_succ_equiv' i) m.succ = some m :=
by rw [← fin.succ_above_above _ _ h]; rw [ fin_succ_equiv'_succ_above]
...
begin
 rw [←option.some_inj]; rw [ ←option.coe_def],
...
begin
 ext ⟨i, h⟩,
 by_cases h' : i < n,
 { rw [fin_rotate_of_lt h']; rw [ fin.snoc]; rw [ fin.cons]; rw [ dif_pos h'],
 refl, },
 { have h'' : n = i,
 { simp only [not_lt] at h', exact (nat.eq_of_le_of_lt_succ h' h).symm, },
 subst h'',
 rw [fin_rotate_last']; rw [ fin.snoc]; rw [ fin.cons]; rw [ dif_neg (lt_irrefl _)],
 refl, }
end
...
by rw [fin_rotate_succ_apply]; rw [ zero_add]
...
by rw [fin_rotate_succ_apply]; rw [ fin.coe_add_one i]
...
{ to_fun := λ a, (a / n, ↑a),
 ...
 dsimp only,
 rw [add_comm]; rw [ nat.add_mul_div_right _ _ (ne_zero.pos n)]; rw [ nat.div_eq_of_lt p.2.is_lt]; rw [ zero_add],
 end }
...
{ -- TODO: could cast from int directly if we import `data.zmod.defs`, though there are few lemmas
 ...
 left_inv := λ a, by simp_rw [coe_coe, fin.coe_of_nat_eq_mod, int.coe_nat_mod, int.nat_mod, int.to_nat_of_nonneg (int.mod_nonneg _ $ ne_zero.ne n), int.mod_mod, int.div_add_mod'],
 ...
 rw [add_comm]; rw [ int.add_mul_div_right _ _ (ne_zero.ne n)]; rw [ int.div_eq_zero_of_lt h1 h2]; rw [ int.nat_mod]; rw [ int.add_mul_mod_self]; rw [ int.mod_eq_of_lt h1 h2]; rw [ int.to_nat_coe_nat],
