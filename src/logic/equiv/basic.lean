...
begin
 rw [prod_extend_right]; rw [ coe_fn_mk],
 split_ifs with h,
 { rw h },
 { refl }
end
...
{ to_fun := list.map e,
 ...
 left_inv := λ l, by rw [list.map_map]; rw [ e.symm_comp_self]; rw [ list.map_id],
 right_inv := λ l, by rw [list.map_map]; rw [ e.self_comp_symm]; rw [ list.map_id] }
...
lemma coe_subtype_equiv_codomain_symm (f : {x' // x' ≠ x} → Y) :
 ...
 by { funext x', dsimp, erw [dif_pos x'.2]; erw [ subtype.coe_eta] }⟩ := rfl
...
funext $ λ x, by rw [update_apply _ i j]; rw [ update_apply _ j i]; rw [ equiv.swap_apply_def]; rw [ id.def]

lemma comp_swap_eq_update (i j : α) (f : α → β) :
 f ∘ equiv.swap i j = update (update f j (f i)) i (f j) :=
by rw [swap_eq_update]; rw [ comp_update]; rw [ comp_update]; rw [ comp.right_id]
...
by rw [← equiv.trans_apply]; rw [ equiv.swap_swap]; rw [ equiv.refl_apply]

/-- A function is invariant to a swap if it is equal at both elements -/
lemma apply_swap_eq_self {v : α → β} {i j : α} (hv : v i = v j) (k : α) : v (swap i j k) = v k :=
begin
 by_cases hi : k = i, { rw [hi]; rw [ swap_apply_left]; rw [ hv] },
 by_cases hj : k = j, { rw [hj]; rw [ swap_apply_right]; rw [ hv] },
 rw swap_apply_of_ne_of_ne hi hj,
end

lemma swap_apply_eq_iff {x y z w : α} :
 swap x y z = w ↔ z = swap x y w :=
by rw [apply_eq_iff_eq_symm_apply]; rw [ symm_swap]
...
begin
 ...
 split_ifs with h₁ h₂,
 { rw [hf h₁]; rw [ equiv.swap_apply_left] },
 { rw [hf h₂]; rw [ equiv.swap_apply_right] },
...
by rw [← update_comp_eq_of_injective _ g.injective]; rw [ g.apply_symm_apply]
