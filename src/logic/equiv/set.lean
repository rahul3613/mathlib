...
@[simp] protected lemma subset_image {α β} (e : α ≃ β) (s : set α) (t : set β) :
 e.symm '' t ⊆ s ↔ t ⊆ e '' s :=
by rw [image_subset_iff]; rw [ e.image_eq_preimage]
...
... ≃ s ⊕ t : by { rw (_ : t \ s ∪ s ∩ t = t), rw [union_comm]; rw [ inter_comm]; rw [ inter_union_diff] }
...
begin
 ext i,
 by_cases h : i ∈ s,
 { rw [dif_pos h]; rw [ function.update_apply_equiv_apply]; rw [ equiv.symm_symm]; rw [ function.comp]; rw [ function.update_apply]; rw [ function.update_apply]; rw [ dif_pos h],
