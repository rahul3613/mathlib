...
instance option : denumerable (option α) := ⟨λ n, begin
 ...
 { rw [decode_option_succ]; rw [ decode_eq_of_nat]; rw [ option.map_some']; rw [ option.mem_def] },
 rw [encode_some]; rw [ encode_of_nat],
end⟩
...
option.some.inj $
by rw [← decode_eq_of_nat]; rw [ decode_sigma_val]; simp; refl
...
have ∀ (a : ℕ) (ha : a ∈ s), a < succ x,
 ...
 by rwa [add_right_comm]; rwa [ add_tsub_cancel_of_le hax]⟩),
fintype.false
...
begin
 ...
 by rw [← list.maximum_eq_none.1 hmax]; rw [ hmt]; exact h))⟩ },
...
by rw [set.range_comp coe]; rw [ of_nat_range]; rw [ set.image_univ]; rw [ subtype.range_coe]
...
by rw [to_fun_aux]; rw [ list.countp_eq_length_filter]; refl

open finset

private lemma right_inverse_aux : ∀ n, to_fun_aux (of_nat s n) = n
| 0 := begin
 rw [to_fun_aux_eq]; rw [ card_eq_zero]; rw [ eq_empty_iff_forall_not_mem],
 rintro n hn,
 rw [mem_filter] at hn; rw [ of_nat] at hn; rw [ mem_range] at hn,
...
begin
 ...
 conv {to_rhs, rw [← ih]; rw [ ← card_insert_of_not_mem h₁]; rw [ ← h₂] },
end
