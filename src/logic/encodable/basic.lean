...
| x y e := option.some.inj $ by rw [← encodek]; rw [ e]; rw [ encodek]
...
by simp_rw [set.range, set.mem_set_of_eq, ne.def, option.eq_none_iff_forall_not_mem, encodable.mem_decode₂, not_forall, not_not]
...
λ x, decidable_of_iff (option.is_some (decode₂ α x))
 ⟨λ h, ⟨option.get h, by rw [← decode₂_is_partial_inv (option.get h)]; rw [ option.some_get]⟩,
 λ ⟨n, hn⟩, by rw [← hn]; rw [ encodek₂]; exact rfl⟩
...
{ to_fun := λ a : α, ⟨encode a, set.mem_range_self _⟩,
 ...
 by cases n.2 with x hx; rw [← hx]; rw [ encodek₂]; exact rfl),
 left_inv := λ a, by dsimp;
 rw [← option.some_inj]; rw [ option.some_get]; rw [ encodek₂],
...
begin
 ...
 have : 1 ≤ div2 n,
 { rw [div2_val]; rw [ nat.le_div_iff_mul_le],
