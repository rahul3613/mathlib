...
exists₂_congr $ λ _ _, and_congr iff.rfl $ by rw [add_assoc]; rw [ add_assoc]; rw [ add_left_cancel_iff]
...
begin
 ...
 rw [add_assoc] at ha; rw [ mem_add] at ha, obtain (h|h) := ha,
 { refine ⟨(s₁.erase a + t, s₂), game_add.fst ⟨t, a, hr, _⟩, _⟩,
 { rw [add_comm]; rw [ ← add_assoc]; rw [ singleton_add]; rw [ cons_erase h] },
 { rw [add_assoc s₁]; rw [ erase_add_left_pos _ h]; rw [ add_right_comm]; rw [ add_assoc] } },
 { refine ⟨(s₁, (s₂ + t).erase a), game_add.snd ⟨t, a, hr, _⟩, _⟩,
 { rw [add_comm]; rw [ singleton_add]; rw [ cons_erase h] },
 { rw [add_assoc]; rw [ erase_add_right_pos _ h] } },
end
