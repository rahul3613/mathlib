...
@[simp] lemma ρ_inv_self_apply {G : Type u} [group G] (A : Rep k G) (g : G) (x : A) :
 A.ρ g⁻¹ (A.ρ g x) = x :=
show (A.ρ g⁻¹ * A.ρ g) x = x, by rw [←map_mul]; rw [ inv_mul_self]; rw [ map_one]; rw [ linear_map.one_apply]

@[simp] lemma ρ_self_inv_apply {G : Type u} [group G] {A : Rep k G} (g : G) (x : A) :
 A.ρ g (A.ρ g⁻¹ x) = x :=
show (A.ρ g * A.ρ g⁻¹) x = x, by rw [←map_mul]; rw [ mul_inv_self]; rw [ map_one]; rw [ linear_map.one_apply]
...
by rw [linearization_obj_ρ]; rw [ finsupp.lmap_domain_apply]; rw [ finsupp.map_domain_single]
...
by rw [linearization_map_hom]; rw [ finsupp.lmap_domain_apply]; rw [ finsupp.map_domain_single]
