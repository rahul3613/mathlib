...
{ to_fun := λ g,
 { hom := A.ρ g,
 inv := A.ρ (g⁻¹ : G),
 hom_inv_id' := ((A.ρ).map_mul (g⁻¹ : G) g).symm.trans (by rw [inv_mul_self]; rw [ ρ_one]),
 inv_hom_id' := ((A.ρ).map_mul g (g⁻¹ : G)).symm.trans (by rw [mul_inv_self]; rw [ ρ_one]), },
...
def comp {M N K : Action V G} (p : Action.hom M N) (q : Action.hom N K) :
 Action.hom M K :=
{ hom := p.hom ≫ q.hom,
 comm' := λ g, by rw [←category.assoc]; rw [ p.comm]; rw [ category.assoc]; rw [ q.comm]; rw [ ←category.assoc] }
...
by { rw [iso.conj_apply]; rw [ iso.eq_inv_comp], simp [f.hom.comm'] }
...
begin
 rw [←nat_iso.app_inv]; rw [ ←is_iso.iso.inv_hom],
 refine is_iso.inv_eq_of_hom_inv_id _,
 rw [category.comp_id]; rw [ nat_iso.app_hom]; rw [ monoidal_functor.μ_iso_hom]; rw [ functor_category_monoidal_equivalence.μ_app],
end
...
begin
 ...
 rw [nat_iso.is_iso_inv_app]; rw [ category.id_comp],
 exact is_iso.inv_id,
end
...
{ obj := λ M,
 ...
 map := λ M N f,
 { hom := F.map f.hom,
 comm' := λ g, by { dsimp, rw [←F.map_comp]; rw [ f.comm]; rw [ F.map_comp], }, },
...
{ ε :=
 { hom := F.ε,
 comm' := λ g,
 by { dsimp, erw [category.id_comp]; erw [ category_theory.functor.map_id]; erw [ category.comp_id], }, },
 μ := λ X Y,
 { hom := F.μ X.V Y.V,
