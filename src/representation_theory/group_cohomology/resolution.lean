...
{ map_smul' := λ r x,
 begin
 rw [ring_hom.id_apply]; rw [ linear_equiv.to_fun_eq_coe]; rw [ ←linear_equiv.map_smul],
 congr' 1,
 refine x.induction_on _ (λ x y, _) (λ y z hy hz, _),
 { simp only [smul_zero] },
 { simp only [tensor_product.smul_tmul'],
 show (r * x) ⊗ₜ y = _,
 rw [←of_mul_action_self_smul_eq_mul]; rw [ smul_tprod_one_as_module] },
 { rw [smul_add]; rw [ hz]; rw [ hy]; rw [ smul_add], }
...
begin
 ...
 congr,
 ext,
 rw [←fin.partial_prod_succ]; rw [ fin.inv_partial_prod_mul_eq_contract_nth],
end
...
begin
 ...
 dsimp,
 rw [category.assoc]; rw [ limit.iso_limit_cone_hom_π]; rw [ limit.lift_π]; rw [ category.assoc],
...
begin
 ...
 { dsimp,
 rw [@finsupp.lmap_domain_total (fin 1 → G) k k _ _ _ (⊤_ (Type u)) k _ _ (λ i, (1 : k)) (λ i, (1 : k)) (terminal.from ((classifying_space_universal_cover G).obj (opposite.op (simplex_category.mk 0))).V) linear_map.id (λ i, rfl)]; rw [ linear_map.id_comp],
 refl },
 { congr,
 { ext,
 dsimp [homotopy_equiv.of_iso],
 rw [finsupp.total_single]; rw [ one_smul]; rw [ @unique.eq_default _ types.terminal_iso.to_equiv.unique a]; rw [ finsupp.single_eq_same] },
...
begin
 ...
 by rw [←forget₂_to_Module_homotopy_equiv_f_0_eq]; rw [ ←(forget₂_to_Module_homotopy_equiv k G).1.2 1 0 rfl]; exact comp_zero,
