...
by rw [char_mul_comm]; rw [ inv_mul_cancel_left]
...
by rw [←char_iso (dual_tensor_iso_lin_hom _ _)]; rw [ char_tensor]; rw [ pi.mul_apply]; rw [ char_dual]
...
begin
 ...
 rw [show (of (lin_hom W.ρ V.ρ)).ρ = lin_hom W.ρ V.ρ]; rw [ from fdRep.of_ρ (lin_hom W.ρ V.ρ)],
