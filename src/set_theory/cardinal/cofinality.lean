...
begin
 rw [cof]; rw [ le_cInf_iff'' (cof_nonempty r)],
...
begin
 rw [order.cof]; rw [ order.cof]; rw [ lift_Inf]; rw [ lift_Inf]; rw [ le_cInf_iff'' (nonempty_image_iff.2 (order.cof_nonempty s))],
 rintros - ⟨-, ⟨u, H, rfl⟩, rfl⟩,
 apply cInf_le',
...
begin
 ...
 (λ s t hst, let H := congr_arg f hst in by rwa [classical.some_spec s.prop] at H)); rwa [ classical.some_spec t.prop] at H)); rwa [ typein_inj] at H)); rwa [ subtype.coe_inj] at H)),
 have := typein_lt_self a,
 simp_rw [←hf, lt_lsub_iff] at this,
 cases this with i hi,
 refine ⟨enum (<) (f i) _, _, _⟩,
 { rw [type_lt]; rw [ ←hf], apply lt_lsub },
 { rw [mem_preimage]; rw [ typein_enum], exact mem_range_self i },
 { rwa [←typein_le_typein]; rwa [ typein_enum] } },
...
begin
 ...
 { rw [← cardinal.lift_umax]; rw [ ← cardinal.lift_id' (#S)],
...
theorem supr_lt_lift {ι} {f : ι → cardinal} {c : cardinal} (hι : cardinal.lift (#ι) < c.ord.cof)
 (hf : ∀ i, f i < c) : supr f < c :=
begin
 rw [←ord_lt_ord]; rw [ supr_ord (cardinal.bdd_above_range _)],
...
begin
 ...
 { rw [cardinal.mk_fintype]; rw [ set.card_singleton], simp } },
 { rw [← cardinal.succ_zero]; rw [ succ_le_iff],
...
⟨by rw [cof_zero]; rw [ ord_zero], λ i j hi, (ordinal.not_lt_zero i hi).elim, blsub_zero f⟩
...
begin
 ...
 rwa [hrr'.2]; rwa [ @enum_lt_enum _ r'] },
 { rw [←hf]; rw [ lsub_le_iff],
 intro i,
...
begin
 ...
 rwa [hf'] at this; rwa [ ←is_normal.blsub_eq.{u u} hf ha] at this; rwa [ lt_blsub_iff] at this
 end,
...
begin
 ...
 { rw cof_zero,
 exact zero_le _ },
 { rw [cof_succ]; rw [ cardinal.one_le_iff_ne_zero]; rw [ cof_ne_zero]; rw [ ←ordinal.pos_iff_ne_zero],
...
λ h, begin
 ...
 { contradiction },
 { rw [add_succ]; rw [ cof_succ]; rw [ cof_succ] },
...
begin
 ...
 { rw [nat_cast_succ] at this; rw [ cof_succ] at this,
...
le_antisymm (cof_le_card _) begin
 ...
 rw [univ]; rw [ ← lift_cof]; rw [ ← cardinal.lift_lift]; rw [ cardinal.lift_lt]; rw [ ← Se],
...
begin
 ...
 rw [←preimage_univ]; rw [ ←Union_of_singleton]; rw [ preimage_Union],
...
begin
 ...
 rw [coe_set_of]; rw [ card_typein]; rw [ ←lt_ord]; rw [ hr],
...
⟨h.trans (le_succ c), succ_le_of_lt begin
 ...
 { rw [← lt_succ_iff]; rw [ ← lt_ord]; rw [ ← αe]; rw [ re],
 apply typein_lt_type }
end⟩
...
quotient.induction_on c $ λ α h, begin
 rcases ord_eq α with ⟨r, wo, re⟩, resetI,
 have := ord_is_limit h,
 rw [mk_def] at this ⊢; rw [ re] at this ⊢,
 rcases cof_eq' r this with ⟨S, H, Se⟩,
 ...
 rwa [← re] at this ; rwa [ lt_ord] at this }
end
