...
by { transitivity _, rw [←quotient.out_eq c]; rw [ ←quotient.out_eq c'], refl }
...
begin
 ...
 rwa [mk_congr e] at hα ; rwa [ fintype.card_congr e] at hα },
 { refl },
...
{ zero := 0,
 ...
 npow_succ' := λ n c, show c ^ (n + 1) = c * c ^ n, by rw [power_add]; rw [ power_one]; rw [ mul_comm'] }

theorem power_bit0 (a b : cardinal) : a ^ (bit0 b) = a ^ b * a ^ b :=
power_add

theorem power_bit1 (a b : cardinal) : a ^ (bit1 b) = a ^ b * a ^ b * a :=
by rw [bit1]; rw [ ←power_bit0]; rw [ power_add]; rw [ power_one]
...
by { by_cases h : c = 0, rw [h]; rw [ power_zero], rw [zero_power h], apply zero_le }
...
by rw [← not_le]; rw [ le_one_iff_subsingleton]; rw [ ← not_nontrivial_iff_subsingleton]; rw [ not_not]
...
equiv.cardinal_eq $ equiv.ulift.trans $ equiv.sigma_congr_right $ λ a, nonempty.some $
 by rw [←lift_mk_eq]; rw [ mk_out]; rw [ mk_out]; rw [ lift_lift]

theorem sum_le_sum {ι} (f g : ι → cardinal) (H : ∀ i, f i ≤ g i) : sum f ≤ sum g :=
⟨(embedding.refl _).sigma_map $ λ i, classical.choice $
 by have := H i; rwa [← quot.out_eq (f i)] at this⟩; rwa [ ← quot.out_eq (g i)] at this⟩
...
theorem sum_le_supr_lift {ι : Type u} (f : ι → cardinal.{max u v}) :
 sum f ≤ (#ι).lift * supr f :=
begin
 rw [←(supr f).lift_id]; rw [ ←lift_umax]; rw [ lift_umax.{(max u v) u}]; rw [ ←sum_const],
...
by rw [← lift_umax']; rw [ lift_mk_shrink.{(max u v) v 0} α]; rw [ ← lift_umax]; rw [ lift_id]
...
⟨embedding.Pi_congr_right $ λ i, classical.choice $
 by have := H i; rwa [← mk_out (f i)] at this⟩; rwa [ ← mk_out (g i)] at this⟩
...
begin
 ...
 { intros α hα h f,
 rw [cardinal.prod]; rw [ mk_congr equiv.pi_option_equiv_prod]; rw [ mk_prod]; rw [ lift_umax']; rw [ mk_out]; rw [ ←cardinal.prod]; rw [ lift_prod]; rw [ fintype.prod_option]; rw [ lift_mul]; rw [ ←h (λ a, f (some a))],
 simp only [lift_id] },
end
...
theorem lift_down {a : cardinal.{u}} {b : cardinal.{max u v}} :
 b ≤ lift a → ∃ a', lift a' = b :=
induction_on₂ a b $ λ α β,
by rw [← lift_id (#β)]; rw [ ← lift_umax]; rw [ ← lift_umax.{u v}]; rw [ lift_mk_le]; exact
...
by rw [←lift_lift]; rw [ ←lift_lift]; rw [ lift_inj]
...
by rw [supr]; rw [ supr]; rw [ lift_Sup hf]; rw [ ←range_comp]
...
begin
 rw [lift_supr hf]; rw [ lift_supr hf'],
...
by rw [←lift_nat_cast.{v} n]; rw [ lift_inj]
...
by rw [← lift_mk_fin]; rw [ ← lift_mk_fin]; rw [ lift_le]; rw [ le_def]; rw [ function.embedding.nonempty_iff_card_le]; rw [ fintype.card_fin]; rw [ fintype.card_fin]
...
by rw [←succ_zero]; rw [ succ_le_iff]

theorem one_le_iff_ne_zero {c : cardinal} : 1 ≤ c ↔ c ≠ 0 :=
by rw [one_le_iff_pos]; rw [ pos_iff_ne_zero]

theorem nat_lt_aleph_0 (n : ℕ) : (n : cardinal.{u}) < ℵ₀ :=
succ_le_iff.1 begin
 rw [←nat_succ]; rw [ ←lift_mk_fin]; rw [ aleph_0]; rw [ lift_mk_le.{0 0 u}],
...
by rw [← lift_mk_fin]; rw [ ← lift_uzero (#α)]; rw [ lift_mk_eq']
...
by rw [countable_iff_nonempty_embedding]; rw [ aleph_0]; rw [ ← lift_uzero (#α)]; rw [ lift_mk_le']
...
by rw [mk_le_aleph_0_iff]; rw [ countable_coe_iff]
...
begin
 ...
 { simp },
 rw [succ_nsmul]; rw [ add_lt_aleph_0_iff]; rw [ ih]; rw [ and_self]
end
...
begin
 ...
 right, rw [←ne] at ha hb; rw [ ←one_le_iff_ne_zero] at ha hb, split,
 { rw ←mul_one a,
...
by rwa [not_lt] at h; rwa [ not_or_distrib] at h; rwa [ not_or_distrib] at h; rwa [ not_and_distrib] at h; rwa [ not_lt] at h; rwa [ not_lt] at h
...
by rw [← not_lt]; rw [ lt_aleph_0_iff_finite]; rw [ not_finite_iff_infinite]
...
le_antisymm (lift_mk_fin n ▸ mk_le_aleph_0) $ le_mul_of_one_le_left (zero_le _) $
 by rwa [← nat.cast_one]; rwa [ nat_cast_le]; rwa [ nat.one_le_iff_ne_zero]

@[simp] lemma aleph_0_mul_nat {n : ℕ} (hn : n ≠ 0) : ℵ₀ * n = ℵ₀ :=
by rw [mul_comm]; rw [ nat_mul_aleph_0 hn]
...
@[simp] lemma nat_add_aleph_0 (n : ℕ) : ↑n + ℵ₀ = ℵ₀ := by rw [add_comm]; rw [ aleph_0_add_nat]
...
⟨λ c, if h : c < aleph_0.{v} then classical.some (lt_aleph_0.1 h) else 0,
 ...
 rw [dif_pos h]; rw [ ← cardinal.nat_cast_inj]; rw [ ← classical.some_spec (lt_aleph_0.1 h)]; rw [ nat.cast_zero],
 end⟩
...
by rw [to_nat_apply_of_lt_aleph_0 h]; rw [ ← classical.some_spec (lt_aleph_0.1 h)]

lemma cast_to_nat_of_aleph_0_le {c : cardinal} (h : ℵ₀ ≤ c) : ↑c.to_nat = (0 : cardinal) :=
by rw [to_nat_apply_of_aleph_0_le h]; rw [ nat.cast_zero]

lemma to_nat_eq_iff_eq_of_lt_aleph_0 {c d : cardinal} (hc : c < ℵ₀) (hd : d < ℵ₀) :
 c.to_nat = d.to_nat ↔ c = d :=
by rw [←nat_cast_inj]; rw [ cast_to_nat_of_lt_aleph_0 hc]; rw [ cast_to_nat_of_lt_aleph_0 hd]

lemma to_nat_le_iff_le_of_lt_aleph_0 {c d : cardinal} (hc : c < ℵ₀) (hd : d < ℵ₀) :
 c.to_nat ≤ d.to_nat ↔ c ≤ d :=
by rw [←nat_cast_le]; rw [ cast_to_nat_of_lt_aleph_0 hc]; rw [ cast_to_nat_of_lt_aleph_0 hd]

lemma to_nat_lt_iff_lt_of_lt_aleph_0 {c d : cardinal} (hc : c < ℵ₀) (hd : d < ℵ₀) :
 c.to_nat < d.to_nat ↔ c < d :=
by rw [←nat_cast_lt]; rw [ cast_to_nat_of_lt_aleph_0 hc]; rw [ cast_to_nat_of_lt_aleph_0 hd]
...
begin
 rw [to_nat_apply_of_lt_aleph_0 (nat_lt_aleph_0 n)]; rw [ ← nat_cast_inj],
...
by rw [←to_nat_cast 0]; rw [ nat.cast_zero]

@[simp] lemma one_to_nat : to_nat 1 = 1 :=
by rw [←to_nat_cast 1]; rw [ nat.cast_one]
...
by rw [to_nat_eq_iff one_ne_zero]; rw [ nat.cast_one]
...
begin
 ...
 { rw [cast_to_nat_of_lt_aleph_0]; rw [ ←lift_nat_cast]; rw [ cast_to_nat_of_lt_aleph_0 hc],
 rwa [lift_lt_aleph_0] },
 { rw [cast_to_nat_of_aleph_0_le]; rw [ ←lift_nat_cast]; rw [ cast_to_nat_of_aleph_0_le hc]; rw [ lift_zero],
 rwa [aleph_0_le_lift] },
end

lemma to_nat_congr {β : Type v} (e : α ≃ β) : (#α).to_nat = (#β).to_nat :=
by rw [←to_nat_lift]; rw [ lift_mk_eq.mpr ⟨e⟩]; rw [ to_nat_lift]

@[simp] lemma to_nat_mul (x y : cardinal) : (x * y).to_nat = x.to_nat * y.to_nat :=
begin
 rcases eq_or_ne x 0 with rfl | hx1,
 { rw [zero_mul]; rw [ zero_to_nat]; rw [ zero_mul] },
 rcases eq_or_ne y 0 with rfl | hy1,
 { rw [mul_zero]; rw [ zero_to_nat]; rw [ mul_zero] },
 cases lt_or_le x ℵ₀ with hx2 hx2,
 { cases lt_or_le y ℵ₀ with hy2 hy2,
 { lift x to ℕ using hx2, lift y to ℕ using hy2,
 rw [← nat.cast_mul]; rw [ to_nat_cast]; rw [ to_nat_cast]; rw [ to_nat_cast] },
 { rw [to_nat_apply_of_aleph_0_le hy2]; rw [ mul_zero]; rw [ to_nat_apply_of_aleph_0_le],
 exact aleph_0_le_mul_iff'.2 (or.inl ⟨hx1, hy2⟩) } },
 { rw [to_nat_apply_of_aleph_0_le hx2]; rw [ zero_mul]; rw [ to_nat_apply_of_aleph_0_le],
...
begin
 ...
 rw [nat.cast_add]; rw [ ←to_nat_lift.{v u} a]; rw [ ←to_nat_lift.{u v} b]; rw [ cast_to_nat_of_lt_aleph_0 ha]; rw [ cast_to_nat_of_lt_aleph_0 hb]; rw [ cast_to_nat_of_lt_aleph_0 (add_lt_aleph_0 ha hb)]
end
...
{ to_fun := λ c, if c < ℵ₀ then c.to_nat else ⊤,
 ...
 rw [← nat.cast_add]; rw [ to_nat_cast]; rw [ nat.cast_add] },
 { rw [if_neg hy]; rw [ if_neg]; rw [ part_enat.add_top],
 contrapose! hy,
 apply le_add_self.trans_lt hy } },
 { rw [if_neg hx]; rw [ if_neg]; rw [ part_enat.top_add],
 contrapose! hx,
...
by rw [to_part_enat_apply_of_lt_aleph_0 (nat_lt_aleph_0 n)]; rw [ to_nat_cast]
...
rw [le_antisymm_iff]; rw [ le_antisymm_iff]; rw [ to_part_enat_le_iff_le_of_le_aleph_0 hc]; rw [ to_part_enat_le_iff_le_of_le_aleph_0 hc']
...
begin
 ...
 rw to_part_enat_apply_of_aleph_0_le hc',
 exact le_top,
 rw [to_part_enat_apply_of_aleph_0_le hc]; rw [ to_part_enat_apply_of_aleph_0_le (le_trans hc h)],
end
...
begin
 cases lt_or_ge c ℵ₀ with hc hc,
 { rw [to_part_enat_apply_of_lt_aleph_0 hc]; rw [ cardinal.to_part_enat_apply_of_lt_aleph_0 _],
 simp only [to_nat_lift],
 rw [← lift_aleph_0]; rw [ lift_lt], exact hc },
 { rw [to_part_enat_apply_of_aleph_0_le hc]; rw [ cardinal.to_part_enat_apply_of_aleph_0_le _],
 rw [← lift_aleph_0]; rw [ lift_le], exact hc }
end

lemma to_part_enat_congr {β : Type v} (e : α ≃ β) : (#α).to_part_enat = (#β).to_part_enat :=
by rw [←to_part_enat_lift]; rw [ lift_mk_eq.mpr ⟨e⟩]; rw [ to_part_enat_lift]
...
by rw [← mk_univ]; rw [ mk_set_eq_nat_iff_finset]
...
by { rw [← union_singleton]; rw [ mk_union_of_disjoint]; rw [ mk_singleton], simpa }
...
by rw [← nat.cast_two]; rw [ nat_succ]; rw [ succ_le_iff]; rw [ nat.cast_one]; rw [ one_lt_iff_nontrivial]; rw [ nontrivial_iff]

lemma two_le_iff' (x : α) : (2 : cardinal) ≤ #α ↔ ∃y : α, y ≠ x :=
by rw [two_le_iff]; rw [ ← nontrivial_iff]; rw [ nontrivial_iff_exists_ne x]
...
begin
 ...
 have : ↑(2 : ℕ) < # α, rwa [← succ_le_iff]; rwa [ ← cardinal.nat_succ],
