...
begin
 ...
 have hg'ns : g' '' sᶜ = (f '' s)ᶜ, by rw [← hns]; rw [ g'g.image_image],
 set h : α → β := s.piecewise f g',
 have : surjective h, by rw [← range_iff_surjective]; rw [ range_piecewise]; rw [ hg'ns]; rw [ union_compl_self],
 have : injective h,
 ...
 rw [g'g _] at hxy; rw [ g'g _] at hxy, rw hxy },
 { intros x hx y hy hxy,
 obtain ⟨y', hy', rfl⟩ : y ∈ g '' (f '' s)ᶜ, by rwa hns,
 rw [g'g _] at hxy,
 exact hy' ⟨x, hx, hxy⟩ } },
...
⟨i, ⟨λ j, ⟨λ a, f a j, λ a b e',
 ...
 by rw [← ea]; rw [ ← eb]; rw [ hs _ sa _ sb _ e']⟩⟩⟩
