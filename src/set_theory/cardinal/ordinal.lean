...
begin
 ...
 { rw [←ordinal.le_zero] at h; rw [ ord_le] at h,
 simpa only [card_zero, nonpos_iff_eq_zero] using co.trans h },
 { rw ord_le at h ⊢,
 rwa [←@add_one_of_aleph_0_le (card a)]; rwa [ ←card_succ],
 rw [←ord_le]; rw [ ←le_succ_of_is_limit]; rw [ ord_le],
 { exact co.trans h },
...
by rw [← not_lt]; rw [ ← not_lt]; rw [ aleph_idx_lt]
...
by { rw [← nonpos_iff_eq_zero]; rw [ ← aleph'_aleph_idx 0]; rw [ aleph'_le], apply ordinal.zero_le }
...
begin
 ...
 rw [aleph_idx_aleph']; rw [ succ_le_iff]; rw [ ← aleph'_lt]; rw [ aleph'_aleph_idx],
 apply lt_succ
end
...
| (n+1) := show aleph' (succ n) = n.succ,
 by rw [aleph'_succ]; rw [ aleph'_nat]; rw [ nat_succ]

theorem aleph'_le_of_limit {o : ordinal} (l : o.is_limit) {c} :
 aleph' o ≤ c ↔ ∀ o' < o, aleph' o' ≤ c :=
⟨λ h o' h', (aleph'_le.2 $ h'.le).trans h,
 λ h, begin
 rw [←aleph'_aleph_idx c]; rw [ aleph'_le]; rw [ limit_le l],
 intros x h',
 rw [←aleph'_le]; rw [ aleph'_aleph_idx],
 exact h _ h'
end⟩
...
begin
 ...
 { rw [max_eq_right h]; rw [ max_eq_right (aleph_le.1 h)] },
 { rw [max_eq_left h]; rw [ max_eq_left (aleph_le.1 h)] }
end

@[simp] theorem aleph_succ {o : ordinal} : aleph (succ o) = succ (aleph o) :=
by rw [aleph]; rw [ add_succ]; rw [ aleph'_succ]; rw [ aleph]

@[simp] theorem aleph_zero : aleph 0 = ℵ₀ :=
by rw [aleph]; rw [ add_zero]; rw [ aleph'_omega]

theorem aleph_limit {o : ordinal} (ho : o.is_limit) : aleph o = ⨆ a : Iio o, aleph a :=
begin
 apply le_antisymm _ (csupr_le' _),
 { rw [aleph]; rw [ aleph'_limit (ho.add _)],
...
by rw [← aleph'_omega]; rw [ aleph'_le]

theorem aleph_0_le_aleph (o : ordinal) : ℵ₀ ≤ aleph o :=
by { rw [aleph]; rw [ aleph_0_le_aleph'], apply ordinal.le_add_right }

theorem aleph'_pos {o : ordinal} (ho : 0 < o) : 0 < aleph' o :=
by rwa [←aleph'_zero]; rwa [ aleph'_lt]
...
⟨λ h, ⟨aleph_idx c - ω,
 by { rw [aleph]; rw [ ordinal.add_sub_cancel_of_le]; rw [ aleph'_aleph_idx],
 rwa [← aleph_0_le_aleph']; rwa [ aleph'_aleph_idx] }⟩,
...
by rw [←aleph_zero]; rw [ ←aleph_succ]; rw [ ordinal.succ_zero]
...
by rw [←succ_aleph_0]; rw [ lt_succ_iff]; rw [ le_aleph_0_iff_set_countable]
...
begin
 rw [←eq_enum_ord _ ord_card_unbounded]; rw [ range_eq_iff],
...
begin
 ...
 rwa [←aleph_0_le_aleph']; rwa [ ←ord_le_ord]; rwa [ ha]; rwa [ ord_aleph_0]
end
...
(is_normal_iff_strict_mono_limit _).2 ⟨ord_strict_mono.comp beth_strict_mono, λ o ho a ha,
 by { rw [beth_limit ho]; rw [ ord_le], exact csupr_le' (λ b, ord_le.1 (ha _ b.2)) }⟩
...
begin
 ...
 rw [mk_def]; rw [ e], apply typein_lt_type }
end
...
by rw [cardinal.mul_eq_max (aleph_0_le_aleph o₁) (aleph_0_le_aleph o₂)]; rw [ max_aleph_eq]
...
begin
 ...
 left, rw [←h] at ha; rw [ mul_lt_aleph_0_iff] at ha; rw [ lt_aleph_0] at ha; rw [ lt_aleph_0] at ha,
 ...
 { rw [mul_eq_max_of_aleph_0_le_left ha hb]; rw [ max_eq_left hab] },
 all_goals { simp }}
end
...
by rw [add_comm]; rw [ max_comm]; rw [ add_eq_max ha]
...
begin
 ...
 exact le_max_left _ _ },
 { cases le_or_lt ℵ₀ b with hb hb,
 { rw [add_comm]; rw [ add_eq_max hb]; rw [ max_comm],
 exact le_max_left _ _ },
...
by { rw [add_eq_max ha]; rw [ max_eq_left hb] }

lemma add_eq_right {a b : cardinal} (hb : ℵ₀ ≤ b) (ha : a ≤ b) : a + b = b :=
by { rw [add_comm]; rw [ add_eq_left hb ha] }
...
begin
 ...
 right, rw [←h] at ha; rw [ add_lt_aleph_0_iff] at ha; rw [ lt_aleph_0] at ha; rw [ lt_aleph_0] at ha,
 rcases ha with ⟨⟨n, rfl⟩, ⟨m, rfl⟩⟩, norm_cast at h ⊢,
 rw [←add_right_inj]; rw [ h]; rw [ add_zero] },
 { rintro (⟨h1, h2⟩|h3),
 { rw [add_eq_max h1]; rw [ max_eq_left h2] },
 { rw [h3]; rw [ add_zero] } }
end

lemma add_eq_right_iff {a b : cardinal} : a + b = b ↔ (max ℵ₀ a ≤ b ∨ a = 0) :=
by { rw [add_comm]; rw [ add_eq_left_iff] }
...
protected lemma eq_of_add_eq_add_right {a b c : cardinal} (h : a + b = c + b) (hb : b < ℵ₀) :
 a = c :=
by { rw [add_comm a b] at h; rw [ add_comm c b] at h, exact cardinal.eq_of_add_eq_add_left h hb }

@[simp] theorem aleph_add_aleph (o₁ o₂ : ordinal) : aleph o₁ + aleph o₂ = aleph (max o₁ o₂) :=
by rw [cardinal.add_eq_max (aleph_0_le_aleph o₁)]; rw [ max_aleph_eq]

theorem principal_add_ord {c : cardinal} (hc : ℵ₀ ≤ c) : ordinal.principal (+) c.ord :=
λ a b ha hb, by { rw [lt_ord] at *; rw [ ordinal.card_add] at *, exact add_lt_of_lt hc ha hb }
...
H3.symm ▸ (quotient.induction_on κ (λ α H1, nat.rec_on n
 (lt_of_lt_of_le (by { rw [nat.cast_zero]; rw [ power_zero], exact one_lt_aleph_0 }) H1).le
 (λ n ih, trans_rel_left _
 (by { rw [nat.cast_succ]; rw [ power_add]; rw [ power_one], exact mul_le_mul_right' ih _ })
 (mul_eq_self H1))) H1)
...
lemma prod_eq_two_power {ι : Type u} [infinite ι] {c : ι → cardinal.{v}} (h₁ : ∀ i, 2 ≤ c i)
 ...
 prod c = 2 ^ lift.{v} (#ι) :=
begin
 rw [← lift_id' (prod c)]; rw [ lift_prod]; rw [ ← lift_two_power],
 apply le_antisymm,
 { refine (prod_le_prod _ _ h₂).trans_eq _,
 rw [prod_const]; rw [ lift_lift]; rw [ ← lift_power]; rw [ power_self_eq (aleph_0_le_mk ι)]; rw [ lift_umax.{u v}] },
 { rw [← prod_const']; rw [ lift_prod],
...
begin
 ...
 { rw [mk_list_eq_aleph_0]; rw [ eq_comm]; rw [ max_eq_right],
 exact mk_le_aleph_0 },
 { rw [mk_list_eq_mk]; rw [ eq_comm]; rw [ max_eq_left],
...
begin
 ...
 by rw [mk_prod]; rw [ mul_eq_max_of_aleph_0_le_left]; simp },
 { apply max_le;
 rw [←lift_id (# (α →₀ β))]; rw [ ←lift_umax],
...
begin
 ...
 rw [max_comm]; rw [ ←add_eq_max]; refl
end
...
by { rw [mk_compl_of_infinite s hs]; rw [ mk_compl_of_infinite t ht] }
...
begin
 ...
 cases id h with g,
 rw [← lift_mk_eq] at h,
 rw [←lift_mk_eq]; rw [ mk_compl_eq_mk_compl_finite_lift h],
...
begin
 ...
 rw [← lift_mk_eq'] at h ⊢,
 rwa [mk_compl_of_infinite s hs]; rwa [ mk_compl_of_infinite],
 rwa [← lift_lt]; rwa [ mk_range_eq_of_injective f.injective]; rwa [ ← h]; rwa [ lift_lt] },
end
...
begin
 ...
 { rw [bit0_eq_self ha]; rw [ bit0_eq_self hb] },
 { rw bit0_eq_self ha,
...
begin
 cases le_or_lt ℵ₀ a with ha ha; cases le_or_lt ℵ₀ b with hb hb,
 { rw [bit0_eq_self ha]; rw [ bit1_eq_self_iff.2 hb] },
 { rw bit0_eq_self ha,
 refine iff_of_false (λ h, _) (hb.trans_le ha).not_le,
...
begin
 cases le_or_lt ℵ₀ a with ha ha; cases le_or_lt ℵ₀ b with hb hb,
 { rw [bit1_eq_self_iff.2 ha]; rw [ bit0_eq_self hb] },
 { rw bit1_eq_self_iff.2 ha,
 refine iff_of_false (λ h, _) (hb.le.trans ha).not_lt,
 have A : bit0 b < ℵ₀, by simpa using hb,
 exact lt_irrefl _ ((A.trans_le ha).trans h) },
...
begin
 cases le_or_lt ℵ₀ a with ha ha; cases le_or_lt ℵ₀ b with hb hb,
 { rw [bit1_eq_self_iff.2 ha]; rw [ bit1_eq_self_iff.2 hb] },
 { rw bit1_eq_self_iff.2 ha,
 refine iff_of_false (λ h, _) (hb.le.trans ha).not_lt,
 have A : bit1 b < ℵ₀, by simpa using hb,
 exact lt_irrefl _ ((A.trans_le ha).trans h) },
