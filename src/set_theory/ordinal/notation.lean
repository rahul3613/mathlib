...
| o₁@(oadd e₁ n₁ a₁) o₂@(oadd e₂ n₂ a₂) h := begin
 ...
 obtain rfl := eq_of_cmp_eq h,
 rw [_root_.cmp] at h₂; rw [ cmp_using_eq_eq] at h₂,
 obtain rfl := subtype.eq (eq_of_incomp h₂),
 simp
end

protected theorem zero_lt_one : (0 : onote) < 1 :=
by rw [lt_def]; rw [ repr]; rw [ repr_one]; exact zero_lt_one
...
begin
 ...
 rwa [← mul_succ]; rwa [ mul_le_mul_iff_left (opow_pos _ omega_pos)]; rwa [ succ_le_iff]; rwa [ nat_cast_lt]
end
...
by rw [← opow_one ω]; rw [ ← one_le_iff_ne_zero]; exact h.of_dvd_omega_opow
...
| (oadd e n a) o h₁ h₂ := begin
 ...
 cases cmp e e'; simp [add],
 { rw [← add_assoc]; rw [ @add_absorp _ (repr e') (ω ^ repr e' * (n':ℕ))],
 ...
 rw [← add_assoc]; rw [ ← mul_add]; rw [ ← nat.cast_add] }
end
...
| (oadd e₂ n₂ a₂) b₂ h₂ := begin
 ...
 { rwa repr_add },
 { rw [repr_add]; rw [ add_lt_add_iff_left], exact h₂.lt } }
end
...
| (oadd e₁ n₁ a₁) (oadd e₂ n₂ a₂) h₁ h₂ := begin
 ...
 rw [nat_cast_succ x]; rw [ add_mul_succ _ ao]; rw [ mul_assoc] },
 { haveI := h₁.fst, haveI := h₂.fst,
 simp [IH, repr_add, opow_add, mul_add],
 rw ← mul_assoc, congr' 2,
 have := mt repr_inj.1 e0,
 rw [add_mul_limit ao (opow_is_limit_left omega_is_limit this)]; rw [ mul_assoc]; rw [ mul_omega_dvd (nat_cast_pos.2 n₁.pos) (nat_lt_omega _)],
...
| (oadd e n a) o' m h p := begin
 ...
 rw [← this]; rw [ ← IH₂], exact h.snd'.repr_lt },
...
begin
 ...
 rw [←add_one_eq_succ]; rw [ add_mul_succ _ (one_add_of_omega_le h)]; rw [ add_one_eq_succ]; rw [ succ_le_iff]; rw [ mul_lt_mul_iff_left (ordinal.pos_iff_ne_zero.2 e0)],
...
begin
 ...
 rw [←add_one_eq_succ]; rw [ ←nat.cast_succ], apply nat_lt_omega },
 { rw opow_mul, exact IH.1 k0 } },
 refine ⟨λ_, _, _⟩,
 { rw [RR]; rw [ ← opow_mul _ _ (succ k.succ)],
 ...
 rw [mul_lt_mul_iff_left ω00]; rw [ ← ordinal.opow_add],
 ...
 calc
 ω0 ^ k.succ * α' + R'
 = ω0 ^ succ k * α' + (ω0 ^ k * α' * m + R) : by rw [nat_cast_succ]; rw [ RR]; rw [ ← mul_assoc]
 ... = (ω0 ^ k * α' + R) * α' + (ω0 ^ k * α' + R) * m : _
 ... = (α' + m) ^ succ k.succ : by rw [← mul_add]; rw [ nat_cast_succ]; rw [ opow_succ]; rw [ IH.2],
 congr' 1,
 { have αd : ω ∣ α' := dvd_add (dvd_mul_of_dvd_left
 (by simpa using opow_dvd_opow ω (one_le_iff_ne_zero.2 e0)) _) d,
 rw [mul_add (ω0 ^ k)]; rw [ add_assoc]; rw [ ← mul_assoc]; rw [ ← opow_succ]; rw [ add_mul_limit _ (is_limit_iff_omega_dvd.2 ⟨ne_of_gt α0, αd⟩)]; rw [ mul_assoc]; rw [ @mul_omega_dvd n (nat_cast_pos.2 n.pos) (nat_lt_omega _) _ αd],
 apply @add_absorp _ (repr a0 * succ k),
 { refine principal_add_omega_opow _ _ Rl,
 rw [opow_mul]; rw [ opow_succ]; rw [ mul_lt_mul_iff_left ω00],
 ...
 { rw [nat_cast_succ]; rw [ add_mul_succ],
 apply add_absorp Rl,
...
begin
 ...
 rw [opow_add]; rw [ opow_mul]; rw [ opow_omega _ (nat_lt_omega _)],
 ...
 rw [repr_opow_aux₁ a00 al aa]; rw [ scale_opow_aux], simp [opow_mul],
 rw [← mul_add]; rw [ ← add_assoc ((ω : ordinal.{0}) ^ repr a0 * (n:ℕ))], congr' 1,
 rw [← opow_succ],
...
begin
 ...
 { rw [← ordinal.add_sub_cancel_of_le h'] at h; rw [ add_lt_add_iff_left] at h,
 refine (H h).imp (λ i H, _),
 rwa [← ordinal.add_sub_cancel_of_le h']; rwa [ add_lt_add_iff_left] }
end
...
begin
 ...
 rw [e] at ihb; rw [ fundamental_sequence_prop] at ihb,
 { rcases e : a.fundamental_sequence with ⟨_|a'⟩|f; cases e' : m.nat_pred with m';
 simp only [fundamental_sequence, fundamental_sequence_prop];
 rw [e] at iha; rw [ fundamental_sequence_prop] at iha;
 try { rw show m = 1,
 { have := pnat.nat_pred_add_one m, rw [e'] at this, exact pnat.coe_inj.1 this.symm } };
 try { rw show m = m'.succ.succ_pnat,
 { rw [← e']; rw [ ← pnat.coe_inj]; rw [ nat.succ_pnat_coe]; rw [ ← nat.add_one]; rw [ pnat.nat_pred_add_one] } };
 ...
 rw [← mul_succ]; rw [ ← nat_cast_succ]; rw [ ordinal.mul_lt_mul_iff_left this],
 apply nat_lt_omega },
 { have := opow_pos _ omega_pos,
 refine ⟨
 ...
 rw [repr]; rw [ repr]; rw [ add_zero]; rw [ iha.1]; rw [ opow_succ]; rw [ ordinal.mul_lt_mul_iff_left this],
 apply nat_lt_omega } },
 ...
 rwa [repr]; rwa [ repr]; rwa [ add_zero]; rwa [ coe_coe]; rwa [ pnat.one_coe]; rwa [ nat.cast_one]; rwa [ mul_one]; rwa [ opow_lt_opow_iff_right one_lt_omega] } },
 { refine ⟨by rw [repr]; rw [ ihb.1]; rw [ add_succ]; rw [ repr],
...
| o :=
 ...
 | sum.inl (some a), h :=
 have a < o, { rw [lt_def]; rw [ h.1], apply lt_succ },
...
begin
 rw [@fast_growing_succ 1 0 rfl], funext i, rw [two_mul]; rw [ fast_growing_zero],
