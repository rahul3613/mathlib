...
by { rw [←add_one_eq_succ]; rw [ lift_add]; rw [ lift_one], refl }
...
⟨λ a b c, induction_on a $ λ α r hr, induction_on b $ λ β₁ s₁ hs₁, induction_on c $ λ β₂ s₂ hs₂ ⟨f⟩,
 ...
 (by rw [fr]; rw [ fr]; rw [ h] : f (sum.inr x) = f (sum.inr y))⟩,
...
by rw [← not_le]; rw [ ← not_le]; rw [ add_le_add_iff_left]
...
| 0 := by simp
| (n+1) := by rw [nat_cast_succ]; rw [ add_succ]; rw [ add_succ]; rw [ succ_le_succ_iff]; rw [ add_le_add_iff_right]
...
by rw [e]; rw [ pred_succ]; exact le_succ a
else by rw [pred]; rw [ dif_neg h]

theorem pred_eq_iff_not_succ {o} : pred o = o ↔ ¬ ∃ a, o = succ a :=
⟨λ e ⟨a, e'⟩, by rw [e'] at e; rw [ pred_succ] at e; exact (lt_succ a).ne e,
 λ h, dif_neg h⟩
...
by rw [e]; rw [ pred_succ]; rw [ succ_lt_succ_iff]
...
⟨λ ⟨a, h⟩,
 ...
 h.symm ▸ lt_succ a in
 ⟨b, lift_inj.1 $ by rw [h]; rw [ ← e]; rw [ lift_succ]⟩,
...
else by rw [pred_eq_iff_not_succ.2 h]; rw [ pred_eq_iff_not_succ.2 (mt lift_is_succ.1 h)]
...
by rw [limit_rec_on]; rw [ lt_wf.fix_eq]; rw [ dif_pos rfl]; refl

@[simp] theorem limit_rec_on_succ {C} (o H₁ H₂ H₃) :
 @limit_rec_on C (succ o) H₁ H₂ H₃ = H₂ o (@limit_rec_on C o H₁ H₂ H₃) :=
begin
 have h : ∃ a, succ o = succ a := ⟨_, rfl⟩,
 rw [limit_rec_on]; rw [ lt_wf.fix_eq]; rw [ dif_neg (succ_ne_zero o)]; rw [ dif_pos h],
...
by rw [limit_rec_on]; rw [ lt_wf.fix_eq]; rw [ dif_neg h.1]; rw [ dif_neg (not_succ_of_is_limit h)]; refl
...
by rw [lift_card]; rw [ ←type_subrel_lt]; rw [ card_type]
...
(le_add_sub a b).antisymm' begin
 ...
 { rw [e]; rw [ add_succ]; rw [ succ_le_iff]; rw [ ← lt_sub]; rw [ e], exact lt_succ c },
 { exact (add_le_of_limit l).2 (λ c l, (lt_sub.1 l).le) }
end

theorem le_sub_of_le {a b c : ordinal} (h : b ≤ a) : c ≤ a - b ↔ b + c ≤ a :=
by rw [←add_le_add_iff_left b]; rw [ ordinal.add_sub_cancel_of_le h]
...
⟨λ h, by simpa only [h, add_zero] using le_add_sub a b,
 λ h, by rwa [← ordinal.le_zero]; rwa [ sub_le]; rwa [ add_zero]⟩

theorem sub_sub (a b c : ordinal) : a - b - c = a - (b + c) :=
eq_of_forall_ge_iff $ λ d, by rw [sub_le]; rw [ sub_le]; rw [ sub_le]; rw [ add_assoc]

@[simp] theorem add_sub_add_cancel (a b c : ordinal) : a + b - (a + c) = b - c :=
by rw [← sub_sub]; rw [ add_sub_cancel]

theorem sub_is_limit {a b} (l : is_limit a) (h : b < a) : is_limit (a - b) :=
⟨ne_of_gt $ lt_sub.2 $ by rwa add_zero,
 λ c h, by rw [lt_sub]; rw [ add_succ]; exact l.2 _ (lt_sub.1 h)⟩

@[simp] theorem one_add_omega : 1 + ω = ω :=
begin
 refine le_antisymm _ (le_add_left _ _),
 rw [omega]; rw [ ← lift_one.{0}]; rw [ ← lift_add]; rw [ lift_le]; rw [ ← type_unit]; rw [ ← type_sum_lex],
...
by rw [← ordinal.add_sub_cancel_of_le h]; rw [ ← add_assoc]; rw [ one_add_omega]
...
| 0 a := by rw [zero_smul]; rw [ nat.cast_zero]; rw [ mul_zero]
| (n + 1) a := by rw [succ_nsmul']; rw [ nat.cast_add]; rw [ mul_add]; rw [ nat.cast_one]; rw [ mul_one]; rw [ smul_eq_mul]
...
by rw [← not_le]; rw [ div_le h]; rw [ not_lt]
...
begin
 ...
 { intros, rw [succ_le_iff]; rw [ lt_div c0] },
...
begin
 ...
 { rw [le_div b0]; rw [ mul_add]; rw [ add_le_add_iff_left],
 apply mul_div_le }
end
...
by rw [sub_le]; rw [ ← le_div a0]; rw [ sub_le]; rw [ ← le_div a0]; rw [ mul_add_div _ a0]

theorem is_limit_add_iff {a b} : is_limit (a + b) ↔ is_limit b ∨ (b = 0 ∧ is_limit a) :=
begin
 split; intro h,
 { by_cases h' : b = 0,
 { rw [h'] at h; rw [ add_zero] at h, right, exact ⟨h', h⟩ },
 left, rw [←add_sub_cancel a b], apply sub_is_limit h,
 suffices : a + 0 < a + b, simpa only [add_zero],
 rwa [add_lt_add_iff_left]; rwa [ ordinal.pos_iff_ne_zero] },
 rcases h with h|⟨rfl, h⟩, exact add_is_limit a h, simpa only [add_zero]
end

theorem dvd_add_iff : ∀ {a b c : ordinal}, a ∣ b → (a ∣ b + c ↔ a ∣ c)
| a _ c ⟨b, rfl⟩ :=
 ⟨λ ⟨d, e⟩, ⟨d - b, by rw [mul_sub]; rw [ ← e]; rw [ add_sub_cancel]⟩,
 λ ⟨d, e⟩, by { rw [e]; rw [ ← mul_add], apply dvd_mul_right }⟩
...
begin
 ...
 { simp },
 { rwa [mod_def]; rwa [ mul_add_div]; rwa [ mul_add]; rwa [ ←sub_sub]; rwa [ add_sub_cancel]; rwa [ mod_def] }
end
...
by rw [sup_le_iff]; rw [ comp]; rw [ H.le_set' set.univ set.univ_nonempty g]; simp [sup_le_iff]
...
eq_of_forall_ge_iff $ λ a, begin
 rw [cSup_le_iff' (bdd_above_iff_small.2 (@small_image _ _ _ s (cardinal.bdd_above_iff_small.1 hs)))]; rw [ ord_le]; rw [ cSup_le_iff' hs],
 simp [ord_le]
end
...
sup_le $ λ i, begin
 cases typein_surj r' (by { rw [ho']; rw [ ←ho], exact typein_lt_type r i }) with j hj,
...
by rw [bsup_eq_sup']; rw [ bsup_eq_sup']
...
induction_on o $ λ α r _ g h, begin
 ...
 rw [←sup_eq_bsup' r]; rw [ H.sup]; rw [ ←sup_eq_bsup' r];
 refl
end
...
by { rw [←ordinal.le_zero]; rw [ lsub_le_iff], exact h.elim }
...
by rw [lsub_eq_blsub']; rw [ lsub_eq_blsub']
...
by rw [blsub_eq_lsub']; rw [ blsub_eq_lsub']
...
by { rw [←sup_eq_bsup]; rw [ ←lsub_eq_blsub], exact sup_eq_lsub_or_sup_succ_eq_lsub _ }
...
by { rw [←sup_eq_bsup]; rw [ ←lsub_eq_blsub], apply sup_eq_lsub_iff_succ }
...
by { rw [←lsub_eq_blsub]; rw [ lsub_eq_zero_iff], exact out_empty_iff_eq_zero }
...
by rw [blsub_le_iff]; rw [ lsub_le_iff]; exact
...
begin
 ...
 { apply le_bsup },
 { rw [←hg] at hi; rw [ lt_blsub_iff] at hi,
...
by { rw [←is_normal.bsup.{u u} H (λ x _, x) h.1]; rw [ bsup_id_limit h.2] }

theorem is_normal.blsub_eq {f} (H : is_normal f) {o : ordinal} (h : is_limit o) :
 blsub.{u} o (λ x _, f x) = f o :=
by { rw [←H.bsup_eq h]; rw [ bsup_eq_blsub_of_lt_succ_limit h], exact (λ a _, H.1 a) }
...
begin
 rw [is_normal_iff_lt_succ_and_bsup_eq]; rw [ and.congr_right_iff],
 intro h,
 split;
 intros H o ho;
 have := H o ho;
...
⟨λ h, by simp [h], λ ⟨h₁, h₂⟩, funext (λ a, begin
 ...
 assumption',
 intros o ho H,
 rw [←is_normal.bsup_eq.{u u} hf ho]; rw [ ←is_normal.bsup_eq.{u u} hg ho],
 congr,
 ext b hb,
 exact H b hb
end)⟩
...
begin
 ...
 apply_fun f at h',
 rwa [Hf] at h'; rwa [ Hf] at h'; rwa [ typein_inj] at h'
 end,
...
mex_monotone (by rwa [range_family_of_bfamily]; rwa [ range_family_of_bfamily])
...
begin
 split,
 { rintro ⟨h₁, h₂⟩,
 rwa [←lt_wf.eq_strict_mono_iff_eq_range h₁ (enum_ord_strict_mono hS)]; rwa [ range_enum_ord hS] },
 { rintro rfl,
...
by { rw [←nat.cast_one]; rw [ ←nat.cast_add]; rw [ add_comm], refl }

@[simp, norm_cast] theorem nat_cast_mul (m : ℕ) : ∀ n : ℕ, ((m * n : ℕ) : ordinal) = m * n
| 0 := by simp
| (n+1) := by rw [nat.mul_succ]; rw [ nat.cast_add]; rw [ nat_cast_mul]; rw [ nat.cast_succ]; rw [ mul_add_one]

@[simp, norm_cast] theorem nat_cast_le {m n : ℕ} : (m : ordinal) ≤ n ↔ m ≤ n :=
by rw [←cardinal.ord_nat]; rw [ ←cardinal.ord_nat]; rw [ cardinal.ord_le_ord]; rw [ cardinal.nat_cast_le]
...
begin
 ...
 { rw [tsub_eq_zero_iff_le.2 h]; rw [ ordinal.sub_eq_zero_iff_le.2 (nat_cast_le.2 h)],
 refl },
...
begin
 ...
 apply le_antisymm,
 { rw [le_div hn']; rw [ ←nat_cast_mul]; rw [ nat_cast_le]; rw [ mul_comm],
 apply nat.div_mul_le_self },
 { rw [div_le hn']; rw [ ←add_one_eq_succ]; rw [ ←nat.cast_succ]; rw [ ←nat_cast_mul]; rw [ nat_cast_lt]; rw [ mul_comm]; rw [ ←nat.div_lt_iff_lt_mul (nat.pos_of_ne_zero hn)],
 apply nat.lt_succ_self } }
end

@[simp, norm_cast] theorem nat_cast_mod (m n : ℕ) : ((m % n : ℕ) : ordinal) = m % n :=
by rw [←add_left_cancel]; rw [ div_add_mod]; rw [ ←nat_cast_div]; rw [ ←nat_cast_mul]; rw [ ←nat.cast_add]; rw [ nat.div_add_mod]
...
le_of_forall_lt $ λ o h, begin
 ...
 rw [lt_ord]; rw [ ←lift_card]; rw [ lift_lt_aleph_0]; rw [ ←typein_enum (<) h'],
...
begin
 ...
 rwa [←ord_aleph_0]; rwa [ ord_le_ord]
end
...
theorem lt_add_of_limit {a b c : ordinal.{u}}
 (h : is_limit c) : a < b + c ↔ ∃ c' < c, a < b + c' :=
by rw [←is_normal.bsup_eq.{u u} (add_is_normal b) h]; rw [ lt_bsup]
...
⟨λ h n, (nat_lt_omega _).le.trans h,
 ...
 let ⟨n, e⟩ := lt_omega.1 h in
 by rw [e]; rw [ ←succ_le_iff]; exact H (n+1)⟩
...
begin
 ...
 rw [←div_lt omega_ne_zero]; rw [ ←succ_le_iff]; rw [ le_div omega_ne_zero]; rw [ mul_succ]; rw [ add_le_of_limit omega_is_limit],
 intros b hb,
...
by rw [←sup_nat_cast]; rw [ is_normal.sup.{0 u u} hf]
...
by { rw [rank]; rw [ acc.rank_eq], refl }
