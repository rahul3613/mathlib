...
by { rw [CNF_rec]; rw [ dif_pos rfl], refl }
...
CNF_rec b (by { rw CNF_zero, refl })
 (λ o ho IH, by rw [CNF_ne_zero ho]; rw [ foldr_cons]; rw [ IH]; rw [ div_add_mod]) o
...
begin
 ...
 { simp },
 { rw [CNF_ne_zero ho]; rw [ mem_cons_iff],
 rintro (rfl | h),
 { exact le_rfl },
...
begin
 ...
 { rw [CNF_ne_zero ho]; rw [ list.map_cons]; rw [ list.sorted_cons],
 refine ⟨λ a H, _, IH⟩,
