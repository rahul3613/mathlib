...
by rw [ordinal.type]; rw [ Well_order.eta]; rw [ quotient.out_eq]
...
by rw [←@type_eq_zero_iff_is_empty o.out.α (<)]; rw [ type_lt]
...
by rw [←@type_ne_zero_iff_nonempty o.out.α (<)]; rw [ type_lt]
...
by rw [← typein_lt_typein r]; rw [ typein_enum]; rw [ typein_enum]
...
begin
 ...
 resetI, rw [enum_type g]; rw [ enum_type (principal_seg.lt_equiv g f)], refl
end
...
by rw [one_le_iff_pos]; rw [ ordinal.pos_iff_ne_zero]
...
by { rw [←enum_typein (<) a]; rw [ enum_le_enum']; rw [ ←lt_succ_iff], apply typein_lt_self }
...
⟨↑lift.initial_seg.{u (max (u+1) v)}, univ.{u v}, begin
 ...
 rw [univ]; rw [ ← lift_umax], split; intro h,
...
begin
 ...
 rwa [lt_ord]; rwa [ ← lift_card]; rwa [ lift_lt]; rwa [ ← lt_ord]; rwa [ ← ordinal.lift_lt] },
 { rw [ord_le]; rw [ ← lift_card]; rw [ card_ord] }
end
...
by { rw [←lt_ord]; rw [ h], apply typein_lt_type }
...
by { intros c c' h, rw [←card_ord c]; rw [ ←card_ord c']; rw [ h] }
...
⟨λ h, begin
 ...
 have := card_ord c,
 rw [← e] at this; rw [ lift.principal_seg_coe] at this; rw [ ← lift_card] at this,
 exact ⟨_, this.symm⟩
...
by rw [← cardinal.ord_le]; rw [ cardinal.ord_nat]

@[simp] theorem nat_lt_card {o} {n : ℕ} : (n : cardinal) < card o ↔ (n : ordinal) < o :=
by { rw [←succ_le_iff]; rw [ ←succ_le_iff]; rw [ ←nat_succ]; rw [ nat_le_card], refl }
...
by rw [←card_eq_nat]; rw [ card_type]; rw [ mk_fintype]
