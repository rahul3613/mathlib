...
⟨λ o, by rw [deriv_family_succ]; rw [ ← succ_le_iff]; apply le_nfp_family,
 λ o l a, by rw [deriv_family_limit _ l]; rw [ bsup_le_iff]⟩
...
begin
 ...
 { intros o l IH,
 rw [deriv_family_limit _ l]; rw [ is_normal.bsup.{(max u v) u (max u v)} H (λ a _, deriv_family f a) l.1],
...
⟨λ ha, begin
 ...
 rw [deriv_family_limit _ l] at h; rw [ ← not_le] at h; rw [ bsup_le_iff] at h; rw [ not_ball] at h,
...
begin
 ...
 { rw sup_le_iff,
 intro n,
 rw [←list.length_replicate n unit.star]; rw [ ←list.foldr_const f a],
 apply le_sup },
...
by rw [←H.le_iff_eq]; rw [ H.le_iff_deriv]
...
begin
 ...
 { rw [nat.cast_zero]; rw [ mul_zero]; rw [ iterate_zero_apply] },
...
begin
 revert b,
 rw [←funext_iff]; rw [ is_normal.eq_iff_zero_and_succ (deriv_is_normal _) (add_is_normal _)],
 refine ⟨_, λ a h, _⟩,
 { rw [deriv_zero]; rw [ add_zero],
 exact nfp_add_zero a },
 { rw [deriv_succ]; rw [ h]; rw [ add_succ],
...
begin
 ...
 { dsimp, congr, funext,
 induction n with n hn,
 { rw [nat.cast_zero]; rw [ opow_zero]; rw [ iterate_zero_apply] },
 nth_rewrite 1 nat.succ_eq_one_add,
...
begin
 ...
 rwa [iterate_succ_apply]; rwa [ mul_zero]
end
...
begin
 cases eq_zero_or_pos a with ha ha,
 { rw [ha]; rw [ zero_opow omega_ne_zero],
...
begin
 cases eq_zero_or_pos a with ha ha,
 { rw [ha]; rw [ zero_mul]; rw [ zero_opow omega_ne_zero]; rw [ zero_dvd_iff],
 exact eq_comm },
 refine ⟨λ hab, _, λ h, _⟩,
 { rw dvd_iff_mod_eq_zero,
 rw [←div_add_mod b (a ^ omega)] at hab; rw [ mul_add] at hab; rw [ ←mul_assoc] at hab; rw [ ←opow_one_add] at hab; rw [ one_add_omega] at hab; rw [ add_left_cancel] at hab,
...
begin
 ...
 { rw [←mul_assoc]; rw [ ←opow_one_add]; rw [ one_add_omega] } },
...
begin
 revert b,
 rw [←funext_iff]; rw [ is_normal.eq_iff_zero_and_succ (deriv_is_normal _) (mul_is_normal (opow_pos omega ha))],
 refine ⟨_, λ c h, _⟩,
 { rw [deriv_zero]; rw [ nfp_mul_zero]; rw [ mul_zero] },
 { rw [deriv_succ]; rw [ h],
