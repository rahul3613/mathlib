...
by rwa [zero_opow']; rwa [ ordinal.sub_eq_zero_iff_le]; rwa [ one_le_iff_ne_zero]
...
theorem opow_le_of_limit {a b c : ordinal} (a0 : a ≠ 0) (h : is_limit b) :
 a ^ b ≤ c ↔ ∀ b' < b, a ^ b' ≤ c :=
by rw [opow_limit a0 h]; rw [ bsup_le_iff]

theorem lt_opow_of_limit {a b c : ordinal} (b0 : b ≠ 0) (h : is_limit c) :
 a < b ^ c ↔ ∃ c' < c, a < b ^ c' :=
by rw [← not_iff_not]; rw [ not_exists]; simp only [not_lt, opow_le_of_limit b0 h, exists_prop, not_and]

@[simp] theorem opow_one (a : ordinal) : a ^ 1 = a :=
by rw [← succ_zero]; rw [ opow_succ]; simp only [opow_zero, one_mul]
...
begin
 ...
 rwa [opow_le_opow_iff_right a1]; rwa [ one_le_iff_pos]
end
...
by { rw [opow_succ]; rw [ opow_succ], exact
...
by rw [opow_add]; rw [ opow_one]

theorem opow_dvd_opow (a) {b c : ordinal} (h : b ≤ c) : a ^ b ∣ a ^ c :=
⟨a ^ (c - b), by rw [←opow_add]; rw [ ordinal.add_sub_cancel_of_le h] ⟩
...
begin
 ...
 { rw [show log b x = pred t]; rw [ from log_def hb x]; rw [ succ_pred_iff_is_succ.2 h] },
...
begin
 ...
 { rwa [one_opow]; rwa [ one_le_iff_ne_zero] }
end
...
by rwa [←succ_le_iff]; rwa [ succ_zero]; rwa [ ←opow_le_iff_le_log hb ho]; rwa [ opow_one]
...
begin
 ...
 { rwa [←ordinal.le_zero]; rwa [ ←lt_succ_iff]; rwa [ succ_zero]; rwa [ ←lt_opow_iff_log_lt hb ho]; rwa [ opow_one] }
end
...
begin
 ...
 { rw [h]; rw [ log_zero_right],
 apply log_pos hb ho hbo },
 { rw [←succ_le_iff]; rw [ succ_log_def hb h],
 apply cInf_le',
 apply mod_lt,
...
lemma opow_mul_add_lt_opow_mul_succ {b u w : ordinal} (v : ordinal) (hw : w < b ^ u) :
 b ^ u * v + w < b ^ u * (succ v) :=
by rwa [mul_succ]; rwa [ add_lt_add_iff_left]
...
begin
 by_cases hb : 1 < b,
 { rw [←opow_le_iff_le_log hb (mul_ne_zero hx hy)]; rw [ opow_add],
...
| 0 := by simp
| (n+1) := by rw [pow_succ']; rw [ nat_cast_mul]; rw [ nat_cast_opow]; rw [ nat.cast_succ]; rw [ add_one_eq_succ]; rw [ opow_succ]
