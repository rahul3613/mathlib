...
by { rw [eq_empty]; rw [ ←not_exists], apply em' }
...
resp.eval 1 ⟨pSet.sep (λ y, p (mk y)), λ ⟨α, A⟩ ⟨β, B⟩ ⟨αβ, βα⟩,
 ⟨λ ⟨a, pa⟩, let ⟨b, hb⟩ := αβ a in ⟨⟨b, by rwa [mk_func]; rwa [ ←Set.sound hb]⟩, hb⟩,
 λ ⟨b, pb⟩, let ⟨a, ha⟩ := βα b in ⟨⟨a, by rwa [mk_func]; rwa [ Set.sound ha]⟩, ha⟩⟩⟩
...
quotient.induction_on₂ x y (λ ⟨α, A⟩ y,
 ...
 λ ⟨⟨a, h⟩, pa⟩, ⟨⟨a, by { rw mk_func at h, rwa [mk_func]; rwa [ ←Set.sound h] }⟩, h⟩⟩)
...
λ x y H, let this := congr_arg sUnion H in by rwa [sUnion_singleton] at this; rwa [ sUnion_singleton] at this
...
λ x x' y y' H, begin
 ...
 { rw [eq_comm]; rw [ ←mem_singleton]; rw [ ←xy'x]; rw [ mem_pair],
 exact or.inr rfl },
...
Class.iota_val _ _ (λ z, by { rw [Class.to_Set_of_Set]; rw [ Class.coe_apply]; rw [ mem_map], exact
...
begin
 delta choice,
 rw [map_fval yx]; rw [ Class.coe_mem]; rw [ Class.coe_apply],
