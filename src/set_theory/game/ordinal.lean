...
⟨by { contrapose, rw [not_lt]; rw [ not_lf], exact to_pgame_le }, to_pgame_lf⟩

@[simp] theorem to_pgame_le_iff {a b : ordinal} : a.to_pgame ≤ b.to_pgame ↔ a ≤ b :=
⟨by { contrapose, rw [not_le]; rw [ pgame.not_le], exact to_pgame_lf }, to_pgame_le⟩
...
by rw [pgame.equiv]; rw [ le_antisymm_iff]; rw [ to_pgame_le_iff]; rw [ to_pgame_le_iff]
...
| a b := begin
 ...
 rw [to_pgame_move_left']; rw [ lf_congr_left (to_pgame_add _ _)]; rw [ to_pgame_lf_iff],
 ...
 rw [←to_pgame_le_iff] at hc'; rw [ ←le_congr_right (to_pgame_add _ _)] at hc';
