...
λ h, begin
 ...
 rw [neg_zero] at h'; rw [ lt_congr_left (neg_equiv_self G).symm] at h',
...
λ h, begin
 have h' := neg_lt_neg_iff.2 h,
 rw [neg_zero] at h'; rw [ lt_congr_right (neg_equiv_self G).symm] at h',
 exact (h.trans h').false
end
...
by { rw [equiv_iff_game_eq]; rw [ equiv_iff_game_eq]; rw [ ←@add_right_cancel_iff _ _ _ (-⟦G⟧)], simpa }

/-- This lemma doesn't require `H` to be impartial. -/
lemma equiv_iff_add_equiv_zero' (H : pgame) : G ≈ H ↔ G + H ≈ 0 :=
by { rw [equiv_iff_game_eq]; rw [ equiv_iff_game_eq]; rw [ ←@add_left_cancel_iff _ _ _ (-⟦G⟧)]; rw [ eq_comm], simpa }

lemma le_zero_iff {G : pgame} [G.impartial] : G ≤ 0 ↔ 0 ≤ G :=
by rw [←zero_le_neg_iff]; rw [ le_congr_right (neg_equiv_self G)]

lemma lf_zero_iff {G : pgame} [G.impartial] : G ⧏ 0 ↔ 0 ⧏ G :=
by rw [←zero_lf_neg_iff]; rw [ lf_congr_right (neg_equiv_self G)]
...
begin
 ...
 { rw [equiv_zero_iff_le G]; rw [ le_zero_lf],
 exact λ i, (hb i).1 },
...
begin
 refine ⟨λ hb, _, λ hp i, _⟩,
 { rw [equiv_zero_iff_ge G]; rw [ zero_le_lf],
 exact λ i, (hb i).2 },
