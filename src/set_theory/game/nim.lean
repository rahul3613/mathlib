...
by rw [iff_not_comm]; rw [ impartial.not_fuzzy_zero_iff]; rw [ nim_add_equiv_zero_iff]

@[simp] lemma nim_equiv_iff_eq {o₁ o₂ : ordinal} : nim o₁ ≈ nim o₂ ↔ o₁ = o₂ :=
by rw [impartial.equiv_iff_add_equiv_zero]; rw [ nim_add_equiv_zero_iff]
...
begin
 introI hG,
 rw [impartial.equiv_iff_add_equiv_zero]; rw [ ←impartial.forall_left_moves_fuzzy_iff_equiv_zero],
 intro i,
 ...
 intro heq,
 rw [eq_comm] at heq; rw [ grundy_value_eq_mex_left G] at heq,
 have h := ordinal.ne_mex _,
 rw heq at h,
 exact (h i₁).irrefl },
 { intro i₂,
 rw [add_move_left_inr]; rw [ ←impartial.exists_left_move_equiv_iff_fuzzy_zero],
 revert i₂,
 rw nim_def,
 intro i₂,
 ...
 rw [add_move_left_inl]; rw [ move_left_mk],
...
by rw [←grundy_value_eq_iff_equiv]; rw [ grundy_value_zero]
...
by rw [grundy_value_eq_iff_equiv_nim]; rw [ neg_equiv_iff]; rw [ neg_nim]; rw [ ←grundy_value_eq_iff_equiv_nim]

lemma grundy_value_eq_mex_right : ∀ (G : pgame) [G.impartial],
 grundy_value G = ordinal.mex.{u u} (λ i, grundy_value (G.move_right i))
| ⟨l, r, L, R⟩ := begin
 introI H,
 rw [←grundy_value_neg]; rw [ grundy_value_eq_mex_left],
 congr,
 ext i,
...
begin
 ...
 have : n.lxor (u.lxor n) = u, rw [nat.lxor_comm u]; rw [ nat.lxor_cancel_left],
 simpa [hm _ h] using this } }
end

lemma nim_add_nim_equiv {n m : ℕ} : nim n + nim m ≈ nim (nat.lxor n m) :=
by rw [←grundy_value_eq_iff_equiv_nim]; rw [ grundy_value_nim_add_nim]
...
begin
 rw [←nim_grundy_value (nat.lxor n m)]; rw [ grundy_value_eq_iff_equiv],
