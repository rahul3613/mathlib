...
{ desc := λ s, ⟨colimit.desc (F ⋙ forget_to_SheafedSpace) (forget_to_SheafedSpace.map_cocone s),
 ...
 erw [← this]; erw [ PresheafedSpace.stalk_map.congr_hom _ _ (colimit.ι_desc (forget_to_SheafedSpace.map_cocone s) i : _)],
...
begin
 ...
 rw [preimage_basic_open f]; rw [ preimage_basic_open g],
 dsimp only [functor.op, unop_op],
 rw [← comp_apply]; rw [ ← SheafedSpace.comp_c_app']; rw [ ← comp_apply]; rw [ ← SheafedSpace.comp_c_app']; rw [ SheafedSpace.congr_app (coequalizer.condition f.1 g.1)]; rw [ comp_apply],
...
begin
 rw [← (Top.homeo_of_iso (preserves_coequalizer.iso (SheafedSpace.forget _) f.1 g.1)) .is_open_preimage]; rw [ Top.coequalizer_is_open_iff]; rw [ ← set.preimage_comp],
 erw ← coe_comp,
 rw [preserves_coequalizer.iso_hom]; rw [ ι_comp_coequalizer_comparison],
...
begin
 ...
 rw [← is_unit_map_iff ((coequalizer.π f.val g.val : _).c.app _)]; rw [ ← comp_apply]; rw [ nat_trans.naturality]; rw [ comp_apply]; rw [ Top.presheaf.pushforward_obj_map]; rw [ ← is_unit_map_iff (Y.presheaf.map (eq_to_hom hV').op)]; rw [ ← comp_apply]; rw [ ← functor.map_comp],
...
begin
 ...
 rintro ⟨⟩,
 { rw [← (colimit.cocone (parallel_pair f.val g.val)).w walking_parallel_pair_hom.left]; rw [ category.assoc],
 change _ ≫ _ ≫ _ = _ ≫ _,
 congr,
 exact h },
 { exact h }
