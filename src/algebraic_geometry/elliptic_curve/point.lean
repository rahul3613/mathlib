...
by rw [← base_change_neg_Y]; rw [ base_change_base_change]

@[simp] lemma eval_neg_polynomial : (W.neg_polynomial.eval $ C y₁).eval x₁ = W.neg_Y x₁ y₁ :=
by { rw [neg_Y]; rw [ sub_sub]; rw [ neg_polynomial], eval_simp }
...
begin
 ...
 rw [← span_pair_add_mul_right $ adjoin_root.mk _ $ C $ C $ -L]; rw [ ← _root_.map_mul]; rw [ ← map_add],
...
lemma C_add_polynomial :
 ...
 + W.polynomial :=
by { rw [add_polynomial]; rw [ line_polynomial]; rw [ weierstrass_curve.polynomial]; rw [ neg_polynomial], eval_simp,
 C_simp, ring1 }
...
lemma coordinate_ring.C_add_polynomial :
 ...
 ((Y - C (line_polynomial x₁ y₁ L)) * (W.neg_polynomial - C (line_polynomial x₁ y₁ L))) :=
adjoin_root.mk_eq_mk.mpr ⟨1, by rw [C_add_polynomial]; rw [ add_sub_cancel']; rw [ mul_one]⟩
...
by { rw [add_polynomial]; rw [ line_polynomial]; rw [ weierstrass_curve.polynomial]; rw [ cubic.to_poly], eval_simp,
 C_simp, ring1 }
...
by rw [← base_change_add_X]; rw [ base_change_base_change]
...
by rw [← base_change_add_Y']; rw [ base_change_base_change]
...
begin
 ...
 line_polynomial],
 conv_rhs { rw [sub_sub]; rw [ ← neg_add']; rw [ map_neg]; rw [ span_singleton_neg]; rw [ sup_comm]; rw [ ← span_insert] },
 rw [← span_pair_add_mul_right $ adjoin_root.mk _ $ C $ C $ W.a₁ + L]; rw [ ← _root_.map_mul]; rw [ ← map_add],
 apply congr_arg (_ ∘ _ ∘ _ ∘ _),
 C_simp,
 ring1
end
...
by { rw [equation]; rw [ add_Y']; rw [ add_polynomial]; rw [ line_polynomial]; rw [ weierstrass_curve.polynomial],
 eval_simp }
...
begin
 rw [nonsingular]; rw [ and_iff_right hx']; rw [ add_Y']; rw [ polynomial_X]; rw [ polynomial_Y],
 eval_simp,
 contrapose! hx,
 rw [add_polynomial]; rw [ line_polynomial]; rw [ weierstrass_curve.polynomial],
 eval_simp,
 derivative_simp,
...
by { rw [equation_iff]; rw [ equation_iff]; rw [ neg_Y], congr' 2, ring1 }
...
begin
 rw [nonsingular_iff]; rw [ equation_neg_iff]; rw [ ← neg_Y]; rw [ neg_Y_neg_Y]; rw [ ← @ne_comm _ y₁]; rw [ nonsingular_iff],
...
begin
 ...
 { rw [slope_of_Y_eq hx hy]; rw [ slope_of_Y_eq $ congr_arg _ hx]; rw [ map_zero],
 { rw [hy]; rw [ base_change_neg_Y] } },
 { rw [slope_of_Y_ne hx hy]; rw [ slope_of_Y_ne $ congr_arg _ hx],
 { map_simp,
 simpa only [base_change_neg_Y] },
 { rw [base_change_neg_Y],
 contrapose! hy,
 exact no_zero_smul_divisors.algebra_map_injective F K hy } } },
 { rw [slope_of_X_ne hx]; rw [ slope_of_X_ne],
 { map_simp },
 { contrapose! hx,
...
by rw [← base_change_slope]; rw [ base_change_base_change]
...
begin
 ...
 rw [← sub_eq_zero]; rw [ ← @sub_eq_zero _ _ y₁]; rw [ ← mul_eq_zero]; rw [ neg_Y],
...
begin
 ...
 simp only [XY_ideal, X_class, Y_class, line_polynomial],
 rw [← span_pair_add_mul_right $ adjoin_root.mk W.polynomial $ C $ C $ -W.slope x₁ x₂ y₁ y₂]; rw [ ← _root_.map_mul]; rw [ ← map_add],
 apply congr_arg (_ ∘ _ ∘ _ ∘ _),
 eval_simp,
 C_simp,
 ring1
...
begin
 rw [add_polynomial_eq]; rw [ neg_inj]; rw [ cubic.prod_X_sub_C_eq]; rw [ cubic.to_poly_injective],
 by_cases hx : x₁ = x₂,
 { rcases ⟨hx, Y_eq_of_Y_ne h₁' h₂' hx (hxy hx)⟩ with ⟨rfl, rfl⟩,
 rw [equation_iff] at h₁' h₂',
 rw [slope_of_Y_ne rfl $ hxy rfl],
 rw [neg_Y] at hxy; rw [ ← sub_ne_zero] at hxy,
 ext,
 { refl },
 { simp only [add_X],
 ring1 },
...
by { rw [equation_add_iff]; rw [ add_polynomial_slope h₁' h₂' hxy], eval_simp,
...
begin
 ...
 { rwa [add_Y']; rwa [ hx₁]; rwa [ sub_self]; rwa [ mul_zero]; rwa [ zero_add] },
 { by_cases hx₂ : W.add_X x₁ x₂ (W.slope x₁ x₂ y₁ y₂) = x₂,
 { by_cases hx : x₁ = x₂,
 { subst hx,
 contradiction },
 { rwa [add_Y']; rwa [ ← neg_sub]; rwa [ mul_neg]; rwa [ hx₂]; rwa [ slope_of_X_ne hx]; rwa [ div_mul_cancel _ $ sub_ne_zero_of_ne hx]; rwa [ neg_sub]; rwa [ sub_add_cancel] } },
...
@[simp] lemma some_add_some_of_Y_eq (hx : x₁ = x₂) (hy : y₁ = W.neg_Y x₂ y₂) :
 some h₁ + some h₂ = 0 :=
by rw [← add_def]; rw [ add]; rw [ dif_pos hx]; rw [ dif_pos hy]
...
by rw [← add_def]; rw [ add]; rw [ dif_pos hx]; rw [ dif_neg hy]
...
by rw [← add_def]; rw [ add]; rw [ dif_neg hx]
 ...
 * (C (X ^ 2 + C (x₁ + W.a₂) * X + C (x₁ ^ 2 + W.a₂ * x₁ + W.a₄)) - C (C W.a₁) * Y)
 ...
 with { normalization_tactic := `[rw [neg_Y]; rw [ weierstrass_curve.polynomial], C_simp, ring1] },
 simp_rw [XY_ideal, X_class, Y_class, span_pair_mul_span_pair, mul_comm, ← _root_.map_mul, adjoin_root.mk_eq_mk.mpr ⟨1, Y_rw⟩, _root_.map_mul, span_insert, ← span_singleton_mul_span_singleton, ← mul_sup, ← span_insert],
 ...
 rw [neg_Y]; rw [ ← mul_right_inj' $ C_ne_zero.mpr $ C_ne_zero.mpr hy],
...
by rw [← mul_assoc]; rw [ ← fractional_ideal.coe_ideal_mul]; rw [ mul_comm $ XY_ideal W _ _]; rw [ XY_ideal_neg_mul h₁]; rw [ X_ideal]; rw [ fractional_ideal.coe_ideal_span_singleton_mul_inv W.function_field $ X_class_ne_zero W x₁]
...
begin
 ...
 λ _ _ c _, by rw [← sup_assoc]; rw [ @sup_comm _ _ c]; rw [ sup_sup_sup_comm]; rw [ ← sup_assoc],
 rw [XY_ideal_add_eq]; rw [ X_ideal]; rw [ mul_comm]; rw [ W.XY_ideal_eq₁ x₁ y₁ $ W.slope x₁ x₂ y₁ y₂]; rw [ XY_ideal]; rw [ XY_ideal_eq₂ h₁' h₂' hxy]; rw [ XY_ideal]; rw [ span_pair_mul_span_pair],
 simp_rw [span_insert, sup_rw, sup_mul, span_singleton_mul_span_singleton],
 rw [← neg_eq_iff_eq_neg.mpr $ coordinate_ring.C_add_polynomial_slope h₁' h₂' hxy]; rw [ span_singleton_neg]; rw [ coordinate_ring.C_add_polynomial]; rw [ _root_.map_mul]; rw [ Y_class],
 simp_rw [mul_comm $ X_class W x₁, mul_assoc, ← span_singleton_mul_span_singleton, ← mul_sup],
 rw [span_singleton_mul_span_singleton]; rw [ ← span_insert]; rw [ ← span_pair_add_mul_right $ -(X_class W $ W.add_X x₁ x₂ $ W.slope x₁ x₂ y₁ y₂)]; rw [ mul_neg]; rw [ ← sub_eq_add_neg]; rw [ ← sub_mul]; rw [ ← map_sub]; rw [ sub_sub_sub_cancel_right]; rw [ span_insert]; rw [ ← span_singleton_mul_span_singleton]; rw [ ← sup_rw]; rw [ ← sup_mul]; rw [ ← sup_mul],
 apply congr_arg (_ ∘ _),
 convert top_mul _,
 simp_rw [X_class, ← @set.image_singleton _ _ $ adjoin_root.mk _, ← map_span, ← ideal.map_sup, eq_top_iff_one, mem_map_iff_of_surjective _ $ adjoin_root.mk_surjective W.monic_polynomial, ← span_insert, mem_span_insert', mem_span_singleton'],
 by_cases hx : x₁ = x₂,
 { rcases ⟨hx, Y_eq_of_Y_ne h₁' h₂' hx (hxy hx)⟩ with ⟨rfl, rfl⟩,
 ...
 by rw [map_add]; rw [ map_one]; rw [ _root_.map_mul]; rw [ adjoin_root.mk_self]; rw [ mul_zero]; rw [ add_zero]⟩,
 rw [weierstrass_curve.polynomial]; rw [ neg_polynomial]; rw [ ← mul_right_inj' $ C_ne_zero.mpr $ C_ne_zero.mpr hxy],
 simp only [mul_add, ← mul_assoc, ← C_mul, mul_inv_cancel hxy],
 linear_combination -4 * congr_arg C (congr_arg C $ (W.equation_iff _ _).mp h₁')
 with { normalization_tactic := `[rw [b₂]; rw [ b₄]; rw [ neg_Y], C_simp, ring1] } },
...
begin
 ...
 any_goals { refl },
 { rw [zero_def]; rw [ zero_add]; rw [ ← neg_eq_iff_eq_neg]; rw [ neg_zero]; rw [ eq_comm] },
 { simp only [neg_some],
 split,
 { intro h,
 by_cases hx : x₁ = x₂,
...
@[simp] lemma neg_add_eq_zero (P Q : W.point) : -P + Q = 0 ↔ P = Q := by rw [add_eq_zero]; rw [ neg_inj]
...
⟨begin
 ...
 rw [← finrank_quotient_span_eq_nat_degree_norm W^.coordinate_ring.basis h0]; rw [ ← (quotient_equiv_alg_of_eq F hp).to_linear_equiv.finrank_eq]; rw [ (quotient_XY_ideal_equiv W h).to_linear_equiv.finrank_eq]; rw [ finite_dimensional.finrank_self] }
end, congr_arg to_class⟩

lemma to_class_injective : function.injective $ @to_class _ _ W :=
begin
 rintro (_ | h) _ hP,
 all_goals { rw [← neg_add_eq_zero]; rw [ ← to_class_eq_zero]; rw [ map_add]; rw [ ← hP] },
 { exact zero_add 0 },
...
{ to_fun := of_base_change_fun W F K,
 ...
 { rw [hy]; rw [ base_change_neg_Y_of_base_change] } },
