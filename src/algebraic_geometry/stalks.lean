...
by rw [stalk_map]; rw [ stalk_functor_map_germ_assoc]; rw [ stalk_pushforward_germ]
...
by rw [← restrict_stalk_iso_hom_eq_germ]; rw [ category.assoc]; rw [ iso.hom_inv_id]; rw [ category.comp_id]
...
begin
 ...
 erw [iso.comp_inv_eq]; erw [ colimit.ι_map_assoc]; erw [ colimit.ι_map_assoc]; erw [ colimit.ι_pre],
...
lemma congr {X Y : PresheafedSpace.{v} C} (α β : X ⟶ Y) (h₁ : α = β) (x x': X) (h₂ : x = x') :
 ...
 eq_to_hom (show Y.stalk (α.base x) = Y.stalk (β.base x'), by rw [h₁]; rw [ h₂]) ≫ stalk_map β x' :=
...
by rw [← stalk_map.congr α β h x x rfl]; rw [ eq_to_hom_refl]; rw [ category.comp_id]
...
{ out := begin
 ...
 { rw [is_iso.hom_inv_id α]; rw [ id_base]; rw [ Top.id_app] },
 ...
 { rw [← category.assoc]; rw [ congr_point α x ((α ≫ β).base x) h_eq.symm]; rw [ category.assoc],
 erw ← stalk_map.comp β α (α.base x),
 rw [congr_hom _ _ (is_iso.inv_hom_id α)]; rw [ stalk_map.id]; rw [ eq_to_hom_trans_assoc]; rw [ eq_to_hom_refl]; rw [ category.id_comp] },
 { rw [category.assoc]; rw [ ← stalk_map.comp]; rw [ congr_hom _ _ (is_iso.hom_inv_id α)]; rw [ stalk_map.id]; rw [ eq_to_hom_trans_assoc]; rw [ eq_to_hom_refl]; rw [ category.id_comp] },
end }
