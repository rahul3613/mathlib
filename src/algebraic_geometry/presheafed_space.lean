...
{ base := f,
 ...
 by { rw [← map_comp]; rw [ ← map_comp], refl } } }
...
instance of_restrict_mono {U : Top} (X : PresheafedSpace C) (f : U ⟶ X.1)
 ...
 erw [g₁.c.naturality]; erw [ g₂.c.naturality_assoc] at h,
...
lemma of_restrict_top_c (X : PresheafedSpace C) :
 ...
 (by { rw [restrict_top_presheaf]; rw [ ←presheaf.pushforward.comp_eq],
...
begin
 ...
 /- or `rw [opens.inclusion_top_functor]; rw [ ←comp_obj]; rw [ ←opens.map_comp_eq],
