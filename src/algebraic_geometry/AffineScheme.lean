...
begin
 ...
 erw [← category.assoc]; erw [ Scheme.comp_val_base],
 rw [coe_comp]; rw [ set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ set.image_univ],
...
begin
 ...
 { rw [Scheme.comp_val_base]; rw [ coe_comp]; rw [ set.range_comp],
...
begin
 ...
 rw [← hU.from_Spec_range]; rw [ ← set.image_univ],
...
begin
 ...
 rw [Scheme.comp_val_c_app]; rw [ Scheme.comp_val_c_app]; rw [ ← e₁],
 ...
 erw [e₃]; erw [ e₄]; erw [ ← Scheme.comp_val_c_app_assoc]; erw [ iso.inv_hom_id],
 simp only [eq_to_hom_map, eq_to_hom_op, Scheme.Spec_map_presheaf_map_eq_to_hom],
 erw [Scheme.Spec_map_presheaf_map_eq_to_hom]; erw [ category.id_comp],
...
lemma is_affine_open.from_Spec_app_eq {X : Scheme} {U : opens X.carrier}
 ...
 (Scheme.Spec.obj _).presheaf.map (eq_to_hom hU.from_Spec_base_preimage).op :=
by rw [← hU.Spec_Γ_identity_hom_app_from_Spec]; rw [ iso.inv_hom_id_app_assoc]
...
begin
 ...
 { rw [set.image_preimage_eq_inter_range]; rw [ set.inter_eq_left_iff_subset]; rw [ hU.from_Spec_range],
 exact Scheme.basic_open_le _ _ },
 rw [Scheme.hom.opens_range_coe]; rw [ Scheme.comp_val_base]; rw [ ← this]; rw [ coe_comp]; rw [ set.range_comp],
 congr' 1,
...
begin
 ...
 dsimp,
 erw [opens.functor_obj_map_obj]; erw [ opens.open_embedding_obj_top]; erw [ inf_comm]; erw [ ← Scheme.basic_open_res _ _ (hom_of_le le_top).op],
...
begin
 ...
 congr,
 { rw [← is_iso.inv_eq_inv]; rw [ is_iso.inv_inv]; rw [ is_iso.iso.inv_inv]; rw [ nat_iso.app_hom],
 erw ← Γ_Spec.adjunction_unit_app_app_top,
 refl },
 { rw eq_to_hom_map, refl },
 { dsimp, congr },
 { refine (Scheme.preimage_basic_open _ _).trans _,
 rw [is_iso.inv_hom_id_apply]; rw [ Scheme.basic_open_res_eq] }
end
...
begin
 ...
 rw [← comp_apply]; rw [ ← category_theory.functor.map_comp]; rw [ ← op_comp]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]; rw [ op_id]; rw [ category_theory.functor.map_id]; rw [ Scheme.hom.inv_app],
...
begin
 ...
 use f * x,
 rw [algebra.smul_def]; rw [ Scheme.basic_open_mul]; rw [ Scheme.basic_open_mul],
...
begin
 ...
 erw [← Scheme.comp_val_base_apply]; erw [ ← Scheme.comp_val_base_apply],
...
begin
 ...
 rw [Scheme.inv_val_c_app]; rw [ is_iso.comp_inv_eq]; rw [ Scheme.app_eq _ e]; rw [ Γ_Spec.adjunction_unit_app_app_top],
 simp only [category.assoc, eq_to_hom_op],
 erw ← functor.map_comp_assoc,
 rw [eq_to_hom_trans]; rw [ eq_to_hom_refl]; rw [ category_theory.functor.map_id]; rw [ category.id_comp],
...
begin
 ...
 erw [← X.presheaf.map_comp]; erw [ Spec_Γ_naturality_assoc],
 congr' 1,
 simp only [← category.assoc],
 ...
 rw [← LocallyRingedSpace.Γ_map_op]; rw [ ← LocallyRingedSpace.Γ.map_comp_assoc]; rw [ ← op_comp],
 erw ← Scheme.Spec.map_comp,
 rw [← op_comp]; rw [ ← X.presheaf.map_comp],
...
begin
 rw [← Scheme.basic_open_res_eq _ _ (eq_to_hom hU.from_Spec_base_preimage.symm).op]; rw [ basic_open_eq_of_affine']; rw [ is_affine_open.from_Spec_app_eq],
 congr,
 rw [← comp_apply]; rw [ ← comp_apply]; rw [ category.assoc]; rw [ ← functor.map_comp_assoc]; rw [ eq_to_hom_op]; rw [ eq_to_hom_op]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]; rw [ category_theory.functor.map_id]; rw [ category.id_comp]; rw [ ← iso.app_inv]; rw [ iso.inv_hom_id],
 refl
end
...
begin
 ...
 rw [set.image_preimage_eq_inter_range] at h; rw [ set.image_preimage_eq_inter_range] at h; rw [ hU.from_Spec_range] at h,
 ...
 rw [← set.compl_Inter]; rw [ set.compl_univ_iff]; rw [ ← prime_spectrum.zero_locus_Union]; rw [ ← prime_spectrum.zero_locus_empty_iff_eq_top]; rw [ prime_spectrum.zero_locus_span],
...
begin
 rw [← hU.basic_open_union_eq_self_iff]; rw [ @comm _ eq],
...
begin
 ...
 intros x hx,
 rw [supr_range']; rw [ opens.mem_supr],
 exact ⟨_, hf₁ ⟨x, hx⟩⟩
end
