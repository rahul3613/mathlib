...
begin
 ext f,
 rw [vanishing_ideal]; rw [ set_like.mem_coe]; rw [ submodule.mem_infi],
...
by rw [← set_like.mem_coe]; rw [ coe_vanishing_ideal]; rw [ set.mem_set_of_eq]
...
begin
 rw [mem_vanishing_ideal]; rw [ ideal.radical_eq_Inf]; rw [ submodule.mem_Inf],
...
by rw [← zero_locus_span {f}]; rw [ ← zero_locus_span {g}]; rw [ zero_locus_subset_zero_locus_iff]; rw [ ideal.span_le]; rw [ set.singleton_subset_iff]; rw [ set_like.mem_coe]
...
by rw [← top_le_iff]; rw [ ← subset_zero_locus_iff_le_vanishing_ideal]; rw [ submodule.top_coe]; rw [ zero_locus_univ]; rw [ set.subset_empty_iff]
...
begin
 intros r,
 rw [submodule.mem_sup]; rw [ mem_vanishing_ideal],
...
by rw [set.mem_compl_iff]; rw [ mem_zero_locus]; rw [ set.singleton_subset_iff]; refl
...
by rw [← is_open_compl_iff]; rw [ is_open_iff]; rw [ compl_compl]
...
begin
 ...
 { obtain ⟨s, hs⟩ := h,
 rw [eq_comm] at hs; rw [ set.eq_singleton_iff_unique_mem] at hs,
 ...
 rw [eq_comm]; rw [ set.eq_singleton_iff_unique_mem],
...
by rw [← zero_locus_vanishing_ideal_eq_closure]; rw [ vanishing_ideal_singleton]

lemma is_radical_vanishing_ideal (s : set (prime_spectrum R)) :
 (vanishing_ideal s).is_radical :=
by { rw [← vanishing_ideal_closure]; rw [ ← zero_locus_vanishing_ideal_eq_closure]; rw [ vanishing_ideal_zero_locus_eq_radical], apply ideal.radical_is_radical }
...
by rw [set.ssubset_def]; rw [ vanishing_ideal_anti_mono_iff hs]; rw [ vanishing_ideal_anti_mono_iff ht]; rw [ lt_iff_le_not_le]
...
begin
 rw [ideal.is_prime_iff]; rw [ is_irreducible],
 apply and_congr,
 { rw [set.nonempty_iff_ne_empty]; rw [ ne.def]; rw [ zero_locus_empty_iff_eq_top] },
 ...
 { simp_rw [← zero_locus_inf, subset_zero_locus_iff_le_vanishing_ideal, vanishing_ideal_zero_locus_eq_radical, hI.radical],
 split,
 { simp_rw [← set_like.mem_coe, ← set.singleton_subset_iff, ← ideal.span_le, ← ideal.span_singleton_mul_span_singleton],
...
by rw [← is_irreducible_iff_closure]; rw [ ← zero_locus_vanishing_ideal_eq_closure]; rw [ is_irreducible_zero_locus_iff_of_radical _ (is_radical_vanishing_ideal s)]

instance [is_domain R] : irreducible_space (prime_spectrum R) :=
begin
 rw [irreducible_space_def]; rw [ set.top_eq_univ]; rw [ ← zero_locus_bot]; rw [ is_irreducible_zero_locus_iff],
...
⟨λ S h₁ h₂, ⟨⟨_, is_irreducible_iff_vanishing_ideal_is_prime.1 h₁⟩,
 by rw [is_generic_point]; rw [ closure_singleton]; rw [ zero_locus_vanishing_ideal_eq_closure]; rw [ h₂.closure_eq]⟩⟩
...
begin
 ...
 rw [preimage_comap_zero_locus]; rw [ ← zero_locus_span]; rw [ ← zero_locus_span s],
 congr' 1,
...
begin
 ...
 dsimp only at h,
 erw [is_localization.map_comap M S]; erw [ is_localization.map_comap M S] at h,
 ext1,
 exact h
end
...
{ induced := begin
 ...
 by rw [preimage_comap_zero_locus]; rw [ surjective.image_preimage hf]; rw [ hF]⟩, _⟩,
 rintros ⟨-, ⟨F, rfl⟩, hF⟩,
...
begin
 ...
 rwa [mem_ker]; rwa [ map_sub]; rwa [ sub_eq_zero] } },
end
...
by rw [← set_like.coe_subset_coe]; rw [ basic_open_eq_zero_locus_compl]; rw [ basic_open_eq_zero_locus_compl]; rw [ set.compl_subset_compl]; rw [ zero_locus_subset_zero_locus_singleton_iff]
...
begin
 ...
 { rintros p U hp ⟨s, hs⟩,
 rw [← compl_compl U] at hp; rw [ set.mem_compl_iff] at hp; rw [ ← hs] at hp; rw [ mem_zero_locus] at hp; rw [ set.not_subset] at hp,
 obtain ⟨f, hfs, hfp⟩ := hp,
 refine ⟨basic_open f, ⟨f, rfl⟩, hfp, _⟩,
 rw [← set.compl_subset_compl]; rw [ ← hs]; rw [ basic_open_eq_zero_locus_compl]; rw [ compl_compl],
...
begin
 ...
 rw [basic_open_eq_zero_locus_compl f] at hZ; rw [ set.inter_comm] at hZ; rw [ ← set.diff_eq] at hZ; rw [ set.diff_eq_empty] at hZ; rw [ funext hI] at hZ; rw [ ← zero_locus_supr] at hZ,
 ...
 simp_rw [basic_open_eq_zero_locus_compl f, set.inter_comm (zero_locus {f})ᶜ, ← set.diff_eq, set.diff_eq_empty, hI, ← zero_locus_supr],
...
begin
 rw [← topological_space.opens.coe_inj]; rw [ basic_open_eq_zero_locus_compl],
...
by rw [← as_ideal_le_as_ideal]; rw [ ← zero_locus_vanishing_ideal_eq_closure]; rw [ mem_zero_locus]; rw [ vanishing_ideal_singleton]; rw [ set_like.coe_subset_coe]
...
@is_localization.lift _ _ _ _ _ _ _ _
 ...
 begin
 rintro ⟨a, ha⟩,
 rw [← prime_spectrum.le_iff_specializes] at h; rw [ ← as_ideal_le_as_ideal] at h; rw [ ← set_like.coe_subset_coe] at h; rw [ ← set.compl_subset_compl] at h,
...
by { rw [(local_hom_tfae f).out 0 4]; rw [ prime_spectrum.ext_iff], refl }
