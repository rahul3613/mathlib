...
begin
 ext x,
 rw [algebra.mem_bot]; rw [ algebra.mem_infi],
 split,
 ...
 exact hrange ⟨y, by rw [← mul_one $ algebra_map R K y]; rw [ ← algebra.smul_def]; rw [ hy]; rw [ one_smul]⟩
 end,
 ...
 ⟨y * n, by rw [algebra.smul_def]; rw [ mul_one]; rw [ map_mul]; rw [ smul_comm]; rw [ algebra.smul_def]; rw [ algebra.smul_def]; rw [ mul_comm $ algebra_map R K d]; rw [ inv_mul_cancel_right₀ $ (map_ne_zero_iff _ $ no_zero_smul_divisors.algebra_map_injective R K).mpr $ λ h]; rw [ (h ▸ hd) max.zero_mem]⟩ },
 { rintro ⟨y, rfl⟩ ⟨v, hv⟩,
 exact ⟨y, 1, v.ne_top_iff_one.mp hv.ne_top, by rw [map_one]; rw [ inv_one]; rw [ mul_one]⟩ }
end
...
begin
 ...
 rw [algebra.mem_infi],
 split,
 { rw [← maximal_spectrum.infi_localization_eq_bot]; rw [ algebra.mem_infi],
