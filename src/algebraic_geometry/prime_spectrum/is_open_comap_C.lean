...
begin
 rw [image_of_Df]; rw [ set_of_exists (λ i (x : prime_spectrum R), coeff f i ∉ x.as_ideal)],
...
begin
 ...
 { rw [mem_compl_iff]; rw [ mem_zero_locus]; rw [ singleton_subset_iff],
 cases hx with i hi,
...
begin
 rintros U ⟨s, z⟩,
 rw [← compl_compl U]; rw [ ← z]; rw [ ← Union_of_singleton_coe s]; rw [ zero_locus_Union]; rw [ compl_Inter]; rw [ image_Union],
