...
by erw [local_ring.mem_maximal_ideal]; erw [ not_not]
...
begin
 ...
 rw [← category.assoc]; rw [ category.assoc (to_open _ _)],
 erw stalk_functor_map_germ,
 rw [← category.assoc (to_open _ _)]; rw [ X.to_Γ_Spec_SheafedSpace_app_spec 1],
 unfold Γ_to_stalk,
...
{ val := X.to_Γ_Spec_SheafedSpace,
 ...
 rw [← to_stalk_stalk_map_to_Γ_Spec]; rw [ comp_apply],
 erw ← he,
 rw ring_hom.map_mul,
...
begin
 ...
 rw category.assoc,
 erw [to_Γ_Spec_SheafedSpace_app_spec]; erw [ ← X.presheaf.map_comp],
 convert h r },
 exact w,
end
...
begin
 ...
 erw category.assoc,
 rw [nat_trans.naturality]; rw [ ← category.assoc],
 erw [X.to_Γ_Spec_SheafedSpace_app_spec 1]; erw [ ← functor.map_comp],
...
{ app := LocallyRingedSpace.to_Γ_Spec,
 ...
 rw [← local_ring.comap_closed_point (PresheafedSpace.stalk_map _ x)]; rw [ ← prime_spectrum.comap_comp_apply]; rw [ ← prime_spectrum.comap_comp_apply],
 congr' 2,
 exact (PresheafedSpace.stalk_map_germ f.1 ⊤ ⟨x,trivial⟩).symm,
 apply_instance },
 { intro r,
 rw [LocallyRingedSpace.comp_val_c_app]; rw [ ← category.assoc],
 erw [Y.to_Γ_Spec_SheafedSpace_app_spec]; erw [ f.1.c.naturality],
 refl },
 end }
...
by { rw [← adjunction.hom_equiv_symm_id]; rw [ ← adjunction.hom_equiv_symm_id]; rw [ adjunction_hom_equiv_symm_apply], refl }

@[simp] lemma adjunction_unit_app {X : Scheme} :
 Γ_Spec.adjunction.unit.app X = LocallyRingedSpace_adjunction.unit.app X.1 :=
by { rw [← adjunction.hom_equiv_id]; rw [ ← adjunction.hom_equiv_id]; rw [ adjunction_hom_equiv_apply], refl }
...
begin
 ...
 rw [← op_inv] at this; rw [ nat_iso.inv_inv_app] at this; rw [ quiver.hom.op_inj.eq_iff] at this,
 exact this
end
