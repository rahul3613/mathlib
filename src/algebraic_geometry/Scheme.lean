...
begin
 rw [← is_iso.inv_comp_eq]; rw [ ← functor.map_inv]; rw [ f.val.c.naturality]; rw [ presheaf.pushforward_obj_map],
 congr
end
...
begin
 ...
 rw [Scheme.congr_app (is_iso.hom_inv_id f)]; rw [ Scheme.id_app]; rw [ ← functor.map_comp]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_op],
 refl
end
...
{ obj := λ R, Spec_obj (unop R),
 ...
 map_id' := λ R, by rw [unop_id]; rw [ Spec_map_id],
 map_comp' := λ R S T f g, by rw [unop_comp]; rw [ Spec_map_comp] }
...
begin
 ...
 { exact this },
 erw [← is_unit_map_iff (structure_sheaf.stalk_to_fiber_ring_hom R x)]; erw [ structure_sheaf.stalk_to_fiber_ring_hom_to_stalk],
