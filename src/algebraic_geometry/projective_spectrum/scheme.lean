...
begin
 rw [←submodule_span_eq] at hz; rw [ finsupp.span_eq_range_total] at hz; rw [ linear_map.mem_range] at hz,
 ...
 congr' 1,
 ext i,
 rw [_root_.map_mul]; rw [ hacd]; rw [ (classical.some_spec i.1.2).2]; rw [ smul_eq_mul]; rw [ smul_mul_assoc],
 refl
end
...
begin
 ...
 exact x.2 hg1 },
 { erw [show k = 0]; erw [ by linarith]; erw [ pow_zero]; erw [ ←ideal.eq_top_iff_one] at hg1,
...
begin
 ...
 rw [algebra.smul_def] at eq1; rw [ homogeneous_localization.one_val] at eq1; rw [ mul_one] at eq1,
 change localization.mk (f ^ N) 1 = mk (∑ _, _) 1 at eq1,
 simp only [mk_eq_mk', is_localization.eq] at eq1,
 rcases eq1 with ⟨⟨_, ⟨M, rfl⟩⟩, eq1⟩,
 erw [one_mul]; erw [ one_mul] at eq1,
...
⟨carrier 𝒜 x, carrier_ne_top x, λ x1 x2 hx12, begin
 ...
 by rw [localization.mk_mul]; rw [ mul_one]; rw [ one_mul]],
 ...
 { rw [←mul_comm (f^M)]; rw [ ←mul_comm (f^N)]; rw [ eq1],
...
begin
 ...
 { refine ⟨y.2, _⟩,
 rw [set.mem_preimage] at hy; rw [ set_like.mem_coe] at hy; rw [ prime_spectrum.mem_basic_open] at hy,
 rw projective_spectrum.mem_coe_basic_open,
 intro a_mem_y,
 apply hy,
 rw [to_fun]; rw [ mem_carrier_iff]; rw [ homogeneous_localization.val_mk']; rw [ subtype.coe_mk],
 dsimp, rcases b_mem2 with ⟨k, hk⟩,
 simp only [show (mk a ⟨b, ⟨k, hk⟩⟩ : away f) = mk 1 ⟨f^k, ⟨_, rfl⟩⟩ * mk a 1,
 by { rw [mk_mul]; rw [ one_mul]; rw [ mul_one], congr, rw hk }],
 exact ideal.mul_mem_left _ _ (ideal.subset_span ⟨_, a_mem_y, rfl⟩), },
 { change y.1 ∈ _ at hy,
 rcases hy with ⟨hy1, hy2⟩,
 rw projective_spectrum.mem_coe_basic_open at hy1 hy2,
 rw [set.mem_preimage]; rw [ to_fun]; rw [ set_like.mem_coe]; rw [ prime_spectrum.mem_basic_open],
 ...
 rw [mk_mul] at eq1; rw [ one_mul] at eq1; rw [ mk_eq_mk'] at eq1; rw [ is_localization.eq] at eq1,
 rcases eq1 with ⟨⟨_, ⟨M, rfl⟩⟩, eq1⟩,
 rw [submonoid.coe_one] at eq1; rw [ one_mul] at eq1,
 ...
 { rw [mul_comm _ (f^N)]; rw [ mul_comm _ (f^M)]; rw [ eq1],
 refine mul_mem_left _ _ (mul_mem_left _ _ (sum_mem _ (λ i hi, mul_mem_left _ _ _))),
 generalize_proofs h₁ h₂, exact (classical.some_spec h₂).1, }, },
end
...
(mem_carrier_iff f_deg q a).trans begin
 ...
 convert h, rw [ext_iff_val]; rw [ val_mk'], dsimp only [subtype.coe_mk], rw ←hx, refl, },
end
...
begin
 ...
 rotate,
 { rw (_ : m*i = _), mem_tac, rw [← add_smul]; rw [ nat.add_sub_of_le h1], refl },
...
begin
 ...
 { erw [ext_iff_val]; erw [ val_mk']; erw [ mul_val]; erw [ val_mk']; erw [ val_mk']; erw [ subtype.coe_mk],
 simp_rw [mul_pow, subtype.coe_mk], rw [localization.mk_mul],
 congr, erw [← pow_add]; erw [ nat.add_sub_of_le h] },
 { exact ideal.mul_mem_left _ _ (hx _), rw [smul_eq_mul]; rw [ mul_comm], mem_tac, } },
...
λ i a ha j, (em (i = j)).elim
 ...
 zero_pow hm], convert carrier.zero_mem f_deg hm q j, rw [map_zero]; rw [ zero_pow hm],
 end)
...
show (∀ i, _ ∈ _) ∨ ∀ i, _ ∈ _, begin
 ...
 simp_rw [proj_apply, decompose_of_mem_same 𝒜 hnx, decompose_of_mem_same 𝒜 hny, decompose_of_mem_same 𝒜 (mul_mem hnx hny), mul_pow, pow_add],
 ...
 convert mk_zero _, rw [decompose_of_mem_ne 𝒜 _ hn.symm]; rw [ zero_pow hm],
 { exact hnx <|> exact hny } },
end
