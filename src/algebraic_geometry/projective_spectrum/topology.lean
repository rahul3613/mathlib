...
begin
 ext f,
 rw [vanishing_ideal]; rw [ set_like.mem_coe]; rw [ ← homogeneous_ideal.mem_iff]; rw [ homogeneous_ideal.to_ideal_infi]; rw [ submodule.mem_infi],
 apply forall_congr (λ x, _),
 rw [homogeneous_ideal.to_ideal_infi]; rw [ submodule.mem_infi]; rw [ homogeneous_ideal.mem_iff],
end
...
begin
 intros r,
 rw [← homogeneous_ideal.mem_iff]; rw [ homogeneous_ideal.to_ideal_sup]; rw [ mem_vanishing_ideal]; rw [ submodule.mem_sup],
...
topological_space.opens.ext $ set.ext $ λ z, begin
 erw [mem_coe_basic_open]; erw [ topological_space.opens.mem_Sup],
 split; intros hz,
...
begin
 rw [← as_ideal_le_as_ideal]; rw [ ← zero_locus_vanishing_ideal_eq_closure]; rw [ mem_zero_locus]; rw [ vanishing_ideal_singleton],
