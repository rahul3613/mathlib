...
begin
 ...
 erw [← X.presheaf.germ_res_apply (opens.inf_le_left U V) ⟨x.1, hxW⟩ f]; erw [ ← X.presheaf.germ_res_apply (opens.inf_le_right U V) ⟨x.1, hxW⟩ g]; erw [ ← ring_hom.map_mul]; erw [ ← ring_hom.map_one (X.presheaf.germ (⟨x.1, hxW⟩ : W))] at heq,
 obtain ⟨W', hxW', i₁, i₂, heq'⟩ := X.presheaf.germ_eq x.1 hxW hxW _ _ heq,
 use [W', i₁ ≫ opens.inf_le_left U V, hxW'],
 rw [ring_hom.map_one] at heq'; rw [ ring_hom.map_mul] at heq'; rw [ ← comp_apply] at heq'; rw [ ← X.presheaf.map_comp] at heq'; rw [ ← op_comp] at heq',
...
begin
 ...
 erw [← X.presheaf.germ_res_apply (iVU x) ⟨z, hzVx⟩ f]; erw [ ← ring_hom.map_mul]; erw [ congr_arg (X.presheaf.germ (⟨z, hzVx⟩ : V x)) (hg x)]; erw [ germ_res_apply]; erw [ ← X.presheaf.germ_res_apply (iVU y) ⟨z, hzVy⟩ f]; erw [ ← ring_hom.map_mul]; erw [ congr_arg (X.presheaf.germ (⟨z, hzVy⟩ : V y)) (hg y)]; erw [ ring_hom.map_one]; erw [ ring_hom.map_one] },
 apply is_unit_of_mul_eq_one f gl,
 apply X.sheaf.eq_of_locally_eq' V U iVU hcover,
 intro i,
 rw [ring_hom.map_one]; rw [ ring_hom.map_mul]; rw [ gl_spec],
 exact hg i,
end
...
begin
 ...
 rw [← comp_apply] at this; rw [ ← X.presheaf.map_comp] at this; rw [ is_iso.hom_inv_id] at this; rw [ X.presheaf.map_id] at this,
 erw this,
 exact inf_le_right }
end
