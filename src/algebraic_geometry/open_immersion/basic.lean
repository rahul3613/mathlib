...
begin
 ...
 erw [f.c.naturality_assoc]; erw [ ←X.presheaf.map_comp],
 congr }
end
...
begin
 ...
 erw [category.comp_id]; erw [ f.c.naturality_assoc]; erw [ ←X.presheaf.map_comp],
 transitivity f.c.app x ≫ X.presheaf.map (𝟙 _),
 { congr },
 { erw [X.presheaf.map_id]; erw [ category.comp_id] } },
 { refl, }
end

@[simp] lemma iso_restrict_inv_of_restrict : H.iso_restrict.inv ≫ f = Y.of_restrict _ :=
by { rw [iso.inv_comp_eq]; rw [ iso_restrict_hom_of_restrict] }
...
{ base_open := hg.base_open.comp hf.base_open,
 ...
 rw [coe_comp]; rw [ ← set.image_image]; rw [ set.preimage_image_eq _ hg.base_open.inj] },
 rw this,
 apply_instance },
 { have : h.functor.obj U = hg.open_functor.obj (hf.open_functor.obj U),
 { ext1,
 dsimp only [is_open_map.functor_obj_coe],
 rw [comp_base]; rw [ coe_comp]; rw [ ←set.image_image] },
 rw this,
 apply_instance }
 end }
...
@[simp, reassoc, elementwise] lemma inv_app_app (U : opens X) :
 ...
 X.presheaf.map (eq_to_hom (by simp [opens.map, set.preimage_image_eq _ H.base_open.inj])) :=
by rw [inv_app]; rw [ category.assoc]; rw [ is_iso.inv_hom_id]; rw [ category.comp_id]
...
by { erw ← category.assoc, rw [is_iso.comp_inv_eq]; rw [ f.c.naturality], congr }
...
begin
 ...
 rw [is_iso.comp_inv_eq]; rw [ category.id_comp],
...
begin
 ...
 erw [← Y.presheaf.map_comp]; erw [ ← Y.presheaf.map_comp],
 congr },
...
{ base := pullback.lift s.fst.base s.snd.base
 ...
 rw category.assoc,
 erw [← s.X.presheaf.map_comp]; erw [ ← s.X.presheaf.map_comp],
 congr
 end } }
...
begin
 ...
 reassoc! this,
 erw [← this]; erw [ hf.inv_app_app_assoc]; erw [ s.fst.c.naturality_assoc],
 simpa [eq_to_hom_map], },
...
begin
 rw [←limit.w (cospan f g) walking_cospan.hom.inl]; rw [ cospan_map_inl],
 apply_instance
end
...
by rw [← cancel_mono f]; rw [ hl]; rw [ lift_fac]
...
{ base_open := hf,
 ...
 rw [category.assoc] at this; rw [ is_iso.hom_inv_id] at this; rw [ category.comp_id] at this,
 exact this
 end }
...
begin
 ...
 cases i, cases j,
 rw [ι_preserves_colimits_iso_hom_assoc] at eq; rw [ ι_preserves_colimits_iso_hom_assoc] at eq; rw [ has_colimit.iso_of_nat_iso_ι_hom_assoc] at eq; rw [ has_colimit.iso_of_nat_iso_ι_hom_assoc] at eq; rw [ Top.sigma_iso_sigma_hom_ι.{v}] at eq; rw [ Top.sigma_iso_sigma_hom_ι.{v}] at eq,
...
{ base_open := sigma_ι_open_embedding F i,
 ...
 { convert this },
 rw [PresheafedSpace.comp_c_app]; rw [ ← PresheafedSpace.colimit_presheaf_obj_iso_componentwise_limit_hom_π],
...
begin
 ...
 rw [PresheafedSpace.stalk_map.comp] at this; rw [ PresheafedSpace.stalk_map.comp] at this,
...
begin
 ...
 rw [PresheafedSpace.stalk_map.comp] at this; rw [ ← is_iso.eq_inv_comp] at this,
 rw this,
 apply_instance },
 split,
 { exact LocallyRingedSpace.hom.ext _ _
...
begin
 ...
 rw [set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ set.image_univ]; rw [ Top.pullback_fst_range],
 ext,
 split,
