...
{ J := J,
 ...
 f := λ x, e₁.symm (𝒰.f x),
 covers := λ x, begin
 rw [e₂]; rw [ Scheme.comp_val_base]; rw [ coe_comp]; rw [ set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ set.image_univ]; rw [ e₁.right_inverse_symm],
 { exact 𝒰.covers x },
...
begin
 erw [coe_comp]; erw [ set.range_comp],
 congr,
...
begin
 ...
 rw [← classical.some_spec this] at h'; rw [ set.mem_Union] at h',
...
begin
 rw [is_iso_iff_is_open_immersion]; rw [ is_open_immersion.iff_stalk_iso]; rw [ and_comm]; rw [ ← and_assoc],
...
begin
 rw [← (show _ = (pullback.snd : pullback f g ⟶ _).1.base, from preserves_pullback.iso_hom_snd Scheme.forget_to_Top f g)]; rw [ coe_comp]; rw [ set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ ← @set.preimage_univ _ _ (pullback.fst : pullback f.1.base g.1.base ⟶ _)]; rw [ Top.pullback_snd_image_fst_preimage]; rw [ set.image_univ],
 refl,
 rw ← Top.epi_iff_surjective,
 apply_instance
end
...
lemma range_pullback_fst_of_right :
 ...
 (opens.map g.1.base).obj ⟨set.range f.1.base, H.base_open.open_range⟩ :=
begin
 rw [← (show _ = (pullback.fst : pullback g f ⟶ _).1.base, from preserves_pullback.iso_hom_fst Scheme.forget_to_Top g f)]; rw [ coe_comp]; rw [ set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ ← @set.preimage_univ _ _ (pullback.snd : pullback g.1.base f.1.base ⟶ _)]; rw [ Top.pullback_fst_image_snd_preimage]; rw [ set.image_univ],
 refl,
 rw ← Top.epi_iff_surjective,
 apply_instance
end
...
begin
 rw [pullback.condition]; rw [ Scheme.comp_val_base]; rw [ coe_comp]; rw [ set.range_comp]; rw [ range_pullback_snd_of_left]; rw [ opens.map_obj]; rw [ opens.coe_mk]; rw [ set.image_preimage_eq_inter_range]; rw [ set.inter_comm],
end
...
begin
 rw [Scheme.comp_val_base]; rw [ coe_comp]; rw [ set.range_comp]; rw [ range_pullback_fst_of_right]; rw [ opens.map_obj]; rw [ opens.coe_mk]; rw [ set.image_preimage_eq_inter_range]; rw [ set.inter_comm],
end
...
begin
 subst H,
 rw [Scheme.comp_val_c_app]; rw [ category.assoc]; rw [ Scheme.hom.inv_app]; rw [ PresheafedSpace.is_open_immersion.inv_app_app_assoc]; rw [ f.val.c.naturality_assoc]; rw [ Top.presheaf.pushforward_obj_map]; rw [ ← functor.map_comp],
...
begin
 ...
 rw [Scheme.hom.inv_app] at e; rw [ PresheafedSpace.is_open_immersion.inv_app_app_apply] at e; rw [ Scheme.basic_open_res] at e; rw [ inf_eq_right.mpr _] at e,
 rw ← e,
 ext1,
...
{ obj := λ U, over.mk (X.of_restrict U.open_embedding),
 ...
 rw [← cancel_mono (X.of_restrict U.open_embedding)]; rw [ category.id_comp]; rw [ is_open_immersion.lift_fac],
 end,
 map_comp' := λ U V W i j, begin
 ext1,
 dsimp only [over.hom_mk_left, over.comp_left],
 rw [← cancel_mono (X.of_restrict W.open_embedding)]; rw [ category.assoc],
...
begin
 ...
 dsimp at e₁ e₂ ⊢,
 rw [e₂] at e₁; rw [ W.adjunction_counit_map_functor] at e₁; rw [ ← is_iso.eq_inv_comp] at e₁; rw [ is_iso.inv_comp_eq] at e₁; rw [ ← is_iso.eq_comp_inv] at e₁,
 simp_rw [eq_to_hom_map (opens.map _), eq_to_hom_map (is_open_map.functor _), ← functor.map_inv, ← functor.map_comp] at e₁,
 rw e₁,
 congr' 1,
end
...
begin
 ...
 rw [X.restrict_functor_map_app]; rw [ ← functor.map_comp]; rw [ ← functor.map_comp],
 congr' 1
end
...
begin
 ...
 dsimp,
 rw [subtype.range_coe]; rw [ subtype.range_coe],
...
{ J := 𝒰.J,
 ...
 rw [coe_comp]; rw [ set.range_comp]; rw [ set.range_iff_surjective.mpr]; rw [ set.image_univ]; rw [ Top.pullback_fst_range],
...
begin
 ...
 rw [← is_compact_univ_iff]; rw [ ← 𝒰.Union_range],
...
by { delta morphism_restrict,
 rw [category.assoc]; rw [ pullback.condition.symm]; rw [ pullback_restrict_iso_restrict_inv_fst_assoc] }
...
begin
 ...
 rw [pullback_restrict_iso_restrict_inv_fst]; rw [ category.comp_id],
end
...
begin
 ...
 simp_rw category.assoc,
 rw [pullback_restrict_iso_restrict_inv_fst]; rw [ pullback_right_pullback_fst_iso_inv_snd_fst]; rw [ ← pullback.condition]; rw [ pullback_restrict_iso_restrict_inv_fst_assoc]; rw [ pullback_restrict_iso_restrict_inv_fst_assoc],
 refl,
 apply_instance
end
...
begin
 ...
 rw [← is_iso.eq_comp_inv] at this; rw [ ← functor.map_inv] at this; rw [ category.assoc] at this,
 rw this,
 congr' 1,
 erw [← X.presheaf.map_comp]; erw [ ← X.presheaf.map_comp],
 congr' 1,
end
...
begin
 rw [Scheme.Γ_map_op]; rw [ morphism_restrict_c_app f U ⊤]; rw [ f.val.c.naturality_assoc],
...
begin
 ...
 pullback.map _ _ _ _ (𝟙 _) e.hom (𝟙 _) (by rw [category.comp_id]; rw [ category.id_comp])
 (by rw [category.comp_id]; rw [ is_open_immersion.iso_of_range_eq_hom]; rw [ is_open_immersion.lift_fac]),
 symmetry,
 refine arrow.iso_mk (as_iso t ≪≫ pullback_restrict_iso_restrict f V) e _,
 rw [iso.trans_hom]; rw [ as_iso_hom]; rw [ ← iso.comp_inv_eq]; rw [ ← cancel_mono g]; rw [ arrow.mk_hom]; rw [ arrow.mk_hom]; rw [ is_open_immersion.iso_of_range_eq_inv]; rw [ category.assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ is_open_immersion.lift_fac]; rw [ ← pullback.condition]; rw [ morphism_restrict_ι]; rw [ pullback_restrict_iso_restrict_hom_restrict_assoc]; rw [ pullback.lift_fst_assoc]; rw [ category.comp_id],
end
...
begin
 ...
 erw [Scheme.of_restrict_val_c_app]; erw [ opens.adjunction_counit_app_self]; erw [ eq_to_hom_op] at e,
 rw [← (Y.restrict U.open_embedding).basic_open_res_eq _ (eq_to_hom U.inclusion_map_eq_top).op]; rw [ ← comp_apply],
 erw ← Y.presheaf.map_comp,
 rw [eq_to_hom_op]; rw [ eq_to_hom_op]; rw [ eq_to_hom_map]; rw [ eq_to_hom_trans],
 erw ← e,
 ext1, dsimp [opens.map, opens.inclusion],
 rw [set.image_preimage_eq_inter_range]; rw [ set.inter_eq_left_iff_subset]; rw [ subtype.range_coe],
...
begin
 ...
 rw [morphism_restrict_c_app]; rw [ category.assoc]; rw [ Top.presheaf.germ_res],
 refl }
end
