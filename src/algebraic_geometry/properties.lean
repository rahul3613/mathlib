...
begin
 ...
 { rw [set.top_eq_univ]; rw [ set.sUnion_range]; rw [ set.eq_univ_iff_forall],
...
begin
 ...
 specialize H (f.1.c.app _ s) _ ⟨x, by { rw [opens.mem_mk]; rw [ e], trivial }⟩,
 { rw [← Scheme.preimage_basic_open]; rw [ hs], ext1, simp [opens.map] },
 { erw ← PresheafedSpace.stalk_map_germ_apply f.1 ⟨_,_⟩ ⟨x,_⟩ at H,
 apply_fun (inv $ PresheafedSpace.stalk_map f.val x) at H,
 erw [category_theory.is_iso.hom_inv_id_apply]; erw [ map_zero] at H,
 exact H } },
 { intros R hX s hs x,
 erw [basic_open_eq_of_affine']; erw [ prime_spectrum.basic_open_eq_bot_iff] at hs,
...
begin
 ...
 rw [ring_hom.map_mul] at e; rw [ ring_hom.map_zero] at e,
