...
{ obj := λ R, Spec.Top_obj (unop R),
 ...
 map_id' := λ R, by rw [unop_id]; rw [ Spec.Top_map_id],
 map_comp' := λ R S T f g, by rw [unop_comp]; rw [ Spec.Top_map_comp] }
...
begin
 dsimp,
 erw [PresheafedSpace.id_c_app]; erw [ comap_id], swap,
 { rw [Spec.Top_map_id]; rw [ topological_space.opens.map_id_obj_unop] },
 simpa [eq_to_hom_map],
end
...
{ obj := λ R, Spec.SheafedSpace_obj (unop R),
 ...
 map_id' := λ R, by rw [unop_id]; rw [ Spec.SheafedSpace_map_id],
 map_comp' := λ R S T f g, by rw [unop_comp]; rw [ Spec.SheafedSpace_map_comp] }
...
lemma stalk_map_to_stalk {R S : CommRing} (f : R ⟶ S) (p : prime_spectrum S) :
 ...
 f ≫ to_stalk S p :=
begin
 erw [← to_open_germ S ⊤ ⟨p, trivial⟩]; erw [ ← to_open_germ R ⊤ ⟨prime_spectrum.comap f p, trivial⟩]; erw [ category.assoc]; erw [ PresheafedSpace.stalk_map_germ (Spec.SheafedSpace_map f) ⊤ ⟨p, trivial⟩]; erw [ Spec.SheafedSpace_map_c_app]; erw [ to_open_comp_comap_assoc],
 refl
end
...
rw [stalk_iso_hom]; rw [ stalk_iso_inv]; rw [ comp_apply]; rw [ comp_apply]; rw [ localization_to_stalk_of]; rw [ stalk_map_to_stalk_apply]; rw [ stalk_to_fiber_ring_hom_to_stalk]
...
LocallyRingedSpace.hom.ext _ _ $
 by { rw [Spec.LocallyRingedSpace_map_val]; rw [ Spec.SheafedSpace_map_id], refl }
...
LocallyRingedSpace.hom.ext _ _ $
 by { rw [Spec.LocallyRingedSpace_map_val]; rw [ Spec.SheafedSpace_map_comp], refl }
...
{ obj := λ R, Spec.LocallyRingedSpace_obj (unop R),
 ...
 map_id' := λ R, by rw [unop_id]; rw [ Spec.LocallyRingedSpace_map_id],
 map_comp' := λ R S T f g, by rw [unop_comp]; rw [ Spec.LocallyRingedSpace_map_comp] }
...
begin
 ...
 refine ⟨⟨s, ⟨r, hpr⟩ ^ n⟩, _⟩,
 rw [submonoid.smul_def]; rw [ algebra.smul_def]; rw [ algebra_map_pushforward_stalk]; rw [ to_pushforward_stalk]; rw [ comp_apply]; rw [ comp_apply],
 iterate 2 { erw ← (Spec.Top_map (algebra_map R S) _* (structure_sheaf S).1).germ_res_apply
 (hom_of_le le_top) ⟨p, hpr⟩ },
 rw [← e]; rw [ ← map_mul]; rw [ mul_comm],
...
begin
 ...
 { intros x hx, rw [algebra_map_pushforward_stalk]; rw [ to_pushforward_stalk_comp]; rw [ comp_apply],
 ...
 { intros x hx,
 rw [to_pushforward_stalk_alg_hom_apply] at hx; rw [ ring_hom.to_fun_eq_coe] at hx; rw [ ← (to_pushforward_stalk (algebra_map R S) p).map_zero] at hx; rw [ to_pushforward_stalk] at hx; rw [ comp_apply] at hx; rw [ comp_apply] at hx; rw [ map_zero] at hx,
 ...
 refine ⟨⟨r, hpr⟩ ^ n, _⟩,
 rw [submonoid.smul_def]; rw [ algebra.smul_def]; rw [ submonoid.coe_pow]; rw [ subtype.coe_mk]; rw [ map_pow],
 exact e },
end
