...
begin
 ...
 { rw [← set_like.mem_coe]; rw [ (generic_point_spec X.carrier).mem_open_set_iff]; rw [ set.top_eq_univ]; rw [ set.univ_inter]; rw [ set.nonempty_iff_ne_empty]; rw [ ne.def]; rw [ ← opens.coe_bot]; rw [ ← set_like.ext'_iff],
...
begin
 ...
 rw [hU.prime_ideal_of_generic_point]; rw [ generic_point_eq_bot_of_affine],
