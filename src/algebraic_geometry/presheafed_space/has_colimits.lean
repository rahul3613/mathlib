...
{ obj := λ j, (F.obj (unop j)).presheaf.obj (op ((opens.map (colimit.ι F (unop j)).base).obj U)),
 ...
 (eq_to_hom (by { rw [← colimit.w F f.unop]; rw [ comp_base], refl })),
 ...
 congr' 1,
 rw [Top.presheaf.pushforward.comp_inv_app]; rw [ Top.presheaf.pushforward_eq_hom_app]; rw [ category_theory.nat_trans.naturality_assoc]; rw [ Top.presheaf.pushforward_map_app],
 congr' 1,
 rw [category.id_comp]; rw [ ← (F.obj (unop k)).presheaf.map_comp],
...
begin
 ...
 rw category.assoc,
 erw [← (F.obj (unop Y)).presheaf.map_comp]; erw [ (F.map f.unop).c.naturality_assoc]; erw [ ← (F.obj (unop Y)).presheaf.map_comp],
 congr }
end
...
begin
 ...
 rw [iso.trans_inv]; rw [ iso.trans_inv]; rw [ iso.app_inv]; rw [ sheaf_iso_of_iso_inv]; rw [ pushforward_to_of_iso_app]; rw [ congr_app (iso.symm_inv _)],
 simp_rw category.assoc,
 rw [← functor.map_comp_assoc]; rw [ nat_trans.naturality],
 erw ← comp_c_app_assoc,
 rw congr_app (colimit.iso_colimit_cocone_ι_hom _ _),
 simp_rw category.assoc,
 erw [limit_obj_iso_limit_comp_evaluation_inv_π_app_assoc]; erw [ lim_map_π_assoc],
...
by rw [← iso.eq_inv_comp]; rw [ colimit_presheaf_obj_iso_componentwise_limit_inv_ι_app]
