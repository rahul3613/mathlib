...
begin
 ...
 rw [coe_to_fun_eq]; rw [ coe_to_fun_eq]; rw [ ← eq₁]; rw [ ← eq₂]; rw [ coe_comp]; rw [ set.image_comp]; rw [ coe_comp]; rw [ set.preimage_comp]; rw [ set.image_preimage_eq]; rw [ Top.pullback_snd_image_fst_preimage],
 refl,
...
begin
 ...
 rw [← cancel_epi (inv ((D.f_open i j).inv_app U))]; rw [ is_iso.inv_hom_id_assoc]; rw [ is_open_immersion.inv_inv_app],
 simp_rw category.assoc,
 erw [(π₁ i, j, k).c.naturality_assoc]; erw [ reassoc_of this]; erw [ ← functor.map_comp_assoc]; erw [ is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.app_inv_app_assoc]; erw [ ← (D.V (i, k)).presheaf.map_comp]; erw [ ← (D.V (i, k)).presheaf.map_comp],
...
begin
 split,
 rw [← is_iso.eq_inv_comp]; rw [ is_open_immersion.inv_inv_app]; rw [ category.assoc]; rw [ (D.t' k i j).c.naturality_assoc],
 simp_rw ← category.assoc,
 erw ← comp_c_app,
 rw [congr_app (D.t_fac k i j)]; rw [ comp_c_app],
 simp_rw category.assoc,
 erw [is_open_immersion.inv_naturality]; erw [ is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.app_inv_app'_assoc],
 ...
 rw [𝖣 .t_inv] at eq; rw [ id_base] at eq; rw [ Top.id_app] at eq,
 subst eq,
 use (inv (D.t' k i j)).base y,
 change ((inv (D.t' k i j)) ≫ (π₁ k, i, j)).base y = _,
 congr' 2,
 rw [is_iso.inv_comp_eq]; rw [ 𝖣 .t_fac_assoc]; rw [ 𝖣 .t_inv]; rw [ category.comp_id]
end
...
begin
 ...
 rw [← (show _ = (𝖣 .ι i).base, from 𝖣 .ι_glued_iso_inv (PresheafedSpace.forget _) i)]; rw [ ← (show _ = (𝖣 .ι j).base, from 𝖣 .ι_glued_iso_inv (PresheafedSpace.forget _) j)]; rw [ coe_comp]; rw [ coe_comp]; rw [ set.image_comp]; rw [ set.preimage_comp]; rw [ set.preimage_image_eq],
...
begin
 ...
 simp_rw category.assoc,
 rw [(D.f j k).c.naturality]; rw [ f_inv_app_f_app_assoc],
 erw ← (D.V (j, k)).presheaf.map_comp,
 simp_rw ← category.assoc,
 erw [← comp_c_app]; erw [ ← comp_c_app],
 simp_rw category.assoc,
 dsimp only [functor.op, unop_op, quiver.hom.unop_op],
 rw [eq_to_hom_map (opens.map _)]; rw [ eq_to_hom_op]; rw [ eq_to_hom_trans],
 congr
end
...
{ X := (D.U i).presheaf.obj (op U),
 ...
 { erw [category.id_comp]; erw [ category_theory.functor.map_id],
 rw category.comp_id },
 ...
 simp_rw category.assoc,
 erw [opens_image_preimage_map_app_assoc]; erw [ (D.t j k).c.naturality_assoc],
 ...
 { rw [← 𝖣 .t_fac_assoc]; rw [ 𝖣 .t'_comp_eq_pullback_symmetry_assoc]; rw [ pullback_symmetry_hom_comp_snd_assoc]; rw [ pullback.condition]; rw [ 𝖣 .t_fac_assoc] },
 rw congr_app this,
 ...
 simp_rw category.assoc,
 erw [nat_trans.naturality_assoc]; erw [ ← PresheafedSpace.comp_c_app_assoc]; erw [ congr_app (pullback_symmetry_hom_comp_snd _ _)],
 simp_rw category.assoc,
 erw [is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.app_inv_app_assoc],
...
begin
 ...
 erw [is_open_immersion.inv_naturality_assoc]; erw [ is_open_immersion.app_inv_app'_assoc],
...
begin
 ...
 simp_rw category.assoc,
 rw [← functor.map_comp_assoc]; rw [ is_open_immersion.inv_naturality_assoc],
...
begin
 ...
 rcases j with (⟨j, k⟩|⟨j⟩),
 { rw [← limit.w (componentwise_diagram 𝖣 .diagram.multispan _) (quiver.hom.op (walking_multispan.hom.fst (j, k)))]; rw [ ← category.assoc]; rw [ category.id_comp],
 congr' 1,
 simp_rw category.assoc,
 apply π_ι_inv_app_π },
 { simp_rw category.assoc,
...
begin
 ...
 { rw [category.assoc]; rw [ (D.ι_inv_app_π _).some_spec],
...
begin
 ...
 { rw [← 𝖣 .ι_glued_iso_hom (PresheafedSpace.forget _) _]; rw [ ← 𝖣 .ι_glued_iso_hom (PresheafedSpace.forget _) _],
 have := congr_arg PresheafedSpace.hom.base s.condition,
 rw [comp_base] at this; rw [ comp_base] at this,
 reassoc! this,
 exact this _ },
 rw [← set.image_subset_iff]; rw [ ← set.image_univ]; rw [ ← set.image_comp]; rw [ set.image_univ]; rw [ ← coe_comp]; rw [ this]; rw [ coe_comp]; rw [ ← set.image_univ]; rw [ set.image_comp],
 exact set.image_subset_range _ _ },
 { apply is_open_immersion.lift_fac },
 { rw [← cancel_mono (𝖣 .ι j)]; rw [ category.assoc]; rw [ ← (𝖣 .V_pullback_cone i j).condition],
