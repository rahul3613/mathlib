...
by erw [ring_hom.map_zero]; erw [ subtype.val_eq_coe]; erw [ subring.coe_zero]; erw [ pi.zero_apply]; erw [ zero_mul]
...
subtype.eq $ funext $ λ x, is_localization.mk'_eq_of_eq
 (by rw [mul_comm]; rw [ subtype.coe_mk]; rw [ ←h]; rw [ mul_comm]; rw [ subtype.coe_mk])
...
by rw [const_mul]; rw [ const_congr R rfl (mul_comm g f)]; rw [ const_self]

lemma const_mul_cancel (f g₁ g₂ : R) (U hu₁ hu₂) :
 const R f g₁ U hu₁ * const R g₁ g₂ U hu₂ = const R f g₂ U hu₂ :=
by { rw [const_mul]; rw [ const_ext], rw mul_assoc }

lemma const_mul_cancel' (f g₁ g₂ : R) (U hu₁ hu₂) :
 const R g₁ g₂ U hu₂ * const R f g₁ U hu₁ = const R f g₂ U hu₂ :=
by rw [mul_comm]; rw [ const_mul_cancel]
...
{ to_fun := λ f, ⟨λ x, algebra_map R _ f,
 ...
 by { rw [ring_hom.map_one]; rw [ mul_one], refl } ⟩⟩⟩,
...
@[simp] lemma to_open_germ (U : opens (prime_spectrum.Top R)) (x : U) :
 ...
 to_stalk R x :=
by { rw [← to_open_res R ⊤ U (hom_of_le le_top : U ⟶ ⊤)]; rw [ category.assoc]; rw [ presheaf.germ_res], refl }
...
by rw [to_open_eq_const]; rw [ const_mul_rev]
...
by erw [← germ_to_open R (basic_open s) ⟨x, s.2⟩]; erw [ ← germ_to_open R (basic_open s) ⟨x, s.2⟩]; erw [ ← ring_hom.map_mul]; erw [ to_open_eq_const]; erw [ to_open_eq_const]; erw [ const_mul_cancel']
...
by { erw [to_stalk]; erw [ category.assoc]; erw [ germ_comp_stalk_to_fiber_ring_hom], refl }
...
{ hom := stalk_to_fiber_ring_hom R x,
 ...
 begin
 ext s, simp only [comp_apply], rw [id_apply]; rw [ stalk_to_fiber_ring_hom_germ'],
 obtain ⟨V, hxV, iVU, f, g, hg, hs⟩ := exists_const _ _ s x hxU,
 erw [← res_apply R U V iVU s ⟨x, hxV⟩]; erw [ ← hs]; erw [ const_apply]; erw [ localization_to_stalk_mk'],
 refine (structure_sheaf R).presheaf.germ_ext V hxV (hom_of_le hg) iVU _,
 erw [← hs]; erw [ res_const']
 end,
...
(is_localization.lift_mk'_spec _ _ _ _).2 $
by rw [to_open_eq_const]; rw [ to_open_eq_const]; rw [ const_mul_cancel']
...
ring_hom.ext $ λ g,
by rw [to_basic_open]; rw [ is_localization.away.lift]; rw [ ring_hom.comp_apply]; rw [ is_localization.lift_eq]
...
begin
 ...
 exact ⟨⟨f ^ n, n, rfl⟩, hn⟩, },
 rw [← vanishing_ideal_zero_locus_eq_radical]; rw [ mem_vanishing_ideal],
 intros p hfp,
 contrapose hfp,
 rw [mem_zero_locus]; rw [ set.not_subset],
 have := congr_fun (congr_arg subtype.val h_eq) ⟨p,hfp⟩,
 rw [const_apply] at this; rw [ const_apply] at this; rw [ is_localization.eq] at this,
...
begin
 ...
 rw [op_comp]; rw [ functor.map_comp]; rw [ comp_apply]; rw [ ← s_eq]; rw [ res_const],
...
begin
 ...
 rw [to_basic_open_mk']; rw [ to_basic_open_mk'],
 ...
 intros i hi,
 rw [op_comp]; rw [ functor.map_comp]; rw [ comp_apply]; rw [ ← hs]; rw [ res_const],
...
begin
 ...
 { rw [← vanishing_ideal_zero_locus_eq_radical]; rw [ zero_locus_span],
 simp only [basic_open_eq_zero_locus_compl] at ht_cover,
 rw set.compl_subset_comm at ht_cover, -- Why doesn't `simp_rw` do this?
 simp_rw [set.compl_Union, compl_compl, ← zero_locus_Union, ← finset.set_bUnion_coe, ← set.image_eq_Union ] at ht_cover,
 apply vanishing_ideal_anti_mono ht_cover,
 exact subset_vanishing_ideal_zero_locus {f} (set.mem_singleton f) },

 replace hn := ideal.mul_mem_left _ f hn,
 erw [←pow_succ]; erw [ finsupp.mem_span_image_iff_total] at hn,
 ...
 have := ht_cover hx,
 rw [← finset.set_bUnion_coe] at this; rw [ set.mem_Union₂] at this,
...
begin
 ...
 rw [localization_to_basic_open R]; rw [ to_open_res],
end
...
begin
 ...
 rw [is_localization.lift_comp]; rw [ is_localization.lift_comp]; rw [ is_localization.lift_comp],
...
begin
 ...
 erw [← localization.local_ring_hom_to_map ((prime_spectrum.comap f q).as_ideal)]; erw [ ← ring_hom.map_mul]; erw [ h_frac.2]; erw [ localization.local_ring_hom_to_map],
 refl,
end
...
{ to_fun := λ s, ⟨comap_fun f U V hUV s.1, comap_fun_is_locally_fraction f U V hUV s.1 s.2⟩,
 ...
 { rw [subtype.coe_mk]; rw [ subtype.val_eq_coe]; rw [ comap_fun]; rw [ (sections_subring R (op U)).coe_one]; rw [ pi.one_apply]; rw [ ring_hom.map_one], refl },
 map_zero' := subtype.ext $ funext $ λ p, by
 { rw [subtype.coe_mk]; rw [ subtype.val_eq_coe]; rw [ comap_fun]; rw [ (sections_subring R (op U)).coe_zero]; rw [ pi.zero_apply]; rw [ ring_hom.map_zero], refl },
 map_add' := λ s t, subtype.ext $ funext $ λ p, by
 { rw [subtype.coe_mk]; rw [ subtype.val_eq_coe]; rw [ comap_fun]; rw [ (sections_subring R (op U)).coe_add]; rw [ pi.add_apply]; rw [ ring_hom.map_add], refl },
 map_mul' := λ s t, subtype.ext $ funext $ λ p, by
 { rw [subtype.coe_mk]; rw [ subtype.val_eq_coe]; rw [ comap_fun]; rw [ (sections_subring R (op U)).coe_mul]; rw [ pi.mul_apply]; rw [ ring_hom.map_mul], refl } }
...
begin
 ...
 erw [s_eq₂]; erw [ localization.local_ring_hom_mk']; erw [ ← s_eq₁]; erw [ ← res_apply],
end
...
lemma comap_id (U V : opens (prime_spectrum.Top R)) (hUV : U = V) :
 comap (ring_hom.id R) U V (λ p hpV, by rwa [hUV]; rwa [ prime_spectrum.comap_id]) =
 eq_to_hom (show (structure_sheaf R).1.obj (op U) = _, by rw hUV) :=
by erw [comap_id_eq_map U V (eq_to_hom hUV.symm)]; erw [ eq_to_hom_op]; erw [ eq_to_hom_map]
