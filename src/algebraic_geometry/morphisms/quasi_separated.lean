...
begin
 ...
 erw [subtype.range_coe]; erw [ subtype.range_coe] at e,
...
begin
 rw [quasi_separated_eq_diagonal_is_quasi_compact]; rw [ quasi_compact_eq_affine_property],
...
by rw [quasi_separated_eq_affine_property_diagonal]; rw [ quasi_compact_affine_property_diagonal_eq]
...
begin
 ...
 simp_rw [← quasi_compact_eq_affine_property, ← quasi_separated_eq_diagonal_is_quasi_compact, quasi_compact_affine_property_diagonal_eq] at this,
 exact this
end
...
by rw [quasi_separated_eq_affine_property]; rw [ quasi_separated.affine_property_is_local.affine_target_iff f]; rw [ quasi_separated.affine_property]
...
begin
 rw [quasi_separated_eq_affine_property]; rw [ quasi_separated.affine_property_is_local.affine_open_cover_iff f 𝒰],
 refl,
end
...
begin
 ...
 { rw [pow_add]; rw [ mul_assoc], congr' 1,
 convert congr_arg (X.presheaf.map (hom_of_le _).op) e₁,
 { simp only [map_pow, map_mul, ← comp_apply, ← functor.map_comp, ← op_comp], congr },
 { simp only [map_pow, map_mul, ← comp_apply, ← functor.map_comp, ← op_comp], congr },
 { rw [X.basic_open_res]; rw [ X.basic_open_res], rintros x ⟨H₁, H₂⟩, exact ⟨h₁ H₁, H₂⟩ } },
 { rw [add_comm]; rw [ pow_add]; rw [ mul_assoc], congr' 1,
...
begin
 ...
 { rw X.basic_open_res, exact inf_le_right },
 { rw [X.basic_open_res]; rw [ X.basic_open_res],
 erw ← inf_sup_right,
 ...
 { delta Scheme.sheaf SheafedSpace.sheaf,
 simp only [map_pow, map_mul, ← comp_apply, ← functor.map_comp, ← op_comp, mul_assoc,
 pow_add], erw hy₁, congr' 1, rw [← mul_assoc]; rw [ ← mul_assoc], congr' 1,
 rw [mul_comm]; rw [ ← comp_apply]; rw [ ← functor.map_comp], congr } },
 { convert congr_arg (X.presheaf.map (hom_of_le _).op)
 ...
 { delta Scheme.sheaf SheafedSpace.sheaf,
 simp only [map_pow, map_mul, ← comp_apply, ← functor.map_comp, ← op_comp, mul_assoc,
 pow_add], erw hy₂, rw [← comp_apply]; rw [ ← functor.map_comp], congr } } }
end
...
begin
 ...
 using e.symm },
 { intros x y,
 rw [← sub_eq_zero]; rw [ ← map_sub]; rw [ ring_hom.algebra_map_to_algebra],
 ...
 rw [← ((RingedSpace.is_unit_res_basic_open _ f).pow n).mul_right_inj]; rw [ mul_zero]; rw [ ← map_pow]; rw [ ← map_mul]; rw [ e]; rw [ map_zero] } }
end
