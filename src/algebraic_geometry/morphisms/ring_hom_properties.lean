...
begin
 rw [Γ_map_morphism_restrict]; rw [ hP.cancel_left_is_iso]; rw [ hP.cancel_right_is_iso]; rw [ ← (hP.cancel_right_is_iso (f.val.c.app (opposite.op (Y.basic_open r))) (X.presheaf.map (eq_to_hom (Scheme.preimage_basic_open f r).symm).op))]; rw [ ← eq_iff_iff],
 congr,
...
begin
 ...
 rw [opens.open_embedding_obj_top]; rw [ opens.functor_obj_map_obj],
 convert (X.basic_open_res (Scheme.Γ.map f.op r) (hom_of_le le_top).op).symm using 1,
 rw [opens.open_embedding_obj_top]; rw [ opens.open_embedding_obj_top]; rw [ inf_comm]; rw [ Scheme.Γ_map_op]; rw [ ← Scheme.preimage_basic_open] },
 { apply is_localization.ring_hom_ext (submonoid.powers r) _,
 swap, { exact algebraic_geometry.Γ_restrict_is_localization Y r },
 rw [is_localization.away.map]; rw [ is_localization.map_comp]; rw [ ring_hom.algebra_map_to_algebra]; rw [ ring_hom.algebra_map_to_algebra]; rw [ op_comp]; rw [ functor.map_comp]; rw [ op_comp]; rw [ functor.map_comp],
 ...
 rw [morphism_restrict_c_app]; rw [ category.assoc]; rw [ category.assoc]; rw [ category.assoc],
 erw [f.1.c.naturality_assoc]; erw [ ← X.presheaf.map_comp]; erw [ ← X.presheaf.map_comp]; erw [ ← X.presheaf.map_comp],
 congr },
end
...
begin
 rw [← preserves_pullback.iso_inv_fst AffineScheme.forget_to_Scheme (AffineScheme.of_hom f) (AffineScheme.of_hom g)]; rw [ op_comp]; rw [ functor.map_comp]; rw [ hP'.cancel_right_is_iso]; rw [ AffineScheme.forget_to_Scheme_map],
 ...
 rw [← this]; rw [ hP'.cancel_right_is_iso]; rw [ ← pushout_iso_unop_pullback_inl_hom (quiver.hom.unop _) (quiver.hom.unop _)]; rw [ hP'.cancel_right_is_iso],
...
begin
 ...
 { introv H U,
 rw [← h₁.cancel_right_is_iso _ (Scheme.Γ.map (Scheme.restrict_map_iso e.inv U.1).hom.op)]; rw [ ← functor.map_comp]; rw [ ← op_comp],
 convert H ⟨_, U.prop.map_is_iso e.inv⟩ using 3,
 rw [is_open_immersion.iso_of_range_eq_hom]; rw [ is_open_immersion.lift_fac_assoc]; rw [ category.assoc]; rw [ e.inv_hom_id_assoc],
 refl },
 { introv H U,
 rw [← category.assoc]; rw [ op_comp]; rw [ functor.map_comp]; rw [ h₁.cancel_left_is_iso],
 exact H U }
end
...
begin
 ...
 rw [← hP.cancel_right_is_iso _ (X.presheaf.map (eq_to_hom _))]; rw [ category.assoc]; rw [ ← X.presheaf.map_comp],
...
begin
 ...
 subst hs,
 rw [CommRing.comp_eq_ring_hom_comp]; rw [ ← ring_hom.comp_assoc],
 erw [is_localization.map_comp]; erw [ ring_hom.comp_id],
 rw [ring_hom.algebra_map_to_algebra]; rw [ op_comp]; rw [ functor.map_comp]; rw [ ← CommRing.comp_eq_ring_hom_comp]; rw [ Scheme.Γ_map_op]; rw [ Scheme.Γ_map_op]; rw [ Scheme.Γ_map_op]; rw [ category.assoc],
 erw ← X.presheaf.map_comp,
 rw [← h₁.cancel_right_is_iso _ (X.presheaf.map (eq_to_hom _))],
 convert hs' ⟨r, hr⟩ using 1,
 { erw category.assoc, rw [← X.presheaf.map_comp]; rw [ op_comp]; rw [ Scheme.Γ.map_comp]; rw [ Scheme.Γ_map_op]; rw [ Scheme.Γ_map_op], congr },
 { dsimp [functor.op],
...
begin
 rw [← h₁.cancel_right_is_iso _ (Scheme.Γ.map (is_open_immersion.iso_of_range_eq (Y.of_restrict _) f _).hom.op)]; rw [ ← functor.map_comp]; rw [ ← op_comp],
 convert h₂ ⟨_, range_is_affine_open_of_open_immersion f⟩ using 3,
 { rw [is_open_immersion.iso_of_range_eq_hom]; rw [ is_open_immersion.lift_fac_assoc] },
 { apply_instance },
...
begin
 ...
 { refine X.presheaf.map
 (@hom_of_le _ _ ((is_open_map.functor _).obj _) ((is_open_map.functor _).obj _) _).op,
 rw [unop_op]; rw [ unop_op]; rw [ opens.open_embedding_obj_top]; rw [ opens.open_embedding_obj_top],
 ...
 rwa [← Scheme.Γ.map_comp] at H ; rwa [ ← op_comp] at H ; rwa [ is_open_immersion.iso_of_range_eq_inv] at H ; rwa [ is_open_immersion.lift_fac_assoc] at H }
end
...
begin
 ...
 rw [← hP.respects_iso.cancel_right_is_iso _ (Scheme.Γ.map (is_open_immersion.iso_of_range_eq g (X.of_restrict (opens.open_embedding ⟨_, hg.base_open.open_range⟩)) subtype.range_coe.symm).hom.op)] at H; rw [ ← Scheme.Γ.map_comp] at H; rw [ ← op_comp] at H; rw [ is_open_immersion.iso_of_range_eq_hom] at H,
...
begin
 ...
 rw [category.assoc] at H; rw [ affine_cancel_left_is_iso this] at H; rw [ ← affine_cancel_left_is_iso this (pullback_symmetry _ _).hom] at H; rw [ pullback_symmetry_hom_comp_snd_assoc] at H,
 exact H }
end
...
begin
 ...
 { intro i, erw [category.id_comp]; erw [ op_id]; erw [ Scheme.Γ.map_id],
...
begin
 ...
 rw [category.assoc] at h; rw [ category.assoc] at h; rw [ pullback_right_pullback_fst_iso_hom_snd] at h; rw [ pullback.lift_snd_assoc] at h; rw [ category.assoc] at h; rw [ ← category.assoc] at h; rw [ op_comp] at h; rw [ functor.map_comp] at h,
 exact H _ _ h,
end
...
begin
 ...
 Scheme.pullback.open_cover_of_right_obj, Scheme.open_cover.pushforward_iso_map,
 Scheme.pullback.open_cover_of_right_map, Scheme.open_cover.bind_obj],
 rw [category.assoc]; rw [ category.assoc]; rw [ pullback_right_pullback_fst_iso_hom_snd]; rw [ pullback.lift_snd_assoc]; rw [ category.assoc]; rw [ ← category.assoc]; rw [ op_comp]; rw [ functor.map_comp],
