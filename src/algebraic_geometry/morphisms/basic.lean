...
by rw [← P.to_property_apply]; rw [ ← P.to_property_apply]; rw [ hP.cancel_left_is_iso]
...
by rw [← P.to_property_apply]; rw [ ← P.to_property_apply]; rw [ hP.cancel_right_is_iso]
...
begin
 split,
 { introv H U,
 rw [morphism_restrict_comp]; rw [ affine_cancel_left_is_iso hP],
 exact H U },
 { introv H,
 rintro ⟨U, hU : is_affine_open U⟩, dsimp,
 haveI : is_affine _ := hU,
 haveI : is_affine _ := hU.map_is_iso e.hom,
 rw [morphism_restrict_comp]; rw [ affine_cancel_right_is_iso hP],
...
begin
 ...
 intro i,
 specialize H i,
 rw [← P.to_property_apply]; rw [ ← hP.1.arrow_mk_iso_iff (morphism_restrict_opens_range f _)],
 rw ← P.to_property_apply at H,
 convert H,
...
begin
 ...
 { intro i, dsimp,
 rwa [← category.comp_id pullback.snd]; rwa [ ← pullback.condition]; rwa [ affine_cancel_left_is_iso hP] } },
 { introv hs hs',
 resetI,
 replace hs := ((top_is_affine_open Y).basic_open_union_eq_self_iff _).mpr hs,
 have := H f ⟨Y.open_cover_of_supr_eq_top _ hs, _, _⟩ (𝟙 _),
 rwa [← category.comp_id pullback.snd] at this; rwa [ ← pullback.condition] at this; rwa [ affine_cancel_left_is_iso hP] at this,
...
begin
 ...
 rw [← category.comp_id pullback.snd]; rw [ ← pullback.condition]; rw [ affine_cancel_left_is_iso hP.1],
end
...
begin
 ...
 { intros X Y f U H V,
 rw [← P.to_property_apply]; rw [ hP.1.arrow_mk_iso_iff (morphism_restrict_restrict f _ _)],
...
begin
 ...
 rw [← this]; rw [ affine_cancel_left_is_iso hP.1],
 apply hP'; assumption,
end
...
begin
 ...
 have : e.hom ≫ pullback.fst = pullback.snd := by simp,
 rw [← this]; rw [ (target_affine_locally_respects_iso hP.1).cancel_left_is_iso],
 apply hP.target_affine_locally_pullback_fst_of_right_of_stable_under_base_change hP',
 rw [← pullback_symmetry_hom_comp_snd]; rw [ affine_cancel_left_is_iso hP.1],
 apply H
end
...
begin
 ...
 { introv H _ _,
 resetI,
 rw [pullback.map_desc_comp]; rw [ affine_cancel_left_is_iso hP]; rw [ affine_cancel_right_is_iso hP],
 apply H },
 { introv H _ _,
 resetI,
 rw [pullback.map_desc_comp]; rw [ affine_cancel_right_is_iso hP],
 apply H }
end
...
begin
 ...
 (f₁ ≫ pullback.fst)
 (f₂ ≫ pullback.fst) g
 (by rw [category.assoc]; rw [ category.assoc]; rw [ pullback.condition])
 (by rw [category.assoc]; rw [ category.assoc]; rw [ pullback.condition]),
...
begin
 ...
 swap,
 { rw [category.assoc]; rw [ category.assoc]; rw [ ← pullback.condition]; rw [ ← pullback.condition_assoc]; rw [ H.w] },
