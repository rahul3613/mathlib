...
begin
 ...
 dsimp at eq,
 erw [w_assoc]; erw [ eq]; erw [ category.comp_id],
end
...
is_iso.eq_inv_of_hom_inv_id $ by rw [← comma.comp_left]; rw [ is_iso.hom_inv_id]; rw [ id_left]

@[simp] lemma inv_right [is_iso sq] : (inv sq).right = inv sq.right :=
is_iso.eq_inv_of_hom_inv_id $ by rw [← comma.comp_right]; rw [ is_iso.hom_inv_id]; rw [ id_right]
