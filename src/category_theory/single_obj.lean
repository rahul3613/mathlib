...
by { ext, rw [comp_as_mul]; rw [ inv_mul_self]; rw [ id_as_one] }
...
{ obj := λ _, (),
 map := λ x y _, f y * (f x)⁻¹,
 map_id' := by { intro, rw [single_obj.id_as_one]; rw [ mul_right_inv] },
 map_comp' := by { intros, rw [single_obj.comp_as_mul]; rw [ ←mul_assoc]; rw [ mul_left_inj]; rw [ mul_assoc]; rw [ inv_mul_self]; rw [ mul_one] } }
