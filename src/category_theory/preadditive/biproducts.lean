...
{ is_limit :=
 ...
 uniq' := λ s m h,
 begin
 erw [←category.comp_id m]; erw [ ←total]; erw [ comp_sum],
 ...
 uniq' := λ s m h,
 begin
 erw [←category.id_comp m]; erw [ ←total]; erw [ sum_comp],
 apply finset.sum_congr rfl,
 intros j m,
 erw [category.assoc]; erw [ h ⟨j⟩],
 end,
 fac' := λ s j,
 begin
 cases j,
...
{ is_limit :=
 ...
 uniq' := λ s m h, by erw [←category.comp_id m]; erw [ ←total]; erw [ comp_add]; erw [ reassoc_of (h ⟨walking_pair.left⟩)]; erw [ reassoc_of (h ⟨walking_pair.right⟩)],
 fac' := λ s j, by rcases j with ⟨⟨⟩⟩; simp, },
 is_colimit :=
 { desc := λ s, b.fst ≫ binary_cofan.inl s +
 b.snd ≫ binary_cofan.inr s,
 uniq' := λ s m h, by erw [←category.id_comp m]; erw [ ←total]; erw [ add_comp]; erw [ category.assoc]; erw [ category.assoc]; erw [ h ⟨walking_pair.left⟩]; erw [ h ⟨walking_pair.right⟩],
 fac' := λ s j, by rcases j with ⟨⟨⟩⟩; simp, } }
...
{ X := Y,
 ...
 begin
 dsimp only,
 rw [split_epi_of_idempotent_of_is_colimit_cofork_section_]; rw [ is_colimit_cofork_of_cokernel_cofork_desc]; rw [ is_cokernel_epi_comp_desc],
...
begin
 ...
 rw [is_colimit_cofork_of_cokernel_cofork_desc]; rw [ is_cokernel_epi_comp_desc],
...
{ X := X,
 ...
 begin
 dsimp only,
 rw [split_mono_of_idempotent_of_is_limit_fork_retraction]; rw [ is_limit_fork_of_kernel_fork_lift]; rw [ is_kernel_comp_mono_lift],
