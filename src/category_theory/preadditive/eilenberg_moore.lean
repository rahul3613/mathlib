...
{ hom_group := λ F G,
 ...
 nsmul := λ n α,
 { f := n • α.f,
 h' := by rw [functor.map_nsmul]; rw [ nsmul_comp]; rw [ monad.algebra.hom.h]; rw [ comp_nsmul] },
 neg := λ α,
 { f := -α.f,
 ...
 zsmul := λ r α,
 { f := r • α.f,
 h' := by rw [functor.map_zsmul]; rw [ zsmul_comp]; rw [ monad.algebra.hom.h]; rw [ comp_zsmul] },
...
{ hom_group := λ F G,
 ...
 nsmul := λ n α,
 { f := n • α.f,
 h' := by rw [functor.map_nsmul]; rw [ comp_nsmul]; rw [ comonad.coalgebra.hom.h]; rw [ nsmul_comp] },
 neg := λ α,
 { f := -α.f,
 ...
 zsmul := λ r α,
 { f := r • α.f,
 h' := by rw [functor.map_zsmul]; rw [ comp_zsmul]; rw [ comonad.coalgebra.hom.h]; rw [ zsmul_comp] },
 add_assoc := by { intros, ext, apply add_assoc },
 zero_add := by { intros, ext, apply zero_add },
 add_zero := by { intros, ext, apply add_zero },
 nsmul_zero' := by { intros, ext, apply zero_smul },
