...
begin
 ...
 rw [← s.condition]; rw [ ← F.map_comp_assoc]; rw [ ← U.map_comp]; rw [ regular_epi.w]; rw [ U.map_comp]; rw [ F.map_comp_assoc]; rw [ s.condition]; rw [ ← adj₁.counit_naturality_assoc] },
...
is_reflexive_pair.mk'
 ...
 (by {rw [← F'.map_comp]; rw [ adj₁.right_triangle_components], apply F'.map_id })
 begin
 dsimp [other_map],
...
calc (construct_left_adjoint_obj _ _ adj₁ adj₂ X ⟶ Y)
 ...
 intro f,
 rw [← (adj₂.hom_equiv _ _).injective.eq_iff]; rw [ eq_comm]; rw [ adj₂.hom_equiv_naturality_left]; rw [ other_map]; rw [ assoc]; rw [ adj₂.hom_equiv_naturality_left]; rw [ ← adj₂.counit_naturality]; rw [ adj₂.hom_equiv_naturality_left]; rw [ adj₂.hom_equiv_unit]; rw [ adj₂.right_triangle_components]; rw [ comp_id]; rw [ functor.comp_map]; rw [ ← U.map_comp]; rw [ assoc]; rw [ ← adj₁.counit_naturality]; rw [ adj₂.hom_equiv_unit]; rw [ adj₂.hom_equiv_unit]; rw [ F.map_comp]; rw [ assoc],
 refl,
 end
 ...
 intro g,
 rw [← (adj₁.hom_equiv _ _).symm.injective.eq_iff]; rw [ adj₁.hom_equiv_counit]; rw [ adj₁.hom_equiv_counit]; rw [ adj₁.hom_equiv_counit]; rw [ F.map_comp]; rw [ assoc]; rw [ U.map_comp]; rw [ F.map_comp]; rw [ assoc]; rw [ adj₁.counit_naturality]; rw [ adj₁.counit_naturality_assoc],
 apply eq_comm,
 end
...
begin
 ...
 intros X Y Y' g h,
 rw [construct_left_adjoint_equiv_apply]; rw [ construct_left_adjoint_equiv_apply]; rw [ function.comp_app]; rw [ function.comp_app]; rw [ equiv.trans_apply]; rw [ equiv.trans_apply]; rw [ equiv.trans_apply]; rw [ equiv.trans_apply]; rw [ equiv.symm_apply_eq]; rw [ subtype.ext_iff]; rw [ cofork.is_colimit.hom_iso_natural]; rw [ equiv.apply_symm_apply]; rw [ equiv.subtype_equiv_apply]; rw [ equiv.subtype_equiv_apply]; rw [ equiv.subtype_equiv_apply]; rw [ equiv.subtype_equiv_apply]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk]; rw [ subtype.coe_mk]; rw [ ← adj₁.hom_equiv_naturality_right_symm]; rw [ cofork.is_colimit.hom_iso_natural]; rw [ adj₂.hom_equiv_naturality_right]; rw [ functor.comp_map],
end
