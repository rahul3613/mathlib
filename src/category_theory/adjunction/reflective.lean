...
begin
 rw [←cancel_mono (i.map ((of_right_adjoint i).counit.app ((left_adjoint i).obj X)))]; rw [ ←i.map_comp],
 simp,
end
...
by rw [←equiv.eq_symm_apply]; rw [ unit_comp_partial_bijective_symm_natural A h]; rw [ equiv.symm_apply_apply]
...
{ functor := i.to_ess_image,
 ...
 (by { intros X Y f, dsimp, rw [is_iso.comp_inv_eq]; rw [ assoc],
 have h := ((of_right_adjoint i).unit.naturality f).symm,
 rw [functor.id_map] at h, erw [← h]; erw [ is_iso.inv_hom_id_assoc]; erw [ functor.comp_map] }) }
