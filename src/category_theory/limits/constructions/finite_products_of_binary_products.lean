...
{ lift := λ s,
 ...
 rw [fin.cases_succ]; rw [ ← assoc]; rw [ (binary_fan.is_limit.lift' t₂ _ _).2.2]; rw [ t₁.fac],
 refl }
 end,
 uniq' := λ s m w,
 begin
...
| (n+1) :=
 ...
 rw [fin.cases_succ]; rw [ fin.cases_succ],
 change F.map _ ≫ _ = 𝟙 _ ≫ _,
 rw [id_comp]; rw [ ←F.map_comp],
 refl }
 end
...
{ desc := λ s,
 ...
 rw [fin.cases_succ]; rw [ assoc]; rw [ (binary_cofan.is_colimit.desc' t₂ _ _).2.2]; rw [ t₁.fac],
 refl }
 end,
 uniq' := λ s m w,
 begin
