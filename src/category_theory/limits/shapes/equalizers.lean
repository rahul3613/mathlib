...
by rw [←s.app_zero_eq_ι]; rw [ ←s.w left]; rw [ parallel_pair_map_left]

@[reassoc] lemma fork.app_one_eq_ι_comp_right (s : fork f g) : s.π.app one = s.ι ≫ g :=
by rw [←s.app_zero_eq_ι]; rw [ ←s.w right]; rw [ parallel_pair_map_right]

@[simp] lemma cofork.app_zero_eq_comp_π_left (s : cofork f g) : s.ι.app zero = f ≫ s.π :=
by rw [←s.app_one_eq_π]; rw [ ←s.w left]; rw [ parallel_pair_map_left]

@[reassoc] lemma cofork.app_zero_eq_comp_π_right (s : cofork f g) : s.ι.app zero = g ≫ s.π :=
by rw [←s.app_one_eq_π]; rw [ ←s.w right]; rw [ parallel_pair_map_right]
...
by rw [←t.app_one_eq_ι_comp_left]; rw [ ←t.app_one_eq_ι_comp_right]

@[simp, reassoc]
lemma cofork.condition (t : cofork f g) : f ≫ t.π = g ≫ t.π :=
by rw [←t.app_zero_eq_comp_π_left]; rw [ ←t.app_zero_eq_comp_π_right]
...
| zero := h
| one := by rw [s.app_one_eq_ι_comp_left]; rw [ reassoc_of h]
...
{ lift := lift,
 ...
 by erw [←s.w left]; erw [ ←t.w left]; erw [ ←category.assoc]; erw [ fac]; refl,
...
{ desc := desc,
 ...
 (by erw [←s.w left]; erw [ ←t.w left]; erw [ category.assoc]; erw [ fac]; refl) (fac s),
 uniq' := by tidy }
...
{ hom := fork.mk_hom i.hom w,
 inv := fork.mk_hom i.inv (by rw [← w]; rw [ iso.inv_hom_id_assoc]) }
...
{ hom := cofork.mk_hom i.hom w,
 inv := cofork.mk_hom i.inv (by rw [iso.comp_inv_eq]; rw [ w]) }
...
lemma coequalizer.π_colim_map_desc {X' Y' Z : C} (f' g' : X' ⟶ Y') [has_coequalizer f' g']
 ...
 q ≫ h :=
by rw [ι_colim_map_assoc]; rw [ parallel_pair_hom_app_one]; rw [ coequalizer.π_desc]
...
⟨s.ι ≫ retraction f,
 by { dsimp, rw [category.assoc]; rw [ ←s.condition], apply category.comp_id },
 λ m hm, by simp [←hm]⟩
...
{ retraction := i.lift (fork.of_ι f (by simp [hf])),
 ...
 rw [←cancel_mono_id c.ι]; rw [ category.assoc]; rw [ fork.is_limit.lift_ι]; rw [ fork.ι_of_ι]; rw [ ←c.condition],
...
⟨section_ f ≫ s.π,
 by { dsimp, rw [← category.assoc]; rw [ ← s.condition]; rw [ category.id_comp] },
 λ m hm, by simp [← hm]⟩
...
{ section_ := i.desc (cofork.of_π f (by simp [hf])),
 ...
 rw [← cancel_epi_id c.π]; rw [ ← category.assoc]; rw [ cofork.is_colimit.π_desc]; rw [ cofork.π_of_π]; rw [ ← c.condition],
