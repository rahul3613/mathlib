...
by erw [fork.condition]; erw [ has_zero_morphisms.comp_zero]
...
fork.of_ι ι $ by rw [w]; rw [ has_zero_morphisms.comp_zero]
...
def of_ι_congr {P : C} {ι ι' : P ⟶ X} {w : ι ≫ f = 0} (h : ι = ι') :
 kernel_fork.of_ι ι w ≅ kernel_fork.of_ι ι' (by rw [←h]; rw [ w]) :=
cones.ext (iso.refl _) $ by tidy
...
lemma is_kernel_comp_mono_lift {c : kernel_fork f} (i : is_limit c) {Z} (g : Y ⟶ Z) [hg : mono g]
 ...
 = i.lift (fork.of_ι s.ι (by { rw [←cancel_mono g]; rw [ category.assoc]; rw [ ←hh], simp })) := rfl
...
fork.is_limit.mk _ (λ s, 0)
 (λ s, by rw [hf _ _ (kernel_fork.condition s)]; rw [ zero_comp])
 (λ s m h, by ext)
...
by rw [cofork.condition]; rw [ zero_comp]
...
cofork.of_π π $ by rw [w]; rw [ zero_comp]
...
def of_π_congr {P : C} {π π' : Y ⟶ P} {w : f ≫ π = 0} (h : π = π') :
 cokernel_cofork.of_π π w ≅ cokernel_cofork.of_π π' (by rw [←h]; rw [ w]) :=
cocones.ext (iso.refl _) $ λ j, by cases j; tidy
...
cofork.is_colimit.mk' _ $ λ s,
 ...
 (by { apply hg.left_cancellation, rw [←category.assoc]; rw [ ←hh]; rw [ s.condition], simp }) in
...
lemma is_cokernel_epi_comp_desc {c : cokernel_cofork f} (i : is_colimit c) {W}
 ...
 = i.desc (cofork.of_π s.π (by { rw [←cancel_epi g]; rw [ ←category.assoc]; rw [ ←hh], simp })) := rfl
...
{ exists_colimit :=
 ...
 (by { rw [←category.assoc]; rw [ cokernel_cofork.condition], }))
 (by tidy)
 (λ s m w, by { simp_rw [←w], ext, simp, }), }⟩ }
...
{ hom := cokernel.desc _ (inv g ≫ cokernel.π f) (by simp),
 inv := cokernel.desc _ (g ≫ cokernel.π (f ≫ g)) (by rw [←category.assoc]; rw [ cokernel.condition]), }
...
{ hom := cokernel.desc _ (cokernel.π g) (by simp),
 inv := cokernel.desc _ (cokernel.π (f ≫ g)) (by { rw [←cancel_epi f]; rw [ ←category.assoc], simp, }), }
...
@[simp] lemma mono_factorisation.kernel_ι_comp [has_kernel f] (F : mono_factorisation f) :
 kernel.ι f ≫ F.e = 0 :=
by rw [← cancel_mono F.m]; rw [ zero_comp]; rw [ category.assoc]; rw [ F.fac]; rw [ kernel.condition]
...
{ hom := cokernel.desc _ (cokernel.π f)
 ...
 rw [←has_zero_morphisms.comp_zero (limits.factor_thru_image f)]; rw [ category.assoc]; rw [ cancel_epi]
 at w,
 exact w,
 end,
 ...
 rw [category.assoc]; rw [ cokernel.condition]; rw [ has_zero_morphisms.comp_zero],
 end, }
...
cofork.is_colimit.mk _ (λ s, 0)
 (λ s, by rw [hf _ _ (cokernel_cofork.condition s)]; rw [ comp_zero])
 (λ s m h, by ext)
...
@[reassoc] lemma kernel_comparison_comp_kernel_map {X' Y' : C} [has_kernel f]
 ...
 (by rw [←G.map_comp]; rw [ hpq]; rw [ G.map_comp])) =
 G.map (kernel.map f g p q hpq) ≫ kernel_comparison g G :=
kernel.lift_map _ _ (by rw [←G.map_comp]; rw [ kernel.condition]; rw [ G.map_zero]) _ _
 (by rw [←G.map_comp]; rw [ kernel.condition]; rw [ G.map_zero]) _ _ _
...
@[reassoc] lemma cokernel_map_comp_cokernel_comparison {X' Y' : C} [has_cokernel f]
 ...
 cokernel.map (G.map f) (G.map g) (G.map p) (G.map q) (by rw [←G.map_comp]; rw [ hpq]; rw [ G.map_comp])
 ≫ cokernel_comparison _ G = cokernel_comparison _ G ≫ G.map (cokernel.map f g p q hpq) :=
cokernel.map_desc _ _ (by rw [←G.map_comp]; rw [ cokernel.condition]; rw [ G.map_zero]) _ _
 (by rw [←G.map_comp]; rw [ cokernel.condition]; rw [ G.map_zero]) _ _ _ _
