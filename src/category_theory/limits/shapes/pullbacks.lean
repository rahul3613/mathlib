...
{ lift := lift,
 ...
 (by { rw [← s.w inl]; rw [ ← t.w inl]; rw [ ←category.assoc], congr, exact fac_left s, } )
...
| none := by rw [← t.w inl]; rw [ reassoc_of h₀]
...
is_limit.mk _
 ...
 (λ s, by rw [←cancel_mono f]; rw [ category.comp_id]; rw [ s.condition])
...
pullback_cone.is_limit_aux' _ $ λ t,
 ⟨hs.lift (pullback_cone.mk t.fst t.snd $ by rw [←hxh]; rw [ ←hyh]; rw [ reassoc_of t.condition]),
...
def is_limit_of_comp_mono (f : X ⟶ W) (g : Y ⟶ W) (i : W ⟶ Z) [mono i]
 ...
 by rw [← category.assoc]; rw [ ← category.assoc]; rw [ s.condition])) :=
begin
...
| (some walking_pair.left) := h₀
| (some walking_pair.right) := h₁
| none := by rw [← t.w fst]; rw [ category.assoc]; rw [ category.assoc]; rw [ h₀]
...
is_colimit.mk _
 ...
 (λ s, by rw [←cancel_epi f]; rw [ category.id_comp]; rw [ s.condition])
 (λ s m m₁ m₂, by simpa using m₁)
...
def is_colimit_of_factors (f : X ⟶ Y) (g : X ⟶ Z) (h : X ⟶ W) [epi h]
 ...
 from (cancel_epi h).1 $ by rw [reassoc_of hhx]; rw [ reassoc_of hhy]; rw [ s.condition])) :=
pushout_cocone.is_colimit_aux' _ $ λ t,
 ⟨hs.desc (pushout_cocone.mk t.inl t.inr $
 by rw [←hhx]; rw [ ←hhy]; rw [ category.assoc]; rw [ category.assoc]; rw [ t.condition]),
...
begin
 ...
 { rw [is_iso.comp_inv_eq]; rw [ category.assoc]; rw [ eq₁]; rw [ is_iso.inv_hom_id_assoc] },
 { rw [is_iso.comp_inv_eq]; rw [ category.assoc]; rw [ eq₂]; rw [ is_iso.inv_hom_id_assoc] },
 tidy
end
...
def big_square_is_pullback (H : is_limit (pullback_cone.mk _ _ h₂))
 ...
 by rw [← category.assoc]; rw [ h₁]; rw [ category.assoc]; rw [ h₂]; rw [ category.assoc])) :=
begin
 fapply pullback_cone.is_limit_aux',
 intro s,
 have : (s.fst ≫ g₁) ≫ g₂ = s.snd ≫ i₃ := by rw [← s.condition]; rw [ category.assoc],
 ...
 use l₂,
 use hl₂,
 use show l₂ ≫ f₁ ≫ f₂ = s.snd, by { rw [← hl₁']; rw [ ← hl₂']; rw [ category.assoc], refl },
 intros m hm₁ hm₂,
 apply pullback_cone.is_limit.hom_ext H',
 { erw [hm₁]; erw [ hl₂] },
 { apply pullback_cone.is_limit.hom_ext H,
 { erw [category.assoc]; erw [ ← h₁]; erw [ ← category.assoc]; erw [ hm₁]; erw [ ← hl₂]; erw [ category.assoc]; erw [ category.assoc]; erw [ h₁], refl },
 { erw [category.assoc]; erw [ hm₂]; erw [ ← hl₁']; erw [ ← hl₂'] } }
end
...
begin
 ...
 intro s,
 have : i₁ ≫ s.inl = f₁ ≫ (f₂ ≫ s.inr) := by rw [s.condition]; rw [ category.assoc],
 ...
 use l₂,
 use show (g₁ ≫ g₂) ≫ l₂ = s.inl, by { rw [← hl₁]; rw [ ← hl₂]; rw [ category.assoc], refl },
 use hl₂',
 intros m hm₁ hm₂,
...
begin
 ...
 by { rw [← category.assoc]; rw [ s.condition]; rw [ category.assoc]; rw [ category.assoc]; rw [ h₂] },
 rcases pullback_cone.is_limit.lift' H' s.fst (s.snd ≫ f₂) this with ⟨l₁, hl₁, hl₁'⟩,
 use l₁,
 use hl₁,
 split,
 { apply pullback_cone.is_limit.hom_ext H,
 { erw [category.assoc]; erw [ ← h₁]; erw [ ← category.assoc]; erw [ hl₁]; erw [ s.condition], refl },
 { erw [category.assoc]; erw [ hl₁'], refl } },
 { intros m hm₁ hm₂,
 apply pullback_cone.is_limit.hom_ext H',
 { erw [hm₁]; erw [ hl₁] },
 { erw [hl₁']; erw [ ← hm₂], exact (category.assoc _ _ _).symm } }
end
...
begin
 ...
 { apply pushout_cocone.is_colimit.hom_ext H,
 { erw [← category.assoc]; erw [ hl₁], refl },
 { erw [← category.assoc]; erw [ h₂]; erw [ category.assoc]; erw [ hl₁']; erw [ s.condition] } },
 { exact hl₁' },
 { intros m hm₁ hm₂,
 apply pushout_cocone.is_colimit.hom_ext H',
 { erw [hl₁]; erw [ category.assoc]; erw [ hm₁] },
 { erw [hm₂]; erw [ hl₁'] } }
end
...
by rw [iso.comp_inv_eq]; rw [ inr_pushout_left_pushout_inr_iso_hom]

@[simp, reassoc]
lemma inl_inl_pushout_left_pushout_inr_iso_hom :
 pushout.inl ≫ pushout.inl ≫ (pushout_left_pushout_inr_iso f g g').hom = pushout.inl :=
by rw [← category.assoc]; rw [ ← iso.eq_comp_inv]; rw [ inl_pushout_left_pushout_inr_iso_inv]
...
by rw [← category.assoc]; rw [ ← iso.eq_comp_inv]; rw [ category.assoc]; rw [ inr_pushout_left_pushout_inr_iso_inv]; rw [ pushout.condition]
...
is_limit (pullback_cone.mk (l₁ ≫ g₁) l₂ (show (l₁ ≫ g₁) ≫ f₁ = l₂ ≫ (g₃ ≫ f₂),
 by rw [pullback.lift_fst_assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ pullback.condition])) :=
begin
...
is_limit (pullback_cone.mk l₁' (l₂' ≫ g₄) (show l₁' ≫ (g₂ ≫ f₃) = (l₂' ≫ g₄) ≫ f₄,
 by rw [pullback.lift_snd_assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ pullback.condition])) :=
begin
 apply big_square_is_pullback,
 exact pullback_is_pullback f₃ f₄,
...
by rw [← iso.eq_inv_comp]; rw [ pullback_assoc_inv_fst_fst]
...
by rw [iso.inv_comp_eq]; rw [ pullback_assoc_hom_snd_fst]

@[simp, reassoc]
lemma pullback_assoc_inv_snd :
 (pullback_assoc f₁ f₂ f₃ f₄).inv ≫ pullback.snd = pullback.snd ≫ pullback.snd :=
by rw [iso.inv_comp_eq]; rw [ pullback_assoc_hom_snd_snd]
...
def pushout_assoc_is_pushout [has_pushout (g₃ ≫ f₂) g₄] :
 ...
 by rw [category.assoc]; rw [ pushout.inl_desc]; rw [ pushout.condition_assoc])) :=
begin
...
def pushout_assoc_symm_is_pushout [has_pushout g₁ (g₂ ≫ f₃)] :
 ...
 by rw [category.assoc]; rw [ pushout.inr_desc]; rw [ pushout.condition_assoc]))) :=
begin
 apply pushout_cocone.flip_is_colimit,
 apply big_square_is_pushout,
 { apply pushout_cocone.flip_is_colimit,
...
by rw [iso.comp_inv_eq]; rw [ category.assoc]; rw [ inl_inl_pushout_assoc_hom]

@[simp, reassoc]
lemma inl_inr_pushout_assoc_inv :
 pushout.inl ≫ pushout.inr ≫ (pushout_assoc g₁ g₂ g₃ g₄).inv = pushout.inr ≫ pushout.inl :=
by rw [← category.assoc]; rw [ iso.comp_inv_eq]; rw [ category.assoc]; rw [ inr_inl_pushout_assoc_hom]

@[simp, reassoc]
lemma inr_pushout_assoc_hom :
 pushout.inr ≫ (pushout_assoc g₁ g₂ g₃ g₄).hom = pushout.inr ≫ pushout.inr :=
by rw [← iso.eq_comp_inv]; rw [ category.assoc]; rw [ inr_inr_pushout_assoc_inv]
