...
by rw [←app_right_eq_ι_comp_fst]; rw [ ←app_right_eq_ι_comp_snd]
...
{ lift := lift,
 ...
 rintros E (a|b),
 { apply fac },
 { rw [← E.w (walking_multicospan.hom.fst b)]; rw [ ← K.w (walking_multicospan.hom.fst b)]; rw [ ← category.assoc],
 congr' 1,
 apply fac }
 end,
 uniq' := begin
...
by rw [←K.snd_app_right]; rw [ ←K.fst_app_right]
...
{ desc := desc,
 fac' := begin
 rintros S (a|b),
 { rw [← K.w (walking_multispan.hom.fst a)]; rw [ ← S.w (walking_multispan.hom.fst a)]; rw [ category.assoc],
 congr' 1,
 apply fac },
 { apply fac },
 end,
...
{ X := c.X,
 ...
 rw [←I.ι_fst_sigma_map_assoc]; rw [ c.condition] },
...
by { rw [ι_pi]; rw [ category.assoc]; rw [ ← iso.eq_inv_comp]; rw [ iso_equalizer], simpa }
...
by { rw [sigma_π]; rw [ ← category.assoc]; rw [ iso.comp_inv_eq]; rw [ iso_coequalizer], simpa }
