...
begin
 ...
 { rw [← category.comp_id pullback.snd]; rw [ ← condition]; rw [ category.assoc]; rw [ is_iso.inv_hom_id_assoc] },
 { rw [← category.comp_id pullback.snd]; rw [ ← condition]; rw [ category.assoc]; rw [ is_iso.inv_hom_id_assoc] },
 { rw [category.comp_id]; rw [ category.id_comp] },
 { ext; simp },
 { apply_instance },
 { rw [category.assoc]; rw [ category.id_comp]; rw [ is_iso.inv_hom_id]; rw [ category.comp_id] },
 { rw [category.assoc]; rw [ category.id_comp]; rw [ is_iso.inv_hom_id]; rw [ category.comp_id] },
 { apply_instance },
end
...
lemma pullback_lift_map_is_pullback {X Y S X' Y' S' : C} (f : X ⟶ S) (g : Y ⟶ S) (f' : X' ⟶ S')
 ...
 pullback.fst pullback.fst :=
is_pullback.of_iso_pullback ⟨by rw [lift_fst]; rw [ lift_fst]⟩
