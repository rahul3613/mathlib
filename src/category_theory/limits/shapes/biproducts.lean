...
begin
 ...
 split_ifs,
 { subst h, rw [eq_to_hom_refl]; rw [ category.id_comp], erw category.comp_id, },
 { simp, },
end
...
begin
 ext i,
 rw [biproduct.from_subtype]; rw [ biproduct.ι_desc_assoc]; rw [ biproduct.ι_π],
 by_cases h : p j,
 { rw [dif_pos h]; rw [ biproduct.ι_π],
 split_ifs with h₁ h₂ h₂,
 exacts [rfl, false.elim (h₂ (subtype.ext h₁)),
 false.elim (h₁ (congr_arg subtype.val h₂)), rfl] },
 { rw [dif_neg h]; rw [ dif_neg (show (i : J) ≠ j, from λ h₂, h (h₂ ▸ i.2))]; rw [ comp_zero] }
end
...
begin
 ext i,
 rw [biproduct.from_subtype]; rw [ biproduct.ι_desc_assoc]; rw [ biproduct.ι_π]; rw [ biproduct.ι_π],
 split_ifs with h₁ h₂ h₂,
...
begin
 ext i,
 rw [biproduct.to_subtype]; rw [ category.assoc]; rw [ biproduct.lift_π]; rw [ biproduct.ι_π],
 by_cases h : p j,
 { rw [dif_pos h]; rw [ biproduct.ι_π],
 split_ifs with h₁ h₂ h₂,
 exacts [rfl, false.elim (h₂ (subtype.ext h₁)),
 false.elim (h₁ (congr_arg subtype.val h₂)), rfl] },
 { rw [dif_neg h]; rw [ dif_neg (show j ≠ i, from λ h₂, h (h₂.symm ▸ i.2))]; rw [ zero_comp] }
end
...
begin
 ext i,
 rw [biproduct.to_subtype]; rw [ category.assoc]; rw [ biproduct.lift_π]; rw [ biproduct.ι_π]; rw [ biproduct.ι_π],
 split_ifs with h₁ h₂ h₂,
 exacts [rfl, false.elim (h₂ (subtype.ext h₁)), false.elim (h₁ (congr_arg subtype.val h₂)), rfl]
end
...
begin
 ...
 rw [category.assoc]; rw [ biproduct.to_subtype_π]; rw [ biproduct.from_subtype_π_subtype]; rw [ category.id_comp]
end
...
⟨s.ι ≫ biproduct.to_subtype _ _,
 begin
 ext j,
 rw [kernel_fork.ι_of_ι]; rw [ category.assoc]; rw [ category.assoc]; rw [ biproduct.to_subtype_from_subtype_assoc]; rw [ biproduct.map_π],
 rcases em (i = j) with (rfl|h),
 { rw [if_neg (not_not.2 rfl)]; rw [ comp_zero]; rw [ comp_zero]; rw [ kernel_fork.condition] },
 { rw [if_pos (ne.symm h)]; rw [ category.comp_id], }
 end,
 begin
 intros m hm,
 rw [← hm]; rw [ kernel_fork.ι_of_ι]; rw [ category.assoc]; rw [ biproduct.from_subtype_to_subtype],
...
⟨biproduct.from_subtype _ _ ≫ s.π,
 begin
 ext j,
 rw [cokernel_cofork.π_of_π]; rw [ biproduct.to_subtype_from_subtype_assoc]; rw [ biproduct.ι_map_assoc],
 rcases em (i = j) with (rfl|h),
 { rw [if_neg (not_not.2 rfl)]; rw [ zero_comp]; rw [ cokernel_cofork.condition] },
 { rw [if_pos (ne.symm h)]; rw [ category.id_comp], }
 end,
 begin
 intros m hm,
 rw [← hm]; rw [ cokernel_cofork.π_of_π]; rw [ ← category.assoc]; rw [ biproduct.from_subtype_to_subtype],
...
begin
 ...
 discrete_cases,
 rw [category.assoc]; rw [ is_limit.cone_point_unique_up_to_iso_inv_comp]; rw [ bicone.to_cone_π_app]; rw [ biproduct.bicone_π]; rw [ biproduct.ι_desc]; rw [ biproduct.ι_π]; rw [ b.to_cone_π_app]; rw [ b.ι_π]
end
...
{ hom := biproduct.lift b.π,
 ...
 hom_inv_id' := by rw [← biproduct.cone_point_unique_up_to_iso_hom f hb]; rw [ ← biproduct.cone_point_unique_up_to_iso_inv f hb]; rw [ iso.hom_inv_id],
 inv_hom_id' := by rw [← biproduct.cone_point_unique_up_to_iso_hom f hb]; rw [ ← biproduct.cone_point_unique_up_to_iso_inv f hb]; rw [ iso.inv_hom_id] }
...
{ hom := biprod.lift b.fst b.snd,
 ...
 hom_inv_id' := by rw [← biprod.cone_point_unique_up_to_iso_hom X Y hb]; rw [ ← biprod.cone_point_unique_up_to_iso_inv X Y hb]; rw [ iso.hom_inv_id],
 inv_hom_id' := by rw [← biprod.cone_point_unique_up_to_iso_hom X Y hb]; rw [ ← biprod.cone_point_unique_up_to_iso_inv X Y hb]; rw [ iso.inv_hom_id] }
...
begin
 ...
 rw [is_iso.inv_hom_id_assoc] at this; rw [ category.comp_id] at this,
 rw [this]; rw [ is_iso.inv_hom_id], },
