...
begin
 ...
 split,
 { rintro ⟨h⟩,
 rw [← show _ = c.inl]; rw [ from h.comp_cocone_point_unique_up_to_iso_inv (binary_coproduct_colimit X Y) ⟨walking_pair.left⟩]; rw [ ← show _ = c.inr]; rw [ from h.comp_cocone_point_unique_up_to_iso_inv (binary_coproduct_colimit X Y) ⟨walking_pair.right⟩],
 ...
 erw [set.range_comp]; erw [ ← eq_compl_iff_is_compl]; erw [ set.range_comp _ sum.inr]; erw [ ← set.image_compl_eq (h.cocone_point_unique_up_to_iso (binary_coproduct_colimit X Y)).symm.to_equiv.bijective],
 congr' 1,
