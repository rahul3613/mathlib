...
{ out := λ I A f hI,
 begin
 haveI := h A (f ≫ hI.to _),
 exact ⟨⟨hI.to _ ≫ inv (f ≫ hI.to ⊥_ C), by rw [←assoc]; rw [ is_iso.hom_inv_id], hI.hom_ext _ _⟩⟩,
 end }
...
begin
 ...
 dsimp only,
 split_ifs,
 { cases h, rw [id_comp]; rw [ eq_to_hom_refl], exact comp_id _ },
 { apply (H _ h).hom_ext } },
...
{ out := λ I' A f hI',
 ...
 hI'.hom_ext _ _, by rw [assoc]; rw [ is_iso.inv_hom_id]⟩⟩,
 end }
