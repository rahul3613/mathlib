...
lemma cancel_right_of_mono {f₁ : X ⟶ Y} {f₂ : Y ⟶ Z} [mono f₂]
 (big_k : is_kernel_pair (f₁ ≫ f₂) a b) :
 is_kernel_pair f₁ a b :=
cancel_right (begin rw [← cancel_mono f₂]; rw [ assoc]; rw [ assoc]; rw [ big_k.w] end) big_k
...
{ w := by rw [small_k.w_assoc],
 ...
 rw [← cancel_mono f₂]; rw [ assoc]; rw [ s.condition]; rw [ assoc],
...
begin
 ...
 rw [← ht]; rw [ assoc]; rw [ s.condition]; rw [ reassoc_of kt] },
 { intro s,
...
begin
 refine ⟨⟨_⟩, ⟨_⟩⟩,
 { rw [pullback.lift_fst]; rw [ pullback.lift_fst] },
 ...
 { rw [← category.assoc]; rw [ (h.lift' _ _ _).2.1]; rw [ category.assoc]; rw [ category.assoc]; rw [ pullback.condition] },
 ...
 conv_rhs { rw [← h₁]; rw [ category.assoc]; rw [ pullback_cone.mk_fst] },
 congr' 1,
 ...
 { conv_rhs { rw [← h₁]; rw [ category.assoc]; rw [ pullback_cone.mk_fst]; rw [ pullback.lift_snd] } },
 { conv_rhs { rw [← h₂]; rw [ category.assoc]; rw [ pullback_cone.mk_snd]; rw [ pullback.lift_snd] } } } } }
end
...
begin
 ...
 rw [is_pullback.cone_fst] at h₁; rw [ ← is_iso.eq_comp_inv] at h₁; rw [ category.id_comp] at h₁,
 rw [h₁] at h₂; rw [ is_iso.inv_comp_eq] at h₂; rw [ category.comp_id] at h₂,
 constructor,
 intros Z g₁ g₂ e,
 obtain ⟨l', rfl, rfl⟩ := limits.pullback_cone.is_limit.lift' h.is_limit _ _ e,
 rw [is_pullback.cone_fst]; rw [ h₂],
end
