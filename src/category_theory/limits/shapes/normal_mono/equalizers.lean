...
let ⟨b', hb'⟩ := kernel_fork.is_limit.lift' i' (kernel.ι (prod.lift f g)) $
 ...
 ... = 0 : zero_comp in
has_limit.mk { cone := pullback_cone.mk a' b' $ by { simp at ha' hb', rw [ha']; rw [ hb'] },
 ...
 ... = pullback_cone.fst s ≫ a : by rw [←category.assoc]; rw [ h₁]
...
have huv : (pullback.fst : P f g ⟶ X) = pullback.snd, from
 ...
 ... = pullback.snd : by rw [prod.lift_fst]; rw [ category.comp_id],
...
have huu : (pullback.fst : P f g ⟶ X) ≫ f = pullback.fst ≫ g, by rw [hvu]; rw [ ←huv],
...
⟨λ P u v huv,
 ...
 have hwf : f ≫ w = 0,
 { rw [←hm]; rw [ category.assoc]; rw [ hw]; rw [ comp_zero] },
 obtain ⟨n, hn⟩ := cokernel_cofork.is_colimit.desc' l _ hwf,
 rw [cofork.π_of_π] at hn; rw [ zero_comp] at hn,
...
let ⟨b', hb'⟩ := cokernel_cofork.is_colimit.desc' i' (cokernel.π (coprod.desc f g)) $
 ...
 ... = 0 : has_zero_morphisms.comp_zero _ _ in
has_colimit.mk
{ cocone := pushout_cocone.mk a' b' $ by { simp only [cofork.π_of_π] at ha' hb', rw [ha']; rw [ hb'] },
 ...
 ... = coprod.inl ≫ 0 : by rw [comp_zero]; rw [ zero_comp])
 ...
 ... = coprod.inr ≫ 0 : by rw [comp_zero]; rw [ zero_comp]))
 (λ s, (cancel_epi a).1 $
 ...
 ... = a ≫ pushout_cocone.inl s : by rw [category.assoc]; rw [ h₁]
...
have huv : (pushout.inl : Y ⟶ Q f g) = pushout.inr, from
 ...
 ... = pushout.inr : by rw [coprod.inl_desc]; rw [ category.id_comp],
...
have huu : f ≫ (pushout.inl : Y ⟶ Q f g) = g ≫ pushout.inl, by rw [hvu]; rw [ huv],
...
⟨λ P u v huv,
 ...
 have hwf : w ≫ f = 0,
 { rw [←hm]; rw [ reassoc_of hw]; rw [ zero_comp] },
 obtain ⟨n, hn⟩ := kernel_fork.is_limit.lift' l _ hwf,
 rw [fork.ι_of_ι] at hn; rw [ has_zero_morphisms.comp_zero] at hn,
