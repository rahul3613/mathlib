...
by rw [iso.inv_comp_eq]; rw [ ← category.assoc]; rw [ iso.eq_comp_inv]; rw [ prod.left_unitor_hom_naturality]
...
by rw [iso.inv_comp_eq]; rw [ ← category.assoc]; rw [ iso.eq_comp_inv]; rw [ prod.right_unitor_hom_naturality]
...
begin
 rw [prod_comparison]; rw [ prod_comparison]; rw [ prod.lift_map]; rw [ ← F.map_comp]; rw [ ← F.map_comp]; rw [ prod.comp_lift]; rw [ ← F.map_comp]; rw [ prod.map_fst]; rw [ ← F.map_comp]; rw [ prod.map_snd]
end
...
by rw [is_iso.eq_comp_inv]; rw [ category.assoc]; rw [ is_iso.inv_comp_eq]; rw [ prod_comparison_natural]
...
begin
 rw [coprod_comparison]; rw [ coprod_comparison]; rw [ coprod.map_desc]; rw [ ← F.map_comp]; rw [ ← F.map_comp]; rw [ coprod.desc_comp]; rw [ ← F.map_comp]; rw [ coprod.inl_map]; rw [ ← F.map_comp]; rw [ coprod.inr_map]
end
...
by rw [is_iso.eq_comp_inv]; rw [ category.assoc]; rw [ is_iso.inv_comp_eq]; rw [ coprod_comparison_natural]
...
{ obj := λ f, over.coprod_obj f,
 ...
 (by { dsimp, rw [coprod.map_desc]; rw [ category.id_comp]; rw [ over.w k] }),
