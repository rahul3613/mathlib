...
by rw [←s.w (line j)]; rw [ parallel_family_map_left]
...
| zero := h
| one := by rw [←s.app_zero (classical.arbitrary J)]; rw [ reassoc_of h]
...
| zero := by rw [←s.app_one (classical.arbitrary J)]; rw [ category.assoc]; rw [ category.assoc]; rw [ h]
| one := h
...
{ lift := lift,
 ...
 (by rw [←t.w (line (classical.arbitrary J))]; rw [ reassoc_of fac]; rw [ s.w]),
 uniq' := uniq }
...
{ desc := desc,
 ...
 (by rw [←t.w_assoc (line (classical.arbitrary J))]; rw [ fac]; rw [ s.w]) (fac s),
 uniq' := uniq }
...
{ hom := trident.mk_hom i.hom w,
 inv := trident.mk_hom i.inv (by rw [← w]; rw [ iso.inv_hom_id_assoc]) }
...
{ hom := cotrident.mk_hom i.hom w,
 inv := cotrident.mk_hom i.inv (by rw [iso.comp_inv_eq]; rw [ w]) }
