...
{ map_zero' := λ X Y, calc
 ...
 ... = 0 : by rw [F.map_comp]; rw [ F.image_preimage]; rw [ comp_zero] }
...
{ hom := 0,
 inv := 0,
 hom_inv_id' := by rw [← F.map_id]; rw [ id_zero]; rw [ F.map_zero]; rw [ zero_comp],
 inv_hom_id' := by rw [id_zero]; rw [ comp_zero] }

variables {F}

lemma preserves_zero_morphisms_of_map_zero_object (i : F.obj 0 ≅ 0) : preserves_zero_morphisms F :=
{ map_zero' := λ X Y, calc
 F.map (0 : X ⟶ Y) = F.map (0 : X ⟶ 0) ≫ F.map 0 : by rw [← functor.map_comp]; rw [ comp_zero]
 ... = F.map 0 ≫ (i.hom ≫ i.inv) ≫ F.map 0
 : by rw [iso.hom_inv_id]; rw [ category.id_comp]
