...
{ X := F.obj b.X,
 ...
 { rw [bicone_ι_π_ne _ h]; rw [ F.map_zero] }
 end }
...
{ X := F.obj b.X,
 ...
 inl := F.map b.inl,
 inr := F.map b.inr,
 inl_fst' := by rw [← F.map_comp]; rw [ b.inl_fst]; rw [ F.map_id],
 inl_snd' := by rw [← F.map_comp]; rw [ b.inl_snd]; rw [ F.map_zero],
 inr_fst' := by rw [← F.map_comp]; rw [ b.inr_fst]; rw [ F.map_zero],
 inr_snd' := by rw [← F.map_comp]; rw [ b.inr_snd]; rw [ F.map_id] }
...
by rw [← biproduct.map_biproduct_inv_map_desc]; rw [ iso.hom_inv_id_assoc]
...
by rw [← biprod.map_lift_map_biprod]; rw [ category.assoc]; rw [ iso.hom_inv_id]; rw [ category.comp_id]
...
by rw [← biprod.map_biprod_inv_map_desc]; rw [ iso.hom_inv_id_assoc]
