...
begin
 rw [←ι_comp_coequalizer_comparison_assoc]; rw [ ←preserves_coequalizer.iso_hom]; rw [ iso.hom_inv_id]; rw [ comp_id],
end
...
by rw [←category.assoc]; rw [ map_π_preserves_coequalizer_inv]; rw [ coequalizer.π_desc]
...
lemma map_π_preserves_coequalizer_inv_colim_map
 ...
 q ≫ coequalizer.π f' g' :=
by rw [←category.assoc]; rw [ map_π_preserves_coequalizer_inv]; rw [ ι_colim_map]; rw [ parallel_pair_hom_app_one]
