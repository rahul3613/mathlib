...
@[reassoc] lemma kernel_map_comp_preserves_kernel_iso_inv {X' Y' : C} (g : X' ⟶ Y') [has_kernel g]
 ...
 (by rw [←G.map_comp]; rw [ hpq]; rw [ G.map_comp]) ≫ (preserves_kernel.iso G _).inv
 = (preserves_kernel.iso G _).inv ≫ G.map (kernel.map f g p q hpq) :=
begin
 rw [iso.comp_inv_eq]; rw [ category.assoc]; rw [ preserves_kernel.iso_hom]; rw [ iso.eq_inv_comp],
...
begin
 rw [←iso.comp_inv_eq]; rw [ category.assoc]; rw [ ←iso.eq_inv_comp],
