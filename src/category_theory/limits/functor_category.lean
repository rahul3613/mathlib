...
by rw [iso.inv_comp_eq]; rw [ ← category.assoc]; rw [ iso.eq_comp_inv]; rw [ limit_map_limit_obj_iso_limit_comp_evaluation_hom]
...
lemma colimit_map_colimit_obj_iso_colimit_comp_evaluation_hom
 ...
 colim_map (whisker_left _ ((evaluation _ _).map f)) :=
by rw [← iso.inv_comp_eq]; rw [ ← category.assoc]; rw [ ← iso.eq_comp_inv]; rw [ colimit_obj_iso_colimit_comp_evaluation_inv_colimit_map]
