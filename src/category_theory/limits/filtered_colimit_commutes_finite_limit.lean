...
begin
 ...
 erw [colimit.w_apply']; erw [ t]; erw [ functor_to_types.map_comp_apply]; erw [ colimit.w_apply']; erw [ e]; erw [ ←limit.w_apply' f]; erw [ ←e],
 simp, },
 ...
 : by rw [←functor_to_types.map_comp_apply]; rw [ prod_comp]; rw [ comp_id]; rw [ assoc]
 ... = F.map ((𝟙 j', i f) : (j', kf f) ⟶ (j', k''))
 (F.map ((𝟙 j', g j' ≫ gf f) : (j', k j') ⟶ (j', kf f)) (y j'))
 : by rw ←wf f
 ... = F.map ((𝟙 j', g j' ≫ gf f ≫ i f) : (j', k j') ⟶ (j', k'')) (y j')
 : by rw [←functor_to_types.map_comp_apply]; rw [ prod_comp]; rw [ id_comp]; rw [ assoc]
 ... = F.map ((𝟙 j', g j' ≫ gf (𝟙 j') ≫ i (𝟙 j')) : (j', k j') ⟶ (j', k'')) (y j')
 : by rw [s f (𝟙 j')]; rw [ ←s (𝟙 j') (𝟙 j')], }, },
