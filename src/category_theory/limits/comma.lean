...
{ lift := λ s,
 ...
 begin
 rw [cone_of_preserves_X_hom]; rw [ assoc]; rw [ assoc]; rw [ (is_limit_of_preserves R t₂).fac]; rw [ limit_auxiliary_cone_π_app]; rw [ ←L.map_comp_assoc]; rw [ t₁.fac]; rw [ R.map_cone_π_app]; rw [ ←R.map_comp]; rw [ t₂.fac],
 exact (s.π.app j).w,
 end },
...
{ desc := λ s,
 ...
 begin
 rw [cocone_of_preserves_X_hom]; rw [ (is_colimit_of_preserves L t₁).fac_assoc]; rw [ colimit_auxiliary_cocone_ι_app]; rw [ assoc]; rw [ ←R.map_comp]; rw [ t₂.fac]; rw [ L.map_cocone_ι_app]; rw [ ←L.map_comp_assoc]; rw [ t₁.fac],
 exact (s.ι.app j).w,
 end },
 uniq' := λ s m w, comma_morphism.ext _ _
