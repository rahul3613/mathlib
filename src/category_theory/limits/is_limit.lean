...
by rw [h.hom_lift f]; rw [ h.hom_lift f']; congr; exact funext w
...
{ hom := Q.lift ((cones.equivalence_of_reindexing e.symm w').functor.obj s),
 ...
 rw [counit_app_functor]; rw [ ←functor.comp_map]; rw [ w.hom.naturality],
 simp,
 end,
...
{ lift := lift,
 fac' := λ s j, by apply G.map_injective; rw [G.map_comp]; rw [ h]; apply ht.fac,
 uniq' := λ s m w, begin
...
lemma hom_desc (h : is_colimit t) {W : C} (m : t.X ⟶ W) :
 ...
 naturality' := by intros; erw [←assoc]; erw [ t.ι.naturality]; erw [ comp_id]; erw [ comp_id] } } :=
...
by rw [h.hom_desc f]; rw [ h.hom_desc f']; congr; exact funext w
...
{ hom := P.desc ((cocones.equivalence_of_reindexing e w).functor.obj t),
 ...
 rw [counit_inv_app_functor]; rw [ ←functor.comp_map]; rw [ ←w.inv.naturality_assoc],
 dsimp,
 simp,
 end,
 inv_hom_id' := by { apply hom_ext Q, tidy, }, }
