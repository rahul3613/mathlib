...
by rw [←whisker_left_comp]; rw [ hom_inv_id]; rw [ whisker_left_id]

@[simp, reassoc]
lemma hom_inv_whisker_right {f g : a ⟶ b} (η : f ≅ g) (h : b ⟶ c) :
 η.hom ▷ h ≫ η.inv ▷ h = 𝟙 (f ≫ h) :=
by rw [←comp_whisker_right]; rw [ hom_inv_id]; rw [ id_whisker_right]

@[simp, reassoc]
lemma inv_hom_whisker_left (f : a ⟶ b) {g h : b ⟶ c} (η : g ≅ h) :
 f ◁ η.inv ≫ f ◁ η.hom = 𝟙 (f ≫ h) :=
by rw [←whisker_left_comp]; rw [ inv_hom_id]; rw [ whisker_left_id]

@[simp, reassoc]
lemma inv_hom_whisker_right {f g : a ⟶ b} (η : f ≅ g) (h : b ⟶ c) :
 η.inv ▷ h ≫ η.hom ▷ h = 𝟙 (g ≫ h) :=
by rw [←comp_whisker_right]; rw [ inv_hom_id]; rw [ id_whisker_right]
...
by { rw [←cancel_epi (f ◁ (α_ g h i).inv)]; rw [ ←cancel_mono (α_ (f ≫ g) h i).inv], simp }
...
by { rw [←cancel_epi (α_ f g (h ≫ i)).inv]; rw [ ←cancel_mono ((α_ f g h).inv ▷ i)], simp }
...
by rw [←triangle]; rw [ inv_hom_id_assoc]
...
by rw [←whisker_left_iff]; rw [ whisker_left_comp]; rw [ ←cancel_epi (α_ _ _ _).hom]; rw [ ←cancel_epi ((α_ _ _ _).hom ▷ _)]; rw [ pentagon_assoc]; rw [ triangle]; rw [ ←associator_naturality_middle]; rw [ ←comp_whisker_right_assoc]; rw [ triangle]; rw [ associator_naturality_left]; apply_instance
...
by rw [←whisker_right_iff]; rw [ comp_whisker_right]; rw [ ←cancel_epi (α_ _ _ _).inv]; rw [ ←cancel_epi (f ◁ (α_ _ _ _).inv)]; rw [ pentagon_inv_assoc]; rw [ triangle_assoc_comp_right]; rw [ ←associator_inv_naturality_middle]; rw [ ←whisker_left_comp_assoc]; rw [ triangle_assoc_comp_right]; rw [ associator_inv_naturality_right]; apply_instance
...
by rw [←whisker_left_iff]; rw [ ←cancel_epi (α_ _ _ _).hom]; rw [ ←cancel_mono (ρ_ _).hom]; rw [ triangle]; rw [ ←right_unitor_comp]; rw [ right_unitor_naturality]; apply_instance
