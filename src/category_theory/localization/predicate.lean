...
begin
 ...
 morphism_property.Q],
 erw [nat_trans.comp_app]; erw [ nat_trans.comp_app]; erw [ eq_to_hom_app]; erw [ eq_to_hom_app]; erw [ eq_to_hom_refl]; erw [ eq_to_hom_refl]; erw [ comp_id]; erw [ id_comp],
 all_goals
...
begin
 ...
 ext Y,
 rw [← cancel_epi (F₁.map (L.obj_obj_preimage_iso Y).hom)]; rw [ τ.naturality]; rw [ τ'.naturality]; rw [ h],
end
