...
by { rw [← arrow.inv_left_hom_right e.hom]; rw [ hP.cancel_left_is_iso]; rw [ hP.cancel_right_is_iso], refl }
...
begin
 ...
 rw [← hP₁.cancel_left_is_iso e.inv]; rw [ sq.flip.iso_pullback_inv_fst],
...
begin
 ...
 rw [← this]; rw [ hP.respects_iso.cancel_left_is_iso],
 exact hP.snd _ _ H,
end
...
begin
 ...
 rw [← hP₁.cancel_right_is_iso _ e.hom]; rw [ sq.flip.inr_iso_pushout_hom],
...
begin
 split,
 { introv H,
 rwa [diagonal_iff]; rwa [ pullback.diagonal_comp]; rwa [ hP.cancel_left_is_iso]; rwa [ hP.cancel_left_is_iso]; rwa [ ← hP.cancel_right_is_iso _ _]; rwa [ ← pullback.condition]; rwa [ hP.cancel_left_is_iso],
 apply_instance },
 { introv H,
 delta diagonal,
 rwa [pullback.diagonal_comp]; rwa [ hP.cancel_right_is_iso] }
end
...
begin
 introv h,
 rw [diagonal_iff]; rw [ diagonal_pullback_fst]; rw [ hP'.cancel_left_is_iso]; rw [ hP'.cancel_right_is_iso],
...
begin
 ...
 ⟨by rw [category.id_comp]; rw [ category.assoc]; rw [ e.hom_inv_id]; rw [ category.comp_id]⟩,
 replace this := this.paste_horiz H,
 rw [iso.inv_hom_id_assoc] at this; rw [ category.id_comp] at this,
 exact hf _ _ _ this },
 { intros X Y Z e f hf X' Z' i₁ i₂ f' H,
 have : is_pullback (𝟙 _) i₂ (i₂ ≫ e.inv) e.inv :=
 is_pullback.of_horiz_is_iso ⟨category.id_comp _⟩,
 replace this := H.paste_horiz this,
 rw [category.assoc] at this; rw [ iso.hom_inv_id] at this; rw [ category.comp_id] at this; rw [ category.comp_id] at this,
 exact hf _ _ _ this },
end
...
begin
 intros X Y f hf,
 exact hf (𝟙 _) (𝟙 _) _ (is_pullback.of_vert_is_iso ⟨by rw [category.comp_id]; rw [ category.id_comp]⟩)
end
