...
by { symmetry, erw [←iso.comp_hom_eq_id (e.counit_iso.app _)]; erw [ functor_unit_comp], refl }

lemma counit_app_functor (e : C ≌ D) (X : C) :
 e.counit.app (e.functor.obj X) = e.functor.map (e.unit_inv.app X) :=
by { erw [←iso.hom_comp_eq_id (e.functor.map_iso (e.unit_iso.app X))]; erw [ functor_unit_comp], refl }
...
begin
 rw [←id_comp (e.inverse.map _)]; rw [ ←map_id e.inverse]; rw [ ←counit_inv_functor_comp]; rw [ map_comp],
 dsimp,
 rw [←iso.hom_inv_id_assoc (e.unit_iso.app _) (e.inverse.map (e.functor.map _))]; rw [ app_hom]; rw [ app_inv],
 slice_lhs 2 3 { erw [e.unit.naturality] },
 slice_lhs 1 2 { erw [e.unit.naturality] },
 slice_lhs 4 4
 { rw [←iso.hom_inv_id_assoc (e.inverse.map_iso (e.counit_iso.app _)) (e.unit_inv.app _)] },
 slice_lhs 3 4 { erw [←map_comp e.inverse]; erw [ e.counit.naturality],
 erw [(e.counit_iso.app _).hom_inv_id]; erw [ map_id] }, erw [id_comp],
 slice_lhs 2 3 { erw [←map_comp e.inverse]; erw [ e.counit_iso.inv.naturality]; erw [ map_comp] },
 slice_lhs 3 4 { erw [e.unit_inv.naturality] },
 slice_lhs 4 5 { erw [←map_comp (e.functor ⋙ e.inverse)]; erw [ (e.unit_iso.app _).hom_inv_id]; erw [ map_id] },
 erw [id_comp],
 slice_lhs 3 4 { erw [←e.unit_inv.naturality] },
 slice_lhs 2 3 { erw [←map_comp e.inverse]; erw [ ←e.counit_iso.inv.naturality]; erw [ (e.counit_iso.app _).hom_inv_id]; erw [ map_id] }, erw [id_comp]; erw [ (e.unit_iso.app _).hom_inv_id], refl
end
...
by { erw [←iso.comp_hom_eq_id (e.inverse.map_iso (e.counit_iso.app Y))]; erw [ unit_inverse_comp], refl }

lemma unit_inv_app_inverse (e : C ≌ D) (Y : D) :
 e.unit_inv.app (e.inverse.obj Y) = e.inverse.map (e.counit.app Y) :=
by { symmetry, erw [←iso.hom_comp_eq_id (e.unit_iso.app _)]; erw [ unit_inverse_comp], refl }
...
{ functor := e.functor ⋙ f.functor,
 ...
 begin
 dsimp,
 rw [← f.functor.map_comp_assoc]; rw [ e.functor.map_comp]; rw [ ←counit_inv_app_functor]; rw [ fun_inv_map]; rw [ iso.inv_hom_id_app_assoc]; rw [ assoc]; rw [ iso.inv_hom_id_app]; rw [ counit_app_functor]; rw [ ← functor.map_comp],
 erw [comp_id]; erw [ iso.hom_inv_id_app]; erw [ functor.map_id],
 end }
...
{ inverse := hF.inverse,
 ...
 dsimp [nat_iso.hcomp],
 erw [id_comp]; erw [ F.map_id]; erw [ comp_id],
 apply (cancel_epi (e.hom.app X)).mp,
 slice_lhs 1 2 { rw ← e.hom.naturality, },
 slice_lhs 2 3 { rw [← nat_trans.vcomp_app']; rw [ e.hom_inv_id], },
...
{ to_fun := of_iso e,
 ...
 left_inv := λ hF, by rw [of_iso_trans]; rw [ iso.self_symm_id]; rw [ of_iso_refl],
 right_inv := λ hF, by rw [of_iso_trans]; rw [ iso.symm_self_id]; rw [ of_iso_refl], }
