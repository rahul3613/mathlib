...
begin
 ...
 { cone := fork.of_ι i
 (show i ≫ 𝟙 X = i ≫ p, by rw [comp_id]; rw [ ← h₂]; rw [ ← assoc]; rw [ h₁]; rw [ id_comp]),
 is_limit := begin
 apply fork.is_limit.mk',
 intro s,
 refine ⟨s.ι ≫ e, _⟩,
 split,
 { erw [assoc]; erw [ h₂]; erw [ ← limits.fork.condition s]; erw [ comp_id], },
 { intros m hm,
 ...
 use equalizer.lift p (show p ≫ 𝟙 X = p ≫ p, by rw [hp]; rw [ comp_id]),
 split,
 { ext,
 rw [assoc]; rw [ equalizer.lift_ι]; rw [ id_comp],
...
begin
 ...
 (by rw [← ε.inverse.map_comp]; rw [ hp]) with ⟨Y, i, e, ⟨h₁,h₂⟩⟩,
 use [ε.functor.obj Y, ε.functor.map i, ε.functor.map e],
 split,
 { rw [← ε.functor.map_comp]; rw [ h₁]; rw [ ε.functor.map_id], },
...
begin
 ...
 (by rw [← op_comp]; rw [ hp]) with ⟨Y, i, e, ⟨h₁, h₂⟩⟩,
