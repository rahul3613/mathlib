...
begin
 ...
 (by rw [comp_id]; rw [ assoc]; rw [ p.naturality φ]; rw [ ← assoc]; rw [ ← limits.equalizer.condition]; rw [ comp_id]),
...
{ preimage := λ P Q f,
 ...
 erw [← h]; erw [ assoc]; erw [ ← P.p.naturality_assoc φ]; erw [ p_comp (f.app j')],
 end },
