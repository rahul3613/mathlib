...
⟨⟨0, by rw [zero_comp]; rw [ comp_zero]⟩⟩
...
begin
 ...
 use ⟨P.X, p.f, hp'⟩,
 use ⟨p.f, by rw [comp_p p]; rw [ hp']⟩,
 use ⟨p.f, by rw [hp']; rw [ p_comp p]⟩,
...
instance [is_idempotent_complete C] : ess_surj (to_karoubi C) := ⟨λ P, begin
 ...
 { hom := ⟨i, by erw [id_comp]; erw [ ← h₂]; erw [ ← assoc]; erw [ h₁]; erw [ id_comp]⟩,
 inv := ⟨e, by erw [comp_id]; erw [ ← h₂]; erw [ assoc]; erw [ h₁]; erw [ comp_id]⟩, },
end⟩
...
@[simps]
def decomp_id_i (P : karoubi C) : P ⟶ P.X := ⟨P.p, by erw [coe_p]; erw [ comp_id]; erw [ P.idem]⟩

/-- The split epi which appears in the factorisation `decomp_id P`. -/
@[simps]
def decomp_id_p (P : karoubi C) : (P.X : karoubi C) ⟶ P :=
⟨P.p, by erw [coe_p]; erw [ id_comp]; erw [ P.idem]⟩
...
lemma decomp_id_i_naturality {P Q : karoubi C} (f : P ⟶ Q) : f ≫ decomp_id_i _ =
 decomp_id_i _ ≫ ⟨f.f, by erw [comp_id]; erw [ id_comp]⟩ :=
...
lemma decomp_id_p_naturality {P Q : karoubi C} (f : P ⟶ Q) : decomp_id_p P ≫ f =
 (⟨f.f, by erw [comp_id]; erw [ id_comp]⟩ : (P.X : karoubi C) ⟶ Q.X) ≫ decomp_id_p Q :=
