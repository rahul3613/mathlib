...
λ hS Z g hg, hS (g ≫ f) (by rwa [J₁.covers_iff]; rwa [ sieve.pullback_comp])
...
begin
 ...
 rw [← J₁.covers_iff_mem_of_closed hM]; rw [ ← J₁.covers_iff_mem_of_closed hN],
 have q : ∀ ⦃Z : C⦄ (g : Z ⟶ X) (hg : S g), M.pullback g = N.pullback g,
 { intros Z g hg,
 apply congr_arg subtype.val ((hM₂ g hg).trans (hN₂ g hg).symm) },
 have MSNS : M ⊓ S = N ⊓ S,
 { ext Z g,
 rw [sieve.inter_apply]; rw [ sieve.inter_apply]; rw [ and_comm (N g)]; rw [ and_comm],
...
{ sieves := λ X, {S | c X S = ⊤},
 ...
 rw [set.mem_set_of_eq]; rw [ hc]; rw [ hS]; rw [ sieve.pullback_top],
 end,
 transitive' := λ X S hS R hR,
 begin
 rw set.mem_set_of_eq at hS,
 rw [set.mem_set_of_eq]; rw [ ←(c X).idempotent]; rw [ eq_top_iff]; rw [ ←hS],
