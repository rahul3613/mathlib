...
begin
 ...
 erw [multiequalizer.lift_ι]; erw [ meq.equiv_symm_eq_apply],
 cases i, refl,
end
...
{ val := λ I, t I.from_middle I.to_middle,
 ...
 rw [← comp_apply]; rw [ ← comp_apply]; rw [ (J.to_plus P).naturality]; rw [ (J.to_plus P).naturality]; rw [ comp_apply]; rw [ comp_apply],
 erw [to_plus_apply (T II.fst.from_middle) (t II.fst.from_middle) II.fst.to_middle]; erw [ to_plus_apply (T II.snd.from_middle) (t II.snd.from_middle) II.snd.to_middle]; erw [ ← ht]; erw [ ← ht]; erw [ ← comp_apply]; erw [ ← comp_apply]; erw [ ← (J.plus_obj P).map_comp]; erw [ ← (J.plus_obj P).map_comp],
...
begin
 ...
 use mk w,
 ext I,
 erw [ht]; erw [ res_mk_eq_mk_pullback],
 ...
 apply inj IV.Y,
 erw [to_plus_apply (T I) (t I) ID]; erw [ to_plus_apply (T IB) (t IB) IC]; erw [ ← ht]; erw [ ← ht],
...
begin
 ...
 convert h,
 { erw [meq.equiv_apply]; erw [ ← comp_apply]; erw [ multiequalizer.lift_ι] },
 { erw [meq.equiv_apply]; erw [ ← comp_apply]; erw [ multiequalizer.lift_ι] } },
...
begin
 ...
 apply plus_lift_unique,
 rw [← category.assoc]; rw [ ← plus_map_to_plus],
 exact h,
end
...
begin
 ...
 apply J.plus_hom_ext _ _ hQ,
 rw [← category.assoc]; rw [ ← category.assoc]; rw [ ← plus_map_to_plus],
 exact h,
end
...
begin
 apply J.sheafify_lift_unique,
 rw [← category.assoc]; rw [ ← J.to_sheafify_naturality]; rw [ category.assoc]; rw [ to_sheafify_sheafify_lift],
end
...
{ map_zero' := λ F G, by { ext, erw [colimit.ι_map]; erw [ comp_zero]; erw [ J.plus_map_zero]; erw [ J.diagram_nat_trans_zero]; erw [ zero_comp] } }
