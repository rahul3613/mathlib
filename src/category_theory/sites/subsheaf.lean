...
{ arrows := λ V f, F.map f.op s ∈ G.obj (op V),
 ...
 by { rw [op_comp]; rw [ functor_to_types.map_comp_apply], exact G.map _ hi } }
...
begin
 ...
 rw [← functor_to_types.map_comp_apply]; rw [ ← functor_to_types.map_comp_apply]; rw [ ← op_comp]; rw [ ← op_comp]; rw [ e],
end
...
begin
 ...
 rw [← functor_to_types.map_comp_apply]; rw [ ← functor_to_types.map_comp_apply],
...
begin
 ...
 rintros V i hi,
 dsimp at hi,
 erw [← functor_to_types.naturality]; erw [ ← functor_to_types.naturality],
...
begin
 ...
 rw [← nat_trans.naturality]; rw [ ← nat_trans.naturality],
...
{ F := image_mono_factorization f,
 ...
 generalize_proofs h,
 rw [← subpresheaf.hom_of_le_ι h]; rw [ category.assoc],
 congr' 1,
 rw [is_iso.inv_comp_eq]; rw [ to_image_presheaf_ι],
 end } }
