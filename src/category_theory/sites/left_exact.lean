...
begin
 ...
 erw [category.comp_id]; erw [ category.comp_id]; erw [ category.assoc]; erw [ category.assoc]; erw [ ← (limit.lift F _).naturality]; erw [ ← (limit.lift F _).naturality]; erw [ ← category.assoc]; erw [ ← category.assoc],
 congr' 1, ext1,
 erw [category.assoc]; erw [ category.assoc]; erw [ limit.lift_π]; erw [ limit.lift_π]; erw [ limit.lift_π_assoc]; erw [ limit.lift_π_assoc]; erw [ category.assoc]; erw [ category.assoc]; erw [ multiequalizer.condition],
 refl,
end
...
{ lift := λ E, lift_to_diagram_limit_obj F E,
 ...
 change (_ ≫ _) ≫ _ = _,
 dsimp [evaluate_combined_cones],
 erw [category.comp_id]; erw [ category.assoc]; erw [ ← nat_trans.comp_app]; erw [ limit.lift_π]; erw [ limit.lift_π],
 refl,
 end,
 uniq' := begin
 intros E m hm,
 ...
 erw [multiequalizer.lift_ι]; erw [ category.assoc],
 change _ = (_ ≫ _) ≫ _,
 dsimp [evaluate_combined_cones],
 erw [category.comp_id]; erw [ category.assoc]; erw [ ← nat_trans.comp_app]; erw [ limit.lift_π]; erw [ limit.lift_π],
 dsimp,
...
let
 ...
 ext w,
 dsimp [plus_map],
 erw [colimit.ι_map_assoc]; erw [ colimit_obj_iso_colimit_comp_evaluation_ι_inv ((F ⋙ J.diagram_functor D X).flip) w j]; erw [ colimit_obj_iso_colimit_comp_evaluation_ι_inv_assoc ((F ⋙ J.diagram_functor D X).flip) w i],
...
begin
 ...
 rw [← (limit.is_limit (F ⋙ J.plus_functor D ⋙ (evaluation Cᵒᵖ D).obj (op X))).fac S k]; rw [ category.assoc],
 congr' 1,
 dsimp,
 simp only [category.assoc],
 rw [← iso.eq_inv_comp]; rw [ iso.inv_comp_eq]; rw [ iso.inv_comp_eq],
 ext,
 dsimp [plus_map],
...
begin
 ...
 erw [colimit.ι_map]; erw [ colimit.ι_desc_assoc]; erw [ limit.lift_π],
 dsimp,
 simp only [category.assoc],
 rw ι_colimit_limit_iso_limit_π_assoc,
 ...
 rw [← category.assoc]; rw [ ← nat_trans.comp_app]; rw [ limit.lift_π],
 refl }
end
