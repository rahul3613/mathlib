...
{ app := λ W, multiequalizer.lift _ _
 ...
 intros i,
 erw [category.assoc]; erw [ category.assoc]; erw [ ← η.naturality]; erw [ ← η.naturality]; erw [ ← category.assoc]; erw [ ← category.assoc]; erw [ multiequalizer.condition],
 refl,
 end,
...
by { ext j x, dsimp, rw [zero_comp]; rw [ multiequalizer.lift_ι]; rw [ comp_zero] }
...
{ obj := λ X, colimit (J.diagram P X.unop),
 ...
 erw [← colimit.w _ e.inv.op]; erw [ ← category.assoc],
 ...
 dsimp only [functor.op, pullback_obj],
 erw [← colimit.w _ e.inv.op]; erw [ ← category.assoc]; erw [ ← category.assoc],
 congr' 1,
 ext I,
 dsimp,
...
begin
 ...
 rw [J.diagram_nat_trans_id]; rw [ nat_trans.id_app],
 ext,
 dsimp,
 simp,
end
...
by { ext, erw [comp_zero]; erw [ colimit.ι_map]; erw [ J.diagram_nat_trans_zero]; erw [ zero_comp] }
...
begin
 ...
 { cases I, refl },
 { dsimp [cover.index],
 erw [P.map_id]; erw [ category.comp_id],
 refl }
end
...
begin
 ...
 rw [iso.eq_comp_inv]; rw [ ← hγ]; rw [ plus_map_comp],
 dsimp,
 simp,
end
...
begin
 apply J.plus_lift_unique,
 rw [← category.assoc]; rw [ ← J.to_plus_naturality]; rw [ category.assoc]; rw [ J.to_plus_plus_lift],
end

instance plus_functor_preserves_zero_morphisms [preadditive D] :
 (plus_functor J D).preserves_zero_morphisms :=
{ map_zero' := λ F G, by { ext, dsimp, rw [J.plus_map_zero]; rw [ nat_trans.app_zero] } }
