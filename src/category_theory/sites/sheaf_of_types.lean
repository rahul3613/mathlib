...
begin
 ...
 rw [←pullback.lift_fst _ _ comm]; rw [ op_comp]; rw [ functor_to_types.map_comp_apply]; rw [ t hf₁ hf₂]; rw [ ←functor_to_types.map_comp_apply]; rw [ ←op_comp]; rw [ pullback.lift_snd] }
end
...
λ h, by { rw [←extend_restrict t₁]; rw [ ←extend_restrict t₂], congr, exact h }
...
begin
 ...
 rwa [← functor_to_types.naturality]; rwa [ ← functor_to_types.naturality]; rwa [ h],
end
...
begin
 ...
 rw [←ht _ h₁]; rw [ ←ht _ h₂]; rw [ ←functor_to_types.map_comp_apply]; rw [ ←op_comp]; rw [ comm],
 simp,
end
...
begin
 ...
 rw [←ht _]; rw [ ←functor_to_types.map_comp_apply]; rw [ ←op_comp]; rw [ hf.some_spec.some_spec.some_spec.2],
end
...
begin
 rw [is_sheaf_for]; rw [ yoneda_sheaf_condition],
...
lemma is_separated_for_and_exists_is_amalgamation_iff_sheaf_for :
 ...
 is_sheaf_for P R :=
begin
 rw [is_separated_for]; rw [ ←forall_and_distrib],
 apply forall_congr,
 intro x,
 split,
...
begin
 ...
 apply (trans hj).ext,
 intros Y f hf,
 rw [←functor_to_types.map_comp_apply]; rw [ ←op_comp]; rw [ hS.valid_glue (hx.restrict h) _ hf]; rw [ family_of_elements.restrict]; rw [ ←hx (𝟙 _) f _ _ (id_comp _)],
 simp },
end
...
begin
 ...
 intros Y f,
 rw [← pullback_arrows_comm]; rw [ ← is_sheaf_for_iff_generate],
...
begin
 rw [types.type_equalizer_iff_unique]; rw [ ← equiv.forall_congr_left (first_obj_eq_family P S).to_equiv.symm],
