...
begin
 ...
 { dsimp,
 rw [← curry_natural_left]; rw [ curry_eq_iff]; rw [ uncurry_id_eq_ev]; rw [ ← ir.hom_equiv_naturality_left]; rw [ ir.hom_equiv_apply_eq]; rw [ assoc]; rw [ assoc]; rw [ prod_comparison_natural_assoc]; rw [ L.map_id]; rw [ ← prod.map_id_comp_assoc]; rw [ ir.left_triangle_components]; rw [ prod.map_id_id]; rw [ id_comp],
...
begin
 dsimp [bijection],
 rw [comp_id]; rw [ comp_id]; rw [ comp_id]; rw [ i.map_id]; rw [ comp_id]; rw [ unit_comp_partial_bijective_symm_apply]; rw [ unit_comp_partial_bijective_symm_apply]; rw [ uncurry_natural_left]; rw [ uncurry_curry]; rw [ uncurry_natural_left]; rw [ uncurry_curry]; rw [ prod.lift_map_assoc]; rw [ comp_id]; rw [ prod.lift_map_assoc]; rw [ comp_id]; rw [ prod.comp_lift_assoc]; rw [ prod.lift_snd]; rw [ prod.lift_fst_assoc]; rw [ prod.lift_fst_comp_snd_comp]; rw [ ←adjunction.eq_hom_equiv_apply]; rw [ adjunction.hom_equiv_unit]; rw [ iso.comp_inv_eq]; rw [ assoc]; rw [ preserves_limit_pair.iso_hom],
 apply prod.hom_ext,
 { rw [limits.prod.map_fst]; rw [ assoc]; rw [ assoc]; rw [ prod_comparison_fst]; rw [ ←i.map_comp]; rw [ prod_comparison_fst],
 apply (adjunction.of_right_adjoint i).unit.naturality },
 { rw [limits.prod.map_snd]; rw [ assoc]; rw [ assoc]; rw [ prod_comparison_snd]; rw [ ←i.map_comp]; rw [ prod_comparison_snd],
 apply (adjunction.of_right_adjoint i).unit.naturality },
end
...
begin
 ...
 rw [i.image_preimage]; rw [ i.map_comp]; rw [ i.image_preimage]; rw [ comp_id]; rw [ comp_id]; rw [ comp_id]; rw [ comp_id]; rw [ comp_id]; rw [ comp_id]; rw [ adjunction.hom_equiv_naturality_right]; rw [ ← assoc]; rw [ curry_natural_right _ (i.map g)]; rw [ unit_comp_partial_bijective_natural]; rw [ uncurry_natural_right]; rw [ ← assoc]; rw [ curry_natural_right]; rw [ unit_comp_partial_bijective_natural]; rw [ uncurry_natural_right]; rw [ assoc],
end
...
⟨⟨bijection i _ _ _ (𝟙 _),
 by rw [←(bijection i _ _ _).injective.eq_iff]; rw [ bijection_natural]; rw [ ← bijection_symm_apply_id]; rw [ equiv.apply_symm_apply]; rw [ id_comp],
 by rw [←bijection_natural]; rw [ id_comp]; rw [ ←bijection_symm_apply_id]; rw [ equiv.apply_symm_apply]⟩⟩
