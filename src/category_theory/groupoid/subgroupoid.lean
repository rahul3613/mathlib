...
by { rw [set_like.le_def]; rw [ sigma.forall], exact forall_congr (λ c, sigma.forall) }
...
{ arrows := λ c d, {f : c ⟶ d | φ.map f ∈ S.arrows (φ.obj c) (φ.obj d)},
 inv := λ c d p hp, by { rw [mem_set_of]; rw [ inv_eq_inv]; rw [ φ.map_inv p]; rw [ ← inv_eq_inv], exact S.inv hp },
 mul := begin
 rintros,
...
{ wide := λ c, by { rw [comap]; rw [ mem_set_of]; rw [ functor.map_id], apply Sn.wide, },
 conj := λ c d f γ hγ, by
...
{ arrows := map.arrows φ hφ S,
 ...
 cases hφ he, rw [gq]; rw [ ← eq_conj_eq_to_hom]; rw [ ← φ.map_comp],
