...
{ obj := λ X,
 ...
 assoc' := by { dsimp, rw [← R.map_comp]; rw [ ← adjunction.counit_naturality]; rw [ R.map_comp], refl } },
 map := λ X Y f,
 { f := R.map f,
 h' := by { dsimp, rw [← R.map_comp]; rw [ adjunction.counit_naturality]; rw [ R.map_comp] } } }.
...
{ obj := λ X,
 ...
 coassoc' := by { dsimp, rw [← L.map_comp]; rw [ ← adjunction.unit_naturality]; rw [ L.map_comp], refl } },
 map := λ X Y f,
 { f := L.map f,
...
begin
 ...
 rw [←cancel_epi ((adjunction.of_right_adjoint R).unit.app X.A)]; rw [ adjunction.unit_naturality_assoc]; rw [ adjunction.right_triangle_components]; rw [ comp_id],
