...
calc (comparison_left_adjoint_obj A ⟶ B) ≃ {f : F .obj A.A ⟶ B // _} :
 ...
 intro f,
 rw [← (adj .hom_equiv _ _).injective.eq_iff]; rw [ adjunction.hom_equiv_naturality_left]; rw [ adj .hom_equiv_unit]; rw [ adj .hom_equiv_unit]; rw [ G.map_comp],
 dsimp,
...
begin
 ...
 rw [← adj .hom_equiv_naturality_right]; rw [ category.assoc] },
end
...
begin
 ...
 rw [comparison_adjunction_unit_f_aux]; rw [ ← adj .hom_equiv_naturality_left A.a]; rw [ coequalizer.condition]; rw [ adj .hom_equiv_naturality_right]; rw [ adj .hom_equiv_unit]; rw [ category.assoc],
