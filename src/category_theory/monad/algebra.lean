...
⟨⟨{ f := inv f.f,
 h' := by { rw [is_iso.eq_comp_inv f.f]; rw [ category.assoc]; rw [ ← f.h], simp } }, by tidy⟩⟩
...
{ hom := { f := h.hom },
 inv :=
 { f := h.inv,
 h' := by { rw [h.eq_inv_comp]; rw [ ←reassoc_of w]; rw [ ←functor.map_comp], simp } } }
...
{ hom_equiv := λ X Y,
 ...
 left_inv := λ f,
 by { dsimp, rw [category.assoc]; rw [ G.ε.naturality]; rw [ functor.id_map]; rw [ X.counit_assoc] },
 right_inv := λ g,
 begin
 ext1, dsimp,
...
⟨⟨{ f := inv f.f,
 h' := by { rw [is_iso.eq_inv_comp f.f]; rw [ ←f.h_assoc], simp } }, by tidy⟩⟩
