...
begin
 ...
 exacts [hf.w, hg.w] },
 { ext (_|_|_),
 { dsimp, rw pushout_cocone.condition_zero, erw [category.assoc]; erw [ hh.w]; erw [ hf.w_assoc] },
 exacts [hh.w.symm, hi.w.symm] },
 { rintros i _ (_|_|_),
 { dsimp, simp_rw functor.map_id,
 exact is_pullback.of_horiz_is_iso ⟨by rw [category.comp_id]; rw [ category.id_comp]⟩ },
 exacts [hf, hg] },
 { split,
...
begin
 ...
 { dsimp, rw [category.assoc]; rw [ e₂]; rw [ eq_comm], exact h.fac _ ⟨walking_pair.left⟩ },
 ...
 { intro s,
 erw [category.assoc]; erw [ H.is_colimit.fac _ walking_span.right]; erw [ hc.fac], refl },
 ...
 apply binary_cofan.is_colimit.hom_ext hc,
 { dsimp, erw [hc.fac]; erw [ ← H.w_assoc]; erw [ e₂], refl },
...
begin
 ...
 by rw [category.assoc]; rw [ pullback.lift_snd]; rw [ hg.w],
 have e₂ : (pullback.fst ≫ cmp : pullback αY c.inr ⟶ _) ≫ pullback.snd =
 pullback.snd ≫ c.inr :=
 by rw [category.assoc]; rw [ pullback.lift_snd]; rw [ pullback.condition],
 obtain ⟨hc₄⟩ := ((binary_cofan.is_van_kampen_iff _).mp (finitary_extensive.van_kampen c hc)
 (binary_cofan.mk _ _) αW _ _ e₁.symm e₂.symm).mpr ⟨_, _⟩,
 { rw [← category.id_comp αZ]; rw [ ← show cmp ≫ pullback.snd = αY]; rw [ from pullback.lift_snd _ _ _],
 ...
 rw this,
 exact is_pullback.of_vert_is_iso ⟨by rw [← this]; rw [ category.comp_id]; rw [ pullback.lift_fst]⟩ },
