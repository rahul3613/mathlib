...
by rw [← cancel_epi ((h.add 0 n).hom.app X)]; rw [ iso.hom_inv_id_app]; rw [ h.zero_add_hom_app]; rw [ category.assoc]; rw [ ← functor.map_comp_assoc]; rw [ iso.inv_hom_id_app]; rw [ functor.map_id]; rw [ category.id_comp]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]
...
by rw [← cancel_epi ((h.add n 0).hom.app X)]; rw [ iso.hom_inv_id_app]; rw [ h.add_zero_hom_app]; rw [ category.assoc]; rw [ iso.inv_hom_id_app_assoc]; rw [ eq_to_hom_trans]; rw [ eq_to_hom_refl]
...
begin
 ...
 { rw [← cancel_mono (h.zero.inv)]; rw [ h.zero.hom_inv_id]; rw [ ← this]; rw [ iso.hom_inv_id],
 refl, },
 refl,
end
...
begin
 ...
 { rw [← cancel_mono ((h.add a b).inv)]; rw [ (h.add a b).hom_inv_id]; rw [ ← this]; rw [ iso.hom_inv_id],
 refl, },
 refl,
end
...
lemma shift_functor_add'_assoc (a₁ a₂ a₃ a₁₂ a₂₃ a₁₂₃ : A)
 ...
 shift_functor_add' C a₁₂ a₃ a₁₂₃ (by rw [← h₁₂]; rw [ h₁₂₃]) ≪≫
 iso_whisker_right (shift_functor_add' C a₁ a₂ a₁₂ h₁₂) _ ≪≫ functor.associator _ _ _ =
 shift_functor_add' C a₁ a₂₃ a₁₂₃ (by rw [← h₂₃]; rw [ ← add_assoc]; rw [ h₁₂₃]) ≪≫
...
lemma shift_functor_add'_assoc_hom_app (a₁ a₂ a₃ a₁₂ a₂₃ a₁₂₃ : A)
 ...
 (shift_functor_add' C a₁₂ a₃ a₁₂₃ (by rw [← h₁₂]; rw [ h₁₂₃])).hom.app X ≫
 ((shift_functor_add' C a₁ a₂ a₁₂ h₁₂).hom.app X)⟦a₃⟧' =
 (shift_functor_add' C a₁ a₂₃ a₁₂₃ (by rw [← h₂₃]; rw [ ← add_assoc]; rw [ h₁₂₃])).hom.app X ≫
 (shift_functor_add' C a₂ a₃ a₂₃ h₂₃).hom.app (X⟦a₁⟧) :=
...
lemma shift_functor_add'_assoc_inv_app (a₁ a₂ a₃ a₁₂ a₂₃ a₁₂₃ : A)
 ...
 (shift_functor_add' C a₁₂ a₃ a₁₂₃ (by rw [← h₁₂]; rw [ h₁₂₃])).inv.app X =
 (shift_functor_add' C a₂ a₃ a₂₃ h₂₃).inv.app (X⟦a₁⟧) ≫
 (shift_functor_add' C a₁ a₂₃ a₁₂₃ (by rw [← h₂₃]; rw [ ← add_assoc]; rw [ h₁₂₃])).inv.app X :=
...
{ functor := shift_functor C i,
 ...
 (by rw [← add_left_inj j]; rw [ add_assoc]; rw [ h]; rw [ zero_add]; rw [ add_zero])),
 functor_unit_iso_comp' := λ X, begin
 let E := equiv_of_tensor_iso_unit (shift_monoidal_functor C A) ⟨i⟩ ⟨j⟩
 (eq_to_iso (by ext; exact h))
 (eq_to_iso (by ext; dsimp; rw [← add_left_inj j]; rw [ add_assoc]; rw [ h]; rw [ zero_add]; rw [ add_zero]))
...
lemma shift_shift_functor_comp_iso_id_hom_app (n m : A) (h : n + m = 0) (X : C) :
 ...
 (by rw [← neg_eq_of_add_eq_zero_left h]; rw [ add_right_neg])).hom.app (X⟦n⟧) :=
begin
 ...
 (by rw [← neg_eq_of_add_eq_zero_left h]; rw [ add_right_neg]) (by rw [h]; rw [ zero_add]) X,
end

lemma shift_shift_functor_comp_iso_id_inv_app (n m : A) (h : n + m = 0) (X : C) :
 ((shift_functor_comp_iso_id C n m h).inv.app X)⟦n⟧' =
 ((shift_functor_comp_iso_id C m n
 (by rw [← neg_eq_of_add_eq_zero_left h]; rw [ add_right_neg])).inv.app (X⟦n⟧)) :=
begin
 rw [← cancel_mono (((shift_functor_comp_iso_id C n m h).hom.app X)⟦n⟧')]; rw [ ← functor.map_comp]; rw [ iso.inv_hom_id_app]; rw [ functor.map_id]; rw [ shift_shift_functor_comp_iso_id_hom_app]; rw [ iso.inv_hom_id_app],
 refl,
end
...
lemma shift_functor_comm_eq (i j k : A) (h : i + j = k):
 ...
 shift_functor_add' C j i k (by rw [add_comm j i]; rw [ h]) :=
begin
 subst h,
...
begin
 erw [← shift_comm_symm Y i j]; erw [ ← ((shift_functor_comm C i j).hom.naturality_assoc f)]; erw [ iso.hom_inv_id_app]; erw [ category.comp_id],
 refl,
end

@[reassoc] lemma shift_comm_hom_comp (i j : A) :
 (shift_comm X i j).hom ≫ f⟦j⟧'⟦i⟧' = f⟦i⟧'⟦j⟧' ≫ (shift_comm Y i j).hom :=
by rw [shift_comm']; rw [ ← shift_comm_symm]; rw [ iso.symm_hom]; rw [ iso.inv_hom_id_assoc]
...
begin
 rw [← shift_functor_add'_zero_add_inv_app n X]; rw [ shift_functor_comm_eq C n 0 n (add_zero n)],
 dsimp,
 rw [category.assoc]; rw [ iso.hom_inv_id_app]; rw [ category.comp_id]; rw [ shift_functor_add'_add_zero_inv_app],
end
...
begin
 rw [← cancel_mono ((shift_functor_zero C A).hom.app (X⟦n⟧))]; rw [ category.assoc]; rw [ iso.inv_hom_id_app]; rw [ shift_functor_zero_hom_app_shift]; rw [ iso.inv_hom_id_app_assoc]; rw [ ← functor.map_comp]; rw [ iso.inv_hom_id_app],
 dsimp,
 rw [functor.map_id],
end
...
begin
 ...
 (m₁ + (m₂ + m₃)) (add_comm _ _) rfl (by rw [add_comm m₂ m₁]; rw [ add_assoc]) X,
...
def has_shift_of_fully_faithful : has_shift C A := has_shift_mk C A
 ...
 dsimp at this,
 dsimp,
 rw [iso.inv_hom_id_app_assoc]; rw [ map_has_shift_of_fully_faithful_add_hom_app]; rw [ this]; rw [ eq_to_hom_map]; rw [ category.comp_id]; rw [ ← functor.map_comp]; rw [ category.assoc]; rw [ iso.inv_hom_id_app_assoc]; rw [ functor.map_comp]; rw [ functor.map_comp]; rw [ shift_functor_add_assoc_hom_app_assoc m₁ m₂ m₃ (F.obj X)],
 dsimp [shift_functor_add'],
