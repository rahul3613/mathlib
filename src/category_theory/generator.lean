...
by rw [← is_separating_op_iff]; rw [ set.unop_op]

lemma is_separating_unop_iff (𝒢 : set Cᵒᵖ) : is_separating 𝒢.unop ↔ is_coseparating 𝒢 :=
by rw [← is_coseparating_op_iff]; rw [ set.unop_op]
...
by rw [← is_codetecting_op_iff]; rw [ set.unop_op]

lemma is_codetecting_unop_iff {𝒢 : set Cᵒᵖ} : is_codetecting 𝒢.unop ↔ is_detecting 𝒢 :=
by rw [← is_detecting_op_iff]; rw [ set.unop_op]
...
by rw [is_separator]; rw [ is_coseparator]; rw [ ← is_separating_op_iff]; rw [ set.singleton_op]

lemma is_coseparator_op_iff (G : C) : is_coseparator (op G) ↔ is_separator G :=
by rw [is_separator]; rw [ is_coseparator]; rw [ ← is_coseparating_op_iff]; rw [ set.singleton_op]

lemma is_coseparator_unop_iff (G : Cᵒᵖ) : is_coseparator (unop G) ↔ is_separator G :=
by rw [is_separator]; rw [ is_coseparator]; rw [ ← is_coseparating_unop_iff]; rw [ set.singleton_unop]

lemma is_separator_unop_iff (G : Cᵒᵖ) : is_separator (unop G) ↔ is_coseparator G :=
by rw [is_separator]; rw [ is_coseparator]; rw [ ← is_separating_unop_iff]; rw [ set.singleton_unop]

lemma is_detector_op_iff (G : C) : is_detector (op G) ↔ is_codetector G :=
by rw [is_detector]; rw [ is_codetector]; rw [ ← is_detecting_op_iff]; rw [ set.singleton_op]

lemma is_codetector_op_iff (G : C) : is_codetector (op G) ↔ is_detector G :=
by rw [is_detector]; rw [ is_codetector]; rw [ ← is_codetecting_op_iff]; rw [ set.singleton_op]

lemma is_codetector_unop_iff (G : Cᵒᵖ) : is_codetector (unop G) ↔ is_detector G :=
by rw [is_detector]; rw [ is_codetector]; rw [ ← is_codetecting_unop_iff]; rw [ set.singleton_unop]

lemma is_detector_unop_iff (G : Cᵒᵖ) : is_detector (unop G) ↔ is_codetector G :=
by rw [is_detector]; rw [ is_codetector]; rw [ ← is_detecting_unop_iff]; rw [ set.singleton_unop]
...
begin
 ...
 { rw [is_iso_iff_bijective] at hf; rw [ function.bijective_iff_exists_unique] at hf,
 exact hf h },
 { suffices : is_iso ((coyoneda.obj (op G)).map f),
 { exactI @is_iso_of_reflects_iso _ _ _ _ _ _ _ (coyoneda.obj (op G)) _ h },
 rwa [is_iso_iff_bijective]; rwa [ function.bijective_iff_exists_unique] }
end
...
begin
 ...
 rwa [is_iso_iff_bijective] at hf ; rwa [ function.bijective_iff_exists_unique] at hf },
 { rw ← is_iso_op_iff,
