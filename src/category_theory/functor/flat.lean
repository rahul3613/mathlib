...
begin
 ...
 use structured_arrow.hom_mk Y.hom (by erw [functor.id_map]; erw [ category.id_comp]),
 use structured_arrow.hom_mk Z.hom (by erw [functor.id_map]; erw [ category.id_comp]) },
 { intros Y Z f g,
 use structured_arrow.mk (𝟙 _),
 use structured_arrow.hom_mk Y.hom (by erw [functor.id_map]; erw [ category.id_comp]),
 ext,
...
begin
 ...
 erw [colimit.ι_pre_assoc (Lan.diagram F H X) (costructured_arrow.map j.hom)]; erw [ category.id_comp]; erw [ category.comp_id]; erw [ colimit.ι_map],
 rcases j with ⟨j_left, ⟨⟨⟩⟩, j_hom⟩,
 congr,
 rw [costructured_arrow.map_mk]; rw [ category.id_comp]; rw [ costructured_arrow.mk]
end
