...
lemma comp_app_assoc {F G H : C ⥤ D} (α : F ⟶ G) (β : G ⟶ H) (X : C) {X' : D}
 (f : H.obj X ⟶ X') :
 (α ≫ β).app X ≫ f = α.app X ≫ β.app X ≫ f := by rw [comp_app]; rw [ assoc]
...
⟨λ H g h eq, by { ext X, rw [←cancel_mono (α.app X)]; rw [ ←comp_app]; rw [ eq]; rw [ comp_app] }⟩

/-- A natural transformation is an epimorphism if each component is. -/
lemma epi_of_epi_app (α : F ⟶ G) [∀ (X : C), epi (α.app X)] : epi α :=
⟨λ H g h eq, by { ext X, rw [←cancel_epi (α.app X)]; rw [ ←comp_app]; rw [ eq]; rw [ comp_app] }⟩
...
{ obj := λ k,
 ...
 map_comp' := λ X Y Z f g, by rw [map_comp]; rw [ ←comp_app] },
 map := λ c c' f,
