...
{ preserves := λ X Y f h,
 ...
 rw [iso.eq_inv_comp]; rw [ iso.app_hom]; rw [ iso.app_hom]; rw [ nat_trans.naturality]
 end }
...
{ reflects := λ X Y f h,
 ...
 rw [← category.assoc]; rw [ iso.eq_comp_inv]; rw [ iso.app_hom]; rw [ iso.app_hom]; rw [ nat_trans.naturality]
 end }
...
{ preserves := λ X Y f hf,
 ...
 rwa [adj.hom_equiv_naturality_left] at H; rwa [ adj.hom_equiv_naturality_left] at H; rwa [ cancel_epi] at H; rwa [ equiv.apply_eq_iff_eq] at H
 end⟩ }
...
{ preserves := λ X Y f hf,
 ...
 rwa [adj.hom_equiv_naturality_right_symm] at H; rwa [ adj.hom_equiv_naturality_right_symm] at H; rwa [ cancel_mono] at H; rwa [ equiv.apply_eq_iff_eq] at H
 end⟩ }
...
{ reflects := λ X Y f hf, ⟨λ Z g h hgh, by exactI F.map_injective ((cancel_mono (F.map f)).1
 (by rw [← F.map_comp]; rw [ hgh]; rw [ F.map_comp]))⟩ }
