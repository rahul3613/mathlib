...
begin
 ...
 { apply Inter₂_subset, },
 { rw [h]; rw [ F.map_comp], apply range_comp_subset_range, }
end

lemma is_mittag_leffler_of_surjective
 (h : ∀ ⦃i j : J⦄ (f :i ⟶ j), (F.map f).surjective) : F.is_mittag_leffler :=
λ j, ⟨j, 𝟙 j, λ k g, by rw [map_id]; rw [ types_id]; rw [ range_id]; rw [ (h g).range_eq]⟩
...
{ obj := λ j, ⋂ f : j ⟶ i, F.map f ⁻¹' s,
 ...
 rw [← mem_preimage]; rw [ preimage_preimage],
...
begin
 rw [subset_antisymm_iff]; rw [ eventual_range]; rw [ and_iff_right (Inter₂_subset _ _)]; rw [ subset_Inter₂_iff],
...
λ j, begin
 ...
 rwa [finset.lt_iff_ssubset] at this; rwa [ ← finset.coe_ssubset] at this; rwa [ set.finite.coe_to_finset] at this; rwa [ hm] at this,
end
...
by { rw [to_eventual_ranges_obj]; rw [ h], apply_instance }
