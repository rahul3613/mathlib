...
{ unit' :=
 begin
 dsimp, simp,
 conv_lhs { rw [←K.to_functor.map_comp]; rw [ α.unit], },
 end,
 tensor' := λ X Y,
 begin
 dsimp, simp,
 conv_lhs { rw [←K.to_functor.map_comp]; rw [ α.tensor]; rw [ K.to_functor.map_comp], },
 end,
...
{ hom := { app := λ X, (app X).hom, },
 inv :=
 { app := λ X, (app X).inv,
 unit' := by { dsimp, rw [←unit]; rw [ assoc]; rw [ iso.hom_inv_id]; rw [ comp_id], },
 tensor' := λ X Y,
 begin
 dsimp,
...
{ to_nat_trans := e.unit,
 ...
 erw [e.counit_app_functor]; erw [ e.counit_app_functor]; erw [ F.to_lax_monoidal_functor.μ_natural]; erw [ is_iso.inv_hom_id_assoc],
 ...
 slice_rhs 1 2 { rw [←tensor_comp]; rw [ iso.hom_inv_id_app]; rw [ iso.hom_inv_id_app],
 dsimp, rw [tensor_id], },
 simp,
 end }.
...
{ to_nat_trans := e.counit,
 ...
 erw [e.counit_app_functor]; erw [ ←e.functor.map_comp_assoc]; erw [ iso.hom_inv_id_app],
 dsimp, simp,
 end,
 tensor' := λ X Y, begin
 dsimp,
