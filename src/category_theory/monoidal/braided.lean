...
{ braiding := β,
 ...
 rw [functor.map_comp]; rw [ ←lax_monoidal_functor.μ_natural_assoc]; rw [ w]; rw [ functor.map_comp]; rw [ reassoc_of w]; rw [ braiding_naturality_assoc]; rw [ lax_monoidal_functor.μ_natural],
 end,
 ...
 rw [functor.map_comp]; rw [ functor.map_comp]; rw [ functor.map_comp]; rw [ functor.map_comp]; rw [ ←lax_monoidal_functor.μ_natural_assoc]; rw [ functor.map_id]; rw [ ←comp_tensor_id_assoc]; rw [ w]; rw [ comp_tensor_id]; rw [ category.assoc]; rw [ lax_monoidal_functor.associativity_assoc]; rw [ lax_monoidal_functor.associativity_assoc]; rw [ ←lax_monoidal_functor.μ_natural]; rw [ functor.map_id]; rw [ ←id_tensor_comp_assoc]; rw [ w]; rw [ id_tensor_comp_assoc]; rw [ reassoc_of w]; rw [ braiding_naturality_assoc]; rw [ lax_monoidal_functor.associativity]; rw [ hexagon_forward_assoc],
 end,
 ...
 rw [functor.map_comp]; rw [ functor.map_comp]; rw [ functor.map_comp]; rw [ functor.map_comp]; rw [ ←lax_monoidal_functor.μ_natural_assoc]; rw [ functor.map_id]; rw [ ←id_tensor_comp_assoc]; rw [ w]; rw [ id_tensor_comp_assoc]; rw [ lax_monoidal_functor.associativity_inv_assoc]; rw [ lax_monoidal_functor.associativity_inv_assoc]; rw [ ←lax_monoidal_functor.μ_natural]; rw [ functor.map_id]; rw [ ←comp_tensor_id_assoc]; rw [ w]; rw [ comp_tensor_id_assoc]; rw [ reassoc_of w]; rw [ braiding_naturality_assoc]; rw [ lax_monoidal_functor.associativity_inv]; rw [ hexagon_reverse_assoc],
 end, }
...
by { rw [←left_unitor_tensor]; rw [ left_unitor_naturality], simp, }
...
... = ((β_ X (𝟙_ C)).hom ⊗ (𝟙 (𝟙_ C))) ≫ (α_ _ _ _).hom ≫ (𝟙 _ ⊗ (β_ X _).hom) ≫
 ...
 : by { slice_rhs 3 4 { rw [←id_tensor_comp]; rw [ iso.hom_inv_id]; rw [ tensor_id], }, rw [id_comp], }
...
... = (α_ _ _ _).hom ≫ (𝟙 _ ⊗ (λ_ _).hom)
 : by rw [iso.hom_inv_id]; rw [ comp_id]
... = (ρ_ X).hom ⊗ (𝟙 (𝟙_ C))
 : by rw triangle

@[simp]
lemma braiding_left_unitor (X : C) : (β_ X (𝟙_ C)).hom ≫ (λ_ X).hom = (ρ_ X).hom :=
by rw [←tensor_right_iff]; rw [ comp_tensor_id]; rw [ braiding_left_unitor_aux₂]
...
by { rw [←right_unitor_tensor]; rw [ right_unitor_naturality], simp, }
...
... = ((𝟙 (𝟙_ C)) ⊗ (β_ (𝟙_ C) X).hom) ≫ (α_ _ _ _).inv ≫ ((β_ _ X).hom ⊗ 𝟙 _) ≫
 ...
 : by { slice_rhs 3 4 { rw [←comp_tensor_id]; rw [ iso.hom_inv_id]; rw [ tensor_id], }, rw [id_comp], }
...
by rw [←tensor_left_iff]; rw [ id_tensor_comp]; rw [ braiding_right_unitor_aux₂]
...
{ braided' := λ X Y,
 begin
 dsimp,
 slice_lhs 2 3 { rw [←category_theory.functor.map_comp]; rw [ F.braided]; rw [ category_theory.functor.map_comp], },
...
begin
 ...
 slice_lhs 2 3 { rw [←tensor_comp]; rw [ comp_id f₁]; rw [ ←id_comp f₁]; rw [ associator_inv_naturality]; rw [ tensor_comp] },
 slice_lhs 3 4 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ comp_id f₁]; rw [ ←id_comp f₁]; rw [ comp_id g₂]; rw [ ←id_comp g₂]; rw [ braiding_naturality]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 4 5 { rw [←tensor_comp]; rw [ comp_id f₁]; rw [ ←id_comp f₁]; rw [ associator_naturality]; rw [ tensor_comp] },
...
begin
 ...
 slice_rhs 1 2 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ comp_id]; rw [ comp_id]; rw [ left_unitor_inv_braiding] },
 simp only [assoc],
 coherence,
end
...
begin
 ...
 slice_rhs 1 3 { rw this }, clear this,
 slice_rhs 2 3 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ comp_id]; rw [ comp_id]; rw [ right_unitor_inv_braiding] },
 simp only [assoc],
 coherence,
end
...
lemma tensor_associativity_aux (W X Y Z : C) :
 ...
 (α_ X (Z ⊗ W) Y).hom :=
begin
 slice_rhs 3 5 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ hexagon_forward]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_rhs 5 6 { rw [associator_naturality] },
 slice_rhs 2 3 { rw [←associator_inv_naturality] },
 slice_rhs 3 5 { rw [←pentagon_hom_inv] },
 slice_rhs 1 2 { rw [tensor_id]; rw [ id_tensor_comp_tensor_id]; rw [ ←tensor_id_comp_id_tensor] },
 slice_rhs 2 3 { rw [← tensor_id]; rw [ associator_naturality] },
 slice_rhs 3 5 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←hexagon_reverse]; rw [ tensor_comp]; rw [ tensor_comp] },
end
...
begin
 ...
 slice_lhs 4 5 { rw [←tensor_id]; rw [ associator_naturality] },
 slice_lhs 5 6 { rw [←tensor_comp]; rw [ associator_inv_naturality]; rw [ tensor_comp] },
 ...
 slice_lhs 1 3 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ tensor_μ_def₁]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 3 4 { rw [←tensor_id]; rw [ associator_inv_naturality] },
 slice_lhs 4 5 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 5 6 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 6 10 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ tensor_id]; rw [ tensor_associativity_aux]; rw [ ←tensor_id]; rw [ ←id_comp (𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁)]; rw [ ←id_comp (𝟙 Z₂ ≫ 𝟙 Z₂ ≫ 𝟙 Z₂ ≫ 𝟙 Z₂ ≫ 𝟙 Z₂)]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 11 12 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ iso.hom_inv_id],
 simp },
 simp only [assoc, id_comp],
 slice_lhs 10 11 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ iso.hom_inv_id],
 simp },
 simp only [assoc, id_comp],
 slice_lhs 9 10 { rw [associator_naturality] },
 slice_lhs 10 11 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 11 13 { rw [tensor_id]; rw [ ←tensor_μ_def₂] },
 have :
 ...
 slice_lhs 7 8 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 8 9 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 9 10 { rw [associator_inv_naturality] },
 slice_lhs 10 12 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_μ_def₂]; rw [ tensor_comp]; rw [ tensor_comp] },
 dsimp,
 coherence,
end
...
begin
 ...
 slice_lhs 3 4 { rw [←id_comp (𝟙 X₂)]; rw [ tensor_comp] },
...
begin
 ...
 slice_lhs 3 4 { rw [←id_comp (𝟙 X₁)]; rw [ tensor_comp]; rw [ id_comp] },
 slice_lhs 3 4 { rw [←tensor_comp]; rw [ ←right_unitor_naturality]; rw [ tensor_comp] },
 coherence,
end
...
begin
 ...
 slice_rhs 3 5 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ hexagon_reverse]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_rhs 5 6 { rw associator_naturality },
 slice_rhs 6 7 { rw [tensor_id]; rw [ tensor_id_comp_id_tensor]; rw [ ←id_tensor_comp_tensor_id] },
 slice_rhs 2 3 { rw ←associator_inv_naturality },
 slice_rhs 3 5 { rw pentagon_inv_inv_hom },
 slice_rhs 4 5 { rw [←tensor_id]; rw [ ←associator_inv_naturality] },
 slice_rhs 2 4 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←hexagon_forward]; rw [ tensor_comp]; rw [ tensor_comp] },
 simp,
end
...
begin
 ...
 rw this, clear this,
 slice_lhs 2 4 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ tensor_μ_def₁]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 4 5 { rw [←tensor_id]; rw [ associator_inv_naturality] },
 slice_lhs 5 6 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 6 7 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp]; rw [ tensor_comp] },
 have :
 ...
 slice_lhs 3 4 { rw [←tensor_id]; rw [ associator_naturality] },
 slice_lhs 4 5 { rw [←tensor_comp]; rw [ associator_inv_naturality]; rw [ tensor_comp] },
 slice_lhs 5 6 { rw associator_inv_naturality },
 slice_lhs 6 9 { rw [←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ tensor_id]; rw [ associator_monoidal_aux]; rw [ ←id_comp (𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁)]; rw [ ←id_comp (𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁ ≫ 𝟙 X₁)]; rw [ ←id_comp (𝟙 Y₃ ≫ 𝟙 Y₃ ≫ 𝟙 Y₃ ≫ 𝟙 Y₃)]; rw [ ←id_comp (𝟙 Y₃ ≫ 𝟙 Y₃ ≫ 𝟙 Y₃ ≫ 𝟙 Y₃ ≫ 𝟙 Y₃)]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 11 12 { rw associator_naturality },
 slice_lhs 12 13 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 13 14 { rw [←tensor_comp]; rw [ ←tensor_id]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 14 15 { rw associator_inv_naturality },
 slice_lhs 15 17 { rw [tensor_id]; rw [ ←tensor_comp]; rw [ ←tensor_comp]; rw [ ←tensor_μ_def₂]; rw [ tensor_comp]; rw [ tensor_comp] },
 have :
 ...
 slice_lhs 9 10 { rw [←tensor_comp]; rw [ associator_naturality]; rw [ tensor_comp] },
 slice_lhs 10 12 { rw [tensor_id]; rw [ ←tensor_μ_def₂] },
 dsimp,
 coherence,
end
