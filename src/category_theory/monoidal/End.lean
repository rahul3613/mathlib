...
{ ε := (right_unitor_nat_iso C).inv,
 ...
 left_unitality' := λ X, by { ext Y, dsimp, rw [category.id_comp]; rw [ triangle]; rw [ ←tensor_comp], simp, },
 right_unitality' := λ X,
 begin
 ext Y, dsimp,
 rw [tensor_id]; rw [ category.comp_id]; rw [ right_unitor_tensor_inv]; rw [ category.assoc]; rw [ iso.inv_hom_id_assoc]; rw [ ←id_tensor_comp]; rw [ iso.inv_hom_id]; rw [ tensor_id],
 end,
...
begin
 rw [← is_iso.comp_inv_eq]; rw [ category.assoc]; rw [ ← is_iso.eq_inv_comp],
 simp,
end
...
begin
 ...
 rw [← category.assoc]; rw [ ← is_iso.comp_inv_eq]; rw [ ← is_iso.comp_inv_eq]; rw [ category.assoc],
...
begin
 rw [← is_iso.inv_comp_eq]; rw [ ← is_iso.comp_inv_eq],
 simp,
end
