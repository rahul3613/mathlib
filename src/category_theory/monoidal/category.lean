...
{ hom := f.hom ⊗ g.hom,
 inv := f.inv ⊗ g.inv,
 hom_inv_id' := by rw [←tensor_comp]; rw [ iso.hom_inv_id]; rw [ iso.hom_inv_id]; rw [ ←tensor_id],
 inv_hom_id' := by rw [←tensor_comp]; rw [ iso.inv_hom_id]; rw [ iso.inv_hom_id]; rw [ ←tensor_id] }
...
by rw [←right_unitor_naturality_assoc]; rw [ iso.hom_inv_id]; rw [ category.comp_id]

@[simp]
lemma left_unitor_conjugation {X Y : C} (f : X ⟶ Y) :
 ((𝟙 (𝟙_ C)) ⊗ f) = (λ_ X).hom ≫ f ≫ (λ_ Y).inv :=
by rw [←left_unitor_naturality_assoc]; rw [ iso.hom_inv_id]; rw [ category.comp_id]
...
by
 rw [←tensor_right_iff]; rw [ comp_tensor_id]; rw [ ←cancel_mono (α_ X Y (𝟙_ C)).hom]; rw [ assoc]; rw [ associator_naturality]; rw [ ←triangle_assoc]; rw [ ←triangle]; rw [ id_tensor_comp]; rw [ pentagon_assoc]; rw [ ←associator_naturality]; rw [ tensor_id]
...
by rw [←triangle]; rw [ iso.inv_hom_id_assoc]
