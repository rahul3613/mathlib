...
{ map_zero' := A.one.map_zero,
 ...
 rwa [monoidal_category.left_unitor_hom_apply] at h; rwa [ ←A.one.map_smul] at h,
 end,
 commutes' := λ r a,
 begin dsimp,
...
{ X := Module.of R A,
 ...
 rw [linear_map.mul'_apply]; rw [ monoidal_category.left_unitor_hom_apply]; rw [ ← algebra.smul_def]
 end,
 mul_one' :=
 begin
 ext x,
 ...
 Algebra.coe_comp],
 rw [linear_map.mul'_apply]; rw [ Module.monoidal_category.right_unitor_hom_apply]; rw [ ← algebra.commutes]; rw [ ← algebra.smul_def]
 end,
 mul_assoc' :=
 begin
 ext x y z,
 ...
 simp only [linear_map.mul'_apply, mul_assoc]
 end }
