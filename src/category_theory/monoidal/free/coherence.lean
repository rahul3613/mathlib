...
begin
 ...
 cases n, simp },
 { dsimp,
 rw [←(iso.inv_comp_eq _).2 (right_unitor_tensor _ _)]; rw [ category.assoc]; rw [ ←right_unitor_naturality],
 ...
 ext, refl, },
 { dsimp at *,
 rw [id_tensor_comp]; rw [ category.assoc]; rw [ f_ih_g ⟦f_g⟧]; rw [ ←category.assoc]; rw [ f_ih_f ⟦f_f⟧]; rw [ category.assoc]; rw [ ←functor.map_comp],
 congr' 2 },
 { dsimp at *,
 rw associator_inv_naturality_assoc,
 slice_lhs 2 3 { rw [←tensor_comp]; rw [ f_ih_f ⟦f_f⟧] },
 conv_lhs { rw [←@category.id_comp (F C) _ _ _ ⟦f_g⟧] },
 simp only [category.comp_id, tensor_comp, category.assoc],
 congr' 2,
 rw [←mk_tensor]; rw [ quotient.lift_mk],
 dsimp,
 rw [functor.map_comp]; rw [ ←category.assoc]; rw [ ←f_ih_g ⟦f_g⟧]; rw [ ←@category.comp_id (F C) _ _ _ ⟦f_g⟧]; rw [ ←category.id_comp ((discrete.functor inclusion_obj).map _)]; rw [ tensor_comp],
 dsimp,
...
nat_iso.of_components
 ...
 intros X Y f,
 dsimp,
 rw [left_unitor_inv_naturality_assoc]; rw [ category.assoc]; rw [ iso.cancel_iso_inv_left],
...
λ _ _,
 ...
 begin
 rw [←functor.id_map f]; rw [ ←functor.id_map g],
