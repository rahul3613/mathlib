...
{ obj := λ X, F.obj X ⊗ G.obj X,
 ...
 map_id' := λ X, by rw [F.map_id]; rw [ G.map_id]; rw [ tensor_id],
 map_comp' := λ X Y Z f g, by rw [F.map_comp]; rw [ G.map_comp]; rw [ tensor_comp], }
...
{ app := λ X, α.app X ⊗ β.app X,
 naturality' :=
 λ X Y f, by { dsimp, rw [←tensor_comp]; rw [ α.naturality]; rw [ β.naturality]; rw [ tensor_comp], } }
