...
by rw [←id_tensor_comp_tensor_id]; rw [ category.assoc]; rw [ M.one_mul]; rw [ left_unitor_naturality]

@[simp] lemma mul_one_hom {Z : C} (f : Z ⟶ M.X) : (f ⊗ M.one) ≫ M.mul = (ρ_ Z).hom ≫ f :=
by rw [←tensor_id_comp_id_tensor]; rw [ category.assoc]; rw [ M.mul_one]; rw [ right_unitor_naturality]
...
{ obj := λ A,
 ...
 one_mul' :=
 begin
 conv_lhs { rw [comp_tensor_id]; rw [ ←F.to_functor.map_id], },
 slice_lhs 2 3 { rw [F.μ_natural], },
 slice_lhs 3 4 { rw [←F.to_functor.map_comp]; rw [ A.one_mul], },
 rw [F.to_functor.map_id],
 rw [F.left_unitality],
 end,
 mul_one' :=
 begin
 conv_lhs { rw [id_tensor_comp]; rw [ ←F.to_functor.map_id], },
 slice_lhs 2 3 { rw [F.μ_natural], },
 slice_lhs 3 4 { rw [←F.to_functor.map_comp]; rw [ A.mul_one], },
 rw [F.to_functor.map_id],
 ...
 conv_lhs { rw [comp_tensor_id]; rw [ ←F.to_functor.map_id], },
 slice_lhs 2 3 { rw [F.μ_natural], },
 slice_lhs 3 4 { rw [←F.to_functor.map_comp]; rw [ A.mul_assoc], },
 conv_lhs { rw [F.to_functor.map_id] },
 conv_lhs { rw [F.to_functor.map_comp]; rw [ F.to_functor.map_comp] },
 conv_rhs { rw [id_tensor_comp]; rw [ ←F.to_functor.map_id], },
 ...
 map := λ A B f,
 { hom := F.map f.hom,
 one_hom' := by { dsimp, rw [category.assoc]; rw [ ←F.to_functor.map_comp]; rw [ f.one_hom], },
 mul_hom' :=
 begin
 dsimp,
 rw [category.assoc]; rw [ F.μ_natural_assoc]; rw [ ←F.to_functor.map_comp]; rw [ ←F.to_functor.map_comp]; rw [ f.mul_hom],
 end },
...
{ obj := λ A,
 ...
 map := λ A B f,
 { app := λ _, f.hom,
 naturality' := λ _ _ _, by { dsimp, rw [category.id_comp]; rw [ category.comp_id], },
 unit' := f.one_hom,
...
begin
 simp,
 slice_lhs 1 3 { rw [←category.id_comp P.one]; rw [ tensor_comp] },
 slice_lhs 2 3 { rw associator_naturality },
 slice_rhs 1 2 { rw [←category.id_comp M.one]; rw [ tensor_comp] },
...
by { slice_lhs 2 3 { rw [right_unitor_naturality]; rw [ ←unitors_equal] }, simp }
...
begin
 ...
 slice_lhs 2 3 { rw [←tensor_id]; rw [ tensor_μ_natural] },
 slice_lhs 3 4 { rw [←tensor_comp]; rw [ one_mul M]; rw [ one_mul N] },
 symmetry,
...
begin
 rw [←category.id_comp (𝟙 (M.X ⊗ N.X))]; rw [ tensor_comp],
 slice_lhs 2 3 { rw [←tensor_id]; rw [ tensor_μ_natural] },
 slice_lhs 3 4 { rw [←tensor_comp]; rw [ mul_one M]; rw [ mul_one N] },
 symmetry,
...
begin
 rw [←category.id_comp (𝟙 (M.X ⊗ N.X))]; rw [ tensor_comp],
 slice_lhs 2 3 { rw [←tensor_id]; rw [ tensor_μ_natural] },
 slice_lhs 3 4 { rw [←tensor_comp]; rw [ mul_assoc M]; rw [ mul_assoc N]; rw [ tensor_comp]; rw [ tensor_comp] },
 slice_lhs 1 3 { rw [tensor_associativity] },
 slice_lhs 3 4 { rw [←tensor_μ_natural] },
 slice_lhs 2 3 { rw [←tensor_comp]; rw [ tensor_id] },
 simp only [category.assoc],
end
...
begin
 simp,
 slice_lhs 2 3 { rw [←category.id_comp P.mul]; rw [ tensor_comp] },
 slice_lhs 3 4 { rw [associator_naturality] },
 slice_rhs 3 4 { rw [←category.id_comp M.mul]; rw [ tensor_comp] },
...
{ tensor_obj := λ M N,
 ...
 begin
 dsimp,
 slice_lhs 2 3 { rw [←tensor_comp]; rw [ hom.one_hom f]; rw [ hom.one_hom g] },
 end,
 mul_hom' :=
 begin
 dsimp,
 slice_rhs 1 2 { rw [tensor_μ_natural] },
 slice_lhs 2 3 { rw [←tensor_comp]; rw [ hom.mul_hom f]; rw [ hom.mul_hom g]; rw [ tensor_comp] },
 simp only [category.assoc],
 end },
