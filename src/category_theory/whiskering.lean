...
{ app := λ X, α.app (F.obj X),
 naturality' := λ X Y f, by rw [functor.comp_map]; rw [ functor.comp_map]; rw [ α.naturality] }
...
{ app := λ X, F.map (α.app X),
 naturality' := λ X Y f,
 by rw [functor.comp_map]; rw [ functor.comp_map]; rw [ ←F.map_comp]; rw [ ←F.map_comp]; rw [ α.naturality] }
...
{ obj := λ F,
 ...
 naturality' := λ X Y f, begin dsimp, rw [←H.map_comp]; rw [ ←H.map_comp]; rw [ ←τ.naturality] end },
