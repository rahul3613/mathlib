...
begin
 ...
 rw [←kernel.lift_ι g u hu]; rw [ category.assoc]; rw [ h.2]; rw [ has_zero_morphisms.comp_zero] },
 { tidy },
 { intros, rw [←cancel_mono (image_subobject f).arrow]; rw [ w],
 simp, } }
end
...
begin
 ...
 rw [is_iso.comp_left_eq_zero (kernel_comparison g F)]; rw [ ← category.assoc]; rw [ is_iso.comp_right_eq_zero _ (cokernel_comparison f F)],
end
...
begin
 ...
 (by rw [← cancel_epi h]; rw [ ← category.assoc]; rw [ cokernel_cofork.condition]; rw [ comp_zero]) rfl,
 refine (exact_iff' _ _ (limit.is_limit _) hc).2 ⟨_, ((exact_iff _ _).1 hfg).2⟩,
 exact zero_of_epi_comp h (by rw [← hfg.1]; rw [ category.assoc])
end
...
begin
 ...
 rw [←cokernel.π_desc f u hu]; rw [ ←category.assoc]; rw [ h.2]; rw [ has_zero_morphisms.zero_comp] },
 tidy
end
...
⟨λ e, by rwa [← f.op_unop] at e; rwa [ ← g.op_unop] at e; rwa [ ← exact.op_iff] at e, λ e, @@exact.unop _ _ g f e⟩
...
begin
 ...
 rw [fork.ι_of_ι]; rw [ cofork.π_of_π]; rw [ ← L.map_comp]; rw [ kernel_comp_cokernel _ _ e1]; rw [ L.map_zero]
end
