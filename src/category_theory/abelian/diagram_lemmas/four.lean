...
mono_of_zero_of_map_zero _ $ λ c hc,
 ...
 calc δ (h c) = h' (γ c) : by rw [←comp_apply]; rw [ ←comm₃]; rw [ comp_apply]
 ... = h' 0 : by rw hc
 ... = 0 : apply_zero _,
 exists.elim ((pseudo_exact_of_exact hgh).2 _ this) $ λ b hb,
 have g' (β b) = 0, from
 calc g' (β b) = γ (g b) : by rw [←comp_apply]; rw [ comm₂]; rw [ comp_apply]
 ... = γ c : by rw hb
 ... = 0 : hc,
 ...
 calc β (f a) = f' (α a) : by rw [←comp_apply]; rw [ ←comm₁]; rw [ comp_apply]
 ... = f' a' : by rw ha
 ... = β b : ha',
 calc c = g b : hb.symm
 ... = g (f a) : by rw this
...
preadditive.epi_of_cancel_zero _ $ λ R r hβr,
 ...
 calc γ ≫ z ≫ v = h ≫ δ ≫ w : by rw [←category.assoc]; rw [ pushout.condition]; rw [ category.assoc]
 ... = γ ≫ h' ≫ w : by rw reassoc_of comm₃,
 suffices (r ≫ y) ≫ v = 0, by exactI zero_of_comp_mono _ (zero_of_comp_mono _ this),
 calc (r ≫ y) ≫ v = g' ≫ z ≫ v : by rw [pushout.condition]; rw [ category.assoc]
 ... = g' ≫ h' ≫ w : by rw hzv
 ... = 0 ≫ w : hg'h'.w_assoc _
 ... = 0 : has_zero_morphisms.zero_comp _ _
