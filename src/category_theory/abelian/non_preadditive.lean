...
begin
 ...
 ... = 0 ≫ l : by rw [←category.assoc]; rw [ kernel.condition]
 ... = 0 : zero_comp,
 -- i factors through u = ker h via some s.
 obtain ⟨s, hs⟩ := normal_mono.lift' u i hih,
 have hs' : (s ≫ kernel.ι g) ≫ i = 𝟙 I ≫ i, by rw [category.assoc]; rw [ hs]; rw [ category.id_comp],
...
begin
 ...
 ... = 0 ≫ t : by rw [←category.assoc]; rw [ hu.w]
 ... = 0 : zero_comp,
 -- h factors through the kernel of f via some l.
 obtain ⟨l, hl⟩ := kernel.lift' f h hf,
 have hhp : h ≫ p = 0, calc
 h ≫ p = (l ≫ kernel.ι f) ≫ p : hl ▸ rfl
 ... = l ≫ 0 : by rw [category.assoc]; rw [ cokernel.condition]
 ... = 0 : comp_zero,
 -- p factors through u = coker h via some s.
 obtain ⟨s, hs⟩ := normal_epi.desc' u p hhp,
 have hs' : p ≫ cokernel.π g ≫ s = p ≫ 𝟙 I, by rw [←category.assoc]; rw [ hs]; rw [ category.comp_id],
...
begin
 ...
 have hyy : y = 0,
 { erw [←category.comp_id y]; erw [ ←limits.prod.lift_snd (𝟙 A) (𝟙 A)]; erw [ ←category.assoc]; erw [ hy]; erw [ category.assoc]; erw [ prod.lift_snd]; erw [ has_zero_morphisms.comp_zero] },
...
by rw [cokernel.condition_assoc]; rw [ zero_comp]

@[simp, reassoc] lemma lift_σ {X : C} : prod.lift (𝟙 X) 0 ≫ σ = 𝟙 X :=
by rw [←category.assoc]; rw [ is_iso.hom_inv_id]
...
cokernel.cokernel_iso _ σ (as_iso (r X)).symm (by rw [iso.symm_hom]; rw [ as_iso_inv])
...
begin
 ...
 suffices hfg : f = g,
 { rw [←hg]; rw [ cofork.π_of_π]; rw [ hfg] },
 calc f = f ≫ prod.lift (𝟙 Y) 0 ≫ σ : by rw [lift_σ]; rw [ category.comp_id]
 ... = prod.lift (𝟙 X) 0 ≫ limits.prod.map f f ≫ σ : by rw lift_map_assoc
 ... = prod.lift (𝟙 X) 0 ≫ σ ≫ g : by rw [←hg]; rw [ cokernel_cofork.π_of_π]
 ... = g : by rw [←category.assoc]; rw [ lift_σ]; rw [ category.id_comp]
end
...
by rw [sub_def]; rw [ ←category.comp_id a]; rw [ ← prod.comp_lift]; rw [ category.assoc]; rw [ diag_σ]; rw [ comp_zero]
...
begin
 simp only [sub_def],
 ext,
 { rw [category.assoc]; rw [ σ_comp]; rw [ prod.lift_map_assoc]; rw [ prod.lift_fst]; rw [ prod.lift_fst]; rw [ prod.lift_fst] },
 { rw [category.assoc]; rw [ σ_comp]; rw [ prod.lift_map_assoc]; rw [ prod.lift_snd]; rw [ prod.lift_snd]; rw [ prod.lift_snd] }
end

lemma sub_sub_sub {X Y : C} (a b c d : X ⟶ Y) : (a - c) - (b - d) = (a - b) - (c - d) :=
begin
 rw [sub_def]; rw [ ←lift_sub_lift]; rw [ sub_def]; rw [ category.assoc]; rw [ σ_comp]; rw [ prod.lift_map_assoc], refl
end

lemma neg_sub {X Y : C} (a b : X ⟶ Y) : (-a) - b = (-b) - a :=
by conv_lhs { rw [neg_def]; rw [ ←sub_zero b]; rw [ sub_sub_sub]; rw [ sub_zero]; rw [ ←neg_def] }
...
by rw [add_def]; rw [ neg_sub']; rw [ add_neg]

lemma sub_add {X Y : C} (a b c : X ⟶ Y) : (a - b) + c = a - (b - c) :=
by rw [add_def]; rw [ neg_def]; rw [ sub_sub_sub]; rw [ sub_zero]
...
by rw [add_def]; rw [ neg_def]; rw [ sub_self]; rw [ sub_zero]

lemma comp_sub {X Y Z : C} (f : X ⟶ Y) (g h : Y ⟶ Z) : f ≫ (g - h) = f ≫ g - f ≫ h :=
by rw [sub_def]; rw [ ←category.assoc]; rw [ prod.comp_lift]; rw [ sub_def]

lemma sub_comp {X Y Z : C} (f g : X ⟶ Y) (h : Y ⟶ Z) : (f - g) ≫ h = f ≫ h - g ≫ h :=
by rw [sub_def]; rw [ category.assoc]; rw [ σ_comp]; rw [ ←category.assoc]; rw [ prod.lift_map]; rw [ sub_def]

lemma comp_add (X Y Z : C) (f : X ⟶ Y) (g h : Y ⟶ Z) : f ≫ (g + h) = f ≫ g + f ≫ h :=
by rw [add_def]; rw [ comp_sub]; rw [ neg_def]; rw [ comp_sub]; rw [ comp_zero]; rw [ add_def]; rw [ neg_def]

lemma add_comp (X Y Z : C) (f g : X ⟶ Y) (h : Y ⟶ Z) : (f + g) ≫ h = f ≫ h + g ≫ h :=
by rw [add_def]; rw [ sub_comp]; rw [ neg_def]; rw [ sub_comp]; rw [ zero_comp]; rw [ add_def]; rw [ neg_def]
