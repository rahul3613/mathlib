...
begin
 ...
 rw [←homology_functor_map]; rw [ homotopy_category.homology_functor_map_factors],
...
preadditive.exact_of_iso_of_exact' (F.map (P.ι.f 0)) (F.map (P.cocomplex.d 0 1)) _ _
 ...
 (by simp)
 (by rw [iso.refl_hom]; rw [ category.id_comp]; rw [ iso.symm_hom]; rw [ homological_complex.d_from_eq]; congr')
...
homology.lift _ _ _ (F.map (P.ι.f 0) ≫ cokernel.π _) begin
 ...
 rw [category.assoc]; rw [ cokernel.π_desc]; rw [ homological_complex.d_from_eq _ this]; rw [ map_homological_complex_obj_d]; rw [ ← category.assoc]; rw [ ← functor.map_comp],
...
begin
 ...
 rw [← category.assoc]; rw [ iso.comp_inv_eq]; rw [ category.id_comp]; rw [ category.assoc]; rw [ category.assoc]; rw [ ← iso.eq_inv_comp]; rw [ iso.inv_hom_id],
 ext,
 rw [category.assoc]; rw [ category.assoc]; rw [ homology.lift_ι]; rw [ category.id_comp]; rw [ homology.π'_ι]; rw [ category.assoc]; rw [ ←category.assoc _ _ (cokernel.π _)]; rw [ abelian.kernel.lift.inv]; rw [ ← category.assoc]; rw [ ← category.assoc _ (kernel.ι _)]; rw [ limits.kernel.lift_ι]; rw [ category.assoc]; rw [ category.assoc]; rw [ ← category.assoc (homology_iso_kernel_desc _ _ _).hom _ _]; rw [ ← homology.ι]; rw [ ←category.assoc]; rw [ homology.π'_ι]; rw [ category.assoc]; rw [ ←category.assoc (cokernel.π _)]; rw [ cokernel.π_desc]; rw [ whisker_eq],
...
begin
 dsimp [right_derived_zero_to_self_app, right_derived_zero_to_self_app_inv],
 rw [← category.assoc _ (F.right_derived_obj_iso 0 P).hom]; rw [ category.assoc _ _ (F.right_derived_obj_iso 0 P).hom]; rw [ iso.inv_hom_id]; rw [ category.comp_id]; rw [ ← category.assoc]; rw [ ← category.assoc]; rw [ is_iso.comp_inv_eq]; rw [ category.id_comp],
 ext,
 simp only [limits.kernel.lift_ι_assoc, category.assoc, limits.kernel.lift_ι, homology.lift],
 rw [← category.assoc]; rw [ ← category.assoc]; rw [ category.assoc _ _ (homology_iso_kernel_desc _ _ _).hom],
 simp,
end
...
begin
 ...
 rw [iso.comp_inv_eq]; rw [ right_derived_map_eq F 0 f (InjectiveResolution.desc f Q P) (by simp)]; rw [ category.assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ iso.inv_hom_id]; rw [ category.comp_id]; rw [ ← category.assoc (F.right_derived_obj_iso 0 P).inv]; rw [ iso.inv_hom_id]; rw [ category.id_comp],
 dsimp only [homology_functor_map],
 ext,
 rw [category.assoc]; rw [ homology.lift_ι]; rw [ category.assoc]; rw [ homology.map_ι]; rw [ ←category.assoc (homology.lift _ _ _ _ _) _ _]; rw [ homology.lift_ι]; rw [ category.assoc]; rw [ cokernel.π_desc]; rw [ ←category.assoc]; rw [ ← functor.map_comp]; rw [ ← category.assoc]; rw [ homological_complex.hom.sq_from_left]; rw [ map_homological_complex_map_f]; rw [ ← functor.map_comp]; rw [ show f ≫ Q.ι.f 0 = P.ι.f 0 ≫ (InjectiveResolution.desc f Q P).f 0]; rw [ from homological_complex.congr_hom (InjectiveResolution.desc_commutes f Q P).symm 0],
end
