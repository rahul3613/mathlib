...
{ hom := (kernel.lift f.op (cokernel.π f).op $ by simp [← op_comp]).unop,
 ...
 by { rw [← f.unop_op]; rw [ ← unop_comp]; rw [ f.unop_op], simp },
 hom_inv_id' := begin
...
by rw [iso.comp_inv_eq]; rw [ image_ι_op_comp_image_unop_op_hom]

lemma image_unop_op_inv_comp_op_factor_thru_image :
 (image_unop_op g).inv ≫ (factor_thru_image g.unop).op = image.ι g :=
by rw [iso.inv_comp_eq]; rw [ image_unop_op_hom_comp_image_ι]
