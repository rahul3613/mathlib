...
{ normal_mono_of_mono := λ X Y f m,
 ...
 rw [← cancel_mono f]; rw [ hg]; rw [ ← aux]; rw [ kernel_fork.ι_of_ι], },
 { intro A,
 simp only [kernel_fork.ι_of_ι, category.assoc],
 convert limit.lift_π _ _ using 2,
 rw [is_iso.inv_comp_eq]; rw [ eq_comm],
...
{ normal_epi_of_epi := λ X Y f m,
 ...
 rw [← cancel_epi f]; rw [ hg]; rw [ ← aux]; rw [ cokernel_cofork.π_of_π], },
 { intro A,
 simp only [cokernel_cofork.π_of_π, ← category.assoc],
 convert colimit.ι_desc _ _ using 2,
 rw [is_iso.comp_inv_eq]; rw [ is_iso.comp_inv_eq]; rw [ eq_comm]; rw [ ←image_mono_factorisation_e'],
 exact (image_mono_factorisation f).fac, }
 end }, }
...
by rw [biprod.lift_desc]; rw [ comp_neg]; rw [ pullback.condition]; rw [ add_right_neg]
...
fork.is_limit.mk _
 ...
 sub_eq_zero.1 $ by rw [category.assoc]; rw [ category.assoc]; rw [ ←comp_sub]; rw [ sub_eq_add_neg]; rw [ ←comp_neg]; rw [ ←biprod.desc_eq]; rw [ kernel_fork.condition s])
 (λ s,
 begin
 ext; rw [fork.ι_of_ι]; rw [ category.assoc],
 { rw [biprod.lift_fst]; rw [ pullback.lift_fst] },
 { rw [biprod.lift_snd]; rw [ pullback.lift_snd] }
 end)
...
by rw [biprod.lift_desc]; rw [ neg_comp]; rw [ pushout.condition]; rw [ add_right_neg]
...
cofork.is_colimit.mk _
 ...
 sub_eq_zero.1 $ by rw [←category.assoc]; rw [ ←category.assoc]; rw [ ←sub_comp]; rw [ sub_eq_add_neg]; rw [ ←neg_comp]; rw [ ←biprod.lift_eq]; rw [ cofork.condition s]; rw [ zero_comp])
 (λ s, by ext; simp)
 (λ s m h, by ext; simp [←h] )
...
begin
 ...
 ... = biprod.inl ≫ u : by rw [category.assoc]; rw [ hd]
...
begin
 ...
 ... = biprod.inr ≫ u : by rw [category.assoc]; rw [ hd]
...
begin
 ...
 ... = u ≫ biprod.fst : by rw [←category.assoc]; rw [ hd]
...
begin
 ...
 ... = u ≫ biprod.snd : by rw [←category.assoc]; rw [ hd]
