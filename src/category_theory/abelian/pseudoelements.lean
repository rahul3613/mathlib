...
by { unfold app, rw [←category.assoc]; rw [ over.coe_hom] }
...
⟨λ ⟨R, p, q, ep, eq, comm⟩, by exactI zero_of_epi_comp p (by simp [comm]),
 ...
 by rw [hf]; rw [ over.coe_hom]; rw [ has_zero_morphisms.comp_zero]; rw [ has_zero_morphisms.comp_zero]⟩⟩
...
by { rw [pseudo_zero_def]; rw [ pseudo_apply_mk], simp }
...
λ qbar, quotient.induction_on qbar $ λ q, ⟨((pullback.fst : pullback f q.hom ⟶ P) : over P),
 ...
 by apply_instance, by rw [category.id_comp]; rw [ ←pullback.condition]; rw [ app_hom]; rw [ over.coe_hom]⟩⟩
...
λ h, match h (𝟙 Q) with ⟨pbar, hpbar⟩ :=
 ...
 by { dsimp at comm, rw [category.assoc]; rw [ comm], apply category.comp_id }
 end
 end
end
...
⟨λ a, by { rw [←comp_apply]; rw [ h.w], exact zero_apply _ _ },
 ...
 : by rw [←category.assoc]; rw [ pullback.condition]
 ... = pullback.snd ≫ b.hom
...
λ ⟨h₁, h₂⟩, (abelian.exact_iff _ _).2 ⟨zero_morphism_ext _ $ λ a, by rw [comp_apply]; rw [ h₁ a],
begin
...
begin
 ...
 refine ⟨q a'', _⟩,
 rw [← linear_map.comp_apply]; rw [ ← Module.comp_def]; rw [ ← H]; rw [ Module.comp_def]; rw [ linear_map.comp_apply]; rw [ ha'']; rw [ ha'] },
 { obtain ⟨a', ha'⟩ := ha,
