...
begin
 rw [kernel.lift_ι_assoc]; rw [ homological_complex.d_to_eq _ (by simp : (complex_shape.down ℕ).rel 1 0)]; rw [ map_homological_complex_obj_d]; rw [ category.assoc]; rw [ ← functor.map_comp],
 simp
end
...
begin
 ...
 rw [← category.assoc]; rw [ ← category.assoc]; rw [ ← category.assoc]; rw [ iso.comp_inv_eq]; rw [ category.id_comp]; rw [ category.assoc]; rw [ category.assoc]; rw [ category.assoc],
 convert category.comp_id _,
 rw [← category.assoc]; rw [ ← category.assoc]; rw [ iso.comp_inv_eq]; rw [ category.id_comp],
 ext,
 rw [← category.assoc]; rw [ ← category.assoc]; rw [ homology.π'_desc']; rw [ category.assoc]; rw [ category.assoc]; rw [ ← category.assoc (F.map _)]; rw [ abelian.cokernel.desc.inv]; rw [ cokernel.π_desc]; rw [ homology.π']; rw [ category.assoc]; rw [ iso.inv_hom_id]; rw [ category.comp_id]; rw [ ← category.assoc],
 convert category.id_comp _ using 2,
 ext,
 rw [category.id_comp]; rw [ category.assoc]; rw [ equalizer_as_kernel]; rw [ kernel.lift_ι]; rw [ category.comp_id],
end
...
begin
 dsimp [left_derived_zero_to_self_app, left_derived_zero_to_self_app_inv],
 rw [category.assoc]; rw [ category.assoc]; rw [ category.assoc]; rw [ ← category.assoc (F.left_derived_obj_iso 0 P).inv]; rw [ iso.inv_hom_id]; rw [ category.id_comp]; rw [ is_iso.inv_comp_eq]; rw [ category.comp_id],
 ext,
 simp only [cokernel.π_desc_assoc, category.assoc, cokernel.π_desc, homology.desc'],
 rw [← category.assoc]; rw [ ← category.assoc (homology_iso_cokernel_lift _ _ _).inv]; rw [ iso.inv_hom_id]; rw [ category.id_comp],
...
begin
 ...
 rw [functor.left_derived_map_eq F 0 f (ProjectiveResolution.lift f P Q) (by simp)]; rw [ category.assoc]; rw [ category.assoc]; rw [ ← category.assoc _ (F.left_derived_obj_iso 0 Q).hom]; rw [ iso.inv_hom_id]; rw [ category.id_comp]; rw [ category.assoc]; rw [ whisker_eq],
