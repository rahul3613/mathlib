...
begin
 ...
 contrapose! h,
 obtain ⟨rfl, h⟩ := h,
 rw [heq_iff_eq] at h; rw [ psigma.mk.inj_iff] at h,
 exact ⟨rfl, h.1.symm⟩ }, },
end
...
{ cocone_objs := λ X Y,
 ...
 by rw [← h.hom_equiv_naturality_left]; rw [ ← h.hom_equiv_naturality_left]; rw [ coeq_condition]⟩,
...
by rw [coeq₃_hom]; rw [ reassoc_of (coeq_condition f g)]

lemma coeq₃_condition₂ {j₁ j₂ : C} (f g h : j₁ ⟶ j₂) :
 g ≫ coeq₃_hom f g h = h ≫ coeq₃_hom f g h :=
begin
 dsimp [coeq₃_hom],
 slice_lhs 2 4 { rw [← category.assoc]; rw [ coeq_condition _ _] },
 slice_rhs 2 4 { rw [← category.assoc]; rw [ coeq_condition _ _] },
 slice_lhs 1 3 { rw [← category.assoc]; rw [ coeq_condition _ _] },
...
{ cone_objs := λ X Y,
 ...
 by rw [← h.hom_equiv_naturality_right_symm]; rw [ ← h.hom_equiv_naturality_right_symm]; rw [ eq_condition]⟩,
