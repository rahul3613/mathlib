...
def is_van_kampen_colimit.is_colimit {F : J ⥤ C} {c : cocone F} (h : is_van_kampen_colimit c) :
 is_colimit c :=
begin
 refine ((h c (𝟙 F) (𝟙 c.X : _) (by rw [functor.map_id]; rw [ category.comp_id]; rw [ category.id_comp])
 (nat_trans.equifibered_of_is_iso _)).mpr $ λ j, _).some,
 haveI : is_iso (𝟙 c.X) := infer_instance,
 exact is_pullback.of_vert_is_iso ⟨by erw [nat_trans.id_app]; erw [ category.comp_id]; erw [ category.id_comp]⟩,
end
...
begin
 ...
 split,
 { rw [← category.id_comp αX]; rw [ ← iso.hom_inv_id_assoc e f],
 ...
 { rw [← category.id_comp αY]; rw [ ← iso.hom_inv_id_assoc e f],
...
begin
 ...
 (by rw [← reassoc_of hX]; rw [ hd]; rw [ category.assoc])
 (by rw [← reassoc_of hY]; rw [ hd']; rw [ category.assoc]),
...
begin
 rw [finitary_extensive_iff_of_is_terminal (Type u) punit types.is_terminal_punit _ (types.binary_coproduct_colimit _ _)],
...
begin
 rw [finitary_extensive_iff_of_is_terminal Top.{u} _ Top.is_terminal_punit _ (Top.binary_cofan_is_colimit _ _)],
