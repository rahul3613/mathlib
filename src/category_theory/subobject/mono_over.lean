...
λ f g,
 ...
 intros h₁ h₂,
 ext1,
 erw [← cancel_mono g.arrow]; erw [ over.w h₁]; erw [ over.w h₂],
 end⟩
...
def iso_mk {f g : mono_over X} (h : f.obj.left ≅ g.obj.left) (w : h.hom ≫ g.arrow = f.arrow) :
 f ≅ g :=
{ hom := hom_mk h.hom w,
 inv := hom_mk h.inv (by rw [h.inv_comp_eq]; rw [ w]) }
...
begin
 ...
 rw [← cancel_mono (Z₂.arrow ≫ f)]; rw [ assoc]; rw [ assoc]; rw [ w_assoc g]; rw [ image_mono_iso_source_hom_self]; rw [ image_mono_iso_source_hom_self],
 apply image.lift_fac,
end
