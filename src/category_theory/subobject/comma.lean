...
subobject.ind _ $ λ P f hf, ⟨S.map (subobject.underlying_iso _).unop.inv ≫ P.unop.hom,
 by { dsimp, rw [← category.assoc]; rw [ ← S.map_comp]; rw [ ← unop_comp], simp }⟩
...
begin
 ...
 rw [← unop_comp]; rw [ subobject.underlying_iso_hom_comp_eq_mk]
end
...
{ to_fun := λ P, ⟨project_quotient P, project_quotient_factors P⟩,
 ...
 rw [← category.assoc]; rw [ ← S.map_comp]; rw [ unop_left_comp_of_mk_le_mk_unop],
 dsimp,
 simp },
