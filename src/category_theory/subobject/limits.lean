...
by rw [←equalizer_subobject_arrow]; rw [ category.assoc]; rw [ category.assoc]; rw [ equalizer.condition]
...
⟨λ w, by rw [←subobject.factor_thru_arrow _ _ w]; rw [ category.assoc]; rw [ equalizer_subobject_arrow_comp]; rw [ category.assoc],
...
⟨λ w, by rw [←subobject.factor_thru_arrow _ _ w]; rw [ category.assoc]; rw [ kernel_subobject_arrow_comp]; rw [ comp_zero],
...
{ to_fun := subobject.lift (λ A f hf, subobject.mk (cokernel.π f).op)
 ...
 { rw [← subobject.of_mk_le_mk_comp h]; rw [ category.assoc]; rw [ cokernel.condition]; rw [ comp_zero] },
...
{ to_fun := subobject.lift (λ A f hf, subobject.mk (kernel.ι f.unop))
 ...
 { rw [← subobject.of_mk_le_mk_comp h]; rw [ unop_comp]; rw [ kernel.condition_assoc]; rw [ zero_comp] },
...
begin
 ...
 erw [image.map_ι]; erw [ ←category.assoc]; erw [ image_subobject_arrow],
end
...
by rw [←iso.comp_inv_eq]; rw [ category.assoc]; rw [ ←(image_subobject_iso (arrow.mk f).hom).eq_inv_comp]; rw [ ←image_map_comp_image_subobject_iso_inv]; refl
